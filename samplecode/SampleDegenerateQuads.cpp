...
static void draw_extended_line(SkCanvas* canvas, const SkPaint paint, const SkPoint& p0,
                               const SkPoint& p1) {
    SkVector v = p1 - p0;
...
static void make_aa_line(const SkPoint& p0, const SkPoint& p1, bool aaOn, bool outset,
                         SkPoint line[2]) {
    SkVector n = {0.f, 0.f};
    if (aaOn) {
        SkVector v = p1 - p0;
...
public:
    DegenerateQuadSample(const SkRect& rect)
            : fOuterRect(rect), fCoverageMode(CoverageMode::kArea) {
            ...
            for (SkScalar py = covRect.fTop; py < covRect.fBottom; py += 1.f) {
                for (SkScalar px = covRect.fLeft; px < covRect.fRight; px += 1.f) {
                    ...
                    if (fCoverageMode == CoverageMode::kArea) {
                        ...
                        coverage = get_framed_coverage(gpuOutset, gpuOutsetCoverage, gpuInset,
                                                       gpuInsetCoverage, gpuDomain, pixelCenter);
                    }
...
private:
    ...
    void getTessellatedPoints(SkPoint inset[4], SkScalar insetCoverage[4], SkPoint outset[4],
        ...
        float vertices[56];  // 2 quads, with x, y, coverage, and geometry domain (7 floats x 8
                             // vert)
