...
static void draw_extended_line(
    SkCanvas* canvas, const SkPaint paint, const SkPoint& p0, const SkPoint& p1) {
  SkVector v = p1 - p0;
...
static void make_aa_line(
    const SkPoint& p0, const SkPoint& p1, bool aaOn, bool outset, SkPoint line[2]) {
  SkVector n = {0.f, 0.f};
  if (aaOn) {
    SkVector v = p1 - p0;
...
static SkScalar get_area_coverage(
    const bool edgeAA[4], const SkPoint corners[4], const SkPoint& point) {
  SkPath shape;
...
static SkScalar get_edge_dist_coverage(
    const bool edgeAA[4], const SkPoint corners[4], const SkPoint outsetLines[8],
    const SkPoint insetLines[8], const SkPoint& point) {
  bool flip = false;
...
static bool inside_triangle(
    const SkPoint& point, const SkPoint& t0, const SkPoint& t1, const SkPoint& t2,
    SkScalar bary[3]) {
...
static SkScalar get_framed_coverage(
    const SkPoint outer[4], const SkScalar outerCoverages[4], const SkPoint inner[4],
    const SkScalar innerCoverages[4], const SkRect& geomDomain, const SkPoint& point) {
  // Triangles are ordered clock wise. Indices >= 4 refer to inner[i - 4]. Otherwise its outer[i].
  static const int kFrameTris[] = {0, 1, 4, 4, 1, 5, 1, 2, 5, 5, 2, 6, 2, 3, 6,
                                   6, 3, 7, 3, 0, 7, 7, 0, 4, 4, 5, 7, 7, 5, 6};
...
class DegenerateQuadSample : public Sample {
 public:
  DegenerateQuadSample(const SkRect& rect) : fOuterRect(rect), fCoverageMode(CoverageMode::kArea) {
  ...
  void onDrawContent(SkCanvas* canvas) override {
    ...
    if (valid) {
      ...
      SkRect gpuDomain;
      this->getTessellatedPoints(
          gpuInset, gpuInsetCoverage, gpuOutset, gpuOutsetCoverage, &gpuDomain);
      ...
      for (SkScalar py = covRect.fTop; py < covRect.fBottom; py += 1.f) {
        for (SkScalar px = covRect.fLeft; px < covRect.fRight; px += 1.f) {
          ...
          if (fCoverageMode == CoverageMode::kArea) {
            ...
            coverage = get_framed_coverage(
                gpuOutset, gpuOutsetCoverage, gpuInset, gpuInsetCoverage, gpuDomain, pixelCenter);
          }
 ...
 private:
  ...
  enum class CoverageMode { kArea, kEdgeDistance, kGPUMesh };
