...
static sk_sp<SkImage> zoom_up(SkSurface* origSurf, SkImage* orig) {
  ...
  canvas->scale(S, S);
  canvas->translate(
      -SkScalarHalf(orig->width()) * (S - D) / S, -SkScalarHalf(orig->height()) * (S - D) / S);
...
class FilterQualityView : public Sample {
 ...
 protected:
  ...
  void drawTheImage(
      SkCanvas* canvas, const SkISize& size, SkFilterQuality filter, SkScalar dx, SkScalar dy) {
    SkPaint paint;
    ...
    canvas->drawImage(
        fImage.get(), -SkScalarHalf(fImage->width()), -SkScalarHalf(fImage->height()), &paint);
  ...
  void drawHere(SkCanvas* canvas, SkFilterQuality filter, SkScalar dx, SkScalar dy) {
    ...
    if (fShowFatBits) {
      ...
      SkImageInfo info =
          SkImageInfo::MakeN32(fImage->width() * 2, fImage->height() * 2, kOpaque_SkAlphaType);
      ...
      canvas->translate(
          SkScalarHalf(fCell.width() - fImage->width()),
    ...
    if (surface) {
      ...
      origCanvas->drawImage(
          zoomed.get(), SkScalarHalf(fCell.width() - zoomed->width()),
