...
class ShadowsView : public Sample {
  ...
  void onOnceBeforeDraw() override {
    ...
    fFunkyRRPath.addRoundRect(
        SkRect::MakeXYWH(-50, -50, SK_Scalar1 * 100, SK_Scalar1 * 100), 40 * SK_Scalar1,
    ...
    fStarPath.lineTo(-43.30f, -25.0f);
    fStarPath.lineTo(-14.43f, -25.0f);
    fSquareRRectPath.addRRect(SkRRect::MakeRectXY(SkRect::MakeXYWH(-50, -50, 100, 100), 10, 10));
  ...
  void drawShadowedPath(
      ...
      SkScalar ambientAlpha, const SkPoint3& lightPos, SkScalar lightWidth, SkScalar spotAlpha) {
    ...
    SkShadowUtils::DrawShadow(
        canvas, path, zPlaneParams, lightPos, lightWidth, ambientColor, spotColor, flags);
  ...
  void onDrawContent(SkCanvas* canvas) override {
    ...
    this->drawShadowedPath(
        canvas, fRRPath, zPlaneParams, paint, fAnimAlpha * kAmbientAlpha, lightPos, kLightWidth,
        fAnimAlpha * kSpotAlpha);

    paint.setColor(SK_ColorRED);
    canvas->translate(250, 0);
    zPlaneParams.fZ = std::max(1.0f, 8 + fZDelta);
    this->drawShadowedPath(
        canvas, fRectPath, zPlaneParams, paint, fAnimAlpha * kAmbientAlpha, lightPos, kLightWidth,
        fAnimAlpha * kSpotAlpha);
    ...
    this->drawShadowedPath(
        canvas, fCirclePath, zPlaneParams, paint, fAnimAlpha * kAmbientAlpha, lightPos, kLightWidth,
        fAnimAlpha * 0.5f);
    ...
    zPlaneParams.fZ = std::max(1.0f, 8 + fZDelta);
    this->drawShadowedPath(
        canvas, fFunkyRRPath, zPlaneParams, paint, fAnimAlpha * kAmbientAlpha, lightPos,
        kLightWidth, fAnimAlpha * kSpotAlpha);

    paint.setColor(SK_ColorCYAN);
    canvas->translate(250, 0);
    zPlaneParams.fZ = std::max(1.0f, 16 + fZDelta);
    this->drawShadowedPath(
        canvas, fCubicPath, zPlaneParams, paint, fAnimAlpha * kAmbientAlpha, lightPos, kLightWidth,
        fAnimAlpha * kSpotAlpha);
    ...
    zPlaneParams.fZ = std::max(1.0f, 8 + fZDelta);
    this->drawShadowedPath(
        canvas, fStarPath, zPlaneParams, paint, kAmbientAlpha, lightPos, kLightWidth, kSpotAlpha);

    paint.setColor(SK_ColorWHITE);
    canvas->translate(150, 0);
    zPlaneParams.fZ = std::max(1.0f, 2 + fZDelta);
    this->drawShadowedPath(
        canvas, fNotchPath, zPlaneParams, paint, kAmbientAlpha, lightPos, kLightWidth, kSpotAlpha);

    paint.setColor(SK_ColorWHITE);
    canvas->translate(200, 0);
    zPlaneParams.fZ = std::max(1.0f, 16 + fZDelta);
    this->drawShadowedPath(
        canvas, fTabPath, zPlaneParams, paint, kAmbientAlpha, lightPos, kLightWidth, kSpotAlpha);
    ...
    this->drawShadowedPath(canvas, tmpPath, zPlaneParams, paint, .1f, lightPos, kLightWidth, .5f);
    ...
    // perspective paths
    SkPoint pivot = SkPoint::Make(
        fWideRectPath.getBounds().width() / 2, fWideRectPath.getBounds().height() / 2);
    ...
    zPlaneParams = SkPoint3::Make(
        0, SkScalarSin(radians), std::max(1.0f, 16 + fZDelta) - SkScalarSin(radians) * pivot.fY);
    this->drawShadowedPath(
        canvas, fWideRectPath, zPlaneParams, paint, .1f, lightPos, kLightWidth, .5f);

    pivot = SkPoint::Make(
        fWideOvalPath.getBounds().width() / 2, fWideOvalPath.getBounds().height() / 2);
    ...
    canvas->setMatrix(persp);
    zPlaneParams = SkPoint3::Make(
        -SkScalarSin(radians), 0, std::max(1.0f, 32 + fZDelta) + SkScalarSin(radians) * pivot.fX);
    this->drawShadowedPath(
        canvas, fWideOvalPath, zPlaneParams, paint, .1f, lightPos, kLightWidth, .5f);

    pivot = SkPoint::Make(fStarPath.getBounds().width() / 2, fStarPath.getBounds().height() / 2);
    ...
    canvas->setMatrix(persp);
    zPlaneParams = SkPoint3::Make(
        -SkScalarSin(radians), 0, std::max(1.0f, 8 + fZDelta) + SkScalarSin(radians) * pivot.fX);
