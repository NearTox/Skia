...
struct VSphere {
  ...
  bool contains(SkVec2 v) const { return (v - fCenter).length() <= fRadius; }
...
class SampleRR3D : public Sample3DView {
  SkRRect fRR;
  Light fLight = {{60, 60}, {60, 60}, 50, 10};
  ...
  void onOnceBeforeDraw() override {
    ...
    fShader = GetResourceAsImage("images/mandrill_128.png")->makeShader(SkMatrix::MakeScale(3, 3));
  }

  bool onChar(SkUnichar uni) override { return this->Sample3DView::onChar(uni); }
  ...
  bool onClick(Click* click) override {
    ...
    if (click->fMeta.hasS32("type", 1)) {
      if (click->fState == skui::InputState::kUp) {
        ...
        fClickRotation = fSphere.computeRotation(
