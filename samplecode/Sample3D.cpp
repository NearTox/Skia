...
struct VSphere {
  ...
  bool contains(SkV2 v) const { return (v - fCenter).length() <= fRadius; }
...
class Sample3DView : public Sample {
 ...
 public:
  void concatCamera(SkCanvas* canvas, const SkRect& area, SkScalar zscale) {
    SkM44 camera = Sk3LookAt(fEye, fCOA, fUp), perspective = Sk3Perspective(fNear, fFar, fAngle),
...
struct Face {
  ...
  static SkM44 T(SkScalar x, SkScalar y, SkScalar z) { return SkM44::Translate(x, y, z); }

  static SkM44 R(SkV3 axis, SkScalar rad) { return SkM44::Rotate(axis, rad); }
...
struct LightOnSphere {
  ...
  SkV3 computeWorldPos(const VSphere& s) const { return s.computeUnitV3(fLoc) * fDistance; }
...
class RotateAnimator {
  SkV3 fAxis = {0, 0, 0};
  SkScalar fAngle = 0, fPrevAngle = 1234567;
  double fNow = 0, fPrevNow = 0;

  SkScalar fAngleSpeed = 0, fAngleSign = 1;
...
class SampleCubeBase : public Sample3DView {
 ...
 public:
  SampleCubeBase(Flags flags) : fSphere({200 + DX, 200 + DY}, 400), fFlags(flags) {}
  ...
  void onDrawContent(SkCanvas* canvas) override {
    ...
    if (fFlags & kShowLightDome) {
      ...
      canvas->drawLine(
          fSphere.fCenter.x, fSphere.fCenter.y - fSphere.fRadius, fSphere.fCenter.x,
          fSphere.fCenter.y + fSphere.fRadius, paint);
      canvas->drawLine(
          fSphere.fCenter.x - fSphere.fRadius, fSphere.fCenter.y,
    ...
    if (click->fMeta.hasS32("type", 1)) {
      if (click->fState == skui::InputState::kUp) {
        ...
        auto [axis, angle] = fSphere.computeRotationInfo(
            {click->fOrig.fX, click->fOrig.fY}, {click->fCurr.fX, click->fCurr.fY});
  ...
  bool onAnimate(double nanos) override { return fRotateAnimator.isAnimating(); }
 ...
 public:
  ...
  void onOnceBeforeDraw() override {
    ...
    for (int i = 0; i < 65; ++i) {
      ...
      SkScalar s = SkScalarSin(t), c = SkScalarCos(t);
...
class SampleSkottieCube : public SampleCubeBase {
 ...
 public:
  ...
  void onOnceBeforeDraw() override {
    const char* files[] = {
        "skottie/skottie-chained-mattes.json",  "skottie/skottie-gradient-ramp.json",
        "skottie/skottie_sample_2.json",        "skottie/skottie-3d-3planes.json",
        "skottie/skottie-text-animator-4.json", "skottie/skottie-motiontile-effect-phase.json",
