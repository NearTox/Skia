 ...
 public:
  ...
  void draw(SkCanvas* canvas) {
    ...
    SkTextUtils::DrawString(
        canvas, fLabel.c_str(), fRect.centerX(), fRect.fTop + 0.68f * fRect.height(), font, paint,
...
class CircDrawable : public ModeDrawable {
 ...
 public:
  CircDrawable(SkScalar size, SkColor c) {
    ...
    fPaint.setShader(SkGradientShader::MakeRadial(
        SkPoint::Make(size / 2, size / 2), size / 2, colors, nullptr, 2, SkTileMode::kClamp));
...
class CubicResamplerDemo : public Sample {
  struct Rec {
    ...
    void draw(SkCanvas* canvas, SkImage::CubicResampler cubic) const {
      ...
      SkMatrix lm = SkMatrix::Translate(r.x(), r.y()) * SkMatrix::Scale(10, 10);
      ...
      paint.setShader(fImage->makeShader(
          SkTileMode::kClamp, SkTileMode::kClamp, {SkSamplingMode::kLinear, SkMipmapMode::kNone},
          &lm));
      canvas->drawRect(r, paint);
 ...
 protected:
  ...
  void onDrawContent(SkCanvas* canvas) override {
    ...
    canvas->drawSimpleText(
        str.c_str(), str.size(), SkTextEncoding::kUTF8, fDomain.fLeft + 10, fDomain.fBottom + 40,
        font, paint);
  }
