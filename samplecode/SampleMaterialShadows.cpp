...
class MaterialShadowsView : public Sample {
  ...
  void drawShadowedPath(
      ...
      SkScalar ambientAlpha, const SkPoint3& lightPos, SkScalar lightRadius, SkScalar spotAlpha) {
    uint32_t flags = 0;
    ...
    SkShadowUtils::DrawShadow(
        canvas, path, zPlaneParams, lightPos, lightRadius, ambientColor, spotColor, flags);
  ...
  void onDrawContent(SkCanvas* canvas) override {
    ...
    for (unsigned int i = 0; i < SK_ARRAY_COUNT(elevations); ++i) {
      ...
      this->drawShadowedPath(
          canvas, fCirclePath, zPlaneParams, paint, kAmbientAlpha, lightPos, kLightRadius,
          kSpotAlpha);
    ...
    for (unsigned int i = 0; i < SK_ARRAY_COUNT(elevations); ++i) {
      zPlaneParams.fZ = elevations[i];
      this->drawShadowedPath(
          canvas, fCapsulePath, zPlaneParams, paint, kAmbientAlpha, lightPos, kLightRadius,
          kSpotAlpha);
    ...
    for (unsigned int i = 0; i < SK_ARRAY_COUNT(elevations); ++i) {
      zPlaneParams.fZ = elevations[i];
      this->drawShadowedPath(
          canvas, fLargeRRPath, zPlaneParams, paint, kAmbientAlpha, lightPos, kLightRadius,
          kSpotAlpha);
      canvas->translate(160, 0);
    }
    canvas->restore();
    ...
    for (unsigned int i = 0; i < SK_ARRAY_COUNT(elevations); ++i) {
      zPlaneParams.fZ = elevations[i];
      this->drawShadowedPath(
          canvas, fSmallRRPath, zPlaneParams, paint, kAmbientAlpha, lightPos, kLightRadius,
          kSpotAlpha);
      canvas->translate(160, 0);
    }
    canvas->restore();
