...
static void draw_atlas(
    ...
    const SkColor colors[], int count, const SkRect* cull, const SkPaint* paint) {
...
static void draw_atlas_sim(
  ...
  for (int i = 0; i < count; ++i) {
    ...
    canvas->drawImageRect(
        atlas, tex[i], tex[i].makeOffset(-tex[i].x(), -tex[i].y()), paint,
  ...
  DrawShipView(const char name[], DrawAtlasProc proc) : fName(name), fProc(proc) {
    ...
    for (int x = 0; x < kGrid; x++) {
      for (int y = 0; y < kGrid; y++) {
        ...
        fTex[currIndex] = SkRect::MakeLTRB(
            0.0f, 0.0f, SkIntToScalar(fAtlas->width()), SkIntToScalar(fAtlas->height()));
        fXform[currIndex] =
            SkRSXform::MakeFromRadians(0.1f, SK_ScalarPI * 0.5f, xPos, yPos, anchorX, anchorY);
        currIndex++;
      }
    }
    fTex[currIndex] = SkRect::MakeLTRB(
        0.0f, 0.0f, SkIntToScalar(fAtlas->width()), SkIntToScalar(fAtlas->height()));
    fXform[currIndex] = SkRSXform::MakeFromRadians(
        0.5f, SK_ScalarPI * 0.5f, kWidth * 0.5f, kHeight * 0.5f, anchorX, anchorY);
  }
