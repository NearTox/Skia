...
static SkColor floats_to_color(const SkScalar f[4]) {
  return SkColorSetARGB(
      SkScalarRoundToInt(f[0]), SkScalarRoundToInt(f[1]), SkScalarRoundToInt(f[2]),
...
class HTDrawable : public SkDrawable {
 ...
 public:
  HTDrawable(SkRandom& rand) {
    fR = SkRect::MakeXYWH(
        rand.nextRangeF(0, 640), rand.nextRangeF(0, 480), rand.nextRangeF(20, 200),
        rand.nextRangeF(20, 200));
