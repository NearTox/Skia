...
class RectRenderer : public ShapeRenderer {
 public:
  ...
  void draw(SkCanvas* canvas, SkPaint* paint, SkScalar tx, SkScalar ty, SkScalar angle) override {
    ...
    canvas->drawRect(
        SkRect::MakeLTRB(
            kTileWidth / 2.f - width / 2.f, 2.f, kTileWidth / 2.f + width / 2.f, kTileHeight - 2.f),
        *paint);
  }
...
class PathRenderer : public ShapeRenderer {
 public:
  ...
  void draw(SkCanvas* canvas, SkPaint* paint, SkScalar tx, SkScalar ty, SkScalar angle) override {
    ...
    if (fDepth > 0.f) {
      path.quadTo(
          kTileWidth / 2.f + fDepth, kTileHeight / 2.f, kTileWidth / 2.f, kTileHeight - 2.f);
    } else {
      if (fDepth < 0.f) {
...
class OffscreenShapeRenderer : public ShapeRenderer {
 public:
  ...
  static sk_sp<OffscreenShapeRenderer> Make(
      sk_sp<ShapeRenderer> renderer, int supersample, bool forceRaster = false) {
  ...
  void prepareBuffer(SkCanvas* canvas, SkPaint* paint, SkScalar tx, SkScalar ty, SkScalar angle) {
    auto info = SkImageInfo::Make(
        fSupersampleFactor * kTileWidth, fSupersampleFactor * kTileHeight, kRGBA_8888_SkColorType,
        kPremul_SkAlphaType);
    auto surface = fForceRasterBackend ? SkSurface::MakeRaster(info) : canvas->makeSurface(info);
  ...
  void redraw(SkCanvas* canvas, SkScalar scale = 1.f, bool debugMode = false) {
    ...
    if (debugMode) {
      ...
      static constexpr float kFilter[] = {0.f,        0.f, 0.f,         0.f, 16.f / 255, 0.f, 0.f,
                                          0.f,        0.f, 200.f / 255, 0.f, 0.f,        0.f, 0.f,
                                          16.f / 255, 0.f, 0.f,         0.f, 255.f,      0.f};
...
class ThinAASample : public Sample {
 ...
 protected:
  ...
  void onDrawContent(SkCanvas* canvas) override {
    ...
    canvas->drawString(
        SkStringPrintf("Stroke width: %.2f ('-' to decrease, '=' to increase)", fStrokeWidth), 0,
        24, font, text);
    canvas->drawString(
        SkStringPrintf(
            "Rotation: %.3f ('r' to animate, 'y' sets to 90, 'u' sets"
            " to 0, 'space' adds 15)",
            fAngle),
        0, 36, font, text);
    canvas->drawString(
        SkStringPrintf("Translation: %.3f, %.3f ('t' to animate)", fSubpixelX, fSubpixelY), 0, 48,
        font, text);
 ...
 private:
  ...
  void drawShapes(
      SkCanvas* canvas, const char* name, int gridX,
  ...
  void drawShape(
      SkCanvas* canvas, const char* name, int gridX, OffscreenShapeRenderer* shape,
      bool drawNameLabels) {
