...
static void set_path_pt(int index, const SkPoint& pt, SkPath* path) {
  ...
  while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
    switch (verb) {
      case SkPath::kMove_Verb: endIndex += 1; break;
      case SkPath::kLine_Verb: endIndex += 1; break;
      case SkPath::kQuad_Verb:
      case SkPath::kConic_Verb: endIndex += 2; break;
      case SkPath::kCubic_Verb: endIndex += 3; break;
...
static void add_path_segment(int index, SkPath* path) {
  ...
  while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
    ...
    switch (verb) {
      case SkPath::kMove_Verb: result.moveTo(firstPt = pts[0]); break;
      case SkPath::kLine_Verb: result.lineTo(lastPt = pts[1]); break;
      case SkPath::kQuad_Verb: result.quadTo(pts[1], lastPt = pts[2]); break;
      case SkPath::kConic_Verb: result.conicTo(pts[1], lastPt = pts[2], weight); break;
      case SkPath::kCubic_Verb: result.cubicTo(pts[1], pts[2], lastPt = pts[3]); break;
...
static void delete_path_segment(int index, SkPath* path) {
  ...
  while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
    ...
    switch (verb) {
      ...
      case SkPath::kConic_Verb: result.conicTo(pts[1], pts[2], iter.conicWeight()); break;
...
#undef SET_BUTTON
  ...
  static SkScalar MapScreenYtoValue(int y, const UniControl& control) {
    return SkTMin(
               1.f,
               SkTMax(0.f, SkIntToScalar(y) - control.fBounds.fTop) / control.fBounds.height()) *
