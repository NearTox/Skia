...
static void add_path_segment(int index, SkPath* path) {
  ...
  for (auto [verb, pts, w] : SkPathPriv::Iterate(*path)) {
    ...
    switch (verb) {
      case SkPathVerb::kMove: result.moveTo(firstPt = pts[0]); break;
      case SkPathVerb::kLine: result.lineTo(lastPt = pts[1]); break;
      case SkPathVerb::kQuad: result.quadTo(pts[1], lastPt = pts[2]); break;
      case SkPathVerb::kConic: result.conicTo(pts[1], lastPt = pts[2], *w); break;
      case SkPathVerb::kCubic: result.cubicTo(pts[1], pts[2], lastPt = pts[3]); break;
      case SkPathVerb::kClose: result.close(); break;
      default: SkASSERT(0);
    }
  }
  *path = result;
...
static void delete_path_segment(int index, SkPath* path) {
  ...
  for (auto [verb, pts, w] : SkPathPriv::Iterate(*path)) {
    ...
    switch (verb) {
      case SkPathVerb::kMove: result.moveTo(pts[0]); break;
      case SkPathVerb::kLine: result.lineTo(pts[1]); break;
      case SkPathVerb::kQuad: result.quadTo(pts[1], pts[2]); break;
      case SkPathVerb::kConic: result.conicTo(pts[1], pts[2], *w); break;
      case SkPathVerb::kCubic: result.cubicTo(pts[1], pts[2], pts[3]); break;
      case SkPathVerb::kClose: result.close(); break;
      default: SkASSERT(0);
    }
  }
...
static void set_path_weight(int index, SkScalar w, SkPath* path) {
  ...
  while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
    ++counter;
    switch (verb) {
      case SkPath::kMove_Verb: result.moveTo(pts[0]); break;
      case SkPath::kLine_Verb: result.lineTo(pts[1]); break;
      case SkPath::kQuad_Verb: result.quadTo(pts[1], pts[2]); break;
      case SkPath::kConic_Verb:
        result.conicTo(pts[1], pts[2], counter == index ? w : iter.conicWeight());
        break;
      case SkPath::kCubic_Verb: result.cubicTo(pts[1], pts[2], pts[3]); break;
      case SkPath::kClose_Verb: result.close(); break;
...
static void set_path_verb(int index, SkPath::Verb v, SkPath* path, SkScalar w) {
  ...
  while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
    ...
    switch (verb) {
      ...
      case SkPath::kLine_Verb: result.lineTo(pts[1]); break;
      case SkPath::kQuad_Verb: result.quadTo(pts[1], pts[2]); break;
      case SkPath::kConic_Verb: result.conicTo(pts[1], pts[2], weight); break;
      case SkPath::kCubic_Verb: result.cubicTo(pts[1], pts[2], pts[3]); break;
      case SkPath::kClose_Verb: result.close(); break;
      default: SkASSERT(0); break;
    }
...
static void filter_coverage(
    const uint8_t* map, int len, uint8_t min, uint8_t max, uint8_t* filter) {
...
struct Button {
  ...
  void draw(SkCanvas* canvas, const ButtonPaints& paints) {
    ...
    SkTextUtils::Draw(
        canvas, &fLabel, 1, SkTextEncoding::kUTF8, fBounds.centerX(), fBounds.fBottom - 5,
...
struct UniControl {
  ...
  virtual void draw(SkCanvas* canvas, const ControlPaints& paints) {
    ...
    canvas->drawString(
        fName, fBounds.fLeft, fBounds.bottom() + 11, paints.fLabelFont, paints.fLabel);
  }
};
...
class MyClick : public Sample::Click {
 public:
  ...
  MyClick(ClickType type, ControlType control)
      : fType(type), fControl(control), fVerb((SkPath::Verb)-1), fWeight(1) {}

  MyClick(ClickType type, int index)
      : fType(type), fControl((ControlType)index), fVerb((SkPath::Verb)-1), fWeight(1) {}

  MyClick(ClickType type, int index, SkPath::Verb verb, SkScalar weight)
      : fType(type), fControl((ControlType)index), fVerb(verb), fWeight(weight) {}
...
#undef SET_BUTTON
  ...
  bool scaleToFit() {
    ...
    SkRect bounds = fPath.getBounds();
    SkScalar scale =
        std::min(this->width() / bounds.width(), this->height() / bounds.height()) * 0.8f;
  ...
  void setControlButtonsPos() {
    ...
    for (int index = 0; index < kButtonCount; ++index) {
      kButtonList[index].fButton->fBounds.setXYWH(this->width() - 50, buttonOffset += 50, 30, 30);
    }
  }
  ...
  void draw_bisect(
      SkCanvas* canvas, const SkVector& lastVector, const SkVector& vector, const SkPoint& pt) {
    ...
    if (fJoinButton.enabled()) {
      ...
      if (endAngle > startAngle) {
        ...
        canvas->drawArc(oval, startAngle, 360 - (startAngle - endAngle), false, fSkeletonPaint);
      }
    }
  }
      ...
      switch (verb) {
        ...
        case SkPath::kQuad_Verb: {
          ...
          if (0 < t && t < 1) {
            ...
            canvas->drawLine(maxPt, {maxPt.fX + tangent.fY, maxPt.fY - tangent.fX}, fSkeletonPaint);
          }
        } break;
        case SkPath::kConic_Verb:
          nextLast = pts[1] - pts[2];
  ...
  void path_coverage(const SkPath& path, uint8_t* distanceMap, int w, int h) {
    ...
    while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
      switch (verb) {
        case SkPath::kLine_Verb: (void)coverage(pts[0], pts[1], distanceMap, w, h); break;
        case SkPath::kQuad_Verb: quad_coverage(pts, distanceMap, w, h); break;
        case SkPath::kConic_Verb: conic_coverage(pts, iter.conicWeight(), distanceMap, w, h); break;
        case SkPath::kCubic_Verb: cubic_coverage(pts, distanceMap, w, h); break;
        default: break;
      }
    }
  }
  ...
  void onDrawContent(SkCanvas* canvas) override {
    ...
    if (fFilterButton.fState == 2 && (0 < fFilterControl.fValLo || fFilterControl.fValHi < 255)) {
      SkBitmap filteredMap;
      uint8_t* filtered = set_up_dist_map(imageInfo, &filteredMap);
      filter_coverage(
          distanceMap, sizeof(uint8_t) * w * h, (uint8_t)fFilterControl.fValLo,
  ...
  Sample::Click* onFindClickHandler(SkScalar x, SkScalar y, skui::ModifierKey modi) override {
    ...
    if (!fHideAll) {
      ...
      for (int index = 0; index < kControlCount; ++index) {
        if (kControlList[index].fControl->contains(rectPt)) {
          return new MyClick(MyClick::kControlType, kControlList[index].fControlType);
        }
      }
      for (int index = 0; index < kButtonCount; ++index) {
    ...
    fLineButton.fVisible = fQuadButton.fVisible = fConicButton.fVisible = fCubicButton.fVisible =
        fWeightControl.fVisible = fAddButton.fVisible = fDeleteButton.fVisible = false;
    fActiveVerb = -1;
    fActivePt = -1;
    if (fHandlePathMove) {
  ...
  static SkScalar MapScreenYtoValue(int y, const UniControl& control) {
    return std::min(
               1.f,
               std::max(0.f, SkIntToScalar(y) - control.fBounds.fTop) / control.fBounds.height()) *
  ...
  bool onClick(Click* click) override {
    ...
    switch (myClick->fType) {
      case MyClick::kPtType: {
        ...
        pt.offset(
            SkIntToScalar(click->fCurr.fX - click->fPrev.fX),
      ...
      case MyClick::kControlType: {
        ...
        switch (myClick->fControl) {
          ...
          case MyClick::kDeleteButton:
            ...
            validatePath();
            break;
          case MyClick::kFillButton: fFillButton.toggle(); break;
          case MyClick::kSkeletonButton: fSkeletonButton.toggle(); break;
          case MyClick::kFilterButton:
            fFilterButton.toggle();
            fFilterControl.fVisible = fFilterButton.fState == 2;
            break;
          case MyClick::kBisectButton: fBisectButton.toggle(); break;
          case MyClick::kJoinButton: fJoinButton.toggle(); break;
          case MyClick::kInOutButton: fInOutButton.toggle(); break;
          default: SkASSERT(0); break;
        }
      } break;
      default: SkASSERT(0); break;
...
void AAGeometryView::draw_legend(SkCanvas* canvas) {
  ...
  for (int index = kKeyCommandCount - 1; index >= 0; --index) {
    ...
    SkTextUtils::DrawString(
        canvas, kKeyCommandList[index].fDescriptionL, this->width() - 160, bottomOffset,
        fLegendLeftFont, SkPaint());
    SkTextUtils::DrawString(
        canvas, kKeyCommandList[index].fDescriptionR, this->width() - 20, bottomOffset,
