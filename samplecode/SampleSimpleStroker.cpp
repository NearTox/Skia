...
class PathRecorder {
 public:
  SkPath getPath() const {
    return SkPath::Make(
        fPoints.data(), fPoints.size(), fVerbs.data(), fVerbs.size(), nullptr, 0,
...
SkPath SkPathStroker2::getFillPath(const SkPath& path, const SkPaint& paint) {
  ...
  while ((segment.fVerb = it.next(segment.fPoints)) != SkPath::kDone_Verb) {
    ...
    switch (segment.fVerb) {
      case SkPath::kLine_Verb: strokeLine(segment, firstSegment); break;
      case SkPath::kMove_Verb:
        // Don't care about multiple contours currently
        continue;
      default: SkDebugf("Unhandled path verb %d\n", segment.fVerb); break;
    }
...
void SkPathStroker2::endcap(CapLocation loc) {
  ...
  switch (fCap) {
    case SkPaint::kButt_Cap: buttCap(loc); break;
    default:
...
void SkPathStroker2::join(const PathSegment& prev, const PathSegment& curr) {
  ...
  switch (fJoin) {
    case SkPaint::kMiter_Join: miterJoin(prev, curr); break;
    default:
...
class SimpleStroker : public Sample {
  ...
  float fWidth = 175;
  SkPaint fPtsPaint, fStrokePaint, fMirrorStrokePaint, fNewFillPaint, fHiddenPaint, fSkeletonPaint;
 ...
 protected:
  ...
  bool onChar(SkUnichar uni) override {
    switch (uni) {
      ...
      case '2': this->toggle(fShowSkiaStroke); return true;
