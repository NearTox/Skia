...
class TextureUploadSample : public Sample {
  ...
  void onDrawContent(SkCanvas* canvas) override {
    ...
    if (direct) {
      ...
      int textureCount = fTileRows * fTileCols;
      for (int i = 0; i < textureCount; i++) {
        fTextures[i]->uploadRasterSurface(i == fActiveTileIndex ? fBlueSurface : fGraySurface);
      }
      ...
      if (fDrawTexturesToScreen) {
        for (int y = 0; y < fTileRows; y++) {
          for (int x = 0; x < fTileCols; x++) {
            ...
            canvas->drawImage(
                fTextures[currentIndex]->getImage(), x * fTileSize, y * fTileSize, &paint);
          }
        }
      }
    }
  ...
  bool onAnimate(double nanos) override {
    ...
    fActiveTileIndex =
        floorf(TimeUtils::Scaled(1e-9 * nanos, numTiles / kDesiredDurationSecs, numTiles));
    return true;
  }
};
