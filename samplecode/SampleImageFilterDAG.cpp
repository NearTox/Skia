 ...
 private:
  void computeForwardBounds(const SkIRect srcRect) {
    if (fFilter) {
      ...
      // intermediate values.
      fForwardBounds =
          fFilter->filterBounds(srcRect, fLocalCTM, SkImageFilter::kForward_MapDirection, nullptr);
      ...
      fForwardIsolatedBounds = SkFilterNodeBounds(
          fFilter.get(), srcRect, fLocalCTM, SkImageFilter::kForward_MapDirection, nullptr);
    } else {
  ...
  void computeReverseLocalIsolatedBounds(const SkIRect& srcRect) {
    if (fFilter) {
      fReverseLocalIsolatedBounds = SkFilterNodeBounds(
          fFilter.get(), srcRect, fLocalCTM, SkImageFilter::kReverse_MapDirection, &srcRect);
    } else {
  ...
  void computeReverseBounds(const SkIRect& srcRect, bool writeToLayerBounds) {
    ...
    if (fFilter) {
      ...
      reverseBounds =
          fFilter->filterBounds(srcRect, fLocalCTM, SkImageFilter::kReverse_MapDirection, &srcRect);
      ...
      SkASSERT(
          netReverseBounds == reverseBounds ||
...
static FilterNode build_dag(
    const SkMatrix& local, const SkMatrix& remainder, const SkRect& rect,
...
static FilterNode build_dag(
    const SkMatrix& ctm, const SkRect& rect, const SkImageFilter* rootFilter) {
  ...
  rootNode.fInputNodes.push_back() = build_dag(local, remaining, rect, finalFilter.get(), 1);
...
static float print_matrix(
    SkCanvas* canvas, const char* prefix, const SkMatrix& matrix, float x, float y,
  ...
  for (int i = 0; i < 3; ++i) {
    SkString row;
    row.appendf(
        "[%.2f %.2f %.2f]", matrix.get(i * 3), matrix.get(i * 3 + 1), matrix.get(i * 3 + 2));
...
static float print_size(
    SkCanvas* canvas, const char* prefix, const SkIRect& rect, float x, float y, const SkFont& font,
    const SkPaint& paint) {
  canvas->drawString(prefix, x, y, font, paint);
  y += kLineHeight;
  SkString sz;
...
static float print_info(SkCanvas* canvas, const FilterNode& node) {
  ...
  if (node.fDepth == 0) {
    ...
    y = print_matrix(
        canvas, "Local", node.fInputNodes[0].fLocalCTM, kLineInset, y + kLineHeight, font, text);
    y = print_matrix(
        canvas, "Embedded", node.fInputNodes[0].fRemainingCTM, kLineInset, y, font, text);
  } else if (node.fFilter) {
    canvas->drawString(node.fFilter->getTypeName(), kLineInset, y, font, text);
    print_size(canvas, "Layer Size", node.fLayerBounds, kLineInset, y + kLineHeight, font, text);
    y = print_size(
        canvas, "Ideal Size", node.fReverseLocalBounds, 10 * kLineInset, y + kLineHeight, font,
        text);
  } else {
...
static float draw_dag(SkCanvas* canvas, SkSurface* nodeSurface, const FilterNode& node) {
  ...
  canvas->drawRect(SkRect::MakeWH(nodeResults->width(), nodeResults->height() + textHeight), line);
  ...
  for (int i = 0; i < node.fInputNodes.count(); ++i) {
    ...
    canvas->drawLine(
        nodeResults->width(), 0.5f * nodeResults->height(),  // right of node
...
static void draw_dag(
    SkCanvas* canvas, sk_sp<SkImageFilter> filter, const SkRect& rect, const SkISize& surfaceSize) {
  ...
  sk_sp<SkSurface> nodeSurface =
      canvas->makeSurface(canvas->imageInfo().makeWH(surfaceSize.width(), surfaceSize.height()));
 ...
 public:
  ...
  void onDrawContent(SkCanvas* canvas) override {
    ...
    static const SkISize kFilterSurfaceSize = SkISize::Make(
        2 * (kFilterRect.fRight + kFilterRect.fLeft), 2 * (kFilterRect.fBottom + kFilterRect.fTop));
