...
static void show_text(SkCanvas* canvas, bool doAA) {
  ...
  for (int i = 0; i < 200; ++i) {
    ...
    canvas->drawString(
        "Hamburgefons", rand.nextSScalar1() * W, rand.nextSScalar1() * H + 20, font, paint);
  }
}
...
static void show_fill(SkCanvas* canvas, bool doAA) {
  ...
  for (int i = 0; i < 50; ++i) {
    ...
    r.setXYWH(
        rand.nextSScalar1() * W, rand.nextSScalar1() * H, rand.nextUScalar1() * W,
        rand.nextUScalar1() * H);
...
static void show_stroke(SkCanvas* canvas, bool doAA, SkScalar strokeWidth, int n) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    paint.setColor(rand.nextU());
    canvas->drawLine(
        randRange(rand, minx, maxx), randRange(rand, miny, maxy), randRange(rand, minx, maxx),
        randRange(rand, miny, maxy), paint);
  }
}
...
static void clip(const SkPath& path, SkPoint p0, SkPoint p1, SkPath* clippedPath) {
  SkMatrix mx, inv;
  SkVector v = p1 - p0;
  mx.setAll(v.fX, -v.fY, p0.fX, v.fY, v.fX, p0.fY, 0, 0, 1);
...
static void compute_half_planes(const SkMatrix& mx, SkScalar W, SkScalar H, SkHalfPlane planes[4]) {
  SkScalar a = mx[0], b = mx[1], c = mx[2], d = mx[3], e = mx[4], f = mx[5], g = mx[6], h = mx[7],
           i = mx[8];
 ...
 public:
  SkM44 get44(const SkRect& r) const {
    ...
    SkM44 camera = Sk3LookAt(fEye, fCOA, fUp), perspective = Sk3Perspective(fNear, fFar, fAngle),
          ...
          viewport =
              SkM44::Translate(r.centerX(), r.centerY(), 0) * SkM44::Scale(w * 0.5f, h * 0.5f, 1);
