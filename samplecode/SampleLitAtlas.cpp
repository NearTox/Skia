...
class DrawLitAtlasDrawable : public SkDrawable {
 ...
 private:
  ...
  void drawLightDir(SkCanvas* canvas, SkScalar centerX, SkScalar centerY) {
    ...
    SkPoint p1 = SkPoint::Make(centerX + kBgLen * fLightDir.fX, centerY - kBgLen * fLightDir.fY);
    SkPoint p2 = SkPoint::Make(
        centerX + (kBgLen - kSmLen) * fLightDir.fX, centerY - (kBgLen - kSmLen) * fLightDir.fY);

    SkPaint p;
    canvas->drawLine(centerX, centerY, p1.fX, p1.fY, p);
    canvas->drawLine(p1.fX, p1.fY, p2.fX - kSmLen * fLightDir.fY, p2.fY - kSmLen * fLightDir.fX, p);
    canvas->drawLine(p1.fX, p1.fY, p2.fX + kSmLen * fLightDir.fY, p2.fY + kSmLen * fLightDir.fX, p);
  }
#endif
    ...
    // big asteroid
    {
      ...
      for (int y = kBigYOff; y < kBigYOff + kBigSize; ++y) {
        for (int x = kDiffXOff; x < kDiffXOff + kBigSize; ++x) {
          SkScalar distSq =
              (x - bigCenter.fX) * (x - bigCenter.fX) + (y - bigCenter.fY) * (y - bigCenter.fY);
    ...
    // small asteroid
    {
      ...
      for (int y = kSmYOff; y < kSmYOff + kSmSize; ++y) {
        for (int x = kDiffXOff; x < kDiffXOff + kSmSize; ++x) {
          SkScalar distSq =
              (x - smCenter.fX) * (x - smCenter.fX) + (y - smCenter.fY) * (y - smCenter.fY);
  ...
  class ObjectRecord {
   public:
    void initAsteroid(SkRandom* rand, const SkRect& bounds, SkRect* diffTex, SkRect* normTex) {
      ...
      fPosition.set(
          bounds.fLeft + rand->nextUScalar1() * bounds.width(),
      ...
      diffTex->setXYWH(
          SkIntToScalar(kDiffXOff), gYOffs[fObjType], gSizes[fObjType], gSizes[fObjType]);
      normTex->setXYWH(
          SkIntToScalar(kNormXOff), gYOffs[fObjType], gSizes[fObjType], gSizes[fObjType]);
    }
    ...
    void initShip(const SkRect& bounds, SkRect* diffTex, SkRect* normTex) {
      ...
      diffTex->setXYWH(
          SkIntToScalar(kDiffXOff), SkIntToScalar(kShipYOff), SkIntToScalar(kMedSize),
          SkIntToScalar(kMedSize));
      normTex->setXYWH(
          SkIntToScalar(kNormXOff), SkIntToScalar(kShipYOff), SkIntToScalar(kMedSize),
          SkIntToScalar(kMedSize));
    }
    ...
    SkRSXform asRSXform() const {
      ...
      return SkRSXform::MakeFromRadians(
          1.0f, fRot, fPosition.x(), fPosition.y(), gHalfSizes[fObjType], gHalfSizes[fObjType]);
    }
