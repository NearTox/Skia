 ...
 protected:
  ...
  void drawTest(SkCanvas* canvas, SkScalar w, SkScalar h, SkColor fg, SkColor bg) {
    ...
    for (int i = 9; i < 24; i += 2) {
      ...
      std::unique_ptr<SkShaper::FontRunIterator> font(SkShaper::MakeFontMgrRunIterator(
          utf8, utf8Bytes, srcFont, SkFontMgr::RefDefault(), "Arial", SkFontStyle::Bold(),
          &*language));
      if (!font) {
        return;
      }
...
class SampleShaper : public Sample {
 ...
 protected:
  ...
  void drawTest(
      SkCanvas* canvas, const char str[], SkScalar size, std::unique_ptr<SkShaper> shaper) {
    if (!shaper) return;
    ...
    std::unique_ptr<SkShaper::BiDiRunIterator> bidi(SkShaper::MakeBiDiRunIterator(str, len, 0xfe));
    if (!bidi) {
      return;
    }
    ...
    std::unique_ptr<SkShaper::FontRunIterator> font(SkShaper::MakeFontMgrRunIterator(
        str, len, srcFont, SkFontMgr::RefDefault(), "Arial", SkFontStyle::Bold(), &*language));
    if (!font) {
      return;
    }
