...
class SamplePathTessellatorOp : public GrDrawOp {
 private:
  ...
  SamplePathTessellatorOp(
      const SkRect& drawBounds, const SkPath& path, const SkMatrix& m,
      GrPipeline::InputFlags pipelineFlags, Mode mode)
      : GrDrawOp(ClassID()), fPath(path), fMatrix(m), fPipelineFlags(pipelineFlags), fMode(mode) {
  ...
  FixedFunctionFlags fixedFunctionFlags() const override { return FixedFunctionFlags::kUsesHWAA; }
  GrProcessorSet::Analysis finalize(
      const GrCaps& caps, const GrAppliedClip* clip, GrClampType clampType) override {
    SkPMColor4f color;
    return fProcessors.finalize(
        SK_PMColor4fWHITE, GrProcessorAnalysisCoverage::kNone, clip, nullptr, caps, clampType,
        &color);
  }
  void onPrePrepare(
...
void SamplePathTessellators::onDrawContent(SkCanvas* canvas) {
  ...
  sdc->addDrawOp(GrOp::Make<SamplePathTessellatorOp>(
      ctx, sdc->asRenderTargetProxy()->getBoundsRect(), fPath, canvas->getTotalMatrix(),
  ...
  devPath.transform(canvas->getTotalMatrix());
  {
    ...
    canvas->drawPoints(
        SkCanvas::kPoints_PointMode, devPath.countPoints(), SkPathPriv::PointData(devPath),
        pointsPaint);
  }
}
...
Sample::Click* SamplePathTessellators::onFindClickHandler(
...
static SkPath update_weight(const SkPath& path, float w) {
  ...
  for (auto [verb, pts, _] : SkPathPriv::Iterate(path)) {
    switch (verb) {
      case SkPathVerb::kMove: path_.moveTo(pts[0]); break;
      case SkPathVerb::kLine: path_.lineTo(pts[1]); break;
      case SkPathVerb::kQuad: path_.quadTo(pts[1], pts[2]); break;
      case SkPathVerb::kCubic: path_.cubicTo(pts[1], pts[2], pts[3]); break;
      case SkPathVerb::kConic: path_.conicTo(pts[1], pts[2], (w != 1) ? w : .99f); break;
...
bool SamplePathTessellators::onChar(SkUnichar unichar) {
  switch (unichar) {
    case 'w':
      fPipelineFlags =
          (GrPipeline::InputFlags)((int)fPipelineFlags ^ (int)GrPipeline::InputFlags::kWireframe);
      return true;
    case 'D': {
      fPath.dump();
      return true;
