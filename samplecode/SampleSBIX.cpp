...
struct SBIXView : public Sample {
  ...
  sk_sp<SkData> updateSBIXData(SkData* originalData, bool setPts) {
    ...
    SkOTTableGlyph* glyfTable = SkTAddOffset<SkOTTableGlyph>(sfntHeader, glyfTableOffset);

    size_t headTableOffset = SkEndian_SwapBE32(headTableEntry->offset);
    SkOTTableHead* headTable = SkTAddOffset<SkOTTableHead>(sfntHeader, headTableOffset);
    ...
    if (glyphData) {
      if (setPts) {
        fPts[0].set(
            (int16_t)SkEndian_SwapBE16(glyphData->xMin) / toEm,
            (int16_t)SkEndian_SwapBE16(glyphData->yMin) / -toEm);
        fPts[1].set(
            (int16_t)SkEndian_SwapBE16(glyphData->xMax) / toEm,
      ...
      if (contourCount > 0) {
        ...
        SK_OT_USHORT* numInstructions = SkTAfter<SK_OT_USHORT>(endPtsOfContours, contourCount);
        SK_OT_BYTE* instructions = SkTAfter<SK_OT_BYTE>(numInstructions);
        SkOTTableGlyphData::Simple::Flags* flags = SkTAfter<SkOTTableGlyphData::Simple::Flags>(
