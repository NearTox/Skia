...
class TilingView : public Sample {
 ...
 public:
  TilingView()
      : fLooper(SkBlurDrawLooper::Make(
            0x88000000, SkBlurMask::ConvertRadiusToSigma(SkIntToScalar(1)), SkIntToScalar(2),
            SkIntToScalar(2))) {
 ...
 protected:
  ...
  virtual void onDrawContent(SkCanvas* canvas) {
    ...
    static const SkTileMode gModes[] = {SkTileMode::kClamp, SkTileMode::kRepeat,
                                        SkTileMode::kMirror};
      ...
      for (size_t kx = 0; kx < SK_ARRAY_COUNT(gModes); kx++) {
        for (size_t ky = 0; ky < SK_ARRAY_COUNT(gModes); ky++) {
          ...
          SkTextUtils::DrawString(
              textCanvas, str.c_str(), x + r.width() / 2, y, SkFont(), p,
