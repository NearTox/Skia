...
class PathText::MovingGlyphAnimator : public PathText::GlyphAnimator {
 public:
  MovingGlyphAnimator(Glyph* glyphs)
      : GlyphAnimator(glyphs), fFrontMatrices(kNumPaths), fBackMatrices(kNumPaths) {}

  ~MovingGlyphAnimator() override { fBackgroundAnimationTask.wait(); }
  ...
  bool animate(double nanos, int screenWidth, int screenHeight) final {
    ...
    fBackgroundAnimationTask.add(std::bind(
        &MovingGlyphAnimator::runAnimationTask, this, tsec, dt, screenWidth, screenHeight));
...
class PathText::WavyGlyphAnimator : public PathText::MovingGlyphAnimator {
 public:
  WavyGlyphAnimator(Glyph* glyphs)
      : MovingGlyphAnimator(glyphs), fFrontPaths(kNumPaths), fBackPaths(kNumPaths) {}

  ~WavyGlyphAnimator() override { fBackgroundAnimationTask.wait(); }
...
SkPoint PathText::WavyGlyphAnimator::Waves::apply(
    float tsec, const Sk2f matrix[3], const SkPoint& pt) const {
  ...
  const Sk4f t =
      (frequencies * (dirsX * devicePt[0] + dirsY * devicePt[1]) + speeds * tsec + offsets).abs() *
  ...
  const Sk4f left(
      sin2table[indices[0]], sin2table[indices[1]], sin2table[indices[2]], sin2table[indices[3]]);
  const Sk4f right(
      sin2table[indices[0] + 1], sin2table[indices[1] + 1], sin2table[indices[2] + 1],
      sin2table[indices[3] + 1]);
...
bool PathText::onChar(SkUnichar unichar) {
  switch (unichar) {
    case 'X': fDoClip = !fDoClip; return true;
    case 'S':
