...
SkPoint PathText::WavyGlyphAnimator::Waves::apply(
    float tsec, const skvx::float2 matrix[3], const SkPoint& pt) const {
  ...
  const skvx::float4 t =
      abs(frequencies * (dirsX * devicePt[0] + dirsY * devicePt[1]) + speeds * tsec + offsets) *
      (float(kTablePeriod) / SK_ScalarPI);
