...
class Patch {
 public:
  ...
  void draw(SkCanvas*, const SkPaint&, int segsU, int segsV, bool doTextures, bool doColors);
...
static void eval_sheet(const SkPoint edge[], int nu, int nv, int iu, int iv, SkPoint* pt) {
  const int TL = 0;
  const int TR = nu;
  const int BR = TR + nv;
  const int BL = BR + nu;
  ...
  SkScalar x = (1 - v) * edge[TL + iu].fX + u * edge[TR + iv].fX + v * edge[BR + nu - iu].fX +
               (1 - u) * edge[BL + nv - iv].fX - x0;
  SkScalar y = (1 - v) * edge[TL + iu].fY + u * edge[TR + iv].fY + v * edge[BR + nu - iu].fY +
               (1 - u) * edge[BL + nv - iv].fY - y0;
  pt->set(x, y);
}
...
void Patch::draw(
    SkCanvas* canvas, const SkPaint& paint, int nu, int nv, bool doTextures, bool doColors) {
  if (nu < 1 || nv < 1) {
    return;
  }
  ...
  for (row = 0; row < nv; row++) {
    ...
    canvas->drawVertices(
        SkVertices::MakeCopy(
            SkVertices::kTriangleStrip_VertexMode, stripCount, strip, doTextures ? tex : nullptr,
            doColors ? colors : nullptr),
        SkBlendMode::kModulate, paint);
  }
}

static void drawpatches(SkCanvas* canvas, const SkPaint& paint, int nu, int nv, Patch* patch) {
...
struct PatchView : public Sample {
  ...
  SkPoint fPts[12] = {
      {kS * 0, kT * 1}, {kS * 1, kT * 1}, {kS * 2, kT * 1}, {kS * 3, kT * 1},
      {kS * 3, kT * 2}, {kS * 3, kT * 3}, {kS * 3, kT * 4}, {kS * 2, kT * 4},
      {kS * 1, kT * 4}, {kS * 0, kT * 4}, {kS * 0, kT * 3}, {kS * 0, kT * 2},
  };
...
class ManyStrokesView : public Sample {
 ...
 protected:
  ...
  void dodraw(
      SkCanvas* canvas, sk_sp<SkPathEffect> pe, SkScalar x, SkScalar y,
