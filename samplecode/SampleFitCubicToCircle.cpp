...
static float fit_cubic_to_unit_circle(
    double x0, double y0, double x1, double y1, std::array<double, 4>* X,
...
static double lerp(double x, double y, double T) { return x + T * (y - x); }
...
void SampleFitCubicToCircle::fitCubic() {
  ...
  fControlLength =
      fit_cubic_to_unit_circle(fEndptsX[0], fEndptsY[0], fEndptsX[1], fEndptsY[1], &X, &Y);
...
void SampleFitCubicToCircle::onDrawContent(SkCanvas* canvas) {
  ...
  canvas->drawArc(
      SkRect::MakeXYWH(kCenterX - kRadius, kCenterY - kRadius, kRadius * 2, kRadius * 2), 0, 360,
      false, circlePaint);
  ...
  SkPoint points[2] = {
      {(float)fCubicX[0], (float)fCubicY[0]}, {(float)fCubicX[3], (float)fCubicY[3]}};
...
Sample::Click* SampleFitCubicToCircle::onFindClickHandler(
