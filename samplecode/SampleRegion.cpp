...
static void drawFadingText(
    SkCanvas* canvas, const char* text, size_t len, SkScalar x, SkScalar y, const SkFont& font,
    const SkPaint& paint) {
  ...
  void onDrawContent(SkCanvas* canvas) override {
    ...
    for (size_t op = 0; op < SK_ARRAY_COUNT(gOps); op++) {
      canvas->drawSimpleText(
          gOps[op].fName, strlen(gOps[op].fName), SkTextEncoding::kUTF8, SkIntToScalar(75),
          SkIntToScalar(50), font, SkPaint());
