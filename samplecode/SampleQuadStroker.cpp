...
static int getOnCurvePoints(const SkPath& path, SkPoint storage[]) {
  ...
  while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
    switch (verb) {
      ...
      case SkPath::kCubic_Verb: storage[count++] = pts[0]; break;
      default: break;
    }
  }
  return count;
...
class QuadStrokerView : public Sample {
  enum { SKELETON_COLOR = 0xFF0000FF, WIREFRAME_COLOR = 0x80FF0000 };
...
#define kWidthMax 100
 ...
 protected:
  ...
  void draw_points(SkCanvas* canvas, const SkPath& path, SkColor color, bool show_lines) {
    SkPaint paint;
  ...
  void draw_ribs(SkCanvas* canvas, const SkPath& path, SkScalar width, SkColor color) {
    ...
    for (SkScalar dist = 0; dist <= total; dist += delta) {
      if (meas.getPosTan(dist, &pos, &tan)) {
        ...
        canvas->drawLine(
            pos.x() + tan.x(), pos.y() + tan.y(), pos.x() - tan.x(), pos.y() - tan.y(), paint);
        if (0 == index % 10) {
          SkString label;
          ...
          canvas->drawString(
              label, pos.x() - tan.x() * 1.25f, pos.y() - tan.y() * 1.25f, font, labelP);
        }
      }
      ++index;
    }
  ...
  void draw_t_divs(SkCanvas* canvas, const SkPath& path, SkScalar width, SkColor color) {
    ...
    for (int index = 0; index < ribs; ++index) {
      ...
      switch (verb) {
        ...
        case SkPath::kCubic_Verb: SkEvalCubicAt(pts, t, &pos, &tan, nullptr); break;
        default: SkASSERT(0); return;
      }
      ...
      if (0 == index % 10) {
        ...
        label.appendS32(index);
        canvas->drawString(
            label, pos.x() + tan.x() * 1.25f, pos.y() + tan.y() * 1.25f, SkFont(), paint);
      }
    }
  }

  void draw_stroke(
      SkCanvas* canvas, const SkPath& path, SkScalar width, SkScalar scale, bool drawText) {
    if (path.isEmpty()) {
      return;
    }
    SkRect bounds = path.getBounds();
    this->setWHZ(
        SkScalarCeilToInt(bounds.right()),
        drawText ? SkScalarRoundToInt(scale * 3 / 2) : SkScalarRoundToInt(scale),
  ...
  void draw_fill(SkCanvas* canvas, const SkRect& rect, SkScalar width) {
    ...
    path.addCircle(center.fX, center.fY, maxSide + width / 2);
    SkRect outside = SkRect::MakeXYWH(
        center.fX - maxSide - width, center.fY - maxSide - width, (maxSide + width) * 2,
        (maxSide + width) * 2);
  ...
  void draw_button(SkCanvas* canvas, const StrokeTypeButton& button) {
    ...
    font.setSize(25.0f);
    SkTextUtils::Draw(
        canvas, &button.fLabel, 1, SkTextEncoding::kUTF8, button.fBounds.centerX(),
        button.fBounds.fBottom - 5, font, paint, SkTextUtils::kCenter_Align);
  }

  void draw_control(
      SkCanvas* canvas, const SkRect& bounds, SkScalar value, SkScalar min, SkScalar max,
      const char* name) {
    SkPaint paint;
    paint.setAntiAlias(true);
    paint.setStyle(SkPaint::kStroke_Style);
  ...
  void onDrawContent(SkCanvas* canvas) override {
    ...
    if (fTextButton.fEnabled) {
      ...
      SkTextUtils::GetPath(
          fText.c_str(), fText.size(), SkTextEncoding::kUTF8, 0, fTextSize, font, &path);
      setForText();
...
#ifdef SK_DEBUG
    draw_control(
        canvas, fErrorControl, gDebugStrokerError, kStrokerErrorMin, kStrokerErrorMax, "error");
#endif
    draw_control(
        canvas, fWidthControl, fWidth * fWidthScale, kWidthMin * fWidthScale,
  ...
  static SkScalar MapScreenYtoValue(int y, const SkRect& control, SkScalar min, SkScalar max) {
  ...
  bool onClick(Click* click) override {
    ...
    if (index < (int)SK_ARRAY_COUNT(fPts)) {
      ...
      gDebugStrokerError = SkTMax(
          FLT_EPSILON,
          MapScreenYtoValue(click->fICurr.fY, fErrorControl, kStrokerErrorMin, kStrokerErrorMax));
      gDebugStrokerErrorSet = true;
    }
#endif
    else if (index == (int)SK_ARRAY_COUNT(fPts) + 4) {
      fWidth = SkTMax(
          FLT_EPSILON, MapScreenYtoValue(click->fICurr.fY, fWidthControl, kWidthMin, kWidthMax));
