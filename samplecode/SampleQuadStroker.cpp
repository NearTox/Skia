...
static int getOnCurvePoints(const SkPath& path, SkPoint storage[]) {
  ...
  for (auto [verb, pts, w] : SkPathPriv::Iterate(path)) {
    switch (verb) {
      ...
      case SkPathVerb::kCubic: storage[count++] = pts[0]; break;
      default: break;
    }
  }
  return count;
...
static void getContourCounts(const SkPath& path, SkTArray<int>* contourCounts) {
  ...
  for (auto [verb, pts, w] : SkPathPriv::Iterate(path)) {
    switch (verb) {
      ...
      case SkPathVerb::kQuad:
      case SkPathVerb::kConic: count += 2; break;
...
#define kWidthMax 100
 ...
 protected:
  ...
  static SkScalar MapScreenYtoValue(SkScalar y, const SkRect& control, SkScalar min, SkScalar max) {
  ...
  bool onClick(Click* click) override {
    ...
    if (index < (int)SK_ARRAY_COUNT(fPts)) {
      fPts[index].offset(click->fCurr.fX - click->fPrev.fX, click->fCurr.fY - click->fPrev.fY);
