...
#define ACQUIRE_INST_VK_PROC(name)                                      \
  do {                                                                  \
    fVk##name = reinterpret_cast<PFN_vk##name>(                         \
        getProc("vk" #name, backendContext.fInstance, VK_NULL_HANDLE)); \
    if (fVk##name == nullptr) {                                         \
...
int main(int argc, char** argv) {
  ...
  // do because it is againt meant for Skia testing.
  {
    ...
    if (!sk_gpu_test::CreateVkBackendContext(
            instProc, &backendContext, extensions.get(), features.get(), &debugCallback)) {
      return 1;
    }
  }
  ...
  sk_sp<SkSurface> surface =
      SkSurface::MakeRenderTarget(context.get(), SkBudgeted::kYes, imageInfo);
  if (!surface) {
    context.reset();
    fVkDestroyDevice(backendContext.fDevice, nullptr);
    if (debugCallback != VK_NULL_HANDLE) {
