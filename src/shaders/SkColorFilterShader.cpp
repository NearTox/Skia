...
SkColorFilterShader::SkColorFilterShader(
    sk_sp<SkShader> shader, float alpha, sk_sp<SkColorFilter> filter)
    : fShader(std::move(shader)), fFilter(as_CFB_sp(std::move(filter))), fAlpha(alpha) {
  SkASSERT(fShader);
  SkASSERT(fFilter);
}
...
bool SkColorFilterShader::isOpaque() const noexcept {
...
skvm::Color SkColorFilterShader::onProgram(
...
std::unique_ptr<GrFragmentProcessor> SkColorFilterShader::asFragmentProcessor(
  ...
  auto [success, fp] =
      fFilter->asFragmentProcessor(std::move(shaderFP), args.fContext, *args.fDstColorInfo);
