...
sk_sp<SkShader> SkPicture::makeShader(
    SkTileMode tmx, SkTileMode tmy, SkFilterMode filter, const SkMatrix* localMatrix,
...
struct ImageFromPictureKey : public SkResourceCache::Key {
 public:
  ImageFromPictureKey(
      SkColorSpace* colorSpace, SkColorType colorType, uint32_t pictureID, const SkRect& subset,
      SkSize scale)
    ...
    static const size_t keySize = sizeof(fColorSpaceXYZHash) + sizeof(fColorSpaceTransferFnHash) +
                                  sizeof(fColorType) + sizeof(fSubset) + sizeof(fScale);
    // This better be packed.
    SkASSERT(sizeof(uint32_t) * (&fEndOfStruct - &fColorSpaceXYZHash) == keySize);
    this->init(
        &gImageFromPictureKeyNamespaceLabel, SkPicturePriv::MakeSharedID(pictureID), keySize);
  }
...
struct ImageFromPictureRec : public SkResourceCache::Rec {
  ImageFromPictureRec(const ImageFromPictureKey& key, sk_sp<SkImage> image)
      : fKey(key), fImage(std::move(image)) {}
...
SkPictureShader::SkPictureShader(
    sk_sp<SkPicture> picture, SkTileMode tmx, SkTileMode tmy, SkFilterMode filter,
...
sk_sp<SkShader> SkPictureShader::Make(
    sk_sp<SkPicture> picture, SkTileMode tmx, SkTileMode tmy, SkFilterMode filter,
    const SkMatrix* lm, const SkRect* tile) {
...
struct CachedImageInfo {
  ...
  static CachedImageInfo Make(
      const SkRect& bounds, const SkMatrix& viewMatrix,
      SkTCopyOnFirstWrite<SkMatrix>* localMatrix,  // in/out
      SkColorType dstColorType, SkColorSpace* dstColorSpace, const int maxTextureSize) {
    ...
    const SkSize scaledSize = [&]() {
      ...
      if (maxTextureSize) {
        if (size.width() > maxTextureSize || size.height() > maxTextureSize) {
          ...
          size.set(
              SkScalarFloorToScalar(size.width() * downScale),
    ...
    return {
        ...
        SkImageInfo::Make(tileSize.width(), tileSize.height(), imgCT, kPremul_SkAlphaType, imgCS),
    };
  }
...
sk_sp<SkShader> SkPictureShader::rasterShader(
    const SkMatrix& viewMatrix, SkTCopyOnFirstWrite<SkMatrix>* localMatrix,
    SkColorType dstColorType, SkColorSpace* dstColorSpace) const {
  const int maxTextureSize_NotUsedForCPU = 0;
  CachedImageInfo info = CachedImageInfo::Make(
      fTile, viewMatrix, localMatrix, dstColorType, dstColorSpace, maxTextureSize_NotUsedForCPU);
  if (!info.success) {
    return nullptr;
  }

  ImageFromPictureKey key(
      info.imageInfo.colorSpace(), info.imageInfo.colorType(), fPicture->uniqueID(), fTile,
      info.tileScale);
...
bool SkPictureShader::onAppendStages(const SkStageRec& rec) const {
  ...
  bitmapShader =
      this->rasterShader(rec.fMatrixProvider.localToDevice(), &lm, rec.fDstColorType, rec.fDstCS);
  if (!bitmapShader) {
    return false;
  }
...
skvm::Color SkPictureShader::onProgram(
  ...
  bitmapShader =
      this->rasterShader(matrices.localToDevice(), &lm, dst.colorType(), dst.colorSpace());
  if (!bitmapShader) {
    return {};
  }

  return as_SB(bitmapShader)->program(p, device, local, paint, matrices, lm, dst, uniforms, alloc);
}

/////////////////////////////////////////////////////////////////////////////////////////

#ifdef SK_ENABLE_LEGACY_SHADERCONTEXT
SkShaderBase::Context* SkPictureShader::onMakeContext(
    const ContextRec& rec, SkArenaAlloc* alloc) const {
  auto lm = this->totalLocalMatrix(rec.fLocalMatrix);
  sk_sp<SkShader> bitmapShader =
      this->rasterShader(*rec.fMatrix, &lm, rec.fDstColorType, rec.fDstColorSpace);
  if (!bitmapShader) {
    return nullptr;
  }
...
SkPictureShader::PictureShaderContext::PictureShaderContext(
    ...
    SkArenaAlloc* alloc)
    : INHERITED(shader, rec), fBitmapShader(std::move(bitmapShader)) {
...
std::unique_ptr<GrFragmentProcessor> SkPictureShader::asFragmentProcessor(
  ...
  auto info = CachedImageInfo::Make(
      fTile, args.fMatrixProvider.localToDevice(), &lm, dstColorType, dstCS.get(),
  ...
  if (auto proxy = provider->findOrCreateProxyByUniqueKey(key)) {
    ...
    auto image = info.makeImage(
        SkSurface::MakeRenderTarget(
            ctx, SkBudgeted::kYes, info.imageInfo, msaaSampleCount, kTopLeft_GrSurfaceOrigin, props,
            createWithMips),
        fPicture.get());
    if (!image) {
      return nullptr;
  ...
  const GrSamplerState sampler(
      static_cast<GrSamplerState::WrapMode>(fTmx), static_cast<GrSamplerState::WrapMode>(fTmy),
      fFilter);
