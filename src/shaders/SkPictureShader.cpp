...
sk_sp<SkShader> SkPicture::makeShader(
    SkTileMode tmx, SkTileMode tmy, const SkMatrix* localMatrix, const SkRect* tile) const {
...
struct BitmapShaderKey : public SkResourceCache::Key {
 public:
  BitmapShaderKey(
      SkColorSpace* colorSpace, SkImage::BitDepth bitDepth, uint32_t shaderID, const SkSize& scale)
...
SkPictureShader::SkPictureShader(
    sk_sp<SkPicture> picture, SkTileMode tmx, SkTileMode tmy, const SkMatrix* localMatrix,
    const SkRect* tile)
...
sk_sp<SkShader> SkPictureShader::refBitmapShader(
    const SkMatrix& viewMatrix, SkTCopyOnFirstWrite<SkMatrix>* localMatrix,
    SkColorType dstColorType, SkColorSpace* dstColorSpace, const int maxTextureSize) const {
  ...
  if (!SkDecomposeUpper2x2(m, nullptr, &scale, nullptr)) {
    ...
    scale.set(
        SkScalarSqrt(m.getScaleX() * m.getScaleX() + m.getSkewX() * m.getSkewX()),
  ...
  const SkSize tileScale = SkSize::Make(
      SkIntToScalar(tileSize.width()) / fTile.width(),
      SkIntToScalar(tileSize.height()) / fTile.height());

  sk_sp<SkColorSpace> imgCS = dstColorSpace ? sk_ref_sp(dstColorSpace) : SkColorSpace::MakeSRGB();
  SkImage::BitDepth bitDepth =
      dstColorType >= kRGBA_F16Norm_SkColorType ? SkImage::BitDepth::kF16 : SkImage::BitDepth::kU8;
  ...
  if (!SkResourceCache::Find(key, BitmapShaderRec::Visitor, &tileShader)) {
    ...
    tileMatrix.setRectToRect(
        fTile, SkRect::MakeIWH(tileSize.width(), tileSize.height()), SkMatrix::kFill_ScaleToFit);

    sk_sp<SkImage> tileImage = SkImage::MakeFromPicture(
        fPicture, tileSize, &tileMatrix, nullptr, bitDepth, std::move(imgCS));
    if (!tileImage) {
      return nullptr;
    }
...
SkShaderBase::Context* SkPictureShader::onMakeContext(
  ...
  sk_sp<SkShader> bitmapShader =
      this->refBitmapShader(*rec.fMatrix, &lm, rec.fDstColorType, rec.fDstColorSpace);
  if (!bitmapShader) {
    return nullptr;
  }
