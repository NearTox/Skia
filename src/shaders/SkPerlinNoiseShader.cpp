...
class SkPerlinNoiseShaderImpl : public SkShaderBase {
 public:
  struct StitchData {
    constexpr StitchData() noexcept : fWidth(0), fWrapX(0), fHeight(0), fWrapY(0) {}

    StitchData(SkScalar w, SkScalar h) noexcept
    ...
    bool operator==(const StitchData& other) const noexcept {
  ...
  struct PaintingData {
    PaintingData(
        const SkISize& tileSize, SkScalar seed, SkScalar baseFrequencyX, SkScalar baseFrequencyY,
        const SkMatrix& matrix) {
      SkVector tileVec;
      matrix.mapVector(SkIntToScalar(tileSize.fWidth), SkIntToScalar(tileSize.fHeight), &tileVec);
      ...
      fBaseFrequency.set(
          baseFrequencyX * SkScalarInvert(scale.width()),
      ...
      fImprovedPermutationsBitmap.installPixels(
          info, improved_noise_permutations, info.minRowBytes());
      fImprovedPermutationsBitmap.setImmutable();

      static uint8_t gradients[] = {2, 2, 1, 0, 0, 2, 1, 0, 2, 0, 1, 0, 0, 0, 1, 0,
                                    2, 1, 2, 0, 0, 1, 2, 0, 2, 1, 0, 0, 0, 1, 0, 0,
                                    1, 2, 2, 0, 1, 0, 2, 0, 1, 2, 0, 0, 1, 0, 0, 0,
                                    2, 2, 1, 0, 1, 0, 2, 0, 0, 2, 1, 0, 1, 0, 0, 0};
...
#if SK_SUPPORT_GPU
    PaintingData(const PaintingData& that) noexcept
        : fSeed(that.fSeed),
   ...
   private:
    ...
    inline int random() noexcept {
      static constexpr int gRandAmplitude = 16807;  // 7**5; primitive root of m
      static constexpr int gRandQ = 127773;         // m / a
      static constexpr int gRandR = 2836;           // m % a

      int result = gRandAmplitude * (fSeed % gRandQ) - gRandR * (fSeed / gRandQ);
      if (result <= 0) result += kRandMaximum;
      fSeed = result;
      return result;
    }

    // Only called once. Could be part of the constructor.
    void init(SkScalar seed) noexcept {
      static constexpr SkScalar gInvBlockSizef = SkScalarInvert(SkIntToScalar(kBlockSize));
      ...
      static constexpr SkScalar gHalfMax16bits = 32767.5f;
      ...
      for (int channel = 0; channel < 4; ++channel) {
        for (int i = 0; i < kBlockSize; ++i) {
          ...
          fNoise[channel][i][0] =
              SkScalarRoundToInt((fGradient[channel][i].fX + 1) * gHalfMax16bits);
          fNoise[channel][i][1] =
              SkScalarRoundToInt((fGradient[channel][i].fY + 1) * gHalfMax16bits);
        }
      }
    }
    ...
    void stitch() noexcept {
      ...
      fStitchDataInit = StitchData(tileWidth * fBaseFrequency.fX, tileHeight * fBaseFrequency.fY);
    }

   public:
#if SK_SUPPORT_GPU
    const SkBitmap& getPermutationsBitmap() const noexcept { return fPermutationsBitmap; }

    const SkBitmap& getNoiseBitmap() const noexcept { return fNoiseBitmap; }

    const SkBitmap& getImprovedPermutationsBitmap() const noexcept {
      return fImprovedPermutationsBitmap;
    }

    const SkBitmap& getGradientBitmap() const noexcept { return fGradientBitmap; }
#endif
  };
  ...
  SkPerlinNoiseShaderImpl(
      SkPerlinNoiseShaderImpl::Type type, SkScalar baseFrequencyX, SkScalar baseFrequencyY,
      int numOctaves, SkScalar seed, const SkISize* tileSize) noexcept;
  ...
  class PerlinNoiseShaderContext : public Context {
   ...
   private:
    ...
    SkScalar calculateImprovedNoiseValueForPoint(int channel, const SkPoint& point) const noexcept;
    SkScalar noise2D(
        int channel, const StitchData& stitchData, const SkPoint& noiseVector) const noexcept;
...
inline constexpr int checkNoise(int noiseValue, int limitValue, int newValue) noexcept {
...
inline constexpr SkScalar smoothCurve(SkScalar t) noexcept { return t * t * (3 - 2 * t); }
...
void SkPerlinNoiseShaderImpl::flatten(SkWriteBuffer& buffer) const noexcept {
...
SkScalar SkPerlinNoiseShaderImpl::PerlinNoiseShaderContext::noise2D(
  ...
  struct Noise {
    ...
    Noise(SkScalar component) noexcept {
  ...
  SkPoint fractionValue = SkPoint::Make(
...
SkScalar SkPerlinNoiseShaderImpl::PerlinNoiseShaderContext::calculateTurbulenceValueForPoint(
  ...
  SkPoint noiseVector(SkPoint::Make(
      point.x() * fPaintingData.fBaseFrequency.fX, point.y() * fPaintingData.fBaseFrequency.fY));
  ...
  for (int octave = 0; octave < perlinNoiseShader.fNumOctaves; ++octave) {
    ...
    if (perlinNoiseShader.fStitchTiles) {
      ...
      stitchData =
          StitchData(SkIntToScalar(stitchData.fWidth) * 2, SkIntToScalar(stitchData.fHeight) * 2);
    }
  }
...
static constexpr SkScalar fade(SkScalar t) noexcept { return t * t * t * (t * (t * 6 - 15) + 10); }

static constexpr SkScalar lerp(SkScalar t, SkScalar a, SkScalar b) noexcept {
  return a + t * (b - a);
}

static constexpr SkScalar grad(int hash, SkScalar x, SkScalar y, SkScalar z) noexcept {
  int h = hash & 15;
  SkScalar u = h < 8 ? x : y;
...
SkScalar SkPerlinNoiseShaderImpl::PerlinNoiseShaderContext::calculateImprovedNoiseValueForPoint(
    int channel, const SkPoint& point) const noexcept {
  const SkPerlinNoiseShaderImpl& perlinNoiseShader =
      static_cast<const SkPerlinNoiseShaderImpl&>(fShader);
  ...
  static constexpr SkScalar CHANNEL_DELTA = 1000.0f;
  ...
  for (int i = 0; i < perlinNoiseShader.fNumOctaves; i++) {
    ...
    result +=
        lerp(
            ...
            lerp(
                v,
                lerp(u, grad(permutations[AA], px, py, pz), grad(permutations[BA], px - 1, py, pz)),
                lerp(
                    u, grad(permutations[AB], px, py - 1, pz),
                    grad(permutations[BB], px - 1, py - 1, pz))),
            lerp(
                v,
                lerp(
                    u, grad(permutations[AA + 1], px, py, pz - 1),
                    grad(permutations[BA + 1], px - 1, py, pz - 1)),
                lerp(
                    u, grad(permutations[AB + 1], px, py - 1, pz - 1),
...
static inline SkMatrix total_matrix(
    const SkShaderBase::ContextRec& rec, const SkShaderBase& shader) noexcept {
...
SkPerlinNoiseShaderImpl::PerlinNoiseShaderContext::PerlinNoiseShaderContext(
    ...
    : INHERITED(shader, rec),
      ...
      ,
      fPaintingData(
          shader.fTileSize, shader.fSeed, shader.fBaseFrequencyX, shader.fBaseFrequencyY, fMatrix) {
  // This (1,1) translation is due to WebKit's 1 based coordinates for the noise
  // (as opposed to 0 based, usually). The same adjustment is in the setData() function.
  fMatrix.setTranslate(
      -fMatrix.getTranslateX() + SK_Scalar1, -fMatrix.getTranslateY() + SK_Scalar1);
}
...
class GrPerlinNoise2Effect : public GrFragmentProcessor {
 public:
  static std::unique_ptr<GrFragmentProcessor> Make(
      ...
      GrSurfaceProxyView permutationsView, GrSurfaceProxyView noiseView, const SkMatrix& matrix) {
    return std::unique_ptr<GrFragmentProcessor>(new GrPerlinNoise2Effect(
        type, numOctaves, stitchTiles, std::move(paintingData), std::move(permutationsView),
        std::move(noiseView), matrix));
  }
  ...
  const SkPerlinNoiseShaderImpl::StitchData& stitchData() const noexcept {
    return fPaintingData->fStitchDataInit;
  }

  SkPerlinNoiseShaderImpl::Type type() const noexcept { return fType; }
  bool stitchTiles() const noexcept { return fStitchTiles; }
  const SkVector& baseFrequency() const noexcept { return fPaintingData->fBaseFrequency; }
  int numOctaves() const noexcept { return fNumOctaves; }
  const SkMatrix& matrix() const noexcept { return fCoordTransform.matrix(); }
...
std::unique_ptr<GrFragmentProcessor> GrPerlinNoise2Effect::TestCreate(GrProcessorTestData* d) {
  ...
  SkISize tileSize =
      SkISize::Make(d->fRandom->nextRangeU(4, 4096), d->fRandom->nextRangeU(4, 4096));
...
void GrGLPerlinNoise::emitCode(EmitArgs& args) {
  ...
  SkString vCoords =
      fragBuilder->ensureCoords2D(args.fTransformedCoords[0].fVaryingPoint, pne.sampleMatrix());

  fBaseFrequencyUni =
      uniformHandler->addUniform(&pne, kFragment_GrShaderFlag, kHalf2_GrSLType, "baseFrequency");
  const char* baseFrequencyUni = uniformHandler->getUniformCStr(fBaseFrequencyUni);

  const char* stitchDataUni = nullptr;
  if (pne.stitchTiles()) {
    fStitchDataUni =
        uniformHandler->addUniform(&pne, kFragment_GrShaderFlag, kHalf2_GrSLType, "stitchData");
  ...
  noiseCode.appendf(
      "\n\thalf2 %s = %s * %s * (half2(3.0) - half2(2.0) * %s);", noiseSmooth, fractVal, fractVal,
      fractVal);

  // Adjust frequencies if we're stitching tiles
  if (pne.stitchTiles()) {
    noiseCode.appendf(
        "\n\tif(%s.x >= %s.x) { %s.x -= %s.x; }", floorVal, stitchData, floorVal, stitchData);
    noiseCode.appendf(
        "\n\tif(%s.y >= %s.y) { %s.y -= %s.y; }", floorVal, stitchData, floorVal, stitchData);
    noiseCode.appendf(
        "\n\tif(%s.z >= %s.x) { %s.z -= %s.x; }", floorVal, stitchData, floorVal, stitchData);
    noiseCode.appendf(
        "\n\tif(%s.w >= %s.y) { %s.w -= %s.y; }", floorVal, stitchData, floorVal, stitchData);
  }
...
#  if defined(SK_BUILD_FOR_ANDROID)
  ...
  noiseCode.appendf(
      "\n\t%s = floor(%s * half2(255.0) + half2(0.5)) * half2(0.003921569);", latticeIdx,
      latticeIdx);
#  endif
  ...
  if (pne.stitchTiles()) {
    fragBuilder->emitFunction(
        kHalf_GrSLType, "perlinnoise", SK_ARRAY_COUNT(gPerlinNoiseStitchArgs),
        gPerlinNoiseStitchArgs, noiseCode.c_str(), &noiseFuncName);
  } else {
    fragBuilder->emitFunction(
        kHalf_GrSLType, "perlinnoise", SK_ARRAY_COUNT(gPerlinNoiseArgs), gPerlinNoiseArgs,
        noiseCode.c_str(), &noiseFuncName);
  }

  // There are rounding errors if the floor operation is not performed here
  fragBuilder->codeAppendf(
      "\n\t\thalf2 %s = half2(floor(%s.xy) * %s);", noiseVec, vCoords.c_str(), baseFrequencyUni);
  ...
  if (pne.stitchTiles()) {
    fragBuilder->codeAppendf(
        "half4(\n\t\t\t\t%s(%s, %s, %s),\n\t\t\t\t%s(%s, %s, %s),"
        "\n\t\t\t\t%s(%s, %s, %s),\n\t\t\t\t%s(%s, %s, %s))",
        noiseFuncName.c_str(), chanCoordR, noiseVec, stitchData, noiseFuncName.c_str(), chanCoordG,
        noiseVec, stitchData, noiseFuncName.c_str(), chanCoordB, noiseVec, stitchData,
        noiseFuncName.c_str(), chanCoordA, noiseVec, stitchData);
  } else {
    fragBuilder->codeAppendf(
        "half4(\n\t\t\t\t%s(%s, %s),\n\t\t\t\t%s(%s, %s),"
        "\n\t\t\t\t%s(%s, %s),\n\t\t\t\t%s(%s, %s))",
        noiseFuncName.c_str(), chanCoordR, noiseVec, noiseFuncName.c_str(), chanCoordG, noiseVec,
        noiseFuncName.c_str(), chanCoordB, noiseVec, noiseFuncName.c_str(), chanCoordA, noiseVec);
  }
  if (pne.type() != SkPerlinNoiseShaderImpl::kFractalNoise_Type) {
  ...
  if (pne.type() == SkPerlinNoiseShaderImpl::kFractalNoise_Type) {
    ...
    // by fractalNoise and (turbulenceFunctionResult) by turbulence.
    fragBuilder->codeAppendf(
        "\n\t\t%s = %s * half4(0.5) + half4(0.5);", args.fOutputColor, args.fOutputColor);
  }
...
void GrGLPerlinNoise::GenKey(
  ...
  switch (turbulence.type()) {
    case SkPerlinNoiseShaderImpl::kFractalNoise_Type: key |= 0x1; break;
    case SkPerlinNoiseShaderImpl::kTurbulence_Type: key |= 0x2; break;
    default:
      // leave key at 0
      break;
  }
...
void GrGLPerlinNoise::onSetData(
  ...
  if (turbulence.stitchTiles()) {
    ...
    pdman.set2f(
        fStitchDataUni, SkIntToScalar(stitchData.fWidth), SkIntToScalar(stitchData.fHeight));
  }
}
...
class GrImprovedPerlinNoiseEffect : public GrFragmentProcessor {
 public:
  static std::unique_ptr<GrFragmentProcessor> Make(
      int octaves, SkScalar z, std::unique_ptr<SkPerlinNoiseShaderImpl::PaintingData> paintingData,
    ...
    return std::unique_ptr<GrFragmentProcessor>(new GrImprovedPerlinNoiseEffect(
        octaves, z, std::move(paintingData), std::move(permutationsView), std::move(gradientView),
        matrix));
  }
  ...
  int octaves() const noexcept { return fOctaves; }
  const SkMatrix& matrix() const noexcept { return fCoordTransform.matrix(); }
 ...
 private:
  ...
  GrImprovedPerlinNoiseEffect(
      int octaves, SkScalar z, std::unique_ptr<SkPerlinNoiseShaderImpl::PaintingData> paintingData,
      GrSurfaceProxyView permutationsView, GrSurfaceProxyView gradientView, const SkMatrix& matrix)
...
std::unique_ptr<GrFragmentProcessor> GrImprovedPerlinNoiseEffect::TestCreate(
  ...
  sk_sp<SkShader> shader(
      SkPerlinNoiseShader::MakeImprovedNoise(baseFrequencyX, baseFrequencyY, numOctaves, z));
...
void GrGLImprovedPerlinNoise::emitCode(EmitArgs& args) {
  ...
  // fade function
  const GrShaderVar fadeArgs[] = {GrShaderVar("t", kHalf3_GrSLType)};
  SkString fadeFuncName;
  fragBuilder->emitFunction(
      kHalf3_GrSLType, "fade", SK_ARRAY_COUNT(fadeArgs), fadeArgs,
      "return t * t * t * (t * (t * 6.0 - 15.0) + 10.0);", &fadeFuncName);

  // perm function
  const GrShaderVar permArgs[] = {GrShaderVar("x", kHalf_GrSLType)};
  ...
  fragBuilder->appendTextureLookup(
      &permCode, args.fTexSamplers[0], "float2(fract(x / 256.0), 0.0)");
  ...
  noiseCode.appendf(
      "%s(%s(%s(%s(AA), p),", lerpFuncName.c_str(), lerpFuncName.c_str(), gradFuncName.c_str(),
      permFuncName.c_str());
  noiseCode.appendf(
      "%s(%s(BA), p + half3(-1.0, 0.0, 0.0)), f.x),", gradFuncName.c_str(), permFuncName.c_str());
  noiseCode.appendf(
      "%s(%s(%s(AB), p + half3(0.0, -1.0, 0.0)),", lerpFuncName.c_str(), gradFuncName.c_str(),
      permFuncName.c_str());
  noiseCode.appendf(
      "%s(%s(BB), p + half3(-1.0, -1.0, 0.0)), f.x), f.y),", gradFuncName.c_str(),
      permFuncName.c_str());
  noiseCode.appendf(
      "%s(%s(%s(%s(AA + 1.0), p + half3(0.0, 0.0, -1.0)),", lerpFuncName.c_str(),
      lerpFuncName.c_str(), gradFuncName.c_str(), permFuncName.c_str());
  noiseCode.appendf(
      "%s(%s(BA + 1.0), p + half3(-1.0, 0.0, -1.0)), f.x),", gradFuncName.c_str(),
      permFuncName.c_str());
  noiseCode.appendf(
      "%s(%s(%s(AB + 1.0), p + half3(0.0, -1.0, -1.0)),", lerpFuncName.c_str(),
      gradFuncName.c_str(), permFuncName.c_str());
  noiseCode.appendf(
      "%s(%s(BB + 1.0), p + half3(-1.0, -1.0, -1.0)), f.x), f.y), f.z);", gradFuncName.c_str(),
      permFuncName.c_str());
  noiseCode.append("return result;");
  fragBuilder->emitFunction(
      kHalf_GrSLType, "noise", SK_ARRAY_COUNT(noiseArgs), noiseArgs, noiseCode.c_str(),
      &noiseFuncName);

  // noiseOctaves function
  const GrShaderVar noiseOctavesArgs[] = {GrShaderVar("p", kHalf3_GrSLType)};
  ...
  fragBuilder->emitFunction(
      kHalf_GrSLType, "noiseOctaves", SK_ARRAY_COUNT(noiseOctavesArgs), noiseOctavesArgs,
      noiseOctavesCode.c_str(), &noiseOctavesFuncName);

  fragBuilder->codeAppendf("half2 coords = half2(%s * %s);", vCoords.c_str(), baseFrequencyUni);
  fragBuilder->codeAppendf("half r = %s(half3(coords, %s));", noiseOctavesFuncName.c_str(), zUni);
  fragBuilder->codeAppendf(
      "half g = %s(half3(coords, %s + 0000.0));", noiseOctavesFuncName.c_str(), zUni);
  fragBuilder->codeAppendf(
      "half b = %s(half3(coords, %s + 0000.0));", noiseOctavesFuncName.c_str(), zUni);
  fragBuilder->codeAppendf(
      "half a = %s(half3(coords, %s + 0000.0));", noiseOctavesFuncName.c_str(), zUni);
...
std::unique_ptr<GrFragmentProcessor> SkPerlinNoiseShaderImpl::asFragmentProcessor(
  ...
  std::unique_ptr<SkPerlinNoiseShaderImpl::PaintingData> paintingData =
      std::make_unique<SkPerlinNoiseShaderImpl::PaintingData>(
          fTileSize, fSeed, fBaseFrequencyX, fBaseFrequencyY, paintMatrix);
  ...
  if (fType == kImprovedNoise_Type) {
    ...
    return GrImprovedPerlinNoiseEffect::Make(
        fNumOctaves, fSeed, std::move(paintingData), std::move(permutationsView),
  ...
  if (0 == fNumOctaves) {
    if (kFractalNoise_Type == fType) {
      ...
      auto inner = GrConstColorProcessor::Make(
          SkPMColor4f::FromBytes_RGBA(0x80404040), GrConstColorProcessor::InputMode::kModulateRGBA);
  ...
  if (permutationsView.proxy() && noiseView.proxy()) {
    auto inner = GrPerlinNoise2Effect::Make(
        fType, fNumOctaves, fStitchTiles, std::move(paintingData), std::move(permutationsView),
        std::move(noiseView), m);
    return GrFragmentProcessor::MulChildByInputAlpha(std::move(inner));
  }
  return nullptr;
}
...
static bool valid_input(
    SkScalar baseX, SkScalar baseY, int numOctaves, const SkISize* tileSize,
    SkScalar seed) noexcept {
  if (!(baseX >= 0 && baseY >= 0)) {
    return false;
  }
...
sk_sp<SkShader> SkPerlinNoiseShader::MakeFractalNoise(
    SkScalar baseFrequencyX, SkScalar baseFrequencyY, int numOctaves, SkScalar seed,
    const SkISize* tileSize) {
  if (!valid_input(baseFrequencyX, baseFrequencyY, numOctaves, tileSize, seed)) {
    return nullptr;
  }
  return sk_sp<SkShader>(new SkPerlinNoiseShaderImpl(
      SkPerlinNoiseShaderImpl::kFractalNoise_Type, baseFrequencyX, baseFrequencyY, numOctaves, seed,
      tileSize));
}

sk_sp<SkShader> SkPerlinNoiseShader::MakeTurbulence(
    SkScalar baseFrequencyX, SkScalar baseFrequencyY, int numOctaves, SkScalar seed,
    const SkISize* tileSize) {
  if (!valid_input(baseFrequencyX, baseFrequencyY, numOctaves, tileSize, seed)) {
    return nullptr;
  ...
  return sk_sp<SkShader>(new SkPerlinNoiseShaderImpl(
      SkPerlinNoiseShaderImpl::kTurbulence_Type, baseFrequencyX, baseFrequencyY, numOctaves, seed,
      tileSize));
}

sk_sp<SkShader> SkPerlinNoiseShader::MakeImprovedNoise(
    SkScalar baseFrequencyX, SkScalar baseFrequencyY, int numOctaves, SkScalar z) {
  if (!valid_input(baseFrequencyX, baseFrequencyY, numOctaves, nullptr, z)) {
    return nullptr;
  }
  return sk_sp<SkShader>(new SkPerlinNoiseShaderImpl(
      SkPerlinNoiseShaderImpl::kImprovedNoise_Type, baseFrequencyX, baseFrequencyY, numOctaves, z,
      nullptr));
}
