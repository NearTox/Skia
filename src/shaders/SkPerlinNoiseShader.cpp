 ...
 public:
  ...
  struct PaintingData {
    PaintingData(
      ...
      fImprovedPermutationsImage = SkImage::MakeFromRaster(impPermutationsPixmap, nullptr, nullptr);
   ...
   public:
#if SK_SUPPORT_GPU
    const sk_sp<SkImage> getPermutationsImage() const noexcept { return fPermutationsImage; }

    const sk_sp<SkImage> getNoiseImage() const noexcept { return fNoiseImage; }

    const sk_sp<SkImage> getImprovedPermutationsImage() const noexcept {
      return fImprovedPermutationsImage;
    }

    const sk_sp<SkImage> getGradientImage() const noexcept { return fGradientImage; }
#endif
  };
  ...
  class PerlinNoiseShaderContext : public Context {
   ...
   private:
    ...
    SkScalar calculateTurbulenceValueForPoint(
        int channel, StitchData& stitchData, const SkPoint& point) const noexcept;
...
constexpr inline int checkNoise(int noiseValue, int limitValue, int newValue) noexcept {
...
constexpr inline SkScalar smoothCurve(SkScalar t) noexcept { return t * t * (3 - 2 * t); }
 ...
 public:
  static std::unique_ptr<GrFragmentProcessor> Make(
    ...
    return std::unique_ptr<GrFragmentProcessor>(new GrPerlinNoise2Effect(
        type, numOctaves, stitchTiles, std::move(paintingData), std::move(permutationsProxy),
        std::move(noiseProxy), matrix));
  }
  ...
  const SkMatrix& matrix() const noexcept { return fCoordTransform.getMatrix(); }
...
void GrGLPerlinNoise::emitCode(EmitArgs& args) {
  ...
  fBaseFrequencyUni =
      uniformHandler->addUniform(kFragment_GrShaderFlag, kHalf2_GrSLType, "baseFrequency");
  const char* baseFrequencyUni = uniformHandler->getUniformCStr(fBaseFrequencyUni);

  const char* stitchDataUni = nullptr;
  if (pne.stitchTiles()) {
    fStitchDataUni =
        uniformHandler->addUniform(kFragment_GrShaderFlag, kHalf2_GrSLType, "stitchData");
  ...
  // Get permutation for x
  {
    ...
    fragBuilder->appendTextureLookup(
        &noiseCode, args.fTexSamplers[0], xCoords.c_str(), kHalf2_GrSLType);
    noiseCode.append(".r;");
  }
  ...
  // Compute u, at offset (0,0)
  {
    ...
    noiseCode.appendf("\n\thalf4 %s = ", lattice);
    fragBuilder->appendTextureLookup(
        &noiseCode, args.fTexSamplers[1], latticeCoords.c_str(), kHalf2_GrSLType);
 ...
 public:
  static std::unique_ptr<GrFragmentProcessor> Make(
    ...
    return std::unique_ptr<GrFragmentProcessor>(new GrImprovedPerlinNoiseEffect(
        octaves, z, std::move(paintingData), std::move(permutationsProxy), std::move(gradientProxy),
        matrix));
  }
...
void GrGLImprovedPerlinNoise::emitCode(EmitArgs& args) {
  ...
  fragBuilder->appendTextureLookup(
      &permCode, args.fTexSamplers[0], "float2(fract(x / 256.0), 0.0)", kHalf2_GrSLType);
  ...
  fragBuilder->appendTextureLookup(
      &gradCode, args.fTexSamplers[1], "float2(fract(x / 16.0), 0.0)", kHalf2_GrSLType);
...
std::unique_ptr<GrFragmentProcessor> SkPerlinNoiseShaderImpl::asFragmentProcessor(
  ...
  std::unique_ptr<SkPerlinNoiseShaderImpl::PaintingData> paintingData =
      skstd::make_unique<SkPerlinNoiseShaderImpl::PaintingData>(
  ...
  if (fType == kImprovedNoise_Type) {
    ...
    return GrImprovedPerlinNoiseEffect::Make(
        fNumOctaves, fSeed, std::move(paintingData), std::move(permutationsTexture),
  ...
  sk_sp<GrTextureProxy> noiseProxy = GrMakeCachedImageProxy(proxyProvider, std::move(noiseImage));

  if (permutationsProxy && noiseProxy) {
    auto inner = GrPerlinNoise2Effect::Make(
        fType, fNumOctaves, fStitchTiles, std::move(paintingData), std::move(permutationsProxy),
        std::move(noiseProxy), m);
    return GrFragmentProcessor::MulChildByInputAlpha(std::move(inner));
  }
  return nullptr;
}
