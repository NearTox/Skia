 ...
 public:
  static std::unique_ptr<GrFragmentProcessor> Make(
    ...
    return GrMatrixEffect::Make(
        matrix, std::unique_ptr<GrFragmentProcessor>(new GrPerlinNoise2Effect(
...
#  endif
  ...
  if (pne.stitchTiles()) {
    fragBuilder->emitFunction(
        kHalf_GrSLType, noiseFuncName.c_str(),
        {gPerlinNoiseStitchArgs, SK_ARRAY_COUNT(gPerlinNoiseStitchArgs)}, noiseCode.c_str());
  } else {
    fragBuilder->emitFunction(
        kHalf_GrSLType, noiseFuncName.c_str(), {gPerlinNoiseArgs, SK_ARRAY_COUNT(gPerlinNoiseArgs)},
        noiseCode.c_str());
  }
...
class GrImprovedPerlinNoiseEffect : public GrFragmentProcessor {
 public:
  static std::unique_ptr<GrFragmentProcessor> Make(
    ...
    return GrMatrixEffect::Make(
        matrix, std::unique_ptr<GrFragmentProcessor>(new GrImprovedPerlinNoiseEffect(
 ...
 private:
  ...
  GrImprovedPerlinNoiseEffect(const GrImprovedPerlinNoiseEffect& that)
      : INHERITED(kGrImprovedPerlinNoiseEffect_ClassID, kNone_OptimizationFlags),
        ...
        fZ(that.fZ),
        fPaintingData(
            std::make_unique<SkPerlinNoiseShaderImpl::PaintingData>(*that.fPaintingData)) {
    this->cloneAndRegisterAllChildProcessors(that);
    this->setUsesSampleCoordsDirectly();
  }
...
void GrGLImprovedPerlinNoise::emitCode(EmitArgs& args) {
  ...
  fragBuilder->emitFunction(
      kHalf3_GrSLType, fadeFuncName.c_str(), {fadeArgs, SK_ARRAY_COUNT(fadeArgs)},
  ...
  fragBuilder->emitFunction(
      kHalf_GrSLType, permFuncName.c_str(), {permArgs, SK_ARRAY_COUNT(permArgs)}, permCode.c_str());
  ...
  fragBuilder->emitFunction(
      kHalf_GrSLType, gradFuncName.c_str(), {gradArgs, SK_ARRAY_COUNT(gradArgs)}, gradCode.c_str());
  ...
  fragBuilder->emitFunction(
      kHalf_GrSLType, lerpFuncName.c_str(), {lerpArgs, SK_ARRAY_COUNT(lerpArgs)},
      "return a + w * (b - a);");
  ...
  fragBuilder->emitFunction(
      kHalf_GrSLType, noiseFuncName.c_str(), {noiseArgs, SK_ARRAY_COUNT(noiseArgs)},
      noiseCode.c_str());
  ...
  fragBuilder->emitFunction(
      kHalf_GrSLType, noiseOctavesFuncName.c_str(),
      {noiseOctavesArgs, SK_ARRAY_COUNT(noiseOctavesArgs)}, noiseOctavesCode.c_str());
