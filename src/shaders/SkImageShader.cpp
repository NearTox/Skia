...
std::unique_ptr<GrFragmentProcessor> SkImageShader::asFragmentProcessor(
  ...
  if (doBicubic) {
    ...
    inner = GrSimpleTextureEffect::Make(std::move(proxy), srcAlphaType, lmInverse, samplerState);
    if (domainX != GrTextureDomain::kIgnore_Mode || domainY != GrTextureDomain::kIgnore_Mode) {
      SkRect domain =
          GrTextureDomain::MakeTexelDomain(SkIRect::MakeSize(dimensions), domainX, domainY);
      inner =
          GrDomainEffect::Make(std::move(inner), domain, domainX, domainY, samplerState.filter());
    }
  }
...
class SkImageStageUpdater : public SkStageUpdater {
 public:
  ...
  bool update(const SkMatrix& ctm, const SkMatrix* localM) override {
    SkMatrix matrix;
    return fShader->computeTotalInverse(ctm, localM, &matrix) && matrix.asAffine(fMatrixStorage);
  }
};

bool SkImageShader::doStages(const SkStageRec& rec, SkImageStageUpdater* updater) const {
  if (updater &&
      (rec.fPaint.getFilterQuality() == kMedium_SkFilterQuality || rec.fCTM.hasPerspective())) {
  ...
  auto append_misc = [&] {
    ...
    if (quality > kLow_SkFilterQuality) {
      ...
      p->append(fClampAsIfUnpremul ? SkRasterPipeline::clamp_1 : SkRasterPipeline::clamp_a);
    }
    ...
    if (rec.fDstCS) {
      ...
      alloc
          ->make<SkColorSpaceXformSteps>(
              srcCS, kPremul_SkAlphaType, rec.fDstCS, kPremul_SkAlphaType)
