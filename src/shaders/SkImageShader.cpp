...
#endif
  return SkM44(
      (1.f / 6) * B, -(3.f / 6) * B - C, (3.f / 6) * B + 2 * C, -(1.f / 6) * B - C,
      1 - (2.f / 6) * B, 0, -3 + (12.f / 6) * B + C, 2 - (9.f / 6) * B - C, (1.f / 6) * B,
      (3.f / 6) * B + C, 3 - (15.f / 6) * B - 2 * C, -2 + (9.f / 6) * B + C, 0, 0, -C,
      (1.f / 6) * B + C);
}
...
SkImageShader::SkImageShader(
    sk_sp<SkImage> img, SkTileMode tmx, SkTileMode tmy, const SkMatrix* localMatrix,
    FilterEnum filtering, bool clampAsIfUnpremul)
...
SkImageShader::SkImageShader(
    sk_sp<SkImage> img, SkTileMode tmx, SkTileMode tmy, const SkFilterOptions& options,
...
SkImageShader::SkImageShader(
    sk_sp<SkImage> img, SkTileMode tmx, SkTileMode tmy, SkImage::CubicResampler cubic,
    const SkMatrix* localMatrix)
    : INHERITED(localMatrix),
      fImage(std::move(img)),
      fTileModeX(optimize(tmx, fImage->width())),
...
sk_sp<SkFlattenable> SkImageShader::CreateProc(SkReadBuffer& buffer) {
  ...
  switch (fe) {
    case kUseFilterOptions: return SkImageShader::Make(std::move(img), tmx, tmy, fo, &localMatrix);
    case kUseCubicResampler:
...
static bool legacy_shader_can_handle(const SkMatrix& inv) {
  ...
  if (!SkRect::MakeLTRB(-max_fixed32dot32, -max_fixed32dot32, +max_fixed32dot32, +max_fixed32dot32)
           .contains(src)) {
    return false;
  }
...
SkShaderBase::Context* SkImageShader::onMakeContext(
  ...
  if (fImage->width() > 32767 || fImage->height() > 32767) {
    return nullptr;
  }
  ...
  return SkBitmapProcLegacyShader::MakeContext(
      *this, fTileModeX, fTileModeY, as_IB(fImage.get()), modifiedRec, alloc);
}
#endif
...
sk_sp<SkShader> SkImageShader::Make(
    sk_sp<SkImage> image, SkTileMode tmx, SkTileMode tmy, const SkMatrix* localMatrix,
    FilterEnum filtering, bool clampAsIfUnpremul) {
  if (!image) {
...
sk_sp<SkShader> SkImageShader::Make(
    sk_sp<SkImage> image, SkTileMode tmx, SkTileMode tmy, const SkFilterOptions& options,
    const SkMatrix* localMatrix) {
  if (!image) {
    return sk_make_sp<SkEmptyShader>();
  }
  return sk_sp<SkShader>{new SkImageShader(image, tmx, tmy, options, localMatrix)};
}

sk_sp<SkShader> SkImageShader::Make(
    sk_sp<SkImage> image, SkTileMode tmx, SkTileMode tmy, SkImage::CubicResampler cubic,
    const SkMatrix* localMatrix) {
  if (!(cubic.B >= 0 && cubic.B <= 1 && cubic.C >= 0 && cubic.C <= 1)) {
    return nullptr;
  }
  if (!image) {
    return sk_make_sp<SkEmptyShader>();
  }
  return sk_sp<SkShader>{new SkImageShader(image, tmx, tmy, cubic, localMatrix)};
}
...
std::unique_ptr<GrFragmentProcessor> SkImageShader::asFragmentProcessor(
  ...
  static constexpr size_t kSize = std::max(
      {sizeof(GrYUVAImageTextureMaker), sizeof(GrTextureAdjuster), sizeof(GrImageTextureMaker),
       sizeof(GrBitmapTextureMaker)});
  static constexpr size_t kAlign = std::max(
      {alignof(GrYUVAImageTextureMaker), alignof(GrTextureAdjuster), alignof(GrImageTextureMaker),
       alignof(GrBitmapTextureMaker)});
  std::aligned_storage_t<kSize, kAlign> storage;
  GrTextureProducer* producer = nullptr;
  SkScopeExit destroyProducer([&producer] {
    if (producer) {
  ...
  if (as_IB(fImage)->isYUVA()) {
    producer = new (&storage) GrYUVAImageTextureMaker(args.fContext, fImage.get());
      ...
      GrSurfaceProxyView view = as_IB(fImage)->refPinnedView(args.fContext, &pinnedUniqueID)) {
    GrColorInfo colorInfo;
    if (args.fContext->priv().caps()->isFormatSRGB(view.proxy()->backendFormat())) {
      SkASSERT(fImage->colorType() == kRGBA_8888_SkColorType);
      colorInfo =
          GrColorInfo(GrColorType::kRGBA_8888_SRGB, fImage->alphaType(), fImage->refColorSpace());
    } else {
      colorInfo = fImage->imageInfo().colorInfo();
    }
    producer =
        new (&storage) GrTextureAdjuster(args.fContext, std::move(view), colorInfo, pinnedUniqueID);
  } else if (fImage->isLazyGenerated()) {
    producer =
        new (&storage) GrImageTextureMaker(args.fContext, fImage.get(), GrImageTexGenPolicy::kDraw);
  } else if (as_IB(fImage)->getROPixels(nullptr, &bm)) {
    producer = new (&storage) GrBitmapTextureMaker(args.fContext, bm, GrImageTexGenPolicy::kDraw);
  } else {
    return nullptr;
  }
  ...
  switch (fFilterEnum) {
    ...
    default:  // none, low, medium, high
      std::tie(fm, mm, bicubic) = GrInterpretFilterQuality(
          fImage->dimensions(), this->resolveFiltering(args.fFilterQuality),
          args.fMatrixProvider.localToDevice(), *lm, sharpen, args.fAllowFilterQualityReduction);
      break;
  }
  ...
  fp = GrColorSpaceXformEffect::Make(
      std::move(fp), fImage->colorSpace(), producer->alphaType(), args.fDstColorInfo->colorSpace(),
      kPremul_SkAlphaType);
...
sk_sp<SkShader> SkMakeBitmapShader(
    const SkBitmap& src, SkTileMode tmx, SkTileMode tmy, const SkMatrix* localMatrix,
    SkCopyPixelsMode cpm) {
  return SkImageShader::Make(
      SkMakeImageFromRasterBitmap(src, cpm), tmx, tmy, localMatrix,
      SkImageShader::kInheritFromPaint);
}

sk_sp<SkShader> SkMakeBitmapShaderForPaint(
    const SkPaint& paint, const SkBitmap& src, SkTileMode tmx, SkTileMode tmy,
...
class SkImageStageUpdater : public SkStageUpdater {
 public:
  SkImageStageUpdater(const SkImageShader* shader, bool usePersp)
      : fShader(shader), fUsePersp(usePersp || as_SB(shader)->getLocalMatrix().hasPerspective()) {}
...
static void tweak_quality_and_inv_matrix(SkFilterQuality* quality, SkMatrix* matrix) {
  ...
  if (*quality == kLow_SkFilterQuality && matrix->getType() <= SkMatrix::kTranslate_Mask &&
  ...
  if (*quality == kNone_SkFilterQuality) {
    if (matrix->getScaleX() >= 0) {
      matrix->setTranslateX(nextafterf(matrix->getTranslateX(), floorf(matrix->getTranslateX())));
    }
    if (matrix->getScaleY() >= 0) {
      matrix->setTranslateY(nextafterf(matrix->getTranslateY(), floorf(matrix->getTranslateY())));
    }
  }
}
...
bool SkImageShader::doStages(const SkStageRec& rec, SkImageStageUpdater* updater) const {
  ...
  switch (fFilterEnum) {
    case FilterEnum::kUseFilterOptions:
    case FilterEnum::kUseCubicResampler: return false;  // TODO: support these in stages
    case FilterEnum::kInheritFromPaint: quality = rec.fPaint.getFilterQuality(); break;
    default: quality = (SkFilterQuality)fFilterEnum; break;
  }
  ...
  const auto* state =
      SkBitmapController::RequestBitmap(as_IB(fImage.get()), matrix, quality, alloc);
  if (!state) {
    return false;
  }
  ...
  auto append_tiling_and_gather = [&] {
    ...
    switch (info.colorType()) {
      ...
      case kR16G16B16A16_unorm_SkColorType:
        p->append(SkRasterPipeline::gather_16161616, ctx);
        break;
      ...
      case kGray_8_SkColorType:
        p->append(SkRasterPipeline::gather_a8, ctx);
        p->append(SkRasterPipeline::alpha_to_gray);
        break;
      ...
      case kBGRA_1010102_SkColorType:
        p->append(SkRasterPipeline::gather_1010102, ctx);
  ...
  auto append_misc = [&] {
    ...
    if (quality == kHigh_SkFilterQuality) {
      ...
      p->append(
          at == kUnpremul_SkAlphaType || fClampAsIfUnpremul ? SkRasterPipeline::clamp_1
  ...
  if (true && (ct == kRGBA_8888_SkColorType || ct == kBGRA_8888_SkColorType) &&
      quality == kLow_SkFilterQuality && fTileModeX == SkTileMode::kClamp &&
  ...
  if (true && (ct == kRGBA_8888_SkColorType || ct == kBGRA_8888_SkColorType)  // TODO: all formats
      && quality == kLow_SkFilterQuality && fTileModeX != SkTileMode::kDecal  // TODO decal too?
  ...
  if (true && (ct == kRGBA_8888_SkColorType || ct == kBGRA_8888_SkColorType) &&
      quality == kHigh_SkFilterQuality && fTileModeX == SkTileMode::kClamp &&
      fTileModeY == SkTileMode::kClamp) {
  ...
  if (true && (ct == kRGBA_8888_SkColorType || ct == kBGRA_8888_SkColorType)   // TODO: all formats
      && quality == kHigh_SkFilterQuality && fTileModeX != SkTileMode::kDecal  // TODO decal too?
      && fTileModeY != SkTileMode::kDecal) {
    auto ctx = alloc->make<SkRasterPipeline_SamplerCtx2>();
    *(SkRasterPipeline_GatherCtx*)(ctx) = *gather;
    ctx->ct = ct;
  ...
  auto sample = [&](SkRasterPipeline::StockStage setup_x, SkRasterPipeline::StockStage setup_y) {
...
skvm::Color SkImageShader::onProgram(
    skvm::Builder* p, skvm::Coord device, skvm::Coord origLocal, skvm::Color paint,
    const SkMatrixProvider& matrices, const SkMatrix* localM, SkFilterQuality paintQuality,
  ...
  const SkPixmap *upper = nullptr, *lower = nullptr;
  SkMatrix upperInv;
  float lowerWeight = 0;
  ...
  auto post_scale = [&](SkISize level, const SkMatrix& base) {
    return SkMatrix::Scale(
               SkIntToScalar(level.width()) / fImage->width(),
  ...
  if (fFilterEnum == kUseFilterOptions) {
    auto* access =
        alloc->make<SkMipmapAccessor>(as_IB(fImage.get()), baseInv, fFilterOptions.fMipmap);
    upper = &access->level();
    ...
    auto* access = alloc->make<SkMipmapAccessor>(as_IB(fImage.get()), baseInv, SkMipmapMode::kNone);
    upper = &access->level();
    upperInv = post_scale(upper->dimensions(), baseInv);
  ...
  const bool input_is_opaque =
      SkAlphaTypeIsOpaque(upper->alphaType()) || SkColorTypeIsAlwaysOpaque(upper->colorType());
  ...
  //
  struct Uniforms {
    skvm::F32 w, iw, i2w, h, ih, i2h;

    skvm::F32 clamp_w, clamp_h;
  ...
  auto sample_texel = [&](const Uniforms& u, skvm::F32 sx, skvm::F32 sy) -> skvm::Color {
    ...
    auto repeat = [&](skvm::F32 v, skvm::F32 S, skvm::F32 I) { return v - floor(v * I) * S; };
    auto mirror = [&](skvm::F32 v, skvm::F32 S, skvm::F32 I2) {
      // abs( (v-scale) - (2*scale)*floor((v-scale)*(0.5f/scale)) - scale )
      //      {---A---}   {------------------B------------------}
      skvm::F32 A = v - S, B = (S + S) * floor(A * I2);
      return abs(A - B - S);
    };
    switch (fTileModeX) {
    ...
    skvm::F32 clamped_x = clamp(sx, 0, u.clamp_w), clamped_y = clamp(sy, 0, u.clamp_h);

    // Load pixels from pm.addr()[(int)sx + (int)sy*stride].
    skvm::I32 index = trunc(clamped_x) + trunc(clamped_y) * u.rowBytesAsPixels;
    ...
    if (fTileModeX == SkTileMode::kDecal || fTileModeY == SkTileMode::kDecal) {
      ...
      if (fTileModeX == SkTileMode::kDecal) {
        mask &= (sx == clamped_x);
      }
      if (fTileModeY == SkTileMode::kDecal) {
        mask &= (sy == clamped_y);
      }
  ...
  auto sample_level = [&](const SkPixmap& pm, const SkMatrix& inv, skvm::Coord local) {
    ...
    if (sampling == SamplingEnum::kNearest) {
      ...
      skvm::F32 left = local.x - 0.5f, top = local.y - 0.5f, right = local.x + 0.5f,
                bottom = local.y + 0.5f;

      // The fractional parts of right and bottom are our lerp factors in x and y respectively.
      skvm::F32 fx = fract(right), fy = fract(bottom);

      return lerp(
          lerp(sample_texel(u, left, top), sample_texel(u, right, top), fx),
      ...
      skvm::F32 fx = fract(local.x + 0.5f), fy = fract(local.y + 0.5f);
      skvm::F32 wx[4], wy[4];
  ...
  if (lower) {
    ...
    c = lerp(
        c, sample_level(*lower, lowerInv, lowerLocal), p->uniformF(uniforms->pushF(lowerWeight)));
  }
  ...
  if (input_is_opaque && fTileModeX != SkTileMode::kDecal && fTileModeY != SkTileMode::kDecal) {
    c.a = p->splat(1.0f);
  }
  ...
  if (sampling == SamplingEnum::kBicubic) {
    ...
    skvm::F32 limit = (at == kUnpremul_SkAlphaType || fClampAsIfUnpremul) ? p->splat(1.0f) : c.a;
