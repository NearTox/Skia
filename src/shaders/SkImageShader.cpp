...
SkImageShader::SkImageShader(
    sk_sp<SkImage> img, const SkRect& subset, SkTileMode tmx, SkTileMode tmy,
    const SkSamplingOptions& sampling, const SkMatrix* localMatrix, bool raw,
...
sk_sp<SkFlattenable> SkImageShader::CreateProc(SkReadBuffer& buffer) {
  ...
  bool raw =
      buffer.isVersionLT(SkPicturePriv::Version::kRawImageShaders) ? false : buffer.readBool();
...
SkShaderBase::Context* SkImageShader::onMakeContext(
  ...
  return SkBitmapProcLegacyShader::MakeContext(
      *this, fTileModeX, fTileModeY, sampling, as_IB(fImage.get()), rec, alloc);
}
#endif
...
sk_sp<SkShader> SkImageShader::MakeSubset(
    sk_sp<SkImage> image, const SkRect& subset, SkTileMode tmx, SkTileMode tmy,
    const SkSamplingOptions& options, const SkMatrix* localMatrix, bool clampAsIfUnpremul) {
...
std::unique_ptr<GrFragmentProcessor> SkImageShader::asFragmentProcessor(
  ...
  auto fp = as_IB(fImage.get())
                ->asFragmentProcessor(args.fContext, fSampling, tileModes, lmInverse, subset);
  if (!fp) {
    return nullptr;
  }
  ...
  if (!fRaw) {
    fp = GrColorSpaceXformEffect::Make(
        std::move(fp), fImage->colorSpace(), fImage->alphaType(), args.fDstColorInfo->colorSpace(),
        kPremul_SkAlphaType);
...
class SkImageShader::TransformShader : public SkTransformShader {
 public:
  explicit TransformShader(const SkImageShader& shader)
      : SkTransformShader{shader}, fImageShader{shader} {}
  ...
  auto append_tiling_and_gather = [&] {
    ...
    switch (pm.colorType()) {
      ...
      case kR8_unorm_SkColorType:
        p->append(SkRasterPipeline::gather_a8, ctx);
        p->append(SkRasterPipeline::alpha_to_red);
        break;
...
skvm::Color SkImageShader::onProgram(
    skvm::Builder* b, skvm::Coord device, skvm::Coord origLocal, skvm::Color paint,
    const SkMatrixProvider& matrices, const SkMatrix* localM, const SkColorInfo& dst,
