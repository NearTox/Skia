...
std::unique_ptr<GrFragmentProcessor> SkImageShader::asFragmentProcessor(
  ...
  GrSamplerState::WrapMode wrapModes[] = {
      tile_mode_to_wrap_mode(fTileModeX), tile_mode_to_wrap_mode(fTileModeY)};
  ...
  sk_sp<GrTextureProxy> proxy(
      as_IB(fImage)->asTextureProxyRef(args.fContext, samplerState, scaleAdjust));
  if (!proxy) {
    return nullptr;
  }
  ...
  if (doBicubic) {
    ...
    inner = GrBicubicEffect::Make(
        std::move(proxy), lmInverse, wrapModes, domainX, domainY, kDir, srcAlphaType);
  } else {
    inner = GrTextureEffect::Make(std::move(proxy), srcAlphaType, lmInverse, samplerState, caps);
  }
