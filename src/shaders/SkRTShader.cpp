...
bool SkRTShader::onAppendStages(const SkStageRec& rec) const {
  ...
  if (!fInterpreter) {
    ...
    fInterpreter.reset(
        new SkSL::Interpreter<SkRasterPipeline_InterpreterCtx::VECTOR_WIDTH>(std::move(byteCode)));
  }
  ctx->fn = fMain;
...
sk_sp<SkFlattenable> SkRTShader::CreateProc(SkReadBuffer& buffer) {
  ...
  return sk_sp<SkFlattenable>(new SkRTShader(
      std::move(effect), std::move(inputs), localMPtr, children.data(), children.size(), isOpaque));
}
...
std::unique_ptr<GrFragmentProcessor> SkRTShader::asFragmentProcessor(const GrFPArgs& args) const {
  ...
  auto fp = GrSkSLFP::Make(
      args.fContext, fEffect, "runtime-shader", fInputs->data(), fInputs->size(), &matrix);
  for (const auto& child : fChildren) {
