...
static inline uint32_t new_sksl_unique_id() { return GrSkSLFP::NewIndex(); }
#else
...
SkRTShader::SkRTShader(
    int index, SkString sksl, sk_sp<SkData> inputs, const SkMatrix* localMatrix, bool isOpaque)
...
sk_sp<SkFlattenable> SkRTShader::CreateProc(SkReadBuffer& buffer) {
  ...
  return sk_sp<SkFlattenable>(
      new SkRTShader(index, std::move(sksl), std::move(inputs), localMPtr, isOpaque));
}
...
std::unique_ptr<GrFragmentProcessor> SkRTShader::asFragmentProcessor(const GrFPArgs& args) const {
  ...
  return GrSkSLFP::Make(
      args.fContext, fUniqueID, "runtime-shader", fSkSL, fInputs->data(), fInputs->size(),
      SkSL::Program::kPipelineStage_Kind, &matrix);
}
#endif

SkRuntimeShaderFactory::SkRuntimeShaderFactory(SkString sksl, bool isOpaque)
    : fIndex(new_sksl_unique_id()), fSkSL(std::move(sksl)), fIsOpaque(isOpaque) {}

sk_sp<SkShader> SkRuntimeShaderFactory::make(sk_sp<SkData> inputs, const SkMatrix* localMatrix) {
  return sk_sp<SkShader>(new SkRTShader(fIndex, fSkSL, std::move(inputs), localMatrix, fIsOpaque));
}
