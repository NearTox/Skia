...
SkRTShader::SkRTShader(
    SkString sksl, sk_sp<SkData> inputs, const SkMatrix* localMatrix, bool isOpaque)
...
sk_sp<SkFlattenable> SkRTShader::CreateProc(SkReadBuffer& buffer) {
  ...
  return sk_sp<SkFlattenable>(
      new SkRTShader(std::move(sksl), std::move(inputs), localMPtr, isOpaque));
}

sk_sp<SkShader> SkRuntimeShaderMaker(
    SkString sksl, sk_sp<SkData> inputs, const SkMatrix* localMatrix, bool isOpaque) {
  return sk_sp<SkShader>(new SkRTShader(std::move(sksl), std::move(inputs), localMatrix, isOpaque));
}
