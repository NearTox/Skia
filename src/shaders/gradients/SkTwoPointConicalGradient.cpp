...
sk_sp<SkShader> SkTwoPointConicalGradient::Create(
    const SkPoint& c0, SkScalar r0, const SkPoint& c1, SkScalar r1, const Descriptor& desc) {
  ...
  return sk_sp<SkShader>(
      new SkTwoPointConicalGradient(c0, r0, c1, r1, desc, gradientType, gradientMatrix, focalData));
}
...
sk_sp<SkFlattenable> SkTwoPointConicalGradient::CreateProc(SkReadBuffer& buffer) {
  ...
  return SkGradientShader::MakeTwoPointConical(
      c1, r1, c2, r2, desc.fColors, std::move(desc.fColorSpace), desc.fPos, desc.fCount,
...
void SkTwoPointConicalGradient::appendGradientStages(
    SkArenaAlloc* alloc, SkRasterPipeline* p, SkRasterPipeline* postPipeline) const {
...
skvm::F32 SkTwoPointConicalGradient::transformT(
  ...
  if (fType == Type::kRadial) {
    float denom = 1.0f / (fRadius2 - fRadius1), scale = std::max(fRadius1, fRadius2) * denom,
          bias = -fRadius1 * denom;
    return mag(x, y) * p->uniformF(uniforms->pushF(scale)) + p->uniformF(uniforms->pushF(bias));
  }
...
#ifdef SK_ENABLE_SKSL
void SkTwoPointConicalGradient::addToKey(
    const SkKeyContext& keyContext, SkPaintParamsKeyBuilder* builder,
    SkPipelineDataGatherer* gatherer) const {
  GradientShaderBlocks::GradientData data(
      kConical_GradientType, SkM44(this->getLocalMatrix()), fCenter1, fCenter2, fRadius1, fRadius2,
      0.0f, 0.0f, fTileMode, fColorCount, fOrigColors4f, fOrigPos);
