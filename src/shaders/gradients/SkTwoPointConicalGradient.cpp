...
skvm::F32 SkTwoPointConicalGradient::transformT(
  ...
  if (fType == Type::kRadial) {
    float denom = 1.0f / (fRadius2 - fRadius1), scale = SkTMax(fRadius1, fRadius2) * denom,
          bias = -fRadius1 * denom;
    return p->mad(
        p->norm(x, y), p->uniformF(uniforms->pushF(scale)), p->uniformF(uniforms->pushF(bias)));
  }

  if (fType == Type::kStrip) {
    float r = fRadius1 / this->getCenterX1();
    skvm::F32 t = p->add(x, p->sqrt(p->sub(p->splat(r * r), p->mul(y, y))));
