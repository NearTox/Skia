...
struct DstTraits {
  ...
  static skvx::float4 load(const SkPMColor4f& c) {
    ...
    return premul == ApplyPremul::False ? c4f * skvx::float4(255) : c4f;
  }
  ...
  static void store4x(
      const skvx::float4& c0, const skvx::float4& c1, const skvx::float4& c2,
      const skvx::float4& c3, SkPMColor* dst, const skvx::float4& bias0,
