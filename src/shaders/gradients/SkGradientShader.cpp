...
SkGradientShaderBase::SkGradientShaderBase(const Descriptor& desc, const SkMatrix& ptsToUnit)
  ...
  fOrigPos = desc.fPos ? reinterpret_cast<SkScalar*>(fOrigColors4f + fColorCount) : nullptr;
...
bool SkGradientShaderBase::onAppendStages(const SkStageRec& rec) const {
  ...
  auto prepareColor = [premulGrad, &xformedColors](int i) {
    ...
    return premulGrad ? c.premul() : SkPMColor4f{c.fR, c.fG, c.fB, c.fA};
  };

  // The two-stop case with stops at 0 and 1.
  if (fColorCount == 2 && fOrigPos == nullptr) {
    const SkPMColor4f c_l = prepareColor(0), c_r = prepareColor(1);
...
bool SkGradientShaderBase::onProgram(
  ...
  switch (fTileMode) {
    ...
    case SkTileMode::kMirror: {
      ...
      //       {-A-}      {--------B-------}
      skvm::F32 A = p->sub(t, p->splat(1.0f)), B = p->floor(p->mul(A, p->splat(0.5f)));
      t = p->abs(p->sub(p->sub(A, p->add(B, B)), p->splat(1.0f)));
    } break;
  }

  // Transform our colors as we want them interpolated, in dst color space, possibly premul.
  SkImageInfo common =
                  SkImageInfo::Make(fColorCount, 1, kRGBA_F32_SkColorType, kUnpremul_SkAlphaType),
  ...
  SkConvertPixels(dst, rgba.data(), dst.minRowBytes(), src, fOrigColors4f, src.minRowBytes());
  ...
  if (fColorCount == 2) {
    ...
    F4 lo = F4::Load(rgba.data() + 0), hi = F4::Load(rgba.data() + 4);
    F4 F = hi - lo, B = lo;
    ...
    *a = p->mad(T, p->uniformF(uniforms->pushF(F[3])), p->uniformF(uniforms->pushF(B[3])));
    ...
    if (fOrigPos == nullptr) {
      ...
      ix = p->trunc(p->mad(
          p->clamp(t, p->splat(0.0f), p->splat(1.0f)),
          p->uniformF(uniforms->pushF(stops.size() - 1.0f)), p->splat(1.0f)));
    } else {
    ...
    ix = p->shl(ix, 3);
    skvm::F32 Fr = p->bit_cast(p->gather32(fbs, ix));
    ix = p->add(ix, p->splat(1));
    skvm::F32 Fg = p->bit_cast(p->gather32(fbs, ix));
    ix = p->add(ix, p->splat(1));
    skvm::F32 Fb = p->bit_cast(p->gather32(fbs, ix));
    ix = p->add(ix, p->splat(1));
    skvm::F32 Fa = p->bit_cast(p->gather32(fbs, ix));

    ix = p->add(ix, p->splat(1));
    skvm::F32 Br = p->bit_cast(p->gather32(fbs, ix));
    ix = p->add(ix, p->splat(1));
    skvm::F32 Bg = p->bit_cast(p->gather32(fbs, ix));
    ix = p->add(ix, p->splat(1));
    skvm::F32 Bb = p->bit_cast(p->gather32(fbs, ix));
    ix = p->add(ix, p->splat(1));
    skvm::F32 Ba = p->bit_cast(p->gather32(fbs, ix));
  ...
  if (0 == (fGradFlags & SkGradientShader::kInterpolateColorsInPremul_Flag) && !fColorsAreOpaque) {
    p->premul(r, g, b, *a);
  }
...
SkColor4fXformer::SkColor4fXformer(
    const SkColor4f* colors, int colorCount, SkColorSpace* src, SkColorSpace* dst) {
  fColors = colors;
  ...
  if (dst && !SkColorSpace::Equals(src, dst)) {
    ...
    SkConvertPixels(
        info.makeColorSpace(sk_ref_sp(dst)), fStorage.begin(), info.minRowBytes(),
...
static sk_sp<SkShader> make_degenerate_gradient(
    const SkColor4f colors[], const SkScalar pos[], int colorCount, sk_sp<SkColorSpace> colorSpace,
    SkTileMode mode) {
  switch (mode) {
    case SkTileMode::kDecal:
  ...
  ColorStopOptimizer(const SkColor4f* colors, const SkScalar* pos, int count, SkTileMode mode)
    ...
    if (SkScalarNearlyEqual(pos[0], 0.0f) && SkScalarNearlyEqual(pos[1], 0.0f) &&
      ...
      if (SkTileMode::kRepeat == mode || SkTileMode::kMirror == mode || colors[0] == colors[1]) {
        ...
        SkScalarNearlyEqual(pos[0], 0.0f) && SkScalarNearlyEqual(pos[1], 1.0f) &&
        SkScalarNearlyEqual(pos[2], 1.0f)) {
    ...
    for (int i = 0; i < count; ++i) {
      fColors4f.push_back(
          {SkColorGetR(colors[i]) * ONE_OVER_255, SkColorGetG(colors[i]) * ONE_OVER_255,
           SkColorGetB(colors[i]) * ONE_OVER_255, SkColorGetA(colors[i]) * ONE_OVER_255});
    }
  }
...
sk_sp<SkShader> SkGradientShader::MakeLinear(
    const SkPoint pts[2], const SkColor colors[], const SkScalar pos[], int colorCount,
    SkTileMode mode, uint32_t flags, const SkMatrix* localMatrix) {
  ColorConverter converter(colors, colorCount);
  return MakeLinear(
      pts, converter.fColors4f.begin(), nullptr, pos, colorCount, mode, flags, localMatrix);
}

sk_sp<SkShader> SkGradientShader::MakeLinear(
    const SkPoint pts[2], const SkColor4f colors[], sk_sp<SkColorSpace> colorSpace,
    const SkScalar pos[], int colorCount, SkTileMode mode, uint32_t flags,
  ...
  desc_init(
      &desc, opt.fColors, std::move(colorSpace), opt.fPos, opt.fCount, mode, flags, localMatrix);
  return sk_make_sp<SkLinearGradient>(pts, desc);
}

sk_sp<SkShader> SkGradientShader::MakeRadial(
    const SkPoint& center, SkScalar radius, const SkColor colors[], const SkScalar pos[],
    int colorCount, SkTileMode mode, uint32_t flags, const SkMatrix* localMatrix) {
  ColorConverter converter(colors, colorCount);
  return MakeRadial(
      center, radius, converter.fColors4f.begin(), nullptr, pos, colorCount, mode, flags,
      localMatrix);
}

sk_sp<SkShader> SkGradientShader::MakeRadial(
    const SkPoint& center, SkScalar radius, const SkColor4f colors[],
    sk_sp<SkColorSpace> colorSpace, const SkScalar pos[], int colorCount, SkTileMode mode,
    uint32_t flags, const SkMatrix* localMatrix) {
  if (radius < 0) {
    return nullptr;
...
sk_sp<SkShader> SkGradientShader::MakeTwoPointConical(
    const SkPoint& start, SkScalar startRadius, const SkPoint& end, SkScalar endRadius,
    const SkColor colors[], const SkScalar pos[], int colorCount, SkTileMode mode, uint32_t flags,
    const SkMatrix* localMatrix) {
  ColorConverter converter(colors, colorCount);
  return MakeTwoPointConical(
      start, startRadius, end, endRadius, converter.fColors4f.begin(), nullptr, pos, colorCount,
      mode, flags, localMatrix);
}

sk_sp<SkShader> SkGradientShader::MakeTwoPointConical(
    const SkPoint& start, SkScalar startRadius, const SkPoint& end, SkScalar endRadius,
    const SkColor4f colors[], sk_sp<SkColorSpace> colorSpace, const SkScalar pos[], int colorCount,
    SkTileMode mode, uint32_t flags, const SkMatrix* localMatrix) {
  ...
  if (SkScalarNearlyZero((start - end).length(), kDegenerateThreshold)) {
    ...
    if (SkScalarNearlyEqual(startRadius, endRadius, kDegenerateThreshold)) {
      ...
      if (mode == SkTileMode::kClamp && endRadius > kDegenerateThreshold) {
        ...
        return MakeRadial(
            start, endRadius, reColors, std::move(colorSpace), circlePos, 3, mode, flags,
            localMatrix);
      } else {
      ...
      return MakeRadial(
          start, endRadius, colors, std::move(colorSpace), pos, colorCount, mode, flags,
          localMatrix);
    }
...
sk_sp<SkShader> SkGradientShader::MakeSweep(
    SkScalar cx, SkScalar cy, const SkColor colors[], const SkScalar pos[], int colorCount,
    SkTileMode mode, SkScalar startAngle, SkScalar endAngle, uint32_t flags,
    const SkMatrix* localMatrix) {
  ColorConverter converter(colors, colorCount);
  return MakeSweep(
      cx, cy, converter.fColors4f.begin(), nullptr, pos, colorCount, mode, startAngle, endAngle,
      flags, localMatrix);
}

sk_sp<SkShader> SkGradientShader::MakeSweep(
    SkScalar cx, SkScalar cy, const SkColor4f colors[], sk_sp<SkColorSpace> colorSpace,
    const SkScalar pos[], int colorCount, SkTileMode mode, SkScalar startAngle, SkScalar endAngle,
    uint32_t flags, const SkMatrix* localMatrix) {
  if (!valid_grad(colors, pos, colorCount, mode)) {
    return nullptr;
  }
  ...
  if (SkScalarNearlyEqual(startAngle, endAngle, kDegenerateThreshold)) {
    ...
    if (mode == SkTileMode::kClamp && endAngle > kDegenerateThreshold) {
      ...
      SkColor4f reColors[3] = {colors[0], colors[0], colors[colorCount - 1]};
      return MakeSweep(
          cx, cy, reColors, std::move(colorSpace), clampPos, 3, mode, 0, endAngle, flags,
          localMatrix);
    } else {
      return make_degenerate_gradient(colors, pos, colorCount, std::move(colorSpace), mode);
    }
  ...
  const SkScalar t0 = startAngle / 360, t1 = endAngle / 360;
