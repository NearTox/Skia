...
skvm::Color SkGradientShaderBase::onProgram(
    ...
    const SkMatrixProvider& mats, const SkMatrix* localM, const SkColorInfo& dstInfo,
  ...
  switch (fTileMode) {
    ...
    case SkTileMode::kDecal: mask &= (t == clamp01(t)); break;

    case SkTileMode::kRepeat: t = fract(t); break;
...
SkColor4fXformer::SkColor4fXformer(
  ...
  if (dst && !SkColorSpace::Equals(src, dst)) {
    ...
    SkAssertResult(SkConvertPixels(
        dstInfo, fStorage.begin(), info.minRowBytes(), srcInfo, fColors, info.minRowBytes()));
...
sk_sp<SkShader> SkGradientShader::MakeRadial(
    ...
    sk_sp<SkColorSpace> colorSpace, const SkScalar pos[], int count, SkTileMode mode) {
...
sk_sp<SkShader> SkGradientShader::MakeTwoPointConical(
    const SkPoint& start, SkScalar startRadius, const SkPoint& end, SkScalar endRadius,
    const SkColor4f colors[], sk_sp<SkColorSpace> colorSpace, const SkScalar pos[], int count,
    SkTileMode mode) {
  return MakeTwoPointConical(
      start, startRadius, end, endRadius, colors, std::move(colorSpace), pos, count, mode, 0,
      nullptr);
}
...
sk_sp<SkShader> SkGradientShader::MakeSweep(
    ...
    const SkScalar pos[], int count, uint32_t flags, const SkMatrix* localMatrix) {
  return MakeSweep(
      cx, cy, colors, std::move(colorSpace), pos, count, SkTileMode::kClamp, 0, 360, flags,
      localMatrix);
}
sk_sp<SkShader> SkGradientShader::MakeSweep(
    SkScalar cx, SkScalar cy, const SkColor4f colors[], sk_sp<SkColorSpace> colorSpace,
