...
static void add_stop_color(SkRasterPipeline_GradientCtx* ctx, size_t stop, SkPMColor4f Fs,
                           SkPMColor4f Bs) noexcept {
    (ctx->fs[0])[stop] = Fs.fR;
    (ctx->fs[1])[stop] = Fs.fG;
    (ctx->fs[2])[stop] = Fs.fB;
    (ctx->fs[3])[stop] = Fs.fA;
...
static void add_const_color(SkRasterPipeline_GradientCtx* ctx, size_t stop,
                            SkPMColor4f color) noexcept {
...
static void init_stop_evenly(SkRasterPipeline_GradientCtx* ctx, float gapCount, size_t stop,
                             SkPMColor4f c_l, SkPMColor4f c_r) noexcept {
...
bool SkGradientShaderBase::onAppendStages(const SkStageRec& rec) const {
    ...
    switch (fTileMode) {
        case SkTileMode::kMirror:
            p->append(SkRasterPipeline::mirror_x_1);
            break;
        case SkTileMode::kRepeat:
            p->append(SkRasterPipeline::repeat_x_1);
            break;
...
SkColor4fXformer::SkColor4fXformer(const SkColor4f* colors, int colorCount, SkColorSpace* src,
                                   SkColorSpace* dst) {
    fColors = colors;
...
static bool valid_grad(const SkColor4f colors[], const SkScalar pos[], int count,
                       SkTileMode tileMode) noexcept {
    return nullptr != colors && count >= 1 && (unsigned)tileMode < kSkTileModeCount;
}

static void desc_init(SkGradientShaderBase::Descriptor* desc, const SkColor4f colors[],
                      sk_sp<SkColorSpace> colorSpace, const SkScalar pos[], int colorCount,
                      SkTileMode mode, uint32_t flags, const SkMatrix* localMatrix) noexcept {
...
sk_sp<SkShader> SkGradientShader::MakeLinear(const SkPoint pts[2], const SkColor colors[],
                                             const SkScalar pos[], int colorCount, SkTileMode mode,
...
sk_sp<SkShader> SkGradientShader::MakeLinear(const SkPoint pts[2], const SkColor4f colors[],
                                             sk_sp<SkColorSpace> colorSpace, const SkScalar pos[],
                                             int colorCount, SkTileMode mode, uint32_t flags,
    ...
    if (SkScalarNearlyZero((start - end).length(), kDegenerateThreshold)) {
        ...
        if (SkScalarNearlyEqual(startRadius, endRadius, kDegenerateThreshold)) {
            ...
            if (mode == SkTileMode::kClamp && endRadius > kDegenerateThreshold) {
                ...
                return MakeRadial(start, endRadius, reColors, std::move(colorSpace), circlePos, 3,
                                  mode, flags, localMatrix);
            } else {
                // Otherwise use the default degenerate case
                return make_degenerate_gradient(colors, pos, colorCount, std::move(colorSpace),
                                                mode);
            }
...
sk_sp<SkShader> SkGradientShader::MakeSweep(SkScalar cx, SkScalar cy, const SkColor colors[],
                                            const SkScalar pos[], int colorCount, SkTileMode mode,
                                            SkScalar startAngle, SkScalar endAngle, uint32_t flags,
                                            const SkMatrix* localMatrix) {
    ColorConverter converter(colors, colorCount);
    return MakeSweep(cx, cy, converter.fColors4f.begin(), nullptr, pos, colorCount, mode,
                     startAngle, endAngle, flags, localMatrix);
}

sk_sp<SkShader> SkGradientShader::MakeSweep(SkScalar cx, SkScalar cy, const SkColor4f colors[],
                                            sk_sp<SkColorSpace> colorSpace, const SkScalar pos[],
                                            int colorCount, SkTileMode mode, SkScalar startAngle,
