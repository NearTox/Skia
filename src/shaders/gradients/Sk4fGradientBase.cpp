...
skvx::float4 pack_color(const SkColor4f& c4f, bool premul, const skvx::float4& component_scale) {
  auto pm4f = premul ? skvx::float4::Load(c4f.premul().vec()) : skvx::float4::Load(c4f.vec());
...
void addMirrorIntervals(
    const SkGradientShaderBase& shader, const SkColor4f* colors, const skvx::float4& componentScale,
...
Sk4fGradientInterval::Sk4fGradientInterval(
    const skvx::float4& c0, SkScalar t0, const skvx::float4& c1, SkScalar t1)
    : fT0(t0), fT1(t1) {
  SkASSERT(t0 != t1);
...
void Sk4fGradientIntervalBuffer::init(
  ...
  const skvx::float4 componentScale =
      premulColors ? skvx::float4(alpha) : skvx::float4(1.0f, 1.0f, 1.0f, alpha);
...
bool SkGradientShaderBase::GradientShaderBase4fContext::isValid() const {
