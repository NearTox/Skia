...
Sk4f pack_color(const SkColor4f& c4f, bool premul, const Sk4f& component_scale) noexcept {
  Sk4f pm4f = premul ? Sk4f::Load(c4f.premul().vec()) : Sk4f::Load(c4f.vec());
...
class IntervalIterator {
 public:
  IntervalIterator(const SkGradientShaderBase& shader, bool reverse) noexcept
      : fShader(shader),
  ...
  void iterate(
      ...
      std::function<void(const SkColor4f&, const SkColor4f&, SkScalar, SkScalar)> func) const {
    if (!fShader.fOrigPos) {
...
void addMirrorIntervals(
    const SkGradientShaderBase& shader, const SkColor4f* colors, const Sk4f& componentScale,
    bool premulColors, bool reverse, Sk4fGradientIntervalBuffer::BufferType* buffer) {
  ...
  iter.iterate(colors, [&](const SkColor4f& c0, const SkColor4f& c1, SkScalar t0, SkScalar t1) {
    ...
    if (mirror_t0 != mirror_t1) {
      buffer->emplace_back(
          pack_color(c0, premulColors, componentScale), mirror_t0,
...
Sk4fGradientInterval::Sk4fGradientInterval(
    const Sk4f& c0, SkScalar t0, const Sk4f& c1, SkScalar t1) noexcept
    : fT0(t0), fT1(t1) {
  SkASSERT(t0 != t1);
...
void Sk4fGradientIntervalBuffer::init(
    const SkGradientShaderBase& shader, SkColorSpace* dstCS, SkTileMode tileMode, bool premulColors,
  ...
  const Sk4f componentScale = premulColors ? Sk4f(alpha) : Sk4f(1.0f, 1.0f, 1.0f, alpha);
  ...
  if (tileMode == SkTileMode::kClamp) {
    ...
    const Sk4f clamp_color =
        pack_color(xformedColors.fColors[first_index], premulColors, componentScale);
    const SkScalar clamp_pos = reverse ? SK_ScalarInfinity : SK_ScalarNegativeInfinity;
    fIntervals.emplace_back(clamp_color, clamp_pos, clamp_color, first_pos);
  } else if (tileMode == SkTileMode::kMirror && reverse) {
    // synthetic mirror intervals injected before main intervals: (2 .. 1]
    addMirrorIntervals(
        shader, xformedColors.fColors, componentScale, premulColors, false, &fIntervals);
  }

  const IntervalIterator iter(shader, reverse);
  iter.iterate(
      xformedColors.fColors,
      [&](const SkColor4f& c0, const SkColor4f& c1, SkScalar t0, SkScalar t1) noexcept {
        SkASSERT(fIntervals.empty() || fIntervals.back().fT1 == t0);

        fIntervals.emplace_back(
            pack_color(c0, premulColors, componentScale), t0,
  ...
  if (tileMode == SkTileMode::kClamp) {
    ...
    const Sk4f clamp_color =
        pack_color(xformedColors.fColors[last_index], premulColors, componentScale);
    const SkScalar clamp_pos = reverse ? SK_ScalarNegativeInfinity : SK_ScalarInfinity;
    fIntervals.emplace_back(clamp_color, last_pos, clamp_color, clamp_pos);
  } else if (tileMode == SkTileMode::kMirror && !reverse) {
    // synthetic mirror intervals injected after main intervals: [1 .. 2)
    addMirrorIntervals(
        shader, xformedColors.fColors, componentScale, premulColors, true, &fIntervals);
  }
}

const Sk4fGradientInterval* Sk4fGradientIntervalBuffer::find(SkScalar t) const noexcept {
  // Binary search.
...
SkGradientShaderBase::GradientShaderBase4fContext::GradientShaderBase4fContext(
    const SkGradientShaderBase& shader, const ContextRec& rec) noexcept
...
bool SkGradientShaderBase::GradientShaderBase4fContext::isValid() const noexcept {
