...
void SkSweepGradient::appendGradientStages(
  ...
  p->append_matrix(
      alloc, SkMatrix::Concat(SkMatrix::MakeScale(fTScale, 1), SkMatrix::MakeTrans(fTBias, 0)));
}

skvm::F32 SkSweepGradient::transformT(
    skvm::Builder* p, skvm::Uniforms* uniforms, skvm::F32 x, skvm::F32 y, skvm::I32* mask) const {
  skvm::F32 xabs = p->abs(x), yabs = p->abs(y),
            slope = p->div(p->min(xabs, yabs), p->max(xabs, yabs));
  ...
  const float A = +0.15912117063999176025390625f, B = -5.185396969318389892578125e-2f,
              C = +2.476101927459239959716796875e-2f, D = -7.0547382347285747528076171875e-3f;
  skvm::F32 phi = p->mul(
      slope, p->mad(s, p->mad(s, p->mad(s, p->splat(D), p->splat(C)), p->splat(B)), p->splat(A)));
  ...
  skvm::F32 t = p->bit_cast(p->bit_and(
      p->bit_cast(phi),  // t = phi if phi != NaN
      p->eq(phi, phi)));

  if (fTScale != 1.0f || fTBias != 0.0f) {
    t = p->mad(
        t, p->uniformF(uniforms->pushF(fTScale)), p->uniformF(uniforms->pushF(fTScale * fTBias)));
  }
  return t;
}
