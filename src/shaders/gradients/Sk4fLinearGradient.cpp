...
SkLinearGradient::LinearGradient4fContext::LinearGradient4fContext(
...
void SkLinearGradient::LinearGradient4fContext::shadeSpan(
    int x, int y, SkPMColor dst[], int count) {
  SkASSERT(count > 0);
...
template <ApplyPremul premul>
void SkLinearGradient::LinearGradient4fContext::shadePremulSpan(
    int x, int y, SkPMColor dst[], int count, float bias0, float bias1) const {
...
void SkLinearGradient::LinearGradient4fContext::shadeSpanInternal(
    int x, int y, SkPMColor dst[], int count, float bias0, float bias1) const {
  SkPoint pt;
  ...
  while (count > 0) {
    ...
    SkASSERT(SkScalarIsFinite(proc.currentAdvance()) || (n == count && proc.currentRampIsZero()));
...
class SkLinearGradient::LinearGradient4fContext::LinearIntervalProcessor {
 public:
  LinearIntervalProcessor(
      const Sk4fGradientInterval* firstInterval, const Sk4fGradientInterval* lastInterval,
      const Sk4fGradientInterval* i, SkScalar fx, SkScalar dx, bool is_vertical)
