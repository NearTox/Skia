...
struct Sk4fGradientInterval {
  Sk4fGradientInterval(const skvx::float4& c0, SkScalar t0, const skvx::float4& c1, SkScalar t1);
...
class SkGradientShaderBase::GradientShaderBase4fContext : public Context {
 ...
 private:
  ...
  void addMirrorIntervals(
      const SkGradientShaderBase&, const skvx::float4& componentScale, bool reverse);
};
