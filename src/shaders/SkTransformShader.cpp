...
skvm::Color SkTransformShader::onProgram(
  ...
  return fShader.program(b, device, newLocal, color, matrixProvider, localM, dst, uniforms, alloc);
}
...
skvm::Coord SkTransformShader::applyMatrix(
  ...
  auto dot = [&, x, y](int row) {
    return b->mad(
        x, b->arrayF(fMatrix, 3 * row + 0),
        b->mad(y, b->arrayF(fMatrix, 3 * row + 1), b->arrayF(fMatrix, 3 * row + 2)));
  };
