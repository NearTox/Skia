...
SkUpdatableShader* SkShaderBase::onUpdatableShader(SkArenaAlloc* alloc) const { return nullptr; }

#ifdef SK_ENABLE_SKSL
// TODO: add implementations for derived classes
void SkShaderBase::addToKey(
...
sk_sp<SkShader> SkBitmap::makeShader(
  ...
  return SkImageShader::Make(
      SkMakeImageFromRasterBitmap(*this, kIfMutable_SkCopyPixelsMode), tmx, tmy, sampling, lm);
}

bool SkShaderBase::appendStages(const SkStageRec& rec) const { return this->onAppendStages(rec); }
...
bool SkShaderBase::onAppendStages(const SkStageRec& rec) const {
  ...
  ContextRec cr(
      *opaquePaint, rec.fMatrixProvider.localToDevice(), rec.fLocalM, rec.fDstColorType,
      sk_srgb_singleton());
...
skvm::Color SkShaderBase::program(
  ...
  if (auto color =
          this->onProgram(p, device, local, paint, matrices, localM, tweaked, uniforms, alloc)) {
    if (this->isOpaque()) {
...
skvm::Coord SkShaderBase::ApplyMatrix(
    skvm::Builder* p, const SkMatrix& m, skvm::Coord coord, skvm::Uniforms* uniforms) {
  ...
  if (m.isIdentity()) {
    ...
    auto dot = [&, x, y](int row) {
      return p->mad(
          x, p->uniformF(uniforms->pushF(m[3 * row + 0])),
          p->mad(
              y, p->uniformF(uniforms->pushF(m[3 * row + 1])),
...
skvm::Color SkEmptyShader::onProgram(
    skvm::Builder*, skvm::Coord, skvm::Coord, skvm::Color, const SkMatrixProvider&, const SkMatrix*,
    const SkColorInfo&, skvm::Uniforms*, SkArenaAlloc*) const {
  return {};  // signal failure
}

sk_sp<SkFlattenable> SkEmptyShader::CreateProc(SkReadBuffer&) { return SkShaders::Empty(); }
