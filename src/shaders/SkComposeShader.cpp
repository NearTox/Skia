...
static skvm::Color program_or_paint(
    const sk_sp<SkShader>& sh, skvm::Builder* p, skvm::Coord device, skvm::Coord local,
    skvm::Color paint, const SkMatrixProvider& mats, const SkMatrix* localM, SkFilterQuality q,
    const SkColorInfo& dst, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) {
...
skvm::Color SkShader_Blend::onProgram(
  ...
  skvm::Color d, s;
  if ((d = program_or_paint(
           fDst, p, device, local, paint, mats, localM, q, dst, uniforms, alloc)) &&
      (s = program_or_paint(
           fSrc, p, device, local, paint, mats, localM, q, dst, uniforms, alloc))) {
