...
sk_sp<SkShader> SkShaders::Blend(
    sk_sp<SkBlender> blender, sk_sp<SkShader> dst, sk_sp<SkShader> src) {
  if (!src || !dst) {
    return nullptr;
  }
  if (!blender) {
...
skvm::Color SkShader_Blend::onProgram(
    ...
    const SkMatrixProvider& mats, const SkMatrix* localM, const SkColorInfo& cinfo,
