...
sk_sp<SkShader> SkShaders::Blend(
    SkBlendMode mode, sk_sp<SkShader> dst, sk_sp<SkShader> src, const SkMatrix* lm) {
  switch (mode) {
...
sk_sp<SkShader> SkShaders::Lerp(
    float weight, sk_sp<SkShader> dst, sk_sp<SkShader> src, const SkMatrix* lm) {
...
sk_sp<SkShader> SkShaders::Lerp(
    sk_sp<SkShader> red, sk_sp<SkShader> dst, sk_sp<SkShader> src, const SkMatrix* lm) {
  if (!red) {
    return nullptr;
  }
  if (dst == src) {
  ...
  return sk_sp<SkShader>(new SkShader_LerpRed(std::move(red), std::move(dst), std::move(src), lm));
}
...
sk_sp<SkFlattenable> SkShader_LerpRed::CreateProc(SkReadBuffer& buffer) {
  ...
  return buffer.isValid() ? SkShaders::Lerp(std::move(red), std::move(dst), std::move(src))
                          : nullptr;
}
