...
static SkString svg_transform(const SkMatrix& t) {
  ...
  switch (t.getType()) {
    ...
    case SkMatrix::kTranslate_Mask:
      ...
      break;
    case SkMatrix::kScale_Mask: tstr.printf("scale(%g %g)", t.getScaleX(), t.getScaleY()); break;
    default:
      ...
      //    | 0 0 1 |
      tstr.printf(
          "matrix(%g %g %g %g %g %g)", t.getScaleX(), t.getSkewY(), t.getSkewX(), t.getScaleY(),
...
bool RequiresViewportReset(const SkPaint& paint) {
  ...
  for (int i = 0; i < 2; i++) {
    if (xy[i] == SkTileMode::kRepeat) return true;
  }
  return false;
}
...
void AddPath(const SkGlyphRun& glyphRun, const SkPoint& offset, SkPath* path) {
  ...
  glyphRun.font().getPaths(
      glyphRun.glyphsIDs().data(), SkToInt(glyphRun.glyphsIDs().size()),
...
class SkSVGDevice::ResourceBucket : ::SkNoncopyable {
 public:
  ResourceBucket()
      : fGradientCount(0), fPathCount(0), fImageCount(0), fPatternCount(0), fColorFilterCount(0) {}
...
class SkSVGDevice::AutoElement : ::SkNoncopyable {
 public:
  AutoElement(const char name[], SkXMLWriter* writer) : fWriter(writer), fResourceBucket(nullptr) {
  ...
  AutoElement(
      const char name[], SkSVGDevice* svgdev, ResourceBucket* bucket, const MxCp& mc,
      const SkPaint& paint)
      : fWriter(svgdev->fWriter.get()), fResourceBucket(bucket) {
 ...
 private:
  ...
  void addGradientShaderResources(
      const SkShader* shader, const SkPaint& paint, Resources* resources);
  void addColorFilterResources(const SkColorFilter& cf, Resources* resources);
  void addImageShaderResources(const SkShader* shader, const SkPaint& paint, Resources* resources);
...
void SkSVGDevice::AutoElement::addGradientShaderResources(
    const SkShader* shader, const SkPaint& paint, Resources* resources) {
  ...
  resources->fPaintServer =
      gradient_type == SkShader::kColor_GradientType
          ? svg_color(grColors[0])
          : SkStringPrintf("url(#%s)", addLinearGradientDef(grInfo, shader).c_str());
}

void SkSVGDevice::AutoElement::addColorFilterResources(
...
void SkSVGDevice::AutoElement::addImageShaderResources(
    const SkShader* shader, const SkPaint& paint, Resources* resources) {
  SkMatrix outMatrix;
  ...
  for (int i = 0; i < 2; i++) {
    ...
    switch (xy[i]) {
      case SkTileMode::kRepeat: patternDims[i].appendScalar(imageDimension); break;
      default:
...
void SkSVGDevice::AutoElement::addPathAttributes(
    const SkPath& path, SkParsePath::PathEncoding encoding) {
  SkString pathData;
...
sk_sp<SkBaseDevice> SkSVGDevice::Make(
    const SkISize& size, std::unique_ptr<SkXMLWriter> writer, uint32_t flags) {
  return writer ? sk_sp<SkBaseDevice>(new SkSVGDevice(size, std::move(writer), flags)) : nullptr;
}
...
SkParsePath::PathEncoding SkSVGDevice::pathEncoding() const {
  return (fFlags & SkSVGCanvas::kRelativePathEncoding_Flag) ? SkParsePath::PathEncoding::Relative
...
void SkSVGDevice::drawImageRect(
  ...
  SkMatrix adjustedMatrix =
      this->localToDevice() * SkMatrix::RectToRect(src ? *src : SkRect::Make(bm.bounds()), dst);
...
class SVGTextBuilder : SkNoncopyable {
 public:
  SVGTextBuilder(SkPoint origin, const SkGlyphRun& glyphRun) : fOrigin(origin) {
    auto runSize = glyphRun.runSize();
    SkAutoSTArray<64, SkUnichar> unichars(runSize);
    SkFontPriv::GlyphsToUnichars(
        glyphRun.font(), glyphRun.glyphsIDs().data(), runSize, unichars.get());
 ...
 private:
  void appendUnichar(SkUnichar c, SkPoint position) {
    ...
    switch (c) {
      ...
      case '<': fText.append("&lt;"); break;
      case '>': fText.append("&gt;"); break;
      default: fText.appendUnichar(c); break;
    }
