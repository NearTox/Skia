...
sk_sp<SkAnimatedImage> SkAnimatedImage::Make(
    std::unique_ptr<SkAndroidCodec> codec, SkISize scaledSize, SkIRect cropRect,
...
sk_sp<SkAnimatedImage> SkAnimatedImage::Make(
    std::unique_ptr<SkAndroidCodec> codec, const SkImageInfo& requestedInfo, SkIRect cropRect,
    sk_sp<SkPicture> postProcess) {
  if (!codec) {
    return nullptr;
  }
...
SkAnimatedImage::SkAnimatedImage(
    std::unique_ptr<SkAndroidCodec> codec, SkISize scaledSize, SkImageInfo decodeInfo,
    ...
    : fCodec(std::move(codec)),
      ...
      fSimple(
          fScaledSize == fDecodeInfo.dimensions() && !fPostProcess &&
...
sk_sp<SkImage> SkAnimatedImage::getCurrentFrame() {
  ...
  // lazily copies the pixel ref if it has any other references. So it is safe to not
  // do a deep copy here.
  return SkMakeImageFromRasterBitmap(fDisplayFrame.fBitmap, kNever_SkCopyPixelsMode);
}
