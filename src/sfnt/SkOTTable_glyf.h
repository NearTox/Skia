...
struct SkOTTableGlyph {
  ...
  class Iterator {
   public:
    Iterator(
        SkOTTableGlyph& glyf, const SkOTTableIndexToLocation& loca,
      ...
      SkASSERT(
          locaFormat.value == SkOTTableHead::IndexToLocFormat::ShortOffsets ||
    ...
    void advance(uint16_t num) {
      ...
      if (fLocaFormat.value == SkOTTableHead::IndexToLocFormat::ShortOffsets) {
        fCurrentGlyphOffset = SkEndian_SwapBE16(fLoca.offsets.shortOffset[fCurrentGlyph]) << 1;
...
struct SkOTTableGlyphData {
  ...
  struct Simple {
    ...
    union Flags {
      struct Field {
        SK_OT_BYTE_BITFIELD(
            OnCurve, xShortVector, yShortVector, Repeat, xIsSame_xShortVectorPositive,
            yIsSame_yShortVectorPositive, Reserved6, Reserved7)
      } field;
      struct Raw {
      ...
      union Flags {
        struct Field {
          ...
          SK_OT_BYTE_BITFIELD(
              ...
              UNSCALED_COMPONENT_OFFSET, Reserved13, Reserved14, Reserved15)
          // 0-7
          SK_OT_BYTE_BITFIELD(
              ARG_1_AND_2_ARE_WORDS, ARGS_ARE_XY_VALUES, ROUND_XY_TO_GRID, WE_HAVE_A_SCALE,
              RESERVED, MORE_COMPONENTS, WE_HAVE_AN_X_AND_Y_SCALE, WE_HAVE_A_TWO_BY_TWO)
        } field;
        struct Raw {
