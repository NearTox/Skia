...
struct SkOTTableGlyph {
    ...
    public:
        Iterator(const SkOTTableGlyph& glyf,
                ...
                : fGlyf(glyf)
                , fLocaFormat(SkOTTableHead::IndexToLocFormat::ShortOffsets == locaFormat.value ? 0
                                                                                                : 1)
        ...
        void advance(uint16_t num) {
            ...
            fCurrentGlyphOffset = fLocaFormat
                                          ? SkEndian_SwapBE32(*fLocaPtr.longOffset)
    ...
    struct Composite {
        struct Component {
            union Flags {
                struct Field {
                    // 8-15
                    SK_OT_BYTE_BITFIELD(WE_HAVE_INSTRUCTIONS,
                                        USE_MY_METRICS,
                                        OVERLAP_COMPOUND,
                                        ...
                                        Reserved15)
                    // 0-7
                    SK_OT_BYTE_BITFIELD(ARG_1_AND_2_ARE_WORDS,
                                        ARGS_ARE_XY_VALUES,
                                        ROUND_XY_TO_GRID,
                                        WE_HAVE_A_SCALE,
                                        RESERVED,
