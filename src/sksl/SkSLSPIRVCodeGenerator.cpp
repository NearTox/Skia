...
static bool is_signed(const Context& context, const Type& type) {
  ...
  return type == *context.fInt_Type || type == *context.fShort_Type || type == *context.fByte_Type;
}
  ...
  if (entry == fTypeMap.end()) {
    ...
    switch (type.typeKind()) {
      case Type::TypeKind::kScalar:
        if (type == *fContext.fBool_Type) {
          this->writeInstruction(SpvOpTypeBool, result, fConstantBuffer);
            ...
            type == *fContext.fInt_Type || type == *fContext.fShort_Type ||
      ...
      case Type::TypeKind::kTexture: {
        this->writeInstruction(
            SpvOpTypeImage, result, this->getType(*fContext.fFloat_Type, layout), type.dimensions(),
            type.isDepth(), type.isArrayed(), type.isMultisampled(), type.isSampled() ? 1 : 2,
...
SpvId SPIRVCodeGenerator::writeIntrinsicCall(const FunctionCall& c, OutputStream& out) {
  ...
  switch (std::get<0>(intrinsic->second)) {
    ...
    default: ABORT("unsupported intrinsic kind");
  }
}
...
SpvId SPIRVCodeGenerator::writeConstantVector(const Constructor& c) {
  ...
  if (c.arguments().size() == 1) {
    ...
    this->writeOpCode(SpvOpConstantComposite, 3 + (int32_t)c.arguments().size(), fConstantBuffer);
    this->writeWord(typeId, fConstantBuffer);
    this->writeWord(result, fConstantBuffer);
...
SpvId SPIRVCodeGenerator::writeFloatConstructor(const Constructor& c, OutputStream& out) {
  ...
  if (argType.isSigned()) {
    this->writeInstruction(
        SpvOpConvertSToF, this->getType(constructorType), result, parameter, out);
  } else {
    SkASSERT(argType.isUnsigned());
    this->writeInstruction(
        SpvOpConvertUToF, this->getType(constructorType), result, parameter, out);
  }
  return result;
}
...
SpvId SPIRVCodeGenerator::writeIntConstructor(const Constructor& c, OutputStream& out) {
  ...
  if (argType.isFloat()) {
    this->writeInstruction(
        SpvOpConvertFToS, this->getType(constructorType), result, parameter, out);
  } else {
    SkASSERT(argType.isUnsigned());
    this->writeInstruction(SpvOpBitcast, this->getType(constructorType), result, parameter, out);
  }
  return result;
}
...
SpvId SPIRVCodeGenerator::writeUIntConstructor(const Constructor& c, OutputStream& out) {
  ...
  if (argType.isFloat()) {
    this->writeInstruction(
        SpvOpConvertFToU, this->getType(constructorType), result, parameter, out);
  } else {
...
SpvId SPIRVCodeGenerator::writeMatrixConstructor(const Constructor& c, OutputStream& out) {
  ...
  } else if (arguments.size() == 1 && arg0Type.typeKind() == Type::TypeKind::kVector) {
      ...
      if (dst == *fContext.fFloat_Type || dst == *fContext.fHalf_Type) {
        if (src == *fContext.fFloat_Type || src == *fContext.fHalf_Type) {
          if (c.arguments().size() == 1) {
            ...
            src == *fContext.fInt_Type || src == *fContext.fShort_Type ||
            src == *fContext.fByte_Type) {
          op = SpvOpConvertSToF;
        } else if (
            src == *fContext.fUInt_Type || src == *fContext.fUShort_Type ||
          ...
          dst == *fContext.fInt_Type || dst == *fContext.fShort_Type ||
        ...
        if (src == *fContext.fFloat_Type || src == *fContext.fHalf_Type) {
          ...
          dst == *fContext.fUInt_Type || dst == *fContext.fUShort_Type ||
...
SpvId SPIRVCodeGenerator::writeConstructor(const Constructor& c, OutputStream& out) {
  ...
  if (type == *fContext.fFloat_Type || type == *fContext.fHalf_Type) {
    ...
    return this->writeIntConstructor(c, out);
      ...
      type == *fContext.fUInt_Type || type == *fContext.fUShort_Type ||
  ...
  switch (expr.kind()) {
    case Expression::Kind::kVariableReference: {
      ...
      if (var.modifiers().fLayout.fBuiltin == SK_IN_BUILTIN) {
        typeId = this->getType(
            Type("sk_in", Type::TypeKind::kArray, var.type().componentType(), fSkInCount));
      } else {
        typeId = this->getType(type);
      }
      auto entry = fVariableMap.find(&var);
      SkASSERT(entry != fVariableMap.end());
      return std::unique_ptr<SPIRVCodeGenerator::LValue>(
          new PointerLValue(*this, entry->second, typeId, precision));
    }
    ...
    case Expression::Kind::kFieldAccess: {
      ...
      return std::unique_ptr<SPIRVCodeGenerator::LValue>(
          new PointerLValue(*this, member, this->getType(type), precision));
    }
    ...
    case Expression::Kind::kTernary: {
      ...
      SpvId result = this->nextId();
      this->writeInstruction(
          SpvOpPhi, this->getType(*fContext.fBool_Type), result, ifTrue, ifTrueLabel, ifFalse,
          ifFalseLabel, out);
      return std::unique_ptr<SPIRVCodeGenerator::LValue>(
          new PointerLValue(*this, result, this->getType(type), precision));
    }
    default: {
...
SpvId SPIRVCodeGenerator::writeVariableReference(const VariableReference& ref, OutputStream& out) {
    ...
    if (fProgram.fSettings.fFlipY) {
      ...
      if (fRTHeightStructId == (SpvId)-1) {
        ...
        const Variable* intfVar = fSynthetics.takeOwnershipOfSymbol(std::make_unique<Variable>(
            /*offset=*/-1, fProgram.fModifiers->handle(modifiers), name, &intfStruct,
            /*builtin=*/false, Variable::Storage::kGlobal));
        InterfaceBlock intf(
            /*offset=*/-1, intfVar, name, /*instanceName=*/"",
      ...
      this->writeWord(this->getPointerType(*fContext.fFloat_Type, fRTHeightStorageClass), out);
  ...
  if (ref.variable()->modifiers().fLayout.fBuiltin == SK_CLOCKWISE_BUILTIN &&
    ...
    this->writeInstruction(
        SpvOpLogicalNot, this->getType(*fContext.fBool_Type), inverse, result, out);
    return inverse;
  }
  return result;
}
...
SpvId SPIRVCodeGenerator::writeMatrixComparison(
  ...
  SpvId bvecType = this->getType(fContext.fBool_Type->toCompound(fContext, operandType.rows(), 1));
...
SpvId SPIRVCodeGenerator::writeBinaryExpression(
  ...
  switch (op) {
    case Token::Kind::TK_EQEQ: {
      ...
      if (operandType->typeKind() == Type::TypeKind::kVector) {
        tmpType =
            &fContext.fBool_Type->toCompound(fContext, operandType->columns(), operandType->rows());
      } else {
        tmpType = &resultType;
      }
    ...
    case Token::Kind::TK_BITWISEXOR:
      return this->writeBinaryOperation(
          ...
          SpvOpUndef, out);
    case Token::Kind::TK_COMMA: return rhs;
    default: SkASSERT(false); return -1;
  }
}
...
SpvId SPIRVCodeGenerator::writeLogicalAnd(const BinaryExpression& a, OutputStream& out) {
  ...
  SpvId result = this->nextId();
  this->writeInstruction(
      SpvOpPhi, this->getType(*fContext.fBool_Type), result, falseConstant, lhsBlock, rhs, rhsBlock,
      out);
  return result;
}
...
SpvId SPIRVCodeGenerator::writeLogicalOr(const BinaryExpression& o, OutputStream& out) {
  ...
  SpvId result = this->nextId();
  this->writeInstruction(
      SpvOpPhi, this->getType(*fContext.fBool_Type), result, trueConstant, lhsBlock, rhs, rhsBlock,
      out);
  return result;
}
...
SpvId SPIRVCodeGenerator::writeIntLiteral(const IntLiteral& i) {
  ...
  if (entry == fNumberConstants.end()) {
    SpvId result = this->nextId();
    this->writeInstruction(
        SpvOpConstant, this->getType(type), result, (SpvId)i.value(), fConstantBuffer);
...
SpvId SPIRVCodeGenerator::writeFloatLiteral(const FloatLiteral& f) {
  ...
  if (entry == fNumberConstants.end()) {
    ...
    this->writeInstruction(SpvOpConstant, this->getType(type), result, bits, fConstantBuffer);
    fNumberConstants[key] = result;
    return result;
  }
    return entry->second;
...
SpvId SPIRVCodeGenerator::writeInterfaceBlock(const InterfaceBlock& intf, bool appendRTHeight) {
  ...
  if (fProgram.fInputs.fRTHeight && appendRTHeight) {
    ...
    fields.emplace_back(
        Modifiers(), StringFragment(SKSL_RTHEIGHT_NAME), fContext.fFloat_Type.get());
  ...
  if (intfModifiers.fLayout.fBuiltin == SK_IN_BUILTIN) {
    ...
    typeId = this->getType(
        Type("sk_in", Type::TypeKind::kArray, intf.variable().type().componentType(), fSkInCount),
        memoryLayout);
  } else {
...
void SPIRVCodeGenerator::writeGlobalVar(
  ...
  if (var.modifiers().fLayout.fBuiltin == SK_IN_BUILTIN) {
    typeId = this->getPointerType(
        Type("sk_in", Type::TypeKind::kArray, type.componentType(), fSkInCount), storageClass);
  } else {
...
void SPIRVCodeGenerator::writeStatement(const Statement& s, OutputStream& out) {
  switch (s.kind()) {
    ...
    case Statement::Kind::kWhile: this->writeWhileStatement(s.as<WhileStatement>(), out); break;
