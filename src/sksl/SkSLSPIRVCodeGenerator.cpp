...
#define ALL_GLSL(x) \
  std::make_tuple(  \
      kGLSL_STD_450_IntrinsicKind, GLSLstd450##x, GLSLstd450##x, GLSLstd450##x, GLSLstd450##x)
#define BY_TYPE_GLSL(ifFloat, ifInt, ifUInt)                                                   \
  std::make_tuple(                                                                             \
      kGLSL_STD_450_IntrinsicKind, GLSLstd450##ifFloat, GLSLstd450##ifInt, GLSLstd450##ifUInt, \
      SpvOpUndef)
#define ALL_SPIRV(x) std::make_tuple(kSPIRV_IntrinsicKind, SpvOp##x, SpvOp##x, SpvOp##x, SpvOp##x)
#define SPECIAL(x)                                                              \
  std::make_tuple(                                                              \
      kSpecial_IntrinsicKind, k##x##_SpecialIntrinsic, k##x##_SpecialIntrinsic, \
  ...
  fIntrinsicMap[String("dot")] =
      std::make_tuple(kSPIRV_IntrinsicKind, SpvOpDot, SpvOpUndef, SpvOpUndef, SpvOpUndef);
...
#define PACK(type)                                            \
  fIntrinsicMap[String("pack" #type)] = ALL_GLSL(Pack##type); \
  ...
  fIntrinsicMap[String("dFdx")] =
      std::make_tuple(kSPIRV_IntrinsicKind, SpvOpDPdx, SpvOpUndef, SpvOpUndef, SpvOpUndef);
  fIntrinsicMap[String("dFdy")] = SPECIAL(DFdy);
  fIntrinsicMap[String("fwidth")] =
      std::make_tuple(kSPIRV_IntrinsicKind, SpvOpFwidth, SpvOpUndef, SpvOpUndef, SpvOpUndef);
  ...
  fIntrinsicMap[String("any")] =
      std::make_tuple(kSPIRV_IntrinsicKind, SpvOpUndef, SpvOpUndef, SpvOpUndef, SpvOpAny);
  fIntrinsicMap[String("all")] =
      std::make_tuple(kSPIRV_IntrinsicKind, SpvOpUndef, SpvOpUndef, SpvOpUndef, SpvOpAll);
  fIntrinsicMap[String("not")] =
      std::make_tuple(kSPIRV_IntrinsicKind, SpvOpUndef, SpvOpUndef, SpvOpUndef, SpvOpLogicalNot);
  fIntrinsicMap[String("equal")] = std::make_tuple(
      kSPIRV_IntrinsicKind, SpvOpFOrdEqual, SpvOpIEqual, SpvOpIEqual, SpvOpLogicalEqual);
  fIntrinsicMap[String("notEqual")] = std::make_tuple(
      kSPIRV_IntrinsicKind, SpvOpFOrdNotEqual, SpvOpINotEqual, SpvOpINotEqual,
      SpvOpLogicalNotEqual);
  fIntrinsicMap[String("lessThan")] = std::make_tuple(
      kSPIRV_IntrinsicKind, SpvOpFOrdLessThan, SpvOpSLessThan, SpvOpULessThan, SpvOpUndef);
  fIntrinsicMap[String("lessThanEqual")] = std::make_tuple(
      kSPIRV_IntrinsicKind, SpvOpFOrdLessThanEqual, SpvOpSLessThanEqual, SpvOpULessThanEqual,
      SpvOpUndef);
  fIntrinsicMap[String("greaterThan")] = std::make_tuple(
      kSPIRV_IntrinsicKind, SpvOpFOrdGreaterThan, SpvOpSGreaterThan, SpvOpUGreaterThan, SpvOpUndef);
  fIntrinsicMap[String("greaterThanEqual")] = std::make_tuple(
      kSPIRV_IntrinsicKind, SpvOpFOrdGreaterThanEqual, SpvOpSGreaterThanEqual,
...
void SPIRVCodeGenerator::writeOpCode(SpvOp_ opCode, int length, OutputStream& out) {
  ...
  switch (opCode) {
    ...
    default: SkASSERT(fCurrentBlock);
  }
...
void SPIRVCodeGenerator::writeInstruction(
    SpvOp_ opCode, int32_t word1, StringFragment string, OutputStream& out) {
...
void SPIRVCodeGenerator::writeInstruction(
    SpvOp_ opCode, int32_t word1, int32_t word2, StringFragment string, OutputStream& out) {
  ...
  if (entry == fTypeMap.end()) {
    ...
    switch (type.typeKind()) {
      case Type::TypeKind::kScalar:
        if (type.isBoolean()) {
          this->writeInstruction(SpvOpTypeBool, result, fConstantBuffer);
            ...
            type == *fContext.fTypes.fInt || type == *fContext.fTypes.fShort ||
          ...
          this->writeInstruction(SpvOpTypeInt, result, 32, 0, fConstantBuffer);
            ...
            type == *fContext.fTypes.fFloat || type == *fContext.fTypes.fHalf ||
      ...
      case Type::TypeKind::kMatrix:
        this->writeInstruction(
            SpvOpTypeMatrix, result, this->getType(index_type(fContext, type), layout),
      ...
      case Type::TypeKind::kArray: {
        ...
        if (type.columns() > 0) {
          ...
          this->writeInstruction(
              SpvOpTypeArray, result, this->getType(type.componentType(), layout),
...
SpvId SPIRVCodeGenerator::writeExpression(const Expression& expr, OutputStream& out) {
  switch (expr.kind()) {
    ...
    case Expression::Kind::kConstructor: return this->writeConstructor(expr.as<Constructor>(), out);
  ...
  switch (kind) {
    ...
    case kDFdy_SpecialIntrinsic: {
      ...
      if (fProgram.fSettings.fFlipY) {
        ...
        this->writeInstruction(SpvOpFNegate, this->getType(callType), flipped, result, out);
    ...
    case kSaturate_SpecialIntrinsic: {
      ...
      finalArgs.push_back(std::make_unique<FloatLiteral>(
          fContext, /*offset=*/-1,
          /*value=*/0));
      finalArgs.push_back(std::make_unique<FloatLiteral>(
          fContext, /*offset=*/-1,
    ...
    case kMatrixCompMult_SpecialIntrinsic: {
      ...
      result = this->writeComponentwiseMatrixBinary(callType, lhs, rhs, SpvOpFMul, SpvOpUndef, out);
      break;
    }
  }
  return result;
...
SpvId SPIRVCodeGenerator::writeFunctionCall(const FunctionCall& c, OutputStream& out) {
  ...
  for (size_t i = 0; i < arguments.size(); i++) {
    ...
    this->writeInstruction(
        SpvOpVariable, this->getPointerType(arguments[i]->type(), SpvStorageClassFunction), tmpVar,
  ...
  for (const auto& tuple : lvalues) {
    ...
    this->writeInstruction(SpvOpLoad, getType(*std::get<1>(tuple)), load, std::get<0>(tuple), out);
...
SpvId SPIRVCodeGenerator::writeConstantVector(const Constructor& c) {
  ...
  if (c.componentType().isFloat()) {
    ...
    SkDEBUGFAILF("unexpected vector component type: %s", c.componentType().displayName().c_str());
    return SpvId(-1);
  }
...
void SPIRVCodeGenerator::writeUniformScaleMatrix(
  ...
  for (int column = 0; column < type.columns(); column++) {
    ...
    this->writeWord(this->getType(type.componentType().toCompound(fContext, type.rows(), 1)), out);
...
void SPIRVCodeGenerator::writeMatrixCopy(
    SpvId id, SpvId src, const Type& srcType, const Type& dstType, OutputStream& out) {
...
void SPIRVCodeGenerator::addColumnEntry(
    SpvId columnType, Precision precision, std::vector<SpvId>* currentColumn,
    std::vector<SpvId>* columnIds, int* currentCount, int rows, SpvId entry, OutputStream& out) {
...
SpvId SPIRVCodeGenerator::writeMatrixConstructor(const Constructor& c, OutputStream& out) {
  ...
  if (arguments.size() == 1 && arg0Type.isScalar()) {
    ...
    this->writeInstruction(
        SpvOpCompositeConstruct, this->getType(type), result, column1, column2, out);
  } else {
    ...
    for (size_t i = 0; i < arguments.size(); i++) {
      ...
      if (currentCount == 0 && argType.isVector() && argType.columns() == type.rows()) {
        // this is a complete column by itself
        columnIds.push_back(arguments[i]);
      } else {
        if (argType.columns() == 1) {
          this->addColumnEntry(
              columnType, precision, &currentColumn, &columnIds, &currentCount, rows, arguments[i],
              out);
        } else {
          ...
          for (int j = 0; j < argType.columns(); ++j) {
            ...
            this->writeInstruction(
                SpvOpCompositeExtract, componentType, swizzle, arguments[i], j, out);
            this->addColumnEntry(
                columnType, precision, &currentColumn, &columnIds, &currentCount, rows, swizzle,
                out);
          }
        }
      }
...
SpvId SPIRVCodeGenerator::writeVectorConstructor(const Constructor& c, OutputStream& out) {
  ...
  for (size_t i = 0; i < c.arguments().size(); i++) {
    ...
    if (argType.isVector()) {
      ...
      if (dst == *fContext.fTypes.fFloat || dst == *fContext.fTypes.fHalf) {
        if (src == *fContext.fTypes.fFloat || src == *fContext.fTypes.fHalf) {
          if (c.arguments().size() == 1) {
            ...
            src == *fContext.fTypes.fInt || src == *fContext.fTypes.fShort ||
            src == *fContext.fTypes.fByte) {
          op = SpvOpConvertSToF;
        } else if (
            src == *fContext.fTypes.fUInt || src == *fContext.fTypes.fUShort ||
            src == *fContext.fTypes.fUByte) {
          op = SpvOpConvertUToF;
        } else {
          fErrors.error(
              c.arguments()[i]->fOffset, "unsupported cast in SPIR-V: vector " + src.description() +
                                             " to " + dst.description());
        }
      } else if (
          dst == *fContext.fTypes.fInt || dst == *fContext.fTypes.fShort ||
        ...
        if (src == *fContext.fTypes.fFloat || src == *fContext.fTypes.fHalf) {
          ...
          dst == *fContext.fTypes.fUInt || dst == *fContext.fTypes.fUShort ||
      ...
      for (int j = 0; j < argType.columns(); j++) {
        SpvId swizzle = this->nextId();
        this->writeInstruction(SpvOpCompositeExtract, this->getType(src), swizzle, vec, j, out);
        if (op != SpvOpUndef) {
...
SpvId SPIRVCodeGenerator::writeConstructor(const Constructor& c, OutputStream& out) {
  ...
  if (type == *fContext.fTypes.fFloat || type == *fContext.fTypes.fHalf) {
    ...
    return this->writeIntConstructor(c, out);
      ...
      type == *fContext.fTypes.fUInt || type == *fContext.fTypes.fUShort ||
      type == *fContext.fTypes.fUByte) {
    return this->writeUIntConstructor(c, out);
  }
  switch (type.typeKind()) {
    case Type::TypeKind::kVector: return this->writeVectorConstructor(c, out);
    case Type::TypeKind::kMatrix: return this->writeMatrixConstructor(c, out);
    case Type::TypeKind::kArray: return this->writeArrayConstructor(c, out);
    default:
#ifdef SK_DEBUG
  ...
  switch (expr.kind()) {
    case Expression::Kind::kVariableReference: {
      ...
      if (var.modifiers().fLayout.fBuiltin == SK_IN_BUILTIN) {
        typeId =
            this->getType(*Type::MakeArrayType("sk_in", var.type().componentType(), fSkInCount));
      } else {
    ...
    case Expression::Kind::kSwizzle: {
      ...
      if (count == 1) {
        ...
        SpvId member = this->nextId();
        this->writeInstruction(
            SpvOpAccessChain, this->getPointerType(type, get_storage_class(*swizzle.base())),
            member, base, this->writeIntLiteral(index), out);
        return std::make_unique<PointerLValue>(*this, member, this->getType(type), precision);
      } else {
        return std::make_unique<SwizzleLValue>(
            *this, base, swizzle.components(), swizzle.base()->type(), type, precision);
      }
    }
    case Expression::Kind::kTernary: {
      ...
      SpvId result = this->nextId();
      this->writeInstruction(
          SpvOpPhi, this->getType(*fContext.fTypes.fBool), result, ifTrue, ifTrueLabel, ifFalse,
          ifFalseLabel, out);
...
SpvId SPIRVCodeGenerator::writeVariableReference(const VariableReference& ref, OutputStream& out) {
  ...
  if (ref.variable()->modifiers().fLayout.fBuiltin == SK_FRAGCOORD_BUILTIN &&
    ...
    this->writeInstruction(
        SpvOpCompositeExtract, this->getType(*fContext.fTypes.fFloat), xId, result, 0, out);

    // Calculate the y component which may need to be flipped
    SpvId rawYId = this->nextId();
    this->writeInstruction(
        SpvOpCompositeExtract, this->getType(*fContext.fTypes.fFloat), rawYId, result, 1, out);
    SpvId flippedYId = 0;
    if (fProgram.fSettings.fFlipY) {
      ...
      if (fRTHeightStructId == (SpvId)-1) {
        ...
        std::unique_ptr<Type> intfStruct = Type::MakeStructType(/*offset=*/-1, name, fields);
        ...
        const Variable* intfVar = fSynthetics.takeOwnershipOfSymbol(std::make_unique<Variable>(
            /*offset=*/-1, fProgram.fModifiers->addToPool(modifiers), name, intfStruct.get(),
      ...
      this->writeWord(this->getPointerType(*fContext.fTypes.fFloat, fRTHeightStorageClass), out);
      ...
      this->writeInstruction(
          SpvOpLoad, this->getType(*fContext.fTypes.fFloat), heightRead, heightPtr, out);

      flippedYId = this->nextId();
      this->writeInstruction(
          SpvOpFSub, this->getType(*fContext.fTypes.fFloat), flippedYId, heightRead, rawYId, out);
    }
    ...
    this->writeInstruction(
        SpvOpCompositeExtract, this->getType(*fContext.fTypes.fFloat), rawWId, result, 3, out);
  ...
  if (ref.variable()->modifiers().fLayout.fBuiltin == SK_CLOCKWISE_BUILTIN &&
    ...
    this->writeInstruction(
        SpvOpLogicalNot, this->getType(*fContext.fTypes.fBool), inverse, result, out);
    return inverse;
  }
  return result;
}
...
SpvId SPIRVCodeGenerator::writeBinaryExpression(
    const Type& leftType, SpvId lhs, Token::Kind op, const Type& rightType, SpvId rhs,
  ...
  switch (op) {
    ...
    case Token::Kind::TK_PLUS:
      if (leftType.isMatrix() && rightType.isMatrix()) {
        ...
        return this->writeComponentwiseMatrixBinary(leftType, lhs, rhs, SpvOpFAdd, SpvOpIAdd, out);
      }
      return this->writeBinaryOperation(
    ...
    case Token::Kind::TK_MINUS:
      if (leftType.isMatrix() && rightType.isMatrix()) {
        SkASSERT(leftType == rightType);
        return this->writeComponentwiseMatrixBinary(leftType, lhs, rhs, SpvOpFSub, SpvOpISub, out);
      }
      return this->writeBinaryOperation(
    ...
    case Token::Kind::TK_BITWISEXOR:
      return this->writeBinaryOperation(
          ...
          SpvOpUndef, out);
    default: fErrors.error(0, "unsupported token"); return -1;
  }
}
...
SpvId SPIRVCodeGenerator::writeBinaryExpression(const BinaryExpression& b, OutputStream& out) {
  ...
  switch (op) {
    ...
    case Token::Kind::TK_LOGICALAND: return this->writeLogicalAnd(b, out);
    case Token::Kind::TK_LOGICALOR: return this->writeLogicalOr(b, out);
    default: break;
  }
  ...
  SpvId result = this->writeBinaryExpression(
      left.type(), lhs, Compiler::RemoveAssignment(op), right.type(), rhs, b.type(), out);
  if (lvalue) {
...
SpvId SPIRVCodeGenerator::writeFunctionStart(const FunctionDeclaration& f, OutputStream& out) {
  ...
  this->writeInstruction(
      SpvOpFunction, this->getType(f.returnType()), result, SpvFunctionControlMaskNone,
      this->getFunctionType(f), out);
...
static void update_sk_in_count(const Modifiers& m, int* outSkInCount) {
  switch (m.fLayout.fPrimitive) {
    ...
    case Layout::kLinesAdjacency_Primitive: *outSkInCount = 4; break;
    case Layout::kTriangles_Primitive: *outSkInCount = 3; break;
    case Layout::kTrianglesAdjacency_Primitive: *outSkInCount = 6; break;
    default: return;
  }
}
...
SpvId SPIRVCodeGenerator::writeInterfaceBlock(const InterfaceBlock& intf, bool appendRTHeight) {
  ...
  bool pushConstant =
      ((intf.variable().modifiers().fLayout.fFlags & Layout::kPushConstant_Flag) != 0);
  ...
  if (fProgram.fInputs.fRTHeight && appendRTHeight) {
    ...
    fields.emplace_back(
        Modifiers(), StringFragment(SKSL_RTHEIGHT_NAME), fContext.fTypes.fFloat.get());
...
bool is_dead(const Variable& var, const ProgramUsage* usage) {
  ...
  if (!(var.modifiers().fFlags & (Modifiers::kIn_Flag | Modifiers::kOut_Flag |
                                  Modifiers::kUniform_Flag | Modifiers::kBuffer_Flag))) {
    return true;
  }
...
void SPIRVCodeGenerator::writeGlobalVar(
    Program::Kind kind, const VarDeclaration& varDecl, OutputStream& out) {
  const Variable& var = varDecl.var();
  // These haven't been implemented in our SPIR-V generator yet and we only currently use them
  // in the OpenGL backend.
  SkASSERT(
      !(var.modifiers().fFlags &
        (Modifiers::kReadOnly_Flag | Modifiers::kWriteOnly_Flag | Modifiers::kCoherent_Flag |
         Modifiers::kVolatile_Flag | Modifiers::kRestrict_Flag)));
  if (var.modifiers().fLayout.fBuiltin == BUILTIN_IGNORE) {
    return;
  }
  if (var.modifiers().fLayout.fBuiltin == SK_FRAGCOLOR_BUILTIN && kind != Program::kFragment_Kind) {
  ...
  if (var.modifiers().fLayout.fBuiltin == SK_IN_BUILTIN) {
    typeId = this->getPointerType(
        *Type::MakeArrayType("sk_in", type.componentType(), fSkInCount), storageClass);
  } else {
...
void SPIRVCodeGenerator::writeStatement(const Statement& s, OutputStream& out) {
  switch (s.kind()) {
    ...
    case Statement::Kind::kBlock: this->writeBlock((Block&)s, out); break;
  ...
  for (const ProgramElement* e : fProgram.elements()) {
    if (e->is<ModifiersDeclaration>()) {
      ...
      if (m.fFlags & Modifiers::kIn_Flag) {
        ...
        SpvId input;
        switch (m.fLayout.fPrimitive) {
          case Layout::kPoints_Primitive: input = SpvExecutionModeInputPoints; break;
          case Layout::kLines_Primitive: input = SpvExecutionModeInputLines; break;
          case Layout::kLinesAdjacency_Primitive:
            input = SpvExecutionModeInputLinesAdjacency;
            break;
          case Layout::kTriangles_Primitive: input = SpvExecutionModeTriangles; break;
        ...
        SpvId output;
        switch (m.fLayout.fPrimitive) {
          case Layout::kPoints_Primitive: output = SpvExecutionModeOutputPoints; break;
          case Layout::kLineStrip_Primitive: output = SpvExecutionModeOutputLineStrip; break;
        ...
        if (m.fLayout.fMaxVertices != -1) {
          this->writeInstruction(
              SpvOpExecutionMode, entryPoint, SpvExecutionModeOutputVertices,
              m.fLayout.fMaxVertices, out);
        }
      }
    }
  }
  this->writeInstruction(
      SpvOpExecutionMode, entryPoint, SpvExecutionModeInvocations, invocations, out);
}
...
void SPIRVCodeGenerator::writeInstructions(const Program& program, OutputStream& out) {
  ...
  for (const ProgramElement* e : program.elements()) {
    if (e->is<InterfaceBlock>()) {
      ...
      if (((modifiers.fFlags & Modifiers::kIn_Flag) || (modifiers.fFlags & Modifiers::kOut_Flag)) &&
          modifiers.fLayout.fBuiltin == -1 && !is_dead(intf.variable(), fProgram.fUsage.get())) {
  ...
  for (const ProgramElement* e : program.elements()) {
    if (e->is<GlobalVarDeclaration>()) {
      this->writeGlobalVar(
          program.fKind, e->as<GlobalVarDeclaration>().declaration()->as<VarDeclaration>(), body);
    }
  }
  for (const ProgramElement* e : program.elements()) {
  ...
  this->writeOpCode(
      SpvOpEntryPoint, (SpvId)(3 + (main->name().fLength + 4) / 4) + (int32_t)interfaceVars.size(),
      out);
  switch (program.fKind) {
    case Program::kVertex_Kind: this->writeWord(SpvExecutionModelVertex, out); break;
    case Program::kFragment_Kind: this->writeWord(SpvExecutionModelFragment, out); break;
    case Program::kGeometry_Kind: this->writeWord(SpvExecutionModelGeometry, out); break;
    default: ABORT("cannot write this kind of program to SPIR-V\n");
  }
