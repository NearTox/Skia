  ...
  if (entry == fTypeMap.end()) {
    ...
    for (size_t i = 0; i < function.fParameters.size(); i++) {
      ...
      parameterTypes.push_back(
          this->getPointerType(function.fParameters[i]->type(), SpvStorageClassFunction));
      //            } else {
    ...
    case kTexture_SpecialIntrinsic: {
      ...
      if (c.fArguments.size() == 3) {
        this->writeInstruction(
            ...
            this->writeExpression(*c.fArguments[2], out), out);
      } else {
...
SpvId SPIRVCodeGenerator::writeFunctionCall(const FunctionCall& c, OutputStream& out) {
  ...
  for (size_t i = 0; i < c.fArguments.size(); i++) {
    ...
    this->writeInstruction(
        SpvOpVariable, this->getPointerType(c.fArguments[i]->type(), SpvStorageClassFunction),
        tmpVar, SpvStorageClassFunction, fVariableBuffer);
...
SpvStorageClass_ get_storage_class(const Expression& expr) {
  switch (expr.kind()) {
    ...
    case Expression::Kind::kFieldAccess: return get_storage_class(*expr.as<FieldAccess>().fBase);
    case Expression::Kind::kIndex: return get_storage_class(*expr.as<IndexExpression>().fBase);
  ...
  switch (expr.kind()) {
    ...
    case Expression::Kind::kSwizzle: {
      ...
      if (count == 1) {
        ...
        SpvId member = this->nextId();
        this->writeInstruction(
            SpvOpAccessChain, this->getPointerType(type, get_storage_class(*swizzle.fBase)), member,
            base, this->writeIntLiteral(index), out);
        return std::unique_ptr<SPIRVCodeGenerator::LValue>(
            new PointerLValue(*this, member, this->getType(type), precision));
      } else {
        return std::unique_ptr<SPIRVCodeGenerator::LValue>(new SwizzleLValue(
            *this, base, swizzle.fComponents, swizzle.fBase->type(), type, precision));
      }
    }
    case Expression::Kind::kTernary: {
...
SpvId SPIRVCodeGenerator::writeSwizzle(const Swizzle& swizzle, OutputStream& out) {
  ...
  if (count == 1) {
    this->writeInstruction(
        SpvOpCompositeExtract, this->getType(swizzle.type()), result, base, swizzle.fComponents[0],
        out);
  } else {
...
SpvId SPIRVCodeGenerator::writeTernaryExpression(const TernaryExpression& t, OutputStream& out) {
  ...
  SpvId test = this->writeExpression(*t.fTest, out);
  if (t.fIfTrue->type().columns() == 1 && t.fIfTrue->isCompileTimeConstant() &&
...
SpvId SPIRVCodeGenerator::writePrefixExpression(const PrefixExpression& p, OutputStream& out) {
  ...
  switch (p.fOperator) {
    case Token::Kind::TK_PLUS: return this->writeExpression(*p.fOperand, out);
    ...
    case Token::Kind::TK_LOGICALNOT: {
      ...
      SpvId result = this->nextId();
      this->writeInstruction(
          SpvOpLogicalNot, this->getType(p.fOperand->type()), result,
    ...
    case Token::Kind::TK_BITWISENOT: {
      SpvId result = this->nextId();
      this->writeInstruction(
          SpvOpNot, this->getType(p.fOperand->type()), result,
          this->writeExpression(*p.fOperand, out), out);
      return result;
    }
    default:
...
SpvId SPIRVCodeGenerator::writeFunctionStart(const FunctionDeclaration& f, OutputStream& out) {
  ...
  this->writeInstruction(
      SpvOpFunction, this->getType(f.fReturnType), result, SpvFunctionControlMaskNone,
...
SpvId SPIRVCodeGenerator::writeInterfaceBlock(const InterfaceBlock& intf, bool appendRTHeight) {
  ...
  if (intf.fVariable.fModifiers.fLayout.fBuiltin == SK_IN_BUILTIN) {
    ...
    typeId = this->getType(
        Type("sk_in", Type::TypeKind::kArray, intf.fVariable.type().componentType(), fSkInCount),
        memoryLayout);
  } else {
...
void SPIRVCodeGenerator::writeGlobalVars(
    Program::Kind kind, const VarDeclarations& decl, OutputStream& out) {
...
void SPIRVCodeGenerator::writeReturnStatement(const ReturnStatement& r, OutputStream& out) {
  if (r.fExpression) {
    this->writeInstruction(SpvOpReturnValue, this->writeExpression(*r.fExpression, out), out);
  } else {
    this->writeInstruction(SpvOpReturn, out);
  }
}
...
void SPIRVCodeGenerator::writeInstructions(const Program& program, OutputStream& out) {
      ...
      case ProgramElement::Kind::kModifiers: {
        ...
        if (m.fFlags & Modifiers::kIn_Flag) {
          switch (m.fLayout.fPrimitive) {
            ...
            case Layout::kLines_Primitive: skInSize = 1; break;
            case Layout::kLinesAdjacency_Primitive:  // break
              skInSize = 2;
              break;
            case Layout::kTriangles_Primitive:  // break
            case Layout::kTrianglesAdjacency_Primitive: skInSize = 3; break;
            default: break;
          }
        }
        break;
  ...
  for (const auto& e : program) {
    if (e.kind() == ProgramElement::Kind::kInterfaceBlock) {
      ...
      if (((intf.fVariable.fModifiers.fFlags & Modifiers::kIn_Flag) ||
          ...
          intf.fVariable.fModifiers.fLayout.fBuiltin == -1 && !is_dead(intf.fVariable)) {
  ...
  this->writeOpCode(
      SpvOpEntryPoint, (SpvId)(3 + (main->fName.fLength + 4) / 4) + (int32_t)interfaceVars.size(),
      out);
