  ...
  if (entry == fTypeMap.end()) {
    ...
    switch (type.kind()) {
      ...
      case Type::kMatrix_Kind:
        ...
        break;
      case Type::kStruct_Kind: this->writeStruct(type, layout, result); break;
  ...
  if (entry == fTypeMap.end()) {
    ...
    for (size_t i = 0; i < function.fParameters.size(); i++) {
      ...
      parameterTypes.push_back(
          this->getPointerType(function.fParameters[i]->fType, SpvStorageClassFunction));
      //            } else {
...
SpvId SPIRVCodeGenerator::writeSpecialIntrinsic(
  ...
  switch (kind) {
    ...
    case kSampledImage_SpecialIntrinsic: {
      ...
      this->writeInstruction(SpvOpSampledImage, this->getType(c.fType), result, img, sampler, out);
      break;
    }
    case kSubpassLoad_SpecialIntrinsic: {
      ...
      if (1 == c.fArguments.size()) {
        this->writeInstruction(SpvOpImageRead, this->getType(c.fType), result, img, coords, out);
      } else {
        SkASSERT(2 == c.fArguments.size());
        SpvId sample = this->writeExpression(*c.fArguments[1], out);
        this->writeInstruction(
            SpvOpImageRead, this->getType(c.fType), result, img, coords, SpvImageOperandsSampleMask,
            sample, out);
      }
      break;
    }
    ...
    case kClamp_SpecialIntrinsic: {
      ...
      this->writeGLSLExtendedInstruction(
          c.fType, result, GLSLstd450FClamp, GLSLstd450SClamp, GLSLstd450UClamp, args, out);
      break;
    }
    case kMax_SpecialIntrinsic: {
      ...
      SkASSERT(args.size() == 2);
      this->writeGLSLExtendedInstruction(
          c.fType, result, GLSLstd450FMax, GLSLstd450SMax, GLSLstd450UMax, args, out);
      break;
    }
    case kMin_SpecialIntrinsic: {
      ...
      SkASSERT(args.size() == 2);
      this->writeGLSLExtendedInstruction(
          c.fType, result, GLSLstd450FMin, GLSLstd450SMin, GLSLstd450UMin, args, out);
      break;
    }
    case kMix_SpecialIntrinsic: {
      ...
      SkASSERT(args.size() == 3);
      this->writeGLSLExtendedInstruction(
          c.fType, result, GLSLstd450FMix, SpvOpUndef, SpvOpUndef, args, out);
      break;
    }
    case kSaturate_SpecialIntrinsic: {
      ...
      this->writeGLSLExtendedInstruction(
          c.fType, result, GLSLstd450FClamp, GLSLstd450SClamp, GLSLstd450UClamp, spvArgs, out);
      break;
    }
  }
  return result;
...
SpvId SPIRVCodeGenerator::writeFunctionCall(const FunctionCall& c, OutputStream& out) {
  ...
  for (size_t i = 0; i < c.fArguments.size(); i++) {
    ...
    this->writeInstruction(
        SpvOpVariable, this->getPointerType(c.fArguments[i]->fType, SpvStorageClassFunction),
...
SpvId SPIRVCodeGenerator::writeConstantVector(const Constructor& c) {
  ...
  if (c.fArguments.size() == 1) {
    ...
    this->writeOpCode(SpvOpConstantComposite, 3 + (int32_t)c.fArguments.size(), fConstantBuffer);
    this->writeWord(type, fConstantBuffer);
    this->writeWord(result, fConstantBuffer);
    for (SpvId id : arguments) {
      this->writeWord(id, fConstantBuffer);
...
SpvId SPIRVCodeGenerator::writeFloatConstructor(const Constructor& c, OutputStream& out) {
  ...
  if (c.fArguments[0]->fType.isSigned()) {
    this->writeInstruction(SpvOpConvertSToF, this->getType(c.fType), result, parameter, out);
  } else {
    SkASSERT(c.fArguments[0]->fType.isUnsigned());
    this->writeInstruction(SpvOpConvertUToF, this->getType(c.fType), result, parameter, out);
  }
  return result;
}
...
SpvId SPIRVCodeGenerator::writeIntConstructor(const Constructor& c, OutputStream& out) {
  ...
  if (c.fArguments[0]->fType.isFloat()) {
    this->writeInstruction(SpvOpConvertFToS, this->getType(c.fType), result, parameter, out);
  } else {
    SkASSERT(c.fArguments[0]->fType.isUnsigned());
    this->writeInstruction(SpvOpBitcast, this->getType(c.fType), result, parameter, out);
  }
  return result;
}
...
SpvId SPIRVCodeGenerator::writeUIntConstructor(const Constructor& c, OutputStream& out) {
  ...
  SpvId parameter = this->writeExpression(*c.fArguments[0], out);
  if (c.fArguments[0]->fType.isFloat()) {
    this->writeInstruction(SpvOpConvertFToU, this->getType(c.fType), result, parameter, out);
  } else {
...
SpvId SPIRVCodeGenerator::writeMatrixConstructor(const Constructor& c, OutputStream& out) {
  ...
  if (arguments.size() == 1 && c.fArguments[0]->fType.kind() == Type::kScalar_Kind) {
    ...
    this->writeInstruction(
        SpvOpCompositeConstruct, this->getType(c.fType), result, column1, column2, out);
  } else {
...
SpvId SPIRVCodeGenerator::writeConstructor(const Constructor& c, OutputStream& out) {
  ...
  if (c.fType == *fContext.fFloat_Type || c.fType == *fContext.fHalf_Type) {
    return this->writeFloatConstructor(c, out);
      ...
      c.fType == *fContext.fInt_Type || c.fType == *fContext.fShort_Type ||
      c.fType == *fContext.fByte_Type) {
    return this->writeIntConstructor(c, out);
  } else if (
      c.fType == *fContext.fUInt_Type || c.fType == *fContext.fUShort_Type ||
...
SpvStorageClass_ get_storage_class(const Expression& expr) {
  switch (expr.fKind) {
    ...
    case Expression::kFieldAccess_Kind: return get_storage_class(*((FieldAccess&)expr).fBase);
    case Expression::kIndex_Kind: return get_storage_class(*((IndexExpression&)expr).fBase);
  ...
  switch (expr.fKind) {
    case Expression::kVariableReference_Kind: {
      ...
      if (var.fModifiers.fLayout.fBuiltin == SK_IN_BUILTIN) {
        type =
            this->getType(Type("sk_in", Type::kArray_Kind, var.fType.componentType(), fSkInCount));
      } else {
      ...
      return std::unique_ptr<SPIRVCodeGenerator::LValue>(
          new PointerLValue(*this, entry->second, type, precision));
    }
    ...
    case Expression::kFieldAccess_Kind: {
      ...
      return std::unique_ptr<SPIRVCodeGenerator::LValue>(
          new PointerLValue(*this, member, this->getType(expr.fType), precision));
    }
    case Expression::kSwizzle_Kind: {
      ...
      if (count == 1) {
        ...
        this->writeInstruction(
            SpvOpAccessChain,
            this->getPointerType(swizzle.fType, get_storage_class(*swizzle.fBase)), member, base,
            this->writeIntLiteral(index), out);
        return std::unique_ptr<SPIRVCodeGenerator::LValue>(
            new PointerLValue(*this, member, this->getType(expr.fType), precision));
      } else {
        return std::unique_ptr<SPIRVCodeGenerator::LValue>(new SwizzleLValue(
            *this, base, swizzle.fComponents, swizzle.fBase->fType, expr.fType, precision));
      }
    }
    case Expression::kTernary_Kind: {
      ...
      this->writeInstruction(
          ...
          ifFalseLabel, out);
      return std::unique_ptr<SPIRVCodeGenerator::LValue>(
          new PointerLValue(*this, result, this->getType(expr.fType), precision));
    }
    default:
      ...
      this->writeInstruction(SpvOpVariable, type, result, SpvStorageClassFunction, fVariableBuffer);
...
SpvId SPIRVCodeGenerator::writeIndexExpression(const IndexExpression& expr, OutputStream& out) {
  if (expr.fBase->fType.kind() == Type::Kind::kVector_Kind) {
    ...
    SpvId result = this->nextId();
    this->writeInstruction(
        SpvOpVectorExtractDynamic, this->getType(expr.fType), result, base, index, out);
    return result;
  }
...
SpvId SPIRVCodeGenerator::writeSwizzle(const Swizzle& swizzle, OutputStream& out) {
  ...
  if (count == 1) {
    this->writeInstruction(
        SpvOpCompositeExtract, this->getType(swizzle.fType), result, base, swizzle.fComponents[0],
        out);
  } else {
...
SpvId SPIRVCodeGenerator::writeBinaryExpression(
  ...
  switch (op) {
    ...
    case Token::Kind::TK_PLUS:
      if (leftType.kind() == Type::kMatrix_Kind && rightType.kind() == Type::kMatrix_Kind) {
...
SpvId SPIRVCodeGenerator::writeBinaryExpression(const BinaryExpression& b, OutputStream& out) {
  ...
  SpvId result = this->writeBinaryExpression(
      b.fLeft->fType, lhs, remove_assignment(b.fOperator), b.fRight->fType, rhs, b.fType, out);
  if (lvalue) {
...
SpvId SPIRVCodeGenerator::writeTernaryExpression(const TernaryExpression& t, OutputStream& out) {
  ...
  if (t.fIfTrue->fType.columns() == 1 && t.fIfTrue->isCompileTimeConstant() &&
    ...
    this->writeInstruction(SpvOpSelect, this->getType(t.fType), result, test, trueId, falseId, out);
    return result;
  }
  ...
  this->writeInstruction(
      SpvOpVariable, this->getPointerType(t.fType, SpvStorageClassFunction), var,
  ...
  switch (p.fOperator) {
    ...
    case Token::Kind::TK_PLUSPLUS: {
      ...
      SpvId result = this->writeBinaryOperation(
          p.fType, p.fType, lv->load(out), one, SpvOpFAdd, SpvOpIAdd, SpvOpIAdd, SpvOpUndef, out);
      lv->store(result, out);
      return result;
    }
    case Token::Kind::TK_MINUSMINUS: {
      ...
      SpvId one = this->writeExpression(*create_literal_1(fContext, p.fType), out);
      SpvId result = this->writeBinaryOperation(
          p.fType, p.fType, lv->load(out), one, SpvOpFSub, SpvOpISub, SpvOpISub, SpvOpUndef, out);
      lv->store(result, out);
      return result;
    }
    case Token::Kind::TK_LOGICALNOT: {
      ...
      SpvId result = this->nextId();
      this->writeInstruction(
          SpvOpLogicalNot, this->getType(p.fOperand->fType), result,
    ...
    case Token::Kind::TK_BITWISENOT: {
      SpvId result = this->nextId();
      this->writeInstruction(
          SpvOpNot, this->getType(p.fOperand->fType), result,
          this->writeExpression(*p.fOperand, out), out);
      return result;
    }
    default:
  ...
  switch (p.fOperator) {
    case Token::Kind::TK_PLUSPLUS: {
      SpvId temp = this->writeBinaryOperation(
          p.fType, p.fType, result, one, SpvOpFAdd, SpvOpIAdd, SpvOpIAdd, SpvOpUndef, out);
      lv->store(temp, out);
      return result;
    }
    case Token::Kind::TK_MINUSMINUS: {
      SpvId temp = this->writeBinaryOperation(
          p.fType, p.fType, result, one, SpvOpFSub, SpvOpISub, SpvOpISub, SpvOpUndef, out);
      lv->store(temp, out);
      return result;
    }
    default:
...
SpvId SPIRVCodeGenerator::writeBoolLiteral(const BoolLiteral& b) {
  if (b.fValue) {
    if (fBoolTrue == 0) {
      ...
      this->writeInstruction(SpvOpConstantTrue, this->getType(b.fType), fBoolTrue, fConstantBuffer);
    }
    return fBoolTrue;
  } else {
    if (fBoolFalse == 0) {
      ...
      this->writeInstruction(
          SpvOpConstantFalse, this->getType(b.fType), fBoolFalse, fConstantBuffer);
    }
    return fBoolFalse;
  }
}
...
SpvId SPIRVCodeGenerator::writeFloatLiteral(const FloatLiteral& f) {
    ...
    if (entry == fNumberConstants.end()) {
      ...
      this->writeInstruction(SpvOpConstant, this->getType(f.fType), result, bits, fConstantBuffer);
      fNumberConstants[key] = result;
      return result;
    }
    return entry->second;
...
SpvId SPIRVCodeGenerator::writeInterfaceBlock(const InterfaceBlock& intf, bool appendRTHeight) {
  ...
  if (intf.fVariable.fModifiers.fLayout.fBuiltin == SK_IN_BUILTIN) {
    ...
    typeId = this->getType(
        Type("sk_in", Type::kArray_Kind, intf.fVariable.fType.componentType(), fSkInCount),
        memoryLayout);
  } else {
...
void SPIRVCodeGenerator::writeGlobalVars(
  ...
  for (size_t i = 0; i < decl.fVars.size(); i++) {
    ...
    SpvId type;
    if (var->fModifiers.fLayout.fBuiltin == SK_IN_BUILTIN) {
      type = this->getPointerType(
          Type("sk_in", Type::kArray_Kind, var->fType.componentType(), fSkInCount), storageClass);
    } else {
