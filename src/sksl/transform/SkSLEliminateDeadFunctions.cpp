...
bool Transform::EliminateDeadFunctions(Program& program, ProgramUsage* usage) {
  ...
  if (program.fConfig->fSettings.fRemoveDeadFunctions) {
    ...
    program.fOwnedElements.erase(
        std::remove_if(
            program.fOwnedElements.begin(), program.fOwnedElements.end(),
            [&](const std::unique_ptr<ProgramElement>& pe) { return isDeadFunction(pe.get()); }),
