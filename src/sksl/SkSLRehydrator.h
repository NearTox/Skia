...
class Rehydrator {
 public:
  ...
  Rehydrator(
      Compiler& compiler, const uint8_t* src, size_t length,
 ...
 private:
  ...
  int8_t readS8() noexcept {
    SkASSERT(fIP < fEnd);
    return *(fIP++);
  }

  uint8_t readU8() { return this->readS8(); }
  ...
  uint16_t readU16() { return this->readS16(); }
  ...
  uint32_t readU32() { return this->readS32(); }
  ...
  ErrorReporter* errorReporter() const noexcept { return this->context().fErrors; }

  ModifiersPool& modifiersPool() const noexcept { return *this->context().fModifiersPool; }
