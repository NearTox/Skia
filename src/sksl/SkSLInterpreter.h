...
#  define CHECK_LABEL(name) SkASSERT(labels[(int)ByteCode::Instruction::name] == &&name)
...
#  define BINARY_OP(inst, src, result, op)                            \
    LABEL(inst) {                                                     \
      ...
      fRegisters[target.fIndex].result =                              \
          fRegisters[src1.fIndex].src op fRegisters[src2.fIndex].src; \
      NEXT();                                                         \
    }
...
#  define MASKED_BINARY_OP(inst, src, result, op)                               \
    LABEL(inst) {                                                               \
      ...
      for (int i = 0; i < width; ++i) {                                         \
        if (m[i]) {                                                             \
          fRegisters[target.fIndex].result[i] =                                 \
              fRegisters[src1.fIndex].src[i] op fRegisters[src2.fIndex].src[i]; \
        }                                                                       \
      }                                                                         \
      NEXT();                                                                   \
    }
      ...
      for (int i = 0; i < width; ++i) {                                                   \
        if (m[i]) {                                                                       \
          for (int j = 0; j < count; ++j) {                                               \
            fRegisters[target.fIndex + j].result[i] =                                     \
                fRegisters[src1.fIndex + j].src[i] op fRegisters[src2.fIndex + j].src[i]; \
          }                                                                               \
        }                                                                                 \
      }                                                                                   \
      NEXT();                                                                             \
...
#  define VECTOR_BINARY_OP(inst, src, result, op)                               \
    ...
    LABEL(inst##N) {                                                            \
      ...
      for (int i = 0; i < count; ++i) {                                         \
        fRegisters[target.fIndex + i].result =                                  \
            fRegisters[src1.fIndex + i].src op fRegisters[src2.fIndex + i].src; \
      }                                                                         \
      NEXT();                                                                   \
    }
...
class Interpreter {
 public:
  ...
  Interpreter(std::unique_ptr<ByteCode> code) : fCode(std::move(code)) {
  ...
  ~Interpreter() { free(fBackingStore); }
   ...
   */
  bool runStriped(
      const ByteCodeFunction* f, int count, float* args[], float* outResult[] = nullptr) {
    SkASSERT(f);
    Vector* stack = fMemory + MEMORY_SIZE;
    int stackCount = f->fStackSlotCount + f->fParameterSlotCount;
    stack -= stackCount;
  ...
  const ByteCode& getCode() { return *fCode; }
 ...
 private:
  ...
  struct StackFrame {
    StackFrame(
        const ByteCodeFunction* function, const uint8_t* ip, const int stackSlotCount,
  ...
  struct Context {
    Context(
        Vector* memory, Vector* stack, VectorI* condStack, VectorI* maskStack, VectorI* loopStack,
        VectorI* continueStack)
        : fMemory(memory),
          fStack(stack),
  ...
  void disassemble(const uint8_t** ip) {
    ...
    switch (inst) {
      ...
      case ByteCode::Instruction::kCallExternal: {
        ...
        printf(
            "callExternal %s($%d(%d)...) -> $%d(%d)", String(ev->fName).c_str(), args.fIndex,
            argCount, target.fIndex, targetCount);
        printf("\n");
        break;
      }
      ...
      case ByteCode::Instruction::kMatrixToMatrix: {
        ...
        printf(
            "matrixToMatrix $%d, %dx%d to %dx%d -> $%d\n", src.fIndex, srcColumns, srcRows,
            dstColumns, dstRows, target.fIndex);
        break;
      }
      ...
      case ByteCode::Instruction::kScalarToMatrix: {
        ...
        printf("scalarToMatrix $%d, %dx%d -> $%d\n", src.fIndex, columns, rows, target.fIndex);
        break;
      }
      case ByteCode::Instruction::kSelect: {
        ...
        ByteCode::Register src2 = read<ByteCode::Register>(ip);
        printf(
            "select $%d, $%d, $%d -> %d\n", test.fIndex, src1.fIndex, src2.fIndex, target.fIndex);
        break;
      }
      ...
      default: printf("unsupported: %d\n", (int)inst); SkASSERT(false);
    }
  }
...
#  define CHECK_STACK_BOUNDS(address) \
    SkASSERT(                         \
        context.fStack + address >= fMemory && context.fStack + address <= fMemory + MEMORY_SIZE)

  static void Inverse2x2(Vector* in, Vector* out) {
    VectorF a = in[0].fFloat, b = in[1].fFloat, c = in[2].fFloat, d = in[3].fFloat;
  ...
  static void Inverse3x3(Vector* in, Vector* out) {
    VectorF a11 = in[0].fFloat, a12 = in[3].fFloat, a13 = in[6].fFloat, a21 = in[1].fFloat,
            a22 = in[4].fFloat, a23 = in[7].fFloat, a31 = in[2].fFloat, a32 = in[5].fFloat,
            a33 = in[8].fFloat;
...
#  define outf(index) out[index].fFloat
    VectorF a00 = inf(0), a10 = inf(4), a20 = inf(8), a30 = inf(12), a01 = inf(1), a11 = inf(5),
            a21 = inf(9), a31 = inf(13), a02 = inf(2), a12 = inf(6), a22 = inf(10), a32 = inf(14),
            a03 = inf(3), a13 = inf(7), a23 = inf(11), a33 = inf(15);

    VectorF b00 = a00 * a11 - a01 * a10, b01 = a00 * a12 - a02 * a10, b02 = a00 * a13 - a03 * a10,
            b03 = a01 * a12 - a02 * a11, b04 = a01 * a13 - a03 * a11, b05 = a02 * a13 - a03 * a12,
            b06 = a20 * a31 - a21 * a30, b07 = a20 * a32 - a22 * a30, b08 = a20 * a33 - a23 * a30,
            b09 = a21 * a32 - a22 * a31, b10 = a21 * a33 - a23 * a31, b11 = a22 * a33 - a23 * a32;

    VectorF idet =
        VectorF(1) / (b00 * b11 - b01 * b10 + b02 * b09 + b03 * b08 - b04 * b07 + b05 * b06);
...
#  endif
    ...
    auto parameterBase = [&]() {
      return context.fCallStack.empty() ? context.fStack : context.fCallStack.top().fParameters;
    };
...
#  endif
    ...
    LABEL(kBoundsCheck) {
      ...
      int length = read<int>(&ip);
      if (skvx::any(
              mask() & ((fRegisters[r.fIndex].fInt < 0) | (fRegisters[r.fIndex].fInt >= length)))) {
        return false;
      }
      NEXT();
    }
    ...
    *context.fLoopStack &= ~mask();
    ...
    LABEL(kCall) {
      ...
      context.fCallStack.push(StackFrame(
          f, ip, stackSlotCount, &fRegisters[args.fIndex], &fRegisters[returnValue.fIndex]));
      f = target;
      code = f->fCode.data();
      ip = code;
      context.fStack -= stackSlotCount;
      memcpy(context.fStack, &fRegisters[args.fIndex], f->fParameterSlotCount * sizeof(Vector));
      NEXT();
    }
    LABEL(kCallExternal) {
      ByteCode::Register target = read<ByteCode::Register>(&ip);
    ...
    LABEL(kFloatToSigned) {
      ByteCode::Register target = read<ByteCode::Register>(&ip);
      ByteCode::Register src = read<ByteCode::Register>(&ip);
      fRegisters[target.fIndex] = Vector(skvx::cast<int32_t>(fRegisters[src.fIndex].fFloat));
      NEXT();
    }
    LABEL(kFloatToUnsigned) {
      ByteCode::Register target = read<ByteCode::Register>(&ip);
      ByteCode::Register src = read<ByteCode::Register>(&ip);
      fRegisters[target.fIndex] = Vector(skvx::cast<uint32_t>(fRegisters[src.fIndex].fFloat));
      NEXT();
    }
    LABEL(kImmediate) {
      ByteCode::Register target = read<ByteCode::Register>(&ip);
    ...
    LABEL(kLoad) {
      ...
      for (int i = 0; i < width; ++i) {
        if (m[i]) {
          fRegisters[target.fIndex].fInt[i] = fMemory[fRegisters[src.fIndex].fInt[i]].fInt[i];
        }
      }
      NEXT();
    }
    ...
    LABEL(kLoadParameter) {
      ...
      for (int i = 0; i < width; ++i) {
        if (m[i]) {
          fRegisters[target.fIndex].fInt[i] = base[fRegisters[src.fIndex].fInt[i]].fInt[i];
        }
      }
      NEXT();
    }
      ...
      for (int i = 0; i < dstColumns; ++i) {
        for (int j = 0; j < dstRows; ++j) {
          if (i < srcColumns && j < srcRows) {
            fRegisters[target.fIndex + offset] = fRegisters[src.fIndex + (srcRows * i) + j];
          } else {
            if (i == j) {
    ...
    LABEL(kRemainderF) {
      ...
      ByteCode::Register src1 = read<ByteCode::Register>(&ip);
      ByteCode::Register src2 = read<ByteCode::Register>(&ip);
      fRegisters[target.fIndex] = VecMod(fRegisters[src1.fIndex], fRegisters[src2.fIndex]);
      NEXT();
    }
    LABEL(kRemainderFN) {
      uint8_t count = read<uint8_t>(&ip);
      ...
      for (int i = 0; i < count; ++i) {
        fRegisters[target.fIndex + i] =
            VecMod(fRegisters[src1.fIndex + i], fRegisters[src2.fIndex + i]);
      }
      NEXT();
    }
    LABEL(kReturn) {
    ...
    LABEL(kReturnValue) {
      ...
      context.fStack += frame.fStackSlotCount;
      memcpy(
          frame.fReturnValue, &fRegisters[returnValue.fIndex],
    ...
    LABEL(kSelect) {
      ...
      ByteCode::Register src1 = read<ByteCode::Register>(&ip);
      ByteCode::Register src2 = read<ByteCode::Register>(&ip);
      fRegisters[target.fIndex] = skvx::if_then_else(
          fRegisters[test.fIndex].fInt, fRegisters[src1.fIndex].fFloat,
    ...
    LABEL(kSignedToFloat) {
      ByteCode::Register target = read<ByteCode::Register>(&ip);
      ByteCode::Register src = read<ByteCode::Register>(&ip);
      fRegisters[target.fIndex] = Vector(skvx::cast<float>(fRegisters[src.fIndex].fInt));
      NEXT();
    }
    ...
    LABEL(kStore) {
      ...
      for (int i = 0; i < width; ++i) {
        if (m[i]) {
          fMemory[fRegisters[target.fIndex].fInt[i]].fInt[i] = fRegisters[src.fIndex].fInt[i];
        }
      }
      NEXT();
    }
    ...
    LABEL(kStoreDirect) {
      ByteCode::Pointer target = read<ByteCode::Pointer>(&ip);
      ByteCode::Register src = read<ByteCode::Register>(&ip);
      fMemory[target.fAddress] = skvx::if_then_else(
          mask(), fRegisters[src.fIndex].fFloat, fMemory[target.fAddress].fFloat);
      NEXT();
    }
    LABEL(kStoreDirectN) {
      uint8_t count = read<uint8_t>(&ip);
      ...
      for (int i = 0; i < count; ++i) {
        fMemory[target.fAddress + i] = skvx::if_then_else(
            mask(), fRegisters[src.fIndex + i].fFloat, fMemory[target.fAddress + i].fFloat);
      }
      NEXT();
    }
    LABEL(kStoreParameter) {
      ...
      for (int i = 0; i < width; ++i) {
        if (m[i]) {
          base[fRegisters[target.fIndex].fInt[i]].fInt[i] = fRegisters[src.fIndex].fInt[i];
        }
      }
      NEXT();
    }
    ...
    LABEL(kStoreParameterDirect) {
      ...
      base[target.fAddress].fFloat =
          skvx::if_then_else(mask(), fRegisters[src.fIndex].fFloat, base[target.fAddress].fFloat);
      NEXT();
    }
    LABEL(kStoreParameterDirectN) {
      ...
      for (int i = 0; i < count; ++i) {
        base[target.fAddress + i].fFloat = skvx::if_then_else(
            mask(), fRegisters[src.fIndex + i].fFloat, base[target.fAddress + i].fFloat);
      }
      NEXT();
    }
    LABEL(kStoreStack) {
    ...
    LABEL(kStoreStackDirect) {
      ...
      context.fStack[target.fAddress] = skvx::if_then_else(
          mask(), fRegisters[src.fIndex].fFloat, context.fStack[target.fAddress].fFloat);
      NEXT();
    }
    LABEL(kStoreStackDirectN) {
      ...
      for (int i = 0; i < count; ++i) {
        context.fStack[target.fAddress + i] = skvx::if_then_else(
            mask(), fRegisters[src.fIndex + i].fFloat, context.fStack[target.fAddress + i].fFloat);
      }
      NEXT();
    }
    ...
    LABEL(kUnsignedToFloat) {
      ByteCode::Register target = read<ByteCode::Register>(&ip);
      ByteCode::Register src = read<ByteCode::Register>(&ip);
      fRegisters[target.fIndex] = Vector(skvx::cast<float>(fRegisters[src.fIndex].fUInt));
      NEXT();
    }
