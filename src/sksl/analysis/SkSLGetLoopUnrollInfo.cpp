...
std::unique_ptr<LoopUnrollInfo> Analysis::GetLoopUnrollInfo(
    Position loopPos, const ForLoopPositions& positions, const Statement* loopInitializer,
    const Expression* loopTest, const Expression* loopNext, const Statement* loopStatement,
  ...
  if (!ConstantFolder::GetConstantValue(*initDecl.value(), &loopInfo->fStart)) {
    errors.error(
        loopInitializer->fPosition, "loop index initializer must be a constant expression");
    return nullptr;
  }
  ...
  switch (cond.getOperator().kind()) {
    ...
    case Operator::Kind::NEQ: break;
    default: errors.error(loopTest->fPosition, "invalid relational operator"); return nullptr;
  }
  double loopEnd = 0;
  ...
  switch (loopNext->kind()) {
    case Expression::Kind::kBinary: {
      ...
      if (!ConstantFolder::GetConstantValue(*next.right(), &loopInfo->fDelta)) {
        errors.error(loopNext->fPosition, "loop index must be modified by a constant expression");
        return nullptr;
      }
    ...
    default: errors.error(loopNext->fPosition, "invalid loop expression"); return nullptr;
  }
  ...
  if (Analysis::StatementWritesToVariable(*loopStatement, initDecl.var())) {
    errors.error(
        loopStatement->fPosition, "loop index must not be modified within body of the loop");
    return nullptr;
  }
  ...
  switch (cond.getOperator().kind()) {
    case Operator::Kind::LT:
      loopInfo->fCount = calculate_count(
