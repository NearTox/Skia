 ...
 public:
  ...
  void checkGlobalVariableSizeLimit(const GlobalVarDeclaration& globalDecl) {
    ...
    if (prevSlotsUsed < kVariableSlotLimit && fGlobalSlotsUsed >= kVariableSlotLimit) {
      fContext.fErrors->error(
          decl.fPosition,
          "global variable '" + std::string(decl.var().name()) + "' exceeds the size limit");
    }
  }
  ...
  void checkOutParamsAreAssigned(const FunctionDefinition& funcDef) {
    ...
    for (const Variable* param : funcDecl.parameters()) {
      const int paramInout =
          param->modifiers().fFlags & (Modifiers::Flag::kIn_Flag | Modifiers::Flag::kOut_Flag);
      if (!param->type().isStruct() && paramInout == Modifiers::Flag::kOut_Flag) {
        ProgramUsage::VariableCounts counts = fUsage.get(*param);
        if (counts.fWrite <= 0) {
          fContext.fErrors->error(
              param->fPosition, "function '" + std::string(funcDecl.name()) +
    ...
    switch (stmt.kind()) {
      case Statement::Kind::kIf:
        if (stmt.as<IfStatement>().isStatic()) {
          fContext.fErrors->error(
              stmt.as<IfStatement>().test()->fPosition, "static if has non-static test");
        }
        break;
      ...
      case Statement::Kind::kSwitch:
        if (stmt.as<SwitchStatement>().isStatic()) {
          fContext.fErrors->error(
              stmt.as<SwitchStatement>().value()->fPosition, "static switch has non-static test");
        }
        break;
    ...
    switch (expr.kind()) {
      case Expression::Kind::kFunctionCall: {
        ...
        if (!decl.isBuiltin() && !decl.definition()) {
          fContext.fErrors->error(
              expr.fPosition, "function '" + decl.description() + "' is not defined");
        }
        break;
      }
