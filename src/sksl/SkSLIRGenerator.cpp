...
class AutoSymbolTable {
 public:
  AutoSymbolTable(IRGenerator* ir) : fIR(ir), fPrevious(fIR->fSymbolTable) {
    fIR->pushSymbolTable();
  }
...
class AutoLoopLevel {
 public:
  AutoLoopLevel(IRGenerator* ir) : fIR(ir) { fIR->fLoopLevel++; }

  ~AutoLoopLevel() { fIR->fLoopLevel--; }
...
class AutoSwitchLevel {
 public:
  AutoSwitchLevel(IRGenerator* ir) : fIR(ir) { fIR->fSwitchLevel++; }

  ~AutoSwitchLevel() { fIR->fSwitchLevel--; }

  IRGenerator* fIR;
};

IRGenerator::IRGenerator(
    const Context* context, std::shared_ptr<SymbolTable> symbolTable, ErrorReporter& errorReporter)
    : fContext(*context),
...
void IRGenerator::popSymbolTable() { fSymbolTable = fSymbolTable->fParent; }

static void fill_caps(
    const SKSL_CAPS_CLASS& caps, std::unordered_map<String, Program::Settings::Value>* capsMap) {
#define CAP(name) \
...
void IRGenerator::start(
    const Program::Settings* settings, std::vector<std::unique_ptr<ProgramElement>>* inherited) {
  fSettings = settings;
  fCapsMap.clear();
  if (settings->fCaps) {
    fill_caps(*settings->fCaps, &fCapsMap);
  } else {
    fCapsMap.insert(std::make_pair(String("integerSupport"), Program::Settings::Value(true)));
  }
...
std::unique_ptr<Statement> IRGenerator::convertSingleStatement(const ASTNode& statement) {
  switch (statement.fKind) {
    case ASTNode::Kind::kBlock: return this->convertBlock(statement);
    case ASTNode::Kind::kVarDeclarations: return this->convertVarDeclarationStatement(statement);
    case ASTNode::Kind::kIf: return this->convertIf(statement);
    case ASTNode::Kind::kFor: return this->convertFor(statement);
    case ASTNode::Kind::kWhile: return this->convertWhile(statement);
    case ASTNode::Kind::kDo: return this->convertDo(statement);
    case ASTNode::Kind::kSwitch: return this->convertSwitch(statement);
    case ASTNode::Kind::kReturn: return this->convertReturn(statement);
    case ASTNode::Kind::kBreak: return this->convertBreak(statement);
    case ASTNode::Kind::kContinue: return this->convertContinue(statement);
    case ASTNode::Kind::kDiscard: return this->convertDiscard(statement);
    default:
      // it's an expression
        ...
        if (expr.fKind == Expression::kFunctionCall_Kind) {
          ...
          if (fc.fFunction.fBuiltin && fc.fFunction.fName == "EmitVertex") {
            ...
            return std::unique_ptr<Block>(
                new Block(statement.fOffset, std::move(statements), fSymbolTable));
          }
        }
      }
      return result;
...
std::unique_ptr<Statement> IRGenerator::convertStatement(const ASTNode& statement) {
  ...
  if (fExtraStatements.size()) {
    ...
    std::unique_ptr<Statement> block(new Block(-1, std::move(fExtraStatements), nullptr, false));
    fExtraStatements = std::move(oldExtraStatements);
    return block;
  }
  fExtraStatements = std::move(oldExtraStatements);
...
std::unique_ptr<VarDeclarations> IRGenerator::convertVarDeclarations(
  ...
  if (fKind != Program::kFragmentProcessor_Kind) {
    if ((modifiers.fFlags & Modifiers::kIn_Flag) && baseType->kind() == Type::Kind::kMatrix_Kind) {
    ...
    if ((modifiers.fFlags & Modifiers::kIn_Flag) && (modifiers.fFlags & Modifiers::kUniform_Flag)) {
      fErrors.error(
          decls.fOffset, "'in uniform' variables only permitted within fragment processors");
    }
  ...
  if (modifiers.fLayout.fFlags & Layout::kSRGBUnpremul_Flag) {
    ...
    if (!(modifiers.fFlags & Modifiers::kUniform_Flag)) {
      fErrors.error(decls.fOffset, "'srgb_unpremul' is only permitted on 'uniform' variables");
    }
  ...
  for (; iter != decls.end(); ++iter) {
    ...
    if (modifiers.fLayout.fLocation == 0 && modifiers.fLayout.fIndex == 0 &&
      ...
      fErrors.error(varDecl.fOffset, "out location=0, index=0 is reserved for sk_FragColor");
    }
    ...
    auto var = std::unique_ptr<Variable>(
        new Variable(varDecl.fOffset, modifiers, varData.fName, *type, storage));
    ...
    if (storage == Variable::kGlobal_Storage && var->fName == "sk_FragColor" &&
      ...
      // already defined, ignore
        ...
        storage == Variable::kGlobal_Storage && (*fSymbolTable)[var->fName] &&
      ...
      variables.emplace_back(new VarDeclaration(var.get(), std::move(sizes), std::move(value)));
      StringFragment name = var->fName;
      fSymbolTable->add(name, std::move(var));
    }
  }
  return std::unique_ptr<VarDeclarations>(
      new VarDeclarations(decls.fOffset, baseType, std::move(variables)));
}
...
std::unique_ptr<Statement> IRGenerator::convertIf(const ASTNode& n) {
  ...
  if (test->fKind == Expression::kBoolLiteral_Kind) {
    ...
    if (((BoolLiteral&)*test).fValue) {
      ...
      return std::unique_ptr<Statement>(new Block(n.fOffset, std::move(empty), fSymbolTable));
    }
  }
  return std::unique_ptr<Statement>(new IfStatement(
      n.fOffset, n.getBool(), std::move(test), std::move(ifTrue), std::move(ifFalse)));
}
...
std::unique_ptr<Statement> IRGenerator::convertFor(const ASTNode& f) {
  ...
  return std::unique_ptr<Statement>(new ForStatement(
      f.fOffset, std::move(initializer), std::move(test), std::move(next), std::move(statement),
      fSymbolTable));
}
...
std::unique_ptr<Statement> IRGenerator::convertWhile(const ASTNode& w) {
  ...
  return std::unique_ptr<Statement>(
      new WhileStatement(w.fOffset, std::move(test), std::move(statement)));
}
...
std::unique_ptr<Statement> IRGenerator::convertDo(const ASTNode& d) {
  ...
  return std::unique_ptr<Statement>(
      new DoStatement(d.fOffset, std::move(statement), std::move(test)));
}
...
std::unique_ptr<Statement> IRGenerator::convertReturn(const ASTNode& r) {
  ...
  if (r.begin() != r.end()) {
    ...
    if (fCurrentFunction->fReturnType != *fContext.fVoid_Type) {
      fErrors.error(
          r.fOffset,
          "expected function to return '" + fCurrentFunction->fReturnType.displayName() + "'");
    }
...
std::unique_ptr<Block> IRGenerator::applyInvocationIDWorkaround(std::unique_ptr<Block> main) {
  ...
  FunctionDeclaration* invokeDecl = new FunctionDeclaration(
      -1, invokeModifiers, "_invoke", std::vector<const Variable*>(), *fContext.fVoid_Type);
  fProgramElements->push_back(
      std::unique_ptr<ProgramElement>(new FunctionDefinition(-1, *invokeDecl, std::move(main))));
  ...
  SkASSERT(loopIdx);
  std::unique_ptr<Expression> test(new BinaryExpression(
      -1, std::unique_ptr<Expression>(new VariableReference(-1, *loopIdx)), Token::Kind::TK_LT,
  ...
  std::unique_ptr<Expression> next(new PostfixExpression(
      std::unique_ptr<Expression>(
          new VariableReference(-1, *loopIdx, VariableReference::kReadWrite_RefKind)),
  ...
  loopBody.push_back(std::unique_ptr<Statement>(new ExpressionStatement(
      this->call(-1, *invokeDecl, std::vector<std::unique_ptr<Expression>>()))));
  loopBody.push_back(std::unique_ptr<Statement>(new ExpressionStatement(
      this->call(-1, std::move(endPrimitive), std::vector<std::unique_ptr<Expression>>()))));
  std::unique_ptr<Expression> assignment(new BinaryExpression(
      -1, std::unique_ptr<Expression>(new VariableReference(-1, *loopIdx)), Token::Kind::TK_EQ,
      std::unique_ptr<IntLiteral>(new IntLiteral(fContext, -1, 0)), *fContext.fInt_Type));
  std::unique_ptr<Statement> initializer(new ExpressionStatement(std::move(assignment)));
  std::unique_ptr<Statement> loop = std::unique_ptr<Statement>(new ForStatement(
      -1, std::move(initializer), std::move(test), std::move(next),
      std::unique_ptr<Block>(new Block(-1, std::move(loopBody))), fSymbolTable));
...
#define REF(var) \
  std::unique_ptr<Expression>(new VariableReference(-1, *var, VariableReference::kRead_RefKind))
#define FIELD(var, idx)        \
  std::unique_ptr<Expression>( \
      new FieldAccess(REF(var), idx, FieldAccess::kAnonymousInterfaceBlock_OwnerKind))
#define POS                    \
  std::unique_ptr<Expression>( \
      new FieldAccess(REF(fSkPerVertex), 0, FieldAccess::kAnonymousInterfaceBlock_OwnerKind))
#define ADJUST \
  (fRTAdjustInterfaceBlock ? FIELD(fRTAdjustInterfaceBlock, fRTAdjustFieldIndex) : REF(fRTAdjust))
#define SWIZZLE(expr, ...) std::unique_ptr<Expression>(new Swizzle(fContext, expr, {__VA_ARGS__}))
#define OP(left, op, right) \
  std::unique_ptr<Expression>(new BinaryExpression(-1, left, op, right, *fContext.fFloat2_Type))
  std::vector<std::unique_ptr<Expression>> children;
  children.push_back(
      OP(OP(SWIZZLE(POS, 0, 1), Token::Kind::TK_STAR, SWIZZLE(ADJUST, 0, 2)), Token::Kind::TK_PLUS,
  ...
  std::unique_ptr<Expression> result =
      OP(POS, Token::Kind::TK_EQ,
         std::unique_ptr<Expression>(
             new Constructor(-1, *fContext.fFloat4_Type, std::move(children))));
...
void IRGenerator::convertFunction(const ASTNode& f) {
  ...
  for (size_t i = 0; i < fd.fParameterCount; ++i) {
    ...
    for (int j = (int)pd.fSizeCount; j >= 1; j--) {
      ...
      type = (Type*)fSymbolTable->takeOwnership(
          std::unique_ptr<Symbol>(new Type(std::move(name), Type::kArray_Kind, *type, size)));
    }
    StringFragment name = pd.fName;
    Variable* var = (Variable*)fSymbolTable->takeOwnership(std::unique_ptr<Symbol>(
        new Variable(param.fOffset, pd.fModifiers, name, *type, Variable::kParameter_Storage)));
    ...
    switch (fKind) {
      case Program::kPipelineStage_Kind: {
        ...
        switch (parameters.size()) {
          case 2:
            valid =
                parameters[0]->fType == *fContext.fFloat2_Type &&
                parameters[0]->fModifiers.fFlags == 0 &&
                parameters[1]->fType == *fContext.fHalf4_Type &&
                parameters[1]->fModifiers.fFlags == (Modifiers::kIn_Flag | Modifiers::kOut_Flag);
            break;
          case 1:
            valid =
                parameters[0]->fType == *fContext.fHalf4_Type &&
                parameters[0]->fModifiers.fFlags == (Modifiers::kIn_Flag | Modifiers::kOut_Flag);
            break;
          default: valid = false;
        }
        if (!valid) {
          fErrors.error(
              f.fOffset,
              "pipeline stage 'main' must be declared main(float2, "
              "inout half4) or main(inout half4)");
          return;
        }
        break;
      }
      case Program::kGeneric_Kind: break;
      default:
        if (parameters.size()) {
  ...
  if (entry) {
    ...
    switch (entry->fKind) {
      ...
      case Symbol::kFunctionDeclaration_Kind:
        ...
        break;
      default: fErrors.error(f.fOffset, "symbol '" + fd.fName + "' was already defined"); return;
    }
        ...
        if (match) {
          if (*returnType != other->fReturnType) {
            FunctionDeclaration newDecl(
                f.fOffset, fd.fModifiers, fd.fName, parameters, *returnType);
            fErrors.error(
                f.fOffset, "functions '" + newDecl.declaration() + "' and '" +
                               other->declaration() + "' differ only in return type");
            return;
          }
          decl = other;
          for (size_t i = 0; i < parameters.size(); i++) {
            if (parameters[i]->fModifiers != other->fParameters[i]->fModifiers) {
              fErrors.error(
                  f.fOffset, "modifiers on parameter " + to_string((uint64_t)i + 1) +
          ...
          if (other->fDefinition && !other->fBuiltin) {
            fErrors.error(f.fOffset, "duplicate definition of " + other->declaration());
          }
          break;
        }
      }
  ...
  if (!decl) {
    ...
    auto newDecl = std::unique_ptr<FunctionDeclaration>(
        new FunctionDeclaration(f.fOffset, fd.fModifiers, fd.fName, parameters, *returnType));
    decl = newDecl.get();
  ...
  if (iter != f.end()) {
    ...
    bool needInvocationIDWorkaround = fInvocations != -1 && fd.fName == "main" &&
                                      fSettings->fCaps && !fSettings->fCaps->gsInvocationsSupport();
...
std::unique_ptr<InterfaceBlock> IRGenerator::convertInterfaceBlock(const ASTNode& intf) {
  ...
  for (size_t i = 0; i < id.fDeclarationCount; ++i) {
    ...
    for (const auto& stmt : decl->fVars) {
      ...
      fields.push_back(Type::Field(vd.fVar->fModifiers, vd.fVar->fName, &vd.fVar->fType));
      if (vd.fValue) {
        fErrors.error(decl->fOffset, "initializers are not permitted on interface block fields");
      }
      if (vd.fVar->fModifiers.fFlags &
          (Modifiers::kIn_Flag | Modifiers::kOut_Flag | Modifiers::kUniform_Flag |
           Modifiers::kBuffer_Flag | Modifiers::kConst_Flag)) {
        fErrors.error(decl->fOffset, "interface block fields may not have storage qualifiers");
      }
      if (vd.fVar->fType.kind() == Type::kArray_Kind && vd.fVar->fType.columns() == -1) {
        haveRuntimeArray = true;
      }
    }
  }
  this->popSymbolTable();
  Type* type = (Type*)old->takeOwnership(
      std::unique_ptr<Symbol>(new Type(intf.fOffset, id.fTypeName, fields)));
  std::vector<std::unique_ptr<Expression>> sizes;
  ...
  Variable* var = (Variable*)old->takeOwnership(std::unique_ptr<Symbol>(new Variable(
      intf.fOffset, id.fModifiers, id.fInstanceName.fLength ? id.fInstanceName : id.fTypeName,
  ...
  if (id.fInstanceName.fLength) {
    ...
    for (size_t i = 0; i < fields.size(); i++) {
      old->add(fields[i].fName, std::unique_ptr<Field>(new Field(intf.fOffset, *var, (int)i)));
    }
  }
  return std::unique_ptr<InterfaceBlock>(new InterfaceBlock(
      intf.fOffset, var, id.fTypeName, id.fInstanceName, std::move(sizes), symbols));
}

void IRGenerator::getConstantInt(const Expression& value, int64_t* out) {
  switch (value.fKind) {
    case Expression::kIntLiteral_Kind: *out = ((const IntLiteral&)value).fValue; break;
    ...
    default: fErrors.error(value.fOffset, "expected a constant int");
  }
}
...
void IRGenerator::convertEnum(const ASTNode& e) {
  ...
  Layout layout;
  ASTNode enumType(
      e.fNodes, e.fOffset, ASTNode::Kind::kType, ASTNode::TypeData(e.getString(), false, false));
  ...
  for (auto iter = e.begin(); iter != e.end(); ++iter) {
    ...
    ++currentValue;
    auto var = std::unique_ptr<Variable>(new Variable(
        e.fOffset, modifiers, child.getString(), *type, Variable::kGlobal_Storage, value.get()));
    variables.push_back(var.get());
    symbols->add(child.getString(), std::move(var));
    symbols->takeOwnership(std::move(value));
  }
  fProgramElements->push_back(
      std::unique_ptr<ProgramElement>(new Enum(e.fOffset, e.getString(), symbols)));
  fSymbolTable = symbols->fParent;
}
  ...
  if (result && result->fKind == Symbol::kType_Kind) {
    if (td.fIsNullable) {
      if (((Type&)*result) == *fContext.fFragmentProcessor_Type) {
        if (type.begin() != type.end()) {
          fErrors.error(
              type.fOffset, "type '" + td.fName +
                                "' may not be used in "
                                "an array");
        }
        result = fSymbolTable->takeOwnership(std::unique_ptr<Symbol>(
            new Type(String(result->fName) + "?", Type::kNullable_Kind, (const Type&)*result)));
      } else {
    ...
    for (const auto& size : type) {
      ...
      result = (Type*)fSymbolTable->takeOwnership(std::unique_ptr<Symbol>(
          new Type(name, Type::kArray_Kind, (const Type&)*result, size ? size.getInt() : 0)));
    }
...
std::unique_ptr<Expression> IRGenerator::convertExpression(const ASTNode& expr) {
  switch (expr.fKind) {
    case ASTNode::Kind::kBinary: return this->convertBinaryExpression(expr);
    case ASTNode::Kind::kBool:
      return std::unique_ptr<Expression>(new BoolLiteral(fContext, expr.fOffset, expr.getBool()));
    case ASTNode::Kind::kCall: return this->convertCallExpression(expr);
    case ASTNode::Kind::kField: return this->convertFieldExpression(expr);
    case ASTNode::Kind::kFloat:
      return std::unique_ptr<Expression>(new FloatLiteral(fContext, expr.fOffset, expr.getFloat()));
    case ASTNode::Kind::kIdentifier: return this->convertIdentifier(expr);
    case ASTNode::Kind::kIndex: return this->convertIndexExpression(expr);
    case ASTNode::Kind::kInt:
      return std::unique_ptr<Expression>(new IntLiteral(fContext, expr.fOffset, expr.getInt()));
    case ASTNode::Kind::kNull:
      return std::unique_ptr<Expression>(new NullLiteral(fContext, expr.fOffset));
    case ASTNode::Kind::kPostfix: return this->convertPostfixExpression(expr);
    case ASTNode::Kind::kPrefix: return this->convertPrefixExpression(expr);
    case ASTNode::Kind::kTernary: return this->convertTernaryExpression(expr);
    default:
#ifdef SK_DEBUG
...
std::unique_ptr<Expression> IRGenerator::convertIdentifier(const ASTNode& identifier) {
  ...
  switch (result->fKind) {
    case Symbol::kFunctionDeclaration_Kind: {
      std::vector<const FunctionDeclaration*> f = {(const FunctionDeclaration*)result};
      return std::unique_ptr<FunctionReference>(
          new FunctionReference(fContext, identifier.fOffset, f));
    }
    case Symbol::kUnresolvedFunction_Kind: {
      const UnresolvedFunction* f = (const UnresolvedFunction*)result;
      return std::unique_ptr<FunctionReference>(
          new FunctionReference(fContext, identifier.fOffset, f->fFunctions));
    }
    case Symbol::kVariable_Kind: {
      const Variable* var = (const Variable*)result;
      switch (var->fModifiers.fLayout.fBuiltin) {
        case SK_WIDTH_BUILTIN: fInputs.fRTWidth = true; break;
      ...
      return std::unique_ptr<VariableReference>(
          new VariableReference(identifier.fOffset, *var, VariableReference::kRead_RefKind));
    }
    case Symbol::kField_Kind: {
      const Field* field = (const Field*)result;
      VariableReference* base = new VariableReference(
          identifier.fOffset, field->fOwner, VariableReference::kRead_RefKind);
      return std::unique_ptr<Expression>(new FieldAccess(
          std::unique_ptr<Expression>(base), field->fFieldIndex,
          FieldAccess::kAnonymousInterfaceBlock_OwnerKind));
    }
    case Symbol::kType_Kind: {
      const Type* t = (const Type*)result;
      return std::unique_ptr<TypeReference>(new TypeReference(fContext, identifier.fOffset, *t));
    }
    ...
    default: ABORT("unsupported symbol type %d\n", result->fKind);
  }
}
...
std::unique_ptr<Section> IRGenerator::convertSection(const ASTNode& s) {
  ...
  return std::unique_ptr<Section>(
      new Section(s.fOffset, section.fName, section.fArgument, section.fText));
}

std::unique_ptr<Expression> IRGenerator::coerce(
    std::unique_ptr<Expression> expr, const Type& type) {
  if (!expr) {
    return nullptr;
  }
  if (expr->fType == type) {
  ...
  if (expr->coercionCost(type) == INT_MAX) {
    fErrors.error(
        expr->fOffset,
        "expected '" + type.displayName() + "', but found '" + expr->fType.displayName() + "'");
    return nullptr;
  }
  if (type.kind() == Type::kScalar_Kind) {
    ...
    if (type == *fContext.fFloatLiteral_Type) {
      ctor =
          this->convertIdentifier(ASTNode(&fFile->fNodes, -1, ASTNode::Kind::kIdentifier, "float"));
    } else if (type == *fContext.fIntLiteral_Type) {
      ctor =
          this->convertIdentifier(ASTNode(&fFile->fNodes, -1, ASTNode::Kind::kIdentifier, "int"));
    } else {
      ctor = this->convertIdentifier(
          ASTNode(&fFile->fNodes, -1, ASTNode::Kind::kIdentifier, type.fName));
    }
    if (!ctor) {
 ...
 */
static bool determine_binary_type(
    const Context& context, Token::Kind op, const Type& left, const Type& right,
    const Type** outLeftType, const Type** outRightType, const Type** outResultType,
    bool tryFlipped) {
  bool isLogical;
  ...
  switch (op) {
    ...
    case Token::Kind::TK_LOGICALXOREQ:
      ...
      *outResultType = context.fBool_Type.get();
      return left.canCoerceTo(*context.fBool_Type) && right.canCoerceTo(*context.fBool_Type);
    ...
    case Token::Kind::TK_STAR:
      if (is_matrix_multiply(left, right)) {
        ...
        if (determine_binary_type(
                context, Token::Kind::TK_STAR, left.componentType(), right.componentType(),
    ...
    case Token::Kind::TK_COMMA:
      ...
      *outResultType = &right;
      return true;
    default: isLogical = false; validMatrixOrVectorOp = false;
  }
  ...
  if ((left.kind() == Type::kVector_Kind || left.kind() == Type::kMatrix_Kind) &&
    ...
    if (determine_binary_type(
            context, op, left.componentType(), right, outLeftType, outRightType, outResultType,
            false)) {
  ...
  if (tryFlipped) {
    return determine_binary_type(
        context, op, right, left, outRightType, outLeftType, outResultType, false);
  }
  return false;
}

static std::unique_ptr<Expression> short_circuit_boolean(
    const Context& context, const Expression& left, Token::Kind op, const Expression& right) {
  ...
  if (op == Token::Kind::TK_LOGICALAND) {
    ...
    return leftVal ? std::unique_ptr<Expression>(
                         new PrefixExpression(Token::Kind::TK_LOGICALNOT, right.clone()))
                   : right.clone();
  } else {
    return nullptr;
  }
...
std::unique_ptr<Expression> IRGenerator::constantFold(
    const Expression& left, Token::Kind op, const Expression& right) const {
  ...
  if (left.fKind == Expression::kBoolLiteral_Kind && right.fKind == Expression::kBoolLiteral_Kind) {
    bool leftVal = ((BoolLiteral&)left).fValue;
...
#define RESULT(t, op) \
  std::unique_ptr<Expression>(new t##Literal(fContext, left.fOffset, leftVal op rightVal))
#define URESULT(t, op)         \
  std::unique_ptr<Expression>( \
      new t##Literal(fContext, left.fOffset, (uint32_t)leftVal op(uint32_t) rightVal))
...
#define RETURN_VEC_COMPONENTWISE_RESULT(op)                              \
  for (int i = 0; i < left.fType.columns(); i++) {                       \
    float value = left.getFVecComponent(i) op right.getFVecComponent(i); \
    args.emplace_back(new FloatLiteral(fContext, -1, value));            \
  }                                                                      \
  return std::unique_ptr<Expression>(new Constructor(-1, left.fType, std::move(args)))
    switch (op) {
      case Token::Kind::TK_EQEQ:
        return std::unique_ptr<Expression>(
            new BoolLiteral(fContext, -1, left.compareConstant(fContext, right)));
      case Token::Kind::TK_NEQ:
        return std::unique_ptr<Expression>(
            new BoolLiteral(fContext, -1, !left.compareConstant(fContext, right)));
  ...
  if (left.fType.kind() == Type::kMatrix_Kind && right.fType.kind() == Type::kMatrix_Kind &&
...
std::unique_ptr<Expression> IRGenerator::convertBinaryExpression(const ASTNode& expression) {
  ...
  if (!determine_binary_type(
          fContext, op, *rawLeftType, *rawRightType, &leftType, &rightType, &resultType,
          !Compiler::IsAssignment(op))) {
    fErrors.error(
        expression.fOffset, String("type mismatch: '") +
  ...
  if (Compiler::IsAssignment(op)) {
    this->setRefKind(
        *left, op != Token::Kind::TK_EQ ? VariableReference::kReadWrite_RefKind
  ...
  if (!result) {
    result = std::unique_ptr<Expression>(new BinaryExpression(
...
std::unique_ptr<Expression> IRGenerator::convertTernaryExpression(const ASTNode& node) {
  ...
  const Type* resultType;
  if (!determine_binary_type(
          fContext, Token::Kind::TK_EQEQ, ifTrue->fType, ifFalse->fType, &trueType, &falseType,
          &resultType, true) ||
      trueType != falseType) {
    fErrors.error(
        node.fOffset, "ternary operator result mismatch: '" + ifTrue->fType.displayName() + "', '" +
  ...
  return std::unique_ptr<Expression>(
      new TernaryExpression(node.fOffset, std::move(test), std::move(ifTrue), std::move(ifFalse)));
}

std::unique_ptr<Expression> IRGenerator::inlineExpression(
    int offset, std::map<const Variable*, const Variable*>* varMap, const Expression& expression) {
  ...
  switch (expression.fKind) {
    case Expression::kBinary_Kind: {
      ...
      return std::unique_ptr<Expression>(
          new BinaryExpression(offset, expr(b.fLeft), b.fOperator, expr(b.fRight), b.fType));
    }
    case Expression::kBoolLiteral_Kind:
    case Expression::kIntLiteral_Kind:
    case Expression::kFloatLiteral_Kind:
    case Expression::kNullLiteral_Kind: return expression.clone();
    ...
    case Expression::kExternalFunctionCall_Kind: {
      ...
      return std::unique_ptr<Expression>(
          new ExternalFunctionCall(offset, e.fType, e.fFunction, std::move(args)));
    }
    case Expression::kExternalValue_Kind: return expression.clone();
    case Expression::kFieldAccess_Kind: {
      const FieldAccess& f = (const FieldAccess&)expression;
      return std::unique_ptr<Expression>(
          new FieldAccess(expr(f.fBase), f.fFieldIndex, f.fOwnerKind));
    }
    case Expression::kFunctionCall_Kind: {
      ...
      return std::unique_ptr<Expression>(
          new FunctionCall(offset, c.fType, c.fFunction, std::move(args)));
    }
    case Expression::kIndex_Kind: {
      const IndexExpression& idx = (const IndexExpression&)expression;
      return std::unique_ptr<Expression>(
          new IndexExpression(fContext, expr(idx.fBase), expr(idx.fIndex)));
    }
    ...
    case Expression::kPostfix_Kind: {
      ...
      return std::unique_ptr<Expression>(new PostfixExpression(expr(p.fOperand), p.fOperator));
    }
    case Expression::kSetting_Kind: return expression.clone();
    ...
    case Expression::kTernary_Kind: {
      ...
      return std::unique_ptr<Expression>(
          new TernaryExpression(offset, expr(t.fTest), expr(t.fIfTrue), expr(t.fIfFalse)));
    }
    case Expression::kVariableReference_Kind: {
      ...
      if (found != varMap->end()) {
        return std::unique_ptr<Expression>(
            new VariableReference(offset, *found->second, v.fRefKind));
      }
      return v.clone();
    }
...
std::unique_ptr<Statement> IRGenerator::inlineStatement(
    int offset, std::map<const Variable*, const Variable*>* varMap, const Variable* returnVar,
    bool haveEarlyReturns, const Statement& statement) {
  ...
  switch (statement.fKind) {
    case Statement::kBlock_Kind: {
      ...
      return std::unique_ptr<Statement>(
          new Block(offset, stmts(b.fStatements), b.fSymbols, b.fIsScope));
    }

    case Statement::kBreak_Kind:
    case Statement::kContinue_Kind:
    case Statement::kDiscard_Kind: return statement.clone();

    case Statement::kDo_Kind: {
      const DoStatement& d = (const DoStatement&)statement;
      return std::unique_ptr<Statement>(new DoStatement(offset, stmt(d.fStatement), expr(d.fTest)));
    }
    ...
    case Statement::kFor_Kind: {
      ...
      return std::unique_ptr<Statement>(new ForStatement(
          offset, std::move(initializer), expr(f.fTest), expr(f.fNext), stmt(f.fStatement),
          f.fSymbols));
    }
    case Statement::kIf_Kind: {
      const IfStatement& i = (const IfStatement&)statement;
      return std::unique_ptr<Statement>(
          new IfStatement(offset, i.fIsStatic, expr(i.fTest), stmt(i.fIfTrue), stmt(i.fIfFalse)));
    }
    case Statement::kNop_Kind: return statement.clone();
    case Statement::kReturn_Kind: {
      const ReturnStatement& r = (const ReturnStatement&)statement;
      if (r.fExpression) {
        std::unique_ptr<Statement> assignment(
            new ExpressionStatement(std::unique_ptr<Expression>(new BinaryExpression(
                offset,
                std::unique_ptr<Expression>(
                    new VariableReference(offset, *returnVar, VariableReference::kWrite_RefKind)),
                Token::Kind::TK_EQ, expr(r.fExpression), returnVar->fType))));
        if (haveEarlyReturns) {
          std::vector<std::unique_ptr<Statement>> block;
          block.push_back(std::move(assignment));
          block.emplace_back(new BreakStatement(offset));
          return std::unique_ptr<Statement>(new Block(offset, std::move(block), nullptr, false));
        } else {
          return assignment;
        }
      } else {
    ...
    case Statement::kSwitch_Kind: {
      ...
      for (const auto& sc : ss.fCases) {
        cases.emplace_back(new SwitchCase(offset, expr(sc->fValue), stmts(sc->fStatements)));
      }
      return std::unique_ptr<Statement>(new SwitchStatement(
          offset, ss.fIsStatic, expr(ss.fValue), std::move(cases), ss.fSymbols));
    }
    case Statement::kVarDeclaration_Kind: {
      ...
      Variable* clone = (Variable*)fSymbolTable->takeOwnership(std::unique_ptr<Symbol>(new Variable(
          offset, old->fModifiers, namePtr->c_str(), *typePtr, old->fStorage, initialValue.get())));
      (*varMap)[old] = clone;
      return std::unique_ptr<Statement>(
          new VarDeclaration(clone, std::move(sizes), std::move(initialValue)));
    }
    case Statement::kVarDeclarations_Kind: {
      ...
      return std::unique_ptr<Statement>(new VarDeclarationsStatement(
          std::unique_ptr<VarDeclarations>(new VarDeclarations(offset, typePtr, std::move(vars)))));
    }
    case Statement::kWhile_Kind: {
      const WhileStatement& w = (const WhileStatement&)statement;
      return std::unique_ptr<Statement>(
          new WhileStatement(offset, expr(w.fTest), stmt(w.fStatement)));
    }
    default: SkASSERT(false); return nullptr;
  }
}
...
int return_count(const Statement& statement) {
  switch (statement.fKind) {
    ...
    case Statement::kReturn_Kind: return 1;
    case Statement::kSwitch_Kind: {
      const SwitchStatement& ss = (const SwitchStatement&)statement;
      int result = 0;
      for (const auto& sc : ss.fCases) {
    ...
    case Statement::kVarDeclaration_Kind:
    case Statement::kVarDeclarations_Kind: return 0;
...
std::unique_ptr<Expression> IRGenerator::inlineCall(
    int offset, const FunctionDefinition& function,
  ...
  if (function.fDeclaration.fReturnType != *fContext.fVoid_Type) {
    ...
    String* namePtr = (String*)fSymbolTable->takeOwnership(std::move(name));
    resultVar = (Variable*)fSymbolTable->takeOwnership(std::unique_ptr<Symbol>(new Variable(
        -1, Modifiers(), namePtr->c_str(), function.fDeclaration.fReturnType,
        Variable::kLocal_Storage, nullptr)));
    std::vector<std::unique_ptr<VarDeclaration>> variables;
    variables.emplace_back(new VarDeclaration(resultVar, {}, nullptr));
    fExtraStatements.emplace_back(new VarDeclarationsStatement(std::unique_ptr<VarDeclarations>(
        new VarDeclarations(offset, &resultVar->fType, std::move(variables)))));
  ...
  for (int i = 0; i < (int)arguments.size(); ++i) {
    ...
    Variable* argVar = (Variable*)fSymbolTable->takeOwnership(std::unique_ptr<Symbol>(new Variable(
        -1, Modifiers(), argNamePtr->c_str(), arguments[i]->fType, Variable::kLocal_Storage,
        arguments[i].get())));
    ...
    fExtraStatements.emplace_back(new VarDeclarationsStatement(std::unique_ptr<VarDeclarations>(
        new VarDeclarations(offset, &argVar->fType, std::move(vars)))));
  }
  ...
  if (hasEarlyReturn) {
    ...
    // emulate a goto.
    fExtraStatements.emplace_back(new DoStatement(
        -1, std::unique_ptr<Statement>(new Block(-1, std::move(inlined))),
    ...
    fExtraStatements.emplace_back(std::unique_ptr<Statement>(new Block(-1, std::move(inlined))));
  }
  ...
  for (size_t i = 0; i < arguments.size(); ++i) {
    ...
    if (p->fModifiers.fFlags & Modifiers::kOut_Flag) {
      ...
      fExtraStatements.emplace_back(
          new ExpressionStatement(std::unique_ptr<Expression>(new BinaryExpression(
              offset, arguments[i]->clone(), Token::Kind::TK_EQ, std::move(varRef),
              arguments[i]->fType))));
    }
  }
  if (function.fDeclaration.fReturnType != *fContext.fVoid_Type) {
...
std::unique_ptr<Expression> IRGenerator::call(
    int offset, const FunctionDeclaration& function,
    std::vector<std::unique_ptr<Expression>> arguments) {
  ...
  if (function.fParameters.size() != arguments.size()) {
    String msg = "call to '" + function.fName + "' expected " +
                 to_string((uint64_t)function.fParameters.size()) + " argument";
  ...
  for (size_t i = 0; i < arguments.size(); i++) {
    ...
    if (arguments[i] && (function.fParameters[i]->fModifiers.fFlags & Modifiers::kOut_Flag)) {
      this->setRefKind(
          *arguments[i], function.fParameters[i]->fModifiers.fFlags & Modifiers::kIn_Flag
  ...
  return std::unique_ptr<FunctionCall>(
      new FunctionCall(offset, *returnType, function, std::move(arguments)));
}
...
std::unique_ptr<Expression> IRGenerator::call(
    int offset, std::unique_ptr<Expression> functionValue,
    std::vector<std::unique_ptr<Expression>> arguments) {
  switch (functionValue->fKind) {
    case Expression::kTypeReference_Kind:
      return this->convertConstructor(
          offset, ((TypeReference&)*functionValue).fValue, std::move(arguments));
    case Expression::kExternalValue_Kind: {
      ...
      if (count != (int)arguments.size()) {
        fErrors.error(
            offset, "external function expected " + to_string(count) + " arguments, but found " +
      ...
      return std::unique_ptr<Expression>(
          new ExternalFunctionCall(offset, v->callReturnType(), v, std::move(arguments)));
    }
    ...
    default: fErrors.error(offset, "not a function"); return nullptr;
  }
}

std::unique_ptr<Expression> IRGenerator::convertNumberConstructor(
    int offset, const Type& type, std::vector<std::unique_ptr<Expression>> args) {
  ...
  if (args[0]->fKind == Expression::kIntLiteral_Kind &&
    ...
    return std::unique_ptr<Expression>(
        new IntLiteral(offset, ((IntLiteral&)*args[0]).fValue, &type));
  }
  if (args[0]->fType == *fContext.fBool_Type) {
    ...
    return std::unique_ptr<Expression>(new TernaryExpression(
        offset, std::move(args[0]), this->coerce(std::move(one), type),
...
int component_count(const Type& type) {
  switch (type.kind()) {
    case Type::kVector_Kind: return type.columns();
    case Type::kMatrix_Kind: return type.columns() * type.rows();
    default: return 1;
  }
}
  ...
  if (args.size() != 1 || expected != component_count(args[0]->fType) ||
    ...
    for (size_t i = 0; i < args.size(); i++) {
      if (args[i]->fType.kind() == Type::kVector_Kind) {
        if (type.componentType().isNumber() != args[i]->fType.componentType().isNumber()) {
          fErrors.error(
              offset, "'" + args[i]->fType.displayName() +
                          "' is not a valid "
                          "parameter to '" +
    ...
    if (actual != 1 && actual != expected) {
      fErrors.error(
          offset, "invalid arguments to '" + type.displayName() + "' constructor (expected " +
                      to_string(expected) + " scalars, but found " + to_string(actual) + ")");
      return nullptr;
    }
  }
  return std::unique_ptr<Expression>(new Constructor(offset, type, std::move(args)));
  ...
  switch (expression.getToken().fKind) {
    case Token::Kind::TK_PLUS:
      if (!base->fType.isNumber() && base->fType.kind() != Type::kVector_Kind &&
        ...
        fErrors.error(
            expression.fOffset, "'+' cannot operate on '" + base->fType.displayName() + "'");
        return nullptr;
      }
      return base;
    case Token::Kind::TK_MINUS:
      if (base->fKind == Expression::kIntLiteral_Kind) {
        return std::unique_ptr<Expression>(
            new IntLiteral(fContext, base->fOffset, -((IntLiteral&)*base).fValue));
      }
      if (base->fKind == Expression::kFloatLiteral_Kind) {
        double value = -((FloatLiteral&)*base).fValue;
        return std::unique_ptr<Expression>(new FloatLiteral(fContext, base->fOffset, value));
      }
      if (!base->fType.isNumber() && base->fType.kind() != Type::kVector_Kind) {
        fErrors.error(
            expression.fOffset, "'-' cannot operate on '" + base->fType.displayName() + "'");
      ...
      return std::unique_ptr<Expression>(
          new PrefixExpression(Token::Kind::TK_MINUS, std::move(base)));
    ...
    case Token::Kind::TK_LOGICALNOT:
      ...
      if (base->fKind == Expression::kBoolLiteral_Kind) {
        return std::unique_ptr<Expression>(
            new BoolLiteral(fContext, base->fOffset, !((BoolLiteral&)*base).fValue));
      }
      break;
    case Token::Kind::TK_BITWISENOT:
      ...
      break;
    default: ABORT("unsupported prefix operator\n");
  }
  return std::unique_ptr<Expression>(
      new PrefixExpression(expression.getToken().fKind, std::move(base)));
}

std::unique_ptr<Expression> IRGenerator::convertIndex(
    std::unique_ptr<Expression> base, const ASTNode& index) {
  if (base->fKind == Expression::kTypeReference_Kind) {
    if (index.fKind == ASTNode::Kind::kInt) {
      ...
      Type* newType = (Type*)fSymbolTable->takeOwnership(std::unique_ptr<Symbol>(new Type(
          oldType.name() + "[" + to_string(size) + "]", Type::kArray_Kind, oldType, size)));
      return std::unique_ptr<Expression>(new TypeReference(fContext, base->fOffset, *newType));
  ...
  return std::unique_ptr<Expression>(
      new IndexExpression(fContext, std::move(base), std::move(converted)));
}

std::unique_ptr<Expression> IRGenerator::convertField(
    std::unique_ptr<Expression> base, StringFragment field) {
  ...
  fErrors.error(
      base->fOffset, "type '" + base->fType.displayName() +
                         "' does not have a "
                         "field named '" +
                         field + "");
  return nullptr;
...
std::unique_ptr<Expression> IRGenerator::convertSwizzle(
    std::unique_ptr<Expression> base, StringFragment fields) {
  ...
  for (size_t i = 0; i < fields.fLength; i++) {
    switch (fields[i]) {
      ...
      case 'B':
        ...
        [[fallthrough]];
      default:
        fErrors.error(base->fOffset, String::printf("invalid swizzle component '%c'", fields[i]));
        return nullptr;
    }
  }
...
std::unique_ptr<Expression> IRGenerator::getCap(int offset, String name) {
  ...
  return std::unique_ptr<Expression>(
      new Setting(offset, fullName, found->second.literal(fContext, offset)));
}
...
std::unique_ptr<Expression> IRGenerator::convertTypeField(
    int offset, const Type& type, StringFragment field) {
  std::unique_ptr<Expression> result;
...
std::unique_ptr<Expression> IRGenerator::convertIndexExpression(const ASTNode& index) {
  ...
  if (iter != index.end()) {
    ...
    const Type& oldType = ((TypeReference&)*base).fValue;
    Type* newType = (Type*)fSymbolTable->takeOwnership(
        std::unique_ptr<Symbol>(new Type(oldType.name() + "[]", Type::kArray_Kind, oldType, -1)));
    return std::unique_ptr<Expression>(new TypeReference(fContext, base->fOffset, *newType));
  }
...
std::unique_ptr<Expression> IRGenerator::convertFieldExpression(const ASTNode& fieldNode) {
  ...
  if (base->fKind == Expression::kTypeReference_Kind) {
    return this->convertTypeField(base->fOffset, ((TypeReference&)*base).fValue, field);
  }
  if (base->fKind == Expression::kExternalValue_Kind) {
    return this->convertField(std::move(base), field);
  }
  switch (base->fType.kind()) {
    case Type::kVector_Kind: return this->convertSwizzle(std::move(base), field);
    case Type::kOther_Kind:
    case Type::kStruct_Kind: return this->convertField(std::move(base), field);
    default:
      fErrors.error(
          base->fOffset, "cannot swizzle value of type '" + base->fType.displayName() + "'");
      return nullptr;
  }
}
...
std::unique_ptr<Expression> IRGenerator::convertPostfixExpression(const ASTNode& expression) {
  ...
  this->setRefKind(*base, VariableReference::kReadWrite_RefKind);
  return std::unique_ptr<Expression>(
      new PostfixExpression(std::move(base), expression.getToken().fKind));
}
...
bool IRGenerator::checkSwizzleWrite(const Swizzle& swizzle) {
  ...
  for (int idx : swizzle.fComponents) {
    ...
    int bit = 1 << idx;
    if (bits & bit) {
      fErrors.error(swizzle.fOffset, "cannot write to the same swizzle field more than once");
      return false;
    }
    bits |= bit;
  }
...
void IRGenerator::setRefKind(const Expression& expr, VariableReference::RefKind kind) {
  switch (expr.fKind) {
    ...
    case Expression::kFieldAccess_Kind: this->setRefKind(*((FieldAccess&)expr).fBase, kind); break;
    case Expression::kSwizzle_Kind: {
    ...
    case Expression::kIndex_Kind: this->setRefKind(*((IndexExpression&)expr).fBase, kind); break;
    case Expression::kTernary_Kind: {
    ...
    case Expression::kExternalValue_Kind: {
      ...
      if (!v.canWrite()) {
        fErrors.error(expr.fOffset, "cannot modify immutable external value '" + v.fName + "'");
      }
      break;
    }
    default: fErrors.error(expr.fOffset, "cannot assign to this expression"); break;
  }
}

void IRGenerator::convertProgram(
    Program::Kind kind, const char* text, size_t length, SymbolTable& types,
  ...
  for (const auto& decl : fFile->root()) {
    switch (decl.fKind) {
      case ASTNode::Kind::kVarDeclarations: {
        std::unique_ptr<VarDeclarations> s =
      ...
      case ASTNode::Kind::kExtension: {
        std::unique_ptr<Extension> e = this->convertExtension(decl.fOffset, decl.getString());
        if (e) {
