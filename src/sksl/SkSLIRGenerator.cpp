...
std::unique_ptr<Statement> IRGenerator::convertStatement(const ASTStatement& statement) {
  switch (statement.fKind) {
    case ASTStatement::kBlock_Kind: return this->convertBlock((ASTBlock&)statement);
    ...
    case ASTStatement::kIf_Kind: return this->convertIf((ASTIfStatement&)statement);
    case ASTStatement::kFor_Kind: return this->convertFor((ASTForStatement&)statement);
    case ASTStatement::kWhile_Kind: return this->convertWhile((ASTWhileStatement&)statement);
    case ASTStatement::kDo_Kind: return this->convertDo((ASTDoStatement&)statement);
    case ASTStatement::kSwitch_Kind: return this->convertSwitch((ASTSwitchStatement&)statement);
    case ASTStatement::kReturn_Kind: return this->convertReturn((ASTReturnStatement&)statement);
    case ASTStatement::kBreak_Kind: return this->convertBreak((ASTBreakStatement&)statement);
    case ASTStatement::kContinue_Kind:
      return this->convertContinue((ASTContinueStatement&)statement);
    case ASTStatement::kDiscard_Kind: return this->convertDiscard((ASTDiscardStatement&)statement);
    default: ABORT("unsupported statement type: %d\n", statement.fKind);
  }
}
...
std::unique_ptr<VarDeclarations> IRGenerator::convertVarDeclarations(
    const ASTVarDeclarations& decl, Variable::Storage storage) {
  std::vector<std::unique_ptr<VarDeclaration>> variables;
  const Type* baseType = this->convertType(*decl.fType);
  if (!baseType) {
    return nullptr;
  }
  if (fKind != Program::kFragmentProcessor_Kind && (decl.fModifiers.fFlags & Modifiers::kIn_Flag) &&
  ...
  for (const auto& varDecl : decl.fVars) {
    if (decl.fModifiers.fLayout.fLocation == 0 && decl.fModifiers.fLayout.fIndex == 0 &&
      ...
      fErrors.error(decl.fOffset, "out location=0, index=0 is reserved for sk_FragColor");
    }
    ...
    auto var = std::unique_ptr<Variable>(
        new Variable(decl.fOffset, decl.fModifiers, varDecl.fName, *type, storage));
    ...
    if (storage == Variable::kGlobal_Storage && varDecl.fName == "sk_FragColor" &&
      ...
      // already defined, ignore
        ...
        storage == Variable::kGlobal_Storage && (*fSymbolTable)[varDecl.fName] &&
  ...
  return std::unique_ptr<VarDeclarations>(
      new VarDeclarations(decl.fOffset, baseType, std::move(variables)));
}
...
std::unique_ptr<Statement> IRGenerator::convertIf(const ASTIfStatement& s) {
  std::unique_ptr<Expression> test =
      this->coerce(this->convertExpression(*s.fTest), *fContext.fBool_Type);
  if (!test) {
    return nullptr;
  }
  ...
  if (test->fKind == Expression::kBoolLiteral_Kind) {
    ...
    if (((BoolLiteral&)*test).fValue) {
      ...
      return std::unique_ptr<Statement>(new Block(s.fOffset, std::move(empty), fSymbolTable));
    }
  }
  return std::unique_ptr<Statement>(new IfStatement(
      s.fOffset, s.fIsStatic, std::move(test), std::move(ifTrue), std::move(ifFalse)));
}
...
std::unique_ptr<Statement> IRGenerator::convertWhile(const ASTWhileStatement& w) {
  AutoLoopLevel level(this);
  std::unique_ptr<Expression> test =
      this->coerce(this->convertExpression(*w.fTest), *fContext.fBool_Type);
  if (!test) {
    return nullptr;
  }
...
std::unique_ptr<Statement> IRGenerator::convertDo(const ASTDoStatement& d) {
  AutoLoopLevel level(this);
  std::unique_ptr<Expression> test =
      this->coerce(this->convertExpression(*d.fTest), *fContext.fBool_Type);
  if (!test) {
    return nullptr;
  }
...
std::unique_ptr<Statement> IRGenerator::convertReturn(const ASTReturnStatement& r) {
  ...
  if (r.fExpression) {
    ...
    if (fCurrentFunction->fReturnType != *fContext.fVoid_Type) {
      fErrors.error(
          r.fOffset,
          "expected function to return '" + fCurrentFunction->fReturnType.description() + "'");
    }
...
std::unique_ptr<Block> IRGenerator::applyInvocationIDWorkaround(std::unique_ptr<Block> main) {
  ...
  std::unique_ptr<Expression> test(new BinaryExpression(
      -1, std::unique_ptr<Expression>(new VariableReference(-1, *loopIdx)), Token::LT,
  ...
  std::unique_ptr<Expression> assignment(new BinaryExpression(
      -1, std::unique_ptr<Expression>(new VariableReference(-1, *loopIdx)), Token::EQ,
...
#define OP(left, op, right) \
  ...
  children.push_back(
      OP(OP(SWIZZLE(POS, 0, 1), Token::STAR, SWIZZLE(ADJUST, 0, 2)), Token::PLUS,
...
void IRGenerator::convertFunction(const ASTFunction& f) {
  ...
  for (const auto& param : f.fParameters) {
    ...
    Variable* var = (Variable*)fSymbolTable->takeOwnership(std::unique_ptr<Symbol>(new Variable(
        param->fOffset, param->fModifiers, name, *type, Variable::kParameter_Storage)));
    ...
    switch (fKind) {
      case Program::kPipelineStage_Kind: {
        ...
        switch (parameters.size()) {
          case 3:
            valid =
                ...
                parameters[2]->fModifiers.fFlags == (Modifiers::kIn_Flag | Modifiers::kOut_Flag);
            break;
          case 1:
            valid =
        ...
        if (!valid) {
          fErrors.error(
              f.fOffset,
              "pipeline stage 'main' must be declared main(int, "
  ...
  if (entry) {
    ...
    switch (entry->fKind) {
      ...
      case Symbol::kFunctionDeclaration_Kind:
        ...
        break;
      default: fErrors.error(f.fOffset, "symbol '" + f.fName + "' was already defined"); return;
    }
      ...
      if (parameters.size() == other->fParameters.size()) {
        ...
        if (match) {
          if (*returnType != other->fReturnType) {
            FunctionDeclaration newDecl(f.fOffset, f.fModifiers, f.fName, parameters, *returnType);
            fErrors.error(
  ...
  if (!decl) {
    ...
    auto newDecl = std::unique_ptr<FunctionDeclaration>(
        new FunctionDeclaration(f.fOffset, f.fModifiers, f.fName, parameters, *returnType));
    decl = newDecl.get();
  ...
  if (f.fBody) {
    ...
    bool needInvocationIDWorkaround = fInvocations != -1 && f.fName == "main" && fSettings->fCaps &&
                                      !fSettings->fCaps->gsInvocationsSupport();
...
std::unique_ptr<InterfaceBlock> IRGenerator::convertInterfaceBlock(const ASTInterfaceBlock& intf) {
  ...
  for (size_t i = 0; i < intf.fDeclarations.size(); i++) {
    std::unique_ptr<VarDeclarations> decl =
        this->convertVarDeclarations(*intf.fDeclarations[i], Variable::kInterfaceBlock_Storage);
    if (!decl) {
      return nullptr;
    }
  ...
  Type* type = (Type*)old->takeOwnership(
      std::unique_ptr<Symbol>(new Type(intf.fOffset, intf.fTypeName, fields)));
  std::vector<std::unique_ptr<Expression>> sizes;
  ...
  Variable* var = (Variable*)old->takeOwnership(std::unique_ptr<Symbol>(new Variable(
      intf.fOffset, intf.fModifiers,
      intf.fInstanceName.fLength ? intf.fInstanceName : intf.fTypeName, *type,
  ...
  return std::unique_ptr<InterfaceBlock>(new InterfaceBlock(
      intf.fOffset, var, intf.fTypeName, intf.fInstanceName, std::move(sizes), symbols));
}
...
void IRGenerator::convertEnum(const ASTEnum& e) {
  ...
  for (size_t i = 0; i < e.fNames.size(); i++) {
    ...
    auto var = std::unique_ptr<Variable>(new Variable(
        e.fOffset, modifiers, e.fNames[i], *type, Variable::kGlobal_Storage, value.get()));
    variables.push_back(var.get());
    symbols->add(e.fNames[i], std::move(var));
    symbols->takeOwnership(std::move(value));
  }
  fProgramElements->push_back(
      std::unique_ptr<ProgramElement>(new Enum(e.fOffset, e.fTypeName, symbols)));
  fSymbolTable = symbols->fParent;
}
...
const Type* IRGenerator::convertType(const ASTType& type) {
  ...
  if (result && result->fKind == Symbol::kType_Kind) {
    ...
    for (int size : type.fSizes) {
      ...
      name += "]";
      result = (Type*)fSymbolTable->takeOwnership(
          std::unique_ptr<Symbol>(new Type(name, Type::kArray_Kind, (const Type&)*result, size)));
    }
...
std::unique_ptr<Expression> IRGenerator::convertExpression(const ASTExpression& expr) {
  switch (expr.fKind) {
    case ASTExpression::kIdentifier_Kind: return this->convertIdentifier((ASTIdentifier&)expr);
    case ASTExpression::kBool_Kind:
      return std::unique_ptr<Expression>(
          new BoolLiteral(fContext, expr.fOffset, ((ASTBoolLiteral&)expr).fValue));
    case ASTExpression::kInt_Kind:
      return std::unique_ptr<Expression>(
          new IntLiteral(fContext, expr.fOffset, ((ASTIntLiteral&)expr).fValue));
    case ASTExpression::kFloat_Kind:
      return std::unique_ptr<Expression>(
          new FloatLiteral(fContext, expr.fOffset, ((ASTFloatLiteral&)expr).fValue));
    ...
    default: ABORT("unsupported expression type: %d\n", expr.fKind);
  }
}
...
std::unique_ptr<Expression> IRGenerator::coerce(
  ...
  if (expr->coercionCost(type) == INT_MAX) {
    fErrors.error(
        expr->fOffset,
        "expected '" + type.description() + "', but found '" + expr->fType.description() + "'");
    return nullptr;
  }
    ...
    case Token::STAR:
      if (is_matrix_multiply(left, right)) {
        ...
        if (determine_binary_type(
                context, Token::STAR, left.componentType(), right.componentType(), outLeftType,
...
std::unique_ptr<Expression> IRGenerator::convertBinaryExpression(
  ...
  if (!determine_binary_type(
          fContext, expression.fOperator, *rawLeftType, *rawRightType, &leftType, &rightType,
          &resultType, !Compiler::IsAssignment(expression.fOperator))) {
    fErrors.error(
  ...
  if (Compiler::IsAssignment(expression.fOperator)) {
    this->setRefKind(
        *left, expression.fOperator != Token::EQ ? VariableReference::kReadWrite_RefKind
  ...
  std::unique_ptr<Expression> result =
      this->constantFold(*left.get(), expression.fOperator, *right.get());
  if (!result) {
    result = std::unique_ptr<Expression>(new BinaryExpression(
        expression.fOffset, std::move(left), expression.fOperator, std::move(right), *resultType));
  }
  return result;
}
...
std::unique_ptr<Expression> IRGenerator::convertTernaryExpression(
  ...
  const Type* resultType;
  if (!determine_binary_type(
          fContext, Token::EQEQ, ifTrue->fType, ifFalse->fType, &trueType, &falseType, &resultType,
          true) ||
      trueType != falseType) {
    fErrors.error(
        expression.fOffset, "ternary operator result mismatch: '" + ifTrue->fType.description() +
  ...
  return std::unique_ptr<Expression>(new TernaryExpression(
      expression.fOffset, std::move(test), std::move(ifTrue), std::move(ifFalse)));
}
...
std::unique_ptr<Expression> IRGenerator::convertNumberConstructor(
  ...
  if (args.size() != 1) {
    fErrors.error(
        offset, "invalid arguments to '" + type.description() +
  ...
  if (!args[0]->fType.isNumber()) {
    fErrors.error(
        offset, "invalid argument to '" + type.description() +
...
std::unique_ptr<Expression> IRGenerator::convertCompoundConstructor(
  ...
  if (args.size() != 1 || expected != component_count(args[0]->fType) ||
    ...
    for (size_t i = 0; i < args.size(); i++) {
      if (args[i]->fType.kind() == Type::kVector_Kind) {
        if (type.componentType().isNumber() != args[i]->fType.componentType().isNumber()) {
          fErrors.error(
              offset, "'" + args[i]->fType.description() +
                          "' is not a valid "
                          "parameter to '" +
    ...
    if (actual != 1 && actual != expected) {
      fErrors.error(
          offset, "invalid arguments to '" + type.description() + "' constructor (expected " +
...
std::unique_ptr<Expression> IRGenerator::convertPrefixExpression(
  ...
  switch (expression.fOperator) {
    case Token::PLUS:
      if (!base->fType.isNumber() && base->fType.kind() != Type::kVector_Kind &&
        ...
        fErrors.error(
            expression.fOffset, "'+' cannot operate on '" + base->fType.description() + "'");
        return nullptr;
      }
      return base;
    case Token::MINUS:
    ...
    case Token::PLUSPLUS:
      if (!base->fType.isNumber()) {
        fErrors.error(
            expression.fOffset, String("'") + Compiler::OperatorName(expression.fOperator) +
  ...
  return std::unique_ptr<Expression>(new PrefixExpression(expression.fOperator, std::move(base)));
}

std::unique_ptr<Expression> IRGenerator::convertIndex(
    std::unique_ptr<Expression> base, const ASTExpression& index) {
...
std::unique_ptr<Expression> IRGenerator::convertField(
  ...
  fErrors.error(
      base->fOffset, "type '" + base->fType.description() +
                         "' does not have a "
                         "field named '" +
                         field + "");
  return nullptr;
  ...
  switch (expression.fSuffix->fKind) {
    case ASTSuffix::kIndex_Kind: {
      ...
      if (expr) {
        ...
        Type* newType = (Type*)fSymbolTable->takeOwnership(std::unique_ptr<Symbol>(
            new Type(oldType.name() + "[]", Type::kArray_Kind, oldType, -1)));
        return std::unique_ptr<Expression>(new TypeReference(fContext, base->fOffset, *newType));
      } else {
    ...
    case ASTSuffix::kCall_Kind: {
      ...
      for (size_t i = 0; i < rawArguments->size(); i++) {
        std::unique_ptr<Expression> converted = this->convertExpression(*(*rawArguments)[i]);
        if (!converted) {
          return nullptr;
        }
    ...
    case ASTSuffix::kField_Kind: {
      ...
      switch (base->fType.kind()) {
        ...
        default:
          fErrors.error(
              base->fOffset, "cannot swizzle value of type '" + base->fType.description() + "'");
          return nullptr;
      }
    }
    case ASTSuffix::kPostIncrement_Kind:
      ...
      this->setRefKind(*base, VariableReference::kReadWrite_RefKind);
      return std::unique_ptr<Expression>(new PostfixExpression(std::move(base), Token::PLUSPLUS));
    case ASTSuffix::kPostDecrement_Kind:
      ...
      this->setRefKind(*base, VariableReference::kReadWrite_RefKind);
      return std::unique_ptr<Expression>(new PostfixExpression(std::move(base), Token::MINUSMINUS));
...
void IRGenerator::setRefKind(const Expression& expr, VariableReference::RefKind kind) {
  switch (expr.fKind) {
    ...
    default: fErrors.error(expr.fOffset, "cannot assign to '" + expr.description() + "'"); break;
  }
}
    ...
    switch (decl.fKind) {
      case ASTDeclaration::kVar_Kind: {
        std::unique_ptr<VarDeclarations> s =
            this->convertVarDeclarations((ASTVarDeclarations&)decl, Variable::kGlobal_Storage);
        if (s) {
      ...
      case ASTDeclaration::kModifiers_Kind: {
        std::unique_ptr<ModifiersDeclaration> f =
            this->convertModifiersDeclaration((ASTModifiersDeclaration&)decl);
        if (f) {
      ...
      case ASTDeclaration::kInterfaceBlock_Kind: {
        std::unique_ptr<InterfaceBlock> i = this->convertInterfaceBlock((ASTInterfaceBlock&)decl);
        if (i) {
      ...
      default: ABORT("unsupported declaration: %s\n", decl.description().c_str());
    }
  }
}
