...
StatementArray IRGenerator::convertVarDeclarations(
  ...
  if (modifiers.fFlags & Modifiers::kVarying_Flag) {
    ...
    if (!baseType->isFloat() &&
        !(baseType->typeKind() == Type::TypeKind::kVector && baseType->componentType().isFloat())) {
  ...
  for (; declarationsIter != decls.end(); ++declarationsIter) {
    ...
    auto var = std::make_unique<Variable>(
        varDecl.fOffset, fModifiers->handle(modifiers), varData.fName, type, fIsBuiltinCode,
        storage);
...
std::unique_ptr<Statement> IRGenerator::convertWhile(const ASTNode& w) {
  ...
  auto whileStmt =
      std::make_unique<WhileStatement>(w.fOffset, std::move(test), std::move(statement));
...
std::unique_ptr<Statement> IRGenerator::convertSwitch(const ASTNode& s) {
  ...
  if (value->type() != *fContext.fUInt_Type && value->type().typeKind() != Type::TypeKind::kEnum) {
...
std::unique_ptr<Block> IRGenerator::applyInvocationIDWorkaround(std::unique_ptr<Block> main) {
  ...
  const FunctionDeclaration* invokeDecl = fSymbolTable->add(std::make_unique<FunctionDeclaration>(
      /*offset=*/-1, fModifiers->handle(invokeModifiers), "_invoke", std::vector<const Variable*>(),
      fContext.fVoid_Type.get(), fIsBuiltinCode));
  fProgramElements->push_back(std::make_unique<FunctionDefinition>(
  ...
  loopBody.push_back(std::make_unique<ExpressionStatement>(this->call(
      /*offset=*/-1, *invokeDecl, ExpressionArray{})));
  loopBody.push_back(std::make_unique<ExpressionStatement>(this->call(
      /*offset=*/-1, std::move(endPrimitive), ExpressionArray{})));
...
std::unique_ptr<Statement> IRGenerator::getNormalizeSkPositionCode() {
  ...
  auto Op = [&](std::unique_ptr<Expression> left, Token::Kind op,
    ...
    return std::make_unique<BinaryExpression>(
        -1, std::move(left), op, std::move(right), fContext.fFloat2_Type.get());
  };
  ...
  std::unique_ptr<Expression> result =
      Op(Pos(), Token::Kind::TK_EQ,
         std::make_unique<Constructor>(
             /*offset=*/-1, fContext.fFloat4_Type.get(), std::move(children)));
...
void IRGenerator::convertFunction(const ASTNode& f) {
  ...
  for (size_t i = 0; i < funcData.fParameterCount; ++i) {
    ...
    if (funcData.fName == "main" &&
        (fKind == Program::kPipelineStage_Kind || fKind == Program::kFragmentProcessor_Kind)) {
      if (i == 0) {
    ...
    const Variable* var = fSymbolTable->takeOwnershipOfSymbol(std::make_unique<Variable>(
        param.fOffset, fModifiers->handle(m), pd.fName, type, fIsBuiltinCode,
  ...
  if (funcData.fName == "main") {
    switch (fKind) {
      ...
      case Program::kFragmentProcessor_Kind: {
        bool valid = (parameters.size() == 0) || (parameters.size() == 1 && paramIsCoords(0));
        if (!valid) {
        ...
        if (match) {
          if (*returnType != other->returnType()) {
            FunctionDeclaration newDecl(
                f.fOffset, fModifiers->handle(funcData.fModifiers), funcData.fName, parameters,
  ...
  if (!decl) {
    ...
    decl = fSymbolTable->add(std::make_unique<FunctionDeclaration>(
        f.fOffset, fModifiers->handle(declModifiers), funcData.fName, parameters, returnType,
        fIsBuiltinCode));
  }
  if (iter == f.end()) {
...
std::unique_ptr<InterfaceBlock> IRGenerator::convertInterfaceBlock(const ASTNode& intf) {
  ...
  for (size_t i = 0; i < id.fSizeCount; ++i) {
    ...
    if (size) {
      ...
      type = symbols->takeOwnershipOfSymbol(
          std::make_unique<Type>(name, Type::TypeKind::kArray, *type, Type::kUnsizedArray));
      sizes.push_back(nullptr);
    }
  }
  const Variable* var = old->takeOwnershipOfSymbol(std::make_unique<Variable>(
      intf.fOffset, fModifiers->handle(id.fModifiers),
...
void IRGenerator::convertEnum(const ASTNode& e) {
  ...
  for (auto iter = e.begin(); iter != e.end(); ++iter) {
    ...
    fSymbolTable->add(std::make_unique<Variable>(
        e.fOffset, fModifiers->handle(modifiers), child.getString(), type, fIsBuiltinCode,
...
const Type* IRGenerator::convertType(const ASTNode& type, bool allowVoid) {
  ...
  if (result && result->is<Type>()) {
    if (td.fIsNullable) {
      if (result->as<Type>() == *fContext.fFragmentProcessor_Type) {
        ...
        result = fSymbolTable->takeOwnershipOfSymbol(std::make_unique<Type>(
            String(result->name()) + "?", Type::TypeKind::kNullable, result->as<Type>()));
      } else {
    ...
    for (const auto& size : type) {
      ...
      name += "]";
      result = fSymbolTable->takeOwnershipOfSymbol(std::make_unique<Type>(
          name, Type::TypeKind::kArray, result->as<Type>(),
...
std::unique_ptr<Expression> IRGenerator::convertIdentifier(const ASTNode& identifier) {
  ...
  switch (result->kind()) {
    ...
    case Symbol::Kind::kUnresolvedFunction: {
      ...
      return std::make_unique<FunctionReference>(fContext, identifier.fOffset, f->functions());
    }
    case Symbol::Kind::kVariable: {
      ...
      return std::make_unique<VariableReference>(
          identifier.fOffset, var, VariableReference::RefKind::kRead);
    }
    case Symbol::Kind::kField: {
      const Field* field = &result->as<Field>();
      auto base = std::make_unique<VariableReference>(
          identifier.fOffset, &field->owner(), VariableReference::RefKind::kRead);
      return std::make_unique<FieldAccess>(
  ...
  if (type.typeKind() == Type::TypeKind::kScalar) {
    ...
    if (type == *fContext.fFloatLiteral_Type) {
      ...
      ctor = this->convertIdentifier(
          ASTNode(&fFile->fNodes, -1, ASTNode::Kind::kIdentifier, type.name()));
    }
    if (!ctor) {
...
static bool is_matrix_multiply(const Type& left, const Type& right) {
  ...
  return left.typeKind() == Type::TypeKind::kVector && right.typeKind() == Type::TypeKind::kMatrix;
}
...
static bool determine_binary_type(
  ...
  switch (op) {
    ...
    case Token::Kind::TK_GTEQ: isLogical = true; break;
    ...
    case Token::Kind::TK_STAR:
      if (is_matrix_multiply(left, right)) {
        ...
        if (determine_binary_type(
                context, allowNarrowing, op, left.componentType(), right.componentType(),
    ...
    case Token::Kind::TK_PERCENT: validMatrixOrVectorOp = true; break;
  ...
  bool leftIsVectorOrMatrix =
           left.typeKind() == Type::TypeKind::kVector || left.typeKind() == Type::TypeKind::kMatrix,
...
template <typename T>
std::unique_ptr<Expression> IRGenerator::constantFoldVector(
...
#define URESULT(t, op) \
  ...
  if (leftType.typeKind() == Type::TypeKind::kMatrix &&
      rightType.typeKind() == Type::TypeKind::kMatrix && left.kind() == right.kind()) {
    switch (op) {
...
std::unique_ptr<Expression> IRGenerator::convertTernaryExpression(const ASTNode& node) {
  ...
  if (!determine_binary_type(
      ...
      trueType != falseType) {
    fErrors.error(
        node.fOffset, "ternary operator result mismatch: '" + ifTrue->type().displayName() +
  ...
  if (!ifTrue) {
    ...
    return std::make_unique<TernaryExpression>(
        node.fOffset, std::move(test), std::move(ifTrue), std::move(ifFalse));
}
...
std::unique_ptr<Expression> IRGenerator::call(
  ...
  auto funcCall =
      std::make_unique<FunctionCall>(offset, returnType, &function, std::move(arguments));
  if (fCanInline && fInliner->isSafeToInline(funcCall->function().definition()) &&
...
std::unique_ptr<Expression> IRGenerator::convertNumberConstructor(
  ...
  if (args[0]->is<IntLiteral>() && (type == *fContext.fInt_Type || type == *fContext.fUInt_Type)) {
...
static int component_count(const Type& type) {
  switch (type.typeKind()) {
    case Type::TypeKind::kVector: return type.columns();
    case Type::TypeKind::kMatrix: return type.columns() * type.rows();
    default: return 1;
  }
}
...
std::unique_ptr<Expression> IRGenerator::convertCompoundConstructor(
    int offset, const Type& type, ExpressionArray args) {
  SkASSERT(
      type.typeKind() == Type::TypeKind::kVector || type.typeKind() == Type::TypeKind::kMatrix);
  ...
  switch (expression.getToken().fKind) {
    case Token::Kind::TK_PLUS:
      if (!baseType.isNumber() && baseType.typeKind() != Type::TypeKind::kVector &&
        ...
        fErrors.error(expression.fOffset, "'+' cannot operate on '" + baseType.displayName() + "'");
        return nullptr;
      }
      return base;
    ...
    case Token::Kind::TK_MINUS:
      if (base->is<IntLiteral>()) {
        return std::make_unique<IntLiteral>(
            fContext, base->fOffset, -base->as<IntLiteral>().value());
      }
      if (base->is<FloatLiteral>()) {
        return std::make_unique<FloatLiteral>(
            fContext, base->fOffset, -base->as<FloatLiteral>().value());
      }
      if (!baseType.isNumber() && baseType.typeKind() != Type::TypeKind::kVector) {
        fErrors.error(expression.fOffset, "'-' cannot operate on '" + baseType.displayName() + "'");
        return nullptr;
    ...
    case Token::Kind::TK_LOGICALNOT:
      ...
      if (base->kind() == Expression::Kind::kBoolLiteral) {
        return std::make_unique<BoolLiteral>(
            fContext, base->fOffset, !base->as<BoolLiteral>().value());
      }
      break;
  ...
  if (base->kind() == Expression::Kind::kTypeReference) {
    if (index.fKind == ASTNode::Kind::kInt) {
      ...
      const Type* newType = fSymbolTable->takeOwnershipOfSymbol(std::make_unique<Type>(
          oldType.name() + "[" + to_string(size) + "]", Type::TypeKind::kArray, oldType, size));
...
std::unique_ptr<Expression> IRGenerator::convertField(
  ...
  fErrors.error(
      base->fOffset, "type '" + baseType.displayName() +
...
std::unique_ptr<Expression> IRGenerator::convertTypeField(
  ...
  if (enumElement) {
    ...
    if (result) {
      ...
      fErrors.error(
          offset, "type '" + type.name() + "' does not have a member named '" + field + "'");
    }
    fSymbolTable = old;
    return result;
  } else {
    ...
    fErrors.error(
        offset, "type '" + type.displayName() + "' does not have a member named '" + field + "'");
    return nullptr;
  }
}
...
void IRGenerator::cloneBuiltinVariables() {
   ...
   public:
    ...
    void cloneVariable(const String& name) {
      ...
      if (const ProgramElement* sharedDecl = fGenerator->fIntrinsics->findAndInclude(name)) {
        SkASSERT(sharedDecl->is<GlobalVarDeclaration>() || sharedDecl->is<InterfaceBlock>());
        ...
        const Variable* clonedVar =
            fGenerator->fSymbolTable->takeOwnershipOfSymbol(std::make_unique<Variable>(
                ...
                &sharedVar->type(), /*builtin=*/false, sharedVar->storage(), initialValue));
  ...
  switch (fKind) {
    case Program::kFragment_Kind: remapper.cloneVariable("sk_Clockwise"); break;
    default: break;
  }

  fProgramElements->insert(
      fProgramElements->begin(), std::make_move_iterator(remapper.fNewElements.begin()),
      std::make_move_iterator(remapper.fNewElements.end()));
}

IRGenerator::IRBundle IRGenerator::convertProgram(
    Program::Kind kind, const Program::Settings* settings, const ShaderCapsClass* caps,
    const ParsedModule& base, bool isBuiltinCode, const char* text, size_t length,
  ...
  if (kind == Program::kGeometry_Kind && !fIsBuiltinCode) {
    ...
    auto var = std::make_unique<Variable>(
        -1, fModifiers->handle(m), "sk_InvocationID", fContext.fInt_Type.get(), false,
        Variable::Storage::kGlobal);
    ...
    switch (decl.fKind) {
      case ASTNode::Kind::kVarDeclarations: {
        ...
        for (auto& varDecl : decls) {
          fProgramElements->push_back(
              std::make_unique<GlobalVarDeclaration>(decl.fOffset, std::move(varDecl)));
        }
        break;
      }
