...
class MemoryLayout {
 public:
  ...
  size_t alignment(const Type& type) const {
    ...
    switch (type.kind()) {
      case Type::kScalar_Kind: return this->size(type);
      ...
      case Type::kArray_Kind: return this->roundUpIfNeeded(this->alignment(type.componentType()));
  ...
  size_t size(const Type& type) const {
    switch (type.kind()) {
      ...
      case Type::kArray_Kind: return type.columns() * this->stride(type);
      case Type::kStruct_Kind: {
        size_t total = 0;
        for (const auto& f : type.fields()) {
          size_t alignment = this->alignment(*f.fType);
