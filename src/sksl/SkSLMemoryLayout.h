...
class MemoryLayout {
 public:
    ...
    switch (type.typeKind()) {
      ...
      case Type::TypeKind::kMatrix:
        return this->roundUpIfNeeded(
            GetVectorAlignment(this->size(type.componentType()), type.rows()), type.typeKind());
      case Type::TypeKind::kArray:
        return this->roundUpIfNeeded(this->alignment(type.componentType()), type.typeKind());
      ...
      default: SK_ABORT("cannot determine alignment of type %s", type.displayName().c_str());
    }
  }
  ...
  size_t stride(const Type& type) const {
    switch (type.typeKind()) {
      case Type::TypeKind::kMatrix: return this->alignment(type);
      case Type::TypeKind::kArray: {
  ...
  size_t size(const Type& type) const {
    switch (type.typeKind()) {
      case Type::TypeKind::kScalar:
        ...
        if ((this->isMetal() || this->isWGSL()) && !type.highPrecision() && type.isNumber()) {
          return 2;
        }
        return 4;
      case Type::TypeKind::kVector:
      ...
      case Type::TypeKind::kArray: return type.columns() * this->stride(type);
      case Type::TypeKind::kStruct: {
        size_t total = 0;
        for (const auto& f : type.fields()) {
          size_t alignment = this->alignment(*f.fType);
      ...
      default: SK_ABORT("cannot determine size of type %s", type.displayName().c_str());
    }
  }
  ...
  size_t isSupported(const Type& type) const {
    switch (type.typeKind()) {
      case Type::TypeKind::kScalar:
        ...
        return !this->isWGSL() || (!type.isBoolean() && (type.isFloat() || type.highPrecision()));

      case Type::TypeKind::kVector:
      case Type::TypeKind::kMatrix:
      case Type::TypeKind::kArray: return this->isSupported(type.componentType());

      case Type::TypeKind::kStruct:
        return std::all_of(
            type.fields().begin(), type.fields().end(),
            [this](const Type::Field& f) { return this->isSupported(*f.fType); });
