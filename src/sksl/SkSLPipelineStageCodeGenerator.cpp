...
void PipelineStageCodeGenerator::writeFunctionCall(const FunctionCall& c) {
  if (c.fFunction.fBuiltin && c.fFunction.fName == "sample" &&
    ...
    SkASSERT("fragmentProcessor" == arg0Type.name() || "fragmentProcessor?" == arg0Type.name());
    ...
    bool matrixCall =
        c.fArguments.size() == 2 && c.fArguments[1]->type().typeKind() == Type::TypeKind::kMatrix;
  ...
  switch (ref.fVariable->fModifiers.fLayout.fBuiltin) {
    ...
    default: {
      ...
      if (ref.fVariable->fModifiers.fFlags & Modifiers::kUniform_Flag) {
        this->write(Compiler::kFormatArgPlaceholderStr);
        fArgs->fFormatArgs.push_back(Compiler::FormatArg(
            Compiler::FormatArg::Kind::kUniform, varIndexByFlag(Modifiers::kUniform_Flag)));
