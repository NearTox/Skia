...
String operator+(skstd::string_view left, skstd::string_view right) { return String(left) + right; }

String to_string(int32_t value) { return SkSL::String(std::to_string(value)); }

String to_string(uint32_t value) { return SkSL::String(std::to_string(value)); }

String to_string(int64_t value) { return SkSL::String(std::to_string(value)); }

String to_string(uint64_t value) { return SkSL::String(std::to_string(value)); }
...
bool stoi(const skstd::string_view& s, SKSL_INT* value) {
  char* p;
  errno = 0;
  unsigned long long result = strtoull(s.data(), &p, /*base=*/0);
  *value = static_cast<SKSL_INT>(result);
  return p == (s.data() + s.size()) && errno == 0 && result <= 0xFFFFFFFF;
}
