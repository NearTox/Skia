...
void MetalCodeGenerator::write(const String& s) { this->write(s.c_str()); }

void MetalCodeGenerator::writeLine(const String& s) { this->writeLine(s.c_str()); }

void MetalCodeGenerator::writeLine() { this->writeLine(""); }
...
String MetalCodeGenerator::typeName(const Type& type) {
  switch (type.typeKind()) {
    ...
    case Type::TypeKind::kEnum: return "int";
    default:
...
void MetalCodeGenerator::writeBaseType(const Type& type) {
  switch (type.typeKind()) {
    case Type::TypeKind::kStruct:
      ...
      break;
    case Type::TypeKind::kArray: this->writeBaseType(type.componentType()); break;
    default: this->write(this->typeName(type)); break;
  }
}
...
void MetalCodeGenerator::writeIntrinsicCall(const FunctionCall& c, IntrinsicKind kind) {
  ...
  switch (kind) {
    ...
    case kCompareNotEqual_IntrinsicKind: {
      ...
      switch (kind) {
        case kCompareEqual_IntrinsicKind: this->write(" == "); break;
        case kCompareNotEqual_IntrinsicKind: this->write(" != "); break;
        case kCompareLessThan_IntrinsicKind: this->write(" < "); break;
        case kCompareLessThanEqual_IntrinsicKind: this->write(" <= "); break;
        case kCompareGreaterThan_IntrinsicKind: this->write(" > "); break;
        case kCompareGreaterThanEqual_IntrinsicKind: this->write(" >= "); break;
        default: ABORT("unsupported comparison intrinsic kind");
      }
...
void MetalCodeGenerator::assembleMatrixFromExpressions(
    const ExpressionArray& args, int rows, int columns) {
  size_t argIndex = 0;
  int argPosition = 0;
...
void MetalCodeGenerator::writeConstructor(const Constructor& c, Precedence parentPrecedence) {
  ...
  for (const std::unique_ptr<Expression>& arg : c.arguments()) {
    ...
    this->write(separator);
    separator = ", ";
    if (constructorType.isMatrix() && argType.columns() < constructorType.rows()) {
...
bool MetalCodeGenerator::writeFunctionDeclaration(const FunctionDeclaration& f) {
  ...
  if ("main" == f.name()) {
    switch (fProgram.fKind) {
      ...
      default: fErrors.error(-1, "unsupported kind of program"); return false;
    }
      ...
      if (e->is<GlobalVarDeclaration>()) {
        ...
        if (var.var().type().typeKind() == Type::TypeKind::kSampler) {
          if (var.var().modifiers().fLayout.fBinding < 0) {
            fErrors.error(decls.fOffset, "Metal samplers must have 'layout(binding=...)'");
            return false;
          }
        ...
        if (intf.variable().modifiers().fLayout.fBinding < 0) {
          fErrors.error(intf.fOffset, "Metal interface blocks must have 'layout(binding=...)'");
          return false;
        }
        this->write(", constant ");
...
void MetalCodeGenerator::writeModifiers(const Modifiers& modifiers, bool globalContext) {
...
void MetalCodeGenerator::writeReturnStatementFromMain() {
  ...
  switch (fProgram.fKind) {
    case Program::kFragment_Kind: this->write("return *_out;"); break;
      ...
      if (var.modifiers().fFlags & Modifiers::kUniform_Flag &&
        ...
        if (-1 == fUniformBuffer) {
          ...
          if (-1 == fUniformBuffer) {
            fErrors.error(
                decls.fOffset,
                "Metal backend requires all uniforms to have "
                "the same 'layout(set=...)'");
          }
        }
        this->write("    ");
  ...
  for (const ProgramElement* e : fProgram.elements()) {
    if (e->is<GlobalVarDeclaration>()) {
      ...
      if (var.modifiers().fFlags & Modifiers::kOut_Flag && -1 == var.modifiers().fLayout.fBuiltin) {
        ...
        if (location < 0) {
          fErrors.error(var.fOffset, "Metal out variables must have 'layout(location=...)'");
        } else if (fProgram.fKind == Program::kVertex_Kind) {
...
void MetalCodeGenerator::writeStructDefinitions() {
  for (const ProgramElement* e : fProgram.elements()) {
    if (e->is<StructDefinition>()) {
      ...
      const Type* type =
          &e->as<GlobalVarDeclaration>().declaration()->as<VarDeclaration>().baseType();
      if (type->isStruct()) {
        if (this->writeStructDefinition(*type)) {
          this->writeLine(";");
        }
...
void MetalCodeGenerator::writeGlobalInit() {
  class : public GlobalStructVisitor {
   public:
    void visitInterfaceBlock(const InterfaceBlock& blockType, const String& blockName) override {
      this->addElement();
      fCodeGen->write("&");
      fCodeGen->writeName(blockName);
    }
...
void MetalCodeGenerator::writeProgramElement(const ProgramElement& e) {
  switch (e.kind()) {
    ...
    case ProgramElement::Kind::kModifiers:
      this->writeModifiers(
          e.as<ModifiersDeclaration>().modifiers(),
          /*globalContext=*/true);
      this->writeLine(";");
      break;
    case ProgramElement::Kind::kEnum: break;
    default:
#ifdef SK_DEBUG
