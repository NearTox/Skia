  ...
  switch (intrinsic.first) {
    ...
    case kMetal_IntrinsicKind:
      ...
      switch ((MetalIntrinsic)intrinsicId) {
        case kEqual_MetalIntrinsic: this->write(" == "); break;
        case kNotEqual_MetalIntrinsic: this->write(" != "); break;
        case kLessThan_MetalIntrinsic: this->write(" < "); break;
        case kLessThanEqual_MetalIntrinsic: this->write(" <= "); break;
        case kGreaterThan_MetalIntrinsic: this->write(" > "); break;
        case kGreaterThanEqual_MetalIntrinsic: this->write(" >= "); break;
        default: ABORT("unsupported metal intrinsic kind");
      }
...
void MetalCodeGenerator::writeSpecialIntrinsic(const FunctionCall& c, SpecialIntrinsic kind) {
  ...
  switch (kind) {
    ...
    case kMod_SpecialIntrinsic: {
      ...
      this->fFunctionHeader +=
          "    " + this->typeName(arguments[0]->type()) + " " + tmpX + ", " + tmpY + ";\n";
      this->write("(" + tmpX + " = ");
      this->writeExpression(*arguments[0], kSequence_Precedence);
      this->write(", " + tmpY + " = ");
      this->writeExpression(*arguments[1], kSequence_Precedence);
    ...
    default: ABORT("unsupported special intrinsic kind");
  }
}
...
String MetalCodeGenerator::getMatrixConstructHelper(const Constructor& c) {
  ...
  for (const std::unique_ptr<Expression>& expr : args) {
    fExtraFunctions.printf(
        "%s%s x%zu", argSeparator, expr->type().displayName().c_str(), argIndex++);
    argSeparator = ", ";
  }
...
void MetalCodeGenerator::writeMatrixTimesEqualHelper(
  ...
  if (fHelpers.find(key) == fHelpers.end()) {
    fExtraFunctions.printf(
        "%s operator*=(thread %s& left, thread const %s& right) {\n"
        "    left = left * right;\n"
        "    return left;\n"
        "}",
        String(result.name()).c_str(), String(left.name()).c_str(), String(right.name()).c_str());
  }
}
...
void MetalCodeGenerator::writeFunction(const FunctionDefinition& f) {
  ...
  if (f.declaration().name() == "main") {
    switch (fProgram.fKind) {
      case Program::kFragment_Kind: this->writeLine("return *_out;"); break;
    ...
    if (e->is<GlobalVarDeclaration>()) {
      ...
      if (var.modifiers().fFlags & Modifiers::kOut_Flag && -1 == var.modifiers().fLayout.fBuiltin) {
        ...
        if (fProgram.fKind == Program::kVertex_Kind) {
          ...
          this->write(" [[color(" + to_string(var.modifiers().fLayout.fLocation) + ")");
...
MetalCodeGenerator::Requirements MetalCodeGenerator::requirements(const Statement* s) {
  ...
  switch (s->kind()) {
    ...
    case Statement::Kind::kWhile: {
      ...
      return this->requirements(w.test().get()) | this->requirements(w.statement().get());
    }
