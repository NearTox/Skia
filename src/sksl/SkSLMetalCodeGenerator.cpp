  ...
  switch (type.kind()) {
    ...
    case Type::kMatrix_Kind:
      return this->typeName(type.componentType()) + to_string(type.columns()) + "x" +
             to_string(type.rows());
    case Type::kSampler_Kind: return "texture2d<float>";  // FIXME - support other texture types;
    default:
...
void MetalCodeGenerator::writeSpecialIntrinsic(const FunctionCall& c, SpecialIntrinsic kind) {
  switch (kind) {
    case kTexture_SpecialIntrinsic:
      ...
      if (c.fArguments[1]->fType == *fContext.fFloat3_Type) {
        ...
        this->fFunctionHeader +=
            "    " + this->typeName(c.fArguments[1]->fType) + " " + tmpVar + ";\n";
        this->write("(" + tmpVar + " = ");
        this->writeExpression(*c.fArguments[1], kSequence_Precedence);
    ...
    case kMod_SpecialIntrinsic: {
      ...
      this->fFunctionHeader +=
          "    " + this->typeName(c.fArguments[0]->fType) + " " + tmpX + ", " + tmpY + ";\n";
      this->write("(" + tmpX + " = ");
      this->writeExpression(*c.fArguments[0], kSequence_Precedence);
      this->write(", " + tmpY + " = ");
      this->writeExpression(*c.fArguments[1], kSequence_Precedence);
...
String MetalCodeGenerator::getMatrixConstructHelper(const Constructor& c) {
  ...
  for (const std::unique_ptr<Expression>& expr : args) {
    fExtraFunctions.printf(
        "%s%s x%zu", argSeparator, expr->fType.displayName().c_str(), argIndex++);
    argSeparator = ", ";
  }
...
void MetalCodeGenerator::writeFieldAccess(const FieldAccess& f) {
  ...
  switch (field->fModifiers.fLayout.fBuiltin) {
    case SK_CLIPDISTANCE_BUILTIN: this->write("gl_ClipDistance"); break;
...
void MetalCodeGenerator::writeStatement(const Statement& s) {
  switch (s.fKind) {
    ...
    case Statement::kDiscard_Kind: this->write("discard_fragment();"); break;
...
void MetalCodeGenerator::writeStatements(
...
void MetalCodeGenerator::writeProgramElement(const ProgramElement& e) {
  switch (e.fKind) {
    case ProgramElement::kExtension_Kind: break;
    ...
    case ProgramElement::kInterfaceBlock_Kind:
      ...
      break;
    case ProgramElement::kFunction_Kind: this->writeFunction((FunctionDefinition&)e); break;
...
MetalCodeGenerator::Requirements MetalCodeGenerator::requirements(const Expression* e) {
  ...
  switch (e->fKind) {
    ...
    case Expression::kSwizzle_Kind: return this->requirements(((const Swizzle&)*e).fBase.get());
    case Expression::kBinary_Kind: {
...
MetalCodeGenerator::Requirements MetalCodeGenerator::requirements(const Statement* s) {
  ...
  switch (s->fKind) {
    ...
    case Statement::kDo_Kind: {
      ...
      return this->requirements(d.fTest.get()) | this->requirements(d.fStatement.get());
    }
