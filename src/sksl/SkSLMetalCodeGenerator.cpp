  ...
  switch (kind) {
    ...
    case kMod_SpecialIntrinsic: {
      ...
      this->fFunctionHeader +=
          "    " + this->typeName(c.fArguments[0]->type()) + " " + tmpX + ", " + tmpY + ";\n";
      this->write("(" + tmpX + " = ");
      this->writeExpression(*c.fArguments[0], kSequence_Precedence);
      this->write(", " + tmpY + " = ");
      this->writeExpression(*c.fArguments[1], kSequence_Precedence);
      ...
      if (Variable::kGlobal_Storage == ref.fVariable->fStorage) {
        if (ref.fVariable->fModifiers.fFlags & Modifiers::kIn_Flag) {
          ...
          this->write("_out->");
            ...
            ref.fVariable->fModifiers.fFlags & Modifiers::kUniform_Flag &&
...
void MetalCodeGenerator::writeFunction(const FunctionDefinition& f) {
  ...
  if ("main" == f.fDeclaration.fName) {
    switch (fProgram.fKind) {
      ...
      default: fErrors.error(-1, "unsupported kind of program"); return;
    }
        ...
        for (const auto& stmt : decls.fVars) {
          ...
          if (-1 != var.fVar->fModifiers.fLayout.fLocation) {
            if (fProgram.fKind == Program::kVertex_Kind) {
              this->write(
                  "  [[attribute(" + to_string(var.fVar->fModifiers.fLayout.fLocation) + ")]]");
            } else if (fProgram.fKind == Program::kFragment_Kind) {
              this->write(
                  "  [[user(locn" + to_string(var.fVar->fModifiers.fLayout.fLocation) + ")]]");
            }
          }
        }
        this->write(";\n");
      ...
      if (first.fModifiers.fFlags & Modifiers::kOut_Flag &&
        ...
        for (const auto& stmt : decls.fVars) {
          ...
          if (fProgram.fKind == Program::kVertex_Kind) {
            ...
            this->write(" [[color(" + to_string(var.fVar->fModifiers.fLayout.fLocation) + ")");
...
MetalCodeGenerator::Requirements MetalCodeGenerator::requirements(const Expression* e) {
  ...
  switch (e->kind()) {
    ...
    case Expression::Kind::kSwizzle: return this->requirements(e->as<Swizzle>().fBase.get());
    case Expression::Kind::kBinary: {
      const BinaryExpression& bin = e->as<BinaryExpression>();
      return this->requirements(&bin.left()) | this->requirements(&bin.right());
    }
      ...
      if (v.fVariable->fModifiers.fLayout.fBuiltin == SK_FRAGCOORD_BUILTIN) {
        ...
        if (v.fVariable->fModifiers.fFlags & Modifiers::kIn_Flag) {
          ...
          result = kOutputs_Requirement;
            ...
            v.fVariable->fModifiers.fFlags & Modifiers::kUniform_Flag &&
...
MetalCodeGenerator::Requirements MetalCodeGenerator::requirements(const Statement* s) {
  ...
  switch (s->kind()) {
    ...
    case Statement::Kind::kIf: {
      ...
      return this->requirements(i.fTest.get()) | this->requirements(i.fIfTrue.get()) |
             this->requirements(i.fIfFalse.get());
    }
    case Statement::Kind::kFor: {
      const ForStatement& f = s->as<ForStatement>();
      return this->requirements(f.fInitializer.get()) | this->requirements(f.fTest.get()) |
             this->requirements(f.fNext.get()) | this->requirements(f.fStatement.get());
    }
    case Statement::Kind::kWhile: {
      const WhileStatement& w = s->as<WhileStatement>();
      return this->requirements(w.fTest.get()) | this->requirements(w.fStatement.get());
    }
