...
class IRGenerator {
 public:
   ...
   */
  IRBundle convertProgram(const ParsedModule& base, bool isBuiltinCode, skstd::string_view text);
  ...
  std::shared_ptr<SymbolTable>& symbolTable() { return fSymbolTable; }

  void setSymbolTable(std::shared_ptr<SymbolTable>& symbolTable) { fSymbolTable = symbolTable; }
 ...
 private:
  ...
  std::unique_ptr<Statement> convertVarDeclaration(
      std::unique_ptr<Variable> var, std::unique_ptr<Expression> value,
  ...
  std::unique_ptr<Expression> call(
      int offset, std::unique_ptr<Expression> function, ExpressionArray arguments);
  std::unique_ptr<Expression> call(
      int offset, const FunctionDeclaration& function, ExpressionArray arguments);
  CoercionCost callCost(
  ...
  std::unique_ptr<Expression> convertField(
      std::unique_ptr<Expression> base, skstd::string_view field);
  ...
  std::unique_ptr<Expression> convertSwizzle(
  ...
  bool strictES2Mode() const { return fContext.fConfig->strictES2Mode(); }

  bool isRuntimeEffect() const { return ProgramConfig::IsRuntimeEffect(fContext.fConfig->fKind); }
