...
SampleUsage SampleUsage::merge(const SampleUsage& other) {
  if (other.fExplicitCoords) {
    fExplicitCoords = true;
  }
  if (other.fPassThrough) {
    fPassThrough = true;
  }
  if (other.fHasPerspective) {
    fHasPerspective = true;
  }
...
std::string SampleUsage::constructor(std::string perspectiveExpression) const {
  ...
  if (!fExplicitCoords && !fPassThrough) {
    if (fKind == Kind::kVariable) {
      ...
      return "SkSL::SampleUsage::UniformMatrix(\"" + fExpression + "\", " + perspectiveExpression +
             ")";
    }
  }
