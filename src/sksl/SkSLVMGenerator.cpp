...
class SkVMGenerator {
 public:
  SkVMGenerator(
      ...
      skvm::Coord device, skvm::Coord local, SampleChildFn sampleChild);
...
SkVMGenerator::SkVMGenerator(
    ...
    skvm::Coord local, SampleChildFn sampleChild)
    : fProgram(program),
      fBuilder(builder),
      fLocalCoord(local),
      ...
      if (int builtin = var.modifiers().fLayout.fBuiltin; builtin >= 0) {
        ...
        switch (builtin) {
          case SK_FRAGCOORD_BUILTIN:
            ...
            fSlots.insert(
                fSlots.end(),
                {device.x.id, device.y.id, fBuilder->splat(0.0f).id, fBuilder->splat(1.0f).id});
            break;
...
SkVMGenerator::Slot SkVMGenerator::getSlot(const Variable& v) {
  ...
  size_t slot = fSlots.size(), nslots = slot_count(v.type());
...
SkVMGenerator::Slot SkVMGenerator::getSlot(const Expression& e) {
  switch (e.kind()) {
    ...
    default: SkDEBUGFAIL("Invalid expression type"); return ~static_cast<Slot>(0);
  }
}
...
Value SkVMGenerator::writeConstructor(const Constructor& c) {
  ...
  Type::NumberKind srcKind = base_number_kind(srcType), dstKind = base_number_kind(dstType);
  ...
  if (srcKind != dstKind) {
    ...
    SkDEBUGFAILF(
        "Unsupported type conversion: %s -> %s", srcType.displayName().c_str(),
...
Value SkVMGenerator::writeMatrixInverse4x4(const Value& m) {
  ...
  skvm::F32 b00 = a00 * a11 - a01 * a10, b01 = a00 * a12 - a02 * a10, b02 = a00 * a13 - a03 * a10,
...
Value SkVMGenerator::writeIntrinsicCall(const FunctionCall& c) {
  ...
  switch (found->second) {
    ...
    case Intrinsic::kMin: return binary([](skvm::F32 x, skvm::F32 y) { return skvm::min(x, y); });
    case Intrinsic::kMax: return binary([](skvm::F32 x, skvm::F32 y) { return skvm::max(x, y); });
    case Intrinsic::kClamp:
      return ternary(
          [](skvm::F32 x, skvm::F32 lo, skvm::F32 hi) { return skvm::clamp(x, lo, hi); });
    case Intrinsic::kSaturate: return unary(args[0], [](skvm::F32 x) { return skvm::clamp01(x); });
    case Intrinsic::kMix:
    ...
    case Intrinsic::kMatrixCompMult: return binary([](skvm::F32 x, skvm::F32 y) { return x * y; });
...
Value SkVMGenerator::writeExpression(const Expression& e) {
  switch (e.kind()) {
    ...
    case Expression::Kind::kConstructor: return this->writeConstructor(e.as<Constructor>());
    case Expression::Kind::kFieldAccess:
    case Expression::Kind::kIndex:
    case Expression::Kind::kVariableReference: return this->writeVariableExpression(e);
...
Value SkVMGenerator::writeStore(const Expression& lhs, const Value& rhs) {
  SkASSERT(
      lhs.is<FieldAccess>() || lhs.is<IndexExpression>() || lhs.is<Swizzle>() ||
  ...
  for (size_t i = rhs.slots(); i-- > 0;) {
    ...
    skvm::F32 curr = f32(fSlots[slot + component]), next = f32(rhs[i]);
...
void SkVMGenerator::writeForStatement(const ForStatement& f) {
  ...
  for (int i = 0; i < loop.fCount; ++i) {
    fSlots[index] = loop.fIndex->type().isInteger() ? fBuilder->splat(static_cast<int>(val)).id
...
skvm::Color ProgramToSkVM(
    ...
    SkSpan<skvm::Val> uniforms, skvm::Coord device, skvm::Coord local, SampleChildFn sampleChild) {
...
bool ProgramToSkVM(
  ...
  SkVMGenerator generator(
      program, b, uniforms, /*device=*/zeroCoord, /*local=*/zeroCoord,
      /*sampleChild=*/{});
...
bool testingOnly_ProgramToSkVMShader(const Program& program, skvm::Builder* builder) {
  ...
  auto sampleChild = [&](int i, skvm::Coord coord) {
    ...
    skvm::I32 index = trunc(coord.x) + trunc(coord.y) * children[i].rowBytesAsPixels;
