...
static bool is_vec_or_mat(const Type& type) {
  switch (type.typeKind()) {
    ...
    case Type::TypeKind::kVector: return true;

    default: return false;
  }
}

static std::unique_ptr<Expression> eliminate_no_op_boolean(
    Position pos, const Expression& left, Operator op, const Expression& right) {
...
static std::unique_ptr<Expression> simplify_constant_equality(
    const Context& context, Position pos, const Expression& left, Operator op,
  ...
  if (op.kind() == Operator::Kind::EQEQ || op.kind() == Operator::Kind::NEQ) {
    ...
    switch (left.compareConstant(right)) {
      case Expression::ComparisonResult::kNotEqual: equality = !equality; [[fallthrough]];
...
static std::unique_ptr<Expression> simplify_matrix_multiplication(
    const Context& context, Position pos, const Expression& left, const Expression& right,
    int leftColumns, int leftRows, int rightColumns, int rightRows) {
  ...
  int outColumns = rightColumns, outRows = leftRows;
...
static std::unique_ptr<Expression> simplify_matrix_times_matrix(
  ...
  return simplify_matrix_multiplication(
      context, pos, left, right, leftType.columns(), leftType.rows(), rightType.columns(),
      rightType.rows());
}
...
static std::unique_ptr<Expression> simplify_vector_times_matrix(
  ...
  return simplify_matrix_multiplication(
      context, pos, left, right,
      /*leftColumns=*/leftType.columns(), /*leftRows=*/1, rightType.columns(), rightType.rows());
}
...
static std::unique_ptr<Expression> simplify_matrix_times_vector(
  ...
  return simplify_matrix_multiplication(
      context, pos, left, right, leftType.columns(), leftType.rows(),
...
static std::unique_ptr<Expression> simplify_componentwise(
  ...
  if (std::unique_ptr<Expression> result =
          simplify_constant_equality(context, pos, left, op, right)) {
    return result;
  }
...
static std::unique_ptr<Expression> splat_scalar(
    const Context& context, const Expression& scalar, const Type& type) {
  if (type.isVector()) {
...
static std::unique_ptr<Expression> cast_expression(
    const Context& context, Position pos, const Expression& expr, const Type& type) {
...
static std::unique_ptr<Expression> negate_expression(
    const Context& context, Position pos, const Expression& expr, const Type& type) {
...
static bool error_on_divide_by_zero(
    const Context& context, Position pos, Operator op, const Expression& right) {
  switch (op.kind()) {
...
static std::unique_ptr<Expression> simplify_no_op_arithmetic(
    const Context& context, Position pos, const Expression& left, Operator op,
  ...
  switch (op.kind()) {
    ...
    case Operator::Kind::MINUSEQ:
      if (is_constant_value(right, 0.0)) {  // x += 0, x -= 0
        if (std::unique_ptr<Expression> var = cast_expression(context, pos, left, resultType)) {
...
template <typename T>
static std::unique_ptr<Expression> fold_float_expression(
    Position pos, T result, const Type* resultType) {
...
template <typename T>
static std::unique_ptr<Expression> fold_int_expression(
    Position pos, T result, const Type* resultType) {
...
std::unique_ptr<Expression> ConstantFolder::Simplify(
    const Context& context, Position pos, const Expression& leftExpr, Operator op,
    const Expression& rightExpr, const Type& resultType) {
  ...
  const Type& leftType = left->type();
  const Type& rightType = right->type();
  if (context.fConfig->fSettings.fOptimize && (leftType.isScalar() || leftType.isVector()) &&
      (rightType.isScalar() || rightType.isVector())) {
    if (std::unique_ptr<Expression> expr =
            simplify_no_op_arithmetic(context, pos, *left, op, *right, resultType)) {
      return expr;
    }
  }
...
#define RESULT(Op) fold_int_expression(pos, (SKSL_INT)(leftVal)Op(SKSL_INT)(rightVal), &resultType)
#define URESULT(Op) \
  fold_int_expression(pos, (SKSL_INT)((SKSL_UINT)(leftVal)Op(SKSL_UINT)(rightVal)), &resultType)
    switch (op.kind()) {
...
#undef RESULT
  ...
  if (rightType.isScalar() && is_vec_or_mat(leftType) &&
    ...
    return simplify_componentwise(
        context, pos, *left, op, *splat_scalar(context, *right, left->type()));
  }
  ...
  if (leftType.isScalar() && is_vec_or_mat(rightType) &&
    ...
    return simplify_componentwise(
        context, pos, *splat_scalar(context, *left, right->type()), op, *right);
  }
