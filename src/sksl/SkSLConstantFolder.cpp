...
static std::unique_ptr<Expression> simplify_vector_equality(
    const Context& context, const Expression& left, Operator op, const Expression& right) {
...
static std::unique_ptr<Expression> simplify_vector(
    const Context& context, const Expression& left, Operator op, const Expression& right) {
  ...
  const auto vectorComponentwiseFold = [&](auto foldFn) -> std::unique_ptr<Expression> {
    ...
    for (int i = 0; i < type.columns(); i++) {
      U value = foldFn(
          left.getConstantSubexpression(i)->as<Literal<T>>().value(),
...
static std::unique_ptr<Expression> cast_expression(
    const Context& context, const Expression& expr, const Type& type) {
  ExpressionArray ctorArgs;
  ctorArgs.push_back(expr.clone());
  std::unique_ptr<Expression> ctor =
      Constructor::Convert(context, expr.fOffset, type, std::move(ctorArgs));
  SkASSERT(ctor);
  return ctor;
}
...
bool ConstantFolder::ErrorOnDivideByZero(
    const Context& context, int offset, Operator op, const Expression& right) {
  switch (op.kind()) {
    ...
    case Token::Kind::TK_MINUS:
      ...
      if (is_constant_value(left, 0.0)) {  // 0 - x (to `-x`)
        return PrefixExpression::Make(
            context, Token::Kind::TK_MINUS, cast_expression(context, right, resultType));
      }
      break;
...
static std::unique_ptr<Expression> fold_float_expression(
    int offset, T result, const Type* resultType) {
...
std::unique_ptr<Expression> ConstantFolder::Simplify(
    const Context& context, int offset, const Expression& leftExpr, Operator op,
  ...
  if ((leftType.isScalar() || leftType.isVector()) &&
    ...
    std::unique_ptr<Expression> expr =
        simplify_no_op_arithmetic(context, *left, op, *right, resultType);
    if (expr) {
      return expr;
    }
  }
...
#define RESULT(Op) \
  fold_int_expression(offset, (SKSL_INT)(leftVal)Op(SKSL_INT)(rightVal), &resultType)
#define URESULT(Op) \
  fold_int_expression(offset, (SKSL_INT)((SKSL_UINT)(leftVal)Op(SKSL_UINT)(rightVal)), &resultType)
    switch (op.kind()) {
...
#undef RESULT
  ...
  if (leftType.isVector() && leftType.componentType() == rightType) {
    ...
    if (rightType.isInteger()) {
      return simplify_vector<SKSL_INT, SKSL_UINT>(
          context, *left, op, splat_scalar(*right, left->type()));
    }
    if (rightType.isBoolean()) {
      return simplify_vector_equality(context, *left, op, splat_scalar(*right, left->type()));
    }
    return nullptr;
  }
  ...
  if (rightType.isVector() && rightType.componentType() == leftType) {
    if (leftType.isFloat()) {
      return simplify_vector<SKSL_FLOAT>(context, splat_scalar(*left, right->type()), op, *right);
    }
    if (leftType.isInteger()) {
      return simplify_vector<SKSL_INT, SKSL_UINT>(
          context, splat_scalar(*left, right->type()), op, *right);
    }
    if (leftType.isBoolean()) {
      return simplify_vector_equality(context, splat_scalar(*left, right->type()), op, *right);
    }
    return nullptr;
  }
  ...
  if ((leftType.isMatrix() && rightType.isMatrix()) ||
    ...
    bool equality;
    switch (op.kind()) {
      case Token::Kind::TK_EQEQ: equality = true; break;
      case Token::Kind::TK_NEQ: equality = false; break;
      default: return nullptr;
    }
    ...
    switch (left->compareConstant(*right)) {
      ...
      case Expression::ComparisonResult::kUnknown: return nullptr;
    }
  }
