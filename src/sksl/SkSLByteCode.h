...
class ByteCodeFunction {
 ...
 private:
  ByteCodeFunction(const FunctionDeclaration* declaration) : fName(declaration->fName) {}
...
class SK_API ByteCode {
 public:
  ...
  union Vector {
    ...
    Vector(skvx::Vec<width, float> f) : fFloat(f) {}

    Vector(skvx::Vec<width, int32_t> i) : fInt(i) {}

    Vector(skvx::Vec<width, uint32_t> u) : fUInt(u) {}
  };
  ...
  struct Register {
    ...
    Register operator+(uint16_t offset) const { return Register{(uint16_t)(fIndex + offset)}; }
  };

  struct Pointer {
    uint16_t fAddress;

    Pointer operator+(uint16_t offset) const { return Pointer{(uint16_t)(fAddress + offset)}; }
  };
  ...
  union Immediate {
    ...
    Immediate(float f) : fFloat(f) {}

    Immediate(int32_t i) : fInt(i) {}

    Immediate(uint32_t u) : fUInt(u) {}
  };
  ...
  int getGlobalSlotCount() const { return fGlobalSlotCount; }
