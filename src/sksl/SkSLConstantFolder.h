...
class ConstantFolder {
 public:
   ...
   */
  static std::unique_ptr<Expression> MakeConstantValueForVariable(std::unique_ptr<Expression> expr);

  /**
   * Reports an error and returns true if op is a division / mod operator and right is zero or
   * contains a zero element.
   */
  static bool ErrorOnDivideByZero(
  ...
  static std::unique_ptr<Expression> Simplify(
      const Context& context, int offset, const Expression& left, Operator op,
