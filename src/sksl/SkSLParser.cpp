...
Parser::Parser(const char* text, size_t length, SymbolTable& types, ErrorReporter& errors)
    : fText(text), fPushback(Token::INVALID, -1, -1), fTypes(types), fErrors(errors) {
...
std::vector<std::unique_ptr<ASTDeclaration>> Parser::file() {
  ...
  for (;;) {
    switch (this->peek().fKind) {
      case Token::END_OF_FILE: return result;
...
bool Parser::expect(Token::Kind kind, const char* expected, Token* result) {
  ...
  if (next.fKind == kind) {
    ...
    this->error(next, "expected " + String(expected) + ", but found '" + this->text(next) + "'");
    return false;
  }
}
...
void Parser::error(Token token, String msg) { this->error(token.fOffset, msg); }

void Parser::error(int offset, String msg) { fErrors.error(offset, msg); }

bool Parser::isType(StringFragment name) { return nullptr != fTypes[name]; }
...
std::unique_ptr<ASTDeclaration> Parser::directive() {
  ...
  if (text == "#version") {
    ...
    return std::unique_ptr<ASTDeclaration>(
        new ASTExtension(start.fOffset, String(this->text(name))));
  } else {
...
std::unique_ptr<ASTDeclaration> Parser::section() {
  ...
  for (;;) {
    ...
    switch (next.fKind) {
      case Token::LBRACE: ++level; break;
      case Token::RBRACE: --level; break;
  ...
  --name.fLength;
  return std::unique_ptr<ASTDeclaration>(
      new ASTSection(start.fOffset, String(name), argument, text));
}
...
std::unique_ptr<ASTDeclaration> Parser::enumDeclaration() {
  ...
  fTypes.add(
      this->text(name), std::unique_ptr<Symbol>(new Type(this->text(name), Type::kEnum_Kind)));
  ...
  return std::unique_ptr<ASTDeclaration>(
      new ASTEnum(name.fOffset, this->text(name), names, std::move(values)));
}
...
std::unique_ptr<ASTDeclaration> Parser::declaration() {
  ...
  if (this->checkNext(Token::LPAREN)) {
    ...
    return std::unique_ptr<ASTDeclaration>(new ASTFunction(
        name.fOffset, modifiers, std::move(type), this->text(name), std::move(parameters),
        std::move(body)));
  } else {
...
std::unique_ptr<ASTType> Parser::structDeclaration() {
  ...
  while (this->peek().fKind != Token::RBRACE) {
    ...
    for (const auto& var : decl->fVars) {
      ...
      for (int i = (int)var.fSizes.size() - 1; i >= 0; i--) {
        ...
        type = (Type*)fTypes.takeOwnership(
            std::unique_ptr<Symbol>(new Type(name, Type::kArray_Kind, *type, (int)columns)));
      }
  ...
  fTypes.add(
      this->text(name), std::unique_ptr<Type>(new Type(name.fOffset, this->text(name), fields)));
  return std::unique_ptr<ASTType>(new ASTType(
      name.fOffset, this->text(name), ASTType::kStruct_Kind, std::vector<int>(), false));
}
...
std::unique_ptr<ASTVarDeclarations> Parser::structVarDeclaration(Modifiers modifiers) {
  ...
  if (this->checkNext(Token::IDENTIFIER, &name)) {
    std::unique_ptr<ASTVarDeclarations> result =
        this->varDeclarationEnd(modifiers, std::move(type), this->text(name));
    if (result) {
      for (const auto& var : result->fVars) {
        if (var.fValue) {
          this->error(var.fValue->fOffset, "struct variables cannot be initialized");
        }
      }
    }
    return result;
...
std::unique_ptr<ASTVarDeclarations> Parser::varDeclarationEnd(
    Modifiers mods, std::unique_ptr<ASTType> type, StringFragment name) {
  ...
  return std::unique_ptr<ASTVarDeclarations>(
...
std::unique_ptr<ASTParameter> Parser::parameter() {
  ...
  return std::unique_ptr<ASTParameter>(new ASTParameter(
      name.fOffset, modifiers, std::move(type), this->text(name), std::move(sizes)));
}
...
String Parser::layoutCode() {
  ...
  while (!done) {
    Token next = this->nextRawToken();
    switch (next.fKind) {
      case Token::LPAREN: ++level; break;
      case Token::RPAREN: --level; break;
      case Token::COMMA:
        if (level == 1) {
          done = true;
        }
...
Layout Parser::layout() {
  ...
  if (this->checkNext(Token::LAYOUT)) {
    if (!this->expect(Token::LPAREN, "'('")) {
      return Layout(
          flags, location, offset, binding, index, set, builtin, inputAttachmentIndex, format,
          primitive, maxVertices, invocations, when, key, ctype);
    }
    for (;;) {
      Token t = this->nextToken();
      String text = this->text(t);
      ...
      if (found != layoutTokens->end()) {
        switch (found->second) {
          case LayoutToken::LOCATION: location = this->layoutInt(); break;
          case LayoutToken::OFFSET: offset = this->layoutInt(); break;
          case LayoutToken::BINDING: binding = this->layoutInt(); break;
          case LayoutToken::INDEX: index = this->layoutInt(); break;
          case LayoutToken::SET: set = this->layoutInt(); break;
          case LayoutToken::BUILTIN: builtin = this->layoutInt(); break;
          case LayoutToken::INPUT_ATTACHMENT_INDEX: inputAttachmentIndex = this->layoutInt(); break;
          case LayoutToken::ORIGIN_UPPER_LEFT: flags |= Layout::kOriginUpperLeft_Flag; break;
          case LayoutToken::OVERRIDE_COVERAGE: flags |= Layout::kOverrideCoverage_Flag; break;
          ...
          case LayoutToken::BLEND_SUPPORT_MULTIPLY:
            ...
            break;
          case LayoutToken::BLEND_SUPPORT_SCREEN: flags |= Layout::kBlendSupportScreen_Flag; break;
          case LayoutToken::BLEND_SUPPORT_OVERLAY:
            flags |= Layout::kBlendSupportOverlay_Flag;
            break;
          case LayoutToken::BLEND_SUPPORT_DARKEN: flags |= Layout::kBlendSupportDarken_Flag; break;
          ...
          case LayoutToken::BLEND_SUPPORT_EXCLUSION:
            ...
            break;
          case LayoutToken::BLEND_SUPPORT_HSL_HUE: flags |= Layout::kBlendSupportHSLHue_Flag; break;
          ...
          case LayoutToken::BLEND_SUPPORT_HSL_LUMINOSITY:
            ...
            break;
          case LayoutToken::PUSH_CONSTANT: flags |= Layout::kPushConstant_Flag; break;
          case LayoutToken::TRACKED: flags |= Layout::kTracked_Flag; break;
          case LayoutToken::POINTS: primitive = Layout::kPoints_Primitive; break;
          case LayoutToken::LINES: primitive = Layout::kLines_Primitive; break;
          case LayoutToken::LINE_STRIP: primitive = Layout::kLineStrip_Primitive; break;
          case LayoutToken::LINES_ADJACENCY: primitive = Layout::kLinesAdjacency_Primitive; break;
          case LayoutToken::TRIANGLES: primitive = Layout::kTriangles_Primitive; break;
          case LayoutToken::TRIANGLE_STRIP: primitive = Layout::kTriangleStrip_Primitive; break;
          case LayoutToken::TRIANGLES_ADJACENCY:
            primitive = Layout::kTrianglesAdjacency_Primitive;
            break;
          case LayoutToken::MAX_VERTICES: maxVertices = this->layoutInt(); break;
          case LayoutToken::INVOCATIONS: invocations = this->layoutInt(); break;
          case LayoutToken::WHEN: when = this->layoutCode(); break;
          case LayoutToken::KEY: key = this->layoutKey(); break;
          case LayoutToken::CTYPE: ctype = this->layoutCType(); break;
          default:
...
std::unique_ptr<ASTStatement> Parser::statement() {
  ...
  switch (start.fKind) {
    ...
    case Token::STATIC_IF: return this->ifStatement();
    case Token::FOR: return this->forStatement();
    case Token::DO: return this->doStatement();
    case Token::WHILE: return this->whileStatement();
    case Token::SWITCH:  // fall through
    case Token::STATIC_SWITCH: return this->switchStatement();
    case Token::RETURN: return this->returnStatement();
    case Token::BREAK: return this->breakStatement();
    case Token::CONTINUE: return this->continueStatement();
    case Token::DISCARD: return this->discardStatement();
    case Token::LBRACE: return this->block();
    case Token::SEMICOLON:
      this->nextToken();
      return std::unique_ptr<ASTStatement>(
          new ASTBlock(start.fOffset, std::vector<std::unique_ptr<ASTStatement>>()));
    case Token::CONST: {
...
std::unique_ptr<ASTType> Parser::type() {
  ...
  return std::unique_ptr<ASTType>(
      new ASTType(type.fOffset, this->text(type), ASTType::kIdentifier_Kind, sizes, nullable));
}
...
std::unique_ptr<ASTDeclaration> Parser::interfaceBlock(Modifiers mods) {
  ...
  this->expect(Token::SEMICOLON, "';'");
  return std::unique_ptr<ASTDeclaration>(new ASTInterfaceBlock(
      name.fOffset, mods, this->text(name), std::move(decls), instanceName, std::move(sizes)));
}
...
std::unique_ptr<ASTIfStatement> Parser::ifStatement() {
  ...
  return std::unique_ptr<ASTIfStatement>(new ASTIfStatement(
      start.fOffset, isStatic, std::move(test), std::move(ifTrue), std::move(ifFalse)));
}
...
std::unique_ptr<ASTDoStatement> Parser::doStatement() {
  ...
  return std::unique_ptr<ASTDoStatement>(
      new ASTDoStatement(start.fOffset, std::move(statement), std::move(test)));
}
...
std::unique_ptr<ASTWhileStatement> Parser::whileStatement() {
  ...
  return std::unique_ptr<ASTWhileStatement>(
      new ASTWhileStatement(start.fOffset, std::move(test), std::move(statement)));
}
...
std::unique_ptr<ASTSwitchCase> Parser::switchCase() {
  ...
  return std::unique_ptr<ASTSwitchCase>(
      new ASTSwitchCase(start.fOffset, std::move(value), std::move(statements)));
}
...
std::unique_ptr<ASTStatement> Parser::switchStatement() {
  ...
  if (this->peek().fKind == Token::DEFAULT) {
    ...
    cases.emplace_back(new ASTSwitchCase(defaultStart.fOffset, nullptr, std::move(statements)));
  }
  if (!this->expect(Token::RBRACE, "'}'")) {
    return nullptr;
  }
  return std::unique_ptr<ASTStatement>(
      new ASTSwitchStatement(start.fOffset, isStatic, std::move(value), std::move(cases)));
}
...
std::unique_ptr<ASTForStatement> Parser::forStatement() {
  ...
  switch (nextToken.fKind) {
    case Token::SEMICOLON: this->nextToken(); break;
    case Token::CONST: {
      std::unique_ptr<ASTVarDeclarations> vd = this->varDeclarations();
      if (!vd) {
        return nullptr;
      }
      initializer = std::unique_ptr<ASTStatement>(new ASTVarDeclarationStatement(std::move(vd)));
      break;
    }
    case Token::IDENTIFIER: {
    ...
    default: initializer = this->expressionStatement();
  }
  ...
  return std::unique_ptr<ASTForStatement>(new ASTForStatement(
      start.fOffset, std::move(initializer), std::move(test), std::move(next),
      std::move(statement)));
}
...
std::unique_ptr<ASTReturnStatement> Parser::returnStatement() {
  ...
  return std::unique_ptr<ASTReturnStatement>(
      new ASTReturnStatement(start.fOffset, std::move(expression)));
}
...
std::unique_ptr<ASTBlock> Parser::block() {
  ...
  for (;;) {
    switch (this->peek().fKind) {
      case Token::RBRACE:
        this->nextToken();
        return std::unique_ptr<ASTBlock>(new ASTBlock(start.fOffset, std::move(statements)));
      case Token::END_OF_FILE:
  ...
  for (;;) {
    switch (this->peek().fKind) {
      ...
      case Token::LOGICALOREQ: {
        ...
        result = std::unique_ptr<ASTExpression>(
            new ASTBinaryExpression(std::move(result), std::move(t), std::move(right)));
        return result;
      }
  ...
  if (this->checkNext(Token::QUESTION)) {
    ...
    if (this->expect(Token::COLON, "':'")) {
      ...
      return std::unique_ptr<ASTExpression>(
          new ASTTernaryExpression(std::move(result), std::move(trueExpr), std::move(falseExpr)));
    }
    return nullptr;
  }
  return result;
...
std::unique_ptr<ASTExpression> Parser::unaryExpression() {
  switch (this->peek().fKind) {
    ...
    case Token::MINUSMINUS: {
      ...
      return std::unique_ptr<ASTExpression>(new ASTPrefixExpression(std::move(t), std::move(expr)));
    }
...
std::unique_ptr<ASTSuffix> Parser::suffix() {
  ...
  switch (next.fKind) {
    ...
    case Token::LPAREN: {
      ...
      return std::unique_ptr<ASTSuffix>(new ASTCallSuffix(next.fOffset, std::move(parameters)));
    }
    case Token::PLUSPLUS:
      return std::unique_ptr<ASTSuffix>(
          new ASTSuffix(next.fOffset, ASTSuffix::kPostIncrement_Kind));
    case Token::MINUSMINUS:
      return std::unique_ptr<ASTSuffix>(
          new ASTSuffix(next.fOffset, ASTSuffix::kPostDecrement_Kind));
    default: {
...
bool Parser::boolLiteral(bool* dest) {
  Token t = this->nextToken();
  switch (t.fKind) {
    case Token::TRUE_LITERAL: *dest = true; return true;
    case Token::FALSE_LITERAL: *dest = false; return true;
    default:
