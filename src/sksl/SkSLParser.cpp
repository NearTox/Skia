...
Parser::Parser(const char* text, size_t length, SymbolTable& types, ErrorReporter& errors)
    : fText(text), fPushback(Token::Kind::TK_INVALID, -1, -1), fTypes(types), fErrors(errors) {
...
ASTNode::ID Parser::section() {
  ...
  ++name.fChars;
  --name.fLength;
  RETURN_NODE(start.fOffset, ASTNode::Kind::kSection, ASTNode::SectionData(name, argument, text));
}
...
ASTNode::ID Parser::structDeclaration() {
  ...
  RETURN_NODE(name.fOffset, ASTNode::Kind::kType, ASTNode::TypeData(this->text(name), true, false));
}
...
ASTNode::ID Parser::parameter() {
  ...
  while (this->checkNext(Token::Kind::TK_LBRACKET)) {
    ...
    CREATE_CHILD(
        child, result, sizeToken.fOffset, ASTNode::Kind::kInt, SkSL::stoi(this->text(sizeToken)));
    if (!this->expect(Token::Kind::TK_RBRACKET, "']'")) {
      return ASTNode::ID::Invalid();
    }
    ++pd.fSizeCount;
