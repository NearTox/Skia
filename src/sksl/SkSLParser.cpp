...
std::unique_ptr<ASTFile> Parser::file() {
  ...
  for (;;) {
    switch (this->peek().fKind) {
      case Token::END_OF_FILE: return std::move(fFile);
...
ASTNode::ID Parser::statement() {
  ...
  switch (start.fKind) {
    ...
    case Token::SEMICOLON: this->nextToken(); RETURN_NODE(start.fOffset, ASTNode::Kind::kBlock);
    case Token::CONST: return this->varDeclarations();
...
ASTNode::ID Parser::block() {
  ...
  for (;;) {
    switch (this->peek().fKind) {
      case Token::RBRACE: this->nextToken(); return result;
      case Token::END_OF_FILE:
...
ASTNode::ID Parser::term() {
  ...
  switch (t.fKind) {
    ...
    case Token::NULL_LITERAL: this->nextToken(); RETURN_NODE(t.fOffset, ASTNode::Kind::kNull);
    case Token::LPAREN: {
      this->nextToken();
      AutoDepth depth(this);
      if (!depth.increase()) {
