...
void HCodeGenerator::writeMake() {
  ...
  if (!this->writeSection(kMakeSection)) {
    ...
    for (const auto& param : fSectionAndParameterHelper.getParameters()) {
      this->writef(
          "%s%s %s", separator,
          ParameterType(fContext, param->type(), param->modifiers().fLayout).c_str(),
...
void HCodeGenerator::writeConstructor() {
  ...
  for (const Variable* param : fSectionAndParameterHelper.getParameters()) {
    ...
    if (paramType.typeKind() == Type::TypeKind::kSampler) {
      ...
      this->writef(
          "        this->registerChild(std::move(%s), %s);", String(param->name()).c_str(),
          usageArg.c_str());
    }
  }
  if (samplerCount) {
...
void HCodeGenerator::writeFields() {
  ...
  for (const auto& param : fSectionAndParameterHelper.getParameters()) {
    ...
    if (param->type().nonnullable() == *fContext.fFragmentProcessor_Type) {
      ...
      this->writef(
          "    %s %s;\n", FieldType(fContext, param->type(), param->modifiers().fLayout).c_str(),
          name.c_str());
    }
  }
}
