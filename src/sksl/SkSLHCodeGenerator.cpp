...
String HCodeGenerator::ParameterType(
...
Layout::CType HCodeGenerator::ParameterCType(
    const Context& context, const Type& type, const Layout& layout) {
  ...
  if (type.kind() == Type::kNullable_Kind) {
    ...
    return Layout::CType::kFloat;
      ...
      type == *context.fInt_Type || type == *context.fShort_Type || type == *context.fByte_Type) {
    return Layout::CType::kInt32;
  } else if (type == *context.fFloat2_Type || type == *context.fHalf2_Type) {
    return Layout::CType::kSkPoint;
  } else if (
      type == *context.fInt2_Type || type == *context.fShort2_Type ||
      type == *context.fByte2_Type) {
    return Layout::CType::kSkIPoint;
  } else if (
      type == *context.fInt4_Type || type == *context.fShort4_Type ||
...
String HCodeGenerator::FieldType(const Context& context, const Type& type, const Layout& layout) {
...
String HCodeGenerator::AccessType(const Context& context, const Type& type, const Layout& layout) {
...
void HCodeGenerator::writeConstructor() {
  ...
  for (const auto& param : fSectionAndParameterHelper.getParameters()) {
    ...
    if (type.kind() == Type::kSampler_Kind) {
      ...
      for (const Section* s : fSectionAndParameterHelper.getSections(SAMPLER_PARAMS_SECTION)) {
        if (s->fArgument == name) {
  ...
  for (const Variable* param : fSectionAndParameterHelper.getParameters()) {
    if (param->fType.kind() == Type::kSampler_Kind) {
      ...
      if (explicitCoords && matrix.fKind == SampleMatrix::Kind::kNone) {
        ...
        switch (matrix.fKind) {
          ...
          case SampleMatrix::Kind::kConstantOrUniform:
            matrixArg.appendf(
                ", SkSL::SampleMatrix::MakeConstUniform(\"%s\")", matrix.fExpression.c_str());
            break;
      ...
      this->writef(
          "            %s_index = this->%s(std::move(%s)%s%s);",
          FieldName(String(param->fName).c_str()).c_str(), registerFunc.c_str(),
          String(param->fName).c_str(), matrixArg.c_str(), explicitArg.c_str());
...
void HCodeGenerator::writeFields() {
  ...
  for (const auto& param : fSectionAndParameterHelper.getParameters()) {
    ...
    if (param->fType.nonnullable() == *fContext.fFragmentProcessor_Type) {
      ...
      this->writef(
          "    %s %s;\n", FieldType(fContext, param->fType, param->fModifiers.fLayout).c_str(),
          name.c_str());
    }
  }
}
