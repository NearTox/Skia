...
Layout::CType HCodeGenerator::ParameterCType(const Context& context, const Type& type,
    ...
    } else if (type == *context.fInt_Type || type == *context.fShort_Type ||
    ...
    } else if (type == *context.fInt2_Type || type == *context.fShort2_Type ||
...
void HCodeGenerator::writeConstructor() {
    ...
    for (const auto& param : fSectionAndParameterHelper.getParameters()) {
        ...
        if (type.kind() == Type::kSampler_Kind) {
            ...
            for (const Section* s :
                 fSectionAndParameterHelper.getSections(SAMPLER_PARAMS_SECTION)) {
                if (s->fArgument == name) {
