...
void DSLFunction::init(
    DSLModifiers modifiers, const DSLType& returnType, skstd::string_view name,
  ...
  fDecl = SkSL::FunctionDeclaration::Convert(
      DSLWriter::Context(), *DSLWriter::SymbolTable(), pos.offset(),
      DSLWriter::Modifiers(modifiers.fModifiers), name == "main" ? name : DSLWriter::Name(name),
      std::move(paramVars), &returnType.skslType(), DSLWriter::IsModule());
  ...
  if (fDecl) {
    ...
    DSLWriter::ProgramElements().push_back(
        std::make_unique<SkSL::FunctionPrototype>(pos.offset(), fDecl, DSLWriter::IsModule()));
  }
}
