...
void DSLFunction::init(
    DSLModifiers modifiers, const DSLType& returnType, std::string_view name,
  ...
  fDecl = SkSL::FunctionDeclaration::Convert(
      ThreadContext::Context(), *ThreadContext::SymbolTable(), pos, modifiers.fPosition,
      ThreadContext::Modifiers(modifiers.fModifiers), name, std::move(paramVars), pos,
      &returnType.skslType());
  if (fDecl) {
    ...
    ThreadContext::ProgramElements().push_back(
        std::make_unique<SkSL::FunctionPrototype>(pos, fDecl, ThreadContext::IsModule()));
  }
}
...
void DSLFunction::define(DSLBlock block, Position pos) {
  ...
  if (fDecl->definition()) {
    ThreadContext::ReportError(
        SkSL::String::printf("function '%s' was already defined", fDecl->description().c_str()),
        pos);
    block.release();
    return;
  }
  std::unique_ptr<FunctionDefinition> function = FunctionDefinition::Convert(
      ThreadContext::Context(), pos, *fDecl, std::move(body),
      /*builtin=*/false);
