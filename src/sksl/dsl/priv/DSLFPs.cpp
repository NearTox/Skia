...
void EndFragmentProcessor() { DSLWriter::EndFragmentProcessor(); }

DSLGlobalVar sk_SampleCoord() { return DSLGlobalVar("sk_SampleCoord"); }
...
DSLExpression SampleChild(int index, DSLExpression sampleExpr) {
  ...
  switch (expr ? expr->type().columns() : 0) {
    default:
      ...
      [[fallthrough]];
    case 0: code = proc->invokeChild(index, emitArgs); break;
    case 2: code = proc->invokeChild(index, emitArgs, /*skslCoords=*/expr->description()); break;
    case 4:
