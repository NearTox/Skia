...
DSLWriter::DSLWriter(SkSL::Compiler* compiler) : fCompiler(compiler) {
...
const SkSL::Context& DSLWriter::Context() { return IRGenerator().fContext; }
...
DSLExpression DSLWriter::Construct(const SkSL::Type& type, std::vector<DSLExpression> rawArgs) {
  ...
  return DSLExpression(DSLWriter::IRGenerator().call(
      /*offset=*/-1,
      std::make_unique<SkSL::TypeReference>(
          DSLWriter::Context(),
          /*offset=*/-1, &type),
      std::move(args)));
}

DSLExpression DSLWriter::ConvertBinary(
    std::unique_ptr<Expression> left, Token::Kind op, std::unique_ptr<Expression> right) {
  return DSLExpression(
      Check(IRGenerator().convertBinaryExpression(std::move(left), op, std::move(right))));
}

DSLExpression DSLWriter::ConvertIndex(
...
DSLWriter& DSLWriter::Instance() { SkUNREACHABLE; }

void DSLWriter::SetInstance(std::unique_ptr<DSLWriter> instance) { SkDEBUGFAIL("unimplemented"); }
