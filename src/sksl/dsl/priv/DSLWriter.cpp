...
const SkSL::Variable* DSLWriter::Var(DSLVarBase& var) {
  ...
  if (!var.fInitialized) {
    ...
    std::unique_ptr<SkSL::Variable> skslvar = SkSL::Variable::Convert(
        ThreadContext::Context(), var.fPosition, var.fModifiers.fPosition,
        var.fModifiers.fModifiers, &var.fType.skslType(), var.fNamePosition, var.fName,
        /*isArray=*/false,
        /*arraySize=*/nullptr, var.storage());
    SkSL::Variable* varPtr = skslvar.get();
    if (var.storage() != SkSL::VariableStorage::kParameter) {
      var.fDeclaration = VarDeclaration::Convert(
          ThreadContext::Context(), std::move(skslvar), var.fInitialValue.releaseIfPossible(),
...
std::unique_ptr<SkSL::Variable> DSLWriter::CreateParameterVar(DSLParameter& var) {
  ...
  return SkSL::Variable::Convert(
      ThreadContext::Context(), var.fPosition, var.fModifiers.fPosition, var.fModifiers.fModifiers,
      &var.fType.skslType(), var.fNamePosition, var.fName, /*isArray=*/false, /*arraySize=*/nullptr,
      var.storage());
}
...
void DSLWriter::AddVarDeclaration(DSLStatement& existing, DSLVar& additional) {
  if (existing.fStatement->is<Block>()) {
    ...
    existing.fStatement = SkSL::Block::Make(pos, std::move(stmts), Block::Kind::kCompoundStatement);
