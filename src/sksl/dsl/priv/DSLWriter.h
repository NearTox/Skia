...
class DSLWriter {
 public:
  ...
  static std::vector<const ProgramElement*>& SharedElements() { return Instance().fSharedElements; }
  ...
  static bool InFragmentProcessor() { return !Instance().fStack.empty(); }

  /**
   * Pushes a new processor / emitArgs pair for the current thread.
   */
  static void StartFragmentProcessor(
...
#else
  static bool InFragmentProcessor() { return false; }
#endif  // !defined(SKSL_STANDALONE) && SK_SUPPORT_GPU

  static std::unique_ptr<SkSL::Expression> Call(
   ...
   */
  static std::unique_ptr<SkSL::Expression> Call(
      std::unique_ptr<SkSL::Expression> expr, ExpressionArray arguments,
      PositionInfo pos = PositionInfo::Capture());
   ...
   */
  static ErrorReporter& GetErrorReporter() { return *Context().fErrors; }
   ...
   */
  static bool ManglingEnabled() { return Instance().fSettings.fDSLMangling; }

  /**
   * Returns whether DSLVars should automatically be marked declared upon creation. This is used
   * to simplify testing.
   */
  static bool MarkVarsDeclared() { return Instance().fSettings.fDSLMarkVarsDeclared; }
