...
DSLExpression::DSLExpression(std::unique_ptr<SkSL::Expression> expression)
  ...
  DSLWriter::ReportErrors(
      PositionInfo::Offset(nullptr, GetErrorReporter().source(), fExpression->fOffset));
}

DSLExpression::DSLExpression(float value, PositionInfo pos)
    : fExpression(SkSL::FloatLiteral::Make(DSLWriter::Context(), pos.offset(), value)) {
  if (!isfinite(value)) {
    if (isinf(value)) {
...
DSLExpression::DSLExpression(int value, PositionInfo pos)
    : fExpression(SkSL::IntLiteral::Make(DSLWriter::Context(), pos.offset(), value)) {}
...
DSLExpression::DSLExpression(bool value, PositionInfo pos)
    : fExpression(SkSL::BoolLiteral::Make(DSLWriter::Context(), pos.offset(), value)) {}

DSLExpression::DSLExpression(DSLVarBase& var, PositionInfo pos) {
  fExpression = std::make_unique<SkSL::VariableReference>(
      pos.offset(), DSLWriter::Var(var), SkSL::VariableReference::RefKind::kRead);
}

DSLExpression::DSLExpression(DSLVarBase&& var, PositionInfo pos) : DSLExpression(var) {}
...
DSLExpression::~DSLExpression() {
  ...
  if (fExpression && DSLWriter::InFragmentProcessor()) {
    DSLWriter::CurrentEmitArgs()->fFragBuilder->codeAppend(DSLStatement(this->release()).release());
    return;
  }
#endif
  SkASSERTF(
      !fExpression || !DSLWriter::Settings().fAssertDSLObjectsReleased,
...
DSLExpression DSLExpression::x(PositionInfo pos) { return Swizzle(std::move(*this), X, pos); }

DSLExpression DSLExpression::y(PositionInfo pos) { return Swizzle(std::move(*this), Y, pos); }

DSLExpression DSLExpression::z(PositionInfo pos) { return Swizzle(std::move(*this), Z, pos); }

DSLExpression DSLExpression::w(PositionInfo pos) { return Swizzle(std::move(*this), W, pos); }

DSLExpression DSLExpression::r(PositionInfo pos) { return Swizzle(std::move(*this), R, pos); }

DSLExpression DSLExpression::g(PositionInfo pos) { return Swizzle(std::move(*this), G, pos); }

DSLExpression DSLExpression::b(PositionInfo pos) { return Swizzle(std::move(*this), B, pos); }

DSLExpression DSLExpression::a(PositionInfo pos) { return Swizzle(std::move(*this), A, pos); }
...
DSLPossibleExpression DSLExpression::operator()(
    SkTArray<DSLWrapper<DSLExpression>> args, PositionInfo pos) {
...
DSLPossibleExpression LogicalXor(DSLExpression left, DSLExpression right) {
  return DSLWriter::ConvertBinary(
      left.release(), SkSL::Token::Kind::TK_LOGICALXOR, right.release());
}
OP(==, TK_EQEQ)
OP(!=, TK_NEQ)
OP(>, TK_GT)
...
DSLPossibleExpression operator,(DSLExpression left, DSLPossibleExpression right) {
  return DSLWriter::ConvertBinary(
      left.release(), SkSL::Token::Kind::TK_COMMA, DSLExpression(std::move(right)).release());
}
...
DSLPossibleExpression::DSLPossibleExpression(std::unique_ptr<SkSL::Expression> expr) noexcept
    : fExpression(std::move(expr)) {}

DSLPossibleExpression::DSLPossibleExpression(DSLPossibleExpression&& other) noexcept
    : fExpression(std::move(other.fExpression)) {}
