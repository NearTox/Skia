...
class ByteCodeGenerator : public CodeGenerator {
 public:
  ...
  void writeTypedInstruction(
      ...
      int count, bool writeCount = true);
 ...
 private:
...
#ifdef SK_DEBUG
    ~DeferredLocation() { SkASSERT(fSet); }
#endif
  ...
  struct Intrinsic {
    Intrinsic(ByteCodeInstruction instruction) : fIsSpecial(false), fValue(instruction) {}
    ...
    union Value {
      Value(ByteCodeInstruction instruction) : fInstruction(instruction) {}
