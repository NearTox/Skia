...
class ByteCodeGenerator : public CodeGenerator {
 ...
 private:
  ...
  struct Intrinsic {
    Intrinsic(ByteCode::Instruction instruction) : fIsSpecial(false), fValue(instruction) {}

    Intrinsic(SpecialIntrinsic special) : fIsSpecial(true), fValue(special) {}

    bool fIsSpecial;

    union Value {
      Value(ByteCode::Instruction instruction) : fInstruction(instruction) {}

      Value(SpecialIntrinsic special) : fSpecial(special) {}
  ...
  struct Location {
    enum { kPointer_Kind, kRegister_Kind } fKind;
    ...
    Location(ByteCode::Pointer p) : fKind(kPointer_Kind), fPointer(p) {}

    Location(ByteCode::Register r) : fKind(kRegister_Kind), fRegister(r) {}
    ...
    Location offset(ByteCodeGenerator& generator, ByteCode::Register offset) {
      ...
      switch (fKind) {
        case kPointer_Kind:
          ...
          break;
        case kRegister_Kind: current = fRegister;
      }
      ByteCode::Register result = generator.next(1);
      generator.write(ByteCode::Instruction::kAddI);
      generator.write(result);
  ...
  class DeferredLocation {
   public:
    explicit DeferredLocation(ByteCodeGenerator* generator)
        : fGenerator(*generator), fOffset(generator->fCode->size()) {
    ...
    void set() {
      ...
      static_assert(
          sizeof(ByteCode::Pointer) == 2, "failed assumption that ByteCode::Pointer is uint16_t");
  ...
  void writeVectorBinaryInstruction(
      const Type& operandType, ByteCode::Register left, ByteCode::Register right,
      ByteCode::Instruction s, ByteCode::Instruction u, ByteCode::Instruction f,
      ByteCode::Register result);
...
inline void ByteCodeGenerator::write(ByteCodeGenerator::Location loc) {
  switch (loc.fKind) {
    case ByteCodeGenerator::Location::kPointer_Kind: this->write(loc.fPointer); break;
    case ByteCodeGenerator::Location::kRegister_Kind: this->write(loc.fRegister); break;
  }
}
