...
#    define READ_INST() \
      (ip += sizeof(instruction), sk_unaligned_load<instruction>(ip - sizeof(instruction)))
...
#    define VECTOR_DISASSEMBLE(op, text) \
      case ByteCodeInstruction::op:      \
        ...
        ++ip;                            \
        break;                           \
      case ByteCodeInstruction::op##2:   \
        printf(text "2");                \
        ++ip;                            \
        break;                           \
      case ByteCodeInstruction::op##3:   \
        printf(text "3");                \
        ++ip;                            \
        break;                           \
...
#    define VECTOR_MATRIX_DISASSEMBLE(op, text) \
      ...
      case ByteCodeInstruction::op##N:          \
        printf(text "N %d", READ8());           \
        break;
  ...
  static const uint8_t* DisassembleInstruction(const uint8_t* ip) {
    switch ((ByteCodeInstruction)(intptr_t)READ_INST()) {
      ...
      case ByteCodeInstruction::kReadExternal4:
        printf("readexternal4 %d", READ16() >> 8);
        break;
...
#    define VECTOR_LABELS(base) &&base##4, &&base##3, &&base##2, &&base

#    define VECTOR_MATRIX_LABELS(base) VECTOR_LABELS(base), &&base##N
...
#    define CHECK_LABEL(name) SkASSERT(labels[(int)ByteCodeInstruction::name] == &&name)
...
#      ifdef TRACE
  next:
    printf(
        "at %3d (stack: %d) (disable threaded code for disassembly)\n", (int)(ip - code),
        (int)(sp - stack) + 1);
    goto* READ_INST();
#      endif
#    else
...
#    endif
  ...
  while (ip < fCode.data() + fCode.size()) {
    ...
    switch (inst) {
      ...
      case ByteCodeInstruction::kLoadExtendedGlobal:
      case ByteCodeInstruction::kLoadExtendedUniform: READ8(); break;
      ...
      case ByteCodeInstruction::kOrB: break; VECTOR_MATRIX_PREPROCESS_NO_COUNT(kPop)
