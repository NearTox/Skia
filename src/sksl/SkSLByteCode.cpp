...
#  define DISASSEMBLE_COUNT(op, text)   \
    case ByteCodeInstruction::op:       \
      out->printf(text " %d", READ8()); \
      break;

#  define DISASSEMBLE_COUNT_SLOT(op, text)   \
    case ByteCodeInstruction::op: {          \
      int N = READ8(), slot = READ8();       \
...
#  define VECTOR_UNARY_FN(inst, fn, field) \
  ...
  static F32 VecMod(F32 a, F32 b) { return a - skvx::trunc(a / b) * b; }

#  define spf(index) sp[index].fFloat

  static void CallExternal(
      const ByteCode* byteCode, const uint8_t*& ip, VValue*& sp, int baseIndex, I32 mask) {
    int argumentCount = READ8();
    int returnCount = READ8();
    int target = READ8();
  ...
  static void Inverse4x4(VValue* sp) {
    F32 a00 = spf(-15), a10 = spf(-11), a20 = spf(-7), a30 = spf(-3), a01 = spf(-14),
        a11 = spf(-10), a21 = spf(-6), a31 = spf(-2), a02 = spf(-13), a12 = spf(-9), a22 = spf(-5),
        a32 = spf(-1), a03 = spf(-12), a13 = spf(-8), a23 = spf(-4), a33 = spf(0);
  ...
  static bool InnerRun(
      ...
      VValue globals[], const float uniforms[], bool stripedOutput, int N, int baseIndex) {
...
#  endif
      ...
      switch (inst) {
        ...
        case ByteCodeInstruction::kBranch: ip = code + READ16(); continue;
        ...
        case ByteCodeInstruction::kInverse3x3: Inverse3x3(sp); continue;
        case ByteCodeInstruction::kInverse4x4: Inverse4x4(sp); continue;

        case ByteCodeInstruction::kLerp: {
          int count = READ8();
          VValue *T = sp - count + 1, *B = T - count, *A = B - count;
          for (int i = count; i-- > 0;) {
        ...
        case ByteCodeInstruction::kMix: {
          ...
          for (int i = count; i-- > 0;) {
            ...
            sp[-(2 * count + i)] = skvx::if_then_else(
                sp[-(i)].fSigned, sp[-(count + i)].fFloat, sp[-(2 * count + i)].fFloat);
          }
          sp -= 2 * count;
        }
          continue;

          VECTOR_BINARY_FN(kMod, fFloat, [](auto x, auto y) { return x - y * skvx::floor(x / y); })
        ...
        case ByteCodeInstruction::kReserve: sp += READ8(); continue;
        ...
        case ByteCodeInstruction::kShiftLeft: sp[0] = sp[0].fSigned << READ8(); continue;
        case ByteCodeInstruction::kShiftRightS: sp[0] = sp[0].fSigned >> READ8(); continue;
        case ByteCodeInstruction::kShiftRightU:
          ...
          VECTOR_UNARY_FN(
              kSign,
              [](auto x) {
                return skvx::if_then_else(
                    x < 0, F32(-1.0f), skvx::if_then_else(x > 0, F32(1.0f), F32(0.0f)));
              },
              fFloat)
        ...
        case ByteCodeInstruction::kMaskPop:
          ...
          --maskPtr;
          continue;
        case ByteCodeInstruction::kMaskNegate: maskPtr[0] = maskPtr[-1] & ~condPtr[0]; continue;
        ...
        case ByteCodeInstruction::kLoopNext:
          ...
          *contPtr = 0;
          continue;
        case ByteCodeInstruction::kLoopMask: *loopPtr &= POP().fSigned; continue;
        case ByteCodeInstruction::kLoopEnd:
          --contPtr;
          --loopPtr;
          continue;
        case ByteCodeInstruction::kLoopBreak: *loopPtr &= ~mask(); continue;
...
bool ByteCode::run(
    const ByteCodeFunction* f, float* args, int argCount, float* outReturn, int returnCount,
  ...
  if (argCount != f->fParameterCount || returnCount != f->fReturnCount ||
...
bool ByteCode::runStriped(
    const ByteCodeFunction* f, int N, float* args[], int argCount, float* outReturn[],
    int returnCount, const float* uniforms, int uniformCount) const {
#if defined(SK_ENABLE_SKSL_INTERPRETER)
  ...
  while (N) {
    ...
    if (!Interpreter::InnerRun(
            this, f, stack, outReturn, globals, uniforms, stripedOutput, w, baseIndex)) {
      return false;
    }
