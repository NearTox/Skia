...
void Interpreter::run(
    const ByteCodeFunction& f, Interpreter::Value args[], Interpreter::Value* outReturn) {
#  ifdef TRACE
  this->disassemble(f);
#  endif
...
#  define VECTOR_UNARY_FN(base, fn, field)                        \
    ...
    case ByteCodeInstruction::base: sp[0] = fn(sp[0].field); break;
...
static float mix(float start, float end, float t) { return start * (1 - t) + end * t; }
...
#  endif
    ...
    switch (inst) {
      ...
      case ByteCodeInstruction::kConvertFtoI: sp[0].fSigned = (int)sp[0].fFloat; break;
      ...
      case ByteCodeInstruction::kConvertStoF: sp[0].fFloat = sp[0].fSigned; break;
      ...
      case ByteCodeInstruction::kCross: {
        SkPoint3 cross = SkPoint3::CrossProduct(
            SkPoint3::Make(sp[-5].fFloat, sp[-4].fFloat, sp[-3].fFloat),
            SkPoint3::Make(sp[-2].fFloat, sp[-1].fFloat, sp[0].fFloat));
        sp -= 3;
        sp[-2] = cross.fX;
        sp[-1] = cross.fY;
        sp[0] = cross.fZ;
      ...
      case ByteCodeInstruction::kNot: sp[0].fBool = !sp[0].fBool; break;
      ...
      case ByteCodeInstruction::kNegateI:
        sp[0] = -sp[0].fSigned;
        break;
      ...
      case ByteCodeInstruction::kPushImmediate: PUSH(READ32()); break;
