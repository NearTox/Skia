...
Compiler::Compiler(Flags flags) : fFlags(flags), fContext(new Context()), fErrorCount(0) {
  ...
  fTypes = types;
#define ADD_TYPE(t) \
  types->addWithoutOwnership(fContext->f##t##_Type->fName, fContext->f##t##_Type.get())
  ADD_TYPE(Void);
  ADD_TYPE(Float);
  ADD_TYPE(Float2);
  ADD_TYPE(Float3);
  ...
  Variable* skCaps =
      new Variable(-1, Modifiers(), skCapsName, *fContext->fSkCaps_Type, Variable::kGlobal_Storage);
  fIRGenerator->fSymbolTable->add(skCapsName, std::unique_ptr<Symbol>(skCaps));

  StringFragment skArgsName("sk_Args");
  Variable* skArgs =
      new Variable(-1, Modifiers(), skArgsName, *fContext->fSkArgs_Type, Variable::kGlobal_Storage);
  ...
  this->processIncludeFile(
      Program::kFragment_Kind, SKSL_GPU_INCLUDE, strlen(SKSL_GPU_INCLUDE), symbols, &gpuIntrinsics,
      &fGpuSymbolTable);
  this->processIncludeFile(
      Program::kFragment_Kind, SKSL_BLEND_INCLUDE, strlen(SKSL_BLEND_INCLUDE),
      std::move(fGpuSymbolTable), &gpuIntrinsics, &fGpuSymbolTable);
  ...
  this->processIncludeFile(
      Program::kVertex_Kind, SKSL_VERT_INCLUDE, strlen(SKSL_VERT_INCLUDE), fGpuSymbolTable,
      &fVertexInclude, &fVertexSymbolTable);
  this->processIncludeFile(
      Program::kFragment_Kind, SKSL_FRAG_INCLUDE, strlen(SKSL_FRAG_INCLUDE), fGpuSymbolTable,
      &fFragmentInclude, &fFragmentSymbolTable);
  this->processIncludeFile(
      Program::kGeometry_Kind, SKSL_GEOM_INCLUDE, strlen(SKSL_GEOM_INCLUDE), fGpuSymbolTable,
      &fGeometryInclude, &fGeometrySymbolTable);
  this->processIncludeFile(
      Program::kPipelineStage_Kind, SKSL_PIPELINE_INCLUDE, strlen(SKSL_PIPELINE_INCLUDE),
      fGpuSymbolTable, &fPipelineInclude, &fPipelineSymbolTable);
  std::vector<std::unique_ptr<ProgramElement>> interpIntrinsics;
  this->processIncludeFile(
      Program::kGeneric_Kind, SKSL_INTERP_INCLUDE, strlen(SKSL_INTERP_INCLUDE), symbols,
...
void Compiler::addDefinition(
  ...
  switch (lvalue->fKind) {
    ...
    case Expression::kTernary_Kind:
      ...
      this->addDefinition(
          ((TernaryExpression*)lvalue)->fIfTrue.get(),
          (std::unique_ptr<Expression>*)&fContext->fDefined_Expression, definitions);
      this->addDefinition(
          ((TernaryExpression*)lvalue)->fIfFalse.get(),
          (std::unique_ptr<Expression>*)&fContext->fDefined_Expression, definitions);
      break;
...
static bool is_dead(const Expression& lvalue) {
  switch (lvalue.fKind) {
    case Expression::kVariableReference_Kind: return ((VariableReference&)lvalue).fVariable.dead();
    case Expression::kSwizzle_Kind: return is_dead(*((Swizzle&)lvalue).fBase);
    case Expression::kFieldAccess_Kind: return is_dead(*((FieldAccess&)lvalue).fBase);
    case Expression::kIndex_Kind: {
...
bool is_constant(const Expression& expr, double value) {
  switch (expr.fKind) {
    case Expression::kIntLiteral_Kind: return ((IntLiteral&)expr).fValue == value;
...
void clear_write(const Expression& expr) {
  switch (expr.fKind) {
    ...
    case Expression::kFieldAccess_Kind: clear_write(*((FieldAccess&)expr).fBase); break;
    case Expression::kSwizzle_Kind: clear_write(*((Swizzle&)expr).fBase); break;
    case Expression::kIndex_Kind: clear_write(*((IndexExpression&)expr).fBase); break;
  ...
  for (BasicBlock& b : cfg.fBlocks) {
    ...
    for (auto iter = b.fNodes.begin(); iter != b.fNodes.end() && !needsRescan;) {
      if (iter->fKind == BasicBlock::Node::kStatement_Kind) {
        ...
        switch (s.fKind) {
          case Statement::kIf_Kind:
            if (((const IfStatement&)s).fIsStatic && !(fFlags & kPermitInvalidStaticTests_Flag)) {
...
std::unique_ptr<Program> Compiler::convertProgram(
  ...
  switch (kind) {
    ...
    case Program::kFragmentProcessor_Kind:
      ...
      fIRGenerator->start(&settings, nullptr);
      fIRGenerator->fIntrinsics = &fGPUIntrinsics;
      fIRGenerator->convertProgram(
          kind, SKSL_FP_INCLUDE, strlen(SKSL_FP_INCLUDE), *fTypes, &elements);
      fIRGenerator->fSymbolTable->markAllFunctionsBuiltin();
      break;
...
std::unique_ptr<Program> Compiler::specialize(
  ...
  std::unique_ptr<Program> result(new Program(
      program.fKind, std::move(sourceCopy), settings, program.fContext, program.fInheritedElements,
      std::move(elements), program.fSymbols, program.fInputs));
  return result;
}
