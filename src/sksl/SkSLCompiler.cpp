...
Compiler::Compiler(const ShaderCapsClass* caps, Flags flags)
    : fCaps(caps), fFlags(flags), fContext(std::make_shared<Context>()), fErrorCount(0) {
...
#undef TYPE
  ...
  fPrivateSymbolTable->add(std::make_unique<Variable>(
      /*offset=*/-1, fIRGenerator->fModifiers->handle(Modifiers()), "sk_Caps",
...
const ParsedModule& Compiler::loadFPModule() {
  if (!fFPModule.fSymbols) {
    fFPModule = this->parseModule(Program::kFragmentProcessor_Kind, MODULE_DATA(fp), fGPUModule);
  }
  return fFPModule;
}
...
const ParsedModule& Compiler::loadPipelineModule() {
  if (!fPipelineModule.fSymbols) {
    fPipelineModule = this->parseModule(
        Program::kPipelineStage_Kind, MODULE_DATA(pipeline), this->loadPublicModule());
...
LoadedModule Compiler::loadModule(
  ...
  IRGenerator::IRBundle ir = fIRGenerator->convertProgram(
      kind, &settings, &standaloneCaps, baseModule,
...
std::unique_ptr<Program> Compiler::convertProgram(
  ...
  auto program = std::make_unique<Program>(
      ...
      std::move(ir.fModifiers), std::move(ir.fSymbolTable), std::move(pool), ir.fInputs);
  bool success = false;
  if (fErrorCount) {
      ...
      program.fElements.erase(
          std::remove_if(
              ...
              [&](const std::unique_ptr<ProgramElement>& element) {
                ...
                bool dead = fn.declaration().name() != "main" && usage->get(fn.declaration()) == 0;
                if (dead) {
                  madeChanges = true;
      ...
      program.fElements.erase(
          std::remove_if(
              ...
              [&](const std::unique_ptr<ProgramElement>& element) {
                ...
                const auto& varDecl = global.declaration()->as<VarDeclaration>();
