...
class AutoSource {
 public:
  AutoSource(Compiler* compiler, std::string_view source) : fCompiler(compiler) {
    SkASSERT(!fCompiler->errorReporter().source().data());
    fCompiler->errorReporter().setSource(source);
  }

  ~AutoSource() { fCompiler->errorReporter().setSource(std::string_view()); }
...
class AutoProgramConfig {
 public:
  AutoProgramConfig(std::shared_ptr<Context>& context, ProgramConfig* config) noexcept
      : fContext(context.get()), fOldConfig(fContext->fConfig) {
...
class AutoModifiersPool {
 public:
  AutoModifiersPool(std::shared_ptr<Context>& context, ModifiersPool* modifiersPool) noexcept
      : fContext(context.get()) {
    SkASSERT(!fContext->fModifiersPool);
    fContext->fModifiersPool = modifiersPool;
  }

  ~AutoModifiersPool() { fContext->fModifiersPool = nullptr; }
...
inline static constexpr BuiltinTypePtr kPrivateTypes[] = {
    TYPE(Sampler1D),     TYPE(Sampler2D),    TYPE(Sampler3D),      TYPE(SamplerExternalOES),
    TYPE(Sampler2DRect),

    TYPE(ISampler2D),    TYPE(SubpassInput), TYPE(SubpassInputMS),

    TYPE(Sampler),       TYPE(Texture2D),
};
...
std::shared_ptr<SymbolTable> Compiler::makePrivateSymbolTable(std::shared_ptr<SymbolTable> parent) {
  ...
  privateSymbolTable->add(std::make_unique<Variable>(
      /*pos=*/Position(),
      /*modifiersPosition=*/Position(), fCoreModifiers.add(Modifiers{}), "sk_Caps",
...
const ParsedModule& Compiler::loadFragmentModule() {
  if (!fFragmentModule.fSymbols) {
    fFragmentModule =
        this->parseModule(ProgramKind::kFragment, MODULE_DATA(frag), this->loadGPUModule());
  }
...
const ParsedModule& Compiler::loadVertexModule() {
  if (!fVertexModule.fSymbols) {
    fVertexModule =
        this->parseModule(ProgramKind::kVertex, MODULE_DATA(vert), this->loadGPUModule());
  }
...
const ParsedModule& Compiler::loadGraphiteFragmentModule() {
  if (!fGraphiteFragmentModule.fSymbols) {
    fGraphiteFragmentModule = this->parseModule(
        ProgramKind::kGraphiteFragment, MODULE_DATA(graphite_frag), this->loadFragmentModule());
  }
...
const ParsedModule& Compiler::loadGraphiteVertexModule() {
  if (!fGraphiteVertexModule.fSymbols) {
    fGraphiteVertexModule = this->parseModule(
        ProgramKind::kGraphiteVertex, MODULE_DATA(graphite_vert), this->loadVertexModule());
  }
...
LoadedModule Compiler::loadModule(
    ProgramKind kind, ModuleData data, std::shared_ptr<SymbolTable> base, bool dehydrate) {
  if (dehydrate) {
  ...
  LoadedModule module =
      DSLParser(this, settings, kind, std::move(text)).moduleInheritingFrom(baseModule);
  if (this->errorCount()) {
...
std::unique_ptr<Program> Compiler::convertProgram(
    ProgramKind kind, std::string text, Program::Settings settings) {
  ...
  switch (sOptimizer) {
    case OverrideFlag::kDefault: break;
    case OverrideFlag::kOff: settings.fOptimize = false; break;
    case OverrideFlag::kOn: settings.fOptimize = true; break;
  }

  switch (sInliner) {
    case OverrideFlag::kDefault: break;
    case OverrideFlag::kOff: settings.fInlineThreshold = 0; break;
  ...
  switch (var.modifiers().fLayout.fBuiltin) {
    case SK_FRAGCOORD_BUILTIN:
      if (fContext->fCaps.fCanUseFragCoord) {
        ThreadContext::Inputs().fUseFlipRTUniform = !fContext->fConfig->fSettings.fForceNoRTFlip;
      }
      break;
...
std::unique_ptr<Expression> Compiler::convertIdentifier(Position pos, std::string_view name) {
  ...
  switch (result->kind()) {
    ...
    case Symbol::Kind::kField: {
      ...
      auto base = VariableReference::Make(pos, &field->owner(), VariableReference::RefKind::kRead);
      return FieldAccess::Make(
          *fContext, pos, std::move(base), field->fieldIndex(),
...
void Compiler::handleError(std::string_view msg, Position pos) {
  ...
  if (printLocation) {
    ...
    for (int i = lineStart; i < (int)src.length(); i++) {
      ...
      switch (src[i]) {
        case '\t': fErrorText += (i >= pos.startOffset()) ? "^^^^" : "    "; break;
        case '\n':
          ...
          i = src.length();
          break;
        default: fErrorText += (i >= pos.startOffset()) ? '^' : ' '; break;
      }
    }
    fErrorText += '\n';
  }
