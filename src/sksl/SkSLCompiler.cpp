...
class AutoSource {
 public:
  AutoSource(Compiler* compiler, const char* source) : fCompiler(compiler) {
    SkASSERT(!fCompiler->errorReporter().source());
    fCompiler->errorReporter().setSource(source);
  }

  ~AutoSource() { fCompiler->errorReporter().setSource(nullptr); }
...
std::shared_ptr<SymbolTable> Compiler::makePrivateSymbolTable(std::shared_ptr<SymbolTable> parent) {
  ...
  privateSymbolTable->add(std::make_unique<Variable>(
      /*offset=*/-1, fCoreModifiers.add(Modifiers{}), "sk_Caps", fContext->fTypes.fSkCaps.get(),
...
LoadedModule Compiler::loadModule(
  ...
  IRGenerator::IRBundle ir =
      fIRGenerator->convertProgram(baseModule, /*isBuiltinCode=*/true, *source);
...
std::unique_ptr<Program> Compiler::convertProgram(
    ProgramKind kind, String text, Program::Settings settings) {
  ...
  IRGenerator::IRBundle ir =
      fIRGenerator->convertProgram(baseModule, /*isBuiltinCode=*/false, *textPtr);
  ...
  auto program = std::make_unique<Program>(
      std::move(textPtr), std::move(dsl::DSLWriter::GetProgramConfig()), fContext,
      std::move(ir.fElements), std::move(ir.fSharedElements),
      std::move(dsl::DSLWriter::GetModifiersPool()), std::move(ir.fSymbolTable),
      std::move(dsl::DSLWriter::MemoryPool()), ir.fInputs);
