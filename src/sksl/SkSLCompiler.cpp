...
#  define MODULE_DATA(name) \
    MakeModuleData(SKSL_INCLUDE_sksl_##name, SKSL_INCLUDE_sksl_##name##_LENGTH)
...
#undef TYPE
  ...
  fPrivateSymbolTable->add(std::make_unique<Variable>(
      /*offset=*/-1, fIRGenerator->fModifiers->addToPool(Modifiers()), "sk_Caps",
...
const ParsedModule& Compiler::loadFragmentModule() {
  if (!fFragmentModule.fSymbols) {
    fFragmentModule =
        this->parseModule(Program::kFragment_Kind, MODULE_DATA(frag), this->loadGPUModule());
  }
...
const ParsedModule& Compiler::loadVertexModule() {
  if (!fVertexModule.fSymbols) {
    fVertexModule =
        this->parseModule(Program::kVertex_Kind, MODULE_DATA(vert), this->loadGPUModule());
  }
...
const ParsedModule& Compiler::loadGeometryModule() {
  if (!fGeometryModule.fSymbols) {
    fGeometryModule =
        this->parseModule(Program::kGeometry_Kind, MODULE_DATA(geom), this->loadGPUModule());
  }
...
const ParsedModule& Compiler::loadFPModule() {
  if (!fFPModule.fSymbols) {
    fFPModule =
        this->parseModule(Program::kFragmentProcessor_Kind, MODULE_DATA(fp), this->loadGPUModule());
  }
  return fFPModule;
}
...
const ParsedModule& Compiler::loadRuntimeEffectModule() {
  if (!fRuntimeEffectModule.fSymbols) {
    fRuntimeEffectModule = this->parseModule(
        Program::kRuntimeEffect_Kind, MODULE_DATA(runtime), this->loadPublicModule());
...
const ParsedModule& Compiler::loadInterpreterModule() {
  if (!fInterpreterModule.fSymbols) {
    fInterpreterModule =
        this->parseModule(Program::kGeneric_Kind, MODULE_DATA(interp), this->loadPublicModule());
  }
...
LoadedModule Compiler::loadModule(
    Program::Kind kind, ModuleData data, std::shared_ptr<SymbolTable> base) {
  if (!base) {
  ...
  Rehydrator rehydrator(
      fContext.get(), fIRGenerator->fModifiers.get(), base, this, data.fData, data.fSize);
...
void Compiler::addDefinition(
  ...
  switch (lvalue->kind()) {
    ...
    case Expression::Kind::kTernary:
      ...
      this->addDefinition(
          lvalue->as<TernaryExpression>().ifTrue().get(),
          (std::unique_ptr<Expression>*)&fContext->fDefined_Expression, definitions);
      this->addDefinition(
          lvalue->as<TernaryExpression>().ifFalse().get(),
          (std::unique_ptr<Expression>*)&fContext->fDefined_Expression, definitions);
      break;
    default:
    ...
    switch (expr->kind()) {
      case Expression::Kind::kBinary: {
        ...
        if (b->getOperator() == Token::Kind::TK_EQ) {
          ...
          this->addDefinition(
              b->left().get(), (std::unique_ptr<Expression>*)&fContext->fDefined_Expression,
              definitions);
        }
        break;
      }
      ...
      case Expression::Kind::kPrefix: {
        ...
        if (p->getOperator() == Token::Kind::TK_MINUSMINUS ||
          ...
          this->addDefinition(
              p->operand().get(), (std::unique_ptr<Expression>*)&fContext->fDefined_Expression,
              definitions);
        }
        break;
      }
...
static bool is_dead(const Expression& lvalue, ProgramUsage* usage) {
  switch (lvalue.kind()) {
    ...
    case Expression::Kind::kSwizzle: return is_dead(*lvalue.as<Swizzle>().base(), usage);
    case Expression::Kind::kFieldAccess: return is_dead(*lvalue.as<FieldAccess>().base(), usage);
    case Expression::Kind::kIndex: {
    ...
    case Expression::Kind::kTernary: {
      ...
      return !t.test()->hasSideEffects() && is_dead(*t.ifTrue(), usage) &&
...
static bool is_constant(const Expression& expr, T value) {
  switch (expr.kind()) {
    ...
    case Expression::Kind::kFloatLiteral: return expr.as<FloatLiteral>().value() == value;
...
static void clear_write(Expression& expr) {
  switch (expr.kind()) {
    ...
    case Expression::Kind::kFieldAccess: clear_write(*expr.as<FieldAccess>().base()); break;
    case Expression::Kind::kSwizzle: clear_write(*expr.as<Swizzle>().base()); break;
    case Expression::Kind::kIndex: clear_write(*expr.as<IndexExpression>().base()); break;
  ...
  switch (expr->kind()) {
    case Expression::Kind::kVariableReference: {
      ...
      if (ref.refKind() != VariableReference::RefKind::kWrite &&
        ...
        this->error(expr->fOffset, "'" + var->name() + "' has not been assigned");
      }
      break;
    }
    case Expression::Kind::kTernary: {
      ...
      switch (bin->getOperator()) {
        case Token::Kind::TK_STAR:
          if (is_constant(left, 1)) {
            ...
            if (leftType.isScalar() && rightType.isVector() && !right.hasSideEffects()) {
              // 0 * float4(x) -> float4(0)
    ...
    case Expression::Kind::kSwizzle: {
      ...
      if (s.base()->is<Swizzle>()) {
        ...
        std::unique_ptr<Expression> replacement(
            new Swizzle(*fContext, base.base()->clone(), final));
      ...
      if (s.base()->is<Constructor>()) {
        ...
        SkASSERT(std::all_of(s.components().begin(), s.components().end(), [](int8_t c) {
          return c >= 0 && c <= 3;
        }));
        ...
        if (base.arguments().size() == 1 && base.arguments().front()->type().isScalar()) {
          ...
          const Type& constructorType = componentType.toCompound(
              *fContext, swizzleSize,
              /*rows=*/1);
          ...
          if (!replacement) {
            ...
            replacement =
                std::make_unique<Constructor>(base.fOffset, &constructorType, std::move(newArgs));
          }
        ...
        if (safeToOptimize) {
          ...
          for (int c : s.components()) {
            ...
            if (baseArg.type().isScalar()) {
              ...
              reorderedArgs.push_back({argument.fArgIndex, ComponentArray{}});
            } else {
              // This argument is a component from a vector.
              SkASSERT(argument.fComponent < baseArg.type().columns());
              if (reorderedArgs.empty() || reorderedArgs.back().fArgIndex != argument.fArgIndex) {
                // This can't be combined with the previous argument. Add a new one.
                reorderedArgs.push_back({argument.fArgIndex, ComponentArray{argument.fComponent}});
              } else {
          ...
          for (const ReorderedArgument& reorderedArg : reorderedArgs) {
            ...
            if (reorderedArg.fComponents.empty()) {
              ...
              newArgs.push_back(
                  std::make_unique<Swizzle>(*fContext, baseArg.clone(), reorderedArg.fComponents));
            }
          }

          // Create a new constructor.
          replacement = std::make_unique<Constructor>(
              base.fOffset, &componentType.toCompound(*fContext, swizzleSize, /*rows=*/1),
              std::move(newArgs));
  ...
  switch (stmt->kind()) {
    case Statement::Kind::kVarDeclaration: {
      ...
      if (usage->isDead(varDecl.var()) &&
          (!varDecl.value() || !varDecl.value()->hasSideEffects())) {
        if (varDecl.value()) {
    ...
    case Statement::Kind::kIf: {
      ...
      if (!i.ifFalse() && i.ifTrue()->isEmpty()) {
        ...
        if (i.test()->hasSideEffects()) {
          ...
          (*iter)->setStatement(std::make_unique<ExpressionStatement>(std::move(i.test())), usage);
        } else {
          ...
          if (caseValue == switchValue) {
            ...
            if (newBlock) {
              ...
              if (s.isStatic() && !(fFlags & kPermitInvalidStaticTests_Flag) &&
                  optimizationContext->fSilences.find(&s) == optimizationContext->fSilences.end()) {
...
bool Compiler::scanCFG(FunctionDefinition& f, ProgramUsage* usage) {
  ...
  do {
    ...
    for (BlockId blockId = 0; blockId < cfg.fBlocks.size(); ++blockId) {
      ...
      for (auto iter = b.fNodes.begin();
           iter != b.fNodes.end() && !optimizationContext.fNeedsRescan; ++iter) {
  ...
  for (BasicBlock& b : cfg.fBlocks) {
    for (auto iter = b.fNodes.begin();
         iter != b.fNodes.end() && !optimizationContext.fNeedsRescan;) {
      if (iter->isStatement()) {
        const Statement& s = **iter->statement();
        switch (s.kind()) {
          case Statement::Kind::kIf:
            if (s.as<IfStatement>().isStatic() && !(fFlags & kPermitInvalidStaticTests_Flag)) {
              this->error(s.fOffset, "static if has non-static test");
            }
            ++iter;
            break;
          case Statement::Kind::kSwitch:
            if (s.as<SwitchStatement>().isStatic() && !(fFlags & kPermitInvalidStaticTests_Flag) &&
                optimizationContext.fSilences.find(&s) == optimizationContext.fSilences.end()) {
  ...
  if (f.declaration().returnType() != *fContext->fTypes.fVoid) {
    if (cfg.fBlocks[cfg.fExit].fIsReachable) {
      this->error(
          f.fOffset, String(
                         "function '" + String(f.declaration().name()) +
...
std::unique_ptr<Program> Compiler::convertProgram(
  ...
  IRGenerator::IRBundle ir = fIRGenerator->convertProgram(
      kind, &settings, baseModule, /*isBuiltinCode=*/false, textPtr->c_str(), textPtr->size(),
      externalFunctions);
  auto program = std::make_unique<Program>(
      kind, std::move(textPtr), settings, fCaps, fContext, std::move(ir.fElements),
      std::move(ir.fSharedElements), std::move(ir.fModifiers), std::move(ir.fSymbolTable),
      std::move(pool), ir.fInputs);
  bool success = false;
  if (fErrorCount) {
  ...
  while (fErrorCount == 0) {
    ...
    if (program.fSettings.fRemoveDeadFunctions) {
      ...
      program.fSharedElements.erase(
          std::remove_if(
              program.fSharedElements.begin(), program.fSharedElements.end(), isDeadFunction),
    ...
    if (program.fKind != Program::kFragmentProcessor_Kind) {
      ...
      program.fSharedElements.erase(
          std::remove_if(
              program.fSharedElements.begin(), program.fSharedElements.end(), isDeadVariable),
          program.fSharedElements.end());
    }
...
bool Compiler::IsAssignment(Token::Kind op) {
  switch (op) {
    ...
    case Token::Kind::TK_BITWISEANDEQ: return true;
    default: return false;
  }
}
