...
static void grab_intrinsics(
    std::vector<std::unique_ptr<ProgramElement>>* src,
  ...
  for (auto iter = src->begin(); iter != src->end();) {
    ...
    switch (element->fKind) {
      ...
      default: printf("unsupported include file element\n"); SkASSERT(false);
    }
  }
}

Compiler::Compiler(Flags flags) : fFlags(flags), fContext(new Context()), fErrorCount(0) {
  auto symbols = std::shared_ptr<SymbolTable>(new SymbolTable(this));
  fIRGenerator = new IRGenerator(fContext.get(), symbols, *this);
#define ADD_TYPE(t) \
  symbols->addWithoutOwnership(fContext->f##t##_Type->fName, fContext->f##t##_Type.get())
  ADD_TYPE(Void);
  ADD_TYPE(Float);
  ADD_TYPE(Float2);
  ADD_TYPE(Float3);
  ...
  fIRGenerator->fSymbolTable->add(
      skCapsName, std::make_unique<Variable>(
                      /*offset=*/-1, Modifiers(), skCapsName, *fContext->fSkCaps_Type,
  ...
  this->processIncludeFile(
      Program::kFragment_Kind, SKSL_GPU_INCLUDE, symbols, &gpuIntrinsics, &fGpuSymbolTable);
  this->processIncludeFile(
...
#else
  {
    Rehydrator rehydrator(
        fContext.get(), symbols, this, SKSL_INCLUDE_sksl_gpu, SKSL_INCLUDE_sksl_gpu_LENGTH);
...
Compiler::~Compiler() { delete fIRGenerator; }
...
void Compiler::loadInterpreterIntrinsics() {
  ...
  {
    Rehydrator rehydrator(
        fContext.get(), fPipelineSymbolTable, this, SKSL_INCLUDE_sksl_interp,
...
#else
  this->processIncludeFile(
      Program::kGeneric_Kind, SKSL_INTERP_INCLUDE, fIRGenerator->fSymbolTable, &fInterpreterInclude,
  ...
  switch (lvalue->fKind) {
    ...
    case Expression::kTernary_Kind:
      ...
      this->addDefinition(
          ...
          (std::unique_ptr<Expression>*)&fContext->fDefined_Expression, definitions);
      break;
    case Expression::kExternalValue_Kind: break;
    default:
      ...
      switch (expr->fKind) {
        case Expression::kBinary_Kind: {
          ...
          if (b->fOperator == Token::Kind::TK_EQ) {
            ...
            this->addDefinition(
                b->fLeft.get(), (std::unique_ptr<Expression>*)&fContext->fDefined_Expression,
                definitions);
          }
          break;
        }
...
static bool is_dead(const Expression& lvalue) {
  switch (lvalue.fKind) {
    ...
    case Expression::kSwizzle_Kind: return is_dead(*lvalue.as<Swizzle>().fBase);
    case Expression::kFieldAccess_Kind: return is_dead(*lvalue.as<FieldAccess>().fBase);
    case Expression::kIndex_Kind: {
    ...
    case Expression::kExternalValue_Kind: return false;
    default:
#ifdef SK_DEBUG
...
static bool is_constant(const Expression& expr, T value) {
  switch (expr.fKind) {
    case Expression::kIntLiteral_Kind: return expr.as<IntLiteral>().fValue == value;

    case Expression::kFloatLiteral_Kind: return expr.as<FloatLiteral>().fValue == value;

    case Expression::kConstructor_Kind: {
      const Constructor& constructor = expr.as<Constructor>();
      if (constructor.isCompileTimeConstant()) {
        bool isFloat = constructor.fType.columns() > 1 ? constructor.fType.componentType().isFloat()
...
static void clear_write(Expression& expr) {
  switch (expr.fKind) {
    ...
    case Expression::kFieldAccess_Kind: clear_write(*expr.as<FieldAccess>().fBase); break;
    case Expression::kSwizzle_Kind: clear_write(*expr.as<Swizzle>().fBase); break;
    case Expression::kIndex_Kind: clear_write(*expr.as<IndexExpression>().fBase); break;
      ...
      switch (bin->fOperator) {
        case Token::Kind::TK_STAR:
          if (is_constant(*bin->fLeft, 1)) {
            ...
            if (bin->fLeft->fType.kind() == Type::kScalar_Kind &&
                bin->fRight->fType.kind() == Type::kVector_Kind && !bin->fRight->hasSideEffects()) {
              // 0 * float4(x) -> float4(0)
            ...
            if (bin->fLeft->fType.kind() == Type::kVector_Kind &&
                bin->fRight->fType.kind() == Type::kScalar_Kind && !bin->fLeft->hasSideEffects()) {
              // float4(x) * 0 -> float4(0)
              vectorize_right(&b, iter, outUpdated, outNeedsRescan);
            } else {
              // x * 0 -> 0
...
static bool contains_conditional_break_impl(Statement& s, bool inConditional) {
  switch (s.fKind) {
    ...
    case Statement::kBreak_Kind: return inConditional;

    case Statement::kIf_Kind: {
      const IfStatement& i = s.as<IfStatement>();
      return contains_conditional_break_impl(*i.fIfTrue, /*inConditional=*/true) ||
             (i.fIfFalse && contains_conditional_break_impl(*i.fIfFalse, /*inConditional=*/true));
    }
...
static bool contains_unconditional_break(Statement& s) {
  switch (s.fKind) {
    ...
    case Statement::kBreak_Kind: return true;
  ...
  switch (stmt->fKind) {
    case Statement::kBlock_Kind: {
      ...
      target->push_back(std::make_unique<Block>(
          block.fOffset, std::move(blockStmts), block.fSymbols, block.fIsScope));
      break;
    }
...
void Compiler::scanCFG(FunctionDefinition& f) {
  ...
  for (size_t i = 0; i < cfg.fBlocks.size(); i++) {
    if (i != cfg.fStart && !cfg.fBlocks[i].fEntrances.size() && cfg.fBlocks[i].fNodes.size()) {
      int offset;
      switch (cfg.fBlocks[i].fNodes[0].fKind) {
        ...
        case BasicBlock::Node::kExpression_Kind:
          ...
          if ((*cfg.fBlocks[i].fNodes[0].expression())->fKind == Expression::kBoolLiteral_Kind) {
...
std::unique_ptr<Program> Compiler::convertProgram(
  ...
  switch (kind) {
    ...
    case Program::kFragmentProcessor_Kind: {
      ...
      const String* source =
          fGpuSymbolTable->takeOwnershipOfString(std::make_unique<String>(stdText.c_str()));
