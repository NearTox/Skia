...
static void grab_intrinsics(
    std::vector<std::unique_ptr<ProgramElement>>* src, IRIntrinsicMap* target) {
...
Compiler::Compiler(Flags flags)
  ...
  fIRGenerator = std::make_unique<IRGenerator>(fContext.get(), &fInliner, fRootSymbolTable, *this);
#define ADD_TYPE(t) \
  fRootSymbolTable->addWithoutOwnership(fContext->f##t##_Type->fName, fContext->f##t##_Type.get())
  ADD_TYPE(Void);
  ADD_TYPE(Float);
  ADD_TYPE(Float2);
  ADD_TYPE(Float3);
  ...
  fRootSymbolTable->add(
      skCapsName, std::make_unique<Variable>(
                      /*offset=*/-1, Modifiers(), skCapsName, fContext->fSkCaps_Type.get(),
  ...
  this->processIncludeFile(
      Program::kFragment_Kind, SKSL_GPU_INCLUDE, fRootSymbolTable, &gpuIntrinsics,
      &fGpuSymbolTable);
  this->processIncludeFile(
      Program::kVertex_Kind, SKSL_VERT_INCLUDE, fGpuSymbolTable, &fVertexInclude,
      &fVertexSymbolTable);
  this->processIncludeFile(
      Program::kFragment_Kind, SKSL_FRAG_INCLUDE, fGpuSymbolTable, &fFragmentInclude,
      &fFragmentSymbolTable);
#else
  {
    Rehydrator rehydrator(
        fContext.get(), fRootSymbolTable, this, SKSL_INCLUDE_sksl_gpu,
  ...
  {
    Rehydrator rehydrator(
        fContext.get(), fGpuSymbolTable, this, SKSL_INCLUDE_sksl_vert,
  ...
  {
    Rehydrator rehydrator(
        fContext.get(), fGpuSymbolTable, this, SKSL_INCLUDE_sksl_frag,
...
void Compiler::loadGeometryIntrinsics() {
  ...
  {
    Rehydrator rehydrator(
        fContext.get(), fGpuSymbolTable, this, SKSL_INCLUDE_sksl_geom,
...
#else
  this->processIncludeFile(
      Program::kGeometry_Kind, SKSL_GEOM_INCLUDE, fGpuSymbolTable, &fGeometryInclude,
...
void Compiler::loadPipelineIntrinsics() {
  ...
  {
    Rehydrator rehydrator(
        fContext.get(), fGpuSymbolTable, this, SKSL_INCLUDE_sksl_pipeline,
...
#else
  this->processIncludeFile(
      Program::kPipelineStage_Kind, SKSL_PIPELINE_INCLUDE, fGpuSymbolTable, &fPipelineInclude,
...
#else
  this->processIncludeFile(
      Program::kGeneric_Kind, SKSL_INTERP_INCLUDE, fIRGenerator->fSymbolTable, &interpIntrinsics,
...
void Compiler::processIncludeFile(
  ...
  std::unique_ptr<String> text = std::make_unique<String>(
...
void Compiler::addDefinition(
  ...
  switch (lvalue->kind()) {
    ...
    case Expression::Kind::kTernary:
      ...
      this->addDefinition(
          lvalue->as<TernaryExpression>().fIfTrue.get(),
          (std::unique_ptr<Expression>*)&fContext->fDefined_Expression, definitions);
      this->addDefinition(
          lvalue->as<TernaryExpression>().fIfFalse.get(),
          (std::unique_ptr<Expression>*)&fContext->fDefined_Expression, definitions);
      break;
    case Expression::Kind::kExternalValue: break;
    default:
    ...
    switch (expr->kind()) {
      case Expression::Kind::kBinary: {
        ...
        if (b->getOperator() == Token::Kind::TK_EQ) {
          ...
          this->addDefinition(
              &b->left(), (std::unique_ptr<Expression>*)&fContext->fDefined_Expression,
              definitions);
        }
        break;
      }
      case Expression::Kind::kFunctionCall: {
        ...
        for (size_t i = 0; i < c.fFunction.fParameters.size(); ++i) {
          if (c.fFunction.fParameters[i]->fModifiers.fFlags & Modifiers::kOut_Flag) {
            this->addDefinition(
                c.fArguments[i].get(), (std::unique_ptr<Expression>*)&fContext->fDefined_Expression,
                definitions);
          }
        }
        break;
      ...
      case Expression::Kind::kPrefix: {
        ...
        if (p->fOperator == Token::Kind::TK_MINUSMINUS ||
          ...
          this->addDefinition(
              p->fOperand.get(), (std::unique_ptr<Expression>*)&fContext->fDefined_Expression,
              definitions);
        }
        break;
      }
      ...
      case Expression::Kind::kVariableReference: {
        ...
        if (v->fRefKind != VariableReference::kRead_RefKind) {
          this->addDefinition(
              v, (std::unique_ptr<Expression>*)&fContext->fDefined_Expression, definitions);
        }
        break;
      }
      default: break;
    ...
    for (const auto& pair : after) {
      ...
      if (found == exit.fBefore.end()) {
        ...
        if (e1 != e2) {
          ...
          workList->insert(exitId);
          if (e1 && e2) {
            exit.fBefore[pair.first] = (std::unique_ptr<Expression>*)&fContext->fDefined_Expression;
          } else {
...
static bool is_dead(const Expression& lvalue) {
  switch (lvalue.kind()) {
    ...
    case Expression::Kind::kSwizzle: return is_dead(*lvalue.as<Swizzle>().fBase);
    case Expression::Kind::kFieldAccess: return is_dead(*lvalue.as<FieldAccess>().fBase);
    case Expression::Kind::kIndex: {
      const IndexExpression& idx = lvalue.as<IndexExpression>();
      return is_dead(*idx.fBase) && !idx.fIndex->hasProperty(Expression::Property::kSideEffects);
    }
    case Expression::Kind::kTernary: {
      const TernaryExpression& t = lvalue.as<TernaryExpression>();
      return !t.fTest->hasSideEffects() && is_dead(*t.fIfTrue) && is_dead(*t.fIfFalse);
    }
    case Expression::Kind::kExternalValue: return false;
    default:
#ifdef SK_DEBUG
 ...
 */
static bool try_replace_expression(
    BasicBlock* b, std::vector<BasicBlock::Node>::iterator* iter,
...
static bool is_constant(const Expression& expr, T value) {
  switch (expr.kind()) {
    case Expression::Kind::kIntLiteral: return expr.as<IntLiteral>().value() == value;

    case Expression::Kind::kFloatLiteral: return expr.as<FloatLiteral>().fValue == value;
    ...
    case Expression::Kind::kConstructor: {
      ...
      if (constructor.isCompileTimeConstant()) {
        ...
        bool isFloat = constructorType.columns() > 1 ? constructorType.componentType().isFloat()
...
static void delete_left(
    BasicBlock* b, std::vector<BasicBlock::Node>::iterator* iter, bool* outUpdated,
    bool* outNeedsRescan) {
  *outUpdated = true;
  std::unique_ptr<Expression>* target = (*iter)->expression();
...
static void vectorize(
    BasicBlock* b, std::vector<BasicBlock::Node>::iterator* iter, const Type& type,
    std::unique_ptr<Expression>* otherExpression, bool* outUpdated, bool* outNeedsRescan) {
...
static void clear_write(Expression& expr) {
  switch (expr.kind()) {
    ...
    case Expression::Kind::kFieldAccess: clear_write(*expr.as<FieldAccess>().fBase); break;
    case Expression::Kind::kSwizzle: clear_write(*expr.as<Swizzle>().fBase); break;
    case Expression::Kind::kIndex: clear_write(*expr.as<IndexExpression>().fBase); break;
    default: ABORT("shouldn't be writing to this kind of expression\n"); break;
  }
}

void Compiler::simplifyExpression(
      ...
      switch (bin->getOperator()) {
        case Token::Kind::TK_STAR:
          if (is_constant(left, 1)) {
            ...
            if (leftType.typeKind() == Type::TypeKind::kScalar &&
                rightType.typeKind() == Type::TypeKind::kVector && !right.hasSideEffects()) {
              // 0 * float4(x) -> float4(0)
            ...
            if (leftType.typeKind() == Type::TypeKind::kVector &&
                rightType.typeKind() == Type::TypeKind::kScalar && !left.hasSideEffects()) {
              // float4(x) * 0 -> float4(0)
              vectorize_right(&b, iter, outUpdated, outNeedsRescan);
            } else {
              // x * 0 -> 0
  ...
  class ContainsConditionalBreak : public ProgramVisitor {
   public:
    bool visitStatement(const Statement& stmt) override {
      switch (stmt.kind()) {
        case Statement::Kind::kBlock: return this->INHERITED::visitStatement(stmt);

        case Statement::Kind::kBreak: return fInConditional > 0;
   ...
   public:
    bool visitStatement(const Statement& stmt) override {
      switch (stmt.kind()) {
        ...
        case Statement::Kind::kBreak: return true;
...
static void move_all_but_break(
    std::unique_ptr<Statement>& stmt, std::vector<std::unique_ptr<Statement>>* target) {
  switch (stmt->kind()) {
    case Statement::Kind::kBlock: {
      ...
      target->push_back(std::make_unique<Block>(
          block.fOffset, std::move(blockStmts), block.symbolTable(), block.isScope()));
      break;
    }
...
static std::unique_ptr<Statement> block_for_case(
    SwitchStatement* switchStatement, SwitchCase* caseToCapture) {
...
void Compiler::simplifyStatement(
    DefinitionMap& definitions, BasicBlock& b, std::vector<BasicBlock::Node>::iterator* iter,
    std::unordered_set<const Variable*>* undefinedVariables, bool* outUpdated,
    bool* outNeedsRescan) {
  ...
  switch (stmt->kind()) {
    case Statement::Kind::kVarDeclaration: {
      ...
      if (varDecl.fVar->dead() && (!varDecl.fValue || !varDecl.fValue->hasSideEffects())) {
        if (varDecl.fValue) {
    ...
    case Statement::Kind::kIf: {
      ...
      if (!i.fIfFalse && i.fIfTrue->isEmpty()) {
        ...
        if (i.fTest->hasSideEffects()) {
          ...
          (*iter)->setStatement(
              std::unique_ptr<Statement>(new ExpressionStatement(std::move(i.fTest))));
        } else {
          ...
          if (caseValue == switchValue) {
            ...
            if (newBlock) {
              ...
              if (s.fIsStatic && !(fFlags & kPermitInvalidStaticTests_Flag)) {
                this->error(s.fOffset, "static switch contains non-static conditional break");
                s.fIsStatic = false;
              }
...
bool Compiler::scanCFG(FunctionDefinition& f) {
  ...
  do {
    ...
    for (BasicBlock& b : cfg.fBlocks) {
      if (!first && b.fEntrances.empty()) {
        ...
        b.fNodes.erase(
            std::remove_if(
                b.fNodes.begin(), b.fNodes.end(),
                [](const BasicBlock::Node& node) {
                  return node.isStatement() && (*node.statement())->is<VarDeclaration>();
                }),
            b.fNodes.end());
      ...
      for (auto iter = b.fNodes.begin(); iter != b.fNodes.end() && !needsRescan; ++iter) {
        if (iter->isExpression()) {
          this->simplifyExpression(
              definitions, b, &iter, &undefinedVariables, &updated, &needsRescan);
        } else {
          this->simplifyStatement(
              definitions, b, &iter, &undefinedVariables, &updated, &needsRescan);
        }
        if (needsRescan) {
          break;
  ...
  for (BasicBlock& b : cfg.fBlocks) {
    ...
    for (auto iter = b.fNodes.begin(); iter != b.fNodes.end() && !needsRescan;) {
      if (iter->isStatement()) {
        ...
        switch (s.kind()) {
          case Statement::Kind::kIf:
            if (s.as<IfStatement>().fIsStatic && !(fFlags & kPermitInvalidStaticTests_Flag)) {
              this->error(s.fOffset, "static if has non-static test");
            }
            ++iter;
            break;
          case Statement::Kind::kSwitch:
            if (s.as<SwitchStatement>().fIsStatic && !(fFlags & kPermitInvalidStaticTests_Flag)) {
          ...
          case Statement::Kind::kVarDeclarations: {
            ...
            decls.fVars.erase(
                std::remove_if(
                    decls.fVars.begin(), decls.fVars.end(),
  ...
  if (f.fDeclaration.fReturnType != *fContext->fVoid_Type) {
    if (cfg.fBlocks[cfg.fExit].fEntrances.size()) {
      this->error(
          f.fOffset, String(
                         "function '" + String(f.fDeclaration.fName) +
...
std::unique_ptr<Program> Compiler::convertProgram(
  ...
  switch (kind) {
    ...
    case Program::kFragmentProcessor_Kind: {
      ...
      {
        Rehydrator rehydrator(
            fContext.get(), fGpuSymbolTable, this, SKSL_INCLUDE_sksl_fp,
      ...
      inherited = nullptr;
      fIRGenerator->start(
          &settings, fGpuSymbolTable, /*inherited=*/nullptr,
          /*builtin=*/true);
      fIRGenerator->fIntrinsics = fGPUIntrinsics.get();
      std::ifstream in(SKSL_FP_INCLUDE);
      std::string stdText{std::istreambuf_iterator<char>(in), std::istreambuf_iterator<char>()};
      if (in.rdstate()) {
        printf("error reading %s\n", SKSL_FP_INCLUDE);
        abort();
      }
      const String* source =
          fRootSymbolTable->takeOwnershipOfString(std::make_unique<String>(stdText.c_str()));
  ...
  auto result = std::make_unique<Program>(
      kind, std::move(textPtr), settings, fContext, inherited, std::move(elements),
      fIRGenerator->fSymbolTable, fIRGenerator->fInputs);
  fIRGenerator->finish();
  if (fErrorCount) {
    return nullptr;
  }
  ...
  while (fErrorCount == 0) {
    ...
    if (program.fSettings.fRemoveDeadFunctions) {
      program.fElements.erase(
          std::remove_if(
              program.fElements.begin(), program.fElements.end(),
              [&](const std::unique_ptr<ProgramElement>& element) {
                if (!element->is<FunctionDefinition>()) {
                  return false;
                }
                const auto& fn = element->as<FunctionDefinition>();
                bool dead = fn.fDeclaration.fCallCount == 0 && fn.fDeclaration.fName != "main";
                madeChanges |= dead;
                return dead;
              }),
    ...
    if (program.fKind != Program::kFragmentProcessor_Kind) {
      ...
      for (ProgramElement& element : program) {
        ...
        vars.fVars.erase(
            std::remove_if(
                vars.fVars.begin(), vars.fVars.end(),
...
const char* Compiler::OperatorName(Token::Kind op) {
  switch (op) {
    ...
    default: ABORT("unsupported operator: %d\n", (int)op);
  }
}
...
bool Compiler::IsAssignment(Token::Kind op) {
  switch (op) {
    ...
    case Token::Kind::TK_LOGICALANDEQ: return true;
    default: return false;
  }
}
