...
bool Operator::isOperator() const {
  switch (this->kind()) {
    ...
    case Kind::TK_COMMA: return true;
...
bool Operator::isAssignment() const {
  switch (this->kind()) {
    ...
    case Kind::TK_BITWISEXOREQ:  // fall through
    case Kind::TK_BITWISEANDEQ: return true;
    default: return false;
  }
}
 ...
 */
bool Operator::determineBinaryType(
    const Context& context, const Type& left, const Type& right, const Type** outLeftType,
    const Type** outRightType, const Type** outResultType) {
  ...
  if (this->isMatrixMultiply(left, right)) {  // left * right
    ...
    if (!this->determineBinaryType(
            context, left.componentType(), right.componentType(), outLeftType, outRightType,
            outResultType)) {
      return false;
    }
  ...
  if (leftIsVectorOrMatrix && validMatrixOrVectorOp && right.isScalar()) {
    // Determine final component type.
    if (!this->determineBinaryType(
            context, left.componentType(), right, outLeftType, outRightType, outResultType)) {
      return false;
    }
    // Convert component type to compound.
  ...
  if (!isAssignment && rightIsVectorOrMatrix && validMatrixOrVectorOp && left.isScalar()) {
    // Determine final component type.
    if (!this->determineBinaryType(
            context, left, right.componentType(), outLeftType, outRightType, outResultType)) {
      return false;
    }
    // Convert component type to compound.
