...
static int slot_count(const Type& type) { return type.columns() * type.rows(); }
...
void ByteCodeGenerator::align(int divisor, int remainder) {
    switch (remainder - (int)fCode->size() % divisor) {
        case 0:
            return;
        case 3:
            this->write(ByteCodeInstruction::kNop3);  // fall through
        case 2:
            this->write(ByteCodeInstruction::kNop2);  // fall through
        case 1:
            this->write(ByteCodeInstruction::kNop1);
            break;
        default:
            SkASSERT(false);
    }
...
void ByteCodeGenerator::write(ByteCodeInstruction i) { this->write8((uint8_t)i); }
    ...
    switch (op) {
        ...
        case Token::Kind::PLUS:
            this->writeTypedInstruction(b.fLeft->fType, ByteCodeInstruction::kAddI,
                                        ByteCodeInstruction::kAddI, ByteCodeInstruction::kAddF);
            break;
    ...
    switch (e.fKind) {
        ...
        case Expression::kVariableReference_Kind:
            return std::unique_ptr<LValue>(
                    new ByteCodeVariableLValue(this, ((VariableReference&)e).fVariable));
        case Expression::kSwizzle_Kind:
