...
static TypeCategory type_category(const Type& type) {
  switch (type.typeKind()) {
    ...
    default:
      ...
      if (name == "int" || name == "short" || name == "$intLiteral") {
        return TypeCategory::kSigned;
      }
      if (name == "uint" || name == "ushort") {
...
#undef VEC_UNARY
    ...
    case ByteCodeInstruction::kStoreGlobal: return -count;
...
void ByteCodeGenerator::writeSmoothstep(const ExpressionArray& args) {
  ...
  int edgeCount = SlotCount(args[0]->type()), xCount = SlotCount(args[2]->type());
  ...
  const Variable* edge0Var = fSynthetics.takeOwnershipOfSymbol(std::make_unique<Variable>(
      /*offset=*/-1, fProgram.fModifiers->addToPool(Modifiers()), "sksl_smoothstep_edge0",
      &args[0]->type(),
      /*builtin=*/true, Variable::Storage::kLocal));
...
void ByteCodeGenerator::writeIntrinsicCall(const FunctionCall& c) {
  ...
  if (intrin.is_special && intrin.special == SpecialIntrinsic::kSample) {
    ...
    if (nargs == 2) {
      ...
      this->write(
          args[1]->type() == *fContext.fTypes.fFloat3x3 ? ByteCodeInstruction::kSampleMatrix
  ...
  if (intrin.is_special) {
    ...
    switch (intrin.special) {
      ...
      case SpecialIntrinsic::kATan: {
        ...
        this->write(nargs == 1 ? ByteCodeInstruction::kATan : ByteCodeInstruction::kATan2, count);
      } break;
