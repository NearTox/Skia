...
ByteCodeGenerator::ByteCodeGenerator(
    const Program* program, ErrorReporter* errors, ByteCode* output)
    : INHERITED(program, errors, nullptr),
      fOutput(output),
      fIntrinsics{
          {"cos", ByteCode::Instruction::kCos},    {"dot", SpecialIntrinsic::kDot},
          {"inverse", SpecialIntrinsic::kInverse}, {"print", ByteCode::Instruction::kPrint},
          {"sin", ByteCode::Instruction::kSin},    {"sqrt", ByteCode::Instruction::kSqrt},
...
ByteCodeGenerator::Location ByteCodeGenerator::getLocation(const Variable& var) {
  ...
  switch (var.fStorage) {
    ...
    default: SkASSERT(false); return ByteCode::Pointer{0};
  }
}
...
Variable::Storage ByteCodeGenerator::getStorage(const Expression& expr) {
  switch (expr.fKind) {
    ...
    default: SkASSERT(false); return Variable::kLocal_Storage;
  }
}

ByteCode::Instruction ByteCodeGenerator::getLoadInstruction(
    ByteCodeGenerator::Location location, Variable::Storage storage) {
  switch (storage) {
    case Variable::kGlobal_Storage:
...
ByteCode::Instruction ByteCodeGenerator::getStoreInstruction(
    ByteCodeGenerator::Location location, Variable::Storage storage) {
  switch (storage) {
    case Variable::kGlobal_Storage:
      switch (location.fKind) {
...
class ByteCodeSimpleLValue : public ByteCodeGenerator::LValue {
 public:
  ByteCodeSimpleLValue(
      ByteCodeGenerator* generator, ByteCodeGenerator::Location location, int count,
      ByteCode::Instruction load, ByteCode::Instruction store)
      : INHERITED(*generator),
        fLocation(location),
...
class ByteCodeSwizzleLValue : public ByteCodeGenerator::LValue {
 public:
  ...
  void load(ByteCode::Register result) override { fGenerator.writeSwizzle(fSwizzle, result); }

  void store(ByteCode::Register src) override {
    ByteCodeGenerator::Location target = fGenerator.getLocation(*fSwizzle.fBase);
    ByteCode::Instruction inst =
        fGenerator.getStoreInstruction(target, fGenerator.getStorage(*fSwizzle.fBase));
...
std::unique_ptr<ByteCodeGenerator::LValue> ByteCodeGenerator::getLValue(const Expression& expr) {
  switch (expr.fKind) {
    ...
    case Expression::kVariableReference_Kind: {
      ...
      return std::unique_ptr<LValue>(
          new ByteCodeSimpleLValue(this, location, SlotCount(expr.fType), loadInst, storeInst));
    }
    case Expression::kSwizzle_Kind:
...
void ByteCodeGenerator::writeTypedInstruction(
    const Type& type, ByteCode::Instruction s, ByteCode::Instruction u, ByteCode::Instruction f) {
  switch (type_category(type)) {
    case TypeCategory::kSigned: this->write(s); break;
    case TypeCategory::kUnsigned: this->write(u); break;
...
void ByteCodeGenerator::writeVectorBinaryInstruction(
    const Type& operandType, ByteCode::Register left, ByteCode::Register right,
    ByteCode::Instruction s, ByteCode::Instruction u, ByteCode::Instruction f,
...
void ByteCodeGenerator::writeBinaryInstruction(
    const Type& operandType, ByteCode::Register left, ByteCode::Register right,
    ByteCode::Instruction s, ByteCode::Instruction u, ByteCode::Instruction f,
    ByteCode::Register result) {
...
void ByteCodeGenerator::writeBinaryExpression(
    const BinaryExpression& b, ByteCode::Register result) {
  ...
  switch (b.fOperator) {
    ...
    case Token::Kind::LOGICALOREQ: left = result; break;
    default: left = this->next(SlotCount(*operandType));
  }
  ...
  switch (op) {
    ...
    case Token::Kind::SHR: {
      ...
      if (op == Token::Kind::SHL) {
        ...
        this->write(
            type_category(lType) == TypeCategory::kSigned ? ByteCode::Instruction::kShiftRightS
    ...
    case Token::Kind::STAR:
      ...
      if (lType.columns() > 1 && rType.columns() > 1 && (lType.rows() > 1 || rType.rows() > 1)) {
  ...
  switch (op) {
    case Token::Kind::EQEQ:
      this->writeBinaryInstruction(
          *operandType, left, right, ByteCode::Instruction::kCompareEQI,
          ByteCode::Instruction::kCompareEQI, ByteCode::Instruction::kCompareEQF, result);
    ...
    case Token::Kind::GT:
      this->writeBinaryInstruction(
          *operandType, left, right, ByteCode::Instruction::kCompareGTS,
          ByteCode::Instruction::kCompareGTU, ByteCode::Instruction::kCompareGTF, result);
      break;
    case Token::Kind::GTEQ:
      this->writeBinaryInstruction(
          *operandType, left, right, ByteCode::Instruction::kCompareGTEQS,
          ByteCode::Instruction::kCompareGTEQU, ByteCode::Instruction::kCompareGTEQF, result);
      break;
    case Token::Kind::LT:
      this->writeBinaryInstruction(
          *operandType, left, right, ByteCode::Instruction::kCompareLTS,
          ByteCode::Instruction::kCompareLTU, ByteCode::Instruction::kCompareLTF, result);
      break;
    case Token::Kind::LTEQ:
      this->writeBinaryInstruction(
          *operandType, left, right, ByteCode::Instruction::kCompareLTEQS,
          ByteCode::Instruction::kCompareLTEQU, ByteCode::Instruction::kCompareLTEQF, result);
      break;
    case Token::Kind::MINUS:
      this->writeVectorBinaryInstruction(
          *operandType, left, right, ByteCode::Instruction::kSubtractI,
          ByteCode::Instruction::kSubtractI, ByteCode::Instruction::kSubtractF, result);
      break;
    case Token::Kind::NEQ:
      this->writeBinaryInstruction(
          *operandType, left, right, ByteCode::Instruction::kCompareNEQI,
          ByteCode::Instruction::kCompareNEQI, ByteCode::Instruction::kCompareNEQF, result);
      // Collapse to a single bool
      for (int i = 1; i < count; ++i) {
        this->write(ByteCode::Instruction::kOr);
        this->write(result);
    ...
    case Token::Kind::PERCENT:
      this->writeVectorBinaryInstruction(
          *operandType, left, right, ByteCode::Instruction::kRemainderS,
          ByteCode::Instruction::kRemainderU, ByteCode::Instruction::kRemainderF, result);
      break;
    case Token::Kind::PLUS:
      this->writeVectorBinaryInstruction(
          *operandType, left, right, ByteCode::Instruction::kAddI, ByteCode::Instruction::kAddI,
          ByteCode::Instruction::kAddF, result);
      break;
    case Token::Kind::SLASH:
      this->writeVectorBinaryInstruction(
          *operandType, left, right, ByteCode::Instruction::kDivideS,
          ByteCode::Instruction::kDivideU, ByteCode::Instruction::kDivideF, result);
      break;
    case Token::Kind::STAR:
      this->writeVectorBinaryInstruction(
          *operandType, left, right, ByteCode::Instruction::kMultiplyI,
          ByteCode::Instruction::kMultiplyI, ByteCode::Instruction::kMultiplyF, result);
      break;
...
void ByteCodeGenerator::writeExternalFunctionCall(
    const ExternalFunctionCall& f, ByteCode::Register result) {
  int argumentCount = 0;
...
void ByteCodeGenerator::writeExternalValue(
    const ExternalValueReference& e, ByteCode::Register result) {
...
void ByteCodeGenerator::writeIntrinsicCall(
    const FunctionCall& c, Intrinsic intrinsic, ByteCode::Register result) {
    ...
    switch (intrinsic.fValue.fSpecial) {
      case SpecialIntrinsic::kDot: {
        ...
        this->writeTypedInstruction(
            c.fType, ByteCode::Instruction::kMultiplyIN, ByteCode::Instruction::kMultiplyIN,
        ...
        for (int i = 1; i < count; ++i) {
          this->writeTypedInstruction(
              c.fType, ByteCode::Instruction::kAddI, ByteCode::Instruction::kAddI,
...
void ByteCodeGenerator::incOrDec(
    Token::Kind op, Expression& operand, bool prefix, ByteCode::Register result) {
  ...
  if (op == Token::Kind::PLUSPLUS) {
    this->writeTypedInstruction(
        operand.fType, ByteCode::Instruction::kAddI, ByteCode::Instruction::kAddI,
        ByteCode::Instruction::kAddF);
  } else {
    this->writeTypedInstruction(
        operand.fType, ByteCode::Instruction::kSubtractI, ByteCode::Instruction::kSubtractI,
...
void ByteCodeGenerator::writePostfixExpression(
    const PostfixExpression& p, ByteCode::Register result) {
  this->incOrDec(p.fOperator, *p.fOperand, false, result);
}

void ByteCodeGenerator::writePrefixExpression(
    const PrefixExpression& p, ByteCode::Register result) {
  switch (p.fOperator) {
    ...
    case Token::Kind::MINUS: {
      ...
      for (int i = 0; i < SlotCount(p.fType); ++i) {
        this->writeTypedInstruction(
            p.fType, ByteCode::Instruction::kNegateS, ByteCode::Instruction::kNegateS,
...
void ByteCodeGenerator::writeTernaryExpression(
    const TernaryExpression& t, ByteCode::Register result) {
...
void ByteCodeGenerator::writeVariableExpression(const Expression& expr, ByteCode::Register result) {
...
void ByteCodeGenerator::writeExpression(const Expression& expr, ByteCode::Register result) {
  switch (expr.fKind) {
    ...
    case Expression::kPrefix_Kind:
      ...
      break;
    case Expression::kSwizzle_Kind: this->writeSwizzle((Swizzle&)expr, result); break;
    case Expression::kTernary_Kind:
      this->writeTernaryExpression((TernaryExpression&)expr, result);
      break;
    case Expression::kFieldAccess_Kind:
    case Expression::kIndex_Kind:
    case Expression::kVariableReference_Kind: this->writeVariableExpression(expr, result); break;
    default:
#ifdef SK_DEBUG
...
void ByteCodeGenerator::writeStatement(const Statement& s) {
  switch (s.fKind) {
    ...
    case Statement::kBreak_Kind: this->write(ByteCode::Instruction::kBreak); break;
    case Statement::kContinue_Kind: this->write(ByteCode::Instruction::kContinue); break;
    ...
    case Statement::kReturn_Kind: this->writeReturn((ReturnStatement&)s); break;
    ...
    default: ABORT("unsupported statement\n");
  }
}
