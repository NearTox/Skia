...
static TypeCategory type_category(const Type& type) {
  switch (type.kind()) {
    ...
    case Type::Kind::kMatrix_Kind: return type_category(type.componentType());
    default:
      if (type.fName == "bool") {
...
static bool swizzle_is_simple(const Swizzle& s) {
  switch (s.fBase->fKind) {
    ...
    case Expression::kVariableReference_Kind: break;
...
#define VECTOR_UNARY_OP(base)        \
  ...
  case ByteCodeInstruction::base##4: return 0;
...
#undef VECTOR_MATRIX_BINARY_OP
    ...
    case ByteCodeInstruction::kReadExternal2: return 2;
    ...
    case ByteCodeInstruction::kReadExternal3: return 3;
...
void ByteCodeGenerator::writeTypedInstruction(
  ...
  switch (type_category(type)) {
    ...
    case TypeCategory::kSigned: this->write(vector_instruction(s, count)); break;
    case TypeCategory::kUnsigned: this->write(vector_instruction(u, count)); break;
  ...
  if (op == Token::Kind::TK_STAR && lVecOrMtx && rVecOrMtx &&
    ...
    switch (op) {
      ...
      case Token::Kind::TK_BITWISEAND:
        SkASSERT(
            count == 1 &&
            (tc == SkSL::TypeCategory::kSigned || tc == SkSL::TypeCategory::kUnsigned));
        this->write(ByteCodeInstruction::kAndB);
        break;
...
void ByteCodeGenerator::writeIntrinsicCall(const FunctionCall& c) {
  ...
  if (intrin.is_special && (intrin.special == SpecialIntrinsic::kClamp ||
    ...
    this->writeTypedInstruction(
        c.fArguments[0]->fType, ByteCodeInstruction::kMaxS, ByteCodeInstruction::kMaxS,
    ...
    dupSmallerType(limitCount);
    this->writeTypedInstruction(
        c.fArguments[0]->fType, ByteCodeInstruction::kMinS, ByteCodeInstruction::kMinS,
  ...
  if (intrin.is_special) {
    ...
    switch (intrin.inst_f) {
      ...
      default:
        this->writeTypedInstruction(
            c.fArguments[0]->fType, intrin.inst_s, intrin.inst_u, intrin.inst_f, count);
        break;
    }
  }
}
  ...
  switch (p.fOperator) {
    ...
    case Token::Kind::TK_MINUSMINUS: {
      ...
      if (p.fOperator == Token::Kind::TK_PLUSPLUS) {
        this->writeTypedInstruction(
            p.fType, ByteCodeInstruction::kAddI, ByteCodeInstruction::kAddI,
            ByteCodeInstruction::kAddF, 1);
      } else {
        this->writeTypedInstruction(
            p.fType, ByteCodeInstruction::kSubtractI, ByteCodeInstruction::kSubtractI,
    ...
    case Token::Kind::TK_MINUS: {
      ...
      this->writeTypedInstruction(
          p.fType, ByteCodeInstruction::kNegateI, ByteCodeInstruction::kNegateI,
          ByteCodeInstruction::kNegateF, SlotCount(p.fOperand->fType));
      break;
    }
...
void ByteCodeGenerator::writeExpression(const Expression& e, bool discard) {
  switch (e.fKind) {
    case Expression::kBinary_Kind:
      ...
      break;
    case Expression::kBoolLiteral_Kind: this->writeBoolLiteral((BoolLiteral&)e); break;
    case Expression::kConstructor_Kind: this->writeConstructor((Constructor&)e); break;
    ...
    case Expression::kFieldAccess_Kind:
    case Expression::kIndex_Kind:
    case Expression::kVariableReference_Kind: this->writeVariableExpression(e); break;
    case Expression::kFloatLiteral_Kind: this->writeFloatLiteral((FloatLiteral&)e); break;
    case Expression::kFunctionCall_Kind: this->writeFunctionCall((FunctionCall&)e); break;
    case Expression::kIntLiteral_Kind: this->writeIntLiteral((IntLiteral&)e); break;
    case Expression::kNullLiteral_Kind: this->writeNullLiteral((NullLiteral&)e); break;
    ...
    case Expression::kPostfix_Kind:
      ...
      break;
    case Expression::kSwizzle_Kind: this->writeSwizzle((Swizzle&)e); break;
    case Expression::kTernary_Kind: this->writeTernaryExpression((TernaryExpression&)e); break;
    default:
#ifdef SK_DEBUG
 ...
 public:
  ...
  void store(bool discard) override {
    ...
    if (location.isOnStack() || count > 4) {
      ...
      fGenerator.write(vector_instruction(
...
void ByteCodeGenerator::writeStatement(const Statement& s) {
  switch (s.fKind) {
    ...
    case Statement::kBreak_Kind: this->writeBreakStatement((BreakStatement&)s); break;
    case Statement::kContinue_Kind: this->writeContinueStatement((ContinueStatement&)s); break;
