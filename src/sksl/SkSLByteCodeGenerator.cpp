...
ByteCodeGenerator::ByteCodeGenerator(
    ...
    : INHERITED(program, errors, nullptr),
      ...
      fIntrinsics{
          ...
          {"dot", SpecialIntrinsic::kDot},      {"sin", ByteCodeInstruction::kSin},
          {"sqrt", ByteCodeInstruction::kSqrt}, {"tan", ByteCodeInstruction::kTan},
...
void ByteCodeGenerator::write(ByteCodeInstruction i) { this->write16((uint16_t)i); }
...
void ByteCodeGenerator::writeExternalValue(const ExternalValueReference& e) {
  this->write(vector_instruction(ByteCodeInstruction::kReadExternal, SlotCount(e.fValue->type())));
  int index = fOutput->fExternalValues.size();
...
void ByteCodeGenerator::writeVariableExpression(const Expression& expr) {
  ...
  if (location < 0 || count > 4) {
    ...
    this->write(
        isGlobal ? ByteCodeInstruction::kLoadExtendedGlobal : ByteCodeInstruction::kLoadExtended);
    this->write8(count);
  } else {
    this->write(vector_instruction(
        isGlobal ? ByteCodeInstruction::kLoadGlobal : ByteCodeInstruction::kLoad, count));
...
void ByteCodeGenerator::writeIntrinsicCall(const FunctionCall& c) {
  ...
  if (found->second.fIsSpecial) {
    ...
    this->write((ByteCodeInstruction)(
        (int)ByteCodeInstruction::kMultiplyF + SlotCount(c.fArguments[0]->fType) - 1));
    ...
    switch (found->second.fValue.fInstruction) {
      ...
      case ByteCodeInstruction::kTan:
        ...
        this->write((ByteCodeInstruction)(
            (int)found->second.fValue.fInstruction + SlotCount(c.fArguments[0]->fType) - 1));
        break;
...
void ByteCodeGenerator::writePrefixExpression(const PrefixExpression& p) {
  switch (p.fOperator) {
    ...
    case Token::Kind::MINUSMINUS: {
      ...
      this->write(ByteCodeInstruction::kPushImmediate);
      this->write32(
          type_category(p.fType) == TypeCategory::kFloat ? Interpreter::Value(1.0f).fUnsigned : 1);
...
void ByteCodeGenerator::writeSwizzle(const Swizzle& s) {
  ...
  switch (s.fBase->fKind) {
    case Expression::kVariableReference_Kind: {
      ...
      this->write(
          var.fStorage == Variable::kGlobal_Storage ? ByteCodeInstruction::kLoadSwizzleGlobal
...
void ByteCodeGenerator::writeExpression(const Expression& e) {
  switch (e.fKind) {
    case Expression::kBinary_Kind: this->writeBinaryExpression((BinaryExpression&)e); break;
    ...
    case Expression::kPrefix_Kind: this->writePrefixExpression((PrefixExpression&)e); break;
    case Expression::kPostfix_Kind: this->writePostfixExpression((PostfixExpression&)e); break;
    ...
    default: printf("unsupported expression %s\n", e.description().c_str()); SkASSERT(false);
  }
}
...
class ByteCodeSwizzleLValue : public ByteCodeGenerator::LValue {
 public:
  ...
  void store() override {
    fGenerator.write(vector_instruction(ByteCodeInstruction::kDup, fSwizzle.fComponents.size()));
    Variable::Storage storage;
    int location = fGenerator.getLocation(*fSwizzle.fBase, &storage);
    bool isGlobal = storage == Variable::kGlobal_Storage;
    if (location < 0) {
      fGenerator.write(
          isGlobal ? ByteCodeInstruction::kStoreSwizzleIndirectGlobal
                   : ByteCodeInstruction::kStoreSwizzleIndirect);
    } else {
      fGenerator.write(
          isGlobal ? ByteCodeInstruction::kStoreSwizzleGlobal : ByteCodeInstruction::kStoreSwizzle);
 ...
 public:
  ...
  void store() override {
    ...
    if (location < 0 || count > 4) {
      ...
      fGenerator.write(
          isGlobal ? ByteCodeInstruction::kStoreExtendedGlobal
                   : ByteCodeInstruction::kStoreExtended);
      fGenerator.write8(count);
    } else {
      fGenerator.write(vector_instruction(
          isGlobal ? ByteCodeInstruction::kStoreGlobal : ByteCodeInstruction::kStore, count));
      fGenerator.write8(location);
    }
  }
...
std::unique_ptr<ByteCodeGenerator::LValue> ByteCodeGenerator::getLValue(const Expression& e) {
  switch (e.fKind) {
    ...
    default: printf("unsupported lvalue %s\n", e.description().c_str()); return nullptr;
  }
}
