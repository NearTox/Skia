...
static TypeCategory type_category(const Type& type) {
  switch (type.typeKind()) {
    ...
    default:
      ...
      } else if (type.fName == "int" || type.fName == "short" || type.fName == "$intLiteral") {
        return TypeCategory::kSigned;
      } else if (type.fName == "uint" || type.fName == "ushort") {
        return TypeCategory::kUnsigned;
      } else {
        SkASSERT(type.fName == "float" || type.fName == "half" || type.fName == "$floatLiteral");
    ...
    case Expression::Kind::kIndex: {
      ...
      if (i.fIndex->isCompileTimeConstant()) {
        ...
        if (i.fIndex->hasSideEffects()) {
          ...
          fErrors.error(
              i.fIndex->fOffset, "Index expressions with side-effects not supported in byte code.");
          return Location::MakeInvalid();
        }
...
void ByteCodeGenerator::writeIntrinsicCall(const FunctionCall& c) {
  ...
  if (found == fIntrinsics.end()) {
    fErrors.error(
        c.fOffset,
        String::printf("Unsupported intrinsic: '%s'", String(c.fFunction.fName).c_str()));
    return;
  }
  ...
  switch (p.fOperator) {
    ...
    case Token::Kind::TK_MINUS: {
      ...
      this->writeTypedInstruction(
          ...
          ByteCodeInstruction::kNegateF, SlotCount(p.fOperand->type()));
      break;
    }
    ...
    case Token::Kind::TK_BITWISENOT: {
      ...
      SkASSERT(
          (p.fOperator == Token::Kind::TK_LOGICALNOT && tc == TypeCategory::kBool) ||
          (p.fOperator == Token::Kind::TK_BITWISENOT &&
