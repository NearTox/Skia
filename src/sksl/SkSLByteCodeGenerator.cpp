...
static TypeCategory type_category(const Type& type) {
  switch (type.typeKind()) {
    ...
    case Type::TypeKind::kMatrix: return type_category(type.componentType());
    default:
      const StringFragment& name = type.name();
      if (name == "bool") {
        return TypeCategory::kBool;
      } else if (name == "int" || name == "short" || name == "$intLiteral") {
        return TypeCategory::kSigned;
      } else if (name == "uint" || name == "ushort") {
        return TypeCategory::kUnsigned;
      } else {
        SkASSERT(name == "float" || name == "half" || name == "$floatLiteral");
...
ByteCodeGenerator::ByteCodeGenerator(
    const Context* context, const Program* program, ErrorReporter* errors, ByteCode* output)
    : INHERITED(program, errors, nullptr),
      ...
      fIntrinsics{
          ...
          {"lessThan",
           {ByteCodeInstruction::kCompareFLT, ByteCodeInstruction::kCompareSLT,
            ByteCodeInstruction::kCompareULT}},
          {"lessThanEqual",
           {ByteCodeInstruction::kCompareFLTEQ, ByteCodeInstruction::kCompareSLTEQ,
            ByteCodeInstruction::kCompareULTEQ}},
          {"greaterThan",
           {ByteCodeInstruction::kCompareFGT, ByteCodeInstruction::kCompareSGT,
            ByteCodeInstruction::kCompareUGT}},
          {"greaterThanEqual",
           {ByteCodeInstruction::kCompareFGTEQ, ByteCodeInstruction::kCompareSGTEQ,
            ByteCodeInstruction::kCompareUGTEQ}},
          {"equal",
           {ByteCodeInstruction::kCompareFEQ, ByteCodeInstruction::kCompareIEQ,
            ByteCodeInstruction::kCompareIEQ}},
          {"notEqual",
           {ByteCodeInstruction::kCompareFNEQ, ByteCodeInstruction::kCompareINEQ,
...
int ByteCodeGenerator::SlotCount(const Type& type) {
  switch (type.typeKind()) {
    case Type::TypeKind::kOther: return 0;
    ...
    default: return type.columns() * type.rows();
  }
}
...
void ByteCodeGenerator::gatherUniforms(const Type& type, const String& name) {
  switch (type.typeKind()) {
    case Type::TypeKind::kOther: break;
    case Type::TypeKind::kStruct:
      for (const auto& f : type.fields()) {
    ...
    case Type::TypeKind::kArray:
      ...
      break;
    default:
      fOutput->fUniforms.push_back(
          {name, type_category(type), type.rows(), type.columns(), fOutput->fUniformSlotCount});
  ...
  for (const auto& e : fProgram.elements()) {
    switch (e->kind()) {
      case ProgramElement::Kind::kFunction: {
        std::unique_ptr<ByteCodeFunction> f = this->writeFunction(e->as<FunctionDefinition>());
        if (!f) {
          return false;
        }
...
static bool swizzle_is_simple(const Swizzle& s) {
  ...
  switch (s.base()->kind()) {
    ...
    case Expression::Kind::kVariableReference: break;
...
#define VEC_UNARY(inst) \
  case ByteCodeInstruction::inst: return count - count;
...
#undef VEC_UNARY
    ...
    case ByteCodeInstruction::kPushImmediate: return 1;
    ...
    case ByteCodeInstruction::kReserve: return count;
    ...
    case ByteCodeInstruction::kLoadExtendedUniform: return count - 1;
    ...
    case ByteCodeInstruction::kWriteExternal: return -count;
  ...
  switch (var.storage()) {
    ...
    case Variable::Storage::kGlobal: {
      ...
      if (is_in(var)) {
        ...
        // 'uniform' instead?).
        fErrors.error(var.fOffset, "'in' variable is not specialized or has unsupported type");
        return Location::MakeInvalid();
      }
      int offset = 0;
    ...
    case Expression::Kind::kIndex: {
      ...
      if (index.isCompileTimeConstant()) {
        ...
        if (index.hasSideEffects()) {
          ...
          fErrors.error(
              index.fOffset, "Index expressions with side-effects not supported in byte code.");
          return Location::MakeInvalid();
        }
...
void ByteCodeGenerator::write8(uint8_t b) { fCode->push_back(b); }
...
void ByteCodeGenerator::write(ByteCodeInstruction i, int count) {
    ...
    if (count != kUnusedStackCount) {
      switch (i) {
        ...
        case ByteCodeInstruction::kMatrixMultiply:
        case ByteCodeInstruction::kScalarToMatrix:
        case ByteCodeInstruction::kSwizzle: break;
        default: this->write8(count); break;
      }
    }
}

void ByteCodeGenerator::writeTypedInstruction(
    const Type& type, ByteCodeInstruction s, ByteCodeInstruction u, ByteCodeInstruction f,
    int count) {
...
bool ByteCodeGenerator::writeBinaryExpression(const BinaryExpression& b, bool discard) {
  ...
  bool lVecOrMtx =
      (lType.typeKind() == Type::TypeKind::kVector || lType.typeKind() == Type::TypeKind::kMatrix);
  bool rVecOrMtx =
      (rType.typeKind() == Type::TypeKind::kVector || rType.typeKind() == Type::TypeKind::kMatrix);
  ...
  switch (op) {
    ...
    case Token::Kind::TK_SHR: {
      SkASSERT(
          count == 1 && (tc == SkSL::TypeCategory::kSigned || tc == SkSL::TypeCategory::kUnsigned));
      ...
      if (op == Token::Kind::TK_SHL) {
        ...
        this->write(
            type_category(lType) == TypeCategory::kSigned ? ByteCodeInstruction::kShiftRightS
  ...
  if (op == Token::Kind::TK_STAR && lVecOrMtx && rVecOrMtx &&
    ...
    this->write(
        ByteCodeInstruction::kMatrixMultiply,
    ...
    switch (op) {
      case Token::Kind::TK_EQEQ:
        this->writeTypedInstruction(
            lType, ByteCodeInstruction::kCompareIEQ, ByteCodeInstruction::kCompareIEQ,
            ByteCodeInstruction::kCompareFEQ, count);
      ...
      case Token::Kind::TK_GT:
        this->writeTypedInstruction(
            lType, ByteCodeInstruction::kCompareSGT, ByteCodeInstruction::kCompareUGT,
            ByteCodeInstruction::kCompareFGT, count);
        break;
      case Token::Kind::TK_GTEQ:
        this->writeTypedInstruction(
            lType, ByteCodeInstruction::kCompareSGTEQ, ByteCodeInstruction::kCompareUGTEQ,
            ByteCodeInstruction::kCompareFGTEQ, count);
        break;
      case Token::Kind::TK_LT:
        this->writeTypedInstruction(
            lType, ByteCodeInstruction::kCompareSLT, ByteCodeInstruction::kCompareULT,
            ByteCodeInstruction::kCompareFLT, count);
        break;
      case Token::Kind::TK_LTEQ:
        this->writeTypedInstruction(
            lType, ByteCodeInstruction::kCompareSLTEQ, ByteCodeInstruction::kCompareULTEQ,
            ByteCodeInstruction::kCompareFLTEQ, count);
        break;
      case Token::Kind::TK_MINUS:
        this->writeTypedInstruction(
            lType, ByteCodeInstruction::kSubtractI, ByteCodeInstruction::kSubtractI,
            ByteCodeInstruction::kSubtractF, count);
        break;
      case Token::Kind::TK_NEQ:
        this->writeTypedInstruction(
            lType, ByteCodeInstruction::kCompareINEQ, ByteCodeInstruction::kCompareINEQ,
            ByteCodeInstruction::kCompareFNEQ, count);
        // Collapse to a single bool
        for (int i = count; i > 1; --i) {
          this->write(ByteCodeInstruction::kOrB, 1);
        }
      ...
      case Token::Kind::TK_PERCENT:
        this->writeTypedInstruction(
            lType, ByteCodeInstruction::kRemainderS, ByteCodeInstruction::kRemainderU,
            ByteCodeInstruction::kRemainderF, count);
        break;
      case Token::Kind::TK_PLUS:
        this->writeTypedInstruction(
            lType, ByteCodeInstruction::kAddI, ByteCodeInstruction::kAddI,
            ByteCodeInstruction::kAddF, count);
        break;
      case Token::Kind::TK_SLASH:
        this->writeTypedInstruction(
            lType, ByteCodeInstruction::kDivideS, ByteCodeInstruction::kDivideU,
            ByteCodeInstruction::kDivideF, count);
        break;
      case Token::Kind::TK_STAR:
        this->writeTypedInstruction(
            lType, ByteCodeInstruction::kMultiplyI, ByteCodeInstruction::kMultiplyI,
            ByteCodeInstruction::kMultiplyF, count);
        break;
  ...
  if (c.arguments().size() == 1) {
    ...
    if (inCategory != outCategory) {
      ...
      if (inCategory == TypeCategory::kFloat) {
        SkASSERT(outCategory == TypeCategory::kSigned || outCategory == TypeCategory::kUnsigned);
    ...
    if (inType.typeKind() == Type::TypeKind::kMatrix &&
      ...
      this->write(ByteCodeInstruction::kMatrixToMatrix, SlotCount(outType) - SlotCount(inType));
...
void ByteCodeGenerator::writeIntrinsicCall(const FunctionCall& c) {
  ...
  if (found == fIntrinsics.end()) {
    fErrors.error(
        c.fOffset,
        String::printf("Unsupported intrinsic: '%s'", String(c.function().name()).c_str()));
    return;
  }
  ...
  if (intrin.is_special && intrin.special == SpecialIntrinsic::kSample) {
    ...
    if (nargs == 2) {
      ...
      this->write(
          args[1]->type() == *fContext.fFloat3x3_Type ? ByteCodeInstruction::kSampleMatrix
  ...
  if (intrin.is_special && (intrin.special == SpecialIntrinsic::kClamp ||
    ...
    this->writeTypedInstruction(
        args[0]->type(), ByteCodeInstruction::kMaxS, ByteCodeInstruction::kMaxS,
    ...
    dupSmallerType(limitCount);
    this->writeTypedInstruction(
        args[0]->type(), ByteCodeInstruction::kMinS, ByteCodeInstruction::kMinS,
  ...
  if (intrin.is_special) {
    ...
    switch (intrin.inst_f) {
      ...
      default:
        this->writeTypedInstruction(
            args[0]->type(), intrin.inst_s, intrin.inst_u, intrin.inst_f, count);
        break;
    }
  }
}
...
void ByteCodeGenerator::writeFunctionCall(const FunctionCall& f) {
  ...
  fMaxStackCount =
      std::max(fMaxStackCount, fStackCount + callee->fLocalCount + callee->fStackCount);
...
bool ByteCodeGenerator::writePrefixExpression(const PrefixExpression& p, bool discard) {
  switch (p.getOperator()) {
    ...
    case Token::Kind::TK_MINUSMINUS: {
      ...
      lvalue->load();
      this->write(ByteCodeInstruction::kPushImmediate);
      this->write32(type_category(p.type()) == TypeCategory::kFloat ? float_to_bits(1.0f) : 1);
      if (p.getOperator() == Token::Kind::TK_PLUSPLUS) {
        this->writeTypedInstruction(
            p.type(), ByteCodeInstruction::kAddI, ByteCodeInstruction::kAddI,
            ByteCodeInstruction::kAddF, 1);
      } else {
        this->writeTypedInstruction(
            p.type(), ByteCodeInstruction::kSubtractI, ByteCodeInstruction::kSubtractI,
    ...
    case Token::Kind::TK_MINUS: {
      ...
      this->writeTypedInstruction(
          p.type(), ByteCodeInstruction::kNegateI, ByteCodeInstruction::kNegateI,
          ByteCodeInstruction::kNegateF, SlotCount(p.operand()->type()));
      break;
    }
    ...
    case Token::Kind::TK_BITWISENOT: {
      ...
      SkASSERT(
          (p.getOperator() == Token::Kind::TK_LOGICALNOT && tc == TypeCategory::kBool) ||
...
void ByteCodeGenerator::writeExpression(const Expression& e, bool discard) {
  switch (e.kind()) {
    case Expression::Kind::kBinary:
      ...
      break;
    case Expression::Kind::kBoolLiteral: this->writeBoolLiteral(e.as<BoolLiteral>()); break;
    case Expression::Kind::kConstructor: this->writeConstructor(e.as<Constructor>()); break;
    ...
    case Expression::Kind::kFieldAccess:
    case Expression::Kind::kIndex:
    case Expression::Kind::kVariableReference: this->writeVariableExpression(e); break;
    case Expression::Kind::kFloatLiteral: this->writeFloatLiteral(e.as<FloatLiteral>()); break;
    case Expression::Kind::kFunctionCall: this->writeFunctionCall(e.as<FunctionCall>()); break;
    case Expression::Kind::kIntLiteral: this->writeIntLiteral(e.as<IntLiteral>()); break;
    case Expression::Kind::kNullLiteral: this->writeNullLiteral(e.as<NullLiteral>()); break;
    ...
    case Expression::Kind::kPostfix:
      ...
      break;
    case Expression::Kind::kSwizzle: this->writeSwizzle(e.as<Swizzle>()); break;
    case Expression::Kind::kTernary: this->writeTernaryExpression(e.as<TernaryExpression>()); break;
    default:
#ifdef SK_DEBUG
...
class ByteCodeExternalValueLValue : public ByteCodeGenerator::LValue {
 public:
  ByteCodeExternalValueLValue(ByteCodeGenerator* generator, const ExternalValue& value, int index)
      : INHERITED(*generator), fCount(ByteCodeGenerator::SlotCount(value.type())), fIndex(index) {}
...
class ByteCodeSwizzleLValue : public ByteCodeGenerator::LValue {
 public:
  ByteCodeSwizzleLValue(ByteCodeGenerator* generator, const Swizzle& swizzle)
      : INHERITED(*generator), fSwizzle(swizzle) {}

  void load() override { fGenerator.writeSwizzle(fSwizzle); }
...
class ByteCodeExpressionLValue : public ByteCodeGenerator::LValue {
 public:
  ByteCodeExpressionLValue(ByteCodeGenerator* generator, const Expression& expr)
      : INHERITED(*generator), fExpression(expr) {}

  void load() override { fGenerator.writeVariableExpression(fExpression); }
...
std::unique_ptr<ByteCodeGenerator::LValue> ByteCodeGenerator::getLValue(const Expression& e) {
  switch (e.kind()) {
    ...
    case Expression::Kind::kSwizzle: {
      ...
      return swizzle_is_simple(s) ? std::unique_ptr<LValue>(new ByteCodeExpressionLValue(this, e))
...
void ByteCodeGenerator::writeStatement(const Statement& s) {
  switch (s.kind()) {
    ...
    case Statement::Kind::kBreak: this->writeBreakStatement(s.as<BreakStatement>()); break;
    case Statement::Kind::kContinue: this->writeContinueStatement(s.as<ContinueStatement>()); break;
    ...
    case Statement::Kind::kWhile: this->writeWhileStatement(s.as<WhileStatement>()); break;
