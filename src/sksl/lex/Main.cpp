...
void writeH(
    const DFA& dfa, const char* lexer, const char* token, const std::vector<std::string>& tokens,
    const char* hPath) {
  ...
  out << R"(
    ...
    , fLength(-1) {}
      ...
      << R"((Kind kind, int32_t offset, int32_t length)
    : fKind(kind)
    , fOffset(offset)
    , fLength(length) {}
...
void writeCPP(
  ...
  out << R"(
    ...
    if (startOffset == (int32_t)fText.length()) {
        return )"
      << token << "(" << token << R"(::Kind::TK_END_OF_FILE, startOffset, 0);
    }
    State state = 1;
    for (;;) {
    ...
    Token::Kind kind = ()"
      << token << R"(::Kind) accepts[state];
    return )"
      << token << R"((kind, startOffset, fOffset - startOffset);
}
...
void process(
    const char* inPath, const char* lexer, const char* token, const char* hPath,
    const char* cppPath) {
  NFA nfa;
    ...
    if (split >> name >> delimiter >> pattern) {
      ...
      if (pattern[0] == '"') {
        ...
        for (size_t i = 2; i < pattern.size() - 1; ++i) {
          node = RegexNode(
              RegexNode::kConcat_Kind, node, RegexNode(RegexNode::kChar_Kind, pattern[i]));
        }
        nfa.addRegex(node);
      } else {
