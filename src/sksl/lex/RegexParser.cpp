...
void RegexParser::quantifiedTerm() {
  ...
  switch (this->peek()) {
    case '*':
      fStack.push(RegexNode(RegexNode::kStar_Kind, this->pop()));
      ++fIndex;
      break;
    case '+':
      fStack.push(RegexNode(RegexNode::kPlus_Kind, this->pop()));
      ++fIndex;
      break;
    case '?':
      fStack.push(RegexNode(RegexNode::kQuestion_Kind, this->pop()));
      ++fIndex;
      break;
    default: break;
  }
