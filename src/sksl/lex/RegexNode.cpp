...
std::vector<int> RegexNode::createStates(NFA* nfa, const std::vector<int>& accept) const {
  ...
  switch (fKind) {
    case kChar_Kind: result.push_back(nfa->addState(NFAState(fPayload.fChar, accept))); break;
    case kCharset_Kind: {
      ...
      for (const RegexNode& child : fChildren) {
        if (child.fKind == kChar_Kind) {
          ...
          for (char c = child.fChildren[0].fPayload.fChar; c <= child.fChildren[1].fPayload.fChar;
               ++c) {
            chars[c] = true;
          }
        }
    ...
    case kDot_Kind: result.push_back(nfa->addState(NFAState(NFAState::kDot_Kind, accept))); break;
    case kOr_Kind: {
...
std::string RegexNode::description() const {
  switch (fKind) {
    ...
    case kConcat_Kind: return fChildren[0].description() + fChildren[1].description();
    case kDot_Kind: return ".";
    case kOr_Kind: return "(" + fChildren[0].description() + "|" + fChildren[1].description() + ")";
    case kPlus_Kind: return fChildren[0].description() + "+";
    case kQuestion_Kind: return fChildren[0].description() + "?";
    case kRange_Kind: return fChildren[0].description() + "-" + fChildren[1].description();
    case kStar_Kind: return fChildren[0].description() + "*";
