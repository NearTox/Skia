...
static bool is_private(const Variable& var) {
  return !(var.fModifiers.fFlags & Modifiers::kUniform_Flag) &&
         ...
         var.fStorage == Variable::kGlobal_Storage && var.fModifiers.fLayout.fBuiltin == -1;
}
      ...
      if (ref.fVariable->fModifiers.fFlags & Modifiers::kUniform_Flag) {
        ...
        if (ref.fVariable->fModifiers.fLayout.fWhen.fLength) {
          code = String::printf(
              ...
              var.c_str(), default_value(ref.fVariable->type()).c_str());
        } else {
          code = var;
        }
        ...
        this->writeRuntimeValue(
            ref.fVariable->type(), ref.fVariable->fModifiers.fLayout,
    ...
    if (p.kind() == ProgramElement::Kind::kVar) {
      ...
      for (const auto& raw : decls.fVars) {
        ...
        if (is_private(*decl.fVar)) {
          ...
          this->writef(
              "%s %s = %s;\n",
              HCodeGenerator::FieldType(fContext, decl.fVar->type(), decl.fVar->fModifiers.fLayout)
                  .c_str(),
              String(decl.fVar->fName).c_str(), default_value(*decl.fVar).c_str());
  ...
  for (const auto& p : fProgram) {
    if (p.kind() == ProgramElement::Kind::kVar) {
      ...
      for (const auto& raw : decls.fVars) {
        ...
        if (SectionAndParameterHelper::IsParameter(*decl.fVar) && is_accessible(*decl.fVar)) {
          this->writef(
              "        auto %s = _outer.%s;\n"
              "        (void) %s;\n",
              name, name, name);
  ...
  for (const Variable* u : uniforms) {
    if (is_uniform_in(*u)) {
      ...
      if (needsValueDeclaration) {
        ...
        String valueType = HCodeGenerator::AccessType(fContext, u->type(), u->fModifiers.fLayout);
        this->writef(
  ...
  for (const auto& param : fSectionAndParameterHelper.getParameters()) {
    if (param->type().typeKind() == Type::TypeKind::kSampler) {
      if (!foundSampler) {
        ...
        this->writef(", %s", HCodeGenerator::FieldName(String(param->fName).c_str()).c_str());
      }
    }
  }
  if (foundSampler) {
  ...
  if (!this->writeSection(kDumpInfoSection)) {
    ...
    for (const Variable* param : fSectionAndParameterHelper.getParameters()) {
      ...
      String runtimeValue = this->formatRuntimeValue(
          param->type(), param->fModifiers.fLayout, param->fName, &argumentList);
...
void CPPCodeGenerator::writeTest() {
  ...
  if (test) {
    this->writef(
        ...
        fFullName.c_str(), fFullName.c_str(), test->fArgument.c_str());
        ...
        switch (var.fModifiers.fLayout.fKey) {
          case Layout::kKey_Key:
            if (is_private(var)) {
              this->writef(
                  "%s %s =",
                  HCodeGenerator::FieldType(fContext, varType, var.fModifiers.fLayout).c_str(),
      ...
      for (const auto& raw : decls.fVars) {
        ...
        if (is_uniform_in(*decl.fVar)) {
          ...
          if (mapper == nullptr) {
            fErrors.error(
                decl.fOffset,
                String(decl.fVar->fName) + "'s type is not supported for use as a 'uniform in'");
            return false;
          }
          if (decl.fVar->fModifiers.fLayout.fFlags & Layout::kTracked_Flag) {
            if (!mapper->supportsTracking()) {
              fErrors.error(
                  decl.fOffset,
                  String(decl.fVar->fName) + "'s type does not support state tracking");
              return false;
            }
          }
