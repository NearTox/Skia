...
CPPCodeGenerator::CPPCodeGenerator(
    const Context* context, const Program* program, ErrorReporter* errors, String name,
    OutputStream* out)
...
bool CPPCodeGenerator::usesPrecisionModifiers() const { return false; }

String CPPCodeGenerator::getTypeName(const Type& type) { return type.name(); }
...
void CPPCodeGenerator::writeBinaryExpression(
  ...
  if (op == Token::Kind::TK_PERCENT) {
    ...
    if (precedence >= parentPrecedence) {
      ...
      left.kind() == Expression::Kind::kNullLiteral ||
    ...
    SkASSERT(
        var->type().typeKind() == Type::TypeKind::kNullable &&
        var->type().componentType() == *fContext.fFragmentProcessor_Type);
    this->write("%s");
    const char* prefix = "";
    switch (op) {
      case Token::Kind::TK_EQEQ: prefix = "!"; break;
      case Token::Kind::TK_NEQ: prefix = ""; break;
      default: SkASSERT(false);
    }
    int childIndex = this->getChildFPIndex(*var);
    fFormatArgs.push_back(
        String(prefix) + "_outer.childProcessor(" + to_string(childIndex) +
        ") ? \"true\" : \"false\"");
  } else {
...
static bool is_private(const Variable& var) {
  ...
  return !(modifiers.fFlags & Modifiers::kUniform_Flag) &&
         !(modifiers.fFlags & Modifiers::kIn_Flag) && var.storage() == Variable::Storage::kGlobal &&
...
String CPPCodeGenerator::formatRuntimeValue(
  ...
  if (type.typeKind() == Type::TypeKind::kArray) {
    ...
    for (int i = 0; i < type.columns(); i++) {
      result += separator + this->formatRuntimeValue(
                                type.componentType(), layout,
                                "(" + cppCode + ")[" + to_string(i) + "]", formatArgs);
      separator = ",";
    }
    result += "]";
    return result;
  ...
  if (type.typeKind() == Type::TypeKind::kMatrix) {
    SkASSERT(
        type.componentType() == *fContext.fFloat_Type ||
  ...
  if (type == *fContext.fInt4_Type || type == *fContext.fShort4_Type ||
...
void CPPCodeGenerator::writeRuntimeValue(
    const Type& type, const Layout& layout, const String& cppCode) {
...
void CPPCodeGenerator::setReturnType(int offset, ReturnType typeToSet) {
  if (fReturnType == ReturnType::kNothing) {
    ...
    fErrors.error(offset, "Fragment processors must not mix sk_OutColor and return statements\n");
  }
}
...
void CPPCodeGenerator::writeVariableReference(const VariableReference& ref) {
  ...
  switch (ref.variable()->modifiers().fLayout.fBuiltin) {
    ...
    case SK_MAIN_COORDS_BUILTIN:
      ...
      break;
    case SK_WIDTH_BUILTIN: this->write("sk_Width"); break;
    case SK_HEIGHT_BUILTIN: this->write("sk_Height"); break;
    default:
      const Variable& var = *ref.variable();
      if (var.type().typeKind() == Type::TypeKind::kSampler) {
        this->write("%s");
        fFormatArgs.push_back(
            "fragBuilder->getProgramBuilder()->samplerVariable(" +
      ...
      if (var.modifiers().fFlags & Modifiers::kUniform_Flag) {
        ...
        String varCode = String::printf(
            "args.fUniformHandler->getUniformCStr(%sVar)",
            HCodeGenerator::FieldName(name.c_str()).c_str());
        String code;
        if (var.modifiers().fLayout.fWhen.fLength) {
          code = String::printf(
              "%sVar.isValid() ? %s : \"%s\"", HCodeGenerator::FieldName(name.c_str()).c_str(),
              varCode.c_str(), default_value(var.type()).c_str());
        } else {
          code = varCode;
        }
        ...
        String name(var.name());
        this->writeRuntimeValue(
            var.type(), var.modifiers().fLayout, String::printf("_outer.%s", name.c_str()).c_str());
      } else {
        this->write(var.name());
      }
  }
...
void CPPCodeGenerator::writeFieldAccess(const FieldAccess& access) {
  if (access.base()->type().name() == "fragmentProcessor") {
    ...
    String cppAccess = String::printf(
        "_outer.childProcessor(%d)->%s()", this->getChildFPIndex(var), String(field.fName).c_str());
...
void CPPCodeGenerator::writeFunctionCall(const FunctionCall& c) {
  ...
  if (function.isBuiltin() && function.name() == "sample" &&
    ...
    addExtraEmitCodeLine(
        "SkString " + childName + " = this->" + invokeFunction + "(" + childIndexStr + inputColor +
        ", args" + inputCoord + ");");
  ...
  if (function.isBuiltin() && function.name() == "sample") {
    ...
    String sampler = this->getSamplerHandle(*arguments[0]->as<VariableReference>().variable());
    fFormatArgs.push_back(
        "fragBuilder->getProgramBuilder()->samplerSwizzle(" + sampler + ").asString().c_str()");
  }
}
...
void CPPCodeGenerator::prepareHelperFunction(const FunctionDeclaration& decl) {
  ...
  this->addExtraEmitCodeLine(String::printf(
      "SkString %s_name = fragBuilder->getMangledFunctionName(\"%s\");", funcName.c_str(),
      funcName.c_str()));
  ...
  for (const Variable* param : decl.parameters()) {
    ...
    args.appendf(
        "%sGrShaderVar(\"%s\", %s)", separator, paramName.c_str(),
...
void CPPCodeGenerator::prototypeHelperFunction(const FunctionDeclaration& decl) {
  ...
  this->addExtraEmitCodeLine(String::printf(
      ...
      glsltype_string(fContext, decl.returnType()), funcName.c_str(), funcName.c_str(),
  ...
  if (decl.name() == "main") {
    ...
    if (!fFormatArgs.empty()) {
      this->addExtraEmitCodeLine(
          "const String " + funcName + "_impl = String::printf(" +
    ...
    this->addExtraEmitCodeLine(String::printf(
        ...
        glsltype_string(fContext, decl.returnType()), funcName.c_str(), funcName.c_str(),
        decl.parameters().size(), funcImpl.c_str()));
  }
}
...
void CPPCodeGenerator::writeProgramElement(const ProgramElement& p) {
  switch (p.kind()) {
    case ProgramElement::Kind::kSection: return;
...
void CPPCodeGenerator::addUniform(const Variable& var) {
  ...
  if (var.type().typeKind() != Type::TypeKind::kArray) {
    this->writef(
        "        %sVar = args.fUniformHandler->addUniform(&_outer, "
        "kFragment_GrShaderFlag, %s, \"%s\");\n",
        HCodeGenerator::FieldName(name.c_str()).c_str(), glsltype_string(fContext, var.type()),
        name.c_str());
  } else {
    this->writef(
        "        %sVar = args.fUniformHandler->addUniformArray(&_outer, "
        "kFragment_GrShaderFlag, %s, \"%s\", %d);\n",
        HCodeGenerator::FieldName(name.c_str()).c_str(),
        glsltype_string(fContext, var.type().componentType()), name.c_str(), var.type().columns());
  }
  if (var.modifiers().fLayout.fWhen.fLength) {
    this->write("        }\n");
  }
  ...
  for (const auto& p : fProgram.elements()) {
    if (p->is<GlobalVarDeclaration>()) {
      ...
      if (is_private(var)) {
        ...
        this->writef(
            "%s %s = %s;\n",
            HCodeGenerator::FieldType(fContext, var.type(), var.modifiers().fLayout).c_str(),
            String(var.name()).c_str(), default_value(var).c_str());
...
static bool is_accessible(const Variable& var) {
  ...
  return Type::TypeKind::kSampler != type.typeKind() && Type::TypeKind::kOther != type.typeKind();
}
  ...
  while (i < sksl.size()) {
    if (tokenStart >= 0) {
      ...
      if (sksl[i] == '}') {
        ...
        String compacted = String(sksl.c_str() + flushPoint + 1, tokenStart - flushPoint - 1);
        if (i < sksl.size() - 1) {
...
String CPPCodeGenerator::convertSKSLExpressionToCPP(const Expression& e, const String& cppVar) {
...
bool CPPCodeGenerator::writeEmitCode(std::vector<const Variable*>& uniforms) {
  ...
  for (const auto& p : fProgram.elements()) {
    if (p->is<GlobalVarDeclaration>()) {
      ...
      if (SectionAndParameterHelper::IsParameter(decl.var()) && is_accessible(decl.var())) {
        this->writef(
            "        auto %s = _outer.%s;\n"
            "        (void) %s;\n",
            name, name, name);
...
void CPPCodeGenerator::writeSetData(std::vector<const Variable*>& uniforms) {
  ...
  this->writef(
      "    void onSetData(const GrGLSLProgramDataManager& %s, "
  ...
  for (const Variable* u : uniforms) {
    if (is_uniform_in(*u)) {
      ...
      if (needsValueDeclaration) {
        ...
        String valueType = HCodeGenerator::AccessType(fContext, u->type(), u->modifiers().fLayout);
        this->writef(
      ...
      if (isTracked) {
        ...
        this->writef(
            ...
            "%s    %s;\n"
            "%s}\n",
            indent.c_str(), mapper->dirtyExpression(valueVar, prevVar).c_str(), indent.c_str(),
    ...
    for (const auto& p : fProgram.elements()) {
      if (p->is<GlobalVarDeclaration>()) {
        ...
        if (variable.type().typeKind() == Type::TypeKind::kSampler) {
          this->writef(
              "        const GrSurfaceProxyView& %sView = "
          ...
          this->writef(
              "        UniformHandle& %s = %sVar;\n"
              "        (void) %s;\n",
              name, HCodeGenerator::FieldName(name).c_str(), name);
        } else if (
            SectionAndParameterHelper::IsParameter(variable) &&
    ...
    if (param->type().typeKind() == Type::TypeKind::kSampler) {
      if (!foundSampler) {
        this->writef(
            ...
            fFullName.c_str());
        this->writef(
            "    return IthTextureSampler(index, %s",
            HCodeGenerator::FieldName(String(param->name()).c_str()).c_str());
        foundSampler = true;
      } else {
        this->writef(", %s", HCodeGenerator::FieldName(String(param->name()).c_str()).c_str());
      }
    }
    }
    if (foundSampler) {
...
void CPPCodeGenerator::writeClone() {
  if (!this->writeSection(kCloneSection)) {
    if (fSectionAndParameterHelper.getSection(kFieldsSection)) {
      fErrors.error(
          /*offset=*/0,
          "fragment processors with custom @fields must also have a "
          "custom @clone");
    }
    this->writef(
        "%s::%s(const %s& src)\n"
        ": INHERITED(k%s_ClassID, src.optimizationFlags())",
...
void CPPCodeGenerator::writeDumpInfo() {
  ...
  if (!this->writeSection(kDumpInfoSection)) {
    ...
    for (const Variable* param : fSectionAndParameterHelper.getParameters()) {
      ...
      String fieldName = HCodeGenerator::FieldName(String(param->name()).c_str());
      String runtimeValue = this->formatRuntimeValue(
          param->type(), param->modifiers().fLayout, param->name(), &argumentList);
      formatString.appendf(
          "%s%s=%s", formatString.empty() ? "" : ", ", fieldName.c_str(), runtimeValue.c_str());
    }
...
void CPPCodeGenerator::writeTest() {
  ...
  if (test) {
    this->writef(
        ...
        fFullName.c_str(), fFullName.c_str(), test->argument().c_str());
...
void CPPCodeGenerator::writeGetKey() {
  this->writef(
      "void %s::onGetGLSLProcessorKey(const GrShaderCaps& caps, "
      ...
      switch (var.modifiers().fLayout.fKey) {
        case Layout::kKey_Key:
          if (is_private(var)) {
            this->writef(
                "%s %s =",
                HCodeGenerator::FieldType(fContext, varType, var.modifiers().fLayout).c_str(),
          ...
          if (varType == *fContext.fHalf4_Type) {
            this->writef(
                "    uint16_t red = SkFloatToHalf(%s.fR);\n",
                HCodeGenerator::FieldName(name).c_str());
            this->writef(
                "    uint16_t green = SkFloatToHalf(%s.fG);\n",
                HCodeGenerator::FieldName(name).c_str());
            this->writef(
                "    uint16_t blue = SkFloatToHalf(%s.fB);\n",
                HCodeGenerator::FieldName(name).c_str());
            this->writef(
                "    uint16_t alpha = SkFloatToHalf(%s.fA);\n",
                HCodeGenerator::FieldName(name).c_str());
            this->write("    b->add32(((uint32_t)red << 16) | green);\n");
            this->write("    b->add32(((uint32_t)blue << 16) | alpha);\n");
          } else if (varType == *fContext.fHalf_Type || varType == *fContext.fFloat_Type) {
            this->writef(
                "    b->add32(sk_bit_cast<uint32_t>(%s));\n",
                HCodeGenerator::FieldName(name).c_str());
          } else if (
              varType.isInteger() || varType == *fContext.fBool_Type ||
              varType.typeKind() == Type::TypeKind::kEnum) {
            this->writef("    b->add32((uint32_t) %s);\n", HCodeGenerator::FieldName(name).c_str());
          } else {
            ABORT(
                "NOT YET IMPLEMENTED: automatic key handling for %s\n",
        ...
        case Layout::kIdentity_Key:
          ...
          this->writef(
              "    b->add32(%s.isIdentity() ? 1 : 0);\n", HCodeGenerator::FieldName(name).c_str());
          break;
        case Layout::kNo_Key: break;
      }
    }
  }
  this->write("}\n");
...
bool CPPCodeGenerator::generateCode() {
    ...
    if (p->is<GlobalVarDeclaration>()) {
      ...
      if (is_uniform_in(decl.var())) {
        ...
        if (mapper == nullptr) {
          fErrors.error(
              decl.fOffset,
              String(decl.var().name()) + "'s type is not supported for use as a 'uniform in'");
          return false;
        }
        if (decl.var().modifiers().fLayout.fFlags & Layout::kTracked_Flag) {
          if (!mapper->supportsTracking()) {
            fErrors.error(
                decl.fOffset,
                String(decl.var().name()) + "'s type does not support state tracking");
            return false;
          }
        }
  ...
  this->writeGetKey();
  this->writef(
      "bool %s::onIsEqual(const GrFragmentProcessor& other) const {\n"
  ...
  this->write(
      ...
      "}\n");
  this->writef(
      "bool %s::usesExplicitReturn() const {\n"
      "    return %s;\n"
      "}\n",
