...
void CPPCodeGenerator::writeBinaryExpression(
  ...
  if (b.fOperator == Token::Kind::TK_PERCENT) {
    ...
    if (precedence >= parentPrecedence) {
      ...
      b.fLeft->fKind == Expression::kNullLiteral_Kind ||
    ...
    SkASSERT(
        var->fType.kind() == Type::kNullable_Kind &&
    ...
    fFormatArgs.push_back(
        String(op) + "_outer.childProcessor(" + to_string(childIndex) + ") ? \"true\" : \"false\"");
  } else {
  ...
  if (base.fKind == Expression::kVariableReference_Kind) {
    ...
    if (SK_TEXTURESAMPLERS_BUILTIN == builtin) {
      ...
      if (i.fIndex->fKind != Expression::kIntLiteral_Kind) {
        fErrors.error(
            i.fIndex->fOffset, "index into sk_TextureSamplers must be an integer literal");
        return;
      }
...
static bool is_uniform_in(const Variable& var) {
  return (var.fModifiers.fFlags & Modifiers::kUniform_Flag) &&
         (var.fModifiers.fFlags & Modifiers::kIn_Flag) && var.fType.kind() != Type::kSampler_Kind;
}
      ...
      if (ref.fVariable.fModifiers.fFlags & Modifiers::kUniform_Flag) {
        ...
        if (ref.fVariable.fModifiers.fLayout.fWhen.fLength) {
          code = String::printf(
              ...
              var.c_str(), default_value(ref.fVariable.fType).c_str());
        } else {
          code = var;
        }
        ...
        this->writeRuntimeValue(
            ref.fVariable.fType, ref.fVariable.fModifiers.fLayout,
    ...
    if (ProgramElement::kVar_Kind == p.fKind) {
      ...
      for (const auto& raw : decls.fVars) {
        ...
        if (is_private(*decl.fVar)) {
          ...
          this->writef(
              "%s %s = %s;\n",
              HCodeGenerator::FieldType(fContext, decl.fVar->fType, decl.fVar->fModifiers.fLayout)
                  .c_str(),
...
static bool is_accessible(const Variable& var) {
  ...
  return Type::kSampler_Kind != type.kind() && Type::kOther_Kind != type.kind();
}
  ...
  for (const Variable* u : uniforms) {
    if (is_uniform_in(*u)) {
      ...
      if (needsValueDeclaration) {
        ...
        String valueType = HCodeGenerator::AccessType(fContext, u->fType, u->fModifiers.fLayout);
        this->writef(
  ...
  if (!this->writeSection(kDumpInfoSection)) {
    ...
    for (const Variable* param : fSectionAndParameterHelper.getParameters()) {
      ...
      String runtimeValue = this->formatRuntimeValue(
          param->fType, param->fModifiers.fLayout, param->fName, &argumentList);
        ...
        switch (var.fModifiers.fLayout.fKey) {
          case Layout::kKey_Key:
            if (is_private(var)) {
              this->writef(
                  "%s %s =",
                  HCodeGenerator::FieldType(fContext, var.fType, var.fModifiers.fLayout).c_str(),
            ...
            } else if (var.fType == *fContext.fHalf_Type || var.fType == *fContext.fFloat_Type) {
              this->writef(
                  "    b->add32(sk_bit_cast<uint32_t>(%s));\n",
                  HCodeGenerator::FieldName(name).c_str());
            } else if (
                var.fType.isInteger() || var.fType == *fContext.fBool_Type ||
