...
String CPPCodeGenerator::formatRuntimeValue(
  ...
  if (type.isMatrix()) {
    SkASSERT(
        type.componentType() == *fContext.fTypes.fFloat ||
  ...
  if (type == *fContext.fTypes.fInt4 || type == *fContext.fTypes.fShort4 ||
...
void CPPCodeGenerator::writeIntLiteral(const IntLiteral& i) { this->write(to_string(i.value())); }
...
void CPPCodeGenerator::writeFieldAccess(const FieldAccess& access) {
  if (access.base()->type().name() == "fragmentProcessor") {
    ...
    const Type::Field& field = fContext.fTypes.fFragmentProcessor->fields()[access.fieldIndex()];
  ...
  while (i < sksl.size()) {
    if (tokenStart >= 0) {
      ...
      if (sksl[i] == '}') {
        ...
        SKSL_INT codeBlock;
        SkAssertResult(
            stoi(StringFragment(sksl.c_str() + tokenStart + 2, i - tokenStart - 2), &codeBlock));
    ...
    if (p->is<GlobalVarDeclaration>()) {
      ...
      switch (var.modifiers().fLayout.fKey) {
        case Layout::kKey_Key:
          ...
          } else if (varType == *fContext.fTypes.fHalf || varType == *fContext.fTypes.fFloat) {
            this->writef(
                "    b->add32(sk_bit_cast<uint32_t>(%s));\n",
                HCodeGenerator::FieldName(name).c_str());
          } else if (
              varType.isInteger() || varType == *fContext.fTypes.fBool ||
