      ...
      case ProgramElement::Kind::kSection: {
        ...
        if (IsSupportedSection(name.c_str())) {
          if (SectionRequiresArgument(name.c_str()) && !arg.size()) {
            errors.error(s.fOffset, ("section '@" + name + "' requires one parameter").c_str());
          }
          if (!SectionAcceptsArgument(name.c_str()) && arg.size()) {
            errors.error(s.fOffset, ("section '@" + name + "' has no parameters").c_str());
          }
        } else {
          errors.error(s.fOffset, ("unsupported section '@" + name + "'").c_str());
        }
        if (!SectionPermitsDuplicates(name.c_str()) && fSections.find(name) != fSections.end()) {
