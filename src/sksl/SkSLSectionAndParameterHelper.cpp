      ...
      case ProgramElement::kSection_Kind: {
        ...
        if (IsSupportedSection(s.fName.c_str())) {
          if (SectionRequiresArgument(s.fName.c_str()) && !s.fArgument.size()) {
            errors.error(s.fOffset, ("section '@" + s.fName + "' requires one parameter").c_str());
          }
          if (!SectionAcceptsArgument(s.fName.c_str()) && s.fArgument.size()) {
            errors.error(s.fOffset, ("section '@" + s.fName + "' has no parameters").c_str());
          }
        } else {
          errors.error(s.fOffset, ("unsupported section '@" + s.fName + "'").c_str());
        }
        if (!SectionPermitsDuplicates(s.fName.c_str()) &&
            fSections.find(s.fName) != fSections.end()) {
          errors.error(s.fOffset, ("duplicate section '@" + s.fName + "'").c_str());
        }
...
bool SectionAndParameterHelper::hasCoordOverrides(const Expression& e, const Variable& fp) {
  switch (e.fKind) {
    ...
    case Expression::kBinary_Kind: {
      ...
      return this->hasCoordOverrides(*b.fLeft, fp) || this->hasCoordOverrides(*b.fRight, fp);
    }
    case Expression::kIndex_Kind: {
      const IndexExpression& idx = (const IndexExpression&)e;
      return this->hasCoordOverrides(*idx.fBase, fp) || this->hasCoordOverrides(*idx.fIndex, fp);
    }
    ...
    case Expression::kTernary_Kind: {
      ...
      return this->hasCoordOverrides(*t.fTest, fp) || this->hasCoordOverrides(*t.fIfTrue, fp) ||
             this->hasCoordOverrides(*t.fIfFalse, fp);
    }
    case Expression::kVariableReference_Kind: return false;
    ...
    case Expression::kTypeReference_Kind: return false;
  }
  SkASSERT(false);
  return false;
}
...
bool SectionAndParameterHelper::hasCoordOverrides(const Statement& s, const Variable& fp) {
  switch (s.fKind) {
    ...
    case Statement::kIf_Kind: {
      ...
      return this->hasCoordOverrides(*i.fTest, fp) || this->hasCoordOverrides(*i.fIfTrue, fp) ||
    ...
    case Statement::kFor_Kind: {
      ...
      return this->hasCoordOverrides(*f.fInitializer, fp) ||
             this->hasCoordOverrides(*f.fTest, fp) || this->hasCoordOverrides(*f.fNext, fp) ||
             this->hasCoordOverrides(*f.fStatement, fp);
    }
    case Statement::kWhile_Kind: {
      const WhileStatement& w = (const WhileStatement&)s;
      return this->hasCoordOverrides(*w.fTest, fp) || this->hasCoordOverrides(*w.fStatement, fp);
    }
    case Statement::kDo_Kind: {
      const DoStatement& d = (const DoStatement&)s;
      return this->hasCoordOverrides(*d.fTest, fp) || this->hasCoordOverrides(*d.fStatement, fp);
    }
    ...
    case Statement::kNop_Kind: return false;
  }
  SkASSERT(false);
  return false;
}
...
SampleMatrix SectionAndParameterHelper::getMatrix(const Expression& e, const Variable& fp) {
  switch (e.fKind) {
    case Expression::kFunctionCall_Kind: {
      ...
      if (f.fBuiltin && f.fName == "sample" && fc.fArguments.size() >= 2 &&
        ...
        if (fc.fArguments.back()->isConstantOrUniform()) {
          return SampleMatrix(
              SampleMatrix::Kind::kConstantOrUniform, nullptr, fc.fArguments.back()->description());
        } else {
    ...
    case Expression::kTernary_Kind: {
      const TernaryExpression& t = (const TernaryExpression&)e;
      return this->getMatrix(*t.fTest, fp)
          .merge(this->getMatrix(*t.fIfTrue, fp))
          .merge(this->getMatrix(*t.fIfFalse, fp));
    }
    case Expression::kVariableReference_Kind: return SampleMatrix();
    case Expression::kBoolLiteral_Kind:
    case Expression::kDefined_Kind:
    case Expression::kExternalFunctionCall_Kind:
    case Expression::kExternalValue_Kind:
    ...
    case Expression::kNullLiteral_Kind:
    case Expression::kSetting_Kind:
    case Expression::kTypeReference_Kind: return SampleMatrix();
  }
  SkASSERT(false);
  return SampleMatrix();
}
...
SampleMatrix SectionAndParameterHelper::getMatrix(const Statement& s, const Variable& fp) {
  switch (s.fKind) {
    ...
    case Statement::kIf_Kind: {
      const IfStatement& i = (const IfStatement&)s;
      return this->getMatrix(*i.fTest, fp)
          .merge(this->getMatrix(*i.fIfTrue, fp))
          .merge((i.fIfFalse ? this->getMatrix(*i.fIfFalse, fp) : SampleMatrix()));
    }
    case Statement::kFor_Kind: {
      const ForStatement& f = (const ForStatement&)s;
      return this->getMatrix(*f.fInitializer, fp)
          .merge(
              this->getMatrix(*f.fTest, fp)
                  .merge(this->getMatrix(*f.fNext, fp).merge(this->getMatrix(*f.fStatement, fp))));
    }
    case Statement::kWhile_Kind: {
      const WhileStatement& w = (const WhileStatement&)s;
    ...
    case Statement::kDiscard_Kind:
    case Statement::kGroup_Kind:
    case Statement::kNop_Kind: return SampleMatrix();
  }
  SkASSERT(false);
  return SampleMatrix();
}
