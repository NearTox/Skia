  ...
  switch (operand.kind()) {
    case Expression::Kind::kFloatLiteral:
      ...
      return std::make_unique<FloatLiteral>(
          operand.fOffset, -operand.as<FloatLiteral>().value(), &operand.type());

    case Expression::Kind::kIntLiteral:
      // Convert -intLiteral(1) to intLiteral(-1).
      return std::make_unique<IntLiteral>(
          operand.fOffset, -operand.as<IntLiteral>().value(), &operand.type());

    default:
      // Convert Expr to Prefix(TK_MINUS, Expr).
      return std::make_unique<PrefixExpression>(Token::Kind::TK_MINUS, operand.clone());
  }
}

std::unique_ptr<Expression> PrefixExpression::constantPropagate(
  ...
  if (this->operand()->isCompileTimeConstant()) {
    if (this->getOperator() == Token::Kind::TK_MINUS) {
      ...
      switch (this->operand()->kind()) {
        case Expression::Kind::kFloatLiteral:
        case Expression::Kind::kIntLiteral: return negate_operand(*this->operand());
        ...
        case Expression::Kind::kConstructor: {
          ...
          return std::make_unique<Constructor>(
              constructor.fOffset, &constructor.type(), std::move(args));
        }
