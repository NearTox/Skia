...
static ExpressionArray negate_operands(
    const Context& context, Position pos, const ExpressionArray& operands);

static std::unique_ptr<Expression> simplify_negation(
    const Context& context, Position pos, const Expression& originalExpr) {
  ...
  switch (value->kind()) {
    ...
    case Expression::Kind::kConstructorArray:
      ...
      if (value->isCompileTimeConstant()) {
        ...
        return ConstructorArray::Make(
            context, pos, ctor.type(), negate_operands(context, pos, ctor.arguments()));
      }
      break;
    ...
    case Expression::Kind::kConstructorDiagonalMatrix:
      ...
      if (value->isCompileTimeConstant()) {
        ...
        if (std::unique_ptr<Expression> simplified =
                simplify_negation(context, pos, *ctor.argument())) {
          return ConstructorDiagonalMatrix::Make(context, pos, ctor.type(), std::move(simplified));
        }
      }
      break;
...
static ExpressionArray negate_operands(
    const Context& context, Position pos, const ExpressionArray& array) {
  ...
  for (const std::unique_ptr<Expression>& expr : array) {
    ...
    if (std::unique_ptr<Expression> simplified = simplify_negation(context, pos, *expr)) {
      ...
      replacement.push_back(
          std::make_unique<PrefixExpression>(pos, Operator::Kind::MINUS, expr->clone()));
    }
  }
  return replacement;
}

static std::unique_ptr<Expression> negate_operand(
    const Context& context, Position pos, std::unique_ptr<Expression> value) {
...
static std::unique_ptr<Expression> logical_not_operand(
    const Context& context, Position pos, std::unique_ptr<Expression> operand) {
...
std::unique_ptr<Expression> PrefixExpression::Convert(
    const Context& context, Position pos, Operator op, std::unique_ptr<Expression> base) {
  const Type& baseType = base->type();
  switch (op.kind()) {
    case Operator::Kind::PLUS:
      if (baseType.isArray() || !baseType.componentType().isNumber()) {
        context.fErrors->error(pos, "'+' cannot operate on '" + baseType.displayName() + "'");
        return nullptr;
      }
      break;
    ...
    case Operator::Kind::MINUSMINUS:
      if (!baseType.isNumber()) {
        context.fErrors->error(
            pos, "'" + std::string(op.tightOperatorName()) + "' cannot operate on '" +
                     baseType.displayName() + "'");
        return nullptr;
      }
      if (!Analysis::UpdateVariableRefKind(
              base.get(), VariableReference::RefKind::kReadWrite, context.fErrors)) {
        return nullptr;
      }
      break;
    ...
    default: SK_ABORT("unsupported prefix operator");
  }
...
std::unique_ptr<Expression> PrefixExpression::Make(
    const Context& context, Position pos, Operator op, std::unique_ptr<Expression> base) {
  switch (op.kind()) {
    case Operator::Kind::PLUS:
    ...
    default: SkDEBUGFAILF("unsupported prefix operator: %s", op.operatorName());
  }
