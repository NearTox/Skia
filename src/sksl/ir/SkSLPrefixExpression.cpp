...
static std::unique_ptr<Expression> simplify_negation(
    const Context& context, const Expression& originalExpr) {
  ...
  switch (value->kind()) {
    case Expression::Kind::kFloatLiteral:
      ...
      return FloatLiteral::Make(
          originalExpr.fOffset, -value->as<FloatLiteral>().value(), &value->type());

    case Expression::Kind::kIntLiteral:
      // Convert -intLiteral(1) to intLiteral(-1).
      return IntLiteral::Make(
          originalExpr.fOffset, -value->as<IntLiteral>().value(), &value->type());
    ...
    case Expression::Kind::kConstructorArray:
      ...
      if (context.fConfig->fSettings.fOptimize && value->isCompileTimeConstant()) {
        ...
        return ConstructorArray::Make(
            context, originalExpr.fOffset, ctor.type(), negate_operands(context, ctor.arguments()));
      }
      break;
    ...
    case Expression::Kind::kConstructorDiagonalMatrix:
      ...
      if (context.fConfig->fSettings.fOptimize && value->isCompileTimeConstant()) {
        ...
        return ConstructorDiagonalMatrix::Make(
            context, originalExpr.fOffset, ctor.type(),
  ...
  for (const std::unique_ptr<Expression>& expr : array) {
    ...
    if (std::unique_ptr<Expression> simplified = simplify_negation(context, *expr)) {
      ...
      replacement.push_back(
          std::make_unique<PrefixExpression>(Token::Kind::TK_MINUS, expr->clone()));
    }
  }
  return replacement;
}
...
static std::unique_ptr<Expression> negate_operand(
    const Context& context, std::unique_ptr<Expression> value) {
...
static std::unique_ptr<Expression> logical_not_operand(
    const Context& context, std::unique_ptr<Expression> operand) {
...
std::unique_ptr<Expression> PrefixExpression::Convert(
    const Context& context, Operator op, std::unique_ptr<Expression> base) {
  ...
  switch (op.kind()) {
    ...
    case Token::Kind::TK_MINUSMINUS:
      if (!baseType.isNumber()) {
        context.fErrors->error(
            base->fOffset, String("'") + op.operatorName() + "' cannot operate on '" +
    ...
    case Token::Kind::TK_BITWISENOT:
      if (context.fConfig->strictES2Mode()) {
        ...
        context.fErrors->error(
            base->fOffset, String("operator '") + op.operatorName() + "' is not allowed");
        return nullptr;
      }
...
std::unique_ptr<Expression> PrefixExpression::Make(
    const Context& context, Operator op, std::unique_ptr<Expression> base) {
  switch (op.kind()) {
    case Token::Kind::TK_PLUS: SkASSERT(base->type().componentType().isNumber()); return base;
