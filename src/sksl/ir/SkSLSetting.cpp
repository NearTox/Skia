...
class CapsLookupMethod {
 public:
  virtual ~CapsLookupMethod() = default;
...
class BoolCapsLookup : public CapsLookupMethod {
 public:
  ...
  const Type* type(const Context& context) const override { return context.fTypes.fBool.get(); }
...
class IntCapsLookup : public CapsLookupMethod {
 public:
  ...
  const Type* type(const Context& context) const override { return context.fTypes.fInt.get(); }
  std::unique_ptr<Expression> value(const Context& context) const override {
...
#    name, new T##CapsLookup { &ShaderCapsClass::name } \
  }
...
static std::unique_ptr<Expression> get_value(
    const Context& context, int offset, const skstd::string_view& name) {
  if (const CapsLookupMethod* caps = caps_lookup_table().lookup(name)) {
