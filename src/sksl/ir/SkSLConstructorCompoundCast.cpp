...
static std::unique_ptr<Expression> cast_constant_composite(
    const Context& context, const Type& destType, std::unique_ptr<Expression> constCtor) {
  ...
  if (constCtor->is<ConstructorSplat>()) {
    ...
    return ConstructorSplat::Make(
        ...
        ConstructorScalarCast::Make(
            context, constCtor->fOffset, scalarType, std::move(splat.argument())));
  }
  ...
  if (constCtor->is<ConstructorDiagonalMatrix>() && destType.isMatrix()) {
    ...
    return ConstructorDiagonalMatrix::Make(
        context, constCtor->fOffset, destType,
        ConstructorScalarCast::Make(
            context, constCtor->fOffset, scalarType, std::move(matrixCtor.argument())));
  }
  ...
  for (size_t index = 0; index < numSlots; ++index) {
    ...
    typecastArgs.push_back(
        ConstructorScalarCast::Make(context, constCtor->fOffset, scalarType, arg->clone()));
  }

  return ConstructorCompound::Make(context, constCtor->fOffset, destType, std::move(typecastArgs));
}

std::unique_ptr<Expression> ConstructorCompoundCast::Make(
    const Context& context, int offset, const Type& type, std::unique_ptr<Expression> arg) {
