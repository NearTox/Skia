...
static std::unique_ptr<Expression> cast_scalar_literal(
    const Type& constructorType, const Expression& expr) {
...
std::unique_ptr<Expression> ConstructorScalarCast::Convert(
    const Context& context, int offset, const Type& rawType, ExpressionArray args) {
  ...
  if (!argType.isScalar()) {
    ...
    context.fErrors->error(
        offset, "'" + argType.displayName() + "' is not a valid parameter to '" +
...
std::unique_ptr<Expression> ConstructorScalarCast::Make(
