...
std::unique_ptr<Expression> ConstructorScalarCast::Convert(
    const Context& context, Position pos, const Type& rawType, ExpressionArray args) {
  ...
  if (args.size() != 1) {
    context.fErrors->error(
        pos, "invalid arguments to '" + type.displayName() +
  ...
  if (!argType.isScalar()) {
    ...
    context.fErrors->error(
        pos, "'" + argType.displayName() + "' is not a valid parameter to '" + type.displayName() +
