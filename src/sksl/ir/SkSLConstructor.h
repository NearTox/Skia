...
struct Constructor : public Expression {
    ...
    if (fArguments.size() == 1 && fArguments[0]->fKind == Expression::kIntLiteral_Kind) {
      if (fType.isFloat()) {
        ...
        return std::unique_ptr<Expression>(
            new FloatLiteral(irGenerator.fContext, fOffset, intValue));
      ...
      switch (arg->fKind) {
        ...
        case kPrefix_Kind: {
          ...
          if (current + prefix.fType.columns() > index) {
            ...
            const Constructor& constructor = static_cast<const Constructor&>(*prefix.fOperand);
        ...
        default: {
          SkDEBUGFAILF(
              "unexpected component %d { %s } in %s\n", index, arg->description().c_str(),
              description().c_str());
          break;
        }
      }
  ...
  SKSL_FLOAT getFVecComponent(int n) const override { return this->getVecComponent<SKSL_FLOAT>(n); }

  /**
   * For a literal vector expression, return the integer value of the n'th vector component. It is
   * an error to call this method on an expression which is not a literal vector.
   */
  SKSL_INT getIVecComponent(int n) const override { return this->getVecComponent<SKSL_INT>(n); }
