...
class FunctionDefinition final : public ProgramElement {
 public:
  ...
  FunctionDefinition(
      int offset, const FunctionDeclaration* declaration, bool builtin,
  ...
  const ASTNode* source() const { return fSource; }

  void setSource(const ASTNode* source) { fSource = source; }

  std::unique_ptr<ProgramElement> clone() const override {
    return std::make_unique<FunctionDefinition>(
        fOffset, &this->declaration(),
        /*builtin=*/false, this->body()->clone(), this->referencedIntrinsics());
  }
