...
struct Program {
  ...
  struct Inputs {
    ...
    bool operator==(const Inputs& that) const noexcept {
      return fUseFlipRTUniform == that.fUseFlipRTUniform;
    }
    bool operator!=(const Inputs& that) const noexcept { return !(*this == that); }
  };
  ...
  class ElementsCollection {
   public:
    class iterator {
     public:
      const ProgramElement* operator*() noexcept {
   ...
   private:
    ...
    ElementsCollection(const Program& program) noexcept : fProgram(program) {}
