...
class Enum final : public ProgramElement {
 public:
  ...
  StringFragment typeName() const { return fTypeName; }

  std::shared_ptr<SymbolTable> symbols() const { return fSymbols; }

  bool isBuiltin() const { return fIsBuiltin; }

  bool isSharedWithCpp() const { return fIsSharedWithCpp; }

  std::unique_ptr<ProgramElement> clone() const override {
    return std::make_unique<Enum>(
        fOffset, this->typeName(), SymbolTable::WrapIfBuiltin(this->symbols()),
  ...
  String code() const {
    ...
    this->symbols()->foreach (
        [&](StringFragment, const Symbol* symbol) { sortedSymbols.push_back(symbol); });
