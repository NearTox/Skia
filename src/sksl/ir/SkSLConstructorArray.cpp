...
std::unique_ptr<Expression> ConstructorArray::Convert(
  ...
  if (context.fConfig->strictES2Mode()) {
    context.fErrors->error(
        pos, "construction of array type '" + type.displayName() + "' is not supported");
    return nullptr;
  }
...
std::unique_ptr<Expression> ConstructorArray::Make(
    const Context& context, Position pos, const Type& type, ExpressionArray args) {
