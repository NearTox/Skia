...
class FunctionReference final : public Expression {
 public:
  ...
  FunctionReference(
      const Context& context, Position pos, std::vector<const FunctionDeclaration*> functions)
  ...
  std::unique_ptr<Expression> clone(Position pos) const override {
    return std::unique_ptr<Expression>(
        new FunctionReference(pos, this->functions(), &this->type()));
  }

  std::string description() const override { return "<function>"; }

 private:
  FunctionReference(
      Position pos, std::vector<const FunctionDeclaration*> functions, const Type* type)
      : INHERITED(pos, kExpressionKind, type), fFunctions(std::move(functions)) {}
