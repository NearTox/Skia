...
std::unique_ptr<Statement> VarDeclaration::Convert(
  ...
  if (value) {
    ...
    if (var->modifiers().fFlags & Modifiers::kIn_Flag) {
      context.fErrors->error(value->fOffset, "'in' variables cannot use initializer expressions");
      return nullptr;
    }
    if (var->modifiers().fFlags & Modifiers::kUniform_Flag) {
      context.fErrors->error(
          value->fOffset, "'uniform' variables cannot use initializer expressions");
      return nullptr;
    }
    if (var->storage() == Variable::Storage::kInterfaceBlock) {
      context.fErrors->error(
          value->fOffset, "initializers are not permitted on interface block fields");
      return nullptr;
    }
  ...
  if (var->storage() == Variable::Storage::kInterfaceBlock) {
    if (var->type().isOpaque()) {
      context.fErrors->error(
          var->fOffset,
          "opaque type '" + var->type().name() + "' is not permitted in an interface block");
      return nullptr;
    }
  }
