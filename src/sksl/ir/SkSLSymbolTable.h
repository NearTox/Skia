...
class SymbolTable {
 public:
  SymbolTable(const Context& context, bool builtin) noexcept
      : fBuiltin(builtin), fContext(context) {}

  SymbolTable(std::shared_ptr<SymbolTable> parent, bool builtin) noexcept 
  ...
  static void Push(std::shared_ptr<SymbolTable>* table) { Push(table, (*table)->isBuiltin()); }
   ...
   */
  static void Pop(std::shared_ptr<SymbolTable>* table) noexcept { *table = (*table)->fParent; }
  ...
  void foreach (Fn&& fn) const {
    fSymbols.foreach ([&fn](const SymbolKey& key, const Symbol* symbol) { fn(key.fName, symbol); });
  }
   ...
   */
  SkSL::SymbolTable* builtinParent() { return this->isBuiltin() ? this : fParent->builtinParent(); }
 ...
 private:
  struct SymbolKey {
    ...
    bool operator==(const SymbolKey& that) const noexcept { return fName == that.fName; }
    bool operator!=(const SymbolKey& that) const noexcept { return fName != that.fName; }
    struct Hash {
      uint32_t operator()(const SymbolKey& key) const noexcept { return key.fHash; }
    };
  };

  static SymbolKey MakeSymbolKey(std::string_view name) noexcept {
...
class AutoSymbolTable {
 public:
  AutoSymbolTable(std::shared_ptr<SymbolTable>* s) : fSymbolTable(s) {
