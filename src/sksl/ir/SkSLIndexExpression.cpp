...
static bool index_out_of_range(
    const Context& context, Position pos, SKSL_INT index, const Expression& base) {
  if (index >= 0) {
  ...
  context.fErrors->error(
      pos,
      "index " + std::to_string(index) + " out of range for '" + base.type().displayName() + "'");
  return true;
}
...
std::unique_ptr<Expression> IndexExpression::Convert(
  ...
  if (base->is<TypeReference>()) {
    ...
    return TypeReference::Convert(
        context, pos, symbolTable.addArrayDimension(&baseType, arraySize));
  }
  ...
  if (!baseType.isArray() && !baseType.isMatrix() && !baseType.isVector()) {
    context.fErrors->error(
        base->fPosition, "expected array, but found '" + baseType.displayName() + "'");
    return nullptr;
  }
...
std::unique_ptr<Expression> IndexExpression::Make(
  ...
  if (indexExpr->isIntLiteral()) {
    ...
    if (!index_out_of_range(context, index->fPosition, indexValue, *base)) {
      if (baseType.isVector()) {
        ...
        return Swizzle::Make(context, pos, std::move(base), ComponentArray{(int8_t)indexValue});
      }
      ...
      if (baseType.isMatrix() && !base->hasSideEffects()) {
        ...
        for (int slot = 0; slot < vecWidth; ++slot) {
          ...
          if (slotVal.has_value()) {
            ctorArgs.push_back(Literal::Make(baseExpr->fPosition, *slotVal, &scalarType));
          } else {
            ctorArgs.reset();
            break;
          }
