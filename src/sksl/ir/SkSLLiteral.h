...
class Literal : public Expression {
 public:
  ...
  Literal(Position pos, double value, const Type* type)
      : INHERITED(pos, kExpressionKind, type), fValue(value) {}
  ...
  static std::unique_ptr<Literal> MakeInt(Position pos, SKSL_INT value, const Type* type) {
    ...
    SkASSERTF(
        value >= type->minimumValue(), "Value %" PRId64 " does not fit in type %s", value,
        type->description().c_str());
    SkASSERTF(
        value <= type->maximumValue(), "Value %" PRId64 " does not fit in type %s", value,
        type->description().c_str());
    return std::make_unique<Literal>(pos, value, type);
  }
  ...
  SKSL_INT intValue() const noexcept {
    SkASSERT(this->type().isInteger());
    return (SKSL_INT)fValue;
  }

  SKSL_INT boolValue() const noexcept {
  ...
  ComparisonResult compareConstant(const Expression& other) const override {
    ...
    return this->value() == other.as<Literal>().value() ? ComparisonResult::kEqual
