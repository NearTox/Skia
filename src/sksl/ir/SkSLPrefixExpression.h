...
class PrefixExpression final : public Expression {
 public:
  ...
  std::unique_ptr<Expression>& operand() noexcept { return fOperand; }

  const std::unique_ptr<Expression>& operand() const noexcept { return fOperand; }
  ...
  std::unique_ptr<Expression> clone(Position pos) const override {
    return std::make_unique<PrefixExpression>(pos, this->getOperator(), this->operand()->clone());
  }
