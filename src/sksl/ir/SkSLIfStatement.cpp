...
std::unique_ptr<Statement> IfStatement::clone() const {
  return std::make_unique<IfStatement>(
      fPosition, this->isStatic(), this->test()->clone(), this->ifTrue()->clone(),
...
std::unique_ptr<Statement> IfStatement::Convert(
  ...
  return IfStatement::Make(
      context, pos, isStatic, std::move(test), std::move(ifTrue), std::move(ifFalse));
}
...
std::unique_ptr<Statement> IfStatement::Make(
  ...
  return std::make_unique<IfStatement>(
      pos, isStatic, std::move(test), std::move(ifTrue), std::move(ifFalse));
}
