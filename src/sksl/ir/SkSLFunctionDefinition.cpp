...
static void append_rtadjust_fixup_to_vertex_main(
    const Context& context, const FunctionDeclaration& decl, Block& body) {
  ...
  if (rtAdjust.fVar || rtAdjust.fInterfaceBlock) {
    ...
    if (const ProgramElement* perVertexDecl = context.fBuiltins->find(Compiler::PERVERTEX_NAME)) {
    ...
    auto Field = [&](const Variable* var, int idx) -> std::unique_ptr<Expression> {
      return FieldAccess::Make(
          context, Position(), Ref(var), idx, OwnerKind::kAnonymousInterfaceBlock);
    };
    auto Pos = [&]() -> DSLExpression {
      return DSLExpression(FieldAccess::Make(
          context, Position(), Ref(skPerVertex),
          /*fieldIndex=*/0, OwnerKind::kAnonymousInterfaceBlock));
    };
    auto Adjust = [&]() -> DSLExpression {
      return DSLExpression(
          rtAdjust.fInterfaceBlock ? Field(rtAdjust.fInterfaceBlock, rtAdjust.fFieldIndex)
                                   : Ref(rtAdjust.fVar));
    };

    auto fixupStmt = DSLStatement(Pos().assign(Float4(
...
std::unique_ptr<FunctionDefinition> FunctionDefinition::Convert(
    const Context& context, Position pos, const FunctionDeclaration& function,
    std::unique_ptr<Statement> body, bool builtin) {
  class Finalizer : public ProgramWriter {
   public:
    Finalizer(
        const Context& context, const FunctionDeclaration& function,
    ...
    void copyBuiltinFunctionIfNeeded(const FunctionDeclaration& function) {
      if (const ProgramElement* found =
        ...
        std::sort(
            builtinFunctions.begin(), builtinFunctions.end(),
    ...
    bool functionReturnsValue() const { return !fFunction.returnType().isVoid(); }
    ...
    bool visitStatement(Statement& stmt) override {
      switch (stmt.kind()) {
        case Statement::Kind::kVarDeclaration: {
          ...
          fSlotsUsed =
              SkSafeMath::Add(fSlotsUsed, stmt.as<VarDeclaration>().var().type().slotCount());
          ...
          if (prevSlotsUsed < kVariableSlotLimit && fSlotsUsed >= kVariableSlotLimit) {
            fContext.fErrors->error(
                stmt.fPosition, "variable '" + std::string(stmt.as<VarDeclaration>().var().name()) +
        ...
        case Statement::Kind::kReturn: {
          ...
          if (returnStmt.expression()) {
            if (this->functionReturnsValue()) {
              ...
              fContext.fErrors->error(
                  returnStmt.expression()->fPosition,
            ...
            if (this->functionReturnsValue()) {
              ...
              fContext.fErrors->error(
                  returnStmt.fPosition,
                  "expected function to return '" + fFunction.returnType().displayName() + "'");
            }
          }
          break;
        }
        ...
        case Statement::Kind::kBreak:
          if (fBreakableLevel == 0) {
            fContext.fErrors->error(
                stmt.fPosition, "break statement must be inside a loop or switch");
          }
          break;
        case Statement::Kind::kContinue:
          if (fContinuableLevel.front() == 0) {
            if (std::any_of(fContinuableLevel.begin(), fContinuableLevel.end(), [](int level) {
                  return level > 0;
                })) {
              fContext.fErrors->error(
                  stmt.fPosition, "continue statement cannot be used in a switch");
            } else {
  ...
  if (Analysis::CanExitWithoutReturningValue(function, *body)) {
    context.fErrors->error(
        body->fPosition,
        "function '" + std::string(function.name()) + "' can exit without returning a value");
  }

  SkASSERTF(
      !function.isIntrinsic(), "Intrinsic %s should not have a definition",
      std::string(function.name()).c_str());
  return std::make_unique<FunctionDefinition>(
      pos, &function, builtin, std::move(body), std::move(referencedBuiltinFunctions));
}
