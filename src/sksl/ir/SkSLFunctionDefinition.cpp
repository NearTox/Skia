...
std::unique_ptr<FunctionDefinition> FunctionDefinition::Convert(
  ...
  class Finalizer : public ProgramWriter {
   public:
    Finalizer(
        ...
        : fContext(context), fFunction(function), fReferencedIntrinsics(referencedIntrinsics) {}
    ...
    bool visitStatement(Statement& stmt) override {
      switch (stmt.kind()) {
        case Statement::Kind::kReturn: {
          ...
          if (fContext.fConfig->fKind == ProgramKind::kVertex && fFunction.isMain()) {
            fContext.fErrors->error(
                stmt.fOffset, "early returns from vertex programs are not supported");
          }
          ...
          if (returnStmt.expression()) {
            if (this->functionReturnsValue()) {
              ...
              fContext.fErrors->error(
                  returnStmt.fOffset, "may not return a value from a void function");
            }
          } else {
            if (this->functionReturnsValue()) {
        ...
        case Statement::Kind::kBreak:
          if (!fBreakableLevel) {
            fContext.fErrors->error(
                stmt.fOffset, "break statement must be inside a loop or switch");
          }
          break;
        case Statement::Kind::kContinue:
          if (!fContinuableLevel) {
            fContext.fErrors->error(stmt.fOffset, "continue statement must be inside a loop");
          }
          break;
        default: break;
      }
  ...
  if (Analysis::CanExitWithoutReturningValue(function, *body)) {
    context.fErrors->error(
        function.fOffset, "function '" + function.name() + "' can exit without returning a value");
  }

  return std::make_unique<FunctionDefinition>(
      offset, &function, builtin, std::move(body), std::move(referencedIntrinsics));
}
