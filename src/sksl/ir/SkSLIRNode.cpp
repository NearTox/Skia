...
IRNode::IRNode(
    ...
    : fOffset(offset), fKind(kind), fData(data), fStatementChildren(std::move(stmts)) {}

IRNode::IRNode(int offset, int kind, const BoolLiteralData& data)
    : fOffset(offset), fKind(kind), fData(data) {}
...
IRNode::IRNode(const IRNode& other)
    : fOffset(other.fOffset), fKind(other.fKind), fData(other.fData) {
