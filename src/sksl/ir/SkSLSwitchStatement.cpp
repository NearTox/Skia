...
std::unique_ptr<Statement> SwitchStatement::clone() const {
  ...
  return std::make_unique<SwitchStatement>(
      fPosition, this->isStatic(), this->value()->clone(), std::move(cases),
...
static void move_all_but_break(std::unique_ptr<Statement>& stmt, StatementArray* target) {
  switch (stmt->kind()) {
    case Statement::Kind::kBlock: {
      ...
      target->push_back(Block::Make(
          block.fPosition, std::move(blockStmts), block.blockKind(), block.symbolTable()));
      break;
    }
...
std::unique_ptr<Statement> SwitchStatement::BlockForCase(
  ...
  return Block::Make(
      caseToCapture->fPosition, std::move(caseStmts), Block::Kind::kBracedScope,
      std::move(symbolTable));
}

std::unique_ptr<Statement> SwitchStatement::Convert(
    const Context& context, Position pos, bool isStatic, std::unique_ptr<Expression> value,
  ...
  for (int i = 0; i < caseValues.count(); ++i) {
    if (caseValues[i]) {
      ...
      std::unique_ptr<Expression> caseValue =
          value->type().coerceExpression(std::move(caseValues[i]), context);
      if (!caseValue) {
        return nullptr;
      }
      SKSL_INT intValue;
    ...
    for (const SwitchCase* sc : duplicateCases) {
      if (sc->isDefault()) {
        ...
        context.fErrors->error(
            sc->fPosition, "duplicate case value '" + std::to_string(sc->value()) + "'");
      }
    }
    return nullptr;
  }
  ...
  return SwitchStatement::Make(
      context, pos, isStatic, std::move(value), std::move(cases), std::move(symbolTable));
}
...
std::unique_ptr<Statement> SwitchStatement::Make(
  ...
  return std::make_unique<SwitchStatement>(
      pos, isStatic, std::move(value), std::move(cases), std::move(symbolTable));
}
