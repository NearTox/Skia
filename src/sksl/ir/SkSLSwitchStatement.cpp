...
std::unique_ptr<Statement> SwitchStatement::clone() const {
  ...
  return std::make_unique<SwitchStatement>(
      fOffset, this->isStatic(), this->value()->clone(), std::move(cases),
...
static void move_all_but_break(std::unique_ptr<Statement>& stmt, StatementArray* target) {
  switch (stmt->kind()) {
    case Statement::Kind::kBlock: {
      ...
      target->push_back(
          Block::Make(block.fOffset, std::move(blockStmts), block.symbolTable(), block.isScope()));
      break;
    }
...
std::unique_ptr<Statement> SwitchStatement::BlockForCase(
    StatementArray* cases, SwitchCase* caseToCapture, std::shared_ptr<SymbolTable> symbolTable) {
...
std::unique_ptr<Statement> SwitchStatement::Convert(
    const Context& context, int offset, bool isStatic, std::unique_ptr<Expression> value,
    ExpressionArray caseValues, StatementArray caseStatements,
  ...
  for (int i = 0; i < caseValues.count(); ++i) {
    ...
    cases.push_back(std::make_unique<SwitchCase>(
        caseOffset, std::move(caseValue), std::move(caseStatements[i])));
  }
    ...
    for (const SwitchCase* sc : duplicateCases) {
      if (sc->value() != nullptr) {
        context.fErrors->error(
            sc->fOffset, "duplicate case value '" + sc->value()->description() + "'");
      } else {
  ...
  return SwitchStatement::Make(
      context, offset, isStatic, std::move(value), std::move(cases), std::move(symbolTable));
}

std::unique_ptr<Statement> SwitchStatement::Make(
    const Context& context, int offset, bool isStatic, std::unique_ptr<Expression> value,
    StatementArray cases, std::shared_ptr<SymbolTable> symbolTable) {
  ...
  if (isStatic || context.fConfig->fSettings.fOptimize) {
    ...
    if (ConstantFolder::GetConstantInt(*value, &switchValue)) {
      ...
      if (isStatic && !context.fConfig->fSettings.fPermitInvalidStaticTests) {
        context.fErrors->error(
            value->fOffset, "static switch contains non-static conditional exit");
        return nullptr;
      }
    }
  }
  ...
  return std::make_unique<SwitchStatement>(
      offset, isStatic, std::move(value), std::move(cases), std::move(symbolTable));
}
