...
std::unique_ptr<Statement> ForStatement::clone() const {
  ...
  return std::make_unique<ForStatement>(
      fOffset, this->initializer() ? this->initializer()->clone() : nullptr,
      this->test() ? this->test()->clone() : nullptr,
      this->next() ? this->next()->clone() : nullptr, this->statement()->clone(),
      std::move(unrollInfo), SymbolTable::WrapIfBuiltin(this->symbols()));
}
...
std::unique_ptr<Statement> ForStatement::Convert(
    const Context& context, int offset, std::unique_ptr<Statement> initializer,
    std::unique_ptr<Expression> test, std::unique_ptr<Expression> next,
    std::unique_ptr<Statement> statement, std::shared_ptr<SymbolTable> symbolTable) {
  ...
  if (context.fConfig->strictES2Mode()) {
    unrollInfo = Analysis::GetLoopUnrollInfo(
        offset, initializer.get(), test.get(), next.get(), statement.get(), context.fErrors);
    if (!unrollInfo) {
      return nullptr;
    }
  }
  ...
  if (isVardeclBlockInitializer) {
    ...
    scope.push_back(ForStatement::Make(
        context, offset, /*initializer=*/nullptr, std::move(test), std::move(next),
        std::move(statement), std::move(unrollInfo), std::move(symbolTable)));
  ...
  return ForStatement::Make(
      context, offset, std::move(initializer), std::move(test), std::move(next),
      std::move(statement), std::move(unrollInfo), std::move(symbolTable));
}

std::unique_ptr<Statement> ForStatement::ConvertWhile(
    const Context& context, int offset, std::unique_ptr<Expression> test,
    std::unique_ptr<Statement> statement, std::shared_ptr<SymbolTable> symbolTable) {
  if (context.fConfig->strictES2Mode()) {
  ...
  return ForStatement::Convert(
      context, offset, /*initializer=*/nullptr, std::move(test),
      /*next=*/nullptr, std::move(statement), std::move(symbolTable));
}

std::unique_ptr<Statement> ForStatement::Make(
    const Context& context, int offset, std::unique_ptr<Statement> initializer,
    std::unique_ptr<Expression> test, std::unique_ptr<Expression> next,
    std::unique_ptr<Statement> statement, std::unique_ptr<LoopUnrollInfo> unrollInfo,
    std::shared_ptr<SymbolTable> symbolTable) {
  SkASSERT(
      is_simple_initializer(initializer.get()) || is_vardecl_block_initializer(initializer.get()));
  ...
  if (!unrollInfo && context.fConfig->strictES2Mode()) {
    unrollInfo = Analysis::GetLoopUnrollInfo(
        offset, initializer.get(), test.get(), next.get(), statement.get(), /*errors=*/nullptr);
    SkASSERT(unrollInfo);
  }

  return std::make_unique<ForStatement>(
      offset, std::move(initializer), std::move(test), std::move(next), std::move(statement),
