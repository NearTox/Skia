...
std::unique_ptr<Statement> ForStatement::clone() const {
  ...
  return std::make_unique<ForStatement>(
      fPosition, fForLoopPositions, this->initializer() ? this->initializer()->clone() : nullptr,
...
std::unique_ptr<Statement> ForStatement::Convert(
  ...
  if (context.fConfig->strictES2Mode()) {
    ...
    unrollInfo = Analysis::GetLoopUnrollInfo(
        pos, positions, initializer.get(), test.get(), next.get(), statement.get(),
        context.fErrors);
    if (!unrollInfo) {
      return nullptr;
    }
  ...
  if (isVardeclBlockInitializer) {
    ...
    scope.push_back(ForStatement::Make(
        context, pos, positions, /*initializer=*/nullptr, std::move(test), std::move(next),
        std::move(statement), std::move(unrollInfo), /*symbolTable=*/nullptr));
    return Block::Make(pos, std::move(scope), Block::Kind::kBracedScope, std::move(symbolTable));
  }

  return ForStatement::Make(
      context, pos, positions, std::move(initializer), std::move(test), std::move(next),
      std::move(statement), std::move(unrollInfo), std::move(symbolTable));
}

std::unique_ptr<Statement> ForStatement::ConvertWhile(
    const Context& context, Position pos, std::unique_ptr<Expression> test,
  ...
  return ForStatement::Convert(
      context, pos, ForLoopPositions(), /*initializer=*/nullptr, std::move(test), /*next=*/nullptr,
...
std::unique_ptr<Statement> ForStatement::Make(
    ...
    std::unique_ptr<Statement> initializer, std::unique_ptr<Expression> test,
    std::unique_ptr<Expression> next, std::unique_ptr<Statement> statement,
    std::unique_ptr<LoopUnrollInfo> unrollInfo, std::shared_ptr<SymbolTable> symbolTable) {
  SkASSERT(
  ...
  return std::make_unique<ForStatement>(
      pos, positions, std::move(initializer), std::move(test), std::move(next),
      std::move(statement), std::move(unrollInfo), std::move(symbolTable));
}
