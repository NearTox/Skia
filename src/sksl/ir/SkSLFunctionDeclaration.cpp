...
#define SKSL_INTRINSIC(name) {#name, k_##name##_IntrinsicKind},
  static const auto* kAllIntrinsics =
      new SkTHashMap<std::string_view, IntrinsicKind>{SKSL_INTRINSIC_LIST};
...
static bool check_modifiers(const Context& context, Position pos, const Modifiers& modifiers) {
...
static bool check_parameters(
  ...
  for (auto& param : parameters) {
    param->modifiers().checkPermitted(
        context, param->modifiersPosition(),
    ...
    if (type.isEffectChild() && !context.fConfig->fIsBuiltinCode) {
      context.fErrors->error(
          param->fPosition, "parameters of type '" + type.displayName() + "' not allowed");
      return false;
    }
...
static bool check_main_signature(
  ...
  auto paramIsCoords = [&](int idx) {
    ...
    return p.type().matches(*context.fTypes.fFloat2) && p.modifiers().fFlags == 0 &&
 ...
 */
static bool parameters_match(
    const std::vector<std::unique_ptr<Variable>>& params,
...
static bool find_existing_declaration(
    const Context& context, SymbolTable& symbols, Position pos, std::string_view name,
    std::vector<std::unique_ptr<Variable>>& parameters, Position returnTypePos,
    const Type* returnType, const FunctionDeclaration** outExistingDecl) {
  ErrorReporter& errors = *context.fErrors;
    ...
    for (const FunctionDeclaration* other : functions) {
      ...
      if (!returnType->matches(other->returnType())) {
        ...
        FunctionDeclaration invalidDecl(
            pos, &other->modifiers(), name, std::move(paramPtrs), returnType,
            context.fConfig->fIsBuiltinCode);
        errors.error(
            returnTypePos, "functions '" + invalidDecl.description() + "' and '" +
      ...
      for (size_t i = 0; i < parameters.size(); i++) {
        if (parameters[i]->modifiers() != other->parameters()[i]->modifiers()) {
          errors.error(
              parameters[i]->fPosition, "modifiers on parameter " + std::to_string(i + 1) +
...
const FunctionDeclaration* FunctionDeclaration::Convert(
    const Context& context, SymbolTable& symbols, Position pos, Position modifiersPosition,
  ...
  if (!check_modifiers(context, modifiersPosition, *modifiers) ||
      ...
      !find_existing_declaration(
          context, symbols, pos, name, parameters, returnTypePos, returnType, &decl)) {
    return nullptr;
  }
...
std::string FunctionDeclaration::mangledName() const {
  ...
  std::string result =
      std::string(name) + splitter + builtinMarker + this->returnType().abbreviatedName();
