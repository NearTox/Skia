...
#define SKSL_INTRINSIC(name) {#name, k_##name##_IntrinsicKind},
  static const auto* kAllIntrinsics =
      new std::unordered_map<skstd::string_view, IntrinsicKind>{SKSL_INTRINSIC_LIST};
...
static bool check_modifiers(
    const Context& context, int offset, const Modifiers& modifiers, bool isBuiltin) {
  const int permitted = Modifiers::kHasSideEffects_Flag | Modifiers::kInline_Flag |
                        Modifiers::kNoInline_Flag | (isBuiltin ? Modifiers::kES3_Flag : 0);
...
static bool check_return_type(
    const Context& context, int offset, const Type& returnType, bool isBuiltin) {
...
static bool check_parameters(
    const Context& context, std::vector<std::unique_ptr<Variable>>& parameters, bool isMain,
    bool isBuiltin) {
  ...
  for (auto& param : parameters) {
    IRGenerator::CheckModifiers(
        context, param->fOffset, param->modifiers(),
      ...
      if (ProgramConfig::IsRuntimeEffect(context.fConfig->fKind)) {
        ...
        if (type == *context.fTypes.fFloat2) {
          m.fLayout.fBuiltin = SK_MAIN_COORDS_BUILTIN;
            ...
            typeIsValidForColor(type) && builtinColorIndex < SK_ARRAY_COUNT(kBuiltinColorIDs)) {
...
static bool check_main_signature(
    const Context& context, int offset, const Type& returnType,
    std::vector<std::unique_ptr<Variable>>& parameters, bool isBuiltin) {
  ErrorReporter& errors = *context.fErrors;
  ...
  auto paramIsCoords = [&](int idx) {
    ...
    return p.type() == *context.fTypes.fFloat2 && p.modifiers().fFlags == 0 &&
           p.modifiers().fLayout.fBuiltin == SK_MAIN_COORDS_BUILTIN;
  };

  auto paramIsBuiltinColor = [&](int idx, int builtinID) {
    const Variable& p = *parameters[idx];
    return typeIsValidForColor(p.type()) && p.modifiers().fFlags == 0 &&
  ...
  switch (kind) {
    ...
    case ProgramKind::kRuntimeBlender: {
      ...
      if (!(parameters.size() == 2 && paramIsInputColor(0) && paramIsDestColor(1))) {
        errors.error(
            offset,
 ...
 */
static bool find_existing_declaration(
    const Context& context, SymbolTable& symbols, int offset, skstd::string_view name,
    std::vector<std::unique_ptr<Variable>>& parameters, const Type* returnType, bool isBuiltin,
  ...
  if (entry) {
    ...
    switch (entry->kind()) {
      ...
      case Symbol::Kind::kFunctionDeclaration:
        ...
        break;
      default: errors.error(offset, "symbol '" + name + "' was already defined"); return false;
    }
    for (const FunctionDeclaration* other : functions) {
      ...
      if (*returnType != other->returnType()) {
        ...
        FunctionDeclaration invalidDecl(
            offset, &other->modifiers(), name, std::move(paramPtrs), returnType, isBuiltin);
        errors.error(
            offset, "functions '" + invalidDecl.description() + "' and '" + other->description() +
      ...
      for (size_t i = 0; i < parameters.size(); i++) {
        if (parameters[i]->modifiers() != other->parameters()[i]->modifiers()) {
          errors.error(
              offset, "modifiers on parameter " + to_string((uint64_t)i + 1) +
...
const FunctionDeclaration* FunctionDeclaration::Convert(
  ...
  if (!check_modifiers(context, offset, *modifiers, isBuiltin) ||
      ...
      !find_existing_declaration(
          context, symbols, offset, name, parameters, returnType, isBuiltin, &decl)) {
    return nullptr;
  }
  ...
  auto result = std::make_unique<FunctionDeclaration>(
      offset, modifiers, name, std::move(finalParameters), returnType, isBuiltin);
...
bool FunctionDeclaration::determineFinalTypes(
    const ExpressionArray& arguments, ParamTypes* outParameterTypes,
