...
class Variable final : public Symbol {
 public:
  ...
  Variable(
      Position pos, Position modifiersPosition, const Modifiers* modifiers, std::string_view name,
      const Type* type, bool builtin, Storage storage)
  ...
  static ScratchVariable MakeScratchVariable(
      const Context& context, std::string_view baseName, const Type* type,
      const Modifiers& modifiers, SymbolTable* symbolTable,
      std::unique_ptr<Expression> initialValue);
  const Modifiers& modifiers() const noexcept { return *fModifiers; }

  void setModifiers(const Modifiers* modifiers) noexcept { fModifiers = modifiers; }

  Position modifiersPosition() const noexcept { return fModifiersPosition; }

  bool isBuiltin() const noexcept { return fBuiltin; }

  Storage storage() const noexcept { return (Storage)fStorage; }

  const Expression* initialValue() const;

  void setDeclaration(VarDeclaration* declaration) noexcept {
    SkASSERT(!fDeclaration);
    fDeclaration = declaration;
  }

  void detachDeadVarDeclaration() const noexcept {
