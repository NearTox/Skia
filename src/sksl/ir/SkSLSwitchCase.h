...
class SwitchCase final : public Statement {
 public:
  ...
  static std::unique_ptr<SwitchCase> Make(
      Position pos, SKSL_INT value, std::unique_ptr<Statement> statement) {
    return std::unique_ptr<SwitchCase>(
        new SwitchCase(pos, /*isDefault=*/false, value, std::move(statement)));
  }

  static std::unique_ptr<SwitchCase> MakeDefault(
      Position pos, std::unique_ptr<Statement> statement) {
    return std::unique_ptr<SwitchCase>(
        new SwitchCase(pos, /*isDefault=*/true, -1, std::move(statement)));
  }
  ...
  std::string description() const override {
    if (this->isDefault()) {
      ...
      return String::printf(
          "case %" PRId64 ":\n%s", (int64_t)this->value(), fStatement->description().c_str());
    }
  }
