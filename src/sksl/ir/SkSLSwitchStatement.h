...
class SwitchStatement final : public Statement {
 public:
  ...
  std::vector<std::unique_ptr<SwitchCase>>& cases() { return fCases; }

  const std::vector<std::unique_ptr<SwitchCase>>& cases() const { return fCases; }
  ...
  std::unique_ptr<Statement> clone() const override {
    ...
    return std::unique_ptr<Statement>(new SwitchStatement(
        fOffset, this->isStatic(), this->value()->clone(), std::move(cloned),
