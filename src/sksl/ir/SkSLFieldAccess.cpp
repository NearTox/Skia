...
std::unique_ptr<Expression> FieldAccess::Convert(
    const Context& context, Position pos, SymbolTable& symbolTable,
    std::unique_ptr<Expression> base, std::string_view field) {
  ...
  if (baseType.isEffectChild()) {
    ...
    if (result) {
      switch (result->kind()) {
        case Symbol::Kind::kFunctionDeclaration: {
          ...
          return std::make_unique<MethodReference>(context, pos, std::move(base), f);
        }
        case Symbol::Kind::kUnresolvedFunction: {
          const UnresolvedFunction& f = result->as<UnresolvedFunction>();
          return std::make_unique<MethodReference>(context, pos, std::move(base), f.functions());
        }
        default: break;
      }
    }
    context.fErrors->error(
        pos,
        "type '" + baseType.displayName() + "' has no method named '" + std::string(field) + "'");
    return nullptr;
  }
  ...
  context.fErrors->error(
      pos, "type '" + baseType.displayName() + "' does not have a field named '" +
               std::string(field) + "'");
  return nullptr;
}

static std::unique_ptr<Expression> extract_field(
    Position pos, const ConstructorStruct& ctor, int fieldIndex) {
...
std::unique_ptr<Expression> FieldAccess::Make(
    const Context& context, Position pos, std::unique_ptr<Expression> base, int fieldIndex,
    OwnerKind ownerKind) {
  ...
  if (expr->is<ConstructorStruct>()) {
    if (std::unique_ptr<Expression> field =
            extract_field(pos, expr->as<ConstructorStruct>(), fieldIndex)) {
      return field;
    }
  }
