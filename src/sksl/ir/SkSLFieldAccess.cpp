...
std::unique_ptr<Expression> FieldAccess::Convert(
  ...
  if (baseType.isEffectChild()) {
    ...
    if (result) {
      switch (result->kind()) {
        case Symbol::Kind::kFunctionDeclaration: {
          ...
          return std::make_unique<MethodReference>(context, base->fOffset, std::move(base), f);
        }
    ...
    context.fErrors->error(
        base->fOffset, "type '" + baseType.displayName() + "' has no method named '" + field + "'");
    return nullptr;
  }
...
std::unique_ptr<Expression> FieldAccess::Make(
    const Context& context, std::unique_ptr<Expression> base, int fieldIndex, OwnerKind ownerKind) {
