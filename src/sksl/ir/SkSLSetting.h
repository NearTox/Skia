...
class Setting final : public Expression {
 public:
  ...
  Setting(Position pos, std::string_view name, const Type* type)
      : INHERITED(pos, kExpressionKind, type), fName(std::move(name)) {}
  ...
  const std::string_view& name() const { return fName; }

  std::string description() const override { return std::string(this->name()); }
