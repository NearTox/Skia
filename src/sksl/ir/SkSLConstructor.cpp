...
static std::unique_ptr<Expression> convert_compound_constructor(
    const Context& context, int offset, const Type& type, ExpressionArray args) {
  ...
  if (args.size() == 1) {
    ...
    if (type.isVector() && argument->type().isVector() &&
      ...
      switch (type.slotCount()) {
        ...
        default:
          swizzleHint = "";
          SkDEBUGFAIL("unexpected slicing cast");
          break;
      }

      context.fErrors->error(
          offset, "'" + argument->type().displayName() + "' is not a valid parameter to '" +
    ...
    if (argument->type().isScalar()) {
      ...
      std::unique_ptr<Expression> typecast =
          ConstructorScalarCast::Make(context, offset, type.componentType(), std::move(argument));
      ...
      if (type.isMatrix()) {
        ...
        const Type& typecastType = type.componentType().toCompound(
            context, argument->type().columns(), argument->type().rows());
        argument =
            ConstructorCompoundCast::Make(context, offset, typecastType, std::move(argument));

        // Casting a matrix type into another matrix type is a resize.
        return ConstructorMatrixResize::Make(context, offset, type, std::move(argument));
      }
      ...
      if (type.isVector() && type.columns() == 4 && argument->type().slotCount() == 4) {
        ...
        const Type& vectorType = argument->type().componentType().toCompound(
            context,
            /*columns=*/4,
            /*rows=*/1);
...
std::unique_ptr<Expression> Constructor::Convert(
    const Context& context, int offset, const Type& type, ExpressionArray args) {
