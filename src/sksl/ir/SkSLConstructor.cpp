...
static std::unique_ptr<Expression> convert_compound_constructor(
    const Context& context, Position pos, const Type& type, ExpressionArray args) {
  ...
  if (args.size() == 1) {
    ...
    if (type.isVector() && argument->type().isVector() &&
      ...
      context.fErrors->error(
          pos, "'" + argument->type().displayName() + "' is not a valid parameter to '" +
    ...
    if (argument->type().isScalar()) {
      ...
      std::unique_ptr<Expression> typecast =
          ConstructorScalarCast::Convert(context, pos, type.componentType(), std::move(args));
      if (!typecast) {
        return nullptr;
      }
      ...
      if (type.isMatrix()) {
        ...
        argument = ConstructorCompoundCast::Make(context, pos, typecastType, std::move(argument));

        // Casting a matrix type into another matrix type is a resize.
        return ConstructorMatrixResize::Make(context, pos, type, std::move(argument));
      }
  ...
  for (std::unique_ptr<Expression>& arg : args) {
    if (!arg->type().isScalar() && !arg->type().isVector()) {
      context.fErrors->error(
          pos, "'" + arg->type().displayName() + "' is not a valid parameter to '" +
  ...
  if (actual != expected) {
    context.fErrors->error(
        pos, "invalid arguments to '" + type.displayName() + "' constructor (expected " +
                 std::to_string(expected) + " scalars, but found " + std::to_string(actual) + ")");
    return nullptr;
  }
