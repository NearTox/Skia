...
std::unique_ptr<Expression> Constructor::SimplifyConversion(
  ...
  if (expr.is<IntLiteral>()) {
    ...
    if (constructorType.isFloat()) {
      ...
      return std::make_unique<FloatLiteral>(expr.fOffset, value ? 1.0f : 0.0f, &constructorType);
    } else if (constructorType.isInteger()) {
...
ResultType Constructor::getVecComponent(int index) const {
  static_assert(
      std::is_same<ResultType, SKSL_FLOAT>::value || std::is_same<ResultType, SKSL_INT>::value ||
      std::is_same<ResultType, bool>::value);

  SkASSERT(this->type().isVector());
  SkASSERT(this->isCompileTimeConstant());

  if (this->arguments().size() == 1 && this->arguments()[0]->type().isScalar()) {
