...
class ArrayType final : public Type {
 public:
  ...
  bool allowedInES2() const override { return fComponentType.allowedInES2(); }
...
class ScalarType final : public Type {
 public:
  ...
  ScalarType(
      skstd::string_view name, const char* abbrev, NumberKind numberKind, int8_t priority,
      int8_t bitWidth)
  ...
  bool allowedInES2() const override { return fNumberKind != NumberKind::kUnsigned; }
...
class MatrixType final : public Type {
 public:
  ...
  MatrixType(
      skstd::string_view name, const char* abbrev, const Type& componentType, int8_t columns,
      int8_t rows)
      : INHERITED(name, abbrev, kTypeKind),
  ...
  bool allowedInES2() const override { return fColumns == fRows; }
...
class StructType final : public Type {
 public:
  ...
  StructType(int offset, skstd::string_view name, std::vector<Field> fields)
      : INHERITED(std::move(name), "S", kTypeKind, offset), fFields(std::move(fields)) {}
  ...
  bool allowedInES2() const override {
    return std::all_of(
        fFields.begin(), fFields.end(), [](const Field& f) { return f.fType->allowedInES2(); });
  }
...
class VectorType final : public Type {
 public:
  ...
  VectorType(skstd::string_view name, const char* abbrev, const Type& componentType, int8_t columns)
      : INHERITED(name, abbrev, kTypeKind),
        fComponentType(componentType.as<ScalarType>()),
        fColumns(columns) {
    SkASSERT(columns >= 2 && columns <= 4);
...
const Type* Type::applyPrecisionQualifiers(
  ...
  if (component.highPrecision()) {
    ...
    switch (component.numberKind()) {
      ...
      default: mediumpType = nullptr; break;
    }
  ...
  context.fErrors->error(
      offset, "type '" + this->displayName() + "' does not support precision qualifiers");
  return nullptr;
}
    ...
    if (valueExpr->allowsConstantSubexpressions()) {
      ...
      for (int slot = 0; slot < numSlots; ++slot) {
        ...
        if (value < baseType.minimumValue() || value > baseType.maximumValue()) {
          ...
          context.fErrors->error(
              expr.fOffset, String("integer is out of range for type '") +
...
SKSL_INT Type::convertArraySize(const Context& context, std::unique_ptr<Expression> size) const {
  ...
  if (this->isOpaque()) {
    context.fErrors->error(
        size->fOffset, "opaque type '" + this->name() + "' may not be used in an array");
    return 0;
  }
