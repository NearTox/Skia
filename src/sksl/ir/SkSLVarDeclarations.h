...
struct VarDeclaration : public Statement {
  ...
  std::unique_ptr<Statement> clone() const override {
    ...
    return std::unique_ptr<Statement>(
        new VarDeclaration(fVar, std::move(sizesClone), fValue ? fValue->clone() : nullptr));
  }
...
struct VarDeclarations : public ProgramElement {
  VarDeclarations(
      int offset, const Type* baseType, std::vector<std::unique_ptr<VarDeclaration>> vars)
  ...
  std::unique_ptr<ProgramElement> clone() const override {
    ...
    for (const auto& v : fVars) {
      cloned.push_back(std::unique_ptr<VarDeclaration>((VarDeclaration*)v->clone().release()));
    }
  ...
  String description() const override {
    ...
    String result =
        ((VarDeclaration&)*fVars[0]).fVar->fModifiers.description() + fBaseType.description() + " ";
    String separator;
