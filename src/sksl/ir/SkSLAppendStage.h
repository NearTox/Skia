...
struct AppendStage : public Expression {
  AppendStage(
      const Context& context, int offset, SkRasterPipeline::StockStage stage,
  ...
  std::unique_ptr<Expression> clone() const override {
    ...
    return std::unique_ptr<Expression>(new AppendStage(fOffset, fStage, std::move(cloned), &fType));
  }
