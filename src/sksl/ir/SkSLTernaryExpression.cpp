...
std::unique_ptr<Expression> TernaryExpression::Convert(
    const Context& context, std::unique_ptr<Expression> test, std::unique_ptr<Expression> ifTrue,
  ...
  if (!equalityOp.determineBinaryType(
          context, ifTrue->type(), ifFalse->type(), &trueType, &falseType, &resultType) ||
      (*trueType != *falseType)) {
...
std::unique_ptr<Expression> TernaryExpression::Make(
    const Context& context, std::unique_ptr<Expression> test, std::unique_ptr<Expression> ifTrue,
    std::unique_ptr<Expression> ifFalse) {
  ...
  if (context.fConfig->fSettings.fOptimize) {
    ...
    if (testExpr->is<BoolLiteral>()) {
      ...
      return testExpr->as<BoolLiteral>().value() ? std::move(ifTrue) : std::move(ifFalse);
    }
  }

  return std::make_unique<TernaryExpression>(
      test->fOffset, std::move(test), std::move(ifTrue), std::move(ifFalse));
}
