...
std::unique_ptr<Expression> TernaryExpression::Convert(
  ...
  if (ifTrue->type().componentType().isOpaque()) {
    context.fErrors->error(
        pos,
        "ternary expression of opaque type '" + ifTrue->type().displayName() + "' not allowed");
    return nullptr;
  }
  const Type* trueType;
  ...
  if (!equalityOp.determineBinaryType(
    ...
    context.fErrors->error(
        ifTrue->fPosition.rangeThrough(ifFalse->fPosition),
  ...
  return TernaryExpression::Make(
      context, pos, std::move(test), std::move(ifTrue), std::move(ifFalse));
}
  ...
  if (context.fConfig->fSettings.fOptimize) {
    ...
    if (Analysis::IsSameExpressionTree(*ifTrueExpr, *ifFalseExpr)) {
      ...
      return BinaryExpression::Make(
          context, pos, std::move(test), Operator::Kind::COMMA, std::move(ifTrue));
    }
  }
