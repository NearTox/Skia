...
static std::unique_ptr<Expression> cast_constant_array(
  ...
  for (std::unique_ptr<Expression>& arg : inputArgs) {
    ...
    if (arg->type().isScalar()) {
      typecastArgs.push_back(ConstructorScalarCast::Make(context, pos, scalarType, std::move(arg)));
    } else {
      typecastArgs.push_back(
          ConstructorCompoundCast::Make(context, pos, scalarType, std::move(arg)));
    }
  }
...
std::unique_ptr<Expression> ConstructorArrayCast::Make(
    const Context& context, Position pos, const Type& type, std::unique_ptr<Expression> arg) {
