...
class ExpressionStatement final : public Statement {
 public:
  ...
  ExpressionStatement(std::unique_ptr<Expression> expression)
      : INHERITED(expression->fPosition, kStatementKind), fExpression(std::move(expression)) {}

  // Creates an SkSL expression-statement; reports errors via ErrorReporter.
  static std::unique_ptr<Statement> Convert(
      const Context& context, std::unique_ptr<Expression> expr);
  ...
  const std::unique_ptr<Expression>& expression() const { return fExpression; }
  ...
  std::string description() const override { return this->expression()->description() + ";"; }
