...
std::vector<const FunctionDeclaration*> SymbolTable::GetFunctions(const Symbol& s) {
  switch (s.fKind) {
    case Symbol::kFunctionDeclaration_Kind: return {&s.as<FunctionDeclaration>()};
    case Symbol::kUnresolvedFunction_Kind: return s.as<UnresolvedFunction>().fFunctions;
    ...
    if (functions.size() > 0) {
      ...
      if (previous) {
        ...
        if (modified) {
          ...
          return this->takeOwnershipOfSymbol(std::make_unique<UnresolvedFunction>(functions));
        }
      }
    }
  }
...
void SymbolTable::addWithoutOwnership(StringFragment name, const Symbol* symbol) {
  ...
  if (existing == fSymbols.end()) {
    ...
    if (oldSymbol->fKind == Symbol::kFunctionDeclaration_Kind) {
      ...
      std::unique_ptr<const Symbol> u =
          std::unique_ptr<const Symbol>(new UnresolvedFunction(std::move(functions)));
