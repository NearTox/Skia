...
std::vector<const FunctionDeclaration*> SymbolTable::GetFunctions(const Symbol& s) {
  switch (s.fKind) {
    case Symbol::kFunctionDeclaration_Kind: return {&((FunctionDeclaration&)s)};
    case Symbol::kUnresolvedFunction_Kind: return ((UnresolvedFunction&)s).fFunctions;
    ...
    if (functions.size() > 0) {
      ...
      if (previous) {
        ...
        if (modified) {
          ...
          return this->takeOwnership(std::unique_ptr<Symbol>(new UnresolvedFunction(functions)));
        }
      }
    }
  }
...
void SymbolTable::addWithoutOwnership(StringFragment name, const Symbol* symbol) {
  ...
  if (existing == fSymbols.end()) {
    ...
    if (oldSymbol->fKind == Symbol::kFunctionDeclaration_Kind) {
      ...
      std::unique_ptr<Symbol> u =
          std::unique_ptr<Symbol>(new UnresolvedFunction(std::move(functions)));
