...
std::vector<const FunctionDeclaration*> SymbolTable::GetFunctions(const Symbol& s) {
  switch (s.kind()) {
    case Symbol::Kind::kFunctionDeclaration: return {&s.as<FunctionDeclaration>()};
    case Symbol::Kind::kUnresolvedFunction: return s.as<UnresolvedFunction>().functions();
    default: return std::vector<const FunctionDeclaration*>();
  }
}
    ...
    if (functions.size() > 0) {
      ...
      if (previous) {
        ...
        if (modified) {
          ...
          return writableSymbolTable ? writableSymbolTable->takeOwnershipOfSymbol(
...
void SymbolTable::addWithoutOwnership(const Symbol* symbol) {
  ...
  if (refInSymbolTable->is<FunctionDeclaration>()) {
    functions = {&refInSymbolTable->as<FunctionDeclaration>(), &symbol->as<FunctionDeclaration>()};

    refInSymbolTable =
        this->takeOwnershipOfSymbol(std::make_unique<UnresolvedFunction>(std::move(functions)));
...
const Type* SymbolTable::addArrayDimension(const Type* type, int arraySize) {
  if (arraySize != 0) {
    ...
    type = this->takeOwnershipOfSymbol(Type::MakeArrayType(std::move(arrayName), *type, arraySize));
  }
  return type;
}
