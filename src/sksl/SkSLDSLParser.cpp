...
DSLParser::DSLParser(
    Compiler* compiler, const ProgramSettings& settings, ProgramKind kind, String text)
    : fCompiler(*compiler),
      fSettings(settings),
      fKind(kind),
  ...
  fLexer.start(*fText);
  static const bool layoutMapInitialized = [] {
    InitLayoutMap();
    return true;
  }();
...
Token DSLParser::nextToken() {
  ...
  while (token.fKind == Token::Kind::TK_WHITESPACE || token.fKind == Token::Kind::TK_LINE_COMMENT ||
...
bool DSLParser::expectIdentifier(Token* result) {
  ...
  if (IsType(this->text(*result))) {
    this->error(*result, "expected an identifier, but found type '" + this->text(*result) + "'");
    this->fEncounteredFatalError = true;
    return false;
  }
  return true;
...
PositionInfo DSLParser::position(Token t) { return this->position(t.fOffset); }

PositionInfo DSLParser::position(int offset) {
  return PositionInfo::Offset("<unknown>", fText->c_str(), offset);
}

void DSLParser::error(Token token, String msg) { this->error(token.fOffset, msg); }
  ...
  while (!done) {
    switch (this->peek().fKind) {
      case Token::Kind::TK_END_OF_FILE:
        ...
        break;
      case Token::Kind::TK_DIRECTIVE: this->directive(); break;
      ...
      default: this->declaration(); done = fEncounteredFatalError;
    }
  }
  End();
...
bool DSLParser::declaration() {
  ...
  if (this->checkNext(Token::Kind::TK_LPAREN)) {
    ...
    SkTArray<DSLGlobalVar> result = this->varDeclarationEnd<DSLGlobalVar>(
        this->position(name), modifiers, *type, this->text(name));
    Declare(result);
    return true;
  }
}
...
bool DSLParser::functionDeclarationEnd(
    const DSLModifiers& modifiers, DSLType type, const Token& name) {
...
static skstd::optional<DSLStatement> declaration_statements(
    SkTArray<DSLVar> vars, SymbolTable& symbols) {
  if (vars.empty()) {
...
SKSL_INT DSLParser::arraySize() {
  ...
  if (next.fKind == Token::Kind::TK_INT_LITERAL) {
    ...
    return 1;
      ...
      this->checkNext(Token::Kind::TK_MINUS) && this->checkNext(Token::Kind::TK_INT_LITERAL)) {
    this->error(next, "array size must be positive");
    return 1;
  } else {
...
template <class T>
SkTArray<T> DSLParser::varDeclarationEnd(
    PositionInfo pos, const dsl::DSLModifiers& mods, dsl::DSLType baseType,
  ...
  if (nextToken.fKind == Token::Kind::TK_HIGHP || nextToken.fKind == Token::Kind::TK_MEDIUMP ||
    ...
    if (this->varDeclarationsPrefix(&prefix)) {
      ...
      return declaration_statements(
          this->varDeclarationEnd<DSLVar>(
              prefix.fPosition, prefix.fModifiers, prefix.fType, this->text(prefix.fName)),
          this->symbols());
    }
...
skstd::optional<DSLType> DSLParser::structDeclaration() {
  ...
  while (!this->checkNext(Token::Kind::TK_RBRACE)) {
    ...
    do {
      ...
      fields.push_back(DSLField(
          modifiers, std::move(actualType), this->text(memberName), this->position(memberName)));
    } while (this->checkNext(Token::Kind::TK_COMMA));
  ...
  if (fields.empty()) {
    this->error(name.fOffset, "struct '" + this->text(name) + "' must contain at least one field");
  }
...
SkTArray<dsl::DSLGlobalVar> DSLParser::structVarDeclaration(const DSLModifiers& modifiers) {
  ...
  if (this->checkNext(Token::Kind::TK_IDENTIFIER, &name)) {
    return this->varDeclarationEnd<DSLGlobalVar>(
        this->position(name), modifiers, std::move(*type), this->text(name));
  }
  this->expect(Token::Kind::TK_SEMICOLON, "';'");
  return {};
}
      ...
      if (found != layoutTokens->end()) {
        switch (found->second) {
          ...
          case LayoutToken::BLEND_SUPPORT_ALL_EQUATIONS:
            ...
            break;
          case LayoutToken::SRGB_UNPREMUL: result.srgbUnpremul(this->position(t)); break;
...
bool DSLParser::interfaceBlock(const dsl::DSLModifiers& modifiers) {
    ...
    do {
      ...
      if (this->checkNext(Token::Kind::TK_LBRACKET)) {
        ...
        if (sizeToken.fKind != Token::Kind::TK_RBRACKET) {
          actualType = Array(std::move(actualType), this->arraySize(), this->position(typeName));
        } else {
      ...
      fields.push_back(dsl::Field(
          fieldModifiers, std::move(actualType), this->text(fieldName), this->position(fieldName)));
    } while (this->checkNext(Token::Kind::TK_COMMA));
  }
  if (fields.empty()) {
    this->error(
        typeName,
        "interface block '" + this->text(typeName) + "' must contain at least one member");
  }
  ...
  this->expect(Token::Kind::TK_SEMICOLON, "';'");
  dsl::InterfaceBlock(
      modifiers, this->text(typeName), std::move(fields), instanceName, arraySize,
...
skstd::optional<DSLStatement> DSLParser::ifStatement() {
  ...
  if (isStatic) {
    return StaticIf(
        std::move(**test), std::move(*ifTrue), ifFalse ? std::move(*ifFalse) : DSLStatement(),
        this->position(start));
  } else {
    return If(
        std::move(**test), std::move(*ifTrue), ifFalse ? std::move(*ifFalse) : DSLStatement(),
...
skstd::optional<dsl::DSLStatement> DSLParser::forStatement() {
  ...
  return For(
      initializer ? std::move(*initializer) : DSLStatement(),
      test ? std::move(**test) : DSLExpression(), next ? std::move(**next) : DSLExpression(),
      std::move(*statement), this->position(start));
}
...
skstd::optional<DSLWrapper<DSLExpression>> DSLParser::expression() {
  ...
  while (this->checkNext(Token::Kind::TK_COMMA, &t)) {
    ...
    result = skstd::optional<DSLWrapper<DSLExpression>>(
        dsl::operator,(std::move(**result), std::move(**right)));
  }
  return result;
}
...
skstd::optional<DSLWrapper<DSLExpression>> DSLParser::swizzle(
    int offset, DSLExpression base, skstd::string_view swizzleMask) {
  ...
  for (int i = 0; i < length; ++i) {
    ...
    switch (swizzleMask[i]) {
      ...
      default:
        this->error(
            offset, String::printf("invalid swizzle component '%c'", swizzleMask[i]).c_str());
        return {{DSLExpression::Poison()}};
    }
  }
  switch (length) {
    ...
    case 4:
      return dsl::Swizzle(
          std::move(base), components[0], components[1], components[2], components[3]);
    default: SkUNREACHABLE;
  }
}

skstd::optional<dsl::Wrapper<dsl::DSLExpression>> DSLParser::call(
    int offset, dsl::DSLExpression base, SkTArray<Wrapper<DSLExpression>> args) {
...
skstd::optional<DSLWrapper<DSLExpression>> DSLParser::suffix(DSLExpression base) {
  ...
  switch (next.fKind) {
    ...
    case Token::Kind::TK_PLUSPLUS: return {{std::move(base)++}};
    case Token::Kind::TK_MINUSMINUS: {
