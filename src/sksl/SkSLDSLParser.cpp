...
static int parse_modifier_token(Token::Kind token) noexcept {
  switch (token) {
...
class AutoDSLDepth {
 public:
  AutoDSLDepth(DSLParser* p) : fParser(p), fDepth(0) {}

  ~AutoDSLDepth() { fParser->fDepth -= fDepth; }
...
class AutoDSLSymbolTable {
 public:
  AutoDSLSymbolTable() { dsl::PushSymbolTable(); }

  ~AutoDSLSymbolTable() { dsl::PopSymbolTable(); }
};
  ...
  if (fPushback.fKind != Token::Kind::TK_NONE) {
    ...
    switch (token.fKind) {
      ...
      case Token::Kind::TK_BAD_OCTAL:
        this->error(token, "'" + std::string(this->text(token)) + "' is not a valid octal number");
        break;
...
static bool is_whitespace(Token::Kind kind) {
  switch (kind) {
    ...
    case Token::Kind::TK_BLOCK_COMMENT: return true;
...
bool DSLParser::expect(Token::Kind kind, const char* expected, Token* result) {
  ...
  if (next.fKind == kind) {
    ...
    this->error(
        next, "expected " + std::string(expected) + ", but found '" +
...
bool DSLParser::expectIdentifier(Token* result) {
  ...
  if (IsBuiltinType(this->text(*result))) {
    this->error(
        *result,
        "expected an identifier, but found type '" + std::string(this->text(*result)) + "'");
    this->fEncounteredFatalError = true;
    return false;
  }
  return true;
...
Position DSLParser::rangeFrom(Position start) {
  int offset =
      fPushback.fKind != Token::Kind::TK_NONE ? fPushback.fOffset : fLexer.getCheckpoint().fOffset;
  return Position::Range(start.startOffset(), offset);
}

Position DSLParser::rangeFrom(Token start) { return this->rangeFrom(this->position(start)); }
...
SkSL::LoadedModule DSLParser::moduleInheritingFrom(SkSL::ParsedModule baseModule) {
  ...
  SkSL::LoadedModule result{
      fKind, CurrentSymbolTable(), std::move(ThreadContext::ProgramElements())};
  errorReporter->setSource(std::string_view());
  End();
  return result;
}
...
void DSLParser::declarations() {
  ...
  while (!done) {
    switch (this->peek().fKind) {
      case Token::Kind::TK_END_OF_FILE: done = true; break;
      case Token::Kind::TK_DIRECTIVE: this->directive(/*allowVersion=*/false); break;
...
void DSLParser::directive(bool allowVersion) {
  ...
  if (text == "#extension" && allowExtensions) {
    ...
    switch (version) {
      case 100: ThreadContext::GetProgramConfig()->fRequiredSkSLVersion = Version::k100; break;
      case 300: ThreadContext::GetProgramConfig()->fRequiredSkSLVersion = Version::k300; break;
      default: this->error(start, "unsupported version number"); return;
    }
  } else {
...
bool DSLParser::functionDeclarationEnd(
    Position start, const DSLModifiers& modifiers, DSLType type, const Token& name) {
  ...
  DSLFunction result(modifiers, type, this->text(name), parameterPointers, this->rangeFrom(start));
...
void DSLParser::globalVarDeclarationEnd(
    Position pos, const dsl::DSLModifiers& mods, dsl::DSLType baseType, Token name) {
  using namespace dsl;
  ...
  DSLGlobalVar first(
      mods, type, this->text(name), std::move(initializer), this->rangeFrom(pos),
      this->position(name));
  Declare(first);
  ...
  while (this->checkNext(Token::Kind::TK_COMMA)) {
    ...
    DSLGlobalVar next(
        mods, type, this->text(identifierName), std::move(anotherInitializer),
...
/* (LBRACKET expression? RBRACKET)* (EQ assignmentExpression)? (COMMA IDENTIFER
   (LBRACKET expression? RBRACKET)* (EQ assignmentExpression)?)* SEMICOLON */
DSLStatement DSLParser::localVarDeclarationEnd(
    Position pos, const dsl::DSLModifiers& mods, dsl::DSLType baseType, Token name) {
  using namespace dsl;
  DSLType type = baseType;
  DSLExpression initializer;
...
DSLStatement DSLParser::varDeclarationsOrExpressionStatement() {
  ...
  if (nextToken.fKind == Token::Kind::TK_HIGHP || nextToken.fKind == Token::Kind::TK_MEDIUMP ||
      nextToken.fKind == Token::Kind::TK_LOWP || IsType(this->text(nextToken))) {
    ...
    if (this->varDeclarationsPrefix(&prefix)) {
      checkpoint.accept();
      return this->localVarDeclarationEnd(
          prefix.fPosition, prefix.fModifiers, prefix.fType, prefix.fName);
    }
...
DSLType DSLParser::structDeclaration() {
  ...
  while (!this->checkNext(Token::Kind::TK_RBRACE)) {
    ...
    do {
      ...
      while (this->checkNext(Token::Kind::TK_LBRACKET)) {
        ...
        actualType = dsl::Array(actualType, size, this->rangeFrom(this->position(fieldStart)));
      }

      std::string key(this->text(memberName));
      auto found = field_names.find(key);
      if (found == field_names.end()) {
        fields.push_back(DSLField(
            modifiers, std::move(actualType), this->text(memberName), this->rangeFrom(fieldStart)));
        field_names.emplace(key);
      } else {
        this->error(
            memberName, "field '" + key + "' was already defined in the same struct ('" +
  ...
  if (fields.empty()) {
    this->error(
        ...
        "struct '" + std::string(this->text(name)) + "' must contain at least one field");
  }
  return dsl::Struct(this->text(name), SkMakeSpan(fields), this->rangeFrom(start));
}

/* structDeclaration ((IDENTIFIER varDeclarationEnd) | SEMICOLON) */
SkTArray<dsl::DSLGlobalVar> DSLParser::structVarDeclaration(
    Position start, const DSLModifiers& modifiers) {
...
DSLLayout DSLParser::layout() {
  ...
  if (this->checkNext(Token::Kind::TK_LAYOUT)) {
    ...
    for (;;) {
      ...
      if (found != nullptr) {
        switch (*found) {
          case LayoutToken::ORIGIN_UPPER_LEFT: result.originUpperLeft(this->position(t)); break;
          case LayoutToken::PUSH_CONSTANT: result.pushConstant(this->position(t)); break;
          case LayoutToken::BLEND_SUPPORT_ALL_EQUATIONS:
            result.blendSupportAllEquations(this->position(t));
            break;
          case LayoutToken::COLOR: result.color(this->position(t)); break;
          case LayoutToken::LOCATION: result.location(this->layoutInt(), this->position(t)); break;
          case LayoutToken::OFFSET: result.offset(this->layoutInt(), this->position(t)); break;
          case LayoutToken::BINDING: result.binding(this->layoutInt(), this->position(t)); break;
          case LayoutToken::INDEX: result.index(this->layoutInt(), this->position(t)); break;
          case LayoutToken::SET: result.set(this->layoutInt(), this->position(t)); break;
          case LayoutToken::BUILTIN: result.builtin(this->layoutInt(), this->position(t)); break;
          case LayoutToken::INPUT_ATTACHMENT_INDEX:
            result.inputAttachmentIndex(this->layoutInt(), this->position(t));
            break;
          default: this->error(t, "'" + text + "' is not a valid layout qualifier"); break;
        }
      } else {
...
DSLStatement DSLParser::statement() {
  ...
  switch (start.fKind) {
    ...
    case Token::Kind::TK_STATIC_IF: return this->ifStatement();
    case Token::Kind::TK_FOR: return this->forStatement();
    case Token::Kind::TK_DO: return this->doStatement();
    case Token::Kind::TK_WHILE: return this->whileStatement();
    case Token::Kind::TK_SWITCH:  // fall through
    case Token::Kind::TK_STATIC_SWITCH: return this->switchStatement();
    case Token::Kind::TK_RETURN: return this->returnStatement();
    case Token::Kind::TK_BREAK: return this->breakStatement();
    case Token::Kind::TK_CONTINUE: return this->continueStatement();
    case Token::Kind::TK_DISCARD: return this->discardStatement();
    case Token::Kind::TK_LBRACE: {
      std::optional<DSLBlock> result = this->block();
      return result ? DSLStatement(std::move(*result)) : DSLStatement();
    }
    case Token::Kind::TK_SEMICOLON: this->nextToken(); return dsl::Block();
    case Token::Kind::TK_HIGHP:
    case Token::Kind::TK_MEDIUMP:
    case Token::Kind::TK_LOWP:
    case Token::Kind::TK_CONST:
    case Token::Kind::TK_IDENTIFIER: return this->varDeclarationsOrExpressionStatement();
...
bool DSLParser::interfaceBlock(const dsl::DSLModifiers& modifiers) {
  ...
  while (!this->checkNext(Token::Kind::TK_RBRACE)) {
    ...
    do {
      ...
      if (field_names.find(key) == field_names.end()) {
        fields.push_back(dsl::Field(
            fieldModifiers, std::move(actualType), this->text(fieldName),
            this->rangeFrom(fieldPos)));
        field_names.emplace(key);
      } else {
        this->error(
            fieldName, "field '" + key + "' was already defined in the same interface block ('" +
  ...
  if (fields.empty()) {
    this->error(
        this->rangeFrom(typeName), "interface block '" + std::string(this->text(typeName)) +
  ...
  if (!fields.empty()) {
    dsl::InterfaceBlock(
        modifiers, this->text(typeName), std::move(fields), instanceName, size,
...
dsl::DSLStatement DSLParser::forStatement() {
  ...
  return For(
      initializer.hasValue() ? std::move(initializer) : DSLStatement(),
      test.hasValue() ? std::move(test) : DSLExpression(),
      next.hasValue() ? std::move(next) : DSLExpression(), std::move(statement),
      this->rangeFrom(start),
      ForLoopPositions{
...
DSLStatement DSLParser::returnStatement() {
  ...
  return Return(
      expression.hasValue() ? std::move(expression) : DSLExpression(), this->rangeFrom(start));
}
  ...
  for (;;) {
    switch (this->peek().fKind) {
      case Token::Kind::TK_RBRACE:
        this->nextToken();
        return DSLBlock(std::move(statements), CurrentSymbolTable(), this->rangeFrom(start));
...
bool DSLParser::operatorRight(
    AutoDSLDepth& depth, Operator::Kind op, BinaryParseFn rightFn, DSLExpression& result) {
  this->nextToken();
  if (!depth.increase()) {
    return false;
  }
...
DSLExpression DSLParser::expression() {
  ...
  SkASSERTF(
      result.position().valid(), "Expression %s has invalid position",
  ...
  SkASSERTF(
      ...
      (int)this->text(start).length(), this->text(start).data(), result.position().startOffset(),
  ...
  for (;;) {
    switch (this->peek().fKind) {
      case Token::Kind::TK_EQ:
        if (!operatorRight(depth, Operator::Kind::EQ, &DSLParser::assignmentExpression, result)) {
          return {};
        }
        break;
      case Token::Kind::TK_STAREQ:
        if (!operatorRight(
                depth, Operator::Kind::STAREQ, &DSLParser::assignmentExpression, result)) {
          return {};
        }
        break;
      case Token::Kind::TK_SLASHEQ:
        if (!operatorRight(
                depth, Operator::Kind::SLASHEQ, &DSLParser::assignmentExpression, result)) {
          return {};
        }
        break;
      case Token::Kind::TK_PERCENTEQ:
        if (!operatorRight(
                depth, Operator::Kind::PERCENTEQ, &DSLParser::assignmentExpression, result)) {
          return {};
        }
        break;
      case Token::Kind::TK_PLUSEQ:
        if (!operatorRight(
                depth, Operator::Kind::PLUSEQ, &DSLParser::assignmentExpression, result)) {
          return {};
        }
        break;
      case Token::Kind::TK_MINUSEQ:
        if (!operatorRight(
                depth, Operator::Kind::MINUSEQ, &DSLParser::assignmentExpression, result)) {
          return {};
        }
        break;
      case Token::Kind::TK_SHLEQ:
        if (!operatorRight(
                depth, Operator::Kind::SHLEQ, &DSLParser::assignmentExpression, result)) {
          return {};
        }
        break;
      case Token::Kind::TK_SHREQ:
        if (!operatorRight(
                depth, Operator::Kind::SHREQ, &DSLParser::assignmentExpression, result)) {
          return {};
        }
        break;
      case Token::Kind::TK_BITWISEANDEQ:
        if (!operatorRight(
                depth, Operator::Kind::BITWISEANDEQ, &DSLParser::assignmentExpression, result)) {
          return {};
        }
        break;
      case Token::Kind::TK_BITWISEXOREQ:
        if (!operatorRight(
                depth, Operator::Kind::BITWISEXOREQ, &DSLParser::assignmentExpression, result)) {
          return {};
        }
        break;
      case Token::Kind::TK_BITWISEOREQ:
        if (!operatorRight(
                depth, Operator::Kind::BITWISEOREQ, &DSLParser::assignmentExpression, result)) {
          return {};
        }
        break;
...
DSLExpression DSLParser::logicalOrExpression() {
  ...
  while (this->peek().fKind == Token::Kind::TK_LOGICALOR) {
    if (!operatorRight(
            depth, Operator::Kind::LOGICALOR, &DSLParser::logicalXorExpression, result)) {
      return {};
    }
  }
  return result;
...
DSLExpression DSLParser::logicalXorExpression() {
  ...
  while (this->peek().fKind == Token::Kind::TK_LOGICALXOR) {
    if (!operatorRight(
            depth, Operator::Kind::LOGICALXOR, &DSLParser::logicalAndExpression, result)) {
      return {};
    }
  }
  return result;
...
DSLExpression DSLParser::logicalAndExpression() {
  ...
  while (this->peek().fKind == Token::Kind::TK_LOGICALAND) {
    if (!operatorRight(
            depth, Operator::Kind::LOGICALAND, &DSLParser::bitwiseOrExpression, result)) {
      return {};
    }
  }
  return result;
...
DSLExpression DSLParser::bitwiseOrExpression() {
  ...
  while (this->peek().fKind == Token::Kind::TK_BITWISEOR) {
    if (!operatorRight(
            depth, Operator::Kind::BITWISEOR, &DSLParser::bitwiseXorExpression, result)) {
      return {};
    }
  }
  return result;
...
DSLExpression DSLParser::bitwiseXorExpression() {
  ...
  while (this->peek().fKind == Token::Kind::TK_BITWISEXOR) {
    if (!operatorRight(
            depth, Operator::Kind::BITWISEXOR, &DSLParser::bitwiseAndExpression, result)) {
      return {};
    }
  }
  return result;
...
DSLExpression DSLParser::bitwiseAndExpression() {
  ...
  while (this->peek().fKind == Token::Kind::TK_BITWISEAND) {
    if (!operatorRight(depth, Operator::Kind::BITWISEAND, &DSLParser::equalityExpression, result)) {
      return {};
    }
  }
  return result;
  ...
  for (;;) {
    switch (this->peek().fKind) {
      case Token::Kind::TK_EQEQ:
        if (!operatorRight(depth, Operator::Kind::EQEQ, &DSLParser::relationalExpression, result)) {
          return {};
        }
        break;
      case Token::Kind::TK_NEQ:
        if (!operatorRight(depth, Operator::Kind::NEQ, &DSLParser::relationalExpression, result)) {
          return {};
        }
        break;
      default: return result;
  ...
  for (;;) {
    switch (this->peek().fKind) {
      case Token::Kind::TK_LT:
        if (!operatorRight(depth, Operator::Kind::LT, &DSLParser::shiftExpression, result)) {
          return {};
        }
        break;
      case Token::Kind::TK_GT:
        if (!operatorRight(depth, Operator::Kind::GT, &DSLParser::shiftExpression, result)) {
          return {};
        }
        break;
      case Token::Kind::TK_LTEQ:
        if (!operatorRight(depth, Operator::Kind::LTEQ, &DSLParser::shiftExpression, result)) {
          return {};
        }
        break;
      case Token::Kind::TK_GTEQ:
        if (!operatorRight(depth, Operator::Kind::GTEQ, &DSLParser::shiftExpression, result)) {
          return {};
        }
        break;
      default: return result;
  ...
  for (;;) {
    switch (this->peek().fKind) {
      case Token::Kind::TK_SHL:
        if (!operatorRight(depth, Operator::Kind::SHL, &DSLParser::additiveExpression, result)) {
          return {};
        }
        break;
      case Token::Kind::TK_SHR:
        if (!operatorRight(depth, Operator::Kind::SHR, &DSLParser::additiveExpression, result)) {
          return {};
        }
        break;
      default: return result;
  ...
  for (;;) {
    switch (this->peek().fKind) {
      case Token::Kind::TK_PLUS:
        if (!operatorRight(
                depth, Operator::Kind::PLUS, &DSLParser::multiplicativeExpression, result)) {
          return {};
        }
        break;
      case Token::Kind::TK_MINUS:
        if (!operatorRight(
                depth, Operator::Kind::MINUS, &DSLParser::multiplicativeExpression, result)) {
          return {};
        }
        break;
      default: return result;
  ...
  for (;;) {
    switch (this->peek().fKind) {
      case Token::Kind::TK_STAR:
        if (!operatorRight(depth, Operator::Kind::STAR, &DSLParser::unaryExpression, result)) {
          return {};
        }
        break;
      case Token::Kind::TK_SLASH:
        if (!operatorRight(depth, Operator::Kind::SLASH, &DSLParser::unaryExpression, result)) {
          return {};
        }
        break;
      case Token::Kind::TK_PERCENT:
        if (!operatorRight(depth, Operator::Kind::PERCENT, &DSLParser::unaryExpression, result)) {
          return {};
        }
        break;
      default: return result;
...
DSLExpression DSLParser::swizzle(
    Position pos, DSLExpression base, std::string_view swizzleMask, Position maskPos) {
  ...
  for (int i = 0; i < length; ++i) {
    if (i >= 4) {
      Position errorPos =
          maskPos.valid() ? Position::Range(maskPos.startOffset() + 4, maskPos.endOffset()) : pos;
    ...
    switch (swizzleMask[i]) {
      ...
      default: {
        Position componentPos =
            Position::Range(maskPos.startOffset() + i, maskPos.startOffset() + i + 1);
        this->error(
            componentPos, String::printf("invalid swizzle component '%c'", swizzleMask[i]).c_str());
        return DSLExpression::Poison(pos);
      }
    }
  }
  switch (length) {
    ...
    case 3:
      return dsl::Swizzle(
          std::move(base), components[0], components[1], components[2], pos, maskPos);
    case 4:
      return dsl::Swizzle(
          std::move(base), components[0], components[1], components[2], components[3], pos,
          maskPos);
    default: SkUNREACHABLE;
  }
}
  ...
  switch (next.fKind) {
    ...
    case Token::Kind::TK_DOT: {
      ...
      if (this->identifier(&text)) {
        ...
        return this->swizzle(
            pos, std::move(base), text, this->rangeFrom(this->position(next).after()));
      }
      [[fallthrough]];
    }
    case Token::Kind::TK_FLOAT_LITERAL: {
      ...
      if (id.fKind == Token::Kind::TK_IDENTIFIER) {
        ...
        return this->swizzle(
            pos, std::move(base), std::string(field) + std::string(this->text(id)), maskPos);
      } else if (field.empty()) {
    ...
    default: {
      this->error(
          next, "expected expression suffix, but found '" + std::string(this->text(next)) + "'");
      return {};
    }
  }
}
...
bool DSLParser::boolLiteral(bool* dest) {
  Token t = this->nextToken();
  switch (t.fKind) {
    case Token::Kind::TK_TRUE_LITERAL: *dest = true; return true;
    case Token::Kind::TK_FALSE_LITERAL: *dest = false; return true;
    default:
      this->error(t, "expected 'true' or 'false', but found '" + std::string(this->text(t)) + "'");
      return false;
  }
}
