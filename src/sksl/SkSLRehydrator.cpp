...
Rehydrator::Rehydrator(
    const Context* context, ModifiersPool* modifiers, std::shared_ptr<SymbolTable> symbolTable,
    ErrorReporter* errorReporter, const uint8_t* src, size_t length)
    : fContext(*context),
      fModifiers(*modifiers),
      fErrors(errorReporter),
      fSymbolTable(std::move(symbolTable)),
      fStart(src) SkDEBUGCODE(, fEnd(fStart + length))
{
  ...
  switch (this->readU8()) {
    ...
    case kLayout_Command: {
      ...
      return Layout(
          ...
          (Layout::Format)format, (Layout::Primitive)primitive, maxVertices, invocations, marker,
          when, (Layout::Key)key, (Layout::CType)ctype);
    }
...
const Symbol* Rehydrator::symbol() {
  ...
  switch (kind) {
    ...
    case kEnumType_Command: {
      ...
      const Type* result =
          fSymbolTable->takeOwnershipOfSymbol(std::make_unique<Type>(name, Type::TypeKind::kEnum));
      this->addSymbol(id, result);
      return result;
    }
    case kFunctionDeclaration_Command: {
      ...
      const FunctionDeclaration* result =
          fSymbolTable->takeOwnershipOfSymbol(std::make_unique<FunctionDeclaration>(
              /*offset=*/-1, fModifiers.handle(modifiers), name, std::move(parameters), returnType,
              /*builtin=*/true));
      this->addSymbol(id, result);
      return result;
    }
    case kField_Command: {
      ...
      const Field* result =
          fSymbolTable->takeOwnershipOfSymbol(std::make_unique<Field>(/*offset=*/-1, owner, index));
      return result;
    }
  ...
  switch (kind) {
    case Rehydrator::kEnum_Command: {
      ...
      return std::make_unique<Enum>(
          /*offset=*/-1, typeName, std::move(symbols),
    ...
    case Rehydrator::kFunctionDefinition_Command: {
      ...
      for (int i = 0; i < refCount; ++i) {
        refs.insert(this->symbolRef<FunctionDeclaration>(Symbol::Kind::kFunctionDeclaration));
      }
      auto result = std::make_unique<FunctionDefinition>(
          /*offset=*/-1, decl,
          /*builtin=*/true, std::move(body), std::move(refs));
    ...
    case Rehydrator::kInterfaceBlock_Command: {
      ...
      return std::make_unique<InterfaceBlock>(
          /*offset=*/-1, &var->as<Variable>(), typeName, instanceName, std::move(sizes), nullptr);
    }
...
std::unique_ptr<Statement> Rehydrator::statement() {
  ...
  switch (kind) {
    case Rehydrator::kBlock_Command: {
      ...
      return std::make_unique<Block>(/*offset=*/-1, std::move(statements), fSymbolTable, isScope);
    }
    case Rehydrator::kBreak_Command: return std::unique_ptr<Statement>(new BreakStatement(-1));
    case Rehydrator::kContinue_Command:
      return std::unique_ptr<Statement>(new ContinueStatement(-1));
    case Rehydrator::kDiscard_Command: return std::unique_ptr<Statement>(new DiscardStatement(-1));
    case Rehydrator::kDo_Command: {
      std::unique_ptr<Statement> stmt = this->statement();
      std::unique_ptr<Expression> expr = this->expression();
      return std::unique_ptr<Statement>(new DoStatement(-1, std::move(stmt), std::move(expr)));
    }
    ...
    case Rehydrator::kFor_Command: {
      ...
      return std::unique_ptr<Statement>(new ForStatement(
          -1, std::move(initializer), std::move(test), std::move(next), std::move(body),
          std::move(symbols)));
    }
    case Rehydrator::kIf_Command: {
      ...
      return std::unique_ptr<Statement>(
          new IfStatement(-1, isStatic, std::move(test), std::move(ifTrue), std::move(ifFalse)));
    }
    ...
    case Rehydrator::kSwitch_Command: {
      ...
      for (int i = 0; i < caseCount; ++i) {
        ...
        cases.push_back(
            std::make_unique<SwitchCase>(/*offset=*/-1, std::move(value), std::move(statements)));
      }
      return std::make_unique<SwitchStatement>(
          -1, isStatic, std::move(expr), std::move(cases), fSymbolTable);
    }
    case Rehydrator::kVarDeclaration_Command: {
      ...
      return std::make_unique<VarDeclaration>(var, baseType, std::move(sizes), std::move(value));
    }
    case Rehydrator::kVoid_Command: return nullptr;
    case Rehydrator::kWhile_Command: {
      std::unique_ptr<Expression> expr = this->expression();
      std::unique_ptr<Statement> stmt = this->statement();
      return std::unique_ptr<Statement>(new WhileStatement(-1, std::move(expr), std::move(stmt)));
    }
    default:
...
std::unique_ptr<Expression> Rehydrator::expression() {
  ...
  switch (kind) {
    case Rehydrator::kBinary_Command: {
      ...
      const Type* type = this->type();
      return std::make_unique<BinaryExpression>(-1, std::move(left), op, std::move(right), type);
    }
    ...
    case Rehydrator::kNullLiteral_Command: return std::make_unique<NullLiteral>(fContext, -1);
    ...
    case Rehydrator::kTernary_Command: {
      ...
      return std::make_unique<TernaryExpression>(
          -1, std::move(test), std::move(ifTrue), std::move(ifFalse));
    }
