...
const Symbol* Rehydrator::symbol() {
  ...
  switch (kind) {
    case kArrayType_Command: {
      ...
      const Type* result =
          fSymbolTable->takeOwnershipOfSymbol(Type::MakeArrayType(name, *componentType, count));
    ...
    case kEnumType_Command: {
      ...
      const Type* result =
          fSymbolTable->takeOwnershipOfSymbol(Type::MakeSimpleType(name, Type::TypeKind::kEnum));
      this->addSymbol(id, result);
      return result;
    }
    case kFunctionDeclaration_Command: {
      ...
      const FunctionDeclaration* result =
          fSymbolTable->takeOwnershipOfSymbol(std::make_unique<FunctionDeclaration>(
              /*offset=*/-1, fModifiers.addToPool(modifiers), name, std::move(parameters),
              returnType, /*builtin=*/true));
      this->addSymbol(id, result);
      return result;
    }
    case kField_Command: {
  ...
  switch (kind) {
    ...
    case Rehydrator::kInterfaceBlock_Command: {
      ...
      return std::make_unique<InterfaceBlock>(
          /*offset=*/-1, &var->as<Variable>(), typeName, instanceName, arraySize, nullptr);
    }
