...
class AutoRehydratorSymbolTable {
 public:
  AutoRehydratorSymbolTable(Rehydrator* rehydrator)
      : fRehydrator(rehydrator), fOldSymbols(fRehydrator->fSymbolTable) {
  ...
  ~AutoRehydratorSymbolTable() { fRehydrator->fSymbolTable = std::move(fOldSymbols); }
...
Rehydrator::Rehydrator(
    Compiler& compiler, const uint8_t* src, size_t length, std::shared_ptr<SymbolTable> symbols)
    : fCompiler(compiler),
          ...
          SkDEBUGCODE(, fEnd(src + length))
{
  ...
  SkASSERTF(
      version == kVersion,
      "Dehydrated file is an unsupported version (current version is "
      "%d, found version %d)",
      kVersion, version);
  fStringStart = fIP;
...
Context& Rehydrator::context() const noexcept { return fCompiler.context(); }
...
Layout Rehydrator::layout() {
  switch (this->readU8()) {
    ...
    case kDefaultLayout_Command: return Layout();
    ...
    default: SkASSERT(false); return Layout();
  }
}

Modifiers Rehydrator::modifiers() {
  switch (this->readU8()) {
    case kDefaultModifiers_Command: return Modifiers();
    ...
    default: SkASSERT(false); return Modifiers();
  }
}
    ...
    case kFunctionDeclaration_Command: {
      ...
      const FunctionDeclaration* result =
          fSymbolTable->takeOwnershipOfSymbol(std::make_unique<FunctionDeclaration>(
              Position(), this->modifiersPool().add(modifiers), name, std::move(parameters),
              returnType, fSymbolTable->isBuiltin()));
      this->addSymbol(id, result);
      return result;
    }
    case kField_Command: {
      ...
      const Field* result =
          fSymbolTable->takeOwnershipOfSymbol(std::make_unique<Field>(Position(), owner, index));
      return result;
    }
...
std::unique_ptr<Program> Rehydrator::program() {
  ...
  std::unique_ptr<Program> result = std::make_unique<Program>(
      nullptr, std::move(config), fCompiler.fContext, std::move(elements),
      /*sharedElements=*/std::vector<const ProgramElement*>(), std::move(modifiers), fSymbolTable,
  ...
  switch (kind) {
    case Rehydrator::kFunctionDefinition_Command: {
      ...
      auto result = FunctionDefinition::Convert(
          this->context(), Position(), *decl, std::move(body), fSymbolTable->isBuiltin());
    ...
    case Rehydrator::kInterfaceBlock_Command: {
      ...
      return std::make_unique<InterfaceBlock>(
          Position(), var->as<Variable>(), typeName, instanceName, arraySize, nullptr);
    }
    ...
    case Rehydrator::kElementsComplete_Command: return nullptr;
    default: SkDEBUGFAILF("unsupported element %d\n", kind); return nullptr;
  }
}
...
std::unique_ptr<Statement> Rehydrator::statement() {
  ...
  switch (kind) {
    ...
    case Rehydrator::kBreak_Command: return BreakStatement::Make(Position());
    case Rehydrator::kContinue_Command: return ContinueStatement::Make(Position());
    case Rehydrator::kDiscard_Command: return DiscardStatement::Make(Position());
    ...
    case Rehydrator::kFor_Command: {
      ...
      std::unique_ptr<Statement> body = this->statement();
      std::unique_ptr<LoopUnrollInfo> unrollInfo = Analysis::GetLoopUnrollInfo(
          Position(), ForLoopPositions{}, initializer.get(), test.get(), next.get(), body.get(),
          /*errors=*/nullptr);
      return ForStatement::Make(
          this->context(), Position(), ForLoopPositions{}, std::move(initializer), std::move(test),
          std::move(next), std::move(body), std::move(unrollInfo), fSymbolTable);
    }
    case Rehydrator::kIf_Command: {
      ...
      return IfStatement::Make(
          this->context(), Position(), isStatic, std::move(test), std::move(ifTrue),
          std::move(ifFalse));
    }
    case Rehydrator::kNop_Command: return std::make_unique<SkSL::Nop>();
    ...
    case Rehydrator::kSwitch_Command: {
      ...
      for (int i = 0; i < caseCount; ++i) {
        ...
        if (isDefault) {
          ...
          std::unique_ptr<Statement> statement = this->statement();
          cases.push_back(SwitchCase::Make(Position(), std::move(value), std::move(statement)));
        }
      }
      return SwitchStatement::Make(
          this->context(), Position(), isStatic, std::move(expr), std::move(cases), fSymbolTable);
    }
    case Rehydrator::kVarDeclaration_Command: {
      ...
      return VarDeclaration::Make(this->context(), var, baseType, arraySize, std::move(value));
    }
    case Rehydrator::kVoid_Command: return nullptr;
    default:
...
std::unique_ptr<Expression> Rehydrator::expression() {
  ...
  switch (kind) {
    case Rehydrator::kBinary_Command: {
      ...
      return BinaryExpression::Make(this->context(), pos, std::move(left), op, std::move(right));
    }
    ...
    case Rehydrator::kFunctionCall_Command: {
      ...
      if (symbol->is<FunctionDeclaration>()) {
        ...
        f = FunctionCall::FindBestFunctionForCall(this->context(), unresolved.functions(), args);
        SkASSERT(f);
      } else {
        SkASSERT(false);
        return nullptr;
    ...
    case Rehydrator::kTernary_Command: {
      ...
      return TernaryExpression::Make(
          this->context(), pos, std::move(test), std::move(ifTrue), std::move(ifFalse));
    }
