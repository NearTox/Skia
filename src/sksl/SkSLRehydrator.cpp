...
Rehydrator::Rehydrator(
    const Context* context, std::shared_ptr<SymbolTable> symbolTable, const uint8_t* src,
    size_t length)
    : fContext(*context),
    ...
    case kFunctionDeclaration_Command: {
      ...
      const FunctionDeclaration* result =
          fSymbolTable->takeOwnershipOfSymbol(std::make_unique<FunctionDeclaration>(
              /*offset=*/-1, this->modifiersPool().add(modifiers), name, std::move(parameters),
              returnType,
              /*builtin=*/true));
      this->addSymbol(id, result);
      return result;
  ...
  switch (kind) {
    case Rehydrator::kFunctionDefinition_Command: {
      ...
      auto result = FunctionDefinition::Convert(
          fContext, /*offset=*/-1, *decl, std::move(body), /*builtin=*/true);
...
std::unique_ptr<Statement> Rehydrator::statement() {
  ...
  switch (kind) {
    ...
    case Rehydrator::kBreak_Command: return BreakStatement::Make(/*offset=*/-1);
    case Rehydrator::kContinue_Command: return ContinueStatement::Make(/*offset=*/-1);
    case Rehydrator::kDiscard_Command: return DiscardStatement::Make(/*offset=*/-1);
    ...
    case Rehydrator::kIf_Command: {
      ...
      return IfStatement::Make(
          fContext, /*offset=*/-1, isStatic, std::move(test), std::move(ifTrue),
          std::move(ifFalse));
    }
    ...
    case Rehydrator::kSwitch_Command: {
      ...
      for (int i = 0; i < caseCount; ++i) {
        ...
        cases.push_back(
            std::make_unique<SwitchCase>(/*offset=*/-1, std::move(value), std::move(statement)));
      }
      return SwitchStatement::Make(
          fContext, /*offset=*/-1, isStatic, std::move(expr), std::move(cases), fSymbolTable);
    }
...
std::unique_ptr<Expression> Rehydrator::expression() {
  ...
  switch (kind) {
    ...
    case Rehydrator::kConstructorCompound_Command: {
      const Type* type = this->type();
      return ConstructorCompound::Make(fContext, /*offset=*/-1, *type, this->expressionArray());
    }
    case Rehydrator::kConstructorDiagonalMatrix_Command: {
      const Type* type = this->type();
      ExpressionArray args = this->expressionArray();
      SkASSERT(args.size() == 1);
      return ConstructorDiagonalMatrix::Make(fContext, /*offset=*/-1, *type, std::move(args[0]));
    }
    case Rehydrator::kConstructorMatrixResize_Command: {
      const Type* type = this->type();
      ExpressionArray args = this->expressionArray();
      SkASSERT(args.size() == 1);
      return ConstructorMatrixResize::Make(fContext, /*offset=*/-1, *type, std::move(args[0]));
    }
