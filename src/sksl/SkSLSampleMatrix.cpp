...
struct SampleMatrixExtractor {
  SampleMatrixExtractor(const Program& program, const Variable& fp) : fProgram(program), fFP(fp) {}
...
SampleMatrix SampleMatrixExtractor::getMatrix(const Expression& e) const {
  switch (e.fKind) {
    ...
    case Expression::kFieldAccess_Kind: return this->getMatrix(*((const FieldAccess&)e).fBase);
    case Expression::kSwizzle_Kind: return this->getMatrix(*((const Swizzle&)e).fBase);
    case Expression::kBinary_Kind: {
      const BinaryExpression& b = (const BinaryExpression&)e;
      return this->getMatrix(*b.fLeft).merge(this->getMatrix(*b.fRight));
    }
    case Expression::kIndex_Kind: {
      const IndexExpression& idx = (const IndexExpression&)e;
      return this->getMatrix(*idx.fBase).merge(this->getMatrix(*idx.fIndex));
    }
    case Expression::kPrefix_Kind: return this->getMatrix(*((const PrefixExpression&)e).fOperand);
    case Expression::kPostfix_Kind: return this->getMatrix(*((const PostfixExpression&)e).fOperand);
    case Expression::kTernary_Kind: {
      const TernaryExpression& t = (const TernaryExpression&)e;
      return this->getMatrix(*t.fTest)
          .merge(this->getMatrix(*t.fIfTrue))
          .merge(this->getMatrix(*t.fIfFalse));
    }
...
SampleMatrix SampleMatrixExtractor::getMatrix(const Statement& s) const {
  switch (s.fKind) {
    ...
    case Statement::kIf_Kind: {
      ...
      return this->getMatrix(*i.fTest)
          .merge(this->getMatrix(*i.fIfTrue))
          .merge(this->getMatrix(i.fIfFalse.get()));
    }
    case Statement::kFor_Kind: {
      const ForStatement& f = (const ForStatement&)s;
      return this->getMatrix(f.fInitializer.get())
          .merge(this->getMatrix(f.fTest.get())
                     .merge(this->getMatrix(f.fNext.get()).merge(this->getMatrix(*f.fStatement))));
    }
    case Statement::kWhile_Kind: {
      const WhileStatement& w = (const WhileStatement&)s;
      return this->getMatrix(*w.fTest).merge(this->getMatrix(*w.fStatement));
    }
    case Statement::kDo_Kind: {
      const DoStatement& d = (const DoStatement&)s;
      return this->getMatrix(*d.fTest).merge(this->getMatrix(*d.fStatement));
    }
