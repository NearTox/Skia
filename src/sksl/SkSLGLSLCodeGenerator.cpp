...
String GLSLCodeGenerator::getTypeName(const Type& type) {
  switch (type.kind()) {
    ...
    case Type::kEnum_Kind: return "int";
...
void GLSLCodeGenerator::writeTransposeHack(const Expression& mat) {
  ...
  if (fWrittenIntrinsics.find(name) == fWrittenIntrinsics.end()) {
    ...
    String transposed =
        this->getTypeName(base.toCompound(fContext, mat.fType.rows(), mat.fType.columns()));
    fExtraFunctions.writeText(
        (transposed + " " + name + "(" + type + " m) {\nreturn " + transposed + "(").c_str());
    ...
    if (found != fFunctionClasses->end()) {
      switch (found->second) {
        case FunctionClass::kAbs: {
          ...
          if (c.fArguments[0]->fType != *fContext.fInt_Type) break;
...
void GLSLCodeGenerator::writeSwizzleConstructor(
    const Swizzle& swizzle, const String& constants, const String& mask,
    ...
    case 2:
      switch (constantBits) {
        ...
        case 1:  // 01
          ...
          break;
        case 2:  // 10
          this->writeSwizzleConstructor(swizzle, constants, mask, SwizzleOrder::CONSTANTS_FIRST);
          break;
        case 3:  // 11
...
void GLSLCodeGenerator::writeBinaryExpression(
  ...
  if (fProgram.fSettings.fCaps->unfoldShortCircuitAsTernary() &&
      (b.fOperator == Token::Kind::TK_LOGICALAND || b.fOperator == Token::Kind::TK_LOGICALOR)) {
...
const char* GLSLCodeGenerator::getTypePrecision(const Type& type) {
  if (usesPrecisionModifiers()) {
    switch (type.kind()) {
      ...
      case Type::kMatrix_Kind: return this->getTypePrecision(type.componentType());
      default: break;
    }
  }
  return "";
...
void GLSLCodeGenerator::writeProgramElement(const ProgramElement& e) {
  switch (e.fKind) {
    case ProgramElement::kExtension_Kind: this->writeExtension(((Extension&)e).fName); break;
    ...
    case ProgramElement::kInterfaceBlock_Kind: this->writeInterfaceBlock((InterfaceBlock&)e); break;
