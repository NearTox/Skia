...
struct ASTPrefixExpression : public ASTExpression {
  ASTPrefixExpression(Token op, std::unique_ptr<ASTExpression> operand) noexcept
      : INHERITED(op.fOffset, kPrefix_Kind), fOperator(op.fKind), fOperand(std::move(operand)) {}
