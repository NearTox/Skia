  ...
  void visitExpression(Expression& expr) {
    switch (expr.kind()) {
      ...
      case Expression::Kind::kExternalValue: {
        ...
        if (!var->canWrite()) {
          fErrors->error(
              expr.fOffset, "cannot modify immutable external value '" + var->fName + "'");
        }
        break;
      }
...
bool TProgramVisitor<PROG, EXPR, STMT, ELEM>::visitExpression(EXPR e) {
  switch (e.kind()) {
    ...
    case Expression::Kind::kSwizzle: return this->visitExpression(*e.template as<Swizzle>().fBase);
  ...
  switch (s.kind()) {
    ...
    case Statement::Kind::kFor: {
      ...
      return (f.fInitializer && this->visitStatement(*f.fInitializer)) ||
             ...
             (f.fNext && this->visitExpression(*f.fNext)) || this->visitStatement(*f.fStatement);
    }
    case Statement::Kind::kIf: {
      auto& i = s.template as<IfStatement>();
      return this->visitExpression(*i.fTest) || this->visitStatement(*i.fIfTrue) ||
    ...
    case Statement::Kind::kVarDeclarations:
      return this->visitProgramElement(*s.template as<VarDeclarationsStatement>().fDeclaration);
