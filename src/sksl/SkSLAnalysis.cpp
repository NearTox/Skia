...
bool ProgramUsage::isDead(const Variable& v) const {
  ...
  return !counts.fWrite ||
         (!counts.fRead && !(modifiers.fFlags & (Modifiers::kPLS_Flag | Modifiers::kPLSOut_Flag)));
}
...
bool Analysis::IsTrivialExpression(const Expression& expr) {
  return expr.is<IntLiteral>() || expr.is<FloatLiteral>() || expr.is<BoolLiteral>() ||
         ...
         (expr.is<Constructor>() && expr.as<Constructor>().arguments().size() == 1 &&
          IsTrivialExpression(*expr.as<Constructor>().arguments().front())) ||
         (expr.is<Constructor>() && expr.isConstantOrUniform()) ||
...
static const char* invalid_for_ES2(
    const ForStatement& loop, Analysis::UnrollableLoopInfo& loopInfo) {
...
bool Analysis::ForLoopIsValidForES2(
    const ForStatement& loop, Analysis::UnrollableLoopInfo* outLoopInfo, ErrorReporter* errors) {
  UnrollableLoopInfo ignored, *loopInfo = outLoopInfo ? outLoopInfo : &ignored;
...
bool TProgramVisitor<PROG, EXPR, STMT, ELEM>::visitExpression(EXPR e) {
  switch (e.kind()) {
    ...
    case Expression::Kind::kFunctionCall: {
      ...
      for (auto& arg : c.arguments()) {
        if (arg && this->visitExpression(*arg)) {
          return true;
        }
      }
      return false;
  ...
  switch (s.kind()) {
    ...
    case Statement::Kind::kFor: {
      ...
      return (f.initializer() && this->visitStatement(*f.initializer())) ||
             ...
             (f.next() && this->visitExpression(*f.next())) || this->visitStatement(*f.statement());
    }
