 ...
 public:
  class Handle {
   public:
    ...
    Handle(const ModifiersPool* pool, int index) : fPool(pool), fIndex(index) {}

    const Modifiers* operator->() const { return &fPool->fModifiers[fIndex]; }
  ...
  bool empty() { return fModifiers.empty(); }
