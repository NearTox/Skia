  ...
  switch (type.typeKind()) {
    case Type::TypeKind::kArray:
      ...
      return String::printf(
          "array<%s, %d>", this->typeName(type.componentType()).c_str(), type.columns());
...
void MetalCodeGenerator::writeType(const Type& type) { this->write(this->typeName(type)); }
...
void MetalCodeGenerator::writeExpression(const Expression& expr, Precedence parentPrecedence) {
  switch (expr.kind()) {
    case Expression::Kind::kBinary:
      ...
      break;
    case Expression::Kind::kBoolLiteral: this->writeBoolLiteral(expr.as<BoolLiteral>()); break;
    ...
    case Expression::Kind::kConstructorMatrixResize:
      this->writeConstructorMatrixResize(expr.as<ConstructorMatrixResize>(), parentPrecedence);
      break;
    case Expression::Kind::kConstructorScalarCast:
    case Expression::Kind::kConstructorCompoundCast:
      this->writeCastConstructor(expr.asAnyConstructor(), "(", ")", parentPrecedence);
      break;
    case Expression::Kind::kIntLiteral: this->writeIntLiteral(expr.as<IntLiteral>()); break;
    case Expression::Kind::kFieldAccess: this->writeFieldAccess(expr.as<FieldAccess>()); break;
    case Expression::Kind::kFloatLiteral: this->writeFloatLiteral(expr.as<FloatLiteral>()); break;
    case Expression::Kind::kFunctionCall: this->writeFunctionCall(expr.as<FunctionCall>()); break;
    ...
    case Expression::Kind::kPostfix:
      ...
      break;
    case Expression::Kind::kSetting: this->writeSetting(expr.as<Setting>()); break;
    case Expression::Kind::kSwizzle: this->writeSwizzle(expr.as<Swizzle>()); break;
    ...
    case Expression::Kind::kTernary:
      ...
      break;
    case Expression::Kind::kIndex: this->writeIndexExpression(expr.as<IndexExpression>()); break;
    default: SkDEBUGFAILF("unsupported expression: %s", expr.description().c_str()); break;
  }
}

String MetalCodeGenerator::getOutParamHelper(
    const FunctionCall& call, const ExpressionArray& arguments,
  ...
  String name =
      "_skOutParamHelper" + to_string(fSwizzleHelperCount++) + "_" + function.mangledName();
...
String MetalCodeGenerator::getInversePolyfill(const ExpressionArray& arguments) {
  ...
  if (arguments.size() == 1) {
    ...
    if (type.isMatrix() && type.rows() == type.columns()) {
      ...
      if (didInsert) {
        switch (type.rows()) {
          case 2: fExtraFunctions.writeText(kInverse2x2); break;
          case 3: fExtraFunctions.writeText(kInverse3x3); break;
          case 4: fExtraFunctions.writeText(kInverse4x4); break;
        }
      }
      return name;
    }
...
bool MetalCodeGenerator::writeIntrinsicCall(const FunctionCall& c, IntrinsicKind kind) {
  ...
  switch (kind) {
    ...
    case k_notEqual_IntrinsicKind: {
      ...
      switch (kind) {
        case k_equal_IntrinsicKind: this->write(" == "); break;
        case k_notEqual_IntrinsicKind: this->write(" != "); break;
        case k_lessThan_IntrinsicKind: this->write(" < "); break;
        case k_lessThanEqual_IntrinsicKind: this->write(" <= "); break;
        case k_greaterThan_IntrinsicKind: this->write(" > "); break;
        case k_greaterThanEqual_IntrinsicKind: this->write(" >= "); break;
        default: SK_ABORT("unsupported comparison intrinsic kind");
      }
...
void MetalCodeGenerator::assembleMatrixFromExpressions(
    const AnyConstructor& ctor, int columns, int rows) {
  SkASSERT(rows <= 4);
  SkASSERT(columns <= 4);
        ...
        switch (argType.typeKind()) {
          ...
          case Type::TypeKind::kMatrix: {
            fExtraFunctions.printf(
                "x%zu[%d][%d]", argIndex, argPosition / argType.rows(),
...
String MetalCodeGenerator::getMatrixConstructHelper(const AnyConstructor& c) {
  ...
  for (const std::unique_ptr<Expression>& expr : args) {
    fExtraFunctions.printf(
        "%s%s x%zu", argSeparator, this->typeName(expr->type()).c_str(), argIndex++);
    argSeparator = ", ";
  }
...
void MetalCodeGenerator::writeConstructorMatrixResize(
    const ConstructorMatrixResize& c, Precedence parentPrecedence) {
...
void MetalCodeGenerator::writeConstructorCompound(
    const ConstructorCompound& c, Precedence parentPrecedence) {
  if (c.type().isVector()) {
...
void MetalCodeGenerator::writeConstructorCompoundVector(
    const ConstructorCompound& c, Precedence parentPrecedence) {
...
void MetalCodeGenerator::writeConstructorCompoundMatrix(
    const ConstructorCompound& c, Precedence parentPrecedence) {
  SkASSERT(c.type().isMatrix());
  ...
  const Type& columnType =
      matrixType.componentType().toCompound(fContext, /*columns=*/matrixType.rows(), /*rows=*/1);
...
void MetalCodeGenerator::writeAnyConstructor(
    const AnyConstructor& c, const char* leftBracket, const char* rightBracket,
...
void MetalCodeGenerator::writeCastConstructor(
    const AnyConstructor& c, const char* leftBracket, const char* rightBracket,
    Precedence parentPrecedence) {
...
void MetalCodeGenerator::writeVariableReference(const VariableReference& ref) {
  ...
  switch (ref.variable()->modifiers().fLayout.fBuiltin) {
    case SK_FRAGCOLOR_BUILTIN: this->write("_out.sk_FragColor"); break;
    case SK_FRAGCOORD_BUILTIN: this->writeFragCoord(); break;
    case SK_VERTEXID_BUILTIN: this->write("sk_VertexID"); break;
...
void MetalCodeGenerator::writeFieldAccess(const FieldAccess& f) {
  ...
  switch (field->fModifiers.fLayout.fBuiltin) {
    case SK_POSITION_BUILTIN: this->write("_out.sk_Position"); break;
    default:
...
void MetalCodeGenerator::writeMatrixTimesEqualHelper(
    const Type& left, const Type& right, const Type& result) {
...
thread bool operator!=(const %s left, const %s right);
        ...
        this->typeName(left).c_str(), this->typeName(right).c_str(), this->typeName(left).c_str(),
        this->typeName(right).c_str());
...
thread bool operator!=(thread const %s& left, thread const %s& right);
        ...
        this->typeName(type).c_str(), this->typeName(type).c_str(), this->typeName(type).c_str(),
        this->typeName(type).c_str());

    fExtraFunctions.printf(
        "thread bool operator==(thread const %s& left, thread const %s& right) {\n"
        "    return ",
        this->typeName(type).c_str(), this->typeName(type).c_str());

    const char* separator = "";
    for (const Type::Field& field : type.fields()) {
      fExtraFunctions.printf(
          "%s(left.%.*s == right.%.*s)", separator, (int)field.fName.size(), field.fName.data(),
...
void MetalCodeGenerator::writeBinaryExpression(
...
void MetalCodeGenerator::writeTernaryExpression(
...
void MetalCodeGenerator::writePrefixExpression(
...
void MetalCodeGenerator::writePostfixExpression(
...
void MetalCodeGenerator::writeFunctionRequirementArgs(
    const FunctionDeclaration& f, const char*& separator) {
...
void MetalCodeGenerator::writeFunctionRequirementParams(
    const FunctionDeclaration& f, const char*& separator) {
  Requirements requirements = this->requirements(f);
  if (requirements & kInputs_Requirement) {
    this->write(separator);
...
int MetalCodeGenerator::getUniformSet(const Modifiers& m) {
  return (m.fLayout.fSet >= 0) ? m.fLayout.fSet : fProgram.fConfig->fSettings.fDefaultUniformSet;
}

bool MetalCodeGenerator::writeFunctionDeclaration(const FunctionDeclaration& f) {
  fRTFlipName =
      fProgram.fInputs.fUseFlipRTUniform ? "_globals._anonInterface0->" SKSL_RTFLIP_NAME : "";
  const char* separator = "";
  if (f.isMain()) {
    switch (fProgram.fConfig->fKind) {
      case ProgramKind::kFragment: this->write("fragment Outputs fragmentMain"); break;
      case ProgramKind::kVertex: this->write("vertex Outputs vertexMain"); break;
      default: fContext.fErrors->error(-1, "unsupported kind of program"); return false;
    }
    this->write("(Inputs _in [[stage_in]]");
    if (-1 != fUniformBuffer) {
      this->write(", constant Uniforms& _uniforms [[buffer(" + to_string(fUniformBuffer) + ")]]");
    }
        ...
        if (var.var().type().typeKind() == Type::TypeKind::kSampler) {
          if (var.var().modifiers().fLayout.fBinding < 0) {
            fContext.fErrors->error(
                decls.fOffset, "Metal samplers must have 'layout(binding=...)'");
            return false;
          }
...
void MetalCodeGenerator::writeInterfaceBlock(const InterfaceBlock& intf) {
  ...
  if (intf.instanceName().size()) {
    ...
    fInterfaceBlockNameMap[&intf] = *fProgram.fSymbols->takeOwnershipOfString(
        "_anonInterface" + to_string(fAnonInterfaceCount++));
  }
  this->writeLine(";");
}
...
void MetalCodeGenerator::writeFields(
  ...
  for (const Type::Field& field : fields) {
    ...
    if (!MemoryLayout::LayoutIsSupported(*fieldType)) {
      fContext.fErrors->error(
          parentOffset, "type '" + fieldType->name() + "' is not permitted here");
      return;
    }
    if (fieldOffset != -1) {
      if (currentOffset > fieldOffset) {
        fContext.fErrors->error(
            parentOffset, "offset of field '" + field.fName + "' must be at least " +
      ...
      if (fieldOffset % alignment) {
        fContext.fErrors->error(
            parentOffset, "offset of field '" + field.fName + "' must be a multiple of " +
...
void MetalCodeGenerator::writeStatement(const Statement& s) {
  switch (s.kind()) {
    ...
    case Statement::Kind::kDo: this->writeDoStatement(s.as<DoStatement>()); break;
    case Statement::Kind::kSwitch: this->writeSwitchStatement(s.as<SwitchStatement>()); break;
    case Statement::Kind::kBreak: this->write("break;"); break;
    case Statement::Kind::kContinue: this->write("continue;"); break;
    case Statement::Kind::kDiscard: this->write("discard_fragment();"); break;
    case Statement::Kind::kInlineMarker:
    case Statement::Kind::kNop: this->write(";"); break;
...
void MetalCodeGenerator::writeReturnStatementFromMain() {
  ...
  switch (fProgram.fConfig->fKind) {
    ...
    case ProgramKind::kFragment: this->write("return _out;"); break;
    default: SkDEBUGFAIL("unsupported kind of program");
  }
}
  ...
  if (fCurrentFunction && fCurrentFunction->isMain()) {
    if (r.expression()) {
      if (r.expression()->type() == *fContext.fTypes.fHalf4) {
        ...
        fContext.fErrors->error(
            r.fOffset, "Metal does not support returning '" + r.expression()->type().description() +
                           "' from main()");
      }
    }
    this->writeReturnStatementFromMain();
...
void MetalCodeGenerator::writeInputStruct() {
  ...
  for (const ProgramElement* e : fProgram.elements()) {
    if (e->is<GlobalVarDeclaration>()) {
      const GlobalVarDeclaration& decls = e->as<GlobalVarDeclaration>();
      const Variable& var = decls.declaration()->as<VarDeclaration>().var();
      if (var.modifiers().fFlags & Modifiers::kIn_Flag && -1 == var.modifiers().fLayout.fBuiltin) {
        this->write("    ");
        this->writeType(var.type());
        this->write(" ");
        this->writeName(var.name());
        if (-1 != var.modifiers().fLayout.fLocation) {
          if (fProgram.fConfig->fKind == ProgramKind::kVertex) {
            this->write("  [[attribute(" + to_string(var.modifiers().fLayout.fLocation) + ")]]");
          } else if (fProgram.fConfig->fKind == ProgramKind::kFragment) {
            this->write("  [[user(locn" + to_string(var.modifiers().fLayout.fLocation) + ")]]");
          }
        }
        this->write(";\n");
      }
...
void MetalCodeGenerator::writeOutputStruct() {
  ...
  for (const ProgramElement* e : fProgram.elements()) {
    if (e->is<GlobalVarDeclaration>()) {
      const GlobalVarDeclaration& decls = e->as<GlobalVarDeclaration>();
      const Variable& var = decls.declaration()->as<VarDeclaration>().var();
      if (var.modifiers().fFlags & Modifiers::kOut_Flag && -1 == var.modifiers().fLayout.fBuiltin) {
        this->write("    ");
        this->writeType(var.type());
        this->write(" ");
        this->writeName(var.name());
        ...
        if (location < 0) {
          fContext.fErrors->error(
              var.fOffset, "Metal out variables must have 'layout(location=...)'");
        } else if (fProgram.fConfig->fKind == ProgramKind::kVertex) {
...
void MetalCodeGenerator::writeGlobalStruct() {
  class : public GlobalStructVisitor {
   public:
    void visitInterfaceBlock(const InterfaceBlock& block, skstd::string_view blockName) override {
      this->addElement();
...
void MetalCodeGenerator::writeGlobalInit() {
  class : public GlobalStructVisitor {
   public:
    void visitInterfaceBlock(
        const InterfaceBlock& blockType, skstd::string_view blockName) override {
      this->addElement();
      fCodeGen->write("&");
      fCodeGen->writeName(blockName);
    }
...
void MetalCodeGenerator::writeProgramElement(const ProgramElement& e) {
  switch (e.kind()) {
    case ProgramElement::Kind::kExtension: break;
    case ProgramElement::Kind::kGlobalVar: break;
    ...
    case ProgramElement::Kind::kStructDefinition:
      ...
      break;
    case ProgramElement::Kind::kFunction: this->writeFunction(e.as<FunctionDefinition>()); break;
...
MetalCodeGenerator::Requirements MetalCodeGenerator::requirements(const Expression* e) {
  ...
  switch (e->kind()) {
    ...
    case Expression::Kind::kSwizzle: return this->requirements(e->as<Swizzle>().base().get());
    case Expression::Kind::kBinary: {
      const BinaryExpression& bin = e->as<BinaryExpression>();
      return this->requirements(bin.left().get()) | this->requirements(bin.right().get());
    }
    ...
    default: return kNo_Requirements;
  }
}
...
MetalCodeGenerator::Requirements MetalCodeGenerator::requirements(const Statement* s) {
  ...
  switch (s->kind()) {
    ...
    case Statement::Kind::kIf: {
      ...
      return this->requirements(i.test().get()) | this->requirements(i.ifTrue().get()) |
             this->requirements(i.ifFalse().get());
    }
    case Statement::Kind::kFor: {
      const ForStatement& f = s->as<ForStatement>();
      return this->requirements(f.initializer().get()) | this->requirements(f.test().get()) |
             this->requirements(f.next().get()) | this->requirements(f.statement().get());
    }
    case Statement::Kind::kDo: {
      const DoStatement& d = s->as<DoStatement>();
      return this->requirements(d.test().get()) | this->requirements(d.statement().get());
    }
