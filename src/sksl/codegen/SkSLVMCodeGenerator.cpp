...
class SkSLTracer : public skvm::TraceHook {
 public:
  ...
  void line(int lineNum) override {
    fTrace->fTraceInfo.push_back(
        {SkSL::SkVMTraceInfo::Op::kLine,
         /*data=*/{lineNum, 0}});
  }
  void var(int slot, int32_t val) override {
    fTrace->fTraceInfo.push_back(
        {SkSL::SkVMTraceInfo::Op::kVar,
         /*data=*/{slot, val}});
  }
  void enter(int fnIdx) override {
    fTrace->fTraceInfo.push_back(
        {SkSL::SkVMTraceInfo::Op::kEnter,
         /*data=*/{fnIdx, 0}});
  }
  void exit(int fnIdx) override {
    fTrace->fTraceInfo.push_back(
        {SkSL::SkVMTraceInfo::Op::kExit,
         /*data=*/{fnIdx, 0}});
  }
  void scope(int delta) override {
    fTrace->fTraceInfo.push_back(
        {SkSL::SkVMTraceInfo::Op::kScope,
         /*data=*/{delta, 0}});
  }
...
struct Value {
  Value() = default;
  explicit Value(size_t slots) { fVals.resize(slots); }
  ...
  struct ValRef {
    ...
    ValRef& operator=(ValRef v) {
      fVal = v.fVal;
      return *this;
    }
    ValRef& operator=(skvm::Val v) {
      fVal = v;
      return *this;
    }
    ValRef& operator=(skvm::F32 v) {
      fVal = v.id;
      return *this;
    }
    ValRef& operator=(skvm::I32 v) {
      fVal = v.id;
      return *this;
    }
...
class SkVMGenerator {
 public:
  SkVMGenerator(
      const Program& program, skvm::Builder* builder, SkVMDebugTrace* debugTrace,
      SkVMCallbacks* callbacks);

  void writeProgram(
      SkSpan<skvm::Val> uniforms, skvm::Coord device, const FunctionDefinition& function,
      SkSpan<skvm::Val> arguments, SkSpan<skvm::Val> outReturn);
 ...
 private:
  ...
  void addDebugSlotInfo(const std::string& varName, const Type& type, int line, int fnReturnValue);

  void addDebugSlotInfoForGroup(
      const std::string& varName, const Type& type, int line, int* groupIndex, int fnReturnValue);
  ...
  skvm::F32 f32(skvm::Val id) {
    SkASSERT(id != skvm::NA);
    return {fBuilder, id};
  }
  skvm::I32 i32(skvm::Val id) {
    SkASSERT(id != skvm::NA);
    return {fBuilder, id};
  }

  // Shorthand for scalars
  skvm::F32 f32(const Value& v) {
    SkASSERT(v.slots() == 1);
    return f32(v[0]);
  }
  skvm::I32 i32(const Value& v) {
    SkASSERT(v.slots() == 1);
    return i32(v[0]);
  }
  ...
  void recursiveBinaryCompare(
      const Value& lVal, const Type& lType, const Value& rVal, const Type& rType,
...
SkVMGenerator::SkVMGenerator(
    const Program& program, skvm::Builder* builder, SkVMDebugTrace* debugTrace,
    SkVMCallbacks* callbacks)
    : fProgram(program), fBuilder(builder), fDebugTrace(debugTrace), fCallbacks(callbacks) {}

void SkVMGenerator::writeProgram(
    SkSpan<skvm::Val> uniforms, skvm::Coord device, const FunctionDefinition& function,
    SkSpan<skvm::Val> arguments, SkSpan<skvm::Val> outReturn) {
...
void SkVMGenerator::setupGlobals(SkSpan<skvm::Val> uniforms, skvm::Coord device) {
  if (fDebugTrace) {
    ...
    skvm::Coord traceCoord = {
        to_F32(fBuilder->splat(fDebugTrace->fTraceCoord.fX)) + 0.5f,
    ...
    skvm::I32 xMask = (device.x == traceCoord.x), yMask = (device.y == traceCoord.y);
  ...
  for (const ProgramElement* e : fProgram.elements()) {
    if (e->is<GlobalVarDeclaration>()) {
      ...
      size_t slot = this->getSlot(var), nslots = var.type().slotCount();
      ...
      if (int builtin = var.modifiers().fLayout.fBuiltin; builtin >= 0) {
        switch (builtin) {
          case SK_FRAGCOORD_BUILTIN:
            ...
            break;
          default: SkDEBUGFAILF("Unsupported builtin %d", builtin);
        }
        continue;
      }
...
size_t SkVMGenerator::writeFunction(
    const IRNode& caller, const FunctionDefinition& function, SkSpan<skvm::Val> arguments) {
  ...
  for (const Variable* p : decl.parameters()) {
    size_t paramSlot = this->getSlot(*p), nslots = p->type().slotCount();
...
void SkVMGenerator::addDebugSlotInfoForGroup(
    const std::string& varName, const Type& type, int line, int* groupIndex, int fnReturnValue) {
  SkASSERT(fDebugTrace);
  switch (type.typeKind()) {
    case Type::TypeKind::kArray: {
      ...
      for (int slot = 0; slot < nslots; ++slot) {
        this->addDebugSlotInfoForGroup(
            varName + "[" + std::to_string(slot) + "]", elemType, line, groupIndex, fnReturnValue);
      }
      break;
    }
    case Type::TypeKind::kStruct: {
      for (const Type::Field& field : type.fields()) {
        this->addDebugSlotInfoForGroup(
            varName + "." + std::string(field.fName), *field.fType, line, groupIndex,
            fnReturnValue);
      }
      break;
    }
    default: SkASSERTF(0, "unsupported slot type %d", (int)type.typeKind()); [[fallthrough]];
...
void SkVMGenerator::addDebugSlotInfo(
    const std::string& varName, const Type& type, int line, int fnReturnValue) {
  int groupIndex = 0;
  this->addDebugSlotInfoForGroup(varName, type, line, &groupIndex, fnReturnValue);
  SkASSERT((size_t)groupIndex == type.slotCount());
}

size_t SkVMGenerator::createSlot(
    const std::string& name, const Type& type, int line, int fnReturnValue) {
  size_t slot = fSlots.size(), nslots = type.slotCount();
...
size_t SkVMGenerator::getSlot(const Variable& v) {
  ...
  size_t slot = this->createSlot(
      std::string(v.name()), v.type(), this->getLine(v.fPosition),
      /*fnReturnValue=*/-1);
  fSlotMap.set(&v, slot);
  return slot;
}
...
size_t SkVMGenerator::getFunctionSlot(const IRNode& callSite, const FunctionDefinition& fn) {
  ...
  size_t slot = this->createSlot(
      "[" + std::string(decl.name()) + "].result", decl.returnType(), this->getLine(fn.fPosition),
      /*fnReturnValue=*/1);
...
void SkVMGenerator::recursiveBinaryCompare(
    const Value& lVal, const Type& lType, const Value& rVal, const Type& rType, size_t* slotOffset,
    Value* result, const std::function<Value(skvm::F32 x, skvm::F32 y)>& float_comp,
    const std::function<Value(skvm::I32 x, skvm::I32 y)>& int_comp) {
  switch (lType.typeKind()) {
    case Type::TypeKind::kStruct:
      ...
      for (size_t f = 0; f < lType.fields().size(); ++f) {
        ...
        this->recursiveBinaryCompare(
            lVal, *lField.fType, rVal, *rField.fType, slotOffset, result, float_comp, int_comp);
      }
      break;
    ...
    case Type::TypeKind::kMatrix:
      ...
      for (int c = 0; c < lType.columns(); ++c) {
        this->recursiveBinaryCompare(
            lVal, lType.componentType(), rVal, rType.componentType(), slotOffset, result,
            float_comp, int_comp);
      }
      break;
    default:
      SkASSERT(lType.typeKind() == rType.typeKind() && lType.slotCount() == rType.slotCount());
      Type::NumberKind nk = base_number_kind(lType);
      auto L = lVal[*slotOffset];
      auto R = rVal[*slotOffset];
      (*result)[*slotOffset] = i32(nk == Type::NumberKind::kFloat ? float_comp(f32(L), f32(R))
                                                                  : int_comp(i32(L), i32(R)))
                                   .id;
...
Value SkVMGenerator::writeBinaryExpression(const BinaryExpression& b) {
  ...
  Value lVal = this->writeExpression(left), rVal = this->writeExpression(right);

  // Special case for M*V, V*M, M*M (but not V*V!)
  if (op.kind() == Operator::Kind::STAR && lVecOrMtx && rVecOrMtx &&
      !(lType.isVector() && rType.isVector())) {
    int rCols = rType.columns(), rRows = rType.rows(), lCols = lType.columns(),
        lRows = lType.rows();
  ...
  auto binary = [&](const std::function<Value(skvm::F32 x, skvm::F32 y)>& f_fn,
    ...
    if (op.isEquality() && (lType.isStruct() || lType.isArray())) {
      ...
      size_t slotOffset = 0;
      this->recursiveBinaryCompare(lVal, lType, rVal, rType, &slotOffset, &result, f_fn, i_fn);
  ...
  switch (op.kind()) {
    case Operator::Kind::EQEQ:
      SkASSERT(!isAssignment);
      return binary(
          [](skvm::F32 x, skvm::F32 y) { return x == y; },
    ...
    case Operator::Kind::SLASH:
      ...
      return binary(
          [](skvm::F32 x, skvm::F32 y) { return x / y; },
          [](skvm::I32 x, skvm::I32 y) { return skvm::trunc(skvm::to_F32(x) / skvm::to_F32(y)); });
    ...
    default: SkDEBUGFAIL("Unsupported operator"); return {};
  }
}
...
Value SkVMGenerator::writeTypeConversion(
    const Value& src, Type::NumberKind srcKind, Type::NumberKind dstKind) {
...
Value SkVMGenerator::writeMatrixInverse2x2(const Value& m) {
  SkASSERT(m.slots() == 4);
  skvm::F32 a = f32(m[0]), b = f32(m[1]), c = f32(m[2]), d = f32(m[3]);
...
Value SkVMGenerator::writeMatrixInverse3x3(const Value& m) {
  SkASSERT(m.slots() == 9);
  skvm::F32 a11 = f32(m[0]), a12 = f32(m[3]), a13 = f32(m[6]), a21 = f32(m[1]), a22 = f32(m[4]),
            a23 = f32(m[7]), a31 = f32(m[2]), a32 = f32(m[5]), a33 = f32(m[8]);
  skvm::F32 idet = 1.0f / (a11 * a22 * a33 + a12 * a23 * a31 + a13 * a21 * a32 - a11 * a23 * a32 -
...
Value SkVMGenerator::writeMatrixInverse4x4(const Value& m) {
  SkASSERT(m.slots() == 16);
  skvm::F32 a00 = f32(m[0]), a10 = f32(m[4]), a20 = f32(m[8]), a30 = f32(m[12]), a01 = f32(m[1]),
            a11 = f32(m[5]), a21 = f32(m[9]), a31 = f32(m[13]), a02 = f32(m[2]), a12 = f32(m[6]),
            a22 = f32(m[10]), a32 = f32(m[14]), a03 = f32(m[3]), a13 = f32(m[7]), a23 = f32(m[11]),
            a33 = f32(m[15]);

  skvm::F32 b00 = a00 * *a11 - a01 * *a10, b01 = a00 * *a12 - a02 * *a10,
...
Value SkVMGenerator::writeChildCall(const ChildCall& c) {
  ...
  switch (c.child().type().typeKind()) {
    ...
    case Type::TypeKind::kColorFilter: {
      SkASSERT(c.arguments().size() == 1);
      SkASSERT(
          arg->type().matches(*fProgram.fContext->fTypes.fHalf4) ||
...
Value SkVMGenerator::writeIntrinsicCall(const FunctionCall& c) {
  ...
  auto binary = [&](auto&& fn) {
    ...
    for (size_t i = 0; i < nslots; ++i) {
      result[i] =
          fn({fBuilder, args[0][args[0].slots() == 1 ? 0 : i]},
  ...
  switch (intrinsicKind) {
    ...
    case k_atan_IntrinsicKind:
      return nargs == 1 ? unary(args[0], skvm::approx_atan) : binary(skvm::approx_atan2);
    ...
    case k_sign_IntrinsicKind:
      return unary(
          args[0], [](skvm::F32 x) { return select(x < 0, -1.0f, select(x > 0, +1.0f, 0.0f)); });
    ...
    case k_mix_IntrinsicKind:
      return ternary([](skvm::F32 x, skvm::F32 y, skvm::F32 t) { return skvm::lerp(x, y, t); });
    ...
    case k_inverse_IntrinsicKind: {
      switch (args[0].slots()) {
        ...
        default: SkDEBUGFAIL("Invalid call to inverse"); return {};
      }
    }
  ...
  switch (p.getOperator().kind()) {
    ...
    case Operator::Kind::MINUSMINUS: {
      ...
      switch (base_number_kind(p.type())) {
        case Type::NumberKind::kFloat: val = f32(val) + fBuilder->splat(incr ? 1.0f : -1.0f); break;
        case Type::NumberKind::kSigned: val = i32(val) + fBuilder->splat(incr ? 1 : -1); break;
    ...
    case Operator::Kind::MINUS: {
      switch (base_number_kind(p.type())) {
        case Type::NumberKind::kFloat: return this->unary(val, [](skvm::F32 x) { return -x; });
        case Type::NumberKind::kSigned: return this->unary(val, [](skvm::I32 x) { return -x; });
        default: SkASSERT(false); return {};
      }
    }
    case Operator::Kind::LOGICALNOT:
    case Operator::Kind::BITWISENOT: return this->unary(val, [](skvm::I32 x) { return ~x; });
    default: SkASSERT(false); return {};
  }
}
...
Value SkVMGenerator::writePostfixExpression(const PostfixExpression& p) {
  switch (p.getOperator().kind()) {
    case Operator::Kind::PLUSPLUS:
    case Operator::Kind::MINUSMINUS: {
      Value old = this->writeExpression(*p.operand()), val = old;
...
Value SkVMGenerator::writeExpression(const Expression& e) {
  switch (e.kind()) {
    case Expression::Kind::kBinary: return this->writeBinaryExpression(e.as<BinaryExpression>());
    case Expression::Kind::kChildCall: return this->writeChildCall(e.as<ChildCall>());
    ...
    case Expression::Kind::kFieldAccess: return this->writeFieldAccess(e.as<FieldAccess>());
    case Expression::Kind::kIndex: return this->writeIndexExpression(e.as<IndexExpression>());
    case Expression::Kind::kVariableReference:
      return this->writeVariableExpression(e.as<VariableReference>());
    case Expression::Kind::kLiteral: return this->writeLiteral(e.as<Literal>());
    case Expression::Kind::kFunctionCall: return this->writeFunctionCall(e.as<FunctionCall>());
    case Expression::Kind::kExternalFunctionCall:
      return this->writeExternalFunctionCall(e.as<ExternalFunctionCall>());
    case Expression::Kind::kPrefix: return this->writePrefixExpression(e.as<PrefixExpression>());
    case Expression::Kind::kPostfix: return this->writePostfixExpression(e.as<PostfixExpression>());
    case Expression::Kind::kSwizzle: return this->writeSwizzle(e.as<Swizzle>());
    case Expression::Kind::kTernary: return this->writeTernaryExpression(e.as<TernaryExpression>());
    case Expression::Kind::kExternalFunctionReference:
    default: SkDEBUGFAIL("Unsupported expression"); return {};
  }
}

Value SkVMGenerator::writeStore(const Expression& lhs, const Value& rhs) {
  SkASSERTF(
      rhs.slots() == lhs.type().slotCount(), "lhs=%s (%s)\nrhs=%zu slot",
...
void SkVMGenerator::writeForStatement(const ForStatement& f) {
  ...
  skvm::I32 oldLoopMask = fLoopMask, oldContinueMask = fContinueMask;
  ...
  if (loop.fCount > 0) {
    ...
    for (int i = 0; i < loop.fCount; ++i) {
      this->writeToSlot(
          indexSlot, (indexKind == Type::NumberKind::kFloat)
...
void SkVMGenerator::writeReturnStatement(const ReturnStatement& r) {
  ...
  if (r.expression()) {
    ...
    for (size_t i = 0; i < nslots; ++i) {
      ...
      skvm::Val conditionalStore =
          this->writeConditionalStore(fSlots[slot + i].val, val[i], returnsHere);
  ...
  for (const std::unique_ptr<Statement>& stmt : s.cases()) {
    ...
    if (!c.isDefault()) {
      ...
      ScopedCondition conditionalCaseBlock(
          this, switchFallthrough | (i32(caseValue) == i32(switchValue)));
      ...
      switchFallthrough.id =
          this->writeConditionalStore(switchFallthrough.id, trueValue.id, this->mask());
    } else {
...
void SkVMGenerator::writeVarDeclaration(const VarDeclaration& decl) {
  size_t slot = this->getSlot(decl.var()), nslots = decl.var().type().slotCount();
...
void SkVMGenerator::writeStatement(const Statement& s) {
  ...
  switch (s.kind()) {
    case Statement::Kind::kBlock: this->writeBlock(s.as<Block>()); break;
    case Statement::Kind::kBreak: this->writeBreakStatement(); break;
    case Statement::Kind::kContinue: this->writeContinueStatement(); break;
    case Statement::Kind::kExpression:
      this->writeExpression(*s.as<ExpressionStatement>().expression());
      break;
    case Statement::Kind::kFor: this->writeForStatement(s.as<ForStatement>()); break;
    case Statement::Kind::kIf: this->writeIfStatement(s.as<IfStatement>()); break;
    case Statement::Kind::kReturn: this->writeReturnStatement(s.as<ReturnStatement>()); break;
    case Statement::Kind::kSwitch: this->writeSwitchStatement(s.as<SwitchStatement>()); break;
    case Statement::Kind::kVarDeclaration: this->writeVarDeclaration(s.as<VarDeclaration>()); break;
    case Statement::Kind::kDiscard:
    case Statement::Kind::kDo: SkDEBUGFAIL("Unsupported control flow"); break;
    case Statement::Kind::kNop: break;
    default: SkDEBUGFAIL("Unrecognized statement"); break;
  }
}

skvm::Color ProgramToSkVM(
    const Program& program, const FunctionDefinition& function, skvm::Builder* builder,
    SkVMDebugTrace* debugTrace, SkSpan<skvm::Val> uniforms, skvm::Coord device, skvm::Coord local,
    skvm::Color inputColor, skvm::Color destColor, SkVMCallbacks* callbacks) {
  ...
  for (const SkSL::Variable* param : function.declaration().parameters()) {
    switch (param->modifiers().fLayout.fBuiltin) {
      ...
      case SK_DEST_COLOR_BUILTIN:
        ...
        break;
      default: SkDEBUGFAIL("Invalid parameter to main()"); return {};
    }
  }
  ...
  return skvm::Color{
      {builder, result[0]}, {builder, result[1]}, {builder, result[2]}, {builder, result[3]}};
}

bool ProgramToSkVM(
    const Program& program, const FunctionDefinition& function, skvm::Builder* b,
    SkVMDebugTrace* debugTrace, SkSpan<skvm::Val> uniforms, SkVMSignature* outSignature) {
  SkVMSignature ignored, *signature = outSignature ? outSignature : &ignored;
  ...
  switch (type.typeKind()) {
    ...
    case Type::TypeKind::kArray:
      for (int i = 0; i < type.columns(); ++i) {
        gather_uniforms(info, type.componentType(), String::printf("%s[%d]", name.c_str(), i));
      }
      break;
    case Type::TypeKind::kScalar:
    case Type::TypeKind::kVector:
    case Type::TypeKind::kMatrix:
      info->fUniforms.push_back(
          {name, base_number_kind(type), type.rows(), type.columns(), info->fUniformSlotCount});
 ...
 */
bool testingOnly_ProgramToSkVMShader(
    const Program& program, skvm::Builder* builder, SkVMDebugTrace* debugTrace) {
   ...
   public:
    Callbacks(skvm::Builder* builder, skvm::Uniforms* uniforms, int numChildren) {
      for (int i = 0; i < numChildren; ++i) {
        fChildren.push_back({uniforms->pushPtr(nullptr), builder->uniform32(uniforms->push(0))});
      }
    }
    ...
    skvm::Color sampleColorFilter(int i, skvm::Color color) override { return color; }
    ...
    // test files.
    skvm::Color toLinearSrgb(skvm::Color color) override { return color; }
    skvm::Color fromLinearSrgb(skvm::Color color) override { return color; }
  ...
  skvm::Color result = SkSL::ProgramToSkVM(
      program, *main, builder, debugTrace, SkMakeSpan(uniformVals), device, local, inColor,
      destColor, &callbacks);
