...
class SkVMGenerator {
 public:
  SkVMGenerator(
      const Program& program, skvm::Builder* builder, SkSpan<skvm::Val> uniforms,
      skvm::Coord device, skvm::Coord local, SampleShaderFn sampleShader,
      SampleColorFilterFn sampleColorFilter, SampleBlenderFn sampleBlender);

  void writeFunction(
      const FunctionDefinition& function, SkSpan<skvm::Val> arguments, SkSpan<skvm::Val> outReturn);
...
SkVMGenerator::SkVMGenerator(
    const Program& program, skvm::Builder* builder, SkSpan<skvm::Val> uniforms, skvm::Coord device,
    skvm::Coord local, SampleShaderFn sampleShader, SampleColorFilterFn sampleColorFilter,
      ...
      if (int builtin = var.modifiers().fLayout.fBuiltin; builtin >= 0) {
        ...
        switch (builtin) {
          case SK_FRAGCOORD_BUILTIN:
            ...
            break;
          default: SkDEBUGFAIL("Unsupported builtin");
        }
...
void SkVMGenerator::writeFunction(
    const FunctionDefinition& function, SkSpan<skvm::Val> arguments, SkSpan<skvm::Val> outReturn) {
...
size_t SkVMGenerator::getSlot(const Variable& v) {
  ...
  size_t slot = fSlots.size(), nslots = v.type().slotCount();
...
Value SkVMGenerator::writeBinaryExpression(const BinaryExpression& b) {
  ...
  if (op.kind() == Token::Kind::TK_STAR && lVecOrMtx && rVecOrMtx &&
...
Value SkVMGenerator::writeChildCall(const ChildCall& c) {
  ...
  switch (c.child().type().typeKind()) {
    ...
    case Type::TypeKind::kColorFilter: {
      SkASSERT(c.arguments().size() == 1);
      SkASSERT(
          arg->type() == *fProgram.fContext->fTypes.fHalf4 ||
...
Value SkVMGenerator::writeFunctionCall(const FunctionCall& f) {
...

  {
    ...
    SkASSERTF(
        f.function().definition(), "no definition for function '%s'",
...
Value SkVMGenerator::writePrefixExpression(const PrefixExpression& p) {
  ...
  switch (p.getOperator().kind()) {
    ...
    case Token::Kind::TK_BITWISENOT: return this->unary(val, [](skvm::I32 x) { return ~x; });
    default: SkASSERT(false); return {};
  }
}
...
Value SkVMGenerator::writeExpression(const Expression& e) {
  switch (e.kind()) {
    ...
    case Expression::Kind::kFloatLiteral: return fBuilder->splat(e.as<FloatLiteral>().value());
...
Value SkVMGenerator::writeStore(const Expression& lhs, const Value& rhs) {
  ...
  for (size_t i = rhs.slots(); i-- > 0;) {
    ...
    skvm::F32 curr = f32(fSlots[varSlot + slots[i]]), next = f32(rhs[i]);
...
void SkVMGenerator::writeStatement(const Statement& s) {
  switch (s.kind()) {
    ...
    case Statement::Kind::kSwitch: SkDEBUGFAIL("Unsupported control flow"); break;
...
skvm::Color ProgramToSkVM(
    ...
    SkSpan<skvm::Val> uniforms, skvm::Coord device, skvm::Coord local, skvm::Color inputColor,
    skvm::Color destColor, SampleShaderFn sampleShader, SampleColorFilterFn sampleColorFilter,
    SampleBlenderFn sampleBlender) {
  ...
  SkVMGenerator generator(
      program, builder, uniforms, device, local, std::move(sampleShader),
...
bool ProgramToSkVM(
    ...
    SkSpan<skvm::Val> uniforms, SkVMSignature* outSignature) {
  ...
  SkVMGenerator generator(
      program, b, uniforms, /*device=*/zeroCoord, /*local=*/zeroCoord, sampleShader,
...
bool testingOnly_ProgramToSkVMShader(const Program& program, skvm::Builder* builder) {
  ...
  skvm::Color result = SkSL::ProgramToSkVM(
      program, *main, builder, SkMakeSpan(uniformVals), device, local, inColor, destColor,
      sampleShader,
