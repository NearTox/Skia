...
class CodeGenerator {
 public:
  CodeGenerator(const Context* context, const Program* program, OutputStream* out) noexcept 
      : fContext(*context), fProgram(*program), fOut(out) {}
  ...
  OutputStream* outputStream() noexcept { return fOut; }
  void setOutputStream(OutputStream* output) noexcept { fOut = output; }
...
class AutoOutputStream {
 public:
  ...
  AutoOutputStream(CodeGenerator* codeGen, OutputStream* newOutput)
      : fCodeGen(codeGen), fOldOutput(codeGen->outputStream()) {
