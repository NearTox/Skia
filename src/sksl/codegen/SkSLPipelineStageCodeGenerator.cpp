  ...
  String sampleOutput;
  {
    ...
    switch (c.child().type().typeKind()) {
      ...
      case Type::TypeKind::kColorFilter: {
        SkASSERT(arguments.size() == 1);
        SkASSERT(
            arguments[0]->type() == *fProgram.fContext->fTypes.fHalf4 ||
      ...
      case Type::TypeKind::kBlender: {
        ...
        SkASSERT(
            ...
            arguments[0]->type() == *fProgram.fContext->fTypes.fFloat4);
        SkASSERT(
            arguments[1]->type() == *fProgram.fContext->fTypes.fHalf4 ||
...
void PipelineStageCodeGenerator::writeFunction(const FunctionDefinition& f) {
  ...
  String declString = String::printf(
  ...
  for (const Variable* p : decl.parameters()) {
    // TODO: Handle arrays
    declString.appendf(
        "%s%s%s %s", separator, this->modifierString(p->modifiers()).c_str(),
        this->typeName(p->type()).c_str(), String(p->name()).c_str());
    separator = ", ";
  }
  declString.append(")");
...
void PipelineStageCodeGenerator::writeGlobalVarDeclaration(const GlobalVarDeclaration& g) {
  ...
  if (var.isBuiltin() || var.type().isOpaque()) {
    ...
    String declaration = this->modifierString(var.modifiers()) +
                         this->typedVariable(var.type(), skstd::string_view(mangledName.c_str()));
    if (decl.value()) {
...
void PipelineStageCodeGenerator::writeExpression(
  ...
  switch (expr.kind()) {
    ...
    case Expression::Kind::kIntLiteral: this->write(expr.description()); break;
