...
class PipelineStageCodeGenerator {
 public:
  PipelineStageCodeGenerator(
      const Program& program, const char* sampleCoords, const char* inputColor,
      const char* destColor, Callbacks* callbacks)
      : fProgram(program),
 ...
 private:
  ...
  struct AutoOutputBuffer {
    ...
    ~AutoOutputBuffer() { fGenerator->fBuffer = fOldBuffer; }
...
void PipelineStageCodeGenerator::write(std::string_view s) { fBuffer->write(s.data(), s.length()); }
  ...
  std::string sampleOutput;
  {
    ...
    switch (c.child().type().typeKind()) {
      ...
      case Type::TypeKind::kColorFilter: {
        SkASSERT(arguments.size() == 1);
        SkASSERT(
            arguments[0]->type().matches(*fProgram.fContext->fTypes.fHalf4) ||
      ...
      case Type::TypeKind::kBlender: {
        ...
        SkASSERT(
            ...
            arguments[0]->type().matches(*fProgram.fContext->fTypes.fFloat4));
        SkASSERT(
            arguments[1]->type().matches(*fProgram.fContext->fTypes.fHalf4) ||
        ...
        sampleOutput =
            fCallbacks->sampleBlender(index, exprBuffer.fBuffer.str(), exprBuffer2.fBuffer.str());
        break;
      }
      default: {
...
void PipelineStageCodeGenerator::writeFunction(const FunctionDefinition& f) {
  ...
  if (decl.isMain() && fProgram.fConfig->fKind != SkSL::ProgramKind::kMeshVertex &&
  ...
  fCallbacks->defineFunction(
      this->functionDeclaration(decl).c_str(), body.fBuffer.str().c_str(), decl.isMain());
}
...
std::string PipelineStageCodeGenerator::functionDeclaration(const FunctionDeclaration& decl) {
  ...
  std::string declString = String::printf(
      "%s%s%s %s(", (decl.modifiers().fFlags & Modifiers::kInline_Flag) ? "inline " : "",
      (decl.modifiers().fFlags & Modifiers::kNoInline_Flag) ? "noinline " : "",
      this->typeName(decl.returnType()).c_str(), this->functionName(decl).c_str());
  const char* separator = "";
...
void PipelineStageCodeGenerator::writeGlobalVarDeclaration(const GlobalVarDeclaration& g) {
  ...
  if (var.isBuiltin() || var.type().isOpaque()) {
    ...
    std::string declaration =
        this->modifierString(var.modifiers()) +
        this->typedVariable(var.type(), std::string_view(mangledName.c_str()));
    if (decl.value()) {
...
void PipelineStageCodeGenerator::writeType(const Type& type) { this->write(this->typeName(type)); }

void PipelineStageCodeGenerator::writeExpression(
  ...
  switch (expr.kind()) {
    case Expression::Kind::kBinary:
      ...
      break;
    case Expression::Kind::kLiteral: this->write(expr.description()); break;
    case Expression::Kind::kChildCall: this->writeChildCall(expr.as<ChildCall>()); break;
...
void PipelineStageCodeGenerator::writeAnyConstructor(
...
void PipelineStageCodeGenerator::writeBinaryExpression(
...
void PipelineStageCodeGenerator::writeTernaryExpression(
...
void PipelineStageCodeGenerator::writePrefixExpression(
...
void PipelineStageCodeGenerator::writePostfixExpression(
...
void PipelineStageCodeGenerator::writeStatement(const Statement& s) {
  switch (s.kind()) {
    ...
    case Statement::Kind::kDiscard: SkDEBUGFAIL("Unsupported control flow"); break;
...
void ConvertProgram(
    const Program& program, const char* sampleCoords, const char* inputColor, const char* destColor,
    Callbacks* callbacks) {
