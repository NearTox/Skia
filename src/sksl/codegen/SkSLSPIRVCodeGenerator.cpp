...
bool SPIRVCodeGenerator::Instruction::operator==(
    const SPIRVCodeGenerator::Instruction& that) const {
  return fOp == that.fOp && fResultKind == that.fResultKind && fWords == that.fWords;
}
...
struct SPIRVCodeGenerator::Word {
  ...
  static Word Number(int32_t val) { return Word{val, Kind::kNumber}; }

  static Word Result(const Type& type) {
    return (type.hasPrecision() && !type.highPrecision()) ? RelaxedResult() : Result();
  }

  static Word RelaxedResult() { return Word{(int32_t)NA, kRelaxedPrecisionResult}; }

  static Word UniqueResult() { return Word{(int32_t)NA, kUniqueResult}; }

  static Word Result() { return Word{(int32_t)NA, kDefaultPrecisionResult}; }
...
#define ALL_GLSL(x)                                                                               \
  Intrinsic {                                                                                     \
    kGLSL_STD_450_IntrinsicOpcodeKind, GLSLstd450##x, GLSLstd450##x, GLSLstd450##x, GLSLstd450##x \
  }
#define BY_TYPE_GLSL(ifFloat, ifInt, ifUInt)                                                       \
  Intrinsic {                                                                                      \
    kGLSL_STD_450_IntrinsicOpcodeKind, GLSLstd450##ifFloat, GLSLstd450##ifInt, GLSLstd450##ifUInt, \
        SpvOpUndef                                                                                 \
  }
#define ALL_SPIRV(x) \
  Intrinsic { kSPIRV_IntrinsicOpcodeKind, SpvOp##x, SpvOp##x, SpvOp##x, SpvOp##x }
#define BOOL_SPIRV(x) \
  Intrinsic { kSPIRV_IntrinsicOpcodeKind, SpvOpUndef, SpvOpUndef, SpvOpUndef, SpvOp##x }
#define FLOAT_SPIRV(x) \
  Intrinsic { kSPIRV_IntrinsicOpcodeKind, SpvOp##x, SpvOpUndef, SpvOpUndef, SpvOpUndef }
#define SPECIAL(x)                                                                  \
  Intrinsic {                                                                       \
    kSpecial_IntrinsicOpcodeKind, k##x##_SpecialIntrinsic, k##x##_SpecialIntrinsic, \
...
#undef PACK
    ...
    case k_equal_IntrinsicKind:
      return Intrinsic{
          kSPIRV_IntrinsicOpcodeKind, SpvOpFOrdEqual, SpvOpIEqual, SpvOpIEqual, SpvOpLogicalEqual};
    case k_notEqual_IntrinsicKind:
      return Intrinsic{
          kSPIRV_IntrinsicOpcodeKind, SpvOpFUnordNotEqual, SpvOpINotEqual, SpvOpINotEqual,
          SpvOpLogicalNotEqual};
    case k_lessThan_IntrinsicKind:
      return Intrinsic{
          kSPIRV_IntrinsicOpcodeKind, SpvOpFOrdLessThan, SpvOpSLessThan, SpvOpULessThan,
          SpvOpUndef};
    case k_lessThanEqual_IntrinsicKind:
      return Intrinsic{
          kSPIRV_IntrinsicOpcodeKind, SpvOpFOrdLessThanEqual, SpvOpSLessThanEqual,
          SpvOpULessThanEqual, SpvOpUndef};
    case k_greaterThan_IntrinsicKind:
      return Intrinsic{
          kSPIRV_IntrinsicOpcodeKind, SpvOpFOrdGreaterThan, SpvOpSGreaterThan, SpvOpUGreaterThan,
          SpvOpUndef};
    case k_greaterThanEqual_IntrinsicKind:
      return Intrinsic{
          kSPIRV_IntrinsicOpcodeKind, SpvOpFOrdGreaterThanEqual, SpvOpSGreaterThanEqual,
          SpvOpUGreaterThanEqual, SpvOpUndef};
    default: return Intrinsic{kInvalid_IntrinsicOpcodeKind, 0, 0, 0, 0};
  }
}
...
static bool is_float(const Type& type) {
  return (type.isScalar() || type.isVector() || type.isMatrix()) && type.componentType().isFloat();
}
...
static bool is_out(const Modifiers& m) { return (m.fFlags & Modifiers::kOut_Flag) != 0; }
...
static bool is_control_flow_op(SpvOp_ op) {
  switch (op) {
    ...
    case SpvOpSwitch:
    case SpvOpBranch:
    case SpvOpBranchConditional: return true;
...
static bool is_globally_reachable_op(SpvOp_ op) {
  switch (op) {
    ...
    case SpvOpDecorate:
    case SpvOpMemberDecorate: return true;
    default: return false;
  }
}
...
void SPIRVCodeGenerator::writeLabel(
    SpvId label, BranchingLabelType type, ConditionalOpCounts ops, OutputStream& out) {
  switch (type) {
    case kBranchIsBelow:
...
void SPIRVCodeGenerator::writeString(std::string_view s, OutputStream& out) {
  ...
  switch (s.length() % 4) {
    ...
    default: this->writeWord(0, out); break;
  }
}

void SPIRVCodeGenerator::writeInstruction(
    SpvOp_ opCode, std::string_view string, OutputStream& out) {
  this->writeOpCode(opCode, 1 + (string.length() + 4) / 4, out);
  this->writeString(string, out);
}

void SPIRVCodeGenerator::writeInstruction(
    SpvOp_ opCode, int32_t word1, std::string_view string, OutputStream& out) {
...
void SPIRVCodeGenerator::writeInstruction(
    SpvOp_ opCode, int32_t word1, int32_t word2, std::string_view string, OutputStream& out) {
...
void SPIRVCodeGenerator::writeInstruction(
    SpvOp_ opCode, int32_t word1, int32_t word2, int32_t word3, OutputStream& out) {
...
void SPIRVCodeGenerator::writeInstruction(
    SpvOp_ opCode, int32_t word1, int32_t word2, int32_t word3, int32_t word4, OutputStream& out) {
...
void SPIRVCodeGenerator::writeInstruction(
    SpvOp_ opCode, int32_t word1, int32_t word2, int32_t word3, int32_t word4, int32_t word5,
    OutputStream& out) {
...
void SPIRVCodeGenerator::writeInstruction(
    SpvOp_ opCode, int32_t word1, int32_t word2, int32_t word3, int32_t word4, int32_t word5,
    int32_t word6, int32_t word7, int32_t word8, OutputStream& out) {
...
SpvId SPIRVCodeGenerator::writeInstruction(
    SpvOp_ opCode, const SkTArray<Word>& words, OutputStream& out) {
  ...
  switch (key.fResultKind) {
    ...
    case Word::Kind::kRelaxedPrecisionResult: precision = Precision::kRelaxed; [[fallthrough]];
    ...
    default: SkDEBUGFAIL("unexpected result kind"); break;
  }
...
SpvId SPIRVCodeGenerator::writeOpLoad(
    SpvId type, Precision precision, SpvId pointer, OutputStream& out) {
...
void SPIRVCodeGenerator::writeOpStore(
    SpvStorageClass_ storageClass, SpvId pointer, SpvId value, OutputStream& out) {
...
SpvId SPIRVCodeGenerator::writeOpConstantTrue(const Type& type) {
  return this->writeInstruction(
      SpvOpConstantTrue, Words{this->getType(type), Word::Result()}, fConstantBuffer);
}

SpvId SPIRVCodeGenerator::writeOpConstantFalse(const Type& type) {
  return this->writeInstruction(
      SpvOpConstantFalse, Words{this->getType(type), Word::Result()}, fConstantBuffer);
}

SpvId SPIRVCodeGenerator::writeOpConstant(const Type& type, int32_t valueBits) {
  return this->writeInstruction(
      SpvOpConstant, Words{this->getType(type), Word::Result(), Word::Number(valueBits)},
      fConstantBuffer);
}

SpvId SPIRVCodeGenerator::writeOpConstantComposite(
    const Type& type, const SkTArray<SpvId>& values) {
...
bool SPIRVCodeGenerator::toConstants(SpvId value, SkTArray<SpvId>* constants) {
  ...
  switch (instr->fOp) {
    case SpvOpConstant:
    case SpvOpConstantTrue:
    case SpvOpConstantFalse: constants->push_back(value); return true;
...
SpvId SPIRVCodeGenerator::writeOpCompositeConstruct(
    const Type& type, const SkTArray<SpvId>& values, OutputStream& out) {
  ...
  if (type.isMatrix()) {
    ...
    if (this->toConstants(SkMakeSpan(values), &constants)) {
      ...
      const Type& vecType = type.componentType().toCompound(
          fContext,
          /*columns=*/type.rows(),
          /*rows=*/1);
      SkSTArray<4, SpvId> columnIDs;
      for (int index = 0; index < type.columns(); ++index) {
        SkSTArray<4, SpvId> columnConstants(&constants[index * type.rows()], type.rows());
...
SPIRVCodeGenerator::Instruction* SPIRVCodeGenerator::resultTypeForInstruction(
  ...
  switch (instr.fOp) {
    ...
    case SpvOpLoad: resultTypeWord = 0; break;
...
int SPIRVCodeGenerator::numComponentsForVecInstruction(const Instruction& instr) {
  ...
  SkASSERT(typeInstr);
  SkASSERT(
      typeInstr->fOp == SpvOpTypeVector || typeInstr->fOp == SpvOpTypeFloat ||
  ...
  return (typeInstr->fOp == SpvOpTypeVector) ? typeInstr->fWords[2] : 1;
}
...
SpvId SPIRVCodeGenerator::writeOpCompositeExtract(
    const Type& type, SpvId base, int component, OutputStream& out) {
  ...
  return this->writeInstruction(
      ...
      {this->getType(type), Word::Result(type), base, Word::Number(component)}, out);
}

SpvId SPIRVCodeGenerator::writeOpCompositeExtract(
    const Type& type, SpvId base, int componentA, int componentB, OutputStream& out) {
  // If the base op is a composite, we can extract from it directly.
  ...
  return this->writeInstruction(
      SpvOpCompositeExtract,
      {this->getType(type), Word::Result(type), base, Word::Number(componentA),
...
SpvId SPIRVCodeGenerator::nextId(const Type* type) {
  return this->nextId(
      type && type->hasPrecision() && !type->highPrecision() ? Precision::kRelaxed
                                                             : Precision::kDefault);
}
...
SpvId SPIRVCodeGenerator::nextId(Precision precision) {
  if (precision == Precision::kRelaxed && !fProgram.fConfig->fSettings.fForceHighPrecision) {
    this->writeInstruction(
        SpvOpDecorate, fIdCount, SpvDecorationRelaxedPrecision, fDecorationBuffer);
  }
  return fIdCount++;
}
  ...
  for (int32_t i = 0; i < (int32_t)type.fields().size(); i++) {
    ...
    if (!memoryLayout.isSupported(*field.fType)) {
      fContext.fErrors->error(
          type.fPosition, "type '" + field.fType->displayName() + "' is not permitted here");
      return resultId;
    }
    ...
    if (fieldLayout.fOffset >= 0) {
      if (fieldLayout.fOffset < (int)offset) {
        fContext.fErrors->error(
            field.fPosition, "offset of field '" + std::string(field.fName) +
    ...
    if (field.fModifiers.fLayout.fBuiltin < 0) {
      this->writeInstruction(
          SpvOpMemberDecorate, resultId, (SpvId)i, SpvDecorationOffset, (SpvId)offset,
          fDecorationBuffer);
    }
    if (field.fType->isMatrix()) {
      this->writeInstruction(
          SpvOpMemberDecorate, resultId, i, SpvDecorationColMajor, fDecorationBuffer);
      this->writeInstruction(
          SpvOpMemberDecorate, resultId, i, SpvDecorationMatrixStride,
          (SpvId)memoryLayout.stride(*field.fType), fDecorationBuffer);
    }
    if (!field.fType->highPrecision()) {
      this->writeInstruction(
          SpvOpMemberDecorate, resultId, (SpvId)i, SpvDecorationRelaxedPrecision,
          fDecorationBuffer);
    }
    offset += size;
...
SpvId SPIRVCodeGenerator::getType(const Type& type) { return this->getType(type, fDefaultLayout); }
...
SpvId SPIRVCodeGenerator::getType(const Type& rawType, const MemoryLayout& layout) {
  ...
  switch (type->typeKind()) {
    ...
    case Type::TypeKind::kLiteral: {
      ...
      if (type->isSigned()) {
        return this->writeInstruction(
            SpvOpTypeInt, Words{Word::Result(), Word::Number(32), Word::Number(1)},
            fConstantBuffer);
      }
      if (type->isUnsigned()) {
        return this->writeInstruction(
            SpvOpTypeInt, Words{Word::Result(), Word::Number(32), Word::Number(0)},
            fConstantBuffer);
      }
      if (type->isFloat()) {
        return this->writeInstruction(
            SpvOpTypeFloat, Words{Word::Result(), Word::Number(32)}, fConstantBuffer);
      }
    ...
    case Type::TypeKind::kVector: {
      ...
      return this->writeInstruction(
          SpvOpTypeVector, Words{Word::Result(), scalarTypeId, Word::Number(type->columns())},
          fConstantBuffer);
    }
    case Type::TypeKind::kMatrix: {
      SpvId vectorTypeId = this->getType(IndexExpression::IndexType(fContext, *type), layout);
      return this->writeInstruction(
          SpvOpTypeMatrix, Words{Word::Result(), vectorTypeId, Word::Number(type->columns())},
          fConstantBuffer);
    }
    case Type::TypeKind::kArray: {
      if (!layout.isSupported(*type)) {
        fContext.fErrors->error(
            type->fPosition, "type '" + type->displayName() + "' is not permitted here");
        return NA;
      }
      if (type->columns() == 0) {
      ...
      this->writeInstruction(
          SpvOpDecorate, {result, SpvDecorationArrayStride, Word::Number(layout.stride(*type))},
          fDecorationBuffer);
      return result;
    }
    ...
    case Type::TypeKind::kSampler: {
      ...
      return this->writeInstruction(
          SpvOpTypeSampledImage, Words{Word::Result(), imageTypeId}, fConstantBuffer);
    }
    case Type::TypeKind::kTexture: {
      ...
      return this->writeInstruction(
          SpvOpTypeImage,
          Words{
              Word::Result(), floatTypeId, Word::Number(type->dimensions()),
              Word::Number(type->isDepth()), Word::Number(type->isArrayedTexture()),
              Word::Number(type->isMultisampled()), Word::Number(type->isSampled() ? 1 : 2),
...
SpvId SPIRVCodeGenerator::getPointerType(
    const Type& type, const MemoryLayout& layout, SpvStorageClass_ storageClass) {
  return this->writeInstruction(
      SpvOpTypePointer, Words{Word::Result(), Word::Number(storageClass), this->getType(type)},
      fConstantBuffer);
}
...
SpvId SPIRVCodeGenerator::writeExpression(const Expression& expr, OutputStream& out) {
  switch (expr.kind()) {
    ...
    case Expression::Kind::kFieldAccess: return this->writeFieldAccess(expr.as<FieldAccess>(), out);
    case Expression::Kind::kFunctionCall:
      return this->writeFunctionCall(expr.as<FunctionCall>(), out);
    case Expression::Kind::kLiteral: return this->writeLiteral(expr.as<Literal>());
    ...
    case Expression::Kind::kSwizzle: return this->writeSwizzle(expr.as<Swizzle>(), out);
    ...
    default: SkDEBUGFAILF("unsupported expression: %s", expr.description().c_str()); break;
  }
  return NA;
}
...
SpvId SPIRVCodeGenerator::writeIntrinsicCall(const FunctionCall& c, OutputStream& out) {
  ...
  if (arguments.size() > 0) {
    ...
    if (intrinsic.opKind == kSpecial_IntrinsicOpcodeKind) {
      ...
      intrinsicId = pick_by_type(
          type, intrinsic.floatOp, intrinsic.signedOp, intrinsic.unsignedOp, intrinsic.boolOp);
    }
  }
  switch (intrinsic.opKind) {
    ...
    default:
      fContext.fErrors->error(
          c.fPosition, "unsupported intrinsic '" + function.description() + "'");
      return NA;
  }
}
...
SpvId SPIRVCodeGenerator::vectorize(const Expression& arg, int vectorSize, OutputStream& out) {
  ...
  if (argType.isScalar() && vectorSize > 1) {
    ConstructorSplat splat{
        arg.fPosition, argType.toCompound(fContext, vectorSize, /*rows=*/1), arg.clone()};
...
void SPIRVCodeGenerator::writeGLSLExtendedInstruction(
    const Type& type, SpvId id, SpvId floatInst, SpvId signedInst, SpvId unsignedInst,
...
SpvId SPIRVCodeGenerator::writeSpecialIntrinsic(
  ...
  switch (kind) {
    ...
    case kSampledImage_SpecialIntrinsic: {
      ...
      this->writeInstruction(SpvOpSampledImage, this->getType(callType), result, img, sampler, out);
      break;
    }
    case kSubpassLoad_SpecialIntrinsic: {
      ...
      if (arguments.size() == 1) {
        this->writeInstruction(SpvOpImageRead, this->getType(callType), result, img, coords, out);
      } else {
        SkASSERT(arguments.size() == 2);
        SpvId sample = this->writeExpression(*arguments[1], out);
        this->writeInstruction(
            SpvOpImageRead, this->getType(callType), result, img, coords,
            SpvImageOperandsSampleMask, sample, out);
      }
      break;
    }
    case kTexture_SpecialIntrinsic: {
      ...
      if (arguments.size() == 3) {
        this->writeInstruction(
            op, type, result, sampler, uv, SpvImageOperandsBiasMask,
            this->writeExpression(*arguments[2], out), out);
      } else {
        SkASSERT(arguments.size() == 2);
        if (fProgram.fConfig->fSettings.fSharpenTextures) {
          SpvId lodBias = this->writeLiteral(kSharpenTexturesBias, *fContext.fTypes.fFloat);
          this->writeInstruction(
              op, type, result, sampler, uv, SpvImageOperandsBiasMask, lodBias, out);
        } else {
          this->writeInstruction(op, type, result, sampler, uv, out);
        }
      }
      break;
    }
    ...
    case kClamp_SpecialIntrinsic: {
      ...
      this->writeGLSLExtendedInstruction(
          callType, result, GLSLstd450FClamp, GLSLstd450SClamp, GLSLstd450UClamp, args, out);
      break;
    }
    case kMax_SpecialIntrinsic: {
      ...
      SkASSERT(args.size() == 2);
      this->writeGLSLExtendedInstruction(
          callType, result, GLSLstd450FMax, GLSLstd450SMax, GLSLstd450UMax, args, out);
      break;
    }
    case kMin_SpecialIntrinsic: {
      ...
      SkASSERT(args.size() == 2);
      this->writeGLSLExtendedInstruction(
          callType, result, GLSLstd450FMin, GLSLstd450SMin, GLSLstd450UMin, args, out);
      break;
    }
    case kMix_SpecialIntrinsic: {
      ...
      if (arguments[2]->type().componentType().isBoolean()) {
        ...
        this->writeInstruction(
            SpvOpSelect, this->getType(arguments[0]->type()), result, conditionId, trueId, falseId,
            out);
      } else {
        this->writeGLSLExtendedInstruction(
            callType, result, GLSLstd450FMix, SpvOpUndef, SpvOpUndef, args, out);
      }
      break;
    }
    case kSaturate_SpecialIntrinsic: {
      ...
      this->writeGLSLExtendedInstruction(
          callType, result, GLSLstd450FClamp, GLSLstd450SClamp, GLSLstd450UClamp, spvArgs, out);
      break;
    }
    case kSmoothStep_SpecialIntrinsic: {
      ...
      SkASSERT(args.size() == 3);
      this->writeGLSLExtendedInstruction(
          callType, result, GLSLstd450SmoothStep, SpvOpUndef, SpvOpUndef, args, out);
      break;
    }
    case kStep_SpecialIntrinsic: {
      ...
      SkASSERT(args.size() == 2);
      this->writeGLSLExtendedInstruction(
          callType, result, GLSLstd450Step, SpvOpUndef, SpvOpUndef, args, out);
      break;
    }
...
SpvId SPIRVCodeGenerator::writeFunctionCallArgument(
    const FunctionCall& call, int argIndex, std::vector<TempVar>* tempVars, OutputStream& out) {
  ...
  this->writeInstruction(
      SpvOpVariable, this->getPointerType(arg.type(), SpvStorageClassFunction), tmpVar,
      SpvStorageClassFunction, fVariableBuffer);
  if (tmpValueId != NA) {
...
SpvId SPIRVCodeGenerator::writeFunctionCall(const FunctionCall& c, OutputStream& out) {
  ...
  if (!entry) {
    fContext.fErrors->error(
        c.fPosition, "function '" + function.description() + "' is not defined");
    return NA;
  }
...
SpvId SPIRVCodeGenerator::castScalarToType(
    SpvId inputExprId, const Type& inputType, const Type& outputType, OutputStream& out) {
  ...
  fContext.fErrors->error(
      Position(),
      "unsupported cast: " + inputType.description() + " to " + outputType.description());
  return inputExprId;
}
...
SpvId SPIRVCodeGenerator::castScalarToFloat(
    SpvId inputId, const Type& inputType, const Type& outputType, OutputStream& out) {
  ...
  if (inputType.isBoolean()) {
    ...
    this->writeInstruction(
        SpvOpSelect, this->getType(outputType), result, inputId, oneID, zeroID, out);
...
SpvId SPIRVCodeGenerator::castScalarToSignedInt(
    SpvId inputId, const Type& inputType, const Type& outputType, OutputStream& out) {
  ...
  if (inputType.isBoolean()) {
    ...
    SkDEBUGFAILF("unsupported type for signed int typecast: %s", inputType.description().c_str());
    return NA;
  }
  return result;
}
...
SpvId SPIRVCodeGenerator::castScalarToUnsignedInt(
    SpvId inputId, const Type& inputType, const Type& outputType, OutputStream& out) {
  ...
  if (inputType.isBoolean()) {
    ...
    SkDEBUGFAILF("unsupported type for unsigned int typecast: %s", inputType.description().c_str());
    return NA;
  }
  return result;
}
...
SpvId SPIRVCodeGenerator::castScalarToBoolean(
    SpvId inputId, const Type& inputType, const Type& outputType, OutputStream& out) {
  ...
  if (inputType.isSigned()) {
    ...
    const SpvId zeroID = this->writeLiteral(0.0, *fContext.fTypes.fInt);
    this->writeInstruction(SpvOpINotEqual, this->getType(outputType), result, inputId, zeroID, out);
  } else if (inputType.isUnsigned()) {
    ...
    const SpvId zeroID = this->writeLiteral(0.0, *fContext.fTypes.fFloat);
    this->writeInstruction(
        SpvOpFUnordNotEqual, this->getType(outputType), result, inputId, zeroID, out);
  } else {
...
SpvId SPIRVCodeGenerator::writeMatrixCopy(
    SpvId src, const Type& srcType, const Type& dstType, OutputStream& out) {
...
void SPIRVCodeGenerator::addColumnEntry(
    const Type& columnType, SkTArray<SpvId>* currentColumn, SkTArray<SpvId>* columnIds, int rows,
...
SpvId SPIRVCodeGenerator::writeMatrixConstructor(const ConstructorCompound& c, OutputStream& out) {
  ...
  int rows = type.rows();
  const Type& columnType = type.componentType().toCompound(
      fContext,
      /*columns=*/rows, /*rows=*/1);
  ...
  for (size_t i = 0; i < arguments.size(); i++) {
    ...
    if (currentColumn.empty() && argType.isVector() && argType.columns() == rows) {
      ...
      for (int j = 0; j < argType.columns(); ++j) {
        SpvId swizzle =
            this->writeOpCompositeExtract(argType.componentType(), arguments[i], j, out);
...
SpvId SPIRVCodeGenerator::writeConstructorCompound(
...
SpvId SPIRVCodeGenerator::writeVectorConstructor(const ConstructorCompound& c, OutputStream& out) {
  ...
  for (size_t i = 0; i < c.arguments().size(); i++) {
    ...
    if (argType.isMatrix()) {
      ...
      for (int j = 0; j < 4; ++j) {
        arguments.push_back(this->writeOpCompositeExtract(componentType, arg, j / 2, j % 2, out));
      }
...
SpvId SPIRVCodeGenerator::writeConstructorScalarCast(
...
SpvId SPIRVCodeGenerator::writeConstructorCompoundCast(
...
SpvId SPIRVCodeGenerator::writeConstructorDiagonalMatrix(
...
SpvId SPIRVCodeGenerator::writeConstructorMatrixResize(
...
static SpvStorageClass_ get_storage_class(
...
static SpvStorageClass_ get_storage_class(const Expression& expr) {
  switch (expr.kind()) {
    ...
    case Expression::Kind::kFieldAccess: return get_storage_class(*expr.as<FieldAccess>().base());
    case Expression::Kind::kIndex: return get_storage_class(*expr.as<IndexExpression>().base());
...
class PointerLValue : public SPIRVCodeGenerator::LValue {
 public:
  PointerLValue(
      SPIRVCodeGenerator& gen, SpvId pointer, bool isMemoryObject, SpvId type,
  ...
  bool isMemoryObjectPointer() const override { return fIsMemoryObject; }
...
class SwizzleLValue : public SPIRVCodeGenerator::LValue {
 public:
  SwizzleLValue(
      SPIRVCodeGenerator& gen, SpvId vecPointer, const ComponentArray& components,
...
std::unique_ptr<SPIRVCodeGenerator::LValue> SPIRVCodeGenerator::getLValue(
  ...
  switch (expr.kind()) {
    case Expression::Kind::kVariableReference: {
      ...
      if (uniformIdx >= 0) {
        ...
        this->writeInstruction(
            SpvOpAccessChain, typeId, memberId, fUniformBufferId, uniformIdxId, out);
        return std::make_unique<PointerLValue>(
            *this, memberId,
            /*isMemoryObjectPointer=*/true, this->getType(type), precision, SpvStorageClassUniform);
      }
      ...
      return std::make_unique<PointerLValue>(
          *this, *entry,
          /*isMemoryObjectPointer=*/true, typeId, precision, get_storage_class(expr));
    }
    ...
    case Expression::Kind::kFieldAccess: {
      ...
      return std::make_unique<PointerLValue>(
          *this, member, /*isMemoryObjectPointer=*/false, this->getType(type), precision,
          storageClass);
    }
    case Expression::Kind::kSwizzle: {
      ...
      if (base == NA) {
        fContext.fErrors->error(swizzle.fPosition, "unable to retrieve lvalue from swizzle");
      }
      ...
      if (swizzle.components().size() == 1) {
        ...
        return std::make_unique<PointerLValue>(
            *this, member,
            /*isMemoryObjectPointer=*/false, this->getType(type), precision, storageClass);
      } else {
        return std::make_unique<SwizzleLValue>(
            *this, base, swizzle.components(), swizzle.base()->type(), type, storageClass);
      }
    }
    default: {
      ...
      this->writeInstruction(
          SpvOpVariable, pointerType, result, SpvStorageClassFunction, fVariableBuffer);
      this->writeOpStore(SpvStorageClassFunction, result, this->writeExpression(expr, out), out);
      return std::make_unique<PointerLValue>(
          *this, result, /*isMemoryObjectPointer=*/true, this->getType(type), precision,
...
SpvId SPIRVCodeGenerator::writeVariableReference(const VariableReference& ref, OutputStream& out) {
  ...
  switch (variable->modifiers().fLayout.fBuiltin) {
    ...
    case SK_SECONDARYFRAGCOLOR_BUILTIN: {
      ...
      fContext.fErrors->error(ref.fPosition, "sk_SecondaryFragColor is not allowed in SPIR-V");
      return NA;
    }
    case SK_FRAGCOORD_BUILTIN: {
      ...
      if (!symbols[DEVICE_COORDS_NAME]) {
        ...
        auto coordsVar = std::make_unique<Variable>(
            /*pos=*/Position(),
            /*modifiersPosition=*/Position(), fContext.fModifiersPool->add(modifiers),
            DEVICE_COORDS_NAME, fContext.fTypes.fFloat4.get(),
            /*builtin=*/true, Variable::Storage::kGlobal);
      ...
      return this->writeExpression(
          *dsl::Float4(
               deviceCoord.x(), std::move(x) + std::move(y) * deviceCoord.y(), deviceCoord.z(),
               deviceCoord.w())
               .release(),
          out);
    }
    case SK_CLOCKWISE_BUILTIN: {
      ...
      if (!symbols[DEVICE_CLOCKWISE_NAME]) {
        ...
        auto clockwiseVar = std::make_unique<Variable>(
            /*pos=*/Position(),
            /*modifiersPosition=*/Position(), fContext.fModifiersPool->add(modifiers),
            DEVICE_CLOCKWISE_NAME, fContext.fTypes.fBool.get(),
            /*builtin=*/true, Variable::Storage::kGlobal);
      ...
      return this->writeExpression(
          *dsl::Bool(Select(rtFlip.y() > 0, !deviceClockwise, deviceClockwise)).release(), out);
    }
    default: return this->getLValue(ref, out)->load(out);
  }
}
...
SpvId SPIRVCodeGenerator::writeIndexExpression(const IndexExpression& expr, OutputStream& out) {
  if (expr.base()->type().isVector()) {
    ...
    SpvId result = this->nextId(nullptr);
    this->writeInstruction(
        SpvOpVectorExtractDynamic, this->getType(expr.type()), result, base, index, out);
    return result;
  }
...
SpvId SPIRVCodeGenerator::writeBinaryOperation(
    const Type& resultType, const Type& operandType, SpvId lhs, SpvId rhs, SpvOp_ ifFloat,
...
SpvId SPIRVCodeGenerator::foldToBool(
    SpvId id, const Type& operandType, SpvOp op, OutputStream& out) {
...
SpvId SPIRVCodeGenerator::writeMatrixComparison(
    const Type& operandType, SpvId lhs, SpvId rhs, SpvOp_ floatOperator, SpvOp_ intOperator,
    SpvOp_ vectorMergeOperator, SpvOp_ mergeOperator, OutputStream& out) {
  SpvOp_ compareOp = is_float(operandType) ? floatOperator : intOperator;
  SkASSERT(operandType.isMatrix());
  const Type& columnType = operandType.componentType().toCompound(fContext, operandType.rows(), 1);
  SpvId bvecType =
      this->getType(fContext.fTypes.fBool->toCompound(fContext, operandType.rows(), 1));
...
SpvId SPIRVCodeGenerator::writeComponentwiseMatrixUnary(
    const Type& operandType, SpvId operand, SpvOp_ op, OutputStream& out) {
  SkASSERT(operandType.isMatrix());
  const Type& columnType = operandType.componentType().toCompound(
      fContext,
...
SpvId SPIRVCodeGenerator::writeComponentwiseMatrixBinary(
    const Type& operandType, SpvId lhs, SpvId rhs, SpvOp_ op, OutputStream& out) {
  SkASSERT(operandType.isMatrix());
  const Type& columnType = operandType.componentType().toCompound(
      fContext,
      /*columns=*/operandType.rows(),
...
SpvId SPIRVCodeGenerator::writeScalarToMatrixSplat(
    const Type& matrixType, SpvId scalarId, OutputStream& out) {
  // Splat the scalar into a vector.
  const Type& vectorType = matrixType.componentType().toCompound(
      fContext,
...
static bool types_match(const Type& a, const Type& b) {
  ...
  return (a.typeKind() == b.typeKind()) && (a.isScalar() || a.isVector() || a.isMatrix()) &&
         (a.columns() == b.columns() && a.rows() == b.rows()) &&
         a.componentType().numberKind() == b.componentType().numberKind();
}

SpvId SPIRVCodeGenerator::writeBinaryExpression(
    const Type& leftType, SpvId lhs, Operator op, const Type& rightType, SpvId rhs,
    ...
    if (leftType.isVector() && rightType.isNumber()) {
      if (resultType.componentType().isFloat()) {
        switch (op.kind()) {
          ...
          case Operator::Kind::STAR: {
            SpvId result = this->nextId(&resultType);
            this->writeInstruction(
                SpvOpVectorTimesScalar, this->getType(resultType), result, lhs, rhs, out);
            return result;
          }
          default: break;
        }
      ...
      if (resultType.componentType().isFloat()) {
        if (op.kind() == Operator::Kind::STAR) {
          SpvId result = this->nextId(&resultType);
          this->writeInstruction(
              SpvOpVectorTimesScalar, this->getType(resultType), result, rhs, lhs, out);
          return result;
        }
      }
      ...
      if (op.kind() == Operator::Kind::STAR) {
        ...
        return this->writeBinaryExpression(leftType, lhs, op, leftType, rhsMatrix, resultType, out);
      }
      ...
      if (op.kind() == Operator::Kind::STAR) {
        ...
        if (leftType.isVector()) {
          this->writeInstruction(
              SpvOpVectorTimesMatrix, this->getType(resultType), result, lhs, rhs, out);
        } else {
          SkASSERT(leftType.isScalar());
          this->writeInstruction(
              SpvOpMatrixTimesScalar, this->getType(resultType), result, rhs, lhs, out);
        }
        return result;
      } else {
        ...
        return this->writeBinaryExpression(
            rightType, lhsMatrix, op, rightType, rhs, resultType, out);
      }
    } else {
  ...
  switch (op.kind()) {
    case Operator::Kind::EQEQ: {
      if (operandType->isMatrix()) {
        return this->writeMatrixComparison(
            *operandType, lhs, rhs, SpvOpFOrdEqual, SpvOpIEqual, SpvOpAll, SpvOpLogicalAnd, out);
      }
      ...
      const Type* tmpType;
      if (operandType->isVector()) {
        tmpType = &fContext.fTypes.fBool->toCompound(
            fContext, operandType->columns(), operandType->rows());
      } else {
        tmpType = &resultType;
      }
      if (lhs == rhs) {
      ...
      return this->foldToBool(
          this->writeBinaryOperation(
              *tmpType, *operandType, lhs, rhs, SpvOpFOrdEqual, SpvOpIEqual, SpvOpIEqual,
              SpvOpLogicalEqual, out),
          *operandType, SpvOpAll, out);
    }
    case Operator::Kind::NEQ:
      if (operandType->isMatrix()) {
        return this->writeMatrixComparison(
            *operandType, lhs, rhs, SpvOpFUnordNotEqual, SpvOpINotEqual, SpvOpAny, SpvOpLogicalOr,
            out);
      }
      if (operandType->isStruct()) {
        return this->writeStructComparison(*operandType, lhs, op, rhs, out);
    ...
    case Operator::Kind::LOGICALXOR:
      ...
      return this->foldToBool(
          this->writeBinaryOperation(
              *tmpType, *operandType, lhs, rhs, SpvOpFUnordNotEqual, SpvOpINotEqual, SpvOpINotEqual,
              SpvOpLogicalNotEqual, out),
          *operandType, SpvOpAny, out);
    case Operator::Kind::GT:
      SkASSERT(resultType.isBoolean());
      return this->writeBinaryOperation(
          resultType, *operandType, lhs, rhs, SpvOpFOrdGreaterThan, SpvOpSGreaterThan,
          SpvOpUGreaterThan, SpvOpUndef, out);
    case Operator::Kind::LT:
      SkASSERT(resultType.isBoolean());
      return this->writeBinaryOperation(
          resultType, *operandType, lhs, rhs, SpvOpFOrdLessThan, SpvOpSLessThan, SpvOpULessThan,
          SpvOpUndef, out);
    case Operator::Kind::GTEQ:
      SkASSERT(resultType.isBoolean());
      return this->writeBinaryOperation(
          resultType, *operandType, lhs, rhs, SpvOpFOrdGreaterThanEqual, SpvOpSGreaterThanEqual,
          SpvOpUGreaterThanEqual, SpvOpUndef, out);
    case Operator::Kind::LTEQ:
      SkASSERT(resultType.isBoolean());
      return this->writeBinaryOperation(
          resultType, *operandType, lhs, rhs, SpvOpFOrdLessThanEqual, SpvOpSLessThanEqual,
    ...
    case Operator::Kind::PLUS:
      ...
      return this->writeBinaryOperation(
          resultType, *operandType, lhs, rhs, SpvOpFAdd, SpvOpIAdd, SpvOpIAdd, SpvOpUndef, out);
    case Operator::Kind::MINUS:
      ...
      return this->writeBinaryOperation(
          resultType, *operandType, lhs, rhs, SpvOpFSub, SpvOpISub, SpvOpISub, SpvOpUndef, out);
    case Operator::Kind::STAR:
      if (leftType.isMatrix() && rightType.isMatrix()) {
        // matrix multiply
        SpvId result = this->nextId(&resultType);
        this->writeInstruction(
            SpvOpMatrixTimesMatrix, this->getType(resultType), result, lhs, rhs, out);
        return result;
      }
      return this->writeBinaryOperation(
          resultType, *operandType, lhs, rhs, SpvOpFMul, SpvOpIMul, SpvOpIMul, SpvOpUndef, out);
    case Operator::Kind::SLASH:
      if (leftType.isMatrix() && rightType.isMatrix()) {
        SkASSERT(leftType.matches(rightType));
      ...
      return this->writeBinaryOperation(
          resultType, *operandType, lhs, rhs, SpvOpFDiv, SpvOpSDiv, SpvOpUDiv, SpvOpUndef, out);
    case Operator::Kind::PERCENT:
      return this->writeBinaryOperation(
          resultType, *operandType, lhs, rhs, SpvOpFMod, SpvOpSMod, SpvOpUMod, SpvOpUndef, out);
    case Operator::Kind::SHL:
      return this->writeBinaryOperation(
          resultType, *operandType, lhs, rhs, SpvOpUndef, SpvOpShiftLeftLogical,
          SpvOpShiftLeftLogical, SpvOpUndef, out);
    case Operator::Kind::SHR:
      return this->writeBinaryOperation(
          resultType, *operandType, lhs, rhs, SpvOpUndef, SpvOpShiftRightArithmetic,
          SpvOpShiftRightLogical, SpvOpUndef, out);
    case Operator::Kind::BITWISEAND:
      return this->writeBinaryOperation(
          resultType, *operandType, lhs, rhs, SpvOpUndef, SpvOpBitwiseAnd, SpvOpBitwiseAnd,
          SpvOpUndef, out);
    ...
    case Operator::Kind::BITWISEXOR:
      return this->writeBinaryOperation(
          resultType, *operandType, lhs, rhs, SpvOpUndef, SpvOpBitwiseXor, SpvOpBitwiseXor,
          SpvOpUndef, out);
    default: fContext.fErrors->error(Position(), "unsupported token"); return NA;
  }
}

SpvId SPIRVCodeGenerator::writeArrayComparison(
    const Type& arrayType, SpvId lhs, Operator op, SpvId rhs, OutputStream& out) {
  ...
  for (int index = 0; index < arraySize; ++index) {
    ...
    SpvId comparison =
        this->writeBinaryExpression(componentType, itemL, op, componentType, itemR, boolType, out);
...
SpvId SPIRVCodeGenerator::writeStructComparison(
    const Type& structType, SpvId lhs, Operator op, SpvId rhs, OutputStream& out) {
  ...
  for (int index = 0; index < (int)fields.size(); ++index) {
    ...
    SpvId comparison =
        this->writeBinaryExpression(fieldType, fieldL, op, fieldType, fieldR, boolType, out);
    // Merge this comparison result with all the other comparisons we've done.
    allComparisons = this->mergeComparisons(comparison, allComparisons, op, out);
  }
  return allComparisons;
...
SpvId SPIRVCodeGenerator::mergeComparisons(
    SpvId comparison, SpvId allComparisons, Operator op, OutputStream& out) {
  ...
  switch (op.kind()) {
    case Operator::Kind::EQEQ:
      this->writeInstruction(
          SpvOpLogicalAnd, boolTypeId, logicalOp, comparison, allComparisons, out);
      break;
    case Operator::Kind::NEQ:
      this->writeInstruction(
          SpvOpLogicalOr, boolTypeId, logicalOp, comparison, allComparisons, out);
      break;
    default:
...
SpvId SPIRVCodeGenerator::writeBinaryExpression(const BinaryExpression& b, OutputStream& out) {
  ...
  SpvId result = this->writeBinaryExpression(
      left->type(), lhs, op.removeAssignment(), right->type(), rhs, b.type(), out);
  if (lvalue) {
...
SpvId SPIRVCodeGenerator::writeLogicalAnd(
    const Expression& left, const Expression& right, OutputStream& out) {
  ...
  SpvId result = this->nextId(nullptr);
  this->writeInstruction(
      SpvOpPhi, this->getType(*fContext.fTypes.fBool), result, falseConstant, lhsBlock, rhs,
      rhsBlock, out);

  return result;
}

SpvId SPIRVCodeGenerator::writeLogicalOr(
    const Expression& left, const Expression& right, OutputStream& out) {
  ...
  SpvId result = this->nextId(nullptr);
  this->writeInstruction(
      SpvOpPhi, this->getType(*fContext.fTypes.fBool), result, trueConstant, lhsBlock, rhs,
      rhsBlock, out);
...
SpvId SPIRVCodeGenerator::writeTernaryExpression(const TernaryExpression& t, OutputStream& out) {
  ...
  if (t.ifTrue()->type().columns() == 1 && t.ifTrue()->isCompileTimeConstant() &&
    ...
    this->writeInstruction(SpvOpSelect, this->getType(type), result, test, trueId, falseId, out);
    return result;
  }
  ...
  this->writeInstruction(
      SpvOpVariable, this->getPointerType(type, SpvStorageClassFunction), var,
      SpvStorageClassFunction, fVariableBuffer);
...
SpvId SPIRVCodeGenerator::writePrefixExpression(const PrefixExpression& p, OutputStream& out) {
  ...
  switch (p.getOperator().kind()) {
    case Operator::Kind::PLUS: return this->writeExpression(*p.operand(), out);
    case Operator::Kind::PLUSPLUS: {
      std::unique_ptr<LValue> lv = this->getLValue(*p.operand(), out);
      SpvId one = this->writeLiteral(1.0, type);
      SpvId result = this->writeBinaryOperation(
          type, type, lv->load(out), one, SpvOpFAdd, SpvOpIAdd, SpvOpIAdd, SpvOpUndef, out);
      lv->store(result, out);
      return result;
    }
    case Operator::Kind::MINUSMINUS: {
      ...
      SpvId one = this->writeLiteral(1.0, type);
      SpvId result = this->writeBinaryOperation(
          type, type, lv->load(out), one, SpvOpFSub, SpvOpISub, SpvOpISub, SpvOpUndef, out);
      lv->store(result, out);
      return result;
    }
    case Operator::Kind::LOGICALNOT: {
      ...
      SpvId result = this->nextId(nullptr);
      this->writeInstruction(
          SpvOpLogicalNot, this->getType(type), result, this->writeExpression(*p.operand(), out),
          out);
      return result;
    }
    case Operator::Kind::BITWISENOT: {
      SpvId result = this->nextId(nullptr);
      this->writeInstruction(
          SpvOpNot, this->getType(type), result, this->writeExpression(*p.operand(), out), out);
      return result;
    }
    default: SkDEBUGFAILF("unsupported prefix expression: %s", p.description().c_str()); return NA;
  }
}
  ...
  switch (p.getOperator().kind()) {
    case Operator::Kind::PLUSPLUS: {
      SpvId temp = this->writeBinaryOperation(
          type, type, result, one, SpvOpFAdd, SpvOpIAdd, SpvOpIAdd, SpvOpUndef, out);
      lv->store(temp, out);
      return result;
    }
    case Operator::Kind::MINUSMINUS: {
      SpvId temp = this->writeBinaryOperation(
          type, type, result, one, SpvOpFSub, SpvOpISub, SpvOpISub, SpvOpUndef, out);
      lv->store(temp, out);
      return result;
    }
    default: SkDEBUGFAILF("unsupported postfix expression %s", p.description().c_str()); return NA;
  }
}
...
SpvId SPIRVCodeGenerator::writeLiteral(double value, const Type& type) {
  switch (type.numberKind()) {
    ...
    case Type::NumberKind::kBoolean: {
      return value ? this->writeOpConstantTrue(type) : this->writeOpConstantFalse(type);
    }
    default: {
...
SpvId SPIRVCodeGenerator::writeFunctionStart(const FunctionDeclaration& f, OutputStream& out) {
  ...
  this->writeInstruction(
      SpvOpFunction, returnTypeId, result, SpvFunctionControlMaskNone, functionTypeId, out);
  std::string mangledName = f.mangledName();
  this->writeInstruction(
      SpvOpName, result, std::string_view(mangledName.c_str(), mangledName.size()), fNameBuffer);
...
void SPIRVCodeGenerator::writeLayout(const Layout& layout, SpvId target, Position pos) {
  ...
  if (layout.fLocation >= 0) {
    this->writeInstruction(
        SpvOpDecorate, target, SpvDecorationLocation, layout.fLocation, fDecorationBuffer);
  }
  if (layout.fBinding >= 0) {
    if (isPushConstant) {
      ...
      this->writeInstruction(
          SpvOpDecorate, target, SpvDecorationBinding, layout.fBinding, fDecorationBuffer);
    }
  }
  if (layout.fIndex >= 0) {
    this->writeInstruction(
        SpvOpDecorate, target, SpvDecorationIndex, layout.fIndex, fDecorationBuffer);
  }
  if (layout.fSet >= 0) {
    if (isPushConstant) {
      ...
      this->writeInstruction(
          SpvOpDecorate, target, SpvDecorationDescriptorSet, layout.fSet, fDecorationBuffer);
    }
  }
  if (layout.fInputAttachmentIndex >= 0) {
    this->writeInstruction(
        SpvOpDecorate, target, SpvDecorationInputAttachmentIndex, layout.fInputAttachmentIndex,
        fDecorationBuffer);
    fCapabilities |= (((uint64_t)1) << SpvCapabilityInputAttachment);
  }
  if (layout.fBuiltin >= 0 && layout.fBuiltin != SK_FRAGCOLOR_BUILTIN) {
    this->writeInstruction(
        SpvOpDecorate, target, SpvDecorationBuiltIn, layout.fBuiltin, fDecorationBuffer);
  }
}
...
void SPIRVCodeGenerator::writeFieldLayout(const Layout& layout, SpvId target, int member) {
  ...
  if (layout.fLocation >= 0) {
    this->writeInstruction(
        SpvOpMemberDecorate, target, member, SpvDecorationLocation, layout.fLocation,
        fDecorationBuffer);
  }
  if (layout.fIndex >= 0) {
    this->writeInstruction(
        SpvOpMemberDecorate, target, member, SpvDecorationIndex, layout.fIndex, fDecorationBuffer);
  }
  if (layout.fInputAttachmentIndex >= 0) {
    this->writeInstruction(
        SpvOpDecorate, target, member, SpvDecorationInputAttachmentIndex,
        layout.fInputAttachmentIndex, fDecorationBuffer);
  }
  if (layout.fBuiltin >= 0) {
    this->writeInstruction(
        SpvOpMemberDecorate, target, member, SpvDecorationBuiltIn, layout.fBuiltin,
        fDecorationBuffer);
  }
}
  ...
  if (fProgram.fInputs.fUseFlipRTUniform && appendRTFlip && type.isStruct()) {
    ...
    fields.emplace_back(
        ...
        Modifiers(
            Layout(
                /*flags=*/0,
                /*location=*/-1, fProgram.fConfig->fSettings.fRTFlipOffset,
                /*binding=*/-1,
                /*index=*/-1,
                /*set=*/-1,
                /*builtin=*/-1,
            ...
            /*flags=*/0),
        SKSL_RTFLIP_NAME, fContext.fTypes.fFloat2.get());
    {
      AutoAttachPoolToThread attach(fProgram.fPool.get());
      const Type* rtFlipStructType = fProgram.fSymbols->takeOwnershipOfSymbol(Type::MakeStructType(
          type.fPosition, type.name(), std::move(fields),
          /*interfaceBlock=*/true));
      const Variable* modifiedVar =
          fProgram.fSymbols->takeOwnershipOfSymbol(std::make_unique<Variable>(
              intfVar.fPosition, intfVar.modifiersPosition(), &intfVar.modifiers(), intfVar.name(),
              rtFlipStructType, intfVar.isBuiltin(), intfVar.storage()));
      fSPIRVBonusVariables.add(modifiedVar);
      InterfaceBlock modifiedCopy(
          intf.fPosition, *modifiedVar, intf.typeName(), intf.instanceName(), intf.arraySize(),
          intf.typeOwner());
      result = this->writeInterfaceBlock(modifiedCopy, false);
      fProgram.fSymbols->add(
          std::make_unique<Field>(Position(), modifiedVar, rtFlipStructType->fields().size() - 1));
    }
...
void SPIRVCodeGenerator::writeGlobalVar(ProgramKind kind, const VarDeclaration& varDecl) {
  ...
  if (var.modifiers().fFlags & Modifiers::kNoPerspective_Flag) {
    this->writeInstruction(SpvOpDecorate, id, SpvDecorationNoPerspective, fDecorationBuffer);
  }
}
...
void SPIRVCodeGenerator::writeStatement(const Statement& s, OutputStream& out) {
  switch (s.kind()) {
    case Statement::Kind::kNop: break;
    case Statement::Kind::kBlock: this->writeBlock(s.as<Block>(), out); break;
    case Statement::Kind::kExpression:
      this->writeExpression(*s.as<ExpressionStatement>().expression(), out);
      break;
    case Statement::Kind::kReturn: this->writeReturnStatement(s.as<ReturnStatement>(), out); break;
    case Statement::Kind::kVarDeclaration:
      this->writeVarDeclaration(s.as<VarDeclaration>(), out);
      break;
    case Statement::Kind::kIf: this->writeIfStatement(s.as<IfStatement>(), out); break;
    case Statement::Kind::kFor: this->writeForStatement(s.as<ForStatement>(), out); break;
    case Statement::Kind::kDo: this->writeDoStatement(s.as<DoStatement>(), out); break;
    case Statement::Kind::kSwitch: this->writeSwitchStatement(s.as<SwitchStatement>(), out); break;
    ...
    case Statement::Kind::kContinue:
      ...
      break;
    case Statement::Kind::kDiscard: this->writeInstruction(SpvOpKill, out); break;
    default: SkDEBUGFAILF("unsupported statement: %s", s.description().c_str()); break;
  }
}
...
void SPIRVCodeGenerator::writeReturnStatement(const ReturnStatement& r, OutputStream& out) {
  if (r.expression()) {
    this->writeInstruction(SpvOpReturnValue, this->writeExpression(*r.expression(), out), out);
  } else {
    this->writeInstruction(SpvOpReturn, out);
  }
}
...
SPIRVCodeGenerator::EntrypointAdapter SPIRVCodeGenerator::writeEntrypointAdapter(
  ...
  auto skFragColorRef = std::make_unique<VariableReference>(
      Position(), &skFragColorVar, VariableReference::RefKind::kWrite);
  // Synthesize a call to the `main()` function.
  if (!main.returnType().matches(skFragColorRef->type())) {
    fContext.fErrors->error(
        main.fPosition,
        "SPIR-V does not support returning '" + main.returnType().description() + "' from main()");
    return {};
  }
  ExpressionArray args;
  if (main.parameters().size() == 1) {
    if (!main.parameters()[0]->type().matches(*fContext.fTypes.fFloat2)) {
      fContext.fErrors->error(
          main.fPosition, "SPIR-V does not support parameter of type '" +
  ...
  auto callMainFn =
      std::make_unique<FunctionCall>(Position(), &main.returnType(), &main, std::move(args));
  ...
  auto assignmentStmt = std::make_unique<ExpressionStatement>(std::make_unique<BinaryExpression>(
      Position(), std::move(skFragColorRef), Operator::Kind::EQ, std::move(callMainFn),
      &main.returnType()));
  ...
  auto entrypointBlock =
      Block::Make(Position(), std::move(entrypointStmts), Block::Kind::kBracedScope, symbolTable);
  ...
  adapter.entrypointDecl = std::make_unique<FunctionDeclaration>(
      Position(), &adapter.fModifiers, "_entrypoint",
      /*parameters=*/std::vector<const Variable*>{},
      /*returnType=*/fContext.fTypes.fVoid.get(),
      /*builtin=*/false);
  // Define it.
  adapter.entrypointDef = FunctionDefinition::Convert(
      fContext, Position(), *adapter.entrypointDecl, std::move(entrypointBlock),
      /*builtin=*/false);
...
void SPIRVCodeGenerator::writeUniformBuffer(std::shared_ptr<SymbolTable> topLevelSymbolTable) {
  ...
  fUniformBuffer.fStruct = Type::MakeStructType(
      Position(), kUniformBufferName, std::move(fields),
      /*interfaceBlock=*/true);
  ...
  fUniformBuffer.fInnerVariable = std::make_unique<Variable>(
      /*pos=*/Position(), /*modifiersPosition=*/Position(), fProgram.fModifiers->add(modifiers),
      kUniformBufferName, fUniformBuffer.fStruct.get(),
  ...
  fUniformBuffer.fInterfaceBlock = std::make_unique<InterfaceBlock>(
      Position(), *fUniformBuffer.fInnerVariable, kUniformBufferName, kUniformBufferName,
      /*arraySize=*/0, topLevelSymbolTable);
...
void SPIRVCodeGenerator::addRTFlipUniform(Position pos) {
  ...
  const Variable* intfVar = fSynthetics.takeOwnershipOfSymbol(std::make_unique<Variable>(
      /*pos=*/Position(),
      /*modifiersPosition=*/Position(), modsPtr, name, intfStruct,
      /*builtin=*/false, Variable::Storage::kGlobal));
  ...
  InterfaceBlock intf(
      Position(), *intfVar, name,
      /*instanceName=*/"",
      /*arraySize=*/0, std::make_shared<SymbolTable>(fContext, /*builtin=*/false));
...
void SPIRVCodeGenerator::writeInstructions(const Program& program, OutputStream& out) {
  ...
  this->writeOpCode(
      SpvOpEntryPoint, (SpvId)(3 + (main->name().length() + 4) / 4) + (int32_t)interfaceVars.size(),
      out);
  ...
  if (ProgramConfig::IsFragment(program.fConfig->fKind)) {
    this->writeInstruction(
        SpvOpExecutionMode, fFunctionMap[main], SpvExecutionModeOriginUpperLeft, out);
  }
  for (const ProgramElement* e : program.elements()) {
    if (e->is<Extension>()) {
