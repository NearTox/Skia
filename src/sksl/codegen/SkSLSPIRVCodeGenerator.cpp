...
#define ALL_GLSL(x)                                                                   \
  std::make_tuple(                                                                    \
      kGLSL_STD_450_IntrinsicOpcodeKind, GLSLstd450##x, GLSLstd450##x, GLSLstd450##x, \
      GLSLstd450##x)
#define BY_TYPE_GLSL(ifFloat, ifInt, ifUInt)                                     \
  std::make_tuple(                                                               \
      kGLSL_STD_450_IntrinsicOpcodeKind, GLSLstd450##ifFloat, GLSLstd450##ifInt, \
      GLSLstd450##ifUInt, SpvOpUndef)
#define ALL_SPIRV(x) \
  std::make_tuple(kSPIRV_IntrinsicOpcodeKind, SpvOp##x, SpvOp##x, SpvOp##x, SpvOp##x)
#define SPECIAL(x)                                                                    \
  std::make_tuple(                                                                    \
  ...
  fIntrinsicMap[k_dot_IntrinsicKind] =
      std::make_tuple(kSPIRV_IntrinsicOpcodeKind, SpvOpDot, SpvOpUndef, SpvOpUndef, SpvOpUndef);
...
#define PACK(type)                                                    \
  fIntrinsicMap[k_pack##type##_IntrinsicKind] = ALL_GLSL(Pack##type); \
...
#undef PACK
  ...
  fIntrinsicMap[k_dFdx_IntrinsicKind] =
      std::make_tuple(kSPIRV_IntrinsicOpcodeKind, SpvOpDPdx, SpvOpUndef, SpvOpUndef, SpvOpUndef);
  fIntrinsicMap[k_dFdy_IntrinsicKind] = SPECIAL(DFdy);
  fIntrinsicMap[k_fwidth_IntrinsicKind] =
      std::make_tuple(kSPIRV_IntrinsicOpcodeKind, SpvOpFwidth, SpvOpUndef, SpvOpUndef, SpvOpUndef);
  ...
  fIntrinsicMap[k_any_IntrinsicKind] =
      std::make_tuple(kSPIRV_IntrinsicOpcodeKind, SpvOpUndef, SpvOpUndef, SpvOpUndef, SpvOpAny);
  fIntrinsicMap[k_all_IntrinsicKind] =
      std::make_tuple(kSPIRV_IntrinsicOpcodeKind, SpvOpUndef, SpvOpUndef, SpvOpUndef, SpvOpAll);
  fIntrinsicMap[k_not_IntrinsicKind] = std::make_tuple(
      kSPIRV_IntrinsicOpcodeKind, SpvOpUndef, SpvOpUndef, SpvOpUndef, SpvOpLogicalNot);
  fIntrinsicMap[k_equal_IntrinsicKind] = std::make_tuple(
      kSPIRV_IntrinsicOpcodeKind, SpvOpFOrdEqual, SpvOpIEqual, SpvOpIEqual, SpvOpLogicalEqual);
  fIntrinsicMap[k_notEqual_IntrinsicKind] = std::make_tuple(
      kSPIRV_IntrinsicOpcodeKind, SpvOpFOrdNotEqual, SpvOpINotEqual, SpvOpINotEqual,
      SpvOpLogicalNotEqual);
  fIntrinsicMap[k_lessThan_IntrinsicKind] = std::make_tuple(
      kSPIRV_IntrinsicOpcodeKind, SpvOpFOrdLessThan, SpvOpSLessThan, SpvOpULessThan, SpvOpUndef);
  fIntrinsicMap[k_lessThanEqual_IntrinsicKind] = std::make_tuple(
      kSPIRV_IntrinsicOpcodeKind, SpvOpFOrdLessThanEqual, SpvOpSLessThanEqual, SpvOpULessThanEqual,
      SpvOpUndef);
  fIntrinsicMap[k_greaterThan_IntrinsicKind] = std::make_tuple(
      kSPIRV_IntrinsicOpcodeKind, SpvOpFOrdGreaterThan, SpvOpSGreaterThan, SpvOpUGreaterThan,
      SpvOpUndef);
  fIntrinsicMap[k_greaterThanEqual_IntrinsicKind] = std::make_tuple(
...
void SPIRVCodeGenerator::writeInstruction(
    SpvOp_ opCode, skstd::string_view string, OutputStream& out) {
  this->writeOpCode(opCode, 1 + (string.length() + 4) / 4, out);
  this->writeString(string, out);
}

void SPIRVCodeGenerator::writeInstruction(
    SpvOp_ opCode, int32_t word1, skstd::string_view string, OutputStream& out) {
...
void SPIRVCodeGenerator::writeInstruction(
    SpvOp_ opCode, int32_t word1, int32_t word2, skstd::string_view string, OutputStream& out) {
  ...
  for (int32_t i = 0; i < (int32_t)type.fields().size(); i++) {
    ...
    if (!MemoryLayout::LayoutIsSupported(*field.fType)) {
      fContext.fErrors->error(
          type.fOffset, "type '" + field.fType->name() + "' is not permitted here");
      return;
    }
    ...
    if (fieldLayout.fOffset >= 0) {
      if (fieldLayout.fOffset < (int)offset) {
        fContext.fErrors->error(
            type.fOffset, "offset of field '" + field.fName +
                              "' must be at "
                              "least " +
                              to_string((int)offset));
      }
...
#ifdef SK_DEBUG
    SkASSERT(
        layout.fStd == MemoryLayout::Standard::k140_Standard ||
  ...
  if (entry == fTypeMap.end()) {
    ...
    switch (type->typeKind()) {
      ...
      case Type::TypeKind::kVector:
        this->writeInstruction(
            SpvOpTypeVector, result, this->getType(type->componentType(), layout), type->columns(),
            fConstantBuffer);
        break;
      case Type::TypeKind::kMatrix:
        this->writeInstruction(
            SpvOpTypeMatrix, result,
            this->getType(IndexExpression::IndexType(fContext, *type), layout), type->columns(),
            fConstantBuffer);
        break;
      case Type::TypeKind::kStruct: this->writeStruct(*type, layout, result); break;
      case Type::TypeKind::kArray: {
        if (!MemoryLayout::LayoutIsSupported(*type)) {
          fContext.fErrors->error(
              type->fOffset, "type '" + type->name() + "' is not permitted here");
          return this->nextId(nullptr);
        }
        if (type->columns() > 0) {
          SpvId typeId = this->getType(type->componentType(), layout);
          IntLiteral countLiteral(/*offset=*/-1, type->columns(), fContext.fTypes.fInt.get());
          SpvId countId = this->writeIntLiteral(countLiteral);
          this->writeInstruction(SpvOpTypeArray, result, typeId, countId, fConstantBuffer);
          this->writeInstruction(
              SpvOpDecorate, result, SpvDecorationArrayStride, (int32_t)layout.stride(*type),
              fDecorationBuffer);
        } else {
          // We shouldn't have any runtime-sized arrays right now
          fContext.fErrors->error(
              type->fOffset, "runtime-sized arrays are not supported in SPIR-V");
          this->writeInstruction(
              SpvOpTypeRuntimeArray, result, this->getType(type->componentType(), layout),
              fConstantBuffer);
          this->writeInstruction(
              SpvOpDecorate, result, SpvDecorationArrayStride, (int32_t)layout.stride(*type),
              fDecorationBuffer);
      ...
      case Type::TypeKind::kTexture: {
        this->writeInstruction(
            SpvOpTypeImage, result, this->getType(*fContext.fTypes.fFloat, layout),
            type->dimensions(), type->isDepth(), type->isArrayedTexture(), type->isMultisampled(),
            type->isSampled() ? 1 : 2, SpvImageFormatUnknown, fConstantBuffer);
  ...
  if (entry == fTypeMap.end()) {
    ...
    for (size_t i = 0; i < parameters.size(); i++) {
      ...
      parameterTypes.push_back(
          this->getPointerType(parameters[i]->type(), SpvStorageClassFunction));
    }
...
SpvId SPIRVCodeGenerator::getPointerType(
  ...
  if (entry == fTypeMap.end()) {
    SpvId result = this->nextId(nullptr);
    this->writeInstruction(
        SpvOpTypePointer, result, storageClass, this->getType(type), fConstantBuffer);
    fTypeMap[key] = result;
    return result;
  }
  return entry->second;
...
SpvId SPIRVCodeGenerator::writeExpression(const Expression& expr, OutputStream& out) {
  switch (expr.kind()) {
    ...
    case Expression::Kind::kBoolLiteral: return this->writeBoolLiteral(expr.as<BoolLiteral>());
    ...
    case Expression::Kind::kIntLiteral: return this->writeIntLiteral(expr.as<IntLiteral>());
    case Expression::Kind::kFieldAccess: return this->writeFieldAccess(expr.as<FieldAccess>(), out);
    case Expression::Kind::kFloatLiteral: return this->writeFloatLiteral(expr.as<FloatLiteral>());
    ...
    case kGLSL_STD_450_IntrinsicOpcodeKind: {
      ...
      for (size_t i = 0; i < arguments.size(); i++) {
        if (is_out(function.parameters()[i]->modifiers())) {
          argumentIds.push_back(this->writeFunctionCallArgument(
              *arguments[i], function.parameters()[i]->modifiers(), &tempVars, out));
        } else {
  ...
  switch (kind) {
    ...
    case kTexture_SpecialIntrinsic: {
      ...
      if (arguments.size() == 3) {
        ...
        if (fProgram.fConfig->fSettings.fSharpenTextures) {
          FloatLiteral lodBias(/*offset=*/-1, /*value=*/-0.5, fContext.fTypes.fFloat.get());
          this->writeInstruction(
              op, type, result, sampler, uv, SpvImageOperandsBiasMask,
              this->writeFloatLiteral(lodBias), out);
        } else {
    ...
    case kDFdy_SpecialIntrinsic: {
      ...
      DSLExpression rtFlip(
          DSLWriter::IRGenerator().convertIdentifier(/*offset=*/-1, SKSL_RTFLIP_NAME));
...
SpvId SPIRVCodeGenerator::writeFunctionCallArgument(
    const Expression& arg, const Modifiers& paramModifiers, std::vector<TempVar>* tempVars,
    OutputStream& out) {
...
SpvId SPIRVCodeGenerator::writeFunctionCall(const FunctionCall& c, OutputStream& out) {
  ...
  if (entry == fFunctionMap.end()) {
    fContext.fErrors->error(c.fOffset, "function '" + function.description() + "' is not defined");
    return -1;
  }
...
SpvId SPIRVCodeGenerator::castScalarToType(
  ...
  fContext.fErrors->error(
      -1, "unsupported cast: " + inputType.description() + " to " + outputType.description());
  return inputExprId;
}
...
void SPIRVCodeGenerator::writeUniformScaleMatrix(
    SpvId id, SpvId diagonal, const Type& type, OutputStream& out) {
  FloatLiteral zero(/*offset=*/-1, /*value=*/0, fContext.fTypes.fFloat.get());
  SpvId zeroId = this->writeFloatLiteral(zero);
  std::vector<SpvId> columnIds;
  columnIds.reserve(type.columns());
  for (int column = 0; column < type.columns(); column++) {
    this->writeOpCode(SpvOpCompositeConstruct, 3 + type.rows(), out);
    this->writeWord(
        this->getType(
            type.componentType().toCompound(fContext, /*columns=*/type.rows(), /*rows=*/1)),
        out);
...
SpvId SPIRVCodeGenerator::writeMatrixCopy(
  ...
  SpvId srcColumnType =
      this->getType(srcType.componentType().toCompound(fContext, srcType.rows(), 1));
  SpvId dstColumnType =
      this->getType(dstType.componentType().toCompound(fContext, dstType.rows(), 1));
...
void SPIRVCodeGenerator::addColumnEntry(
    const Type& columnType, std::vector<SpvId>* currentColumn, std::vector<SpvId>* columnIds,
  ...
  for (size_t i = 0; i < arguments.size(); i++) {
    ...
    if (currentColumn.empty() && argType.isVector() && argType.columns() == rows) {
      ...
      for (int j = 0; j < argType.columns(); ++j) {
        ...
        this->writeInstruction(SpvOpCompositeExtract, componentType, swizzle, arguments[i], j, out);
  ...
  for (size_t i = 0; i < c.arguments().size(); i++) {
    ...
    if (argType.isMatrix()) {
      ...
      for (int j = 0; j < 4; ++j) {
        ...
        this->writeInstruction(
            SpvOpCompositeExtract, this->getType(componentType), componentId, arg, j / 2, j % 2,
            out);
      ...
      for (int j = 0; j < argType.columns(); j++) {
        SpvId componentId = this->nextId(&componentType);
        this->writeInstruction(
            SpvOpCompositeExtract, this->getType(componentType), componentId, arg, j, out);
        arguments.push_back(componentId);
      }
    } else {
...
SpvId SPIRVCodeGenerator::writeComposite(
    const std::vector<SpvId>& arguments, const Type& type, OutputStream& out) {
...
SpvId SPIRVCodeGenerator::writeConstructorCompoundCast(
  ...
  for (int index = 0; index < argType.columns(); ++index) {
    ...
    this->writeInstruction(
        SpvOpCompositeExtract, this->getType(srcType), componentId, compositeId, index, out);
  ...
  switch (expr.kind()) {
    case Expression::Kind::kVariableReference: {
      ...
      if (uniformIdx >= 0) {
        ...
        return std::make_unique<PointerLValue>(
            *this, memberId,
            /*isMemoryObjectPointer=*/true, this->getType(type), precision);
      }
      ...
      return std::make_unique<PointerLValue>(
          *this, entry->second,
          /*isMemoryObjectPointer=*/true, typeId, precision);
    }
    ...
    case Expression::Kind::kSwizzle: {
      ...
      if (swizzle.components().size() == 1) {
        ...
        IntLiteral index(/*offset=*/-1, swizzle.components()[0], fContext.fTypes.fInt.get());
        SpvId indexId = this->writeIntLiteral(index);
        this->writeInstruction(SpvOpAccessChain, typeId, member, base, indexId, out);
        return std::make_unique<PointerLValue>(
            *this, member,
            /*isMemoryObjectPointer=*/false, this->getType(type), precision);
      } else {
        return std::make_unique<SwizzleLValue>(
            *this, base, swizzle.components(), swizzle.base()->type(), type);
      }
    }
    default: {
  ...
  if (variable->modifiers().fLayout.fBuiltin == SK_FRAGCOORD_BUILTIN) {
    ...
    if (!symbols[DEVICE_COORDS_NAME]) {
      ...
      auto coordsVar = std::make_unique<Variable>(
          /*offset=*/-1, fContext.fModifiersPool->add(modifiers), DEVICE_COORDS_NAME,
          fContext.fTypes.fFloat4.get(), true, Variable::Storage::kGlobal);
  ...
  if (variable->modifiers().fLayout.fBuiltin == SK_CLOCKWISE_BUILTIN) {
    ...
    if (!symbols[DEVICE_CLOCKWISE_NAME]) {
      ...
      auto clockwiseVar = std::make_unique<Variable>(
          /*offset=*/-1, fContext.fModifiersPool->add(modifiers), DEVICE_CLOCKWISE_NAME,
          fContext.fTypes.fBool.get(), true, Variable::Storage::kGlobal);
...
SpvId SPIRVCodeGenerator::writeSwizzle(const Swizzle& swizzle, OutputStream& out) {
  ...
  if (count == 1) {
    this->writeInstruction(
        SpvOpCompositeExtract, this->getType(swizzle.type()), result, base, swizzle.components()[0],
        out);
  } else {
...
SpvId SPIRVCodeGenerator::writeMatrixComparison(
  ...
  SpvId columnType =
      this->getType(operandType.componentType().toCompound(fContext, operandType.rows(), 1));
  SpvId bvecType =
...
SpvId SPIRVCodeGenerator::writeBinaryExpression(
  ...
  switch (op.kind()) {
    ...
    case Token::Kind::TK_NEQ:
      if (operandType->isMatrix()) {
        return this->writeMatrixComparison(
            *operandType, lhs, rhs, SpvOpFOrdNotEqual, SpvOpINotEqual, SpvOpAny, SpvOpLogicalOr,
            out);
      }
      if (operandType->isStruct()) {
        return this->writeStructComparison(*operandType, lhs, op, rhs, out);
    ...
    case Token::Kind::TK_LOGICALXOR:
      ...
      return this->foldToBool(
          this->writeBinaryOperation(
              *tmpType, *operandType, lhs, rhs, SpvOpFOrdNotEqual, SpvOpINotEqual, SpvOpINotEqual,
    ...
    case Token::Kind::TK_BITWISEXOR:
      return this->writeBinaryOperation(
          ...
          SpvOpUndef, out);
    default: fContext.fErrors->error(0, "unsupported token"); return -1;
  }
}
...
SpvId SPIRVCodeGenerator::writePrefixExpression(const PrefixExpression& p, OutputStream& out) {
  ...
  switch (p.getOperator().kind()) {
    case Token::Kind::TK_PLUS: return this->writeExpression(*p.operand(), out);
    ...
    default: SkDEBUGFAILF("unsupported prefix expression: %s", p.description().c_str()); return -1;
  }
}
...
SpvId SPIRVCodeGenerator::writePostfixExpression(const PostfixExpression& p, OutputStream& out) {
  ...
  switch (p.getOperator().kind()) {
    ...
    default: SkDEBUGFAILF("unsupported postfix expression %s", p.description().c_str()); return -1;
  }
}
...
SpvId SPIRVCodeGenerator::writeBoolLiteral(const BoolLiteral& b) {
  if (b.value()) {
    if (fBoolTrue == 0) {
      ...
      this->writeInstruction(
          SpvOpConstantTrue, this->getType(b.type()), fBoolTrue, fConstantBuffer);
    }
    return fBoolTrue;
  } else {
    if (fBoolFalse == 0) {
      ...
      this->writeInstruction(
          SpvOpConstantFalse, this->getType(b.type()), fBoolFalse, fConstantBuffer);
    }
    return fBoolFalse;
  }
}
...
SpvId SPIRVCodeGenerator::writeIntLiteral(const IntLiteral& i) {
  ...
  if (newlyCreated) {
    SpvId result = this->nextId(nullptr);
    this->writeInstruction(
        SpvOpConstant, this->getType(i.type()), result, (SpvId)i.value(), fConstantBuffer);
    iter->second = result;
  }
  return iter->second;
}
...
SpvId SPIRVCodeGenerator::writeFloatLiteral(const FloatLiteral& f) {
  ...
  if (newlyCreated) {
    SpvId result = this->nextId(nullptr);
    this->writeInstruction(
        SpvOpConstant, this->getType(f.type()), result, (SpvId)valueBits, fConstantBuffer);
    iter->second = result;
  }
  return iter->second;
}
...
SpvId SPIRVCodeGenerator::writeFunctionStart(const FunctionDeclaration& f, OutputStream& out) {
  ...
  this->writeInstruction(
      SpvOpName, result, skstd::string_view(mangledName.c_str(), mangledName.size()), fNameBuffer);
...
void SPIRVCodeGenerator::writeLayout(const Layout& layout, SpvId target, int member) {
  ...
  if (layout.fBinding >= 0) {
    this->writeInstruction(
        SpvOpMemberDecorate, target, member, SpvDecorationBinding, layout.fBinding,
        fDecorationBuffer);
  }
  if (layout.fIndex >= 0) {
    this->writeInstruction(
  ...
  if (layout.fSet >= 0) {
    this->writeInstruction(
        SpvOpMemberDecorate, target, member, SpvDecorationDescriptorSet, layout.fSet,
        fDecorationBuffer);
  }
  if (layout.fInputAttachmentIndex >= 0) {
    this->writeInstruction(
        ...
        SKSL_RTFLIP_NAME, fContext.fTypes.fFloat2.get());
    {
      ...
      const Variable* modifiedVar =
          fProgram.fSymbols->takeOwnershipOfSymbol(std::make_unique<Variable>(
              intfVar.fOffset, &intfVar.modifiers(), intfVar.name(), rtFlipStructType,
              intfVar.isBuiltin(), intfVar.storage()));
      fSPIRVBonusVariables.insert(modifiedVar);
      InterfaceBlock modifiedCopy(
          intf.fOffset, modifiedVar, intf.typeName(), intf.instanceName(), intf.arraySize(),
          intf.typeOwner());
...
void SPIRVCodeGenerator::writeGlobalVar(ProgramKind kind, const VarDeclaration& varDecl) {
  ...
  if (var.modifiers().fLayout.fBuiltin == SK_FRAGCOLOR_BUILTIN && kind != ProgramKind::kFragment) {
...
SPIRVCodeGenerator::EntrypointAdapter SPIRVCodeGenerator::writeEntrypointAdapter(
  ...
  auto skFragColorRef = std::make_unique<VariableReference>(
      /*offset=*/-1, &skFragColorVar, VariableReference::RefKind::kWrite);
  ...
  if (main.parameters().size() == 1) {
    if (main.parameters()[0]->type() != *fContext.fTypes.fFloat2) {
      fContext.fErrors->error(
          main.fOffset, "SPIR-V does not support parameter of type '" +
  ...
  auto callMainFn =
      std::make_unique<FunctionCall>(/*offset=*/-1, &main.returnType(), &main, std::move(args));
  ...
  auto assignmentStmt = std::make_unique<ExpressionStatement>(std::make_unique<BinaryExpression>(
      /*offset=*/-1, std::move(skFragColorRef), Token::Kind::TK_EQ, std::move(callMainFn),
      &main.returnType()));
  ...
  auto entrypointBlock =
      Block::Make(/*offset=*/-1, std::move(entrypointStmts), symbolTable, /*isScope=*/true);
  ...
  adapter.entrypointDecl = std::make_unique<FunctionDeclaration>(
      /*offset=*/-1, &adapter.fModifiers, "_entrypoint",
  ...
  adapter.entrypointDef = FunctionDefinition::Convert(
      fContext,
      /*offset=*/-1, *adapter.entrypointDecl, std::move(entrypointBlock),
      /*builtin=*/false);
...
void SPIRVCodeGenerator::writeUniformBuffer(std::shared_ptr<SymbolTable> topLevelSymbolTable) {
  ...
  fUniformBuffer.fStruct =
      Type::MakeStructType(/*offset=*/-1, kUniformBufferName, std::move(fields));
  ...
  fUniformBuffer.fInterfaceBlock = std::make_unique<InterfaceBlock>(
      /*offset=*/-1, fUniformBuffer.fInnerVariable.get(), kUniformBufferName, kUniformBufferName,
...
void SPIRVCodeGenerator::addRTFlipUniform(int offset) {
  ...
  const Variable* intfVar = fSynthetics.takeOwnershipOfSymbol(std::make_unique<Variable>(
      /*offset=*/-1, modsPtr, name, intfStruct,
...
void SPIRVCodeGenerator::writeInstructions(const Program& program, OutputStream& out) {
  ...
  switch (program.fConfig->fKind) {
    case ProgramKind::kVertex: this->writeWord(SpvExecutionModelVertex, out); break;
    case ProgramKind::kFragment: this->writeWord(SpvExecutionModelFragment, out); break;
    default: SK_ABORT("cannot write this kind of program to SPIR-V\n");
  }
