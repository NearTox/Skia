...
static String eval_template(
    const String& format, const std::vector<String>& tokens,
...
UniformCTypeMapper::UniformCTypeMapper(
    Layout::CType ctype, const std::vector<String>& skslTypes, const String& setUniformSingleFormat,
    const String& setUniformArrayFormat, bool enableTracking, const String& defaultValue,
    ...
    : fCType(ctype),
      ...
      fInlineValue(
          determine_inline_from_template(setUniformSingleFormat) &&
...
static UniformCTypeMapper register_array(
    Layout::CType ctype, const std::vector<String>& skslTypes, const char* singleSet,
    const char* arraySet, const char* defaultValue, const char* dirtyExpression) {
  return UniformCTypeMapper(
      ctype, skslTypes, singleSet, arraySet, defaultValue, dirtyExpression,
      "${oldVar} = ${newVar}");
}

static UniformCTypeMapper register_array(
    Layout::CType ctype, const std::vector<String>& skslTypes, const char* singleSet,
    const char* arraySet, const char* defaultValue) {
  return register_array(
      ctype, skslTypes, singleSet, arraySet, defaultValue, "${oldVar} != ${newVar}");
}

static UniformCTypeMapper register_type(
    Layout::CType ctype, const std::vector<String>& skslTypes, const char* uniformFormat,
    const char* defaultValue, const char* dirtyExpression) {
  return register_array(
      ctype, skslTypes, uniformFormat, uniformFormat, defaultValue, dirtyExpression);
}
...
static const std::vector<UniformCTypeMapper>& get_mappers() {
  static const std::vector<UniformCTypeMapper> registeredMappers = {
      ...
      register_array(
          Layout::CType::kSkIPoint, {"int2", "short2", "byte2"},
      ...
      register_type(
          Layout::CType::kSkMatrix, {"half3x3", "float3x3", "double3x3"},
      ...
      register_type(
          Layout::CType::kSkM44, {"half4x4", "float4x4", "double4x4"},
      ...
      register_array(
          Layout::CType::kInt32, {"int", "short", "byte"},
...
const UniformCTypeMapper* UniformCTypeMapper::Get(
