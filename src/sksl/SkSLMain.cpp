...
int main(int argc, const char** argv) {
  ...
  if (input.endsWith(".vert")) {
    ...
    printf(
        "input filename must end in '.vert', '.frag', '.geom', '.fp', '.stage', or "
        "'.sksl'\n");
    exit(1);
  }
  ...
  if (name.endsWith(".spirv")) {
    ...
    for (int i = symbols->fParent->fOwnedSymbols.size() - 1; i >= 0; --i) {
      symbols->fOwnedSymbols.insert(
          symbols->fOwnedSymbols.begin(), std::move(symbols->fParent->fOwnedSymbols[i]));
    }
    ...
    out.printf(
        "static constexpr size_t SKSL_INCLUDE_%s_LENGTH = %d;\n", name.c_str(), (int)data.length());
