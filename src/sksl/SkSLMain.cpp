...
static bool detect_shader_settings(
    const SkSL::String& text, SkSL::Program::Settings* settings,
...
ResultCode processCommand(std::vector<SkSL::String>& args) {
  ...
  SkSL::String text((std::istreambuf_iterator<char>(in)), std::istreambuf_iterator<char>());
  if (in.rdstate()) {
  ...
  if (outputPath.ends_with(".spirv")) {
    ...
    out.printf("};\n");
    out.printf(
        "static constexpr size_t SKSL_INCLUDE_%s_LENGTH = sizeof(SKSL_INCLUDE_%s);\n",
