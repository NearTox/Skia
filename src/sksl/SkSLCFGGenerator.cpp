...
void CFG::dump() {
  for (size_t i = 0; i < fBlocks.size(); i++) {
    ...
    for (auto iter = fBlocks[i].fBefore.begin(); iter != fBlocks[i].fBefore.end(); iter++) {
      printf(
          "%s%s = %s", separator, iter->first->description().c_str(),
    ...
    for (size_t j = 0; j < fBlocks[i].fNodes.size(); j++) {
      ...
      printf(
          "Node %d (%p): %s\n", (int)j, &n,
          n.fKind == BasicBlock::Node::kExpression_Kind ? (*n.expression())->description().c_str()
...
bool BasicBlock::tryRemoveLValueBefore(
  ...
  switch (lvalue->fKind) {
    ...
    case Expression::kVariableReference_Kind: return true;
    ...
    case Expression::kTernary_Kind:
      if (!this->tryRemoveExpressionBefore(iter, ((TernaryExpression*)lvalue)->fTest.get())) {
        return false;
      }
...
bool BasicBlock::tryRemoveExpression(std::vector<BasicBlock::Node>::iterator* iter) {
  ...
  switch (expr->fKind) {
    ...
    case Expression::kPrefix_Kind:
      if (!this->tryRemoveExpressionBefore(iter, ((PrefixExpression*)expr)->fOperand.get())) {
        return false;
      }
      *iter = fNodes.erase(*iter);
      return true;
...
void CFGGenerator::addExpression(CFG& cfg, std::unique_ptr<Expression>* e, bool constantPropagate) {
  ...
  switch ((*e)->fKind) {
      ...
      switch (b->fOperator) {
        ...
        case Token::LOGICALOR: {
          ...
          cfg.fBlocks[cfg.fCurrent].fNodes.push_back(
              {BasicBlock::Node::kExpression_Kind, constantPropagate, e, nullptr});
          break;
        }
        case Token::EQ: {
          this->addExpression(cfg, &b->fRight, constantPropagate);
    ...
    case Expression::kDefined_Kind: SkASSERT(false); break;
  }
}
...
void CFGGenerator::addLValue(CFG& cfg, std::unique_ptr<Expression>* e) {
  switch ((*e)->fKind) {
    case Expression::kFieldAccess_Kind: this->addLValue(cfg, &((FieldAccess&)**e).fBase); break;
    case Expression::kIndex_Kind:
      this->addLValue(cfg, &((IndexExpression&)**e).fBase);
      this->addExpression(cfg, &((IndexExpression&)**e).fIndex, true);
      break;
    case Expression::kSwizzle_Kind: this->addLValue(cfg, &((Swizzle&)**e).fBase); break;
    case Expression::kExternalValue_Kind:  // fall through
...
void CFGGenerator::addStatement(CFG& cfg, std::unique_ptr<Statement>* s) {
  switch ((*s)->fKind) {
    ...
    case Statement::kIf_Kind: {
      ...
      cfg.fBlocks[cfg.fCurrent].fNodes.push_back(
          {BasicBlock::Node::kStatement_Kind, false, nullptr, s});
      BlockId start = cfg.fCurrent;
      cfg.newBlock();
    ...
    case Statement::kVarDeclarations_Kind: {
      ...
      for (auto& stmt : decls.fDeclaration->fVars) {
        ...
        cfg.fBlocks[cfg.fCurrent].fNodes.push_back(
            {BasicBlock::Node::kStatement_Kind, false, nullptr, &stmt});
      }
      cfg.fBlocks[cfg.fCurrent].fNodes.push_back(
          {BasicBlock::Node::kStatement_Kind, false, nullptr, s});
      break;
