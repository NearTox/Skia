...
void BasicBlock::dump() const {
  ...
  for (const auto& [var, expr] : fBefore) {
    printf(
        "%s%s = %s", separator, var->description().c_str(),
...
bool BasicBlock::tryRemoveLValueBefore(
    std::vector<BasicBlock::Node>::iterator* iter, Expression* lvalue) {
  switch (lvalue->kind()) {
    case Expression::Kind::kVariableReference: return true;
...
bool BasicBlock::tryRemoveExpression(std::vector<BasicBlock::Node>::iterator* iter) {
  ...
  switch (expr->kind()) {
    ...
    case Expression::Kind::kPrefix:
      if (!this->tryRemoveExpressionBefore(iter, expr->as<PrefixExpression>().operand().get())) {
        return false;
      }
      *iter = fNodes.erase(*iter);
      return true;
    case Expression::Kind::kPostfix:
      if (!this->tryRemoveExpressionBefore(iter, expr->as<PostfixExpression>().operand().get())) {
        return false;
      }
      *iter = fNodes.erase(*iter);
      return true;
    ...
    case Expression::Kind::kVariableReference: *iter = fNodes.erase(*iter); return true;
    default:
#ifdef SK_DEBUG
...
bool BasicBlock::tryInsertExpression(
  ...
  switch ((*expr)->kind()) {
    case Expression::Kind::kBinary: {
      ...
      ++(*iter);
      *iter = fNodes.insert(*iter, BasicBlock::MakeExpression(expr, /*constantPropagation=*/true));
      return true;
    }
    case Expression::Kind::kBoolLiteral:   // fall through
    case Expression::Kind::kFloatLiteral:  // fall through
  ...
  switch ((*e)->kind()) {
    case Expression::Kind::kBinary: {
      ...
      switch (op) {
        ...
        default:
          this->addExpression(cfg, &b.left(), !Compiler::IsAssignment(b.getOperator()));
          this->addExpression(cfg, &b.right(), constantPropagate);
          cfg.currentBlock().fNodes.push_back(BasicBlock::MakeExpression(e, constantPropagate));
      }
      break;
    ...
    case Expression::Kind::kPrefix: {
      ...
      this->addExpression(
          cfg, &p.operand(),
          constantPropagate && p.getOperator() != Token::Kind::TK_PLUSPLUS &&
    ...
    case Expression::Kind::kDefined: SkASSERT(false); break;
  }
}
...
void CFGGenerator::addLValue(CFG& cfg, std::unique_ptr<Expression>* e) {
  switch ((*e)->kind()) {
    ...
    case Expression::Kind::kSwizzle: this->addLValue(cfg, &e->get()->as<Swizzle>().base()); break;
...
void CFGGenerator::addStatement(CFG& cfg, std::unique_ptr<Statement>* s) {
  switch ((*s)->kind()) {
    ...
    case Statement::Kind::kExpression: {
      this->addExpression(
          cfg, &(*s)->as<ExpressionStatement>().expression(),
