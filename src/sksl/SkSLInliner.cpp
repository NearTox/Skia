...
const Variable* Inliner::RemapVariable(const Variable* variable, const VariableRewriteMap* varMap) {
  ...
  if (!remap) {
    SkDEBUGFAILF(
        "rewrite map does not contain variable '%.*s'", (int)variable->name().size(),
        variable->name().data());
    return variable;
  }
  ...
  if (!expr->is<VariableReference>()) {
    SkDEBUGFAILF(
        "rewrite map contains non-variable replacement for '%.*s'", (int)variable->name().size(),
        variable->name().data());
    return variable;
  }
...
std::unique_ptr<Expression> Inliner::inlineExpression(
    Position pos, VariableRewriteMap* varMap, SymbolTable* symbolTableForExpression,
  ...
  switch (expression.kind()) {
    case Expression::Kind::kBinary: {
      ...
      return BinaryExpression::Make(
          *fContext, pos, expr(binaryExpr.left()), binaryExpr.getOperator(),
          expr(binaryExpr.right()));
    }
    case Expression::Kind::kLiteral: return expression.clone();
    case Expression::Kind::kChildCall: {
      const ChildCall& childCall = expression.as<ChildCall>();
      return ChildCall::Make(
          *fContext, pos, childCall.type().clone(symbolTableForExpression), childCall.child(),
          argList(childCall.arguments()));
    }
    case Expression::Kind::kConstructorArray: {
      const ConstructorArray& ctor = expression.as<ConstructorArray>();
      return ConstructorArray::Make(
          *fContext, pos, *ctor.type().clone(symbolTableForExpression), argList(ctor.arguments()));
    }
    case Expression::Kind::kConstructorArrayCast: {
      const ConstructorArrayCast& ctor = expression.as<ConstructorArrayCast>();
      return ConstructorArrayCast::Make(
          *fContext, pos, *ctor.type().clone(symbolTableForExpression), expr(ctor.argument()));
    }
    ...
    case Expression::Kind::kExternalFunctionCall: {
      ...
      return std::make_unique<ExternalFunctionCall>(
          pos, &externalCall.function(), argList(externalCall.arguments()));
    }
    ...
    case Expression::Kind::kFunctionCall: {
      ...
      return FunctionCall::Make(
          *fContext, pos, funcCall.type().clone(symbolTableForExpression), funcCall.function(),
    ...
    case Expression::Kind::kTernary: {
      ...
      return TernaryExpression::Make(
          *fContext, pos, expr(t.test()), expr(t.ifTrue()), expr(t.ifFalse()));
    }
...
std::unique_ptr<Statement> Inliner::inlineStatement(
    Position pos, VariableRewriteMap* varMap, SymbolTable* symbolTableForStatement,
  ...
  auto stmt = [&](const std::unique_ptr<Statement>& s) -> std::unique_ptr<Statement> {
    if (s) {
      return this->inlineStatement(
          pos, varMap, symbolTableForStatement, resultExpr, returnComplexity, *s, isBuiltinCode);
    }
    return nullptr;
  };
  ...
  switch (statement.kind()) {
    case Statement::Kind::kBlock: {
      ...
      return Block::Make(
          pos, blockStmts(b), b.blockKind(), SymbolTable::WrapIfBuiltin(b.symbolTable()));
    }
    ...
    case Statement::Kind::kFor: {
      ...
      return ForStatement::Make(
          *fContext, pos, ForLoopPositions{}, std::move(initializer), expr(f.test()),
    ...
    case Statement::Kind::kIf: {
      ...
      return IfStatement::Make(
          *fContext, pos, i.isStatic(), expr(i.test()), stmt(i.ifTrue()), stmt(i.ifFalse()));
    }
    ...
    case Statement::Kind::kReturn: {
      ...
      auto assignment = ExpressionStatement::Make(
          *fContext, BinaryExpression::Make(
                         *fContext, pos, clone_with_ref_kind(**resultExpr, VariableRefKind::kWrite),
                         Operator::Kind::EQ, expr(r.expression())));
    ...
    case Statement::Kind::kSwitch: {
      ...
      return SwitchStatement::Make(
          *fContext, pos, ss.isStatic(), expr(ss.value()), std::move(cases),
    ...
    case Statement::Kind::kVarDeclaration: {
      ...
      auto clonedVar = std::make_unique<Variable>(
          pos, variable.modifiersPosition(), &variable.modifiers(), name->c_str(),
...
Inliner::InlinedCall Inliner::inlineCall(
  ...
  if (returnComplexity > ReturnComplexity::kSingleSafeReturn &&
    ...
    ScratchVariable var = Variable::MakeScratchVariable(
        *fContext, function.declaration().name(), &function.declaration().returnType(), Modifiers{},
        symbolTable.get(),
  ...
  for (int i = 0; i < arguments.count(); ++i) {
    ...
    if (!paramUsage.fWrite) {
      ...
      if ((paramUsage.fRead > 1) ? Analysis::IsTrivialExpression(*arg) : !arg->hasSideEffects()) {
    ...
    ScratchVariable var = Variable::MakeScratchVariable(
        *fContext, param->name(), &arg->type(), param->modifiers(), symbolTable.get(),
  ...
  for (const std::unique_ptr<Statement>& stmt : body.children()) {
    inlineStatements.push_back(this->inlineStatement(
        pos, &varMap, symbolTable.get(), &resultExpr, returnComplexity, *stmt,
        caller->isBuiltin()));
  }
  ...
  inlinedCall.fInlinedBody =
      Block::MakeBlock(pos, std::move(inlineStatements), Block::Kind::kUnbracedBlock);
  if (resultExpr) {
    ...
    fContext->fErrors->error(
        function.fPosition, "inliner found non-void function '" +
...
bool Inliner::candidateCanBeInlined(
    const InlineCandidate& candidate, const ProgramUsage& usage, InlinabilityCache* cache) {
  ...
  candidates.erase(
      std::remove_if(
          ...
          [&](const InlineCandidate& candidate) {
            return !this->candidateCanBeInlined(candidate, *usage, &cache);
          }),
      candidates.end());
...
bool Inliner::analyze(
  ...
  using StatementRemappingTable =
      SkTHashMap<std::unique_ptr<Statement>*, std::unique_ptr<Statement>*>;
