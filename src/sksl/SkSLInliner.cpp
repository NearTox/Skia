  ...
  class ContainsRecursiveCall : public ProgramVisitor {
   public:
    bool visit(const FunctionDeclaration& funcDecl) {
      fFuncDecl = &funcDecl;
      return funcDecl.fDefinition ? this->visitProgramElement(*funcDecl.fDefinition) : false;
    }
  ...
  switch (expression.kind()) {
    case Expression::Kind::kBinary: {
      ...
      return std::make_unique<BinaryExpression>(
          offset, expr(b.leftPointer()), b.getOperator(), expr(b.rightPointer()), &b.type());
    }
    ...
    case Expression::Kind::kExternalFunctionCall: {
      ...
      return std::make_unique<ExternalFunctionCall>(
          offset, &externalCall.type(), externalCall.function(), argList(externalCall.arguments()));
    }
    ...
    case Expression::Kind::kFunctionCall: {
      ...
      return std::make_unique<FunctionCall>(
          offset, &funcCall.type(), funcCall.fFunction, argList(funcCall.fArguments));
    }
    ...
    case Expression::Kind::kTernary: {
      ...
      return std::make_unique<TernaryExpression>(
          offset, expr(t.fTest), expr(t.fIfTrue), expr(t.fIfFalse));
    }
    ...
    case Statement::Kind::kReturn: {
      ...
      if (r.fExpression) {
        ...
        auto assignment = std::make_unique<ExpressionStatement>(std::make_unique<BinaryExpression>(
            offset, clone_with_ref_kind(*resultExpr, VariableReference::kWrite_RefKind),
            Token::Kind::TK_EQ, expr(r.fExpression), &resultExpr->type()));
        if (haveEarlyReturns) {
    ...
    case Statement::Kind::kVarDeclaration: {
      ...
      const Variable* clone =
          symbolTableForStatement->takeOwnershipOfSymbol(std::make_unique<Variable>(
              offset, old->fModifiers, namePtr->c_str(), typePtr, isBuiltinCode, old->fStorage,
              initialValue.get()));
...
Inliner::InlinedCall Inliner::inlineCall(
  ...
  inlinedCall.fInlinedBody = std::make_unique<Block>(
      offset, std::vector<std::unique_ptr<Statement>>{},
      /*symbols=*/nullptr,
      /*isScope=*/false);

  Block& inlinedBody = *inlinedCall.fInlinedBody;
  inlinedBody.children().reserve(
      1 +                 // Inline marker
      1 +                 // Result variable
  ...
  auto makeInlineVar =
    ...
    auto newVar = std::make_unique<Variable>(
        /*offset=*/-1, Modifiers(), nameFrag, type, caller->fBuiltin, Variable::kLocal_Storage,
        initialValue->get());
  ...
  if (function.fDeclaration.fReturnType != *fContext->fVoid_Type) {
    ...
    resultExpr = makeInlineVar(
        String(function.fDeclaration.fName), &function.fDeclaration.fReturnType, Modifiers{},
        &noInitialValue);
  }
  ...
  for (int i = 0; i < (int)arguments.size(); ++i) {
    ...
    varMap[param] = makeInlineVar(
        String(param->fName), &arguments[i]->type(), param->fModifiers, &arguments[i]);
  }
  ...
  for (int i : argsToCopyBack) {
    ...
    inlinedBody.children().push_back(
        std::make_unique<ExpressionStatement>(std::make_unique<BinaryExpression>(
            offset, clone_with_ref_kind(*arguments[i], VariableReference::kWrite_RefKind),
...
bool Inliner::analyze(Program& program) {
  ...
  class InlineCandidateAnalyzer {
   public:
    ...
    void visitStatement(
        std::unique_ptr<Statement>* stmt, bool isViableAsEnclosingStatement = true) {
      if (!*stmt) {
        return;
      }
    ...
    void addInlineCandidate(std::unique_ptr<Expression>* candidate) {
      fInlineCandidates.push_back(InlineCandidate{
  ...
  for (const InlineCandidate& candidate : analyzer.fInlineCandidates) {
    ...
    if (inlinableMap.find(funcDecl) == inlinableMap.end()) {
      ...
      // inlining.
      int inlineThreshold =
          (funcDecl->fCallCount.load() > 1) ? fSettings->fInlineThreshold : INT_MAX;
      inlinableMap[funcDecl] =
          this->isSafeToInline(funcCall, inlineThreshold) && !contains_recursive_call(*funcDecl);
    }
  }
  ...
  for (const InlineCandidate& candidate : analyzer.fInlineCandidates) {
    ...
    InlinedCall inlinedCall = this->inlineCall(
        &funcCall, candidate.fSymbols, &candidate.fEnclosingFunction->fDeclaration);
