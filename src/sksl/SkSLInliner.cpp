...
static const Type* copy_if_needed(const Type* src, SymbolTable& symbolTable) {
  if (src->typeKind() == Type::TypeKind::kArray) {
    return symbolTable.takeOwnershipOfSymbol(
        std::make_unique<Type>(src->name(), src->typeKind(), src->componentType(), src->columns()));
  }
  return src;
}
...
bool is_trivial_argument(const Expression& argument) {
  return argument.is<VariableReference>() ||
         ...
         (argument.is<FieldAccess>() && is_trivial_argument(*argument.as<FieldAccess>().base())) ||
         (argument.is<Constructor>() && argument.as<Constructor>().arguments().size() == 1 &&
...
void Inliner::reset(
    const Context* context, ModifiersPool* modifiers, const Program::Settings* settings,
...
std::unique_ptr<Expression> Inliner::inlineExpression(
    int offset, VariableRewriteMap* varMap, const Expression& expression) {
  ...
  switch (expression.kind()) {
    ...
    case Expression::Kind::kNullLiteral: return expression.clone();
    case Expression::Kind::kConstructor: {
      const Constructor& constructor = expression.as<Constructor>();
      return std::make_unique<Constructor>(
          offset, &constructor.type(), argList(constructor.arguments()));
    }
    ...
    case Expression::Kind::kExternalValue: return expression.clone();
...
std::unique_ptr<Statement> Inliner::inlineStatement(
    ...
    const Expression* resultExpr, bool haveEarlyReturns, const Statement& statement,
    bool isBuiltinCode) {
  auto stmt = [&](const std::unique_ptr<Statement>& s) -> std::unique_ptr<Statement> {
    if (s) {
      return this->inlineStatement(
          offset, varMap, symbolTableForStatement, resultExpr, haveEarlyReturns, *s, isBuiltinCode);
    }
    return nullptr;
  };
    ...
    case Statement::Kind::kReturn: {
      ...
      if (r.expression()) {
        ...
        auto assignment = std::make_unique<ExpressionStatement>(std::make_unique<BinaryExpression>(
            ...
            Token::Kind::TK_EQ, expr(r.expression()), &resultExpr->type()));
        if (haveEarlyReturns) {
    ...
    case Statement::Kind::kVarDeclaration: {
      ...
      const Variable* clone =
          symbolTableForStatement->takeOwnershipOfSymbol(std::make_unique<Variable>(
              offset, old.modifiersHandle(), namePtr->c_str(), typePtr, isBuiltinCode,
              old.storage(), initialValue.get()));
      (*varMap)[&old] = std::make_unique<VariableReference>(offset, clone);
      return std::make_unique<VarDeclaration>(
          clone, baseTypePtr, std::move(sizes), std::move(initialValue));
    }
    case Statement::Kind::kWhile: {
...
Inliner::InlinedCall Inliner::inlineCall(
    FunctionCall* call, SymbolTable* symbolTableForCall, const FunctionDeclaration* caller) {
  ...
  auto makeInlineVar =
    ...
    const String* namePtr =
        symbolTableForCall->takeOwnershipOfString(std::make_unique<String>(std::move(uniqueName)));
    ...
    const Variable* variableSymbol = symbolTableForCall->add(std::make_unique<Variable>(
        /*offset=*/-1, fModifiers->handle(Modifiers()), nameFrag, type, caller->isBuiltin(),
  ...
  if (function.declaration().returnType() != *fContext->fVoid_Type) {
    ...
    resultExpr = makeInlineVar(
        String(function.declaration().name()), &function.declaration().returnType(), Modifiers{},
        &noInitialValue);
  }
  ...
  for (int i = 0; i < (int)arguments.size(); ++i) {
    ...
    varMap[param] = makeInlineVar(
        String(param->name()), &arguments[i]->type(), param->modifiers(), &arguments[i]);
  }
  ...
  if (resultExpr != nullptr) {
    ...
    SkASSERT(resultExpr->as<VariableReference>().refKind() == VariableReference::RefKind::kRead);
...
bool Inliner::candidateCanBeInlined(const InlineCandidate& candidate, InlinabilityCache* cache) {
  const FunctionDeclaration& funcDecl = (*candidate.fCandidateExpr)->as<FunctionCall>().function();
...
bool Inliner::analyze(Program& program) {
  ...
  for (const InlineCandidate& candidate : candidateList.fCandidates) {
    ...
    // idea to inline it.
    if (candidate.fIsLargeFunction && !(funcDecl.modifiers().fFlags & Modifiers::kInline_Flag) &&
        usage->get(funcDecl) > 1) {
      continue;
    }
