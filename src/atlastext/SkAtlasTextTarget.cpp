...
class SkInternalAtlasTextTarget : public GrTextTarget, public SkAtlasTextTarget {
 public:
  SkInternalAtlasTextTarget(sk_sp<SkAtlasTextContext> context, int width, int height, void* handle)
  ...
  ~SkInternalAtlasTextTarget() override { this->deleteOps(); }
  ...
  void drawText(
      const SkGlyphID[], const SkPoint[], int glyphCnt, uint32_t color,
...
void SkInternalAtlasTextTarget::drawText(
    const SkGlyphID glyphs[], const SkPoint positions[], int glyphCnt, uint32_t color,
...
void GrAtlasTextOp::executeForTextTarget(SkAtlasTextTarget* target) {
  ...
  for (int i = 0; i < fGeoCount; ++i) {
    ...
    GrTextBlob::VertexRegenerator regenerator(
        resourceProvider, fGeoData[i].fBlob, fGeoData[i].fSubRunPtr, fGeoData[i].fViewMatrix,
        fGeoData[i].fX, fGeoData[i].fY, fGeoData[i].fColor.toBytes_RGBA(), &context, glyphCache,
        atlasManager);
    bool done = false;
    while (!done) {
      ...
      context.recordDraw(
          result.fFirstVertex, result.fGlyphsRegenerated, fGeoData[i].fViewMatrix,
          target->handle());
      if (!result.fFinished) {
