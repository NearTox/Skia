...
bool SkXPSDevice::beginSheet(const SkVector& unitsPerMeter,
...
HRESULT SkXPSDevice::createXpsThumbnail(IXpsOMPage* page,
    ...
    HRM(CoCreateInstance(CLSID_XpsOMThumbnailGenerator,
                         nullptr,
    ...
    HRM(thumbnailGenerator->GenerateThumbnail(page, XPS_IMAGE_TYPE_PNG, XPS_THUMBNAIL_SIZE_LARGE,
                                              partUri.get(), image),
...
HRESULT SkXPSDevice::initXpsDocumentWriter(IXpsOMImageResource* image) {
    ...
    // Begin the lone document.
    {
        ...
        HRM(this->fPackageWriter->StartNewDocument(partUri.get(), nullptr, nullptr, nullptr,
                                                   nullptr),
...
bool SkXPSDevice::endSheet() {
    ...
    HRBM(this->fPackageWriter->AddPage(page.get(), &pageSize, nullptr, nullptr, nullptr, nullptr),
...
static void transform_offsets(SkScalar* stopOffsets, const int numOffsets, const SkPoint& start,
                              const SkPoint& end, const SkMatrix& transform) {
...
HRESULT SkXPSDevice::createXpsImageBrush(const SkBitmap& bitmap,
    ...
    HRM(this->fXpsFactory->CreateImageResource(readWrapper.get(), XPS_IMAGE_TYPE_PNG,
                                               imagePartUri.get(), &imageResource),
    ...
    HRM(this->fXpsFactory->CreateImageBrush(imageResource.get(), &bitmapRect, &bitmapRect,
                                            &xpsImageBrush),
    ...
    if (SkTileMode::kClamp != xy[0] && SkTileMode::kClamp != xy[1]) {
        ...
        if (SkTileMode::kClamp == xy[0]) {
            ...
            HR(this->sideOfClamp(leftArea, leftImageViewBox, imageResource.get(),
                                 brushVisuals.get()));
            ...
            HR(this->sideOfClamp(rightArea, rightImageViewBox, imageResource.get(),
                                 brushVisuals.get()));
        }
        ...
        if (SkTileMode::kClamp == xy[1]) {
            ...
            HR(this->sideOfClamp(topArea, topImageViewBox, imageResource.get(),
                                 brushVisuals.get()));
            ...
            HR(this->sideOfClamp(bottomArea, bottomImageViewBox, imageResource.get(),
                                 brushVisuals.get()));
        }
...
HRESULT SkXPSDevice::createXpsGradientStop(const SkColor skColor,
    ...
    HRM(this->fXpsFactory->CreateGradientStop(&gradStopXpsColor, nullptr, SkScalarToFLOAT(offset),
                                              xpsGradStop),
...
HRESULT SkXPSDevice::createXpsLinearGradient(SkShader::GradientInfo info,
    ...
    if (xpsMatrix) {
        ...
        transform_offsets(info.fColorOffsets, info.fColorCount, info.fPoint[0], info.fPoint[1],
                          localMatrix);
    ...
    HRM(this->fXpsFactory->CreateLinearGradientBrush(gradStop0.get(), gradStop1.get(), &startPoint,
                                                     &endPoint, &gradientBrush),
...
HRESULT SkXPSDevice::createXpsBrush(const SkPaint& skPaint,
    ...
    if (SkShader::kNone_GradientType == gradientType) {
        ...
        if (SkShader::kLinear_GradientType == gradientType) {
            HR(this->createXpsLinearGradient(info, skPaint.getAlpha(), localMatrix,
                                             xpsMatrixToUse.get(), brush));
            return S_OK;
        }

        if (SkShader::kRadial_GradientType == gradientType) {
            HR(this->createXpsRadialGradient(info, skPaint.getAlpha(), localMatrix,
                                             xpsMatrixToUse.get(), brush));
            return S_OK;
        }
...
HRESULT SkXPSDevice::createXpsQuad(const SkPoint (&points)[4], BOOL stroke, BOOL fill,
...
template <typename F, typename... Args> void draw(SkClipStackDevice* dev, F f, Args&&... args) {
...
void SkXPSDevice::drawPoints(SkCanvas::PointMode mode, size_t count, const SkPoint points[],
                             const SkPaint& paint) {
...
static HRESULT close_figure(const SkTDArray<XPS_SEGMENT_TYPE>& segmentTypes,
                            ...
                            const SkTDArray<FLOAT>& segmentData, BOOL stroke, BOOL fill,
                            IXpsOMGeometryFigure* figure, IXpsOMGeometryFigureCollection* figures) {
    // Add the segment data to the figure.
    HRM(figure->SetSegments(segmentTypes.count(), segmentData.count(), segmentTypes.begin(),
                            segmentData.begin(), segmentStrokes.begin()),
...
HRESULT SkXPSDevice::addXpsPathGeometry(IXpsOMGeometryFigureCollection* xpsFigures, BOOL stroke,
                                        BOOL fill, const SkPath& path) {
    ...
    while ((verb = iter.next(points)) != SkPath::kDone_Verb) {
        switch (verb) {
            case SkPath::kMove_Verb: {
                if (xpsFigure.get()) {
                    HR(close_figure(segmentTypes, segmentStrokes, segmentData, stroke, fill,
    ...
    if (xpsFigure.get()) {
        HR(close_figure(segmentTypes, segmentStrokes, segmentData, stroke, fill, xpsFigure.get(),
                        xpsFigures));
    }
    return S_OK;
}

void SkXPSDevice::convertToPpm(const SkMaskFilter* filter, SkMatrix* matrix, SkVector* ppuScale,
...
HRESULT SkXPSDevice::applyMask(const SkMask& mask,
    ...
    const SkRect universeRect = SkRect::MakeLTRB(0, 0, this->fCurrentCanvasSize.fWidth,
...
HRESULT SkXPSDevice::shadePath(IXpsOMPath* shadedPath, const SkPaint& shaderPaint,
                               const SkMatrix& matrix, BOOL* fill, BOOL* stroke) {
    *fill = FALSE;
    *stroke = FALSE;
    ...
    const bool hasStroke =
            SkPaint::kStroke_Style == style || SkPaint::kStrokeAndFill_Style == style;
...
void SkXPSDevice::drawPath(const SkPath& platonicPath,
    ...
    if (filter) {
        SkIRect clipIRect;
        SkVector ppuScale;
        this->convertToPpm(filter, &matrix, &ppuScale, this->cs().bounds(size(*this)).roundOut(),
                           &clipIRect);
        ...
        //[Pixel-path -> Mask]
        SkMask rasteredMask;
        if (SkDraw::DrawToMask(*pixelPath,
                               &clipIRect,
    ...
    switch (fillablePath->getFillType()) {
        ...
        case SkPath::kInverseEvenOdd_FillType: {
            const SkRect universe = SkRect::MakeLTRB(0, 0, this->fCurrentCanvasSize.fWidth,
                                                     this->fCurrentCanvasSize.fHeight);
...
void SkXPSDevice::drawBitmapRect(const SkBitmap& bitmap,
    ...
    auto bitmapShader =
            SkMakeBitmapShaderForPaint(paint, bitmap, SkTileMode::kClamp, SkTileMode::kClamp,
