...
class SkXPSDevice : public SkClipStackDevice {
 public:
  ...
  bool beginSheet(
      ...
      const SkRect* mediaBox = nullptr, const SkRect* bleedBox = nullptr,
      const SkRect* artBox = nullptr, const SkRect* cropBox = nullptr);
 ...
 private:
  class TypefaceUse {
   public:
    TypefaceUse(
        SkTypefaceID id, int index, std::unique_ptr<SkStream> data,
...
#  endif
  ...
  HRESULT createXpsThumbnail(
      IXpsOMPage* page, const unsigned int pageNumber, IXpsOMImageResource** image);

  void internalDrawRect(const SkRect& r, bool transformRect, const SkPaint& paint);

  HRESULT createXpsBrush(
      const SkPaint& skPaint, IXpsOMBrush** xpsBrush, const SkMatrix* parentTransform = nullptr);
  ...
  HRESULT createXpsLinearGradient(
      ...
      IXpsOMMatrixTransform* xpsMatrixToUse, IXpsOMBrush** xpsBrush);
  ...
  HRESULT addXpsPathGeometry(
      IXpsOMGeometryFigureCollection* figures, BOOL stroke, BOOL fill, const SkPath& path);

  HRESULT createPath(
      IXpsOMGeometryFigure* figure, IXpsOMVisualCollection* visuals, IXpsOMPath** path);

  HRESULT sideOfClamp(
      const SkRect& leftPoints, const XPS_RECT& left, IXpsOMImageResource* imageResource,
      IXpsOMVisualCollection* visuals);

  HRESULT cornerOfClamp(
      const SkRect& tlPoints, const SkColor color, IXpsOMVisualCollection* visuals);
