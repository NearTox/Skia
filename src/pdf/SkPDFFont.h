...
class SkPDFFont {
 public:
  ...
  static SkAdvancedTypefaceMetrics::FontType FontType(
      const SkTypeface&, const SkAdvancedTypefaceMetrics&);
   ...
   */
  static const SkAdvancedTypefaceMetrics* GetMetrics(
      const SkTypeface* typeface, SkPDFDocument* canon);

  static const std::vector<SkUnichar>& GetUnicodeMap(
      const SkTypeface* typeface, SkPDFDocument* canon);

  static void PopulateCommonFontDescriptor(
      SkPDFDict* descriptor, const SkAdvancedTypefaceMetrics&, uint16_t emSize,
 ...
 private:
  ...
  SkPDFFont(
      sk_sp<SkTypeface>, SkGlyphID firstGlyphID, SkGlyphID lastGlyphID,
