...
void SkPDFDevice::drawPoints(
  ...
  if (paint->getPathEffect()) {
    draw_points(mode, count, points, *paint, this->devClipBounds(), this->localToDevice(), this);
    return;
  }
...
void SkPDFDevice::internalDrawGlyphRun(
  ...
  if (runPaint.getPathEffect() || runPaint.getMaskFilter() || glyphRunFont.isEmbolden() ||
      this->localToDevice().hasPerspective() || SkPaint::kFill_Style != runPaint.getStyle()) {
  ...
  while (SkClusterator::Cluster c = clusterator.next()) {
    ...
    for (; index < glyphLimit; ++index) {
      ...
      SkRect glyphBounds = get_glyph_bounds_device_space(
          glyphs[index], textScaleX, textScaleY, xy + offset, this->localToDevice());
