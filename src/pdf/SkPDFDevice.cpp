...
sk_sp<SkImage> mask_to_greyscale_image(SkMask* mask) {
  ...
  SkPixmap pm(
      SkImageInfo::Make(
          mask->fBounds.width(), mask->fBounds.height(), kGray_8_SkColorType, kOpaque_SkAlphaType),
...
sk_sp<SkImage> alpha_image_to_greyscale_image(const SkImage* mask) {
  ...
  if (!mask->readPixels(
          SkImageInfo::MakeA8(w, h), greyBitmap.getPixels(), greyBitmap.rowBytes(), 0, 0)) {
    return nullptr;
  }
...
static void draw_points(
    ...
    const SkIRect& bounds, const SkMatrix& ctm, SkBaseDevice* device) {
...
class ScopedContentEntry {
 public:
  ScopedContentEntry(
      SkPDFDevice* device, const SkClipStack* clipStack, const SkMatrix& matrix,
      const SkPaint& paint, SkScalar textScale = 0)
      : fDevice(device), fBlendMode(SkBlendMode::kSrcOver), fClipStack(clipStack) {
  ...
  bool needShape() {
    switch (fBlendMode) {
      ...
      case SkBlendMode::kModulate: return true;
   ...
   */
  void setShape(const SkPath& shape) { fShape = shape; }
...
void SkPDFDevice::drawAnnotation(const SkRect& rect, const char key[], SkData* value) {
  ...
  if (!strcmp(SkAnnotationKeys::URL_Key(), key)) {
    fDocument->fCurrentPageLinkToURLs.push_back(std::make_pair(sk_ref_sp(value), transformedRect));
...
void SkPDFDevice::drawPoints(
    SkCanvas::PointMode mode, size_t count, const SkPoint* points, const SkPaint& srcPaint) {
  if (this->hasEmptyClip()) {
    return;
  }
  if (count == 0) {
  ...
  if (paint->getPathEffect()) {
    draw_points(mode, count, points, *paint, this->devClipBounds(), this->ctm(), this);
    return;
  }
...
void SkPDFDevice::internalDrawPathWithFilter(
    const SkClipStack& clipStack, const SkMatrix& ctm, const SkPath& origPath,
...
void SkPDFDevice::internalDrawPath(
    const SkClipStack& clipStack, const SkMatrix& ctm, const SkPath& origPath,
    const SkPaint& srcPaint, bool pathIsMutable) {
...
void SkPDFDevice::drawImageRect(
  ...
  this->internalDrawImageRect(
      SkKeyedImage(sk_ref_sp(const_cast<SkImage*>(image))), src, dst, paint, this->ctm());
}

void SkPDFDevice::drawBitmapRect(
...
class GlyphPositioner {
 public:
  GlyphPositioner(SkDynamicMemoryWStream* content, SkScalar textSkewX, SkPoint origin)
  ...
  void writeGlyph(SkPoint xy, SkScalar advanceWidth, uint16_t glyph) {
    if (!fInitialized) {
...
static SkRect get_glyph_bounds_device_space(
    SkGlyphID gid, SkStrike* cache, SkScalar xScale, SkScalar yScale, SkPoint xy,
    const SkMatrix& ctm) {
  const SkGlyph& glyph = cache->getGlyphIDMetrics(gid);
  SkRect glyphBounds = {glyph.fLeft * xScale, glyph.fTop * yScale,
...
void SkPDFDevice::drawGlyphRunAsPath(
  ...
  font.getPaths(
      glyphRun.glyphsIDs().data(), glyphRun.glyphsIDs().size(),
      [](const SkPath* path, const SkMatrix& mx, void* ctx) {
        Rec* rec = reinterpret_cast<Rec*>(ctx);
        if (path) {
          SkMatrix total = mx;
          total.postTranslate(rec->fPos->fX + rec->fOffset.fX, rec->fPos->fY + rec->fOffset.fY);
...
static bool needs_new_font(
    SkPDFFont* font, SkGlyphID gid, SkStrike* cache, SkAdvancedTypefaceMetrics::FontType fontType) {
...
void SkPDFDevice::internalDrawGlyphRun(
  ...
  if (runPaint.getPathEffect() || runPaint.getMaskFilter() || glyphRunFont.isEmbolden() ||
      this->ctm().hasPerspective() || SkPaint::kFill_Style != runPaint.getStyle()) {
    ...
    for (; index < glyphLimit; ++index) {
      ...
      if (needs_new_font(font, gid, glyphCache.get(), fontType)) {
        ...
        SkPDFWriteResourceName(
            out, SkPDFResourceType::kFont, add_resource(fFontResources, font->indirectReference()));
        out->writeText(" ");
        SkPDFUtils::AppendScalar(textSize, out);
        out->writeText(" Tf\n");
      }
      font->noteGlyphUsage(gid);
      SkGlyphID encodedGlyph = font->multiByteGlyphs() ? gid : font->glyphToPDFFontEncoding(gid);
...
void SkPDFDevice::drawFormXObject(SkPDFIndirectReference xObject, SkDynamicMemoryWStream* content) {
  SkASSERT(xObject);
  SkPDFWriteResourceName(
      content, SkPDFResourceType::kXObject, add_resource(fXObjectResources, xObject));
...
std::unique_ptr<SkPDFDict> SkPDFDevice::makeResourceDict() {
  return SkPDFMakeResourceDict(
      sort(fGraphicStateResources), sort(fShaderResources), sort(fXObjectResources),
      sort(fFontResources));
}
 ...
 * in the first place.
 */
bool SkPDFDevice::handleInversePath(
    const SkPath& origPath, const SkPaint& paint, bool pathIsMutable) {
  ...
  bounds.outset(paint.getStrokeWidth() + SK_Scalar1, paint.getStrokeWidth() + SK_Scalar1);
...
SkPDFIndirectReference SkPDFDevice::makeFormXObjectFromDevice(SkIRect bounds, bool alpha) {
  ...
  SkPDFIndirectReference xobject = SkPDFMakeFormXObject(
      fDocument, this->content(),
      SkPDFMakeArray(bounds.left(), bounds.top(), bounds.right(), bounds.bottom()),
...
void SkPDFDevice::drawFormXObjectWithMask(
    SkPDFIndirectReference xObject, SkPDFIndirectReference sMask, SkBlendMode mode,
    bool invertClip) {
  SkASSERT(sMask);
  SkPaint paint;
  ...
  this->setGraphicState(
      SkPDFGraphicState::GetSMaskGraphicState(
          sMask, invertClip, SkPDFGraphicState::kAlpha_SMaskMode, fDocument),
      content.stream());
...
static void populate_graphic_state_entry_from_paint(
    SkPDFDocument* doc, const SkMatrix& matrix, const SkClipStack* clipStack, SkIRect deviceBounds,
    const SkPaint& paint, const SkMatrix& initialTransform, SkScalar textScale,
    SkPDFGraphicStackState::Entry* entry, SkTHashSet<SkPDFIndirectReference>* shaderResources,
...
SkDynamicMemoryWStream* SkPDFDevice::setUpContentEntry(
    const SkClipStack* clipStack, const SkMatrix& matrix, const SkPaint& paint, SkScalar textScale,
  ...
  if (!treat_as_regular_pdf_blend_mode(blendMode) && blendMode != SkBlendMode::kDstOver) {
    ...
    } else if (blendMode != SkBlendMode::kSrc && blendMode != SkBlendMode::kSrcOut) {
  ...
  populate_graphic_state_entry_from_paint(
      fDocument, matrix, clipStack, this->bounds(), paint, fInitialTransform, textScale, &entry,
      &fShaderResources, &fGraphicStateResources);
...
void SkPDFDevice::finishContentEntry(
    const SkClipStack* clipStack, SkBlendMode blendMode, SkPDFIndirectReference dst,
    SkPath* shape) {
  ...
  if (!dst) {
    SkASSERT(blendMode == SkBlendMode::kSrc || blendMode == SkBlendMode::kSrcOut);
    return;
  }
  ...
  if (blendMode == SkBlendMode::kSrcATop) {
    ...
    if (shape != nullptr) {
      ...
      shapeDev.internalDrawPath(
          clipStack ? *clipStack : empty, SkMatrix::I(), *shape, filledPaint, true);
      this->drawFormXObjectWithMask(
          dst, shapeDev.makeFormXObjectFromDevice(), SkBlendMode::kSrcOver, true);
    } else {
  ...
  if (blendMode == SkBlendMode::kClear) {
    return;
  } else if (blendMode == SkBlendMode::kSrc || blendMode == SkBlendMode::kDstATop) {
    ScopedContentEntry content(this, nullptr, SkMatrix::I(), stockPaint);
    if (content) {
  ...
  SkASSERT(
      blendMode == SkBlendMode::kSrcIn || blendMode == SkBlendMode::kDstIn ||
      blendMode == SkBlendMode::kSrcOut || blendMode == SkBlendMode::kDstOut ||
      blendMode == SkBlendMode::kSrcATop || blendMode == SkBlendMode::kDstATop ||
      blendMode == SkBlendMode::kModulate);

  if (blendMode == SkBlendMode::kSrcIn || blendMode == SkBlendMode::kSrcOut ||
      blendMode == SkBlendMode::kSrcATop) {
    this->drawFormXObjectWithMask(
        srcFormXObject, dst, SkBlendMode::kSrcOver, blendMode == SkBlendMode::kSrcOut);
    return;
  } else {
...
static bool is_integer(SkScalar x) { return x == SkScalarTruncToScalar(x); }

static bool is_integral(const SkRect& r) {
  return is_integer(r.left()) && is_integer(r.top()) && is_integer(r.right()) &&
         is_integer(r.bottom());
}
...
void SkPDFDevice::internalDrawImageRect(
  ...
  if (src && *src != SkRect::Make(bounds)) {
    ...
    transform.preTranslate(SkIntToScalar(bounds.x()), SkIntToScalar(bounds.y()));
  ...
  SkTCopyOnFirstWrite<SkPaint> paint(srcPaint);
  if (SkBlendMode::kSrcOver != paint->getBlendMode() && imageSubset.image()->isOpaque() &&
      kSrcOver_SkXfermodeInterpretation == SkInterpretXfermode(*paint, false)) {
    paint.writable()->setBlendMode(SkBlendMode::kSrcOver);
  }
  ...
  scaled.postScale(SkIntToScalar(subset.width()), SkIntToScalar(subset.height()));
...
void SkPDFDevice::drawSpecial(
  ...
  if (filter) {
    ...
    const SkIRect clipBounds = this->cs().bounds(this->bounds()).roundOut().makeOffset(-x, -y);
