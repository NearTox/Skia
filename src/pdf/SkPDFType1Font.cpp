...
static bool parsePFB(
    const uint8_t* src, size_t size, size_t* headerLen, size_t* dataLen, size_t* trailerLen) {
 ...
 *
 */
static bool parsePFA(
    const char* src, size_t size, size_t* headerLen, size_t* hexDataLen, size_t* dataLen,
    size_t* trailerLen) {
...
static sk_sp<SkData> convert_type1_font_stream(
    std::unique_ptr<SkStreamAsset> srcStream, size_t* headerLen, size_t* dataLen,
    size_t* trailerLen) {
...
static SkPDFIndirectReference make_type1_font_descriptor(
    SkPDFDocument* doc, const SkTypeface* typeface, const SkAdvancedTypefaceMetrics* info) {
  ...
  if (info) {
    ...
    if (can_embed(*info)) {
      ...
      sk_sp<SkData> fontData =
          convert_type1_font_stream(std::move(rawFontData), &header, &data, &trailer);
      if (fontData) {
        ...
        descriptor.insertRef(
            "FontFile", SkPDFStreamOut(std::move(dict), std::move(fontStream), doc, true));
      }
    }
  }
...
static const std::vector<SkString>& type_1_glyphnames(
    SkPDFDocument* canon, const SkTypeface* typeface) {
...
static SkPDFIndirectReference type1_font_descriptor(
    SkPDFDocument* doc, const SkTypeface* typeface) {
  SkFontID fontID = typeface->uniqueID();
...
void SkPDFEmitType1Font(const SkPDFFont& pdfFont, SkPDFDocument* doc) {
  ...
  font.insertInt("LastChar", (size_t)glyphCount);
  {
    ...
    glyphCache->getAdvances(
        SkSpan<const SkGlyphID>{glyphIDs.get(), SkTo<size_t>(glyphRangeSize)}, advances.get());
    for (int i = 0; i < glyphRangeSize; ++i) {
      widths->appendScalar(from_font_units(advances[i].x(), SkToU16(emSize)));
    }
