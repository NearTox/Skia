...
static void append_bfchar_section(
    const std::vector<BFChar>& bfchar, bool multiByte, SkDynamicMemoryWStream* cmap) {
...
static void append_bfrange_section(
    const std::vector<BFRange>& bfrange, bool multiByte, SkDynamicMemoryWStream* cmap) {
  // PDF spec defines that every bf* list can have at most 100 entries.
...
void SkPDFAppendCmapSections(
    const SkUnichar* glyphToUnicode, const SkPDFGlyphUse* subset, SkDynamicMemoryWStream* cmap,
    bool multiByteGlyphs, SkGlyphID firstGlyphID, SkGlyphID lastGlyphID) {
  int glyphOffset = 0;
  if (!multiByteGlyphs) {
  ...
  for (int i = firstGlyphID - glyphOffset; i < limit + 1; ++i) {
    ...
    if (!rangeEmpty) {
      ...
      bool inRange =
          ...
          glyphToUnicode[gid] == currentRangeEntry.fUnicode + i - currentRangeEntry.fStart;
      if (!inSubset || !inRange) {
...
std::unique_ptr<SkStreamAsset> SkPDFMakeToUnicodeCmap(
    const SkUnichar* glyphToUnicode, const SkPDFGlyphUse* subset, bool multiByteGlyphs,
    SkGlyphID firstGlyphID, SkGlyphID lastGlyphID) {
  SkDynamicMemoryWStream cmap;
  append_tounicode_header(&cmap, multiByteGlyphs);
  SkPDFAppendCmapSections(
      glyphToUnicode, subset, &cmap, multiByteGlyphs, firstGlyphID, lastGlyphID);
