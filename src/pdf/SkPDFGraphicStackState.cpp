...
static bool is_rect(const SkClipStack& clipStack, const SkRect& bounds, SkRect* dst) {
  ...
  while (const SkClipStack::Element* element = iter.next()) {
    ...
    switch (element->getDeviceSpaceType()) {
      case SkClipStack::Element::DeviceSpaceType::kEmpty: break;
    ...
    switch (element->getOp()) {
      case kReplace_SkClipOp: currentClip = rect_intersect(bounds, elementRect); break;
      case SkClipOp::kIntersect: currentClip = rect_intersect(currentClip, elementRect); break;
      default: return false;
    }
  }
  *dst = currentClip;
...
static bool is_complex_clip(const SkClipStack& stack) {
  ...
  while (const SkClipStack::Element* element = iter.next()) {
    switch (element->getOp()) {
      ...
      case SkClipOp::kIntersect: break;
      default: return true;
    }
  }
  return false;
...
void SkPDFGraphicStackState::updateMatrix(const SkMatrix& matrix) {
  ...
  if (currentEntry()->fMatrix.getType() != SkMatrix::kIdentity_Mask) {
    ...
    SkASSERT(fEntries[fStackDepth].fClipStackGenID == fEntries[fStackDepth - 1].fClipStackGenID);
    this->pop();
