...
static void interpolate_color_code(
    SkScalar range, const ColorTuple& curColor, const ColorTuple& prevColor,
...
static void gradient_function_code(
  ...
  for (int i = 1; i < info.fColorCount; i++) {
    ...
    interpolate_color_code(
        info.fColorOffsets[i] - info.fColorOffsets[i - 1], colorData[i], colorData[i - 1], result);
    result->writeText("}\n");
  }
