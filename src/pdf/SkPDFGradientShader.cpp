...
static uint32_t hash(const SkPDFGradientShader::Key& k) {
  uint32_t buffer[] = {
      (uint32_t)k.fType, hash(k.fInfo), SkOpts::hash(&k.fCanvasTransform, sizeof(SkMatrix)),
      SkOpts::hash(&k.fShaderTransform, sizeof(SkMatrix)), SkOpts::hash(&k.fBBox, sizeof(SkIRect))};
  return SkOpts::hash(buffer, sizeof(buffer));
}
 ...
 */
static void interpolate_color_code(
    SkScalar range, SkColor beginColor, SkColor endColor, SkDynamicMemoryWStream* result) {
  ...
  ColorTuple curColor = {
      SkTo<uint8_t>(SkColorGetR(endColor)), SkTo<uint8_t>(SkColorGetG(endColor)),
      SkTo<uint8_t>(SkColorGetB(endColor))};

  ColorTuple prevColor = {
      SkTo<uint8_t>(SkColorGetR(beginColor)), SkTo<uint8_t>(SkColorGetG(beginColor)),
...
static void write_gradient_ranges(
    const SkShader::GradientInfo& info, SkSpan<size_t> rangeEnds, bool top, bool first,
  ...
  if (rangeEnds.size() == 1) {
    ...
    interpolate_color_code(
        rangeEnd - rangeBegin, info.fColors[rangeBeginIndex], info.fColors[rangeEndIndex], result);
    result->writeText("\n");
  } else {
...
/* Generate Type 4 function code to map t to the passed gradient, clamping at the ends.
  ...
  0 gt {1 1 1} if
 */
static void gradient_function_code(
    const SkShader::GradientInfo& info, SkDynamicMemoryWStream* result) {
...
static std::unique_ptr<SkPDFDict> createInterpolationFunction(
    const ColorTuple& color1, const ColorTuple& color2) {
...
static void apply_perspective_to_coordinates(
    const SkMatrix& inversePerspectiveMatrix, SkDynamicMemoryWStream* code) {
...
static void linearCode(
    const SkShader::GradientInfo& info, const SkMatrix& perspectiveRemover,
 ...
 */
static void twoPointConicalCode(
    const SkShader::GradientInfo& info, const SkMatrix& perspectiveRemover,
    SkDynamicMemoryWStream* function) {
...
static bool split_perspective(const SkMatrix in, SkMatrix* affine, SkMatrix* perspectiveInverse) {
  ...
  perspectiveInverse->setAll(one, zero, zero, zero, one, zero, -p0 / p2, -p1 / p2, 1 / p2);

  affine->setAll(
      sx - p0 * tx / p2, kx - p1 * tx / p2, tx / p2, ky - p0 * ty / p2, sy - p1 * ty / p2, ty / p2,
      zero, zero, one);

  return true;
}

static SkPDFIndirectReference make_ps_function(
    std::unique_ptr<SkStreamAsset> psCode, std::unique_ptr<SkPDFArray> domain,
    std::unique_ptr<SkPDFObject> range, SkPDFDocument* doc) {
...
static SkPDFIndirectReference make_function_shader(
    SkPDFDocument* doc, const SkPDFGradientShader::Key& state) {
  ...
  if (doStitchFunctions) {
    ...
    if (state.fType == SkShader::kConical_GradientType) {
      ...
      coords = SkPDFMakeArray(pt1.x(), pt1.y(), r1, pt2.x(), pt2.y(), r2);
    } else if (state.fType == SkShader::kRadial_GradientType) {
      const SkPoint& pt1 = info.fPoint[0];
      coords = SkPDFMakeArray(pt1.x(), pt1.y(), 0, pt1.x(), pt1.y(), info.fRadius[0]);
    } else {
      const SkPoint& pt1 = info.fPoint[0];
      const SkPoint& pt2 = info.fPoint[1];
      coords = SkPDFMakeArray(pt1.x(), pt1.y(), pt2.x(), pt2.y());
    }
    ...
    switch (state.fType) {
      case SkShader::kLinear_GradientType: break;
      ...
      default: return SkPDFIndirectReference();
    }
    ...
    pdfShader->insertRef(
        "Function",
        make_ps_function(
            functionCode.detachAsStream(), std::move(domain), std::move(rangeObject), doc));
  }
...
static SkPDFIndirectReference find_pdf_shader(
    SkPDFDocument* doc, SkPDFGradientShader::Key key, bool keyHasAlpha);

static std::unique_ptr<SkPDFDict> get_gradient_resource_dict(
    SkPDFIndirectReference functionShader, SkPDFIndirectReference gState) {
  ...
  return SkPDFMakeResourceDict(
      std::move(graphicStates), std::move(patternShaders), std::vector<SkPDFIndirectReference>(),
...
static std::unique_ptr<SkStreamAsset> create_pattern_fill_content(
    int gsIndex, int patternIndex, SkRect& bounds) {
...
static SkPDFIndirectReference create_smask_graphic_state(
    SkPDFDocument* doc, const SkPDFGradientShader::Key& state) {
  ...
  std::unique_ptr<SkPDFDict> resources =
      get_gradient_resource_dict(luminosityShader, SkPDFIndirectReference());
  SkRect bbox = SkRect::Make(state.fBBox);
  SkPDFIndirectReference alphaMask = SkPDFMakeFormXObject(
      doc, create_pattern_fill_content(-1, luminosityShader.fValue, bbox),
      SkPDFUtils::RectToArray(bbox), std::move(resources), SkMatrix::I(), "DeviceRGB");
  return SkPDFGraphicState::GetSMaskGraphicState(
      alphaMask, false, SkPDFGraphicState::kLuminosity_SMaskMode, doc);
}

static SkPDFIndirectReference make_alpha_function_shader(
    SkPDFDocument* doc, const SkPDFGradientShader::Key& state) {
  SkASSERT(state.fType != SkShader::kNone_GradientType);
  ...
  SkPDFUtils::PopulateTilingPatternDict(
      alphaFunctionShader.get(), bbox, std::move(resourceDict), SkMatrix::I());
  return SkPDFStreamOut(std::move(alphaFunctionShader), std::move(colorStream), doc);
}

static SkPDFGradientShader::Key make_key(
    const SkShader* shader, const SkMatrix& canvasTransform, const SkIRect& bbox) {
...
SkPDFIndirectReference SkPDFGradientShader::Make(
    SkPDFDocument* doc, SkShader* shader, const SkMatrix& canvasTransform, const SkIRect& bbox) {
  SkASSERT(shader);
