...
static void draw_image_matrix(
    SkCanvas* canvas, const SkImage* img, const SkMatrix& matrix, const SkPaint& paint) {
...
static void draw_bitmap_matrix(
    SkCanvas* canvas, const SkBitmap& bm, const SkMatrix& matrix, const SkPaint& paint) {
  SkAutoCanvasRestore acr(canvas, true);
  canvas->concat(matrix);
...
SkColor mix(SkColor c, U8CPU alpha) {
  return SkColorSetARGB(
      SkMulDiv255Round(alpha, SkColorGetA(c)), SkColorGetR(c), SkColorGetG(c), SkColorGetB(c));
}

static SkPDFIndirectReference make_image_shader(
    SkPDFDocument* doc, const SkPDFImageShaderKey& key, SkImage* image) {
  SkASSERT(image);
  ...
  if (tileModes[0] == SkTileMode::kClamp && tileModes[1] == SkTileMode::kClamp) {
    ...
    rect = SkRect::MakeLTRB(width, deviceBounds.top(), deviceBounds.right(), 0);
    if (!rect.isEmpty()) {
      paint2.setColor(mix(bitmap.getColor(bitmap.width() - 1, 0), alpha));
      canvas.drawRect(rect, paint2);
    }

    rect = SkRect::MakeLTRB(width, height, deviceBounds.right(), deviceBounds.bottom());
    if (!rect.isEmpty()) {
      paint2.setColor(mix(bitmap.getColor(bitmap.width() - 1, bitmap.height() - 1), alpha));
      canvas.drawRect(rect, paint2);
    }

    rect = SkRect::MakeLTRB(deviceBounds.left(), height, 0, deviceBounds.bottom());
    if (!rect.isEmpty()) {
...
static SkPDFIndirectReference make_fallback_shader(
    ...
    const SkIRect& surfaceBBox, SkColor paintColor) {
  // TODO(vandebo) This drops SKComposeShader on the floor.  We could
  // handle compose shader by pulling things up to a layer, drawing with
  // the first shader, applying the xfer mode and drawing again with the
  // second shader, then applying the layer to the original drawing.
  SkPDFImageShaderKey key = {canvasTransform,
                             SkMatrix::I(),
                             surfaceBBox,
