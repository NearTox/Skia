...
static void draw_matrix(
    SkCanvas* canvas, const SkImage* image, const SkMatrix& matrix, SkColor4f paintColor) {
  SkAutoCanvasRestore acr(canvas, true);
  canvas->concat(matrix);
  draw(canvas, image, paintColor);
}

static void draw_bitmap_matrix(
    SkCanvas* canvas, const SkBitmap& bm, const SkMatrix& matrix, SkColor4f paintColor) {
  SkAutoCanvasRestore acr(canvas, true);
  canvas->concat(matrix);
  SkPaint paint(paintColor);
...
static void fill_color_from_bitmap(
    SkCanvas* canvas, float left, float top, float right, float bottom, const SkBitmap& bitmap,
...
static SkPDFIndirectReference make_image_shader(
    SkPDFDocument* doc, SkMatrix finalMatrix, SkTileMode tileModesX, SkTileMode tileModesY,
    SkRect bBox, const SkImage* image, SkColor4f paintColor) {
  ...
  if (tileModesX == SkTileMode::kClamp && tileModesY == SkTileMode::kClamp) {
    ...
    fill_color_from_bitmap(
        &canvas, deviceBounds.left(), deviceBounds.top(), 0, 0, bitmap, 0, 0, paintColor.fA);

    fill_color_from_bitmap(
        &canvas, width, deviceBounds.top(), deviceBounds.right(), 0, bitmap, bitmap.width() - 1, 0,
        paintColor.fA);

    fill_color_from_bitmap(
        &canvas, width, height, deviceBounds.right(), deviceBounds.bottom(), bitmap,
        bitmap.width() - 1, bitmap.height() - 1, paintColor.fA);

    fill_color_from_bitmap(
        &canvas, deviceBounds.left(), height, 0, deviceBounds.bottom(), bitmap, 0,
        bitmap.height() - 1, paintColor.fA);
  }
  ...
  SkPDFUtils::PopulateTilingPatternDict(
      dict.get(), patternBBox, std::move(resourceDict), finalMatrix);
...
static SkPDFIndirectReference make_fallback_shader(
    SkPDFDocument* doc, SkShader* shader, const SkMatrix& canvasTransform,
    const SkIRect& surfaceBBox, SkColor4f paintColor) {
  ...
  SkASSERT(image);
  return make_image_shader(
      doc, SkMatrix::Concat(canvasTransform, shaderTransform), SkTileMode::kClamp,
      SkTileMode::kClamp, SkRect::Make(surfaceBBox), image.get(), paintColor);
}
...
SkPDFIndirectReference SkPDFMakeShader(
  ...
  if (SkImage* skimg = shader->isAImage(&shaderTransform, imageTileModes)) {
    ...
    SkPDFIndirectReference pdfShader = make_image_shader(
        doc, finalMatrix, imageTileModes[0], imageTileModes[1], SkRect::Make(surfaceBBox), skimg,
        paintColor);
