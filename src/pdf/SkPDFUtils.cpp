...
static void append_cubic(
    SkScalar ctl1X, SkScalar ctl1Y, SkScalar ctl2X, SkScalar ctl2Y, SkScalar dstX, SkScalar dstY,
    SkWStream* content) {
  SkString cmd("y\n");
...
static void append_quad(const SkPoint quad[], SkWStream* content) {
  ...
  append_cubic(
      cubic[1].fX, cubic[1].fY, cubic[2].fX, cubic[2].fY, cubic[3].fX, cubic[3].fY, content);
}
...
void SkPDFUtils::EmitPath(
    const SkPath& path, SkPaint::Style paintStyle, bool doConsumeDegerates, SkWStream* content,
    SkScalar tolerance) {
  ...
  if (path.isRect(&rect, &isClosed, &direction) && isClosed &&
      (SkPath::kCW_Direction == direction || SkPath::kEvenOdd_FillType == path.getFillType())) {
  ...
  for (SkPath::Verb verb = iter.next(args, doConsumeDegerates); verb != SkPath::kDone_Verb;
    ...
    switch (verb) {
      ...
      case SkPath::kCubic_Verb:
        append_cubic(
            args[1].fX, args[1].fY, args[2].fX, args[2].fY, args[3].fX, args[3].fY,
            &currentSegment);
        fillState = kNonSingleLine_SkipFillState;
        break;
...
void SkPDFUtils::ClosePath(SkWStream* content) { content->writeText("h\n"); }

void SkPDFUtils::PaintPath(SkPaint::Style style, SkPath::FillType fill, SkWStream* content) {
...
void SkPDFUtils::StrokePath(SkWStream* content) {
  SkPDFUtils::PaintPath(SkPaint::kStroke_Style, SkPath::kWinding_FillType, content);
}
...
void SkPDFUtils::PopulateTilingPatternDict(
    SkPDFDict* pattern, SkRect& bbox, std::unique_ptr<SkPDFDict> resources,
...
void SkPDFUtils::Base85Encode(std::unique_ptr<SkStreamAsset> stream, SkDynamicMemoryWStream* dst) {
  ...
  while (true) {
    ...
    uint32_t v =
        ((uint32_t)src[0] << 24) | ((uint32_t)src[1] << 16) | ((uint32_t)src[2] << 8) | src[3];
    if (v == 0 && count == 4) {
      dst->writeText("z");
      column += 1;
    } else {
