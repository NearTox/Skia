...
static HBBlob to_blob(sk_sp<SkData> data) {
  ...
  return HBBlob(hb_blob_create(
      blobData, blobSize, HB_MEMORY_MODE_READONLY, data.release(),
...
static sk_sp<SkData> to_data(HBBlob blob) {
  ...
  return SkData::MakeWithProc(
      data, SkToSizeT(length), [](const void*, void* ctx) { hb_blob_destroy((hb_blob_t*)ctx); },
      blob.release());
}

static sk_sp<SkData> subset_harfbuzz(
    sk_sp<SkData> fontData, const SkPDFGlyphUse& glyphUsage, int ttcIndex) {
#  if defined(SK_USING_THIRD_PARTY_ICU)
  if (!SkLoadICU()) {
    return nullptr;
  }
...
static sk_sp<SkData> subset_sfntly(
    sk_sp<SkData> fontData, const SkPDFGlyphUse& glyphUsage, const char* fontName, int ttcIndex) {
#  if defined(SK_USING_THIRD_PARTY_ICU)
  if (!SkLoadICU()) {
    return nullptr;
  }
...
#  endif  // defined(SK_BUILD_FOR_GOOGLE3)
  ...
  return SkData::MakeWithProc(
      subsetFont, subsetFontSize, [](const void* p, void*) { delete[](unsigned char*) p; },
      nullptr);
}
...
sk_sp<SkData> SkPDFSubsetFont(
    sk_sp<SkData> fontData, const SkPDFGlyphUse& glyphUsage, SkPDF::Metadata::Subsetter subsetter,
    const char* fontName, int ttcIndex) {
  switch (subsetter) {
...
sk_sp<SkData> SkPDFSubsetFont(
    sk_sp<SkData> fontData, const SkPDFGlyphUse& glyphUsage, SkPDF::Metadata::Subsetter,
    const char* fontName, int ttcIndex) {
  return subset_sfntly(std::move(fontData), glyphUsage, fontName, ttcIndex);
}
...
sk_sp<SkData> SkPDFSubsetFont(
    sk_sp<SkData>, const SkPDFGlyphUse&, SkPDF::Metadata::Subsetter, const char*, int) {
  return nullptr;
}
#endif  // defined(SK_PDF_USE_SFNTLY)
