...
CFTypeID SkCFGetTypeID();
#  define SK_GETCFTYPEID(cf)            \
    template <>                         \
    CFTypeID SkCFGetTypeID<cf##Ref>() { \
      return cf##GetTypeID();           \
    }
...
static bool SkCFDynamicCast(CFTypeRef cf, CF* cfAsCF, char const* name) {
  ...
  if (CFGetTypeID(cf) != SkCFGetTypeID<CF>()) {
    if (name) {
      SkDEBUGF(
          "%s is a %s but expected a %s\n", name, SkCFTypeIDDescription(CFGetTypeID(cf)).c_str(),
          SkCFTypeIDDescription(SkCFGetTypeID<CF>()).c_str());
    }
    return false;
  }
...
#  define SK_CFNUMBERTYPE_FOR(c, cf) \
    template <>                      \
    struct SkCFNumberTypeFor<c> : std::integral_constant<CFNumberType, cf> {};
...
template <typename T>
static bool SkCFNumberDynamicCast(
    CFTypeRef cf, T* number, CFNumberRef* cfNumber, char const* name) {
...
SkUniqueCFRef<CTFontRef> SkCTFontCreateExactCopy(
    CTFontRef baseFont, CGFloat textSize, OpszVariation opszVariation) {
...
sk_sp<SkTypeface> SkTypeface_Mac::Make(
  ...
  auto makeTypeface = [&]() {
    ...
    return sk_sp<SkTypeface>(new SkTypeface_Mac(
...
sk_sp<SkTypeface> SkMakeTypefaceFromCTFont(CTFontRef font) {
  CFRetain(font);
  return SkTypeface_Mac::Make(SkUniqueCFRef<CTFontRef>(font), OpszVariation(), nullptr);
}
...
void SkTypeface_Mac::getGlyphToUnicodeMap(SkUnichar* dstArray) const {
  SkUniqueCFRef<CTFontRef> ctFont =
      SkCTFontCreateExactCopy(fFontRef.get(), CTFontGetUnitsPerEm(fFontRef.get()), fOpszVariation);
...
std::unique_ptr<SkAdvancedTypefaceMetrics> SkTypeface_Mac::onGetAdvancedMetrics() const {
  ...
  SkRect r;
  r.setLTRB(
      SkScalarFromCGFloat(SkCGRectGetMinX(bbox)),   // Left
...
CFArrayRef SkTypeface_Mac::getVariationAxes() const {
  fInitVariationAxes([this] { fVariationAxes.reset(CTFontCopyVariationAxes(fFontRef.get())); });
...
CTFontVariation SkCTVariationFromSkFontArguments(
    CTFontRef ct, CFArrayRef ctAxes, const SkFontArguments& args) {
  OpszVariation opsz;
  ...
  SkUniqueCFRef<CFMutableDictionaryRef> newCtVariation(CFDictionaryCreateMutable(
  ...
  for (int i = 0; i < axisCount; ++i) {
    ...
    if (tagLong == opszTag) {
      ...
      if (haveCurrentDouble && value == currentDouble) {
        ...
        wrongOpszVariation.reset(CFDictionaryCreateMutable(
  ...
  return {
      SkUniqueCFRef<CFDictionaryRef>(std::move(newCtVariation)),
      SkUniqueCFRef<CFDictionaryRef>(std::move(wrongOpszVariation)), opsz};
}

sk_sp<SkTypeface> SkTypeface_Mac::onMakeClone(const SkFontArguments& args) const {
  CTFontVariation ctVariation =
      SkCTVariationFromSkFontArguments(fFontRef.get(), this->getVariationAxes(), args);
  ...
  if (ctVariation.variation) {
    ...
    if (ctVariation.wrongOpszVariation) {
      ...
      CFDictionarySetValue(
          attributes.get(), kCTFontVariationAttribute, ctVariation.wrongOpszVariation.get());
    ...
    CFDictionarySetValue(attributes.get(), kCTFontVariationAttribute, ctVariation.variation.get());
    SkUniqueCFRef<CTFontDescriptorRef> varDesc(
        CTFontDescriptorCreateWithAttributes(attributes.get()));
