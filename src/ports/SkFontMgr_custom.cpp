...
SkTypeface_Custom::SkTypeface_Custom(
    const SkFontStyle& style, bool isFixedPitch, bool sysFont, const SkString familyName, int index)
    : INHERITED(style, isFixedPitch), fIsSysFont(sysFont), fFamilyName(familyName), fIndex(index) {}

bool SkTypeface_Custom::isSysFont() const { return fIsSysFont; }

void SkTypeface_Custom::onGetFamilyName(SkString* familyName) const { *familyName = fFamilyName; }
...
SkTypeface_Stream::SkTypeface_Stream(
    std::unique_ptr<SkFontData> fontData, const SkFontStyle& style, bool isFixedPitch, bool sysFont,
...
SkTypeface_File::SkTypeface_File(
    ...
    : INHERITED(style, isFixedPitch, sysFont, familyName, index), fPath(path) {}
...
int SkFontStyleSet_Custom::count() { return fStyles.count(); }
...
SkFontMgr_Custom::SkFontMgr_Custom(const SystemFontLoader& loader) : fDefaultFamily(nullptr) {
  ...
  static const char* defaultNames[] = {"Arial", "Verdana", "Times New Roman", "Droid Sans",
                                       nullptr};
...
int SkFontMgr_Custom::onCountFamilies() const { return fFamilies.count(); }
...
sk_sp<SkTypeface> SkFontMgr_Custom::onMakeFromStreamArgs(
...
sk_sp<SkTypeface> SkFontMgr_Custom::onMakeFromFontData(std::unique_ptr<SkFontData> data) const {
  ...
  SkString name;
  if (!fScanner.scanFont(
          data->getStream(), data->getIndex(), &name, &style, &isFixedPitch, nullptr)) {
    return nullptr;
  }
  return sk_sp<SkTypeface>(
      new SkTypeface_Stream(std::move(data), style, isFixedPitch, false, name));
...
sk_sp<SkTypeface> SkFontMgr_Custom::onLegacyMakeTypeface(
