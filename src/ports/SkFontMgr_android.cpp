...
class SkTypeface_Android : public SkTypeface_FreeType {
 public:
  SkTypeface_Android(const SkFontStyle& style, bool isFixedPitch, const SkString& familyName)
      : INHERITED(style, isFixedPitch), fFamilyName(familyName) {}
...
class SkTypeface_AndroidSystem : public SkTypeface_Android {
 public:
  SkTypeface_AndroidSystem(
      const SkString& pathName, const bool cacheFontFiles, int index, const SkFixed* axes,
      int axesCount, const SkFontStyle& style, bool isFixedPitch, const SkString& familyName,
      const SkTArray<SkLanguage, true>& lang, FontVariant variantStyle)
  ...
  sk_sp<SkTypeface> onMakeClone(const SkFontArguments& args) const override {
    ...
    return sk_make_sp<SkTypeface_AndroidSystem>(
        fPathName, fFile, fIndex, data->getAxis(), data->getAxisCount(), this->fontStyle(),
        this->isFixedPitch(), fFamilyName, fLang, fVariantStyle);
  }
...
class SkTypeface_AndroidStream : public SkTypeface_Android {
 public:
  SkTypeface_AndroidStream(
      std::unique_ptr<SkFontData> data, const SkFontStyle& style, bool isFixedPitch,
  ...
  sk_sp<SkTypeface> onMakeClone(const SkFontArguments& args) const override {
    ...
    return sk_make_sp<SkTypeface_AndroidStream>(
        std::move(data), this->fontStyle(), this->isFixedPitch(), fFamilyName);
  }
...
class SkFontStyleSet_Android : public SkFontStyleSet {
 ...
 public:
  explicit SkFontStyleSet_Android(
      const FontFamily& family, const Scanner& scanner, const bool cacheFontFiles) {
    ...
    for (int i = 0; i < family.fFonts.count(); ++i) {
      ...
      if (!scanner.scanFont(
              stream.get(), ttcIndex, &familyName, &style, &isFixedWidth, &axisDefinitions)) {
      ...
      fStyles.push_back().reset(new SkTypeface_AndroidSystem(
          ...
          isFixedWidth, familyName, family.fLanguages, variant));
    }
  }

  int count() override { return fStyles.count(); }
...
class SkFontMgr_Android : public SkFontMgr {
 ...
 protected:
   ...
   */
  int onCountFamilies() const override { return fNameToFamilyMap.count(); }
      ...
      for (int elegant = 2; elegant-- > 0;) {
        for (int bcp47Index = bcp47Count; bcp47Index-- > 0;) {
          ...
          while (!lang.getTag().isEmpty()) {
            sk_sp<SkTypeface_AndroidSystem> matchingTypeface = find_family_style_character(
                currentFamilyName, fFallbackNameToFamilyMap, style, SkToBool(elegant),
        ...
        sk_sp<SkTypeface_AndroidSystem> matchingTypeface = find_family_style_character(
            currentFamilyName, fFallbackNameToFamilyMap, style, SkToBool(elegant), SkString(),
            character);
        if (matchingTypeface) {
          return matchingTypeface.release();
        }
...
#ifdef SK_DEBUG
static char const* const gSystemFontUseStrings[] = {"OnlyCustom", "PreferCustom", "PreferSystem"};
#endif
...
sk_sp<SkFontMgr> SkFontMgr_New_Android(const SkFontMgr_Android_CustomFonts* custom) {
  if (custom) {
    ...
    SkDEBUGF(
        ...
        gSystemFontUseStrings[custom->fSystemFontUse], custom->fBasePath, custom->fFontsXml,
