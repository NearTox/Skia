...
class StreamFontFileLoader : public IDWriteFontFileLoader {
 public:
  ...
  static HRESULT Create(
      std::unique_ptr<SkStreamAsset> stream, StreamFontFileLoader** streamFontFileLoader) {
...
SK_STDMETHODIMP StreamFontFileEnumerator::MoveNext(BOOL* hasCurrentFile) {
  *hasCurrentFile = FALSE;
  ...
  HR(fFactory->CreateCustomFontFileReference(
      ...
      fFontFileLoader.get(), &fCurrentFile));
...
class SkFontMgr_DirectWrite : public SkFontMgr {
 public:
  ...
  SkFontMgr_DirectWrite(
      IDWriteFactory* factory, IDWriteFontCollection* fontCollection, IDWriteFontFallback* fallback,
      const WCHAR* localeName, int localeNameLength, const WCHAR* defaultFamilyName,
 ...
 private:
  ...
  sk_sp<SkTypeface> fallback(
      const WCHAR* dwFamilyName, DWriteStyle, const WCHAR* dwBcp47, UINT32 character) const;
  sk_sp<SkTypeface> layoutFallback(
      const WCHAR* dwFamilyName, DWriteStyle, const WCHAR* dwBcp47, UINT32 character) const;

  /** Creates a typeface using a typeface cache. */
  sk_sp<SkTypeface> makeTypefaceFromDWriteFont(
...
SkTypeface* SkFontMgr_DirectWrite::onMatchFamilyStyle(
 ...
 public:
  ...
  SK_STDMETHODIMP DrawGlyphRun(
    ...
    if (exists) {
      ...
      fResolvedTypeface =
          fOuter->makeTypefaceFromDWriteFont(glyphRun->fontFace, font.get(), fontFamily.get());
    }
...
class FontFallbackSource : public IDWriteTextAnalysisSource {
 public:
  FontFallbackSource(
      const WCHAR* string, UINT32 length, const WCHAR* locale,
...
SkTypeface* SkFontMgr_DirectWrite::onMatchFamilyStyleCharacter(
...
sk_sp<SkTypeface> SkFontMgr_DirectWrite::fallback(
    const WCHAR* dwFamilyName, DWriteStyle dwStyle, const WCHAR* dwBcp47, UINT32 character) const {
  WCHAR str[16];
  ...
  HRNM(
      fFactory->CreateNumberSubstitution(
  ...
  FLOAT scale;
  HRNM(
      fFontFallback->MapCharacters(
          fontFallbackSource.get(),
          0,  // textPosition,
          strLen, fFontCollection.get(), dwFamilyName, dwStyle.fWeight, dwStyle.fSlant,
          dwStyle.fWidth, &mappedLength, &font, &scale),
...
sk_sp<SkTypeface> SkFontMgr_DirectWrite::layoutFallback(
  ...
  HRNM(
      fFactory->CreateTextFormat(
          dwFamilyName ? dwFamilyName : L"", fFontCollection.get(), dwStyle.fWeight, dwStyle.fSlant,
          dwStyle.fWidth, 72.0f, dwBcp47, &fallbackFormat),
  ...
  HRNM(
      fallbackLayout->SetFontCollection(fFontCollection.get(), {0, strLen}),
...
SkTypeface* SkFontMgr_DirectWrite::onMatchFaceStyle(
    const SkTypeface* familyMember, const SkFontStyle& fontstyle) const {
  SkString familyName;
...
static HRESULT apply_fontargument_variation(
    SkTScopedComPtr<IDWriteFontFace>& fontFace, const SkFontArguments& args) {
...
sk_sp<SkTypeface> SkFontMgr_DirectWrite::onMakeFromStreamArgs(
    ...
    for (UINT32 fontIndex = 0; fontIndex < fontCount; ++fontIndex) {
      ...
      return DWriteFontTypeface::Make(
          ...
          sk_make_sp<DWriteFontTypeface::Loaders>(
              fFactory.get(), autoUnregisterFontFileLoader.detatch(),
...
static HRESULT apply_fontdata_variation(
    SkTScopedComPtr<IDWriteFontFace>& fontFace, const std::unique_ptr<SkFontData>& data) {
#  if defined(NTDDI_WIN10_RS3) && NTDDI_VERSION >= NTDDI_WIN10_RS3
  SkTScopedComPtr<IDWriteFontFace5> fontFace5;
  if (FAILED(fontFace->QueryInterface(&fontFace5)) || !fontFace5->HasVariations()) {
    return S_OK;
  ...
  for (UINT32 i = 0; i < fontAxisCount; ++i) {
    if (fontResource->GetFontAxisAttributes(i) & DWRITE_FONT_AXIS_ATTRIBUTES_VARIABLE) {
      ...
      coordinate.value = SkTPin(
          SkFixedToFloat(data->getAxis()[variableAxisIndex]), axisRange.minValue,
          axisRange.maxValue);
      ++variableAxisIndex;
    }
  }
...
sk_sp<SkTypeface> SkFontMgr_DirectWrite::onMakeFromFontData(
    std::unique_ptr<SkFontData> data) const {
  if (!data->hasStream()) {
    return nullptr;
  }
...
HRESULT SkFontMgr_DirectWrite::getByFamilyName(
    const WCHAR wideFamilyName[], IDWriteFontFamily** fontFamily) const {
  UINT32 index;
  BOOL exists;
...
sk_sp<SkTypeface> SkFontMgr_DirectWrite::onLegacyMakeTypeface(
  ...
  if (!fontFamily) {
    ...
    HRNM(
        this->getByFamilyName(fDefaultFamilyName, &fontFamily),
  ...
  if (!fontFamily) {
    ...
    HRNM(
        fFontCollection->GetFontFamily(0, &fontFamily),
        "Could not get default-default font family.");
  }

  SkTScopedComPtr<IDWriteFont> font;
  HRNM(
      fontFamily->GetFirstMatchingFont(dwStyle.fWeight, dwStyle.fWidth, dwStyle.fSlant, &font),
...
SkTypeface* SkFontStyleSet_DirectWrite::matchStyle(const SkFontStyle& pattern) {
  ...
  HRNM(
      fFontFamily->GetFirstMatchingFont(dwStyle.fWeight, dwStyle.fWidth, dwStyle.fSlant, &font),
...
SK_API sk_sp<SkFontMgr> SkFontMgr_New_DirectWrite(
    IDWriteFactory* factory, IDWriteFontCollection* collection, IDWriteFontFallback* fallback) {
  if (nullptr == factory) {
  ...
  return sk_make_sp<SkFontMgr_DirectWrite>(
      factory, collection, fallback, localeName, localeNameLen, defaultFamilyName,
