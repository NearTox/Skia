...
bool HasBitmapStrikes(const SkTScopedComPtr<IDWriteFont>& font) {
  ...
  AutoDWriteTable ebdtTable(
      fontFace.get(), SkEndian_SwapBE32(SkSetFourByteTag('E', 'B', 'D', 'T')));
...
HRESULT FirstMatchingFontWithoutSimulations(
    const SkTScopedComPtr<IDWriteFontFamily>& family, DWriteStyle dwStyle,
    SkTScopedComPtr<IDWriteFont>& font) {
  bool noSimulations = false;
  while (!noSimulations) {
    SkTScopedComPtr<IDWriteFont> searchFont;
    HR(family->GetFirstMatchingFont(dwStyle.fWeight, dwStyle.fWidth, dwStyle.fSlant, &searchFont));
...
sk_sp<SkTypeface> SkFontMgr_DirectWrite::makeTypefaceFromDWriteFont(
  ...
  if (nullptr == face) {
    face = DWriteFontTypeface::Make(
        fFactory.get(), fontFace, font, fontFamily, nullptr,
 ...
 public:
  ...
  SK_STDMETHODIMP DrawGlyphRun(
    ...
    if (exists) {
      ...
      fHasSimulations =
          (font->GetSimulations() != DWRITE_FONT_SIMULATIONS_NONE) && !HasBitmapStrikes(font);
    }
  ...
  while (!noSimulations) {
    ...
    HRNM(
        fFactory->CreateTextFormat(
            dwFamilyName ? dwFamilyName : L"", fFontCollection.get(), dwStyle.fWeight,
            dwStyle.fSlant, dwStyle.fWidth, 72.0f, dwBcp47, &fallbackFormat),
...
sk_sp<SkTypeface> SkFontMgr_DirectWrite::onLegacyMakeTypeface(
  ...
  SkTScopedComPtr<IDWriteFont> font;
  HRNM(
      FirstMatchingFontWithoutSimulations(fontFamily, dwStyle, font), "No font found from family.");
...
SkTypeface* SkFontStyleSet_DirectWrite::matchStyle(const SkFontStyle& pattern) {
  ...
  HRNM(
      FirstMatchingFontWithoutSimulations(fFontFamily, dwStyle, font),
