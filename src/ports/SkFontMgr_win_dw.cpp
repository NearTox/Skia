    ...
    for (UINT32 fontIndex = 0; fontIndex < fontCount; ++fontIndex) {
      ...
      if (faceIndex == ttcIndex) {
        return DWriteFontTypeface::Make(
            ...
            autoUnregisterFontFileLoader.detatch(), autoUnregisterFontCollectionLoader.detatch());
      }
    }
  }
...
#  if defined(NTDDI_WIN10_RS3) && NTDDI_VERSION >= NTDDI_WIN10_RS3
      ...
      if (SUCCEEDED(fontFace->QueryInterface(&fontFace5)) && fontFace5->HasVariations()) {
        ...
        HRN(fontResource->CreateFontFace(
            DWRITE_FONT_SIMULATIONS_NONE, fontAxisValues.get(), fontAxisCount, &fontFace5_Out));
        fontFace.reset();
