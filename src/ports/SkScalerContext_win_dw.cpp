...
SkScalerContext_DW::SkScalerContext_DW(
    ...
    : SkScalerContext(std::move(typefaceRef), effects, desc), fGlyphCount(-1) {
...
bool SkScalerContext_DW::generateAdvance(SkGlyph* glyph) {
  ...
  if (DWRITE_MEASURING_MODE_GDI_CLASSIC == fMeasuringMode ||
    ...
    Exclusive l(DWriteFactoryMutex);
    HRBM(
        this->getDWriteTypeface()->fDWriteFontFace->GetGdiCompatibleGlyphMetrics(
            fTextSizeMeasure,
            1.0f,  // pixelsPerDip
    ...
    Exclusive l(DWriteFactoryMutex);
    HRBM(
        this->getDWriteTypeface()->fDWriteFontFace->GetDesignGlyphMetrics(&glyphId, 1, &gm),
...
void SkScalerContext_DW::generateColorGlyphImage(const SkGlyph& glyph) {
  ...
  SkDraw draw;
  draw.fDst = SkPixmap(
      SkImageInfo::MakeN32(glyph.fWidth, glyph.fHeight, kPremul_SkAlphaType), glyph.fImage,
