...
class SK_SCOPED_CAPABILITY Exclusive {
 public:
  explicit Exclusive(SkSharedMutex* maybe_lock) SK_ACQUIRE(*maybe_lock) : fLock(maybe_lock) {
    if (fLock) {
      fLock->acquire();
    }
  }
...
class SK_SCOPED_CAPABILITY Shared {
 public:
  explicit Shared(SkSharedMutex* maybe_lock) SK_ACQUIRE_SHARED(*maybe_lock) : fLock(maybe_lock) {
    if (fLock) {
      fLock->acquireShared();
    }
  }
...
static bool has_bitmap_strike(DWriteFontTypeface* typeface, GaspRange range) {
  Exclusive l(maybe_dw_mutex(*typeface));
  {
    ...
    for (uint32_t i = 0; i < numSizes; ++i, ++sizeTable) {
      if (sizeTable->ppemX == sizeTable->ppemY && range.fMin <= sizeTable->ppemX &&
          sizeTable->ppemX <= range.fMax) {
...

  {
    ...
    for (uint32_t i = 0; i < numSizes; ++i, ++scaleTable) {
      if (scaleTable->ppemX == scaleTable->ppemY && range.fMin <= scaleTable->ppemX &&
          scaleTable->ppemX <= range.fMax) {
...
static bool both_zero(SkScalar a, SkScalar b) { return 0 == a && 0 == b; }

// returns false if there is any non-90-rotation or skew
static bool is_axis_aligned(const SkScalerContextRec& rec) {
  return 0 == rec.fPreSkewX && (both_zero(rec.fPost2x2[0][1], rec.fPost2x2[1][0]) ||
...
SkScalerContext_DW::SkScalerContext_DW(
    sk_sp<DWriteFontTypeface> typefaceRef, const SkScalerContextEffects& effects,
  ...
  fIsColorFont = typeface->fFactory2 && typeface->fDWriteFontFace2 &&
  ...
  if (SkMask::kBW_Format == fRec.fMaskFormat) {
    ...
    // If the font has a gasp table version 1, use it to determine symmetric rendering.
      ...
      get_gasp_range(typeface, SkScalarRoundToInt(gdiTextSize), &range) && range.fVersion >= 1) {
    fTextSizeRender = realTextSize;
    fRenderingMode = range.fFlags.field.SymmetricSmoothing ? DWRITE_RENDERING_MODE_NATURAL_SYMMETRIC
  ...
  if (typeface->fFactory2 && typeface->fDWriteFontFace2 && SkMask::kA8_Format == fRec.fMaskFormat &&
...
bool SkScalerContext_DW::generateAdvance(SkGlyph* glyph) {
  ...
  if (DWRITE_MEASURING_MODE_GDI_CLASSIC == fMeasuringMode ||
    ...
    Exclusive l(maybe_dw_mutex(*typeface));
    HRBM(
        typeface->fDWriteFontFace->GetGdiCompatibleGlyphMetrics(
            fTextSizeMeasure,
            1.0f,  // pixelsPerDip
            ...
            nullptr, DWRITE_MEASURING_MODE_GDI_NATURAL == fMeasuringMode, &glyphId, 1, &gm),
        "Could not get gdi compatible glyph metrics.");
  } else {
    Exclusive l(maybe_dw_mutex(*typeface));
    HRBM(
        typeface->fDWriteFontFace->GetDesignGlyphMetrics(&glyphId, 1, &gm),
...
HRESULT SkScalerContext_DW::getBoundingBox(
    SkGlyph* glyph, DWRITE_RENDERING_MODE renderingMode, DWRITE_TEXTURE_TYPE textureType,
    RECT* bbox) {
  DWriteFontTypeface* typeface = this->getDWriteTypeface();
  ...
  SkTScopedComPtr<IDWriteGlyphRunAnalysis> glyphRunAnalysis;
  {
    ...
    if (typeface->fFactory2 && (fGridFitMode == DWRITE_GRID_FIT_MODE_DISABLED ||
                                fAntiAliasMode == DWRITE_TEXT_ANTIALIAS_MODE_GRAYSCALE)) {
      HRM(typeface->fFactory2->CreateGlyphRunAnalysis(
              &run, &fXform, renderingMode, fMeasuringMode, fGridFitMode, fAntiAliasMode,
              0.0f,  // baselineOriginX,
              0.0f,  // baselineOriginY,
              &glyphRunAnalysis),
          "Could not create DW2 glyph run analysis.");
    } else {
      HRM(typeface->fFactory->CreateGlyphRunAnalysis(
              &run,
              1.0f,  // pixelsPerDip,
              &fXform, renderingMode, fMeasuringMode,
              0.0f,  // baselineOriginX,
              0.0f,  // baselineOriginY,
              &glyphRunAnalysis),
...
bool SkScalerContext_DW::isPngGlyph(const SkGlyph& glyph) {
  ...
  HRBM(
      fontFace4->GetGlyphImageFormats(glyph.getGlyphID(), 0, UINT32_MAX, &f),
      "Cannot get glyph image formats.");
  return f & DWRITE_GLYPH_IMAGE_FORMATS_PNG;
}

bool SkScalerContext_DW::getColorGlyphRun(
    const SkGlyph& glyph, IDWriteColorGlyphRunEnumerator** colorGlyph) {
  FLOAT advance = 0;
  UINT16 glyphId = glyph.getGlyphID();
        ...
        "Could not create geometry to path converter.");
    {
      ...
      HRVM(
          colorGlyph->glyphRun.fontFace->GetGlyphRunOutline(
              colorGlyph->glyphRun.fontEmSize, colorGlyph->glyphRun.glyphIndices,
              colorGlyph->glyphRun.glyphAdvances, colorGlyph->glyphRun.glyphOffsets,
              colorGlyph->glyphRun.glyphCount, colorGlyph->glyphRun.isSideways,
...
struct Context {
  ...
  Context(IDWriteFontFace4* face4, void* context)
      : fontFace4(SkRefComPtr(face4)), glyphDataContext(context) {}
};
...
void SkScalerContext_DW::generatePngMetrics(SkGlyph* glyph) {
  ...
  HRVM(
      fontFace4->GetGlyphImageData(
          glyph->getGlyphID(), fTextSizeRender, DWRITE_GLYPH_IMAGE_FORMATS_PNG, &glyphData,
          &glyphDataContext),
      "Glyph image data could not be acquired.");

  Context* context = new Context(fontFace4, glyphDataContext);
  sk_sp<SkData> data =
      SkData::MakeWithProc(glyphData.imageData, glyphData.imageDataSize, &ReleaseProc, context);
  ...
  SkRect bounds = SkRect::MakeLTRB(
      SkIntToScalar(info.bounds().fLeft), SkIntToScalar(info.bounds().fTop),
      SkIntToScalar(info.bounds().fRight), SkIntToScalar(info.bounds().fBottom));
...
void SkScalerContext_DW::generateMetrics(SkGlyph* glyph) {
  ...
  auto glyphCheckAndSetBounds = [](SkGlyph* glyph, const RECT& bbox) {
    ...
    if (!SkIRect::MakeXYWH(-32767, -32767, 65535, 65535)
             .contains(SkIRect::MakeLTRB(bbox.left, bbox.top, bbox.right, bbox.bottom))) {
      return false;
    }
  ...
  if (DWRITE_TEXTURE_ALIASED_1x1 != fTextureType ||
    ...
    HRVM(
        this->getBoundingBox(
            glyph, DWRITE_RENDERING_MODE_ALIASED, DWRITE_TEXTURE_ALIASED_1x1, &bbox),
...
void SkScalerContext_DW::generateFontMetrics(SkFontMetrics* metrics) {
  ...
  metrics->fStrikeoutThickness =
      fTextSizeRender * SkIntToScalar(dwfm.strikethroughThickness) / upem;
  metrics->fStrikeoutPosition =
      -(fTextSizeRender * SkIntToScalar(dwfm.strikethroughPosition) / upem);
  ...
  if (head.fExists && head.fSize >= sizeof(SkOTTableHead) &&
...
void SkScalerContext_DW::GrayscaleToA8(
...
template <bool APPLY_PREBLEND>
void SkScalerContext_DW::RGBToA8(
    const uint8_t* SK_RESTRICT src, const SkGlyph& glyph, const uint8_t* table8) {
  const size_t dstRB = glyph.rowBytes();
  const int width = glyph.width();
  uint8_t* SK_RESTRICT dst = static_cast<uint8_t*>(glyph.fImage);
...
void SkScalerContext_DW::RGBToLcd16(
...
const void* SkScalerContext_DW::drawDWMask(
  ...
  run.glyphOffsets = &offset;
  {
    SkTScopedComPtr<IDWriteGlyphRunAnalysis> glyphRunAnalysis;
    {
      ...
      if (this->getDWriteTypeface()->fFactory2 &&
        ...
        HRNM(
            this->getDWriteTypeface()->fFactory->CreateGlyphRunAnalysis(
                &run,
                1.0f,  // pixelsPerDip,
                &fXform, renderingMode, fMeasuringMode,
                0.0f,  // baselineOriginX,
    ...
    bbox.bottom = glyph.top() + glyph.height();
    {
      Shared l(maybe_dw_mutex(*typeface));
      HRNM(
          glyphRunAnalysis->CreateAlphaTexture(textureType, &bbox, fBits.begin(), sizeNeeded),
          "Could not draw mask.");
    }
  }
  return fBits.begin();
...
void SkScalerContext_DW::generateColorGlyphImage(const SkGlyph& glyph) {
  ...
  SkDraw draw;
  draw.fDst = SkPixmap(
      SkImageInfo::MakeN32(glyph.width(), glyph.height(), kPremul_SkAlphaType), glyph.fImage,
  ...
  while (SUCCEEDED(colorLayers->MoveNext(&hasNextRun)) && hasNextRun) {
    ...
    if (colorGlyph->paletteIndex != 0xffff) {
      color = SkColorSetARGB(
          sk_float_round2int(colorGlyph->runColor.a * 255),
...
bool SkScalerContext_DW::generatePath(SkGlyphID glyph, SkPath* path) {
  ...
  UINT16 glyphId = SkTo<UINT16>(glyph);
  {
    ...
    HRBM(
        this->getDWriteTypeface()->fDWriteFontFace->GetGlyphRunOutline(
            SkScalarToFloat(fTextSizeRender), &glyphId,
            nullptr,  // advances
            nullptr,  // offsets
            1,        // num glyphs
            FALSE,    // sideways
