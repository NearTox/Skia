...
class SkTypeface_Mac : public SkTypeface {
 ...
 protected:
  ...
  sk_sp<SkTypeface> onMakeClone(const SkFontArguments&) const override { return nullptr; }
...
std::unique_ptr<SkFontData> SkTypeface_Mac::onMakeFontData() const {
  ...
  if (get_variations(fFontRef.get(), &cgAxisCount, &axisValues)) {
    return skstd::make_unique<SkFontData>(std::move(stream), index, axisValues.get(), cgAxisCount);
  }
...
#  endif
  ...
  static CTFontVariation ctvariation_from_skfontarguments(
      CTFontRef ct, const SkFontArguments& args) {
    OpszVariation opsz;
