...
class SkTypeface_Mac : public SkTypeface {
 public:
  SkTypeface_Mac(
      SkUniqueCFRef<CTFontRef> fontRef, const SkFontStyle& fs, bool isFixedPitch,
...
static sk_sp<SkTypeface> create_from_CTFontRef(
    SkUniqueCFRef<CTFontRef> font, OpszVariation opszVariation,
  ...
  sk_sp<SkTypeface> face(new SkTypeface_Mac(
      std::move(font), style, isFixedPitch, opszVariation, std::move(providedData)));
  if (!isFromStream) {
...
sk_sp<SkTypeface> SkMakeTypefaceFromCTFont(CTFontRef font) {
  CFRetain(font);
  return create_from_CTFontRef(SkUniqueCFRef<CTFontRef>(font), OpszVariation(), nullptr);
}
...
#  if !defined(SK_IGNORE_MAC_OPSZ_FORCE)
    ...
    double opsz_val;
    if (!opsz || CFGetTypeID(opsz.get()) != CFNumberGetTypeID() ||
...
#  endif
  ...
  sk_sp<SkTypeface> onMakeFromData(sk_sp<SkData> data, int ttcIndex) const override {
    ...
    return create_from_CTFontRef(
        std::move(ct), OpszVariation(), SkMemoryStream::Make(std::move(data)));
  }
  ...
  sk_sp<SkTypeface> onMakeFromStreamArgs(
    ...
    return create_from_CTFontRef(std::move(ctVariant), ctVariation.opsz, std::move(stream));
  }
  ...
  sk_sp<SkTypeface> onMakeFromFontData(std::unique_ptr<SkFontData> fontData) const override {
    ...
    return create_from_CTFontRef(std::move(ctVariant), ctVariation.opsz, fontData->detachStream());
  }
