...
static SmoothBehavior smooth_behavior() {
  static SmoothBehavior gSmoothBehavior = [] {
    ...
    SkUniqueCFRef<CGDataProviderRef> data(CGDataProviderCreateWithData(
        nullptr, kSpiderSymbol_ttf, SK_ARRAY_COUNT(kSpiderSymbol_ttf), nullptr));
...
class SkTypeface_Mac : public SkTypeface {
 public:
  SkTypeface_Mac(
      ...
      bool isFixedPitch, std::unique_ptr<SkStreamAsset> providedData)
...
static sk_sp<SkTypeface> create_from_CTFontRef(
    SkUniqueCFRef<CTFontRef> font, SkUniqueCFRef<CFTypeRef> resource,
  ...
  sk_sp<SkTypeface> face(new SkTypeface_Mac(
      std::move(font), std::move(resource), style, isFixedPitch, std::move(providedData)));
  if (!isFromStream) {
...
SkTypeface* SkCreateTypefaceFromCTFont(CTFontRef font, CFTypeRef resource) {
  ...
  return create_from_CTFontRef(
             SkUniqueCFRef<CTFontRef>(font), SkUniqueCFRef<CFTypeRef>(resource), nullptr)
      .release();
}
...
static SkUniqueCFRef<CTFontRef> ctfont_create_exact_copy(
    CTFontRef baseFont, CGFloat textSize, const CGAffineTransform* transform) {
...
static sk_sp<SkTypeface> create_from_dataProvider(
    SkUniqueCFRef<CGDataProviderRef> provider, std::unique_ptr<SkStreamAsset> providedData,
    int ttcIndex) {
  if (ttcIndex != 0) {
    return nullptr;
  }
...
static SkUniqueCFRef<CFDictionaryRef> ct_variation_from_cg_variation(
  ...
  SkUniqueCFRef<CFMutableDictionaryRef> ctVariations(CFDictionaryCreateMutable(
      kCFAllocatorDefault, 0, &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks));
...
#  endif
  ...
  sk_sp<SkTypeface> onMakeFromData(sk_sp<SkData> data, int ttcIndex) const override {
    ...
    return create_from_dataProvider(std::move(pr), SkMemoryStream::Make(std::move(data)), ttcIndex);
  }
  ...
  sk_sp<SkTypeface> onMakeFromStreamArgs(
    ...
    SkUniqueCFRef<CTFontRef> ct(CTFontCreateWithGraphicsFont(cgVariant.get(), 0, nullptr, nullptr));
    if (!ct) {
      return nullptr;
    }
