...
SkUniqueCFRef<CFArrayRef> SkCopyAvailableFontFamilyNames(CTFontCollectionRef collection) {
  ...
  SkUniqueCFRef<CFArrayRef> descriptors(CTFontCollectionCreateMatchingFontDescriptors(collection));
  ...
  CFArrayApplyFunction(
      descriptors.get(), CFRangeMake(0, CFArrayGetCount(descriptors.get())),
...
#  ifdef SK_BUILD_FOR_IOS
  ...
  *(void**)(&ctFontManagerCopyAvailableFontFamilyNames) =
  ...
  SkUniqueCFRef<CTFontCollectionRef> collection(CTFontCollectionCreateFromAvailableFonts(nullptr));
...
class SkFontMgr_Mac : public SkFontMgr {
 ...
 public:
  ...
  SkFontMgr_Mac(CTFontCollectionRef fontCollection)
      : fNames(
            fontCollection ? SkCopyAvailableFontFamilyNames(fontCollection)
                           : SkCTFontManagerCopyAvailableFontFamilyNames()),
        fCount(fNames ? SkToInt(CFArrayGetCount(fNames.get())) : 0),
        fFontCollection(
            fontCollection ? (CTFontCollectionRef)CFRetain(fontCollection)
...
#  endif
  ...
  sk_sp<SkTypeface> onMakeFromFontData(std::unique_ptr<SkFontData> fontData) const override {
    ...
    return SkTypeface_Mac::Make(std::move(ctVariant), ctVariation.opsz, fontData->detachStream());
  }
