...
bool SkEncodeImageWithNDK(
    SkWStream* stream, const SkPixmap& pmap, SkEncodedImageFormat format, int quality) {
  ...
  switch (pmap.alphaType()) {
    case kPremul_SkAlphaType: info.flags = ANDROID_BITMAP_FLAGS_ALPHA_PREMUL; break;
    case kOpaque_SkAlphaType: info.flags = ANDROID_BITMAP_FLAGS_ALPHA_OPAQUE; break;
    case kUnpremul_SkAlphaType: info.flags = ANDROID_BITMAP_FLAGS_ALPHA_UNPREMUL; break;
    default: return false;
  }

  AndroidBitmapCompressFormat androidFormat;
  switch (format) {
    case SkEncodedImageFormat::kJPEG: androidFormat = ANDROID_BITMAP_COMPRESS_FORMAT_JPEG; break;
    case SkEncodedImageFormat::kPNG: androidFormat = ANDROID_BITMAP_COMPRESS_FORMAT_PNG; break;
  ...
  return ANDROID_BITMAP_RESULT_SUCCESS ==
         AndroidBitmap_compress(
             &info, SkNDKConversions::toDataSpace(pmap.colorSpace()), pmap.addr(), androidFormat,
             quality, reinterpret_cast<void*>(stream), write_to_stream);
}
