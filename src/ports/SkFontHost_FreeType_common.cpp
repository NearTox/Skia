...
struct OpaquePaintHasher {
  size_t operator()(const FT_OpaquePaint& opaque_paint) {
    return SkGoodHash()(opaque_paint.p) ^ SkGoodHash()(opaque_paint.insert_root_transform);
  }
};
...
void colrv1_configure_skpaint(
    FT_Face face, const FT_Color* palette, FT_COLR_Paint colrv1_paint, SkPaint* paint) {
  auto fetch_color_stops = [&face, &palette](
    ...
    while (FT_Get_Colorline_Stops(face, &color_stop, &color_stop_iterator)) {
      ...
      sorted_stops[index].color = SkColorSetARGB(
          ...
          palette[palette_index].red, palette[palette_index].green, palette[palette_index].blue);
    }

    std::stable_sort(
        sorted_stops.begin(), sorted_stops.end(),
  ...
  switch (colrv1_paint.format) {
    case FT_COLR_PAINTFORMAT_SOLID: {
      ...
      SkColor color = SkColorSetARGB(
          palette[solid.color.palette_index].alpha * SkColrV1AlphaToFloat(solid.color.alpha),
          palette[solid.color.palette_index].red, palette[solid.color.palette_index].green,
    ...
    case FT_COLR_PAINTFORMAT_LINEAR_GRADIENT: {
      ...
      // to compute a new gradient end point as the orthogonal projection of the vector from p0 to
      // p1 onto a line perpendicular to line p0p2 and passing through p0.
      SkVector perpendicular_to_p2_p0 = (p2 - p0);
      perpendicular_to_p2_p0 =
          SkPoint::Make(perpendicular_to_p2_p0.y(), -perpendicular_to_p2_p0.x());
    ...
    case FT_COLR_PAINTFORMAT_SWEEP_GRADIENT: {
      ...
      auto clampAngleToRange = [](SkScalar angle) {
        ...
        if (clamped_angle < 0) return clamped_angle + 360.f;
      ...
      if (startAngle >= endAngle) endAngle += 360.f;
      ...
      paint->setShader(SkGradientShader::MakeSweep(
          center.x(), center.y(), colors.data(), stops.data(), stops.size(), SkTileMode::kDecal,
          startAngle, endAngle, 0, &angle_adjust));
      break;
    }
    default: {
...
void colrv1_draw_paint(
    SkCanvas* canvas, const FT_Color* palette, FT_Face face, FT_COLR_Paint colrv1_paint) {
  SkPaint paint;
...
void colrv1_draw_glyph_with_path(
    SkCanvas* canvas, const FT_Color* palette, FT_Face face, FT_COLR_Paint glyphPaint,
...
void colrv1_transform(
    FT_Face face, FT_COLR_Paint colrv1_paint, SkCanvas* canvas, SkMatrix* out_transform = 0) {
  SkMatrix transform;
  ...
  switch (colrv1_paint.format) {
    ...
    case FT_COLR_PAINTFORMAT_ROTATE: {
      transform = SkMatrix::RotateDeg(
          ...
          SkPoint::Make(
              SkFixedToScalar(colrv1_paint.u.rotate.center_x),
    ...
    case FT_COLR_PAINTFORMAT_SKEW: {
      ...
      transform.setSkew(
          tan_x, tan_y, SkFixedToScalar(colrv1_paint.u.skew.center_x),
...
bool colrv1_start_glyph(
    SkCanvas* canvas, const FT_Color* palette, FT_Face ft_face, uint16_t glyph_id,
    FT_Color_Root_Transform root_transform);

bool colrv1_traverse_paint(
    SkCanvas* canvas, const FT_Color* palette, FT_Face face, FT_OpaquePaint opaque_paint,
  ...
  switch (paint.format) {
    ...
    case FT_COLR_PAINTFORMAT_GLYPH:
      ...
      if (fillPaint.format == FT_COLR_PAINTFORMAT_SOLID ||
        ...
        traverse_result =
            colrv1_traverse_paint(canvas, palette, face, paint.u.glyph.paint, visited_set);
      }
      break;
    case FT_COLR_PAINTFORMAT_COLR_GLYPH:
      traverse_result = colrv1_start_glyph(
          canvas, palette, face, paint.u.colr_glyph.glyphID, FT_COLOR_NO_ROOT_TRANSFORM);
      break;
    case FT_COLR_PAINTFORMAT_TRANSFORM:
      colrv1_transform(face, paint, canvas);
      traverse_result =
          colrv1_traverse_paint(canvas, palette, face, paint.u.transform.paint, visited_set);
      break;
    case FT_COLR_PAINTFORMAT_TRANSLATE:
      colrv1_transform(face, paint, canvas);
      traverse_result =
          colrv1_traverse_paint(canvas, palette, face, paint.u.translate.paint, visited_set);
      break;
    case FT_COLR_PAINTFORMAT_SCALE:
      colrv1_transform(face, paint, canvas);
      traverse_result =
          colrv1_traverse_paint(canvas, palette, face, paint.u.scale.paint, visited_set);
      break;
    case FT_COLR_PAINTFORMAT_ROTATE:
      colrv1_transform(face, paint, canvas);
      traverse_result =
          colrv1_traverse_paint(canvas, palette, face, paint.u.rotate.paint, visited_set);
      break;
    case FT_COLR_PAINTFORMAT_SKEW:
      colrv1_transform(face, paint, canvas);
      traverse_result =
          colrv1_traverse_paint(canvas, palette, face, paint.u.skew.paint, visited_set);
      break;
    case FT_COLR_PAINTFORMAT_COMPOSITE: {
      ...
      traverse_result =
          traverse_result &&
          colrv1_traverse_paint(canvas, palette, face, paint.u.composite.source_paint, visited_set);
      canvas->restore();
      canvas->restore();
      break;
    }
...
SkPath GetClipBoxPath(FT_Face ft_face, uint16_t glyph_id, bool untransformed) {
  ...
  if (untransformed) {
    unscaledFtSize.reset([ft_face]() -> FT_Size {
      FT_Size size;
      FT_Error err = FT_New_Size(ft_face, &size);
      if (err != 0) {
        SK_TRACEFTR(
            err, "FT_New_Size(%s) failed in generateFacePathStaticCOLRv1.", ft_face->family_name);
        return nullptr;
      }
      return size;
    }());
...
bool colrv1_start_glyph_bounds(
    SkMatrix* ctm, SkRect* bounds, FT_Face ft_face, uint16_t glyph_id,
    FT_Color_Root_Transform root_transform);

bool colrv1_traverse_paint_bounds(
    SkMatrix* ctm, SkRect* bounds, FT_Face face, FT_OpaquePaint opaque_paint,
    VisitedSet* visited_set) {
  // Cycle detection, see section "5.7.11.1.9 Color glyphs as a directed acyclic graph".
  if (visited_set->contains(opaque_paint)) {
    return false;
  ...
  switch (paint.format) {
    ...
    case FT_COLR_PAINTFORMAT_TRANSFORM: {
      ...
      traverse_result =
          colrv1_traverse_paint_bounds(ctm, bounds, face, paint.u.transform.paint, visited_set);
      break;
    }
    case FT_COLR_PAINTFORMAT_TRANSLATE: {
      SkMatrix transform_matrix;
      ...
      ctm->preConcat(transform_matrix);
      traverse_result =
          colrv1_traverse_paint_bounds(ctm, bounds, face, paint.u.translate.paint, visited_set);
      break;
    }
    case FT_COLR_PAINTFORMAT_SCALE: {
      SkMatrix transform_matrix;
      ...
      ctm->preConcat(transform_matrix);
      traverse_result =
          colrv1_traverse_paint_bounds(ctm, bounds, face, paint.u.scale.paint, visited_set);
      break;
    }
    case FT_COLR_PAINTFORMAT_ROTATE: {
      SkMatrix transform_matrix;
      ...
      ctm->preConcat(transform_matrix);
      traverse_result =
          colrv1_traverse_paint_bounds(ctm, bounds, face, paint.u.rotate.paint, visited_set);
      break;
    }
    case FT_COLR_PAINTFORMAT_SKEW: {
      SkMatrix transform_matrix;
      ...
      ctm->preConcat(transform_matrix);
      traverse_result =
          colrv1_traverse_paint_bounds(ctm, bounds, face, paint.u.skew.paint, visited_set);
      break;
    }
    case FT_COLR_PAINTFORMAT_COMPOSITE: {
  ...
  switch (face->glyph->format) {
    case FT_GLYPH_FORMAT_OUTLINE: {
      ...
      if (SkMask::kARGB32_Format == glyph.fMaskFormat) {
        ...
        dstBitmap.setInfo(
            SkImageInfo::Make(glyph.fWidth, glyph.fHeight, kN32_SkColorType, kPremul_SkAlphaType),
            glyph.rowBytes());
...
#  endif
        ...
        if (this->isSubpixel()) {
          canvas.translate(
              SkFixedToScalar(glyph.getSubXFixed()), SkFixedToScalar(glyph.getSubYFixed()));
        }
...
#  ifdef TT_SUPPORT_COLRV1
        ...
        haveLayers = colrv1_start_glyph(
            &canvas, palette, face, glyph.getGlyphID(), FT_COLOR_INCLUDE_ROOT_TRANSFORM);
#  else
        haveLayers = false;
#  endif
        if (!haveLayers) {
          ...
          while (FT_Get_Color_Glyph_Layer(
            ...
            if (layerColorIndex == 0xFFFF) {
              ...
              SkColor color = SkColorSetARGB(
                  palette[layerColorIndex].alpha, palette[layerColorIndex].red,
                  palette[layerColorIndex].green, palette[layerColorIndex].blue);
              paint.setColor(color);
            }
            SkPath path;
        ...
        if (!haveLayers) {
          SK_TRACEFTR(
              err, "Could not get layers (neither v0, nor v1) from %s fontFace.",
              face->family_name);
          return;
        }
      } else
