...
static bool ios_get_path_in_bundle(const char path[], SkString* result) {
  ...
  sk_cfp<CFURLRef> pathURL(CFURLCreateFromFileSystemRepresentation(
      /*allocator=*/nullptr, (const UInt8*)path, strlen(path),
      /*isDirectory=*/false));
  ...
  sk_cfp<CFURLRef> fileURL(CFBundleCopyResourceURL(
      mainBundle, pathRef.get(),
