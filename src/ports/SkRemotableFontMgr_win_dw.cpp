...
class SK_API SkRemotableFontMgr_DirectWrite : public SkRemotableFontMgr {
 private:
  ...
  int FindOrAdd(
      IDWriteFontFileLoader* fontFileLoader, const void* refKey, UINT32 refKeySize) const {
    SkTScopedComPtr<IUnknown> fontFileLoaderId;
    HR_GENERAL(
        fontFileLoader->QueryInterface(&fontFileLoaderId),
        "Failed to re-convert to IDWriteFontFileLoader.", SkFontIdentity::kInvalidDataId);
    ...
    for (i = 0; i < count; ++i) {
      ...
      if (fontFileLoaderId.get() == current.fLoader && refKeySize == current.fKeySize &&
 ...
 public:
  ...
  SkRemotableFontMgr_DirectWrite(
      IDWriteFontCollection* fontCollection, WCHAR* localeName, int localeNameLength)
      : fFontCollection(SkRefComPtr(fontCollection)), fLocaleName(localeNameLength) {
  ...
  SkRemotableFontIdentitySet* getIndex(int familyIndex) const override {
    ...
    sk_sp<SkRemotableFontIdentitySet> fontIdSet(new SkRemotableFontIdentitySet(count, &fontIds));
  ...
  virtual SkFontIdentity matchIndexStyle(
      int familyIndex, const SkFontStyle& pattern) const override {
    SkFontIdentity identity = {SkFontIdentity::kInvalidDataId};

    SkTScopedComPtr<IDWriteFontFamily> fontFamily;
    HR_GENERAL(
        fFontCollection->GetFontFamily(familyIndex, &fontFamily), "Could not get requested family.",
        identity);
    ...
    HR_GENERAL(
        ...
        "Could not match font in family.", identity);
  ...
  SkRemotableFontIdentitySet* matchName(const char familyName[]) const override {
    ...
    if (nullptr == familyName) {
      HR_GENERAL(
          getDefaultFontFamilyName(&dwFamilyName), nullptr, SkRemotableFontIdentitySet::NewEmpty());
    } else {
      HR_GENERAL(
          sk_cstring_to_wchar(familyName, &dwFamilyName), nullptr,
    ...
    HR_GENERAL(
        ...
        "Failed while finding family by name.", SkRemotableFontIdentitySet::NewEmpty());
    if (!exists) {
  ...
  virtual SkFontIdentity matchNameStyle(
    ...
    HR_GENERAL(
        fFontCollection->FindFamilyName(dwFamilyName.get(), &index, &exists),
        "Failed while finding family by name.", identity);
    if (!exists) {
      return identity;
    }
  ...
  virtual SkFontIdentity matchNameStyleCharacter(
      const char familyName[], const SkFontStyle& pattern, const char* bcp47[], int bcp47Count,
    ...
    HR_GENERAL(
        dwFactory->CreateTextFormat(
            dwFamilyName, fFontCollection.get(), dwStyle.fWeight, dwStyle.fSlant, dwStyle.fWidth,
            72.0f, *dwBcp47, &fallbackFormat),
        "Could not create text format.", identity);

    WCHAR str[16];
    UINT32 strLen =
        static_cast<UINT32>(SkUTF::ToUTF16(character, reinterpret_cast<uint16_t*>(str)));
    ...
    HR_GENERAL(
        ...
        "Could not create text layout.", identity);
    ...
    HR_GENERAL(
        ...
        "Could not draw layout with renderer.", identity);
  ...
  SkStreamAsset* getData(int dataId) const override {
    ...
    HRNM(
        loader->CreateStreamFromKey(id.fKey, id.fKeySize, &fontFileStream),
...
SkRemotableFontMgr* SkRemotableFontMgr_New_DirectWrite() {
  ...
  HRNM(
      factory->GetSystemFontCollection(&sysFontCollection, FALSE),
