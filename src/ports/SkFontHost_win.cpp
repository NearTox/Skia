...
static FIXED SkFloatToFIXED(float x) { return SkFixedToFIXED(SkFloatToFixed(x)); }

static inline float SkFIXEDToFloat(FIXED x) { return SkFixedToFloat(SkFIXEDToFixed(x)); }
...
class SkGDIGeometrySink {
  ...
  void goingTo(const POINTFX pt) {
    if (!fStarted) {
      fStarted = true;
      fPath->moveTo(SkFIXEDToScalar(fCurrent.x), -SkFIXEDToScalar(fCurrent.y));
    }
    fCurrent = pt;
  }
      ...
      if (pc->wType == TT_PRIM_LINE) {
        for (uint16_t i = 0; i < cpfx; i++) {
          ...
          if (this->currentIsNot(pnt_b)) {
            this->goingTo(pnt_b);
            fPath->lineTo(SkFIXEDToScalar(pnt_b.x), -SkFIXEDToScalar(pnt_b.y));
          }
        }
      }
      ...
      if (pc->wType == TT_PRIM_QSPLINE) {
        for (uint16_t u = 0; u < cpfx - 1; u++) {  // Walk through points in spline
          ...
          if (this->currentIsNot(pnt_b) || this->currentIsNot(pnt_c)) {
            this->goingTo(pnt_c);
            fPath->quadTo(
                SkFIXEDToScalar(pnt_b.x), -SkFIXEDToScalar(pnt_b.y), SkFIXEDToScalar(pnt_c.x),
...
bool SkGDIGeometrySink::process(
    const uint8_t* glyphbuf, DWORD total_size, GDIGlyphbufferPointIter hintedYs) {
  const uint8_t* cur_glyph = glyphbuf;
  const uint8_t* end_glyph = glyphbuf + total_size;
...
bool SkScalerContext_GDI::generatePath(SkGlyphID glyph, SkPath* path) {
  ...
  if (fRec.getHinting() != SkFontHinting::kSlight) {
    ...
    if (!sinkXBufYIter.process(
            glyphbuf, total_size, GDIGlyphbufferPointIter(hintedGlyphbuf, hinted_total_size))) {
