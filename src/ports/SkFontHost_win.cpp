...
static inline SkScalar SkFIXEDToScalar(FIXED x) { return SkFixedToScalar(SkFIXEDToFixed(x)); }
...
static unsigned calculateGlyphCount(HDC hdc, const LOGFONT& lf) {
  ...
  while (min < max) {
    ...
    if (GetGlyphOutlineW(hdc, mid, GGO_METRICS | GGO_GLYPH_INDEX, &gm, 0, nullptr, &mat2) ==
        GDI_ERROR) {
      max = mid;
    } else {
      min = mid + 1;
...
class LogFontTypeface : public SkTypeface {
 public:
  LogFontTypeface(const SkFontStyle& style, const LOGFONT& lf, bool serializeAsStream)
    ...
    fCanBeLCD = !(
        (textMetric.tmPitchAndFamily & TMPF_VECTOR) && (textMetric.tmPitchAndFamily & TMPF_DEVICE));
  }
 ...
 */
sk_sp<SkTypeface> SkCreateFontMemResourceTypefaceFromLOGFONT(
    const LOGFONT& origLF, HANDLE fontMemResource) {
  LOGFONT lf = origLF;
  make_canonical(&lf);
...
static void populate_glyph_to_unicode(
    HDC fontHdc, const unsigned glyphCount, SkUnichar* glyphToUnicode) {
  ...
  for (DWORD i = 0; i < glyphSet->cRanges; ++i) {
    ...
    GetGlyphIndicesW(fontHdc, chars.get(), count, glyph.get(), GGI_MARK_NONEXISTING_GLYPHS);
...
static int alignTo32(int n) { return (n + 31) & ~31; }
...
const void* HDCOffscreen::draw(const SkGlyph& glyph, bool isBW, size_t* srcRBPtr) {
  ...
  BOOL ret = ExtTextOutW(
      fDC, 0, 0, ETO_GLYPH_INDEX, nullptr, reinterpret_cast<LPCWSTR>(&glyphID), 1, nullptr);
  GdiFlush();
  if (0 == ret) {
    return nullptr;
  }
...
class SkScalerContext_GDI : public SkScalerContext {
 public:
  SkScalerContext_GDI(
      sk_sp<LogFontTypeface>, const SkScalerContextEffects&, const SkDescriptor* desc);
 ...
 private:
  DWORD getGDIGlyphPath(SkGlyphID glyph, UINT flags, SkAutoSTMalloc<BUFFERSIZE, uint8_t>* glyphbuf);
   ...
   */
  SkMatrix fG_inv;
  enum Type { kTrueType_Type, kBitmap_Type, kLine_Type } fType;
  TEXTMETRIC fTM;
};

static FIXED float2FIXED(float x) { return SkFixedToFIXED(SkFloatToFixed(x)); }

static inline float FIXED2float(FIXED x) { return SkFixedToFloat(SkFIXEDToFixed(x)); }

static BYTE compute_quality(const SkScalerContextRec& rec) {
  switch (rec.fMaskFormat) {
    case SkMask::kBW_Format: return NONANTIALIASED_QUALITY;
    case SkMask::kLCD16_Format: return CLEARTYPE_QUALITY;
    default:
...
void SkScalerContext_GDI::generateMetrics(SkGlyph* glyph) {
  ...
  if (fType == SkScalerContext_GDI::kBitmap_Type || fType == SkScalerContext_GDI::kLine_Type) {
    ...
    if (fType == SkScalerContext_GDI::kLine_Type) {
      SkRect bounds = SkRect::MakeXYWH(glyph->fLeft, glyph->fTop, glyph->fWidth, glyph->fHeight);
      SkMatrix m;
      m.setAll(
          SkFIXEDToScalar(fMat22.eM11), -SkFIXEDToScalar(fMat22.eM21), 0,
          -SkFIXEDToScalar(fMat22.eM12), SkFIXEDToScalar(fMat22.eM22), 0, 0, 0, 1);
      m.mapRect(&bounds);
  ...
  DWORD status =
      GetGlyphOutlineW(fDDC, glyphId, GGO_METRICS | GGO_GLYPH_INDEX, &gm, 0, nullptr, &fMat22);
  if (GDI_ERROR == status) {
  ...
  if (1 == gm.gmBlackBoxX && 1 == gm.gmBlackBoxY) {
    ...
    DWORD bufferSize =
        GetGlyphOutlineW(fDDC, glyphId, GGO_NATIVE | GGO_GLYPH_INDEX, &gm, 0, nullptr, &fMat22);
    empty = (0 == bufferSize);
  }
  ...
  if ((fTM.tmPitchAndFamily & TMPF_VECTOR) && this->isLinearMetrics()) {
    sk_bzero(&gm, sizeof(gm));
    status = GetGlyphOutlineW(
        fDDC, glyphId, GGO_METRICS | GGO_GLYPH_INDEX, &gm, 0, nullptr, &fHighResMat22);
    if (GDI_ERROR != status) {
      SkPoint advance;
...
static const uint8_t* getInverseGammaTableGDI() {
  ...
  once([] { build_power_table(gTableGdi, 2.3f); });
  return gTableGdi;
}
...
static inline uint16_t rgb_to_lcd16(
    SkGdiRGB rgb, const uint8_t* tableR, const uint8_t* tableG, const uint8_t* tableB) {
...
static void rgb_to_a8(
    const SkGdiRGB* SK_RESTRICT src, size_t srcRB, const SkGlyph& glyph, const uint8_t* table8) {
...
static void rgb_to_lcd16(
    const SkGdiRGB* SK_RESTRICT src, size_t srcRB, const SkGlyph& glyph, const uint8_t* tableR,
...
void SkScalerContext_GDI::generateImage(const SkGlyph& glyph) {
  ...
  if (isBW) {
    ...
    if (fPreBlend.isApplicable()) {
      rgb_to_lcd16<true>(src, srcRB, glyph, fPreBlend.fR, fPreBlend.fG, fPreBlend.fB);
    } else {
      rgb_to_lcd16<false>(src, srcRB, glyph, fPreBlend.fR, fPreBlend.fG, fPreBlend.fB);
    }
  }
}
...
static void sk_path_from_gdi_path(SkPath* path, const uint8_t* glyphbuf, DWORD total_size) {
  ...
  while (cur_glyph < end_glyph) {
    ...
    path->moveTo(
        SkFixedToScalar(SkFIXEDToFixed(th->pfxStart.x)),
    ...
    while (cur_poly < end_poly) {
      ...
      if (pc->wType == TT_PRIM_LINE) {
        for (uint16_t i = 0; i < pc->cpfx; i++) {
          path->lineTo(
              SkFixedToScalar(SkFIXEDToFixed(pc->apfx[i].x)),
      ...
      if (pc->wType == TT_PRIM_QSPLINE) {
        for (uint16_t u = 0; u < pc->cpfx - 1; u++) {  // Walk through points in spline
          ...
          if (u < pc->cpfx - 2) {  // If not on last spline, compute C
            pnt_c.x = SkFixedToFIXED(SkFixedAve(SkFIXEDToFixed(pnt_b.x), SkFIXEDToFixed(pnt_c.x)));
            pnt_c.y = SkFixedToFIXED(SkFixedAve(SkFIXEDToFixed(pnt_b.y), SkFIXEDToFixed(pnt_c.y)));
          }

          path->quadTo(
              SkFixedToScalar(SkFIXEDToFixed(pnt_b.x)), SkFixedToScalar(-SkFIXEDToFixed(pnt_b.y)),
              SkFixedToScalar(SkFIXEDToFixed(pnt_c.x)), SkFixedToScalar(-SkFIXEDToFixed(pnt_c.y)));
        }
      }
...
static bool sk_path_from_gdi_paths(
    SkPath* path, const uint8_t* glyphbuf, DWORD total_size, GDIGlyphbufferPointIter hintedYs) {
  const uint8_t* cur_glyph = glyphbuf;
  const uint8_t* end_glyph = glyphbuf + total_size;
...
DWORD SkScalerContext_GDI::getGDIGlyphPath(
    SkGlyphID glyph, UINT flags, SkAutoSTMalloc<BUFFERSIZE, uint8_t>* glyphbuf) {
  GLYPHMETRICS gm;

  DWORD total_size =
      GetGlyphOutlineW(fDDC, glyph, flags, &gm, BUFFERSIZE, glyphbuf->get(), &fMat22);
...
bool SkScalerContext_GDI::generatePath(SkGlyphID glyph, SkPath* path) {
  ...
  if (fRec.getHinting() != SkFontHinting::kSlight) {
    ...
    if (!sk_path_from_gdi_paths(
            path, glyphbuf, total_size,
...
static void logfont_for_name(const char* familyName, LOGFONT* lf) {
  ...
  size_t str_len = ::MultiByteToWideChar(CP_UTF8, 0, familyName, -1, nullptr, 0);
  // Allocate a buffer (str_len already has terminating null
  // accounted for).
  wchar_t* wideFamilyName = new wchar_t[str_len];
  // Now actually convert the string.
  ::MultiByteToWideChar(CP_UTF8, 0, familyName, -1, wideFamilyName, str_len);
...
static sk_sp<SkTypeface> create_from_stream(std::unique_ptr<SkStreamAsset> stream) {
  ...
  sk_sp<SkData> rewrittenFontData(
      SkOTUtils::RenameFont(stream.get(), familyName, familyNameSize - 1));
...
static void bmpCharsToGlyphs(
    HDC hdc, const WCHAR* bmpChars, int count, uint16_t* glyphs, bool Ox1FHack) {
...
static uint16_t nonBmpCharToGlyph(HDC hdc, SCRIPT_CACHE* scriptCache, const WCHAR utf16[2]) {
  ...
  int numItems;
  HRZM(
      ScriptItemize(utf16, numWCHAR, maxItems, &scriptControl, nullptr, si, &numItems),
  ...
  HRZM(
      ScriptShape(
          hdc, scriptCache, utf16, numWCHAR, maxGlyphs, &script, outGlyphs, logClust, vsa,
          &numGlyphs),
...
void LogFontTypeface::onFilterRec(SkScalerContextRec* rec) const {
  ...
  unsigned flagsWeDontSupport =
      SkScalerContext::kForceAutohinting_Flag | SkScalerContext::kEmbeddedBitmapText_Flag |
  ...
  switch (h) {
    case SkFontHinting::kNone: break;
    ...
    case SkFontHinting::kFull:
      ...
      break;
    default: SkDEBUGFAIL("unknown hinting");
  }
 ...
 */
static int CALLBACK
enum_family_proc(const LOGFONT* lf, const TEXTMETRIC*, DWORD fontType, LPARAM builderParam) {
...
class SkFontMgrGDI : public SkFontMgr {
 ...
 protected:
  int onCountFamilies() const override { return fLogFontArray.count(); }
  ...
  virtual SkTypeface* onMatchFamilyStyle(
      const char familyName[], const SkFontStyle& fontstyle) const override {
