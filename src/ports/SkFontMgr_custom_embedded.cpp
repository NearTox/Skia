...
static void load_font_from_data(
    const SkTypeface_FreeType::Scanner& scanner, const uint8_t* data, size_t size, int index,
...
class DataFontLoader : public SkFontMgr_Custom::SystemFontLoader {
 public:
  DataFontLoader(const uint8_t** datas, const size_t* sizes, int n)
      : fDatas(datas), fSizes(sizes), fNum(n) {}
...
static SkFontStyleSet_Custom* find_family(
    SkFontMgr_Custom::Families& families, const char familyName[]) {
...
static void load_font_from_data(
  ...
  for (int faceIndex = 0; faceIndex < numFaces; ++faceIndex) {
    ...
    addTo->appendTypeface(
        sk_make_sp<SkTypeface_Stream>(std::move(data), style, isFixedPitch, true, realname));
  }
}
