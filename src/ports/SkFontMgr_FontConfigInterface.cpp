...
class SkFontRequestCache {
 public:
  struct Request : public SkResourceCache::Key {
   ...
   public:
    ...
    void operator delete(void* storage) { delete[] reinterpret_cast<char*>(storage); }
  };
 ...
 public:
  ...
  sk_sp<SkTypeface> findAndRef(Request* request) {
    ...
    fCachedResults.find(
        *request,
        [](const SkResourceCache::Rec& rec, void* context) -> bool {
...
class SkFontMgr_FCI : public SkFontMgr {
 ...
 protected:
  ...
  SkTypeface* onMatchFamilyStyle(
      const char requestedFamilyName[], const SkFontStyle& requestedStyle) const override {
    SkAutoMutexExclusive ama(fMutex);

    SkFontConfigInterface::FontIdentity identity;
    SkString outFamilyName;
    SkFontStyle outStyle;
    if (!fFCI->matchFamilyName(
            requestedFamilyName, requestedStyle, &identity, &outFamilyName, &outStyle)) {
      return nullptr;
    }
  ...
  sk_sp<SkTypeface> onLegacyMakeTypeface(
      const char requestedFamilyName[], SkFontStyle requestedStyle) const override {
    SkAutoMutexExclusive ama(fMutex);
