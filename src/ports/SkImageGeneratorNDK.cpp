...
static bool ok(int result) { return result == ANDROID_IMAGE_DECODER_SUCCESS; }
...
static SkColorType colorType(AImageDecoder* decoder, const AImageDecoderHeaderInfo* headerInfo) {
  ...
  auto format =
      static_cast<AndroidBitmapFormat>(AImageDecoderHeaderInfo_getAndroidBitmapFormat(headerInfo));
...
std::unique_ptr<SkImageGenerator> SkImageGeneratorNDK::MakeFromEncodedNDK(sk_sp<SkData> data) {
  ...
  SkAlphaType at =
      AImageDecoderHeaderInfo_getAlphaFlags(headerInfo) == ANDROID_BITMAP_FLAGS_ALPHA_OPAQUE
          ? kOpaque_SkAlphaType
          : kPremul_SkAlphaType;
...
ImageGeneratorNDK::ImageGeneratorNDK(
    const SkImageInfo& info, sk_sp<SkData> data, AImageDecoder* decoder)
    : INHERITED(info), fData(std::move(data)), fDecoder(decoder), fPreviouslySetADataSpace(false) {
  SkASSERT(fDecoder);
}

ImageGeneratorNDK::~ImageGeneratorNDK() { AImageDecoder_delete(fDecoder); }
...
bool ImageGeneratorNDK::onGetPixels(
    const SkImageInfo& info, void* pixels, size_t rowBytes, const Options& opts) {
  ...
  switch (info.alphaType()) {
    case kUnknown_SkAlphaType: return false;
  ...
  switch (AImageDecoder_decodeImage(fDecoder, pixels, rowBytes, byteSize)) {
    ...
    case ANDROID_IMAGE_DECODER_SUCCESS: return true;
    default: return false;
  }
}

sk_sp<SkData> ImageGeneratorNDK::onRefEncodedData() { return fData; }
