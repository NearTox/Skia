...
class SkScalerContext_FreeType : public SkScalerContext_FreeType_Base {
 ...
 private:
  ...
  FT_Error setupSize();
  void getBBoxForCurrentGlyph(
      const SkGlyph* glyph, FT_BBox* bbox, bool snapToPixelBoundary = false);
...
void SkScalerContext_FreeType::getBBoxForCurrentGlyph(
    const SkGlyph* glyph, FT_BBox* bbox, bool snapToPixelBoundary) {
...
void SkScalerContext_FreeType::generateMetrics(SkGlyph* glyph) {
  ...
  FT_Error err;
  err = FT_Load_Glyph(fFace, glyph->getGlyphID(), fLoadGlyphFlags | FT_LOAD_BITMAP_METRICS_ONLY);
  if (err != 0) {
    glyph->zeroMetrics();
    return;
  }
...
#  ifdef TT_SUPPORT_COLRV1
    ...
    if (FT_Get_Color_Glyph_Paint(
      ...
      if (FT_Get_Color_Glyph_ClipBox(fFace, glyph->getGlyphID(), &colrGlyphBbox)) {
        ...
        for (auto& corner :
             {colrGlyphBbox.top_left, colrGlyphBbox.top_right, colrGlyphBbox.bottom_right}) {
...
#  endif  // #TT_SUPPORT_COLRV1
    ...
    if (!haveLayers) {
      ...
      while (FT_Get_Color_Glyph_Layer(
        ...
        haveLayers = true;
        err = FT_Load_Glyph(fFace, layerGlyphIndex, fLoadGlyphFlags | FT_LOAD_BITMAP_METRICS_ONLY);
        if (err != 0) {
          glyph->zeroMetrics();
          return;
        }
    ...
    if (!SkTFitsIn<decltype(glyph->fWidth)>(width) ||
        !SkTFitsIn<decltype(glyph->fHeight)>(height) || !SkTFitsIn<decltype(glyph->fTop)>(top) ||
...

    {
      ...
      if (this->shouldSubpixelBitmap(*glyph, fMatrix22Scalar)) {
        rect.offset(SkFixedToScalar(glyph->getSubXFixed()), SkFixedToScalar(glyph->getSubYFixed()));
      }
...
void SkScalerContext_FreeType::generateImage(const SkGlyph& glyph) {
  ...
  if (err != 0) {
    SK_TRACEFTR(
        err,
        "SkScalerContext_FreeType::generateImage: FT_Load_Glyph(glyph:%d "
        "width:%d height:%d rb:%zu flags:%d) failed.",
        glyph.getGlyphID(), glyph.width(), glyph.height(), glyph.rowBytes(), fLoadGlyphFlags);
    sk_bzero(glyph.fImage, glyph.imageSize());
    return;
  }
