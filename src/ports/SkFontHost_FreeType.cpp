...
#    define SK_FREETYPE_MINIMUM_RUNTIME_VERSION \
      (((FREETYPE_MAJOR) << 24) | ((FREETYPE_MINOR) << 16) | ((FREETYPE_PATCH) << 8))
#  else
#    define SK_FREETYPE_MINIMUM_RUNTIME_VERSION \
      ((2 << 24) | (8 << 16) | (1 << 8) | (SK_FREETYPE_DLOPEN))
#  endif
#endif
...
static SkScalar SkFT_FixedToScalar(FT_Fixed x) { return SkFixedToScalar(x); }
...
static_assert(
    std::is_same<FT_Alloc_size_t, long>::value || std::is_same<FT_Alloc_size_t, size_t>::value, "");

extern "C" {
static void* sk_ft_alloc(FT_Memory, FT_Alloc_size_t size) { return sk_malloc_throw(size); }
static void sk_ft_free(FT_Memory, void* block) { sk_free(block); }
static void* sk_ft_realloc(
    FT_Memory, FT_Alloc_size_t cur_size, FT_Alloc_size_t new_size, void* block) {
...
extern "C" {
static unsigned long sk_ft_stream_io(
    FT_Stream ftStream, unsigned long offset, unsigned char* buffer, unsigned long count) {
...
void SkTypeface_FreeType::FaceRec::setupAxes(const SkFontData& data) {
  ...
  SkDEBUGCODE(
      FT_MM_Var* variations = nullptr; if (FT_Get_MM_Var(fFace.get(), &variations)) {
...
void SkTypeface_FreeType::FaceRec::setupPalette(const SkFontData& data) {
  ...
  for (int i = 0; i < fFTPaletteEntryCount; ++i) {
    fSkPalette[i] =
        SkColorSetARGB(ftPalette[i].alpha, ftPalette[i].red, ftPalette[i].green, ftPalette[i].blue);
  }
#endif
}
...
// Caller must lock f_t_mutex() before calling this function.
std::unique_ptr<SkTypeface_FreeType::FaceRec> SkTypeface_FreeType::FaceRec::Make(
    const SkTypeface_FreeType* typeface) {
  f_t_mutex().assertHeld();
...
class AutoFTAccess {
 public:
  ...
  ~AutoFTAccess() { f_t_mutex().release(); }
...
class SkScalerContext_FreeType : public SkScalerContext_FreeType_Base {
 public:
  SkScalerContext_FreeType(
      sk_sp<SkTypeface_FreeType>, const SkScalerContextEffects&, const SkDescriptor* desc);
  ~SkScalerContext_FreeType() override;

  bool success() const { return fFTSize != nullptr && fFace != nullptr; }
...
static SkAdvancedTypefaceMetrics::FontType get_font_type(FT_Face face) {
  ...
  static struct {
    const char* s;
    SkAdvancedTypefaceMetrics::FontType t;
  } values[] = {
...
std::unique_ptr<SkAdvancedTypefaceMetrics> SkTypeface_FreeType::onGetAdvancedMetrics() const {
  ...
  if ((pcltTable = (TT_PCLT*)FT_Get_Sfnt_Table(face, ft_sfnt_pclt)) != nullptr) {
    ...
    if (2 <= serif_style && serif_style <= 6) {
      ...
      ((os2Table = (TT_OS2*)FT_Get_Sfnt_Table(face, ft_sfnt_os2)) != nullptr) &&
      // sCapHeight is available only when version 2 or later.
      os2Table->version != 0xFFFF && os2Table->version >= 2) {
    info->fCapHeight = os2Table->sCapHeight;
  }
  info->fBBox =
      SkIRect::MakeLTRB(face->bbox.xMin, face->bbox.yMax, face->bbox.xMax, face->bbox.yMin);
  return info;
}
 ...
 *  cannot.
 */
static int GetVariationDesignPosition(
    AutoFTAccess& fta, SkFontArguments::VariationPosition::Coordinate coordinates[],
    int coordinateCount) {
  FT_Face face = fta.face();
  if (!face) {
    return -1;
...
std::unique_ptr<SkFontData> SkTypeface_FreeType::cloneFontData(const SkFontArguments& args) const {
  ...
  return std::make_unique<SkFontData>(
      std::move(stream), ttcIndex, args.getPalette().index, axisValues.get(), axisCount,
...
bool SkTypeface_FreeType::onGetKerningPairAdjustments(
...
SkScalerContext_FreeType::SkScalerContext_FreeType(
    sk_sp<SkTypeface_FreeType> typeface, const SkScalerContextEffects& effects,
    ...
    if (SkMask::kBW_Format == fRec.fMaskFormat) {
      ...
      switch (fRec.getHinting()) {
        ...
        case SkFontHinting::kSlight:
          ...
          linearMetrics = true;
          break;
        case SkFontHinting::kNormal: loadFlags = FT_LOAD_TARGET_NORMAL; break;
        case SkFontHinting::kFull:
          ...
          break;
        default: LOG_INFO("---------- UNKNOWN hinting %d\n", fRec.getHinting()); break;
      }
    }
...
bool SkScalerContext_FreeType::generateAdvance(SkGlyph* glyph) {
  ...
  error = FT_Get_Advance(
      fFace, glyph->getGlyphID(), fLoadGlyphFlags | FT_ADVANCE_FLAG_FAST_ONLY, &advance);
...
bool SkScalerContext_FreeType::getBoundsOfCurrentOutlineGlyph(FT_GlyphSlot glyph, SkRect* bounds) {
  ...
  *bounds = SkRect::MakeLTRB(
      SkFDot6ToScalar(bbox.xMin), -SkFDot6ToScalar(bbox.yMax), SkFDot6ToScalar(bbox.xMax),
...
void SkScalerContext_FreeType::updateGlyphBoundsIfLCD(SkGlyph* glyph) {
  if (glyph->fMaskFormat == SkMask::kLCD16_Format && glyph->fWidth > 0 && glyph->fHeight > 0) {
    if (fLCDIsVert) {
      glyph->fHeight += 2;
      glyph->fTop -= 1;
    } else {
...
bool SkScalerContext_FreeType::shouldSubpixelBitmap(const SkGlyph& glyph, const SkMatrix& matrix) {
  ...
  bool mechanism = fFace->glyph->format == FT_GLYPH_FORMAT_BITMAP && this->isSubpixel() &&
...
#  ifdef TT_SUPPORT_COLRV1
    ...
    if (FT_Get_Color_Glyph_Paint(
            fFace, glyph->getGlyphID(), FT_COLOR_INCLUDE_ROOT_TRANSFORM, &opaqueLayerPaint)) {
      haveLayers = true;
...
#  endif  // #TT_SUPPORT_COLRV1
    ...
    if (!haveLayers) {
      ...
      while (FT_Get_Color_Glyph_Layer(
          fFace, glyph->getGlyphID(), &layerGlyphIndex, &layerColorIndex, &layerIterator)) {
        haveLayers = true;
  ...
  if (!haveLayers) {
    ...
    if (fFace->glyph->format == FT_GLYPH_FORMAT_OUTLINE) {
      ...
      SkRect bounds = SkRect::MakeXYWH(
          SkIntToScalar(fFace->glyph->bitmap_left), -SkIntToScalar(fFace->glyph->bitmap_top),
          SkIntToScalar(fFace->glyph->bitmap.width), SkIntToScalar(fFace->glyph->bitmap.rows));
      ...
      SkRect infiniteRect = SkRect::MakeLTRB(
          -SK_ScalarInfinity, -SK_ScalarInfinity, SK_ScalarInfinity, SK_ScalarInfinity);
  ...
  LOG_INFO(
      "Metrics(glyph:%d flags:0x%x) w:%d\n", glyph->getGlyphID(), fLoadGlyphFlags, glyph->fWidth);
#endif
}
...
void SkScalerContext_FreeType::generateImage(const SkGlyph& glyph) {
  ...
  if (this->shouldSubpixelBitmap(glyph, *bitmapMatrix)) {
    ...
    subpixelBitmapMatrix.postTranslate(
        SkFixedToScalar(glyph.getSubXFixed()), SkFixedToScalar(glyph.getSubYFixed()));
...
void SkScalerContext_FreeType::generateFontMetrics(SkFontMetrics* metrics) {
  ...
  if (face->face_flags & FT_FACE_FLAG_SCALABLE) {  // scalable outline font
    ...
    underlinePosition =
        -SkIntToScalar(face->underline_position + face->underline_thickness / 2) / upem;
...
void SkScalerContext_FreeType::emboldenIfNeeded(FT_Face face, FT_GlyphSlot glyph, SkGlyphID gid) {
  ...
  switch (glyph->format) {
    case FT_GLYPH_FORMAT_OUTLINE:
      FT_Pos strength;
      strength =
          FT_MulFix(face->units_per_EM, face->size->metrics.y_scale) / SK_OUTLINE_EMBOLDEN_DIVISOR;
    ...
    case FT_GLYPH_FORMAT_BITMAP:
      ...
      break;
    default: SkDEBUGFAIL("unknown glyph format");
  }
}
...
void SkTypeface_FreeType::onCharsToGlyphs(
...
int SkTypeface_FreeType::onGetVariationDesignParameters(
  ...
  for (FT_UInt i = 0; i < variations->num_axis; ++i) {
    ...
    FT_UInt flags = 0;
    bool hidden =
        !FT_Get_Var_Axis_Flags(variations, i, &flags) && (flags & FT_VAR_AXIS_FLAG_HIDDEN);
...
size_t SkTypeface_FreeType::onGetTableData(
...
void SkTypeface_FreeType::FontDataPaletteToDescriptorPalette(
    const SkFontData& fontData, SkFontDescriptor* desc) {
...
FT_Face SkTypeface_FreeType::Scanner::openFace(
...
bool SkTypeface_FreeType::Scanner::scanFont(
    SkStreamAsset* stream, int ttcIndex, SkString* name, SkFontStyle* style, bool* isFixedPitch,
  ...
  if (hasAxes) {
    ...
    if (GetAxes(face.get(), &axisDefinitions)) {
      ...
      for (size_t i = 0; i < numAxes; ++i) {
        ...
        if (axisDefinitions[i].fTag == slntTag) slntIndex = i;
      }
  ...
  if (!hasOs2 && !hasAxes && 0 == FT_Get_PS_Font_Info(face.get(), &psFontInfo) &&
      psFontInfo.weight) {
    static const struct {
    ...
    int const index = SkStrLCSearch(
        &commonWeights[0].name, SK_ARRAY_COUNT(commonWeights), psFontInfo.weight,
      ...
      if (axisDefinition.fTag == coordinate.axis) {
        ...
        if (coordinate.value != axisValue) {
          LOG_INFO(
              ...
              name.c_str(), (axisDefinition.fTag >> 24) & 0xFF, (axisDefinition.fTag >> 16) & 0xFF,
