...
#  else
#    define SK_FREETYPE_MINIMUM_RUNTIME_VERSION \
      ((2 << 24) | (3 << 16) | (11 << 8) | (SK_FREETYPE_DLOPEN))
#  endif
#endif
...
class FreeTypeLibrary : SkNoncopyable {
 public:
  FreeTypeLibrary()
    ...
    Version version;
    FT_Library_Version(
        fLibrary, &std::get<0>(version), &std::get<1>(version), &std::get<2>(version));
...
// The 'light' hinting is vertical only starting in 2.8.0.
    ...
    fIsLCDSupported =
        ...
        ((SK_FREETYPE_MINIMUM_RUNTIME_VERSION) >= 0x02080100) || (Version(2, 8, 1) <= version);
...
SkFaceRec::SkFaceRec(std::unique_ptr<SkStreamAsset> stream, uint32_t fontID)
    : fNext(nullptr),
      ...
      fFontID(fontID),
      fAxesCount(0),
      fNamedVariationSpecified(false) {
...
static void ft_face_setup_axes(SkFaceRec* rec, const SkFontData& data) {
  ...
  SkDEBUGCODE(
      FT_MM_Var* variations = nullptr; if (FT_Get_MM_Var(rec->fFace.get(), &variations)) {
...
class AutoFTAccess {
 public:
  ...
  bool isNamedVariationSpecified() { return fFaceRec ? fFaceRec->fNamedVariationSpecified : false; }
