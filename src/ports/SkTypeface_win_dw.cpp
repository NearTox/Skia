...
HRESULT DWriteFontTypeface::initializePalette() {
  ...
  HRM(fDWriteFontFace2->GetPaletteEntries(basePaletteIndex, 0, dwPaletteEntryCount, dwPaletteEntry),
      "Could not retrieve palette entries.");

  fPalette.reset(new SkColor[dwPaletteEntryCount]);
  for (UINT32 i = 0; i < dwPaletteEntryCount; ++i) {
    fPalette[i] = SkColorSetARGB(
        sk_float_round2int(dwPaletteEntry[i].a * 255),
...
DWriteFontTypeface::DWriteFontTypeface(
    const SkFontStyle& style, IDWriteFactory* factory, IDWriteFontFace* fontFace, IDWriteFont* font,
    IDWriteFontFamily* fontFamily, sk_sp<Loaders> loaders, const SkFontArguments::Palette& palette)
    : SkTypeface(style, false),
      ...
      fRequestedPaletteEntryOverrides(
          palette.overrideCount
              ? (SkFontArguments::Palette::Override*)memcpy(
                    new SkFontArguments::Palette::Override[palette.overrideCount],
                    palette.overrides, palette.overrideCount * sizeof(palette.overrides[0]))
              : nullptr),
      fRequestedPalette{
          palette.index, fRequestedPaletteEntryOverrides.get(), palette.overrideCount},
...
void DWriteFontTypeface::onGetFontDescriptor(SkFontDescriptor* desc, bool* isLocalStream) const {
  ...
  sk_careful_memcpy(
      desc->setPaletteEntryOverrides(fRequestedPalette.overrideCount), fRequestedPalette.overrides,
...
int DWriteFontTypeface::onCountGlyphs() const { return fDWriteFontFace->GetGlyphCount(); }
...
#  if defined(NTDDI_WIN10_RS3) && NTDDI_VERSION >= NTDDI_WIN10_RS3
  ...
  HR_GENERAL(
      fontResource->GetDefaultFontAxisValues(fontAxisDefaultValue.get(), fontAxisCount), nullptr,
      -1);
  UINT32 coordIndex = 0;
...
int DWriteFontTypeface::onGetTableTags(SkFontTableTag tags[]) const {
  ...
  if (type != DWRITE_FONT_FACE_TYPE_CFF && type != DWRITE_FONT_FACE_TYPE_TRUETYPE &&
...
sk_sp<SkData> DWriteFontTypeface::onCopyTableData(SkFontTableTag tag) const {
  ...
  void* lock;
  BOOL exists;
  fDWriteFontFace->TryGetFontTable(
      SkEndian_SwapBE32(tag), reinterpret_cast<const void**>(&data), &size, &lock, &exists);
  if (!exists) {
    return nullptr;
  }
  struct Context {
...
#  if defined(NTDDI_WIN10_RS3) && NTDDI_VERSION >= NTDDI_WIN10_RS3
  ...
  if (SUCCEEDED(fDWriteFontFace->QueryInterface(&fontFace5)) && fontFace5->HasVariations()) {
    ...
    HRN(fontResource->CreateFontFace(
        fDWriteFont->GetSimulations(), fontAxisValue.get(), fontAxisCount, &newFontFace5));
...
#  endif
  ...
  if (args.getPalette().index != fRequestedPalette.index ||
      ...
      memcmp(
          args.getPalette().overrides, fRequestedPalette.overrides,
          fRequestedPalette.overrideCount * sizeof(fRequestedPalette.overrides[0]))) {
    return DWriteFontTypeface::Make(
        fFactory.get(), fDWriteFontFace.get(), fDWriteFont.get(), fDWriteFontFamily.get(), fLoaders,
        args.getPalette());
  }
...
std::unique_ptr<SkStreamAsset> DWriteFontTypeface::onOpenStream(int* ttcIndex) const {
  *ttcIndex = fDWriteFontFace->GetIndex();
  ...
  UINT32 numFiles = 0;
  HRNM(fDWriteFontFace->GetFiles(&numFiles, nullptr), "Could not get number of font files.");
  if (numFiles != 1) {
    return nullptr;
  }
  ...
  HRNM(
      fontFile->GetReferenceKey(&fontFileKey, &fontFileKeySize),
  ...
  HRNM(
      fontFileLoader->CreateStreamFromKey(fontFileKey, fontFileKeySize, &fontFileStream),
...
static void glyph_to_unicode_map(
    IDWriteFontFace* fontFace, DWRITE_UNICODE_RANGE range, UINT32* remainingGlyphCount,
    UINT32 numGlyphs, SkUnichar* glyphToUnicode) {
  ...
  for (UINT32 c = range.first; c <= range.last && *remainingGlyphCount != 0; c += batchSize) {
    ...
    HRVM(
        fontFace->GetGlyphIndices(codepoints, numBatchedCodePoints, glyphs),
...
std::unique_ptr<SkAdvancedTypefaceMetrics> DWriteFontTypeface::onGetAdvancedMetrics() const {
  ...
  bool fixedWidth =
      (postTable->isFixedPitch && (1 == SkEndian_SwapBE16(hheaTable->numberOfHMetrics)));
  // Monospace
  if (fixedWidth) {
  ...
  if (SkPanose::FamilyType::TextAndDisplay == os2Table->version.v0.panose.bFamilyType) {
    if (SerifStyle::Cove == serifStyle || SerifStyle::ObtuseCove == serifStyle ||
        SerifStyle::SquareCove == serifStyle || SerifStyle::ObtuseSquareCove == serifStyle ||
        SerifStyle::Square == serifStyle || SerifStyle::Thin == serifStyle ||
        SerifStyle::Bone == serifStyle || SerifStyle::Exaggerated == serifStyle ||
  ...
  info->fBBox = SkIRect::MakeLTRB(
      (int32_t)SkEndian_SwapBE16((uint16_t)headTable->xMin),
