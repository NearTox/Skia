...
public:
    ...
    static SkTypeface_FCI* Create(std::unique_ptr<SkFontData> data, SkString familyName,
                                  SkFontStyle style, bool isFixedPitch) {
        return new SkTypeface_FCI(std::move(data), std::move(familyName), style, isFixedPitch);
    }

    const SkFontConfigInterface::FontIdentity& getIdentity() const { return fIdentity; }
    ...
    sk_sp<SkTypeface> onMakeClone(const SkFontArguments& args) const override {
        ...
        return sk_sp<SkTypeface>(new SkTypeface_FCI(std::move(data), fFamilyName, this->fontStyle(),
                                                    this->isFixedPitch()));
    }
...
protected:
    ...
    SkTypeface_FCI(std::unique_ptr<SkFontData> data, SkString familyName, SkFontStyle style,
                   bool isFixedPitch)
