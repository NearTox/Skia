...
class FCLocker {
 ...
 public:
  ...
  static void AssertHeld() {
    SkDEBUGCODE(if (FcGetVersion() < FontConfigThreadSafeVersion) { f_c_mutex().assertHeld(); })
  }
};
...
class SkAutoFc : public SkAutoTCallVProc<T, FcTDestroy<T, D>> {
...
static SkWeakReturn is_weak(FcPattern* pattern, const char object[], int id) {
  ...
  SkAutoFcPattern match(
      FcFontSetMatch(config, fontSets, SK_ARRAY_COUNT(fontSets), minimal, &result));
...
static int map_range(
    SkScalar value, SkScalar old_min, SkScalar old_max, SkScalar new_min, SkScalar new_max) {
...
static SkScalar map_ranges(SkScalar val, MapRanges const ranges[], int rangesCount) {
  ...
  for (int i = 0; i < rangesCount - 1; ++i) {
    if (val < ranges[i + 1].old_val) {
      return map_range(
          val, ranges[i].old_val, ranges[i + 1].old_val, ranges[i].new_val, ranges[i + 1].new_val);
    }
  }
...
static SkFontStyle skfontstyle_from_fcpattern(FcPattern* pattern) {
  ...
  SkScalar weight = map_ranges(
      get_int(pattern, FC_WEIGHT, FC_WEIGHT_REGULAR), weightRanges, SK_ARRAY_COUNT(weightRanges));
  ...
  SkScalar width = map_ranges(
      get_int(pattern, FC_WIDTH, FC_WIDTH_NORMAL), widthRanges, SK_ARRAY_COUNT(widthRanges));
...
class SkTypeface_stream : public SkTypeface_FreeType {
 public:
  SkTypeface_stream(
      std::unique_ptr<SkFontData> data, SkString familyName, const SkFontStyle& style,
      bool fixedWidth)
      : INHERITED(style, fixedWidth), fFamilyName(std::move(familyName)), fData(std::move(data)) {}
  ...
  sk_sp<SkTypeface> onMakeClone(const SkFontArguments& args) const override {
    ...
    return sk_make_sp<SkTypeface_stream>(
        std::move(data), fFamilyName, this->fontStyle(), this->isFixedPitch());
  }
...
class SkTypeface_fontconfig : public SkTypeface_FreeType {
 public:
  static sk_sp<SkTypeface_fontconfig> Make(SkAutoFcPattern pattern, SkString sysroot) {
    return sk_sp<SkTypeface_fontconfig>(
        new SkTypeface_fontconfig(std::move(pattern), std::move(sysroot)));
  }
  ...
  void onFilterRec(SkScalerContextRec* rec) const override {
    ...
    if (fcOutline && fcMatrix) {
      ...
      SkMatrix fm;
      fm.setAll(fcMatrix->xx, -fcMatrix->xy, 0, -fcMatrix->yx, fcMatrix->yy, 0, 0, 0, 1);
  ...
  std::unique_ptr<SkAdvancedTypefaceMetrics> onGetAdvancedMetrics() const override {
    std::unique_ptr<SkAdvancedTypefaceMetrics> info = this->INHERITED::onGetAdvancedMetrics();
 ...
 private:
  SkTypeface_fontconfig(SkAutoFcPattern pattern, SkString sysroot)
      : INHERITED(
            skfontstyle_from_fcpattern(pattern),
...
class SkFontMgr_fontconfig : public SkFontMgr {
   ...
   public:
    ...
    SkTypeface* matchStyle(const SkFontStyle& style) override {
      SkAutoFcPattern match([this, &style]() {
        ...
        return FcFontSetMatch(fFontMgr->fFC, fontSets, SK_ARRAY_COUNT(fontSets), pattern, &result);
      }());
      return fFontMgr->createTypefaceFromFcPattern(std::move(match)).release();
    }
  ...
  static sk_sp<SkDataTable> GetFamilyNames(FcConfig* fcconfig) {
    ...
    return SkDataTable::MakeCopyArrays(
        (void const* const*)names.begin(), sizes.begin(), names.count());
  }
 ...
 protected:
  ...
  sk_sp<SkTypeface> onMakeFromFontData(std::unique_ptr<SkFontData> fontData) const override {
    ...
    return sk_sp<SkTypeface>(
        new SkTypeface_stream(std::move(fontData), std::move(name), style, isFixedWidth));
  }
