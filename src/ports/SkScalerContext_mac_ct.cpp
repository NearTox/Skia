...
static CGAffineTransform MatrixToCGAffineTransform(const SkMatrix& matrix) {
  return CGAffineTransformMake(
      SkScalarToCGFloat(matrix[SkMatrix::kMScaleX]), -SkScalarToCGFloat(matrix[SkMatrix::kMSkewY]),
      -SkScalarToCGFloat(matrix[SkMatrix::kMSkewX]), SkScalarToCGFloat(matrix[SkMatrix::kMScaleY]),
      SkScalarToCGFloat(matrix[SkMatrix::kMTransX]), SkScalarToCGFloat(matrix[SkMatrix::kMTransY]));
}
...

{
  ...
  fCTFont = SkCTFontCreateExactCopy(
...
CGRGBPixel* SkScalerContext_Mac::Offscreen::getCG(
...
static void RGBToA8(
    ...
    void* glyphImage, const uint8_t* table8) {
...
static void RGBToLcd16(
    const CGRGBPixel* SK_RESTRICT cgPixels, size_t cgRowBytes, const SkGlyph& glyph,
    void* glyphImage, const uint8_t* tableR, const uint8_t* tableG, const uint8_t* tableB) {
  const int width = glyph.width();
  const int height = glyph.height();
  size_t dstRB = glyph.rowBytes();
  ...
  switch (glyph.fMaskFormat) {
    case SkMask::kLCD16_Format: {
      if (fPreBlend.isApplicable()) {
        RGBToLcd16<true>(
            cgPixels, cgRowBytes, glyph, glyph.fImage, fPreBlend.fR, fPreBlend.fG, fPreBlend.fB);
      } else {
        RGBToLcd16<false>(
            cgPixels, cgRowBytes, glyph, glyph.fImage, fPreBlend.fR, fPreBlend.fG, fPreBlend.fB);
      }
...
bool SkScalerContext_Mac::generatePath(SkGlyphID glyph, SkPath* path) {
  ...
  if (fDoSubPosition) {
    ...
    CGAffineTransform scale(
        CGAffineTransformMakeScale(SkScalarToCGFloat(scaleX), SkScalarToCGFloat(scaleY)));
