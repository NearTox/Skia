...
bool SkHeifCodec::IsSupported(const void* buffer, size_t bytesRead, SkEncodedImageFormat* format) {
  ...
  for (size_t i = 0; i < numCompatibleBrands + 2; ++i) {
    ...
    if (brand == FOURCC('m', 'i', 'f', '1') || brand == FOURCC('h', 'e', 'i', 'c') ||
      ...
      isHeif = true;
      if (brand == FOURCC('a', 'v', 'i', 'f') || brand == FOURCC('a', 'v', 'i', 's')) {
        if (format != nullptr) {
...
std::unique_ptr<SkCodec> SkHeifCodec::MakeFromStream(
    std::unique_ptr<SkStream> stream, SkCodec::SelectionPolicy selectionPolicy,
    SkEncodedImageFormat format, Result* result) {
  ...
  if (selectionPolicy == SkCodec::SelectionPolicy::kPreferAnimation) {
    ...
    if (heifDecoder->getSequenceInfo(&sequenceInfo, &frameCount) && frameCount > 1) {
  ...
  SkEncodedInfo info = SkEncodedInfo::Make(
      heifInfo.mWidth, heifInfo.mHeight, SkEncodedInfo::kYUV_Color, SkEncodedInfo::kOpaque_Alpha, 8,
      std::move(profile));
  SkEncodedOrigin orientation = get_orientation(heifInfo);

  *result = kSuccess;
  return std::unique_ptr<SkCodec>(
      new SkHeifCodec(std::move(info), heifDecoder.release(), orientation, frameCount > 1, format));
}

SkHeifCodec::SkHeifCodec(
    SkEncodedInfo&& info, HeifDecoder* heifDecoder, SkEncodedOrigin origin, bool useAnimation,
...
bool SkHeifCodec::conversionSupported(
  ...
  switch (dstInfo.colorType()) {
    case kRGBA_8888_SkColorType: return fHeifDecoder->setOutputColor(kHeifColorFormat_RGBA_8888);

    case kBGRA_8888_SkColorType: return fHeifDecoder->setOutputColor(kHeifColorFormat_BGRA_8888);
...
int SkHeifCodec::onGetFrameCount() {
  ...
  if (fFrameHolder.size() == 0) {
    ...
    if (!fHeifDecoder->getSequenceInfo(&frameInfo, &frameCount) || frameCount <= 1) {
      fUseAnimation = false;
      return 1;
    }
 ...
 */
SkCodec::Result SkHeifCodec::onGetPixels(
  ...
  if (fUseAnimation) {
    ...
    fFrameHolder.editFrameAt(options.fFrameIndex)->setDuration(fFrameInfo.mDurationUs / 1000);
  } else {
