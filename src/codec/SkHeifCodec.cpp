...
std::unique_ptr<SkCodec> SkHeifCodec::MakeFromStream(
    std::unique_ptr<SkStream> stream, SkCodec::SelectionPolicy selectionPolicy, Result* result) {
  ...
  *result = kSuccess;
  return std::unique_ptr<SkCodec>(
      new SkHeifCodec(std::move(info), heifDecoder.release(), orientation, frameCount > 1));
}
