...
static bool parse_encoded_origin(
    const uint8_t* exifData, size_t data_length, uint64_t offset, bool littleEndian, bool is_root,
  ...
  for (uint32_t i = 0; i < numEntries; i++, data += kEntrySize) {
    ...
    if (kOriginTag == tag && kOriginType == type && 1 == count) {
      ...
      if (0 < subifd && subifd < data_length) {
        if (parse_encoded_origin(exifData, data_length, subifd, littleEndian, false, orientation)) {
          return true;
        }
      }
    }
