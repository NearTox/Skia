...
static inline bool process_data(
    png_structp png_ptr, png_infop info_ptr, SkStream* stream, void* buffer, size_t bufferSize,
    size_t length) {
  while (length > 0) {
...
bool SkPngCodec::createColorTable(const SkImageInfo& dstInfo) {
  ...
  if (numColorsWithAlpha < numColors) {
    ...
    if (is_rgba(tableColorType)) {
      SkOpts::RGB_to_RGB1(
          colorTable + numColorsWithAlpha, (const uint8_t*)palette, numColors - numColorsWithAlpha);
    } else {
      SkOpts::RGB_to_BGR1(
          colorTable + numColorsWithAlpha, (const uint8_t*)palette, numColors - numColorsWithAlpha);
    }
...
std::unique_ptr<SkEncodedInfo::ICCProfile> read_color_profile(
  ...
  if (png_get_cHRM_fixed(
          png_ptr, info_ptr, &chrm[0], &chrm[1], &chrm[2], &chrm[3], &chrm[4], &chrm[5], &chrm[6],
          &chrm[7])) {
...
void SkPngCodec::allocateStorage(const SkImageInfo& dstInfo) {
  switch (fXformMode) {
    case kSwizzleOnly_XformMode: break;
...
void SkPngCodec::applyXformRow(void* dst, const void* src) {
  switch (fXformMode) {
    case kSwizzleOnly_XformMode: fSwizzler->swizzle(dst, (const uint8_t*)src); break;
    case kColorOnly_XformMode: this->applyColorXform(dst, src, fXformWidth); break;
    case kSwizzleColor_XformMode:
...
class SkPngNormalDecoder : public SkPngCodec {
 public:
  SkPngNormalDecoder(
      SkEncodedInfo&& info, std::unique_ptr<SkStream> stream, SkPngChunkReader* reader,
  ...
  static void AllRowsCallback(
      png_structp png_ptr, png_bytep row, png_uint_32 rowNum, int /*pass*/) {
...
class SkPngInterlacedDecoder : public SkPngCodec {
 public:
  ...
  static void InterlacedRowCallback(
      png_structp png_ptr, png_bytep row, png_uint_32 rowNum, int pass) {
...
static SkCodec::Result read_header(
    SkStream* stream, SkPngChunkReader* chunkReader, SkCodec** outCodec, png_structp* png_ptrp,
    png_infop* info_ptrp) {
...
void AutoCleanPng::infoCallback(size_t idatLength) {
  ...
  png_get_IHDR(
      fPng_ptr, fInfo_ptr, &origWidth, &origHeight, &bitDepth, &encodedColorType, nullptr, nullptr,
      nullptr);

  // TODO: Should we support 16-bits of precision for gray images?
  if (bitDepth == 16 &&
      (PNG_COLOR_TYPE_GRAY == encodedColorType || PNG_COLOR_TYPE_GRAY_ALPHA == encodedColorType)) {
    bitDepth = 8;
  ...
  switch (encodedColorType) {
    case PNG_COLOR_TYPE_PALETTE:
      ...
      alpha = png_get_valid(fPng_ptr, fInfo_ptr, PNG_INFO_tRNS) ? SkEncodedInfo::kUnpremul_Alpha
  ...
  if (fOutCodec) {
    ...
    if (profile) {
      switch (profile->profile()->data_color_space) {
        case skcms_Signature_CMYK: profile = nullptr; break;
...
#ifdef SK_BUILD_FOR_ANDROID_FRAMEWORK
    if (encodedColorType != PNG_COLOR_TYPE_GRAY_ALPHA && SkEncodedInfo::kOpaque_Alpha == alpha) {
      png_color_8p sigBits;
      if (png_get_sBIT(fPng_ptr, fInfo_ptr, &sigBits)) {
        if (5 == sigBits->red && 6 == sigBits->green && 5 == sigBits->blue) {
    ...
    if (1 == numberPasses) {
      *fOutCodec = new SkPngNormalDecoder(
          std::move(encodedInfo), std::unique_ptr<SkStream>(fStream), fChunkReader, fPng_ptr,
          fInfo_ptr, bitDepth);
    } else {
      *fOutCodec = new SkPngInterlacedDecoder(
          std::move(encodedInfo), std::unique_ptr<SkStream>(fStream), fChunkReader, fPng_ptr,
          fInfo_ptr, bitDepth, numberPasses);
    }
...
SkPngCodec::SkPngCodec(
    SkEncodedInfo&& encodedInfo, std::unique_ptr<SkStream> stream, SkPngChunkReader* chunkReader,
...
void SkPngCodec::initializeXformParams() {
  switch (fXformMode) {
    case kColorOnly_XformMode: fXformWidth = this->dstInfo().width(); break;
    case kSwizzleColor_XformMode: fXformWidth = this->swizzler()->swizzleWidth(); break;
    default: break;
  }
}

void SkPngCodec::initializeSwizzler(
    const SkImageInfo& dstInfo, const Options& options, bool skipFormatConversion) {
  ...
  if (skipFormatConversion) {
    ...
    switch (this->getEncodedInfo().color()) {
      ...
      case SkEncodedInfo::kRGBA_Color:
        ...
        break;
      case SkEncodedInfo::kGray_Color: srcBPP = 1; break;
...
SkCodec::Result SkPngCodec::onGetPixels(
...
SkCodec::Result SkPngCodec::onStartIncrementalDecode(
...
std::unique_ptr<SkCodec> SkPngCodec::MakeFromStream(
    std::unique_ptr<SkStream> stream, Result* result, SkPngChunkReader* chunkReader) {
