...
std::unique_ptr<SkCodec> SkWebpCodec::MakeFromStream(
  ...
  switch (state) {
    case WEBP_DEMUX_PARSE_ERROR: *result = kInvalidInput; return nullptr;
    case WEBP_DEMUX_PARSING_HEADER: *result = kIncompleteInput; return nullptr;
    case WEBP_DEMUX_PARSED_HEADER:
    case WEBP_DEMUX_DONE: SkASSERT(demux); break;
  }
  ...
  switch (WebPGetFeatures(frame.fragment.bytes, frame.fragment.size, &features)) {
    case VP8_STATUS_OK: break;
    case VP8_STATUS_SUSPENDED:
    case VP8_STATUS_NOT_ENOUGH_DATA: *result = kIncompleteInput; return nullptr;
    default: *result = kInvalidInput; return nullptr;
  }

  const bool hasAlpha = SkToBool(frame.has_alpha) || frame.width != width || frame.height != height;
  ...
  *result = kSuccess;
  ...
  return std::unique_ptr<SkCodec>(new SkWebpCodec(
      std::move(info), std::move(stream), demux.release(), std::move(data), origin));
}

static WEBP_CSP_MODE webp_decode_mode(SkColorType dstCT, bool premultiply) {
  switch (dstCT) {
    case kBGRA_8888_SkColorType: return premultiply ? MODE_bgrA : MODE_BGRA;
    case kRGBA_8888_SkColorType: return premultiply ? MODE_rgbA : MODE_RGBA;
    case kRGB_565_SkColorType: return MODE_RGB_565;
    default: return MODE_LAST;
  }
}

SkWebpCodec::Frame* SkWebpCodec::FrameHolder::appendNewFrame(bool hasAlpha) {
  const int i = this->size();
  fFrames.emplace_back(i, hasAlpha ? SkEncodedInfo::kUnpremul_Alpha : SkEncodedInfo::kOpaque_Alpha);
  return &fFrames[i];
}
...
static void blend_line(
    SkColorType dstCT, void* dst, SkColorType srcCT, const void* src, SkAlphaType dstAt,
    bool srcHasAlpha, int width) {
...
SkCodec::Result SkWebpCodec::onGetPixels(
  ...
  config.output.colorspace =
      webp_decode_mode(webpInfo.colorType(), webpInfo.alphaType() == kPremul_SkAlphaType);
  ...
  switch (WebPIUpdate(idec, frame.fragment.bytes, frame.fragment.size)) {
    case VP8_STATUS_OK:
      ...
      break;
    case VP8_STATUS_SUSPENDED:
      if (!WebPIDecGetRGB(idec, &rowsDecoded, nullptr, nullptr, nullptr) || rowsDecoded <= 0) {
        return kInvalidInput;
      }
      *rowsDecodedPtr = rowsDecoded + dstY;
      result = kIncompleteInput;
      break;
    default: return kInvalidInput;
  }
  ...
  if (this->colorXform()) {
    ...
    for (int y = 0; y < rowsDecoded; y++) {
      ...
      if (blendWithPrevFrame) {
        blend_line(dstCT, dst, dstCT, xformDst, dstInfo.alphaType(), frame.has_alpha, scaledWidth);
    ...
    for (int y = 0; y < rowsDecoded; y++) {
      blend_line(
          dstCT, dst, webpDst.colorType(), src, dstInfo.alphaType(), frame.has_alpha, scaledWidth);
...
SkWebpCodec::SkWebpCodec(
    SkEncodedInfo&& info, std::unique_ptr<SkStream> stream, WebPDemuxer* demux, sk_sp<SkData> data,
