 ...
 */
SkBmpStandardCodec::SkBmpStandardCodec(
 ...
 */
SkCodec::Result SkBmpStandardCodec::onGetPixels(
    const SkImageInfo& dstInfo, void* dst, size_t dstRowBytes, const Options& opts,
    int* rowsDecoded) {
  if (opts.fSubset) {
...
static SkEncodedInfo make_info(
    SkEncodedInfo::Color color, SkEncodedInfo::Alpha alpha, int bitsPerPixel) {
...
SkEncodedInfo SkBmpStandardCodec::swizzlerInfo() const {
  ...
  if (fInIco) {
    if (this->bitsPerPixel() <= 8) {
      return make_info(SkEncodedInfo::kPalette_Color, info.alpha(), this->bitsPerPixel());
    }
    if (this->bitsPerPixel() == 24) {
      return make_info(SkEncodedInfo::kBGR_Color, SkEncodedInfo::kOpaque_Alpha, 8);
    }
  }
...
SkCodec::Result SkBmpStandardCodec::onPrepareToDecode(
 ...
 */
int SkBmpStandardCodec::decodeRows(
    const SkImageInfo& dstInfo, void* dst, size_t dstRowBytes, const Options& opts) {
  ...
  if (fInIco && fIsOpaque) {
    ...
    const size_t bytesToSkip =
        remainingScanlines * this->srcRowBytes() + startScanline * fAndMaskRowBytes;
    ...
    const void* subStreamMemoryBase = SkTAddOffset<const void>(memoryBase, subStreamStartPosition);
...
void SkBmpStandardCodec::decodeIcoMask(
    SkStream* stream, const SkImageInfo& dstInfo, void* dst, size_t dstRowBytes) {
  // BMP in ICO have transparency, so this cannot be 565. The below code depends
  // on the output being an SkPMColor.
  SkASSERT(
      kRGBA_8888_SkColorType == dstInfo.colorType() ||
