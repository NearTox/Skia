...
static void copy(void* dst, const uint8_t* src, int width, int bpp, int deltaSrc, int offset,
                 const SkPMColor ctable[]) noexcept {
...
static void swizzle_bit_to_grayscale(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
...
static void swizzle_bit_to_n32(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                               int dstWidth, int bpp, int deltaSrc, int offset,
                               const SkPMColor* /*ctable*/) noexcept {
...
static void swizzle_bit_to_565(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                               int dstWidth, int bpp, int deltaSrc, int offset,
                               const SkPMColor* /*ctable*/) noexcept {
...
static void swizzle_bit_to_f16(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                               int dstWidth, int bpp, int deltaSrc, int offset,
                               const SkPMColor* /*ctable*/) noexcept {
    constexpr uint64_t kWhite = (((uint64_t)SK_Half1) << 0) | (((uint64_t)SK_Half1) << 16) |
                                (((uint64_t)SK_Half1) << 32) | (((uint64_t)SK_Half1) << 48);
    constexpr uint64_t kBlack = (((uint64_t)0) << 0) | (((uint64_t)0) << 16) |
                                (((uint64_t)0) << 32) | (((uint64_t)SK_Half1) << 48);
...
static void swizzle_small_index_to_565(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                                       int dstWidth, int bpp, int deltaSrc, int offset,
                                       const SkPMColor ctable[]) noexcept {
...
static void swizzle_small_index_to_n32(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                                       int dstWidth, int bpp, int deltaSrc, int offset,
                                       const SkPMColor ctable[]) noexcept {
...
static void swizzle_index_to_n32(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                                 int dstWidth, int bpp, int deltaSrc, int offset,
                                 const SkPMColor ctable[]) noexcept {
    src += offset;
    SkPMColor* SK_RESTRICT dst = (SkPMColor*)dstRow;
...
static void swizzle_index_to_n32_skipZ(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                                       int dstWidth, int bpp, int deltaSrc, int offset,
                                       const SkPMColor ctable[]) noexcept {
    src += offset;
    SkPMColor* SK_RESTRICT dst = (SkPMColor*)dstRow;
...
static void swizzle_index_to_565(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                                 int dstWidth, int bytesPerPixel, int deltaSrc, int offset,
                                 const SkPMColor ctable[]) noexcept {
    src += offset;
    uint16_t* SK_RESTRICT dst = (uint16_t*)dstRow;
    for (int x = 0; x < dstWidth; x++) {
...
static void swizzle_gray_to_n32(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                                int dstWidth, int bpp, int deltaSrc, int offset,
                                const SkPMColor ctable[]) noexcept {
    src += offset;
    SkPMColor* SK_RESTRICT dst = (SkPMColor*)dstRow;
...
static void fast_swizzle_gray_to_n32(void* dst, const uint8_t* src, int width, int bpp,
                                     int deltaSrc, int offset, const SkPMColor ctable[]) noexcept {
    // This function must not be called if we are sampling.  If we are not
    // sampling, deltaSrc should equal bpp.
    SkASSERT(deltaSrc == bpp);
...
static void swizzle_gray_to_565(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                                int dstWidth, int bytesPerPixel, int deltaSrc, int offset,
                                const SkPMColor ctable[]) noexcept {
    src += offset;
    uint16_t* SK_RESTRICT dst = (uint16_t*)dstRow;
...
static void swizzle_grayalpha_to_n32_unpremul(void* dst, const uint8_t* src, int width, int bpp,
                                              int deltaSrc, int offset,
                                              const SkPMColor ctable[]) noexcept {
    src += offset;
...
static void fast_swizzle_grayalpha_to_n32_unpremul(void* dst, const uint8_t* src, int width,
                                                   int bpp, int deltaSrc, int offset,
                                                   const SkPMColor ctable[]) noexcept {
    // This function must not be called if we are sampling.  If we are not
    // sampling, deltaSrc should equal bpp.
    SkASSERT(deltaSrc == bpp);
...
static void swizzle_grayalpha_to_n32_premul(void* dst, const uint8_t* src, int width, int bpp,
                                            int deltaSrc, int offset,
                                            const SkPMColor ctable[]) noexcept {
    src += offset;
    SkPMColor* dst32 = (SkPMColor*)dst;
...
static void fast_swizzle_grayalpha_to_n32_premul(void* dst, const uint8_t* src, int width, int bpp,
                                                 int deltaSrc, int offset,
                                                 const SkPMColor ctable[]) noexcept {
    // This function must not be called if we are sampling.  If we are not
    // sampling, deltaSrc should equal bpp.
...
static void swizzle_grayalpha_to_a8(void* dst, const uint8_t* src, int width, int bpp, int deltaSrc,
                                    int offset, const SkPMColor[]) noexcept {
    src += offset;
    uint8_t* dst8 = (uint8_t*)dst;
    for (int x = 0; x < width; ++x) {
...
static void swizzle_bgr_to_565(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                               int dstWidth, int bpp, int deltaSrc, int offset,
                               const SkPMColor ctable[]) noexcept {
    src += offset;
    uint16_t* SK_RESTRICT dst = (uint16_t*)dstRow;
...
static void swizzle_rgb_to_rgba(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                                int dstWidth, int bpp, int deltaSrc, int offset,
                                const SkPMColor ctable[]) noexcept {
    src += offset;
    SkPMColor* SK_RESTRICT dst = (SkPMColor*)dstRow;
...
static void swizzle_rgb_to_bgra(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                                int dstWidth, int bpp, int deltaSrc, int offset,
                                const SkPMColor ctable[]) noexcept {
    src += offset;
    SkPMColor* SK_RESTRICT dst = (SkPMColor*)dstRow;
...
static void fast_swizzle_rgb_to_rgba(void* dst, const uint8_t* src, int width, int bpp,
                                     int deltaSrc, int offset, const SkPMColor ctable[]) noexcept {
    // This function must not be called if we are sampling.  If we are not
    // sampling, deltaSrc should equal bpp.
    SkASSERT(deltaSrc == bpp);
...
static void fast_swizzle_rgb_to_bgra(void* dst, const uint8_t* src, int width, int bpp,
                                     int deltaSrc, int offset, const SkPMColor ctable[]) noexcept {
    // This function must not be called if we are sampling.  If we are not
    // sampling, deltaSrc should equal bpp.
    SkASSERT(deltaSrc == bpp);
...
static void swizzle_rgb_to_565(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                               int dstWidth, int bytesPerPixel, int deltaSrc, int offset,
                               const SkPMColor ctable[]) noexcept {
    src += offset;
    uint16_t* SK_RESTRICT dst = (uint16_t*)dstRow;
...
static void swizzle_rgba_to_rgba_premul(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                                        int dstWidth, int bpp, int deltaSrc, int offset,
                                        const SkPMColor ctable[]) noexcept {
    src += offset;
    SkPMColor* SK_RESTRICT dst = (SkPMColor*)dstRow;
...
static void swizzle_rgba_to_bgra_premul(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                                        int dstWidth, int bpp, int deltaSrc, int offset,
                                        const SkPMColor ctable[]) noexcept {
    src += offset;
    SkPMColor* SK_RESTRICT dst = (SkPMColor*)dstRow;
...
static void fast_swizzle_rgba_to_rgba_premul(void* dst, const uint8_t* src, int width, int bpp,
                                             int deltaSrc, int offset,
                                             const SkPMColor ctable[]) noexcept {
    // This function must not be called if we are sampling.  If we are not
    // sampling, deltaSrc should equal bpp.
...
static void fast_swizzle_rgba_to_bgra_premul(void* dst, const uint8_t* src, int width, int bpp,
                                             int deltaSrc, int offset,
                                             const SkPMColor ctable[]) noexcept {
    // This function must not be called if we are sampling.  If we are not
    // sampling, deltaSrc should equal bpp.
...
static void swizzle_rgba_to_bgra_unpremul(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                                          int dstWidth, int bpp, int deltaSrc, int offset,
                                          const SkPMColor ctable[]) noexcept {
    src += offset;
...
static void fast_swizzle_rgba_to_bgra_unpremul(void* dst, const uint8_t* src, int width, int bpp,
                                               int deltaSrc, int offset,
                                               const SkPMColor ctable[]) noexcept {
    // This function must not be called if we are sampling.  If we are not
    // sampling, deltaSrc should equal bpp.
...
static void swizzle_rgb16_to_rgba(void* dst, const uint8_t* src, int width, int bpp, int deltaSrc,
                                  int offset, const SkPMColor ctable[]) noexcept {
...
static void swizzle_rgb16_to_bgra(void* dst, const uint8_t* src, int width, int bpp, int deltaSrc,
                                  int offset, const SkPMColor ctable[]) noexcept {
    auto strip16to8 = [](const uint8_t* ptr) {
...
static void swizzle_rgb16_to_565(void* dst, const uint8_t* src, int width, int bpp, int deltaSrc,
                                 int offset, const SkPMColor ctable[]) noexcept {
    auto strip16to565 = [](const uint8_t* ptr) { return SkPack888ToRGB16(ptr[0], ptr[2], ptr[4]); };
...
static void swizzle_rgba16_to_rgba_unpremul(void* dst, const uint8_t* src, int width, int bpp,
                                            int deltaSrc, int offset,
                                            const SkPMColor ctable[]) noexcept {
    auto strip16to8 = [](const uint8_t* ptr) {
...
static void swizzle_rgba16_to_rgba_premul(void* dst, const uint8_t* src, int width, int bpp,
                                          int deltaSrc, int offset,
                                          const SkPMColor ctable[]) noexcept {
...
static void swizzle_rgba16_to_bgra_unpremul(void* dst, const uint8_t* src, int width, int bpp,
                                            int deltaSrc, int offset,
                                            const SkPMColor ctable[]) noexcept {
    auto strip16to8 = [](const uint8_t* ptr) {
...
static void swizzle_rgba16_to_bgra_premul(void* dst, const uint8_t* src, int width, int bpp,
                                          int deltaSrc, int offset,
                                          const SkPMColor ctable[]) noexcept {
    auto stripAndPremul16to8 = [](const uint8_t* ptr) {
...
// G = M * K / 255
// B = Y * K / 255
static void swizzle_cmyk_to_rgba(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                                 int dstWidth, int bpp, int deltaSrc, int offset,
                                 const SkPMColor ctable[]) noexcept {
    src += offset;
    SkPMColor* SK_RESTRICT dst = (SkPMColor*)dstRow;
...
static void swizzle_cmyk_to_bgra(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                                 int dstWidth, int bpp, int deltaSrc, int offset,
                                 const SkPMColor ctable[]) noexcept {
    src += offset;
    SkPMColor* SK_RESTRICT dst = (SkPMColor*)dstRow;
...
static void fast_swizzle_cmyk_to_rgba(void* dst, const uint8_t* src, int width, int bpp,
                                      int deltaSrc, int offset, const SkPMColor ctable[]) noexcept {
    // This function must not be called if we are sampling.  If we are not
    // sampling, deltaSrc should equal bpp.
    SkASSERT(deltaSrc == bpp);
...
static void fast_swizzle_cmyk_to_bgra(void* dst, const uint8_t* src, int width, int bpp,
                                      int deltaSrc, int offset, const SkPMColor ctable[]) noexcept {
    // This function must not be called if we are sampling.  If we are not
    // sampling, deltaSrc should equal bpp.
    SkASSERT(deltaSrc == bpp);
...
static void swizzle_cmyk_to_565(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                                int dstWidth, int bpp, int deltaSrc, int offset,
                                const SkPMColor ctable[]) noexcept {
    src += offset;
    uint16_t* SK_RESTRICT dst = (uint16_t*)dstRow;
...
void SkSwizzler::SkipLeadingGrayAlphaZerosThen(void* dst, const uint8_t* src, int width, int bpp,
...
template <SkSwizzler::RowProc proc>
void SkSwizzler::SkipLeading8888ZerosThen(void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src,
                                          int dstWidth, int bpp, int deltaSrc, int offset,
...
std::unique_ptr<SkSwizzler> SkSwizzler::MakeSimple(int srcBPP, const SkImageInfo& dstInfo,
    ...
    return Make(dstInfo, &copy, proc, nullptr /*ctable*/, srcBPP, dstInfo.bytesPerPixel(), options,
                nullptr /*frame*/);
}
                ...
                case kBGRA_8888_SkColorType:
                    ...
                    if (premultiply) {
                        ...
                        if (SkCodec::kYes_ZeroInitialized == zeroInit) {
                            ...
                            fastProc =
                                    &SkipLeading8888ZerosThen<fast_swizzle_rgba_to_bgra_unpremul>;
                        } else {
...
std::unique_ptr<SkSwizzler> SkSwizzler::Make(const SkImageInfo& dstInfo, RowProc fastProc,
...
int SkSwizzler::onSetSampleX(int sampleX) noexcept {
    SkASSERT(sampleX > 0);
