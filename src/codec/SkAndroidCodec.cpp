...
SkAndroidCodec::SkAndroidCodec(SkCodec* codec) : fInfo(codec->getInfo()), fCodec(codec) {}
...
std::unique_ptr<SkAndroidCodec> SkAndroidCodec::MakeFromCodec(std::unique_ptr<SkCodec> codec) {
  ...
  switch ((SkEncodedImageFormat)codec->getEncodedFormat()) {
    ...
    case SkEncodedImageFormat::kAVIF: return std::make_unique<SkSampledCodec>(codec.release());
...
std::unique_ptr<SkAndroidCodec> SkAndroidCodec::MakeFromData(
    sk_sp<SkData> data, SkPngChunkReader* chunkReader) {
  if (!data) {
    return nullptr;
  }
...
SkColorType SkAndroidCodec::computeOutputColorType(SkColorType requestedColorType) {
  ...
  switch (requestedColorType) {
    ...
    case kRGBA_1010102_SkColorType:
      ...
      break;
    case kRGBA_F16_SkColorType: return kRGBA_F16_SkColorType;
    default: break;
  }
...
sk_sp<SkColorSpace> SkAndroidCodec::computeOutputColorSpace(
    SkColorType outputColorType, sk_sp<SkColorSpace> prefColorSpace) {
...
int SkAndroidCodec::computeSampleSize(SkISize* desiredSize) const {
  ...
  if (desiredSize->width() < 1 || desiredSize->height() < 1) {
    *desiredSize =
        SkISize::Make(std::max(1, desiredSize->width()), std::max(1, desiredSize->height()));
  }
...
SkCodec::Result SkAndroidCodec::getAndroidPixels(
  ...
  if (auto result =
          fCodec->handleFrameIndex(requestInfo, requestPixels, requestRowBytes, *options, this);
