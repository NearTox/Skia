...
public:
    static bool IsJpeg(const void*, size_t) noexcept;
...
protected:
    ...
    bool onQueryYUV8(SkYUVASizeInfo* sizeInfo, SkYUVColorSpace* colorSpace) const noexcept override;
...
private:
    ...
    static std::unique_ptr<SkCodec> MakeFromStream(
            std::unique_ptr<SkStream>, Result*,
     ...
     */
    static Result ReadHeader(SkStream* stream, SkCodec** codecOut, JpegDecoderMgr** decoderMgrOut,
                             std::unique_ptr<SkEncodedInfo::ICCProfile> defaultColorProfile);
     ...
     *                   takes ownership
     */
    SkJpegCodec(SkEncodedInfo&& info, std::unique_ptr<SkStream> stream, JpegDecoderMgr* decoderMgr,
