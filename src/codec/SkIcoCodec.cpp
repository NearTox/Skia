...
std::unique_ptr<SkCodec> SkIcoCodec::MakeFromStream(
  ...
  *result = kSuccess;
  return std::unique_ptr<SkCodec>(
      new SkIcoCodec(std::move(maxInfo), std::move(stream), codecs.release()));
}

SkIcoCodec::SkIcoCodec(
    SkEncodedInfo&& info, std::unique_ptr<SkStream> stream,
 ...
 */
SkCodec::Result SkIcoCodec::onGetPixels(
...
SkCodec::Result SkIcoCodec::onStartScanlineDecode(
...
SkCodec::Result SkIcoCodec::onStartIncrementalDecode(
  ...
  while (true) {
    ...
    switch (embeddedCodec->startIncrementalDecode(dstInfo, pixels, rowBytes, &options)) {
      case kSuccess: fCurrCodec = embeddedCodec; return kSuccess;
      case kUnimplemented:
