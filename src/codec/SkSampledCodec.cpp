...
SkISize SkSampledCodec::onGetSampledDimensions(int sampleSize) const {
  ...
  return SkISize::Make(
      get_scaled_dimension(size.width(), sampleSize),
      get_scaled_dimension(size.height(), sampleSize));
}

SkCodec::Result SkSampledCodec::onGetAndroidPixels(
  ...
  AndroidOptions subsetOptions = options;
  {
    ...
    SkIRect incrementalSubset =
        SkIRect::MakeXYWH(scaledSubsetX, scaledSubsetY, scaledSubsetWidth, scaledSubsetHeight);
    subsetOptions.fSubset = &incrementalSubset;
    const SkCodec::Result startResult =
        this->codec()->startIncrementalDecode(scaledInfo, pixels, rowBytes, &subsetOptions);
    if (SkCodec::kSuccess == startResult) {
      ...
      this->codec()->fillIncompleteImage(
          scaledInfo, pixels, rowBytes, options.fZeroInitialized, scaledSubsetHeight, rowsDecoded);
      return incResult;
  ...
  SkIRect scanlineSubset =
      SkIRect::MakeXYWH(scaledSubsetX, 0, scaledSubsetWidth, scaledSize.height());
  subsetOptions.fSubset = &scanlineSubset;

  SkCodec::Result result = this->codec()->startScanlineDecode(scaledInfo, &subsetOptions);
  ...
  if (!this->codec()->skipScanlines(scaledSubsetY)) {
    this->codec()->fillIncompleteImage(
        info, pixels, rowBytes, options.fZeroInitialized, scaledSubsetHeight, 0);
...
SkCodec::Result SkSampledCodec::sampledDecode(
    const SkImageInfo& info, void* pixels, size_t rowBytes, const AndroidOptions& options) {
...

  {
    ...
    const SkCodec::Result startResult =
        this->codec()->startIncrementalDecode(nativeInfo, pixels, rowBytes, &incrementalOptions);
    if (SkCodec::kSuccess == startResult) {
  ...
  SkCodec::Result result = this->codec()->startScanlineDecode(nativeInfo, &sampledOptions);
  ...
  switch (this->codec()->getScanlineOrder()) {
    case SkCodec::kTopDown_SkScanlineOrder: {
      if (!this->codec()->skipScanlines(startY)) {
        this->codec()->fillIncompleteImage(
            info, pixels, rowBytes, options.fZeroInitialized, dstHeight, 0);
        return SkCodec::kIncompleteInput;
      }
      ...
      for (int y = 0; y < dstHeight; y++) {
        if (1 != this->codec()->getScanlines(pixelPtr, 1, rowBytes)) {
          this->codec()->fillIncompleteImage(
              info, pixels, rowBytes, options.fZeroInitialized, dstHeight, y + 1);
          return SkCodec::kIncompleteInput;
        }
        if (y < dstHeight - 1) {
    ...
    case SkCodec::kBottomUp_SkScanlineOrder: {
      ...
      for (y = 0; y < nativeSize.height(); y++) {
        ...
        if (is_coord_necessary(srcY, sampleY, dstHeight)) {
          void* pixelPtr = SkTAddOffset<void>(pixels, rowBytes * get_dst_coord(srcY, sampleY));
          if (1 != this->codec()->getScanlines(pixelPtr, 1, rowBytes)) {
            break;
          }
        } else {
    ...
    default: SkASSERT(false); return SkCodec::kUnimplemented;
  }
}
