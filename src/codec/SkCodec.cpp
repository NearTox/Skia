...
bool SkCodec::conversionSupported(const SkImageInfo& dst, bool srcIsOpaque, bool needsColorXform) {
  ...
  switch (dst.colorType()) {
    ...
    case kBGRA_8888_SkColorType: return true;
    case kRGBA_F16_SkColorType: return dst.colorSpace();
  ...
  if (requiredFrame != kNoFrame) {
    if (options.fPriorFrame != kNoFrame) {
      ...
      switch (prevFrame->getDisposalMethod()) {
        case SkCodecAnimation::DisposalMethod::kRestorePrevious: return kInvalidParameters;
...
SkCodec::Result SkCodec::getPixels(
    const SkImageInfo& dstInfo, void* pixels, size_t rowBytes, const Options* options) {
...
SkCodec::Result SkCodec::startIncrementalDecode(
    const SkImageInfo& dstInfo, void* pixels, size_t rowBytes, const SkCodec::Options* options) {
  fStartedIncrementalDecode = false;
...
SkCodec::Result SkCodec::startScanlineDecode(
    const SkImageInfo& dstInfo, const SkCodec::Options* options) {
