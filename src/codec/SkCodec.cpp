...
std::unique_ptr<SkCodec> SkCodec::MakeFromStream(
    std::unique_ptr<SkStream> stream, Result* outResult, SkPngChunkReader* chunkReader,
  ...
  if (SkPngCodec::IsPng(buffer, bytesRead)) {
    ...
    if (SkHeifCodec::IsSupported(buffer, bytesRead, &format)) {
      return SkHeifCodec::MakeFromStream(std::move(stream), selectionPolicy, format, outResult);
    }
#endif
...
void SkCodec::setSrcXformFormat(XformFormat pixelFormat) { fSrcXformFormat = pixelFormat; }
...
bool SkCodec::conversionSupported(const SkImageInfo& dst, bool srcIsOpaque, bool needsColorXform) {
  ...
  switch (dst.colorType()) {
    ...
    case kRGB_565_SkColorType: return srcIsOpaque;
...
bool zero_rect(
    const SkImageInfo& dstInfo, void* pixels, size_t rowBytes, SkISize srcDimensions,
    SkIRect prevRect) {
...
SkCodec::Result SkCodec::handleFrameIndex(
    const SkImageInfo& info, void* pixels, size_t rowBytes, const Options& options,
...
SkCodec::Result SkCodec::getPixels(
    const SkImageInfo& info, void* pixels, size_t rowBytes, const Options* options) {
  ...
  if ((kIncompleteInput == result || kErrorInInput == result) && rowsDecoded != info.height()) {
    ...
    this->fillIncompleteImage(
        info, pixels, rowBytes, options->fZeroInitialized, info.height(), rowsDecoded);
  }
...
std::tuple<sk_sp<SkImage>, SkCodec::Result> SkCodec::getImage(
    const SkImageInfo& info, const Options* options) {
  SkBitmap bm;
  ...
  switch (result) {
    ...
    case kErrorInInput: bm.setImmutable(); return {bm.asImage(), result};
...
SkCodec::Result SkCodec::startScanlineDecode(
    const SkImageInfo& info, const SkCodec::Options* options) {
...
int SkCodec::getScanlines(void* dst, int countLines, size_t rowBytes) {
  ...
  if (linesDecoded < countLines) {
    this->fillIncompleteImage(
        this->dstInfo(), dst, rowBytes, this->options().fZeroInitialized, countLines, linesDecoded);
  }
...
int SkCodec::onOutputScanline(int inputScanline) const {
  switch (this->getScanlineOrder()) {
    case kTopDown_SkScanlineOrder: return inputScanline;
    case kBottomUp_SkScanlineOrder: return fEncodedInfo.height() - inputScanline - 1;
    default:
...
void SkCodec::fillIncompleteImage(
    const SkImageInfo& info, void* dst, size_t rowBytes, ZeroInitialized zeroInit,
...
bool sk_select_xform_format(
  ...
  switch (colorType) {
    case kRGBA_8888_SkColorType: *outFormat = skcms_PixelFormat_RGBA_8888; break;
    case kBGRA_8888_SkColorType: *outFormat = skcms_PixelFormat_BGRA_8888; break;
    case kRGB_565_SkColorType:
      ...
      *outFormat = skcms_PixelFormat_BGR_565;
      break;
    case kRGBA_F16_SkColorType: *outFormat = skcms_PixelFormat_RGBA_hhhh; break;
    case kGray_8_SkColorType: *outFormat = skcms_PixelFormat_G_8; break;
    default: return false;
  }
  return true;
}

bool SkCodec::initializeColorXform(
    const SkImageInfo& dstInfo, SkEncodedInfo::Alpha encodedAlpha, bool srcIsOpaque) {
...
void SkCodec::applyColorXform(void* dst, const void* src, int count) const {
  ...
  SkAssertResult(skcms_Transform(
      src, fSrcXformFormat, skcms_AlphaFormat_Unpremul, srcProfile, dst, fDstXformFormat,
      fDstXformAlphaFormat, &fDstProfile, count));
}
...
const char* SkCodec::ResultToString(Result result) {
  switch (result) {
    ...
    case kErrorInInput: return "error in input";
    case kInvalidConversion: return "invalid conversion";
    case kInvalidScale: return "invalid scale";
    case kInvalidParameters: return "invalid parameters";
    case kInvalidInput: return "invalid input";
    case kCouldNotRewind: return "could not rewind";
    case kInternalError: return "internal error";
    case kUnimplemented: return "unimplemented";
    default: SkASSERT(false); return "bogus result value";
  }
}
...
void SkFrame::fillIn(SkCodec::FrameInfo* frameInfo, bool fullyReceived) const {
  ...
  frameInfo->fAlphaType = fHasAlpha ? kUnpremul_SkAlphaType : kOpaque_SkAlphaType;
