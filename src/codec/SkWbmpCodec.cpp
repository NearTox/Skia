...
static inline size_t get_src_row_bytes(int width) { return SkAlign8(width) >> 3; }
...
static inline bool valid_color_type(const SkImageInfo& dstInfo) {
  switch (dstInfo.colorType()) {
    ...
    case kRGBA_F16_SkColorType: return dstInfo.colorSpace();
...
SkCodec::Result SkWbmpCodec::onGetPixels(
    const SkImageInfo& info, void* dst, size_t rowBytes, const Options& options, int* rowsDecoded) {
  if (options.fSubset) {
...
std::unique_ptr<SkCodec> SkWbmpCodec::MakeFromStream(
  ...
  *result = kSuccess;
  auto info = SkEncodedInfo::Make(
      size.width(), size.height(), SkEncodedInfo::kGray_Color, SkEncodedInfo::kOpaque_Alpha, 1);
...
SkCodec::Result SkWbmpCodec::onStartScanlineDecode(
