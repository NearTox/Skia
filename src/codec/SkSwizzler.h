...
class SkSwizzler : public SkSampler {
 public:
   ...
   */
  static std::unique_ptr<SkSwizzler> Make(
      const SkEncodedInfo& encodedInfo, const SkPMColor* ctable, const SkImageInfo& dstInfo,
      const SkCodec::Options&, const SkIRect* frame = nullptr);
   ...
   *  @return A new SkSwizzler or nullptr on failure.
   */
  static std::unique_ptr<SkSwizzler> MakeSimple(
      int srcBPP, const SkImageInfo& dstInfo, const SkCodec::Options&);
 ...
 private:
  ...
  static void SkipLeading8888ZerosThen(
      void* SK_RESTRICT dstRow, const uint8_t* SK_RESTRICT src, int dstWidth, int bpp, int deltaSrc,
      int offset, const SkPMColor ctable[]);

  template <RowProc Proc>
  static void SkipLeadingGrayAlphaZerosThen(
  ...
  static std::unique_ptr<SkSwizzler> Make(
      const SkImageInfo& dstInfo, RowProc fastProc, RowProc proc, const SkPMColor* ctable,
      int srcBPP, int dstBPP, const SkCodec::Options& options, const SkIRect* frame);
