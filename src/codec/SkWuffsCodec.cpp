...
class SkWuffsCodec final : public SkScalingCodec {
 public:
  SkWuffsCodec(
      ...
      std::unique_ptr<uint8_t, decltype(&sk_free)> pixbuf_ptr, bool pixbuf_zeroed,
      std::unique_ptr<uint8_t, decltype(&sk_free)> workbuf_ptr, size_t workbuf_len,
      wuffs_base__image_config imgcfg, wuffs_base__pixel_buffer pixbuf,
...
SkWuffsCodec::SkWuffsCodec(
    ...
    std::unique_ptr<uint8_t, decltype(&sk_free)> pixbuf_ptr, bool pixbuf_zeroed,
    std::unique_ptr<uint8_t, decltype(&sk_free)> workbuf_ptr, size_t workbuf_len,
    wuffs_base__image_config imgcfg, wuffs_base__pixel_buffer pixbuf, wuffs_base__io_buffer iobuf)
    : INHERITED(
