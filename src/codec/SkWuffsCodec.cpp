...
static SkCodecAnimation::DisposalMethod wuffs_disposal_to_skia_disposal(
  ...
  switch (w) {
    ...
    default: return SkCodecAnimation::DisposalMethod::kKeep;
  }
}
...
static SkCodec::Result reset_and_decode_image_config(
    wuffs_gif__decoder* decoder, wuffs_base__image_config* imgcfg, wuffs_base__io_buffer* b,
    SkStream* s) {
  ...
  switch (kN32_SkColorType) {
    case kBGRA_8888_SkColorType: pixfmt = WUFFS_BASE__PIXEL_FORMAT__BGRA_NONPREMUL; break;
    case kRGBA_8888_SkColorType: pixfmt = WUFFS_BASE__PIXEL_FORMAT__RGBA_NONPREMUL; break;
    default: return SkCodec::kInternalError;
  }
  if (imgcfg) {
    imgcfg->pixcfg.set(
        pixfmt, WUFFS_BASE__PIXEL_SUBSAMPLING__NONE, imgcfg->pixcfg.width(),
...
class SkWuffsCodec final : public SkScalingCodec {
 public:
  SkWuffsCodec(
      SkEncodedInfo&& encodedInfo, std::unique_ptr<SkStream> stream,
      std::unique_ptr<wuffs_gif__decoder, decltype(&sk_free)> dec,
      std::unique_ptr<uint8_t, decltype(&sk_free)> workbuf_ptr, size_t workbuf_len,
      wuffs_base__image_config imgcfg, wuffs_base__io_buffer iobuf);
 ...
 private:
  ...
  Result onStartIncrementalDecode(
      const SkImageInfo& dstInfo, void* dst, size_t rowBytes,
  ...
  Result onStartIncrementalDecodeOnePass(
      const SkImageInfo& dstInfo, uint8_t* dst, size_t rowBytes, const SkCodec::Options& options,
      uint32_t pixelFormat, size_t bytesPerPixel);
...
SkWuffsFrame::SkWuffsFrame(wuffs_base__frame_config* fc)
    : INHERITED(fc->index()),
      ...
      fReportedAlpha(
          fc->opaque_within_bounds() ? SkEncodedInfo::kOpaque_Alpha
  ...
  this->setBlend(
      fc->overwrite_instead_of_blend() ? SkCodecAnimation::Blend::kSrc
                                       : SkCodecAnimation::Blend::kSrcOver);
}

uint64_t SkWuffsFrame::ioPosition() const { return fIOPosition; }

SkEncodedInfo::Alpha SkWuffsFrame::onReportedAlpha() const { return fReportedAlpha; }
...
const SkFrame* SkWuffsFrameHolder::onGetFrame(int i) const { return fCodec->frame(i); };

// -------------------------------- SkWuffsCodec implementation

SkWuffsCodec::SkWuffsCodec(
    SkEncodedInfo&& encodedInfo, std::unique_ptr<SkStream> stream,
    std::unique_ptr<wuffs_gif__decoder, decltype(&sk_free)> dec,
    std::unique_ptr<uint8_t, decltype(&sk_free)> workbuf_ptr, size_t workbuf_len,
    wuffs_base__image_config imgcfg, wuffs_base__io_buffer iobuf)
    : INHERITED(
          std::move(encodedInfo), skcms_PixelFormat_RGBA_8888,
...
SkEncodedImageFormat SkWuffsCodec::onGetEncodedFormat() const { return SkEncodedImageFormat::kGIF; }

SkCodec::Result SkWuffsCodec::onGetPixels(
    const SkImageInfo& dstInfo, void* dst, size_t rowBytes, const Options& options,
    int* rowsDecoded) {
...
const SkFrameHolder* SkWuffsCodec::getFrameHolder() const { return &fFrameHolder; }

SkCodec::Result SkWuffsCodec::onStartIncrementalDecode(
    const SkImageInfo& dstInfo, void* dst, size_t rowBytes, const SkCodec::Options& options) {
  if (!dst) {
...
SkCodec::Result SkWuffsCodec::onStartIncrementalDecodeOnePass(
    const SkImageInfo& dstInfo, uint8_t* dst, size_t rowBytes, const SkCodec::Options& options,
    uint32_t pixelFormat, size_t bytesPerPixel) {
  wuffs_base__pixel_config pixelConfig;
  pixelConfig.set(
      pixelFormat, WUFFS_BASE__PIXEL_SUBSAMPLING__NONE, dstInfo.width(), dstInfo.height());
...
SkCodec::Result SkWuffsCodec::onStartIncrementalDecodeTwoPass() {
  ...
  if (!fTwoPassPixbufPtr) {
    ...
    void* pixbuf_ptr_raw =
        (pixbuf_len <= SIZE_MAX) ? sk_malloc_flags(pixbuf_len, SK_MALLOC_ZERO_INITIALIZE) : nullptr;
    if (!pixbuf_ptr_raw) {
      return SkCodec::kInternalError;
    }
...
SkCodec::Result SkWuffsCodec::onIncrementalDecodeTwoPass() {
  ...
  if (fFirstCallToIncrementalDecode) {
    ...
    auto bounds = SkIRect::MakeLTRB(
        frame_rect.min_incl_x, frame_rect.min_incl_y, frame_rect.max_excl_x, frame_rect.max_excl_y);
...
SkCodec::Result SkWuffsCodec::seekFrame(int frameIndex) {
  ...
  wuffs_base__status status = fDecoder->restart_frame(frameIndex, fIOBuffer.reader_io_position());
  if (status.repr != nullptr) {
    return SkCodec::kInternalError;
  }
  return SkCodec::kSuccess;
...
const char* SkWuffsCodec::decodeFrameConfig() {
  while (true) {
    wuffs_base__status status = fDecoder->decode_frame_config(&fFrameConfig, &fIOBuffer);
...
std::unique_ptr<SkCodec> SkWuffsCodec_MakeFromStream(
    std::unique_ptr<SkStream> stream, SkCodec::Result* result) {
  ...
  wuffs_base__io_buffer iobuf = wuffs_base__make_io_buffer(
      wuffs_base__make_slice_u8(buffer, SK_WUFFS_CODEC_BUFFER_SIZE),
  ...
  SkEncodedInfo::Alpha alpha =
      imgcfg.first_frame_is_opaque() ? SkEncodedInfo::kOpaque_Alpha : SkEncodedInfo::kBinary_Alpha;
