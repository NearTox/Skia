 ...
 */
std::unique_ptr<SkCodec> SkBmpCodec::MakeFromStream(
...
static BmpHeaderType get_header_type(size_t infoBytes) {
  if (infoBytes >= kBmpInfoBaseBytes) {
    ...
    switch (infoBytes) {
      case kBmpInfoV1Bytes: return kInfoV1_BmpHeaderType;
      case kBmpInfoV2Bytes: return kInfoV2_BmpHeaderType;
      case kBmpInfoV3Bytes: return kInfoV3_BmpHeaderType;
      case kBmpInfoV4Bytes: return kInfoV4_BmpHeaderType;
      case kBmpInfoV5Bytes: return kInfoV5_BmpHeaderType;
      case 16:
      case 20:
      case 24:
      case 28:
      ...
      case 48:
      case 60:
      case kBmpOS2V2Bytes: return kOS2VX_BmpHeaderType;
      default: SkCodecPrintf("Error: unknown bmp header format.\n"); return kUnknown_BmpHeaderType;
    }
  }
...
SkCodec::Result SkBmpCodec::ReadHeader(
    SkStream* stream, bool inIco, std::unique_ptr<SkCodec>* codecOut) {
  ...
  if (!inIco) {
    ...
    if (stream->read(hBuffer, kBmpHeaderBytesPlusFour) != kBmpHeaderBytesPlusFour) {
  ...
  switch (compression) {
    ...
    case kBitMasks_BmpCompressionMethod:
      ...
      switch (headerType) {
        ...
        default: SkCodecPrintf("Error: invalid bmp bit masks header.\n"); return kInvalidInput;
      }
      break;
    ...
    case kCMYK4BitRLE_BmpCompressionMethod:
      ...
      return kUnimplemented;
    default: SkCodecPrintf("Error: invalid format for bitmap decoding.\n"); return kInvalidInput;
  }
  iBuffer.reset();
  ...
  switch (inputFormat) {
    case kStandard_BmpInputFormat: {
      ...
      SkEncodedInfo::Alpha alpha =
          inIco ? SkEncodedInfo::kBinary_Alpha : SkEncodedInfo::kOpaque_Alpha;
      bool isOpaque = true;
      ...
      if (codecOut) {
        ...
        codecOut->reset(new SkBmpStandardCodec(
            std::move(info), std::unique_ptr<SkStream>(stream), bitsPerPixel, numColors,
    ...
    case kBitMask_BmpInputFormat: {
      ...
      if (codecOut) {
        ...
        codecOut->reset(new SkBmpMaskCodec(
            std::move(info), std::unique_ptr<SkStream>(stream), bitsPerPixel, masks.release(),
            rowOrder));
        return static_cast<SkBmpMaskCodec*>(codecOut->get())->didCreateSrcBuffer() ? kSuccess
                                                                                   : kInvalidInput;
      }
      return kSuccess;
    }
    ...
    case kRLE_BmpInputFormat: {
      ...
      if (codecOut) {
        ...
        auto info = SkEncodedInfo::Make(
            width, height, SkEncodedInfo::kBGRA_Color, SkEncodedInfo::kBinary_Alpha, 8);
        codecOut->reset(new SkBmpRLECodec(
            std::move(info), std::unique_ptr<SkStream>(stream), bitsPerPixel, numColors,
            bytesPerColor, offset - bytesRead, rowOrder));
      }
      return kSuccess;
    }
    default: SkASSERT(false); return kInvalidInput;
  }
}
 ...
 */
std::unique_ptr<SkCodec> SkBmpCodec::MakeFromStream(
    std::unique_ptr<SkStream> stream, Result* result, bool inIco) {
...
bool SkBmpCodec::onSkipScanlines(int count) { return this->skipRows(count); }
