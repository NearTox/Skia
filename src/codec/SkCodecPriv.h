...
static constexpr inline float get_scale_from_sample_size(int sampleSize) {
 ...
 */
static constexpr inline int get_start_coord(int sampleFactor) { return sampleFactor / 2; };
 ...
 * This does not need to be called and is not called when sampleFactor == 1.
 */
static constexpr inline int get_dst_coord(int srcCoord, int sampleFactor) {
 ...
 */
static constexpr inline size_t compute_row_bytes_ppb(int width, uint32_t pixelsPerByte) {
 ...
 */
static constexpr inline size_t compute_row_bytes_bpp(int width, uint32_t bytesPerPixel) {
...
static constexpr inline SkPMColor premultiply_argb_as_rgba(U8CPU a, U8CPU r, U8CPU g, U8CPU b) {
  if (a != 255) {
...
static constexpr inline SkPMColor premultiply_argb_as_bgra(U8CPU a, U8CPU r, U8CPU g, U8CPU b) {
  if (a != 255) {
    r = SkMulDiv255Round(r, a);
    g = SkMulDiv255Round(g, a);
    b = SkMulDiv255Round(b, a);
...
static constexpr inline bool is_rgba(SkColorType colorType) {
