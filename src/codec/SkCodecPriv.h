 ...
 * This does not need to be called and is not called when sampleFactor == 1.
 */
static constexpr inline bool is_coord_necessary(
    int srcCoord, int sampleFactor, int scaledDim) noexcept {
...
static inline uint32_t get_endian_int(const uint8_t* data, bool littleEndian) noexcept {
  if (littleEndian) {
