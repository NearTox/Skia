...
// directions.
dng_point num_tiles_in_area(const dng_point& areaSize, const dng_point_real64& tileSize) {
  // FIXME: Add a ceil_div() helper in SkCodecPriv.h
  return dng_point(
      static_cast<int32>((areaSize.v + tileSize.v - 1) / tileSize.v),
      static_cast<int32>((areaSize.h + tileSize.h - 1) / tileSize.h));
}

int num_tasks_required(const dng_point& tilesInTask, const dng_point& tilesInArea) {
...
dng_point num_tiles_per_task(const int maxTasks, const dng_point& tilesInArea) {
...
std::vector<dng_rect> compute_task_areas(
    const int maxTasks, const dng_rect& area, const dng_point& tileSize) {
  ...
  const dng_point taskAreaSize = {tilesPerTask.v * tileSize.v, tilesPerTask.h * tileSize.h};
...
class SkDngHost : public dng_host {
 public:
  ...
  void PerformAreaTask(dng_area_task& task, const dng_rect& area) override {
    ...
    const std::vector<dng_rect> taskAreas =
        compute_task_areas(this->PerformAreaTaskThreads(), area, tileSize);
...
bool is_asset_stream(const SkStream& stream) { return stream.hasLength() && stream.hasPosition(); }
...
class SkRawBufferedStream : public SkRawStream {
 public:
  explicit SkRawBufferedStream(std::unique_ptr<SkStream> stream)
      : fStream(std::move(stream)), fWholeStreamRead(false) {
...
class SkRawAssetStream : public SkRawStream {
 public:
  explicit SkRawAssetStream(std::unique_ptr<SkStream> stream) : fStream(std::move(stream)) {
  ...
  uint64 getLength() override { return fStream->getLength(); }
...
class SkPiexStream : public ::piex::StreamInterface {
 public:
  ...
  ::piex::Error GetData(const size_t offset, const size_t length, uint8* data) override {
    return fStream->read(static_cast<void*>(data), offset, length) ? ::piex::Error::kOk
                                                                   : ::piex::Error::kFail;
  }
...
class SkDngImage {
 public:
  ...
  bool isScalable() const { return fIsScalable; }

  bool isXtransImage() const { return fIsXtransImage; }
  ...
  bool readDng() {
    try {
      ...
      return this->init(
          ...
          static_cast<int>(fNegative->DefaultCropSizeV().As_real64()), cfaPatternSize);
    } catch (...) {
      return false;
    }
  }

  SkDngImage(SkRawStream* stream) : fStream(stream) {}
 ...
 */
std::unique_ptr<SkCodec> SkRawCodec::MakeFromStream(
  ...
  if (::piex::IsRaw(&piexStream)) {
    ...
    if (error == ::piex::Error::kOk && imageData.preview.length > 0 &&
      ...
      return SkJpegCodec::MakeFromStream(std::move(memoryStream), result, std::move(profile));
    }
  }
...
SkCodec::Result SkRawCodec::onGetPixels(
  ...
  for (int i = 0; i < height; ++i) {
    ...
    if (!skcms_Transform(
            &srcRow[0], srcFormat, skcms_AlphaFormat_Unpremul, srcProfile, dstRow, dstFormat,
            skcms_AlphaFormat_Unpremul, dstProfile, dstInfo.width())) {
...
SkISize SkRawCodec::onGetScaledDimensions(float desiredScale) const {
  ...
  return SkISize::Make(
      static_cast<int32_t>(std::floor(dim.fWidth / finalScale)),
...
SkRawCodec::SkRawCodec(SkDngImage* dngImage)
    : INHERITED(
          SkEncodedInfo::Make(
              dngImage->width(), dngImage->height(), SkEncodedInfo::kRGB_Color,
