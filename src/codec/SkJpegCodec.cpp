...
static bool is_orientation_marker(jpeg_marker_struct* marker, SkEncodedOrigin* orientation) {
  ...
  return SkParseEncodedOrigin(marker->data + kOffset, marker->data_length - kOffset, orientation);
}
...
SkCodec::Result SkJpegCodec::ReadHeader(
    SkStream* stream, SkCodec** codecOut, JpegDecoderMgr** decoderMgrOut,
  ...
  switch (jpeg_read_header(dinfo, true)) {
    ...
    case JPEG_SUSPENDED: return decoderMgr->returnFailure("ReadHeader", kIncompleteInput);
    default: return decoderMgr->returnFailure("ReadHeader", kInvalidInput);
  }
  ...
  if (codecOut) {
    ...
    if (profile) {
      ...
      switch (decoderMgr->dinfo()->jpeg_color_space) {
        ...
        case JCS_YCCK:
          ...
          break;
        case JCS_GRAYSCALE:
          if (type != skcms_Signature_Gray && type != skcms_Signature_RGB) {
            profile = nullptr;
          }
          break;
        default:
    ...
    SkEncodedInfo info = SkEncodedInfo::Make(
        dinfo->image_width, dinfo->image_height, color, SkEncodedInfo::kOpaque_Alpha, 8,
        std::move(profile));

    SkJpegCodec* codec = new SkJpegCodec(
        std::move(info), std::unique_ptr<SkStream>(stream), decoderMgr.release(), orientation);
    *codecOut = codec;
  } else {
...
SkJpegCodec::SkJpegCodec(
    SkEncodedInfo&& info, std::unique_ptr<SkStream> stream, JpegDecoderMgr* decoderMgr,
...
static size_t get_row_bytes(const j_decompress_ptr dinfo) {
  const size_t colorBytes =
      (dinfo->out_color_space == JCS_RGB565) ? 2 : dinfo->out_color_components;
...
bool SkJpegCodec::conversionSupported(
  ...
  switch (dstInfo.colorType()) {
    case kRGBA_8888_SkColorType: fDecoderMgr->dinfo()->out_color_space = JCS_EXT_RGBA; break;
...
int SkJpegCodec::readRows(
    const SkImageInfo& dstInfo, void* dst, size_t rowBytes, int count, const Options& opts) {
 ...
 */
static inline bool needs_swizzler_to_convert_from_cmyk(
    J_COLOR_SPACE jpegColorType, const skcms_ICCProfile* srcProfile, bool hasColorSpaceXform) {
 ...
 */
SkCodec::Result SkJpegCodec::onGetPixels(
  ...
  if (needs_swizzler_to_convert_from_cmyk(
          dinfo->out_color_space, this->getEncodedInfo().profile(), this->colorXform())) {
...
void SkJpegCodec::initializeSwizzler(
    const SkImageInfo& dstInfo, const Options& options, bool needsCMYKToRGB) {
  ...
  if (options.fSubset) {
    ...
    SkASSERT(
        !fSwizzlerSubset.isEmpty() && fSwizzlerSubset.x() <= options.fSubset->x() &&
  ...
  if (needsCMYKToRGB) {
    ...
    auto swizzlerInfo = SkEncodedInfo::Make(
        0, 0, SkEncodedInfo::kInvertedCMYK_Color, SkEncodedInfo::kOpaque_Alpha, 8);
    ...
    switch (fDecoderMgr->dinfo()->out_color_space) {
      case JCS_EXT_RGBA:
      case JCS_EXT_BGRA:
      case JCS_CMYK: srcBPP = 4; break;
      case JCS_RGB565: srcBPP = 2; break;
      case JCS_GRAYSCALE: srcBPP = 1; break;
...
SkCodec::Result SkJpegCodec::onStartScanlineDecode(
  ...
  if (options.fSubset) {
    ...
    fSwizzlerSubset.setXYWH(
        options.fSubset->x() - startX, 0, options.fSubset->width(), options.fSubset->height());
...
static bool is_yuv_supported(jpeg_decompress_struct* dinfo) {
  ...
  if ((1 != dinfo->comp_info[1].h_samp_factor) || (1 != dinfo->comp_info[1].v_samp_factor) ||
      (1 != dinfo->comp_info[2].h_samp_factor) || (1 != dinfo->comp_info[2].v_samp_factor)) {
    return false;
  }
  ...
  return (1 == hSampY && 1 == vSampY) || (2 == hSampY && 1 == vSampY) ||
         (2 == hSampY && 2 == vSampY) || (1 == hSampY && 2 == vSampY) ||
         (4 == hSampY && 1 == vSampY) || (4 == hSampY && 2 == vSampY);
}
...
SkCodec::Result SkJpegCodec::onGetYUV8Planes(
    const SkYUVASizeInfo& sizeInfo, void* planes[SkYUVASizeInfo::kMaxCount]) {
  ...
  if (!supportsYUV || sizeInfo.fSizes[0] != defaultInfo.fSizes[0] ||
      sizeInfo.fSizes[1] != defaultInfo.fSizes[1] || sizeInfo.fSizes[2] != defaultInfo.fSizes[2] ||
  ...
  SkASSERT(
      (uint32_t)sizeInfo.fSizes[0].width() == dinfo->output_width &&
  ...
  for (int i = 0; i < DCTSIZE; i++) {
    rowptrs[i + 2 * DCTSIZE] = SkTAddOffset<JSAMPLE>(planes[1], i * sizeInfo.fWidthBytes[1]);
    rowptrs[i + 3 * DCTSIZE] = SkTAddOffset<JSAMPLE>(planes[2], i * sizeInfo.fWidthBytes[2]);
  }
