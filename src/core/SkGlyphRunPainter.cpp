...
void SkGlyphRunListPainter::processGlyphRunList(
  ...
  for (const auto& glyphRun : glyphRunList) {
    ...
    bool usePaths = useSDFT ? false : SkStrikeSpec::ShouldDrawAsPath(runPaint, runFont, viewMatrix);
    ...
    if (!useSDFT && !usePaths) {
      ...
      if (!strikeSpec.isEmpty()) {
        ...
        if (process) {
          ...
          process->processSourceSDFT(fDrawable.drawable(), strikeSpec, runFont, minScale, maxScale);
        }
      }
    }
  ...
  if (cacheBlob) {
    if (cacheBlob->mustRegenerate(
      ...
      cacheBlob->generateFromGlyphRunList(
          *context->priv().caps()->shaderCaps(), fOptions, listPaint, viewMatrix, props,
      ...
      if (CACHE_SANITY_CHECK) {
        sk_sp<GrTextBlob> sanityBlob(
            textBlobCache->makeBlob(glyphRunList, grStrikeCache, viewMatrix, color, forceW));
    ...
    if (canCache) {
      ...
      cacheBlob = textBlobCache->makeBlob(glyphRunList, grStrikeCache, viewMatrix, color, forceW);
    }
    cacheBlob->generateFromGlyphRunList(
        *context->priv().caps()->shaderCaps(), fOptions, listPaint, viewMatrix, props, glyphRunList,
        target->glyphPainter());
  }
...
GrTextBlob::SubRun::SubRun(
    ...
    const GrTextBlob::SubRunBufferSpec& bufferSpec, sk_sp<GrTextStrike>&& grStrike)
    : fType{type},
      fBlob{textBlob},
      fMaskFormat{format},
...
class GrTextBlob::SubRun* GrTextBlob::makeSubRun(
  ...
  SubRunBufferSpec bufferSpec = std::make_tuple(glyphsStart, glyphsEnd, verticesStart, verticesEnd);

  sk_sp<GrTextStrike> grStrike = strikeSpec.findOrCreateGrStrike(fStrikeCache);

  SubRun& subRun =
      fSubRuns.emplace_back(type, this, strikeSpec, format, bufferSpec, std::move(grStrike));
...
std::unique_ptr<GrDrawOp> GrTextContext::createOp_TestingOnly(
  ...
  if (!glyphRunList.empty()) {
    ...
    blob->generateFromGlyphRunList(
        *context->priv().caps()->shaderCaps(), textContext->fOptions, skPaint, viewMatrix,
        surfaceProps, glyphRunList, rtc->textTarget()->glyphPainter());
  }

  return blob->test_makeOp(
      textLen, viewMatrix, x, y, skPaint, filteredColor, surfaceProps, textContext->dfAdjustTable(),
      rtc->textTarget());
}
...
SkVector SkGlyphPositionRoundingSpec::HalfAxisSampleFreq(
  ...
  if (!isSubpixel) {
    ...
    switch (axisAlignment) {
      case kX_SkAxisAlignment: return {SkPackedGlyphID::kSubpixelRound, SK_ScalarHalf};
      case kY_SkAxisAlignment: return {SK_ScalarHalf, SkPackedGlyphID::kSubpixelRound};
...
SkIPoint SkGlyphPositionRoundingSpec::IgnorePositionFieldMask(
    bool isSubpixel, SkAxisAlignment axisAlignment) {
  SkIPoint ignoreMask = IgnorePositionMask(isSubpixel, axisAlignment);
  SkIPoint answer{
      ignoreMask.x() & SkPackedGlyphID::kXYFieldMask.x(),
