...
void SkGlyphRunListPainter::drawForBitmapDevice(
  ...
  for (auto& glyphRun : glyphRunList) {
    ...
    if (!fRejects.source().empty()) {
      ...
      fDrawable.startDevice(fRejects.source(), drawOrigin, deviceMatrix, strike->roundingSpec());
...
void SkGlyphRunListPainter::processGlyphRunList(
  ...
  for (const auto& glyphRun : glyphRunList) {
    ...
    bool usePaths = useSDFT ? false : SkStrikeSpec::ShouldDrawAsPath(runPaint, runFont, drawMatrix);
    ...
    if (!usePaths && !fRejects.source().empty()) {
      ...
      SkStrikeSpec strikeSpec =
          SkStrikeSpec::MakeMask(runFont, runPaint, fDeviceProps, fScalerContextFlags, drawMatrix);
...
void GrTextContext::drawGlyphRunList(
    ...
    const SkMatrix& drawMatrix, const SkSurfaceProps& props, const SkGlyphRunList& glyphRunList) {
  ...
  if (canCache) {
    ...
    GrColor canonicalColor =
        hasLCD ? SK_ColorTRANSPARENT : ComputeCanonicalColor(blobPaint, hasLCD);
  ...
  if (cachedBlob) {
    if (cachedBlob->mustRegenerate(
            blobPaint, glyphRunList.anyRunsSubpixelPositioned(), blurRec, drawMatrix, drawOrigin)) {
      // We have to remake the blob because changes may invalidate our masks.
      // TODO we could probably get away reuse most of the time if the pointer is unique,
      // but we'd have to clear the subrun information
      textBlobCache->remove(cachedBlob.get());
      cachedBlob = textBlobCache->makeCachedBlob(
          glyphRunList, grStrikeCache, key, blurRec, drawMatrix, initialVertexColor, forceW);
  ...
  cachedBlob->flush(
      target, props, fDistanceAdjustTable.get(), blobPaint, drawingColor, clip, drawMatrix,
      drawOrigin);
}
...
std::unique_ptr<GrDrawOp> GrTextContext::createOp_TestingOnly(
    ...
    const SkPaint& skPaint, const SkFont& font, const SkMatrix& drawMatrix, const char* text, int x,
    int y) {
  ...
  if (!glyphRunList.empty()) {
    ...
    painter->processGlyphRunList(
        ...
        context->priv().caps()->shaderCaps()->supportsDistanceFieldText(), textContext->fOptions,
        blob.get());
  }

  return blob->test_makeOp(
      textLen, drawMatrix, drawOrigin, skPaint, filteredColor, surfaceProps,
