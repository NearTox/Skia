  ...
  for (const auto& glyphRun : glyphRunList) {
    ...
    if (useSDFT) {
      ...
      auto packedGlyphIDs = DeviceSpacePackedGlyphIDs(
          strike->roundingSpec(), viewMatrix, origin, glyphRun.runSize(),
          glyphRun.glyphsIDs().data(), glyphRun.positions().data(), fPositions, fPackedGlyphIDs);
...
SkPMColor4f generate_filtered_color(const SkPaint& paint, const GrColorInfo& colorInfo) {
  ...
  if (paint.getColorFilter() != nullptr) {
    filteredColor = paint.getColorFilter()->filterColor4f(
        filteredColor, colorInfo.colorSpace(), colorInfo.colorSpace());
  }
