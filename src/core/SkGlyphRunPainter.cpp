...
SkVector SkStrikeCommon::PixelRounding(bool isSubpixel, SkAxisAlignment axisAlignment) {
  if (!isSubpixel) {
    ...
    switch (axisAlignment) {
      case kX_SkAxisAlignment: return {kSubpixelRounding, SK_ScalarHalf};
      case kY_SkAxisAlignment: return {SK_ScalarHalf, kSubpixelRounding};
      case kNone_SkAxisAlignment: return {kSubpixelRounding, kSubpixelRounding};
    }
  }
...
SkGlyphRunListPainter::SkGlyphRunListPainter(
    const SkSurfaceProps& props, SkColorType colorType, SkColorSpace* cs,
    SkStrikeCacheInterface* strikeCache)
...
SkGlyphRunListPainter::SkGlyphRunListPainter(
    ...
    : SkGlyphRunListPainter(
          props, kUnknown_SkColorType, compute_scaler_context_flags(csi.colorSpace()),
...
static bool check_glyph_position(SkPoint position) {
  ...
  return !(
      gt(position.fX, INT_MAX - (INT16_MAX + SkTo<int>(UINT16_MAX))) ||
...
SkSpan<const SkPackedGlyphID> SkGlyphRunListPainter::SourceSpacePackedGlyphIDs(
  ...
  SkMatrix::MakeTrans(origin.x(), origin.y()).mapPoints(mappedPositions, positions, n);
...
void SkGlyphRunListPainter::drawForBitmapDevice(
  ...
  auto& props = (kN32_SkColorType == fColorType && runPaint.isSrcOver()) ? fDeviceProps
  ...
  for (auto& glyphRun : glyphRunList) {
    ...
    if (SkStrikeSpec::ShouldDrawAsPath(runPaint, runFont, deviceMatrix)) {
      SkStrikeSpec strikeSpec =
          SkStrikeSpec::MakePath(runFont, runPaint, props, fScalerContextFlags);

      auto strike = strikeSpec.findOrCreateExclusiveStrike();

      auto packedGlyphIDs = SourceSpacePackedGlyphIDs(
          origin, runSize, glyphRun.glyphsIDs().data(), glyphRun.positions().data(), fPositions,
          fPackedGlyphIDs);

      auto glyphPosSpan = strike->prepareForDrawing(
          packedGlyphIDs.data(), fPositions, glyphRun.runSize(), 0, SkStrikeInterface::kBoundsOnly,
          fGlyphPos);
      ...
      for (const SkGlyphPos& glyphPos : glyphPosSpan) {
        ...
        if (check_glyph_position(position) && !glyph.isEmpty() && !glyph.isColor() &&
            glyph.path() != nullptr) {
      ...
      SkStrikeSpec strikeSpec =
          SkStrikeSpec::MakeMask(runFont, runPaint, props, fScalerContextFlags, deviceMatrix);

      auto strike = strikeSpec.findOrCreateExclusiveStrike();

      auto packedGlyphIDs = DeviceSpacePackedGlyphIDs(
          strike.get(), deviceMatrix, origin, runSize, glyphRun.glyphsIDs().data(),
          glyphRun.positions().data(), fPositions, fPackedGlyphIDs);

      SkSpan<const SkGlyphPos> glyphPosSpan = strike->prepareForDrawing(
          packedGlyphIDs.data(), fPositions, glyphRun.runSize(), std::numeric_limits<int>::max(),
          SkStrikeInterface::kImageIfNeeded, fGlyphPos);
...
void SkGlyphRunListPainter::processARGBFallback(
  ...
  bool useDeviceCache = viewMatrix.isScaleTranslate() &&
  ...
  if (useDeviceCache) {
    ...
    SkStrikeSpec strikeSpec =
        SkStrikeSpec::MakeMask(runFont, runPaint, fDeviceProps, fScalerContextFlags, viewMatrix);
    ...
    SkSpan<const SkGlyphPos> glyphPosSpan = strike->prepareForDrawing(
        fPackedGlyphIDs, fARGBPositions.data(), fARGBGlyphsIDs.size(),
        SkStrikeCommon::kSkSideTooBigForAtlas, SkStrikeInterface::kBoundsOnly, fGlyphPos);
    ...
    if (process) {
      process->processSourceFallback(glyphPosSpan, strikeSpec, viewMatrix.hasPerspective());
    }
  }
}

#if SK_SUPPORT_GPU
void SkGlyphRunListPainter::processGlyphRunList(
  ...
  for (const auto& glyphRun : glyphRunList) {
    ...
    auto addFallback = [this, &maxFallbackDimension](const SkGlyph& glyph, SkPoint sourcePosition) {
      maxFallbackDimension = std::max(maxFallbackDimension, SkIntToScalar(glyph.maxDimension()));
    ...
    if (useSDFT) {
      ...
      auto packedGlyphIDs = SourceSpacePackedGlyphIDs(
          origin, glyphRun.runSize(), glyphRun.glyphsIDs().data(), glyphRun.positions().data(),
          fPositions, fPackedGlyphIDs);

      SkSpan<const SkGlyphPos> glyphPosSpan = strike->prepareForDrawing(
          packedGlyphIDs.data(), fPositions, glyphRun.runSize(),
          SkStrikeCommon::kSkSideTooBigForAtlas, SkStrikeInterface::kBoundsOnly, fGlyphPos);
      ...
      for (const SkGlyphPos& glyphPos : glyphPosSpan) {
        ...
        SkASSERT(glyph.isEmpty() || glyph.maskFormat() == SkMask::kSDF_Format || glyph.isColor());
      ...
      if (process) {
        bool hasWCoord = viewMatrix.hasPerspective() || options.fDistanceFieldVerticesAlwaysHaveW;

        // processSourceSDFT must be called even if there are no glyphs to make sure runs
        // are set correctly.
        process->processSourceSDFT(
            SkSpan<const SkGlyphPos>{fGlyphPos, glyphsWithMaskCount}, strikeSpec, runFont, minScale,
            maxScale, hasWCoord);
      ...
      if (!fARGBGlyphsIDs.empty()) {
        this->processARGBFallback(
            maxFallbackDimension * strikeSpec.strikeToSourceRatio(), runPaint, runFont, viewMatrix,
            process);
      }
      ...
      if (process) {
        ...
        process->processSourcePaths(
            SkSpan<const SkGlyphPos>{fGlyphPos, glyphsWithPathCount}, strikeSpec);
      }
      ...
      auto packedGlyphIDs = DeviceSpacePackedGlyphIDs(
          strike.get(), viewMatrix, origin, glyphRun.runSize(), glyphRun.glyphsIDs().data(),
          glyphRun.positions().data(), fPositions, fPackedGlyphIDs);
      ...
      if (!fARGBGlyphsIDs.empty()) {
        this->processARGBFallback(
            maxFallbackDimension / viewMatrix.getMaxScale(), runPaint, runFont, viewMatrix,
            process);
      }
    }  // Mask case
  }    // For all glyph runs
...
SkGlyphRunListPainter::ScopedBuffers SkGlyphRunListPainter::ensureBuffers(
...
SkPMColor4f generate_filtered_color(const SkPaint& paint, const GrColorSpaceInfo& colorSpaceInfo) {
  ...
  if (paint.getColorFilter() != nullptr) {
    filteredColor =
        paint.getColorFilter()->filterColor4f(filteredColor, colorSpaceInfo.colorSpace());
  }
...
void GrTextContext::drawGlyphRunList(
  ...
  bool canCache = glyphRunList.canCache() &&
                  !(listPaint.getPathEffect() || (mf && !as_MFB(mf)->asABlur(&blurRec)));
  ...
  if (canCache) {
    ...
    GrColor canonicalColor =
        hasLCD ? SK_ColorTRANSPARENT : ComputeCanonicalColor(listPaint, hasLCD);
  ...
  if (cacheBlob) {
    if (cacheBlob->mustRegenerate(
            listPaint, glyphRunList.anyRunsSubpixelPositioned(), blurRec, viewMatrix, origin.x(),
            origin.y())) {
      ...
      cacheBlob->generateFromGlyphRunList(
          *context->priv().caps()->shaderCaps(), fOptions, listPaint, scalerContextFlags,
          viewMatrix, props, glyphRunList, target->glyphPainter());
    } else {
      textBlobCache->makeMRU(cacheBlob.get());

      if (CACHE_SANITY_CHECK) {
        sk_sp<GrTextBlob> sanityBlob(textBlobCache->makeBlob(glyphRunList, color, grStrikeCache));
    ...
    cacheBlob->generateFromGlyphRunList(
        *context->priv().caps()->shaderCaps(), fOptions, listPaint, scalerContextFlags, viewMatrix,
        props, glyphRunList, target->glyphPainter());
  }

  cacheBlob->flush(
      target, props, fDistanceAdjustTable.get(), listPaint, filteredColor, clip, viewMatrix,
      origin.x(), origin.y());
}
...
void GrTextBlob::Run::switchSubRunIfNeededAndAppendGlyph(
    GrGlyph* glyph, const sk_sp<GrTextStrike>& strike, const SkRect& destRect,
    bool needsTransform) {
...
void GrTextBlob::Run::appendDeviceSpaceGlyph(
    const sk_sp<GrTextStrike>& strike, const SkGlyph& skGlyph, SkPoint origin) {
...
void GrTextBlob::Run::appendSourceSpaceGlyph(
    const sk_sp<GrTextStrike>& strike, const SkGlyph& skGlyph, SkPoint origin, SkScalar textScale) {
  if (GrGlyph* glyph = strike->getGlyph(skGlyph)) {
...
void GrTextBlob::generateFromGlyphRunList(
    const GrShaderCaps& shaderCaps, const GrTextContext::Options& options, const SkPaint& paint,
    SkScalerContextFlags scalerContextFlags, const SkMatrix& viewMatrix,
    const SkSurfaceProps& props, const SkGlyphRunList& glyphRunList,
    SkGlyphRunListPainter* glyphPainter) {
  SkPoint origin = glyphRunList.origin();
  const SkPaint& runPaint = glyphRunList.paint();
  this->initReusableBlob(
      SkPaintPriv::ComputeLuminanceColor(runPaint), viewMatrix, origin.x(), origin.y());

  glyphPainter->processGlyphRunList(
      glyphRunList, viewMatrix, props, shaderCaps.supportsDistanceFieldText(), options, this);
}

GrTextBlob::Run* GrTextBlob::currentRun() { return &fRuns[fRunCount - 1]; }
...
void GrTextBlob::processDeviceMasks(
    SkSpan<const SkGlyphPos> masks, const SkStrikeSpec& strikeSpec) {
  ...
  for (const auto& mask : masks) {
    SkPoint pt{SkScalarFloorToScalar(mask.position.fX), SkScalarFloorToScalar(mask.position.fY)};
    run->appendDeviceSpaceGlyph(currStrike, *mask.glyph, pt);
  }
}

void GrTextBlob::processSourcePaths(
    SkSpan<const SkGlyphPos> paths, const SkStrikeSpec& strikeSpec) {
  Run* run = this->currentRun();
  this->setHasBitmap();
  run->setupFont(strikeSpec);
  for (const auto& path : paths) {
    if (const SkPath* glyphPath = path.glyph->path()) {
      run->appendPathGlyph(*glyphPath, path.position, strikeSpec.strikeToSourceRatio(), false);
    }
  }
}
...
void GrTextBlob::processDevicePaths(SkSpan<const SkGlyphPos> paths) {
  ...
  this->setHasBitmap();
  for (const auto& path : paths) {
    SkPoint pt{SkScalarFloorToScalar(path.position.fX), SkScalarFloorToScalar(path.position.fY)};
...
void GrTextBlob::processSourceSDFT(
    ...
    SkScalar minScale, SkScalar maxScale, bool hasWCoord) {
  Run* run = this->currentRun();
...
void GrTextBlob::processSourceFallback(
    SkSpan<const SkGlyphPos> masks, const SkStrikeSpec& strikeSpec, bool hasW) {
  Run* run = this->currentRun();
...
void GrTextBlob::processDeviceFallback(
    SkSpan<const SkGlyphPos> masks, const SkStrikeSpec& strikeSpec) {
  Run* run = this->currentRun();
  this->setHasBitmap();
  sk_sp<GrTextStrike> grStrike = strikeSpec.findOrCreateGrStrike(fStrikeCache);
...
std::unique_ptr<GrDrawOp> GrTextContext::createOp_TestingOnly(
    ...
    const SkPaint& skPaint, const SkFont& font, const SkMatrix& viewMatrix, const char* text, int x,
    int y) {
  ...
  if (!glyphRunList.empty()) {
    ...
    SkScalerContextFlags scalerContextFlags = ComputeScalerContextFlags(rtc->colorSpaceInfo());
    blob->generateFromGlyphRunList(
        *context->priv().caps()->shaderCaps(), textContext->fOptions, skPaint, scalerContextFlags,
        viewMatrix, surfaceProps, glyphRunList, rtc->textTarget()->glyphPainter());
  }

  return blob->test_makeOp(
      textLen, 0, 0, viewMatrix, x, y, skPaint, filteredColor, surfaceProps,
