...
struct SkBitmapProcState {
  ...
  bool setup(const SkMatrix& inv, SkColor color, const SkSamplingOptions& sampling) {
    return this->init(inv, color, sampling) && this->chooseProcs();
  }
...
class SkBitmapProcStateAutoMapper {
 public:
  SkBitmapProcStateAutoMapper(
    ...
    fX =
        (SkFractionalInt)((uint64_t)SkScalarToFractionalInt(pt.x()) - (uint64_t)SkFixedToFractionalInt(biasX));
    fY =
        (SkFractionalInt)((uint64_t)SkScalarToFractionalInt(pt.y()) - (uint64_t)SkFixedToFractionalInt(biasY));
