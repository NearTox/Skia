...
bool SkTriColorShader::update(
  ...
  Sk4f c0 = Sk4f::Load(colors[index0].vec()), c1 = Sk4f::Load(colors[index1].vec()),
...
static SkPMColor4f* convert_colors(
  ...
  SkImageInfo dstInfo =
      SkImageInfo::Make(count, 1, kRGBA_F32_SkColorType, kPremul_SkAlphaType, sk_ref_sp(deviceCS));
...
void SkDraw::drawFixedVertices(
  ...
  if (gUseSkVMBlitter) {
    ...
    if (colors) {
      ...
      if (shader) {
        shader =
            outerAlloc->make<SkShader_Blend>(blendMode, sk_ref_sp(colorShader), sk_ref_sp(shader));
      } else {
        shader = colorShader;
      }
    }
    ...
    if (auto blitter = SkVMBlitter::Make(
      ...
      while (vertProc(&state)) {
        ...
        if (colorShader &&
            !colorShader->update(ctmInverse, positions, dstColors, state.f0, state.f1, state.f2)) {
          continue;
        }
  ...
  if (colors) {
    ...
    if (shader) {
      shader = outerAlloc->make<SkShader_Blend>(blendMode, sk_ref_sp(triShader), sk_ref_sp(shader));
    } else {
      shader = triShader;
    }
  }
  ...
  if (!texCoords) {  // only tricolor shader
    if (auto blitter = SkCreateRasterPipelineBlitter(
      ...
      while (vertProc(&state)) {
        if (triShader &&
            !triShader->update(ctmInverse, positions, dstColors, state.f0, state.f1, state.f2)) {
          continue;
        }
        fill_triangle(state, blitter, *fRC, dev2, dev3);
      }
...
void SkDraw::drawVertices(
    const SkVertices* vertices, SkBlendMode bmode, const SkPaint& paint) const {
  SkVerticesPriv info(vertices->priv());
  const int vertexCount = info.vertexCount();
  const int indexCount = info.indexCount();
