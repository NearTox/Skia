...
struct Matrix43 {
  ...
  skvx::float4 map(float x, float y) const {
    return skvx::float4::Load(&fMat[0]) * x + skvx::float4::Load(&fMat[4]) * y +
...
static bool SK_WARN_UNUSED_RESULT texture_to_matrix(
    const VertState& state, const SkPoint verts[], const SkPoint texs[], SkMatrix* matrix) {
  SkPoint src[3], dst[3];
...
class SkTriColorShader : public SkShaderBase {
 public:
  ...
  bool update(
      const SkMatrix& ctmInv, const SkPoint pts[], const SkPMColor4f colors[], int index0,
...
skvm::Color SkTriColorShader::onProgram(
    skvm::Builder* b, skvm::Coord device, skvm::Coord local, skvm::Color,
    const SkMatrixProvider& matrices, const SkMatrix* localM, const SkColorInfo&,
  ...
  if (fUsePersp) {
    ...
    auto dot = [&, x, y](int row) {
      return b->mad(
          x, b->arrayF(fCoordMatrix, row),
          b->mad(y, b->arrayF(fCoordMatrix, row + 3), b->arrayF(fCoordMatrix, row + 6)));
    };
  ...
  auto colorDot = [&, x, y](int row) {
    return b->mad(
        x, b->arrayF(fColorMatrix, row),
        b->mad(y, b->arrayF(fColorMatrix, row + 4), b->arrayF(fColorMatrix, row + 8)));
  };
...
bool SkTriColorShader::update(
  ...
  auto c0 = skvx::float4::Load(colors[index0].vec()), c1 = skvx::float4::Load(colors[index1].vec()),
...
//
static SkPMColor4f* convert_colors(
    const SkColor src[], int count, SkColorSpace* deviceCS, SkArenaAlloc* alloc,
    bool skipColorXform) {
...
static void fill_triangle_2(
    const VertState& state, SkBlitter* blitter, const SkRasterClip& rc, const SkPoint dev2[]) {
  SkPoint tmp[] = {dev2[state.f0], dev2[state.f1], dev2[state.f2]};
  SkScan::FillTriangle(tmp, rc, blitter);
}

static constexpr int kMaxClippedTrianglePointCount = 4;
static void fill_triangle_3(
    const VertState& state, SkBlitter* blitter, const SkRasterClip& rc, const SkPoint3 dev3[]) {
...
void SkDraw::drawFixedVertices(
    const SkVertices* vertices, sk_sp<SkBlender> blender, const SkPaint& paint,
    const SkMatrix& ctmInverse, const SkPoint* dev2, const SkPoint3* dev3, SkArenaAlloc* outerAlloc,
  ...
  if (std::optional<SkBlendMode> bm = as_BB(blender)->asBlendMode(); bm.has_value() && colors) {
    switch (*bm) {
      case SkBlendMode::kSrc: colors = nullptr; break;
  ...
  if (colors) {
    dstColors = convert_colors(colors, vertexCount, fDst.colorSpace(), outerAlloc, skipColorXform);
    triColorShader =
        outerAlloc->make<SkTriColorShader>(compute_is_opaque(colors, vertexCount), usePerspective);
  }
  ...
  auto applyShaderColorBlend = [&](SkShader* shader) -> SkShader* {
    ...
    return outerAlloc->make<SkShader_Blend>(blender, sk_ref_sp(triColorShader), sk_ref_sp(shader));
  };
  ...
  auto rpblit = [&]() {
    ...
    if (!texCoords) {  // only tricolor shader
      ...
      while (vertProc(&state)) {
        if (triColorShader && !triColorShader->update(
                                  ctmInverse, positions, dstColors, state.f0, state.f1, state.f2)) {
          continue;
        }
    ...
    SkStageRec rec = {&pipeline,   outerAlloc, fDst.colorType(), fDst.colorSpace(),
                      shaderPaint, nullptr,    *fMatrixProvider};
    if (auto updater = as_SB(shader)->appendUpdatableStages(rec)) {
      ...
      while (vertProc(&state)) {
        ...
        SkMatrix localM;
        if ((texCoords == positions) || (texture_to_matrix(state, positions, texCoords, &localM) &&
  ...
  if (gUseSkVMBlitter || !rpblit()) {
    ...
    shaderPaint.setShader(sk_ref_sp(shader));
    auto blitter =
        SkVMBlitter::Make(fDst, shaderPaint, *fMatrixProvider, outerAlloc, this->fRC->clipShader());
    if (!blitter) {
      return;
    }
    while (vertProc(&state)) {
      ...
      if (triColorShader &&
          !triColorShader->update(ctmInverse, positions, dstColors, state.f0, state.f1, state.f2)) {
        continue;
      }
...
void SkDraw::drawVertices(
  ...
  constexpr size_t kOuterSize = sizeof(SkTriColorShader) + sizeof(SkShader_Blend) +
