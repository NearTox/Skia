...
static SkPoint3 lerp(SkPoint3 curr, SkPoint3 next, float t) { return curr + t * (next - curr); }
...
void SkDraw::draw_fixed_vertices(
    const SkVertices* vertices, SkBlendMode bmode, const SkPaint& paint, const SkMatrix& ctmInv,
    const SkPoint dev2[], const SkPoint3 dev3[], SkArenaAlloc* outerAlloc) const {
  ...
  if (colors && textures) {
    switch (bmode) {
      ...
      case SkBlendMode::kDst: textures = nullptr; break;
      default: break;
    }
  }
  ...
  if (!(colors || textures)) {
    ...
    while (vertProc(&state)) {
      if (dev3) {
        ...
        SkPoint array[] = {dev2[state.f0], dev2[state.f1], dev2[state.f2], dev2[state.f0]};
  ...
  if (colors) {
    ...
    if (shader) {
      shader =
          outerAlloc->make<SkShader_Blend>(bmode, sk_ref_sp(triShader), sk_ref_sp(shader), nullptr);
    } else {
      shader = triShader;
    }
  }
  ...
  if (!textures) {  // only tricolor shader
    auto blitter =
        SkCreateRasterPipelineBlitter(fDst, p, *fMatrix, outerAlloc, this->fRC->clipShader());
    while (vertProc(&state)) {
  ...
  if (auto updater = as_SB(shader)->appendUpdatableStages(rec)) {
    ...
    while (vertProc(&state)) {
      ...
      auto blitter =
          SkCreateRasterPipelineBlitter(fDst, p, *ctm, &innerAlloc, this->fRC->clipShader());
      fill_triangle(state, blitter, *fRC, dev2, dev3);
    }
  }
}

void SkDraw::draw_vdata_vertices(
    const SkVertices* vt, const SkPaint& paint, const SkMatrix& ctmInv, const SkPoint dev2[],
    const SkPoint3 dev3[], SkArenaAlloc* outerAlloc) const {
  SkASSERT(!!dev2 != !!dev3);
