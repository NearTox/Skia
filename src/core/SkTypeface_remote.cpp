...
void SkScalerContextProxy::generateImage(const SkGlyph& glyph) {
  ...
  fDiscardableManager->notifyCacheMiss(SkStrikeClient::CacheMissType::kGlyphImage, fRec.fTextSize);
}
...
bool SkScalerContextProxy::generatePath(SkGlyphID glyphID, SkPath* path) {
  ...
  fDiscardableManager->notifyCacheMiss(SkStrikeClient::CacheMissType::kGlyphPath, fRec.fTextSize);
  return false;
}

void SkScalerContextProxy::generateFontMetrics(SkFontMetrics* metrics) {
  TRACE_EVENT1("skia", "generateFontMetrics", "rec", TRACE_STR_COPY(this->getRec().dump().c_str()));
  if (this->getProxyTypeface()->isLogging()) {
  ...
  fDiscardableManager->notifyCacheMiss(SkStrikeClient::CacheMissType::kFontMetrics, fRec.fTextSize);
