...
struct SkScalerContextEffects {
    SkScalerContextEffects() noexcept : fPathEffect(nullptr), fMaskFilter(nullptr) {}
...
public:
    SkStrikeSpec(const SkDescriptor& desc,
                 ...
                 const SkScalerContextEffects& effects) noexcept
            : fDesc{desc}, fTypeface{typeface}, fEffects{effects} {}

    const SkDescriptor& desc() const noexcept { return fDesc; }
    const SkTypeface& typeface() const noexcept { return fTypeface; }
    SkScalerContextEffects effects() const noexcept { return fEffects; }
...
public:
    ...
    struct Deleter {
        void operator()(SkStrikeInterface* ptr) const { ptr->onAboutToExitScope(); }
    };
};
