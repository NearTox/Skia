...
SkPathRef::~SkPathRef() {
  ...
  SkDEBUGCODE(fPoints = nullptr;) SkDEBUGCODE(fVerbs = nullptr;) SkDEBUGCODE(fVerbCnt = 0x9999999;)
      SkDEBUGCODE(fPointCnt = 0xAAAAAAA;) SkDEBUGCODE(fPointCnt = 0xBBBBBBB;)
...
void SkPathRef::CreateTransformedCopy(
  ...
  if (dst->get() != &src) {
    ...
    sk_careful_memcpy(
        (*dst)->verbsMemWritable(), src.verbsMemBegin(), src.fVerbCnt * sizeof(uint8_t));
...
static bool validate_verb_sequence(const uint8_t verbs[], int vCount) {
  ...
  for (int i = vCount - 1; i >= 0; --i) {
    switch (verbs[i]) {
      case SkPath::kMove_Verb: needsMoveTo = false; break;
      case SkPath::kLine_Verb:
      case SkPath::kQuad_Verb:
      case SkPath::kConic_Verb:
      case SkPath::kCubic_Verb: invalidSequence |= needsMoveTo; break;
      case SkPath::kClose_Verb: needsMoveTo = true; break;
      default: return false;  // unknown verb
    }
  }
...
static bool deduce_pts_conics(
  ...
  for (int i = 0; i < vCount; ++i) {
    switch (verbs[i]) {
      ...
      case SkPath::kLine_Verb: ptCount = safe.addInt(ptCount, 1); break;
      case SkPath::kConic_Verb:
        conicCount += 1;
        // fall-through
      case SkPath::kQuad_Verb: ptCount = safe.addInt(ptCount, 2); break;
      case SkPath::kCubic_Verb: ptCount = safe.addInt(ptCount, 3); break;
...
SkPathRef* SkPathRef::CreateFromBuffer(SkRBuffer* buffer) {
  ...
  if (!buffer->readU32(&(ref->fGenerationID)) || !buffer->readS32(&verbCount) || (verbCount < 0) ||
      !buffer->readS32(&pointCount) || (pointCount < 0) || !buffer->readS32(&conicCount) ||
      (conicCount < 0)) {
    return nullptr;
  }
  ...
  if (!buffer->read(ref->verbsMemWritable(), verbSize) || !buffer->read(ref->fPoints, pointSize) ||
...
void SkPathRef::writeToBuffer(SkWBuffer* buffer) const {
  SkDEBUGCODE(this->validate();) SkDEBUGCODE(size_t beforePos = buffer->pos();)
...
void SkPathRef::copy(
  ...
  SkDEBUGCODE(this->validate();) this->resetToSize(
      ref.fVerbCnt, ref.fPointCnt, ref.fConicWeights.count(), additionalReserveVerbs,
    ...
    if (SkPath::kConic_Verb == verb) {
      ...
      SkRRect::Corner corner =
          pts[1].fX == bounds.fLeft
              ? pts[1].fY == bounds.fTop ? SkRRect::kUpperLeft_Corner : SkRRect::kLowerLeft_Corner
      ...
      SkASSERT(
          (verb == SkPath::kLine_Verb && (!(pts[1].fX - pts[0].fX) || !(pts[1].fY - pts[0].fY))) ||
...
uint8_t SkPathRef::Iter::next(SkPoint pts[4]) {
  ...
  switch (verb) {
    ...
    case SkPath::kCubic_Verb:
      ...
      break;
    case SkPath::kClose_Verb: break;
    case SkPath::kDone_Verb: SkASSERT(fVerbs == fVerbStop); break;
  }
  fPts = srcPts;
...
bool SkPathRef::isValid() const {
  ...
  if (this->currSize() != fFreeSpace + sizeof(SkPoint) * fPointCnt + sizeof(uint8_t) * fVerbCnt) {
    return false;
  }
