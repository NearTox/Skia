...
static MutateResult mutate_conservative_op(SkRegion::Op* op, bool inverseFilled) {
  if (inverseFilled) {
    ...
    switch (*op) {
      case SkRegion::kIntersect_Op:
      case SkRegion::kUnion_Op:
      case SkRegion::kReplace_Op: return kContinue_MutateResult;
      case SkRegion::kDifference_Op:
...
void SkConservativeClip::opRect(
  ...
  switch (mutate_conservative_op(&op, false)) {
    ...
    case kReplaceClippedAgainstGlobalBounds_MutateResult: ir = devBounds; break;
...
bool SkRasterClip::op(
    const SkRRect& rrect, const SkMatrix& matrix, const SkIRect& devBounds, SkRegion::Op op,
    bool doAA) {
...
bool SkRasterClip::op(
    const SkPath& path, const SkMatrix& matrix, const SkIRect& devBounds, SkRegion::Op op,
    bool doAA) {
  AUTO_RASTERCLIP_VALIDATE(*this);
  SkIRect bounds(devBounds);
  this->applyClipRestriction(op, &bounds);
...
bool SkRasterClip::op(
    const SkRect& localRect, const SkMatrix& matrix, const SkIRect& devBounds, SkRegion::Op op,
    bool doAA) {
  AUTO_RASTERCLIP_VALIDATE(*this);
  SkRect devRect;
