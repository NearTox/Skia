...
//-- SkGlyph ---------------------------------------------------------------------------------------
SkGlyph::SkGlyph(const SkGlyph&) noexcept = default;
SkGlyph& SkGlyph::operator=(const SkGlyph&) noexcept = default;
SkGlyph::SkGlyph(SkGlyph&&) noexcept = default;
SkGlyph& SkGlyph::operator=(SkGlyph&&) noexcept = default;
...
static size_t format_alignment(SkMask::Format format) {
  switch (format) {
    ...
    case SkMask::kSDF_Format: return alignof(uint8_t);
    case SkMask::kARGB32_Format: return alignof(uint32_t);
    case SkMask::kLCD16_Format: return alignof(uint16_t);
    default: SK_ABORT("Unknown mask format."); break;
  }
  return 0;
}
...
size_t SkGlyph::setMetricsAndImage(SkArenaAlloc* alloc, const SkGlyph& from) {
  ...
  if (fImage == nullptr) {
    ...
    SkDEBUGCODE(fAdvancesBoundsFormatAndInitialPathDone =
                    from.fAdvancesBoundsFormatAndInitialPathDone;)
  }
  return 0;
}
...
static std::tuple<SkScalar, SkScalar> calculate_path_gap(
  ...
  SkScalar left = SK_ScalarMax, right = SK_ScalarMin;
  ...
  while (SkPath::kDone_Verb != (verb = iter.next(pts))) {
    switch (verb) {
      ...
      case SkPath::kCubic_Verb: {
        SkScalar quadTop = std::min(std::min(std::min(pts[0].fY, pts[1].fY), pts[2].fY), pts[3].fY);
        if (bottomOffset < quadTop) {
          break;
        }
        SkScalar quadBottom =
...
void SkGlyph::ensureIntercepts(
    const SkScalar* bounds, SkScalar scale, SkScalar xPos, SkScalar* array, int* count,
    SkArenaAlloc* alloc) {
  ...
  const SkGlyph::Intercept* match = [this](const SkScalar bounds[2]) -> const SkGlyph::Intercept* {
    if (!fPathData) {
      return nullptr;
    }
...
SkVector SkGlyphPositionRoundingSpec::HalfAxisSampleFreq(
  ...
  if (!isSubpixel) {
    ...
    switch (axisAlignment) {
      case SkAxisAlignment::kX: return {SkPackedGlyphID::kSubpixelRound, SK_ScalarHalf};
      case SkAxisAlignment::kY: return {SK_ScalarHalf, SkPackedGlyphID::kSubpixelRound};
...
SkIPoint SkGlyphPositionRoundingSpec::IgnorePositionMask(
  ...
  return SkIPoint::Make(
      (!isSubpixel || axisAlignment == SkAxisAlignment::kY) ? 0 : ~0,
