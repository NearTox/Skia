...
sk_sp<SkDataTable> SkDataTable::MakeCopyArrays(
    const void* const* ptrs, const size_t sizes[], int count) {
  if (count <= 0) {
...
sk_sp<SkDataTable> SkDataTable::MakeArrayProc(
    const void* array, size_t elemSize, int count, FreeProc proc, void* ctx) {
  if (count <= 0) {
    return SkDataTable::MakeEmpty();
  }
