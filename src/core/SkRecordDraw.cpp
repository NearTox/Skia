...
void SkRecordDraw(
    const SkRecord& record, SkCanvas* canvas, SkPicture const* const drawablePicts[],
    SkDrawable* const drawables[], int drawableCount, const SkBBoxHierarchy* bbh,
...
void SkRecordPartialDraw(
    const SkRecord& record, SkCanvas* canvas, SkPicture const* const drawablePicts[],
    int drawableCount, int start, int stop, const SkM44& initialCTM) {
  SkAutoCanvasRestore saveRestore(canvas, true /*save now, restore at exit*/);
...
template <>
void Draw::draw(const DrawImageLattice2& r) {
  SkCanvas::Lattice lattice;
  lattice.fXCount = r.xCount;
  lattice.fXDivs = r.xDivs;
  lattice.fYCount = r.yCount;
...
DRAW(
    DrawAtlas,
    drawAtlas(r.atlas.get(), r.xforms, r.texs, r.colors, r.count, r.mode, r.cull, r.paint));
DRAW(
    DrawAtlas2,
    drawAtlas(
        r.atlas.get(), r.xforms, r.texs, r.colors, r.count, r.mode, r.sampling, r.cull, r.paint));
...
class FillBounds : SkNoncopyable {
 ...
 private:
  ...
  void pushSaveBlock(const SkPaint* paint) {
    ...
    sb.bounds = PaintMayAffectTransparentBlack(paint) ? fCullRect : Bounds::MakeEmpty();
    sb.paint = paint;
    sb.ctm = this->fCTM;
  ...
  static bool PaintMayAffectTransparentBlack(const SkPaint* paint) {
    if (paint) {
      ...
      switch (paint->getBlendMode()) {
        ...
        case SkBlendMode::kModulate: return true; break;
        default: break;
      }
    }
    return false;
  ...
  Bounds bounds(const DrawImageLattice& op) const { return this->adjustAndMap(op.dst, op.paint); }
  Bounds bounds(const DrawImageLattice2& op) const { return this->adjustAndMap(op.dst, op.paint); }
  Bounds bounds(const DrawImageRect& op) const { return this->adjustAndMap(op.dst, op.paint); }
  Bounds bounds(const DrawImageRect2& op) const { return this->adjustAndMap(op.dst, op.paint); }
  ...
  Bounds bounds(const DrawAnnotation& op) const { return this->adjustAndMap(op.rect, nullptr); }
