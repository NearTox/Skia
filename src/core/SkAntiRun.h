...
class SkAlphaRuns {
 public:
   ...
   */
  SK_ALWAYS_INLINE int add(
      int x, U8CPU startAlpha, int middleCount, U8CPU stopAlpha, U8CPU maxValue, int offsetX) {
    ...
    if (startAlpha) {
      ...
      */
      ...
      SkASSERT(tmp <= 256);
      alpha[x] = SkToU8(
          tmp - (tmp >> 8));  // was (tmp >> 7), but that seems wrong if we're trying to catch 256
