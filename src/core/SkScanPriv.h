...
class SkScanClipper {
 public:
  SkScanClipper(
      SkBlitter* blitter, const SkRegion* clip, const SkIRect& bounds, bool skipRejectTest = false,
      bool boundsPreClipped = false);
...
void sk_fill_path(
    const SkPathView&, const SkIRect& clipRect, SkBlitter* blitter, int start_y, int stop_y,
...
static inline bool TryBlitFatAntiRect(
    SkBlitter* blitter, const SkPathView& path, const SkIRect& clip) {
  SkRect rect;
  if (!path.isRect(&rect)) {
