...
bool SkBitmap::tryAllocN32Pixels(int width, int height, bool isOpaque) {
  SkImageInfo info =
      SkImageInfo::MakeN32(width, height, isOpaque ? kOpaque_SkAlphaType : kPremul_SkAlphaType);
...
void SkBitmap::allocPixels() { this->allocPixels((Allocator*)nullptr); }
...
void SkBitmap::allocPixels(const SkImageInfo& info) { this->allocPixels(info, info.minRowBytes()); }
...
bool SkBitmap::tryAllocPixelsFlags(const SkImageInfo& requestedInfo, uint32_t allocFlags) {
  ...
  sk_sp<SkPixelRef> pr = SkMallocPixelRef::MakeAllocate(correctedInfo, correctedInfo.minRowBytes());
  if (!pr) {
    return reset_return_false(this);
  }
  this->setPixelRef(std::move(pr), 0, 0);
...
bool SkBitmap::installPixels(
    const SkImageInfo& requestedInfo, void* pixels, size_t rb,
  ...
  sk_sp<SkPixelRef> pr =
      SkMallocPixelRef::MakeWithProc(correctedInfo, rb, pixels, releaseProc, context);
  if (!pr) {
    this->reset();
    return false;
  }
...
bool SkBitmap::installPixels(const SkPixmap& pixmap) {
  return this->installPixels(
      pixmap.info(), pixmap.writable_addr(), pixmap.rowBytes(), nullptr, nullptr);
}
...
bool SkBitmap::readPixels(
    const SkImageInfo& requestedDstInfo, void* dstPixels, size_t dstRB, int x, int y) const {
  SkPixmap src;
...
bool SkBitmap::extractAlpha(
  ...
  tmpBitmap.setInfo(
      SkImageInfo::MakeA8(dstM.fBounds.width(), dstM.fBounds.height()), dstM.fRowBytes);
  if (!tmpBitmap.tryAllocPixels(allocator)) {
    // Allocation of pixels for alpha bitmap failed.
    SkDebugf(
        "extractAlpha failed to allocate (%d,%d) alpha bitmap\n", tmpBitmap.width(),
