...
class InverseBlitter : public SkBlitter {
 public:
  ...
  void blitV(int x, int y, int height, SkAlpha alpha) override { SkDEBUGFAIL("blitV unexpected"); }
  void blitRect(int x, int y, int width, int height) override {
    SkDEBUGFAIL("blitRect unexpected");
  }
  void blitMask(const SkMask&, const SkIRect& clip) override { SkDEBUGFAIL("blitMask unexpected"); }
...
void sk_fill_path(
  ...
  if (0 == count) {
    if (path.isInverseFillType()) {
      ...
      if (!rect.isEmpty()) {
        blitter->blitRect(
            rect.fLeft << shiftEdgesUp, rect.fTop << shiftEdgesUp, rect.width() << shiftEdgesUp,
 ...
 */
SkScanClipper::SkScanClipper(
    SkBlitter* blitter, const SkRegion* clip, const SkIRect& ir, bool skipRejectTest,
    bool irPreClipped) {
...
void SkScan::FillPath(const SkPath& path, const SkRegion& origClip, SkBlitter* blitter) {
  ...
  if (blitter) {
    ...
    sk_fill_path(
        path, clipPtr->getBounds(), blitter, ir.fTop, ir.fBottom, 0,
...
static int build_tri_edges(
    SkEdge edge[], const SkPoint pts[], const SkIRect* clipRect, SkEdge* list[]) {
  SkEdge** start = list;
...
static void sk_fill_triangle(
    const SkPoint pts[], const SkIRect* clipRect, SkBlitter* blitter, const SkIRect& ir) {
