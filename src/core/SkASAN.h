...
static inline void sk_asan_poison_memory_region(void const volatile* addr, size_t size) noexcept {
#ifdef SK_SANITIZE_ADDRESS
  __asan_poison_memory_region(addr, size);
#endif
}

static inline void sk_asan_unpoison_memory_region(void const volatile* addr, size_t size) noexcept {
#ifdef SK_SANITIZE_ADDRESS
