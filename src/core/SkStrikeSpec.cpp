...
SkStrikeSpec::SkStrikeSpec(const SkDescriptor& descriptor, sk_sp<SkTypeface> typeface)
    : fAutoDescriptor{descriptor}, fTypeface{std::move(typeface)} {}
...
std::tuple<SkStrikeSpec, SkScalar> SkStrikeSpec::MakePath(
  ...
  return {
      SkStrikeSpec(pathFont, pathPaint, surfaceProps, scalerContextFlags, SkMatrix::I()),
      strikeToSourceScale};
}
...
std::tuple<SkStrikeSpec, SkScalar> SkStrikeSpec::MakeSourceFallback(
  ...
  if (maxSourceGlyphDimension > maxAtlasDimension) {
    ...
    fallbackTextSize =
        SkScalarFloorToScalar((maxAtlasDimension / maxSourceGlyphDimension) * runFontTextSize);
  }
  ...
  return {
      SkStrikeSpec(fallbackFont, paint, surfaceProps, scalerContextFlags, SkMatrix::I()),
      strikeToSourceScale};
}
...
std::tuple<SkStrikeSpec, SkScalar> SkStrikeSpec::MakeCanonicalized(
  ...
  return {
      SkStrikeSpec(
          *canonicalizedFont, canonicalizedPaint, SkSurfaceProps(),
...
SkStrikeSpec SkStrikeSpec::MakeWithNoDevice(const SkFont& font, const SkPaint* paint) {
  ...
  return SkStrikeSpec(
      font, setupPaint, SkSurfaceProps(), SkScalerContextFlags::kFakeGammaAndBoostContrast,
      SkMatrix::I());
}
...
SkString SkStrikeSpec::dump() const { return fAutoDescriptor.getDesc()->dumpRec(); }
...
#if SK_SUPPORT_GPU || defined(SK_GRAPHITE_ENABLED)
std::tuple<SkStrikeSpec, SkScalar, sktext::gpu::SDFTMatrixRange> SkStrikeSpec::MakeSDFT(
    const SkFont& font, const SkPaint& paint, const SkSurfaceProps& surfaceProps,
    const SkMatrix& deviceMatrix, const sktext::gpu::SDFTControl& control) {
...
void SkBulkGlyphMetricsAndPaths::findIntercepts(
    const SkScalar* bounds, SkScalar scale, SkScalar xPos, const SkGlyph* glyph, SkScalar* array,
    int* count) {
