...
static skvm::Coord device_coord(skvm::Builder* p, skvm::Uniforms* uniforms) {
  skvm::I32 dx = p->uniform32(uniforms->base, offsetof(BlitterUniforms, right)) - p->index(),
...
struct NoopColorFilter : public SkColorFilterBase {
  skvm::Color onProgram(
      skvm::Builder*, skvm::Color c, const SkColorInfo&, skvm::Uniforms*,
...
struct DitherShader : public SkShaderBase {
  ...
  skvm::Color onProgram(
    ...
    skvm::Color c =
        as_SB(fShader)->program(p, device, local, paint, matrices, localM, dst, uniforms, alloc);
    if (!c) {
      return {};
    }
    ...
    switch (dst.colorType()) {
      ...
      case kR8_unorm_SkColorType: rate = 1 / 255.0f; break;
    ...
    skvm::I32 X = trunc(device.x - 0.5f), Y = X ^ trunc(device.y - 0.5f);

    // If X's low bits are abc and Y's def, M is fcebda,
    // 6 bits producing all values [0,63] shuffled over an 8x8 grid.
    skvm::I32 M = shl(Y & 1, 5) | shl(X & 1, 4) | shl(Y & 2, 2) | shl(X & 2, 1) | shr(Y & 4, 1) |
                  shr(X & 4, 2);
    ...
    float scale = rate * (2 / 128.0f), bias = rate * (-63 / 128.0f);
...
bool SkVMBlitter::Key::operator==(const Key& that) const {
  return this->shader == that.shader && this->clip == that.clip && this->blender == that.blender &&
         this->colorSpace == that.colorSpace && this->colorType == that.colorType &&
         this->alphaType == that.alphaType && this->coverage == that.coverage;
}
...
SkVMBlitter::Params SkVMBlitter::EffectiveParams(
    const SkPixmap& device, const SkPixmap* sprite, SkPaint paint, const SkMatrixProvider& matrices,
    sk_sp<SkShader> clip) {
  ...
  SkColorSpaceXformSteps{
      sk_srgb_singleton(), kUnpremul_SkAlphaType, device.colorSpace(), kUnpremul_SkAlphaType}
      .apply(paintColor.vec());

  return {
      std::move(shader),  std::move(clip),
      std::move(blender), {device.colorType(), device.alphaType(), device.refColorSpace()},
...
void SkVMBlitter::BuildProgram(
    skvm::Builder* p, const Params& params, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) {
  ...
  skvm::Color src = as_SB(params.shader)
                        ->program(
                            p, device, /*local=*/device, paint, params.matrices, /*localM=*/nullptr,
  ...
  switch (params.coverage) {
    case Coverage::Full: cov.r = cov.g = cov.b = cov.a = p->splat(1.0f); break;

    case Coverage::UniformF: cov.r = cov.g = cov.b = cov.a = p->uniformF(p->uniform(), 0); break;
    ...
    case Coverage::kCount: SkUNREACHABLE;
  }
  if (params.clip) {
    skvm::Color clip = as_SB(params.clip)
                           ->program(
                               p, device, /*local=*/device, paint, params.matrices,
  ...
  if (as_blendmode && SkBlendMode_ShouldPreScaleCoverage(
                          as_blendmode.value(), params.coverage == Coverage::MaskLCD16)) {
...
SkVMBlitter::Key SkVMBlitter::CacheKey(
  ...
  skvm::Uniform r = uniforms->pushF(params.paint.fR), g = uniforms->pushF(params.paint.fG),
                b = uniforms->pushF(params.paint.fB), a = uniforms->pushF(params.paint.fA);

  auto hash_shader = [&](skvm::Builder& p, const sk_sp<SkShader>& shader, skvm::Color* outColor) {
    ...
    *outColor = sb->program(
    ...
    if (*outColor) {
      ...
      const skvm::Val outputs[] = {outColor->r.id, outColor->g.id, outColor->b.id, outColor->a.id};
...
SkVMBlitter::SkVMBlitter(
    const SkPixmap& device, const SkPaint& paint, const SkPixmap* sprite, SkIPoint spriteOffset,
    const SkMatrixProvider& matrices, sk_sp<SkShader> clip, bool* ok)
    : fDevice(device),
...
SkString SkVMBlitter::DebugName(const Key& key) {
  return SkStringPrintf(
      "Shader-%" PRIx64 "_Clip-%" PRIx64 "_Blender-%" PRIx64 "_CS-%" PRIx64 "_CT-%d_AT-%d_Cov-%d",
      key.shader, key.clip, key.blender, key.colorSpace, key.colorType, key.alphaType,
      key.coverage);
}
...
const void* SkVMBlitter::isSprite(int x, int y) const {
  if (fSprite.colorType() != kUnknown_SkColorType) {
    return fSprite.addr(x - fSpriteOffset.x(), y - fSpriteOffset.y());
  }
  return nullptr;
}
...
void SkVMBlitter::blitMask(const SkMask& mask, const SkIRect& clip) {
  ...
  switch (mask.fFormat) {
    ...
    case SkMask::k3D_Format: program = this->buildProgram(Coverage::Mask3D); break;

    case SkMask::kA8_Format: program = this->buildProgram(Coverage::MaskA8); break;

    case SkMask::kLCD16_Format: program = this->buildProgram(Coverage::MaskLCD16); break;
  }
  ...
  if (program) {
    ...
    for (int y = clip.top(); y < clip.bottom(); y++) {
      ...
      if (mask.fFormat == SkMask::k3D_Format) {
        ...
        if (const void* sprite = this->isSprite(x, y)) {
          program->eval(
              w, fUniforms.buf.data(), dptr, sprite, mptr + 1 * plane, mptr + 2 * plane,
              mptr + 0 * plane);
        } else {
          program->eval(
...
SkVMBlitter* SkVMBlitter::Make(
    const SkPixmap& device, const SkPaint& paint, const SkMatrixProvider& matrices,
    SkArenaAlloc* alloc, sk_sp<SkShader> clip) {
  bool ok = true;
...
SkVMBlitter* SkVMBlitter::Make(
    const SkPixmap& device, const SkPaint& paint, const SkPixmap& sprite, int left, int top,
    SkArenaAlloc* alloc, sk_sp<SkShader> clip) {
  ...
  auto blitter = alloc->make<SkVMBlitter>(
      device, paint, &sprite, SkIPoint{left, top}, SkMatrixProvider{SkMatrix{}}, std::move(clip),
      &ok);
  return ok ? blitter : nullptr;
}
