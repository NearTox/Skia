...
#define TODO SkUNREACHABLE
    ...
    Color cov;
    switch (key.coverage) {
      case Coverage::Full: apply_coverage = false; break;

      case Coverage::UniformA8:
        cov.r = cov.g = cov.b = cov.a = uniform8(uniforms, offsetof(Uniforms, coverage));
        break;

      case Coverage::MaskA8: cov.r = cov.g = cov.b = cov.a = load8(varying<uint8_t>()); break;

      case Coverage::MaskLCD16:
        cov = unpack_565(load16(varying<uint16_t>()));
        cov.a =
            select(lt(src.a, dst.a), min(cov.r, min(cov.g, cov.b)), max(cov.r, max(cov.g, cov.b)));
        break;
