...
struct Key {
  uint64_t shader, clip, colorSpace;
...
static Key cache_key(
  ...
  auto hash_shader = [&](const sk_sp<SkShader>& shader) {
    ...
    uint64_t hash = 0;
    if (auto c = sb->program(
            &p, device, /*local=*/device, paint, params.matrices, /*localM=*/nullptr,
            params.quality, params.dst, uniforms, alloc)) {
      hash = p.hash();
  ...
  if (params.clip) {
    skvm::Color clip = as_SB(params.clip)
                           ->program(
                               ...
                               /*localM=*/nullptr, params.quality, params.dst, uniforms, alloc);
    SkAssertResult(clip);
...
struct NoopColorFilter : public SkColorFilterBase {
  skvm::Color onProgram(
      skvm::Builder*, skvm::Color c, SkColorSpace*, skvm::Uniforms*, SkArenaAlloc*) const override {
    return c;
  }
...
struct SpriteShader : public SkShaderBase {
  ...
  skvm::Color onProgram(
      ...
      const SkMatrixProvider&, const SkMatrix* /*localM*/, SkFilterQuality, const SkColorInfo& dst,
...
struct DitherShader : public SkShaderBase {
  ...
  skvm::Color onProgram(
    ...
    skvm::Color c = as_SB(fShader)->program(
      ...
      for (int y = clip.top(); y < clip.bottom(); y++) {
        ...
        if (program == &fBlitMask3D) {
          ...
          if (const void* sprite = this->isSprite(x, y)) {
            ...
            program->eval(
                w, fUniforms.buf.data(), dptr, mptr + 1 * plane, mptr + 2 * plane,
                mptr + 0 * plane);
          }
        } else {
          if (const void* sprite = this->isSprite(x, y)) {
