...
struct PaintColorShader : public SkShaderBase {
  ...
  // Only created here temporarily... never serialized.
  Factory getFactory() const override { return nullptr; }
  const char* getTypeName() const noexcept override { return "PaintColorShader"; }

  bool isOpaque() const override { return fIsOpaque; }

  skvm::Color onProgram(
      skvm::Builder*, skvm::Coord, skvm::Coord, skvm::Color paint, const SkMatrixProvider&,
...
void SkVMBlitter::BuildProgram(
  ...
  if (!src_in_gamut && params.dst.alphaType() == kPremul_SkAlphaType &&
  ...
  if (as_blendmode && src_in_gamut) {
    ...
    skvm::F32 lo = pun_to_F32(p->splat(0xb400'0000)), hi = pun_to_F32(p->splat(0x3f80'0001));
...
SkVMBlitter* SkVMBlitter::Make(
  ...
  auto blitter = alloc->make<SkVMBlitter>(
      device, paint, &sprite, SkIPoint{left, top}, SkSimpleMatrixProvider{SkMatrix{}},
      std::move(clip), &ok);
  return ok ? blitter : nullptr;
}
