...
static bool operator==(const Key& x, const Key& y) {
  return x.colorType == y.colorType && x.alphaType == y.alphaType && x.coverage == y.coverage &&
         x.blendMode == y.blendMode && x.shader == y.shader && x.colorFilter == y.colorFilter;
}

static SkString debug_name(const Key& key) {
  return SkStringPrintf(
      "CT%d-AT%d-Cov%d-Blend%d-Shader%d-CF%d", key.colorType, key.alphaType, key.coverage,
      key.blendMode, SkToBool(key.shader), SkToBool(key.colorFilter));
}
...
struct Builder : public skvm::Builder {
  ...
  skvm::I32 inv(skvm::I32 x) { return sub(splat(255), x); }
  ...
  skvm::I32 mix(skvm::I32 x, skvm::I32 y, skvm::I32 t) {
    return div255(add(mul(x, inv(t)), mul(y, t)));
  }
  ...
  skvm::I32 pack_8888(Color c) { return pack(pack(c.r, c.g, 8), pack(c.b, c.a, 8), 16); }

  Color unpack_565(skvm::I32 bgr) {
    // N.B. kRGB_565_SkColorType is named confusingly;
    //      blue is in the low bits and red the high.
    skvm::I32 r = extract(bgr, 11, splat(0b011'111)), g = extract(bgr, 5, splat(0b111'111)),
  ...
  skvm::I32 pack_565(Color c) {
    skvm::I32 r = div255(mul(c.r, splat(31))), g = div255(mul(c.g, splat(63))),
  ...
  static bool CanBuild(const Key& key) {
    ...
    if (key.alphaType == kUnpremul_SkAlphaType) {
      return false;
    }
...
#define TODO SkUNREACHABLE
    ...
    skvm::Arg uniforms = uniform(), dst_ptr = arg(SkColorTypeBytesPerPixel(key.colorType));
    ...
    if (SkBlendMode_ShouldPreScaleCoverage(key.blendMode, key.coverage == Coverage::MaskLCD16)) {
      Color cov;
...
class Blitter final : public SkBlitter {
 public:
  ...
  Blitter(const SkPixmap& device, const SkPaint& paint)
      : fDevice(device),
        fKey{
            device.colorType(),   device.alphaType(), Coverage::Full,
            paint.getBlendMode(), paint.getShader(),  paint.getColorFilter(),
        } {
    SkColor4f color = paint.getColor4f();
    SkColorSpaceXformSteps{sk_srgb_singleton(), kUnpremul_SkAlphaType, device.colorSpace(),
 ...
 private:
  ...
  const Key fKey;
  Uniforms fUniforms;
  skvm::Program fBlitH, fBlitAntiH, fBlitMaskA8, fBlitMaskLCD16;
  ...
  void blitMask(const SkMask& mask, const SkIRect& clip) override {
    ...
    if (program) {
      for (int y = clip.top(); y < clip.bottom(); y++) {
        program->eval(
            clip.width(), &fUniforms, fDevice.addr(clip.left(), y), mask.getAddr(clip.left(), y));
      }
    }
  }
};
...
SkBlitter* SkCreateSkVMBlitter(
  ...
  return blitter->ok ? blitter : nullptr;
}
