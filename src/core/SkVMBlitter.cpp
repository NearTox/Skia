...
static void build_program(
        ...
        switch (params.coverage) {
          ...
          case Coverage::MaskLCD16:
            ...
            *cov = unpack_565(p->load16(p->varying<uint16_t>()));
            cov->a = select(
                src.a < dst.a, min(cov->r, min(cov->g, cov->b)), max(cov->r, max(cov->g, cov->b)));
            break;
        }
  ...
  SkDEBUGCODE(dst_loaded = true;) switch (params.dst.colorType()) {
    ...
    case kRGBA_8888_SkColorType: dst = unpack_8888(p->load32(dst_ptr)); break;

    case kBGRA_8888_SkColorType:
      dst = unpack_8888(p->load32(dst_ptr));
      std::swap(dst.r, dst.b);
      break;

    case kRGB_101010x_SkColorType: [[fallthrough]];
    case kRGBA_1010102_SkColorType: dst = unpack_1010102(p->load32(dst_ptr)); break;

    case kBGR_101010x_SkColorType: [[fallthrough]];
    case kBGRA_1010102_SkColorType:
      dst = unpack_1010102(p->load32(dst_ptr));
      std::swap(dst.r, dst.b);
      break;
  }
