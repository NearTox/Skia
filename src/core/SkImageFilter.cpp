...
SkImageFilter::~SkImageFilter() { SkImageFilterCache::Get()->purgeByImageFilter(this); }

SkImageFilter::SkImageFilter(int inputCount, SkReadBuffer& buffer)
    : fUsesSrcInput(false), fCropRect(SkRect(), 0x0), fUniqueID(next_image_filter_unique_id()) {
  Common common;
...
sk_sp<SkSpecialImage> SkImageFilter::filterImage(
    SkSpecialImage* src, const Context& context, SkIPoint* offset) const {
  SkASSERT(src && offset);
  if (!context.isValid()) {
    return nullptr;
  }
...
#if SK_SUPPORT_GPU
sk_sp<SkSpecialImage> SkImageFilter::DrawWithFP(
    GrRecordingContext* context, std::unique_ptr<GrFragmentProcessor> fp, const SkIRect& bounds,
    const OutputProperties& outputProperties, GrProtected isProtected) {
  GrPaint paint;
  ...
  sk_sp<GrRenderTargetContext> renderTargetContext(context->priv().makeDeferredRenderTargetContext(
      SkBackingFit::kApprox, bounds.width(), bounds.height(), colorType, std::move(colorSpace), 1,
      GrMipMapped::kNo, kBottomLeft_GrSurfaceOrigin, nullptr, SkBudgeted::kYes, isProtected));
...
bool SkImageFilter::applyCropRect(
...
#if SK_SUPPORT_GPU
sk_sp<SkSpecialImage> SkImageFilter::ImageToColorSpace(
    SkSpecialImage* src, const OutputProperties& outProps) {
  ...
  auto colorSpaceXform = GrColorSpaceXform::Make(
      src->getColorSpace(), src->alphaType(), outProps.colorSpace(), kPremul_SkAlphaType);
  ...
  sk_sp<SkSpecialSurface> surf(
      src->makeSurface(outProps, SkISize::Make(src->width(), src->height())));
  if (!surf) {
    return sk_ref_sp(src);
  }
...
static sk_sp<SkSpecialImage> pad_image(
    SkSpecialImage* src, const SkImageFilter::OutputProperties& outProps, int newWidth,
    int newHeight, int offX, int offY) {
...
sk_sp<SkSpecialImage> SkImageFilter::applyCropRectAndPad(
  ...
  if (srcBounds.contains(*bounds)) {
    ...
    sk_sp<SkSpecialImage> img(pad_image(
        src, ctx.outputProperties(), bounds->width(), bounds->height(),
        Sk32_sat_sub(srcOffset->x(), bounds->x()), Sk32_sat_sub(srcOffset->y(), bounds->y())));
    *srcOffset = SkIPoint::Make(bounds->x(), bounds->y());
    return img;
  }
}

SkIRect SkImageFilter::onFilterBounds(
...
SkIRect SkImageFilter::onFilterNodeBounds(
...
sk_sp<SkSpecialImage> SkImageFilter::filterInput(
    int index, SkSpecialImage* src, const Context& ctx, SkIPoint* offset) const {
...
void SkImageFilter::PurgeCache() { SkImageFilterCache::Get()->purge(); }
...
sk_sp<SkImageFilter> SkApplyCTMToFilter(
...
SkIRect SkFilterNodeBounds(
    const SkImageFilter* filter, const SkIRect& srcRect, const SkMatrix& ctm,
