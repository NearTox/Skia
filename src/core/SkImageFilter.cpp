...
SkIRect SkImageFilter::filterBounds(
    const SkIRect& src, const SkMatrix& ctm, MapDirection direction,
  ...
  if (kReverse_MapDirection == direction) {
    ...
    if (as_IFB(this)->cropRectIsSet()) {
      skif::LayerSpace<SkIRect> outputCrop =
          mapping.paramToLayer(skif::ParameterSpace<SkRect>(as_IFB(this)->getCropRect().rect()))
              .roundOut();
...
SkImageFilter_Base::SkImageFilter_Base(
    sk_sp<SkImageFilter> const* inputs, int inputCount, const SkRect* cropRect)
    : fUsesSrcInput(false), fCropRect(cropRect), fUniqueID(next_image_filter_unique_id()) {
...
SkImageFilter_Base::~SkImageFilter_Base() { SkImageFilterCache::Get()->purgeByImageFilter(this); }
...
bool SkImageFilter_Base::Common::unflatten(SkReadBuffer& buffer, int expectedCount) {
  ...
  if (!buffer.isValid() || !buffer.validate(flags == 0x0 || flags == CropRect::kHasAll_CropEdge)) {
    return false;
  }
...
skif::FilterResult SkImageFilter_Base::filterImage(const skif::Context& context) const {
  ...
  const SkIRect srcSubset = fUsesSrcInput ? context.sourceImage()->subset() : SkIRect::MakeWH(0, 0);

  SkImageFilterCacheKey key(
      fUniqueID, context.mapping().layerMatrix(), context.clipBounds(), srcGenID, srcSubset);
...
skif::LayerSpace<SkIRect> SkImageFilter_Base::getInputBounds(
  ...
  skif::LayerSpace<SkIRect> contentBounds =
      knownContentBounds ? mapping.paramToLayer(*knownContentBounds).roundOut() : desiredBounds;

  // Process the layer-space desired output with the filter DAG to determine required input
  skif::LayerSpace<SkIRect> requiredInput =
      this->onGetInputLayerBounds(mapping, desiredBounds, contentBounds);
...
skif::DeviceSpace<SkIRect> SkImageFilter_Base::getOutputBounds(
  ...
  skif::LayerSpace<SkIRect> filterOutput =
  ...
  SkIRect dst;
  as_IFB(this)->getCropRect().applyTo(
      SkIRect(filterOutput), mapping.layerMatrix(), as_IFB(this)->onAffectsTransparentBlack(),
      &dst);
...
void SkImageFilter_Base::CropRect::applyTo(
    const SkIRect& imageBounds, const SkMatrix& ctm, bool embiggen, SkIRect* cropped) const {
  *cropped = imageBounds;
  if (fFlags) {
    SkRect devCropR;
...
bool SkImageFilter_Base::applyCropRect(
    const Context& ctx, const SkIRect& srcBounds, SkIRect* dstBounds) const {
...
// around it.
static sk_sp<SkSpecialImage> pad_image(
    SkSpecialImage* src, const SkImageFilter_Base::Context& ctx, int newWidth, int newHeight,
    int offX, int offY) {
...
sk_sp<SkSpecialImage> SkImageFilter_Base::applyCropRectAndPad(
    const Context& ctx, SkSpecialImage* src, SkIPoint* srcOffset, SkIRect* bounds) const {
  const SkIRect srcBounds =
      SkIRect::MakeXYWH(srcOffset->x(), srcOffset->y(), src->width(), src->height());
  ...
  if (srcBounds.contains(*bounds)) {
    ...
    sk_sp<SkSpecialImage> img(pad_image(
        src, ctx, bounds->width(), bounds->height(), Sk32_sat_sub(srcOffset->x(), bounds->x()),
...
SkIRect SkImageFilter_Base::onFilterBounds(
...
SkIRect SkImageFilter_Base::onFilterNodeBounds(
    const SkIRect& src, const SkMatrix&, MapDirection, const SkIRect*) const {
  return src;
}
...
skif::LayerSpace<SkIRect> SkImageFilter_Base::visitOutputLayerBounds(
  ...
  for (int i = 0; i < this->countInputs(); ++i) {
    ...
    skif::LayerSpace<SkIRect> output =
        filter ? as_IFB(filter)->onGetOutputLayerBounds(mapping, contentBounds) : contentBounds;
    // Accumulate with all other filters
    if (i == 0) {
      netOutput = output;
    } else {
...
skif::LayerSpace<SkIRect> SkImageFilter_Base::onGetInputLayerBounds(
  ...
  SkIRect input = this->onFilterNodeBounds(
      SkIRect(desiredOutput), mapping.layerMatrix(), kReverse_MapDirection, &content);
  if (recurse == VisitChildren::kYes) {
    SkIRect aggregate =
        this->onFilterBounds(input, mapping.layerMatrix(), kReverse_MapDirection, &input);
...
skif::LayerSpace<SkIRect> SkImageFilter_Base::onGetOutputLayerBounds(
  ...
  SkIRect aggregate = this->onFilterBounds(
      SkIRect(contentBounds), mapping.layerMatrix(), kForward_MapDirection, nullptr);
  SkIRect output =
      this->onFilterNodeBounds(aggregate, mapping.layerMatrix(), kForward_MapDirection, nullptr);
...
#if SK_SUPPORT_GPU
sk_sp<SkSpecialImage> SkImageFilter_Base::DrawWithFP(
    GrRecordingContext* rContext, std::unique_ptr<GrFragmentProcessor> fp, const SkIRect& bounds,
    SkColorType colorType, const SkColorSpace* colorSpace, const SkSurfaceProps& surfaceProps,
    GrProtected isProtected) {
  GrImageInfo info(
      SkColorTypeToGrColorType(colorType), kPremul_SkAlphaType, sk_ref_sp(colorSpace),
      bounds.size());

  auto sfc = rContext->priv().makeSFC(
      info, SkBackingFit::kApprox, 1, GrMipmapped::kNo, isProtected, kBottomLeft_GrSurfaceOrigin);
  if (!sfc) {
    return nullptr;
  }
  ...
  return SkSpecialImage::MakeDeferredFromGpu(
      rContext, dstIRect, kNeedNewImageUniqueID_SpecialImage, sfc->readSurfaceView(),
      sfc->colorInfo().colorType(), sfc->colorInfo().refColorSpace(), surfaceProps);
}

sk_sp<SkSpecialImage> SkImageFilter_Base::ImageToColorSpace(
    SkSpecialImage* src, SkColorType colorType, SkColorSpace* colorSpace,
  ...
  auto colorSpaceXform = GrColorSpaceXform::Make(
      src->getColorSpace(), src->alphaType(), colorSpace, kPremul_SkAlphaType);
  ...
  sk_sp<SkSpecialSurface> surf(src->makeSurface(
      colorType, colorSpace, SkISize::Make(src->width(), src->height()), kPremul_SkAlphaType,
      surfaceProps));
  if (!surf) {
    return sk_ref_sp(src);
  }
...
SkIRect SkImageFilter_Base::DetermineRepeatedSrcBound(
  ...
  tmp.adjust(
      ...
      filterSize.fHeight - filterOffset.fY);
...
void SkImageFilter_Base::PurgeCache() { SkImageFilterCache::Get()->purge(); }
