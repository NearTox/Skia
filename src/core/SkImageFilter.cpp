...
SkIRect SkImageFilter::filterBounds(
  ...
  if (kReverse_MapDirection == direction) {
    ...
    SkIRect dst;
    as_IFB(this)->getCropRect().applyTo(bounds, ctm, as_IFB(this)->affectsTransparentBlack(), &dst);
    return dst;
  }
}
...
skif::FilterResult<For::kOutput> SkImageFilter_Base::filterImage(
  ...
  SkImageFilterCacheKey key(
      fUniqueID, context.layerMatrix(), context.clipBounds(), srcGenID, srcSubset);
...
SkImageFilter_Base::Context SkImageFilter_Base::mapContext(const Context& ctx) const {
  SkIRect clipBounds = this->onFilterNodeBounds(
      ctx.clipBounds(), ctx.ctm(), MapDirection::kReverse_MapDirection, &ctx.clipBounds());
...
static sk_sp<SkImageFilter> apply_ctm_to_filter(
    sk_sp<SkImageFilter> input, const SkMatrix& ctm, SkMatrix* remainder, bool asBackdrop) {
  ...
  if (asBackdrop) {
    ...
    if (ctmToEmbed.invert(&invEmbed)) {
      input = SkComposeImageFilter::Make(
          std::move(input), SkMatrixImageFilter::Make(invEmbed, kLow_SkFilterQuality, nullptr));
    }
  }
...
sk_sp<SkImageFilter> SkImageFilter_Base::applyCTMForBackdrop(
    const SkMatrix& ctm, SkMatrix* remainder) const {
