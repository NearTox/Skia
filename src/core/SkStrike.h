...
public:
    ...
    */
    void findIntercepts(const SkScalar bounds[2], SkScalar scale, SkScalar xPos, bool yAxis,
    ...
    */
    ...
    SkStrikeSpec strikeSpec() const override {
        return SkStrikeSpec{this->getDescriptor(), *this->getScalerContext()->getTypeface(),
...
private:
    ...
    static void OffsetResults(const SkGlyph::Intercept* intercept, SkScalar scale, SkScalar xPos,
                              SkScalar* array, int* count);
    static void AddInterval(SkScalar val, SkGlyph::Intercept* intercept);
    static void AddPoints(const SkPoint* pts, int ptCount, const SkScalar bounds[2], bool yAxis,
