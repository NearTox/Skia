...
public:
    ...
    static bool AreFinite(const SkPoint array[], int count) noexcept {
        return SkScalarsAreFinite(&array[0].fX, count << 1);
    }

    static const SkScalar* AsScalars(const SkPoint& pt) noexcept { return &pt.fX; }

    static constexpr bool CanNormalize(SkScalar dx, SkScalar dy) noexcept {
        return SkScalarsAreFinite(dx, dy) && (dx || dy);
    }

    static SkScalar DistanceToLineBetweenSqd(const SkPoint& pt, const SkPoint& a, const SkPoint& b,
                                             Side* side = nullptr);

    static SkScalar DistanceToLineBetween(const SkPoint& pt, const SkPoint& a, const SkPoint& b,
                                          Side* side = nullptr) {
    ...
    static SkScalar DistanceToSqd(const SkPoint& pt, const SkPoint& a) noexcept {
        SkScalar dx = pt.fX - a.fX;
        SkScalar dy = pt.fY - a.fY;
        return dx * dx + dy * dy;
    }

    static bool EqualsWithinTolerance(const SkPoint& p1, const SkPoint& p2) noexcept {
        return !CanNormalize(p1.fX - p2.fX, p1.fY - p2.fY);
    }

    static bool EqualsWithinTolerance(const SkPoint& pt, const SkPoint& p, SkScalar tol) noexcept {
    ...
    static void Negate(SkIPoint& pt) noexcept {
        pt.fX = -pt.fX;
        pt.fY = -pt.fY;
    }

    static void RotateCCW(const SkPoint& src, SkPoint* dst) noexcept {
    ...
    static void RotateCCW(SkPoint* pt) noexcept { RotateCCW(*pt, pt); }

    static void RotateCW(const SkPoint& src, SkPoint* dst) noexcept {
        // use a tmp in case src == dst
        SkScalar tmp = src.fX;
        dst->fX = -src.fY;
        dst->fY = tmp;
    ...
    static void SetRectTriStrip(SkPoint v[], const SkRect& rect, size_t stride) noexcept {
