...
class SkSpecialImage_Base : public SkSpecialImage {
 public:
  ...
  virtual sk_sp<SkSpecialSurface> onMakeSurface(
      const SkImageFilter::OutputProperties& outProps, const SkISize& size, SkAlphaType at,
  ...
  virtual sk_sp<SkSurface> onMakeTightSurface(
      const SkImageFilter::OutputProperties& outProps, const SkISize& size,
...
sk_sp<SkSpecialImage> SkSpecialImage::makeTextureImage(GrRecordingContext* context) {
  ...
  return SkSpecialImage::MakeDeferredFromGpu(
      context, rect, this->uniqueID(), std::move(proxy), sk_ref_sp(this->getColorSpace()),
      &this->props(), this->alphaType());
#else
  return nullptr;
#endif
}
...
sk_sp<SkSurface> SkSpecialImage::makeTightSurface(
    const SkImageFilter::OutputProperties& outProps, const SkISize& size, SkAlphaType at) const {
...
sk_sp<SkSpecialImage> SkSpecialImage::MakeFromImage(
  ...
  if (sk_sp<GrTextureProxy> proxy = as_IB(image)->asTextureProxyRef(context)) {
    ...
    return MakeDeferredFromGpu(
        context, subset, image->uniqueID(), std::move(proxy), image->refColorSpace(), props);
  } else
#endif
  {
    SkBitmap bm;
...
class SkSpecialImage_Raster : public SkSpecialImage_Base {
 public:
  ...
  sk_sp<SkSpecialSurface> onMakeSurface(
    ...
    SkImageInfo info =
        SkImageInfo::Make(size.width(), size.height(), colorType, at, sk_ref_sp(colorSpace));
  ...
  sk_sp<SkSpecialImage> onMakeSubset(const SkIRect& subset) const override {
    ...
    return SkSpecialImage::MakeFromRaster(
        SkIRect::MakeWH(subset.width(), subset.height()), subsetBM, &this->props());
  }
...
static sk_sp<SkImage> wrap_proxy_in_image(
    GrRecordingContext* context, sk_sp<GrTextureProxy> proxy, SkAlphaType alphaType,
    sk_sp<SkColorSpace> colorSpace) {
  // CONTEXT TODO: remove this use of 'backdoor' to create an SkImage
  return sk_make_sp<SkImage_Gpu>(
      sk_ref_sp(context->priv().backdoor()), kNeedNewImageUniqueID, alphaType, std::move(proxy),
      std::move(colorSpace));
}
...
class SkSpecialImage_Gpu : public SkSpecialImage_Base {
 public:
  SkSpecialImage_Gpu(
      ...
      sk_sp<GrTextureProxy> proxy, SkAlphaType at, sk_sp<SkColorSpace> colorSpace,
      const SkSurfaceProps* props)
      : INHERITED(subset, uniqueID, props),
        fContext(context),
  ...
  void onDraw(SkCanvas* canvas, SkScalar x, SkScalar y, const SkPaint* paint) const override {
    ...
    sk_sp<SkImage> img = sk_sp<SkImage>(new SkImage_Gpu(
        sk_ref_sp(canvas->getGrContext()), this->uniqueID(), fAlphaType, fTextureProxy,
        fColorSpace));
  ...
  bool onGetROPixels(SkBitmap* dst) const override {
    ...
    sk_sp<GrSurfaceContext> sContext =
        fContext->priv().makeWrappedSurfaceContext(fTextureProxy, fColorSpace);
    if (!sContext) {
      return false;
    }

    if (!sContext->readPixels(
            info, pmap.writable_addr(), pmap.rowBytes(), this->subset().left(),
            this->subset().top())) {
      return false;
    }
  ...
  sk_sp<SkSpecialImage> onMakeSubset(const SkIRect& subset) const override {
    return SkSpecialImage::MakeDeferredFromGpu(
        fContext, subset, this->uniqueID(), fTextureProxy, fColorSpace, &this->props(), fAlphaType);
  }
  ...
  sk_sp<SkImage> onAsImage(const SkIRect* subset) const override {
    if (subset) {
      ...
      if (GrProxyProvider::IsFunctionallyExact(fTextureProxy.get()) && 0 == subset->fLeft &&
          0 == subset->fTop && fTextureProxy->width() == subset->width() &&
      ...
      sk_sp<GrTextureProxy> subsetProxy(GrSurfaceProxy::Copy(
          fContext, fTextureProxy.get(), GrMipMapped::kNo, *subset, SkBackingFit::kExact,
          SkBudgeted::kYes));
      if (!subsetProxy) {
        return nullptr;
      }
...
sk_sp<SkSpecialImage> SkSpecialImage::MakeDeferredFromGpu(
    GrRecordingContext* context, const SkIRect& subset, uint32_t uniqueID,
    sk_sp<GrTextureProxy> proxy, sk_sp<SkColorSpace> colorSpace, const SkSurfaceProps* props,
    SkAlphaType at) {
  ...
  return sk_make_sp<SkSpecialImage_Gpu>(
      context, subset, uniqueID, std::move(proxy), at, std::move(colorSpace), props);
}
#endif
