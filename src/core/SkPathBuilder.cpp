...
SkPathBuilder::SkPathBuilder() noexcept { this->reset(); }

SkPathBuilder::~SkPathBuilder() = default;

SkPathBuilder& SkPathBuilder::reset() noexcept {
  fPts.reset();
  fVerbs.reset();
  fConicWeights.reset();
...
void SkPathBuilder::incReserve(int extraPtCount, int extraVbCount) noexcept {
...
SkPathBuilder& SkPathBuilder::moveTo(SkPoint pt) noexcept {
  fPts.push_back(pt);
...
SkPathBuilder& SkPathBuilder::lineTo(SkPoint pt) noexcept {
  this->ensureMove();
...
SkPathBuilder& SkPathBuilder::quadTo(SkPoint pt1, SkPoint pt2) noexcept {
  this->ensureMove();
...
SkPathBuilder& SkPathBuilder::conicTo(SkPoint pt1, SkPoint pt2, SkScalar w) noexcept {
  this->ensureMove();
...
SkPathBuilder& SkPathBuilder::cubicTo(SkPoint pt1, SkPoint pt2, SkPoint pt3) noexcept {
  this->ensureMove();
...
SkPathBuilder& SkPathBuilder::close() noexcept {
  this->ensureMove();
...
SkPath SkPathBuilder::snapshot() {
  return SkPath(
      sk_sp<SkPathRef>(new SkPathRef(fPts, fVerbs, fConicWeights, fSegmentMask)), fFillType,
      fIsVolatile);
}
...
SkPath SkPathBuilder::detach() {
  auto path = SkPath(
      sk_sp<SkPathRef>(new SkPathRef(
          std::move(fPts), std::move(fVerbs), std::move(fConicWeights), fSegmentMask)),
...
class PointIterator {
 public:
  PointIterator(SkPathDirection dir, unsigned startIndex) noexcept
...
class RectPointIterator : public PointIterator<4> {
 public:
  RectPointIterator(const SkRect& rect, SkPathDirection dir, unsigned startIndex) noexcept
...
class OvalPointIterator : public PointIterator<4> {
 public:
  OvalPointIterator(const SkRect& oval, SkPathDirection dir, unsigned startIndex) noexcept
      : PointIterator(dir, startIndex) {
...
class RRectPointIterator : public PointIterator<8> {
 public:
  RRectPointIterator(const SkRRect& rrect, SkPathDirection dir, unsigned startIndex) noexcept
      : PointIterator(dir, startIndex) {
...
SkPathBuilder& SkPathBuilder::addRect(
    const SkRect& rect, SkPathDirection dir, unsigned index) noexcept {
  constexpr int kPts = 4;    // moveTo + 3 lines
  constexpr int kVerbs = 5;  // moveTo + 3 lines + close
...
SkPathBuilder& SkPathBuilder::addOval(
    const SkRect& oval, SkPathDirection dir, unsigned index) noexcept {
  constexpr int kPts = 9;    // moveTo + 4 conics(2 pts each)
  constexpr int kVerbs = 6;  // moveTo + 4 conics + close
  this->incReserve(kPts, kVerbs);
...
SkPathBuilder& SkPathBuilder::addRRect(
    const SkRRect& rrect, SkPathDirection dir, unsigned index) noexcept {
  const SkRect& bounds = rrect.getBounds();
