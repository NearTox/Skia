...
static bool init_uniform_type(
    const SkSL::Context& ctx, const SkSL::Type* type, SkRuntimeEffect::Uniform* v) {
  using Type = SkRuntimeEffect::Uniform::Type;
  if (type->matches(*ctx.fTypes.fFloat)) {
    v->type = Type::kFloat;
    return true;
  }
  if (type->matches(*ctx.fTypes.fHalf)) {
    v->type = Type::kFloat;
    return true;
  }
  if (type->matches(*ctx.fTypes.fFloat2)) {
    v->type = Type::kFloat2;
    return true;
  }
  if (type->matches(*ctx.fTypes.fHalf2)) {
    v->type = Type::kFloat2;
    return true;
  }
  if (type->matches(*ctx.fTypes.fFloat3)) {
    v->type = Type::kFloat3;
    return true;
  }
  if (type->matches(*ctx.fTypes.fHalf3)) {
    v->type = Type::kFloat3;
    return true;
  }
  if (type->matches(*ctx.fTypes.fFloat4)) {
    v->type = Type::kFloat4;
    return true;
  }
  if (type->matches(*ctx.fTypes.fHalf4)) {
    v->type = Type::kFloat4;
    return true;
  }
  if (type->matches(*ctx.fTypes.fFloat2x2)) {
    v->type = Type::kFloat2x2;
    return true;
  }
  if (type->matches(*ctx.fTypes.fHalf2x2)) {
    v->type = Type::kFloat2x2;
    return true;
  }
  if (type->matches(*ctx.fTypes.fFloat3x3)) {
    v->type = Type::kFloat3x3;
    return true;
  }
  if (type->matches(*ctx.fTypes.fHalf3x3)) {
    v->type = Type::kFloat3x3;
    return true;
  }
  if (type->matches(*ctx.fTypes.fFloat4x4)) {
    v->type = Type::kFloat4x4;
    return true;
  }
  if (type->matches(*ctx.fTypes.fHalf4x4)) {
    v->type = Type::kFloat4x4;
    return true;
  }

  if (type->matches(*ctx.fTypes.fInt)) {
    v->type = Type::kInt;
    return true;
  }
  if (type->matches(*ctx.fTypes.fInt2)) {
    v->type = Type::kInt2;
    return true;
  }
  if (type->matches(*ctx.fTypes.fInt3)) {
    v->type = Type::kInt3;
    return true;
  }
  if (type->matches(*ctx.fTypes.fInt4)) {
    v->type = Type::kInt4;
    return true;
  }
...
SkRuntimeEffect::Uniform SkRuntimeEffectPriv::VarAsUniform(
    const SkSL::Variable& var, const SkSL::Context& context, size_t* offset) {
...
sk_sp<const SkData> SkRuntimeEffectPriv::TransformUniforms(
    SkSpan<const SkRuntimeEffect::Uniform> uniforms, sk_sp<const SkData> originalData,
...
static bool flattenable_is_valid_as_child(const SkFlattenable* f) {
  ...
  switch (f->getFlattenableType()) {
    ...
    case SkFlattenable::kSkBlender_Type: return true;
...
static sk_sp<SkSL::SkVMDebugTrace> make_skvm_debug_trace(
    SkRuntimeEffect* effect, const SkIPoint& coord) {
...
static bool verify_child_effects(
    const std::vector<SkRuntimeEffect::Child>& reflected,
 ...
 */
static bool read_child_effects(
    SkReadBuffer& buffer, const SkRuntimeEffect* effect,
...
static void write_child_effects(
...
static std::vector<skvm::Val> make_skvm_uniforms(
    skvm::Builder* p, skvm::Uniforms* uniforms, size_t inputSize, const SkData& inputs) {
...
SkRuntimeEffect::Result SkRuntimeEffect::MakeFromSource(
    SkString sksl, const Options& options, SkSL::ProgramKind kind) {
...
SkRuntimeEffect::Result SkRuntimeEffect::MakeInternal(
    std::unique_ptr<SkSL::Program> program, const Options& options, SkSL::ProgramKind kind) {
  SkSL::SharedCompiler compiler;
  ...
  for (const SkSL::ProgramElement* elem : program->elements()) {
    ...
    if (elem->is<SkSL::GlobalVarDeclaration>()) {
      ...
      if (varType.isEffectChild()) {
        ...
        sampleUsages.push_back(usage.isSampled() ? usage : SkSL::SampleUsage::PassThrough());
      }
      // 'uniform' variables
...
#  undef RETURN_FAILURE
  ...
  sk_sp<SkRuntimeEffect> effect(new SkRuntimeEffect(
      std::move(program), options, *main, std::move(uniforms), std::move(children),
      std::move(sampleUsages), flags));
...
sk_sp<SkRuntimeEffect> SkMakeCachedRuntimeEffect(
    SkRuntimeEffect::Result (*make)(SkString sksl), SkString sksl) {
  ...
  struct Key {
    ...
    bool operator==(const Key& that) const {
      return this->skslHashA == that.skslHashA && this->skslHashB == that.skslHashB;
    }
...
SkRuntimeEffect::SkRuntimeEffect(
    std::unique_ptr<SkSL::Program> baseProgram, const Options& options,
    const SkSL::FunctionDefinition& main, std::vector<Uniform>&& uniforms,
    std::vector<Child>&& children, std::vector<SkSL::SampleUsage>&& sampleUsages, uint32_t flags)
  ...
  fHash = SkOpts::hash_fn(&options.forceUnoptimized, sizeof(options.forceUnoptimized), fHash);
  fHash = SkOpts::hash_fn(
      &options.enforceES2Restrictions, sizeof(options.enforceES2Restrictions), fHash);
  fHash = SkOpts::hash_fn(
      &options.usePrivateRTShaderModule, sizeof(options.usePrivateRTShaderModule), fHash);
...
const std::string& SkRuntimeEffect::source() const { return *fBaseProgram->fSource; }

size_t SkRuntimeEffect::uniformSize() const {
  return fUniforms.empty() ? 0 : SkAlign4(fUniforms.back().offset + fUniforms.back().sizeInBytes());
}
...
std::unique_ptr<SkFilterColorProgram> SkFilterColorProgram::Make(const SkRuntimeEffect* effect) {
  ...
  if (!std::all_of(
          effect->fChildren.begin(), effect->fChildren.end(),
          [](const SkRuntimeEffect::Child& c) { return c.type == ChildType::kColorFilter; })) {
    return nullptr;
  }
  ...
  class Callbacks : public SkSL::SkVMCallbacks {
   public:
    Callbacks(
        skvm::Builder* builder, const skvm::Uniforms* skslUniforms,
        skvm::Uniforms* childColorUniforms, skvm::Color inputColor,
        std::vector<SkFilterColorProgram::SampleCall>* sampleCalls)
        : fBuilder(builder),
    ...
    bool isSimpleUniform(skvm::Color c, int* baseOffset) {
      ...
      *baseOffset = ur.offset;
      return ug.offset == ur.offset + 4 && ub.offset == ur.offset + 8 &&
    ...
    skvm::Color sampleColorFilter(int ix, skvm::Color c) override {
      ...
      } else if (fBuilder->allImm(
                     c.r.id, &call.fImm.fR, c.g.id, &call.fImm.fG, c.b.id, &call.fImm.fB, c.a.id,
                     &call.fImm.fA)) {
        call.fKind = SkFilterColorProgram::SampleCall::Kind::kImmediate;
      } else if (auto it = std::find_if(
                     fChildColors.begin(), fChildColors.end(),
  ...
  skvm::Color result = SkSL::ProgramToSkVM(
      *effect->fBaseProgram, effect->fMain, &p,
      /*debugTrace=*/nullptr, SkMakeSpan(uniform),
      /*device=*/zeroCoord,
      /*local=*/zeroCoord, inputColor, inputColor, &callbacks);

  // Then store the result to the *third* arg ptr
  p.store(
      {skvm::PixelFormat::FLOAT, 32, 32, 32, 32, 0, 32, 64, 96}, p.varying<skvm::F32>(), result);
  ...
  return std::unique_ptr<SkFilterColorProgram>(new SkFilterColorProgram(
      p.done(/*debug_name=*/nullptr, /*allow_jit=*/false), std::move(sampleCalls), alphaUnchanged));
}

SkFilterColorProgram::SkFilterColorProgram(
    skvm::Program program, std::vector<SampleCall> sampleCalls, bool alphaUnchanged)
    : fProgram(std::move(program)),
      fSampleCalls(std::move(sampleCalls)),
      fAlphaUnchanged(alphaUnchanged) {}

SkPMColor4f SkFilterColorProgram::eval(
    const SkPMColor4f& inColor, const void* uniformData,
...
static GrFPResult make_effect_fp(
    sk_sp<SkRuntimeEffect> effect, const char* name, sk_sp<const SkData> uniforms,
    std::unique_ptr<GrFragmentProcessor> inputFP, std::unique_ptr<GrFragmentProcessor> destColorFP,
    SkSpan<const SkRuntimeEffect::ChildPtr> children, const GrFPArgs& childArgs) {
    ...
    if (type == ChildType::kShader) {
      ...
      auto [success, childFP] =
          as_CFB(child.colorFilter())
              ->asFragmentProcessor(
                  /*inputFP=*/nullptr, childArgs.fContext, *childArgs.fDstColorInfo);
      if (!success) {
        return GrFPFailure(std::move(inputFP));
      }
      childFPs.push_back(std::move(childFP));
      ...
      auto childFP =
          as_BB(child.blender())
              ->asFragmentProcessor(
                  /*srcFP=*/nullptr, GrFragmentProcessor::UseDestColorAsInput(/*dstFP=*/nullptr),
                  childArgs);
      if (!childFP) {
        return GrFPFailure(std::move(inputFP));
      }
  ...
  auto fp = GrSkSLFP::MakeWithData(
      std::move(effect), name, childArgs.fDstColorInfo->refColorSpace(), std::move(inputFP),
      std::move(destColorFP), std::move(uniforms), SkMakeSpan(childFPs));
  SkASSERT(fp);
...
class RuntimeEffectVMCallbacks : public SkSL::SkVMCallbacks {
 public:
  RuntimeEffectVMCallbacks(
      skvm::Builder* builder, skvm::Uniforms* uniforms, SkArenaAlloc* alloc,
      const std::vector<SkRuntimeEffect::ChildPtr>& children, skvm::Color inColor,
  ...
  skvm::Color sampleShader(int ix, skvm::Coord coord) override {
    if (SkShader* shader = fChildren[ix].shader()) {
      ...
      return as_SB(shader)->program(
          fBuilder, coord, coord, fInColor, matrixProvider,
  ...
  skvm::Color toLinearSrgb(skvm::Color color) override {
    ...
    return SkColorSpaceXformSteps{
        fColorInfo.colorSpace(), kUnpremul_SkAlphaType, sk_srgb_linear_singleton(),
  ...
  skvm::Color fromLinearSrgb(skvm::Color color) override {
    ...
    return SkColorSpaceXformSteps{
        sk_srgb_linear_singleton(), kUnpremul_SkAlphaType, fColorInfo.colorSpace(),
        kUnpremul_SkAlphaType}
        .program(fBuilder, fUniforms, color);
  }
...
class SkRuntimeColorFilter : public SkColorFilterBase {
 public:
  SkRuntimeColorFilter(
      sk_sp<SkRuntimeEffect> effect, sk_sp<const SkData> uniforms,
...
#  if SK_SUPPORT_GPU
  GrFPResult asFragmentProcessor(
    ...
    return make_effect_fp(
        fEffect, "runtime_color_filter", std::move(uniforms), std::move(inputFP),
        /*destColorFP=*/nullptr, SkMakeSpan(fChildren), childArgs);
  }
#  endif

  bool onAppendStages(const SkStageRec& rec, bool shaderIsOpaque) const override { return false; }

  skvm::Color onProgram(
      skvm::Builder* p, skvm::Color c, const SkColorInfo& colorInfo, skvm::Uniforms* uniforms,
    ...
    std::vector<skvm::Val> uniform =
        make_skvm_uniforms(p, uniforms, fEffect->uniformSize(), *inputs);
    ...
    skvm::Coord zeroCoord = {p->splat(0.0f), p->splat(0.0f)};
    return SkSL::ProgramToSkVM(
        *fEffect->fBaseProgram, fEffect->fMain, p, /*debugTrace=*/nullptr, SkMakeSpan(uniform),
        /*device=*/zeroCoord, /*local=*/zeroCoord, c, c, &callbacks);
  }
  ...
  bool onIsAlphaUnchanged() const override {
    return fEffect->getFilterColorProgram() && fEffect->getFilterColorProgram()->isAlphaUnchanged();
  }
  ...
  SkRuntimeEffect* asRuntimeEffect() const noexcept override { return fEffect.get(); }
...
class SkRTShader : public SkShaderBase {
 public:
  SkRTShader(
      sk_sp<SkRuntimeEffect> effect, sk_sp<SkSL::SkVMDebugTrace> debugTrace,
      sk_sp<const SkData> uniforms, const SkMatrix* localMatrix,
      SkSpan<SkRuntimeEffect::ChildPtr> children)
  ...
  SkRuntimeEffect::TracedShader makeTracedClone(const SkIPoint& coord) {
    ...
    auto debugShader = sk_make_sp<SkRTShader>(
        unoptimized, debugTrace, fUniforms, &this->getLocalMatrix(), SkMakeSpan(fChildren));
...
#  if SK_SUPPORT_GPU
  std::unique_ptr<GrFragmentProcessor> asFragmentProcessor(const GrFPArgs& args) const override {
    ...
    sk_sp<const SkData> uniforms = SkRuntimeEffectPriv::TransformUniforms(
        fEffect->uniforms(), fUniforms, args.fDstColorInfo->colorSpace());
    SkASSERT(uniforms);

    auto [success, fp] = make_effect_fp(
        fEffect, "runtime_shader", std::move(uniforms),
        /*inputFP=*/nullptr,
        /*destColorFP=*/nullptr, SkMakeSpan(fChildren), args);
    if (!success) {
      return nullptr;
    }
...
#  endif
  ...
  bool onAppendStages(const SkStageRec& rec) const override { return false; }

  skvm::Color onProgram(
      skvm::Builder* p, skvm::Coord device, skvm::Coord local, skvm::Color paint,
      const SkMatrixProvider& matrices, const SkMatrix* localM, const SkColorInfo& colorInfo,
    ...
    return SkSL::ProgramToSkVM(
        *fEffect->fBaseProgram, fEffect->fMain, p, fDebugTrace.get(), SkMakeSpan(uniform), device,
        local, paint, paint, &callbacks);
  }
...
class SkRuntimeBlender : public SkBlenderBase {
 public:
  ...
  skvm::Color onProgram(
      skvm::Builder* p, skvm::Color src, skvm::Color dst, const SkColorInfo& colorInfo,
      skvm::Uniforms* uniforms, SkArenaAlloc* alloc) const override {
    sk_sp<const SkData> inputs = SkRuntimeEffectPriv::TransformUniforms(
        fEffect->uniforms(), fUniforms, colorInfo.colorSpace());
    SkASSERT(inputs);
    ...
    return SkSL::ProgramToSkVM(
        *fEffect->fBaseProgram, fEffect->fMain, p, /*debugTrace=*/nullptr, SkMakeSpan(uniform),
        /*device=*/zeroCoord, /*local=*/zeroCoord, src, dst, &callbacks);
  }

#  if SK_SUPPORT_GPU
  std::unique_ptr<GrFragmentProcessor> asFragmentProcessor(
      std::unique_ptr<GrFragmentProcessor> srcFP, std::unique_ptr<GrFragmentProcessor> dstFP,
      const GrFPArgs& args) const override {
    sk_sp<const SkData> uniforms = SkRuntimeEffectPriv::TransformUniforms(
        fEffect->uniforms(), fUniforms, args.fDstColorInfo->colorSpace());
    SkASSERT(uniforms);
    auto [success, fp] = make_effect_fp(
        fEffect, "runtime_blender", std::move(uniforms), std::move(srcFP), std::move(dstFP),
        SkMakeSpan(fChildren), args);
...
sk_sp<SkShader> SkRuntimeEffect::makeShader(
    sk_sp<const SkData> uniforms, sk_sp<SkShader> childShaders[], size_t childCount,
...
sk_sp<SkShader> SkRuntimeEffect::makeShader(
    sk_sp<const SkData> uniforms, SkSpan<ChildPtr> children, const SkMatrix* localMatrix) const {
  if (!this->allowShader()) {
    return nullptr;
  }
  ...
  return sk_make_sp<SkRTShader>(
      sk_ref_sp(this), /*debugTrace=*/nullptr, std::move(uniforms), localMatrix, children);
}

sk_sp<SkImage> SkRuntimeEffect::makeImage(
    GrRecordingContext* rContext, sk_sp<const SkData> uniforms, SkSpan<ChildPtr> children,
    const SkMatrix* localMatrix, SkImageInfo resultInfo, bool mipmapped) const {
  if (rContext) {
#  if SK_SUPPORT_GPU
    if (!rContext->priv().caps()->mipmapSupport()) {
      mipmapped = false;
    }
    auto fillContext = rContext->priv().makeSFC(
        resultInfo, SkBackingFit::kExact,
        /*sample count*/ 1, GrMipmapped(mipmapped));
    if (!fillContext) {
      return nullptr;
    }
    uniforms = SkRuntimeEffectPriv::TransformUniforms(
        this->uniforms(), std::move(uniforms), resultInfo.colorSpace());
    SkASSERT(uniforms);
    ...
    auto fp = GrSkSLFP::MakeWithData(
        sk_ref_sp(this), "runtime_image", colorInfo.refColorSpace(),
        /*inputFP=*/nullptr,
        /*destColorFP=*/nullptr, std::move(uniforms), SkMakeSpan(childFPs));
    ...
    return sk_sp<SkImage>(new SkImage_Gpu(
        sk_ref_sp(rContext), kNeedNewImageUniqueID, fillContext->readSurfaceView(),
...
sk_sp<SkColorFilter> SkRuntimeEffect::makeColorFilter(
    sk_sp<const SkData> uniforms, sk_sp<SkColorFilter> childColorFilters[],
...
sk_sp<SkColorFilter> SkRuntimeEffect::makeColorFilter(
    sk_sp<const SkData> uniforms, SkSpan<ChildPtr> children) const {
...
sk_sp<SkBlender> SkRuntimeEffect::makeBlender(
    sk_sp<const SkData> uniforms, SkSpan<ChildPtr> children) const {
...
SkRuntimeEffect::TracedShader SkRuntimeEffect::MakeTraced(
    sk_sp<SkShader> shader, const SkIPoint& traceCoord) {
...
std::optional<ChildType> SkRuntimeEffect::ChildPtr::type() const {
  if (fChild) {
    switch (fChild->getFlattenableType()) {
      case SkFlattenable::kSkShader_Type: return ChildType::kShader;
      case SkFlattenable::kSkColorFilter_Type: return ChildType::kColorFilter;
      case SkFlattenable::kSkBlender_Type: return ChildType::kBlender;
      default: break;
    }
  }
  return std::nullopt;
...
sk_sp<SkImage> SkRuntimeShaderBuilder::makeImage(
    GrRecordingContext* recordingContext, const SkMatrix* localMatrix, SkImageInfo resultInfo,
    bool mipmapped) {
  return this->effect()->makeImage(
      recordingContext, this->uniforms(), SkMakeSpan(this->children(), this->numChildren()),
      localMatrix, resultInfo, mipmapped);
}
...
sk_sp<SkBlender> SkRuntimeBlendBuilder::makeBlender() {
  return this->effect()->makeBlender(
      this->uniforms(), SkMakeSpan(this->children(), this->numChildren()));
}
