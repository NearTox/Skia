...
static bool init_uniform_type(
  ...
  if (*type == *ctx.fTypes.fFloat) {
    v->type = Type::kFloat;
    return true;
  }
  if (*type == *ctx.fTypes.fHalf) {
    v->type = Type::kFloat;
    return true;
  }
  if (*type == *ctx.fTypes.fFloat2) {
    v->type = Type::kFloat2;
    return true;
  }
  if (*type == *ctx.fTypes.fHalf2) {
    v->type = Type::kFloat2;
    return true;
  }
  if (*type == *ctx.fTypes.fFloat3) {
    v->type = Type::kFloat3;
    return true;
  }
  if (*type == *ctx.fTypes.fHalf3) {
    v->type = Type::kFloat3;
    return true;
  }
  if (*type == *ctx.fTypes.fFloat4) {
    v->type = Type::kFloat4;
    return true;
  }
  if (*type == *ctx.fTypes.fHalf4) {
    v->type = Type::kFloat4;
    return true;
  }
  if (*type == *ctx.fTypes.fFloat2x2) {
    v->type = Type::kFloat2x2;
    return true;
  }
  if (*type == *ctx.fTypes.fHalf2x2) {
    v->type = Type::kFloat2x2;
    return true;
  }
  if (*type == *ctx.fTypes.fFloat3x3) {
    v->type = Type::kFloat3x3;
    return true;
  }
  if (*type == *ctx.fTypes.fHalf3x3) {
    v->type = Type::kFloat3x3;
    return true;
  }
  if (*type == *ctx.fTypes.fFloat4x4) {
    v->type = Type::kFloat4x4;
    return true;
  }
  if (*type == *ctx.fTypes.fHalf4x4) {
    v->type = Type::kFloat4x4;
    return true;
  }

  if (*type == *ctx.fTypes.fInt) {
    v->type = Type::kInt;
    return true;
  }
  if (*type == *ctx.fTypes.fInt2) {
    v->type = Type::kInt2;
    return true;
  }
  if (*type == *ctx.fTypes.fInt3) {
    v->type = Type::kInt3;
    return true;
  }
  if (*type == *ctx.fTypes.fInt4) {
    v->type = Type::kInt4;
    return true;
  }
...
SkRuntimeEffect::Result SkRuntimeEffect::MakeFromDSL(
...
SkRuntimeEffect::Result SkRuntimeEffect::MakeForColorFilter(
    std::unique_ptr<SkSL::Program> program, const Options& options) {
...
SkRuntimeEffect::Result SkRuntimeEffect::MakeForBlender(
    std::unique_ptr<SkSL::Program> program, const Options& options) {
...
SkRuntimeEffect::SkRuntimeEffect(
  ...
  struct KnownOptions {
    bool forceNoInline, enforceES2Restrictions, allowFragCoord;
  };
  ...
  fHash = SkOpts::hash_fn(&options.allowFragCoord, sizeof(options.allowFragCoord), fHash);
...
std::unique_ptr<SkFilterColorProgram> SkFilterColorProgram::Make(const SkRuntimeEffect* effect) {
  ...
  auto is_simple_uniform = [&](skvm::Color c, int* baseOffset) {
    ...
    *baseOffset = ur.offset;
    return ug.offset == ur.offset + 4 && ub.offset == ur.offset + 8 && ua.offset == ur.offset + 12;
  };
  ...
  auto sampleColorFilter = [&](int ix, skvm::Color c) {
    ...
    if (ids_equal(c, inputColor)) {
      ...
      call.fKind = SkFilterColorProgram::SampleCall::Kind::kImmediate;
                   ...
                   childColors.begin(), childColors.end(),
  ...
  skvm::Color result = SkSL::ProgramToSkVM(
      *effect->fBaseProgram, effect->fMain, &p, SkMakeSpan(uniform),
      /*device=*/zeroCoord,
      /*local=*/zeroCoord, inputColor, inputColor,
      /*sampleShader=*/nullptr, sampleColorFilter,
...
static sk_sp<SkData> get_xformed_uniforms(
    const SkRuntimeEffect* effect, sk_sp<SkData> baseUniforms, const SkColorSpace* dstCS) {
...
static GrFPResult make_effect_fp(
  ...
  for (const auto& child : children) {
    ...
    if (type == ChildType::kShader) {
      ...
      auto childFP = as_BB(child.blender())
                         ->asFragmentProcessor(
                             /*srcFP=*/nullptr,
                             /*dstFP=*/nullptr, childArgs);
      if (!childFP) {
        return GrFPFailure(std::move(inputFP));
      }
      childFPs.push_back(std::move(childFP));
  ...
  auto fp = GrSkSLFP::MakeWithData(
      std::move(effect), name, std::move(inputFP), std::move(destColorFP), std::move(uniforms),
      SkMakeSpan(childFPs));
  SkASSERT(fp);
...
class SkRuntimeColorFilter : public SkColorFilterBase {
 public:
  SkRuntimeColorFilter(
      sk_sp<SkRuntimeEffect> effect, sk_sp<SkData> uniforms,
...
#  if SK_SUPPORT_GPU
  GrFPResult asFragmentProcessor(
    ...
    sk_sp<SkData> uniforms = get_xformed_uniforms(fEffect.get(), fUniforms, colorInfo.colorSpace());
    SkASSERT(uniforms);
...
#  endif
  ...
  skvm::Color onProgram(
    ...
    sk_sp<SkData> inputs = get_xformed_uniforms(fEffect.get(), fUniforms, colorInfo.colorSpace());
    SkASSERT(inputs);
    ...
    auto sampleShader = [&](int ix, skvm::Coord coord) {
      if (SkShader* shader = fChildren[ix].shader()) {
        ...
        return as_SB(shader)->program(
            p, coord, coord, c, mats, /*localM=*/nullptr, colorInfo, uniforms, alloc);
      }
      return c;
    };
    ...
    return SkSL::ProgramToSkVM(
        *fEffect->fBaseProgram, fEffect->fMain, p, SkMakeSpan(uniform),
        /*device=*/zeroCoord, /*local=*/zeroCoord, c, c, sampleShader, sampleColorFilter,
        sampleBlender);
  }
...
class SkRTShader : public SkShaderBase {
 public:
  SkRTShader(
      sk_sp<SkRuntimeEffect> effect, sk_sp<SkData> uniforms, const SkMatrix* localMatrix,
      SkSpan<SkRuntimeEffect::ChildPtr> children, bool isOpaque)
  ...
  skvm::Color onProgram(
    ...
    auto sampleShader = [&](int ix, skvm::Coord coord) {
      if (SkShader* shader = fChildren[ix].shader()) {
        ...
        return as_SB(shader)->program(
            p, device, coord, paint, mats, /*localM=*/nullptr, colorInfo, uniforms, alloc);
      }
      return paint;
    };
    auto sampleColorFilter = [&](int ix, skvm::Color color) {
    ...
    return SkSL::ProgramToSkVM(
        *fEffect->fBaseProgram, fEffect->fMain, p, SkMakeSpan(uniform), device, local, paint, paint,
  ...
  skvm::Color onProgram(
    ...
    auto sampleShader = [&](int ix, skvm::Coord coord) {
      if (SkShader* shader = fChildren[ix].shader()) {
        SkSimpleMatrixProvider mats{SkMatrix::I()};
        return as_SB(shader)->program(
            p, coord, coord, src, mats, /*localM=*/nullptr, colorInfo, uniforms, alloc);
      }
      return src;
    };
    auto sampleColorFilter = [&](int ix, skvm::Color color) {
    ...
    return SkSL::ProgramToSkVM(
        *fEffect->fBaseProgram, fEffect->fMain, p, SkMakeSpan(uniform),
        /*device=*/zeroCoord, /*local=*/zeroCoord, src, dst, sampleShader, sampleColorFilter,
        sampleBlender);
  }
...
sk_sp<SkShader> SkRuntimeEffect::makeShader(
    sk_sp<SkData> uniforms, sk_sp<SkShader> childShaders[], size_t childCount,
...
sk_sp<SkShader> SkRuntimeEffect::makeShader(
    sk_sp<SkData> uniforms, SkSpan<ChildPtr> children, const SkMatrix* localMatrix,
    bool isOpaque) const {
  if (!this->allowShader()) {
    return nullptr;
  }
  ...
  return sk_sp<SkShader>(
      new SkRTShader(sk_ref_sp(this), std::move(uniforms), localMatrix, children, isOpaque));
}

sk_sp<SkImage> SkRuntimeEffect::makeImage(
    GrRecordingContext* rContext, sk_sp<SkData> uniforms, SkSpan<ChildPtr> children,
...
sk_sp<SkColorFilter> SkRuntimeEffect::makeColorFilter(
    sk_sp<SkData> uniforms, sk_sp<SkColorFilter> childColorFilters[], size_t childCount) const {
  SkSTArray<4, ChildPtr> children(childCount);
  for (size_t i = 0; i < childCount; ++i) {
...
sk_sp<SkColorFilter> SkRuntimeEffect::makeColorFilter(
  ...
  return sk_sp<SkColorFilter>(
      new SkRuntimeColorFilter(sk_ref_sp(this), std::move(uniforms), children));
}
...
sk_sp<SkShader> SkRuntimeShaderBuilder::makeShader(const SkMatrix* localMatrix, bool isOpaque) {
  return this->effect()->makeShader(
      this->uniforms(), SkMakeSpan(this->children(), this->numChildren()), localMatrix, isOpaque);
}
