...
class SharedCompiler {
 public:
  ...
  SkSL::Compiler* operator->() const noexcept { return gCompiler; }
...
SkRuntimeEffect::EffectResult SkRuntimeEffect::Make(SkString sksl) {
  ...
  auto program = compiler->convertProgram(
      SkSL::Program::kPipelineStage_Kind, SkSL::String(sksl.c_str(), sksl.size()),
    ...
    for (const auto& e : *program) {
      if (e.fKind == SkSL::ProgramElement::kVar_Kind) {
        ...
        for (const auto& varStatement : v.fVars) {
          ...
          SkASSERT(
              (var.fModifiers.fFlags & SkSL::Modifiers::kIn_Flag) == 0 ||
...
#undef SET_TYPES
            ...
            switch (v.fType) {
              ...
              case Variable::Type::kInt:
                if (v.fQualifier == Variable::Qualifier::kUniform) {
                  RETURN_FAILURE(
                      "'uniform' variables may not have '%s' type", type->displayName().c_str());
                }
                break;
              ...
              case Variable::Type::kFloat4x4:
                if (v.fQualifier == Variable::Qualifier::kIn) {
                  RETURN_FAILURE(
                      "'in' variables may not have '%s' type", type->displayName().c_str());
                }
                break;
            }
            ...
            if (marker.fLength) {
              ...
              if (!parse_marker(marker, &v.fMarker, &v.fFlags)) {
                RETURN_FAILURE(
                    "Invalid 'marker' string: '%.*s'", (int)marker.fLength, marker.fChars);
              }
            }
...
size_t SkRuntimeEffect::inputSize() const {
  return fInAndUniformVars.empty()
             ? 0
             : SkAlign4(fInAndUniformVars.back().fOffset + fInAndUniformVars.back().sizeInBytes());
}

const SkRuntimeEffect::Variable* SkRuntimeEffect::findInput(const char* name) const {
  auto iter = std::find_if(
      fInAndUniformVars.begin(), fInAndUniformVars.end(),
...
int SkRuntimeEffect::findChild(const char* name) const {
  auto iter = std::find_if(
      fChildren.begin(), fChildren.end(), [name](const SkString& s) { return s.equals(name); });
  return iter == fChildren.end() ? -1 : static_cast<int>(iter - fChildren.begin());
}

SkRuntimeEffect::SpecializeResult SkRuntimeEffect::specialize(
    SkSL::Program& baseProgram, const void* inputs, const SkSL::SharedCompiler& compiler) const {
...
#if SK_SUPPORT_GPU
bool SkRuntimeEffect::toPipelineStage(
  ...
  auto baseProgram = compiler->convertProgram(
      SkSL::Program::kPipelineStage_Kind, SkSL::String(fSkSL.c_str(), fSkSL.size()), settings);
  if (!baseProgram) {
...
static std::vector<skvm::F32> program_fn(
    skvm::Builder* p, const SkSL::ByteCodeFunction& fn, const std::vector<skvm::F32>& uniform,
    const SkMatrixProvider& matrices, std::vector<skvm::F32> stack,
    /*these parameters are used to call program() on children*/
    const std::vector<sk_sp<SkShader>>& children, skvm::Coord device, skvm::Color paint,
    SkFilterQuality quality, const SkColorInfo& dst, skvm::Uniforms* uniforms,
    SkArenaAlloc* alloc) {
  auto push = [&](skvm::F32 x) { stack.push_back(x); };
  auto pop = [&]() noexcept {
    skvm::F32 x = stack.back();
    stack.pop_back();
    return x;
  };
  ...
  for (const uint8_t *ip = fn.code(), *end = ip + fn.size(); ip != end;) {
    ...
    auto u8 = [&] {
      auto x = sk_unaligned_load<uint8_t>(ip);
      ip += sizeof(x);
      return x;
    };
    // auto u16 = [&]{ auto x = sk_unaligned_load<uint16_t>(ip); ip += sizeof(x); return x; };
    auto u32 = [&] {
      auto x = sk_unaligned_load<uint32_t>(ip);
      ip += sizeof(x);
      return x;
    };
    ...
    switch (inst) {
      ...
      case Inst::kSampleExplicit: {
        ...
        skvm::F32 y = pop(), x = pop();

        SkOverrideDeviceMatrixProvider mats{matrices, SkMatrix::I()};
        skvm::Color c =
            as_SB(children[ix])
                ->program(p, device, {x, y}, paint, mats, nullptr, quality, dst, uniforms, alloc);
        if (!c) {
          return {};
        }
        push(c.r);
      ...
      case Inst::kMatrixMultiply: {
        ...
        int aCols = u8(), aRows = u8(), bCols = u8(), bRows = aCols;
        std::vector<skvm::F32> A(aCols * aRows), B(bCols * bRows);
      ...
      case Inst::kCompareFLT: {
        skvm::F32 x = pop(), a = pop();
        push(bit_cast(a < x));
      } break;

      case Inst::kMaskBlend: {
        std::vector<skvm::F32> if_true, if_false;
        int count = u8();
...
class SkRuntimeColorFilter : public SkColorFilterBase {
 public:
  SkRuntimeColorFilter(sk_sp<SkRuntimeEffect> effect, sk_sp<SkData> inputs)
      : fEffect(std::move(effect)), fInputs(std::move(inputs)) {}
  ...
  skvm::Color onProgram(
    ...
    std::vector<skvm::F32> stack = program_fn(
        p, *fn, uniform, SkSimpleMatrixProvider{SkMatrix::I()}, {c.r, c.g, c.b, c.a},
...
class SkRTShader : public SkShaderBase {
 public:
  SkRTShader(
      sk_sp<SkRuntimeEffect> effect, sk_sp<SkData> inputs, const SkMatrix* localMatrix,
  ...
  sk_sp<SkData> getUniforms(
      const SkMatrixProvider& matrixProvider, const SkColorSpace* dstCS) const {
  ...
  bool onAppendStages(const SkStageRec& rec) const override {
    SkMatrix inverse;
    if (!this->computeTotalInverse(rec.fMatrixProvider.localToDevice(), rec.fLocalM, &inverse)) {
      return false;
    }
  ...
  skvm::Color onProgram(
    ...
    std::vector<skvm::F32> stack = program_fn(
        p, *fn, uniform, matrices, {local.x, local.y, paint.r, paint.g, paint.b, paint.a},
...
sk_sp<SkFlattenable> SkRTShader::CreateProc(SkReadBuffer& buffer) {
  ...
  return effect->makeShader(
      std::move(inputs), children.data(), children.size(), localMPtr, isOpaque);
}
...
sk_sp<SkShader> SkRuntimeEffect::makeShader(
  ...
  return inputs->size() == this->inputSize() && childCount == fChildren.size()
             ? sk_sp<SkShader>(new SkRTShader(
                   sk_ref_sp(this), std::move(inputs), localMatrix, children, childCount, isOpaque))
             : nullptr;
}
...
SkRuntimeShaderBuilder::BuilderChild& SkRuntimeShaderBuilder::BuilderChild::operator=(
    const sk_sp<SkShader>& val) {
  if (fIndex < 0) {
