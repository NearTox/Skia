...
SkRuntimeEffect::EffectResult SkRuntimeEffect::Make(SkString sksl) {
  ...
  auto program = compiler->convertProgram(
      SkSL::Program::kPipelineStage_Kind, SkSL::String(sksl.c_str(), sksl.size()), settings);
  ...
  for (const auto& elem : *program) {
    ...
    if (elem.fKind == SkSL::ProgramElement::kVar_Kind) {
      ...
      for (const auto& varDecl : varDecls.fVars) {
        const SkSL::Variable& var = *(static_cast<const SkSL::VarDeclaration&>(*varDecl).fVar);
...
#undef RETURN_FAILURE
  ...
  sk_sp<SkRuntimeEffect> effect(new SkRuntimeEffect(
      std::move(sksl), std::move(program), std::move(uniforms), std::move(children),
      std::move(sampleUsages), std::move(varyings), usesSampleCoords, allowColorFilter));
...
bool SkRuntimeEffect::toPipelineStage(
    const GrShaderCaps* shaderCaps, GrContextOptions::ShaderErrorHandler* errorHandler,
...
static skvm::Color program_fn(
    skvm::Builder* p, const SkSL::ByteCodeFunction& fn, const std::vector<skvm::F32>& uniform,
    skvm::Color inColor, SampleChildFn sampleChild, skvm::Coord device, skvm::Coord local) {
  ...
  for (const uint8_t *ip = fn.code(), *end = ip + fn.size(); ip != end;) {
    ...
    switch (inst) {
      ...
      case Inst::kMinF: binary([](skvm::F32 x, skvm::F32 y) { return skvm::min(x, y); }); break;

      case Inst::kMaxF: binary([](skvm::F32 x, skvm::F32 y) { return skvm::max(x, y); }); break;
  ...
  skvm::F32 a = pop(), b = pop(), g = pop(), r = pop();
  return {r, g, b, a};
}

static sk_sp<SkData> get_xformed_uniforms(
    const SkRuntimeEffect* effect, sk_sp<SkData> baseUniforms,
    const SkMatrixProvider* matrixProvider, const SkColorSpace* dstCS) {
...
class SkRuntimeColorFilter : public SkColorFilterBase {
 public:
  SkRuntimeColorFilter(
      sk_sp<SkRuntimeEffect> effect, sk_sp<SkData> uniforms, sk_sp<SkColorFilter> children[],
      size_t childCount)
  ...
  skvm::Color onProgram(
    ...
    auto sampleChild = [&](int ix, skvm::Coord coord) {
      if (fChildren[ix]) {
        ...
        return as_SB(fChildren[ix])
            ->program(p, device, coord, paint, mats, nullptr, quality, dst, uniforms, alloc);
      } else {
        return paint;
      }
    };
...
sk_sp<SkFlattenable> SkRTShader::CreateProc(SkReadBuffer& buffer) {
  ...
  return effect->makeShader(
      std::move(uniforms), children.data(), children.size(), localMPtr, isOpaque);
}

///////////////////////////////////////////////////////////////////////////////////////////////////

sk_sp<SkShader> SkRuntimeEffect::makeShader(
    sk_sp<SkData> uniforms, sk_sp<SkShader> children[], size_t childCount,
  ...
  return uniforms->size() == this->uniformSize() && childCount == fChildren.size()
             ? sk_sp<SkShader>(new SkRTShader(
                   sk_ref_sp(this), std::move(uniforms), localMatrix, children, childCount,
                   isOpaque))
             : nullptr;
}
