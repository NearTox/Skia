 ...
 public:
  ...
  void reject(size_t index, int rejectedMaxDimension) {
    ...
    fMaxDimensionHintForRejects = {
        std::min(prevMin, rejectedMaxDimension), std::max(prevMax, rejectedMaxDimension)};
    this->reject(index);
  }
  ...
  std::tuple<int, int> maxDimensionHint() const noexcept { return fMaxDimensionHintForSource; }
...
class SkGlyphVariant {
 public:
  ...
  operator const SkGlyph*() const noexcept { return this->glyph(); }
...
class SkDrawableGlyphBuffer {
 public:
  ...
  void startDevicePositioning(
      const SkZip<const SkGlyphID, const SkPoint>& source, const SkMatrix& positionMatrix,
  ...
  void accept(SkGlyph* glyph, size_t from) noexcept {
...
#ifdef SK_DEBUG
  enum { kReset, kInput, kProcess, kDraw } fPhase{kReset};
#endif
};
