...
void SkPictureRecord::didScale(SkScalar x, SkScalar y) { this->didConcat44(SkM44::Scale(x, y)); }
...
static bool clipOpExpands(SkClipOp op) {
  switch (op) {
    ...
    case kReplace_SkClipOp: return true;
    case kIntersect_SkClipOp:
    case kDifference_SkClipOp: return false;
    default: SkDEBUGFAIL("unknown clipop"); return false;
  }
}
...
void SkPictureRecord::onDrawPoints(
...
void SkPictureRecord::onDrawDRRect(
...
void SkPictureRecord::onDrawImage(
    const SkImage* image, SkScalar x, SkScalar y, const SkPaint* paint) {
...
void SkPictureRecord::onDrawImageRect(
...
void SkPictureRecord::onDrawImageLattice(
...
void SkPictureRecord::onDrawAtlas(
...
void SkPictureRecord::onDrawEdgeAAImageSet(
...
void SkPictureRecord::onDrawImage2(
...
void SkPictureRecord::onDrawImageRect2(
...
void SkPictureRecord::onDrawImageLattice2(
...
void SkPictureRecord::onDrawTextBlob(
...
void SkPictureRecord::onDrawPicture(
...
void SkPictureRecord::onDrawVerticesObject(
...
void SkPictureRecord::onDrawPatch(
...
void SkPictureRecord::onDrawAtlas2(
    const SkImage* atlas, const SkRSXform xform[], const SkRect tex[], const SkColor colors[],
...
void SkPictureRecord::onDrawEdgeAAQuad(
  ...
  size_t size =
      4 * kUInt32Size + sizeof(SkColor4f) + sizeof(rect) + (clip ? 4 : 0) * sizeof(SkPoint);
...
void SkPictureRecord::onDrawEdgeAAImageSet2(
    const SkCanvas::ImageSetEntry set[], int count, const SkPoint dstClips[],
  ...
  size_t size =
      6 * kUInt32Size + sizeof(SkPoint) * totalDstClipCount + kMatrixSize * totalMatrixCount +
      (4 * kUInt32Size + 2 * sizeof(SkRect) + sizeof(SkScalar)) * count + SkSamplingPriv::kFlatSize;
...
void SkPictureRecord::addMatrix(const SkMatrix& matrix) { fWriter.writeMatrix(matrix); }
...
void SkPictureRecord::addPath(const SkPath& path) { this->addInt(this->addPathToHeap(path)); }
...
void SkPictureRecord::addPoint(const SkPoint& point) { fWriter.writePoint(point); }
...
void SkPictureRecord::addRect(const SkRect& rect) { fWriter.writeRect(rect); }
...
void SkPictureRecord::addIRect(const SkIRect& rect) { fWriter.write(&rect, sizeof(rect)); }
...
void SkPictureRecord::addRRect(const SkRRect& rrect) { fWriter.writeRRect(rrect); }

void SkPictureRecord::addRegion(const SkRegion& region) { fWriter.writeRegion(region); }
