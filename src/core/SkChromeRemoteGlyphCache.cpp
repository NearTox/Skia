...
class LowerRangeBitVector {
 ...
 private:
  ...
  static_assert(
      GID::kSubPixelX < GID::kGlyphID && GID::kGlyphID < GID::kSubPixelY,
...
class RemoteStrike final : public SkStrikeForGPU {
 public:
  ...
  RemoteStrike(
      const SkStrikeSpec& strikeSpec, std::unique_ptr<SkScalerContext> context,
 ...
 private:
  ...
  struct PathSummaryTraits {
    static SkPackedGlyphID GetKey(PathSummary summary) { return summary.packedID; }
  ...
  struct DrawableSummaryTraits {
    static SkGlyphID GetKey(DrawableSummary summary) { return summary.glyphID; }
...
void RemoteStrike::setStrikeSpec(const SkStrikeSpec& strikeSpec) { fStrikeSpec = &strikeSpec; }
...
void RemoteStrike::commonMaskLoop(
    SkDrawableGlyphBuffer* accepted, SkSourceGlyphBuffer* rejected, Rejector&& reject) {
  accepted->forEachInput([&](size_t i, SkPackedGlyphID packedID, SkPoint position) {
...
void RemoteStrike::prepareForSDFTDrawing(
    SkDrawableGlyphBuffer* accepted, SkSourceGlyphBuffer* rejected) {
  this->commonMaskLoop(
      accepted, rejected, [](SkGlyphDigest digest) { return !digest.canDrawAsSDFT(); });
}
...
struct WireTypeface {
  ...
  WireTypeface(
      SkTypefaceID typefaceId, int glyphCount, SkFontStyle style, bool isFixed,
      bool needsCurrentColor)
      : fTypefaceID(typefaceId),
        fGlyphCount(glyphCount),
        fStyle(style),
        fIsFixed(isFixed),
        fGlyphMaskNeedsCurrentColor(needsCurrentColor) {}
...
class SkStrikeServerImpl final : public SkStrikeForGPUCacheInterface {
 public:
  explicit SkStrikeServerImpl(SkStrikeServer::DiscardableHandleManager* discardableHandleManager);
 ...
 private:
  ...
  struct MapOps {
    ...
    bool operator()(const SkDescriptor* lhs, const SkDescriptor* rhs) const { return *lhs == *rhs; }
  };
...
sk_sp<SkData> SkStrikeServerImpl::serializeTypeface(SkTypeface* tf) {
  ...
  WireTypeface wire(
      SkTypeface::UniqueID(tf), tf->countGlyphs(), tf->fontStyle(), tf->isFixedPitch(),
      tf->glyphMaskNeedsCurrentColor());
  data =
      fSerializedTypefaces.set(SkTypeface::UniqueID(tf), SkData::MakeWithCopy(&wire, sizeof(wire)));
  return *data;
}
...
RemoteStrike* SkStrikeServerImpl::getOrCreateCache(const SkStrikeSpec& strikeSpec) {
  ...
  TRACE_EVENT1(
      "skia", "RecForDesc", "rec",
      TRACE_STR_COPY([&strikeSpec]() {
        auto ptr = strikeSpec.descriptor().findEntry(kRec_SkDescriptorTag, nullptr);
  ...
  if (!fCachedTypefaces.contains(typefaceId)) {
    ...
    fTypefacesToSend.emplace_back(
        typefaceId, typeface.countGlyphs(), typeface.fontStyle(), typeface.isFixedPitch(),
...
class GlyphTrackingDevice final : public SkNoPixelsDevice {
 public:
  ...
  SkBaseDevice* onCreateDevice(const CreateInfo& cinfo, const SkPaint*) override {
    ...
    return new GlyphTrackingDevice(
        cinfo.fInfo.dimensions(), surfaceProps, fStrikeServerImpl, cinfo.fInfo.refColorSpace(),
        fSDFTControl);
  }
 ...
 protected:
  void onDrawGlyphRunList(
    ...
    SubRunContainer::MakeInAlloc(
        glyphRunList, drawMatrix, drawingPaint, this->strikeDeviceInfo(), fStrikeServerImpl,
        nullptr, "Cache Diff");
  }

  sk_sp<sktext::gpu::Slug> convertGlyphRunListToSlug(
      const SkGlyphRunList& glyphRunList, const SkPaint& initialPaint,
      const SkPaint& drawingPaint) override {
    ...
    SubRunContainer::MakeInAlloc(
        glyphRunList, positionMatrix, drawingPaint, this->strikeDeviceInfo(), fStrikeServerImpl,
        nullptr, "Convert Slug Analysis");

    // Use the glyph strike cache to get actual glyph information.
    return skgpu::v1::MakeSlug(
        this->localToDevice(), glyphRunList, initialPaint, drawingPaint, this->strikeDeviceInfo(),
...
std::unique_ptr<SkCanvas> SkStrikeServer::makeAnalysisCanvas(
    int width, int height, const SkSurfaceProps& props, sk_sp<SkColorSpace> colorSpace,
    bool DFTSupport) {
#if SK_SUPPORT_GPU
  GrContextOptions ctxOptions;
  auto control = sktext::gpu::SDFTControl{
      DFTSupport, props.isUseDeviceIndependentFonts(), ctxOptions.fMinDistanceFieldFontSize,
      ctxOptions.fGlyphsAsPathsFontSize};

  sk_sp<SkBaseDevice> trackingDevice(new GlyphTrackingDevice(
      SkISize::Make(width, height), props, this->impl(), std::move(colorSpace), control));
#else
  sk_sp<SkBaseDevice> trackingDevice(
      new SkNoPixelsDevice(SkIRect::MakeWH(width, height), props, std::move(colorSpace)));
#endif
...
class SkStrikeClientImpl {
 public:
  explicit SkStrikeClientImpl(
      sk_sp<SkStrikeClient::DiscardableHandleManager>, bool isLogging = true,
...
SkStrikeClientImpl::SkStrikeClientImpl(
    sk_sp<SkStrikeClient::DiscardableHandleManager> discardableManager, bool isLogging,
    ...
    for (size_t j = 0; j < glyphDrawablesCount; j++) {
      ...
      if (drawableSize > 0) {
        ...
        sk_sp<SkPicture> picture(
            SkPicture::MakeFromData(const_cast<const void*>(drawableData), drawableSize));
...
SkStrikeClient::SkStrikeClient(
    sk_sp<DiscardableHandleManager> discardableManager, bool isLogging, SkStrikeCache* strikeCache)
