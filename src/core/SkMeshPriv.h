...
#  if SK_SUPPORT_GPU
  ...
  class GpuBuffer final : public Base {
   public:
    GpuBuffer() = default;
...
template <typename Base>
sk_sp<Base> SkMeshPriv::CpuBuffer<Base>::Make(sk_sp<const SkData> data) {
  SkASSERT(data);
...
SkMeshPriv::GpuBuffer<Base, Type>::~GpuBuffer() {
...
sk_sp<Base> SkMeshPriv::GpuBuffer<Base, Type>::Make(GrDirectContext* dc, sk_sp<const SkData> data) {
  ...
  sk_sp<GrGpuBuffer> buffer = dc->priv().resourceProvider()->createBuffer(
      data->data(), data->size(), Type, kStatic_GrAccessPattern);
  if (!buffer) {
    return nullptr;
  }
