...
struct SolidColorShaderBlock {
  static void BeginBlock(
      const SkKeyContext&, SkPaintParamsKeyBuilder*, SkPipelineDataGatherer*, const SkPMColor4f&);
};
...
struct GradientShaderBlocks {
  struct GradientData {
    ...
    GradientData(
        SkShader::GradientType, const SkM44& localMatrix, SkPoint point0, SkPoint point1,
        float radius0, float radius1, float bias, float scale, SkTileMode, int numStops,
        SkColor4f* colors, float* offsets);

    bool operator==(const GradientData& rhs) const {
      return fType == rhs.fType && fLocalMatrix == rhs.fLocalMatrix &&
             fPoints[0] == rhs.fPoints[0] && fPoints[1] == rhs.fPoints[1] &&
             fRadii[0] == rhs.fRadii[0] && fRadii[1] == rhs.fRadii[1] && fBias == rhs.fBias &&
             fScale == rhs.fScale && fTM == rhs.fTM && fNumStops == rhs.fNumStops &&
  ...
  static void BeginBlock(
      const SkKeyContext&, SkPaintParamsKeyBuilder*, SkPipelineDataGatherer*, const GradientData&);
};

struct LocalMatrixShaderBlock {
  struct LMShaderData {
    LMShaderData(const SkMatrix& localMatrix) : fLocalMatrix(localMatrix) {}

    const SkM44 fLocalMatrix;
  };

  static void BeginBlock(
      const SkKeyContext&, SkPaintParamsKeyBuilder*, SkPipelineDataGatherer*, const LMShaderData&);
};

struct ImageShaderBlock {
  struct ImageData {
    ImageData(
        const SkSamplingOptions& sampling, SkTileMode tileModeX, SkTileMode tileModeY,
        SkRect subset, const SkMatrix& localMatrix);
  ...
  static void BeginBlock(
      const SkKeyContext&, SkPaintParamsKeyBuilder*, SkPipelineDataGatherer*, const ImageData&);
};
...
struct BlendShaderBlock {
  ...
  static void BeginBlock(
      const SkKeyContext&, SkPaintParamsKeyBuilder*, SkPipelineDataGatherer*,
      const BlendShaderData&);
};

struct BlendModeBlock {
  static void BeginBlock(
      const SkKeyContext&, SkPaintParamsKeyBuilder*, SkPipelineDataGatherer*, SkBlendMode);
};

// Bridge between the combinations system and the SkPaintParamsKey
SkUniquePaintParamsID CreateKey(
    const SkKeyContext&, SkPaintParamsKeyBuilder*, SkShaderType, SkBlendMode);
