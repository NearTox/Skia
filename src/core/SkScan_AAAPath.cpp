...
void MaskAdditiveBlitter::blitAntiRect(
...
class RunBasedAdditiveBlitter : public AdditiveBlitter {
 ...
 protected:
  ...
  void advanceRuns() {
    ...
    fRuns.fRuns =
        reinterpret_cast<int16_t*>(reinterpret_cast<uint8_t*>(fRunsBuffer) + fCurrentRun * kRunsSz);
...
static void compute_alpha_above_line(
...
// Here we always send in l < SK_Fixed1, and the first alpha we want to compute is alphas[0]
static void compute_alpha_below_line(
    SkAlpha* alphas, SkFixed l, SkFixed r, SkFixed dY, SkAlpha fullAlpha) {
  SkASSERT(l <= r);
  SkASSERT(l >> 16 == 0);
  int R = SkFixedCeilToInt(r);
...
static SK_ALWAYS_INLINE void blit_single_alpha(
    AdditiveBlitter* blitter, int y, int x, SkAlpha alpha, SkAlpha fullAlpha, SkAlpha* maskRow,
...
static SK_ALWAYS_INLINE void blit_two_alphas(
    AdditiveBlitter* blitter, int y, int x, SkAlpha a1, SkAlpha a2, SkAlpha fullAlpha,
    SkAlpha* maskRow, bool isUsingMask, bool noRealBlitter, bool needSafeCheck) {
  if (isUsingMask) {
    if (needSafeCheck) {
...
static SK_ALWAYS_INLINE void blit_full_alpha(
    AdditiveBlitter* blitter, int y, int x, int len, SkAlpha fullAlpha, SkAlpha* maskRow,
    bool isUsingMask, bool noRealBlitter, bool needSafeCheck) {
  if (isUsingMask) {
    for (int i = 0; i < len; ++i) {
      if (needSafeCheck) {
...
static void blit_aaa_trapezoid_row(
    AdditiveBlitter* blitter, int y, SkFixed ul, SkFixed ur, SkFixed ll, SkFixed lr, SkFixed lDY,
    SkFixed rDY, SkAlpha fullAlpha, SkAlpha* maskRow, bool isUsingMask, bool noRealBlitter,
    bool needSafeCheck) {
  ...
  if (len == 1) {
    ...
    blit_single_alpha(
        blitter, y, L, alpha, fullAlpha, maskRow, isUsingMask, noRealBlitter, needSafeCheck);
    return;
  }
...
static SK_ALWAYS_INLINE void blit_trapezoid_row(
    AdditiveBlitter* blitter, int y, SkFixed ul, SkFixed ur, SkFixed ll, SkFixed lr, SkFixed lDY,
    SkFixed rDY, SkAlpha fullAlpha, SkAlpha* maskRow, bool isUsingMask, bool noRealBlitter = false,
  ...
  if (joinLeft <= joinRite) {  // There's a rect from joinLeft to joinRite that we can blit
    if (ul < joinLeft) {
      ...
      if (len == 1) {
        ...
        blit_single_alpha(
            blitter, y, ul >> 16, alpha, fullAlpha, maskRow, isUsingMask, noRealBlitter,
            needSafeCheck);
      } else if (len == 2) {
        ...
        blit_two_alphas(
            blitter, y, ul >> 16, a1, a2, fullAlpha, maskRow, isUsingMask, noRealBlitter,
            needSafeCheck);
      } else {
        blit_aaa_trapezoid_row(
            blitter, y, ul, joinLeft, ll, joinLeft, lDY, SK_MaxS32, fullAlpha, maskRow, isUsingMask,
    ...
    if (joinLeft < joinRite) {
      blit_full_alpha(
          blitter, y, SkFixedFloorToInt(joinLeft), SkFixedFloorToInt(joinRite - joinLeft),
    ...
    if (lr > joinRite) {
      ...
      if (len == 1) {
        ...
        blit_single_alpha(
            blitter, y, joinRite >> 16, alpha, fullAlpha, maskRow, isUsingMask, noRealBlitter,
            needSafeCheck);
      } else if (len == 2) {
        ...
        blit_two_alphas(
            blitter, y, joinRite >> 16, a1, a2, fullAlpha, maskRow, isUsingMask, noRealBlitter,
            needSafeCheck);
      } else {
        blit_aaa_trapezoid_row(
            blitter, y, joinRite, ur, joinRite, lr, SK_MaxS32, rDY, fullAlpha, maskRow, isUsingMask,
            noRealBlitter, needSafeCheck);
      }
    }
  } else {
    blit_aaa_trapezoid_row(
        blitter, y, ul, ur, ll, lr, lDY, rDY, fullAlpha, maskRow, isUsingMask, noRealBlitter,
        needSafeCheck);
  }
}
...
static bool is_smooth_enough(
    SkAnalyticEdge* leftE, SkAnalyticEdge* riteE, SkAnalyticEdge* currE, int stop_y) {
...
static void aaa_walk_convex_edges(
    SkAnalyticEdge* prevHead, AdditiveBlitter* blitter, int start_y, int stop_y, SkFixed leftBound,
    SkFixed riteBound, bool isUsingMask) {
    ...
    if (0 == (dLeft | dRite)) {
      ...
      if (fullRite >= fullLeft) {
        if (partialTop > 0) {  // blit first partial row
          if (partialLeft > 0) {
            blitter->blitAntiH(
                fullLeft - 1, fullTop - 1, fixed_to_alpha(SkFixedMul(partialTop, partialLeft)));
          }
          blitter->blitAntiH(
              fullLeft, fullTop - 1, fullRite - fullLeft, fixed_to_alpha(partialTop));
          if (partialRite > 0) {
            blitter->blitAntiH(
                fullRite, fullTop - 1, fixed_to_alpha(SkFixedMul(partialTop, partialRite)));
          }
        ...
        if (fullBot > fullTop &&
          ...
          blitter->getRealBlitter()->blitAntiRect(
              fullLeft - 1, fullTop, fullRite - fullLeft, fullBot - fullTop,
              fixed_to_alpha(partialLeft), fixed_to_alpha(partialRite));
        }

        if (partialBot > 0) {  // blit last partial row
          if (partialLeft > 0) {
            blitter->blitAntiH(
                fullLeft - 1, fullBot, fixed_to_alpha(SkFixedMul(partialBot, partialLeft)));
          }
          blitter->blitAntiH(fullLeft, fullBot, fullRite - fullLeft, fixed_to_alpha(partialBot));
          if (partialRite > 0) {
            blitter->blitAntiH(
                fullRite, fullBot, fixed_to_alpha(SkFixedMul(partialBot, partialRite)));
          }
        }
      } else {  // left and rite are within the same pixel
        if (partialTop > 0) {
          blitter->blitAntiH(
              fullLeft - 1, fullTop - 1, 1, fixed_to_alpha(SkFixedMul(partialTop, rite - left)));
          blitter->flush_if_y_changed(y, y + partialTop);
        }
        if (fullBot > fullTop) {
        ...
        if (partialBot > 0) {
          blitter->blitAntiH(
              fullLeft - 1, fullBot, 1, fixed_to_alpha(SkFixedMul(partialBot, rite - left)));
        }
      }
      ...
      if (count > 1) {
        if ((int)(y & 0xFFFF0000) != y) {  // There's a partial-row on the top
          ...
          blit_trapezoid_row(
              blitter, y >> 16, left & kSnapMask, rite & kSnapMask, nextLeft & kSnapMask,
              nextRite & kSnapMask, leftE->fDY, riteE->fDY, get_partial_alpha(0xFF, dY), maskRow,
              isUsingMask);
        ...
        while (count > 1) {  // Full rows in the middle
          ...
          blit_trapezoid_row(
              blitter, y >> 16, left & kSnapMask, rite & kSnapMask, nextLeft & kSnapMask,
              nextRite & kSnapMask, leftE->fDY, riteE->fDY, 0xFF, maskRow, isUsingMask);
          blitter->flush_if_y_changed(y, nextY);
          left = nextLeft;
          rite = nextRite;
          y = nextY;
...
static void blit_saved_trapezoid(
    SkAnalyticEdge* leftE, SkFixed lowerY, SkFixed lowerLeft, SkFixed lowerRite,
  ...
  SkAlpha fullAlpha =
      fixed_to_alpha(lowerY - SkIntToFixed(y)) - fixed_to_alpha(leftE->fSavedY - SkIntToFixed(y));
  // We need fSavedDY because the (quad or cubic) edge might be updated
  blit_trapezoid_row(
      blitter, y, std::max(leftE->fSavedX, leftClip), std::min(riteE->fSavedX, rightClip),
      std::max(lowerLeft, leftClip), std::min(lowerRite, rightClip), leftE->fSavedDY,
      riteE->fSavedDY, fullAlpha, maskRow, isUsingMask,
...
static void deferred_blit(
    SkAnalyticEdge* leftE, SkAnalyticEdge* riteE, SkFixed left,
  ...
  if (leftE->fRiteE && leftE->fRiteE != riteE) {
    ...
    blit_saved_trapezoid(
        leftE, y, left, leftE->fRiteE->fX, blitter, maskRow, isUsingMask, noRealBlitter, leftClip,
        rightClip);
  }
  if (!leftE->fRiteE) {
  ...
  if (isIntegralNextY || leftEnds || riteEnds) {
    blit_saved_trapezoid(
        leftE, nextY, leftE->fX, riteE->fX, blitter, maskRow, isUsingMask, noRealBlitter, leftClip,
        rightClip);
  }
}
...
static void aaa_walk_edges(
  ...
  if (isInverse && SkIntToFixed(start_y) != y) {
    ...
    blit_full_alpha(
        blitter, start_y, SkFixedFloorToInt(leftClip), width,
        fixed_to_alpha(y - SkIntToFixed(start_y)), maskRow, isUsingMask, false, false);
  }
  ...
  while (true) {
    ...
    while (currE->fUpperY <= y) {
      ...
      if (useDeferred) {
        if (currE->fRiteE && !isLeft) {
          ...
          blit_saved_trapezoid(
              currE, y, currE->fX, currE->fRiteE->fX, blitter, maskRow, isUsingMask, noRealBlitter,
              leftClip, rightClip);
        }
        if (leftE->fRiteE == currE && !isRite) {
          ...
          blit_saved_trapezoid(
              leftE, y, left, currE->fX, blitter, maskRow, isUsingMask, noRealBlitter, leftClip,
              rightClip);
        }
      }
      ...
      if (isRite) {
        if (useDeferred) {
          deferred_blit(
              leftE, currE, left, leftDY, y, nextY, isIntegralNextY, leftEnds, currEnds, blitter,
              maskRow, isUsingMask, noRealBlitter, leftClip, rightClip, yShift);
        } else {
          ...
          blit_trapezoid_row(
              blitter, y >> 16, left, rite, nextLeft, nextRite, leftDY, currE->fDY, fullAlpha,
              maskRow, isUsingMask,
              noRealBlitter || (fullAlpha == 0xFF && (edges_too_close(prevRite, left, leftE->fX) ||
      ...
      if (currE->fLowerY <= nextY) {
        ...
        if (newX < prevX) {  // ripple currE backwards until it is x-sorted
          ...
          if (leftE->fRiteE == currE && useDeferred) {
            ...
            blit_saved_trapezoid(
                leftE, nextY, leftE->fX, currE->fX, blitter, maskRow, isUsingMask, noRealBlitter,
                leftClip, rightClip);
          }
    ...
    if (in_interval) {
      if (useDeferred) {
        deferred_blit(
            leftE, nextTail, left, leftDY, y, nextY, isIntegralNextY, leftEnds, false, blitter,
            maskRow, isUsingMask, noRealBlitter, leftClip, rightClip, yShift);
      } else {
        blit_trapezoid_row(
            blitter, y >> 16, left, rightClip, std::max(leftClip, leftE->fX), rightClip, leftDY, 0,
...
static SK_ALWAYS_INLINE void aaa_fill_path(
  ...
  if (0 == count) {
    if (path.isInverseFillType()) {
      ...
      if (!rect.isEmpty()) {
        blitter->getRealBlitter()->blitRect(rect.fLeft, rect.fTop, rect.width(), rect.height());
      }
    }
    return;
  }
  ...
  if (!path.isInverseFillType() && path.isConvex() && count >= 2) {
    aaa_walk_convex_edges(&headEdge, blitter, start_y, stop_y, leftBound, rightBound, isUsingMask);
  } else {
...
void SkScan::AAAFillPath(
  ...
  if (MaskAdditiveBlitter::CanHandleRect(ir) && !isInverse && !forceRLE) {
    ...
    if (!TryBlitFatAntiRect(blitter, path, clipBounds)) {
      ...
      aaa_fill_path(
          path, clipBounds, &additiveBlitter, ir.fTop, ir.fBottom, containedInClip, true, forceRLE);
    }
    ...
    aaa_fill_path(
        path, clipBounds, &additiveBlitter, ir.fTop, ir.fBottom, containedInClip, false, forceRLE);
  } else {
