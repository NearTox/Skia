...
static void generateMask(
    const SkMask& mask, const SkPath& path, const SkMaskGamma::PreBlend& maskPreBlend, bool doBGR,
    bool doVert) {
  SkPaint paint;
  ...
  switch (mask.fFormat) {
    case SkMask::kBW_Format:
      ...
      break;
    case SkMask::kA8_Format: break;
    case SkMask::kLCD16_Format:
      if (doVert) {
        dstW = 4 * dstH - 8;
        dstH = srcW;
        matrix.setAll(
            0, 4, -SkIntToScalar(mask.fBounds.fTop + 1) * 4, 1, 0,
            -SkIntToScalar(mask.fBounds.fLeft), 0, 0, 1);
      } else {
        dstW = 4 * dstW - 8;
        matrix.setAll(
            4, 0, -SkIntToScalar(mask.fBounds.fLeft + 1) * 4, 0, 1,
            -SkIntToScalar(mask.fBounds.fTop), 0, 0, 1);
      }
      dstRB = 0;  // signals we need a copy
      break;
    default: SkDEBUGFAIL("unexpected mask format");
  }
  ...
  switch (mask.fFormat) {
    ...
    case SkMask::kA8_Format:
      ...
      break;
    case SkMask::kLCD16_Format: pack4xHToLCD16(dst, mask, maskPreBlend, doBGR, doVert); break;
    default: break;
  }
}
