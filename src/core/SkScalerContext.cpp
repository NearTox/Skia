...
SkScalerContextRec SkScalerContext::PreprocessRec(
  ...
  if (rec.fMaskFormat == SkMask::kA8_Format) {
    U8CPU lum =
        SkComputeLuminance(SkColorGetR(lumColor), SkColorGetG(lumColor), SkColorGetB(lumColor));
...
SkScalerContext::SkScalerContext(
  ...
  if constexpr (kSkScalerContextDumpRec) {
    SkDebugf(
        "SkScalerContext checksum %x count %d length %d\n", desc->getChecksum(), desc->getCount(),
        desc->getLength());
 ...
 */
static const SkMaskGamma& cached_mask_gamma(
    SkScalar contrast, SkScalar paintGamma, SkScalar deviceGamma) {
...
SkMaskGamma::PreBlend SkScalerContext::GetMaskPreBlend(const SkScalerContextRec& rec) {
  ...
  const SkMaskGamma& maskGamma =
      cached_mask_gamma(rec.getContrast(), rec.getPaintGamma(), rec.getDeviceGamma());

  // TODO: remove CanonicalColor when we to fix up Chrome layout tests.
  return maskGamma.preBlend(rec.getLuminanceColor());
}

size_t SkScalerContext::GetGammaLUTSize(
    SkScalar contrast, SkScalar paintGamma, SkScalar deviceGamma, int* width, int* height) {
  SkAutoMutexExclusive ama(mask_gamma_cache_mutex());
  const SkMaskGamma& maskGamma = cached_mask_gamma(contrast, paintGamma, deviceGamma);
...
bool SkScalerContext::GetGammaLUTData(
    SkScalar contrast, SkScalar paintGamma, SkScalar deviceGamma, uint8_t* data) {
  SkAutoMutexExclusive ama(mask_gamma_cache_mutex());
  const SkMaskGamma& maskGamma = cached_mask_gamma(contrast, paintGamma, deviceGamma);
...
SkGlyph SkScalerContext::internalMakeGlyph(
    SkPackedGlyphID packedID, SkMask::Format format, SkArenaAlloc* alloc) {
  ...
  if (fGenerateImageFromPath) {
    ...
    if (devPath) {
      ...
      if (glyph.fMaskFormat != SkMask::kBW_Format && glyph.fMaskFormat != SkMask::kA8_Format &&
...
static void generateMask(
    ...
    const bool doBGR, const bool doVert, const bool a8FromLCD, const bool hairline) {
  SkASSERT(
      mask.fFormat == SkMask::kBW_Format || mask.fFormat == SkMask::kA8_Format ||
  ...
  const bool fromLCD =
      (mask.fFormat == SkMask::kLCD16_Format) || (mask.fFormat == SkMask::kA8_Format && a8FromLCD);
  ...
  if (fromLCD) {
    if (doVert) {
      ...
      dstH = srcW;
      matrix.setAll(
          0, 4, -SkIntToScalar(mask.fBounds.fTop + 1) * 4, 1, 0, -SkIntToScalar(mask.fBounds.fLeft),
          0, 0, 1);
    } else {
      dstW = 4 * dstW - 8;
      matrix.setAll(
          4, 0, -SkIntToScalar(mask.fBounds.fLeft + 1) * 4, 0, 1, -SkIntToScalar(mask.fBounds.fTop),
          0, 0, 1);
    }
  ...
  switch (mask.fFormat) {
    case SkMask::kBW_Format: packA8ToA1(mask, dst.addr8(0, 0), dst.rowBytes()); break;
    case SkMask::kA8_Format:
      ...
      break;
    case SkMask::kLCD16_Format: pack4xHToMask(dst, mask, maskPreBlend, doBGR, doVert); break;
    default: break;
  }
}
...
sk_sp<SkDrawable> SkScalerContext::generateDrawable(const SkGlyph&) { return nullptr; }
...
void SkScalerContext::internalGetPath(SkGlyph& glyph, SkArenaAlloc* alloc) {
  ...
  if (fRec.fFrameWidth < 0 && fPathEffect == nullptr) {
    ...
    if (fRec.fFrameWidth > 0) {
      rec.setStrokeStyle(fRec.fFrameWidth, SkToBool(fRec.fFlags & kFrameAndFill_Flag));
      // glyphs are always closed contours, so cap type is ignored,
      // so we just pass something.
      rec.setStrokeParams(
          (SkPaint::Cap)fRec.fStrokeCap, (SkPaint::Join)fRec.fStrokeJoin, fRec.fMiterLimit);
    }
...
bool SkScalerContextRec::computeMatrices(
    PreMatrixScale preMatrixScale, SkVector* s, SkMatrix* sA, SkMatrix* GsA, SkMatrix* G_inv,
    SkMatrix* A_out) {
  ...
  if (SkScalarAbs(GA.get(SkMatrix::kMScaleX)) <= SK_ScalarNearlyZero ||
      SkScalarAbs(GA.get(SkMatrix::kMScaleY)) <= SK_ScalarNearlyZero || !GA.isFinite()) {
    s->fX = SK_Scalar1;
    s->fY = SK_Scalar1;
    sA->setScale(0, 0);
    if (GsA) {
...
static SkMask::Format compute_mask_format(const SkFont& font) {
  switch (font.getEdging()) {
    ...
    case SkFont::Edging::kSubpixelAntiAlias: return SkMask::kLCD16_Format;
  }
  SkASSERT(false);
...
void SkScalerContext::MakeRecAndEffects(
  ...
  if (font.isEmbolden()) {
    ...
    SkScalar fakeBoldScale = SkScalarInterpFunc(
        font.getSize(), kStdFakeBoldInterpKeys, kStdFakeBoldInterpValues, kStdFakeBoldInterpLength);
    ...
    if (too_big_for_lcd(*rec, checkPost2x2)) {
      ...
      switch (geometry) {
        ...
        case kRGB_H_SkPixelGeometry:
          ...
          break;
        case kBGR_H_SkPixelGeometry: flags |= SkScalerContext::kLCD_BGROrder_Flag; break;
        case kRGB_V_SkPixelGeometry: flags |= SkScalerContext::kLCD_Vertical_Flag; break;
...
static void generate_descriptor(
  ...
  if (effectBuffer.bytesWritten() > 0) {
    effectBuffer.writeToMemory(
        desc->addEntry(kEffects_SkDescriptorTag, effectBuffer.bytesWritten(), nullptr));
  }
...
SkDescriptor* SkScalerContext::AutoDescriptorGivenRecAndEffects(
    const SkScalerContextRec& rec, const SkScalerContextEffects& effects, SkAutoDescriptor* ad) {
...
bool SkScalerContext::CheckBufferSizeForRec(
    const SkScalerContextRec& rec, const SkScalerContextEffects& effects, size_t size) {
  SkBinaryWriteBuffer buf;
