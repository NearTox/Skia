...
class SkEdgeClipper {
 public:
  ...
  SkPath::Verb next(SkPoint pts[]) noexcept;

  bool canCullToTheRight() const noexcept { return fCanCullToTheRight; }
 ...
 private:
  ...
  void clipMonoQuad(const SkPoint srcPts[3], const SkRect& clip) noexcept;
  void clipMonoCubic(const SkPoint srcPts[4], const SkRect& clip) noexcept;
  void appendLine(SkPoint p0, SkPoint p1) noexcept;
  void appendVLine(SkScalar x, SkScalar y0, SkScalar y1, bool reverse) noexcept;
  void appendQuad(const SkPoint pts[3], bool reverse) noexcept;
  void appendCubic(const SkPoint pts[4], bool reverse) noexcept;
};
