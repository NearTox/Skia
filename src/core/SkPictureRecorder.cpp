...
sk_sp<SkPicture> SkPictureRecorder::finishRecordingAsPicture(uint32_t finishFlags) {
  ...
  if (fBBH.get()) {
    ...
    SkASSERT(
        (bbhBound.isEmpty() || fCullRect.contains(bbhBound)) ||
  ...
  return sk_make_sp<SkBigPicture>(
      fCullRect, fRecord.release(), pictList, fBBH.release(), subPictureBytes);
}
...
void SkPictureRecorder::partialReplay(SkCanvas* canvas) const {
  ...
  SkRecordDraw(
      *fRecord, canvas, nullptr, drawables, drawableCount, nullptr /*bbh*/, nullptr /*callback*/);
}
