...
class SkTLazy {
 public:
  ...
  explicit SkTLazy(const T* src) noexcept : fPtr(src ? new (&fStorage) T(*src) : nullptr) {
    static_assert(std::is_nothrow_copy_constructible_v<T> == true);
  }
  SkTLazy(const SkTLazy& that) noexcept
      : fPtr(that.fPtr ? new (&fStorage) T(*that.fPtr) : nullptr) {
    static_assert(std::is_nothrow_copy_constructible_v<T> == true);
  }
  SkTLazy(SkTLazy&& that) noexcept
  ...
  T* init(Args&&... args) noexcept {
    static_assert(std::is_nothrow_constructible_v<T, Args...> == true);
    this->reset();
...
class SkTCopyOnFirstWrite {
 public:
  ...
  SkTCopyOnFirstWrite(SkTCopyOnFirstWrite&& that) noexcept { *this = std::move(that); }
