...
#define CASE(op)                    \
  ...
  case 2 * (int)op + 1
        ...
        CASE(Op::sub_i16x2)
            : r(d).i32 =
            skvx::bit_pun<I32>(skvx::bit_pun<I16x2>(r(x).i32) - skvx::bit_pun<I16x2>(r(y).i32));
        break;
        CASE(Op::mul_i16x2)
            : r(d).i32 =
            skvx::bit_pun<I32>(skvx::bit_pun<I16x2>(r(x).i32) * skvx::bit_pun<I16x2>(r(y).i32));
        break;
        CASE(Op::shr_i16x2) : r(d).i32 = skvx::bit_pun<I32>(skvx::bit_pun<U16x2>(r(x).i32) >> imm);
        break;
        ...
        CASE(Op::shl) : r(d).i32 = r(x).i32 << imm;
        break;
        CASE(Op::sra) : r(d).i32 = r(x).i32 >> imm;
        break;
        CASE(Op::shr) : r(d).u32 = r(x).u32 >> imm;
        break;
  ...
  for (Val id = 0; id < (Val)instructions.size(); id++) {
    ...
    if (inst.death != 0 && !inst.hoist) {
      ...
      auto maybe_recycle_register = [&](Val input) {
        ...
        if (input != NA && !instructions[input].hoist && instructions[input].death == id) {
...
#  endif
  ...
  auto emit = [&](Val id, bool scalar) {
    ...
    if (x != NA && instructions[x].death == id && !hoisted(x)) {
      avail |= 1 << r[x];
    }
    if (y != NA && instructions[y].death == id && !hoisted(y)) {
      avail |= 1 << r[y];
    }
    if (z != NA && instructions[z].death == id && !hoisted(z)) {
      avail |= 1 << r[z];
    }
