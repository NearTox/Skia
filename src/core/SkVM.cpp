...
static void dump_builder_program(const std::vector<Builder::Instruction>& program, SkWStream* o) {
  for (Val id = 0; id < (Val)program.size(); id++) {
    ...
    int imm = inst.imm;
    write(
        o, inst.death == 0 ? "☠️ " : !inst.can_hoist ? "  " : inst.used_in_loop ? "↑ " : "↟ ");
    switch (op) {
...
Program Builder::done(const char* debug_name) {
  ...
  for (Val id = fProgram.size(); id-- > 0;) {
    ...
    if (inst.death != 0) {
      ...
      if (inst.x != NA && fProgram[inst.x].death == 0) {
        fProgram[inst.x].death = id;
      }
      if (inst.y != NA && fProgram[inst.y].death == 0) {
        fProgram[inst.y].death = id;
      }
      if (inst.z != NA && fProgram[inst.z].death == 0) {
...
static bool operator==(const Builder::Instruction& a, const Builder::Instruction& b) {
  return a.op == b.op && a.x == b.x && a.y == b.y && a.z == b.z && a.imm == b.imm &&
         a.death == b.death && a.can_hoist == b.can_hoist && a.used_in_loop == b.used_in_loop;
}
...
size_t Builder::InstructionHash::operator()(const Instruction& inst) const {
  return Hash((uint8_t)inst.op) ^ Hash(inst.x) ^ Hash(inst.y) ^ Hash(inst.z) ^ Hash(inst.imm) ^
         Hash(inst.death) ^ Hash(inst.can_hoist) ^ Hash(inst.used_in_loop);
}
...
bool Builder::isZero(Val id) const { return fProgram[id].op == Op::splat && fProgram[id].imm == 0; }
...
int Assembler::disp19(Label* l) {
  SkASSERT(l->kind == Label::None || l->kind == Label::ARMDisp19);
...
int Assembler::disp32(Label* l) {
  SkASSERT(l->kind == Label::None || l->kind == Label::X86Disp32);
...
#define STRIDE_K(op) case 2 * (int)op + 1
          STRIDE_1(Op::store8) : memcpy(arg(imm), &r(x).i32, 1);
          break;
          STRIDE_1(Op::store16) : memcpy(arg(imm), &r(x).i32, 2);
          break;
          STRIDE_1(Op::store32) : memcpy(arg(imm), &r(x).i32, 4);
          break;

          STRIDE_K(Op::store8) : skvx::cast<uint8_t>(r(x).i32).store(arg(imm));
          break;
          STRIDE_K(Op::store16) : skvx::cast<uint16_t>(r(x).i32).store(arg(imm));
          break;
          STRIDE_K(Op::store32) : (r(x).i32).store(arg(imm));
          break;

          STRIDE_1(Op::load8) : r(d).i32 = 0;
          memcpy(&r(d).i32, arg(imm), 1);
          break;
          STRIDE_1(Op::load16) : r(d).i32 = 0;
          memcpy(&r(d).i32, arg(imm), 2);
          break;
          STRIDE_1(Op::load32) : r(d).i32 = 0;
          memcpy(&r(d).i32, arg(imm), 4);
          break;

          STRIDE_K(Op::load8) : r(d).i32 = skvx::cast<int>(U8 ::Load(arg(imm)));
          break;
          STRIDE_K(Op::load16) : r(d).i32 = skvx::cast<int>(U16::Load(arg(imm)));
          break;
          STRIDE_K(Op::load32) : r(d).i32 = I32::Load(arg(imm));
          break;
...
#define CASE(op)                    \
  ...
  case 2 * (int)op + 1
          ...
          CASE(Op::shl_i32) : r(d).i32 = r(x).i32 << imm;
          break;
          CASE(Op::sra_i32) : r(d).i32 = r(x).i32 >> imm;
          break;
          CASE(Op::shr_i32) : r(d).u32 = r(x).u32 >> imm;
          break;

          CASE(Op::shl_i16x2) : r(d).i16x2 = r(x).i16x2 << imm;
          break;
          CASE(Op::sra_i16x2) : r(d).i16x2 = r(x).i16x2 >> imm;
          break;
          CASE(Op::shr_i16x2) : r(d).u16x2 = r(x).u16x2 >> imm;
          break;
          ...
          break;
          CASE(Op::lt_f32) : r(d).i32 = r(x).f32 < r(y).f32;
          break;
          CASE(Op::lte_f32) : r(d).i32 = r(x).f32 <= r(y).f32;
          break;
          ...
          break;
          CASE(Op::lt_i32) : r(d).i32 = r(x).i32 < r(y).i32;
          break;
          CASE(Op::lte_i32) : r(d).i32 = r(x).i32 <= r(y).i32;
          break;
          ...
          break;
          CASE(Op::lt_i16x2) : r(d).i16x2 = r(x).i16x2 < r(y).i16x2;
          break;
          CASE(Op::lte_i16x2) : r(d).i16x2 = r(x).i16x2 <= r(y).i16x2;
          break;
          ...
          CASE(Op::extract) : r(d).u32 = (r(x).u32 >> imm) & r(y).u32;
          break;
          CASE(Op::pack) : r(d).u32 = r(x).u32 | (r(y).u32 << imm);
          break;
          ...
          CASE(Op::bytes) : {
            ...
            r(d).u32 = table[(imm >> 0) & 0xf] << 0 | table[(imm >> 4) & 0xf] << 8 |
                       table[(imm >> 8) & 0xf] << 16 | table[(imm >> 12) & 0xf] << 24;
          }
          break;

          CASE(Op::to_f32) : r(d).f32 = skvx::cast<float>(r(x).i32);
          break;
          CASE(Op::to_i32) : r(d).i32 = skvx::cast<int>(r(x).f32);
          break;
#undef CASE
      }
    }
...
#  if defined(__x86_64__)
  ...
  A::GP64 N = A::rdi, arg[] = {A::rsi, A::rdx, A::rcx, A::r8, A::r9};
...
#  elif defined(__aarch64__)
  A::X N = A::x0, arg[] = {A::x1, A::x2, A::x3, A::x4, A::x5, A::x6, A::x7};
...
#  endif
  ...
  SkTHashMap<int, LabelAndReg> splats, bytes_masks;
...
#  if defined(__x86_64__)
      ...
      case Op::store32:
        if (scalar) {
          a->vmovd(arg[imm], (A::Xmm)r[x]);
        } else {
          a->vmovups(arg[imm], r[x]);
        }
        break;
      ...
      case Op::load32:
        if (scalar) {
          a->vmovd((A::Xmm)dst(), arg[imm]);
        } else {
          a->vmovups(dst(), arg[imm]);
        }
        break;

      case Op::uniform8:
        a->movzbl(A::rax, arg[imm & 0xffff], imm >> 16);
        a->vmovd_direct((A::Xmm)dst(), A::rax);
        a->vbroadcastss(dst(), (A::Xmm)dst());
        break;

      case Op::uniform32: a->vbroadcastss(dst(), arg[imm & 0xffff], imm >> 16); break;

      case Op::splat:
        a->vbroadcastss(dst(), &splats.find(imm)->label);
        break;
      ...
      case Op::bytes: a->vpshufb(dst(), r[x], &bytes_masks.find(imm)->label); break;
...
#  elif defined(__aarch64__)
      ...
      case Op::splat:
        a->ldrq(dst(), &splats.find(imm)->label);
        break;
      ...
      case Op::bytes:
        if (try_hoisting) {
          a->tbl(dst(), r[x], bytes_masks.find(imm)->reg);
        } else {
          a->ldrq(tmp(), &bytes_masks.find(imm)->label);
          a->tbl(dst(), r[x], tmp());
        }
        break;
#  endif
...
#  endif
  ...
  struct CodeLoad {
    ...
    uint64_t vma /*???*/, code_addr, code_size, id;
      ...
      0 /*code load*/,
      (uint32_t)(sizeof(CodeLoad) + strlen(name) + 1 + size),
      timestamp_ns(),
