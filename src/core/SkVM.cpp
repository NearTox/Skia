...
Program Builder::done(const char* debug_name) {
  ...
  for (Val id = 0; id < (Val)fProgram.size(); id++) {
    ...
    if (inst.hoist) {
      if (inst.x != NA) {
        inst.hoist &= fProgram[inst.x].hoist;
      }
      if (inst.y != NA) {
        inst.hoist &= fProgram[inst.y].hoist;
      }
      if (inst.z != NA) {
        inst.hoist &= fProgram[inst.z].hoist;
      }
    }
    ...
    if (!inst.hoist) {
      ...
      if (inst.x != NA && fProgram[inst.x].hoist) {
        make_immortal(inst.x);
      }
      if (inst.y != NA && fProgram[inst.y].hoist) {
        make_immortal(inst.y);
      }
      if (inst.z != NA && fProgram[inst.z].hoist) {
        make_immortal(inst.z);
      }
    }
  }
...
static bool operator==(const Builder::Instruction& a, const Builder::Instruction& b) {
  return a.op == b.op && a.x == b.x && a.y == b.y && a.z == b.z && a.imm == b.imm &&
         a.death == b.death && a.hoist == b.hoist;
}
...
bool Program::jit(
    const std::vector<Builder::Instruction>& instructions, const bool hoist, Assembler* a) const {
  using A = Assembler;
...
#  endif
  ...
  auto emit = [&](Val id, bool scalar) {
    ...
    if (x != NA && instructions[x].death == id) {
      avail |= 1 << r[x];
    }
    if (y != NA && instructions[y].death == id) {
      avail |= 1 << r[y];
    }
    if (z != NA && instructions[z].death == id) {
      avail |= 1 << r[z];
    }
