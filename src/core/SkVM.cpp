...
static void notify_vtune(
    const char* name, void* addr, size_t len, const std::vector<skvm::LineTableEntry>& lines) {
...
static bool operator==(const Builder::Instruction& a, const Builder::Instruction& b) {
  return a.op == b.op && a.x == b.x && a.y == b.y && a.z == b.z && a.immy == b.immy &&
         a.immz == b.immz && a.death == b.death && a.can_hoist == b.can_hoist &&
...
size_t Builder::InstructionHash::operator()(const Instruction& inst) const {
  auto hash = [](auto v) { return std::hash<decltype(v)>{}(v); };
  return hash((uint8_t)inst.op) ^ hash(inst.x) ^ hash(inst.y) ^ hash(inst.z) ^ hash(inst.immy) ^
         hash(inst.immz) ^ hash(inst.death) ^ hash(inst.can_hoist) ^ hash(inst.used_in_loop);
}
...
I32 Builder::extract(I32 x, int bits, I32 z) {
  int X, Z;
  if (this->allImm(x.id, &X, z.id, &Z)) {
    return this->splat((unsigned(X) >> bits) & Z);
  }
...
#define CASE(op)                    \
  ...
  case 2 * (int)op + 1
          ...
          CASE(Op::assert_true) : SkASSERT(all(r(x).i32));
          break;
          ...
          CASE(Op::extract) : r(d).u32 = (r(x).u32 >> immy) & r(z).u32;
          break;
...
bool Program::hasJIT() const { return fJITBuf != nullptr; }
...
Program::Program(
    ...
    : fStrides(strides), fOriginalProgram(instructions) {
...
bool Program::jit(
    ...
    std::vector<LineTableEntry>* line_table, Assembler* a) const {
  using A = Assembler;
...
#  if defined(__x86_64__)
  ...
  A::GP64 N = A::rdi, scratch = A::rax, arg[] = {A::rsi, A::rdx, A::rcx, A::r8, A::r9};
...
#  elif defined(__aarch64__)
      ...
      case Op::extract:
        if (immy) {
          a->ushr4s(tmp(), r[x], immy);
          a->and16b(dst(), tmp(), r[z]);
        } else {
          a->and16b(dst(), r[x], r[z]);
        }
        break;
...
void Program::setupJIT(
  ...
  __builtin___clear_cache((char*)fJITBuf, (char*)fJITBuf + fJITSize);
...
#  endif
  ...
  static FILE* jitdump = [&] {
    ...
    void* marker = mmap(
        nullptr, sysconf(_SC_PAGESIZE), PROT_READ | PROT_EXEC, MAP_PRIVATE, fileno(f),
        /*offset=*/0);
...
#  endif
    ...
    struct Header {
      ...
      uint64_t timestamp_us, flags;
        ...
        0x4A695444, 1, sizeof(Header), elf_mach, 0, (uint32_t)getpid(), timestamp_ns() / 1000, 0,
    };
  ...
  struct CodeLoad {
    ...
    uint64_t vma /*???*/, code_addr, code_size, id;
      ...
      0 /*code load*/,
      (uint32_t)(sizeof(CodeLoad) + strlen(debug_name) + 1 + size),
      timestamp_ns(),

      (uint32_t)getpid(),
      (uint32_t)SkGetThreadID(),
      (uint64_t)fJITBuf,
      (uint64_t)fJITBuf,
      size,
      next_id++,
