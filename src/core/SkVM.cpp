...
static void write(SkWStream* o, Op op) { o->writeText(name(op)); }
...
static void write(SkWStream* o, TraceHookID h) { o->writeDecAsText(h.bits); }
...
static void write_one_instruction(Val id, const OptimizedInstruction& inst, SkWStream* o) {
  ...
  switch (op) {
    ...
    case Op::trace_var:
      write(o, op, TraceHookID{immA}, V{x}, V{y}, VarSlot{immB}, "=", V{z});
      break;
...
void Program::dump(SkWStream* o) const {
  ...
  for (Val i = 0; i < (Val)fImpl->instructions.size(); i++) {
    ...
    switch (op) {
      ...
      case Op::trace_line: write(o, op, TraceHookID{immA}, R{x}, R{y}, Line{immB}); break;
      case Op::trace_var:
        write(o, op, TraceHookID{immA}, R{x}, R{y}, VarSlot{immB}, "=", R{z});
        break;
      case Op::trace_enter: write(o, op, TraceHookID{immA}, R{x}, R{y}, FnIdx{immB}); break;
      case Op::trace_exit: write(o, op, TraceHookID{immA}, R{x}, R{y}, FnIdx{immB}); break;
      case Op::trace_scope: write(o, op, TraceHookID{immA}, R{x}, R{y}, Shift{immB}); break;
...
std::vector<Instruction> eliminate_dead_code(
    std::vector<Instruction> program, viz::Visualizer* visualizer) {
...
std::vector<OptimizedInstruction> finalize(
    const std::vector<Instruction> program, viz::Visualizer* visualizer) {
...
Program Builder::done(
    const char* debug_name, bool allow_jit, std::unique_ptr<viz::Visualizer> visualizer) const {
  ...
  return {optimized, std::move(visualizer), fStrides, fTraceHooks, debug_name, allow_jit};
}
...
bool Builder::mergeMasks(I32& mask, I32& traceMask) {
  if (this->isImm(mask.id, 0)) {
    return false;
  }
  if (this->isImm(traceMask.id, 0)) {
    return false;
  }
  if (this->isImm(mask.id, ~0)) {
    mask = traceMask;
...
void Builder::trace_line(int traceHookID, I32 mask, I32 traceMask, int line) {
  ...
  if (!this->mergeMasks(mask, traceMask)) {
    return;
  }
...
F32 Builder::approx_log2(F32 x) {
  ...
  F32 m = pun_to_F32(bit_or(bit_and(pun_to_I32(x), 0x007fffff), 0x3f000000));
...
F32 Builder::approx_powf(F32 x, F32 y) {
  ...
  if (this->isImm(x.id, 2.0f)) {
    return this->approx_pow2(y);
  }  // 2^y is pow2(y)
  if (this->isImm(y.id, 0.5f)) {
    return this->sqrt(x);
  }  // x^0.5 is sqrt(x)
  if (this->isImm(y.id, 1.0f)) {
    return x;
  }  // x^1 is x
  if (this->isImm(y.id, 2.0f)) {
    return x * x;
  }  // x^2 is x*x
...
static F32 approx_atan_unit(F32 x) {
  ...
  return poly(
      x, 0.14130025741326729f, -0.34312835980675116f, -0.016172900528248768f, 1.0037696976200385f,
...
I32 Builder::pack(I32 x, I32 y, int bits) { return bit_or(x, shl(y, bits)); }
...
void Builder::unpremul(F32* r, F32* g, F32* b, F32 a) {
  skvm::F32 invA = 1.0f / a, inf = pun_to_F32(splat(0x7f800000));
...
HSLA Builder::to_hsla(Color c) {
  F32 mx = max(max(c.r, c.g), c.b), mn = min(min(c.r, c.g), c.b), d = mx - mn, invd = 1.0f / d,
  ...
  F32 h = (1 / 6.0f) *
          select(
              ...
              select(
                  ...
                  select(mx == c.g, invd * (c.b - c.r) + 2.0f, invd * (c.r - c.g) + 4.0f)));

  F32 sum = mx + mn, l = sum * 0.5f,
...
static void clip_color(skvm::F32* r, skvm::F32* g, skvm::F32* b, skvm::F32 a) {
  F32 mn = min(*r, min(*g, *b)), mx = max(*r, max(*g, *b)), lu = luminance(*r, *g, *b);
...
Color Builder::blend(SkBlendMode mode, Color src, Color dst) {
  auto mma = [](skvm::F32 x, skvm::F32 y, skvm::F32 z, skvm::F32 w) { return x * y + z * w; };
  ...
  auto non_sep = [&](auto R, auto G, auto B) {
    return Color{
        R + mma(src.r, 1 - dst.a, dst.r, 1 - src.a), G + mma(src.g, 1 - dst.a, dst.g, 1 - src.a),
  ...
  switch (mode) {
    ...
    case SkBlendMode::kPlus: return apply_rgba([&](auto s, auto d) { return min(s + d, 1.0f); });

    case SkBlendMode::kModulate: return apply_rgba([&](auto s, auto d) { return s * d; });

    case SkBlendMode::kScreen:
      // (s+d)-(s*d) gave us trouble with our "r,g,b <= after blending" asserts.
      // It's kind of plausible that s + (d - sd) keeps more precision?
      return apply_rgba([&](auto s, auto d) { return s + (d - s * d); });

    case SkBlendMode::kDarken:
      return apply_rgb_srcover_a(
          [&](auto s, auto d) { return s + (d - max(s * dst.a, d * src.a)); });

    case SkBlendMode::kLighten:
      return apply_rgb_srcover_a(
          [&](auto s, auto d) { return s + (d - min(s * dst.a, d * src.a)); });

    case SkBlendMode::kDifference:
      return apply_rgb_srcover_a(
          [&](auto s, auto d) { return s + (d - two(min(s * dst.a, d * src.a))); });

    case SkBlendMode::kExclusion:
      return apply_rgb_srcover_a([&](auto s, auto d) { return s + (d - two(s * d)); });
    ...
    case SkBlendMode::kHardLight:
      return apply_rgb_srcover_a([&](auto s, auto d) {
        return mma(s, 1 - dst.a, d, 1 - src.a) +
               select(two(s) <= src.a, two(s * d), src.a * dst.a - two((dst.a - d) * (src.a - s)));
      });

    case SkBlendMode::kOverlay:
      return apply_rgb_srcover_a([&](auto s, auto d) {
        return mma(s, 1 - dst.a, d, 1 - src.a) +
               select(two(d) <= dst.a, two(s * d), src.a * dst.a - two((dst.a - d) * (src.a - s)));
      });

    case SkBlendMode::kMultiply:
      return apply_rgba([&](auto s, auto d) { return mma(s, 1 - dst.a, d, 1 - src.a) + s * d; });

    case SkBlendMode::kSoftLight:
      return apply_rgb_srcover_a([&](auto s, auto d) {
        auto m = select(dst.a > 0.0f, d / dst.a, 0.0f), s2 = two(s), m4 = 4 * m;
        ...
        auto darkSrc = d * ((s2 - src.a) * (1 - m) + src.a),
            ...
            liteDst = sqrt(m) - m,
             // Used in 2 or 3?
            liteSrc = dst.a * (s2 - src.a) * select(4 * d <= dst.a, darkDst, liteDst) + d * src.a;
        return s * (1 - dst.a) + d * (1 - src.a) + select(s2 <= src.a, darkSrc, liteSrc);
      });

    case SkBlendMode::kHue: {
      skvm::F32 R = src.r * src.a, G = src.g * src.a, B = src.b * src.a;
    ...
    case SkBlendMode::kSaturation: {
      skvm::F32 R = dst.r * src.a, G = dst.g * src.a, B = dst.b * src.a;
    ...
    case SkBlendMode::kColor: {
      skvm::F32 R = src.r * dst.a, G = src.g * dst.a, B = src.b * dst.a;
...
void Assembler::op(uint32_t hi, V m, uint32_t lo, V n, V d) {
  this->word(
      (hi & mask(11)) << 21 | (m & mask(5)) << 16 | (lo & mask(6)) << 10 | (n & mask(5)) << 5 |
      (d & mask(5)) << 0);
}
void Assembler::op(uint32_t op22, V n, V d, int imm) {
  this->word(
      (op22 & mask(22)) << 10 | imm  // size and location depends on the instruction
      | (n & mask(5)) << 5 | (d & mask(5)) << 0);
}
...
void Assembler::brk(int imm16) { this->op(0b11010100'001'00000000000, (imm16 & mask(16)) << 5); }
...
void Assembler::add(X d, X n, X m, Shift shift, int imm6) {
  ...
  int imm =
      (imm6 & mask(6)) << 0 | (m & mask(5)) << 6 | (0 & mask(1)) << 11 | (shift & mask(2)) << 12;
...
void Assembler::dup4s(V dst, X src) { this->op(0b0'1'0'01110000'00100'0'0001'1, src, dst); }

void Assembler::ld1r4s(V dst, X src) { this->op(0b0'1'0011010'1'0'00000'110'0'10, src, dst); }
  ...
  if (fCode) {
    ...
    if (l->kind == Label::ARMDisp19) {
      for (int ref : l->references) {
        ...
        inst = ((disp << 5) & (mask(19) << 5)) | ((inst) & ~(mask(19) << 5));
...
#if SKVM_JIT_STATS
  static std::atomic<int64_t> calls{0}, jits{0}, pixels{0}, fast{0};
  pixels += n;
  if (0 == calls++) {
    atexit([] {
      int64_t num = jits.load(), den = calls.load();
...
#  endif
  ...
  SkOpts::interpret_skvm(
      fImpl->instructions.data(), (int)fImpl->instructions.size(), this->nregs(), this->loop(),
      fImpl->strides.data(), fImpl->traceHooks.data(), fImpl->traceHooks.size(), this->nargs(), n,
      args);
}

#if defined(SKVM_LLVM)
// -- SKVM_LLVM --------------------------------------------------------------------------------
void Program::setupLLVM(
  ...
  llvm::FunctionType* fn_type =
      llvm::FunctionType::get(llvm::Type::getVoidTy(*ctx), arg_types, /*vararg?=*/false);
  llvm::Function* fn =
  ...
  auto emit = [&](size_t i, bool scalar, IRBuilder* b) {
    ...
    switch (llvm::Type* t = nullptr; op) {
      default: SkDebugf("can't llvm %s (%d)\n", name(op), op); return false;
      ...
      case Op::index:
        if (I32->isVectorTy()) {
          ...
          vals[i] = b->CreateSub(b->CreateVectorSplat(K, n), llvm::ConstantVector::get(iota));
        } else {
          vals[i] = n;
        }
        break;
      ...
      load : {
        ...
        vals[i] = b->CreateZExt(b->CreateAlignedLoad(t, ptr, llvm::MaybeAlign{1}), I32);
      } break;
      ...
      case Op::uniform32: {
        llvm::Value* ptr = b->CreateBitCast(
            b->CreateConstInBoundsGEP1_32(i8, args[immA], immB), i32->getPointerTo());
        llvm::Value* val = b->CreateZExt(b->CreateAlignedLoad(i32, ptr, llvm::MaybeAlign{1}), i32);
      ...
      gather : {
        ...
        if (ptr->getType()->isVectorTy()) {
          gathered = b->CreateMaskedGather(vt, ptr, llvm::Align{1});
        } else {
      ...
      case Op::fma_f32:
        vals[i] = I(
            b->CreateIntrinsic(llvm::Intrinsic::fma, {F32}, {F(vals[x]), F(vals[y]), F(vals[z])}));
        break;

      case Op::fms_f32:
        vals[i] = I(b->CreateIntrinsic(
            llvm::Intrinsic::fma, {F32}, {F(vals[x]), F(vals[y]), b->CreateFNeg(F(vals[z]))}));
        break;
      ...
      case Op::max_f32:
        vals[i] =
            I(b->CreateSelect(b->CreateFCmpOLT(F(vals[x]), F(vals[y])), F(vals[y]), F(vals[x])));
        break;
      case Op::min_f32:
        vals[i] =
            I(b->CreateSelect(b->CreateFCmpOLT(F(vals[y]), F(vals[x])), F(vals[y]), F(vals[x])));
        break;
...
#  if 1 && defined(SK_CPU_X86)
        ...
        if (scalar) {
          ...
          llvm::Value* v = llvm::UndefValue::get(llvm::VectorType::get(f32, 4, false));
...
#  endif
  ...
  // loopK:  ... insts on K x T vectors; N -= K, args += K*stride; goto testK;
  {
    ...
    for (size_t i = 0; i < fImpl->strides.size(); i++) {
      llvm::Value* arg_next =
          b.CreateConstInBoundsGEP1_32(llvm::Type::getInt8Ty(*ctx), args[i], K * fImpl->strides[i]);
  ...
  // loop1:  ... insts on scalars; N -= 1, args += stride; goto test1;
  {
    ...
    for (size_t i = 0; i < fImpl->strides.size(); i++) {
      llvm::Value* arg_next =
          b.CreateConstInBoundsGEP1_32(llvm::Type::getInt8Ty(*ctx), args[i], fImpl->strides[i]);
...
#  else
    fImpl->llvm_compiling = std::async(
        std::launch::async,
        [dst = &fImpl->jit_entry, ee = fImpl->llvm_ee.get(), name = std::string(debug_name)] {
          // std::atomic<void*>*    dst;
...
Program::Program(
    const std::vector<OptimizedInstruction>& instructions,
    std::unique_ptr<viz::Visualizer> visualizer, const std::vector<int>& strides,
    const std::vector<TraceHook*>& traceHooks, const char* debug_name, bool allow_jit)
    : Program() {
...
bool Program::jit(
  ...
  //   - id:   holding Val id
  constexpr Val RES = NA - 1, TMP = RES - 1;
  ...
  *registers_used = 0;  // We'll update this as we go.
  ...
  auto emit = [&](Val id, bool scalar) {
    ...
    auto dst = [&](Val hint1 = NA, Val hint2 = NA) -> Reg {
      if (hint1 != NA && try_alias(hint1)) {
        return r(id);
      }
      if (hint2 != NA && try_alias(hint2)) {
        return r(id);
      }
      return r(id);
    };
...
#  if defined(__x86_64__) || defined(_M_X64)
    ...
    auto in_reg = [&](Val v) -> bool { return find_existing_reg(v) != NA; };
#  endif
