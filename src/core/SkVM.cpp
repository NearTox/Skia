...
static void write(SkWStream* o, Shift s) { o->writeDecAsText(s.bits); }
...
static void write(SkWStream* o, Hex h) { o->writeHexAsText(h.bits); }
...
static void dump_builder_program(const std::vector<Builder::Instruction>& program, SkWStream* o) {
  for (Val id = 0; id < (Val)program.size(); id++) {
    const Builder::Instruction& inst = program[id];
    Op op = inst.op;
    Val x = inst.x, y = inst.y, z = inst.z;
    int immy = inst.immy, immz = inst.immz;
    write(o, !inst.can_hoist ? "  " : inst.used_in_loop ? "↑ " : "↟ ");
    switch (op) {
...
Program Builder::done(const char* debug_name) {
  ...
  for (Val id = 0; id < (Val)fProgram.size(); id++) {
    ...
    if (inst.x != NA) {
      fProgram[inst.x].death = id;
    }
    if (inst.y != NA) {
      fProgram[inst.y].death = id;
    }
    if (inst.z != NA) {
      fProgram[inst.z].death = id;
    }
  }
  ...
  for (Val id = 0; id < (Val)fProgram.size(); id++) {
    ...
    if (inst.can_hoist) {
      if (inst.x != NA) {
        inst.can_hoist &= fProgram[inst.x].can_hoist;
      }
      if (inst.y != NA) {
        inst.can_hoist &= fProgram[inst.y].can_hoist;
      }
      if (inst.z != NA) {
        inst.can_hoist &= fProgram[inst.z].can_hoist;
      }
    }
    ...
    if (!inst.can_hoist /*i.e. we're in the loop, so the arguments are used_in_loop*/) {
      if (inst.x != NA) {
        fProgram[inst.x].used_in_loop = true;
      }
      if (inst.y != NA) {
        fProgram[inst.y].used_in_loop = true;
      }
      if (inst.z != NA) {
        fProgram[inst.z].used_in_loop = true;
      }
    }
  }
...
F32 Builder::add(F32 x, F32 y) {
  ...
  int imm;
  if (this->allImm(y.id, &imm)) {
    return {this->push(Op::add_f32_imm, x.id, NA, NA, imm)};
  }
  if (this->allImm(x.id, &imm)) {
    return {this->push(Op::add_f32_imm, y.id, NA, NA, imm)};
  }
#endif
...
F32 Builder::sub(F32 x, F32 y) {
  ...
  int imm;
  if (this->allImm(y.id, &imm)) {
    return {this->push(Op::sub_f32_imm, x.id, NA, NA, imm)};
  }
#endif
...
F32 Builder::mul(F32 x, F32 y) {
  ...
  int imm;
  if (this->allImm(y.id, &imm)) {
    return {this->push(Op::mul_f32_imm, x.id, NA, NA, imm)};
  }
  if (this->allImm(x.id, &imm)) {
    return {this->push(Op::mul_f32_imm, y.id, NA, NA, imm)};
  }
#endif
...
F32 Builder::mad(F32 x, F32 y, F32 z) {
  float X, Y, Z;
  if (this->allImm(x.id, &X, y.id, &Y, z.id, &Z)) {
...
F32 Builder::min(F32 x, F32 y) {
  ...
  int imm;
  if (this->allImm(y.id, &imm)) {
    return {this->push(Op::min_f32_imm, x.id, NA, NA, imm)};
  }
  if (this->allImm(x.id, &imm)) {
    return {this->push(Op::min_f32_imm, y.id, NA, NA, imm)};
  }
#endif
...
F32 Builder::max(F32 x, F32 y) {
  ...
  int imm;
  if (this->allImm(y.id, &imm)) {
    return {this->push(Op::max_f32_imm, x.id, NA, NA, imm)};
  }
  if (this->allImm(x.id, &imm)) {
    return {this->push(Op::max_f32_imm, y.id, NA, NA, imm)};
  }
#endif
...
I32 Builder::bit_and(I32 x, I32 y) {
  ...
  int imm;
  if (this->allImm(y.id, &imm)) {
    return {this->push(Op::bit_and_imm, x.id, NA, NA, imm)};
  }
  if (this->allImm(x.id, &imm)) {
    return {this->push(Op::bit_and_imm, y.id, NA, NA, imm)};
  }
#endif
...
I32 Builder::bit_or(I32 x, I32 y) {
  ...
  int imm;
  if (this->allImm(y.id, &imm)) {
    return {this->push(Op::bit_or_imm, x.id, NA, NA, imm)};
  }
  if (this->allImm(x.id, &imm)) {
    return {this->push(Op::bit_or_imm, y.id, NA, NA, imm)};
  }
#endif
...
I32 Builder::bit_xor(I32 x, I32 y) {
  ...
  int imm;
  if (this->allImm(y.id, &imm)) {
    return {this->push(Op::bit_xor_imm, x.id, NA, NA, imm)};
  }
  if (this->allImm(x.id, &imm)) {
    return {this->push(Op::bit_xor_imm, y.id, NA, NA, imm)};
  }
#endif
...
I32 Builder::bit_clear(I32 x, I32 y) {
  ...
  int imm;
  if (this->allImm(y.id, &imm)) {
    return this->bit_and(x, this->splat(~imm));
  }
#endif
...
I32 Builder::extract(I32 x, int bits, I32 z) {
  int Z;
  if (this->allImm(z.id, &Z) && (~0u >> bits) == (unsigned)Z) {
    return this->shr(x, bits);
  }
...
static VEX vex(
  ...
  if (X == 0 && B == 0 && WE == 0 && map == 0b00001) {
    ...
    vex.len = 2;
    vex.bytes[0] = 0xc5;
    vex.bytes[1] = (pp & 3) << 0 | (L & 1) << 2 | (~vvvv & 15) << 3 | (~(int)R & 1) << 7;
  } else {
    // We could use this 3-byte VEX prefix all the time if we like.
    vex.len = 3;
    vex.bytes[0] = 0xc4;
    vex.bytes[1] = (map & 31) << 0 | (~(int)B & 1) << 5 | (~(int)X & 1) << 6 | (~(int)R & 1) << 7;
    vex.bytes[2] = (pp & 3) << 0 | (L & 1) << 2 | (~vvvv & 15) << 3 | (WE & 1) << 7;
  }
  return vex;
}
...
void Assembler::op(int prefix, int map, int opcode, Ymm dst, Ymm x, Ymm y, bool W /*=false*/) {
  VEX v = vex(W, dst >> 3, 0, y >> 3, map, x, 1 /*ymm, not xmm*/, prefix);
...
void Assembler::vpblendvb(Ymm dst, Ymm x, Ymm y, Ymm z) {
  int prefix = 0x66, map = 0x3a0f, opcode = 0x4c;
  VEX v = vex(0, dst >> 3, 0, y >> 3, map, x, /*ymm?*/ 1, prefix);
  this->bytes(v.bytes, v.len);
  this->byte(opcode);
  this->byte(mod_rm(Mod::Direct, dst & 7, y & 7));
  this->byte(z << 4);
...
Assembler::Label Assembler::here() { return {(int)this->size(), Label::NotYetSet, {}}; }
...
void Assembler::op(int prefix, int map, int opcode, Ymm dst, Ymm x, Label* l) {
  ...
  VEX v = vex(0, dst >> 3, 0, rip >> 3, map, x, /*ymm?*/ 1, prefix);
  this->bytes(v.bytes, v.len);
  this->byte(opcode);
...
void Assembler::vbroadcastss(Ymm dst, GP64 ptr, int off) {
  int prefix = 0x66, map = 0x380f, opcode = 0x18;
  VEX v = vex(0, dst >> 3, 0, ptr >> 3, map, 0, /*ymm?*/ 1, prefix);
  this->bytes(v.bytes, v.len);
  this->byte(opcode);
...
void Assembler::load_store(int prefix, int map, int opcode, Ymm ymm, GP64 ptr) {
  VEX v = vex(0, ymm >> 3, 0, ptr >> 3, map, 0, /*ymm?*/ 1, prefix);
  this->bytes(v.bytes, v.len);
  this->byte(opcode);
...
void Assembler::vmovups(GP64 dst, Xmm src) {
  ...
  int prefix = 0, map = 0x0f, opcode = 0x11;
  VEX v = vex(0, src >> 3, 0, dst >> 3, map, 0, /*ymm?*/ 0, prefix);
  this->bytes(v.bytes, v.len);
  this->byte(opcode);
...
void Assembler::vmovq(GP64 dst, Xmm src) {
  int prefix = 0x66, map = 0x0f, opcode = 0xd6;
  VEX v = vex(0, src >> 3, 0, dst >> 3, map, 0, /*ymm?*/ 0, prefix);
  this->bytes(v.bytes, v.len);
  this->byte(opcode);
  this->byte(mod_rm(Mod::Indirect, src & 7, dst & 7));
...
void Assembler::vmovd(GP64 dst, Xmm src) {
  int prefix = 0x66, map = 0x0f, opcode = 0x7e;
  VEX v = vex(0, src >> 3, 0, dst >> 3, map, 0, /*ymm?*/ 0, prefix);
  this->bytes(v.bytes, v.len);
  this->byte(opcode);
  this->byte(mod_rm(Mod::Indirect, src & 7, dst & 7));
...
void Assembler::vmovd(Xmm dst, GP64 src) {
  int prefix = 0x66, map = 0x0f, opcode = 0x6e;
  VEX v = vex(0, dst >> 3, 0, src >> 3, map, 0, /*ymm?*/ 0, prefix);
  this->bytes(v.bytes, v.len);
  this->byte(opcode);
...
void Assembler::vmovd(Xmm dst, Scale scale, GP64 index, GP64 base) {
  int prefix = 0x66, map = 0x0f, opcode = 0x6e;
  VEX v = vex(0, dst >> 3, index >> 3, base >> 3, map, 0, /*ymm?*/ 0, prefix);
  this->bytes(v.bytes, v.len);
  this->byte(opcode);
...
void Assembler::vpinsrw(Xmm dst, Xmm src, GP64 ptr, int imm) {
  int prefix = 0x66, map = 0x0f, opcode = 0xc4;
  VEX v = vex(0, dst >> 3, 0, ptr >> 3, map, src, /*ymm?*/ 0, prefix);
  this->bytes(v.bytes, v.len);
  this->byte(opcode);
...
void Assembler::vpinsrb(Xmm dst, Xmm src, GP64 ptr, int imm) {
  int prefix = 0x66, map = 0x3a0f, opcode = 0x20;
  VEX v = vex(0, dst >> 3, 0, ptr >> 3, map, src, /*ymm?*/ 0, prefix);
  this->bytes(v.bytes, v.len);
  this->byte(opcode);
  this->byte(mod_rm(Mod::Indirect, dst & 7, ptr & 7));
...
void Assembler::vpextrw(GP64 ptr, Xmm src, int imm) {
  int prefix = 0x66, map = 0x3a0f, opcode = 0x15;

  VEX v = vex(0, src >> 3, 0, ptr >> 3, map, 0, /*ymm?*/ 0, prefix);
  this->bytes(v.bytes, v.len);
  this->byte(opcode);
...
void Assembler::vpextrb(GP64 ptr, Xmm src, int imm) {
  int prefix = 0x66, map = 0x3a0f, opcode = 0x14;
...
void Assembler::vgatherdps(Ymm dst, Scale scale, Ymm ix, GP64 base, Ymm mask) {
  ...
  int prefix = 0x66, map = 0x380f, opcode = 0x92;
...
void Assembler::op(uint32_t op22, int imm, V n, V d) {
  this->word(
      (op22 & 22_mask) << 10 | imm << 16  // imm is embedded inside op, bit size depends on op
...
void Assembler::brk(int imm16) {
  this->word(0b11010100'001'0000000000000000'000'00 | (imm16 & 16_mask) << 5);
}

void Assembler::ret(X n) { this->word(0b1101011'0'0'10'11111'0000'0'0 << 10 | (n & 5_mask) << 5); }

void Assembler::add(X d, X n, int imm12) {
  this->word(
      0b1'0'0'10001'00 << 22 | (imm12 & 12_mask) << 10 | (n & 5_mask) << 5 | (d & 5_mask) << 0);
}
void Assembler::sub(X d, X n, int imm12) {
  this->word(
      0b1'1'0'10001'00 << 22 | (imm12 & 12_mask) << 10 | (n & 5_mask) << 5 | (d & 5_mask) << 0);
}
void Assembler::subs(X d, X n, int imm12) {
  this->word(
      0b1'1'1'10001'00 << 22 | (imm12 & 12_mask) << 10 | (n & 5_mask) << 5 | (d & 5_mask) << 0);
}

void Assembler::b(Condition cond, Label* l) {
  const int imm19 = this->disp19(l);
  this->word(0b0101010'0 << 24 | (imm19 & 19_mask) << 5 | ((int)cond & 4_mask) << 0);
}
void Assembler::cbz(X t, Label* l) {
  const int imm19 = this->disp19(l);
  this->word(0b1'011010'0 << 24 | (imm19 & 19_mask) << 5 | (t & 5_mask) << 0);
}
void Assembler::cbnz(X t, Label* l) {
  const int imm19 = this->disp19(l);
  this->word(0b1'011010'1 << 24 | (imm19 & 19_mask) << 5 | (t & 5_mask) << 0);
}
...
void Assembler::fmovs(X dst, V src) {
  this->word(0b0'0'0'11110'00'1'00'110'000000 << 10 | (src & 5_mask) << 5 | (dst & 5_mask) << 0);
}

void Assembler::ldrq(V dst, Label* l) {
  const int imm19 = this->disp19(l);
  this->word(0b10'011'1'00 << 24 | (imm19 & 19_mask) << 5 | (dst & 5_mask) << 0);
}
...
void Program::interpret(int n, void* args[]) const {
  ...
  if (fRegs > (int)SK_ARRAY_COUNT(few_regs)) {
    ...
    addr += alignof(Slot) - (addr & (alignof(Slot) - 1));
...
#define STRIDE_K(op) case 2 * (int)op + 1
          STRIDE_1(Op::store8) : memcpy(arg(immy), &r(x).i32, 1);
          break;
          STRIDE_1(Op::store16) : memcpy(arg(immy), &r(x).i32, 2);
          break;
          STRIDE_1(Op::store32) : memcpy(arg(immy), &r(x).i32, 4);
          break;

          STRIDE_K(Op::store8) : skvx::cast<uint8_t>(r(x).i32).store(arg(immy));
          break;
          STRIDE_K(Op::store16) : skvx::cast<uint16_t>(r(x).i32).store(arg(immy));
          break;
          STRIDE_K(Op::store32) : (r(x).i32).store(arg(immy));
          break;

          STRIDE_1(Op::load8) : r(d).i32 = 0;
          memcpy(&r(d).i32, arg(immy), 1);
          break;
          STRIDE_1(Op::load16) : r(d).i32 = 0;
          memcpy(&r(d).i32, arg(immy), 2);
          break;
          STRIDE_1(Op::load32) : r(d).i32 = 0;
          memcpy(&r(d).i32, arg(immy), 4);
          break;

          STRIDE_K(Op::load8) : r(d).i32 = skvx::cast<int>(U8 ::Load(arg(immy)));
          break;
          STRIDE_K(Op::load16) : r(d).i32 = skvx::cast<int>(U16::Load(arg(immy)));
          break;
          STRIDE_K(Op::load32) : r(d).i32 = I32::Load(arg(immy));
          break;
          ...
          STRIDE_1(Op::gather8) : for (int i = 0; i < K; i++) {
            auto ptr = *(const uint8_t**)((const uint8_t*)arg(immy) + immz);
            r(d).i32[i] = (i == 0) ? ptr[r(x).i32[i]] : 0;
          }
          break;
          STRIDE_1(Op::gather16) : for (int i = 0; i < K; i++) {
            auto ptr = *(const uint16_t**)((const uint8_t*)arg(immy) + immz);
            r(d).i32[i] = (i == 0) ? ptr[r(x).i32[i]] : 0;
          }
          break;
          STRIDE_1(Op::gather32) : for (int i = 0; i < K; i++) {
            auto ptr = *(const int**)((const uint8_t*)arg(immy) + immz);
            r(d).i32[i] = (i == 0) ? ptr[r(x).i32[i]] : 0;
          }
          break;

          STRIDE_K(Op::gather8) : for (int i = 0; i < K; i++) {
            auto ptr = *(const uint8_t**)((const uint8_t*)arg(immy) + immz);
            r(d).i32[i] = ptr[r(x).i32[i]];
          }
          break;
          STRIDE_K(Op::gather16) : for (int i = 0; i < K; i++) {
            auto ptr = *(const uint16_t**)((const uint8_t*)arg(immy) + immz);
            r(d).i32[i] = ptr[r(x).i32[i]];
          }
          break;
          STRIDE_K(Op::gather32) : for (int i = 0; i < K; i++) {
            auto ptr = *(const int**)((const uint8_t*)arg(immy) + immz);
            r(d).i32[i] = ptr[r(x).i32[i]];
          }
          break;
...
#define CASE(op)                    \
  ...
  case 2 * (int)op + 1
          ...
          CASE(Op::uniform8) : r(d).i32 = *(const uint8_t*)((const char*)arg(immy) + immz);
          break;
          CASE(Op::uniform16) : r(d).i32 = *(const uint16_t*)((const char*)arg(immy) + immz);
          break;
          CASE(Op::uniform32) : r(d).i32 = *(const int*)((const char*)arg(immy) + immz);
          break;

          CASE(Op::splat) : r(d).i32 = immy;
          break;

          CASE(Op::add_f32) : r(d).f32 = r(x).f32 + r(y).f32;
          break;
          CASE(Op::sub_f32) : r(d).f32 = r(x).f32 - r(y).f32;
          break;
          CASE(Op::mul_f32) : r(d).f32 = r(x).f32 * r(y).f32;
          break;
          CASE(Op::div_f32) : r(d).f32 = r(x).f32 / r(y).f32;
          break;
          CASE(Op::min_f32) : r(d).f32 = min(r(x).f32, r(y).f32);
          break;
          CASE(Op::max_f32) : r(d).f32 = max(r(x).f32, r(y).f32);
          break;
          ...
          CASE(Op::mad_f32) : r(d).f32 = r(x).f32 * r(y).f32 + r(z).f32;
          break;

          CASE(Op::sqrt_f32) : r(d).f32 = sqrt(r(x).f32);
          break;

          CASE(Op::add_i32) : r(d).i32 = r(x).i32 + r(y).i32;
          break;
          CASE(Op::sub_i32) : r(d).i32 = r(x).i32 - r(y).i32;
          break;
          CASE(Op::mul_i32) : r(d).i32 = r(x).i32 * r(y).i32;
          break;

          CASE(Op::add_i16x2) : r(d).i16x2 = r(x).i16x2 + r(y).i16x2;
          break;
          CASE(Op::sub_i16x2) : r(d).i16x2 = r(x).i16x2 - r(y).i16x2;
          break;
          CASE(Op::mul_i16x2) : r(d).i16x2 = r(x).i16x2 * r(y).i16x2;
          break;

          CASE(Op::shl_i32) : r(d).i32 = r(x).i32 << immy;
          break;
          CASE(Op::sra_i32) : r(d).i32 = r(x).i32 >> immy;
          break;
          CASE(Op::shr_i32) : r(d).u32 = r(x).u32 >> immy;
          break;

          CASE(Op::shl_i16x2) : r(d).i16x2 = r(x).i16x2 << immy;
          break;
          CASE(Op::sra_i16x2) : r(d).i16x2 = r(x).i16x2 >> immy;
          break;
          CASE(Op::shr_i16x2) : r(d).u16x2 = r(x).u16x2 >> immy;
          break;

          CASE(Op::eq_f32) : r(d).i32 = r(x).f32 == r(y).f32;
          break;
          CASE(Op::neq_f32) : r(d).i32 = r(x).f32 != r(y).f32;
          break;
          CASE(Op::gt_f32) : r(d).i32 = r(x).f32 > r(y).f32;
          break;
          CASE(Op::gte_f32) : r(d).i32 = r(x).f32 >= r(y).f32;
          break;

          CASE(Op::eq_i32) : r(d).i32 = r(x).i32 == r(y).i32;
          break;
          CASE(Op::neq_i32) : r(d).i32 = r(x).i32 != r(y).i32;
          break;
          CASE(Op::gt_i32) : r(d).i32 = r(x).i32 > r(y).i32;
          break;
          CASE(Op::gte_i32) : r(d).i32 = r(x).i32 >= r(y).i32;
          break;

          CASE(Op::eq_i16x2) : r(d).i16x2 = r(x).i16x2 == r(y).i16x2;
          break;
          CASE(Op::neq_i16x2) : r(d).i16x2 = r(x).i16x2 != r(y).i16x2;
          break;
          CASE(Op::gt_i16x2) : r(d).i16x2 = r(x).i16x2 > r(y).i16x2;
          break;
          CASE(Op::gte_i16x2) : r(d).i16x2 = r(x).i16x2 >= r(y).i16x2;
          break;

          CASE(Op::bit_and) : r(d).i32 = r(x).i32 & r(y).i32;
          break;
          CASE(Op::bit_or) : r(d).i32 = r(x).i32 | r(y).i32;
          break;
          CASE(Op::bit_xor) : r(d).i32 = r(x).i32 ^ r(y).i32;
          break;
          CASE(Op::bit_clear) : r(d).i32 = r(x).i32 & ~r(y).i32;
          break;

          CASE(Op::bit_and_imm) : r(d).i32 = r(x).i32 & immy;
          break;
          CASE(Op::bit_or_imm) : r(d).i32 = r(x).i32 | immy;
          break;
          CASE(Op::bit_xor_imm) : r(d).i32 = r(x).i32 ^ immy;
          break;

          CASE(Op::select) : r(d).i32 = skvx::if_then_else(r(x).i32, r(y).i32, r(z).i32);
          break;

          CASE(Op::pack) : r(d).u32 = r(x).u32 | (r(y).u32 << immz);
          break;
          ...
          CASE(Op::bytes) : {
            ...
            r(d).u32 = table[(immy >> 0) & 0xf] << 0 | table[(immy >> 4) & 0xf] << 8 |
                       table[(immy >> 8) & 0xf] << 16 | table[(immy >> 12) & 0xf] << 24;
          }
          break;

          CASE(Op::floor) : r(d).f32 = skvx::floor(r(x).f32);
          break;
          CASE(Op::to_f32) : r(d).f32 = skvx::cast<float>(r(x).i32);
          break;
          CASE(Op::trunc) : r(d).i32 = skvx::cast<int>(r(x).f32);
          break;
          CASE(Op::round) : r(d).i32 = skvx::cast<int>(r(x).f32 + 0.5f);
          break;
#undef CASE
      }
    }
...
bool Program::hasJIT() const { return fJITEntry != nullptr; }
...
static void bytes_control(int imm, int mask[4]) {
  ...
  for (int i = 0; i < 4; i++) {
    mask[i] =
        (int)control[0] << 0 | (int)control[1] << 8 | (int)control[2] << 16 | (int)control[3] << 24;
...
bool Program::jit(
    const std::vector<Builder::Instruction>& instructions, const bool try_hoisting,
    Assembler* a) const {
  using A = Assembler;
...
#  if defined(__x86_64__)
  ...
  A::GP64 N = A::rdi, scratch = A::rax, scratch2 = A::r11,
...
#  elif defined(__aarch64__)
  A::X N = A::x0, scratch = A::x8, arg[] = {A::x1, A::x2, A::x3, A::x4, A::x5, A::x6, A::x7};
...
#  endif
  ...
  auto emit = [&](Val id, bool scalar) {
    ...
    bool tmp_is_set = false, dst_is_set = false;
...
#  if defined(__x86_64__)
      ...
      case Op::store8:
        if (scalar) {
          a->vpextrb(arg[immy], (A::Xmm)r[x], 0);
        } else {
      ...
      case Op::store16:
        if (scalar) {
          a->vpextrw(arg[immy], (A::Xmm)r[x], 0);
        } else {
          a->vpackusdw(tmp(), r[x], r[x]);
          a->vpermq(tmp(), tmp(), 0xd8);
      ...
      case Op::store32:
        if (scalar) {
          a->vmovd(arg[immy], (A::Xmm)r[x]);
        } else {
          a->vmovups(arg[immy], r[x]);
        }
        break;
      ...
      case Op::load32:
        if (scalar) {
          a->vmovd((A::Xmm)dst(), arg[immy]);
        } else {
          a->vmovups(dst(), arg[immy]);
        }
        break;

      case Op::gather32:
        if (scalar) {
          auto base = scratch, index = scratch2;
      ...
      case Op::uniform8:
        a->movzbl(scratch, arg[immy], immz);
        a->vmovd_direct((A::Xmm)dst(), scratch);
        a->vbroadcastss(dst(), (A::Xmm)dst());
        break;

      case Op::uniform32: a->vbroadcastss(dst(), arg[immy], immz); break;

      case Op::index:
        a->vmovd_direct((A::Xmm)tmp(), N);
      ...
      case Op::splat:
        if (immy) {
          a->vbroadcastss(dst(), &constants[immy].label);
        } else {
          a->vpxor(dst(), dst(), dst());
        }
        break;
      ...
      case Op::mad_f32:
        if (avail & (1 << r[x])) {
          set_dst(r[x]);
          a->vfmadd132ps(r[x], r[z], r[y]);
        } else if (avail & (1 << r[y])) {
          set_dst(r[y]);
          a->vfmadd213ps(r[y], r[x], r[z]);
        } else if (avail & (1 << r[z])) {
          set_dst(r[z]);
          a->vfmadd231ps(r[z], r[x], r[y]);
        } else {
          SkASSERT(dst() == tmp());
          a->vmovdqa(dst(), r[x]);
...
#  elif defined(__aarch64__)
      ...
      case Op::splat:
        if (immy) {
          a->ldrq(dst(), &constants[immy].label);
        } else {
          a->eor16b(dst(), dst(), dst());
        }
        break;
      ...
      case Op::mad_f32:  // fmla4s is z += x*y
        if (avail & (1 << r[z])) {
          ...
          a->orr16b(tmp(), r[z], r[z]);
          a->fmla4s(tmp(), r[x], r[y]);
          if (dst() != tmp()) {
            a->orr16b(dst(), tmp(), tmp());
          }
        }
        break;
      ...
      case Op::bytes:
        if (try_hoisting) {
          a->tbl(dst(), r[x], bytes_masks.find(immy)->reg);
        } else {
          a->ldrq(tmp(), &bytes_masks.find(immy)->label);
          a->tbl(dst(), r[x], tmp());
        }
        break;
#  endif
...
void Program::setupJIT(
    const std::vector<Builder::Instruction>& instructions, const char* debug_name) {
  ...
  __builtin___clear_cache((char*)fJITEntry, (char*)fJITEntry + fJITSize);
