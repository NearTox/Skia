...
struct SkScalerContextRec {
 ...
 public:
  ...
  SkString dump() const {
    ...
    msg.appendf("Rec\n");
    msg.appendf(
        "  textsize %g prescale %g preskew %g post [%g %g %g %g]\n", fTextSize, fPreScaleX,
        fPreSkewX, fPost2x2[0][0], fPost2x2[0][1], fPost2x2[1][0], fPost2x2[1][1]);
    msg.appendf(
  ...
  bool computeMatrices(
      PreMatrixScale preMatrixScale, SkVector* scale, SkMatrix* remaining,
...
class SkScalerContext {
 public:
  ...
  static void MakeRecAndEffects(
      ...
      SkScalerContextRec* rec, SkScalerContextEffects* effects);

  // If we are creating rec and effects from a font only, then there is no device around either.
  static void MakeRecAndEffectsFromFont(
  ...
  static bool CheckBufferSizeForRec(
  ...
  static SkDescriptor* CreateDescriptorAndEffectsUsingPaint(
      ...
      const SkMatrix& deviceMatrix, SkAutoDescriptor* ad, SkScalerContextEffects* effects);
