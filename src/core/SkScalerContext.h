...
public:
    ...
    SkString dump() const {
        ...
        msg.appendf("  textsize %g prescale %g preskew %g post [%g %g %g %g]\n", fTextSize,
                    fPreScaleX, fPreSkewX, fPost2x2[0][0], fPost2x2[0][1], fPost2x2[1][0],
                    fPost2x2[1][1]);
        msg.appendf("  frame %g miter %g format %d join %d cap %d flags %#hx\n", fFrameWidth,
                    fMiterLimit, fMaskFormat, fStrokeJoin, fStrokeCap, fFlags);
    ...
    bool computeMatrices(PreMatrixScale preMatrixScale, SkVector* scale, SkMatrix* remaining,
                         ...
                         SkMatrix* remainingRotation = nullptr, SkMatrix* total = nullptr);
...
public:
    ...
    static void MakeRecAndEffectsFromFont(const SkFont& font,
                                          ...
                                          SkScalerContextEffects* effects) {
        SkPaint paint;
        return MakeRecAndEffects(font, paint,
