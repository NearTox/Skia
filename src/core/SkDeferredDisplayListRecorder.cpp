...
bool SkDeferredDisplayListRecorder::init() {
  ...
  auto device = fContext->priv().createDevice(
      grColorType, fTargetProxy, fCharacterization.refColorSpace(), fCharacterization.origin(),
      fCharacterization.surfaceProps(), skgpu::BaseDevice::InitContents::kUninit);
  if (!device) {
    return false;
  }
...
sk_sp<SkImage> SkDeferredDisplayListRecorder::makeYUVAPromiseTexture(
  ...
  return SkImage::MakePromiseYUVATexture(
      fContext->threadSafeProxy(), backendTextureInfo, std::move(imageColorSpace),
