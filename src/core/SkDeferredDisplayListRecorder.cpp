...
sk_sp<SkImage> SkDeferredDisplayListRecorder::makePromiseTexture(
    ...
    PromiseImageTextureContext textureContext, PromiseImageApiVersion) {
  return nullptr;
}
...
sk_sp<SkImage> SkDeferredDisplayListRecorder::makeYUVAPromiseTexture(
    ...
    PromiseImageTextureReleaseProc textureReleaseProc, PromiseImageTextureDoneProc textureDoneProc,
    PromiseImageTextureContext textureContexts[], PromiseImageApiVersion) {
  return nullptr;
}
...
bool SkDeferredDisplayListRecorder::init() {
  ...
  if (fCharacterization.vulkanSecondaryCBCompatible()) {
    ...
    if (usesGLFBO0 || fCharacterization.isTextureable() ||
  ...
  GrPixelConfig config =
      caps->getConfigFromBackendFormat(fCharacterization.backendFormat(), grColorType);
  ...
  sk_sp<GrRenderTargetProxy> proxy = proxyProvider->createLazyRenderTargetProxy(
      ...
      fCharacterization.backendFormat(), desc, fCharacterization.sampleCount(),
      fCharacterization.origin(), surfaceFlags, optionalTextureInfo, SkBackingFit::kExact,
      SkBudgeted::kYes, fCharacterization.isProtected(),
  ...
  sk_sp<GrSurfaceContext> c = fContext->priv().makeWrappedSurfaceContext(
      std::move(proxy), grColorType, kPremul_SkAlphaType, fCharacterization.refColorSpace(),
      &fCharacterization.surfaceProps());
  fSurface =
      SkSurface_Gpu::MakeWrappedRenderTarget(fContext.get(), sk_ref_sp(c->asRenderTargetContext()));
...
sk_sp<SkImage> SkDeferredDisplayListRecorder::makePromiseTexture(
  ...
  return SkImage_Gpu::MakePromiseTexture(
      ...
      std::move(colorSpace), textureFulfillProc, textureReleaseProc, textureDoneProc,
      textureContext, version);
}
...
sk_sp<SkImage> SkDeferredDisplayListRecorder::makeYUVAPromiseTexture(
  ...
  return SkImage_GpuYUVA::MakePromiseYUVATexture(
      ...
      textureDoneProc, textureContexts, version);
}
