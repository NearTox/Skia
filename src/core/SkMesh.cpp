...
static std::vector<Uniform>::iterator find_uniform(
    std::vector<Uniform>& uniforms, std::string_view name) {
  return std::find_if(
      uniforms.begin(), uniforms.end(), [name](const SkMeshSpecification::Uniform& u) {
        return u.name.equals(name.data(), name.size());
      });
}

static std::tuple<bool, SkString> gather_uniforms_and_check_for_main(
    const SkSL::Program& program, std::vector<Uniform>* uniforms,
    SkMeshSpecification::Uniform::Flags stage, size_t* offset) {
    ...
    if (elem->is<SkSL::FunctionDefinition>()) {
      ...
      if (var.modifiers().fFlags & SkSL::Modifiers::kUniform_Flag) {
        ...
        if (iter == uniforms->end()) {
          ...
          if (uniform.isArray() != iter->isArray() || uniform.type != iter->type ||
              uniform.count != iter->count) {
            return {
                false, SkStringPrintf(
                           "Uniform %s declared with different types"
          ...
          if (uniform.isColor() != iter->isColor()) {
            return {
                false, SkStringPrintf(
                           "Uniform %s declared with different color"
...
static std::tuple<ColorType, bool> get_fs_color_type_and_local_coords(
    const SkSL::Program& fsProgram) {
    ...
    if (elem->is<SkSL::FunctionDefinition>()) {
      ...
      if (decl.isMain()) {
        ...
        if (decl.parameters().size() == 1) {
          ...
          SkASSERT(
              paramType.matches(*fsProgram.fContext->fTypes.fHalf4) ||
              paramType.matches(*fsProgram.fContext->fTypes.fFloat4));
          ct = paramType.matches(*fsProgram.fContext->fTypes.fHalf4) ? ColorType::kHalf4
                                                                     : ColorType::kFloat4;
        }

        const SkSL::Type& returnType = decl.returnType();
        SkASSERT(
            returnType.matches(*fsProgram.fContext->fTypes.fVoid) ||
...
std::tuple<bool, SkString> check_vertex_offsets_and_stride(
    SkSpan<const Attribute> attributes, size_t stride) {
  ...
  if (attributes.size() > SkMeshSpecification::kMaxAttributes) {
    RETURN_ERROR("A maximum of %zu attributes is allowed.", SkMeshSpecification::kMaxAttributes);
  }
  static_assert(SkIsPow2(SkMeshSpecification::kStrideAlignment));
  if (stride == 0 || stride & (SkMeshSpecification::kStrideAlignment - 1)) {
    RETURN_ERROR(
        "Vertex stride must be a non-zero multiple of %zu.", SkMeshSpecification::kStrideAlignment);
  }
  ...
  for (const auto& a : attributes) {
    if (a.offset & (SkMeshSpecification::kOffsetAlignment - 1)) {
      RETURN_ERROR(
          "Attribute offset must be a multiple of %zu.", SkMeshSpecification::kOffsetAlignment);
    }
...
SkMeshSpecification::Result SkMeshSpecification::Make(
    SkSpan<const Attribute> attributes, size_t vertexStride, SkSpan<const Varying> varyings,
    const SkString& vs, const SkString& fs) {
  return Make(
      attributes, vertexStride, varyings, vs, fs, SkColorSpace::MakeSRGB(), kPremul_SkAlphaType);
}
...
SkMeshSpecification::Result SkMeshSpecification::Make(
  ...
  return MakeFromSourceWithStructs(
      attributes, vertexStride, varyings, fullVS, fullFS, std::move(cs), at);
}
...
SkMeshSpecification::Result SkMeshSpecification::MakeFromSourceWithStructs(
  ...
  std::unique_ptr<SkSL::Program> vsProgram =
      compiler->convertProgram(SkSL::ProgramKind::kMeshVertex, std::string(vs.c_str()), settings);
  if (!vsProgram) {
    RETURN_FAILURE("VS: %s", compiler->errorText().c_str());
  }

  if (auto [result, error] = gather_uniforms_and_check_for_main(
          *vsProgram, &uniforms, SkMeshSpecification::Uniform::Flags::kVertex_Flag, &offset);
      !result) {
  ...
  std::unique_ptr<SkSL::Program> fsProgram =
      compiler->convertProgram(SkSL::ProgramKind::kMeshFragment, std::string(fs.c_str()), settings);
  ...
  if (auto [result, error] = gather_uniforms_and_check_for_main(
          *fsProgram, &uniforms, SkMeshSpecification::Uniform::Flags::kFragment_Flag, &offset);
      !result) {
    return {nullptr, std::move(error)};
  }
  ...
  return {
      sk_sp<SkMeshSpecification>(new SkMeshSpecification(
          attributes, stride, varyings, std::move(uniforms), std::move(vsProgram),
          std::move(fsProgram), ct, hasLocalCoords, std::move(cs), at)),
      /*error=*/{}};
}
...
SkMeshSpecification::SkMeshSpecification(
    SkSpan<const Attribute> attributes, size_t stride, SkSpan<const Varying> varyings,
    std::vector<Uniform> uniforms, std::unique_ptr<const SkSL::Program> vs,
    std::unique_ptr<const SkSL::Program> fs, ColorType ct, bool hasLocalCoords,
    sk_sp<SkColorSpace> cs, SkAlphaType at)
...
SkMesh SkMesh::Make(
    sk_sp<SkMeshSpecification> spec, Mode mode, sk_sp<VertexBuffer> vb, size_t vertexCount,
    size_t vertexOffset, sk_sp<const SkData> uniforms, const SkRect& bounds) {
  SkMesh cm;
...
SkMesh SkMesh::MakeIndexed(
    sk_sp<SkMeshSpecification> spec, Mode mode, sk_sp<VertexBuffer> vb, size_t vertexCount,
    size_t vertexOffset, sk_sp<IndexBuffer> ib, size_t indexCount, size_t indexOffset,
