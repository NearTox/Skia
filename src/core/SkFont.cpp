...
void SkFont::dump() const noexcept {
  SkDebugf("typeface %p\n", fTypeface.get());
  SkDebugf("size %g\n", fSize);
  SkDebugf("skewx %g\n", fSkewX);
  SkDebugf("scalex %g\n", fScaleX);
...
bool SkFont::hasSomeAntiAliasing() const noexcept {
 ...
 public:
  ...
  const SkUnichar* convert(const void* text, size_t byteLength, SkTextEncoding encoding) {
    ...
    switch (encoding) {
      ...
      case SkTextEncoding::kUTF32: uni = (const SkUnichar*)text; break;
      default: SK_ABORT("unexpected enum");
    }
    return uni;
  }
...
static void set_bounds(const SkGlyph& g, SkRect* bounds) noexcept {
  bounds->set(
      SkIntToScalar(g.fLeft), SkIntToScalar(g.fTop), SkIntToScalar(g.fLeft + g.fWidth),
      SkIntToScalar(g.fTop + g.fHeight));
}

static void join_bounds_x(const SkGlyph& g, SkRect* bounds, SkScalar dx) noexcept {
  bounds->join(
      SkIntToScalar(g.fLeft) + dx, SkIntToScalar(g.fTop), SkIntToScalar(g.fLeft + g.fWidth) + dx,
      SkIntToScalar(g.fTop + g.fHeight));
}
...
SkScalar SkFont::measureText(
    const void* text, size_t length, SkTextEncoding encoding, SkRect* bounds,
...
static SkRect make_bounds(const SkGlyph& g, SkScalar scale) noexcept {
...
void VisitGlyphs(
    const SkFont& origFont, const SkPaint* paint, const SkGlyphID glyphs[], int count,
    HANDLER handler) {
  if (count <= 0) {
    return;
  }
...
void SkFont::getWidthsBounds(
    const SkGlyphID glyphs[], int count, SkScalar widths[], SkRect bounds[],
    const SkPaint* paint) const {
  if (bounds) {
    VisitGlyphs(
...
void SkFont::getPaths(
    const SkGlyphID glyphs[], int count, void (*proc)(const SkPath*, const SkMatrix&, void*),
    void* ctx) const {
  SkFont font(*this);
...
int SkFontPriv::CountTextElements(const void* text, size_t byteLength, SkTextEncoding encoding) {
  switch (encoding) {
    ...
    case SkTextEncoding::kUTF32: return byteLength >> 2;
    case SkTextEncoding::kGlyphID: return byteLength >> 1;
  }
  SkASSERT(false);
  return 0;
}
...
void SkFontPriv::GlyphsToUnichars(
    const SkFont& font, const SkGlyphID glyphs[], int count, SkUnichar text[]) {
  if (count <= 0) {
    return;
  }
...
static constexpr bool scalar_is_byte(SkScalar x) noexcept {
  int ix = (int)x;
