...
bool SkLatticeIter::Valid(int width, int height, const SkCanvas::Lattice& lattice) noexcept {
  ...
  bool zeroXDivs =
      lattice.fXCount <= 0 || (1 == lattice.fXCount && latticeBounds.fLeft == lattice.fXDivs[0]);
  bool zeroYDivs =
      lattice.fYCount <= 0 || (1 == lattice.fYCount && latticeBounds.fTop == lattice.fYDivs[0]);
...
SkLatticeIter::SkLatticeIter(const SkCanvas::Lattice& lattice, const SkRect& dst) noexcept {
  ...
  set_points(
      fDstX.begin(), fSrcX.begin(), xDivs, xCount, xCountFixed, xCountScalable, src.fLeft,
      src.fRight, dst.fLeft, dst.fRight, xIsScalable);

  fSrcY.reset(yCount + 2);
  fDstY.reset(yCount + 2);
  set_points(
      fDstY.begin(), fSrcY.begin(), yDivs, yCount, yCountFixed, yCountScalable, src.fTop,
...
bool SkLatticeIter::next(
    SkIRect* src, SkRect* dst, bool* isFixedColor, SkColor* fixedColor) noexcept {
  ...
  if (fRectTypes.count() > 0 && SkToBool(SkCanvas::Lattice::kTransparent == fRectTypes[currRect])) {
  ...
  if (isFixedColor && fixedColor) {
    *isFixedColor =
        fRectTypes.count() > 0 && SkToBool(SkCanvas::Lattice::kFixedColor == fRectTypes[currRect]);
    if (*isFixedColor) {
