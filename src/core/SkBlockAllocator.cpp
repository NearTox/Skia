...
void SkBlockAllocator::reset() noexcept {
  for (Block* b : this->rblocks()) {
    if (b == &fHead) {
  ...
  SkASSERT(
      fTail == &fHead && fHead.fNext == nullptr && fHead.fPrev == nullptr &&
...
void SkBlockAllocator::resetScratchSpace() noexcept {
  if (fHead.fPrev) {
    delete fHead.fPrev;
    fHead.fPrev = nullptr;
  }
...
void SkBlockAllocator::addBlock(int minSize, int maxSize) {
  ...
  if (this->scratchBlockSize() >= minSize) {
    ...
    if (maxSize / sizeIncrement < nextN1) {
      ...
      allocSize = std::min(alignAllocSize(std::max(minSize, sizeIncrement * nextN1)), maxSize);
    }
  } else {
