...
class SkPictureImageGenerator : public SkImageGenerator {
 public:
  SkPictureImageGenerator(
      const SkImageInfo& info, sk_sp<SkPicture>, const SkMatrix*, const SkPaint*,
...
std::unique_ptr<SkImageGenerator> SkImageGenerator::MakeFromPicture(
    ...
    SkImage::BitDepth bitDepth, sk_sp<SkColorSpace> colorSpace, SkSurfaceProps props) {
...
SkPictureImageGenerator::SkPictureImageGenerator(
    const SkImageInfo& info, sk_sp<SkPicture> picture, const SkMatrix* matrix, const SkPaint* paint,
    const SkSurfaceProps& props)
    : INHERITED(info), fPicture(std::move(picture)), fProps(props) {
  if (matrix) {
    fMatrix = *matrix;
  } else {
    fMatrix.reset();
...
GrSurfaceProxyView SkPictureImageGenerator::onGenerateTexture(
  ...
  auto surface = SkSurface::MakeRenderTarget(
      ctx, budgeted, info, 0, kTopLeft_GrSurfaceOrigin, &fProps, mipmapped == GrMipmapped::kYes);
  if (!surface) {
    return {};
  }
