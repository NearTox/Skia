...
sk_sp<SkData> SkVertices::encode() const {
  ...
  for (int i = 0; i < fAttributeCount; ++i) {
    encodedAttrs[i] = {
        fAttributes[i].fType, fAttributes[i].fUsage, SkToBool(fAttributes[i].fMarkerName)};
  }
...
sk_sp<SkVertices> SkVertices::Decode(const void* data, size_t length) {
  ...
  const unsigned version =
      safe.checkLE<unsigned>((packed & kVersion_Mask) >> kVersion_Shift, kCurrent_Version);
  ...
  for (int i = 0; i < attrCount; ++i) {
    attrs[i] = Attribute(
        encodedAttrs[i].fType, encodedAttrs[i].fUsage,
