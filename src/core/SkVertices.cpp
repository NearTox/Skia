...
bool SkVertices::Attribute::isValid() const {
  ...
  switch (fUsage) {
    case Usage::kRaw: return fMarkerID == 0;
    case Usage::kColor:
      return fMarkerID == 0 &&
             (fType == Type::kFloat3 || fType == Type::kFloat4 || fType == Type::kByte4_unorm);
    case Usage::kVector:
    case Usage::kNormalVector:
    case Usage::kPosition: return fType == Type::kFloat2 || fType == Type::kFloat3;
  }
  SkUNREACHABLE;
}
...
struct SkVertices::Sizes {
  Sizes(const Desc& desc) {
    ...
    fDSize = safe.mul(custom_data_size(desc.fAttributes, desc.fAttributeCount), desc.fVertexCount);
    ...
    fTotal = safe.add(
        ...
        safe.add(
            ...
            safe.add(
                fNameSize,
                safe.add(fVSize, safe.add(fDSize, safe.add(fTSize, safe.add(fCSize, fISize)))))));
...
SkVertices::Builder::Builder(
    VertexMode mode, int vertexCount, int indexCount, uint32_t builderFlags) {
...
SkVertices::Builder::Builder(
    VertexMode mode, int vertexCount, int indexCount, const SkVertices::Attribute* attrs,
    int attrCount) {
...
SkVertices::Builder::Builder(const Desc& desc) { this->init(desc); }
...
void SkVertices::Builder::init(const Desc& desc) {
  ...
  sk_careful_memcpy(
      fVertices->fAttributes, desc.fAttributes, desc.fAttributeCount * sizeof(Attribute));
...
sk_sp<SkVertices> SkVertices::MakeCopy(
    VertexMode mode, int vertexCount, const SkPoint pos[], const SkPoint texs[],
    const SkColor colors[], int indexCount, const uint16_t indices[]) {
...
size_t SkVertices::approximateSize() const { return this->getSizes().fTotal; }

SkVertices::Sizes SkVertices::getSizes() const {
  Sizes sizes({fMode, fVertexCount, fIndexCount, !!fTexs, !!fColors, fAttributes, fAttributeCount});
  SkASSERT(sizes.isValid());
  return sizes;
}
...
sk_sp<SkVertices> SkVerticesPriv::Decode(SkReadBuffer& buffer) {
  ...
  auto decode = [](SkReadBuffer& buffer) -> sk_sp<SkVertices> {
    ...
    const SkVertices::VertexMode mode =
        safe.checkLE<SkVertices::VertexMode>(packed & kMode_Mask, SkVertices::kLast_VertexMode);
    ...
    for (int i = 0; i < attrCount; ++i) {
      auto type = buffer.checkRange(
          SkVertices::Attribute::Type::kFloat, SkVertices::Attribute::Type::kByte4_unorm);
      auto usage = buffer.checkRange(
          SkVertices::Attribute::Usage::kRaw, SkVertices::Attribute::Usage::kPosition);
    ...
    size_t isize =
        (mode == SkVertices::kTriangleFan_VertexMode) ? sizes.fBuilderTriFanISize : sizes.fISize;
