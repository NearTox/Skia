...
SkYUVAPixmapInfo::SkYUVAPixmapInfo(
  ...
  for (size_t i = 0; i < static_cast<size_t>(n); ++i) {
    ...
    fPlaneInfos[i] = SkImageInfo::Make(planeDimensions[i], colorTypes[i], kUnpremul_SkAlphaType);
...
SkYUVAPixmaps SkYUVAPixmaps::MakeCopy(const SkYUVAPixmaps& src) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    SkRectMemcpy(
        d.writable_addr(), d.rowBytes(), s.addr(), s.rowBytes(), s.info().minRowBytes(),
        s.height());
  }
  return result;
}
...
SkYUVAPixmaps::SkYUVAPixmaps(
    const SkYUVAInfo& yuvaInfo, DataType dataType, const SkPixmap pixmaps[kMaxPlanes])
