  ...
  for (int n = 1; n <= 4; ++n) {
    if (context
            .defaultBackendFormat(
                DefaultColorTypeForDataType(DataType::kUnorm8, n), GrRenderable::kNo)
            .isValid()) {
      this->enableDataType(DataType::kUnorm8, n);
    }
    if (context
            .defaultBackendFormat(
                DefaultColorTypeForDataType(DataType::kUnorm16, n), GrRenderable::kNo)
            .isValid()) {
      this->enableDataType(DataType::kUnorm16, n);
    }
    if (context
            .defaultBackendFormat(
                DefaultColorTypeForDataType(DataType::kFloat16, n), GrRenderable::kNo)
            .isValid()) {
      this->enableDataType(DataType::kFloat16, n);
    }
    if (context
            .defaultBackendFormat(
                DefaultColorTypeForDataType(DataType::kUnorm10_Unorm2, n), GrRenderable::kNo)
            .isValid()) {
...
SkYUVAPixmapInfo::SkYUVAPixmapInfo(
    const SkYUVAInfo& yuvaInfo, const SkColorType colorTypes[kMaxPlanes],
...
SkYUVAPixmapInfo::SkYUVAPixmapInfo(
    const SkYUVAInfo& yuvaInfo, DataType dataType, const size_t rowBytes[kMaxPlanes]) {
...
bool SkYUVAPixmapInfo::operator==(const SkYUVAPixmapInfo& that) const {
  bool result =
      fYUVAInfo == that.fYUVAInfo && fPlaneInfos == that.fPlaneInfos && fRowBytes == that.fRowBytes;
...
bool SkYUVAPixmapInfo::initPixmapsFromSingleAllocation(
    void* memory, SkPixmap pixmaps[kMaxPlanes]) const {
  if (!this->isValid()) {
    return false;
  }
  SkASSERT(pixmaps);
...
SkYUVAPixmaps SkYUVAPixmaps::Allocate(const SkYUVAPixmapInfo& yuvaPixmapInfo) {
  ...
  return SkYUVAPixmaps(
      yuvaPixmapInfo, SkData::MakeUninitialized(yuvaPixmapInfo.computeTotalBytes()));
}
...
SkYUVAPixmaps SkYUVAPixmaps::FromExternalMemory(
    const SkYUVAPixmapInfo& yuvaPixmapInfo, void* memory) {
  if (!yuvaPixmapInfo.isValid()) {
    return {};
  }
...
SkYUVAPixmaps SkYUVAPixmaps::FromExternalPixmaps(
    const SkYUVAInfo& yuvaInfo, const SkPixmap pixmaps[kMaxPlanes]) {
  SkColorType colorTypes[kMaxPlanes] = {};
...
SkYUVAPixmaps::SkYUVAPixmaps(const SkYUVAPixmapInfo& yuvaPixmapInfo, sk_sp<SkData> data)
    : fYUVAInfo(yuvaPixmapInfo.yuvaInfo()), fData(std::move(data)) {
  SkASSERT(yuvaPixmapInfo.isValid());
  SkASSERT(yuvaPixmapInfo.computeTotalBytes() <= fData->size());
  SkAssertResult(
      yuvaPixmapInfo.initPixmapsFromSingleAllocation(fData->writable_data(), fPlanes.data()));
}
...
bool SkYUVAPixmaps::toLegacy(SkYUVASizeInfo* yuvaSizeInfo, SkYUVAIndex yuvaIndices[4]) const {
  ...
  auto getIthChannel = [&ok](SkColorType ct, int idx) -> SkColorChannel {
    switch (SkColorTypeChannelFlags(ct)) {
      case kAlpha_SkColorChannelFlag: ok &= idx == 0; return SkColorChannel::kA;
      case kGray_SkColorChannelFlag:
      case kRed_SkColorChannelFlag: ok &= idx == 0; return SkColorChannel::kR;
      case kRG_SkColorChannelFlags: ok &= idx < 2; return static_cast<SkColorChannel>(idx);
      case kRGB_SkColorChannelFlags: ok &= idx < 3; return static_cast<SkColorChannel>(idx);
      case kRGBA_SkColorChannelFlags: ok &= idx < 4; return static_cast<SkColorChannel>(idx);
      default: ok = false; return SkColorChannel::kR;
    }
  };
  SkColorType cts[] = {
      fPlanes[0].colorType(), fPlanes[1].colorType(), fPlanes[2].colorType(),
      fPlanes[3].colorType()};
