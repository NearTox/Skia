...
SkIPoint SkBaseDevice::getOrigin() const {
  ...
  return SkIPoint::Make(
      SkScalarFloorToInt(fDeviceToGlobal.getTranslateX()),
...
void SkBaseDevice::drawImageLattice(
  ...
  while (iter.next(&srcR, &dstR, &isFixedColor, &c)) {
    ...
    if (isFixedColor || (srcR.width() <= 1.0f && srcR.height() <= 1.0f &&
      ...
      this->drawImageRect(image, &srcR, dstR, sampling, paint, SkCanvas::kStrict_SrcRectConstraint);
    }
  }
}
...
void SkNoPixelsDevice::onClipRRect(const SkRRect& rrect, SkClipOp op, bool aa) {
  this->writableClip().opRRect(rrect, this->localToDevice(), this->bounds(), (SkRegion::Op)op, aa);
}

void SkNoPixelsDevice::onClipPath(const SkPath& path, SkClipOp op, bool aa) {
  this->writableClip().opPath(path, this->localToDevice(), this->bounds(), (SkRegion::Op)op, aa);
}
...
void SkNoPixelsDevice::onClipRegion(const SkRegion& globalRgn, SkClipOp op) {
  if (globalRgn.isEmpty()) {
    ...
    this->writableClip().opRect(
        SkRect::Make(globalRgn.getBounds()), this->globalToDevice(), this->bounds(),
