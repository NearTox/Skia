...
void SkBaseDevice::setDeviceCoordinateSystem(
    const SkM44& deviceToGlobal, const SkM44& globalToDevice, const SkM44& localToDevice,
...
SkIPoint SkBaseDevice::getOrigin() const {
  ...
  return SkIPoint::Make(
      SkScalarFloorToInt(fDeviceToGlobal.rc(0, 3)), SkScalarFloorToInt(fDeviceToGlobal.rc(1, 3)));
}
...
void SkBaseDevice::drawDRRect(const SkRRect& outer, const SkRRect& inner, const SkPaint& paint) {
  SkPath path;
  path.addRRect(outer);
  path.addRRect(inner);
...
void SkBaseDevice::drawImageLattice(
  ...
  while (iter.next(&srcR, &dstR, &isFixedColor, &c)) {
    ...
    if (isFixedColor || (srcR.width() <= 1.0f && srcR.height() <= 1.0f &&
      ...
      this->drawImageRect(
          image, &srcR, dstR, SkSamplingOptions(filter), paint,
...
void SkBaseDevice::drawAtlas(
    const SkRSXform xform[], const SkRect tex[], const SkColor colors[], int quadCount,
    sk_sp<SkBlender> blender, const SkPaint& paint) {
...
void SkBaseDevice::drawEdgeAAImageSet(
    const SkCanvas::ImageSetEntry images[], int count, const SkPoint dstClips[],
  ...
  for (int i = 0; i < count; ++i) {
    ...
    this->drawImageRect(
        images[i].fImage.get(), &images[i].fSrcRect, images[i].fDstRect, sampling, entryPaint,
        constraint);
    if (needsRestore) {
...
void SkBaseDevice::drawSpecial(
    SkSpecialImage*, const SkMatrix&, const SkSamplingOptions&, const SkPaint&) {}
...
bool SkBaseDevice::onWritePixels(const SkPixmap&, int, int) { return false; }

bool SkBaseDevice::onReadPixels(const SkPixmap&, int x, int y) { return false; }
...
void SkBaseDevice::drawGlyphRunList(
...
void SkBaseDevice::simplifyGlyphRunRSXFormAndRedraw(
    SkCanvas* canvas, const SkGlyphRunList& glyphRunList, const SkPaint& initialPaint,
    const SkPaint& drawingPaint) {
  for (const SkGlyphRun& run : glyphRunList) {
    if (run.scaledRotations().empty()) {
      auto subList =
          glyphRunList.builder()->makeGlyphRunList(run, run.sourceBounds(drawingPaint), {0, 0});
      ...
      for (auto [i, glyphID, pos] : SkMakeEnumerate(run.source())) {
        ...
        invertingPaint.setShader(make_post_inverse_lm(drawingPaint.getShader(), glyphToLocal));
...
sk_sp<sktext::gpu::Slug> SkBaseDevice::convertGlyphRunListToSlug(
    const SkGlyphRunList& glyphRunList, const SkPaint& initialPaint, const SkPaint& drawingPaint) {
  return nullptr;
}
...
SkNoPixelsDevice::SkNoPixelsDevice(
    const SkIRect& bounds, const SkSurfaceProps& props, sk_sp<SkColorSpace> colorSpace)
    : SkBaseDevice(
          SkImageInfo::Make(
...
void SkNoPixelsDevice::onClipRect(const SkRect& rect, SkClipOp op, bool aa) {
  this->writableClip().op(op, this->localToDevice44(), rect, aa, /*fillsBounds=*/true);
}

void SkNoPixelsDevice::onClipRRect(const SkRRect& rrect, SkClipOp op, bool aa) {
  this->writableClip().op(
      op, this->localToDevice44(), rrect.getBounds(), aa, /*fillsBounds=*/rrect.isRect());
}
...
void SkNoPixelsDevice::onClipPath(const SkPath& path, SkClipOp op, bool aa) {
  ...
  this->writableClip().op(op, this->localToDevice44(), path.getBounds(), aa, /*fillsBounds=*/false);
}

void SkNoPixelsDevice::onClipRegion(const SkRegion& globalRgn, SkClipOp op) {
  this->writableClip().op(
      op, this->globalToDevice(), SkRect::Make(globalRgn.getBounds()),
...
void SkNoPixelsDevice::ClipState::op(
    SkClipOp op, const SkM44& transform, const SkRect& bounds, bool isAA, bool fillsBounds) {
  const bool isRect = fillsBounds && SkMatrixPriv::IsScaleTranslateAsM33(transform);
  fIsAA |= isAA;

  SkRect devBounds =
      bounds.isEmpty() ? SkRect::MakeEmpty() : SkMatrixPriv::MapRect(transform, bounds);
  if (op == SkClipOp::kIntersect) {
    ...
    if (SkRectPriv::Subtract(
            fClipBounds, isAA ? devBounds.roundIn() : devBounds.round(), &difference)) {
