...
static bool decompress_etc1(SkISize dimensions, const uint8_t* srcData, SkBitmap* dst) {
  ...
  for (int y = 0; y < numYBlocks; ++y) {
    for (int x = 0; x < numXBlocks; ++x) {
      ...
      const int* tables[2] = {kETC1ModifierTables[tableIndex0], kETC1ModifierTables[tableIndex1]};
      ...
      for (int i = 0; i < 4; ++i, ++baseShift) {
        for (int j = 0; j < 4; ++j) {
          ...
          int pixelIndex =
              ((low >> (baseShift + (j * 4))) & 0x1) | (low >> (baseShift + (j * 4) + 15) & 0x2);
    ...
    for (int x = 0; x < numXBlocks; ++x) {
      ...
      if (curBlock->fColor0 <= curBlock->fColor1) {  // signal for a transparent block
        colors[2] = SkPackARGB32(
            0xFF, (SkGetPackedR32(colors[0]) + SkGetPackedR32(colors[1])) >> 1,
...
size_t SkCompressedDataSize(
  ...
  size_t totalSize = 0;
  switch (type) {
    case SkImage::CompressionType::kNone: break;
