...
SkCachedData::~SkCachedData() {
  switch (fStorageType) {
    case kMalloc_StorageType: sk_free(fStorage.fMalloc); break;
    case kDiscardableMemory_StorageType: delete fStorage.fDM; break;
  }
}
...
void SkCachedData::inMutexLock() {
  ...
  switch (fStorageType) {
    case kMalloc_StorageType: this->setData(fStorage.fMalloc); break;
...
void SkCachedData::validate() const {
  if (fIsLocked) {
    ...
    switch (fStorageType) {
      case kMalloc_StorageType: SkASSERT(fData == fStorage.fMalloc); break;
      case kDiscardableMemory_StorageType:
