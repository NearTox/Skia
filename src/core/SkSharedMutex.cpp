...
class SkSharedMutex::ThreadIDSet {
 public:
  ...
  void swap(ThreadIDSet& other) { fThreadIDs.swap(other.fThreadIDs); }

  int count() const { return fThreadIDs.count(); }
...
void SkSharedMutex::acquire() {
  ...
  int waitingExclusiveCount;
  {
    ...
    SkASSERTF(!fCurrentShared->find(threadID), "Thread %lx already has an shared lock\n", threadID);
...
void SkSharedMutex::release() {
  ...
  } while (!fQueueCounts.compare_exchange_strong(
      oldQueueCounts, newQueueCounts, std::memory_order_release, std::memory_order_relaxed));
...
void SkSharedMutex::acquireShared() {
  ...
  do {
    ...
    if ((newQueueCounts & kWaitingExclusiveMask) > 0) {
      ...
      oldQueueCounts, newQueueCounts, std::memory_order_acquire, std::memory_order_relaxed));
...
void SkSharedMutex::releaseShared() {
  ...
  int32_t oldQueueCounts = fQueueCounts.fetch_sub(1 << kSharedOffset, std::memory_order_release);
