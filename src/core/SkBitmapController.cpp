...
SkBitmapController::State* SkBitmapController::RequestBitmap(
    const SkImage_Base* image, const SkMatrix& inv, SkFilterQuality quality, SkArenaAlloc* alloc) {
...
bool SkBitmapController::State::processMediumRequest(const SkImage_Base* image) {
  ...
  if (invScaleSize.width() > SK_Scalar1 || invScaleSize.height() > SK_Scalar1) {
    ...
    const SkSize scale =
        SkSize::Make(SkScalarInvert(invScaleSize.width()), SkScalarInvert(invScaleSize.height()));
...
SkBitmapController::State::State(
    const SkImage_Base* image, const SkMatrix& inv, SkFilterQuality qual) {
  fInvMatrix = inv;
  fQuality = qual;
...
SkMipmapAccessor::SkMipmapAccessor(
    const SkImage_Base* image, const SkMatrix& inv, SkMipmapMode requestedMode) {
