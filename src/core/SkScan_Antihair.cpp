...
class SkAntiHairBlitter {
 public:
  constexpr SkAntiHairBlitter() noexcept : fBlitter(nullptr) {}
  virtual ~SkAntiHairBlitter() = default;

  SkBlitter* getBlitter() const { return fBlitter; }

  void setup(SkBlitter* blitter) { fBlitter = blitter; }
...
class Vertish_SkAntiHairBlitter : public SkAntiHairBlitter {
 public:
  SkFixed drawCap(int y, SkFixed fx, SkFixed dx, int mod64) override {
    ...
    this->getBlitter()->blitAntiH2(
        x - 1, y, SmallDot6Scale(255 - a, mod64), SmallDot6Scale(a, mod64));
...
static void do_anti_hairline(
    SkFDot6 x0, SkFDot6 y0, SkFDot6 x1, SkFDot6 y1, const SkIRect* clip, SkBlitter* blitter) {
...
void SkScan::AntiHairLineRgn(
    const SkPoint array[], int arrayCount, const SkRegion* clip, SkBlitter* blitter) {
  ...
  for (int i = 0; i < arrayCount - 1; ++i) {
    ...
    if (clip) {
      ...
      ir.setLTRB(
          SkFDot6Floor(left) - 1, SkFDot6Floor(top) - 1, SkFDot6Ceil(right) + 1,
...
static inline FDot8 SkFixedToFDot8(SkFixed x) { return (x + 0x80) >> 8; }
...
static void antifillrect(const SkXRect& xr, SkBlitter* blitter) {
  antifilldot8(
      SkFixedToFDot8(xr.fLeft), SkFixedToFDot8(xr.fTop), SkFixedToFDot8(xr.fRight),
      SkFixedToFDot8(xr.fBottom), blitter, true);
}
...
static inline FDot8 SkScalarToFDot8(SkScalar x) { return (int)(x * 256); }

static inline int FDot8Floor(FDot8 x) { return x >> 8; }

static inline int FDot8Ceil(FDot8 x) { return (x + 0xFF) >> 8; }
...
void SkScan::AntiFrameRect(
    const SkRect& r, const SkPoint& strokeSize, const SkRegion* clip, SkBlitter* blitter) {
