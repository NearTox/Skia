...
SkScalerCache::SkScalerCache(
    std::unique_ptr<SkScalerContext> scaler, const SkFontMetrics* fontMetrics)
...
std::tuple<SkDrawable*, size_t> SkScalerCache::mergeDrawable(
    SkGlyph* glyph, sk_sp<SkDrawable> drawable) {
  SkAutoMutexExclusive lock{fMu};
  size_t delta = 0;
...
size_t SkScalerCache::prepareForDrawingMasksCPU(SkDrawableGlyphBuffer* accepted) {
  ...
  size_t delta = this->commonFilterLoop(
      accepted, [&](size_t i, SkGlyphDigest digest, SkPoint pos) SK_REQUIRES(fMu) {
...
void SkScalerCache::findIntercepts(
...
void SkScalerCache::dump() const {
  ...
  msg.printf(
      ...
      style.weight(), style.width(), style.slant(), rec.dump().c_str(),
