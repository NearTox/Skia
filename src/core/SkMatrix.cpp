...
static inline bool checkForZero(float x) { return x * x == 0; }
...
bool SkTreatAsSprite(
    const SkMatrix& mat, const SkISize& size, const SkSamplingOptions& sampling,
    const SkPaint& paint) {
...
SkScalar SkMatrixPriv::DifferentialAreaScale(const SkMatrix& m, const SkPoint& p) {
  ...
  SkMatrix jacobian = SkMatrix::MakeAll(
