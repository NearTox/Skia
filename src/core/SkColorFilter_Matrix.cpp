...
bool SkColorFilter_Matrix::program(
  ...
  if (fDomain == Domain::kRGBA) {
    // Unpremul.
    skvm::F32 invA = p->div(p->splat(1.0f), *a), inf = p->bit_cast(p->splat(0x7f800000));

    // If *a is 0, so are *r,*g,*b, so set invA to 0 to avoid 0*inf=NaN (instead 0*0 = 0).
    invA = p->bit_cast(p->bit_and(p->lt(invA, inf), p->bit_cast(invA)));
    *r = p->mul(*r, invA);
    *g = p->mul(*g, invA);
    *b = p->mul(*b, invA);
  ...
  switch (fDomain) {
    ...
    case Domain::kHSLA: {
      std::unique_ptr<GrFragmentProcessor> series[] = {
          ...
          GrColorMatrixFragmentProcessor::Make(
              fMatrix,
              /* premulInput = */ false,
...
static sk_sp<SkColorFilter> MakeMatrix(const float array[20], SkColorFilter_Matrix::Domain domain) {
  return sk_floats_are_finite(array, 20) ? sk_make_sp<SkColorFilter_Matrix>(array, domain)
                                         : nullptr;
}
