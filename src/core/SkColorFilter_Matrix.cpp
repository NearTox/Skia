  ...
  class GLSLProcessor : public GrGLSLFragmentProcessor {
   public:
    ...
    void emitCode(EmitArgs& args) override {
      ...
      fMatrixHandle =
          uniformHandler->addUniform(kFragment_GrShaderFlag, kHalf4x4_GrSLType, "ColorMatrix");
      fVectorHandle =
          uniformHandler->addUniform(kFragment_GrShaderFlag, kHalf4_GrSLType, "ColorMatrixVector");
      ...
      fragBuilder->codeAppendf(
          "\t%s = %s * half4(%s.rgb / nonZeroAlpha, nonZeroAlpha) + "
          "%s;\n",
          args.fOutputColor, uniformHandler->getUniformCStr(fMatrixHandle), args.fInputColor,
   ...
   protected:
    void onSetData(
        const GrGLSLProgramDataManager& uniManager, const GrFragmentProcessor& proc) override {
      const ColorMatrixEffect& cme = proc.cast<ColorMatrixEffect>();
      const float* m = cme.fMatrix;
      // The GL matrix is transposed from SkColorMatrix.
      float mt[] = {
          m[0], m[5], m[10], m[15], m[1], m[6], m[11], m[16],
          m[2], m[7], m[12], m[17], m[3], m[8], m[13], m[18],
      };
      float vec[] = {
          m[4],
          m[9],
...
sk_sp<SkColorFilter> SkColorFilters::Matrix(const SkColorMatrix& cm) { return Matrix(cm.fMat); }
...
void SkColorFilter_Matrix::RegisterFlattenables() {
  ...
  SkFlattenable::Register(
      "SkColorMatrixFilterRowMajor255", [](SkReadBuffer& buffer) -> sk_sp<SkFlattenable> {
        float matrix[20];
        if (buffer.readScalarArray(matrix, 20)) {
          matrix[4] *= (1.0f / 255);
          matrix[9] *= (1.0f / 255);
