...
bool SkColorFilter_Matrix::onAppendStages(const SkStageRec& rec, bool shaderIsOpaque) const {
  const bool willStayOpaque = shaderIsOpaque && fAlphaIsUnchanged, hsla = fDomain == Domain::kHSLA;
...
skvm::Color SkColorFilter_Matrix::onProgram(
  ...
  auto apply_matrix = [&](auto xyzw) {
    auto dot = [&](int j) {
      ...
      return custom_mad(
          ...
          custom_mad(
              fMatrix[1 + j * 5], y,
              custom_mad(fMatrix[2 + j * 5], z, custom_mad(fMatrix[3 + j * 5], w, bias))));
    };
...
GrFPResult SkColorFilter_Matrix::asFragmentProcessor(
  ...
  switch (fDomain) {
    case Domain::kRGBA:
      fp = GrFragmentProcessor::ColorMatrix(
          std::move(fp), fMatrix,
          /* unpremulInput = */ true,
          /* clampRGBOutput = */ true,
          /* premulOutput = */ true);
...
void SkColorFilter_Matrix::RegisterFlattenables() { SK_REGISTER_FLATTENABLE(SkColorFilter_Matrix); }
