...
template <typename D, typename T> static const D* pod(const T* op, size_t offset = 0) {
...
#define TYPES(M)        \
    ...
    M(DrawPatch)        \
    M(DrawPoints)       \
    M(DrawVertices) M(DrawAtlas) M(DrawShadowRec) M(DrawEdgeAAQuad) M(DrawEdgeAAImageSet)
...
struct DrawEdgeAAQuad final : Op {
    ...
    DrawEdgeAAQuad(const SkRect& rect, const SkPoint clip[4], SkCanvas::QuadAAFlags aa,
...
struct DrawEdgeAAImageSet final : Op {
    ...
    void draw(SkCanvas* c, const SkMatrix&) const {
        ...
        c->experimental_DrawEdgeAAImageSet(set.get(), count, dstClips, preViewMatrices, &paint,
                                           constraint);
    }
};
}  // namespace

template <typename T, typename... Args> void* SkLiteDL::push(size_t pod, Args&&... args) {
...
void SkLiteDL::saveLayer(const SkRect* bounds, const SkPaint* paint, const SkImageFilter* backdrop,
                         const SkImage* clipMask, const SkMatrix* clipMatrix,
                         SkCanvas::SaveLayerFlags flags) {
    this->push<SaveLayer>(0, bounds, paint, backdrop, clipMask, clipMatrix, flags);
}
void SkLiteDL::saveBehind(const SkRect* subset) { this->push<SaveBehind>(0, subset); }
...
void SkLiteDL::drawPaint(const SkPaint& paint) { this->push<DrawPaint>(0, paint); }
void SkLiteDL::drawBehind(const SkPaint& paint) { this->push<DrawBehind>(0, paint); }
...
void SkLiteDL::drawPicture(const SkPicture* picture, const SkMatrix* matrix, const SkPaint* paint) {
...
void SkLiteDL::drawImageNine(sk_sp<const SkImage> image, const SkIRect& center, const SkRect& dst,
                             const SkPaint* paint) {
...
void SkLiteDL::drawEdgeAAQuad(const SkRect& rect, const SkPoint clip[4], SkCanvas::QuadAAFlags aa,
                              SkColor color, SkBlendMode mode) {
...
void SkLiteDL::drawEdgeAAImageSet(const SkCanvas::ImageSetEntry set[], int count,
    ...
    void* pod =
            this->push<DrawEdgeAAImageSet>(bytes, set, count, totalDstClipCount, paint, constraint);
    copy_v(pod, dstClips, totalDstClipCount, preViewMatrices, totalMatrixCount);
}
