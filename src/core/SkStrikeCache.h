...
class SkStrike final : public SkRefCnt, public SkStrikeForGPU {
 public:
  SkStrike(
      SkStrikeCache* strikeCache, const SkStrikeSpec& strikeSpec,
  ...
  const SkDescriptor& getDescriptor() const override { return fStrikeSpec.descriptor(); }

  const SkStrikeSpec& strikeSpec() const { return fStrikeSpec; }

  void verifyPinnedStrike() const { SkASSERT_RELEASE(fPinner == nullptr || !fPinner->canDelete()); }
  ...
  sk_sp<SkStrike> getUnderlyingStrike() const override { return sk_ref_sp(this); }
...
class SkStrikeCache final : public SkStrikeForGPUCacheInterface {
 public:
  ...
  sk_sp<SkStrike> createStrike(
      const SkStrikeSpec& strikeSpec, SkFontMetrics* maybeMetrics = nullptr,
