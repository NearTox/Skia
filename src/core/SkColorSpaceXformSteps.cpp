...
void SkColorSpaceXformSteps::apply(float* rgba) const {
  ...
  if (flags.gamut_transform) {
    ...
    for (int i = 0; i < 3; ++i) {
      rgba[i] = src_to_dst_matrix[i] * temp[0] + src_to_dst_matrix[3 + i] * temp[1] +
...
skvm::Color sk_program_transfer_fn(
    skvm::Builder* p, skvm::Uniforms* uniforms, const skcms_TransferFunction& tf, skvm::Color c) {
  skvm::F32 G = p->uniformF(uniforms->pushF(tf.g)), A = p->uniformF(uniforms->pushF(tf.a)),
            B = p->uniformF(uniforms->pushF(tf.b)), C = p->uniformF(uniforms->pushF(tf.c)),
            D = p->uniformF(uniforms->pushF(tf.d)), E = p->uniformF(uniforms->pushF(tf.e)),
            F = p->uniformF(uniforms->pushF(tf.f));

  auto apply = [&](skvm::F32 v) -> skvm::F32 {
    // Strip off the sign bit and save it for later.
    skvm::I32 bits = p->bit_cast(v), sign = p->bit_and(bits, p->splat(0x80000000));
    ...
    switch (classify_transfer_fn(tf)) {
      ...
      case sRGBish_TF:
        v = p->select(p->lte(v, D), p->mad(C, v, F), p->add(p->approx_powf(p->mad(A, v, B), G), E));
        break;

      case PQish_TF:
        v = p->approx_powf(
            p->div(
                p->max(p->mad(B, p->approx_powf(v, C), A), p->splat(0.f)),
      ...
      case HLGish_TF: {
        auto vA = p->mul(v, A);
        v = p->select(
            p->lte(vA, p->splat(1.0f)), p->approx_powf(vA, B),
            p->approx_exp(p->mad(p->sub(v, E), C, D)));
      } break;

      case HLGinvish_TF:
        v = p->select(
            p->lte(v, p->splat(1.0f)), p->mul(A, p->approx_powf(v, B)),
...
skvm::Color SkColorSpaceXformSteps::program(
    skvm::Builder* p, skvm::Uniforms* uniforms, skvm::Color c) const {
  if (flags.unpremul) {
    c = unpremul(c);
  }
  if (flags.linearize) {
