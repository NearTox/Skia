...
#define SK_RASTER_PIPELINE_STAGES(M)                                                                             \
  ...
  M(force_opaque)                                                                                                \
  M(force_opaque_dst)                                                                                            \
  M(set_rgb) M(unbounded_set_rgb) M(swap_rb) M(swap_rb_dst) M(from_srgb) M(to_srgb) M(                           \
      black_color) M(white_color) M(uniform_color) M(unbounded_uniform_color) M(seed_shader)                     \
      M(dither) M(load_a8) M(load_a8_dst) M(store_a8) M(gather_a8) M(load_565) M(load_565_dst) M(                \
          store_565) M(gather_565) M(load_4444) M(load_4444_dst) M(store_4444) M(gather_4444)                    \
          M(load_f16) M(load_f16_dst) M(store_f16) M(gather_f16) M(load_f32) M(load_f32_dst) M(                  \
              store_f32) M(gather_f32) M(load_8888) M(load_8888_dst) M(store_8888) M(gather_8888)                \
              M(load_1010102) M(load_1010102_dst) M(store_1010102) M(gather_1010102) M(                          \
                  alpha_to_gray) M(alpha_to_gray_dst) M(luminance_to_alpha) M(bilerp_clamp_8888)                 \
                  M(store_u16_be) M(load_src) M(store_src) M(load_dst) M(store_dst) M(scale_u8) M(               \
                      scale_565) M(scale_1_float) M(lerp_u8) M(lerp_565) M(lerp_1_float)                         \
                      M(lerp_native) M(dstatop) M(dstin) M(dstout) M(dstover) M(srcatop) M(                      \
                          srcin) M(srcout) M(srcover) M(clear) M(modulate) M(multiply) M(plus_)                  \
                          M(screen) M(xor_) M(colorburn) M(colordodge) M(darken) M(difference) M(                \
                              exclusion) M(hardlight) M(lighten) M(overlay) M(softlight) M(hue)                  \
                              M(saturation) M(color) M(luminosity) M(srcover_rgba_8888) M(matrix_translate) M(   \
                                  matrix_scale_translate) M(matrix_2x3) M(matrix_3x3)                            \
                                  M(matrix_3x4) M(                                                               \
                                      matrix_4x5) M(matrix_4x3) M(matrix_perspective)                            \
                                      M(parametric) M(                                                           \
                                          gamma_) M(mirror_x) M(repeat_x) M(mirror_y) M(repeat_y)                \
                                          M(decal_x) M(decal_y) M(decal_x_and_y) M(check_decal_mask) M(          \
                                              negate_x) M(bilinear_nx)                                           \
                                              M(bilinear_px) M(bilinear_ny) M(bilinear_py) M(                    \
                                                  bicubic_n3x) M(bicubic_n1x) M(bicubic_p1x)                     \
                                                                  ...
                                                                  M(xy_to_2pt_conical_well_behaved) M(           \
                                                                      ...
                                                                      M(xy_to_2pt_conical_greater) M(            \
                                                                          ...
                                                                          M(alter_2pt_conical_unswap) M(         \
                                                                              ...
                                                                              M(mask_2pt_conical_degenerates) M( \
                                                                                  apply_vector_mask)             \
                                                                                  M(byte_tables) M(              \
                                                                                      rgb_to_hsl)                \
                                                                                      M(hsl_to_rgb) M(           \
