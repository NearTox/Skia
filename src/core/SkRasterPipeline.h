...
#define SK_RASTER_PIPELINE_STAGES(M)    \
  ...
  M(apply_vector_mask) M(byte_tables) M(rgb_to_hsl) M(hsl_to_rgb) M(gauss_a_to_rgba) M(emboss)
...
#undef M
  ...
  void run(size_t x, size_t y, size_t w, size_t h) const noexcept;
 ...
 private:
  ...
  StartPipelineFn build_pipeline(void**) const noexcept;
