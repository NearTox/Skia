...
#define SK_RASTER_PIPELINE_STAGES(M)                                                                                                  \
  M(callback)                                                                                                                         \
  M(interpreter) M(move_src_dst) M(move_dst_src) M(clamp_0) M(clamp_1) M(clamp_a) M(                                                  \
      clamp_gamut) M(unpremul) M(premul) M(premul_dst) M(force_opaque) M(force_opaque_dst)                                            \
      M(set_rgb) M(unbounded_set_rgb) M(swap_rb) M(swap_rb_dst) M(from_srgb) M(to_srgb) M(                                            \
          ...
          M(dither) M(load_a8) M(load_a8_dst) M(store_a8) M(gather_a8) M(load_565) M(                                                 \
              load_565_dst) M(store_565) M(gather_565) M(load_4444) M(load_4444_dst) M(store_4444)                                    \
              M(gather_4444) M(load_f16) M(load_f16_dst) M(store_f16) M(gather_f16) M(load_af16) M(store_af16) M(                     \
                  ...
                  M(load_rgf32) M(store_rgf32) M(load_8888) M(load_8888_dst) M(store_8888) M(gather_8888) M(load_rg88) M(             \
                      ...
                      M(store_16161616) M(load_1010102) M(load_1010102_dst) M(store_1010102) M(                                       \
                          ...
                          M(bilerp_clamp_8888) M(store_u16_be) M(load_src) M(store_src) M(load_dst) M(                                \
                              store_dst) M(scale_u8) M(scale_565) M(scale_1_float)                                                    \
                                          ...
                                          M(luminosity) M(srcover_rgba_8888) M(matrix_translate) M(matrix_scale_translate) M(         \
                                              ...
                                              M(matrix_4x5) M(                                                                        \
                                                  ...
                                                  M(gamma_) M(mirror_x) M(repeat_x) M(mirror_y) M(repeat_y) M(                        \
                                                      decal_x) M(decal_y) M(decal_x_and_y) M(check_decal_mask) M(negate_x)            \
                                                      M(bilinear_nx) M(bilinear_px) M(bilinear_ny) M(                                 \
