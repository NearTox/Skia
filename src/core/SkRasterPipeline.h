...
#define SK_RASTER_PIPELINE_STAGES(M)                                                                                  \
  ...
  M(clamp_0) M(clamp_1) M(clamp_a) M(clamp_gamut) M(unpremul) M(premul) M(premul_dst) M(                              \
      ...
      M(from_srgb) M(to_srgb) M(black_color) M(white_color) M(uniform_color) M(                                       \
          unbounded_uniform_color) M(seed_shader) M(dither) M(load_a8) M(load_a8_dst) M(store_a8)                     \
          M(gather_a8) M(load_565) M(load_565_dst) M(store_565) M(gather_565) M(load_4444) M(                         \
              ...
              M(gather_f16) M(load_f32) M(load_f32_dst) M(store_f32) M(gather_f32) M(load_8888) M(                    \
                  load_8888_dst) M(store_8888) M(gather_8888) M(load_1010102) M(load_1010102_dst)                     \
                  M(store_1010102) M(gather_1010102) M(alpha_to_gray) M(alpha_to_gray_dst) M(                         \
                      luminance_to_alpha) M(bilerp_clamp_8888) M(store_u16_be) M(load_src)                            \
                      M(store_src) M(load_dst) M(store_dst) M(scale_u8) M(scale_565) M(                               \
                          scale_1_float) M(lerp_u8) M(lerp_565) M(lerp_1_float) M(lerp_native)                        \
                          M(dstatop) M(dstin) M(dstout) M(dstover) M(srcatop) M(srcin) M(                             \
                              srcout) M(srcover) M(clear) M(modulate) M(multiply) M(plus_) M(screen) M(xor_)          \
                              M(colorburn) M(colordodge) M(darken) M(difference) M(exclusion) M(hardlight) M(         \
