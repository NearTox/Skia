...
SkColorSpace* SkPixmap::colorSpace() const noexcept { return fInfo.colorSpace(); }

sk_sp<SkColorSpace> SkPixmap::refColorSpace() const noexcept { return fInfo.refColorSpace(); }
...
float SkPixmap::getAlphaf(int x, int y) const {
  ...
  switch (this->colorType()) {
    ...
    case kAlpha_8_SkColorType: value = static_cast<const uint8_t*>(srcPtr)[0] * (1.0f / 255); break;
    ...
    case kRGBA_F32_SkColorType: value = static_cast<const float*>(srcPtr)[3]; break;
  }
  return value;
}

bool SkPixmap::readPixels(
    const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB, int x, int y) const {
...
bool SkPixmap::scalePixels(const SkPixmap& actualDst, const SkSamplingOptions& sampling) const {
  ...
  SkPixmap src = *this, dst = actualDst;
...
SkColor SkPixmap::getColor(int x, int y) const {
  ...
  switch (this->colorType()) {
    ...
    case kR8G8_unorm_SkColorType: {
      uint16_t value = *this->addr16(x, y);
      return (uint32_t)(((value >> 0) & 0xff)) << 16 | (uint32_t)(((value >> 8) & 0xff)) << 8 |
             0xff000000;
    }
    case kR16G16_unorm_SkColorType: {
    ...
    case kRGB_101010x_SkColorType: {
      ...
      return (uint32_t)(((value >> 0) & 0x3ff) * (255 / 1023.0f)) << 16 |
             ...
             (uint32_t)(((value >> 20) & 0x3ff) * (255 / 1023.0f)) << 0 | 0xff000000;
    }
    case kBGR_101010x_SkColorType: {
      uint32_t value = *this->addr32(x, y);
      ...
      return (uint32_t)(((value >> 0) & 0x3ff) * (255 / 1023.0f)) << 0 |
             (uint32_t)(((value >> 10) & 0x3ff) * (255 / 1023.0f)) << 8 |
             (uint32_t)(((value >> 20) & 0x3ff) * (255 / 1023.0f)) << 16 | 0xff000000;
    }
    case kRGBA_1010102_SkColorType:
    case kBGRA_1010102_SkColorType: {
      uint32_t value = *this->addr32(x, y);

      float r = ((value >> 0) & 0x3ff) * (1 / 1023.0f), g = ((value >> 10) & 0x3ff) * (1 / 1023.0f),
            b = ((value >> 20) & 0x3ff) * (1 / 1023.0f), a = ((value >> 30) & 0x3) * (1 / 3.0f);
    ...
    case kR16G16B16A16_unorm_SkColorType: {
      ...
      float r = ((value)&0xffff) * (1 / 65535.0f), g = ((value >> 16) & 0xffff) * (1 / 65535.0f),
...
bool SkPixmap::computeIsOpaque() const noexcept {
  ...
  switch (this->colorType()) {
    ...
    case kRGB_101010x_SkColorType:
    case kBGR_101010x_SkColorType:
    case kR8_unorm_SkColorType: return true; break;
    case kARGB_4444_SkColorType: {
      unsigned c = 0xFFFF;
      for (int y = 0; y < height; ++y) {
    ...
    case kUnknown_SkColorType: SkDEBUGFAIL(""); break;
  }
  return false;
}
