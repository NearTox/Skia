...
enum class SkBackend : uint8_t { kGanesh, kGraphite, kSkVM };
...
class SkPaintParamsKey {
 public:
  ...
  class BlockReader {
   ...
   private:
    ...
    BlockReader(
        const SkShaderCodeDictionary*, SkSpan<const uint8_t> parentSpan, int offsetInParent);
 ...
 private:
  ...
  static void AddBlockToShaderInfo(
      SkShaderCodeDictionary*, const SkPaintParamsKey::BlockReader&, SkShaderInfo*);
...
class SkPaintParamsKeyBuilder {
 public:
  ...
  ~SkPaintParamsKeyBuilder() { SkASSERT(!this->isLocked()); }
  ...
  void addByte(uint8_t data) { this->addBytes(1, &data); }
