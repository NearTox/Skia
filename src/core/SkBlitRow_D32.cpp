...
static void blit_row_s32_blend(SkPMColor* dst, const SkPMColor* src, int count, U8CPU alpha) {
    ...
    while (count >= 4) {
        _mm_storeu_si128(
                dst4,
                SkPMLerp_SSE2(
                        _mm_loadu_si128(src4), _mm_loadu_si128(dst4), SkAlpha255To256(alpha)));
        src4++;
        dst4++;
        count -= 4;
    }
...
static void blit_row_s32a_blend(SkPMColor* dst, const SkPMColor* src, int count, U8CPU alpha) {
    ...
    while (count >= 4) {
        _mm_storeu_si128(dst4,
                         SkBlendARGB32_SSE2(_mm_loadu_si128(src4), _mm_loadu_si128(dst4), alpha));
        src4++;
        dst4++;
        count -= 4;
    }
...
SkBlitRow::Proc32 SkBlitRow::Factory32(unsigned flags) {
    ...
    return flags == 2 ? SkOpts::blit_row_s32a_opaque : kProcs[flags];
}
