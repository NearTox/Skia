 ...
 public:
  class Scan {
   public:
    ...
    void blur(
        const AlphaIter srcBegin, const AlphaIter srcEnd, uint8_t* dst, int dstStride,
...
static void blur_x_radius_1(
    const fp88& s0, const fp88& g0, const fp88& g1, const fp88&, const fp88&, const fp88&, fp88* d0,
    fp88* d8) {
  auto v1 = mulhi(s0, g1);
  auto v0 = mulhi(s0, g0);
...
static void blur_x_radius_2(
    const fp88& s0, const fp88& g0, const fp88& g1, const fp88& g2, const fp88&, const fp88&,
    fp88* d0, fp88* d8) {
  auto v0 = mulhi(s0, g0);
  auto v1 = mulhi(s0, g1);
  auto v2 = mulhi(s0, g2);
...
static void blur_x_radius_3(
    const fp88& s0, const fp88& g0, const fp88& g1, const fp88& g2, const fp88& g3, const fp88&,
    fp88* d0, fp88* d8) {
  auto v0 = mulhi(s0, g0);
  auto v1 = mulhi(s0, g1);
  auto v2 = mulhi(s0, g2);
...
static void blur_x_radius_4(
    const fp88& s0, const fp88& g0, const fp88& g1, const fp88& g2, const fp88& g3, const fp88& g4,
    fp88* d0, fp88* d8) {
  auto v0 = mulhi(s0, g0);
  auto v1 = mulhi(s0, g1);
  auto v2 = mulhi(s0, g2);
...
static void blur_row(
    BlurX blur, const fp88& g0, const fp88& g1, const fp88& g2, const fp88& g3, const fp88& g4,
    const uint8_t* src, int srcW, uint8_t* dst, int dstW) {
...
// BlurX will only be one of the functions blur_x_radius_(1|2|3|4).
static void blur_x_rect(
    BlurX blur, uint16_t* gauss, const uint8_t* src, size_t srcStride, int srcW, uint8_t* dst,
    size_t dstStride, int dstW, int dstH) {
  fp88 g0(gauss[0]), g1(gauss[1]), g2(gauss[2]), g3(gauss[3]), g4(gauss[4]);
...
static void direct_blur_x(
    int radius, uint16_t* gauss, const uint8_t* src, size_t srcStride, int srcW, uint8_t* dst,
    size_t dstStride, int dstW, int dstH) {
  switch (radius) {
    case 1:
...
static fp88 blur_y_radius_1(
    const fp88& s0, const fp88& g0, const fp88& g1, const fp88&, const fp88&, const fp88&,
...
static void blur_column(
    ToA8 toA8, BlurY blur, int radius, int width, const fp88& g0, const fp88& g1, const fp88& g2,
    const fp88& g3, const fp88& g4, const uint8_t* src, size_t srcRB, int srcH, uint8_t* dst,
    size_t dstRB) {
  fp88 d01(kHalf), d12(kHalf), d23(kHalf), d34(kHalf), d45(kHalf), d56(kHalf), d67(kHalf),
      d78(kHalf);
...
// BlurY will be one of blur_y_radius_(1|2|3|4).
static void blur_y_rect(
    ToA8 toA8, const int strideOf8, BlurY blur, int radius, uint16_t* gauss, const uint8_t* src,
    size_t srcRB, int srcW, int srcH, uint8_t* dst, size_t dstRB) {
  fp88 g0(gauss[0]), g1(gauss[1]), g2(gauss[2]), g3(gauss[3]), g4(gauss[4]);
...
static void direct_blur_y(
    ToA8 toA8, const int strideOf8, int radius, uint16_t* gauss, const uint8_t* src, size_t srcRB,
    int srcW, int srcH, uint8_t* dst, size_t dstRB) {
  switch (radius) {
    case 1:
  ...
  *dst = SkMask::PrepareDestination(radiusX, radiusY, src);
  ...
  switch (src.fFormat) {
    case SkMask::kBW_Format:
      direct_blur_y(
          bw_to_a8, 1, radiusY, gaussFactorsY, src.fImage, srcRB, srcW, srcH, dst->fImage + radiusX,
          dstRB);
      break;
    case SkMask::kA8_Format:
      direct_blur_y(
          nullptr, 8, radiusY, gaussFactorsY, src.fImage, srcRB, srcW, srcH, dst->fImage + radiusX,
          dstRB);
      break;
    case SkMask::kARGB32_Format:
      direct_blur_y(
          argb32_to_a8, 32, radiusY, gaussFactorsY, src.fImage, srcRB, srcW, srcH,
          dst->fImage + radiusX, dstRB);
      break;
    case SkMask::kLCD16_Format:
      direct_blur_y(
          lcd_to_a8, 16, radiusY, gaussFactorsY, src.fImage, srcRB, srcW, srcH,
          dst->fImage + radiusX, dstRB);
      break;
    default: SK_ABORT("Unhandled format.");
  }

  // Blur horizontally in place.
  direct_blur_x(
      radiusX, gaussFactorsX, dst->fImage + radiusX, dstRB, srcW, dst->fImage, dstRB, dstW, dstH);
...
SkIPoint SkMaskBlurFilter::blur(const SkMask& src, SkMask* dst) const {
  ...
  *dst = SkMask::PrepareDestination(borderW, borderH, src);
  ...
  for (int y = 0; y < tmpH; y++) {
    ...
    scanH.blur(
        tmpStart, tmpStart + tmpW, dstStart, dst->fRowBytes, dstStart + dst->fRowBytes * dstH);
  }
