...
class SkSpecialSurface_Gpu : public SkSpecialSurface_Base {
 public:
  SkSpecialSurface_Gpu(
      GrRecordingContext* context, sk_sp<GrRenderTargetContext> renderTargetContext, int width,
    ...
    sk_sp<SkBaseDevice> device(SkGpuDevice::Make(
        context->priv().backdoor(), fRenderTargetContext, width, height,
...
#  endif
  ...
  sk_sp<SkSpecialImage> onMakeImageSnapshot() override {
    ...
    sk_sp<SkSpecialImage> tmp(SkSpecialImage::MakeDeferredFromGpu(
        ...
        fRenderTargetContext->colorSpaceInfo().refColorSpace(), &this->props()));
...
sk_sp<SkSpecialSurface> SkSpecialSurface::MakeRenderTarget(
  ...
  return sk_make_sp<SkSpecialSurface_Gpu>(
      context, std::move(renderTargetContext), width, height, subset);
}
