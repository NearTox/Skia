...
static void nofilter_scale(const SkBitmapProcState& s, uint32_t xy[], int count, int x, int y) {
  ...
  for (; count >= 2; count -= 2) {
    *xy++ = pack_two_shorts(
        tile(SkFractionalIntToFixed(fx), maxX), tile(SkFractionalIntToFixed(fx + dx), maxX));
    fx += dx + dx;
  }
...
template <
    unsigned (*tile)(SkFixed, int), int16x8_t (*tile8)(int32x4_t, int32x4_t, unsigned),
    bool tryDecal>
static void nofilter_scale_neon(
...
template <
    unsigned (*tile)(SkFixed, int), int32x4_t (*tile4)(int32x4_t, unsigned),
    unsigned (*extract_low_bits)(SkFixed, int), int32x4_t (*extract_low_bits4)(int32x4_t, unsigned),
    bool tryDecal>
...
static const SkBitmapProcState::MatrixProc RepeatX_RepeatY_Procs[] = {
    ...
    filter_scale_neon<
        repeat, repeat4, extract_low_bits_repeat_mirror, extract_low_bits_repeat_mirror4, false>,
};
