 ...
 */
static inline bool can_truncate_to_fixed_for_decal(
    SkFixed fx, SkFixed dx, int count, unsigned max) {
  SkASSERT(count > 0);
...
static void nofilter_scale(const SkBitmapProcState& s, uint32_t xy[], int count, int x, int y) {
  ...
  for (; count >= 2; count -= 2) {
    *xy++ = pack_two_shorts(
        tilex(SkFractionalIntToFixed(fx), maxX), tilex(SkFractionalIntToFixed(fx + dx), maxX));
    fx += dx + dx;
  }
...
static void nofilter_affine(const SkBitmapProcState& s, uint32_t xy[], int count, int x, int y) {
  ...
  SkFractionalInt fx = mapper.fractionalIntX(), fy = mapper.fractionalIntY(),
                  dx = s.fInvSxFractionalInt, dy = s.fInvKyFractionalInt;
  int maxX = s.fPixmap.width() - 1, maxY = s.fPixmap.height() - 1;

  while (count-- > 0) {
    *xy++ =
        (tiley(SkFractionalIntToFixed(fy), maxY) << 16) | (tilex(SkFractionalIntToFixed(fx), maxX));
    fx += dx;
    fy += dy;
  }
}
...
template <
    unsigned (*tilex)(SkFixed, int), unsigned (*tiley)(SkFixed, int),
...
static void filter_scale(const SkBitmapProcState& s, uint32_t xy[], int count, int x, int y) {
  ...
  if (tryDecal && (unsigned)SkFractionalIntToInt(fx) < maxX &&
...
static void filter_affine(const SkBitmapProcState& s, uint32_t xy[], int count, int x, int y) {
  ...
  SkFixed oneX = s.fFilterOneX, oneY = s.fFilterOneY;

  SkFractionalInt fx = mapper.fractionalIntX(), fy = mapper.fractionalIntY(),
                  dx = s.fInvSxFractionalInt, dy = s.fInvKyFractionalInt;
  unsigned maxX = s.fPixmap.width() - 1, maxY = s.fPixmap.height() - 1;
  while (count-- > 0) {
...
static inline unsigned SK_USHIFT16(unsigned x) { return x >> 16; }
...
static unsigned clamp(SkFixed fx, int max) { return SkClampMax(fx >> 16, max); }

#if defined(SK_SUPPORT_LEGACY_TILED_BITMAPS)
// For use with extract_low_bits_general(), where clamp() above expects
// extract_low_bits_clamp_clamp()
static unsigned general_clamp(SkFixed fx, int max) { return repeat(SkClampMax(fx, 0xFFFF), max); }
#endif
...
static const SkBitmapProcState::MatrixProc RepeatX_RepeatY_Procs[] = {
    ...
    filter_scale<repeat, repeat, extract_low_bits_general, false>, nofilter_affine<repeat, repeat>,
...
static inline U16CPU int_repeat(int x, int n) { return sk_int_mod(x, n); }
...
static void clampx_nofilter_trans(
    const SkBitmapProcState& s, uint32_t xy[], int count, int x, int y) {
...
template <U16CPU(tiley)(int x, int n)>
static void repeatx_nofilter_trans(
    const SkBitmapProcState& s, uint32_t xy[], int count, int x, int y) {
  SkASSERT(s.fInvMatrix.isTranslate());
...
template <U16CPU(tiley)(int x, int n)>
static void mirrorx_nofilter_trans(
    const SkBitmapProcState& s, uint32_t xy[], int count, int x, int y) {
  SkASSERT(s.fInvMatrix.isTranslate());
...
static const SkBitmapProcState::MatrixProc GeneralProcs[] = {
    ...
    clampx_nofilter_trans<int_repeat>, clampx_nofilter_trans<int_mirror>,
    repeatx_nofilter_trans<int_clamp>, repeatx_nofilter_trans<int_mirror>,
    mirrorx_nofilter_trans<int_clamp>, mirrorx_nofilter_trans<int_repeat>,
    // below proc entries are configured using nofilter_scale< tilex, tiley, tryDecal >
    nofilter_scale<general_clamp, repeat, false>, nofilter_scale<general_clamp, mirror, false>,
    nofilter_scale<repeat, general_clamp, false>, nofilter_scale<repeat, mirror, false>,
    nofilter_scale<mirror, general_clamp, false>, nofilter_scale<mirror, repeat, false>,
    // below proc entries are configured using filter_scale< tilex, tiley, extract_low_bits,
    // tryDecal >
    ...
    nofilter_affine<general_clamp, repeat>, nofilter_affine<general_clamp, mirror>,
    nofilter_affine<repeat, general_clamp>, nofilter_affine<repeat, mirror>,
    nofilter_affine<mirror, general_clamp>, nofilter_affine<mirror, repeat>,
