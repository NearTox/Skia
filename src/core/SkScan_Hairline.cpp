...
void SkScan::HairLineRgn(
    const SkPoint array[], int arrayCount, const SkRegion* clip, SkBlitter* origBlitter) {
  ...
  for (int i = 0; i < arrayCount - 1; ++i) {
    ...
    if (clip) {
      ...
      clipR.setLTRB(
          SkIntToFDot6(bounds.fLeft), SkIntToFDot6(bounds.fTop), SkIntToFDot6(bounds.fRight),
          SkIntToFDot6(bounds.fBottom));
...
void SkScan::HairRect(const SkRect& rect, const SkRasterClip& clip, SkBlitter* blitter) {
  ...
  SkIRect r = SkIRect::MakeLTRB(
      SkScalarFloorToInt(rect.fLeft), SkScalarFloorToInt(rect.fTop),
      SkScalarFloorToInt(rect.fRight + 1), SkScalarFloorToInt(rect.fBottom + 1));
...
static void hair_quad(
    const SkPoint pts[3], const SkRegion* clip, SkBlitter* blitter, int level,
...
static bool is_inverted(const SkRect& r) { return r.fLeft > r.fRight || r.fTop > r.fBottom; }
...
static inline void hairquad(
    const SkPoint pts[3], const SkRegion* clip, const SkRect* insetClip, const SkRect* outsetClip,
...
static inline Sk2s abs(const Sk2s& value) { return Sk2s::Max(value, Sk2s(0) - value); }
...
static bool quick_cubic_niceness_check(const SkPoint pts[4]) {
  return lt_90(pts[1], pts[0], pts[3]) && lt_90(pts[2], pts[0], pts[3]) &&
         lt_90(pts[1], pts[3], pts[0]) && lt_90(pts[2], pts[3], pts[0]);
}
...
static void hair_cubic(
    const SkPoint pts[4], const SkRegion* clip, SkBlitter* blitter, SkScan::HairRgnProc lineproc) {
...
static inline void haircubic(
    const SkPoint pts[4], const SkRegion* clip, const SkRect* insetClip, const SkRect* outsetClip,
    SkBlitter* blitter, int level, SkScan::HairRgnProc lineproc) {
  if (insetClip) {
    SkASSERT(outsetClip);
...
template <SkPaint::Cap capStyle>
void hair_path(
    const SkPathView& path, const SkRasterClip& rclip, SkBlitter* blitter,
    SkScan::HairRgnProc lineproc) {
  if (path.isEmpty()) {
    return;
  }
  ...
  while (iter != end) {
    ...
    switch (verb) {
      case SkPath::kMove_Verb: firstPt = lastPt = pts[0]; break;
    ...
    if (SkPaint::kButt_Cap != capStyle) {
      if (prevVerb == SkPath::kMove_Verb && verb >= SkPath::kLine_Verb &&
...
void SkScan::AntiHairSquarePath(
    const SkPathView& path, const SkRasterClip& clip, SkBlitter* blitter) {
...
void SkScan::FrameRect(
    const SkRect& r, const SkPoint& strokeSize, const SkRasterClip& clip, SkBlitter* blitter) {
...
void SkScan::HairLine(
    const SkPoint pts[], int count, const SkRasterClip& clip, SkBlitter* blitter) {
  if (clip.isBW()) {
