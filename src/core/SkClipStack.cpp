...
SkClipStack::Element::Element(const Element& that) {
  switch (that.getDeviceSpaceType()) {
    ...
    case DeviceSpaceType::kRRect:
      ...
      break;
    case DeviceSpaceType::kPath: fDeviceSpacePath.set(that.getDeviceSpacePath()); break;
  }
...
SkClipStack::Element::~Element() {
  ...
  for (int i = 0; i < fKeysToInvalidate.count(); ++i) {
    fProxyProvider->processInvalidUniqueKey(
        fKeysToInvalidate[i], nullptr, GrProxyProvider::InvalidateGPUResource::kYes);
  }
#endif
}
...
bool SkClipStack::Element::operator==(const Element& element) const noexcept {
  ...
  switch (fDeviceSpaceType) {
    case DeviceSpaceType::kPath: return this->getDeviceSpacePath() == element.getDeviceSpacePath();
    case DeviceSpaceType::kRRect: return fDeviceSpaceRRect == element.fDeviceSpaceRRect;
    case DeviceSpaceType::kRect: return this->getDeviceSpaceRect() == element.getDeviceSpaceRect();
    case DeviceSpaceType::kEmpty: return true;
...
const SkRect& SkClipStack::Element::getBounds() const noexcept {
  ...
  switch (fDeviceSpaceType) {
    ...
    case DeviceSpaceType::kEmpty: return kEmpty;
    default: SkDEBUGFAIL("Unexpected type."); return kEmpty;
  }
}
...
bool SkClipStack::Element::contains(const SkRect& rect) const noexcept {
  switch (fDeviceSpaceType) {
    ...
    case DeviceSpaceType::kEmpty: return false;
    default: SkDEBUGFAIL("Unexpected type."); return false;
  }
}
...
void SkClipStack::Element::initRect(
    int saveCount, const SkRect& rect, const SkMatrix& m, SkClipOp op, bool doAA) {
  if (m.rectStaysRect()) {
    SkRect devRect;
...
void SkClipStack::Element::initRRect(
    int saveCount, const SkRRect& rrect, const SkMatrix& m, SkClipOp op, bool doAA) {
...
void SkClipStack::Element::asDeviceSpacePath(SkPath* path) const {
  switch (fDeviceSpaceType) {
    case DeviceSpaceType::kEmpty: path->reset(); break;
    case DeviceSpaceType::kRect:
      path->reset();
...
bool SkClipStack::Element::canBeIntersectedInPlace(int saveCount, SkClipOp op) const noexcept {
  ...
  return fSaveCount == saveCount && kIntersect_SkClipOp == op &&
...
void SkClipStack::Element::combineBoundsDiff(
  ...
  switch (combination) {
    ...
    case kPrev_Cur_FillCombo:
      ...
      break;
    default: SkDEBUGFAIL("SkClipStack::Element::combineBoundsDiff Invalid fill combination"); break;
  }
}
...
void SkClipStack::Element::combineBoundsXOR(int combination, const SkRect& prevFinite) noexcept {
  switch (combination) {
    ...
    case kPrev_Cur_FillCombo:
      ...
      fFiniteBoundType = kNormal_BoundsType;
      break;
    default: SkDEBUGFAIL("SkClipStack::Element::combineBoundsXOR Invalid fill combination"); break;
  }
}
...
void SkClipStack::Element::combineBoundsUnion(int combination, const SkRect& prevFinite) noexcept {
  switch (combination) {
    ...
    case kPrev_InvCur_FillCombo:
      ...
      break;
    case kPrev_Cur_FillCombo: fFiniteBound.join(prevFinite); break;
    default:
...
void SkClipStack::Element::updateBoundAndGenID(const Element* prior) noexcept {
  ...
  SkASSERT(
      kInvPrev_InvCur_FillCombo == combination || kInvPrev_Cur_FillCombo == combination ||
      kPrev_InvCur_FillCombo == combination || kPrev_Cur_FillCombo == combination);

  // Now integrate with clip with the prior clips
  switch (fOp) {
    case kDifference_SkClipOp: this->combineBoundsDiff(combination, prevFinite); break;
    case kXOR_SkClipOp: this->combineBoundsXOR(combination, prevFinite); break;
    case kUnion_SkClipOp: this->combineBoundsUnion(combination, prevFinite); break;
    case kIntersect_SkClipOp: this->combineBoundsIntersection(combination, prevFinite); break;
    case kReverseDifference_SkClipOp: this->combineBoundsRevDiff(combination, prevFinite); break;
...
SkClipStack& SkClipStack::operator=(const SkClipStack& b) {
  ...
  for (const Element* element = (const Element*)recIter.next(); element != nullptr;
...
bool SkClipStack::internalQuickContains(const SkRect& rect) const noexcept {
...
bool SkClipStack::internalQuickContains(const SkRRect& rrect) const noexcept {
  Iter iter(*this, Iter::kTop_IterStart);
  const Element* element = iter.prev();
  while (element != nullptr) {
    if (kIntersect_SkClipOp != element->getOp() && kReplace_SkClipOp != element->getOp())
        ...
        case Element::DeviceSpaceType::kRect:
          if (Element::DeviceSpaceType::kRect == element.getDeviceSpaceType()) {
            if (prior->rectRectIntersectAllowed(element.getDeviceSpaceRect(), element.isAA())) {
              SkRect isectRect;
              if (!isectRect.intersect(prior->getDeviceSpaceRect(), element.getDeviceSpaceRect())) {
                prior->setEmpty();
                return;
              }
...
void SkClipStack::clipRRect(const SkRRect& rrect, const SkMatrix& matrix, SkClipOp op, bool doAA) {
  ...
  if (this->hasClipRestriction(op)) {
    Element restriction(
        fSaveCount, fClipRestrictionRect, SkMatrix::I(), kIntersect_SkClipOp, false);
...
const SkClipStack::Element* SkClipStack::Iter::skipToTopmost(SkClipOp op) noexcept {
  ...
  for (element = (const SkClipStack::Element*)fIter.prev(); element;
...
bool SkClipStack::isRRect(const SkRect& bounds, SkRRect* rrect, bool* aa) const noexcept {
  ...
  if (back->getOp() == kIntersect_SkClipOp) {
    ...
    if (cnt > 1) {
      ...
      while (const Element* prior = (const Element*)iter.prev()) {
        if ((prior->getOp() != kIntersect_SkClipOp && prior->getOp() != kReplace_SkClipOp) ||
...
void SkClipStack::Element::dump() const {
  static const char* kTypeStrings[] = {"empty", "rect", "rrect", "path"};
  ...
  static const char* kOpStrings[] = {
      "difference", "intersect", "union", "xor", "reverse-difference", "replace",
  };
  ...
  switch (fDeviceSpaceType) {
    case DeviceSpaceType::kEmpty: SkDebugf("\n"); break;
    case DeviceSpaceType::kRect:
