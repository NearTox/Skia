...
bool GetSpotShadowTransform(
    const SkPoint3& lightPos, SkScalar lightRadius, const SkMatrix& ctm,
    const SkPoint3& zPlaneParams, const SkRect& pathBounds, bool directional,
  ...
  if (!ctm.hasPerspective() || directional) {
    ...
    if (directional) {
      SkDrawShadowMetrics::GetDirectionalParams(
          occluderHeight, lightPos.fX, lightPos.fY, lightPos.fZ, lightRadius, radius, &scale,
          &translate);
    } else {
    ...
    shadowTransform->setAll(
        h0.fX / h2.fZ, h1.fX / h2.fZ, h2.fX / h2.fZ, h0.fY / h2.fZ, h1.fY / h2.fZ, h2.fY / h2.fZ,
    ...
    toHomogeneous.setAll(
        xScale, 0, -xScale * pathBounds.fLeft - 1, 0, yScale, -yScale * pathBounds.fTop - 1, 0, 0,
        1);
...
void GetLocalBounds(
    const SkPath& path, const SkDrawShadowRec& rec, const SkMatrix& ctm, SkRect* bounds) {
  ...
  if (ctm.hasPerspective()) {
    ...
    if (SkToBool(rec.fFlags & SkShadowFlags::kDirectionalLight_ShadowFlag)) {
      ...
      lightDir.normalize();
      SkDrawShadowMetrics::GetDirectionalParams(
          occluderZ, lightDir.fX, lightDir.fY, lightDir.fZ, rec.fLightRadius, &spotBlur, &spotScale,
          &spotOffset);
    } else {
      SkPoint devLightPos = SkPoint::Make(rec.fLightPos.fX, rec.fLightPos.fY);
      ctm.mapPoints(&devLightPos, 1);
      SkDrawShadowMetrics::GetSpotParams(
          occluderZ, devLightPos.fX, devLightPos.fY, rec.fLightPos.fZ, rec.fLightRadius, &spotBlur,
    ...
    if (SkToBool(rec.fFlags & SkShadowFlags::kDirectionalLight_ShadowFlag)) {
      ...
      SkDrawShadowMetrics::GetSpotParams(
          occluderZ, rec.fLightPos.fX, rec.fLightPos.fY, rec.fLightPos.fZ, rec.fLightRadius,
