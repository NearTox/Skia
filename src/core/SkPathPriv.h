...
public:
    ...
    static FirstDirection AsFirstDirection(SkPath::Direction dir) noexcept {
    ...
    static void AddGenIDChangeListener(const SkPath& path,
                                       sk_sp<SkPathRef::GenIDChangeListener> listener) noexcept {
     ...
     * oval.
     */
    static bool DrawArcIsConvex(SkScalar sweepAngle, bool useCenter,
    ...
    */
     ...
     */
    static bool IsOval(const SkPath& path, SkRect* rect, SkPath::Direction* dir,
                       unsigned* start) noexcept {
        bool isCCW = false;
    ...
    static bool TooBigForMath(const SkRect& bounds) noexcept {
        ...
        return !(bounds.fLeft >= -max && bounds.fTop >= -max && bounds.fRight <= max &&
                 bounds.fBottom <= max);
    }
    static bool TooBigForMath(const SkPath& path) noexcept {
