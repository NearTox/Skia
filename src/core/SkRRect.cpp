...
void SkRRect::setRectXY(const SkRect& rect, SkScalar xRad, SkScalar yRad) noexcept {
  ...
  if (fRect.width() < xRad + xRad || fRect.height() < yRad + yRad) {
    ...
    SkScalar scale = SkMinScalar(
        sk_ieee_float_divide(fRect.width(), xRad + xRad),
...
bool SkRRect::checkCornerContainment(SkScalar x, SkScalar y) const noexcept {
  ...
  if (kOval_Type == this->type()) {
    ...
    if (x < fRect.fLeft + fRadii[kUpperLeft_Corner].fX &&
      ...
      canonicalPt.set(
          x - (fRect.fLeft + fRadii[kUpperLeft_Corner].fX),
          y - (fRect.fTop + fRadii[kUpperLeft_Corner].fY));
      SkASSERT(canonicalPt.fX < 0 && canonicalPt.fY < 0);
    } else if (
        x < fRect.fLeft + fRadii[kLowerLeft_Corner].fX &&
      ...
      SkASSERT(canonicalPt.fX < 0 && canonicalPt.fY > 0);
        ...
        x > fRect.fRight - fRadii[kUpperRight_Corner].fX &&
      ...
      SkASSERT(canonicalPt.fX > 0 && canonicalPt.fY < 0);
        ...
        x > fRect.fRight - fRadii[kLowerRight_Corner].fX &&
 ...
 */
static constexpr bool are_radius_check_predicates_valid(SkScalar rad, SkScalar min, SkScalar max) {
