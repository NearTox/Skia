...
template <
    typename T, bool Forward, bool Const, IndexFn Start, IndexFn End, NextFn Next,
    ItemFn<
        T, typename std::conditional<
               Const, const SkBlockAllocator::Block, SkBlockAllocator::Block>::type>
        Resolve>
...
class SkTBlockList {
 public:
   ...
   */
  explicit SkTBlockList(
      int itemsPerBlock,
      SkBlockAllocator::GrowthPolicy policy = SkBlockAllocator::GrowthPolicy::kFixed)
      : fAllocator(
            policy, SkBlockAllocator::BlockOverhead<alignof(T)>() + sizeof(T) * itemsPerBlock) {}
   ...
   */
  T& push_back() { return *new (this->pushItem()) T; }
  T& push_back(const T& t) { return *new (this->pushItem()) T(t); }
  T& push_back(T&& t) { return *new (this->pushItem()) T(std::move(t)); }
  ...
  const T& item(int i) const { return const_cast<SkTBlockList*>(this)->item(i); }
 ...
 private:
  ...
  static int First(const SkBlockAllocator::Block* b) { return b->firstAlignedOffset<alignof(T)>(); }
  static int Last(const SkBlockAllocator::Block* b) { return b->metadata(); }
  static int Increment(const SkBlockAllocator::Block* b, int index) { return index + sizeof(T); }
  static int Decrement(const SkBlockAllocator::Block* b, int index) { return index - sizeof(T); }
  ...
  void* pushItem() {
    ...
    SkASSERT(
        br.fStart == br.fAlignedOffset || br.fAlignedOffset == First(fAllocator->currentBlock()));
...
void SkTBlockList<T, SI1>::concat(SkTBlockList<T, SI2>&& other) {
  ...
  if (headBlock->metadata() > 0) {
    ...
    if (headItemCount > avail) {
      ...
      fAllocator->template reserve<alignof(T)>(
          (headItemCount - avail) * sizeof(T),
          SkBlockAllocator::kIgnoreExistingBytes_Flag | SkBlockAllocator::kIgnoreGrowthPolicy_Flag);
    }
    ...
    if constexpr (std::is_trivially_copy_constructible<T>::value) {
      ...
      if (headItemCount > avail) {
        ...
        copy(
            headBlock, headStart + avail * sizeof(T), fAllocator.allocator(),
            headItemCount - avail);
      }
  ...
  SkASSERT(
      other.fAllocator->headBlock()->metadata() == 0 &&
      fAllocator->metadata() == oldCount + headItemCount);
  fAllocator->stealHeapBlocks(other.fAllocator.allocator());
  fAllocator->setMetadata(fAllocator->metadata() + (other.fAllocator->metadata() - headItemCount));
 ...
 */
template <
    typename T,     // The element type (including any modifiers)
  ...
  class Item {
   public:
    ...
    Item& operator++() {
      ...
      SkASSERT(
          (Forward && Next(block, fIndex) > fIndex) || (!Forward && Next(block, fIndex) < fIndex));
