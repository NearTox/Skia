 ...
 public:
  ...
  class Block final {
   ...
   private:
    ...
    void poisonRange(int start, int end) noexcept {
      sk_asan_poison_memory_region(reinterpret_cast<char*>(this) + start, end - start);
    }
    void unpoisonRange(int start, int end) noexcept {
...
class SkSBlockAllocator : SkNoncopyable {
 public:
  ...
  ~SkSBlockAllocator() { this->allocator()->~SkBlockAllocator(); }

  SkBlockAllocator* operator->() noexcept { return this->allocator(); }
  const SkBlockAllocator* operator->() const noexcept { return this->allocator(); }

  SkBlockAllocator* allocator() noexcept { return reinterpret_cast<SkBlockAllocator*>(fStorage); }
  const SkBlockAllocator* allocator() const noexcept {
...
SkBlockAllocator::ByteRange SkBlockAllocator::allocate(size_t size) {
  ...
  static_assert(
      kMaxAllocationSize + kBlockOverhead + ((1 << 12) - 1)  // 4K align for large blocks
  ...
  SkASSERT(
      offset - fTail->fCursor >= (int)Padding &&
...
SkBlockAllocator::BlockIter<true, false> SkBlockAllocator::blocks() noexcept {
  return BlockIter<true, false>(this);
}
SkBlockAllocator::BlockIter<true, true> SkBlockAllocator::blocks() const noexcept {
  return BlockIter<true, true>(this);
}
SkBlockAllocator::BlockIter<false, false> SkBlockAllocator::rblocks() noexcept {
  return BlockIter<false, false>(this);
}
SkBlockAllocator::BlockIter<false, true> SkBlockAllocator::rblocks() const noexcept {
