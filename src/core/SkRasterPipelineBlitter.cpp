...
class SkRasterPipelineBlitter final : public SkBlitter {
 public:
  ...
  static SkBlitter* Create(
      const SkPixmap&, const SkPaint&, SkArenaAlloc*, const SkRasterPipeline& shaderPipeline,
      bool is_opaque, bool is_constant, sk_sp<SkShader> clipShader);
 ...
 private:
  ...
  SkRasterPipeline_MemoryCtx fDstPtr = {nullptr, 0},  // Always points to the top-left of fDst.
  ...
  std::function<void(size_t, size_t, size_t, size_t)> fBlitRect, fBlitAntiH, fBlitMaskA8,
      fBlitMaskLCD16, fBlitMask3D;
...
SkBlitter* SkCreateRasterPipelineBlitter(
    const SkPixmap& dst, const SkPaint& paint, const SkMatrixProvider& matrixProvider,
    SkArenaAlloc* alloc, sk_sp<SkShader> clipShader) {
  SkColorSpace* dstCS = dst.colorSpace();
  SkColorType dstCT = dst.colorType();
  SkColor4f paintColor = paint.getColor4f();
  SkColorSpaceXformSteps(sk_srgb_singleton(), kUnpremul_SkAlphaType, dstCS, kUnpremul_SkAlphaType)
  ...
  if (!shader) {
    ...
    bool is_opaque = paintColor.fA == 1.0f, is_constant = true;
    return SkRasterPipelineBlitter::Create(
        dst, paint, alloc, shaderPipeline, is_opaque, is_constant, std::move(clipShader));
  }
  ...
  if (shader->appendStages(
    ...
    if (paintColor.fA != 1.0f) {
      shaderPipeline.append(SkRasterPipeline::scale_1_float, alloc->make<float>(paintColor.fA));
    }
    return SkRasterPipelineBlitter::Create(
        dst, paint, alloc, shaderPipeline, is_opaque, is_constant, std::move(clipShader));
  }
...
SkBlitter* SkCreateRasterPipelineBlitter(
    ...
    bool is_opaque, SkArenaAlloc* alloc, sk_sp<SkShader> clipShader) {
  bool is_constant = false;  // If this were the case, it'd be better to just set a paint color.
  return SkRasterPipelineBlitter::Create(
      dst, paint, alloc, shaderPipeline, is_opaque, is_constant, clipShader);
}

SkBlitter* SkRasterPipelineBlitter::Create(
    const SkPixmap& dst, const SkPaint& paint, SkArenaAlloc* alloc,
    const SkRasterPipeline& shaderPipeline, bool is_opaque, bool is_constant,
  ...
  if (auto colorFilter = paint.getColorFilter()) {
    ...
    is_opaque =
        is_opaque && (as_CFB(colorFilter)->getFlags() & SkColorFilterBase::kAlphaUnchanged_Flag);
  }
...
void SkRasterPipelineBlitter::blitRect(int x, int y, int w, int h) {
  ...
  if (!fBlitRect) {
    ...
    if (fBlend == SkBlendMode::kSrcOver &&
        ...
        !fDst.colorSpace() && fDst.info().alphaType() != kUnpremul_SkAlphaType &&
        fDitherRate == 0.0f) {
...
void SkRasterPipelineBlitter::blitMask(const SkMask& mask, const SkIRect& clip) {
  ...
  SkASSERT(
      mask.fFormat == SkMask::kA8_Format || mask.fFormat == SkMask::kLCD16_Format ||
  ...
  auto extract_mask_plane = [&mask](int plane, SkRasterPipeline_MemoryCtx* ctx) {
    ...
    auto ptr = (uintptr_t)mask.fImage + plane * mask.computeImageSize();
    ...
    ctx->pixels = (void*)(ptr - mask.fBounds.left() * bpp - mask.fBounds.top() * rowBytes);
  };
