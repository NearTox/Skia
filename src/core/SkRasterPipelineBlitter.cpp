...
SkBlitter* SkCreateRasterPipelineBlitter(
  ...
  if (!shader) {
    ...
    return SkRasterPipelineBlitter::Create(
        dst, paint, alloc, shaderPipeline, is_opaque, is_constant);
  }
...
SkBlitter* SkCreateRasterPipelineBlitter(
    ...
    bool is_opaque, SkArenaAlloc* alloc) {
  bool is_constant = false;  // If this were the case, it'd be better to just set a paint color.
  return SkRasterPipelineBlitter::Create(dst, paint, alloc, shaderPipeline, is_opaque, is_constant);
}
