...
class SkMatrixPriv {
 public:
  ...
  InverseMapRect(const SkMatrix& mx, SkRect* dst, const SkRect& src) {
  ...
  */
  ...
  /** Maps src SkPoint array of length count to dst SkPoint array, skipping stride bytes
      ...
      @param count   number of points to transform
  */
  static void MapPointsWithStride(
      const SkMatrix& mx, SkPoint dst[], size_t dstStride, const SkPoint src[], size_t srcStride,
      int count) {
