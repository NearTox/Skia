...
#define EQUAL(field) (a.field == b.field)
  return EQUAL(fPathEffect) && EQUAL(fShader) && EQUAL(fMaskFilter) && EQUAL(fColorFilter) &&
         EQUAL(fBlender) && EQUAL(fImageFilter) && EQUAL(fColor4f) && EQUAL(fWidth) &&
...
static uint32_t pack_v68(const SkPaint& paint, unsigned flatFlags) {
  ...
  const unsigned mode = bm ? static_cast<unsigned>(bm.value()) : CUSTOM_BLEND_MODE_SENTINEL;
...
void SkPaintPriv::Flatten(const SkPaint& paint, SkWriteBuffer& buffer) {
  ...
  if (paint.getPathEffect() || paint.getShader() || paint.getMaskFilter() ||
      paint.getColorFilter() || paint.getImageFilter() || !paint.asBlendMode()) {
...
bool SkPaint::canComputeFastBounds() const {
  ...
  if (this->getPathEffect() && !as_PEB(this->getPathEffect())->computeFastBounds(nullptr)) {
    return false;
  }
  return true;
}
