...
SkBaseDevice* SkTextBlobCacheDiffCanvas::TrackLayerDevice::onCreateDevice(
  ...
  return new TrackLayerDevice(
      this->getGlobalBounds(), surfaceProps, fStrikeServer, cinfo.fInfo.refColorSpace(), fSettings);
}
...
void SkTextBlobCacheDiffCanvas::TrackLayerDevice::drawGlyphRunList(
  ...
  fPainter.processGlyphRunList(
      glyphRunList, this->ctm(), this->surfaceProps(), fSettings.fContextSupportsDistanceFieldText,
      options, nullptr);
...
SkTextBlobCacheDiffCanvas::SkTextBlobCacheDiffCanvas(
    int width, int height, const SkSurfaceProps& props, SkStrikeServer* strikeServer,
    sk_sp<SkColorSpace> colorSpace, Settings settings)
    : SkNoDrawCanvas{sk_make_sp<TrackLayerDevice>(
          SkIRect::MakeWH(width, height), props, strikeServer, std::move(colorSpace), settings)} {}
...
void SkStrikeServer::SkGlyphCacheState::writeGlyphPath(
...
SkSpan<const SkGlyphPos> SkStrikeServer::SkGlyphCacheState::prepareForDrawing(
    ...
    PreparationDetail detail, SkGlyphPos results[]) {
