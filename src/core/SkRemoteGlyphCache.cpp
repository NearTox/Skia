...
static SkDescriptor* auto_descriptor_from_desc(
    const SkDescriptor* source_desc, SkFontID font_id, SkAutoDescriptor* ad) {
...
static const SkDescriptor* create_descriptor(
    const SkPaint& paint, const SkFont& font, const SkMatrix& m, const SkSurfaceProps& props,
    SkScalerContextFlags flags, SkAutoDescriptor* ad, SkScalerContextEffects* effects) {
  SkScalerContextRec rec;
...
size_t MapOps::operator()(const SkDescriptor* key) const { return key->getChecksum(); }
...
void RemoteStrike::commonMaskLoop(
    SkDrawableGlyphBuffer* drawables, SkSourceGlyphBuffer* rejects, Rejector&& reject) {
  drawables->forEachGlyphID([&](size_t i, SkPackedGlyphID packedID, SkPoint position) {
...
void RemoteStrike::prepareForSDFTDrawing(
    SkDrawableGlyphBuffer* drawables, SkSourceGlyphBuffer* rejects) {
  this->commonMaskLoop(
      drawables, rejects, [](MaskSummary summary) { return !summary.canDrawAsSDFT; });
}
...
RemoteStrike* SkStrikeServerImpl::getOrCreateCache(
    const SkPaint& paint, const SkFont& font, const SkSurfaceProps& props, const SkMatrix& matrix,
    SkScalerContextFlags flags, SkScalerContextEffects* effects) {
...
SkScopedStrikeForGPU SkStrikeServerImpl::findOrCreateScopedStrike(
...
class GlyphTrackingDevice final : public SkNoPixelsDevice {
 ...
 protected:
  void onDrawGlyphRunList(const SkGlyphRunList& glyphRunList, const SkPaint& paint) override {
    ...
    GrSDFTControl control = GrSDFTControl{
        fDFTSupport, this->surfaceProps().isUseDeviceIndependentFonts(),
    ...
    for (auto& glyphRun : glyphRunList) {
      fPainter.processGlyphRun(glyphRun, drawMatrix, paint, control, nullptr, "Cache Diff");
    }
...
std::unique_ptr<SkCanvas> SkStrikeServer::makeAnalysisCanvas(
  ...
  sk_sp<SkBaseDevice> trackingDevice(new GlyphTrackingDevice(
      SkISize::Make(width, height), props, this->impl(), std::move(colorSpace), DFTSupport));
...
sk_sp<SkTypeface> SkStrikeClientImpl::addTypeface(const WireTypeface& wire) {
  ...
  auto newTypeface = sk_make_sp<SkTypefaceProxy>(
      wire.typefaceID, wire.glyphCount, wire.style, wire.isFixed, fDiscardableHandleManager,
      fIsLogging);
