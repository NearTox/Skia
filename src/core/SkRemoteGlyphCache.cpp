...
static const SkDescriptor* create_descriptor(const SkPaint& paint, const SkFont& font,
                                             const SkMatrix& m, const SkSurfaceProps& props,
                                             SkScalerContextFlags flags, SkAutoDescriptor* ad,
...
SkBaseDevice* SkTextBlobCacheDiffCanvas::TrackLayerDevice::onCreateDevice(const CreateInfo& cinfo,
                                                                          const SkPaint*) {
...
SkStrikeServer::SkGlyphCacheState* SkStrikeServer::getOrCreateCache(
    ...
    if (!fCachedTypefaces.contains(typefaceId)) {
        ...
        fTypefacesToSend.emplace_back(typefaceId, typeface.countGlyphs(), typeface.fontStyle(),
...
// -- SkGlyphCacheState ----------------------------------------------------------------------------
SkStrikeServer::SkGlyphCacheState::SkGlyphCacheState(const SkDescriptor& descriptor,
...
void SkStrikeServer::SkGlyphCacheState::setTypefaceAndEffects(const SkTypeface* typeface,
                                                              SkScalerContextEffects effects) {
    fTypeface = typeface;
    fEffects = effects;
}
...
const SkGlyph& SkStrikeServer::SkGlyphCacheState::getGlyphMetrics(SkGlyphID glyphID,
                                                                  SkPoint position) {
...
sk_sp<SkTypeface> SkStrikeClient::addTypeface(const WireTypeface& wire) {
    ...
    auto newTypeface =
            sk_make_sp<SkTypefaceProxy>(wire.typefaceID, wire.glyphCount, wire.style, wire.isFixed,
