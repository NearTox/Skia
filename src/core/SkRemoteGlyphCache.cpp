 ...
 public:
  ...
  SkBaseDevice* onCreateDevice(const CreateInfo& cinfo, const SkPaint*) override {
    ...
    return new TrackLayerDevice(
        this->getGlobalBounds(), surfaceProps, fStrikeServer, cinfo.fInfo.refColorSpace(),
        fDFTSupport);
  }
 ...
 protected:
  void drawGlyphRunList(const SkGlyphRunList& glyphRunList) override {
    ...
    fPainter.processGlyphRunList(
        glyphRunList, this->ctm(), this->surfaceProps(), fDFTSupport, options, nullptr);
...
SkTextBlobCacheDiffCanvas::SkTextBlobCacheDiffCanvas(
    ...
    : SkNoDrawCanvas{sk_make_sp<TrackLayerDevice>(
          SkIRect::MakeWH(width, height), props, strikeServer, std::move(colorSpace), DFTSupport)} {
}
...
#else
      [&serializer](RemoteStrike* strike) { strike->writePendingGlyphs(&serializer); }
#endif
  );
...
SkScopedStrike SkStrikeServer::findOrCreateScopedStrike(
...
void SkStrikeServer::AddGlyphForTesting(RemoteStrike* cache, SkPackedGlyphID glyphID, bool asPath) {
...
void SkStrikeServer::RemoteStrike::writeGlyphPath(
    const SkPackedGlyphID& glyphID, Serializer* serializer) const {
  SkPath path;
...
SkSpan<const SkGlyphPos> SkStrikeServer::RemoteStrike::prepareForDrawingRemoveEmpty(
    const SkPackedGlyphID packedGlyphIDs[], const SkPoint positions[], size_t n, int maxDimension,
    SkGlyphPos results[]) {
...
#define READ_FAILURE                                                                              \
  {                                                                                               \
    ...
    DiscardableHandleManager::ReadFailureData data = {memorySize,       deserializer.bytesRead(), \
                                                      typefaceSize,     strikeCount,              \
                                                      glyphImagesCount, glyphPathsCount};         \
    ...
    if (strike == nullptr) {
      ...
      strike = fStrikeCache->createStrikeExclusive(
          *client_desc, std::move(scaler), &fontMetrics,
          skstd::make_unique<DiscardableStrikePinner>(
