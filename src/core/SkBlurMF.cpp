...
class SkBlurMaskFilterImpl : public SkMaskFilterBase {
 public:
  ...
  bool canFilterMaskGPU(
      const GrShape& shape, const SkIRect& devSpaceShapeBounds, const SkIRect& clipBounds,
...
bool SkBlurMaskFilterImpl::directFilterMaskGPU(
    ...
    const GrClip& clip, const SkMatrix& viewMatrix, const GrShape& shape) const {
  ...
  if (!this->ignoreXform()) {
    ...
    paint.addCoverageFragmentProcessor(std::move(fp));
    renderTargetContext->drawVertices(
        clip, std::move(paint), viewMatrix, std::move(vertices), nullptr, 0);
  } else {
    SkMatrix inverse;
...
GrSurfaceProxyView SkBlurMaskFilterImpl::filterMaskGPU(
  ...
  if (!isNormalBlur) {
    ...
    paint.addCoverageFragmentProcessor(GrTextureEffect::Make(srcView.detachProxy(), srcAlphaType));
    if (kInner_SkBlurStyle == fBlurStyle) {
      // inner:  dst = dst * src
