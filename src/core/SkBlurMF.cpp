...
bool SkComputeBlurredRRectParams(
    ...
    SkRRect* rrectToDraw, SkISize* widthHeight, SkScalar rectXs[kSkBlurRRectMaxDivisions],
    SkScalar rectYs[kSkBlurRRectMaxDivisions], SkScalar texXs[kSkBlurRRectMaxDivisions],
...
bool SkBlurMaskFilterImpl::directFilterMaskGPU(
  ...
  if (canBeRect || canBeCircle) {
    if (canBeRect) {
      fp = GrRectBlurEffect::Make(
          /*inputFP=*/nullptr, context, *context->priv().caps()->shaderCaps(), srcRRect.rect(),
      ...
      fp = GrCircleBlurFragmentProcessor::Make(
          /*inputFP=*/nullptr, context, devBounds, xformedSigma);
    }
  ...
  fp = GrRRectBlurEffect::Make(
      /*inputFP=*/nullptr, context, fSigma, xformedSigma, srcRRect, devRRect);
  if (!fp) {
    return false;
  }
