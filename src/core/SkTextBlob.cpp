...
size_t SkTextBlob::RunRecord::StorageSize(
...
const SkTextBlob::RunRecord* SkTextBlob::RunRecord::First(const SkTextBlob* blob) noexcept {
...
const SkTextBlob::RunRecord* SkTextBlob::RunRecord::Next(const RunRecord* run) noexcept {
...
void SkTextBlob::RunRecord::validate(const uint8_t* storageTop) const noexcept {
  ...
  SkASSERT(posBuffer() + fCount * ScalarsPerGlyph(positioning()) <= (SkScalar*)NextUnchecked(this));
  if (isExtended()) {
    SkASSERT(textSize() > 0);
  ...
  static_assert(
      sizeof(SkTextBlob::RunRecord) == sizeof(RunRecordStorageEquivalent),
      "runrecord_should_stay_packed");
}

const SkTextBlob::RunRecord* SkTextBlob::RunRecord::NextUnchecked(const RunRecord* run) noexcept {
  SkSafeMath safe;
...
size_t SkTextBlob::RunRecord::PosCount(
...
uint32_t* SkTextBlob::RunRecord::textSizePtr() const noexcept {
...
void SkTextBlob::RunRecord::grow(uint32_t count) noexcept {
...
SkTextBlob::SkTextBlob(const SkRect& bounds) noexcept
...
SkTextBlobRunIterator::SkTextBlobRunIterator(const SkTextBlob* blob) noexcept
    : fCurrentRun(SkTextBlob::RunRecord::First(blob)) {
  SkDEBUGCODE(fStorageTop = (uint8_t*)blob + blob->fStorageSize);
}

void SkTextBlobRunIterator::next() noexcept {
  SkASSERT(!this->done());
...
SkTextBlobRunIterator::GlyphPositioning SkTextBlobRunIterator::positioning() const noexcept {
  SkASSERT(!this->done());
  static_assert(
      static_cast<GlyphPositioning>(SkTextBlob::kDefault_Positioning) == kDefault_Positioning, "");
  static_assert(
      static_cast<GlyphPositioning>(SkTextBlob::kHorizontal_Positioning) == kHorizontal_Positioning,
      "");
  static_assert(
      static_cast<GlyphPositioning>(SkTextBlob::kFull_Positioning) == kFull_Positioning, "");
  static_assert(
      static_cast<GlyphPositioning>(SkTextBlob::kRSXform_Positioning) == kRSXform_Positioning, "");

  return SkTo<GlyphPositioning>(fCurrentRun->positioning());
}

bool SkTextBlobRunIterator::isLCD() const noexcept {
  return fCurrentRun->font().getEdging() == SkFont::Edging::kSubpixelAntiAlias;
}

SkTextBlobBuilder::SkTextBlobBuilder() noexcept
...
SkRect SkTextBlobBuilder::TightRunBounds(const SkTextBlob::RunRecord& run) {
  ...
  if (SkTextBlob::kDefault_Positioning == run.positioning()) {
    font.measureText(
        run.glyphBuffer(), run.glyphCount() * sizeof(uint16_t), SkTextEncoding::kGlyphID, &bounds);
  ...
  SkASSERT(
      SkTextBlob::kFull_Positioning == run.positioning() ||
  ...
  constexpr SkScalar horizontalConstY = 0;
  const SkScalar* glyphPosX = run.posBuffer();
  const SkScalar* glyphPosY =
      (run.positioning() == SkTextBlob::kFull_Positioning) ? glyphPosX + 1 : &horizontalConstY;
...
SkRect SkTextBlobBuilder::ConservativeRunBounds(const SkTextBlob::RunRecord& run) {
  ...
  switch (run.positioning()) {
    ...
    default: SK_ABORT("unsupported positioning mode");
  }
...
void SkTextBlobBuilder::updateDeferredBounds() {
  ...
  SkRect runBounds = SkTextBlob::kDefault_Positioning == run->positioning()
                         ? TightRunBounds(*run)
                         : ConservativeRunBounds(*run);
  fBounds.join(runBounds);
  fDeferredBounds = false;
}

void SkTextBlobBuilder::reserve(size_t size) noexcept {
  SkSafeMath safe;
...
bool SkTextBlobBuilder::mergeRun(
  ...
  if (run->positioning() != positioning || run->font() != font ||
  ...
  if (SkTextBlob::kFull_Positioning != positioning &&
      (SkTextBlob::kHorizontal_Positioning != positioning || run->offset().y() != offset.y())) {
    return false;
  }
  ...
  fCurrentRunBuffer.pos =
      run->posBuffer() + preMergeCount * SkTextBlob::ScalarsPerGlyph(positioning);
...
void SkTextBlobBuilder::allocInternal(
    const SkFont& font, SkTextBlob::GlyphPositioning positioning, int count, int textSize,
...
const SkTextBlobBuilder::RunBuffer& SkTextBlobBuilder::allocRun(
    const SkFont& font, int count, SkScalar x, SkScalar y, const SkRect* bounds) {
  this->allocInternal(font, SkTextBlob::kDefault_Positioning, count, 0, {x, y}, bounds);
  return fCurrentRunBuffer;
}

const SkTextBlobBuilder::RunBuffer& SkTextBlobBuilder::allocRunPosH(
    const SkFont& font, int count, SkScalar y, const SkRect* bounds) {
  this->allocInternal(font, SkTextBlob::kHorizontal_Positioning, count, 0, {0, y}, bounds);
  return fCurrentRunBuffer;
}

const SkTextBlobBuilder::RunBuffer& SkTextBlobBuilder::allocRunPos(
    const SkFont& font, int count, const SkRect* bounds) {
  this->allocInternal(font, SkTextBlob::kFull_Positioning, count, 0, {0, 0}, bounds);
  return fCurrentRunBuffer;
}

const SkTextBlobBuilder::RunBuffer& SkTextBlobBuilder::allocRunRSXform(
    const SkFont& font, int count) {
  this->allocInternal(font, SkTextBlob::kRSXform_Positioning, count, 0, {0, 0}, nullptr);
  return fCurrentRunBuffer;
}

const SkTextBlobBuilder::RunBuffer& SkTextBlobBuilder::allocRunText(
    const SkFont& font, int count, SkScalar x, SkScalar y, int textByteCount, SkString lang,
    const SkRect* bounds) {
  this->allocInternal(
      font, SkTextBlob::kDefault_Positioning, count, textByteCount, SkPoint::Make(x, y), bounds);
  return fCurrentRunBuffer;
}

const SkTextBlobBuilder::RunBuffer& SkTextBlobBuilder::allocRunTextPosH(
    const SkFont& font, int count, SkScalar y, int textByteCount, SkString lang,
    const SkRect* bounds) {
  this->allocInternal(
      font, SkTextBlob::kHorizontal_Positioning, count, textByteCount, SkPoint::Make(0, y), bounds);
  return fCurrentRunBuffer;
}

const SkTextBlobBuilder::RunBuffer& SkTextBlobBuilder::allocRunTextPos(
    const SkFont& font, int count, int textByteCount, SkString lang, const SkRect* bounds) {
  this->allocInternal(
      font, SkTextBlob::kFull_Positioning, count, textByteCount, SkPoint::Make(0, 0), bounds);
  return fCurrentRunBuffer;
}

const SkTextBlobBuilder::RunBuffer& SkTextBlobBuilder::allocRunRSXform(
    const SkFont& font, int count, int textByteCount, SkString lang, const SkRect* bounds) {
  this->allocInternal(font, SkTextBlob::kRSXform_Positioning, count, textByteCount, {0, 0}, bounds);
  return fCurrentRunBuffer;
}
  ...
  while (!it.done()) {
    ...
    buffer.writeByteArray(
        ...
        it.glyphCount() * sizeof(SkScalar) *
            SkTextBlob::ScalarsPerGlyph(SkTo<SkTextBlob::GlyphPositioning>(it.positioning())));
    if (pe.extended) {
...
sk_sp<SkTextBlob> SkTextBlobPriv::MakeFromBuffer(SkReadBuffer& reader) {
  ...
  for (;;) {
    ...
    const size_t glyphSize = safe.mul(glyphCount, sizeof(uint16_t)),
                 posSize = safe.mul(
                     glyphCount, safe.mul(sizeof(SkScalar), SkTextBlob::ScalarsPerGlyph(pos))),
    ...
    switch (pos) {
      case SkTextBlob::kDefault_Positioning:
        buf = &blobBuilder.allocRunText(
            font, glyphCount, offset.x(), offset.y(), textSize, SkString(), &bounds);
        break;
      case SkTextBlob::kHorizontal_Positioning:
        buf = &blobBuilder.allocRunTextPosH(
            font, glyphCount, offset.y(), textSize, SkString(), &bounds);
        break;
      case SkTextBlob::kFull_Positioning:
...
sk_sp<SkTextBlob> SkTextBlob::MakeFromText(
    const void* text, size_t byteLength, const SkFont& font, SkTextEncoding encoding) {
...
sk_sp<SkTextBlob> SkTextBlob::MakeFromPosText(
...
sk_sp<SkTextBlob> SkTextBlob::MakeFromPosTextH(
...
sk_sp<SkTextBlob> SkTextBlob::MakeFromRSXform(
...
sk_sp<SkTextBlob> SkTextBlob::Deserialize(
    const void* data, size_t length, const SkDeserialProcs& procs) {
...
namespace {
int get_glyph_run_intercepts(
    const SkGlyphRun& glyphRun, const SkPaint& paint, const SkScalar bounds[2],
    SkScalar intervals[], int* intervalCount) {
  ...
  for (const SkGlyph* glyph : metricsAndPaths.glyphs(glyphRun.glyphsIDs())) {
    ...
    if (glyph->path() != nullptr) {
      ...
      metricsAndPaths.findIntercepts(scaledBounds, scale, pos.x(), glyph, intervals, intervalCount);
    }
  }
...
int SkTextBlob::getIntercepts(
    const SkScalar bounds[2], SkScalar intervals[], const SkPaint* paint) const {
...
SkTextBlob::Iter::Iter(const SkTextBlob& blob) noexcept { fRunRecord = RunRecord::First(&blob); }

bool SkTextBlob::Iter::next(Run* rec) noexcept {
  if (fRunRecord) {
    if (rec) {
