...
skvm::Color SkModeColorFilter::onProgram(
    skvm::Builder* p, skvm::Color c, const SkColorInfo& dstInfo, skvm::Uniforms* uniforms,
    SkArenaAlloc*) const {
  SkColor4f color = SkColor4f::FromColor(fColor);
  SkColorSpaceXformSteps(
      sk_srgb_singleton(), kUnpremul_SkAlphaType, dstInfo.colorSpace(), kPremul_SkAlphaType)
      .apply(color.vec());
...
GrFPResult SkModeColorFilter::asFragmentProcessor(
  ...
  SkASSERT(
      fMode > SkBlendMode::kLastCoeffMode ||
...
sk_sp<SkColorFilter> SkColorFilters::Blend(SkColor color, SkBlendMode mode) {
  ...
  if (SkBlendMode::kDst == mode ||
      (0 == alpha && (SkBlendMode::kSrcOver == mode || SkBlendMode::kDstOver == mode ||
                      SkBlendMode::kDstOut == mode || SkBlendMode::kSrcATop == mode ||
                      SkBlendMode::kXor == mode || SkBlendMode::kDarken == mode)) ||
