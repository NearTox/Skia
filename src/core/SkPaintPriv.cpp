...
bool SkPaintPriv::Overwrites(const SkPaint* paint, ShaderOverrideOpacity overrideOpacity) noexcept {
  if (!paint) {
...
void SkPaintPriv::RemoveColorFilter(SkPaint* p, SkColorSpace* dstCS) {
  if (SkColorFilter* filter = p->getColorFilter()) {
    if (SkShader* shader = p->getShader()) {
      ...
      p->setShader(
          sk_make_sp<SkColorFilterShader>(sk_ref_sp(shader), p->getAlphaf(), sk_ref_sp(filter)));
      p->setAlphaf(1.0f);
    } else {
