...
class SkStrikeServer {
 public:
  ...
  SK_API std::unique_ptr<SkCanvas> makeAnalysisCanvas(
 ...
 public:
  ...
  class DiscardableHandleManager : public SkRefCnt {
   public:
    ...
    virtual void notifyCacheMiss(CacheMissType type, int fontSize) { this->notifyCacheMiss(type); }
  ...
  SK_SPI explicit SkStrikeClient(
      sk_sp<DiscardableHandleManager>, bool isLogging = true, SkStrikeCache* strikeCache = nullptr);
