...
static bool rect_memcpy(
    const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB, const SkImageInfo& srcInfo,
    const void* srcPixels, size_t srcRB, const SkColorSpaceXformSteps& steps) {
  // We can copy the pixels when no color type, alpha type, or color space changes.
  if (dstInfo.colorType() != srcInfo.colorType()) {
    return false;
  }
  if (dstInfo.colorType() != kAlpha_8_SkColorType && steps.flags.mask() != 0b00000) {
    return false;
  }
...
static bool swizzle_or_premul(
    const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB, const SkImageInfo& srcInfo,
    const void* srcPixels, size_t srcRB, const SkColorSpaceXformSteps& steps) {
  auto is_8888 = [](SkColorType ct) {
    return ct == kRGBA_8888_SkColorType || ct == kBGRA_8888_SkColorType;
  };
  if (!is_8888(dstInfo.colorType()) || !is_8888(srcInfo.colorType()) || steps.flags.linearize ||
      steps.flags.gamut_transform || steps.flags.unpremul || steps.flags.encode) {
    return false;
  }
  ...
  if (steps.flags.premul) {
    fn = swapRB ? SkOpts::RGBA_to_bgrA : SkOpts::RGBA_to_rgbA;
  } else {
...
static bool convert_to_alpha8(
    const SkImageInfo& dstInfo, void* vdst, size_t dstRB, const SkImageInfo& srcInfo,
    const void* src, size_t srcRB, const SkColorSpaceXformSteps&) {
...
static void convert_with_pipeline(
    const SkImageInfo& dstInfo, void* dstRow, int dstStride, const SkImageInfo& srcInfo,
    const void* srcRow, int srcStride, const SkColorSpaceXformSteps& steps) {
  SkRasterPipeline_MemoryCtx src = {(void*)srcRow, srcStride}, dst = {(void*)dstRow, dstStride};
...
bool SkConvertPixels(
    const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB, const SkImageInfo& srcInfo,
    const void* srcPixels, size_t srcRB) {
