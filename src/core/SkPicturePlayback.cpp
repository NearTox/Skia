...
void SkPicturePlayback::draw(
  ...
  while (!reader.eof() && reader.isValid()) {
    ...
    uint32_t op = bits >> 24, size = bits & 0xffffff;
    if (size == 0xffffff) {
      size = reader.readInt();
    }
...
static bool do_clip_op(
    SkReadBuffer* reader, SkCanvas* canvas, SkRegion::Op op, SkClipOp* clipOpToUse) {
  switch (op) {
...
#define BREAK_ON_READ_ERROR(r) \
  ...
  switch (op) {
    ...
    case DRAW_EDGEAA_IMAGE_SET2: {
      ...
      SkCanvas::SrcRectConstraint constraint = reader->checkRange(
      ...
      if (!reader->validate((maxMatrixIndex + 1) <= matrixCount) ||
          !reader->validate(SkSafeMath::Mul(matrixCount, kMatrixSize) <= reader->available())) {
      ...
      canvas->experimental_DrawEdgeAAImageSet(
          set.get(), cnt, dstClips, matrices.begin(), sampling, paint, constraint);
    } break;
    case DRAW_IMAGE: {
      const SkPaint* paint = fPictureData->optionalPaint(reader);
      ...
      canvas->drawImage(image, loc.fX, loc.fY, SkSamplingOptions(SkFilterMode::kNearest), paint);
    } break;
    case DRAW_IMAGE2: {
      const SkPaint* paint = fPictureData->optionalPaint(reader);
      const SkImage* image = fPictureData->getImage(reader);
    ...
    case DRAW_PATCH: {
      ...
      const SkPoint* cubics =
          (const SkPoint*)reader->skip(SkPatchUtils::kNumCtrlPts, sizeof(SkPoint));
    ...
    case DRAW_POINTS: {
      ...
      SkCanvas::PointMode mode =
          reader->checkRange(SkCanvas::kPoints_PointMode, SkCanvas::kPolygon_PointMode);
