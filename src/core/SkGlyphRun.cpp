...
SkSpan<const SkGlyphID> SkGlyphIDSet::uniquifyGlyphIDs(
    uint32_t universeSize, SkSpan<const SkGlyphID> glyphIDs, SkGlyphID* uniqueGlyphIDs,
...
void SkGlyphRunBuilder::drawTextUTF8(
    const SkPaint& paint, const SkFont& font, const void* bytes, size_t byteLength,
    SkPoint origin) {
...
void SkGlyphRunBuilder::drawTextBlob(
    const SkPaint& paint, const SkTextBlob& blob, SkPoint origin, SkBaseDevice* device) {
  ...
  for (SkTextBlobRunIterator it(&blob); !it.done(); it.next()) {
    if (it.positioning() != SkTextBlobRunIterator::kRSXform_Positioning) {
      ...
      device->drawGlyphRunRSXform(
          it.font(), it.glyphs(), (const SkRSXform*)it.pos(), it.glyphCount(), origin, paint);
...
void SkGlyphRunBuilder::simplifyTextBlobIgnoringRSXForm(
  ...
  switch (it.positioning()) {
    ...
    case SkTextBlobRunIterator::kHorizontal_Positioning: {
      ...
      this->simplifyDrawPosTextH(it.font(), glyphIDs, it.pos(), constY, positions, text, clusters);
      break;
    }
    case SkTextBlobRunIterator::kFull_Positioning: {
      this->simplifyDrawPosText(it.font(), glyphIDs, (const SkPoint*)it.pos(), text, clusters);
      break;
    }
...
void SkGlyphRunBuilder::simplifyDrawText(
  ...
  if (!glyphIDs.empty()) {
    ...
    this->makeGlyphRun(font, glyphIDs, SkSpan<const SkPoint>{positions, runSize}, text, clusters);
  }
}

void SkGlyphRunBuilder::simplifyDrawPosTextH(
    const SkFont& font, SkSpan<const SkGlyphID> glyphIDs, const SkScalar* xpos, SkScalar constY,
