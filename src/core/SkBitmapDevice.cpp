...
static bool valid_for_bitmap_device(const SkImageInfo& info, SkAlphaType* newAlphaType) {
  ...
  if (newAlphaType) {
    *newAlphaType =
        SkColorTypeIsAlwaysOpaque(info.colorType()) ? kOpaque_SkAlphaType : info.alphaType();
  }
...
SkBitmapDevice::SkBitmapDevice(const SkBitmap& bitmap)
    : INHERITED(bitmap.info(), SkSurfaceProps(SkSurfaceProps::kLegacyFontHost_InitType)),
      ...
      fGlyphPainter(
          this->surfaceProps(), bitmap.colorType(), bitmap.colorSpace(),
...
SkBitmapDevice* SkBitmapDevice::Create(
    const SkImageInfo& origInfo, const SkSurfaceProps& surfaceProps, bool trackCoverage,
  ...
  if (trackCoverage) {
    SkImageInfo ci =
        SkImageInfo::Make(info.width(), info.height(), kAlpha_8_SkColorType, kPremul_SkAlphaType);
...
SkBaseDevice* SkBitmapDevice::onCreateDevice(const CreateInfo& cinfo, const SkPaint*) {
  ...
  return SkBitmapDevice::Create(cinfo.fInfo, surfaceProps, cinfo.fTrackCoverage, cinfo.fAllocator);
}
...
void SkBitmapDevice::drawPaint(const SkPaint& paint) { BDDraw(this).drawPaint(paint); }

void SkBitmapDevice::drawPoints(
...
void SkBitmapDevice::drawBitmap(
    const SkBitmap& bitmap, const SkMatrix& matrix, const SkRect* dstOrNull, const SkPaint& paint) {
...
void SkBitmapDevice::drawBitmapRect(
  ...
  if (src && !src->contains(bitmapBounds) && SkCanvas::kFast_SrcRectConstraint == constraint &&
  ...
  if (src) {
    ...
    SkRect extractedBitmapBounds = SkRect::MakeXYWH(
        dx, dy, SkIntToScalar(bitmapPtr->width()), SkIntToScalar(bitmapPtr->height()));
#else
...
USE_SHADER:
  ...
  auto s = SkMakeBitmapShaderForPaint(
      paint, *bitmapPtr, SkTileMode::kClamp, SkTileMode::kClamp, &matrix, kNever_SkCopyPixelsMode);
  if (!s) {
    return;
  }
...
class SkAutoDeviceClipRestore {
 public:
  ...
  ~SkAutoDeviceClipRestore() { fDevice->restore(fPrevCTM); }
...
void SkBitmapDevice::drawSpecial(
  ...
  if (SkImageFilter* filter = paint->getImageFilter()) {
    ...
    const SkMatrix matrix =
        SkMatrix::Concat(SkMatrix::MakeTrans(SkIntToScalar(-x), SkIntToScalar(-y)), this->ctm());
    const SkIRect clipBounds = fRCStack.rc().getBounds().makeOffset(-x, -y);
    sk_sp<SkImageFilterCache> cache(this->getImageFilterCache());
    SkImageFilter_Base::Context ctx(
        matrix, clipBounds, cache.get(), fBitmap.colorType(), fBitmap.colorSpace(), src);
  ...
  if (clipImage->isAlphaOnly() && totalMatrix.invert(&totalInverse)) {
    ...
    sk_sp<SkSurface> surf =
        SkSurface::MakeRaster(SkImageInfo::MakeA8(maskBounds.width(), maskBounds.height()));
  ...
  this->drawImageRect(
      mask.get(), nullptr,
      SkRect::MakeXYWH(maskBounds.x(), maskBounds.y(), mask->width(), mask->height()), *paint,
...
void SkBitmapDevice::onSave() { fRCStack.save(); }

void SkBitmapDevice::onRestore() { fRCStack.restore(); }
