...
static void convolve_gaussian_1d(
  ...
  if (bounds) {
    switch (direction) {
      case Direction::kX:
        subset.fLeft = bounds[0];
        subset.fRight = bounds[1];
        break;
      case Direction::kY:
        subset.fTop = bounds[0];
        subset.fBottom = bounds[1];
        break;
    }
  }
  ...
  renderTargetContext->fillRectToRect(
      nullptr, std::move(paint), GrAA::kNo, SkMatrix::I(), SkRect::Make(rtcRect), srcRect);
}
  ...
  for (int i = 1; i < scaleFactorX || i < scaleFactorY; i *= 2) {
    ...
    if (i == 1) {
      ...
      fp = GrTextureEffect::Make(
          std::move(srcView), srcAlphaType, SkMatrix::I(), GrSamplerState::Filter::kLinear);
    }
...
std::unique_ptr<GrRenderTargetContext> LegacyGaussianBlur(
    GrRecordingContext* context, GrSurfaceProxyView srcView, GrColorType srcColorType,
