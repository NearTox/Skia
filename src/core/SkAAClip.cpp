...
class Iter {
 public:
  ...
  Iter(int y, const uint8_t* data, const YOffset* start, const YOffset* end) noexcept
      : fCurrYOff(start),
        fStopYOff(end),
...
struct SkAAClip::RunHead {
  ...
  static Iter Iterate(const SkAAClip& clip) {
    ...
    return Iter(
        clip.getBounds().fTop, head->data(), head->yoffsets(), head->yoffsets() + head->fRowCount);
  }
};
...
bool SkAAClip::op(const SkRect& rect, SkClipOp op, bool doAA) {
  if (!doAA) {
    ...
    if (!pixelBounds.intersect(rect.roundOut())) {
      ...
      SkAAClip rectClip;
      rectClip.setPath(
          SkPath::Rect(rect), op == SkClipOp::kDifference ? fBounds : pixelBounds,
          /*doAA=*/true);
