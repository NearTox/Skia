...
bool SkBlitter::isNullBlitter() const noexcept { return false; }

const SkPixmap* SkBlitter::justAnOpaqueColor(uint32_t* value) { return nullptr; }
...
inline constexpr static SkAlpha ScalarToAlpha(SkScalar a) noexcept {
...
void SkBlitter::blitFatAntiRect(const SkRect& rect) {
  ...
  if (bounds.height() > 2) {
    this->blitAntiRect(
        bounds.fLeft, bounds.fTop + 1, bounds.width() - 2, bounds.height() - 2,
...
static inline void bits_to_runs(
    SkBlitter* blitter, int x, int y, const uint8_t bits[], uint8_t left_mask, ptrdiff_t rowBytes,
    uint8_t right_mask) {
  int inFill = 0;
  int pos = 0;
...
static uint8_t generate_right_mask(int maskBitCount) noexcept {
...
void SkBlitter::blitMask(const SkMask& mask, const SkIRect& clip) {
  ...
  if (mask.fFormat == SkMask::kBW_Format) {
    ...
    SkDEBUGCODE(
        const uint8_t* endOfImage = mask.fImage + (mask.fBounds.height() - 1) * maskRowBytes +
...
void SkBlitter::blitRegion(const SkRegion& clip) {
  SkRegionPriv::VisitSpans(
      clip, [this](const SkIRect& r) { this->blitRect(r.left(), r.top(), r.width(), r.height()); });
}
...
const SkPixmap* SkNullBlitter::justAnOpaqueColor(uint32_t* value) { return nullptr; }

bool SkNullBlitter::isNullBlitter() const noexcept { return true; }
...
void SkRectClipBlitter::blitAntiRect(
    int left, int y, int width, int height, SkAlpha leftAlpha, SkAlpha rightAlpha) {
  SkIRect r;
...
void SkRgnClipBlitter::blitAntiRect(
    int x, int y, int width, int height, SkAlpha leftAlpha, SkAlpha rightAlpha) {
  ...
  while (!iter.done()) {
    ...
    if (255 == effectiveLeftAlpha && 255 == effectiveRightAlpha) {
      ...
      fBlitter->blitAntiRect(
          r.fLeft, r.fTop, r.width() - 2, r.height(), effectiveLeftAlpha, effectiveRightAlpha);
    }
    iter.next();
  }
}
...
bool SkBlitter::UseRasterPipelineBlitter(
  ...
  if (device.alphaType() == kUnpremul_SkAlphaType || matrix.hasPerspective() ||
      paint.getColorFilter() || paint.getBlendMode() > SkBlendMode::kLastCoeffMode ||
  ...
  return device.colorType() != kN32_SkColorType && device.colorType() != kRGB_565_SkColorType;
#endif
}

SkBlitter* SkBlitter::Choose(
    const SkPixmap& device, const SkMatrixProvider& matrixProvider, const SkPaint& origPaint,
    SkArenaAlloc* alloc, bool drawCoverage, sk_sp<SkShader> clipShader) {
  SkASSERT(alloc);
  ...
  if (paint->getBlendMode() != SkBlendMode::kSrcOver) {
    switch (SkInterpretXfermode(*paint, SkColorTypeIsAlwaysOpaque(device.colorType()))) {
      case kSrcOver_SkXfermodeInterpretation:
        ...
        break;
      case kSkipDrawing_SkXfermodeInterpretation: return alloc->make<SkNullBlitter>();
      default: break;
    }
  }
  ...
  if (UseRasterPipelineBlitter(device, *paint, ctm) || clipShader) {
    auto blitter = SkCreateRasterPipelineBlitter(device, *paint, matrixProvider, alloc, clipShader);
    SkASSERT(blitter);
    return blitter;
  }
  ...
  if (paint->getShader()) {
    shaderContext =
        as_SB(paint->getShader())
            ->makeContext({*paint, ctm, nullptr, device.colorType(), device.colorSpace()}, alloc);
  ...
  switch (device.colorType()) {
    ...
    case kRGB_565_SkColorType:
      if (shaderContext && SkRGB565_Shader_Blitter::Supports(device, *paint)) {
        ...
        return SkCreateRasterPipelineBlitter(device, *paint, matrixProvider, alloc, clipShader);
      }

    default: SkASSERT(false); return alloc->make<SkNullBlitter>();
  }
}
...
SkShaderBlitter::SkShaderBlitter(
    const SkPixmap& device, const SkPaint& paint, SkShaderBase::Context* shaderContext)
...
SkShaderBlitter::~SkShaderBlitter() { fShader->unref(); }
...
void SkRectClipCheckBlitter::blitAntiRect(
    int x, int y, int width, int height, SkAlpha leftAlpha, SkAlpha rightAlpha) {
