...
SkTypeface* SkFontMgr::matchFamilyStyleCharacter(
    const char familyName[], const SkFontStyle& style, const char* bcp47[], int bcp47Count,
...
sk_sp<SkTypeface> SkFontMgr::makeFromStream(
...
sk_sp<SkTypeface> SkFontMgr::onMakeFromStreamArgs(
    std::unique_ptr<SkStreamAsset> stream, const SkFontArguments& args) const {
...
SkTypeface* SkFontStyleSet::matchStyleCSS3(const SkFontStyle& pattern) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    static_assert(
        SkFontStyle::kUpright_Slant == 0 && SkFontStyle::kItalic_Slant == 1 &&
            SkFontStyle::kOblique_Slant == 2,
        "SkFontStyle::Slant values not as required.");
    SkASSERT(
        0 <= pattern.slant() && pattern.slant() <= 2 && 0 <= current.slant() &&
        current.slant() <= 2);
