...
template <SkSRGBGammaColorFilter::Direction dir> sk_sp<SkColorFilter> MakeSRGBGammaCF() {
...
sk_sp<SkColorFilter> SkColorFilters::Lerp(float weight, sk_sp<SkColorFilter> cf0,
    ...
    return sk_sp<SkColorFilter>(cf0 ? new SkMixerColorFilter(std::move(cf0), std::move(cf1), weight)
...
public:
    ...
    std::unique_ptr<GrFragmentProcessor> asFragmentProcessor(
        ...
        return GrSkSLFP::Make(context, fIndex, "Runtime Color Filter", fSkSL,
                              fInputs ? fInputs->data() : nullptr, fInputs ? fInputs->size() : 0);
    }
#endif
            ...
            ctx->fn = [](SkRasterPipeline_CallbackCtx* arg, int active_pixels) {
                ...
                for (int i = 0; i < active_pixels; i++) {
                    ctx->interpreter->run(*ctx->main,
                                          (SkSL::Interpreter::Value*)(ctx->rgba + i * 4), nullptr);
                }
            };
            rec.fPipeline->append(SkRasterPipeline::callback, ctx);
        }
...
sk_sp<SkColorFilter> SkRuntimeColorFilterFactory::make(sk_sp<SkData> inputs) {
    return sk_sp<SkColorFilter>(
            new SkRuntimeColorFilter(fIndex, fSkSL, std::move(inputs), fCpuFunc));
}
