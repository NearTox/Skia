...
bool SkColorFilter::onAsAColorMode(SkColor*, SkBlendMode*) const { return false; }

bool SkColorFilter::onAsAColorMatrix(float matrix[20]) const { return false; }
...
SkColor SkColorFilter::filterColor(SkColor c) const {
  return this->filterColor4f(SkColor4f::FromColor(c), nullptr).toSkColor();
}
...
SkColor4f SkColorFilter::filterColor4f(const SkColor4f& c, SkColorSpace* colorSpace) const {
  ...
  SkPaint dummyPaint;
  SkStageRec rec = {&pipeline,  &alloc,  kRGBA_F32_SkColorType, colorSpace,
...
class SkSRGBGammaColorFilter : public SkColorFilter {
 public:
  ...
  SkSRGBGammaColorFilter(Direction dir)
      : fDir(dir), fSteps([&] {
 ...
 public:
  ...
  std::unique_ptr<GrFragmentProcessor> asFragmentProcessor(
    ...
    return GrMixerEffect::Make(
        ...
        fCF1 ? fCF1->asFragmentProcessor(context, dstColorSpaceInfo) : nullptr, fWeight);
  }
#endif
...
sk_sp<SkColorFilter> SkColorFilters::Lerp(
    float weight, sk_sp<SkColorFilter> cf0, sk_sp<SkColorFilter> cf1) {
  if (!cf0 && !cf1) {
    return nullptr;
  }
  ...
  return sk_sp<SkColorFilter>(
      cf0 ? new SkMixerColorFilter(std::move(cf0), std::move(cf1), weight)
...
class SkRuntimeColorFilter : public SkColorFilter {
 public:
  SkRuntimeColorFilter(
      int index, SkString sksl, sk_sp<SkData> inputs, void (*cpuFunction)(float[4], const void*))
      : fIndex(index),
        fSkSL(std::move(sksl)),
  ...
  std::unique_ptr<GrFragmentProcessor> asFragmentProcessor(
    ...
    return GrSkSLFP::Make(
        context, fIndex, "Runtime Color Filter", fSkSL, fInputs ? fInputs->data() : nullptr,
  ...
  bool onAppendStages(const SkStageRec& rec, bool shaderIsOpaque) const override {
    if (fCpuFunction) {
      ...
      ctx->inputs = fInputs->data();
      SkSL::Compiler c;
      std::unique_ptr<SkSL::Program> prog = c.convertProgram(
          SkSL::Program::kPipelineStage_Kind, SkSL::String(fSkSL.c_str()),
      ...
      ctx->interpreter.reset(new SkSL::Interpreter(
          std::move(prog), std::move(byteCode), (SkSL::Interpreter::Value*)ctx->inputs));
      ctx->fn = [](SkRasterPipeline_CallbackCtx* arg, int active_pixels) {
        auto ctx = (InterpreterCtx*)arg;
        for (int i = 0; i < active_pixels; i++) {
          ctx->interpreter->run(
              *ctx->main, (SkSL::Interpreter::Value*)(ctx->rgba + i * 4), nullptr);
        }
      };
      rec.fPipeline->append(SkRasterPipeline::callback, ctx);
    }
...
sk_sp<SkFlattenable> SkRuntimeColorFilter::CreateProc(SkReadBuffer& buffer) {
  ...
  return sk_sp<SkFlattenable>(
      new SkRuntimeColorFilter(index, std::move(sksl), std::move(inputs), nullptr));
}

SkRuntimeColorFilterFactory::SkRuntimeColorFilterFactory(
    SkString sksl, SkRuntimeColorFilterFn cpuFunc)
    : fIndex(GrSkSLFP::NewIndex()), fSkSL(std::move(sksl)), fCpuFunc(cpuFunc) {}

sk_sp<SkColorFilter> SkRuntimeColorFilterFactory::make(sk_sp<SkData> inputs) {
  return sk_sp<SkColorFilter>(new SkRuntimeColorFilter(fIndex, fSkSL, std::move(inputs), fCpuFunc));
}
