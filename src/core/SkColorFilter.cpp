...
bool SkColorFilter::isAlphaUnchanged() const { return as_CFB(this)->onIsAlphaUnchanged(); }

sk_sp<SkColorFilter> SkColorFilter::Deserialize(
...
bool SkColorFilterBase::onAsAColorMode(SkColor*, SkBlendMode*) const { return false; }
...
skvm::Color SkColorFilterBase::program(
    skvm::Builder* p, skvm::Color c, const SkColorInfo& dst, skvm::Uniforms* uniforms,
...
SkPMColor4f SkColorFilterBase::onFilterColor4f(
    const SkPMColor4f& color, SkColorSpace* dstCS) const {
  ...
  SkStageRec rec = {&pipeline,  &alloc,  kRGBA_F32_SkColorType, dstCS,
                    blankPaint, nullptr, matrixProvider};
  ...
  if (skvm::Color filtered =
    ...
    b.store(
        {skvm::PixelFormat::FLOAT, 32, 32, 32, 32, 0, 32, 64, 96}, b.varying<SkColor4f>(),
        filtered);
...
struct SkWorkingFormatColorFilter : public SkColorFilterBase {
  ...
  SkWorkingFormatColorFilter(
      sk_sp<SkColorFilter> child, const skcms_TransferFunction* tf, const skcms_Matrix3x3* gamut,
  ...
  skvm::Color onProgram(
      skvm::Builder* p, skvm::Color c, const SkColorInfo& rawDst, skvm::Uniforms* uniforms,
      SkArenaAlloc* alloc) const override {
...
sk_sp<SkFlattenable> SkWorkingFormatColorFilter::CreateProc(SkReadBuffer& buffer) {
  ...
  bool useDstTF = buffer.readBool(), useDstGamut = buffer.readBool(), useDstAT = buffer.readBool();
  ...
  return SkColorFilters::WithWorkingFormat(
      std::move(child), useDstTF ? nullptr : &tf, useDstGamut ? nullptr : &gamut,
      useDstAT ? nullptr : &at);
}

sk_sp<SkColorFilter> SkColorFilters::WithWorkingFormat(
    sk_sp<SkColorFilter> child, const skcms_TransferFunction* tf, const skcms_Matrix3x3* gamut,
    const SkAlphaType* at) {
...
sk_sp<SkColorFilter> SkColorFilters::Lerp(
  ...
  return effect->makeColorFilter(
      SkData::MakeWithCopy(&weight, sizeof(weight)), inputs, SK_ARRAY_COUNT(inputs));
#else
  // TODO(skia:12197)
  return nullptr;
#endif
