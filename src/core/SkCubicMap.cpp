...
static float eval_poly2(float a, float b, float c, float t) { return (a * t + b) * t + c; }

static float eval_poly1(float a, float b, float t) { return a * t + b; }

static float guess_nice_cubic_root(float A, float B, float C, float D) { return -D; }

#ifdef SK_DEBUG
static bool valid(float r) { return r >= 0 && r <= 1; }
#endif
...
static inline bool delta_nearly_zero(float delta) { return sk_float_abs(delta) <= 0.0001f; }
...
#endif
  float answer2 =
      true ? solve_nice_cubic_halley(A, B, C, -x) : solve_nice_cubic_householder(A, B, C, -x);
#ifdef CUBICMAP_TRACK_MAX_ERROR
