...
bool SkBitmapProcState::chooseProcs() {
  ...
  if (fAlphaScale == 256 && fFilterQuality == kNone_SkFilterQuality &&
...
static void S32_D32_constX_shaderproc(const void* sIn, int x, int y, SkPMColor* colors, int count) {
  ...
  if (kNone_SkFilterQuality != s.fFilterQuality) {
    ...
    const int stopY = s.fPixmap.height();
    switch (s.fTileModeY) {
      case SkTileMode::kClamp: iY0 = SkClampMax(yTemp, stopY - 1); break;
...
#ifdef SK_DEBUG
    {
      ...
      switch (s.fTileModeY) {
        case SkTileMode::kClamp: iY2 = SkClampMax(iY2, stopY - 1); break;
...
SkBitmapProcState::ShaderProc32 SkBitmapProcState::chooseShaderProc32() {
  ...
  if (1 == fPixmap.width() && fInvMatrix.isScaleTranslate()) {
    if (kNone_SkFilterQuality == fFilterQuality && fInvMatrix.isTranslate() &&
