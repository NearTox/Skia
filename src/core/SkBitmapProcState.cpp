...
static void S32_alpha_D32_nofilter_DX(
...
static void S32_alpha_D32_nofilter_DXDY(
    const SkBitmapProcState& s, const uint32_t* xy, int count, SkPMColor* colors) {
  SkASSERT(count > 0 && colors != nullptr);
  SkASSERT(!s.fBilerp);
  SkASSERT(4 == s.fPixmap.info().bytesPerPixel());
  ...
  while (count-- > 0) {
    uint32_t XY = *xy++, x = XY & 0xffff, y = XY >> 16;
...
SkBitmapProcState::SkBitmapProcState(const SkImage_Base* image, SkTileMode tmx, SkTileMode tmy)
    : fImage(image), fTileModeX(tmx), fTileModeY(tmy) {}
...
bool SkBitmapProcState::init(
    const SkMatrix& inv, SkAlpha paintAlpha, const SkSamplingOptions& sampling) {
...
bool SkBitmapProcState::chooseProcs() {
  ...
  SkASSERT(
      fPixmap.alphaType() == kPremul_SkAlphaType || fPixmap.alphaType() == kOpaque_SkAlphaType);
  ...
  if (fAlphaScale == 256 && !fBilerp && SkTileMode::kClamp == fTileModeX &&
      SkTileMode::kClamp == fTileModeY && fInvMatrix.isScaleTranslate()) {
...
static void S32_D32_constX_shaderproc(const void* sIn, int x, int y, SkPMColor* colors, int count) {
  ...
  if (s.fBilerp) {
    ...
    const int stopY = s.fPixmap.height();
    switch (s.fTileModeY) {
      case SkTileMode::kClamp: iY0 = SkTPin(yTemp, 0, stopY - 1); break;
      case SkTileMode::kRepeat: iY0 = sk_int_mod(yTemp, stopY); break;
      case SkTileMode::kMirror:
      default: iY0 = sk_int_mirror(yTemp, stopY); break;
    }
...
#ifdef SK_DEBUG
    {
      ...
      switch (s.fTileModeY) {
        case SkTileMode::kClamp: iY2 = SkTPin(iY2, 0, stopY - 1); break;
        case SkTileMode::kRepeat: iY2 = sk_int_mod(iY2, stopY); break;
        case SkTileMode::kMirror:
        default: iY2 = sk_int_mirror(iY2, stopY); break;
      }
...
void SkBitmapProcState::DebugMatrixProc(
    const SkBitmapProcState& state, uint32_t bitmapXY[], int count, int x, int y) {
  SkASSERT(bitmapXY);
  SkASSERT(count > 0);
...
SkBitmapProcState::MatrixProc SkBitmapProcState::getMatrixProc() const { return DebugMatrixProc; }
