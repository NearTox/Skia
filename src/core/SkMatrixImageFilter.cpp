...
SkMatrixImageFilter::SkMatrixImageFilter(
    ...
    : INHERITED(&input, 1, nullptr), fTransform(transform), fSampling(sampling) {
...
sk_sp<SkImageFilter> SkMatrixImageFilter::Make(
    const SkMatrix& transform, const SkSamplingOptions& sampling, sk_sp<SkImageFilter> input) {
  return sk_sp<SkImageFilter>(new SkMatrixImageFilter(transform, sampling, std::move(input)));
}
...
SkIRect SkMatrixImageFilter::onFilterNodeBounds(
