...
SkMipmap* SkMipmap::Build(
    const SkPixmap& src, SkDiscardableFactoryProc fact, bool computeContents) {
...
bool SkMipmap::Deserialize(SkMipmapBuilder* builder, const void* data, size_t size) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    auto gen = SkImageGenerator::MakeFromEncoded(SkData::MakeWithProc(ptr, size, nullptr, nullptr));
    if (!gen) {
      return false;
    }
