...
sk_sp<SkPicture> SkPicture::Forwardport(
    const SkPictInfo& info, const SkPictureData* data, SkReadBuffer* buffer) {
  if (!data) {
    return nullptr;
  }
  if (!data->opData()) {
...
sk_sp<SkPicture> SkPicture::MakeFromData(
...
sk_sp<SkPicture> SkPicture::MakeFromStream(
    SkStream* stream, const SkDeserialProcs* procsPtr, SkTypefacePlayback* typefaces) {
  SkPictInfo info;
