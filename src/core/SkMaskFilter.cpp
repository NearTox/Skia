...
static void blitClippedMask(
    SkBlitter* blitter, const SkMask& mask, const SkIRect& bounds, const SkIRect& clipR) {
  SkIRect r;
...
static void draw_nine_clipped(
    const SkMask& mask, const SkIRect& outerR, const SkIPoint& center, bool fillCenter,
  ...
  if (m.fBounds.width() > 0 && m.fBounds.height() > 0) {
    extractMaskSubset(mask, &m);
    m.fBounds.offsetTo(outerR.right() - m.fBounds.width(), outerR.bottom() - m.fBounds.height());
    blitClippedMask(blitter, m, m.fBounds, clipR);
  }
  ...
  if (r.intersect(clipR)) {
    SkMask m;
    m.fImage =
        mask.getAddr8(mask.fBounds.left() + r.left() - outerR.left(), mask.fBounds.top() + cy);
    m.fBounds = r;
  ...
  if (r.intersect(clipR)) {
    SkMask m;
    m.fImage =
        mask.getAddr8(mask.fBounds.right() - outerR.right() + r.left(), mask.fBounds.top() + cy);
    m.fBounds = r;
    m.fRowBytes = 0;  // so we repeat the scanline for our height
    m.fFormat = SkMask::kA8_Format;
    blitter->blitMask(m, r);
...
bool SkMaskFilterBase::filterRRect(
    const SkRRect& devRRect, const SkMatrix& matrix, const SkRasterClip& clip,
    SkBlitter* blitter) const {
...
bool SkMaskFilterBase::filterPath(
    const SkPath& devPath, const SkMatrix& matrix, const SkRasterClip& clip, SkBlitter* blitter,
  ...
  if (rectCount > 0) {
    ...
    switch (this->filterRectsToNine(rects, rectCount, matrix, clip.getBounds(), &patch)) {
      case kFalse_FilterReturn: SkASSERT(nullptr == patch.fMask.fImage); return false;
...
SkMaskFilterBase::FilterReturn SkMaskFilterBase::filterRRectToNine(
    const SkRRect&, const SkMatrix&, const SkIRect& clipBounds, NinePatch*) const {
  return kUnimplemented_FilterReturn;
}

SkMaskFilterBase::FilterReturn SkMaskFilterBase::filterRectsToNine(
    const SkRect[], int count, const SkMatrix&, const SkIRect& clipBounds, NinePatch*) const {
  return kUnimplemented_FilterReturn;
}
...
bool SkMaskFilterBase::canFilterMaskGPU(
...
bool SkMaskFilterBase::directFilterMaskGPU(
    ...
    const SkMatrix& viewMatrix, const GrShape&) const {
  return false;
}
...
template <typename T>
static inline T join(const T& a, const T& b) {
  T r = a;
  r.join(b);
  return r;
}
...
bool SkComposeMF::filterMask(
    SkMask* dst, const SkMask& src, const SkMatrix& ctm, SkIPoint* margin) const {
...
class SkMatrixMF : public SkMaskFilterBase {
 public:
  ...
  bool filterMask(
      SkMask* dst, const SkMask& src, const SkMatrix& ctm, SkIPoint* margin) const override {
...
sk_sp<SkMaskFilter> SkMaskFilter::MakeCompose(
    sk_sp<SkMaskFilter> outer, sk_sp<SkMaskFilter> inner) {
  if (!outer) {
    return inner;
  }
  if (!inner) {
...
sk_sp<SkMaskFilter> SkMaskFilter::MakeCombine(
    sk_sp<SkMaskFilter> dst, sk_sp<SkMaskFilter> src, SkCoverageMode mode) {
  if (!dst) {
    return src;
  }
  if (!src) {
