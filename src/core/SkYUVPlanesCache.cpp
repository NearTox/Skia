...
struct YUVPlanesKey : public SkResourceCache::Key {
  YUVPlanesKey(uint32_t genID) : fGenID(genID) {
    this->init(
        &gYUVPlanesKeyNamespaceLabel, SkMakeResourceCacheSharedIDForBitmap(genID), sizeof(genID));
  }
...
struct YUVPlanesRec : public SkResourceCache::Rec {
  YUVPlanesRec(YUVPlanesKey key, SkCachedData* data, const SkYUVAPixmaps& pixmaps) : fKey(key) {
    fValue.fData = data;
...
SkCachedData* SkYUVPlanesCache::FindAndRef(
    uint32_t genID, SkYUVAPixmaps* pixmaps, SkResourceCache* localCache) {
  YUVValue result;
...
void SkYUVPlanesCache::Add(
    uint32_t genID, SkCachedData* data, const SkYUVAPixmaps& pixmaps, SkResourceCache* localCache) {
  YUVPlanesKey key(genID);
