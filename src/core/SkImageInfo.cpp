...
SkColorInfo::SkColorInfo(SkColorType ct, SkAlphaType at, sk_sp<SkColorSpace> cs) noexcept
    : fColorSpace(std::move(cs)), fColorType(ct), fAlphaType(at) {}

SkColorInfo::SkColorInfo(const SkColorInfo&) noexcept = default;
SkColorInfo::SkColorInfo(SkColorInfo&&) noexcept = default;

SkColorInfo& SkColorInfo::operator=(const SkColorInfo&) noexcept = default;
SkColorInfo& SkColorInfo::operator=(SkColorInfo&&) noexcept = default;

SkColorSpace* SkColorInfo::colorSpace() const noexcept { return fColorSpace.get(); }
sk_sp<SkColorSpace> SkColorInfo::refColorSpace() const noexcept { return fColorSpace; }
...
SkColorInfo SkColorInfo::makeAlphaType(SkAlphaType newAlphaType) const noexcept {
  return SkColorInfo(this->colorType(), newAlphaType, this->refColorSpace());
}

SkColorInfo SkColorInfo::makeColorType(SkColorType newColorType) const noexcept {
  return SkColorInfo(newColorType, this->alphaType(), this->refColorSpace());
}

SkColorInfo SkColorInfo::makeColorSpace(sk_sp<SkColorSpace> cs) const noexcept {
...
size_t SkImageInfo::computeByteSize(size_t rowBytes) const noexcept {
  ...
  SkSafeMath safe;
  size_t bytes = safe.add(
      safe.mul(safe.addInt(this->height(), -1), rowBytes),
...
SkColorSpace* SkImageInfo::colorSpace() const noexcept { return fColorInfo.colorSpace(); }

sk_sp<SkColorSpace> SkImageInfo::refColorSpace() const noexcept {
  return fColorInfo.refColorSpace();
}

SkImageInfo SkImageInfo::makeColorSpace(sk_sp<SkColorSpace> cs) const noexcept {
  return Make(fDimensions, fColorInfo.makeColorSpace(std::move(cs)));
}

SkImageInfo SkImageInfo::Make(int width, int height, SkColorType ct, SkAlphaType at) noexcept {
  return Make(width, height, ct, at, nullptr);
}

SkImageInfo SkImageInfo::Make(
    int width, int height, SkColorType ct, SkAlphaType at, sk_sp<SkColorSpace> cs) noexcept {
  return SkImageInfo({width, height}, {ct, at, std::move(cs)});
}

SkImageInfo SkImageInfo::Make(SkISize dimensions, SkColorType ct, SkAlphaType at) noexcept {
  return Make(dimensions, ct, at, nullptr);
}

SkImageInfo SkImageInfo::Make(
    SkISize dimensions, SkColorType ct, SkAlphaType at, sk_sp<SkColorSpace> cs) noexcept {
  return SkImageInfo(dimensions, {ct, at, std::move(cs)});
}

SkImageInfo SkImageInfo::MakeN32(int width, int height, SkAlphaType at) noexcept {
  return MakeN32(width, height, at, nullptr);
}

SkImageInfo SkImageInfo::MakeN32(
    int width, int height, SkAlphaType at, sk_sp<SkColorSpace> cs) noexcept {
...
SkImageInfo SkImageInfo::MakeN32Premul(int width, int height) noexcept {
  return MakeN32Premul(width, height, nullptr);
}

SkImageInfo SkImageInfo::MakeN32Premul(int width, int height, sk_sp<SkColorSpace> cs) noexcept {
  return Make({width, height}, kN32_SkColorType, kPremul_SkAlphaType, std::move(cs));
}

SkImageInfo SkImageInfo::MakeN32Premul(SkISize dimensions) noexcept {
  return MakeN32Premul(dimensions, nullptr);
}

SkImageInfo SkImageInfo::MakeN32Premul(SkISize dimensions, sk_sp<SkColorSpace> cs) noexcept {
  return Make(dimensions, kN32_SkColorType, kPremul_SkAlphaType, std::move(cs));
}

SkImageInfo SkImageInfo::MakeA8(int width, int height) noexcept {
  return Make({width, height}, kAlpha_8_SkColorType, kPremul_SkAlphaType, nullptr);
}

SkImageInfo SkImageInfo::MakeA8(SkISize dimensions) noexcept {
  return Make(dimensions, kAlpha_8_SkColorType, kPremul_SkAlphaType, nullptr);
}

SkImageInfo SkImageInfo::MakeUnknown(int width, int height) noexcept {
...
bool SkColorTypeValidateAlphaType(
  ...
  switch (colorType) {
    ...
    case kR8_unorm_SkColorType: alphaType = kOpaque_SkAlphaType; break;
  }
  if (canonical) {
    *canonical = alphaType;
  }
