...
static bool set_normal_unitnormal(const SkPoint& before, const SkPoint& after, SkScalar scale,
                                  SkScalar radius, SkVector* normal, SkVector* unitNormal) {
...
static bool set_normal_unitnormal(const SkVector& vec, SkScalar radius, SkVector* normal,
                                  SkVector* unitNormal) {
...
class SkPathStroker {
public:
    SkPathStroker(const SkPath& src, SkScalar radius, SkScalar miterLimit, SkPaint::Cap,
                  SkPaint::Join, SkScalar resScale, bool canIgnoreCenter);
...
private:
    ...
    void setConicEndNormal(const SkConic&, const SkVector& normalAB, const SkVector& unitNormalAB,
                           SkVector* normalBC, SkVector* unitNormalBC);
    void setCubicEndNormal(const SkPoint cubic[4], const SkVector& normalAB,
                           const SkVector& unitNormalAB, SkVector* normalCD,
                           SkVector* unitNormalCD);
    void setQuadEndNormal(const SkPoint quad[3], const SkVector& normalAB,
...
bool SkPathStroker::preJoinTo(const SkPoint& currPt, SkVector* normal, SkVector* unitNormal,
                              bool currIsLine) {
...
void SkPathStroker::finishContour(bool close, bool currIsLine) {
    if (fSegmentCount > 0) {
        ...
        if (close) {
            fJoiner(&fOuter, &fInner, fPrevUnitNormal, fPrevPt, fFirstUnitNormal, fRadius,
                    fInvMiterLimit, fPrevIsLine, currIsLine);
            fOuter.close();
...
SkPathStroker::SkPathStroker(const SkPath& src, SkScalar radius, SkScalar miterLimit,
...
void SkPathStroker::setQuadEndNormal(const SkPoint quad[3], const SkVector& normalAB,
                                     const SkVector& unitNormalAB, SkVector* normalBC,
...
SkPathStroker::ResultType SkPathStroker::intersectRay(
    ...
    if ((numerA >= 0) == (numerB >= 0)) {  // if the control point is outside the quad ends
        ...
        if (SkTMax(dist1, dist2) <= fInvResScaleSquared) {
            return STROKER_RESULT(kDegenerate_ResultType, depth, quadPts,
                                  "SkTMax(dist1=%g, dist2=%g) <= fInvResScaleSquared", dist1,
                                  dist2);
        }
    ...
    return STROKER_RESULT(kDegenerate_ResultType, depth, quadPts, "SkScalarNearlyZero(denom=%g)",
                          denom);
}
...
SkPathStroker::ResultType SkPathStroker::strokeCloseEnough(
    ...
    if (points_within_dist(ray[0], strokeMid, fInvResScale)) {  // if the difference is small
        if (sharp_angle(quadPts->fQuad)) {
            return STROKER_RESULT(kSplit_ResultType, depth, quadPts,
                                  "sharp_angle (1) =%g,%g, %g,%g, %g,%g", quadPts->fQuad[0].fX,
                                  quadPts->fQuad[0].fY, quadPts->fQuad[1].fX, quadPts->fQuad[1].fY,
    ...
    if (!ptInQuadBounds(stroke, ray[0])) {  // if far, subdivide
        return STROKER_RESULT(kSplit_ResultType, depth, quadPts,
                              ...
                              stroke[0].fX, stroke[0].fY, stroke[1].fX, stroke[1].fY, stroke[2].fX,
                              stroke[2].fY, ray[0].fX, ray[0].fY);
    }
    ...
    if (points_within_dist(ray[0], quadPt, error)) {  // if the difference is small, we're done
        if (sharp_angle(quadPts->fQuad)) {
            return STROKER_RESULT(kSplit_ResultType, depth, quadPts,
                                  "sharp_angle (2) =%g,%g, %g,%g, %g,%g", quadPts->fQuad[0].fX,
                                  quadPts->fQuad[0].fY, quadPts->fQuad[1].fX, quadPts->fQuad[1].fY,
                                  quadPts->fQuad[2].fX, quadPts->fQuad[2].fY);
        }
        return STROKER_RESULT(kQuad_ResultType, depth, quadPts,
                              "points_within_dist(ray[0]=%g,%g, quadPt=%g,%g, error=%g)", ray[0].fX,
                              ray[0].fY, quadPt.fX, quadPt.fY, error);
    }
    // otherwise, subdivide
...
SkPathStroker::ResultType SkPathStroker::compareQuadCubic(const SkPoint cubic[4],
    ...
    return this->strokeCloseEnough(quadPts->fQuad, ray,
                                   quadPts STROKER_DEBUG_PARAMS(fRecursionDepth));
}
...
bool SkPathStroker::cubicStroke(const SkPoint cubic[4], SkQuadConstruct* quadPts) {
    ...
    SkDEBUGCODE(gMaxRecursion[fFoundTangents] =
                        SkTMax(gMaxRecursion[fFoundTangents], fRecursionDepth + 1));
#endif
...
#if QUAD_STROKE_APPROX_EXTENDED_DEBUGGING
    SkDEBUGCODE(gMaxRecursion[kConic_RecursiveLimit] =
...
#if QUAD_STROKE_APPROX_EXTENDED_DEBUGGING
    SkDEBUGCODE(gMaxRecursion[kQuad_RecursiveLimit] =
