...
static sk_sp<GrRenderTargetContext> convolve_gaussian(GrRecordingContext* context,
    ...
    if (Direction::kX == direction) {
        ...
        rightRect = SkIRect::MakeLTRB(midRect.right(), midRect.top(), dstRect.width(),
                                      midRect.bottom());
        ...
        rightRect = SkIRect::MakeLTRB(midRect.left(), midRect.bottom(), midRect.right(),
                                      dstRect.height());
    ...
    if (midRect.isEmpty()) {
        ...
        convolve_gaussian_1d(dstRenderTargetContext.get(), clip, leftRect, srcOffset, proxy,
                             direction, radius, sigma, mode, bounds);
        convolve_gaussian_1d(dstRenderTargetContext.get(), clip, rightRect, srcOffset, proxy,
                             direction, radius, sigma, mode, bounds);
...
static sk_sp<GrTextureProxy> decimate(GrRecordingContext* context, sk_sp<GrTextureProxy> src,
                                      SkIPoint* srcOffset, SkIRect* contentRect, int scaleFactorX,
                                      int scaleFactorY, bool willBeXFiltering,
                                      bool willBeYFiltering, int radiusX, int radiusY,
                                      GrTextureDomain::Mode mode, const SkImageInfo& dstII) {
    ...
    *contentRect = dstRect;
    ...
    if (willBeXFiltering) {
        if (scaleFactorX > 1) {
            ...
            SkIRect clearRect = SkIRect::MakeXYWH(contentRect->fRight, contentRect->fTop, radiusX,
                                                  contentRect->height());
...
static sk_sp<GrRenderTargetContext> reexpand(GrRecordingContext* context,
                                             ...
                                             const SkIRect& localSrcBounds, int scaleFactorX,
                                             int scaleFactorY, GrTextureDomain::Mode mode,
...
sk_sp<GrRenderTargetContext> GaussianBlur(GrRecordingContext* context,
    ...
    if (sigmaX > 0.0f && sigmaY > 0.0f &&
        ...
        return convolve_gaussian_2d(context, std::move(srcProxy), srcBounds, srcOffset, radiusX,
                                    radiusY, sigmaX, sigmaY, mode, finalDestII, fit);
    }
    ...
    if (scaleFactorX > 1 || scaleFactorY > 1) {
        srcProxy = decimate(context, std::move(srcProxy), &srcOffset, &localSrcBounds, scaleFactorX,
                            scaleFactorY, sigmaX > 0.0f, sigmaY > 0.0f, radiusX, radiusY, mode,
                            finalDestII);
        if (!srcProxy) {
            return nullptr;
        }
    ...
    if (sigmaX > 0.0f) {
        dstRenderTargetContext =
                convolve_gaussian(context, std::move(srcProxy), srcRect, srcOffset, Direction::kX,
    ...
    if (sigmaY > 0.0f) {
        dstRenderTargetContext =
                convolve_gaussian(context, std::move(srcProxy), srcRect, srcOffset, Direction::kY,
    ...
    if (scaleFactorX > 1 || scaleFactorY > 1) {
        dstRenderTargetContext =
                reexpand(context, std::move(dstRenderTargetContext), localSrcBounds, scaleFactorX,
                         scaleFactorY, mode, finalDestII, fit);
    }
