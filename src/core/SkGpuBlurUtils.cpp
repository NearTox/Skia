...
static void convolve_gaussian_1d(
    skgpu::SurfaceFillContext* sfc, GrSurfaceProxyView srcView, const SkIRect srcSubset,
    SkIVector dstToSrcOffset, const SkIRect& dstRect, SkAlphaType srcAlphaType, Direction direction,
...
static std::unique_ptr<skgpu::v1::SurfaceDrawContext> convolve_gaussian_2d(
    GrRecordingContext* rContext, GrSurfaceProxyView srcView, GrColorType srcColorType,
  ...
  auto sdc = skgpu::v1::SurfaceDrawContext::Make(
      rContext, srcColorType, std::move(finalCS), dstFit, dstBounds.size(), SkSurfaceProps(), 1,
  ...
  auto conv = GrMatrixConvolutionEffect::Make(
      rContext, std::move(srcView), srcBounds, size, kernel, 1.0f, 0.0f, kernelOffset, wm, true,
      *sdc->caps());
...
static std::unique_ptr<skgpu::v1::SurfaceDrawContext> convolve_gaussian(
  ...
  if (!canSplit || canHWTile) {
    ...
    convolve_gaussian_1d(
        dstSDC.get(), std::move(srcView), srcBounds, rtcToSrcOffset, dstRect, srcAlphaType,
  ...
  auto convolve = [&](SkIRect rect) {
    ...
    convolve_gaussian_1d(
        dstSDC.get(), srcView, srcBounds, rtcToSrcOffset, rect, srcAlphaType, direction, radius,
        sigma, mode);
  };
...
static std::unique_ptr<skgpu::v1::SurfaceDrawContext> reexpand(
    GrRecordingContext* rContext, std::unique_ptr<skgpu::SurfaceContext> src,
    const SkRect& srcBounds, SkISize dstSize, sk_sp<SkColorSpace> colorSpace, SkBackingFit fit) {
...
static std::unique_ptr<skgpu::v1::SurfaceDrawContext> two_pass_gaussian(
  ...
  return convolve_gaussian(
      rContext, std::move(srcView), srcColorType, srcAlphaType, srcBounds, dstBounds, Direction::kY,
...
std::unique_ptr<skgpu::v1::SurfaceDrawContext> GaussianBlur(
  ...
  if (!radiusX && !radiusY) {
    // Create the sdc with default SkSurfaceProps. Gaussian blurs will soon use a
    // SurfaceFillContext, at which point the SkSurfaceProps won't exist anymore.
    auto result = skgpu::v1::SurfaceDrawContext::Make(
        rContext, srcColorType, std::move(colorSpace), fit, dstBounds.size(), SkSurfaceProps(), 1,
        GrMipmapped::kNo, srcView.proxy()->isProtected(), srcView.origin());
    if (!result) {
      return nullptr;
    }
    ...
    auto fp = GrTextureEffect::MakeSubset(
        ...
        SkRect::Make(dstBounds), *rContext->priv().caps());
  ...
  if (sigmaX <= kMaxSigma && sigmaY <= kMaxSigma) {
    ...
    if (radiusX > 0 && radiusY > 0 && kernelSize <= GrMatrixConvolutionEffect::kMaxUniformSize &&
      ...
      return convolve_gaussian_2d(
          rContext, std::move(srcView), srcColorType, srcBounds, dstBounds, radiusX, radiusY,
          sigmaX, sigmaY, mode, std::move(colorSpace), fit);
    }
    // This will automatically degenerate into a single pass of X or Y if only one of the
    // radii are non-zero.
    return two_pass_gaussian(
        rContext, std::move(srcView), srcColorType, srcAlphaType, std::move(colorSpace), srcBounds,
  ...
  SkISize rescaledSize = {
      std::max(sk_float_floor2int(srcBounds.width() * scaleX), 1),
  ...
  int padX =
      mode == SkTileMode::kClamp || (mode == SkTileMode::kDecal && sigmaX > kMaxSigma) ? 1 : 0;
  int padY =
      mode == SkTileMode::kClamp || (mode == SkTileMode::kDecal && sigmaY > kMaxSigma) ? 1 : 0;
  // Create the sdc with default SkSurfaceProps. Gaussian blurs will soon use a
  // SurfaceFillContext, at which point the SkSurfaceProps won't exist anymore.
  auto rescaledSDC = skgpu::v1::SurfaceDrawContext::Make(
      srcCtx->recordingContext(), colorInfo.colorType(), colorInfo.refColorSpace(),
      SkBackingFit::kApprox, {rescaledSize.width() + 2 * padX, rescaledSize.height() + 2 * padY},
      SkSurfaceProps(), 1, GrMipmapped::kNo, srcCtx->asSurfaceProxy()->isProtected(),
      srcCtx->origin());
  if (!rescaledSDC) {
    return nullptr;
  }
  ...
  if (!srcCtx->rescaleInto(
          rescaledSDC.get(), SkIRect::MakeSize(rescaledSize).makeOffset(padX, padY), srcBounds,
  ...
  if (mode == SkTileMode::kClamp) {
    ...
    auto cheapDownscale = [&](SkIRect dstRect, SkIRect srcRect) {
      rescaledSDC->drawTexture(
          ...
          SkRect::Make(srcRect), SkRect::Make(dstRect), GrAA::kNo, GrQuadAAFlags::kNone,
    ...
    cheapDownscale(SkIRect::MakeXYWH(dw + 1, dh + 1, 1, 1), SkIRect::MakeXYWH(sRCol, sBRow, 1, 1));
  ...
  auto sdc = GaussianBlur(
      rContext, std::move(srcView), srcColorType, srcAlphaType, colorSpace, scaledDstBoundsI,
  ...
  return reexpand(
      rContext, std::move(sdc), scaledDstBounds, dstBounds.size(), std::move(colorSpace), fit);
}
#  endif  // SK_GPU_V1
...
void Compute1DLinearGaussianKernel(float* kernel, float* offset, float sigma, int radius) {
  ...
  if (radius & 1) {
    ...
    //      \---^-----/ Upper sample
    get_new_weight(
        &kernel[halfradius], &offset[halfradius], temp_kernel[index] * 0.5f,
        temp_kernel[index + 1]);
