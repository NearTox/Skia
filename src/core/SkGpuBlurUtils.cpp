...
static void convolve_gaussian_1d(
    GrRenderTargetContext* renderTargetContext, GrSurfaceProxyView srcView,
  ...
  renderTargetContext->fillRectToRect(
      GrNoClip(), std::move(paint), GrAA::kNo, SkMatrix::I(), SkRect::Make(rtcRect), srcRect);
}
...
static std::unique_ptr<GrRenderTargetContext> convolve_gaussian_2d(
  ...
  renderTargetContext->fillRectToRect(
      GrNoClip(), std::move(paint), GrAA::kNo, SkMatrix::I(), SkRect::Make(dstBounds.size()),
...
static std::unique_ptr<GrRenderTargetContext> convolve_gaussian(
    GrRecordingContext* context, GrSurfaceProxyView srcView, GrColorType srcColorType,
    SkAlphaType srcAlphaType, SkIRect* contentRect, SkIRect dstBounds, Direction direction,
    int radius, float sigma, SkTileMode mode, sk_sp<SkColorSpace> finalCS, SkBackingFit fit) {
  ...
  if (SkTileMode::kClamp == mode &&
    ...
    convolve_gaussian_1d(
        dstRenderTargetContext.get(), std::move(srcView), rtcToSrcOffset, dstRect, srcAlphaType,
        direction, radius, sigma, SkTileMode::kClamp, nullptr);
    *contentRect = dstRect;
  ...
  if (!top.isEmpty()) {
    dstRenderTargetContext->clear(
        &top, SK_PMColor4fTRANSPARENT, GrRenderTargetContext::CanClearFullscreen::kYes);
  }

  if (!bottom.isEmpty()) {
    dstRenderTargetContext->clear(
        &bottom, SK_PMColor4fTRANSPARENT, GrRenderTargetContext::CanClearFullscreen::kYes);
  }

  if (mid.isEmpty()) {
    convolve_gaussian_1d(
        dstRenderTargetContext.get(), std::move(srcView), rtcToSrcOffset, *contentRect,
        srcAlphaType, direction, radius, sigma, mode, bounds);
  } else {
    // Draw right and left margins with bounds; middle without.
    convolve_gaussian_1d(
        dstRenderTargetContext.get(), srcView, rtcToSrcOffset, left, srcAlphaType, direction,
        radius, sigma, mode, bounds);
    convolve_gaussian_1d(
        dstRenderTargetContext.get(), srcView, rtcToSrcOffset, right, srcAlphaType, direction,
        radius, sigma, mode, bounds);
    convolve_gaussian_1d(
        dstRenderTargetContext.get(), std::move(srcView), rtcToSrcOffset, mid, srcAlphaType,
        direction, radius, sigma, SkTileMode::kClamp, nullptr);
  }
...
// MAX_BLUR_SIGMA.
static GrSurfaceProxyView decimate(
    GrRecordingContext* context, GrSurfaceProxyView srcView, GrColorType srcColorType,
    SkAlphaType srcAlphaType, SkIPoint srcOffset, SkIRect* contentRect, int scaleFactorX,
    int scaleFactorY, SkTileMode mode, sk_sp<SkColorSpace> finalCS) {
  ...
  for (int i = 1; i < scaleFactorX || i < scaleFactorY; i *= 2) {
    ...
    dstRenderTargetContext = GrRenderTargetContext::Make(
        context, srcColorType, finalCS, SkBackingFit::kApprox, {dstRect.fRight, dstRect.fBottom}, 1,
    ...
    if (i == 1) {
      ...
      fp = GrTextureEffect::MakeSubset(
          srcView.detachProxy(), srcAlphaType, SkMatrix::I(), sampler, domain, caps);
      srcRect.offset(-srcOffset);
    } else {
      fp = GrTextureEffect::Make(
          srcView.detachProxy(), srcAlphaType, SkMatrix::I(), GrSamplerState::Filter::kBilerp);
    }
...
static std::unique_ptr<GrRenderTargetContext> reexpand(
    GrRecordingContext* context, std::unique_ptr<GrRenderTargetContext> src,
    const SkIRect& srcBounds, int scaleFactorX, int scaleFactorY, SkISize dstSize,
    sk_sp<SkColorSpace> colorSpace, SkBackingFit fit) {
...
static std::unique_ptr<GrRenderTargetContext> two_pass_gaussian(
    GrRecordingContext* context, GrSurfaceProxyView srcView, GrColorType srcColorType,
...
std::unique_ptr<GrRenderTargetContext> GaussianBlur(
    GrRecordingContext* context, GrSurfaceProxyView srcView, GrColorType srcColorType,
    SkAlphaType srcAlphaType, sk_sp<SkColorSpace> colorSpace, const SkIRect& dstBounds,
    const SkIRect& srcBounds, float sigmaX, float sigmaY, SkTileMode mode, SkBackingFit fit) {
  SkASSERT(context);
  ...
  if (scaleFactorX == 1 && scaleFactorY == 1) {
    ...
    if (sigmaX > 0 && sigmaY > 0 && (2 * radiusX + 1) * (2 * radiusY + 1) <= MAX_KERNEL_SIZE) {
      ...
      return convolve_gaussian_2d(
          ...
          sigmaY, mode, colorSpace, fit);
    }
    return two_pass_gaussian(
        context, std::move(srcView), srcColorType, srcAlphaType, std::move(colorSpace),
        &localSrcBounds, dstBounds, sigmaX, sigmaY, radiusX, radiusY, mode, fit);
  }

  auto srcOffset = -dstBounds.topLeft();
  srcView = decimate(
      context, std::move(srcView), srcColorType, srcAlphaType, srcOffset, &localSrcBounds,
      scaleFactorX, scaleFactorY, mode, colorSpace);
  if (!srcView.proxy()) {
    return nullptr;
  }
  SkASSERT(srcView.asTextureProxy());
  auto scaledDstBounds = SkIRect::MakeWH(
      sk_float_ceil(dstBounds.width() / (float)scaleFactorX),
      sk_float_ceil(dstBounds.height() / (float)scaleFactorY));
  auto rtc = two_pass_gaussian(
      context, std::move(srcView), srcColorType, srcAlphaType, colorSpace, &localSrcBounds,
      scaledDstBounds, sigmaX, sigmaY, radiusX, radiusY, mode, SkBackingFit::kApprox);
  if (!rtc) {
    return nullptr;
  }
  return reexpand(
      context, std::move(rtc), localSrcBounds, scaleFactorX, scaleFactorY, dstBounds.size(),
