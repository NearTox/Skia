...
#  ifdef SK_USE_LEGACY_GPU_BLUR
  return LegacyGaussianBlur(
      context, srcView, srcColorType, srcAlphaType, std::move(colorSpace), dstBounds, srcBounds,
...
#  endif
  ...
  if (!sigmaX && !sigmaY) {
    auto result = GrRenderTargetContext::Make(
        context, srcColorType, std::move(colorSpace), fit, dstBounds.size());
