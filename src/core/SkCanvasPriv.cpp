...
SkAutoCanvasMatrixPaint::SkAutoCanvasMatrixPaint(
    SkCanvas* canvas, const SkMatrix* matrix, const SkPaint* paint, const SkRect& bounds)
...
void SkCanvasPriv::GetDstClipAndMatrixCounts(
    const SkCanvas::ImageSetEntry set[], int count, int* totalDstClipCount, int* totalMatrixCount) {
  int dstClipCount = 0;
...
void SkCanvasPriv::DrawMesh(
    SkCanvas* canvas, const SkMesh& mesh, sk_sp<SkBlender> blender, const SkPaint& paint) {
...
GrRenderTargetProxy* SkCanvasPriv::TopDeviceTargetProxy(SkCanvas* canvas) { return nullptr; }
