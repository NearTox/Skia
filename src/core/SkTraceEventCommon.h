...
#  define TRACE_EMPTY_FMT(fmt, ...)           \
    do {                                      \
      skprintf_like_noop(fmt, ##__VA_ARGS__); \
    } while (0)
#else
#  define TRACE_EMPTY(...) \
    do {                   \
...
class SkAndroidFrameworkTraceUtilAlways {
 public:
  SkAndroidFrameworkTraceUtilAlways(const char* name) { ATRACE_BEGIN(name); }
  ...
  ~SkAndroidFrameworkTraceUtilAlways() { ATRACE_END(); }
};
...
#  define ATRACE_ANDROID_FRAMEWORK_ALWAYS(fmt, ...) \
    SkAndroidFrameworkTraceUtilAlways __trace_always(true, fmt, ##__VA_ARGS__)
...
#  define TRACE_EVENT_ASYNC_END1(category, name, id, arg1_name, arg1_val)                  \
    INTERNAL_TRACE_EVENT_ADD_WITH_ID(                                                      \
        TRACE_EVENT_PHASE_ASYNC_END, category, name, id, TRACE_EVENT_FLAG_NONE, arg1_name, \
        arg1_val)
