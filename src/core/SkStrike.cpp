...
SkStrike::SkStrike(
    const SkDescriptor& desc, std::unique_ptr<SkScalerContext> scaler,
...
SkGlyph* SkStrike::lookupByPackedGlyphID(SkPackedGlyphID packedGlyphID, MetricsType type) {
  ...
  if (glyphPtr == nullptr) {
    ...
    switch (type) {
      ...
      case kNothing_MetricsType: break;
      case kJustAdvance_MetricsType: fScalerContext->getAdvance(glyphPtr); break;
      case kFull_MetricsType: fScalerContext->getMetrics(glyphPtr); break;
    }
  } else {
...
SkSpan<const SkGlyphPos> SkStrike::prepareForDrawing(
    const SkGlyphID glyphIDs[], const SkPoint positions[], size_t n, int maxDimension,
    PreparationDetail detail, SkGlyphPos result[]) {
    ...
    if (SkScalarsAreFinite(position.x(), position.y())) {
      ...
      if (!glyph.isEmpty()) {
        ...
        if (glyph.maxDimension() <= maxDimension) {
          ...
          SkASSERT(
              glyph.maxDimension() > maxDimension && glyph.fMaskFormat == SkMask::kARGB32_Format);
        }
      }
    }
  }
...
void SkStrike::OffsetResults(
    const SkGlyph::Intercept* intercept, SkScalar scale, SkScalar xPos, SkScalar* array,
    int* count) {
  if (array) {
    array += *count;
...
void SkStrike::AddPoints(
    const SkPoint* pts, int ptCount, const SkScalar bounds[2], bool yAxis,
...
void SkStrike::AddLine(
    const SkPoint pts[2], SkScalar axis, bool yAxis, SkGlyph::Intercept* intercept) {
  ...
  if (0 <= t && t < 1) {  // this handles divide by zero above
    AddInterval(
        yAxis ? pts[0].fY + t * (pts[1].fY - pts[0].fY) : pts[0].fX + t * (pts[1].fX - pts[0].fX),
        intercept);
  }
}

void SkStrike::AddQuad(
    const SkPoint pts[3], SkScalar axis, bool yAxis, SkGlyph::Intercept* intercept) {
  SkDQuad quad;
  quad.set(pts);
  double roots[2];
...
void SkStrike::AddCubic(
    const SkPoint pts[4], SkScalar axis, bool yAxis, SkGlyph::Intercept* intercept) {
  SkDCubic cubic;
  cubic.set(pts);
  double roots[3];
  int count = yAxis ? cubic.verticalIntersect(axis, roots) : cubic.horizontalIntersect(axis, roots);
  while (--count >= 0) {
...
void SkStrike::findIntercepts(
    const SkScalar bounds[2], SkScalar scale, SkScalar xPos, bool yAxis, SkGlyph* glyph,
