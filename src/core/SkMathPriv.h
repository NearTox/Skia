...
// From Hacker's Delight 2nd Edition
constexpr int SkCTZ_portable(uint32_t x) noexcept { return 32 - SkCLZ_portable(~x & (x - 1)); }
...
static inline int SkCTZ(uint32_t mask) noexcept {
  if (mask) {
    unsigned long index = 0;
...
#else
static inline int SkCTZ(uint32_t mask) noexcept { return SkCTZ_portable(mask); }
#endif
...
constexpr int SkNextLog2_portable(uint32_t value) noexcept {
  SkASSERT(value != 0);
...
constexpr int SkPrevLog2_portable(uint32_t value) noexcept {
  SkASSERT(value != 0);
  return 32 - SkCLZ_portable(value >> 1);
}
...
constexpr int SkNextPow2_portable(int value) noexcept {
  SkASSERT(value > 0);
...
constexpr int SkPrevPow2_portable(int value) noexcept {
  SkASSERT(value > 0);
