...
static constexpr int return_check_zero(int value) noexcept {
  if (value == 0) {
    return 0;
  }
  return value;
...
static bool cubic_dchop_at_intercept(
    const SkPoint src[4], SkScalar intercept, SkPoint dst[7], InterceptProc method) {
  SkDCubic cubic;
  double roots[3];
...
static constexpr bool between(SkScalar a, SkScalar b, SkScalar c) { return (a - b) * (c - b) <= 0; }
...
void SkConic::computeTightBounds(SkRect* bounds) const noexcept {
  SkPoint pts[4];
  pts[0] = fPts[0];
  pts[1] = fPts[2];
  int count = 2;
