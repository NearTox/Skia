...
static inline float2 interp(const float2& v0, const float2& v1, const float2& t) {
  return v0 + (v1 - v0) * t;
}
...
inline static skvx::Vec<N, T> unchecked_mix(
    const skvx::Vec<N, T>& a, const skvx::Vec<N, T>& b, const skvx::Vec<N, T>& t) {
  return (b - a) * t + a;
}
...
float SkMeasureNonInflectCubicRotation(const SkPoint pts[4]) {
  ...
  return 2 * SK_ScalarPI - SkMeasureAngleBetweenVectors(a, -b) -
         SkMeasureAngleBetweenVectors(b, -c);
}
...
float SkFindCubicMidTangent(const SkPoint src[4]) {
  ...
  static const skvx::float4 kM[4] = {
      skvx::float4(-1, 2, -1, 0), skvx::float4(3, -4, 1, 0), skvx::float4(-3, 2, 0, 0)};
  auto C_x =
      fma(kM[0], src[0].fX,
          fma(kM[1], src[1].fX, fma(kM[2], src[2].fX, skvx::float4(src[3].fX, 0, 0, 0))));
  auto C_y =
      fma(kM[0], src[0].fY,
          fma(kM[1], src[1].fY, fma(kM[2], src[2].fY, skvx::float4(src[3].fY, 0, 0, 0))));
