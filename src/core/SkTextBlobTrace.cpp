...
void SkTextBlobTrace::DumpTrace(const std::vector<SkTextBlobTrace::Record>& trace) {
  for (const SkTextBlobTrace::Record& record : trace) {
    ...
    bool weirdPaint = p.getStyle() != SkPaint::kFill_Style || p.getMaskFilter() != nullptr ||
                      p.getPathEffect() != nullptr;

    SkDebugf(
        "Blob %d ( %g %g ) %d\n  ", blob->uniqueID(), record.offset.x(), record.offset.y(),
        weirdPaint);
    ...
    while (!iter.done()) {
      ...
      SkDebugf(
          ...
          font.getSize(), font.getScaleX(), font.getSkewX(), SkFontPriv::Flags(font),
          (int)font.getEdging(), (int)font.getHinting());
