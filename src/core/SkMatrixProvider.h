...
class SkMatrixProvider {
 public:
  ...
  bool localToDeviceHitsPixelCenters() const noexcept { return fHitsPixelCenters; }

 protected:
  SkMatrixProvider(const SkMatrix& localToDevice, bool hitsPixelCenters) noexcept 
      : fLocalToDevice(localToDevice),
        fLocalToDevice33(localToDevice),
        fHitsPixelCenters(hitsPixelCenters) {}

  SkMatrixProvider(const SkM44& localToDevice, bool hitsPixelCenters) noexcept 
      : fLocalToDevice(localToDevice),
...
class SkPostTranslateMatrixProvider : public SkMatrixProvider {
 public:
  SkPostTranslateMatrixProvider(const SkMatrixProvider& parent, SkScalar dx, SkScalar dy)
      : SkMatrixProvider(
            SkM44::Translate(dx, dy) * parent.localToDevice44(),
...
class SkPreConcatMatrixProvider : public SkMatrixProvider {
 public:
  SkPreConcatMatrixProvider(const SkMatrixProvider& parent, const SkMatrix& preMatrix)
      : SkMatrixProvider(
            parent.localToDevice44() * SkM44(preMatrix), parent.localToDeviceHitsPixelCenters()) {}
};
