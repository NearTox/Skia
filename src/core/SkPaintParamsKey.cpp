...
SkPaintParamsKeyBuilder::SkPaintParamsKeyBuilder(
    const SkShaderCodeDictionary* dict, SkBackend backend)
    : fDict(dict), fBackend(backend) {}
...
void SkPaintParamsKeyBuilder::beginBlock(int codeSnippetID) {
  ...
  fStack.push_back({ codeSnippetID, this->sizeInBytes(), SkDEBUGCODE(kHeaderExpectations, 0) });
...
#ifdef SK_DEBUG
void SkPaintParamsKeyBuilder::checkExpectations(
    SkPaintParamsKey::DataPayloadType actualType, uint32_t actualCount) {
  ...
  SkASSERT(
      frame.fCurDataPayloadEntry < SkTo<int>(expectations.size()) && expectations.data() &&
...
SkPaintParamsKey::SkPaintParamsKey(
    SkSpan<const uint8_t> span, SkPaintParamsKeyBuilder* originatingBuilder)
    : fData(span), fOriginatingBuilder(originatingBuilder) {
  fOriginatingBuilder->lock();
}

SkPaintParamsKey::SkPaintParamsKey(SkSpan<const uint8_t> rawData)
    : fData(rawData), fOriginatingBuilder(nullptr) {}
...
SkPaintParamsKey::BlockReader SkPaintParamsKey::reader(
    const SkShaderCodeDictionary* dict, int headerOffset) const {
...
void SkPaintParamsKey::AddBlockToShaderInfo(
    SkShaderCodeDictionary* dict, const SkPaintParamsKey::BlockReader& reader,
    SkShaderInfo* result) {
  result->add(reader);
#ifdef SK_GRAPHITE_ENABLED
...
std::pair<SkBuiltInCodeSnippetID, uint8_t> read_header(
    SkSpan<const uint8_t> parentSpan, int headerOffset) {
  SkASSERT(
      headerOffset + SkPaintParamsKey::kBlockHeaderSizeInBytes <= SkTo<int>(parentSpan.size()));
...
SkPaintParamsKey::BlockReader::BlockReader(
    const SkShaderCodeDictionary* dict, SkSpan<const uint8_t> parentSpan, int offsetInParent) {
...
SkPaintParamsKey::BlockReader SkPaintParamsKey::BlockReader::child(
    const SkShaderCodeDictionary* dict, int childIndex) const {
...
SkSpan<const uint8_t> SkPaintParamsKey::BlockReader::bytes(int fieldIndex) const {
  ...
  return dataPayload.subspan(
      byteOffsetInPayload, fEntry->fDataPayloadExpectations[fieldIndex].fCount);
}
...
void SkPaintParamsKey::BlockReader::dump(const SkShaderCodeDictionary* dict, int indent) const {
  ...
  for (int i = 0; i < (int)fEntry->fDataPayloadExpectations.size(); ++i) {
    output_indent(indent);
    SkDebugf(
        "%s[%d]: ", fEntry->fDataPayloadExpectations[i].fName,
