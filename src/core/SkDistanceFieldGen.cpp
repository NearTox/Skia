...
static void init_glyph_data(
    DFData* data, unsigned char* edges, const unsigned char* image, int dataWidth, int dataHeight,
    int imageWidth, int imageHeight, int pad) {
  data += pad * dataWidth;
  data += pad;
    ...
    for (int i = 0; i < width; ++i) {
      if (*edges) {
        ...
        currGrad.fY = (nextData - 1)->fAlpha - (prevData - 1)->fAlpha +
                      SK_ScalarSqrt2 * nextData->fAlpha - SK_ScalarSqrt2 * prevData->fAlpha +
...
static bool generate_distance_field_from_image(
    unsigned char* distanceField, const unsigned char* copyPtr, int width, int height) {
  ...
  init_glyph_data(
      dataPtr, edgePtr, copyPtr, dataWidth, dataHeight, width + 2, height + 2, SK_DistanceFieldPad);
...
bool SkGenerateDistanceFieldFromA8Image(
    unsigned char* distanceField, const unsigned char* image, int width, int height,
    size_t rowBytes) {
  SkASSERT(distanceField);
  SkASSERT(image);
