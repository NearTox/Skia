...
void add_solid_uniform_data(
    const SkShaderCodeDictionary* dict, const SkPMColor4f& premulColor,
...
void SolidColorShaderBlock::BeginBlock(
    const SkKeyContext& keyContext, SkPaintParamsKeyBuilder* builder,
    SkPipelineDataGatherer* gatherer, const SkPMColor4f& premulColor) {
#ifdef SK_GRAPHITE_ENABLED
...
void add_linear_gradient_uniform_data(
    const SkShaderCodeDictionary* dict, SkBuiltInCodeSnippetID codeSnippetID,
    const GradientShaderBlocks::GradientData& gradData, SkPipelineDataGatherer* gatherer) {
...
void add_radial_gradient_uniform_data(
    const SkShaderCodeDictionary* dict, SkBuiltInCodeSnippetID codeSnippetID,
    const GradientShaderBlocks::GradientData& gradData, SkPipelineDataGatherer* gatherer) {
  VALIDATE_UNIFORMS(gatherer, dict, codeSnippetID)
...
void add_sweep_gradient_uniform_data(
    const SkShaderCodeDictionary* dict, SkBuiltInCodeSnippetID codeSnippetID,
    const GradientShaderBlocks::GradientData& gradData, SkPipelineDataGatherer* gatherer) {
  VALIDATE_UNIFORMS(gatherer, dict, codeSnippetID)
...
void add_conical_gradient_uniform_data(
    const SkShaderCodeDictionary* dict, SkBuiltInCodeSnippetID codeSnippetID,
    const GradientShaderBlocks::GradientData& gradData, SkPipelineDataGatherer* gatherer) {
  VALIDATE_UNIFORMS(gatherer, dict, codeSnippetID)
...
GradientShaderBlocks::GradientData::GradientData(SkShader::GradientType type, int numStops)
    : fType(type),
...
GradientShaderBlocks::GradientData::GradientData(
    SkShader::GradientType type, const SkM44& localMatrix, SkPoint point0, SkPoint point1,
    float radius0, float radius1, float bias, float scale, SkTileMode tm, int numStops,
    SkColor4f* color4fs, float* offsets)
    : fType(type),
...
void GradientShaderBlocks::BeginBlock(
    const SkKeyContext& keyContext, SkPaintParamsKeyBuilder* builder,
    SkPipelineDataGatherer* gatherer, const GradientData& gradData) {
#ifdef SK_GRAPHITE_ENABLED
  if (builder->backend() == SkBackend::kGraphite) {
    auto dict = keyContext.dict();
    ...
    switch (gradData.fType) {
      case SkShader::kLinear_GradientType:
        codeSnippetID = gradData.fNumStops <= 4 ? SkBuiltInCodeSnippetID::kLinearGradientShader4
      ...
      case SkShader::kRadial_GradientType:
        codeSnippetID = gradData.fNumStops <= 4 ? SkBuiltInCodeSnippetID::kRadialGradientShader4
      ...
      case SkShader::kSweep_GradientType:
        codeSnippetID = gradData.fNumStops <= 4 ? SkBuiltInCodeSnippetID::kSweepGradientShader4
      ...
      case SkShader::GradientType::kConical_GradientType:
        codeSnippetID = gradData.fNumStops <= 4 ? SkBuiltInCodeSnippetID::kConicalGradientShader4
...
void add_localmatrixshader_uniform_data(
    const SkShaderCodeDictionary* dict, const SkM44& localMatrix,
    SkPipelineDataGatherer* gatherer) {
  ...
  gatherer->addFlags(dict->getSnippetRequirementFlags(SkBuiltInCodeSnippetID::kLocalMatrixShader));
}
...
void LocalMatrixShaderBlock::BeginBlock(
    const SkKeyContext& keyContext, SkPaintParamsKeyBuilder* builder,
    SkPipelineDataGatherer* gatherer, const LMShaderData& lmShaderData) {
#ifdef SK_GRAPHITE_ENABLED
  if (builder->backend() == SkBackend::kGraphite) {
    auto dict = keyContext.dict();
...
void add_image_uniform_data(
    const SkShaderCodeDictionary* dict, const ImageShaderBlock::ImageData& imgData,
    SkPipelineDataGatherer* gatherer) {
...
ImageShaderBlock::ImageData::ImageData(
    const SkSamplingOptions& sampling, SkTileMode tileModeX, SkTileMode tileModeY, SkRect subset,
...
void ImageShaderBlock::BeginBlock(
    const SkKeyContext& keyContext, SkPaintParamsKeyBuilder* builder,
    SkPipelineDataGatherer* gatherer, const ImageData& imgData) {
#ifdef SK_GRAPHITE_ENABLED
  if (builder->backend() == SkBackend::kGraphite) {
...
void add_blendshader_uniform_data(
    const SkShaderCodeDictionary* dict, SkBlendMode bm, SkPipelineDataGatherer* gatherer) {
...
void BlendShaderBlock::BeginBlock(
    const SkKeyContext& keyContext, SkPaintParamsKeyBuilder* builder,
    SkPipelineDataGatherer* gatherer, const BlendShaderData& blendData) {
#ifdef SK_GRAPHITE_ENABLED
  if (builder->backend() == SkBackend::kGraphite) {
    auto dict = keyContext.dict();
    // When extracted into SkShaderInfo::SnippetEntries the children will appear after their
...
constexpr skgpu::BlendInfo make_simple_blendInfo(
    skgpu::BlendCoeff srcCoeff, skgpu::BlendCoeff dstCoeff) {
  return {
      skgpu::BlendEquation::kAdd, srcCoeff, dstCoeff, SK_PMColor4fTRANSPARENT,
...
void add_shaderbasedblender_uniform_data(
    const SkShaderCodeDictionary* dict, SkBlendMode bm, SkPipelineDataGatherer* gatherer) {
  ...
  gatherer->addFlags(dict->getSnippetRequirementFlags(SkBuiltInCodeSnippetID::kShaderBasedBlender));
}
...
void BlendModeBlock::BeginBlock(
    const SkKeyContext& keyContext, SkPaintParamsKeyBuilder* builder,
    SkPipelineDataGatherer* gatherer, SkBlendMode bm) {
#ifdef SK_GRAPHITE_ENABLED
  if (builder->backend() == SkBackend::kGraphite) {
    auto dict = keyContext.dict();
...
// combination code
SkUniquePaintParamsID CreateKey(
    const SkKeyContext& keyContext, SkPaintParamsKeyBuilder* builder, SkShaderType s,
    SkBlendMode bm) {
  ...
  switch (s) {
    ...
    case SkShaderType::kLinearGradient:
      GradientShaderBlocks::BeginBlock(
          keyContext, builder, nullptr,
    ...
    case SkShaderType::kLocalMatrix:
      LocalMatrixShaderBlock::BeginBlock(
          keyContext, builder, nullptr,
          // matrix is unused
          {SkMatrix::I()});
    ...
    case SkShaderType::kImage:
      ImageShaderBlock::BeginBlock(
          keyContext, builder, nullptr,
          // none of the ImageData is used
          {SkSamplingOptions(), SkTileMode::kClamp, SkTileMode::kClamp, SkRect::MakeEmpty(),
           SkMatrix::I()});
      builder->endBlock();
      break;
    case SkShaderType::kBlendShader:
      BlendShaderBlock::BeginBlock(keyContext, builder, nullptr, {SkBlendMode::kSrc});
