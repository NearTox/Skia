...
static bool is_plane_config_compatible_with_subsampling(
    SkYUVAInfo::PlaneConfig config, SkYUVAInfo::Subsampling subsampling) {
  ...
  return subsampling == SkYUVAInfo::Subsampling::k444 ||
         (config != SkYUVAInfo::PlaneConfig::kYUV && config != SkYUVAInfo::PlaneConfig::kYUVA &&
          config != SkYUVAInfo::PlaneConfig::kUYV && config != SkYUVAInfo::PlaneConfig::kUYVA);
}

int SkYUVAInfo::PlaneDimensions(
    SkISize imageDimensions, PlaneConfig planeConfig, Subsampling subsampling,
    SkEncodedOrigin origin, SkISize planeDimensions[SkYUVAInfo::kMaxPlanes]) {
...
static bool channel_index_to_channel(
...
bool SkYUVAInfo::GetYUVAIndices(
    PlaneConfig config, const uint32_t planeChannelFlags[kMaxPlanes],
...
SkYUVAInfo::SkYUVAInfo(
    SkISize dimensions, PlaneConfig planeConfig, Subsampling subsampling,
    SkYUVColorSpace yuvColorSpace, SkEncodedOrigin origin, Siting sitingX, Siting sitingY)
...
SkYUVAInfo::SkYUVAInfo(
    SkISize dimensions, PlanarConfig planarConfig, SkYUVColorSpace yuvColorSpace,
    SkEncodedOrigin origin, Siting sitingX, Siting sitingY) {
...
size_t SkYUVAInfo::computeTotalBytes(
    const size_t rowBytes[kMaxPlanes], size_t planeSizes[kMaxPlanes]) const {
  if (!this->isValid()) {
    return 0;
  }
  SkSafeMath safe;
...
SkYUVAInfo::PlanarConfig SkYUVAInfo::planarConfig() const {
  SkASSERT(
      !this->isValid() || is_plane_config_compatible_with_subsampling(fPlaneConfig, fSubsampling));
  switch (fPlaneConfig) {
    ...
    case PlaneConfig::kY_V_U:
      return fSubsampling == Subsampling::k420 ? PlanarConfig::kY_V_U_420 : PlanarConfig::kUnknown;

    case PlaneConfig::kY_UV:
      return fSubsampling == Subsampling::k420 ? PlanarConfig::kY_UV_420 : PlanarConfig::kUnknown;

    case PlaneConfig::kY_VU:
      return fSubsampling == Subsampling::k420 ? PlanarConfig::kY_VU_420 : PlanarConfig::kUnknown;
...
bool SkYUVAInfo::operator==(const SkYUVAInfo& that) const {
  return fPlaneConfig == that.fPlaneConfig && fSubsampling == that.fSubsampling &&
         fYUVColorSpace == that.fYUVColorSpace && fDimensions == that.fDimensions &&
         fSitingX == that.fSitingX && fSitingY == that.fSitingY && fOrigin == that.fOrigin;
}
