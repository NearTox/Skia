...
struct ColorTypeFilter_88 {
  typedef uint16_t Type;
  static constexpr uint32_t Expand(uint16_t x) noexcept { return (x & 0xFF) | ((x & ~0xFF) << 8); }
  static constexpr uint16_t Compact(uint32_t x) noexcept { return (x & 0xFF) | ((x >> 8) & ~0xFF); }
};

struct ColorTypeFilter_1616 {
  typedef uint32_t Type;
  static constexpr uint64_t Expand(uint32_t x) noexcept {
    return (x & 0xFFFF) | ((x & ~0xFFFF) << 16);
  }
  static constexpr uint16_t Compact(uint64_t x) noexcept {
...
struct ColorTypeFilter_16 {
  typedef uint16_t Type;
  static constexpr uint32_t Expand(uint16_t x) noexcept { return x; }
  static constexpr uint16_t Compact(uint32_t x) noexcept { return (uint16_t)x; }
};

struct ColorTypeFilter_1010102 {
  typedef uint32_t Type;
  static constexpr uint64_t Expand(uint64_t x) noexcept {
    return (((x)&0x3ff)) | (((x >> 10) & 0x3ff) << 20) | (((x >> 20) & 0x3ff) << 40) |
           (((x >> 30) & 0x3) << 60);
  }
  static constexpr uint32_t Compact(uint64_t x) noexcept {
...
SkMipMap* SkMipMap::Build(const SkPixmap& src, SkDiscardableFactoryProc fact) {
  ...
  switch (ct) {
    ...
    case kRGBA_F32_SkColorType: return nullptr;
  }
  ...
  for (int i = 0; i < countLevels; ++i) {
    ...
    levels[i].fScale =
        SkSize::Make(SkIntToScalar(width) / src.width(), SkIntToScalar(height) / src.height());
