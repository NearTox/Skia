...
static bool write_string(SkWStream* stream, const SkString& string, uint32_t id) {
  ...
  return stream->writePackedUInt(id) && stream->writePackedUInt(string.size()) &&
...
static bool write_scalar(SkWStream* stream, SkScalar n, uint32_t id) {
  return stream->writePackedUInt(id) && stream->writeScalar(n);
}
...
static constexpr SkScalar usWidths[9]{1, 2, 3, 4, 5, 6, 7, 8, 9};
static constexpr SkScalar width_for_usWidth[0x10] = {50,  50,  62.5, 75,  87.5, 100, 112.5, 125,
  ...
  for (size_t id; (id = read_id(stream)) != kSentinel;) {
    switch (id) {
      ...
      case kPostscriptName:
        ...
        break;
      case kWeight:
        if (!stream->readScalar(&weight)) {
          return false;
        }
        break;
      case kWidth:
        if (!stream->readScalar(&width)) {
          return false;
        }
        break;
      case kSlant:
        if (!stream->readScalar(&slant)) {
          return false;
        }
        break;
      case kItalic:
        if (!stream->readScalar(&italic)) {
          return false;
        }
        break;
      case kFontVariation:
      ...
      case kFontIndex:
        ...
        break;
      case kPaletteIndex:
        if (!stream->readPackedUInt(&paletteIndex)) {
          return false;
        }
        if (!SkTFitsIn<PaletteIndexType>(paletteIndex)) {
          return false;
        }
        result->fPaletteIndex = SkTo<PaletteIndexType>(paletteIndex);
        break;
      case kPaletteEntryOverrides:
        if (!stream->readPackedUInt(&paletteEntryOverrideCount)) {
          return false;
        }
        ...
        for (size_t i = 0; i < paletteEntryOverrideCount; ++i) {
          if (!stream->readPackedUInt(&paletteEntryOverrideIndex)) {
            return false;
          }
        ...
        break;
      default: SkDEBUGFAIL("Unknown id used by a font descriptor"); return false;
    }
  }

  SkFontStyle::Slant slantEnum = SkFontStyle::kUpright_Slant;
  if (slant != 0) {
    slantEnum = SkFontStyle::kOblique_Slant;
  }
  if (0 < italic) {
    slantEnum = SkFontStyle::kItalic_Slant;
  }
