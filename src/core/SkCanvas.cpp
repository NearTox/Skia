...
class AutoLayerForImageFilter {
 public:
  ...
  AutoLayerForImageFilter(
      SkCanvas* canvas, const SkPaint& origPaint, bool skipLayerForImageFilter = false,
...
SkSurface* SkCanvas::getSurface() const noexcept { return fSurfaceBase; }

SkISize SkCanvas::getBaseLayerSize() const noexcept {
...
void SkCanvas::internalSaveLayer(const SaveLayerRec& rec, SaveLayerStrategy strategy) {
  ...
  if (imageFilter) {
    ...
    sk_sp<SkImageFilter> modifiedFilter =
        as_IFB(imageFilter)->applyCTM(stashedMatrix, &modifiedCTM);
  ...
  if (layer) {
    if (fMCRec) {
      ...
      this->internalDrawDevice(
          layer->fDevice.get(), layer->fPaint.get(), layer->fClipImage.get(), layer->fClipMatrix);
...
void SkCanvas::internalDrawDevice(
    SkBaseDevice* srcDev, const SkPaint* paint, SkImage* clipImage, const SkMatrix& clipMatrix) {
  SkPaint tmp;
  if (nullptr == paint) {
    paint = &tmp;
  }
...
void SkCanvas::onClipRect(const SkRect& rect, SkClipOp op, ClipEdgeStyle edgeStyle) {
  ...
  fMCRec->fRasterClip.opRect(
      rect, fMCRec->fMatrix.asM33(), this->getTopLayerBounds(), (SkRegion::Op)op, isAA);
  fDeviceClipBounds = qr_clip_bounds(fMCRec->fRasterClip.getBounds());
}
...
void SkCanvas::onClipRRect(const SkRRect& rrect, SkClipOp op, ClipEdgeStyle edgeStyle) {
  ...
  fMCRec->fRasterClip.opRRect(
      rrect, fMCRec->fMatrix.asM33(), this->getTopLayerBounds(), (SkRegion::Op)op, isAA);
  fDeviceClipBounds = qr_clip_bounds(fMCRec->fRasterClip.getBounds());
}
...
void SkCanvas::onClipPath(const SkPath& path, SkClipOp op, ClipEdgeStyle edgeStyle) {
  ...
  fMCRec->fRasterClip.opPath(
      *rasterClipPath, fMCRec->fMatrix.asM33(), this->getTopLayerBounds(), (SkRegion::Op)op, isAA);
  fDeviceClipBounds = qr_clip_bounds(fMCRec->fRasterClip.getBounds());
}
...
SkMatrix SkCanvas::getTotalMatrix() const noexcept { return fMCRec->fMatrix.asM33(); }
...
void SkCanvas::LayerIter::next() noexcept {
  fDone = !fImpl->next();
  if (!fDone) {
...
const SkMatrix& SkCanvas::LayerIter::matrix() const noexcept {
...
int SkCanvas::LayerIter::x() const noexcept { return fDeviceOrigin.fX; }
int SkCanvas::LayerIter::y() const noexcept { return fDeviceOrigin.fY; }
