 ...
 */
bool SkCanvas::wouldOverwriteEntireSurface(
    const SkRect* rect, const SkPaint* paint, ShaderOverrideOpacity overrideOpacity) const {
  static_assert(
      (int)SkPaintPriv::kNone_ShaderOverrideOpacity == (int)kNone_ShaderOverrideOpacity,
      "need_matching_enums0");
  static_assert(
      (int)SkPaintPriv::kOpaque_ShaderOverrideOpacity == (int)kOpaque_ShaderOverrideOpacity,
      "need_matching_enums1");
  static_assert(
      (int)SkPaintPriv::kNotOpaque_ShaderOverrideOpacity == (int)kNotOpaque_ShaderOverrideOpacity,
...
bool SkCanvas::predrawNotify(bool willOverwritesEntireSurface) {
  if (fSurfaceBase) {
    if (!fSurfaceBase->aboutToDraw(
            willOverwritesEntireSurface ? SkSurface::kDiscard_ContentChangeMode
...
bool SkCanvas::predrawNotify(
    const SkRect* rect, const SkPaint* paint, ShaderOverrideOpacity overrideOpacity) {
  if (fSurfaceBase) {
...
SkCanvas::Layer::Layer(
    sk_sp<SkBaseDevice> device, sk_sp<SkImageFilter> imageFilter, const SkPaint& paint) noexcept
...
SkCanvas::MCRec::MCRec(SkBaseDevice* device) noexcept : fDevice(device) {
  SkASSERT(fDevice);
  inc_rec();
}

SkCanvas::MCRec::MCRec(const MCRec* prev) noexcept
    : fDevice(prev->fDevice), fMatrix(prev->fMatrix) {
  SkASSERT(fDevice);
  inc_rec();
}
...
void SkCanvas::MCRec::newLayer(
    sk_sp<SkBaseDevice> layerDevice, sk_sp<SkImageFilter> filter, const SkPaint& restorePaint) {
  SkASSERT(!fBackImage);
...
class AutoLayerForImageFilter {
 public:
  ...
  AutoLayerForImageFilter(SkCanvas* canvas, const SkPaint& paint, const SkRect* rawBounds = nullptr)
    ...
    if (fPaint.getImageFilter() && !image_to_color_filter(&fPaint)) {
      ...
      canvas->fSaveCount += 1;
      (void)canvas->internalSaveLayer(
          SkCanvas::SaveLayerRec(rawBounds, &restorePaint), SkCanvas::kFullLayer_SaveLayerStrategy);
  ...
  AutoLayerForImageFilter(AutoLayerForImageFilter&&) noexcept = default;
  AutoLayerForImageFilter& operator=(AutoLayerForImageFilter&&) noexcept = default;
...
std::optional<AutoLayerForImageFilter> SkCanvas::aboutToDraw(
    SkCanvas* canvas, const SkPaint& paint, const SkRect* rawBounds,
    CheckForOverwrite checkOverwrite, ShaderOverrideOpacity overrideOpacity) {
...
SkCanvas::SkCanvas(
    const SkBitmap& bitmap, std::unique_ptr<SkRasterHandleAllocator> alloc,
    SkRasterHandleAllocator::Handle hndl, const SkSurfaceProps* props)
    : fMCStack(sizeof(MCRec), fMCRecStorage, sizeof(fMCRecStorage)),
      fProps(SkSurfacePropsCopyOrDefault(props)),
...
SkSurface* SkCanvas::getSurface() const { return fSurfaceBase; }

SkISize SkCanvas::getBaseLayerSize() const { return this->baseDevice()->imageInfo().dimensions(); }
...
bool SkCanvas::writePixels(
    const SkImageInfo& srcInfo, const void* pixels, size_t rowBytes, int x, int y) {
...
static skif::ParameterSpace<SkPoint> compute_decomposition_center(
    const SkMatrix& dstToLocal, const skif::ParameterSpace<SkRect>* contentBounds,
...
static std::pair<skif::Mapping, skif::LayerSpace<SkIRect>> get_layer_mapping_and_bounds(
    const SkImageFilter* filter, const SkMatrix& localToDst,
    const skif::DeviceSpace<SkIRect>& targetOutput,
    const skif::ParameterSpace<SkRect>* contentBounds = nullptr, bool mustCoverDst = true,
  ...
  SkMatrix dstToLocal;
  if (!localToDst.isFinite() || !localToDst.invert(&dstToLocal)) {
  ...
  if (scaleFactor != 1.0f && !mapping.adjustLayerSpace(SkMatrix::Scale(scaleFactor, scaleFactor))) {
    return failedMapping();
  }
  ...
  int maxLayerDim = std::max(
      Sk64_pin_to_s32(
          2 * std::max(SkIRect(targetOutput).width64(), SkIRect(targetOutput).height64())),
      kMinDimThreshold);
  ...
  if (filter) {
    ...
    if (layerBounds.width() > maxLayerDim || layerBounds.height() > maxLayerDim) {
      ...
      auto idealLayerBounds =
          as_IFB(filter)->getInputBounds(idealMapping, targetOutput, contentBounds);
      maxLayerDim =
          std::max(std::max(idealLayerBounds.width(), idealLayerBounds.height()), maxLayerDim);
    }
  } else {
  ...
  if (layerBounds.width() > maxLayerDim || layerBounds.height() > maxLayerDim) {
    skif::LayerSpace<SkIRect> newLayerBounds(SkIRect::MakeWH(
        std::min(layerBounds.width(), maxLayerDim), std::min(layerBounds.height(), maxLayerDim)));
    SkMatrix adjust = SkMatrix::MakeRectToRect(
        SkRect::Make(SkIRect(layerBounds)), SkRect::Make(SkIRect(newLayerBounds)),
...
static SkImageInfo make_layer_info(const SkImageInfo& prev, int w, int h, bool f16) {
  ...
  if (!f16 && prev.bytesPerPixel() <= 4 && prev.colorType() != kRGBA_8888_SkColorType &&
...
void SkCanvas::internalDrawDeviceWithFilter(
    SkBaseDevice* src, SkBaseDevice* dst, const SkImageFilter* filter, const SkPaint& paint,
    DeviceCompatibleWithFilter compat, SkScalar scaleFactor) {
  check_drawdevice_colorspaces(dst->imageInfo().colorSpace(), src->imageInfo().colorSpace());
  ...
  if (compat == DeviceCompatibleWithFilter::kYes) {
    ...
    mapping =
        skif::Mapping(src->getRelativeTransform(*dst), dst->getRelativeTransform(*src), localToSrc);
    ...
    std::tie(mapping, requiredInput) = get_layer_mapping_and_bounds(
        filter, dst->localToDevice(), skif::DeviceSpace<SkIRect>(dst->devClipBounds()), nullptr,
    ...
    if (draw_layer_as_sprite(srcToIntermediate, srcDims)) {
      ...
      skif::LayerSpace<SkIPoint> srcOrigin(
          {(int)srcToIntermediate.getTranslateX(), (int)srcToIntermediate.getTranslateY()});
  ...
  if (!needsIntermediateImage) {
    ...
    SkRect srcRect;
    if (!SkMatrixPriv::InverseMapRect(
            srcToIntermediate, &srcRect, SkRect::Make(SkIRect(requiredInput)))) {
      return;
    }
    ...
    if (srcSubset.intersect(SkIRect::MakeSize(srcDims)) &&
      ...
      SkBaseDevice::CreateInfo info(
          make_layer_info(src->imageInfo(), requiredInput.width(), requiredInput.height(), false),
          SkPixelGeometry::kUnknown_SkPixelGeometry, SkBaseDevice::TileUsage::kNever_TileUsage,
          fAllocator.get());
      sk_sp<SkBaseDevice> intermediateDevice(src->onCreateDevice(info, &paint));
      if (!intermediateDevice) {
        return;
      }
      intermediateDevice->setOrigin(
          SkM44(srcToIntermediate), requiredInput.left(), requiredInput.top());
      ...
      SkPaint imageFill;
      imageFill.setShader(srcImage->asShader(
          SkTileMode::kClamp, SkSamplingOptions{SkFilterMode::kLinear},
  ...
  if (filterInput) {
    const bool use_nn = draw_layer_as_sprite(mapping.layerToDevice(), filterInput->subset().size());
...
static const SkImageFilter* optimize_layer_filter(
    const SkImageFilter* filter, SkPaint* paint, bool* coversDevice = nullptr) {
  SkASSERT(paint);
  SkColorFilter* cf;
...
// See skbug.com/8783
static bool must_cover_prior_device(const SkImageFilter* backdrop, const SkPaint& restorePaint) {
...
void SkCanvas::internalSaveLayer(const SaveLayerRec& rec, SaveLayerStrategy strategy) {
  ...
  if (strategy == kFullLayer_SaveLayerStrategy) {
    ...
    SkImageInfo info = make_layer_info(
        priorDevice->imageInfo(), layerBounds.width(), layerBounds.height(),
    ...
    const auto createInfo =
        SkBaseDevice::CreateInfo(info, geo, SkBaseDevice::kNever_TileUsage, fAllocator.get());
  ...
  if (!newDevice) {
    ...
    // doesn't exist.
    newDevice = sk_make_sp<SkNoPixelsDevice>(
        SkIRect::MakeWH(layerBounds.width(), layerBounds.height()), fProps,
  ...
  newDevice->setDeviceCoordinateSystem(
      ...
      SkM44(newLayerMapping.layerMatrix()), layerBounds.left(), layerBounds.top());
  ...
  if (initBackdrop) {
    ...
    auto compat = (filter || backdropFilter || scaleBackdrop) ? DeviceCompatibleWithFilter::kUnknown
                                                              : DeviceCompatibleWithFilter::kYes;
    this->internalDrawDeviceWithFilter(
        priorDevice,      // src
        newDevice.get(),  // dst
        backdropFilter, backdropPaint, compat, rec.fExperimentalBackdropScale);
  }
...
void SkCanvas::internalRestore() {
  ...
  if (backImage) {
    ...
    this->topDevice()->drawSpecial(
        backImage->fImage.get(), SkMatrix::Translate(backImage->fLoc), SkSamplingOptions(), paint);
  }
  ...
  if (layer && !layer->fDevice->isNoPixelsDevice() && !layer->fDiscard) {
    ...
    if (this->predrawNotify()) {
      ...
      if (layer->fImageFilter) {
        this->internalDrawDeviceWithFilter(
            layer->fDevice.get(),  // src
            dstDev,                // dst
            layer->fImageFilter.get(), layer->fPaint, DeviceCompatibleWithFilter::kYes);
      } else {
...
SkImageInfo SkCanvas::onImageInfo() const { return this->baseDevice()->imageInfo(); }
...
bool SkCanvas::onPeekPixels(SkPixmap* pmap) { return this->baseDevice()->peekPixels(pmap); }
...
void SkCanvas::setMatrix(const SkMatrix& matrix) { this->setMatrix(SkM44(matrix)); }
...
void SkCanvas::resetMatrix() { this->setMatrix(SkM44()); }
...
bool SkCanvas::androidFramework_isClipAA() const { return this->topDevice()->onClipIsAA(); }
...
bool SkCanvas::internalQuickReject(
    const SkRect& bounds, const SkPaint& paint, const SkMatrix* matrix) {
...
SkMatrix SkCanvas::getTotalMatrix() const { return fMCRec->fMatrix.asM33(); }

SkM44 SkCanvas::getLocalToDevice() const { return fMCRec->fMatrix; }

#if defined(SK_BUILD_FOR_ANDROID_FRAMEWORK) && SK_SUPPORT_GPU

SkIRect SkCanvas::topLayerBounds() const { return this->topDevice()->getGlobalBounds(); }
...
void SkCanvas::drawVertices(
    const sk_sp<SkVertices>& vertices, SkBlendMode mode, const SkPaint& paint) {
...
void SkCanvas::drawImageLattice(
  ...
  if (SkLatticeIter::Valid(image->width(), image->height(), latticePlusBounds)) {
    ...
    this->drawImageRect(
        image, SkRect::MakeIWH(image->width(), image->height()), dst, SkSamplingOptions(filter),
        &latticePaint, kStrict_SrcRectConstraint);
  }
}

void SkCanvas::drawAtlas(
    const SkImage* atlas, const SkRSXform xform[], const SkRect tex[], const SkColor colors[],
    int count, SkBlendMode mode, const SkSamplingOptions& sampling, const SkRect* cull,
    const SkPaint* paint) {
  TRACE_EVENT0("skia", TRACE_FUNC);
...
void SkCanvas::experimental_DrawEdgeAAQuad(
...
void SkCanvas::experimental_DrawEdgeAAImageSet(
    const ImageSetEntry imageSet[], int cnt, const SkPoint dstClips[],
    const SkMatrix preViewMatrices[], const SkSamplingOptions& sampling, const SkPaint* paint,
    SrcRectConstraint constraint) {
  TRACE_EVENT0("skia", TRACE_FUNC);
  this->onDrawEdgeAAImageSet2(
      imageSet, cnt, dstClips, preViewMatrices, sampling, paint, constraint);
}
...
void SkCanvas::onDrawBehind(const SkPaint& paint) {
  ...
  for (;;) {
    ...
    if (rec->fBackImage) {
      ...
      bounds = SkIRect::MakeXYWH(
          rec->fBackImage->fLoc.fX, rec->fBackImage->fLoc.fY, rec->fBackImage->fImage->width(),
...
void SkCanvas::onDrawArc(
    const SkRect& oval, SkScalar startAngle, SkScalar sweepAngle, bool useCenter,
    const SkPaint& paint) {
  SkASSERT(oval.isSorted());
  if (this->internalQuickReject(oval, paint)) {
    return;
...
bool SkCanvas::canDrawBitmapAsSprite(
    SkScalar x, SkScalar y, int w, int h, const SkSamplingOptions& sampling, const SkPaint& paint) {
...
void SkCanvas::onDrawImage2(
    const SkImage* image, SkScalar x, SkScalar y, const SkSamplingOptions& sampling,
    const SkPaint* paint) {
  ...
  if (layer) {
    this->topDevice()->drawImageRect(
        image, nullptr, bounds, sampling, layer->paint(), kFast_SrcRectConstraint);
  }
}

static SkSamplingOptions clean_sampling_for_constraint(
    const SkSamplingOptions& sampling, SkCanvas::SrcRectConstraint constraint) {
...
void SkCanvas::onDrawImageRect2(
  ...
  auto layer = this->aboutToDraw(
      this, realPaint, &dst, CheckForOverwrite::kYes,
      image->isOpaque() ? kOpaque_ShaderOverrideOpacity : kNotOpaque_ShaderOverrideOpacity);
  if (layer) {
    this->topDevice()->drawImageRect(image, &src, dst, realSampling, layer->paint(), constraint);
  }
}

void SkCanvas::onDrawImageLattice2(
    const SkImage* image, const Lattice& lattice, const SkRect& dst, SkFilterMode filter,
    const SkPaint* paint) {
  SkPaint realPaint = clean_paint_for_drawImage(paint);
...
void SkCanvas::drawImageRect(
    const SkImage* image, const SkRect& dst, const SkSamplingOptions& sampling,
    const SkPaint* paint) {
  RETURN_ON_NULL(image);
  this->drawImageRect(
      image, SkRect::MakeIWH(image->width(), image->height()), dst, sampling, paint,
      kFast_SrcRectConstraint);
}

void SkCanvas::onDrawTextBlob(
...
sk_sp<Slug> SkCanvas::onConvertGlyphRunListToSlug(
...
void SkCanvas::drawSimpleText(
    const void* text, size_t byteLength, SkTextEncoding encoding, SkScalar x, SkScalar y,
    const SkFont& font, const SkPaint& paint) {
  TRACE_EVENT0("skia", TRACE_FUNC);
  if (byteLength) {
    sk_msan_assert_initialized(text, SkTAddOffset<const void>(text, byteLength));
    const SkGlyphRunList& glyphRunList = fScratchGlyphRunBuilder->textToGlyphRunList(
...
void SkCanvas::drawGlyphs(
    int count, const SkGlyphID* glyphs, const SkPoint* positions, const uint32_t* clusters,
    int textByteCount, const char* utf8text, SkPoint origin, const SkFont& font,
    const SkPaint& paint) {
  if (count <= 0) {
    return;
  }
...
void SkCanvas::drawGlyphs(
    int count, const SkGlyphID glyphs[], const SkPoint positions[], SkPoint origin,
    const SkFont& font, const SkPaint& paint) {
  if (count <= 0) {
    return;
  }
...
void SkCanvas::drawGlyphs(
    int count, const SkGlyphID glyphs[], const SkRSXform xforms[], SkPoint origin,
    const SkFont& font, const SkPaint& paint) {
  if (count <= 0) {
    return;
  }
...
void SkCanvas::drawTextBlob(const SkTextBlob* blob, SkScalar x, SkScalar y, const SkPaint& paint) {
  ...
  // form.
  if (!gSkBlobAsSlugTesting || this->topDevice()->asGaneshDevice() == nullptr ||
...
void SkCanvas::onDrawVerticesObject(
...
void SkCanvas::onDrawPatch(
  ...
  auto layer = this->aboutToDraw(this, simplePaint, &bounds);
  if (layer) {
    this->topDevice()->drawPatch(cubics, colors, texCoords, SkBlender::Mode(bmode), layer->paint());
  }
}
...
void SkCanvas::onDrawAtlas2(
    const SkImage* atlas, const SkRSXform xform[], const SkRect tex[], const SkColor colors[],
    int count, SkBlendMode bmode, const SkSamplingOptions& sampling, const SkRect* cull,
    const SkPaint* paint) {
  ...
  if (layer) {
    this->topDevice()->drawAtlas(xform, tex, colors, count, SkBlender::Mode(bmode), layer->paint());
  }
}
...
void SkCanvas::onDrawEdgeAAQuad(
    const SkRect& r, const SkPoint clip[4], QuadAAFlags edgeAA, const SkColor4f& color,
    SkBlendMode mode) {
  SkASSERT(r.isSorted());
...
void SkCanvas::onDrawEdgeAAImageSet2(
  ...
  if (layer) {
    this->topDevice()->drawEdgeAAImageSet(
        imageSet, count, dstClips, preViewMatrices, realSampling, layer->paint(), constraint);
  }
}
...
std::unique_ptr<SkCanvas> SkCanvas::MakeRasterDirect(
    const SkImageInfo& info, void* pixels, size_t rowBytes, const SkSurfaceProps* props) {
...
SkNoDrawCanvas::SkNoDrawCanvas(int width, int height) : INHERITED(SkIRect::MakeWH(width, height)) {}

SkNoDrawCanvas::SkNoDrawCanvas(const SkIRect& bounds) : INHERITED(bounds) {}
...
bool SkNoDrawCanvas::onDoSaveBehind(const SkRect*) { return false; }
...
static bool install(
    SkBitmap* bm, const SkImageInfo& info, const SkRasterHandleAllocator::Rec& rec) {
  return bm->installPixels(info, rec.fPixels, rec.fRowBytes, rec.fReleaseProc, rec.fReleaseCtx);
}

SkRasterHandleAllocator::Handle SkRasterHandleAllocator::allocBitmap(
    const SkImageInfo& info, SkBitmap* bm) {
