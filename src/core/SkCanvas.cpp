...
struct DeviceCM {
  ...
  DeviceCM(
      sk_sp<SkBaseDevice> device, const SkPaint* paint, const SkMatrix& stashed,
...
static SkPaint* set_if_needed(SkLazyPaint* lazy, const SkPaint& orig) noexcept {
 ...
 */
static const SkRect& apply_paint_to_bounds_sans_imagefilter(
    const SkPaint& paint, const SkRect& rawBounds, SkRect* storage) {
...
class AutoDrawLooper {
 public:
  ...
  AutoDrawLooper(
      SkCanvas* canvas, const SkPaint& paint, bool skipLayerForImageFilter = false,
    ...
    if (!skipLayerForImageFilter && fPaint->getImageFilter()) {
      ...
      (void)canvas->internalSaveLayer(
          SkCanvas::SaveLayerRec(rawBounds, &tmp), SkCanvas::kFullLayer_SaveLayerStrategy);
...
SkCanvas::SkCanvas(int width, int height, const SkSurfaceProps* props)
  ...
  inc_canvas();
  this->init(
      sk_make_sp<SkNoPixelsDevice>(SkIRect::MakeWH(SkTMax(width, 0), SkTMax(height, 0)), fProps));
}
...
bool SkCanvas::clipRectBounds(
    const SkRect* bounds, SaveLayerFlags saveLayerFlags, SkIRect* intersection,
...
void SkCanvas::DrawDeviceWithFilter(
  ...
  if (filter) {
    ...
    SkIRect newBounds =
        filter->filterBounds(snapBounds, ctm, SkImageFilter::kReverse_MapDirection, &snapBounds);
    ...
    SkASSERT(
        localCTM.isScaleTranslate() || filter->canHandleComplexCTM() || localCTM.hasPerspective());
...
void SkCanvas::internalSaveLayer(const SaveLayerRec& rec, SaveLayerStrategy strategy) {
  ...
  if (imageFilter) {
    ...
    sk_sp<SkImageFilter> modifiedFilter =
        SkApplyCTMToFilter(imageFilter, stashedMatrix, &modifiedCTM);
  ...
  if ((rec.fSaveLayerFlags & kInitWithPrevious_SaveLayerFlag) || rec.fBackdrop) {
    DrawDeviceWithFilter(
        priorDevice, rec.fBackdrop, newDevice.get(), {ir.fLeft, ir.fTop}, fMCRec->fMatrix);
  }
...
void SkCanvas::internalRestore() {
  ...
  if (backImage) {
    ...
    this->getTopDevice()->drawSpecial(backImage->fImage.get(), x, y, paint, nullptr, SkMatrix::I());
  }
  ...
  */
  if (layer) {
    if (fMCRec) {
      ...
      this->internalDrawDevice(
          layer->fDevice.get(), origin.x(), origin.y(), layer->fPaint.get(),
...
SkImageInfo SkCanvas::imageInfo() const noexcept { return this->onImageInfo(); }

SkImageInfo SkCanvas::onImageInfo() const noexcept {
  SkBaseDevice* dev = this->getDevice();
  if (dev) {
...
bool SkCanvas::getProps(SkSurfaceProps* props) const noexcept { return this->onGetProps(props); }

bool SkCanvas::onGetProps(SkSurfaceProps* props) const noexcept {
  SkBaseDevice* dev = this->getDevice();
  if (dev) {
    if (props) {
      *props = fProps;
...
void SkCanvas::internalDrawDevice(
    SkBaseDevice* srcDev, int x, int y, const SkPaint* paint, SkImage* clipImage,
  ...
  while (iter.next()) {
    ...
    check_drawdevice_colorspaces(
        dstDev->imageInfo().colorSpace(), srcDev->imageInfo().colorSpace());
    paint = &looper.paint();
    ...
    if (filter || clipImage) {
      ...
      if (specialImage) {
        check_drawdevice_colorspaces(
            dstDev->imageInfo().colorSpace(), specialImage->getColorSpace());
        dstDev->drawSpecial(specialImage.get(), pos.x(), pos.y(), *paint, clipImage, clipMatrix);
      }
    } else {
...
void SkCanvas::internalSetMatrix(const SkMatrix& matrix) noexcept {
...
void SkCanvas::onClipRect(const SkRect& rect, SkClipOp op, ClipEdgeStyle edgeStyle) {
  ...
  fMCRec->fRasterClip.opRect(
      rect, fMCRec->fMatrix, this->getTopLayerBounds(), (SkRegion::Op)op, isAA);
  fDeviceClipBounds = qr_clip_bounds(fMCRec->fRasterClip.getBounds());
}
...
void SkCanvas::onClipRRect(const SkRRect& rrect, SkClipOp op, ClipEdgeStyle edgeStyle) {
  ...
  fMCRec->fRasterClip.opRRect(
      rrect, fMCRec->fMatrix, this->getTopLayerBounds(), (SkRegion::Op)op, isAA);
  fDeviceClipBounds = qr_clip_bounds(fMCRec->fRasterClip.getBounds());
}
...
void SkCanvas::onClipPath(const SkPath& path, SkClipOp op, ClipEdgeStyle edgeStyle) {
  ...
  fMCRec->fRasterClip.opPath(
      *rasterClipPath, *matrix, this->getTopLayerBounds(), (SkRegion::Op)op, isAA);
  fDeviceClipBounds = qr_clip_bounds(fMCRec->fRasterClip.getBounds());
}
...
bool SkCanvas::isClipEmpty() const noexcept {
...
void SkCanvas::drawBitmapLattice(
...
void SkCanvas::legacy_drawBitmapRect(
    const SkBitmap& bitmap, const SkRect* src, const SkRect& dst, const SkPaint* paint,
    SrcRectConstraint constraint) {
  if (src) {
...
static bool needs_autodrawlooper(SkCanvas* canvas, const SkPaint& paint) noexcept {
  return ((intptr_t)paint.getImageFilter() | (intptr_t)paint.getLooper()) != 0;
}
...
// null.
static const SkPaint* init_image_paint(SkPaint* real, const SkPaint* paintParam) noexcept {
  if (paintParam) {
    *real = *paintParam;
...
void SkCanvas::onDrawImage(const SkImage* image, SkScalar x, SkScalar y, const SkPaint* paint) {
  ...
  while (iter.next()) {
    ...
    if (special) {
      ...
      iter.fDevice->drawSpecial(
          special.get(), SkScalarRoundToInt(pt.fX), SkScalarRoundToInt(pt.fY), pnt, nullptr,
          SkMatrix::I());
    } else {
...
void SkCanvas::onDrawBitmap(const SkBitmap& bitmap, SkScalar x, SkScalar y, const SkPaint* paint) {
  ...
  sk_sp<SkSpecialImage> special;
  bool drawAsSprite =
      canFastBounds && this->canDrawBitmapAsSprite(x, y, bitmap.width(), bitmap.height(), *paint);
  if (drawAsSprite && paint->getImageFilter()) {
  ...
  while (iter.next()) {
    ...
    if (special) {
      ...
      iter.fDevice->drawBitmapRect(
          bitmap, &fullImage, fullImage.makeOffset(x, y), pnt, kStrict_SrcRectConstraint);
    }
  }
...
void SkCanvas::internalDrawBitmapRect(
    const SkBitmap& bitmap, const SkRect* src, const SkRect& dst, const SkPaint* paint,
    SrcRectConstraint constraint) {
  if (bitmap.drawsNothing() || dst.isEmpty()) {
    return;
...
void SkCanvas::onDrawBitmapLattice(
    const SkBitmap& bitmap, const Lattice& lattice, const SkRect& dst, const SkPaint* paint) {
  SkPaint realPaint;
  paint = init_image_paint(&realPaint, paint);
...
void SkCanvas::onDrawEdgeAAQuad(
    const SkRect& r, const SkPoint clip[4], QuadAAFlags edgeAA, SkColor color, SkBlendMode mode) {
  SkASSERT(r.isSorted());
...
SkCanvas::ImageSetEntry& SkCanvas::ImageSetEntry::operator=(const ImageSetEntry&) noexcept =
    default;
