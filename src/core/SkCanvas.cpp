...
static SkSamplingOptions paint_to_sampling(const SkPaint* paint, const GrRecordingContext* ctx) {
  SkSamplingOptions::MediumBehavior mb =
      ctx ? SkSamplingOptions::kMedium_asMipmapLinear : SkSamplingOptions::kMedium_asMipmapNearest;
...
class SkCanvas::MCRec {
 public:
  ...
  MCRec(SkBaseDevice* device)
      : fLayer(nullptr), fDevice(device), fBackImage(nullptr), fDeferredSaveCount(0) {
    SkASSERT(fDevice);
    fMatrix.setIdentity();
    inc_rec();
  }
  MCRec(const MCRec& prev)
      : fLayer(nullptr), fDevice(prev.fDevice), fMatrix(prev.fMatrix), fDeferredSaveCount(0) {
    SkASSERT(fDevice);
    inc_rec();
  }
  ~MCRec() { dec_rec(); }
  ...
  void newLayer(
      sk_sp<SkBaseDevice> layerDevice, const SkPaint* restorePaint, const SkM44& stashedMatrix) {
    SkASSERT(!fBackImage);
...
SkCanvas::SkCanvas() : fMCStack(sizeof(MCRec), fMCRecStorage, sizeof(fMCRecStorage)), fProps() {
  inc_canvas();
  this->init(nullptr);
}
...
bool SkCanvas::clipRectBounds(
    const SkRect* bounds, SkIRect* intersection, const SkImageFilter* imageFilter) {
...
void SkCanvas::DrawDeviceWithFilter(
  ...
  SkIRect layerTargetBounds =
      fromRoot.mapRect(SkRect::MakeXYWH(dstOrigin.x(), dstOrigin.y(), dst->width(), dst->height()))
          .roundOut();
  ...
  if (!toRoot.isIdentity()) {
    ...
    tmpCanvas->drawImageRect(
        special->asImage().get(), SkRect::Make(special->subset()), SkRect::Make(backdropBounds),
        sampling, &p, kStrict_SrcRectConstraint);
...
void SkCanvas::internalSaveLayer(const SaveLayerRec& rec, SaveLayerStrategy strategy) {
  ...
  if (imageFilter) {
    ...
    sk_sp<SkImageFilter> modifiedFilter =
        as_IFB(imageFilter)->applyCTM(stashedMatrix.asM33(), &modifiedCTM);
  ...
  if (!newDevice) {
    ...
    newDevice = sk_make_sp<SkNoPixelsDevice>(
        SkIRect::MakeWH(ir.width(), ir.height()), fProps, this->imageInfo().refColorSpace());
    initBackdrop = false;
  }
  ...
  if (initBackdrop) {
    DrawDeviceWithFilter(
        priorDevice, rec.fBackdrop, newDevice.get(), {ir.fLeft, ir.fTop}, fMCRec->fMatrix.asM33());
  }
...
void SkCanvas::internalDrawDevice(
    SkBaseDevice* srcDev, const SkSamplingOptions& sampling, const SkPaint* paint) {
  ...
  check_drawdevice_colorspaces(dstDev->imageInfo().colorSpace(), srcDev->imageInfo().colorSpace());
...
static SK_NEVER_INLINE bool quick_reject_slow_path(
    const SkRect& src, const SkRect& deviceClip, const SkMatrix& matrix) {
  SkRect deviceRect;
...
SkIRect SkCanvas::getDeviceClipBounds() const { return this->computeDeviceClipBounds().roundOut(); }
...
GrRecordingContext* SkCanvas::recordingContext() { return this->topDevice()->recordingContext(); }
...
#else
  this->drawImageRect(
      image, src, dst, paint_to_sampling(paint, this->recordingContext()), paint, constraint);
#endif
}

void SkCanvas::drawImageRect(
    const SkImage* image, const SkIRect& isrc, const SkRect& dst, const SkPaint* paint,
    SrcRectConstraint constraint) {
  RETURN_ON_NULL(image);
...
void SkCanvas::drawImageRect(const SkImage* image, const SkRect& dst, const SkPaint* paint) {
  RETURN_ON_NULL(image);
  this->drawImageRect(
      image, SkRect::MakeIWH(image->width(), image->height()), dst, paint, kFast_SrcRectConstraint);
}
...
void SkCanvas::drawImageLattice(
  ...
  if (SkLatticeIter::Valid(image->width(), image->height(), latticePlusBounds)) {
    ...
    this->drawImageRect(
        image, SkRect::MakeIWH(image->width(), image->height()), dst,
        SkSamplingOptions(filter, SkMipmapMode::kNone), paint, kStrict_SrcRectConstraint);
  }
}
...
void SkCanvas::drawBitmapRect(
    const SkBitmap& bitmap, const SkIRect& isrc, const SkRect& dst, const SkPaint* paint,
    SrcRectConstraint constraint) {
  this->drawBitmapRect(bitmap, SkRect::Make(isrc), dst, paint, constraint);
}

void SkCanvas::drawBitmapRect(
    const SkBitmap& bitmap, const SkRect& dst, const SkPaint* paint, SrcRectConstraint constraint) {
  this->drawBitmapRect(
      bitmap, SkRect::MakeIWH(bitmap.width(), bitmap.height()), dst, paint, constraint);
}
...
#else
  this->drawAtlas(
      atlas, xform, tex, colors, count, mode, paint_to_sampling(paint, this->recordingContext()),
      cull, paint);
#endif
}
...
void SkCanvas::legacy_drawImageRect(
    const SkImage* image, const SkRect* src, const SkRect& dst, const SkPaint* paint,
    SrcRectConstraint constraint) {
  RETURN_ON_NULL(image);
  if (src) {
    ...
    this->drawImageRect(
        image, SkRect::MakeIWH(image->width(), image->height()), dst, paint, constraint);
  }
}
...
#else
  this->experimental_DrawEdgeAAImageSet(
      imageSet, cnt, dstClips, preViewMatrices, paint_to_sampling(paint, this->recordingContext()),
      paint, constraint);
#endif
}
...
void SkCanvas::onDrawImageLattice(
    const SkImage* image, const Lattice& lattice, const SkRect& dst, const SkPaint* paint) {
  SkPaint realPaint = clean_paint_for_drawImage(paint);
  ...
  this->topDevice()->drawImageLattice(
      image, lattice, dst, paint_to_filter(&realPaint), layer.paint());
}
...
void SkCanvas::onDrawEdgeAAImageSet(
  ...
  this->topDevice()->drawEdgeAAImageSet(
      ...
      SkSamplingOptions(SkPaintPriv::GetFQ(layer.paint())), layer.paint(), constraint);
}
...
void SkCanvas::drawImageRect(
  ...
  RETURN_ON_NULL(image);
  this->drawImageRect(
      image, SkRect::MakeIWH(image->width(), image->height()), dst, sampling, paint, constraint);
}
