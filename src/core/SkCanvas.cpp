...
void SkCanvas::internalRestore() {
    ...
    if (backImage) {
        ...
        this->getTopDevice()->drawSpecial(backImage->fImage.get(), x, y, paint, nullptr,
                                          SkMatrix::I());
    }
...
static SK_NEVER_INLINE bool quick_reject_slow_path(const SkRect& src, const SkRect& deviceClip,
                                                   const SkMatrix& matrix) noexcept {
    SkRect deviceRect;
    matrix.mapRect(&deviceRect, src);
    return !deviceRect.isFinite() || !deviceRect.intersect(deviceClip);
}

bool SkCanvas::quickReject(const SkRect& src) const noexcept {
#ifdef SK_DEBUG
...
void SkCanvas::onDrawEdgeAAImageSet(const ImageSetEntry imageSet[], int count,
    ...
    while (iter.next()) {
        iter.fDevice->drawEdgeAAImageSet(imageSet, count, dstClips, preViewMatrices, looper.paint(),
                                         constraint);
    }
    LOOPER_END
}
...
SkCanvas::ImageSetEntry::ImageSetEntry(sk_sp<const SkImage> image, const SkRect& srcRect,
                                       ...
                                       unsigned aaFlags, bool hasClip) noexcept
...
SkCanvas::ImageSetEntry::ImageSetEntry(sk_sp<const SkImage> image, const SkRect& srcRect,
                                       const SkRect& dstRect, float alpha,
