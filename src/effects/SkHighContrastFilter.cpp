...
skvm::Color SkHighContrast_Filter::onProgram(
  ...
  if (fConfig.fGrayscale) {
    skvm::F32 gray = c.r * SK_LUM_COEFF_R + c.g * SK_LUM_COEFF_G + c.b * SK_LUM_COEFF_B;
...
void GLHighContrastFilterEffect::onSetData(
    const GrGLSLProgramDataManager& pdm, const GrFragmentProcessor& proc) {
...
void GLHighContrastFilterEffect::emitCode(EmitArgs& args) {
  ...
  if (config.fGrayscale) {
    fragBuilder->codeAppendf(
        "half luma = dot(color, half4(%f, %f, %f, 0));", SK_LUM_COEFF_R, SK_LUM_COEFF_G,
        SK_LUM_COEFF_B);
  ...
  if (config.fInvertStyle == InvertStyle::kInvertLightness) {
    ...
    fragBuilder->emitFunction(
        kHalf_GrSLType, "hue2rgb", SK_ARRAY_COUNT(gHue2rgbArgs), gHue2rgbArgs,
        "if (t < 0)"
        "  t += 1;"
        "if (t > 1)"
        "  t -= 1;"
