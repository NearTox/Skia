...
sk_sp<SkColorFilter> SkHighContrastFilter::Make(const SkHighContrastConfig& config) {
  ...
  sk_sp<SkRuntimeEffect> effect =
      SkMakeCachedRuntimeEffect(SkRuntimeEffect::MakeForColorFilter, std::move(code));
  SkASSERT(effect);
  ...
  return SkColorFilters::WithWorkingFormat(
      effect->makeColorFilter(SkData::MakeWithCopy(&uniforms, sizeof(uniforms))), &linear,
      nullptr /*use dst gamut*/, &unpremul);
#else
  // TODO(skia:12197)
  return nullptr;
#endif
