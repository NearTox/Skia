...
sk_sp<SkPathEffect> SkMergePathEffect::Make(
    sk_sp<SkPathEffect> one, sk_sp<SkPathEffect> two, SkPathOp op) {
...
bool SkOpPE::onFilterPath(
    SkPath* dst, const SkPath& src, SkStrokeRec* rec, const SkRect* cull,
...
bool SkOpPE::computeFastBounds(SkRect* bounds) const {
  ...
  switch (fOp) {
    ...
    case SkPathOp::kUnion_SkPathOp: bounds->join(b2); break;
  }
...
SkMatrixPE::SkMatrixPE(const SkMatrix& matrix) : fMatrix(matrix) { SkASSERT(matrix.isFinite()); }

bool SkMatrixPE::onFilterPath(
...
sk_sp<SkPathEffect> SkStrokePathEffect::Make(
    SkScalar width, SkPaint::Join join, SkPaint::Cap cap, SkScalar miter) {
...
static bool known_to_be_opposite_directions(const SkPath& a, const SkPath& b) {
  auto a_dir = SkPathPriv::ComputeFirstDirection(a), b_dir = SkPathPriv::ComputeFirstDirection(b);

  return (a_dir == SkPathFirstDirection::kCCW && b_dir == SkPathFirstDirection::kCW) ||
         (a_dir == SkPathFirstDirection::kCW && b_dir == SkPathFirstDirection::kCCW);
}

bool SkStrokeAndFillPE::onFilterPath(
    SkPath* dst, const SkPath& src, SkStrokeRec* rec, const SkRect*, const SkMatrix&) const {
