...
sk_sp<SkImageFilter> SkImageFilters::Tile(
  ...
  if (src.width() == dst.width() && src.height() == dst.height()) {
    ...
    return SkImageFilters::Offset(dst.x() - src.x(), dst.y() - src.y(), std::move(input), &ir);
  }
...
sk_sp<SkSpecialImage> SkTileImageFilter::onFilterImage(const Context& ctx, SkIPoint* offset) const {
  ...
  if (inputBounds.contains(srcIRect)) {
    ...
    sk_sp<SkSurface> surf(
        input->makeTightSurface(ctx.colorType(), ctx.colorSpace(), srcIRect.size()));
    if (!surf) {
      return nullptr;
    }
    ...
    input->draw(
        canvas, SkIntToScalar(inputOffset.x()), SkIntToScalar(inputOffset.y()), SkSamplingOptions(),
        &paint);
  ...
  paint.setBlendMode(SkBlendMode::kSrc);
  paint.setShader(
      subset->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions()));
...
SkIRect SkTileImageFilter::onFilterBounds(
    const SkIRect& src, const SkMatrix&, MapDirection, const SkIRect* inputRect) const {
  // Don't recurse into inputs.
  return src;
}

SkRect SkTileImageFilter::computeFastBounds(const SkRect& src) const { return fDstRect; }
