...
sk_sp<SkImageFilter> SkTileImageFilter::Make(
    const SkRect& srcRect, const SkRect& dstRect, sk_sp<SkImageFilter> input) {
  ...
  if (srcRect.width() == dstRect.width() && srcRect.height() == dstRect.height()) {
    ...
    return SkOffsetImageFilter::Make(
        dstRect.x() - srcRect.x(), dstRect.y() - srcRect.y(), std::move(input), &cropRect);
  }
...
sk_sp<SkSpecialImage> SkTileImageFilterImpl::onFilterImage(
  ...
  if (inputBounds.contains(srcIRect)) {
    ...
    input->draw(canvas, SkIntToScalar(inputOffset.x()), SkIntToScalar(inputOffset.y()), &paint);
