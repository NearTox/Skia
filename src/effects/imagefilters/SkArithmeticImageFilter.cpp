...
sk_sp<SkImageFilter> SkImageFilters::Arithmetic(
  ...
  if (mode >= 0) {
    return SkImageFilters::Blend(
        (SkBlendMode)mode, std::move(background), std::move(foreground), cropRect);
  }
...
void arith_span(const SkV4& k, SkPMColor dst[], const SkPMColor src[], int count) {
  const skvx::float4 k1 = k[0] * (1 / 255.0f), k2 = k[1], k3 = k[2], k4 = k[3] * 255.0f + 0.5f;
...
template <bool EnforcePMColor>
void arith_transparent(const SkV4& k, SkPMColor dst[], int count) {
  const skvx::float4 k3 = k[2], k4 = k[3] * 255.0f + 0.5f;
...
sk_sp<SkSpecialImage> SkArithmeticImageFilter::onFilterImage(
...
SkIRect SkArithmeticImageFilter::onFilterBounds(
...
std::unique_ptr<GrFragmentProcessor> make_arithmetic_fp(
  ...
  return GrSkSLFP::Make(
      effect, "arithmetic_fp", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "srcFP",
      std::move(srcFP), "dstFP", std::move(dstFP), "k", k, "pmClamp", enforcePMColor ? 0.0f : 1.0f);
}
...
void SkArithmeticImageFilter::drawForeground(
