...
sk_sp<SkSpecialImage> ArithmeticImageFilterImpl::filterImageGPU(
  ...
  if (background) {
    ...
    bgFP = GrTextureEffect::MakeTexelSubset(
        backgroundView.detachProxy(), background->alphaType(), backgroundMatrix, sampler, bgSubset,
        caps);
  ...
  if (foreground) {
    ...
    auto fgFP = GrTextureEffect::MakeTexelSubset(
        foregroundView.detachProxy(), foreground->alphaType(), foregroundMatrix, sampler, fgSubset,
        caps);
    ...
    std::unique_ptr<GrFragmentProcessor> xferFP =
        GrSkSLFP::Make(context, effect, "Arithmetic", &inputs, sizeof(inputs));
    if (xferFP) {
