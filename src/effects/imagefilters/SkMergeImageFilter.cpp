...
sk_sp<SkImageFilter> SkMergeImageFilter::Make(
...
sk_sp<SkSpecialImage> SkMergeImageFilter::onFilterImage(
  ...
  for (int i = 0; i < inputCount; ++i) {
    ...
    const SkIRect inputBounds =
        SkIRect::MakeXYWH(offsets[i].fX, offsets[i].fY, inputs[i]->width(), inputs[i]->height());
  ...
  for (int i = 0; i < inputCount; ++i) {
    ...
    inputs[i]->draw(
        canvas, SkIntToScalar(offsets[i].x() - x0), SkIntToScalar(offsets[i].y() - y0), nullptr);
  }
...
void SkMergeImageFilter::flatten(SkWriteBuffer& buffer) const { this->INHERITED::flatten(buffer); }
