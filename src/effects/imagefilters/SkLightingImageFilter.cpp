...
static void setUniformPoint3(
    const GrGLSLProgramDataManager& pdman, UniformHandle uni, const SkPoint3& point) {
  GR_STATIC_ASSERT(sizeof(SkPoint3) == 3 * sizeof(float));
  pdman.set3fv(uni, 1, &point.fX);
}

static void setUniformNormal3(
    const GrGLSLProgramDataManager& pdman, UniformHandle uni, const SkPoint3& point) {
...
static SkPoint3 read_point3(SkReadBuffer& buffer) noexcept {
  ...
  buffer.validate(
      SkScalarIsFinite(point.fX) && SkScalarIsFinite(point.fY) && SkScalarIsFinite(point.fZ));
  return point;
};

static void write_point3(const SkPoint3& point, SkWriteBuffer& buffer) noexcept {
...
class BaseLightingType {
 public:
  constexpr BaseLightingType() noexcept {}
  virtual ~BaseLightingType() {}

  virtual SkPMColor light(
      const SkPoint3& normal, const SkPoint3& surfaceTolight, const SkPoint3& lightColor) const
      noexcept = 0;
};
...
class DiffuseLightingType : public BaseLightingType {
 public:
  ...
  SkPMColor light(
    ...
    return SkPackARGB32(
        255, SkClampMax(SkScalarRoundToInt(color.fX), 255),
...
class SpecularLightingType : public BaseLightingType {
 public:
  ...
  SkPMColor light(
    ...
    SkPoint3 color = lightColor.makeScale(colorScale);
    return SkPackARGB32(
        SkClampMax(SkScalarRoundToInt(max_component(color)), 255),
...
static constexpr inline SkScalar sobel(
    int a, int b, int c, int d, int e, int f, SkScalar scale) noexcept {
...
static void lightBitmap(
    const BaseLightingType& lightingType, const SkImageFilterLight* l, const SkBitmap& src,
    SkBitmap* dst, SkScalar surfaceScale, const SkIRect& bounds) {
  ...
  SkPMColor* dptr = dst->getAddr32(0, 0);
  {
    ...
    *dptr++ = lightingType.light(
        topLeftNormal(m, surfaceScale), surfaceToLight, l->lightColor(surfaceToLight));
    for (++x; x < right - 1; ++x) {
      shiftMatrixLeft(m);
      m[5] = PixelFetcher::Fetch(src, x + 1, y, srcBounds);
      m[8] = PixelFetcher::Fetch(src, x + 1, y + 1, srcBounds);
      ...
      *dptr++ = lightingType.light(
          topNormal(m, surfaceScale), surfaceToLight, l->lightColor(surfaceToLight));
    }
    shiftMatrixLeft(m);
    surfaceToLight = l->surfaceToLight(x, y, m[4], surfaceScale);
    *dptr++ = lightingType.light(
        topRightNormal(m, surfaceScale), surfaceToLight, l->lightColor(surfaceToLight));
  }
  ...
  for (++y; y < bottom - 1; ++y) {
    ...
    SkPoint3 surfaceToLight = l->surfaceToLight(x, y, m[4], surfaceScale);
    *dptr++ = lightingType.light(
        leftNormal(m, surfaceScale), surfaceToLight, l->lightColor(surfaceToLight));
    for (++x; x < right - 1; ++x) {
      shiftMatrixLeft(m);
      m[2] = PixelFetcher::Fetch(src, x + 1, y - 1, srcBounds);
      m[5] = PixelFetcher::Fetch(src, x + 1, y, srcBounds);
      ...
      surfaceToLight = l->surfaceToLight(x, y, m[4], surfaceScale);
      *dptr++ = lightingType.light(
          interiorNormal(m, surfaceScale), surfaceToLight, l->lightColor(surfaceToLight));
    }
    shiftMatrixLeft(m);
    surfaceToLight = l->surfaceToLight(x, y, m[4], surfaceScale);
    *dptr++ = lightingType.light(
        rightNormal(m, surfaceScale), surfaceToLight, l->lightColor(surfaceToLight));
  }

  {
    ...
    SkPoint3 surfaceToLight = l->surfaceToLight(x, y, m[4], surfaceScale);
    *dptr++ = lightingType.light(
        bottomLeftNormal(m, surfaceScale), surfaceToLight, l->lightColor(surfaceToLight));
    for (++x; x < right - 1; ++x) {
      shiftMatrixLeft(m);
      m[2] = PixelFetcher::Fetch(src, x + 1, bottom - 2, srcBounds);
      m[5] = PixelFetcher::Fetch(src, x + 1, bottom - 1, srcBounds);
      surfaceToLight = l->surfaceToLight(x, y, m[4], surfaceScale);
      *dptr++ = lightingType.light(
          bottomNormal(m, surfaceScale), surfaceToLight, l->lightColor(surfaceToLight));
    }
    shiftMatrixLeft(m);
    surfaceToLight = l->surfaceToLight(x, y, m[4], surfaceScale);
    *dptr++ = lightingType.light(
        bottomRightNormal(m, surfaceScale), surfaceToLight, l->lightColor(surfaceToLight));
  }
}

static void lightBitmap(
    const BaseLightingType& lightingType, const SkImageFilterLight* light, const SkBitmap& src,
    SkBitmap* dst, SkScalar surfaceScale, const SkIRect& bounds) {
  if (src.bounds().contains(bounds)) {
    lightBitmap<UncheckedPixelFetcher>(lightingType, light, src, dst, surfaceScale, bounds);
  } else {
    lightBitmap<DecalPixelFetcher>(lightingType, light, src, dst, surfaceScale, bounds);
  }
}
...
class SkLightingImageFilterInternal : public SkLightingImageFilter {
 protected:
  ...
  sk_sp<SkSpecialImage> filterImageGPU(
      SkSpecialImage* source, SkSpecialImage* input, const SkIRect& bounds, const SkMatrix& matrix,
      const OutputProperties& outputProperties) const;
  virtual std::unique_ptr<GrFragmentProcessor> makeFragmentProcessor(
      sk_sp<GrTextureProxy>, const SkMatrix&, const SkIRect* srcBounds,
      BoundaryMode boundaryMode) const = 0;
#endif
 private:
#if SK_SUPPORT_GPU
  void drawRect(
      GrRenderTargetContext*, sk_sp<GrTextureProxy> srcProxy, const SkMatrix& matrix,
      const GrClip& clip, const SkRect& dstRect, BoundaryMode boundaryMode,
      const SkIRect* srcBounds, const SkIRect& bounds) const;
#endif
...
void SkLightingImageFilterInternal::drawRect(
    GrRenderTargetContext* renderTargetContext, sk_sp<GrTextureProxy> srcProxy,
    const SkMatrix& matrix, const GrClip& clip, const SkRect& dstRect, BoundaryMode boundaryMode,
    const SkIRect* srcBounds, const SkIRect& bounds) const {
...
sk_sp<SkSpecialImage> SkLightingImageFilterInternal::filterImageGPU(
    ...
    const SkMatrix& matrix, const OutputProperties& outputProperties) const {
  ...
  this->drawRect(
      renderTargetContext.get(), inputProxy, matrix, clip, topLeft, kTopLeft_BoundaryMode,
      pSrcBounds, offsetBounds);
  this->drawRect(
      renderTargetContext.get(), inputProxy, matrix, clip, top, kTop_BoundaryMode, pSrcBounds,
      offsetBounds);
  this->drawRect(
      renderTargetContext.get(), inputProxy, matrix, clip, topRight, kTopRight_BoundaryMode,
      pSrcBounds, offsetBounds);
  this->drawRect(
      renderTargetContext.get(), inputProxy, matrix, clip, left, kLeft_BoundaryMode, pSrcBounds,
      offsetBounds);
  this->drawRect(
      renderTargetContext.get(), inputProxy, matrix, clip, interior, kInterior_BoundaryMode,
      pSrcBounds, offsetBounds);
  this->drawRect(
      renderTargetContext.get(), inputProxy, matrix, clip, right, kRight_BoundaryMode, pSrcBounds,
      offsetBounds);
  this->drawRect(
      renderTargetContext.get(), inputProxy, matrix, clip, bottomLeft, kBottomLeft_BoundaryMode,
      pSrcBounds, offsetBounds);
  this->drawRect(
      renderTargetContext.get(), inputProxy, matrix, clip, bottom, kBottom_BoundaryMode, pSrcBounds,
      offsetBounds);
  this->drawRect(
      renderTargetContext.get(), inputProxy, matrix, clip, bottomRight, kBottomRight_BoundaryMode,
      pSrcBounds, offsetBounds);

  return SkSpecialImage::MakeDeferredFromGpu(
      context, SkIRect::MakeWH(offsetBounds.width(), offsetBounds.height()),
      kNeedNewImageUniqueID_SpecialImage, renderTargetContext->asTextureProxyRef(),
...
#if SK_SUPPORT_GPU
  std::unique_ptr<GrFragmentProcessor> makeFragmentProcessor(
      sk_sp<GrTextureProxy>, const SkMatrix&, const SkIRect* bounds, BoundaryMode) const override;
#endif
...
class GrLightingEffect : public GrFragmentProcessor {
 ...
 protected:
  GrLightingEffect(
      ClassID classID, sk_sp<GrTextureProxy>, sk_sp<const SkImageFilterLight> light,
      SkScalar surfaceScale, const SkMatrix& matrix, BoundaryMode boundaryMode,
      const SkIRect* srcBounds);

  GrLightingEffect(const GrLightingEffect& that) noexcept;
...
class GrDiffuseLightingEffect : public GrLightingEffect {
 public:
  static std::unique_ptr<GrFragmentProcessor> Make(
      sk_sp<GrTextureProxy> proxy, sk_sp<const SkImageFilterLight> light, SkScalar surfaceScale,
    ...
    return std::unique_ptr<GrFragmentProcessor>(new GrDiffuseLightingEffect(
        std::move(proxy), std::move(light), surfaceScale, matrix, kd, boundaryMode, srcBounds));
  }
 ...
 private:
  ...
  GrDiffuseLightingEffect(
      sk_sp<GrTextureProxy>, sk_sp<const SkImageFilterLight> light, SkScalar surfaceScale,
      const SkMatrix& matrix, SkScalar kd, BoundaryMode boundaryMode, const SkIRect* srcBounds);

  explicit GrDiffuseLightingEffect(const GrDiffuseLightingEffect& that) noexcept;
...
class GrSpecularLightingEffect : public GrLightingEffect {
 public:
  static std::unique_ptr<GrFragmentProcessor> Make(
    ...
    return std::unique_ptr<GrFragmentProcessor>(new GrSpecularLightingEffect(
        std::move(proxy), std::move(light), surfaceScale, matrix, ks, shininess, boundaryMode,
        srcBounds));
  }
 ...
 private:
  ...
  explicit GrSpecularLightingEffect(const GrSpecularLightingEffect&) noexcept;
...
class GrGLLight {
 public:
   ...
   */
  virtual void emitSurfaceToLight(
      GrGLSLUniformHandler*, GrGLSLFPFragmentBuilder*, const char* z) = 0;
  virtual void emitLightColor(
      GrGLSLUniformHandler*, GrGLSLFPFragmentBuilder*, const char* surfaceToLight);
...
class GrGLSpotLight : public GrGLLight {
 public:
  ...
  void emitSurfaceToLight(GrGLSLUniformHandler*, GrGLSLFPFragmentBuilder*, const char* z) override;
  void emitLightColor(
      GrGLSLUniformHandler*, GrGLSLFPFragmentBuilder*, const char* surfaceToLight) override;
...
class SkDistantLight : public SkImageFilterLight {
 ...
 protected:
  ...
  void onFlattenLight(SkWriteBuffer& buffer) const noexcept override {
...
class SkPointLight : public SkImageFilterLight {
 public:
  ...
  SkImageFilterLight* transform(const SkMatrix& matrix) const override {
    ...
    SkPoint3 location =
        SkPoint3::Make(location2.fX, location2.fY, SkScalarAve(locationZ.fX, locationZ.fY));
...
class SkSpotLight : public SkImageFilterLight {
 public:
  SkSpotLight(
      ...
      : INHERITED(color),
        ...
        fTarget(target),
        fSpecularExponent(
            SkScalarPin(specularExponent, kSpecularExponentMin, kSpecularExponentMax)) {
    fS = target - location;
    fast_normalize(&fS);
  ...
  SkImageFilterLight* transform(const SkMatrix& matrix) const override {
    ...
    SkPoint3 target = SkPoint3::Make(target2.fX, target2.fY, SkScalarAve(targetZ.fX, targetZ.fY));
    SkPoint3 s = target - location;
    fast_normalize(&s);
    return new SkSpotLight(
        location, target, fSpecularExponent, fCosOuterConeAngle, fCosInnerConeAngle, fConeScale, s,
        color());
  }
  ...
  SkSpotLight(SkReadBuffer& buffer) : INHERITED(buffer) {
    ...
    buffer.validate(
        SkScalarIsFinite(fSpecularExponent) && SkScalarIsFinite(fCosOuterConeAngle) &&
        SkScalarIsFinite(fCosInnerConeAngle) && SkScalarIsFinite(fConeScale));
  }
 ...
 protected:
  ...
  bool isEqual(const SkImageFilterLight& other) const noexcept override {
    ...
    return INHERITED::isEqual(other) && fLocation == o.fLocation && fTarget == o.fTarget &&
           fSpecularExponent == o.fSpecularExponent && fCosOuterConeAngle == o.fCosOuterConeAngle;
  }
...
/*static*/ SkImageFilterLight* SkImageFilterLight::UnflattenLight(SkReadBuffer& buffer) {
  ...
  switch (type) {
    ...
    case SkImageFilterLight::kDistant_LightType: return new SkDistantLight(buffer);
    case SkImageFilterLight::kPoint_LightType: return new SkPointLight(buffer);
    case SkImageFilterLight::kSpot_LightType: return new SkSpotLight(buffer);
    default:
...
SkLightingImageFilter::SkLightingImageFilter(
    sk_sp<SkImageFilterLight> light, SkScalar surfaceScale, sk_sp<SkImageFilter> input,
    const CropRect* cropRect)
    : INHERITED(&input, 1, cropRect), fLight(std::move(light)), fSurfaceScale(surfaceScale / 255) {}
...
sk_sp<SkImageFilter> SkLightingImageFilter::MakeDistantLitSpecular(
    ...
    SkScalar shine, sk_sp<SkImageFilter> input, const CropRect* cropRect) {
  sk_sp<SkImageFilterLight> light(new SkDistantLight(direction, lightColor));
  return SkSpecularLightingImageFilter::Make(
      std::move(light), surfaceScale, ks, shine, std::move(input), cropRect);
}
...
sk_sp<SkImageFilter> SkLightingImageFilter::MakeSpotLitSpecular(
    const SkPoint3& location, const SkPoint3& target, SkScalar specularExponent,
    SkScalar cutoffAngle, SkColor lightColor, SkScalar surfaceScale, SkScalar ks, SkScalar shine,
    sk_sp<SkImageFilter> input, const CropRect* cropRect) {
  sk_sp<SkImageFilterLight> light(
      new SkSpotLight(location, target, specularExponent, cutoffAngle, lightColor));
  return SkSpecularLightingImageFilter::Make(
...
sk_sp<const SkImageFilterLight> SkLightingImageFilter::refLight() const noexcept { return fLight; }
...
sk_sp<SkImageFilter> SkDiffuseLightingImageFilter::Make(
  ...
  return sk_sp<SkImageFilter>(new SkDiffuseLightingImageFilter(
      std::move(light), surfaceScale, kd, std::move(input), cropRect));
}
...
sk_sp<SkSpecialImage> SkDiffuseLightingImageFilter::onFilterImage(
...
std::unique_ptr<GrFragmentProcessor> SkDiffuseLightingImageFilter::makeFragmentProcessor(
    sk_sp<GrTextureProxy> proxy, const SkMatrix& matrix, const SkIRect* srcBounds,
    BoundaryMode boundaryMode) const {
  SkScalar scale = this->surfaceScale() * 255;
  return GrDiffuseLightingEffect::Make(
      std::move(proxy), this->refLight(), scale, matrix, this->kd(), boundaryMode, srcBounds);
}
#endif
...
sk_sp<SkImageFilter> SkSpecularLightingImageFilter::Make(
  ...
  return sk_sp<SkImageFilter>(new SkSpecularLightingImageFilter(
...
sk_sp<SkSpecialImage> SkSpecularLightingImageFilter::onFilterImage(
    SkSpecialImage* source, const Context& ctx, SkIPoint* offset) const {
  SkIPoint inputOffset = SkIPoint::Make(0, 0);
  sk_sp<SkSpecialImage> input(this->filterInput(0, source, ctx, &inputOffset));
  if (!input) {
...
std::unique_ptr<GrFragmentProcessor> SkSpecularLightingImageFilter::makeFragmentProcessor(
  ...
  return GrSpecularLightingEffect::Make(
      std::move(proxy), this->refLight(), scale, matrix, this->ks(), this->shininess(),
...
static SkString emitNormalFunc(
    BoundaryMode mode, const char* pointToNormalName, const char* sobelFuncName) {
  SkString result;
  switch (mode) {
    case kTopLeft_BoundaryMode:
      result.printf(
          "\treturn %s(%s(0.0, 0.0, m[4], m[5], m[7], m[8], %g),\n"
          "\t          %s(0.0, 0.0, m[4], m[7], m[5], m[8], %g),\n"
          "\t          surfaceScale);\n",
          pointToNormalName, sobelFuncName, gTwoThirds, sobelFuncName, gTwoThirds);
      break;
    case kTop_BoundaryMode:
      result.printf(
          "\treturn %s(%s(0.0, 0.0, m[3], m[5], m[6], m[8], %g),\n"
          "\t          %s(0.0, 0.0, m[4], m[7], m[5], m[8], %g),\n"
          "\t          surfaceScale);\n",
          pointToNormalName, sobelFuncName, gOneThird, sobelFuncName, gOneHalf);
      break;
    case kTopRight_BoundaryMode:
      result.printf(
          "\treturn %s(%s( 0.0,  0.0, m[3], m[4], m[6], m[7], %g),\n"
    ...
    case kLeft_BoundaryMode:
      result.printf(
          "\treturn %s(%s(m[1], m[2], m[4], m[5], m[7], m[8], %g),\n"
          "\t          %s( 0.0,  0.0, m[1], m[7], m[2], m[8], %g),\n"
          "\t          surfaceScale);\n",
          pointToNormalName, sobelFuncName, gOneHalf, sobelFuncName, gOneThird);
      break;
    case kInterior_BoundaryMode:
      result.printf(
          "\treturn %s(%s(m[0], m[2], m[3], m[5], m[6], m[8], %g),\n"
          "\t          %s(m[0], m[6], m[1], m[7], m[2], m[8], %g),\n"
          "\t          surfaceScale);\n",
          pointToNormalName, sobelFuncName, gOneQuarter, sobelFuncName, gOneQuarter);
      break;
    case kRight_BoundaryMode:
      result.printf(
          "\treturn %s(%s(m[0], m[1], m[3], m[4], m[6], m[7], %g),\n"
    ...
    case kBottomLeft_BoundaryMode:
      result.printf(
          "\treturn %s(%s(m[1], m[2], m[4], m[5],  0.0,  0.0, %g),\n"
    ...
    case kBottom_BoundaryMode:
      result.printf(
          "\treturn %s(%s(m[0], m[2], m[3], m[5],  0.0,  0.0, %g),\n"
    ...
    case kBottomRight_BoundaryMode:
      result.printf(
          "\treturn %s(%s(m[0], m[1], m[3], m[4],  0.0,  0.0, %g),\n"
...
class GrGLLightingEffect : public GrGLSLFragmentProcessor {
 ...
 protected:
  ...
  virtual void emitLightFunc(
      GrGLSLUniformHandler*, GrGLSLFPFragmentBuilder*, SkString* funcName) = 0;
...
static GrTextureDomain create_domain(
    GrTextureProxy* proxy, const SkIRect* srcBounds, GrTextureDomain::Mode mode) noexcept {
  if (srcBounds) {
...
GrLightingEffect::GrLightingEffect(
    ClassID classID, sk_sp<GrTextureProxy> proxy, sk_sp<const SkImageFilterLight> light,
    SkScalar surfaceScale, const SkMatrix& matrix, BoundaryMode boundaryMode,
    const SkIRect* srcBounds)
...
GrLightingEffect::GrLightingEffect(const GrLightingEffect& that) noexcept
...
GrDiffuseLightingEffect::GrDiffuseLightingEffect(
    ...
    const SkMatrix& matrix, SkScalar kd, BoundaryMode boundaryMode, const SkIRect* srcBounds)
    : INHERITED(
          kGrDiffuseLightingEffect_ClassID, std::move(proxy), std::move(light), surfaceScale,
          matrix, boundaryMode, srcBounds),
      fKD(kd) {}

GrDiffuseLightingEffect::GrDiffuseLightingEffect(const GrDiffuseLightingEffect& that) noexcept
...
static SkPoint3 random_point3(SkRandom* random) {
  return SkPoint3::Make(
      SkScalarToFloat(random->nextSScalar1()), SkScalarToFloat(random->nextSScalar1()),
...
static SkImageFilterLight* create_random_light(SkRandom* random) {
  ...
  switch (type) {
    ...
    case 2: {
      return new SkSpotLight(
          random_point3(random), random_point3(random), random->nextUScalar1(),
          random->nextUScalar1(), random->nextU());
    }
    default: SK_ABORT("Unexpected value."); return nullptr;
  }
}
...
std::unique_ptr<GrFragmentProcessor> GrDiffuseLightingEffect::TestCreate(GrProcessorTestData* d) {
  ...
  SkIRect srcBounds = SkIRect::MakeXYWH(
...
void GrGLLightingEffect::emitCode(EmitArgs& args) {
  ...
  fSurfaceScaleUni =
      uniformHandler->addUniform(kFragment_GrShaderFlag, kHalf_GrSLType, "SurfaceScale");
  ...
  const GrShaderVar gSobelArgs[] = {
      ...
      GrShaderVar("c", kHalf_GrSLType),     GrShaderVar("d", kHalf_GrSLType),
  ...
  for (int dy = 1; dy >= -1; dy--) {
    for (int dx = -1; dx <= 1; dx++) {
      ...
      fDomain.sampleTexture(
          fragBuilder, args.fUniformHandler, args.fShaderCaps, le.domain(), temp.c_str(), texCoords,
          args.fTexSamplers[0]);
  ...
  fragBuilder->codeAppendf(
      "\t\t%s = %s(%s(m, %s), surfaceToLight, ", args.fOutputColor, lightFunc.c_str(),
...
void GrGLDiffuseLightingEffect::emitLightFunc(
    GrGLSLUniformHandler* uniformHandler, GrGLSLFPFragmentBuilder* fragBuilder,
    SkString* funcName) {
  const char* kd;
...
GrSpecularLightingEffect::GrSpecularLightingEffect(
    ...
    const SkIRect* srcBounds)
    : INHERITED(
          kGrSpecularLightingEffect_ClassID, std::move(proxy), std::move(light), surfaceScale,
          matrix, boundaryMode, srcBounds),
      fKS(ks),
      fShininess(shininess) {}

GrSpecularLightingEffect::GrSpecularLightingEffect(const GrSpecularLightingEffect& that) noexcept
...
void GrGLDistantLight::emitSurfaceToLight(
    GrGLSLUniformHandler* uniformHandler, GrGLSLFPFragmentBuilder* fragBuilder, const char* z) {
  const char* dir;
  fDirectionUni =
      uniformHandler->addUniform(kFragment_GrShaderFlag, kHalf3_GrSLType, "LightDirection", &dir);
...
void GrGLPointLight::emitSurfaceToLight(
  ...
  const char* loc;
  fLocationUni =
      uniformHandler->addUniform(kFragment_GrShaderFlag, kHalf3_GrSLType, "LightLocation", &loc);
  fragBuilder->codeAppendf("normalize(%s - half3(sk_FragCoord.xy, %s))", loc, z);
}
...
void GrGLSpotLight::emitSurfaceToLight(
    GrGLSLUniformHandler* uniformHandler, GrGLSLFPFragmentBuilder* fragBuilder, const char* z) {
  const char* location;
  fLocationUni = uniformHandler->addUniform(
      kFragment_GrShaderFlag, kHalf3_GrSLType, "LightLocation", &location);
...
void GrGLSpotLight::emitLightColor(
    ...
    const char* surfaceToLight) {
  const char* color =
      uniformHandler->getUniformCStr(this->lightColorUni());  // created by parent class.
  ...
  const char* s;
  fExponentUni =
      uniformHandler->addUniform(kFragment_GrShaderFlag, kHalf_GrSLType, "Exponent", &exponent);
  fCosInnerConeAngleUni = uniformHandler->addUniform(
      kFragment_GrShaderFlag, kHalf_GrSLType, "CosInnerConeAngle", &cosInner);
  fCosOuterConeAngleUni = uniformHandler->addUniform(
      kFragment_GrShaderFlag, kHalf_GrSLType, "CosOuterConeAngle", &cosOuter);
  fConeScaleUni =
      uniformHandler->addUniform(kFragment_GrShaderFlag, kHalf_GrSLType, "ConeScale", &coneScale);
  fSUni = uniformHandler->addUniform(kFragment_GrShaderFlag, kHalf3_GrSLType, "S", &s);

  const GrShaderVar gLightColorArgs[] = {GrShaderVar("surfaceToLight", kHalf3_GrSLType)};
