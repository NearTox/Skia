...
class SkPaintImageFilterImpl final : public SkImageFilter_Base {
 public:
  SkPaintImageFilterImpl(const SkPaint& paint, const CropRect* rect)
      : INHERITED(nullptr, 0, rect), fPaint(paint) {}
...
sk_sp<SkImageFilter> SkPaintImageFilter::Make(
    const SkPaint& paint, const SkImageFilter::CropRect* cropRect) {
...
sk_sp<SkSpecialImage> SkPaintImageFilterImpl::onFilterImage(
    const Context& ctx, SkIPoint* offset) const {
  SkIRect bounds;
  const SkIRect srcBounds =
      SkIRect::MakeWH(ctx.sourceImage()->width(), ctx.sourceImage()->height());
...
bool SkPaintImageFilterImpl::affectsTransparentBlack() const { return true; }
