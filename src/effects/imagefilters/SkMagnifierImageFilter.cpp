...
SkMagnifierImageFilter::SkMagnifierImageFilter(
    const SkRect& srcRect, SkScalar inset, sk_sp<SkImageFilter> input, const CropRect* cropRect)
...
sk_sp<SkSpecialImage> SkMagnifierImageFilter::onFilterImage(
  ...
  if (source->isTextureBacked()) {
    ...
    auto fp = GrMagnifierEffect::Make(
        std::move(inputProxy), bounds, fSrcRect, invXZoom, invYZoom, bounds.width() * invInset,
        bounds.height() * invInset);
    fp = GrColorSpaceXformEffect::Make(
        std::move(fp), input->getColorSpace(), input->alphaType(),
