...
sk_sp<SkImageFilter> SkMagnifierImageFilter::Make(
  ...
  return sk_sp<SkImageFilter>(
      new SkMagnifierImageFilterImpl(srcRect, inset, std::move(input), cropRect));
}
...
sk_sp<SkFlattenable> SkMagnifierImageFilterImpl::CreateProc(SkReadBuffer& buffer) {
  ...
  return SkMagnifierImageFilter::Make(
      src, buffer.readScalar(), common.getInput(0), &common.cropRect());
}
...
sk_sp<SkSpecialImage> SkMagnifierImageFilterImpl::onFilterImage(
  ...
  if (ctx.gpuBacked()) {
    ...
    auto fp = GrMagnifierEffect::Make(
        std::move(inputView), bounds, srcRect, invXZoom, invYZoom, bounds.width() * invInset,
