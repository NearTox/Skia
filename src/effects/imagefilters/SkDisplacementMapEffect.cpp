...
sk_sp<SkSpecialImage> SkDisplacementMapEffectImpl::onFilterImage(
  ...
  if (ctx.gpuBacked()) {
    ...
    surfaceFillContext->fillRectToRectWithFP(
        colorBounds, SkIRect::MakeSize(colorBounds.size()), std::move(fp));
...
void GrDisplacementMapEffect::Impl::emitCode(EmitArgs& args) {
  ...
  fragBuilder->codeAppendf(
      "float2 cCoords = %s + %s * (dColor.%c%c - half2(0.5));", args.fSampleCoord, scaleUni,
