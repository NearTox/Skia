...
bool channel_selector_type_is_valid(SkDisplacementMapEffect::ChannelSelectorType cst) {
  switch (cst) {
    ...
    case SkDisplacementMapEffect::kA_ChannelSelectorType: return true;
    default: break;
  }
  return false;
}
...
sk_sp<SkImageFilter> SkDisplacementMapEffect::Make(
  ...
  return sk_sp<SkImageFilter>(
      new SkDisplacementMapEffect(xChannelSelector, yChannelSelector, scale, inputs, cropRect));
}
...
class GrDisplacementMapEffect : public GrFragmentProcessor {
 ...
 private:
  ...
  GrDisplacementMapEffect(
      SkDisplacementMapEffect::ChannelSelectorType xChannelSelector,
      SkDisplacementMapEffect::ChannelSelectorType yChannelSelector, const SkVector& scale,
      sk_sp<GrTextureProxy> displacement, const SkIRect& displSubset, const SkMatrix& offsetMatrix,
      sk_sp<GrTextureProxy> color, const SkIRect& colorSubset);
...
sk_sp<SkSpecialImage> SkDisplacementMapEffect::onFilterImage(
  ...
  Context displContext(
      ctx.ctm(), ctx.clipBounds(), ctx.cache(), OutputProperties(kN32_SkColorType, nullptr));
  ...
  if (source->isTextureBacked()) {
    ...
    fp = GrColorSpaceXformEffect::Make(
        std::move(fp), color->getColorSpace(), color->alphaType(), colorSpace);
...
SkIRect SkDisplacementMapEffect::onFilterNodeBounds(
...
SkIRect SkDisplacementMapEffect::onFilterBounds(
...
void GrGLDisplacementMapEffect::emitCode(EmitArgs& args) {
  ...
  fragBuilder->appendTextureLookup(
      args.fTexSamplers[0], args.fTransformedCoords[0].c_str(),
  ...
  switch (displacementMap.xChannelSelector()) {
    case SkDisplacementMapEffect::kR_ChannelSelectorType: fragBuilder->codeAppend("r"); break;
    case SkDisplacementMapEffect::kG_ChannelSelectorType: fragBuilder->codeAppend("g"); break;
    case SkDisplacementMapEffect::kB_ChannelSelectorType: fragBuilder->codeAppend("b"); break;
    case SkDisplacementMapEffect::kA_ChannelSelectorType: fragBuilder->codeAppend("a"); break;
    case SkDisplacementMapEffect::kUnknown_ChannelSelectorType:
