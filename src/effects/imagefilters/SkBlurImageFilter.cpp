...
static SkTileMode to_sktilemode(SkBlurImageFilter::TileMode tileMode) {
  switch (tileMode) {
    ...
    case SkBlurImageFilter::kClampToBlack_TileMode:
      // Fall through
    default: return SkTileMode::kDecal;
  }
}

sk_sp<SkImageFilter> SkBlurImageFilter::Make(
    SkScalar sigmaX, SkScalar sigmaY, sk_sp<SkImageFilter> input,
    const SkImageFilter::CropRect* cropRect, TileMode tileMode) {
...
sk_sp<SkImageFilter> SkBlurImageFilter::Make(
  ...
  return sk_sp<SkImageFilter>(new SkBlurImageFilterImpl(sigmaX, sigmaY, tileMode, input, cropRect));
}
...
sk_sp<SkFlattenable> SkBlurImageFilterImpl::CreateProc(SkReadBuffer& buffer) {
  ...
  return SkBlurImageFilter::Make(sigmaX, sigmaY, tileMode, common.getInput(0), &common.cropRect());
}
...
void SkBlurImageFilterImpl::flatten(SkWriteBuffer& buffer) const {
  ...
  static_assert(
      (int)SkTileMode::kLastTileMode == 3 && SkBlurImageFilter::kLast_TileMode == 2,
...
static void blur_one_direction(
    Sk4u* buffer, int window, int srcLeft, int srcRight, int dstRight, const uint32_t* src,
    int srcXStride, int srcYStride, int srcH, uint32_t* dst, int dstXStride, int dstYStride) {
  // The circular buffers are one less than the window.
  auto pass0Count = window - 1, pass1Count = window - 1,
  ...
  auto srcStart = srcLeft - border, srcEnd = srcRight - border, dstEnd = dstRight;
  ...
  for (auto y = 0; y < srcH; y++) {
    ...
    auto processValue = [&](const Sk4u& leadingEdge) -> Sk4u {
      ...
      *buffer2Cursor = sum1;
      ...
      buffer01Cursor = (buffer01Cursor + 1) < buffer01End ? buffer01Cursor + 1 : buffer01Start;
...
static sk_sp<SkSpecialImage> copy_image_with_bounds(
    const SkImageFilter_Base::Context& ctx, const sk_sp<SkSpecialImage>& input, SkIRect srcBounds,
    SkIRect dstBounds) {
  SkBitmap inputBM;
  ...
  auto srcW = srcBounds.width(), dstW = dstBounds.width(), dstH = dstBounds.height();
  ...
  for (; y < srcBounds.bottom(); y++) {
    ...
    memcpy(
        dstPtr, src.getAddr32(x - srcBounds.left(), y - srcBounds.top()), srcW * sizeof(uint32_t));
...
static sk_sp<SkSpecialImage> cpu_blur(
    const SkImageFilter_Base::Context& ctx, SkVector sigma, const sk_sp<SkSpecialImage>& input,
    SkIRect srcBounds, SkIRect dstBounds) {
  auto windowW = calculate_window(sigma.x()), windowH = calculate_window(sigma.y());
  ...
  auto srcW = srcBounds.width(), srcH = srcBounds.height(), dstW = dstBounds.width(),
  ...
  auto bufferSizeW = calculate_buffer(windowW), bufferSizeH = calculate_buffer(windowH);
  ...
  if (windowW > 1) {
    ...
    intermediateSrc =
        static_cast<uint32_t*>(dst.getPixels()) + (shift > 0 ? shift * dst.rowBytesAsPixels() : 0);
    ...
    blur_one_direction(
        buffer, windowW, srcBounds.left(), srcBounds.right(), dstBounds.right(),
        static_cast<uint32_t*>(src.getPixels()), 1, src.rowBytesAsPixels(), srcH, intermediateSrc,
        1, intermediateRowBytesAsPixels);
  }

  if (windowH > 1) {
    blur_one_direction(
        buffer, windowH, srcBounds.top(), srcBounds.bottom(), dstBounds.bottom(), intermediateSrc,
        intermediateRowBytesAsPixels, 1, intermediateWidth, intermediateDst, dst.rowBytesAsPixels(),
        1);
  }
...
sk_sp<SkSpecialImage> SkBlurImageFilterImpl::onFilterImage(
  ...
  SkIRect inputBounds =
      SkIRect::MakeXYWH(inputOffset.fX, inputOffset.fY, input->width(), input->height());
  ...
  if (ctx.gpuBacked()) {
    ...
    result = this->gpuFilter(ctx, sigma, input, inputBounds, dstBounds, inputOffset, &resultOffset);
  } else
#endif
  {
    ...
    // additional + 1 added to window represents adding one more leading element before subtracting
    // the trailing element. Explanation of maximums:
    //   sum0 = (window + 1) * 255
    //   sum1 = (window + 1) * sum0 -> (window + 1) * (window + 1) * 255
    //   sum2 = (window + 1) * sum1 -> (window + 1) * (window + 1) * (window + 1) * 255 -> window^3
    //   * 255
    //
...
sk_sp<SkSpecialImage> SkBlurImageFilterImpl::gpuFilter(
  ...
  auto renderTargetContext = SkGpuBlurUtils::GaussianBlur(
      context, std::move(inputTexture), SkColorTypeToGrColorType(input->colorType()),
      input->alphaType(), input->subset().topLeft(),
      ctx.colorSpace() ? sk_ref_sp(input->getColorSpace()) : nullptr, dstBounds, inputBounds,
      sigma.x(), sigma.y(), to_texture_domain_mode(fTileMode));
  if (!renderTargetContext) {
    return nullptr;
  }

  return SkSpecialImage::MakeDeferredFromGpu(
      context, SkIRect::MakeWH(dstBounds.width(), dstBounds.height()),
      kNeedNewImageUniqueID_SpecialImage, renderTargetContext->asTextureProxyRef(),
      renderTargetContext->colorInfo().colorType(), sk_ref_sp(input->getColorSpace()),
      ctx.surfaceProps());
}
#endif
...
SkIRect SkBlurImageFilterImpl::onFilterNodeBounds(
