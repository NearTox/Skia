...
sk_sp<SkImageFilter> SkImageFilters::Blur(
  ...
  return sk_sp<SkImageFilter>(new SkBlurImageFilter(sigmaX, sigmaY, tileMode, input, cropRect));
}
...
sk_sp<SkFlattenable> SkBlurImageFilter::CreateProc(SkReadBuffer& buffer) {
  ...
  return SkImageFilters::Blur(sigmaX, sigmaY, tileMode, common.getInput(0), common.cropRect());
}
...
class Pass {
 public:
  ...
  void blur(
      int srcLeft, int srcRight, int dstRight, const uint32_t* src, int srcStride, uint32_t* dst,
      int dstStride) {
    this->startBlur();

    auto srcStart = srcLeft - fBorder, srcEnd = srcRight - fBorder, dstEnd = dstRight,
         srcIdx = srcStart, dstIdx = 0;
...
class GaussPass final : public Pass {
 public:
  ...
  GaussPass(
      skvx::Vec<4, uint32_t>* buffer0, skvx::Vec<4, uint32_t>* buffer1,
      skvx::Vec<4, uint32_t>* buffer2, skvx::Vec<4, uint32_t>* buffersEnd, int border, int divisor)
      : Pass{border},
        fBuffer0{buffer0},
        fBuffer1{buffer1},
        fBuffer2{buffer2},
...
class TentPass final : public Pass {
 public:
  ...
  TentPass(
      skvx::Vec<4, uint32_t>* buffer0, skvx::Vec<4, uint32_t>* buffer1,
      skvx::Vec<4, uint32_t>* buffersEnd, int border, int divisor)
      : Pass{border},
        fBuffer0{buffer0},
        fBuffer1{buffer1},
        fBuffersEnd{buffersEnd},
...
sk_sp<SkSpecialImage> cpu_blur(
  ...
  if (makerX->window() > 1) {
    ...
    for (auto y = 0; y < srcH; y++) {
      pass->blur(
          srcBounds.left(), srcBounds.right(), dstBounds.right(), srcCursor, 1, dstCursor, 1);
  ...
  if (makerY->window() > 1) {
    ...
    for (auto x = 0; x < intermediateWidth; x++) {
      pass->blur(
          srcBounds.top(), srcBounds.bottom(), dstBounds.bottom(), srcCursor,
          intermediateRowBytesAsPixels, dstCursor, dst.rowBytesAsPixels());
      srcCursor += 1;
      dstCursor += 1;
    }
  }
...
sk_sp<SkSpecialImage> SkBlurImageFilter::onFilterImage(const Context& ctx, SkIPoint* offset) const {
  ...
  SkASSERT(
      SkScalarIsFinite(sigma.x()) && sigma.x() >= 0.f && sigma.x() <= kMaxSigma &&
...
#  if SK_GPU_V1
  ...
  return SkSpecialImage::MakeDeferredFromGpu(
      ...
      sdc->readSurfaceView(), sdc->colorInfo().colorType(), sk_ref_sp(input->getColorSpace()),
      ctx.surfaceProps());
#  else   // SK_GPU_V1
  return nullptr;
#  endif  // SK_GPU_V1
...
SkIRect SkBlurImageFilter::onFilterNodeBounds(
