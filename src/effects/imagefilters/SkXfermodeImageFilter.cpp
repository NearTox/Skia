...
class SkXfermodeImageFilter_Base : public SkImageFilter {
 public:
  SkXfermodeImageFilter_Base(
      SkBlendMode mode, sk_sp<SkImageFilter> inputs[2], const CropRect* cropRect);
...
#if SK_SUPPORT_GPU
  std::unique_ptr<GrFragmentProcessor> makeFGFrag(std::unique_ptr<GrFragmentProcessor> bgFP) const;
#endif
...
sk_sp<SkImageFilter> SkXfermodeImageFilter::Make(
...
SkXfermodeImageFilter_Base::SkXfermodeImageFilter_Base(
    SkBlendMode mode, sk_sp<SkImageFilter> inputs[2], const CropRect* cropRect)
    : INHERITED(inputs, 2, cropRect), fMode(mode) {}
...
sk_sp<SkFlattenable> SkXfermodeImageFilter_Base::CreateProc(SkReadBuffer& buffer) {
  ...
  return SkXfermodeImageFilter::Make(
      (SkBlendMode)mode, common.getInput(0), common.getInput(1), &common.cropRect());
}
...
sk_sp<SkSpecialImage> SkXfermodeImageFilter_Base::onFilterImage(
  ...
  if (source->isTextureBacked()) {
    return this->filterImageGPU(
        source, background, backgroundOffset, foreground, foregroundOffset, bounds,
...
SkIRect SkXfermodeImageFilter_Base::onFilterBounds(
...
void SkXfermodeImageFilter_Base::drawForeground(
...
sk_sp<SkSpecialImage> SkXfermodeImageFilter_Base::filterImageGPU(
  ...
  if (backgroundProxy) {
    ...
    bgFP = GrColorSpaceXformEffect::Make(
        std::move(bgFP), background->getColorSpace(), background->alphaType(),
  ...
  if (foregroundProxy) {
    ...
    foregroundFP = GrColorSpaceXformEffect::Make(
        std::move(foregroundFP), foreground->getColorSpace(), foreground->alphaType(),
        outputProperties.colorSpace());
