...
#if SK_SUPPORT_GPU
  sk_sp<GrTextureProxy> createMaskTexture(
      GrRecordingContext*, const SkMatrix&, const SkIRect& bounds) const;
#endif
...
static SkScalar pin_0_1(SkScalar x) { return SkMinScalar(SkMaxScalar(x, 0), 1); }

sk_sp<SkImageFilter> SkAlphaThresholdFilter::Make(
    const SkRegion& region, SkScalar innerThreshold, SkScalar outerThreshold,
...
sk_sp<SkFlattenable> SkAlphaThresholdFilterImpl::CreateProc(SkReadBuffer& buffer) {
  ...
  return SkAlphaThresholdFilter::Make(rgn, inner, outer, common.getInput(0), &common.cropRect());
}

SkAlphaThresholdFilterImpl::SkAlphaThresholdFilterImpl(
    const SkRegion& region, SkScalar innerThreshold, SkScalar outerThreshold,
...
sk_sp<SkSpecialImage> SkAlphaThresholdFilterImpl::onFilterImage(
  ...
  if (source->isTextureBacked()) {
    ...
    auto textureFP = GrSimpleTextureEffect::Make(
        std::move(inputProxy), SkMatrix::MakeTrans(input->subset().x(), input->subset().y()));
    textureFP = GrColorSpaceXformEffect::Make(
        std::move(textureFP), input->getColorSpace(), input->alphaType(), outProps.colorSpace());
    if (!textureFP) {
      return nullptr;
    }

    auto thresholdFP = GrAlphaThresholdFragmentProcessor::Make(
        std::move(maskProxy), fInnerThreshold, fOuterThreshold, bounds);
    if (!thresholdFP) {
      return nullptr;
    }
    ...
    return DrawWithFP(
        context, std::move(fp), bounds, outProps,
