...
enum PictureResolution { kDeviceSpace_PictureResolution, kLocalSpace_PictureResolution };
static sk_sp<SkImageFilter> make_localspace_filter(
    sk_sp<SkPicture> pic, const SkRect& cropRect, SkFilterQuality fq) {
  SkISize dim = {SkScalarRoundToInt(cropRect.width()), SkScalarRoundToInt(cropRect.height())};
  auto img = SkImage::MakeFromPicture(
      std::move(pic), dim, nullptr, nullptr, SkImage::BitDepth::kU8, SkColorSpace::MakeSRGB());
...
sk_sp<SkFlattenable> SkPictureImageFilter::CreateProc(SkReadBuffer& buffer) {
  ...
  if (buffer.isVersionLT(SkReadBuffer::kRemovePictureImageFilterLocalSpace)) {
    ...
    if (kLocalSpace_PictureResolution == pictureResolution) {
      return make_localspace_filter(std::move(picture), cropRect, buffer.checkFilterQuality());
    }
  }
...
sk_sp<SkSpecialImage> SkPictureImageFilter::onFilterImage(
  ...
  sk_sp<SkSpecialSurface> surf(
      source->makeSurface(ctx.outputProperties(), bounds.size(), kPremul_SkAlphaType, &props));
  if (!surf) {
    return nullptr;
  }
