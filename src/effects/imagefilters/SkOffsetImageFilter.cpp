...
sk_sp<SkImageFilter> SkOffsetImageFilter::Make(
    SkScalar dx, SkScalar dy, sk_sp<SkImageFilter> input, const SkImageFilter::CropRect* cropRect) {
...
sk_sp<SkFlattenable> SkOffsetImageFilterImpl::CreateProc(SkReadBuffer& buffer) {
  ...
  return SkOffsetImageFilter::Make(offset.x(), offset.y(), common.getInput(0), &common.cropRect());
}
...
sk_sp<SkSpecialImage> SkOffsetImageFilterImpl::onFilterImage(
  ...
  if (!this->cropRectIsSet()) {
    ...
    const SkIRect srcBounds =
        SkIRect::MakeXYWH(srcOffset.fX, srcOffset.fY, input->width(), input->height());
