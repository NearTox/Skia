...
sk_sp<SkImageFilter> SkImageFilters::DisplacementMap(
  ...
  return sk_sp<SkImageFilter>(new SkDisplacementMapImageFilter(
      xChannelSelector, yChannelSelector, scale, inputs, cropRect));
}
...
void SkRegisterDisplacementMapImageFilterFlattenable() {
  ...
  SkFlattenable::Register("SkDisplacementMapEffectImpl", SkDisplacementMapImageFilter::CreateProc);
}
...
sk_sp<SkSpecialImage> SkDisplacementMapImageFilter::onFilterImage(
  ...
  if (ctx.gpuBacked()) {
    ...
    std::unique_ptr<GrFragmentProcessor> fp = GrDisplacementMapEffect::Make(
        ...
        offsetMatrix, std::move(colorView), color->subset(), *rContext->priv().caps());
    ...
    sfc->fillRectToRectWithFP(colorBounds, SkIRect::MakeSize(colorBounds.size()), std::move(fp));

    offset->fX = bounds.left();
    offset->fY = bounds.top();
    return SkSpecialImage::MakeDeferredFromGpu(
        rContext, SkIRect::MakeWH(bounds.width(), bounds.height()),
...
std::unique_ptr<GrFragmentProcessor::ProgramImpl> GrDisplacementMapEffect::onMakeProgramImpl()
    const {
...
void GrDisplacementMapEffect::Impl::emitCode(EmitArgs& args) {
  ...
  fragBuilder->codeAppendf(
      ...
      chanChar(displacementMap.fXChannelSelector), chanChar(displacementMap.fYChannelSelector));
