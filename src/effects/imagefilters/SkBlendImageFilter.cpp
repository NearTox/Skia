...
class SkBlendImageFilter : public SkImageFilter_Base {
 public:
  SkBlendImageFilter(
      sk_sp<SkBlender> blender, sk_sp<SkImageFilter> inputs[2], const SkRect* cropRect)
      : INHERITED(inputs, 2, cropRect), fBlender(std::move(blender)) {
    SkASSERT(fBlender);
  }
...
sk_sp<SkImageFilter> SkImageFilters::Blend(
    sk_sp<SkBlender> blender, sk_sp<SkImageFilter> background, sk_sp<SkImageFilter> foreground,
    const CropRect& cropRect) {
  if (!blender) {
...
sk_sp<SkFlattenable> SkBlendImageFilter::CreateProc(SkReadBuffer& buffer) {
  ...
  return SkImageFilters::Blend(
      std::move(blender), common.getInput(0), common.getInput(1), common.cropRect());
}
...
sk_sp<SkSpecialImage> SkBlendImageFilter::onFilterImage(
  ...
  if (background) {
    ...
    background->draw(
        canvas, SkIntToScalar(backgroundOffset.fX), SkIntToScalar(backgroundOffset.fY),
...
SkIRect SkBlendImageFilter::onFilterBounds(
  ...
  if (auto bm = as_BB(fBlender)->asBlendMode()) {
    switch (bm.value()) {
      case SkBlendMode::kClear: return SkIRect::MakeEmpty();

      case SkBlendMode::kSrc:
      case SkBlendMode::kDstATop: return getForeground();

      case SkBlendMode::kDst:
      case SkBlendMode::kSrcATop: return getBackground();
...
void SkBlendImageFilter::drawForeground(
    SkCanvas* canvas, SkSpecialImage* img, const SkIRect& fgBounds) const {
  SkPaint paint;
  paint.setBlender(fBlender);
  if (img) {
    img->draw(
        canvas, SkIntToScalar(fgBounds.fLeft), SkIntToScalar(fgBounds.fTop), SkSamplingOptions(),
        &paint);
  }
...
sk_sp<SkSpecialImage> SkBlendImageFilter::filterImageGPU(
    const Context& ctx, sk_sp<SkSpecialImage> background, const SkIPoint& backgroundOffset,
    sk_sp<SkSpecialImage> foreground, const SkIPoint& foregroundOffset,
    const SkIRect& bounds) const {
  ...
  if (backgroundView.asTextureProxy()) {
    ...
    fp = GrTextureEffect::MakeSubset(
        std::move(backgroundView), background->alphaType(), bgMatrix, sampler, bgSubset, caps);
  ...
  if (foregroundView.asTextureProxy()) {
    ...
    auto fgFP = GrTextureEffect::MakeSubset(
        std::move(foregroundView), foreground->alphaType(), fgMatrix, sampler, fgSubset, caps);
