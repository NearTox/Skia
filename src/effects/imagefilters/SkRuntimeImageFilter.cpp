...
class SkRuntimeImageFilter final : public SkImageFilter_Base {
 public:
  SkRuntimeImageFilter(
      sk_sp<SkRuntimeEffect> effect, sk_sp<SkData> uniforms, sk_sp<SkImageFilter> input)
  ...
  SkRuntimeImageFilter(
      ...
      : INHERITED(inputs, inputCount, /*cropRect=*/nullptr), fShaderBuilder(builder) {
...
sk_sp<SkImageFilter> SkMakeRuntimeImageFilter(
    sk_sp<SkRuntimeEffect> effect, sk_sp<SkData> uniforms, sk_sp<SkImageFilter> input) {
...
void SkRegisterRuntimeImageFilterFlattenable() { SK_REGISTER_FLATTENABLE(SkRuntimeImageFilter); }
...
sk_sp<SkSpecialImage> SkRuntimeImageFilter::onFilterImage(
  ...
  for (int i = 0; i < inputCount; i++) {
    ...
    sk_sp<SkShader> inputShader = input->asShader(SkSamplingOptions(SkFilterMode::kLinear), localM);
...
sk_sp<SkImageFilter> SkImageFilters::RuntimeShader(
