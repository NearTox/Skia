...
sk_sp<SkImageFilter> SkImageSource::Make(
    sk_sp<SkImage> image, const SkRect& srcRect, const SkRect& dstRect,
    SkFilterQuality filterQuality) {
  ...
  return sk_sp<SkImageFilter>(
      new SkImageSourceImpl(std::move(image), srcRect, dstRect, filterQuality));
}
...
sk_sp<SkSpecialImage> SkImageSourceImpl::onFilterImage(const Context& ctx, SkIPoint* offset) const {
  SkRect dstRect;
  ...
  if (fSrcRect == bounds) {
    ...
    if (dstRect.width() == bounds.width() && dstRect.height() == bounds.height() &&
      ...
      return SkSpecialImage::MakeFromImage(
          ctx.getContext(), SkIRect::MakeWH(fImage->width(), fImage->height()), fImage,
          ctx.surfaceProps());
    }
  }
  ...
  canvas->drawImageRect(
      fImage.get(), fSrcRect, dstRect, &paint, SkCanvas::kStrict_SrcRectConstraint);
...
SkRect SkImageSourceImpl::computeFastBounds(const SkRect& src) const { return fDstRect; }

SkIRect SkImageSourceImpl::onFilterNodeBounds(
