...
sk_sp<SkImageFilter> SkImageSource::Make(
  ...
  return sk_sp<SkImageFilter>(new SkImageSource(std::move(image), srcRect, dstRect, filterQuality));
}
...
sk_sp<SkSpecialImage> SkImageSource::onFilterImage(
  ...
  if (fSrcRect == bounds) {
    ...
    if (dstRect.width() == bounds.width() && dstRect.height() == bounds.height() &&
      ...
      return SkSpecialImage::MakeFromImage(
          source->getContext(), SkIRect::MakeWH(fImage->width(), fImage->height()), fImage,
          &source->props());
    }
  }
...
SkIRect SkImageSource::onFilterNodeBounds(
