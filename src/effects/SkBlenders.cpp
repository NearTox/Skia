...
sk_sp<SkBlender> SkBlenders::Arithmetic(
    float k1, float k2, float k3, float k4, bool enforcePremul) {
  ...
  for (const auto& t : table) {
    if (SkScalarNearlyEqual(k1, t.k1) && SkScalarNearlyEqual(k2, t.k2) &&
        SkScalarNearlyEqual(k3, t.k3) && SkScalarNearlyEqual(k4, t.k4)) {
  ...
  const float array[] = {
      k1, k2, k3, k4, enforcePremul ? 0.0f : 1.0f,
  };
