...
SkDashImpl::SkDashImpl(const SkScalar intervals[], int count, SkScalar phase)
  ...
  SkDashPath::CalcDashParameters(
      phase, fIntervals, fCount, &fInitialDashLength, &fInitialDashIndex, &fIntervalLength,
      &fPhase);
}
...
static bool cull_line(
    SkPoint* pts, const SkStrokeRec& rec, const SkMatrix& ctm, const SkRect* cullRect,
...
bool SkDashImpl::onAsPoints(
    PointData* results, const SkPath& src, const SkStrokeRec& rec, const SkMatrix& matrix,
  ...
  if (SkScalarNearlyEqual(SK_Scalar1, tangent.fX) || SkScalarNearlyEqual(-SK_Scalar1, tangent.fX)) {
    results->fSize.set(SkScalarHalf(fIntervals[0]), SkScalarHalf(rec.getWidth()));
      ...
      SkScalarNearlyEqual(SK_Scalar1, tangent.fY) || SkScalarNearlyEqual(-SK_Scalar1, tangent.fY)) {
      ...
      if (0 == fInitialDashIndex) {
        if (clampedInitialDashLength > 0) {
          ...
          if (clampedInitialDashLength < fIntervals[0]) {
            ...
            results->fFirst.addRect(x - halfWidth, y - halfHeight, x + halfWidth, y + halfHeight);
          } else {
