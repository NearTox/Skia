...
class SkPath1DPathEffectImpl : public Sk1DPathEffect {
 public:
  SkPath1DPathEffectImpl(
      const SkPath& path, SkScalar advance, SkScalar phase, SkPath1DPathEffect::Style style)
      : fPath(path) {
  ...
  SkScalar begin(SkScalar contourLength) const override { return fInitialOffset; }
...
static bool morphpoints(
    SkPoint dst[], const SkPoint src[], int count, SkPathMeasure& meas, SkScalar dist) {
  ...
  while ((verb = iter.next(srcP)) != SkPath::kDone_Verb) {
    switch (verb) {
      ...
      case SkPath::kLine_Verb:
        srcP[2] = srcP[1];
        srcP[1].set(SkScalarAve(srcP[0].fX, srcP[2].fX), SkScalarAve(srcP[0].fY, srcP[2].fY));
        [[fallthrough]];
      ...
      case SkPath::kCubic_Verb:
        ...
        break;
      case SkPath::kClose_Verb: dst->close(); break;
      default: SkDEBUGFAIL("unknown verb"); break;
    }
  }
}

SkScalar SkPath1DPathEffectImpl::next(SkPath* dst, SkScalar distance, SkPathMeasure& meas) const {
  ...
  switch (fStyle) {
    ...
    case SkPath1DPathEffect::kMorph_Style: morphpath(dst, fPath, meas, distance); break;
  }
  return fAdvance;
}
...
sk_sp<SkPathEffect> SkPath1DPathEffect::Make(
    const SkPath& path, SkScalar advance, SkScalar phase, Style style) {
...
void SkPath1DPathEffect::RegisterFlattenables() { SK_REGISTER_FLATTENABLE(SkPath1DPathEffectImpl); }
