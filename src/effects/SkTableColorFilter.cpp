...
class SkTable_ColorFilter : public SkColorFilter {
 public:
  ...
  bool onAppendStages(const SkStageRec& rec, bool shaderIsOpaque) const override {
    const uint8_t *r = gIdentityTable, *g = gIdentityTable, *b = gIdentityTable,
                  *a = gIdentityTable;
...
static const uint8_t gCountNibBits[] = {0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3, 2, 3, 3, 4};
...
sk_sp<SkFlattenable> SkTable_ColorFilter::CreateProc(SkReadBuffer& buffer) {
  ...
  size_t unpackedSize =
      SkPackBits::Unpack8(packedStorage, packedSize, unpackedStorage, sizeof(unpackedStorage));
...
void GLColorTableEffect::emitCode(EmitArgs& args) {
  ...
  if (nullptr == args.fInputColor) {
    ...
    fragBuilder->codeAppendf(
        "\t\thalf4 coord = half4(%s.rgb / nonZeroAlpha, nonZeroAlpha);\n", args.fInputColor);
    fragBuilder->codeAppendf(
        "\t\tcoord = coord * %f + half4(%f, %f, %f, %f);\n", kColorScaleFactor, kColorOffsetFactor,
        kColorOffsetFactor, kColorOffsetFactor, kColorOffsetFactor);
  }
...
std::unique_ptr<GrFragmentProcessor> ColorTableEffect::Make(
  ...
  sk_sp<GrTextureProxy> proxy =
      GrMakeCachedImageProxy(context->priv().proxyProvider(), std::move(srcImage));
  if (!proxy) {
    return nullptr;
  }
