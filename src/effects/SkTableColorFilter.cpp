...
public:
    SkTable_ColorFilter(const uint8_t tableA[], const uint8_t tableR[], const uint8_t tableG[],
...
void GLColorTableEffect::emitCode(EmitArgs& args) {
    ...
    if (nullptr == args.fInputColor) {
        ...
        fragBuilder->codeAppendf("\t\tcoord = coord * %f + half4(%f, %f, %f, %f);\n",
                                 kColorScaleFactor, kColorOffsetFactor, kColorOffsetFactor,
...
std::unique_ptr<GrFragmentProcessor> ColorTableEffect::TestCreate(GrProcessorTestData* d) {
    ...
    auto filter(SkTableColorFilter::MakeARGB((flags & (1 << 0)) ? luts[0] : nullptr,
