...
class SkTable_ColorFilter : public SkColorFilterBase {
 public:
  ...
  bool onAppendStages(const SkStageRec& rec, bool shaderIsOpaque) const override {
    ...
    const uint8_t *a = fBitmap.getAddr8(0, 0), *r = fBitmap.getAddr8(0, 1),
                  *g = fBitmap.getAddr8(0, 2), *b = fBitmap.getAddr8(0, 3);
    struct Tables {
...
sk_sp<SkFlattenable> SkTable_ColorFilter::CreateProc(SkReadBuffer& buffer) {
  ...
  if (buffer.readByteArray(argb, sizeof(argb))) {
    return SkTableColorFilter::MakeARGB(
        argb + 0 * 256, argb + 1 * 256, argb + 2 * 256, argb + 3 * 256);
  }
  return nullptr;
}
...
ColorTableEffect::ColorTableEffect(
  ...
  this->registerChild(
      GrTextureEffect::Make(std::move(view), kUnknown_SkAlphaType), SkSL::SampleUsage::Explicit());
  this->registerChild(std::move(inputFP));
}

ColorTableEffect::ColorTableEffect(const ColorTableEffect& that) : INHERITED(that) {}
...
GrFPResult SkTable_ColorFilter::asFragmentProcessor(
    std::unique_ptr<GrFragmentProcessor> inputFP, GrRecordingContext* context,
...
sk_sp<SkColorFilter> SkTableColorFilter::MakeARGB(
    const uint8_t tableA[256], const uint8_t tableR[256], const uint8_t tableG[256],
