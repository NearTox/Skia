...
bool Instruction::operator==(const Instruction& o) const {
  return this->kind == o.kind && this->startCode == o.startCode && this->endCode == o.endCode &&
         this->instructionIndex == o.instructionIndex && this->instruction == o.instruction &&
...
void Visualizer::markAsDeadCode(std::vector<bool>& live, const std::vector<int>& newIds) {
  for (size_t id = 0ul; id < fInstructions.size(); ++id) {
    ...
    if (live[instruction.instructionIndex]) {
      ...
      instruction.kind = static_cast<InstructionFlags>(instruction.kind | InstructionFlags::kDead);
  ...
  for (Val id = 0; id < (Val)program.size(); id++) {
    ...
    this->addInstruction(
        {viz::InstructionFlags::kNormal,
         /*startCode=*/0, /*endCode=0*/ 0, id, isDuplicate ? -1 : 0, instr});
  }
}
...
void Visualizer::finalize(
    const std::vector<skvm::Instruction>& all,
...
void Visualizer::formatPVVVV(const char* op, int imm, int v1, int v2, int v3, int v4) const {
  this->writeText(
      "%s Ptr%d, %s, %s, %s, %s", op, imm, V(v1).c_str(), V(v2).c_str(), V(v3).c_str(),
      V(v4).c_str());
}
void Visualizer::formatA_(int id, const char* op) const { writeText("%s = %s", V(id).c_str(), op); }
...
void Visualizer::dumpInstruction(int id0) const {
  ...
  const int x = instruction.instruction.x, y = instruction.instruction.y,
            z = instruction.instruction.z, w = instruction.instruction.w;
  const int immA = instruction.instruction.immA, immB = instruction.instruction.immB,
  ...
  if (instruction.instruction.op == skvm::Op::trace_line) {
    ...
    this->writeText(
        "<tr class='source'><td class='mask'></td><td colspan=2>// %s</td></tr>\n",
        fDebugInfo->fSource[immB].c_str());
    return;
  } else if (
      instruction.instruction.op == skvm::Op::trace_var ||
  ...
  if ((instruction.kind & InstructionFlags::kDead) == 0) {
    ...
    for (const MachineCommand* line = commands.first; line != commands.second; ++line) {
      this->writeText(
          "</td></tr>\n<tr class='machine'><td>%s</td><td colspan='2'>%s", line->label.c_str(),
          line->command.c_str());
    }
...
void Visualizer::dumpHead() const {
  this->writeText(
      ...
      "     <caption style='font-family:Roboto; font-size:15px; "
      "text-align:left;'>Legend</caption>\n"
      "     <tr>\n"
      ...
      "    </table>\n"
      "    <table border = \"0\"style='font-family:\"monospace\"; font-size: 10px;'>\n"
      "     <caption style='font-family:Roboto;font-size:15px;text-align:left;'>SkVM "
      "Code</caption>\n"
      "     <tr>\n"
      "        <th style=\"min-width:100px;\"><u>Kind</u></th>\n"
