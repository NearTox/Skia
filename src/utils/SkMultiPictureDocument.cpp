...
struct MultiPictureDocument final : public SkDocument {
  ...
  MultiPictureDocument(
      SkWStream* s, const SkSerialProcs* procs, std::function<void(const SkPicture*)> onEndPage)
      : SkDocument(s), fProcs(procs ? *procs : SkSerialProcs()), fOnEndPage(onEndPage) {}
...
sk_sp<SkDocument> SkMakeMultiPictureDocument(
    SkWStream* wStream, const SkSerialProcs* procs,
...
bool SkMultiPictureDocumentReadPageSizes(
    SkStreamSeekable* stream, SkDocumentPage* dstArray, int dstArrayCount) {
...
bool SkMultiPictureDocumentRead(
  ...
  for (int i = 0; i < dstArrayCount; ++i) {
    joined = SkSize{
        std::max(joined.width(), dstArray[i].fSize.width()),
