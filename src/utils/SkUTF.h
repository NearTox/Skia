...
*/
SK_SPI int CountUTF8(const char* utf8, size_t byteLength) noexcept;
...
/** Given a sequence of aligned UTF-16 characters in machine-endian form,
    ...
    UTF-16, return -1.
*/
SK_SPI int CountUTF16(const uint16_t* utf16, size_t byteLength) noexcept;

/** Given a sequence of aligned UTF-32 characters in machine-endian form,
    return the number of unicode codepoints.  If the sequence is invalid
    UTF-32, return -1.
*/
SK_SPI int CountUTF32(const int32_t* utf32, size_t byteLength) noexcept;
...
*/
SK_SPI SkUnichar NextUTF8(const char** ptr, const char* end) noexcept;
...
*/
SK_SPI SkUnichar NextUTF16(const uint16_t** ptr, const uint16_t* end) noexcept;
...
/** Given a sequence of aligned UTF-32 characters in machine-endian form,
    ...
    set *ptr to end and return -1.
*/
SK_SPI SkUnichar NextUTF32(const int32_t** ptr, const int32_t* end) noexcept;
...
*/
SK_SPI size_t ToUTF8(SkUnichar uni, char utf8[kMaxBytesInUTF8Sequence] = nullptr) noexcept;
...
*/
SK_SPI size_t ToUTF16(SkUnichar uni, uint16_t utf16[2] = nullptr) noexcept;
 ...
 */
SK_SPI int UTF8ToUTF16(
    uint16_t dst[], int dstCapacity, const char src[], size_t srcByteLength) noexcept;

/** Returns the number of resulting UTF8 values needed to convert the src utf16 sequence.
 *  If dst is not null, it is filled with the corresponding values up to its capacity.
 *  If there is an error, -1 is returned and the dst[] buffer is undefined.
 */
SK_SPI int UTF16ToUTF8(
    char dst[], int dstCapacity, const uint16_t src[], size_t srcLength) noexcept;

/**
 * Given a UTF-16 code point, returns true iff it is a leading surrogate.
 * https://unicode.org/faq/utf_bom.html#utf16-2
 */
static inline bool IsLeadingSurrogateUTF16(uint16_t c) noexcept { return ((c)&0xFC00) == 0xD800; }

/**
 * Given a UTF-16 code point, returns true iff it is a trailing surrogate.
 * https://unicode.org/faq/utf_bom.html#utf16-2
 */
static inline bool IsTrailingSurrogateUTF16(uint16_t c) noexcept { return ((c)&0xFC00) == 0xDC00; }
