...
static bool is_one_to_one(
    const char utf8[], size_t utf8Begin, size_t utf8End, std::vector<uint32_t>& clusters) {
...
void SkShaperJSONWriter::commitRunBuffer(const SkShaper::RunHandler::RunInfo& info) {
  ...
  if (is_one_to_one(fUTF8.c_str(), info.utf8Range.begin(), info.utf8Range.end(), fClusters)) {
    ...
    VisualizeClusters(
        fUTF8.c_str(), info.utf8Range.begin(), info.utf8Range.end(),
        SkSpan<const SkGlyphID>{fGlyphs}, SkSpan<const uint32_t>{fClusters},
        [this](
            size_t codePointCount, SkSpan<const char> utf1to1, SkSpan<const SkGlyphID> glyph1to1) {
...
void SkShaperJSONWriter::BreakupClusters(
    size_t utf8Begin, size_t utf8End, SkSpan<const uint32_t> clusters,
  ...
  if (clusters.front() <= clusters.back()) {
    ...
    for (size_t glyphEndIndex = 0; glyphEndIndex < clusters.size(); glyphEndIndex++) {
      if (clusters[glyphStartIndex] == clusters[glyphEndIndex]) {
        continue;
      }

      processMToN(
          glyphStartIndex, glyphEndIndex, clusters[glyphStartIndex], clusters[glyphEndIndex]);
    ...
    for (size_t glyphEndIndex = 0; glyphEndIndex < clusters.size(); glyphEndIndex++) {
      ...
      processMToN(glyphStartIndex, glyphEndIndex, clusters[glyphStartIndex], utf8EndIndex);
...
void SkShaperJSONWriter::VisualizeClusters(
    const char* utf8, size_t utf8Begin, size_t utf8End, SkSpan<const SkGlyphID> glyphIDs,
    SkSpan<const uint32_t> clusters, const VisualizeClustersCallback& processMToN) {
  ...
  auto gatherRuns = [&](size_t glyphStartIndex, size_t glyphEndIndex, uint32_t utf8StartIndex,
...
void SkShaperJSONWriter::displayMToN(
    size_t codePointCount, SkSpan<const char> utf8, SkSpan<const SkGlyphID> glyphIDs) {
