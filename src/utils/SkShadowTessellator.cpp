...
class SkBaseShadowTessellator {
 public:
  ...
  sk_sp<SkVertices> releaseVertices() {
    ...
    return SkVertices::MakeCopy(
        SkVertices::kTriangles_VertexMode, this->vertexCount(), fPositions.begin(), nullptr,
        fColors.begin(), this->indexCount(), fIndices.begin());
  }
 ...
 protected:
  ...
  void addEdge(
      const SkVector& nextPoint, const SkVector& nextNormal, SkColor umbraColor,
      const SkTDArray<SkPoint>& umbraPolygon, bool lastEdge, bool doClip);
  bool addInnerPoint(
      const SkPoint& pathPoint, SkColor umbraColor, const SkTDArray<SkPoint>& umbraPolygon,
      int* currUmbraIndex);
  ...
  void stitchConcaveRings(
      const SkTDArray<SkPoint>& umbraPolygon, SkTDArray<int>* umbraIndices,
      const SkTDArray<SkPoint>& penumbraPolygon, SkTDArray<int>* penumbraIndices);
...
static bool compute_normal(
    const SkPoint& p0, const SkPoint& p1, SkScalar dir, SkVector* newNormal) {
  SkVector normal;
...
SkBaseShadowTessellator::SkBaseShadowTessellator(
    const SkPoint3& zPlaneParams, const SkRect& bounds, bool transparent)
...
bool SkBaseShadowTessellator::checkConvexity(
...
void SkBaseShadowTessellator::finishPathPolygon() {
  ...
  if (fPathPolygon.count() > 2) {
    ...
    if (!checkConvexity(
            fPathPolygon[fPathPolygon.count() - 2], fPathPolygon[fPathPolygon.count() - 1],
            fPathPolygon[0])) {
...
bool SkBaseShadowTessellator::computeConvexShadow(SkScalar inset, SkScalar outset, bool doClip) {
  ...
  SkScalar minDistSq =
      SkPointPriv::DistanceToLineSegmentBetweenSqd(fCentroid, fPathPolygon[0], fPathPolygon[1]);
  SkRect bounds;
  ...
  for (int i = 1; i < fPathPolygon.count(); ++i) {
    ...
    SkScalar distSq = SkPointPriv::DistanceToLineSegmentBetweenSqd(fCentroid, currPoint, nextPoint);
    if (distSq < minDistSq) {
      minDistSq = distSq;
    }
  }
  ...
  if (inset > SK_ScalarNearlyZero) {
    ...
    // generate inner ring
    if (!SkInsetConvexPolygon(&fPathPolygon[0], fPathPolygon.count(), inset, &insetPolygon)) {
      // not ideal, but in this case we'll inset using the centroid
      fValidUmbra = false;
    }
  }
  const SkTDArray<SkPoint>& umbraPolygon =
      (inset > SK_ScalarNearlyZero) ? insetPolygon : fPathPolygon;
  ...
  for (int i = 1; i < polyCount; ++i) {
    ...
    this->addEdge(fPathPolygon[i], normal, umbraColor, umbraPolygon, i == polyCount - 1, doClip);
  }
  ...
  if (this->addArc(fFirstOutset, outset, false)) {
    if (fFirstUmbraOutside) {
      this->appendTriangle(fFirstVertexIndex, fPositions.count() - 1, fFirstVertexIndex + 2);
    } else {
...
void SkBaseShadowTessellator::addEdge(
    const SkPoint& nextPoint, const SkVector& nextNormal, SkColor umbraColor,
    const SkTDArray<SkPoint>& umbraPolygon, bool lastEdge, bool doClip) {
  ...
  if (!duplicate) {
    ...
    if (fTransparent) {
      ...
      SkPoint clipPoint;
      bool isOutside =
          lastEdge ? fFirstUmbraOutside
                   : this->clipUmbraPoint(fPositions[currUmbraIndex], fCentroid, &clipPoint);
      if (isOutside) {
        if (!lastEdge) {
          fPositions.push_back(clipPoint);
          fColors.push_back(umbraColor);
        ...
        if (fPrevUmbraOutside) {
          // fill out quad
          this->appendTriangle(fPrevUmbraIndex, currUmbraIndex + 1, fPrevUmbraIndex + 1);
        }
...
bool SkBaseShadowTessellator::clipUmbraPoint(
    const SkPoint& umbraPoint, const SkPoint& centroid, SkPoint* clipPoint) {
...
bool SkBaseShadowTessellator::addInnerPoint(
    const SkPoint& pathPoint, SkColor umbraColor, const SkTDArray<SkPoint>& umbraPolygon,
    int* currUmbraIndex) {
  SkPoint umbraPoint;
  if (!fValidUmbra) {
  ...
  // merge "close" points
  if (fPrevUmbraIndex == -1 || !duplicate_pt(umbraPoint, fPositions[fPrevUmbraIndex])) {
...
int SkBaseShadowTessellator::getClosestUmbraIndex(
    const SkPoint& p, const SkTDArray<SkPoint>& umbraPolygon) {
...
bool SkBaseShadowTessellator::computeConcaveShadow(SkScalar inset, SkScalar outset) {
  ...
  inset = std::min(
      inset,
      std::min(
          SkTAbs(SkRectPriv::HalfWidth(fPathBounds)), SkTAbs(SkRectPriv::HalfHeight(fPathBounds))));
  // generate inner ring
  SkTDArray<SkPoint> umbraPolygon;
  SkTDArray<int> umbraIndices;
  umbraIndices.setReserve(fPathPolygon.count());
  if (!SkOffsetSimplePolygon(
          &fPathPolygon[0], fPathPolygon.count(), fPathBounds, inset, &umbraPolygon,
          &umbraIndices)) {
  ...
  if (!SkOffsetSimplePolygon(
          &fPathPolygon[0], fPathPolygon.count(), fPathBounds, -outset, &penumbraPolygon,
          &penumbraIndices)) {
    // TODO: figure out how to handle this case
    return false;
  }
...
void SkBaseShadowTessellator::stitchConcaveRings(
    const SkTDArray<SkPoint>& umbraPolygon, SkTDArray<int>* umbraIndices,
    const SkTDArray<SkPoint>& penumbraPolygon, SkTDArray<int>* penumbraIndices) {
  ...
  while ((*penumbraIndices)[nextPenumbra] <= maxPenumbraIndex ||
    ...
    if ((*umbraIndices)[nextUmbra] == (*penumbraIndices)[nextPenumbra]) {
      ...
      this->appendQuad(prevPenumbraIndex, currPenumbraIndex, fPrevUmbraIndex, currUmbraIndex);
  ...
  if (fTransparent) {
    SkTriangulateSimplePolygon(umbraPolygon.begin(), indexMap, umbraPolygon.count(), &fIndices);
  }
}
...
void SkBaseShadowTessellator::handleQuad(const SkPoint pts[3]) {
  ...
  int count = GrPathUtils::generateQuadraticPoints(
      pts[0], pts[1], pts[2], kQuadToleranceSqd, &target, maxCount);
...
void SkBaseShadowTessellator::handleCubic(const SkMatrix& m, SkPoint pts[4]) {
  ...
  fPointBuffer.setCount(maxCount);
  SkPoint* target = fPointBuffer.begin();
  int count = GrPathUtils::generateCubicPoints(
      pts[0], pts[1], pts[2], pts[3], kCubicToleranceSqd, &target, maxCount);
  fPointBuffer.setCount(count);
  for (int i = 0; i < count; i++) {
    this->handleLine(fPointBuffer[i]);
  }
...
void SkBaseShadowTessellator::appendQuad(
    uint16_t index0, uint16_t index1, uint16_t index2, uint16_t index3) {
...
class SkAmbientShadowTessellator : public SkBaseShadowTessellator {
 public:
  SkAmbientShadowTessellator(
      const SkPath& path, const SkMatrix& ctm, const SkPoint3& zPlaneParams, bool transparent);
...
bool SkAmbientShadowTessellator::computePathPolygon(const SkPath& path, const SkMatrix& ctm) {
  ...
  while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
    ...
    switch (verb) {
      case SkPath::kLine_Verb: this->handleLine(ctm, &pts[1]); break;
      case SkPath::kQuad_Verb: this->handleQuad(ctm, pts); break;
      case SkPath::kCubic_Verb: this->handleCubic(ctm, pts); break;
      case SkPath::kConic_Verb: this->handleConic(ctm, pts, iter.conicWeight()); break;
      ...
      case SkPath::kDone_Verb: closeSeen = true; break;
    }
    verbSeen = true;
  }
...
class SkSpotShadowTessellator : public SkBaseShadowTessellator {
 public:
  SkSpotShadowTessellator(
      const SkPath& path, const SkMatrix& ctm, const SkPoint3& zPlaneParams,
      const SkPoint3& lightPos, SkScalar lightRadius, bool transparent, bool directional);

 private:
  bool computeClipAndPathPolygons(
...
SkSpotShadowTessellator::SkSpotShadowTessellator(
    const SkPath& path, const SkMatrix& ctm, const SkPoint3& zPlaneParams, const SkPoint3& lightPos,
  ...
  SkScalar outset;
  if (!SkDrawShadowMetrics::GetSpotShadowTransform(
          lightPos, lightRadius, ctm, zPlaneParams, path.getBounds(), directional, &shadowTransform,
          &outset)) {
    return;
  }
...
bool SkSpotShadowTessellator::computeClipAndPathPolygons(
    const SkPath& path, const SkMatrix& ctm, const SkMatrix& shadowTransform) {
  fPathPolygon.setReserve(path.countPoints());
  ...
  while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
    ...
    switch (verb) {
      ...
      case SkPath::kCubic_Verb:
        ...
        // point at t = 5/16
        curvePoint.fX =
            kA * clipPts[0].fX + kB * clipPts[1].fX + kC * clipPts[2].fX + kD * clipPts[3].fX;
        curvePoint.fY =
            kA * clipPts[0].fY + kB * clipPts[1].fY + kC * clipPts[2].fY + kD * clipPts[3].fY;
        this->addToClip(curvePoint);
        // point at t = 11/16
        curvePoint.fX =
            kD * clipPts[0].fX + kC * clipPts[1].fX + kB * clipPts[2].fX + kA * clipPts[3].fX;
        curvePoint.fY =
            kD * clipPts[0].fY + kC * clipPts[1].fY + kB * clipPts[2].fY + kA * clipPts[3].fY;
        this->addToClip(curvePoint);
...
sk_sp<SkVertices> SkShadowTessellator::MakeAmbient(
    const SkPath& path, const SkMatrix& ctm, const SkPoint3& zPlane, bool transparent) {
...
sk_sp<SkVertices> SkShadowTessellator::MakeSpot(
    const SkPath& path, const SkMatrix& ctm, const SkPoint3& zPlane, const SkPoint3& lightPos,
    SkScalar lightRadius, bool transparent, bool directional) {
  if (!ctm.mapRect(path.getBounds()).isFinite() || !zPlane.isFinite() || !lightPos.isFinite() ||
      !(lightPos.fZ >= SK_ScalarNearlyZero) || !SkScalarIsFinite(lightRadius) ||
      !(lightRadius >= SK_ScalarNearlyZero)) {
    return nullptr;
  }
  SkSpotShadowTessellator spotTess(
      path, ctm, zPlane, lightPos, lightRadius, transparent, directional);
