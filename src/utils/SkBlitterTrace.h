...
#    error \
        "One blitter trace type should be defined if we have flag skia_compare_vm_vs_rp flag = true."
#  endif
...
#    define SK_BLITTER_TRACE_STEP(name, trace, scanlines, pixels) \
      SkBlitterTrace::Step name(trace ? &gSkVMBlitterTrace : nullptr, #name, scanlines, pixels)
#  endif

#  ifdef SK_BLITTER_TRACE_IS_RASTER_PIPELINE
SkBlitterTrace gSkRPBlitterTrace("RP", false);
#    define SK_BLITTER_TRACE_STEP(name, trace, scanlines, pixels) \
      SkBlitterTrace::Step name(trace ? &gSkRPBlitterTrace : nullptr, #name, scanlines, pixels)
#  endif

#  define SK_BLITTER_TRACE_STEP_ACCUMULATE(step, pixels) step.add(/*scanlines=*/0, /*pixels=*/run)
#else
