...
static bool lua2bool(lua_State* L, int index) { return !!lua_toboolean(L, index); }
...
static void setfield_function(lua_State* L, const char key[], lua_CFunction value) {
...
static SkScalar byte2unit(U8CPU byte) { return byte / 255.0f; }
...
static SkColor lua2color(lua_State* L, int index) {
  return SkColorSetARGB(
      unit2byte(getfield_scalar_default(L, index, "a", 1)),
...
static int lcanvas_drawOval(lua_State* L) {
  SkRect rect;
  get_ref<SkCanvas>(L, 1)->drawOval(*lua2rect(L, 2, &rect), *get_obj<SkPaint>(L, 3));
  return 0;
}

static int lcanvas_drawCircle(lua_State* L) {
  get_ref<SkCanvas>(L, 1)->drawCircle(
      lua2scalar(L, 2), lua2scalar(L, 3), lua2scalar(L, 4), *get_obj<SkPaint>(L, 5));
  return 0;
}
...
static int lcanvas_drawPath(lua_State* L) {
  get_ref<SkCanvas>(L, 1)->drawPath(*get_obj<SkPath>(L, 2), *get_obj<SkPaint>(L, 3));
  return 0;
}
...
static int ldocument_beginPage(lua_State* L) {
  ...
  push_ptr(
      L, get_obj<DocHolder>(L, 1)->fDoc->beginPage(lua2scalar(L, 2), lua2scalar(L, 3), contentPtr));
  return 1;
}
...
static const char* gradtype2string(SkShader::GradientType t) {
  static const char* gNames[] = {"none",    "color", "linear", "radial",
                                 "radial2", "sweep", "conical"};
...
static const struct luaL_Reg gSkPathEffect_Methods[] = {
    {"asADash", lpatheffect_asADash}, {"__gc", lpatheffect_gc}, {nullptr, nullptr}};
...
static int lpath_getVerbs(lua_State* L) {
  ...
  do {
    switch (iter.next(pts)) {
      ...
      case SkPath::kQuad_Verb: setarray_string(L, ++i, "quad"); break;
      case SkPath::kConic_Verb: setarray_string(L, ++i, "conic"); break;
      case SkPath::kCubic_Verb: setarray_string(L, ++i, "cubic"); break;
...
static int lpath_quadTo(lua_State* L) {
  get_obj<SkPath>(L, 1)->quadTo(
      lua2scalar(L, 2), lua2scalar(L, 3), lua2scalar(L, 4), lua2scalar(L, 5));
  return 0;
}

static int lpath_cubicTo(lua_State* L) {
  get_obj<SkPath>(L, 1)->cubicTo(
      lua2scalar(L, 2), lua2scalar(L, 3), lua2scalar(L, 4), lua2scalar(L, 5), lua2scalar(L, 6),
      lua2scalar(L, 7));
  return 0;
}
...
static const struct luaL_Reg gSkTextBlob_Methods[] = {
    {"bounds", ltextblob_bounds}, {"__gc", ltextblob_gc}, {nullptr, nullptr}};
