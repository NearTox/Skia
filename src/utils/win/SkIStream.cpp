...
SK_STDMETHODIMP_(ULONG) SkBaseIStream::AddRef() { return (ULONG)InterlockedIncrement(&_refcount); }
...
SK_STDMETHODIMP SkBaseIStream::Read(void* pv, ULONG cb, ULONG* pcbRead) { return E_NOTIMPL; }
...
// IStream Interface
SK_STDMETHODIMP SkBaseIStream::SetSize(ULARGE_INTEGER) { return E_NOTIMPL; }

SK_STDMETHODIMP SkBaseIStream::CopyTo(IStream*, ULARGE_INTEGER, ULARGE_INTEGER*, ULARGE_INTEGER*) {
  return E_NOTIMPL;
}

SK_STDMETHODIMP SkBaseIStream::Commit(DWORD) { return E_NOTIMPL; }

SK_STDMETHODIMP SkBaseIStream::Revert() { return E_NOTIMPL; }
...
SK_STDMETHODIMP SkBaseIStream::Clone(IStream**) { return E_NOTIMPL; }

SK_STDMETHODIMP SkBaseIStream::Seek(
    LARGE_INTEGER liDistanceToMove, DWORD dwOrigin, ULARGE_INTEGER* lpNewFilePointer) {
  return E_NOTIMPL;
}

SK_STDMETHODIMP SkBaseIStream::Stat(STATSTG* pStatstg, DWORD grfStatFlag) { return E_NOTIMPL; }

/**
 * SkIStream
 */
SkIStream::SkIStream(std::unique_ptr<SkStreamAsset> stream)
    : SkBaseIStream(), fSkStream(std::move(stream)), fLocation() {
...
// IStream Interface
SK_STDMETHODIMP SkIStream::Seek(
    LARGE_INTEGER liDistanceToMove, DWORD dwOrigin, ULARGE_INTEGER* lpNewFilePointer) {
  HRESULT hr = S_OK;
  ...
  switch (dwOrigin) {
    ...
    case STREAM_SEEK_END: {
      if (!this->fSkStream->rewind()) {
        ...
        size_t skip = static_cast<size_t>(this->fSkStream->getLength() + liDistanceToMove.QuadPart);
        size_t skipped = this->fSkStream->skip(skip);
        this->fLocation.QuadPart = skipped;
        if (skipped != skip) {
          hr = E_FAIL;
    ...
    default: hr = STG_E_INVALIDFUNCTION; break;
  }
