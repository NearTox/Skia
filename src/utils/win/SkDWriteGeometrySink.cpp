...
SkDWriteGeometrySink::BeginFigure(D2D1_POINT_2F startPoint, D2D1_FIGURE_BEGIN figureBegin) {
...
SK_STDMETHODIMP_(void)
SkDWriteGeometrySink::AddLines(const D2D1_POINT_2F* points, UINT pointsCount) {
...
SK_STDMETHODIMP_(void)
SkDWriteGeometrySink::AddBeziers(const D2D1_BEZIER_SEGMENT* beziers, UINT beziersCount) {
  for (const D2D1_BEZIER_SEGMENT* end = &beziers[beziersCount]; beziers < end; ++beziers) {
    if (this->currentIsNot(beziers->point1) || this->currentIsNot(beziers->point2) ||
        this->currentIsNot(beziers->point3)) {
      Cubic cubic = {
          {fCurrent.x, fCurrent.y},
      ...
      if (check_quadratic(cubic, quadraticP1)) {
        fPath->quadTo(quadraticP1.x, quadraticP1.y, beziers->point3.x, beziers->point3.y);
      } else {
        fPath->cubicTo(
            beziers->point1.x, beziers->point1.y, beziers->point2.x, beziers->point2.y,
            beziers->point3.x, beziers->point3.y);
      }
    }
  }
...
SK_STDMETHODIMP SkDWriteGeometrySink::Close() { return S_OK; }
