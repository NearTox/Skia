...
static bool get_bitmap_info(
    SkColorType skColorType, SkAlphaType skAlphaType, size_t* bitsPerComponent, CGBitmapInfo* info,
    bool* upscaleTo32) {
  if (upscaleTo32) {
    *upscaleTo32 = false;
  }
...
static std::unique_ptr<SkBitmap> prepare_for_image_ref(
    const SkBitmap& bm, size_t* bitsPerComponent, CGBitmapInfo* info) {
  bool upscaleTo32;
...
CGImageRef SkCreateCGImageRefWithColorspace(const SkBitmap& bm, CGColorSpaceRef colorSpace) {
  ...
  return CGImageCreate(
      pm.width(), pm.height(), bitsPerComponent, pm.info().bytesPerPixel() * CHAR_BIT,
      pm.rowBytes(), colorSpace, info, dataRef.get(), nullptr, false, kCGRenderingIntentDefault);
}
...
CGContextRef SkCreateCGContext(const SkPixmap& pmap) {
  ...
  switch (pmap.colorType()) {
    ...
    case kBGRA_8888_SkColorType:
      ...
      break;
    default: return nullptr;  // no other colortypes are supported (for now)
  }

  size_t rb = pmap.addr() ? pmap.rowBytes() : 0;
  SkUniqueCFRef<CGColorSpaceRef> cs(CGColorSpaceCreateDeviceRGB());
  CGContextRef cg = CGBitmapContextCreate(
      pmap.writable_addr(), pmap.width(), pmap.height(), bitsPerComponent, rb, cs.get(),
      cg_bitmap_info);
  return cg;
}

bool SkCopyPixelsFromCGImage(
    const SkImageInfo& info, size_t rowBytes, void* pixels, CGImageRef image) {
  CGBitmapInfo cg_bitmap_info = 0;
  size_t bitsPerComponent = 0;
  switch (info.colorType()) {
    ...
    case kBGRA_8888_SkColorType:
      ...
      break;
    default: return false;  // no other colortypes are supported (for now)
  }

  SkUniqueCFRef<CGColorSpaceRef> cs(CGColorSpaceCreateDeviceRGB());
  SkUniqueCFRef<CGContextRef> cg(CGBitmapContextCreate(
      pixels, info.width(), info.height(), bitsPerComponent, rowBytes, cs.get(), cg_bitmap_info));
  if (!cg) {
    return false;
  }
