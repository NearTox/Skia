...
static bool getBitmapInfo(
    const SkBitmap& bm, size_t* bitsPerComponent, CGBitmapInfo* info, bool* upscaleTo32) {
  if (upscaleTo32) {
    *upscaleTo32 = false;
  }
...
static SkBitmap* prepareForImageRef(
...
CGImageRef SkCreateCGImageRefWithColorspace(const SkBitmap& bm, CGColorSpaceRef colorSpace) {
  ...
  CGImageRef ref = CGImageCreate(
      w, h, bitsPerComponent, bitmap->bytesPerPixel() * 8, bitmap->rowBytes(), colorSpace, info,
      dataRef, nullptr, false, kCGRenderingIntentDefault);
...
CGContextRef SkCreateCGContext(const SkPixmap& pmap) {
  ...
  CGContextRef cg = CGBitmapContextCreate(
      pmap.writable_addr(), pmap.width(), pmap.height(), bitsPerComponent, rb, cs, cg_bitmap_info);
  CFRelease(cs);
  return cg;
}
...
bool SkCopyPixelsFromCGImage(
  ...
  CGColorSpaceRef cs = CGColorSpaceCreateDeviceRGB();
  CGContextRef cg = CGBitmapContextCreate(
      pixels, info.width(), info.height(), bitsPerComponent, rowBytes, cs, cg_bitmap_info);
  CFRelease(cs);
  if (nullptr == cg) {
    return false;
  }
