...
static bool cull_path(
    const SkPath& srcPath, const SkStrokeRec& rec, const SkRect* cullRect, SkScalar intervalLength,
    SkPath* dstPath) {
  SkPoint pts[4];
  if (nullptr == cullRect) {
    ...
    if (isRect) {
      ...
      while (SkPath::kLine_Verb == iter.next(pts)) {
        ...
        if (v.fX ? between(bounds.fTop, pts[0].fY, bounds.fBottom)
          ...
          if (clip_line(pts, bounds, intervalLength, SkScalarMod(priorLength, intervalLength))) {
...
class SpecialLineRec {
 public:
  bool init(
      const SkPath& src, SkPath* dst, SkStrokeRec* rec, int intervalCount,
...
bool SkDashPath::InternalFilter(
    ...
    int32_t initialDashIndex, SkScalar intervalLength, StrokeRecApplication strokeRecApplication) {
  ...
  do {
    ...
    while (distance < length) {
      ...
      if (is_even(index) && !skipFirstSegment) {
        ...
        if (specialLine) {
          lineRec.addSegment(SkDoubleToScalar(distance), SkDoubleToScalar(distance + dlen), dst);
        } else {
          meas.getSegment(SkDoubleToScalar(distance), SkDoubleToScalar(distance + dlen), dst, true);
        }
      }
      distance += dlen;
...
bool SkDashPath::FilterDashPath(
  ...
  CalcDashParameters(
      info.fPhase, info.fIntervals, info.fCount, &initialDashLength, &initialDashIndex,
      &intervalLength);
  return InternalFilter(
      dst, src, rec, cullRect, info.fIntervals, info.fCount, initialDashLength, initialDashIndex,
      intervalLength);
}
