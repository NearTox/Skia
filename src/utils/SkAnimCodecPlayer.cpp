...
SkAnimCodecPlayer::SkAnimCodecPlayer(std::unique_ptr<SkCodec> codec) : fCodec(std::move(codec)) {
  ...
  if (!fTotalDuration) {
    ...
    fImages.clear();
    fImages.push_back(
        SkImage::MakeFromGenerator(SkCodecImageGenerator::MakeFromCodec(std::move(fCodec))));
  }
}
...
sk_sp<SkImage> SkAnimCodecPlayer::getFrameAt(int index) {
  ...
  const auto originMatrix =
      SkEncodedOriginToMatrix(origin, orientedDims.width(), orientedDims.height());
...
bool SkAnimCodecPlayer::seek(uint32_t msec) {
  ...
  auto lower = std::lower_bound(
      fFrameInfos.begin(), fFrameInfos.end(), msec,
