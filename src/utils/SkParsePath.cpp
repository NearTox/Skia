    ...
    switch (op) {
      ...
      case 'A': {
        ...
        if ((data = find_points(data, &radii, 1, false, nullptr)) && (data = skip_sep(data)) &&
            ...
            (data = find_flag(data, &largeArc)) && (data = skip_sep(data)) &&
            (data = find_flag(data, &sweep)) && (data = skip_sep(data)) &&
            (data = find_points(data, &points[0], 1, relative, &c))) {
          path.arcTo(radii, angle, (SkPath::ArcSize)largeArc, (SkPathDirection)!sweep, points[0]);
          path.getLastPt(&c);
        }
      } break;
      case 'Z':
...
void SkParsePath::ToSVGString(const SkPath& path, SkString* str, PathEncoding encoding) {
  ...
  for (;;) {
    switch (iter.next(pts)) {
      ...
      case SkPath::kMove_Verb: append_command('M', &pts[0], 1); break;
      case SkPath::kLine_Verb: append_command('L', &pts[1], 1); break;
      case SkPath::kQuad_Verb: append_command('Q', &pts[1], 2); break;
      case SkPath::kCubic_Verb: append_command('C', &pts[1], 3); break;
