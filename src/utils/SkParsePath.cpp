...
static const char* find_points(
    const char str[], SkPoint value[], int count, bool isRelative, SkPoint* relative) {
...
static const char* find_scalar(
    const char str[], SkScalar* value, bool isRelative, SkScalar relative) {
...
bool SkParsePath::FromSVGString(const char data[], SkPath* result) {
  ...
  for (;;) {
    ...
    switch (op) {
      ...
      case 'C': data = find_points(data, points, 3, relative, &c); goto cubicCommon;
      case 'S':
      ...
      case 'A': {
        ...
        if ((data = find_points(data, &radii, 1, false, nullptr)) && (data = skip_sep(data)) &&
            (data = find_scalar(data, &angle, false, 0)) && (data = skip_sep(data)) &&
            (data = find_scalar(data, &largeArc, false, 0)) && (data = skip_sep(data)) &&
            (data = find_scalar(data, &sweep, false, 0)) && (data = skip_sep(data)) &&
            (data = find_points(data, &points[0], 1, relative, &c))) {
          path.arcTo(
              radii, angle, (SkPath::ArcSize)SkToBool(largeArc), (SkPathDirection)!SkToBool(sweep),
              points[0]);
          path.getLastPt(&c);
        }
      } break;
...
void SkParsePath::ToSVGString(const SkPath& path, SkString* str) {
  ...
  for (;;) {
    switch (iter.next(pts)) {
      ...
      case SkPath::kMove_Verb: append_scalars(&stream, 'M', &pts[0].fX, 2); break;
      case SkPath::kLine_Verb: append_scalars(&stream, 'L', &pts[1].fX, 2); break;
      case SkPath::kQuad_Verb: append_scalars(&stream, 'Q', &pts[1].fX, 4); break;
      case SkPath::kCubic_Verb: append_scalars(&stream, 'C', &pts[1].fX, 6); break;
      case SkPath::kClose_Verb: stream.write("Z", 1); break;
