...
class SkGaussianColorFilter : public SkColorFilterBase {
 ...
 protected:
  ...
  skvm::Color onProgram(
      skvm::Builder* p, skvm::Color c, const SkColorInfo& dst, skvm::Uniforms*,
...
struct AmbientVerticesFactory {
  ...
  sk_sp<SkVertices> makeVertices(
      const SkPath& path, const SkMatrix& ctm, SkVector* translate) const {
...
struct SpotVerticesFactory {
  ...
  bool isCompatible(const SpotVerticesFactory& that, SkVector* translate) const {
    ...
    switch (fOccluderType) {
      ...
      case OccluderType::kDirectionalTransparent: *translate = that.fOffset - fOffset; return true;
    }
  ...
  sk_sp<SkVertices> makeVertices(
    ...
    if (directional) {
      translate->set(0, 0);
      return SkShadowTessellator::MakeSpot(
          path, ctm, zParams, fDevLightPos, fLightRadius, transparent, true);
    } else if (ctm.hasPerspective() || OccluderType::kPointOpaquePartialUmbra == fOccluderType) {
      translate->set(0, 0);
      return SkShadowTessellator::MakeSpot(
          path, ctm, zParams, fDevLightPos, fLightRadius, transparent, false);
    } else {
      ...
      *translate = fOffset;
      return SkShadowTessellator::MakeSpot(
          path, noTrans, zParams, centerLightPos, fLightRadius, transparent, false);
    }
  }
};
...
class CachedTessellations : public SkRefCnt {
 public:
  ...
  sk_sp<SkVertices> add(
      const SkPath& devPath, const AmbientVerticesFactory& ambient, const SkMatrix& matrix,
      SkVector* translate) {
  ...
  sk_sp<SkVertices> add(
      const SkPath& devPath, const SpotVerticesFactory& spot, const SkMatrix& matrix,
      SkVector* translate) {
 ...
 private:
  ...
  class Set {
   public:
        ...
        if (fEntries[i].fFactory.isCompatible(factory, translate)) {
          ...
          if (matrix.hasPerspective() || m.hasPerspective()) {
            if (matrix != fEntries[i].fMatrix) {
              ...
              matrix.getScaleX() != m.getScaleX() || matrix.getSkewX() != m.getSkewX() ||
    ...
    sk_sp<SkVertices> add(
        const SkPath& path, const FACTORY& factory, const SkMatrix& matrix, SkVector* translate) {
...
bool draw_shadow(
    ...
    std::function<
        void(const SkVertices*, SkBlendMode, const SkPaint&, SkScalar tx, SkScalar ty, bool)>
        drawProc,
  ...
  paint.setColorFilter(SkColorFilters::Blend(color, SkBlendMode::kModulate)
                           ->makeComposed(SkColorFilterPriv::MakeGaussian()));

  drawProc(
      vertices.get(), SkBlendMode::kModulate, paint, context.fTranslate.fX, context.fTranslate.fY,
...
void SkShadowUtils::ComputeTonalColors(
  ...
  *outSpotColor = SkColorSetARGB(
      tonalAlpha * 255.999f, unPremulScale * spotR, unPremulScale * spotG, unPremulScale * spotB);
}

static bool fill_shadow_rec(
    const SkPath& path, const SkPoint3& zPlaneParams, const SkPoint3& lightPos,
    SkScalar lightRadius, SkColor ambientColor, SkColor spotColor, uint32_t flags,
    const SkMatrix& ctm, SkDrawShadowRec* rec) {
...
void SkShadowUtils::DrawShadow(
    SkCanvas* canvas, const SkPath& path, const SkPoint3& zPlaneParams, const SkPoint3& lightPos,
    SkScalar lightRadius, SkColor ambientColor, SkColor spotColor, uint32_t flags) {
  SkDrawShadowRec rec;
  if (!fill_shadow_rec(
          path, zPlaneParams, lightPos, lightRadius, ambientColor, spotColor, flags,
...
bool SkShadowUtils::GetLocalBounds(
  ...
  SkDrawShadowRec rec;
  if (!fill_shadow_rec(
          path, zPlaneParams, lightPos, lightRadius, SK_ColorBLACK, SK_ColorBLACK, flags, ctm,
          &rec)) {
    return false;
  }
...
void SkBaseDevice::drawShadow(const SkPath& path, const SkDrawShadowRec& rec) {
  ...
  bool useBlur =
      SkToBool(rec.fFlags & SkShadowFlags::kConcaveBlurOnly_ShadowFlag) && !path.isConvex();
  ...
  if (SkColorGetA(rec.fAmbientColor) > 0) {
    ...
    if (uncached && !useBlur) {
      sk_sp<SkVertices> vertices =
          SkShadowTessellator::MakeAmbient(path, viewMatrix, zPlaneParams, transparent);
      if (vertices) {
        SkPaint paint;
        ...
        // their alpha. We can skip expensive per-vertex color conversion for this.
        this->drawVertices(
            vertices.get(), SkBlender::Mode(SkBlendMode::kModulate), paint,
            /*skipColorXform=*/true);
        success = true;
      }
    }
  ...
  if (SkColorGetA(rec.fSpotColor) > 0) {
    bool success = false;
    if (uncached && !useBlur) {
      sk_sp<SkVertices> vertices = SkShadowTessellator::MakeSpot(
          path, viewMatrix, zPlaneParams, devLightPos, lightRadius, transparent, directional);
      if (vertices) {
        SkPaint paint;
        // Run the vertex color through a GaussianColorFilter and then modulate the
        // grayscale result of that against our 'color' param.
    ...
    if (!success && !useBlur) {
      ...
      if (SkToBool(rec.fFlags & kDirectionalLight_ShadowFlag)) {
        SkDrawShadowMetrics::GetDirectionalParams(
            zPlaneParams.fZ, devLightPos.fX, devLightPos.fY, devLightPos.fZ, lightRadius, &radius,
            &scale, &factory.fOffset);
      } else {
        SkDrawShadowMetrics::GetSpotParams(
            zPlaneParams.fZ, devLightPos.fX - center.fX, devLightPos.fY - center.fY, devLightPos.fZ,
    ...
    if (!success) {
      ...
      if (!SkDrawShadowMetrics::GetSpotShadowTransform(
              devLightPos, lightRadius, viewMatrix, zPlaneParams, path.getBounds(), directional,
              &shadowMatrix, &radius)) {
        return;
      }
