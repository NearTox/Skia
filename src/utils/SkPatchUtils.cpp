...
static SkScalar bilerp(
    SkScalar tx, SkScalar ty, SkScalar c00, SkScalar c10, SkScalar c01, SkScalar c11) {
  SkScalar a = c00 * (1.f - tx) + c10 * tx;
  SkScalar b = c01 * (1.f - tx) + c11 * tx;
  return a * (1.f - ty) + b * ty;
}

static skvx::float4 bilerp(
    SkScalar tx, SkScalar ty, const skvx::float4& c00, const skvx::float4& c10,
    const skvx::float4& c01, const skvx::float4& c11) {
...
sk_sp<SkVertices> SkPatchUtils::MakeVertices(
    const SkPoint cubics[12], const SkColor srcColors[4], const SkPoint srcTexCoords[4], int lodX,
    int lodY, SkColorSpace* colorSpace) {
  ...
  for (int x = 0; x <= lodX; x++) {
    ...
    for (int y = 0; y <= lodY; y++) {
      ...
      SkPoint s0 = SkPoint::Make(
          (1.0f - v) * top.x() + v * bottom.x(), (1.0f - v) * top.y() + v * bottom.y());
      SkPoint s1 = SkPoint::Make(
          (1.0f - u) * left.x() + u * right.x(), (1.0f - u) * left.y() + u * right.y());
      SkPoint s2 = SkPoint::Make(
          (1.0f - v) *
                  ((1.0f - u) * fTop.getCtrlPoints()[0].x() + u * fTop.getCtrlPoints()[3].x()) +
              v * ((1.0f - u) * fBottom.getCtrlPoints()[0].x() +
                   u * fBottom.getCtrlPoints()[3].x()),
          (1.0f - v) *
                  ((1.0f - u) * fTop.getCtrlPoints()[0].y() + u * fTop.getCtrlPoints()[3].y()) +
      ...
      if (cornerColors) {
        bilerp(
            u, v, skvx::float4::Load(cornerColors[kTopLeft_Corner].vec()),
      ...
      if (texs) {
        texs[dataIndex] = SkPoint::Make(
            bilerp(
                u, v, srcTexCoords[kTopLeft_Corner].x(), srcTexCoords[kTopRight_Corner].x(),
                srcTexCoords[kBottomLeft_Corner].x(), srcTexCoords[kBottomRight_Corner].x()),
            bilerp(
                u, v, srcTexCoords[kTopLeft_Corner].y(), srcTexCoords[kTopRight_Corner].y(),
                srcTexCoords[kBottomLeft_Corner].y(), srcTexCoords[kBottomRight_Corner].y()));
      }
