...
class SkUserTypeface : public SkTypeface {
  ...
  SkUserTypeface(int count) : SkTypeface(SkFontStyle()), fGlyphCount(count) {}
...
class SkUserScalerContext : public SkScalerContext {
 public:
  SkUserScalerContext(
      sk_sp<SkUserTypeface> face, const SkScalerContextEffects& effects, const SkDescriptor* desc)
 ...
 protected:
  unsigned generateGlyphCount() override { return this->userTF()->fGlyphCount; }
  ...
  for (auto [v, p, w] : SkPathPriv::Iterate(path)) {
    switch (v) {
      ...
      case SkPathVerb::kMove:
        verbs.push_back(kMove);
        pCount += 1;
        break;
      case SkPathVerb::kQuad:
        verbs.push_back(kCurve);
        pCount += 2;
        break;
      case SkPathVerb::kLine:
        verbs.push_back(kLine);
        pCount += 1;
        break;
...
class AutoRestorePosition {
 ...
 public:
  AutoRestorePosition(SkStream* stream) : fStream(stream) { fPosition = stream->getPosition(); }
