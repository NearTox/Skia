  ...
  bool insert(const SkPoint& p0, const SkPoint& p1, uint16_t index0, uint16_t index1) {
    ...
    while (true) {
      if (!curr) {
        ...
        curr->fBelow = succ;
        if (pred) {
          if (pred->fSegment.fP0 == curr->fSegment.fP0 && pred->fSegment.fV == curr->fSegment.fV) {
            return false;
          }
          pred->fBelow = curr;
        }
        if (succ) {
          if (succ->fSegment.fP0 == curr->fSegment.fP0 && succ->fSegment.fV == curr->fSegment.fV) {
            return false;
          }
          succ->fAbove = curr;
        }
...
bool SkOffsetSimplePolygon(
  ...
  if (offset >
      std::min(SkTAbs(SkRectPriv::HalfWidth(bounds)), SkTAbs(SkRectPriv::HalfHeight(bounds)))) {
    return false;
  }
