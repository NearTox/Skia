...
static constexpr inline bool outside_interval(
    SkScalar numer, SkScalar denom, bool denomPositive) noexcept {
...
bool SkInsetConvexPolygon(
  ...
  for (int curr = 0; curr < inputPolygonSize; prev = curr, ++curr) {
    ...
    if (compute_side(
            inputPolygonVerts[prev], inputPolygonVerts[curr] - inputPolygonVerts[prev],
            inputPolygonVerts[next]) *
            winding <
        0) {
      return false;
    }
  ...
  while (head && prevEdge != currEdge) {
    ...
    if (compute_intersection(prevEdge->fOffset, currEdge->fOffset, &intersection, &s, &t)) {
      ...
      if (s < prevEdge->fTValue) {
        ...
        // we've already considered this intersection, we're done
          ...
          currEdge->fTValue > SK_ScalarMin &&
          SkPointPriv::EqualsWithinTolerance(intersection, currEdge->fIntersection, 1.0e-6f)) {
        break;
      } else {
        // add intersection
      ...
      int side = winding *
                 compute_side(currEdge->fOffset.fP0, currEdge->fOffset.fV, prevEdge->fOffset.fP0);
      if (side < 0 && side == winding * compute_side(
                                            currEdge->fOffset.fP0, currEdge->fOffset.fV,
  ...
  *insetPolygon->push() = head->fIntersection;
  currEdge = head->fNext;
  while (currEdge != head) {
    if (!SkPointPriv::EqualsWithinTolerance(
            currEdge->fIntersection, (*insetPolygon)[currIndex], kCleanupTolerance)) {
  ...
  if (currIndex >= 1 && SkPointPriv::EqualsWithinTolerance(
                            (*insetPolygon)[0], (*insetPolygon)[currIndex], kCleanupTolerance)) {
    insetPolygon->pop();
  }
  ...
  while (vertexQueue.count() > 0) {
    ...
    if (v.fFlags == 0) {
      ...
      if (v.fFlags & kPrevLeft_VertexFlag) {
        if (!sweepLine.replace(
                polygon[v.fPrevIndex], v.fPosition, polygon[v.fNextIndex], v.fPrevIndex, v.fIndex,
                v.fNextIndex)) {
          break;
        }
      } else {
        ...
        if (!sweepLine.replace(
                polygon[v.fNextIndex], v.fPosition, polygon[v.fPrevIndex], v.fNextIndex, v.fIndex,
                v.fPrevIndex)) {
          break;
        }
      }
...
bool SkOffsetSimplePolygon(
  ...
  for (int currIndex = 0, prevIndex = inputPolygonSize - 1; currIndex < inputPolygonSize;
       prevIndex = currIndex, ++currIndex) {
    if (!inputPolygonVerts[currIndex].isFinite()) {
      return false;
    }
    int nextIndex = (currIndex + 1) % inputPolygonSize;
    if (!compute_offset_vector(
            inputPolygonVerts[currIndex], inputPolygonVerts[nextIndex], offset, winding,
            &normals[currIndex])) {
      return false;
    }
    if (currIndex > 0) {
      // if reflex point, we need to add extra edges
      if (is_reflex_vertex(inputPolygonVerts, winding, offset, prevIndex, currIndex, nextIndex)) {
        SkScalar rotSin, rotCos;
        int numSteps;
        if (!SkComputeRadialSteps(
  ...
  for (int currIndex = 0, prevIndex = inputPolygonSize - 1; currIndex < inputPolygonSize;
    ...
    if (is_reflex_vertex(inputPolygonVerts, winding, offset, prevIndex, currIndex, nextIndex)) {
      ...
      for (int i = 0; i < numSteps - 1; ++i) {
        ...
        setup_offset_edge(
            currEdge, inputPolygonVerts[currIndex] + prevNormal,
            inputPolygonVerts[currIndex] + currNormal, currIndex, currIndex);
      ...
      setup_offset_edge(
          currEdge, inputPolygonVerts[currIndex] + prevNormal,
          inputPolygonVerts[currIndex] + normals[currIndex], currIndex, currIndex);
      currEdge->fPrev = prevEdge;
      if (prevEdge) {
        prevEdge->fNext = currEdge;
      }
    ...
    setup_offset_edge(
        currEdge, inputPolygonVerts[currIndex] + normals[currIndex],
        inputPolygonVerts[nextIndex] + normals[currIndex], currIndex, nextIndex);
    currEdge->fPrev = prevEdge;
    if (prevEdge) {
      prevEdge->fNext = currEdge;
    }
  ...
  while (head && prevEdge != currEdge && offsetVertexCount > 0) {
    ...
    if (prevEdge->checkIntersection(currEdge, &intersection, &s, &t)) {
      ...
      if (dist0 < 0 && dist1 < 0) {
        ...
        bool prevSameContour = SkPointPriv::EqualsWithinTolerance(p1, prevEdge->fOffset.fP0);
        p1 = currEdge->fOffset.fP0 + currEdge->fOffset.fV;
        bool currSameContour = SkPointPriv::EqualsWithinTolerance(p1, currNextEdge->fOffset.fP0);
  ...
  *offsetPolygon->push() = head->fIntersection;
  ...
  while (currEdge != head) {
    if (!SkPointPriv::EqualsWithinTolerance(
            currEdge->fIntersection, (*offsetPolygon)[currIndex], kCleanupTolerance)) {
  ...
  // make sure the first and last points aren't coincident
  if (currIndex >= 1 && SkPointPriv::EqualsWithinTolerance(
                            (*offsetPolygon)[0], (*offsetPolygon)[currIndex], kCleanupTolerance)) {
    offsetPolygon->pop();
    if (polygonIndices) {
      polygonIndices->pop();
    }
...
static void reclassify_vertex(
    TriangulationVertex* p, const SkPoint* polygonVerts, int winding, ReflexHash* reflexHash,
...
bool SkTriangulateSimplePolygon(
  ...
  while (vertexCount > 3) {
    ...
    for (SkTInternalLList<TriangulationVertex>::Iter convexIter = convexList.begin();
      ...
      bool failed = reflexHash.checkTriangle(
          p0->fPosition, earVertex->fPosition, p2->fPosition, p0->fIndex, p2->fIndex);
      if (failed) {
        continue;
      }
...
static constexpr double crs(SkVector a, SkVector b) noexcept { return a.fX * b.fY - a.fY * b.fX; }

static constexpr int sign(SkScalar v) noexcept { return v < 0 ? -1 : (v > 0); }
