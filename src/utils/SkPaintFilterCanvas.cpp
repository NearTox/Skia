...
SkPaintFilterCanvas::SkPaintFilterCanvas(SkCanvas* canvas)
    : SkCanvasVirtualEnforcer<SkNWayCanvas>(
          canvas->imageInfo().width(), canvas->imageInfo().height()) {
...
void SkPaintFilterCanvas::onDrawPoints(
...
void SkPaintFilterCanvas::onDrawDRRect(
...
void SkPaintFilterCanvas::onDrawArc(
...
void SkPaintFilterCanvas::onDrawImageRect2(
...
void SkPaintFilterCanvas::onDrawImageLattice2(
...
void SkPaintFilterCanvas::onDrawPatch(
    const SkPoint cubics[], const SkColor colors[], const SkPoint texCoords[], SkBlendMode bmode,
    const SkPaint& paint) {
  AutoPaintFilter apf(this, paint);
  if (apf.shouldDraw()) {
...
void SkPaintFilterCanvas::onDrawTextBlob(
...
void SkPaintFilterCanvas::onDrawAtlas2(
  ...
  AutoPaintFilter apf(this, paint);
  if (apf.shouldDraw()) {
    this->SkNWayCanvas::onDrawAtlas2(
        image, xform, tex, colors, count, bmode, sampling, cull, &apf.paint());
  }
}
...
void SkPaintFilterCanvas::onDrawEdgeAAImageSet2(
