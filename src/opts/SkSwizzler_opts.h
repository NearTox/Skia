...
static uint8x8_t scale(uint8x8_t x, uint8x8_t y) { return div255_round(vmull_u8(x, y)); }
...
static void premul_should_swapRB(bool kSwapRB, uint32_t* dst, const uint32_t* src, int count) {
  auto premul8 = [=](__m512i* lo, __m512i* hi) {
    ...
    if (kSwapRB) {
      mask = {2, 6,  10, 14, 1, 5,  9,  13, 0,  4,  8, 12, 3,  7,  11, 15, 2, 6,  10, 14, 1, 5,
              9, 13, 0,  4,  8, 12, 3,  7,  11, 15, 2, 6,  10, 14, 1,  5,  9, 13, 0,  4,  8, 12,
              3, 7,  11, 15, 2, 6,  10, 14, 1,  5,  9, 13, 0,  4,  8,  12, 3, 7,  11, 15};
    } else {
      mask = {0, 4,  8,  12, 1,  5,  9, 13, 2,  6,  10, 14, 3, 7,  11, 15, 0, 4,  8,  12, 1,  5,
              9, 13, 2,  6,  10, 14, 3, 7,  11, 15, 0,  4,  8, 12, 1,  5,  9, 13, 2,  6,  10, 14,
              3, 7,  11, 15, 0,  4,  8, 12, 1,  5,  9,  13, 2, 6,  10, 14, 3, 7,  11, 15};
    }
    ...
    *hi = _mm512_shuffle_epi8(*hi, planar);
    __m512i rg = _mm512_unpacklo_epi32(*lo, *hi), ba = _mm512_unpackhi_epi32(*lo, *hi);

    // Unpack to 16-bit planar.
    __m512i r = _mm512_unpacklo_epi8(rg, zeros), g = _mm512_unpackhi_epi8(rg, zeros),
            b = _mm512_unpacklo_epi8(ba, zeros), a = _mm512_unpackhi_epi8(ba, zeros);
  ...
  if (count >= 16) {
    __m512i lo = _mm512_loadu_si512((const __m512i*)src), hi = _mm512_setzero_si512();
...
/*not static*/ inline void grayA_to_RGBA(uint32_t dst[], const uint8_t* src, int count) {
  while (count >= 32) {
    ...
    __m512i gg =
        _mm512_or_si512(_mm512_and_si512(ga, _mm512_set1_epi16(0x00FF)), _mm512_slli_epi16(ga, 8));
    ...
    __m512i ggga_lo_shuffle_2 = _mm512_shuffle_i32x4(ggga_lo_shuffle_1, ggga_lo_shuffle_1, 0xd8),
            ggga_hi_shuffle_2 = _mm512_shuffle_i32x4(ggga_hi_shuffle_1, ggga_hi_shuffle_1, 0xd8);
...
static void inverted_cmyk_to(Format format, uint32_t* dst, const uint32_t* src, int count) {
  auto convert8 = [=](__m512i* lo, __m512i* hi) {
    ...
    *lo = _mm512_shuffle_epi8(*lo, planar);
    *hi = _mm512_shuffle_epi8(*hi, planar);
    __m512i cm = _mm512_unpacklo_epi32(*lo, *hi), yk = _mm512_unpackhi_epi32(*lo, *hi);

    // Unpack to 16-bit planar.
    __m512i c = _mm512_unpacklo_epi8(cm, zeros), m = _mm512_unpackhi_epi8(cm, zeros),
            y = _mm512_unpacklo_epi8(yk, zeros), k = _mm512_unpackhi_epi8(yk, zeros);

    // Scale to r, g, b.
    __m512i r = scale(c, k), g = scale(m, k), b = scale(y, k);
...
static void premul_should_swapRB(bool kSwapRB, uint32_t* dst, const uint32_t* src, int count) {
  auto premul8 = [=](__m256i* lo, __m256i* hi) {
    ...
    __m256i planar;
    if (kSwapRB) {
      planar = _mm256_setr_epi8(
          2, 6, 10, 14, 1, 5, 9, 13, 0, 4, 8, 12, 3, 7, 11, 15, 2, 6, 10, 14, 1, 5, 9, 13, 0, 4, 8,
          12, 3, 7, 11, 15);
    } else {
      planar = _mm256_setr_epi8(
  ...
  if (count >= 8) {
    __m256i lo = _mm256_loadu_si256((const __m256i*)src), hi = _mm256_setzero_si256();
...
/*not static*/ inline void RGBA_to_BGRA(uint32_t* dst, const uint32_t* src, int count) {
  const __m256i swapRB = _mm256_setr_epi8(
...
/*not static*/ inline void grayA_to_RGBA(uint32_t dst[], const uint8_t* src, int count) {
  while (count >= 16) {
    ...
    __m256i gg =
        _mm256_or_si256(_mm256_and_si256(ga, _mm256_set1_epi16(0x00FF)), _mm256_slli_epi16(ga, 8));
...
static void inverted_cmyk_to(Format format, uint32_t* dst, const uint32_t* src, int count) {
  auto convert8 = [=](__m256i* lo, __m256i* hi) {
    ...
    *hi = _mm256_shuffle_epi8(*hi, planar);        // CCCCMMMM YYYYKKKK CCCCMMMM YYYYKKKK
    ...
    // Scale to r, g, b.
    __m256i r = scale(c, k), g = scale(m, k), b = scale(y, k);
...
static void insert_alpha_should_swaprb(
    bool kSwapRB, uint32_t dst[], const uint8_t* src, int count) {
  while (count >= 16) {
    // Load 16 pixels.
