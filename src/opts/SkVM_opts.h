...
inline void interpret_skvm(
    ...
    const int strides[], skvm::TraceHook* traceHooks[], const int nTraceHooks, const int nargs,
    int n, void* args[]) {
...
#define STRIDE_K(op) case 2 * (int)op + 1
          STRIDE_1(Op::store8) : memcpy(args[immA], &r[x].i32, 1);
          break;
          STRIDE_1(Op::store16) : memcpy(args[immA], &r[x].i32, 2);
          break;
          STRIDE_1(Op::store32) : memcpy(args[immA], &r[x].i32, 4);
          break;
          STRIDE_1(Op::store64) : memcpy((char*)args[immA] + 0, &r[x].i32, 4);
          memcpy((char*)args[immA] + 4, &r[y].i32, 4);
          break;

          STRIDE_K(Op::store8) : skvx::cast<uint8_t>(r[x].i32).store(args[immA]);
          break;
          STRIDE_K(Op::store16) : skvx::cast<uint16_t>(r[x].i32).store(args[immA]);
          break;
          STRIDE_K(Op::store32) : (r[x].i32).store(args[immA]);
          break;
          STRIDE_K(Op::store64)
          ...
          memcpy(&r[d].i32, args[immA], 1);
          break;
          STRIDE_1(Op::load16) : r[d].i32 = 0;
          memcpy(&r[d].i32, args[immA], 2);
          break;
          STRIDE_1(Op::load32) : r[d].i32 = 0;
          memcpy(&r[d].i32, args[immA], 4);
          break;
          STRIDE_1(Op::load64) : r[d].i32 = 0;
          memcpy(&r[d].i32, (char*)args[immA] + 4 * immB, 4);
          break;

          STRIDE_K(Op::load8) : r[d].i32 = skvx::cast<int>(U8 ::Load(args[immA]));
          break;
          STRIDE_K(Op::load16) : r[d].i32 = skvx::cast<int>(U16::Load(args[immA]));
          break;
          STRIDE_K(Op::load32) : r[d].i32 = I32::Load(args[immA]);
          break;
          STRIDE_K(Op::load64)
              :  // Low 32 bits if immB=0, or high 32 bits if immB=1.
                r[d]
                    .i32 = skvx::cast<int>(U64::Load(args[immA]) >> (32 * immB));
          break;
...
#define CASE(op)                    \
  ...
  case 2 * (int)op + 1
          ...
          CASE(Op::store128) : {
            U64 lo = (skvx::cast<uint64_t>(r[x].u32) << 0 | skvx::cast<uint64_t>(r[y].u32) << 32),
                hi = (skvx::cast<uint64_t>(r[z].u32) << 0 | skvx::cast<uint64_t>(r[w].u32) << 32);
          ...
          CASE(Op::trace_line) : if (should_trace(stride, immA, x, y)) {
            traceHooks[immA]->line(immB);
          }
          break;

          CASE(Op::trace_var) : if (should_trace(stride, immA, x, y)) {
            for (int i = 0; i < K; ++i) {
          ...
          CASE(Op::trace_enter) : if (should_trace(stride, immA, x, y)) {
            traceHooks[immA]->enter(immB);
          }
          break;

          CASE(Op::trace_exit) : if (should_trace(stride, immA, x, y)) {
            traceHooks[immA]->exit(immB);
          }
          break;

          CASE(Op::trace_scope) : if (should_trace(stride, immA, x, y)) {
          ...
          CASE(Op::uniform32) : r[d].i32 = *(const int*)((const char*)args[immA] + immB);
          break;

          CASE(Op::array32) : const int * ptr;
          memcpy(&ptr, (const uint8_t*)args[immA] + immB, sizeof(ptr));
          r[d].i32 = ptr[immC / sizeof(int)];
          break;

          CASE(Op::splat) : r[d].i32 = immA;
          break;
          ...
          CASE(Op::shl_i32) : r[d].i32 = r[x].i32 << immA;
          break;
          CASE(Op::sra_i32) : r[d].i32 = r[x].i32 >> immA;
          break;
          CASE(Op::shr_i32) : r[d].u32 = r[x].u32 >> immA;
          break;
          ...
          CASE(Op::to_fp16) : r[d].i32 = skvx::cast<int>(skvx::to_half(r[x].f32));
          break;
          CASE(Op::from_fp16) : r[d].f32 = skvx::from_half(skvx::cast<uint16_t>(r[x].i32));
          break;
