...
inline Sk4px::Wide Sk4px::widen() const noexcept {
  return Sk16h(
      _mm_unpacklo_epi8(this->fVec, _mm_setzero_si128()),
      _mm_unpackhi_epi8(this->fVec, _mm_setzero_si128()));
}

inline Sk4px::Wide Sk4px::mulWiden(const Sk16b& other) const noexcept {
  return this->widen() * Sk4px(other).widen();
}

inline Sk4px Sk4px::Wide::addNarrowHi(const Sk16h& other) const noexcept {
  Sk4px::Wide r = (*this + other) >> 8;
  return Sk4px(_mm_packus_epi16(r.fLo.fVec, r.fHi.fVec));
}

inline Sk4px Sk4px::Wide::div255() const noexcept {
  ...
  return Sk4px(_mm_packus_epi16(
      _mm_mulhi_epu16(_mm_add_epi16(fLo.fVec, _128), _257),
...
inline Sk4px Sk4px::alphas() const noexcept {
...
inline Sk4px Sk4px::Load4Alphas(const SkAlpha a[4]) noexcept {
  uint32_t as;
  memcpy(&as, a, 4);
...
inline Sk4px Sk4px::Load2Alphas(const SkAlpha a[2]) noexcept {
  uint16_t alphas;
  memcpy(&alphas, a, 2);
