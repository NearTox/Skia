...
static inline __m256i SkPMSrcOver_AVX2(const __m256i& src, const __m256i& dst) {
  ...
  __m256i srcA_x2 = _mm256_shuffle_epi8(
      src, _mm256_setr_epi8(
               3, _, 3, _, 7, _, 7, _, 11, _, 11, _, 15, _, 15, _, 3, _, 3, _, 7, _, 7, _, 11, _,
               11, _, 15, _, 15, _));
...
static inline __m128i SkPMSrcOver_SSE2(const __m128i& src, const __m128i& dst) {
  ...
  return _mm_add_epi32(
      src, SkAlphaMulQ_SSE2(dst, _mm_sub_epi32(_mm_set1_epi32(256), _mm_srli_epi32(src, 24))));
}
#endif
...
/*not static*/ inline void blit_row_s32a_opaque(
    SkPMColor* dst, const SkPMColor* src, int len, U8CPU alpha) {
  SkASSERT(alpha == 0xFF);
  ...
  while (len >= 64) {
    ...
    if (0 == _mm512_cmpneq_epi8_mask(_mm512_and_si512(ORed, alphaMask), _mm512_setzero_si512())) {
    ...
    auto d0 = (__m512i*)(dst) + 0, d1 = (__m512i*)(dst) + 1, d2 = (__m512i*)(dst) + 2,
         d3 = (__m512i*)(dst) + 3;

    auto ANDed = _mm512_and_si512(s3, _mm512_and_si512(s2, _mm512_and_si512(s1, s0)));
    if (0 == _mm512_cmpneq_epi8_mask(_mm512_and_si512(ANDed, alphaMask), alphaMask)) {
...
// Require AVX2 because of AVX2 integer calculation intrinsics in SrcOver
  ...
  while (len >= 32) {
    ...
    auto d0 = (__m256i*)(dst) + 0, d1 = (__m256i*)(dst) + 1, d2 = (__m256i*)(dst) + 2,
         d3 = (__m256i*)(dst) + 3;
  ...
  while (len >= 16) {
    ...
    auto ORed = _mm_or_si128(s3, _mm_or_si128(s2, _mm_or_si128(s1, s0)));
    if (0xffff ==
        _mm_movemask_epi8(_mm_cmpeq_epi8(_mm_and_si128(ORed, alphaMask), _mm_setzero_si128()))) {
      // All 16 source pixels are transparent.  Nothing to do.
      src += 16;
      dst += 16;
      len -= 16;
