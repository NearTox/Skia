...
#  endif
    ...
    skvx::Vec<8, uint32_t> tl = gather(row0, x0), tr = gather(row0, x1), bl = gather(row1, x0),
                           br = gather(row1, x1);
...
#  if 1
    ...
    auto lerp_x = [&](skvx::Vec<8, uint32_t> L, skvx::Vec<8, uint32_t> R) {
      __m256i l = skvx::bit_pun<__m256i>(L), r = skvx::bit_pun<__m256i>(R),
              wr = skvx::bit_pun<__m256i>(wx), wl = _mm256_sub_epi8(_mm256_set1_epi8(16), wr);

      // Interlace l,r bytewise and line them up with their weights, then lerp.
      __m256i lo = _mm256_maddubs_epi16(_mm256_unpacklo_epi8(l, r), _mm256_unpacklo_epi8(wl, wr));
      __m256i hi = _mm256_maddubs_epi16(_mm256_unpackhi_epi8(l, r), _mm256_unpackhi_epi8(wl, wr));
      ...
      return skvx::join(
          skvx::bit_pun<skvx::Vec<16, uint16_t>>(abcd),
          skvx::bit_pun<skvx::Vec<16, uint16_t>>(efgh));
    };

    skvx::Vec<32, uint16_t> top = lerp_x(tl, tr), bot = lerp_x(bl, br),
...
#  else
    ...
    auto lerp = [](auto lo, auto hi, auto w) { return 16 * lo + (hi - lo) * w; };
    skvx::Vec<32, uint16_t> sum =
        lerp(lerp(to_16x4(tl), to_16x4(bl), wy), lerp(to_16x4(tr), to_16x4(br), wy), to_16x4(wx));
#  endif
  ...
  if (count > 0) {
    __m256i active = skvx::bit_pun<__m256i>(count > skvx::Vec<8, int>{0, 1, 2, 3, 4, 5, 6, 7}),
            coords = _mm256_maskload_epi32((const int*)xy, active), pixels;
    ...
    sk_msan_mark_initialized(
        colors, colors + count, "MSAN still doesn't understand AVX2 mask loads and stores.");
  }
}
...
/*not static*/ inline void S32_alpha_D32_filter_DX(
  ...
  auto interpolate_in_x = [](uint32_t A0, uint32_t A1, uint32_t B0, uint32_t B1,
    ...
    return _mm_maddubs_epi16(_mm_unpacklo_epi64(interlaced_A, interlaced_B), interlaced_x_weights);
  };
  ...
  auto interpolate_in_x_and_y = [&](uint32_t A0, uint32_t A1, uint32_t A2, uint32_t A3, uint32_t B0,
    ...
    __m128i px = _mm_add_epi16(
        _mm_slli_epi16(top, 4), _mm_mullo_epi16(_mm_sub_epi16(bot, top), _mm_set1_epi16(wy)));
  ...
  while (count >= 4) {
    ...
    __m128i wr = _mm_shuffle_epi8(
                wx, _mm_setr_epi8(0, 0, 0, 0, 4, 4, 4, 4, 8, 8, 8, 8, 12, 12, 12, 12)),
    ...
    __m128i AB = interpolate_in_x_and_y(
        row0[x0[A]], row0[x1[A]], row1[x0[A]], row1[x1[A]], row0[x0[B]], row0[x1[B]], row1[x0[B]],
        row1[x1[B]], interlaced_x_weights_AB, wy);

    // Once more with the other half of the x-weights for two more pixels C,D.
    __m128i CD = interpolate_in_x_and_y(
        row0[x0[C]], row0[x1[C]], row1[x0[C]], row1[x1[C]], row0[x0[D]], row0[x1[D]], row1[x0[D]],
        row1[x1[D]], interlaced_x_weights_CD, wy);
  ...
  while (count-- > 0) {
    ...
    __m128i A = interpolate_in_x_and_y(
        row0[x0], row0[x1], row1[x0], row1[x1], 0, 0, 0, 0, interlaced_x_weights, wy);
...
/*not static*/ inline void S32_alpha_D32_filter_DX(
  ...
  const __m128i allY = _mm_unpacklo_epi64(
      _mm_set1_epi16(wy),        // Bottom pixel goes here.
  ...
  while (count-- > 0) {
    ...
    __m128i inner = _mm_add_epi16(
        _mm_slli_epi16(L, 4), _mm_mullo_epi16(_mm_sub_epi16(R, L), _mm_set1_epi16(wx)));
...
/*not static*/ inline void S32_alpha_D32_filter_DX(
  ...
  while (count-- > 0) {
    ...
    filter_and_scale_by_alpha(
        wx, wy, row0[x0], row0[x1], row1[x0], row1[x1], colors++, s.fAlphaScale);
  }
}
...
/*not static*/ inline void S32_alpha_D32_filter_DXDY(
  ...
  while (count-- > 0) {
    int y0, y1, wy, x0, x1, wx;
    decode_packed_coordinates_and_weight(*xy++, &y0, &y1, &wy);
    decode_packed_coordinates_and_weight(*xy++, &x0, &x1, &wx);

    auto row0 = (const uint32_t*)(src + y0 * rb), row1 = (const uint32_t*)(src + y1 * rb);
...
constexpr static void (*S32_alpha_D32_filter_DXDY)(
    const SkBitmapProcState&, const uint32_t*, int, SkPMColor*) = nullptr;
#endif
