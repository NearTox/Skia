...
static inline __m128i interpolate_in_x(uint32_t A0, uint32_t A1, uint32_t B0, uint32_t B1,
...
static inline __m128i interpolate_in_x_and_y(uint32_t A0, uint32_t A1, uint32_t A2, uint32_t A3,
                                             uint32_t B0, uint32_t B1, uint32_t B2, uint32_t B3,
                                             const __m128i& interlaced_x_weights, int wy) {
    // The stored Y weight wy is for y1, and y0 gets a weight 16-wy.
    const __m128i wy1 = _mm_set1_epi16(wy), wy0 = _mm_sub_epi16(_mm_set1_epi16(16), wy1);
    ...
    return _mm_srli_epi16(_mm_add_epi16(_mm_mullo_epi16(row0, wy0), _mm_mullo_epi16(row1, wy1)), 8);
}

/*not static*/ inline void S32_alpha_D32_filter_DX(const SkBitmapProcState& s, const uint32_t* xy,
    ...
    auto scale_by_alpha = [alpha](const __m128i& px) {
        return alpha == 256 ? px : _mm_srli_epi16(_mm_mullo_epi16(px, _mm_set1_epi16(alpha)), 8);
    };
    ...
    while (count >= 4) {
        ...
        __m128i AB = interpolate_in_x_and_y(row0[x0[0]], row0[x1[0]], row1[x0[0]], row1[x1[0]],
                                            row0[x0[1]], row0[x1[1]], row1[x0[1]], row1[x1[1]],
                                            interlaced_x_weights_AB, wy);

        // Once more with the other half of the x-weights for two more pixels C,D.
        __m128i CD = interpolate_in_x_and_y(row0[x0[2]], row0[x1[2]], row1[x0[2]], row1[x1[2]],
        ...
        _mm_storeu_si128((__m128i*)colors,
                         _mm_packus_epi16(scale_by_alpha(AB), scale_by_alpha(CD)));
        xy += 4;
        colors += 4;
        count -= 4;
    }
    ...
    while (count-- > 0) {
        ...
        __m128i A = interpolate_in_x_and_y(row0[x0], row0[x1], row1[x0], row1[x1], 0, 0, 0, 0,
...
/*not static*/ inline void S32_alpha_D32_filter_DX(const SkBitmapProcState& s, const uint32_t* xy,
    ...
    while (count-- > 0) {
        ...
        const __m128i a00 = _mm_cvtsi32_si128(row0[x0]), a01 = _mm_cvtsi32_si128(row0[x1]),
                      a10 = _mm_cvtsi32_si128(row1[x0]), a11 = _mm_cvtsi32_si128(row1[x1]);

        // Line up low-x pixels a00 and a10 with allY.
        __m128i a00a10 = _mm_unpacklo_epi8(_mm_unpacklo_epi32(a10, a00), _mm_setzero_si128());
        ...
        __m128i a01a11 = _mm_unpacklo_epi8(_mm_unpacklo_epi32(a11, a01), _mm_setzero_si128());
...
static void filter_and_scale_by_alpha(unsigned x, unsigned y, SkPMColor a00, SkPMColor a01,
                                      SkPMColor a10, SkPMColor a11, SkPMColor* dst,
                                      uint16_t scale) {
...
#else
static void filter_and_scale_by_alpha(unsigned x, unsigned y, SkPMColor a00, SkPMColor a01,
                                      SkPMColor a10, SkPMColor a11, SkPMColor* dstColor,
                                      unsigned alphaScale) {
