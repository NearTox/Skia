...
STAGE(HLGish, const skcms_TransferFunction* ctx) {
  auto fn = [&](F v) {
    ...
    const float R = ctx->a, G = ctx->b, a = ctx->c, b = ctx->d, c = ctx->e;
...
STAGE(interpreter, SkRasterPipeline_InterpreterCtx* c) {
  ...
  float xx[N], yy[N], rr[N], gg[N], bb[N], aa[N];
  ...
  SkAssertResult(c->byteCode->runStriped(
      c->fn, tail ? tail : N, in_args, in_count, nullptr, 0, (const float*)c->inputs->data(),
      c->ninputs));
...
SI void sample(const SkRasterPipeline_SamplerCtx2* ctx, F x, F y, U16* r, U16* g, U16* b, U16* a) {
  x = tile(x, ctx->tileX, ctx->width, ctx->invWidth);
  y = tile(y, ctx->tileY, ctx->height, ctx->invHeight);
...
template <int D>
SI void sampler(
    const SkRasterPipeline_SamplerCtx2* ctx, F cx, F cy, const F (&wx)[D], const F (&wy)[D], U16* r,
    U16* g, U16* b, U16* a) {
  float start = -0.5f * (D - 1);
  ...
  *r = *g = *b = *a = 0;
  ...
  for (int j = 0; j < D; j++, y += 1.0f) {
    ...
    for (int i = 0; i < D; i++, x += 1.0f) {
      ...
      U16 w = (i == D - 1 && j == D - 1) ? remaining : cast<U16>(wx[i] * wy[j] * bias);
      remaining -= w;
      *r += w * R;
      *g += w * G;
      *b += w * B;
...
STAGE_GP(bilinear, const SkRasterPipeline_SamplerCtx2* ctx) {
  F fx = fract(x + 0.5f), fy = fract(y + 0.5f);
  const F wx[] = {1.0f - fx, fx};
  const F wy[] = {1.0f - fy, fy};
