...
template <typename T, typename P> SI void unaligned_store(P* p, T v) { memcpy(p, &v, sizeof(v)); }

template <typename Dst, typename Src> SI Dst bit_cast(const Src& src) {
    static_assert(sizeof(Dst) == sizeof(Src), "");
    return unaligned_load<Dst>(&src);
}

template <typename Dst, typename Src> SI Dst widen_cast(const Src& src) {
    static_assert(sizeof(Dst) > sizeof(Src), "");
    Dst dst;
...
template <typename T> SI T gather(const T* p, U32 ix) { return p[ix]; }
...
SI U32 round(F v, F scale) { return vcvtq_u32_f32(mad(v, scale, 0.5f)); }
#endif

template <typename T> SI V<T> gather(const T* p, U32 ix) {
...
SI void load4(const uint16_t* ptr, size_t tail, U16* r, U16* g, U16* b, U16* a) {
    ...
    if (__builtin_expect(tail, 0)) {
        ...
        if (tail > 2) {
            rgba = vld4_lane_u16(ptr + 8, rgba, 2);
        }
    } else {
        rgba = vld4_u16(ptr);
    }
...
SI void store4(uint16_t* ptr, size_t tail, U16 r, U16 g, U16 b, U16 a) {
    if (__builtin_expect(tail, 0)) {
        if (true) {
            vst4_lane_u16(ptr + 0, (uint16x4x4_t{{r, g, b, a}}), 0);
        }
        if (tail > 1) {
            vst4_lane_u16(ptr + 4, (uint16x4x4_t{{r, g, b, a}}), 1);
        }
        if (tail > 2) {
            vst4_lane_u16(ptr + 8, (uint16x4x4_t{{r, g, b, a}}), 2);
        }
    } else {
...
SI void load4(const float* ptr, size_t tail, F* r, F* g, F* b, F* a) {
    ...
    if (__builtin_expect(tail, 0)) {
        if (true) {
            rgba = vld4q_lane_f32(ptr + 0, rgba, 0);
        }
        if (tail > 1) {
            rgba = vld4q_lane_f32(ptr + 4, rgba, 1);
        }
        if (tail > 2) {
            rgba = vld4q_lane_f32(ptr + 8, rgba, 2);
        }
    } else {
        rgba = vld4q_f32(ptr);
    }
...
SI void store4(float* ptr, size_t tail, F r, F g, F b, F a) {
    if (__builtin_expect(tail, 0)) {
        if (true) {
            vst4q_lane_f32(ptr + 0, (float32x4x4_t{{r, g, b, a}}), 0);
        }
        if (tail > 1) {
            vst4q_lane_f32(ptr + 4, (float32x4x4_t{{r, g, b, a}}), 1);
        }
        if (tail > 2) {
            vst4q_lane_f32(ptr + 8, (float32x4x4_t{{r, g, b, a}}), 2);
        }
    } else {
...
SI U16 pack(U32 v) {
    return _mm_packus_epi32(_mm256_extractf128_si256(v, 0), _mm256_extractf128_si256(v, 1));
}
SI U8 pack(U16 v) {
...
template <typename T> SI V<T> gather(const T* p, U32 ix) {
    return {
            p[ix[0]], p[ix[1]], p[ix[2]], p[ix[3]], p[ix[4]], p[ix[5]], p[ix[6]], p[ix[7]],
    };
}
#if defined(JUMPER_IS_HSW) || defined(JUMPER_IS_AVX512)
...
SI void load3(const uint16_t* ptr, size_t tail, U16* r, U16* g, U16* b) {
    ...
    if (__builtin_expect(tail, 0)) {
        ...
        _4 = _45;
        _5 = _mm_srli_si128(_45, 6);
        _6 = _67;
        _7 = _mm_srli_si128(_67, 6);
    }

    auto _02 = _mm_unpacklo_epi16(_0, _2),  // r0 r2 g0 g2 b0 b2 xx xx
            _13 = _mm_unpacklo_epi16(_1, _3), _46 = _mm_unpacklo_epi16(_4, _6),
    ...
    auto rg0123 = _mm_unpacklo_epi16(_02, _13),     // r0 r1 r2 r3 g0 g1 g2 g3
            ...
            rg4567 = _mm_unpacklo_epi16(_46, _57), bx4567 = _mm_unpackhi_epi16(_46, _57);
...
SI void load4(const uint16_t* ptr, size_t tail, U16* r, U16* g, U16* b, U16* a) {
    ...
    if (__builtin_expect(tail, 0)) {
        ...
        if (tail > 0) {
            _01 = _mm_loadl_pd(_01, src + 0);
        }
        if (tail > 1) {
            _01 = _mm_loadh_pd(_01, src + 1);
        }
        if (tail > 2) {
            _23 = _mm_loadl_pd(_23, src + 2);
        }
        if (tail > 3) {
            _23 = _mm_loadh_pd(_23, src + 3);
        }
        if (tail > 4) {
            _45 = _mm_loadl_pd(_45, src + 4);
        }
        if (tail > 5) {
            _45 = _mm_loadh_pd(_45, src + 5);
        }
        if (tail > 6) {
            _67 = _mm_loadl_pd(_67, src + 6);
        }
    } else {
    ...
    auto _02 = _mm_unpacklo_epi16(_01, _23),     // r0 r2 g0 g2 b0 b2 a0 a2
            ...
            _46 = _mm_unpacklo_epi16(_45, _67), _57 = _mm_unpackhi_epi16(_45, _67);

    auto rg0123 = _mm_unpacklo_epi16(_02, _13),     // r0 r1 r2 r3 g0 g1 g2 g3
            ba0123 = _mm_unpackhi_epi16(_02, _13),  // b0 b1 b2 b3 a0 a1 a2 a3
            rg4567 = _mm_unpacklo_epi16(_46, _57), ba4567 = _mm_unpackhi_epi16(_46, _57);
...
SI void store4(uint16_t* ptr, size_t tail, U16 r, U16 g, U16 b, U16 a) {
    auto rg0123 = _mm_unpacklo_epi16(r, g),     // r0 g0 r1 g1 r2 g2 r3 g3
            ...
            ba0123 = _mm_unpacklo_epi16(b, a), ba4567 = _mm_unpackhi_epi16(b, a);

    auto _01 = _mm_unpacklo_epi32(rg0123, ba0123), _23 = _mm_unpackhi_epi32(rg0123, ba0123),
         _45 = _mm_unpacklo_epi32(rg4567, ba4567), _67 = _mm_unpackhi_epi32(rg4567, ba4567);

    if (__builtin_expect(tail, 0)) {
        auto dst = (double*)ptr;
        if (tail > 0) {
            _mm_storel_pd(dst + 0, _01);
        }
        if (tail > 1) {
            _mm_storeh_pd(dst + 1, _01);
        }
        if (tail > 2) {
            _mm_storel_pd(dst + 2, _23);
        }
        if (tail > 3) {
            _mm_storeh_pd(dst + 3, _23);
        }
        if (tail > 4) {
            _mm_storel_pd(dst + 4, _45);
        }
        if (tail > 5) {
            _mm_storeh_pd(dst + 5, _45);
        }
        if (tail > 6) {
            _mm_storel_pd(dst + 6, _67);
        }
    } else {
...
SI void load4(const float* ptr, size_t tail, F* r, F* g, F* b, F* a) {
    ...
    switch (tail) {
        ...
        case 4:
            _37 = _mm256_insertf128_ps(_37, _mm_loadu_ps(ptr + 12), 0);
        case 3:
            _26 = _mm256_insertf128_ps(_26, _mm_loadu_ps(ptr + 8), 0);
        case 2:
            _15 = _mm256_insertf128_ps(_15, _mm_loadu_ps(ptr + 4), 0);
        case 1:
            _04 = _mm256_insertf128_ps(_04, _mm_loadu_ps(ptr + 0), 0);
    }

    F rg0145 = _mm256_unpacklo_ps(_04, _15),  // r0 r1 g0 g1 | r4 r5 g4 g5
            ba0145 = _mm256_unpackhi_ps(_04, _15), rg2367 = _mm256_unpacklo_ps(_26, _37),
...
SI void store4(float* ptr, size_t tail, F r, F g, F b, F a) {
    ...
    if (__builtin_expect(tail, 0)) {
        if (tail > 0) {
            _mm_storeu_ps(ptr + 0, _mm256_extractf128_ps(_04, 0));
        }
        if (tail > 1) {
            _mm_storeu_ps(ptr + 4, _mm256_extractf128_ps(_15, 0));
        }
        if (tail > 2) {
            _mm_storeu_ps(ptr + 8, _mm256_extractf128_ps(_26, 0));
        }
        if (tail > 3) {
            _mm_storeu_ps(ptr + 12, _mm256_extractf128_ps(_37, 0));
        }
        if (tail > 4) {
            _mm_storeu_ps(ptr + 16, _mm256_extractf128_ps(_04, 1));
        }
        if (tail > 5) {
            _mm_storeu_ps(ptr + 20, _mm256_extractf128_ps(_15, 1));
        }
        if (tail > 6) {
            _mm_storeu_ps(ptr + 24, _mm256_extractf128_ps(_26, 1));
        }
    } else {
...
SI F if_then_else(I32 c, F t, F e) { return _mm_or_ps(_mm_and_ps(c, t), _mm_andnot_ps(c, e)); }
...
SI void load3(const uint16_t* ptr, size_t tail, U16* r, U16* g, U16* b) {
    ...
    auto R = _mm_unpacklo_epi16(_02, _13),                               // r0 r1 r2 r3 g0 g1 g2 g3
            G = _mm_srli_si128(R, 8), B = _mm_unpackhi_epi16(_02, _13);  // b0 b1 b2 b3 xx xx xx xx
...
SI void load4(const uint16_t* ptr, size_t tail, U16* r, U16* g, U16* b, U16* a) {
    ...
    if (__builtin_expect(tail, 0)) {
        ...
        if (true) {
            _01 = _mm_loadl_pd(_01, src + 0);
        }  // r0 g0 b0 a0 00 00 00 00
        if (tail > 1) {
            _01 = _mm_loadh_pd(_01, src + 1);
        }  // r0 g0 b0 a0 r1 g1 b1 a1
        if (tail > 2) {
            _23 = _mm_loadl_pd(_23, src + 2);
        }  // r2 g2 b2 a2 00 00 00 00
    } else {
...
SI void store4(uint16_t* ptr, size_t tail, U16 r, U16 g, U16 b, U16 a) {
    ...
    if (__builtin_expect(tail, 0)) {
        auto dst = (double*)ptr;
        if (true) {
            _mm_storel_pd(dst + 0, _mm_unpacklo_epi32(rg, ba));
        }
        if (tail > 1) {
            _mm_storeh_pd(dst + 1, _mm_unpacklo_epi32(rg, ba));
        }
        if (tail > 2) {
            _mm_storel_pd(dst + 2, _mm_unpackhi_epi32(rg, ba));
        }
    } else {
...
template <typename V> SI V if_then_else(I32 c, V t, V e) {
...
SI F approx_pow2(F x) {
    ...
    return bit_cast<F>(
            round(1.0f * (1 << 23),
                  x + 121.274057500f - 1.490129070f * f + 27.728023300f / (4.84252568f - f)));
}
...
SI F from_half(U16 h) {
    ...
    U32 sem = expand(h), s = sem & 0x8000, em = sem ^ s;
    ...
    return if_then_else(denorm, F(0), bit_cast<F>((s << 16) + (em << 13) + ((127 - 15) << 23)));
#endif
}
...
SI U16 to_half(F f) {
    ...
    U32 sem = bit_cast<U32>(f), s = sem & 0x80000000, em = sem ^ s;

    // Convert to 1-5-10 half with 15 bias, flushing denorm halfs (including zero) to zero.
    auto denorm = (I32)em < 0x38800000;  // I32 comparison is often quicker, and always safe here.
    return pack(if_then_else(denorm, U32(0), (s >> 16) + (em >> 13) - ((127 - 15) << 10)));
#endif
}
...
#if JUMPER_NARROW_STAGES
#define STAGE(name, ...)                                                                     \
    SI void name##_k(__VA_ARGS__, size_t dx, size_t dy, size_t tail, F& r, F& g, F& b, F& a, \
                     F& dr, F& dg, F& db, F& da);                                            \
    static void ABI name(Params* params, void** program, F r, F g, F b, F a) {               \
        name##_k(Ctx{program}, params->dx, params->dy, params->tail, r, g, b, a, params->dr, \
                 params->dg, params->db, params->da);                                        \
...
#define STAGE(name, ...)                                                                        \
    SI void name##_k(__VA_ARGS__, size_t dx, size_t dy, size_t tail, F& r, F& g, F& b, F& a,    \
                     F& dr, F& dg, F& db, F& da);                                               \
    static void ABI name(size_t tail, void** program, size_t dx, size_t dy, F r, F g, F b, F a, \
                         F dr, F dg, F db, F da) {                                              \
...
template <typename V, typename T> SI V load(const T* src, size_t tail) {
#if !defined(JUMPER_IS_SCALAR)
    ...
    if (__builtin_expect(tail, 0)) {
        ...
        switch (tail) {
            ...
            case 5:
                v[4] = src[4];
            case 4:
                memcpy(&v, src, 4 * sizeof(T));
                break;
            case 3:
                v[2] = src[2];
            case 2:
                memcpy(&v, src, 2 * sizeof(T));
                break;
            case 1:
                memcpy(&v, src, 1 * sizeof(T));
                break;
        }
        return v;
    }
...
template <typename V, typename T> SI void store(T* dst, V v, size_t tail) {
#if !defined(JUMPER_IS_SCALAR)
    __builtin_assume(tail < N);
    if (__builtin_expect(tail, 0)) {
        switch (tail) {
            ...
            case 5:
                dst[4] = v[4];
            case 4:
                memcpy(dst, &v, 4 * sizeof(T));
                break;
            case 3:
                dst[2] = v[2];
            case 2:
                memcpy(dst, &v, 2 * sizeof(T));
                break;
            case 1:
                memcpy(dst, &v, 1 * sizeof(T));
                break;
        }
        return;
    }
...
template <typename T> SI T* ptr_at_xy(const SkRasterPipeline_MemoryCtx* ctx, size_t dx, size_t dy) {
...
template <typename T> SI U32 ix_and_ptr(T** ptr, const SkRasterPipeline_GatherCtx* ctx, F x, F y) {
    x = clamp(x, ctx->width);
...
STAGE(dither, const float* rate) {
    ...
    U32 X = dx + unaligned_load<U32>(iota), Y = dy;
    ...
    U32 M = (Y & 1) << 5 | (X & 1) << 4 | (Y & 2) << 2 | (X & 2) << 1 | (Y & 4) >> 1 | (X & 4) >> 2;
...
STAGE(white_color, Ctx::None) { r = g = b = a = 1.0f; }
...
BLEND_MODE(softlight) {
    F m = if_then_else(da > 0, d / da, 0), s2 = two(s), m4 = two(two(m));
    ...
    F darkSrc = d * (sa + (s2 - sa) * (1.0f - m)),             // Used in case 1.
            ...
            liteSrc =
                    d * sa +
                    da * (s2 - sa) * if_then_else(two(two(d)) <= da, darkDst, liteDst);  // 2 or 3?
...
SI void set_sat(F* r, F* g, F* b, F s) {
    F mn = min(*r, *g, *b), mx = max(*r, *g, *b), sat = mx - mn;

    // Map min channel to 0, max channel to s, and scale the middle proportionally.
    auto scale = [=](F c) { return if_then_else(sat == 0, 0, (c - mn) * s / sat); };
    *r = scale(*r);
    *g = scale(*g);
    *b = scale(*b);
}
...
SI void clip_color(F* r, F* g, F* b, F a) {
    F mn = min(*r, *g, *b), mx = max(*r, *g, *b), l = lum(*r, *g, *b);
...
STAGE(srcover_rgba_8888, const SkRasterPipeline_MemoryCtx* ctx) {
    ...
    dst = to_unorm(r, 1, 255) | to_unorm(g, 1, 255) << 8 | to_unorm(b, 1, 255) << 16 |
          to_unorm(a, 1, 255) << 24;
    store(ptr, dst, tail);
}
...
STAGE(rgb_to_hsl, Ctx::None) {
    F mx = max(r, g, b), mn = min(r, g, b), d = mx - mn, d_rcp = 1.0f / d;
    ...
    F l = (mx + mn) * 0.5f;
    F s = if_then_else(mx == mn, 0, d / if_then_else(l > 0.5f, 2.0f - mx - mn, mx + mn));
...
SI F alpha_coverage_from_rgb_coverage(F a, F da, F cr, F cg, F cb) {
    return if_then_else(a < da, min(cr, cg, cb), max(cr, cg, cb));
}
...
STAGE(emboss, const SkRasterPipeline_EmbossCtx* ctx) {
    ...
    F mul = from_byte(load<U8>(mptr, tail)), add = from_byte(load<U8>(aptr, tail));
...
SI F apply_sign(F x, U32 sign) { return bit_cast<F>(sign | bit_cast<U32>(x)); }
...
STAGE(store_565, const SkRasterPipeline_MemoryCtx* ctx) {
    ...
    U16 px = pack(to_unorm(r, 31) << 11 | to_unorm(g, 63) << 5 | to_unorm(b, 31));
    store(ptr, px, tail);
}
...
STAGE(store_4444, const SkRasterPipeline_MemoryCtx* ctx) {
    auto ptr = ptr_at_xy<uint16_t>(ctx, dx, dy);
    U16 px = pack(to_unorm(r, 15) << 12 | to_unorm(g, 15) << 8 | to_unorm(b, 15) << 4 |
                  to_unorm(a, 15));
    store(ptr, px, tail);
}
...
STAGE(store_8888, const SkRasterPipeline_MemoryCtx* ctx) {
    ...
    U32 px = to_unorm(r, 255) | to_unorm(g, 255) << 8 | to_unorm(b, 255) << 16 |
             to_unorm(a, 255) << 24;
    store(ptr, px, tail);
}
...
STAGE(store_1010102, const SkRasterPipeline_MemoryCtx* ctx) {
    ...
    U32 px = to_unorm(r, 1023) | to_unorm(g, 1023) << 10 | to_unorm(b, 1023) << 20 |
             to_unorm(a, 3) << 30;
    store(ptr, px, tail);
}
...
STAGE(store_f16, const SkRasterPipeline_MemoryCtx* ctx) {
    ...
    store4((uint16_t*)ptr, tail, to_half(r), to_half(g), to_half(b), to_half(a));
}

STAGE(store_u16_be, const SkRasterPipeline_MemoryCtx* ctx) {
    auto ptr = ptr_at_xy<uint16_t>(ctx, 4 * dx, dy);

    U16 R = bswap(pack(to_unorm(r, 65535))), G = bswap(pack(to_unorm(g, 65535))),
        B = bswap(pack(to_unorm(b, 65535))), A = bswap(pack(to_unorm(a, 65535)));
...
STAGE(mirror_x_1, Ctx::None) {
    r = clamp_01(abs_((r - 1.0f) - two(floor_((r - 1.0f) * 0.5f)) - 1.0f));
}
...
STAGE(decal_x_and_y, SkRasterPipeline_DecalTileCtx* ctx) {
    auto w = ctx->limit_x;
    auto h = ctx->limit_y;
    unaligned_store(ctx->mask, cond_to_mask((0 <= r) & (r < w) & (0 <= g) & (g < h)));
}
...
STAGE(matrix_2x3, const float* m) {
    auto R = mad(r, m[0], mad(g, m[2], m[4])), G = mad(r, m[1], mad(g, m[3], m[5]));
    r = R;
    g = G;
}
STAGE(matrix_3x3, const float* m) {
    auto R = mad(r, m[0], mad(g, m[3], b * m[6])), G = mad(r, m[1], mad(g, m[4], b * m[7])),
...
STAGE(matrix_perspective, const float* m) {
    ...
    auto R = mad(r, m[0], mad(g, m[1], m[2])), G = mad(r, m[3], mad(g, m[4], m[5])),
         Z = mad(r, m[6], mad(g, m[7], m[8]));
    r = R * rcp(Z);
    g = G * rcp(Z);
}

SI void gradient_lookup(const SkRasterPipeline_GradientCtx* c, U32 idx, F t, F* r, F* g, F* b,
                        F* a) {
...
STAGE(xy_to_unit_angle, Ctx::None) {
    ...
    F phi = slope *
            (0.15912117063999176025390625f + s * (-5.185396969318389892578125e-2f +
...
STAGE(save_xy, SkRasterPipeline_SamplerCtx* c) {
    ...
    F fx = fract(r + 0.5f), fy = fract(g + 0.5f);
...
STAGE(accumulate, const SkRasterPipeline_SamplerCtx* c) {
    ...
    auto scale = unaligned_load<F>(c->scalex) * unaligned_load<F>(c->scaley);
    dr = mad(scale, r, dr);
    dg = mad(scale, g, dg);
    db = mad(scale, b, db);
    da = mad(scale, a, da);
...
template <int kScale> SI void bilinear_x(SkRasterPipeline_SamplerCtx* ctx, F* x) {
...
template <int kScale> SI void bilinear_y(SkRasterPipeline_SamplerCtx* ctx, F* y) {
...
template <int kScale> SI void bicubic_x(SkRasterPipeline_SamplerCtx* ctx, F* x) {
    *x = unaligned_load<F>(ctx->x) + (kScale * 0.5f);
    F fx = unaligned_load<F>(ctx->fx);

    F scalex;
    if (kScale == -3) {
        scalex = bicubic_far(1.0f - fx);
    }
    if (kScale == -1) {
        scalex = bicubic_near(1.0f - fx);
    }
    if (kScale == +1) {
        scalex = bicubic_near(fx);
    }
    if (kScale == +3) {
        scalex = bicubic_far(fx);
    }
    unaligned_store(ctx->scalex, scalex);
}
template <int kScale> SI void bicubic_y(SkRasterPipeline_SamplerCtx* ctx, F* y) {
    *y = unaligned_load<F>(ctx->y) + (kScale * 0.5f);
    F fy = unaligned_load<F>(ctx->fy);

    F scaley;
    if (kScale == -3) {
        scaley = bicubic_far(1.0f - fy);
    }
    if (kScale == -1) {
        scaley = bicubic_near(1.0f - fy);
    }
    if (kScale == +1) {
        scaley = bicubic_near(fy);
    }
    if (kScale == +3) {
        scaley = bicubic_far(fy);
    }
    unaligned_store(ctx->scaley, scaley);
}
...
STAGE(bilerp_clamp_8888, const SkRasterPipeline_GatherCtx* ctx) {
    ...
    F fx = fract(cx + 0.5f), fy = fract(cy + 0.5f);
    ...
    for (float dy = -0.5f; dy <= +0.5f; dy += 1.0f)
        for (float dx = -0.5f; dx <= +0.5f; dx += 1.0f) {
            ...
            // In bilinear interpolation, the 4 pixels at +/- 0.5 offsets from the sample pixel
            // center are combined in direct proportion to their area overlapping that logical query
            // pixel. At positive offsets, the x-axis contribution to that rectangle is fx, or
            // (1-fx) at negative x.  Same deal for y.
            F sx = (dx > 0) ? fx : 1.0f - fx, sy = (dy > 0) ? fy : 1.0f - fy, area = sx * sy;
...
using Stage = void(ABI*)(size_t tail, void** program, size_t dx, size_t dy, U16 r, U16 g, U16 b,
                         U16 a, U16 dr, U16 dg, U16 db, U16 da);
#endif

static void start_pipeline(const size_t x0, const size_t y0, const size_t xlimit,
                           const size_t ylimit, void** program) {
    auto start = (Stage)load_and_inc(program);
...
#else
static void ABI just_return(size_t, void**, size_t, size_t, U16, U16, U16, U16, U16, U16, U16,
                            U16) {}
#endif
...
#define STAGE_GG(name, ...)                                                            \
    ...
    static void ABI name(Params* params, void** program, U16 r, U16 g, U16 b, U16 a) { \
        auto x = join<F>(r, g), y = join<F>(b, a);                                     \
...
#define STAGE_GP(name, ...)                                                                        \
    SI void name##_k(__VA_ARGS__, size_t dx, size_t dy, size_t tail, F x, F y, U16& r, U16& g,     \
                     U16& b, U16& a, U16& dr, U16& dg, U16& db, U16& da);                          \
    static void ABI name(Params* params, void** program, U16 r, U16 g, U16 b, U16 a) {             \
        auto x = join<F>(r, g), y = join<F>(b, a);                                                 \
        name##_k(Ctx{program}, params->dx, params->dy, params->tail, x, y, r, g, b, a, params->dr, \
                 params->dg, params->db, params->da);                                              \
        auto next = (Stage)load_and_inc(program);                                                  \
        next(params, program, r, g, b, a);                                                         \
    }                                                                                              \
...
#define STAGE_PP(name, ...)                                                                  \
    SI void name##_k(__VA_ARGS__, size_t dx, size_t dy, size_t tail, U16& r, U16& g, U16& b, \
                     U16& a, U16& dr, U16& dg, U16& db, U16& da);                            \
    static void ABI name(Params* params, void** program, U16 r, U16 g, U16 b, U16 a) {       \
        name##_k(Ctx{program}, params->dx, params->dy, params->tail, r, g, b, a, params->dr, \
                 params->dg, params->db, params->da);                                        \
...
#define STAGE_GG(name, ...)                                                                      \
    SI void name##_k(__VA_ARGS__, size_t dx, size_t dy, size_t tail, F& x, F& y);                \
    static void ABI name(size_t tail, void** program, size_t dx, size_t dy, U16 r, U16 g, U16 b, \
                         U16 a, U16 dr, U16 dg, U16 db, U16 da) {                                \
        auto x = join<F>(r, g), y = join<F>(b, a);                                               \
...
template <typename D, typename S> SI D cast(S src) { return __builtin_convertvector(src, D); }

template <typename D, typename S> SI void split(S v, D* lo, D* hi) {
    static_assert(2 * sizeof(D) == sizeof(S), "");
    memcpy(lo, (const char*)&v + 0 * sizeof(D), sizeof(D));
    memcpy(hi, (const char*)&v + 1 * sizeof(D), sizeof(D));
}
template <typename D, typename S> SI D join(S lo, S hi) {
...
STAGE_GG(matrix_2x3, const float* m) {
    auto X = mad(x, m[0], mad(y, m[2], m[4])), Y = mad(x, m[1], mad(y, m[3], m[5]));
    x = X;
    y = Y;
}
STAGE_GG(matrix_perspective, const float* m) {
    // N.B. Unlike the other matrix_ stages, this matrix is row-major.
    auto X = mad(x, m[0], mad(y, m[1], m[2])), Y = mad(x, m[3], mad(y, m[4], m[5])),
...
STAGE_PP(black_color, Ctx::None) {
    r = g = b = 0;
    a = 255;
}
STAGE_PP(white_color, Ctx::None) {
    r = g = b = 255;
    a = 255;
}
...
template <typename V, typename T> SI V load(const T* ptr, size_t tail) {
    V v = 0;
    switch (tail & (N - 1)) {
        case 0:
...
template <typename V, typename T> SI void store(T* ptr, size_t tail, V v) {
    switch (tail & (N - 1)) {
        case 0:
        ...
        case 5:
            ptr[4] = v[4];
        case 4:
            memcpy(ptr, &v, 4 * sizeof(T));
            break;
        case 3:
            ptr[2] = v[2];
        case 2:
            memcpy(ptr, &v, 2 * sizeof(T));
            break;
        case 1:
            ptr[0] = v[0];
    }
...
#if defined(JUMPER_IS_HSW) || defined(JUMPER_IS_AVX512)
template <typename V, typename T> SI V gather(const T* ptr, U32 ix) {
    return V{
            ptr[ix[0]],  ptr[ix[1]],  ptr[ix[2]],  ptr[ix[3]],  ptr[ix[4]],  ptr[ix[5]],
            ptr[ix[6]],  ptr[ix[7]],  ptr[ix[8]],  ptr[ix[9]],  ptr[ix[10]], ptr[ix[11]],
            ptr[ix[12]], ptr[ix[13]], ptr[ix[14]], ptr[ix[15]],
    };
}

template <> F gather(const float* ptr, U32 ix) {
    __m256i lo, hi;
    split(ix, &lo, &hi);

    return join<F>(_mm256_i32gather_ps(ptr, lo, 4), _mm256_i32gather_ps(ptr, hi, 4));
}

template <> U32 gather(const uint32_t* ptr, U32 ix) {
    __m256i lo, hi;
    split(ix, &lo, &hi);

    return join<U32>(_mm256_i32gather_epi32(ptr, lo, 4), _mm256_i32gather_epi32(ptr, hi, 4));
}
#else
template <typename V, typename T> SI V gather(const T* ptr, U32 ix) {
    return V{
...
#else
    auto cast_U16 = [](U32 v) -> U16 { return cast<U16>(v); };
#endif
...
SI void load_8888_(const uint32_t* ptr, size_t tail, U16* r, U16* g, U16* b, U16* a) {
    ...
    switch (tail & (N - 1)) {
        case 0:
            rgba = vld4_u8((const uint8_t*)(ptr + 0));
            break;
        case 7:
            rgba = vld4_lane_u8((const uint8_t*)(ptr + 6), rgba, 6);
        case 6:
            rgba = vld4_lane_u8((const uint8_t*)(ptr + 5), rgba, 5);
        case 5:
            rgba = vld4_lane_u8((const uint8_t*)(ptr + 4), rgba, 4);
        case 4:
            rgba = vld4_lane_u8((const uint8_t*)(ptr + 3), rgba, 3);
        case 3:
            rgba = vld4_lane_u8((const uint8_t*)(ptr + 2), rgba, 2);
        case 2:
            rgba = vld4_lane_u8((const uint8_t*)(ptr + 1), rgba, 1);
        case 1:
            rgba = vld4_lane_u8((const uint8_t*)(ptr + 0), rgba, 0);
    }
...
SI void store_8888_(uint32_t* ptr, size_t tail, U16 r, U16 g, U16 b, U16 a) {
    ...
    switch (tail & (N - 1)) {
        case 0:
            vst4_u8((uint8_t*)(ptr + 0), rgba);
            break;
        case 7:
            vst4_lane_u8((uint8_t*)(ptr + 6), rgba, 6);
        case 6:
            vst4_lane_u8((uint8_t*)(ptr + 5), rgba, 5);
        case 5:
            vst4_lane_u8((uint8_t*)(ptr + 4), rgba, 4);
        case 4:
            vst4_lane_u8((uint8_t*)(ptr + 3), rgba, 3);
        case 3:
            vst4_lane_u8((uint8_t*)(ptr + 2), rgba, 2);
        case 2:
            vst4_lane_u8((uint8_t*)(ptr + 1), rgba, 1);
        case 1:
            vst4_lane_u8((uint8_t*)(ptr + 0), rgba, 0);
    }
#else
    store(ptr, tail, cast<U32>(r | (g << 8)) << 0 | cast<U32>(b | (a << 8)) << 16);
#endif
}
...
SI void from_565(U16 rgb, U16* r, U16* g, U16* b) {
    ...
    U16 R = (rgb >> 11) & 31, G = (rgb >> 5) & 63, B = (rgb >> 0) & 31;
...
SI void from_4444(U16 rgba, U16* r, U16* g, U16* b, U16* a) {
    ...
    U16 R = (rgba >> 12) & 15, G = (rgba >> 8) & 15, B = (rgba >> 4) & 15, A = (rgba >> 0) & 15;
...
SI void store_4444_(uint16_t* ptr, size_t tail, U16 r, U16 g, U16 b, U16 a) {
    ...
    U16 R = (r + 8) / 17, G = (g + 8) / 17, B = (b + 8) / 17, A = (a + 8) / 17;
    // Pack them back into 15|rrrr gggg bbbb aaaa|0.
    store(ptr, tail, R << 12 | G << 8 | B << 4 | A << 0);
}
...
SI U16 load_8(const uint8_t* ptr, size_t tail) { return cast<U16>(load<U8>(ptr, tail)); }
SI void store_8(uint8_t* ptr, size_t tail, U16 v) { store(ptr, tail, cast<U8>(v)); }
...
SI void round_F_to_U16(F R, F G, F B, F A, bool interpolatedInPremul, U16* r, U16* g, U16* b,
                       U16* a) {
    auto round = [](F x) { return cast<U16>(x * 255.0f + 0.5f); };

    F limit = interpolatedInPremul ? A : 1;
...
SI void gradient_lookup(const SkRasterPipeline_GradientCtx* c, U32 idx, F t, U16* r, U16* g, U16* b,
                        U16* a) {
    ...
    round_F_to_U16(mad(t, fr, br), mad(t, fg, bg), mad(t, fb, bb), mad(t, fa, ba),
                   c->interpolatedInPremul, r, g, b, a);
}
...
STAGE_GP(evenly_spaced_2_stop_gradient, const SkRasterPipeline_EvenlySpaced2StopGradientCtx* c) {
    auto t = x;
    round_F_to_U16(mad(t, c->f[0], c->b[0]), mad(t, c->f[1], c->b[1]), mad(t, c->f[2], c->b[2]),
                   mad(t, c->f[3], c->b[3]), c->interpolatedInPremul, &r, &g, &b, &a);
}
...
STAGE_GG(xy_to_radius, Ctx::None) { x = sqrt_(x * x + y * y); }
...
STAGE_GP(bilerp_clamp_8888, const SkRasterPipeline_GatherCtx* ctx) {
    ...
    for (float dy = -0.5f; dy <= +0.5f; dy += 1.0f)
        for (float dx = -0.5f; dx <= +0.5f; dx += 1.0f) {
            ...
            // pixel. At positive offsets, the x-axis contribution to that rectangle is fx, or
            // (1-fx) at negative x.  Same deal for y.
            F sx = (dx > 0) ? fx : 1.0f - fx, sy = (dy > 0) ? fy : 1.0f - fy;

            U16 area = (dy == 0.5f && dx == 0.5f) ? remaining : cast<U16>(sx * sy * bias);
