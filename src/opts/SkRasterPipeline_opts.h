...
template <typename T>
using V = T __attribute__((ext_vector_type(4)));
using F = V<float>;
using I32 = V<int32_t>;
...
SI void load2(const uint16_t* ptr, size_t tail, U16* r, U16* g) {
  ...
  if (__builtin_expect(tail, 0)) {
    if (true) {
      rg = vld2_lane_u16(ptr + 0, rg, 0);
    }
    if (tail > 1) {
      rg = vld2_lane_u16(ptr + 2, rg, 1);
    }
    if (tail > 2) {
      rg = vld2_lane_u16(ptr + 4, rg, 2);
    }
  } else {
    rg = vld2_u16(ptr);
  }
...
SI void store2(uint16_t* ptr, size_t tail, U16 r, U16 g) {
  if (__builtin_expect(tail, 0)) {
    if (true) {
      vst2_lane_u16(ptr + 0, (uint16x4x2_t{{r, g}}), 0);
    }
    if (tail > 1) {
      vst2_lane_u16(ptr + 2, (uint16x4x2_t{{r, g}}), 1);
    }
    if (tail > 2) {
      vst2_lane_u16(ptr + 4, (uint16x4x2_t{{r, g}}), 2);
    }
  } else {
...
SI void load2(const float* ptr, size_t tail, F* r, F* g) {
  ...
  if (__builtin_expect(tail, 0)) {
    if (true) {
      rg = vld2q_lane_f32(ptr + 0, rg, 0);
    }
    if (tail > 1) {
      rg = vld2q_lane_f32(ptr + 2, rg, 1);
    }
    if (tail > 2) {
      rg = vld2q_lane_f32(ptr + 4, rg, 2);
    }
  } else {
    rg = vld2q_f32(ptr);
  }
...
SI void store2(float* ptr, size_t tail, F r, F g) {
  if (__builtin_expect(tail, 0)) {
    if (true) {
      vst2q_lane_f32(ptr + 0, (float32x4x2_t{{r, g}}), 0);
    }
    if (tail > 1) {
      vst2q_lane_f32(ptr + 2, (float32x4x2_t{{r, g}}), 1);
    }
    if (tail > 2) {
      vst2q_lane_f32(ptr + 4, (float32x4x2_t{{r, g}}), 2);
    }
  } else {
...
template <typename T>
using V = T __attribute__((ext_vector_type(8)));
using F = V<float>;
using I32 = V<int32_t>;
using U64 = V<uint64_t>;
using U32 = V<uint32_t>;
...
SI void load2(const uint16_t* ptr, size_t tail, U16* r, U16* g) {
  ...
  *r = _mm_packs_epi32(
      _mm_srai_epi32(_mm_slli_epi32(_0123, 16), 16), _mm_srai_epi32(_mm_slli_epi32(_4567, 16), 16));
  *g = _mm_packs_epi32(_mm_srai_epi32(_0123, 16), _mm_srai_epi32(_4567, 16));
}
SI void store2(uint16_t* ptr, size_t tail, U16 r, U16 g) {
  auto _0123 = _mm_unpacklo_epi16(r, g), _4567 = _mm_unpackhi_epi16(r, g);
  if (__builtin_expect(tail, 0)) {
    const auto* s = &_0123;
    if (tail > 3) {
...
SI void store2(float* ptr, size_t tail, F r, F g) {
  F _0145 = _mm256_unpacklo_ps(r, g), _2367 = _mm256_unpackhi_ps(r, g);
...
SI void load2(const float* ptr, size_t tail, F* r, F* g) {
  ...
  if (__builtin_expect(tail, 0)) {
    _01 = _23 = _mm_setzero_si128();
    if (true) {
      _01 = _mm_loadl_pi(_01, (__m64 const*)(ptr + 0));
    }
    if (tail > 1) {
      _01 = _mm_loadh_pi(_01, (__m64 const*)(ptr + 2));
    }
    if (tail > 2) {
      _23 = _mm_loadl_pi(_23, (__m64 const*)(ptr + 4));
    }
  } else {
...
SI void store2(float* ptr, size_t tail, F r, F g) {
  F _01 = _mm_unpacklo_ps(r, g), _23 = _mm_unpackhi_ps(r, g);
  if (__builtin_expect(tail, 0)) {
    if (true) {
      _mm_storel_pi((__m64*)(ptr + 0), _01);
    }
    if (tail > 1) {
      _mm_storeh_pi((__m64*)(ptr + 2), _01);
    }
    if (tail > 2) {
      _mm_storel_pi((__m64*)(ptr + 4), _23);
    }
  } else {
...
SI F approx_pow2(F x) {
  F f = fract(x);
  return sk_bit_cast<F>(round(
      1.0f * (1 << 23), x + 121.274057500f - 1.490129070f * f + 27.728023300f / (4.84252568f - f)));
}
...
SI U16 to_half(F f) {
  ...
  U32 sem = sk_bit_cast<U32>(f), s = sem & 0x80000000, em = sem ^ s;
...
#  define STAGE(name, ...)                                                                        \
    SI void name##_k(                                                                             \
        __VA_ARGS__, size_t dx, size_t dy, size_t tail, F& r, F& g, F& b, F& a, F& dr, F& dg,     \
        F& db, F& da);                                                                            \
    static void ABI name(Params* params, void** program, F r, F g, F b, F a) {                    \
      name##_k(                                                                                   \
          Ctx{program}, params->dx, params->dy, params->tail, r, g, b, a, params->dr, params->dg, \
          params->db, params->da);                                                                \
...
#  define STAGE(name, ...)                                                                       \
    SI void name##_k(                                                                            \
        __VA_ARGS__, size_t dx, size_t dy, size_t tail, F& r, F& g, F& b, F& a, F& dr, F& dg,    \
        F& db, F& da);                                                                           \
    static void ABI name(                                                                        \
        size_t tail, void** program, size_t dx, size_t dy, F r, F g, F b, F a, F dr, F dg, F db, \
        F da) {                                                                                  \
...
STAGE(dither, const float* rate) {
  ...
  U32 X = dx + sk_unaligned_load<U32>(iota), Y = dy;
...
STAGE(store_src_a, float* ptr) { sk_unaligned_store(ptr, a); }
...
#endif
      liteSrc =
          d * sa + da * (s2 - sa) * if_then_else(two(two(d)) <= da, darkDst, liteDst);  // 2 or 3?
...
SI void set_sat(F* r, F* g, F* b, F s) {
  F mn = min(*r, min(*g, *b)), mx = max(*r, max(*g, *b)), sat = mx - mn;
...
SI void clip_color(F* r, F* g, F* b, F a) {
  F mn = min(*r, min(*g, *b)), mx = max(*r, max(*g, *b)), l = lum(*r, *g, *b);
...
STAGE(rgb_to_hsl, Ctx::None) {
  ...
  F h =
      (1 / 6.0f) * if_then_else(
                       mx == mn, 0,
                       if_then_else(
                           mx == r, (g - b) * d_rcp + if_then_else(g < b, 6.0f, 0),
                           if_then_else(mx == g, (b - r) * d_rcp + 2.0f, (r - g) * d_rcp + 4.0f)));
...
SI F alpha_coverage_from_rgb_coverage(F a, F da, F cr, F cg, F cb) {
  return if_then_else(a < da, min(cr, min(cg, cb)), max(cr, max(cg, cb)));
}
...
STAGE(parametric, const skcms_TransferFunction* ctx) {
  auto fn = [&](F v) {
    ...
    F r = if_then_else(
        v <= ctx->d, mad(ctx->c, v, ctx->f), approx_powf(mad(ctx->a, v, ctx->b), ctx->g) + ctx->e);
...
STAGE(PQish, const skcms_TransferFunction* ctx) {
  auto fn = [&](F v) {
    ...
    F r = approx_powf(
        max(mad(ctx->b, approx_powf(v, ctx->c), ctx->a), 0) /
...
STAGE(HLGish, const skcms_TransferFunction* ctx) {
  auto fn = [&](F v) {
    ...
    const float R = ctx->a, G = ctx->b, a = ctx->c, b = ctx->d, c = ctx->e, K = ctx->f + 1.0f;

    F r = if_then_else(v * R <= 1, approx_powf(v * R, G), approx_exp((v - c) * a) + b);
...
STAGE(store_4444, const SkRasterPipeline_MemoryCtx* ctx) {
  auto ptr = ptr_at_xy<uint16_t>(ctx, dx, dy);
  U16 px =
      pack(to_unorm(r, 15) << 12 | to_unorm(g, 15) << 8 | to_unorm(b, 15) << 4 | to_unorm(a, 15));
  store(ptr, px, tail);
}
...
STAGE(store_8888, const SkRasterPipeline_MemoryCtx* ctx) {
  ...
  U32 px =
      to_unorm(r, 255) | to_unorm(g, 255) << 8 | to_unorm(b, 255) << 16 | to_unorm(a, 255) << 24;
  store(ptr, px, tail);
}
...
STAGE(store_rg1616, const SkRasterPipeline_MemoryCtx* ctx) {
  ...
  U32 px = to_unorm(r, 65535) | to_unorm(g, 65535) << 16;
  store(ptr, px, tail);
}
...
STAGE(store_16161616, const SkRasterPipeline_MemoryCtx* ctx) {
  ...
  U16 R = pack(to_unorm(r, 65535)), G = pack(to_unorm(g, 65535)), B = pack(to_unorm(b, 65535)),
...
STAGE(store_1010102, const SkRasterPipeline_MemoryCtx* ctx) {
  ...
  U32 px =
      to_unorm(r, 1023) | to_unorm(g, 1023) << 10 | to_unorm(b, 1023) << 20 | to_unorm(a, 3) << 30;
  store(ptr, px, tail);
}
...
STAGE(store_rgf16, const SkRasterPipeline_MemoryCtx* ctx) {
  auto ptr = ptr_at_xy<uint32_t>(ctx, dx, dy);
  store2((uint16_t*)ptr, tail, to_half(r), to_half(g));
}
...
STAGE(decal_x_and_y, SkRasterPipeline_DecalTileCtx* ctx) {
  auto w = ctx->limit_x;
  auto h = ctx->limit_y;
  sk_unaligned_store(ctx->mask, cond_to_mask((0 <= r) & (r < w) & (0 <= g) & (g < h)));
}
...
SI void gradient_lookup(
    const SkRasterPipeline_GradientCtx* c, U32 idx, F t, F* r, F* g, F* b, F* a) {
...
STAGE(xy_to_unit_angle, Ctx::None) {
  ...
  F phi = slope *
          (0.15912117063999176025390625f +
           s * (-5.185396969318389892578125e-2f + s * (2.476101927459239959716796875e-2f +
...
STAGE(accumulate, const SkRasterPipeline_SamplerCtx* c) {
  ...
  auto scale = sk_unaligned_load<F>(c->scalex) * sk_unaligned_load<F>(c->scaley);
  dr = mad(scale, r, dr);
  dg = mad(scale, g, dg);
  db = mad(scale, b, db);
  da = mad(scale, a, da);
...
SI void bicubic_x(SkRasterPipeline_SamplerCtx* ctx, F* x) {
  *x = sk_unaligned_load<F>(ctx->x) + (kScale * 0.5f);
  ...
  if (kScale == -3) {
    scalex = bicubic_wts(fx, w[0], w[4], w[8], w[12]);
  }
  if (kScale == -1) {
    scalex = bicubic_wts(fx, w[1], w[5], w[9], w[13]);
  }
  if (kScale == +1) {
    scalex = bicubic_wts(fx, w[2], w[6], w[10], w[14]);
  }
  if (kScale == +3) {
    scalex = bicubic_wts(fx, w[3], w[7], w[11], w[15]);
  }
  sk_unaligned_store(ctx->scalex, scalex);
}
template <int kScale>
SI void bicubic_y(SkRasterPipeline_SamplerCtx* ctx, F* y) {
  *y = sk_unaligned_load<F>(ctx->y) + (kScale * 0.5f);
  ...
  const float* w = ctx->weights;
  if (kScale == -3) {
    scaley = bicubic_wts(fy, w[0], w[4], w[8], w[12]);
  }
  if (kScale == -1) {
    scaley = bicubic_wts(fy, w[1], w[5], w[9], w[13]);
  }
  if (kScale == +1) {
    scaley = bicubic_wts(fy, w[2], w[6], w[10], w[14]);
  }
  if (kScale == +3) {
    scaley = bicubic_wts(fy, w[3], w[7], w[11], w[15]);
  }
  sk_unaligned_store(ctx->scaley, scaley);
}
...
SI void sample(const SkRasterPipeline_SamplerCtx2* ctx, F x, F y, F* r, F* g, F* b, F* a) {
...
template <int D>
SI void sampler(
    const SkRasterPipeline_SamplerCtx2* ctx, F cx, F cy, const F (&wx)[D], const F (&wy)[D], F* r,
    F* g, F* b, F* a) {
...
STAGE(bilinear, const SkRasterPipeline_SamplerCtx2* ctx) {
  F x = r, fx = fract(x + 0.5f), y = g, fy = fract(y + 0.5f);
...
STAGE(bicubic, SkRasterPipeline_SamplerCtx2* ctx) {
  ...
  const float* w = ctx->weights;
  const F wx[] = {
      bicubic_wts(fx, w[0], w[4], w[8], w[12]), bicubic_wts(fx, w[1], w[5], w[9], w[13]),
      bicubic_wts(fx, w[2], w[6], w[10], w[14]), bicubic_wts(fx, w[3], w[7], w[11], w[15])};
  const F wy[] = {
      bicubic_wts(fy, w[0], w[4], w[8], w[12]), bicubic_wts(fy, w[1], w[5], w[9], w[13]),
      bicubic_wts(fy, w[2], w[6], w[10], w[14]), bicubic_wts(fy, w[3], w[7], w[11], w[15])};
...
STAGE(bilerp_clamp_8888, const SkRasterPipeline_GatherCtx* ctx) {
  ...
  for (float py = -0.5f; py <= +0.5f; py += 1.0f)
    for (float px = -0.5f; px <= +0.5f; px += 1.0f) {
      ...
      F sx = (px > 0) ? fx : 1.0f - fx, sy = (py > 0) ? fy : 1.0f - fy, area = sx * sy;
...
using Stage = void(ABI*)(
    size_t tail, void** program, size_t dx, size_t dy, U16 r, U16 g, U16 b, U16 a, U16 dr, U16 dg,
    U16 db, U16 da);
#  endif

static void start_pipeline(
    const size_t x0, const size_t y0, const size_t xlimit, const size_t ylimit, void** program) {
  auto start = (Stage)load_and_inc(program);
...
#  else
static void ABI
just_return(size_t, void**, size_t, size_t, U16, U16, U16, U16, U16, U16, U16, U16) {}
#  endif
...
#    define STAGE_GP(name, ...)                                                               \
      SI void name##_k(                                                                       \
          __VA_ARGS__, size_t dx, size_t dy, size_t tail, F x, F y, U16& r, U16& g, U16& b,   \
          U16& a, U16& dr, U16& dg, U16& db, U16& da);                                        \
      static void ABI name(Params* params, void** program, U16 r, U16 g, U16 b, U16 a) {      \
        auto x = join<F>(r, g), y = join<F>(b, a);                                            \
        name##_k(                                                                             \
            Ctx{program}, params->dx, params->dy, params->tail, x, y, r, g, b, a, params->dr, \
...
#    define STAGE_PP(name, ...)                                                                    \
      SI void name##_k(                                                                            \
          __VA_ARGS__, size_t dx, size_t dy, size_t tail, U16& r, U16& g, U16& b, U16& a, U16& dr, \
          U16& dg, U16& db, U16& da);                                                              \
      static void ABI name(Params* params, void** program, U16 r, U16 g, U16 b, U16 a) {           \
        name##_k(                                                                                  \
            Ctx{program}, params->dx, params->dy, params->tail, r, g, b, a, params->dr,            \
            params->dg, params->db, params->da);                                                   \
        auto next = (Stage)load_and_inc(program);                                                  \
        next(params, program, r, g, b, a);                                                         \
      }                                                                                            \
...
#    define STAGE_GG(name, ...)                                                                  \
      SI void name##_k(__VA_ARGS__, size_t dx, size_t dy, size_t tail, F& x, F& y);              \
      static void ABI name(                                                                      \
          size_t tail, void** program, size_t dx, size_t dy, U16 r, U16 g, U16 b, U16 a, U16 dr, \
          U16 dg, U16 db, U16 da) {                                                              \
        auto x = join<F>(r, g), y = join<F>(b, a);                                               \
...
SI I32 if_then_else(I32 c, I32 t, I32 e) { return (t & c) | (e & ~c); }
...
SI U32 ix_and_ptr(T** ptr, const SkRasterPipeline_GatherCtx* ctx, I32 x, I32 y) {
  // Exclusive -> inclusive.
  const I32 w = ctx->width - 1, h = ctx->height - 1;

  U32 ax = cast<U32>(min(max(0, x), w)), ay = cast<U32>(min(max(0, y), h));
...
STAGE_PP(store_src_a, uint16_t* ptr) { sk_unaligned_store(ptr, a); }
...
SI void round_F_to_U16(
    F R, F G, F B, F A, bool interpolatedInPremul, U16* r, U16* g, U16* b, U16* a) {
...
SI void gradient_lookup(
    const SkRasterPipeline_GradientCtx* c, U32 idx, F t, U16* r, U16* g, U16* b, U16* a) {
...
#  if defined(JUMPER_IS_HSW) || defined(JUMPER_IS_SKX)
  if (c->stopCount <= 8) {
    ...
    fr = join<F>(
        _mm256_permutevar8x32_ps(_mm256_loadu_ps(c->fs[0]), lo),
        _mm256_permutevar8x32_ps(_mm256_loadu_ps(c->fs[0]), hi));
    br = join<F>(
        _mm256_permutevar8x32_ps(_mm256_loadu_ps(c->bs[0]), lo),
        _mm256_permutevar8x32_ps(_mm256_loadu_ps(c->bs[0]), hi));
    fg = join<F>(
        _mm256_permutevar8x32_ps(_mm256_loadu_ps(c->fs[1]), lo),
        _mm256_permutevar8x32_ps(_mm256_loadu_ps(c->fs[1]), hi));
    bg = join<F>(
        _mm256_permutevar8x32_ps(_mm256_loadu_ps(c->bs[1]), lo),
        _mm256_permutevar8x32_ps(_mm256_loadu_ps(c->bs[1]), hi));
    fb = join<F>(
        _mm256_permutevar8x32_ps(_mm256_loadu_ps(c->fs[2]), lo),
        _mm256_permutevar8x32_ps(_mm256_loadu_ps(c->fs[2]), hi));
    bb = join<F>(
        _mm256_permutevar8x32_ps(_mm256_loadu_ps(c->bs[2]), lo),
        _mm256_permutevar8x32_ps(_mm256_loadu_ps(c->bs[2]), hi));
    fa = join<F>(
        _mm256_permutevar8x32_ps(_mm256_loadu_ps(c->fs[3]), lo),
        _mm256_permutevar8x32_ps(_mm256_loadu_ps(c->fs[3]), hi));
    ba = join<F>(
        _mm256_permutevar8x32_ps(_mm256_loadu_ps(c->bs[3]), lo),
...
#  endif
  ...
  round_F_to_U16(
      mad(t, fr, br), mad(t, fg, bg), mad(t, fb, bb), mad(t, fa, ba), c->interpolatedInPremul, r, g,
      b, a);
}
...
STAGE_GP(evenly_spaced_2_stop_gradient, const SkRasterPipeline_EvenlySpaced2StopGradientCtx* c) {
  auto t = x;
  round_F_to_U16(
      mad(t, c->f[0], c->b[0]), mad(t, c->f[1], c->b[1]), mad(t, c->f[2], c->b[2]),
...
STAGE_GP(bilerp_clamp_8888, const SkRasterPipeline_GatherCtx* ctx) {
  ...
  I16 tx = cast<I16>(qx ^ 0x8000), ty = cast<I16>(qy ^ 0x8000);
  ...
  U16 topR = lerpX(leftR, rightR), topG = lerpX(leftG, rightG), topB = lerpX(leftB, rightB),
  ...
  U16 bottomR = lerpX(leftR, rightR), bottomG = lerpX(leftG, rightG),
      bottomB = lerpX(leftB, rightB), bottomA = lerpX(leftA, rightA);
