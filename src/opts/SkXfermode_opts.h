...
#  define XFERMODE_AA(Xfermode) \
    template <>                 \
    inline Sk4px xfer_aa<Xfermode>(const Sk4px& d, const Sk4px& s, const Sk4px& aa)
...
template <>
inline void mark_dst_initialized_if_safe<Src>(void* dst, void* end) {
  sk_msan_mark_initialized(dst, end, "Src doesn't read dst.");
}
template <>
inline void mark_dst_initialized_if_safe<Clear>(void* dst, void* end) {
