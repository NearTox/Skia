...
sk_sp<SkImage> SkImage::MakeFromYUVATextures(
  ...
  if (!SkImage_GpuBase::MakeTempTextureProxies(
          ...
          yuvaTextures.textureOrigin(), tempViews, std::move(releaseHelper))) {
    return nullptr;
  }

  return sk_make_sp<SkImage_GpuYUVA>(
      sk_ref_sp(context), yuvaTextures.yuvaInfo().dimensions(), kNeedNewImageUniqueID,
      yuvaTextures.yuvaInfo().yuvColorSpace(), tempViews, numTextures, yuvaIndices,
      imageColorSpace);
}

sk_sp<SkImage> SkImage::MakeFromYUVATextures(
    GrRecordingContext* ctx, SkYUVColorSpace colorSpace, const GrBackendTexture yuvaTextures[],
    const SkYUVAIndex yuvaIndices[4], SkISize imageSize, GrSurfaceOrigin textureOrigin,
    sk_sp<SkColorSpace> imageColorSpace, TextureReleaseProc textureReleaseProc,
    ReleaseContext releaseContext) {
  auto releaseHelper = GrRefCntedCallback::Make(textureReleaseProc, releaseContext);
  ...
  GrSurfaceProxyView tempViews[4];
  if (!SkImage_GpuBase::MakeTempTextureProxies(
          ctx, yuvaTextures, numTextures, yuvaIndices, textureOrigin, tempViews,
...
sk_sp<SkImage> SkImage_GpuYUVA::MakePromiseYUVATexture(
  ...
  for (int texIdx = 0; texIdx < numTextures; ++texIdx) {
    auto proxy = MakePromiseImageLazyProxy(
        context, yuvaSizes[texIdx], yuvaFormats[texIdx], GrMipmapped::kNo, textureFulfillProc,
  ...
  return sk_make_sp<SkImage_GpuYUVA>(
      ...
      views, numTextures, yuvaIndices, std::move(imageColorSpace));
}
