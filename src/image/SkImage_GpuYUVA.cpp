...
SkImage_GpuYUVA::SkImage_GpuYUVA(
    sk_sp<GrContext> context, SkISize size, uint32_t uniqueID, SkYUVColorSpace colorSpace,
    sk_sp<GrTextureProxy> proxies[], GrColorType proxyColorTypes[], int numProxies,
    const SkYUVAIndex yuvaIndices[4], GrSurfaceOrigin origin, sk_sp<SkColorSpace> imageColorSpace)
    : INHERITED(
          std::move(context), size, uniqueID, kAssumedColorType,
...
SkImage_GpuYUVA::SkImage_GpuYUVA(const SkImage_GpuYUVA* image, sk_sp<SkColorSpace> targetCS)
    : INHERITED(
          image->fContext, image->dimensions(), kNeedNewImageUniqueID, kAssumedColorType,
          // If an alpha channel is present we always switch to kPremul. This is because,
          // although the planar data is always un-premul, the final interleaved RGB image
          // is/would-be premul.
...
bool SkImage_GpuYUVA::setupMipmapsForPlanes(GrRecordingContext* context) const {
  ...
  for (int i = 0; i < fNumProxies; ++i) {
    ...
    if (mipCount && GrGpu::IsACopyNeededForMips(
                        fContext->priv().caps(), fProxies[i].get(), GrSamplerState::Filter::kMipMap,
                        &copyParams)) {
      auto mippedProxy =
          GrCopyBaseMipMapToTextureProxy(context, fProxies[i].get(), fProxyColorTypes[i]);
      if (!mippedProxy) {
        return false;
      }
...
GrSemaphoresSubmitted SkImage_GpuYUVA::onFlush(GrContext* context, const GrFlushInfo& info) {
  ...
  GrSurfaceProxy* proxies[4] = {
      fProxies[0].get(), fProxies[1].get(), fProxies[2].get(), fProxies[3].get()};
...
void SkImage_GpuYUVA::flattenToRGB(GrRecordingContext* context) const {
  ...
  if (fFromColorSpace) {
    colorSpaceXform = GrColorSpaceXform::Make(
        fFromColorSpace.get(), this->alphaType(), this->colorSpace(), this->alphaType());
  }
  const SkRect rect = SkRect::MakeIWH(this->width(), this->height());
  if (!RenderYUVAToRGBA(
          fContext.get(), renderTargetContext.get(), rect, fYUVColorSpace,
...
sk_sp<GrTextureProxy> SkImage_GpuYUVA::asMippedTextureProxyRef(GrRecordingContext* context) const {
  ...
  if (auto mippedProxy = GrCopyBaseMipMapToTextureProxy(context, fRGBView.proxy(), srcColorType)) {
...
sk_sp<SkImage> SkImage_GpuYUVA::onReinterpretColorSpace(sk_sp<SkColorSpace> newCS) const {
  return sk_make_sp<SkImage_GpuYUVA>(
      fContext, this->dimensions(), kNeedNewImageUniqueID, fYUVColorSpace, fProxies,
      fProxyColorTypes, fNumProxies, fYUVAIndices, fOrigin, std::move(newCS));
}

//////////////////////////////////////////////////////////////////////////////////////////////////

sk_sp<SkImage> SkImage::MakeFromYUVATextures(
  ...
  if (!SkImage_GpuBase::MakeTempTextureProxies(
          ctx, yuvaTextures, numTextures, yuvaIndices, imageOrigin, tempTextureProxies)) {
    return nullptr;
  }
  ...
  return sk_make_sp<SkImage_GpuYUVA>(
      sk_ref_sp(ctx), imageSize, kNeedNewImageUniqueID, colorSpace, tempTextureProxies,
      proxyColorTypes, numTextures, yuvaIndices, imageOrigin, imageColorSpace);
}
...
sk_sp<SkImage> SkImage::MakeFromYUVAPixmaps(
  ...
  for (int i = 0; i < numPixmaps; ++i) {
    ...
    if (limitToMaxTextureSize && maxDim > maxTextureSize) {
      ...
      int newHeight = SkTMin(static_cast<int>(yuvaPixmaps[i].height() * scale), maxTextureSize);
      SkImageInfo info = yuvaPixmaps[i].info().makeWH(newWidth, newHeight);
      if (!resized.tryAlloc(info) || !yuvaPixmaps[i].scalePixels(resized, kLow_SkFilterQuality)) {
        return nullptr;
      }
      pixmap = &resized;
    }
    ...
    std::tie(tempTextureProxies[i], proxyColorTypes[i]) = bitmapMaker.refTextureProxy(mipMapped);
  ...
  return sk_make_sp<SkImage_GpuYUVA>(
      sk_ref_sp(context), imageSize, kNeedNewImageUniqueID, yuvColorSpace, tempTextureProxies,
      proxyColorTypes, numPixmaps, yuvaIndices, imageOrigin, imageColorSpace);
}
...
sk_sp<SkImage> SkImage_GpuYUVA::MakePromiseYUVATexture(
    ...
    const SkISize yuvaSizes[], const SkYUVAIndex yuvaIndices[4], int imageWidth, int imageHeight,
    GrSurfaceOrigin imageOrigin, sk_sp<SkColorSpace> imageColorSpace,
    PromiseImageTextureFulfillProc textureFulfillProc,
    PromiseImageTextureReleaseProc textureReleaseProc, PromiseImageTextureDoneProc promiseDoneProc,
    PromiseImageTextureContext textureContexts[], PromiseImageApiVersion version) {
  int numTextures;
  ...
  SkAlphaType at =
      (-1 != yuvaIndices[SkYUVAIndex::kA_Index].fIndex) ? kPremul_SkAlphaType : kOpaque_SkAlphaType;
  SkImageInfo info =
  ...
  for (int texIdx = 0; texIdx < numTextures; ++texIdx) {
    ...
    proxies[texIdx] = MakePromiseImageLazyProxy(
        context, yuvaSizes[texIdx].width(), yuvaSizes[texIdx].height(), imageOrigin, colorType,
  ...
  return sk_make_sp<SkImage_GpuYUVA>(
      sk_ref_sp(context), SkISize{imageWidth, imageHeight}, kNeedNewImageUniqueID, yuvColorSpace,
      proxies, proxyColorTypes, numTextures, yuvaIndices, imageOrigin, std::move(imageColorSpace));
}
