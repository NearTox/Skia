...
bool SkImage_GpuYUVA::setupMipmapsForPlanes(GrRecordingContext* context) const {
  ...
  for (int i = 0; i < fNumViews; ++i) {
    int mipCount =
        SkMipMap::ComputeLevelCount(fViews[i].proxy()->width(), fViews[i].proxy()->height());
    if (mipCount &&
        GrGpu::IsACopyNeededForMips(
            fContext->priv().caps(), fViews[i].asTextureProxy(), GrSamplerState::Filter::kMipMap)) {
      auto mippedView = GrCopyBaseMipMapToTextureProxy(
          context, fViews[i].asTextureProxy(), fOrigin, fProxyColorTypes[i]);
      if (!mippedView.proxy()) {
        return false;
      }
...
GrSemaphoresSubmitted SkImage_GpuYUVA::onFlush(GrContext* context, const GrFlushInfo& info) {
  ...
  GrSurfaceProxy* proxies[4] = {
      fViews[0].proxy(), fViews[1].proxy(), fViews[2].proxy(), fViews[3].proxy()};
...
GrSurfaceProxyView SkImage_GpuYUVA::refMippedView(GrRecordingContext* context) const {
  ...
  GrSurfaceProxyView mippedView =
      GrCopyBaseMipMapToTextureProxy(context, fRGBView.proxy(), fRGBView.origin(), srcColorType);
  if (mippedView) {
...
sk_sp<SkImage> SkImage_GpuYUVA::onReinterpretColorSpace(sk_sp<SkColorSpace> newCS) const {
  return sk_make_sp<SkImage_GpuYUVA>(
      fContext, this->dimensions(), kNeedNewImageUniqueID, fYUVColorSpace, fViews, fProxyColorTypes,
      fNumViews, fYUVAIndices, fOrigin, std::move(newCS));
}
...
sk_sp<SkImage> SkImage::MakeFromYUVATextures(
  ...
  if (!SkImage_GpuBase::MakeTempTextureProxies(
          ctx, yuvaTextures, numTextures, yuvaIndices, imageOrigin, tempViews)) {
    return nullptr;
  }
  ...
  return sk_make_sp<SkImage_GpuYUVA>(
      sk_ref_sp(ctx), imageSize, kNeedNewImageUniqueID, colorSpace, tempViews, proxyColorTypes,
...
sk_sp<SkImage> SkImage::MakeFromYUVAPixmaps(
  ...
  for (int i = 0; i < numPixmaps; ++i) {
    ...
    if (limitToMaxTextureSize && maxDim > maxTextureSize) {
      ...
      int newHeight = std::min(static_cast<int>(yuvaPixmaps[i].height() * scale), maxTextureSize);
  ...
  return sk_make_sp<SkImage_GpuYUVA>(
      sk_ref_sp(context), imageSize, kNeedNewImageUniqueID, yuvColorSpace, tempViews,
...
sk_sp<SkImage> SkImage_GpuYUVA::MakePromiseYUVATexture(
  ...
  for (int texIdx = 0; texIdx < numTextures; ++texIdx) {
    ...
    GrSwizzle swizzle = context->priv().caps()->getReadSwizzle(proxy->backendFormat(), colorType);
  ...
  return sk_make_sp<SkImage_GpuYUVA>(
      ...
      views, proxyColorTypes, numTextures, yuvaIndices, imageOrigin, std::move(imageColorSpace));
}
