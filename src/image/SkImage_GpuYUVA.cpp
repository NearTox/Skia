...
SkImage_GpuYUVA::SkImage_GpuYUVA(
    sk_sp<GrImageContext> context, SkISize size, uint32_t uniqueID, SkYUVColorSpace colorSpace,
    GrSurfaceProxyView views[], int numViews, const SkYUVAIndex yuvaIndices[4],
...
SkImage_GpuYUVA::SkImage_GpuYUVA(
    sk_sp<GrImageContext> context, const SkImage_GpuYUVA* image, sk_sp<SkColorSpace> targetCS)
    : INHERITED(
          std::move(context), image->dimensions(), kNeedNewImageUniqueID, kAssumedColorType,
          // If an alpha channel is present we always switch to kPremul. This is because,
          // although the planar data is always un-premul, the final interleaved RGB image
          // is/would-be premul.
...
void SkImage_GpuYUVA::flattenToRGB(GrRecordingContext* context) const {
  ...
  if (!RenderYUVAToRGBA(
          caps, renderTargetContext.get(), rect, fYUVColorSpace, std::move(colorSpaceXform), fViews,
          fYUVAIndices)) {
    return;
  }
...
sk_sp<SkImage> SkImage_GpuYUVA::onReinterpretColorSpace(sk_sp<SkColorSpace> newCS) const {
  return sk_make_sp<SkImage_GpuYUVA>(
      fContext, this->dimensions(), kNeedNewImageUniqueID, fYUVColorSpace, fViews, fNumViews,
...
sk_sp<SkImage> SkImage::MakeFromYUVATextures(
  ...
  return sk_make_sp<SkImage_GpuYUVA>(
      sk_ref_sp(ctx), imageSize, kNeedNewImageUniqueID, colorSpace, tempViews, numTextures,
...
sk_sp<SkImage> SkImage::MakeFromYUVAPixmaps(
  ...
  return sk_make_sp<SkImage_GpuYUVA>(
      sk_ref_sp(context), imageSize, kNeedNewImageUniqueID, yuvColorSpace, tempViews, numPixmaps,
...
sk_sp<SkImage> SkImage::MakeFromYUVAPixmaps(
  ...
  return sk_make_sp<SkImage_GpuYUVA>(
      sk_ref_sp(context), pixmaps.yuvaInfo().dimensions(), kNeedNewImageUniqueID,
      pixmaps.yuvaInfo().yuvColorSpace(), tempViews, numPlanes, yuvaIndices,
      kTopLeft_GrSurfaceOrigin, std::move(imageColorSpace));
}

/////////////////////////////////////////////////////////////////////////////////////////////////
sk_sp<SkImage> SkImage_GpuYUVA::MakePromiseYUVATexture(
    GrRecordingContext* context, SkYUVColorSpace yuvColorSpace, const GrBackendFormat yuvaFormats[],
  ...
  for (int texIdx = 0; texIdx < numTextures; ++texIdx) {
    auto proxy = MakePromiseImageLazyProxy(
        context, yuvaSizes[texIdx].width(), yuvaSizes[texIdx].height(), yuvaFormats[texIdx],
        GrMipmapped::kNo, textureFulfillProc, textureReleaseProc, promiseDoneProc,
        textureContexts[texIdx], version);
    ++proxiesCreated;
    if (!proxy) {
      return nullptr;
    }
  ...
  return sk_make_sp<SkImage_GpuYUVA>(
      ...
      views, numTextures, yuvaIndices, imageOrigin, std::move(imageColorSpace));
}
