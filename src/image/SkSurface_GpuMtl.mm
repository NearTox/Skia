...
sk_sp<SkSurface> SkSurface::MakeFromCAMetalLayer(
  ...
  auto rtc = std::make_unique<GrRenderTargetContext>(
      context, std::move(readView), std::move(writeView), grColorType, colorSpace, surfaceProps);
...
sk_sp<SkSurface> SkSurface::MakeFromMTKView(
    GrContext* context, GrMTLHandle view, GrSurfaceOrigin origin, int sampleCnt,
    SkColorType colorType, sk_sp<SkColorSpace> colorSpace, const SkSurfaceProps* surfaceProps) {
  GrProxyProvider* proxyProvider = context->priv().proxyProvider();
  const GrCaps* caps = context->priv().caps();
