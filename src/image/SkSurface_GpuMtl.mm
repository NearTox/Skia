...
sk_sp<SkSurface> SkSurface::MakeFromCAMetalLayer(
  ...
  sk_sp<GrRenderTargetProxy> proxy = proxyProvider->createLazyRenderTargetProxy(
      [layer, drawable, sampleCnt](GrResourceProvider* resourceProvider) {
        ...
        if (metalLayer.framebufferOnly) {
          ...
          surface = GrMtlTextureRenderTarget::MakeWrappedTextureRenderTarget(
              mtlGpu, desc, sampleCnt, currentDrawable.texture, GrWrapCacheable::kNo);
        }
      ...
      backendFormat, desc, readSwizzle, sampleCnt, origin,
  ...
  auto rtc = std::make_unique<GrRenderTargetContext>(
      context, std::move(readView), std::move(outputView), grColorType, colorSpace, surfaceProps);
