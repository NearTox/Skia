...
sk_sp<SkImage> SkImage_Gpu::onMakeColorTypeAndColorSpace(
  ...
  return sk_make_sp<SkImage_Gpu>(
      fContext, kNeedNewImageUniqueID, renderTargetContext->readSurfaceView(), targetCT,
...
static sk_sp<SkImage> new_wrapped_texture_common(
  ...
  return sk_make_sp<SkImage_Gpu>(
      sk_ref_sp(ctx), kNeedNewImageUniqueID, std::move(view), GrColorTypeToSkColorType(colorType),
      at, std::move(colorSpace));
}

sk_sp<SkImage> SkImage::MakeFromCompressedTexture(
    GrContext* ctx, const GrBackendTexture& tex, GrSurfaceOrigin origin, SkAlphaType at,
    sk_sp<SkColorSpace> cs, TextureReleaseProc releaseP, ReleaseContext releaseC) {
  if (!ctx) {
    return nullptr;
  }
  ...
  sk_sp<GrTextureProxy> proxy = proxyProvider->wrapCompressedBackendTexture(
      tex, origin, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, releaseP, releaseC);
  if (!proxy) {
    return nullptr;
  }
  ...
  return sk_make_sp<SkImage_Gpu>(
      sk_ref_sp(ctx), kNeedNewImageUniqueID, std::move(view), ct, at, std::move(cs));
}
...
sk_sp<SkImage> SkImage::MakeFromTexture(
  ...
  return new_wrapped_texture_common(
      ctx, tex, grColorType, origin, at, std::move(cs), kBorrow_GrWrapOwnership, releaseP,
      releaseC);
}
...
sk_sp<SkImage> SkImage::MakeFromAdoptedTexture(
  ...
  return new_wrapped_texture_common(
      ctx, tex, grColorType, origin, at, std::move(cs), kAdopt_GrWrapOwnership, nullptr, nullptr);
}
...
sk_sp<SkImage> SkImage::MakeTextureFromCompressed(
  ...
  return sk_make_sp<SkImage_Gpu>(
      sk_ref_sp(context), kNeedNewImageUniqueID, std::move(view), colorType, kOpaque_SkAlphaType,
      nullptr);
}
...
sk_sp<SkImage> SkImage_Gpu::ConvertYUVATexturesToRGB(
  ...
  // MDB: this call is okay bc we know 'renderTargetContext' was exact
  return sk_make_sp<SkImage_Gpu>(
      sk_ref_sp(ctx), kNeedNewImageUniqueID, renderTargetContext->readSurfaceView(), ct, at,
...
sk_sp<SkImage> SkImage::MakeFromYUVATexturesCopy(
  ...
  auto renderTargetContext = GrRenderTargetContext::Make(
      ctx, GrColorType::kRGBA_8888, std::move(imageColorSpace), SkBackingFit::kExact, imageSize, 1,
      GrMipMapped::kNo, GrProtected::kNo, imageOrigin);
  if (!renderTargetContext) {
    return nullptr;
  }
...
sk_sp<SkImage> SkImage::MakeFromYUVATexturesCopyWithExternalBackend(
  ...
  auto renderTargetContext = GrRenderTargetContext::MakeFromBackendTexture(
      ctx, grColorType, std::move(imageColorSpace), backendTexture, 1, imageOrigin, nullptr,
...
static sk_sp<SkImage> create_image_from_producer(
  ...
  GrSurfaceProxyView view(std::move(proxy), origin, swizzle);
  return sk_make_sp<SkImage_Gpu>(
      sk_ref_sp(context), id, std::move(view), GrColorTypeToSkColorType(colorType),
...
sk_sp<SkImage> SkImage::makeTextureImage(GrContext* context, GrMipMapped mipMapped) const {
  ...
  if (this->isTextureBacked()) {
    ...
    GrTextureAdjuster adjuster(
        context, std::move(proxy), this->imageInfo().colorInfo(), this->uniqueID());
...
sk_sp<SkImage> SkImage::MakeCrossContextFromPixmap(
    GrContext* context, const SkPixmap& originalPixmap, bool buildMips,
  ...
  auto gen = GrBackendTextureImageGenerator::Make(
      std::move(texture), proxy->origin(), std::move(sema), skCT, pixmap->alphaType(),
...
sk_sp<SkImage> SkImage::MakeFromAHardwareBufferWithData(
  ...
  sk_sp<SkImage> image =
      sk_make_sp<SkImage_Gpu>(sk_ref_sp(context), kNeedNewImageUniqueID, view, colorType, at, cs);
  if (!image) {
    return nullptr;
  }
  ...
  GrSurfaceContext surfaceContext(
      context, std::move(view), SkColorTypeToGrColorType(pixmap.colorType()), pixmap.alphaType(),
      cs);
