...
static sk_sp<SkImage> new_wrapped_texture_common(
    GrContext* ctx, const GrBackendTexture& backendTex, GrSurfaceOrigin origin, SkAlphaType at,
    sk_sp<SkColorSpace> colorSpace, GrWrapOwnership ownership,
    SkImage::TextureReleaseProc releaseProc, SkImage::ReleaseContext releaseCtx) {
  ...
  sk_sp<GrTextureProxy> proxy = proxyProvider->wrapBackendTexture(
      backendTex, origin, ownership, GrWrapCacheable::kNo, kRead_GrIOType, releaseProc, releaseCtx);
  if (!proxy) {
    return nullptr;
  }
  return sk_make_sp<SkImage_Gpu>(
...
sk_sp<SkImage> SkImage::MakeFromTexture(
  ...
  return new_wrapped_texture_common(
      ctx, texCopy, origin, at, std::move(cs), kBorrow_GrWrapOwnership, releaseP, releaseC);
}
...
sk_sp<SkImage> SkImage::MakeFromAdoptedTexture(
  ...
  return new_wrapped_texture_common(
      ctx, texCopy, origin, at, std::move(cs), kAdopt_GrWrapOwnership, nullptr, nullptr);
}
...
sk_sp<SkImage> SkImage::MakeFromCompressed(
  ...
  desc.fHeight = height;
  switch (type) {
    case kETC1_CompressionType: desc.fConfig = kRGB_ETC1_GrPixelConfig; break;
    default: desc.fConfig = kUnknown_GrPixelConfig; break;
  }
  desc.fSampleCnt = 1;
...
sk_sp<SkImage> SkImage::MakeFromYUVATexturesCopyWithExternalBackend(
  ...
  if (!SkImage_Gpu::ValidateBackendTexture(
          ctx, backendTextureCopy, &backendTextureCopy.fConfig, kRGBA_8888_SkColorType, at,
          nullptr)) {
    return nullptr;
  }
  ...
  sk_sp<GrRenderTargetContext> renderTargetContext(
      ctx->priv().makeBackendTextureRenderTargetContext(
          backendTextureCopy, imageOrigin, 1, std::move(imageColorSpace)));
...
sk_sp<SkImage> SkImage::makeTextureImage(
  ...
  if (this->isTextureBacked()) {
    ...
    GrTextureAdjuster adjuster(
        context, std::move(proxy), this->alphaType(), this->uniqueID(), this->colorSpace());
...
sk_sp<SkImage> SkImage_Gpu::MakePromiseTexture(
  ...
  auto proxy = MakePromiseImageLazyProxy(
      context, width, height, origin, config, backendFormat, mipMapped, textureFulfillProc,
...
sk_sp<SkImage> SkImage::MakeFromAHardwareBufferWithData(
  ...
  sk_sp<GrTextureProxy> proxy = proxyProvider->wrapBackendTexture(
      backendTexture, surfaceOrigin, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, kRW_GrIOType,
      deleteImageProc, deleteImageCtx);
  if (!proxy) {
    deleteImageProc(deleteImageCtx);
    return nullptr;
