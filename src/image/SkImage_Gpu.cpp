...
SkImage_Gpu::SkImage_Gpu(
  ...
  SkASSERT(caps->isFormatCompressed(format) || caps->areColorTypeAndFormatCompatible(grCT, format));
#endif
}
...
sk_sp<SkImage> SkImage_Gpu::onMakeColorTypeAndColorSpace(
    SkColorType targetCT, sk_sp<SkColorSpace> targetCS, GrDirectContext* direct) const {
  ...
  auto colorFP = GrColorSpaceXformEffect::Make(
      std::move(texFP), this->colorSpace(), this->alphaType(), targetCS.get(), this->alphaType());
  SkASSERT(colorFP);
  ...
  return sk_make_sp<SkImage_Gpu>(
      sk_ref_sp(direct), kNeedNewImageUniqueID, renderTargetContext->readSurfaceView(), targetCT,
      this->alphaType(), std::move(targetCS));
}

sk_sp<SkImage> SkImage_Gpu::onReinterpretColorSpace(sk_sp<SkColorSpace> newCS) const {
  return sk_make_sp<SkImage_Gpu>(
      fContext, kNeedNewImageUniqueID, fView, this->colorType(), this->alphaType(),
      std::move(newCS));
}

void SkImage_Gpu::onAsyncRescaleAndReadPixels(
  ...
  auto ctx = GrSurfaceContext::Make(dContext, fView, ct, this->alphaType(), this->refColorSpace());
  if (!ctx) {
    callback(context, nullptr);
    return;
  }
  ctx->asyncRescaleAndReadPixels(
      dContext, info, srcRect, rescaleGamma, rescaleQuality, callback, context);
}

void SkImage_Gpu::onAsyncRescaleAndReadPixelsYUV420(
  ...
  ctx->asyncRescaleAndReadPixelsYUV420(
...
static sk_sp<SkImage> new_wrapped_texture_common(
    GrRecordingContext* rContext, const GrBackendTexture& backendTex, GrColorType colorType,
    GrSurfaceOrigin origin, SkAlphaType at, sk_sp<SkColorSpace> colorSpace,
    GrWrapOwnership ownership, sk_sp<GrRefCntedCallback> releaseHelper) {
...
sk_sp<SkImage> SkImage::MakeFromCompressedTexture(
    GrRecordingContext* rContext, const GrBackendTexture& tex, GrSurfaceOrigin origin,
    SkAlphaType at, sk_sp<SkColorSpace> cs, TextureReleaseProc releaseP, ReleaseContext releaseC) {
...
sk_sp<SkImage> SkImage::MakeFromTexture(
    GrRecordingContext* rContext, const GrBackendTexture& tex, GrSurfaceOrigin origin,
    SkColorType ct, SkAlphaType at, sk_sp<SkColorSpace> cs, TextureReleaseProc releaseP,
  ...
  return new_wrapped_texture_common(
      rContext, tex, grColorType, origin, at, std::move(cs), kBorrow_GrWrapOwnership,
      std::move(releaseHelper));
}

sk_sp<SkImage> SkImage::MakeFromAdoptedTexture(
    GrRecordingContext* rContext, const GrBackendTexture& tex, GrSurfaceOrigin origin,
  ...
  return new_wrapped_texture_common(
      dContext, tex, grColorType, origin, at, std::move(cs), kAdopt_GrWrapOwnership, nullptr);
}

sk_sp<SkImage> SkImage::MakeTextureFromCompressed(
    GrDirectContext* direct, sk_sp<SkData> data, int width, int height, CompressionType type,
  ...
  return sk_make_sp<SkImage_Gpu>(
      sk_ref_sp(direct), kNeedNewImageUniqueID, std::move(view), colorType, kOpaque_SkAlphaType,
      nullptr);
}

sk_sp<SkImage> SkImage_Gpu::ConvertYUVATexturesToRGB(
    GrRecordingContext* rContext, SkYUVColorSpace yuvColorSpace,
    const GrBackendTexture yuvaTextures[], const SkYUVAIndex yuvaIndices[4], SkISize size,
    GrSurfaceOrigin origin, GrRenderTargetContext* renderTargetContext,
  ...
  if (!SkImage_GpuBase::MakeTempTextureProxies(
          rContext, yuvaTextures, numTextures, yuvaIndices, origin, tempViews,
          std::move(releaseHelper))) {
    return nullptr;
  }
  ...
  const GrCaps& caps = *rContext->priv().caps();
  if (!RenderYUVAToRGBA(
          caps, renderTargetContext, rect, yuvColorSpace, nullptr, tempViews, yuvaIndices)) {
    return nullptr;
  }
...
static sk_sp<SkImage> make_flattened_image_with_external_backend(
    GrRecordingContext* rContext, SkYUVColorSpace yuvColorSpace,
    const GrBackendTexture yuvaTextures[], const SkYUVAIndex yuvaIndices[4], SkISize imageSize,
    GrSurfaceOrigin imageOrigin, const GrBackendTexture& backendTexture, GrColorType colorType,
    sk_sp<SkColorSpace> imageColorSpace, sk_sp<GrRefCntedCallback> yuvaReleaseHelper,
  ...
  if (!SkImage_Gpu::ValidateBackendTexture(
          caps, backendTexture, colorType, kRGBA_8888_SkColorType, at, nullptr)) {
    return nullptr;
  }
  ...
  auto renderTargetContext = GrRenderTargetContext::MakeFromBackendTexture(
      rContext, colorType, std::move(imageColorSpace), backendTexture, 1, imageOrigin, nullptr,
      std::move(rgbaReleaseHelper));
  if (!renderTargetContext) {
    return nullptr;
  }

  return SkImage_Gpu::ConvertYUVATexturesToRGB(
      rContext, yuvColorSpace, yuvaTextures, yuvaIndices, imageSize, imageOrigin,
      renderTargetContext.get(), std::move(yuvaReleaseHelper));
}
...
sk_sp<SkImage> SkImage::MakeFromYUVATexturesCopyToExternal(
    GrRecordingContext* context, const GrYUVABackendTextures& yuvaTextures,
    const GrBackendTexture& rgbaResultTexture, SkColorType colorType,
    sk_sp<SkColorSpace> imageColorSpace, TextureReleaseProc yuvaReleaseProc,
    ReleaseContext yuvaReleaseContext, TextureReleaseProc rgbaReleaseProc,
  ...
  return make_flattened_image_with_external_backend(
      context, yuvaTextures.yuvaInfo().yuvColorSpace(), yuvaTextures.textures().data(), yuvaIndices,
      rgbaResultTexture.dimensions(), yuvaTextures.textureOrigin(), rgbaResultTexture,
      SkColorTypeToGrColorType(colorType), std::move(imageColorSpace), std::move(yuvaReleaseHelper),
      std::move(rgbaReleaseHelper));
}

sk_sp<SkImage> SkImage::MakeFromYUVTexturesCopyWithExternalBackend(
    GrRecordingContext* ctx, SkYUVColorSpace yuvColorSpace, const GrBackendTexture yuvTextures[3],
  ...
  return make_flattened_image_with_external_backend(
      ...
      backendTexture, GrColorType::kRGBA_8888, std::move(imageColorSpace), nullptr, nullptr);
}

sk_sp<SkImage> SkImage::MakeFromNV12TexturesCopyWithExternalBackend(
    GrRecordingContext* ctx, SkYUVColorSpace yuvColorSpace, const GrBackendTexture nv12Textures[2],
    GrSurfaceOrigin imageOrigin, const GrBackendTexture& backendTexture,
  ...
  return make_flattened_image_with_external_backend(
      ...
      GrColorType::kRGBA_8888, std::move(imageColorSpace),
      /* plane release helper*/ nullptr, std::move(releaseHelper));
}

static sk_sp<SkImage> create_image_from_producer(
    GrRecordingContext* context, GrTextureProducer* producer, uint32_t id, GrMipmapped mipMapped) {
  auto view = producer->view(mipMapped);
  if (!view) {
    return nullptr;
  }
  return sk_make_sp<SkImage_Gpu>(
      sk_ref_sp(context), id, std::move(view), GrColorTypeToSkColorType(producer->colorType()),
      producer->alphaType(), sk_ref_sp(producer->colorSpace()));
}

sk_sp<SkImage> SkImage::makeTextureImage(
    GrDirectContext* dContext, GrMipmapped mipMapped, SkBudgeted budgeted) const {
  if (!dContext) {
    return nullptr;
  }
  ...
  if (this->isTextureBacked()) {
    ...
    return sk_make_sp<SkImage_Gpu>(
        sk_ref_sp(dContext), this->uniqueID(), copy, this->colorType(), this->alphaType(),
        this->refColorSpace());
  }
...
sk_sp<SkImage> SkImage_Gpu::MakePromiseTexture(
    GrRecordingContext* context, const GrBackendFormat& backendFormat, SkISize dimensions,
  ...
  GrColorType grColorType =
      SkColorTypeAndFormatToGrColorType(context->priv().caps(), colorType, backendFormat);
  if (GrColorType::kUnknown == grColorType) {
    return nullptr;
  }
  ...
  auto proxy = MakePromiseImageLazyProxy(
      context, dimensions, backendFormat, mipMapped, textureFulfillProc, std::move(releaseHelper));
  if (!proxy) {
    return nullptr;
  }
...
sk_sp<SkImage> SkImage::MakeCrossContextFromPixmap(
    GrDirectContext* dContext, const SkPixmap& originalPixmap, bool buildMips,
  ...
  auto gen = GrBackendTextureImageGenerator::Make(
      std::move(texture), view.origin(), std::move(sema), skCT, pixmap->alphaType(),
...
sk_sp<SkImage> SkImage::MakeFromAHardwareBuffer(
    AHardwareBuffer* graphicBuffer, SkAlphaType at, sk_sp<SkColorSpace> cs,
...
sk_sp<SkImage> SkImage::MakeFromAHardwareBufferWithData(
    GrDirectContext* dContext, const SkPixmap& pixmap, AHardwareBuffer* hardwareBuffer,
    GrSurfaceOrigin surfaceOrigin) {
  ...
  GrBackendFormat backendFormat =
      GrAHardwareBufferUtils::GetBackendFormat(dContext, hardwareBuffer, bufferDesc.format, true);
  ...
  GrBackendTexture backendTexture = GrAHardwareBufferUtils::MakeBackendTexture(
      ...
      &updateImageProc, &deleteImageCtx, false, backendFormat, true);
  ...
  SkColorType colorType = GrAHardwareBufferUtils::GetSkColorTypeFromBufferFormat(bufferDesc.format);
  ...
  sk_sp<SkImage> image =
      sk_make_sp<SkImage_Gpu>(sk_ref_sp(dContext), kNeedNewImageUniqueID, view, colorType, at, cs);
  if (!image) {
    return nullptr;
  }
  ...
  GrSurfaceContext surfaceContext(
      dContext, std::move(view), SkColorTypeToGrColorType(pixmap.colorType()), pixmap.alphaType(),
      cs);

  SkImageInfo srcInfo =
      SkImageInfo::Make(bufferDesc.width, bufferDesc.height, colorType, at, std::move(cs));
...
bool SkImage::MakeBackendTextureFromSkImage(
    GrDirectContext* direct, sk_sp<SkImage> image, GrBackendTexture* backendTexture,
