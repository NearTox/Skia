...
SkImage_Gpu::SkImage_Gpu(
    sk_sp<GrContext> context, uint32_t uniqueID, SkAlphaType at, sk_sp<GrTextureProxy> proxy,
    sk_sp<SkColorSpace> colorSpace)
    : INHERITED(
          std::move(context), proxy->worstCaseWidth(), proxy->worstCaseHeight(), uniqueID,
...
sk_sp<SkImage> SkImage_Gpu::onMakeColorTypeAndColorSpace(
    GrRecordingContext* context, SkColorType targetCT, sk_sp<SkColorSpace> targetCS) const {
  if (!context || !fContext->priv().matches(context)) {
    return nullptr;
  }

  auto xform = GrColorSpaceXformEffect::Make(
      this->colorSpace(), this->alphaType(), targetCS.get(), this->alphaType());
  ...
  return sk_make_sp<SkImage_Gpu>(
      fContext, kNeedNewImageUniqueID, this->alphaType(), renderTargetContext->asTextureProxyRef(),
      std::move(targetCS));
}
...
static sk_sp<SkImage> new_wrapped_texture_common(
    GrContext* ctx, const GrBackendTexture& backendTex, GrColorType colorType,
    GrSurfaceOrigin origin, SkAlphaType at, sk_sp<SkColorSpace> colorSpace,
    GrWrapOwnership ownership, SkImage::TextureReleaseProc releaseProc,
  ...
  sk_sp<GrTextureProxy> proxy = proxyProvider->wrapBackendTexture(
      backendTex, colorType, origin, ownership, GrWrapCacheable::kNo, kRead_GrIOType, releaseProc,
      releaseCtx);
  if (!proxy) {
    return nullptr;
  }
  return sk_make_sp<SkImage_Gpu>(
      sk_ref_sp(ctx), kNeedNewImageUniqueID, at, std::move(proxy), std::move(colorSpace));
}

sk_sp<SkImage> SkImage::MakeFromTexture(
    GrContext* ctx, const GrBackendTexture& tex, GrSurfaceOrigin origin, SkColorType ct,
  ...
  GrColorType grColorType =
      SkColorTypeAndFormatToGrColorType(ctx->priv().caps(), ct, tex.getBackendFormat());
  ...
  if (!SkImage_GpuBase::ValidateBackendTexture(
          ctx, texCopy, &texCopy.fConfig, grColorType, ct, at, cs)) {
    return nullptr;
  }

  return new_wrapped_texture_common(
      ctx, texCopy, grColorType, origin, at, std::move(cs), kBorrow_GrWrapOwnership, releaseP,
      releaseC);
}
...
sk_sp<SkImage> SkImage::MakeFromAdoptedTexture(
  ...
  return new_wrapped_texture_common(
      ctx, texCopy, grColorType, origin, at, std::move(cs), kAdopt_GrWrapOwnership, nullptr,
      nullptr);
}

sk_sp<SkImage> SkImage::MakeFromCompressed(
    GrContext* context, sk_sp<SkData> data, int width, int height, CompressionType type) {
...
sk_sp<SkImage> SkImage_Gpu::ConvertYUVATexturesToRGB(
  ...
  if (!SkImage_GpuBase::MakeTempTextureProxies(
          ctx, yuvaTextures, numTextures, yuvaIndices, origin, tempTextureProxies)) {
    return nullptr;
  }

  const SkRect rect = SkRect::MakeIWH(size.width(), size.height());
  if (!RenderYUVAToRGBA(
          ctx, renderTargetContext, rect, yuvColorSpace, nullptr, tempTextureProxies,
          yuvaIndices)) {
    return nullptr;
  }
  ...
  // MDB: this call is okay bc we know 'renderTargetContext' was exact
  return sk_make_sp<SkImage_Gpu>(
      sk_ref_sp(ctx), kNeedNewImageUniqueID, at, renderTargetContext->asTextureProxyRef(),
...
sk_sp<SkImage> SkImage::MakeFromYUVATexturesCopy(
  ...
  sk_sp<GrRenderTargetContext> renderTargetContext(ctx->priv().makeDeferredRenderTargetContext(
      SkBackingFit::kExact, width, height, GrColorType::kRGBA_8888, std::move(imageColorSpace), 1,
      GrMipMapped::kNo, imageOrigin));
  if (!renderTargetContext) {
    return nullptr;
  }

  return SkImage_Gpu::ConvertYUVATexturesToRGB(
      ctx, yuvColorSpace, yuvaTextures, yuvaIndices, imageSize, imageOrigin,
...
sk_sp<SkImage> SkImage::MakeFromYUVATexturesCopyWithExternalBackend(
  ...
  GrColorType grColorType = SkColorTypeAndFormatToGrColorType(
      ctx->priv().caps(), kRGBA_8888_SkColorType, backendTexture.getBackendFormat());
  if (GrColorType::kUnknown == grColorType) {
    return nullptr;
  }
  ...
  if (!SkImage_Gpu::ValidateBackendTexture(
          ctx, backendTextureCopy, &backendTextureCopy.fConfig, grColorType, kRGBA_8888_SkColorType,
          at, nullptr)) {
    return nullptr;
  }
  ...
  sk_sp<GrRenderTargetContext> renderTargetContext(
      ctx->priv().makeBackendTextureRenderTargetContext(
          backendTextureCopy, imageOrigin, 1, grColorType, std::move(imageColorSpace)));
...
sk_sp<SkImage> SkImage::MakeFromNV12TexturesCopy(
    GrContext* ctx, SkYUVColorSpace yuvColorSpace, const GrBackendTexture nv12Textures[2],
    GrSurfaceOrigin imageOrigin, sk_sp<SkColorSpace> imageColorSpace) {
  // TODO: SkImageSourceChannel input is being ingored right now. Setup correctly in the future.
  SkYUVAIndex yuvaIndices[4] = {
      SkYUVAIndex{0, SkColorChannel::kR}, SkYUVAIndex{1, SkColorChannel::kR},
  ...
  return SkImage_Gpu::MakeFromYUVATexturesCopy(
      ctx, yuvColorSpace, nv12Textures, yuvaIndices, size, imageOrigin, std::move(imageColorSpace));
}
...
static sk_sp<SkImage> create_image_from_producer(
    GrContext* context, GrTextureProducer* producer, SkAlphaType at, uint32_t id,
  ...
  return sk_make_sp<SkImage_Gpu>(
      sk_ref_sp(context), id, at, std::move(proxy), sk_ref_sp(producer->colorSpace()));
}

sk_sp<SkImage> SkImage::makeTextureImage(
    GrContext* context, SkColorSpace* dstColorSpace, GrMipMapped mipMapped) const {
  if (!context) {
    return nullptr;
  }
  ...
  if (this->isTextureBacked()) {
    ...
    GrTextureAdjuster adjuster(
        context, std::move(proxy), SkColorTypeToGrColorType(this->colorType()), this->alphaType(),
        this->uniqueID(), this->colorSpace());
    return create_image_from_producer(
        context, &adjuster, this->alphaType(), this->uniqueID(), mipMapped);
  }

  if (this->isLazyGenerated()) {
    GrImageTextureMaker maker(context, this, kDisallow_CachingHint);
    return create_image_from_producer(
        context, &maker, this->alphaType(), this->uniqueID(), mipMapped);
  }
...
sk_sp<SkImage> SkImage_Gpu::MakePromiseTexture(
    ...
    PromiseImageTextureReleaseProc textureReleaseProc, PromiseImageTextureDoneProc textureDoneProc,
  ...
  auto proxy = MakePromiseImageLazyProxy(
      context, width, height, origin, grColorType, backendFormat, mipMapped, textureFulfillProc,
      textureReleaseProc, textureDoneProc, textureContext, version);
  if (!proxy) {
    return nullptr;
  }
  return sk_make_sp<SkImage_Gpu>(
      sk_ref_sp(context), kNeedNewImageUniqueID, alphaType, std::move(proxy),
      std::move(colorSpace));
}

///////////////////////////////////////////////////////////////////////////////////////////////////

sk_sp<SkImage> SkImage::MakeCrossContextFromEncoded(
    GrContext* context, sk_sp<SkData> encoded, bool buildMips, SkColorSpace* dstColorSpace,
...
sk_sp<SkImage> SkImage::MakeCrossContextFromPixmap(
    GrContext* context, const SkPixmap& originalPixmap, bool buildMips, SkColorSpace* dstColorSpace,
    bool limitToMaxTextureSize) {
  // Some backends or drivers don't support (safely) moving resources between contexts
  if (!context || !context->priv().caps()->crossContextTextureSupport()) {
  ...
  auto gen = GrBackendTextureImageGenerator::Make(
      std::move(texture), proxy->origin(), std::move(sema), pixmap->colorType(),
      pixmap->alphaType(), pixmap->info().refColorSpace());
  return SkImage::MakeFromGenerator(std::move(gen));
}
...
sk_sp<SkImage> SkImage::MakeFromAHardwareBufferWithData(
    GrContext* context, const SkPixmap& pixmap, AHardwareBuffer* hardwareBuffer,
    GrSurfaceOrigin surfaceOrigin) {
  ...
  GrBackendFormat backendFormat =
      GrAHardwareBufferUtils::GetBackendFormat(context, hardwareBuffer, bufferDesc.format, true);
  ...
  GrBackendTexture backendTexture = GrAHardwareBufferUtils::MakeBackendTexture(
      context, hardwareBuffer, bufferDesc.width, bufferDesc.height, &deleteImageProc,
  ...
  backendTexture.fConfig =
      context->priv().caps()->getConfigFromBackendFormat(backendFormat, grColorType);
  ...
  sk_sp<GrTextureProxy> proxy = proxyProvider->wrapBackendTexture(
      backendTexture, grColorType, surfaceOrigin, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo,
  ...
  sk_sp<SkImage> image =
      sk_make_sp<SkImage_Gpu>(sk_ref_sp(context), kNeedNewImageUniqueID, at, proxy, cs);
  if (!image) {
    return nullptr;
  }
  ...
  sk_sp<GrTextureContext> texContext = drawingManager->makeTextureContext(
      proxy, SkColorTypeToGrColorType(pixmap.colorType()), pixmap.alphaType(), cs);
  if (!texContext) {
    return nullptr;
  }
  ...
  drawingManager->flush(
      p, 1, SkSurface::BackendSurfaceAccess::kNoAccess, info, GrPrepareForExternalIORequests());
...
bool SkImage::MakeBackendTextureFromSkImage(
    GrContext* ctx, sk_sp<SkImage> image, GrBackendTexture* backendTexture,
