...
sk_sp<SkImage> SkImage_Gpu::ConvertYUVATexturesToRGB(
  ...
  if (!SkImage_GpuBase::MakeTempTextureProxies(
          rContext, yuvaTextures, numTextures, yuvaIndices, origin, tempViews, nullptr)) {
    return nullptr;
  }
...
sk_sp<SkImage> SkImage::MakeFromYUVATexturesCopyWithExternalBackend(
  ...
  GrColorType grColorType = SkColorTypeAndFormatToGrColorType(
      caps, kRGBA_8888_SkColorType, backendTexture.getBackendFormat());
  if (GrColorType::kUnknown == grColorType) {
    return nullptr;
  }
  ...
  if (!SkImage_Gpu::ValidateBackendTexture(
          caps, backendTexture, grColorType, kRGBA_8888_SkColorType, at, nullptr)) {
    return nullptr;
  }
  ...
  auto renderTargetContext = GrRenderTargetContext::MakeFromBackendTexture(
      rContext, grColorType, std::move(imageColorSpace), backendTexture, 1, imageOrigin, nullptr,
      std::move(releaseHelper));
  if (!renderTargetContext) {
    return nullptr;
  }
...
static sk_sp<SkImage> create_image_from_producer(
    GrContext* context, GrTextureProducer* producer, uint32_t id, GrMipmapped mipMapped) {
...
sk_sp<SkImage> SkImage_Gpu::MakePromiseTexture(
    GrRecordingContext* context, const GrBackendFormat& backendFormat, int width, int height,
...
sk_sp<SkImage> SkImage::MakeFromAHardwareBufferWithData(
  ...
  sk_sp<GrRefCntedCallback> releaseHelper(new GrRefCntedCallback(deleteImageProc, deleteImageCtx));
