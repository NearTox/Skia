...
sk_sp<SkImage> SkImage::MakeFromEncoded(sk_sp<SkData> encoded, const SkIRect* subset) {
  ...
  return SkImage::MakeFromGenerator(SkImageGenerator::MakeFromEncoded(std::move(encoded)), subset);
}
...
GrTexture* SkImage::getTexture() const { return as_IB(this)->onGetTexture(); }
...
sk_sp<SkImage> SkImage::makeWithFilter(
    GrContext* grContext, const SkImageFilter* filter, const SkIRect& subset,
  ...
  SkImageFilter::Context context(
      SkMatrix::MakeTrans(-subset.x(), -subset.y()),
      clipBounds.makeOffset(-subset.x(), -subset.y()), cache.get(), outputProperties);
...
sk_sp<SkImage> SkImage::MakeFromYUVTexturesCopy(
    GrContext* ctx, SkYUVColorSpace space, const GrBackendTexture[3], GrSurfaceOrigin origin,
    sk_sp<SkColorSpace> imageColorSpace) {
  return nullptr;
}
...
sk_sp<SkImage> SkImage::MakeFromNV12TexturesCopy(
    GrContext* ctx, SkYUVColorSpace space, const GrBackendTexture[2], GrSurfaceOrigin origin,
    sk_sp<SkColorSpace> imageColorSpace) {
  return nullptr;
}

sk_sp<SkImage> SkImage::makeTextureImage(
    GrContext*, SkColorSpace* dstColorSpace, GrMipMapped mipMapped) const {
  return nullptr;
}
...
sk_sp<SkImage> SkImageMakeRasterCopyAndAssignColorSpace(
    const SkImage* src, SkColorSpace* colorSpace) {
