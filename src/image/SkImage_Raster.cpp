...
class SkImage_Raster : public SkImage_Base {
 public:
  ...
  sk_sp<SkImage> onMakeColorTypeAndColorSpace(
      SkColorType, sk_sp<SkColorSpace>, GrDirectContext*) const override;
 ...
 private:
  ...
  std::tuple<GrSurfaceProxyView, GrColorType> onAsView(
      GrRecordingContext*, GrMipmapped, GrImageTexGenPolicy) const override;

  std::unique_ptr<GrFragmentProcessor> onAsFragmentProcessor(
      GrRecordingContext*, SkSamplingOptions, const SkTileMode[2], const SkMatrix&, const SkRect*,
...
SkImage_Raster::SkImage_Raster(
    const SkImageInfo& info, sk_sp<SkData> data, size_t rowBytes, uint32_t id)
    : INHERITED(info, id) {
...
bool SkImage_Raster::onPinAsTexture(GrRecordingContext* rContext) const {
  if (fPinnedView) {
    ...
    std::tie(fPinnedView, fPinnedColorType) =
        GrMakeCachedBitmapProxyView(rContext, fBitmap, GrMipmapped::kNo);
    if (!fPinnedView) {
...
sk_sp<SkImage> SkImage_Raster::onMakeSubset(const SkIRect& subset, GrDirectContext*) const {
  ...
  SkRectMemcpy(dst, bitmap.rowBytes(), src, fBitmap.rowBytes(), bitmap.rowBytes(), subset.height());
...
sk_sp<SkImage> SkImage::MakeRasterData(
    const SkImageInfo& info, sk_sp<SkData> data, size_t rowBytes) {
  size_t size;
...
sk_sp<SkImage> SkImage::MakeRasterFromCompressed(
  ...
  SkAlphaType at = SkCompressionTypeIsOpaque(type) ? kOpaque_SkAlphaType : kPremul_SkAlphaType;
...
sk_sp<SkImage> SkImage::MakeFromRaster(
    const SkPixmap& pmap, RasterReleaseProc proc, ReleaseContext ctx) {
  size_t size;
  if (!SkImage_Raster::ValidArgs(pmap.info(), pmap.rowBytes(), &size) || !pmap.addr()) {
    return nullptr;
  }
...
sk_sp<SkImage> SkMakeImageFromRasterBitmapPriv(
    const SkBitmap& bm, SkCopyPixelsMode cpm, uint32_t idForCopy) {
...
std::tuple<GrSurfaceProxyView, GrColorType> SkImage_Raster::onAsView(
  ...
  return GrMakeUncachedBitmapProxyView(
      rContext, fBitmap, mipmapped, SkBackingFit::kExact, budgeted);
}
