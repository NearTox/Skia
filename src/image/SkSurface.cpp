...
void SkSurface_Base::onAsyncRescaleAndReadPixels(
  ...
  if (rescaleGamma == SkSurface::RescaleGamma::kLinear &&
    ...
    auto ii = SkImageInfo::Make(
        srcW, srcH, kRGBA_F16_SkColorType, this->getCanvas()->imageInfo().alphaType(),
        std::move(cs));
...
bool SkSurface::replaceBackendTexture(
  ...
  return asSB(this)->onReplaceBackendTexture(
      backendTexture, origin, textureReleaseProc, releaseContext);
}
...
bool SkSurface::draw(SkDeferredDisplayList* ddl) { return asSB(this)->onDraw(ddl); }
...
sk_sp<SkSurface> SkSurface::MakeRenderTarget(
    GrContext*, SkBudgeted, const SkImageInfo&, int, GrSurfaceOrigin, const SkSurfaceProps*, bool) {
  return nullptr;
}

sk_sp<SkSurface> SkSurface::MakeRenderTarget(
    GrRecordingContext*, const SkSurfaceCharacterization&, SkBudgeted) {
  return nullptr;
}
