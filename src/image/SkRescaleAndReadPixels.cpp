...
void SkRescaleAndReadPixels(
    SkBitmap bmp, const SkImageInfo& resultInfo, const SkIRect& srcRect,
    SkImage::RescaleGamma rescaleGamma, SkFilterQuality rescaleQuality,
    SkImage::ReadPixelsCallback callback, SkImage::ReadPixelsContext context) {
  ...
  if (rescaleGamma == SkSurface::RescaleGamma::kLinear && bmp.info().colorSpace() &&
    ...
    auto ii =
        SkImageInfo::Make(srcW, srcH, kRGBA_F16_SkColorType, bmp.info().alphaType(), std::move(cs));
