...
SkImage_GpuBase::SkImage_GpuBase(sk_sp<GrImageContext> context, SkImageInfo info, uint32_t uniqueID)
    : INHERITED(std::move(info), uniqueID), fContext(std::move(context)) {}
...
sk_sp<SkImage> SkImage_GpuBase::onMakeSubset(const SkIRect& subset, GrDirectContext* direct) const {
  ...
  auto copyView = GrSurfaceProxyView::Copy(
      direct, std::move(view), GrMipmapped::kNo, subset, SkBackingFit::kExact, isBudgeted);
  ...
  return sk_make_sp<SkImage_Gpu>(
      sk_ref_sp(direct), kNeedNewImageUniqueID, std::move(copyView), this->imageInfo().colorInfo());
}
...
sk_sp<GrTextureProxy> SkImage_GpuBase::MakePromiseImageLazyProxy(
    GrContextThreadSafeProxy* tsp, SkISize dimensions, GrBackendFormat backendFormat,
  ...
  class PromiseLazyInstantiateCallback {
   public:
    PromiseLazyInstantiateCallback(
        PromiseImageTextureFulfillProc fulfillProc, sk_sp<skgpu::RefCntedCallback> releaseHelper)
