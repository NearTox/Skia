...
bool SkImage_GpuBase::getROPixels(SkBitmap* dst, CachingHint chint) const {
  ...
  GrColorType grColorType = SkColorTypeAndFormatToGrColorType(
      fContext->priv().caps(), this->colorType(), texProxy->backendFormat());
...
sk_sp<SkImage> SkImage_GpuBase::onMakeSubset(
  ...
  sk_sp<GrTextureProxy> copyProxy = GrSurfaceProxy::Copy(
      context, proxy.get(), GrMipMapped::kNo, subset, SkBackingFit::kExact, proxy->isBudgeted());
  ...
  return sk_make_sp<SkImage_Gpu>(
      fContext, kNeedNewImageUniqueID, this->alphaType(), std::move(copyProxy),
      this->refColorSpace());
}
...
sk_sp<GrTextureProxy> SkImage_GpuBase::asTextureProxyRef(
  ...
  GrTextureAdjuster adjuster(
      fContext.get(), this->asTextureProxyRef(context), SkColorTypeToGrColorType(this->colorType()),
      this->alphaType(), this->uniqueID(), this->colorSpace());
    ...
    GrSurfaceProxy::LazyCallbackResult operator()(GrResourceProvider* resourceProvider) {
      ...
      if (auto surf = resourceProvider->findByUniqueKey<GrSurface>(key)) {
        ...
        if ((tex = resourceProvider->wrapBackendTexture(
                 backendTexture, fColorType, kBorrow_GrWrapOwnership, GrWrapCacheable::kYes,
                 kRead_GrIOType))) {
