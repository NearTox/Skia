...
sk_sp<SkImage> SkImage_GpuBase::onMakeSubset(
  ...
  sk_sp<GrSurfaceContext> sContext(context->priv().makeDeferredSurfaceContext(
      format, desc, proxy->origin(), GrMipMapped::kNo, SkBackingFit::kExact, proxy->isBudgeted()));
  if (!sContext) {
    return nullptr;
  }
  ...
  return sk_make_sp<SkImage_Gpu>(
      fContext, kNeedNewImageUniqueID, this->alphaType(), sContext->asTextureProxyRef(),
      this->refColorSpace());
}
...
bool SkImage_GpuBase::onReadPixels(
  ...
  uint32_t flags = 0;
  if (kUnpremul_SkAlphaType == rec.fInfo.alphaType() && kPremul_SkAlphaType == this->alphaType()) {
  ...
  //
  if (kPremul_SkAlphaType == rec.fInfo.alphaType() && kUnpremul_SkAlphaType == this->alphaType()) {
...
sk_sp<GrTextureProxy> SkImage_GpuBase::asTextureProxyRef(
  ...
  GrTextureAdjuster adjuster(
      fContext.get(), this->asTextureProxyRef(context), this->alphaType(), this->uniqueID(),
      this->colorSpace());
...
bool SkImage_GpuBase::MakeTempTextureProxies(
  ...
  for (int textureIndex = 0; textureIndex < numTextures; ++textureIndex) {
    ...
    tempTextureProxies[textureIndex] = proxyProvider->wrapBackendTexture(
        yuvaTexturesCopy[textureIndex], imageOrigin, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo,
        kRead_GrIOType);
...
bool SkImage_GpuBase::RenderYUVAToRGBA(
  ...
  auto fp =
      GrYUVtoRGBEffect::Make(proxies, yuvaIndices, yuvColorSpace, GrSamplerState::Filter::kNearest);
  if (colorSpaceXform) {
...
sk_sp<GrTextureProxy> SkImage_GpuBase::MakePromiseImageLazyProxy(
  ...
  class PromiseLazyInstantiateCallback {
   public:
    PromiseLazyInstantiateCallback(
        ...
        GrPixelConfig config, PromiseImageApiVersion version)
  ...
  return proxyProvider->createLazyProxy(
      std::move(callback), backendFormat, desc, origin, mipMapped,
