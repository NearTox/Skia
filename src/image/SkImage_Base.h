...
class SkImage_Base : public SkImage {
 public:
  ...
  virtual void onAsyncRescaleAndReadPixelsYUV420(
      ...
      const SkISize& dstSize, RescaleGamma, RescaleMode, ReadPixelsCallback,
      ReadPixelsContext) const;
  ...
  std::tuple<GrSurfaceProxyView, GrColorType> asView(
      GrRecordingContext* context, GrMipmapped mipmapped,
  ...
  std::unique_ptr<GrFragmentProcessor> asFragmentProcessor(
      GrRecordingContext*, SkSamplingOptions, const SkTileMode[2], const SkMatrix&,
      const SkRect* subset = nullptr, const SkRect* domain = nullptr) const;
  ...
  std::tuple<skgpu::graphite::TextureProxyView, SkColorType> asView(
      skgpu::graphite::Recorder*, skgpu::graphite::Mipmapped mipmapped) const;
#endif
 ...
 protected:
  ...
  static GrSurfaceProxyView CopyView(
      GrRecordingContext*, GrSurfaceProxyView src, GrMipmapped, GrImageTexGenPolicy);

  static std::unique_ptr<GrFragmentProcessor> MakeFragmentProcessorFromView(
      GrRecordingContext*, GrSurfaceProxyView, SkAlphaType, SkSamplingOptions, const SkTileMode[2],
      const SkMatrix&, const SkRect* subset, const SkRect* domain);
   ...
   * function.
   */
  static GrSurfaceProxyView FindOrMakeCachedMipmappedView(
 ...
 private:
  ...
  virtual std::tuple<GrSurfaceProxyView, GrColorType> onAsView(
      GrRecordingContext*, GrMipmapped, GrImageTexGenPolicy policy) const = 0;

  virtual std::unique_ptr<GrFragmentProcessor> onAsFragmentProcessor(
      GrRecordingContext*, SkSamplingOptions, const SkTileMode[2], const SkMatrix&,
      const SkRect* subset, const SkRect* domain) const = 0;
#endif
#ifdef SK_GRAPHITE_ENABLED
...
#if SK_SUPPORT_GPU
inline GrSurfaceProxyView SkImage_Base::CopyView(
    GrRecordingContext* context, GrSurfaceProxyView src, GrMipmapped mipmapped,
    GrImageTexGenPolicy policy) {
  SkBudgeted budgeted =
      policy == GrImageTexGenPolicy::kNew_Uncached_Budgeted ? SkBudgeted::kYes : SkBudgeted::kNo;
  return GrSurfaceProxyView::Copy(
      context, std::move(src), mipmapped, SkBackingFit::kExact, budgeted);
}
#endif
