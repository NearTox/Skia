...
GrSurfaceProxyView SkImage_Lazy::textureProxyViewFromPlanes(
  ...
  for (int i = 0; i < SkYUVASizeInfo::kMaxCount; ++i) {
    ...
    SkBackingFit fit = (componentWidth != yuvSizeInfo.fSizes[0].fWidth) ||
    ...
    SkAssertResult(bitmap.installPixels(
        imageInfo, const_cast<void*>(planes[i]), yuvSizeInfo.fWidthBytes[i], releaseProc,
        dataStoragePtr));
    bitmap.setImmutable();
  ...
  std::unique_ptr<GrFragmentProcessor> colorConversionProcessor = GrColorSpaceXformEffect::Make(
      std::move(yuvToRgbProcessor), srcColorSpace, kOpaque_SkAlphaType, dstColorSpace,
      kOpaque_SkAlphaType);
...
sk_sp<SkCachedData> SkImage_Lazy::getPlanes(
    SkYUVASizeInfo* yuvaSizeInfo, SkYUVAIndex yuvaIndices[SkYUVAIndex::kIndexCount],
    SkYUVColorSpace* yuvColorSpace, const void* outPlanes[SkYUVASizeInfo::kMaxCount]) const {
  ScopedGenerator generator(fSharedGenerator);
  ...
  if (data) {
    ...
    for (int i = 1; i < SkYUVASizeInfo::kMaxCount; ++i) {
      ...
      planes[i] = (uint8_t*)planes[i - 1] +
                  (yuvInfo.fSizeInfo.fWidthBytes[i - 1] * yuvInfo.fSizeInfo.fSizes[i - 1].fHeight);
    }
  } else {
    // Fetch yuv plane sizes for memory allocation.
    if (!generator->queryYUVA8(&yuvInfo.fSizeInfo, yuvInfo.fYUVAIndices, &yuvInfo.fColorSpace)) {
      return nullptr;
    }
...
GrSurfaceProxyView SkImage_Lazy::lockTextureProxyView(
    GrRecordingContext* ctx, GrImageTexGenPolicy texGenPolicy, GrMipmapped mipMapped) const {
  ...
  if (key.isValid()) {
    ...
    if (proxy) {
      SK_HISTOGRAM_ENUMERATION(
          "LockTexturePath", kPreExisting_LockTexturePath, kLockTexturePathCount);
  ...
  // 2. Ask the generator to natively create one.
  {
    ScopedGenerator generator(fSharedGenerator);
    if (auto view =
            generator->generateTexture(ctx, this->imageInfo(), {0, 0}, mipMapped, texGenPolicy)) {
  ...
  if (SkBitmap bitmap; this->getROPixels(&bitmap, hint)) {
    ...
    if (view) {
      installKey(view);
      SK_HISTOGRAM_ENUMERATION("LockTexturePath", kRGBA_LockTexturePath, kLockTexturePathCount);
      return view;
    }
  }
