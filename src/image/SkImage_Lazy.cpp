...
bool SkImage_Lazy::getROPixels(SkBitmap* bitmap, SkImage::CachingHint chint) const {
  ...
  if (SkImage::kAllow_CachingHint == chint) {
    ...
    if (!bitmap->tryAllocPixels(this->imageInfo()) ||
        !generate_pixels(
            ScopedGenerator(fSharedGenerator), bitmap->pixmap(), fOrigin.x(), fOrigin.y())) {
      return false;
    }
    bitmap->setImmutable();
  }
...
#if SK_SUPPORT_GPU
sk_sp<GrTextureProxy> SkImage_Lazy::asTextureProxyRef(
...
sk_sp<SkImage> SkImage_Lazy::onMakeSubset(
    GrRecordingContext* context, const SkIRect& subset) const {
...
sk_sp<SkImage> SkImage_Lazy::onMakeColorTypeAndColorSpace(
    GrRecordingContext*, SkColorType targetCT, sk_sp<SkColorSpace> targetCS) const {
...
sk_sp<SkImage> SkImage::MakeFromGenerator(
    std::unique_ptr<SkImageGenerator> generator, const SkIRect* subset) {
  SkImage_Lazy::Validator validator(
      SharedGenerator::Make(std::move(generator)), subset, nullptr, nullptr);
...
class Generator_GrYUVProvider : public GrYUVProvider {
 ...
 private:
  ...
  bool onGetYUVA8Planes(
      const SkYUVASizeInfo& sizeInfo, const SkYUVAIndex yuvaIndices[SkYUVAIndex::kIndexCount],
...
static void set_key_on_proxy(
    GrProxyProvider* proxyProvider, GrTextureProxy* proxy, GrTextureProxy* originalProxy,
    const GrUniqueKey& key) {
  if (key.isValid()) {
    if (originalProxy && originalProxy->getUniqueKey().isValid()) {
      SkASSERT(originalProxy->getUniqueKey() == key);
      SkASSERT(
          GrMipMapped::kYes == proxy->mipMapped() &&
...
sk_sp<SkCachedData> SkImage_Lazy::getPlanes(
    ...
    SkYUVColorSpace* yuvColorSpace, const void* planes[SkYUVASizeInfo::kMaxCount]) {
  ScopedGenerator generator(fSharedGenerator);
...
sk_sp<GrTextureProxy> SkImage_Lazy::lockTextureProxy(
    GrRecordingContext* ctx, const GrUniqueKey& origKey, SkImage::CachingHint chint,
    bool willBeMipped, GrTextureMaker::AllowedTexGenType genType) const {
  ...
  if (!proxy && this->getROPixels(&bitmap, chint)) {
    ...
    std::tie(proxy, std::ignore) =
        bitmapMaker.refTextureProxy(willBeMipped ? GrMipMapped::kYes : GrMipMapped::kNo);
...
sk_sp<SkImage> SkImage::DecodeToTexture(
    GrContext* ctx, const void* encoded, size_t length, const SkIRect* subset) {
