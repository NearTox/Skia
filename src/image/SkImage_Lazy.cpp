...
GrSurfaceProxyView SkImage_Lazy::textureProxyViewFromPlanes(
  ...
  std::unique_ptr<GrFragmentProcessor> yuvToRgbProcessor = GrYUVtoRGBEffect::Make(
      yuvViews, yuvaIndices, yuvaPixmaps.yuvaInfo().yuvColorSpace(),
      GrSamplerState::Filter::kNearest, *ctx->priv().caps());
  ...
  SkMatrix m =
      SkEncodedOriginToMatrix(yuvaPixmaps.yuvaInfo().origin(), this->width(), this->height());
