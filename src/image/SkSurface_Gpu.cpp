...
sk_sp<SkImage> SkSurface_Gpu::onNewImageSnapshot(const SkIRect* subset) {
  ...
  if (subset || !srcView.asTextureProxy() || sdc->refsWrappedObjects()) {
    ...
    srcView = GrSurfaceProxyView::Copy(
        rContext, std::move(srcView), sdc->mipmapped(), rect, SkBackingFit::kExact, budgeted);
  }
...
bool SkSurface_Gpu::onCharacterize(SkSurfaceCharacterization* characterization) const {
  ...
  bool mipmapped =
      sdc->asTextureProxy() ? GrMipmapped::kYes == sdc->asTextureProxy()->mipmapped() : false;
  ...
  SkImageInfo ii = SkImageInfo::Make(
      sdc->width(), sdc->height(), ct, kPremul_SkAlphaType, sdc->colorInfo().refColorSpace());

  GrBackendFormat format = sdc->asSurfaceProxy()->backendFormat();

  characterization->set(
      direct->threadSafeProxy(), maxResourceBytes, ii, format, sdc->origin(), sdc->numSamples(),
      ...
      sdc->asRenderTargetProxy()->isProtected(), this->props());
  return true;
}
...
bool SkSurface_Gpu::onIsCompatible(const SkSurfaceCharacterization& characterization) const {
  ...
  if (characterization.isTextureable()) {
    ...
    if (characterization.isMipMapped() && GrMipmapped::kNo == sdc->asTextureProxy()->mipmapped()) {
...
sk_sp<SkSurface> SkSurface::MakeRenderTarget(
  ...
  auto sdc = GrSurfaceDrawContext::Make(
      context, grColorType, c.refColorSpace(), SkBackingFit::kExact, c.dimensions(),
  ...
  sk_sp<SkGpuDevice> device(
      SkGpuDevice::Make(context, std::move(sdc), SkGpuDevice::kClear_InitContents));
  if (!device) {
    return nullptr;
  }
...
sk_sp<SkSurface> SkSurface_Gpu::MakeWrappedRenderTarget(
    GrRecordingContext* context, std::unique_ptr<GrSurfaceDrawContext> sdc) {
  if (!context) {
    return nullptr;
  }
...
sk_sp<SkSurface> SkSurface::MakeFromBackendTexture(
    GrRecordingContext* context, const GrBackendTexture& tex, GrSurfaceOrigin origin, int sampleCnt,
...
bool SkSurface_Gpu::onReplaceBackendTexture(
  ...
  auto sdc = GrSurfaceDrawContext::MakeFromBackendTexture(
      context, oldSDC->colorInfo().colorType(), std::move(colorSpace), backendTexture, sampleCnt,
...
sk_sp<SkSurface> SkSurface::MakeFromBackendRenderTarget(
    GrRecordingContext* context, const GrBackendRenderTarget& rt, GrSurfaceOrigin origin,
    SkColorType colorType, sk_sp<SkColorSpace> colorSpace, const SkSurfaceProps* props,
  ...
  auto sdc = GrSurfaceDrawContext::MakeFromBackendRenderTarget(
...
sk_sp<SkSurface> SkSurface::MakeFromAHardwareBuffer(
  ...
  if (isTextureable) {
    ...
    sk_sp<SkSurface> surface = SkSurface::MakeFromBackendTexture(
        dContext, backendTexture, origin, 0, colorType, std::move(colorSpace), surfaceProps,
