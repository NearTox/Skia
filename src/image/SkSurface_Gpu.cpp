...
void SkSurface_Gpu::onDraw(SkCanvas* canvas, SkScalar x, SkScalar y, const SkPaint* paint) {
  ...
  auto tryDraw = [&] {
    ...
    sk_sp<SkImage> image;
    image = sk_make_sp<SkImage_Gpu>(
        sk_ref_sp(canvasContext), kNeedNewImageUniqueID, std::move(view), info.colorType(),
        info.alphaType(), info.refColorSpace());
...
bool SkSurface_Gpu::onIsCompatible(const SkSurfaceCharacterization& characterization) const {
  ...
  return characterization.contextInfo() && characterization.contextInfo()->priv().matches(direct) &&
...
sk_sp<SkSurface> SkSurface::MakeRenderTarget(
  ...
  auto rtc = GrRenderTargetContext::Make(
      context, grColorType, c.refColorSpace(), SkBackingFit::kExact, {c.width(), c.height()},
      c.sampleCount(), GrMipmapped(c.isMipMapped()), c.isProtected(), c.origin(), budgeted,
      &c.surfaceProps());
  if (!rtc) {
    return nullptr;
  }
  ...
  sk_sp<SkGpuDevice> device(
      SkGpuDevice::Make(context, std::move(rtc), SkGpuDevice::kClear_InitContents));
  if (!device) {
    return nullptr;
  }
...
static bool validate_backend_texture(
    const GrCaps* caps, const GrBackendTexture& tex, int sampleCnt, GrColorType grCT,
    bool texturable) {
  if (!tex.isValid()) {
    return false;
  }
...
sk_sp<SkSurface> SkSurface::MakeRenderTarget(
    GrRecordingContext* ctx, SkBudgeted budgeted, const SkImageInfo& info, int sampleCount,
...
sk_sp<SkSurface> SkSurface::MakeRenderTarget(
    ...
    GrSurfaceOrigin surfaceOrigin, const SkSurfaceProps* surfaceProps, bool shouldCreateWithMips) {
  return MakeRenderTarget(
      static_cast<GrRecordingContext*>(context), budgeted, imageInfo, sampleCount, surfaceOrigin,
      surfaceProps, shouldCreateWithMips);
}
...
sk_sp<SkSurface> SkSurface::MakeRenderTarget(
  ...
  return MakeRenderTarget(
      static_cast<GrRecordingContext*>(context), budgeted, imageInfo, sampleCount, surfaceProps);
}
...
bool SkSurface_Gpu::onReplaceBackendTexture(
  ...
  auto rtc = GrRenderTargetContext::MakeFromBackendTexture(
      context, oldRTC->colorInfo().colorType(), std::move(colorSpace), backendTexture, sampleCnt,
      origin, &this->props(), std::move(releaseHelper));
  if (!rtc) {
    return false;
  }
...
bool validate_backend_render_target(
    const GrCaps* caps, const GrBackendRenderTarget& rt, GrColorType grCT) {
...
sk_sp<SkSurface> SkSurface::MakeFromBackendRenderTarget(
  ...
  auto rtc = GrRenderTargetContext::MakeFromBackendRenderTarget(
      context, grColorType, std::move(colorSpace), rt, origin, props, relProc, releaseContext);
  if (!rtc) {
    return nullptr;
  }
  callProc.clear();
...
sk_sp<SkSurface> SkSurface::MakeFromAHardwareBuffer(
  ...
  GrBackendFormat backendFormat =
      GrAHardwareBufferUtils::GetBackendFormat(direct, hardwareBuffer, bufferDesc.format, true);
  if (!backendFormat.isValid()) {
    return nullptr;
  }
  ...
  if (isTextureable) {
    ...
    GrBackendTexture backendTexture = GrAHardwareBufferUtils::MakeBackendTexture(
        direct, hardwareBuffer, bufferDesc.width, bufferDesc.height, &deleteImageProc,
    ...
    sk_sp<SkSurface> surface = SkSurface::MakeFromBackendTexture(
        direct, backendTexture, origin, 0, colorType, std::move(colorSpace), surfaceProps,
