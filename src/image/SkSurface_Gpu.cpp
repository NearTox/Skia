...
void SkSurface_Gpu::onAsyncRescaleAndReadPixels(
    ...
    SkFilterQuality rescaleQuality, SkSurface::ReadPixelsCallback callback,
...
bool SkSurface_Gpu::onCharacterize(SkSurfaceCharacterization* characterization) const {
  ...
  characterization->set(
      ctx->threadSafeProxy(), maxResourceBytes, ii, rtc->origin(), rtc->colorSpaceInfo().config(),
      rtc->fsaaType(), rtc->numStencilSamples(),
...
bool SkSurface_Gpu::Valid(const SkImageInfo& info) { return true; }

bool SkSurface_Gpu::Valid(const GrCaps* caps, GrPixelConfig config, SkColorSpace* colorSpace) {
  switch (config) {
    case kSRGBA_8888_GrPixelConfig:
    case kSBGRA_8888_GrPixelConfig: return caps->srgbSupport();
    default: return true;
  }
}
...
sk_sp<SkSurface> SkSurface::MakeRenderTarget(
  ...
  sk_sp<GrSurfaceContext> sc(context->priv().makeDeferredSurfaceContext(
      format, desc, c.origin(), GrMipMapped(c.isMipMapped()), SkBackingFit::kExact, budgeted,
  ...
  sk_sp<SkGpuDevice> device(SkGpuDevice::Make(
      context->priv().backdoor(), sk_ref_sp(sc->asRenderTargetContext()), c.width(), c.height(),
...
bool validate_backend_texture(
    GrContext* ctx, const GrBackendTexture& tex, GrPixelConfig* config, int sampleCnt,
    SkColorType ct, sk_sp<SkColorSpace> cs, bool texturable) {
  if (!tex.isValid()) {
    return false;
  }
...
sk_sp<SkSurface> SkSurface::MakeFromBackendTexture(
  ...
  if (!validate_backend_texture(
          context, texCopy, &texCopy.fConfig, sampleCnt, colorType, colorSpace, true)) {
    return nullptr;
  }
  ...
  sk_sp<GrRenderTargetContext> rtc(context->priv().makeBackendTextureRenderTargetContext(
      texCopy, origin, sampleCnt, std::move(colorSpace), props, textureReleaseProc,
      releaseContext));
  if (!rtc) {
    return nullptr;
  }
...
bool SkSurface_Gpu::onReplaceBackendTexture(
  ...
  if (!validate_backend_texture(
          context, texCopy, &texCopy.fConfig, sampleCnt, this->getCanvas()->imageInfo().colorType(),
          colorSpace, true)) {
    return false;
  }
  sk_sp<GrRenderTargetContext> rtc(context->priv().makeBackendTextureRenderTargetContext(
      texCopy, origin, sampleCnt, std::move(colorSpace), &this->props(), releaseProc,
      releaseContext));
  if (!rtc) {
    return false;
  }
...
bool validate_backend_render_target(
    GrContext* ctx, const GrBackendRenderTarget& rt, GrPixelConfig* config, SkColorType ct,
...
sk_sp<SkSurface> SkSurface::MakeFromBackendTextureAsRenderTarget(
  ...
  GrBackendTexture texCopy = tex;
  if (!validate_backend_texture(
          context, texCopy, &texCopy.fConfig, sampleCnt, colorType, colorSpace, false)) {
    return nullptr;
  }
