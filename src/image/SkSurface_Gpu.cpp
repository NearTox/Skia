...
static GrRenderTarget* prepare_rt_for_external_access(
    SkSurface_Gpu* surface, SkSurface::BackendHandleAccess access) {
  switch (access) {
    case SkSurface::kFlushRead_BackendHandleAccess: break;
...
sk_sp<SkSurface> SkSurface_Gpu::onNewSurface(const SkImageInfo& info) {
  ...
  return SkSurface::MakeRenderTarget(
      fDevice->context(), kBudgeted, info, sampleCount, origin, &this->props());
}
...
sk_sp<SkImage> SkSurface_Gpu::onNewImageSnapshot(const SkIRect* subset) {
  ...
  if (srcProxy) {
    ...
    image = sk_make_sp<SkImage_Gpu>(
        sk_ref_sp(ctx), kNeedNewImageUniqueID, info.alphaType(), std::move(srcProxy),
        info.refColorSpace());
  }
  return image;
}
...
void SkSurface_Gpu::onAsyncRescaleAndReadPixels(
...
void SkSurface_Gpu::onDiscard() { fDevice->accessRenderTargetContext()->discard(); }
...
bool SkSurface_Gpu::onCharacterize(SkSurfaceCharacterization* characterization) const {
  ...
  characterization->set(
      ctx->threadSafeProxy(), maxResourceBytes, ii, format, rtc->origin(), rtc->numSamples(),
      ...
      GrProtected(rtc->asRenderTargetProxy()->isProtected()), this->props());
  return true;
}
...
void SkSurface_Gpu::onDraw(SkCanvas* canvas, SkScalar x, SkScalar y, const SkPaint* paint) {
  ...
  auto tryDraw = [&] {
    ...
    sk_sp<SkImage> image;
    image = sk_make_sp<SkImage_Gpu>(
        sk_ref_sp(context), kNeedNewImageUniqueID, info.alphaType(), std::move(srcProxy),
        info.refColorSpace());
...
bool SkSurface_Gpu::onIsCompatible(const SkSurfaceCharacterization& characterization) const {
  ...
  if (characterization.isTextureable()) {
    ...
    if (characterization.isMipMapped() && GrMipMapped::kNo == rtc->asTextureProxy()->mipMapped()) {
...
sk_sp<SkSurface> SkSurface::MakeRenderTarget(
  ...
  sk_sp<GrRenderTargetContext> rtc(context->priv().makeDeferredRenderTargetContext(
      SkBackingFit::kExact, c.width(), c.height(), grColorType, c.refColorSpace(), c.sampleCount(),
      GrMipMapped(c.isMipMapped()), c.origin(), &c.surfaceProps(), budgeted, c.isProtected()));
  if (!rtc) {
    return nullptr;
  }
  ...
  sk_sp<SkGpuDevice> device(SkGpuDevice::Make(
      context->priv().backdoor(), std::move(rtc), c.width(), c.height(),
...
static bool validate_backend_texture(
    GrContext* ctx, const GrBackendTexture& tex, GrPixelConfig* config, int sampleCnt,
    GrColorType grCT, bool texturable) {
  if (!tex.isValid()) {
    return false;
  }
...
sk_sp<SkSurface> SkSurface::MakeFromBackendTexture(
  ...
  if (!validate_backend_texture(context, texCopy, &texCopy.fConfig, c.sampleCount(), grCT, true)) {
    return nullptr;
  }
  ...
  sk_sp<GrRenderTargetContext> rtc(context->priv().makeBackendTextureRenderTargetContext(
      texCopy, c.origin(), c.sampleCount(), grCT, c.refColorSpace(), &c.surfaceProps(),
  ...
  sk_sp<SkGpuDevice> device(SkGpuDevice::Make(
      context, std::move(rtc), texCopy.width(), texCopy.height(),
...
sk_sp<SkSurface> SkSurface::MakeRenderTarget(
    ...
    GrSurfaceOrigin origin, const SkSurfaceProps* props, bool shouldCreateWithMips) {
  if (!ctx) {
    return nullptr;
  }
...
sk_sp<SkSurface> SkSurface_Gpu::MakeWrappedRenderTarget(
    GrContext* context, sk_sp<GrRenderTargetContext> rtc) {
  if (!context) {
    return nullptr;
  }
...
sk_sp<SkSurface> SkSurface::MakeFromBackendTexture(
    GrContext* context, const GrBackendTexture& tex, GrSurfaceOrigin origin, int sampleCnt,
  ...
  GrColorType grColorType =
      SkColorTypeAndFormatToGrColorType(context->priv().caps(), colorType, tex.getBackendFormat());
  if (grColorType == GrColorType::kUnknown) {
    return nullptr;
  }

  GrBackendTexture texCopy = tex;
  if (!validate_backend_texture(context, texCopy, &texCopy.fConfig, sampleCnt, grColorType, true)) {
    return nullptr;
  }
  ...
  sk_sp<GrRenderTargetContext> rtc(context->priv().makeBackendTextureRenderTargetContext(
      texCopy, origin, sampleCnt, grColorType, std::move(colorSpace), props, textureReleaseProc,
      releaseContext));
  if (!rtc) {
    return nullptr;
  }
...
bool SkSurface_Gpu::onReplaceBackendTexture(
    const GrBackendTexture& backendTexture, GrSurfaceOrigin origin, TextureReleaseProc releaseProc,
  ...
  sk_sp<GrRenderTargetContext> rtc(context->priv().makeBackendTextureRenderTargetContext(
      texCopy, origin, sampleCnt, oldRTC->colorSpaceInfo().colorType(), std::move(colorSpace),
...
bool validate_backend_render_target(
    const GrCaps* caps, const GrBackendRenderTarget& rt, GrPixelConfig* config, GrColorType grCT) {
...
sk_sp<SkSurface> SkSurface::MakeFromBackendRenderTarget(
    GrContext* context, const GrBackendRenderTarget& rt, GrSurfaceOrigin origin,
    SkColorType colorType, sk_sp<SkColorSpace> colorSpace, const SkSurfaceProps* props,
  ...
  GrColorType grColorType =
      SkColorTypeAndFormatToGrColorType(context->priv().caps(), colorType, rt.getBackendFormat());
  if (grColorType == GrColorType::kUnknown) {
    return nullptr;
  }

  GrBackendRenderTarget rtCopy = rt;
  if (!validate_backend_render_target(
          context->priv().caps(), rtCopy, &rtCopy.fConfig, grColorType)) {
    return nullptr;
  }
  ...
  sk_sp<SkGpuDevice> device(SkGpuDevice::Make(
      context, std::move(rtc), rtCopy.width(), rtCopy.height(), SkGpuDevice::kUninit_InitContents));
  if (!device) {
    return nullptr;
  }
...
sk_sp<SkSurface> SkSurface::MakeFromBackendTextureAsRenderTarget(
  ...
  if (!validate_backend_texture(
          context, texCopy, &texCopy.fConfig, sampleCnt, grColorType, false)) {
    return nullptr;
  }
  ...
  sk_sp<SkGpuDevice> device(SkGpuDevice::Make(
      context, std::move(rtc), tex.width(), tex.height(), SkGpuDevice::kUninit_InitContents));
  if (!device) {
    return nullptr;
  }
  return sk_make_sp<SkSurface_Gpu>(std::move(device));
...
sk_sp<SkSurface> SkSurface::MakeFromAHardwareBuffer(
  ...
  if (isTextureable) {
    ...
    GrBackendTexture backendTexture = GrAHardwareBufferUtils::MakeBackendTexture(
        ...
        &deleteImageCtx, isProtectedContent, backendFormat, true);
    if (!backendTexture.isValid()) {
      return nullptr;
    }
