...
GrContext* SkSurface_Gpu::onGetContext() noexcept { return fDevice->context(); }
...
sk_sp<SkImage> SkSurface_Gpu::onNewImageSnapshot(const SkIRect* subset) {
  ...
  if (subset || !srcView.asTextureProxy() || rtc->priv().refsWrappedObjects()) {
    ...
    srcView = GrSurfaceProxyView::Copy(
        ctx, std::move(srcView), rtc->mipMapped(), rect, SkBackingFit::kExact, budgeted);
  }
  ...
  if (srcView.asTextureProxy()) {
    ...
    image = sk_make_sp<SkImage_Gpu>(
        sk_ref_sp(ctx), kNeedNewImageUniqueID, std::move(srcView), info.colorType(),
...
void SkSurface_Gpu::onDraw(SkCanvas* canvas, SkScalar x, SkScalar y, const SkPaint* paint) {
  ...
  auto tryDraw = [&] {
    ...
    sk_sp<SkImage> image;
    image = sk_make_sp<SkImage_Gpu>(
        sk_ref_sp(context), kNeedNewImageUniqueID, std::move(view), info.colorType(),
        info.alphaType(), info.refColorSpace());
...
sk_sp<SkSurface> SkSurface::MakeRenderTarget(
  ...
  sk_sp<SkGpuDevice> device(SkGpuDevice::Make(
      context->priv().backdoor(), std::move(rtc), SkGpuDevice::kClear_InitContents));
  if (!device) {
    return nullptr;
  }
...
bool SkSurface_Gpu::onReplaceBackendTexture(
  ...
  auto rtc = GrRenderTargetContext::MakeFromBackendTexture(
      ...
      origin, &this->props(), releaseProc, releaseContext);
  if (!rtc) {
    return false;
  }
  callProc.clear();
