...
skgpu::BaseDevice* SkSurface_Gpu::getDevice() { return fDevice.get(); }
...
sk_sp<SkImage> SkSurface_Gpu::onNewImageSnapshot(const SkIRect* subset) {
  ...
  if (subset || !srcView.asTextureProxy() || rtp->refsWrappedObjects()) {
    ...
    if (!subset && srcView.asTextureProxy()) {
      return SkImage_Gpu::MakeWithVolatileSrc(
    ...
    srcView = GrSurfaceProxyView::Copy(
        rContext, std::move(srcView), mipmapped, rect, SkBackingFit::kExact, budgeted);
  }
...
void SkSurface_Gpu::onAsyncRescaleAndReadPixelsYUV420(
  ...
  fDevice->asyncRescaleAndReadPixelsYUV420(
...
bool SkSurface_Gpu::onCharacterize(SkSurfaceCharacterization* characterization) const {
  ...
  characterization->set(
      direct->threadSafeProxy(), maxResourceBytes, ii, format, readSurfaceView.origin(), numSamples,
      ...
      SkSurfaceCharacterization::VulkanSecondaryCBCompatible(false), isProtected, this->props());
  return true;
}
...
void SkSurface_Gpu::onDraw(
  ...
  auto tryDraw = [&] {
    ...
    sk_sp<SkImage> image = sk_make_sp<SkImage_Gpu>(
        sk_ref_sp(canvasContext), kNeedNewImageUniqueID, std::move(srcView), info.colorInfo());
...
sk_sp<SkSurface> SkSurface::MakeRenderTarget(
    GrRecordingContext* rContext, const SkSurfaceCharacterization& c, SkBudgeted budgeted) {
  ...
  auto device = rContext->priv().createDevice(
      budgeted, c.imageInfo(), SkBackingFit::kExact, c.sampleCount(), GrMipmapped(c.isMipMapped()),
      c.isProtected(), c.origin(), c.surfaceProps(), skgpu::BaseDevice::InitContents::kClear);
  if (!device) {
    return nullptr;
  }
...
sk_sp<SkSurface> SkSurface::MakeRenderTarget(
    GrRecordingContext* rContext, SkBudgeted budgeted, const SkImageInfo& info, int sampleCount,
...
sk_sp<SkSurface> SkSurface::MakeFromBackendTexture(
  ...
  auto device = rContext->priv().createDevice(
      grColorType, std::move(proxy), std::move(colorSpace), origin,
      SkSurfacePropsCopyOrDefault(props), skgpu::BaseDevice::InitContents::kUninit);
  if (!device) {
    return nullptr;
  }
...
bool SkSurface_Gpu::onReplaceBackendTexture(
  ...
  return fDevice->replaceBackingProxy(
      mode, sk_ref_sp(proxy->asRenderTargetProxy()), grColorType, std::move(colorSpace), origin,
      this->props());
}
