...
public:
    ...
    public:
        ...
        Iterator(SubRun* subRun) noexcept : fPtr{subRun} {}
        ...
        bool operator==(const Iterator& rhs) const noexcept { return fPtr == rhs.fPtr; }
        bool operator!=(const Iterator& rhs) const noexcept { return fPtr != rhs.fPtr; }
        reference operator*() noexcept { return *fPtr; }

    private:
        SubRun* fPtr;
    };

    void append(SubRunOwner subRun) noexcept {
        SubRunOwner* newTail = &subRun->fNext;
        *fTail = std::move(subRun);
        fTail = newTail;
    }
    bool isEmpty() const noexcept { return fHead == nullptr; }
...
public:
    ...
    const SkMatrix& initialPosition() const noexcept { return fInitialPositionMatrix; }
