...
BagOfBytes::BagOfBytes(size_t firstHeapAllocation) : BagOfBytes(nullptr, 0, firstHeapAllocation) {}
...
BagOfBytes::Block::Block(char* previous, char* startOfBlock)
    : fBlockStart{startOfBlock}, fPrevious{reinterpret_cast<Block*>(previous)} {}
...
void BagOfBytes::needMoreBytes(int requestedSize, int alignment) {
  ...
  const int size =
      PlatformMinimumSizeWithOverhead(std::max(requestedSize, nextBlockSize), kAllocationAlignment);
