...
class BagOfBytes {
 public:
  ...
  static constexpr int PlatformMinimumSizeWithOverhead(int requestedSize, int assumedAlignment) {
    return MinimumSizeWithOverhead(requestedSize, assumedAlignment, sizeof(Block), kMaxAlignment);
  }
  ...
  static constexpr int MinimumSizeWithOverhead(
    ...
    int minimumSize =
        AlignUp(requestedSize, minAlignment) + blockSize + maxAlignment - minAlignment;
...
class SubRunInitializer {
 public:
  ...
  ~SubRunInitializer() { ::operator delete(fMemory); }
...
class SubRunAllocator {
 public:
  ...
  template <typename T>
  static std::tuple<SubRunInitializer<T>, int, SubRunAllocator> AllocateClassMemoryAndArena(
      int allocSizeHint) {
