...
#define FAIL_IF(cond, coin)                                    \
  do {                                                         \
    if (cond) log->record(SkPathOpsDebug::kFail_Glitch, coin); \
  } while (false)

#undef FAIL_WITH_NULL_IF
#define FAIL_WITH_NULL_IF(cond, span)                          \
  do {                                                         \
    if (cond) log->record(SkPathOpsDebug::kFail_Glitch, span); \
  } while (false)

#undef RETURN_FALSE_IF
#define RETURN_FALSE_IF(cond, span)                                   \
  do {                                                                \
    if (cond) log->record(SkPathOpsDebug::kReturnFalse_Glitch, span); \
  } while (false)
...
bool SkPathOpsDebug::ChaseContains(
    const SkTDArray<SkOpSpanBase*>& chaseArray, const SkOpSpanBase* span) {
...
struct SkPathOpsDebug::GlitchLog {
  void init(const SkOpGlobalState* state) { fGlobalState = state; }
  ...
  void record(GlitchType type, const SkOpSpanBase* base, const SkOpSpanBase* suspect = NULL) {
  ...
  void record(GlitchType type, const SkCoincidentSpans* coin, const SkCoincidentSpans* opp = NULL) {
    SpanGlitch* glitch = recordCommon(type);
  ...
  void record(
      GlitchType type, const SkOpSpanBase* base, const SkOpSegment* seg, double t, SkPoint pt) {
    SpanGlitch* glitch = recordCommon(type);
    glitch->fBase = base;
    glitch->fSegment = seg;
    glitch->fStartT = t;
  ...
  void record(GlitchType type, const SkOpSpanBase* base, double t, SkPoint pt) {
    SpanGlitch* glitch = recordCommon(type);
    glitch->fBase = base;
    glitch->fStartT = t;
    glitch->fPt = pt;
  ...
  void record(
      GlitchType type, const SkCoincidentSpans* coin, const SkOpPtT* coinSpan,
      const SkOpPtT* endSpan) {
    SpanGlitch* glitch = recordCommon(type);
    glitch->fCoinSpan = coin->coinPtTStart();
    glitch->fEndSpan = coin->coinPtTEnd();
  ...
  void record(GlitchType type, const SkCoincidentSpans* coin, const SkOpSpanBase* base) {
    SpanGlitch* glitch = recordCommon(type);
    glitch->fBase = base;
    glitch->fCoinSpan = coin->coinPtTStart();
    glitch->fEndSpan = coin->coinPtTEnd();
  ...
  void record(
      GlitchType type, const SkOpPtT* ptTS, const SkOpPtT* ptTE, const SkOpPtT* oPtTS,
      const SkOpPtT* oPtTE) {
    SpanGlitch* glitch = recordCommon(type);
  ...
  void record(
      GlitchType type, const SkOpSegment* seg, double startT, double endT,
      const SkOpSegment* oppSeg, double oppStartT, double oppEndT) {
    SpanGlitch* glitch = recordCommon(type);
    glitch->fSegment = seg;
  ...
  void record(GlitchType type, const SkOpSegment* seg, const SkOpSpan* span) {
    SpanGlitch* glitch = recordCommon(type);
    glitch->fSegment = seg;
    glitch->fBase = span;
  }
  ...
  void record(GlitchType type, const SkCoincidentSpans* coin, const SkOpPtT* ptT) {
    SpanGlitch* glitch = recordCommon(type);
    glitch->fCoinSpan = coin->coinPtTStart();
    glitch->fEndSpan = ptT;
  }
...
#if DEBUG_COIN
static void missing_coincidence(
    SkPathOpsDebug::GlitchLog* glitches, const SkOpContourHead* contourList) {
...
#  if DEBUG_COIN
  ...
  for (int index = 0; index < glitches.fGlitches.count(); ++index) {
    ...
    SkDebugf("%02d: ", index);
    if (glitch.fBase) {
      SkDebugf(" seg/base=%d/%d", glitch.fBase->segment()->debugID(), glitch.fBase->debugID());
    }
    if (glitch.fSuspect) {
      SkDebugf(
          " seg/base=%d/%d", glitch.fSuspect->segment()->debugID(), glitch.fSuspect->debugID());
    }
    if (glitch.fSegment) {
...
static const char* gOpStrs[] = {
    "kDifference_SkPathOp", "kIntersect_SkPathOp",         "kUnion_SkPathOp",
    "kXOR_PathOp",          "kReverseDifference_SkPathOp",
};

const char* SkPathOpsDebug::OpStr(SkPathOp op) { return gOpStrs[op]; }
...
void SkOpGlobalState::debugAddLoopCount(
    SkIntersections* i, const SkIntersectionHelper& wt, const SkIntersectionHelper& wn) {
  for (int index = 0; index < (int)SK_ARRAY_COUNT(fDebugLoopCount); ++index) {
    ...
    memcpy(
        &fDebugWorstPts[index * 2 * 4], wt.pts(),
        (SkPathOpsVerbToPoints(wt.segment()->verb()) + 1) * sizeof(SkPoint));
    memcpy(
        &fDebugWorstPts[(index * 2 + 1) * 4], wn.pts(),
...
void SkOpGlobalState::debugDoYourWorst(SkOpGlobalState* local) {
  for (int index = 0; index < (int)SK_ARRAY_COUNT(fDebugLoopCount); ++index) {
    ...
    memcpy(
        &fDebugWorstPts[index * 2 * 4], &local->fDebugWorstPts[index * 2 * 4], sizeof(SkPoint) * 8);
...
void SkOpGlobalState::debugLoopReport() {
  ...
  for (int index = 0; index < (int)SK_ARRAY_COUNT(fDebugLoopCount); ++index) {
    ...
    dump_curve(
        fDebugWorstVerb[index * 2], fDebugWorstPts[index * 2 * 4], fDebugWorstWeight[index * 2]);
    dump_curve(
        fDebugWorstVerb[index * 2 + 1], fDebugWorstPts[(index * 2 + 1) * 4],
...
bool SkOpGlobalState::DebugRunFail() { return SkPathOpsDebug::gRunFail; }
...
#if DEBUG_T_SECT_LOOP_COUNT
void SkIntersections::debugBumpLoopCount(DebugLoop index) { fDebugLoopCount[index]++; }

int SkIntersections::debugLoopCount(DebugLoop index) const { return fDebugLoopCount[index]; }

void SkIntersections::debugResetLoopCount() { sk_bzero(fDebugLoopCount, sizeof(fDebugLoopCount)); }
#endif
...
void SkOpSegment::debugMissingCoincidence(SkPathOpsDebug::GlitchLog* log) const {
  ...
  do {
    ...
    while ((ptT = ptT->next()) != spanStopPtT) {
      ...
      if (span && span->segment() != opp &&
          span->containsCoincidence(
              opp)) {  // debug has additional condition since it may be called before inner
                       // duplicate points have been deleted
        continue;
      }
      if (spanBase->segment() != opp &&
          spanBase->containsCoinEnd(
              opp)) {  // debug has additional condition since it may be called before inner
                       // duplicate points have been deleted
        continue;
      }
...
void SkOpSegment::debugMoveNearby(SkPathOpsDebug::GlitchLog* glitches) const {
  ...
  do {
    ...
    while ((ptT = ptT->next()) != headPtT) {
      ...
      if (ptT->segment() == this && !ptT->deleted() && test != spanBase && test->ptT() == ptT) {
        if (test->final()) {
          if (spanBase == &fHead) {
...
void SkOpSegment::debugShowNewWinding(const char* fun, const SkOpSpan* span, int winding) {
  ...
  SkDebugf(
      ") t=%1.9g [%d] (%1.9g,%1.9g) tEnd=%1.9g newWindSum=", span->t(), span->debugID(), pt.fX,
      pt.fY, span->next()->t());
...
void SkOpSegment::debugShowNewWinding(
    const char* fun, const SkOpSpan* span, int winding, int oppWinding) {
  const SkPoint& pt = span->ptT()->fPt;
  SkDebugf("%s id=%d", fun, this->debugID());
  SkDebugf(" (%1.9g,%1.9g", fPts[0].fX, fPts[0].fY);
  for (int vIndex = 1; vIndex <= SkPathOpsVerbToPoints(fVerb); ++vIndex) {
  ...
  SkDebugf(
      ") t=%1.9g [%d] (%1.9g,%1.9g) tEnd=%1.9g newWindSum=", span->t(), span->debugID(), pt.fX,
      pt.fY, span->next()->t(), winding, oppWinding);
  if (winding == SK_MinS32) {
    SkDebugf("?");
  } else {
    SkDebugf("%d", winding);
...
void SkOpAngle::debugCheckNearCoincidence() const {
  ...
  do {
    ...
    while ((next = next->fNext) != this) {
      ...
      SkDebugf(
          "%s distSq=%1.9g testId=%d nextId=%d\n", __FUNCTION__, distSq, testSegment->debugID(),
          nextSegment->debugID());
...
SkString SkOpAngle::debugPart() const {
  ...
  switch (this->segment()->verb()) {
    case SkPath::kLine_Verb:
      result.printf(
          LINE_DEBUG_STR " id=%d", LINE_DEBUG_DATA(fPart.fCurve), this->segment()->debugID());
      break;
    case SkPath::kQuad_Verb:
      result.printf(
          QUAD_DEBUG_STR " id=%d", QUAD_DEBUG_DATA(fPart.fCurve), this->segment()->debugID());
      break;
    case SkPath::kConic_Verb:
      result.printf(
          CONIC_DEBUG_STR " id=%d", CONIC_DEBUG_DATA(fPart.fCurve, fPart.fCurve.fConic.fWeight),
          this->segment()->debugID());
      break;
    case SkPath::kCubic_Verb:
      result.printf(
          CUBIC_DEBUG_STR " id=%d", CUBIC_DEBUG_DATA(fPart.fCurve), this->segment()->debugID());
      break;
    default: SkASSERT(0);
  }
  return result;
...
#ifdef SK_DEBUG
void SkCoincidentSpans::debugStartCheck(
    const SkOpSpanBase* outer, const SkOpSpanBase* over, const SkOpGlobalState* debugState) const {
...
void SkCoincidentSpans::debugCorrectOneEnd(
    SkPathOpsDebug::GlitchLog* log, const SkOpPtT* (SkCoincidentSpans::*getEnd)() const,
  ...
  const SkOpPtT* testPtT = prev ? prev->next()->ptT() : origSpan->upCast()->next()->prev()->ptT();
  if (origPtT != testPtT) {
...
// description below
void SkOpCoincidence::debugAddEndMovedSpans(
    SkPathOpsDebug::GlitchLog* log, const SkOpSpan* base, const SkOpSpanBase* testSpan) const {
  ...
  while ((testPtT = testPtT->next()) != stopPtT) {
    ...
    for (int index = 0; index < i.used(); ++index) {
      ...
      bool added;
      if (this->debugAddOrOverlap(log, coinSeg, oppSeg, coinTs, coinTe, oppTs, oppTe, &added),
          false) {
        return;
      }
    }
...
// description below
void SkOpCoincidence::debugAddEndMovedSpans(
    SkPathOpsDebug::GlitchLog* log, const SkOpPtT* ptT) const {
    ...
    while (test != end || oTest != oEnd) {
      ...
      if (!containedOpp || !containedThis) {
        ...
        if (containedOpp) {
          ...
          } while (!(walkOpp = walk->ptT()->contains(oSeg)) && walk != coin->coinPtTEnd()->span());
        ...
        bool addToOpp = !containedOpp && !containedThis ? startPart < oStartPart : !!containedThis;
        bool startOver = false;
        addToOpp
            ? log->record(
                  SkPathOpsDebug::kAddExpandedCoin_Glitch, oPriorT + oStartRange * startPart, test)
            : log->record(
                  SkPathOpsDebug::kAddExpandedCoin_Glitch, priorT + startRange * oStartPart, oTest);
...
void SkOpCoincidence::debugAddIfMissing(
    SkPathOpsDebug::GlitchLog* log, const SkOpPtT* over1s, const SkOpPtT* over2s, double tStart,
    double tEnd, const SkOpSegment* coinSeg, const SkOpSegment* oppSeg, bool* added,
...
void SkOpCoincidence::debugAddOrOverlap(
    SkPathOpsDebug::GlitchLog* log, const SkOpSegment* coinSeg, const SkOpSegment* oppSeg,
  ...
  if (fHead &&
      !this->checkOverlap(fHead, coinSeg, oppSeg, coinTs, coinTe, oppTs, oppTe, &overlaps)) {
    return;
  }
  ...
  RETURN_FALSE_IF(
      csExisting && (csExisting == ce || csExisting->contains(ceExisting ? ceExisting : ce)),
      coinSeg);
  RETURN_FALSE_IF(
      ceExisting && (ceExisting == cs || ceExisting->contains(csExisting ? csExisting : cs)),
      coinSeg);
  ...
  RETURN_FALSE_IF(
      osExisting && (osExisting == oe || osExisting->contains(oeExisting ? oeExisting : oe)),
      oppSeg);
  RETURN_FALSE_IF(
      oeExisting && (oeExisting == os || oeExisting->contains(osExisting ? osExisting : os)),
      oppSeg);
  bool csDeleted = false, osDeleted = false, ceDeleted = false, oeDeleted = false;
  this->debugValidate();
  if (!cs || !os) {
    if (!cs) cs = coinSeg->debugAddT(coinTs, log);
    if (!os) os = oppSeg->debugAddT(oppTs, log);
  ...
  if (!ce || !oe) {
    if (!ce) ce = coinSeg->debugAddT(coinTe, log);
    if (!oe) oe = oppSeg->debugAddT(oppTe, log);
  ...
  RETURN_FALSE_IF(
      !cs || !ce || cs == ce || cs->contains(ce) || !os || !oe || os == oe || os->contains(oe),
      coinSeg);
  bool result = true;
  if (overlap) {
    if (overlap->coinPtTStart()->segment() == coinSeg) {
      log->record(
          SkPathOpsDebug::kAddMissingExtend_Glitch, coinSeg, coinTs, coinTe, oppSeg, oppTs, oppTe);
    } else {
      if (oppTs > oppTe) {
        using std::swap;
        swap(coinTs, coinTe);
      ...
      if (outerCoin == innerCoin) {
        ...
        if (outerOpp != innerOpp && this->overlap(ocs, oce, ics, ice, &overS, &overE)) {
          this->debugAddIfMissing(
              log, ocs->starter(oce), ics->starter(ice), overS, overE, outerOpp, innerOpp, added,
              ocs->debugEnder(oce), ics->debugEnder(ice));
        }
        ...
        if (outerOpp != innerCoin && this->overlap(ocs, oce, ios, ioe, &overS, &overE)) {
          this->debugAddIfMissing(
              log, ocs->starter(oce), ios->starter(ioe), overS, overE, outerOpp, innerCoin, added,
              ocs->debugEnder(oce), ios->debugEnder(ioe));
        }
        ...
        if (this->overlap(oos, ooe, ics, ice, &overS, &overE)) {
          this->debugAddIfMissing(
              log, oos->starter(ooe), ics->starter(ice), overS, overE, outerCoin, innerOpp, added,
              oos->debugEnder(ooe), ics->debugEnder(ice));
        }
        ...
        if (this->overlap(oos, ooe, ios, ioe, &overS, &overE)) {
          this->debugAddIfMissing(
              log, oos->starter(ooe), ios->starter(ioe), overS, overE, outerCoin, innerCoin, added,
              oos->debugEnder(ooe), ios->debugEnder(ioe));
        }
      }
      this->debugValidate();
    }
...
void SkOpCoincidence::debugRelease(
    SkPathOpsDebug::GlitchLog* log, const SkCoincidentSpans* coin,
    const SkCoincidentSpans* remove) const {
...
void SkOpCoincidence::debugRelease(
    SkPathOpsDebug::GlitchLog* log, const SkOpSegment* deleted) const {
...
void SkOpCoincidence::debugMarkCollapsed(
    SkPathOpsDebug::GlitchLog* log, const SkCoincidentSpans* coin, const SkOpPtT* test) const {
  const SkCoincidentSpans* head = coin;
  while (coin) {
...
// Commented-out lines keep this in sync with markCollapsed()
void SkOpCoincidence::debugMarkCollapsed(
    SkPathOpsDebug::GlitchLog* log, const SkOpPtT* test) const {
...
void SkCoincidentSpans::debugShow() const {
  SkDebugf(
      "coinSpan - id=%d t=%1.9g tEnd=%1.9g\n", coinPtTStart()->segment()->debugID(),
...
#if DEBUG_COIN
static void DebugCheckBetween(
    const SkOpSpanBase* next, const SkOpSpanBase* end, double oStart, double oEnd,
    const SkOpSegment* oSegment, SkPathOpsDebug::GlitchLog* log) {
  SkASSERT(next != end);
...
static void DebugCheckOverlap(
    const SkCoincidentSpans* test, const SkCoincidentSpans* list, SkPathOpsDebug::GlitchLog* log) {
  if (!list) {
    return;
  }
...
static void DebugCheckOverlapTop(
    const SkCoincidentSpans* head, const SkCoincidentSpans* opt, SkPathOpsDebug::GlitchLog* log) {
...
static void DebugValidate(
  ...
  while (coin) {
    SkASSERT(
        SkOpCoincidence::Ordered(coin->coinPtTStart()->segment(), coin->oppPtTStart()->segment()));
...
static void DebugCheckBetween(
  ...
  while (coin) {
    DebugCheckBetween(
        coin->coinPtTStart()->span(), coin->coinPtTEnd()->span(), coin->oppPtTStart()->fT,
        coin->oppPtTEnd()->fT, coin->oppPtTStart()->segment(), log);
    DebugCheckBetween(
        coin->oppPtTStart()->span(), coin->oppPtTEnd()->span(), coin->coinPtTStart()->fT,
        coin->coinPtTEnd()->fT, coin->coinPtTStart()->segment(), log);
    coin = coin->next();
  }
  DebugCheckOverlapTop(head, opt, log);
}
...
void SkOpSpanBase::debugInsertCoinEnd(
    SkPathOpsDebug::GlitchLog* log, const SkOpSpanBase* coin) const {
...
void SkOpSpanBase::debugMergeMatches(
    SkPathOpsDebug::GlitchLog* log, const SkOpSpanBase* opp) const {
...
void SkOpSpan::debugInsertCoincidence(
    SkPathOpsDebug::GlitchLog* log, const SkOpSegment* segment, bool flipped, bool ordered) const {
  ...
  while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
    switch (verb) {
      ...
      case SkPath::kCubic_Verb:
        ...
        SkDebugf(");\n");
        break;
      case SkPath::kClose_Verb: SkDebugf("    %s.close();\n", pathName); break;
      default: SkDEBUGFAIL("bad verb"); return;
    }
  }
}

static const char* gFillTypeStr[] = {"kWinding", "kEvenOdd", "kInverseWinding", "kInverseEvenOdd"};
  ...
  if (rectCount > 0) {
    ...
    for (int contour = 0; contour < rectCount; ++contour) {
      ...
      SkDebugf(
          ...
          rect.fBottom,
          directions[contour] == SkPathDirection::kCCW ? "SkPathDirection::kCCW"
                                                       : "SkPathDirection::kCW");
    }
    return;
  }
...
void SkPathOpsDebug::DumpOp(
    const SkPath& one, const SkPath& two, SkPathOp op, const char* testName) {
  FILE* file = sk_fopen("op_dump.txt", kWrite_SkFILE_Flag);
  DumpOp(file, one, two, op, testName);
}

void SkPathOpsDebug::DumpOp(
    FILE* file, const SkPath& one, const SkPath& two, SkPathOp op, const char* testName) {
...
static int debug_paths_draw_the_same(const SkPath& one, const SkPath& two, SkBitmap& bits) {
  ...
  for (int y = 0; y < bitHeight - 1; ++y) {
    ...
    for (int x = 0; x < bitWidth - 1; ++x) {
      ...
      if (err) {
        errors +=
            addr1[x + 1] != addr3[x + 1] && addr2[x] != addr4[x] && addr2[x + 1] != addr4[x + 1];
      }
    }
  }
  return errors;
...
void SkPathOpsDebug::VerifyOp(
    const SkPath& one, const SkPath& two, SkPathOp op, const SkPath& result) {
...
void Dump(const SkPath& path) { path.dump(); }

void DumpHex(const SkPath& path) { path.dumpHex(); }
