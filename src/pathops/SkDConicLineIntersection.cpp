...
class LineConicIntersections {
 public:
  ...
  static bool close_to(double a, double b, const double c[3]) {
    double max =
        std::max(-std::min(std::min(c[0], c[1]), c[2]), std::max(std::max(c[0], c[1]), c[2]));
  ...
  int horizontalIntersect(double axisIntercept, double left, double right, bool flipped) {
    ...
    for (int index = 0; index < count; ++index) {
      ...
      if (this->pinTs(&conicT, &lineT, &pt, kPointInitialized) && this->uniqueAnswer(conicT, pt)) {
...
#ifdef SK_DEBUG
      if (!fIntersections->globalState() || !fIntersections->globalState()->debugSkipAssert()) {
 ...
 protected:
  ...
  void addLineNearEndPoints() {
    for (int lIndex = 0; lIndex < 2; ++lIndex) {
      ...
      double conicT =
          ((SkDCurve*)&fConic)->nearPoint(SkPath::kConic_Verb, (*fLine)[lIndex], (*fLine)[!lIndex]);
      if (conicT < 0) {
        continue;
      }
...
int SkIntersections::horizontal(
    const SkDConic& conic, double left, double right, double y, bool flipped) {
...
int SkIntersections::vertical(
    const SkDConic& conic, double top, double bottom, double x, bool flipped) {
