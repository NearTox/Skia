...
static bool findChaseOp(
    SkTDArray<SkOpSpanBase*>& chase, SkOpSpanBase** startPtr, SkOpSpanBase** endPtr,
    SkOpSegment** result) {
  while (chase.count()) {
    SkOpSpanBase* span;
    chase.pop(&span);
    ...
    *endPtr = nullptr;
    ...
    while ((angle = angle->next()) != firstAngle) {
      ...
      if (!segment->done(angle)) {
        ...
        if (sortable) {
          if (!segment->markAngle(
                  maxWinding, sumWinding, oppMaxWinding, oppSumWinding, angle, nullptr)) {
            return false;
          }
        }
      }
...
static bool bridgeOp(
    SkOpContourHead* contourList, const SkPathOp op, const int xorMask, const int xorOpMask,
    SkPathWriter* writer) {
    ...
    do {
      if (current->activeOp(start, end, xorMask, xorOpMask, op)) {
        do {
          ...
          lastSimple = simple;
          SkOpSegment* next = current->findNextOp(
              &chase, &nextStart, &nextEnd, &unsortable, &simple, op, xorMask, xorOpMask);
          if (!next) {
            if (!unsortable && writer->hasMove() && current->verb() != SkPath::kLine_Verb &&
                !writer->isClosed()) {
          ...
          SkDebugf(
              ...
              current->debugID(), start->pt().fX, start->pt().fY, end->pt().fX, end->pt().fY);
#endif
          if (!current->addCurveTo(start, end, writer)) {
            return false;
          }
...
#  endif
  ...
  SkPath::FillType fillType =
      inverseFill ? SkPath::kInverseEvenOdd_FillType : SkPath::kEvenOdd_FillType;
  SkRect rect1, rect2;
  ...
  if (one.isEmpty() || two.isEmpty()) {
    SkPath work;
    switch (op) {
      case kIntersect_SkPathOp: break;
      case kUnion_SkPathOp:
      case kXOR_SkPathOp: work = one.isEmpty() ? two : one; break;
      ...
      case kReverseDifference_SkPathOp:
        ...
        break;
      default: SkASSERT(0);  // unhandled case
    }
