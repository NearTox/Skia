...
#define SkOPASSERT(cond)                                                          \
    SkASSERT((this->globalState() && (this->globalState()->debugCheckHealth() ||  \
                                      this->globalState()->debugSkipAssert())) || \
             (cond))
#else
#define SkOPASSERT(cond) \
...
inline bool zero_or_one(double x) noexcept { return x == 0 || x == 1; }
...
inline bool approximately_greater(double x, double y) noexcept { return x - FLT_EPSILON >= y; }

inline bool approximately_greater_double(double x, double y) noexcept {
    return x - FLT_EPSILON_DOUBLE >= y;
}

inline bool approximately_greater_orderable(double x, double y) noexcept {
    return x - FLT_EPSILON_ORDERABLE_ERR >= y;
}

inline bool approximately_greater_or_equal(double x, double y) noexcept {
    return x + FLT_EPSILON > y;
}

inline bool approximately_greater_or_equal_double(double x, double y) noexcept {
    return x + FLT_EPSILON_DOUBLE > y;
}

inline bool approximately_greater_or_equal_orderable(double x, double y) noexcept {
    return x + FLT_EPSILON_ORDERABLE_ERR > y;
}

inline bool approximately_lesser(double x, double y) noexcept { return x + FLT_EPSILON <= y; }

inline bool approximately_lesser_double(double x, double y) noexcept {
    return x + FLT_EPSILON_DOUBLE <= y;
}

inline bool approximately_lesser_orderable(double x, double y) noexcept {
    return x + FLT_EPSILON_ORDERABLE_ERR <= y;
}

inline bool approximately_lesser_or_equal(double x, double y) noexcept {
    return x - FLT_EPSILON < y;
}

inline bool approximately_lesser_or_equal_double(double x, double y) noexcept {
    return x - FLT_EPSILON_DOUBLE < y;
}

inline bool approximately_lesser_or_equal_orderable(double x, double y) noexcept {
    return x - FLT_EPSILON_ORDERABLE_ERR < y;
}

inline bool approximately_greater_than_one(double x) noexcept { return x > 1 - FLT_EPSILON; }

inline bool precisely_greater_than_one(double x) noexcept { return x > 1 - DBL_EPSILON_ERR; }

inline bool approximately_less_than_zero(double x) noexcept { return x < FLT_EPSILON; }

inline bool precisely_less_than_zero(double x) noexcept { return x < DBL_EPSILON_ERR; }

inline bool approximately_negative(double x) noexcept { return x < FLT_EPSILON; }

inline bool approximately_negative_orderable(double x) noexcept {
    return x < FLT_EPSILON_ORDERABLE_ERR;
}

inline bool precisely_negative(double x) noexcept { return x < DBL_EPSILON_ERR; }

inline bool approximately_one_or_less(double x) noexcept { return x < 1 + FLT_EPSILON; }

inline bool approximately_one_or_less_double(double x) noexcept {
    return x < 1 + FLT_EPSILON_DOUBLE;
}

inline bool approximately_positive(double x) noexcept { return x > -FLT_EPSILON; }

inline bool approximately_positive_squared(double x) noexcept { return x > -(FLT_EPSILON_SQUARED); }

inline bool approximately_zero_or_more(double x) noexcept { return x > -FLT_EPSILON; }

inline bool approximately_zero_or_more_double(double x) noexcept { return x > -FLT_EPSILON_DOUBLE; }

inline bool approximately_between_orderable(double a, double b, double c) noexcept {
    return a <= c ? approximately_negative_orderable(a - b) &&
                            approximately_negative_orderable(b - c)
                  : approximately_negative_orderable(b - a) &&
                            approximately_negative_orderable(c - b);
}

inline bool approximately_between(double a, double b, double c) noexcept {
    return a <= c ? approximately_negative(a - b) && approximately_negative(b - c)
                  : approximately_negative(b - a) && approximately_negative(c - b);
}

inline bool precisely_between(double a, double b, double c) noexcept {
    return a <= c ? precisely_negative(a - b) && precisely_negative(b - c)
                  : precisely_negative(b - a) && precisely_negative(c - b);
}

// returns true if (a <= b <= c) || (a >= b >= c)
inline bool between(double a, double b, double c) noexcept {
...
inline bool roughly_negative(double x) noexcept { return x < ROUGH_EPSILON; }
...
inline double SkDInterp(double A, double B, double t) noexcept { return A + (B - A) * t; }

double SkDCubeRoot(double x);

/* Returns -1 if negative, 0 if zero, 1 if positive
 */
inline int SkDSign(double x) noexcept { return (x > 0) - (x < 0); }

/* Returns 0 if negative, 1 if zero, 2 if positive
 */
inline int SKDSide(double x) noexcept { return (x > 0) + (x >= 0); }

/* Returns 1 if negative, 2 if zero, 4 if positive
 */
inline int SkDSideBit(double x) noexcept { return 1 << SKDSide(x); }

inline double SkPinT(double t) noexcept {
