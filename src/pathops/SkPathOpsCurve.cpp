...
void SkDCurve::setConicBounds(
    const SkPoint curve[3], SkScalar curveWeight, double tStart, double tEnd,
...
void SkDCurve::setCubicBounds(
    const SkPoint curve[4], SkScalar, double tStart, double tEnd, SkPathOpsBounds* bounds) {
  SkDCubic dCurve;
  dCurve.set(curve);
  SkDRect dRect;
...
void SkDCurve::setQuadBounds(
    const SkPoint curve[3], SkScalar, double tStart, double tEnd, SkPathOpsBounds* bounds) {
  SkDQuad dCurve;
  dCurve.set(curve);
  SkDRect dRect;
...
void SkDCurveSweep::setCurveHullSweep(SkPath::Verb verb) {
  ...
  for (int index = 0; index <= SkPathOpsVerbToPoints(verb); ++index) {
    maxVal = std::max(maxVal, std::max(SkTAbs(fCurve[index].fX), SkTAbs(fCurve[index].fY)));
  }
  {
