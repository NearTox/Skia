    ...
    for (auto contour : contours) {
      ...
      do {
        ...
        switch (iter.next(pts)) {
          case SkPath::kMove_Verb: temp->moveTo(pts[0]); break;
          case SkPath::kLine_Verb: temp->lineTo(pts[1]); break;
          case SkPath::kQuad_Verb: temp->quadTo(pts[1], pts[2]); break;
          case SkPath::kConic_Verb: temp->conicTo(pts[1], pts[2], iter.conicWeight()); break;
          case SkPath::kCubic_Verb: temp->cubicTo(pts[1], pts[2], pts[3]); break;
          case SkPath::kClose_Verb: temp->close(); break;
...
bool AsWinding(const SkPath& path, SkPath* result) {
  ...
  fillType = path.isInverseFillType() ? SkPathFillType::kInverseWinding : SkPathFillType::kWinding;
  ...
  if (std::all_of(
          sorted.fChildren.begin(), sorted.fChildren.end(),
