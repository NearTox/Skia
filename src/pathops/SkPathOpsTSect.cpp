...
#if DEBUG_PERP
      SkDebugf(
          "%s priorSpan=%d t=%1.9g opp=%d\n", __FUNCTION__, priorSpan ? priorSpan->debugID() : -1,
          t, opp->debugID());
#endif
    }
#if DEBUG_PERP
...
bool SkTSect::binarySearchCoin(
    SkTSect* sect2, double tStart, double tStep, double* resultT, double* oppT,
    SkTSpan** oppFirst) {
...
bool SkTSect::extractCoincident(SkTSect* sect2, SkTSpan* first, SkTSpan* last, SkTSpan** result) {
  ...
  if (prev && prev->fEndT == startT &&
      this->binarySearchCoin(
          sect2, startT, prev->fStartT - startT, &coinStart, &oppStartT, &oppFirst) &&
...
struct SkClosestRecord {
  ...
  bool matesWith(const SkClosestRecord& mate SkDEBUGPARAMS(SkIntersections* i)) const noexcept {
    ...
    SkOPOBJASSERT(
        i, fC2Span == mate.fC2Span || fC2Span->endT() <= mate.fC2Span->startT() ||
...
void SkTSect::BinarySearch(SkTSect* sect1, SkTSect* sect2, SkIntersections* intersections) {
  ...
  if (coincident) {
    ...
    do {
      ...
      int index =
          intersections->insertCoincident(coincident->fStartT, perpT, coincident->pointFirst());
      if ((intersections->insertCoincident(
               coincident->fEndT, coincident->fCoinEnd.perpT(), coincident->pointLast()) < 0) &&
          index >= 0) {
