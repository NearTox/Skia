...
static bool CompareResult(
    const char* func, SkString* bugOut, SkString* bugPart, int append, bool compare) {
...
bool SkOpAngle::after(SkOpAngle* test) {
  ...
  bugOut.printf(
      ...
      lh->fStart->t(), lh->fEnd->t(), segment()->debugID(), debugID(), fSectorStart, fSectorEnd,
      fStart->t(), fEnd->t(), rh->segment()->debugID(), rh->debugID(), rh->fSectorStart,
      rh->fSectorEnd, rh->fStart->t(), rh->fEnd->t());
  ...
  if (!lrOverlap) {     // no lh/rh sector overlap
    if (!ltrOverlap) {  // no lh/this/rh sector overlap
      return COMPARE_RESULT(
          4, (lh->fSectorEnd > rh->fSectorStart) ^ (fSectorStart > lh->fSectorEnd) ^
...
int SkOpAngle::lineOnOneSide(
    const SkDPoint& origin, const SkDVector& line, const SkOpAngle* test, bool useOriginal) const {
  double crosses[3];
...
bool SkOpAngle::endsIntersect(SkOpAngle* rh) {
  ...
  for (int index = 0; index < 2; ++index) {
    ...
    // in the bug.
    if (delta < 4e-3 && delta > 1e-3 && !useIntersect && fPart.isCurve() && rh->fPart.isCurve() &&
        fOriginalCurvePart[0] != fPart.fCurve.fLine[0]) {
...
void SkOpAngle::setSpans() {
  ...
  SkDEBUGCODE(
      fPart.fCurve[2].fX = fPart.fCurve[2].fY = fPart.fCurve[3].fX = fPart.fCurve[3].fY =
