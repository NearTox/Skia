...
void SkOpEdgeBuilder::init() {
  ...
  fXorMask[0] = fXorMask[1] =
      (fPath->getFillType() & 1) ? kEvenOdd_PathOpsMask : kWinding_PathOpsMask;
  fUnparseable = false;
...
void SkOpEdgeBuilder::addOperand(const SkPath& path) {
  ...
  fPathVerbs.pop();
  fPath = &path;
  fXorMask[1] = (fPath->getFillType() & 1) ? kEvenOdd_PathOpsMask : kWinding_PathOpsMask;
  preFetch();
}
...
void SkOpEdgeBuilder::closeContour(const SkPoint& curveEnd, const SkPoint& curveStart) {
  if (!SkDPoint::ApproximatelyEqual(curveEnd, curveStart)) {
    ...
    *fPathPts.append() = curveStart;
    ...
    if (SkPath::kLine_Verb == fPathVerbs[verbCount - 1] && fPathPts[ptsCount - 2] == curveStart) {
      fPathVerbs.pop();
      fPathPts.pop();
    } else {
  ...
  do {
    ...
    switch (verb) {
      ...
      case SkPath::kClose_Verb:
        ...
        lastCurve = false;
        continue;
      case SkPath::kDone_Verb: continue;
    }
    *fPathVerbs.append() = verb;
  ...
  while ((verb = (SkPath::Verb)*verbPtr) != SkPath::kDone_Verb) {
    ...
    switch (verb) {
      case SkPath::kMove_Verb:
        ...
        contour->init(fGlobalState, fOperand, fXorMask[fOperand] == kEvenOdd_PathOpsMask);
        pointsPtr += moveToPtrBump;
        moveToPtrBump = 1;
        continue;
      case SkPath::kLine_Verb: fContourBuilder.addLine(pointsPtr); break;
      case SkPath::kQuad_Verb: {
      ...
      case SkPath::kCubic_Verb: {
        ...
        for (int index = 0; index <= breaks; ++index) {
          ...
          SkPoint* curve = SkPath::kCubic_Verb == verb ? split->fPts : split->fReduced;
        ...
        for (int index = 0; index <= breaks; ++index) {
          ...
          SkPoint* curve = SkPath::kCubic_Verb == split->fVerb ? split->fPts : split->fReduced;
      ...
      case SkPath::kClose_Verb:
        ...
        contour = nullptr;
        continue;
      default: SkDEBUGFAIL("bad verb"); return false;
    }
    SkASSERT(contour);
    if (contour->count()) {
