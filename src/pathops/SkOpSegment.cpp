...
SkOpAngle* SkOpSegment::activeAngle(
...
SkOpAngle* SkOpSegment::activeAngleOther(
    SkOpSpanBase* start, SkOpSpanBase** startPtr, SkOpSpanBase** endPtr, bool* done) {
...
bool SkOpSegment::activeOp(
    SkOpSpanBase* start, SkOpSpanBase* end, int xorMiMask, int xorSuMask, SkPathOp op) {
...
bool SkOpSegment::activeOp(
  ...
  this->setUpWindings(
      start, end, sumMiWinding, sumSuWinding, &maxWinding, &sumWinding, &oppMaxWinding,
      &oppSumWinding);
  bool miFrom;
  bool miTo;
  bool suFrom;
  ...
  SkDebugf(
      ...
      __FUNCTION__, debugID(), start->t(), end->t(), SkPathOpsDebug::kPathOpStr[op], miFrom, miTo,
      suFrom, suTo, result);
#endif
  return result;
}
...
bool SkOpSegment::addCurveTo(
    const SkOpSpanBase* start, const SkOpSpanBase* end, SkPathWriter* path) const {
  ...
  switch (verb) {
    case SkPath::kLine_Verb: FAIL_IF(!path->deferredLine(end->ptT())); break;
    case SkPath::kQuad_Verb: path->quadTo(curvePart.fCurve.fQuad[1].asSkPoint(), end->ptT()); break;
    case SkPath::kConic_Verb:
      path->conicTo(
          curvePart.fCurve.fConic[1].asSkPoint(), end->ptT(), curvePart.fCurve.fConic.fWeight);
      break;
    case SkPath::kCubic_Verb:
      path->cubicTo(
          curvePart.fCurve.fCubic[1].asSkPoint(), curvePart.fCurve.fCubic[2].asSkPoint(),
          end->ptT());
      break;
    default: SkASSERT(0);
  }
...
#if DEBUG_ADD_T
      SkDebugf(
          "%s insert t=%1.9g segID=%d spanID=%d\n", __FUNCTION__, t, span->segment()->debugID(),
          span->debugID());
#endif
      span->bumpSpanAdds();
      return span->ptT();
...
SkOpPtT* SkOpSegment::addT(double t) { return addT(t, this->ptAtT(t)); }
...
bool SkOpSegment::ComputeOneSum(
    const SkOpAngle* baseAngle, SkOpAngle* nextAngle, SkOpAngle::IncludeType includeType) {
  ...
  if (binary) {
    ...
    nextSegment->setUpWindings(
        nextAngle->start(), nextAngle->end(), &sumMiWinding, &sumSuWinding, &maxWinding,
        &sumWinding, &oppMaxWinding, &oppSumWinding);
    if (!nextSegment->markAngle(
            maxWinding, sumWinding, oppMaxWinding, oppSumWinding, nextAngle, &last)) {
      return false;
    }
  } else {
    nextSegment->setUpWindings(
        nextAngle->start(), nextAngle->end(), &sumMiWinding, &maxWinding, &sumWinding);
...
bool SkOpSegment::ComputeOneSumReverse(
    SkOpAngle* baseAngle, SkOpAngle* nextAngle, SkOpAngle::IncludeType includeType) {
  SkOpSegment* baseSegment = baseAngle->segment();
  ...
  if (binary) {
    int oppMaxWinding, oppSumWinding;
    nextSegment->setUpWindings(
        nextAngle->end(), nextAngle->start(), &sumMiWinding, &sumSuWinding, &maxWinding,
        &sumWinding, &oppMaxWinding, &oppSumWinding);
    if (!nextSegment->markAngle(
            maxWinding, sumWinding, oppMaxWinding, oppSumWinding, nextAngle, &last)) {
      return false;
    ...
    nextSegment->setUpWindings(
        nextAngle->end(), nextAngle->start(), &sumMiWinding, &maxWinding, &sumWinding);
    if (!nextSegment->markAngle(maxWinding, sumWinding, nextAngle, &last)) {
      return false;
    }
  }
...
int SkOpSegment::computeSum(
    SkOpSpanBase* start, SkOpSpanBase* end, SkOpAngle::IncludeType includeType) {
 ...
 */
SkOpSegment* SkOpSegment::findNextOp(
    SkTDArray<SkOpSpanBase*>* chase, SkOpSpanBase** nextStart, SkOpSpanBase** nextEnd,
  ...
  do {
    ...
    bool activeAngle = nextSegment->activeOp(
        xorMiMask, xorSuMask, nextAngle->start(), nextAngle->end(), op, &sumMiWinding,
        &sumSuWinding);
    if (activeAngle) {
      ++activeCount;
...
#if DEBUG_WINDING
      SkDebugf(
          "%s chase.append segment=%d span=%d", __FUNCTION__, last->segment()->debugID(),
          last->debugID());
      if (!last->final()) {
...
#if DEBUG_WINDING
  SkDebugf(
      "%s from:[%d] to:[%d] start=%d end=%d\n", __FUNCTION__, debugID(), nextSegment->debugID(),
      *nextStart, *nextEnd);
#endif
  return nextSegment;
}

SkOpSegment* SkOpSegment::findNextWinding(
    SkTDArray<SkOpSpanBase*>* chase, SkOpSpanBase** nextStart, SkOpSpanBase** nextEnd,
    bool* unsortable) {
  SkOpSpanBase* start = *nextStart;
  SkOpSpanBase* end = *nextEnd;
  ...
  do {
    nextSegment = nextAngle->segment();
    bool activeAngle =
        nextSegment->activeWinding(nextAngle->start(), nextAngle->end(), &sumWinding);
    if (activeAngle) {
      ++activeCount;
      if (!foundAngle || (foundDone && activeCount & 1)) {
        foundAngle = nextAngle;
...
SkOpSegment* SkOpSegment::findNextXor(
...
bool SkOpSegment::markAndChaseWinding(
    SkOpSpanBase* start, SkOpSpanBase* end, int winding, SkOpSpanBase** lastPtr) {
  ...
  while ((other = other->nextChase(&start, &step, &spanStart, &last))) {
    ...
    if (spanStart->windSum() != SK_MinS32) {
      //            SkASSERT(spanStart->windSum() == winding);   // FIXME: is this assert too
      //            aggressive?
      SkASSERT(!last);
      break;
    }
...
bool SkOpSegment::markAndChaseWinding(
    SkOpSpanBase* start, SkOpSpanBase* end, int winding, int oppWinding, SkOpSpanBase** lastPtr) {
  SkOpSpan* spanStart = start->starter(end);
  int step = start->step(end);
...
bool SkOpSegment::markAngle(
    int maxWinding, int sumWinding, const SkOpAngle* angle, SkOpSpanBase** result) {
  ...
  if (last) {
    SkDebugf("%s last seg=%d span=%d", __FUNCTION__, last->segment()->debugID(), last->debugID());
    if (!last->final()) {
      SkDebugf(" windSum=");
...
bool SkOpSegment::markAngle(
    int maxWinding, int sumWinding, int oppMaxWinding, int oppSumWinding, const SkOpAngle* angle,
    SkOpSpanBase** result) {
  SkASSERT(angle->segment() == this);
  if (UseInnerWinding(maxWinding, sumWinding)) {
    maxWinding = sumWinding;
  ...
  if (result) {
    ...
    if (last) {
      SkDebugf(
          "%s last segment=%d span=%d", __FUNCTION__, last->segment()->debugID(), last->debugID());
      if (!last->final()) {
        SkDebugf(" windSum=");
        SkPathOpsDebug::WindingPrintf(last->upCast()->windSum());
      }
...
bool SkOpSegment::match(
    const SkOpPtT* base, const SkOpSegment* testParent, double testT, const SkPoint& testPt) const {
...
SkOpSegment* SkOpSegment::nextChase(
    SkOpSpanBase** startPtr, int* stepPtr, SkOpSpan** minPtr, SkOpSpanBase** last) const {
  ...
  if (angle == nullptr) {
    ...
    otherEnd = step > 0 ? foundSpan->upCastable() ? foundSpan->upCast()->next() : nullptr
                        : foundSpan->prev();
  } else {
...
bool SkOpSegment::spansNearby(
    const SkOpSpanBase* refSpan, const SkOpSpanBase* checkSpan, bool* found) const {
  ...
  do {
    ...
    do {
      ...
      if (distSqBest > distSq &&
          (refSeg != check->segment() || !refSeg->ptsDisjoint(*ref, *check))) {
        distSqBest = distSq;
        refBest = ref;
        checkBest = check;
      }
...
doneCheckingDistance:
  *found = checkBest &&
           refBest->segment()->match(refBest, checkBest->segment(), checkBest->fT, checkBest->fPt);
  return true;
}
...
bool SkOpSegment::sortAngles() {
  ...
  do {
    ...
    if (fromAngle && toAngle) {
#if DEBUG_ANGLE
      SkDebugf("%s [%d] tStart=%1.9g [%d]\n", __FUNCTION__, debugID(), span->t(), span->debugID());
...
bool SkOpSegment::subDivide(
    const SkOpSpanBase* start, const SkOpSpanBase* end, SkDCurve* edge) const {
  SkASSERT(start != end);
  ...
  if (fVerb == SkPath::kQuad_Verb) {
    ...
    edge->fConic[1] = SkDConic::SubDivide(
        fPts, fWeight, edge->fQuad[0], edge->fQuad[2], startT, endT, &edge->fConic.fWeight);
  } else {
    SkASSERT(fVerb == SkPath::kCubic_Verb);
...
bool SkOpSegment::testForCoincidence(
...
int SkOpSegment::updateOppWinding(const SkOpAngle* angle) const noexcept {
  const SkOpSpanBase* startSpan = angle->start();
  const SkOpSpanBase* endSpan = angle->end();
  return updateOppWinding(endSpan, startSpan);
}

int SkOpSegment::updateOppWindingReverse(const SkOpAngle* angle) const noexcept {
  const SkOpSpanBase* startSpan = angle->start();
  const SkOpSpanBase* endSpan = angle->end();
