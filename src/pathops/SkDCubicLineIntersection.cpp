...
class LineCubicIntersections {
 public:
  ...
  int intersectRay(double roots[3]) {
    ...
    for (int index = 0; index < count; ++index) {
      ...
      if (!approximately_zero(calcPt.fX)) {
        for (int n = 0; n < 4; ++n) {
          c[n].fY = (fCubic[n].fY - fLine[0].fY) * opp + (fCubic[n].fX - fLine[0].fX) * adj;
        }
        double extremeTs[6];
  ...
  bool uniqueAnswer(double cubicT, const SkDPoint& pt) {
    ...
    SkDebugf("%s pt=(%1.9g,%1.9g) cPt=(%1.9g,%1.9g)\n", __FUNCTION__, pt.fX, pt.fY, cPt.fX, cPt.fY);
#endif
    return true;
  }
 ...
 protected:
  ...
  void addLineNearEndPoints() {
    for (int lIndex = 0; lIndex < 2; ++lIndex) {
      ...
      double cubicT =
          ((SkDCurve*)&fCubic)->nearPoint(SkPath::kCubic_Verb, fLine[lIndex], fLine[!lIndex]);
      if (cubicT < 0) {
        continue;
      }
...
int SkIntersections::horizontal(
    const SkDCubic& cubic, double left, double right, double y, bool flipped) {
...
int SkIntersections::vertical(
    const SkDCubic& cubic, double top, double bottom, double x, bool flipped) {
