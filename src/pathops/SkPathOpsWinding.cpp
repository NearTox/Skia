...
void SkOpContour::rayCheck(
    const SkOpRayHit& base, SkOpRayDir dir, SkOpRayHit** hits, SkArenaAlloc* allocator) {
    ...
    if (approximately_zero(t)) {
      ...
      if (SkDPoint::ApproximatelyEqual(pt, base.fPt)) {
        ...
        if (fVerb == SkPath::kCubic_Verb && base.fSpan->segment() == this &&
            roughly_equal(base.fT, t) && SkDPoint::RoughlyEqual(pt, base.fPt)) {
#if DEBUG_WINDING
...
bool SkOpSpan::sortableTop(SkOpContour* contourHead) {
  ...
  SkDebugf(
      ...
      gDebugRayDirName[static_cast<int>(dir)], hitBase.fSpan->segment()->debugID(), hitBase.fT,
      hitBase.fPt.fX, hitBase.fPt.fY);
  for (int index = 0; index < count; ++index) {
    ...
    SkDebugf(
        "%s [%d] valid=%d operand=%d span=%d ccw=%d ", __FUNCTION__, index, hit->fValid, operand,
        span ? span->debugID() : -1, ccw);
    if (span) {
      hitSegment->dumpPtsInner();
    }
    SkDebugf(
        " t=%1.9g pt=(%1.9g,%1.9g) slope=(%1.9g,%1.9g)\n", hit->fT, hit->fPt.fX, hit->fPt.fY,
        hit->fSlope.fX, hit->fSlope.fY);
  }
#endif
