...
bool SkCoincidentSpans::extend(
  ...
  if (fCoinPtTEnd->fT < coinPtTEnd->fT ||
      (this->flipped() ? fOppPtTEnd->fT > oppPtTEnd->fT : fOppPtTEnd->fT < oppPtTEnd->fT)) {
...
bool SkCoincidentSpans::ordered(bool* result) const {
  ...
  do {
    ...
    if (!opp) {
      //            SkOPOBJASSERT(start, 0);  // may assert if coincident span isn't fully
      //            processed
      return false;
    }
...
bool SkOpCoincidence::extend(
  ...
  do {
    ...
    SkASSERT(
        test->coinPtTStart()->fT > coinPtTStart->fT || coinPtTEnd->fT > test->coinPtTEnd()->fT ||
...
static void DebugCheckAdd(
  ...
  while (check) {
    SkASSERT(
        check->coinPtTStart() != coinPtTStart || check->coinPtTEnd() != coinPtTEnd ||
        check->oppPtTStart() != oppPtTStart || check->oppPtTEnd() != oppPtTEnd);
    SkASSERT(
        check->coinPtTStart() != oppPtTStart || check->coinPtTEnd() != oppPtTEnd ||
...
void SkOpCoincidence::add(
    SkOpPtT* coinPtTStart, SkOpPtT* coinPtTEnd, SkOpPtT* oppPtTStart, SkOpPtT* oppPtTEnd) {
...
bool SkOpCoincidence::checkOverlap(
    SkCoincidentSpans* check, const SkOpSegment* coinSeg, const SkOpSegment* oppSeg, double coinTs,
    double coinTe, double oppTs, double oppTe, SkTDArray<SkCoincidentSpans*>* overlaps) const {
  if (!Ordered(coinSeg, oppSeg)) {
    if (oppTs < oppTe) {
...
bool SkOpCoincidence::addIfMissing(
    const SkOpPtT* over1s, const SkOpPtT* over2s, double tStart, double tEnd, SkOpSegment* coinSeg,
    SkOpSegment* oppSeg,
    bool* added SkDEBUGPARAMS(const SkOpPtT* over1e) SkDEBUGPARAMS(const SkOpPtT* over2e)) {
...
bool SkOpCoincidence::addOrOverlap(
    SkOpSegment* coinSeg, SkOpSegment* oppSeg, double coinTs, double coinTe, double oppTs,
    double oppTe, bool* added) {
  ...
  FAIL_IF(ceExisting && (ceExisting == cs || ceExisting->contains(csExisting ? csExisting : cs)));
  ...
  FAIL_IF(osExisting && (osExisting == oe || osExisting->contains(oeExisting ? oeExisting : oe)));
  FAIL_IF(oeExisting && (oeExisting == os || oeExisting->contains(osExisting ? osExisting : os)));
  // extra line in debug code
  this->debugValidate();
  if (!cs || !os) {
    SkOpPtT* csWritable = cs ? const_cast<SkOpPtT*>(cs) : coinSeg->addT(coinTs);
    if (csWritable == ce) {
      return true;
    }
    SkOpPtT* osWritable = os ? const_cast<SkOpPtT*>(os) : oppSeg->addT(oppTs);
  ...
  if (!ce || !oe) {
    SkOpPtT* ceWritable = ce ? const_cast<SkOpPtT*>(ce) : coinSeg->addT(coinTe);
    SkOpPtT* oeWritable = oe ? const_cast<SkOpPtT*>(oe) : oppSeg->addT(oppTe);
    ...
    while ((inner = inner->next())) {
      ...
      if (outerCoin == innerCoin) {
        ...
        if (outerOpp != innerOpp && this->overlap(ocs, oce, ics, ice, &overS, &overE)) {
          FAIL_IF(!this->addIfMissing(
              ocs->starter(oce), ics->starter(ice), overS, overE, outerOppWritable,
              innerOppWritable,
              added SkDEBUGPARAMS(ocs->debugEnder(oce)) SkDEBUGPARAMS(ics->debugEnder(ice))));
        }
        ...
        if (outerOpp != innerCoin && this->overlap(ocs, oce, ios, ioe, &overS, &overE)) {
          FAIL_IF(!this->addIfMissing(
              ocs->starter(oce), ios->starter(ioe), overS, overE, outerOppWritable,
              innerCoinWritable,
              added SkDEBUGPARAMS(ocs->debugEnder(oce)) SkDEBUGPARAMS(ios->debugEnder(ioe))));
        }
        ...
        if (this->overlap(oos, ooe, ics, ice, &overS, &overE)) {
          FAIL_IF(!this->addIfMissing(
              oos->starter(ooe), ics->starter(ice), overS, overE, outerCoinWritable,
              innerOppWritable,
              added SkDEBUGPARAMS(oos->debugEnder(ooe)) SkDEBUGPARAMS(ics->debugEnder(ice))));
        }
        ...
        if (this->overlap(oos, ooe, ios, ioe, &overS, &overE)) {
          FAIL_IF(!this->addIfMissing(
              oos->starter(ooe), ios->starter(ioe), overS, overE, outerCoinWritable,
              innerCoinWritable,
              added SkDEBUGPARAMS(oos->debugEnder(ooe)) SkDEBUGPARAMS(ios->debugEnder(ioe))));
        }
      }
      this->debugValidate();
    }
...
bool SkOpCoincidence::addOverlap(
    const SkOpSegment* seg1, const SkOpSegment* seg1o, const SkOpSegment* seg2,
    const SkOpSegment* seg2o, const SkOpPtT* overS, const SkOpPtT* overE) {
...
bool SkOpCoincidence::contains(
    const SkCoincidentSpans* coin, const SkOpSegment* seg, const SkOpSegment* opp,
    double oppT) const {
  if (!coin) {
    return false;
  }
...
bool SkOpCoincidence::apply(DEBUG_COIN_DECLARE_ONLY_PARAMS()) {
  ...
  do {
    ...
    SkOpSpanBase* oStartBase =
        (flipped ? coin->oppPtTEndWritable() : coin->oppPtTStartWritable())->span();
    ...
    do {
      ...
      bool addToStart =
          windValue && (windValue > windDiff || (windValue == windDiff && oWindValue <= oWindDiff));
...
void SkOpCoincidence::releaseDeleted(SkCoincidentSpans* coin) {
  ...
  do {
    ...
    if (coin->coinPtTStart()->deleted()) {
      SkOPASSERT(coin->flipped() ? coin->oppPtTEnd()->deleted() : coin->oppPtTStart()->deleted());
      if (prev) {
        prev->setNext(next);
      } else if (head == fHead) {
        fHead = next;
  ...
  while (outer) {
    ...
    while ((inner = inner->next())) {
      ...
      if ((outerOpp == innerCoin &&
           SkOpPtT::Overlaps(
               outer->oppPtTStart(), outer->oppPtTEnd(), inner->coinPtTStart(), inner->coinPtTEnd(),
               &overlapS, &overlapE)) ||
          (outerCoin == innerOpp &&
           SkOpPtT::Overlaps(
               outer->coinPtTStart(), outer->coinPtTEnd(), inner->oppPtTStart(), inner->oppPtTEnd(),
               &overlapS, &overlapE)) ||
          (outerOpp == innerOpp &&
           SkOpPtT::Overlaps(
               outer->oppPtTStart(), outer->oppPtTEnd(), inner->oppPtTStart(), inner->oppPtTEnd(),
               &overlapS, &overlapE))) {
