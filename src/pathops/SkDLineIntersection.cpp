...
void SkIntersections::cleanUpParallelLines(bool parallel) {
  ...
  if (fUsed == 2 && !parallel) {
    ...
    if ((!startMatch && !endMatch) || approximately_equal(fT[0][0], fT[0][1])) {
      ...
      if (startMatch && endMatch && (fT[0][0] != 0 || !zero_or_one(fT[1][0])) && fT[0][1] == 1 &&
          zero_or_one(fT[1][1])) {
        removeOne(0);
      } else {
        removeOne(endMatch);
...
int SkIntersections::intersectRay(const SkDLine& a, const SkDLine& b) {
  ...
  if (!approximately_zero(denom)) {
     ...
     */
    if (!AlmostEqualUlps(
            aLen.fX * a[0].fY - aLen.fY * a[0].fX, aLen.fX * b[0].fY - aLen.fY * b[0].fX)) {
      return fUsed = 0;
    }
...
int SkIntersections::intersect(const SkDLine& a, const SkDLine& b) {
  ...
  bool unparallel =
      fAllowNear ? NotAlmostEqualUlps_Pin(axByLen, ayBxLen) : NotAlmostDequalUlps(axByLen, ayBxLen);
...
int SkIntersections::horizontal(
    const SkDLine& line, double left, double right, double y, bool flipped) {
...
int SkIntersections::vertical(
    const SkDLine& line, double top, double bottom, double x, bool flipped) {
