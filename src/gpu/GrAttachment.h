...
class GrAttachment : public GrSurface {
 public:
  ...
  static void ComputeSharedAttachmentUniqueKey(
      ...
      UsageFlags requiredUsage, int sampleCnt, GrProtected isProtected, GrUniqueKey* key);

  // TODO: Once attachments start having multiple usages, we'll need to figure out how to search
  // the cache for an attachment that simply contains the requested usage instead of equaling it.
  static void ComputeScratchKey(
      const GrCaps& caps, const GrBackendFormat& format, SkISize dimensions,
      UsageFlags requiredUsage, int sampleCnt, GrProtected, GrScratchKey* key);
 ...
 private:
  ...
  const char* getResourceType() const override {
    ...
    switch (fSupportedUsages) {
      case (UsageFlags::kColorAttachment): return "ColorAttachment";
      case (UsageFlags::kStencilAttachment): return "StencilAttachment";
