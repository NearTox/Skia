...
sk_sp<GrTexture> GrGpu::createTexture(
    const GrSurfaceDesc& origDesc, SkBudgeted budgeted, const GrMipLevel texels[],
    int mipLevelCount) {
...
bool GrGpu::copySurface(
  ...
  return this->onCopySurface(
      dst, dstOrigin, src, srcOrigin, srcRect, dstPoint, canDiscardOutsideDstRect);
}
...
bool GrGpu::readPixels(
  ...
  if (!GrSurfacePriv::AdjustReadPixelParams(
          surface->width(), surface->height(), bpp, &left, &top, &width, &height, &buffer,
          &rowBytes)) {
    return false;
  }
...
GrBackendTexture GrGpu::createTestingOnlyBackendTexture(
    int w, int h, SkColorType colorType, GrMipMapped mipMapped, GrRenderable renderable,
  ...
  return this->createBackendTexture(w, h, format, mipMapped, renderable, pixels, rowBytes);
}
