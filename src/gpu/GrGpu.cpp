...
bool GrGpu::IsACopyNeededForRepeatWrapMode(
    const GrCaps* caps, GrTextureProxy* texProxy, SkISize dimensions, GrSamplerState::Filter filter,
    GrTextureProducer::CopyParams* copyParams, SkScalar scaleAdjust[2]) {
...
static bool validate_texel_levels(
    int w, int h, GrColorType texelColorType, const GrMipLevel* texels, int mipLevelCount,
    const GrCaps* caps) {
...
sk_sp<GrTexture> GrGpu::createTextureCommon(
  ...
  auto tex = this->onCreateTexture(
      desc, format, renderable, renderTargetSampleCnt, budgeted, isProtected, mipLevelCount,
      levelClearMask);
  if (tex) {
...
sk_sp<GrTexture> GrGpu::createTexture(
  ...
  if (texelLevelCount) {
    if (!validate_texel_levels(
            desc.fWidth, desc.fHeight, srcColorType, texels, texelLevelCount, this->caps())) {
      return nullptr;
    }
  }
  ...
  auto tex = this->createTextureCommon(
      desc, format, renderable, renderTargetSampleCnt, budgeted, isProtected, texelLevelCount,
      levelClearMask);
  if (tex) {
    ...
    if (texelLevelCount && texels[0].fPixels) {
      if (!this->writePixels(
              tex.get(), 0, 0, desc.fWidth, desc.fHeight, textureColorType, srcColorType, texels,
              texelLevelCount)) {
        return nullptr;
      }
...
void GrGpu::resolveRenderTarget(
    GrRenderTarget* target, const SkIRect& resolveRect, GrSurfaceOrigin origin,
...
GrSemaphoresSubmitted GrGpu::finishFlush(
  ...
  if (!this->onFinishFlush(proxies, n, access, info, externalRequests) || failedSemaphoreCreation) {
...
GrBackendTexture GrGpu::createBackendTexture(
  ...
  return this->onCreateBackendTexture(dimensions, format, renderable, mipMapped, isProtected, data);
}
...
GrBackendTexture GrGpu::createCompressedBackendTexture(
  ...
  return this->onCreateCompressedBackendTexture(dimensions, format, mipMapped, isProtected, data);
}
