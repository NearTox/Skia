...
GrGpu::~GrGpu() { SkASSERT(fBusyStagingBuffers.isEmpty()); }
...
bool GrGpu::IsACopyNeededForMips(
...
sk_sp<GrTexture> GrGpu::createTexture(
  ...
  auto tex = this->createTextureCommon(
      dimensions, format, renderable, renderTargetSampleCnt, budgeted, isProtected, texelLevelCount,
      levelClearMask);
  if (tex) {
    ...
    if (texelLevelCount && texels[0].fPixels) {
      if (!this->writePixels(
              tex.get(), 0, 0, dimensions.fWidth, dimensions.fHeight, textureColorType,
...
bool GrGpu::readPixels(
  ...
  return this->onReadPixels(
      surface, left, top, width, height, surfaceColorType, dstColorType, buffer, rowBytes);
}
...
bool GrGpu::writePixels(
  ...
  if (this->onWritePixels(
          surface, left, top, width, height, surfaceColorType, srcColorType, texels, mipLevelCount,
          prepForTexSampling)) {
...
bool GrGpu::transferPixelsTo(
  ...
  if (this->onTransferPixelsTo(
          texture, left, top, width, height, textureColorType, bufferColorType, transferBuffer,
          offset, rowBytes)) {
    SkIRect rect = SkIRect::MakeXYWH(left, top, width, height);
...
bool GrGpu::transferPixelsFrom(
  ...
  if (this->onTransferPixelsFrom(
          surface, left, top, width, height, surfaceColorType, bufferColorType, transferBuffer,
          offset)) {
...
void GrGpu::resolveRenderTarget(
...
void GrGpu::validateStagingBuffers() const {
  for (const auto& i : fStagingBuffers) {
    ...
    SkASSERT(
        fAvailableStagingBuffers.isInList(buffer) || fActiveStagingBuffers.isInList(buffer) ||
...
void GrGpu::executeFlushInfo(
    ...
    for (int i = 0; i < info.fNumSemaphores; ++i) {
      if (info.fSignalSemaphores[i].isInitialized()) {
        semaphores[i] = resourceProvider->wrapBackendSemaphore(
            info.fSignalSemaphores[i], GrResourceProvider::SemaphoreWrapType::kWillSignal,
  ...
  this->prepareSurfacesForBackendAccessAndExternalIO(proxies, numProxies, access, externalRequests);
}
...
void GrGpu::Stats::dump(SkString* out) {
  ...
  for (int i = 0; i < Stats::kNumProgramCacheResults - 1; ++i) {
    out->appendf(
        "Inline Program Cache %s %d\n", cache_result_to_str(i), fInlineProgramCacheStats[i]);
  }
  ...
  for (int i = 0; i < Stats::kNumProgramCacheResults - 1; ++i) {
    out->appendf(
        "Precompile Program Cache %s %d\n", cache_result_to_str(i), fPreProgramCacheStats[i]);
  }
...
bool GrGpu::updateBackendTexture(
    const GrBackendTexture& backendTexture, GrGpuFinishedProc finishedProc,
    GrGpuFinishedContext finishedContext, const BackendTextureData* data) {
  SkASSERT(data);
  const GrCaps* caps = this->caps();
...
GrBackendTexture GrGpu::createCompressedBackendTexture(
  ...
  return this->onCreateCompressedBackendTexture(
      dimensions, format, mipMapped, isProtected, std::move(callback), data);
}
