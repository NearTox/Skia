...
void draw_texture(
    ...
    const SkRect& dstRect, const SkPoint dstClip[4], GrAA aa, GrQuadAAFlags aaFlags,
  ...
  if (constraint != SkCanvas::kStrict_SrcRectConstraint && !proxy->isFunctionallyExact()) {
    ...
    float buffer = 0.5f * (aa == GrAA::kYes) + 0.5f * (filter == GrSamplerState::Filter::kLinear);
  ...
  if (dstClip) {
    ...
    sdc->drawTexture(
        ...
        paint.getBlendMode_or(SkBlendMode::kSrcOver), color, srcRect, dstRect, aa, aaFlags,
...
void draw_image(
    ...
    SkSamplingOptions sampling, SkTileMode tm = SkTileMode::kClamp) {
  const SkMatrix& ctm(matrixProvider.localToDevice());
  if (tm == SkTileMode::kClamp && !image.isYUVA() &&
    ...
    draw_texture(
        sdc, clip, ctm, paint, sampling.filter, src, dst, dstClip, aa, aaFlags, constraint,
        std::move(view), info);
    return;
  }
  ...
  if (!sampling.useCubic && sampling.filter == SkFilterMode::kLinear && restrictToSubset &&
      sampling.mipmap == SkMipmapMode::kNone && coordsAllInsideSrcRect && !image.isYUVA()) {
  ...
  if (!SkPaintToGrPaintWithTexture(
          rContext, sdc->colorInfo(), paint, matrixProvider, std::move(fp), image.isAlphaOnly(),
          &grPaint)) {
    return;
  }
  ...
  if (!mf) {
    ...
    if (dstClip) {
      ...
      sdc->fillRectWithEdgeAA(
          clip, std::move(grPaint), aa, aaFlags, ctm, dst,
...
void draw_tiled_bitmap(
    ...
    const SkPaint& paint, GrAA aa, SkCanvas::SrcRectConstraint constraint,
    ...
    for (int y = 0; y <= ny; y++) {
      ...
      if (SkBitmap subsetBmp; bitmap.extractSubset(&subsetBmp, iTileR)) {
        ...
        draw_image(
            ...
            nullptr, offsetSrcToDst, aa, aaFlags, constraint, sampling, tileMode);
      }
    }
  }
}
...
bool can_disable_mipmap(
    const SkMatrix& viewM, const SkMatrix& localM, bool sharpenMipmappedTextures) {
  SkMatrix matrix;
...
void Device::drawSpecial(
  ...
  draw_image(
      ...
      dst, nullptr, srcToDst, aa, aaFlags, SkCanvas::kStrict_SrcRectConstraint, sampling);
}

void Device::drawImageQuad(
    const SkImage* image, const SkRect* srcRect, const SkRect* dstRect, const SkPoint dstClip[4],
    GrAA aa, GrQuadAAFlags aaFlags, const SkMatrix* preViewMatrix,
    const SkSamplingOptions& origSampling, const SkPaint& paint,
    ...
    if (should_tile_image_id(
      ...
      if (SkBitmap bm; as_IB(image)->getROPixels(nullptr, &bm)) {
        ...
        draw_tiled_bitmap(
            ...
            src, clippedSubset, paint, aa, constraint, sampling, tileMode);
        return;
      }
    }
  }
  ...
  draw_image(
      ...
      dst, dstClip, srcToDst, aa, aaFlags, constraint, sampling);
  return;
}
  ...
  auto draw = [&](int nextBase) {
    if (n > 0) {
      ...
      fSurfaceDrawContext->drawTextureSet(
          ...
          mode, GrAA::kYes, constraint, this->localToDevice(), std::move(textureXform));
    }
    base = nextBase;
    n = 0;
    p = 0;
