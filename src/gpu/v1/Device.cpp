...
SkImageInfo make_info(skgpu::v1::SurfaceDrawContext* sdc, bool opaque) {
  ...
  return SkImageInfo::Make(
      sdc->width(), sdc->height(), colorType, opaque ? kOpaque_SkAlphaType : kPremul_SkAlphaType,
...
bool init_vertices_paint(
    ...
    const SkMatrixProvider& matrixProvider, SkBlendMode bmode, bool hasColors, GrPaint* grPaint) {
  if (skPaint.getShader()) {
    if (hasColors) {
      ...
      return SkPaintToGrPaintWithBlend(
          rContext, colorInfo, skPaint, matrixProvider, bmode, grPaint);
    } else {
    ...
    if (hasColors) {
      ...
      return SkPaintToGrPaintWithPrimitiveColor(
          rContext, colorInfo, skPaint, matrixProvider, grPaint);
    } else {
...
sk_sp<BaseDevice> Device::Make(
  ...
  auto sdc = MakeSurfaceDrawContext(
      rContext, budgeted, ii, fit, sampleCount, mipMapped, isProtected, origin, props);
...
Device::Device(std::unique_ptr<SurfaceDrawContext> sdc, DeviceFlags flags)
    : INHERITED(
          sk_ref_sp(sdc->recordingContext()),
          make_info(sdc.get(), SkToBool(flags & DeviceFlags::kIsOpaque)), sdc->surfaceProps()),
...
std::unique_ptr<SurfaceDrawContext> Device::MakeSurfaceDrawContext(
    GrRecordingContext* rContext, SkBudgeted budgeted, const SkImageInfo& origInfo,
    SkBackingFit fit, int sampleCount, GrMipmapped mipmapped, GrProtected isProtected,
    GrSurfaceOrigin origin, const SkSurfaceProps& surfaceProps) {
  if (!rContext) {
    return nullptr;
  }
  ...
  return SurfaceDrawContext::Make(
      rContext, SkColorTypeToGrColorType(origInfo.colorType()), origInfo.refColorSpace(), fit,
      origInfo.dimensions(), surfaceProps, sampleCount, mipmapped, isProtected, origin, budgeted);
}
...
void Device::drawPoints(
  ...
  if (paint.getPathEffect() && 2 == count && SkCanvas::kLines_PointMode == mode) {
    ...
    fSurfaceDrawContext->drawPath(
        this->clip(), std::move(grPaint), aa, this->localToDevice(), path, style);
    return;
  }
...
void Device::drawVertices(const SkVertices* vertices, SkBlendMode mode, const SkPaint& paint) {
  ...
  if (!init_vertices_paint(
          fContext.get(), fSurfaceDrawContext->colorInfo(), paint, this->asMatrixProvider(), mode,
          info.hasColors(), &grPaint)) {
    return;
  }
  fSurfaceDrawContext->drawVertices(
...
void Device::drawAtlas(
    ...
    int count, SkBlendMode mode, const SkSamplingOptions& sampling, const SkPaint& paint) {
  ASSERT_SINGLE_OWNER
