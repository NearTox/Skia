...
class Device : public BaseDevice {
 public:
  GrSurfaceProxyView readSurfaceView() override { return fSurfaceDrawContext->readSurfaceView(); }

  bool wait(
      int numSemaphores, const GrBackendSemaphore* waitSemaphores,
      bool deleteSemaphoresAfterWait) override;

  void discard() override { fSurfaceDrawContext->discard(); }

  bool replaceBackingProxy(
      SkSurface::ContentChangeMode, sk_sp<GrRenderTargetProxy>, GrColorType, sk_sp<SkColorSpace>,
      GrSurfaceOrigin, const SkSurfaceProps&) override;
  ...
  void asyncRescaleAndReadPixels(
      ...
      RescaleMode rescaleMode, ReadPixelsCallback callback, ReadPixelsContext context) override;

  void asyncRescaleAndReadPixelsYUV420(
      SkYUVColorSpace yuvColorSpace, sk_sp<SkColorSpace> dstColorSpace, const SkIRect& srcRect,
      SkISize dstSize, RescaleGamma rescaleGamma, RescaleMode, ReadPixelsCallback callback,
   ...
   */
  static sk_sp<BaseDevice> Make(
      GrRecordingContext*, SkBudgeted, const SkImageInfo&, SkBackingFit, int sampleCount,
      GrMipmapped, GrProtected, GrSurfaceOrigin, const SkSurfaceProps&, InitContents);
  ...
  void drawPoints(
      SkCanvas::PointMode mode, size_t count, const SkPoint[], const SkPaint& paint) override;
  ...
  void drawAtlas(
      const SkImage* atlas, const SkRSXform[], const SkRect[], const SkColor[], int count,
  ...
  void drawImageLattice(
      const SkImage*, const SkCanvas::Lattice&, const SkRect& dst, SkFilterMode,
  ...
  void drawSpecial(
      SkSpecialImage*, const SkMatrix& localToDevice, const SkSamplingOptions&,
      const SkPaint&) override;
  ...
  void drawEdgeAAImageSet(
      ...
      const SkSamplingOptions&, const SkPaint&, SkCanvas::SrcRectConstraint) override;
 ...
 private:
  ...
  static sk_sp<BaseDevice> Make(std::unique_ptr<SurfaceDrawContext>, SkAlphaType, InitContents);
  ...
  void drawImageQuad(
      const SkImage*, const SkRect* src, const SkRect* dst, const SkPoint dstClip[4], GrAA aa,
      GrQuadAAFlags aaFlags, const SkMatrix* preViewMatrix, const SkSamplingOptions&,
  ...
  void drawViewLattice(
      GrSurfaceProxyView, const GrColorInfo& colorInfo, std::unique_ptr<SkLatticeIter>,
      const SkRect& dst, SkFilterMode, const SkPaint&);

  static std::unique_ptr<SurfaceDrawContext> MakeSurfaceDrawContext(
      GrRecordingContext*, SkBudgeted, const SkImageInfo&, SkBackingFit, int sampleCount,
      GrMipmapped, GrProtected, GrSurfaceOrigin, const SkSurfaceProps&);
