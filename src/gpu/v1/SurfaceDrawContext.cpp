...
std::unique_ptr<SurfaceDrawContext> SurfaceDrawContext::Make(
    ...
    GrProtected isProtected, GrSwizzle readSwizzle, GrSwizzle writeSwizzle, GrSurfaceOrigin origin,
...
std::unique_ptr<SurfaceDrawContext> SurfaceDrawContext::MakeFromBackendTexture(
    ...
    const SkSurfaceProps& surfaceProps, sk_sp<GrRefCntedCallback> releaseHelper) {
...
SurfaceDrawContext::SurfaceDrawContext(
    ...
    : SurfaceFillContext(
          ...
          {colorType, kPremul_SkAlphaType, std::move(colorSpace)}, flushTimeOpsTask),
...
void SurfaceDrawContext::drawGlyphRunListNoCache(
    const GrClip* clip, const SkMatrixProvider& viewMatrix, const SkGlyphRunList& glyphRunList,
    const SkPaint& paint) {
  ...
  for (auto& glyphRun : glyphRunList) {
    ...
    fGlyphPainter.processGlyphRun(glyphRun, drawMatrix, paint, control, &painter);
  }
}

void SurfaceDrawContext::drawGlyphRunListWithCache(
    const GrClip* clip, const SkMatrixProvider& viewMatrix, const SkGlyphRunList& glyphRunList,
    const SkPaint& paint) {
  SkMatrix drawMatrix(viewMatrix.localToDevice());
  drawMatrix.preTranslate(glyphRunList.origin().x(), glyphRunList.origin().y());

  GrSDFTControl control = this->recordingContext()->priv().getSDFTControl(
      this->surfaceProps().isUseDeviceIndependentFonts());

  auto [canCache, key] = GrTextBlob::Key::Make(
      glyphRunList, paint, fSurfaceProps, this->colorInfo(), drawMatrix, control);
...
SurfaceDrawContext::QuadOptimization SurfaceDrawContext::attemptQuadOptimization(
  ...
  GrClip::PreClipResult result =
      clip ? clip->preApply(drawBounds, *aa) : GrClip::PreClipResult(GrClip::Effect::kUnclipped);
...
void SurfaceDrawContext::drawTexturedQuad(
    ...
    GrSamplerState::MipmapMode mm, const SkPMColor4f& color, SkBlendMode blendMode, GrAA aa,
    DrawQuad* quad, const SkRect* subset) {
  ASSERT_SINGLE_OWNER
  RETURN_IF_ABANDONED
  SkDEBUGCODE(this->validate();)
  ...
  QuadOptimization opt =
      this->attemptQuadOptimization(clip, nullptr /*stencil*/, &aa, quad, nullptr /*paint*/);
...
void SurfaceDrawContext::fillRectToRect(
  ...
  if ((fContext->priv().caps()->reducedShaderMode() || this->alwaysAntialias()) &&
    ...
    QuadOptimization opt =
        this->attemptQuadOptimization(clip, nullptr /*stencil*/, &aa, &quad, &paint);
    if (opt < QuadOptimization::kClipApplied) {
...
void SurfaceDrawContext::drawQuadSet(
    const GrClip* clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix,
    const GrQuadSetEntry quads[], int cnt) {
  GrAAType aaType = this->chooseAAType(aa);
...
void SurfaceDrawContext::drawTextureSet(
    ...
    GrSamplerState::Filter filter, GrSamplerState::MipmapMode mm, SkBlendMode mode, GrAA aa,
...
void SurfaceDrawContext::drawVertices(
  ...
  GrOp::Owner op = DrawVerticesOp::Make(
...
void SurfaceDrawContext::drawStrokedLine(
  ...
  SkPoint corners[4] = {
      points[0] - ortho - parallel, points[0] + ortho - parallel, points[1] + ortho + parallel,
