...
bool GrRecordingContext::init(sk_sp<const GrCaps> caps, sk_sp<GrSkSLFPFactoryCache> cache) {
  ...
  fStrikeCache.reset(
      new GrStrikeCache(this->caps(), this->options().fGlyphCacheTextureMaximumBytes));
...
void GrRecordingContext::setupDrawingManager(bool sortOpLists, bool reduceOpListSplitting) {
  ...
  fDrawingManager.reset(new GrDrawingManager(
      this, prcOptions, textContextOptions, sortOpLists, reduceOpListSplitting));
}
...
sk_sp<GrSurfaceContext> GrRecordingContext::makeDeferredSurfaceContext(
  ...
  sk_sp<GrSurfaceContext> sContext =
      this->makeWrappedSurfaceContext(std::move(proxy), std::move(colorSpace), props);
...
sk_sp<GrRenderTargetContext> GrRecordingContext::makeDeferredRenderTargetContext(
  ...
  sk_sp<GrRenderTargetContext> renderTargetContext =
      drawingManager->makeRenderTargetContext(std::move(rtp), std::move(colorSpace), surfaceProps);
...
static inline GrPixelConfig GrPixelConfigFallback(GrPixelConfig config) {
  switch (config) {
    ...
    case kRGBA_half_Clamped_GrPixelConfig: return kRGBA_8888_GrPixelConfig;
    case kSBGRA_8888_GrPixelConfig: return kSRGBA_8888_GrPixelConfig;
    case kAlpha_half_GrPixelConfig:
    case kAlpha_half_as_Red_GrPixelConfig: return kRGBA_half_GrPixelConfig;
    case kGray_8_GrPixelConfig:
    case kGray_8_as_Lum_GrPixelConfig:
    case kGray_8_as_Red_GrPixelConfig: return kRGB_888_GrPixelConfig;
...
sk_sp<GrRenderTargetContext> GrRecordingContext::makeDeferredRenderTargetContextWithFallback(
  ...
  return this->makeDeferredRenderTargetContext(
      localFormat, fit, width, height, config, std::move(colorSpace), sampleCnt, mipMapped, origin,
