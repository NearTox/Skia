...
GrGLProgram::GrGLProgram(
    GrGLGpu* gpu, const GrGLSLBuiltinUniformHandles& builtinUniforms, GrGLuint programID,
    const UniformInfoArray& uniforms, const UniformInfoArray& textureSamplers,
    const VaryingInfoArray& pathProcVaryings,
    std::unique_ptr<GrGLSLPrimitiveProcessor> geometryProcessor,
    std::unique_ptr<GrGLSLXferProcessor> xferProcessor,
    std::unique_ptr<std::unique_ptr<GrGLSLFragmentProcessor>[]> fragmentProcessors,
    int fragmentProcessorCnt, std::unique_ptr<Attribute[]> attributes, int vertexAttributeCnt,
    int instanceAttributeCnt, int vertexStride, int instanceStride)
...
void GrGLProgram::updateUniformsAndTextureBindings(
    const GrRenderTarget* renderTarget, const GrProgramInfo& programInfo) {
  ...
  if (programInfo.hasFixedPrimProcTextures()) {
    this->updatePrimitiveProcessorTextureBindings(
        programInfo.primProc(), programInfo.fixedPrimProcTextures());
  }
  ...
  if (dstTexture) {
    fGpu->bindTexture(
        nextTexSamplerIdx++, GrSamplerState::Filter::kNearest,
        programInfo.pipeline().dstProxyView().swizzle(), static_cast<GrGLTexture*>(dstTexture));
  }
  SkASSERT(nextTexSamplerIdx == fNumTextureSamplers);
}

void GrGLProgram::updatePrimitiveProcessorTextureBindings(
    const GrPrimitiveProcessor& primProc, const GrSurfaceProxy* const proxies[]) {
  for (int i = 0; i < primProc.numTextureSamplers(); ++i) {
    SkASSERT(proxies[i]->asTextureProxy());
    auto* tex = static_cast<GrGLTexture*>(proxies[i]->peekTexture());
    fGpu->bindTexture(
        i, primProc.textureSampler(i).samplerState(), primProc.textureSampler(i).swizzle(), tex);
  }
}
...
void GrGLProgram::setFragmentData(const GrPipeline& pipeline, int* nextTexSamplerIdx) {
  ...
  for (; fpIter && glslIter; ++fpIter, ++glslIter) {
    ...
    for (int i = 0; i < fpIter->numTextureSamplers(); ++i) {
      ...
      fGpu->bindTexture(
          (*nextTexSamplerIdx)++, sampler.samplerState(), sampler.view().swizzle(),
...
void GrGLProgram::setRenderTargetState(
    const GrRenderTarget* rt, GrSurfaceOrigin origin, const GrPrimitiveProcessor& primProc) {
