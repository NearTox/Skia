...
sk_sp<GrGLProgram> GrGLProgram::Make(
  ...
  sk_sp<GrGLProgram> program(new GrGLProgram(
      gpu, builtinUniforms, programID, uniforms, textureSamplers, std::move(gpImpl),
      std::move(xpImpl), std::move(fpImpls), std::move(attributes), vertexAttributeCnt,
      instanceAttributeCnt, vertexStride, instanceStride));
...
void GrGLProgram::bindTextures(
  ...
  for (int i = 0; i < geomProc.numTextureSamplers(); ++i) {
    ...
    fGpu->bindTexture(
        i, geomProc.textureSampler(i).samplerState(), geomProc.textureSampler(i).swizzle(),
        overrideTexture);
  }
...
void GrGLProgram::setRenderTargetState(
    const GrRenderTarget* rt, GrSurfaceOrigin origin, const GrGeometryProcessor& geomProc) {
