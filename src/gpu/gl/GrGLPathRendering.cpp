...
static GrGLenum gr_stencil_op_to_gl_path_rendering_fill_mode(GrStencilOp op) noexcept {
  switch (op) {
    default:
      ...
      /* fallthrough */
    case GrStencilOp::kIncWrap: return GR_GL_COUNT_UP;
    case GrStencilOp::kInvert: return GR_GL_INVERT;
  }
}
...
void GrGLPathRendering::onDrawPath(
  ...
  if (!this->gpu()->flushGLState(
          renderTarget, origin, primProc, pipeline, &fixedDynamicState, nullptr, 1, false)) {
    return;
  }
  const GrGLPath* glPath = static_cast<const GrGLPath*>(path);
  ...
  if (glPath->shouldStroke()) {
    ...
    GL_CALL(StencilThenCoverStrokePath(glPath->pathID(), 0xffff, writeMask, GR_GL_BOUNDING_BOX));
  } else {
    GL_CALL(StencilThenCoverFillPath(glPath->pathID(), fillMode, writeMask, GR_GL_BOUNDING_BOX));
  }
}

void GrGLPathRendering::setProgramPathFragmentInputTransform(
...
void GrGLPathRendering::setProjectionMatrix(
    const SkMatrix& matrix, const SkISize& renderTargetSize, GrSurfaceOrigin renderTargetOrigin) {
...
void GrGLPathRendering::flushPathStencilSettings(const GrStencilSettings& stencilSettings) {
  ...
  if (fHWPathStencilSettings != stencilSettings) {
    ...
    if (!fHWPathStencilSettings.isValid() || ref != fHWPathStencilSettings.frontAndBack().fRef ||
