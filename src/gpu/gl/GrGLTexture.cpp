...
GrGLTexture::GrGLTexture(
    GrGLGpu* gpu, const GrSurfaceDesc& desc, const IDDesc& idDesc, GrMipMapsStatus mipMapsStatus)
    : GrSurface(gpu, desc),
      INHERITED(gpu, desc, TextureTypeFromTarget(idDesc.fInfo.fTarget), mipMapsStatus) {
  this->init(desc, idDesc);
}
...
void GrGLTexture::dumpMemoryStatistics(SkTraceMemoryDump* traceMemoryDump) const {
  ...
  bool refsWrappedTextureObjects = this->fTextureIDOwnership == GrBackendObjectOwnership::kBorrowed;
  ...
  this->dumpMemoryStatisticsPriv(
      traceMemoryDump, resourceName, "Texture", GrGLTexture::gpuMemorySize());
