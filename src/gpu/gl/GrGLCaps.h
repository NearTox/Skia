...
class GrGLCaps : public GrCaps {
 public:
  ...
  int getRenderTargetSampleCount(
      int requestedCount, GrColorType ct, const GrBackendFormat& format) const override {
    return this->getRenderTargetSampleCount(
        requestedCount, ct, GrGLBackendFormatToGLFormat(format));
  }
 ...
 private:
  ...
  void applyDriverCorrectnessWorkarounds(
      const GrGLContextInfo&, const GrContextOptions&, GrShaderCaps*, FormatWorkarounds*);
