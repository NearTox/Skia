...
bool GrGLRenderTarget::completeStencilAttachment(GrAttachment* stencil, bool useMultisampleFBO) {
  ...
  if (nullptr == stencil) {
    ...
    GR_GL_CALL(
        interface, FramebufferRenderbuffer(
                       GR_GL_FRAMEBUFFER, GR_GL_STENCIL_ATTACHMENT, GR_GL_RENDERBUFFER, rb));
    if (GrGLFormatIsPackedDepthStencil(glStencil->format())) {
      GR_GL_CALL(
          interface, FramebufferRenderbuffer(
                         GR_GL_FRAMEBUFFER, GR_GL_DEPTH_ATTACHMENT, GR_GL_RENDERBUFFER, rb));
    } else {
      GR_GL_CALL(
          interface, FramebufferRenderbuffer(
...
bool GrGLRenderTarget::ensureDynamicMSAAAttachment() {
  ...
  if (resourceProvider->caps()->msaaResolvesAutomatically()) {
    if (GrGLTexture* glTex = static_cast<GrGLTexture*>(this->asTexture())) {
      GL_CALL(FramebufferTexture2DMultisample(
          GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, glTex->target(), glTex->textureID(),
  ...
  fDynamicMSAAAttachment.reset(
      static_cast<GrGLAttachment*>(resourceProvider
                                       ->getDiscardableMSAAAttachment(
                                           this->dimensions(), this->backendFormat(),
                                           internalSampleCount, GrProtected(this->isProtected()))
                                       .release()));
