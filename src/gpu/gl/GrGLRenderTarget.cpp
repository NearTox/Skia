...
sk_sp<GrGLRenderTarget> GrGLRenderTarget::MakeWrapped(
  ...
  if (stencilBits) {
    ...
    sb = new GrGLAttachment(
        gpu, sbDesc, dimensions, GrAttachment::UsageFlags::kStencilAttachment, sampleCount, sFmt);
  }
...
size_t GrGLRenderTarget::onGpuMemorySize() const {
  return GrSurface::ComputeSize(
      this->backendFormat(), this->dimensions(), fNumSamplesOwnedPerPixel, GrMipmapped::kNo);
}
...
void GrGLRenderTarget::dumpMemoryStatistics(SkTraceMemoryDump* traceMemoryDump) const {
  ...
  if (fMSColorRenderbufferID) {
    size_t size = GrSurface::ComputeSize(
        this->backendFormat(), this->dimensions(), this->msaaSamples(), GrMipmapped::kNo);
