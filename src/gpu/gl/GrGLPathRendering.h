...
class GrGLPathRendering : public GrPathRendering {
 public:
  ...
  void setProgramPathFragmentInputTransform(
      GrGLuint program, GrGLint location, GrGLenum genMode, GrGLint components, const SkMatrix&);
  ...
  struct MatrixState {
    ...
    void getRTAdjustedGLMatrix(float* destMatrix) {
      ...
      if (kBottomLeft_GrSurfaceOrigin == fRenderTargetOrigin) {
        combined.setAll(
            SkIntToScalar(2) / fRenderTargetSize.fWidth, 0, -SK_Scalar1, 0,
