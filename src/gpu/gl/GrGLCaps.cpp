...
void GrGLCaps::init(
  ...
  if (GR_IS_GR_GL(standard)) {
    ...
    fMultiDrawIndirectSupport =
        ...
        (fDrawIndirectSupport && !fBaseInstanceSupport &&  // The ARB extension has no base inst.
  ...
  if (!contextOptions.fDisableDriverCorrectnessWorkarounds) {
    this->applyDriverCorrectnessWorkarounds(
        ctxInfo, contextOptions, shaderCaps, &formatWorkarounds);
  }
...
void GrGLCaps::initFSAASupport(
  ...
  if (GR_IS_GR_GL(ctxInfo.standard())) {
    ...
    if (ctxInfo.version() >= GR_GL_VER(3, 0) && ctxInfo.renderer() != kGalliumLLVM_GrGLRenderer) {
...
void GrGLCaps::onDumpJSON(SkJSONWriter* writer) const {
  ...
  for (int i = 0; i < kGrPixelConfigCnt; ++i) {
    ...
    //        writer->appendHexU32("i_for_teximage",
...
void GrGLCaps::initFormatTable(
  ...
  // R8
  {
    ...
    info.fInternalFormatForTexImage = texImageSupportsSizedInternalFormat ? GR_GL_R8 : GR_GL_RED;
    info.fInternalFormatForRenderbuffer =
  ...
  // R16F
  {
    ...
    info.fInternalFormatForTexImage = texImageSupportsSizedInternalFormat ? GR_GL_R16F : GR_GL_RED;
    info.fInternalFormatForRenderbuffer =
    ...
    if (textureRedSupport && hasFP16Textures) {
      ...
      {
        uint32_t flags = ColorTypeInfo::kUploadData_Flag | ColorTypeInfo::kRenderable_Flag;
        info.fColorTypeInfos.emplace_back(GrColorType::kAlpha_F16, flags, GrSwizzle("000r"));
  ...
  // LUMINANCE16F
  {
    ...
    info.fInternalFormatForRenderbuffer =
        renderbufferStorageSupportsSizedInternalFormat ? GR_GL_LUMINANCE16F : GR_GL_LUMINANCE;
    info.fDefaultExternalType = halfFloatType;
  ...
  // RGB8
  {
    ...
    info.fInternalFormatForTexImage = texImageSupportsSizedInternalFormat ? GR_GL_RGB8 : GR_GL_RGB;
    info.fInternalFormatForRenderbuffer =
  ...
  // RG8
  {
    ...
    info.fInternalFormatForTexImage = texImageSupportsSizedInternalFormat ? GR_GL_RG8 : GR_GL_RG;
    info.fInternalFormatForRenderbuffer =
  ...
  // SRGB8_ALPHA8
  {
    ...
    info.fInternalFormatForRenderbuffer =
        renderbufferStorageSupportsSizedInternalFormat ? GR_GL_SRGB8_ALPHA8 : GR_GL_SRGB_ALPHA;
    info.fDefaultExternalType = GR_GL_UNSIGNED_BYTE;
    if (fSRGBSupport) {
      uint32_t srgbRenderFlags = formatWorkarounds.fDisableSRGBRenderWithMSAAForMacAMD
                                     ? nonMSAARenderFlags
                                     : msaaRenderFlags;
  ...
  // GR_GL_R16
  {
    ...
    info.fInternalFormatForTexImage = texImageSupportsSizedInternalFormat ? GR_GL_R16 : GR_GL_RED;
    info.fInternalFormatForRenderbuffer =
  ...
  // GR_GL_RG16
  {
    ...
    info.fInternalFormatForTexImage = texImageSupportsSizedInternalFormat ? GR_GL_RG16 : GR_GL_RG;
    info.fInternalFormatForRenderbuffer =
  ...
  // GR_GL_RG16F
  {
    ...
    info.fInternalFormatForTexImage = texImageSupportsSizedInternalFormat ? GR_GL_RG16F : GR_GL_RG;
    info.fInternalFormatForRenderbuffer =
  ...
  for (int i = 0; i < kGrGLFormatCount; ++i) {
    ...
    SkASSERT(
        !((fFormatTable[i].fFlags & FormatInfo::kFBOColorAttachmentWithMSAA_Flag) &&
...
void GrGLCaps::initConfigTable(
  ...
  */
  ...
  fConfigTable[kUnknown_GrPixelConfig].fFormats.fExternalFormat[kReadPixels_ExternalFormatUsage] =
      0;
  ...
  fConfigTable[kRGB_888_GrPixelConfig].fFormats.fExternalFormat[kReadPixels_ExternalFormatUsage] =
      GR_GL_RGBA;
  ...
  fConfigTable[kSRGBA_8888_GrPixelConfig]
      .fFormats.fExternalFormat[kReadPixels_ExternalFormatUsage] = GR_GL_RGBA;
  ...
  fConfigTable[kRGBA_1010102_GrPixelConfig]
      .fFormats.fExternalFormat[kReadPixels_ExternalFormatUsage] = GR_GL_RGBA;
...
for (int i = 0; i < kGrPixelConfigCnt; ++i) {
  for (int j = 0; j < kExternalFormatUsageCnt; ++j) {
    SkASSERT(
        defaultEntry.fFormats.fExternalFormat[j] != fConfigTable[i].fFormats.fExternalFormat[j]);
  }
...
bool GrGLCaps::canCopyTexSubImage(
    GrPixelConfig dstConfig, bool dstHasMSAARenderBuffer, const GrTextureType* dstTypeIfTexture,
...
bool GrGLCaps::canCopyAsBlit(
    GrPixelConfig dstConfig, int dstSampleCnt, const GrTextureType* dstTypeIfTexture,
    GrPixelConfig srcConfig, int srcSampleCnt, const GrTextureType* srcTypeIfTexture,
...
bool GrGLCaps::onCanCopySurface(
  ...
  return this->canCopyTexSubImage(
             dstConfig, has_msaa_render_buffer(dst, *this), dstTexTypePtr, srcConfig,
             has_msaa_render_buffer(src, *this), srcTexTypePtr) ||
         this->canCopyAsBlit(
             dstConfig, dstSampleCnt, dstTexTypePtr, srcConfig, srcSampleCnt, srcTexTypePtr,
...
void GrGLCaps::applyDriverCorrectnessWorkarounds(
    ...
    GrShaderCaps* shaderCaps, FormatWorkarounds* formatWorkarounds) {
...
GrCaps::SupportedRead GrGLCaps::supportedReadPixelsColorType(
  ...
  switch (this->getFormatInfo(srcFormat).fFormatType) {
    case FormatType::kUnknown: return {GrSwizzle{}, GrColorType::kUnknown};
    ...
    case FormatType::kFloat: return {swizzle, GrColorType::kRGBA_F32};
  }
  SkUNREACHABLE;
  return {GrSwizzle{}, GrColorType::kUnknown};
}
...
int GrGLCaps::getRenderTargetSampleCount(
    int requestedCount, GrColorType grCT, GrGLFormat format) const {
  const FormatInfo& info = this->getFormatInfo(format);
...
GrGLFormat GrGLCaps::pixelConfigToFormat(GrPixelConfig config) const {
  switch (config) {
    case kRGB_888X_GrPixelConfig: return GrGLFormat::kRGBA8;
    case kAlpha_8_as_Alpha_GrPixelConfig: return GrGLFormat::kALPHA8;
    case kAlpha_8_as_Red_GrPixelConfig: return GrGLFormat::kR8;
    case kGray_8_as_Lum_GrPixelConfig: return GrGLFormat::kLUMINANCE8;
    case kGray_8_as_Red_GrPixelConfig: return GrGLFormat::kR8;
    case kAlpha_half_as_Lum_GrPixelConfig: return GrGLFormat::kLUMINANCE16F;
    case kAlpha_half_as_Red_GrPixelConfig: return GrGLFormat::kR16F;
    case kRGB_ETC1_GrPixelConfig: {
      ...
      return usesETC1 ? GrGLFormat::kCOMPRESSED_ETC1_RGB8 : GrGLFormat::kCOMPRESSED_RGB8_ETC2;
    }
...
GrPixelConfig GrGLCaps::validateBackendRenderTarget(
    const GrBackendRenderTarget& rt, GrColorType ct) const {
  GrGLFramebufferInfo fbInfo;
...
GrPixelConfig GrGLCaps::onGetConfigFromBackendFormat(
    const GrBackendFormat& format, GrColorType ct) const {
  const GrGLenum* glFormat = format.getGLFormat();
  if (!glFormat) {
    return kUnknown_GrPixelConfig;
  }
...
static GrPixelConfig get_yuva_config(GrGLenum format) {
  ...
  switch (format) {
    case GR_GL_ALPHA8: config = kAlpha_8_as_Alpha_GrPixelConfig; break;
    case GR_GL_R8: config = kAlpha_8_as_Red_GrPixelConfig; break;
    case GR_GL_RG8: config = kRG_88_GrPixelConfig; break;
    case GR_GL_RGBA8: config = kRGBA_8888_GrPixelConfig; break;
    case GR_GL_RGB8: config = kRGB_888_GrPixelConfig; break;
    case GR_GL_BGRA8: config = kBGRA_8888_GrPixelConfig; break;
    case GR_GL_RGB10_A2: config = kRGBA_1010102_GrPixelConfig; break;
    case GR_GL_LUMINANCE16F: config = kAlpha_half_as_Lum_GrPixelConfig; break;
    case GR_GL_R16F: config = kAlpha_half_as_Red_GrPixelConfig; break;
    case GR_GL_R16: config = kR_16_GrPixelConfig; break;
    case GR_GL_RG16: config = kRG_1616_GrPixelConfig; break;
    // Experimental (for Y416 and mutant P016/P010)
    case GR_GL_RGBA16: config = kRGBA_16161616_GrPixelConfig; break;
    case GR_GL_RG16F: config = kRG_half_GrPixelConfig; break;
  }
...
static bool format_color_type_valid_pair(GrGLenum format, GrColorType colorType) {
  switch (colorType) {
    ...
    case GrColorType::kAlpha_8: return GR_GL_ALPHA8 == format || GR_GL_R8 == format;
    case GrColorType::kBGR_565: return GR_GL_RGB565 == format;
    case GrColorType::kABGR_4444: return GR_GL_RGBA4 == format;
    case GrColorType::kRGBA_8888: return GR_GL_RGBA8 == format;
    case GrColorType::kRGBA_8888_SRGB: return GR_GL_SRGB8_ALPHA8 == format;
    case GrColorType::kRGB_888x:
      GR_STATIC_ASSERT(
          GrCompressionTypeClosestColorType(SkImage::kETC1_CompressionType) ==
    ...
    case GrColorType::kRG_88: return GR_GL_RG8 == format;
    case GrColorType::kBGRA_8888:
      return GR_GL_RGBA8 == format || GR_GL_BGRA8 == format || GR_GL_SRGB8_ALPHA8 == format;
    case GrColorType::kRGBA_1010102: return GR_GL_RGB10_A2 == format;
    case GrColorType::kGray_8: return GR_GL_LUMINANCE8 == format || GR_GL_R8 == format;
    case GrColorType::kAlpha_F16: return GR_GL_R16F == format || GR_GL_LUMINANCE16F == format;
    case GrColorType::kRGBA_F16: return GR_GL_RGBA16F == format;
    case GrColorType::kRGBA_F16_Clamped: return GR_GL_RGBA16F == format;
    case GrColorType::kRGBA_F32: return GR_GL_RGBA32F == format;
    case GrColorType::kR_16: return GR_GL_R16 == format;
    case GrColorType::kRG_1616: return GR_GL_RG16 == format;
    // Experimental (for Y416 and mutant P016/P010)
    case GrColorType::kRGBA_16161616: return GR_GL_RGBA16 == format;
    case GrColorType::kRG_F16: return GR_GL_RG16F == format;
  }
...
static GrSwizzle get_swizzle(const GrBackendFormat& format, GrColorType colorType, bool forOutput) {
