...
void GrGLCaps::init(const GrContextOptions& contextOptions,
    ...
    fTextureUsageSupport =
            GR_IS_GR_GL_ES(standard) && ctxInfo.hasExtension("GL_ANGLE_texture_usage");
    ...
    bool isANGLE =
            kANGLE_GrGLDriver == ctxInfo.driver() || kChromium_GrGLDriver == ctxInfo.driver();
    ...
    if (GR_IS_GR_GL(standard)) {
        ...
        shaderCaps->fIntegerSupport =
                ...
                ctxInfo.glslGeneration() >=
                        k330_GrGLSLGeneration;  // We use this value for GLSL ES 3.0.
    } else if (GR_IS_GR_WEBGL(standard)) {
        shaderCaps->fShaderDerivativeSupport =
...
void GrGLCaps::initGLSL(const GrGLContextInfo& ctxInfo, const GrGLInterface* gli) {
    ...
    if (GR_IS_GR_GL(standard)) {
        ...
        shaderCaps->fFlatInterpolationSupport =
                ctxInfo.glslGeneration() >=
                k330_GrGLSLGeneration;  // This is the value for GLSL ES 3.0.
    }                                   // not sure for WebGL
...
void GrGLCaps::initConfigTable(const GrContextOptions& contextOptions,
    ...
    */
    ...
    if (!contextOptions.fDisableDriverCorrectnessWorkarounds) {
        ...
        disableSRGBWriteControlForAdreno4xx = (kAdreno430_GrGLRenderer == ctxInfo.renderer() ||
                                               kAdreno4xx_other_GrGLRenderer == ctxInfo.renderer());
    ...
    uint32_t nonMSAARenderFlags =
            ConfigInfo::kRenderable_Flag | ConfigInfo::kFBOColorAttachment_Flag;
    ...
    fConfigTable[kRGBA_8888_GrPixelConfig]
    ...
    fConfigTable[kBGRA_8888_GrPixelConfig]
    ...
    fConfigTable[kRGBA_4444_GrPixelConfig]
            .fFormats.fExternalFormat[kReadPixels_ExternalFormatUsage] = GR_GL_RGBA;
    ...
    fConfigTable[kRGBA_half_GrPixelConfig]
            .fFormats.fExternalFormat[kReadPixels_ExternalFormatUsage] = GR_GL_RGBA;
    if (GR_IS_GR_GL(standard) || (GR_IS_GR_GL_ES(standard) && version >= GR_GL_VER(3, 0))) {
    ...
    fConfigTable[kRGB_ETC1_GrPixelConfig]
            .fFormats.fExternalFormat[kReadPixels_ExternalFormatUsage] = 0;
    ...
    for (int i = 0; i < kGrPixelConfigCnt; ++i) {
        if (ConfigInfo::kRenderableWithMSAA_Flag & fConfigTable[i].fFlags) {
            ...
            if ((GR_IS_GR_GL(standard) && (version >= GR_GL_VER(4, 2) ||
...
bool GrGLCaps::canCopyTexSubImage(GrPixelConfig dstConfig, bool dstHasMSAARenderBuffer,
                                  ...
                                  bool srcHasMSAARenderBuffer, bool srcIsTextureable,
                                  bool srcIsGLTexture2D, GrSurfaceOrigin srcOrigin) const {
...
bool GrGLCaps::canCopyAsBlit(GrPixelConfig dstConfig, int dstSampleCnt, bool dstIsTextureable,
                             bool dstIsGLTexture2D, GrSurfaceOrigin dstOrigin,
                             GrPixelConfig srcConfig, int srcSampleCnt, bool srcIsTextureable,
                             bool srcIsGLTexture2D, GrSurfaceOrigin srcOrigin,
                             const SkRect& srcBounds, const SkIRect& srcRect,
...
#ifdef SK_DEBUG
    if (this->canCopyAsBlit(dstConfig, dstSampleCnt, SkToBool(dstTex), dstIsTex2D, dstOrigin,
                            srcConfig, srcSampleCnt, SkToBool(srcTex), srcIsTex2D, srcOrigin,
                            src->getBoundsRect(), srcRect, dstPoint) &&
        !src->priv().isExact()) {
        SkASSERT(this->canCopyAsDraw(dstConfig, SkToBool(srcTex)));
    }
#endif

    return this->canCopyTexSubImage(dstConfig, has_msaa_render_buffer(dst, *this), SkToBool(dstTex),
                                    dstIsTex2D, dstOrigin, srcConfig,
                                    has_msaa_render_buffer(src, *this), SkToBool(srcTex),
                                    srcIsTex2D, srcOrigin) ||
           this->canCopyAsBlit(dstConfig, dstSampleCnt, SkToBool(dstTex), dstIsTex2D, dstOrigin,
                               srcConfig, srcSampleCnt, SkToBool(srcTex), srcIsTex2D, srcOrigin,
                               src->getBoundsRect(), srcRect, dstPoint) ||
...
GrColorType GrGLCaps::supportedReadPixelsColorType(GrPixelConfig config,
                                                   GrColorType dstColorType) const noexcept {
...
size_t GrGLCaps::onTransferFromOffsetAlignment(GrColorType bufferColorType) const {
    ...
    switch (fConfigTable[config].fFormats.fExternalType) {
        case GR_GL_UNSIGNED_BYTE:
            return sizeof(GrGLubyte);
        case GR_GL_BYTE:
            return sizeof(GrGLbyte);
        case GR_GL_UNSIGNED_SHORT:
            return sizeof(GrGLushort);
        case GR_GL_SHORT:
            return sizeof(GrGLshort);
        case GR_GL_UNSIGNED_INT:
            return sizeof(GrGLuint);
        case GR_GL_INT:
            return sizeof(GrGLint);
        case GR_GL_HALF_FLOAT:
            return sizeof(GrGLhalf);
        case GR_GL_FLOAT:
            return sizeof(GrGLfloat);
