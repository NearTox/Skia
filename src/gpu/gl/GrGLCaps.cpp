...
void GrGLCaps::init(
  ...
  if (GR_IS_GR_GL(standard)) {
    if (fUsesMixedSamples || version >= GR_GL_VER(3, 0) ||
...
bool GrGLCaps::readPixelsSupported(
    GrPixelConfig surfaceConfig, GrPixelConfig readConfig,
    std::function<void(GrGLenum, GrGLint*)> getIntegerv, std::function<bool()> bindRenderTarget,
  ...
  if (GR_IS_GR_GL(fStandard)) {
    ...
    if (readFormat != GR_GL_RED && readFormat != GR_GL_RG && readFormat != GR_GL_RGB &&
        readFormat != GR_GL_RGBA && readFormat != GR_GL_BGRA && readFormat != GR_GL_RGBA_INTEGER) {
      return false;
    }
...
void GrGLCaps::onDumpJSON(SkJSONWriter* writer) const {
  ...
  static const char* kMSFBOExtStr[] = {
      "None", "Standard", "Apple", "IMG MS To Texture", "EXT MS To Texture", "MixedSamples",
  };
  ...
  for (int i = 0; i < kGrPixelConfigCnt; ++i) {
    ...
    writer->appendHexU32(
        "i_for_renderbuffer", fConfigTable[i].fFormats.fInternalFormatRenderbuffer);
    writer->endObject();
  }
...
void GrGLCaps::initConfigTable(
    const GrContextOptions& contextOptions, const GrGLContextInfo& ctxInfo,
    const GrGLInterface* gli, GrShaderCaps* shaderCaps) {
  /*
  ...
  */
  ...
  if (!contextOptions.fDisableDriverCorrectnessWorkarounds) {
    ...
    disableBGRATextureStorageForIntelWindowsES =
        kIntel_GrGLDriver == ctxInfo.driver() && GR_IS_GR_GL_ES(standard);
#endif
  ...
  uint32_t nonMSAARenderFlags = ConfigInfo::kRenderable_Flag | ConfigInfo::kFBOColorAttachment_Flag;
  ...
  if (GR_IS_GR_GL(standard)) {
    ...
    if (version >= GR_GL_VER(1, 2) || ctxInfo.hasExtension("GL_EXT_bgra")) {
      ...
      fConfigTable[kBGRA_8888_GrPixelConfig].fFlags =
          ConfigInfo::kTextureable_Flag | allRenderFlags;
    }
    ...
    if (ctxInfo.hasExtension("GL_EXT_texture_format_BGRA8888")) {
      fConfigTable[kBGRA_8888_GrPixelConfig].fFlags =
          ConfigInfo::kTextureable_Flag | nonMSAARenderFlags;
  ...
  if (GR_IS_GR_GL(standard)) {
    ...
    fSRGBWriteControl =
        !disableSRGBWriteControlForAdreno4xx && ctxInfo.hasExtension("GL_EXT_sRGB_write_control");
  } else if (GR_IS_GR_WEBGL(standard)) {
  ...
  if (fSRGBSupport) {
    fConfigTable[kSRGBA_8888_GrPixelConfig].fFlags =
        ConfigInfo::kTextureable_Flag | srgbRenderFlags;
  }
  ...
  fConfigTable[kSBGRA_8888_GrPixelConfig]
      .fFormats.fExternalFormat[kReadPixels_ExternalFormatUsage] = GR_GL_BGRA;
  ...
  if (fSRGBSupport && GR_IS_GR_GL(standard)) {
    fConfigTable[kSBGRA_8888_GrPixelConfig].fFlags =
        ConfigInfo::kTextureable_Flag | srgbRenderFlags;
  }
  ...
  if (GR_IS_GR_GL(standard) || (GR_IS_GR_GL_ES(standard) && version >= GR_GL_VER(3, 0))) {
    fConfigTable[kRGBA_1010102_GrPixelConfig].fFlags =
        ConfigInfo::kTextureable_Flag | allRenderFlags;
  } else if (
  ...
  if ((GR_IS_GR_GL(standard) && version <= GR_GL_VER(3, 0)) ||
      (GR_IS_GR_GL_ES(standard) && version < GR_GL_VER(3, 0)) || (GR_IS_GR_WEBGL(standard))) {
  ...
  fConfigTable[kRGB_ETC1_GrPixelConfig].fFormats.fSizedInternalFormat = GR_GL_COMPRESSED_RGB8_ETC2;
  ...
  if (GR_IS_GR_GL(standard)) {
    ...
    if (version >= GR_GL_VER(3, 0) || ctxInfo.hasExtension("GL_OES_compressed_ETC2_RGB8_texture")) {
      fConfigTable[kRGB_ETC1_GrPixelConfig].fFlags = ConfigInfo::kTextureable_Flag;
  ...
  for (int i = 0; i < kGrPixelConfigCnt; ++i) {
    ...
    fConfigTable[i].fFormats.fInternalFormatTexImage =
        useSizedTexFormats ? fConfigTable[i].fFormats.fSizedInternalFormat
                           : fConfigTable[i].fFormats.fBaseInternalFormat;
    fConfigTable[i].fFormats.fInternalFormatRenderbuffer =
        useSizedRbFormats ? fConfigTable[i].fFormats.fSizedInternalFormat
                          : fConfigTable[i].fFormats.fBaseInternalFormat;
  }
  ...
  if (useSizedTexFormats && GR_IS_GR_GL_ES(standard) && !textureRedSupport) {
    ...
    SkASSERT(
        fConfigTable[kAlpha_8_as_Alpha_GrPixelConfig].fFormats.fBaseInternalFormat == GR_GL_ALPHA8);
  ...
  if (GR_IS_GR_GL_ES(standard) && version == GR_GL_VER(2, 0)) {
    fConfigTable[kSRGBA_8888_GrPixelConfig]
        .fFormats.fExternalFormat[kTexImage_ExternalFormatUsage] = GR_GL_SRGB_ALPHA;
  ...
  if (useSizedTexFormats && this->bgraIsInternalFormat()) {
    if (ctxInfo.hasExtension("GL_APPLE_texture_format_BGRA8888")) {
      ...
      fConfigTable[kBGRA_8888_GrPixelConfig].fFormats.fInternalFormatRenderbuffer = GR_GL_RGBA8;
    } else {
    ...
    if (ConfigInfo::kRenderableWithMSAA_Flag & fConfigTable[i].fFlags) {
      ...
      if ((GR_IS_GR_GL(standard) &&
        ...
        GR_GL_GetInternalformativ(
            gli, GR_GL_RENDERBUFFER, format, GR_GL_NUM_SAMPLE_COUNTS, 1, &count);
        if (count) {
          int* temp = new int[count];
          GR_GL_GetInternalformativ(gli, GR_GL_RENDERBUFFER, format, GR_GL_SAMPLES, count, temp);
  ...
  for (int i = 0; i < kGrPixelConfigCnt; ++i) {
    ...
    SkASSERT(
        !((fConfigTable[i].fFlags & ConfigInfo::kRenderableWithMSAA_Flag) &&
          !(fConfigTable[i].fFlags & ConfigInfo::kRenderable_Flag)));

    SkASSERT(
        defaultEntry.fFormats.fBaseInternalFormat != fConfigTable[i].fFormats.fBaseInternalFormat);
    SkASSERT(
        defaultEntry.fFormats.fSizedInternalFormat !=
...
bool GrGLCaps::canCopyTexSubImage(
  ...
  // is required
  if (this->canConfigBeFBOColorAttachment(srcConfig) && (!srcIsTextureable || srcIsGLTexture2D) &&
      dstIsGLTexture2D && dstOrigin == srcOrigin) {
    return true;
  } else {
    return false;
  }
...
static bool has_msaa_render_buffer(const GrSurfaceProxy* surf, const GrGLCaps& glCaps) {
  ...
  return rt->numColorSamples() > 1 && glCaps.usesMSAARenderBuffers() &&
...
bool GrGLCaps::onCanCopySurface(
  ...
  if (this->canCopyAsBlit(
          dstConfig, dstSampleCnt, SkToBool(dstTex), dstIsTex2D, dstOrigin, srcConfig, srcSampleCnt,
          SkToBool(srcTex), srcIsTex2D, srcOrigin, src->getBoundsRect(), srcRect, dstPoint) &&
      !src->priv().isExact()) {
  ...
  return this->canCopyTexSubImage(
             dstConfig, has_msaa_render_buffer(dst, *this), SkToBool(dstTex), dstIsTex2D, dstOrigin,
             srcConfig, has_msaa_render_buffer(src, *this), SkToBool(srcTex), srcIsTex2D,
             srcOrigin) ||
         this->canCopyAsBlit(
             dstConfig, dstSampleCnt, SkToBool(dstTex), dstIsTex2D, dstOrigin, srcConfig,
             srcSampleCnt, SkToBool(srcTex), srcIsTex2D, srcOrigin, src->getBoundsRect(), srcRect,
             dstPoint) ||
         this->canCopyAsDraw(dstConfig, SkToBool(srcTex));
}

bool GrGLCaps::initDescForDstCopy(
    const GrRenderTargetProxy* src, GrSurfaceDesc* desc, GrSurfaceOrigin* origin,
  ...
  *disallowSubrect = false;
  ...
  {
    bool srcIsMSAARenderbuffer =
        GrFSAAType::kUnifiedMSAA == src->fsaaType() && this->usesMSAARenderBuffers();
...
#  endif
  ...
  if (fMultisampleDisableSupport && this->shaderCaps()->dualSourceBlendingSupport() &&
      this->shaderCaps()->pathRenderingSupport() && fUsesMixedSamples &&
#if GR_TEST_UTILS
      (contextOptions.fGpuPathRenderers & GpuPathRenderers::kStencilAndCover) &&
#endif
...
GrColorType GrGLCaps::supportedReadPixelsColorType(
  ...
  switch (fConfigTable[config].fFormatType) {
    ...
    case kFloat_FormatType:
      if ((kAlpha_half_GrPixelConfig == config || kAlpha_half_as_Red_GrPixelConfig == config) &&
...
GrPixelConfig validate_sized_format(GrGLenum format, SkColorType ct, GrGLStandard standard) {
  switch (ct) {
    ...
    case kRGBA_1010102_SkColorType:
      ...
      break;
    case kRGB_101010x_SkColorType: break;
...
GrPixelConfig GrGLCaps::validateBackendRenderTarget(
    const GrBackendRenderTarget& rt, SkColorType ct) const {
  GrGLFramebufferInfo fbInfo;
...
GrPixelConfig GrGLCaps::getConfigFromBackendFormat(
    const GrBackendFormat& format, SkColorType ct) const {
...
GrBackendFormat GrGLCaps::getBackendFormatFromGrColorType(
    GrColorType ct, GrSRGBEncoded srgbEncoded) const {
...
static bool format_color_type_valid_pair(GrGLenum format, GrColorType colorType) {
  switch (colorType) {
    ...
    case GrColorType::kRGB_565: return GR_GL_RGB565 == format;
    case GrColorType::kABGR_4444: return GR_GL_RGBA4 == format;
    case GrColorType::kRGBA_8888: return GR_GL_RGBA8 == format || GR_GL_SRGB8_ALPHA8 == format;
    case GrColorType::kRGB_888x: return GR_GL_RGB8 == format || GR_GL_RGBA8 == format;
