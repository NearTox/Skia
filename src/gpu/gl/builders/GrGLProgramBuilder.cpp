...
GrGLProgram* GrGLProgramBuilder::CreateProgram(
  ...
  GrGLProgramBuilder builder(gpu, renderTarget, origin, pipeline, primProc, primProcProxies, desc);
...
GrGLProgramBuilder::GrGLProgramBuilder(
    GrGLGpu* gpu, GrRenderTarget* renderTarget, GrSurfaceOrigin origin, const GrPipeline& pipeline,
...
bool GrGLProgramBuilder::compileAndAttachShaders(
    ...
    const SkSL::Program::Inputs& inputs, GrContextOptions::ShaderErrorHandler* errHandler) {
...
void GrGLProgramBuilder::storeShaderInCache(
  ...
  if (fGpu->glCaps().programBinarySupport()) {
    ...
    auto data = GrPersistentCacheUtils::PackCachedShaders(
        isSkSL ? kSKSL_Tag : kGLSL_Tag, shaders, &inputs, 1);
    this->gpu()->getContext()->priv().getPersistentCache()->store(*key, *data);
  }
}
...
GrGLProgram* GrGLProgramBuilder::finalize() {
  ...
  if (cached) {
    if (fGpu->glCaps().programBinarySupport()) {
      ...
      if (kSKSL_Tag ==
          GrPersistentCacheUtils::UnpackCachedShaders(fCached.get(), cached_sksl, &inputs, 1)) {
      ...
      if (kGLSL_Tag !=
          GrPersistentCacheUtils::UnpackCachedShaders(fCached.get(), glsl, &inputs, 1)) {
        for (int i = 0; i < kGrShaderTypeCount; ++i) {
          glsl[i].clear();
        }
      }
  ...
  if (!cached || !fGpu->glCaps().programBinarySupport()) {
    ...
    if (!this->compileAndAttachShaders(
            glsl[kVertex_GrShaderType], programID, GR_GL_VERTEX_SHADER, &shadersToDelete, inputs,
            errorHandler)) {
      this->cleanupProgram(programID, shadersToDelete);
      return nullptr;
    }
...
void GrGLProgramBuilder::resolveProgramResourceLocations(GrGLuint programID) {
  ...
  for (int i = 0; i < count; ++i) {
    GrGLint location;
    GL_CALL_RET(
        location, GetProgramResourceLocation(
