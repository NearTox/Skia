...
const char* get_error_string(uint32_t err) {
  switch (err) {
    case GR_GL_NO_ERROR: return "";
    case GR_GL_INVALID_ENUM: return "Invalid Enum";
    case GR_GL_INVALID_VALUE: return "Invalid Value";
    case GR_GL_INVALID_OPERATION: return "Invalid Operation";
    case GR_GL_OUT_OF_MEMORY: return "Out of Memory";
    case GR_GL_CONTEXT_LOST: return "Context Lost";
  }
  return "Unknown";
}
}  // namespace
...
void GrGLGetDriverInfo(
    GrGLStandard standard, GrGLVendor vendor, const char* rendererString, const char* versionString,
    GrGLDriver* outDriver, GrGLDriverVersion* outVersion) {
  ...
  if (GR_IS_GR_GL(standard)) {
    ...
    if (0 == strncmp("ANGLE", rendererString, 5)) {
      *outDriver = kANGLE_GrGLDriver;
      n = sscanf(
          versionString, "OpenGL ES %d.%d (ANGLE %d.%d", &major, &minor, &driverMajor,
          &driverMinor);
      if (4 == n) {
        *outVersion = GR_GL_DRIVER_VER(driverMajor, driverMinor, 0);
      }
...
GrGLRenderer GrGLGetRendererFromStrings(
    const char* rendererString, const GrGLExtensions& extensions) {
  if (rendererString) {
    ...
    if (const char* amdString = strstr(rendererString, "Radeon")) {
      ...
      n = sscanf(amdString, "Radeon (TM) R9 M%c%c%c", &amdGeneration, &amdTier, &amdRevision);
      if (3 == n) {
        if ('4' == amdGeneration) {
...
void GrGLGetANGLEInfoFromString(
    const char* rendererString, GrGLANGLEBackend* backend, GrGLANGLEVendor* vendor,
    GrGLANGLERenderer* renderer) {
  ...
  if (strstr(rendererString, "Intel")) {
    *vendor = GrGLANGLEVendor::kIntel;
    ...
    if ((modelStr = strstr(rendererString, "HD Graphics")) &&
      ...
      switch (modelNumber) {
        case 2000:
        case 3000: *renderer = GrGLANGLERenderer::kSandyBridge; break;
        case 4000:
        case 2500: *renderer = GrGLANGLERenderer::kIvyBridge; break;
        case 510:
        case 515:
        case 520:
        case 530: *renderer = GrGLANGLERenderer::kSkylake; break;
      }
    } else if (
        (modelStr = strstr(rendererString, "Iris")) &&
      ...
      switch (modelNumber) {
        ...
        case 550:
        case 555:
        case 580: *renderer = GrGLANGLERenderer::kSkylake; break;
      }
    }
  }
...
bool GrGLFormatIsCompressed(GrGLFormat format) {
  switch (format) {
    ...
    case GrGLFormat::kCOMPRESSED_ETC1_RGB8: return true;
...
size_t GrGLBytesPerFormat(GrGLFormat glFormat) {
  switch (glFormat) {
    ...
    case GrGLFormat::kRGB8: return 3;
    ...
    case GrGLFormat::kRGBA32F: return 16;
