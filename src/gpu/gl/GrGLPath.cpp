...
inline bool init_path_object_for_general_path(GrGLGpu* gpu, GrGLuint pathID, const SkPath& skPath) {
    ...
    GR_GL_CALL(gpu->glInterface(),
               PathCommands(pathID, pathCommands.count(), pathCommands.begin(), pathCoords.count(),
                            GR_GL_FLOAT, pathCoords.begin()));
    return true;
}
...
void GrGLPath::InitPathObjectPathData(GrGLGpu* gpu, GrGLuint pathID, const SkPath& skPath) {
    ...
    if ((skPath.getSegmentMasks() & SkPath::kConic_SegmentMask) == 0) {
        ...
        GR_GL_CALL(gpu->glInterface(),
                   PathCommands(pathID, pathCommands.count(), &pathCommands[0], pathCoords.count(),
