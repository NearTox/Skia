...
inline bool init_path_object_for_general_path(GrGLGpu* gpu, GrGLuint pathID, const SkPath& skPath) {
  ...
  while ((verb = iter.next(points)) != SkPath::kDone_Verb) {
    ...
    switch (verb) {
      ...
      case SkPath::kCubic_Verb:
        if (checkForDegenerates) {
          if (SkPath::IsCubicDegenerate(points[0], points[1], points[2], points[3], true)) {
            return false;
          }
          lastVerbWasMove = false;
        }
  ...
  GR_GL_CALL(
      gpu->glInterface(), PathCommands(
                              pathID, pathCommands.count(), pathCommands.begin(),
...
static GrPathRendering::FillType convert_skpath_filltype(SkPath::FillType fill) {
  switch (fill) {
    ...
    case SkPath::kInverseWinding_FillType: return GrPathRendering::kWinding_FillType;
    case SkPath::kEvenOdd_FillType:
    case SkPath::kInverseEvenOdd_FillType: return GrPathRendering::kEvenOdd_FillType;
  }
}

}  // namespace

bool GrGLPath::InitPathObjectPathDataCheckingDegenerates(
    GrGLGpu* gpu, GrGLuint pathID, const SkPath& skPath) {
  return init_path_object_for_general_path<true>(gpu, pathID, skPath);
}

void GrGLPath::InitPathObjectPathData(GrGLGpu* gpu, GrGLuint pathID, const SkPath& skPath) {
  ...
  if ((skPath.getSegmentMasks() & SkPath::kConic_SegmentMask) == 0) {
    ...
    GR_GL_CALL(
        gpu->glInterface(), PathCommands(
                                pathID, pathCommands.count(), &pathCommands[0], pathCoords.count(),
                                GR_GL_FLOAT, &pathCoords[0]));
    return;
  }
#endif
...
GrGLPath::GrGLPath(GrGLGpu* gpu, const SkPath& origSkPath, const GrStyle& style)
    : INHERITED(gpu, origSkPath, style), fPathID(gpu->glPathRendering()->genPaths(1)) {
