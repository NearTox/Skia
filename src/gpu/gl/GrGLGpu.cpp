...
static bool check_backend_texture(
    const GrBackendTexture& backendTex, const GrColorType colorType, const GrGLCaps& caps,
    GrGLTexture::Desc* desc, bool skipRectTexSupportCheck = false) {
...
sk_sp<GrTexture> GrGLGpu::onWrapBackendTexture(
  ...
  auto texture = GrGLTexture::MakeWrapped(
      this, mipMapsStatus, desc, backendTex.getGLTextureParams(), cacheable, ioType);
...
sk_sp<GrTexture> GrGLGpu::onWrapRenderableBackendTexture(
  ...
  sk_sp<GrGLTextureRenderTarget> texRT(GrGLTextureRenderTarget::MakeWrapped(
      this, sampleCnt, desc, backendTex.getGLTextureParams(), rtIDs, cacheable, mipMapsStatus));
...
sk_sp<GrRenderTarget> GrGLGpu::onWrapBackendRenderTarget(
  ...
  return GrGLRenderTarget::MakeWrapped(
      this, size, format, config, sampleCount, rtIDs, backendRT.stencilBits());
}
...
sk_sp<GrRenderTarget> GrGLGpu::onWrapBackendTextureAsRenderTarget(
  ...
  return GrGLRenderTarget::MakeWrapped(
      this, desc.fSize, desc.fFormat, desc.fConfig, sampleCount, rtIDs, 0);
}
...
bool GrGLGpu::onWritePixels(
  ...
  return this->uploadTexData(
      glTex->format(), surfaceColorType, glTex->width(), glTex->height(), glTex->target(),
      kWrite_UploadType, left, top, width, height, srcColorType, texels, mipLevelCount);
}

bool GrGLGpu::onTransferPixelsTo(
    GrTexture* texture, int left, int top, int width, int height, GrColorType textureColorType,
    GrColorType bufferColorType, GrGpuBuffer* transferBuffer, size_t offset, size_t rowBytes) {
  ...
  this->glCaps().getTexImageFormats(
      textureFormat, textureColorType, bufferColorType, &internalFormat, &externalFormat,
      &externalType);
...
bool GrGLGpu::onTransferPixelsFrom(
    GrSurface* surface, int left, int top, int width, int height, GrColorType surfaceColorType,
    GrColorType dstColorType, GrGpuBuffer* transferBuffer, size_t offset) {
  ...
  return this->readOrTransferPixelsFrom(
      surface, left, top, width, height, surfaceColorType, dstColorType, offsetAsPtr, width);
}
...
static bool allocate_and_populate_texture(
    GrGLFormat format, const GrGLInterface& interface, const GrGLCaps& caps, GrGLenum target,
    GrGLenum internalFormat, GrGLenum internalFormatForTexStorage, GrGLenum externalFormat,
    GrGLenum externalType, size_t dataBpp, const GrMipLevel texels[], int mipLevelCount,
    int baseWidth, int baseHeight, bool* changedUnpackRowLength, GrMipMapsStatus* mipMapsStatus) {
        ...
        if (currentMipData) {
          GR_GL_CALL(
              &interface, TexSubImage2D(
                              target, currentMipLevel, 0, 0, currentWidth, currentHeight,
...
bool GrGLGpu::uploadTexData(
    GrGLFormat textureFormat, GrColorType textureColorType, int texWidth, int texHeight,
    GrGLenum target, UploadType uploadType, int left, int top, int width, int height,
    GrColorType srcColorType, const GrMipLevel texels[], int mipLevelCount,
    GrMipMapsStatus* mipMapsStatus) {
  ...
  this->glCaps().getTexImageFormats(
      textureFormat, textureColorType, srcColorType, &internalFormat, &externalFormat,
      &externalType);
  if (!externalFormat || !externalType) {
    return false;
  }
  ...
  if (mipMapsStatus) {
    *mipMapsStatus = (mipLevelCount > 1) ? GrMipMapsStatus::kValid : GrMipMapsStatus::kNotAllocated;
  }
  ...
  if (kNewTexture_UploadType == uploadType) {
    if (0 == left && 0 == top && texWidth == width && texHeight == height) {
      succeeded = allocate_and_populate_texture(
          textureFormat, *interface, caps, target, internalFormat, internalFormatForTexStorage,
          externalFormat, externalType, bpp, texels, mipLevelCount, width, height,
          &restoreGLRowLength, mipMapsStatus);
    } else {
      succeeded = false;
    }
  } else {
...
bool GrGLGpu::uploadCompressedTexData(
    GrGLFormat format, SkImage::CompressionType compressionType, const SkISize& size,
  ...
  if (useTexStorage) {
    ...
    GL_CALL(CompressedTexSubImage2D(
        ...
        0,  // left
        0,  // top
        size.width(), size.height(), internalFormat, SkToInt(dataSize), data));
  } else {
    GL_ALLOC_CALL(
...
sk_sp<GrTexture> GrGLGpu::onCreateTexture(
  ...
  texDesc.fID = this->createTexture2D(
      {desc.fWidth, desc.fHeight}, texDesc.fFormat, renderable, &initialState, textureColorType,
...
sk_sp<GrTexture> GrGLGpu::onCreateCompressedTexture(
    int width, int height, const GrBackendFormat& format, SkImage::CompressionType compression,
  ...
  desc.fID =
      this->createCompressedTexture2D(desc.fSize, desc.fFormat, compression, &initialState, data);
  if (!desc.fID) {
    return nullptr;
  }
...
GrGLuint GrGLGpu::createCompressedTexture2D(
    const SkISize& size, GrGLFormat format, SkImage::CompressionType compression,
...
GrGLuint GrGLGpu::createTexture2D(
    const SkISize& size, GrGLFormat format, GrRenderable renderable,
    GrGLTextureParameters::SamplerOverriddenState* initialState, GrColorType textureColorType,
    GrColorType srcColorType, const GrMipLevel texels[], int mipLevelCount,
    GrMipMapsStatus* mipMapsStatus) {
  SkASSERT(format != GrGLFormat::kUnknown);
  ...
  *initialState = set_initial_texture_params(this->glInterface(), GR_GL_TEXTURE_2D);
  ...
  if (!this->uploadTexData(
          format, textureColorType, size.width(), size.height(), GR_GL_TEXTURE_2D,
          kNewTexture_UploadType, 0, 0, size.width(), size.height(), srcColorType, texels,
...
bool GrGLGpu::readOrTransferPixelsFrom(
    GrSurface* surface, int left, int top, int width, int height, GrColorType surfaceColorType,
    GrColorType dstColorType, void* offsetOrPtr, int rowWidthInPixels) {
  SkASSERT(surface);
  ...
  GL_CALL(ReadPixels(
      readRect.fX, readRect.fY, readRect.fWidth, readRect.fHeight, externalFormat, externalType,
      offsetOrPtr));
...
bool GrGLGpu::onReadPixels(
  ...
  return this->readOrTransferPixelsFrom(
      surface, left, top, width, height, surfaceColorType, dstColorType, buffer, rowPixelWidth);
}
    ...
    if (this->glCaps().usesMSAARenderBuffers()) {
      ...
      if (GrGLCaps::kES_Apple_MSFBOType == this->glCaps().msFBOType()) {
        ...
        if (GrGLCaps::kResolveMustBeFull_BlitFrambufferFlag &
          ...
          auto rect = GrNativeRect::MakeRelativeTo(kDirtyRectOrigin, rt->height(), dirtyRect);
          l = rect.fX;
          b = rect.fY;
...
GrBackendTexture GrGLGpu::createBackendTexture(
  ...
  if (GrGLFormatToCompressionType(glFormat, &compressionType)) {
    ...
    // TODO: Take these as parameters.
    auto srcColorType = GrPixelConfigToColorType(desc.fConfig);
    info.fID = this->createTexture2D(
        {desc.fWidth, desc.fHeight}, glFormat, renderable, &initialState, textureColorType,
        srcColorType, texels, mipLevelCount, nullptr);
    if (!info.fID) {
      return GrBackendTexture();  // invalid
    }
  }
...
GrBackendRenderTarget GrGLGpu::createTestingOnlyBackendRenderTarget(
  ...
  if (format == GrGLFormat::kBGRA8) {
    ...
    this->glCaps().getTexImageFormats(
        format, colorType, colorType, &colorBufferFormat, &externalFormat, &externalType);
    useTexture = true;
  } else {
