...
static int gl_target_to_binding_index(GrGLenum target) {
  switch (target) {
    ...
    case GR_GL_TEXTURE_EXTERNAL: return 2;
  }
 ...
 public:
  ...
  void bindSampler(int unitIdx, const GrSamplerState& state) {
    ...
    if (!fSamplers[index]) {
      ...
      GR_GL_CALL(fGpu->glInterface(), SamplerParameteri(s, GR_GL_TEXTURE_MIN_FILTER, minFilter));
      GR_GL_CALL(fGpu->glInterface(), SamplerParameteri(s, GR_GL_TEXTURE_MAG_FILTER, magFilter));
...
sk_sp<GrGpu> GrGLGpu::Make(
    sk_sp<const GrGLInterface> interface, const GrContextOptions& options, GrContext* context) {
  if (!interface) {
...
void GrGLGpu::onResetContext(uint32_t resetBits) {
  if (resetBits & kMisc_GrGLBackendState) {
    ...
    if (GR_IS_GR_GL_ES(this->glStandard()) && this->glCaps().fbFetchRequiresEnablePerSample()) {
...
static bool check_backend_texture(
    const GrBackendTexture& backendTex, const GrGLCaps& caps, GrGLTexture::IDDesc* idDesc) {
...
sk_sp<GrTexture> GrGLGpu::onWrapBackendTexture(
  ...
  GrMipMapsStatus mipMapsStatus =
      backendTex.hasMipMaps() ? GrMipMapsStatus::kValid : GrMipMapsStatus::kNotAllocated;

  auto texture = GrGLTexture::MakeWrapped(this, surfDesc, mipMapsStatus, idDesc, cacheable, ioType);
  // We don't know what parameters are already set on wrapped textures.
  texture->textureParamsModified();
  return std::move(texture);
}

sk_sp<GrTexture> GrGLGpu::onWrapRenderableBackendTexture(
  ...
  GrMipMapsStatus mipMapsStatus =
      backendTex.hasMipMaps() ? GrMipMapsStatus::kDirty : GrMipMapsStatus::kNotAllocated;
...
sk_sp<GrRenderTarget> GrGLGpu::onWrapBackendTextureAsRenderTarget(
  ...
  if (GR_GL_TEXTURE_RECTANGLE != info.fTarget && GR_GL_TEXTURE_2D != info.fTarget) {
...
bool GrGLGpu::onWritePixels(
    GrSurface* surface, int left, int top, int width, int height, GrColorType srcColorType,
  ...
  return this->uploadTexData(
      glTex->config(), glTex->width(), glTex->height(), glTex->target(), kWrite_UploadType, left,
      top, width, height, srcAsConfig, texels, mipLevelCount);
}
...
static inline GrGLint config_alignment(GrPixelConfig config) {
  ...
  switch (config) {
    ...
    case kGray_8_as_Red_GrPixelConfig: return 1;
    ...
    case kRGBA_half_Clamped_GrPixelConfig: return 2;
    ...
    case kRG_float_GrPixelConfig: return 4;
...
bool GrGLGpu::onTransferPixelsTo(
    GrTexture* texture, int left, int top, int width, int height, GrColorType bufferColorType,
  ...
  if (!this->glCaps().getTexImageFormats(
          texConfig, bufferAsConfig, &internalFormat, &externalFormat, &externalType)) {
    return false;
  }

  GL_CALL(PixelStorei(GR_GL_UNPACK_ALIGNMENT, config_alignment(texConfig)));
  GL_CALL(TexSubImage2D(
      glTex->target(), 0, left, top, width, height, externalFormat, externalType, pixels));
...
bool GrGLGpu::onTransferPixelsFrom(
  ...
  return this->readOrTransferPixelsFrom(
      surface, left, top, width, height, dstColorType, offsetAsPtr, width);
}
 ...
 */
static bool allocate_and_populate_texture(
    GrPixelConfig config, const GrGLInterface& interface, const GrGLCaps& caps, GrGLenum target,
    GrGLenum internalFormat, GrGLenum internalFormatForTexStorage, GrGLenum externalFormat,
    GrGLenum externalType, const GrMipLevel texels[], int mipLevelCount, int baseWidth,
    int baseHeight) {
  ...
  if (caps.isConfigTexSupportEnabled(config)) {
    ...
    GL_ALLOC_CALL(
        &interface,
        TexStorage2D(
            target, SkTMax(mipLevelCount, 1), internalFormatForTexStorage, baseWidth, baseHeight));
      ...
      for (int currentMipLevel = 0; currentMipLevel < mipLevelCount; currentMipLevel++) {
        ...
        GR_GL_CALL(
            ...
            TexSubImage2D(
                ...
                0,  // left
                0,  // top
                currentWidth, currentHeight, externalFormat, externalType, currentMipData));
      }
      return true;
    }
  } else {
    if (!mipLevelCount) {
      GL_ALLOC_CALL(
          &interface, TexImage2D(
                          target, 0, internalFormat, baseWidth, baseHeight,
                          0,  // border
      ...
      for (int currentMipLevel = 0; currentMipLevel < mipLevelCount; currentMipLevel++) {
        ...
        GL_ALLOC_CALL(
            &interface, TexImage2D(
                            target, currentMipLevel, internalFormat, currentWidth, currentHeight,
                            0,  // border
                            externalFormat, externalType, currentMipData));
        GrGLenum error = CHECK_ALLOC_ERROR(&interface);
        if (error != GR_GL_NO_ERROR) {
          return false;
        }
 ...
 * @param baseHeight     The height of the texture's base mipmap level
 */
static bool allocate_and_populate_compressed_texture(
    GrPixelConfig config, const GrGLInterface& interface, const GrGLCaps& caps, GrGLenum target,
    GrGLenum internalFormat, const GrMipLevel texels[], int mipLevelCount, int baseWidth,
    int baseHeight) {
  CLEAR_ERROR_BEFORE_ALLOC(&interface);
  SkASSERT(GrPixelConfigIsCompressed(config));
  ...
  if (useTexStorage) {
    // We never resize or change formats of textures.
    GL_ALLOC_CALL(
        &interface, TexStorage2D(target, mipLevelCount, internalFormat, baseWidth, baseHeight));
    GrGLenum error = CHECK_ALLOC_ERROR(&interface);
    if (error != GR_GL_NO_ERROR) {
      return false;
    } else {
    ...
    for (int currentMipLevel = 0; currentMipLevel < mipLevelCount; currentMipLevel++) {
      ...
      GL_ALLOC_CALL(
          &interface, CompressedTexImage2D(
                          target, currentMipLevel, internalFormat, currentWidth, currentHeight,
                          0,  // border
                          SkToInt(dataSize), currentMipData));
 ...
 */
static void restore_pixelstore_state(
    const GrGLInterface& interface, const GrGLCaps& caps, bool restoreGLRowLength) {
  if (restoreGLRowLength) {
...
bool GrGLGpu::uploadTexData(
    GrPixelConfig texConfig, int texWidth, int texHeight, GrGLenum target, UploadType uploadType,
  ...
  GrGLenum externalType;
  if (!this->glCaps().getTexImageFormats(
          texConfig, dataConfig, &internalFormat, &externalFormat, &externalType)) {
    return false;
  }
  ...
  for (int currentMipLevel = 0; currentMipLevel < mipLevelCount; currentMipLevel++) {
    ...
    if (caps.unpackRowLengthSupport() && !usesMips) {
      ...
      texelsShallowCopy[currentMipLevel].fPixels = buffer + individualMipOffsets[currentMipLevel];
  ...
  if (kNewTexture_UploadType == uploadType) {
    if (0 == left && 0 == top && texWidth == width && texHeight == height) {
      succeeded = allocate_and_populate_texture(
          texConfig, *interface, caps, target, internalFormat, internalFormatForTexStorage,
          externalFormat, externalType, texelsShallowCopy, mipLevelCount, width, height);
    } else {
      succeeded = false;
    }
  } else {
    for (int currentMipLevel = 0; currentMipLevel < mipLevelCount; currentMipLevel++) {
      ...
      GL_CALL(TexSubImage2D(
          target, currentMipLevel, left, top, currentWidth, currentHeight, externalFormat,
          externalType, texelsShallowCopy[currentMipLevel].fPixels));
    }
  }
...
bool GrGLGpu::uploadCompressedTexData(
    GrPixelConfig texConfig, int texWidth, int texHeight, GrGLenum target,
    const GrMipLevel texels[], int mipLevelCount, GrMipMapsStatus* mipMapsStatus) {
  SkASSERT(this->caps()->isConfigTexturable(texConfig));
  ...
  return allocate_and_populate_compressed_texture(
      texConfig, *interface, caps, target, internalFormat, texels, mipLevelCount, texWidth,
      texHeight);
}

static bool renderbuffer_storage_msaa(
    const GrGLContext& ctx, int sampleCount, GrGLenum format, int width, int height) {
  ...
  switch (ctx.caps()->msFBOType()) {
    ...
    case GrGLCaps::kMixedSamples_MSFBOType:
      GL_ALLOC_CALL(
          ctx.interface(),
          RenderbufferStorageMultisample(GR_GL_RENDERBUFFER, sampleCount, format, width, height));
      break;
    case GrGLCaps::kES_Apple_MSFBOType:
      GL_ALLOC_CALL(
          ctx.interface(), RenderbufferStorageMultisampleES2APPLE(
    ...
    case GrGLCaps::kES_IMG_MsToTexture_MSFBOType:
      GL_ALLOC_CALL(
          ctx.interface(), RenderbufferStorageMultisampleES2EXT(
                               GR_GL_RENDERBUFFER, sampleCount, format, width, height));
      break;
...
bool GrGLGpu::createRenderTargetObjects(
    const GrSurfaceDesc& desc, const GrGLTextureInfo& texInfo, GrGLRenderTarget::IDDesc* idDesc) {
  idDesc->fMSColorRenderbufferID = 0;
  idDesc->fRTFBOID = 0;
  idDesc->fRTFBOOwnership = GrBackendObjectOwnership::kOwned;
  idDesc->fTexFBOID = 0;
  SkASSERT(
      (GrGLCaps::kMixedSamples_MSFBOType == this->glCaps().msFBOType()) ==
  ...
  if (idDesc->fRTFBOID != idDesc->fTexFBOID) {
    ...
    if (!renderbuffer_storage_msaa(
            *fGLContext, desc.fSampleCnt, colorRenderbufferFormat, desc.fWidth, desc.fHeight)) {
      goto FAILED;
    }
    this->bindFramebuffer(GR_GL_FRAMEBUFFER, idDesc->fRTFBOID);
    GL_CALL(FramebufferRenderbuffer(
        GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_RENDERBUFFER,
  ...
  if (this->glCaps().usesImplicitMSAAResolve() && desc.fSampleCnt > 1) {
    GL_CALL(FramebufferTexture2DMultisample(
        GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, texInfo.fTarget, texInfo.fID, 0,
        desc.fSampleCnt));
  } else {
...
static GrGLTexture::SamplerParams set_initial_texture_params(
...
size_t GLBytesPerPixel(GrGLenum glFormat) {
  switch (glFormat) {
    ...
    case GR_GL_ALPHA8:
    case GR_GL_R8: return 1;

    case GR_GL_RGB565:
    case GR_GL_RGBA4:
    case GR_GL_RG8:
    case GR_GL_R16F: return 2;

    case GR_GL_RGB8: return 3;

    case GR_GL_RGBA8:
    case GR_GL_SRGB8_ALPHA8:
    case GR_GL_BGRA8:
    case GR_GL_RGB10_A2: return 4;

    case GR_GL_RGBA16F:
    case GR_GL_RG32F: return 8;

    case GR_GL_RGBA32F: return 16;

    case GR_GL_COMPRESSED_RGB8_ETC2: return 0;
  }
...
sk_sp<GrTexture> GrGLGpu::onCreateTexture(
    const GrSurfaceDesc& desc, SkBudgeted budgeted, const GrMipLevel texels[], int mipLevelCount) {
  ...
  if (!this->createTextureImpl(
          desc, &idDesc.fInfo, isRenderTarget ? GrRenderable::kYes : GrRenderable::kNo,
  ...
  if (isRenderTarget) {
    ...
    tex =
        sk_make_sp<GrGLTextureRenderTarget>(this, budgeted, desc, idDesc, rtIDDesc, mipMapsStatus);
  ...
  tex->setCachedParams(
      &initialTexParams, tex->getCachedNonSamplerParams(), this->getResetTimestamp());
#ifdef TRACE_TEXTURE_CREATION
  SkDebugf(
      "--- new texture [%d] size=(%d %d) config=%d\n", idDesc.fInfo.fID, desc.fWidth, desc.fHeight,
      desc.fConfig);
#endif
  if (tex && performClear) {
...
void inline get_stencil_rb_sizes(const GrGLInterface* gl, GrGLStencilAttachment::Format* format) {
  ...
  if (kUnknownBitCount == format->fStencilBits) {
    GR_GL_GetRenderbufferParameteriv(
        gl, GR_GL_RENDERBUFFER, GR_GL_RENDERBUFFER_STENCIL_SIZE, (GrGLint*)&format->fStencilBits);
    if (format->fPacked) {
      GR_GL_GetRenderbufferParameteriv(
...
int GrGLGpu::getCompatibleStencilIndex(GrPixelConfig config) {
  ...
  if (!this->glCaps().hasStencilFormatBeenDeterminedForConfig(config)) {
    ...
    GrGLenum externalType;
    if (!this->glCaps().getTexImageFormats(
            config, config, &internalFormat, &externalFormat, &externalType)) {
      return false;
    }
    this->unbindCpuToGpuXferBuffer();
    CLEAR_ERROR_BEFORE_ALLOC(this->glInterface());
    GL_ALLOC_CALL(
        this->glInterface(), TexImage2D(
                                 GR_GL_TEXTURE_2D, 0, internalFormat, kSize, kSize, 0,
                                 externalFormat, externalType, nullptr));
    ...
    fHWBoundRenderTargetUniqueID.makeInvalid();
    GL_CALL(FramebufferTexture2D(
        GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_TEXTURE_2D, colorID, 0));
    GrGLuint sbRBID = 0;
    ...
    if (sbRBID) {
      ...
      for (int i = 0; i < stencilFmtCnt && sbRBID; ++i) {
        ...
        GL_ALLOC_CALL(
            this->glInterface(),
            RenderbufferStorage(GR_GL_RENDERBUFFER, sFmt.fInternalFormat, kSize, kSize));
        if (GR_GL_NO_ERROR == CHECK_ALLOC_ERROR(this->glInterface())) {
          GL_CALL(FramebufferRenderbuffer(
              GR_GL_FRAMEBUFFER, GR_GL_STENCIL_ATTACHMENT, GR_GL_RENDERBUFFER, sbRBID));
          if (sFmt.fPacked) {
            GL_CALL(FramebufferRenderbuffer(
                GR_GL_FRAMEBUFFER, GR_GL_DEPTH_ATTACHMENT, GR_GL_RENDERBUFFER, sbRBID));
          } else {
            GL_CALL(FramebufferRenderbuffer(
                GR_GL_FRAMEBUFFER, GR_GL_DEPTH_ATTACHMENT, GR_GL_RENDERBUFFER, 0));
          }
          GrGLenum status;
          GL_CALL_RET(status, CheckFramebufferStatus(GR_GL_FRAMEBUFFER));
          if (status == GR_GL_FRAMEBUFFER_COMPLETE) {
          ...
          GL_CALL(FramebufferRenderbuffer(
              GR_GL_FRAMEBUFFER, GR_GL_STENCIL_ATTACHMENT, GR_GL_RENDERBUFFER, 0));
          if (sFmt.fPacked) {
            GL_CALL(FramebufferRenderbuffer(
                GR_GL_FRAMEBUFFER, GR_GL_DEPTH_ATTACHMENT, GR_GL_RENDERBUFFER, 0));
          }
...
bool GrGLGpu::createTextureImpl(
    const GrSurfaceDesc& desc, GrGLTextureInfo* info, GrRenderable renderable,
  ...
  if (GrRenderable::kYes == renderable && this->glCaps().textureUsageSupport()) {
    ...
    GL_CALL(TexParameteri(info->fTarget, GR_GL_TEXTURE_USAGE, GR_GL_FRAMEBUFFER_ATTACHMENT));
  }
  ...
  *initialTexParams = set_initial_texture_params(this->glInterface(), *info);
  ...
  if (GrPixelConfigIsCompressed(desc.fConfig)) {
    ...
    success = this->uploadCompressedTexData(
        desc.fConfig, desc.fWidth, desc.fHeight, info->fTarget, texels, mipLevelCount,
        mipMapsStatus);
  } else {
    success = this->uploadTexData(
        desc.fConfig, desc.fWidth, desc.fHeight, info->fTarget, kNewTexture_UploadType, 0, 0,
        desc.fWidth, desc.fHeight, desc.fConfig, texels, mipLevelCount, mipMapsStatus);
  }
  if (!success) {
...
GrStencilAttachment* GrGLGpu::createStencilAttachmentForRenderTarget(
    const GrRenderTarget* rt, int width, int height) {
  ...
  if (samples > 1) {
    SkAssertResult(
        renderbuffer_storage_msaa(*fGLContext, samples, sFmt.fInternalFormat, width, height));
  } else {
    GL_ALLOC_CALL(
        this->glInterface(),
...
sk_sp<GrGpuBuffer> GrGLGpu::onCreateBuffer(
    size_t size, GrGpuBufferType intendedType, GrAccessPattern accessPattern, const void* data) {
  return GrGLBuffer::Make(this, size, intendedType, accessPattern, data);
}

void GrGLGpu::flushScissor(
...
void GrGLGpu::flushWindowRectangles(
    const GrWindowRectsState& windowState, const GrGLRenderTarget* rt, GrSurfaceOrigin origin) {
#ifndef USE_NSIGHT
...
bool GrGLGpu::flushGLState(
    GrRenderTarget* renderTarget, GrSurfaceOrigin origin, const GrPrimitiveProcessor& primProc,
    const GrPipeline& pipeline, const GrPipeline::FixedDynamicState* fixedDynamicState,
    const GrPipeline::DynamicStateArrays* dynamicStateArrays, int dynamicStateArraysLength,
    bool willDrawPoints) {
  ...
  sk_sp<GrGLProgram> program(fProgramCache->refProgram(
      this, renderTarget, origin, primProc, primProcProxiesForMipRegen, pipeline, willDrawPoints));
  if (!program) {
  ...
  const GrSwizzle& swizzle =
      this->caps()->shaderCaps()->configOutputSwizzle(renderTarget->config());
  this->flushBlend(blendInfo, swizzle);

  fHWProgram->updateUniformsAndTextureBindings(
      renderTarget, origin, primProc, pipeline, primProcProxiesToBind);
  ...
  if (pipeline.isStencilEnabled()) {
    ...
    stencil.reset(
        *pipeline.getUserStencil(), pipeline.hasStencilClip(),
...
void GrGLGpu::setupGeometry(
    const GrBuffer* indexBuffer, const GrBuffer* vertexBuffer, int baseVertex,
    const GrBuffer* instanceBuffer, int baseInstance, GrPrimitiveRestart enablePrimitiveRestart) {
  SkASSERT((enablePrimitiveRestart == GrPrimitiveRestart::kNo) || indexBuffer);

  GrGLAttribArrayState* attribState;
  if (indexBuffer) {
    SkASSERT(
        indexBuffer->isCpuBuffer() || !static_cast<const GrGpuBuffer*>(indexBuffer)->isMapped());
  ...
  if (int vertexStride = fHWProgram->vertexStride()) {
    ...
    SkASSERT(
        vertexBuffer->isCpuBuffer() || !static_cast<const GrGpuBuffer*>(vertexBuffer)->isMapped());
    ...
    for (int i = 0; i < fHWProgram->numVertexAttributes(); ++i) {
      ...
      attribState->set(
          this, attrib.fLocation, vertexBuffer, attrib.fCPUType, attrib.fGPUType, vertexStride,
  ...
  if (int instanceStride = fHWProgram->instanceStride()) {
    ...
    for (int i = 0; i < fHWProgram->numInstanceAttributes(); ++i, ++attribIdx) {
      ...
      attribState->set(
          this, attrib.fLocation, instanceBuffer, attrib.fCPUType, attrib.fGPUType, instanceStride,
          bufferOffset + attrib.fOffset, kDivisor);
    }
  }
}
...
GrGLenum GrGLGpu::bindBuffer(GrGpuBufferType type, const GrBuffer* buffer) {
  ...
  if (buffer->isCpuBuffer()) {
    if (!bufferState->fBufferZeroKnownBound) {
      ...
      static_cast<const GrGpuBuffer*>(buffer)->uniqueID() != bufferState->fBoundBufferUniqueID) {
...
void GrGLGpu::clear(
    const GrFixedClip& clip, const SkPMColor4f& color, GrRenderTarget* target,
...
void GrGLGpu::clearStencilClip(
    const GrFixedClip& clip, bool insideStencilMask, GrRenderTarget* target,
    GrSurfaceOrigin origin) {
  SkASSERT(target);
  SkASSERT(!this->caps()->performStencilClearsAsDraws());
  this->handleDirtyContext();
...
bool GrGLGpu::readPixelsSupported(GrRenderTarget* target, GrPixelConfig readConfig) {
  ...
  return this->glCaps().readPixelsSupported(
      rtConfig, readConfig, getIntegerv, bindRenderTarget, unbindRenderTarget);
}
...
bool GrGLGpu::readOrTransferPixelsFrom(
    GrSurface* surface, int left, int top, int width, int height, GrColorType dstColorType,
  ...
  if (!this->glCaps().getReadPixelsFormat(
          surface->config(), dstAsConfig, &externalFormat, &externalType)) {
    return false;
  }
  ...
  if (renderTarget) {
    ...
    switch (renderTarget->getResolveType()) {
      ...
      case GrGLRenderTarget::kCanResolve_ResolveType:
        ...
        break;
      default: SK_ABORT("Unknown resolve type");
    }
  } else {
    // Use a temporary FBO.
  ...
  if (this->glCaps().detachStencilFromMSAABuffersBeforeReadPixels() && renderTarget &&
  ...
  GL_CALL(ReadPixels(
      readRect.fLeft, readRect.fBottom, readRect.fWidth, readRect.fHeight, externalFormat,
      externalType, offsetOrPtr));

  if (reattachStencil) {
    GrGLStencilAttachment* stencilAttachment = static_cast<GrGLStencilAttachment*>(
        renderTarget->renderTargetPriv().getStencilAttachment());
    GL_CALL(FramebufferRenderbuffer(
        GR_GL_FRAMEBUFFER, GR_GL_STENCIL_ATTACHMENT, GR_GL_RENDERBUFFER,
        stencilAttachment->renderbufferID()));
  }
...
bool GrGLGpu::onReadPixels(
  ...
  if (!this->readOrTransferPixelsFrom(
          surface, left, top, width, height, dstColorType, readDst, rowPixelWidth)) {
    return false;
  }
  ...
  if (readDst != buffer) {
    ...
    SkRectMemcpy(dst, rowBytes, src, rowPixelWidth * bytesPerPixel, width * bytesPerPixel, height);
  }
  return true;
}
...
void GrGLGpu::flushRenderTarget(
    GrGLRenderTarget* target, GrSurfaceOrigin origin, const SkIRect& bounds) {
...
void GrGLGpu::draw(
    GrRenderTarget* renderTarget, GrSurfaceOrigin origin, const GrPrimitiveProcessor& primProc,
    const GrPipeline& pipeline, const GrPipeline::FixedDynamicState* fixedDynamicState,
    const GrPipeline::DynamicStateArrays* dynamicStateArrays, const GrMesh meshes[],
    int meshCount) {
  this->handleDirtyContext();
  ...
  if (!this->flushGLState(
          renderTarget, origin, primProc, pipeline, fixedDynamicState, dynamicStateArrays,
          meshCount, hasPoints)) {
    return;
  }
  ...
  for (int m = 0; m < meshCount; ++m) {
    if (GrXferBarrierType barrierType =
            pipeline.xferBarrierType(renderTarget->asTexture(), *this->caps())) {
    ...
    if (dynamicScissor) {
      GrGLRenderTarget* glRT = static_cast<GrGLRenderTarget*>(renderTarget);
      this->flushScissor(
          GrScissorState(dynamicStateArrays->fScissorRects[m]), glRT->width(), glRT->height(),
          origin);
    }
    if (dynamicPrimProcTextures) {
      auto texProxyArray =
          dynamicStateArrays->fPrimitiveProcessorTextures + m * primProc.numTextureSamplers();
      fHWProgram->updatePrimitiveProcessorTextureBindings(primProc, texProxyArray);
    }
    if (this->glCaps().requiresCullFaceEnableDisableWhenDrawingLinesAfterNonLines() &&
        GrIsPrimTypeLines(meshes[m].primitiveType()) && !GrIsPrimTypeLines(fLastPrimitiveType)) {
...
static GrGLenum gr_primitive_type_to_gl_mode(GrPrimitiveType primitiveType) {
  switch (primitiveType) {
    ...
    case GrPrimitiveType::kPoints: return GR_GL_POINTS;
    case GrPrimitiveType::kLines: return GR_GL_LINES;
    case GrPrimitiveType::kLineStrip: return GR_GL_LINE_STRIP;
    case GrPrimitiveType::kLinesAdjacency: return GR_GL_LINES_ADJACENCY;
  }
  SK_ABORT("invalid GrPrimitiveType");
  return GR_GL_TRIANGLES;
}

void GrGLGpu::sendMeshToGpu(
    GrPrimitiveType primitiveType, const GrBuffer* vertexBuffer, int vertexCount, int baseVertex) {
...
void GrGLGpu::sendIndexedMeshToGpu(
  ...
  if (this->glCaps().drawRangeElementsSupport()) {
    GL_CALL(DrawRangeElements(
        glPrimType, minIndexValue, maxIndexValue, indexCount, GR_GL_UNSIGNED_SHORT, elementPtr));
  } else {
...
void GrGLGpu::sendInstancedMeshToGpu(
    GrPrimitiveType primitiveType, const GrBuffer* vertexBuffer, int vertexCount, int baseVertex,
  ...
  for (int i = 0; i < instanceCount; i += maxInstances) {
    this->setupGeometry(
        nullptr, vertexBuffer, 0, instanceBuffer, baseInstance + i, GrPrimitiveRestart::kNo);
    GL_CALL(DrawArraysInstanced(
        glPrimType, baseVertex, vertexCount, SkTMin(instanceCount - i, maxInstances)));
    fStats.incNumDraws();
  }
}

void GrGLGpu::sendIndexedInstancedMeshToGpu(
    GrPrimitiveType primitiveType, const GrBuffer* indexBuffer, int indexCount, int baseIndex,
    const GrBuffer* vertexBuffer, int baseVertex, const GrBuffer* instanceBuffer, int instanceCount,
    int baseInstance, GrPrimitiveRestart enablePrimitiveRestart) {
  const GrGLenum glPrimType = gr_primitive_type_to_gl_mode(primitiveType);
  const GrGLvoid* elementPtr = element_ptr(indexBuffer, baseIndex);
  int maxInstances = this->glCaps().maxInstancesPerDrawWithoutCrashing(instanceCount);
  for (int i = 0; i < instanceCount; i += maxInstances) {
    this->setupGeometry(
        indexBuffer, vertexBuffer, baseVertex, instanceBuffer, baseInstance + i,
        enablePrimitiveRestart);
    GL_CALL(DrawElementsInstanced(
        glPrimType, indexCount, GR_GL_UNSIGNED_SHORT, elementPtr,
...
void set_gl_stencil(const GrGLInterface* gl, const GrStencilSettings::Face& face, GrGLenum glFace) {
...
void GrGLGpu::flushStencil(const GrStencilSettings& stencilSettings, GrSurfaceOrigin origin) {
  if (stencilSettings.isDisabled()) {
    ...
    if (stencilSettings.isTwoSided()) {
      ...
      set_gl_stencil(this->glInterface(), stencilSettings.frontAndBack(), GR_GL_FRONT_AND_BACK);
    }
    fHWStencilSettings = stencilSettings;
...
void GrGLGpu::flushBlend(const GrXferProcessor::BlendInfo& blendInfo, const GrSwizzle& swizzle) {
  ...
  if (fHWBlendState.fSrcCoeff != srcCoeff || fHWBlendState.fDstCoeff != dstCoeff) {
    GL_CALL(BlendFunc(gXfermodeCoeff2Blend[srcCoeff], gXfermodeCoeff2Blend[dstCoeff]));
...
void GrGLGpu::bindTexture(int unitIdx, GrSamplerState samplerState, GrGLTexture* texture) {
  ...
  if (samplerState.filter() == GrSamplerState::Filter::kMipMap) {
    if (!this->caps()->mipMapSupport() || texture->texturePriv().mipMapped() == GrMipMapped::kNo) {
  ...
  if (this->glCaps().mipMapLevelAndLodControlSupport() &&
    ...
    if (newNonSamplerParams.fBaseMipMapLevel != oldNonSamplerParams.fBaseMipMapLevel) {
      this->setTextureUnit(unitIdx);
      GL_CALL(
          TexParameteri(target, GR_GL_TEXTURE_BASE_LEVEL, newNonSamplerParams.fBaseMipMapLevel));
    }
    if (newNonSamplerParams.fMaxMipMapLevel != oldNonSamplerParams.fMaxMipMapLevel) {
      this->setTextureUnit(unitIdx);
      GL_CALL(TexParameteri(target, GR_GL_TEXTURE_MAX_LEVEL, newNonSamplerParams.fMaxMipMapLevel));
    }
  }
...
void GrGLGpu::flushClearColor(GrGLfloat r, GrGLfloat g, GrGLfloat b, GrGLfloat a) {
  if (r != fHWClearColor[0] || g != fHWClearColor[1] || b != fHWClearColor[2] ||
      a != fHWClearColor[3]) {
...
static inline bool can_blit_framebuffer_for_copy_surface(
  ...
  return caps.canCopyAsBlit(
      dst->config(), dstSampleCnt, SkToBool(dstTex), dstIsGLTexture2D, dstOrigin, src->config(),
      srcSampleCnt, SkToBool(srcTex), srcIsGLTexture2D, srcOrigin, src->getBoundsRect(), srcRect,
      dstPoint);
}
...
static inline bool can_copy_texsubimage(
    const GrSurface* dst, GrSurfaceOrigin dstOrigin, const GrSurface* src,
    GrSurfaceOrigin srcOrigin, const GrGLCaps& caps) {
  ...
  return caps.canCopyTexSubImage(
      dst->config(), dstHasMSAARenderBuffer, SkToBool(dstTex), dstIsGLTexture2D, dstOrigin,
      src->config(), srcHasMSAARenderBuffer, SkToBool(srcTex), srcIsGLTexture2D, srcOrigin);
}

// If a temporary FBO was created, its non-zero ID is returned.
void GrGLGpu::bindSurfaceFBOForPixelOps(
    GrSurface* surface, GrGLenum fboTarget, TempFBOTarget tempFBOTarget) {
  ...
  if (!rt) {
    ...
    GR_GL_CALL(
        this->glInterface(),
        FramebufferTexture2D(fboTarget, GR_GL_COLOR_ATTACHMENT0, target, texID, 0));
...
void GrGLGpu::unbindTextureFBOForPixelOps(GrGLenum fboTarget, GrSurface* surface) {
  ...
  if (!surface->asRenderTarget()) {
    ...
    GR_GL_CALL(
        this->glInterface(),
        FramebufferTexture2D(fboTarget, GR_GL_COLOR_ATTACHMENT0, textureTarget, 0, 0));
  }
}
...
void GrGLGpu::deleteFramebuffer(GrGLuint fboid) {
  if (fboid == fBoundDrawFramebuffer &&
    ...
    GL_CALL(
        FramebufferRenderbuffer(GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_RENDERBUFFER, 0));
    GL_CALL(FramebufferRenderbuffer(
        GR_GL_FRAMEBUFFER, GR_GL_STENCIL_ATTACHMENT, GR_GL_RENDERBUFFER, 0));
    GL_CALL(
        FramebufferRenderbuffer(GR_GL_FRAMEBUFFER, GR_GL_DEPTH_ATTACHMENT, GR_GL_RENDERBUFFER, 0));
  }
...
bool GrGLGpu::onCopySurface(
  ...
  if (can_blit_framebuffer_for_copy_surface(
          dst, dstOrigin, src, srcOrigin, srcRect, dstPoint, this->glCaps())) {
    return this->copySurfaceAsBlitFramebuffer(dst, dstOrigin, src, srcOrigin, srcRect, dstPoint);
  }
...
bool GrGLGpu::createCopyProgram(GrTexture* srcTex) {
  ...
  GrSLType samplerType = GrSLCombinedSamplerTypeForTextureType(srcTex->texturePriv().textureType());

  if (!fCopyProgramArrayBuffer) {
    static const GrGLfloat vdata[] = {0, 0, 0, 1, 1, 0, 1, 1};
    fCopyProgramArrayBuffer = GrGLBuffer::Make(
        this, sizeof(vdata), GrGpuBufferType::kVertex, kStatic_GrAccessPattern, vdata);
  }
  if (!fCopyProgramArrayBuffer) {
    return false;
  }
  ...
  GrShaderVar uTexCoordXform(
      "u_texCoordXform", kHalf4_GrSLType, GrShaderVar::kUniform_TypeModifier);
  ...
  GL_CALL_RET(
      fCopyPrograms[progIdx].fPosXformUniform,
      GetUniformLocation(fCopyPrograms[progIdx].fProgram, "u_posXform"));
  GL_CALL_RET(
      fCopyPrograms[progIdx].fTexCoordXformUniform,
...
bool GrGLGpu::createMipmapProgram(int progIdx) {
  ...
  GrShaderVar uTexCoordXform(
      "u_texCoordXform", kHalf4_GrSLType, GrShaderVar::kUniform_TypeModifier);
  GrShaderVar uTexture("u_texture", kTexture2DSampler_GrSLType, GrShaderVar::kUniform_TypeModifier);
  ...
  if (oddWidth && oddHeight) {
    ...
    vshaderTxt.append(
        ...
        "  v_texCoord1 = a_vertex.xy * half2(u_texCoordXform.y, 1) + half2(u_texCoordXform.x, "
        "0);");
  } else if (oddHeight) {
    vshaderTxt.append(
        "  v_texCoord0 = a_vertex.xy * half2(1, u_texCoordXform.w);"
        "  v_texCoord1 = a_vertex.xy * half2(1, u_texCoordXform.w) + half2(0, "
        "u_texCoordXform.z);");
  } else {
    vshaderTxt.append("  v_texCoord0 = a_vertex.xy;");
  }
  ...
  if (oddWidth && oddHeight) {
    ...
    fshaderTxt.append("  sk_FragColor = texture(u_texture, v_texCoord0);");
  }
  ...
  GL_CALL_RET(
      fMipmapPrograms[progIdx].fTexCoordXformUniform,
...
bool GrGLGpu::copySurfaceAsDraw(
    GrSurface* dst, GrSurfaceOrigin dstOrigin, GrSurface* src, GrSurfaceOrigin srcOrigin,
  ...
  attribs->set(
      this, 0, fCopyProgramArrayBuffer.get(), kFloat2_GrVertexAttribType, kFloat2_GrSLType,
  ...
  GL_CALL(Uniform4f(fCopyPrograms[progIdx].fTexCoordXformUniform, sx1 - sx0, sy1 - sy0, sx0, sy0));
...
void GrGLGpu::copySurfaceAsCopyTexSubImage(
    GrSurface* dst, GrSurfaceOrigin dstOrigin, GrSurface* src, GrSurfaceOrigin srcOrigin,
    const SkIRect& srcRect, const SkIPoint& dstPoint) {
  ...
  GL_CALL(CopyTexSubImage2D(
      dstTex->target(), 0, dstPoint.fX, dstY, srcGLRect.fLeft, srcGLRect.fBottom, srcGLRect.fWidth,
      srcGLRect.fHeight));
  this->unbindTextureFBOForPixelOps(GR_GL_FRAMEBUFFER, src);
  SkIRect dstRect = SkIRect::MakeXYWH(dstPoint.fX, dstPoint.fY, srcRect.width(), srcRect.height());
  this->didWriteToSurface(dst, dstOrigin, &dstRect);
}

bool GrGLGpu::copySurfaceAsBlitFramebuffer(
    GrSurface* dst, GrSurfaceOrigin dstOrigin, GrSurface* src, GrSurfaceOrigin srcOrigin,
    const SkIRect& srcRect, const SkIPoint& dstPoint) {
  SkASSERT(can_blit_framebuffer_for_copy_surface(
      dst, dstOrigin, src, srcOrigin, srcRect, dstPoint, this->glCaps()));
  SkIRect dstRect = SkIRect::MakeXYWH(dstPoint.fX, dstPoint.fY, srcRect.width(), srcRect.height());
  if (dst == src) {
  ...
  GL_CALL(BlitFramebuffer(
      srcGLRect.fLeft, srcY0, srcGLRect.fLeft + srcGLRect.fWidth, srcY1, dstGLRect.fLeft,
      dstGLRect.fBottom, dstGLRect.fLeft + dstGLRect.fWidth, dstGLRect.fBottom + dstGLRect.fHeight,
...
bool GrGLGpu::onRegenerateMipMapLevels(GrTexture* texture) {
  ...
  if (!fMipmapProgramArrayBuffer) {
    static const GrGLfloat vdata[] = {0, 0, 0, 1, 1, 0, 1, 1};
    fMipmapProgramArrayBuffer = GrGLBuffer::Make(
        this, sizeof(vdata), GrGpuBufferType::kVertex, kStatic_GrAccessPattern, vdata);
  }
  if (!fMipmapProgramArrayBuffer) {
    return false;
  ...
  attribs->enableVertexArrays(this, 1);
  attribs->set(
      this, 0, fMipmapProgramArrayBuffer.get(), kFloat2_GrVertexAttribType, kFloat2_GrSLType,
      2 * sizeof(GrGLfloat), 0);
  ...
  for (GrGLint level = 1; level < levelCount; ++level) {
    ...
    GL_CALL(Uniform4f(
        fMipmapPrograms[progIdx].fTexCoordXformUniform, invWidth, (width - 1) * invWidth, invHeight,
        (height - 1) * invHeight));
    ...
    GL_CALL(FramebufferTexture2D(
        GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_TEXTURE_2D, glTex->textureID(), level));
  ...
  // Unbind:
  GL_CALL(FramebufferTexture2D(GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_TEXTURE_2D, 0, 0));
...
static bool gl_format_to_pixel_config(GrGLenum format, GrPixelConfig* config) {
  ...
  switch (format) {
    case GR_GL_RGBA8: *config = kRGBA_8888_GrPixelConfig; return true;
    case GR_GL_RGB8: *config = kRGB_888_GrPixelConfig; return true;
    case GR_GL_RG8: *config = kRG_88_GrPixelConfig; return true;
    case GR_GL_BGRA8: *config = kBGRA_8888_GrPixelConfig; return true;
    case GR_GL_LUMINANCE8: *config = kGray_8_GrPixelConfig; return true;
    case GR_GL_SRGB8_ALPHA8:
      *config = kSRGBA_8888_GrPixelConfig;  // aliasing kSBGRA_8888 here
      return true;
    case GR_GL_RGB10_A2: *config = kRGBA_1010102_GrPixelConfig; return true;
    case GR_GL_RGB565: *config = kRGB_565_GrPixelConfig; return true;
    case GR_GL_RGBA4: *config = kRGBA_4444_GrPixelConfig; return true;
    case GR_GL_ALPHA8:  // fall through
    case GR_GL_R8: *config = kAlpha_8_GrPixelConfig; return true;
    case GR_GL_RGBA32F: *config = kRGBA_float_GrPixelConfig; return true;
    case GR_GL_RG32F: *config = kRG_float_GrPixelConfig; return true;
    case GR_GL_RGBA16F: *config = kRGBA_half_GrPixelConfig; return true;
    case GR_GL_COMPRESSED_RGB8_ETC2: *config = kRGB_ETC1_GrPixelConfig; return true;
    case GR_GL_R16F: *config = kAlpha_half_GrPixelConfig; return true;
  }
...
GrBackendTexture GrGLGpu::createBackendTexture(
    int w, int h, const GrBackendFormat& format, GrMipMapped mipMapped, GrRenderable renderable,
    const void* pixels, size_t rowBytes, const SkColor4f& colorf) {
  this->handleDirtyContext();
  ...
  if (w == 0 || w > this->caps()->maxTextureSize() || h == 0 ||
      h > this->caps()->maxTextureSize()) {
    return GrBackendTexture();  // invalid
  }
  ...
  if (!this->createTextureImpl(
          desc, &info, renderable, &initialTexParams, texels.get(), mipLevelCount, nullptr)) {
    return GrBackendTexture();  // invalid
  }
...
GrBackendRenderTarget GrGLGpu::createTestingOnlyBackendRenderTarget(
    int w, int h, GrColorType colorType) {
  ...
  if (config == kBGRA_8888_GrPixelConfig && this->glCaps().bgraIsInternalFormat()) {
    ...
    this->glCaps().getTexImageFormats(
        config, config, &colorBufferFormat, &externalFormat, &externalType);
    useTexture = true;
  } else {
  ...
  if (useTexture) {
    this->bindTextureToScratchUnit(GR_GL_TEXTURE_2D, colorID);
    GL_CALL(TexImage2D(
        GR_GL_TEXTURE_2D, 0, colorBufferFormat, w, h, 0, externalFormat, externalType, nullptr));
    GL_CALL(FramebufferTexture2D(
        GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_TEXTURE_2D, colorID, 0));
  } else {
    GL_CALL(BindRenderbuffer(GR_GL_RENDERBUFFER, colorID));
    GL_ALLOC_CALL(
        this->glInterface(), RenderbufferStorage(GR_GL_RENDERBUFFER, colorBufferFormat, w, h));
    GL_CALL(FramebufferRenderbuffer(
        GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_RENDERBUFFER, colorID));
  }
  ...
  GL_ALLOC_CALL(
      this->glInterface(), RenderbufferStorage(GR_GL_RENDERBUFFER, stencilBufferFormat, w, h));
  GL_CALL(FramebufferRenderbuffer(
      GR_GL_FRAMEBUFFER, GR_GL_STENCIL_ATTACHMENT, GR_GL_RENDERBUFFER, stencilID));
  if (this->glCaps().stencilFormats()[sFormatIdx].fPacked) {
    GL_CALL(FramebufferRenderbuffer(
        GR_GL_FRAMEBUFFER, GR_GL_DEPTH_ATTACHMENT, GR_GL_RENDERBUFFER, stencilID));
  }
  ...
  if (skColorType != kUnknown_SkColorType) {
    SkASSERT(
        this->caps()->validateBackendRenderTarget(beRT, GrColorTypeToSkColorType(colorType)) !=
...
void GrGLGpu::testingOnly_flushGpuAndSync() { GL_CALL(Finish()); }
#endif

///////////////////////////////////////////////////////////////////////////////

GrGLAttribArrayState* GrGLGpu::HWVertexArrayState::bindInternalVertexArray(
    GrGLGpu* gpu, const GrBuffer* ibuf) {
  GrGLAttribArrayState* attribState;
...
void GrGLGpu::onFinishFlush(
    GrSurfaceProxy*[], int, SkSurface::BackendSurfaceAccess access, const GrFlushInfo& info,
...
void GrGLGpu::deleteFence(GrFence fence) const { this->deleteSync((GrGLsync)fence); }
...
sk_sp<GrSemaphore> GrGLGpu::wrapBackendSemaphore(
...
void GrGLGpu::checkFinishProcs() {
  ...
  while (!fFinishCallbacks.empty() && this->waitSync(
                                          fFinishCallbacks.front().fSync,
...
void GrGLGpu::deleteSync(GrGLsync sync) const { GL_CALL(DeleteSync(sync)); }

void GrGLGpu::insertEventMarker(const char* msg) { GL_CALL(InsertEventMarker(strlen(msg), msg)); }
...
int GrGLGpu::TextureToCopyProgramIdx(GrTexture* texture) {
  switch (GrSLCombinedSamplerTypeForTextureType(texture->texturePriv().textureType())) {
    ...
    case kTextureExternalSampler_GrSLType: return 2;
    default: SK_ABORT("Unexpected samper type"); return 0;
  }
}
