...
bool GrGLGpu::createRenderTargetObjects(
  ...
  if (rtIDs->fMSColorRenderbufferID) {
    ...
    GL_CALL(FramebufferTexture2DMultisample(
        GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, desc.fTarget, desc.fID, 0, sampleCount));
  }
...
bool GrGLGpu::onUpdateCompressedBackendTexture(
    const GrBackendTexture& backendTexture, sk_sp<GrRefCntedCallback> finishedCallback,
...
int GrGLGpu::getCompatibleStencilIndex(GrGLFormat format) {
  ...
  if (!this->glCaps().hasStencilFormatBeenDeterminedForFormat(format)) {
    ...
    GrGLuint colorID = this->createTexture(
        {kSize, kSize}, format, GR_GL_TEXTURE_2D, GrRenderable::kYes, nullptr, 1);
    if (!colorID) {
      return -1;
    }
    // unbind the texture from the texture unit before binding it to the frame buffer
...
void GrGLGpu::endCommandBuffer(
  ...
  if (GrGLCaps::kNone_InvalidateFBType != this->glCaps().invalidateFBType()) {
    ...
    if (GrStoreOp::kDiscard == colorLoadStore.fStoreOp) {
      GrGLuint renderFBOID = (useMultisampleFBO) ? rt->multisampleFBOID() : rt->singleSampleFBOID();
...
void GrGLGpu::flushRenderTargetNoColorWrites(GrGLRenderTarget* target, bool useMultisampleFBO) {
  ...
  if (fHWBoundRenderTargetUniqueID != rtID || fHWBoundFramebufferIsMSAA != useMultisampleFBO) {
    this->bindFramebuffer(
        GR_GL_FRAMEBUFFER,
        (useMultisampleFBO) ? target->multisampleFBOID() : target->singleSampleFBOID());
#ifdef SK_DEBUG
...
GrGLenum GrGLGpu::prepareToDraw(GrPrimitiveType primitiveType) {
  ...
  switch (primitiveType) {
    ...
    case GrPrimitiveType::kPatches: return GR_GL_PATCHES;
    case GrPrimitiveType::kPath: SK_ABORT("non-mesh-based GrPrimitiveType"); return 0;
  }
...
void GrGLGpu::resolveRenderFBOs(
  ...
  if (caps.invalidateFBType() != GrGLCaps::kNone_InvalidateFBType &&
    ...
    GrGLenum colorDiscardAttachment =
        (rt->multisampleFBOID() == 0) ? GR_GL_COLOR : GR_GL_COLOR_ATTACHMENT0;
...
void GrGLGpu::flushBlendAndColorWrite(
  ...
  bool blendOff = GrBlendShouldDisable(equation, srcCoeff, dstCoeff) || !blendInfo.fWriteColor;
...
bool GrGLGpu::createCopyProgram(GrTexture* srcTex) {
  ...
  GrShaderVar uTexCoordXform(
      "u_texCoordXform", kHalf4_GrSLType, GrShaderVar::TypeModifier::Uniform);
...
bool GrGLGpu::createMipmapProgram(int progIdx) {
  ...
  GrShaderVar uTexCoordXform(
      "u_texCoordXform", kHalf4_GrSLType, GrShaderVar::TypeModifier::Uniform);
  GrShaderVar uTexture("u_texture", kTexture2DSampler_GrSLType, GrShaderVar::TypeModifier::Uniform);
...
GrBackendTexture GrGLGpu::onCreateBackendTexture(
  ...
  info.fID = this->createTexture(
      dimensions, glFormat, info.fTarget, renderable, &initialState, numMipLevels);
  if (!info.fID) {
    return {};
  }
...
GrBackendRenderTarget GrGLGpu::createTestingOnlyBackendRenderTarget(
  ...
  if (useTexture) {
    ...
    colorID = this->createTexture(
        dimensions, format, GR_GL_TEXTURE_2D, GrRenderable::kYes, &initialState, 1);
    if (!colorID) {
      deleteIDs();
      return {};
    }
