...
sk_sp<GrTexture> GrGLGpu::onWrapBackendTexture(
  ...
  GrPixelConfig config =
      this->caps()->getConfigFromBackendFormat(backendTex.getBackendFormat(), grColorType);
  ...
  auto texture = GrGLTexture::MakeWrapped(
      this, surfDesc, mipMapsStatus, idDesc, backendTex.getGLTextureParams(), cacheable, ioType);
...
sk_sp<GrTexture> GrGLGpu::onWrapRenderableBackendTexture(
  ...
  GrPixelConfig config = caps->getConfigFromBackendFormat(backendTex.getBackendFormat(), colorType);
  SkASSERT(kUnknown_GrPixelConfig != config);
  ...
  sampleCnt = caps->getRenderTargetSampleCount(sampleCnt, colorType, backendTex.getBackendFormat());
  if (sampleCnt < 1) {
    return nullptr;
  }
...
sk_sp<GrRenderTarget> GrGLGpu::onWrapBackendRenderTarget(
  ...
  GrPixelConfig config =
      this->caps()->getConfigFromBackendFormat(backendRT.getBackendFormat(), grColorType);
  SkASSERT(kUnknown_GrPixelConfig != config);
  ...
  desc.fConfig = config;
  int sampleCount = this->caps()->getRenderTargetSampleCount(
      backendRT.sampleCnt(), grColorType, backendRT.getBackendFormat());

  return GrGLRenderTarget::MakeWrapped(
      this, desc, sampleCount, info.fFormat, idDesc, backendRT.stencilBits());
}
...
sk_sp<GrRenderTarget> GrGLGpu::onWrapBackendTextureAsRenderTarget(
  ...
  GrPixelConfig config =
      this->caps()->getConfigFromBackendFormat(tex.getBackendFormat(), grColorType);
  SkASSERT(kUnknown_GrPixelConfig != config);
  ...
  surfDesc.fConfig = config;
  int sampleCount =
      this->caps()->getRenderTargetSampleCount(sampleCnt, grColorType, tex.getBackendFormat());
...
static inline GrGLint config_alignment(GrPixelConfig config) {
  ...
  switch (config) {
    ...
    case kR_16_GrPixelConfig: return 2;
    ...
    case kRG_1616_GrPixelConfig: return 4;
    case kRGB_ETC1_GrPixelConfig:
    case kUnknown_GrPixelConfig: return 0;

    // Experimental (for Y416 and mutant P016/P010)
    case kRGBA_16161616_GrPixelConfig: return 8;
    case kRG_half_GrPixelConfig: return 4;
  }
...
static bool allocate_and_populate_texture(
    ...
    int baseHeight, bool* changedUnpackRowLength) {
  ...
  if (kNewTexture_UploadType == uploadType) {
    if (0 == left && 0 == top && texWidth == width && texHeight == height) {
      succeeded = allocate_and_populate_texture(
          ...
          externalFormat, externalType, texels, mipLevelCount, width, height, &restoreGLRowLength);
    } else {
      succeeded = false;
    }
  } else {
    for (int currentMipLevel = 0; currentMipLevel < mipLevelCount; currentMipLevel++) {
      ...
      GL_CALL(TexSubImage2D(
          ...
          externalType, texels[currentMipLevel].fPixels));
    }
  }
...
GrGLenum GrGLGpu::uploadCompressedTexData(
    SkImage::CompressionType compressionType, int width, int height, GrGLenum target,
    const void* data) {
  const GrGLCaps& caps = this->glCaps();
  ...
  if (useTexStorage) {
    // We never resize or change formats of textures.
    GL_ALLOC_CALL(
        this->glInterface(), TexStorage2D(target, kMipLevelCount, internalFormat, width, height));
    GrGLenum error = CHECK_ALLOC_ERROR(this->glInterface());
    if (error != GR_GL_NO_ERROR) {
      return 0;
    }
    GL_CALL(CompressedTexSubImage2D(
        target,
        0,  // level
        0,  // left
        0,  // top
        width, height, internalFormat, SkToInt(dataSize), data));
  } else {
    GL_ALLOC_CALL(
        this->glInterface(), CompressedTexImage2D(
                                 target,
                                 0,  // level
                                 internalFormat, width, height,
                                 0,  // border
                                 SkToInt(dataSize), data));
...
bool GrGLGpu::createRenderTargetObjects(
    const GrSurfaceDesc& desc, int sampleCount, const GrGLTextureInfo& texInfo,
    GrGLRenderTarget::IDDesc* idDesc) {
  idDesc->fMSColorRenderbufferID = 0;
  idDesc->fRTFBOID = 0;
  ...
  if (idDesc->fRTFBOID != idDesc->fTexFBOID) {
    ...
    if (!renderbuffer_storage_msaa(
            *fGLContext, sampleCount, colorRenderbufferFormat, desc.fWidth, desc.fHeight)) {
      goto FAILED;
    }
  ...
  if (this->glCaps().usesImplicitMSAAResolve() && sampleCount > 1) {
    GL_CALL(FramebufferTexture2DMultisample(
        GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, texInfo.fTarget, texInfo.fID, 0, sampleCount));
  } else {
...
sk_sp<GrTexture> GrGLGpu::onCreateTexture(
    ...
    SkBudgeted budgeted, GrProtected isProtected, const GrMipLevel texels[], int mipLevelCount) {
  ...
  if (!this->createTextureImpl(
          desc, &idDesc.fInfo, renderable, &initialState, texels, mipLevelCount, &mipMapsStatus)) {
  ...
  if (renderable == GrRenderable::kYes) {
    ...
    if (!this->createRenderTargetObjects(desc, renderTargetSampleCnt, idDesc.fInfo, &rtIDDesc)) {
      GL_CALL(DeleteTextures(1, &idDesc.fInfo.fID));
      return return_null_texture();
    }
    tex = sk_make_sp<GrGLTextureRenderTarget>(
        this, budgeted, desc, renderTargetSampleCnt, idDesc, rtIDDesc, mipMapsStatus);
  ...
  if (clearLevelsWithoutData) {
    ...
    for (int i = 0; i < levelCnt; ++i) {
      if (i >= mipLevelCount || !texels[i].fPixels) {
        GL_CALL(ClearTexImage(tex->textureID(), i, GR_GL_RGBA, GR_GL_UNSIGNED_BYTE, &kZero));
      }
    }
  }
  return std::move(tex);
...
sk_sp<GrTexture> GrGLGpu::onCreateCompressedTexture(
    int width, int height, SkImage::CompressionType compression, SkBudgeted budgeted,
    const void* data) {
  GrGLTexture::IDDesc idDesc;
  GrGLTextureParameters::SamplerOverriddenState initialState;
  if (!this->createCompressedTextureImpl(
          &idDesc.fInfo, width, height, compression, &initialState, data)) {
    return nullptr;
  }
  idDesc.fOwnership = GrBackendObjectOwnership::kOwned;
  ...
  desc.fWidth = width;
  desc.fHeight = height;
  auto tex = sk_make_sp<GrGLTexture>(this, budgeted, desc, idDesc, GrMipMapsStatus::kNotAllocated);
  // The non-sampler params are still at their default values.
  tex->parameters()->set(
      &initialState, GrGLTextureParameters::NonsamplerState(), fResetTimestampForTextureParameters);
  return std::move(tex);
...
GrStencilAttachment* GrGLGpu::createStencilAttachmentForRenderTarget(
  ...
  if (numStencilSamples > 1) {
    SkAssertResult(renderbuffer_storage_msaa(
...
bool GrGLGpu::readOrTransferPixelsFrom(
  ...
  if (this->glCaps().detachStencilFromMSAABuffersBeforeReadPixels() && renderTarget &&
      renderTarget->renderTargetPriv().getStencilAttachment() && renderTarget->numSamples() > 1) {
...
bool GrGLGpu::onReadPixels(
  ...
  return this->readOrTransferPixelsFrom(
      surface, left, top, width, height, dstColorType, buffer, rowPixelWidth);
}
...
static inline bool can_blit_framebuffer_for_copy_surface(
  ...
  return caps.canCopyAsBlit(
      dst->config(), dstSampleCnt, dstTexTypePtr, src->config(), srcSampleCnt, srcTexTypePtr,
...
static inline bool can_copy_texsubimage(
  ...
  return caps.canCopyTexSubImage(
      dst->config(), dstHasMSAARenderBuffer, dstTexTypePtr, src->config(), srcHasMSAARenderBuffer,
      srcTexTypePtr);
}
...
GrBackendTexture GrGLGpu::createBackendTexture(
    ...
    const void* srcPixels, size_t rowBytes, const SkColor4f* color, GrProtected isProtected) {
  this->handleDirtyContext();
  ...
  if (w < 1 || w > this->caps()->maxTextureSize() || h < 1 || h > this->caps()->maxTextureSize()) {
    return GrBackendTexture();  // invalid
  }
  ...
  if (GrGLFormatToCompressionType(glFormat, &compressionType)) {
    ...
    if (!this->createCompressedTextureImpl(
            &info, w, h, compressionType, &initialState, srcPixels)) {
      return GrBackendTexture();
    }
  } else {
    if (srcPixels) {
    ...
    desc.fConfig = config;
    if (!this->createTextureImpl(
            desc, &info, renderable, &initialState, texels.get(), mipLevelCount, nullptr)) {
      return GrBackendTexture();  // invalid
    }
  }
