...
bool GrGLGpu::uploadCompressedTexData(
    GrGLFormat format, SkImage::CompressionType compressionType, const SkISize& dimensions,
  ...
  if (useTexStorage) {
    ...
    GL_ALLOC_CALL(
        this->glInterface(),
        TexStorage2D(
            target, kMipLevelCount, internalFormat, dimensions.width(), dimensions.height()));
    ...
    GL_CALL(CompressedTexSubImage2D(
        ...
        0,  // left
        0,  // top
        dimensions.width(), dimensions.height(), internalFormat, SkToInt(dataSize), data));
  } else {
    GL_ALLOC_CALL(
...
GrGLuint GrGLGpu::createTexture2D(
    const SkISize& dimensions, GrGLFormat format, GrRenderable renderable,
...
GrBackendTexture GrGLGpu::onCreateBackendTexture(
  ...
  if (data && data->type() == BackendTextureData::Type::kPixmaps) {
    ...
    if (!this->uploadTexData(
            glFormat, colorType, desc.fWidth, desc.fHeight, GR_GL_TEXTURE_2D, 0, 0, desc.fWidth,
            desc.fHeight, colorType, texels.begin(), texels.count())) {
