...
bool GrGLGpu::onWritePixels(
  ...
  return this->uploadColorTypeTexData(
      ...
      srcColorType, texels, mipLevelCount);
}
...
bool GrGLGpu::uploadColorToTex(
    GrGLFormat textureFormat, SkISize texDims, GrGLenum target, SkColor4f color,
    uint32_t levelMask) {
  ...
  if (levelClearMask) {
    if (this->glCaps().clearTextureSupport()) {
      ...
      this->uploadColorToTex(
          texDesc.fFormat, texDesc.fSize, texDesc.fTarget, kZeroColor, levelClearMask);
    }
  }
  return std::move(tex);
}
...
bool GrGLGpu::onUpdateCompressedBackendTexture(
  ...
  if (data->type() == BackendTextureData::Type::kCompressed) {
    ...
    rawDataSize = SkCompressedDataSize(
        compression, backendTexture.dimensions(), nullptr, backendTexture.hasMipmaps());

    am.reset(rawDataSize);

    GrFillInCompressedData(
        compression, backendTexture.dimensions(), mipMapped, (char*)am.get(), data->color());
  ...
  bool result = this->uploadCompressedTexData(
      compression, glFormat, backendTexture.dimensions(), mipMapped, GR_GL_TEXTURE_2D, rawData,
      rawDataSize);
...
sk_sp<GrAttachment> GrGLGpu::makeStencilAttachmentForRenderTarget(
  ...
  if (numStencilSamples > 1) {
    if (!this->renderbufferStorageMSAA(
            *fGLContext, numStencilSamples, glFmt, dimensions.width(), dimensions.height())) {
      GL_CALL(DeleteRenderbuffers(1, &sbDesc.fRenderbufferID));
      return nullptr;
    }
  } else {
    GrGLenum error = GL_ALLOC_CALL(
        RenderbufferStorage(GR_GL_RENDERBUFFER, glFmt, dimensions.width(), dimensions.height()));
    if (error != GR_GL_NO_ERROR) {
      GL_CALL(DeleteRenderbuffers(1, &sbDesc.fRenderbufferID));
      return nullptr;
    }
  ...
  return sk_sp<GrAttachment>(new GrGLAttachment(
      this, sbDesc, dimensions, GrAttachment::UsageFlags::kStencilAttachment, numStencilSamples,
      sFmt));
}
...
void GrGLGpu::beginCommandBuffer(
    GrRenderTarget* rt, const SkIRect& bounds, GrSurfaceOrigin origin,
...
void GrGLGpu::endCommandBuffer(
    GrRenderTarget* rt, const GrOpsRenderPass::LoadAndStoreInfo& colorLoadStore,
    const GrOpsRenderPass::StencilLoadAndStoreInfo& stencilLoadStore) {
  SkASSERT(fIsExecutingCommandBuffer_DebugOnly);
...
bool GrGLGpu::readOrTransferPixelsFrom(
  ...
  if (this->glCaps().detachStencilFromMSAABuffersBeforeReadPixels() && renderTarget &&
      renderTarget->getStencilAttachment() && renderTarget->numSamples() > 1) {
  ...
  if (reattachStencil) {
    auto* stencilAttachment = static_cast<GrGLAttachment*>(renderTarget->getStencilAttachment());
    GL_CALL(FramebufferRenderbuffer(
...
GrOpsRenderPass* GrGLGpu::onGetOpsRenderPass(
    GrRenderTarget* rt, GrAttachment*, GrSurfaceOrigin origin, const SkIRect& bounds,
...
bool GrGLGpu::onUpdateBackendTexture(
    const GrBackendTexture& backendTexture, sk_sp<GrRefCntedCallback> finishedCallback,
    const BackendTextureData* data) {
  GrGLTextureInfo info;
  SkAssertResult(backendTexture.getGLTextureInfo(&info));
  ...
  if (data->type() == BackendTextureData::Type::kPixmaps) {
    ...
    result = this->uploadColorTypeTexData(
        glFormat, colorType, backendTexture.dimensions(), info.fTarget, dstRect, colorType,
        texels.begin(), texels.count());
  } else if (data->type() == BackendTextureData::Type::kColor) {
    uint32_t levelMask = (1 << numMipLevels) - 1;
    result = this->uploadColorToTex(
        glFormat, backendTexture.dimensions(), info.fTarget, data->color(), levelMask);
  }
