...
sk_sp<GrTexture> GrGLGpu::onCreateTexture(
  ...
  if (levelClearMask) {
    ...
    size_t bpp;
    this->glCaps().getTexSubImageZeroFormatTypeAndBpp(
        texDesc.fFormat, &externalFormat, &externalType, &bpp);
...
bool GrGLGpu::flushGLState(
  ...
  sk_sp<GrGLProgram> program(
      fProgramCache->refProgram(this, renderTarget, programInfo, primitiveType));
  if (!program) {
...
GrBackendTexture GrGLGpu::onCreateBackendTexture(
  ...
  if (srcData) {
    ...
    size_t totalSize =
        GrComputeTightCombinedBufferSize(bytesPerPixel, w, h, &individualMipOffsets, mipLevelCount);
  ...
  info.fID = this->createTexture2D(
      {desc.fWidth, desc.fHeight}, glFormat, renderable, &initialState, SkTMax(1, texels.count()));
  if (!info.fID) {
    return GrBackendTexture();  // invalid
  }
  ...
  if (!texels.empty() &&
      !this->uploadTexData(
          glFormat, textureColorType, desc.fWidth, desc.fHeight, GR_GL_TEXTURE_2D, 0, 0,
          desc.fWidth, desc.fHeight, srcColorType, texels.begin(), texels.count())) {
