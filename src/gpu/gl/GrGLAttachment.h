...
class GrGLAttachment : public GrAttachment {
 public:
  static sk_sp<GrGLAttachment> MakeStencil(
      GrGLGpu* gpu, SkISize dimensions, int sampleCnt, GrGLFormat format);

  static sk_sp<GrGLAttachment> MakeMSAA(
      GrGLGpu* gpu, SkISize dimensions, int sampleCnt, GrGLFormat format);

  static sk_sp<GrGLAttachment> MakeWrappedRenderBuffer(
      GrGpu* gpu, GrGLuint renderbufferID, SkISize dimensions, UsageFlags supportedUsages,
      int sampleCnt, GrGLFormat format) {
    return sk_sp<GrGLAttachment>(
        new GrGLAttachment(gpu, renderbufferID, dimensions, supportedUsages, sampleCnt, format));
  }
 ...
 private:
  GrGLAttachment(
      ...
      int sampleCnt, GrGLFormat format)
      : GrAttachment(
            gpu, dimensions, supportedUsages, sampleCnt, GrMipmapped::kNo, GrProtected::kNo),
        fFormat(format),
        fRenderbufferID(renderbufferID) {
    SkASSERT(
        supportedUsages == UsageFlags::kStencilAttachment ||
