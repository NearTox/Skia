...
template <int N>
using vec = skvx::Vec<N, float>;
using float2 = vec<2>;
using float4 = vec<4>;

template <int N>
using ivec = skvx::Vec<N, int32_t>;
using int2 = ivec<2>;
using int4 = ivec<4>;

template <int N>
using uvec = skvx::Vec<N, uint32_t>;
using uint2 = uvec<2>;
using uint4 = uvec<4>;
...
template <int N>
SK_ALWAYS_INLINE vec<N> fast_madd(vec<N> f, vec<N> m, vec<N> a) {
#if FP_FAST_FMAF
...
template <int N>
SK_ALWAYS_INLINE vec<N> approx_acos(vec<N> x) {
...
template <typename T>
SK_ALWAYS_INLINE void strided_load4(
    const T* v, skvx::Vec<1, T>& a, skvx::Vec<1, T>& b, skvx::Vec<1, T>& c, skvx::Vec<1, T>& d) {
  a.val = v[0];
  b.val = v[1];
  c.val = v[2];
  d.val = v[3];
...
SK_ALWAYS_INLINE typename std::enable_if<N >= 2, void>::type strided_load4(
    const T* v, skvx::Vec<N, T>& a, skvx::Vec<N, T>& b, skvx::Vec<N, T>& c, skvx::Vec<N, T>& d) {
...
#    define IMPL_LOAD4_TRANSPOSED(N, T, VLD)                                      \
      template <>                                                                 \
      SK_ALWAYS_INLINE void strided_load4(                                        \
          const T* v, skvx::Vec<N, T>& a, skvx::Vec<N, T>& b, skvx::Vec<N, T>& c, \
          skvx::Vec<N, T>& d) {                                                   \
        auto mat = VLD(v);                                                        \
...
template <int N, typename T>
SK_ALWAYS_INLINE typename std::enable_if<N >= 2, void>::type strided_load2(
    const T* v, skvx::Vec<N, T>& a, skvx::Vec<N, T>& b) {
