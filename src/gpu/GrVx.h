...
template <int N>
vec<N> inline fast_madd(vec<N> f, vec<N> m, vec<N> a) {
#if FP_FAST_FMAF
...
template <int N>
inline vec<N> approx_acos(vec<N> x) {
...
template <int N>
inline vec<N> approx_angle_between_vectors(vec<N> ax, vec<N> ay, vec<N> bx, vec<N> by) {
