...
void GrOpsRenderPass::bindPipeline(const GrProgramInfo& programInfo, const SkRect& drawBounds) {
  ...
  fXferBarrierType =
      programInfo.pipeline().xferBarrierType(fRenderTarget->asTexture(), *this->gpu()->caps());
}
...
void GrOpsRenderPass::bindTextures(
...
void GrOpsRenderPass::drawIndexed(
...
void GrOpsRenderPass::drawInstanced(
...
void GrOpsRenderPass::drawIndirect(
  ...
  if (!this->gpu()->caps()->nativeDrawIndirectSupport()) {
    ...
    auto cmd =
        reinterpret_cast<const GrDrawIndirectCommand*>(cpuIndirectBuffer->data() + bufferOffset);
    auto end = cmd + drawCount;
    for (; cmd != end; ++cmd) {
      this->onDrawInstanced(
          cmd->fInstanceCount, cmd->fBaseInstance, cmd->fVertexCount, cmd->fBaseVertex);
    }
    return;
  }
...
void GrOpsRenderPass::drawIndexedIndirect(
    const GrBuffer* drawIndirectBuffer, size_t bufferOffset, int drawCount) {
  SkASSERT(this->gpu()->caps()->drawInstancedSupport());
  SkASSERT(
      drawIndirectBuffer->isCpuBuffer() ||
  ...
  if (!this->gpu()->caps()->nativeDrawIndirectSupport() ||
    ...
    for (; cmd != end; ++cmd) {
      this->onDrawIndexedInstanced(
          cmd->fIndexCount, cmd->fBaseIndex, cmd->fInstanceCount, cmd->fBaseInstance,
          cmd->fBaseVertex);
    }
    return;
  }
...
void GrOpsRenderPass::drawIndexPattern(
  ...
  while (baseRepetition < patternRepeatCount) {
    int repeatCount =
        std::min(patternRepeatCount - baseRepetition, maxPatternRepetitionsInIndexBuffer);
    ...
    this->drawIndexed(
        drawIndexCount, 0, minIndexValue, maxIndexValue,
