...
const char* equation_string(skgpu::BlendEquation eq) {
  switch (eq) {
    ...
    case skgpu::BlendEquation::kIllegal: SkASSERT(false); return "<illegal>";
  }
...
const char* coeff_string(skgpu::BlendCoeff coeff) {
  switch (coeff) {
    ...
    case skgpu::BlendCoeff::kIllegal: SkASSERT(false); return "<illegal>";
  }
...
SkString BlendInfo::dump() const {
  SkString out;
  out.printf(
      "writes_color(%d) equation(%s) src_coeff(%s) dst_coeff:(%s) const(0x%08x)", fWritesColor,
