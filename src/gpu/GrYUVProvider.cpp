...
sk_sp<SkCachedData> GrYUVProvider::getPlanes(
    SkYUVASizeInfo* size, SkYUVAIndex yuvaIndices[SkYUVAIndex::kIndexCount],
    SkYUVColorSpace* colorSpace, const void* constPlanes[SkYUVASizeInfo::kMaxCount]) {
...
sk_sp<GrTextureProxy> GrYUVProvider::refAsTextureProxy(
    GrRecordingContext* ctx, const GrSurfaceDesc& desc, GrColorType colorType,
  ...
  for (int i = 0; i < SkYUVASizeInfo::kMaxCount; ++i) {
    ...
    dataStoragePtr->ref();
    sk_sp<SkImage> yuvImage =
        SkImage::MakeFromRaster(pixmap, YUVGen_DataReleaseProc, dataStoragePtr);

    auto proxyProvider = ctx->priv().proxyProvider();
    yuvTextureProxies[i] = proxyProvider->createTextureProxy(yuvImage, 1, SkBudgeted::kYes, fit);
  ...
  auto renderTargetContext = ctx->priv().makeDeferredRenderTargetContext(
      SkBackingFit::kExact, desc.fWidth, desc.fHeight, colorType, nullptr, 1, GrMipMapped::kNo,
  ...
  GrPaint paint;
  auto yuvToRgbProcessor = GrYUVtoRGBEffect::Make(
      yuvTextureProxies, yuvaIndices, yuvColorSpace, GrSamplerState::Filter::kNearest);
  ...
  std::unique_ptr<GrFragmentProcessor> colorConversionProcessor = GrColorSpaceXformEffect::Make(
      srcColorSpace, kOpaque_SkAlphaType, dstColorSpace, kOpaque_SkAlphaType);
