...
sk_sp<GrTextureProxy> GrYUVProvider::refAsTextureProxy(
  ...
  for (int i = 0; i < SkYUVASizeInfo::kMaxCount; ++i) {
    ...
    SkAssertResult(bitmap.installPixels(
        imageInfo, const_cast<void*>(planes[i]), yuvSizeInfo.fWidthBytes[i], YUVGen_DataReleaseProc,
        dataStoragePtr));
    bitmap.setImmutable();
  ...
  auto yuvToRgbProcessor = GrYUVtoRGBEffect::Make(
      yuvTextureProxies, yuvaIndices, yuvColorSpace, GrSamplerState::Filter::kNearest, caps);
