...
static void add_geomproc_sampler_keys(
    GrProcessorKeyBuilder* b, const GrGeometryProcessor& geomProc, const GrCaps& caps) {
 ...
 */
static void gen_geomproc_key(
    const GrGeometryProcessor& geomProc, const GrCaps& caps, GrProcessorKeyBuilder* b) {
...
static void gen_xp_key(
    const GrXferProcessor& xp, const GrCaps& caps, const GrPipeline& pipeline,
  ...
  xp.addToKey(
      *caps.shaderCaps(), b, originIfDstTexture,
      pipeline.dstSampleFlags() & GrDstSampleFlags::kAsInputAttachment);
}

static void gen_fp_key(
    const GrFragmentProcessor& fp, const GrCaps& caps, GrProcessorKeyBuilder* b) {
  b->appendComment(fp.name());
  b->addBits(kClassIDBits, fp.classID(), "fpClassID");
  b->addBits(
      GrGeometryProcessor::kCoordTransformKeyBits,
...
static void gen_key(
    GrProcessorKeyBuilder* b, const GrProgramInfo& programInfo, const GrCaps& caps) {
...
void GrProgramDesc::Build(
    GrProgramDesc* desc, const GrProgramInfo& programInfo, const GrCaps& caps) {
  desc->reset();
...
SkString GrProgramDesc::Describe(const GrProgramInfo& programInfo, const GrCaps& caps) {
  GrProgramDesc desc;
