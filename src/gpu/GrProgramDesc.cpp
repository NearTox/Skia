...
static inline uint16_t texture_type_key(GrTextureType type) {
  ...
  switch (type) {
    ...
    case GrTextureType::kExternal: value = 1; break;
    case GrTextureType::kRectangle: value = 2; break;
    default:
...
static uint32_t sampler_key(
    GrTextureType textureType, const GrSwizzle& swizzle, const GrShaderCaps& caps) {
...
static void add_sampler_keys(
    GrProcessorKeyBuilder* b, const GrFragmentProcessor& fp, GrGpu* gpu, const GrShaderCaps& caps) {
  ...
  for (int i = 0; i < numTextureSamplers; ++i) {
    ...
    uint32_t samplerKey = sampler_key(tex->texturePriv().textureType(), sampler.swizzle(), caps);
    uint32_t extraSamplerKey =
        gpu->getExtraSamplerKeyForProgram(sampler.samplerState(), sampler.proxy()->backendFormat());
    if (extraSamplerKey) {
...
static void add_sampler_keys(
    GrProcessorKeyBuilder* b, const GrPrimitiveProcessor& pp, const GrShaderCaps& caps) {
  ...
  for (int i = 0; i < numTextureSamplers; ++i) {
    ...
    uint32_t samplerKey = sampler_key(sampler.textureType(), sampler.swizzle(), caps);
 ...
 */
static bool gen_meta_key(
    const GrFragmentProcessor& fp, GrGpu* gpu, const GrShaderCaps& shaderCaps,
    uint32_t transformKey, GrProcessorKeyBuilder* b) {
...
static bool gen_meta_key(
    const GrPrimitiveProcessor& pp, const GrShaderCaps& shaderCaps, uint32_t transformKey,
...
static bool gen_meta_key(
    const GrXferProcessor& xp, const GrShaderCaps& shaderCaps, GrProcessorKeyBuilder* b) {
  size_t processorKeySize = b->size();
...
static bool gen_frag_proc_and_meta_keys(
    const GrPrimitiveProcessor& primProc, const GrFragmentProcessor& fp, GrGpu* gpu,
    const GrShaderCaps& shaderCaps, GrProcessorKeyBuilder* b) {
  ...
  return gen_meta_key(
      fp, gpu, shaderCaps, primProc.getTransformKey(fp.coordTransforms(), fp.numCoordTransforms()),
      b);
}

bool GrProgramDesc::Build(
    GrProgramDesc* desc, const GrRenderTarget* renderTarget, const GrPrimitiveProcessor& primProc,
    bool hasPointSize, const GrPipeline& pipeline, GrGpu* gpu) {
