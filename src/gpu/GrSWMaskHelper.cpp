 ...
 */
void GrSWMaskHelper::drawRect(
    const SkRect& rect, const SkMatrix& matrix, SkRegion::Op op, GrAA aa, uint8_t alpha) {
...
void GrSWMaskHelper::drawRRect(
    const SkRRect& rrect, const SkMatrix& matrix, SkRegion::Op op, GrAA aa, uint8_t alpha) {
  SkMatrix translatedMatrix = matrix;
  translatedMatrix.postTranslate(fTranslate.fX, fTranslate.fY);
  SkSimpleMatrixProvider matrixProvider(translatedMatrix);
  fDraw.fMatrixProvider = &matrixProvider;
 ...
 */
void GrSWMaskHelper::drawShape(
    const GrStyledShape& shape, const SkMatrix& matrix, SkRegion::Op op, GrAA aa, uint8_t alpha) {
...
void GrSWMaskHelper::drawShape(
    const GrShape& shape, const SkMatrix& matrix, SkRegion::Op op, GrAA aa, uint8_t alpha) {
  SkPaint paint = get_paint(op, aa, alpha);
...
GrSurfaceProxyView GrSWMaskHelper::toTextureView(GrRecordingContext* context, SkBackingFit fit) {
  ...
  SkAssertResult(bitmap.installPixels(
      ii, fPixels->detachPixels(), rowBytes, [](void* addr, void* context) { sk_free(addr); },
      nullptr));
  bitmap.setImmutable();
