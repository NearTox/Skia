...
GrBackendTexture GrContext::createBackendTexture(const SkSurfaceCharacterization& c) {
  ...
  GrBackendTexture result = this->createBackendTexture(
      c.width(), c.height(), format, GrMipmapped(c.isMipMapped()), GrRenderable::kYes,
      c.isProtected());
...
static GrBackendTexture create_and_update_backend_texture(
    GrDirectContext* context, SkISize dimensions, const GrBackendFormat& backendFormat,
...
GrBackendTexture GrContext::createBackendTexture(
    const SkSurfaceCharacterization& c, const SkColor4f& color, GrGpuFinishedProc finishedProc,
...
GrBackendTexture GrContext::createBackendTexture(
  ...
  return create_and_update_backend_texture(
      this->asDirectContext(), {width, height}, backendFormat, mipMapped, renderable, isProtected,
...
GrBackendTexture GrContext::createBackendTexture(
  ...
  return create_and_update_backend_texture(
      this->asDirectContext(), {width, height}, format, mipMapped, renderable, isProtected,
      std::move(finishedCallback), &data);
}
...
GrBackendTexture GrContext::createBackendTexture(
  ...
  return create_and_update_backend_texture(
      this->asDirectContext(), {baseWidth, baseHeight}, backendFormat, mipMapped, renderable,
      isProtected, std::move(finishedCallback), &data);
}

bool GrContext::updateBackendTexture(
...
GrBackendTexture GrContext::createCompressedBackendTexture(
    int width, int height, const GrBackendFormat& backendFormat, const SkColor4f& color,
  ...
  return create_and_update_compressed_backend_texture(
      this->asDirectContext(), {width, height}, backendFormat, mipMapped, isProtected,
      std::move(finishedCallback), &data);
}
...
bool GrContext::setBackendTextureState(
...
bool GrContext::updateCompressedBackendTexture(
    const GrBackendTexture& backendTexture, const SkColor4f& color, GrGpuFinishedProc finishedProc,
    GrGpuFinishedContext finishedContext) {
  sk_sp<GrRefCntedCallback> finishedCallback;
  if (finishedProc) {
...
bool GrContext::setBackendRenderTargetState(
