...
sk_sp<GrTextureProxy> GrProxyProvider::findOrCreateProxyByUniqueKey(
    const GrUniqueKey& key, UseAllocator useAllocator) {
  ASSERT_SINGLE_OWNER
...
GrSurfaceProxyView GrProxyProvider::findCachedProxyWithColorTypeFallback(
    const GrUniqueKey& key, GrSurfaceOrigin origin, GrColorType ct, int sampleCnt) {
  ...
  if (renderable == GrRenderable::kYes) {
    ...
    return sk_sp<GrTextureProxy>(new GrTextureRenderTargetProxy(
        *caps, format, dimensions, renderTargetSampleCnt, mipMapped, mipmapStatus, fit, budgeted,
        isProtected, surfaceFlags | extraFlags, useAllocator, this->isDDLProvider()));
  }
...
sk_sp<GrTextureProxy> GrProxyProvider::wrapBackendTexture(
    ...
    GrIOType ioType, sk_sp<GrRefCntedCallback> releaseHelper) {
...
sk_sp<GrTextureProxy> GrProxyProvider::wrapRenderableBackendTexture(
    ...
    GrWrapCacheable cacheable, sk_sp<GrRefCntedCallback> releaseHelper) {
  if (this->isAbandoned()) {
    return nullptr;
  }
...
sk_sp<GrSurfaceProxy> GrProxyProvider::wrapBackendRenderTarget(
    const GrBackendRenderTarget& backendRT, sk_sp<GrRefCntedCallback> releaseHelper) {
  if (this->isAbandoned()) {
    return nullptr;
  }
  ...
  if (textureInfo) {
    ...
    return sk_sp<GrRenderTargetProxy>(new GrTextureRenderTargetProxy(
        *this->caps(), std::move(callback), format, dimensions, sampleCnt, textureInfo->fMipmapped,
        mipmapStatus, fit, budgeted, isProtected, surfaceFlags, useAllocator,
        this->isDDLProvider()));
  }
...
void GrProxyProvider::processInvalidUniqueKey(
    const GrUniqueKey& key, GrTextureProxy* proxy, InvalidateGPUResource invalidateGPUResource) {
