...
sk_sp<GrTextureProxy> GrProxyProvider::testingOnly_createInstantiatedProxy(
  ...
  if (SkBackingFit::kApprox == fit) {
    ...
    tex = resourceProvider->createTexture(desc, budgeted, GrResourceProvider::Flags::kNoPendingIO);
  }
  if (!tex) {
    return nullptr;
  }
...
sk_sp<GrTextureProxy> GrProxyProvider::createTextureProxy(
    sk_sp<SkImage> srcImage, GrSurfaceDescFlags descFlags, int sampleCnt, SkBudgeted budgeted,
...
sk_sp<GrTextureProxy> GrProxyProvider::createMipMapProxy(
  ...
  return this->createProxy(
      format, desc, origin, GrMipMapped::kYes, SkBackingFit::kExact, budgeted,
...
sk_sp<GrTextureProxy> GrProxyProvider::createProxyFromBitmap(
  ...
  if (GrMipMapped::kNo == mipMapped ||
    ...
    return this->createTextureProxy(
        std::move(baseLevel), kNone_GrSurfaceFlags, 1, SkBudgeted::kYes, SkBackingFit::kExact);
  }
  ...
  sk_sp<GrTextureProxy> proxy = this->createLazyProxy(
      [desc, baseLevel, mipmaps](GrResourceProvider* resourceProvider) {
        ...
        return LazyInstantiationResult(
            resourceProvider->createTexture(desc, SkBudgeted::kYes, texels.get(), mipLevelCount));
      },
...
sk_sp<GrTextureProxy> GrProxyProvider::createProxy(
  ...
  if (desc.fFlags & kRenderTarget_GrSurfaceFlag) {
    copyDesc.fSampleCnt = this->caps()->getRenderTargetSampleCount(desc.fSampleCnt, desc.fConfig);
  }
  ...
  if (copyDesc.fFlags & kRenderTarget_GrSurfaceFlag) {
    ...
    return sk_sp<GrTextureProxy>(new GrTextureRenderTargetProxy(
        *this->caps(), format, copyDesc, origin, mipMapped, fit, budgeted, surfaceFlags));
  }

  return sk_sp<GrTextureProxy>(
      new GrTextureProxy(format, copyDesc, origin, mipMapped, fit, budgeted, surfaceFlags));
}
...
sk_sp<GrTextureProxy> GrProxyProvider::wrapBackendTexture(
    const GrBackendTexture& backendTex, GrSurfaceOrigin origin, GrWrapOwnership ownership,
...
sk_sp<GrRenderTargetProxy> GrProxyProvider::wrapVulkanSecondaryCBAsRenderTarget(
  ...
  return sk_sp<GrRenderTargetProxy>(new GrRenderTargetProxy(
      std::move(rt), kTopLeft_GrSurfaceOrigin, GrRenderTargetProxy::WrapsVkSecondaryCB::kYes));
}
...
sk_sp<GrTextureProxy> GrProxyProvider::createLazyProxy(
    ...
    GrSurfaceOrigin origin, GrMipMapped mipMapped, SkBackingFit fit, SkBudgeted budgeted) {
  return this->createLazyProxy(
      std::move(callback), format, desc, origin, mipMapped, GrInternalSurfaceFlags::kNone, fit,
      budgeted);
}

sk_sp<GrTextureProxy> GrProxyProvider::createLazyProxy(
    LazyInstantiateCallback&& callback, const GrBackendFormat& format, const GrSurfaceDesc& desc,
    GrSurfaceOrigin origin, GrMipMapped mipMapped, GrInternalSurfaceFlags surfaceFlags,
  ...
  return this->createLazyProxy(
      std::move(callback), format, desc, origin, mipMapped, surfaceFlags, fit, budgeted, lazyType);
}

sk_sp<GrTextureProxy> GrProxyProvider::createLazyProxy(
    LazyInstantiateCallback&& callback, const GrBackendFormat& format, const GrSurfaceDesc& desc,
    GrSurfaceOrigin origin, GrMipMapped mipMapped, GrInternalSurfaceFlags surfaceFlags,
    SkBackingFit fit, SkBudgeted budgeted, LazyInstantiationType lazyType) {
  ...
  return sk_sp<GrTextureProxy>(
      SkToBool(kRenderTarget_GrSurfaceFlag & desc.fFlags)
          ? new GrTextureRenderTargetProxy(
                std::move(callback), lazyType, format, desc, origin, mipMapped, fit, budgeted,
                surfaceFlags)
          : new GrTextureProxy(
                std::move(callback), lazyType, format, desc, origin, mipMapped, fit, budgeted,
                surfaceFlags));
...
sk_sp<GrRenderTargetProxy> GrProxyProvider::createLazyRenderTargetProxy(
    LazyInstantiateCallback&& callback, const GrBackendFormat& format, const GrSurfaceDesc& desc,
    GrSurfaceOrigin origin, GrInternalSurfaceFlags surfaceFlags, const TextureInfo* textureInfo,
    SkBackingFit fit, SkBudgeted budgeted, bool wrapsVkSecondaryCB) {
  SkASSERT((desc.fWidth <= 0 && desc.fHeight <= 0) || (desc.fWidth > 0 && desc.fHeight > 0));
  ...
  if (textureInfo) {
    ...
    return sk_sp<GrRenderTargetProxy>(new GrTextureRenderTargetProxy(
        std::move(callback), lazyType, format, desc, origin, textureInfo->fMipMapped, fit, budgeted,
        surfaceFlags));
  }
  ...
  return sk_sp<GrRenderTargetProxy>(new GrRenderTargetProxy(
      std::move(callback), lazyType, format, desc, origin, fit, budgeted, surfaceFlags, vkSCB));
}
