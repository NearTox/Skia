...
GrTextureAdjuster::GrTextureAdjuster(
    GrRecordingContext* context, sk_sp<GrTextureProxy> original, const GrColorInfo& colorInfo,
    uint32_t uniqueID, bool useDecal)
...
sk_sp<GrTextureProxy> GrTextureAdjuster::refTextureProxyCopy(
  ...
  if (key.isValid()) {
    cachedCopy = proxyProvider->findOrCreateProxyByUniqueKey(
        key, this->colorType(), this->originalProxy()->origin());
  ...
  if (copy) {
    if (key.isValid()) {
      ...
      if (cachedCopy) {
        SkASSERT(
            GrMipMapped::kYes == copy->mipMapped() && GrMipMapped::kNo == cachedCopy->mipMapped());
...
sk_sp<GrTextureProxy> GrTextureAdjuster::onRefTextureProxyForParams(
  ...
  SkASSERT(
      this->width() <= this->context()->priv().caps()->maxTextureSize() &&
  ...
  if (!params.isRepeated() || !GrGpu::IsACopyNeededForRepeatWrapMode(
                                  this->context()->priv().caps(), proxy.get(), proxy->width(),
                                  proxy->height(), params.filter(), &copyParams, scaleAdjust)) {
...
std::unique_ptr<GrFragmentProcessor> GrTextureAdjuster::createFragmentProcessor(
    const SkMatrix& origTextureMatrix, const SkRect& constraintRect,
    FilterConstraint filterConstraint, bool coordsLimitedToConstraintRect,
    const GrSamplerState::Filter* filterOrNullForBicubic) {
  SkMatrix textureMatrix = origTextureMatrix;

  SkScalar scaleAdjust[2] = {1.0f, 1.0f};
  sk_sp<GrTextureProxy> proxy(this->refTextureProxyForParams(filterOrNullForBicubic, scaleAdjust));
  if (!proxy) {
    return nullptr;
  }
  ...
  DomainMode domainMode = DetermineDomainMode(
      constraintRect, filterConstraint, coordsLimitedToConstraintRect, proxy.get(),
      filterOrNullForBicubic, &domain);
  if (kTightCopy_DomainMode == domainMode) {
    ...
    SkASSERT(filterOrNullForBicubic && GrSamplerState::Filter::kMipMap == *filterOrNullForBicubic);
    static const GrSamplerState::Filter kBilerp = GrSamplerState::Filter::kBilerp;
    domainMode = DetermineDomainMode(
        constraintRect, filterConstraint, coordsLimitedToConstraintRect, proxy.get(), &kBilerp,
        &domain);
    SkASSERT(kTightCopy_DomainMode != domainMode);
  }
  SkASSERT(
      kNoDomain_DomainMode == domainMode ||
