...
GrTextureAdjuster::GrTextureAdjuster(
    GrRecordingContext* context, GrSurfaceProxyView original, const GrColorInfo& colorInfo,
    uint32_t uniqueID)
...
std::unique_ptr<GrFragmentProcessor> GrTextureAdjuster::createFragmentProcessor(
  ...
  DomainMode domainMode = DetermineDomainMode(
      constraintRect, filterConstraint, coordsLimitedToConstraintRect, view.proxy(),
  ...
  if (kTightCopy_DomainMode == domainMode) {
    ...
    domainMode = DetermineDomainMode(
        constraintRect, filterConstraint, coordsLimitedToConstraintRect, view.proxy(), &kBilerp,
        &domain);
  ...
  return this->createFragmentProcessorForSubsetAndFilter(
      std::move(view), textureMatrix, domainMode, domain, wrapX, wrapY, filterOrNullForBicubic);
}
