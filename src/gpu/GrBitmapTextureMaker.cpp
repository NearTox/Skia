...
GrBitmapTextureMaker::GrBitmapTextureMaker(
    GrRecordingContext* context, const SkBitmap& bitmap, Cached cached, SkBackingFit fit,
    bool useDecal)
    : INHERITED(context, get_image_info(context, bitmap), useDecal), fBitmap(bitmap), fFit(fit) {
...
sk_sp<GrTextureProxy> GrBitmapTextureMaker::refOriginalTextureProxy(
  ...
  if (fOriginalKey.isValid()) {
    ...
    proxy = proxyProvider->findOrCreateProxyByUniqueKey(
        fOriginalKey, colorType, kTopLeft_GrSurfaceOrigin);
  ...
  if (proxy) {
    ...
    if (auto mippedProxy =
            GrCopyBaseMipMapToTextureProxy(this->context(), proxy.get(), srcColorType)) {
