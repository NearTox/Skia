...
GrBitmapTextureMaker::GrBitmapTextureMaker(
    GrRecordingContext* context, const SkBitmap& bitmap, GrImageTexGenPolicy cachePolicy)
...
GrBitmapTextureMaker::GrBitmapTextureMaker(
    ...
    : INHERITED(context, get_image_info(context, bitmap)),
      ...
      fFit(fit),
      fBudgeted(
          cachePolicy == GrImageTexGenPolicy::kNew_Uncached_Unbudgeted ? SkBudgeted::kNo
                                                                       : SkBudgeted::kYes) {
...
GrSurfaceProxyView GrBitmapTextureMaker::refOriginalTextureProxyView(GrMipMapped mipMapped) {
  ...
  if (fKey.isValid()) {
    ...
    if (proxy) {
      swizzle =
          this->context()->priv().caps()->getReadSwizzle(proxy->backendFormat(), this->colorType());
  ...
  if (proxy) {
    ...
    auto mippedView = GrCopyBaseMipMapToTextureProxy(
        this->context(), proxy.get(), kTopLeft_GrSurfaceOrigin, srcColorType);
