...
GrYUVABackendTextureInfo::GrYUVABackendTextureInfo(
    const SkYUVAInfo& yuvaInfo, const GrBackendFormat formats[kMaxPlanes], GrMipmapped mipmapped,
...
bool GrYUVABackendTextureInfo::toYUVAIndices(SkYUVAIndex indices[SkYUVAIndex::kIndexCount]) const {
  ...
  uint32_t channelFlags[] = {
      fPlaneFormats[0].channelMask(), fPlaneFormats[1].channelMask(),
      fPlaneFormats[2].channelMask(), fPlaneFormats[3].channelMask()};
...
GrYUVABackendTextures::GrYUVABackendTextures(
    const SkYUVAInfo& yuvaInfo, const GrBackendTexture textures[SkYUVAInfo::kMaxPlanes],
  ...
  for (int i = 0; i < numPlanes; ++i) {
    ...
    if (!textures[i].isValid() || textures[i].dimensions() != planeDimensions[i] ||
...
bool GrYUVABackendTextures::toYUVAIndices(SkYUVAIndex indices[SkYUVAIndex::kIndexCount]) const {
  SkASSERT(indices);
  uint32_t channelFlags[] = {
      fTextures[0].getBackendFormat().channelMask(), fTextures[1].getBackendFormat().channelMask(),
      fTextures[2].getBackendFormat().channelMask(), fTextures[3].getBackendFormat().channelMask()};
  return fYUVAInfo.toYUVAIndices(channelFlags, indices);
}
