...
bool GrContextThreadSafeProxy::init(
    sk_sp<const GrCaps> caps, sk_sp<GrSkSLFPFactoryCache> FPFactoryCache) {
...
SkSurfaceCharacterization GrContextThreadSafeProxy::createCharacterization(
    ...
    bool willUseGLFBO0, bool isTextureable) {
  ...
  if (sampleCnt > 1) {
    FSAAType =
        this->caps()->usesMixedSamples() ? GrFSAAType::kMixedSamples : GrFSAAType::kUnifiedMSAA;
  }
  ...
  return SkSurfaceCharacterization(
      sk_ref_sp<GrContextThreadSafeProxy>(this), cacheMaxResourceBytes, ii, origin, config,
      FSAAType, sampleCnt, SkSurfaceCharacterization::Textureable(isTextureable),
      SkSurfaceCharacterization::MipMapped(isMipMapped),
      SkSurfaceCharacterization::UsesGLFBO0(willUseGLFBO0),
      SkSurfaceCharacterization::VulkanSecondaryCBCompatible(false), surfaceProps);
}
...
sk_sp<GrContextThreadSafeProxy> GrContextThreadSafeProxyPriv::Make(
    GrBackendApi backend, const GrContextOptions& options, uint32_t contextID,
    sk_sp<const GrCaps> caps, sk_sp<GrSkSLFPFactoryCache> cache) {
  sk_sp<GrContextThreadSafeProxy> proxy(new GrContextThreadSafeProxy(backend, options, contextID));
