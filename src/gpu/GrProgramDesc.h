...
class GrProcessorKeyBuilder {
 public:
  ...
  void addBool(bool b, const char* label) { this->addBits(1, b, label); }

  void add32(uint32_t v, const char* label = "unknown") { this->addBits(32, v, label); }
...
class GrProcessorStringKeyBuilder : public GrProcessorKeyBuilder {
 public:
  ...
  void appendComment(const char* comment) override { fDescription.appendf("%s\n", comment); }
...
class GrProgramDesc {
 public:
  ...
  const uint32_t* asKey() const noexcept { return fKey.data(); }

  // Gets the number of bytes in asKey(). It will be a 4-byte aligned value.
  uint32_t keyLength() const noexcept { return fKey.size() * sizeof(uint32_t); }

  bool operator==(const GrProgramDesc& that) const noexcept { return this->fKey == that.fKey; }

  bool operator!=(const GrProgramDesc& other) const noexcept { return !(*this == other); }

  uint32_t initialKeyLength() const noexcept { return fInitialKeyLength; }
 ...
 protected:
  ...
  enum {
    ...
    kPreAllocSize = kHeaderSize + kMaxPreallocProcessors * kIntsPerProcessor,
  };
