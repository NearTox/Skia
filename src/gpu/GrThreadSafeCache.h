...
class GrThreadSafeCache {
 public:
  ...
  std::tuple<GrSurfaceProxyView, sk_sp<SkData>> findWithData(const GrUniqueKey&)
  ...
  GrSurfaceProxyView findOrAdd(const GrUniqueKey&, const GrSurfaceProxyView&)
      SK_EXCLUDES(fSpinLock);
  ...
  class VertexData : public SkNVRefCnt<VertexData> {
   ...
   private:
    ...
    VertexData(const void* vertices, int numVertices, size_t vertexSize)
        : fVertices(vertices), fNumVertices(numVertices), fVertexSize(vertexSize) {}
  ...
  static sk_sp<VertexData> MakeVertexData(
      sk_sp<GrGpuBuffer> buffer, int vertexCount, size_t vertexSize);

  std::tuple<sk_sp<VertexData>, sk_sp<SkData>> findVertsWithData(const GrUniqueKey&)
      SK_EXCLUDES(fSpinLock);

  typedef bool (*IsNewerBetter)(SkData* incumbent, SkData* challenger);

  std::tuple<sk_sp<VertexData>, sk_sp<SkData>> addVertsWithData(
      const GrUniqueKey&, sk_sp<VertexData>, IsNewerBetter) SK_EXCLUDES(fSpinLock);
  ...
  static std::tuple<GrSurfaceProxyView, sk_sp<Trampoline>> CreateLazyView(
      GrDirectContext*, GrColorType, SkISize dimensions, GrSurfaceOrigin, SkBackingFit);

 private:
  struct Entry {
    Entry(const GrUniqueKey& key, const GrSurfaceProxyView& view)
        : fKey(key), fView(view), fTag(Entry::kView) {}

    Entry(const GrUniqueKey& key, sk_sp<VertexData> vertData)
        : fKey(key), fVertData(std::move(vertData)), fTag(Entry::kVertData) {}

    ~Entry() { this->makeEmpty(); }
  ...
  std::tuple<GrSurfaceProxyView, sk_sp<SkData>> internalFind(const GrUniqueKey&)
  ...
  std::tuple<sk_sp<VertexData>, sk_sp<SkData>> internalFindVerts(const GrUniqueKey&)
      SK_REQUIRES(fSpinLock);
  std::tuple<sk_sp<VertexData>, sk_sp<SkData>> internalAddVerts(
      const GrUniqueKey&, sk_sp<VertexData>, IsNewerBetter) SK_REQUIRES(fSpinLock);
