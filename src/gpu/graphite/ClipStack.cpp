...
bool ClipStack::TransformedShape::contains(const TransformedShape& o) const {
  ...
  if (fLocalToDevice == o.fLocalToDevice) {
    ...
    Rect localBounds = fLocalToDevice.inverseMapRect(o.fLocalToDevice.mapRect(o.fShape.bounds()));
...
ClipStack::SimplifyResult ClipStack::Simplify(
    const TransformedShape& a, const TransformedShape& b) {
  enum class ClipCombo { kDD = 0b00, kDI = 0b01, kID = 0b10, kII = 0b11 };
...
ClipStack::RawElement::RawElement(
    const Rect& deviceBounds, const Transform& localToDevice, const Shape& shape, SkClipOp op)
...
void ClipStack::RawElement::validate() const {
  ...
  SkASSERT(
      (fShape.isEmpty() || !fOuterBounds.isEmptyNegativeOrNaN()) &&
...
bool ClipStack::RawElement::combine(const RawElement& other, const SaveRecord& current) {
  ...
  if (fShape.isRect() && other.fShape.isRect()) {
    ...
    if (fLocalToDevice == other.fLocalToDevice) {
      ...
      SkRRect a = fShape.isRect() ? SkRRect::MakeRect(fShape.rect().asSkRect()) : fShape.rrect();
...
std::pair<bool, CompressedPaintersOrder> ClipStack::RawElement::updateForDraw(
    const BoundsManager* boundsManager, const TransformedShape& draw, PaintersDepth drawZ) {
  if (this->isInvalid()) {
...
ClipStack::ClipState ClipStack::RawElement::clipType() const {
  ...
  switch (fShape.type()) {
    case Shape::Type::kEmpty: return ClipState::kEmpty;

    case Shape::Type::kRect:
      return fOp == SkClipOp::kIntersect && fLocalToDevice.type() == Transform::Type::kIdentity
                 ? ClipState::kDeviceRect
                 : ClipState::kComplex;

    case Shape::Type::kRRect:
      return fOp == SkClipOp::kIntersect && fLocalToDevice.type() == Transform::Type::kIdentity
                 ? ClipState::kDeviceRRect
                 : ClipState::kComplex;
    ...
    case Shape::Type::kPath: return ClipState::kComplex;
  }
  SkUNREACHABLE;
}
...
ClipStack::SaveRecord::SaveRecord(const SaveRecord& prior, int startingElementIndex)
...
bool ClipStack::SaveRecord::addElement(
    RawElement&& toAdd, RawElement::Stack* elements, Device* device) {
  ...
  SkASSERT(
      !fOuterBounds.isEmptyNegativeOrNaN() &&
      (fInnerBounds.isEmptyNegativeOrNaN() || fOuterBounds.contains(fInnerBounds)));

  return this->appendElement(std::move(toAdd), elements, device);
}

bool ClipStack::SaveRecord::appendElement(
    RawElement&& toAdd, RawElement::Stack* elements, Device* device) {
...
void ClipStack::SaveRecord::replaceWithElement(
    RawElement&& toAdd, RawElement::Stack* elements, Device* device) {
...
ClipStack::ClipStack(Device* owningDevice)
    : fElements(kElementStackIncrement), fSaves(kSaveStackIncrement), fDevice(owningDevice) {
...
Rect ClipStack::deviceBounds() const { return Rect::WH(fDevice->width(), fDevice->height()); }
...
std::pair<Clip, CompressedPaintersOrder> ClipStack::applyClipToDraw(
    const BoundsManager* boundsManager, const Transform& localToDevice, const Shape& shape,
    const SkStrokeRec& style, PaintersDepth z) {
  ...
  TransformedShape draw{
      style.isHairlineStyle() ? kIdentity : localToDevice,
      *styledShape,
