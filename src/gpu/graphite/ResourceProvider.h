...
class ResourceProvider {
 public:
  ...
  sk_sp<Sampler> findOrCreateCompatibleSampler(
      const SkSamplingOptions&, SkTileMode xTileMode, SkTileMode yTileMode);
 ...
 private:
  virtual sk_sp<GraphicsPipeline> onCreateGraphicsPipeline(
      const GraphicsPipelineDesc&, const RenderPassDesc&) = 0;
  ...
  virtual sk_sp<Sampler> createSampler(
      const SkSamplingOptions&, SkTileMode xTileMode, SkTileMode yTileMode) = 0;

  sk_sp<Texture> findOrCreateTextureWithKey(
      SkISize dimensions, const TextureInfo& info, const GraphiteResourceKey& key, SkBudgeted);
  ...
  class GraphicsPipelineCache {
   public:
    ...
    void release();
    sk_sp<GraphicsPipeline> refPipeline(
        const Caps* caps, const GraphicsPipelineDesc&, const RenderPassDesc&);
