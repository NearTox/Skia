...
class DrawPass::SortKey {
 public:
  SortKey(
      const DrawList::Draw* draw, int renderStep, uint32_t pipelineIndex,
      UniformDataCache::Index geomUniformIndex, UniformDataCache::Index shadingUniformIndex,
      TextureDataCache::Index textureDataIndex)
      : fPipelineKey(
            ColorDepthOrderField::set(draw->fGeometry.order().paintOrder().bits()) |
        ...
        fUniformKey(
            GeometryUniformField::set(geomUniformIndex.asUInt()) |
 ...
 private:
  ...
  static_assert(
      PipelineField::kBits >= SkNextLog2_portable(Renderer::kMaxRenderSteps * DrawList::kMaxDraws));
  static_assert(
      GeometryUniformField::kBits >=
      SkNextLog2_portable(Renderer::kMaxRenderSteps * DrawList::kMaxDraws));
  static_assert(
      ShadingUniformField::kBits >=
      SkNextLog2_portable(Renderer::kMaxRenderSteps * DrawList::kMaxDraws));
  static_assert(
      TextureBindingsField::kBits >=
      SkNextLog2_portable(Renderer::kMaxRenderSteps * DrawList::kMaxDraws));
};
...
class DrawPass::Drawer final : public DrawDispatcher {
 public:
  ...
  void bindDrawBuffers(
      BindBufferInfo vertexAttribs, BindBufferInfo instanceAttribs,
  ...
  void drawIndexed(
      PrimitiveType type, unsigned int baseIndex, unsigned int indexCount,
      unsigned int baseVertex) override {
    fPass->fCommands.emplace_back(DrawIndexed{type, baseIndex, indexCount, baseVertex});
  }

  void drawInstanced(
      PrimitiveType type, unsigned int baseVertex, unsigned int vertexCount,
      unsigned int baseInstance, unsigned int instanceCount) override {
    fPass->fCommands.emplace_back(
        DrawInstanced{type, baseVertex, vertexCount, baseInstance, instanceCount});
  }

  void drawIndexedInstanced(
      PrimitiveType type, unsigned int baseIndex, unsigned int indexCount, unsigned int baseVertex,
      unsigned int baseInstance, unsigned int instanceCount) override {
    fPass->fCommands.emplace_back(
        DrawIndexedInstanced{type, baseIndex, indexCount, baseVertex, baseInstance, instanceCount});
  }
...
class UniformBindingCache {
 public:
  UniformBindingCache(DrawBufferManager* bufferMgr, UniformDataCache* uniformDataCache)
      : fBufferMgr(bufferMgr), fUniformDataCache(uniformDataCache) {}
...
DrawPass::DrawPass(
    sk_sp<TextureProxy> target, std::pair<LoadOp, StoreOp> ops, std::array<float, 4> clearColor,
    int renderStepCount)
...
struct SamplerDesc {
  ...
  bool isEqual(const SkTextureDataBlock::TextureInfo& info) {
    return fSamplingOptions == info.fSamplingOptions && fTileModes[0] == info.fTileModes[0] &&
...
std::pair<int, int> get_unique_texture_sampler_indices(
    std::vector<sk_sp<TextureProxy>>& sampledTextures, std::vector<SamplerDesc>& samplerDescs,
  ...
  if (samplerIndex == -1) {
    samplerDescs.push_back({info.fSamplingOptions, {info.fTileModes[0], info.fTileModes[1]}});
...
std::unique_ptr<DrawPass> DrawPass::Make(
    Recorder* recorder, std::unique_ptr<DrawList> draws, sk_sp<TextureProxy> target,
  ...
  std::unique_ptr<DrawPass> drawPass(
      new DrawPass(std::move(target), ops, clearColor, draws->renderStepCount()));
  ...
  for (const DrawList::Draw& draw : draws->fDraws.items()) {
    if (occlusionCuller &&
        occlusionCuller->isOccluded(
            draw.fGeometry.clip().drawBounds(), draw.fGeometry.order().depth())) {
      continue;
    }
    ...
    if (draw.fPaintParams.has_value()) {
      ...
      std::tie(shaderID, uniformDataIndex, textureBindingIndex) = ExtractPaintData(
          recorder, &gatherer, &builder, draw.fGeometry.transform().inverse(),
    ...
    for (int stepIndex = 0; stepIndex < draw.fRenderer.numRenderSteps(); ++stepIndex) {
      ...
      if (step->numUniforms() > 0) {
        UniformDataCache::Index uniformDataIndex;
        uniformDataIndex =
            ExtractRenderStepData(&geometryUniformDataCache, &gatherer, step, draw.fGeometry);
      ...
      keys.push_back(
          {&draw, stepIndex, pipelineIndex, geometryUniformIndex, stepShadingUniformIndex,
  ...
  for (const SortKey& key : keys) {
    ...
    const bool geometryUniformChange =
        key.geometryUniforms().isValid() && key.geometryUniforms() != lastGeometryUniforms;
    const bool shadingUniformChange =
        key.shadingUniforms().isValid() && key.shadingUniforms() != lastShadingUniforms;
    const bool textureBindingsChange =
        key.textureBindings().isValid() && key.textureBindings() != lastTextureBindings;

    const bool pipelineChange = key.pipeline() != lastPipeline;
    const bool stateChange = geometryUniformChange || shadingUniformChange ||
    ...
    if (pipelineChange) {
      drawWriter.newPipelineState(
          renderStep.primitiveType(), renderStep.vertexStride(), renderStep.instanceStride());
    } else if (stateChange) {
    ...
    if (stateChange) {
      if (geometryUniformChange) {
        ...
        drawPass->fCommands.emplace_back(BindUniformBuffer{binding, UniformSlot::kRenderStep});
      ...
      if (shadingUniformChange) {
        ...
        drawPass->fCommands.emplace_back(BindUniformBuffer{binding, UniformSlot::kPaint});
      ...
      if (textureBindingsChange) {
        ...
        for (int i = 0; i < bts.fNumTexSamplers; ++i) {
          ...
          std::tie(bts.fTextureIndices[i], bts.fSamplerIndices[i]) =
              get_unique_texture_sampler_indices(
                  drawPass->fSampledTextures, drawPass->fSamplerDescs, info);
        }
  ...
  passBounds.roundOut();
  drawPass->fBounds = SkIRect::MakeLTRB(
      (int)passBounds.left(), (int)passBounds.top(), (int)passBounds.right(),
      (int)passBounds.bot());
  return drawPass;
}

bool DrawPass::prepareResources(
    ResourceProvider* resourceProvider, const RenderPassDesc& renderPassDesc) {
  fFullPipelines.reserve(fPipelineDescs.count());
  for (const GraphicsPipelineDesc& pipelineDesc : fPipelineDescs.items()) {
    auto pipeline = resourceProvider->findOrCreateGraphicsPipeline(pipelineDesc, renderPassDesc);
    if (!pipeline) {
  ...
  for (size_t i = 0; i < fSamplerDescs.size(); ++i) {
    sk_sp<Sampler> sampler = resourceProvider->findOrCreateCompatibleSampler(
        fSamplerDescs[i].fSamplingOptions, fSamplerDescs[i].fTileModes[0],
    ...
    switch (c.fType) {
      ...
      case CommandType::kBindTexturesAndSamplers: {
        ...
        for (int i = 0; i < d.fNumTexSamplers; ++i) {
          ...
          buffer->bindTextureAndSampler(
              fSampledTextures[d.fTextureIndices[i]]->refTexture(), fSamplers[d.fSamplerIndices[i]],
              i);
        }
      ...
      case CommandType::kDrawInstanced: {
        ...
        buffer->drawInstanced(
            d.fType, d.fBaseVertex, d.fVertexCount, d.fBaseInstance, d.fInstanceCount);
        break;
      }
      case CommandType::kDrawIndexedInstanced: {
        auto& d = c.fDrawIndexedInstanced;
        buffer->drawIndexedInstanced(
            d.fType, d.fBaseIndex, d.fIndexCount, d.fBaseVertex, d.fBaseInstance, d.fInstanceCount);
        break;
      }
      case CommandType::kSetScissor: {
        auto& d = c.fSetScissor;
        buffer->setScissor(
            d.fScissor.fLeft, d.fScissor.fTop, d.fScissor.width(), d.fScissor.height());
        break;
      }
    }
  }
