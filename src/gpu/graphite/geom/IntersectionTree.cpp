...
class IntersectionTree::TreeNode final : public Node {
 public:
  TreeNode(float splitCoord, Node* lo, Node* hi) : fSplitCoord(splitCoord), fLo(lo), fHi(hi) {}
...
class IntersectionTree::LeafNode final : public Node {
 ...
 private:
  ...
  IntersectionTree::Node* split(SkArenaAlloc* arena) {
    ...
    SplitType splitType = (splittableSize.x() > splittableSize.y()) ? SplitType::kX : SplitType::kY;
...
IntersectionTree::IntersectionTree() : fRoot(fArena.make<LeafNode>()) {
