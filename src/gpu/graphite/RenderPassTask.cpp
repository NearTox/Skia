...
sk_sp<RenderPassTask> RenderPassTask::Make(
    std::vector<std::unique_ptr<DrawPass>> passes, const RenderPassDesc& desc,
...
bool RenderPassTask::prepareResources(ResourceProvider* resourceProvider) {
  ...
  if (!fTarget->instantiate(resourceProvider)) {
    ...
    SKGPU_LOG_W(
        "Dimensions are (%d, %d).", fTarget->dimensions().width(), fTarget->dimensions().height());
    return false;
  }
...
bool RenderPassTask::addCommands(ResourceProvider* resourceProvider, CommandBuffer* commandBuffer) {
  ...
  if (commandBuffer->beginRenderPass(
          fRenderPassDesc, std::move(colorAttachment), std::move(resolveAttachment),
