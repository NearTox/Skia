...
Recorder* Surface::onGetRecorder() { return fDevice->recorder(); }
...
sk_sp<SkImage> Surface::onNewImageSnapshot(const SkIRect* subset) {
  SkImageInfo ii = subset ? this->imageInfo().makeDimensions(subset->size()) : this->imageInfo();
...
bool Surface::onReadPixels(
    Context* context, Recorder* recorder, const SkPixmap& dst, int srcX, int srcY) {
...
GrSemaphoresSubmitted Surface::onFlush(
    BackendSurfaceAccess, const GrFlushInfo&, const GrBackendSurfaceMutableState*) {
