...
UploadInstance::UploadInstance(
    const Buffer* buffer, sk_sp<TextureProxy> textureProxy,
...
size_t compute_combined_buffer_size(
    int mipLevelCount, size_t bytesPerPixel, size_t minTransferBufferAlignment,
    const SkISize& baseDimensions, SkTArray<size_t>* individualMipOffsets) {
...
UploadInstance UploadInstance::Make(
    Recorder* recorder, sk_sp<TextureProxy> textureProxy, SkColorType dataColorType,
  ...
  SkASSERT(caps->areColorTypeAndTextureInfoCompatible(dataColorType, textureProxy->textureInfo()));
  ...
  size_t combinedBufferSize = compute_combined_buffer_size(
      mipLevelCount, bpp, minAlignment, dstRect.size(), &individualMipOffsets);
  ...
  ATRACE_ANDROID_FRAMEWORK(
      "Upload %sTexture [%ux%u]", mipLevelCount > 1 ? "MipMap " : "", dstRect.width(),
      dstRect.height());
...
bool UploadList::recordUpload(
    Recorder* recorder, sk_sp<TextureProxy> textureProxy, SkColorType dataColorType,
    const std::vector<MipLevel>& levels, const SkIRect& dstRect) {
  UploadInstance instance =
      UploadInstance::Make(recorder, std::move(textureProxy), dataColorType, levels, dstRect);
  if (!instance.isValid()) {
    return false;
  }
...
UploadTask::UploadTask(const UploadInstance& instance) { fInstances.push_back(instance); }
...
bool UploadTask::addCommands(ResourceProvider*, CommandBuffer* commandBuffer) {
  for (unsigned int i = 0; i < fInstances.size(); ++i) {
