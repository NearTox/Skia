...
class DrawWriter {
 public:
  ...
  DrawWriter(
      DrawDispatcher*, DrawBufferManager*, PrimitiveType type, size_t vertexStride,
  ...
  void drawInstanced(
      BindBufferInfo vertices, unsigned int vertexCount, BindBufferInfo instances,
      unsigned int instanceCount) {
    SkASSERT(vertexCount > 0);
    this->bindAndFlush(vertices, {}, instances, vertexCount, instanceCount);
  }
  void drawIndexedInstanced(
      BindBufferInfo vertices, BindBufferInfo indices, unsigned int indexCount,
 ...
 private:
  ...
  void setTemplate(
      BindBufferInfo vertices, BindBufferInfo indices, BindBufferInfo instances, int templateCount);
  // NOTE: bindAndFlush's templateCount is unsigned because dynamic index count instancing
  // isn't applicable.
  void bindAndFlush(
      BindBufferInfo vertices, BindBufferInfo indices, BindBufferInfo instances,
...
class DrawDispatcher {
 public:
  ...
  virtual void bindDrawBuffers(
      BindBufferInfo vertexAttribs, BindBufferInfo instanceAttribs, BindBufferInfo indices) = 0;
...
class DrawWriter::Instances : private DrawWriter::Appender {
 public:
  Instances(
      DrawWriter& w, BindBufferInfo vertices, BindBufferInfo indices, unsigned int vertexCount)
