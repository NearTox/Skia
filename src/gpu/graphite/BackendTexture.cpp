...
BackendTexture::BackendTexture(const BackendTexture& that) { *this = that; }
...
BackendTexture& BackendTexture::operator=(const BackendTexture& that) {
  ...
  switch (that.backend()) {
#ifdef SK_METAL
    case BackendApi::kMetal: fMtlTexture = that.fMtlTexture; break;
#endif
    default: SK_ABORT("Unsupport Backend");
  }
  return *this;
}
...
BackendTexture::BackendTexture(SkISize dimensions, MtlHandle mtlTexture)
    : fDimensions(dimensions), fInfo(MtlTextureInfo(mtlTexture)), fMtlTexture(mtlTexture) {}
