...
class Image final : public SkImage_Base {
 public:
  ...
  bool onReadPixels(
      ...
      int srcY, CachingHint) const override {
    return false;
  }
  // Temporary and only for testing purposes.
  // To be removed once asynchronous readback is working.
  bool testingOnly_ReadPixels(
      Context*, Recorder*, const SkImageInfo& dstInfo, void* dstPixels, size_t dstRowBytes,
      int srcX, int srcY);
  ...
  sk_sp<SkImage> onMakeSubset(const SkIRect&, GrDirectContext*) const override { return nullptr; }
  ...
  sk_sp<SkImage> onReinterpretColorSpace(sk_sp<SkColorSpace>) const override { return nullptr; }
 ...
 private:
  ...
  std::unique_ptr<GrFragmentProcessor> onAsFragmentProcessor(
      ...
      const SkRect* subset, const SkRect* domain) const override {
    return nullptr;
  }
  std::tuple<GrSurfaceProxyView, GrColorType> onAsView(
      GrRecordingContext*, GrMipmapped, GrImageTexGenPolicy policy) const override {
    return {};
  }
#endif
