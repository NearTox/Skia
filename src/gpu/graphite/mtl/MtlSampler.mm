...
MtlSampler::MtlSampler(const MtlGpu* gpu, sk_cfp<id<MTLSamplerState>> samplerState)
    : Sampler(gpu), fSamplerState(std::move(samplerState)) {}

static inline MTLSamplerAddressMode tile_mode_to_mtl_sampler_address(
    SkTileMode tileMode, const Caps& caps) {
  switch (tileMode) {
    case SkTileMode::kClamp: return MTLSamplerAddressModeClampToEdge;
    case SkTileMode::kRepeat: return MTLSamplerAddressModeRepeat;
    case SkTileMode::kMirror: return MTLSamplerAddressModeMirrorRepeat;
...
sk_sp<MtlSampler> MtlSampler::Make(
    const MtlGpu* gpu, const SkSamplingOptions& samplingOptions, SkTileMode xTileMode,
...
void MtlSampler::freeGpuData() { fSamplerState.reset(); }
