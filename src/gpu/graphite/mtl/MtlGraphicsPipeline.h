...
class MtlGraphicsPipeline final : public GraphicsPipeline {
 public:
  ...
  static sk_sp<MtlGraphicsPipeline> Make(
      MtlResourceProvider*, const MtlGpu*, const GraphicsPipelineDesc&, const RenderPassDesc&);
 ...
 private:
  MtlGraphicsPipeline(
      const skgpu::graphite::Gpu* gpu, sk_cfp<id<MTLRenderPipelineState>> pso,
      sk_cfp<id<MTLDepthStencilState>> dss, uint32_t refValue, size_t vertexStride,
