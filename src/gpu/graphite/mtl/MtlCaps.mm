...
MtlCaps::MtlCaps(const id<MTLDevice> device) : Caps() {
...
static constexpr MTLPixelFormat kMtlFormats[] = {
    ...
    MTLPixelFormatBGRA8Unorm, kMTLPixelFormatB5G6R5Unorm,

    MTLPixelFormatStencil8,   MTLPixelFormatDepth32Float, MTLPixelFormatDepth32Float_Stencil8,
...
size_t MtlCaps::GetFormatIndex(MTLPixelFormat pixelFormat) {
  static_assert(
      SK_ARRAY_COUNT(kMtlFormats) == MtlCaps::kNumMtlFormats,
...
void MtlCaps::initFormatTable() {
  ...
  this->setColorType(kAlpha_8_SkColorType, {MTLPixelFormatR8Unorm, MTLPixelFormatA8Unorm});
  if (@available(macOS 11.0, iOS 8.0, *)) {
    if (this->isApple()) {
...
TextureInfo MtlCaps::getDefaultSampledTextureInfo(
    SkColorType colorType, uint32_t levelCount, Protected, Renderable renderable) const {
...
TextureInfo MtlCaps::getDefaultDepthStencilTextureInfo(
    SkEnumBitMask<DepthStencilFlags> depthStencilType, uint32_t sampleCount, Protected) const {
  MtlTextureInfo info;
...
UniqueKey MtlCaps::makeGraphicsPipelineKey(
  ...
  UniqueKey pipelineKey;
  {
    ...
    UniqueKey::Builder builder(
        &pipelineKey, kGraphicsPipelineDomain, pipelineDescKey.size() + 1, "GraphicsPipeline");
...
void MtlCaps::buildKeyForTexture(
    SkISize dimensions, const TextureInfo& info, ResourceType type, Shareable shareable,
  ...
  builder[4] =
      (samplesKey << 0) | (static_cast<uint32_t>(isMipped) << 3) |
      (static_cast<uint32_t>(isProtected) << 4) | (static_cast<uint32_t>(mtlSpec.fUsage) << 5) |
      (static_cast<uint32_t>(mtlSpec.fStorageMode) << 10) | (static_cast<uint32_t>(isFBOnly) << 12);
}
