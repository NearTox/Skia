...
bool MtlCommandBuffer::onBeginRenderPass(
    const RenderPassDesc& renderPassDesc, const Texture* colorTexture,
  ...
  const static MTLStoreAction mtlStoreAction[]{MTLStoreActionStore, MTLStoreActionDontCare};
  ...
  if (depthStencilTexture) {
    ...
    if (MtlFormatIsDepth(mtlTexture.pixelFormat)) {
      ...
      depthAttachment.loadAction = mtlLoadAction[static_cast<int>(depthStencilInfo.fLoadOp)];
      depthAttachment.storeAction = mtlStoreAction[static_cast<int>(depthStencilInfo.fStoreOp)];
    }
    if (MtlFormatIsStencil(mtlTexture.pixelFormat)) {
      ...
      stencilAttachment.loadAction = mtlLoadAction[static_cast<int>(depthStencilInfo.fLoadOp)];
      stencilAttachment.storeAction = mtlStoreAction[static_cast<int>(depthStencilInfo.fStoreOp)];
    }
  } else {
  ...
  fActiveRenderCommandEncoder =
      MtlRenderCommandEncoder::Make(fGpu, fCommandBuffer.get(), descriptor.get());
...
void MtlCommandBuffer::onBindUniformBuffer(
    UniformSlot slot, const Buffer* uniformBuffer, size_t uniformOffset) {
  SkASSERT(fActiveRenderCommandEncoder);

  id<MTLBuffer> mtlBuffer =
      uniformBuffer ? static_cast<const MtlBuffer*>(uniformBuffer)->mtlBuffer() : nullptr;
  ...
  switch (slot) {
    case UniformSlot::kRenderStep:
      ...
      break;
    case UniformSlot::kPaint: bufferIndex = MtlGraphicsPipeline::kPaintUniformBufferIndex; break;
  }
...
void MtlCommandBuffer::onBindVertexBuffers(
    const Buffer* vertexBuffer, size_t vertexOffset, const Buffer* instanceBuffer,
  ...
  if (vertexBuffer) {
    ...
    fActiveRenderCommandEncoder->setVertexBuffer(
        mtlBuffer, vertexOffset, MtlGraphicsPipeline::kVertexBufferIndex);
  }
  if (instanceBuffer) {
    id<MTLBuffer> mtlBuffer = static_cast<const MtlBuffer*>(instanceBuffer)->mtlBuffer();
    SkASSERT((instanceOffset & 0b11) == 0);
    fActiveRenderCommandEncoder->setVertexBuffer(
        mtlBuffer, instanceOffset, MtlGraphicsPipeline::kInstanceBufferIndex);
  }
}
...
void MtlCommandBuffer::onBindTextureAndSampler(
    sk_sp<Texture> texture, sk_sp<Sampler> sampler, unsigned int bindIndex) {
...
void MtlCommandBuffer::onSetScissor(
...
void MtlCommandBuffer::onSetViewport(
  ...
  fActiveRenderCommandEncoder->setVertexBytes(
      rtAdjust, 4 * sizeof(float), MtlGraphicsPipeline::kIntrinsicUniformBufferIndex);
}
...
void MtlCommandBuffer::onDrawIndexed(
  ...
  if (@available(macOS 10.11, iOS 9.0, *)) {
    ...
    fActiveRenderCommandEncoder->drawIndexedPrimitives(
        mtlPrimitiveType, indexCount, MTLIndexTypeUInt16, fCurrentIndexBuffer, indexOffset, 1,
        baseVertex, 0);
...
void MtlCommandBuffer::onDrawInstanced(
    PrimitiveType type, unsigned int baseVertex, unsigned int vertexCount,
  ...
  fActiveRenderCommandEncoder->drawPrimitives(
      mtlPrimitiveType, baseVertex, vertexCount, instanceCount, baseInstance);
}

void MtlCommandBuffer::onDrawIndexedInstanced(
    PrimitiveType type, unsigned int baseIndex, unsigned int indexCount, unsigned int baseVertex,
    unsigned int baseInstance, unsigned int instanceCount) {
  SkASSERT(fActiveRenderCommandEncoder);
  ...
  if (@available(macOS 10.11, iOS 9.0, *)) {
    ...
    size_t indexOffset = fCurrentIndexBufferOffset + sizeof(uint16_t) * baseIndex;
    fActiveRenderCommandEncoder->drawIndexedPrimitives(
        mtlPrimitiveType, indexCount, MTLIndexTypeUInt16, fCurrentIndexBuffer, indexOffset,
...
bool MtlCommandBuffer::onCopyTextureToBuffer(
    const Texture* texture, SkIRect srcRect, const Buffer* buffer, size_t bufferOffset,
...
bool MtlCommandBuffer::onCopyBufferToTexture(
    const Buffer* buffer, const Texture* texture, const BufferTextureCopyData* copyData,
    int count) {
  SkASSERT(!fActiveRenderCommandEncoder);
  ...
  for (int i = 0; i < count; ++i) {
    ...
    blitCmdEncoder->copyFromBuffer(
        mtlBuffer, copyData[i].fBufferOffset, copyData[i].fBufferRowBytes, mtlTexture,
        copyData[i].fRect, copyData[i].fMipLevel);
  }
