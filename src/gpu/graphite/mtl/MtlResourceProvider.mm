...
sk_sp<GraphicsPipeline> MtlResourceProvider::onCreateGraphicsPipeline(
  ...
  return MtlGraphicsPipeline::Make(this, this->mtlGpu(), pipelineDesc, renderPassDesc);
}

sk_sp<Texture> MtlResourceProvider::createTexture(
    SkISize dimensions, const TextureInfo& info, SkBudgeted budgeted) {
...
sk_sp<Texture> MtlResourceProvider::createWrappedTexture(const BackendTexture& texture) {
  ...
  return MtlTexture::MakeWrapped(
      this->mtlGpu(), texture.dimensions(), texture.info(), std::move(mtlTexture));
}

sk_sp<Buffer> MtlResourceProvider::createBuffer(
    size_t size, BufferType type, PrioritizeGpuReads prioritizeGpuReads) {
  return MtlBuffer::Make(this->mtlGpu(), size, type, prioritizeGpuReads);
}

sk_sp<Sampler> MtlResourceProvider::createSampler(
    const SkSamplingOptions& samplingOptions, SkTileMode xTileMode, SkTileMode yTileMode) {
...
MTLCompareFunction compare_op_to_mtl(CompareOp op) {
  switch (op) {
    ...
    case CompareOp::kGEqual: return MTLCompareFunctionGreaterEqual;
    case CompareOp::kLess: return MTLCompareFunctionLess;
    case CompareOp::kLEqual: return MTLCompareFunctionLessEqual;
...
MTLStencilOperation stencil_op_to_mtl(StencilOp op) {
  switch (op) {
    ...
    case StencilOp::kIncClamp: return MTLStencilOperationIncrementClamp;
    case StencilOp::kDecClamp: return MTLStencilOperationDecrementClamp;
  }
}
...
sk_cfp<id<MTLDepthStencilState>> MtlResourceProvider::findOrCreateCompatibleDepthStencilState(
  ...
  if (!depthStencilState) {
    ...
    SkASSERT(
        depthStencilSettings.fDepthTestEnabled ||
