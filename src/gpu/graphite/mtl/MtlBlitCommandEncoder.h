...
class MtlBlitCommandEncoder : public Resource {
 public:
  static sk_sp<MtlBlitCommandEncoder> Make(
      const skgpu::graphite::Gpu* gpu, id<MTLCommandBuffer> commandBuffer) {
  ...
  void copyFromTexture(
      id<MTLTexture> texture, SkIRect srcRect, id<MTLBuffer> buffer, size_t bufferOffset,
  ...
  void copyFromBuffer(
      id<MTLBuffer> buffer, size_t bufferOffset, size_t bufferRowBytes, id<MTLTexture> texture,
      SkIRect dstRect, unsigned int dstLevel) {
 ...
 private:
  MtlBlitCommandEncoder(const skgpu::graphite::Gpu* gpu, sk_cfp<id<MTLBlitCommandEncoder>> encoder)
