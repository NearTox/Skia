...
class MtlCommandBuffer final : public CommandBuffer {
 public:
  ...
  void waitUntilFinished() {
    ...
    if (!this->isFinished()) {
      SKGPU_LOG_E("Unfinished command buffer status: %d", (int)(*fCommandBuffer).status);
      SkASSERT(false);
    }
  }
  bool commit();
 ...
 private:
  ...
  void onBindVertexBuffers(
      const Buffer* vertexBuffer, size_t vertexOffset, const Buffer* instanceBuffer,
      size_t instanceOffset) override;
  void onBindIndexBuffer(const Buffer* indexBuffer, size_t offset) override;

  void onBindTextureAndSampler(sk_sp<Texture>, sk_sp<Sampler>, unsigned int bindIndex) override;

  void onSetScissor(
      unsigned int left, unsigned int top, unsigned int width, unsigned int height) override;
  void onSetViewport(
      float x, float y, float width, float height, float minDepth, float maxDepth) override;
  ...
  bool onCopyBufferToTexture(
      const Buffer*, const Texture*, const BufferTextureCopyData* copyData, int count) override;
