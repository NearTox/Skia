...
class MtlRenderCommandEncoder : public Resource {
 public:
  static sk_sp<MtlRenderCommandEncoder> Make(
      const Gpu* gpu, id<MTLCommandBuffer> commandBuffer, MTLRenderPassDescriptor* descriptor) {
  ...
  void setFragmentSamplerState(id<MTLSamplerState> sampler, NSUInteger index) {
    ...
    if (fCurrentSampler[index] != sampler) {
      [(*fCommandEncoder) setFragmentSamplerState:sampler atIndex:index];
 ...
 private:
  ...
  MtlRenderCommandEncoder(const Gpu* gpu, sk_cfp<id<MTLRenderCommandEncoder>> encoder)
      : Resource(gpu, Ownership::kOwned, SkBudgeted::kYes), fCommandEncoder(std::move(encoder)) {
