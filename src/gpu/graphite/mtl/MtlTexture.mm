...
sk_cfp<id<MTLTexture>> MtlTexture::MakeMtlTexture(
    const MtlGpu* gpu, SkISize dimensions, const TextureInfo& info) {
  const skgpu::graphite::Caps* caps = gpu->caps();
  if (dimensions.width() > caps->maxTextureSize() || dimensions.height() > caps->maxTextureSize()) {
    return nullptr;
  }
...
MtlTexture::MtlTexture(
    const MtlGpu* gpu, SkISize dimensions, const TextureInfo& info, sk_cfp<id<MTLTexture>> texture,
    Ownership ownership, SkBudgeted budgeted)
    : Texture(gpu, dimensions, info, ownership, budgeted), fTexture(std::move(texture)) {}

sk_sp<Texture> MtlTexture::Make(
    const MtlGpu* gpu, SkISize dimensions, const TextureInfo& info, SkBudgeted budgeted) {
  sk_cfp<id<MTLTexture>> texture = MakeMtlTexture(gpu, dimensions, info);
  if (!texture) {
    return nullptr;
  }
  return sk_sp<Texture>(
      new MtlTexture(gpu, dimensions, info, std::move(texture), Ownership::kOwned, budgeted));
}

sk_sp<Texture> MtlTexture::MakeWrapped(
    const MtlGpu* gpu, SkISize dimensions, const TextureInfo& info,
    sk_cfp<id<MTLTexture>> texture) {
  return sk_sp<Texture>(new MtlTexture(
      gpu, dimensions, info, std::move(texture), Ownership::kWrapped, SkBudgeted::kNo));
}

void MtlTexture::freeGpuData() { fTexture.reset(); }
