...
sk_sp<skgpu::graphite::Gpu> MtlGpu::Make(const MtlBackendContext& context) {
  ...
  return sk_sp<skgpu::graphite::Gpu>(
      new MtlGpu(std::move(device), std::move(queue), std::move(caps)));
}

MtlGpu::MtlGpu(
    sk_cfp<id<MTLDevice>> device, sk_cfp<id<MTLCommandQueue>> queue, sk_sp<const MtlCaps> caps)
    : skgpu::graphite::Gpu(std::move(caps)), fDevice(std::move(device)), fQueue(std::move(queue)) {
  this->initCompiler();
}
...
std::unique_ptr<ResourceProvider> MtlGpu::makeResourceProvider(
  ...
  return std::unique_ptr<ResourceProvider>(
      new MtlResourceProvider(this, std::move(globalCache), singleOwner));
}

class WorkSubmission final : public GpuWorkSubmission {
 public:
  WorkSubmission(sk_sp<CommandBuffer> cmdBuffer) : GpuWorkSubmission(std::move(cmdBuffer)) {}
