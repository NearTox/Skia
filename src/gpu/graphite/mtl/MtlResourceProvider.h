...
class MtlResourceProvider final : public ResourceProvider {
 ...
 private:
  ...
  sk_sp<GraphicsPipeline> onCreateGraphicsPipeline(
      const GraphicsPipelineDesc&, const RenderPassDesc&) override;
  ...
  sk_sp<Sampler> createSampler(
      const SkSamplingOptions&, SkTileMode xTileMode, SkTileMode yTileMode) override;
