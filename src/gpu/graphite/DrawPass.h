...
class DrawPass {
 public:
  ...
  static std::unique_ptr<DrawPass> Make(
      Recorder*, std::unique_ptr<DrawList>, sk_sp<TextureProxy>, std::pair<LoadOp, StoreOp>,
      std::array<float, 4> clearColor, const BoundsManager* occlusionCuller);
 ...
 private:
  ...
  struct Command {
    ...
    explicit Command(Draw d) : fType(CommandType::kDraw), fDraw(d) {}
    explicit Command(DrawIndexed d) : fType(CommandType::kDrawIndexed), fDrawIndexed(d) {}
    explicit Command(DrawInstanced d) : fType(CommandType::kDrawInstanced), fDrawInstanced(d) {}
    explicit Command(DrawIndexedInstanced d)
        : fType(CommandType::kDrawIndexedInstanced), fDrawIndexedInstanced(d) {}
    explicit Command(SetScissor d) : fType(CommandType::kSetScissor), fSetScissor(d) {}
  };
