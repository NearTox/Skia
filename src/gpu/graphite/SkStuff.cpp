...
static bool validate_backend_texture(
    const Caps* caps, const BackendTexture& texture, SkColorType ct) {
  if (!texture.isValid()) {
    return false;
  }
...
sk_sp<SkSurface> MakeGraphiteFromBackendTexture(
    Recorder* recorder, const BackendTexture& beTexture, SkColorType colorType,
  ...
  sk_sp<Device> device = Device::Make(
      recorder, std::move(proxy), std::move(colorSpace), colorType, kPremul_SkAlphaType);
  if (!device) {
    return nullptr;
  }
