...
DrawWriter::DrawWriter(
    DrawDispatcher* dispatcher, DrawBufferManager* bufferManager, PrimitiveType primitiveType,
...
void DrawWriter::setTemplate(
  ...
  if (vertices != fVertices || instances != fInstances || fIndices != indices) {
    ...
    fPendingBufferBinds = true;
      ...
      (templateCount >= 0 && templateCount != fTemplateCount) ||  // vtx or reg. instances
...
void DrawWriter::flush() {
  ...
  if (fTemplateCount) {
    ...
    if (fIndices) {
      fDispatcher->drawIndexedInstanced(
          fPrimitiveType, 0, realVertexCount, 0, fPendingBase, fPendingCount);
    } else {
      fDispatcher->drawInstanced(fPrimitiveType, 0, realVertexCount, fPendingBase, fPendingCount);
    }
  } else {
    SkASSERT(!fInstances);
    if (fIndices) {
