...
SkIRect rect_to_pixelbounds(const Rect& r) { return r.makeRoundOut().asSkIRect(); }

// TODO: this doesn't support the SrcRectConstraint option.
sk_sp<SkShader> make_img_shader_for_paint(
    const SkPaint& paint, sk_sp<SkImage> image, const SkRect& subset, SkTileMode tmx,
...
class Device::IntersectionTreeSet {
 ...
 private:
  ...
  IntersectionTree* makeTree() { return fTreeStore.make<IntersectionTree>(); }
...
sk_sp<Device> Device::Make(Recorder* recorder, const SkImageInfo& ii, SkBudgeted budgeted) {
  ...
  auto textureInfo = recorder->priv().caps()->getDefaultSampledTextureInfo(
      ii.colorType(),
      /*levelCount=*/1, Protected::kNo, Renderable::kYes);
  sk_sp<TextureProxy> target(new TextureProxy(ii.dimensions(), textureInfo, budgeted));
  return Make(recorder, std::move(target), ii.refColorSpace(), ii.colorType(), ii.alphaType());
}

sk_sp<Device> Device::Make(
    Recorder* recorder, sk_sp<TextureProxy> target, sk_sp<SkColorSpace> colorSpace,
  ...
  sk_sp<DrawContext> dc =
      DrawContext::Make(std::move(target), std::move(colorSpace), colorType, alphaType);
  if (!dc) {
    return nullptr;
  }
...
void Device::abandonRecorder() { fRecorder = nullptr; }
...
bool Device::readPixels(
    Context* context, Recorder* recorder, const SkPixmap& pm, int srcX, int srcY) {
  return ReadPixelsHelper(
      [this]() { this->flushPendingWorkToRecorder(); }, context, recorder, fDC->target(), pm.info(),
      pm.writable_addr(), pm.rowBytes(), srcX, srcY);
}
...
bool Device::onWritePixels(const SkPixmap& src, int x, int y) {
  ...
  if (!caps->areColorTypeAndTextureInfoCompatible(src.colorType(), target->textureInfo())) {
    return false;
  }
...
SkIRect Device::onDevClipBounds() const { return rect_to_pixelbounds(fClip.conservativeBounds()); }
...
void Device::onClipShader(sk_sp<SkShader> shader) { fClip.clipShader(std::move(shader)); }
...
void Device::drawPaint(const SkPaint& paint) {
  ...
  this->drawShape(
      Shape(localCoveringBounds), paint, kFillStyle,
...
void Device::drawPoints(
  ...
  if (mode == SkCanvas::kPoints_PointMode) {
    ...
    for (size_t i = 0; i < count; ++i) {
      SkRect pointRect = SkRect::MakeLTRB(
          points[i].fX - radius, points[i].fY - radius, points[i].fX + radius,
          points[i].fY + radius);
...
void Device::drawImageRect(
  ...
  auto s = make_img_shader_for_paint(
      paint, sk_ref_sp(image), tmpSrc, SkTileMode::kClamp, SkTileMode::kClamp, sampling, &matrix);
  if (!s) {
    return;
  }
...
void Device::drawShape(
    const Shape& shape, const SkPaint& paint, const SkStrokeRec& style,
  ...
  if (!(flags & DrawFlags::kIgnorePathEffect) && paint.getPathEffect()) {
    ...
    SkPath dst;
    if (paint.getPathEffect()->filterPath(
            &dst, shape.asPath(), &newStyle, nullptr, localToDevice)) {
  ...
  CompressedPaintersOrder prevDraw = fColorDepthBoundsManager->getMostRecentDraw(clip.drawBounds());
  if (dependsOnDst) {
    order.dependsOnPaintersOrder(prevDraw);
  }

  if (styleType == SkStrokeRec::kStroke_Style || styleType == SkStrokeRec::kHairline_Style ||
  ...
  const bool fullyOpaque = !dependsOnDst && clipOrder == DrawOrder::kNoIntersection &&
                           shape.isRect() &&
                           localToDevice.type() <= Transform::Type::kRectStaysRect;
  fColorDepthBoundsManager->recordDraw(
      clip.drawBounds(), order.paintOrder(), order.depth(), fullyOpaque);
...
void Device::recordDraw(
  ...
  if (renderer->depthStencilFlags() & DepthStencilFlags::kStencil) {
    DisjointStencilIndex setIndex =
        fDisjointStencilSet->add(ordering.paintOrder(), clip.drawBounds());
...
sk_sp<SkSpecialImage> Device::makeSpecial(const SkBitmap&) { return nullptr; }

sk_sp<SkSpecialImage> Device::makeSpecial(const SkImage*) { return nullptr; }
...
TextureProxy* Device::proxy() { return fDC->target(); }
