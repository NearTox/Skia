...
sk_sp<DrawContext> DrawContext::Make(
    sk_sp<TextureProxy> target, sk_sp<SkColorSpace> colorSpace, SkColorType colorType,
  ...
  SkImageInfo imageInfo =
      SkImageInfo::Make(target->dimensions(), colorType, alphaType, std::move(colorSpace));
...
void DrawContext::recordDraw(
    const Renderer& renderer, const Transform& localToDevice, const Shape& shape, const Clip& clip,
    DrawOrder ordering, const PaintParams* paint, const StrokeStyle* stroke) {
...
bool DrawContext::recordUpload(
    Recorder* recorder, sk_sp<TextureProxy> targetProxy, SkColorType colorType,
    const std::vector<MipLevel>& levels, const SkIRect& dstRect) {
  ...
  return fPendingUploads->recordUpload(
      recorder, std::move(targetProxy), colorType, levels, dstRect);
}
...
void DrawContext::snapDrawPass(Recorder* recorder, const BoundsManager* occlusionCuller) {
  ...
  auto pass = DrawPass::Make(
      recorder, std::move(fPendingDraws), fTarget, std::make_pair(fPendingLoadOp, fPendingStoreOp),
      fPendingClearColor, occlusionCuller);
...
sk_sp<Task> DrawContext::snapRenderPassTask(
    Recorder* recorder, const BoundsManager* occlusionCuller) {
  ...
  if (drawPass->depthStencilFlags() != DepthStencilFlags::kNone) {
    desc.fDepthStencilAttachment.fTextureInfo = caps->getDefaultDepthStencilTextureInfo(
        drawPass->depthStencilFlags(), desc.fColorAttachment.fTextureInfo.numSamples(),
        Protected::kNo);
