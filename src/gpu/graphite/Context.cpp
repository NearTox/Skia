...
Context::Context(sk_sp<Gpu> gpu, BackendApi backend)
    : fGpu(std::move(gpu)), fGlobalCache(sk_make_sp<GlobalCache>()), fBackend(backend) {}
Context::~Context() {}
...
void Context::checkAsyncWorkCompletion() { fGpu->checkForFinishedWork(SyncToCpu::kNo); }
...
void Context::precompile(SkCombinationBuilder* combinationBuilder) {
  ...
  combinationBuilder->buildCombinations(dict, [&](SkUniquePaintParamsID uniqueID) {
