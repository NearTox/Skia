...
class CommandBuffer : public SkRefCnt, private DrawDispatcher {
 public:
  ...
  bool beginRenderPass(
      const RenderPassDesc&, sk_sp<Texture> colorTexture, sk_sp<Texture> resolveTexture,
  ...
  void bindDrawBuffers(
      BindBufferInfo vertices, BindBufferInfo instances, BindBufferInfo indices) final;
  ...
  void drawIndexedInstanced(
      ...
      unsigned int baseInstance, unsigned int instanceCount) final {
    this->onDrawIndexedInstanced(
        type, baseIndex, indexCount, baseVertex, baseInstance, instanceCount);
    SkDEBUGCODE(fHasWork = true;)
  }
  ...
  bool copyTextureToBuffer(
      sk_sp<Texture>, SkIRect srcRect, sk_sp<Buffer>, size_t bufferOffset, size_t bufferRowBytes);
 ...
 private:
  ...
  void bindVertexBuffers(
      sk_sp<Buffer> vertexBuffer, size_t vertexOffset, sk_sp<Buffer> instanceBuffer,
      size_t instanceOffset);
  void bindIndexBuffer(sk_sp<Buffer> indexBuffer, size_t bufferOffset);

  virtual bool onBeginRenderPass(
      const RenderPassDesc&, const Texture* colorTexture, const Texture* resolveTexture,
  ...
  virtual void onBindVertexBuffers(
  ...
  virtual void onBindTextureAndSampler(sk_sp<Texture>, sk_sp<Sampler>, unsigned int bindIndex) = 0;

  virtual void onSetScissor(
      unsigned int left, unsigned int top, unsigned int width, unsigned int height) = 0;
  virtual void onSetViewport(
      float x, float y, float width, float height, float minDepth, float maxDepth) = 0;
  ...
  virtual bool onCopyTextureToBuffer(
      const Texture*, SkIRect srcRect, const Buffer*, size_t bufferOffset,
      size_t bufferRowBytes) = 0;
  virtual bool onCopyBufferToTexture(
      const Buffer*, const Texture*, const BufferTextureCopyData*, int count) = 0;
