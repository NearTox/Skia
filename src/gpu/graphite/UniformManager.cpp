...
class Writer {
 private:
  ...
  static void CopyUniforms(void* dst, const void* src, int numUniforms) {
    ...
    if constexpr (std::is_same<MemType, float>::value && std::is_same<UniformType, SkHalf>::value) {
  ...
  static uint32_t WriteSkMatrices(void* dst, int n, const SkMatrix m[]) {
    ...
    if (dst) {
      ...
      for (int i = 0; i < n; ++i) {
        float mt[] = {
            m[i].get(SkMatrix::kMScaleX), m[i].get(SkMatrix::kMSkewY),
            m[i].get(SkMatrix::kMPersp0), m[i].get(SkMatrix::kMSkewX),
            m[i].get(SkMatrix::kMScaleY), m[i].get(SkMatrix::kMPersp1),
            m[i].get(SkMatrix::kMTransX), m[i].get(SkMatrix::kMTransY),
 ...
 public:
  static uint32_t WriteUniform(SkSLType type, CType ctype, void* dest, int n, const void* src) {
    ...
    switch (type) {
      ...
      case SkSLType::kHalf: return Write<float, SkHalf>(dest, n, static_cast<const float*>(src));

      case SkSLType::kFloat: return Write<float, float>(dest, n, static_cast<const float*>(src));
      ...
      default: SK_ABORT("Unexpected uniform type");
    }
  }
};
...
static uint32_t sksltype_to_alignment_mask(SkSLType type) {
  switch (type) {
    ...
    case SkSLType::kHalf: return 0x1;
    case SkSLType::kShort2:
    case SkSLType::kUShort2:
    case SkSLType::kHalf2: return 0x3;
    case SkSLType::kShort3:
    case SkSLType::kShort4:
    case SkSLType::kUShort3:
    case SkSLType::kUShort4:
    case SkSLType::kHalf3:
    case SkSLType::kHalf4: return 0x7;

    case SkSLType::kHalf2x2: return 0x3;
    case SkSLType::kHalf3x3: return 0x7;
    case SkSLType::kHalf4x4: return 0x7;
...
inline uint32_t sksltype_to_mtl_size(SkSLType type) {
  switch (type) {
    case SkSLType::kInt:
    case SkSLType::kUInt:
    case SkSLType::kFloat: return 4;
    case SkSLType::kInt2:
    case SkSLType::kUInt2:
    case SkSLType::kFloat2: return 8;
    case SkSLType::kInt3:
    case SkSLType::kUInt3:
    case SkSLType::kFloat3:
    case SkSLType::kInt4:
    case SkSLType::kUInt4:
    case SkSLType::kFloat4: return 16;

    case SkSLType::kFloat2x2: return 16;
    case SkSLType::kFloat3x3: return 48;
    case SkSLType::kFloat4x4: return 64;

    case SkSLType::kShort:
    case SkSLType::kUShort:
    case SkSLType::kHalf: return 2;
    case SkSLType::kShort2:
    case SkSLType::kUShort2:
    case SkSLType::kHalf2: return 4;
    case SkSLType::kShort3:
    case SkSLType::kShort4:
    case SkSLType::kUShort3:
    case SkSLType::kUShort4:
    case SkSLType::kHalf3:
    case SkSLType::kHalf4: return 8;

    case SkSLType::kHalf2x2: return 8;
    case SkSLType::kHalf3x3: return 24;
    case SkSLType::kHalf4x4: return 32;
...
static uint32_t get_ubo_aligned_offset(
    uint32_t* currentOffset, uint32_t* maxAlignment, SkSLType type, int arrayCount) {
...
UniformManager::UniformManager(Layout layout) : fLayout(layout) {
  switch (layout) {
    case Layout::kStd140: fWriteUniform = Writer<Rules140>::WriteUniform; break;
    case Layout::kStd430: fWriteUniform = Writer<Rules430>::WriteUniform; break;
    case Layout::kMetal: fWriteUniform = Writer<RulesMetal>::WriteUniform; break;
  }
...
void UniformManager::checkExpected(SkSLType type, unsigned int count) {
  ...
  SkASSERT(
      (fExpectedUniforms[fExpectedUniformIndex].count() == 0 && count == 1) ||
  ...
  uint32_t debugOffset =
      get_ubo_aligned_offset(&fCurUBOOffset, &fCurUBOMaxAlignment, revisedType, count);
