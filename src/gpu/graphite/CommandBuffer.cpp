...
CommandBuffer::~CommandBuffer() { this->releaseResources(); }
...
bool CommandBuffer::beginRenderPass(
    const RenderPassDesc& renderPassDesc, sk_sp<Texture> colorTexture,
    sk_sp<Texture> resolveTexture, sk_sp<Texture> depthStencilTexture) {
  if (!this->onBeginRenderPass(
          renderPassDesc, colorTexture.get(), resolveTexture.get(), depthStencilTexture.get())) {
    return false;
  }
...
void CommandBuffer::bindUniformBuffer(
    UniformSlot slot, sk_sp<Buffer> uniformBuffer, size_t offset) {
...
void CommandBuffer::bindVertexBuffers(
  ...
  this->onBindVertexBuffers(vertexBuffer.get(), vertexOffset, instanceBuffer.get(), instanceOffset);
  if (vertexBuffer) {
...
void CommandBuffer::bindDrawBuffers(
    BindBufferInfo vertices, BindBufferInfo instances, BindBufferInfo indices) {
  this->bindVertexBuffers(
      sk_ref_sp(vertices.fBuffer), vertices.fOffset, sk_ref_sp(instances.fBuffer),
      instances.fOffset);
  this->bindIndexBuffer(sk_ref_sp(indices.fBuffer), indices.fOffset);
}

void CommandBuffer::bindTextureAndSampler(
    sk_sp<Texture> texture, sk_sp<Sampler> sampler, int bindIndex) {
...
bool CommandBuffer::copyTextureToBuffer(
    sk_sp<Texture> texture, SkIRect srcRect, sk_sp<Buffer> buffer, size_t bufferOffset,
    size_t bufferRowBytes) {
  SkASSERT(texture);
  SkASSERT(buffer);

  if (!this->onCopyTextureToBuffer(
          texture.get(), srcRect, buffer.get(), bufferOffset, bufferRowBytes)) {
    return false;
  }
...
bool CommandBuffer::copyBufferToTexture(
    const Buffer* buffer, sk_sp<Texture> texture, const BufferTextureCopyData* copyData,
    int count) {
  SkASSERT(buffer);
  SkASSERT(texture);
