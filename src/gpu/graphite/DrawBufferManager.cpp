...
void* map_offset(BindBufferInfo binding) {
  ...
  return SkTAddOffset<void>(
      const_cast<Buffer*>(binding.fBuffer)->map(), static_cast<ptrdiff_t>(binding.fOffset));
}
...
DrawBufferManager::DrawBufferManager(
    ResourceProvider* resourceProvider, size_t uniformStartAlignment)
    : fResourceProvider(resourceProvider), fUniformStartAlignment(uniformStartAlignment) {}

DrawBufferManager::~DrawBufferManager() {}

static bool can_fit(size_t requestedSize, Buffer* buffer, size_t currentOffset, size_t alignment) {
...
std::tuple<VertexWriter, BindBufferInfo> DrawBufferManager::getVertexWriter(size_t requiredBytes) {
  ...
  if (!fCurrentVertexBuffer) {
    ...
    fCurrentVertexBuffer = fResourceProvider->findOrCreateBuffer(
        bufferSize, BufferType::kVertex, PrioritizeGpuReads::kNo);
    fVertexOffset = 0;
    if (!fCurrentVertexBuffer) {
      return {VertexWriter(), BindBufferInfo()};
    }
...
std::tuple<IndexWriter, BindBufferInfo> DrawBufferManager::getIndexWriter(size_t requiredBytes) {
  ...
  if (!fCurrentIndexBuffer) {
    ...
    fCurrentIndexBuffer = fResourceProvider->findOrCreateBuffer(
        bufferSize, BufferType::kIndex, PrioritizeGpuReads::kNo);
    fIndexOffset = 0;
    if (!fCurrentIndexBuffer) {
      return {IndexWriter(), BindBufferInfo()};
    }
...
std::tuple<UniformWriter, BindBufferInfo> DrawBufferManager::getUniformWriter(
  ...
  if (fCurrentUniformBuffer &&
      !can_fit(
          requiredBytes, fCurrentUniformBuffer.get(), fUniformOffset, fUniformStartAlignment)) {
  ...
  if (!fCurrentUniformBuffer) {
    ...
    fCurrentUniformBuffer = fResourceProvider->findOrCreateBuffer(
        bufferSize, BufferType::kUniform, PrioritizeGpuReads::kNo);
    fUniformOffset = 0;
    if (!fCurrentUniformBuffer) {
      return {UniformWriter(), BindBufferInfo()};
    }
...
BindBufferInfo DrawBufferManager::getStaticBuffer(
    BufferType type, InitializeBufferFn initFn, BufferSizeFn sizeFn) {
