 ...
 public:
  ...
  StrokeStyle(float width, float miterLimit, SkPaint::Join join, SkPaint::Cap cap)
      : fHalfWidth(std::max(0.f, 0.5f * width)),
        fJoinLimit(
            join == SkPaint::kMiter_Join ? std::max(0.f, miterLimit)
...
class Clip {
 public:
  Clip(const Rect& drawBounds, const SkIRect& scissor)
      : fDrawBounds(drawBounds), fScissor(scissor) {}
...
class DrawGeometry {
 public:
  DrawGeometry(
      const Transform& transform, const Shape& shape, const Clip& clip, DrawOrder drawOrder,
