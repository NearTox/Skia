...
TextureInfo& TextureInfo::operator=(const TextureInfo& that) {
  ...
  switch (that.backend()) {
#ifdef SK_METAL
    case BackendApi::kMetal: fMtlSpec = that.fMtlSpec; break;
#endif
...
bool TextureInfo::operator==(const TextureInfo& that) const {
  ...
  if (fSampleCount != that.fSampleCount || fLevelCount != that.fLevelCount ||
      fProtected != that.fProtected) {
    return false;
  }
  ...
  switch (fBackend) {
#ifdef SK_METAL
    case BackendApi::kMetal: return fMtlSpec == that.fMtlSpec;
#endif
