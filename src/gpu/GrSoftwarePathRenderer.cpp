...
// is no intersection.
bool GrSoftwarePathRenderer::GetShapeAndClipBounds(
...
void GrSoftwarePathRenderer::DrawNonAARect(
...
void GrSoftwarePathRenderer::DrawAroundInvPath(
  ...
  if (devClipBounds.fTop < devPathBounds.fTop) {
    ...
    DrawNonAARect(
        surfaceDrawContext, GrPaint::Clone(paint), userStencilSettings, clip, SkMatrix::I(), rect,
        invert);
  }
  ...
  if (devClipBounds.fBottom > devPathBounds.fBottom) {
    ...
    DrawNonAARect(
        surfaceDrawContext, std::move(paint), userStencilSettings, clip, SkMatrix::I(), rect,
        invert);
  }
}

void GrSoftwarePathRenderer::DrawToTargetWithShapeMask(
    GrSurfaceProxyView view, GrSurfaceDrawContext* surfaceDrawContext, GrPaint&& paint,
    const GrUserStencilSettings& userStencilSettings, const GrClip* clip,
  ...
  DrawNonAARect(
      surfaceDrawContext, std::move(paint), userStencilSettings, clip, SkMatrix::I(), dstRect,
      invert);
}

static GrSurfaceProxyView make_deferred_mask_texture_view(
    GrRecordingContext* context, SkBackingFit fit, SkISize dimensions) {
...
bool GrSoftwarePathRenderer::onDrawPath(const DrawPathArgs& args) {
  GR_AUDIT_TRAIL_AUTO_FRAME(
      args.fRenderTargetContext->auditTrail(), "GrSoftwarePathRenderer::onDrawPath");
  if (!fProxyProvider) {
    return false;
  }
  ...
  if (!GetShapeAndClipBounds(
          args.fRenderTargetContext, args.fClip, *args.fShape, *args.fViewMatrix,
          &unclippedDevShapeBounds, &clippedDevShapeBounds, &devClipBounds)) {
    if (inverseFilled) {
      DrawAroundInvPath(
          args.fRenderTargetContext, std::move(args.fPaint), *args.fUserStencilSettings, args.fClip,
  ...
  if (inverseFilled) {
    DrawAroundInvPath(
        args.fRenderTargetContext, GrPaint::Clone(args.fPaint), *args.fUserStencilSettings,
        args.fClip, *args.fViewMatrix, devClipBounds, unclippedDevShapeBounds);
  }
  DrawToTargetWithShapeMask(
      std::move(view), args.fRenderTargetContext, std::move(args.fPaint),
