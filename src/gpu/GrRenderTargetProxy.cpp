...
// Wrapped version
GrRenderTargetProxy::GrRenderTargetProxy(
    sk_sp<GrSurface> surf, GrSurfaceOrigin origin, WrapsVkSecondaryCB wrapsVkSecondaryCB)
...
bool GrRenderTargetProxy::instantiate(GrResourceProvider* resourceProvider) {
  ...
  if (!this->instantiateImpl(
          resourceProvider, fSampleCnt, fNeedsStencil, kDescFlags, GrMipMapped::kNo, nullptr)) {
    return false;
  }
...
sk_sp<GrSurface> GrRenderTargetProxy::createSurface(GrResourceProvider* resourceProvider) const {
  ...
  sk_sp<GrSurface> surface = this->createSurfaceImpl(
      resourceProvider, fSampleCnt, fNeedsStencil, kDescFlags, GrMipMapped::kNo);
  if (!surface) {
    return nullptr;
  }
...
size_t GrRenderTargetProxy::onUninstantiatedGpuMemorySize() const {
  ...
  return GrSurface::ComputeSize(
      this->config(), this->width(), this->height(), colorSamplesPerPixel, GrMipMapped::kNo,
      !this->priv().isExact());
}
