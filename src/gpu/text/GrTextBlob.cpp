...
void GrTextBlob::Run::appendPathGlyph(
  ...
  for (int runIndex = 0; runIndex <= lastRun; runIndex++) {
    ...
    if (run.fPathGlyphs.count()) {
      ...
      for (int i = 0; i < run.fPathGlyphs.count(); i++) {
        ...
        if (!pathGlyph.fPreTransformed) {
          ...
          // blob.
          pathMatrix.postTranslate(originShift.x() + pathGlyph.fX, originShift.y() + pathGlyph.fY);
          ...
          // new blob.
          ctm = SkMatrix::MakeTrans(originShift.x() + pathGlyph.fX, originShift.y() + pathGlyph.fY);
        }
    ...
    for (int subRun = 0; subRun <= lastSubRun; subRun++) {
      ...
      if (!info.drawAsDistanceFields() && !info.needsTransform() &&
        ...
        SkRect subRunBounds;
        this->computeSubRunBounds(&subRunBounds, runIndex, subRun, viewMatrix, x, y, false);
      ...
      if (submitOp) {
        auto op = this->makeOp(
            info, glyphCount, runIndex, subRun, viewMatrix, x, y, clipRect, paint, filteredColor,
            props, distanceAdjustTable, target);
        if (op) {
          if (skipClip) {
...
std::unique_ptr<GrDrawOp> GrTextBlob::test_makeOp(
    ...
    SkScalar y, const SkPaint& paint, const SkPMColor4f& filteredColor, const SkSurfaceProps& props,
    const GrDistanceFieldAdjustTable* distanceAdjustTable, GrTextTarget* target) {
  const GrTextBlob::SubRun& info = fRuns[run].fSubRunInfo[subRun];
  SkIRect emptyRect = SkIRect::MakeEmpty();
  return this->makeOp(
      info, glyphCount, run, subRun, viewMatrix, x, y, emptyRect, paint, filteredColor, props,
