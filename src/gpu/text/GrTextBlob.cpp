...
// -- GrTextBlob::Key ------------------------------------------------------------------------------
GrTextBlob::Key::Key() noexcept { sk_bzero(this, sizeof(Key)); }

bool GrTextBlob::Key::operator==(const GrTextBlob::Key& other) const noexcept {
...
GrTextBlob::SubRun::SubRun(
    SubRunType type, GrTextBlob* textBlob, const SkStrikeSpec& strikeSpec, GrMaskFormat format,
    SkRect vertexBounds, const SkSpan<VertexData>& vertexData)
    : fBlob{textBlob},
      fType{type},
      fMaskFormat{format},
...
void GrTextBlob::SubRun::resetBulkUseToken() noexcept { fBulkUseToken.reset(); }

GrDrawOpAtlas::BulkUseTokenUpdater* GrTextBlob::SubRun::bulkUseToken() noexcept {
  return &fBulkUseToken;
}
GrMaskFormat GrTextBlob::SubRun::maskFormat() const noexcept { return fMaskFormat; }
...
void GrTextBlob::SubRun::fillVertexData(
  ...
  auto direct2D = [&](auto dst, SkIRect* clip) {
    ...
    for (auto [quad, vertexData] : SkMakeZip(dst, fVertexData.subspan(offset, count))) {
      ...
      if (clip == nullptr) {
        SkScalar dx = SkScalarRoundToScalar(fx), dy = SkScalarRoundToScalar(fy);
        ...
        int dx = SkScalarRoundToInt(fx), dy = SkScalarRoundToInt(fy);
        ...
        if (!clip->containsNoEmptyCheck(devIRect)) {
          if (SkIRect clipped; clipped.intersect(devIRect, *clip)) {
            ...
            std::tie(tl, tt, indexLT) = GrDrawOpAtlas::UnpackIndexFromTexCoords(al, at);
            std::tie(tr, tb, indexRB) = GrDrawOpAtlas::UnpackIndexFromTexCoords(ar, ab);
            std::tie(tl, tt) = GrDrawOpAtlas::PackIndexInTexCoords(tl + lD, tt + tD, indexLT);
            std::tie(tr, tb) = GrDrawOpAtlas::PackIndexInTexCoords(tr + rD, tb + bD, indexRB);
          } else {
  ...
  switch (fType) {
    ...
    case kTransformedPath: SK_ABORT("Paths don't generate vertex data.");
  }
}

int GrTextBlob::SubRun::glyphCount() const noexcept { return fVertexData.count(); }

bool GrTextBlob::SubRun::drawAsDistanceFields() const noexcept { return fType == kTransformedSDFT; }

bool GrTextBlob::SubRun::drawAsPaths() const noexcept { return fType == kTransformedPath; }

bool GrTextBlob::SubRun::needsTransform() const noexcept {
  return fType == kTransformedPath || fType == kTransformedMask || fType == kTransformedSDFT;
}

bool GrTextBlob::SubRun::needsPadding() const noexcept {
  return fType == kTransformedPath || fType == kTransformedMask;
}

int GrTextBlob::SubRun::atlasPadding() const { return SkTo<int>(this->needsPadding()); }

auto GrTextBlob::SubRun::vertexData() const noexcept -> SkSpan<const VertexData> {
  return fVertexData;
}
...
GrGlyph* GrTextBlob::SubRun::grGlyph(int i) const noexcept { return fVertexData[i].glyph.grGlyph; }

void GrTextBlob::SubRun::setUseLCDText(bool useLCDText) noexcept { fUseLCDText = useLCDText; }
bool GrTextBlob::SubRun::hasUseLCDText() const noexcept { return fUseLCDText; }
void GrTextBlob::SubRun::setAntiAliased(bool antiAliased) noexcept { fAntiAliased = antiAliased; }
bool GrTextBlob::SubRun::isAntiAliased() const noexcept { return fAntiAliased; }
const SkStrikeSpec& GrTextBlob::SubRun::strikeSpec() const noexcept { return fStrikeSpec; }

auto GrTextBlob::SubRun::MakePaths(
    const SkZip<SkGlyphVariant, SkPoint>& drawables, const SkFont& runFont,
    const SkStrikeSpec& strikeSpec, GrTextBlob* blob, SkArenaAlloc* alloc) -> SubRun* {
...
auto GrTextBlob::SubRun::MakeSDFT(
    ...
    const SkStrikeSpec& strikeSpec, GrTextBlob* blob, SkArenaAlloc* alloc) -> SubRun* {
  SubRun* subRun =
      SubRun::InitForAtlas(kTransformedSDFT, drawables, strikeSpec, kA8_GrMaskFormat, blob, alloc);
...
auto GrTextBlob::SubRun::MakeDirectMask(
    ...
    GrMaskFormat format, GrTextBlob* blob, SkArenaAlloc* alloc) -> SubRun* {
...
void GrTextBlob::SubRun::insertSubRunOpsIntoTarget(
    GrTextTarget* target, const SkSurfaceProps& props, const SkPaint& paint, const GrClip* clip,
    const SkMatrixProvider& deviceMatrix, SkPoint drawOrigin) {
  if (this->drawAsPaths()) {
    ...
    if (!this->drawAsDistanceFields() && !this->needsTransform() &&
        (!clip || (clip->isRRect(&clipRRect, &aa) && clipRRect.isRect() && GrAA::kNo == aa))) {
...
std::unique_ptr<GrAtlasTextOp> GrTextBlob::SubRun::makeOp(
    const SkMatrixProvider& matrixProvider, SkPoint drawOrigin, const SkIRect& clipRect,
    const SkPaint& paint, const SkSurfaceProps& props, GrTextTarget* target) {
  GrPaint grPaint;
  ...
  if (this->drawAsDistanceFields()) {
    ...
    return GrAtlasTextOp::MakeDistanceField(
        target->getContext(), std::move(grPaint), this, matrixProvider.localToDevice(), drawOrigin,
        clipRect, drawingColor, target->colorInfo().isLinearlyBlended(),
        SkPaintPriv::ComputeLuminanceColor(paint), props);
  } else {
    return GrAtlasTextOp::MakeBitmap(
        target->getContext(), std::move(grPaint), this, matrixProvider.localToDevice(), drawOrigin,
        clipRect, drawingColor);
  }
}

auto GrTextBlob::SubRun::InitForAtlas(
    SubRunType type, const SkZip<SkGlyphVariant, SkPoint>& drawables,
    const SkStrikeSpec& strikeSpec, GrMaskFormat format, GrTextBlob* blob, SkArenaAlloc* alloc)
    -> SubRun* {
  ...
  SkSpan<Data> vertexData{alloc->makeInitializedArray<Data>(vertexCount, initializer), vertexCount};
...
bool GrTextBlob::hasDistanceField() const noexcept {
  return SkToBool(fTextType & kHasDistanceField_TextType);
}
bool GrTextBlob::hasBitmap() const noexcept { return SkToBool(fTextType & kHasBitmap_TextType); }
bool GrTextBlob::hasPerspective() const noexcept { return fInitialMatrix.hasPerspective(); }

void GrTextBlob::setHasDistanceField() noexcept { fTextType |= kHasDistanceField_TextType; }
void GrTextBlob::setHasBitmap() noexcept { fTextType |= kHasBitmap_TextType; }
void GrTextBlob::setMinAndMaxScale(SkScalar scaledMin, SkScalar scaledMax) noexcept {
...
void GrTextBlob::insertOpsIntoTarget(
    GrTextTarget* target, const SkSurfaceProps& props, const SkPaint& paint, const GrClip* clip,
    const SkMatrixProvider& deviceMatrix, SkPoint drawOrigin) {
...
const GrTextBlob::Key& GrTextBlob::key() const noexcept { return fKey; }
size_t GrTextBlob::size() const noexcept { return fSize; }
...
GrTextBlob::GrTextBlob(
    size_t allocSize, const SkMatrix& drawMatrix, SkPoint origin, SkColor initialLuminance) noexcept 
    : fSize{allocSize},
...
void GrTextBlob::insertSubRun(SubRun* subRun) noexcept {
...
auto GrTextBlob::firstSubRun() const noexcept -> SubRun* { return fFirstSubRun; }

// -- GrTextBlob::VertexRegenerator ----------------------------------------------------------------
GrTextBlob::VertexRegenerator::VertexRegenerator(
    GrResourceProvider* resourceProvider, GrTextBlob::SubRun* subRun,
    GrDeferredUploadTarget* uploadTarget, GrAtlasManager* fullAtlasManager) noexcept 
...
std::tuple<bool, int> GrTextBlob::VertexRegenerator::updateTextureCoordinates(
  ...
  for (auto [glyph, pos, rect] : vertexData) {
    ...
    fFullAtlasManager->addGlyphToBulkAndSetUseToken(
        fSubRun->bulkUseToken(), fSubRun->maskFormat(), grGlyph, tokenTracker->nextDrawToken());
    glyphsPlacedInAtlas++;
  }
...
std::tuple<bool, int> GrTextBlob::VertexRegenerator::regenerate(int begin, int end) {
  ...
  if (fSubRun->fAtlasGeneration != currentAtlasGen) {
    ...
    if (end == fSubRun->glyphCount()) {
      ...
      fFullAtlasManager->setUseTokenBulk(
          *fSubRun->bulkUseToken(), fUploadTarget->tokenTracker()->nextDrawToken(),
          fSubRun->maskFormat());
    }
