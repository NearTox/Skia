...
GrPathSubRun::GrPathSubRun(
    bool isAntiAliased, const SkStrikeSpec& strikeSpec, SkSpan<PathGlyph> paths)
    : fIsAntiAliased{isAntiAliased}, fStrikeSpec{strikeSpec}, fPaths{paths} {}
...
auto GrPathSubRun::Make(
    ...
    const SkStrikeSpec& strikeSpec, SkArenaAlloc* alloc) -> GrSubRun* {
  PathGlyph* pathData =
...
static SkPMColor4f calculate_colors(
    ...
    if (clip == nullptr) {
      ...
      if (!clip->containsNoEmptyCheck(devIRect)) {
        if (SkIRect clipped; clipped.intersect(devIRect, *clip)) {
          ...
          std::tie(tl, tt) =
              GrDrawOpAtlas::PackIndexInTexCoords(rect.fLeft + lD, rect.fTop + tD, index);
          std::tie(tr, tb) =
              GrDrawOpAtlas::PackIndexInTexCoords(rect.fRight + rD, rect.fBottom + bD, index);
        } else {
...
template <typename Quad, typename VertexData>
static void fill_transformed_vertices_2D(
...
template <typename Quad, typename VertexData>
static void fill_transformed_vertices_3D(
    SkZip<Quad, const GrGlyph*, const VertexData> quadData, SkScalar dstPadding,
    SkScalar strikeToSource, GrColor color, const SkMatrix& matrix) {
  SkPoint inset = {dstPadding, dstPadding};
...
sk_sp<GrTextBlob> GrTextBlob::Make(const SkGlyphRunList& glyphRunList, const SkMatrix& drawMatrix) {
  ...
  sk_sp<GrTextBlob> blob{
      new (allocation) GrTextBlob{arenaSize, drawMatrix, glyphRunList.origin(), initialLuminance}};

  return blob;
}

void GrTextBlob::setupKey(
...
bool GrTextBlob::canReuse(
    const SkPaint& paint, const SkMaskFilterBase::BlurRec& blurRec, const SkMatrix& drawMatrix,
    SkPoint drawOrigin) {
...
GrTextBlob::GrTextBlob(
    size_t allocSize, const SkMatrix& drawMatrix, SkPoint origin, SkColor initialLuminance)
    : fSize{allocSize},
...
void GrTextBlob::processSourcePaths(
  ...
  this->setHasBitmap();
  GrSubRun* subRun =
      GrPathSubRun::Make(drawables, runFont.hasSomeAntiAliasing(), strikeSpec, &fAlloc);
  this->insertSubRun(subRun);
}
