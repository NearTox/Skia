...
GrTextBlob::SubRun::SubRun(
    ...
    const SkSpan<PackedGlyphIDorGrGlyph>& glyphs, const SkSpan<char>& vertexData)
    : fType{type},
      fBlob{textBlob},
      fMaskFormat{format},
      fGlyphs{glyphs},
...
static SkRect dest_rect(const SkGlyph& g, SkPoint origin) {
  return SkRect::MakeXYWH(
      SkIntToScalar(g.left()) + origin.x(), SkIntToScalar(g.top()) + origin.y(),
      SkIntToScalar(g.width()), SkIntToScalar(g.height()));
}

static bool is_SDF(const SkGlyph& skGlyph) { return skGlyph.maskFormat() == SkMask::kSDF_Format; }

static SkRect dest_rect(const SkGlyph& g, SkPoint origin, SkScalar textScale) {
  if (!is_SDF(g)) {
    return SkRect::MakeXYWH(
        SkIntToScalar(g.left()) * textScale + origin.x(),
        SkIntToScalar(g.top()) * textScale + origin.y(), SkIntToScalar(g.width()) * textScale,
...
size_t GrTextBlob::SubRun::texCoordOffset() const {
  switch (fMaskFormat) {
    case kA8_GrMaskFormat:
      return this->hasW() ? offsetof(SDFT3DVertex, atlasPos) : offsetof(Mask2DVertex, atlasPos);
    case kARGB_GrMaskFormat:
      return this->hasW() ? offsetof(ARGB3DVertex, atlasPos) : offsetof(ARGB2DVertex, atlasPos);
    default: SkASSERT(!this->hasW()); return offsetof(Mask2DVertex, atlasPos);
  }
}
...
bool GrTextBlob::SubRun::hasW() const { return fBlob->hasW(fType); }
...
void GrTextBlob::SubRun::translateVerticesIfNeeded(const SkMatrix& drawMatrix, SkPoint drawOrigin) {
...
void GrTextBlob::SubRun::updateTexCoords(int begin, int end) {
  ...
  for (int i = begin; i < end; i++) {
    ...
    int pad = this->drawAsDistanceFields() ? SK_DistanceFieldInset : (this->needsPadding() ? 1 : 0);
...
sk_sp<GrTextBlob> GrTextBlob::Make(
    const SkGlyphRunList& glyphRunList, const SkMatrix& drawMatrix, GrColor color,
...
size_t GrTextBlob::GetVertexStride(GrMaskFormat maskFormat, bool hasWCoord) {
  switch (maskFormat) {
    case kA8_GrMaskFormat: return hasWCoord ? sizeof(SDFT3DVertex) : sizeof(Mask2DVertex);
    case kARGB_GrMaskFormat: return hasWCoord ? sizeof(ARGB3DVertex) : sizeof(ARGB2DVertex);
    default: SkASSERT(!hasWCoord); return sizeof(Mask2DVertex);
  }
}

bool GrTextBlob::mustRegenerate(
    const SkPaint& paint, bool anyRunHasSubpixelPosition, const SkMaskFilterBase::BlurRec& blurRec,
...
void GrTextBlob::addOp(
    GrTextTarget* target, const SkSurfaceProps& props, const SkPaint& paint,
    const SkPMColor4f& filteredColor, const GrClip& clip, const SkMatrixProvider& deviceMatrix,
    SkPoint drawOrigin) {
  for (SubRun* subRun = fFirstSubRun; subRun != nullptr; subRun = subRun->fNextSubRun) {
    if (subRun->drawAsPaths()) {
      ...
      SkMatrix strikeToSource = SkMatrix::MakeScale(subRun->fStrikeSpec.strikeToSourceRatio());
      ...
      if (!subRun->drawAsDistanceFields() && !subRun->needsTransform() &&
          clip.isRRect(rtBounds, &clipRRect, &aa) && clipRRect.isRect() && GrAA::kNo == aa) {
        skipClip = true;
      ...
      auto op = this->makeOp(
          *subRun, deviceMatrix, drawOrigin, clipRect, paint, filteredColor, props, target);
      if (op) {
        if (skipClip) {
...
std::unique_ptr<GrDrawOp> GrTextBlob::test_makeOp(
    const SkMatrixProvider& matrixProvider, SkPoint drawOrigin, const SkPaint& paint,
    const SkPMColor4f& filteredColor, const SkSurfaceProps& props, GrTextTarget* target) {
  SubRun* info = fFirstSubRun;
  SkIRect emptyRect = SkIRect::MakeEmpty();
  return this->makeOp(
      *info, matrixProvider, drawOrigin, emptyRect, paint, filteredColor, props, target);
}
...
GrTextBlob::SubRun* GrTextBlob::makeSubRun(
    SubRunType type, const SkZip<SkGlyphVariant, SkPoint>& drawables,
    const SkStrikeSpec& strikeSpec, GrMaskFormat format) {
...
GrTextBlob::GrTextBlob(
    size_t allocSize, const SkMatrix& drawMatrix, SkPoint origin, GrColor color,
    SkColor initialLuminance, bool forceWForDistanceFields)
    : fSize{allocSize},
...
std::unique_ptr<GrAtlasTextOp> GrTextBlob::makeOp(
    SubRun& info, const SkMatrixProvider& matrixProvider, SkPoint drawOrigin,
  ...
  if (info.drawAsDistanceFields()) {
    ...
    return GrAtlasTextOp::MakeDistanceField(
        target->getContext(), std::move(grPaint), &info, matrixProvider.localToDevice(), drawOrigin,
        clipRect, filteredColor, target->colorInfo().isLinearlyBlended(),
        SkPaintPriv::ComputeLuminanceColor(paint), props);
  } else {
    return GrAtlasTextOp::MakeBitmap(
        target->getContext(), std::move(grPaint), &info, matrixProvider.localToDevice(), drawOrigin,
        clipRect, filteredColor);
  }
}
...
static void expand_bits(
    INT_TYPE* dst, const uint8_t* src, int width, int height, int dstRowBytes, int srcRowBytes) {
  ...
  if (grMaskFormat == expectedMaskFormat) {
    ...
    if (glyph.maskFormat() != SkMask::kBW_Format) {
      ...
      switch (expectedMaskFormat) {
        ...
        default: SK_ABORT("Invalid GrMaskFormat");
      }
    }
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        ...
        uint32_t colorRGBA = GrColorPackRGBA(
            masks.getRed(color565), masks.getGreen(color565), masks.getBlue(color565), 0xFF);
...
static GrDrawOpAtlas::ErrorCode add_glyph_to_atlas(
    const SkGlyph& skGlyph, GrMaskFormat expectedMaskFormat, bool needsPadding,
    GrResourceProvider* resourceProvider, GrDeferredUploadTarget* target,
    GrAtlasManager* fullAtlasManager, GrGlyph* grGlyph) {
  ...
  return fullAtlasManager->addToAtlas(
      resourceProvider, target, expectedMaskFormat, width, height, storage.get(),
...
GrTextBlob::VertexRegenerator::VertexRegenerator(
    ...
    GrDeferredUploadTarget* uploadTarget, GrAtlasManager* fullAtlasManager)
...
std::tuple<bool, int> GrTextBlob::VertexRegenerator::updateTextureCoordinates(
  ...
  if (!fMetricsAndImages.isValid() || fMetricsAndImages->descriptor() != strikeSpec.descriptor()) {
  ...
  for (; i < end; i++) {
    ...
    if (!fFullAtlasManager->hasGlyph(fSubRun->maskFormat(), grGlyph)) {
      ...
      code = add_glyph_to_atlas(
          skGlyph, fSubRun->maskFormat(), fSubRun->needsPadding(), fResourceProvider, fUploadTarget,
          fFullAtlasManager, grGlyph);
