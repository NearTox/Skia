...
GrTextBlob::SubRun::SubRun(
    ...
    const SkSpan<GrGlyph*>& glyphs, const SkSpan<char>& vertexData, sk_sp<GrTextStrike>&& grStrike)
    : fType{type},
      fBlob{textBlob},
      fMaskFormat{format},
      fGlyphs{glyphs},
...
sk_sp<GrTextBlob> GrTextBlob::Make(
    const SkGlyphRunList& glyphRunList, GrStrikeCache* strikeCache, const SkMatrix& drawMatrix,
    GrColor color, bool forceWForDistanceFields) {
  ...
  sk_sp<GrTextBlob> blob{new (allocation) GrTextBlob{
      arenaSize, strikeCache, drawMatrix, glyphRunList.origin(), color, initialLuminance,
  ...
  for (SubRun* subRun = fFirstSubRun; subRun != nullptr; subRun = subRun->fNextSubRun) {
    if (subRun->drawAsPaths()) {
      ...
      if (!subRun->drawAsDistanceFields() && !subRun->needsTransform() &&
        ...
        SkRect subRunBounds;
        this->computeSubRunBounds(&subRunBounds, *subRun, drawMatrix, drawOrigin, false);
      ...
      if (submitOp) {
        auto op = this->makeOp(
            *subRun, glyphCount, drawMatrix, drawOrigin, clipRect, paint, filteredColor, props,
            target);
        if (op) {
          if (skipClip) {
...
void GrTextBlob::computeSubRunBounds(
    SkRect* outBounds, const SubRun& subRun, const SkMatrix& drawMatrix, SkPoint drawOrigin,
...
std::unique_ptr<GrDrawOp> GrTextBlob::test_makeOp(
  ...
  return this->makeOp(
      *info, glyphCount, drawMatrix, drawOrigin, emptyRect, paint, filteredColor, props, target);
}
...
GrTextBlob::SubRun* GrTextBlob::makeSubRun(
  ...
  SubRun* subRun =
      fAlloc.make<SubRun>(type, this, strikeSpec, format, glyphs, vertexData, std::move(grStrike));
...
GrTextBlob::GrTextBlob(
    size_t allocSize, GrStrikeCache* strikeCache, const SkMatrix& drawMatrix, SkPoint origin,
    GrColor color, SkColor initialLuminance, bool forceWForDistanceFields)
    : fSize{allocSize},
  ...
  for (; i < end; i++) {
    ...
    if (!fFullAtlasManager->hasGlyph(fSubRun->maskFormat(), grGlyph)) {
      ...
      code = grStrike->addGlyphToAtlas(
          skGlyph, fSubRun->maskFormat(), fSubRun->needsTransform(), fResourceProvider,
