...
bool GrTextBlob::mustRegenerate(
  ...
  if (this->hasBitmap()) {
    ...
    SkScalar transX =
        viewMatrix.getTranslateX() + viewMatrix.getScaleX() * (x - fInitialOrigin.x()) +
        viewMatrix.getSkewX() * (y - fInitialOrigin.y()) - fInitialViewMatrix.getTranslateX();
    SkScalar transY =
        viewMatrix.getTranslateY() + viewMatrix.getSkewY() * (x - fInitialOrigin.x()) +
        viewMatrix.getScaleY() * (y - fInitialOrigin.y()) - fInitialViewMatrix.getTranslateY();
    ...
    if (subRun.drawAsPaths()) {
      ...
      for (const auto& pathGlyph : subRun.fPaths) {
        ...
        pathMatrix.postTranslate(
            originShift.x() + pathGlyph.fOrigin.x(), originShift.y() + pathGlyph.fOrigin.y());
      ...
      if (submitOp) {
        auto op = this->makeOp(
            subRun, glyphCount, viewMatrix, x, y, clipRect, paint, filteredColor, props,
...
std::unique_ptr<GrDrawOp> GrTextBlob::test_makeOp(
  ...
  return this->makeOp(
      info, glyphCount, viewMatrix, x, y, emptyRect, paint, filteredColor, props,
      distanceAdjustTable, target);
}
  ...
  for (auto t : SkMakeZip(l.fSubRuns, r.fSubRuns)) {
    ...
    if (!lSubRun.drawAsPaths()) {
      ...
      if (lSubRun.strike()) {
        ...
        SkASSERT_RELEASE(
            GrTextStrike::GetKey(*lSubRun.strike()) == GrTextStrike::GetKey(*rSubRun.strike()));
...
void GrTextBlob::computeSubRunBounds(
    SkRect* outBounds, const GrTextBlob::SubRun& subRun, const SkMatrix& viewMatrix, SkScalar x,
...
void GrTextBlob::SubRun::computeTranslation(
  ...
  calculate_translation(
      !this->drawAsDistanceFields() && !this->needsTransform(), viewMatrix, x, y,
