...
class GrTextBlob : public SkNVRefCnt<GrTextBlob>, public SkGlyphRunPainterInterface {
 public:
  ...
  static sk_sp<GrTextBlob> Make(
      int glyphCount, GrStrikeCache* strikeCache, const SkMatrix& viewMatrix, SkPoint origin,
  ...
  bool hasPerspective() const { return fInitialViewMatrix.hasPerspective(); }
  ...
  void computeSubRunBounds(
      SkRect* outBounds, const SubRun& subRun, const SkMatrix& viewMatrix, SkScalar x, SkScalar y,
  ...
  void initReusableBlob(SkColor luminanceColor) { fLuminanceColor = luminanceColor; }
  ...
  std::unique_ptr<GrDrawOp> test_makeOp(
      int glyphCount, const SkMatrix& viewMatrix, SkScalar x, SkScalar y, const SkPaint& paint,
 ...
 public:
  ...
  struct PathGlyph {
    PathGlyph(const SkPath& path, SkPoint origin) : fPath(path), fOrigin(origin) {}
    SkPath fPath;
    SkPoint fOrigin;
  };
  ...
  class SubRun {
   public:
    ...
    SubRun(
        ...
        const SubRunBufferSpec& bufferSpec, sk_sp<GrTextStrike>&& grStrike);
 ...
 private:
  GrTextBlob(
      size_t size, GrStrikeCache* strikeCache, const SkMatrix& viewMatrix, SkPoint origin,
      GrColor color, bool forceWForDistanceFields);
  ...
  std::unique_ptr<GrAtlasTextOp> makeOp(
      SubRun& info, int glyphCount, const SkMatrix& viewMatrix, SkScalar x, SkScalar y,
...
class GrTextBlob::VertexRegenerator {
 public:
   ...
   */
  VertexRegenerator(
      GrResourceProvider*, GrTextBlob*, GrTextBlob::SubRun* subRun, const SkMatrix& viewMatrix,
