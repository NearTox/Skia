...
class GrTextBlob : public SkNVRefCnt<GrTextBlob>, public SkGlyphRunPainterInterface {
 ...
 public:
  ...
  static sk_sp<GrTextBlob> Make(
      int glyphCount, int runCount, GrColor color, GrStrikeCache* strikeCache);
  ...
  static size_t GetVertexStride(GrMaskFormat maskFormat, bool hasWCoord) noexcept {
    switch (maskFormat) {
      ...
      default: SkASSERT(!hasWCoord); return kLCDTextVASize;
    }
  }

  bool mustRegenerate(
      const SkPaint&, bool, const SkMaskFilterBase::BlurRec& blurRec, const SkMatrix& viewMatrix,
      SkScalar x, SkScalar y);

  void flush(
      GrTextTarget*, const SkSurfaceProps& props,
      const GrDistanceFieldAdjustTable* distanceAdjustTable, const SkPaint& paint,
      const SkPMColor4f& filteredColor, const GrClip& clip, const SkMatrix& viewMatrix, SkScalar x,
      SkScalar y);

  void computeSubRunBounds(
      SkRect* outBounds, int runIndex, int subRunIndex, const SkMatrix& viewMatrix, SkScalar x,
      SkScalar y, bool needsGlyphTransform) {
  ...
  std::unique_ptr<GrDrawOp> test_makeOp(
      int glyphCount, uint16_t run, uint16_t subRun, const SkMatrix& viewMatrix, SkScalar x,
      SkScalar y, const SkPaint& paint, const SkPMColor4f& filteredColor, const SkSurfaceProps&,
 ...
 private:
  ...
  class SubRun {
   public:
    SubRun(Run* run, const SkStrikeSpec& strikeSpec, GrColor color) noexcept
    ...
    void computeTranslation(
        const SkMatrix& viewMatrix, SkScalar x, SkScalar y, SkScalar* transX,
    ...
    const SkStrikeSpec& strikeSpec() const noexcept { return fStrikeSpec; }
  ...
  struct Run {
    ...
    void switchSubRunIfNeededAndAppendGlyph(
  ...
  std::unique_ptr<GrAtlasTextOp> makeOp(
      const SubRun& info, int glyphCount, uint16_t run, uint16_t subRun, const SkMatrix& viewMatrix,
      SkScalar x, SkScalar y, const SkIRect& clipRect, const SkPaint& paint,
  ...
  void processSourceSDFT(
      ...
      SkScalar minScale, SkScalar maxScale, bool hasWCoord) override;

  void processSourceFallback(
      SkSpan<const SkGlyphPos> masks, const SkStrikeSpec& strikeSpec, bool hasW) override;

  void processDeviceFallback(
      SkSpan<const SkGlyphPos> masks, const SkStrikeSpec& strikeSpec) override;
...
class GrTextBlob::VertexRegenerator {
 public:
   ...
   */
  VertexRegenerator(
      GrResourceProvider*, GrTextBlob*, int runIdx, int subRunIdx, const SkMatrix& viewMatrix,
      SkScalar x, SkScalar y, GrColor color, GrDeferredUploadTarget*, GrStrikeCache*,
      GrAtlasManager*, SkExclusiveStrikePtr*);
