...
class GrAtlasSubRun {
 public:
  ...
  virtual std::tuple<const GrClip*, GrOp::Owner> makeAtlasTextOp(
      ...
      skgpu::v1::SurfaceDrawContext*, GrAtlasSubRunOwner subRun) const = 0;
#endif
...
class GrTextBlob final : public SkNVRefCnt<GrTextBlob>, public SkGlyphRunPainterInterface {
 public:
  ...
  struct Key {
    static std::tuple<bool, Key> Make(
        const SkGlyphRunList& glyphRunList, const SkPaint& paint,
        const SkSurfaceProps& surfaceProps, const GrColorInfo& colorInfo,
        const SkMatrix& drawMatrix, const GrSDFTControl& control);
    uint32_t fUniqueID;
...
class GrSubRunNoCachePainter : public SkGlyphRunPainterInterface {
 public:
  GrSubRunNoCachePainter(
      skgpu::v1::SurfaceDrawContext*, GrSubRunAllocator*, const GrClip*,
      const SkMatrixProvider& viewMatrix, const SkGlyphRunList&, const SkPaint&);
  void processDeviceMasks(
      const SkZip<SkGlyphVariant, SkPoint>& drawables, const SkStrikeSpec& strikeSpec) override;
  void processSourceMasks(
      const SkZip<SkGlyphVariant, SkPoint>& drawables, const SkStrikeSpec& strikeSpec) override;
