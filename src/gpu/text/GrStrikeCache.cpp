...
GrStrikeCache::GrStrikeCache(const GrCaps* caps, size_t maxTextureBytes)
    : fPreserveStrike(nullptr),
      f565Masks(SkMasks::CreateMasks(
          {0xF800, 0x07E0, 0x001F, 0}, GrMaskFormatBytesPerPixel(kA565_GrMaskFormat))) {}
...
static bool get_packed_glyph_image(
    SkStrike* cache, const SkGlyph& glyph, int width, int height, int dstRB,
    GrMaskFormat expectedMaskFormat, void* dst, const SkMasks& masks) {
...
GrDrawOpAtlas::ErrorCode GrTextStrike::addGlyphToAtlas(
    GrResourceProvider* resourceProvider, GrDeferredUploadTarget* target, GrStrikeCache* glyphCache,
    GrAtlasManager* fullAtlasManager, GrGlyph* glyph, SkStrike* skStrikeCache,
    GrMaskFormat expectedMaskFormat, bool isScaledGlyph) {
  SkASSERT(glyph);
  ...
  if (!get_packed_glyph_image(
          skStrikeCache, skGlyph, glyph->width(), glyph->height(), rowBytes, expectedMaskFormat,
  ...
  GrDrawOpAtlas::ErrorCode result = fullAtlasManager->addToAtlas(
      resourceProvider, glyphCache, this, &glyph->fID, target, expectedMaskFormat, width, height,
