...
class GrTextContext {
 public:
  ...
  void drawGlyphRunList(
      GrRecordingContext*, GrTextTarget*, const GrClip&, const SkMatrixProvider&,
      const SkSurfaceProps&, const SkGlyphRunList&) const;
  ...
  std::unique_ptr<GrDrawOp> createOp_TestingOnly(
      ...
      const SkMatrixProvider&, const char* text, int x, int y);
#endif

  static void SanitizeOptions(Options* options);
  static bool CanDrawAsDistanceFields(
      const SkPaint&, const SkFont&, const SkMatrix& viewMatrix, const SkSurfaceProps& props,
      bool contextSupportsDistanceFieldText, const Options& options);

  static SkFont InitDistanceFieldFont(
      const SkFont& font, const SkMatrix& viewMatrix, const Options& options, SkScalar* textRatio);

  static SkPaint InitDistanceFieldPaint(const SkPaint& paint);

  static std::pair<SkScalar, SkScalar> InitDistanceFieldMinMaxScale(
      SkScalar textSize, const SkMatrix& viewMatrix, const Options& options);
