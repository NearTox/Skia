...
static void regen_texcoords(
    char* vertex, size_t vertexStride, const GrGlyph* glyph, bool useDistanceFields) {
  // This is a bit wonky, but sometimes we have LCD text, in which case we won't have color
  // vertices, hence vertexStride - sizeof(SkIPoint16)
  ...
  GrColor hackColor;
  switch (pageIndex) {
    case 0: hackColor = GrColorPackRGBA(0, 255, 0, 255); break;
    case 1:
      hackColor = GrColorPackRGBA(255, 0, 0, 255);
      ;
      break;
    case 2: hackColor = GrColorPackRGBA(255, 0, 255, 255); break;
    case 3: hackColor = GrColorPackRGBA(0, 255, 255, 255); break;
    default: hackColor = GrColorPackRGBA(0, 0, 0, 255); break;
  }
...
GrTextBlob::VertexRegenerator::VertexRegenerator(
    ...
    GrAtlasManager* fullAtlasManager, SkExclusiveStrikePtr* lazyStrike)
...
bool GrTextBlob::VertexRegenerator::doRegen(
    GrTextBlob::VertexRegenerator::Result* result, bool regenPos, bool regenCol,
    bool regenTexCoords, bool regenGlyphs) {
  ...
  for (int glyphIdx = fCurrGlyph; glyphIdx < (int)fSubRun->glyphCount(); glyphIdx++) {
    ...
    if (regenTexCoords) {
      ...
      if (!fFullAtlasManager->hasGlyph(glyph)) {
        ...
        code = strike->addGlyphToAtlas(
            fResourceProvider, fUploadTarget, fGlyphCache, fFullAtlasManager, glyph,
            fLazyStrike->get(), fSubRun->maskFormat(), fSubRun->needsTransform());
...
bool GrTextBlob::VertexRegenerator::regenerate(GrTextBlob::VertexRegenerator::Result* result) {
  ...
  if (fRegenFlags) {
    return this->doRegen(
        result, fRegenFlags & kRegenPos, fRegenFlags & kRegenCol, fRegenFlags & kRegenTex,
        fRegenFlags & kRegenGlyph);
  } else {
    bool hasW = fSubRun->hasWCoord();
    auto vertexStride = GetVertexStride(fSubRun->maskFormat(), hasW);
