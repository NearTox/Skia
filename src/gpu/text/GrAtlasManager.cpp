...
GrAtlasManager::GrAtlasManager(
    GrProxyProvider* proxyProvider, GrStrikeCache* glyphCache, size_t maxTextureBytes,
...
static GrColorType mask_format_to_gr_color_type(GrMaskFormat format) {
  switch (format) {
    ...
    case kA565_GrMaskFormat: return GrColorType::kRGB_565;
    case kARGB_GrMaskFormat: return GrColorType::kRGBA_8888;
    default: SkDEBUGFAIL("unsupported GrMaskFormat"); return GrColorType::kAlpha_8;
  }
}
...
GrDrawOpAtlas::ErrorCode GrAtlasManager::addToAtlas(
    ...
    GrDrawOpAtlas::AtlasID* id, GrDeferredUploadTarget* target, GrMaskFormat format, int width,
    int height, const void* image, SkIPoint16* loc) {
  glyphCache->setStrikeToPreserve(strike);
  return this->getAtlas(format)->addToAtlas(
      resourceProvider, id, target, width, height, image, loc);
}

void GrAtlasManager::addGlyphToBulkAndSetUseToken(
    GrDrawOpAtlas::BulkUseTokenUpdater* updater, GrGlyph* glyph, GrDeferredUploadToken token) {
  SkASSERT(glyph);
...
static bool save_pixels(GrContext* context, GrSurfaceProxy* sProxy, const char* filename) {
  ...
  SkImageInfo ii = SkImageInfo::Make(
      sProxy->width(), sProxy->height(), kRGBA_8888_SkColorType, kPremul_SkAlphaType);
  SkBitmap bm;
...
bool GrAtlasManager::initAtlas(GrMaskFormat format) {
  ...
  if (fAtlases[index] == nullptr) {
    ...
    const GrBackendFormat format =
        fCaps->getBackendFormatFromGrColorType(grColorType, GrSRGBEncoded::kNo);
