...
static bool sw_draw_with_mask_filter(
    GrRecordingContext* context, GrRenderTargetContext* renderTargetContext, const GrClip& clipData,
    const SkMatrix& viewMatrix, const GrShape& shape, const SkMaskFilter* filter,
    const SkIRect& clipBounds, GrPaint&& paint, const GrUniqueKey& key) {
  SkASSERT(filter);
  ...
  if (key.isValid()) {
    ...
    auto filteredMask =
        proxyProvider->findOrCreateProxyByUniqueKey(key, GrColorType::kAlpha_8, kCacheOrigin);
    if (filteredMask) {
  ...
  if (filteredMaskView.proxy()) {
    ...
    GrBitmapTextureMaker maker(
        context, bm, GrBitmapTextureMaker::Cached::kNo, SkBackingFit::kApprox);
    ...
    GrSwizzle swizzle = context->priv().caps()->getReadSwizzle(filteredMask->backendFormat(), grCT);
    filteredMaskView =
        GrSurfaceProxyView(std::move(filteredMask), kTopLeft_GrSurfaceOrigin, swizzle);
  }
...
static bool get_unclipped_shape_dev_bounds(
    const GrShape& shape, const SkMatrix& matrix, SkIRect* devBounds) {
...
static bool get_shape_and_clip_bounds(
    GrRenderTargetContext* renderTargetContext, const GrClip& clip, const GrShape& shape,
    const SkMatrix& matrix, SkIRect* unclippedDevShapeBounds, SkIRect* devClipBounds) {
  // compute bounds as intersection of rt size, clip, and path
  clip.getConservativeBounds(
      renderTargetContext->width(), renderTargetContext->height(), devClipBounds);
...
static void draw_shape_with_mask_filter(
  ...
  if (maskFilter->directFilterMaskGPU(
          context, renderTargetContext, std::move(paint), clip, viewMatrix, *shape)) {
  ...
  SkIRect maskRect;
  if (maskFilter->canFilterMaskGPU(
          *shape, unclippedDevShapeBounds, *boundsForClip, viewMatrix, &maskRect)) {
    ...
    if (maskKey.isValid()) {
      ...
      static const GrSurfaceOrigin kCacheOrigin = kTopLeft_GrSurfaceOrigin;
      auto filteredMask =
          proxyProvider->findOrCreateProxyByUniqueKey(maskKey, GrColorType::kAlpha_8, kCacheOrigin);
      if (filteredMask) {
        GrSwizzle swizzle = context->priv().caps()->getReadSwizzle(
            filteredMask->backendFormat(), GrColorType::kAlpha_8);
        filteredMaskView = GrSurfaceProxyView(std::move(filteredMask), kCacheOrigin, swizzle);
    ...
    if (!filteredMaskView.proxy()) {
      ...
      if (maskRTC) {
        filteredMaskView = maskFilter->filterMaskGPU(
            context, maskRTC->readSurfaceView(), maskRTC->colorInfo().colorType(),
        ...
        if (filteredMaskView.proxy() && maskKey.isValid()) {
          ...
          proxyProvider->assignUniqueKeyToProxy(maskKey, filteredMaskView.asTextureProxy());
        }
      }
    }
  ...
  sw_draw_with_mask_filter(
      context, renderTargetContext, clip, viewMatrix, *shape, maskFilter, *boundsForClip,
      std::move(paint), maskKey);
}

void GrBlurUtils::drawShapeWithMaskFilter(
    GrRecordingContext* context, GrRenderTargetContext* renderTargetContext, const GrClip& clip,
    const GrShape& shape, GrPaint&& paint, const SkMatrix& viewMatrix, const SkMaskFilter* mf) {
...
void GrBlurUtils::drawShapeWithMaskFilter(
    GrRecordingContext* context, GrRenderTargetContext* renderTargetContext, const GrClip& clip,
    const SkPaint& paint, const SkMatrix& viewMatrix, const GrShape& shape) {
