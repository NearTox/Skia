...
sk_sp<GrDawnTexture> GrDawnTexture::Make(
  ...
  if (renderTarget) {
    result = sk_sp<GrDawnTextureRenderTarget>(
        new GrDawnTextureRenderTarget(gpu, dimensions, sampleCnt, info, status, label));
  } else {
    result = sk_sp<GrDawnTexture>(new GrDawnTexture(gpu, dimensions, info, status, label));
  }
...
sk_sp<GrDawnTexture> GrDawnTexture::MakeWrapped(
    ...
    GrWrapCacheable cacheable, GrIOType ioType, const GrDawnTextureInfo& info) {
