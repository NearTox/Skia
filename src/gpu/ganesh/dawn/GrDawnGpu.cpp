...
static wgpu::FilterMode to_dawn_filter_mode(GrSamplerState::Filter filter) {
  switch (filter) {
    case GrSamplerState::Filter::kNearest: return wgpu::FilterMode::Nearest;
    case GrSamplerState::Filter::kLinear: return wgpu::FilterMode::Linear;
    default: SkASSERT(!"unsupported filter mode"); return wgpu::FilterMode::Nearest;
  }
}
...
static wgpu::FilterMode to_dawn_mipmap_mode(GrSamplerState::MipmapMode mode) {
  switch (mode) {
    ...
    case GrSamplerState::MipmapMode::kNearest: return wgpu::FilterMode::Nearest;
    case GrSamplerState::MipmapMode::kLinear: return wgpu::FilterMode::Linear;
    default: SkASSERT(!"unsupported filter mode"); return wgpu::FilterMode::Nearest;
  }
}
...
static wgpu::AddressMode to_dawn_address_mode(GrSamplerState::WrapMode wrapMode) {
  switch (wrapMode) {
    case GrSamplerState::WrapMode::kClamp: return wgpu::AddressMode::ClampToEdge;
    case GrSamplerState::WrapMode::kRepeat: return wgpu::AddressMode::Repeat;
    case GrSamplerState::WrapMode::kMirrorRepeat: return wgpu::AddressMode::MirrorRepeat;
    case GrSamplerState::WrapMode::kClampToBorder: SkASSERT(!"unsupported address mode");
  }
  SkASSERT(!"unsupported address mode");
  return wgpu::AddressMode::ClampToEdge;
}

sk_sp<GrGpu> GrDawnGpu::Make(
    const wgpu::Device& device, const GrContextOptions& options, GrDirectContext* direct) {
  if (!device) {
    return nullptr;
  }
...
GrDawnGpu::GrDawnGpu(
    GrDirectContext* direct, const GrContextOptions& options, const wgpu::Device& device)
    : INHERITED(direct),
      fDevice(device),
...
GrThreadSafePipelineBuilder* GrDawnGpu::pipelineBuilder() { return nullptr; }

sk_sp<GrThreadSafePipelineBuilder> GrDawnGpu::refPipelineBuilder() { return nullptr; }
...
///////////////////////////////////////////////////////////////////////////////
sk_sp<GrGpuBuffer> GrDawnGpu::onCreateBuffer(
    size_t size, GrGpuBufferType type, GrAccessPattern accessPattern) {
  return GrDawnBuffer::Make(
      this, size, type, accessPattern,
      /*label=*/"DawnGpu_GetOpsRenderPass");
}

////////////////////////////////////////////////////////////////////////////////
bool GrDawnGpu::onWritePixels(
...
bool GrDawnGpu::onTransferPixelsTo(
...
bool GrDawnGpu::onTransferPixelsFrom(
...
sk_sp<GrTexture> GrDawnGpu::onWrapBackendTexture(
  ...
  return GrDawnTexture::MakeWrapped(
      this, dimensions, GrRenderable::kNo, 1, cacheable, ioType, info);
}
...
sk_sp<GrTexture> GrDawnGpu::onWrapRenderableBackendTexture(
    const GrBackendTexture& tex, int sampleCnt, GrWrapOwnership, GrWrapCacheable cacheable) {
  GrDawnTextureInfo info;
  ...
  sk_sp<GrTexture> result = GrDawnTexture::MakeWrapped(
      this, dimensions, GrRenderable::kYes, sampleCnt, cacheable, kRW_GrIOType, info);
...
sk_sp<GrAttachment> GrDawnGpu::makeStencilAttachment(
...
GrBackendTexture GrDawnGpu::onCreateBackendTexture(
    SkISize dimensions, const GrBackendFormat& backendFormat, GrRenderable renderable,
...
void GrDawnGpu::uploadTextureData(
...
bool GrDawnGpu::onClearBackendTexture(
...
bool GrDawnGpu::onUpdateCompressedBackendTexture(
    const GrBackendTexture&, sk_sp<skgpu::RefCntedCallback> finishedCallback, const void* data,
    size_t size) {
  return false;
}
...
GrBackendRenderTarget GrDawnGpu::createTestingOnlyBackendRenderTarget(
...
void GrDawnGpu::addFinishedProc(
...
void GrDawnGpu::onSubmittedWorkDone(WGPUQueueWorkDoneStatus status) {
  ...
  fQueueFences.foreach ([](GrDawnAsyncWait* fence) { fence->signal(); });
}
...
bool GrDawnGpu::onReadPixels(
  ...
  sk_sp<GrDawnBuffer> dawnBuffer = GrDawnBuffer::Make(
      this, sizeInBytes, GrGpuBufferType::kXferGpuToCpu, kStatic_GrAccessPattern, "onReadPixels");
  if (!dawnBuffer) {
...
bool GrDawnGpu::onRegenerateMipMapLevels(GrTexture* tex) {
  ...
  const char* vs =
      ...
      "}\n";
  std::string vsSPIRV = this->SkSLToSPIRV(
      vs, SkSL::ProgramKind::kVertex,
      /*rtFlipOffset*/ 0, nullptr);
  ...
  const char* fs =
      ...
      "}\n";
  std::string fsSPIRV = this->SkSLToSPIRV(
      fs, SkSL::ProgramKind::kFragment,
      /*rtFlipOffset=*/0, nullptr);
...
std::unique_ptr<GrSemaphore> GrDawnGpu::wrapBackendSemaphore(
    const GrBackendSemaphore& /* sema */, GrSemaphoreWrapType /* wrapType */,
    GrWrapOwnership /* ownership */) {
  SkASSERT(!"unimplemented");
  return nullptr;
}

void GrDawnGpu::insertSemaphore(GrSemaphore* semaphore) { SkASSERT(!"unimplemented"); }

void GrDawnGpu::waitSemaphore(GrSemaphore* semaphore) { SkASSERT(!"unimplemented"); }

void GrDawnGpu::checkFinishProcs() { fFinishCallbacks.check(); }
...
sk_sp<GrDawnProgram> GrDawnGpu::getOrCreateRenderPipeline(
  ...
  sk_sp<GrDawnProgram> program = GrDawnProgramBuilder::Build(
      this, rt, programInfo, colorFormat, hasDepthStencil, stencilFormat, &desc);
...
std::string GrDawnGpu::SkSLToSPIRV(
    const char* shaderString, SkSL::ProgramKind kind, uint32_t rtFlipOffset,
  ...
  std::unique_ptr<SkSL::Program> program =
      this->shaderCompiler()->convertProgram(kind, shaderString, settings);
  if (!program) {
