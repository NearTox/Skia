...
class GrGeometryProcessor : public GrProcessor {
 public:
  ...
  class Attribute {
   public:
    static constexpr size_t AlignOffset(size_t offset) noexcept { return SkAlign4(offset); }
     ...
     */
    constexpr Attribute(const char* name, GrVertexAttribType cpuType, SkSLType gpuType) noexcept
     ...
     */
    constexpr Attribute(
        const char* name, GrVertexAttribType cpuType, SkSLType gpuType, size_t offset) noexcept
    ...
    constexpr bool isInitialized() const noexcept { return fGPUType != SkSLType::kVoid; }

    constexpr const char* name() const noexcept { return fName; }
    constexpr GrVertexAttribType cpuType() const noexcept { return fCPUType; }
    constexpr SkSLType gpuType() const noexcept { return fGPUType; }
    /**
     ...
     * types.
     */
    std::optional<size_t> offset() const noexcept {
  ...
  class AttributeSet {
    class Iter {
     public:
      ...
      Iter(const Iter& iter) noexcept = default;
      Iter& operator=(const Iter& iter) noexcept = default;
   ...
   public:
    ...
    void initImplicit(const Attribute* attrs, int count) noexcept;
    ...
    void initExplicit(const Attribute* attrs, int count, size_t stride) noexcept;
  ...
  int numVertexAttributes() const noexcept { return fVertexAttributes.count(); }
  const AttributeSet& vertexAttributes() const noexcept { return fVertexAttributes; }
  int numInstanceAttributes() const noexcept { return fInstanceAttributes.count(); }
  const AttributeSet& instanceAttributes() const noexcept { return fInstanceAttributes; }

  bool hasVertexAttributes() const noexcept { return SkToBool(fVertexAttributes.count()); }
  bool hasInstanceAttributes() const noexcept { return SkToBool(fInstanceAttributes.count()); }
   ...
   */
  size_t vertexStride() const noexcept { return fVertexAttributes.stride(); }
  size_t instanceStride() const noexcept { return fInstanceAttributes.stride(); }
 ...
 protected:
  ...
  void setVertexAttributes(const Attribute* attrs, int attrCount, size_t stride) noexcept {
    fVertexAttributes.initExplicit(attrs, attrCount, stride);
  }
  void setInstanceAttributes(const Attribute* attrs, int attrCount, size_t stride) noexcept {
    SkASSERT(attrCount >= 0);
    fInstanceAttributes.initExplicit(attrs, attrCount, stride);
  }

  void setVertexAttributesWithImplicitOffsets(const Attribute* attrs, int attrCount) noexcept {
    fVertexAttributes.initImplicit(attrs, attrCount);
  }
  void setInstanceAttributesWithImplicitOffsets(const Attribute* attrs, int attrCount) noexcept {
    SkASSERT(attrCount >= 0);
...
class GrGeometryProcessor::ProgramImpl {
 public:
  ...
  struct EmitArgs {
    EmitArgs(
        GrGLSLVertexBuilder* vertBuilder, GrGLSLFPFragmentBuilder* fragBuilder,
        GrGLSLVaryingHandler* varyingHandler, GrGLSLUniformHandler* uniformHandler,
        const GrShaderCaps* caps, const GrGeometryProcessor& geomProc, const char* outputColor,
        const char* outputCoverage, const SamplerHandle* texSamplers) noexcept
  ...
  virtual void setData(
      const GrGLSLProgramDataManager&, const GrShaderCaps&, const GrGeometryProcessor&) = 0;
  ...
  static uint32_t ComputeMatrixKey(const GrShaderCaps& caps, const SkMatrix& mat) noexcept {
  ...
  static uint32_t ComputeMatrixKeys(
      const GrShaderCaps& shaderCaps, const SkMatrix& viewMatrix,
  ...
  static uint32_t AddMatrixKeys(
      const GrShaderCaps& shaderCaps, uint32_t flags, const SkMatrix& viewMatrix,
      const SkMatrix& localMatrix) noexcept {
    // Shifting to make room for the matrix keys shouldn't lose bits
    SkASSERT(((flags << (2 * kMatrixKeyBits)) >> (2 * kMatrixKeyBits)) == flags);
    return (flags << (2 * kMatrixKeyBits)) | ComputeMatrixKeys(shaderCaps, viewMatrix, localMatrix);
  }
 ...
 protected:
  ...
  static void SetTransform(
      const GrGLSLProgramDataManager&, const GrShaderCaps&, const UniformHandle& uniform,
      const SkMatrix& matrix, SkMatrix* state = nullptr);
  ...
  static void WriteOutputPosition(
      GrGLSLVertexBuilder*, GrGLSLUniformHandler*, const GrShaderCaps&, GrGPArgs*,
      const char* posName, const SkMatrix& viewMatrix, UniformHandle* viewMatrixUniform);
  ...
  static void WriteLocalCoord(
      GrGLSLVertexBuilder*, GrGLSLUniformHandler*, const GrShaderCaps&, GrGPArgs*,
      GrShaderVar localVar, const SkMatrix& localMatrix, UniformHandle* localMatrixUniform);
 ...
 private:
  ...
  FPCoordsMap collectTransforms(
      ...
      const GrShaderVar& localCoordsVar, const GrShaderVar& positionVar,
...
class GrGeometryProcessor::TextureSampler {
 public:
  ...
  const skgpu::Swizzle& swizzle() const noexcept { return fSwizzle; }
 ...
 * This was moved from include/private/gpu/ganesh/GrTypesPriv.h in service of Skia dependents that
 * build with C++11.
 */
static constexpr inline size_t GrVertexAttribTypeSize(GrVertexAttribType type) noexcept {
  switch (type) {
    ...
    case kFloat3_GrVertexAttribType: return 3 * sizeof(float);
    ...
    case kShort2_GrVertexAttribType: return 2 * sizeof(int16_t);
    case kShort4_GrVertexAttribType: return 4 * sizeof(int16_t);
    case kUShort2_GrVertexAttribType:  // fall through
    case kUShort2_norm_GrVertexAttribType: return 2 * sizeof(uint16_t);
...
constexpr size_t GrGeometryProcessor::Attribute::size() const noexcept {
