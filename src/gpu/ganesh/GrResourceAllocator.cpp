...
GrResourceAllocator::Register::Register(
    GrSurfaceProxy* originatingProxy, skgpu::ScratchKey scratchKey, GrResourceProvider* provider)
    : fOriginatingProxy(originatingProxy), fScratchKey(std::move(scratchKey)) {
  ...
  if (fScratchKey.isValid()) {
    if (can_proxy_use_scratch(*provider->caps(), originatingProxy)) {
      fExistingSurface =
          provider->findAndRefScratchTexture(fScratchKey, /*label=*/"ResourceAllocatorRegister");
    }
  } else {
...
bool GrResourceAllocator::Register::isRecyclable(
    const GrCaps& caps, GrSurfaceProxy* proxy, int knownUseCount) const {
...
bool GrResourceAllocator::Register::instantiateSurface(
    GrSurfaceProxy* proxy, GrResourceProvider* resourceProvider) {
...
GrResourceAllocator::Register* GrResourceAllocator::findOrCreateRegisterFor(GrSurfaceProxy* proxy) {
  ...
  if (const auto& uniqueKey = proxy->getUniqueKey(); uniqueKey.isValid()) {
    ...
    Register* r = fInternalAllocator.make<Register>(proxy, skgpu::ScratchKey(), resourceProvider);
  ...
  auto filter = [](const Register* r) { return true; };
...
#if GR_ALLOCATION_SPEW
    SkDebugf("Assigning register %d to %d\n", r->uniqueID(), cur->proxy()->uniqueID().asUInt());
#endif
