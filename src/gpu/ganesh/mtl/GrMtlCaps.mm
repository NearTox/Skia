...
GrMtlCaps::GrMtlCaps(const GrContextOptions& contextOptions, const id<MTLDevice> device)
  ...
  fStoreAndMultisampleResolveSupport = (fGPUFamily == GPUFamily::kApple && fFamilyGroup >= 3);
...
bool GrMtlCaps::getGPUFamilyFromFeatureSet(id<MTLDevice> device, GPUFamily* gpuFamily, int* group) {
...
bool GrMtlCaps::isFormatAsColorTypeRenderable(
...
int GrMtlCaps::getRenderTargetSampleCount(int requestedCount, const GrBackendFormat& format) const {
  MTLPixelFormat mtlFormat = GrBackendFormatAsMTLPixelFormat(format);
...
void GrMtlCaps::initFormatTable() {
  ...
  this->setColorType(GrColorType::kAlpha_8, {MTLPixelFormatR8Unorm, MTLPixelFormatA8Unorm});
#if defined(SK_BUILD_FOR_IOS) && !TARGET_OS_SIMULATOR
...
skgpu::Swizzle GrMtlCaps::onGetReadSwizzle(
...
skgpu::Swizzle GrMtlCaps::getWriteSwizzle(
    const GrBackendFormat& format, GrColorType colorType) const {
  MTLPixelFormat mtlFormat = GrBackendFormatAsMTLPixelFormat(format);
  SkASSERT(mtlFormat != MTLPixelFormatInvalid);
  const auto& info = this->getFormatInfo(mtlFormat);
...
GrProgramDesc GrMtlCaps::makeDesc(
    GrRenderTarget*, const GrProgramInfo& programInfo,
...
MTLPixelFormat GrMtlCaps::getStencilPixelFormat(const GrProgramDesc& desc) {
  ...
  SkReadBuffer readBuffer(
      desc.asKey() + desc.initialKeyLength() / sizeof(uint32_t),
...
bool GrMtlCaps::renderTargetSupportsDiscardableMSAA(const GrMtlRenderTarget* rt) const {
  return rt->resolveAttachment() && !rt->resolveAttachment()->framebufferOnly() &&
...
void GrMtlCaps::onDumpJSON(SkJSONWriter* writer) const {
  ...
  switch (fGPUFamily) {
    case GPUFamily::kMac: writer->appendString("GPU Family", "Mac"); break;
    case GPUFamily::kApple: writer->appendString("GPU Family", "Apple"); break;
    default: writer->appendString("GPU Family", "unknown"); break;
  }
