...
sk_sp<GrGpu> GrMtlGpu::Make(
    const GrMtlBackendContext& context, const GrContextOptions& options, GrDirectContext* direct) {
...
GrMtlGpu::GrMtlGpu(
    GrDirectContext* direct, const GrContextOptions& options, id<MTLDevice> device,
    id<MTLCommandQueue> queue, GrMTLHandle binaryArchive)
    : INHERITED(direct),
      fDevice(device),
      fQueue(queue),
...
GrThreadSafePipelineBuilder* GrMtlGpu::pipelineBuilder() { return nullptr; }

sk_sp<GrThreadSafePipelineBuilder> GrMtlGpu::refPipelineBuilder() { return nullptr; }
...
GrOpsRenderPass* GrMtlGpu::onGetOpsRenderPass(
  ...
  // TODO: support DMSAA
  SkASSERT(!useMSAASurface || (renderTarget->numSamples() > 1));
  ...
  return new GrMtlOpsRenderPass(
      this, renderTarget, std::move(framebuffer), origin, colorInfo, stencilInfo);
}
...
bool GrMtlGpu::uploadToTexture(
    GrMtlTexture* tex, SkIRect rect, GrColorType dataColorType, const GrMipLevel texels[],
    int mipLevelCount) {
...
bool GrMtlGpu::clearTexture(GrMtlTexture* tex, size_t bpp, uint32_t levelMask) {
  ...
  [blitCmdEncoder fillBuffer:transferBuffer range:clearRange value:0];
...
sk_sp<GrAttachment> GrMtlGpu::makeStencilAttachment(
...
sk_sp<GrAttachment> GrMtlGpu::makeMSAAAttachment(
...
sk_sp<GrTexture> GrMtlGpu::onCreateTexture(
  ...
  if (renderable == GrRenderable::kYes) {
    ...
    tex = GrMtlTexture::MakeNewTexture(
        this, budgeted, dimensions, mtlPixelFormat, mipLevelCount, mipmapStatus, label);
  }
...
sk_sp<GrTexture> GrMtlGpu::onCreateCompressedTexture(
  ...
  auto tex = GrMtlTexture::MakeNewTexture(
      this, budgeted, dimensions, mtlPixelFormat, numMipLevels, mipmapStatus,
  ...
  GrStagingBufferManager::Slice slice =
      fStagingBufferManager.allocateStagingBufferSlice(dataSize, alignment);
  if (!slice.fBuffer) {
    return nullptr;
  }
  char* bufferData = (char*)slice.fOffsetMapPtr;
...
void copy_src_data(
    char* dst, size_t bytesPerPixel, const SkTArray<size_t>& individualMipOffsets,
    const GrPixmap srcData[], int numMipLevels, size_t bufferSize) {
  ...
  for (int level = 0; level < numMipLevels; ++level) {
    ...
    SkRectMemcpy(
        dst + individualMipOffsets[level], trimRB, srcData[level].addr(), srcData[level].rowBytes(),
        trimRB, srcData[level].height());
  }
}

bool GrMtlGpu::createMtlTextureForBackendSurface(
    MTLPixelFormat mtlFormat, SkISize dimensions, int sampleCnt, GrTexturable texturable,
    GrRenderable renderable, GrMipmapped mipmapped, GrMtlTextureInfo* info) {
...
GrBackendTexture GrMtlGpu::onCreateBackendTexture(
  ...
  if (!this->createMtlTextureForBackendSurface(
          mtlFormat, dimensions, 1, GrTexturable::kYes, renderable, mipmapped, &info)) {
    return {};
  }
...
bool GrMtlGpu::onClearBackendTexture(
...
GrBackendTexture GrMtlGpu::onCreateCompressedBackendTexture(
  ...
  GrMtlTextureInfo info;
  if (!this->createMtlTextureForBackendSurface(
          mtlFormat, dimensions, 1, GrTexturable::kYes, GrRenderable::kNo, mipmapped, &info)) {
    return {};
  }
...
bool GrMtlGpu::onUpdateCompressedBackendTexture(
    const GrBackendTexture& backendTexture, sk_sp<skgpu::RefCntedCallback> finishedCallback,
  ...
  combinedBufferSize = SkCompressedDataSize(
      compression, backendTexture.dimensions(), &individualMipOffsets,
      mipmapped == GrMipmapped::kYes);
  SkASSERT(individualMipOffsets.count() == numMipLevels);

  size_t alignment =
      std::max(SkCompressedBlockSize(compression), this->mtlCaps().getMinBufferAlignment());
  GrStagingBufferManager::Slice slice =
      fStagingBufferManager.allocateStagingBufferSlice(combinedBufferSize, alignment);
  if (!slice.fBuffer) {
    return false;
...
bool GrMtlGpu::compile(const GrProgramDesc& desc, const GrProgramInfo& programInfo) {
  ...
  auto pipelineState =
      this->resourceProvider().findOrCreateCompatiblePipelineState(desc, programInfo, &stat);
  if (!pipelineState) {
    return false;
  }
...
GrBackendRenderTarget GrMtlGpu::createTestingOnlyBackendRenderTarget(
  ...
  GrMtlTextureInfo info;
  if (!this->createMtlTextureForBackendSurface(
          format, dimensions, sampleCnt, GrTexturable::kNo, GrRenderable::kYes, GrMipmapped::kNo,
          &info)) {
    return {};
  }
...
void GrMtlGpu::copySurfaceAsBlit(
    GrSurface* dst, GrSurface* src, GrMtlAttachment* dstAttachment, GrMtlAttachment* srcAttachment,
    const SkIRect& srcRect, const SkIPoint& dstPoint) {
#ifdef SK_DEBUG
  SkASSERT(this->mtlCaps().canCopyAsBlit(
      dstAttachment->mtlFormat(), dstAttachment->numSamples(), srcAttachment->mtlFormat(),
      dstAttachment->numSamples(), srcRect, dstPoint, dst == src));
#endif
...
bool GrMtlGpu::onCopySurface(
  ...
  if (this->mtlCaps().canCopyAsResolve(
          dstFormat, dstSampleCnt, srcFormat, srcSampleCnt, SkToBool(srcRT), src->dimensions(),
  ...
  if (this->mtlCaps().canCopyAsBlit(
          dstFormat, dstSampleCnt, srcFormat, srcSampleCnt, srcRect, dstPoint,
...
bool GrMtlGpu::onReadPixels(
  ...
  sk_sp<GrGpuBuffer> transferBuffer = resourceProvider->createBuffer(
      transBufferImageBytes, GrGpuBufferType::kXferGpuToCpu, kDynamic_GrAccessPattern);
  ...
  if (!this->readOrTransferPixels(
          surface, rect, dstColorType, grMtlBuffer->mtlBuffer(), 0, transBufferImageBytes,
          transBufferRowBytes)) {
    return false;
  }
  ...
  SkRectMemcpy(
      buffer, rowBytes, mappedMemory, transBufferRowBytes, transBufferRowBytes, rect.height());

  return true;
}

bool GrMtlGpu::onTransferPixelsTo(
...
bool GrMtlGpu::onTransferPixelsFrom(
  ...
  return this->readOrTransferPixels(
      surface, rect, bufferColorType, grMtlBuffer->mtlBuffer(), offset, transBufferImageBytes,
      transBufferRowBytes);
}

bool GrMtlGpu::readOrTransferPixels(
    GrSurface* surface, SkIRect rect, GrColorType dstColorType, id<MTLBuffer> transferBuffer,
...
void GrMtlGpu::onResolveRenderTarget(GrRenderTarget* target, const SkIRect&) {
  ...
  this->resolve(
      static_cast<GrMtlRenderTarget*>(target)->resolveAttachment(),
      static_cast<GrMtlRenderTarget*>(target)->colorAttachment());
}

void GrMtlGpu::resolve(GrMtlAttachment* resolveAttachment, GrMtlAttachment* msaaAttachment) {
...
GrMtlRenderCommandEncoder* GrMtlGpu::loadMSAAFromResolve(
  ...
  auto renderPipeline = this->resourceProvider().findOrCreateMSAALoadPipeline(
      mtlDst->mtlFormat(), dst->numSamples(), stencilFormat);
  ...
  auto renderCmdEncoder = this->commandBuffer()->getRenderCommandEncoder(renderPassDesc, nullptr);
  if (!renderCmdEncoder) {
    return nullptr;
  }
...
#  if defined(SK_BUILD_FOR_MAC) && __MAC_OS_X_VERSION_MAX_ALLOWED >= 101500
  if (@available(macOS 10.15, *)) {
    switch (fDevice.location) {
      case MTLDeviceLocationBuiltIn: writer->appendString("location", "builtIn"); break;
      case MTLDeviceLocationSlot: writer->appendString("location", "slot"); break;
      case MTLDeviceLocationExternal: writer->appendString("location", "external"); break;
      case MTLDeviceLocationUnspecified: writer->appendString("location", "unspecified"); break;
      default: writer->appendString("location", "unknown"); break;
    }
...
#  endif  // SK_BUILD_FOR_MAC
  ...
  if (@available(macOS 10.13, iOS 11.0, *)) {
    writer->appendBool(
        "areProgrammableSamplePositionsSupported", fDevice.areProgrammableSamplePositionsSupported);
    writer->appendBool("areRasterOrderGroupsSupported", fDevice.areRasterOrderGroupsSupported);
  }
#  ifdef SK_BUILD_FOR_MAC
  if (@available(macOS 10.11, *)) {
    writer->appendBool(
        "isDepth24Stencil8PixelFormatSupported", fDevice.isDepth24Stencil8PixelFormatSupported);
  }
#    if __MAC_OS_X_VERSION_MAX_ALLOWED >= 101500
  if (@available(macOS 10.15, *)) {
    writer->appendBool("areBarycentricCoordsSupported", fDevice.areBarycentricCoordsSupported);
    writer->appendBool(
        "supportsShaderBarycentricCoordinates", fDevice.supportsShaderBarycentricCoordinates);
  }
#    endif
#  endif  // SK_BUILD_FOR_MAC
  if (@available(macOS 10.14, iOS 12.0, *)) {
  ...
  if (@available(macOS 10.13, iOS 11.0, *)) {
    switch (fDevice.readWriteTextureSupport) {
      ...
      case MTLReadWriteTextureTierNone:
        ...
        break;
      default: writer->appendString("readWriteTextureSupport", "unknown"); break;
    }
    switch (fDevice.argumentBuffersSupport) {
      case MTLArgumentBuffersTier1: writer->appendString("argumentBuffersSupport", "tier1"); break;
      case MTLArgumentBuffersTier2: writer->appendString("argumentBuffersSupport", "tier2"); break;
      default: writer->appendString("argumentBuffersSupport", "unknown"); break;
    }
  }
  if (@available(macOS 10.14, iOS 12.0, *)) {
