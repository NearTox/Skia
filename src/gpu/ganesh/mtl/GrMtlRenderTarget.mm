...
GrMtlRenderTarget::GrMtlRenderTarget(
    GrMtlGpu* gpu, SkISize dimensions, sk_sp<GrMtlAttachment> colorAttachment,
    sk_sp<GrMtlAttachment> resolveAttachment, Wrapped, std::string_view label)
...
GrMtlRenderTarget::GrMtlRenderTarget(
    GrMtlGpu* gpu, SkISize dimensions, sk_sp<GrMtlAttachment> colorAttachment,
    sk_sp<GrMtlAttachment> resolveAttachment, std::string_view label)
    : GrSurface(gpu, dimensions, GrProtected::kNo, label),
      GrRenderTarget(gpu, dimensions, colorAttachment->numSamples(), GrProtected::kNo, label),
      fColorAttachment(std::move(colorAttachment)),
      fResolveAttachment(std::move(resolveAttachment)) {}
...
sk_sp<GrMtlRenderTarget> GrMtlRenderTarget::MakeWrappedRenderTarget(
  ...
  sk_sp<GrMtlAttachment> textureAttachment = GrMtlAttachment::MakeWrapped(
      gpu, dimensions, texture, GrAttachment::UsageFlags::kColorAttachment, GrWrapCacheable::kNo);
  ...
  if (sampleCnt > 1) {
    if ([texture sampleCount] == 1) {
      ...
      mtlRT = new GrMtlRenderTarget(
          gpu, dimensions, std::move(colorAttachment), std::move(textureAttachment), kWrapped,
          /*label=*/{});
      mtlRT->setRequiresManualMSAAResolve();
    } else {
      SkASSERT(sampleCnt == static_cast<int>([texture sampleCount]));
      mtlRT = new GrMtlRenderTarget(
          gpu, dimensions, std::move(textureAttachment), nil, kWrapped, /*label=*/{});
    }
  } else {
    mtlRT = new GrMtlRenderTarget(
        gpu, dimensions, std::move(textureAttachment), nil, kWrapped, /*label=*/{});
...
const GrMtlFramebuffer* GrMtlRenderTarget::getFramebuffer(bool withResolve, bool withStencil) {
  int cacheIndex = renderpass_features_to_index(withResolve, withStencil);
  ...
  GrMtlAttachment* stencil =
      withStencil ? static_cast<GrMtlAttachment*>(this->getStencilAttachment()) : nullptr;
  fCachedFramebuffers[cacheIndex] = GrMtlFramebuffer::Make(colorAttachment, resolve, stencil);
  return fCachedFramebuffers[cacheIndex].get();
}
