...
void GrMtlPipelineStateDataManager::uploadAndBindUniformBuffers(
    GrMtlGpu* gpu, GrMtlRenderCommandEncoder* renderCmdEncoder) const {
  if (fUniformSize && fUniformsDirty) {
    ...
    if (@available(macOS 10.11, iOS 8.3, *)) {
      if (fUniformSize <= gpu->caps()->maxPushConstantsSize()) {
        renderCmdEncoder->setVertexBytes(
            fUniformData.get(), fUniformSize, GrMtlUniformHandler::kUniformBinding);
    ...
    renderCmdEncoder->setVertexBuffer(
        buffer->mtlBuffer(), slice.fOffset, GrMtlUniformHandler::kUniformBinding);
    renderCmdEncoder->setFragmentBuffer(
        buffer->mtlBuffer(), slice.fOffset, GrMtlUniformHandler::kUniformBinding);
  }
}
