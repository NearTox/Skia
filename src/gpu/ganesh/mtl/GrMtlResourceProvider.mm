...
GrMtlResourceProvider::GrMtlResourceProvider(GrMtlGpu* gpu) : fGpu(gpu) {
  fPipelineStateCache.reset(new PipelineStateCache(gpu));
}

GrMtlPipelineState* GrMtlResourceProvider::findOrCreateCompatiblePipelineState(
    const GrProgramDesc& programDesc, const GrProgramInfo& programInfo,
...
const GrMtlRenderPipeline* GrMtlResourceProvider::findOrCreateMSAALoadPipeline(
  ...
  pipelineDescriptor.vertexFunction = [fMSAALoadLibrary newFunctionWithName:@"vertexMain"];
  pipelineDescriptor.fragmentFunction = [fMSAALoadLibrary newFunctionWithName:@"fragmentMain"];
  ...
  NSError* error;
  auto pso = [fGpu->device() newRenderPipelineStateWithDescriptor:pipelineDescriptor error:&error];
  if (!pso) {
    SkDebugf(
...
struct GrMtlResourceProvider::PipelineStateCache::Entry {
  Entry(GrMtlPipelineState* pipelineState) : fPipelineState(pipelineState) {}
  Entry(const GrMtlPrecompiledLibraries& precompiledLibraries)
      : fPipelineState(nullptr), fPrecompiledLibraries(precompiledLibraries) {}
...
GrMtlResourceProvider::PipelineStateCache::~PipelineStateCache() { SkASSERT(0 == fMap.count()); }
...
GrMtlPipelineState* GrMtlResourceProvider::PipelineStateCache::onRefPipelineState(
    const GrProgramDesc& desc, const GrProgramInfo& programInfo, Stats::ProgramCacheResult* stat) {
  *stat = Stats::ProgramCacheResult::kHit;
  ...
  if (entry && !(*entry)->fPipelineState) {
    ...
    (*entry)->fPipelineState.reset(
        GrMtlPipelineStateBuilder::CreatePipelineState(fGpu, desc, programInfo, precompiledLibs));
...
bool GrMtlResourceProvider::PipelineStateCache::precompileShader(
