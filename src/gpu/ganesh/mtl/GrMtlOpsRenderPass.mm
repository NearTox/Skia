...
GrMtlOpsRenderPass::GrMtlOpsRenderPass(
    GrMtlGpu* gpu, GrRenderTarget* rt, sk_sp<GrMtlFramebuffer> framebuffer, GrSurfaceOrigin origin,
    const GrOpsRenderPass::LoadAndStoreInfo& colorInfo,
    const GrOpsRenderPass::StencilLoadAndStoreInfo& stencilInfo)
    : INHERITED(rt, origin), fGpu(gpu), fFramebuffer(std::move(framebuffer)) {
...
static MTLPrimitiveType gr_to_mtl_primitive(GrPrimitiveType primitiveType) {
  const static MTLPrimitiveType mtlPrimitiveType[]{
      MTLPrimitiveTypeTriangle, MTLPrimitiveTypeTriangleStrip, MTLPrimitiveTypePoint,
...
bool GrMtlOpsRenderPass::onBindPipeline(
    const GrProgramInfo& programInfo, const SkRect& drawBounds) {
  const GrMtlCaps& caps = fGpu->mtlCaps();
  GrProgramDesc programDesc =
      caps.makeDesc(fRenderTarget, programInfo, GrCaps::ProgramDescOverrideFlags::kNone);
  if (!programDesc.isValid()) {
    return false;
  }

  fActivePipelineState =
      fGpu->resourceProvider().findOrCreateCompatiblePipelineState(programDesc, programInfo);
  ...
  if (!fActiveRenderCmdEncoder) {
    fActiveRenderCmdEncoder =
        fGpu->commandBuffer()->getRenderCommandEncoder(fRenderPassDesc, fActivePipelineState, this);
    if (!fActiveRenderCmdEncoder) {
      return false;
    }
...
#endif
  fActivePipelineState->setDrawState(
      fActiveRenderCmdEncoder, programInfo.pipeline().writeSwizzle(),
  ...
  if (!programInfo.pipeline().isScissorTestEnabled()) {
    ...
    GrMtlPipelineState::SetDynamicScissorRectState(
        fActiveRenderCmdEncoder, dimensions, fOrigin,
        SkIRect::MakeWH(dimensions.width(), dimensions.height()));
  }
...
void GrMtlOpsRenderPass::onSetScissorRect(const SkIRect& scissor) {
  ...
  GrMtlPipelineState::SetDynamicScissorRectState(
      fActiveRenderCmdEncoder, fFramebuffer->colorAttachment()->dimensions(), fOrigin, scissor);
}

bool GrMtlOpsRenderPass::onBindTextures(
...
void GrMtlOpsRenderPass::setupRenderPass(
  ...
  if (!fActiveRenderCmdEncoder) {
    ...
    if (colorInfo.fLoadOp == GrLoadOp::kClear || stencilInfo.fLoadOp == GrLoadOp::kClear) {
      fBounds = SkRect::MakeWH(color->dimensions().width(), color->dimensions().height());
      fActiveRenderCmdEncoder =
          fGpu->commandBuffer()->getRenderCommandEncoder(fRenderPassDesc, nullptr, this);
    } else {
      fBounds.setEmpty();
...
GrMtlRenderCommandEncoder* GrMtlOpsRenderPass::setupResolve() {
  ...
  if (resolve) {
    ...
    if (colorAttachment.loadAction == MTLLoadActionLoad) {
      ...
      return fGpu->loadMSAAFromResolve(
          color, resolve, nativeBounds, fRenderPassDesc.stencilAttachment);
    }
  }
...
void GrMtlOpsRenderPass::onBindBuffers(
...
void GrMtlOpsRenderPass::onDrawIndexed(
  ...
  this->setVertexBuffer(
      fActiveRenderCmdEncoder, fActiveVertexBuffer.get(), fCurrentVertexStride * baseVertex, 0);
  ...
  fActiveRenderCmdEncoder->drawIndexedPrimitives(
      fActivePrimitiveType, indexCount, MTLIndexTypeUInt16, indexBuffer, indexOffset);
  fGpu->stats()->incNumDraws();
#ifdef SK_ENABLE_MTL_DEBUG_INFO
  SkASSERT(fDebugGroupActive);
  fActiveRenderCmdEncoder->popDebugGroup();
...
void GrMtlOpsRenderPass::onDrawInstanced(
  ...
  if (@available(macOS 10.11, iOS 9.0, *)) {
    fActiveRenderCmdEncoder->drawPrimitives(
        fActivePrimitiveType, baseVertex, vertexCount, instanceCount, baseInstance);
  } else {
    SkASSERT(false);
  }
  fGpu->stats()->incNumDraws();
...
void GrMtlOpsRenderPass::onDrawIndexedInstanced(
  ...
  if (@available(macOS 10.11, iOS 9.0, *)) {
    fActiveRenderCmdEncoder->drawIndexedPrimitives(
        fActivePrimitiveType, indexCount, MTLIndexTypeUInt16, mtlIndexBuffer->mtlBuffer(),
        indexOffset, instanceCount, baseVertex, baseInstance);
  } else {
    SkASSERT(false);
  }
  fGpu->stats()->incNumDraws();
...
void GrMtlOpsRenderPass::onDrawIndirect(
    const GrBuffer* drawIndirectBuffer, size_t bufferOffset, int drawCount) {
  ...
  while (drawCount >= 1) {
    if (@available(macOS 10.11, iOS 9.0, *)) {
      fActiveRenderCmdEncoder->drawPrimitives(
          fActivePrimitiveType, mtlIndirectBuffer->mtlBuffer(), bufferOffset);
    } else {
      SkASSERT(false);
    }
    drawCount--;
...
void GrMtlOpsRenderPass::onDrawIndexedIndirect(
    const GrBuffer* drawIndirectBuffer, size_t bufferOffset, int drawCount) {
  SkASSERT(fGpu->caps()->nativeDrawIndirectSupport());
  SkASSERT(fActivePipelineState);
  SkASSERT(nil != fActiveRenderCmdEncoder);
  ...
  while (drawCount >= 1) {
    if (@available(macOS 10.11, iOS 9.0, *)) {
      fActiveRenderCmdEncoder->drawIndexedPrimitives(
          fActivePrimitiveType, MTLIndexTypeUInt16, mtlIndexBuffer->mtlBuffer(), indexOffset,
...
void GrMtlOpsRenderPass::setVertexBuffer(
    GrMtlRenderCommandEncoder* encoder, const GrBuffer* buffer, size_t vertexOffset,
