...
GrMtlTextureRenderTarget::GrMtlTextureRenderTarget(
    GrMtlGpu* gpu, SkBudgeted budgeted, SkISize dimensions, sk_sp<GrMtlAttachment> texture,
    ...
    : GrSurface(gpu, dimensions, GrProtected::kNo, label),
      ...
      GrMtlRenderTarget(
          gpu, dimensions, std::move(colorAttachment), std::move(resolveAttachment), label) {
  this->registerWithCache(budgeted);
}

GrMtlTextureRenderTarget::GrMtlTextureRenderTarget(
    GrMtlGpu* gpu, SkISize dimensions, sk_sp<GrMtlAttachment> texture,
    sk_sp<GrMtlAttachment> colorAttachment, sk_sp<GrMtlAttachment> resolveAttachment,
    GrMipmapStatus mipmapStatus, GrWrapCacheable cacheable, std::string_view label)
    : GrSurface(gpu, dimensions, GrProtected::kNo, label),
      GrMtlTexture(gpu, dimensions, std::move(texture), mipmapStatus, label),
      GrMtlRenderTarget(
          gpu, dimensions, std::move(colorAttachment), std::move(resolveAttachment), label) {
...
bool create_rt_attachments(
    GrMtlGpu* gpu, SkISize dimensions, MTLPixelFormat format, int sampleCnt,
...
sk_sp<GrMtlTextureRenderTarget> GrMtlTextureRenderTarget::MakeNewTextureRenderTarget(
    GrMtlGpu* gpu, SkBudgeted budgeted, SkISize dimensions, int sampleCnt, MTLPixelFormat format,
    uint32_t mipLevels, GrMipmapStatus mipmapStatus, std::string_view label) {
  sk_sp<GrMtlAttachment> textureAttachment = GrMtlAttachment::MakeTexture(
  ...
  if (!create_rt_attachments(
          gpu, dimensions, format, sampleCnt, textureAttachment, &colorAttachment,
          &resolveAttachment)) {
    return nullptr;
  }
...
sk_sp<GrMtlTextureRenderTarget> GrMtlTextureRenderTarget::MakeWrappedTextureRenderTarget(
  ...
  GrMipmapStatus mipmapStatus =
      texture.mipmapLevelCount > 1 ? GrMipmapStatus::kDirty : GrMipmapStatus::kNotAllocated;
  GrAttachment::UsageFlags textureUsageFlags =
      GrAttachment::UsageFlags::kTexture | GrAttachment::UsageFlags::kColorAttachment;
  ...
  sk_sp<GrMtlAttachment> resolveAttachment;
  if (!create_rt_attachments(
          gpu, dimensions, texture.pixelFormat, sampleCnt, textureAttachment, &colorAttachment,
          &resolveAttachment)) {
    return nullptr;
  }
  SkASSERT(colorAttachment);
