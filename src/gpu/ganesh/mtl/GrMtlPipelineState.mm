...
GrMtlPipelineState::GrMtlPipelineState(
    GrMtlGpu* gpu, sk_sp<GrMtlRenderPipeline> pipeline, MTLPixelFormat pixelFormat,
...
void GrMtlPipelineState::setData(GrMtlFramebuffer* framebuffer, const GrProgramInfo& programInfo) {
...
void GrMtlPipelineState::setTextures(
    const GrGeometryProcessor& geomProc, const GrPipeline& pipeline,
...
void GrMtlPipelineState::setDrawState(
    GrMtlRenderCommandEncoder* renderCmdEncoder, const skgpu::Swizzle& writeSwizzle,
...
void GrMtlPipelineState::setRenderTargetState(
    SkISize colorAttachmentDimensions, GrSurfaceOrigin origin) {
...
void GrMtlPipelineState::setBlendConstants(
    GrMtlRenderCommandEncoder* renderCmdEncoder, const skgpu::Swizzle& swizzle,
    const GrXferProcessor& xferProcessor) {
  if (!renderCmdEncoder) {
    return;
  }
    ...
    if (fStencil.isTwoSided()) {
      if (@available(macOS 10.11, iOS 9.0, *)) {
        renderCmdEncoder->setStencilFrontBackReferenceValues(
            fStencil.postOriginCCWFace(origin).fRef, fStencil.postOriginCWFace(origin).fRef);
      } else {
...
void GrMtlPipelineState::SetDynamicScissorRectState(
    GrMtlRenderCommandEncoder* renderCmdEncoder, SkISize colorAttachmentDimensions,
    GrSurfaceOrigin rtOrigin, SkIRect scissorRect) {
  if (!scissorRect.intersect(
          SkIRect::MakeWH(colorAttachmentDimensions.width(), colorAttachmentDimensions.height()))) {
