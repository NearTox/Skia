...
class GrPipeline {
 public:
   ...
   **/
  GrPipeline(
      GrScissorTest scissor, SkBlendMode blend, const skgpu::Swizzle& writeSwizzle,
      InputFlags flags = InputFlags::kNone)
      : GrPipeline(scissor, GrPorterDuffXPFactory::MakeNoCoverageXP(blend), writeSwizzle, flags) {}

  GrPipeline(
      GrScissorTest, sk_sp<const GrXferProcessor>, const skgpu::Swizzle& writeSwizzle,
  ...
  void setDstTextureUniforms(
      const GrGLSLProgramDataManager& pdm,
      GrGLSLBuiltinUniformHandles* fBuiltinUniformHandles) const;

 private:
  inline static constexpr uint8_t kLastInputFlag = (uint8_t)InputFlags::kSnapVerticesToPixelCenters;
