...
GrD3DRenderTarget::GrD3DRenderTarget(
    ...
    const GrD3DDescriptorHeap::CPUHandle& resolveRenderTargetView, Wrapped, std::string_view label)
...
sk_sp<GrD3DRenderTarget> GrD3DRenderTarget::MakeWrappedRenderTarget(
  ...
  if (sampleCnt != wrappedTextureSampleCnt) {
    ...
    d3dRT = new GrD3DRenderTarget(
        gpu, dimensions, info, std::move(state), msInfo, std::move(msState), msaaRenderTargetView,
