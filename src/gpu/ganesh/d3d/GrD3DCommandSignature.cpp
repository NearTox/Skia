...
sk_sp<GrD3DCommandSignature> GrD3DCommandSignature::Make(
    GrD3DGpu* gpu, ForIndexed forIndexed, unsigned int slot) {
  ...
  commandSigDesc.ByteStride =
      indexed ? sizeof(D3D12_DRAW_INDEXED_ARGUMENTS) : sizeof(D3D12_DRAW_ARGUMENTS);
  ...
  HRESULT hr =
      gpu->device()->CreateCommandSignature(&commandSigDesc, nullptr, IID_PPV_ARGS(&commandSig));
  if (!SUCCEEDED(hr)) {
    SkDebugf("Failed to create command signature.\n");
    return nullptr;
  }

  return sk_sp<GrD3DCommandSignature>(
      new GrD3DCommandSignature(std::move(commandSig), forIndexed, slot));
}
