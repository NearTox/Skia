...
GrThreadSafePipelineBuilder* GrD3DGpu::pipelineBuilder() { return nullptr; }

sk_sp<GrThreadSafePipelineBuilder> GrD3DGpu::refPipelineBuilder() { return nullptr; }

sk_sp<GrGpu> GrD3DGpu::Make(
    const GrD3DBackendContext& backendContext, const GrContextOptions& contextOptions,
    GrDirectContext* direct) {
  sk_sp<GrD3DMemoryAllocator> memoryAllocator = backendContext.fMemoryAllocator;
  if (!memoryAllocator) {
    // We were not given a memory allocator at creation
    memoryAllocator =
        GrD3DAMDMemoryAllocator::Make(backendContext.fAdapter.get(), backendContext.fDevice.get());
  }
  if (!memoryAllocator) {
...
GrD3DGpu::GrD3DGpu(
    GrDirectContext* direct, const GrContextOptions& contextOptions,
  ...
  GR_D3D_CALL_ERRCHECK(
      fDevice->CreateFence(fCurrentFenceValue, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&fFence)));
...
GrD3DGpu::~GrD3DGpu() { this->destroyResources(); }
...
GrOpsRenderPass* GrD3DGpu::onGetOpsRenderPass(
    GrRenderTarget* rt, bool /*useMSAASurface*/, GrAttachment*, GrSurfaceOrigin origin,
...
bool GrD3DGpu::submitDirectCommandList(SyncQueue sync) {
  ...
  new (fOutstandingCommandLists.push_back())
      OutstandingCommandList(std::move(fCurrentDirectCommandList), fence);
...
void GrD3DGpu::endRenderPass(
    GrRenderTarget* target, GrSurfaceOrigin origin, const SkIRect& bounds) {
  this->didWriteToSurface(target, origin, &bounds);
}

void GrD3DGpu::addFinishedProc(
...
sk_sp<GrD3DTexture> GrD3DGpu::createD3DTexture(
    SkISize dimensions, DXGI_FORMAT dxgiFormat, GrRenderable renderable, int renderTargetSampleCnt,
    SkBudgeted budgeted, GrProtected isProtected, int mipLevelCount, GrMipmapStatus mipmapStatus,
  ...
  if (renderable == GrRenderable::kYes) {
    return GrD3DTextureRenderTarget::MakeNewTextureRenderTarget(
        this, budgeted, dimensions, renderTargetSampleCnt, resourceDesc, isProtected, mipmapStatus,
        label);
  } else {
...
sk_sp<GrTexture> GrD3DGpu::onCreateTexture(
  ...
  sk_sp<GrD3DTexture> tex = this->createD3DTexture(
      dimensions, dxgiFormat, renderable, renderTargetSampleCnt, budgeted, isProtected,
...
static void copy_compressed_data(
    ...
    UINT* numRows, UINT64* rowSizeInBytes, const void* compressedData, int numMipLevels) {
  ...
  for (int currentMipLevel = 0; currentMipLevel < numMipLevels; currentMipLevel++) {
    ...
    SkRectMemcpy(
        dst, placedFootprints[currentMipLevel].Footprint.RowPitch, src,
        rowSizeInBytes[currentMipLevel], rowSizeInBytes[currentMipLevel], numRows[currentMipLevel]);
    src += numRows[currentMipLevel] * rowSizeInBytes[currentMipLevel];
  }
}

sk_sp<GrTexture> GrD3DGpu::onCreateCompressedTexture(
  ...
  SkASSERT(
      dataSize ==
      SkCompressedFormatDataSize(compression, dimensions, mipmapped == GrMipmapped::kYes));
  ...
  sk_sp<GrD3DTexture> d3dTex = this->createD3DTexture(
      dimensions, dxgiFormat, GrRenderable::kNo, 1, budgeted, isProtected, mipLevelCount,
      mipmapStatus,
  ...
  fDevice->GetCopyableFootprints(
      &desc, 0, mipLevelCount, 0, placedFootprints.get(), numRows.get(), rowSizeInBytes.get(),
      &combinedBufferSize);
  ...
  copy_compressed_data(
      bufferData, desc.Format, placedFootprints.get(), numRows.get(), rowSizeInBytes.get(), data,
      mipLevelCount);
  ...
  fCurrentDirectCommandList->copyBufferToTexture(
      d3dBuffer, d3dTex.get(), mipLevelCount, placedFootprints.get(), 0, 0);
...
void GrD3DGpu::copySurfaceAsCopyTexture(
    GrSurface* dst, GrSurface* src, GrD3DTextureResource* dstResource,
    GrD3DTextureResource* srcResource, const SkIRect& srcRect, const SkIPoint& dstPoint) {
#ifdef SK_DEBUG
  int dstSampleCnt = get_surface_sample_cnt(dst);
  int srcSampleCnt = get_surface_sample_cnt(src);
  ...
  fCurrentDirectCommandList->copyTextureRegionToTexture(
      dstResource->resource(), &dstLocation, dstPoint.fX, dstPoint.fY, srcResource->resource(),
      &srcLocation, &srcBox);
...
void GrD3DGpu::copySurfaceAsResolve(
    GrSurface* dst, GrSurface* src, const SkIRect& srcRect, const SkIPoint& dstPoint) {
...
void GrD3DGpu::resolveTexture(
    GrSurface* dst, int32_t dstX, int32_t dstY, GrD3DRenderTarget* src, const SkIRect& srcIRect) {
  SkASSERT(dst);
  ...
  fCurrentDirectCommandList->resolveSubresourceRegion(
      dstTextureResource, dstX, dstY, src->msaaTextureResource(), &srcRect);
}
...
bool GrD3DGpu::onReadPixels(
  ...
  fDevice->GetCopyableFootprints(
      &desc, 0, 1, 0, &placedFootprint, nullptr, nullptr, &transferTotalBytes);
  SkASSERT(transferTotalBytes);
  // TODO: implement some way of reusing buffers instead of making a new one every time.
  sk_sp<GrGpuBuffer> transferBuffer = this->createBuffer(
      transferTotalBytes, GrGpuBufferType::kXferGpuToCpu, kDynamic_GrAccessPattern);
  ...
  SkRectMemcpy(
      buffer, rowBytes, mappedMemory, placedFootprint.Footprint.RowPitch, tightRowBytes,
      rect.height());
...
void GrD3DGpu::readOrTransferPixels(
    GrD3DTextureResource* texResource, SkIRect rect, sk_sp<GrGpuBuffer> transferBuffer,
  ...
  fCurrentDirectCommandList->copyTextureRegionToBuffer(
      transferBuffer, &dstLocation, 0, 0, texResource->resource(), &srcLocation, &srcBox);
}

bool GrD3DGpu::onWritePixels(
...
bool GrD3DGpu::uploadToTexture(
    GrD3DTexture* tex, SkIRect rect, GrColorType colorType, const GrMipLevel* texels,
    int mipLevelCount) {
  ...
  fDevice->GetCopyableFootprints(
      &desc, 0, mipLevelCount, 0, placedFootprints.get(), nullptr, nullptr, &combinedBufferSize);
  ...
  for (int currentMipLevel = 0; currentMipLevel < mipLevelCount; currentMipLevel++) {
    if (texels[currentMipLevel].fPixels) {
      ...
      SkRectMemcpy(
          dst, placedFootprints[currentMipLevel].Footprint.RowPitch, src, srcRowBytes, trimRowBytes,
          currentHeight);
    }
  ...
  ID3D12Resource* d3dBuffer = static_cast<GrD3DBuffer*>(slice.fBuffer)->d3dResource();
  fCurrentDirectCommandList->copyBufferToTexture(
      d3dBuffer, tex, mipLevelCount, placedFootprints.get(), rect.left(), rect.top());
...
bool GrD3DGpu::onTransferPixelsTo(
  ...
  desc.Height = rect.height();
  UINT64 totalBytes;
  fDevice->GetCopyableFootprints(&desc, 0, 1, 0, &placedFootprint, nullptr, nullptr, &totalBytes);
  ...
  fCurrentDirectCommandList->copyBufferToTexture(
      d3dBuffer, d3dTex, 1, &placedFootprint, rect.left(), rect.top());
...
bool GrD3DGpu::onTransferPixelsFrom(
  ...
  UINT64 transferTotalBytes;
  fDevice->GetCopyableFootprints(
      &desc, 0, 1, offset, &placedFootprint, nullptr, nullptr, &transferTotalBytes);
  SkASSERT(transferTotalBytes);
...
static bool check_rt_resource_info(
    const GrD3DCaps& caps, const GrD3DTextureResourceInfo& info, int sampleCnt) {
...
sk_sp<GrTexture> GrD3DGpu::onWrapBackendTexture(
    const GrBackendTexture& tex, GrWrapOwnership, GrWrapCacheable wrapType, GrIOType ioType) {
  ...
  SkASSERT(state);
  return GrD3DTexture::MakeWrappedTexture(
      this, tex.dimensions(), wrapType, ioType, textureInfo, std::move(state));
}

sk_sp<GrTexture> GrD3DGpu::onWrapCompressedBackendTexture(
...
sk_sp<GrTexture> GrD3DGpu::onWrapRenderableBackendTexture(
  ...
  return GrD3DTextureRenderTarget::MakeWrappedTextureRenderTarget(
      this, tex.dimensions(), sampleCnt, cacheable, textureInfo, std::move(state));
}
...
static bool is_odd(int x) { return x > 1 && SkToBool(x & 0x1); }
...
bool GrD3DGpu::onRegenerateMipMapLevels(GrTexture* tex) {
  ...
  if (!caps.isFormatTexturable(d3dTex->dxgiFormat()) || !caps.mipmapSupport()) {
    return false;
  }
  ...
  if (caps.isFormatUnorderedAccessible(originalFormat) &&
    ...
    uavTexture = GrD3DTexture::MakeNewTexture(
        this, SkBudgeted::kNo, tex->dimensions(), uavDesc, grProtected, GrMipmapStatus::kDirty,
    ...
    if (!caps.isFormatUnorderedAccessible(originalFormat) && is_bgra(originalFormat)) {
      ...
      bgraAliasTexture = GrD3DTexture::MakeAliasingTexture(
          this, uavTexture, originalDesc, D3D12_RESOURCE_STATE_COPY_DEST);
      // make the BGRA version the active alias
      this->currentCommandList()->aliasingBarrier(
          nullptr, nullptr, bgraAliasTexture->resource(), bgraAliasTexture->d3dResource());
      ...
      this->currentCommandList()->aliasingBarrier(
          bgraAliasTexture->resource(), bgraAliasTexture->d3dResource(), uavTexture->resource(),
  ...
  for (unsigned int dstMip = 1; dstMip < levelCount; ++dstMip) {
    ...
    this->currentCommandList()->setComputeRootConstantBufferView(
        (unsigned int)GrD3DRootSignature::ParamIndex::kConstantBufferView, constantsAddress);
  ...
  if (uavTexture.get() != d3dTex) {
    ...
    if (bgraAliasTexture) {
      // make the BGRA version the active alias
      this->currentCommandList()->aliasingBarrier(
          uavTexture->resource(), uavTexture->d3dResource(), bgraAliasTexture->resource(),
...
sk_sp<GrGpuBuffer> GrD3DGpu::onCreateBuffer(
    size_t sizeInBytes, GrGpuBufferType type, GrAccessPattern accessPattern) {
  return GrD3DBuffer::Make(this, sizeInBytes, type, accessPattern);
}

sk_sp<GrAttachment> GrD3DGpu::makeStencilAttachment(
...
bool GrD3DGpu::createTextureResourceForBackendSurface(
    DXGI_FORMAT dxgiFormat, SkISize dimensions, GrTexturable texturable, GrRenderable renderable,
    GrMipmapped mipmapped, int sampleCnt, GrD3DTextureResourceInfo* info, GrProtected isProtected) {
  ...
  if (!GrD3DTextureResource::InitTextureResourceInfo(
          this, resourceDesc, initialState, isProtected, clearValuePtr, info)) {
...
GrBackendTexture GrD3DGpu::onCreateBackendTexture(
    SkISize dimensions, const GrBackendFormat& format, GrRenderable renderable,
  ...
  GrD3DTextureResourceInfo info;
  if (!this->createTextureResourceForBackendSurface(
          dxgiFormat, dimensions, GrTexturable::kYes, renderable, mipmapped, 1, &info,
          isProtected)) {
    return {};
  }
...
static bool copy_color_data(
    const GrD3DCaps& caps, char* mapPtr, DXGI_FORMAT dxgiFormat, SkISize dimensions,
    D3D12_PLACED_SUBRESOURCE_FOOTPRINT* placedFootprints, std::array<float, 4> color) {
...
bool GrD3DGpu::onClearBackendTexture(
  ...
  SkASSERT(state);
  sk_sp<GrD3DTexture> texture = GrD3DTexture::MakeWrappedTexture(
      this, backendTexture.dimensions(), GrWrapCacheable::kNo, kRW_GrIOType, info,
      std::move(state));
  if (!texture) {
    return false;
  }
  ...
  fDevice->GetCopyableFootprints(
      ...
      /* first resource  */ 0,
      /* mip level count */ 1,
      /* base offset     */ 0, placedFootprints.get(), &numRows, &rowSizeInBytes,
      &combinedBufferSize);
  SkASSERT(combinedBufferSize);
  ...
  SkASSERT(bufferData);
  if (!copy_color_data(
          this->d3dCaps(), bufferData, info.fFormat, backendTexture.dimensions(), placedFootprints,
          color)) {
    return false;
  }
  ...
  cmdList->copyBufferToTexture(
      d3dBuffer, texture.get(), mipLevelCount, placedFootprints.get(),
      /*left*/ 0,
      /*top */ 0);
...
bool GrD3DGpu::onUpdateCompressedBackendTexture(
    const GrBackendTexture& backendTexture, sk_sp<skgpu::RefCntedCallback> finishedCallback,
  ...
  if (backendTexture.hasMipmaps()) {
    mipLevelCount = SkMipmap::ComputeLevelCount(
                        backendTexture.dimensions().width(), backendTexture.dimensions().height()) +
                    1;
  }
  SkASSERT(mipLevelCount == info.fLevelCount);
  SkAutoTMalloc<D3D12_PLACED_SUBRESOURCE_FOOTPRINT> placedFootprints(mipLevelCount);
  ...
  SkASSERT(bufferData);
  copy_compressed_data(
      bufferData, info.fFormat, placedFootprints.get(), numRows.get(), rowSizeInBytes.get(), data,
      info.fLevelCount);
  ...
  ID3D12Resource* d3dBuffer = static_cast<GrD3DBuffer*>(slice.fBuffer)->d3dResource();
  cmdList->copyBufferToTexture(
      d3dBuffer, texture.get(), mipLevelCount, placedFootprints.get(), 0, 0);
...
bool GrD3DGpu::compile(const GrProgramDesc&, const GrProgramInfo&) { return false; }
...
GrBackendRenderTarget GrD3DGpu::createTestingOnlyBackendRenderTarget(
  ...
  GrD3DTextureResourceInfo info;
  if (!this->createTextureResourceForBackendSurface(
          dxgiFormat, dimensions, GrTexturable::kNo, GrRenderable::kYes, GrMipmapped::kNo,
...
void GrD3DGpu::addResourceBarriers(
...
void GrD3DGpu::addBufferResourceBarriers(
...
bool GrD3DGpu::waitFence(GrFence fence) { return (fFence->GetCompletedValue() >= fence); }

void GrD3DGpu::finishOutstandingGpuWork() { this->waitForQueueCompletion(); }
