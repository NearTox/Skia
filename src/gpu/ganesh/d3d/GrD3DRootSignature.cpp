...
sk_sp<GrD3DRootSignature> GrD3DRootSignature::Make(
    GrD3DGpu* gpu, int numTextureSamplers, int numUAVs) {
  ...
  if (numTextureSamplers) {
    ...
    for (int i = 0; i < numTextureSamplers; ++i) {
      ...
      // In the descriptor table the descriptors will all be contiguous.
      shaderViewRanges[i].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
    }
  }
  if (numUAVs) {
  ...
  HRESULT hr = D3D12SerializeRootSignature(
      &rootDesc, D3D_ROOT_SIGNATURE_VERSION_1_0, &rootSigBinary, &error);
  ...
  hr = gpu->device()->CreateRootSignature(
      0, rootSigBinary->GetBufferPointer(), rootSigBinary->GetBufferSize(), IID_PPV_ARGS(&rootSig));
  if (!SUCCEEDED(hr)) {
    SkDebugf("Failed to create root signature.\n");
    return nullptr;
  }

  return sk_sp<GrD3DRootSignature>(
      new GrD3DRootSignature(std::move(rootSig), numTextureSamplers, numUAVs));
}

GrD3DRootSignature::GrD3DRootSignature(
    gr_cp<ID3D12RootSignature> rootSig, int numTextureSamplers, int numUAVs)
