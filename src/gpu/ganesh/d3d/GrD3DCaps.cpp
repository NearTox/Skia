...
GrD3DCaps::GrD3DCaps(
    const GrContextOptions& contextOptions, IDXGIAdapter1* adapter, ID3D12Device* device)
...
void GrD3DCaps::init(
  ...
  GR_D3D_CALL_ERRCHECK(device->CheckFeatureSupport(
      D3D12_FEATURE_D3D12_OPTIONS2, &optionsDesc2, sizeof(optionsDesc2)));
  fResolveSubresourceRegionSupport =
      (optionsDesc2.ProgrammableSamplePositionsTier !=
...
static constexpr DXGI_FORMAT kDxgiFormats[] = {
    DXGI_FORMAT_R8G8B8A8_UNORM,      DXGI_FORMAT_R8_UNORM,           DXGI_FORMAT_B8G8R8A8_UNORM,
    DXGI_FORMAT_B5G6R5_UNORM,        DXGI_FORMAT_R16G16B16A16_FLOAT, DXGI_FORMAT_R16_FLOAT,
    DXGI_FORMAT_R8G8_UNORM,          DXGI_FORMAT_R10G10B10A2_UNORM,  DXGI_FORMAT_B4G4R4A4_UNORM,
    DXGI_FORMAT_R8G8B8A8_UNORM_SRGB, DXGI_FORMAT_BC1_UNORM,          DXGI_FORMAT_R16_UNORM,
    DXGI_FORMAT_R16G16_UNORM,        DXGI_FORMAT_R16G16B16A16_UNORM, DXGI_FORMAT_R16G16_FLOAT};
...
GrD3DCaps::FormatInfo& GrD3DCaps::getFormatInfo(DXGI_FORMAT format) {
  static_assert(
      SK_ARRAY_COUNT(kDxgiFormats) == GrD3DCaps::kNumDxgiFormats,
...
void GrD3DCaps::FormatInfo::InitFormatFlags(
    const D3D12_FEATURE_DATA_FORMAT_SUPPORT& formatSupport, uint16_t* flags) {
...
void GrD3DCaps::FormatInfo::initSampleCounts(
...
void GrD3DCaps::FormatInfo::init(
    const DXGI_ADAPTER_DESC& adapterDesc, ID3D12Device* device, DXGI_FORMAT format) {
  D3D12_FEATURE_DATA_FORMAT_SUPPORT formatSupportDesc;
  formatSupportDesc.Format = format;
  GR_D3D_CALL_ERRCHECK(device->CheckFeatureSupport(
...
bool GrD3DCaps::isFormatAsColorTypeRenderable(
...
int GrD3DCaps::getRenderTargetSampleCount(int requestedCount, const GrBackendFormat& format) const {
  DXGI_FORMAT dxgiFormat;
  if (!format.asDxgiFormat(&dxgiFormat)) {
    return 0;
  }
...
GrCaps::SupportedWrite GrD3DCaps::supportedWritePixelsColorType(
  ...
  size_t offsetAlignment =
      SkAlignTo(GrDxgiFormatBytesPerBlock(dxgiFormat), D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT);
...
skgpu::Swizzle GrD3DCaps::onGetReadSwizzle(
  ...
  SkDEBUGFAILF(
      "Illegal color type (%d) and format (%d) combination.", (int)colorType, (int)dxgiFormat);
  return {};
}

skgpu::Swizzle GrD3DCaps::getWriteSwizzle(
    const GrBackendFormat& format, GrColorType colorType) const {
  DXGI_FORMAT dxgiFormat;
  SkAssertResult(format.asDxgiFormat(&dxgiFormat));
  const auto& info = this->getFormatInfo(dxgiFormat);
...
void GrD3DCaps::addExtraSamplerKey(
