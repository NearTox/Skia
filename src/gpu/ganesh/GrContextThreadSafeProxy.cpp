...
void GrContextThreadSafeProxy::init(
    sk_sp<const GrCaps> caps, sk_sp<GrThreadSafePipelineBuilder> pipelineBuilder) {
  fCaps = std::move(caps);
  fTextBlobRedrawCoordinator = std::make_unique<sktext::gpu::TextBlobRedrawCoordinator>(fContextID);
...
SkSurfaceCharacterization GrContextThreadSafeProxy::createCharacterization(
    ...
    bool vkRTSupportsInputAttachment, bool forVulkanSecondaryCommandBuffer) {
  SkASSERT(fCaps);
  ...
  return SkSurfaceCharacterization(
      sk_ref_sp<GrContextThreadSafeProxy>(this), cacheMaxResourceBytes, ii, backendFormat, origin,
      ...
      isProtected, surfaceProps);
}

GrBackendFormat GrContextThreadSafeProxy::defaultBackendFormat(
  ...
  SkASSERT(
      renderable == GrRenderable::kNo || fCaps->isFormatAsColorTypeRenderable(grColorType, format));

  return format;
}

GrBackendFormat GrContextThreadSafeProxy::compressedBackendFormat(
    SkImage::CompressionType c) const {
  SkASSERT(fCaps);
...
int GrContextThreadSafeProxy::maxSurfaceSampleCountForColorType(SkColorType colorType) const {
  ...
  GrBackendFormat format =
      fCaps->getDefaultBackendFormat(SkColorTypeToGrColorType(colorType), GrRenderable::kYes);
...
void GrContextThreadSafeProxyPriv::init(
    sk_sp<const GrCaps> caps, sk_sp<GrThreadSafePipelineBuilder> builder) const {
