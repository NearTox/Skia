...
static int num_channels(uint32_t channelFlags) {
  switch (channelFlags) {
    ...
    default: SkDEBUGFAILF("Unexpected channel combination 0x%08x", channelFlags); return 0;
  }
}
#endif

GrYUVATextureProxies::GrYUVATextureProxies(
    const SkYUVAInfo& yuvaInfo, sk_sp<GrSurfaceProxy> proxies[SkYUVAInfo::kMaxPlanes],
...
GrYUVATextureProxies::GrYUVATextureProxies(
    const SkYUVAInfo& yuvaInfo, GrSurfaceProxyView views[SkYUVAInfo::kMaxPlanes],
  ...
  for (int i = 0; i < n; ++i) {
    ...
    SkASSERT(
        num_channels(pixmapChannelMasks[i]) <=
