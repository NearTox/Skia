...
GrRecordingContext::GrRecordingContext(sk_sp<GrContextThreadSafeProxy> proxy, bool ddlRecording)
    : INHERITED(std::move(proxy)), fAuditTrail(new GrAuditTrail()), fArenas(ddlRecording) {
...
GrRecordingContext::Arenas::Arenas(
    SkArenaAlloc* recordTimeAllocator, sktext::gpu::SubRunAllocator* subRunAllocator)
    : fRecordTimeAllocator(recordTimeAllocator), fRecordTimeSubRunAllocator(subRunAllocator) {
...
sk_sp<const SkCapabilities> GrRecordingContext::skCapabilities() const { return this->refCaps(); }
...
void GrRecordingContext::DMSAAStats::dumpKeyValuePairs(
    SkTArray<SkString>* keys, SkTArray<double>* values) const {
