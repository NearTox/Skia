...
sk_sp<SkIDChangeListener> GrMakeUniqueKeyInvalidationListener(
    skgpu::UniqueKey* key, uint32_t contextID) {
  class Listener : public SkIDChangeListener {
   public:
    Listener(const skgpu::UniqueKey& key, uint32_t contextUniqueID) : fMsg(key, contextUniqueID) {}
  ...
  auto data = SkData::MakeWithProc(
      new sk_sp<Listener>(listener), sizeof(sk_sp<Listener>), invalidateListener, nullptr);
...
sk_sp<GrSurfaceProxy> GrCopyBaseMipMapToTextureProxy(
    GrRecordingContext* ctx, sk_sp<GrSurfaceProxy> baseProxy, GrSurfaceOrigin origin,
    SkBudgeted budgeted) {
  SkASSERT(baseProxy);
  ...
  auto copy = GrSurfaceProxy::Copy(
      ctx, std::move(baseProxy), origin, GrMipmapped::kYes, SkBackingFit::kExact, budgeted);
  if (!copy) {
    return nullptr;
  }
...
static GrMipmapped adjust_mipmapped(
    GrMipmapped mipmapped, const SkBitmap& bitmap, const GrCaps* caps) {
...
static sk_sp<GrTextureProxy> make_bmp_proxy(
    GrProxyProvider* proxyProvider, const SkBitmap& bitmap, GrColorType ct, GrMipmapped mipmapped,
...
std::tuple<GrSurfaceProxyView, GrColorType> GrMakeCachedBitmapProxyView(
    GrRecordingContext* rContext, const SkBitmap& bitmap, GrMipmapped mipmapped) {
  ...
  if (!proxy) {
    proxy = make_bmp_proxy(
        proxyProvider, bitmap, ct, mipmapped, SkBackingFit::kExact, SkBudgeted::kYes);
    if (!proxy) {
      return {};
    }
...
std::tuple<GrSurfaceProxyView, GrColorType> GrMakeUncachedBitmapProxyView(
    GrRecordingContext* rContext, const SkBitmap& bitmap, GrMipmapped mipmapped, SkBackingFit fit,
    SkBudgeted budgeted) {
  GrProxyProvider* proxyProvider = rContext->priv().proxyProvider();
  const GrCaps* caps = rContext->priv().caps();
...
static inline float dither_range_for_config(GrColorType dstColorType) {
  ...
  switch (dstColorType) {
    ...
    case GrColorType::kBGRA_4444: return 1 / 15.f;
    // 6 bit
    case GrColorType::kBGR_565: return 1 / 63.f;
    // 8 bit
    ...
    case GrColorType::kBGRA_8888: return 1 / 255.f;
    // 10 bit
    case GrColorType::kRGBA_1010102:
    case GrColorType::kBGRA_1010102: return 1 / 1023.f;
    // 16 bit
    case GrColorType::kAlpha_16:
    case GrColorType::kR_16:
    case GrColorType::kRG_1616:
    case GrColorType::kRGBA_16161616: return 1 / 32767.f;
    // Half
    ...
    case GrColorType::kRGBA_F32: return 0.f;  // no dithering
  }
  SkUNREACHABLE;
}
    ...
    constexpr DitherTable() : data() {
      for (int x = 0; x < 8; ++x) {
        for (int y = 0; y < 8; ++y) {
          ...
          unsigned int m = (y & 1) << 5 | (x & 1) << 4 | (y & 2) << 2 | (x & 2) << 1 |
                           (y & 4) >> 1 | (x & 4) >> 2;
...
static std::unique_ptr<GrFragmentProcessor> make_dither_effect(
    GrRecordingContext* rContext, std::unique_ptr<GrFragmentProcessor> inputFP, float range,
    const GrCaps* caps) {
  ...
  auto te =
      GrTextureEffect::Make(std::move(tex), kPremul_SkAlphaType, SkMatrix::I(), sampler, *caps);
  ...
  return GrSkSLFP::Make(
      effect, "Dither", std::move(inputFP), GrSkSLFP::OptFlags::kPreservesOpaqueInput, "range",
      range, "table", std::move(te));
}
#endif
...
static inline bool skpaint_to_grpaint_impl(
    ...
    std::optional<std::unique_ptr<GrFragmentProcessor>> shaderFP, SkBlender* primColorBlender,
    GrPaint* grPaint) {
  ...
  if (paintFP) {
    if (primColorBlender) {
      ...
      paintFP = as_BB(primColorBlender)
                    ->asFragmentProcessor(
                        std::move(paintFP),
                        /*dstFP=*/nullptr, fpArgs);
  ...
  if (colorFilter) {
    if (applyColorFilterToPaintColor) {
      ...
      auto [success, fp] =
          as_CFB(colorFilter)->asFragmentProcessor(std::move(paintFP), context, dstColorInfo);
      if (!success) {
        return false;
      }
      paintFP = std::move(fp);
  ...
  if (auto bm = skPaint.asBlendMode()) {
    ...
    paintFP =
        as_BB(skPaint.getBlender())
            ->asFragmentProcessor(std::move(paintFP), GrFragmentProcessor::SurfaceColor(), fpArgs);
...
bool SkPaintToGrPaint(
  ...
  return skpaint_to_grpaint_impl(
      context, dstColorInfo, skPaint, matrixProvider,
      /*shaderFP=*/std::nullopt,
      /*primColorBlender=*/nullptr, grPaint);
}
...
bool SkPaintToGrPaintReplaceShader(
    ...
    GrPaint* grPaint) {
  return skpaint_to_grpaint_impl(
      context, dstColorInfo, skPaint, matrixProvider, std::move(shaderFP),
      /*primColorBlender=*/nullptr, grPaint);
}
...
bool SkPaintToGrPaintWithBlend(
    GrRecordingContext* context, const GrColorInfo& dstColorInfo, const SkPaint& skPaint,
    const SkMatrixProvider& matrixProvider, SkBlender* primColorBlender, GrPaint* grPaint) {
  return skpaint_to_grpaint_impl(
      context, dstColorInfo, skPaint, matrixProvider,
      /*shaderFP=*/std::nullopt, primColorBlender, grPaint);
}
