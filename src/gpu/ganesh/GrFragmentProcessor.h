...
class GrFragmentProcessor : public GrProcessor {
 public:
  ...
  static GrFPResult Circle(
      std::unique_ptr<GrFragmentProcessor>, GrClipEdgeType, SkPoint center, float radius);

  static GrFPResult Ellipse(
      std::unique_ptr<GrFragmentProcessor>, GrClipEdgeType, SkPoint center, SkPoint radii,
      const GrShaderCaps&);
  ...
  bool willReadDstColor() const { return SkToBool(fFlags & kWillReadDstColor_Flag); }

  /** Does the SkSL for this FP take two colors as its input arguments? */
  bool isBlendFunction() const { return SkToBool(fFlags & kIsBlendFunction_Flag); }
  ...
  bool usesSampleCoords() const {
    return SkToBool(fFlags & (kUsesSampleCoordsDirectly_Flag | kUsesSampleCoordsIndirectly_Flag));
  }
  ...
  void visitWithImpls(
      const std::function<void(const GrFragmentProcessor&, ProgramImpl&)>&, ProgramImpl&) const;
 ...
 protected:
  ...
  void setWillReadDstColor() { fFlags |= kWillReadDstColor_Flag; }
  ...
  void setIsBlendFunction() { fFlags |= kIsBlendFunction_Flag; }
 ...
 public:
  ...
  struct EmitArgs {
    EmitArgs(
        ...
        const char* destColor, const char* sampleCoord)
  ...
  inline SkString invokeChild(
      int childIndex, EmitArgs& parentArgs, std::string_view skslCoords = {}) {
    return this->invokeChild(
        childIndex,
        /*inputColor=*/nullptr,
        /*destColor=*/nullptr, parentArgs, skslCoords);
  }

  inline SkString invokeChildWithMatrix(int childIndex, EmitArgs& parentArgs) {
    return this->invokeChildWithMatrix(
        childIndex,
        /*inputColor=*/nullptr,
  ...
  inline SkString invokeChild(
      int childIndex, const char* inputColor, EmitArgs& parentArgs,
      std::string_view skslCoords = {}) {
    return this->invokeChild(
        childIndex, inputColor,
        /*destColor=*/nullptr, parentArgs, skslCoords);
  }
   ...
   */
  SkString invokeChild(
      int childIndex, const char* inputColor, const char* destColor, EmitArgs& parentArgs,
      std::string_view skslCoords = {});
  ...
  class Iter {
   public:
    ...
    ProgramImpl* operator->() const;
    Iter& operator++();
    explicit operator bool() const noexcept { return !fFPStack.empty(); }
