...
class GrDrawingManager {
 public:
  ...
  sk_sp<skgpu::v1::OpsTask> newOpsTask(GrSurfaceProxyView, sk_sp<GrArenas> arenas);
  ...
  void newTextureResolveRenderTask(
      sk_sp<GrSurfaceProxy> proxy, GrSurfaceProxy::ResolveFlags, const GrCaps&);
  ...
  // signaled.
  void newWaitRenderTask(
      sk_sp<GrSurfaceProxy> proxy, std::unique_ptr<std::unique_ptr<GrSemaphore>[]>,
      int numSemaphores);
  ...
  void newTransferFromRenderTask(
  ...
  sk_sp<GrRenderTask> newCopyRenderTask(
  ...
  PathRenderer* getPathRenderer(
      const PathRenderer::CanDrawPathArgs&, bool allowSW, PathRendererChain::DrawType,
...
#  endif
  ...
  void createDDLTask(
      sk_sp<const SkDeferredDisplayList>, sk_sp<GrRenderTargetProxy> newDest, SkIPoint offset);

 private:
#if SK_GPU_V1
  GrDrawingManager(
      GrRecordingContext*, const PathRendererChain::Options&, bool reduceOpsTaskSplitting);
#else
