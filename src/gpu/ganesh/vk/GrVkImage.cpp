...
sk_sp<GrVkImage> GrVkImage::MakeStencil(
    GrVkGpu* gpu, SkISize dimensions, int sampleCnt, VkFormat format) {
  VkImageUsageFlags vkUsageFlags =
      VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
  return GrVkImage::Make(
      gpu, dimensions, UsageFlags::kStencilAttachment, sampleCnt, format,
      /*mipLevels=*/1, vkUsageFlags, GrProtected::kNo, GrMemoryless::kNo, SkBudgeted::kYes);
}

sk_sp<GrVkImage> GrVkImage::MakeMSAA(
  ...
  return GrVkImage::Make(
      gpu, dimensions, UsageFlags::kColorAttachment, numSamples, format,
      /*mipLevels=*/1, vkUsageFlags, isProtected, memoryless, SkBudgeted::kYes);
}

sk_sp<GrVkImage> GrVkImage::MakeTexture(
    GrVkGpu* gpu, SkISize dimensions, VkFormat format, uint32_t mipLevels, GrRenderable renderable,
    int numSamples, SkBudgeted budgeted, GrProtected isProtected) {
  ...
  return GrVkImage::Make(
      gpu, dimensions, usageFlags, numSamples, format, mipLevels, vkUsageFlags, isProtected,
      GrMemoryless::kNo, budgeted);
}

static bool make_views(
    GrVkGpu* gpu, const GrVkImageInfo& info, GrAttachment::UsageFlags attachmentUsages,
    sk_sp<const GrVkImageView>* framebufferView, sk_sp<const GrVkImageView>* textureView) {
  ...
  if (SkToBool(attachmentUsages & GrAttachment::UsageFlags::kStencilAttachment) ||
    ...
    *framebufferView =
        GrVkImageView::Make(gpu, info.fImage, info.fFormat, viewType, 1, info.fYcbcrConversionInfo);
    if (!*framebufferView) {
      return false;
    }
  }
  ...
  if (attachmentUsages & GrAttachment::UsageFlags::kTexture) {
    *textureView = GrVkImageView::Make(
...
sk_sp<GrVkImage> GrVkImage::Make(
    ...
    uint32_t mipLevels, VkImageUsageFlags vkUsageFlags, GrProtected isProtected,
    GrMemoryless memoryless, SkBudgeted budgeted) {
  ...
  return sk_sp<GrVkImage>(new GrVkImage(
      gpu, dimensions, attachmentUsages, info, std::move(mutableState), std::move(framebufferView),
      std::move(textureView), budgeted,
      /*label=*/"MakeVkImage"));
}

sk_sp<GrVkImage> GrVkImage::MakeWrapped(
    ...
    GrWrapOwnership ownership, GrWrapCacheable cacheable, bool forSecondaryCB) {
  sk_sp<const GrVkImageView> framebufferView;
  sk_sp<const GrVkImageView> textureView;
  if (!forSecondaryCB) {
    if (!make_views(gpu, info, attachmentUsages, &framebufferView, &textureView)) {
  ...
  return sk_sp<GrVkImage>(new GrVkImage(
      gpu, dimensions, attachmentUsages, info, std::move(mutableState), std::move(framebufferView),
      std::move(textureView), backendOwnership, cacheable, forSecondaryCB,
...
GrVkImage::GrVkImage(
    ...
    sk_sp<GrBackendSurfaceMutableStateImpl> mutableState,
    sk_sp<const GrVkImageView> framebufferView, sk_sp<const GrVkImageView> textureView,
    SkBudgeted budgeted, std::string_view label)
    : GrAttachment(
          gpu, dimensions, supportedUsages, info.fSampleCount,
          info.fLevelCount > 1 ? GrMipmapped::kYes : GrMipmapped::kNo, info.fProtected, label,
...
GrVkImage::GrVkImage(
    ...
    sk_sp<GrBackendSurfaceMutableStateImpl> mutableState,
    sk_sp<const GrVkImageView> framebufferView, sk_sp<const GrVkImageView> textureView,
    GrBackendObjectOwnership ownership, GrWrapCacheable cacheable, bool forSecondaryCB,
...
void GrVkImage::init(GrVkGpu* gpu, bool forSecondaryCB) {
  ...
  if (fInfo.fImageUsageFlags & VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT) {
    ...
    if (fInfo.fAlloc.fFlags & GrVkAlloc::kLazilyAllocated_Flag) {
      ...
      SkASSERT(
          !SkToBool(fInfo.fImageUsageFlags & VK_IMAGE_USAGE_TRANSFER_DST_BIT) &&
...
VkAccessFlags GrVkImage::LayoutToSrcAccessMask(const VkImageLayout layout) {
  ...
  if (VK_IMAGE_LAYOUT_GENERAL == layout) {
    flags = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT |
...
// Enable the following block to test new devices to confirm their lazy images stay at 0 memory use.
  ...
  SkASSERT(
      ...
      (VK_IMAGE_LAYOUT_UNDEFINED != newLayout && VK_IMAGE_LAYOUT_PREINITIALIZED != newLayout));
  ...
  if (fInfo.fSharingMode == VK_SHARING_MODE_CONCURRENT) {
    ...
    if (newQueueFamilyIndex == VK_QUEUE_FAMILY_IGNORED || currentQueueIndex == gpu->queueIndex()) {
      SkASSERT(
          currentQueueIndex == VK_QUEUE_FAMILY_IGNORED ||
          currentQueueIndex == VK_QUEUE_FAMILY_EXTERNAL ||
          currentQueueIndex == VK_QUEUE_FAMILY_FOREIGN_EXT ||
        ...
        newQueueFamilyIndex == VK_QUEUE_FAMILY_FOREIGN_EXT) {
      SkASSERT(
          currentQueueIndex == VK_QUEUE_FAMILY_IGNORED || currentQueueIndex == gpu->queueIndex());
    }
  }
#endif
...
void GrVkImage::prepareForPresent(GrVkGpu* gpu) {
  ...
  this->setImageLayoutAndQueueIndex(
      gpu, layout, 0, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, false, fInitialQueueFamily);
}

void GrVkImage::prepareForExternal(GrVkGpu* gpu) {
  this->setImageLayoutAndQueueIndex(
      gpu, this->currentLayout(), 0, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, false,
      fInitialQueueFamily);
}
...
void GrVkImage::BorrowedResource::freeGPUData() const { this->invokeReleaseProc(); }

static void write_input_desc_set(
    GrVkGpu* gpu, VkImageView view, VkImageLayout layout, VkDescriptorSet descSet) {
...
gr_rp<const GrVkDescriptorSet> GrVkImage::inputDescSetForBlending(GrVkGpu* gpu) {
  ...
  write_input_desc_set(
      gpu, this->framebufferView()->imageView(), VK_IMAGE_LAYOUT_GENERAL,
...
gr_rp<const GrVkDescriptorSet> GrVkImage::inputDescSetForMSAALoad(GrVkGpu* gpu) {
  ...
  write_input_desc_set(
      gpu, this->framebufferView()->imageView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
