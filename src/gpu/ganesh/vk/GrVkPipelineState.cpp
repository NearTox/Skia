...
GrVkPipelineState::GrVkPipelineState(
    ...
    uint32_t uniformSize, bool usePushConstants, const UniformInfoArray& samplers,
...
bool GrVkPipelineState::setAndBindUniforms(
    GrVkGpu* gpu, SkISize colorAttachmentDimensions, const GrProgramInfo& programInfo,
  ...
  auto [uniformBuffer, success] =
      fDataManager.uploadUniforms(gpu, fPipeline->layout(), commandBuffer);
  if (!success) {
    return false;
  }
  if (uniformBuffer) {
    ...
    commandBuffer->bindDescriptorSets(
        gpu, fPipeline->layout(), kUniformDSIdx, /*setCount=*/1, vkBuffer->uniformDescriptorSet(),
...
bool GrVkPipelineState::setAndBindTextures(
    GrVkGpu* gpu, const GrGeometryProcessor& geomProc, const GrPipeline& pipeline,
    const GrSurfaceProxy* const geomProcTextures[], GrVkCommandBuffer* commandBuffer) {
  ...
  if (fNumSamplers == 1) {
    ...
    if (descriptorSet) {
      ...
      commandBuffer->bindDescriptorSets(
          gpu, fPipeline->layout(), kSamplerDSIdx,
  ...
  commandBuffer->bindDescriptorSets(
      gpu, fPipeline->layout(), kSamplerDSIdx, /*setCount=*/1, descriptorSet->descriptorSet(),
      /*dynamicOffsetCount=*/0, /*dynamicOffsets=*/nullptr);
  commandBuffer->addRecycledResource(descriptorSet);
  descriptorSet->recycle();
  return true;
...
bool GrVkPipelineState::setAndBindInputAttachment(
    GrVkGpu* gpu, gr_rp<const GrVkDescriptorSet> inputDescSet, GrVkCommandBuffer* commandBuffer) {
  SkASSERT(inputDescSet);
  commandBuffer->bindDescriptorSets(
      gpu, fPipeline->layout(), GrVkUniformHandler::kInputDescSet,
...
void GrVkPipelineState::setRenderTargetState(
