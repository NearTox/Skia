...
static int renderpass_features_to_index(
    bool hasResolve, bool hasStencil, GrVkRenderPass::SelfDependencyFlags selfDepFlags,
...
GrVkRenderTarget::GrVkRenderTarget(
    GrVkGpu* gpu, SkISize dimensions, sk_sp<GrVkImage> colorAttachment,
    sk_sp<GrVkImage> resolveAttachment, CreateType createType, std::string_view label)
    : GrSurface(
          gpu, dimensions, colorAttachment->isProtected() ? GrProtected::kYes : GrProtected::kNo,
          label)
      // for the moment we only support 1:1 color to stencil
      ,
      GrRenderTarget(
          gpu, dimensions, colorAttachment->numSamples(),
          colorAttachment->isProtected() ? GrProtected::kYes : GrProtected::kNo, label),
  ...
  SkASSERT(
      !resolveAttachment || (fResolveAttachment->isProtected() == fColorAttachment->isProtected()));
...
GrVkRenderTarget::GrVkRenderTarget(
    GrVkGpu* gpu, SkISize dimensions, sk_sp<GrVkFramebuffer> externalFramebuffer,
...
sk_sp<GrVkRenderTarget> GrVkRenderTarget::MakeWrappedRenderTarget(
  ...
  sk_sp<GrVkImage> wrappedAttachment = GrVkImage::MakeWrapped(
      gpu, dimensions, info, std::move(mutableState), GrAttachment::UsageFlags::kColorAttachment,
...
sk_sp<GrVkRenderTarget> GrVkRenderTarget::MakeSecondaryCBRenderTarget(
  ...
  sk_sp<GrVkFramebuffer> framebuffer(new GrVkFramebuffer(
      gpu, std::move(colorAttachment), sk_sp<const GrVkRenderPass>(rp), std::move(scb)));

  GrVkRenderTarget* vkRT = new GrVkRenderTarget(
      gpu, dimensions, std::move(framebuffer),
...
GrVkImage* GrVkRenderTarget::dynamicMSAAAttachment() {
  ...
  sk_sp<GrAttachment> msaaAttachment = rp->getDiscardableMSAAAttachment(
      nonMSAAColorAttachment->dimensions(), format, gpu->caps()->internalMultisampleCount(format),
      GrProtected(nonMSAAColorAttachment->isProtected()), memoryless);
  if (!msaaAttachment) {
    return nullptr;
  }
...
bool GrVkRenderTarget::canAttemptStencilAttachment(bool useMSAASurface) const {
  SkASSERT(
      !useMSAASurface || this->numSamples() > 1 ||
  ...
  if (useMSAASurface) {
    validMSAA =
        this->numSamples() > 1 || (this->getVkGpu()->vkCaps().supportsDiscardableMSAAForDMSAA() &&
...
GrVkResourceProvider::CompatibleRPHandle GrVkRenderTarget::compatibleRenderPassHandle(
    bool withResolve, bool withStencil, SelfDependencyFlags selfDepFlags,
...
GrVkRenderTarget::createSimpleRenderPass(
    bool withResolve, bool withStencil, SelfDependencyFlags selfDepFlags,
    LoadFromResolve loadFromResolve) {
  SkASSERT(!this->wrapsSecondaryCommandBuffer());
  ...
  const GrVkRenderPass* renderPass = rp.findCompatibleRenderPass(
      this, &handle, withResolve, withStencil, selfDepFlags, loadFromResolve);
...
void GrVkRenderTarget::createFramebuffer(
  ...
  GrVkImage* stencil =
      withStencil ? static_cast<GrVkImage*>(this->getStencilAttachment(useMSAA)) : nullptr;
  fCachedFramebuffers[cacheIndex] = GrVkFramebuffer::Make(
      gpu, this->dimensions(), sk_sp<const GrVkRenderPass>(renderPass), colorAttachment, resolve,
      stencil, compatibleHandle);
}
...
void GrVkRenderTarget::getAttachmentsDescriptor(
  ...
  SkASSERT(!this->wrapsSecondaryCommandBuffer());
  const GrVkImage* colorAttachment = withResolve ? this->msaaAttachment() : this->colorAttachment();
...
void GrVkRenderTarget::ReconstructAttachmentsDescriptor(
    const GrVkCaps& vkCaps, const GrProgramInfo& programInfo,
...
GrBackendRenderTarget GrVkRenderTarget::getBackendRenderTarget() const {
  ...
  return GrBackendRenderTarget(
      beAttachment->width(), beAttachment->height(), beAttachment->vkImageInfo(),
