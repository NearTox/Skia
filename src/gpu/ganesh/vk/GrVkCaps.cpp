...
static FormatCompatibilityClass format_compatibility_class(VkFormat format) {
  switch (format) {
    ...
    case VK_FORMAT_R16G16_SFLOAT: return FormatCompatibilityClass::k32_4_1;

    case VK_FORMAT_R8_UNORM: return FormatCompatibilityClass::k8_1_1;
    ...
    case VK_FORMAT_R16_UNORM: return FormatCompatibilityClass::k16_2_1;

    case VK_FORMAT_R16G16B16A16_SFLOAT:
    case VK_FORMAT_R16G16B16A16_UNORM: return FormatCompatibilityClass::k64_8_1;

    case VK_FORMAT_R8G8B8_UNORM: return FormatCompatibilityClass::k24_3_1;

    case VK_FORMAT_ETC2_R8G8B8_UNORM_BLOCK: return FormatCompatibilityClass::kETC2_RGB_8_16;

    case VK_FORMAT_BC1_RGB_UNORM_BLOCK: return FormatCompatibilityClass::kBC1_RGB_8_16_1;

    case VK_FORMAT_BC1_RGBA_UNORM_BLOCK: return FormatCompatibilityClass::kBC1_RGBA_8_16;

    default: SK_ABORT("Unsupported VkFormat");
  }
}

bool GrVkCaps::canCopyImage(
    VkFormat dstFormat, int dstSampleCnt, bool dstHasYcbcr, VkFormat srcFormat, int srcSampleCnt,
...
bool GrVkCaps::canCopyAsBlit(
    VkFormat dstFormat, int dstSampleCnt, bool dstIsLinear, bool dstHasYcbcr, VkFormat srcFormat,
    int srcSampleCnt, bool srcIsLinear, bool srcHasYcbcr) const {
  // We require that all vulkan GrSurfaces have been created with transfer_dst and transfer_src
  // as image usage flags.
...
bool GrVkCaps::onCanCopySurface(
  ...
  return this->canCopyImage(
         ...
         this->canCopyAsBlit(
             dstFormat, dstSampleCnt, dstIsLinear, dstHasYcbcr, srcFormat, srcSampleCnt,
...
void GrVkCaps::initFormatTable(
  ...
  this->setColorType(
      GrColorType::kABGR_4444, {VK_FORMAT_R4G4B4A4_UNORM_PACK16, VK_FORMAT_B4G4R4A4_UNORM_PACK16});
  ...
  this->setColorType(GrColorType::kRGB_888x, {VK_FORMAT_R8G8B8_UNORM, VK_FORMAT_R8G8B8A8_UNORM});
...
bool GrVkCaps::isFormatAsColorTypeRenderable(
...
int GrVkCaps::getRenderTargetSampleCount(int requestedCount, const GrBackendFormat& format) const {
  VkFormat vkFormat;
  if (!format.asVkFormat(&vkFormat)) {
    return 0;
  }
...
GrCaps::SupportedWrite GrVkCaps::supportedWritePixelsColorType(
...
bool GrVkCaps::onSupportsDynamicMSAA(const GrRenderTargetProxy* rtProxy) const {
  ...
  // resolve attachment.
  return this->supportsDiscardableMSAAForDMSAA() && rtProxy->supportsVkInputAttachment() &&
         rtProxy->numSamples() == 1;
}

bool GrVkCaps::renderTargetSupportsDiscardableMSAA(const GrVkRenderTarget* rt) const {
  return rt->resolveAttachment() && rt->resolveAttachment()->supportsInputAttachmentUsage() &&
...
bool GrVkCaps::programInfoWillUseDiscardableMSAA(const GrProgramInfo& programInfo) const {
  return programInfo.targetHasVkResolveAttachmentWithInput() && programInfo.numSamples() > 1 &&
...
skgpu::Swizzle GrVkCaps::onGetReadSwizzle(
  ...
  SkDEBUGFAILF(
      "Illegal color type (%d) and format (%d) combination.", (int)colorType, (int)vkFormat);
  return {};
}

skgpu::Swizzle GrVkCaps::getWriteSwizzle(
    const GrBackendFormat& format, GrColorType colorType) const {
  VkFormat vkFormat;
  SkAssertResult(format.asVkFormat(&vkFormat));
  const auto& info = this->getFormatInfo(vkFormat);
...
void GrVkCaps::addExtraSamplerKey(
    skgpu::KeyBuilder* b, GrSamplerState samplerState, const GrBackendFormat& format) const {
  const GrVkYcbcrConversionInfo* ycbcrInfo = format.getVkYcbcrConversionInfo();
  if (!ycbcrInfo) {
    return;
  }
 ...
 * and blend constant.
 */
GrProgramDesc GrVkCaps::makeDesc(
  ...
  if (rt) {
    ...
    auto rp = vkRT->getSimpleRenderPass(needsResolve, needsStencil, selfDepFlags, loadFromResolve);
    SkASSERT(rp);
    rp->genKey(&b);
    ...
    if (!rp->isExternal()) {
      ...
      SkASSERT(
          rp->isCompatible(attachmentsDescriptor, attachmentFlags, selfDepFlags, loadFromResolve));
    }
#endif
  } else {
    GrVkRenderPass::AttachmentsDescriptor attachmentsDescriptor;
    ...
    // parameter to GenKey).
    GrVkRenderPass::GenKey(
        &b, attachmentFlags, attachmentsDescriptor, selfDepFlags, loadFromResolve, 0);
  }
