...
GrVkTexture::GrVkTexture(
    ...
    : GrSurface(
      ...
      GrTexture(
          gpu, dimensions, texture->isProtected() ? GrProtected::kYes : GrProtected::kNo,
          GrTextureType::k2D, mipmapStatus, label),
  ...
  SkASSERT(
      !fTexture->ycbcrConversionInfo().isValid() ||
...
GrVkTexture::GrVkTexture(
    GrVkGpu* gpu, SkISize dimensions, sk_sp<GrVkImage> texture, GrMipmapStatus mipmapStatus,
    GrWrapCacheable cacheable, GrIOType ioType, bool isExternal, std::string_view label)
    : GrSurface(
          gpu, dimensions, texture->isProtected() ? GrProtected::kYes : GrProtected::kNo, label),
      GrTexture(
          gpu, dimensions, texture->isProtected() ? GrProtected::kYes : GrProtected::kNo,
          isExternal ? GrTextureType::kExternal : GrTextureType::k2D, mipmapStatus, label),
      fTexture(std::move(texture)),
      fDescSetCache(kMaxCachedDescSets) {
  SkASSERT((GrMipmapStatus::kNotAllocated == mipmapStatus) == (1 == fTexture->mipLevels()));
  SkASSERT(SkToBool(fTexture->vkUsageFlags() & VK_IMAGE_USAGE_SAMPLED_BIT));
...
sk_sp<GrVkTexture> GrVkTexture::MakeNewTexture(
  ...
  return sk_sp<GrVkTexture>(
      new GrVkTexture(gpu, budgeted, dimensions, std::move(texture), mipmapStatus, label));
}
...
sk_sp<GrVkTexture> GrVkTexture::MakeWrappedTexture(
  ...
  sk_sp<GrVkImage> texture = GrVkImage::MakeWrapped(
      gpu, dimensions, info, std::move(mutableState), GrAttachment::UsageFlags::kTexture,
      wrapOwnership, cacheable);
  if (!texture) {
    return nullptr;
  }
...
GrBackendTexture GrVkTexture::getBackendTexture() const {
  return GrBackendTexture(
      fTexture->width(), fTexture->height(), fTexture->vkImageInfo(), fTexture->getMutableState());
}
