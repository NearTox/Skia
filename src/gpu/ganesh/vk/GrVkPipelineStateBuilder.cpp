...
GrVkPipelineState* GrVkPipelineStateBuilder::CreatePipelineState(
    GrVkGpu* gpu, const GrProgramDesc& desc, const GrProgramInfo& programInfo,
...
SkSL::Compiler* GrVkPipelineStateBuilder::shaderCompiler() const { return fGpu->shaderCompiler(); }
...
bool GrVkPipelineStateBuilder::createVkShaderModule(
    VkShaderStageFlagBits stage, const std::string& sksl, VkShaderModule* shaderModule,
    VkPipelineShaderStageCreateInfo* stageInfo, const SkSL::Program::Settings& settings,
    std::string* outSPIRV, SkSL::Program::Inputs* outInputs) {
...
bool GrVkPipelineStateBuilder::installVkShaderModule(
    ...
    VkPipelineShaderStageCreateInfo* stageInfo, std::string spirv, SkSL::Program::Inputs inputs) {
...
int GrVkPipelineStateBuilder::loadShadersFromCache(
  ...
  bool success = this->installVkShaderModule(
  ...
  if (!success) {
    for (int i = 0; i < kGrShaderTypeCount; ++i) {
      if (outShaderModules[i]) {
        GR_VK_CALL(
            fGpu->vkInterface(), DestroyShaderModule(fGpu->device(), outShaderModules[i], nullptr));
      }
    }
    return 0;
  }
...
void GrVkPipelineStateBuilder::storeShadersInCache(
    const std::string shaders[], const SkSL::Program::Inputs inputs[], bool isSkSL) {
...
GrVkPipelineState* GrVkPipelineStateBuilder::finalize(
  ...
  return new GrVkPipelineState(
      ...
      fUniformHandler.currentOffset(), fUniformHandler.usePushConstants(),
      fUniformHandler.fSamplers, std::move(fGPImpl), std::move(fXPImpl), std::move(fFPImpls));
}
