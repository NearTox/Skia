...
class GrVkRenderTarget : public GrRenderTarget {
 public:
  ...
  const GrVkFramebuffer* getFramebuffer(const GrVkRenderPass& renderPass) {
    return this->getFramebuffer(
        renderPass.hasResolveAttachment(), renderPass.hasStencilAttachment(),
        renderPass.selfDependencyFlags(), renderPass.loadFromResolve());
  }
 ...
 protected:
  ...
  GrVkRenderTarget(
      ...
      sk_sp<GrVkImage> resolveImage, CreateType createType, std::string_view label);
 ...
 private:
  ...
  std::pair<const GrVkRenderPass*, GrVkResourceProvider::CompatibleRPHandle> createSimpleRenderPass(
      bool withResolve, bool withStencil, SelfDependencyFlags selfDepFlags, LoadFromResolve);
  ...
  void onSetRelease(sk_sp<skgpu::RefCntedCallback> releaseHelper) override {
    ...
    GrVkImage* attachment = fResolveAttachment ? fResolveAttachment.get() : fColorAttachment.get();
