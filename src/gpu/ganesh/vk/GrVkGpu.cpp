...
sk_sp<GrGpuBuffer> GrVkGpu::onCreateBuffer(
    size_t size, GrGpuBufferType type, GrAccessPattern accessPattern) {
#ifdef SK_DEBUG
  switch (type) {
...
bool GrVkGpu::onWritePixels(
  ...
  if (linearTiling) {
    ...
    success = this->uploadTexDataLinear(
        texImage, rect, srcColorType, texels[0].fPixels, texels[0].fRowBytes);
  } else {
    SkASSERT(mipLevelCount <= (int)texImage->mipLevels());
    success = this->uploadTexDataOptimal(texImage, rect, srcColorType, texels, mipLevelCount);
    if (1 == mipLevelCount) {
...
bool GrVkGpu::onTransferPixelsTo(
  ...
  this->currentCommandBuffer()->copyBufferToImage(
      this, vkBuffer->vkBuffer(), vkImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
...
bool GrVkGpu::uploadTexDataLinear(
    GrVkImage* texImage, SkIRect rect, GrColorType dataColorType, const void* data,
    size_t rowBytes) {
  SkASSERT(data);
  ...
  SkASSERT(
      VK_IMAGE_LAYOUT_PREINITIALIZED == texImage->currentLayout() ||
  ...
  GR_VK_CALL(interface, GetImageSubresourceLayout(fDevice, texImage->image(), &subres, &layout));
...
bool GrVkGpu::uploadTexDataOptimal(
    GrVkImage* texImage, SkIRect rect, GrColorType dataColorType, const GrMipLevel texels[],
    int mipLevelCount) {
  if (!this->currentCommandBuffer()) {
    return false;
  }
  ...
  this->currentCommandBuffer()->copyBufferToImage(
      this, vkBuffer->vkBuffer(), texImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, regions.count(),
      regions.begin());
  return true;
}
...
bool GrVkGpu::uploadTexDataCompressed(
    GrVkImage* uploadTexture, SkImage::CompressionType compression, VkFormat vkFormat,
...
sk_sp<GrTexture> GrVkGpu::onCreateTexture(
  ...
  if (renderable == GrRenderable::kYes) {
    ...
    tex = GrVkTexture::MakeNewTexture(
        this, budgeted, dimensions, pixelFormat, mipLevelCount, isProtected, mipmapStatus, label);
  }
...
sk_sp<GrTexture> GrVkGpu::onCreateCompressedTexture(
  ...
  if (!this->uploadTexDataCompressed(
          tex->textureImage(), compression, pixelFormat, dimensions, mipmapped, data, dataSize)) {
    return nullptr;
  }
...
bool GrVkGpu::loadMSAAFromResolve(
    ...
    GrVkImage* src, const SkIRect& srcRect) {
...
bool GrVkGpu::onUpdateCompressedBackendTexture(
  ...
  cmdBuffer->copyBufferToImage(
      this, static_cast<GrVkBuffer*>(slice.fBuffer)->vkBuffer(), image, image->currentLayout(),
      regions.count(), regions.begin());
...
bool GrVkGpu::setBackendSurfaceState(
  ...
  sk_sp<GrVkImage> texture = GrVkImage::MakeWrapped(
      this, dimensions, info, std::move(currentState), GrVkImage::UsageFlags::kColorAttachment,
...
bool GrVkGpu::setBackendTextureState(
    ...
    GrBackendSurfaceMutableState* previousState, sk_sp<skgpu::RefCntedCallback> finishedCallback) {
  GrVkImageInfo info;
...
void GrVkGpu::xferBarrier(GrRenderTarget* rt, GrXferBarrierType barrierType) {
  ...
  this->addImageMemoryBarrier(
      image->resource(), VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, dstStage, true, &barrier);
}
...
void GrVkGpu::copySurfaceAsBlit(
  ...
  SkASSERT(this->vkCaps().canCopyAsBlit(
      dstFormat, dstSampleCnt, dstImage->isLinearTiled(), dstHasYcbcr, srcFormat, srcSampleCnt,
