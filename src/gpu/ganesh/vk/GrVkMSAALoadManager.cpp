...
bool GrVkMSAALoadManager::loadMSAAFromResolve(
    ...
    GrAttachment* dst, GrVkImage* src, const SkIRect& rect) {
  if (!dst) {
    return false;
  }
  ...
  sk_sp<GrGpuBuffer> uniformBuffer = resourceProvider->createBuffer(
      uniData, sizeof(uniData), GrGpuBufferType::kUniform, kDynamic_GrAccessPattern);
  if (!uniformBuffer) {
    return false;
  }
  ...
  commandBuffer->bindDescriptorSets(
      gpu, fPipelineLayout, GrVkUniformHandler::kInputDescSet, /*setCount=*/1,
