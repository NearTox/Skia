...
class GrVkPipeline : public GrVkManagedResource {
 public:
  static sk_sp<GrVkPipeline> Make(
      ...
      const GrStencilSettings&, int numSamples, bool isHWAntialiasState, const skgpu::BlendInfo&,
  ...
  static sk_sp<GrVkPipeline> Make(
      GrVkGpu*, const GrProgramInfo&, VkPipelineShaderStageCreateInfo* shaderStageInfo,
      int shaderStageCount, VkRenderPass compatibleRenderPass, VkPipelineLayout layout,
      VkPipelineCache cache, uint32_t subpass);
  ...
  static void SetDynamicBlendConstantState(
      GrVkGpu*, GrVkCommandBuffer*, const skgpu::Swizzle& writeSwizzle, const GrXferProcessor&);
