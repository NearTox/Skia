...
static const SkMatrix& test_matrix(
    SkRandom* random, bool includeNonPerspective, bool includePerspective) {
...
const SkPath& TestPath(SkRandom* random) {
  ...
  if (!gOnce) {
    gOnce = true;
    // line
    gPath[0] = SkPathBuilder().moveTo(0.f, 0.f).lineTo(10.f, 10.f).detach();
    // quad
    gPath[1] = SkPathBuilder().moveTo(0.f, 0.f).quadTo(10.f, 10.f, 20.f, 20.f).detach();
    // conic
    gPath[2] = SkPathBuilder().moveTo(0.f, 0.f).conicTo(10.f, 10.f, 20.f, 20.f, 1.f).detach();
    // cubic
    gPath[3] =
        SkPathBuilder().moveTo(0.f, 0.f).cubicTo(10.f, 10.f, 20.f, 20.f, 30.f, 30.f).detach();
    // all three
    gPath[4] = SkPathBuilder()
                   .moveTo(0.f, 0.f)
                   .lineTo(10.f, 10.f)
...
const SkPath& TestPathConvex(SkRandom* random) {
  ...
  if (!gOnce) {
    gOnce = true;
    // narrow rect
    gPath[0] = SkPath::Polygon(
        {{-1.5f, -50.0f}, {-1.5f, -50.0f}, {1.5f, -50.0f}, {1.5f, 50.0f}, {-1.5f, 50.0f}}, false);
    // degenerate
    gPath[1] = SkPath::Polygon(
        {{-0.025f, -0.025f},
         {-0.025f, -0.025f},
         {0.025f, -0.025f},
         {0.025f, 0.025f},
...
TestDashPathEffect::TestDashPathEffect(const SkScalar* intervals, int count, SkScalar phase) {
  ...
  SkDashPath::CalcDashParameters(
      phase, intervals, count, &fInitialDashLength, &fInitialDashIndex, &fIntervalLength, &fPhase);
}

bool TestDashPathEffect::onFilterPath(
    SkPath* dst, const SkPath& src, SkStrokeRec* rec, const SkRect* cullRect,
    const SkMatrix&) const {
  return SkDashPath::InternalFilter(
      dst, src, rec, cullRect, fIntervals.get(), fCount, fInitialDashLength, fInitialDashIndex,
      fIntervalLength);
}
...
sk_sp<GrColorSpaceXform> TestColorXform(SkRandom* random) {
  ...
  if (!gOnce) {
    ...
    gXforms[0] = nullptr;
    gXforms[1] =
        GrColorSpaceXform::Make(srgb.get(), kPremul_SkAlphaType, spin.get(), kPremul_SkAlphaType);
    gXforms[2] =
        GrColorSpaceXform::Make(spin.get(), kPremul_SkAlphaType, srgb.get(), kPremul_SkAlphaType);
  }
...
TestAsFPArgs::TestAsFPArgs(GrProcessorTestData* d)
    : fMatrixProvider(TestMatrix(d->fRandom)),
      ...
      fArgs(d->context(), fMatrixProvider, fColorInfoStorage.get()) {}
...
GrColor RandomColor(SkRandom* random) {
  ...
  switch (colorMode) {
    case kAllOnes_ColorMode: color = GrColorPackRGBA(0xFF, 0xFF, 0xFF, 0xFF); break;
    case kAllZeros_ColorMode: color = GrColorPackRGBA(0, 0, 0, 0); break;
    case kAlphaOne_ColorMode:
      color = GrColorPackRGBA(
          random->nextULessThan(256), random->nextULessThan(256), random->nextULessThan(256), 0xFF);
      break;
    case kRandom_ColorMode: {
      uint8_t alpha = random->nextULessThan(256);
      color = GrColorPackRGBA(
          random->nextRangeU(0, alpha), random->nextRangeU(0, alpha), random->nextRangeU(0, alpha),
          alpha);
      break;
    }
  }
...
uint8_t RandomCoverage(SkRandom* random) {
  ...
  switch (colorMode) {
    case kZero_CoverageMode: coverage = 0; break;
    case kAllOnes_CoverageMode: coverage = 0xff; break;
    case kRandom_CoverageMode: coverage = random->nextULessThan(256); break;
  }
  return coverage;
}
