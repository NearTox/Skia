...
class GrAtlasManager : public GrOnFlushCallbackObject, public skgpu::AtlasGenerationCounter {
 public:
  ...
  GrDrawOpAtlas::ErrorCode addGlyphToAtlas(
      const SkGlyph&, sktext::gpu::Glyph*, int srcPadding, GrResourceProvider*,
  ...
  void addGlyphToBulkAndSetUseToken(
      skgpu::BulkUsePlotUpdater*, skgpu::MaskFormat, sktext::gpu::Glyph*, skgpu::DrawToken);

  void setUseTokenBulk(
      const skgpu::BulkUsePlotUpdater& updater, skgpu::DrawToken token, skgpu::MaskFormat format) {
    this->getAtlas(format)->setLastUseTokenBulk(updater, token);
  }

  // add to texture atlas that matches this format
  GrDrawOpAtlas::ErrorCode addToAtlas(
      GrResourceProvider*, GrDeferredUploadTarget*, skgpu::MaskFormat, int width, int height,
 ...
 private:
  ...
  static int MaskFormatToAtlasIndex(skgpu::MaskFormat format) { return static_cast<int>(format); }
