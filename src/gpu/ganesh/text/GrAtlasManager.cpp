...
GrAtlasManager::GrAtlasManager(
    GrProxyProvider* proxyProvider, size_t maxTextureBytes,
    GrDrawOpAtlas::AllowMultitexturing allowMultitexturing, bool supportBilerpAtlas)
  ...
  if (maskFormat == expectedMaskFormat) {
    ...
    if (glyph.maskFormat() != SkMask::kBW_Format) {
      ...
      switch (expectedMaskFormat) {
        ...
        default: SK_ABORT("Invalid MaskFormat");
      }
    }
  } else if (maskFormat == MaskFormat::kA565 && expectedMaskFormat == MaskFormat::kARGB) {
...
GrDrawOpAtlas::ErrorCode GrAtlasManager::addGlyphToAtlas(
    const SkGlyph& skGlyph, Glyph* glyph, int srcPadding, GrResourceProvider* resourceProvider,
...
GrDrawOpAtlas::ErrorCode GrAtlasManager::addToAtlas(
    GrResourceProvider* resourceProvider, GrDeferredUploadTarget* target, MaskFormat format,
    int width, int height, const void* image, skgpu::AtlasLocator* atlasLocator) {
  return this->getAtlas(format)->addToAtlas(
      resourceProvider, target, width, height, image, atlasLocator);
}

void GrAtlasManager::addGlyphToBulkAndSetUseToken(
    skgpu::BulkUsePlotUpdater* updater, MaskFormat format, Glyph* glyph, skgpu::DrawToken token) {
  SkASSERT(glyph);
...
#  endif
        ...
        save_pixels(context, views[pageIdx], SkColorTypeToGrColorType(ct), filename.c_str());
      }
    }
  }
  ++gDumpCount;
...
bool GrAtlasManager::initAtlas(MaskFormat format) {
  ...
  if (fAtlases[index] == nullptr) {
    ...
    fAtlases[index] = GrDrawOpAtlas::Make(
        fProxyProvider, backendFormat, GrColorTypeToSkColorType(grColorType),
        GrColorTypeBytesPerPixel(grColorType), atlasDimensions.width(), atlasDimensions.height(),
  ...
  if (fAtlasGeneration != currentAtlasGen) {
    ...
    for (const Variant& variant : glyphs) {
      ...
      atlasManager->addGlyphToBulkAndSetUseToken(
          &fBulkUseUpdater, maskFormat, gpuGlyph, tokenTracker->nextDrawToken());
      glyphsPlacedInAtlas++;
    }
