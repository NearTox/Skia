...
static std::unique_ptr<GrFragmentProcessor> make_textured_colorizer(
    const SkPMColor4f* colors, const SkScalar* positions, int count, bool premul,
    const GrFPArgs& args) {
...
static std::unique_ptr<GrFragmentProcessor> make_single_interval_colorizer(
    const SkPMColor4f& start, const SkPMColor4f& end) {
  ...
  return GrSkSLFP::Make(
      effect, "SingleIntervalColorizer", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "start",
      start, "end", end);
}

static std::unique_ptr<GrFragmentProcessor> make_dual_interval_colorizer(
  ...
  const Vec4 scale[2] = {(vc1 - vc0) / threshold, (vc3 - vc2) / (1 - threshold)};
  const Vec4 bias[2] = {vc0, vc2 - threshold * scale[1]};
  return GrSkSLFP::Make(
      effect, "DualIntervalColorizer", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "scale",
      SkMakeSpan(scale), "bias", SkMakeSpan(bias), "threshold", threshold);
}
...
static std::unique_ptr<GrFragmentProcessor> make_unrolled_colorizer(
    int intervalCount, const SkPMColor4f* scale, const SkPMColor4f* bias, SkRect thresholds1_7,
  ...
  once[intervalCount - 1]([intervalCount] {
    ...
    sksl.appendf(
        ...
        intervalCount, intervalCount, intervalCount, (intervalCount <= 0) ? "//" : "",
        (intervalCount <= 1) ? "//" : "", intervalCount, (intervalCount <= 2) ? "//" : "",
        (intervalCount <= 3) ? "//" : "", intervalCount, intervalCount,
        (intervalCount <= 4) ? "//" : "", (intervalCount <= 5) ? "//" : "", intervalCount,
        (intervalCount <= 6) ? "//" : "", (intervalCount <= 7) ? "//" : "");
  ...
  return GrSkSLFP::Make(
      effects[intervalCount - 1], "UnrolledBinaryColorizer",
      /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "thresholds1_7", thresholds1_7,
      "thresholds9_13", thresholds9_13, "scale", SkMakeSpan(scale, intervalCount), "bias",
      SkMakeSpan(bias, intervalCount));
}
...
static std::unique_ptr<GrFragmentProcessor> make_looping_colorizer(
    int intervalCount, const SkPMColor4f* scale, const SkPMColor4f* bias,
  ...
  return GrSkSLFP::Make(
      cacheEntry->effect, "LoopingBinaryColorizer",
      /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "thresholds",
      SkMakeSpan((const SkV4*)thresholds, intervalChunks), "scale",
      SkMakeSpan(scale, intervalCount), "bias", SkMakeSpan(bias, intervalCount));
}
...
int build_intervals(
    int inputLength, const SkPMColor4f* inColors, const SkScalar* inPositions, int outputLength,
    SkPMColor4f* outScales, SkPMColor4f* outBiases, SkScalar* outThresholds) {
...
static std::unique_ptr<GrFragmentProcessor> make_unrolled_binary_colorizer(
  ...
  int intervalCount = build_intervals(
      count, colors, positions, kMaxUnrolledIntervalCount, scales, biases, thresholds);
  if (intervalCount <= 0) {
    return nullptr;
  }
...
static std::unique_ptr<GrFragmentProcessor> make_looping_binary_colorizer(
    const SkPMColor4f* colors, const SkScalar* positions, int count) {
  ...
  int intervalCount = build_intervals(
      count, colors, positions, kMaxLoopingIntervalCount, scales, biases, thresholds);
  if (intervalCount <= 0) {
    return nullptr;
  }
...
// the gradient.
static std::unique_ptr<GrFragmentProcessor> make_colorizer(
    const SkPMColor4f* colors, const SkScalar* positions, int count, bool premul,
    const GrFPArgs& args) {
  ...
  auto makeDualIntervalColorizer = [&]() -> std::unique_ptr<GrFragmentProcessor> {
    ...
    if (count == 3) {
      ...
      return make_dual_interval_colorizer(colors[0], colors[1], colors[1], colors[2], positions[1]);
    }
    if (count == 4 && SkScalarNearlyEqual(positions[1], positions[2])) {
      // Two separate intervals that join at the same threshold position
      return make_dual_interval_colorizer(colors[0], colors[1], colors[2], colors[3], positions[1]);
    }
    // The gradient can't be represented in only two intervals.
    return nullptr;
  };

  int binaryColorizerLimit =
      caps->fNonconstantArrayIndexSupport ? kMaxLoopingColorCount : kMaxUnrolledColorCount;
...
static std::unique_ptr<GrFragmentProcessor> make_clamped_gradient(
    std::unique_ptr<GrFragmentProcessor> colorizer, std::unique_ptr<GrFragmentProcessor> gradLayout,
    SkPMColor4f leftBorderColor, SkPMColor4f rightBorderColor, bool makePremul,
  ...
  return GrSkSLFP::Make(
      effect, "ClampedGradient", /*inputFP=*/nullptr, optFlags, "colorizer",
      GrSkSLFP::IgnoreOptFlags(std::move(colorizer)), "gradLayout",
      GrSkSLFP::IgnoreOptFlags(std::move(gradLayout)), "leftBorderColor", leftBorderColor,
      "rightBorderColor", rightBorderColor, "makePremul", GrSkSLFP::Specialize<int>(makePremul),
      "layoutPreservesOpacity", GrSkSLFP::Specialize<int>(layoutPreservesOpacity));
}

static std::unique_ptr<GrFragmentProcessor> make_tiled_gradient(
    const GrFPArgs& args, std::unique_ptr<GrFragmentProcessor> colorizer,
  ...
  return GrSkSLFP::Make(
      effect, "TiledGradient", /*inputFP=*/nullptr, optFlags, "colorizer",
      GrSkSLFP::IgnoreOptFlags(std::move(colorizer)), "gradLayout",
      GrSkSLFP::IgnoreOptFlags(std::move(gradLayout)), "mirror", GrSkSLFP::Specialize<int>(mirror),
      "makePremul", GrSkSLFP::Specialize<int>(makePremul), "layoutPreservesOpacity",
      GrSkSLFP::Specialize<int>(layoutPreservesOpacity), "useFloorAbsWorkaround",
...
static std::unique_ptr<GrFragmentProcessor> make_gradient(
    ...
    std::unique_ptr<GrFragmentProcessor> layout, const SkMatrix* overrideMatrix = nullptr) {
  ...
  SkColor4fXformer xformedColors(
      shader.fOrigColors4f, shader.fColorCount, shader.fColorSpace.get(),
  ...
  std::unique_ptr<GrFragmentProcessor> colorizer =
      make_colorizer(colors.get(), positions, shader.fColorCount, inputPremul, args);
  ...
  switch (shader.getTileMode()) {
    case SkTileMode::kRepeat:
      gradient = make_tiled_gradient(
          args, std::move(colorizer), std::move(layout),
    ...
    case SkTileMode::kClamp:
      ...
      // border.
      gradient = make_clamped_gradient(
          std::move(colorizer), std::move(layout), colors[0], colors[shader.fColorCount - 1],
    ...
    case SkTileMode::kDecal:
      ...
      gradient = make_clamped_gradient(
          std::move(colorizer), std::move(layout), SK_PMColor4fTRANSPARENT, SK_PMColor4fTRANSPARENT,
...
std::unique_ptr<GrFragmentProcessor> MakeLinear(
  ...
  auto fp = GrSkSLFP::Make(
      effect, "LinearLayout", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kPreservesOpaqueInput);
...
std::unique_ptr<GrFragmentProcessor> MakeSweep(
    const SkSweepGradient& shader, const GrFPArgs& args) {
  ...
  // using atan instead.
  int useAtanWorkaround = args.fContext->priv().caps()->shaderCaps()->fAtan2ImplementedAsAtanYOverX;
  static auto effect = SkMakeRuntimeEffect(SkRuntimeEffect::MakeForShader, R"(
        uniform half bias;
        uniform half scale;
  ...
  auto fp = GrSkSLFP::Make(
      effect, "SweepLayout", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kPreservesOpaqueInput, "bias",
      shader.getTBias(), "scale", shader.getTScale(), "useAtanWorkaround",
      GrSkSLFP::Specialize(useAtanWorkaround));
  return make_gradient(shader, args, std::move(fp));
}
...
std::unique_ptr<GrFragmentProcessor> MakeConical(
  ...
  switch (shader.getType()) {
    case SkTwoPointConicalGradient::Type::kStrip: {
      ...
      fp = GrSkSLFP::Make(
          effect, "TwoPointConicalStripLayout", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone,
          "r0_2", r0 * r0);
    } break;
    ...
    case SkTwoPointConicalGradient::Type::kRadial: {
      ...
      fp = GrSkSLFP::Make(
          effect, "TwoPointConicalRadialLayout", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone,
          "r0", r0, "lengthScale", isRadiusIncreasing ? 1.0f : -1.0f);
      ...
      // Map center to (0, 0)
      matrix.set(SkMatrix::Translate(-shader.getStartCenter().fX, -shader.getStartCenter().fY));
    ...
    case SkTwoPointConicalGradient::Type::kFocal: {
      ...
      bool isRadiusIncreasing = (1 - focalData.fFocalX) > 0,
           isFocalOnCircle = focalData.isFocalOnCircle(), isWellBehaved = focalData.isWellBehaved(),
           isSwapped = focalData.isSwapped(), isNativelyFocal = focalData.isNativelyFocal();

      fp = GrSkSLFP::Make(
          effect, "TwoPointConicalFocalLayout", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone,
          "isRadiusIncreasing", GrSkSLFP::Specialize<int>(isRadiusIncreasing), "isFocalOnCircle",
          GrSkSLFP::Specialize<int>(isFocalOnCircle), "isWellBehaved",
          GrSkSLFP::Specialize<int>(isWellBehaved), "isSwapped",
          GrSkSLFP::Specialize<int>(isSwapped), "isNativelyFocal",
          GrSkSLFP::Specialize<int>(isNativelyFocal), "invR1", 1.0f / focalData.fR1, "fx",
          focalData.fFocalX);
    } break;
  }
