...
void GrGradientBitmapCache::fillGradient(
    const SkPMColor4f* colors, const SkScalar* positions, int count, SkColorType colorType,
    SkBitmap* bitmap) {
  ...
  pixelWriteFn_t writePixel = (colorType == kRGBA_F16_SkColorType) ? writeF16Pixel : write8888Pixel;
  ...
  for (int i = 1; i < count; i++) {
    ...
    int nextIndex = std::min(positions[i] * fResolution, SkIntToScalar(fResolution - 1));

    if (nextIndex > prevIndex) {
      auto c0 = skvx::float4::Load(colors[i - 1].vec()), c1 = skvx::float4::Load(colors[i].vec());
...
void GrGradientBitmapCache::getGradient(
    const SkPMColor4f* colors, const SkScalar* positions, int count, SkColorType colorType,
    SkAlphaType alphaType, SkBitmap* bitmap) {
