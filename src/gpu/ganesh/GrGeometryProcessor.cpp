...
void GrGeometryProcessor::TextureSampler::reset(
    GrSamplerState samplerState, const GrBackendFormat& backendFormat,
    const skgpu::Swizzle& swizzle) {
  fSamplerState = samplerState;
  fSamplerState = GrSamplerState(
      samplerState.wrapModeX(), samplerState.wrapModeY(),
      clamp_filter(backendFormat.textureType(), samplerState.filter()), samplerState.mipmapMode());
...
std::tuple<ProgramImpl::FPCoordsMap, GrShaderVar> ProgramImpl::emitCode(
    EmitArgs& args, const GrPipeline& pipeline) {
  GrGPArgs gpArgs;
  this->onEmitCode(args, &gpArgs);

  FPCoordsMap transformMap = this->collectTransforms(
      args.fVertBuilder, args.fVaryingHandler, args.fUniformHandler, gpArgs.fLocalCoordShader,
      gpArgs.fLocalCoordVar, gpArgs.fPositionVar, pipeline);

  GrGLSLVertexBuilder* vBuilder = args.fVertBuilder;
  // Emit the vertex position to the hardware in the normalized window coordinates it expects.
  SkASSERT(
      SkSLType::kFloat2 == gpArgs.fPositionVar.getType() ||
...
ProgramImpl::FPCoordsMap ProgramImpl::collectTransforms(
    GrGLSLVertexBuilder* vb, GrGLSLVaryingHandler* varyingHandler,
    GrGLSLUniformHandler* uniformHandler, GrShaderType localCoordsShader,
    const GrShaderVar& localCoordsVar, const GrShaderVar& positionVar, const GrPipeline& pipeline) {
  SkASSERT(
      localCoordsVar.getType() == SkSLType::kFloat2 ||
      localCoordsVar.getType() == SkSLType::kFloat3 || localCoordsVar.getType() == SkSLType::kVoid);
  SkASSERT(
      positionVar.getType() == SkSLType::kFloat2 || positionVar.getType() == SkSLType::kFloat3 ||
  ...
  auto baseLocalCoordFSVar = [&, baseLocalCoordVarying = GrGLSLVarying()]() mutable {
    ...
    if (baseLocalCoordVarying.type() == SkSLType::kVoid) {
      ...
      vb->codeAppendf(
          "%s = %s;\n", baseLocalCoordVarying.vsOut(), localCoordsVar.getName().c_str());
    }
  ...
  auto liftTransforms = [&, traversalIndex = 0](
                            auto& self, const GrFragmentProcessor& fp, bool hasPerspective,
    ...
    if (localCoordsShader == kVertex_GrShaderType) {
      switch (fp.sampleUsage().kind()) {
        case SkSL::SampleUsage::Kind::kNone:
          ...
          SkASSERT(!fp.parent());
          break;
        case SkSL::SampleUsage::Kind::kPassThrough: break;
        ...
        case SkSL::SampleUsage::Kind::kFragCoord:
          ...
          break;
        case SkSL::SampleUsage::Kind::kExplicit: baseCoord = BaseCoord::kNone; break;
      }
    } else {
      ...
      if (!lastMatrixFP) {
        ...
        if (varying.type() == SkSLType::kVoid) {
          ...
          SkString strVaryingName =
              SkStringPrintf("TransformedCoords_%d", lastMatrixTraversalIndex);
    ...
    for (int c = 0; c < fp.numChildProcessors(); ++c) {
      if (auto* child = fp.childProcessor(c)) {
        self(self, *child, hasPerspective, lastMatrixFP, lastMatrixTraversalIndex, baseCoord);
        ...
        hasCoordsParam |= varyingFSVar.getType() == SkSLType::kVoid &&
                          ...
                          !child->sampleUsage().isFragCoord() && result[child].hasCoordsParam;
      }
    }
  };
  ...
  for (; !pq.empty(); pq.pop()) {
    ...
    if (info.varying.type() == SkSLType::kFloat2) {
      if (vb->getProgramBuilder()->shaderCaps()->fNonsquareMatrixSupport) {
        vb->codeAppendf(
            "%s = float3x2(%s) * %s", info.varying.vsOut(), transformExpression.c_str(),
            inputStr.c_str());
      } else {
        vb->codeAppendf(
            "%s = (%s * %s).xy", info.varying.vsOut(), transformExpression.c_str(),
            inputStr.c_str());
      }
    } else {
      SkASSERT(info.varying.type() == SkSLType::kFloat3);
      vb->codeAppendf(
          "%s = %s * %s", info.varying.vsOut(), transformExpression.c_str(), inputStr.c_str());
    }
    vb->codeAppend(";\n");
    vb->codeAppend("}\n");
  }
...
void ProgramImpl::setupUniformColor(
    GrGLSLFPFragmentBuilder* fragBuilder, GrGLSLUniformHandler* uniformHandler,
    const char* outputName, UniformHandle* colorUniform) {
  SkASSERT(colorUniform);
  const char* stagedLocalVarName;
  *colorUniform = uniformHandler->addUniform(
      nullptr, kFragment_GrShaderFlag, SkSLType::kHalf4, "Color", &stagedLocalVarName);
...
void ProgramImpl::SetTransform(
    const GrGLSLProgramDataManager& pdman, const GrShaderCaps& shaderCaps,
    const UniformHandle& uniform, const SkMatrix& matrix, SkMatrix* state) {
  ...
  if (matrix.isScaleTranslate() && !shaderCaps.fReducedShaderMode) {
    ...
    float values[4] = {
        matrix.getScaleX(), matrix.getTranslateX(), matrix.getScaleY(), matrix.getTranslateY()};
...
static void write_passthrough_vertex_position(
    GrGLSLVertexBuilder* vertBuilder, const GrShaderVar& inPos, GrShaderVar* outPos) {
  ...
  vertBuilder->codeAppendf(
      "float%d %s = %s;", SkSLTypeVecLength(inPos.getType()), outName.c_str(),
      inPos.getName().c_str());
}

static void write_vertex_position(
    GrGLSLVertexBuilder* vertBuilder, GrGLSLUniformHandler* uniformHandler,
    const GrShaderCaps& shaderCaps, const GrShaderVar& inPos, const SkMatrix& matrix,
    const char* matrixName, GrShaderVar* outPos, ProgramImpl::UniformHandle* matrixUniform) {
  SkASSERT(inPos.getType() == SkSLType::kFloat3 || inPos.getType() == SkSLType::kFloat2);
  SkString outName = vertBuilder->newTmpVarName(inPos.getName().c_str());
  ...
  *matrixUniform = uniformHandler->addUniform(
      nullptr, kVertex_GrShaderFlag, useCompactTransform ? SkSLType::kFloat4 : SkSLType::kFloat3x3,
      matrixName, &mangledMatrixName);
  ...
  if (matrix.hasPerspective()) {
    ...
    vertBuilder->codeAppendf(
        "float3 %s = (%s * %s.xy1);", outName.c_str(), mangledMatrixName, inPos.getName().c_str());
    outPos->set(SkSLType::kFloat3, outName.c_str());
    return;
  }
  if (useCompactTransform) {
...
void ProgramImpl::WriteOutputPosition(
    GrGLSLVertexBuilder* vertBuilder, GrGPArgs* gpArgs, const char* posName) {
...
void ProgramImpl::WriteOutputPosition(
    GrGLSLVertexBuilder* vertBuilder, GrGLSLUniformHandler* uniformHandler,
    const GrShaderCaps& shaderCaps, GrGPArgs* gpArgs, const char* posName, const SkMatrix& mat,
    UniformHandle* viewMatrixUniform) {
  GrShaderVar inPos(posName, SkSLType::kFloat2);
  write_vertex_position(
      vertBuilder, uniformHandler, shaderCaps, inPos, mat, "viewMatrix", &gpArgs->fPositionVar,
      viewMatrixUniform);
}

void ProgramImpl::WriteLocalCoord(
    GrGLSLVertexBuilder* vertBuilder, GrGLSLUniformHandler* uniformHandler,
    const GrShaderCaps& shaderCaps, GrGPArgs* gpArgs, GrShaderVar localVar,
    const SkMatrix& localMatrix, UniformHandle* localMatrixUniform) {
  write_vertex_position(
      vertBuilder, uniformHandler, shaderCaps, localVar, localMatrix, "localMatrix",
...
GrGeometryProcessor::Attribute AttributeSet::Iter::operator*() const noexcept {
...
void AttributeSet::Iter::operator++() noexcept {
  if (fRemaining) {
    fRemaining--;
...
void AttributeSet::Iter::skipUninitialized() noexcept {
  if (!fRemaining) {
    fCurr = nullptr;
  } else {
...
void AttributeSet::initImplicit(const Attribute* attrs, int count) noexcept {
  fAttributes = attrs;
  fRawCount = count;
  fCount = 0;
  fStride = 0;
...
void AttributeSet::initExplicit(const Attribute* attrs, int count, size_t stride) noexcept {
  fAttributes = attrs;
  fRawCount = count;
  fCount = count;
  fStride = stride;
...
void AttributeSet::addToKey(skgpu::KeyBuilder* b) const {
  ...
  for (int i = 0; i < rawCount; ++i) {
    ...
    b->addBits(8, attr.isInitialized() ? static_cast<int>(attr.gpuType()) : 0xff, "attrGpuType");
    int16_t offset = -1;
...
AttributeSet::Iter AttributeSet::begin() const noexcept { return Iter(fAttributes, fCount); }
AttributeSet::Iter AttributeSet::end() const noexcept { return Iter(); }
