    ...
    class Impl : public ProgramImpl {
     ...
     private:
      void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
        ...
        fColorSpaceXformHelper.emitCode(args.fUniformHandler, latticeGP.fColorSpaceXform.get());
        ...
        args.fVaryingHandler->addPassThroughAttribute(
            latticeGP.fInTextureCoords.asShaderVar(), "textureCoords");
        args.fFragBuilder->codeAppend("float4 textureDomain;");
        args.fVaryingHandler->addPassThroughAttribute(
            latticeGP.fInTextureDomain.asShaderVar(), "textureDomain", Interpolation::kCanBeFlat);
        args.fFragBuilder->codeAppendf("half4 %s;", args.fOutputColor);
        args.fVaryingHandler->addPassThroughAttribute(
            latticeGP.fInColor.asShaderVar(), args.fOutputColor, Interpolation::kCanBeFlat);
...
class NonAALatticeOp final : public GrMeshDrawOp {
 ...
 public:
  ...
  GrProcessorSet::Analysis finalize(
    ...
    auto result = fHelper.finalizeProcessors(
        caps, clip, clampType, GrProcessorAnalysisCoverage::kNone, &analysisColor);
 ...
 private:
  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    for (int i = 0; i < patchCnt; i++) {
      ...
      while (patch.fIter->next(&srcR, &dstR)) {
        skvx::float4 coords(
            SkIntToScalar(srcR.fLeft), SkIntToScalar(srcR.fTop), SkIntToScalar(srcR.fRight),
            SkIntToScalar(srcR.fBottom));
        ...
        if (isScaleTranslate) {
          vertices.writeQuad(
              VertexWriter::TriStripFromRect(dstR), VertexWriter::TriStripFromRect(texCoords),
              texDomain, patchColor);
        } else {
          SkPoint mappedPts[4];
          ...
          vertices << mappedPts[0] << SkPoint::Make(texCoords.fLeft, texCoords.fTop) << texDomain
                   << patchColor;
          vertices << mappedPts[3] << SkPoint::Make(texCoords.fLeft, texCoords.fBottom) << texDomain
                   << patchColor;
          vertices << mappedPts[1] << SkPoint::Make(texCoords.fRight, texCoords.fTop) << texDomain
                   << patchColor;
          vertices << mappedPts[2] << SkPoint::Make(texCoords.fRight, texCoords.fBottom)
                   << texDomain << patchColor;
        }
      }
    }
  ...
  void onExecute(GrOpFlushState* flushState, const SkRect& chainBounds) override {
    ...
    flushState->bindTextures(fProgramInfo->geomProc(), *fView.proxy(), fProgramInfo->pipeline());
  ...
  SkString onDumpInfo() const override {
    ...
    for (int i = 0; i < fPatches.count(); ++i) {
      str.appendf(
          ...
          fPatches[i].fColor.toBytes_RGBA(), fPatches[i].fDst.fLeft, fPatches[i].fDst.fTop,
...
static void init_random_divs(
    int divs[], int count, int subsetStart, int subsetStop, SkRandom* random) {
...
GR_DRAW_OP_TEST_DEFINE(NonAALatticeOp) {
  ...
  do {
    ...
    if (hasFlags) {
      ...
      for (int i = 0; i < n; ++i) {
        flags[i] =
            random->nextBool() ? SkCanvas::Lattice::kTransparent : SkCanvas::Lattice::kDefault;
      }
  ...
  return skgpu::v1::LatticeOp::NonAALatticeOp::Make(
