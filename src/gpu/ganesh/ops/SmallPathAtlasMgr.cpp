...
SmallPathAtlasMgr::~SmallPathAtlasMgr() { this->reset(); }
...
bool SmallPathAtlasMgr::initAtlas(GrProxyProvider* proxyProvider, const GrCaps* caps) {
  ...
  const GrBackendFormat format = caps->getDefaultBackendFormat(atlasColorType, GrRenderable::kNo);
  ...
  fAtlas = GrDrawOpAtlas::Make(
      ...
      GrColorTypeBytesPerPixel(atlasColorType), size.width(), size.height(), kPlotWidth,
...
GrDrawOpAtlas::ErrorCode SmallPathAtlasMgr::addToAtlas(
    GrResourceProvider* resourceProvider, GrDeferredUploadTarget* target, int width, int height,
    const void* image, skgpu::AtlasLocator* locator) {
  return fAtlas->addToAtlas(resourceProvider, target, width, height, image, locator);
}

void SmallPathAtlasMgr::setUseToken(SmallPathShapeData* shapeData, skgpu::DrawToken token) {
