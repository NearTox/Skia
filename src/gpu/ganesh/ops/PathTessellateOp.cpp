...
GrProcessorSet::Analysis PathTessellateOp::finalize(
  ...
  auto analysis = fProcessors.finalize(
      this->headDraw().fColor, GrProcessorAnalysisCoverage::kNone, clip, nullptr, caps, clampType,
...
GrDrawOp::CombineResult PathTessellateOp::onCombineIfPossible(
    GrOp* grOp, SkArenaAlloc*, const GrCaps&) {
  auto* op = grOp->cast<PathTessellateOp>();
  bool canMerge = fAAType == op->fAAType && fStencil == op->fStencil &&
                  fProcessors == op->fProcessors && fShaderMatrix == op->fShaderMatrix;
  if (canMerge) {
...
void PathTessellateOp::prepareTessellator(
  ...
  fTessellator = PathWedgeTessellator::Make(
      args.fArena, args.fCaps->shaderCaps()->fInfinitySupport, fPatchAttribs);
  auto* tessShader = GrPathTessellationShader::Make(
      *args.fCaps->shaderCaps(), args.fArena, fShaderMatrix, this->headDraw().fColor,
      fTessellator->patchAttribs());
  fTessellationProgram = GrTessellationShader::MakeProgram(args, tessShader, pipeline, fStencil);
}

void PathTessellateOp::onPrePrepare(
...
void PathTessellateOp::onPrepare(GrOpFlushState* flushState) {
  ...
  fTessellator->prepare(flushState, fShaderMatrix, *fPathDrawList, fTotalCombinedPathVerbCnt);
}
...
void PathTessellateOp::onExecute(GrOpFlushState* flushState, const SkRect& chainBounds) {
  ...
  flushState->bindTextures(
      fTessellationProgram->geomProc(), nullptr, fTessellationProgram->pipeline());
