...
#if GR_TEST_UTILS
SkString dump_quad_info(
    int index, const GrQuad* deviceQuad, const GrQuad* localQuad, const SkPMColor4f& color,
  ...
  str.appendf(
      ...
      safeLocal.x(0), safeLocal.y(0), safeLocal.w(0), safeLocal.x(1), safeLocal.y(1),
      safeLocal.w(1), safeLocal.x(2), safeLocal.y(2), safeLocal.w(2), safeLocal.x(3),
      safeLocal.y(3), safeLocal.w(3));
  return str;
}
#endif
...
class FillRectOpImpl final : public GrMeshDrawOp {
 ...
 public:
  static GrOp::Owner Make(
    ...
    return Helper::FactoryHelper<FillRectOpImpl>(
        context, std::move(paint), aaType, quad, stencilSettings, inputFlags);
  }
  ...
  FillRectOpImpl(
    ...
    fQuads.append(
        quad->fDevice, {paintColor, quad->fEdgeFlags},
        fHelper.isTrivial() ? nullptr : &quad->fLocal);
    if (count > 1) {
      fQuads.append(
          extra.fDevice, {paintColor, extra.fEdgeFlags},
  ...
  GrProcessorSet::Analysis finalize(
    ...
    if (quadColors.isConstant(&colorOverride)) {
      ...
      while (iter.next()) {
        fColorType = std::max(fColorType, skgpu::v1::QuadPerEdgeAA::MinColorType(iter->fColor));
      }
    }
 ...
 private:
  ...
  VertexSpec vertexSpec() const {
    auto indexBufferOption =
        skgpu::v1::QuadPerEdgeAA::CalcIndexBufferOption(fHelper.aaType(), fQuads.count());

    return VertexSpec(
        fQuads.deviceQuadType(), fColorType, fQuads.localQuadType(), fHelper.usesLocalCoords(),
        Subset::kNo, fHelper.aaType(), fHelper.compatibleWithCoverageAsAlpha(), indexBufferOption);
  }
  ...
  void onCreateProgramInfo(
    ...
    fProgramInfo = fHelper.createProgramInfoWithStencil(
        ...
        vertexSpec.primitiveType(), renderPassXferBarriers, colorLoadOp);
  }
  ...
  void onPrePrepareDraws(
    ...
    INHERITED::onPrePrepareDraws(
        rContext, writeView, clip, dstProxyView, renderPassXferBarriers, colorLoadOp);
  ...
  void tessellate(const VertexSpec& vertexSpec, char* dst) const {
    ...
    while (iter.next()) {
      ...
      tessellator.append(
          iter.deviceQuad(), iter.localQuad(), info.fColor, kEmptyDomain, info.fAAFlags);
    }
  }
  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    void* vdata = target->makeVertexSpace(
        vertexSpec.vertexSize(), totalNumVertices, &fVertexBuffer, &fBaseVertex);
    if (!vdata) {
    ...
    if (vertexSpec.needsIndexBuffer()) {
      fIndexBuffer =
          skgpu::v1::QuadPerEdgeAA::GetIndexBuffer(target, vertexSpec.indexBufferOption());
      if (!fIndexBuffer) {
  ...
  void onExecute(GrOpFlushState* flushState, const SkRect& chainBounds) override {
    ...
    skgpu::v1::QuadPerEdgeAA::IssueDraw(
        flushState->caps(), flushState->opsRenderPass(), vertexSpec, 0, fQuads.count(),
  ...
  CombineResult onCombineIfPossible(GrOp* t, SkArenaAlloc*, const GrCaps& caps) override {
    ...
    if (CombinedQuadCountWillOverflow(
            fHelper.aaType(), upgradeToCoverageAAOnMerge, fQuads.count() + that->fQuads.count())) {
      return CombineResult::kCannotCombine;
    }
  ...
  SkString onDumpInfo() const override {
    ...
    str.appendf(
        "Device quad type: %u, local quad type: %u\n", (uint32_t)fQuads.deviceQuadType(),
        (uint32_t)fQuads.localQuadType());
    str += fHelper.dumpInfo();
    int i = 0;
    auto iter = fQuads.iterator();
    while (iter.next()) {
      const ColorAndAA& info = iter.metadata();
      str += dump_quad_info(i, iter.deviceQuad(), iter.localQuad(), info.fColor, info.fAAFlags);
      i++;
    }
    return str;
  }
  ...
  bool canAddQuads(int numQuads, GrAAType aaType) {
    ...
    if (aaType != fHelper.aaType() && aaType != GrAAType::kNone) {
      auto indexBufferOption = skgpu::v1::QuadPerEdgeAA::CalcIndexBufferOption(aaType, quadCount);
      ...
      auto indexBufferOption =
          skgpu::v1::QuadPerEdgeAA::CalcIndexBufferOption(fHelper.aaType(), quadCount);
      if (quadCount > skgpu::v1::QuadPerEdgeAA::QuadLimit(indexBufferOption)) {
  ...
  bool addQuad(DrawQuad* quad, const SkPMColor4f& color, GrAAType aaType) {
    ...
    if (count == 0) {
      ...
      fQuads.append(
          quad->fDevice, {color, quad->fEdgeFlags}, fHelper.isTrivial() ? nullptr : &quad->fLocal);
      if (count > 1) {
        fQuads.append(
            extra.fDevice, {color, extra.fEdgeFlags},
            fHelper.isTrivial() ? nullptr : &extra.fLocal);
      }
      // Update the bounds
      this->setBounds(
...
GrOp::Owner FillRectOp::Make(
    GrRecordingContext* context, GrPaint&& paint, GrAAType aaType, DrawQuad* quad,
    const GrUserStencilSettings* stencil, InputFlags inputFlags) {
  return FillRectOpImpl::Make(
      context, std::move(paint), aaType, std::move(quad), stencil, inputFlags);
}

GrOp::Owner FillRectOp::MakeNonAARect(
...
GrOp::Owner FillRectOp::MakeOp(
    GrRecordingContext* context, GrPaint&& paint, GrAAType aaType, const SkMatrix& viewMatrix,
    const GrQuadSetEntry quads[], int cnt, const GrUserStencilSettings* stencilSettings,
    int* numConsumed) {
  ...
  *numConsumed = 1;
  ...
  for (int i = 1; i < cnt; ++i) {
    quad = {
        GrQuad::MakeFromRect(quads[i].fRect, viewMatrix),
        GrQuad::MakeFromRect(quads[i].fRect, quads[i].fLocalMatrix), quads[i].fAAFlags};

    GrAAType resolvedAA;
    GrQuadUtils::ResolveAAType(
        aaType, quads[i].fAAFlags, quad.fDevice, &resolvedAA, &quad.fEdgeFlags);
...
void FillRectOp::AddFillRectOps(
    skgpu::v1::SurfaceDrawContext* sdc, const GrClip* clip, GrRecordingContext* context,
    GrPaint&& paint, GrAAType aaType, const SkMatrix& viewMatrix, const GrQuadSetEntry quads[],
...
uint32_t skgpu::v1::FillRectOp::ClassID() { return FillRectOpImpl::ClassID(); }
...
GR_DRAW_OP_TEST_DEFINE(FillRectOp) {
  ...
  if (random->nextBool()) {
    if (random->nextBool()) {
      ...
      DrawQuad quad = {
          GrQuad::MakeFromRect(rect, viewMatrix), GrQuad::MakeFromRect(rect, localMatrix), aaFlags};
