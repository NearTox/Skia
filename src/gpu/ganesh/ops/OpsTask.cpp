...
GrOpsRenderPass* create_render_pass(
    GrGpu* gpu, GrRenderTarget* rt, bool useMSAASurface, GrAttachment* stencil,
    GrSurfaceOrigin origin, const SkIRect& bounds, GrLoadOp colorLoadOp,
    const std::array<float, 4>& loadClearColor, GrLoadOp stencilLoadOp, GrStoreOp stencilStoreOp,
  ...
  return gpu->getOpsRenderPass(
      rt, useMSAASurface, stencil, origin, bounds, kColorLoadStoreInfo, stencilLoadAndStoreInfo,
...
inline OpsTask::OpChain::List::List(GrOp::Owner op) noexcept
    : fHead(std::move(op)), fTail(fHead.get()) {
  this->validate();
}

inline OpsTask::OpChain::List::List(List&& that) noexcept { *this = std::move(that); }

inline OpsTask::OpChain::List& OpsTask::OpChain::List::operator=(List&& that) noexcept {
...
inline void OpsTask::OpChain::List::validate() const noexcept {
#ifdef SK_DEBUG
  if (fHead) {
    SkASSERT(fTail);
...
OpsTask::OpChain::OpChain(
    GrOp::Owner op, GrProcessorSet::Analysis processorAnalysis, GrAppliedClip* appliedClip,
...
OpsTask::OpChain::List OpsTask::OpChain::DoConcat(
    List chainA, List chainB, const GrCaps& caps, SkArenaAlloc* opsTaskArena,
  ...
  do {
    ...
    GrOp* a = origATail;
    while (a) {
      bool canForwardMerge = (a == chainA.tail()) || can_reorder(a->bounds(), forwardMergeBounds);
      if (canForwardMerge || canBackwardMerge) {
        ...
        GrOP_INFO(
            ...
            chainB.head()->uniqueID(), a->name(), a->uniqueID());
      }
      if (merged) {
        ...
        canBackwardMerge = canBackwardMerge && can_reorder(chainB.head()->bounds(), a->bounds());
        a = a->prevInChain();
      }
    }
  ...
  do {
    switch (fList.tail()->combineIfPossible(list->head(), opsTaskArena, caps)) {
      ...
      case GrOp::CombineResult::kMayChain:
        fList = DoConcat(
            std::move(fList), std::exchange(*list, List()), caps, opsTaskArena, auditTrail);
      ...
      case GrOp::CombineResult::kMerged: {
        GrOP_INFO(
            "\t\t: (%s opID: %u) -> Combining with (%s, opID: %u)\n", list->tail()->name(),
            list->tail()->uniqueID(), list->head()->name(), list->head()->uniqueID());
...
bool OpsTask::OpChain::prependChain(
    OpChain* that, const GrCaps& caps, SkArenaAlloc* opsTaskArena, GrAuditTrail* auditTrail) {
  if (!that->tryConcat(
          &fList, fProcessorAnalysis, fDstProxyView, fAppliedClip, fBounds, caps, opsTaskArena,
          auditTrail)) {
    this->validate();
    // append failed
    return false;
...
GrOp::Owner OpsTask::OpChain::appendOp(
    GrOp::Owner op, GrProcessorSet::Analysis processorAnalysis, const GrDstProxyView* dstProxyView,
    const GrAppliedClip* appliedClip, const GrCaps& caps, SkArenaAlloc* opsTaskArena,
    GrAuditTrail* auditTrail) {
  ...
  if (!this->tryConcat(
          &chain, processorAnalysis, *dstProxyView, appliedClip, opBounds, caps, opsTaskArena,
          auditTrail)) {
...
inline void OpsTask::OpChain::validate() const {
  ...
  for (const auto& op : GrOp::ChainRange<>(fList.head())) {
    ...
    SkASSERT(
        fBounds.fLeft <= op.bounds().fLeft && fBounds.fTop <= op.bounds().fTop &&
...
OpsTask::OpsTask(
    GrDrawingManager* drawingMgr, GrSurfaceProxyView view, GrAuditTrail* auditTrail,
    ...
    : GrRenderTask(),
      ...
      fArenas{std::move(arenas)} SkDEBUGCODE(, fNumClips(0))
...
OpsTask::~OpsTask() { this->deleteOps(); }

void OpsTask::addOp(
    GrDrawingManager* drawingMgr, GrOp::Owner op, GrTextureResolveManager textureResolveManager,
    const GrCaps& caps) {
  ...
  this->recordOp(
      std::move(op), false /*usesMSAA*/, GrProcessorSet::EmptySetAnalysis(), nullptr, nullptr,
      caps);
}

void OpsTask::addDrawOp(
    GrDrawingManager* drawingMgr, GrOp::Owner op, bool usesMSAA,
    const GrProcessorSet::Analysis& processorAnalysis, GrAppliedClip&& clip,
    const GrDstProxyView& dstProxyView, GrTextureResolveManager textureResolveManager,
    const GrCaps& caps) {
  auto addDependency = [&](GrSurfaceProxy* p, GrMipmapped mipmapped) {
  ...
  if (dstProxyView.proxy()) {
    ...
    SkASSERT(
        !(dstProxyView.dstSampleFlags() & GrDstSampleFlags::kAsInputAttachment) ||
  ...
  this->recordOp(
      std::move(op), usesMSAA, processorAnalysis, clip.doesClip() ? &clip : nullptr, &dstProxyView,
      caps);
}
  ...
  for (const auto& chain : fOpChains) {
    if (chain.shouldExecute()) {
      chain.head()->prePrepare(
          context, dstView, chain.appliedClip(), chain.dstProxyView(), fRenderPassXferBarriers,
          fColorLoadOp);
    }
  }
}
  ...
  for (const auto& chain : fOpChains) {
    if (chain.shouldExecute()) {
      GrOpFlushState::OpArgs opArgs(
          chain.head(), dstView, fUsesMSAASurface, chain.appliedClip(), chain.dstProxyView(),
          fRenderPassXferBarriers, fColorLoadOp);
...
bool OpsTask::onExecute(GrOpFlushState* flushState) {
  ...
  SkASSERT(
      fColorLoadOp != GrLoadOp::kClear || !flushState->gpu()->caps()->performColorClearsAsDraws());
  ...
  if (proxy->needsStencil()) {
    ...
    if (!flushState->resourceProvider()->attachStencilAttachment(renderTarget, fUsesMSAASurface)) {
  ...
  switch (fInitialStencilContent) {
    case StencilContent::kDontCare: stencilLoadOp = GrLoadOp::kDiscard; break;
  ...
  GrOpsRenderPass* renderPass = create_render_pass(
      flushState->gpu(), proxy->peekRenderTarget(), fUsesMSAASurface, stencil, fTargetOrigin,
...
bool OpsTask::canMerge(const OpsTask* opsTask) const {
  return this->target(0) == opsTask->target(0) && fArenas == opsTask->fArenas &&
...
int OpsTask::mergeFrom(SkSpan<const sk_sp<GrRenderTask>> tasks) {
  ...
  for (const auto& toMerge : mergingNodes) {
    ...
    fDeferredProxies.move_back_n(
        toMerge->fDeferredProxies.count(), toMerge->fDeferredProxies.data());
    fSampledProxies.move_back_n(toMerge->fSampledProxies.count(), toMerge->fSampledProxies.data());
    fOpChains.move_back_n(toMerge->fOpChains.count(), toMerge->fOpChains.data());
...
#if GR_TEST_UTILS
void OpsTask::dump(
    const SkString& label, SkString indent, bool printDependencies, bool close) const {
  GrRenderTask::dump(label, indent, printDependencies, false);

  SkDebugf("%sfColorLoadOp: ", indent.c_str());
  switch (fColorLoadOp) {
    case GrLoadOp::kLoad: SkDebugf("kLoad\n"); break;
    case GrLoadOp::kClear:
      SkDebugf(
          "kClear {%g, %g, %g, %g}\n", fLoadClearColor[0], fLoadClearColor[1], fLoadClearColor[2],
          fLoadClearColor[3]);
      break;
    case GrLoadOp::kDiscard: SkDebugf("kDiscard\n"); break;
  }

  SkDebugf("%sfInitialStencilContent: ", indent.c_str());
  switch (fInitialStencilContent) {
    case StencilContent::kDontCare: SkDebugf("kDontCare\n"); break;
    case StencilContent::kUserBitsCleared: SkDebugf("kUserBitsCleared\n"); break;
    case StencilContent::kPreserved: SkDebugf("kPreserved\n"); break;
  }
...
void OpsTask::gatherProxyIntervals(GrResourceAllocator* alloc) const {
  ...
  if (fOpChains.count()) {
    ...
    alloc->addInterval(
        targetProxy, cur, cur + fOpChains.count() - 1, GrResourceAllocator::ActualUse::kYes);
  } else {
    // This can happen if there is a loadOp (e.g., a clear) but no other draws. In this case we
    // still need to add an interval for the destination so we create a fake op# for
    // the missing clear op.
    alloc->addInterval(
        targetProxy, alloc->curOp(), alloc->curOp(), GrResourceAllocator::ActualUse::kYes);
    alloc->incOps();
  }
  ...
  auto gather = [alloc SkDEBUGCODE(, this)](GrSurfaceProxy* p, GrMipmapped) {
    alloc->addInterval(
        ...
        GrResourceAllocator::ActualUse::kYes SkDEBUGCODE(, this->target(0) == p));
  };
...
void OpsTask::recordOp(
    GrOp::Owner op, bool usesMSAA, GrProcessorSet::Analysis processorAnalysis, GrAppliedClip* clip,
  ...
  GrOP_INFO(
      ...
      this->uniqueID(), op->name(), op->uniqueID(), op->bounds().fLeft, op->bounds().fTop,
  ...
  if (maxCandidates) {
    ...
    while (true) {
      ...
      op = candidate.appendOp(
          std::move(op), processorAnalysis, dstProxyView, clip, caps, fArenas->arenaAlloc(),
          fAuditTrail);
      if (!op) {
        return;
      }
...
GrRenderTask::ExpectedOutcome OpsTask::onMakeClosed(
    GrRecordingContext* rContext, SkIRect* targetUpdateBounds) {
  ...
  if (!this->isColorNoOp()) {
    ...
    if (clippedContentBounds.intersect(fTotalBounds)) {
      ...
      *targetUpdateBounds = GrNativeRect::MakeIRectRelativeTo(
          fTargetOrigin, this->target(0)->backingStoreDimensions().height(), fClippedContentBounds);
