...
class NonAAStrokeRectOp final : public GrMeshDrawOp {
 ...
 public:
  ...
  static GrOp::Owner Make(
    ...
    return Helper::FactoryHelper<NonAAStrokeRectOp>(
        context, std::move(paint), inputFlags, viewMatrix, rect, stroke, aaType);
  }
  ...
  GrProcessorSet::Analysis finalize(
    ...
    return fHelper.finalizeProcessors(
        caps, clip, clampType, GrProcessorAnalysisCoverage::kNone, &fColor, nullptr);
  }
 ...
 private:
  ...
  void onCreateProgramInfo(
      ...
      bool usesMSAASurface, GrAppliedClip&& clip, const GrDstProxyView& dstProxyView,
    ...
    GrGeometryProcessor* gp;
    {
      ...
      gp = GrDefaultGeoProcFactory::Make(
          arena, color, Coverage::kSolid_Type, localCoordsType, fViewMatrix);
    }
    ...
    fProgramInfo = fHelper.createProgramInfo(
        caps, arena, writeView, usesMSAASurface, std::move(clip), dstProxyView, gp, primType,
  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    void* verts = target->makeVertexSpace(kVertexStride, vertexCount, &vertexBuffer, &firstVertex);
...
bool compute_aa_rects(
    const GrCaps& caps, SkRect* devOutside, SkRect* devOutsideAssist, SkRect* devInside,
    bool* isDegenerate, const SkMatrix& viewMatrix, const SkRect& rect, SkScalar strokeWidth,
    bool miterStroke, SkVector* devHalfStrokeSize) {
...
GrGeometryProcessor* create_aa_stroke_rect_gp(
    SkArenaAlloc* arena, bool usesMSAASurface, bool tweakAlphaForCoverage,
    const SkMatrix& viewMatrix, bool usesLocalCoords, bool wideColor) {
  using namespace GrDefaultGeoProcFactory;
  ...
  Coverage::Type coverageType =
      usesMSAASurface
          ? Coverage::kAttributeUnclamped_Type
          : (!tweakAlphaForCoverage ? Coverage::kAttribute_Type : Coverage::kSolid_Type);
  LocalCoords::Type localCoordsType =
...
class AAStrokeRectOp final : public GrMeshDrawOp {
 ...
 public:
  ...
  static GrOp::Owner Make(
    ...
    return Helper::FactoryHelper<AAStrokeRectOp>(
        context, std::move(paint), viewMatrix, devOutside, devInside, devHalfStrokeSize);
  }
  ...
  AAStrokeRectOp(
    ...
    fRects.emplace_back(
        RectInfo{color, devOutside, devOutside, devInside, devHalfStrokeSize, false});
  ...
  static GrOp::Owner Make(
    ...
    RectInfo info;
    if (!compute_aa_rects(
            *context->priv().caps(), &info.fDevOutside, &info.fDevOutsideAssist, &info.fDevInside,
    ...
    return Helper::FactoryHelper<AAStrokeRectOp>(
        context, std::move(paint), viewMatrix, info, isMiter);
  }

  AAStrokeRectOp(
      GrProcessorSet* processorSet, const SkPMColor4f& color, const SkMatrix& viewMatrix,
      const RectInfo& infoExceptColor, bool isMiter)
      : INHERITED(ClassID()), fHelper(processorSet, GrAAType::kCoverage), fViewMatrix(viewMatrix) {
  ...
  GrProcessorSet::Analysis finalize(
      const GrCaps& caps, const GrAppliedClip* clip, GrClampType clampType) override {
    return fHelper.finalizeProcessors(
        caps, clip, clampType, GrProcessorAnalysisCoverage::kSingleChannel, &fRects.back().fColor,
        &fWideColor);
  }
 ...
 private:
  ...
  void generateAAStrokeRectGeometry(
      VertexWriter& vertices, const SkPMColor4f& color, bool wideColor, const SkRect& devOutside,
      const SkRect& devOutsideAssist, const SkRect& devInside, bool miterStroke, bool degenerate,
      const SkVector& devHalfStrokeSize, bool usesMSAASurface) const;
...
void AAStrokeRectOp::onCreateProgramInfo(
  ...
  GrGeometryProcessor* gp = create_aa_stroke_rect_gp(
      arena, usesMSAASurface, this->compatibleWithCoverageAsAlpha(usesMSAASurface),
      this->viewMatrix(), fHelper.usesLocalCoords(), fWideColor);
  if (!gp) {
...
void AAStrokeRectOp::onPrepareDraws(GrMeshDrawTarget* target) {
  ...
  PatternHelper helper(
      ...
      std::move(indexBuffer), verticesPerInstance, indicesPerInstance, instanceCount, maxQuads);
  ...
  for (int i = 0; i < instanceCount; i++) {
    ...
    this->generateAAStrokeRectGeometry(
        vertices, info.fColor, fWideColor, info.fDevOutside, info.fDevOutsideAssist,
        info.fDevInside, fMiterStroke, info.fDegenerate, info.fDevHalfStrokeSize,
...
sk_sp<const GrGpuBuffer> AAStrokeRectOp::GetIndexBuffer(
    GrResourceProvider* resourceProvider, bool miterStroke) {
  if (miterStroke) {
    // clang-format off
...
void AAStrokeRectOp::generateAAStrokeRectGeometry(
    VertexWriter& vertices, const SkPMColor4f& color, bool wideColor, const SkRect& devOutside,
    const SkRect& devOutsideAssist, const SkRect& devInside, bool miterStroke, bool degenerate,
    const SkVector& devHalfStrokeSize, bool usesMSAASurface) const {
  ...
  if (usesMSAASurface) {
    ...
    float insetExtraBloat =
        std::min(inset + msaaExtraBloat, std::min(devHalfStrokeSize.fX, devHalfStrokeSize.fY)) -
        inset;
    ...
    float interiorExtraBloat =
        std::min(
            interiorOutset + msaaExtraBloat, std::min(devInside.width(), devInside.height()) / 2) -
        interiorOutset;
  ...
  vertices.writeQuad(
      inset_fan(devOutside, -outset, -outset), outerColor, maybe_coverage(outerCoverage));

  if (!miterStroke) {
    // Second exterior outset.
    vertices.writeQuad(
        inset_fan(devOutsideAssist, -outset, -outset), outerColor, maybe_coverage(outerCoverage));
  }

  // Exterior inset rect (toward stroke).
  vertices.writeQuad(
      inset_fan(devOutside, inset, inset), innerColor, maybe_coverage(innerCoverage));

  if (!miterStroke) {
    // Second exterior inset.
    vertices.writeQuad(
        inset_fan(devOutsideAssist, inset, inset), innerColor, maybe_coverage(innerCoverage));
  }

  if (!degenerate) {
    // Interior inset rect (toward stroke).
    vertices.writeQuad(
        inset_fan(devInside, -inset, -inset), innerColor, maybe_coverage(innerCoverage));
    ...
    VertexColor interiorColor(tweakAlphaForCoverage ? color * interiorCoverage : color, wideColor);
    vertices.writeQuad(
        VertexWriter::TriFanFromRect(interiorAABoundary), interiorColor,
    ...
    vertices.writeQuad(
        VertexWriter::TriFanFromRect(devInside), innerColor, maybe_coverage(innerCoverage));
...
GrOp::Owner MakeNested(
  ...
  if (devInside.isEmpty() || !devInside.intersect(visibilityBounds)) {
    ...
    DrawQuad quad{
        GrQuad::MakeFromRect(rects[0], viewMatrix), GrQuad(rects[0]), GrQuadAAFlags::kAll};
    return FillRectOp::Make(context, std::move(paint), GrAAType::kCoverage, &quad);
  }

  return AAStrokeRectOp::Make(
      context, std::move(paint), viewMatrix, devOutside, devInside, SkVector{dx, dy} * .5f);
}
...
GR_DRAW_OP_TEST_DEFINE(NonAAStrokeRectOp) {
  ...
  return skgpu::v1::StrokeRectOp::NonAAStrokeRectOp::Make(
      context, std::move(paint), viewMatrix, rect, strokeRec, aaType);
}
...
GR_DRAW_OP_TEST_DEFINE(AAStrokeRectOp) {
  ...
  rec.setStrokeParams(
      SkPaint::kButt_Cap, miterStroke ? SkPaint::kMiter_Join : SkPaint::kBevel_Join, 1.f);
  SkMatrix matrix = GrTest::TestMatrixRectStaysRect(random);
  return skgpu::v1::StrokeRectOp::AAStrokeRectOp::Make(
      context, std::move(paint), matrix, rect, rec);
}
