...
class AtlasRenderTask final : public OpsTask {
 public:
  ...
  bool addPath(
      const SkMatrix&, const SkPath&, SkIPoint pathDevTopLeft, int widthInAtlas, int heightInAtlas,
  ...
  instantiate(GrOnFlushResourceProvider* onFlushRP, sk_sp<GrTexture> backingTexture = nullptr) {
 ...
 private:
  ...
  void stencilAtlasRect(
      GrRecordingContext*, const SkRect&, const SkPMColor4f&, const GrUserStencilSettings*);
  ...
  class AtlasPathList : SkNoncopyable {
   public:
    void add(PathDrawAllocator* alloc, const SkMatrix& pathMatrix, const SkPath& path) {
      fPathDrawList =
          &alloc->emplace_back(pathMatrix, path, SK_PMColor4fTRANSPARENT, fPathDrawList);
