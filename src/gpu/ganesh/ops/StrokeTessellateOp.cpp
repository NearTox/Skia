...
GrOp::CombineResult StrokeTessellateOp::onCombineIfPossible(
  ...
  constexpr static GrTFlagsMask<PatchAttribs> kDynamicStatesMask(
      PatchAttribs::kStrokeParams | PatchAttribs::kColor);
...
void StrokeTessellateOp::prePrepareTessellator(
  ...
  fTessellationShader = args.fArena->make<GrStrokeTessellationShader>(
      *caps.shaderCaps(), fPatchAttribs, fViewMatrix, this->headStroke(), this->headColor());

  auto fillStencil = &GrUserStencilSettings::kUnused;
  if (fNeedsStencil) {
    fStencilProgram =
        GrTessellationShader::MakeProgram(args, fTessellationShader, pipeline, &kMarkStencil);
    fillStencil = &kTestAndResetStencil;
    args.fXferBarrierFlags = GrXferBarrierFlags::kNone;
  }

  fFillProgram =
      GrTessellationShader::MakeProgram(args, fTessellationShader, pipeline, fillStencil);
}
