...
void AtlasTextOp::ClearCache() noexcept {
...
auto AtlasTextOp::Geometry::Make(
    const sktext::gpu::AtlasSubRun& subRun, const SkMatrix& drawMatrix, SkPoint drawOrigin,
    SkIRect clipRect, sk_sp<SkRefCnt>&& supportData, const SkPMColor4f& color, SkArenaAlloc* alloc)
    -> Geometry* {
  ...
  return new (geo)
      Geometry{subRun, drawMatrix, drawOrigin, clipRect, std::move(supportData), color};
}
...
GrOp::Owner AtlasTextOp::CreateOpTestingOnly(
  ...
  std::tie(std::ignore, op) =
      subRun->makeAtlasTextOp(nullptr, mtxProvider, glyphRunList.origin(), skPaint, blob, sdc);
  return op;
}
#endif
