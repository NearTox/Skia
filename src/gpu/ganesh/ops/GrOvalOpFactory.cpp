...
class CircleGeometryProcessor : public GrGeometryProcessor {
 public:
  ...
  void addToKey(const GrShaderCaps& caps, skgpu::KeyBuilder* b) const override {
    ...
    b->addBits(
        ProgramImpl::kMatrixKeyBits, ProgramImpl::ComputeMatrixKey(caps, fLocalMatrix),
        "localMatrixType");
  }
 ...
 private:
  CircleGeometryProcessor(
    ...
    if (roundCaps) {
      SkASSERT(stroke);
      SkASSERT(clipPlane);
      fInRoundCapCenters = {"inRoundCapCenters", kFloat4_GrVertexAttribType, SkSLType::kFloat4};
    }
  ...
  class Impl : public ProgramImpl {
   public:
    void setData(
        const GrGLSLProgramDataManager& pdman, const GrShaderCaps& shaderCaps,
        const GrGeometryProcessor& geomProc) override {
      SetTransform(
          pdman, shaderCaps, fLocalMatrixUniform,
          geomProc.cast<CircleGeometryProcessor>().fLocalMatrix, &fLocalMatrix);
    }
   ...
   private:
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
      ...
      if (cgp.fInClipPlane.isInitialized()) {
        ...
        varyingHandler->addPassThroughAttribute(cgp.fInClipPlane.asShaderVar(), "clipPlane");
      }
      if (cgp.fInIsectPlane.isInitialized()) {
        fragBuilder->codeAppend("half3 isectPlane;");
        varyingHandler->addPassThroughAttribute(cgp.fInIsectPlane.asShaderVar(), "isectPlane");
      }
      if (cgp.fInUnionPlane.isInitialized()) {
        SkASSERT(cgp.fInClipPlane.isInitialized());
        fragBuilder->codeAppend("half3 unionPlane;");
        varyingHandler->addPassThroughAttribute(cgp.fInUnionPlane.asShaderVar(), "unionPlane");
      }
      ...
      if (cgp.fInRoundCapCenters.isInitialized()) {
        ...
        varyingHandler->addPassThroughAttribute(
            cgp.fInRoundCapCenters.asShaderVar(), "roundCapCenters");
      ...
      WriteLocalCoord(
          vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, cgp.fInPosition.asShaderVar(),
          cgp.fLocalMatrix, &fLocalMatrixUniform);
  ...
  class Impl : public ProgramImpl {
   public:
    void setData(
      ...
      SetTransform(
          pdman, shaderCaps, fLocalMatrixUniform,
          geomProc.cast<ButtCapDashedCircleGeometryProcessor>().fLocalMatrix, &fLocalMatrix);
    }
   ...
   private:
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
      ...
      fragBuilder->codeAppend("float4 circleEdge;");
      varyingHandler->addPassThroughAttribute(bcscgp.fInCircleEdge.asShaderVar(), "circleEdge");

      fragBuilder->codeAppend("float4 dashParams;");
      varyingHandler->addPassThroughAttribute(
          bcscgp.fInDashParams.asShaderVar(), "dashParams",
      ...
      fragBuilder->codeAppendf("half4 %s;", args.fOutputColor);
      varyingHandler->addPassThroughAttribute(
          bcscgp.fInColor.asShaderVar(), args.fOutputColor,
          GrGLSLVaryingHandler::Interpolation::kCanBeFlat);

      // Setup position
      WriteOutputPosition(vertBuilder, gpArgs, bcscgp.fInPosition.name());
      WriteLocalCoord(
          vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, bcscgp.fInPosition.asShaderVar(),
          bcscgp.fLocalMatrix, &fLocalMatrixUniform);
      ...
      fragBuilder->emitFunction(
          SkSLType::kFloat, fnName.c_str(), {fnArgs, SK_ARRAY_COUNT(fnArgs)}, R"(
                    float linearDist;
  ...
  class Impl : public ProgramImpl {
   ...
   private:
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
      ...
      WriteLocalCoord(
          vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, egp.fInPosition.asShaderVar(),
          egp.fLocalMatrix, &fLocalMatrixUniform);
...
class DIEllipseGeometryProcessor : public GrGeometryProcessor {
 public:
  ...
  void addToKey(const GrShaderCaps& caps, skgpu::KeyBuilder* b) const override {
    ...
    b->addBits(
        ProgramImpl::kMatrixKeyBits, ProgramImpl::ComputeMatrixKey(caps, fViewMatrix),
        "viewMatrixType");
  }
 ...
 private:
  DIEllipseGeometryProcessor(
    ...
    fInColor = MakeColorAttribute("inColor", wideColor);
    if (useScale) {
      fInEllipseOffsets0 = {"inEllipseOffsets0", kFloat3_GrVertexAttribType, SkSLType::kFloat3};
    } else {
      fInEllipseOffsets0 = {"inEllipseOffsets0", kFloat2_GrVertexAttribType, SkSLType::kFloat2};
    }
  ...
  class Impl : public ProgramImpl {
   ...
   private:
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
      ...
      GrGLSLFPFragmentBuilder* fragBuilder = args.fFragBuilder;
      fragBuilder->codeAppendf("half4 %s;", args.fOutputColor);
      varyingHandler->addPassThroughAttribute(diegp.fInColor.asShaderVar(), args.fOutputColor);

      // Setup position
      WriteOutputPosition(
          vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, diegp.fInPosition.name(),
          diegp.fViewMatrix, &fViewMatrixUniform);
...
class CircleOp final : public GrMeshDrawOp {
 ...
 public:
  ...
  CircleOp(
      ...
      : GrMeshDrawOp(ClassID()), fHelper(processorSet, GrAAType::kCoverage) {
    const SkStrokeRec& stroke = style.strokeRec();
    SkStrokeRec::Style recStyle = stroke.getStyle();
  ...
  GrProcessorSet::Analysis finalize(
    ...
    return fHelper.finalizeProcessors(
        caps, clip, clampType, GrProcessorAnalysisCoverage::kSingleChannel, color, &fWideColor);
  }
 ...
 private:
  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    VertexWriter vertices = target->makeVertexWriter(
        fProgramInfo->geomProc().vertexStride(), fVertCount, &vertexBuffer, &firstVertex);
    if (!vertices) {
    ...
    for (const auto& circle : fCircles) {
      ...
      for (int i = 0; i < 8; ++i) {
        ...
        vertices << (center + offset * halfWidth) << color << offset << radii;
        if (fClipPlane) {
      ...
      if (circle.fStroked) {
        ...
        for (int i = 0; i < 8; ++i) {
          vertices << (center + kOctagonInner[i] * circle.fInnerRadius) << color
 ...
 private:
  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    for (const auto& circle : fCircles) {
      ...
      for (int i = 0; i < 8; ++i) {
        vertices << (center + kOctagonOuter[i] * halfWidth) << color << reflectY(kOctagonOuter[i])
      ...
      for (int i = 0; i < 8; ++i) {
        vertices << (center + kOctagonInner[i] * circle.fInnerRadius) << color
                 << (reflectY(kOctagonInner[i]) * normInnerRadius) << circle.fOuterRadius
                 << normInnerRadius << dashParams;
      }
...
class EllipseOp final : public GrMeshDrawOp {
 ...
 public:
  ...
  EllipseOp(
      ...
      : INHERITED(ClassID()), fHelper(processorSet, GrAAType::kCoverage), fUseScale(false) {
    SkStrokeRec::Style style = stroke.getStyle();
    bool isStrokeOnly =
        SkStrokeRec::kStroke_Style == style || SkStrokeRec::kHairline_Style == style;
  ...
  GrProcessorSet::Analysis finalize(
      const GrCaps& caps, const GrAppliedClip* clip, GrClampType clampType) override {
    fUseScale = !caps.shaderCaps()->fFloatIs32Bits && !caps.shaderCaps()->fHasLowFragmentPrecision;
 ...
 private:
  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    for (const auto& ellipse : fEllipses) {
      ...
      // The inner radius in the vertex data must be specified in normalized space.
      verts.writeQuad(
          VertexWriter::TriStripFromRect(ellipse.fDevBounds.makeOutset(aaBloat, aaBloat)), color,
          origin_centered_tri_strip(xMaxOffset, yMaxOffset),
          VertexWriter::If(fUseScale, std::max(xRadius, yRadius)), invRadii);
    }
    fMesh = helper.mesh();
  }
...
class DIEllipseOp final : public GrMeshDrawOp {
 ...
 public:
  ...
  DIEllipseOp(
      GrProcessorSet* processorSet, const SkPMColor4f& color, const DeviceSpaceParams& params,
 ...
 private:
  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    for (const auto& ellipse : fEllipses) {
      ...
      float aaBloat = target->usesMSAASurface() ? SK_ScalarSqrt2 : .5f;
      SkRect drawBounds =
          ellipse.fBounds.makeOutset(ellipse.fGeoDx * aaBloat, ellipse.fGeoDy * aaBloat);
      ...
      verts.writeQuad(
          VertexWriter::TriStripFromRect(drawBounds), color,
...
class CircularRRectOp final : public GrMeshDrawOp {
 ...
 private:
  static void FillInOverstrokeVerts(
      VertexWriter& verts, const SkRect& bounds, SkScalar smInset, SkScalar bigInset,
      SkScalar xOffset, SkScalar outerRadius, SkScalar innerRadius, const VertexColor& color) {
    SkASSERT(smInset < bigInset);

    // TL
    verts << (bounds.fLeft + smInset) << (bounds.fTop + smInset) << color << xOffset << 0.0f
          << outerRadius << innerRadius;

    // TR
    verts << (bounds.fRight - smInset) << (bounds.fTop + smInset) << color << xOffset << 0.0f
          << outerRadius << innerRadius;

    verts << (bounds.fLeft + bigInset) << (bounds.fTop + bigInset) << color << 0.0f << 0.0f
          << outerRadius << innerRadius;

    verts << (bounds.fRight - bigInset) << (bounds.fTop + bigInset) << color << 0.0f << 0.0f
          << outerRadius << innerRadius;

    verts << (bounds.fLeft + bigInset) << (bounds.fBottom - bigInset) << color << 0.0f << 0.0f
          << outerRadius << innerRadius;

    verts << (bounds.fRight - bigInset) << (bounds.fBottom - bigInset) << color << 0.0f << 0.0f
          << outerRadius << innerRadius;

    // BL
    verts << (bounds.fLeft + smInset) << (bounds.fBottom - smInset) << color << xOffset << 0.0f
          << outerRadius << innerRadius;

    // BR
    verts << (bounds.fRight - smInset) << (bounds.fBottom - smInset) << color << xOffset << 0.0f
          << outerRadius << innerRadius;
  }
  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    for (const auto& rrect : fRRects) {
      ...
      for (int i = 0; i < 4; ++i) {
        verts << bounds.fLeft << yCoords[i] << color << -1.0f << yOuterRadii[i] << outerRadius
              << innerRadius;

        verts << (bounds.fLeft + outerRadius) << yCoords[i] << color << 0.0f << yOuterRadii[i]
              << outerRadius << innerRadius;

        verts << (bounds.fRight - outerRadius) << yCoords[i] << color << 0.0f << yOuterRadii[i]
              << outerRadius << innerRadius;

        verts << bounds.fRight << yCoords[i] << color << 1.0f << yOuterRadii[i] << outerRadius
              << innerRadius;
      }
 ...
 private:
  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    PatternHelper helper(
        target, GrPrimitiveType::kTriangles, fProgramInfo->geomProc().vertexStride(),
    ...
    for (const auto& rrect : fRRects) {
      ...
      for (int i = 0; i < 4; ++i) {
        verts << bounds.fLeft << yCoords[i] << color << xMaxOffset << yOuterOffsets[i] << maybeScale
              << reciprocalRadii;

        verts << (bounds.fLeft + xOuterRadius) << yCoords[i] << color << SK_ScalarNearlyZero
              << yOuterOffsets[i] << maybeScale << reciprocalRadii;

        verts << (bounds.fRight - xOuterRadius) << yCoords[i] << color << SK_ScalarNearlyZero
              << yOuterOffsets[i] << maybeScale << reciprocalRadii;

        verts << bounds.fRight << yCoords[i] << color << xMaxOffset << yOuterOffsets[i]
...
GrOp::Owner GrOvalOpFactory::MakeCircularRRectOp(
...
GrOp::Owner GrOvalOpFactory::MakeRRectOp(
    GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRRect& rrect,
    const SkStrokeRec& stroke, const GrShaderCaps* shaderCaps) {
  if (rrect.isOval()) {
    return MakeOvalOp(
...
GrOp::Owner GrOvalOpFactory::MakeCircleOp(
...
GrOp::Owner GrOvalOpFactory::MakeOvalOp(
    GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRect& oval,
    const GrStyle& style, const GrShaderCaps* shaderCaps) {
  if (style.pathEffect()) {
    return nullptr;
...
GrOp::Owner GrOvalOpFactory::MakeArcOp(
    GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRect& oval,
...
GR_DRAW_OP_TEST_DEFINE(CircularRRectOp) {
  do {
    ...
    GrOp::Owner op = GrOvalOpFactory::MakeCircularRRectOp(
