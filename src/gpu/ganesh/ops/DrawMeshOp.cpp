...
class MeshGP : public GrGeometryProcessor {
 public:
  static GrGeometryProcessor* Make(
      SkArenaAlloc* arena, sk_sp<SkMeshSpecification> spec,
      sk_sp<GrColorSpaceXform> colorSpaceXform, const SkMatrix& viewMatrix,
      const std::optional<SkPMColor4f>& color, bool needsLocalCoords,
      sk_sp<const SkData> uniforms) {
    return arena->make([&](void* ptr) {
      return new (ptr) MeshGP(
          std::move(spec), std::move(colorSpaceXform), viewMatrix, std::move(color),
          needsLocalCoords, std::move(uniforms));
    });
  }
 ...
 private:
  class Impl : public ProgramImpl {
   public:
    void setData(
      ...
      if (mgp.fUniforms) {
        pdman.setRuntimeEffectUniforms(
            mgp.fSpec->uniforms(), SkMakeSpan(fSpecUniformHandles), mgp.fUniforms->data());
      }
    }
   ...
   private:
    class MeshCallbacks : public SkSL::PipelineStage::Callbacks {
     public:
      MeshCallbacks(
          Impl* self, const MeshGP& gp, GrGLSLShaderBuilder* builder,
          GrGLSLUniformHandler* uniformHandler, const char* mainName,
          const SkSL::Context& context) noexcept
          : fSelf(self),
            fGP(gp),
            fBuilder(builder),
      ...
      std::string declareUniform(const SkSL::VarDeclaration* decl) override {
        ...
        auto it = std::find_if(
            ...
            [&name](SkMeshSpecification::Uniform uniform) noexcept {
        ...
        *handle = fUniformHandler->addUniformArray(
            &fGP, shaderFlags, gpuType, name.c_str(), isArray ? var.type().columns() : 0,
            &mangledName);
      ...
      void declareFunction(const char* decl) override { fBuilder->emitFunctionPrototype(decl); }

      void defineStruct(const char* definition) override { fBuilder->definitionAppend(definition); }
    ...
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
      ...
      MeshCallbacks vsCallbacks(
          this, mgp, vertBuilder, uniformHandler, userVertName.c_str(), *customVS->fContext);
      SkSL::PipelineStage::ConvertProgram(
          *customVS,
          /*sampleCoords=*/"",
          /*inputColor=*/"",
          /*destColor=*/"", &vsCallbacks);

      // Copy the individual attributes into a struct
      vertBuilder->codeAppendf("%s attributes;", vsCallbacks.getMangledName("Attributes").c_str());
      size_t i = 0;
      SkASSERT(mgp.vertexAttributes().count() == (int)mgp.fSpec->attributes().size());
      for (auto attr : mgp.vertexAttributes()) {
        vertBuilder->codeAppendf(
            "attributes.%s = %s;", mgp.fSpec->attributes()[i++].name.c_str(), attr.name());
      }

      // Call the user's vert function.
      vertBuilder->codeAppendf("%s varyings;", vsCallbacks.getMangledName("Varyings").c_str());
      vertBuilder->codeAppendf("float2 pos = %s(attributes, varyings);", userVertName.c_str());
      ...
      for (const auto& v : SkMeshSpecificationPriv::Varyings(*mgp.fSpec)) {
        ...
        vertBuilder->codeAppendf("%s = varyings.%s;", varyings.back().vsOut(), v.name.c_str());
      }

      // Setup position
      WriteOutputPosition(
          vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, "pos", mgp.fViewMatrix,
          &fViewMatrixUniform);
      ...
      MeshCallbacks fsCallbacks(
          this, mgp, fragBuilder, uniformHandler, userFragName.c_str(), *customFS->fContext);
      SkSL::PipelineStage::ConvertProgram(
          *customFS,
          /*sampleCoords=*/"",
          /*inputColor=*/"",
          /*destColor=*/"", &fsCallbacks);

      // Pack the varyings into a struct to call the user's frag code.
      fragBuilder->codeAppendf("%s varyings;", fsCallbacks.getMangledName("Varyings").c_str());
      i = 0;
      for (const auto& varying : SkMeshSpecificationPriv::Varyings(*mgp.fSpec)) {
        fragBuilder->codeAppendf("varyings.%s = %s;", varying.name.c_str(), varyings[i++].vsOut());
      }
      ...
      if (mgp.fColor != SK_PMColor4fILLEGAL) {
        fColorUniform = uniformHandler->addUniform(
            nullptr, kFragment_GrShaderFlag, SkSLType::kHalf4, "color", &uniformColorName);
      }
      ...
      if (meshColorType == SkMeshSpecificationPriv::ColorType::kNone) {
        fragBuilder->codeAppendf(
            "%s %s(varyings);", localCoordAssignment.c_str(), userFragName.c_str());
        SkASSERT(uniformColorName);
        fragBuilder->codeAppendf("%s = %s;", args.fOutputColor, uniformColorName);
      } else {
        fColorSpaceHelper.emitCode(
            uniformHandler, mgp.fColorSpaceXform.get(), kFragment_GrShaderFlag);
        ...
        fragBuilder->codeAppendf(
            "%s %s(varyings, color);", localCoordAssignment.c_str(), userFragName.c_str());
  ...
  MeshGP(
      sk_sp<SkMeshSpecification> spec, sk_sp<GrColorSpaceXform> colorSpaceXform,
      const SkMatrix& viewMatrix, const std::optional<SkPMColor4f>& color, bool needsLocalCoords,
      sk_sp<const SkData> uniforms)
    ...
    for (const auto& srcAttr : fSpec->attributes()) {
      fAttributes.emplace_back(
          srcAttr.name.c_str(), SkMeshSpecificationPriv::AttrTypeAsVertexAttribType(srcAttr.type),
          SkMeshSpecificationPriv::AttrTypeAsSLType(srcAttr.type), srcAttr.offset);
    }
...
class MeshOp final : public GrMeshDrawOp {
 ...
 public:
  ...
  MeshOp(
      GrProcessorSet*, const SkPMColor4f&, const SkMesh&, GrAAType, sk_sp<GrColorSpaceXform>,
      const SkMatrixProvider&);

  MeshOp(
      GrProcessorSet*, const SkPMColor4f&, sk_sp<SkVertices>, const GrPrimitiveType*, GrAAType,
      sk_sp<GrColorSpaceXform>, const SkMatrixProvider&);

  const char* name() const noexcept override { return "MeshOp"; }
 ...
 private:
  ...
  void onCreateProgramInfo(
      const GrCaps*, SkArenaAlloc*, const GrSurfaceProxyView& writeView, bool usesMSAASurface,
      GrAppliedClip&&, const GrDstProxyView&, GrXferBarrierFlags renderPassXferBarriers,
  ...
  class Mesh {
   public:
    Mesh() = delete;
    Mesh(const SkMesh& mesh) noexcept;
    Mesh(sk_sp<SkVertices>, const SkMatrix& viewMatrix) noexcept;
    ...
    bool isFromVertices() const noexcept { return SkToBool(fVertices); }
    ...
    void writeVertices(
        skgpu::VertexWriter& writer, const SkMeshSpecification& spec, bool transform) const;
...
MeshOp::Mesh::Mesh(const SkMesh& mesh) noexcept {
...
MeshOp::Mesh::Mesh(sk_sp<SkVertices> vertices, const SkMatrix& viewMatrix) noexcept
    : fVertices(std::move(vertices)), fViewMatrix(viewMatrix) {
  SkASSERT(fVertices);
}

MeshOp::Mesh::Mesh(Mesh&& that) noexcept {
...
void MeshOp::Mesh::writeVertices(
    skgpu::VertexWriter& writer, const SkMeshSpecification& spec, bool transform) const {
...
MeshOp::MeshOp(
    GrProcessorSet* processorSet, const SkPMColor4f& color, const SkMesh& mesh, GrAAType aaType,
    sk_sp<GrColorSpaceXform> colorSpaceXform, const SkMatrixProvider& matrixProvider)
    : INHERITED(ClassID()),
  ...
  if (fColorSpaceXform) {
    fUniforms = SkRuntimeEffectPriv::TransformUniforms(
        mesh.spec()->uniforms(), mesh.uniforms(), fColorSpaceXform->steps());
  } else {
...
static sk_sp<SkMeshSpecification> make_vertices_spec(bool hasColors, bool hasTex) {
  ...
  if (hasColors) {
    ...
    fs +=
        "main(Varyings v, out float4 color) {\n"
  ...
  fs += "}";
  auto [spec, error] =
      SkMeshSpecification::Make(SkMakeSpan(attributes), size, SkMakeSpan(varyings), vs, fs);
  SkASSERT(spec);
  return spec;
}
...
MeshOp::MeshOp(
    ...
    const GrPrimitiveType* overridePrimitiveType, GrAAType aaType,
    sk_sp<GrColorSpaceXform> colorSpaceXform, const SkMatrixProvider& matrixProvider)
    : INHERITED(ClassID()),
      fHelper(processorSet, aaType),
      fColorSpaceXform(std::move(colorSpaceXform)),
  ...
  if (overridePrimitiveType) {
    ...
    switch (vertices->priv().mode()) {
      case SkVertices::kTriangles_VertexMode: fPrimitiveType = GrPrimitiveType::kTriangles; break;
      case SkVertices::kTriangleStrip_VertexMode:
        fPrimitiveType = GrPrimitiveType::kTriangleStrip;
        break;
      case SkVertices::kTriangleFan_VertexMode: SkUNREACHABLE;
    }
  }
...
GrGeometryProcessor* MeshOp::makeGP(SkArenaAlloc* arena) {
  ...
  return MeshGP::Make(
      arena, fSpecification, fColorSpaceXform, vm, color, fHelper.usesLocalCoords(), fUniforms);
}

void MeshOp::onCreateProgramInfo(
  ...
  fProgramInfo = fHelper.createProgramInfo(
      caps, arena, writeView, usesMSAASurface, std::move(appliedClip), dstProxyView,
      this->makeGP(arena), fPrimitiveType, renderPassXferBarriers, colorLoadOp);
}
...
void MeshOp::onPrepareDraws(GrMeshDrawTarget* target) {
  ...
  if (!vertexBuffer) {
    skgpu::VertexWriter verts =
        target->makeVertexWriter(vertexStride, fVertexCount, &vertexBuffer, &firstVertex);
    if (!verts) {
  ...
  if (indexBuffer) {
    fMesh->setIndexed(
        std::move(indexBuffer), fIndexCount, firstIndex,
        /*minIndexValue=*/0, fVertexCount, GrPrimitiveRestart::kNo, std::move(vertexBuffer),
        firstVertex);
  } else {
...
GrOp::CombineResult MeshOp::onCombineIfPossible(GrOp* t, SkArenaAlloc*, const GrCaps& caps) {
  ...
  if (!(fPrimitiveType == GrPrimitiveType::kTriangles ||
        fPrimitiveType == GrPrimitiveType::kLines || fPrimitiveType == GrPrimitiveType::kPoints)) {
  ...
  if (fViewMatrix != that->fViewMatrix) {
    ...
    if (fHelper.usesLocalCoords() && !SkMeshSpecificationPriv::HasLocalCoords(*fSpecification)) {
      return CombineResult::kCannotCombine;
    }
...
GrOp::Owner Make(
    GrRecordingContext* context, GrPaint&& paint, const SkMesh& mesh,
    const SkMatrixProvider& matrixProvider, GrAAType aaType,
    sk_sp<GrColorSpaceXform> colorSpaceXform) {
  return GrSimpleMeshDrawOpHelper::FactoryHelper<MeshOp>(
      context, std::move(paint), mesh, aaType, std::move(colorSpaceXform), matrixProvider);
}

GrOp::Owner Make(
    GrRecordingContext* context, GrPaint&& paint, sk_sp<SkVertices> vertices,
    const GrPrimitiveType* overridePrimitiveType, const SkMatrixProvider& matrixProvider,
    GrAAType aaType, sk_sp<GrColorSpaceXform> colorSpaceXform) {
  return GrSimpleMeshDrawOpHelper::FactoryHelper<MeshOp>(
      context, std::move(paint), std::move(vertices), overridePrimitiveType, aaType,
      std::move(colorSpaceXform), matrixProvider);
}
