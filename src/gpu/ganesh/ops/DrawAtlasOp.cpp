...
class DrawAtlasOpImpl final : public GrMeshDrawOp {
 ...
 public:
  ...
  DrawAtlasOpImpl(
      GrProcessorSet*, const SkPMColor4f& color, const SkMatrix& viewMatrix, GrAAType,
...
DrawAtlasOpImpl::DrawAtlasOpImpl(
    GrProcessorSet* processorSet, const SkPMColor4f& color, const SkMatrix& viewMatrix,
    GrAAType aaType, int spriteCount, const SkRSXform* xforms, const SkRect* rects,
  ...
  for (int spriteIndex = 0; spriteIndex < spriteCount; ++spriteIndex) {
    ...
    if (colors) {
      ...
      if (paintAlpha != 255) {
        spriteColor =
            SkColorSetA(spriteColor, SkMulDiv255Round(SkColorGetA(spriteColor), paintAlpha));
      }
...
void DrawAtlasOpImpl::onCreateProgramInfo(
...
GrOp::CombineResult DrawAtlasOpImpl::onCombineIfPossible(
...
GrProcessorSet::Analysis DrawAtlasOpImpl::finalize(
...
GrOp::Owner Make(
    GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, GrAAType aaType,
    int spriteCount, const SkRSXform* xforms, const SkRect* rects, const SkColor* colors) {
  return GrSimpleMeshDrawOpHelper::FactoryHelper<DrawAtlasOpImpl>(
...
static SkRSXform random_xform(SkRandom* random) {
  ...
  SkRSXform xform = SkRSXform::MakeFromRadians(
      random->nextRangeScalar(kMinScale, kMaxScale),
...
static SkRect random_texRect(SkRandom* random) {
  ...
  SkRect texRect = SkRect::MakeLTRB(
      random->nextRangeScalar(kMinCoord, kMaxCoord), random->nextRangeScalar(kMinCoord, kMaxCoord),
      random->nextRangeScalar(kMinCoord, kMaxCoord), random->nextRangeScalar(kMinCoord, kMaxCoord));
  texRect.sort();
  return texRect;
}
...
static void randomize_params(
    uint32_t count, SkRandom* random, SkTArray<SkRSXform>* xforms, SkTArray<SkRect>* texRects,
...
GR_DRAW_OP_TEST_DEFINE(DrawAtlasOp) {
  ...
  return skgpu::v1::DrawAtlasOp::Make(
      context, std::move(paint), viewMatrix, aaType, spriteCount, xforms.begin(), texRects.begin(),
