...
class SoftwarePathData {
 public:
  SoftwarePathData(
      const SkIRect& maskBounds, const SkMatrix& viewMatrix, const GrStyledShape& shape, GrAA aa)
...
bool get_unclipped_shape_dev_bounds(
...
GrSurfaceProxyView make_deferred_mask_texture_view(
    GrRecordingContext* rContext, SkBackingFit fit, SkISize dimensions) {
...
// is no intersection.
bool SoftwarePathRenderer::GetShapeAndClipBounds(
    SurfaceDrawContext* sdc, const GrClip* clip, const GrStyledShape& shape, const SkMatrix& matrix,
...
void SoftwarePathRenderer::DrawNonAARect(
    SurfaceDrawContext* sdc, GrPaint&& paint, const GrUserStencilSettings& userStencilSettings,
    const GrClip* clip, const SkMatrix& viewMatrix, const SkRect& rect,
    const SkMatrix& localMatrix) {
  sdc->stencilRect(
      clip, &userStencilSettings, std::move(paint), GrAA::kNo, viewMatrix, rect, &localMatrix);
}

void SoftwarePathRenderer::DrawAroundInvPath(
    SurfaceDrawContext* sdc, GrPaint&& paint, const GrUserStencilSettings& userStencilSettings,
    const GrClip* clip, const SkMatrix& viewMatrix, const SkIRect& devClipBounds,
  ...
  if (devClipBounds.fTop < devPathBounds.fTop) {
    rect.setLTRB(
        SkIntToScalar(devClipBounds.fLeft), SkIntToScalar(devClipBounds.fTop),
        SkIntToScalar(devClipBounds.fRight), SkIntToScalar(devPathBounds.fTop));
    DrawNonAARect(
        sdc, GrPaint::Clone(paint), userStencilSettings, clip, SkMatrix::I(), rect, invert);
  }
  if (devClipBounds.fLeft < devPathBounds.fLeft) {
    rect.setLTRB(
        SkIntToScalar(devClipBounds.fLeft), SkIntToScalar(devPathBounds.fTop),
  ...
  if (devClipBounds.fRight > devPathBounds.fRight) {
    rect.setLTRB(
        SkIntToScalar(devPathBounds.fRight), SkIntToScalar(devPathBounds.fTop),
  ...
  if (devClipBounds.fBottom > devPathBounds.fBottom) {
    rect.setLTRB(
        SkIntToScalar(devClipBounds.fLeft), SkIntToScalar(devPathBounds.fBottom),
        SkIntToScalar(devClipBounds.fRight), SkIntToScalar(devClipBounds.fBottom));
    DrawNonAARect(sdc, std::move(paint), userStencilSettings, clip, SkMatrix::I(), rect, invert);
  }
}

void SoftwarePathRenderer::DrawToTargetWithShapeMask(
    GrSurfaceProxyView view, SurfaceDrawContext* sdc, GrPaint&& paint,
  ...
  SkMatrix maskMatrix = SkMatrix::Translate(
  ...
  DrawNonAARect(sdc, std::move(paint), userStencilSettings, clip, SkMatrix::I(), dstRect, invert);
}

////////////////////////////////////////////////////////////////////////////////
// return true on success; false on failure
bool SoftwarePathRenderer::onDrawPath(const DrawPathArgs& args) {
  GR_AUDIT_TRAIL_AUTO_FRAME(args.fContext->priv().auditTrail(), "SoftwarePathRenderer::onDrawPath");
  ...
  bool inverseFilled =
      ...
      !GrIsStrokeHairlineOrEquivalent(args.fShape->style(), *args.fViewMatrix, nullptr);
  ...
  if (!GetShapeAndClipBounds(
          args.fSurfaceDrawContext, args.fClip, *args.fShape, *args.fViewMatrix,
          &unclippedDevShapeBounds, &clippedDevShapeBounds, &devClipBounds)) {
    if (inverseFilled) {
      DrawAroundInvPath(
          args.fSurfaceDrawContext, std::move(args.fPaint), *args.fUserStencilSettings, args.fClip,
  ...
  if (useCache) {
    ...
    int64_t clippedArea = sk_64_mul(clippedDevShapeBounds.width(), clippedDevShapeBounds.height());
  ...
  if (useCache) {
    ...
    skgpu::UniqueKey::Builder builder(
        &maskKey, kDomain, 7 + args.fShape->unstyledKeySize(), "SW Path Mask");
    ...
    uint32_t styleBits = args.fShape->style().isSimpleHairline()
                             ? ((args.fShape->style().strokeRec().getCap() << 1) | 1)
                             : 0;
  ...
  if (!view) {
    ...
    if (taskGroup) {
      ...
      auto drawAndUploadMask = [uploaderRaw] {
        ...
        if (helper.init(uploaderRaw->data().getMaskBounds())) {
          helper.drawShape(
              uploaderRaw->data().getShape(), *uploaderRaw->data().getViewMatrix(),
    ...
    if (useCache) {
      ...
      auto listener =
          GrMakeUniqueKeyInvalidationListener(&maskKey, args.fContext->priv().contextID());
  ...
  if (inverseFilled) {
    DrawAroundInvPath(
        args.fSurfaceDrawContext, GrPaint::Clone(args.fPaint), *args.fUserStencilSettings,
        args.fClip, *args.fViewMatrix, devClipBounds, unclippedDevShapeBounds);
  }
  DrawToTargetWithShapeMask(
      std::move(view), args.fSurfaceDrawContext, std::move(args.fPaint), *args.fUserStencilSettings,
      args.fClip, *args.fViewMatrix, SkIPoint{boundsForMask->fLeft, boundsForMask->fTop},
      *boundsForMask);
