...
class DrawAtlasPathOp final : public GrDrawOp {
 public:
  ...
  DrawAtlasPathOp(
      SkArenaAlloc* arena, const SkIRect& fillBounds, const SkMatrix& localToDevice,
      ...
      : GrDrawOp(ClassID()),
        fHeadInstance(arena->make<Instance>(
            fillBounds, localToDevice, paint.getColor4f(), locationInAtlas, pathDevIBounds,
            transposedInAtlas)),
        fTailInstance(&fHeadInstance->fNext),
        fAtlasHelper(
            std::move(atlasView), isInverseFill
                                      ? AtlasInstancedHelper::ShaderFlags::kCheckBounds |
  ...
  const char* name() const noexcept override { return "DrawAtlasPathOp"; }
 ...
 private:
  void prepareProgram(
      ...
      GrAppliedClip&&, const GrDstProxyView&, GrXferBarrierFlags, GrLoadOp colorLoadOp);

  struct Instance {
    Instance(
        const SkIRect& fillIBounds, const SkMatrix& m, const SkPMColor4f& color,
        SkIPoint16 locationInAtlas, const SkIRect& pathDevIBounds, bool transposedInAtlas)
        : fFillBounds(fillIBounds),
          fLocalToDeviceIfUsingLocalCoords{m.getScaleX(), m.getSkewY(),      m.getSkewX(),
