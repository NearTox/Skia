...
std::tuple<bool /* filter */, bool /* mipmap */> filter_and_mm_have_effect(
...
NormalizationParams proxy_normalization_params(
    const GrSurfaceProxy* proxy, GrSurfaceOrigin origin) {
...
void normalize_src_quad(const NormalizationParams& params, GrQuad* srcQuad) {
...
bool safe_to_ignore_subset_rect(
...
class TextureOpImpl final : public GrMeshDrawOp {
 public:
  ...
  static GrOp::Owner Make(
      GrRecordingContext* context, GrSurfaceProxyView proxyView,
      sk_sp<GrColorSpaceXform> textureXform, GrSamplerState::Filter filter,
      GrSamplerState::MipmapMode mm, const SkPMColor4f& color, Saturate saturate, GrAAType aaType,
      DrawQuad* quad, const SkRect* subset) {
    return GrOp::Make<TextureOpImpl>(
        context, std::move(proxyView), std::move(textureXform), filter, mm, color, saturate, aaType,
        quad, subset);
  }

  static GrOp::Owner Make(
      GrRecordingContext* context, GrTextureSetEntry set[], int cnt, int proxyRunCnt,
    ...
    return GrOp::MakeWithExtraMemory<TextureOpImpl>(
        context, sizeof(ViewCountPair) * (proxyRunCnt - 1), set, cnt, proxyRunCnt, filter, mm,
        saturate, aaType, constraint, viewMatrix, std::move(textureColorSpaceXform));
  }
  ...
  static void ValidateResourceLimits() {
    ...
    SkASSERT(
        GrResourceProvider::MaxNumAAQuads() <= resourceLimit &&
  ...
  GrProcessorSet::Analysis finalize(
    ...
    while (iter.next()) {
      ...
      colorType = std::max(static_cast<ColorType>(fMetadata.fColorType), colorType);
 ...
 private:
  ...
  struct ColorSubsetAndAA {
    ColorSubsetAndAA(const SkPMColor4f& color, const SkRect& subsetRect, GrQuadAAFlags aaFlags)
        : fColor(color), fSubsetRect(subsetRect), fAAFlags(static_cast<uint16_t>(aaFlags)) {
  ...
  struct Metadata {
    ...
    Metadata(
        const skgpu::Swizzle& swizzle, GrSamplerState::Filter filter, GrSamplerState::MipmapMode mm,
        Subset subset, Saturate saturate)
        : fSwizzle(swizzle),
          fProxyCount(1),
          fTotalQuadCount(1),
  ...
  struct Desc {
    ...
    size_t totalSizeInBytes() const { return this->totalNumVertices() * fVertexSpec.vertexSize(); }

    int totalNumVertices() const { return fNumTotalQuads * fVertexSpec.verticesPerQuad(); }
  ...
  TextureOpImpl(
      ...
      GrSamplerState::Filter filter, GrSamplerState::MipmapMode mm, const SkPMColor4f& color,
      Saturate saturate, GrAAType aaType, DrawQuad* quad, const SkRect* subsetRect)
      : INHERITED(ClassID()),
    ...
    GrQuadUtils::ResolveAAType(aaType, quad->fEdgeFlags, quad->fDevice, &aaType, &quad->fEdgeFlags);
    ...
    this->setBounds(
        quad->fDevice.bounds(), HasAABloat(aaType == GrAAType::kCoverage),
  ...
  TextureOpImpl(
      GrTextureSetEntry set[], int cnt, int proxyRunCnt, const GrSamplerState::Filter filter,
      const GrSamplerState::MipmapMode mm, const Saturate saturate, const GrAAType aaType,
      const SkCanvas::SrcRectConstraint constraint, const SkMatrix& viewMatrix,
      sk_sp<GrColorSpaceXform> textureColorSpaceXform)
      : INHERITED(ClassID()),
        ...
        fDesc(nullptr),
        fMetadata(
            set[0].fProxyView.swizzle(), GrSamplerState::Filter::kNearest,
            GrSamplerState::MipmapMode::kNone, Subset::kNo, saturate) {
    ...
    for (int q = 0; q < cnt; ++q) {
      SkASSERT(
          ...
          (set[0].fProxyView.proxy()->asTextureProxy()->mipmapped() == GrMipmapped::kYes));
      if (q == 0) {
      ...
      if (netFilter != filter || netMM != mm) {
        ...
        SkASSERT(mm == netMM || (netMM == GrSamplerState::MipmapMode::kNone && mm > netMM));
    ...
    this->setBounds(
        bounds, HasAABloat(netAAType == GrAAType::kCoverage),
  ...
  void onCreateProgramInfo(
    ...
    {
      const GrBackendFormat& backendFormat = fViewCountPairs[0].fProxy->backendFormat();

      GrSamplerState samplerState =
          GrSamplerState(GrSamplerState::WrapMode::kClamp, fMetadata.filter());
  ...
  static void FillInVertices(
      const GrCaps& caps, TextureOpImpl* texOp, Desc* desc, char* vertexData) {
    SkASSERT(vertexData);
    ...
    for (const auto& op : ChainRange<TextureOpImpl>(texOp)) {
      ...
      for (unsigned p = 0; p < op.fMetadata.fProxyCount; ++p) {
        ...
        SkASSERT(
            (totVerticesSeen + meshVertexCnt) * vertexSize ==
...
#ifdef SK_DEBUG
  static int validate_op(
      GrTextureType textureType, GrAAType aaType, skgpu::Swizzle swizzle, const TextureOpImpl* op) {
  ...
  void validate() const override {
    ...
    for (const GrOp* tmp = this->prevInChain(); tmp; tmp = tmp->prevInChain()) {
      quadCount +=
          validate_op(textureType, aaType, swizzle, static_cast<const TextureOpImpl*>(tmp));
    }
  ...
  void characterize(Desc* desc) const {
    ...
    auto indexBufferOption =
        skgpu::v1::QuadPerEdgeAA::CalcIndexBufferOption(overallAAType, maxQuadsPerMesh);

    desc->fVertexSpec = VertexSpec(
        quadType, colorType, srcQuadType, /* hasLocal */ true, subset, overallAAType,
        /* alpha as coverage */ true, indexBufferOption);
  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    void* vdata = target->makeVertexSpace(
        vertexSize, fDesc->totalNumVertices(), &fDesc->fVertexBuffer, &fDesc->fBaseVertex);
    if (!vdata) {
    ...
    if (fDesc->fVertexSpec.needsIndexBuffer()) {
      fDesc->fIndexBuffer =
          skgpu::v1::QuadPerEdgeAA::GetIndexBuffer(target, fDesc->fVertexSpec.indexBufferOption());
    ...
    for (const auto& op : ChainRange<TextureOpImpl>(this)) {
      for (unsigned p = 0; p < op.fMetadata.fProxyCount; ++p) {
        ...
        flushState->bindTextures(
            fDesc->fProgramInfo->geomProc(), *op.fViewCountPairs[p].fProxy,
            fDesc->fProgramInfo->pipeline());
        skgpu::v1::QuadPerEdgeAA::IssueDraw(
  ...
  void propagateCoverageAAThroughoutChain() {
    ...
    for (GrOp* tmp = this->prevInChain(); tmp; tmp = tmp->prevInChain()) {
      ...
      SkASSERT(
          tex->fMetadata.aaType() == GrAAType::kCoverage ||
  ...
  CombineResult onCombineIfPossible(GrOp* t, SkArenaAlloc*, const GrCaps& caps) override {
    ...
    if (!GrColorSpaceXform::Equals(
            fTextureColorSpaceXform.get(), that->fTextureColorSpaceXform.get())) {
      return CombineResult::kCannotCombine;
    }
    ...
    if (CombinedQuadCountWillOverflow(
            fMetadata.aaType(), upgradeToCoverageAAOnMerge,
    ...
    if (fMetadata.fProxyCount > 1 || that->fMetadata.fProxyCount > 1 || thisProxy != thatProxy) {
...
#if GR_TEST_UTILS
uint32_t TextureOp::ClassID() { return TextureOpImpl::ClassID(); }
#endif

GrOp::Owner TextureOp::Make(
    GrRecordingContext* context, GrSurfaceProxyView proxyView, SkAlphaType alphaType,
    sk_sp<GrColorSpaceXform> textureXform, GrSamplerState::Filter filter,
    GrSamplerState::MipmapMode mm, const SkPMColor4f& color, Saturate saturate,
    SkBlendMode blendMode, GrAAType aaType, DrawQuad* quad, const SkRect* subset) {
...
class TextureOp::BatchSizeLimiter {
 public:
  BatchSizeLimiter(
      SurfaceDrawContext* sdc, const GrClip* clip, GrRecordingContext* rContext, int numEntries,
      GrSamplerState::Filter filter, GrSamplerState::MipmapMode mm, Saturate saturate,
  ...
  void createOp(GrTextureSetEntry set[], int clumpSize, GrAAType aaType) {
    ...
    GrOp::Owner op = TextureOpImpl::Make(
        fContext, &set[fNumClumped], clumpSize, clumpProxyCount, fFilter, fMipmapMode, fSaturate,
        aaType, fConstraint, fViewMatrix, fTextureColorSpaceXform);
...
void TextureOp::AddTextureSetOps(
    SurfaceDrawContext* sdc, const GrClip* clip, GrRecordingContext* context,
    GrTextureSetEntry set[], int cnt, int proxyRunCnt, GrSamplerState::Filter filter,
    GrSamplerState::MipmapMode mm, Saturate saturate, SkBlendMode blendMode, GrAAType aaType,
    SkCanvas::SrcRectConstraint constraint, const SkMatrix& viewMatrix,
    sk_sp<GrColorSpaceXform> textureColorSpaceXform) {
  ...
  if (blendMode != SkBlendMode::kSrcOver ||
    ...
    for (int i = 0; i < cnt; ++i) {
      ...
      auto op = Make(
          context, set[i].fProxyView, set[i].fSrcAlphaType, textureColorSpaceXform, filter, mm,
          set[i].fColor, saturate, blendMode, aaType, &quad, subset);
  ...
  if (cnt <=
      std::min(GrResourceProvider::MaxNumNonAAQuads(), GrResourceProvider::MaxNumAAQuads())) {
    auto op = TextureOpImpl::Make(
        context, set, cnt, proxyRunCnt, filter, mm, saturate, aaType, constraint, viewMatrix,
        std::move(textureColorSpaceXform));
    sdc->addDrawOp(clip, std::move(op));
    return;
  }

  BatchSizeLimiter state(
      sdc, clip, context, cnt, filter, mm, saturate, constraint, viewMatrix,
      std::move(textureColorSpaceXform));
  ...
  if (aaType == GrAAType::kNone || aaType == GrAAType::kMSAA) {
    ...
    while (state.numLeft() > 0) {
      ...
      for (int i = 0; i < state.numLeft(); ++i) {
        ...
        if (set[absIndex].fAAFlags != GrQuadAAFlags::kNone || runningAA == GrAAType::kCoverage) {
          if (i >= GrResourceProvider::MaxNumAAQuads()) {
            ...
            state.createOp(
                set, runningAA == GrAAType::kNone ? i : GrResourceProvider::MaxNumAAQuads(),
          ...
          if (i >= GrResourceProvider::MaxNumNonAAQuads()) {
            ...
            state.createOp(
                set, GrResourceProvider::MaxNumNonAAQuads(),
...
GR_DRAW_OP_TEST_DEFINE(TextureOpImpl) {
  ...
  sk_sp<GrTextureProxy> proxy = proxyProvider->createProxy(
      ...
      /*label=*/"TextureOp", GrInternalSurfaceFlags::kNone);
  ...
  auto alphaType =
      static_cast<SkAlphaType>(random->nextRangeU(kUnknown_SkAlphaType + 1, kLastEnum_SkAlphaType));

  DrawQuad quad = {GrQuad::MakeFromRect(rect, viewMatrix), GrQuad(srcRect), aaFlags};
  return skgpu::v1::TextureOp::Make(
      context, std::move(proxyView), alphaType, std::move(texXform), filter, mm, color, saturate,
      SkBlendMode::kSrcOver, aaType, &quad, useSubset ? &srcRect : nullptr);
}
