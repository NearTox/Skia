...
class TriangulatingPathOp final : public GrMeshDrawOp {
 ...
 private:
  ...
  static void CreateKey(
      skgpu::UniqueKey* key, const GrStyledShape& shape, const SkIRect& devClipBounds) {
  ...
  void onCreateProgramInfo(
    ...
    fProgramInfo = fHelper.createProgramInfoWithStencil(
        ...
        primitiveType, renderPassXferBarriers, colorLoadOp);
  }
  ...
  static GrSimpleMesh* CreateMesh(
      GrMeshDrawTarget* target, sk_sp<const GrBuffer> vb, int firstVertex, int count) {
...
bool TriangulatingPathRenderer::onDrawPath(const DrawPathArgs& args) {
  GR_AUDIT_TRAIL_AUTO_FRAME(
      args.fContext->priv().auditTrail(), "GrTriangulatingPathRenderer::onDrawPath");
