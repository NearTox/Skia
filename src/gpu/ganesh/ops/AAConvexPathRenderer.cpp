...
bool compute_vectors(
    SegmentArray* segments, SkPoint* fanPt, SkPathFirstDirection dir, int* vCount, int* iCount) {
...
inline void add_cubic_segments(
    const SkPoint pts[4], SkPathFirstDirection dir, SegmentArray* segments) {
...
bool get_segments(
    const SkPath& path, const SkMatrix& m, SegmentArray* segments, SkPoint* fanPt, int* vCount,
    int* iCount) {
...
void create_vertices(
    const SegmentArray& segments, const SkPoint& fanPt, const VertexColor& color, DrawArray* draws,
    VertexWriter& verts, uint16_t* idxs, size_t vertexStride) {
    ...
    if (Segment::kLine == segb.fType) {
      ...
      *v += 5;
      ...
      verts << posAndUVPoints[0].fPos << color << posAndUVPoints[0].fUV
            << (-segb.fNorms[0].dot(fanPt) + c0) << (-segb.fNorms[1].dot(fanPt) + c1);

      verts << posAndUVPoints[1].fPos << color << posAndUVPoints[1].fUV << 0.0f
            << (-segb.fNorms[1].dot(qpts[0]) + c1);

      verts << posAndUVPoints[2].fPos << color << posAndUVPoints[2].fUV
            << (-segb.fNorms[0].dot(qpts[2]) + c0) << 0.0f;
      ...
      verts << posAndUVPoints[3].fPos << color << posAndUVPoints[3].fUV << kStableLargeNegativeValue
            << kStableLargeNegativeValue;

      verts << posAndUVPoints[4].fPos << color << posAndUVPoints[4].fUV << kStableLargeNegativeValue
            << kStableLargeNegativeValue;

      verts << posAndUVPoints[5].fPos << color << posAndUVPoints[5].fUV << kStableLargeNegativeValue
            << kStableLargeNegativeValue;
   ...
   private:
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
      ...
      if (qe.fUsesLocalCoords) {
        WriteLocalCoord(
            vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, qe.fInPosition.asShaderVar(),
            qe.fLocalMatrix, &fLocalMatrixUniform);
      }
 ...
 public:
  ...
  GrProcessorSet::Analysis finalize(
    ...
    return fHelper.finalizeProcessors(
        caps, clip, clampType, GrProcessorAnalysisCoverage::kSingleChannel, &fPaths.back().fColor,
        &fWideColor);
  }
 ...
 private:
  ...
  void onCreateProgramInfo(
    ...
    fProgramInfo = fHelper.createProgramInfoWithStencil(
        ...
        quadProcessor, GrPrimitiveType::kTriangles, renderPassXferBarriers, colorLoadOp);
  }
  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    for (int i = 0; i < instanceCount; i++) {
      ...
      if (!get_segments(*pathPtr, *viewMatrix, &segments, &fanPt, &vertexCount, &indexCount)) {
        continue;
      }
      ...
      VertexWriter verts =
          target->makeVertexWriter(kVertexStride, vertexCount, &vertexBuffer, &firstVertex);
...
PathRenderer::CanDrawPath AAConvexPathRenderer::onCanDrawPath(const CanDrawPathArgs& args) const {
  ...
  if (args.fCaps->shaderCaps()->fShaderDerivativeSupport && (GrAAType::kCoverage == args.fAAType) &&
      args.fShape->style().isSimpleFill() && !args.fShape->inverseFilled() &&
      args.fShape->knownToBeConvex() && args.fShape->knownDirection()) {
...
bool AAConvexPathRenderer::onDrawPath(const DrawPathArgs& args) {
  GR_AUDIT_TRAIL_AUTO_FRAME(args.fContext->priv().auditTrail(), "AAConvexPathRenderer::onDrawPath");
...
GR_DRAW_OP_TEST_DEFINE(AAConvexPathOp) {
  ...
  return skgpu::v1::AAConvexPathOp::Make(
      context, std::move(paint), viewMatrix, path, stencilSettings);
}
