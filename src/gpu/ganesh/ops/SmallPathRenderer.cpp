...
class SmallPathOp final : public GrMeshDrawOp {
 ...
 private:
  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    if (fUsesDistanceField) {
      ...
      flushInfo.fGeometryProcessor = GrBitmapTextGeoProc::Make(
          ...
          numActiveProxies, GrSamplerState::Filter::kNearest, MaskFormat::kA8, invert, false);
    }
    ...
    for (int i = 0; i < instanceCount; i++) {
      ...
      this->writePathVertices(
          vertices, VertexColor(args.fColor, fWideColor), args.fViewMatrix, shapeData);
  ...
  bool addToAtlasWithRetry(
      GrMeshDrawTarget* target, FlushInfo* flushInfo, skgpu::v1::SmallPathAtlasMgr* atlasMgr,
      int width, int height, const void* image, const SkRect& bounds, int srcInset,
  ...
  bool addDFPathToAtlas(
      GrMeshDrawTarget* target, FlushInfo* flushInfo, skgpu::v1::SmallPathAtlasMgr* atlasMgr,
      skgpu::v1::SmallPathShapeData* shapeData, const GrStyledShape& shape, uint32_t dimension,
  ...
  bool addBMPathToAtlas(
      GrMeshDrawTarget* target, FlushInfo* flushInfo, skgpu::v1::SmallPathAtlasMgr* atlasMgr,
      skgpu::v1::SmallPathShapeData* shapeData, const GrStyledShape& shape,
  ...
  void writePathVertices(
      VertexWriter& vertices, const VertexColor& color, const SkMatrix& ctm,
    ...
    if (fUsesDistanceField && !ctm.hasPerspective()) {
      ...
      vertices.writeQuad(VertexWriter::TriStripFromRect(translatedBounds), color, texCoords);
    }
  }
...
GR_DRAW_OP_TEST_DEFINE(SmallPathOp) {
  ...
  return skgpu::v1::SmallPathOp::Make(
