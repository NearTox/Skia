...
class AtlasPathRenderer final : public PathRenderer, public GrOnFlushCallbackObject {
 public:
  ...
  const char* name() const noexcept override { return "GrAtlasPathRenderer"; }
  ...
  GrFPResult makeAtlasClipEffect(
      const skgpu::v1::SurfaceDrawContext*, const GrOp* opBeingClipped,
      std::unique_ptr<GrFragmentProcessor> inputFP, const SkIRect& drawBounds, const SkMatrix&,
      const SkPath&);
 ...
 private:
  ...
  bool addPathToAtlas(
      GrRecordingContext*, const SkMatrix&, const SkPath&, const SkRect& pathDevBounds,
  ...
  struct AtlasPathKey {
    ...
    bool operator==(const AtlasPathKey& k) const noexcept {
