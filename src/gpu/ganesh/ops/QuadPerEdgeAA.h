...
class Tessellator {
 ...
 private:
  ...
  typedef void (*WriteQuadProc)(
      VertexWriter* vertices, const VertexSpec& spec, const GrQuad* deviceQuad,
...
GrGeometryProcessor* MakeTexturedProcessor(
    ...
    const skgpu::Swizzle&, sk_sp<GrColorSpaceXform> textureColorSpaceXform, Saturate);
...
void IssueDraw(
    const GrCaps&, GrOpsRenderPass*, const VertexSpec&, int runningQuadCount, int quadCount,
    int maxVerts, int absVertBufferOffset);
