...
static const uint16_t kQuadIdxBufPattern[] = {0, 1, 2, 2, 4, 3, 1, 4, 2};
...
static const uint16_t kLineSegIdxBufPattern[] = {0, 1, 3, 0, 3, 2, 0, 4, 5,
                                                 0, 5, 1, 0, 2, 4, 1, 5, 3};
...
int gather_lines_and_quads(
    const SkPath& path, const SkMatrix& m, const SkIRect& devClipBounds, SkScalar capLength,
    bool convertConicsToQuads, PtArray* lines, PtArray* quads, PtArray* conics,
    IntArray* quadSubdivCnts, FloatArray* conicWeights) {
          ...
          for (int i = 0; i < conicCnt; ++i) {
            ...
            if (SkIRect::Intersects(devClipBounds, ibounds)) {
              if (is_degen_quad_or_conic(devPts)) {
                ...
                pts[2] = devPts[1];
                pts[3] = devPts[2];
                if (verbsInContour == 0 && i == 0 && pts[0] == pts[1] && pts[2] == pts[3]) {
                  seenZeroLengthVerb = true;
                  zeroVerbPt = pts[0];
                }
              } else {
...
bool bloat_quad(
    const SkPoint qpts[3], const SkMatrix* toDevice, const SkMatrix* toSrc,
  ...
  if (toSrc) {
    SkMatrixPriv::MapPointsWithStride(
        *toSrc, &verts[0].fPos, sizeof(BezierVertex), kQuadNumVertices);
  }
...
void set_conic_coeffs(
    const SkPoint p[3], BezierVertex verts[kQuadNumVertices], const SkScalar weight) {
  SkMatrix klm;
...
void add_conics(
    const SkPoint p[3], const SkScalar weight, const SkMatrix* toDevice, const SkMatrix* toSrc,
    BezierVertex** vert) {
...
void add_quads(
    const SkPoint p[3], int subdiv, const SkMatrix* toDevice, const SkMatrix* toSrc,
    BezierVertex** vert) {
  SkASSERT(subdiv >= 0);
...
void add_line(const SkPoint p[2], const SkMatrix* toSrc, uint8_t coverage, LineVertex** vert) {
  const SkPoint& a = p[0];
  const SkPoint& b = p[1];
  ...
  if (vec.setLength(SK_ScalarHalf)) {
    ...
    if (toSrc) {
      SkMatrixPriv::MapPointsWithStride(
          *toSrc, &(*vert)->fPos, sizeof(LineVertex), kLineSegNumVertices);
    }
  } else {
...
class AAHairlineOp final : public GrMeshDrawOp {
 ...
 public:
  ...
  static GrOp::Owner Make(
    ...
    return Helper::FactoryHelper<AAHairlineOp>(
        context, std::move(paint), newCoverage, viewMatrix, path, devClipBounds, capLength,
        stencilSettings);
  }
  ...
  GrProcessorSet::Analysis finalize(
    ...
    return fHelper.finalizeProcessors(
        caps, clip, clampType, GrProcessorAnalysisCoverage::kSingleChannel, &fColor, nullptr);
  }
 ...
 private:
  void makeLineProgramInfo(
      const GrCaps&, SkArenaAlloc*, const GrPipeline*, const GrSurfaceProxyView& writeView,
      bool usesMSAASurface, const SkMatrix* geometryProcessorViewM,
      const SkMatrix* geometryProcessorLocalM, GrXferBarrierFlags renderPassXferBarriers,
      GrLoadOp colorLoadOp);
  void makeQuadProgramInfo(
      const GrCaps&, SkArenaAlloc*, const GrPipeline*, const GrSurfaceProxyView& writeView,
      bool usesMSAASurface, const SkMatrix* geometryProcessorViewM,
      const SkMatrix* geometryProcessorLocalM, GrXferBarrierFlags renderPassXferBarriers,
      GrLoadOp colorLoadOp);
  void makeConicProgramInfo(
      const GrCaps&, SkArenaAlloc*, const GrPipeline*, const GrSurfaceProxyView& writeView,
      bool usesMSAASurface, const SkMatrix* geometryProcessorViewM,
      const SkMatrix* geometryProcessorLocalM, GrXferBarrierFlags renderPassXferBarriers,
      GrLoadOp colorLoadOp);
  ...
  SkString onDumpInfo() const override {
    return SkStringPrintf(
        "Color: 0x%08x Coverage: 0x%02x, Count: %d\n%s", fColor.toBytes_RGBA(), fCoverage,
        fPaths.count(), fHelper.dumpInfo().c_str());
  }
#endif
...
void AAHairlineOp::makeLineProgramInfo(
    const GrCaps& caps, SkArenaAlloc* arena, const GrPipeline* pipeline,
    const GrSurfaceProxyView& writeView, bool usesMSAASurface,
    const SkMatrix* geometryProcessorViewM, const SkMatrix* geometryProcessorLocalM,
  ...
  GrGeometryProcessor* lineGP;
  {
    ...
    lineGP = GrDefaultGeoProcFactory::Make(
        arena, color, Coverage::kAttribute_Type, localCoords, *geometryProcessorViewM);
...
void AAHairlineOp::makeQuadProgramInfo(
    const GrCaps& caps, SkArenaAlloc* arena, const GrPipeline* pipeline,
    const GrSurfaceProxyView& writeView, bool usesMSAASurface,
    const SkMatrix* geometryProcessorViewM, const SkMatrix* geometryProcessorLocalM,
    GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) {
...
void AAHairlineOp::makeConicProgramInfo(
    const GrCaps& caps, SkArenaAlloc* arena, const GrPipeline* pipeline,
    const GrSurfaceProxyView& writeView, bool usesMSAASurface,
    const SkMatrix* geometryProcessorViewM, const SkMatrix* geometryProcessorLocalM,
    GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) {
  ...
  fProgramInfos[2] = GrSimpleMeshDrawOpHelper::CreateProgramInfo(
      &caps, arena, pipeline, writeView, usesMSAASurface, conicGP, GrPrimitiveType::kTriangles,
      renderPassXferBarriers, colorLoadOp, fHelper.stencilSettings());
}
...
void AAHairlineOp::onCreateProgramInfo(
  ...
  auto pipeline = fHelper.createPipeline(
      caps, arena, writeView.swizzle(), std::move(appliedClip), dstProxyView);

  if (fCharacterization & Program::kLine) {
    this->makeLineProgramInfo(
        *caps, arena, pipeline, writeView, usesMSAASurface, geometryProcessorViewM,
        geometryProcessorLocalM, renderPassXferBarriers, colorLoadOp);
  }
  if (fCharacterization & Program::kQuad) {
    this->makeQuadProgramInfo(
        *caps, arena, pipeline, writeView, usesMSAASurface, geometryProcessorViewM,
        geometryProcessorLocalM, renderPassXferBarriers, colorLoadOp);
  }
  if (fCharacterization & Program::kConic) {
    this->makeConicProgramInfo(
        *caps, arena, pipeline, writeView, usesMSAASurface, geometryProcessorViewM,
        geometryProcessorLocalM, renderPassXferBarriers, colorLoadOp);
  }
}

void AAHairlineOp::onPrePrepareDraws(
...
void AAHairlineOp::onPrepareDraws(GrMeshDrawTarget* target) {
  ...
  if (lineCount) {
    ...
    GrMeshDrawOp::PatternHelper helper(
        target, GrPrimitiveType::kTriangles, sizeof(LineVertex), std::move(linesIndexBuffer),
        kLineSegNumVertices, kIdxsPerLineSeg, lineCount, kLineSegsNumInIdxBuffer);
  ...
  if (quadCount || conicCount) {
    ...
    void* vertices =
        target->makeVertexSpace(sizeof(BezierVertex), vertexCount, &vertexBuffer, &firstVertex);
    ...
    for (int i = 0; i < unsubdivQuadCnt; ++i) {
      ...
      if (!quads[3 * i].isFinite() || !quads[3 * i + 1].isFinite() ||
          !quads[3 * i + 2].isFinite()) {
        return;
      }
    ...
    if (quadCount > 0) {
      ...
      fMeshes[1]->setIndexedPatterned(
          quadsIndexBuffer, kIdxsPerQuad, quadCount, kQuadsNumInIdxBuffer, vertexBuffer,
    ...
    if (conicCount > 0) {
      ...
      fMeshes[2]->setIndexedPatterned(
          std::move(quadsIndexBuffer), kIdxsPerQuad, conicCount, kQuadsNumInIdxBuffer,
          std::move(vertexBuffer), kQuadNumVertices, firstVertex);
    }
  }
...
void AAHairlineOp::onExecute(GrOpFlushState* flushState, const SkRect& chainBounds) {
  ...
  for (int i = 0; i < 3; ++i) {
    if (fProgramInfos[i] && fMeshes[i]) {
      ...
      flushState->bindTextures(fProgramInfos[i]->geomProc(), nullptr, fProgramInfos[i]->pipeline());
...
GR_DRAW_OP_TEST_DEFINE(AAHairlineOp) {
  ...
  return AAHairlineOp::Make(
      context, std::move(paint), viewMatrix, path, GrStyle::SimpleHairline(), devClipBounds,
...
bool AAHairLinePathRenderer::onDrawPath(const DrawPathArgs& args) {
  GR_AUDIT_TRAIL_AUTO_FRAME(
      args.fContext->priv().auditTrail(), "AAHairlinePathRenderer::onDrawPath");
