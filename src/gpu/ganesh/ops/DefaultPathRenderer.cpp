...
class PathGeoBuilder {
 public:
  PathGeoBuilder(
      GrPrimitiveType primitiveType, GrMeshDrawTarget* target, SkTDArray<GrSimpleMesh*>* meshes)
  ...
  void addQuad(const SkPoint pts[], SkScalar srcSpaceTolSqd, SkScalar srcSpaceTol) {
    if (!this->ensureSpace(
            GrPathUtils::kMaxPointsPerCurve, GrPathUtils::kMaxPointsPerCurve * this->indexScale(),
            &pts[0])) {
      return;
    }
  ...
  void addConic(
      SkScalar weight, const SkPoint pts[], SkScalar srcSpaceTolSqd, SkScalar srcSpaceTol) {
    ...
    while (!done) {
      ...
      switch (verb) {
        ...
        case SkPath::kConic_Verb:
          ...
          break;
        case SkPath::kQuad_Verb: this->addQuad(pts, srcSpaceTolSqd, srcSpaceTol); break;
        case SkPath::kCubic_Verb: this->addCubic(pts, srcSpaceTolSqd, srcSpaceTol); break;
        case SkPath::kClose_Verb: break;
        case SkPath::kDone_Verb: done = true;
      }
    }
  }
 ...
 private:
  ...
  void allocNewBuffers() {
    ...
    fVertices = static_cast<SkPoint*>(fTarget->makeVertexSpaceAtLeast(
    ...
    if (this->isIndexed()) {
      ...
      fIndices = fTarget->makeIndexSpaceAtLeast(
...
class DefaultPathOp final : public GrMeshDrawOp {
 ...
 public:
  ...
  static GrOp::Owner Make(
      GrRecordingContext* context, GrPaint&& paint, const SkPath& path, SkScalar tolerance,
      uint8_t coverage, const SkMatrix& viewMatrix, bool isHairline, GrAAType aaType,
      const SkRect& devBounds, const GrUserStencilSettings* stencilSettings) {
    return Helper::FactoryHelper<DefaultPathOp>(
        context, std::move(paint), path, tolerance, coverage, viewMatrix, isHairline, aaType,
  ...
  DefaultPathOp(
      GrProcessorSet* processorSet, const SkPMColor4f& color, const SkPath& path,
  ...
  GrProcessorSet::Analysis finalize(
    ...
    GrProcessorAnalysisCoverage gpCoverage = this->coverage() == 0xFF
 ...
 private:
  ...
  void onCreateProgramInfo(
    ...
    fProgramInfo = fHelper.createProgramInfoWithStencil(
        ...
        this->primType(), renderPassXferBarriers, colorLoadOp);
  }
  ...
  SkString onDumpInfo() const override {
    SkString string =
        SkStringPrintf("Color: 0x%08x Count: %d\n", fColor.toBytes_RGBA(), fPaths.count());
...
GR_DRAW_OP_TEST_DEFINE(DefaultPathOp) {
  ...
  return DefaultPathOp::Make(
      context, std::move(paint), path, srcSpaceTol, coverage, viewMatrix, true, aaType, bounds,
...
bool DefaultPathRenderer::internalDrawPath(
    skgpu::v1::SurfaceDrawContext* sdc, GrPaint&& paint, GrAAType aaType,
    ...
    if (single_pass_shape(shape)) {
      ...
      switch (path.getFillType()) {
        ...
        case SkPathFillType::kWinding:
          ...
          break;
        default: SkDEBUGFAIL("Unknown path fFill!"); return false;
      }
    }
  }
  ...
  SkRect devBounds;
  GetPathDevBounds(
      path, sdc->asRenderTargetProxy()->backingStoreDimensions(), viewMatrix, &devBounds);
  ...
  for (int p = 0; p < passCount; ++p) {
    if (lastPassIsBounds && (p == passCount - 1)) {
      ...
      assert_alive(paint);
      sdc->stencilRect(
          clip, passes[p], std::move(paint), GrAA(aaType == GrAAType::kMSAA), viewM, bounds,
          &localMatrix);
    } else {
      ...
      if (stencilPass) {
        ...
        op = DefaultPathOp::Make(
            context, std::move(stencilPaint), path, srcSpaceTol, newCoverage, viewMatrix,
            isHairline, aaType, devBounds, passes[p]);
      } else {
        assert_alive(paint);
        op = DefaultPathOp::Make(
            context, std::move(paint), path, srcSpaceTol, newCoverage, viewMatrix, isHairline,
            aaType, devBounds, passes[p]);
      }
...
PathRenderer::StencilSupport DefaultPathRenderer::onGetStencilSupport(
...
bool DefaultPathRenderer::onDrawPath(const DrawPathArgs& args) {
  GR_AUDIT_TRAIL_AUTO_FRAME(args.fContext->priv().auditTrail(), "DefaultPathRenderer::onDrawPath");
...
void DefaultPathRenderer::onStencilPath(const StencilPathArgs& args) {
  GR_AUDIT_TRAIL_AUTO_FRAME(
      args.fContext->priv().auditTrail(), "DefaultPathRenderer::onStencilPath");
