...
GrGeometryProcessor* make_gp(SkArenaAlloc* arena, const SkMatrix& viewMatrix, bool wideColor) {
...
class RegionOpImpl final : public GrMeshDrawOp {
 ...
 public:
  ...
  static GrOp::Owner Make(
    ...
    return Helper::FactoryHelper<RegionOpImpl>(
        context, std::move(paint), viewMatrix, region, aaType, stencilSettings);
  }
  ...
  GrProcessorSet::Analysis finalize(
    ...
    return fHelper.finalizeProcessors(
        caps, clip, clampType, GrProcessorAnalysisCoverage::kNone, &fRegions[0].fColor,
        &fWideColor);
  }
...
GrOp::Owner Make(
  ...
  return RegionOpImpl::Make(context, std::move(paint), viewMatrix, region, aaType, stencilSettings);
}
...
GR_DRAW_OP_TEST_DEFINE(RegionOp) {
  ...
  return skgpu::v1::RegionOp::RegionOpImpl::Make(
      context, std::move(paint), viewMatrix, region, aaType, GrGetRandomStencil(random, context));
}
