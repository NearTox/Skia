...
void extract_verts(
    ...
    const VertexColor& color, uint16_t firstIndex, uint16_t* idxs) {
  ...
  for (int i = 0; i < tess.numPts(); ++i) {
    ...
    verts << tess.point(i) << color << VertexWriter::If(localCoordsMatrix, lc) << tess.coverage(i);
  }
...
GR_DRAW_OP_TEST_DEFINE(AAFlatteningConvexPathOp) {
  ...
  return skgpu::v1::AAFlatteningConvexPathOp::Make(
      context, std::move(paint), viewMatrix, path, strokeWidth, style, join, miterLimit,
      stencilSettings);
}
