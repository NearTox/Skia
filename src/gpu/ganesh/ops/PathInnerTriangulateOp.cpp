 ...
 public:
  HullShader(const SkMatrix& viewMatrix, SkPMColor4f color, const GrShaderCaps& shaderCaps)
      : GrPathTessellationShader(
            kTessellate_HullShader_ClassID, GrPrimitiveType::kTriangleStrip, viewMatrix, color,
            PatchAttribs::kNone) {
    ...
    this->setInstanceAttributesWithImplicitOffsets(
        fInstanceAttribs.data(), fInstanceAttribs.count());
    SkASSERT(fInstanceAttribs.count() <= kMaxInstanceAttribCount);

    if (!shaderCaps.fVertexIDSupport) {
      constexpr static Attribute kVertexIdxAttrib(
          "vertexidx", kFloat_GrVertexAttribType, SkSLType::kFloat);
...
std::unique_ptr<GrGeometryProcessor::ProgramImpl> HullShader::makeProgramImpl(
  ...
  class Impl : public GrPathTessellationShader::Impl {
    void emitVertexCode(
        const GrShaderCaps& shaderCaps, const GrPathTessellationShader&, GrGLSLVertexBuilder* v,
        GrGLSLVaryingHandler*, GrGPArgs* gpArgs) override {
      if (shaderCaps.fInfinitySupport) {
        v->insertFunction(R"(
...
GrProcessorSet::Analysis PathInnerTriangulateOp::finalize(
    const GrCaps& caps, const GrAppliedClip* clip, GrClampType clampType) {
  return fProcessors.finalize(
      fColor, GrProcessorAnalysisCoverage::kNone, clip, nullptr, caps, clampType, &fColor);
}

void PathInnerTriangulateOp::pushFanStencilProgram(
    const GrTessellationShader::ProgramArgs& args, const GrPipeline* pipelineForStencils,
  ...
  fFanPrograms.push_back(
      GrTessellationShader::MakeProgram(args, shader, pipelineForStencils, stencil));
}

void PathInnerTriangulateOp::pushFanFillProgram(
    const GrTessellationShader::ProgramArgs& args, const GrUserStencilSettings* stencil) {
  SkASSERT(fPipelineForFills);
  auto shader =
      GrPathTessellationShader::MakeSimpleTriangleShader(args.fArena, fViewMatrix, fColor);
  fFanPrograms.push_back(
      GrTessellationShader::MakeProgram(args, shader, fPipelineForFills, stencil));
}

void PathInnerTriangulateOp::prePreparePrograms(
    const GrTessellationShader::ProgramArgs& args, GrAppliedClip&& appliedClip) {
  ...
  if (doFill) {
    fPipelineForFills = GrTessellationShader::MakePipeline(
  ...
  if (!isLinear) {
    fTessellator =
        PathCurveTessellator::Make(args.fArena, args.fCaps->shaderCaps()->fInfinitySupport);
    auto* tessShader = GrPathTessellationShader::Make(
        *args.fCaps->shaderCaps(), args.fArena, fViewMatrix, SK_PMColor4fTRANSPARENT,
  ...
  if (fFanPolys) {
    if (forceRedbookStencilPass) {
      ...
      if (doFill) {
        this->pushFanFillProgram(args, GrPathTessellationShader::TestAndResetStencilSettings());
      }
    } else if (isLinear) {
      ...
      constexpr static GrUserStencilSettings kFillOrInvertStencil(
          GrUserStencilSettings::StaticInit<
              0x0000, GrUserStencilTest::kEqual, 0xffff, GrUserStencilOp::kKeep,
              // "Zero" instead of "Invert" because the fan only touches any given pixel once.
              GrUserStencilOp::kZero, 0xffff>());

      auto* stencil = (fPath.getFillType() == SkPathFillType::kWinding) ? &kFillOrIncrDecrStencil
      ...
      constexpr static GrUserStencilSettings kFillIfZeroAndInClip(
          GrUserStencilSettings::StaticInit<
              0x0000, GrUserStencilTest::kEqualIfInClip, 0xffff, GrUserStencilOp::kKeep,
      ...
      constexpr static GrUserStencilSettings kIncrDecrStencilIfNonzero(
          GrUserStencilSettings::StaticInitSeparate<
              ...
              GrUserStencilTest::kNotEqual, GrUserStencilTest::kNotEqual, 0xffff, 0xffff,
              GrUserStencilOp::kIncWrap, GrUserStencilOp::kDecWrap, GrUserStencilOp::kKeep,
      ...
      constexpr static GrUserStencilSettings kInvertStencilIfNonZero(
          GrUserStencilSettings::StaticInit<
              ...
              // No need to check the clip because the previous stencil pass will have only
              // written to samples already inside the clip.
              GrUserStencilTest::kNotEqual, 0xffff,
              // "Zero" instead of "Invert" because the fan only touches any given pixel once.
              GrUserStencilOp::kZero, GrUserStencilOp::kKeep, 0xffff>());
      ...
      auto* stencil = (fPath.getFillType() == SkPathFillType::kWinding) ? &kIncrDecrStencilIfNonzero
  ...
  if (doFill && !isLinear) {
    ...
    SkASSERT(fTessellator);
    auto* hullShader =
        args.fArena->make<HullShader>(fViewMatrix, fColor, *args.fCaps->shaderCaps());
...
void PathInnerTriangulateOp::onPrePrepare(
  ...
  this->prePreparePrograms(
      {context->priv().recordTimeAllocator(), writeView, usesMSAASurface, &dstProxyView,
...
void PathInnerTriangulateOp::onPrepare(GrOpFlushState* flushState) {
  ...
  if (!fFanTriangulator) {
    this->prePreparePrograms(
        {flushState->allocator(), flushState->writeView(), flushState->usesMSAASurface(),
         &flushState->dstProxyView(), flushState->renderPassBarriers(), flushState->colorLoadOp(),
         &caps},
  ...
  if (fTessellator) {
    ...
    fTessellator->prepareWithTriangles(
        flushState, tessShader->viewMatrix(), &fFanBreadcrumbs,
        {SkMatrix::I(), fPath, SK_PMColor4fTRANSPARENT}, fPath.countVerbs());
  }
  ...
  if (!caps.shaderCaps()->fVertexIDSupport) {
    ...
    fHullVertexBufferIfNoIDSupport = flushState->resourceProvider()->findOrMakeStaticBuffer(
        GrGpuBufferType::kVertex, sizeof(kStripOrderIDs), kStripOrderIDs, gHullVertexBufferKey);
  }
}

void PathInnerTriangulateOp::onExecute(GrOpFlushState* flushState, const SkRect& chainBounds) {
  if (fCoverHullsProgram && fCoverHullsProgram->geomProc().hasVertexAttributes() &&
