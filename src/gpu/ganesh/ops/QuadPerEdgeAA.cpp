...
void write_quad_generic(
    VertexWriter* vb, const VertexSpec& spec, const GrQuad* deviceQuad, const GrQuad* localQuad,
    const float coverage[4], const SkPMColor4f& color, const SkRect& geomSubset,
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    *vb << deviceQuad->x(i) << deviceQuad->y(i)
    ...
    if (spec.hasLocalCoords()) {
      *vb << localQuad->x(i) << localQuad->y(i)
...
void write_2d_color(
  ...
  SkASSERT(
      spec.coverageMode() == CoverageMode::kNone ||
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    *vb << deviceQuad->x(i) << deviceQuad->y(i) << VertexColor(color * coverage[i], wide);
  }
}
...
void write_2d_uv(
  ...
  for (int i = 0; i < 4; ++i) {
    *vb << deviceQuad->x(i) << deviceQuad->y(i) << localQuad->x(i) << localQuad->y(i);
  }
}
...
void write_2d_cov_uv(
  ...
  for (int i = 0; i < 4; ++i) {
    *vb << deviceQuad->x(i) << deviceQuad->y(i) << coverage[i] << localQuad->x(i)
        << localQuad->y(i);
  }
}
...
void write_2d_uv_strict(
  ...
  for (int i = 0; i < 4; ++i) {
    *vb << deviceQuad->x(i) << deviceQuad->y(i) << localQuad->x(i) << localQuad->y(i) << texSubset;
  }
}
...
void write_2d_cov_uv_strict(
  ...
  for (int i = 0; i < 4; ++i) {
    *vb << deviceQuad->x(i) << deviceQuad->y(i) << coverage[i] << localQuad->x(i) << localQuad->y(i)
        << texSubset;
  }
}
...
Tessellator::Tessellator(const VertexSpec& spec, char* vertices)
    : fVertexSpec(spec), fVertexWriter{vertices}, fWriteProc(Tessellator::GetWriteQuadProc(spec)) {}

void Tessellator::append(
  ...
  if (fVertexSpec.usesCoverageAA()) {
    SkASSERT(
        fVertexSpec.coverageMode() == CoverageMode::kWithColor ||
    ...
    if (aaFlags == GrQuadAAFlags::kNone) {
      ...
      fWriteProc(
          &fVertexWriter, fVertexSpec, deviceQuad, localQuad, kFullCoverage, color, geomSubset,
          uvSubset);
      ...
      fWriteProc(
          &fVertexWriter, fVertexSpec, deviceQuad, localQuad, kZeroCoverage, color, geomSubset,
          uvSubset);
    } else {
      ...
      if (aaFlags == GrQuadAAFlags::kAll) {
        ...
        edgeDistances = {
            (aaFlags & GrQuadAAFlags::kLeft) ? 0.5f : 0.f,
      ...
      fWriteProc(
          &fVertexWriter, fVertexSpec, deviceQuad, localQuad, coverage, color, geomSubset,
          uvSubset);
      ...
      const bool hairline = aaFlags == GrQuadAAFlags::kAll && coverage[0] < 1.f &&
                            coverage[1] < 1.f && coverage[2] < 1.f && coverage[3] < 1.f;
      if (hairline) {
        ...
        skvx::Vec<4, float> maxWH =
            max(skvx::shuffle<1, 0, 3, 2>(len), skvx::shuffle<2, 3, 0, 1>(len));
    ...
    SkASSERT(
        fVertexSpec.coverageMode() == CoverageMode::kNone && !fVertexSpec.requiresGeometrySubset());
    fWriteProc(
        &fVertexWriter, fVertexSpec, deviceQuad, localQuad, kFullCoverage, color, kIgnoredSubset,
        uvSubset);
  }
}

sk_sp<const GrBuffer> GetIndexBuffer(
    GrMeshDrawTarget* target, IndexBufferOption indexBufferOption) {
...
void IssueDraw(
    const GrCaps& caps, GrOpsRenderPass* renderPass, const VertexSpec& spec, int runningQuadCount,
    int quadsInDraw, int maxVerts, int absVertBufferOffset) {
  if (spec.indexBufferOption() == IndexBufferOption::kTriStrips) {
    int offset =
        absVertBufferOffset + runningQuadCount * GrResourceProvider::NumVertsPerNonAAQuad();
    renderPass->draw(4, offset);
    return;
  }

  SkASSERT(
      spec.indexBufferOption() == IndexBufferOption::kPictureFramed ||
  ...
  if (caps.avoidLargeIndexBufferDraws()) {
    ...
    renderPass->drawIndexPattern(
        numIndicesPerQuad, quadsInDraw, maxNumQuads, numVertsPerQuad, offset);
  } else {
    ...
    renderPass->drawIndexed(numIndicesToDraw, baseIndex, minVertex, maxVertex, absVertBufferOffset);
  }
}
...
class QuadPerEdgeAAGeometryProcessor : public GrGeometryProcessor {
 public:
  static GrGeometryProcessor* Make(SkArenaAlloc* arena, const VertexSpec& spec) {
    return arena->make([&](void* ptr) { return new (ptr) QuadPerEdgeAAGeometryProcessor(spec); });
  }

  static GrGeometryProcessor* Make(
      SkArenaAlloc* arena, const VertexSpec& vertexSpec, const GrShaderCaps& caps,
      const GrBackendFormat& backendFormat, GrSamplerState samplerState,
      const skgpu::Swizzle& swizzle, sk_sp<GrColorSpaceXform> textureColorSpaceXform,
      Saturate saturate) {
  ...
  std::unique_ptr<ProgramImpl> makeProgramImpl(const GrShaderCaps&) const override {
    class Impl : public ProgramImpl {
     public:
      void setData(
          const GrGLSLProgramDataManager& pdman, const GrShaderCaps&,
     ...
     private:
      void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
        ...
        fTextureColorSpaceXformHelper.emitCode(
            args.fUniformHandler, gp.fTextureColorSpaceXform.get());
        ...
        if (gp.fCoverageMode == CoverageMode::kWithPosition) {
          ...
          if (gp.fNeedsPerspective) {
            args.fVertBuilder->codeAppendf("float3 position = %s.xyz;", gp.fPosition.name());
          } else {
            args.fVertBuilder->codeAppendf("float2 position = %s.xy;", gp.fPosition.name());
          }
          gpArgs->fPositionVar = {
              "position", gp.fNeedsPerspective ? SkSLType::kFloat3 : SkSLType::kFloat2,
        ...
        if (gp.fColor.isInitialized()) {
          ...
          args.fVaryingHandler->addPassThroughAttribute(
              gp.fColor.asShaderVar(), args.fOutputColor,
              gp.fCoverageMode == CoverageMode::kWithColor ? Interpolation::kInterpolated
        ...
        if (gp.fSampler.isInitialized()) {
          ...
          if (gp.fLocalCoord.cpuType() == kFloat3_GrVertexAttribType) {
            ...
            args.fVertBuilder->codeAppendf("%s = %s;", v.vsOut(), gp.fLocalCoord.name());
            args.fFragBuilder->codeAppendf("texCoord = %s.xy / %s.z;", v.fsIn(), v.fsIn());
          } else {
            args.fVaryingHandler->addPassThroughAttribute(gp.fLocalCoord.asShaderVar(), "texCoord");
          }
          ...
          if (gp.fTexSubset.isInitialized()) {
            ...
            args.fVaryingHandler->addPassThroughAttribute(
                gp.fTexSubset.asShaderVar(), "subset", Interpolation::kCanBeFlat);
            args.fFragBuilder->codeAppend("texCoord = clamp(texCoord, subset.LT, subset.RB);");
          }

          // Now modulate the starting output color by the texture lookup
          args.fFragBuilder->codeAppendf(
              "%s = %s(", args.fOutputColor, (gp.fSaturate == Saturate::kYes) ? "saturate" : "");
          args.fFragBuilder->appendTextureLookupAndBlend(
              blendDst, SkBlendMode::kModulate, args.fTexSamplers[0], "texCoord",
        ...
        if (gp.fCoverageMode == CoverageMode::kWithPosition) {
          ...
          if (gp.fNeedsPerspective) {
            ...
            args.fVertBuilder->codeAppendf(
                "%s = %s.w * %s.z;", coverage.vsOut(), gp.fPosition.name(), gp.fPosition.name());
            args.fFragBuilder->codeAppendf(
                "float coverage = %s * sk_FragCoord.w;", coverage.fsIn());
          } else {
            args.fVertBuilder->codeAppendf("%s = %s;", coverage.vsOut(), gp.fCoverage.name());
          ...
          if (gp.fGeomSubset.isInitialized()) {
            ...
            args.fVaryingHandler->addPassThroughAttribute(
                gp.fGeomSubset.asShaderVar(), "geoSubset", Interpolation::kCanBeFlat);
#ifdef SK_USE_LEGACY_AA_QUAD_SUBSET
            args.fFragBuilder->codeAppend(
                "if (coverage < 0.5) {"
          ...
          args.fFragBuilder->codeAppendf("half4 %s = half4(half(coverage));", args.fOutputCoverage);
        } else {
 ...
 private:
  ...
  QuadPerEdgeAAGeometryProcessor(const VertexSpec& spec)
      : INHERITED(kQuadPerEdgeAAGeometryProcessor_ClassID), fTextureColorSpaceXform(nullptr) {
    SkASSERT(!spec.hasSubset());
  ...
  QuadPerEdgeAAGeometryProcessor(
      ...
      GrSamplerState samplerState, const skgpu::Swizzle& swizzle,
