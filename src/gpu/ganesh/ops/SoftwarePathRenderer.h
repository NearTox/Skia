...
class SoftwarePathRenderer final : public PathRenderer {
 public:
  const char* name() const noexcept override { return "SW"; }

  SoftwarePathRenderer(GrProxyProvider* proxyProvider, bool allowCaching) noexcept
      : fProxyProvider(proxyProvider), fAllowCaching(allowCaching) {}

  static bool GetShapeAndClipBounds(
      SurfaceDrawContext*, const GrClip*, const GrStyledShape&, const SkMatrix& viewMatrix,
      SkIRect* unclippedDevShapeBounds, SkIRect* clippedDevShapeBounds, SkIRect* devClipBounds);

 private:
  static void DrawNonAARect(
      SurfaceDrawContext*, GrPaint&&, const GrUserStencilSettings&, const GrClip*,
  ...
  static void DrawToTargetWithShapeMask(
      GrSurfaceProxyView, SurfaceDrawContext*, GrPaint&&, const GrUserStencilSettings&,
      const GrClip*, const SkMatrix& viewMatrix, const SkIPoint& textureOriginInDeviceSpace,
