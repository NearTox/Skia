...
class OpsTask : public GrRenderTask {
 public:
  ...
  OpsTask* asOpsTask() noexcept override { return this; }

  bool isEmpty() const noexcept { return fOpChains.empty(); }
  bool usesMSAASurface() const noexcept { return fUsesMSAASurface; }
  GrXferBarrierFlags renderPassXferBarriers() const noexcept { return fRenderPassXferBarriers; }
  ...
  void addDrawOp(
      GrDrawingManager*, GrOp::Owner, bool usesMSAA, const GrProcessorSet::Analysis&,
...
#if GR_TEST_UTILS
  void dump(
      const SkString& label, SkString indent, bool printDependencies, bool close) const override;
 ...
 protected:
  ...
  void recordOp(
      GrOp::Owner, bool usesMSAA, GrProcessorSet::Analysis, GrAppliedClip*, const GrDstProxyView*,
      const GrCaps&);

  ExpectedOutcome onMakeClosed(GrRecordingContext*, SkIRect* targetUpdateBounds) override;

 private:
  bool isColorNoOp() const noexcept {
  ...
  // resolve its texture.
  void setCannotMergeBackward() noexcept { fCannotMergeBackward = true; }
  ...
  class OpChain {
   public:
    ...
    OpChain(OpChain&&) noexcept = default;
    OpChain& operator=(OpChain&&) noexcept = default;
    ...
    const GrDstProxyView& dstProxyView() const noexcept { return fDstProxyView; }
    ...
    GrOp::Owner appendOp(
        GrOp::Owner op, GrProcessorSet::Analysis, const GrDstProxyView*, const GrAppliedClip*,
        const GrCaps&, SkArenaAlloc* opsTaskArena, GrAuditTrail*);

    bool shouldExecute() const { return SkToBool(this->head()); }

   private:
    class List {
     public:
      List() noexcept = default;
      List(GrOp::Owner) noexcept;
      List(List&&) noexcept;
    ...
    bool tryConcat(
        List*, GrProcessorSet::Analysis, const GrDstProxyView&, const GrAppliedClip*,
        const SkRect& bounds, const GrCaps&, SkArenaAlloc* opsTaskArena, GrAuditTrail*);
