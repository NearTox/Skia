...
class BoundingBoxShader : public GrGeometryProcessor {
 public:
  BoundingBoxShader(SkPMColor4f color, const GrShaderCaps& shaderCaps)
      : GrGeometryProcessor(kTessellate_BoundingBoxShader_ClassID), fColor(color) {
    ...
    this->setInstanceAttributesWithImplicitOffsets(
        kInstanceAttribs, SK_ARRAY_COUNT(kInstanceAttribs));
  }
...
GrProcessorSet::Analysis PathStencilCoverOp::finalize(
...
void PathStencilCoverOp::prePreparePrograms(
  ...
  auto pipelineFlags = (fPathFlags & FillPathFlags::kWireframe) ? GrPipeline::InputFlags::kWireframe
  ...
  if (fTotalCombinedPathVerbCnt > 50 &&
    ...
    auto shader = GrPathTessellationShader::MakeSimpleTriangleShader(
        args.fArena, shaderMatrix, SK_PMColor4fTRANSPARENT);
    fStencilFanProgram =
        GrTessellationShader::MakeProgram(args, shader, stencilPipeline, stencilSettings);
    fTessellator =
        PathCurveTessellator::Make(args.fArena, args.fCaps->shaderCaps()->fInfinitySupport);
  } else {
    fTessellator =
        PathWedgeTessellator::Make(args.fArena, args.fCaps->shaderCaps()->fInfinitySupport);
  }
  auto* tessShader = GrPathTessellationShader::Make(
      *args.fCaps->shaderCaps(), args.fArena, shaderMatrix, SK_PMColor4fTRANSPARENT,
      fTessellator->patchAttribs());
  fStencilPathProgram =
      GrTessellationShader::MakeProgram(args, tessShader, stencilPipeline, stencilSettings);
  ...
  if (!(fPathFlags & FillPathFlags::kStencilOnly)) {
    ...
    fCoverBBoxProgram = GrSimpleMeshDrawOpHelper::CreateProgramInfo(
        args.fCaps, args.fArena, bboxPipeline, args.fWriteView, args.fUsesMSAASurface, bboxShader,
        GrPrimitiveType::kTriangleStrip, args.fXferBarrierFlags, args.fColorLoadOp, bboxStencil);
  }
}

void PathStencilCoverOp::onPrePrepare(
...
void PathStencilCoverOp::onPrepare(GrOpFlushState* flushState) {
  ...
  fTessellator->prepare(
      flushState, tessShader->viewMatrix(), *fPathDrawList, fTotalCombinedPathVerbCnt);

  if (fCoverBBoxProgram) {
    size_t instanceStride = fCoverBBoxProgram->geomProc().instanceStride();
    VertexWriter vertexWriter =
        flushState->makeVertexWriter(instanceStride, fPathCount, &fBBoxBuffer, &fBBoxBaseInstance);
    SkDEBUGCODE(int pathCount = 0;)
    for (auto [pathMatrix, path, color] : *fPathDrawList) {
      SkDEBUGCODE(auto end = vertexWriter.mark(instanceStride));
      vertexWriter << pathMatrix.getScaleX() << pathMatrix.getSkewY() << pathMatrix.getSkewX()
                   << pathMatrix.getScaleY() << pathMatrix.getTranslateX()
...
void PathStencilCoverOp::onExecute(GrOpFlushState* flushState, const SkRect& chainBounds) {
  ...
  if (fCoverBBoxProgram && fCoverBBoxProgram->geomProc().hasVertexAttributes() &&
  ...
  if (fCoverBBoxProgram) {
    ...
    flushState->bindTextures(fCoverBBoxProgram->geomProc(), nullptr, fCoverBBoxProgram->pipeline());
