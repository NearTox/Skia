...
GrOp::Owner make_non_convex_fill_op(
    GrRecordingContext* rContext, SkArenaAlloc* arena, skgpu::v1::FillPathFlags fillPathFlags,
    GrAAType aaType, const SkRect& drawBounds, const SkIRect& clipBounds,
    const SkMatrix& viewMatrix, const SkPath& path, GrPaint&& paint) {
  ...
  if (numVerbs > 0 && !path.isInverseFillType()) {
    ...
    if (clippedDrawBounds.intersect(drawBounds)) {
      ...
      if (cpuTessellationWork * kCpuWeight + kMinNumPixelsToTriangulate < gpuFragmentWork) {
        return GrOp::Make<skgpu::v1::PathInnerTriangulateOp>(
            rContext, viewMatrix, path, std::move(paint), aaType, fillPathFlags, drawBounds);
      }
    }  // we should be clipped out when the GrClip is analyzed, so just return the default op
  }
  return GrOp::Make<skgpu::v1::PathStencilCoverOp>(
...
PathRenderer::CanDrawPath TessellationPathRenderer::onCanDrawPath(
  ...
  if (args.fAAType == GrAAType::kCoverage || shape.style().hasPathEffect() ||
...
bool TessellationPathRenderer::onDrawPath(const DrawPathArgs& args) {
    ...
    if (!args.fShape->style().isSimpleFill()) {
      ...
      if (stroke.isHairlineStyle()) {
        ...
        inflationRadius = SkStrokeRec::GetInflationRadius(
            stroke.getJoin(), stroke.getMiter(), stroke.getCap(), 1);
      } else {
  ...
  if (!args.fShape->style().isSimpleFill()) {
    ...
    auto op = GrOp::Make<StrokeTessellateOp>(
        args.fContext, args.fAAType, *args.fViewMatrix, path, stroke, std::move(args.fPaint));
  ...
  if (path.isConvex() && !path.isInverseFillType()) {
    auto op = GrOp::Make<PathTessellateOp>(
        args.fContext, args.fSurfaceDrawContext->arenaAlloc(), args.fAAType,
        args.fUserStencilSettings, *args.fViewMatrix, path, std::move(args.fPaint), pathDevBounds);
    sdc->addDrawOp(args.fClip, std::move(op));
    return true;
  }
  ...
  const SkRect& drawBounds =
      path.isInverseFillType()
          ...
          : pathDevBounds;
  auto op = make_non_convex_fill_op(
      args.fContext, args.fSurfaceDrawContext->arenaAlloc(), FillPathFlags::kNone, args.fAAType,
      drawBounds, *args.fClipConservativeBounds, *args.fViewMatrix, path, std::move(args.fPaint));
  sdc->addDrawOp(args.fClip, std::move(op));
  return true;
}
  ...
  if (path.isConvex()) {
    constexpr static GrUserStencilSettings kMarkStencil(
        GrUserStencilSettings::StaticInit<
            0x0001, GrUserStencilTest::kAlways, 0xffff, GrUserStencilOp::kReplace,
            GrUserStencilOp::kKeep, 0xffff>());

    GrPaint stencilPaint;
    stencilPaint.setXPFactory(GrDisableColorXPFactory::Get());
    auto op = GrOp::Make<PathTessellateOp>(
        args.fContext, args.fSurfaceDrawContext->arenaAlloc(), aaType, &kMarkStencil,
        *args.fViewMatrix, path, std::move(stencilPaint), pathDevBounds);
    sdc->addDrawOp(args.fClip, std::move(op));
    return;
  }

  auto op = make_non_convex_fill_op(
      args.fContext, args.fSurfaceDrawContext->arenaAlloc(), FillPathFlags::kStencilOnly, aaType,
      pathDevBounds, *args.fClipConservativeBounds, *args.fViewMatrix, path, GrPaint());
  sdc->addDrawOp(args.fClip, std::move(op));
}
