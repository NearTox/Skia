...
sk_sp<GrTextureProxy> GrProxyProvider::testingOnly_createInstantiatedProxy(
    ...
    int renderTargetSampleCnt, SkBackingFit fit, SkBudgeted budgeted, GrProtected isProtected) {
  ASSERT_SINGLE_OWNER
  if (this->isAbandoned()) {
    return nullptr;
  }
  ...
  if (SkBackingFit::kApprox == fit) {
    tex = resourceProvider->createApproxTexture(
        dimensions, format, format.textureType(), renderable, renderTargetSampleCnt, isProtected,
        /*label=*/{});
  } else {
    tex = resourceProvider->createTexture(
        dimensions, format, format.textureType(), renderable, renderTargetSampleCnt,
...
sk_sp<GrTextureProxy> GrProxyProvider::testingOnly_createInstantiatedProxy(
  ...
  return this->testingOnly_createInstantiatedProxy(
      dimensions, format, renderable, renderTargetSampleCnt, fit, budgeted, isProtected);
}
...
sk_sp<GrTextureProxy> GrProxyProvider::createWrapped(
    sk_sp<GrTexture> tex, UseAllocator useAllocator) {
#ifdef SK_DEBUG
  ...
  if (tex->asRenderTarget()) {
    return sk_sp<GrTextureProxy>(
        new GrTextureRenderTargetProxy(std::move(tex), useAllocator, this->isDDLProvider()));
  } else {
    return sk_sp<GrTextureProxy>(
...
sk_sp<GrTextureProxy> GrProxyProvider::findOrCreateProxyByUniqueKey(
    const skgpu::UniqueKey& key, UseAllocator useAllocator) {
  ASSERT_SINGLE_OWNER
...
GrSurfaceProxyView GrProxyProvider::findCachedProxyWithColorTypeFallback(
    const skgpu::UniqueKey& key, GrSurfaceOrigin origin, GrColorType ct, int sampleCnt) {
...
sk_sp<GrTextureProxy> GrProxyProvider::createProxyFromBitmap(
    const SkBitmap& bitmap, GrMipmapped mipmapped, SkBackingFit fit, SkBudgeted budgeted) {
  ASSERT_SINGLE_OWNER
  ...
  ATRACE_ANDROID_FRAMEWORK(
      "Upload %sTexture [%ux%u]", GrMipmapped::kYes == mipmapped ? "MipMap " : "", bitmap.width(),
      bitmap.height());
  ...
  if (!this->renderingDirectly() && !bitmap.isImmutable()) {
    ...
    if (mipmapped == GrMipmapped::kYes && bitmap.fMips) {
      copyBitmap.fMips = sk_sp<SkMipmap>(SkMipmap::Build(copyBitmap.pixmap(), nullptr, false));
...
sk_sp<GrTextureProxy> GrProxyProvider::createNonMippedProxyFromBitmap(
    const SkBitmap& bitmap, SkBackingFit fit, SkBudgeted budgeted) {
  ...
  sk_sp<GrTextureProxy> proxy = this->createLazyProxy(
      [bitmap](GrResourceProvider* resourceProvider, const LazySurfaceDesc& desc) {
        ...
        return LazyCallbackResult(resourceProvider->createTexture(
            desc.fDimensions, desc.fFormat, desc.fTextureType, colorType, desc.fRenderable,
            desc.fSampleCnt, desc.fBudgeted, desc.fFit, desc.fProtected, mipLevel, desc.fLabel));
      },
      format, dims, GrMipmapped::kNo, GrMipmapStatus::kNotAllocated, GrInternalSurfaceFlags::kNone,
      fit, budgeted, GrProtected::kNo, UseAllocator::kYes);
...
sk_sp<GrTextureProxy> GrProxyProvider::createMippedProxyFromBitmap(
    const SkBitmap& bitmap, SkBudgeted budgeted) {
  ...
  sk_sp<GrTextureProxy> proxy = this->createLazyProxy(
      [bitmap, mipmaps](GrResourceProvider* resourceProvider, const LazySurfaceDesc& desc) {
        ...
        return LazyCallbackResult(resourceProvider->createTexture(
            desc.fDimensions, desc.fFormat, desc.fTextureType, colorType, GrRenderable::kNo, 1,
            desc.fBudgeted, GrMipmapped::kYes, GrProtected::kNo, texels.get(), desc.fLabel));
      },
...
sk_sp<GrTextureProxy> GrProxyProvider::createProxy(
    ...
    int renderTargetSampleCnt, GrMipmapped mipmapped, SkBackingFit fit, SkBudgeted budgeted,
    GrProtected isProtected, std::string_view label, GrInternalSurfaceFlags surfaceFlags,
  ...
  if (!caps->validateSurfaceParams(
          dimensions, format, renderable, renderTargetSampleCnt, mipmapped, GrTextureType::k2D)) {
    return nullptr;
  }
  GrMipmapStatus mipmapStatus =
      (GrMipmapped::kYes == mipmapped) ? GrMipmapStatus::kDirty : GrMipmapStatus::kNotAllocated;
  if (renderable == GrRenderable::kYes) {
    ...
    return sk_sp<GrTextureProxy>(new GrTextureRenderTargetProxy(
        *caps, format, dimensions, renderTargetSampleCnt, mipmapped, mipmapStatus, fit, budgeted,
        isProtected, surfaceFlags | extraFlags, useAllocator, this->isDDLProvider(), label));
  }

  return sk_sp<GrTextureProxy>(new GrTextureProxy(
      format, dimensions, mipmapped, mipmapStatus, fit, budgeted, isProtected, surfaceFlags,
...
sk_sp<GrTextureProxy> GrProxyProvider::createCompressedTextureProxy(
  ...
  GrMipmapStatus mipmapStatus =
      (GrMipmapped::kYes == mipmapped) ? GrMipmapStatus::kValid : GrMipmapStatus::kNotAllocated;
  ...
  sk_sp<GrTextureProxy> proxy = this->createLazyProxy(
      [data](GrResourceProvider* resourceProvider, const LazySurfaceDesc& desc) {
        return LazyCallbackResult(resourceProvider->createCompressedTexture(
            desc.fDimensions, desc.fFormat, desc.fBudgeted, desc.fMipmapped, desc.fProtected,
            data.get(), desc.fLabel));
      },
...
sk_sp<GrTextureProxy> GrProxyProvider::wrapBackendTexture(
    ...
    GrIOType ioType, sk_sp<skgpu::RefCntedCallback> releaseHelper) {
...
sk_sp<GrTextureProxy> GrProxyProvider::wrapCompressedBackendTexture(
  ...
  sk_sp<GrTexture> tex =
      resourceProvider->wrapCompressedBackendTexture(beTex, ownership, cacheable);
  if (!tex) {
    return nullptr;
  }
...
sk_sp<GrTextureProxy> GrProxyProvider::wrapRenderableBackendTexture(
    ...
    GrWrapCacheable cacheable, sk_sp<skgpu::RefCntedCallback> releaseHelper) {
  if (this->isAbandoned()) {
    return nullptr;
  }
  ...
  sk_sp<GrTexture> tex =
      resourceProvider->wrapRenderableBackendTexture(backendTex, sampleCnt, ownership, cacheable);
  if (!tex) {
    return nullptr;
  }
  ...
  return sk_sp<GrTextureProxy>(
      new GrTextureRenderTargetProxy(std::move(tex), UseAllocator::kNo, this->isDDLProvider()));
}

sk_sp<GrSurfaceProxy> GrProxyProvider::wrapBackendRenderTarget(
    const GrBackendRenderTarget& backendRT, sk_sp<skgpu::RefCntedCallback> releaseHelper) {
  if (this->isAbandoned()) {
    return nullptr;
  }
...
sk_sp<GrRenderTargetProxy> GrProxyProvider::wrapVulkanSecondaryCBAsRenderTarget(
  ...
  sk_sp<GrRenderTarget> rt =
      resourceProvider->wrapVulkanSecondaryCBAsRenderTarget(imageInfo, vkInfo);
  if (!rt) {
    return nullptr;
  }
...
sk_sp<GrTextureProxy> GrProxyProvider::CreatePromiseProxy(
    GrContextThreadSafeProxy* threadSafeProxy, LazyInstantiateCallback&& callback,
    const GrBackendFormat& format, SkISize dimensions, GrMipmapped mipmapped) {
  ...
  auto proxy = sk_sp<GrTextureProxy>(new GrTextureProxy(
      std::move(callback), format, dimensions, mipmapped, mipmapStatus, SkBackingFit::kExact,
      SkBudgeted::kNo, GrProtected::kNo, GrInternalSurfaceFlags::kReadOnly,
      GrSurfaceProxy::UseAllocator::kYes, GrDDLProvider::kYes,
      /*label=*/{}));
...
sk_sp<GrTextureProxy> GrProxyProvider::createLazyProxy(
    ...
    GrMipmapped mipmapped, GrMipmapStatus mipmapStatus, GrInternalSurfaceFlags surfaceFlags,
    SkBackingFit fit, SkBudgeted budgeted, GrProtected isProtected,
    GrSurfaceProxy::UseAllocator useAllocator) {
  ASSERT_SINGLE_OWNER
  if (this->isAbandoned()) {
  ...
  return sk_sp<GrTextureProxy>(new GrTextureProxy(
      std::move(callback), format, dimensions, mipmapped, mipmapStatus, fit, budgeted, isProtected,
      surfaceFlags, useAllocator, this->isDDLProvider(),
      /*label=*/{}));
}

sk_sp<GrRenderTargetProxy> GrProxyProvider::createLazyRenderTargetProxy(
    LazyInstantiateCallback&& callback, const GrBackendFormat& format, SkISize dimensions,
    int sampleCnt, GrInternalSurfaceFlags surfaceFlags, const TextureInfo* textureInfo,
    GrMipmapStatus mipmapStatus, SkBackingFit fit, SkBudgeted budgeted, GrProtected isProtected,
  ...
  if (textureInfo) {
    ...
    return sk_sp<GrRenderTargetProxy>(new GrTextureRenderTargetProxy(
        *this->caps(), std::move(callback), format, dimensions, sampleCnt, textureInfo->fMipmapped,
        mipmapStatus, fit, budgeted, isProtected, surfaceFlags, useAllocator, this->isDDLProvider(),
        /*label=*/{}));
  }
  ...
  return sk_sp<GrRenderTargetProxy>(new GrRenderTargetProxy(
      std::move(callback), format, dimensions, sampleCnt, fit, budgeted, isProtected, surfaceFlags,
      useAllocator, vkSCB,
      /*label=*/{}));
}
...
sk_sp<GrTextureProxy> GrProxyProvider::MakeFullyLazyProxy(
    ...
    int renderTargetSampleCnt, GrProtected isProtected, const GrCaps& caps,
  ...
  if (GrRenderable::kYes == renderable) {
    return sk_sp<GrTextureProxy>(new GrTextureRenderTargetProxy(
        ...
        GrMipmapStatus::kNotAllocated, SkBackingFit::kApprox, SkBudgeted::kYes, isProtected,
        surfaceFlags, useAllocator, GrDDLProvider::kNo,
        /*label=*/{}));
  } else {
    return sk_sp<GrTextureProxy>(new GrTextureProxy(
        std::move(callback), format, kLazyDims, GrMipmapped::kNo, GrMipmapStatus::kNotAllocated,
...
void GrProxyProvider::processInvalidUniqueKey(
    const skgpu::UniqueKey& key, GrTextureProxy* proxy,
...
void GrProxyProvider::processInvalidUniqueKeyImpl(
    const skgpu::UniqueKey& key, GrTextureProxy* proxy, InvalidateGPUResource invalidateGPUResource,
...
void GrProxyProvider::orphanAllUniqueKeys() {
  fUniquelyKeyedProxies.foreach ([&](GrTextureProxy* proxy) { proxy->fProxyProvider = nullptr; });
}
...
void GrProxyProvider::removeAllUniqueKeys() {
  fUniquelyKeyedProxies.foreach ([&](GrTextureProxy* proxy) {
    ...
    this->processInvalidUniqueKeyImpl(
        proxy->getUniqueKey(), proxy, InvalidateGPUResource::kNo, RemoveTableEntry::kNo);
  });
  // Removing all those table entries is safe now.
  fUniquelyKeyedProxies.reset();
}

bool GrProxyProvider::renderingDirectly() const { return fImageContext->asDirectContext(); }
