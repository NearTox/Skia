 ...
 public:
  ...
  void addToKey(const GrShaderCaps& caps, skgpu::KeyBuilder* b) const override {
    ...
    key = ProgramImpl::AddMatrixKeys(
        caps, key, fViewMatrix, usesLocalMatrix ? fLocalMatrix : SkMatrix::I());
    b->add32(key);
  }
 ...
 private:
  class Impl : public ProgramImpl {
   public:
    void setData(
      ...
      SetTransform(pdman, shaderCaps, fLocalMatrixUniform, dgp.fLocalMatrix, &fLocalMatrixPrev);
   ...
   private:
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
      ...
      if (gp.hasVertexColor() || tweakAlpha) {
        ...
        if (gp.hasVertexColor()) {
          ...
          fColorUniform = uniformHandler->addUniform(
              nullptr, kVertex_GrShaderFlag, SkSLType::kHalf4, "Color", &colorUniformName);
      ...
      // Setup position
      WriteOutputPosition(
          vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, gp.fInPosition.name(),
      ...
      if (gp.fInLocalCoords.isInitialized()) {
        ...
        WriteLocalCoord(
            vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, gp.fInPosition.asShaderVar(),
      ...
      if (gp.hasVertexCoverage() && !tweakAlpha) {
        ...
        if (coverageNeedsSaturate) {
          fragBuilder->codeAppendf("half4 %s = half4(saturate(alpha));", args.fOutputCoverage);
        } else {
        ...
        fCoverageUniform = uniformHandler->addUniform(
            nullptr, kFragment_GrShaderFlag, SkSLType::kHalf, "Coverage", &fragCoverage);
        fragBuilder->codeAppendf("half4 %s = half4(%s);", args.fOutputCoverage, fragCoverage);
      }
    }
  ...
  DefaultGeoProc(
    ...
    if (fFlags & kLocalCoordAttribute_GPFlag) {
      fInLocalCoords = {"inLocalCoord", kFloat2_GrVertexAttribType, SkSLType::kFloat2};
    }
...
GrGeometryProcessor* DefaultGeoProc::TestCreate(GrProcessorTestData* d) {
  ...
  return DefaultGeoProc::Make(
      d->allocator(), flags, SkPMColor4f::FromBytes_RGBA(color), viewMtx, localMtx,
      readsLocalCoords, coverage);
}
#endif
