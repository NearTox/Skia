...
bool GrStyle::applyPathEffect(SkPath* dst, SkStrokeRec* strokeRec, const SkPath& src) const {
  ...
  if (SkPathEffect::kDash_DashType == fDashInfo.fType) {
    ...
    SkDashPath::CalcDashParameters(
        phase, intervals, intervalCnt, &initialLength, &initialIndex, &intervalLength);
    if (!SkDashPath::InternalFilter(
            dst, src, strokeRec, nullptr, intervals, intervalCnt, initialLength, initialIndex,
            intervalLength, SkDashPath::StrokeRecApplication::kDisallow)) {
      return false;
    }
...
bool GrStyle::applyPathEffectToPath(
    SkPath* dst, SkStrokeRec* remainingStroke, const SkPath& src, SkScalar resScale) const {
  SkASSERT(dst);
...
bool GrStyle::applyToPath(
    SkPath* dst, SkStrokeRec::InitStyle* style, const SkPath& src, SkScalar resScale) const {
  SkASSERT(style);
  SkASSERT(dst);
  SkStrokeRec strokeRec = fStrokeRec;
  strokeRec.setResScale(resScale);
  ...
  if (strokeRec.needToApply()) {
    ...
    *style = SkStrokeRec::kFill_InitStyle;
    ...
    SkASSERT(
        SkStrokeRec::kFill_Style == strokeRec.getStyle() ||
        SkStrokeRec::kHairline_Style == strokeRec.getStyle());
    *style = strokeRec.getStyle() == SkStrokeRec::kFill_Style ? SkStrokeRec::kFill_InitStyle
