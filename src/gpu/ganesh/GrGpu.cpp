...
static bool validate_texel_levels(
    SkISize dimensions, GrColorType texelColorType, const GrMipLevel* texels, int mipLevelCount,
    const GrCaps* caps) {
...
sk_sp<GrTexture> GrGpu::createTextureCommon(
    ...
    GrProtected isProtected, int mipLevelCount, uint32_t levelClearMask, std::string_view label) {
  ...
  if (!this->caps()->validateSurfaceParams(
          dimensions, format, renderable, renderTargetSampleCnt, mipmapped, textureType)) {
    return nullptr;
  }

  if (renderable == GrRenderable::kYes) {
    renderTargetSampleCnt = this->caps()->getRenderTargetSampleCount(renderTargetSampleCnt, format);
  }
  // Attempt to catch un- or wrongly initialized sample counts.
  SkASSERT(renderTargetSampleCnt > 0 && renderTargetSampleCnt <= 64);
  this->handleDirtyContext();
  auto tex = this->onCreateTexture(
      dimensions, format, renderable, renderTargetSampleCnt, budgeted, isProtected, mipLevelCount,
      levelClearMask, label);
  if (tex) {
...
sk_sp<GrTexture> GrGpu::createTexture(
  ...
  uint32_t levelClearMask = this->caps()->shouldInitializeTextures() ? (1 << mipLevelCount) - 1 : 0;
  auto tex = this->createTextureCommon(
      dimensions, format, textureType, renderable, renderTargetSampleCnt, budgeted, isProtected,
      mipLevelCount, levelClearMask, label);
...
sk_sp<GrTexture> GrGpu::createTexture(
    SkISize dimensions, const GrBackendFormat& format, GrTextureType textureType,
    GrRenderable renderable, int renderTargetSampleCnt, SkBudgeted budgeted,
    GrProtected isProtected, GrColorType textureColorType, GrColorType srcColorType,
    const GrMipLevel texels[], int texelLevelCount, std::string_view label) {
  TRACE_EVENT0("skia.gpu", TRACE_FUNC);
  if (texelLevelCount) {
    if (!validate_texel_levels(dimensions, srcColorType, texels, texelLevelCount, this->caps())) {
      return nullptr;
    }
  }
  ...
  auto tex = this->createTextureCommon(
      dimensions, format, textureType, renderable, renderTargetSampleCnt, budgeted, isProtected,
      texelLevelCount, levelClearMask, label);
  if (tex) {
    ...
    if (texelLevelCount && texels[0].fPixels) {
      if (!this->writePixels(
              tex.get(), SkIRect::MakeSize(dimensions), textureColorType, srcColorType, texels,
              texelLevelCount)) {
        return nullptr;
      }
...
sk_sp<GrTexture> GrGpu::createCompressedTexture(
  ...
  if (dataSize <
      SkCompressedDataSize(compressionType, dimensions, nullptr, mipmapped == GrMipmapped::kYes)) {
    return nullptr;
  }
  return this->onCreateCompressedTexture(
      dimensions, format, budgeted, mipmapped, isProtected, data, dataSize);
}
...
sk_sp<GrTexture> GrGpu::wrapBackendTexture(
  ...
  if (backendTex.width() > caps->maxTextureSize() || backendTex.height() > caps->maxTextureSize()) {
    return nullptr;
  }
...
sk_sp<GrRenderTarget> GrGpu::onWrapVulkanSecondaryCBAsRenderTarget(
    const SkImageInfo& imageInfo, const GrVkDrawableInfo& vkInfo) {
  // This is only supported on Vulkan so we default to returning nullptr here
  return nullptr;
}

sk_sp<GrGpuBuffer> GrGpu::createBuffer(
...
bool GrGpu::readPixels(
  ...
  SkASSERT(
      this->caps()->areColorTypeAndFormatCompatible(surfaceColorType, surface->backendFormat()));
...
bool GrGpu::writePixels(
  ...
  ATRACE_ANDROID_FRAMEWORK_ALWAYS(
      "Texture upload(%u) %ix%i", surface->uniqueID().asUInt(), rect.width(), rect.height());
  SkASSERT(surface);
  SkASSERT(!surface->framebufferOnly());
  ...
  this->handleDirtyContext();
  if (this->onWritePixels(
          surface, rect, surfaceColorType, srcColorType, texels, mipLevelCount,
          prepForTexSampling)) {
...
bool GrGpu::transferPixelsTo(
  ...
  this->handleDirtyContext();
  if (this->onTransferPixelsTo(
          texture, rect, textureColorType, bufferColorType, std::move(transferBuffer), offset,
          rowBytes)) {
...
bool GrGpu::transferPixelsFrom(
  ...
  this->handleDirtyContext();
  if (this->onTransferPixelsFrom(
          surface, rect, surfaceColorType, bufferColorType, std::move(transferBuffer), offset)) {
...
void GrGpu::didWriteToSurface(
    GrSurface* surface, GrSurfaceOrigin origin, const SkIRect* bounds, uint32_t mipLevels) const {
  SkASSERT(surface);
    ...
    for (size_t i = 0; i < info.fNumSemaphores; ++i) {
      if (info.fSignalSemaphores[i].isInitialized()) {
        semaphores[i] = resourceProvider->wrapBackendSemaphore(
            info.fSignalSemaphores[i], GrSemaphoreWrapType::kWillSignal, kBorrow_GrWrapOwnership);
...
GrOpsRenderPass* GrGpu::getOpsRenderPass(
  ...
  return this->onGetOpsRenderPass(
      renderTarget, useMSAASurface, stencil, origin, bounds, colorInfo, stencilInfo, sampledProxies,
...
bool GrGpu::CompressedDataIsCorrect(
    SkISize dimensions, SkImage::CompressionType compressionType, GrMipmapped mipmapped,
...
GrBackendTexture GrGpu::createBackendTexture(
...
bool GrGpu::clearBackendTexture(
...
GrBackendTexture GrGpu::createCompressedBackendTexture(
...
bool GrGpu::updateCompressedBackendTexture(
    const GrBackendTexture& backendTexture, sk_sp<skgpu::RefCntedCallback> finishedCallback,
    const void* data, size_t length) {
  SkASSERT(data);
  ...
  if (!CompressedDataIsCorrect(
          backendTexture.dimensions(), compressionType, mipmapped, data, length)) {
    return false;
  }

  return this->onUpdateCompressedBackendTexture(
      backendTexture, std::move(finishedCallback), data, length);
}
