 ...
 public:
  SimpleTriangleShader(const SkMatrix& viewMatrix, SkPMColor4f color)
      : GrPathTessellationShader(
            kTessellate_SimpleTriangleShader_ClassID, GrPrimitiveType::kTriangles, viewMatrix,
            color, PatchAttribs::kNone) {
    constexpr static Attribute kInputPointAttrib{
        "inputPoint", kFloat2_GrVertexAttribType, SkSLType::kFloat2};
...
std::unique_ptr<GrGeometryProcessor::ProgramImpl> SimpleTriangleShader::makeProgramImpl(
  ...
  class Impl : public GrPathTessellationShader::Impl {
    void emitVertexCode(
        const GrShaderCaps&, const GrPathTessellationShader&, GrGLSLVertexBuilder* v,
...
class MiddleOutShader : public GrPathTessellationShader {
 public:
  MiddleOutShader(
      const GrShaderCaps& shaderCaps, const SkMatrix& viewMatrix, const SkPMColor4f& color,
      PatchAttribs attribs)
      : GrPathTessellationShader(
            kTessellate_MiddleOutShader_ClassID, GrPrimitiveType::kTriangles, viewMatrix, color,
            attribs) {
    ...
    if (fAttribs & PatchAttribs::kFanPoint) {
      fInstanceAttribs.emplace_back(
          "fanPointAttrib", kFloat2_GrVertexAttribType, SkSLType::kFloat2);
    }
    if (fAttribs & PatchAttribs::kColor) {
      fInstanceAttribs.emplace_back(
          "colorAttrib",
          (fAttribs & PatchAttribs::kWideColorIfEnabled) ? kFloat4_GrVertexAttribType
    ...
    SkASSERT(this->instanceStride() == sizeof(SkPoint) * 4 + PatchAttribsStride(fAttribs));

    constexpr static Attribute kVertexAttrib(
        "resolveLevel_and_idx", kFloat2_GrVertexAttribType, SkSLType::kFloat2);
...
std::unique_ptr<GrGeometryProcessor::ProgramImpl> MiddleOutShader::makeProgramImpl(
  ...
  class Impl : public GrPathTessellationShader::Impl {
    void emitVertexCode(
        const GrShaderCaps& shaderCaps, const GrPathTessellationShader& shader,
        GrGLSLVertexBuilder* v, GrGLSLVaryingHandler* varyingHandler, GrGPArgs* gpArgs) override {
      const MiddleOutShader& middleOutShader = shader.cast<MiddleOutShader>();
      v->defineConstant("PRECISION", skgpu::tess::kPrecision);
      v->defineConstant("MAX_FIXED_RESOLVE_LEVEL", (float)skgpu::tess::kMaxResolveLevel);
      v->defineConstant("MAX_FIXED_SEGMENTS", (float)(skgpu::tess::kMaxParametricSegments));
        ...
        v->insertFunction(SkStringPrintf(
                ...
                bool is_triangular_conic_curve() {
                    return curveType == %g;
                              ...
                              skgpu::tess::kTriangularConicCurveType)
                              .c_str());
      } else {
      ...
      if (middleOutShader.fAttribs & PatchAttribs::kColor) {
        ...
        varyingHandler->addVarying(
            "color", &colorVarying, GrGLSLVaryingHandler::Interpolation::kCanBeFlat);
...
GrPathTessellationShader* GrPathTessellationShader::Make(
...
const GrPipeline* GrPathTessellationShader::MakeStencilOnlyPipeline(
    const ProgramArgs& args, GrAAType aaType, const GrAppliedHardClip& hardClip,
    GrPipeline::InputFlags pipelineFlags) {
  GrPipeline::InitArgs pipelineArgs;
  pipelineArgs.fInputFlags = pipelineFlags;
  pipelineArgs.fCaps = args.fCaps;
  return args.fArena->make<GrPipeline>(
      pipelineArgs, GrDisableColorXPFactory::MakeXferProcessor(), hardClip);
}
...
void GrPathTessellationShader::Impl::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
  ...
  fAffineMatrixUniform = args.fUniformHandler->addUniform(
      nullptr, kVertex_GrShaderFlag, SkSLType::kFloat4, "affineMatrix", &affineMatrix);
  fTranslateUniform = args.fUniformHandler->addUniform(
      nullptr, kVertex_GrShaderFlag, SkSLType::kFloat2, "translate", &translate);
  ...
  this->emitVertexCode(*args.fShaderCaps, shader, args.fVertBuilder, args.fVaryingHandler, gpArgs);

  // Fragment shader.
  if (!(shader.fAttribs & PatchAttribs::kColor)) {
    const char* color;
    fColorUniform = args.fUniformHandler->addUniform(
        nullptr, kFragment_GrShaderFlag, SkSLType::kHalf4, "color", &color);
    args.fFragBuilder->codeAppendf("half4 %s = %s;", args.fOutputColor, color);
  } else {
    args.fFragBuilder->codeAppendf("half4 %s = %s;", args.fOutputColor, fVaryingColorName.c_str());
  }
  args.fFragBuilder->codeAppendf("const half4 %s = half4(1);", args.fOutputCoverage);
}

void GrPathTessellationShader::Impl::setData(
