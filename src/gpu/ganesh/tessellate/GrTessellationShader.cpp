...
const GrPipeline* GrTessellationShader::MakePipeline(
    const ProgramArgs& args, GrAAType aaType, GrAppliedClip&& appliedClip,
  ...
  return args.fArena->make<GrPipeline>(pipelineArgs, std::move(processors), std::move(appliedClip));
}
