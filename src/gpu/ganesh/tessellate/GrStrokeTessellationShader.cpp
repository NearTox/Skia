...
GrStrokeTessellationShader::GrStrokeTessellationShader(
    const GrShaderCaps& shaderCaps, PatchAttribs attribs, const SkMatrix& viewMatrix,
    const SkStrokeRec& stroke, SkPMColor4f color)
    : GrTessellationShader(
          kTessellate_GrStrokeTessellationShader_ClassID, GrPrimitiveType::kTriangleStrip,
          viewMatrix, color),
  ...
  if (fPatchAttribs & PatchAttribs::kStrokeParams) {
    fAttribs.emplace_back("dynamicStrokeAttr", kFloat2_GrVertexAttribType, SkSLType::kFloat2);
  }
  if (fPatchAttribs & PatchAttribs::kColor) {
    fAttribs.emplace_back(
        "dynamicColorAttr",
        (fPatchAttribs & PatchAttribs::kWideColorIfEnabled) ? kFloat4_GrVertexAttribType
  ...
  if (!shaderCaps.fVertexIDSupport) {
    constexpr static Attribute kVertexAttrib("edgeID", kFloat_GrVertexAttribType, SkSLType::kFloat);
...
class GrStrokeTessellationShader::Impl : public ProgramImpl {
  ...
  void setData(
      const GrGLSLProgramDataManager& pdman, const GrShaderCaps&, const GrGeometryProcessor&) final;
...
void GrStrokeTessellationShader::Impl::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
  ...
  if (!shader.hasDynamicStroke()) {
    ...
    fTessControlArgsUniform = args.fUniformHandler->addUniform(
        nullptr, kVertex_GrShaderFlag, SkSLType::kFloat3, "tessControlArgs", &tessArgsName);
    args.fVertBuilder->codeAppendf(
        R"(
        float NUM_RADIAL_SEGMENTS_PER_RADIAN = %s.x;
        float JOIN_TYPE = %s.y;
        float STROKE_RADIUS = %s.z;)",
        tessArgsName, tessArgsName, tessArgsName);
  } else {
    ...
    fTessControlArgsUniform = args.fUniformHandler->addUniform(
        nullptr, kVertex_GrShaderFlag, SkSLType::kFloat, "maxScale", &maxScaleName);
    args.fVertBuilder->codeAppendf(
        R"(
  ...
  fAffineMatrixUniform = args.fUniformHandler->addUniform(
      nullptr, kVertex_GrShaderFlag, SkSLType::kFloat4, "affineMatrix", &affineMatrixName);
  fTranslateUniform = args.fUniformHandler->addUniform(
      nullptr, kVertex_GrShaderFlag, SkSLType::kFloat2, "translate", &translateName);
...
void GrStrokeTessellationShader::Impl::emitFragmentCode(
    const GrStrokeTessellationShader& shader, const EmitArgs& args) {
  if (!shader.hasDynamicColor()) {
    // The fragment shader just outputs a uniform color.
    const char* colorUniformName;
    fColorUniform = args.fUniformHandler->addUniform(
        nullptr, kFragment_GrShaderFlag, SkSLType::kHalf4, "color", &colorUniformName);
    args.fFragBuilder->codeAppendf("half4 %s = %s;", args.fOutputColor, colorUniformName);
  } else {
    args.fFragBuilder->codeAppendf("half4 %s = %s;", args.fOutputColor, fDynamicColorName.c_str());
  }
  args.fFragBuilder->codeAppendf("const half4 %s = half4(1);", args.fOutputCoverage);
}

void GrStrokeTessellationShader::Impl::setData(
