...
using StrokeWriter = PatchWriter<
    VertexChunkPatchAllocator, Required<PatchAttribs::kJoinControlPoint>,
    Optional<PatchAttribs::kStrokeParams>, Optional<PatchAttribs::kColor>,
    Optional<PatchAttribs::kWideColorIfEnabled>, Optional<PatchAttribs::kExplicitCurveType>,
    ReplicateLineEndPoints, TrackJoinControlPoints>;

void write_fixed_count_patches(
    StrokeWriter&& patchWriter, const SkMatrix& shaderMatrix,
  ...
  patchWriter.setShaderTransform(
      wangs_formula::VectorXform{shaderMatrix}, std::abs(shaderMatrix.getMaxScale()));
  ...
  for (auto* pathStroke = pathStrokeList; pathStroke; pathStroke = pathStroke->fNext) {
    ...
    while (strokeIter.next()) {
      ...
      switch (strokeIter.verb()) {
        case Verb::kContourFinished: patchWriter.writeDeferredStrokePatch(); break;
        case Verb::kCircle:
        ...
        case Verb::kMoveWithinContour:
          ...
          break;
        case Verb::kLine: patchWriter.writeLine(p[0], p[1]); break;
        case Verb::kQuad:
          if (ConicHasCusp(p)) {
...
void StrokeTessellator::prepare(
    GrMeshDrawTarget* target, const SkMatrix& shaderMatrix, PathStrokeList* pathStrokeList,
  ...
  if (!target->caps().shaderCaps()->fVertexIDSupport) {
    ...
    fVertexBufferIfNoIDSupport = target->resourceProvider()->findOrMakeStaticBuffer(
        GrGpuBufferType::kVertex, FixedCountStrokes::VertexBufferSize(), gVertexIDFallbackBufferKey,
...
void StrokeTessellator::draw(GrOpFlushState* flushState) const {
  ...
  if (!flushState->caps().shaderCaps()->fVertexIDSupport && !fVertexBufferIfNoIDSupport) {
    return;
  }
