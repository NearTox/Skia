...
class GrResourceProvider {
 public:
  ...
  typename std::enable_if<std::is_base_of<GrGpuResource, T>::value, sk_sp<T>>::type findByUniqueKey(
  ...
  sk_sp<GrTexture> createTexture(
      SkISize dimensions, const GrBackendFormat& format, GrTextureType textureType,
      GrColorType colorType, GrRenderable renderable, int renderTargetSampleCnt,
      SkBudgeted budgeted, GrMipmapped mipmapped, GrProtected isProtected,
      const GrMipLevel texels[], std::string_view label);
   ...
   */
  sk_sp<GrTexture> createTexture(
      SkISize dimensions, const GrBackendFormat&, GrTextureType textureType,
      GrColorType srcColorType, GrRenderable, int renderTargetSampleCnt, SkBudgeted, SkBackingFit,
      GrProtected, const GrMipLevel& mipLevel, std::string_view label);
  ...
  sk_sp<GrTexture> findAndRefScratchTexture(
      SkISize dimensions, const GrBackendFormat&, GrTextureType textureType, GrRenderable,
      int renderTargetSampleCnt, GrMipmapped, GrProtected, std::string_view label);
  ...
  sk_sp<GrTexture> createCompressedTexture(
      ...
      SkData* data, std::string_view label);
  ...
  sk_sp<const GrGpuBuffer> findOrMakeStaticBuffer(
      GrGpuBufferType intendedType, size_t size, const skgpu::UniqueKey& key, InitializeBufferFn);
  ...
  sk_sp<GrGpuBuffer> createBuffer(
      const void* data, size_t size, GrGpuBufferType type, GrAccessPattern pattern) {
    SkASSERT(data);
  ...
  std::unique_ptr<GrSemaphore> wrapBackendSemaphore(
      const GrBackendSemaphore&, GrSemaphoreWrapType, GrWrapOwnership = kBorrow_GrWrapOwnership);
 ...
 private:
  ...
  sk_sp<GrTexture> getExactScratch(
      SkISize dimensions, const GrBackendFormat&, GrTextureType, GrRenderable,
      int renderTargetSampleCnt, SkBudgeted, GrMipmapped, GrProtected, std::string_view label);
  ...
  sk_sp<GrAttachment> refScratchMSAAAttachment(
      SkISize dimensions, const GrBackendFormat&, int sampleCnt, GrProtected,
