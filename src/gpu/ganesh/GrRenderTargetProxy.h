...
class GrArenas : public SkNVRefCnt<GrArenas> {
 public:
  SkArenaAlloc* arenaAlloc() noexcept {
    SkDEBUGCODE(if (fIsFlushed) SK_ABORT("Using a flushed arena");)
    return &fArenaAlloc;
  }
  void flush() noexcept { SkDEBUGCODE(fIsFlushed = true;) }
  sktext::gpu::SubRunAllocator* subRunAlloc() noexcept { return &fSubRunAllocator; }
...
class GrRenderTargetProxy : virtual public GrSurfaceProxy {
 public:
  ...
  int needsStencil() const noexcept { return fNeedsStencil; }
  ...
  bool supportsVkInputAttachment() const noexcept {
    return fSurfaceFlags & GrInternalSurfaceFlags::kVkRTSupportsInputAttachment;
  }

  void markMSAADirty(SkIRect dirtyRect) noexcept {
