...
class GrThreadSafeCache {
 public:
  ...
  std::tuple<GrSurfaceProxyView, sk_sp<SkData>> findWithData(const skgpu::UniqueKey&)
  ...
  GrSurfaceProxyView findOrAdd(const skgpu::UniqueKey&, const GrSurfaceProxyView&)
      SK_EXCLUDES(fSpinLock);
  ...
  std::tuple<sk_sp<VertexData>, sk_sp<SkData>> findVertsWithData(const skgpu::UniqueKey&)
      SK_EXCLUDES(fSpinLock);

  typedef bool (*IsNewerBetter)(SkData* incumbent, SkData* challenger);

  std::tuple<sk_sp<VertexData>, sk_sp<SkData>> addVertsWithData(
      const skgpu::UniqueKey&, sk_sp<VertexData>, IsNewerBetter) SK_EXCLUDES(fSpinLock);
 ...
 private:
  ...
  std::tuple<GrSurfaceProxyView, sk_sp<SkData>> internalFind(const skgpu::UniqueKey&)
  ...
  std::tuple<sk_sp<VertexData>, sk_sp<SkData>> internalFindVerts(const skgpu::UniqueKey&)
      SK_REQUIRES(fSpinLock);
