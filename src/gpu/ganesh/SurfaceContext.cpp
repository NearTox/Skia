...
GrDrawingManager* SurfaceContext::drawingManager() { return fContext->priv().drawingManager(); }
...
static bool alpha_types_compatible(SkAlphaType srcAlphaType, SkAlphaType dstAlphaType) noexcept {
...
bool SurfaceContext::readPixels(GrDirectContext* dContext, GrPixmap dst, SkIPoint pt) {
    ...
    if (this->asTextureProxy()) {
      ...
      if (restrictions.fMustCopyWholeSrc) {
        copy = GrSurfaceProxy::Copy(
            fContext, std::move(srcProxy), this->origin(), kMipMapped, kFit, kBudgeted);
      } else {
        auto srcRect = SkIRect::MakePtSize(pt, dst.dimensions());
        copy = GrSurfaceProxy::Copy(
            fContext, std::move(srcProxy), this->origin(), kMipMapped, srcRect, kFit, kBudgeted,
  ...
  if (!dContext->priv().getGpu()->readPixels(
          srcSurface, SkIRect::MakePtSize(pt, dst.dimensions()), this->colorInfo().colorType(),
...
bool SurfaceContext::writePixels(GrDirectContext* dContext, GrCPixmap src, SkIPoint dstPt) {
  ASSERT_SINGLE_OWNER
  RETURN_FALSE_IF_ABANDONED
  SkDEBUGCODE(this->validate();)
...
bool SurfaceContext::writePixels(GrDirectContext* dContext, const GrCPixmap src[], int numLevels) {
  ASSERT_SINGLE_OWNER
  RETURN_FALSE_IF_ABANDONED
  SkDEBUGCODE(this->validate();)
...
bool SurfaceContext::internalWritePixels(
    GrDirectContext* dContext, const GrCPixmap src[], int numLevels, SkIPoint pt) {
  ...
  SkASSERT(
      SkIRect::MakeSize(this->dimensions()).contains(SkIRect::MakePtSize(pt, src[0].dimensions())));
  ...
  if ((!caps->surfaceSupportsWritePixels(dstSurface) || canvas2DFastPath) && numLevels == 1) {
    ...
    auto tempProxy = dContext->priv().proxyProvider()->createProxy(
        format, src[0].dimensions(), GrRenderable::kNo, 1, GrMipmapped::kNo, SkBackingFit::kApprox,
    ...
    if (canvas2DFastPath) {
      srcBase = GrCPixmap(
          origSrcBase.info().makeColorType(GrColorType::kRGBA_8888), origSrcBase.addr(),
    ...
    if (this->asFillContext()) {
      ...
      this->asFillContext()->fillRectToRectWithFP(
          SkIRect::MakeSize(srcBase.dimensions()), SkIRect::MakePtSize(pt, srcBase.dimensions()),
          std::move(fp));
    } else {
  ...
  auto [allowedColorType, _] = caps->supportedWritePixelsColorType(
      this->colorInfo().colorType(), dstProxy->backendFormat(), srcColorType);
  bool flip = this->origin() == kBottomLeft_GrSurfaceOrigin;

  bool convertAll =
      premul || unpremul || needColorConversion || flip || (srcColorType != allowedColorType);
  ...
  if (mustBeTight || convertAll) {
    for (int i = 0; i < numLevels; ++i) {
      if (convertAll || (mustBeTight && src[i].rowBytes() != src[i].info().minRowBytes())) {
        tmpSize += src[i].info().makeColorType(allowedColorType).minRowBytes() * src[i].height();
      }
    }
  }
  ...
  if (!dContext->priv().drawingManager()->newWritePixelsTask(
          sk_ref_sp(dstProxy), SkIRect::MakePtSize(pt, src[0].dimensions()), allowedColorType,
          this->colorInfo().colorType(), srcLevels.begin(), numLevels)) {
    return false;
  }
  if (numLevels > 1) {
...
void SurfaceContext::asyncRescaleAndReadPixels(
  ...
  return srcCtx->asyncReadPixels(
      dContext, SkIRect::MakePtSize({x, y}, info.dimensions()), info.colorType(), callback,
      callbackContext);
}
...
class SurfaceContext::AsyncReadResult : public SkImage::AsyncReadResult {
 ...
 private:
  ...
  class Plane {
   public:
    Plane(sk_sp<GrGpuBuffer> buffer, size_t rowBytes) noexcept 
        : fMappedBuffer(std::move(buffer)), fRowBytes(rowBytes) {}
    Plane(sk_sp<SkData> data, size_t rowBytes) noexcept
...
void SurfaceContext::asyncReadPixels(
  ...
  auto finishCallback = [](GrGpuFinishedContext c) {
    ...
    size_t rowBytes = SkAlignTo(
        context->fSize.width() * SkColorTypeBytesPerPixel(context->fColorType),
        context->fBufferAlignment);
    if (!result->addTransferResult(context->fTransferResult, context->fSize, rowBytes, manager)) {
      result.reset();
    }
...
void SurfaceContext::asyncRescaleAndReadPixelsYUV420(
  ...
  if (needsRescale) {
    auto info = SkImageInfo::Make(
        dstSize, kRGBA_8888_SkColorType, this->colorInfo().alphaType(), dstColorSpace);
    // TODO: Incorporate the YUV conversion into last pass of rescaling.
    auto tempFC = this->rescale(info, kTopLeft_GrSurfaceOrigin, srcRect, rescaleGamma, rescaleMode);
    if (!tempFC) {
      callback(callbackContext, nullptr);
      return;
    }
  ...
  auto [readCT, offsetAlignment] = this->caps()->supportedReadPixelsColorType(
      yFC->colorInfo().colorType(), yFC->asSurfaceProxy()->backendFormat(), GrColorType::kAlpha_8);
  if (readCT == GrColorType::kUnknown) {
    callback(callbackContext, nullptr);
    return;
  }
  bool doSynchronousRead = !this->caps()->transferFromSurfaceToBufferSupport() || !offsetAlignment;
  ...
  yFP = GrFragmentProcessor::ColorMatrix(
      std::move(yFP), yM,
      /*unpremulInput=*/false,
      /*clampRGBOutput=*/true,
      /*premulOutput=*/false);
  ...
  if (!doSynchronousRead) {
    yTransfer = yFC->transferPixels(GrColorType::kAlpha_8, SkIRect::MakeSize(yFC->dimensions()));
  ...
  if (!doSynchronousRead) {
    uTransfer = uFC->transferPixels(GrColorType::kAlpha_8, SkIRect::MakeSize(uFC->dimensions()));
  ...
  auto vFP = GrTextureEffect::Make(
      std::move(srcView), this->colorInfo().alphaType(), texMatrix,
      GrSamplerState::Filter::kLinear);
  vFP = GrFragmentProcessor::ColorMatrix(
      std::move(vFP), vM,
      /*unpremulInput=*/false,
      /*clampRGBOutput=*/true,
      /*premulOutput=*/false);
  ...
  if (!doSynchronousRead) {
    vTransfer = vFC->transferPixels(GrColorType::kAlpha_8, SkIRect::MakeSize(vFC->dimensions()));
  ...
  if (doSynchronousRead) {
    ...
    if (!yFC->readPixels(dContext, yPmp, {0, 0}) || !uFC->readPixels(dContext, uPmp, {0, 0}) ||
...
sk_sp<GrRenderTask> SurfaceContext::copy(
    sk_sp<GrSurfaceProxy> src, SkIRect srcRect, SkIPoint dstPoint) {
  ASSERT_SINGLE_OWNER
  ...
  return this->drawingManager()->newCopyRenderTask(
      std::move(src), srcRect, this->asSurfaceProxyRef(), dstPoint, this->origin());
}

std::unique_ptr<skgpu::SurfaceFillContext> SurfaceContext::rescale(
    const GrImageInfo& info, GrSurfaceOrigin origin, SkIRect srcRect, RescaleGamma rescaleGamma,
    RescaleMode rescaleMode) {
  auto sfc = fContext->priv().makeSFCWithFallback(
      info, SkBackingFit::kExact, 1, GrMipmapped::kNo, this->asSurfaceProxy()->isProtected(),
      origin);
  if (!sfc ||
      !this->rescaleInto(
          sfc.get(), SkIRect::MakeSize(sfc->dimensions()), srcRect, rescaleGamma, rescaleMode)) {
    return nullptr;
  }
  return sfc;
}

bool SurfaceContext::rescaleInto(
    skgpu::SurfaceFillContext* dst, SkIRect dstRect, SkIRect srcRect, RescaleGamma rescaleGamma,
    RescaleMode rescaleMode) {
  SkASSERT(dst);
  ...
  if (rescaleGamma == RescaleGamma::kLinear && this->colorInfo().colorSpace() &&
    ...
    GrImageInfo ii(
        GrColorType::kRGBA_F16, dst->colorInfo().alphaType(), std::move(cs), srcRect.size());
    auto linearRTC = fContext->priv().makeSFCWithFallback(
        std::move(ii), SkBackingFit::kApprox, 1, GrMipmapped::kNo, GrProtected::kNo, dst->origin());
    if (!linearRTC) {
      return false;
    }
    auto fp = GrTextureEffect::Make(
        std::move(texView), this->colorInfo().alphaType(), SkMatrix::Translate(srcRect.topLeft()),
        GrSamplerState::Filter::kNearest, GrSamplerState::MipmapMode::kNone);
    fp = GrColorSpaceXformEffect::Make(std::move(fp), this->colorInfo(), linearRTC->colorInfo());
  ...
  do {
    ...
    if (rescaleMode == RescaleMode::kRepeatedCubic) {
      ...
      fp = GrBicubicEffect::MakeSubset(
          std::move(texView), input->colorInfo().alphaType(), SkMatrix::I(), kWM, kWM,
      ...
      fp = GrTextureEffect::MakeSubset(
          std::move(texView), this->colorInfo().alphaType(), SkMatrix::I(),
          {filter, GrSamplerState::MipmapMode::kNone}, srcRectF, srcRectF, *this->caps());
    }
    if (xform) {
...
SurfaceContext::PixelTransferResult SurfaceContext::transferPixels(
  ...
  if (supportedRead.fColorType != dstCT || flip) {
    result.fPixelConverter = [w = rect.width(), h = rect.height(), dstCT, supportedRead, at](
      ...
      GrConvertPixels(
          GrPixmap(dstInfo, dst, dstInfo.minRowBytes()),
