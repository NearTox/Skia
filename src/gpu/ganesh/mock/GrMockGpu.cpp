...
sk_sp<GrGpu> GrMockGpu::Make(
    const GrMockOptions* mockOptions, const GrContextOptions& contextOptions,
...
GrOpsRenderPass* GrMockGpu::onGetOpsRenderPass(
...
GrThreadSafePipelineBuilder* GrMockGpu::pipelineBuilder() { return nullptr; }

sk_sp<GrThreadSafePipelineBuilder> GrMockGpu::refPipelineBuilder() { return nullptr; }
...
sk_sp<GrTexture> GrMockGpu::onCreateTexture(
  ...
  if (renderable == GrRenderable::kYes) {
    ...
    return sk_sp<GrTexture>(new GrMockTextureRenderTarget(
        this, budgeted, dimensions, renderTargetSampleCnt, isProtected, mipmapStatus, texInfo,
        rtInfo, label));
  }
  return sk_sp<GrTexture>(
      new GrMockTexture(this, budgeted, dimensions, isProtected, mipmapStatus, texInfo, label));
}

// TODO: why no 'isProtected' ?!
sk_sp<GrTexture> GrMockGpu::onCreateCompressedTexture(
  ...
  GrMockTextureInfo texInfo(
      GrColorType::kUnknown, format.asMockCompressionType(), NextInternalTextureID());
...
sk_sp<GrTexture> GrMockGpu::onWrapCompressedBackendTexture(
    const GrBackendTexture& tex, GrWrapOwnership ownership, GrWrapCacheable wrapType) {
  return nullptr;
}
...
sk_sp<GrTexture> GrMockGpu::onWrapRenderableBackendTexture(
  ...
  auto isProtected = GrProtected(tex.isProtected());
  return sk_sp<GrTexture>(new GrMockTextureRenderTarget(
      this, tex.dimensions(), sampleCnt, isProtected, mipmapStatus, texInfo, rtInfo, cacheable,
...
sk_sp<GrRenderTarget> GrMockGpu::onWrapBackendRenderTarget(const GrBackendRenderTarget& rt) {
  ...
  return sk_sp<GrRenderTarget>(new GrMockRenderTarget(
      this, GrMockRenderTarget::kWrapped, rt.dimensions(), rt.sampleCnt(), isProtected, info,
      /*label=*/"MockGpu_WrapBackendRenderTarget"));
}

sk_sp<GrGpuBuffer> GrMockGpu::onCreateBuffer(
    size_t sizeInBytes, GrGpuBufferType type, GrAccessPattern accessPattern) {
  return sk_sp<GrGpuBuffer>(new GrMockBuffer(
      this, sizeInBytes, type, accessPattern,
      /*label=*/"MockGpu_CreateBuffer"));
}

sk_sp<GrAttachment> GrMockGpu::makeStencilAttachment(
...
GrBackendRenderTarget GrMockGpu::createTestingOnlyBackendRenderTarget(
    SkISize dimensions, GrColorType colorType, int sampleCnt, GrProtected) {
