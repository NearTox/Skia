...
class GrMockTexture : public GrTexture {
 public:
  GrMockTexture(
      GrMockGpu* gpu, SkBudgeted budgeted, SkISize dimensions, GrProtected isProtected,
      GrMipmapStatus mipmapStatus, const GrMockTextureInfo& info, std::string_view label)
      : GrMockTexture(gpu, dimensions, isProtected, mipmapStatus, info, label) {
    this->registerWithCache(budgeted);
  }

  GrMockTexture(
      GrMockGpu* gpu, SkISize dimensions, GrProtected isProtected, GrMipmapStatus mipmapStatus,
...
class GrMockRenderTarget : public GrRenderTarget {
 public:
  GrMockRenderTarget(
      GrMockGpu* gpu, SkBudgeted budgeted, SkISize dimensions, int sampleCnt,
      GrProtected isProtected, const GrMockRenderTargetInfo& info, std::string_view label)
      : GrSurface(gpu, dimensions, isProtected, label),
  ...
  GrMockRenderTarget(
      ...
      const GrMockRenderTargetInfo& info, std::string_view label)
      : GrSurface(gpu, dimensions, isProtected, label),
        INHERITED(gpu, dimensions, sampleCnt, isProtected, label),
        fInfo(info) {
...
class GrMockTextureRenderTarget : public GrMockTexture, public GrMockRenderTarget {
 public:
  ...
  GrMockTextureRenderTarget(
      GrMockGpu* gpu, SkBudgeted budgeted, SkISize dimensions, int sampleCnt,
      GrProtected isProtected, GrMipmapStatus mipmapStatus, const GrMockTextureInfo& texInfo,
      const GrMockRenderTargetInfo& rtInfo, std::string_view label)
      : GrSurface(gpu, dimensions, isProtected, label),
  ...
  GrMockTextureRenderTarget(
      GrMockGpu* gpu, SkISize dimensions, int sampleCnt, GrProtected isProtected,
      GrMipmapStatus mipmapStatus, const GrMockTextureInfo& texInfo,
      const GrMockRenderTargetInfo& rtInfo, GrWrapCacheable cacheable, std::string_view label)
      : GrSurface(gpu, dimensions, isProtected, label),
        GrMockTexture(gpu, dimensions, isProtected, mipmapStatus, texInfo, label),
        GrMockRenderTarget(gpu, dimensions, sampleCnt, isProtected, rtInfo, label) {
    this->registerWithCacheWrapped(cacheable);
 ...
 private:
  ...
  size_t onGpuMemorySize() const override {
    ...
    return GrSurface::ComputeSize(
        this->backendFormat(), this->dimensions(), numColorSamples, this->mipmapped());
  }
