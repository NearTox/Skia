...
void EllipticalRRectEffect::Impl::emitCode(EmitArgs& args) {
  ...
  fInnerRectUniform = uniformHandler->addUniform(
      &erre, kFragment_GrShaderFlag, SkSLType::kFloat4, "innerRect", &rectName);
  ...
  if (elliptical_effect_uses_scale(*args.fShaderCaps, erre.fRRect)) {
    fScaleUniform = uniformHandler->addUniform(
        &erre, kFragment_GrShaderFlag, SkSLType::kHalf2, "scale", &scaleName);
  }
  ...
  switch (erre.fRRect.getType()) {
    case SkRRect::kSimple_Type: {
      ...
      fInvRadiiSqdUniform = uniformHandler->addUniform(
          &erre, kFragment_GrShaderFlag, SkSLType::kFloat2, "invRadiiXY", &invRadiiXYSqdName);
      fragBuilder->codeAppend("float2 dxy = max(max(dxy0, dxy1), 0.0);");
      if (scaleName) {
    ...
    case SkRRect::kNinePatch_Type: {
      ...
      fInvRadiiSqdUniform = uniformHandler->addUniform(
          &erre, kFragment_GrShaderFlag, SkSLType::kFloat4, "invRadiiLTRB", &invRadiiLTRBSqdName);
      if (scaleName) {
