 ...
 public:
  void emitCode(EmitArgs& args) override {
    ...
    class FPCallbacks : public SkSL::PipelineStage::Callbacks {
     public:
      FPCallbacks(
          Impl* self, EmitArgs& args, const char* inputColor, const SkSL::Context& context,
          const uint8_t* uniformData, const Specialized* specialized)
          : fSelf(self),
            fArgs(args),
      ...
      std::string declareUniform(const SkSL::VarDeclaration* decl) override {
        ...
        if (*fSpecialized++ == Specialized::kYes) {
          ...
          for (size_t i = 0; i < slots; ++i) {
            value.append(isFloat ? skstd::to_string(floatData[i]) : std::to_string(intData[i]));
            value.append(",");
          }
          value.back() = ')';
          return value;
        ...
        auto handle = fArgs.fUniformHandler->addUniformArray(
            &fArgs.fFp.cast<GrSkSLFP>(), kFragment_GrShaderFlag, gpuType,
            SkString(var.name()).c_str(), isArray ? var.type().columns() : 0, &uniformName);
      ...
      std::string sampleColorFilter(int index, std::string color) override {
        return std::string(
            fSelf->invokeChild(index, color.empty() ? fInputColor : color.c_str(), fArgs).c_str());
      }
      ...
      std::string sampleBlender(int index, std::string src, std::string dst) override {
        ...
        return std::string(fSelf->invokeChild(index, src.c_str(), dst.c_str(), fArgs).c_str());
      }
      ...
      std::string toLinearSrgb(std::string color) override {
        ...
        SkString xformedColor =
            fSelf->invokeChild(fp.fToLinearSrgbChildIndex, color.c_str(), fArgs);
      ...
      std::string fromLinearSrgb(std::string color) override {
        ...
        color = SkSL::String::printf("(%s).rgb1", color.c_str());
        SkString xformedColor =
            fSelf->invokeChild(fp.fFromLinearSrgbChildIndex, color.c_str(), fArgs);
        return SkSL::String::printf("(%s).rgb", xformedColor.c_str());
      }
    ...
    if (fp.fInputChildIndex >= 0) {
      args.fFragBuilder->codeAppendf(
          "%s = %s;\n", args.fInputColor, this->invokeChild(fp.fInputChildIndex, args).c_str());
    }
    ...
    if (fp.fEffect->samplesOutsideMain()) {
      GrShaderVar inputColorCopy(
          args.fFragBuilder->getMangledFunctionName("inColor"), SkSLType::kHalf4);
    ...
    FPCallbacks callbacks(
        this, args, inputColorName.c_str(), *program.fContext, fp.uniformData(), fp.specialized());
 ...
 private:
  void onSetData(const GrGLSLProgramDataManager& pdman, const GrFragmentProcessor& _proc) override {
    ...
    pdman.setRuntimeEffectUniforms(
        outer.fEffect->uniforms(), SkMakeSpan(fUniformHandles),
        SkMakeSpan(outer.specialized(), outer.uniformCount()), outer.uniformData());
  }
...
std::unique_ptr<GrSkSLFP> GrSkSLFP::MakeWithData(
    sk_sp<SkRuntimeEffect> effect, const char* name, sk_sp<SkColorSpace> dstColorSpace,
    std::unique_ptr<GrFragmentProcessor> inputFP, std::unique_ptr<GrFragmentProcessor> destColorFP,
    sk_sp<const SkData> uniforms, SkSpan<std::unique_ptr<GrFragmentProcessor>> childFPs) {
...
GrSkSLFP::GrSkSLFP(sk_sp<SkRuntimeEffect> effect, const char* name, OptFlags optFlags)
    : INHERITED(
          ...
          static_cast<OptimizationFlags>(optFlags) |
              (effect->getFilterColorProgram() ? kConstantOutputForConstantInput_OptimizationFlag
...
void GrSkSLFP::addColorTransformChildren(sk_sp<SkColorSpace> dstColorSpace) {
  SkASSERTF(
      fToLinearSrgbChildIndex == -1 && fFromLinearSrgbChildIndex == -1,
  ...
  auto workingToLinear = GrColorSpaceXformEffect::Make(
      nullptr, dstColorSpace.get(), kUnpremul_SkAlphaType, sk_srgb_linear_singleton(),
      kUnpremul_SkAlphaType);
  auto linearToWorking = GrColorSpaceXformEffect::Make(
      nullptr, sk_srgb_linear_singleton(), kUnpremul_SkAlphaType, dstColorSpace.get(),
      kUnpremul_SkAlphaType);
...
bool GrSkSLFP::onIsEqual(const GrFragmentProcessor& other) const {
  ...
  return fEffect->hash() == sk.fEffect->hash() && this->uniformCount() == sk.uniformCount() &&
         fUniformSize == sk.fUniformSize &&
         !sk_careful_memcmp(this->uniformData(), sk.uniformData(), fUniformSize + specializedSize);
}
...
std::unique_ptr<GrFragmentProcessor> GrSkSLFP::TestCreate(GrProcessorTestData* d) {
  ...
  auto [success, fp] =
      as_CFB(filter)->asFragmentProcessor(/*inputFP=*/nullptr, d->context(), GrColorInfo{});
  SkASSERT(success);
  return std::move(fp);
}
