...
void GrGaussianConvolutionFragmentProcessor::Impl::emitCode(EmitArgs& args) {
  ...
  fIncrementUni = uniformHandler->addUniform(
      &ce, kFragment_GrShaderFlag, SkSLType::kHalf2, "Increment", &increment);
  ...
  int arrayCount = variableLengthLoop ? SkGpuBlurUtils::LinearKernelWidth(kMaxKernelRadius) : width;

  const char* offsetsAndKernel;
  fOffsetsAndKernelUni = uniformHandler->addUniformArray(
      &ce, kFragment_GrShaderFlag, SkSLType::kHalf2, "OffsetsAndKernel", arrayCount,
      &offsetsAndKernel);
  ...
  const GrShaderVar smoothArgs[] = {
      {args.fInputColor, SkSLType::kHalf4},
  ...
  fragBuilder->emitFunction(
      SkSLType::kHalf4, smoothFuncName.c_str(), {smoothArgs, SK_ARRAY_COUNT(smoothArgs)},
      smoothBody.c_str());
  ...
  if (variableLengthLoop) {
    ...
    fKernelWidthUni = uniformHandler->addUniform(
        &ce, kFragment_GrShaderFlag, SkSLType::kInt, "KernelWidth", &kernelWidth);
    fragBuilder->codeAppendf(
        "for (int i=0; i<%s; ++i) {"
        "    color += %s(%s, coord, %s[i]);"
        "}",
        kernelWidth, smoothFuncName.c_str(), args.fInputColor, offsetsAndKernel);
  } else {
    fragBuilder->codeAppendf(
        "for (int i=0; i<%d; ++i) {"
        "    color += %s(%s, coord, %s[i]);"
        "}",
        width, smoothFuncName.c_str(), args.fInputColor, offsetsAndKernel);
  }
  fragBuilder->codeAppendf("return color;\n");
}

void GrGaussianConvolutionFragmentProcessor::Impl::onSetData(
...
std::unique_ptr<GrFragmentProcessor> GrGaussianConvolutionFragmentProcessor::Make(
    GrSurfaceProxyView view, SkAlphaType alphaType, Direction dir, int halfWidth,
    float gaussianSigma, GrSamplerState::WrapMode wm, const SkIRect& subset,
    const SkIRect* pixelDomain, const GrCaps& caps) {
  ...
  GrSamplerState::Filter filter =
      is_zero_sigma ? GrSamplerState::Filter::kNearest : GrSamplerState::Filter::kLinear;
  ...
  if (pixelDomain && !alwaysUseShaderTileMode) {
    ...
    child = GrTextureEffect::MakeSubset(
        std::move(view), alphaType, SkMatrix::I(), sampler, SkRect::Make(subset), domain, caps,
        GrTextureEffect::kDefaultBorder);
  } else {
    child = GrTextureEffect::MakeSubset(
        std::move(view), alphaType, SkMatrix::I(), sampler, SkRect::Make(subset), caps,
        GrTextureEffect::kDefaultBorder, alwaysUseShaderTileMode);
  }
  ...
  return std::unique_ptr<GrFragmentProcessor>(
      new GrGaussianConvolutionFragmentProcessor(std::move(child), dir, halfWidth, gaussianSigma));
}

GrGaussianConvolutionFragmentProcessor::GrGaussianConvolutionFragmentProcessor(
    std::unique_ptr<GrFragmentProcessor> child, Direction direction, int radius,
    float gaussianSigma)
    : INHERITED(
...
GrGaussianConvolutionFragmentProcessor::GrGaussianConvolutionFragmentProcessor(
    ...
    : INHERITED(that), fRadius(that.fRadius), fDirection(that.fDirection) {
  memcpy(fOffsetsAndKernel, that.fOffsetsAndKernel, sizeof(fOffsetsAndKernel));
}

void GrGaussianConvolutionFragmentProcessor::onAddToKey(
    const GrShaderCaps& shaderCaps, skgpu::KeyBuilder* b) const {
...
bool GrGaussianConvolutionFragmentProcessor::onIsEqual(const GrFragmentProcessor& sBase) const {
  ...
  return fRadius == that.fRadius && fDirection == that.fDirection &&
         std::equal(
             fOffsetsAndKernel, fOffsetsAndKernel + SkGpuBlurUtils::LinearKernelWidth(fRadius),
...
std::unique_ptr<GrFragmentProcessor> GrGaussianConvolutionFragmentProcessor::TestCreate(
  ...
  return GrGaussianConvolutionFragmentProcessor::Make(
      std::move(view), at, dir, radius, sigma, wm, subset, domain, *d->caps());
}
#endif
