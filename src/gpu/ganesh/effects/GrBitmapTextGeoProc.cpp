 ...
 private:
  void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
    ...
    fAtlasDimensionsInvUniform = uniformHandler->addUniform(
        nullptr, kVertex_GrShaderFlag, SkSLType::kFloat2, "AtlasSizeInv", &atlasDimensionsInvName);

    GrGLSLVarying uv, texIdx;
    append_index_uv_varyings(
        args, btgp.numTextureSamplers(), btgp.fInTextureCoords.name(), atlasDimensionsInvName, &uv,
        &texIdx, nullptr);
    ...
    WriteLocalCoord(
        vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, btgp.fInPosition.asShaderVar(),
...
GrBitmapTextGeoProc::GrBitmapTextGeoProc(
    ...
    const GrSurfaceProxyView* views, int numActiveViews, GrSamplerState params, MaskFormat format,
...
void GrBitmapTextGeoProc::addNewViews(
...
GrGeometryProcessor* GrBitmapTextGeoProc::TestCreate(GrProcessorTestData* d) {
  ...
  MaskFormat format;
  switch (ct) {
    case GrColorType::kAlpha_8: format = MaskFormat::kA8; break;
    case GrColorType::kBGR_565: format = MaskFormat::kA565; break;
  ...
  return GrBitmapTextGeoProc::Make(
      d->allocator(), *d->caps()->shaderCaps(), SkPMColor4f::FromBytes_RGBA(color), wideColor,
      &view, 1, samplerState, format, localMatrix, usesW);
}
#endif
