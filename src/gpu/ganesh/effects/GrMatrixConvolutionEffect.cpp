...
void GrMatrixConvolutionEffect::Impl::emitCode(EmitArgs& args) {
  ...
  if (mce.fKernel.isSampled()) {
    fKernelBiasUni =
        uniformHandler->addUniform(&mce, kFragment_GrShaderFlag, SkSLType::kHalf, "KernelBias");
  } else {
    fKernelUni = uniformHandler->addUniformArray(
        &mce, kFragment_GrShaderFlag, SkSLType::kHalf4, "Kernel", arrayCount);
  }
  fKernelOffsetUni =
      uniformHandler->addUniform(&mce, kFragment_GrShaderFlag, SkSLType::kHalf2, "KernelOffset");
...
void GrMatrixConvolutionEffect::Impl::onSetData(
...
void GrMatrixConvolutionEffect::onAddToKey(const GrShaderCaps& caps, skgpu::KeyBuilder* b) const {
...
std::unique_ptr<GrFragmentProcessor::ProgramImpl> GrMatrixConvolutionEffect::onMakeProgramImpl()
    const {
...
bool GrMatrixConvolutionEffect::onIsEqual(const GrFragmentProcessor& sBase) const {
  ...
  return fKernel == s.fKernel && fGain == s.fGain && fBias == s.fBias &&
         fKernelOffset == s.fKernelOffset && fConvolveAlpha == s.fConvolveAlpha;
}
