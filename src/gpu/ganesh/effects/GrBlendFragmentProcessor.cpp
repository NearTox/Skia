...
class BlendFragmentProcessor : public GrFragmentProcessor {
 ...
 private:
  ...
  BlendFragmentProcessor(const BlendFragmentProcessor& that)
      : INHERITED(that), fMode(that.fMode), fShareBlendLogic(that.fShareBlendLogic) {}
    ...
    void emitCode(EmitArgs& args) override {
      ...
      if (bfp.fShareBlendLogic) {
        ...
        std::string blendExpr = GrGLSLBlend::BlendExpression(
            &args.fFp, args.fUniformHandler, &fBlendUniform, srcColor.c_str(), dstColor.c_str(),
            mode);
        ...
        fragBuilder->codeAppendf(
            "return %s(%s, %s);", skgpu::BlendFuncName(mode), srcColor.c_str(), dstColor.c_str());
      }
    }
