...
std::unique_ptr<GrFragmentProcessor> GrYUVtoRGBEffect::Make(
    const GrYUVATextureProxies& yuvaProxies, GrSamplerState samplerState, const GrCaps& caps,
    const SkMatrix& localMatrix, const SkRect* subset, const SkRect* domain) {
  ...
  for (int i = 0; i < numPlanes; ++i) {
    ...
    if (ssx > 1 || ssy > 1) {
      ...
      if (subset) {
        planeSubset = {
            subset->fLeft * scaleX, subset->fTop * scaleY, subset->fRight * scaleX,
      ...
      if (domain) {
        planeDomain = {
            domain->fLeft * scaleX, domain->fTop * scaleY, domain->fRight * scaleX,
      ...
      if (samplerState.filter() == GrSamplerState::Filter::kNearest) {
        ...
        if (domain) {
          ...
          // within this plane.
          planeDomain = {
              std::floor(planeDomain.fLeft) + 0.5f, std::floor(planeDomain.fTop) + 0.5f,
              std::floor(planeDomain.fRight) + 0.5f, std::floor(planeDomain.fBottom) + 0.5f};
        }
      }
    } else {
      if (subset) {
    ...
    if (useSubset) {
      if (makeLinearWithSnap) {
        ...
        planeFPs[i] = GrTextureEffect::MakeCustomLinearFilterInset(
            std::move(view), kUnknown_SkAlphaType, planeMatrix, samplerState.wrapModeX(),
            samplerState.wrapModeY(), planeSubset, domainRect, {scaleX / 2.f, scaleY / 2.f}, caps,
            planeBorders[i]);
      } else if (domain) {
        planeFPs[i] = GrTextureEffect::MakeSubset(
            std::move(view), kUnknown_SkAlphaType, planeMatrix, samplerState, planeSubset,
            planeDomain, caps, planeBorders[i]);
      } else {
        planeFPs[i] = GrTextureEffect::MakeSubset(
            std::move(view), kUnknown_SkAlphaType, planeMatrix, samplerState, planeSubset, caps,
            planeBorders[i]);
      }
    } else {
      GrSamplerState planeSampler = samplerState;
      if (makeLinearWithSnap) {
        planeSampler = GrSamplerState(
            samplerState.wrapModeX(), samplerState.wrapModeY(), GrSamplerState::Filter::kLinear,
            samplerState.mipmapMode());
      }
      planeFPs[i] = GrTextureEffect::Make(
          std::move(view), kUnknown_SkAlphaType, planeMatrix, planeSampler, caps, planeBorders[i]);
    }
  }
  std::unique_ptr<GrFragmentProcessor> fp(new GrYUVtoRGBEffect(
      planeFPs, numPlanes, yuvaProxies.yuvaLocations(), snap,
...
GrYUVtoRGBEffect::GrYUVtoRGBEffect(
    std::unique_ptr<GrFragmentProcessor> planeFPs[4], int numPlanes,
    const SkYUVAInfo::YUVALocations& locations, const bool snap[2], SkYUVColorSpace yuvColorSpace)
    : GrFragmentProcessor(
          kGrYUVtoRGBEffect_ClassID, ModulateForClampedSamplerOptFlags(alpha_type(locations))),
...
SkString GrYUVtoRGBEffect::onDumpInfo() const {
  ...
  for (int i = 0; i < SkYUVAInfo::kYUVAChannelCount; ++i) {
    str.appendf(
        "Locations[%d]=%d %d, ", i, fLocations[i].fPlane, static_cast<int>(fLocations[i].fChannel));
  }
  str.appendf(
      "YUVColorSpace=%d, snap=(%d, %d))", static_cast<int>(fYUVColorSpace), fSnap[0], fSnap[1]);
  return str;
}
#endif
    ...
    void emitCode(EmitArgs& args) override {
      ...
      for (int planeIdx = 0; planeIdx < numPlanes; ++planeIdx) {
        ...
        if (!colorChannel.empty()) {
          fragBuilder->codeAppendf(
              "color.%s = (%s).%s;", colorChannel.c_str(),
              this->invokeChild(planeIdx, args, sampleCoords).c_str(), planeChannel.c_str());
        }
      }
      ...
      if (kIdentity_SkYUVColorSpace != yuvEffect.fYUVColorSpace) {
        fColorSpaceMatrixVar = args.fUniformHandler->addUniform(
            &yuvEffect, kFragment_GrShaderFlag, SkSLType::kHalf3x3, "colorSpaceMatrix");
        fColorSpaceTranslateVar = args.fUniformHandler->addUniform(
            &yuvEffect, kFragment_GrShaderFlag, SkSLType::kHalf3, "colorSpaceTranslate");
        fragBuilder->codeAppendf(
   ...
   private:
    void onSetData(
      ...
      if (yuvEffect.fYUVColorSpace != kIdentity_SkYUVColorSpace) {
        ...
        float mtx[9] = {
            yuvM[0], yuvM[1], yuvM[2], yuvM[5], yuvM[6], yuvM[7], yuvM[10], yuvM[11], yuvM[12],
        };
...
bool GrYUVtoRGBEffect::onIsEqual(const GrFragmentProcessor& other) const {
  ...
  return fLocations == that.fLocations && std::equal(fSnap, fSnap + 2, that.fSnap) &&
         fYUVColorSpace == that.fYUVColorSpace;
}

GrYUVtoRGBEffect::GrYUVtoRGBEffect(const GrYUVtoRGBEffect& src)
    : GrFragmentProcessor(src), fLocations((src.fLocations)), fYUVColorSpace(src.fYUVColorSpace) {
