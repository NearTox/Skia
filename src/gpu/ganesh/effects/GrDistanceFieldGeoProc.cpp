 ...
 private:
  void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
    ...
    fAtlasDimensionsInvUniform = uniformHandler->addUniform(
        nullptr, kVertex_GrShaderFlag, SkSLType::kFloat2, "AtlasDimensionsInv",
    ...
    fDistanceAdjustUni = uniformHandler->addUniform(
        nullptr, kFragment_GrShaderFlag, SkSLType::kHalf, "DistanceAdjust", &distanceAdjustUniName);
#endif

    // Setup pass through color
    fragBuilder->codeAppendf("half4 %s;\n", args.fOutputColor);
    varyingHandler->addPassThroughAttribute(dfTexEffect.fInColor.asShaderVar(), args.fOutputColor);

    // Setup position
    gpArgs->fPositionVar = dfTexEffect.fInPosition.asShaderVar();
    WriteLocalCoord(
        vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, gpArgs->fPositionVar,
        dfTexEffect.fLocalMatrix, &fLocalMatrixUniform);

    // add varyings
    GrGLSLVarying uv, texIdx, st;
    append_index_uv_varyings(
        args, dfTexEffect.numTextureSamplers(), dfTexEffect.fInTextureCoords.name(),
    ...
    append_multitexture_lookup(args, dfTexEffect.numTextureSamplers(), texIdx, "uv", "texColor");

    fragBuilder->codeAppend("half distance = " SK_DistanceFieldMultiplier
                            "*(texColor.r - " SK_DistanceFieldThreshold ");");
#ifdef SK_GAMMA_APPLY_TO_A8
    ...
    if (isAliased) {
      ...
      fragBuilder->codeAppend("half val = saturate((distance + afwidth) / (2.0 * afwidth));");
    } else {
...
GrDistanceFieldA8TextGeoProc::GrDistanceFieldA8TextGeoProc(
    const GrShaderCaps& caps, const GrSurfaceProxyView* views, int numViews, GrSamplerState params,
#ifdef SK_GAMMA_APPLY_TO_A8
...
void GrDistanceFieldA8TextGeoProc::addNewViews(
...
void GrDistanceFieldA8TextGeoProc::addToKey(const GrShaderCaps& caps, skgpu::KeyBuilder* b) const {
  uint32_t key = 0;
  key |= fFlags;
...
class GrDistanceFieldPathGeoProc::Impl : public ProgramImpl {
 ...
 private:
  void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
    ...
    GrGLSLVarying uv, texIdx, st;
    append_index_uv_varyings(
        args, dfPathEffect.numTextureSamplers(), dfPathEffect.fInTextureCoords.name(),
        atlasDimensionsInvName, &uv, &texIdx, &st);

    // setup pass through color
    fragBuilder->codeAppendf("half4 %s;", args.fOutputColor);
    varyingHandler->addPassThroughAttribute(dfPathEffect.fInColor.asShaderVar(), args.fOutputColor);

    if (dfPathEffect.fMatrix.hasPerspective()) {
      // Setup position (output position is transformed, local coords are pass through)
      WriteOutputPosition(
          vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, dfPathEffect.fInPosition.name(),
          dfPathEffect.fMatrix, &fMatrixUniform);
      ...
      WriteLocalCoord(
          vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs,
          dfPathEffect.fInPosition.asShaderVar(), dfPathEffect.fMatrix, &fMatrixUniform);
    }

    // Use highp to work around aliasing issues
    fragBuilder->codeAppendf("float2 uv = %s;", uv.fsIn());
    fragBuilder->codeAppend("half4 texColor;");
    append_multitexture_lookup(args, dfPathEffect.numTextureSamplers(), texIdx, "uv", "texColor");
...
GrDistanceFieldPathGeoProc::GrDistanceFieldPathGeoProc(
    ...
    const GrSurfaceProxyView* views, int numViews, GrSamplerState params, uint32_t flags)
...
void GrDistanceFieldPathGeoProc::addNewViews(
    const GrSurfaceProxyView* views, int numViews, GrSamplerState params) {
  SkASSERT(numViews <= kMaxTextures);
  // Just to make sure we don't try to add too many proxies
  numViews = std::min(numViews, kMaxTextures);
...
void GrDistanceFieldPathGeoProc::addToKey(const GrShaderCaps& caps, skgpu::KeyBuilder* b) const {
  uint32_t key = fFlags;
...
GrGeometryProcessor* GrDistanceFieldPathGeoProc::TestCreate(GrProcessorTestData* d) {
  ...
  return GrDistanceFieldPathGeoProc::Make(
      d->allocator(), *d->caps()->shaderCaps(), localMatrix, wideColor, &view, 1, samplerState,
      flags);
}
#endif
...
class GrDistanceFieldLCDTextGeoProc::Impl : public ProgramImpl {
 ...
 private:
  void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
    ...
    WriteLocalCoord(
        vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs,
        dfTexEffect.fInPosition.asShaderVar(), dfTexEffect.fLocalMatrix, &fLocalMatrixUniform);
    ...
    if (isUniformScale) {
      ...
      fragBuilder->codeAppendf("half2 offset = half2(half(st_grad_len*%s), 0.0);", delta.fsIn());
    } else if (isSimilarity) {
    ...
    append_multitexture_lookup(
        args, dfTexEffect.numTextureSamplers(), texIdx, "uv_adjusted", "texColor");
    ...
    const char* distanceAdjustUniName = nullptr;
    fDistanceAdjustUni = uniformHandler->addUniform(
        nullptr, kFragment_GrShaderFlag, SkSLType::kHalf3, "DistanceAdjust",
...
GrDistanceFieldLCDTextGeoProc::GrDistanceFieldLCDTextGeoProc(
    const GrShaderCaps& caps, const GrSurfaceProxyView* views, int numViews, GrSamplerState params,
    DistanceAdjust distanceAdjust, uint32_t flags, const SkMatrix& localMatrix)
...
void GrDistanceFieldLCDTextGeoProc::addNewViews(
    const GrSurfaceProxyView* views, int numViews, GrSamplerState params) {
  SkASSERT(numViews <= kMaxTextures);
  // Just to make sure we don't try to add too many proxies
  numViews = std::min(numViews, kMaxTextures);
...
void GrDistanceFieldLCDTextGeoProc::addToKey(const GrShaderCaps& caps, skgpu::KeyBuilder* b) const {
  uint32_t key = 0;
