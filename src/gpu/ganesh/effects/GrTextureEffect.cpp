...
struct GrTextureEffect::Sampling {
  ...
  Sampling(
      ...
      const float border[4], bool alwaysUseShaderTileMode, const GrCaps&,
...
GrTextureEffect::Sampling::Sampling(
    ...
    const float border[4], bool alwaysUseShaderTileMode, const GrCaps& caps,
  ...
  auto canDoWrapInHW = [&](int size, Wrap wrap) {
    ...
    if (type != GrTextureType::k2D && !(wrap == Wrap::kClamp || wrap == Wrap::kClampToBorder)) {
      return false;
    }
    return true;
  };
  ...
  if (aniso) {
    bool anisoSubset =
        !proxy.backingStoreBoundsRect().contains(subset) && (!domain || !subset.contains(*domain));
    ...
    if (needsShaderWrap || anisoSubset) {
      MipmapMode newMM = proxy.asTextureProxy()->mipmapped() == GrMipmapped::kYes
                             ...
                             : MipmapMode::kNone;
      sampler =
          GrSamplerState(sampler.wrapModeX(), sampler.wrapModeY(), SkFilterMode::kLinear, newMM);
      aniso = false;
    }
  }
  ...
  fHWSampler =
      aniso ? GrSamplerState::Aniso(
                  x.fHWWrap, y.fHWWrap, sampler.maxAniso(), proxy.asTextureProxy()->mipmapped())
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::Make(
  ...
  std::unique_ptr<GrFragmentProcessor> te(
      new GrTextureEffect(std::move(view), alphaType, sampling));
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::Make(
  ...
  Sampling sampling(
      *view.proxy(), sampler, SkRect::Make(view.proxy()->dimensions()), nullptr, border, false,
      caps);
  std::unique_ptr<GrFragmentProcessor> te(
      new GrTextureEffect(std::move(view), alphaType, sampling));
  return GrMatrixEffect::Make(matrix, std::move(te));
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::MakeSubset(
    GrSurfaceProxyView view, SkAlphaType alphaType, const SkMatrix& matrix, GrSamplerState sampler,
    const SkRect& subset, const GrCaps& caps, const float border[4], bool alwaysUseShaderTileMode) {
...
void GrTextureEffect::Impl::emitCode(EmitArgs& args) {
  ...
  if (te.fShaderModes[0] == ShaderMode::kNone && te.fShaderModes[1] == ShaderMode::kNone) {
    ...
    bool unormCoordsRequiredForShaderMode =
        ShaderModeRequiresUnormCoord(m[0]) || ShaderModeRequiresUnormCoord(m[1]);
    ...
    if (unormCoordsRequiredForShaderMode && sampleCoordsMustBeNormalized) {
      ...
      // always use?
      fIDimsUni = args.fUniformHandler->addUniform(
          &te, kFragment_GrShaderFlag, SkSLType::kFloat2, "idims", &idims);
    }
