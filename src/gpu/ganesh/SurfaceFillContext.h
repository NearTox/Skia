...
class SurfaceFillContext : public SurfaceContext {
 public:
   ...
   */
  void fillRectWithFP(
      const SkIRect& dstRect, const SkMatrix& localMatrix, std::unique_ptr<GrFragmentProcessor>);
   ...
   * respectively.
   */
  virtual bool blitTexture(
      GrSurfaceProxyView, const SkIRect& srcRect, const SkIPoint& dstPoint) = 0;
 ...
 protected:
  SurfaceFillContext(
      ...
      : SurfaceContext(rContext, std::move(readView), colorInfo), fWriteView(std::move(writeView)) {
 ...
 private:
  virtual void internalClear(
      const SkIRect* scissor, std::array<float, 4> color, bool upgradePartialToFull = false) = 0;
