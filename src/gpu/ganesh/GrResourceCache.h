...
class GrResourceCache {
 public:
  GrResourceCache(
      skgpu::SingleOwner* owner, GrDirectContext::DirectContextID owningContextID,
      uint32_t familyID);
  ~GrResourceCache();
   ...
   */
  template <typename T>
  static std::enable_if_t<std::is_base_of_v<GrGpuResource, T>, void> ReturnResourceFromThread(
      sk_sp<T>&& resource, GrDirectContext::DirectContextID id) {
   ...
   */
  bool hasUniqueKey(const skgpu::UniqueKey& key) const { return SkToBool(fUniqueHash.find(key)); }
  ...
  void purgeResourcesNotUsedSince(
      GrStdSteadyClock::time_point purgeTime, bool scratchResourcesOnly = false) {
...
#  endif
  ...
  class UnrefResourceMessage {
   ...
   private:
    ...
    using Bus = SkMessageBus<
        UnrefResourceMessage, GrDirectContext::DirectContextID,
        /*AllowCopyableMessage=*/false>;

    UnrefResourceMessage(
        sk_sp<GrGpuResource>&& resource, GrDirectContext::DirectContextID recipient)
...
class GrResourceCache::ResourceAccess {
 private:
   ...
   */
  void notifyARefCntReachedZero(GrGpuResource* resource, GrGpuResource::LastRemovedRef removedRef) {
...
static inline bool SkShouldPostMessageToBus(
    const GrResourceCache::UnrefResourceMessage& msg,
