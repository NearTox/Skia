...
class GrXferProcessor : public GrProcessor, public GrNonAtomicRef<GrXferProcessor> {
 public:
   ...
   */
  void addToKey(
      const GrShaderCaps&, skgpu::KeyBuilder*, const GrSurfaceOrigin* originIfDstTexture,
...
class GrXPFactory {
 public:
  ...
  static sk_sp<const GrXferProcessor> MakeXferProcessor(
      const GrXPFactory*, const GrProcessorAnalysisColor&, GrProcessorAnalysisCoverage,
      const GrCaps& caps, GrClampType);
...
class GrXferProcessor::ProgramImpl {
 ...
 private:
   ...
   */
  virtual void emitBlendCodeForDstRead(
      GrGLSLXPFragmentBuilder*, GrGLSLUniformHandler*, const char* srcColor,
      const char* srcCoverage, const char* dstColor, const char* outColor,
      const char* outColorSecondary, const GrXferProcessor&) {
    SK_ABORT("emitBlendCodeForDstRead not implemented.");
  }

  virtual void emitWriteSwizzle(
      GrGLSLXPFragmentBuilder*, const skgpu::Swizzle&, const char* outColor,
