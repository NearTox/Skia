...
class GrInnerFanTriangulator : private GrTriangulator {
 public:
  ...
  GrInnerFanTriangulator(const SkPath& path, SkArenaAlloc* alloc) : GrTriangulator(path, alloc) {
    fPreserveCollinearVertices = true;
    fCollectBreadcrumbTriangles = true;
  }

  int pathToTriangles(
      GrEagerVertexAllocator* vertexAlloc, BreadcrumbTriangleList* breadcrumbList, bool* isLinear) {
  ...
  Poly* pathToPolys(BreadcrumbTriangleList* breadcrumbList, bool* isLinear) {
    auto [polys, success] = this->GrTriangulator::pathToPolys(0, SkRect::MakeEmpty(), isLinear);
    if (!success) {
      return nullptr;
    }
  ...
  int polysToTriangles(
      Poly* polys, GrEagerVertexAllocator* vertexAlloc,
