...
void GrAATriangulator::Event::apply(
    VertexList* mesh, const Comparator& c, EventList* events, GrAATriangulator* triangulator) {
  if (!fEdge) {
    return;
  }
...
bool GrAATriangulator::collapseOverlapRegions(
    VertexList* mesh, const Comparator& c, EventComparator comp) {
...
void GrAATriangulator::strokeBoundary(
    EdgeList* boundary, VertexList* innerMesh, const Comparator& c) {
  TESS_LOG("\nstroking boundary\n");
...
void GrAATriangulator::extractBoundaries(
    const VertexList& inMesh, VertexList* innerVertices, const Comparator& c) {
