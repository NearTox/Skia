...
static bool perp_intersect(
    const SkPoint& p0, const SkPoint& n0, const SkPoint& p1, const SkPoint& perp, SkScalar* t) {
  const SkPoint v = p1 - p0;
...
static bool points_are_colinear_and_b_is_middle(
    const SkPoint& a, const SkPoint& b, const SkPoint& c, float* accumError) {
...
int GrAAConvexTessellator::addPt(
    const SkPoint& pt, SkScalar depth, SkScalar coverage, bool movable, CurveState curve) {
...
bool GrAAConvexTessellator::createInsetRings(
    Ring& previousRing, SkScalar initialDepth, SkScalar initialCoverage, SkScalar targetDepth,
  ...
  for (i = 0; i < kMaxNumRings; ++i) {
    ...
    bool done = this->createInsetRing(
        *currentRing, nextRing, initialDepth, initialCoverage, targetDepth, targetCoverage, i == 0);
...
bool GrAAConvexTessellator::tessellate(const SkMatrix& m, const SkPath& path) {
  ...
  if (SkStrokeRec::kStrokeAndFill_Style == fStyle) {
    ...
    this->createOuterRing(
        fInitialRing, effectiveStrokeWidth / 2 + kAntialiasingRadius, 0.0, &outerStrokeAndAARing);
  ...
  if (SkStrokeRec::kStroke_Style == fStyle) {
    ...
    this->createOuterRing(
        fInitialRing, effectiveStrokeWidth / 2 - kAntialiasingRadius, coverage, &outerStrokeRing);
...
bool GrAAConvexTessellator::extractFromPath(const SkMatrix& m, const SkPath& path) {
  ...
  while (!noRemovalsToDo && this->numPts() >= 3) {
    if (points_are_colinear_and_b_is_middle(
            fPts[fPts.count() - 2], fPts.top(), fPts[0], &fAccumLinearError)) {
      this->popLastPt();
    } else if (points_are_colinear_and_b_is_middle(
                   fPts.top(), fPts[0], fPts[1], &fAccumLinearError)) {
    ...
    if (perp2Idx != perp1Idx) {
      if (curve == kCurve_CurveState) {
        ...
        switch (fJoin) {
          ...
          case SkPaint::Join::kBevel_Join: this->addTri(originalIdx, perp1Idx, perp2Idx); break;
          default:
...
static SkScalar compute_coverage(
  ...
  SkScalar result =
      (depth - initialDepth) / (targetDepth - initialDepth) * (targetCoverage - initialCoverage) +
      initialCoverage;
  return SkTPin(result, 0.0f, 1.0f);
}

// return true when processing is complete
bool GrAAConvexTessellator::createInsetRing(
  ...
  for (int cur = 0; cur < lastRing.numPts(); ++cur) {
    ...
    bool result = intersect(
        this->point(lastRing.index(cur)), lastRing.bisector(cur), this->point(lastRing.index(next)),
        lastRing.bisector(next), &t);
  ...
  if (!this->computePtAlongBisector(
          lastRing.index(0), lastRing.bisector(0), lastRing.origEdgeID(0), depth, &newPt)) {
    this->terminate(lastRing);
    return true;
  }
  dst[0] = fCandidateVerts.addNewPt(
      newPt, lastRing.index(0), lastRing.origEdgeID(0), !this->movable(lastRing.index(0)));

  // Handle the middle points (who only compare with the prior point)
  for (int cur = 1; cur < lastRing.numPts() - 1; ++cur) {
    if (!this->computePtAlongBisector(
            lastRing.index(cur), lastRing.bisector(cur), lastRing.origEdgeID(cur), depth, &newPt)) {
      this->terminate(lastRing);
      return true;
    }
    if (!duplicate_pt(newPt, fCandidateVerts.lastPoint())) {
      dst[cur] = fCandidateVerts.addNewPt(
          newPt, lastRing.index(cur), lastRing.origEdgeID(cur),
  ...
  if (!dupPrev && !dupNext) {
    dst[cur] = fCandidateVerts.addNewPt(
        newPt, lastRing.index(cur), lastRing.origEdgeID(cur), !this->movable(lastRing.index(cur)));
  ...
  for (int i = 0; i < fCandidateVerts.numPts(); ++i) {
    ...
    if (fCandidateVerts.needsToBeNew(i) || forceNew) {
      ...
      SkScalar coverage =
          compute_coverage(depth, initialDepth, initialCoverage, targetDepth, targetCoverage);
      newIdx = this->addPt(
          fCandidateVerts.point(i), depth, coverage, fCandidateVerts.originatingIdx(i) != -1,
          kSharp_CurveState);
    } else {
      SkASSERT(fCandidateVerts.originatingIdx(i) != -1);
      this->updatePt(
          fCandidateVerts.originatingIdx(i), fCandidateVerts.point(i), depth, targetCoverage);
...
void GrAAConvexTessellator::lineTo(const SkPoint& p, CurveState curve) {
  ...
  if (this->numPts() >= 2 && points_are_colinear_and_b_is_middle(
                                 fPts[fPts.count() - 2], fPts.top(), p, &fAccumLinearError)) {
...
static void draw_arrow(
    SkCanvas* canvas, const SkPoint& p, const SkPoint& n, SkScalar len, SkColor color) {
  SkPaint paint;
  paint.setColor(color);
  paint.setStrokeWidth(kArrowStrokeWidth);
  paint.setStyle(SkPaint::kStroke_Style);

  canvas->drawLine(p.fX, p.fY, p.fX + len * n.fX, p.fY + len * n.fY, paint);
}
...
void GrAAConvexTessellator::Ring::draw(SkCanvas* canvas, const GrAAConvexTessellator& tess) const {
  ...
  for (int cur = 0; cur < fPts.count(); ++cur) {
    ...
    draw_line(canvas, tess.point(fPts[cur].fIndex), tess.point(fPts[next].fIndex), SK_ColorGREEN);
    ...
    if (fPts.count()) {
      draw_arrow(
          canvas, tess.point(fPts[cur].fIndex), fPts[cur].fBisector, kArrowLength, SK_ColorBLUE);
    }
  }
}
...
void GrAAConvexTessellator::draw(SkCanvas* canvas) const {
  for (int i = 0; i < fIndices.count(); i += 3) {
    ...
    draw_line(
        canvas, this->point(this->fIndices[i]), this->point(this->fIndices[i + 1]), SK_ColorBLACK);
    draw_line(
        canvas, this->point(this->fIndices[i + 1]), this->point(this->fIndices[i + 2]),
        SK_ColorBLACK);
    draw_line(
        canvas, this->point(this->fIndices[i + 2]), this->point(this->fIndices[i]), SK_ColorBLACK);
  }
  ...
  for (int i = 0; i < this->numPts(); ++i) {
    draw_point(
        canvas, this->point(i), 0.5f + (this->depth(i) / (2 * kAntialiasingRadius)),
        !this->movable(i));
    ...
    SkString num;
    num.printf("%d", i);
    canvas->drawString(num, this->point(i).fX, this->point(i).fY + (kPointRadius / 2.0f), paint);
  }
}
