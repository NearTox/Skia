...
static inline skgpu::VertexWriter emit_vertex(
    Vertex* v, bool emitCoverage, skgpu::VertexWriter data) {
  data << v->fPoint;
...
static skgpu::VertexWriter emit_triangle(
    Vertex* v0, Vertex* v1, Vertex* v2, bool emitCoverage, skgpu::VertexWriter data) {
...
static bool edge_line_needs_recursion(const SkPoint& p0, const SkPoint& p1) {
  ...
  int expDiffX = std::abs(
      (std::abs(p0.fX) < 1.f ? 0 : std::ilogbf(p0.fX)) -
      (std::abs(p1.fX) < 1.f ? 0 : std::ilogbf(p1.fX)));
  int expDiffY = std::abs(
      (std::abs(p0.fY) < 1.f ? 0 : std::ilogbf(p0.fY)) -
...
static bool recursive_edge_intersect(
    const Line& u, SkPoint u0, SkPoint u1, const Line& v, SkPoint v0, SkPoint v1, SkPoint* p,
    double* s, double* t) {
  ...
  *t = tNumer / denom;
  ...
  if (!uNeedsSplit && !vNeedsSplit) {
    ...
    if (uNeedsSplit) {
      SkPoint uM = {(float)(0.5 * u0.fX + 0.5 * u1.fX), (float)(0.5 * u0.fY + 0.5 * u1.fY)};
      sScale = 0.5;
      if (*s >= 0.5) {
        u0 = uM;
        sShift = 0.5;
    ...
    if (vNeedsSplit) {
      SkPoint vM = {(float)(0.5 * v0.fX + 0.5 * v1.fX), (float)(0.5 * v0.fY + 0.5 * v1.fY)};
      tScale = 0.5;
      if (*t >= 0.5) {
        v0 = vM;
        tShift = 0.5;
...
bool GrTriangulator::Edge::intersect(const Edge& other, SkPoint* p, uint8_t* alpha) const {
  ...
  const bool intersects = recursive_edge_intersect(
      fLine, fTop->fPoint, fBottom->fPoint, other.fLine, other.fTop->fPoint, other.fBottom->fPoint,
      p, &s, &t);
  if (!intersects) {
    return false;
  }
  ...
  if (alpha) {
    if (fType == EdgeType::kInner || other.fType == EdgeType::kInner) {
      ...
      *alpha = std::max(
          (1.0 - s) * fTop->fAlpha + s * fBottom->fAlpha,
...
skgpu::VertexWriter GrTriangulator::emitMonotonePoly(
    const MonotonePoly* monotonePoly, skgpu::VertexWriter data) const {
...
static bool coincident(const SkPoint& a, const SkPoint& b) { return a == b; }
...
void GrTriangulator::appendQuadraticToContour(
    const SkPoint pts[3], SkScalar toleranceSqd, VertexList* contour) const {
  SkQuadCoeff quad(pts);
...
void GrTriangulator::pathToContours(
    float tolerance, const SkRect& clipBounds, VertexList* contours, bool* isLinear) const {
    ...
    switch (verb) {
      ...
      case SkPath::kCubic_Verb: {
        *isLinear = false;
        ...
        this->generateCubicPoints(
            pts[0], pts[1], pts[2], pts[3], toleranceSqd, contour, pointsLeft);
        break;
      }
...
static void rewind_if_necessary(
    Edge* edge, EdgeList* activeEdges, Vertex** current, const Comparator& c) {
...
void GrTriangulator::setTop(
    Edge* edge, Vertex* v, EdgeList* activeEdges, Vertex** current, const Comparator& c) const {
  remove_edge_below(edge);
  if (fCollectBreadcrumbTriangles) {
    fBreadcrumbList.append(
        fAlloc, edge->fTop->fPoint, edge->fBottom->fPoint, v->fPoint, edge->fWinding);
  }
  edge->fTop = v;
  edge->recompute();
...
void GrTriangulator::mergeEdgesAbove(
    Edge* edge, Edge* other, EdgeList* activeEdges, Vertex** current, const Comparator& c) const {
...
void GrTriangulator::mergeEdgesBelow(
    Edge* edge, Edge* other, EdgeList* activeEdges, Vertex** current, const Comparator& c) const {
...
void GrTriangulator::mergeCollinearEdges(
    Edge* edge, EdgeList* activeEdges, Vertex** current, const Comparator& c) const {
  for (;;) {
...
bool GrTriangulator::intersectEdgePair(
...
void GrTriangulator::mergeVertices(
    Vertex* src, Vertex* dst, VertexList* mesh, const Comparator& c) const {
  TESS_LOG(
...
Vertex* GrTriangulator::makeSortedVertex(
    const SkPoint& p, uint8_t alpha, VertexList* mesh, Vertex* reference,
...
static SkPoint clamp(SkPoint p, SkPoint min, SkPoint max, const Comparator& c) {
  if (c.fDirection == Comparator::Direction::kHorizontal) {
    ...
    return {
        SkTPin(p.fX, min.fX, max.fX),
        min.fY < max.fY ? SkTPin(p.fY, min.fY, max.fY) : SkTPin(p.fY, max.fY, min.fY)};
  } else {
    // And with vertical sorting, we know Y's relation but not necessarily X's.
    return {
        min.fX < max.fX ? SkTPin(p.fX, min.fX, max.fX) : SkTPin(p.fX, max.fX, min.fX),
...
bool GrTriangulator::checkForIntersection(
...
void GrTriangulator::buildEdges(
    VertexList* contours, int contourCnt, VertexList* mesh, const Comparator& c) {
  for (VertexList* contour = contours; contourCnt > 0; --contourCnt, ++contour) {
    Vertex* prev = contour->fTail;
    for (Vertex* v = contour->fHead; v;) {
      Vertex* next = v->fNext;
...
void GrTriangulator::SortedMerge(
    VertexList* front, VertexList* back, VertexList* result, const Comparator& c) {
  if (c.fDirection == Comparator::Direction::kHorizontal) {
    ...
    do {
      ...
      if (v->fFirstEdgeBelow) {
        for (Edge* edge = v->fFirstEdgeBelow; edge; edge = edge->fNextEdgeBelow) {
          if (this->checkForIntersection(leftEnclosingEdge, edge, &activeEdges, &v, mesh, c) ||
              this->checkForIntersection(edge, rightEnclosingEdge, &activeEdges, &v, mesh, c)) {
        ...
        if (leftPoly && rightPoly) {
          if (leftPoly == rightPoly) {
            if (leftPoly->fTail && leftPoly->fTail->fSide == kLeft_Side) {
              leftPoly = this->makePoly(&polys, leftPoly->lastVertex(), leftPoly->fWinding);
              leftEnclosingEdge->fRightPoly = leftPoly;
            } else {
              rightPoly = this->makePoly(&polys, rightPoly->lastVertex(), rightPoly->fWinding);
...
void GrTriangulator::contoursToMesh(
    VertexList* contours, int contourCnt, VertexList* mesh, const Comparator& c) {
#if TRIANGULATOR_LOGGING
...
skgpu::VertexWriter GrTriangulator::polysToTriangles(
    Poly* polys, SkPathFillType overrideFillType, skgpu::VertexWriter data) const {
...
std::tuple<Poly*, bool> GrTriangulator::pathToPolys(
    float tolerance, const SkRect& clipBounds, bool* isLinear) {
