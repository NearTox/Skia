...
static bool crop_rect_edge(
    const SkRect& clipDevRect, int v0, int v1, int v2, int v3, float x[4], float y[4], float lx[4],
    float ly[4], float lw[4]) {
  SkASSERT(v0 >= 0 && v0 < 4);
  SkASSERT(v1 >= 0 && v1 < 4);
  SkASSERT(v2 >= 0 && v2 < 4);
  SkASSERT(v3 >= 0 && v3 < 4);
...
static GrQuadAAFlags crop_rect(
    const SkRect& clipDevRect, float x[4], float y[4], float lx[4], float ly[4], float lw[4]) {
...
static bool barycentric_coords(
    float x0, float y0, float x1, float y1, float x2, float y2, const float4& testX,
    const float4& testY, float4* u, float4* v, float4* w) {
...
void ResolveAAType(
    GrAAType requestedAAType, GrQuadAAFlags requestedEdgeFlags, const GrQuad& quad,
  ...
  *outEdgeFlags = requestedEdgeFlags;
  ...
  switch (requestedAAType) {
    ...
    case GrAAType::kCoverage:
      if (requestedEdgeFlags == GrQuadAAFlags::kNone) {
        ...
        *outAAType = GrAAType::kNone;
          ...
          quad.quadType() == GrQuad::Type::kAxisAligned &&
    ...
    case GrAAType::kNone: *outEdgeFlags = GrQuadAAFlags::kNone; break;
    case GrAAType::kMSAA: *outEdgeFlags = GrQuadAAFlags::kAll; break;
  }
}
...
int ClipToW0(DrawQuad* quad, DrawQuad* extraVertices) {
  ...
  int clipCount =
      (validW[0] ? 0 : 1) + (validW[1] ? 0 : 1) + (validW[2] ? 0 : 1) + (validW[3] ? 0 : 1);
  ...
  if (clipCount != 1) {
    ...
    v.fX = if_then_else(
        validW, v.fX,
        if_then_else(
            (!ccwValid) & (!cwValid), next_ccw(clip.fX),
            if_then_else(ccwValid, clip.fX, /* cwValid */ next_cw(clip.fX))));
    v.fY = if_then_else(
        validW, v.fY,
        if_then_else(
            (!ccwValid) & (!cwValid), next_ccw(clip.fY),
    ...
    v.asGrQuads(&quad->fDevice, GrQuad::Type::kPerspective, &quad->fLocal, localType);
    return 1;
  } else {
    ...
    v2.fX = if_then_else(
        (!validW) | (!ccwValid), clip.fX, if_then_else(cwValid, next_cw(mid.fX), v.fX));
    v2.fY = if_then_else(
        (!validW) | (!ccwValid), clip.fY, if_then_else(cwValid, next_cw(mid.fY), v.fY));
    v2.fW = if_then_else(
        (!validW) | (!ccwValid), clip.fW, if_then_else(cwValid, next_cw(mid.fW), v.fW));

    v2.fU = if_then_else(
        (!validW) | (!ccwValid), clip.fU, if_then_else(cwValid, next_cw(mid.fU), v.fU));
    v2.fV = if_then_else(
        (!validW) | (!ccwValid), clip.fV, if_then_else(cwValid, next_cw(mid.fV), v.fV));
    v2.fR = if_then_else(
        (!validW) | (!ccwValid), clip.fR, if_then_else(cwValid, next_cw(mid.fR), v.fR));
    // The non-AA edge for this quad is the opposite of the clipped vertex's edge
    GrQuadAAFlags v2EdgeFlag =
        (!validW[0] ? GrQuadAAFlags::kRight :             // left clipped -> right
    ...
    // inserted mid point.
    v.fX = if_then_else(!validW, next_cw(clip.fX), if_then_else(!cwValid, mid.fX, v.fX));
    v.fY = if_then_else(!validW, next_cw(clip.fY), if_then_else(!cwValid, mid.fY, v.fY));
    v.fW = if_then_else(!validW, clip.fW, if_then_else(!cwValid, mid.fW, v.fW));

    v.fU = if_then_else(!validW, next_cw(clip.fU), if_then_else(!cwValid, mid.fU, v.fU));
    v.fV = if_then_else(!validW, next_cw(clip.fV), if_then_else(!cwValid, mid.fV, v.fV));
    v.fR = if_then_else(!validW, next_cw(clip.fR), if_then_else(!cwValid, mid.fR, v.fR));
    // The non-AA edge for this quad is the clipped vertex's edge
    GrQuadAAFlags v1EdgeFlag =
        (!validW[0] ? GrQuadAAFlags::kLeft
                    : (!validW[1] ? GrQuadAAFlags::kBottom
                                  : (!validW[2] ? GrQuadAAFlags::kTop : GrQuadAAFlags::kRight)));

    v.asGrQuads(&quad->fDevice, GrQuad::Type::kPerspective, &quad->fLocal, localType);
    quad->fEdgeFlags &= ~v1EdgeFlag;

    v2.asGrQuads(
        &extraVertices->fDevice, GrQuad::Type::kPerspective, &extraVertices->fLocal, localType);
  ...
  if (quad->fDevice.quadType() == GrQuad::Type::kAxisAligned) {
    ...
    if (computeLocal) {
      if (is_simple_rect(quad->fDevice) && is_simple_rect(quad->fLocal)) {
        clippedEdges = crop_simple_rect(
            cropRect, quad->fDevice.xs(), quad->fDevice.ys(), quad->fLocal.xs(), quad->fLocal.ys());
      } else {
        clippedEdges = crop_rect(
            cropRect, quad->fDevice.xs(), quad->fDevice.ys(), quad->fLocal.xs(), quad->fLocal.ys(),
            quad->fLocal.ws());
      }
    } else {
      if (is_simple_rect(quad->fDevice)) {
        clippedEdges =
            crop_simple_rect(cropRect, quad->fDevice.xs(), quad->fDevice.ys(), nullptr, nullptr);
      } else {
        clippedEdges =
            crop_rect(cropRect, quad->fDevice.xs(), quad->fDevice.ys(), nullptr, nullptr, nullptr);
      }
    }
...
float4 TessellationHelper::EdgeEquations::estimateCoverage(
    const float4& x2d, const float4& y2d) const {
...
int TessellationHelper::EdgeEquations::computeDegenerateQuad(
    const float4& signedEdgeDistances, float4* x2d, float4* y2d, mask4* aaMask) const {
  ...
  *aaMask = signedEdgeDistances != 0.f;
  ...
  // p(3) to e0,e2
  float4 dists1 = px * skvx::shuffle<3, 3, 0, 0>(fA) + py * skvx::shuffle<3, 3, 0, 0>(fB) +
                  skvx::shuffle<3, 3, 0, 0>(oc);
  float4 dists2 = px * skvx::shuffle<1, 2, 1, 2>(fA) + py * skvx::shuffle<1, 2, 1, 2>(fB) +
  ...
  if (!any(d1Or2)) {
    ...
    *aaMask = any(*aaMask);
    ...
    V2f ex = (skvx::shuffle<0, 1>(fB) * skvx::shuffle<3, 2>(oc) -
              skvx::shuffle<0, 1>(oc) * skvx::shuffle<3, 2>(fB)) /
             eDenom;
    V2f ey = (skvx::shuffle<0, 1>(oc) * skvx::shuffle<3, 2>(fA) -
              skvx::shuffle<0, 1>(fA) * skvx::shuffle<3, 2>(oc)) /
             eDenom;
...
void TessellationHelper::OutsetRequest::reset(
  ...
  if (quadType <= GrQuad::Type::kRectilinear) {
    ...
    fInsetDegenerate = (widthChange > 0.f && edgeVectors.fInvLengths[1] > 1.f / widthChange) ||
...
void TessellationHelper::Vertices::moveAlong(
    const EdgeVectors& edgeVectors, const float4& signedEdgeDistances) {
  ...
  SkASSERT(
      all(abs(edgeVectors.fCosTheta) < 0.9f) ||
...
void TessellationHelper::Vertices::moveTo(const float4& x2d, const float4& y2d, const mask4& mask) {
  ...
  if (all(mask)) {
    ...
    denom = if_then_else(
        aMask,
        if_then_else(
            bMask, c1x * c2y - c2x * c1y,    /* A & B   */
            if_then_else(useC1x, c1x, c1y)), /* A & !B  */
        if_then_else(
            bMask, if_then_else(useC2x, c2x, c2y), /* !A & B  */
            float4(1.f)));                         /* !A & !B */

    a = if_then_else(
            aMask,
            if_then_else(
                bMask, c2x * c3y - c3x * c2y,      /* A & B   */
...
float4 TessellationHelper::inset(
    const skvx::Vec<4, float>& edgeDistances, GrQuad* deviceInset, GrQuad* localInset) {
  ...
  if (vertexCount < 3) {
    ...
    return this->getEdgeEquations().estimateCoverage(inset.fX / inset.fW, inset.fY / inset.fW);
  } else {
    return 1.f;
  }
}

void TessellationHelper::outset(
    const skvx::Vec<4, float>& edgeDistances, GrQuad* deviceOutset, GrQuad* localOutset) {
  SkASSERT(fVerticesValid);
...
void TessellationHelper::getEdgeEquations(
    skvx::Vec<4, float>* a, skvx::Vec<4, float>* b, skvx::Vec<4, float>* c) {
  SkASSERT(a && b && c);
  SkASSERT(fVerticesValid);
...
void TessellationHelper::adjustVertices(
...
int TessellationHelper::adjustDegenerateVertices(
    const skvx::Vec<4, float>& signedEdgeDistances, Vertices* vertices) {
  SkASSERT(vertices);
  SkASSERT(vertices->fUVRCount == 0 || vertices->fUVRCount == 2 || vertices->fUVRCount == 3);
