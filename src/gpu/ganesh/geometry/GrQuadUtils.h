...
class TessellationHelper {
 ...
 private:
  ...
  struct EdgeVectors {
    ...
    void reset(
        const skvx::float4& xs, const skvx::float4& ys, const skvx::float4& ws,
  ...
  struct EdgeEquations {
    ...
    skvx::float4 estimateCoverage(const skvx::float4& x2d, const skvx::float4& y2d) const;
    ...
    int computeDegenerateQuad(
        const skvx::float4& signedEdgeDistances, skvx::float4* x2d, skvx::float4* y2d,
  ...
  struct OutsetRequest {
    ...
    void reset(
        const EdgeVectors& edgeVectors, GrQuad::Type quadType, const skvx::float4& edgeDistances);
  };
  ...
  struct Vertices {
    ...
    // numerically sound.
    void moveAlong(const EdgeVectors& edgeVectors, const skvx::float4& signedEdgeDistances);
    ...
    void moveTo(
        const skvx::float4& x2d, const skvx::float4& y2d, const skvx::Vec<4, int32_t>& mask);
  };
  ...
  int adjustDegenerateVertices(const skvx::float4& signedEdgeDistances, Vertices* vertices);
