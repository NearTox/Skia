...
class GrCaps : public SkCapabilities {
 public:
  ...
  bool anisoSupport() const noexcept { return fAnisoSupport; }
  ...
  bool msaaResolvesAutomatically() const noexcept { return fMSAAResolvesAutomatically; }
  ...
  bool preferDiscardableMSAAAttachment() const noexcept { return fPreferDiscardableMSAAAttachment; }
  ...
  bool isAdvancedBlendEquationDisabled(skgpu::BlendEquation equation) const noexcept {
  ...
  bool shouldCollapseSrcOverToSrcWhenAble() const noexcept {
  ...
  // resources.
  bool mustSyncGpuDuringAbandon() const noexcept { return fMustSyncGpuDuringAbandon; }

  // Shortcut for shaderCaps()->fReducedShaderMode.
  bool reducedShaderMode() const noexcept { return this->shaderCaps()->fReducedShaderMode; }
  ...
  uint32_t maxPushConstantsSize() const noexcept { return fMaxPushConstantsSize; }

  size_t transferBufferAlignment() const noexcept { return fTransferBufferAlignment; }
  ...
  virtual bool isFormatAsColorTypeRenderable(
      GrColorType ct, const GrBackendFormat& format, int sampleCount = 1) const = 0;
   ...
   */
  virtual SupportedWrite supportedWritePixelsColorType(
   ...
   */
  bool writePixelsRowBytesSupport() const noexcept { return fWritePixelsRowBytesSupport; }

  /**
   * Does GrGpu::transferPixelsTo() support a src buffer where the row bytes is not equal to
   * bpp * w?
   */
  bool transferPixelsToRowBytesSupport() const noexcept { return fTransferPixelsToRowBytesSupport; }

  /**
   * Does GrGpu::readPixels() support a dst buffer where the row bytes is not equal to bpp * w?
   */
  bool readPixelsRowBytesSupport() const noexcept { return fReadPixelsRowBytesSupport; }

  bool transferFromSurfaceToBufferSupport() const noexcept {
    return fTransferFromSurfaceToBufferSupport;
  }
  bool transferFromBufferToTextureSupport() const noexcept {
    return fTransferFromBufferToTextureSupport;
  }
  ...
  bool avoidLargeIndexBufferDraws() const noexcept { return fAvoidLargeIndexBufferDraws; }
  ...
  bool disableTessellationPathRenderer() const noexcept { return fDisableTessellationPathRenderer; }
  ...
  virtual DstCopyRestrictions getDstCopyRestrictions(
  ...
  virtual void addExtraSamplerKey(
      skgpu::KeyBuilder*, GrSamplerState, const GrBackendFormat&) const {}
  ...
  bool avoidReorderingRenderTasks() const noexcept { return fAvoidReorderingRenderTasks; }

  bool avoidDithering() const noexcept { return fAvoidDithering; }
 ...
 private:
  ...
  virtual bool onCanCopySurface(
      const GrSurfaceProxy* dst, const GrSurfaceProxy* src, const SkIRect& srcRect,
      const SkIPoint& dstPoint) const = 0;
