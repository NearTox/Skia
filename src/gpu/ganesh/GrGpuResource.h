...
class GrIORef : public SkNoncopyable {
 public:
  ...
  void addCommandBufferUsage() const noexcept {
    // No barrier required.
 ...
 protected:
  constexpr GrIORef() noexcept : fRefCnt(1), fCommandBufferUsageCnt(0) {}
 ...
 private:
  ...
  bool hasNoCommandBufferUsages() const noexcept {
...
class GrGpuResource : public GrIORef<GrGpuResource> {
 public:
  ...
  const skgpu::UniqueKey& getUniqueKey() const noexcept { return fUniqueKey; }
