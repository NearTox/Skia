...
class GrMeshDrawTarget {
 public:
   ...
   */
  virtual void* makeVertexSpace(
      size_t vertexSize, int vertexCount, sk_sp<const GrBuffer>*, int* startVertex) = 0;
   ...
   */
  virtual void* makeVertexSpaceAtLeast(
   ...
   * buffer is allocated on behalf of this request.
   */
  virtual uint16_t* makeIndexSpaceAtLeast(
   ...
   */
  virtual GrDrawIndirectWriter makeDrawIndirectSpace(
   ...
   */
  virtual GrDrawIndexedIndirectWriter makeDrawIndexedIndirectSpace(
      int drawCount, sk_sp<const GrBuffer>*, size_t* offsetInBytes) = 0;

  /** Helpers for ops that only need to use the VertexWriter to fill the data directly. */
  skgpu::VertexWriter makeVertexWriter(
      size_t vertexSize, int vertexCount, sk_sp<const GrBuffer>*, int* startVertex);
  skgpu::IndexWriter makeIndexWriter(int indexCount, sk_sp<const GrBuffer>*, int* startIndex);
  skgpu::VertexWriter makeVertexWriterAtLeast(
      size_t vertexSize, int minVertexCount, int fallbackVertexCount, sk_sp<const GrBuffer>*,
      int* startVertex, int* actualVertexCount);
  skgpu::IndexWriter makeIndexWriterAtLeast(
      int minIndexCount, int fallbackIndexCount, sk_sp<const GrBuffer>*, int* startIndex,
