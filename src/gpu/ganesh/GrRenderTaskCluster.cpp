...
static void validate(
    SkSpan<const sk_sp<GrRenderTask>> input, const SkTInternalLList<GrRenderTask>& llist) {
  ...
  for (GrRenderTask* t : llist) {
    ...
    for (GrRenderTask* dep : t->dependencies()) {
      SkASSERTF(
          seen.contains(dep), "%s came before dependency %s", describe_task(t).c_str(),
  ...
  for (int i = 0; i < depender->numTargets(); i++) {
    if (dependee->isUsed(depender->target(i))) {
      CLUSTER_DEBUGF(
          ...
          describe_task(depender).c_str(), describe_task(dependee).c_str(),
...
static bool task_cluster_visit(
    GrRenderTask* task, SkTInternalLList<GrRenderTask>* llist,
  ...
  while (clusterHead->fPrev && 1 == clusterHead->fPrev->numTargets() &&
...
bool GrClusterRenderTasks(
    SkSpan<const sk_sp<GrRenderTask>> input, SkTInternalLList<GrRenderTask>* llist) {
