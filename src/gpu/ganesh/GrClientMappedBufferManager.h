 ...
 public:
  ...
  struct BufferFinishedMessage {
    BufferFinishedMessage(
        sk_sp<GrGpuBuffer> buffer, GrDirectContext::DirectContextID intendedRecipient) noexcept
        : fBuffer(std::move(buffer)), fIntendedRecipient(intendedRecipient) {}
    BufferFinishedMessage(BufferFinishedMessage&& other) noexcept {
  ...
  using BufferFinishedMessageBus =
      SkMessageBus<BufferFinishedMessage, GrDirectContext::DirectContextID, false>;
...
bool SkShouldPostMessageToBus(
    const GrClientMappedBufferManager::BufferFinishedMessage&,
