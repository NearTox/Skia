...
class GrSamplerState {
 public:
  ...
  // to aniso.
  static constexpr GrSamplerState Aniso(
      WrapMode wrapX, WrapMode wrapY, int maxAniso, GrMipmapped viewIsMipped) {
    ...
    sampler.fMipmapMode =
        viewIsMipped == GrMipmapped::kYes ? MipmapMode::kLinear : MipmapMode::kNone;
    return sampler;
  }
  ...
  constexpr bool isRepeatedX() const noexcept {
    return fWrapModes[0] == WrapMode::kRepeat || fWrapModes[0] == WrapMode::kMirrorRepeat;
  }

  constexpr bool isRepeatedY() const noexcept {
    return fWrapModes[1] == WrapMode::kRepeat || fWrapModes[1] == WrapMode::kMirrorRepeat;
  }

  constexpr bool isRepeated() const noexcept { return this->isRepeatedX() || this->isRepeatedY(); }
  ...
  int maxAniso() const noexcept { return fMaxAniso; }
  bool isAniso() const noexcept { return fMaxAniso > 1; }
   ...
   */
  uint32_t asKey(bool anisoIsOrthogonal) const noexcept {
