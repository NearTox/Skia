 ...
 public:
  ...
  class AutoSuppressWarningMessages {
   public:
    ...
    ~AutoSuppressWarningMessages() { --fContext->fSuppressWarningMessages; }
  ...
  GrRecordingContext::Stats* stats() { return &this->context()->fStats; }
  ...
  sk_sp<skgpu::BaseDevice> createDevice(
      GrColorType, sk_sp<GrSurfaceProxy>, sk_sp<SkColorSpace>, GrSurfaceOrigin,
      const SkSurfaceProps&, skgpu::BaseDevice::InitContents);
  sk_sp<skgpu::BaseDevice> createDevice(
      SkBudgeted, const SkImageInfo&, SkBackingFit, int sampleCount, GrMipmapped, GrProtected,
      GrSurfaceOrigin, const SkSurfaceProps&, skgpu::BaseDevice::InitContents);
  ...
  std::unique_ptr<skgpu::SurfaceContext> makeSC(GrSurfaceProxyView readView, const GrColorInfo&);

  // Makes either a SurfaceContext, SurfaceFillContext, or a SurfaceDrawContext, depending on
  // GrRenderable and the GrImageInfo.
  std::unique_ptr<skgpu::SurfaceContext> makeSC(
      const GrImageInfo&, const GrBackendFormat&, SkBackingFit = SkBackingFit::kExact,
      GrSurfaceOrigin = kTopLeft_GrSurfaceOrigin, GrRenderable = GrRenderable::kNo,
      int renderTargetSampleCnt = 1, GrMipmapped = GrMipmapped::kNo, GrProtected = GrProtected::kNo,
  ...
  std::unique_ptr<skgpu::SurfaceFillContext> makeSFC(
      GrImageInfo, SkBackingFit = SkBackingFit::kExact, int sampleCount = 1,
      GrMipmapped = GrMipmapped::kNo, GrProtected = GrProtected::kNo,
      GrSurfaceOrigin = kTopLeft_GrSurfaceOrigin, SkBudgeted = SkBudgeted::kYes);
   ...
   */
  std::unique_ptr<skgpu::SurfaceFillContext> makeSFC(
      SkAlphaType, sk_sp<SkColorSpace>, SkISize dimensions, SkBackingFit, const GrBackendFormat&,
      int sampleCount, GrMipmapped, GrProtected, skgpu::Swizzle readSwizzle,
      skgpu::Swizzle writeSwizzle, GrSurfaceOrigin, SkBudgeted);
  ...
  std::unique_ptr<skgpu::SurfaceFillContext> makeSFCFromBackendTexture(
      GrColorInfo, const GrBackendTexture&, int sampleCount, GrSurfaceOrigin,
