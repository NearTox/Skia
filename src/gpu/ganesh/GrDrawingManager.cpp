...
bool GrDrawingManager::flush(
  ...
  if (preFlushSuccessful) {
    ...
    cachePurgeNeeded =
        !resourceAllocator.failedInstantiation() && this->executeRenderTasks(&flushState);
  }
  this->removeRenderTasks();
...
sk_sp<skgpu::v1::OpsTask> GrDrawingManager::newOpsTask(
    GrSurfaceProxyView surfaceView, sk_sp<GrArenas> arenas) {
  SkDEBUGCODE(this->validate());
  SkASSERT(fContext);
...
void GrDrawingManager::newTextureResolveRenderTask(
