...
std::unique_ptr<GrMemoryPool> GrMemoryPool::Make(size_t preallocSize, size_t minAllocSize) {
  ...
  preallocSize =
      SkTPin(preallocSize, kMinAllocationSize, (size_t)SkBlockAllocator::kMaxAllocationSize);
  minAllocSize =
      SkTPin(minAllocSize, kMinAllocationSize, (size_t)SkBlockAllocator::kMaxAllocationSize);
...
GrMemoryPool::GrMemoryPool(size_t preallocSize, size_t minAllocSize) noexcept 
    : fAllocator(
          SkBlockAllocator::GrowthPolicy::kFixed, minAllocSize,
          preallocSize - offsetof(GrMemoryPool, fAllocator) - sizeof(SkBlockAllocator)) {
  SkDEBUGCODE(fDebug = new Debug; fDebug->fAllocationCount = 0;)
}
...
void GrMemoryPool::reportLeaks() const noexcept {
#ifdef SK_DEBUG
  int i = 0;
