...
static int test_table_entry(
    int rOrig, int gOrig, int bOrig, int r8, int g8, int b8, int table, int offset) {
...
static void create_etc1_block(SkColor col, ETC1Block* block) {
  ...
  for (int tableIndex = 0; tableIndex < kNumETC1ModifierTables; ++tableIndex) {
    for (int pixelIndex = 0; pixelIndex < kNumETC1PixelIndices; ++pixelIndex) {
      int score = test_table_entry(rOrig, gOrig, bOrig, r8, g8, b8, tableIndex, pixelIndex);
...
size_t GrNumBlocks(SkImage::CompressionType type, SkISize baseDimensions) {
  switch (type) {
    case SkImage::CompressionType::kNone: return baseDimensions.width() * baseDimensions.height();
...
SkISize GrCompressedDimensions(SkImage::CompressionType type, SkISize baseDimensions) {
  switch (type) {
    case SkImage::CompressionType::kNone: return baseDimensions;
    case SkImage::CompressionType::kETC2_RGB8_UNORM:
    case SkImage::CompressionType::kBC1_RGB8_UNORM:
    case SkImage::CompressionType::kBC1_RGBA8_UNORM: {
      int numBlocksWidth = num_4x4_blocks(baseDimensions.width());
...
static skgpu::Swizzle get_load_and_src_swizzle(
    GrColorType ct, SkRasterPipeline::StockStage* load, bool* isNormalized, bool* isSRGB) {
  ...
  *isSRGB = false;
  switch (ct) {
    ...
    case GrColorType::kBGRA_1010102:
      *load = SkRasterPipeline::load_1010102;
      swizzle = skgpu::Swizzle("bgra");
      break;
    ...
    case GrColorType::kRG_F16:
      *load = SkRasterPipeline::load_rgf16;
      *isNormalized = false;
      break;
    case GrColorType::kRGBA_F16:
      *load = SkRasterPipeline::load_f16;
      *isNormalized = false;
      break;
...
static skgpu::Swizzle get_dst_swizzle_and_store(
  ...
  switch (ct) {
    ...
    case GrColorType::kRGBA_8888_SRGB:
      ...
      *isSRGB = true;
      break;
    case GrColorType::kRG_F16:
      *store = SkRasterPipeline::store_rgf16;
      *isNormalized = false;
      break;
    case GrColorType::kAlpha_F16:
      ...
      *isNormalized = false;
      break;
    case GrColorType::kRGBA_F16:
      *store = SkRasterPipeline::store_f16;
      *isNormalized = false;
      break;
    case GrColorType::kRGBA_F32:
...
bool GrConvertPixels(const GrPixmap& dst, const GrCPixmap& src, bool flipY) {
  ...
  bool premul = src.alphaType() == kUnpremul_SkAlphaType && dst.alphaType() == kPremul_SkAlphaType;
  bool unpremul =
      src.alphaType() == kPremul_SkAlphaType && dst.alphaType() == kUnpremul_SkAlphaType;
  ...
  bool srcIsSRGB;
  auto loadSwizzle = get_load_and_src_swizzle(src.colorType(), &load, &srcIsNormalized, &srcIsSRGB);
  ...
  bool dstIsSRGB;
  auto storeSwizzle =
      get_dst_swizzle_and_store(dst.colorType(), &store, &lumMode, &dstIsNormalized, &dstIsSRGB);
  ...
  SkRasterPipeline_MemoryCtx srcCtx{
      const_cast<void*>(src.addr()), SkToInt(src.rowBytes() / srcBpp)},
