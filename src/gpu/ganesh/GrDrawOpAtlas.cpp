...
std::unique_ptr<GrDrawOpAtlas> GrDrawOpAtlas::Make(
    GrProxyProvider* proxyProvider, const GrBackendFormat& format, SkColorType colorType,
    size_t bpp, int width, int height, int plotWidth, int plotHeight,
    GenerationCounter* generationCounter, AllowMultitexturing allowMultitexturing,
    EvictionCallback* evictor, std::string_view label) {
  if (!format.isValid()) {
    return nullptr;
  }

  std::unique_ptr<GrDrawOpAtlas> atlas(new GrDrawOpAtlas(
      proxyProvider, format, colorType, bpp, width, height, plotWidth, plotHeight,
...
GrDrawOpAtlas::GrDrawOpAtlas(
    ...
    : fFormat(format),
      ...
      fMaxPages(
          AllowMultitexturing::kYes == allowMultitexturing ? PlotLocator::kMaxMultitexturePages
                                                           : 1),
      fNumActivePages(0) {
...
void GrDrawOpAtlas::uploadPlotToTexture(
    GrDeferredTextureUploadWritePixelsFn& writePixels, GrTextureProxy* proxy, Plot* plot) {
  ...
  writePixels(
      proxy, rect, SkColorTypeToGrColorType(fColorType), dataPtr, fBytesPerPixel * fPlotWidth);
}
...
bool GrDrawOpAtlas::createPages(
  ...
  for (uint32_t i = 0; i < this->maxPages(); ++i) {
    ...
    sk_sp<GrSurfaceProxy> proxy = proxyProvider->createProxy(
        ...
        SkBudgeted::kYes, GrProtected::kNo, fLabel, GrInternalSurfaceFlags::kNone,
