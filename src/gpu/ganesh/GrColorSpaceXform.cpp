...
sk_sp<GrColorSpaceXform> GrColorSpaceXform::Make(
...
sk_sp<GrColorSpaceXform> GrColorSpaceXform::Make(
    const GrColorInfo& srcInfo, const GrColorInfo& dstInfo) {
  return Make(srcInfo.colorSpace(), srcInfo.alphaType(), dstInfo.colorSpace(), dstInfo.alphaType());
}
...
bool GrColorSpaceXform::Equals(const GrColorSpaceXform* a, const GrColorSpaceXform* b) {
  ...
  if (a->fSteps.flags.gamut_transform &&
      0 != memcmp(
               &a->fSteps.src_to_dst_matrix, &b->fSteps.src_to_dst_matrix,
...
GrColorSpaceXformEffect::GrColorSpaceXformEffect(
...
GrColorSpaceXformEffect::GrColorSpaceXformEffect(const GrColorSpaceXformEffect& that)
    : INHERITED(that), fColorXform(that.fColorXform) {}
...
std::unique_ptr<GrFragmentProcessor::ProgramImpl> GrColorSpaceXformEffect::onMakeProgramImpl()
    const {
...
GrFragmentProcessor::OptimizationFlags GrColorSpaceXformEffect::OptFlags(
  ...
  return ProcessorOptimizationFlags(child) &
         (kCompatibleWithCoverageAsAlpha_OptimizationFlag | kPreservesOpaqueInput_OptimizationFlag |
