...
GrRenderTargetProxy::GrRenderTargetProxy(
    ...
    WrapsVkSecondaryCB wrapsVkSecondaryCB, std::string_view label)
    : INHERITED(
...
size_t GrRenderTargetProxy::onUninstantiatedGpuMemorySize() const {
  ...
  return GrSurface::ComputeSize(
      this->backendFormat(), this->dimensions(), colorSamplesPerPixel, GrMipmapped::kNo,
      !this->priv().isExact());
}
...
GrSurfaceProxy::LazySurfaceDesc GrRenderTargetProxy::callbackDesc() const {
  ...
  return {
      ...
      this->numSamples(), this->backendFormat(), GrTextureType::kNone, this->isProtected(),
      this->isBudgeted(), this->getLabel(),
  };
}
