...
std::unique_ptr<SurfaceDrawContext> SurfaceDrawContext::Make(
    GrRecordingContext* rContext, GrColorType colorType, sk_sp<GrSurfaceProxy> proxy,
    sk_sp<SkColorSpace> colorSpace, GrSurfaceOrigin origin, const SkSurfaceProps& surfaceProps) {
  ...
  return std::make_unique<SurfaceDrawContext>(
      rContext, std::move(readView), std::move(writeView), colorType, std::move(colorSpace),
      surfaceProps);
}

std::unique_ptr<SurfaceDrawContext> SurfaceDrawContext::Make(
    GrRecordingContext* rContext, sk_sp<SkColorSpace> colorSpace, SkBackingFit fit,
    SkISize dimensions, const GrBackendFormat& format, int sampleCnt, GrMipmapped mipmapped,
    GrProtected isProtected, skgpu::Swizzle readSwizzle, skgpu::Swizzle writeSwizzle,
  ...
  auto sdc = std::make_unique<SurfaceDrawContext>(
      rContext, std::move(readView), std::move(writeView), GrColorType::kUnknown,
...
std::unique_ptr<SurfaceDrawContext> SurfaceDrawContext::Make(
    GrRecordingContext* rContext, GrColorType colorType, sk_sp<SkColorSpace> colorSpace,
    SkBackingFit fit, SkISize dimensions, const SkSurfaceProps& surfaceProps, int sampleCnt,
  ...
  return SurfaceDrawContext::Make(
      rContext, colorType, std::move(proxy), std::move(colorSpace), origin, surfaceProps);
}
...
std::unique_ptr<SurfaceDrawContext> SurfaceDrawContext::MakeWithFallback(
  ...
  return SurfaceDrawContext::Make(
      rContext, ct, colorSpace, fit, dimensions, surfaceProps, sampleCnt, mipmapped, isProtected,
      origin, budgeted);
}
...
std::unique_ptr<SurfaceDrawContext> SurfaceDrawContext::MakeFromBackendTexture(
    ...
    const SkSurfaceProps& surfaceProps, sk_sp<skgpu::RefCntedCallback> releaseHelper) {
  SkASSERT(sampleCnt > 0);
  sk_sp<GrTextureProxy> proxy(rContext->priv().proxyProvider()->wrapRenderableBackendTexture(
      tex, sampleCnt, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, std::move(releaseHelper)));
  if (!proxy) {
    return nullptr;
  }
...
SurfaceDrawContext::SurfaceDrawContext(
    GrRecordingContext* rContext, GrSurfaceProxyView readView, GrSurfaceProxyView writeView,
    GrColorType colorType, sk_sp<SkColorSpace> colorSpace, const SkSurfaceProps& surfaceProps)
    : SurfaceFillContext(
          rContext, std::move(readView), std::move(writeView),
...
SurfaceDrawContext::~SurfaceDrawContext() { ASSERT_SINGLE_OWNER }
...
void SurfaceDrawContext::drawGlyphRunList(
    SkCanvas* canvas, const GrClip* clip, const SkMatrixProvider& viewMatrix,
    const SkGlyphRunList& glyphRunList, SkStrikeDeviceInfo strikeDeviceInfo, const SkPaint& paint) {
  ASSERT_SINGLE_OWNER
  RETURN_IF_ABANDONED
  SkDEBUGCODE(this->validate();)
...
void SurfaceDrawContext::drawPaint(
...
SurfaceDrawContext::QuadOptimization SurfaceDrawContext::attemptQuadOptimization(
  ...
  switch (result.fEffect) {
    case GrClip::Effect::kClippedOut: return QuadOptimization::kDiscarded;
  ...
  if (result.fRRect.isRect()) {
    ...
    if (GrQuadUtils::CropToRect(
      ...
      this->drawRRect(
          nullptr, std::move(*paint), result.fAA, SkMatrix::I(), result.fRRect,
          GrStyle::SimpleFill());
      return QuadOptimization::kSubmitted;
    }
  }
...
void SurfaceDrawContext::drawFilledQuad(
    const GrClip* clip, GrPaint&& paint, DrawQuad* quad, const GrUserStencilSettings* ss) {
  ASSERT_SINGLE_OWNER
  RETURN_IF_ABANDONED
  SkDEBUGCODE(this->validate();)
  ...
  if (opt >= QuadOptimization::kClipApplied) {
    ...
    this->addDrawOp(finalClip, FillRectOp::Make(fContext, std::move(paint), aaType, quad, ss));
  }
  // All other optimization levels were completely handled inside attempt(), so no extra op needed
}

void SurfaceDrawContext::drawTexture(
    const GrClip* clip, GrSurfaceProxyView view, SkAlphaType srcAlphaType,
    GrSamplerState::Filter filter, GrSamplerState::MipmapMode mm, SkBlendMode blendMode,
    const SkPMColor4f& color, const SkRect& srcRect, const SkRect& dstRect, GrQuadAAFlags edgeAA,
  ...
  if ((this->alwaysAntialias() || this->caps()->reducedShaderMode()) &&
    ...
    if (constraint == SkCanvas::kStrict_SrcRectConstraint) {
      fp = GrTextureEffect::MakeSubset(
          view, srcAlphaType, SkMatrix::I(), GrSamplerState(filter, mm), srcRect, *this->caps());
    } else {
  ...
  const SkRect* subset = constraint == SkCanvas::kStrict_SrcRectConstraint ? &srcRect : nullptr;
  DrawQuad quad{GrQuad::MakeFromRect(dstRect, viewMatrix), GrQuad(srcRect), edgeAA};

  this->drawTexturedQuad(
      clip, std::move(view), srcAlphaType, std::move(colorSpaceXform), filter, mm, color, blendMode,
      &quad, subset);
}
...
void SurfaceDrawContext::drawTexturedQuad(
    ...
    GrSamplerState::MipmapMode mm, const SkPMColor4f& color, SkBlendMode blendMode, DrawQuad* quad,
    const SkRect* subset) {
  ASSERT_SINGLE_OWNER
  RETURN_IF_ABANDONED
  SkDEBUGCODE(this->validate();)
  ...
  QuadOptimization opt =
      this->attemptQuadOptimization(clip, nullptr /*stencil*/, quad, nullptr /*paint*/);
  ...
  if (opt != QuadOptimization::kDiscarded) {
    ...
    auto saturate =
        clampType == GrClampType::kManual ? TextureOp::Saturate::kYes : TextureOp::Saturate::kNo;
    ...
    this->addDrawOp(
        finalClip, TextureOp::Make(
                       fContext, std::move(proxyView), srcAlphaType, std::move(textureXform),
                       filter, mm, color, saturate, blendMode, aaType, quad, subset));
  }
}

void SurfaceDrawContext::drawRect(
  ...
  if (stroke.getStyle() == SkStrokeRec::kFill_Style) {
    ...
    return;
      ...
      rect.width() && rect.height() && !this->caps()->reducedShaderMode()) {
    ...
    GrAAType aaType = (fCanUseDynamicMSAA && stroke.getJoin() == SkPaint::kMiter_Join &&
                       stroke.getMiter() >= SK_ScalarSqrt2)
                          ? GrAAType::kCoverage
                          : this->chooseAAType(aa);
    GrOp::Owner op =
        StrokeRectOp::Make(fContext, std::move(paint), aaType, viewMatrix, rect, stroke);
  ...
  this->drawShapeUsingPathRenderer(
      clip, std::move(paint), aa, viewMatrix, GrStyledShape(rect, *style, DoSimplify::kNo));
}

void SurfaceDrawContext::fillRectToRect(
    const GrClip* clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix,
    const SkRect& rectToDraw, const SkRect& localRect) {
  DrawQuad quad{
  ...
  if ((fContext->priv().caps()->reducedShaderMode() || this->alwaysAntialias()) &&
    ...
    QuadOptimization opt = this->attemptQuadOptimization(clip, nullptr /*stencil*/, &quad, &paint);
    if (opt < QuadOptimization::kClipApplied) {
    ...
    if (auto op = FillRRectOp::Make(
            fContext, this->arenaAlloc(), std::move(paint), viewMatrix,
            SkRRect::MakeRect(croppedRect), croppedLocal, GrAA::kYes)) {
...
void SurfaceDrawContext::drawQuadSet(
    const GrClip* clip, GrPaint&& paint, const SkMatrix& viewMatrix, const GrQuadSetEntry quads[],
    int cnt) {
...
void SurfaceDrawContext::setNeedsStencil() {
  ...
  if (!hasInitializedStencil) {
    ...
    if (this->caps()->performStencilClearsAsDraws()) {
      ...
      this->getOpsTask()->setInitialStencilContent(OpsTask::StencilContent::kUserBitsCleared);
    }
  }
}
...
bool SurfaceDrawContext::stencilPath(
    const GrHardClip* clip, GrAA doStencilMSAA, const SkMatrix& viewMatrix, const SkPath& path) {
  SkIRect clipBounds = clip ? clip->getConservativeBounds() : SkIRect::MakeSize(this->dimensions());
  ...
  auto pr = this->drawingManager()->getPathRenderer(
      canDrawArgs, false, PathRendererChain::DrawType::kStencil);
  if (!pr) {
...
void SurfaceDrawContext::drawTextureSet(
    const GrClip* clip, GrTextureSetEntry set[], int cnt, int proxyRunCnt,
    GrSamplerState::Filter filter, GrSamplerState::MipmapMode mm, SkBlendMode mode,
    SkCanvas::SrcRectConstraint constraint, const SkMatrix& viewMatrix,
  ...
  auto saturate =
      clampType == GrClampType::kManual ? TextureOp::Saturate::kYes : TextureOp::Saturate::kNo;
  TextureOp::AddTextureSetOps(
      this, clip, fContext, set, cnt, proxyRunCnt, filter, mm, saturate, mode, aaType, constraint,
      viewMatrix, std::move(texXform));
}

void SurfaceDrawContext::drawVertices(
    const GrClip* clip, GrPaint&& paint, const SkMatrixProvider& matrixProvider,
    sk_sp<SkVertices> vertices, GrPrimitiveType* overridePrimType, bool skipColorXform) {
  ASSERT_SINGLE_OWNER
  RETURN_IF_ABANDONED
  SkDEBUGCODE(this->validate();)
  ...
  GrOp::Owner op = DrawMeshOp::Make(
      fContext, std::move(paint), std::move(vertices), overridePrimType, matrixProvider, aaType,
      std::move(xform));
  this->addDrawOp(clip, std::move(op));
}

void SurfaceDrawContext::drawMesh(
    const GrClip* clip, GrPaint&& paint, const SkMatrixProvider& matrixProvider,
    const SkMesh& mesh) {
  ASSERT_SINGLE_OWNER
  RETURN_IF_ABANDONED
  ...
  auto xform = GrColorSpaceXform::Make(
      SkMeshSpecificationPriv::ColorSpace(*mesh.spec()),
      SkMeshSpecificationPriv::AlphaType(*mesh.spec()), this->colorInfo().colorSpace(),
      this->colorInfo().alphaType());
  GrAAType aaType = fCanUseDynamicMSAA ? GrAAType::kMSAA : this->chooseAAType(GrAA::kNo);
  GrOp::Owner op =
      DrawMeshOp::Make(fContext, std::move(paint), mesh, matrixProvider, aaType, std::move(xform));
  this->addDrawOp(clip, std::move(op));
}

///////////////////////////////////////////////////////////////////////////////

void SurfaceDrawContext::drawAtlas(
  ...
  GrOp::Owner op = DrawAtlasOp::Make(
...
void SurfaceDrawContext::drawRRect(
  ...
  if (clip && stroke.getStyle() == SkStrokeRec::kFill_Style &&
    ...
    switch (result.fEffect) {
      case GrClip::Effect::kClippedOut: return;
      case GrClip::Effect::kUnclipped: clip = nullptr; break;
      case GrClip::Effect::kClipped:
  ...
  GrOp::Owner op;
  if (aaType == GrAAType::kCoverage && !fCanUseDynamicMSAA && !this->caps()->reducedShaderMode() &&
      rrect.isSimple() && rrect.getSimpleRadii().fX == rrect.getSimpleRadii().fY &&
  ...
  if (!op && style.isSimpleFill()) {
    assert_alive(paint);
    op = FillRRectOp::Make(
        fContext, this->arenaAlloc(), std::move(paint), viewMatrix, rrect, rrect.rect(),
  ...
  assert_alive(paint);
  this->drawShapeUsingPathRenderer(
      clip, std::move(paint), aa, viewMatrix, GrStyledShape(rrect, style, DoSimplify::kNo));
}

///////////////////////////////////////////////////////////////////////////////

bool SurfaceDrawContext::drawFastShadow(
  ...
  if (SkColorGetA(rec.fAmbientColor) > 0) {
    ...
    GrOp::Owner op = ShadowRRectOp::Make(
  ...
  if (SkColorGetA(rec.fSpotColor) > 0) {
    ...
    SkVector spotOffset;
    if (directional) {
      SkDrawShadowMetrics::GetDirectionalParams(
...
void SurfaceDrawContext::drawRegion(
    const GrClip* clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix,
...
void SurfaceDrawContext::drawOval(
  ...
  GrOp::Owner op;
  if (aaType == GrAAType::kCoverage && !fCanUseDynamicMSAA && !this->caps()->reducedShaderMode() &&
      oval.width() > SK_ScalarNearlyZero && oval.width() == oval.height() &&
  ...
  if (!op && style.isSimpleFill()) {
    ...
    assert_alive(paint);
    op = FillRRectOp::Make(
        fContext, this->arenaAlloc(), std::move(paint), viewMatrix, SkRRect::MakeOval(oval), oval,
        GrAA(aaType != GrAAType::kNone));
  }
  if (!op && (aaType == GrAAType::kCoverage || fCanUseDynamicMSAA)) {
    assert_alive(paint);
  ...
  this->drawShapeUsingPathRenderer(
      ...
      GrStyledShape(
          SkRRect::MakeOval(oval), SkPathDirection::kCW, 2, false, style, DoSimplify::kNo));
}

void SurfaceDrawContext::drawArc(
    const GrClip* clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix, const SkRect& oval,
  ...
  if (aaType == GrAAType::kCoverage) {
    ...
    GrOp::Owner op = GrOvalOpFactory::MakeArcOp(
  ...
  this->drawShapeUsingPathRenderer(
      clip, std::move(paint), aa, viewMatrix,
      GrStyledShape::MakeArc(oval, startAngle, sweepAngle, useCenter, style, DoSimplify::kNo));
}

void SurfaceDrawContext::drawImageLattice(
...
void SurfaceDrawContext::setLastClip(
...
bool SurfaceDrawContext::mustRenderClip(
    uint32_t clipStackGenID, const SkIRect& devClipBounds, int numClipAnalyticElements) {
  auto opsTask = this->getOpsTask();
...
bool SurfaceDrawContext::waitOnSemaphores(
...
void SurfaceDrawContext::drawPath(
...
void SurfaceDrawContext::drawShape(
    const GrClip* clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix,
    GrStyledShape&& shape) {
  ASSERT_SINGLE_OWNER
  RETURN_IF_ABANDONED
  ...
  this->drawShapeUsingPathRenderer(
      clip, std::move(paint), aa, viewMatrix, std::move(shape),
...
bool SurfaceDrawContext::drawAndStencilPath(
  ...
  if (path.isEmpty() && path.isInverseFillType()) {
    ...
    this->stencilRect(
        clip, ss, std::move(paint), GrAA::kNo, SkMatrix::I(), SkRect::Make(this->dimensions()));
    return true;
  }
  ...
  auto pr = this->drawingManager()->getPathRenderer(canDrawArgs, false, DrawType::kStencilAndColor);
  if (!pr) {
    return false;
  }

  PathRenderer::DrawPathArgs args{
...
void SurfaceDrawContext::drawStrokedLine(
    const GrClip* clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix,
  ...
  if (this->caps()->drawInstancedSupport() &&
    ...
    SkMatrix localMatrix =
        SkMatrix::MakeAll(p1.fX - p0.fX, ortho.fX, p0.fX, p1.fY - p0.fY, ortho.fY, p0.fY, 0, 0, 1);
    if (auto op = FillRRectOp::Make(
            fContext, this->arenaAlloc(), std::move(paint),
            SkMatrix::Concat(viewMatrix, localMatrix), SkRRect::MakeRect({0, -1, 1, 1}),
            localMatrix, GrAA::kYes)) {
      this->addDrawOp(clip, std::move(op));
      return;
    }
  }
  ...
  SkPoint corners[4] = {p0 - ortho, p0 + ortho, p1 + ortho, p1 - ortho};
...
bool SurfaceDrawContext::drawSimpleShape(
    const GrClip* clip, GrPaint* paint, GrAA aa, const SkMatrix& viewMatrix,
  ...
  if (!shape.style().hasPathEffect()) {
    ...
    if (shape.asLine(linePts, &inverted) && !inverted &&
      ...
      if (aaType == GrAAType::kCoverage ||
        ...
        this->drawStrokedLine(
            clip, std::move(*paint), aa, viewMatrix, linePts, shape.style().strokeRec());
        return true;
      }
      ...
      return true;
        ...
        viewMatrix.rectStaysRect() && !this->caps()->reducedShaderMode()) {
      ...
      if (shape.asNestedRects(rects)) {
        ...
        GrOp::Owner op = StrokeRectOp::MakeNested(fContext, std::move(*paint), viewMatrix, rects);
        if (op) {
          this->addDrawOp(clip, std::move(op));
          return true;
        }
...
void SurfaceDrawContext::drawShapeUsingPathRenderer(
    const GrClip* clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix, GrStyledShape&& shape,
  ...
  if (!pr) {
    ...
    pr = this->drawingManager()->getPathRenderer(
        canDrawArgs, kDisallowSWPathRenderer, DrawType::kColor);
  }
  ...
  if (!pr) {
    if (shape.style().applies()) {
      ...
      pr = this->drawingManager()->getPathRenderer(
          canDrawArgs, kAllowSWPathRenderer, DrawType::kColor);
    } else {
...
void SurfaceDrawContext::addDrawOp(
    const GrClip* clip, GrOp::Owner op, const std::function<WillAddOpFn>& willAddFn) {
  ASSERT_SINGLE_OWNER
  if (fContext->abandoned()) {
    return;
  }
  ...
  if (clip) {
    ...
    skipDraw = clip->apply(fContext, this, drawOp, aaType, &appliedClip, &bounds) ==
               GrClip::Effect::kClippedOut;
  } else {
  ...
  bool usesAttachmentIfDMSAA =
      fCanUseDynamicMSAA && (!this->caps()->msaaResolvesAutomatically() || !this->asTextureProxy());
  ...
  opsTask->addDrawOp(
      this->drawingManager(), std::move(op), drawNeedsMSAA, analysis, std::move(appliedClip),
...
bool SurfaceDrawContext::setupDstProxyView(
  ...
  if (!(dstSampleFlags & GrDstSampleFlags::kRequiresTextureBarrier) && fCanUseDynamicMSAA &&
      this->getOpsTask()->usesMSAASurface() && !opRequiresMSAA) {
    auto newFlags = this->caps()->getDstSampleFlagsForProxy(
        this->asRenderTargetProxy(), false /*=opRequiresMSAA*/);
  ...
  auto copy = GrSurfaceProxy::Copy(
      fContext, this->asSurfaceProxyRef(), this->origin(), GrMipmapped::kNo, copyRect, fit,
