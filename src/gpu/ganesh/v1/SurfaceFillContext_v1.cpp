...
SurfaceFillContext::SurfaceFillContext(
    ...
    : skgpu::SurfaceFillContext(
          rContext, std::move(readView), std::move(writeView), std::move(colorInfo)) {
  fOpsTask = sk_ref_sp(rContext->priv().drawingManager()->getLastOpsTask(this->asSurfaceProxy()));

  SkDEBUGCODE(this->validate();)
}

void SurfaceFillContext::fillRectWithFP(
  ...
  auto op =
      FillRectOp::MakeNonAARect(fContext, std::move(paint), SkMatrix::I(), SkRect::Make(dstRect));
...
void SurfaceFillContext::addDrawOp(GrOp::Owner owner) {
  ...
  this->getOpsTask()->addDrawOp(
      fContext->priv().drawingManager(), std::move(owner), op->usesMSAA(), analysis,
      std::move(clip), dstProxyView, GrTextureResolveManager(this->drawingManager()), caps);
}
...
sk_sp<GrRenderTask> SurfaceFillContext::refRenderTask() { return sk_ref_sp(this->getOpsTask()); }

OpsTask* SurfaceFillContext::replaceOpsTask() {
  sk_sp<OpsTask> newOpsTask =
      this->drawingManager()->newOpsTask(this->writeSurfaceView(), this->arenas());
...
void SurfaceFillContext::resolveMSAA() {
  ...
  this->drawingManager()->newTextureResolveRenderTask(
      this->asSurfaceProxyRef(), GrSurfaceProxy::ResolveFlags::kMSAA, *this->caps());
}

void SurfaceFillContext::internalClear(
  ...
  if (clearAsDraw) {
    ...
    auto op = FillRectOp::MakeNonAARect(
...
bool SurfaceFillContext::blitTexture(
