...
std::unique_ptr<GrFragmentProcessor> make_inverse_rrect_fp(
    const SkMatrix& viewMatrix, const SkRRect& rrect, GrAA aa, const GrShaderCaps& shaderCaps) {
  SkTCopyOnFirstWrite<SkRRect> devRRect(rrect);
  if (viewMatrix.isIdentity() || rrect.transform(viewMatrix, devRRect.writable())) {
    auto edgeType =
        (aa == GrAA::kYes) ? GrClipEdgeType::kInverseFillAA : GrClipEdgeType::kInverseFillBW;
    auto [success, fp] = GrRRectEffect::Make(/*inputFP=*/nullptr, edgeType, *devRRect, shaderCaps);
...
bool init_vertices_paint(
    GrRecordingContext* rContext, const GrColorInfo& colorInfo, const SkPaint& skPaint,
    const SkMatrixProvider& matrixProvider, sk_sp<SkBlender> blender, bool hasColors,
    GrPaint* grPaint) {
  if (hasColors) {
    return SkPaintToGrPaintWithBlend(
        rContext, colorInfo, skPaint, matrixProvider, blender.get(), grPaint);
  } else {
...
sk_sp<BaseDevice> Device::Make(
    ...
    InitContents init) {
  auto sdc = SurfaceDrawContext::Make(
...
sk_sp<BaseDevice> Device::Make(
    std::unique_ptr<SurfaceDrawContext> sdc, SkAlphaType alphaType, InitContents init) {
  if (!sdc) {
    return nullptr;
  }
...
sk_sp<BaseDevice> Device::Make(
    GrRecordingContext* rContext, SkBudgeted budgeted, const SkImageInfo& ii, SkBackingFit fit,
    int sampleCount, GrMipmapped mipmapped, GrProtected isProtected, GrSurfaceOrigin origin,
  ...
  auto sdc = SurfaceDrawContext::Make(
      rContext, SkColorTypeToGrColorType(ii.colorType()), ii.refColorSpace(), fit, ii.dimensions(),
      props, sampleCount, mipmapped, isProtected, origin, budgeted);
...
Device::Device(std::unique_ptr<SurfaceDrawContext> sdc, DeviceFlags flags)
    : INHERITED(
          sk_ref_sp(sdc->recordingContext()), MakeInfo(sdc.get(), flags), sdc->surfaceProps()),
      ...
      fClip(
          SkIRect::MakeSize(fSurfaceDrawContext->dimensions()), &this->asMatrixProvider(),
...
SkBaseDevice::ClipType Device::onGetClipType() const {
  ...
  if (state == ClipStack::ClipState::kEmpty) {
    return ClipType::kEmpty;
      ...
      state == ClipStack::ClipState::kDeviceRect || state == ClipStack::ClipState::kWideOpen) {
...
void Device::drawPoints(
  ...
  if (count == 2 && mode == SkCanvas::kLines_PointMode) {
    if (paint.getPathEffect()) {
      ...
      if (SkPaintToGrPaint(
        ...
        path.moveTo(pts[0]);
        path.lineTo(pts[1]);
        fSurfaceDrawContext->drawPath(
            this->clip(), std::move(grPaint), aa, this->localToDevice(), path,
    ...
    if (!paint.getMaskFilter() &&
      ...
      if (SkPaintToGrPaint(
              this->recordingContext(), fSurfaceDrawContext->colorInfo(), paint,
              this->asMatrixProvider(), &grPaint)) {
        fSurfaceDrawContext->drawStrokedLine(
            this->clip(), std::move(grPaint), aa, this->localToDevice(), pts,
  ...
  GrPaint grPaint;
  if (!SkPaintToGrPaint(
          this->recordingContext(), fSurfaceDrawContext->colorInfo(), paint, *matrixProvider,
          &grPaint)) {
    return;
  }
  ...
  fSurfaceDrawContext->drawVertices(
      this->clip(), std::move(grPaint), *matrixProvider, std::move(vertices), &primitiveType);
}
...
void Device::drawRect(const SkRect& rect, const SkPaint& paint) {
  ...
  if (paint.getMaskFilter() || paint.getPathEffect()) {
    ...
    GrBlurUtils::drawShapeWithMaskFilter(
        fContext.get(), fSurfaceDrawContext.get(), this->clip(), paint, this->asMatrixProvider(),
        shape);
    return;
  }
  ...
  fSurfaceDrawContext->drawRect(
      this->clip(), std::move(grPaint), fSurfaceDrawContext->chooseAA(paint), this->localToDevice(),
      rect, &style);
}
...
void Device::drawEdgeAAQuad(
  ...
  if (clip) {
    ...
    fSurfaceDrawContext->fillQuadWithEdgeAA(
        this->clip(), std::move(grPaint), SkToGrQuadAAFlags(aaFlags), this->localToDevice(), clip,
        nullptr);
  } else {
    // Use fillRectWithEdgeAA to preserve mathematical properties of dst being rectangular
    fSurfaceDrawContext->fillRectWithEdgeAA(
        this->clip(), std::move(grPaint), SkToGrQuadAAFlags(aaFlags), this->localToDevice(), rect);
  }
}
...
void Device::drawDRRect(const SkRRect& outer, const SkRRect& inner, const SkPaint& paint) {
  ...
  if (stroke.isFillStyle() && !paint.getMaskFilter() && !paint.getPathEffect()) {
    ...
    if (auto fp = make_inverse_rrect_fp(
            this->localToDevice(), inner, fSurfaceDrawContext->chooseAA(paint),
      ...
      fSurfaceDrawContext->drawRRect(
          this->clip(), std::move(grPaint), fSurfaceDrawContext->chooseAA(paint),
...
void Device::drawRegion(const SkRegion& region, const SkPaint& paint) {
  ...
  fSurfaceDrawContext->drawRegion(
      this->clip(), std::move(grPaint), fSurfaceDrawContext->chooseAA(paint), this->localToDevice(),
...
sk_sp<SkSpecialImage> Device::makeSpecial(const SkImage* image) {
  ...
  if (image->isTextureBacked()) {
    ...
    return SkSpecialImage::MakeDeferredFromGpu(
        ...
        std::move(view), ct, image->refColorSpace(), this->surfaceProps());
...
sk_sp<SkSpecialImage> Device::snapSpecial(const SkIRect& subset, bool forceCopy) {
  ...
  if (forceCopy || !view.asTextureProxy()) {
    ...
    view = GrSurfaceProxyView::Copy(
        fContext.get(), std::move(view),
...
void Device::drawViewLattice(
    GrSurfaceProxyView view, const GrColorInfo& info, std::unique_ptr<SkLatticeIter> iter,
    const SkRect& dst, SkFilterMode filter, const SkPaint& origPaint) {
  ...
  fSurfaceDrawContext->drawImageLattice(
      this->clip(), std::move(grPaint), this->localToDevice(), std::move(view), info.alphaType(),
      std::move(csxf), filter, std::move(iter), dst);
}

void Device::drawImageLattice(
    const SkImage* image, const SkCanvas::Lattice& lattice, const SkRect& dst, SkFilterMode filter,
    const SkPaint& paint) {
  ASSERT_SINGLE_OWNER
...
void Device::drawVertices(
  ...
  if (!init_vertices_paint(
          fContext.get(), fSurfaceDrawContext->colorInfo(), paint, this->asMatrixProvider(),
          std::move(blender), info.hasColors(), &grPaint)) {
    return;
  }
  fSurfaceDrawContext->drawVertices(
      this->clip(), std::move(grPaint), this->asMatrixProvider(),
      sk_ref_sp(const_cast<SkVertices*>(vertices)), nullptr, skipColorXform);
}
...
void Device::drawMesh(const SkMesh& mesh, sk_sp<SkBlender> blender, const SkPaint& paint) {
  ...
  if (!init_vertices_paint(
          fContext.get(), fSurfaceDrawContext->colorInfo(), paint, this->asMatrixProvider(),
          std::move(blender), SkMeshSpecificationPriv::HasColors(*mesh.spec()), &grPaint)) {
    return;
  }
...
void Device::drawAtlas(
    const SkRSXform xform[], const SkRect texRect[], const SkColor colors[], int count,
  ...
  if (colors) {
    if (!SkPaintToGrPaintWithBlend(
            this->recordingContext(), fSurfaceDrawContext->colorInfo(), paint,
            this->asMatrixProvider(), blender.get(), &grPaint)) {
      return;
    }
  } else {
    if (!SkPaintToGrPaint(
...
void Device::testingOnly_drawGlyphRunListWithSlug(
...
void Device::testingOnly_drawGlyphRunListWithSerializedSlug(
  ...
  auto srcSlug =
      Slug::ConvertBlob(canvas, *glyphRunList.blob(), glyphRunList.origin(), initialPaint);
...
class DiscardableManager : public SkStrikeServer::DiscardableHandleManager,
 ...
 public:
  ...
  bool deleteHandle(SkDiscardableHandleId id) override SK_EXCLUDES(fLock) { return false; }
  ...
  bool isHandleDeleted(SkDiscardableHandleId id) override SK_EXCLUDES(fLock) { return false; }
...
void Device::testingOnly_drawGlyphRunListWithSerializedSlugAndStrike(
  ...
  auto analysisCanvas = server.makeAnalysisCanvas(
      canvas->getBaseLayerSize().width(), canvas->getBaseLayerSize().width(), surfaceProps,
      std::move(colorSpace), useDFT);

  analysisCanvas->setMatrix(canvas->getTotalMatrix());
  auto srcSlug = Slug::ConvertBlob(
      analysisCanvas.get(), *glyphRunList.blob(), glyphRunList.origin(), initialPaint);
...
void Device::onDrawGlyphRunList(
  ...
  if (glyphRunList.blob() == nullptr) {
    ...
    fSurfaceDrawContext->drawGlyphRunList(
        canvas, this->clip(), this->asMatrixProvider(), glyphRunList, this->strikeDeviceInfo(),
        drawingPaint);
  }
#endif
}
...
void Device::drawDrawable(SkCanvas* canvas, SkDrawable* drawable, const SkMatrix* matrix) {
  ...
  if (GrBackendApi::kVulkan == api) {
    ...
    std::unique_ptr<SkDrawable::GpuDrawHandler> gpuDraw =
        drawable->snapGpuDrawHandler(api, combinedMatrix, this->devClipBounds(), this->imageInfo());
    if (gpuDraw) {
...
bool Device::wait(
  ...
  return fSurfaceDrawContext->waitOnSemaphores(
      numSemaphores, waitSemaphores, deleteSemaphoresAfterWait);
}

bool Device::replaceBackingProxy(
    SkSurface::ContentChangeMode mode, sk_sp<GrRenderTargetProxy> newRTP, GrColorType grColorType,
    sk_sp<SkColorSpace> colorSpace, GrSurfaceOrigin origin, const SkSurfaceProps& props) {
  auto sdc = SurfaceDrawContext::Make(
      fContext.get(), grColorType, std::move(newRTP), std::move(colorSpace), origin, props);
  if (!sdc) {
    return false;
  }
  ...
  if (mode == SkSurface::kRetain_ContentChangeMode) {
    ...
    SkAssertResult(sdc->blitTexture(
        fSurfaceDrawContext->readSurfaceView(), SkIRect::MakeWH(this->width(), this->height()),
        SkIPoint::Make(0, 0)));
  }
...
void Device::asyncRescaleAndReadPixelsYUV420(
    ...
    SkISize dstSize, RescaleGamma rescaleGamma, RescaleMode rescaleMode,
  ...
  sdc->asyncRescaleAndReadPixelsYUV420(
      dContext, yuvColorSpace, std::move(dstColorSpace), srcRect, dstSize, rescaleGamma,
      rescaleMode, callback, context);
}
...
SkBaseDevice* Device::onCreateDevice(const CreateInfo& cinfo, const SkPaint*) {
  ...
  auto sdc = SurfaceDrawContext::MakeWithFallback(
      ...
      fSurfaceDrawContext->asSurfaceProxy()->isProtected(), fSurfaceDrawContext->origin(),
      SkBudgeted::kYes);
  if (!sdc) {
    return nullptr;
  }
...
sk_sp<SkSurface> Device::makeSurface(const SkImageInfo& info, const SkSurfaceProps& props) {
  ...
  return SkSurface::MakeRenderTarget(
      fContext.get(), kBudgeted, info, fSurfaceDrawContext->numSamples(),
...
bool Device::android_utils_clipWithStencil() {
  ...
  GrAA aa = GrAA(fSurfaceDrawContext->alwaysAntialias());
  sdc->drawRegion(
      nullptr, std::move(grPaint), aa, SkMatrix::I(), clipRegion, GrStyle::SimpleFill(),
      &kDrawToStencil);
  return true;
}
