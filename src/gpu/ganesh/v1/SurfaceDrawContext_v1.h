...
class SurfaceDrawContext final : public SurfaceFillContext {
 public:
  ...
  static std::unique_ptr<SurfaceDrawContext> Make(
      ...
      const GrBackendFormat&, int sampleCnt, GrMipmapped, GrProtected, skgpu::Swizzle readSwizzle,
      skgpu::Swizzle writeSwizzle, GrSurfaceOrigin, SkBudgeted, const SkSurfaceProps&);
  ...
  static std::unique_ptr<SurfaceDrawContext> MakeFromBackendTexture(
      ...
      GrSurfaceOrigin, const SkSurfaceProps&, sk_sp<skgpu::RefCntedCallback> releaseHelper);
  ...
  void fillRectWithEdgeAA(
      const GrClip* clip, GrPaint&& paint, GrQuadAAFlags edgeAA, const SkMatrix& viewMatrix,
      const SkRect& rect, const SkRect* optionalLocalRect = nullptr) {
    if (edgeAA == GrQuadAAFlags::kAll) {
      this->fillRectToRect(
          clip, std::move(paint), GrAA::kYes, viewMatrix, rect,
  ...
  void drawQuadSet(
      const GrClip* clip, GrPaint&& paint, const SkMatrix& viewMatrix, const GrQuadSetEntry[],
      int cnt);
  ...
  void drawTexture(
      ...
      const SkRect& dstRect, GrQuadAAFlags, SkCanvas::SrcRectConstraint, const SkMatrix&,
  ...
  void drawTextureQuad(
      ...
      const SkPoint dstQuad[4], GrQuadAAFlags edgeAA, const SkRect* subset,
    ...
    this->drawTexturedQuad(
        clip, std::move(view), srcAlphaType, std::move(texXform), filter, mm, color, mode, &quad,
        subset);
  }
  ...
  void drawTextureSet(
      ...
      GrSamplerState::MipmapMode, SkBlendMode mode, SkCanvas::SrcRectConstraint,
      const SkMatrix& viewMatrix, sk_sp<GrColorSpaceXform> texXform);
   ...
   */
  void drawMesh(
      const GrClip*, GrPaint&& paint, const SkMatrixProvider& matrixProvider, const SkMesh& mesh);
  ...
  void drawGlyphRunList(
      SkCanvas*, const GrClip*, const SkMatrixProvider& viewMatrix,
      const SkGlyphRunList& glyphRunList, SkStrikeDeviceInfo strikeDeviceInfo,
      const SkPaint& paint);
 ...
 private:
  ...
  void drawFilledQuad(
      const GrClip* clip, GrPaint&& paint, DrawQuad* quad,
  ...
  void drawTexturedQuad(
      ...
      GrSamplerState::MipmapMode, const SkPMColor4f& color, SkBlendMode blendMode, DrawQuad* quad,
