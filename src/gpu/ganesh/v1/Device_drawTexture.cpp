...
bool may_color_bleed(
    const SkRect& srcRect, const SkRect& transformedRect, const SkMatrix& m, int numSamples) {
...
bool can_ignore_linear_filtering_subset(
    const SkRect& srcSubset, const SkMatrix& srcRectToDeviceSpace, int numSamples) {
...
SkIRect determine_clipped_src_rect(
...
bool should_tile_image_id(
    GrRecordingContext* context, SkISize rtSize, const GrClip* clip, uint32_t imageID,
  ...
  if (imageSize.width() > maxTileSize || imageSize.height() > maxTileSize) {
    *clippedSubset = determine_clipped_src_rect(
        rtSize.width(), rtSize.height(), clip, ctm, srcToDst, imageSize, src);
...
inline void clamped_outset_with_offset(
 ...
 */
ImageDrawMode optimize_sample_area(
    const SkISize& image, const SkRect* origSrcRect, const SkRect* origDstRect,
    const SkPoint dstClip[4], SkRect* outSrcRect, SkRect* outDstRect, SkMatrix* srcToDst) {
    ...
    if (dstClip) {
      for (int i = 0; i < 4; ++i) {
        if (!dst.contains(dstClip[i].fX, dstClip[i].fY)) {
          ...
          *outSrcRect = src;
          *outDstRect = (origDstRect ? *origDstRect : (origSrcRect ? *origSrcRect : srcBounds));
...
bool can_use_draw_texture(const SkPaint& paint, const SkSamplingOptions& sampling) {
  return (
      ...
      !paint.getImageFilter() && !paint.getBlender() && !sampling.isAniso() && !sampling.useCubic &&
      sampling.mipmap == SkMipmapMode::kNone);
}

SkPMColor4f texture_color(
    SkColor4f paintColor, float entryAlpha, GrColorType srcColorType,
...
void draw_texture(
    skgpu::v1::SurfaceDrawContext* sdc, const GrClip* clip, const SkMatrix& ctm,
    const SkPaint& paint, GrSamplerState::Filter filter, const SkRect& srcRect,
    const SkRect& dstRect, const SkPoint dstClip[4], GrQuadAAFlags aaFlags,
  ...
  if (dstClip) {
    ...
    sdc->drawTextureQuad(
        clip, std::move(view), srcColorInfo.colorType(), srcColorInfo.alphaType(), filter,
        GrSamplerState::MipmapMode::kNone, paint.getBlendMode_or(SkBlendMode::kSrcOver), color,
        srcQuad, dstClip, aaFlags,
        constraint == SkCanvas::kStrict_SrcRectConstraint ? &srcRect : nullptr, ctm,
        std::move(textureXform));
  } else {
    sdc->drawTexture(
        clip, std::move(view), srcColorInfo.alphaType(), filter, GrSamplerState::MipmapMode::kNone,
        paint.getBlendMode_or(SkBlendMode::kSrcOver), color, srcRect, dstRect, aaFlags, constraint,
...
// Assumes srcRect and dstRect have already been optimized to fit the proxy.
void draw_image(
    GrRecordingContext* rContext, skgpu::v1::SurfaceDrawContext* sdc, const GrClip* clip,
    const SkMatrixProvider& matrixProvider, const SkPaint& paint, const SkImage_Base& image,
    const SkRect& src, const SkRect& dst, const SkPoint dstClip[4], const SkMatrix& srcToDst,
    GrQuadAAFlags aaFlags, SkCanvas::SrcRectConstraint constraint, SkSamplingOptions sampling,
  ...
  if (tm == SkTileMode::kClamp && !image.isYUVA() && can_use_draw_texture(paint, sampling)) {
    ...
    draw_texture(
        sdc, clip, ctm, paint, sampling.filter, src, dst, dstClip, aaFlags, constraint,
        std::move(view), info);
    return;
  }
  ...
  std::unique_ptr<GrFragmentProcessor> fp =
      image.asFragmentProcessor(rContext, sampling, tileModes, textureMatrix, subset, domain);
  fp =
      GrColorSpaceXformEffect::Make(std::move(fp), image.imageInfo().colorInfo(), sdc->colorInfo());
  if (image.isAlphaOnly()) {
    if (const auto* shader = as_SB(paint.getShader())) {
      auto shaderFP =
          shader->asFragmentProcessor(GrFPArgs(rContext, matrixProvider, &sdc->colorInfo()));
      if (!shaderFP) {
        return;
      }
      fp = GrBlendFragmentProcessor::Make<SkBlendMode::kDstIn>(std::move(fp), std::move(shaderFP));
    } else {
  ...
  if (!mf) {
    ...
    if (dstClip) {
      ...
      sdc->fillRectWithEdgeAA(
          clip, std::move(grPaint), aaFlags, ctm, dst,
    ...
    GrBlurUtils::drawShapeWithMaskFilter(rContext, sdc, clip, shape, std::move(grPaint), ctm, mf);
  }
}

void draw_tiled_bitmap(
    GrRecordingContext* rContext, skgpu::v1::SurfaceDrawContext* sdc, const GrClip* clip,
    const SkBitmap& bitmap, int tileSize, const SkMatrixProvider& matrixProvider,
    const SkMatrix& srcToDst, const SkRect& srcRect, const SkIRect& clippedSrcIRect,
    const SkPaint& paint, GrQuadAAFlags origAAFlags, SkCanvas::SrcRectConstraint constraint,
    SkSamplingOptions sampling, SkTileMode tileMode) {
  if (sampling.isAniso()) {
  ...
  for (int x = 0; x <= nx; x++) {
    for (int y = 0; y <= ny; y++) {
      ...
      if (SkBitmap subsetBmp; bitmap.extractSubset(&subsetBmp, iTileR)) {
        ...
        SkASSERT(
            image->width() <= rContext->priv().caps()->maxTextureSize() &&
        ...
        draw_image(
            rContext, sdc, clip, matrixProvider, paint, *as_IB(image.get()), tileR, rectToDraw,
            nullptr, offsetSrcToDst, aaFlags, constraint, sampling, tileMode);
      }
    }
  }
}
...
void Device::drawSpecial(
    SkSpecialImage* special, const SkMatrix& localToDevice, const SkSamplingOptions& origSampling,
    const SkPaint& paint) {
  ...
  SkColorInfo colorInfo(
      special->colorType(), special->alphaType(), sk_ref_sp(special->getColorSpace()));

  GrSurfaceProxyView view = special->view(this->recordingContext());
  SkImage_Gpu image(
      sk_ref_sp(special->getContext()), special->uniqueID(), std::move(view), std::move(colorInfo));
  ...
  draw_image(
      fContext.get(), fSurfaceDrawContext.get(), this->clip(), matrixProvider, paint, image, src,
      dst, nullptr, srcToDst, aaFlags, SkCanvas::kStrict_SrcRectConstraint, sampling);
}

void Device::drawImageQuad(
    const SkImage* image, const SkRect* srcRect, const SkRect* dstRect, const SkPoint dstClip[4],
    GrQuadAAFlags aaFlags, const SkMatrix* preViewMatrix, const SkSamplingOptions& origSampling,
    const SkPaint& paint, SkCanvas::SrcRectConstraint constraint) {
  SkRect src;
  SkRect dst;
  SkMatrix srcToDst;
  ImageDrawMode mode = optimize_sample_area(
      SkISize::Make(image->width(), image->height()), srcRect, dstRect, dstClip, &src, &dst,
      &srcToDst);
  ...
  SkPreConcatMatrixProvider matrixProvider(
      this->asMatrixProvider(), preViewMatrix ? *preViewMatrix : SkMatrix::I());
  const SkMatrix& ctm(matrixProvider.localToDevice());
  ...
  if (!image->isTextureBacked() && !as_IB(image)->isPinnedOnContext(fContext.get())) {
    ...
    if (should_tile_image_id(
            fContext.get(), fSurfaceDrawContext->dimensions(), clip, image->unique(),
            image->dimensions(), ctm, srcToDst, &src, maxTileSize, &tileSize, &clippedSubset)) {
      ...
      if (SkBitmap bm; as_IB(image)->getROPixels(nullptr, &bm)) {
        ...
        draw_tiled_bitmap(
            fContext.get(), fSurfaceDrawContext.get(), clip, bm, tileSize, matrixProvider, srcToDst,
            src, clippedSubset, paint, aaFlags, constraint, sampling, tileMode);
        return;
      }
    }
  }
  ...
  draw_image(
      fContext.get(), fSurfaceDrawContext.get(), clip, matrixProvider, paint, *as_IB(image), src,
      dst, dstClip, srcToDst, aaFlags, constraint, sampling);
  return;
}

void Device::drawEdgeAAImageSet(
  ...
  if (!can_use_draw_texture(paint, sampling)) {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      this->drawImageQuad(
          ...
          set[i].fHasClip ? dstClips + dstClipIndex : nullptr, SkToGrQuadAAFlags(set[i].fAAFlags),
          set[i].fMatrixIndex < 0 ? nullptr : preViewMatrices + set[i].fMatrixIndex, sampling,
  ...
  auto draw = [&](int nextBase) {
    if (n > 0) {
      auto textureXform = GrColorSpaceXform::Make(
          set[base].fImage->imageInfo().colorInfo(), fSurfaceDrawContext->colorInfo());
      fSurfaceDrawContext->drawTextureSet(
          this->clip(), textures.get() + base, n, p, filter, GrSamplerState::MipmapMode::kNone,
          mode, constraint, this->localToDevice(), std::move(textureXform));
    }
    base = nextBase;
    n = 0;
    p = 0;
  ...
  for (int i = 0; i < count; ++i) {
    ...
    if (!image->isYUVA()) {
      ...
      if (image->isAlphaOnly()) {
        skgpu::Swizzle swizzle = skgpu::Swizzle::Concat(view.swizzle(), skgpu::Swizzle("aaaa"));
    ...
    if (!view) {
      ...
      this->drawImageQuad(
          image, &set[i].fSrcRect, &set[i].fDstRect, clip, SkToGrQuadAAFlags(set[i].fAAFlags),
          set[i].fMatrixIndex < 0 ? nullptr : preViewMatrices + set[i].fMatrixIndex, sampling,
          *entryPaint, constraint);
      continue;
    }
    ...
    textures[i].fColor = texture_color(
        paint.getColor4f(), set[i].fAlpha, SkColorTypeToGrColorType(image->colorType()),
