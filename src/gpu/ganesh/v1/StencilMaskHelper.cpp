...
static constexpr GrUserStencilSettings gUserToClipReplace(
    GrUserStencilSettings::StaticInit<
        0x0000, GrUserStencilTest::kNotEqual, 0xffff, GrUserStencilOp::kSetClipAndReplaceUserBits,
        GrUserStencilOp::kZeroClipAndUserBits, 0xffff>());

static constexpr GrUserStencilSettings gInvUserToClipReplace(
    GrUserStencilSettings::StaticInit<
        0x0000, GrUserStencilTest::kEqual, 0xffff, GrUserStencilOp::kSetClipAndReplaceUserBits,
        GrUserStencilOp::kZeroClipAndUserBits, 0xffff>());
...
static constexpr GrUserStencilSettings gUserToClipIsect(
    GrUserStencilSettings::StaticInit<
        ...
        0xffff, GrUserStencilOp::kSetClipAndReplaceUserBits, GrUserStencilOp::kZeroClipAndUserBits,
        0xffff>());
...
static constexpr GrUserStencilSettings gUserToClipDiff(
    GrUserStencilSettings::StaticInit<
        0x0000, GrUserStencilTest::kEqualIfInClip, 0xffff,
        GrUserStencilOp::kSetClipAndReplaceUserBits, GrUserStencilOp::kZeroClipAndUserBits,
        0xffff>());
...
static constexpr GrUserStencilSettings gInvUserToClipUnionPass0(  // Does not zero user bits.
    GrUserStencilSettings::StaticInit<
        0x0000, GrUserStencilTest::kEqual, 0xffff, GrUserStencilOp::kSetClipBit,
...
static constexpr GrUserStencilSettings gUserToClipXorPass0(  // Does not zero user bits.
    GrUserStencilSettings::StaticInit<
        0x0000, GrUserStencilTest::kNotEqual, 0xffff, GrUserStencilOp::kInvertClipBit,
        GrUserStencilOp::kKeep, 0x0000>());

static constexpr GrUserStencilSettings gInvUserToClipXorPass0(  // Does not zero user bits.
    GrUserStencilSettings::StaticInit<
        0x0000, GrUserStencilTest::kEqual, 0xffff, GrUserStencilOp::kInvertClipBit,
        GrUserStencilOp::kKeep, 0x0000>());
...
static constexpr GrUserStencilSettings gXorClip(
    GrUserStencilSettings::StaticInit<
        0x0000, GrUserStencilTest::kAlways, 0xffff, GrUserStencilOp::kInvertClipBit,
        GrUserStencilOp::kInvertClipBit, 0x0000>());

static constexpr GrUserStencilSettings gDiffClip(
    GrUserStencilSettings::StaticInit<
        0x0000, GrUserStencilTest::kAlwaysIfInClip, 0xffff, GrUserStencilOp::kZeroClipBit,
        GrUserStencilOp::kKeep, 0x0000>());
...
static constexpr GrUserStencilSettings gDrawToStencil(
    GrUserStencilSettings::StaticInit<
        0x0000, GrUserStencilTest::kAlways, 0xffff, GrUserStencilOp::kIncMaybeClamp,
...
GrUserStencilSettings const* const* get_stencil_passes(
    SkRegion::Op op, skgpu::v1::PathRenderer::StencilSupport stencilSupport, bool fillInverted,
...
void draw_stencil_rect(
    skgpu::v1::SurfaceDrawContext* sdc, const GrHardClip& clip, const GrUserStencilSettings* ss,
...
void draw_path(
    GrRecordingContext* rContext, skgpu::v1::SurfaceDrawContext* sdc, skgpu::v1::PathRenderer* pr,
    const GrHardClip& clip, const SkIRect& bounds, const GrUserStencilSettings* ss,
    const SkMatrix& matrix, const GrStyledShape& shape, GrAA aa) {
  GrPaint paint;
  paint.setXPFactory(GrDisableColorXPFactory::Get());
  ...
  skgpu::v1::PathRenderer::DrawPathArgs args{rContext, std::move(paint), ss,     sdc,        &clip,
                                             &bounds,  &matrix,          &shape, pathAAType, false};
  pr->drawPath(args);
}

void stencil_path(
    GrRecordingContext* rContext, skgpu::v1::SurfaceDrawContext* sdc, skgpu::v1::PathRenderer* pr,
    const GrFixedClip& clip, const SkMatrix& matrix, const GrStyledShape& shape, GrAA aa) {
...
StencilMaskHelper::StencilMaskHelper(GrRecordingContext* rContext, SurfaceDrawContext* sdc)
    : fContext(rContext), fSDC(sdc), fClip(sdc->dimensions()) {}

bool StencilMaskHelper::init(
    const SkIRect& bounds, uint32_t genID, const GrWindowRectangles& windowRects, int numFPs) {
...
void StencilMaskHelper::drawRect(
    const SkRect& rect, const SkMatrix& matrix, SkRegion::Op op, GrAA aa) {
  if (rect.isEmpty()) {
    return;
  }
  ...
  bool drawDirectToClip;
  auto passes =
      get_stencil_passes(op, PathRenderer::kNoRestriction_StencilSupport, false, &drawDirectToClip);
  ...
  for (GrUserStencilSettings const* const* pass = passes; *pass; ++pass) {
    if (drawDirectToClip) {
      ...
      draw_stencil_rect(
          fSDC, fClip, *pass, SkMatrix::I(), SkRect::Make(fClip.fixedClip().scissorRect()), aa);
    }
  }
}

bool StencilMaskHelper::drawPath(
    const SkPath& path, const SkMatrix& matrix, SkRegion::Op op, GrAA aa) {
  if (path.isEmpty()) {
    return true;
  }
  ...
  if (!drawDirectToClip) {
    if (stencilSupport == PathRenderer::kNoRestriction_StencilSupport) {
      draw_path(
          fContext, fSDC, pr, fClip.fixedClip(), fClip.fixedClip().scissorRect(), &gDrawToStencil,
          matrix, shape, aa);
    } else {
  ...
  for (GrUserStencilSettings const* const* pass = passes; *pass; ++pass) {
    if (drawDirectToClip) {
      draw_path(
          fContext, fSDC, pr, fClip, fClip.fixedClip().scissorRect(), *pass, matrix, shape, aa);
    } else {
      draw_stencil_rect(
          fSDC, fClip, *pass, SkMatrix::I(), SkRect::Make(fClip.fixedClip().scissorRect()), aa);
    }
...
bool StencilMaskHelper::drawShape(
    const GrShape& shape, const SkMatrix& matrix, SkRegion::Op op, GrAA aa) {
...
void StencilMaskHelper::clear(bool insideStencil) {
  if (fClip.fixedClip().hasWindowRectangles()) {
    ...
    draw_stencil_rect(
        fSDC, fClip.fixedClip(), GrStencilSettings::SetClipBitSettings(insideStencil),
