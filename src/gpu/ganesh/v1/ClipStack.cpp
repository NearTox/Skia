...
enum class ClipGeometry { kEmpty, kAOnly, kBOnly, kBoth };
...
bool shape_contains_rect(
    const GrShape& a, const SkMatrix& aToDevice, const SkMatrix& deviceToA, const SkRect& b,
    const SkMatrix& bToDevice, bool mixedAAMode) {
  if (!a.convex()) {
    return false;
  }
...
GrClipEdgeType get_clip_edge_type(SkClipOp op, GrAA aa) noexcept {
...
GrFPResult analytic_clip_fp(
    const skgpu::v1::ClipStack::Element& e, const GrShaderCaps& caps,
...
GrFPResult clip_atlas_fp(
    const skgpu::v1::SurfaceDrawContext* sdc, const GrOp* opBeingClipped,
    skgpu::v1::AtlasPathRenderer* atlasPathRenderer, const SkIRect& scissorBounds,
    const skgpu::v1::ClipStack::Element& e, std::unique_ptr<GrFragmentProcessor> inputFP) {
  if (e.fAA != GrAA::kYes) {
  ...
  return atlasPathRenderer->makeAtlasClipEffect(
      sdc, opBeingClipped, std::move(inputFP), scissorBounds, e.fLocalToDevice, path);
}

void draw_to_sw_mask(
    GrSWMaskHelper* helper, const skgpu::v1::ClipStack::Element& e, bool clearMask) {
...
GrSurfaceProxyView render_sw_mask(
    GrRecordingContext* context, const SkIRect& bounds,
    const skgpu::v1::ClipStack::Element** elements, int count) {
  SkASSERT(count > 0);
  ...
  if (taskGroup) {
    ...
    skgpu::Swizzle swizzle = context->priv().caps()->getReadSwizzle(format, GrColorType::kAlpha_8);
    auto proxy = proxyProvider->createProxy(
        format, bounds.size(), GrRenderable::kNo, 1, GrMipmapped::kNo, SkBackingFit::kApprox,
...
void render_stencil_mask(
    GrRecordingContext* rContext, skgpu::v1::SurfaceDrawContext* sdc, uint32_t genID,
    const SkIRect& bounds, const skgpu::v1::ClipStack::Element** elements, int count,
    GrAppliedClip* out) {
...
class ClipStack::Draw {
 public:
  Draw(const SkRect& drawBounds, GrAA aa)
      : fBounds(GrClip::GetPixelIBounds(drawBounds, aa, BoundsType::kExterior)), fAA(aa) {
  ...
  bool applyDeviceBounds(const SkIRect& deviceBounds) { return fBounds.intersect(deviceBounds); }
...
ClipStack::RawElement::RawElement(
    const SkMatrix& localToDevice, const GrShape& shape, GrAA aa, SkClipOp op)
...
bool ClipStack::RawElement::contains(const Draw& d) const {
  if (fInnerBounds.contains(d.outerBounds())) {
    ...
    return shape_contains_rect(
        fShape, fLocalToDevice, fDeviceToLocal, queryBounds, SkMatrix::I(), /* mixed-aa */ false);
  }
}
...
bool ClipStack::RawElement::contains(const RawElement& e) const {
  ...
  return shape_contains_rect(
      fShape, fLocalToDevice, fDeviceToLocal, e.fShape.bounds(), e.fLocalToDevice, mixedAA);
}
...
bool ClipStack::RawElement::combine(const RawElement& other, const SaveRecord& current) {
  ...
  if (fShape.isRect() && other.fShape.isRect()) {
    ...
    if (aaMatch && fLocalToDevice == other.fLocalToDevice) {
      ...
      (fShape.isRect() || fShape.isRRect()) && (other.fShape.isRect() || other.fShape.isRRect())) {
    ...
    if (fAA == other.fAA && fLocalToDevice == other.fLocalToDevice) {
      ...
      SkRRect b =
          other.fShape.isRect() ? SkRRect::MakeRect(other.fShape.rect()) : other.fShape.rrect();
...
ClipStack::ClipState ClipStack::RawElement::clipType() const {
  ...
  switch (fShape.type()) {
    case GrShape::Type::kEmpty: return ClipState::kEmpty;

    case GrShape::Type::kRect:
      return fOp == SkClipOp::kIntersect && fLocalToDevice.isIdentity() ? ClipState::kDeviceRect
                                                                        : ClipState::kComplex;

    case GrShape::Type::kRRect:
      return fOp == SkClipOp::kIntersect && fLocalToDevice.isIdentity() ? ClipState::kDeviceRRect
                                                                        : ClipState::kComplex;
    ...
    case GrShape::Type::kPath: return ClipState::kComplex;
  }
  SkUNREACHABLE;
}
...
ClipStack::SaveRecord::SaveRecord(const SkIRect& deviceBounds) noexcept
...
ClipStack::SaveRecord::SaveRecord(
    const SaveRecord& prior, int startingMaskIndex, int startingElementIndex) noexcept
...
uint32_t ClipStack::SaveRecord::genID() const noexcept {
...
void ClipStack::SaveRecord::invalidateMasks(GrProxyProvider* proxyProvider, Mask::Stack* masks) {
...
bool ClipStack::SaveRecord::addElement(RawElement&& toAdd, RawElement::Stack* elements) {
  ...
  SkASSERT(
      (toAdd.shape().isEmpty() || !toAdd.outerBounds().isEmpty()) &&
  ...
  SkASSERT(
      !fOuterBounds.isEmpty() && (fInnerBounds.isEmpty() || fOuterBounds.contains(fInnerBounds)));
...
bool ClipStack::SaveRecord::appendElement(RawElement&& toAdd, RawElement::Stack* elements) {
  ...
  SkASSERT(
      oldestValid == elements->count() ||
      (oldestValid >= fOldestValidIndex && oldestValid < elements->count()));
  SkASSERT(
      youngestValid == fStartingElementIndex - 1 ||
      (youngestValid >= fStartingElementIndex && youngestValid < elements->count()));
  SkASSERT(
      (oldestActiveInvalid && oldestActiveInvalidIndex >= fStartingElementIndex &&
       oldestActiveInvalidIndex < elements->count()) ||
      !oldestActiveInvalid);
...
ClipStack::ClipStack(
    const SkIRect& deviceBounds, const SkMatrixProvider* matrixProvider, bool forceAA)
...
GrClip::Effect ClipStack::apply(
    GrRecordingContext* rContext, SurfaceDrawContext* sdc, GrDrawOp* op, GrAAType aa,
  ...
  switch (get_clip_geometry(cs, draw)) {
    case ClipGeometry::kEmpty: return Effect::kClippedOut;
    ...
    case ClipGeometry::kBoth:
      ...
      SkASSERT(
          cs.state() == ClipState::kDeviceRect || cs.state() == ClipState::kDeviceRRect ||
  ...
  for (const RawElement& e : fElements.ritems()) {
    ...
    switch (get_clip_geometry(e, draw)) {
      ...
      case ClipGeometry::kBoth: {
        ...
        auto result = op->clipToShape(
            sdc, e.op(), e.localToDevice(), e.shape(), GrAA(e.aa() == GrAA::kYes || fForceAA));
        ...
        if (!fullyApplied) {
          if (e.op() == SkClipOp::kIntersect) {
            ...
            fullyApplied =
                e.innerBounds() == e.outerBounds() || e.innerBounds().contains(scissorBounds);
          } else {
        ...
        if (!fullyApplied && remainingAnalyticFPs > 0) {
          std::tie(fullyApplied, clipFP) =
              analytic_clip_fp(e.asElement(), *caps->shaderCaps(), std::move(clipFP));
          if (!fullyApplied && atlasPathRenderer) {
            std::tie(fullyApplied, clipFP) = clip_atlas_fp(
                sdc, op, atlasPathRenderer, scissorBounds, e.asElement(), std::move(clipFP));
          }
          if (fullyApplied) {
  ...
  if (!elementsForMask.empty()) {
    bool stencilUnavailable = !sdc->asRenderTargetProxy()->canUseStencil(*rContext->priv().caps());
    ...
    if ((sdc->numSamples() <= 1 && !sdc->canUseDynamicMSAA() && maskRequiresAA) ||
      ...
      std::tie(hasSWMask, clipFP) = GetSWMaskFP(
          rContext, &fMasks, cs, scissorBounds, elementsForMask.begin(), elementsForMask.count(),
          std::move(clipFP));
    }
    ...
    if (!hasSWMask) {
      if (stencilUnavailable) {
        ...
        render_stencil_mask(
            rContext, sdc, cs.genID(), scissorBounds, elementsForMask.begin(),
            elementsForMask.count(), out);
      }
    }
  }
...
GrFPResult ClipStack::GetSWMaskFP(
    GrRecordingContext* context, Mask::Stack* masks, const SaveRecord& current,
  ...
  auto fp = GrTextureEffect::MakeSubset(
      std::move(maskProxy), kPremul_SkAlphaType, m, samplerState, subset, domain,
      *context->priv().caps());
