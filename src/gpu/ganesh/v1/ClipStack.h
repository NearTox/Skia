...
class ClipStack final : public GrClip {
 public:
  ...
  GrClip::Effect apply(
      GrRecordingContext*, skgpu::v1::SurfaceDrawContext*, GrDrawOp*, GrAAType, GrAppliedClip*,
 ...
 private:
  ...
  class RawElement : private Element {
   public:
    ...
    SkClipOp op() const noexcept { return fOp; }
    const SkIRect& outerBounds() const noexcept { return fOuterBounds; }
    ...
    const Element& asElement() const noexcept { return *this; }

    const GrShape& shape() const noexcept { return fShape; }
    const SkMatrix& localToDevice() const noexcept { return fLocalToDevice; }
    const SkIRect& innerBounds() const noexcept { return fInnerBounds; }
    GrAA aa() const noexcept { return fAA; }
    ...
    bool isInvalid() const noexcept { return fInvalidatedByIndex >= 0; }
  ...
  class Mask {
   public:
    ...
    const UniqueKey& key() const noexcept { return fKey; }
    const SkIRect& bounds() const noexcept { return fBounds; }
    uint32_t genID() const noexcept { return fGenID; }
  ...
  class SaveRecord {
   public:
    ...
    explicit SaveRecord(const SkIRect& deviceBounds) noexcept;

    SaveRecord(const SaveRecord& prior, int startingMaskIndex, int startingElementIndex) noexcept;

    // The common clip type interface
    SkClipOp op() const noexcept { return fStackOp; }
    const SkIRect& outerBounds() const noexcept { return fOuterBounds; }
    bool contains(const Draw& d) const;
    bool contains(const RawElement& e) const;

    // Additional save record-specific data/functionality
    const SkShader* shader() const noexcept { return fShader.get(); }
    const SkIRect& innerBounds() const noexcept { return fInnerBounds; }
    int firstActiveElementIndex() const noexcept { return fStartingElementIndex; }
    int oldestElementIndex() const noexcept { return fOldestValidIndex; }
    bool canBeUpdated() const noexcept { return (fDeferredSaveCount == 0); }

    ClipState state() const;
    uint32_t genID() const noexcept;

    // Deferred save manipulation
    void pushSave() noexcept {
      SkASSERT(fDeferredSaveCount >= 0);
      fDeferredSaveCount++;
    }
    // Returns true if the record should stay alive. False means the ClipStack must delete it
    bool popSave() noexcept {
      fDeferredSaveCount--;
...
class ClipStack::ElementIter {
 public:
  ...
  ElementIter(RawElement::Stack::CRIter::Item item, int r) noexcept : fItem(item), fRemaining(r) {}
...
ClipStack::ElementIter ClipStack::end() const { return ElementIter(fElements.ritems().end(), 0); }
