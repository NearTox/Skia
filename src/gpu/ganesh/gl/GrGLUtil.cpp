...
static GrGLRenderer get_renderer(const char* rendererString, const GrGLExtensions& extensions) {
  ...
  if (1 == n) {
    if (adrenoNumber >= 300) {
      ...
      if (adrenoNumber < 500) {
        return adrenoNumber >= 430 ? GrGLRenderer::kAdreno430 : GrGLRenderer::kAdreno4xx_other;
      }
      if (adrenoNumber < 600) {
        return adrenoNumber == 530 ? GrGLRenderer::kAdreno530 : GrGLRenderer::kAdreno5xx_other;
      }
      if (adrenoNumber < 700) {
        if (adrenoNumber == 615) {
...
static bool is_commamd_buffer(const char* rendererString, const char* versionString) {
  ...
  return (
      0 == strcmp(rendererString, kChromium) ||
      (3 == sscanf(versionString, "OpenGL ES %d.%d %8s", &major, &minor, suffix) &&
       0 == strcmp(kChromium, suffix)));
}

static std::tuple<GrGLDriver, GrGLDriverVersion> get_driver_and_version(
    GrGLStandard standard, GrGLVendor vendor, const char* vendorString, const char* rendererString,
  ...
  if (!strcmp(vendorString, "freedreno")) {
    ...
    if (vendor == GrGLVendor::kNVIDIA) {
      ...
      int n = sscanf(
          versionString, "%d.%d.%d NVIDIA %d.%d", &major, &minor, &rev, &driverMajor, &driverMinor);
      // Some older NVIDIA drivers don't report the driver version.
      if (n == 5) {
        driverVersion = GR_GL_DRIVER_VER(driverMajor, driverMinor, 0);
      }
    } else {
      int n = sscanf(versionString, "%d.%d Mesa %d.%d", &major, &minor, &driverMajor, &driverMinor);
      if (n != 4) {
        n = sscanf(
            versionString, "%d.%d (Core Profile) Mesa %d.%d", &major, &minor, &driverMajor,
            &driverMinor);
      }
      if (n == 4) {
    ...
    if (vendor == GrGLVendor::kNVIDIA) {
      driver = GrGLDriver::kNVIDIA;
      int n = sscanf(
          versionString, "OpenGL ES %d.%d NVIDIA %d.%d", &major, &minor, &driverMajor,
          &driverMinor);
      // Some older NVIDIA drivers don't report the driver version.
      if (n == 4) {
        driverVersion = GR_GL_DRIVER_VER(driverMajor, driverMinor, 0);
      ...
      int revision;
      int n = sscanf(
          versionString, "OpenGL ES %d.%d build %d.%d@%d", &major, &minor, &driverMajor,
          &driverMinor, &revision);
      if (n == 5) {
      ...
      int n = sscanf(
          versionString, "OpenGL ES %d.%d Mesa %d.%d", &major, &minor, &driverMajor, &driverMinor);
      if (n == 4) {
        driver = GrGLDriver::kMesa;
        driverVersion = GR_GL_DRIVER_VER(driverMajor, driverMinor, 0);
      }
  ...
  if (driver == GrGLDriver::kUnknown) {
    if (vendor == GrGLVendor::kGoogle) {
      ...
      int n = sscanf(
          versionString, "OpenGL ES %d.%d SwiftShader %d.%d.0.%d", &major, &minor, &driverMajor,
      ...
      int n = sscanf(
          versionString, "OpenGL ES %d.%d V@%d.%d", &major, &minor, &driverMajor, &driverMinor);
      if (n == 4) {
        driverVersion = GR_GL_DRIVER_VER(driverMajor, driverMinor, 0);
      }
    } else if (vendor == GrGLVendor::kImagination) {
      ...
      int ignored1;
      int n = sscanf(
          versionString, "OpenGL ES %d.%d v%d.r%dp%d-%drel", &major, &minor, &driverMajor,
          &driverMinor, &ignored0, &ignored1);
      if (n == 6) {
...
get_angle_gl_vendor_and_renderer(const char* innerString, const GrGLExtensions& extensions) {
  SkTArray<SkString> parts;
  SkStrSplit(innerString, ",", &parts);
  // This would need some fixing if we have substrings that contain commas.
  if (parts.size() != 3) {
    return {
        GrGLVendor::kOther, GrGLRenderer::kOther, GrGLDriver::kUnknown, GR_GL_DRIVER_UNKNOWN_VER};
  }
  ...
  auto [angleDriver, angleDriverVersion] = get_driver_and_version(
      kGLES_GrGLStandard, angleVendor, angleVendorString, angleRendererString, angleVersionString);
...
get_angle_d3d_vendor_and_renderer(const char* innerString) {
  ...
  if (strstr(innerString, "Intel")) {
    ...
    if ((modelStr = strstr(innerString, "HD Graphics")) &&
      ...
      switch (modelNumber) {
        case 2000:
        case 3000: renderer = GrGLRenderer::kIntelSandyBridge; break;
        case 4000:
        case 2500: renderer = GrGLRenderer::kIntelSandyBridge; break;
        case 510:
        case 515:
        case 520:
        case 530: renderer = GrGLRenderer::kIntelSkyLake; break;
      }
    } else if (
        (modelStr = strstr(innerString, "Iris")) &&
      ...
      switch (modelNumber) {
        ...
        case 550:
        case 555:
        case 580: renderer = GrGLRenderer::kIntelSkyLake; break;
      }
    }
...
GrGLDriverInfo GrGLGetDriverInfo(const GrGLInterface* interface) {
  ...
  std::tie(info.fDriver, info.fDriverVersion) =
      get_driver_and_version(interface->fStandard, info.fVendor, vendor, renderer, version);
  ...
  if (info.fANGLEBackend == GrGLANGLEBackend::kD3D9 ||
    ...
    std::tie(info.fANGLEVendor, info.fANGLERenderer, info.fANGLEDriver, info.fANGLEDriverVersion) =
        get_angle_d3d_vendor_and_renderer(innerAngleRendererString.c_str());
  } else if (info.fANGLEBackend == GrGLANGLEBackend::kOpenGL) {
    std::tie(info.fANGLEVendor, info.fANGLERenderer, info.fANGLEDriver, info.fANGLEDriverVersion) =
        get_angle_gl_vendor_and_renderer(innerAngleRendererString.c_str(), interface->fExtensions);
  }
...
bool GrGLFormatIsCompressed(GrGLFormat format) {
  switch (format) {
    ...
    case GrGLFormat::kCOMPRESSED_RGBA8_BC1: return true;
