...
GrGLRenderTarget::GrGLRenderTarget(
    ...
    sk_sp<GrGLAttachment> stencil, std::string_view label)
...
GrGLFormat stencil_bits_to_format(int stencilBits) {
  ...
  switch (stencilBits) {
    ...
    case 16: return GrGLFormat::kSTENCIL_INDEX16;
    default: SkASSERT(false); return GrGLFormat::kUnknown;
  }
}
...
sk_sp<GrGLRenderTarget> GrGLRenderTarget::MakeWrapped(
  ...
  if (stencilBits) {
    ...
    // and delete it.
    sb = GrGLAttachment::MakeWrappedRenderBuffer(
        gpu,
        /*renderbufferID=*/0, dimensions, GrAttachment::UsageFlags::kStencilAttachment, sampleCount,
        sFmt);
  }
...
size_t GrGLRenderTarget::onGpuMemorySize() const {
  return GrSurface::ComputeSize(
      this->backendFormat(), this->dimensions(), fTotalMemorySamplesPerPixel, GrMipmapped::kNo);
}
...
bool GrGLRenderTarget::ensureDynamicMSAAAttachment() {
  ...
  fDynamicMSAAAttachment.reset(static_cast<GrGLAttachment*>(
      resourceProvider
          ->getDiscardableMSAAAttachment(
              this->dimensions(), this->backendFormat(), internalSampleCount,
              GrProtected(this->isProtected()), GrMemoryless::kNo)
          .release()));
...
void GrGLRenderTarget::bindInternal(GrGLenum fboTarget, bool useMultisampleFBO) {
  ...
  if (fSingleSampleFBOID != 0 && fSingleSampleFBOID == fMultisampleFBOID &&
    ...
    if (this->getGLGpu()->glCaps().bindTexture0WhenChangingTextureFBOMultisampleCount()) {
      GL_CALL(FramebufferTexture2D(
          fboTarget, GR_GL_COLOR_ATTACHMENT0, GR_GL_TEXTURE_2D, 0 /*texture*/, 0 /*mipMapLevel*/));
    }
    if (useMultisampleFBO) {
      ...
      GL_CALL(FramebufferTexture2DMultisample(
          fboTarget, GR_GL_COLOR_ATTACHMENT0, glTex->target(), glTex->textureID(),
          0 /*mipMapLevel*/, sampleCount));
    } else {
      GL_CALL(FramebufferTexture2D(
          fboTarget, GR_GL_COLOR_ATTACHMENT0, glTex->target(), glTex->textureID(),
          0 /*mipMapLevel*/));
  ...
  if (fNeedsStencilAttachmentBind[useMultisampleFBO]) {
    if (auto stencil = this->getStencilAttachment(useMultisampleFBO)) {
      ...
      GL_CALL(FramebufferRenderbuffer(
          fboTarget, GR_GL_STENCIL_ATTACHMENT, GR_GL_RENDERBUFFER, glStencil->renderbufferID()));
      if (GrGLFormatIsPackedDepthStencil(glStencil->format())) {
        GL_CALL(FramebufferRenderbuffer(
            fboTarget, GR_GL_DEPTH_ATTACHMENT, GR_GL_RENDERBUFFER, glStencil->renderbufferID()));
      } else {
        GL_CALL(FramebufferRenderbuffer(fboTarget, GR_GL_DEPTH_ATTACHMENT, GR_GL_RENDERBUFFER, 0));
      }
    } else {
      GL_CALL(FramebufferRenderbuffer(fboTarget, GR_GL_STENCIL_ATTACHMENT, GR_GL_RENDERBUFFER, 0));
      GL_CALL(FramebufferRenderbuffer(fboTarget, GR_GL_DEPTH_ATTACHMENT, GR_GL_RENDERBUFFER, 0));
    }
#ifdef SK_DEBUG
...
void GrGLRenderTarget::dumpMemoryStatistics(SkTraceMemoryDump* traceMemoryDump) const {
  ...
  if (numSamplesNotInTexture >= 1) {
    size_t size = GrSurface::ComputeSize(
        this->backendFormat(), this->dimensions(), numSamplesNotInTexture, GrMipmapped::kNo);
    ...
    traceMemoryDump->setMemoryBacking(
        resourceName.c_str(), "gl_renderbuffer", renderbuffer_id.c_str());
  }
}
