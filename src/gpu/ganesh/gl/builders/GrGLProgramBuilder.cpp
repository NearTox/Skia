...
sk_sp<GrGLProgram> GrGLProgramBuilder::CreateProgram(
    GrDirectContext* dContext, const GrProgramDesc& desc, const GrProgramInfo& programInfo,
...
GrGLProgramBuilder::GrGLProgramBuilder(
    GrGLGpu* gpu, const GrProgramDesc& desc, const GrProgramInfo& programInfo)
...
SkSL::Compiler* GrGLProgramBuilder::shaderCompiler() const { return fGpu->shaderCompiler(); }

bool GrGLProgramBuilder::compileAndAttachShaders(
    const std::string& glsl, GrGLuint programId, GrGLenum type, SkTDArray<GrGLuint>* shaderIds,
    GrContextOptions::ShaderErrorHandler* errHandler) {
  GrGLGpu* gpu = this->gpu();
  GrGLuint shaderId = GrGLCompileAndAttachShader(
      gpu->glContext(), programId, type, glsl, gpu->pipelineBuilder()->stats(), errHandler);
  if (!shaderId) {
    return false;
  }
...
void GrGLProgramBuilder::computeCountsAndStrides(
    GrGLuint programID, const GrGeometryProcessor& geomProc, bool bindAttribLocations) {
...
void GrGLProgramBuilder::storeShaderInCache(
    const SkSL::Program::Inputs& inputs, GrGLuint programID, const std::string shaders[],
    ...
    switch (shaderType) {
      case kGLPB_Tag: {
        ...
        if (checkLinked) {
          ...
          cached = this->checkLinkStatus(programID, /*errorHandler=*/nullptr, nullptr, nullptr);
        }
        if (cached) {
    ...
    */
    if (glsl[kFragment_GrShaderType].empty()) {
      ...
      std::unique_ptr<SkSL::Program> fs = GrSkSLtoGLSL(
          this->gpu(), SkSL::ProgramKind::kFragment, *sksl[kFragment_GrShaderType], settings,
    ...
    */
    if (glsl[kVertex_GrShaderType].empty()) {
      ...
      std::unique_ptr<SkSL::Program> vs = GrSkSLtoGLSL(
          this->gpu(), SkSL::ProgramKind::kVertex, *sksl[kVertex_GrShaderType], settings,
...
bool GrGLProgramBuilder::checkLinkStatus(
    GrGLuint programID, GrContextOptions::ShaderErrorHandler* errorHandler, std::string* sksl[],
  ...
  if (!linked && errorHandler) {
    ...
    if (sksl) {
      SkSL::String::appendf(
          ...
          "// Vertex SKSL\n%s\n"
          "// Fragment SKSL\n%s\n",
          sksl[kVertex_GrShaderType]->c_str(), sksl[kFragment_GrShaderType]->c_str());
    }
    if (glsl) {
      SkSL::String::appendf(
          &allShaders,
          "// Vertex GLSL\n%s\n"
          "// Fragment GLSL\n%s\n",
          glsl[kVertex_GrShaderType].c_str(), glsl[kFragment_GrShaderType].c_str());
    }
    ...
    const char* errorMsg =
        (infoLen > 0) ? (const char*)log.get() : "link failed but did not provide an info log";
...
sk_sp<GrGLProgram> GrGLProgramBuilder::createProgram(GrGLuint programID) {
  return GrGLProgram::Make(
      ...
      std::move(fGPImpl), std::move(fXPImpl), std::move(fFPImpls), std::move(fAttributes),
      fVertexAttributeCnt, fInstanceAttributeCnt, fVertexStride, fInstanceStride);
}

bool GrGLProgramBuilder::PrecompileProgram(
    GrDirectContext* dContext, GrGLPrecompiledProgram* precompiledProgram,
  ...
  auto compileShader = [&](SkSL::ProgramKind kind, const std::string& sksl, GrGLenum type) {
    ...
    if (GrGLuint shaderID = GrGLCompileAndAttachShader(
            glGpu->glContext(), programID, type, glsl, glGpu->pipelineBuilder()->stats(),
            errorHandler)) {
  ...
  if (!compileShader(
          SkSL::ProgramKind::kFragment, shaders[kFragment_GrShaderType], GR_GL_FRAGMENT_SHADER) ||
      !compileShader(
          SkSL::ProgramKind::kVertex, shaders[kVertex_GrShaderType], GR_GL_VERTEX_SHADER)) {
  ...
  for (int i = 0; i < meta.fAttributeNames.count(); ++i) {
    GR_GL_CALL(
        glGpu->glInterface(), BindAttribLocation(programID, i, meta.fAttributeNames[i].c_str()));
  }
