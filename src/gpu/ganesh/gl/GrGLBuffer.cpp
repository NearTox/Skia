...
sk_sp<GrGLBuffer> GrGLBuffer::Make(
  ...
  sk_sp<GrGLBuffer> buffer(new GrGLBuffer(
...
inline static GrGLenum gr_to_gl_access_pattern(
  ...
  auto readUsage = [](GrAccessPattern pattern) {
    switch (pattern) {
      case kDynamic_GrAccessPattern: return GR_GL_DYNAMIC_READ;
...
void GrGLBuffer::onMap() {
  ...
  switch (this->glCaps().mapBufferType()) {
    case GrGLCaps::kNone_MapBufferType: return;
    ...
    case GrGLCaps::kChromium_MapBufferType: {
      ...
      GL_CALL_RET(
          fMapPtr,
          MapBufferSubData(target, 0, this->size(), readOnly ? GR_GL_READ_ONLY : GR_GL_WRITE_ONLY));
      break;
    }
  }
...
void GrGLBuffer::onUnmap() {
  ...
  switch (this->glCaps().mapBufferType()) {
    case GrGLCaps::kNone_MapBufferType: SkDEBUGFAIL("Shouldn't get here."); return;
...
bool GrGLBuffer::onUpdateData(const void* src, size_t srcSizeInBytes) {
  ...
  if (this->glCaps().useBufferDataNullHint()) {
    if (this->size() == srcSizeInBytes) {
      GrGLenum error = GL_ALLOC_CALL(BufferData(target, (GrGLsizeiptr)srcSizeInBytes, src, fUsage));
      if (error != GR_GL_NO_ERROR) {
        return false;
      }
    } else {
...
void GrGLBuffer::onSetLabel() {
  ...
  if (!this->getLabel().empty()) {
    ...
    if (glGpu->glCaps().debugSupport()) {
      GR_GL_CALL(glGpu->glInterface(), ObjectLabel(GR_GL_BUFFER, fBufferID, -1, label.c_str()));
    }
  }
}

void GrGLBuffer::setMemoryBacking(
