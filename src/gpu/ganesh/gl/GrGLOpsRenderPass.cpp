...
void GrGLOpsRenderPass::set(
    GrRenderTarget* rt, bool useMSAASurface, const SkIRect& contentBounds, GrSurfaceOrigin origin,
...
GrNativeRect GrGLOpsRenderPass::dmsaaLoadStoreBounds() const {
  if (fGpu->glCaps().framebufferResolvesMustBeFullSize()) {
    ...
    return GrNativeRect::MakeRelativeTo(
        fOrigin, fRenderTarget->height(), SkIRect::MakeSize(fRenderTarget->dimensions()));
  } else {
...
void GrGLOpsRenderPass::onBegin() {
  ...
  if (fUseMultisampleFBO && fColorLoadAndStoreInfo.fLoadOp == GrLoadOp::kLoad &&
    ...
    if (fGpu->glCaps().canResolveSingleToMSAA()) {
      fGpu->resolveRenderFBOs(
          glRT, this->dmsaaLoadStoreBounds().asSkIRect(), GrGLGpu::ResolveDirection::kSingleToMSAA);
    } else {
  ...
  fGpu->beginCommandBuffer(
      glRT, fUseMultisampleFBO, fContentBounds, fOrigin, fColorLoadAndStoreInfo,
      fStencilLoadAndStoreInfo);
}

void GrGLOpsRenderPass::onEnd() {
  auto glRT = static_cast<GrGLRenderTarget*>(fRenderTarget);
  fGpu->endCommandBuffer(
      glRT, fUseMultisampleFBO, fColorLoadAndStoreInfo, fStencilLoadAndStoreInfo);
  ...
  if (fUseMultisampleFBO && fColorLoadAndStoreInfo.fStoreOp == GrStoreOp::kStore &&
    ...
    fGpu->resolveRenderFBOs(
        glRT, this->dmsaaLoadStoreBounds().asSkIRect(), GrGLGpu::ResolveDirection::kMSAAToSingle,
...
bool GrGLOpsRenderPass::onBindTextures(
...
void GrGLOpsRenderPass::onBindBuffers(
  ...
  fAttribArrayState =
      fGpu->bindInternalVertexArray(indexBuffer.get(), numAttribs, primitiveRestart);
...
void GrGLOpsRenderPass::bindInstanceBuffer(const GrBuffer* instanceBuffer, int baseInstance) {
  ...
  if (int instanceStride = program->instanceStride()) {
    ...
    for (int i = 0; i < program->numInstanceAttributes(); ++i, ++attribIdx) {
      ...
      fAttribArrayState->set(
          fGpu, attrib.fLocation, instanceBuffer, attrib.fCPUType, attrib.fGPUType, instanceStride,
...
void GrGLOpsRenderPass::bindVertexBuffer(const GrBuffer* vertexBuffer, int baseVertex) {
  ...
  if (int vertexStride = program->vertexStride()) {
    ...
    for (int i = 0; i < program->numVertexAttributes(); ++i) {
      ...
      fAttribArrayState->set(
          fGpu, attrib.fLocation, vertexBuffer, attrib.fCPUType, attrib.fGPUType, vertexStride,
          bufferOffset + attrib.fOffset, kDivisor);
    }
  }
}
...
void GrGLOpsRenderPass::onDrawIndexed(
    int indexCount, int baseIndex, uint16_t minIndexValue, uint16_t maxIndexValue, int baseVertex) {
  GrGLenum glPrimType = fGpu->prepareToDraw(fPrimitiveType);
  if (fGpu->glCaps().baseVertexBaseInstanceSupport()) {
    ...
    if (baseVertex != 0) {
      GL_CALL(DrawElementsInstancedBaseVertexBaseInstance(
          glPrimType, indexCount, GR_GL_UNSIGNED_SHORT, this->offsetForBaseIndex(baseIndex), 1,
          baseVertex, 0));
      return;
    }
  } else {
  ...
  if (fGpu->glCaps().drawRangeElementsSupport()) {
    GL_CALL(DrawRangeElements(
        glPrimType, minIndexValue, maxIndexValue, indexCount, GR_GL_UNSIGNED_SHORT,
        this->offsetForBaseIndex(baseIndex)));
  } else {
    GL_CALL(DrawElements(
        glPrimType, indexCount, GR_GL_UNSIGNED_SHORT, this->offsetForBaseIndex(baseIndex)));
  }
}

void GrGLOpsRenderPass::onDrawInstanced(
  ...
  for (int i = 0; i < instanceCount; i += maxInstances) {
    ...
    if (fGpu->glCaps().baseVertexBaseInstanceSupport()) {
      ...
      GL_CALL(DrawArraysInstancedBaseInstance(
          glPrimType, baseVertex, vertexCount, instanceCountForDraw, baseInstanceForDraw));
    } else {
...
void GrGLOpsRenderPass::onDrawIndexedInstanced(
  ...
  for (int i = 0; i < instanceCount; i += maxInstances) {
    ...
    if (fGpu->glCaps().baseVertexBaseInstanceSupport()) {
      ...
      GL_CALL(DrawElementsInstancedBaseVertexBaseInstance(
          glPrimType, indexCount, GR_GL_UNSIGNED_SHORT, this->offsetForBaseIndex(baseIndex),
          instanceCountForDraw, baseVertex, baseInstanceForDraw));
    } else {
      this->bindInstanceBuffer(fActiveInstanceBuffer.get(), baseInstanceForDraw);
      this->bindVertexBuffer(fActiveVertexBuffer.get(), baseVertex);
...
void GrGLOpsRenderPass::onDrawIndirect(
  ...
  if (drawCount > 1 && fGpu->glCaps().multiDrawType() == MultiDrawType::kMultiDrawIndirect) {
    GrGLenum glPrimType = fGpu->prepareToDraw(fPrimitiveType);
    GL_CALL(MultiDrawArraysIndirect(
        glPrimType, buffer_offset_to_gl_address(drawIndirectBuffer, offset), drawCount,
  ...
  for (int i = 0; i < drawCount; ++i) {
    GrGLenum glPrimType = fGpu->prepareToDraw(fPrimitiveType);
    GL_CALL(
        DrawArraysIndirect(glPrimType, buffer_offset_to_gl_address(drawIndirectBuffer, offset)));
...
void GrGLOpsRenderPass::multiDrawArraysANGLEOrWebGL(
  ...
  while (drawCount) {
    ...
    if (countInBatch == 1) {
      GL_CALL(DrawArraysInstancedBaseInstance(
          glPrimType, fFirsts[0], fCounts[0], fInstanceCounts[0], fBaseInstances[0]));
    } else {
      GL_CALL(MultiDrawArraysInstancedBaseInstance(
          glPrimType, fFirsts, fCounts, fInstanceCounts, fBaseInstances, countInBatch));
    }
...
void GrGLOpsRenderPass::onDrawIndexedIndirect(
    const GrBuffer* drawIndirectBuffer, size_t offset, int drawCount) {
  using MultiDrawType = GrGLCaps::MultiDrawType;
  ...
  if (drawCount > 1 && fGpu->glCaps().multiDrawType() == MultiDrawType::kMultiDrawIndirect) {
    GrGLenum glPrimType = fGpu->prepareToDraw(fPrimitiveType);
    GL_CALL(MultiDrawElementsIndirect(
        glPrimType, GR_GL_UNSIGNED_SHORT, buffer_offset_to_gl_address(drawIndirectBuffer, offset),
  ...
  for (int i = 0; i < drawCount; ++i) {
    GrGLenum glPrimType = fGpu->prepareToDraw(fPrimitiveType);
    GL_CALL(DrawElementsIndirect(
        glPrimType, GR_GL_UNSIGNED_SHORT, buffer_offset_to_gl_address(drawIndirectBuffer, offset)));
    offset += sizeof(GrDrawIndexedIndirectCommand);
  }
}

void GrGLOpsRenderPass::multiDrawElementsANGLEOrWebGL(
    const GrBuffer* drawIndirectBuffer, size_t offset, int drawCount) {
  SkASSERT(fGpu->glCaps().multiDrawType() == GrGLCaps::MultiDrawType::kANGLEOrWebGL);
  SkASSERT(drawIndirectBuffer->isCpuBuffer());
  ...
  while (drawCount) {
    ...
    if (countInBatch == 1) {
      GL_CALL(DrawElementsInstancedBaseVertexBaseInstance(
          glPrimType, fCounts[0], GR_GL_UNSIGNED_SHORT, fIndices[0], fInstanceCounts[0],
          fBaseVertices[0], fBaseInstances[0]));
    } else {
      GL_CALL(MultiDrawElementsInstancedBaseVertexBaseInstance(
          glPrimType, fCounts, GR_GL_UNSIGNED_SHORT, fIndices, fInstanceCounts, fBaseVertices,
