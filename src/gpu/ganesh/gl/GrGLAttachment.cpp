...
static bool renderbuffer_storage_msaa(
    GrGLGpu* gpu, int sampleCount, GrGLenum format, int width, int height) {
...
sk_sp<GrGLAttachment> GrGLAttachment::MakeStencil(
    GrGLGpu* gpu, SkISize dimensions, int sampleCnt, GrGLFormat format) {
  GrGLuint rbID = 0;
  ...
  return sk_sp<GrGLAttachment>(new GrGLAttachment(
      gpu, rbID, dimensions, GrAttachment::UsageFlags::kStencilAttachment, sampleCnt, format,
      /*label=*/"GLAttachmentMakeStencil"));
}

sk_sp<GrGLAttachment> GrGLAttachment::MakeMSAA(
    GrGLGpu* gpu, SkISize dimensions, int sampleCnt, GrGLFormat format) {
  GrGLuint rbID = 0;
  ...
  if (!renderbuffer_storage_msaa(gpu, sampleCnt, glFmt, dimensions.width(), dimensions.height())) {
    GL_CALL(DeleteRenderbuffers(1, &rbID));
    return nullptr;
  }

  return sk_sp<GrGLAttachment>(new GrGLAttachment(
      gpu, rbID, dimensions, GrAttachment::UsageFlags::kColorAttachment, sampleCnt, format,
...
void GrGLAttachment::setMemoryBacking(
...
void GrGLAttachment::onSetLabel() {
  ...
  if (!this->getLabel().empty()) {
    ...
    if (glGpu->glCaps().debugSupport()) {
      GR_GL_CALL(
          glGpu->glInterface(), ObjectLabel(GR_GL_TEXTURE, fRenderbufferID, -1, label.c_str()));
    }
  }
}
