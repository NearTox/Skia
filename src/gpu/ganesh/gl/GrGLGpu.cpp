...
static GrGLenum filter_to_gl_min_filter(
    GrSamplerState::Filter filter, GrSamplerState::MipmapMode mm) {
  switch (mm) {
    case GrSamplerState::MipmapMode::kNone: return filter_to_gl_mag_filter(filter);
  ...
  void bindSampler(int unitIdx, GrSamplerState state) {
    ...
    if (!sampler) {
      ...
      if (fGpu->glCaps().anisoSupport()) {
        float maxAniso = std::min(
            static_cast<GrGLfloat>(state.maxAniso()), fGpu->glCaps().maxTextureMaxAnisotropy());
        GR_GL_CALL(
            fGpu->glInterface(), SamplerParameterf(s, GR_GL_TEXTURE_MAX_ANISOTROPY, maxAniso));
      }
    }
...
sk_sp<GrGpu> GrGLGpu::Make(
    sk_sp<const GrGLInterface> interface, const GrContextOptions& options,
...
GrThreadSafePipelineBuilder* GrGLGpu::pipelineBuilder() { return fProgramCache.get(); }

sk_sp<GrThreadSafePipelineBuilder> GrGLGpu::refPipelineBuilder() { return fProgramCache; }
...
static bool check_backend_texture(
    const GrBackendTexture& backendTex, const GrGLCaps& caps, GrGLTexture::Desc* desc,
...
sk_sp<GrTexture> GrGLGpu::onWrapBackendTexture(
  ...
  GrMipmapStatus mipmapStatus =
      backendTex.hasMipmaps() ? GrMipmapStatus::kValid : GrMipmapStatus::kNotAllocated;

  auto texture = GrGLTexture::MakeWrapped(
      this, mipmapStatus, desc, backendTex.getGLTextureParams(), cacheable, ioType);
...
static bool check_compressed_backend_texture(
    const GrBackendTexture& backendTex, const GrGLCaps& caps, GrGLTexture::Desc* desc,
    bool skipRectTexSupportCheck = false) {
  GrGLTextureInfo info;
  if (!backendTex.getGLTextureInfo(&info) || !info.fID || !info.fFormat) {
...
sk_sp<GrTexture> GrGLGpu::onWrapCompressedBackendTexture(
    const GrBackendTexture& backendTex, GrWrapOwnership ownership, GrWrapCacheable cacheable) {
  GrGLTexture::Desc desc;
  ...
  auto texture = GrGLTexture::MakeWrapped(
      this, mipmapStatus, desc, backendTex.getGLTextureParams(), cacheable, kRead_GrIOType);
  return std::move(texture);
}
...
sk_sp<GrTexture> GrGLGpu::onWrapRenderableBackendTexture(
  ...
  GrMipmapStatus mipmapStatus =
      backendTex.hasMipmaps() ? GrMipmapStatus::kDirty : GrMipmapStatus::kNotAllocated;

  sk_sp<GrGLTextureRenderTarget> texRT(GrGLTextureRenderTarget::MakeWrapped(
      this, sampleCnt, desc, backendTex.getGLTextureParams(), rtIDs, cacheable, mipmapStatus));
...
sk_sp<GrRenderTarget> GrGLGpu::onWrapBackendRenderTarget(const GrBackendRenderTarget& backendRT) {
  ...
  return GrGLRenderTarget::MakeWrapped(
      this, backendRT.dimensions(), format, sampleCount, rtIDs, backendRT.stencilBits());
}
...
bool GrGLGpu::onWritePixels(
    GrSurface* surface, SkIRect rect, GrColorType surfaceColorType, GrColorType srcColorType,
  ...
  return this->uploadColorTypeTexData(
      glTex->format(), surfaceColorType, glTex->dimensions(), glTex->target(), rect, srcColorType,
      texels, mipLevelCount);
}

bool GrGLGpu::onTransferPixelsTo(
    GrTexture* texture, SkIRect rect, GrColorType textureColorType, GrColorType bufferColorType,
  ...
  GL_CALL(TexSubImage2D(
      glTex->target(), 0, rect.left(), rect.top(), rect.width(), rect.height(), externalFormat,
      externalType, pixels));
...
bool GrGLGpu::onTransferPixelsFrom(
    GrSurface* surface, SkIRect rect, GrColorType surfaceColorType, GrColorType dstColorType,
    sk_sp<GrGpuBuffer> transferBuffer, size_t offset) {
  auto* glBuffer = static_cast<GrGLBuffer*>(transferBuffer.get());
  SkASSERT(
      glBuffer->glSizeInBytes() >=
      offset + (rect.width() * rect.height() * GrColorTypeBytesPerPixel(dstColorType)));
  ...
  return this->readOrTransferPixelsFrom(
      surface, rect, surfaceColorType, dstColorType, offsetAsPtr, rect.width());
}
...
bool GrGLGpu::uploadColorTypeTexData(
    GrGLFormat textureFormat, GrColorType textureColorType, SkISize texDims, GrGLenum target,
    SkIRect dstRect, GrColorType srcColorType, const GrMipLevel texels[], int mipLevelCount) {
  ...
  this->uploadTexData(
      texDims, target, dstRect, externalFormat, externalType, bpp, texels, mipLevelCount);
  return true;
}

bool GrGLGpu::uploadColorToTex(
    GrGLFormat textureFormat, SkISize texDims, GrGLenum target, std::array<float, 4> color,
    uint32_t levelMask) {
  GrColorType colorType;
  GrGLenum externalFormat, externalType;
  this->glCaps().getTexSubImageDefaultFormatTypeAndColorType(
      textureFormat, &externalFormat, &externalType, &colorType);
  ...
  for (int i = 0; i < numLevels;
       ++i, levelDims = {
                std::max(levelDims.width() >> 1, 1), std::max(levelDims.height() >> 1, 1)}) {
    if (levelMask & (1 << i)) {
      if (!pixelStorage) {
  ...
  this->uploadTexData(
      texDims, target, SkIRect::MakeSize(texDims), externalFormat, externalType, bpp,
      levels.begin(), levels.count());
  return true;
}

void GrGLGpu::uploadTexData(
    SkISize texDims, GrGLenum target, SkIRect dstRect, GrGLenum externalFormat,
    GrGLenum externalType, size_t bpp, const GrMipLevel texels[], int mipLevelCount) {
  ...
  for (int level = 0; level < mipLevelCount;
       ++level, dims = {std::max(dims.width() >> 1, 1), std::max(dims.height() >> 1, 1)}) {
    ...
    GL_CALL(TexSubImage2D(
        target, level, dstRect.x(), dstRect.y(), dims.width(), dims.height(), externalFormat,
        externalType, texels[level].fPixels));
  }
  if (restoreGLRowLength) {
...
bool GrGLGpu::uploadCompressedTexData(
    SkImage::CompressionType compressionType, GrGLFormat format, SkISize dimensions,
    GrMipmapped mipmapped, GrGLenum target, const void* data, size_t dataSize) {
  ...
  if (useTexStorage) {
    ...
    GrGLenum error = GL_ALLOC_CALL(TexStorage2D(
        target, numMipLevels, internalFormat, dimensions.width(), dimensions.height()));
    ...
    for (int level = 0; level < numMipLevels; ++level) {
      ...
      error = GL_ALLOC_CALL(CompressedTexSubImage2D(
          target, level,
          0,  // left
          0,  // top
          dimensions.width(), dimensions.height(), internalFormat, SkToInt(levelDataSize),
          &((char*)data)[offset]));
    ...
    for (int level = 0; level < numMipLevels; ++level) {
      ...
      GrGLenum error = GL_ALLOC_CALL(CompressedTexImage2D(
          target, level, internalFormat, dimensions.width(), dimensions.height(),
          0,  // border
          SkToInt(levelDataSize), rawLevelData));
...
bool GrGLGpu::renderbufferStorageMSAA(
  ...
  switch (ctx.caps()->msFBOType()) {
    ...
    case GrGLCaps::kES_IMG_MsToTexture_MSFBOType:
      error = GL_ALLOC_CALL(RenderbufferStorageMultisampleES2EXT(
          GR_GL_RENDERBUFFER, sampleCount, format, width, height));
      break;
    case GrGLCaps::kNone_MSFBOType: SkUNREACHABLE; break;
  }
  return error == GR_GL_NO_ERROR;
}

bool GrGLGpu::createRenderTargetObjects(
    const GrGLTexture::Desc& desc, int sampleCount, GrGLRenderTarget::IDs* rtIDs) {
  rtIDs->fMSColorRenderbufferID = 0;
...
#  define IS_TSAN 0
  ...
  if (rtIDs->fMSColorRenderbufferID) {
    ...
    if (!this->renderbufferStorageMSAA(
            *fGLContext, sampleCount, colorRenderbufferFormat, desc.fSize.width(),
            desc.fSize.height())) {
      return false;
    }
...
// See skbug.com/12644
    ...
    if (!this->glCaps().skipErrorChecks()) {
      ...
      if (this->glCaps().rebindColorAttachmentAfterCheckFramebufferStatus()) {
        GL_CALL(FramebufferRenderbuffer(
            GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_RENDERBUFFER, 0));
        GL_CALL(FramebufferRenderbuffer(
            GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_RENDERBUFFER,
            rtIDs->fMSColorRenderbufferID));
      }
  ...
  GL_CALL(
      FramebufferTexture2D(GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, desc.fTarget, desc.fID, 0));
// See skbug.com/12644
#if !IS_TSAN
  if (!this->glCaps().skipErrorChecks()) {
    GrGLenum status;
    ...
    if (this->glCaps().rebindColorAttachmentAfterCheckFramebufferStatus()) {
      GL_CALL(FramebufferTexture2D(GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, desc.fTarget, 0, 0));
      GL_CALL(FramebufferTexture2D(
          GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, desc.fTarget, desc.fID, 0));
    }
  }
#endif
...
static GrGLTextureParameters::SamplerOverriddenState set_initial_texture_params(
    const GrGLInterface* interface, const GrGLCaps& caps, GrGLenum target) {
...
sk_sp<GrTexture> GrGLGpu::onCreateTexture(
    ...
    uint32_t levelClearMask, std::string_view label) {
  ...
  switch (format.textureType()) {
    ...
    case GrTextureType::kNone: return nullptr;
    case GrTextureType::k2D: texDesc.fTarget = GR_GL_TEXTURE_2D; break;
  ...
  texDesc.fID = this->createTexture(
      dimensions, texDesc.fFormat, texDesc.fTarget, renderable, &initialState, mipLevelCount,
      isProtected);
  ...
  if (renderable == GrRenderable::kYes) {
    ...
    tex = sk_make_sp<GrGLTextureRenderTarget>(
        this, budgeted, renderTargetSampleCnt, texDesc, rtIDDesc, mipmapStatus, label);
  ...
  tex->parameters()->set(
      &initialState, GrGLTextureParameters::NonsamplerState(), fResetTimestampForTextureParameters);
  if (levelClearMask) {
    if (this->glCaps().clearTextureSupport()) {
      ...
      for (int i = 0; i < mipLevelCount; ++i) {
        ...
        this->glCaps().canFormatBeFBOColorAttachment(format.asGLFormat()) &&
      ...
      this->uploadColorToTex(
          texDesc.fFormat, texDesc.fSize, texDesc.fTarget, zeros, levelClearMask);
    }
  }
  return std::move(tex);
}

sk_sp<GrTexture> GrGLGpu::onCreateCompressedTexture(
  ...
  desc.fID = this->createCompressedTexture2D(
      desc.fSize, compression, desc.fFormat, mipmapped, &initialState);
  if (!desc.fID) {
    return nullptr;
  }

  if (data) {
    if (!this->uploadCompressedTexData(
            compression, desc.fFormat, dimensions, mipmapped, GR_GL_TEXTURE_2D, data, dataSize)) {
  ...
  GrMipmapStatus mipmapStatus =
      mipmapped == GrMipmapped::kYes ? GrMipmapStatus::kValid : GrMipmapStatus::kNotAllocated;

  auto tex = sk_make_sp<GrGLTexture>(
...
GrBackendTexture GrGLGpu::onCreateCompressedBackendTexture(
  ...
  info.fID =
      this->createCompressedTexture2D(dimensions, compression, glFormat, mipmapped, &initialState);
  if (!info.fID) {
    return {};
  }
...
bool GrGLGpu::onUpdateCompressedBackendTexture(
    const GrBackendTexture& backendTexture, sk_sp<skgpu::RefCntedCallback> finishedCallback,
  ...
  bool result = this->uploadCompressedTexData(
      compression, glFormat, backendTexture.dimensions(), mipmapped, GR_GL_TEXTURE_2D, data,
      length);
...
int GrGLGpu::getCompatibleStencilIndex(GrGLFormat format) {
  ...
  if (!this->glCaps().hasStencilFormatBeenDeterminedForFormat(format)) {
    ...
    GrGLuint colorID = this->createTexture(
        {kSize, kSize}, format, GR_GL_TEXTURE_2D, GrRenderable::kYes, nullptr, 1, GrProtected::kNo);
    if (!colorID) {
      return -1;
    }
    // unbind the texture from the texture unit before binding it to the frame buffer
    ...
    if (sbRBID) {
      ...
      for (int i = 0; i < stencilFmtCnt && sbRBID; ++i) {
        ...
        GrGLenum error = GL_ALLOC_CALL(
            RenderbufferStorage(GR_GL_RENDERBUFFER, GrGLFormatToEnum(sFmt), kSize, kSize));
        if (error == GR_GL_NO_ERROR) {
          GL_CALL(FramebufferRenderbuffer(
...
GrGLuint GrGLGpu::createCompressedTexture2D(
    SkISize dimensions, SkImage::CompressionType compression, GrGLFormat format,
    GrMipmapped mipmapped, GrGLTextureParameters::SamplerOverriddenState* initialState) {
  ...
  *initialState = set_initial_texture_params(this->glInterface(), this->glCaps(), GR_GL_TEXTURE_2D);

  return id;
}

GrGLuint GrGLGpu::createTexture(
    SkISize dimensions, GrGLFormat format, GrGLenum target, GrRenderable renderable,
  ...
  if (internalFormat) {
    if (this->glCaps().formatSupportsTexStorage(format)) {
      ...
      GrGLenum error = GL_ALLOC_CALL(TexStorage2D(
          target, levelCount, internalFormat, dimensions.width(), dimensions.height()));
      ...
      if (externalFormat && externalType) {
        for (int level = 0; level < mipLevelCount && error == GR_GL_NO_ERROR; level++) {
          ...
          error = GL_ALLOC_CALL(TexImage2D(
              target, level, internalFormat, currentWidth, currentHeight, 0, externalFormat,
...
sk_sp<GrAttachment> GrGLGpu::makeStencilAttachment(
...
sk_sp<GrAttachment> GrGLGpu::makeMSAAAttachment(
...
bool GrGLGpu::flushGLState(
    GrRenderTarget* renderTarget, bool useMultisampleFBO, const GrProgramInfo& programInfo) {
  this->handleDirtyContext();

  sk_sp<GrGLProgram> program = fProgramCache->findOrCreateProgram(this->getContext(), programInfo);
  if (!program) {
  ...
  this->flushWindowRectangles(
      programInfo.pipeline().getWindowRectsState(), glRT, programInfo.origin());
...
void GrGLGpu::clear(
    ...
    bool useMultisampleFBO, GrSurfaceOrigin origin) {
...
static bool use_tiled_rendering(
    const GrGLCaps& glCaps, const GrOpsRenderPass::StencilLoadAndStoreInfo& stencilLoadStore) {
...
void GrGLGpu::beginCommandBuffer(
    GrGLRenderTarget* rt, bool useMultisampleFBO, const SkIRect& bounds, GrSurfaceOrigin origin,
  ...
  if (use_tiled_rendering(this->glCaps(), stencilLoadStore)) {
    ...
    GrGLbitfield preserveMask =
        (GrLoadOp::kLoad == colorLoadStore.fLoadOp) ? GR_GL_COLOR_BUFFER_BIT0 : GR_GL_NONE;
    SkASSERT(GrLoadOp::kLoad != stencilLoadStore.fLoadOp);  // Handled by use_tiled_rendering().
    GL_CALL(StartTiling(
        nativeBounds.fX, nativeBounds.fY, nativeBounds.fWidth, nativeBounds.fHeight, preserveMask));
  }
...
void GrGLGpu::endCommandBuffer(
  ...
  if (GrGLCaps::kNone_InvalidateFBType != this->glCaps().invalidateFBType()) {
    ...
    if (!discardAttachments.empty()) {
      if (GrGLCaps::kInvalidate_InvalidateFBType == this->glCaps().invalidateFBType()) {
        GL_CALL(InvalidateFramebuffer(
            GR_GL_FRAMEBUFFER, discardAttachments.count(), discardAttachments.begin()));
      } else {
        SkASSERT(GrGLCaps::kDiscard_InvalidateFBType == this->glCaps().invalidateFBType());
        GL_CALL(DiscardFramebuffer(
            GR_GL_FRAMEBUFFER, discardAttachments.count(), discardAttachments.begin()));
      }
    }
  }
  ...
  if (use_tiled_rendering(this->glCaps(), stencilLoadStore)) {
    GrGLbitfield preserveMask =
        (GrStoreOp::kStore == colorLoadStore.fStoreOp) ? GR_GL_COLOR_BUFFER_BIT0 : GR_GL_NONE;
...
void GrGLGpu::clearStencilClip(
    const GrScissorState& scissor, bool insideStencilMask, GrRenderTarget* target,
    bool useMultisampleFBO, GrSurfaceOrigin origin) {
  SkASSERT(target);
  SkASSERT(!this->caps()->performStencilClearsAsDraws());
  SkASSERT(!scissor.enabled() || !this->caps()->performPartialClearsAsDraws());
...
bool GrGLGpu::readOrTransferPixelsFrom(
  ...
  this->glCaps().getReadPixelsFormat(
      surface->backendFormat().asGLFormat(), surfaceColorType, dstColorType, &externalFormat,
      &externalType);
  if (!externalFormat || !externalType) {
    return false;
  }
  ...
  GL_CALL(ReadPixels(
      rect.left(), rect.top(), rect.width(), rect.height(), externalFormat, externalType,
      offsetOrPtr));
...
bool GrGLGpu::onReadPixels(
  ...
  return this->readOrTransferPixelsFrom(
      surface, rect, surfaceColorType, dstColorType, buffer, rowPixelWidth);
}

GrOpsRenderPass* GrGLGpu::onGetOpsRenderPass(
    GrRenderTarget* rt, bool useMultisampleFBO, GrAttachment*, GrSurfaceOrigin origin,
...
void GrGLGpu::flushRenderTarget(
    GrGLRenderTarget* target, bool useMultisampleFBO, GrSurfaceOrigin origin,
...
void GrGLGpu::onResolveRenderTarget(GrRenderTarget* target, const SkIRect& resolveRect) {
  ...
  if (this->glCaps().framebufferResolvesMustBeFullSize()) {
    this->resolveRenderFBOs(
        glRT, SkIRect::MakeSize(glRT->dimensions()), ResolveDirection::kMSAAToSingle);
  } else {
    this->resolveRenderFBOs(glRT, resolveRect, ResolveDirection::kMSAAToSingle);
  }
}

void GrGLGpu::resolveRenderFBOs(
    GrGLRenderTarget* rt, const SkIRect& resolveRect, ResolveDirection resolveDirection,
  ...
  if (GrGLCaps::kES_Apple_MSFBOType == caps.msFBOType()) {
    ...
    SkASSERT(
        !caps.framebufferResolvesMustBeFullSize() ||
  ...
  if (caps.invalidateFBType() != GrGLCaps::kNone_InvalidateFBType &&
    ...
    GrGLenum colorDiscardAttachment =
        rt->isFBO0(readBufferIsMSAA) ? GR_GL_COLOR : GR_GL_COLOR_ATTACHMENT0;
...
void GrGLGpu::flushStencil(const GrStencilSettings& stencilSettings, GrSurfaceOrigin origin) {
  if (stencilSettings.isDisabled()) {
    ...
    if (!stencilSettings.isTwoSided()) {
      set_gl_stencil(this->glInterface(), stencilSettings.singleSidedFace(), GR_GL_FRONT_AND_BACK);
    } else {
      set_gl_stencil(this->glInterface(), stencilSettings.postOriginCWFace(origin), GR_GL_FRONT);
      set_gl_stencil(this->glInterface(), stencilSettings.postOriginCCWFace(origin), GR_GL_BACK);
    }
    fHWStencilSettings = stencilSettings;
...
void GrGLGpu::flushBlendAndColorWrite(
    const skgpu::BlendInfo& blendInfo, const skgpu::Swizzle& swizzle) {
  ...
  bool blendOff =
      skgpu::BlendShouldDisable(equation, srcCoeff, dstCoeff) || !blendInfo.fWritesColor;
  ...
  if (blendOff) {
    ...
    if (fHWBlendState.fSrcCoeff != srcCoeff || fHWBlendState.fDstCoeff != dstCoeff) {
      GL_CALL(BlendFunc(gXfermodeCoeff2Blend[(int)srcCoeff], gXfermodeCoeff2Blend[(int)dstCoeff]));
...
void GrGLGpu::bindTexture(
    int unitIdx, GrSamplerState samplerState, const skgpu::Swizzle& swizzle, GrGLTexture* texture) {
  SkASSERT(texture);
  ...
  if (samplerState.mipmapped() == GrMipmapped::kYes) {
    if (!this->caps()->mipmapSupport() || texture->mipmapped() == GrMipmapped::kNo) {
      ...
      samplerState = GrSamplerState(
          samplerState.wrapModeX(), samplerState.wrapModeY(), samplerState.filter(),
  ...
  if (this->glCaps().useSamplerObjects()) {
    ...
    if (this->glCaps().mustSetAnyTexParameterToEnableMipmapping()) {
      if (samplerState.mipmapped() == GrMipmapped::kYes) {
        GrGLenum minFilter =
            filter_to_gl_min_filter(samplerState.filter(), samplerState.mipmapMode());
    ...
    newSamplerState.fMaxAniso = std::min(
        static_cast<GrGLfloat>(samplerState.maxAniso()), this->glCaps().maxTextureMaxAnisotropy());
    ...
    if (this->caps()->anisoSupport()) {
      if (setAll || oldSamplerState.fMaxAniso != newSamplerState.fMaxAniso) {
        GL_CALL(TexParameterf(target, GR_GL_TEXTURE_MAX_ANISOTROPY, newSamplerState.fMaxAniso));
      }
    }
  }
  ...
  if (this->glCaps().mipmapLevelControlSupport() &&
    ...
    if (newNonsamplerState.fBaseMipMapLevel != oldNonsamplerState.fBaseMipMapLevel) {
      this->setTextureUnit(unitIdx);
      GL_CALL(TexParameteri(target, GR_GL_TEXTURE_BASE_LEVEL, newNonsamplerState.fBaseMipMapLevel));
    }
  ...
  texture->parameters()->set(
      samplerStateToRecord, newNonsamplerState, fResetTimestampForTextureParameters);
}

void GrGLGpu::onResetTextureBindings() {
  static constexpr GrGLenum kTargets[] = {
      GR_GL_TEXTURE_2D, GR_GL_TEXTURE_RECTANGLE, GR_GL_TEXTURE_EXTERNAL};
...
static inline bool can_blit_framebuffer_for_copy_surface(
    const GrSurface* dst, const GrSurface* src, const SkIRect& srcRect, const SkIPoint& dstPoint,
    const GrGLCaps& caps) {
  int dstSampleCnt = 0;
  int srcSampleCnt = 0;
  ...
  return caps.canCopyAsBlit(
      dstFormat, dstSampleCnt, dstTexTypePtr, srcFormat, srcSampleCnt, srcTexTypePtr,
...
static inline bool can_copy_texsubimage(
    const GrSurface* dst, const GrSurface* src, const GrGLCaps& caps) {
  ...
  return caps.canCopyTexSubImage(
      dstFormat, dstHasMSAARenderBuffer, dstTexTypePtr, srcFormat, srcHasMSAARenderBuffer,
      srcTexTypePtr);
}

void GrGLGpu::bindSurfaceFBOForPixelOps(
    GrSurface* surface, int mipLevel, GrGLenum fboTarget, TempFBOTarget tempFBOTarget) {
...
bool GrGLGpu::createCopyProgram(GrTexture* srcTex) {
  ...
  if (!fCopyProgramArrayBuffer) {
    ...
    fCopyProgramArrayBuffer =
        GrGLBuffer::Make(this, sizeof(vdata), GrGpuBufferType::kVertex, kStatic_GrAccessPattern);
  ...
  GrShaderVar uTexCoordXform(
      "u_texCoordXform", SkSLType::kHalf4, GrShaderVar::TypeModifier::Uniform);
  ...
  std::unique_ptr<SkSL::Program> program =
      GrSkSLtoGLSL(this, SkSL::ProgramKind::kVertex, sksl, settings, &glsl, errorHandler);
  GrGLuint vshader = GrGLCompileAndAttachShader(
      *fGLContext, fCopyPrograms[progIdx].fProgram, GR_GL_VERTEX_SHADER, glsl,
      fProgramCache->stats(), errorHandler);
  SkASSERT(program->fInputs == SkSL::Program::Inputs());

  sksl.assign(fshaderTxt.c_str(), fshaderTxt.size());
  program = GrSkSLtoGLSL(this, SkSL::ProgramKind::kFragment, sksl, settings, &glsl, errorHandler);
  GrGLuint fshader = GrGLCompileAndAttachShader(
      *fGLContext, fCopyPrograms[progIdx].fProgram, GR_GL_FRAGMENT_SHADER, glsl,
      fProgramCache->stats(), errorHandler);
  SkASSERT(program->fInputs == SkSL::Program::Inputs());
...
bool GrGLGpu::createMipmapProgram(int progIdx) {
  ...
  GrShaderVar uTexCoordXform(
      "u_texCoordXform", SkSLType::kHalf4, GrShaderVar::TypeModifier::Uniform);
  GrShaderVar uTexture(
      "u_texture", SkSLType::kTexture2DSampler, GrShaderVar::TypeModifier::Uniform);
  ...
  if (oddWidth && oddHeight) {
    ...
    fshaderTxt.append("  sk_FragColor = sample(u_texture, v_texCoord0);");
  }
  ...
  std::unique_ptr<SkSL::Program> program =
      GrSkSLtoGLSL(this, SkSL::ProgramKind::kVertex, sksl, settings, &glsl, errorHandler);
  GrGLuint vshader = GrGLCompileAndAttachShader(
      *fGLContext, fMipmapPrograms[progIdx].fProgram, GR_GL_VERTEX_SHADER, glsl,
      fProgramCache->stats(), errorHandler);
  SkASSERT(program->fInputs == SkSL::Program::Inputs());
  ...
  program = GrSkSLtoGLSL(this, SkSL::ProgramKind::kFragment, sksl, settings, &glsl, errorHandler);
  GrGLuint fshader = GrGLCompileAndAttachShader(
      *fGLContext, fMipmapPrograms[progIdx].fProgram, GR_GL_FRAGMENT_SHADER, glsl,
      fProgramCache->stats(), errorHandler);
  SkASSERT(program->fInputs == SkSL::Program::Inputs());
...
bool GrGLGpu::copySurfaceAsDraw(
    GrSurface* dst, bool drawToMultisampleFBO, GrSurface* src, const SkIRect& srcRect,
  ...
  attribs->set(
      this, 0, fCopyProgramArrayBuffer.get(), kFloat2_GrVertexAttribType, SkSLType::kFloat2,
...
void GrGLGpu::copySurfaceAsCopyTexSubImage(
  ...
  GL_CALL(CopyTexSubImage2D(
      dstTex->target(), 0, dstPoint.fX, dstPoint.fY, srcRect.fLeft, srcRect.fTop, srcRect.width(),
      srcRect.height()));
...
bool GrGLGpu::copySurfaceAsBlitFramebuffer(
  ...
  GL_CALL(BlitFramebuffer(
      srcRect.fLeft, srcRect.fTop, srcRect.fRight, srcRect.fBottom, dstRect.fLeft, dstRect.fTop,
      dstRect.fRight, dstRect.fBottom, GR_GL_COLOR_BUFFER_BIT, GR_GL_NEAREST));
...
bool GrGLGpu::onRegenerateMipMapLevels(GrTexture* texture) {
  ...
  attribs->enableVertexArrays(this, 1);
  attribs->set(
      this, 0, fMipmapProgramArrayBuffer.get(), kFloat2_GrVertexAttribType, SkSLType::kFloat2,
      2 * sizeof(GrGLfloat), 0);
...
GrBackendTexture GrGLGpu::onCreateBackendTexture(
    SkISize dimensions, const GrBackendFormat& format, GrRenderable renderable,
  ...
  switch (format.textureType()) {
    ...
    case GrTextureType::kExternal: return {};
    case GrTextureType::k2D: info.fTarget = GR_GL_TEXTURE_2D; break;
    case GrTextureType::kRectangle:
  ...
  info.fFormat = GrGLFormatToEnum(glFormat);
  info.fID = this->createTexture(
      dimensions, glFormat, info.fTarget, renderable, &initialState, numMipLevels, isProtected);
  if (!info.fID) {
    return {};
  }
...
bool GrGLGpu::onClearBackendTexture(
    const GrBackendTexture& backendTexture, sk_sp<skgpu::RefCntedCallback> finishedCallback,
  ...
  if (backendTexture.hasMipmaps()) {
    numMipLevels = SkMipmap::ComputeLevelCount(backendTexture.width(), backendTexture.height()) + 1;
  }
  ...
  bool result =
      this->uploadColorToTex(glFormat, backendTexture.dimensions(), info.fTarget, color, levelMask);
...
bool GrGLGpu::compile(const GrProgramDesc& desc, const GrProgramInfo& programInfo) {
  ...
  sk_sp<GrGLProgram> tmp =
      fProgramCache->findOrCreateProgram(this->getContext(), desc, programInfo, &stat);
  if (!tmp) {
    return false;
  }
...
GrBackendRenderTarget GrGLGpu::createTestingOnlyBackendRenderTarget(
    SkISize dimensions, GrColorType colorType, int sampleCnt, GrProtected isProtected) {
  ...
  if (useTexture) {
    GrGLTextureParameters::SamplerOverriddenState initialState;
    colorID = this->createTexture(
        dimensions, format, GR_GL_TEXTURE_2D, GrRenderable::kYes, &initialState, 1, isProtected);
    if (!colorID) {
      deleteIDs();
      return {};
    }
    if (sampleCnt == 1) {
      ...
      GL_CALL(FramebufferTexture2DMultisample(
          GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_TEXTURE_2D, colorID, 0, sampleCnt));
    }
  } else {
    ...
    if (sampleCnt == 1) {
      GL_CALL(RenderbufferStorage(
          GR_GL_RENDERBUFFER, renderBufferFormat, dimensions.width(), dimensions.height()));
    } else {
      if (!this->renderbufferStorageMSAA(
              this->glContext(), sampleCnt, renderBufferFormat, dimensions.width(),
              dimensions.height())) {
        deleteIDs();
        return {};
      }
  ...
  if (sampleCnt == 1) {
    GL_CALL(RenderbufferStorage(
        GR_GL_RENDERBUFFER, GrGLFormatToEnum(stencilBufferFormat), dimensions.width(),
        dimensions.height()));
  } else {
    if (!this->renderbufferStorageMSAA(
            this->glContext(), sampleCnt, GrGLFormatToEnum(stencilBufferFormat), dimensions.width(),
            dimensions.height())) {
      deleteIDs();
      return {};
    }
  ...
  GrBackendRenderTarget beRT =
      GrBackendRenderTarget(dimensions.width(), dimensions.height(), sampleCnt, stencilBits, info);
...
void GrGLGpu::checkFinishProcs() { fFinishCallbacks.check(); }

void GrGLGpu::finishOutstandingGpuWork() { GL_CALL(Finish()); }
...
int GrGLGpu::TextureToCopyProgramIdx(GrTexture* texture) {
  switch (SkSLCombinedSamplerTypeForTextureType(texture->textureType())) {
    case SkSLType::kTexture2DSampler: return 0;
    case SkSLType::kTexture2DRectSampler: return 1;
    case SkSLType::kTextureExternalSampler: return 2;
    default: SK_ABORT("Unexpected samper type");
  }
}
