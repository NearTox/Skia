...
GrGLTextureRenderTarget::GrGLTextureRenderTarget(
    ...
    const GrGLRenderTarget::IDs& rtIDs, GrMipmapStatus mipmapStatus, std::string_view label)
...
GrGLTextureRenderTarget::GrGLTextureRenderTarget(
    ...
    sk_sp<GrGLTextureParameters> parameters, const GrGLRenderTarget::IDs& rtIDs,
    GrWrapCacheable cacheable, GrMipmapStatus mipmapStatus, std::string_view label)
    : GrSurface(gpu, texDesc.fSize, GrProtected::kNo, label),
...
void GrGLTextureRenderTarget::dumpMemoryStatistics(SkTraceMemoryDump* traceMemoryDump) const {
  ...
  this->dumpMemoryStatisticsPriv(
      traceMemoryDump, resourceName, "RenderTarget", this->gpuMemorySize());
#endif
}
...
sk_sp<GrGLTextureRenderTarget> GrGLTextureRenderTarget::MakeWrapped(
    GrGLGpu* gpu, int sampleCount, const GrGLTexture::Desc& texDesc,
    sk_sp<GrGLTextureParameters> parameters, const GrGLRenderTarget::IDs& rtIDs,
    GrWrapCacheable cacheable, GrMipmapStatus mipmapStatus) {
...
void GrGLTextureRenderTarget::onSetLabel() { GrGLTexture::onSetLabel(); }
