...
GrBackendFormat GetBackendFormat(
    GrDirectContext* dContext, AHardwareBuffer* hardwareBuffer, uint32_t bufferFormat,
...
#    if __ANDROID_API__ >= 33
      case AHARDWAREBUFFER_FORMAT_R8_UNORM: return GrBackendFormat::MakeVk(VK_FORMAT_R8_UNORM);
#    endif
      default: {
        if (requireKnownFormat) {
          return GrBackendFormat();
          ...
          VkResult err =
              VK_CALL(GetAndroidHardwareBufferProperties(device, hardwareBuffer, &hwbProps));
          if (VK_SUCCESS != err) {
            return GrBackendFormat();
          }
...
void GLTextureHelper::rebind(GrDirectContext* dContext) {
  ...
  if ((status = glGetError()) != GL_NO_ERROR) {
    SkDebugf("glBindTexture(%#x, %d) failed (%#x)", (int)fTexTarget, (int)fTexID, (int)status);
    return;
  }
...
static GrBackendTexture make_gl_backend_texture(
    GrDirectContext* dContext, AHardwareBuffer* hardwareBuffer, int width, int height,
...
GrBackendTexture MakeBackendTexture(
  ...
  if (GrBackendApi::kOpenGL == dContext->backend()) {
    return make_gl_backend_texture(
        ...
        createProtectedImage, backendFormat, isRenderable);
  } else {
    SkASSERT(GrBackendApi::kVulkan == dContext->backend());
#  ifdef SK_VULKAN
    return make_vk_backend_texture(
        dContext, hardwareBuffer, width, height, deleteProc, updateProc, imageCtx,
        createProtectedImage, backendFormat, isRenderable, fromAndroidWindow);
#  else
    return GrBackendTexture();
#  endif
  }
