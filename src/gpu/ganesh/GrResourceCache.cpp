...
DECLARE_SKMESSAGEBUS_MESSAGE(
    GrResourceCache::UnrefResourceMessage, GrDirectContext::DirectContextID,
...
GrResourceCache::GrResourceCache(
    skgpu::SingleOwner* singleOwner, GrDirectContext::DirectContextID owningContextID,
    uint32_t familyID)
...
void GrResourceCache::changeUniqueKey(GrGpuResource* resource, const skgpu::UniqueKey& newKey) {
  ...
  if (newKey.isValid()) {
    if (GrGpuResource* old = fUniqueHash.find(newKey)) {
      ...
      if (!old->resourcePriv().getScratchKey().isValid() && old->resourcePriv().isPurgeable()) {
...
void GrResourceCache::notifyARefCntReachedZero(
    GrGpuResource* resource, GrGpuResource::LastRemovedRef removedRef) {
  ASSERT_SINGLE_OWNER
  SkASSERT(resource);
  SkASSERT(!resource->wasDestroyed());
  SkASSERT(this->isInCache(resource));
...
void GrResourceCache::didChangeBudgetStatus(GrGpuResource* resource) {
  ...
  TRACE_COUNTER2(
      "skia.gpu.cache", "skia budget", "used", fBudgetedBytes, "free", fMaxBytes - fBudgetedBytes);
    ...
    for (int i = 0; i < invalidKeyMsgs.count(); ++i) {
      if (invalidKeyMsgs[i].inThreadSafeCache()) {
        ...
        fProxyProvider->processInvalidUniqueKey(
            invalidKeyMsgs[i].key(), nullptr, GrProxyProvider::InvalidateGPUResource::kYes);
...
void GrResourceCache::purgeUnlockedResources(
    const GrStdSteadyClock::time_point* purgeTime, bool scratchResourcesOnly) {
...
void GrResourceCache::dumpStats(SkString* out) const {
  ...
  out->appendf(
      ...
      SkToInt(fHighWaterBytes));
}

void GrResourceCache::dumpStatsKeyValuePairs(
...
void GrResourceCache::validate() const {
  ...
  for (int i = 0; i < fNonpurgeableResources.count(); ++i) {
    SkASSERT(
        !fNonpurgeableResources[i]->resourcePriv().isPurgeable() ||
