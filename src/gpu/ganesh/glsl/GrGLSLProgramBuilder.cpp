...
GrGLSLProgramBuilder::GrGLSLProgramBuilder(
    ...
    : fVS(this), fFS(this), fDesc(desc), fProgramInfo(programInfo), fNumFragmentSamplers(0) {}

GrGLSLProgramBuilder::~GrGLSLProgramBuilder() = default;

void GrGLSLProgramBuilder::addFeature(
    GrShaderFlags shaders, uint32_t featureBit, const char* extensionName) {
...
bool GrGLSLProgramBuilder::emitAndInstallPrimProc(SkString* outputColor, SkString* outputCoverage) {
  ...
  for (int i = 0; i < geomProc.numTextureSamplers(); ++i) {
    ...
    texSamplers[i] = this->emitSampler(
        geomProc.textureSampler(i).backendFormat(), sampler.samplerState(), sampler.swizzle(),
        name.c_str());
  ...
  GrGeometryProcessor::ProgramImpl::EmitArgs args(
      &fVS, &fFS, this->varyingHandler(), this->uniformHandler(), this->shaderCaps(), geomProc,
      outputColor->c_str(), outputCoverage->c_str(), texSamplers.get());
...
SkString GrGLSLProgramBuilder::emitRootFragProc(
    const GrFragmentProcessor& fp, GrFragmentProcessor::ProgramImpl& impl, const SkString& input,
    SkString output) {
  SkASSERT(input.size());
  ...
  fp.visitWithImpls(
      [&, samplerIdx = 0](
          const GrFragmentProcessor& fp, GrFragmentProcessor::ProgramImpl& impl) mutable {
  ...
  if (fp.isBlendFunction()) {
    if (this->fragmentProcessorHasCoordsParam(&fp)) {
      fFS.codeAppendf(
          "%s = %s(%s, half4(1), %s);", output.c_str(), impl.functionName(), input.c_str(),
          fLocalCoordsVar.c_str());
    } else {
      fFS.codeAppendf("%s = %s(%s, half4(1));", output.c_str(), impl.functionName(), input.c_str());
    }
  } else {
    if (this->fragmentProcessorHasCoordsParam(&fp)) {
      fFS.codeAppendf(
          "%s = %s(%s, %s);", output.c_str(), impl.functionName(), input.c_str(),
          fLocalCoordsVar.c_str());
    } else {
...
void GrGLSLProgramBuilder::writeChildFPFunctions(
...
void GrGLSLProgramBuilder::writeFPFunction(
    const GrFragmentProcessor& fp, GrFragmentProcessor::ProgramImpl& impl) {
  ...
  if (this->fragmentProcessorHasCoordsParam(&fp)) {
    ...
    switch (varying.getType()) {
      case SkSLType::kVoid: SkASSERT(!fp.usesSampleCoordsDirectly()); break;
  ...
  GrFragmentProcessor::ProgramImpl::EmitArgs args(
      &fFS, this->uniformHandler(), this->shaderCaps(), fp, inputColor, kDstColor, sampleCoords);

  impl.emitCode(args);
  impl.setFunctionName(fFS.getMangledFunctionName(args.fFp.name()));

  fFS.emitFunction(
      SkSLType::kHalf4, impl.functionName(), SkMakeSpan(params, numParams), fFS.code().c_str());
  fFS.deleteStage();
}
...
bool GrGLSLProgramBuilder::emitAndInstallDstTexture() {
  ...
  if (this->pipeline().usesDstTexture()) {
    ...
    fDstTextureSamplerHandle = this->emitSampler(
        dstTextureProxy->backendFormat(), GrSamplerState(), swizzle, "DstTextureSampler");
    ...
    fUniformHandles.fDstTextureCoordsUni = this->uniformHandler()->addUniform(
        /*owner=*/nullptr, kFragment_GrShaderFlag, SkSLType::kHalf4, "DstTextureCoords",
        &dstTextureCoordsName);
    fFS.codeAppend("// Read color from copy of the destination\n");
    fFS.codeAppendf(
        "half2 _dstTexCoord = (half2(sk_FragCoord.xy) - %s.xy) * %s.zw;\n", dstTextureCoordsName,
        dstTextureCoordsName);
...
bool GrGLSLProgramBuilder::emitAndInstallXferProc(
    const SkString& colorIn, const SkString& coverageIn) {
  // Program builders have a bit of state we need to clear with each effect
  this->advanceStage();
  ...
  GrXferProcessor::ProgramImpl::EmitArgs args(
      ...
      fFS.getSecondaryColorOutputName(), fDstTextureSamplerHandle, fDstTextureOrigin,
...
GrGLSLProgramBuilder::SamplerHandle GrGLSLProgramBuilder::emitSampler(
  ...
  return this->uniformHandler()->addSampler(
      backendFormat, state, swizzle, name, this->shaderCaps());
}
...
void GrGLSLProgramBuilder::addRTFlipUniform(const char* name) {
  ...
  fUniformHandles.fRTFlipUni = uniformHandler->internalAddUniformArray(
      nullptr, kFragment_GrShaderFlag, SkSLType::kFloat2, name, false, 0, nullptr);
}
