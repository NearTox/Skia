...
void GrGLSLShaderBuilder::appendFunctionDecl(
    SkSLType returnType, const char* mangledName, SkSpan<const GrShaderVar> args) {
...
void GrGLSLShaderBuilder::appendColorGamutXform(
  ...
  auto emitTFFunc = [=](const char* name, GrGLSLProgramDataManager::UniformHandle uniform,
    ...
    this->emitFunction(
        SkSLType::kHalf, funcName.c_str(), {gTFArgs, SK_ARRAY_COUNT(gTFArgs)}, body.c_str());
    return funcName;
  };
  ...
  if (colorXformHelper->applyGamutXform()) {
    ...
    this->emitFunction(
        SkSLType::kHalf4, gamutXformFuncName.c_str(),
  ...
  // Now define a wrapper function that applies all the intermediate steps
  {
    ...
    this->emitFunction(
        SkSLType::kHalf4, colorXformFuncName.c_str(),
...
void GrGLSLShaderBuilder::addLayoutQualifier(const char* param, InterfaceQualifier interface) {
  SkASSERT(
      fProgramBuilder->shaderCaps()->fGLSLGeneration >= SkSL::GLSLGeneration::k330 ||
