...
static bool is_matrix(SkSLType type) {
  switch (type) {
    ...
    case SkSLType::kHalf4x4: return true;
...
class GrGLSLVarying {
 public:
  ...
  GrGLSLVarying(SkSLType type, Scope scope = Scope::kVertToFrag) noexcept
...
class GrGLSLVaryingHandler {
 public:
   ...
   */
  void addVarying(
      const char* name, GrGLSLVarying* varying, Interpolation = Interpolation::kInterpolated);
   ...
   */
  void addPassThroughAttribute(
      const GrShaderVar& vsVar, const char* output, Interpolation = Interpolation::kInterpolated);
