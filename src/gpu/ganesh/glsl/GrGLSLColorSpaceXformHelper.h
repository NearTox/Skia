    ...
    if (colorSpaceXform) {
      ...
      if (this->applySrcTF()) {
        fSrcTFVar = uniformHandler->addUniformArray(
            nullptr, visibility, SkSLType::kHalf, "SrcTF", kNumTransferFnCoeffs);
        fSrcTFKind = classify_transfer_fn(colorSpaceXform->fSteps.srcTF);
      }
      if (this->applyGamutXform()) {
        fGamutXformVar =
            uniformHandler->addUniform(nullptr, visibility, SkSLType::kHalf3x3, "ColorXform");
      }
      if (this->applyDstTF()) {
        fDstTFVar = uniformHandler->addUniformArray(
            nullptr, visibility, SkSLType::kHalf, "DstTF", kNumTransferFnCoeffs);
