...
const char* GrGLSLFragmentShaderBuilder::dstColor() {
  ...
  if (shaderCaps->fFBFetchSupport) {
    this->addFeature(
        1 << kFramebufferFetch_GLSLPrivateFeature, shaderCaps->fFBFetchExtensionString);
...
void GrGLSLFragmentShaderBuilder::enableCustomOutput() {
  if (!fCustomColorOutput) {
    fCustomColorOutput = &fOutputs.emplace_back(
        DeclaredColorOutputName(), SkSLType::kHalf4, GrShaderVar::TypeModifier::Out);
...
void GrGLSLFragmentShaderBuilder::enableSecondaryOutput() {
  ...
  if (caps.mustDeclareFragmentShaderOutput()) {
    fOutputs.emplace_back(
        DeclaredSecondaryColorOutputName(), SkSLType::kHalf4, GrShaderVar::TypeModifier::Out);
