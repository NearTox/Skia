...
class GrGpu : public SkRefCnt {
 public:
  ...
  sk_sp<GrTexture> createTexture(
      ...
      SkBudgeted budgeted, GrProtected isProtected, std::string_view label);
  ...
  bool writePixels(
      GrSurface* surface, SkIRect rect, GrColorType surfaceColorType, GrColorType srcColorType,
      const void* buffer, size_t rowBytes, bool prepForTexSampling = false) {
    GrMipLevel mipLevel = {buffer, rowBytes, nullptr};
    return this->writePixels(
        surface, rect, surfaceColorType, srcColorType, &mipLevel, 1, prepForTexSampling);
  }
  ...
  GrOpsRenderPass* getOpsRenderPass(
      GrRenderTarget* renderTarget, bool useMSAASurface, GrAttachment* stencil, GrSurfaceOrigin,
  ...
  virtual std::unique_ptr<GrSemaphore> wrapBackendSemaphore(
      const GrBackendSemaphore&, GrSemaphoreWrapType, GrWrapOwnership) = 0;
...
#  endif
  ...
  virtual bool setBackendRenderTargetState(
  ...
  // the GrAttachment.
  virtual sk_sp<GrAttachment> makeStencilAttachment(
      const GrBackendFormat& colorFormat, SkISize dimensions, int numStencilSamples) = 0;
 ...
 protected:
  static bool CompressedDataIsCorrect(
      SkISize dimensions, SkImage::CompressionType, GrMipmapped, const void* data, size_t length);
 ...
 private:
  ...
  virtual bool onClearBackendTexture(
      const GrBackendTexture&, sk_sp<skgpu::RefCntedCallback> finishedCallback,
  ...
  virtual sk_sp<GrTexture> onCreateTexture(
      ...
      SkBudgeted, GrProtected, int mipLevelCoont, uint32_t levelClearMask,
  ...
  virtual sk_sp<GrGpuBuffer> onCreateBuffer(
      size_t size, GrGpuBufferType intendedType, GrAccessPattern) = 0;
  ...
  virtual bool onWritePixels(
      GrSurface*, SkIRect, GrColorType surfaceColorType, GrColorType srcColorType,
      const GrMipLevel[], int mipLevelCount, bool prepForTexSampling) = 0;

  // overridden by backend-specific derived class to perform the texture transfer
  virtual bool onTransferPixelsTo(
      GrTexture*, SkIRect, GrColorType textiueColorType, GrColorType bufferColorType,
      sk_sp<GrGpuBuffer> transferBuffer, size_t offset, size_t rowBytes) = 0;

  // overridden by backend-specific derived class to perform the surface transfer
  virtual bool onTransferPixelsFrom(
      GrSurface*, SkIRect, GrColorType surfaceColorType, GrColorType bufferColorType,
      sk_sp<GrGpuBuffer> transferBuffer, size_t offset) = 0;
  ...
  sk_sp<GrTexture> createTextureCommon(
      SkISize, const GrBackendFormat&, GrTextureType textureType, GrRenderable,
      int renderTargetSampleCnt, SkBudgeted, GrProtected, int mipLevelCnt, uint32_t levelClearMask,
