 ...
 public:
  ...
  static const SkRect MakeTexelDomain(const SkIRect& texelRect, Mode modeX, Mode modeY) {
    ...
    return SkRect::MakeLTRB(
        texelRect.fLeft + insetX, texelRect.fTop + insetY, texelRect.fRight - insetX,
        texelRect.fBottom - insetY);
  }
  ...
  bool operator==(const GrTextureDomain& that) const {
    return fModeX == that.fModeX && fModeY == that.fModeY &&
           ...
           (kIgnore_Mode == fModeY ||
            (fDomain.fTop == that.fDomain.fTop && fDomain.fBottom == that.fDomain.fBottom));
  }
  ...
  class GLDomain {
   public:
     ...
     */
    void setData(
        const GrGLSLProgramDataManager&, const GrTextureDomain&, const GrSurfaceProxy*,
...
class GrDomainEffect : public GrFragmentProcessor {
 public:
  static std::unique_ptr<GrFragmentProcessor> Make(
      std::unique_ptr<GrFragmentProcessor>, const SkRect& domain, GrTextureDomain::Mode,
      bool decalIsFiltered);

  static std::unique_ptr<GrFragmentProcessor> Make(
      std::unique_ptr<GrFragmentProcessor>, const SkRect& domain, GrTextureDomain::Mode modeX,
  ...
  static std::unique_ptr<GrFragmentProcessor> Make(
      std::unique_ptr<GrFragmentProcessor>, const SkRect& domain, GrTextureDomain::Mode modeX,
      GrTextureDomain::Mode modeY, GrSamplerState::Filter);
  ...
  SkString dumpInfo() const override {
    SkString str;
    str.appendf(
        "Domain: [L: %.2f, T: %.2f, R: %.2f, B: %.2f], filterDecal: %d", fDomain.domain().fLeft,
        fDomain.domain().fTop, fDomain.domain().fRight, fDomain.domain().fBottom, fDecalIsFiltered);
 ...
 private:
  GrFragmentProcessor::OptimizationFlags Flags(
      GrFragmentProcessor*, GrTextureDomain::Mode, GrTextureDomain::Mode);
  ...
  GrDomainEffect(
      std::unique_ptr<GrFragmentProcessor>, const GrCoordTransform& transform, const SkRect& domain,
      GrTextureDomain::Mode modeX, GrTextureDomain::Mode modeY, bool decalIsFiltered);
...
class GrDeviceSpaceTextureDecalFragmentProcessor : public GrFragmentProcessor {
 public:
  static std::unique_ptr<GrFragmentProcessor> Make(
      sk_sp<GrSurfaceProxy>, const SkIRect& subset, const SkIPoint& deviceSpaceOffset);
  ...
  SkString dumpInfo() const override {
    SkString str;
    str.appendf(
        "Domain: [L: %.2f, T: %.2f, R: %.2f, B: %.2f] Offset: [%d %d]",
        fTextureDomain.domain().fLeft, fTextureDomain.domain().fTop, fTextureDomain.domain().fRight,
        fTextureDomain.domain().fBottom, fDeviceSpaceOffset.fX, fDeviceSpaceOffset.fY);
    str.append(INHERITED::dumpInfo());
    return str;
  }
#endif
 ...
 private:
  ...
  GrDeviceSpaceTextureDecalFragmentProcessor(
      sk_sp<GrSurfaceProxy>, const SkIRect&, const SkIPoint&);
