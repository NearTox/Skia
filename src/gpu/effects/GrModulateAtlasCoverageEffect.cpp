...
GrModulateAtlasCoverageEffect::GrModulateAtlasCoverageEffect(
    Flags flags, std::unique_ptr<GrFragmentProcessor> inputFP, GrSurfaceProxyView atlasView,
    const SkMatrix& devToAtlasMatrix, const SkIRect& devIBounds)
    : GrFragmentProcessor(
  ...
  this->registerChild(
      GrTextureEffect::Make(
          std::move(atlasView), kUnknown_SkAlphaType, devToAtlasMatrix,
...
std::unique_ptr<GrFragmentProcessor::ProgramImpl> GrModulateAtlasCoverageEffect::onMakeProgramImpl()
    const {
    ...
    void emitCode(EmitArgs& args) override {
      ...
      if (fp.fFlags & Flags::kCheckBounds) {
        ...
        fBoundsUniform = uniHandler->addUniform(
            &fp, kFragment_GrShaderFlag, kFloat4_GrSLType, "bounds", &boundsName);
      ...
      fCoverageMaybeInvertUniform = uniHandler->addUniform(
          &fp, kFragment_GrShaderFlag, kHalf2_GrSLType, "coverageInvert", &coverageMaybeInvertName);
      // Invert coverage, if needed.
      f->codeAppendf(
          "coverage = coverage * %s.x + %s.y;", coverageMaybeInvertName, coverageMaybeInvertName);
