...
void GrGaussianConvolutionFragmentProcessor::Impl::emitCode(EmitArgs& args) {
  ...
  const char* inc;
  fIncrementUni =
      uniformHandler->addUniform(&ce, kFragment_GrShaderFlag, kHalf2_GrSLType, "Increment", &inc);
...
static void fill_in_1D_gaussian_kernel(float* kernel, float gaussianSigma, int radius) noexcept {
...
std::unique_ptr<GrFragmentProcessor> GrGaussianConvolutionFragmentProcessor::Make(
    ...
    float gaussianSigma, GrSamplerState::WrapMode wm, const int bounds[2], const GrCaps& caps) {
  ...
  if (bounds) {
    ...
    SkRect subset;
    switch (dir) {
      case Direction::kX: subset = SkRect::MakeLTRB(bounds[0], 0, bounds[1], view.height()); break;
      case Direction::kY: subset = SkRect::MakeLTRB(0, bounds[0], view.width(), bounds[1]); break;
    }
...
std::unique_ptr<GrFragmentProcessor> GrGaussianConvolutionFragmentProcessor::TestCreate(
  ...
  return GrGaussianConvolutionFragmentProcessor::Make(
      std::move(view), at, dir, radius, sigma, wm, bounds, *d->caps());
}
#endif
