      ...
      switch (ce.mode()) {
        case GrTextureDomain::kClamp_Mode: {
          fragBuilder->codeAppendf(
              "coordSampled.%s = clamp(coord.%s, %s.x, %s.y);\n", component, component, bounds,
              bounds);
          break;
        }
        ...
        case GrTextureDomain::kDecal_Mode: {
          fragBuilder->codeAppendf(
              "if (coord.%s >= %s.x && coord.%s <= %s.y) {", component, bounds, component, bounds);
          break;
        }
        default: {
...
void GrGLConvolutionEffect::onSetData(
...
GrGaussianConvolutionFragmentProcessor::GrGaussianConvolutionFragmentProcessor(
    sk_sp<GrTextureProxy> proxy, GrColorType srcColorType, Direction direction, int radius,
    float gaussianSigma, GrTextureDomain::Mode mode, int bounds[2])
    : INHERITED(
          kGrGaussianConvolutionFragmentProcessor_ClassID,
          ModulateForSamplerOptFlags(srcColorType, mode == GrTextureDomain::kDecal_Mode)),
...
std::unique_ptr<GrFragmentProcessor> GrGaussianConvolutionFragmentProcessor::TestCreate(
  ...
  return GrGaussianConvolutionFragmentProcessor::Make(
      std::move(proxy), d->textureProxyColorType(texIdx), dir, radius, sigma,
