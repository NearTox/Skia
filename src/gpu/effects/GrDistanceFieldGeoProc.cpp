 ...
 public:
  ...
  void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
    ...
    this->writeLocalCoord(
        vertBuilder, uniformHandler, gpArgs, gpArgs->fPositionVar, dfTexEffect.localMatrix(),
        &fLocalMatrixUniform);
    ...
    bool isAliased = SkToBool(dfTexEffect.getFlags() & kAliased_DistanceFieldEffectFlag);
...
GrGLSLPrimitiveProcessor* GrDistanceFieldA8TextGeoProc::createGLSLInstance(
  ...
  void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
    ...
    if (dfPathEffect.matrix().hasPerspective()) {
      ...
      this->writeOutputPosition(
          vertBuilder, uniformHandler, gpArgs, dfPathEffect.inPosition().name(),
          dfPathEffect.matrix(), &fMatrixUniform);
      ...
      this->writeLocalCoord(
          vertBuilder, uniformHandler, gpArgs, dfPathEffect.inPosition().asShaderVar(),
          dfPathEffect.matrix(), &fMatrixUniform);
    }
    ...
    bool isGammaCorrect = SkToBool(dfPathEffect.getFlags() & kGammaCorrect_DistanceFieldEffectFlag);
    if (isUniformScale) {
      // For uniform scale, we adjust for the effect of the transformation on the distance
      // by using the length of the gradient of the t coordinate in the y direction.
      // We use st coordinates to ensure we're mapping 1:1 from texel space to pixel space.
...
GrGLSLPrimitiveProcessor* GrDistanceFieldPathGeoProc::createGLSLInstance(
    const GrShaderCaps&) const {
...
GrGeometryProcessor* GrDistanceFieldPathGeoProc::TestCreate(GrProcessorTestData* d) {
  ...
  return GrDistanceFieldPathGeoProc::Make(
      ...
      d->fRandom->nextBool(), &view, 1, samplerState, flags);
}
#endif
...
class GrGLDistanceFieldLCDTextGeoProc : public GrGLSLGeometryProcessor {
 public:
  GrGLDistanceFieldLCDTextGeoProc()
      : fAtlasDimensions({0, 0}), fLocalMatrix(SkMatrix::InvalidMatrix()) {
  ...
  void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
    ...
    gpArgs->fPositionVar = dfTexEffect.inPosition().asShaderVar();
    this->writeLocalCoord(
        vertBuilder, uniformHandler, gpArgs, dfTexEffect.inPosition().asShaderVar(),
  ...
  static inline void GenKey(
    ...
    uint32_t key = (dfTexEffect.getFlags() << 16) | ComputeMatrixKey(dfTexEffect.localMatrix());
    b->add32(key);
    b->add32(dfTexEffect.numTextureSamplers());
  }
...
GrGLSLPrimitiveProcessor* GrDistanceFieldLCDTextGeoProc::createGLSLInstance(
    const GrShaderCaps&) const {
