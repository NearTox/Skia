...
GrMatrixConvolutionEffect::KernelWrapper::MakeResult GrMatrixConvolutionEffect::KernelWrapper::Make(
    GrRecordingContext* rContext, SkISize size, const GrCaps& caps, const SkScalar* values) {
  ...
  bool useA16 = rContext->defaultBackendFormat(kA16_float_SkColorType, GrRenderable::kNo).isValid();
  ...
  for (int i = 0; i < length; i++) {
    if (useA16) {
      ...
      *bm.getAddr8(i, 0) = SkScalarRoundToInt((values[i] - min) / scalableSampler.fGain * 255);
    }
  }
  bm.setImmutable();
...
bool GrMatrixConvolutionEffect::KernelWrapper::BiasAndGain::operator==(const BiasAndGain& k) const {
...
void GrGLMatrixConvolutionEffect::emitKernelBlock(EmitArgs& args, SkIPoint loc) {
  ...
  if (mce.kernelIsSampled()) {
    ...
    fragBuilder->codeAppendf("k = %s[%d]%s;", kernel, offset / 4, kVecSuffix[offset & 0x3]);
  }
...
void GrGLMatrixConvolutionEffect::emitCode(EmitArgs& args) {
  ...
  if (mce.kernelIsSampled()) {
    fKernelBiasUni =
        uniformHandler->addUniform(&mce, kFragment_GrShaderFlag, kHalf_GrSLType, "KernelBias");
  } else {
    fKernelUni = uniformHandler->addUniformArray(
        &mce, kFragment_GrShaderFlag, kHalf4_GrSLType, "Kernel", arrayCount);
  }
  fKernelOffsetUni =
      uniformHandler->addUniform(&mce, kFragment_GrShaderFlag, kHalf2_GrSLType, "KernelOffset");
  ...
  if (mce.convolveAlpha()) {
    ...
    fragBuilder->codeAppendf(
        "%s.rgb = clamp(%s.rgb, 0.0, %s.a);", args.fOutputColor, args.fOutputColor,
        args.fOutputColor);
  } else {
    ...
    fragBuilder->codeAppendf(
        "%s.rgb = saturate(sum.rgb * %s + %s);", args.fOutputColor, gain, bias);
...
std::unique_ptr<GrFragmentProcessor> GrMatrixConvolutionEffect::Make(
    GrRecordingContext* context, GrSurfaceProxyView srcView, const SkIRect& srcBounds,
  ...
  auto child = GrTextureEffect::MakeSubset(
...
std::unique_ptr<GrFragmentProcessor> GrMatrixConvolutionEffect::MakeGaussian(
    ...
    GrSamplerState::WrapMode wm, bool convolveAlpha, SkScalar sigmaX, SkScalar sigmaY,
    const GrCaps& caps) {
  SkAutoSTMalloc<32, float> kernel(kernelSize.area());
  fill_in_2D_gaussian_kernel(kernel.get(), kernelSize.width(), kernelSize.height(), sigmaX, sigmaY);
  return Make(
      context, std::move(srcView), srcBounds, kernelSize, kernel.get(), gain, bias, kernelOffset,
      wm, convolveAlpha, caps);
}
...
std::unique_ptr<GrFragmentProcessor> GrMatrixConvolutionEffect::TestCreate(GrProcessorTestData* d) {
  ...
  return GrMatrixConvolutionEffect::Make(
      d->context(), std::move(view), bounds, kernelSize, kernel.get(), gain, bias, kernelOffset, wm,
      convolveAlpha, *d->caps());
}
#endif
