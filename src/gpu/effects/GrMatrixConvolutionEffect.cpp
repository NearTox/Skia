...
void GrGLMatrixConvolutionEffect::emitCode(EmitArgs& args) {
  ...
  for (int y = 0; y < kHeight; y++) {
    for (int x = 0; x < kWidth; x++) {
      ...
      fragBuilder->codeAppendf("half k = %s[%d]%s;", kernel, offset / 4, kVecSuffix[offset & 0x3]);
      SkString coord;
      coord.printf("coord + half2(%d, %d) * %s", x, y, imgInc);
      fDomain.sampleTexture(
          fragBuilder, uniformHandler, args.fShaderCaps, domain, "c", coord, args.fTexSamplers[0]);
  ...
  if (mce.convolveAlpha()) {
    ...
    fDomain.sampleTexture(
        fragBuilder, uniformHandler, args.fShaderCaps, domain, "c", coords2D, args.fTexSamplers[0]);
...
std::unique_ptr<GrFragmentProcessor> GrMatrixConvolutionEffect::TestCreate(GrProcessorTestData* d) {
  ...
  SkIPoint kernelOffset = SkIPoint::Make(
      d->fRandom->nextRangeU(0, kernelSize.width()),
      d->fRandom->nextRangeU(0, kernelSize.height()));
  SkIRect bounds = SkIRect::MakeXYWH(
