 ...
 public:
  ...
  SkString dumpInfo() const override {
    ...
    for (int i = 0; i < this->numChildProcessors(); ++i) {
      str.appendf(
          " [%s %s]", this->childProcessor(i).name(), this->childProcessor(i).dumpInfo().c_str());
    }
    return str;
  }
#endif
...
class ComposeOneFragmentProcessor : public GrFragmentProcessor {
 public:
  ...
  SkString dumpInfo() const override {
    ...
    str.appendf(
        "Mode: %s, Child: %s", SkBlendMode_Name(fMode), kDst_Child == fChild ? "Dst" : "Src");
 ...
 private:
  OptimizationFlags OptFlags(const GrFragmentProcessor* fp, SkBlendMode mode, Child child) {
    ...
    switch (mode) {
      ...
      case SkBlendMode::kDstIn:
      case SkBlendMode::kModulate:
        flags = ProcessorOptimizationFlags(fp) & ~kConstantOutputForConstantInput_OptimizationFlag;
        break;
      ...
      case SkBlendMode::kSaturation:
      case SkBlendMode::kColor:
      case SkBlendMode::kLuminosity: flags = kPreservesOpaqueInput_OptimizationFlag; break;
    }
