...
class BlendFragmentProcessor : public GrFragmentProcessor {
 ...
 private:
  ...
  BlendFragmentProcessor(const BlendFragmentProcessor& that) : INHERITED(that), fMode(that.fMode) {}
  ...
  static OptimizationFlags OptFlags(
    ...
    if (does_cpu_blend_impl_match_gpu(mode) && (!src || src->hasConstantOutputForConstantInput()) &&
   ...
   public:
    void emitCode(EmitArgs& args) override {
      ...
      fragBuilder->codeAppendf(
          "return %s(%s, %s);", GrGLSLBlend::BlendFuncName(mode), srcColor.c_str(),
          dstColor.c_str());
    }
  };
...
std::unique_ptr<GrFragmentProcessor> GrBlendFragmentProcessor::Make(
    ...
    SkBlendMode mode) {
  switch (mode) {
    case SkBlendMode::kClear: return GrFragmentProcessor::MakeColor(SK_PMColor4fTRANSPARENT);
    case SkBlendMode::kSrc: return src;
    case SkBlendMode::kDst: return dst;
    default: return BlendFragmentProcessor::Make(std::move(src), std::move(dst), mode);
  }
}
