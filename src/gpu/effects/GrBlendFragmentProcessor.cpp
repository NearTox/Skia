 ...
 private:
  BlendFragmentProcessor(
    ...
    if (fBlendBehavior == BlendBehavior::kDefault) {
      fBlendBehavior =
          (src && dst) ? BlendBehavior::kComposeTwoBehavior : BlendBehavior::kComposeOneBehavior;
    }
    ...
    switch (mode) {
      ...
      case SkBlendMode::kModulate:
        if (src && dst) {
          ...
          flags =
              ProcessorOptimizationFlags(src) & ~kConstantOutputForConstantInput_OptimizationFlag;
        } else if (dst) {
          flags =
              ProcessorOptimizationFlags(dst) & ~kConstantOutputForConstantInput_OptimizationFlag;
        } else {
          flags = kNone_OptimizationFlags;
        }
        break;
      ...
      case SkBlendMode::kXor: flags = kNone_OptimizationFlags; break;
  ...
  SkPMColor4f constantOutputForConstantInput(const SkPMColor4f& input) const override {
    ...
    switch (fBlendBehavior) {
      case BlendBehavior::kComposeOneBehavior: {
        SkPMColor4f srcColor = src ? ConstantOutputForConstantInput(src, SK_PMColor4fWHITE) : input;
        SkPMColor4f dstColor = dst ? ConstantOutputForConstantInput(dst, SK_PMColor4fWHITE) : input;
      ...
      default: SK_ABORT("unrecognized blend behavior"); return input;
    }
  }
...
void GLBlendFragmentProcessor::emitCode(EmitArgs& args) {
  ...
  switch (behavior) {
    ...
    default: SK_ABORT("unrecognized blend behavior"); break;
  }

  // Blend src and dst colors together.
  GrGLSLBlend::AppendMode(fragBuilder, srcColor.c_str(), dstColor.c_str(), args.fOutputColor, mode);
...
std::unique_ptr<GrFragmentProcessor> GrBlendFragmentProcessor::Make(
    ...
    SkBlendMode mode, BlendBehavior behavior) {
  switch (mode) {
    case SkBlendMode::kClear: return GrConstColorProcessor::Make(SK_PMColor4fTRANSPARENT);
    case SkBlendMode::kSrc:
      return GrFragmentProcessor::OverrideInput(
          std::move(src), SK_PMColor4fWHITE,
          /*useUniform=*/false);
    case SkBlendMode::kDst:
      return GrFragmentProcessor::OverrideInput(
          std::move(dst), SK_PMColor4fWHITE,
          /*useUniform=*/false);
    default: return BlendFragmentProcessor::Make(std::move(src), std::move(dst), mode, behavior);
  }
}
