...
class CircularRRectEffect : public GrFragmentProcessor {
 public:
  enum CornerFlags {
    ...
    kAll_CornerFlags = kTopLeft_CornerFlag | kTopRight_CornerFlag | kBottomLeft_CornerFlag |
  ...
  static GrFPResult Make(
      std::unique_ptr<GrFragmentProcessor>, GrClipEdgeType, uint32_t circularCornerFlags,
      const SkRRect&);
 ...
 private:
  ...
  CircularRRectEffect(
      std::unique_ptr<GrFragmentProcessor> inputFP, GrClipEdgeType, uint32_t circularCornerFlags,
      const SkRRect&);
...
CircularRRectEffect::CircularRRectEffect(
    ...
    uint32_t circularCornerFlags, const SkRRect& rrect)
    : INHERITED(
          kCircularRRectEffect_ClassID, ProcessorOptimizationFlags(inputFP.get()) &
...
std::unique_ptr<GrFragmentProcessor> CircularRRectEffect::TestCreate(GrProcessorTestData* d) {
  ...
  do {
    ...
    std::tie(success, fp) = GrRRectEffect::Make(std::move(fp), et, rrect, *d->caps()->shaderCaps());
  } while (!success);
  return fp;
}
#endif
...
void CircularRRectEffect::Impl::emitCode(EmitArgs& args) {
  ...
  fInnerRectUniform = uniformHandler->addUniform(
      &crre, kFragment_GrShaderFlag, kFloat4_GrSLType, "innerRect", &rectName);
  // x is (r + .5) and y is 1/(r + .5)
  fRadiusPlusHalfUniform = uniformHandler->addUniform(
      &crre, kFragment_GrShaderFlag, kHalf2_GrSLType, "radiusPlusHalf", &radiusPlusHalfName);
  ...
  if (!args.fShaderCaps->floatIs32Bits()) {
    clampedCircleDistance.printf(
        "saturate(%s.x * (1.0 - length(dxy * %s.y)))", radiusPlusHalfName, radiusPlusHalfName);
  } else {
  ...
  switch (crre.fCircularCornerFlags) {
    ...
    case CircularRRectEffect::kTopLeft_CornerFlag:
      fragBuilder->codeAppendf("float2 dxy = max(%s.LT - sk_FragCoord.xy, 0.0);", rectName);
      fragBuilder->codeAppendf(
          "half rightAlpha = half(saturate(%s.R - sk_FragCoord.x));", rectName);
      fragBuilder->codeAppendf(
          "half bottomAlpha = half(saturate(%s.B - sk_FragCoord.y));", rectName);
      fragBuilder->codeAppendf(
          "half alpha = bottomAlpha * rightAlpha * half(%s);", clampedCircleDistance.c_str());
      break;
    case CircularRRectEffect::kTopRight_CornerFlag:
      fragBuilder->codeAppendf(
          ...
          rectName, rectName);
      fragBuilder->codeAppendf("half leftAlpha = half(saturate(sk_FragCoord.x - %s.L));", rectName);
      fragBuilder->codeAppendf(
          "half bottomAlpha = half(saturate(%s.B - sk_FragCoord.y));", rectName);
      fragBuilder->codeAppendf(
          "half alpha = bottomAlpha * leftAlpha * half(%s);", clampedCircleDistance.c_str());
      break;
    case CircularRRectEffect::kBottomRight_CornerFlag:
      fragBuilder->codeAppendf("float2 dxy = max(sk_FragCoord.xy - %s.RB, 0.0);", rectName);
      fragBuilder->codeAppendf("half leftAlpha = half(saturate(sk_FragCoord.x - %s.L));", rectName);
      fragBuilder->codeAppendf("half topAlpha = half(saturate(sk_FragCoord.y - %s.T));", rectName);
      fragBuilder->codeAppendf(
          "half alpha = topAlpha * leftAlpha * half(%s);", clampedCircleDistance.c_str());
      break;
    case CircularRRectEffect::kBottomLeft_CornerFlag:
      ...
      fragBuilder->codeAppendf("half topAlpha = half(saturate(sk_FragCoord.y - %s.T));", rectName);
      fragBuilder->codeAppendf(
          "half alpha = topAlpha * rightAlpha * half(%s);", clampedCircleDistance.c_str());
      break;
    case CircularRRectEffect::kLeft_CornerFlags:
      ...
      fragBuilder->codeAppendf(
          "half rightAlpha = half(saturate(%s.R - sk_FragCoord.x));", rectName);
      fragBuilder->codeAppendf(
          "half alpha = rightAlpha * half(%s);", clampedCircleDistance.c_str());
      break;
    case CircularRRectEffect::kTop_CornerFlags:
      ...
      fragBuilder->codeAppendf(
          "half bottomAlpha = half(saturate(%s.B - sk_FragCoord.y));", rectName);
      fragBuilder->codeAppendf(
          "half alpha = bottomAlpha * half(%s);", clampedCircleDistance.c_str());
      break;
    case CircularRRectEffect::kRight_CornerFlags:
      ...
      fragBuilder->codeAppendf("half leftAlpha = half(saturate(sk_FragCoord.x - %s.L));", rectName);
      fragBuilder->codeAppendf("half alpha = leftAlpha * half(%s);", clampedCircleDistance.c_str());
      break;
    case CircularRRectEffect::kBottom_CornerFlags:
      ...
      fragBuilder->codeAppendf("half topAlpha = half(saturate(sk_FragCoord.y - %s.T));", rectName);
      fragBuilder->codeAppendf("half alpha = topAlpha * half(%s);", clampedCircleDistance.c_str());
      break;
  }
...
void CircularRRectEffect::Impl::onSetData(
  ...
  if (rrect != fPrevRRect) {
    ...
    switch (crre.fCircularCornerFlags) {
      ...
      case CircularRRectEffect::kBottom_CornerFlags:
        ...
        rect.fBottom -= radius;
        break;
      default: SK_ABORT("Should have been one of the above cases.");
    }
...
GrFPResult EllipticalRRectEffect::Make(
    std::unique_ptr<GrFragmentProcessor> inputFP, GrClipEdgeType edgeType, const SkRRect& rrect) {
  if (GrClipEdgeType::kFillAA != edgeType && GrClipEdgeType::kInverseFillAA != edgeType) {
    return GrFPFailure(std::move(inputFP));
  }
  return GrFPSuccess(std::unique_ptr<GrFragmentProcessor>(
...
EllipticalRRectEffect::EllipticalRRectEffect(
    std::unique_ptr<GrFragmentProcessor> inputFP, GrClipEdgeType edgeType, const SkRRect& rrect)
    : INHERITED(
          kEllipticalRRectEffect_ClassID, ProcessorOptimizationFlags(inputFP.get()) &
                                              kCompatibleWithCoverageAsAlpha_OptimizationFlag),
      fRRect(rrect),
      fEdgeType(edgeType) {
  this->registerChild(std::move(inputFP));
...
EllipticalRRectEffect::EllipticalRRectEffect(const EllipticalRRectEffect& that)
    : INHERITED(that), fRRect(that.fRRect), fEdgeType(that.fEdgeType) {}
...
std::unique_ptr<GrFragmentProcessor> EllipticalRRectEffect::TestCreate(GrProcessorTestData* d) {
  ...
  if (d->fRandom->nextBool()) {
    ...
    rrect.setRectXY(
        SkRect::MakeWH(w, h), r[SkRRect::kUpperLeft_Corner].fX, r[SkRRect::kUpperLeft_Corner].fY);
  }
  std::unique_ptr<GrFragmentProcessor> fp = d->inputFP();
  bool success;
  do {
...
void EllipticalRRectEffect::Impl::emitCode(EmitArgs& args) {
  ...
  fInnerRectUniform = uniformHandler->addUniform(
      &erre, kFragment_GrShaderFlag, kFloat4_GrSLType, "innerRect", &rectName);
  ...
  if (!args.fShaderCaps->floatIs32Bits()) {
    fScaleUniform = uniformHandler->addUniform(
        &erre, kFragment_GrShaderFlag, kHalf2_GrSLType, "scale", &scaleName);
  }
  ...
  switch (erre.fRRect.getType()) {
    case SkRRect::kSimple_Type: {
      ...
      fInvRadiiSqdUniform = uniformHandler->addUniform(
          &erre, kFragment_GrShaderFlag, kFloat2_GrSLType, "invRadiiXY", &invRadiiXYSqdName);
      fragBuilder->codeAppend("float2 dxy = max(max(dxy0, dxy1), 0.0);");
      if (scaleName) {
    ...
    case SkRRect::kNinePatch_Type: {
      ...
      fInvRadiiSqdUniform = uniformHandler->addUniform(
          &erre, kFragment_GrShaderFlag, kFloat4_GrSLType, "invRadiiLTRB", &invRadiiLTRBSqdName);
      if (scaleName) {
      ...
      fragBuilder->codeAppendf(
          "float2 Z = max(max(dxy0 * %s.xy, dxy1 * %s.zw), 0.0);", invRadiiLTRBSqdName,
          invRadiiLTRBSqdName);

      break;
    }
    default: SK_ABORT("RRect should always be simple or nine-patch.");
  }
...
void EllipticalRRectEffect::Impl::onSetData(
    ...
    switch (rrect.getType()) {
      case SkRRect::kSimple_Type:
        ...
        if (fScaleUniform.isValid()) {
          ...
          pdman.set2f(fInvRadiiSqdUniform, 1.f / (r0.fX * r0.fX), 1.f / (r0.fY * r0.fY));
        }
        break;
      case SkRRect::kNinePatch_Type: {
        ...
        if (fScaleUniform.isValid()) {
          ...
          pdman.set4f(
              fInvRadiiSqdUniform, scaleSqd / (r0.fX * r0.fX), scaleSqd / (r0.fY * r0.fY),
              scaleSqd / (r1.fX * r1.fX), scaleSqd / (r1.fY * r1.fY));
          pdman.set2f(fScaleUniform, scale, 1.f / scale);
        } else {
          pdman.set4f(
              fInvRadiiSqdUniform, 1.f / (r0.fX * r0.fX), 1.f / (r0.fY * r0.fY),
              1.f / (r1.fX * r1.fX), 1.f / (r1.fY * r1.fY));
        }
        break;
      }
      default: SK_ABORT("RRect should always be simple or nine-patch.");
...
GrFPResult GrRRectEffect::Make(
  ...
  if (rrect.isSimple()) {
    ...
    if (SkRRectPriv::GetSimpleRadii(rrect).fX == SkRRectPriv::GetSimpleRadii(rrect).fY) {
      return CircularRRectEffect::Make(
          std::move(inputFP), edgeType, CircularRRectEffect::kAll_CornerFlags, rrect);
    } else {
  ...
  if (rrect.isComplex() || rrect.isNinePatch()) {
    ...
    switch (cornerFlags) {
      ...
      default: {
        ...
        if (rrect.isNinePatch() && ul.fX >= kRadiusMin && ul.fY >= kRadiusMin &&
            lr.fX >= kRadiusMin && lr.fY >= kRadiusMin) {
          return EllipticalRRectEffect::Make(std::move(inputFP), edgeType, rrect);
        }
        return GrFPFailure(std::move(inputFP));
      }
