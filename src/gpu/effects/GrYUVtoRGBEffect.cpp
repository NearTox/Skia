...
std::unique_ptr<GrFragmentProcessor> GrYUVtoRGBEffect::Make(const sk_sp<GrTextureProxy> proxies[],
    ...
    return std::unique_ptr<GrFragmentProcessor>(
            new GrYUVtoRGBEffect(proxies, scales, filterModes, numPlanes, yuvaIndices,
                                 yuvColorSpace, localMatrix, domain));
}
...
SkString GrYUVtoRGBEffect::dumpInfo() const {
    ...
    for (int i = 0; i < this->numTextureSamplers(); ++i) {
        str.appendf("%d: %d %d ", i, this->textureSampler(i).proxy()->uniqueID().asUInt(),
...
GrGLSLFragmentProcessor* GrYUVtoRGBEffect::onCreateGLSLInstance() const {
        ...
        void emitCode(EmitArgs& args) override {
            ...
            for (int i = 0; i < numSamplers; ++i) {
                ...
                fGLDomains[i].sampleTexture(fragBuilder, args.fUniformHandler, args.fShaderCaps,
                                            _outer.fDomains[i], sampleVar.c_str(), coords[i],
                                            args.fTexSamplers[i]);
            }
            ...
            if (kIdentity_SkYUVColorSpace != _outer.yuvColorSpace()) {
                ...
                fragBuilder->codeAppendf("yuvOne *= %s;", args.fUniformHandler->getUniformCStr(
                                                                  fColorSpaceMatrixVar));
            }

            if (_outer.yuvaIndex(3).fIndex >= 0) {
                fragBuilder->codeAppendf("half a = tmp%d.%c;", _outer.yuvaIndex(3).fIndex,
    ...
    private:
        void onSetData(const GrGLSLProgramDataManager& pdman,
            ...
            for (int i = 0; i < numSamplers; ++i) {
                fGLDomains[i].setData(pdman, _outer.fDomains[i], _outer.textureSampler(i).proxy(),
