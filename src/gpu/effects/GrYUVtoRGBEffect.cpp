  ...
  for (int i = 0; i < numPlanes; ++i) {
    ...
    if (subset) {
      ...
      planeFPs[i] = GrTextureEffect::MakeSubset(
          views[i], kUnknown_SkAlphaType, *planeMatrix, samplerState, planeDomain, caps,
          planeBorders[i]);
    } else {
      planeFPs[i] = GrTextureEffect::Make(
          views[i], kUnknown_SkAlphaType, *planeMatrix, samplerState, caps, planeBorders[i]);
    }
  }
...
SkString GrYUVtoRGBEffect::dumpInfo() const {
  ...
  for (int i = 0; i < this->numTextureSamplers(); ++i) {
    str.appendf(
        "%d: %d %d ", i, this->textureSampler(i).view().proxy()->uniqueID().asUInt(),
