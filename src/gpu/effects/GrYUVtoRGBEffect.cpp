...
std::unique_ptr<GrFragmentProcessor> GrYUVtoRGBEffect::Make(
    ...
    SkYUVColorSpace yuvColorSpace, GrSamplerState::Filter filterMode, const GrCaps& caps,
  ...
  for (int i = 0; i < numPlanes; ++i) {
    ...
    if (domain) {
      ...
      planeFPs[i] = GrTextureEffect::MakeSubset(
          proxies[i], kUnknown_SkAlphaType, *planeMatrix, planeFilter, planeDomain, caps);
    } else {
      planeFPs[i] =
