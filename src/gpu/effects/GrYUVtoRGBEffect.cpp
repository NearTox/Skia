...
std::unique_ptr<GrFragmentProcessor> GrYUVtoRGBEffect::Make(
    const sk_sp<GrTextureProxy> proxies[], const SkYUVAIndex yuvaIndices[4],
    SkYUVColorSpace yuvColorSpace, GrSamplerState::Filter filterMode, const SkMatrix& localMatrix,
    const SkRect* domain) {
  int numPlanes;
  ...
  for (int i = 0; i < numPlanes; ++i) {
    ...
    scales[i] = {
        dimensions.width() / SkIntToScalar(YDimensions.width()),
  ...
  return std::unique_ptr<GrFragmentProcessor>(new GrYUVtoRGBEffect(
      proxies, scales, filterModes, numPlanes, yuvaIndices, yuvColorSpace, localMatrix, domain));
}
...
SkString GrYUVtoRGBEffect::dumpInfo() const {
  ...
  for (int i = 0; i < this->numTextureSamplers(); ++i) {
    str.appendf(
        "%d: %d %d ", i, this->textureSampler(i).proxy()->uniqueID().asUInt(),
...
GrGLSLFragmentProcessor* GrYUVtoRGBEffect::onCreateGLSLInstance() const {
  class GrGLSLYUVtoRGBEffect : public GrGLSLFragmentProcessor {
    ...
    void emitCode(EmitArgs& args) override {
      ...
      if (kIdentity_SkYUVColorSpace != _outer.yuvColorSpace()) {
        fColorSpaceMatrixVar = args.fUniformHandler->addUniform(
            kFragment_GrShaderFlag, kHalf4x4_GrSLType, "colorSpaceMatrix");
      }
      ...
      for (int i = 0; i < numSamplers; ++i) {
        ...
        fGLDomains[i].sampleTexture(
            fragBuilder, args.fUniformHandler, args.fShaderCaps, _outer.fDomains[i],
            sampleVar.c_str(), coords[i], args.fTexSamplers[i]);
      }

      static const char kChannelToChar[4] = {'x', 'y', 'z', 'w'};

      fragBuilder->codeAppendf(
          "half4 yuvOne = half4(tmp%d.%c, tmp%d.%c, tmp%d.%c, 1.0);", _outer.yuvaIndex(0).fIndex,
          kChannelToChar[(int)_outer.yuvaIndex(0).fChannel], _outer.yuvaIndex(1).fIndex,
          kChannelToChar[(int)_outer.yuvaIndex(1).fChannel], _outer.yuvaIndex(2).fIndex,
          kChannelToChar[(int)_outer.yuvaIndex(2).fChannel]);
   ...
   private:
    void onSetData(
        const GrGLSLProgramDataManager& pdman, const GrFragmentProcessor& _proc) override {
      ...
      if (_outer.yuvColorSpace() != kIdentity_SkYUVColorSpace) {
        ...
        float mtx[16] = {
            yuvM[0],  yuvM[1],  yuvM[2],  yuvM[4],  yuvM[5],  yuvM[6],  yuvM[7],  yuvM[9],
            yuvM[10], yuvM[11], yuvM[12], yuvM[14], yuvM[15], yuvM[16], yuvM[17], yuvM[19],
        };
      ...
      for (int i = 0; i < numSamplers; ++i) {
        fGLDomains[i].setData(
            pdman, _outer.fDomains[i], _outer.textureSampler(i).proxy(),
            _outer.textureSampler(i).samplerState());
      }
    }
