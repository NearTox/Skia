...
static void border_colors(
    SkYUVColorSpace cs, const SkYUVAIndex yuvaIndices[4], float planeBorders[4][4]) {
  float m[20];
...
std::unique_ptr<GrFragmentProcessor> GrYUVtoRGBEffect::Make(
    GrSurfaceProxyView views[], const SkYUVAIndex yuvaIndices[4], SkYUVColorSpace yuvColorSpace,
    GrSamplerState samplerState, const GrCaps& caps, const SkMatrix& localMatrix,
  ...
  for (int i = 0; i < numPlanes; ++i) {
    ...
    if (dimensions != yDimensions) {
      ...
      *planeMatrix.writable() = SkMatrix::Scale(sx, sy);
      if (subset) {
        planeSubset = {
            subset->fLeft * sx, subset->fTop * sy, subset->fRight * sx, subset->fBottom * sy};
      }
      if (domain) {
        planeDomain = {
            domain->fLeft * sx, domain->fTop * sy, domain->fRight * sx, domain->fBottom * sy};
      }
      ...
      if (samplerState.filter() == GrSamplerState::Filter::kNearest) {
        bool snapX = (sx != 1.f), snapY = (sy != 1.f);
    ...
    if (subset) {
      ...
      if (makeLinearWithSnap) {
        ...
        planeFPs[i] = GrTextureEffect::MakeSubset(
            views[i], kUnknown_SkAlphaType, *planeMatrix, samplerState, planeSubset, planeDomain,
            caps, planeBorders[i]);
      } else {
        planeFPs[i] = GrTextureEffect::MakeSubset(
            views[i], kUnknown_SkAlphaType, *planeMatrix, samplerState, planeSubset, caps,
            planeBorders[i]);
      }
    } else {
      ...
      planeFPs[i] = GrTextureEffect::Make(
          views[i], kUnknown_SkAlphaType, *planeMatrix, planeSampler, caps, planeBorders[i]);
    }
  }
...
GrYUVtoRGBEffect::GrYUVtoRGBEffect(
    ...
    : GrFragmentProcessor(
          kGrYUVtoRGBEffect_ClassID, ModulateForClampedSamplerOptFlags(alpha_type(yuvaIndices))),
...
SkString GrYUVtoRGBEffect::onDumpInfo() const {
  ...
  for (int i = 0; i < 4; ++i) {
    str.appendf(
        "YUVAIndices[%d]=%d %d, ", i, fYUVAIndices[i].fIndex,
        static_cast<int>(fYUVAIndices[i].fChannel));
  }
  str.appendf(
...
bool GrYUVtoRGBEffect::onIsEqual(const GrFragmentProcessor& other) const {
  ...
  return std::equal(fYUVAIndices, fYUVAIndices + 4, that.fYUVAIndices) &&
         std::equal(fSnap, fSnap + 2, that.fSnap) && fYUVColorSpace == that.fYUVColorSpace;
}
