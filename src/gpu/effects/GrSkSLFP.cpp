  ...
  void emitCode(EmitArgs& args) override {
    ...
    for (const auto& f : fFunctions) {
      ...
      fragBuilder->emitFunction(
          f.fReturnType, f.fName.c_str(), f.fParameters.size(), f.fParameters.data(),
          f.fBody.c_str(), &fFunctionNames.back());
    }
    ...
    for (size_t i = 0; i < fGLSL.length(); ++i) {
      char c = fGLSL[i];
      if (c == '%') {
        fragBuilder->codeAppend(fGLSL.c_str() + substringStartIndex, i - substringStartIndex);
        ++i;
        c = fGLSL[i];
        switch (c) {
          case 's': {
            ...
            switch (arg.fKind) {
              ...
              case SkSL::Compiler::FormatArg::Kind::kUniform:
                fragBuilder->codeAppend(
                    args.fUniformHandler->getUniformCStr(fUniformHandles[arg.fIndex]));
                break;
          ...
          default: fragBuilder->codeAppendf("%c", c);
        }
        substringStartIndex = i + 1;
      }
    }
    fragBuilder->codeAppend(
        fGLSL.c_str() + substringStartIndex, fGLSL.length() - substringStartIndex);
  }
