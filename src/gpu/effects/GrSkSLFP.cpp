  ...
  void emitCode(EmitArgs& args) override {
    ...
    class FPCallbacks : public SkSL::PipelineStage::Callbacks {
     public:
      FPCallbacks(
          ...
          const uint8_t* uniformData, const GrSkSLFP::UniformFlags* uniformFlags)
          : fSelf(self),
            fArgs(args),
      ...
      String declareUniform(const SkSL::VarDeclaration* decl) override {
        ...
        if (*fUniformFlags++ & GrSkSLFP::kSpecialize_Flag) {
          ...
          for (size_t i = 0; i < slots; ++i) {
            value.append(isFloat ? SkSL::to_string(floatData[i]) : SkSL::to_string(intData[i]));
            value.append(",");
          }
          value.back() = ')';
          return value;
    ...
    if (fp.fEffect->samplesOutsideMain()) {
      GrShaderVar inputColorCopy(
          args.fFragBuilder->getMangledFunctionName("inColor"), kHalf4_GrSLType);
    ...
    FPCallbacks callbacks(
        this, args, inputColorName.c_str(), *program.fContext, fp.uniformData(), fp.uniformFlags());
...
GrSkSLFP::GrSkSLFP(const GrSkSLFP& other)
  ...
  sk_careful_memcpy(
      this->uniformFlags(), other.uniformFlags(),
...
bool GrSkSLFP::onIsEqual(const GrFragmentProcessor& other) const {
  ...
  return fEffect->hash() == sk.fEffect->hash() &&
         ...
         !sk_careful_memcmp(this->uniformData(), sk.uniformData(), fUniformSize + uniformFlagSize);
}
