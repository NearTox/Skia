  ...
  void emitCode(EmitArgs& args) override {
    ...
    for (const auto& v : fp.fEffect->inputs()) {
      if (v.fQualifier == SkRuntimeEffect::Variable::Qualifier::kUniform) {
        auto handle = args.fUniformHandler->addUniformArray(
            kFragment_GrShaderFlag, v.fGPUType, v.fName.c_str(), v.isArray() ? v.fCount : 0);
    ...
    fragBuilder->codeAppend(
        this->expandFormatArgs(
                fArgs.fCode.c_str(), args, fArgs.fFormatArgs, coords.c_str(), childNames)
            .c_str());
  }
    ...
    for (const auto& v : outer.fEffect->inputs()) {
      ...
      switch (v.fType) {
        ...
        case SkRuntimeEffect::Variable::Type::kFloat4x4:
          ...
          break;
        default: SkDEBUGFAIL("Unsupported uniform type"); break;
      }
    }
  }
...
std::unique_ptr<GrSkSLFP> GrSkSLFP::Make(
    GrContext_Base* context, sk_sp<SkRuntimeEffect> effect, const char* name, const void* inputs,
    size_t inputSize, const SkMatrix* matrix) {
  return std::unique_ptr<GrSkSLFP>(new GrSkSLFP(
      context->priv().caps()->refShaderCaps(), std::move(effect), name, inputs, inputSize, matrix));
}

GrSkSLFP::GrSkSLFP(
    sk_sp<const GrShaderCaps> shaderCaps, sk_sp<SkRuntimeEffect> effect, const char* name,
...
void GrSkSLFP::onGetGLSLProcessorKey(const GrShaderCaps& caps, GrProcessorKeyBuilder* b) const {
  ...
  for (const auto& v : fEffect->inputs()) {
    ...
    SkASSERT(!v.isArray());
    switch (v.fType) {
      case SkRuntimeEffect::Variable::Type::kBool: b->add32(inputs[v.fOffset]); break;
      case SkRuntimeEffect::Variable::Type::kInt:
      case SkRuntimeEffect::Variable::Type::kFloat:
        b->add32(*(int32_t*)(inputs + v.fOffset));
        break;
      default: SkDEBUGFAIL("Unsupported input variable type"); break;
    }
  }
}
...
std::unique_ptr<GrFragmentProcessor> GrSkSLFP::TestCreate(GrProcessorTestData* d) {
  ...
  switch (type) {
    case 0: {
      ...
      auto result = GrSkSLFP::Make(d->context(), effect, "Dither", &rangeType, sizeof(rangeType));
      return std::unique_ptr<GrFragmentProcessor>(result.release());
    }
    case 1: {
      ...
      auto result = GrSkSLFP::Make(d->context(), effect, "Arithmetic", &inputs, sizeof(inputs));
    ...
    case 2: {
      ...
      auto result = GrSkSLFP::Make(d->context(), effect, "Overdraw", &inputs, sizeof(inputs));
      return std::unique_ptr<GrFragmentProcessor>(result.release());
    }
  }
