...
GrSkSLFPFactory::GrSkSLFPFactory(
    const char* name, const GrShaderCaps* shaderCaps, const char* sksl, SkSL::Program::Kind kind)
...
const SkSL::Program* GrSkSLFPFactory::getSpecialization(
    const SkSL::String& key, const void* inputs, size_t inputSize) {
  ...
  for (const auto& v : fInAndUniformVars) {
    ...
    if (&v->fType == fCompiler.context().fInt_Type.get() ||
      ...
      offset += sizeof(int32_t);
        ...
        &v->fType == fCompiler.context().fFloat_Type.get() ||
...
class GrGLSLSkSLFP : public GrGLSLFragmentProcessor {
 public:
  GrGLSLSkSLFP(
      const SkSL::Context* context, const std::vector<const SkSL::Variable*>* inAndUniformVars,
  ...
  SkSL::String expandFormatArgs(
      const SkSL::String& raw, const EmitArgs& args,
      const std::vector<SkSL::Compiler::FormatArg> formatArgs, const char* coordsName,
      ...
      if (c == '%') {
        ...
        switch (c) {
          case 's': {
            ...
            switch (arg.fKind) {
              case SkSL::Compiler::FormatArg::Kind::kInput: result += args.fInputColor; break;
              case SkSL::Compiler::FormatArg::Kind::kOutput: result += args.fOutputColor; break;
    ...
    result +=
        SkSL::StringFragment(raw.c_str() + substringStartIndex, raw.length() - substringStartIndex);
    return result;
  }
  ...
  void emitCode(EmitArgs& args) override {
    for (const auto& v : fInAndUniformVars) {
      if (v->fModifiers.fFlags & SkSL::Modifiers::kUniform_Flag &&
          v->fType != *fContext.fFragmentProcessor_Type) {
        fUniformHandles.push_back(args.fUniformHandler->addUniform(
            kFragment_GrShaderFlag, this->uniformType(v->fType), SkSL::String(v->fName).c_str()));
      }
    }
    ...
    for (const auto& f : fFunctions) {
      ...
      SkSL::String body =
          this->expandFormatArgs(f.fBody.c_str(), args, f.fFormatArgs, coords.c_str(), childNames);
      fragBuilder->emitFunction(
          f.fReturnType, f.fName.c_str(), f.fParameters.size(), f.fParameters.data(), body.c_str(),
          &fFunctionNames.back());
    }
    fragBuilder->codeAppend(
        this->expandFormatArgs(fGLSL.c_str(), args, fFormatArgs, coords.c_str(), childNames)
            .c_str());
  }
    ...
    for (const auto& v : outer.fFactory->fInAndUniformVars) {
      switch (get_ctype(fContext, *v)) {
        ...
        case SkSL::Layout::CType::kBool:
          ...
          ++offset;
          break;
        default: SkASSERT(&v->fType == fContext.fFragmentProcessor_Type.get());
      }
    }
  }
...
GrGLSLFragmentProcessor* GrSkSLFP::onCreateGLSLInstance() const {
  ...
  return new GrGLSLSkSLFP(
      specialized->fContext.get(), &fFactory->fInAndUniformVars, glsl, formatArgs, functions);
}

static void copy_floats_key(
    char* inputs, GrProcessorKeyBuilder* b, bool isIn, int count, size_t* offset,
    SkSL::String* key) {
  if (isIn) {
...
void GrSkSLFP::onGetGLSLProcessorKey(const GrShaderCaps& caps, GrProcessorKeyBuilder* b) const {
  ...
  for (const auto& v : fFactory->fInAndUniformVars) {
    ...
    switch (get_ctype(context, *v)) {
      ...
      case SkSL::Layout::CType::kFloat2:
        copy_floats_key(
            inputs, b, v->fModifiers.fFlags & SkSL::Modifiers::kIn_Flag, 2, &offset, &fKey);
        break;
      case SkSL::Layout::CType::kFloat3:
        copy_floats_key(
            inputs, b, v->fModifiers.fFlags & SkSL::Modifiers::kIn_Flag, 3, &offset, &fKey);
        break;
      case SkSL::Layout::CType::kSkPMColor:
      case SkSL::Layout::CType::kSkPMColor4f:
      case SkSL::Layout::CType::kSkRect:
        copy_floats_key(
            inputs, b, v->fModifiers.fFlags & SkSL::Modifiers::kIn_Flag, 4, &offset, &fKey);
        break;
      default:
...
std::unique_ptr<GrFragmentProcessor> GrSkSLFP::TestCreate(GrProcessorTestData* d) {
  ...
  switch (type) {
    case 0: {
      ...
      std::unique_ptr<GrSkSLFP> result = GrSkSLFP::Make(
          d->context(), ditherIndex, "Dither", SKSL_DITHER_SRC, &rangeType, sizeof(rangeType));
      return std::unique_ptr<GrFragmentProcessor>(result.release());
    }
    case 1: {
      ...
      std::unique_ptr<GrSkSLFP> result = GrSkSLFP::Make(
          d->context(), arithmeticIndex, "Arithmetic", SKSL_ARITHMETIC_SRC, &inputs,
          sizeof(inputs));
    ...
    case 2: {
      ...
      std::unique_ptr<GrSkSLFP> result = GrSkSLFP::Make(
          d->context(), overdrawIndex, "Overdraw", SKSL_OVERDRAW_SRC, &inputs, sizeof(inputs));
      return std::unique_ptr<GrFragmentProcessor>(result.release());
    }
  }
