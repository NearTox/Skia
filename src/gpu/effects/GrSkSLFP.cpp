        ...
        for (const auto& v : fInputVars) {
            if (v->fModifiers.fFlags & SkSL::Modifiers::kUniform_Flag &&
                ...
                fUniformHandles.push_back(
                        args.fUniformHandler->addUniform(kFragment_GrShaderFlag,
...
std::unique_ptr<GrSkSLFP> GrSkSLFP::Make(GrContext_Base* context, int index, const char* name,
    ...
    return std::unique_ptr<GrSkSLFP>(new GrSkSLFP(context->priv().fpFactoryCache(),
                                                  context->priv().caps()->shaderCaps(), kind, index,
                                                  name, sksl, SkString(), inputs, inputSize));
}
...
std::unique_ptr<GrSkSLFP> GrSkSLFP::Make(GrContext_Base* context, int index, const char* name,
    ...
    return std::unique_ptr<GrSkSLFP>(
            new GrSkSLFP(context->priv().fpFactoryCache(), context->priv().caps()->shaderCaps(),
                         kind, index, name, nullptr, std::move(sksl), inputs, inputSize));
}
...
void GrSkSLFP::createFactory() const {
    if (!fFactory) {
        ...
        if (!fFactory) {
            fFactory = sk_sp<GrSkSLFPFactory>(
                    new GrSkSLFPFactory(fName, fShaderCaps.get(), fSkSL, fKind));
...
std::unique_ptr<GrFragmentProcessor> GrSkSLFP::TestCreate(GrProcessorTestData* d) {
    ...
    switch (type) {
        case 0: {
            ...
            std::unique_ptr<GrSkSLFP> result =
                    GrSkSLFP::Make(d->context(), ditherIndex, "Dither", SKSL_DITHER_SRC, &rangeType,
                                   sizeof(rangeType));
            return std::unique_ptr<GrFragmentProcessor>(result.release());
        }
        case 1: {
            ...
            std::unique_ptr<GrSkSLFP> result =
                    GrSkSLFP::Make(d->context(), arithmeticIndex, "Arithmetic", SKSL_ARITHMETIC_SRC,
        ...
        case 2: {
            ...
            std::unique_ptr<GrSkSLFP> result =
                    GrSkSLFP::Make(d->context(), overdrawIndex, "Overdraw", SKSL_OVERDRAW_SRC,
                                   &inputs, sizeof(inputs));
            return std::unique_ptr<GrFragmentProcessor>(result.release());
        }
    }
