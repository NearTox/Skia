...
void GrGLConicEffect::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
  ...
  if (gp.usesLocalCoords()) {
    this->writeLocalCoord(
        vertBuilder, uniformHandler, gpArgs, gp.inPosition().asShaderVar(), gp.localMatrix(),
        &fLocalMatrixUniform);
  }
...
void GrGLConicEffect::GenKey(
  ...
  key =
      AddMatrixKeys(key, ce.viewMatrix(), ce.usesLocalCoords() ? ce.localMatrix() : SkMatrix::I());
  b->add32(key);
}
...
GrGeometryProcessor* GrConicEffect::TestCreate(GrProcessorTestData* d) {
  return GrConicEffect::Make(
      ...
      GrTest::TestMatrix(d->fRandom), *d->caps(), GrTest::TestMatrix(d->fRandom),
      d->fRandom->nextBool());
}
#endif
