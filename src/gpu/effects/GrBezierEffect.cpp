...
void GrGLConicEffect::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
  ...
  this->writeOutputPosition(
      vertBuilder, uniformHandler, gpArgs, gp.inPosition().name(), gp.viewMatrix(),
      &fViewMatrixUniform);

  // emit transforms with position
  this->emitTransforms(
      vertBuilder, varyingHandler, uniformHandler, gp.inPosition().asShaderVar(), gp.localMatrix(),
  ...
  switch (fEdgeType) {
    case GrClipEdgeType::kHairlineAA: {
      ...
      fragBuilder->codeAppendf(
          "%s = 2.0 * %s.x * %s.x - %s.y * %s.z - %s.z * %s.y;", dfdx.c_str(), v.fsIn(),
          dklmdx.c_str(), v.fsIn(), dklmdx.c_str(), v.fsIn(), dklmdx.c_str());
      fragBuilder->codeAppendf(
          "%s = 2.0 * %s.x * %s.x - %s.y * %s.z - %s.z * %s.y;", dfdy.c_str(), v.fsIn(),
          dklmdy.c_str(), v.fsIn(), dklmdy.c_str(), v.fsIn(), dklmdy.c_str());
    ...
    case GrClipEdgeType::kFillAA: {
      ...
      fragBuilder->codeAppendf(
          "%s = %s.x * %s.x - %s.y * %s.z;", func.c_str(), v.fsIn(), v.fsIn(), v.fsIn(), v.fsIn());
      fragBuilder->codeAppendf("%s = half(%s / %s);", edgeAlpha.c_str(), func.c_str(), gFM.c_str());
      fragBuilder->codeAppendf("%s = saturate(0.5 - %s);", edgeAlpha.c_str(), edgeAlpha.c_str());
      // Add line below for smooth cubic ramp
      // fragBuilder->codeAppend("edgeAlpha = edgeAlpha*edgeAlpha*(3.0-2.0*edgeAlpha);");
      break;
    }
    case GrClipEdgeType::kFillBW: {
      fragBuilder->codeAppendf(
          "%s = half(%s.x * %s.x - %s.y * %s.z);", edgeAlpha.c_str(), v.fsIn(), v.fsIn(), v.fsIn(),
          v.fsIn());
      fragBuilder->codeAppendf("%s = half(%s < 0.0);", edgeAlpha.c_str(), edgeAlpha.c_str());
      break;
    }
    default: SK_ABORT("Shouldn't get here");
  }
  ...
  switch (fEdgeType) {
    case GrClipEdgeType::kHairlineAA: {
      ...
      fragBuilder->codeAppendf(
          ...
          v.fsIn(), v.fsIn());
      fragBuilder->codeAppendf(
          "edgeAlpha = half(%s.x * %s.x - %s.y);", v.fsIn(), v.fsIn(), v.fsIn());
