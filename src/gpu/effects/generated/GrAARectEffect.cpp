...
@if (%d == 2 || %d == 3) {
    alpha = 1.0 - alpha;
        ...
        prevRect.left(), prevRect.top(), prevRect.right(), prevRect.bottom(), (int)_outer.edgeType,
...
SkString GrAARectEffect::onDumpInfo() const {
  return SkStringPrintf(
      "(edgeType=%d, rect=float4(%f, %f, %f, %f))", (int)edgeType, rect.left(), rect.top(),
...
std::unique_ptr<GrFragmentProcessor> GrAARectEffect::TestCreate(GrProcessorTestData* d) {
  SkRect rect = SkRect::MakeLTRB(
      d->fRandom->nextSScalar1(), d->fRandom->nextSScalar1(), d->fRandom->nextSScalar1(),
