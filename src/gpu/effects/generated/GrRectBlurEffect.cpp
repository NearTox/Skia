...
SkString GrRectBlurEffect::onDumpInfo() const {
  return SkStringPrintf(
      "(rect=float4(%f, %f, %f, %f), isFast=%s)", rect.left(), rect.top(), rect.right(),
      rect.bottom(), (isFast ? "true" : "false"));
}
#endif
...
std::unique_ptr<GrFragmentProcessor> GrRectBlurEffect::TestCreate(GrProcessorTestData* data) {
  ...
  return GrRectBlurEffect::Make(
      data->inputFP(), data->context(), *data->caps()->shaderCaps(), SkRect::MakeWH(width, height),
      sigma);
}
#endif
