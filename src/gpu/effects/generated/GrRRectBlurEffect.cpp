...
static bool fillin_view_on_gpu(
    GrDirectContext* dContext, const GrSurfaceProxyView& lazyView, sk_sp<Trampoline> trampoline,
    const SkRRect& rrectToDraw, const SkISize& dimensions, float xformedSigma) {
  ...
  auto rtc2 = SkGpuBlurUtils::GaussianBlur(
      dContext, std::move(srcView), rtc->colorInfo().colorType(), rtc->colorInfo().alphaType(),
...
// Should've been caught up-stream
  ...
  return std::unique_ptr<GrFragmentProcessor>(new GrRRectBlurEffect(
      std::move(inputFP), xformedSigma, devRRect.getBounds(),
...
SkString GrRRectBlurEffect::onDumpInfo() const {
  return SkStringPrintf(
      "(sigma=%f, rect=float4(%f, %f, %f, %f), cornerRadius=%f)", sigma, rect.left(), rect.top(),
