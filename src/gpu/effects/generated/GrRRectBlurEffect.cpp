...
std::unique_ptr<GrFragmentProcessor> GrRRectBlurEffect::Make(
  ...
  SkASSERT(
      !SkRRectPriv::IsCircle(devRRect) && !devRRect.isRect());  // Should've been caught up-stream
  ...
  bool ninePatchable = SkComputeBlurredRRectParams(
      srcRRect, devRRect, SkRect::MakeEmpty(), sigma, xformedSigma, &rrectToDraw, &size, ignored,
      ignored, ignored, ignored, &ignoredSize, &ignoredSize, &ignored32);
  if (!ninePatchable) {
    return nullptr;
  }
...
class GrGLSLRRectBlurEffect : public GrGLSLFragmentProcessor {
 public:
  ...
  void emitCode(EmitArgs& args) override {
    ...
    (void)cornerRadius;
    cornerRadiusVar =
        args.fUniformHandler->addUniform(kFragment_GrShaderFlag, kHalf_GrSLType, "cornerRadius");
    proxyRectVar =
        args.fUniformHandler->addUniform(kFragment_GrShaderFlag, kFloat4_GrSLType, "proxyRect");
    blurRadiusVar =
        args.fUniformHandler->addUniform(kFragment_GrShaderFlag, kHalf_GrSLType, "blurRadius");
