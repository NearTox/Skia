...
static std::unique_ptr<GrFragmentProcessor> create_profile_effect(
    GrRecordingContext* context, const SkRect& circle, float sigma, float* solidRadius,
    float* textureRadius) {
...
std::unique_ptr<GrFragmentProcessor> GrCircleBlurFragmentProcessor::Make(
    std::unique_ptr<GrFragmentProcessor> inputFP, GrRecordingContext* context, const SkRect& circle,
    float sigma) {
  float solidRadius;
  float textureRadius;
...
class GrGLSLCircleBlurFragmentProcessor : public GrGLSLFragmentProcessor {
 public:
  GrGLSLCircleBlurFragmentProcessor() noexcept = default;
...
%s = inputColor * %s.w;
 ...
 private:
  void onSetData(const GrGLSLProgramDataManager& data, const GrFragmentProcessor& _proc) override {
    ...
    data.set4f(
        circleData, circleRect.centerX(), circleRect.centerY(), solidRadius, 1.f / textureRadius);
  }
...
SkString GrCircleBlurFragmentProcessor::onDumpInfo() const {
  return SkStringPrintf(
      "(circleRect=half4(%f, %f, %f, %f), solidRadius=%f, textureRadius=%f)", circleRect.left(),
      circleRect.top(), circleRect.right(), circleRect.bottom(), solidRadius, textureRadius);
}
#endif
