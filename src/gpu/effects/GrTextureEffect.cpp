...
static void get_matrix(
    ...
    bool* outLazyProxyNormalization) {
...
GrTextureEffect::ShaderMode GrTextureEffect::GetShaderMode(
    GrSamplerState::WrapMode mode, GrSamplerState::Filter filter) noexcept {
  switch (mode) {
    case GrSamplerState::WrapMode::kMirrorRepeat: return ShaderMode::kMirrorRepeat;
    case GrSamplerState::WrapMode::kClamp: return ShaderMode::kClamp;
    case GrSamplerState::WrapMode::kRepeat:
      switch (filter) {
        case GrSamplerState::Filter::kNearest: return ShaderMode::kRepeatNearest;
        case GrSamplerState::Filter::kBilerp: return ShaderMode::kRepeatBilerp;
        case GrSamplerState::Filter::kMipMap: return ShaderMode::kRepeatMipMap;
      }
      SkUNREACHABLE;
   ...
   public:
    void emitCode(EmitArgs& args) override {
      ...
      if (te.sampleMatrix().fKind == SkSL::SampleMatrix::Kind::kMixed) {
        ...
        fb->codeAppendf(
            "float3 %s_3d = %s * _matrix * %s.xy1;\n", coords2D.c_str(),
            te.sampleMatrix().fExpression.c_str(), coords.c_str());
        fb->codeAppendf(
            "float2 %s = %s_3d.xy / %s_3d.z;\n", coords2D.c_str(), coords2D.c_str(),
            coords2D.c_str());
        coords = coords2D;
      }
      if (te.fShaderModes[0] == ShaderMode::kNone && te.fShaderModes[1] == ShaderMode::kNone) {
        ...
        const char* norm = nullptr;
        if (normCoords && (modeRequiresUnormCoords(m[0]) || modeRequiresUnormCoords(m[1]))) {
...
void GrTextureEffect::onGetGLSLProcessorKey(
