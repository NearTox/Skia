...
GrTextureEffect::Sampling::Sampling(GrSamplerState sampler, SkISize size, const GrCaps& caps)
  ...
  if (!caps.npotTextureTileSupport()) {
    ...
    if (fHWSampler.wrapModeY() != GrSamplerState::WrapMode::kClamp && !SkIsPow2(size.height())) {
...
GrTextureEffect::Sampling::Sampling(
    const GrSurfaceProxy& proxy, GrSamplerState sampler, const SkRect& subset, bool adjustForFilter,
    const SkRect* domain, const GrCaps& caps) {
  ...
  auto resolve = [adjustForFilter, filter = sampler.filter(), &caps](
                     int size, Mode mode, Span subset, Span domain) {
    float inset;
    Result1D r;
    r.fFilter = filter;
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::Make(
    sk_sp<GrSurfaceProxy> proxy, SkAlphaType alphaType, const SkMatrix& matrix,
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::Make(
    sk_sp<GrSurfaceProxy> proxy, SkAlphaType alphaType, const SkMatrix& matrix,
    GrSamplerState sampler, const GrCaps& caps) {
  Sampling sampling(sampler, proxy->dimensions(), caps);
  return std::unique_ptr<GrFragmentProcessor>(
      new GrTextureEffect(std::move(proxy), alphaType, matrix, sampling));
}

std::unique_ptr<GrFragmentProcessor> GrTextureEffect::MakeTexelSubset(
    sk_sp<GrSurfaceProxy> proxy, SkAlphaType alphaType, const SkMatrix& matrix,
    GrSamplerState sampler, const SkIRect& subset, const GrCaps& caps) {
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::MakeTexelSubset(
    sk_sp<GrSurfaceProxy> proxy, SkAlphaType alphaType, const SkMatrix& matrix,
    GrSamplerState sampler, const SkIRect& subset, const SkRect& domain, const GrCaps& caps) {
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::MakeSubset(
    sk_sp<GrSurfaceProxy> proxy, SkAlphaType alphaType, const SkMatrix& matrix,
    GrSamplerState sampler, const SkRect& subset, const GrCaps& caps) {
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::MakeSubset(
    sk_sp<GrSurfaceProxy> proxy, SkAlphaType alphaType, const SkMatrix& matrix,
    GrSamplerState sampler, const SkRect& subset, const SkRect& domain, const GrCaps& caps) {
   ...
   public:
    void emitCode(EmitArgs& args) override {
      auto appendWrap = [](GrGLSLShaderBuilder* builder, ShaderMode mode, const char* inCoord,
                           ...
                           const char* out) {
        switch (mode) {
          case ShaderMode::kNone: builder->codeAppendf("%s = %s;\n", out, inCoord); break;
          ...
          case ShaderMode::kRepeat:
            builder->codeAppendf(
                "%s = mod(%s - %s, %s - %s) + %s;", out, inCoord, domainStart, domainEnd,
      ...
      if (te.fShaderModes[0] == ShaderMode::kNone && te.fShaderModes[1] == ShaderMode::kNone) {
        ...
        if (te.fShaderModes[0] == te.fShaderModes[1]) {
          ...
          appendWrap(
              fb, te.fShaderModes[0], inCoords.c_str(), start.c_str(), end.c_str(), true,
              "clampedCoord");
        } else {
          ...
          appendWrap(
              fb, te.fShaderModes[0], origX.c_str(), start.c_str(), end.c_str(), false,
              "clampedCoord.x");
          ...
          appendWrap(
              fb, te.fShaderModes[1], origY.c_str(), start.c_str(), end.c_str(), false,
              "clampedCoord.y");
        }
        ...
        if (decalX || decalY) {
          ...
          if (decalX && decalY) {
            ...
            fb->codeAppendf(
                "half err = half(abs(clampedCoord.x - %s.x) * %s.x);", inCoords.c_str(), decalName);
          } else {
            SkASSERT(decalY);
            fb->codeAppendf(
                "half err = half(abs(clampedCoord.y - %s.y) * %s.y);", inCoords.c_str(), decalName);
          }
          ...
          fb->codeAppendf(
              "if (err > %s.z) { err = 1.0; } else if (%s.z < 1) { err = 0.0; }", decalName,
              decalName);
   ...
   protected:
    void onSetData(const GrGLSLProgramDataManager& pdm, const GrFragmentProcessor& fp) override {
      ...
      if (fSubsetUni.isValid()) {
        ...
        if (te.fSampler.peekTexture()->texturePriv().textureType() != GrTextureType::kRectangle) {
          float iw = 1.f / w;
          float ih = 1.f / h;
          rect[0] *= iw;
          rect[2] *= iw;
        ...
        if (fDecalUni.isValid()) {
          bool filter =
              te.textureSampler(0).samplerState().filter() != GrSamplerState::Filter::kNearest;
...
void GrTextureEffect::onGetGLSLProcessorKey(const GrShaderCaps&, GrProcessorKeyBuilder* b) const {
  bool shaderFilter =
      (fShaderModes[0] == ShaderMode::kDecal || fShaderModes[1] == ShaderMode::kDecal) &&
...
GrTextureEffect::GrTextureEffect(
    sk_sp<GrSurfaceProxy> texture, SkAlphaType alphaType, const SkMatrix& matrix,
    const Sampling& sampling)
    : GrFragmentProcessor(
          kGrTextureEffect_ClassID, ModulateForSamplerOptFlags(alphaType, sampling.usesDecal())),
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::TestCreate(GrProcessorTestData* testData) {
  ...
  GrSamplerState params(
      wrapModes, testData->fRandom->nextBool() ? GrSamplerState::Filter::kBilerp
