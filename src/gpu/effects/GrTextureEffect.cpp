...
GrTextureEffect::Sampling::Sampling(
    ...
    const float border[4], const GrCaps& caps, SkVector linearFilterInset) {
  struct Span {
    float fA = 0.f, fB = 0.f;
  ...
  auto resolve = [&](int size, Wrap wrap, Span subset, Span domain, float linearFilterInset) {
    ...
    if (wrap == Wrap::kClampToBorder &&
      ...
      canDoModeInHW = false;
        ...
        type != GrTextureType::k2D && !(wrap == Wrap::kClamp || wrap == Wrap::kClampToBorder)) {
      canDoModeInHW = false;
    }
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::Make(
  ...
  return GrMatrixEffect::Make(
      final, std::unique_ptr<GrFragmentProcessor>(new GrTextureEffect(
                 std::move(view), alphaType, Sampling(filter, mm), lazyProxyNormalization)));
}
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::MakeCustomLinearFilterInset(
    GrSurfaceProxyView view, SkAlphaType alphaType, const SkMatrix& matrix, Wrap wx, Wrap wy,
    const SkRect& subset, const SkRect* domain, SkVector inset, const GrCaps& caps,
...
GrTextureEffect::ShaderMode GrTextureEffect::GetShaderMode(
    Wrap wrap, Filter filter, MipmapMode mm) {
  switch (wrap) {
    case Wrap::kMirrorRepeat: return ShaderMode::kMirrorRepeat;
    case Wrap::kClamp: return ShaderMode::kClamp;
    case Wrap::kRepeat:
      switch (mm) {
...
void GrTextureEffect::Impl::emitCode(EmitArgs& args) {
  ...
  if (te.fShaderModes[0] == ShaderMode::kNone && te.fShaderModes[1] == ShaderMode::kNone) {
    ...
    bool mipmapRepeatX =
        m[0] == ShaderMode::kRepeat_Nearest_Mipmap || m[0] == ShaderMode::kRepeat_Linear_Mipmap;
    bool mipmapRepeatY =
        m[1] == ShaderMode::kRepeat_Nearest_Mipmap || m[1] == ShaderMode::kRepeat_Linear_Mipmap;
    ...
    if (mipmapRepeatX && mipmapRepeatY) {
      fb->codeAppendf(
          "extraRepeatCoord = clamp(extraRepeatCoord, %s.xy, %s.zw);", clampName, clampName);
    } else if (mipmapRepeatX) {
      fb->codeAppendf(
          "extraRepeatCoord.x = clamp(extraRepeatCoord.x, %s.x, %s.z);", clampName, clampName);
    } else if (mipmapRepeatY) {
      fb->codeAppendf(
          "extraRepeatCoord.y = clamp(extraRepeatCoord.y, %s.y, %s.w);", clampName, clampName);
    }
    ...
    if (mipmapRepeatX && mipmapRepeatY) {
      fb->codeAppendf(
          ...
          "       repeatCoordWeightY);",
          read("clampedCoord").c_str(), read("float2(extraRepeatCoord.x, clampedCoord.y)").c_str(),
    ...
    bool repeatLinearFilterX =
        m[0] == ShaderMode::kRepeat_Linear_None || m[0] == ShaderMode::kRepeat_Linear_Mipmap;
    bool repeatLinearFilterY =
        m[1] == ShaderMode::kRepeat_Linear_None || m[1] == ShaderMode::kRepeat_Linear_Mipmap;
    ...
    if (repeatLinearFilterX && repeatLinearFilterY) {
      ...
      fb->codeAppendf(
          ...
          "                       abs(errY));"
          "}",
          repeatLinearReadX.c_str(), repeatLinearReadY.c_str(), repeatLinearReadXY.c_str());
      ifStr = "else if";
    }
    if (repeatLinearFilterX) {
      fb->codeAppendf(
...
void GrTextureEffect::Impl::onSetData(
    const GrGLSLProgramDataManager& pdm, const GrFragmentProcessor& fp) {
