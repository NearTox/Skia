...
struct GrTextureEffect::Sampling {
  ...
  Sampling(GrSamplerState::Filter filter) noexcept : fHWSampler(filter) {}
  Sampling(
      const GrSurfaceProxy& proxy, GrSamplerState sampler, const SkRect&, const SkRect*,
      const float border[4], const GrCaps&);
  inline bool hasBorderAlpha() const noexcept;
};

GrTextureEffect::Sampling::Sampling(
    const GrSurfaceProxy& proxy, GrSamplerState sampler, const SkRect& subset, const SkRect* domain,
    const float border[4], const GrCaps& caps) {
  struct Span {
    float fA = 0.f, fB = 0.f;

    Span makeInset(float o) const noexcept {
      Span r = {fA + o, fB - o};
      if (r.fA > r.fB) {
    ...
    bool contains(Span r) const noexcept { return fA <= r.fA && fB >= r.fB; }
  };
  struct Result1D {
  ...
  auto resolve = [type, &caps, filter, &border](int size, Mode mode, Span subset, Span domain) {
    ...
    if (mode == Mode::kClampToBorder &&
      ...
      canDoModeInHW = false;
        ...
        type != GrTextureType::k2D && !(mode == Mode::kClamp || mode == Mode::kClampToBorder)) {
      canDoModeInHW = false;
    }
  ...
  fShaderSubset = {x.fShaderSubset.fA, y.fShaderSubset.fA, x.fShaderSubset.fB, y.fShaderSubset.fB};
  fShaderClamp = {x.fShaderClamp.fA, y.fShaderClamp.fA, x.fShaderClamp.fB, y.fShaderClamp.fB};
  std::copy_n(border, 4, fBorder);
}

bool GrTextureEffect::Sampling::hasBorderAlpha() const noexcept {
...
static void get_matrix(
    const SkMatrix& preMatrix, const GrSurfaceProxyView& view, SkMatrix* outMatrix,
    bool* outLazyProxyNormalization) noexcept {
  ...
  if (normalize) {
    if (view.proxy()->isFullyLazy()) {
      *outLazyProxyNormalization = true;
      ...
      SkMatrixPriv::PostIDiv(
          &combined, view.proxy()->backingStoreDimensions().fWidth,
  ...
  if (view.origin() == kBottomLeft_GrSurfaceOrigin) {
    if (normalize) {
      ...
      combined.set(SkMatrix::kMScaleY, combined[SkMatrix::kMPersp1] - combined[SkMatrix::kMScaleY]);
      combined.set(SkMatrix::kMTransY, combined[SkMatrix::kMPersp2] - combined[SkMatrix::kMTransY]);
    } else {
      // combined.postScale(1, -1);
      // combined.postTranslate(0,1);
      SkScalar h = view.proxy()->backingStoreDimensions().fHeight;
      combined.set(
          SkMatrix::kMSkewY, h * combined[SkMatrix::kMPersp0] - combined[SkMatrix::kMSkewY]);
      combined.set(
          SkMatrix::kMScaleY, h * combined[SkMatrix::kMPersp1] - combined[SkMatrix::kMScaleY]);
      combined.set(
          SkMatrix::kMTransY, h * combined[SkMatrix::kMPersp2] - combined[SkMatrix::kMTransY]);
    }
  }
  *outMatrix = combined;
}

std::unique_ptr<GrFragmentProcessor> GrTextureEffect::Make(
    GrSurfaceProxyView view, SkAlphaType alphaType, const SkMatrix& matrix, Filter filter) {
  SkMatrix final;
  bool lazyProxyNormalization;
  get_matrix(matrix, view, &final, &lazyProxyNormalization);
  return GrMatrixEffect::Apply(
      final, std::unique_ptr<GrFragmentProcessor>(new GrTextureEffect(
                 std::move(view), alphaType, Sampling(filter), lazyProxyNormalization)));
}

std::unique_ptr<GrFragmentProcessor> GrTextureEffect::Make(
    GrSurfaceProxyView view, SkAlphaType alphaType, const SkMatrix& matrix, GrSamplerState sampler,
    const GrCaps& caps, const float border[4]) {
  Sampling sampling(
      *view.proxy(), sampler, SkRect::Make(view.proxy()->dimensions()), nullptr, border, caps);
  SkMatrix final;
  bool lazyProxyNormalization;
  get_matrix(matrix, view, &final, &lazyProxyNormalization);
  return GrMatrixEffect::Apply(
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::MakeSubset(
    GrSurfaceProxyView view, SkAlphaType alphaType, const SkMatrix& matrix, GrSamplerState sampler,
    const SkRect& subset, const GrCaps& caps, const float border[4]) {
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::MakeSubset(
    GrSurfaceProxyView view, SkAlphaType alphaType, const SkMatrix& matrix, GrSamplerState sampler,
    const SkRect& subset, const SkRect& domain, const GrCaps& caps, const float border[4]) {
...
GrTextureEffect::FilterLogic GrTextureEffect::GetFilterLogic(
    ShaderMode mode, GrSamplerState::Filter filter) {
  switch (mode) {
    case ShaderMode::kMirrorRepeat:
    case ShaderMode::kNone:
    case ShaderMode::kClamp: return FilterLogic::kNone;
    case ShaderMode::kRepeat:
      switch (filter) {
        case GrSamplerState::Filter::kNearest: return FilterLogic::kNone;
        case GrSamplerState::Filter::kBilerp: return FilterLogic::kRepeatBilerp;
        case GrSamplerState::Filter::kMipMap: return FilterLogic::kRepeatMipMap;
      }
      SkUNREACHABLE;
...
GrGLSLFragmentProcessor* GrTextureEffect::onCreateGLSLInstance() const {
  class Impl : public GrGLSLFragmentProcessor {
   ...
   public:
    void emitCode(EmitArgs& args) override {
      ...
      if (te.sampleMatrix().fKind == SkSL::SampleMatrix::Kind::kMixed) {
        ...
        coords = SkStringPrintf(
            "(%s * _matrix * float3(%s, 1)).xy", te.sampleMatrix().fExpression.c_str(),
            coords.c_str());
      }
      if (te.fShaderModes[0] == ShaderMode::kNone && te.fShaderModes[1] == ShaderMode::kNone) {
        ...
        if (te.fLazyProxyNormalization) {
          ...
          fNormUni = args.fUniformHandler->addUniform(
              &te, kFragment_GrShaderFlag, kFloat4_GrSLType, "norm", &norm);
          fb->appendTextureLookupAndBlend(
              args.fInputColor, SkBlendMode::kModulate, args.fTexSamplers[0],
              SkStringPrintf("%s * %s.zw", coords.c_str(), norm).c_str());
        } else {
          fb->appendTextureLookupAndBlend(
              args.fInputColor, SkBlendMode::kModulate, args.fTexSamplers[0], coords.c_str());
        }
        fb->codeAppendf(";");
      } else {
        ...
        fb->codeAppendf(
            "float2 inCoord = %s;",
            fb->ensureCoords2D(args.fTransformedCoords[0].fVaryingPoint, te.sampleMatrix())
                .c_str());
        ...
        FilterLogic filterLogic[2] = {GetFilterLogic(m[0], filter), GetFilterLogic(m[1], filter)};
        ...
        const char* norm = nullptr;
        if (normCoords &&
            (filterLogic[0] != FilterLogic::kNone || filterLogic[1] != FilterLogic::kNone)) {
        ...
        auto subsetCoord = [&](ShaderMode mode, const char* coordSwizzle,
          ...
          switch (mode) {
            ...
            case ShaderMode::kRepeat:
              if (filter == Filter::kMipMap) {
                ...
                fb->codeAppend("{");
                fb->codeAppendf(
                    "float w = %s.%s - %s.%s;", subsetName, subsetStopSwizzle, subsetName,
                    subsetStartSwizzle);
                fb->codeAppendf("float w2 = 2 * w;");
                fb->codeAppendf(
                    "float d = inCoord.%s - %s.%s;", coordSwizzle, subsetName, subsetStartSwizzle);
                fb->codeAppend("float m = mod(d, w2);");
                fb->codeAppend("float o = mix(m, w2 - m, step(w, m));");
                fb->codeAppendf(
                    "subsetCoord.%s = o + %s.%s;", coordSwizzle, subsetName, subsetStartSwizzle);
            ...
            case ShaderMode::kMirrorRepeat: {
              ...
              fb->codeAppendf("float w2 = 2 * w;");
              fb->codeAppendf(
                  "float m = mod(inCoord.%s - %s.%s, w2);", coordSwizzle, subsetName,
                  subsetStartSwizzle);
              fb->codeAppendf(
                  "subsetCoord.%s = mix(m, w2 - m, step(w, m)) + %s.%s;", coordSwizzle, subsetName,
                  subsetStartSwizzle);
              fb->codeAppend("}");
              break;
            }
        ...
        auto clampCoord = [&](bool clamp, const char* coordSwizzle, const char* clampStartSwizzle,
                              const char* clampStopSwizzle) {
          if (clamp) {
            fb->codeAppendf(
                "clampedCoord.%s = clamp(subsetCoord.%s, %s.%s, %s.%s);", coordSwizzle,
        ...
        if (filterLogic[0] == FilterLogic::kRepeatMipMap) {
          fb->codeAppendf(
              "extraRepeatCoordX = clamp(extraRepeatCoordX, %s.x, %s.z);", clampName, clampName);
        }
        if (filterLogic[1] == FilterLogic::kRepeatMipMap) {
          fb->codeAppendf(
              "extraRepeatCoordY = clamp(extraRepeatCoordY, %s.y, %s.w);", clampName, clampName);
        }
        ...
        if (filterLogic[0] == FilterLogic::kRepeatMipMap &&
          ...
          fb->codeAppendf(
              "half4 textureColor = mix(%s, %s, repeatCoordWeightX);", read("clampedCoord").c_str(),
              read("float2(extraRepeatCoordX, clampedCoord.y)").c_str());
        } else if (filterLogic[1] == FilterLogic::kRepeatMipMap) {
          fb->codeAppendf(
              "half4 textureColor = mix(%s, %s, repeatCoordWeightY);", read("clampedCoord").c_str(),
              read("float2(clampedCoord.x, extraRepeatCoordY)").c_str());
        } else {
        ...
        if (filterLogic[0] == FilterLogic::kRepeatBilerp &&
          ...
          fb->codeAppendf(
              ...
              "                       abs(errY));"
              "}",
              repeatBilerpReadX.c_str(), repeatBilerpReadY.c_str(), repeatBilerpReadXY.c_str());
          ifStr = "else if";
        }
        if (filterLogic[0] == FilterLogic::kRepeatBilerp) {
          fb->codeAppendf(
   ...
   protected:
    void onSetData(const GrGLSLProgramDataManager& pdm, const GrFragmentProcessor& fp) override {
...
GrTextureEffect::GrTextureEffect(
    GrSurfaceProxyView view, SkAlphaType alphaType, const Sampling& sampling,
    bool lazyProxyNormalization)
    : GrFragmentProcessor(
...
const GrFragmentProcessor::TextureSampler& GrTextureEffect::onTextureSampler(int) const noexcept {
  return fSampler;
}
...
std::unique_ptr<GrFragmentProcessor> GrTextureEffect::TestCreate(GrProcessorTestData* testData) {
  ...
  if (view.asTextureProxy()->mipMapped() == GrMipMapped::kYes) {
    switch (testData->fRandom->nextULessThan(3)) {
      case 0: filter = Filter::kNearest; break;
      case 1: filter = Filter::kBilerp; break;
      default: filter = Filter::kMipMap; break;
    }
  } else {
