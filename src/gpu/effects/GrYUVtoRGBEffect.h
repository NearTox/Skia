...
class GrYUVtoRGBEffect : public GrFragmentProcessor {
 public:
  ...
  static std::unique_ptr<GrFragmentProcessor> Make(
      const sk_sp<GrTextureProxy> proxies[], const SkYUVAIndex indices[4],
      SkYUVColorSpace yuvColorSpace, GrSamplerState::Filter filterMode,
      const SkMatrix& localMatrix = SkMatrix::I(), const SkRect* domain = nullptr);
#ifdef SK_DEBUG
 ...
 private:
  GrYUVtoRGBEffect(
      const sk_sp<GrTextureProxy> proxies[], const SkSize scales[],
    ...
    for (int i = 0; i < numPlanes; ++i) {
      ...
      if (domain) {
        ...
        fDomains[i] = GrTextureDomain(
            proxies[i].get(), scaledDomain, GrTextureDomain::kClamp_Mode,
            GrTextureDomain::kClamp_Mode, i);
      }

      planeMatrix.preConcat(localMatrix);
      fSamplers[i].reset(
          std::move(proxies[i]), GrSamplerState(GrSamplerState::WrapMode::kClamp, filterModes[i]));
