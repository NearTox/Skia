...
static void append_wrap(
    GrGLSLShaderBuilder* builder, GrTextureDomain::Mode mode, const char* inCoord,
    const char* domainStart, const char* domainEnd, bool is2D, const char* out) {
  switch (mode) {
    case GrTextureDomain::kIgnore_Mode: builder->codeAppendf("%s = %s;\n", out, inCoord); break;
    ...
    case GrTextureDomain::kRepeat_Mode:
      builder->codeAppendf(
          "%s = mod(%s - %s, %s - %s) + %s;", out, inCoord, domainStart, domainEnd, domainStart,
          domainStart);
      break;
...
void GrTextureDomain::GLDomain::sampleTexture(
  ...
  auto appendTextureSample = [&sampler, inModulateColor, builder](const char* coord) {
    ...
    builder->appendTextureLookupAndBlend(inModulateColor, SkBlendMode::kModulate, sampler, coord);
    builder->codeAppend(";");
...
void GrTextureDomain::GLDomain::sample(
  ...
  if (is2D) {
    ...
    append_wrap(
        builder, textureDomain.modeX(), "origCoord", start.c_str(), end.c_str(), true,
        "clampedCoord");
  } else {
    ...
    append_wrap(
        builder, textureDomain.modeX(), "origCoord.x", start.c_str(), end.c_str(), false,
        "clampedCoord.x");
    // Repeat the same logic for y.
    start.printf("%s.y", fDomainName.c_str());
    end.printf("%s.w", fDomainName.c_str());
    append_wrap(
        builder, textureDomain.modeY(), "origCoord.y", start.c_str(), end.c_str(), false,
        "clampedCoord.y");
  }
...
void GrTextureDomain::GLDomain::setData(
    ...
    const GrSurfaceProxyView& view, GrSamplerState state) {
...
void GrTextureDomain::GLDomain::setData(
    const GrGLSLProgramDataManager& pdman, const GrTextureDomain& textureDomain,
    const GrSurfaceProxy* proxy, GrSurfaceOrigin origin, bool filterIfDecal) {
  SkASSERT(fHasMode && textureDomain.modeX() == fModeX && textureDomain.modeY() == fModeY);
  ...
  class GLSLProcessor : public GrGLSLFragmentProcessor {
   ...
   protected:
    void onSetData(const GrGLSLProgramDataManager& pdman, const GrFragmentProcessor& fp) override {
      ...
      fGLDomain.setData(
          pdman, dstdfp.fTextureDomain, view, dstdfp.textureSampler(0).samplerState());
