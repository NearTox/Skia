...
GrTextureDomain::GrTextureDomain(
    GrSurfaceProxy* proxy, const SkRect& domain, Mode modeX, Mode modeY, int index)
...
static SkString clamp_expression(
    GrTextureDomain::Mode mode, const char* inCoord, const char* coordSwizzle, const char* domain,
    const char* minSwizzle, const char* maxSwizzle) {
  SkString clampedExpr;
  switch (mode) {
    case GrTextureDomain::kIgnore_Mode: clampedExpr.printf("%s.%s\n", inCoord, coordSwizzle); break;
    ...
    case GrTextureDomain::kClamp_Mode:
      clampedExpr.printf(
          "clamp(%s.%s, %s.%s, %s.%s)", inCoord, coordSwizzle, domain, minSwizzle, domain,
          maxSwizzle);
      break;
    case GrTextureDomain::kRepeat_Mode:
      clampedExpr.printf(
          "mod(%s.%s - %s.%s, %s.%s - %s.%s) + %s.%s", inCoord, coordSwizzle, domain, minSwizzle,
          domain, maxSwizzle, domain, minSwizzle, domain, minSwizzle);
      break;
    default: SkASSERTF(false, "Unknown texture domain mode: %u\n", (uint32_t)mode); break;
  }
  return clampedExpr;
}

void GrTextureDomain::GLDomain::sampleProcessor(
    const GrTextureDomain& textureDomain, const char* inColor, const char* outColor,
    const SkString& inCoords, GrGLSLFragmentProcessor* parent,
    GrGLSLFragmentProcessor::EmitArgs& args, int childIndex) {
  ...
  this->sample(
      args.fFragBuilder, args.fUniformHandler, textureDomain, outColor, inCoords,
      appendProcessorSample);
}

void GrTextureDomain::GLDomain::sampleTexture(
    GrGLSLShaderBuilder* builder, GrGLSLUniformHandler* uniformHandler,
    const GrShaderCaps* shaderCaps, const GrTextureDomain& textureDomain, const char* outColor,
...
void GrTextureDomain::GLDomain::sample(
    GrGLSLShaderBuilder* builder, GrGLSLUniformHandler* uniformHandler,
    const GrTextureDomain& textureDomain, const char* outColor, const SkString& inCoords,
  ...
  SkDEBUGCODE(fModeX = textureDomain.modeX();) SkDEBUGCODE(fModeY = textureDomain.modeY();)
    ...
    fDomainUni =
        uniformHandler->addUniform(kFragment_GrShaderFlag, kHalf4_GrSLType, uniName.c_str(), &name);
    fDomainName = name;
  }
  ...
  if ((decalX || decalY) && !fDecalUni.isValid()) {
    ...
    fDecalUni =
        uniformHandler->addUniform(kFragment_GrShaderFlag, kHalf3_GrSLType, uniName.c_str(), &name);
    fDecalName = name;
  }
  ...
  if (textureDomain.modeX() != textureDomain.modeY()) {
    ...
    SkString tcX =
        clamp_expression(textureDomain.modeX(), "origCoord", "x", fDomainName.c_str(), "x", "z");
    SkString tcY =
        clamp_expression(textureDomain.modeY(), "origCoord", "y", fDomainName.c_str(), "y", "w");
    ...
    SkString tc =
        clamp_expression(textureDomain.modeX(), "origCoord", "xy", fDomainName.c_str(), "xy", "zw");
  ...
  if (decalX || decalY) {
    ...
    if (decalX && decalY) {
      ...
      builder->codeAppendf(
          "half err = half(abs(clampedCoord.x - origCoord.x) * %s.x);", fDecalName.c_str());
    } else {
      SkASSERT(decalY);
      builder->codeAppendf(
          "half err = half(abs(clampedCoord.y - origCoord.y) * %s.y);", fDecalName.c_str());
    }
    ...
    builder->codeAppendf(
        "if (err > %s.z) { err = 1.0; } else if (%s.z < 1) { err = 0.0; }", fDecalName.c_str(),
        fDecalName.c_str());
...
void GrTextureDomain::GLDomain::setData(
    const GrGLSLProgramDataManager& pdman, const GrTextureDomain& textureDomain,
    const GrSurfaceProxy* proxy, const GrSamplerState& state) {
...
void GrTextureDomain::GLDomain::setData(
    const GrGLSLProgramDataManager& pdman, const GrTextureDomain& textureDomain,
    const GrSurfaceProxy* proxy, bool filterIfDecal) {
  SkASSERT(fHasMode && textureDomain.modeX() == fModeX && textureDomain.modeY() == fModeY);
  ...
  bool sendDecalData = textureDomain.modeX() == kDecal_Mode || textureDomain.modeY() == kDecal_Mode;
...
std::unique_ptr<GrFragmentProcessor> GrDomainEffect::Make(
    std::unique_ptr<GrFragmentProcessor> fp, const SkRect& domain, GrTextureDomain::Mode mode,
    bool decalIsFiltered) {
  return Make(std::move(fp), domain, mode, mode, decalIsFiltered);
}

std::unique_ptr<GrFragmentProcessor> GrDomainEffect::Make(
    std::unique_ptr<GrFragmentProcessor> fp, const SkRect& domain, GrTextureDomain::Mode modeX,
    GrTextureDomain::Mode modeY, bool decalIsFiltered) {
  ...
  *coordTransform = {};
  ...
  return std::unique_ptr<GrFragmentProcessor>(
      new GrDomainEffect(std::move(fp), transformCopy, domain, modeX, modeY, decalIsFiltered));
}
...
std::unique_ptr<GrFragmentProcessor> GrDomainEffect::Make(
    std::unique_ptr<GrFragmentProcessor> fp, const SkRect& domain, GrTextureDomain::Mode modeX,
    GrTextureDomain::Mode modeY, GrSamplerState::Filter filter) {
  bool filterIfDecal = filter != GrSamplerState::Filter::kNearest;
  return Make(std::move(fp), domain, modeX, modeY, filterIfDecal);
}
GrFragmentProcessor::OptimizationFlags GrDomainEffect::Flags(
    GrFragmentProcessor* fp, GrTextureDomain::Mode modeX, GrTextureDomain::Mode modeY) {
...
GrDomainEffect::GrDomainEffect(
    std::unique_ptr<GrFragmentProcessor> fp, const GrCoordTransform& coordTransform,
...
void GrDomainEffect::onGetGLSLProcessorKey(
  ...
  class GLSLProcessor : public GrGLSLFragmentProcessor {
   public:
    void emitCode(EmitArgs& args) override {
      ...
      fGLDomain.sampleProcessor(
          domain, args.fInputColor, args.fOutputColor, coords2D, this, args, 0);
    }
...
std::unique_ptr<GrFragmentProcessor> GrDeviceSpaceTextureDecalFragmentProcessor::Make(
  ...
  return std::unique_ptr<GrFragmentProcessor>(
      new GrDeviceSpaceTextureDecalFragmentProcessor(std::move(proxy), subset, deviceSpaceOffset));
}
...
GrDeviceSpaceTextureDecalFragmentProcessor::GrDeviceSpaceTextureDecalFragmentProcessor(
    ...
    : INHERITED(
      ...
      fTextureDomain(
          proxy.get(), GrTextureDomain::MakeTexelDomain(subset, GrTextureDomain::kDecal_Mode),
  ...
  class GLSLProcessor : public GrGLSLFragmentProcessor {
   public:
    void emitCode(EmitArgs& args) override {
      ...
      fScaleAndTranslateUni = args.fUniformHandler->addUniform(
          kFragment_GrShaderFlag, kHalf4_GrSLType, "scaleAndTranslate", &scaleAndTranslateName);
      args.fFragBuilder->codeAppendf(
          "half2 coords = half2(sk_FragCoord.xy * %s.xy + %s.zw);", scaleAndTranslateName,
          scaleAndTranslateName);
      fGLDomain.sampleTexture(
          args.fFragBuilder, args.fUniformHandler, args.fShaderCaps, dstdfp.fTextureDomain,
          args.fOutputColor, SkString("coords"), args.fTexSamplers[0], args.fInputColor);
    }
...
bool GrDeviceSpaceTextureDecalFragmentProcessor::onIsEqual(const GrFragmentProcessor& fp) const {
  ...
  return dstdfp.fTextureSampler.proxy()->underlyingUniqueID() ==
         ...
         dstdfp.fDeviceSpaceOffset == fDeviceSpaceOffset && dstdfp.fTextureDomain == fTextureDomain;
}
