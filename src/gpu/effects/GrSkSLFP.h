...
class GrSkSLFP : public GrFragmentProcessor {
 public:
  ...
  static std::unique_ptr<GrSkSLFP> MakeWithData(
      ...
      std::unique_ptr<GrFragmentProcessor> destColorFP, sk_sp<SkData> uniforms,
 ...
 private:
  ...
  template <typename... Args>
  void appendArgs(
      uint8_t* uniformDataPtr, UniformFlags* uniformFlagsPtr, const char* name,
  ...
  template <typename T, typename... Args>
  void appendArgs(
      uint8_t* uniformDataPtr, UniformFlags* uniformFlagsPtr, const char* name, const T& val,
      Args&&... remainder) {
  ...
  static void checkArgs(
    ...
    SkASSERTF(
        ...
        name);
    SkASSERTF(
        uIter->sizeInBytes() == sizeof(val),
        "Expected uniform '%s' to be %zu bytes, got %zu instead", name, uIter->sizeInBytes(),
        sizeof(val));
