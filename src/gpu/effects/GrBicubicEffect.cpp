...
std::unique_ptr<GrFragmentProcessor> GrBicubicEffect::Make(
    GrSurfaceProxyView view, SkAlphaType alphaType, const SkMatrix& matrix, Direction direction) {
...
std::unique_ptr<GrFragmentProcessor> GrBicubicEffect::Make(
    ...
    const GrSamplerState::WrapMode wrapX, const GrSamplerState::WrapMode wrapY, Direction direction,
    const GrCaps& caps) {
...
std::unique_ptr<GrFragmentProcessor> GrBicubicEffect::MakeSubset(
    ...
    const SkRect& subset, Direction direction, const GrCaps& caps) {
  GrSamplerState sampler(wrapX, wrapY, GrSamplerState::Filter::kNearest);
  std::unique_ptr<GrFragmentProcessor> fp;
...
GrBicubicEffect::GrBicubicEffect(
    std::unique_ptr<GrFragmentProcessor> fp, Direction direction, Clamp clamp)
...
void GrBicubicEffect::onGetGLSLProcessorKey(
  ...
  uint32_t key =
      (fDirection == GrBicubicEffect::Direction::kXY) | (static_cast<uint32_t>(fClamp) << 1);
  b->add32(key);
}
...
bool GrBicubicEffect::ShouldUseBicubic(const SkMatrix& matrix, GrSamplerState::Filter* filterMode) {
  switch (SkMatrixPriv::AdjustHighQualityFilterLevel(matrix)) {
    case kNone_SkFilterQuality: *filterMode = GrSamplerState::Filter::kNearest; break;
    case kLow_SkFilterQuality: *filterMode = GrSamplerState::Filter::kBilerp; break;
    case kMedium_SkFilterQuality: *filterMode = GrSamplerState::Filter::kMipMap; break;
