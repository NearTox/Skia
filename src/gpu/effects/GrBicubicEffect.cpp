...
void GrGLBicubicEffect::emitCode(EmitArgs& args) {
  ...
  if (bicubicEffect.direction() == GrBicubicEffect::Direction::kXY) {
    ...
    for (int y = 0; y < 4; ++y) {
      for (int x = 0; x < 4; ++x) {
        ...
        fDomain.sampleTexture(
            fragBuilder, args.fUniformHandler, args.fShaderCaps, bicubicEffect.domain(),
            sampleVar.c_str(), coord, args.fTexSamplers[0]);
      }
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      fDomain.sampleTexture(
          fragBuilder, args.fUniformHandler, args.fShaderCaps, bicubicEffect.domain(),
          samplerVar.c_str(), coord, args.fTexSamplers[0]);
    }
    fragBuilder->codeAppend(
...
GrBicubicEffect::GrBicubicEffect(
    ...
    const SkRect& domain, const GrSamplerState::WrapMode wrapModes[2], GrTextureDomain::Mode modeX,
    ...
    : INHERITED{kGrBicubicEffect_ClassID,
                ModulateForSamplerOptFlags(
                    srcColorType, GrTextureDomain::IsDecalSampled(wrapModes, modeX, modeY))},
      fCoordTransform(matrix, proxy.get()),
      fDomain(proxy.get(), domain, modeX, modeY),
      fTextureSampler(
          std::move(proxy), GrSamplerState(wrapModes, GrSamplerState::Filter::kNearest)),
...
std::unique_ptr<GrFragmentProcessor> GrBicubicEffect::TestCreate(GrProcessorTestData* d) {
  ...
  return GrBicubicEffect::Make(
      d->textureProxy(texIdx), d->textureProxyColorType(texIdx), SkMatrix::I(), kClampClamp,
      direction, alphaType);
}
#endif
