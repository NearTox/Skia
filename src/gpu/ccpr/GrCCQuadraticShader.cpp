...
void GrCCQuadraticShader::emitSetupCode(
  ...
  s->codeAppendf(
      "%s = float2x2(1, 1, .5, 0) * inverse(float2x2(%s[2] - %s[0], %s[1] - %s[0]));",
...
void GrCCQuadraticShader::onEmitVaryings(
  ...
  code->appendf(
      "%s.xy = %s * (%s - %s);",  // Quadratic coords.
      OutName(fCoord_fGrad), fQCoordMatrix.c_str(), position, fQCoord0.c_str());
  code->appendf(
      "%s.zw = 2*bloat * float2(2 * %s.x, -1) * %s;",  // Gradient.
  ...
  if (coverage) {
    ...
    varyingHandler->addVarying(
        (cornerCoverage) ? "edge_and_wind_and_corner" : "edge_and_wind", &fEdge_fWind_fCorner);
  ...
  if (cornerCoverage) {
    ...
    code->appendf(
        "%s.zw = half2(hull_coverage, 1) * %s;", OutName(fEdge_fWind_fCorner), cornerCoverage);
  }
}
...
void GrCCQuadraticShader::emitFragmentCoverageCode(
  ...
  this->calcHullCoverage(
      &AccessCodeString(f), fCoord_fGrad.fsIn(),
      SkStringPrintf("%s.x", fEdge_fWind_fCorner.fsIn()).c_str(), outputCoverage);
  f->codeAppendf("%s *= half(%s.y);", outputCoverage, fEdge_fWind_fCorner.fsIn());  // Wind.

  if (kFloat4_GrSLType == fEdge_fWind_fCorner.type()) {
    f->codeAppendf(
        "%s = half(%s.z * %s.w) + %s;",  // Attenuated corner coverage.
        outputCoverage, fEdge_fWind_fCorner.fsIn(), fEdge_fWind_fCorner.fsIn(), outputCoverage);
  }
}

void GrCCQuadraticShader::calcHullCoverage(
    SkString* code, const char* coordAndGrad, const char* edge, const char* outputCoverage) const {
