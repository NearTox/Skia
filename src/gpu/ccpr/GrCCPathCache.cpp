...
inline static bool fuzzy_equals(
  ...
  if (((Sk2f::Load(a.fSubpixelTranslate) - Sk2f::Load(b.fSubpixelTranslate)).abs() > 1.f / 256)
          .anyTrue()) {
    return false;
  }
#endif
...
sk_sp<GrCCPathCache::Key> GrCCPathCache::Key::Make(
    uint32_t pathCacheUniqueID, int dataCountU32, const void* data) {
...
class WriteKeyHelper {
 public:
  ...
  WriteKeyHelper(const GrShape& shape) noexcept : fShapeUnstyledKeyCount(shape.unstyledKeySize()) {}
  ...
  void write(const GrShape& shape, uint32_t* out) noexcept {
    // Stroke key.
...
GrCCPathCache::OnFlushEntryRef GrCCPathCache::find(
    GrOnFlushResourceProvider* onFlushRP, const GrShape& shape, const SkIRect& clippedDrawBounds,
  ...
  if (!entry) {
    ...
    sk_sp<Key> permanentKey = Key::Make(
        fInvalidatedKeysInbox.uniqueID(), writeKeyHelper.allocCountU32(), fScratchKey->data());
...
void GrCCPathCache::purgeEntriesOlderThan(
    GrProxyProvider* proxyProvider, const GrStdSteadyClock::time_point& purgeTime) {
  this->evictInvalidatedCacheKeys();

#ifdef SK_DEBUG
  auto lastTimestamp =
      (fLRU.isEmpty()) ? GrStdSteadyClock::time_point::max() : fLRU.tail()->fTimestamp;
#endif
...
void GrCCPathCache::purgeInvalidatedAtlasTextures(GrProxyProvider* proxyProvider) {
  ...
  for (const GrUniqueKey& key : fInvalidatedProxyUniqueKeys) {
    proxyProvider->processInvalidUniqueKey(
        key, nullptr, GrProxyProvider::InvalidateGPUResource::kYes);
  }
  fInvalidatedProxyUniqueKeys.reset();
}
