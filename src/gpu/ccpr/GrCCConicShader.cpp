...
void GrCCConicShader::emitSetupCode(GrGLSLVertexGeoBuilder* s, const char* pts,
...
void GrCCConicShader::onEmitVaryings(GrGLSLVaryingHandler* varyingHandler,
                                     ...
                                     const char* cornerCoverage, const char* wind) {
    code->appendf("float3 klm = float3(%s - %s, 1) * %s;", position, fControlPoint.c_str(),
                  fKLMMatrix.c_str());
    ...
    if (cornerCoverage) {
        ...
        code->appendf("%s.zw = half2(hull_coverage, 1) * %s;", OutName(fGrad_fCorner),
                      cornerCoverage);
    }
}
