...
void GrCCConicShader::emitSetupCode(
  ...
  s->codeAppendf(
      "%s = float3x3(y2 - y0, x0 - x2, x2*y0 - x0*y2, "
      "2*w * float2(+y0, -x0), 0, "
      "2*w * float2(-y2, +x2), 0);",
      fKLMMatrix.c_str());
  ...
  // on the CPU.
  s->codeAppendf(
      "float kwidth = 2*bloat * (abs(%s[0].x) + abs(%s[0].y)) * sign(%s[0].z);", fKLMMatrix.c_str(),
...
void GrCCConicShader::onEmitVaryings(
  ...
  code->appendf(
      "float3 klm = float3(%s - %s, 1) * %s;", position, fControlPoint.c_str(), fKLMMatrix.c_str());
  if (coverage) {
  ...
  code->appendf(
      "%s.xy = 2*bloat * (float3x2(%s) * float3(2*klm[0], -klm[2], -klm[1]));",
  ...
  if (cornerCoverage) {
    ...
    code->appendf("%s.zw = half2(hull_coverage, 1) * %s;", OutName(fGrad_fCorner), cornerCoverage);
  }
}
...
void GrCCConicShader::emitFragmentCoverageCode(
  ...
  this->calcHullCoverage(
      &AccessCodeString(f), fKLM_fWind.fsIn(), fGrad_fCorner.fsIn(), outputCoverage);
  f->codeAppendf("%s *= half(%s.w);", outputCoverage, fKLM_fWind.fsIn());  // Wind.

  if (kFloat4_GrSLType == fGrad_fCorner.type()) {
    f->codeAppendf(
        "%s = fma(half(%s.z), half(%s.w), %s);",  // Attenuated corner coverage.
        outputCoverage, fGrad_fCorner.fsIn(), fGrad_fCorner.fsIn(), outputCoverage);
  }
}

void GrCCConicShader::calcHullCoverage(
...
void GrCCConicShader::emitSampleMaskCode(GrGLSLFPFragmentBuilder* f) const {
  f->codeAppendf(
      "float k = %s.x, l = %s.y, m = %s.z;", fKLM_fWind.fsIn(), fKLM_fWind.fsIn(),
      fKLM_fWind.fsIn());
