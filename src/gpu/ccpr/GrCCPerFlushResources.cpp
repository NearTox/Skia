...
GrCCPerFlushResources::GrCCPerFlushResources(
  ...
  fPathInstanceBuffer.resetAndMapBuffer(onFlushRP, inst_buffer_count(specs) * sizeof(PathInstance));
  ...
  if (CoverageType::kA8_Multisample == coverageType) {
    ...
    if (!fStencilResolveBuffer.gpuBuffer()) {
      SkDebugf(
          "WARNING: failed to allocate CCPR stencil resolve buffer. "
...
void GrCCPerFlushResources::recordCopyPathInstance(
  ...
  for (int i = fCopyPathRanges.count() - 1; i >= fCurrCopyAtlasRangesIdx; --i) {
    ...
    std::swap(fPathInstanceBuffer[rangeFirstInstanceIdx], fPathInstanceBuffer[currentInstanceIdx]);
