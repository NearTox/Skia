...
void LinearStrokeProcessor::Impl::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
  ...
  v->codeAppendf(
      "%s.xz = float2(-.5, dot(endpts.zw - endpts.xy, tan) / nwidth + .5);", edgeDistances.vsOut());
  v->codeAppendf(
      "%s.xz = (sk_VertexID < 2) ? %s.xz : %s.zx;", edgeDistances.vsOut(), edgeDistances.vsOut(),
      edgeDistances.vsOut());
  ...
  v->codeAppendf(
      "%s.yw = (0 == (sk_VertexID & 1)) ? %s.yw : %s.wy;", edgeDistances.vsOut(),
  ...
  f->codeAppendf(
      "half2 coverages = half2(min(%s.xy, .5) + min(%s.zw, .5));", edgeDistances.fsIn(),
      edgeDistances.fsIn());
...
void CubicStrokeProcessor::Impl::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
  ...
  v->codeAppendf(
      "%s.xy = (0 == (sk_VertexID & 1)) ? %s.xy : %s.yx;", coverages.vsOut(), coverages.vsOut(),
      coverages.vsOut());
...
void GrCCStroker::parseDeviceSpaceStroke(
    const SkPath& path, const SkPoint* deviceSpacePts, const SkStrokeRec& stroke,
    float strokeDevWidth, GrScissorTest scissorTest, const SkIRect& clippedDevIBounds,
  ...
  if (!fHasOpenBatch) {
    fBatches.emplace_back(
        &fTalliesAllocator, *fInstanceCounts[(int)GrScissorTest::kDisabled],
 ...
 public:
  ...
  void appendQuadraticStroke(const SkPoint P[3], int numLinearSegmentsLog2) {
    ...
    this->appendCubicStrokeInstance(numLinearSegmentsLog2)
        .set(ptsT[0], ptsT[1], fCurrDX, fCurrDY, fCurrStrokeRadius, 1 << numLinearSegmentsLog2);
  }
  ...
  void appendJoin(
      Verb joinVerb, const SkPoint& center, const SkVector& leftNorm, const SkVector& rightNorm,
      float miterCapHeightOverWidth, float conicWeight) {
    SkASSERT(this->isMapped());
    ...
    if (!GrCCStrokeGeometry::IsInternalJoinVerb(joinVerb)) {
      ...
      this->appendTriangleInstance().set(
          -n0 * fCurrStrokeRadius, n0 * fCurrStrokeRadius, n1 * fCurrStrokeRadius, offset);
      this->appendTriangleInstance().set(
          -n0 * fCurrStrokeRadius, n1 * fCurrStrokeRadius, -n1 * fCurrStrokeRadius, offset);
    ...
    if (Verb::kMiterJoin == joinVerb) {
      ...
      this->appendConicInstance().setW(
          n0 * fCurrStrokeRadius, c * fCurrStrokeRadius, n1 * fCurrStrokeRadius, offset,
          conicWeight);
      if (Verb::kInternalRoundJoin == joinVerb) {
        this->appendConicInstance().setW(
            -n1 * fCurrStrokeRadius, c * -fCurrStrokeRadius, -n0 * fCurrStrokeRadius, offset,
            conicWeight);
      }
    }
  }
...
bool GrCCStroker::prepareToDraw(GrOnFlushResourceProvider* onFlushRP) {
  ...
  int cubicStrokesIdx = GrSizeDivRoundUp(
      endLinearStrokesIdx * sizeof(LinearStrokeInstance), sizeof(CubicStrokeInstance));
  ...
  int trianglesIdx =
      GrSizeDivRoundUp(cubicStrokesIdx * sizeof(CubicStrokeInstance), sizeof(TriangleInstance));
  ...
  for (Verb verb : fGeometry.verbs()) {
    switch (verb) {
      ...
      case Verb::kQuadraticStroke:
        builder.appendQuadraticStroke(&pts[ptsIdx], params[paramsIdx++].fNumLinearSegmentsLog2);
        ptsIdx += 2;
        ++normalsIdx;
        continue;
      ...
      case Verb::kInternalBevelJoin:
        builder.appendJoin(
            verb, pts[ptsIdx], normals[normalsIdx], normals[normalsIdx + 1],
            miterCapHeightOverWidth, conicWeight);
        ++normalsIdx;
        continue;

      case Verb::kSquareCap:
      case Verb::kRoundCap: builder.appendCap(verb, pts[ptsIdx], normals[normalsIdx]); continue;
...
void GrCCStroker::drawStrokes(
    GrOpFlushState* flushState, GrCCCoverageProcessor* proc, BatchID batchID,
...
void GrCCStroker::appendStrokeMeshesToBuffers(
    int numSegmentsLog2, const Batch& batch, const InstanceTallies* startIndices[2],
  ...
  if (int instanceCount = endIdx - startIdx) {
    ...
    mesh.setInstanced(fInstanceBuffer, instanceCount, baseInstance + startIdx, numStripVertices);
...
void GrCCStroker::flushBufferedMeshesAsStrokes(
    const GrPrimitiveProcessor& processor, GrOpFlushState* flushState, const GrPipeline& pipeline,
    const SkIRect& drawBounds) const {
  ...
  flushState->rtCommandBuffer()->draw(
      processor, pipeline, nullptr, &dynamicStateArrays, fMeshesBuffer.begin(),
...
void GrCCStroker::drawConnectingGeometry(
  ...
  SkASSERT(endIdx >= startIdx);
  if (int instanceCount = endIdx - startIdx) {
    processor.appendMesh(fInstanceBuffer, instanceCount, baseInstance + startIdx, &fMeshesBuffer);
    fScissorsBuffer.push_back(drawBounds);
  }
  ...
  if (!fMeshesBuffer.empty()) {
    SkASSERT(fMeshesBuffer.count() == fScissorsBuffer.count());
    processor.draw(
        flushState, pipeline, fScissorsBuffer.begin(), fMeshesBuffer.begin(), fMeshesBuffer.count(),
