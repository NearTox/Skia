...
void GrCCStroker::drawStrokes(
  ...
  this->drawLog2Strokes(
      0, flushState, LinearStrokeProcessor(), pipeline, batch, startIndices, startScissorSubBatch,
      drawBounds);
  ...
  for (int i = 1; i <= kMaxNumLinearSegmentsLog2; ++i) {
    this->drawLog2Strokes(
        i, flushState, cubicProc, pipeline, batch, startIndices, startScissorSubBatch, drawBounds);
  }
...
void GrCCStroker::drawLog2Strokes(
    int numSegmentsLog2, GrOpFlushState* flushState, const GrPrimitiveProcessor& processor,
