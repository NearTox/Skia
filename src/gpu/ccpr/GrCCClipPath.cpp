...
void GrCCClipPath::init(
  ...
  fAtlasLazyProxy = GrCCAtlas::MakeLazyAtlasProxy(
      [this](GrResourceProvider* resourceProvider, const GrBackendFormat& format, int sampleCount) {
        ...
        return GrSurfaceProxy::LazyCallbackResult(
            std::move(texture), true, GrSurfaceProxy::LazyInstantiationKeyMode::kUnsynced);
      },
...
void GrCCClipPath::renderPathInAtlas(
    GrCCPerFlushResources* resources, GrOnFlushResourceProvider* onFlushRP) {
  SkASSERT(this->isInitialized());
  SkASSERT(!fHasAtlas);
