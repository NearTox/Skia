...
sk_sp<GrTextureProxy> GrCCAtlas::MakeLazyAtlasProxy(
  ...
  switch (coverageType) {
    case CoverageType::kFP16_CoverageCount: sampleCount = 1; break;
    case CoverageType::kA8_Multisample:
      ...
      break;
    case CoverageType::kA8_LiteralCoverage: sampleCount = 1; break;
  }
...
GrCCAtlas::GrCCAtlas(CoverageType coverageType, const Specs& specs, const GrCaps& caps)
    : fCoverageType(coverageType),
      fMaxTextureSize(
          SkTMax(SkTMax(specs.fMinHeight, specs.fMinWidth), specs.fMaxPreferredTextureSize)) {
      ...
      [this](GrResourceProvider* resourceProvider, const GrBackendFormat& format, int sampleCount) {
        if (!fBackingTexture) {
          ...
          fBackingTexture = resourceProvider->createTexture(
              desc, format, GrRenderable::kYes, sampleCount, GrMipMapped::kNo, SkBudgeted::kYes,
              GrProtected::kNo);
        }
...
std::unique_ptr<GrRenderTargetContext> GrCCAtlas::makeRenderTargetContext(
  ...
  if (!rtc) {
    ...
    {
      SkDebugf(
          "WARNING: failed to allocate a %ix%i atlas. Some paths will not be drawn.\n", fWidth,
          fHeight);
    }
    return nullptr;
  }
