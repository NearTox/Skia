...
GrCCAtlas::GrCCAtlas(CoverageType coverageType, const Specs& specs, const GrCaps& caps)
    : GrDynamicAtlas(
          CoverageTypeToColorType(coverageType), CoverageTypeHasInternalMultisample(coverageType),
...
sk_sp<GrCCCachedAtlas> GrCCAtlas::refOrMakeCachedAtlas(GrOnFlushResourceProvider* onFlushRP) {
  if (!fCachedAtlas) {
    ...
    fCachedAtlas =
        sk_make_sp<GrCCCachedAtlas>(fCoverageType, atlasUniqueKey, sk_ref_sp(this->textureProxy()));
  }
