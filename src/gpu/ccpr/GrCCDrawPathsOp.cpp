...
std::unique_ptr<GrCCDrawPathsOp> GrCCDrawPathsOp::InternalMake(
  ...
  return pool->allocate<GrCCDrawPathsOp>(
      m, shape, strokeDevWidth, shapeConservativeIBounds, maskDevIBounds, conservativeDevBounds,
      std::move(paint));
}
...
#endif
      auto coverageMode =
          GrCCAtlas::CoverageTypeToPathCoverageMode(fCacheEntry->cachedAtlas()->coverageType());
      op->recordInstance(
          coverageMode, fCacheEntry->cachedAtlas()->getOnFlushProxy(),
