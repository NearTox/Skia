...
GrOp::Owner GrCCDrawPathsOp::Make(
  ...
  if (!stroke.isFillStyle()) {
    strokeDevWidth =
        GrCoverageCountingPathRenderer::GetStrokeDevWidth(m, stroke, &conservativeInflationRadius);
  ...
  if (conservativeSize > GrCoverageCountingPathRenderer::kPathCropThreshold) {
    ...
    return InternalMake(
        context, clipIBounds, SkMatrix::I(), croppedDevShape, strokeDevWidth, conservativeDevBounds,
        std::move(paint));
  }

  return InternalMake(
      context, clipIBounds, m, shape, strokeDevWidth, conservativeDevBounds, std::move(paint));
}
...
GrOp::Owner GrCCDrawPathsOp::InternalMake(
  ...
  SkASSERT(
      std::max(conservativeDevBounds.height(), conservativeDevBounds.width()) <
  ...
  return GrOp::Make<GrCCDrawPathsOp>(
      context, m, shape, strokeDevWidth, shapeConservativeIBounds, maskDevIBounds,
      conservativeDevBounds, std::move(paint));
}

GrCCDrawPathsOp::GrCCDrawPathsOp(
    const SkMatrix& m, const GrStyledShape& shape, float strokeDevWidth,
    ...
    : GrDrawOp(ClassID()),
      ...
      fDraws(
          m, shape, strokeDevWidth, shapeConservativeIBounds, maskDevIBounds, paint.getColor4f()),
...
GrProcessorSet::Analysis GrCCDrawPathsOp::SingleDraw::finalize(
    const GrCaps& caps, const GrAppliedClip* clip, bool hasMixedSampledCoverage,
    GrClampType clampType, GrProcessorSet* processors) {
...
GrOp::CombineResult GrCCDrawPathsOp::onCombineIfPossible(GrOp* op, SkArenaAlloc*, const GrCaps&) {
  ...
  SkASSERT(fNumDraws);
  SkASSERT(!that->fOwningPerOpsTaskPaths || that->fOwningPerOpsTaskPaths == fOwningPerOpsTaskPaths);
...
void GrCCDrawPathsOp::accountForOwnPaths(
...
void GrCCDrawPathsOp::SingleDraw::accountForOwnPath(
  ...
  if (pathCache) {
    fCacheEntry = pathCache->find(onFlushRP, fShape, fMaskDevIBounds, fMatrix, &fCachedMaskShift);
  }
...
bool GrCCDrawPathsOp::SingleDraw::shouldCachePathMask(int maxRenderTargetSize) const {
  ...
  int shapeMaxDimension =
      std::max(fShapeConservativeIBounds.height(), fShapeConservativeIBounds.width());
...
void GrCCDrawPathsOp::SingleDraw::setupResources(
  ...
  auto fillRule =
      (fShape.style().strokeRec().isFillStyle()) ? GrFillRuleForSkPath(path) : GrFillRule::kNonzero;
    ...
    if (fCachedAtlasProxy) {
      if (DoCopiesToA8Coverage::kYes == doCopies &&
        ...
        if (fCacheEntry->cachedAtlas()) {
          ...
          SkASSERT(fCachedAtlasProxy);
          SkASSERT(CoverageType::kA8_LiteralCoverage == fCacheEntry->cachedAtlas()->coverageType());
          fCachedAtlasCoverageType = CoverageType::kA8_LiteralCoverage;
        } else {
      ...
      op->recordInstance(coverageMode, fCachedAtlasProxy.get(), resources->nextPathInstanceIdx());
      resources->appendDrawPathInstance().set(*fCacheEntry, fCachedMaskShift, fColor, fillRule);
  ...
  if (auto atlas = resources->renderShapeInAtlas(
          ...
          &devToAtlasOffset)) {
    auto coverageMode =
        GrCCAtlas::CoverageTypeToPathCoverageMode(resources->renderedPathCoverageType());
  ...
  for (const InstanceRange& range : fInstanceRanges) {
    ...
    if (atlas->isInstantiated()) {  // Instantiation can fail in exceptional circumstances.
      ...
      GrCCPathProcessor pathProc(
          range.fCoverageMode, atlas->peekTexture(), swizzle, GrCCAtlas::kTextureOrigin,
          fViewMatrixIfUsingLocalCoords);
      pathProc.drawPaths(
          flushState, pipeline, *atlas, *resources, baseInstance, range.fEndInstanceIdx,
          this->bounds());
    }
