...
protected:
    ...
    void emitGeometryShader(const GrGSCoverageProcessor& proc, GrGLSLVaryingHandler* varyingHandler,
...
public:
    ...
    void onEmitGeometryShader(const GrGSCoverageProcessor& proc, GrGLSLGeometryBuilder* g,
        ...
        g->codeAppendf("float2 left = pts[(corneridx + (%s > 0 ? %i : 1)) %% %i];", wind.c_str(),
                       proc.numInputPoints() - 1, proc.numInputPoints());
        g->codeAppendf("float2 right = pts[(corneridx + (%s > 0 ? 1 : %i)) %% %i];", wind.c_str(),
                       proc.numInputPoints() - 1, proc.numInputPoints());
        ...
        if (proc.isTriangles()) {
            ...
            if (!fShader->calculatesOwnEdgeCoverage()) {
                g->codeAppendf("%s = -%s;", fEdgeDistanceEquation.c_str(),
                               fEdgeDistanceEquation.c_str());
            }
...
void GrGSCoverageProcessor::draw(GrOpFlushState* flushState, const GrPipeline& pipeline,
    ...
    for (int i = 0; i < 2; ++i) {
        fSubpass = (Subpass)i;
        this->GrCCCoverageProcessor::draw(flushState, pipeline, scissorRects, meshes, meshCount,
                                          drawBounds);
    }
}
