...
class GrGSCoverageProcessor::Impl : public GrGLSLGeometryProcessor {
 protected:
  ...
  void emitGeometryShader(
    ...
    g->codeAppendf(
        "float%ix2 pts = transpose(float2x%i(sk_in[0].%s, sk_in[1].%s));", inputWidth, inputWidth,
        posValues, posValues);
        ...
        [&]() {
          ...
          if (!coverage) {
            if (!fShader->calculatesOwnEdgeCoverage()) {
              ...
              fnBody.appendf(
                  "float coverage = dot(float3(vertexpos, 1), %s);", fEdgeDistanceEquation.c_str());
            } else {
    ...
    if (!this->hasCoverage(proc) && !fShader->calculatesOwnEdgeCoverage()) {
      ...
      g->codeAppendf(
          "%s = float3(-n, dot(n, p0) - .5*sign(%s));", fEdgeDistanceEquation.c_str(),
          wind.c_str());
    }
  ...
  virtual void onEmitGeometryShader(
      const GrGSCoverageProcessor&, GrGLSLGeometryBuilder*, const GrShaderVar& wind,
...
class GrGSCoverageProcessor::TriangleHullImpl : public GrGSCoverageProcessor::Impl {
 public:
  ...
  void onEmitGeometryShader(
    ...
    g->codeAppendf("int i = (%s > 0 ? sk_InvocationID : 4 - sk_InvocationID) %% 3;", wind.c_str());
    ...
    g->codeAppend(
        "downbloat = float2(0 != downbloat.y ? downbloat.y : downbloat.x, "
    ...
    Shader::CalcEdgeCoverageAtBloatVertex(
        g, "top", "right", "float2(+rightbloat.y, -rightbloat.x)", "coverages[0]");
    ...
    g->codeAppendf("%s(top, float2(-leftbloat.y, +leftbloat.x), coverages[0]);", emitVertexFn);
    g->codeAppend("}");
    ...
    g->codeAppendf(
        "%s((0 == sk_InvocationID) ? left : right, "
        "(0 == sk_InvocationID) ? leftbloat : downbloat, "
        "coverages[2]);",
        emitVertexFn);
    g->codeAppend("}");
    g->codeAppend("if (all(right_down_notequal) && 0 != sk_InvocationID) {");
    g->codeAppendf("%s(right, float2(-rightbloat.y, +rightbloat.x), coverages[3]);", emitVertexFn);
    g->codeAppend("}");
...
class GrGSCoverageProcessor::CurveHullImpl : public GrGSCoverageProcessor::Impl {
 public:
  ...
  void onEmitGeometryShader(
    ...
    g->codeAppend(
        "float2 leftbloat = float2(topleft.y > bottomleft.y ? +1 : -1, "
        "topleft.x > bottomleft.x ? -1 : +1);");
    g->codeAppend(
        "float2 upbloat = float2(topright.y > topleft.y ? +1 : -1, "
        "topright.x > topleft.x ? -1 : +1);");
    g->codeAppend(
        "float2 rightbloat = float2(bottomright.y > topright.y ? +1 : -1, "
...
class GrGSCoverageProcessor::CornerImpl : public GrGSCoverageProcessor::Impl {
 public:
  ...
  void onEmitGeometryShader(
    ...
    g->codeAppendf(
        "float2 left = pts[(corneridx + (%s > 0 ? %i : 1)) %% %i];", wind.c_str(),
        proc.numInputPoints() - 1, proc.numInputPoints());
    g->codeAppendf(
        "float2 right = pts[(corneridx + (%s > 0 ? 1 : %i)) %% %i];", wind.c_str(),
        proc.numInputPoints() - 1, proc.numInputPoints());
    ...
    g->codeAppend(
        "float2 outbloat = float2(leftdir.x > rightdir.x ? +1 : -1, "
    ...
    if (proc.isTriangles()) {
      ...
      Shader::CalcEdgeCoveragesAtBloatVertices(
          g, "left", "corner", "-outbloat", "-crossbloat", "left_coverages");
      g->codeAppend("}");

      g->codeAppend("half2 right_coverages; {");
      Shader::CalcEdgeCoveragesAtBloatVertices(
          g, "corner", "right", "-outbloat", "crossbloat", "right_coverages");
      g->codeAppend("}");
      ...
      g->codeAppendf(
          "%s(corner, -crossbloat, right_coverages[1] - left_coverages[1],"
          "half2(1 + left_coverages[1], 1));",
          emitVertexFn);

      g->codeAppendf(
          "%s(corner, outbloat, 1 + left_coverages[0] + right_coverages[0], "
          "half2(0, attenuation));",
          emitVertexFn);
      ...
      g->codeAppendf(
          "%s(corner, crossbloat, left_coverages[1] - right_coverages[1],"
      ...
      if (!fShader->calculatesOwnEdgeCoverage()) {
        g->codeAppendf("%s = -%s;", fEdgeDistanceEquation.c_str(), fEdgeDistanceEquation.c_str());
      }
      g->codeAppendf("%s(corner, -crossbloat, half2(-1, 1));", emitVertexFn);
      g->codeAppendf("%s(corner, outbloat, half2(0, attenuation));", emitVertexFn);
...
void GrGSCoverageProcessor::reset(PrimitiveType primitiveType, GrResourceProvider*) {
  ...
  if (4 == this->numInputPoints() || this->hasInputWeight()) {
    fInputXOrYValues = {"x_or_y_values", kFloat4_GrVertexAttribType, kFloat4_GrSLType};
    GR_STATIC_ASSERT(
        sizeof(QuadPointInstance) == 2 * GrVertexAttribTypeSize(kFloat4_GrVertexAttribType));
    GR_STATIC_ASSERT(
        offsetof(QuadPointInstance, fY) == GrVertexAttribTypeSize(kFloat4_GrVertexAttribType));
  } else {
    fInputXOrYValues = {"x_or_y_values", kFloat3_GrVertexAttribType, kFloat3_GrSLType};
    GR_STATIC_ASSERT(
        sizeof(TriPointInstance) == 2 * GrVertexAttribTypeSize(kFloat3_GrVertexAttribType));
    GR_STATIC_ASSERT(
        offsetof(TriPointInstance, fY) == GrVertexAttribTypeSize(kFloat3_GrVertexAttribType));
  }

  this->setVertexAttributes(&fInputXOrYValues, 1);
}

void GrGSCoverageProcessor::appendMesh(
...
GrGLSLPrimitiveProcessor* GrGSCoverageProcessor::onCreateGLSLInstance(
  ...
  if (Subpass::kHulls == fSubpass) {
    return this->isTriangles() ? (Impl*)new TriangleHullImpl(std::move(shader))
