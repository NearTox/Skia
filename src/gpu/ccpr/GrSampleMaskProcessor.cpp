...
void GrSampleMaskProcessor::Impl::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
  ...
  if (PrimitiveType::kTriangles == proc.fPrimitiveType) {
    ...
    v->codeAppendf(
        "float%ix2 pts = transpose(float2x%i(X.%s, Y.%s));", inputWidth, inputWidth, swizzle,
        swizzle);
...
void GrSampleMaskProcessor::reset(
  ...
  switch (fPrimitiveType) {
    ...
    case PrimitiveType::kConics: {
      ...
      auto shaderVarType =
          (PrimitiveType::kQuadratics == fPrimitiveType) ? kFloat3_GrSLType : kFloat4_GrSLType;
...
void GrSampleMaskProcessor::bindBuffers(
...
void GrSampleMaskProcessor::drawInstances(
