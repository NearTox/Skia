...
GrCCFiller::GrCCFiller(
    Algorithm algorithm, int numPaths, int numSkPoints, int numSkVerbs, int numConicWeights)
...
void GrCCFiller::PathInfo::tessellateFan(
    Algorithm algorithm, const SkPath& originalPath, const GrCCFillGeometry& geometry, int verbsIdx,
      ...
      case Verb::kBeginPath:
        ...
        if (currFanIsTessellated) {
          this->emitTessellatedFan(
              ...
              devToAtlasOffset, triangleOrdering, triPointInstanceData, quadPointInstanceData,
              currIndices);
        }
        ++nextPathInfo;
        continue;
