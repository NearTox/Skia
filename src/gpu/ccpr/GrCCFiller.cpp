...
void GrCCFiller::parseDeviceSpaceFill(
  ...
  for (SkPath::Verb verb : SkPathPriv::Verbs(path)) {
    switch (verb) {
      ...
      case SkPath::kConic_Verb:
        ...
        ++conicWeightsIdx;
        continue;
      default: SK_ABORT("Unexpected path verb.");
    }
  }
  ...
  if (tessellationWork * (50 * 50) + (100 * 100) <
      fanningWork) {  // Don't tessellate under 100x100.
    fPathInfos.back().tessellateFan(
        fGeometry, currPathVerbsIdx, currPathPointsIdx, clippedDevIBounds,
...
void GrCCFiller::PathInfo::tessellateFan(
    const GrCCFillGeometry& geometry, int verbsIdx, int ptsIdx, const SkIRect& clippedDevIBounds,
  ...
  newTriangleCounts->fTriangles = newTriangleCounts->fWeightedTriangles = 0;
  ...
  for (int i = verbsIdx + 1; i < verbs.count(); ++i) {
    switch (verbs[i]) {
      case Verb::kBeginPath: SK_ABORT("Invalid GrCCFillGeometry"); continue;

      case Verb::kBeginContour: fan.moveTo(pts[ptsIdx++]); continue;

      case Verb::kLineTo: fan.lineTo(pts[ptsIdx++]); continue;
      ...
      case Verb::kEndOpenContour: fan.close(); continue;
    }
  }
...
GrCCFiller::BatchID GrCCFiller::closeCurrentBatch() {
  ...
  PrimitiveTallies batchTotalCounts =
      fTotalPrimitiveCounts[(int)GrScissorTest::kDisabled] - lastBatch.fEndNonScissorIndices;
  ...
  fBatches.push_back() = {fTotalPrimitiveCounts[(int)GrScissorTest::kDisabled],
                          fScissorSubBatches.count(), batchTotalCounts};
...
static TriPointInstance* emit_recursive_fan(
    ...
    int indexCount, const Sk2f& devToAtlasOffset, TriPointInstance out[]) {
  if (indexCount < 3) {
    return out;
  }
  ...
  out++->set(
      pts[indices[firstIndex]], pts[indices[firstIndex + oneThirdCount]],
  ...
  out = emit_recursive_fan(
      ...
      devToAtlasOffset, out);

  int endIndex = firstIndex + indexCount;
  int32_t oldValue = indices[endIndex];
  indices[endIndex] = indices[firstIndex];
  out = emit_recursive_fan(
      pts, indices, firstIndex + twoThirdsCount, indexCount - twoThirdsCount + 1, devToAtlasOffset,
      out);
...
static void emit_tessellated_fan(
...
bool GrCCFiller::prepareToDraw(GrOnFlushResourceProvider* onFlushRP) {
  ...
  fBaseInstances[1].fTriangles = fBaseInstances[0].fTriangles + fTotalPrimitiveCounts[0].fTriangles;
  fBaseInstances[0].fQuadratics =
      fBaseInstances[1].fTriangles + fTotalPrimitiveCounts[1].fTriangles;
  fBaseInstances[1].fQuadratics =
      fBaseInstances[0].fQuadratics + fTotalPrimitiveCounts[0].fQuadratics;
  ...
  fBaseInstances[1].fWeightedTriangles =
      fBaseInstances[0].fWeightedTriangles + fTotalPrimitiveCounts[0].fWeightedTriangles;
  fBaseInstances[0].fCubics =
      fBaseInstances[1].fWeightedTriangles + fTotalPrimitiveCounts[1].fWeightedTriangles;
  ...
  for (Verb verb : fGeometry.verbs()) {
    switch (verb) {
      case Verb::kBeginPath:
        ...
        devToAtlasOffset = Sk2f(
            static_cast<float>(nextPathInfo->devToAtlasOffset().fX),
        ...
        if (currFanIsTessellated) {
          emit_tessellated_fan(
              ...
              devToAtlasOffset, triPointInstanceData, quadPointInstanceData, currIndices);
        }
        ++nextPathInfo;
        continue;
      ...
      case Verb::kMonotonicConicTo:
        quadPointInstanceData[currIndices->fConics++].setW(
            &pts[ptsIdx], devToAtlasOffset, fGeometry.getConicWeight(nextConicWeightIdx));
        ptsIdx += 2;
        ++nextConicWeightIdx;
        if (!currFanIsTessellated) {
          SkASSERT(!currFan.empty());
      ...
      case Verb::kEndOpenContour:  // endPt != startPt.
        ...
        if (!currFanIsTessellated && currFan.count() >= 3) {
          ...
          SkDEBUGCODE(TriPointInstance* end =) emit_recursive_fan(
              pts, currFan, 0, fanSize, devToAtlasOffset,
