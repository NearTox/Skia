...
inline void GrCCFillGeometry::appendMonotonicQuadratic(
    const Sk2f& p0, const Sk2f& p1, const Sk2f& p2) {
...
enum class ExcludedTerm : bool { kQuadraticTerm, kLinearTerm };
...
static inline void find_chops_around_loop_intersection(
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    float roots[3] = {P[i] * std::cos(theta) + D[i], P[i] * std::cos(theta + k2PiOver3) + D[i],
...
void GrCCFillGeometry::cubicTo(const SkPoint P[4], float inflectPad, float loopIntersectPad) {
  ...
  ExcludedTerm skipTerm =
      (std::abs(D[2]) > std::abs(D[1])) ? ExcludedTerm::kQuadraticTerm : ExcludedTerm::kLinearTerm;
  Sk2f C0 = SkNx_fma(Sk2f(3), p1 - p2, p3 - p0);
  Sk2f C1 = (ExcludedTerm::kLinearTerm == skipTerm ? SkNx_fma(Sk2f(-2), p1, p0 + p2) : p1 - p0) * 3;
...
void GrCCFillGeometry::appendCubics(
    AppendCubicMode mode, const Sk2f& p0, const Sk2f& p1, const Sk2f& p2, const Sk2f& p3,
    const float chops[], int numChops, float localT0, float localT1) {
  if (numChops) {
    SkASSERT(numChops > 0);
    ...
    if (T <= localT0) {
      ...
      this->appendCubics(
          rightMode, p0, p1, p2, p3, &chops[midChopIdx + 1], numChops - midChopIdx - 1, localT0,
          localT1);
      return;
    }
    ...
    this->appendCubics(
        rightMode, pT, p11, p12, p3, &chops[midChopIdx + 1], numChops - midChopIdx - 1, T, localT1);
    return;
  }
...
void GrCCFillGeometry::appendCubics(
  ...
  if (SkCubicType::kLoop != fCurrCubicType) {
    ...
    if (maxSubdivisions && !is_convex_curve_monotonic(p0, tan0, p3, tan1)) {
      this->chopAndAppendCubicAtMidTangent(mode, p0, p1, p2, p3, tan0, tan1, maxSubdivisions - 1);
      return;
    }
...
inline void GrCCFillGeometry::chopAndAppendCubicAtMidTangent(
    AppendCubicMode mode, const Sk2f& p0, const Sk2f& p1, const Sk2f& p2, const Sk2f& p3,
    const Sk2f& tan0, const Sk2f& tan1, int maxFutureSubdivisions) {
  float midT =
      find_midtangent(tan0, tan1, p3 + (p1 - p2) * 3 - p0, (p0 - p1 * 2 + p2) * 2, p1 - p0);
...
void GrCCFillGeometry::conicTo(const SkPoint P[3], float w) {
  ...
  if (!is_convex_curve_monotonic(p0, tan0, p2, tan1)) {
    ...
    float midT = find_midtangent(
        tan0, tan1, (w - 1) * (p2 - p0), (p2 - p0) - 2 * w * (p1 - p0), w * (p1 - p0));
...
void GrCCFillGeometry::appendMonotonicConic(
