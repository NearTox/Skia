...
void GrCCCubicShader::emitSetupCode(
  ...
  s->codeAppend(
      "m.ts = float2(2, q) * (discr >= 0 ? float2(D3, 1) "
  ...
  s->codeAppend(
      "float3x3 CI = inverse(float3x3(C[0][0], C[0][middlerow], C[0][3], "
      "C[1][0], C[1][middlerow], C[1][3], "
      "      0,               0,       1));");
  s->codeAppendf(
      "%s = CI * float3x3(K[0], K[middlerow], K[3], "
      "L[0], L[middlerow], L[3], "
      "M[0], M[middlerow], M[3]);",
      fKLMMatrix.c_str());
  ...
  s->codeAppendf(
      "float2 orientation = sign(float3(midpoint, 1) * float2x3(%s[1], %s[2]));",
      fKLMMatrix.c_str(), fKLMMatrix.c_str());
  s->codeAppendf(
      "%s *= float3x3(orientation[0] * orientation[1], 0, 0, "
      "0, orientation[0], 0, "
      "0, 0, orientation[1]);",
      fKLMMatrix.c_str());
}
...
void GrCCCubicShader::onEmitVaryings(
  ...
  code->appendf(
      "%s.xy = 2*bloat * 3 * klm[0] * %s[0].xy;", OutName(fGradMatrix), fKLMMatrix.c_str());
  code->appendf(
      "%s.zw = -2*bloat * (klm[1] * %s[2].xy + klm[2] * %s[1].xy);", OutName(fGradMatrix),
      fKLMMatrix.c_str(), fKLMMatrix.c_str());
  ...
  if (cornerCoverage) {
    ...
    code->appendf("%s = half2(hull_coverage, 1) * %s;", OutName(fCornerCoverage), cornerCoverage);
  }
}
...
void GrCCCubicShader::emitFragmentCoverageCode(
  ...
  if (fCornerCoverage.fsIn()) {
    f->codeAppendf(
        "%s = %s.x * %s.y + %s;",  // Attenuated corner coverage.
        outputCoverage, fCornerCoverage.fsIn(), fCornerCoverage.fsIn(), outputCoverage);
  }
}

void GrCCCubicShader::calcHullCoverage(
...
void GrCCCubicShader::emitSampleMaskCode(GrGLSLFPFragmentBuilder* f) const {
  f->codeAppendf(
      "float k = %s.x, l = %s.y, m = %s.z;", fKLM_fEdge.fsIn(), fKLM_fEdge.fsIn(),
      fKLM_fEdge.fsIn());
