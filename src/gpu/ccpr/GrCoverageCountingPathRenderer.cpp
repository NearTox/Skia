...
void GrCoverageCountingPathRenderer::recordOp(GrOp::Owner op, const DrawPathArgs& args) {
  if (op) {
...
void GrCoverageCountingPathRenderer::preFlush(
  ...
  auto doCopies = DoCopiesToA8Coverage(
      numCopies > kDoCopiesThreshold || specs.fCopyAtlasSpecs.fApproxNumPixels > 256 * 256);
...
void GrCoverageCountingPathRenderer::postFlush(
    GrDeferredUploadToken, SkSpan<const uint32_t> /* taskIDs */) {
  SkASSERT(fFlushing);
