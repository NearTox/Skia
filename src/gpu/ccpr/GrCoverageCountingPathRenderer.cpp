...
bool GrCoverageCountingPathRenderer::IsSupported(const GrCaps& caps) {
  ...
  if (caps.driverBlacklistCCPR() || !caps.allowCoverageCounting() || !shaderCaps.integerSupport() ||
...
sk_sp<GrCoverageCountingPathRenderer> GrCoverageCountingPathRenderer::CreateIfSupported(
  ...
  return sk_sp<GrCoverageCountingPathRenderer>(
      (IsSupported(caps)) ? new GrCoverageCountingPathRenderer(allowCaching, contextUniqueID)
                          : nullptr);
}

GrCoverageCountingPathRenderer::GrCoverageCountingPathRenderer(
    AllowCaching allowCaching, uint32_t contextUniqueID) {
...
std::unique_ptr<GrFragmentProcessor> GrCoverageCountingPathRenderer::makeClipProcessor(
  ...
  return skstd::make_unique<GrCCClipProcessor>(
      &clipPath, MustCheckBounds(mustCheckBounds), deviceSpacePath.getFillType());
}
