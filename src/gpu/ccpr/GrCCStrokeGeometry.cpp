...
static GrCCStrokeGeometry::Verb join_verb_from_join(SkPaint::Join join) {
  ...
  switch (join) {
    case SkPaint::kBevel_Join: return Verb::kBevelJoin;
    case SkPaint::kMiter_Join: return Verb::kMiterJoin;
    case SkPaint::kRound_Join: return Verb::kRoundJoin;
  }
  SK_ABORT("Invalid SkPaint::Join.");
}

void GrCCStrokeGeometry::beginPath(
    const SkStrokeRec& stroke, float strokeDevWidth, InstanceTallies* tallies) {
...
void GrCCStrokeGeometry::cubicTo(const SkPoint P[4]) {
  ...
  this->cubicTo(
      fCurrStrokeJoinVerb, P, numRoots > 0 ? roots[numRoots / 2] : 0,
...
// from the actual curve.
static inline float wangs_formula_cubic(
    const Sk2f& p0, const Sk2f& p1, const Sk2f& p2, const Sk2f& p3) {
  static constexpr float k = (3 * 2) / (8 * kMaxErrorFromLinearization);
  float f = SkScalarSqrt(k * length(Sk2f::Max((p2 - p1 * 2 + p0).abs(), (p3 - p2 * 2 + p1).abs())));
  return SkScalarCeilToInt(f);
}

void GrCCStrokeGeometry::cubicTo(
    Verb leftJoinVerb, const SkPoint P[4], float maxCurvatureT, float leftMaxCurvatureT,
  ...
  if (isCurvatureTooStrong) {
    ...
    if (leftT > 0) {
      ...
      this->cubicTo(
          leftJoinVerb, ptsBuffer, /*maxCurvatureT=*/1,
    ...
    if (rightT < 1) {
      ...
      currCubic = ptsBuffer + 3;
      this->cubicTo(
          Verb::kInternalRoundJoin, currCubic, /*maxCurvatureT=*/0, kLeftMaxCurvatureNone,
          kRightMaxCurvatureNone);
    } else {
  ...
  if (kRightMaxCurvatureNone != rightMaxCurvatureT) {
    this->cubicTo(leftJoinVerb, P, rightMaxCurvatureT, leftMaxCurvatureT, kRightMaxCurvatureNone);
    return;
  }
  if (kLeftMaxCurvatureNone != leftMaxCurvatureT) {
    SkASSERT(kRightMaxCurvatureNone == rightMaxCurvatureT);
    this->cubicTo(
        leftJoinVerb, P, leftMaxCurvatureT, kLeftMaxCurvatureNone, kRightMaxCurvatureNone);
    return;
  }
...
void GrCCStrokeGeometry::recordRoundJoin(
    Verb joinVerb, float miterCapHeightOverWidth, float conicWeight) {
