...
class GrCCCoverageProcessor::TriangleShader : public GrCCCoverageProcessor::Shader {
  ...
  void emitFragmentCoverageCode(
    ...
    if (kHalf_GrSLType == fCoverages.type()) {
      ...
      f->codeAppendf(
          "%s = %s.z * %s.y + %s.x;", outputCoverage, fCoverages.fsIn(), fCoverages.fsIn(),
          fCoverages.fsIn());
    }
  }
...
void GrCCCoverageProcessor::Shader::CalcWind(
    const GrCCCoverageProcessor& proc, GrGLSLVertexGeoBuilder* s, const char* pts,
  ...
  if (proc.isTriangles()) {
    ...
    s->codeAppendf("%s = (abs(area_x2 * 1024) > basewidth) ? sign(half(area_x2)) : 0;", outputWind);
  } else {
...
void GrCCCoverageProcessor::Shader::CalcEdgeCoverageAtBloatVertex(
    GrGLSLVertexGeoBuilder* s, const char* leftPt, const char* rightPt, const char* rasterVertexDir,
  ...
  //
  s->codeAppendf("float2 n = float2(%s.y - %s.y, %s.x - %s.x);", rightPt, leftPt, leftPt, rightPt);
  ...
  s->codeAppendf("%s = half(abs(t) != nwidth ? t / nwidth : sign(t)) * -.5 - .5;", outputCoverage);
}

void GrCCCoverageProcessor::Shader::CalcEdgeCoveragesAtBloatVertices(
    GrGLSLVertexGeoBuilder* s, const char* leftPt, const char* rightPt, const char* bloatDir1,
    const char* bloatDir2, const char* outputCoverages) {
  ...
  s->codeAppendf(
      "%s[i] = half(abs(t[i]) != nwidth ? t[i] / nwidth : sign(t[i])) * -.5 - .5;",
      outputCoverages);
  s->codeAppendf("}");
}

void GrCCCoverageProcessor::Shader::CalcCornerAttenuation(
    GrGLSLVertexGeoBuilder* s, const char* leftDir, const char* rightDir,
  ...
  s->codeAppendf(
      "half2 abs_bisect_maybe_transpose = abs((0 == obtuseness) ? half2(%s - %s) : "
      "half2(%s + %s));",
      leftDir, rightDir, leftDir, rightDir);
  s->codeAppend(
      "half axis_alignedness = "
  ...
  //    (i.e. outputAttenuation = 1)
  s->codeAppendf("%s = max(obtuseness, axis_alignedness * ninety_degreesness);", outputAttenuation);
}
...
void GrCCCoverageProcessor::bindPipeline(
    GrOpFlushState* flushState, const GrPipeline& pipeline, const SkRect& drawBounds,
  ...
  GrProgramInfo programInfo(
      ...
      flushState->proxy()->backendFormat(), flushState->writeView()->origin(), &pipeline, stencil,
      this, this->primType(), 0, flushState->renderPassBarriers());
