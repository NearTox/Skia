...
static constexpr int32_t edge_vertex_data(
  ...
  return pack_vertex_data(
      ...
      kVertexData_IsEdgeBit | (!endptIdx ? kVertexData_InvertNegativeCoverageBit : 0));
}
...
static constexpr int32_t kTriangleVertices[] = {
    hull_vertex_data(0, 0, 3),      hull_vertex_data(0, 1, 3),      hull_vertex_data(0, 2, 3),
    hull_vertex_data(1, 0, 3),      hull_vertex_data(1, 1, 3),      hull_vertex_data(1, 2, 3),
    hull_vertex_data(2, 0, 3),      hull_vertex_data(2, 1, 3),      hull_vertex_data(2, 2, 3),

    edge_vertex_data(0, 0, 0, 3),   edge_vertex_data(0, 0, 1, 3),   edge_vertex_data(0, 0, 2, 3),
    edge_vertex_data(0, 1, 0, 3),   edge_vertex_data(0, 1, 1, 3),   edge_vertex_data(0, 1, 2, 3),

    edge_vertex_data(1, 0, 0, 3),   edge_vertex_data(1, 0, 1, 3),   edge_vertex_data(1, 0, 2, 3),
    edge_vertex_data(1, 1, 0, 3),   edge_vertex_data(1, 1, 1, 3),   edge_vertex_data(1, 1, 2, 3),

    edge_vertex_data(2, 0, 0, 3),   edge_vertex_data(2, 0, 1, 3),   edge_vertex_data(2, 0, 2, 3),
    edge_vertex_data(2, 1, 0, 3),   edge_vertex_data(2, 1, 1, 3),   edge_vertex_data(2, 1, 2, 3),

    corner_vertex_data(2, 0, 1, 0), corner_vertex_data(2, 0, 1, 1), corner_vertex_data(2, 0, 1, 2),
    corner_vertex_data(2, 0, 1, 3),

    corner_vertex_data(0, 1, 2, 0), corner_vertex_data(0, 1, 2, 1), corner_vertex_data(0, 1, 2, 2),
    corner_vertex_data(0, 1, 2, 3),

    corner_vertex_data(1, 2, 0, 0), corner_vertex_data(1, 2, 0, 1), corner_vertex_data(1, 2, 0, 2),
...
static constexpr uint16_t kTriangleIndicesAsStrips[] = {
    ...
    3,
    8,
    kRestartStrip,  // First corner and main body of the hull.
    4,
    5,
    3,
    6,
    8,
    7,
    kRestartStrip,  // Opposite side and corners of the hull.
    10,
    9,
    11,
    14,
    ...
    24,
    25,
    kRestartStrip,  // Third edge.
    28,
    27,
    29,
    30,
    kRestartStrip,  // First corner.
    32,
    31,
    33,
    34,
    kRestartStrip,  // Second corner.
    36,
    35,
    37,
    38  // Third corner.
...
static constexpr int32_t kCurveVertices[] = {
    hull_vertex_data(0, 0, 4),      hull_vertex_data(0, 1, 4),      hull_vertex_data(0, 2, 4),
    hull_vertex_data(1, 0, 4),      hull_vertex_data(1, 1, 4),      hull_vertex_data(1, 2, 4),
    hull_vertex_data(2, 0, 4),      hull_vertex_data(2, 1, 4),      hull_vertex_data(2, 2, 4),
    hull_vertex_data(3, 0, 4),      hull_vertex_data(3, 1, 4),      hull_vertex_data(3, 2, 4),

    corner_vertex_data(3, 0, 1, 0), corner_vertex_data(3, 0, 1, 1), corner_vertex_data(3, 0, 1, 2),
    corner_vertex_data(3, 0, 1, 3),

    corner_vertex_data(2, 3, 0, 0), corner_vertex_data(2, 3, 0, 1), corner_vertex_data(2, 3, 0, 2),
...
static constexpr uint16_t kCurveIndicesAsStrips[] = {
    ...
    5,
    4,
    kRestartStrip,  // First half of the hull (split diagonally).
    7,
    6,
    8,
    5,
    ...
    11,
    10,
    kRestartStrip,  // Second half of the hull.
    13,
    12,
    14,
    15,
...
void GrVSCoverageProcessor::Impl::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
  ...
  v->codeAppendf(
      "float%ix2 pts = transpose(float2x%i(%s.%s, %s.%s));", inputWidth, inputWidth,
  ...
  if (PrimitiveType::kWeightedTriangles == proc.fPrimitiveType) {
    ...
    SkASSERT(kFloat4_GrVertexAttribType == proc.fInputXAndYValues[kInstanceAttribIdx_X].cpuType());
    v->codeAppendf("wind *= half(%s.w);", proc.fInputXAndYValues[kInstanceAttribIdx_X].name());
  }
  ...
  v->codeAppendf(
      "int clockwise_indices = wind > 0 ? %s : 0x%x - %s;", proc.fPerVertexData.name(),
      ((fNumSides - 1) << kVertexData_LeftNeighborIdShift) |
          ((fNumSides - 1) << kVertexData_RightNeighborIdShift) |
          (((1 << kVertexData_RightNeighborIdShift) - 1) ^ 3) | (fNumSides - 1),
  ...
  v->codeAppendf(
      "float2 left = %s[clockwise_indices >> %i];", hullPts, kVertexData_LeftNeighborIdShift);
  v->codeAppendf(
      "float2 right = %s[(clockwise_indices >> %i) & 3];", hullPts,
      kVertexData_RightNeighborIdShift);

  v->codeAppend("float2 leftbloat = sign(corner - left);");
  v->codeAppend(
      "leftbloat = float2(0 != leftbloat.y ? leftbloat.y : leftbloat.x, "
      "0 != leftbloat.x ? -leftbloat.x : -leftbloat.y);");

  v->codeAppend("float2 rightbloat = sign(right - corner);");
  v->codeAppend(
      "rightbloat = float2(0 != rightbloat.y ? rightbloat.y : rightbloat.x, "
  ...
  v->codeAppend(
      "bloatdir = float2(leftdir.x > rightdir.x ? +1 : -1, "
  ...
  v->codeAppendf(
      "int bloatidx = (%s >> %i) & 3;", proc.fPerVertexData.name(), kVertexData_BloatIdxShift);
  ...
  if (3 == fNumSides) {
    ...
    v->codeAppendf(
        "if (0 != (%s & %i)) {",  // Are we an edge?
        proc.fPerVertexData.name(), kVertexData_IsEdgeBit);
    v->codeAppend("coverage = left_coverage;");
    v->codeAppend("}");

    v->codeAppendf(
        "if (0 != (%s & %i)) {",  // Invert coverage?
        proc.fPerVertexData.name(), kVertexData_InvertNegativeCoverageBit);
  ...
  fShader->emitVaryings(
      varyingHandler, GrGLSLVarying::Scope::kVertToFrag, &AccessCodeString(v), "vertexpos",
      "coverage", "corner_coverage", "wind");
...
void GrVSCoverageProcessor::reset(
    PrimitiveType primitiveType, int subpassIdx, GrResourceProvider* rp) {
  ...
  switch (fPrimitiveType) {
    ...
    case PrimitiveType::kWeightedTriangles: {
      ...
      if (caps.usePrimitiveRestart()) {
        fIndexBuffer = rp->findOrMakeStaticBuffer(
            GrGpuBufferType::kIndex, sizeof(kTriangleIndicesAsStrips), kTriangleIndicesAsStrips,
            gTriangleIndexBufferKey);
        fNumIndicesPerInstance = SK_ARRAY_COUNT(kTriangleIndicesAsStrips);
      } else {
        fIndexBuffer = rp->findOrMakeStaticBuffer(
            GrGpuBufferType::kIndex, sizeof(kTriangleIndicesAsTris), kTriangleIndicesAsTris,
            gTriangleIndexBufferKey);
    ...
    case PrimitiveType::kConics: {
      ...
      fVertexBuffer = rp->findOrMakeStaticBuffer(
          GrGpuBufferType::kVertex, sizeof(kCurveVertices), kCurveVertices, gCurveVertexBufferKey);
      GR_DEFINE_STATIC_UNIQUE_KEY(gCurveIndexBufferKey);
      if (caps.usePrimitiveRestart()) {
        fIndexBuffer = rp->findOrMakeStaticBuffer(
            GrGpuBufferType::kIndex, sizeof(kCurveIndicesAsStrips), kCurveIndicesAsStrips,
  ...
  if (4 == this->numInputPoints() || this->hasInputWeight()) {
    ...
    static_assert(
        sizeof(QuadPointInstance::fX) == GrVertexAttribTypeSize(kFloat4_GrVertexAttribType));
    static_assert(
        sizeof(QuadPointInstance::fY) == GrVertexAttribTypeSize(kFloat4_GrVertexAttribType));
    xyAttribType = kFloat4_GrVertexAttribType;
    xySLType = kFloat4_GrSLType;
  } else {
    static_assert(
        sizeof(TriPointInstance) == 2 * GrVertexAttribTypeSize(kFloat3_GrVertexAttribType));
...
void GrVSCoverageProcessor::drawInstances(
    GrOpsRenderPass* renderPass, int instanceCount, int baseInstance) const {
...
GrGLSLPrimitiveProcessor* GrVSCoverageProcessor::onCreateGLSLInstance(
  ...
  switch (fPrimitiveType) {
    case PrimitiveType::kTriangles:
    case PrimitiveType::kWeightedTriangles: return new Impl(std::move(shader), 3);
    case PrimitiveType::kQuadratics:
    case PrimitiveType::kCubics:
