...
class GrOctoBounds {
 public:
  GrOctoBounds() noexcept = default;
  GrOctoBounds(const SkRect& bounds, const SkRect& bounds45) noexcept {
  ...
  void validateBoundsAreTight(
      const std::function<void(bool cond, const char* file, int line, const char* code)>&
          validateFn) const;
#endif
