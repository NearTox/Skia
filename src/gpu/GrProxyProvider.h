...
class GrProxyProvider {
 public:
  ...
  sk_sp<GrTextureProxy> findOrCreateProxyByUniqueKey(
      const GrUniqueKey&, GrColorType colorType, UseAllocator = UseAllocator::kYes);
  ...
  sk_sp<GrTextureProxy> createProxy(
      const GrBackendFormat&, SkISize dimensions, GrSwizzle readSwizzle, GrRenderable,
      int renderTargetSampleCnt, GrMipMapped, SkBackingFit, SkBudgeted, GrProtected,
  ...
  sk_sp<GrSurfaceProxy> wrapBackendRenderTarget(
      const GrBackendRenderTarget&, GrColorType, ReleaseProc = nullptr, ReleaseContext = nullptr);

  /*
   * Create a render target proxy that wraps a backend texture
   */
  sk_sp<GrSurfaceProxy> wrapBackendTextureAsRenderTarget(
      const GrBackendTexture&, GrColorType, int sampleCnt);
  ...
  sk_sp<GrTextureProxy> createLazyProxy(
      LazyInstantiateCallback&&, const GrBackendFormat&, SkISize dimensions, GrSwizzle readSwizzle,
      GrRenderable, int renderTargetSampleCnt, GrMipMapped, GrMipMapsStatus, GrInternalSurfaceFlags,
      SkBackingFit, SkBudgeted, GrProtected, UseAllocator);
  ...
  static sk_sp<GrTextureProxy> MakeFullyLazyProxy(
      ...
      int renderTargetSampleCnt, GrProtected, const GrCaps&, UseAllocator);
 ...
 private:
  ...
  sk_sp<GrTextureProxy> createWrapped(sk_sp<GrTexture> tex, GrColorType, UseAllocator useAllocator);
