...
class GrProxyProvider {
 public:
  ...
  sk_sp<GrTextureProxy> findOrCreateProxyByUniqueKey(
      const GrUniqueKey&, GrColorType colorType, GrSurfaceOrigin,
  ...
  sk_sp<GrTextureProxy> createProxyFromBitmap(
      const SkBitmap& bitmap, GrMipMapped, SkBackingFit fit);

  /*
   * Create a GrSurfaceProxy without any data.
   */
  sk_sp<GrTextureProxy> createProxy(
      const GrBackendFormat&, const GrSurfaceDesc&, GrSwizzle readSwizzle, GrRenderable,
      int renderTargetSampleCnt, GrSurfaceOrigin, GrMipMapped, SkBackingFit, SkBudgeted,
      GrProtected, GrInternalSurfaceFlags = GrInternalSurfaceFlags::kNone,
  ...
  sk_sp<GrTextureProxy> wrapBackendTexture(
      const GrBackendTexture&, GrColorType, GrSurfaceOrigin, GrWrapOwnership, GrWrapCacheable,
      GrIOType, ReleaseProc = nullptr, ReleaseContext = nullptr);

  sk_sp<GrTextureProxy> wrapCompressedBackendTexture(
      const GrBackendTexture&, GrSurfaceOrigin, GrWrapOwnership, GrWrapCacheable,
  ...
  sk_sp<GrTextureProxy> wrapRenderableBackendTexture(
      const GrBackendTexture&, GrSurfaceOrigin, int sampleCnt, GrColorType, GrWrapOwnership,
      GrWrapCacheable, ReleaseProc = nullptr, ReleaseContext = nullptr);

  /*
   * Create a render target proxy that wraps a backend render target
   */
  sk_sp<GrSurfaceProxy> wrapBackendRenderTarget(
      const GrBackendRenderTarget&, GrColorType, GrSurfaceOrigin, ReleaseProc = nullptr,
      ReleaseContext = nullptr);

  /*
   * Create a render target proxy that wraps a backend texture
   */
  sk_sp<GrSurfaceProxy> wrapBackendTextureAsRenderTarget(
      const GrBackendTexture&, GrColorType, GrSurfaceOrigin, int sampleCnt);

  sk_sp<GrRenderTargetProxy> wrapVulkanSecondaryCBAsRenderTarget(
  ...
  sk_sp<GrTextureProxy> createLazyProxy(
      LazyInstantiateCallback&&, const GrBackendFormat&, const GrSurfaceDesc&,
      GrSwizzle readSwizzle, GrRenderable, int renderTargetSampleCnt, GrSurfaceOrigin, GrMipMapped,
      GrMipMapsStatus, GrInternalSurfaceFlags, SkBackingFit, SkBudgeted, GrProtected, UseAllocator);

  /** A null TextureInfo indicates a non-textureable render target. */
  sk_sp<GrRenderTargetProxy> createLazyRenderTargetProxy(
      LazyInstantiateCallback&&, const GrBackendFormat&, const GrSurfaceDesc&,
      GrSwizzle readSwizzle, int renderTargetSampleCnt, GrSurfaceOrigin origin,
      GrInternalSurfaceFlags, const TextureInfo*, GrMipMapsStatus, SkBackingFit, SkBudgeted,
      GrProtected, bool wrapsVkSecondaryCB, UseAllocator useAllocator);
  ...
  static sk_sp<GrTextureProxy> MakeFullyLazyProxy(
      LazyInstantiateCallback&&, const GrBackendFormat&, GrSwizzle readSwizzle, GrRenderable,
      int renderTargetSampleCnt, GrProtected, GrSurfaceOrigin, const GrCaps&, UseAllocator);
 ...
 private:
   ...
   */
  sk_sp<GrTextureProxy> createMippedProxyFromBitmap(
  ...
  sk_sp<GrTextureProxy> createWrapped(
      sk_sp<GrTexture> tex, GrColorType, GrSurfaceOrigin origin, UseAllocator useAllocator);
