...
std::unique_ptr<GrMemoryPool> GrMemoryPool::Make(size_t preallocSize, size_t minAllocSize) {
  ...
  preallocSize =
      SkTPin(preallocSize, kMinAllocationSize, (size_t)GrBlockAllocator::kMaxAllocationSize);
  minAllocSize =
      SkTPin(minAllocSize, kMinAllocationSize, (size_t)GrBlockAllocator::kMaxAllocationSize);
...
std::unique_ptr<GrOpMemoryPool> GrOpMemoryPool::Make(size_t preallocSize, size_t minAllocSize) {
  ...
  preallocSize = SkTPin(
      preallocSize, GrMemoryPool::kMinAllocationSize, (size_t)GrBlockAllocator::kMaxAllocationSize);
  minAllocSize = SkTPin(
      minAllocSize, GrMemoryPool::kMinAllocationSize, (size_t)GrBlockAllocator::kMaxAllocationSize);
  void* mem = operator new(preallocSize);
