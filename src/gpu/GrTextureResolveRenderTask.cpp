...
void GrTextureResolveRenderTask::addProxy(
    GrDrawingManager* drawingMgr, sk_sp<GrSurfaceProxy> proxyRef,
    GrSurfaceProxy::ResolveFlags flags, const GrCaps& caps) {
  ...
  SkASSERT(
      !drawingMgr->getLastRenderTask(proxy) || drawingMgr->getLastRenderTask(proxy)->isClosed());
  ...
  this->addDependency(drawingMgr, proxy, GrMipMapped::kNo, GrTextureResolveManager(nullptr), caps);
  ...
  for (int i = 0; i < fResolves.count(); ++i) {
    ...
    if (GrSurfaceProxy::ResolveFlags::kMSAA & resolve.fFlags) {
      ...
      if (GrRenderTarget* renderTarget = proxy->peekRenderTarget()) {
        flushState->gpu()->resolveRenderTarget(
            renderTarget, resolve.fMSAAResolveRect, GrGpu::ForExternalIO::kNo);
      }
    }
  }
