...
class PatchWriter {
 public:
  ...
  PatchWriter(
      ShaderFlags shaderFlags, GrMeshDrawTarget* target, const SkRect& strokeCullBounds,
      const SkMatrix& viewMatrix, float matrixMaxScale, GrVertexChunkArray* patchChunks,
      size_t patchStride, int minPatchesPerChunk)
  ...
  bool lineFitsInPatch_withJoin() { return fMaxCombinedSegments_withJoin >= 1; }
  ...
  void updateTolerances(float numRadialSegmentsPerRadian, SkPaint::Join joinType) {
    ...
    float2 numRadialSegments_180_360 = skvx::max(
        skvx::ceil(float2{SK_ScalarPI, 2 * SK_ScalarPI} * fNumRadialSegmentsPerRadian), 1);
    ...
    float2 maxParametricSegments_pow4_withJoin =
        pow4(skvx::max(maxParametricSegments - worstCaseNumSegmentsInJoin - 1, 0));
  ...
  void writeLineTo(SkPoint p0, SkPoint p1) { this->writeLineTo(fStrokeJoinType, p0, p1); }
  ...
  // patches.
  SK_ALWAYS_INLINE void writePatchTo(
      bool prevJoinFitsInPatch, const SkPoint p[4], SkPoint endControlPoint) {
  ...
  void writeClose(SkPoint contourEndpoint, const SkMatrix& viewMatrix, const SkStrokeRec& stroke) {
    if (!fHasLastControlPoint) {
  ...
  void writeCaps(SkPoint contourEndpoint, const SkMatrix& viewMatrix, const SkStrokeRec& stroke) {
    ...
    switch (stroke.getCap()) {
      ...
      case SkPaint::kRound_Cap: {
        ...
        JoinType roundCapJoinType =
            (stroke.getJoin() == SkPaint::kRound_Join) ? JoinType::kRound : JoinType::kBowtie;
        ...
        this->internalJoinTo(
            roundCapJoinType, fCurrContourStartPoint, fCurrContourFirstControlPoint);
        break;
      }
      case SkPaint::kSquare_Cap: {
        ...
        if (!stroke.isHairlineStyle()) {
          ...
          firstTangent *= -.5f / viewMatrix.mapVector(firstTangent.fX, firstTangent.fY).length();
        }
        this->writeLineTo(
            strokeJoinType, fCurrContourStartPoint, fCurrContourStartPoint + firstTangent);
        break;
      }
    }
 ...
 private:
  ...
  // Recursively chops the given conic and its previous join until the segments fit in
  // tessellation patches.
  void internalConicPatchesTo(
      JoinType prevJoinType, const SkPoint p[3], float w, int maxDepth = -1) {
    ...
    if (this->stroke180FitsInPatch(numParametricSegments_pow4) || maxDepth == 0) {
      this->internalPatchTo(
          prevJoinType, this->stroke180FitsInPatch_withJoin(numParametricSegments_pow4), asPatch,
          p[2]);
      return;
    }
    ...
    if (numCombinedSegments > fMaxTessellationSegments) {
      ...
      if (maxDepth < 0) {
        ...
        maxDepth =
            sk_float_nextlog2(numParametricSegments) + sk_float_nextlog2(numRadialSegments) + 1;
      ...
      if (w == 1) {
        ...
        SkConic conic(p, w);
        float chopT = (numParametricSegments >= numRadialSegments) ? .5f : conic.findMidTangent();
        SkConic chops[2];
        if (conic.chopAt(chopT, chops)) {
          this->internalConicPatchesTo(prevJoinType, chops[0].fPts, chops[0].fW, maxDepth - 1);
          this->internalConicPatchesTo(JoinType::kBowtie, chops[1].fPts, chops[1].fW, maxDepth - 1);
        }
      }
      return;
    }
    ...
    this->internalPatchTo(
        prevJoinType, (numCombinedSegments <= fMaxCombinedSegments_withJoin), asPatch, p[2]);
  }
  ...
  void internalCubicConvex180PatchesTo(
      JoinType prevJoinType, const SkPoint p[4], int maxDepth = -1) {
    ...
    if (this->stroke180FitsInPatch(numParametricSegments_pow4) || maxDepth == 0) {
      this->internalPatchTo(
          prevJoinType, this->stroke180FitsInPatch_withJoin(numParametricSegments_pow4), p, p[3]);
      return;
    }
    ...
    this->internalPatchTo(
        prevJoinType, (numCombinedSegments <= fMaxCombinedSegments_withJoin), p, p[3]);
  }

  // Writes out the given stroke patch exactly as provided, without chopping or checking the
  // number of segments. Possibly chops its previous join until the segments fit in tessellation
  // patches. It is valid for prevJoinType to be kBowtie.
  void internalPatchTo(
      JoinType prevJoinType, bool prevJoinFitsInPatch, const SkPoint p[4], SkPoint endPt) {
    if (prevJoinType == JoinType::kBowtie) {
      ...
      if (!SkScalarNearlyEqual(
              ab_pow2, ab_cosTheta * fabsf(ab_cosTheta), ab_pow2 * SK_ScalarNearlyZero)) {
  ...
  void internalJoinTo(
      JoinType joinType, SkPoint junctionPoint, SkPoint nextControlPoint, int maxDepth = -1) {
    if (!fHasLastControlPoint) {
...
GrStrokeHardwareTessellator::GrStrokeHardwareTessellator(
    const GrShaderCaps& shaderCaps, ShaderFlags shaderFlags, const SkMatrix& viewMatrix,
    PathStrokeList* pathStrokeList, std::array<float, 2> matrixMinMaxScales,
    const SkRect& strokeCullBounds)
    : GrStrokeTessellator(
          shaderCaps, GrStrokeTessellationShader::Mode::kHardwareTessellation, shaderFlags,
          SkNextLog2(shaderCaps.maxTessellationSegments()), viewMatrix, pathStrokeList,
...
void GrStrokeHardwareTessellator::prepare(GrMeshDrawTarget* target, int totalCombinedVerbCnt) {
  ...
  PatchWriter patchWriter(
      fShader.flags(), target, fStrokeCullBounds, fShader.viewMatrix(), fMatrixMinMaxScales[1],
  ...
  if (!fShader.hasDynamicStroke()) {
    ...
    float localStrokeWidth =
        GrStrokeTolerances::GetLocalStrokeWidth(fMatrixMinMaxScales.data(), stroke.getWidth());
  ...
  for (PathStrokeList* pathStroke = fPathStrokeList; pathStroke; pathStroke = pathStroke->fNext) {
    ...
    if (fShader.hasDynamicStroke()) {
      ...
      patchWriter.updateTolerances(
          toleranceBuffer.fetchRadialSegmentsPerRadian(pathStroke), stroke.getJoin());
    ...
    for (auto [verb, p, w] : SkPathPriv::Iterate(path)) {
      ...
      switch (verb) {
        ...
        case SkPathVerb::kQuad: {
          ...
          prevJoinFitsInPatch =
              patchWriter.stroke180FitsInPatch_withJoin(numParametricSegments_pow4);
        ...
        case SkPathVerb::kCubic: {
          ...
          if (!patchWriter.stroke360FitsInPatch(numParametricSegments_pow4) || cubic_has_cusp(p)) {
          ...
          // Write it out directly.
          prevJoinFitsInPatch =
              patchWriter.stroke360FitsInPatch_withJoin(numParametricSegments_pow4);
          patchPts = p;
