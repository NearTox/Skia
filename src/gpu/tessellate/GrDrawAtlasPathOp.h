...
class GrDrawAtlasPathOp : public GrDrawOp {
 public:
  ...
  GrDrawAtlasPathOp(
      ...
      const SkIPoint16& locationInAtlas, bool transposedInAtlas, const SkMatrix& viewMatrix,
      GrPaint&& paint)
      : GrDrawOp(ClassID()),
        fEnableHWAA(numRenderTargetSamples > 1),
        fAtlasProxy(std::move(atlasProxy)),
        fInstanceList(
            devIBounds, locationInAtlas, transposedInAtlas, paint.getColor4f(), viewMatrix),
 ...
 private:
  ...
  struct InstanceList {
    InstanceList(
        const SkIRect& devIBounds, const SkIPoint16& locationInAtlas, bool transposedInAtlas,
