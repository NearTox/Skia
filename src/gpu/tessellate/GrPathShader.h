...
class GrPathShader : public GrGeometryProcessor {
 public:
  ...
  void issueDraw(
      GrOpFlushState* state, const GrPipeline* pipeline,
      const GrPipeline::FixedDynamicState* fixedDynamicState, sk_sp<const GrBuffer> vertexBuffer,
      int vertexCount, int baseVertex, const SkRect& bounds) {
  ...
  void issueDraw(
    ...
    GrProgramInfo programInfo(
        ...
        state->proxy()->backendFormat(), state->view()->origin(), pipeline, this, fixedDynamicState,
        nullptr, 0, fPrimitiveType, fTessellationPatchVertexCount);
