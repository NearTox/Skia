...
class GrPathShader : public GrGeometryProcessor {
 public:
  ...
  static GrProgramInfo* MakeProgramInfo(
      ...
      const GrUserStencilSettings* stencil, const GrCaps& caps) {
    ...
    return MakeProgramInfo(
        shader, arena, writeView, pipeline, dstProxyView, renderPassXferBarriers, colorLoadOp,
        stencil, caps);
  }
  ...
  static GrProgramInfo* MakeProgramInfo(
    ...
    return arena->make<GrProgramInfo>(
        writeView, pipeline, stencil, shader, shader->fPrimitiveType,
        shader->fTessellationPatchVertexCount, renderPassXferBarriers, colorLoadOp);
  }
