...
class DrawAtlasPathShader::Impl : public GrGLSLGeometryProcessor {
  void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
    ...
    args.fVaryingHandler->addPassThroughAttribute(
        kInstanceAttribs[2], args.fOutputColor, GrGLSLVaryingHandler::Interpolation::kCanBeFlat);
    ...
    this->emitTransforms(
        args.fVertBuilder, args.fVaryingHandler, args.fUniformHandler, localCoord,
...
GrProcessorSet::Analysis GrDrawAtlasPathOp::finalize(
...
void GrDrawAtlasPathOp::onExecute(GrOpFlushState* state, const SkRect& chainBounds) {
  ...
  GrSwizzle swizzle =
      state->caps().getReadSwizzle(fAtlasProxy->backendFormat(), GrColorType::kAlpha_8);
  ...
  GrProgramInfo programInfo(
      ...
      state->proxy()->backendFormat(), state->outputView()->origin(), &pipeline, &shader, nullptr,
