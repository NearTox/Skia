...
class GrMiddleOutPolygonTriangulator {
 public:
  ...
  GrMiddleOutPolygonTriangulator(
      GrVertexWriter&& vertexWriter, int pad32Count, uint32_t pad32Value, int maxPushVertexCalls)
      : fVertexWriter(std::move(vertexWriter)), fPad32Count(pad32Count), fPad32Value(pad32Value) {
  ...
  static GrVertexWriter WritePathInnerFan(
      GrVertexWriter&& vertexWriter, int pad32Count, uint32_t pad32Value,
      const GrPathXform& pathXform, const SkPath& path, int* numTrianglesWritten) {
    GrMiddleOutPolygonTriangulator middleOut(
        std::move(vertexWriter), pad32Count, pad32Value, path.countVerbs());
    for (auto [verb, pts, w] : SkPathPriv::Iterate(path)) {
      switch (verb) {
        SkPoint pt;
        case SkPathVerb::kMove: middleOut.closeAndMove(pathXform.mapPoint(pts[0])); break;
