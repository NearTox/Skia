...
class VectorXform {
 public:
  ...
  AI skvx::float2 operator()(skvx::float2 vector) const {
    switch (fType) {
      case Type::kIdentity: return vector;
      case Type::kScale: return fScaleXY * vector;
      case Type::kAffine: return fScaleXSkewY * skvx::float2(vector[0]) + fSkewXScaleY * vector[1];
    }
    SkUNREACHABLE;
  }
  AI skvx::float4 operator()(skvx::float4 vectors) const {
    switch (fType) {
      case Type::kIdentity: return vectors;
      case Type::kScale: return vectors * fScaleXYXY;
      case Type::kAffine: return fScaleXYXY * vectors + fSkewXYXY * vectors.yxwz();
    }
    SkUNREACHABLE;
  }
 ...
 private:
  ...
  union {
    skvx::float2 fScaleXY, fScaleXSkewY;
  };
...
AI float quadratic_p4(
    float precision, skvx::float2 p0, skvx::float2 p1, skvx::float2 p2,
...
AI float quadratic_p4(
    float precision, const SkPoint pts[], const VectorXform& vectorXform = VectorXform()) {
  return quadratic_p4(
      precision, skvx::bit_pun<skvx::float2>(pts[0]), skvx::bit_pun<skvx::float2>(pts[1]),
      skvx::bit_pun<skvx::float2>(pts[2]), vectorXform);
}
...
AI float cubic_p4(
    float precision, skvx::float2 p0, skvx::float2 p1, skvx::float2 p2, skvx::float2 p3,
    const VectorXform& vectorXform = VectorXform()) {
...
AI float cubic_p4(
  ...
  return cubic_p4(
      precision, skvx::bit_pun<skvx::float2>(pts[0]), skvx::bit_pun<skvx::float2>(pts[1]),
      skvx::bit_pun<skvx::float2>(pts[2]), skvx::bit_pun<skvx::float2>(pts[3]), vectorXform);
}
...
AI float conic_p2(
    float precision, skvx::float2 p0, skvx::float2 p1, skvx::float2 p2, float w,
    const VectorXform& vectorXform = VectorXform()) {
  p0 = vectorXform(p0);
  p1 = vectorXform(p1);
  p2 = vectorXform(p2);
...
AI float conic_p2(
    float precision, const SkPoint pts[], float w, const VectorXform& vectorXform = VectorXform()) {
  return conic_p2(
      precision, skvx::bit_pun<skvx::float2>(pts[0]), skvx::bit_pun<skvx::float2>(pts[1]),
      skvx::bit_pun<skvx::float2>(pts[2]), w, vectorXform);
}

// Returns the value of Wang's formula specialized for a conic curve.
AI float conic(
    float tolerance, const SkPoint pts[], float w, const VectorXform& vectorXform = VectorXform()) {
