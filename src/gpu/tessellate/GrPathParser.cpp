...
static SkPoint write_quadratic_as_cubic(
    SkPoint* data, const SkPoint& p0, const SkPoint& p1, const SkPoint& p2) {
  data[0] = p0;
...
static SkPoint write_cubic(
    SkPoint* data, const SkPoint& p0, const SkPoint& p1, const SkPoint& p2, const SkPoint& p3) {
  data[0] = p0;
  data[1] = p1;
  data[2] = p2;
  data[3] = p3;
...
class MidpointContourParser : public SkTPathContourParser<MidpointContourParser> {
 public:
  ...
  SkPoint midpoint() const {
    SkASSERT(1 == fMidpointWeight);
    return fMidpoint;
  }
      ...
      switch (parser.atVerb(i)) {
        ...
        case SkPathVerb::kDone:
          if (parser.startPoint() != lastPoint) {
            lastPoint =
                write_line_as_cubic(vertexData + vertexCount, lastPoint, parser.startPoint());
            break;
          }  // fallthru
        default: continue;

        case SkPathVerb::kConic: SK_ABORT("Conics are not yet supported."); continue;

        case SkPathVerb::kLine:
          lastPoint =
              write_line_as_cubic(vertexData + vertexCount, lastPoint, parser.atPoint(ptsIdx));
          ++ptsIdx;
          break;
        case SkPathVerb::kQuad:
          lastPoint = write_quadratic_as_cubic(
              vertexData + vertexCount, lastPoint, parser.atPoint(ptsIdx),
              parser.atPoint(ptsIdx + 1));
          ptsIdx += 2;
          break;
        case SkPathVerb::kCubic:
          lastPoint = write_cubic(
              vertexData + vertexCount, lastPoint, parser.atPoint(ptsIdx),
              parser.atPoint(ptsIdx + 1), parser.atPoint(ptsIdx + 2));
          ptsIdx += 3;
          break;
      }
...
class InnerPolygonContourParser : public SkTPathContourParser<InnerPolygonContourParser> {
 public:
  InnerPolygonContourParser(const SkPath& path, int vertexReserveCount)
      : SkTPathContourParser(path), fPolyPoints(vertexReserveCount) {}
...
int GrPathParser::EmitInnerPolygonTriangles(
    const SkPath& path, GrEagerVertexAllocator* vertexAlloc) {
