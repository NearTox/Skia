...
SkPoint* GrStrokePatchBuilder::reservePatch() {
  ...
  SkASSERT(
      fVertexChunkArray->back().fVertexCount + kNumVerticesPerPatch <= fCurrChunkVertexCapacity);
...
void GrStrokePatchBuilder::writeCubicSegment(
    float leftJoinType, const SkPoint pts[4], float overrideNumSegments) {
...
void GrStrokePatchBuilder::writeJoin(
    float joinType, const SkPoint& anchorPoint, const SkPoint& prevControlPoint,
...
void GrStrokePatchBuilder::writeCaps() {
  ...
  switch (fCurrStrokeCapType) {
    ...
    case SkPaint::kRound_Cap:
      ...
      this->writeJoin(
          GrStrokeTessellateShader::kRoundJoinType, fCurrContourStartPoint,
          fCurrContourFirstControlPoint, fCurrContourFirstControlPoint);
      this->writeJoin(
          GrStrokeTessellateShader::kRoundJoinType, fCurrentPoint, fLastControlPoint,
          fLastControlPoint);
      break;
  ...
  for (auto [verb, rawPts, w] : SkPathPriv::Iterate(path)) {
    ...
    switch (verb) {
      ...
      case SkPathVerb::kCubic:
        ...
        this->cubicTo(pts);
        break;
      case SkPathVerb::kConic: SkASSERT(previousVerb != SkPathVerb::kClose); SkUNREACHABLE;
    }
    previousVerb = verb;
  }
  if (previousVerb != SkPathVerb::kMove && previousVerb != SkPathVerb::kClose) {
...
static float join_type_from_join(SkPaint::Join join) {
  switch (join) {
    case SkPaint::kBevel_Join: return GrStrokeTessellateShader::kBevelJoinType;
    case SkPaint::kMiter_Join: return GrStrokeTessellateShader::kMiterJoinType;
    case SkPaint::kRound_Join: return GrStrokeTessellateShader::kRoundJoinType;
  }
  SkUNREACHABLE;
}
...
void GrStrokePatchBuilder::quadraticTo(
    float leftJoinType, const SkPoint P[3], float maxCurvatureT) {
  if (P[1] == P[0] || P[1] == P[2]) {
  ...
  if (numSegments > 1 && calc_curvature_costheta(leftTan, rightTan) < fMaxCurvatureCosTheta) {
    ...
    if (rightT < 1) {
      ...
      this->rotateTo(
          kInternalRoundJoinType, currQuadratic[2], currQuadratic[2] * 2 - currQuadratic[1]);
    }
    return;
  }
...
void GrStrokePatchBuilder::cubicTo(const SkPoint P[4]) {
  ...
  this->cubicTo(
      fCurrStrokeJoinType, P, numRoots > 0 ? roots[numRoots / 2] : 0,
      numRoots > 1 ? roots[0] : kLeftMaxCurvatureNone,
      numRoots > 2 ? roots[2] : kRightMaxCurvatureNone);
}

void GrStrokePatchBuilder::cubicTo(
    float leftJoinType, const SkPoint P[4], float maxCurvatureT, float leftMaxCurvatureT,
  ...
  if (numSegments > 1 && calc_curvature_costheta(leftTan, rightTan) < fMaxCurvatureCosTheta) {
    ...
    if (leftT > 0) {
      ...
      this->cubicTo(
          leftJoinType, ptsBuffer, /*maxCurvatureT=*/1,
    ...
    if (rightT < 1) {
      ...
      currCubic = ptsBuffer + 3;
      this->cubicTo(
          kInternalRoundJoinType, currCubic, /*maxCurvatureT=*/0, kLeftMaxCurvatureNone,
          kRightMaxCurvatureNone);
    } else {
  ...
  if (kRightMaxCurvatureNone != rightMaxCurvatureT) {
    this->cubicTo(leftJoinType, P, rightMaxCurvatureT, leftMaxCurvatureT, kRightMaxCurvatureNone);
    return;
  }
  if (kLeftMaxCurvatureNone != leftMaxCurvatureT) {
    SkASSERT(kRightMaxCurvatureNone == rightMaxCurvatureT);
    this->cubicTo(
        leftJoinType, P, leftMaxCurvatureT, kLeftMaxCurvatureNone, kRightMaxCurvatureNone);
    return;
  }
  if (numSegments > fMaxTessellationSegments) {
    SkPoint ptsBuffer[7];
    ...
    this->cubicTo(
        kInternalRoundJoinType, ptsBuffer + 3, 0, kLeftMaxCurvatureNone, kRightMaxCurvatureNone);
    return;
  }
...
void GrStrokePatchBuilder::rotateTo(
    float leftJoinType, const SkPoint& anchorPoint, const SkPoint& controlPoint) {
...
void GrStrokePatchBuilder::close() {
  ...
  this->writeJoin(
      fCurrStrokeJoinType, fCurrContourStartPoint, fLastControlPoint,
