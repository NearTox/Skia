...
struct PatchStorage {
  ...
  bool hasPending() const { return fN_p4 >= 0.f; }
  void reset() {
    fN_p4 = -1.f;
    fMustDefer = true;
  }
...
#define DEF_ATTRIB_TYPE(name, A, T)                             \
  ...
  using Color = std::conditional_t<
      kRequireWideColor, SkPMColor4f,
      std::conditional_t<
          kOptionalWideColor, VertexColor,
          /* else */ uint32_t>>;
...
#undef DEF_ATTRIB_TYPE
  ...
  static constexpr size_t kMaxStride =
      ...
      (DepthAttrib::kEnabled ? sizeof(float) : 0) + (CurveTypeAttrib::kEnabled ? sizeof(float) : 0);

  // Types that vary depending on the activated features, but do not define the patch data.
  using DeferredPatch =
      std::conditional_t<kTrackJoinControlPoints, PatchStorage<kMaxStride>, std::monostate>;
  using InnerTriangulator =
      std::conditional_t<kAddTrianglesWhenChopping, MiddleOutPolygonTriangulator, NullTriangulator>;

  using float2 = skvx::float2;
  using float4 = skvx::float4;

  static_assert(
      !kTrackJoinControlPoints || req_attrib<PatchAttribs::kJoinControlPoint>::value,
 ...
 public:
  ...
  ENABLE_IF(kTrackJoinControlPoints) writeDeferredStrokePatch() {
    if (fDeferredPatch.hasPending()) {
      ...
      memcpy(
          SkTAddOffset<void>(fDeferredPatch.fData, 4 * sizeof(SkPoint)), &fJoin, sizeof(SkPoint));
  ...
  AI void writeConic(const SkPoint pts[3], float w) {
    this->writeConic(
        skvx::bit_pun<float2>(pts[0]), skvx::bit_pun<float2>(pts[1]), skvx::bit_pun<float2>(pts[2]),
        w);
  }
  ...
  AI void writeQuadratic(const SkPoint pts[3]) {
    this->writeQuadratic(
        skvx::bit_pun<float2>(pts[0]), skvx::bit_pun<float2>(pts[1]),
  ...
  AI void writeTriangle(float2 p0, float2 p1, float2 p2) {
    ...
    this->writePatch(p0, p1, p2, {SK_FloatInfinity, SK_FloatInfinity}, kTriangularConicCurveType);
  }
  AI void writeTriangle(SkPoint p0, SkPoint p1, SkPoint p2) {
    this->writeTriangle(
        skvx::bit_pun<float2>(p0), skvx::bit_pun<float2>(p1), skvx::bit_pun<float2>(p2));
  }
 ...
 private:
  AI void emitPatchAttribs(
      VertexWriter vertexWriter, const JoinAttrib& join, float explicitCurveType) {
  ...
  int accountForCurve(float n4) {
    if (n4 <= kMaxParametricSegments_p4) {
      ...
      return SkScalarCeilToInt(
          wangs_formula::root4(std::min(n4, kMaxSegmentsPerCurve_p4) / kMaxParametricSegments_p4));
    }
  }
  ...
  void chopAndWriteConics(float2 p0, float2 p1, float2 p2, float w, int numPatches) {
    ...
    for (; numPatches >= 2; --numPatches) {
      ...
      this->writeConicPatch(
          h0.xy() / h0.w(), ab.xy() / ab.w(), midpoint, ab.w() / sqrtf(h0.w() * abc.w()));
      if constexpr (kAddTrianglesWhenChopping) {
