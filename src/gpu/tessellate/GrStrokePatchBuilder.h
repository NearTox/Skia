...
class GrStrokePatchBuilder {
 public:
  ...
  GrStrokePatchBuilder(
      GrMeshDrawOp::Target* target, SkTArray<VertexChunk>* vertexChunkArray, float matrixScale,
  ...
  // vertices.
  ~GrStrokePatchBuilder() {
    fTarget->putBackVertices(
        fCurrChunkVertexCapacity - fVertexChunkArray->back().fVertexCount, sizeof(SkPoint));
  }
 ...
 private:
  ...
  void writeCubicSegment(
      float leftJoinType, const Sk2f& p0, const Sk2f& p1, const Sk2f& p2, const Sk2f& p3,
  ...
  void cubicTo(
      float leftJoinType, const SkPoint[4], float maxCurvatureT, float leftMaxCurvatureT,
