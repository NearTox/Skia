 ...
 public:
  MiddleOutShader(
      ...
      : GrPathTessellationShader(
            kTessellate_MiddleOutShader_ClassID, GrPrimitiveType::kTriangles, 0, viewMatrix, color),
    ...
    constexpr static Attribute kVertexAttrib(
        "resolveLevel_and_idx", kFloat2_GrVertexAttribType, kFloat2_GrSLType);
...
void GrPathTessellationShader::InitializeVertexBufferForMiddleOutCurves(
    GrVertexWriter vertexWriter, size_t bufferSize) {
...
void GrPathTessellationShader::InitializeVertexBufferForMiddleOutWedges(
    GrVertexWriter vertexWriter, size_t bufferSize) {
  ...
  InitializeVertexBufferForMiddleOutCurves(
      std::move(vertexWriter), bufferSize - kMiddleOutVertexStride);
}

static void fill_index_buffer_for_curves(
    GrVertexWriter vertexWriter, size_t bufferSize, uint16_t baseIndex) {
  ...
  const auto* neighborInLastResolveLevel =
      &indexData.push_back({baseIndex, (uint16_t)(baseIndex + 2), (uint16_t)(baseIndex + 1)});

  // Resolve levels 2..maxResolveLevel
  int maxResolveLevel = SkPrevLog2(triangleCount + 1);
  uint16_t nextIndex = baseIndex + 3;
  SkASSERT(
      GrPathTessellationShader::NumCurveTrianglesAtResolveLevel(maxResolveLevel) == triangleCount);
  for (int resolveLevel = 2; resolveLevel <= maxResolveLevel; ++resolveLevel) {
    ...
    for (int i = 0; i < numTrianglePairsInResolveLevel; ++i) {
      ...
      indexData.push_back(
          {(*neighborInLastResolveLevel)[0], nextIndex++, (*neighborInLastResolveLevel)[1]});
      // Second triangle shares the right edge of "neighborInLastResolveLevel".
      indexData.push_back(
          {(*neighborInLastResolveLevel)[1], nextIndex++, (*neighborInLastResolveLevel)[2]});
...
void GrPathTessellationShader::InitializeIndexBufferForMiddleOutCurves(
    GrVertexWriter vertexWriter, size_t bufferSize) {
  fill_index_buffer_for_curves(std::move(vertexWriter), bufferSize, 0);
}

void GrPathTessellationShader::InitializeIndexBufferForMiddleOutWedges(
    GrVertexWriter vertexWriter, size_t bufferSize) {
