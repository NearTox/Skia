...
class GrPathTessellationShader : public GrTessellationShader {
 public:
  ...
  static GrPathTessellationShader* MakeSimpleTriangleShader(
      SkArenaAlloc*, const SkMatrix& viewMatrix, const SkPMColor4f&);
  ...
  static GrPathTessellationShader* MakeMiddleOutFixedCountShader(
      const GrShaderCaps&, SkArenaAlloc*, const SkMatrix& viewMatrix, const SkPMColor4f&,
      PatchType);
  ...
  constexpr static size_t SizeOfIndexBufferForMiddleOutCurves() {
    constexpr int kMaxTriangleCount = NumCurveTrianglesAtResolveLevel(kMaxFixedCountResolveLevel);
  ...
  static GrPathTessellationShader* MakeHardwareTessellationShader(
      SkArenaAlloc*, const SkMatrix& viewMatrix, const SkPMColor4f&, PatchType);
  ...
  static const GrUserStencilSettings* TestAndResetStencilSettings(bool isInverseFill = false) {
    constexpr static GrUserStencilSettings kTestAndResetStencil(
        GrUserStencilSettings::StaticInit<
            ...
            GrUserStencilTest::kNotEqual, 0xffff, GrUserStencilOp::kZero, GrUserStencilOp::kKeep,
            0xffff>());
    ...
    constexpr static GrUserStencilSettings kTestAndResetStencilInverted(
        GrUserStencilSettings::StaticInit<
            ...
            // No need to check the clip because the previous stencil pass will have only
            // written to samples already inside the clip.
            GrUserStencilTest::kEqual, 0xffff, GrUserStencilOp::kKeep, GrUserStencilOp::kZero,
            0xffff>());
  ...
  static const GrPipeline* MakeStencilOnlyPipeline(
      const ProgramArgs&, GrAAType, GrTessellationPathFlags, const GrAppliedHardClip&);

 protected:
  constexpr static size_t kMiddleOutVertexStride = 2 * sizeof(float);

  GrPathTessellationShader(
      ClassID classID, GrPrimitiveType primitiveType, int tessellationPatchVertexCount,
      const SkMatrix& viewMatrix, const SkPMColor4f& color)
      : GrTessellationShader(
            classID, primitiveType, tessellationPatchVertexCount, viewMatrix, color) {}
  ...
  class Impl : public ProgramImpl {
   ...
   protected:
    ...
    virtual void emitVertexCode(
        const GrShaderCaps&, const GrPathTessellationShader&, GrGLSLVertexBuilder*, GrGPArgs*) = 0;
