...
void GrStrokeTessellationShader::HardwareImpl::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
  ...
  if (!shader.hasDynamicStroke()) {
    ...
    fTessControlArgsUniform = uniHandler->addUniform(
        nullptr, kVertex_GrShaderFlag | kTessControl_GrShaderFlag | kTessEvaluation_GrShaderFlag,
    ...
    fTessControlArgsUniform = uniHandler->addUniform(
        nullptr, kVertex_GrShaderFlag | kTessControl_GrShaderFlag | kTessEvaluation_GrShaderFlag,
        kFloat_GrSLType, "parametricPrecision", &parametricPrecisionName);
    v->codeAppendf(
        R"(
  ...
  if (shader.hasDynamicColor()) {
    ...
    args.fFragBuilder->declareGlobal(
        GrShaderVar(fDynamicColorName, kHalf4_GrSLType, TypeModifier::In, 0, SkString(), flatness));
  }
  this->emitFragmentCode(shader, args);
}

SkString GrStrokeTessellationShader::HardwareImpl::getTessControlShaderGLSL(
    const GrGeometryProcessor& geomProc, const char* versionAndExtensionDecls,
