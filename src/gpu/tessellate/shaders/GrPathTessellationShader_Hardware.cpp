 ...
 public:
  HardwareWedgeShader(const SkMatrix& viewMatrix, const SkPMColor4f& color)
      : GrPathTessellationShader(
            kTessellate_HardwareWedgeShader_ClassID, GrPrimitiveType::kPatches, 5, viewMatrix,
            color) {
...
std::unique_ptr<GrGeometryProcessor::ProgramImpl> HardwareWedgeShader::makeProgramImpl(
  ...
  class Impl : public GrPathTessellationShader::Impl {
    ...
    SkString getTessControlShaderGLSL(
        ...
        const GrGLSLUniformHandler&, const GrShaderCaps& shaderCaps) const override {
      SkString code(versionAndExtensionDecls);
      code.appendf(
          R"(
            #define MAX_TESSELLATION_SEGMENTS %i)",
...
class HardwareCurveShader : public GrPathTessellationShader {
 public:
  HardwareCurveShader(const SkMatrix& viewMatrix, const SkPMColor4f& color)
      : GrPathTessellationShader(
            kTessellate_HardwareCurveShader_ClassID, GrPrimitiveType::kPatches, 4, viewMatrix,
            color) {
    constexpr static Attribute kInputPointAttrib{
        "inputPoint", kFloat2_GrVertexAttribType, kFloat2_GrSLType};
    this->setVertexAttributes(&kInputPointAttrib, 1);
...
GrPathTessellationShader* GrPathTessellationShader::MakeHardwareTessellationShader(
    ...
    PatchType patchType) {
  switch (patchType) {
    case PatchType::kWedges: return arena->make<HardwareWedgeShader>(viewMatrix, color);
    case PatchType::kCurves: return arena->make<HardwareCurveShader>(viewMatrix, color);
  }
  SkUNREACHABLE;
}
