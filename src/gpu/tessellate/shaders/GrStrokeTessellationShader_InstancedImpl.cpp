...
void GrStrokeTessellationShader::InstancedImpl::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
  ...
  if (!shader.hasDynamicStroke()) {
    ...
    args.fVertBuilder->codeAppendf(
        ...
        float JOIN_TYPE = %s.z;
        float STROKE_RADIUS = %s.w;)",
        tessArgsName, tessArgsName, tessArgsName, tessArgsName);
  } else {
  ...
  if (shader.stroke().getJoin() == SkPaint::kRound_Join || shader.hasDynamicStroke()) {
    ...
    args.fVertBuilder->codeAppendf(
        ...
        GrStrokeTessellationShader::NumFixedEdgesInJoin(joinType));
  }
...
void GrStrokeTessellationShader::InitializeVertexIDFallbackBuffer(
