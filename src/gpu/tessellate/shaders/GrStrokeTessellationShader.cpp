...
GrStrokeTessellationShader::GrStrokeTessellationShader(
    const GrShaderCaps& shaderCaps, Mode mode, ShaderFlags shaderFlags, const SkMatrix& viewMatrix,
    ...
    : GrTessellationShader(
          ...
          (mode == Mode::kHardwareTessellation) ? GrPrimitiveType::kPatches
  ...
  if (fShaderFlags & ShaderFlags::kDynamicStroke) {
    fAttribs.emplace_back("dynamicStrokeAttr", kFloat2_GrVertexAttribType, kFloat2_GrSLType);
  }
  if (fShaderFlags & ShaderFlags::kDynamicColor) {
    fAttribs.emplace_back(
        "dynamicColorAttr",
        (fShaderFlags & ShaderFlags::kWideColor) ? kFloat4_GrVertexAttribType
  ...
  if (fMode == Mode::kHardwareTessellation) {
    ...
    if (!shaderCaps.vertexIDSupport()) {
      constexpr static Attribute kVertexAttrib(
          "edgeID", kFloat_GrVertexAttribType, kFloat_GrSLType);
...
void GrStrokeTessellationShader::Impl::setData(
  ...
  if (!shader.hasDynamicStroke()) {
    ...
    if (!stroke.isHairlineStyle()) {
      tolerances =
          GrStrokeTolerances::MakeNonHairline(shader.viewMatrix().getMaxScale(), stroke.getWidth());
    } else {
    ...
    pdman.set1f(fTessControlArgsUniform, GrStrokeTolerances::CalcParametricPrecision(maxScale));
  }
...
void GrStrokeTessellationShader::addToKey(const GrShaderCaps&, GrProcessorKeyBuilder* b) const {
  bool keyNeedsJoin =
      (fMode != Mode::kHardwareTessellation) && !(fShaderFlags & ShaderFlags::kDynamicStroke);
...
std::unique_ptr<GrGeometryProcessor::ProgramImpl> GrStrokeTessellationShader::makeProgramImpl(
  ...
  switch (fMode) {
    case Mode::kHardwareTessellation: return std::make_unique<HardwareImpl>();
    case Mode::kLog2Indirect:
    case Mode::kFixedCount: return std::make_unique<InstancedImpl>();
  }
  SkUNREACHABLE;
}
