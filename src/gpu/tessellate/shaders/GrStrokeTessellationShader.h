...
class GrStrokeTessellationShader : public GrTessellationShader {
 public:
  ...
  enum class Mode : int8_t { kHardwareTessellation, kLog2Indirect, kFixedCount };
  ...
  GrStrokeTessellationShader(
      const GrShaderCaps&, Mode, ShaderFlags, const SkMatrix& viewMatrix, const SkStrokeRec&,
      SkPMColor4f, int8_t maxParametricSegments_log2);
 ...
 private:
  const char* name() const override {
    switch (fMode) {
      case Mode::kHardwareTessellation: return "GrStrokeTessellationShader_HardwareImpl";
      case Mode::kLog2Indirect:
      case Mode::kFixedCount: return "GrStrokeTessellationShader_InstancedImpl";
    }
    SkUNREACHABLE;
  }
