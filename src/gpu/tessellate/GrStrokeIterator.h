...
class GrStrokeIterator {
 ...
 private:
  ...
  Verb backVerb() const { return this->atVerb(fQueueCount - 1); }
  const SkPoint* atPts(int i) const {
    SkASSERT(0 <= i && i < fQueueCount);
    return fPts[(fQueueFrontIdx + i) & (kQueueBufferCount - 1)];
  }
  const SkPoint* backPts() const { return this->atPts(fQueueCount - 1); }
  ...
  bool finishOpenContour() {
    if (fQueueCount) {
      SkASSERT(
          this->backVerb() == Verb::kLine || this->backVerb() == Verb::kQuad ||
      ...
      switch (fStroke->getCap()) {
        ...
        case SkPaint::kSquare_Cap: {
          ...
          fEndingCapPts = {*fLastDegenerateStrokePt - outset, *fLastDegenerateStrokePt + outset};
  ...
  void fillSquareCapPoints() {
    ...
    if (firstTangent.isZero()) {
      SkASSERT(
          fFirstVerbInContour == Verb::kQuad || fFirstVerbInContour == Verb::kConic ||
