...
bool GrPathTessellateOp::prePrepareInnerPolygonTriangulation(
    const PrePrepareArgs& args, int* numCountedCurves) {
  ...
  fTriangleVertexCount = GrTriangulator::PathToTriangles(
      fPath, 0, SkRect::MakeEmpty(), args.fInnerTriangleAllocator, Mode::kSimpleInnerPolygons,
      numCountedCurves);
...
void GrPathTessellateOp::prePrepareFillTrianglesProgram(
    const PrePrepareArgs& args, int numCountedCurves) {
  SkASSERT(!fFillTrianglesProgram);
  ...
  fFillTrianglesProgram = GrPathShader::MakeProgramInfo(
      ...
      args.fXferBarrierFlags, stencil, *args.fCaps);
}
...
void GrPathTessellateOp::prePrepareFillCubicHullsProgram(const PrePrepareArgs& args) {
  ...
  fFillPathProgram = GrPathShader::MakeProgramInfo(
      ...
      args.fXferBarrierFlags, &kTestAndResetStencil, *args.fCaps);
}
    ...
    switch (verb) {
      ...
      case SkPathVerb::kQuad:
        ...
        if (resolveLevelCounter) {
          ...
          resolveLevelCounter->countCubic(
      ...
      case SkPathVerb::kConic: SkUNREACHABLE;
    }
  }
...
void GrPathTessellateOp::prepareIndirectOuterCubicsAndTriangles(
  ...
  SkASSERT(
      ...
      numTrianglesAtBeginningOfData + resolveLevelCounter.totalCubicInstanceCount());
  ...
  SkASSERT(
      fCubicVertexCount ==
      (numTrianglesAtBeginningOfData + resolveLevelCounter.totalCubicInstanceCount()) * 4);
#endif
}
