...
void GrPathTessellateOp::onPrepare(GrOpFlushState* flushState) {
  ...
  if (drawTrianglesAsIndirectCubicDraw || (fOpFlags & OpFlags::kDisableHWTessellation)) {
    ...
    GrResolveLevelCounter resolveLevelCounter;
    this->prepareMiddleOutTrianglesAndCubics(
        flushState, &resolveLevelCounter, drawTrianglesAsIndirectCubicDraw);
    return;
  }
...
bool GrPathTessellateOp::prepareNonOverlappingInnerTriangles(
  ...
  fTriangleVertexCount = GrTriangulator::PathToTriangles(
      fPath, 0, SkRect::MakeEmpty(), &vertexAlloc, Mode::kSimpleInnerPolygons, numCountedCurves);
...
void GrPathTessellateOp::prepareIndirectOuterCubicsAndTriangles(
  ...
  for (int resolveLevel = 1; resolveLevel <= kMaxResolveLevel; ++resolveLevel) {
    ...
    if (!instanceCountAtCurrLevel) {
      SkDEBUGCODE(instanceLocations[resolveLevel] = nullptr;) continue;
    }
...
void GrPathTessellateOp::drawStencilPass(GrOpFlushState* flushState) {
  ...
  GrPipeline pipeline(
      initArgs, GrDisableColorXPFactory::MakeXferProcessor(), flushState->appliedHardClip());

  if (fDoStencilTriangleBuffer) {
    SkASSERT(fTriangleBuffer);
    GrStencilTriangleShader stencilTriangleShader(fViewMatrix);
    GrPathShader::ProgramInfo programInfo(
        flushState->writeView(), &pipeline, &stencilTriangleShader);
  ...
  if (fStencilCubicsShader) {
    SkASSERT(fCubicBuffer);
    GrPathShader::ProgramInfo programInfo(flushState->writeView(), &pipeline, fStencilCubicsShader);
    flushState->bindPipelineAndScissorClip(programInfo, this->bounds());
...
void GrPathTessellateOp::drawCoverPass(GrOpFlushState* flushState) {
  ...
  if (fDoFillTriangleBuffer) {
    ...
    GrPathShader::ProgramInfo programInfo(flushState->writeView(), &pipeline, &fillTriangleShader);
    flushState->bindPipelineAndScissorClip(programInfo, this->bounds());
    ...
    if (fStencilCubicsShader) {
      ...
      GrPathShader::ProgramInfo programInfo(
          flushState->writeView(), &pipeline, &fillCubicHullShader);
      flushState->bindPipelineAndScissorClip(programInfo, this->bounds());
  ...
  GrPathShader::ProgramInfo programInfo(flushState->writeView(), &pipeline, &fillBoundingBoxShader);
  flushState->bindPipelineAndScissorClip(programInfo, this->bounds());
