...
void GrPathTessellateOp::onPrePrepare(
  ...
  GrAppliedHardClip hardClip =
      GrAppliedHardClip((clip) ? clip->hardClip() : GrAppliedHardClip::Disabled());
...
bool GrPathTessellateOp::prePrepareInnerPolygonTriangulation(
    const PrePrepareArgs& args, bool* isLinear) {
  ...
  fTriangleVertexCount =
      GrTriangulator::TriangulateSimpleInnerPolygons(fPath, args.fInnerTriangleAllocator, isLinear);
  ...
  if ((fOpFlags & (OpFlags::kStencilOnly | OpFlags::kWireframe)) ||
      GrAAType::kCoverage == fAAType || (args.fClip && args.fClip->hasStencilClip())) {
...
constexpr static GrUserStencilSettings kIncrDecrStencil(
    GrUserStencilSettings::StaticInitSeparate<
        0x0000, 0x0000, GrUserStencilTest::kAlwaysIfInClip, GrUserStencilTest::kAlwaysIfInClip,
        0xffff, 0xffff, GrUserStencilOp::kIncWrap, GrUserStencilOp::kDecWrap,
        GrUserStencilOp::kKeep, GrUserStencilOp::kKeep, 0xffff, 0xffff>());

// Inverts the bottom stencil bit. Used for "even/odd" fill.
constexpr static GrUserStencilSettings kInvertStencil(
    GrUserStencilSettings::StaticInit<
        0x0000, GrUserStencilTest::kAlwaysIfInClip, 0xffff, GrUserStencilOp::kInvert,
        GrUserStencilOp::kKeep, 0x0001>());
...
void GrPathTessellateOp::prePreparePipelineForStencils(const PrePrepareArgs& args) {
  ...
  SkASSERT(
      SkPathFillType::kWinding == fPath.getFillType() ||
...
void GrPathTessellateOp::prePrepareFillTrianglesProgram(const PrePrepareArgs& args, bool isLinear) {
  ...
  constexpr static GrUserStencilSettings kFillOrIncrDecrStencil(
      GrUserStencilSettings::StaticInitSeparate<
          0x0000, 0x0000, GrUserStencilTest::kEqual, GrUserStencilTest::kEqual, 0xffff, 0xffff,
          GrUserStencilOp::kKeep, GrUserStencilOp::kKeep, GrUserStencilOp::kIncWrap,
          GrUserStencilOp::kDecWrap, 0xffff, 0xffff>());

  constexpr static GrUserStencilSettings kFillOrInvertStencil(
      GrUserStencilSettings::StaticInit<
          0x0000, GrUserStencilTest::kEqual, 0xffff, GrUserStencilOp::kKeep, GrUserStencilOp::kZero,
          0xffff>());
  ...
  fFillTrianglesProgram = GrPathShader::MakeProgramInfo(
      fillTriangleShader, args.fArena, args.fWriteView, fPipelineForFills, *args.fDstProxyView,
      args.fXferBarrierFlags, args.fColorLoadOp, stencil, *args.fCaps);
}
...
void GrPathTessellateOp::prePrepareFillCubicHullsProgram(const PrePrepareArgs& args) {
  ...
  fFillPathProgram = GrPathShader::MakeProgramInfo(
      fillCubicHullsShader, args.fArena, args.fWriteView, fPipelineForFills, *args.fDstProxyView,
      args.fXferBarrierFlags, args.fColorLoadOp, &kTestAndResetStencil, *args.fCaps);
}
...
void GrPathTessellateOp::prePrepareFillBoundingBoxProgram(const PrePrepareArgs& args) {
  ...
  auto* fillBoundingBoxShader =
      args.fArena->make<GrFillBoundingBoxShader>(fViewMatrix, fColor, fPath.getBounds());
  fFillPathProgram = GrPathShader::MakeProgramInfo(
      fillBoundingBoxShader, args.fArena, args.fWriteView, fPipelineForFills, *args.fDstProxyView,
      args.fXferBarrierFlags, args.fColorLoadOp, &kTestAndResetStencil, *args.fCaps);
}
...
void GrPathTessellateOp::onPrepare(GrOpFlushState* flushState) {
  ...
  if (!fPipelineForStencils && !fPipelineForFills) {
    ...
    GrEagerDynamicVertexAllocator innerTriangleAllocator(
        flushState, &fTriangleBuffer, &fBaseTriangleVertex);
  ...
  if (fTriangleVertexCount != 0) {
    ...
    if (fOffThreadInnerTriangulation) {
      ...
      void* data = flushState->makeVertexSpace(
          sizeof(SkPoint), fTriangleVertexCount, &fTriangleBuffer, &fBaseTriangleVertex);
...
void GrPathTessellateOp::prepareMiddleOutTrianglesAndCubics(
  ...
  if (!fStencilTrianglesProgram) {
    ...
    vertexData = static_cast<SkPoint*>(target->makeVertexSpace(
        sizeof(SkPoint) * 4, maxInnerTriangles + maxCubics, &fCubicBuffer, &baseTriangleInstance));
  ...
  GrMiddleOutPolygonTriangulator middleOut(
      vertexData, vertexAdvancePerTriangle, fPath.countVerbs());
  ...
  for (auto [verb, pts, w] : SkPathPriv::Iterate(fPath)) {
    switch (verb) {
      case SkPathVerb::kMove: middleOut.closeAndMove(pts[0]); break;
      case SkPathVerb::kLine: middleOut.pushVertex(pts[1]); break;
      ...
      case SkPathVerb::kQuad:
        ...
        if (resolveLevelCounter) {
          resolveLevelCounter->countInstance(
              GrWangsFormula::quadratic_log2(kLinearizationIntolerance, pts, xform));
          break;
        }
        ++numCountedCurves;
        break;
      case SkPathVerb::kCubic:
        ...
        if (resolveLevelCounter) {
          resolveLevelCounter->countInstance(
              GrWangsFormula::cubic_log2(kLinearizationIntolerance, pts, xform));
          break;
        }
        ++numCountedCurves;
        break;
      case SkPathVerb::kClose: middleOut.close(); break;
    }
  }
  ...
  if (!fStencilTrianglesProgram) {
    ...
    target->putBackVertices(
        maxInnerTriangles + maxCubics - totalInstanceCount, sizeof(SkPoint) * 4);
    if (totalInstanceCount) {
      this->prepareIndirectOuterCubicsAndTriangles(
          target, *resolveLevelCounter, vertexData, triangleCount);
    }
  } else {
...
void GrPathTessellateOp::prepareIndirectOuterCubics(
  ...
  cubicData = static_cast<SkPoint*>(target->makeVertexSpace(
      sizeof(SkPoint) * 4, resolveLevelCounter.totalInstanceCount(), &fCubicBuffer, &baseInstance));
  if (!cubicData) {
    return;
  }
...
void GrPathTessellateOp::prepareIndirectOuterCubicsAndTriangles(
  ...
  fIndirectIndexBuffer =
      GrMiddleOutCubicShader::FindOrMakeMiddleOutIndexBuffer(target->resourceProvider());
  ...
  SkASSERT(
      ...
      numTrianglesAtBeginningOfData + resolveLevelCounter.totalInstanceCount());
  ...
  int totalInstanceCount = numTrianglesAtBeginningOfData + resolveLevelCounter.totalInstanceCount();
    ...
    for (auto [verb, pts, w] : SkPathPriv::Iterate(fPath)) {
      ...
      switch (verb) {
        case SkPathVerb::kQuad:
          ...
          break;
        case SkPathVerb::kCubic: memcpy(instanceLocations[level], pts, sizeof(SkPoint) * 4); break;
        case SkPathVerb::kConic:
  ...
  SkASSERT(
      fCubicVertexCount ==
      (numTrianglesAtBeginningOfData + resolveLevelCounter.totalInstanceCount()) * 4);
#endif
}

void GrPathTessellateOp::prepareTessellatedOuterCubics(
...
void GrPathTessellateOp::prepareTessellatedCubicWedges(GrMeshDrawOp::Target* target) {
  ...
  while (parser.parseNextContour()) {
    ...
    for (auto [verb, pts, w] : parser.currentContour()) {
      switch (verb) {
        case SkPathVerb::kMove: startPoint = lastPoint = pts[0]; continue;
        case SkPathVerb::kClose: continue;  // Ignore. We can assume an implicit close at the end.
...
void GrPathTessellateOp::drawStencilPass(GrOpFlushState* flushState) {
  ...
  if (fCubicVertexCount > 0) {
    ...
    if (fIndirectDrawBuffer) {
      ...
      flushState->drawIndexedIndirect(
          fIndirectDrawBuffer.get(), fIndirectDrawOffset, fIndirectDrawCount);
    } else {
