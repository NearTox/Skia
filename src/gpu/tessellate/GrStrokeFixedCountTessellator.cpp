...
class InstanceWriter {
 public:
  ...
  InstanceWriter(
      const GrShaderCaps* shaderCaps, ShaderFlags shaderFlags, GrMeshDrawTarget* target,
      float matrixMaxScale, const SkRect& strokeCullBounds, const SkMatrix& viewMatrix,
  ...
  SK_ALWAYS_INLINE void quadraticTo(const SkPoint p[3]) {
    ...
    this->writeStroke(cubic, endControlPoint, GrTessellationShader::kCubicCurveType);
    fMaxParametricSegments_pow4 = std::max(numParametricSegments_pow4, fMaxParametricSegments_pow4);
  }
  ...
  void writeCircle(SkPoint location) {
    if (GrVertexWriter writer = fChunkBuilder.appendVertex()) {
      ...
      writer.write(GrVertexWriter::If(
          !fShaderCaps->infinitySupport(), GrTessellationShader::kCubicCurveType));
  ...
  void finishContour() {
    if (fHasDeferredFirstStroke) {
      ...
      this->writeStroke(fDeferredFirstStroke, SkPoint(), fDeferredCurveTypeIfUnsupportedInfinity);
 ...
 private:
  ...
  SK_ALWAYS_INLINE void writeStroke(
      const SkPoint p[4], SkPoint endControlPoint, float curveTypeIfUnsupportedInfinity) {
    if (!fHasLastControlPoint) {
      ...
      writer.write(
          GrVertexWriter::If(!fShaderCaps->infinitySupport(), curveTypeIfUnsupportedInfinity));
      this->writeDynamicAttribs(&writer);
    }
...
GrStrokeFixedCountTessellator::GrStrokeFixedCountTessellator(
    ...
    : GrStrokeTessellator(
          ...
          kMaxParametricSegments_log2, viewMatrix, pathStrokeList, matrixMinMaxScales,
          strokeCullBounds) {}

GR_DECLARE_STATIC_UNIQUE_KEY(gVertexIDFallbackBufferKey);

void GrStrokeFixedCountTessellator::prepare(GrMeshDrawTarget* target, int totalCombinedVerbCnt) {
  ...
  InstanceWriter instanceWriter(
      target->caps().shaderCaps(), fShader.flags(), target, fMatrixMinMaxScales[1],
      fStrokeCullBounds, fShader.viewMatrix(), &fInstanceChunks, fShader.instanceStride(),
  ...
  for (PathStrokeList* pathStroke = fPathStrokeList; pathStroke; pathStroke = pathStroke->fNext) {
    ...
    if (fShader.hasDynamicStroke()) {
      ...
      float numRadialSegmentsPerRadian = toleranceBuffer.fetchRadialSegmentsPerRadian(pathStroke);
      maxEdgesInJoin = std::max(
          worst_case_edges_in_join(stroke.getJoin(), numRadialSegmentsPerRadian), maxEdgesInJoin);
      maxRadialSegmentsPerRadian = std::max(numRadialSegmentsPerRadian, maxRadialSegmentsPerRadian);
    ...
    while (strokeIter.next()) {
      ...
      switch (strokeIter.verb()) {
        ...
        int numChops;
        case Verb::kContourFinished: instanceWriter.finishContour(); break;
        case Verb::kCircle:
          // Round cap or else an empty stroke that is specified to be drawn as a circle.
          instanceWriter.writeCircle(p[0]);
          [[fallthrough]];
        case Verb::kMoveWithinContour: instanceWriter.setLastControlPoint(p[0]); break;
        case Verb::kLine: instanceWriter.lineTo(p[0], p[1]); break;
        case Verb::kQuad:
  ...
  int maxParametricSegmentsInStroke =
      SkScalarCeilToInt(sqrtf(sqrtf(instanceWriter.maxParametricSegments_pow4())));
  ...
  if (!target->caps().shaderCaps()->vertexIDSupport()) {
    ...
    fVertexBufferIfNoIDSupport = target->resourceProvider()->findOrMakeStaticBuffer(
        GrGpuBufferType::kVertex, kMaxVerticesInFallbackBuffer * sizeof(float),
        gVertexIDFallbackBufferKey, GrStrokeTessellationShader::InitializeVertexIDFallbackBuffer);
  }
