...
template <int Degree>
constexpr float length_term(float intolerance) {
  return (Degree * (Degree - 1) / 8.f) * intolerance;
}
template <int Degree>
constexpr float length_term_pow2(float intolerance) {
...
SK_ALWAYS_INLINE static float quadratic_pow4(
    float intolerance, const SkPoint pts[], const GrVectorXform& vectorXform = GrVectorXform()) {
...
SK_ALWAYS_INLINE static int worst_case_cubic_log2(
    float intolerance, float devWidth, float devHeight) {
