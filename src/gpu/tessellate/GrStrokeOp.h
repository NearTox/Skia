...
class GrStrokeOp : public GrDrawOp {
 protected:
  ...
  GrStrokeOp(
      uint32_t classID, GrAAType, const SkMatrix&, const SkStrokeRec&, const SkPath&, GrPaint&&);
  ...
  void prePrepareColorProgram(
      SkArenaAlloc* arena, GrStrokeTessellateShader*, const GrSurfaceProxyView*, GrAppliedClip&&,
      const GrXferProcessor::DstProxyView&, GrXferBarrierFlags, const GrCaps&);
