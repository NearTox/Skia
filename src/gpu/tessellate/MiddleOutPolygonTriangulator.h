...
class MiddleOutPolygonTriangulator {
 ...
 public:
  ...
  class PoppedTriangleStack {
   public:
    PoppedTriangleStack(
        MiddleOutPolygonTriangulator* middleOut, SkPoint lastPoint, StackVertex* end,
        StackVertex* newTopVertex, StackVertex newTopValue)
 ...
 public:
  ...
  MiddleOutPolygonTriangulator::PoppedTriangleStack nextStack() {
    ...
    switch (auto [verb, pts, w] = *fPathIter++; verb) {
      SkPoint pt;
      case SkPathVerb::kMove: return fMiddleOut.closeAndMove(pts[0]);
      ...
      case SkPathVerb::kClose: return fMiddleOut.close();
    }
    SkUNREACHABLE;
  }
