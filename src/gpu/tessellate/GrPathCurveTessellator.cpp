...
class CurveWriter {
 public:
  ...
  void setMatrices(
      const SkRect& cullBounds, const SkMatrix& shaderMatrix, const SkMatrix& pathMatrix) {
  ...
  SK_ALWAYS_INLINE void writeQuadratic(
      const GrShaderCaps& shaderCaps, GrVertexChunkBuilder* chunker, const SkPoint p[3]) {
    ...
    if (numSegments_pow4 > 1) {
      if (GrVertexWriter vertexWriter = chunker->appendVertex()) {
        ...
        vertexWriter.write(GrVertexWriter::If(
            !shaderCaps.infinitySupport(), GrTessellationShader::kCubicCurveType));
      }
      fNumFixedSegments_pow4 = std::max(numSegments_pow4, fNumFixedSegments_pow4);
    }
  }

  SK_ALWAYS_INLINE void writeConic(
      const GrShaderCaps& shaderCaps, GrVertexChunkBuilder* chunker, const SkPoint p[3], float w) {
    ...
    if (numSegments_pow2 > 1) {
      if (GrVertexWriter vertexWriter = chunker->appendVertex()) {
        ...
        vertexWriter.write(GrVertexWriter::If(
            !shaderCaps.infinitySupport(), GrTessellationShader::kConicCurveType));
      }
      fNumFixedSegments_pow4 =
          std::max(numSegments_pow2 * numSegments_pow2, fNumFixedSegments_pow4);
    }
  }

  SK_ALWAYS_INLINE void writeCubic(
      const GrShaderCaps& shaderCaps, GrVertexChunkBuilder* chunker, const SkPoint p[4]) {
 ...
 private:
  ...
  void chopAndWriteConic(
      const GrShaderCaps& shaderCaps, GrVertexChunkBuilder* chunker, const SkConic& conic) {
    SkConic chops[2];
  ...
  void writeTriangle(
      const GrShaderCaps& shaderCaps, GrVertexChunkBuilder* chunker, SkPoint p0, SkPoint p1,
      SkPoint p2) {
    if (GrVertexWriter vertexWriter = chunker->appendVertex()) {
      vertexWriter.write(fPathXform.mapPoint(p0), fPathXform.mapPoint(p1), fPathXform.mapPoint(p2));
      // Mark this instance as a triangle by setting it to a conic with w=Inf.
      vertexWriter.fill(GrVertexWriter::kIEEE_32_infinity, 2);
      vertexWriter.write(GrVertexWriter::If(
          !shaderCaps.infinitySupport(), GrTessellationShader::kTriangularConicCurveType));
    }
  }
...
GrPathCurveTessellator* GrPathCurveTessellator::Make(
    SkArenaAlloc* arena, const SkMatrix& viewMatrix, const SkPMColor4f& color,
    DrawInnerFan drawInnerFan, int numPathVerbs, const GrPipeline& pipeline, const GrCaps& caps) {
  ...
  if (caps.shaderCaps()->tessellationSupport() &&
    ...
    shader = GrPathTessellationShader::MakeHardwareTessellationShader(
        arena, viewMatrix, color, PatchType::kCurves);
  } else {
    shader = GrPathTessellationShader::MakeMiddleOutFixedCountShader(
        *caps.shaderCaps(), arena, viewMatrix, color, PatchType::kCurves);
  }
  return arena->make(
      [=](void* objStart) { return new (objStart) GrPathCurveTessellator(shader, drawInnerFan); });
}

GR_DECLARE_STATIC_UNIQUE_KEY(gFixedCountVertexBufferKey);
GR_DECLARE_STATIC_UNIQUE_KEY(gFixedCountIndexBufferKey);

void GrPathCurveTessellator::prepare(
    GrMeshDrawTarget* target, const SkRect& cullBounds, const PathDrawList& pathDrawList,
    int totalCombinedPathVerbCnt, const BreadcrumbTriangleList* breadcrumbTriangleList) {
    ...
    if (fDrawInnerFan) {
      ...
      for (auto [pathMatrix, path] : pathDrawList) {
        ...
        vertexWriter = GrMiddleOutPolygonTriangulator::WritePathInnerFan(
            std::move(vertexWriter), pad32Count, pad32Value, pathMatrix, path,
            &numTrianglesWritten);
  ...
  for (auto [pathMatrix, path] : pathDrawList) {
    ...
    for (auto [verb, pts, w] : SkPathPriv::Iterate(path)) {
      switch (verb) {
        case SkPathVerb::kQuad: curveWriter.writeQuadratic(shaderCaps, &chunker, pts); break;
        case SkPathVerb::kConic: curveWriter.writeConic(shaderCaps, &chunker, pts, *w); break;
        case SkPathVerb::kCubic: curveWriter.writeCubic(shaderCaps, &chunker, pts); break;
        default: break;
      }
    }
  }
  ...
  if (!fShader->willUseTessellationShaders()) {
    ...
    fFixedCountVertexBuffer = target->resourceProvider()->findOrMakeStaticBuffer(
        GrGpuBufferType::kVertex, GrPathTessellationShader::SizeOfVertexBufferForMiddleOutCurves(),
    ...
    fFixedCountIndexBuffer = target->resourceProvider()->findOrMakeStaticBuffer(
        GrGpuBufferType::kIndex, GrPathTessellationShader::SizeOfIndexBufferForMiddleOutCurves(),
