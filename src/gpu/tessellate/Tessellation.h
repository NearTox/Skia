...
constexpr static int kMaxParametricSegments_p4 =
    kMaxParametricSegments_p2 * kMaxParametricSegments_p2;
...
SkPath PreChopPathCurves(
    float tessellationPrecision, const SkPath&, const SkMatrix&, const SkRect& viewport);
...
constexpr size_t PatchAttribsStride(PatchAttribs attribs) {
  return (attribs & PatchAttribs::kJoinControlPoint ? sizeof(float) * 2 : 0) +
         ...
         (attribs & PatchAttribs::kColor
              ? (attribs & PatchAttribs::kWideColorIfEnabled ? sizeof(float) : sizeof(uint8_t)) * 4
              : 0) +
...
struct StrokeParams {
  ...
  StrokeParams(const SkStrokeRec& stroke) { this->set(stroke); }
