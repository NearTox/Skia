...
class GrCullTest {
 public:
  ...
  GrCullTest(const SkRect& devCullBounds, const SkMatrix& m) { this->set(devCullBounds, m); }
  ...
  void set(const SkRect& devCullBounds, const SkMatrix& m) {
    ...
    fCullBounds = {
        devCullBounds.fLeft - m.getTranslateX(), devCullBounds.fTop - m.getTranslateY(),
        m.getTranslateX() - devCullBounds.fRight, m.getTranslateY() - devCullBounds.fBottom};
  }
