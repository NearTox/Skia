...
GrStrokeOp::GrStrokeOp(
    uint32_t classID, GrAAType aaType, const SkMatrix& viewMatrix, const SkStrokeRec& stroke,
...
GrProcessorSet::Analysis GrStrokeOp::finalize(
...
GrOp::CombineResult GrStrokeOp::onCombineIfPossible(
    GrOp* grOp, SkArenaAlloc* alloc, const GrCaps&) {
  SkASSERT(grOp->classID() == this->classID());
  auto* op = static_cast<GrStrokeOp*>(grOp);
  if (fNeedsStencil || op->fNeedsStencil || fColor != op->fColor ||
      fViewMatrix != op->fViewMatrix || fAAType != op->fAAType ||
...
void GrStrokeOp::prePreparePrograms(
    GrStrokeTessellateShader::Mode shaderMode, SkArenaAlloc* arena,
  ...
  if (fNeedsStencil) {
    auto* stencilPipeline = (standaloneStencilPipeline) ? standaloneStencilPipeline : fillPipeline;
  ...
  fFillProgram = GrPathShader::MakeProgramInfo(
      strokeTessellateShader, arena, writeView, fillPipeline, dstProxyView, fillXferFlags,
