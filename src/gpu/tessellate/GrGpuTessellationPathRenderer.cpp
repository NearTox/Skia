...
GrGpuTessellationPathRenderer::GrGpuTessellationPathRenderer(const GrCaps& caps)
    : fAtlas(
...
bool GrGpuTessellationPathRenderer::onDrawPath(const DrawPathArgs& args) {
  ...
  if (this->tryAddPathToAtlas(
          *args.fContext->priv().caps(), *args.fViewMatrix, path, args.fAAType, &devIBounds,
          &devToAtlasOffset)) {
    auto op = pool->allocate<GrDrawAtlasPathOp>(
        renderTargetContext->numSamples(), sk_ref_sp(fAtlas.textureProxy()), devIBounds,
        devToAtlasOffset, *args.fViewMatrix, std::move(args.fPaint));
...
void GrGpuTessellationPathRenderer::preFlush(
  ...
  for (auto antialias : {false, true}) {
    for (auto fillType : {SkPathFillType::kWinding, SkPathFillType::kEvenOdd}) {
      ...
      auto op = onFlushRP->opMemoryPool()->allocate<GrTessellatePathOp>(
          SkMatrix::I(), *uberPath, GrPaint(), aaType, GrTessellatePathOp::Flags::kStencilOnly);
