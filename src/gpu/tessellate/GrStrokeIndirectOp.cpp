...
void GrStrokeIndirectOp::onPrePrepare(
  ...
  this->prePreparePrograms(
      GrStrokeTessellateShader::Mode::kIndirect, arena, writeView,
      (clip) ? std::move(*clip) : GrAppliedClip::Disabled(), dstProxyView, renderPassXferBarriers,
...
template <int Nx2>
static SK_ALWAYS_INLINE vec<Nx2 / 2> length_pow2(vec<Nx2> v) {
  auto vv = v * v;
  return vv.lo + vv.hi;
}
...
template <int N>
SK_ALWAYS_INLINE vec<N> unchecked_mix(vec<N> a, vec<N> b, vec<N> T) {
...
class ResolveLevelCounter {
 public:
  ResolveLevelCounter(
      const SkStrokeRec& stroke, GrStrokeTessellateShader::Tolerances tolerances,
  ...
  countLine(const SkPoint pts[2], SkPoint lastControlPoint, int8_t* resolveLevelPtr) {
    if (!fIsRoundJoin) {
  ...
  void countCubic(const SkPoint pts[4], SkPoint lastControlPoint, int8_t* resolveLevelPtr) {
    float numParametricSegments = GrWangsFormula::cubic(fTolerances.fParametricIntolerance, pts);
  ...
  void countChoppedCubic(
      const SkPoint pts[4], const float chopT, SkPoint lastControlPoint, int8_t* resolveLevelPtr) {
    SkPoint chops[7];
 ...
 private:
  void writeResolveLevel(
      float numParametricSegments, float rotation, int8_t* resolveLevelPtr) const {
 ...
 private:
  ...
  struct SIMDQueue {
    // Enqueues a stroke.
    SK_ALWAYS_INLINE int push(
        const SkPoint pts[NumPts], bool pushRoundJoin, SkPoint lastControlPoint,
    ...
    // values in "vec.hi".
    template <int N>
    vec<N * 2> loadPoint(int idx) const {
    ...
    // "vec.hi".
    template <int N>
    vec<N * 2> loadLastControlPoint() const {
    ...
    template <int N>
    vec<N * 2> loadPointFromArray(const float array[8]) const {
      if constexpr (N == 4) {
        if (fCount == 4) {
  ...
  void flushQuads() {
    ...
    this->writeResolveLevels<N>(
        numParametricSegments, rotation, fQuadQueue.fCount, fQuadQueue.fResolveLevelPtrs);
    fQuadQueue.fCount = 0;
  }
  ...
  template <int N>
  SK_ALWAYS_INLINE void flushCubics(
      const SIMDQueue<4>& queue, vec<N * 2> p0, vec<N * 2> p1, vec<N * 2> p2, vec<N * 2> p3,
    ...
    this->writeResolveLevels<N>(
        numParametricSegments, rotation, queue.fCount, queue.fResolveLevelPtrs, resultOffset);
  }

  template <int N>
  SK_ALWAYS_INLINE void writeResolveLevels(
      vec<N> numParametricSegments, vec<N> rotation, int count, int8_t* const* resolveLevelPtrs,
      int offset = 0) const {
      ...
      if (counter.isRoundJoin()) {
        ...
        switch (iter.prevVerb()) {
          ...
          case Verb::kConic:
            ...
            [[fallthrough]];
          case Verb::kLine: lastControlPoint = prevPts[0]; break;
          ...
          case Verb::kCircle:
            ...
            break;
          case Verb::kContourFinished: SkUNREACHABLE;
        }
      }
      switch (verb) {
        case Verb::kLine:
        ...
        case Verb::kCubic: {
          ...
          if (numChops == 0) {
            ...
            *nextResolveLevel++ = -((1 << 1) | (int)areCusps);
            counter.countChoppedCubic(pts, nextChopTs[0], lastControlPoint, nextResolveLevel);
          } else {
            SkASSERT(numChops == 2);
            // A negative resolveLevel indicates how many chops the curve needs, and
            // whether they are cusps.
...
void GrStrokeIndirectOp::onPrepare(GrOpFlushState* flushState) {
  if (!fResolveLevels) {
    ...
    this->prePreparePrograms(
        GrStrokeTessellateShader::Mode::kIndirect, arena, flushState->writeView(),
    ...
    while (iter.next()) {
      ...
      switch (verb) {
        ...
        case Verb::kContourFinished:
          ...
          if (firstCubic) {
            ...
            nextInstanceLocations[firstResolveLevel]++->set(
                firstCubic, lastControlPoint, numEdgesPerResolveLevel[firstResolveLevel]);
            firstCubic = nullptr;
          }
      ...
      for (int i = 0;;) {
        if (!hasLastControlPoint) {
          ...
          nextInstanceLocations[resolveLevel]++->set(
              pts_, lastControlPoint,
