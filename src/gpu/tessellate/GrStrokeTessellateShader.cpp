...
class GrStrokeTessellateShader::TessellationImpl : public GrGLSLGeometryProcessor {
 ...
 private:
  void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
    ...
    fTessArgs1Uniform = uniHandler->addUniform(
        nullptr, kTessControl_GrShaderFlag, kFloat4_GrSLType, "tessArgs1", nullptr);
    // uJoinTolerancePow2, uStrokeRadius.
    fTessArgs2Uniform = uniHandler->addUniform(
        nullptr, kTessControl_GrShaderFlag | kTessEvaluation_GrShaderFlag, kFloat2_GrSLType,
        "tessArgs2", nullptr);
    if (!shader.viewMatrix().isIdentity()) {
      fTranslateUniform = uniHandler->addUniform(
          nullptr, kTessEvaluation_GrShaderFlag, kFloat2_GrSLType, "translate", nullptr);
      ...
      auto affineMatrixVisibility =
          (shader.fStroke.isHairlineStyle()) ? kVertex_GrShaderFlag : kTessEvaluation_GrShaderFlag;
      fAffineMatrixUniform = uniHandler->addUniform(
          nullptr, affineMatrixVisibility, kFloat4_GrSLType, "affineMatrix", &affineMatrixName);
      if (affineMatrixVisibility & kVertex_GrShaderFlag) {
        v->codeAppendf("float2x2 uAffineMatrix = float2x2(%s);\n", affineMatrixName);
      }
    }
    const char* colorUniformName;
    fColorUniform = uniHandler->addUniform(
    ...
    v->defineConstantf(
        "float", "kParametricEpsilon", "1.0 / (%i * 128)",
  ...
  void setData(
    ...
    switch (stroke.getJoin()) {
      case SkPaint::kBevel_Join: numSegmentsInJoin = 1; break;
      case SkPaint::kMiter_Join: numSegmentsInJoin = (stroke.getMiter() > 0) ? 2 : 1; break;
    ...
    pdman.set4f(
        ...
        GrWangsFormula::length_term_pow2<3>(tolerances.fParametricIntolerance),  // uWangsTermPow2
...
SkString GrStrokeTessellateShader::getTessEvaluationShaderGLSL(
  ...
  code.appendf(
      "#define MAX_PARAMETRIC_SEGMENTS_LOG2 %i\n",
...
class GrStrokeTessellateShader::IndirectImpl : public GrGLSLGeometryProcessor {
  void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
    ...
    // Constants.
    args.fVertBuilder->defineConstant(
        "MAX_PARAMETRIC_SEGMENTS_LOG2", GrTessellationPathRenderer::kMaxResolveLevel);
    args.fVertBuilder->defineConstant("float", "PI", "3.141592653589793238");
    args.fVertBuilder->defineConstant("QUAD_TERM_POW2", GrWangsFormula::length_term_pow2<2>(1));
    args.fVertBuilder->defineConstant("CUBIC_TERM_POW2", GrWangsFormula::length_term_pow2<3>(1));
    ...
    if (!shader.viewMatrix().isIdentity()) {
      ...
      fAffineMatrixUniform = args.fUniformHandler->addUniform(
          nullptr, kVertex_GrShaderFlag, kFloat4_GrSLType, "affineMatrix", &affineMatrixName);
    ...
    if (shader.fStroke.getJoin() == SkPaint::kRound_Join) {
      ...
      args.fVertBuilder->codeAppendf(
          ...
          IndirectInstance::NumExtraEdgesInJoin(joinType));
    }
...
void GrStrokeTessellateShader::getGLSLProcessorKey(
...
GrGLSLPrimitiveProcessor* GrStrokeTessellateShader::createGLSLInstance(const GrShaderCaps&) const {
  return (fMode == Mode::kTessellation) ? (GrGLSLPrimitiveProcessor*)new TessellationImpl
                                        : new IndirectImpl;
}
