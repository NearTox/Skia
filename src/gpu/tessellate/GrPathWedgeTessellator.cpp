...
class WedgeWriter {
 public:
  WedgeWriter(
      GrMeshDrawTarget* target, GrVertexChunkArray* vertexChunkArray, size_t patchStride,
      int initialPatchAllocCount, int maxSegments)
  ...
  SK_ALWAYS_INLINE void writeFlatWedge(
      const GrShaderCaps& shaderCaps, SkPoint p0, SkPoint p1, SkPoint midpoint) {
    if (GrVertexWriter vertexWriter = fChunker.appendVertex()) {
      ...
      vertexWriter.write(
          GrVertexWriter::If(!shaderCaps.infinitySupport(), GrTessellationShader::kCubicCurveType));
    }
  }

  SK_ALWAYS_INLINE void writeQuadraticWedge(
      const GrShaderCaps& shaderCaps, const SkPoint p[3], SkPoint midpoint) {
  ...
  SK_ALWAYS_INLINE void writeConicWedge(
      const GrShaderCaps& shaderCaps, const SkPoint p[3], float w, SkPoint midpoint) {
    ...
    if (GrVertexWriter vertexWriter = fChunker.appendVertex()) {
      ...
      vertexWriter.write(midpoint);
      vertexWriter.write(
          GrVertexWriter::If(!shaderCaps.infinitySupport(), GrTessellationShader::kConicCurveType));
    }
    fNumFixedSegments_pow4 = std::max(numSegments_pow2 * numSegments_pow2, fNumFixedSegments_pow4);
  }

  SK_ALWAYS_INLINE void writeCubicWedge(
      const GrShaderCaps& shaderCaps, const SkPoint p[4], SkPoint midpoint) {
 ...
 private:
  void chopAndWriteQuadraticWedges(
      const GrShaderCaps& shaderCaps, const SkPoint p[3], SkPoint midpoint) {
    SkPoint chops[5];
  ...
  void chopAndWriteConicWedges(
      const GrShaderCaps& shaderCaps, const SkConic& conic, SkPoint midpoint) {
    SkConic chops[2];
...
GrPathTessellator* GrPathWedgeTessellator::Make(
    SkArenaAlloc* arena, const SkMatrix& viewMatrix, const SkPMColor4f& color, int numPathVerbs,
  ...
  if (caps.shaderCaps()->tessellationSupport() &&
    ...
    shader = GrPathTessellationShader::MakeHardwareTessellationShader(
        arena, viewMatrix, color, PatchType::kWedges);
  } else {
    shader = GrPathTessellationShader::MakeMiddleOutFixedCountShader(
        *caps.shaderCaps(), arena, viewMatrix, color, PatchType::kWedges);
  }
  return arena->make([=](void* objStart) { return new (objStart) GrPathWedgeTessellator(shader); });
}

GR_DECLARE_STATIC_UNIQUE_KEY(gFixedCountVertexBufferKey);
GR_DECLARE_STATIC_UNIQUE_KEY(gFixedCountIndexBufferKey);

void GrPathWedgeTessellator::prepare(
  ...
  for (auto [pathMatrix, path] : pathDrawList) {
    ...
    while (parser.parseNextContour()) {
      ...
      for (auto [verb, pts, w] : parser.currentContour()) {
        switch (verb) {
          case SkPathVerb::kMove: startPoint = lastPoint = pts[0]; break;
          case SkPathVerb::kClose: break;  // Ignore. We can assume an implicit close at the end.
  ...
  if (!fShader->willUseTessellationShaders()) {
    ...
    fFixedCountVertexBuffer = target->resourceProvider()->findOrMakeStaticBuffer(
        GrGpuBufferType::kVertex, GrPathTessellationShader::SizeOfVertexBufferForMiddleOutWedges(),
    ...
    fFixedCountIndexBuffer = target->resourceProvider()->findOrMakeStaticBuffer(
        GrGpuBufferType::kIndex, GrPathTessellationShader::SizeOfIndexBufferForMiddleOutWedges(),
