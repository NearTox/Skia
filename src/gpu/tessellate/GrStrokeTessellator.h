...
class GrStrokeTessellator {
 public:
  ...
  GrStrokeTessellator(
      const GrShaderCaps& shaderCaps, GrStrokeTessellationShader::Mode shaderMode,
      ShaderFlags shaderFlags, int8_t maxParametricSegments_log2, const SkMatrix& viewMatrix,
      PathStrokeList* pathStrokeList, std::array<float, 2> matrixMinMaxScales,
      const SkRect& strokeCullBounds)
      : fShader(
            shaderCaps, shaderMode, shaderFlags, viewMatrix, pathStrokeList->fStroke,
  ...
  virtual ~GrStrokeTessellator() = default;
...
struct GrStrokeTolerances {
  ...
  static float CalcNumRadialSegmentsPerRadian(float parametricPrecision, float strokeWidth) {
    return .5f / acosf(std::max(1 - 2 / (parametricPrecision * strokeWidth), -1.f));
  }
  template <int N>
  static grvx::vec<N> ApproxNumRadialSegmentsPerRadian(
  ...
  static GrStrokeTolerances Make(const float matrixMinMaxScales[2], float strokeWidth) {
    return MakeNonHairline(
        matrixMinMaxScales[1], GetLocalStrokeWidth(matrixMinMaxScales, strokeWidth));
  }
  static GrStrokeTolerances MakeNonHairline(float matrixMaxScale, float strokeWidth) {
    ...
    return {parametricPrecision, CalcNumRadialSegmentsPerRadian(parametricPrecision, strokeWidth)};
  }
...
class alignas(sizeof(float) * 4) GrStrokeToleranceBuffer {
 public:
  ...
  GrStrokeToleranceBuffer(float parametricPrecision) : fParametricPrecision(parametricPrecision) {}
  ...
  float fetchRadialSegmentsPerRadian(PathStrokeList* head) {
    ...
    if (fBufferIdx == 4) {
      ...
      auto tol =
          GrStrokeTolerances::ApproxNumRadialSegmentsPerRadian(fParametricPrecision, fStrokeWidths);
