...
class GrStrokeTessellateOp : public GrStrokeOp {
 ...
 private:
  ...
  void onPrePrepare(
      ...
      const GrXferProcessor::DstProxyView&, GrXferBarrierFlags, GrLoadOp colorLoadOp) override;
  ...
  enum class Convex180Status : bool { kUnknown, kYes };
  ...
  void quadraticTo(
      const SkPoint[3], JoinType prevJoinType = JoinType::kFromStroke, int maxDepth = -1);
  void cubicTo(
      const SkPoint[4], JoinType prevJoinType = JoinType::kFromStroke,
