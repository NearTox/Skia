...
void GrTessellatePathOp::onPrepare(GrOpFlushState* state) {
  ...
  GrEagerDynamicVertexAllocator cubicInstanceAllocator(
      state, &fCubicInstanceBuffer, &fBaseCubicInstance);
  ...
  if (cpuTessellationWork * 500 + (256 * 256) < gpuFragmentWork) {     // Don't try below 256x256.
    ...
    if ((fPathVertexCount = GrTessellator::PathToTriangles(
      ...
      if (!(Flags::kStencilOnly & fFlags)) {
        fFillPathShader = state->allocator()->make<GrFillTriangleShader>(fViewMatrix, fColor);
      }
      if (!pathIsLinear) {
  ...
  if (rasterEdgeWork > 1000 * 1000) {
    if ((fPathVertexCount = GrPathParser::EmitInnerPolygonTriangles(fPath, &pathVertexAllocator))) {
      fStencilPathShader = state->allocator()->make<GrStencilTriangleShader>(fViewMatrix);
    }
    fCubicInstanceCount = GrPathParser::EmitCubicInstances(fPath, &cubicInstanceAllocator);
    return;
...
void GrTessellatePathOp::drawStencilPass(
    GrOpFlushState* state, const GrAppliedHardClip& hardClip,
  ...
  if (fStencilPathShader) {
    ...
    fStencilPathShader->issueDraw(
        state, &pipeline, fixedDynamicState, fPathVertexBuffer, fPathVertexCount, fBasePathVertex,
        this->bounds());
  }
  ...
  if (fCubicInstanceBuffer) {
    ...
    GrStencilCubicShader(fViewMatrix)
        .issueDraw(state, &pipeline, fixedDynamicState, mesh, this->bounds());
  }
...
void GrTessellatePathOp::drawCoverPass(
    GrOpFlushState* state, GrAppliedClip&& clip,
    const GrPipeline::FixedDynamicState* fixedDynamicState) {
  ...
  if (fFillPathShader) {
    ...
    if (fCubicInstanceBuffer) {
      ...
      pipeline.setUserStencil(&kTestAndResetStencil);
      GrFillCubicHullShader(fViewMatrix, fColor)
          .issueDraw(state, &pipeline, fixedDynamicState, mesh, this->bounds());
    }
  } else {
    ...
    pipeline.setUserStencil(&kTestAndResetStencil);
    GrFillBoundingBoxShader(fViewMatrix, fColor, fPath.getBounds())
        .issueDraw(state, &pipeline, fixedDynamicState, mesh, this->bounds());
  }
}
