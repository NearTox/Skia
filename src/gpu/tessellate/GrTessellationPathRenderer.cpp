...
GrPathRenderer::CanDrawPath GrTessellationPathRenderer::onCanDrawPath(
  ...
  if (!shape.style().isSimpleFill()) {
    ...
    if (shape.style().strokeRec().getStyle() == SkStrokeRec::kStrokeAndFill_Style ||
        !args.fCaps->shaderCaps()->tessellationSupport() || GrAAType::kCoverage == args.fAAType ||
...
bool GrTessellationPathRenderer::onDrawPath(const DrawPathArgs& args) {
  ...
  if (args.fShape->style().isSimpleFill() &&
      this->tryAddPathToAtlas(
          *args.fContext->priv().caps(), *args.fViewMatrix, path, devBounds, args.fAAType,
    ...
    auto op = GrOp::Make<GrDrawAtlasPathOp>(
        args.fContext, renderTargetContext->numSamples(), sk_ref_sp(fAtlas.textureProxy()),
  ...
  int worstCaseResolveLevel = GrWangsFormula::worst_case_cubic_log2(
      kLinearizationIntolerance, devBounds.width(), devBounds.height());
  if (worstCaseResolveLevel > kMaxResolveLevel) {
    ...
    auto viewport = SkRect::MakeIWH(renderTargetContext->width(), renderTargetContext->height());
  ...
  if (args.fShape->style().isSimpleHairline()) {
    ...
    auto op = GrOp::Make<GrStrokeTessellateOp>(
        args.fContext, args.fAAType, SkMatrix::I(), devStroke, devPath, std::move(args.fPaint));
    renderTargetContext->addDrawOp(args.fClip, std::move(op));
    return true;
  }
  ...
  if (!args.fShape->style().isSimpleFill()) {
    ...
    auto op = GrOp::Make<GrStrokeTessellateOp>(
        args.fContext, args.fAAType, *args.fViewMatrix, stroke, path, std::move(args.fPaint));
    renderTargetContext->addDrawOp(args.fClip, std::move(op));
    return true;
  }
  ...
  auto op = GrOp::Make<GrPathTessellateOp>(
      args.fContext, *args.fViewMatrix, path, std::move(args.fPaint), args.fAAType, drawPathFlags);
  renderTargetContext->addDrawOp(args.fClip, std::move(op));
  return true;
}
...
bool GrTessellationPathRenderer::tryAddPathToAtlas(
    ...
    GrAAType aaType, SkIRect* devIBounds, SkIPoint16* locationInAtlas, bool* transposedInAtlas) {
...
void GrTessellationPathRenderer::renderAtlas(GrOnFlushResourceProvider* onFlushRP) {
  ...
  auto coverOp = GrFillRectOp::Make(
      rtc->surfPriv().getContext(), std::move(paint), aaType, &drawQuad, stencil, fillRectFlags);
