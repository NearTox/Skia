...
bool GrTessellationPathRenderer::IsSupported(const GrCaps& caps) {
  return !caps.avoidStencilBuffers() && caps.drawInstancedSupport() &&
         ...
         caps.nativeDrawIndirectSupport() && caps.shaderCaps()->vertexIDSupport() &&
         !caps.disableTessellationPathRenderer();
}

GrTessellationPathRenderer::GrTessellationPathRenderer(GrRecordingContext* rContext)
    : fAtlas(
          kAtlasAlpha8Type, GrDynamicAtlas::InternalMultisample::kYes, kAtlasInitialSize,
...
void GrTessellationPathRenderer::initAtlasFlags(GrRecordingContext* rContext) {
  ...
  SkASSERT(SkScalarNearlyEqual(
      GrWangsFormula::worst_case_cubic(kLinearizationIntolerance, worstCaseWidth, worstCaseHeight),
      s, 1));
#endif
...
GrPathRenderer::CanDrawPath GrTessellationPathRenderer::onCanDrawPath(
  ...
  if (shape.style().hasPathEffect() || args.fViewMatrix->hasPerspective() ||
      shape.style().strokeRec().getStyle() == SkStrokeRec::kStrokeAndFill_Style ||
      shape.inverseFilled() || args.fHasUserStencilSettings) {
...
static GrOp::Owner make_op(
    GrRecordingContext* rContext, const GrSurfaceContext* surfaceContext,
    GrTessellationPathRenderer::OpFlags opFlags, GrAAType aaType, const SkRect& shapeDevBounds,
    const SkMatrix& viewMatrix, const GrStyledShape& shape, GrPaint&& paint) {
  ...
  // divided into.
  int worstCaseResolveLevel = GrWangsFormula::worst_case_cubic_log2(
      kLinearizationIntolerance, shapeDevBounds.width(), shapeDevBounds.height());
  if (worstCaseResolveLevel > kMaxResolveLevel) {
    ...
    if (stroke.getStyle() == SkStrokeRec::kHairline_Style) {
      inflationRadius +=
          SkStrokeRec::GetInflationRadius(stroke.getJoin(), stroke.getMiter(), stroke.getCap(), 1);
    ...
    worstCaseResolveLevel = GrWangsFormula::worst_case_cubic_log2(
        kLinearizationIntolerance, newDevBounds.width(), newDevBounds.height());
  ...
  if (!shape.style().isSimpleFill()) {
    ...
    if (shaderCaps.tessellationSupport() && path.countVerbs() > 50 && !paint.usesVaryingCoords() &&
      ...
      return GrOp::Make<GrStrokeTessellateOp>(
          rContext, aaType, viewMatrix, stroke, path, std::move(paint));
    } else {
      return GrOp::Make<GrStrokeIndirectOp>(
          rContext, aaType, viewMatrix, path, stroke, std::move(paint));
    }
  } else {
    ...
    return GrOp::Make<GrPathTessellateOp>(
        rContext, viewMatrix, path, std::move(paint), aaType, opFlags);
  }
}
...
bool GrTessellationPathRenderer::onDrawPath(const DrawPathArgs& args) {
  ...
  if (this->tryAddPathToAtlas(
          *args.fContext->priv().caps(), *args.fViewMatrix, *args.fShape, devBounds, args.fAAType,
    ...
    if (!(fStencilAtlasFlags & OpFlags::kDisableHWTessellation)) {
      int worstCaseNumSegments = GrWangsFormula::worst_case_cubic(
          kLinearizationIntolerance, devIBounds.width(), devIBounds.height());
    ...
    auto op = GrOp::Make<GrDrawAtlasPathOp>(
        args.fContext, surfaceDrawContext->numSamples(), sk_ref_sp(fAtlas.textureProxy()),
        devIBounds, locationInAtlas, transposedInAtlas, *args.fViewMatrix, std::move(args.fPaint));
    surfaceDrawContext->addDrawOp(args.fClip, std::move(op));
    return true;
  }

  if (auto op = make_op(
          args.fContext, surfaceDrawContext, OpFlags::kNone, args.fAAType, devBounds,
...
bool GrTessellationPathRenderer::tryAddPathToAtlas(
  ...
  *transposedInAtlas = minDimension > maxDimenstion;
  ...
  if (*transposedInAtlas) {
    ...
    atlasMatrix.postTranslate(
        locationInAtlas->x() - devIBounds->x(), locationInAtlas->y() - devIBounds->y());
  }
...
void GrTessellationPathRenderer::onStencilPath(const StencilPathArgs& args) {
  ...
  args.fViewMatrix->mapRect(&devBounds, args.fShape->bounds());
  if (auto op = make_op(
          args.fContext, surfaceDrawContext, OpFlags::kStencilOnly, aaType, devBounds,
          *args.fViewMatrix, *args.fShape, GrPaint())) {
    surfaceDrawContext->addDrawOp(args.fClip, std::move(op));
  }
}

void GrTessellationPathRenderer::preFlush(
...
constexpr static GrUserStencilSettings kTestStencil(
    GrUserStencilSettings::StaticInit<
        0x0000, GrUserStencilTest::kNotEqual, 0xffff, GrUserStencilOp::kKeep,
...
void GrTessellationPathRenderer::renderAtlas(GrOnFlushResourceProvider* onFlushRP) {
  ...
  auto coverOp = GrFillRectOp::Make(
      rtc->recordingContext(), std::move(paint), aaType, &drawQuad, stencil, fillRectFlags);
