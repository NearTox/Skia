...
bool GrTessellationPathRenderer::onDrawPath(const DrawPathArgs& args) {
  ...
  if (args.fShape->style().isSimpleFill() &&
    ...
    auto op = pool->allocate<GrDrawAtlasPathOp>(
        renderTargetContext->numSamples(), sk_ref_sp(fAtlas.textureProxy()), devIBounds,
  ...
  if (args.fShape->style().isSimpleHairline()) {
    ...
    auto op = pool->allocate<GrStrokeTessellateOp>(
        args.fAAType, SkMatrix::I(), devPath, devStroke, std::move(args.fPaint));
    renderTargetContext->addDrawOp(args.fClip, std::move(op));
    return true;
  }
  ...
  if (!args.fShape->style().isSimpleFill()) {
    ...
    auto op = pool->allocate<GrStrokeTessellateOp>(
        args.fAAType, *args.fViewMatrix, path, stroke, std::move(args.fPaint));
    renderTargetContext->addDrawOp(args.fClip, std::move(op));
    return true;
  }
  ...
  auto op = pool->allocate<GrPathTessellateOp>(
      *args.fViewMatrix, path, std::move(args.fPaint), args.fAAType, drawPathFlags);
  renderTargetContext->addDrawOp(args.fClip, std::move(op));
  return true;
}
