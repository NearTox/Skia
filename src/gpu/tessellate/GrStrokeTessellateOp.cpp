...
void GrStrokeTessellateOp::onPrePrepare(
  ...
  this->prePrepareColorProgram(
      arena, strokeTessellateShader, writeView, std::move(*clip), dstProxyView,
...
void GrStrokeTessellateOp::onPrepare(GrOpFlushState* flushState) {
  if (!fColorProgram) {
    ...
    auto* strokeTessellateShader = arena->make<GrStrokeTessellateShader>(
        fStroke, fParametricIntolerance, fNumRadialSegmentsPerRadian, fViewMatrix, fColor);
    this->prePrepareColorProgram(
        flushState->allocator(), strokeTessellateShader, flushState->writeView(),
