...
void GrStrokeTessellateOp::onPrePrepare(
  ...
  this->prePreparePrograms(
      GrStrokeTessellateShader::Mode::kTessellation, context->priv().recordTimeAllocator(),
      writeView, (clip) ? std::move(*clip) : GrAppliedClip::Disabled(), dstProxyView,
...
void GrStrokeTessellateOp::onPrepare(GrOpFlushState* flushState) {
  if (!fFillProgram && !fStencilProgram) {
    this->prePreparePrograms(
        GrStrokeTessellateShader::Mode::kTessellation, flushState->allocator(),
        flushState->writeView(), flushState->detachAppliedClip(), flushState->dstProxyView(),
        flushState->renderPassBarriers(), flushState->colorLoadOp(), flushState->caps());
  }
...
void GrStrokeTessellateOp::prepareBuffers() {
  ...
  float numRadialSegments180 =
      std::max(std::ceil(SK_ScalarPI * fTolerances.fNumRadialSegmentsPerRadian), 1.f);
  float maxParametricSegments180 =
      NumParametricSegments(fMaxTessellationSegments, numRadialSegments180);
  fMaxParametricSegments180_pow4 = pow4(maxParametricSegments180);

  float numRadialSegments360 =
      std::max(std::ceil(2 * SK_ScalarPI * fTolerances.fNumRadialSegmentsPerRadian), 1.f);
  float maxParametricSegments360 =
      NumParametricSegments(fMaxTessellationSegments, numRadialSegments360);
  ...
  switch (fStroke.getJoin()) {
    case SkPaint::kBevel_Join: maxNumSegmentsInJoin = 1; break;
    case SkPaint::kMiter_Join: maxNumSegmentsInJoin = 2; break;
  ...
  fMaxParametricSegments180_pow4_withJoin =
      pow4(std::max(maxParametricSegments180 - maxNumSegmentsInJoin - 1, 0.f));
  fMaxParametricSegments360_pow4_withJoin =
      pow4(std::max(maxParametricSegments360 - maxNumSegmentsInJoin - 1, 0.f));
    ...
    for (auto [verb, pts, w] : SkPathPriv::Iterate(path)) {
      switch (verb) {
        ...
        case SkPathVerb::kLine:
          ...
          this->lineTo(pts[1]);
          break;
        case SkPathVerb::kQuad: this->quadraticTo(pts); break;
        case SkPathVerb::kCubic: this->cubicTo(pts); break;
...
static bool chop_pt_is_cusp(
    const SkPoint& prevControlPoint, const SkPoint& chopPoint, const SkPoint& nextControlPoint) {
...
void GrStrokeTessellateOp::quadraticTo(const SkPoint p[3], JoinType prevJoinType, int maxDepth) {
  ...
  if (numCombinedSegments > fMaxTessellationSegments) {
    ...
    JoinType nextJoinType = (quad_chop_is_cusp(chops)) ? JoinType::kCusp : JoinType::kFromStroke;
...
void GrStrokeTessellateOp::cubicTo(
    const SkPoint p[4], JoinType prevJoinType, Convex180Status convex180Status, int maxDepth) {
  SkASSERT(fHasCurrentPoint);
  SkASSERT(p[0] == fCurrentPoint);
  ...
  float maxParametricSegments_pow4 = (convex180Status == Convex180Status::kYes)
                                         ? fMaxParametricSegments180_pow4
                                         : fMaxParametricSegments360_pow4;
    ...
    if (int n = GrPathUtils::findCubicConvex180Chops(p, chopT)) {
      ...
      for (int i = 1; i <= n; ++i) {
        ...
        JoinType nextJoinType =
            (cubic_chop_is_cusp(chops + (i - 1) * 3)) ? JoinType::kCusp : JoinType::kFromStroke;
  ...
  if (numCombinedSegments > fMaxTessellationSegments) {
    ...
    // If we chopped at a cusp then rotation is not continuous between the two curves. Insert a
    // cusp to make up for lost rotation.
    JoinType nextJoinType = (cubic_chop_is_cusp(chops)) ? JoinType::kCusp : JoinType::kFromStroke;
  ...
  switch (fStroke.getCap()) {
    ...
    case SkPaint::kRound_Cap: {
      ...
      JoinType roundCapJoinType =
          (fStroke.getJoin() == SkPaint::kRound_Join) ? JoinType::kFromStroke : JoinType::kCusp;
    ...
    case SkPaint::kSquare_Cap: {
      ...
      if (!fStroke.isHairlineStyle()) {
        ...
        firstTangent *= -.5f / fViewMatrix.mapVector(firstTangent.fX, firstTangent.fY).length();
      }
...
void GrStrokeTessellateOp::allocPatchChunkAtLeast(int minPatchAllocCount) {
  ...
  fCurrChunkPatchData = (Patch*)fTarget->makeVertexSpaceAtLeast(
      sizeof(Patch), minPatchAllocCount, minPatchAllocCount, &chunk->fPatchBuffer,
