...
class GrStrokeTessellateShader : public GrPathShader {
 public:
  ...
  GrStrokeTessellateShader(
      const SkStrokeRec& stroke, float matrixScale, const SkMatrix& viewMatrix, SkPMColor4f color)
      : GrPathShader(
            kTessellate_GrStrokeTessellateShader_ClassID, viewMatrix, GrPrimitiveType::kPatches, 1),
        fStroke(stroke),
