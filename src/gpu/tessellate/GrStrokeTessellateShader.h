...
class GrStrokeTessellateShader : public GrPathShader {
 public:
  ...
  enum class Mode : bool { kTessellation, kIndirect };
  ...
  struct Tolerances {
    Tolerances() = default;
    Tolerances(float matrixMaxScale, float strokeWidth) { this->set(matrixMaxScale, strokeWidth); }
  ...
  GrStrokeTessellateShader(
      Mode mode, bool hasConics, const SkStrokeRec& stroke, const SkMatrix& viewMatrix,
      SkPMColor4f color)
      : GrPathShader(
            kTessellate_GrStrokeTessellateShader_ClassID, viewMatrix,
            (mode == Mode::kTessellation) ? GrPrimitiveType::kPatches
