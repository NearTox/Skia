...
class GrStrokeTessellateShader : public GrPathShader {
 public:
  ...
  GrStrokeTessellateShader(const SkMatrix& skewMatrix, SkPMColor4f color, float miterLimitOrZero)
      : GrPathShader(
            kTessellate_GrStrokeTessellateShader_ClassID, skewMatrix, GrPrimitiveType::kPatches,
            kNumVerticesPerPatch),
        fColor(color),
 ...
 private:
  const char* name() const noexcept override { return "GrStrokeTessellateShader"; }
