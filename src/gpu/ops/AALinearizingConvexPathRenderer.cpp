...
void extract_verts(
    const GrAAConvexTessellator& tess, const SkMatrix* localCoordsMatrix, void* vertData,
    const GrVertexColor& color, uint16_t firstIndex, uint16_t* idxs) {
  ...
  for (int i = 0; i < tess.numPts(); ++i) {
    ...
    verts.write(tess.point(i), color, GrVertexWriter::If(localCoordsMatrix, lc), tess.coverage(i));
  }
...
GrGeometryProcessor* create_lines_only_gp(
    SkArenaAlloc* arena, bool tweakAlphaForCoverage, bool usesLocalCoords, bool wideColor) {
...
class AAFlatteningConvexPathOp final : public GrMeshDrawOp {
 ...
 public:
  ...
  static GrOp::Owner Make(
    ...
    return Helper::FactoryHelper<AAFlatteningConvexPathOp>(
        context, std::move(paint), viewMatrix, path, strokeWidth, style, join, miterLimit,
        stencilSettings);
  }
 ...
 private:
  ...
  void recordDraw(
      GrMeshDrawTarget* target, int vertexCount, size_t vertexStride, void* vertices,
    ...
    mesh->setIndexed(
        std::move(indexBuffer), indexCount, firstIndex, 0, vertexCount - 1, GrPrimitiveRestart::kNo,
  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    for (int i = 0; i < instanceCount; i++) {
      ...
      GrAAConvexTessellator tess(args.fStyle, args.fStrokeWidth, args.fJoin, args.fMiterLimit);
      ...
      extract_verts(
          tess, localCoordsMatrix, vertices + vertexStride * vertexCount,
          GrVertexColor(args.fColor, fWideColor), vertexCount, indices + indexCount);
  ...
  SkString onDumpInfo() const override {
    ...
    for (const auto& path : fPaths) {
      string.appendf(
          ...
          "MiterLimit: %.2f\n",
          path.fColor.toBytes_RGBA(), path.fStrokeWidth, path.fStyle, path.fJoin, path.fMiterLimit);
    }
...
GR_DRAW_OP_TEST_DEFINE(AAFlatteningConvexPathOp) {
  ...
  SkStrokeRec::Style styles[3] = {
      SkStrokeRec::kFill_Style, SkStrokeRec::kStroke_Style, SkStrokeRec::kStrokeAndFill_Style};
  ...
  return AAFlatteningConvexPathOp::Make(
      context, std::move(paint), viewMatrix, path, strokeWidth, style, join, miterLimit,
      stencilSettings);
}
...
PathRenderer::CanDrawPath AALinearizingConvexPathRenderer::onCanDrawPath(
  ...
  if (stroke.getStyle() == SkStrokeRec::kStroke_Style ||
    ...
    if (strokeWidth > kMaxStrokeWidth || !args.fShape->knownToBeClosed() ||
...
bool AALinearizingConvexPathRenderer::onDrawPath(const DrawPathArgs& args) {
  GR_AUDIT_TRAIL_AUTO_FRAME(
      args.fContext->priv().auditTrail(), "AALinearizingConvexPathRenderer::onDrawPath");
