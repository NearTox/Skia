...
#ifdef SK_DEBUG
static SkString dump_quad_info(
    int index, const GrPerspQuad& deviceQuad, const GrPerspQuad& localQuad,
  ...
  str.appendf(
      ...
      "(%.2f, %.2f, %.2f)]\n",
      index, color.fR, color.fG, color.fB, color.fA, (uint32_t)(aaFlags & GrQuadAAFlags::kLeft),
      (uint32_t)(aaFlags & GrQuadAAFlags::kTop), (uint32_t)(aaFlags & GrQuadAAFlags::kRight),
      (uint32_t)(aaFlags & GrQuadAAFlags::kBottom), deviceQuad.x(0), deviceQuad.y(0),
      deviceQuad.w(0), deviceQuad.x(1), deviceQuad.y(1), deviceQuad.w(1), deviceQuad.x(2),
      deviceQuad.y(2), deviceQuad.w(2), deviceQuad.x(3), deviceQuad.y(3), deviceQuad.w(3),
      localQuad.x(0), localQuad.y(0), localQuad.w(0), localQuad.x(1), localQuad.y(1),
      localQuad.w(1), localQuad.x(2), localQuad.y(2), localQuad.w(2), localQuad.x(3),
      localQuad.y(3), localQuad.w(3));
  return str;
}
#endif
...
class FillRectOp final : public GrMeshDrawOp {
 ...
 public:
  static std::unique_ptr<GrDrawOp> Make(
      GrRecordingContext* context, GrPaint&& paint, GrAAType aaType, GrQuadAAFlags edgeAA,
      const GrUserStencilSettings* stencilSettings, const GrPerspQuad& deviceQuad,
    ...
    return Helper::FactoryHelper<FillRectOp>(
        context, std::move(paint), aaType, edgeAA, stencilSettings, deviceQuad, localQuad);
  }
  ...
  FillRectOp(
      Helper::MakeArgs args, SkPMColor4f paintColor, GrAAType aaType, GrQuadAAFlags edgeFlags,
      const GrUserStencilSettings* stencil, const GrPerspQuad& deviceQuad,
      const GrPerspQuad& localQuad)
      : INHERITED(ClassID()), fHelper(args, aaType, stencil) {
  ...
  SkString dumpInfo() const override {
    ...
    str.appendf("# draws: %u\n", this->quadCount());
    str.appendf(
        "Device quad type: %u, local quad type: %u\n", (uint32_t)fDeviceQuads.quadType(),
        (uint32_t)fLocalQuads.quadType());
  ...
  GrProcessorSet::Analysis finalize(
    ...
    for (int i = 1; i < this->quadCount(); ++i) {
      quadColors = GrProcessorAnalysisColor::Combine(quadColors, fDeviceQuads.metadata(i).fColor);
    ...
    auto result =
        fHelper.finalizeProcessors(caps, clip, fsaaType, clampType, coverage, &quadColors);
    ...
    if (quadColors.isConstant(&colorOverride)) {
      ...
      for (int i = 0; i < this->quadCount(); ++i) {
        ...
        fColorType = SkTMax(fColorType, GrQuadPerEdgeAA::MinColorType(*color, clampType, caps));
      }
    }
 ...
 private:
  ...
  void onPrepareDraws(Target* target) override {
    ...
    VertexSpec vertexSpec(
        fDeviceQuads.quadType(), fColorType, fLocalQuads.quadType(), fHelper.usesLocalCoords(),
        Domain::kNo, fHelper.aaType(), fHelper.compatibleWithCoverageAsAlpha());
    ...
    void* vdata = target->makeVertexSpace(
        vertexSize, this->quadCount() * vertexSpec.verticesPerQuad(), &vbuffer,
    ...
    if (fHelper.isTrivial()) {
      ...
      for (int i = 0; i < this->quadCount(); ++i) {
        const ColorAndAA& info = fDeviceQuads.metadata(i);
        vertices = GrQuadPerEdgeAA::Tessellate(
            vertices, vertexSpec, fDeviceQuads[i], info.fColor, kIgnoredLocal, kEmptyDomain,
            info.fAAFlags);
      }
    } else {
      ...
      for (int i = 0; i < this->quadCount(); ++i) {
        const ColorAndAA& info = fDeviceQuads.metadata(i);
        vertices = GrQuadPerEdgeAA::Tessellate(
            vertices, vertexSpec, fDeviceQuads[i], info.fColor, fLocalQuads[i], kEmptyDomain,
            info.fAAFlags);
      }
    }
  ...
  void addQuad(
      const GrPerspQuad& deviceQuad, const GrPerspQuad& localQuad, const SkPMColor4f& color,
      GrQuadAAFlags edgeAA, GrAAType aaType) {
  ...
  int quadCount() const {
    ...
    SkASSERT(
        fDeviceQuads.count() == fLocalQuads.count() ||
...
std::unique_ptr<GrDrawOp> MakeGeneric(
    GrRecordingContext* context, GrPaint&& paint, GrAAType aaType, GrQuadAAFlags aaFlags,
    const GrPerspQuad& deviceQuad, const GrPerspQuad& localQuad,
    const GrUserStencilSettings* stencil) {
  return FillRectOp::Make(
      context, std::move(paint), aaType, aaFlags, stencil, deviceQuad, localQuad);
}

std::unique_ptr<GrDrawOp> MakeNonAARect(
    GrRecordingContext* context, GrPaint&& paint, const SkMatrix& view, const SkRect& rect,
    const GrUserStencilSettings* stencil) {
  return MakeGeneric(
      context, std::move(paint), GrAAType::kNone, GrQuadAAFlags::kNone,
      GrPerspQuad::MakeFromRect(rect, view), GrPerspQuad(rect), stencil);
}

std::unique_ptr<GrDrawOp> MakePerEdge(
    GrRecordingContext* context, GrPaint&& paint, GrAAType aaType, GrQuadAAFlags edgeAA,
    const SkMatrix& viewMatrix, const SkRect& rect, const GrUserStencilSettings* stencilSettings) {
  return MakeGeneric(
      context, std::move(paint), aaType, edgeAA, GrPerspQuad::MakeFromRect(rect, viewMatrix),
      GrPerspQuad(rect), stencilSettings);
}

std::unique_ptr<GrDrawOp> MakePerEdgeWithLocalMatrix(
    GrRecordingContext* context, GrPaint&& paint, GrAAType aaType, GrQuadAAFlags edgeAA,
    const SkMatrix& viewMatrix, const SkMatrix& localMatrix, const SkRect& rect,
...
std::unique_ptr<GrDrawOp> MakePerEdgeWithLocalRect(
    GrRecordingContext* context, GrPaint&& paint, GrAAType aaType, GrQuadAAFlags edgeAA,
...
std::unique_ptr<GrDrawOp> MakePerEdgeQuad(
    GrRecordingContext* context, GrPaint&& paint, GrAAType aaType, GrQuadAAFlags edgeAA,
    const SkMatrix& viewMatrix, const SkPoint quad[4], const SkPoint localQuad[4],
    const GrUserStencilSettings* stencilSettings) {
  const SkPoint* localPoints = localQuad ? localQuad : quad;
  return MakeGeneric(
      context, std::move(paint), aaType, edgeAA, GrPerspQuad::MakeFromSkQuad(quad, viewMatrix),
...
std::unique_ptr<GrDrawOp> MakeSet(
  ...
  std::unique_ptr<GrDrawOp> op = FillRectOp::Make(
      context, std::move(paint), aaType, quads[0].fAAFlags, stencilSettings,
  ...
  for (int i = 1; i < cnt; ++i) {
    ...
    GrResolveAATypeForQuad(aaType, quads[i].fAAFlags, deviceQuad, &resolvedAA, &resolvedEdgeFlags);

    fillRects->addQuad(
        deviceQuad, GrPerspQuad::MakeFromRect(quads[i].fRect, quads[i].fLocalMatrix),
...
std::unique_ptr<GrDrawOp> Make(
  ...
  return MakePerEdge(
      ...
      aaType == GrAAType::kCoverage ? GrQuadAAFlags::kAll : GrQuadAAFlags::kNone, viewMatrix, rect,
      stencil);
}

std::unique_ptr<GrDrawOp> MakeWithLocalMatrix(
    GrRecordingContext* context, GrPaint&& paint, GrAAType aaType, const SkMatrix& viewMatrix,
    const SkMatrix& localMatrix, const SkRect& rect, const GrUserStencilSettings* stencil) {
  return MakePerEdgeWithLocalMatrix(
      context, std::move(paint), aaType,
      aaType == GrAAType::kCoverage ? GrQuadAAFlags::kAll : GrQuadAAFlags::kNone, viewMatrix,
      localMatrix, rect, stencil);
}

std::unique_ptr<GrDrawOp> MakeWithLocalRect(
    GrRecordingContext* context, GrPaint&& paint, GrAAType aaType, const SkMatrix& viewMatrix,
    const SkRect& rect, const SkRect& localRect, const GrUserStencilSettings* stencil) {
  return MakePerEdgeWithLocalRect(
      context, std::move(paint), aaType,
      aaType == GrAAType::kCoverage ? GrQuadAAFlags::kAll : GrQuadAAFlags::kNone, viewMatrix, rect,
      localRect, stencil);
}
  ...
  if (random->nextBool()) {
    if (random->nextBool()) {
      if (random->nextBool()) {
        ...
        quads.push_back({rect, SkPMColor4f::FromBytes_RGBA(SkColorToPremulGrColor(random->nextU())),
        ...
        for (uint32_t i = 0; i < extraQuadCt; ++i) {
          ...
          quads.push_back({GrTest::TestRect(random),
                           SkPMColor4f::FromBytes_RGBA(SkColorToPremulGrColor(random->nextU())),
                           GrTest::TestMatrixInvertible(random), aaFlags});
        }

        return GrFillRectOp::MakeSet(
            context, std::move(paint), aaType, viewMatrix, quads.begin(), quads.count(), stencil);
      } else {
        // Single local matrix
        SkMatrix localMatrix = GrTest::TestMatrixInvertible(random);
        return GrFillRectOp::MakePerEdgeWithLocalMatrix(
            context, std::move(paint), aaType, aaFlags, viewMatrix, localMatrix, rect, stencil);
      }
    } else {
      // Pass local rect directly
      SkRect localRect = GrTest::TestRect(random);
      return GrFillRectOp::MakePerEdgeWithLocalRect(
          context, std::move(paint), aaType, aaFlags, viewMatrix, rect, localRect, stencil);
    }
  } else {
    // The simplest constructor
    return GrFillRectOp::MakePerEdge(
