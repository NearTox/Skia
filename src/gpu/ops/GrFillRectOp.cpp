 ...
 public:
  ...
  GrProcessorSet::Analysis finalize(
    ...
    if (quadColors.isConstant(&colorOverride)) {
      ...
      while (iter.next()) {
        fColorType =
            SkTMax(fColorType, GrQuadPerEdgeAA::MinColorType(iter->fColor, clampType, caps));
      }
    }
 ...
 private:
  ...
  void onPrepareDraws(Target* target) override {
    ...
    void* vdata = target->makeVertexSpace(
        vertexSize, fQuads.count() * vertexSpec.verticesPerQuad(), &vbuffer, &vertexOffsetInBuffer);
    if (!vdata) {
    ...
    while (iter.next()) {
      ...
      vertices = GrQuadPerEdgeAA::Tessellate(
          vertices, vertexSpec, iter.deviceQuad(), info.fColor, iter.localQuad(), kEmptyDomain,
          info.fAAFlags);
    }
...
std::unique_ptr<GrDrawOp> MakeSet(
  ...
  for (int i = 1; i < cnt; ++i) {
    ...
    fillRects->addQuad(
        deviceQuad, GrQuad::MakeFromRect(quads[i].fRect, quads[i].fLocalMatrix), quads[i].fColor,
