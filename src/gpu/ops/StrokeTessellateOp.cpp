...
StrokeTessellateOp::StrokeTessellateOp(
    GrAAType aaType, const SkMatrix& viewMatrix, const SkPath& path, const SkStrokeRec& stroke,
    GrPaint&& paint)
    : GrDrawOp(ClassID()),
      fAAType(aaType),
...
GrProcessorSet::Analysis StrokeTessellateOp::finalize(
  ...
  const GrProcessorSet::Analysis& analysis = fProcessors.finalize(
      this->headColor(), GrProcessorAnalysisCoverage::kNone, clip, &GrUserStencilSettings::kUnused,
      caps, clampType, &this->headColor());
  fNeedsStencil = !analysis.unaffectedByDstValue();
  return analysis;
}

GrOp::CombineResult StrokeTessellateOp::onCombineIfPossible(
    GrOp* grOp, SkArenaAlloc* alloc, const GrCaps& caps) {
  ...
  if (fNeedsStencil || op->fNeedsStencil || fViewMatrix != op->fViewMatrix ||
      fAAType != op->fAAType || fProcessors != op->fProcessors ||
  ...
  constexpr static GrTFlagsMask<ShaderFlags> kDynamicStatesMask(
      ShaderFlags::kDynamicStroke | ShaderFlags::kDynamicColor);
  ...
  *fPathStrokeTail = headCopy;
  fPathStrokeTail =
      (op->fPathStrokeTail == &op->fPathStrokeList.fNext) ? &headCopy->fNext : op->fPathStrokeTail;
...
constexpr static GrUserStencilSettings kMarkStencil(
    GrUserStencilSettings::StaticInit<
        ...
        0x0000,                            // Always fail.
        GrUserStencilOp::kZero, GrUserStencilOp::kReplace, 0xffff>());
...
constexpr static GrUserStencilSettings kTestAndResetStencil(
    GrUserStencilSettings::StaticInit<
        ...
        0x0001, GrUserStencilOp::kZero, GrUserStencilOp::kReplace, 0xffff>());

void StrokeTessellateOp::prePrepareTessellator(
    GrTessellationShader::ProgramArgs&& args, GrAppliedClip&& clip) {
  SkASSERT(!fTessellator);
  SkASSERT(!fFillProgram);
  SkASSERT(!fStencilProgram);
  // GrOp::setClippedBounds() should have been called by now.
  SkASSERT(
      SkRect::MakeIWH(args.fWriteView.width(), args.fWriteView.height()).contains(this->bounds()));
  ...
  auto* pipeline =
      GrTessellationShader::MakePipeline(args, fAAType, std::move(clip), std::move(fProcessors));
  ...
  if (can_use_hardware_tessellation(fTotalCombinedVerbCnt, *pipeline, caps)) {
    ...
    fTessellator = arena->make<GrStrokeHardwareTessellator>(
        *caps.shaderCaps(), fShaderFlags, fViewMatrix, &fPathStrokeList, matrixMinMaxScales,
        strokeCullBounds);
  } else {
    fTessellator = arena->make<GrStrokeFixedCountTessellator>(
        *caps.shaderCaps(), fShaderFlags, fViewMatrix, &fPathStrokeList, matrixMinMaxScales,
        strokeCullBounds);
  }
  ...
  if (fNeedsStencil) {
    fStencilProgram =
        GrTessellationShader::MakeProgram(args, fTessellator->shader(), pipeline, &kMarkStencil);
    fillStencil = &kTestAndResetStencil;
    args.fXferBarrierFlags = GrXferBarrierFlags::kNone;
  }

  fFillProgram =
      GrTessellationShader::MakeProgram(args, fTessellator->shader(), pipeline, fillStencil);
}

void StrokeTessellateOp::onPrePrepare(
