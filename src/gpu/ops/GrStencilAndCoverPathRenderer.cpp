...
GrPathRenderer* GrStencilAndCoverPathRenderer::Create(
    GrResourceProvider* resourceProvider, const GrCaps& caps) {
...
GrPathRenderer::CanDrawPath GrStencilAndCoverPathRenderer::onCanDrawPath(
...
void GrStencilAndCoverPathRenderer::onStencilPath(const StencilPathArgs& args) {
  ...
  args.fRenderTargetContext->stencilPath(
      args.fClip, args.fDoStencilMSAA, *args.fViewMatrix, std::move(p));
}

bool GrStencilAndCoverPathRenderer::onDrawPath(const DrawPathArgs& args) {
  GR_AUDIT_TRAIL_AUTO_FRAME(
      args.fRenderTargetContext->auditTrail(), "GrStencilAndCoverPathRenderer::onDrawPath");
  ...
  if (args.fShape->inverseFilled()) {
    ...
    SkRect devBounds = SkRect::MakeIWH(
        args.fRenderTargetContext->width(),
    ...
    if (args.fClip && args.fClip->apply(
                          args.fContext, args.fRenderTargetContext, stencilAAType, true,
                          &appliedClip, &devBounds) == GrClip::Effect::kClippedOut) {
      return true;
    }
    GrStencilClip stencilClip(
        args.fRenderTargetContext->dimensions(), appliedClip.stencilStackID());
    ...
    if (appliedClip.windowRectsState().enabled()) {
      stencilClip.fixedClip().setWindowRectangles(
          appliedClip.windowRectsState().windows(), appliedClip.windowRectsState().mode());
    }
...

    {
      ...
      args.fRenderTargetContext->stencilRect(
          args.fClip, &kInvertedCoverPass, std::move(args.fPaint), coverAA, coverMatrix,
