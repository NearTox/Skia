...
void GrStencilAndCoverPathRenderer::onStencilPath(const StencilPathArgs& args) {
    ...
    args.fRenderTargetContext->priv().stencilPath(*args.fClip, args.fDoStencilMSAA,
...
bool GrStencilAndCoverPathRenderer::onDrawPath(const DrawPathArgs& args) {
    ...
    if (args.fShape->inverseFilled()) {
        ...
        if (!args.fClip->apply(args.fContext, args.fRenderTargetContext, doStencilMSAA, true,
        ...
        args.fRenderTargetContext->priv().stencilPath(stencilClip, GrAA(doStencilMSAA), viewMatrix,
                                                      path.get());

        {
            static constexpr GrUserStencilSettings kInvertedCoverPass(
                    GrUserStencilSettings::StaticInit<
                            ...
                            // will be 0 outside the clip. So we can't just fill where the user bits
                            // are 0. We also need to check that the clip bit is set.
