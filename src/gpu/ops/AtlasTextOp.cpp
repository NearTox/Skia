...
AtlasTextOp::AtlasTextOp(
    MaskType maskType, bool needsTransform, int glyphCount, SkRect deviceRect, Geometry* geo,
    GrPaint&& paint)
    : INHERITED{ClassID()},
...
AtlasTextOp::AtlasTextOp(
    MaskType maskType, bool needsTransform, int glyphCount, SkRect deviceRect,
    SkColor luminanceColor, bool useGammaCorrectDistanceTable, uint32_t DFGPFlags, Geometry* geo,
    GrPaint&& paint)
    : INHERITED{ClassID()},
      fProcessors(std::move(paint)),
      fNumGlyphs(glyphCount),
...
auto AtlasTextOp::Geometry::MakeForBlob(
    const GrAtlasSubRun& subRun, const SkMatrix& drawMatrix, SkPoint drawOrigin, SkIRect clipRect,
    sk_sp<GrTextBlob> blob, const SkPMColor4f& color, SkArenaAlloc* alloc) -> Geometry* {
  ...
  return new (geo)
      Geometry{subRun, drawMatrix, drawOrigin, clipRect, std::move(blob), nullptr, color};
}
...
void AtlasTextOp::Geometry::fillVertexData(void* dst, int offset, int count) const {
  ...
  fSubRun.fillVertexData(dst, offset, count, fColor.toBytes_RGBA(), positionMatrix, fClipRect);
}
...
SkString AtlasTextOp::onDumpInfo() const {
  ...
  for (Geometry* geom = fHead; geom != nullptr; geom = geom->fNext) {
    str.appendf(
        "%d: Color: 0x%08x Trans: %.2f,%.2f\n", i++, geom->fColor.toBytes_RGBA(),
        geom->fDrawOrigin.x(), geom->fDrawOrigin.y());
  }
...
GrProcessorSet::Analysis AtlasTextOp::finalize(
  ...
  auto analysis = fProcessors.finalize(
      color, coverage, clip, &GrUserStencilSettings::kUnused, caps, clampType, &fHead->fColor);
...
void AtlasTextOp::onPrepareDraws(GrMeshDrawTarget* target) {
  ...
  if (this->usesDistanceFields()) {
    ...
    flushInfo.fGeometryProcessor = GrBitmapTextGeoProc::Make(
        target->allocator(), *target->caps().shaderCaps(), fHead->fColor, false, views,
        numActiveViews, filter, maskFormat, localMatrix, fHasPerspective);
  }
  ...
  for (const Geometry* geo = fHead; geo != nullptr; geo = geo->fNext) {
    ...
    SkASSERTF(
        ...
        "subRun stride: %d vertex buffer stride: %d\n", (int)subRun.vertexStride(geo->fDrawMatrix),
        vertexStride);
...
void AtlasTextOp::createDrawForGeneratedGlyphs(
...
GrGeometryProcessor* AtlasTextOp::setupDfProcessor(
  ...
  if (this->isLCD()) {
    ...
    return GrDistanceFieldLCDTextGeoProc::Make(
        arena, caps, views, numActiveViews, GrSamplerState::Filter::kLinear, widthAdjust,
    ...
    return GrDistanceFieldA8TextGeoProc::Make(
        arena, caps, views, numActiveViews, GrSamplerState::Filter::kLinear, correction, fDFGPFlags,
        localMatrix);
#else
    return GrDistanceFieldA8TextGeoProc::Make(
        arena, caps, views, numActiveViews, GrSamplerState::Filter::kLinear, fDFGPFlags,
        localMatrix);
#endif
  }
}
...
GrOp::Owner AtlasTextOp::CreateOpTestingOnly(
    SurfaceDrawContext* sdc, const SkPaint& skPaint, const SkFont& font,
  ...
  std::tie(std::ignore, op) =
      subRun->makeAtlasTextOp(nullptr, mtxProvider, glyphRunList, skPaint, sdc, nullptr);
  return op;
}
...
GR_DRAW_OP_TEST_DEFINE(AtlasTextOp) {
  ...
  return skgpu::v1::AtlasTextOp::CreateOpTestingOnly(
      sdc, skPaint, font, matrixProvider, text, xInt, yInt);
}
