...
const GrPipeline* GrSimpleMeshDrawOpHelper::CreatePipeline(
  ...
  return flushState->allocator()->make<GrPipeline>(
      pipelineArgs, std::move(processorSet), flushState->detachAppliedClip());
}
...
bool GrSimpleMeshDrawOpHelperWithStencil::isCompatible(
    const GrSimpleMeshDrawOpHelperWithStencil& that, const GrCaps& caps, const SkRect& thisBounds,
    const SkRect& thatBounds, bool ignoreAAType) const {
