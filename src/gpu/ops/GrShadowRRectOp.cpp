...
class ShadowCircularRRectOp final : public GrMeshDrawOp {
 ...
 private:
  ...
  void fillInRRectVerts(const Geometry& args, CircleVertex** verts) const {
    ...
    SkScalar xInner[4] = {
        bounds.fLeft + umbraInset, bounds.fRight - umbraInset, bounds.fLeft + umbraInset,
        bounds.fRight - umbraInset};
    SkScalar xMid[4] = {
        bounds.fLeft + outerRadius, bounds.fRight - outerRadius, bounds.fLeft + outerRadius,
        bounds.fRight - outerRadius};
    SkScalar xOuter[4] = {bounds.fLeft, bounds.fRight, bounds.fLeft, bounds.fRight};
    SkScalar yInner[4] = {
        bounds.fTop + umbraInset, bounds.fTop + umbraInset, bounds.fBottom - umbraInset,
        bounds.fBottom - umbraInset};
    SkScalar yMid[4] = {
        bounds.fTop + outerRadius, bounds.fTop + outerRadius, bounds.fBottom - outerRadius,
  ...
  void onExecute(GrOpFlushState* flushState, const SkRect& chainBounds) override {
    ...
    flushState->bindTextures(
        fProgramInfo->primProc(), *fFalloffView.proxy(), fProgramInfo->pipeline());
  ...
  SkString onDumpInfo() const override {
    ...
    for (int i = 0; i < fGeoData.count(); ++i) {
      string.appendf(
          ...
          fGeoData[i].fDevBounds.fRight, fGeoData[i].fDevBounds.fBottom, fGeoData[i].fOuterRadius,
          fGeoData[i].fUmbraInset, fGeoData[i].fInnerRadius, fGeoData[i].fBlurRadius);
    }
    return string;
  }
#endif
...
GR_DRAW_OP_TEST_DEFINE(ShadowRRectOp) {
  ...
  do {
    ...
    if (isCircle) {
      ...
      if (auto op =
              GrShadowRRectOp::Make(context, color, viewMatrix, rrect, blurWidth, insetWidth)) {
        return op;
      }
    } else {
      SkRRect rrect;
