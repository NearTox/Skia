...
class TextureOpImpl final : public GrMeshDrawOp {
 public:
  ...
  GrProcessorSet::Analysis finalize(
    ...
    while (iter.next()) {
      ...
      colorType =
          std::max(static_cast<GrQuadPerEdgeAA::ColorType>(fMetadata.fColorType), colorType);
 ...
 private:
  ...
  struct Metadata {
    ...
    Metadata(
        const GrSwizzle& swizzle, GrSamplerState::Filter filter, GrSamplerState::MipmapMode mm,
        GrQuadPerEdgeAA::Subset subset, Saturate saturate)
        : fSwizzle(swizzle),
          fProxyCount(1),
          fTotalQuadCount(1),
  ...
  static int validate_op(
      GrTextureType textureType, GrAAType aaType, GrSwizzle swizzle, const TextureOpImpl* op) {
  ...
  void characterize(Desc* desc) const {
    ...
    auto indexBufferOption = GrQuadPerEdgeAA::CalcIndexBufferOption(overallAAType, maxQuadsPerMesh);
  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    if (fDesc->fVertexSpec.needsIndexBuffer()) {
      fDesc->fIndexBuffer =
          GrQuadPerEdgeAA::GetIndexBuffer(target, fDesc->fVertexSpec.indexBufferOption());
