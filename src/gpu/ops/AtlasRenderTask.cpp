...
AtlasRenderTask::AtlasRenderTask(
    GrRecordingContext* rContext, sk_sp<GrArenas> arenas,
    std::unique_ptr<GrDynamicAtlas> dynamicAtlas)
    : OpsTask(
          rContext->priv().drawingManager(), dynamicAtlas->writeView(*rContext->priv().caps()),
          rContext->priv().auditTrail(), std::move(arenas)),
      fDynamicAtlas(std::move(dynamicAtlas)) {}

bool AtlasRenderTask::addPath(
    const SkMatrix& viewMatrix, const SkPath& path, SkIPoint pathDevTopLeft, int widthInAtlas,
  ...
  if (transposedInAtlas) {
    ...
    pathToAtlasMatrix.postTranslate(
        locationInAtlas->x() - pathDevTopLeft.x(), locationInAtlas->y() - pathDevTopLeft.y());
  }
...
GrRenderTask::ExpectedOutcome AtlasRenderTask::onMakeClosed(
  ...
  if (caps.performColorClearsAsDraws() || caps.performStencilClearsAsDraws()) {
    ...
    constexpr static GrUserStencilSettings kClearStencil(
        GrUserStencilSettings::StaticInit<
            0x0000, GrUserStencilTest::kAlways, 0xffff, GrUserStencilOp::kReplace,
  ...
  for (const auto* pathList : {&fWindingPathList, &fEvenOddPathList}) {
    if (pathList->pathCount() > 0) {
      auto op = GrOp::Make<PathStencilCoverOp>(
          rContext, pathList->pathDrawList(), pathList->totalCombinedPathVerbCnt(),
          pathList->pathCount(), GrPaint(), GrAAType::kMSAA, GrTessellationPathFlags::kStencilOnly,
          drawRect);
...
void AtlasRenderTask::stencilAtlasRect(
    GrRecordingContext* rContext, const SkRect& rect, const SkPMColor4f& color,
    const GrUserStencilSettings* stencil) {
  GrPaint paint;
...
void AtlasRenderTask::addAtlasDrawOp(GrOp::Owner op, const GrCaps& caps) {
  ...
  auto processorAnalysis =
      drawOp->finalize(caps, nullptr, GrColorTypeClampType(fDynamicAtlas->colorType()));
...
bool AtlasRenderTask::onExecute(GrOpFlushState* flushState) {
  ...
  if (this->target(0)->requiresManualMSAAResolve()) {
    ...
    auto nativeRect = GrNativeRect::MakeIRectRelativeTo(
        GrDynamicAtlas::kTextureOrigin, this->target(0)->backingStoreDimensions().height(),
