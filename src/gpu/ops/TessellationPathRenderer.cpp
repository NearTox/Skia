...
GrOp::Owner make_non_convex_fill_op(
    GrRecordingContext* rContext, SkArenaAlloc* arena, GrTessellationPathFlags pathFlags,
    GrAAType aaType, const SkRect& drawBounds, const SkMatrix& viewMatrix, const SkPath& path,
    GrPaint&& paint) {
  ...
  if (numVerbs > 0 && !path.isInverseFillType()) {
    ...
    if (cpuTessellationWork * kCpuWeight + kMinNumPixelsToTriangulate < gpuFragmentWork) {
      return GrOp::Make<skgpu::v1::PathInnerTriangulateOp>(
          rContext, viewMatrix, path, std::move(paint), aaType, pathFlags, drawBounds);
    }
  }
  return GrOp::Make<skgpu::v1::PathStencilCoverOp>(
      rContext, arena, viewMatrix, path, std::move(paint), aaType, pathFlags, drawBounds);
}
...
bool TessellationPathRenderer::onDrawPath(const DrawPathArgs& args) {
  ...
  if (args.fShape->knownToBeConvex() && !path.isInverseFillType()) {
    auto op = GrOp::Make<PathTessellateOp>(
        args.fContext, *args.fViewMatrix, path, std::move(args.fPaint), args.fAAType,
  ...
  auto op = make_non_convex_fill_op(
      args.fContext, args.fSurfaceDrawContext->arenaAlloc(), GrTessellationPathFlags::kNone,
      args.fAAType, drawBounds, *args.fViewMatrix, path, std::move(args.fPaint));
  sdc->addDrawOp(args.fClip, std::move(op));
  return true;
}
...
void TessellationPathRenderer::onStencilPath(const StencilPathArgs& args) {
  ...
  if (args.fShape->knownToBeConvex()) {
    ...
    auto op = GrOp::Make<PathTessellateOp>(
        args.fContext, *args.fViewMatrix, path, std::move(stencilPaint), aaType, &kMarkStencil,
        pathDevBounds);
    sdc->addDrawOp(args.fClip, std::move(op));
    return;
  }
  ...
  auto op = make_non_convex_fill_op(
      args.fContext, args.fSurfaceDrawContext->arenaAlloc(), GrTessellationPathFlags::kStencilOnly,
      aaType, pathDevBounds, *args.fViewMatrix, path, GrPaint());
  sdc->addDrawOp(args.fClip, std::move(op));
}
