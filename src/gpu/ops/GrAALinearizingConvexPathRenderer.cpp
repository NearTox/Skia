...
static void extract_verts(
    const GrAAConvexTessellator& tess, void* vertData, const GrVertexColor& color,
    uint16_t firstIndex, uint16_t* idxs) {
...
static GrGeometryProcessor* create_lines_only_gp(
...
class AAFlatteningConvexPathOp final : public GrMeshDrawOp {
 ...
 public:
  ...
  AAFlatteningConvexPathOp(
    ...
    fPaths.emplace_back(PathData{color, viewMatrix, path, strokeWidth, style, join, miterLimit});
 ...
 private:
  void recordDraw(
      Target* target, const GrGeometryProcessor* gp, int vertexCount, size_t vertexStride,
      void* vertices, int indexCount, uint16_t* indices) const {
  ...
  void onPrepareDraws(Target* target) override {
    ...
    for (int i = 0; i < instanceCount; i++) {
      ...
      extract_verts(
          tess, vertices + vertexStride * vertexCount, GrVertexColor(args.fColor, fWideColor),
