...
bool GrDashOp::CanDrawDashLine(
    const SkPoint pts[2], const GrStyle& style, const SkMatrix& viewMatrix) {
...
static void calc_dash_scaling(
    SkScalar* parallelScale, SkScalar* perpScale, const SkMatrix& viewMatrix,
    const SkPoint pts[2]) {
...
static void align_to_x_axis(
    const SkPoint pts[2], SkMatrix* rotMatrix, SkPoint ptsRot[2] = nullptr) {
...
static SkScalar calc_end_adjustment(
    const SkScalar intervals[2], const SkPoint pts[2], SkScalar phase, SkScalar* endingInt) {
...
static void setup_dashed_rect(
    const SkRect& rect, GrVertexWriter& vertices, const SkMatrix& matrix, SkScalar offset,
    SkScalar bloatX, SkScalar bloatY, SkScalar len, SkScalar stroke, SkScalar startInterval,
    SkScalar endInterval, SkScalar strokeWidth, DashCap cap) {
  SkScalar intervalLength = startInterval + endInterval;
  SkRect dashRect = {offset - bloatX, -stroke - bloatY, offset + len + bloatX, stroke + bloatY};
  ...
  if (kRound_DashCap == cap) {
    ...
    vertices.writeQuad(
        GrQuad::MakeFromRect(rect, matrix), GrVertexWriter::TriStripFromRect(dashRect),
        intervalLength, radius, centerX);
  } else {
    ...
    SkRect rectParam;
    rectParam.set(
        halfOffLen + 0.5f, -halfStroke + 0.5f, halfOffLen + startInterval - 0.5f,
        halfStroke - 0.5f);

    vertices.writeQuad(
        GrQuad::MakeFromRect(rect, matrix), GrVertexWriter::TriStripFromRect(dashRect),
        intervalLength, rectParam);
  }
}
 ...
 */
static sk_sp<GrGeometryProcessor> make_dash_gp(
    const SkPMColor4f&, AAMode aaMode, DashCap cap, const SkMatrix& localMatrix,
...
class DashOp final : public GrMeshDrawOp {
 ...
 private:
  ...
  void onPrepareDraws(Target* target) override {
    ...
    if (this->fullDash()) {
      gp = make_dash_gp(
          this->color(), this->aaMode(), capType, this->viewMatrix(), fUsesLocalCoords);
    } else {
      ...
      gp = MakeForDeviceSpace(
          target->caps().shaderCaps(), color, Coverage::kSolid_Type, localCoordsType,
          this->viewMatrix());
    }
    ...
    for (int i = 0; i < instanceCount; i++) {
      ...
      SkScalar endAdj =
          calc_end_adjustment(draw.fIntervals, draw.fPtsRot, draw.fPhase, &endingInterval);
      ...
      if (draw.fPtsRot[0].fX == draw.fPtsRot[1].fX && (0 != endAdj || 0 == startAdj) && hasCap) {
      ...
      if (!lineDone) {
        ...
        bounds.set(draw.fPtsRot[0].fX, draw.fPtsRot[0].fY, draw.fPtsRot[1].fX, draw.fPtsRot[1].fY);
    ...
    for (int i = 0; i < instanceCount; i++) {
      ...
      if (!draws[i].fLineDone) {
        if (fullDash) {
          setup_dashed_rect(
              rects[rectIndex], vertices, geom.fSrcRotInv, draws[i].fStartOffset,
              draws[i].fDevBloatX, draws[i].fDevBloatY, draws[i].fLineLength,
              draws[i].fHalfDevStroke, draws[i].fIntervals[0], draws[i].fIntervals[1],
              draws[i].fStrokeWidth, capType);
        } else {
      ...
      if (draws[i].fHasStartRect) {
        if (fullDash) {
          setup_dashed_rect(
              rects[rectIndex], vertices, geom.fSrcRotInv, draws[i].fStartOffset,
              draws[i].fDevBloatX, draws[i].fDevBloatY, draws[i].fIntervals[0],
              draws[i].fHalfDevStroke, draws[i].fIntervals[0], draws[i].fIntervals[1],
              draws[i].fStrokeWidth, capType);
        } else {
          vertices.writeQuad(GrQuad::MakeFromRect(rects[rectIndex], geom.fSrcRotInv));
...
std::unique_ptr<GrDrawOp> GrDashOp::MakeDashLineOp(
  ...
  calc_dash_scaling(
      &lineData.fParallelScale, &lineData.fPerpendicularScale, viewMatrix, lineData.fPtsRot);
...
void GLDashingCircleEffect::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
  ...
  // emit transforms
  this->emitTransforms(
      vertBuilder, varyingHandler, uniformHandler, dce.fInPosition.asShaderVar(), dce.localMatrix(),
      args.fFPCoordTransformHandler);

  // transforms all points so that we can compare them to our test circle
  fragBuilder->codeAppendf(
      "half xShifted = half(%s.x - floor(%s.x / %s.z) * %s.z);", dashParams.fsIn(),
      dashParams.fsIn(), dashParams.fsIn(), dashParams.fsIn());
  fragBuilder->codeAppendf(
      "half2 fragPosShifted = half2(xShifted, half(%s.y));", dashParams.fsIn());
...
void GLDashingCircleEffect::setData(
...
void GLDashingCircleEffect::GenKey(
...
sk_sp<GrGeometryProcessor> DashingCircleEffect::Make(
    const SkPMColor4f& color, AAMode aaMode, const SkMatrix& localMatrix, bool usesLocalCoords) {
...
sk_sp<GrGeometryProcessor> DashingCircleEffect::TestCreate(GrProcessorTestData* d) {
  ...
  return DashingCircleEffect::Make(
      SkPMColor4f::FromBytes_RGBA(GrRandomColor(d->fRandom)), aaMode,
 ...
 * This effect also requires the setting of a float2 vertex attribute for the the four corners of
 * the bounding rect. This attribute is the "dash position" of each vertex. In other words it is the
...
void GLDashingLineEffect::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
  ...
  // emit transforms
  this->emitTransforms(
      vertBuilder, varyingHandler, uniformHandler, de.fInPosition.asShaderVar(), de.localMatrix(),
      args.fFPCoordTransformHandler);

  // transforms all points so that we can compare them to our test rect
  fragBuilder->codeAppendf(
      "half xShifted = half(%s.x - floor(%s.x / %s.z) * %s.z);", inDashParams.fsIn(),
      inDashParams.fsIn(), inDashParams.fsIn(), inDashParams.fsIn());
  fragBuilder->codeAppendf(
      "half2 fragPosShifted = half2(xShifted, half(%s.y));", inDashParams.fsIn());
  if (de.aaMode() == AAMode::kCoverage) {
    // The amount of coverage removed in x and y by the edges is computed as a pair of negative
    // numbers, xSub and ySub.
    fragBuilder->codeAppend("half xSub, ySub;");
    fragBuilder->codeAppendf(
        "xSub = half(min(fragPosShifted.x - %s.x, 0.0));", inRectParams.fsIn());
    fragBuilder->codeAppendf(
        "xSub += half(min(%s.z - fragPosShifted.x, 0.0));", inRectParams.fsIn());
    fragBuilder->codeAppendf(
        "ySub = half(min(fragPosShifted.y - %s.y, 0.0));", inRectParams.fsIn());
    fragBuilder->codeAppendf(
        "ySub += half(min(%s.w - fragPosShifted.y, 0.0));", inRectParams.fsIn());
    ...
    fragBuilder->codeAppendf("half alpha = 1.0;");
    fragBuilder->codeAppendf(
        "alpha *= (fragPosShifted.x - %s.x) > -0.5 ? 1.0 : 0.0;", inRectParams.fsIn());
    fragBuilder->codeAppendf(
        "alpha *= (%s.z - fragPosShifted.x) >= -0.5 ? 1.0 : 0.0;", inRectParams.fsIn());
  }
  fragBuilder->codeAppendf("%s = half4(alpha);", args.fOutputCoverage);
}

void GLDashingLineEffect::setData(
    const GrGLSLProgramDataManager& pdman, const GrPrimitiveProcessor& processor,
    FPCoordTransformIter&& transformIter) {
...
void GLDashingLineEffect::GenKey(
    const GrGeometryProcessor& gp, const GrShaderCaps&, GrProcessorKeyBuilder* b) {
...
sk_sp<GrGeometryProcessor> DashingLineEffect::Make(
    const SkPMColor4f& color, AAMode aaMode, const SkMatrix& localMatrix, bool usesLocalCoords) {
  return sk_sp<GrGeometryProcessor>(
...
static sk_sp<GrGeometryProcessor> make_dash_gp(
    const SkPMColor4f& color, AAMode aaMode, DashCap cap, const SkMatrix& viewMatrix,
    bool usesLocalCoords) {
  SkMatrix invert;
  ...
  switch (cap) {
    case kRound_DashCap: return DashingCircleEffect::Make(color, aaMode, invert, usesLocalCoords);
    case kNonRound_DashCap: return DashingLineEffect::Make(color, aaMode, invert, usesLocalCoords);
  }
  return nullptr;
}
...
GR_DRAW_OP_TEST_DEFINE(DashOp) {
  ...
  return GrDashOp::MakeDashLineOp(
      context, std::move(paint), viewMatrix, pts, aaMode, style,
