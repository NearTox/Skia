 ...
 */
GrGeometryProcessor* make_dash_gp(
...
class DashOpImpl final : public GrMeshDrawOp {
 public:
  ...
  GrProcessorSet::Analysis finalize(
    ...
    auto analysis =
        fProcessorSet.finalize(fColor, coverage, clip, fStencilSettings, caps, clampType, &fColor);
 ...
 private:
  ...
  void onCreateProgramInfo(
    ...
    fProgramInfo = GrSimpleMeshDrawOpHelper::CreateProgramInfo(
        ...
        GrPipeline::InputFlags::kNone, fStencilSettings);
  }
...
void DashingCircleEffect::Impl::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
  ...
  if (dce.fUsesLocalCoords) {
    WriteLocalCoord(
        vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, dce.fInPosition.asShaderVar(),
        dce.fLocalMatrix, &fLocalMatrixUniform);
  }
...
void DashingCircleEffect::Impl::setData(
...
GrGeometryProcessor* DashingCircleEffect::TestCreate(GrProcessorTestData* d) {
  ...
  return DashingCircleEffect::Make(
      d->allocator(), SkPMColor4f::FromBytes_RGBA(color), aaMode, matrix, d->fRandom->nextBool());
}
#endif
...
void DashingLineEffect::Impl::onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) {
  ...
  if (de.usesLocalCoords()) {
    WriteLocalCoord(
        vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, de.fInPosition.asShaderVar(),
        de.fLocalMatrix, &fLocalMatrixUniform);
  }
...
void DashingLineEffect::Impl::setData(
    const GrGLSLProgramDataManager& pdman, const GrShaderCaps& shaderCaps,
    const GrGeometryProcessor& geomProc) {
...
GR_DRAW_OP_TEST_DEFINE(DashOpImpl) {
  ...
  return skgpu::v1::DashOp::MakeDashLineOp(
...
GrOp::Owner MakeDashLineOp(
  ...
  return DashOpImpl::Make(
      context, std::move(paint), lineData, cap, aaMode, fullDash, stencilSettings);
}
