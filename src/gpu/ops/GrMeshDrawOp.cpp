...
GrMeshDrawOp::PatternHelper::PatternHelper(
    Target* target, GrPrimitiveType primitiveType, size_t vertexStride,
...
void GrMeshDrawOp::PatternHelper::init(
  ...
  fMesh->setIndexedPatterned(
      std::move(indexBuffer), indicesPerRepetition, verticesPerRepetition, repeatCount,
      maxRepetitions);
...
GrPipeline::DynamicStateArrays* GrMeshDrawOp::Target::AllocDynamicStateArrays(
    SkArenaAlloc* arena, int numMeshes, int numPrimitiveProcessorTextures, bool allocScissors) {
  ...
  if (numPrimitiveProcessorTextures) {
    result->fPrimitiveProcessorTextures =
        arena->makeArrayDefault<GrSurfaceProxy*>(numPrimitiveProcessorTextures * numMeshes);
  }
...
GrPipeline::FixedDynamicState* GrMeshDrawOp::Target::MakeFixedDynamicState(
  ...
  if (haveScissor || numPrimProcTextures) {
    ...
    if (numPrimProcTextures) {
      result->fPrimitiveProcessorTextures =
          arena->makeArrayDefault<GrSurfaceProxy*>(numPrimProcTextures);
    }
    return result;
  }
  return nullptr;
