...
template <typename T>
bool refs_atlas(const T* proxyOwner, const GrSurfaceProxy* atlasProxy) {
...
bool AtlasPathRenderer::pathFitsInAtlas(
    const SkRect& pathDevBounds, GrAAType fallbackAAType) const {
  SkASSERT(fallbackAAType != GrAAType::kNone);  // The atlas doesn't support non-AA.
  float atlasMaxPathHeight_pow2 =
      (fallbackAAType == GrAAType::kMSAA)
...
void AtlasPathRenderer::AtlasPathKey::set(const SkMatrix& m, const SkPath& path) {
  ...
  float2 subpixelPositionKey =
      skvx::trunc(subpixelPosition * GrTessellationShader::kLinearizationPrecision);
...
bool AtlasPathRenderer::addPathToAtlas(
    GrRecordingContext* rContext, const SkMatrix& viewMatrix, const SkPath& path,
    const SkRect& pathDevBounds, SkIRect* devIBounds, SkIPoint16* locationInAtlas,
    bool* transposedInAtlas, const DrawRefsAtlasCallback& drawRefsAtlasCallback) {
  ...
  if (fAtlasRenderTasks.empty() || !fAtlasRenderTasks.back()->addPath(
                                       viewMatrix, path, devIBounds->topLeft(), widthInAtlas,
    ...
    auto currentAtlasTask = (!fAtlasRenderTasks.empty()) ? fAtlasRenderTasks.back().get() : nullptr;
    if (currentAtlasTask && drawRefsAtlasCallback &&
    ...
    auto dynamicAtlas = std::make_unique<GrDynamicAtlas>(
        ...
        SkISize{fAtlasInitialSize, fAtlasInitialSize}, fAtlasMaxSize, *rContext->priv().caps(),
        kAtlasAlgorithm);
    auto newAtlasTask =
        sk_make_sp<AtlasRenderTask>(rContext, sk_make_sp<GrArenas>(), std::move(dynamicAtlas));
    rContext->priv().drawingManager()->addAtlasTask(newAtlasTask, currentAtlasTask);
    SkAssertResult(newAtlasTask->addPath(
        viewMatrix, path, devIBounds->topLeft(), widthInAtlas, heightInAtlas, *transposedInAtlas,
        locationInAtlas));
...
#endif
      !args.fShape->style().hasPathEffect() && !args.fViewMatrix->hasPerspective() &&
      this->pathFitsInAtlas(args.fViewMatrix->mapRect(args.fShape->bounds()), args.fAAType);
...
bool AtlasPathRenderer::onDrawPath(const DrawPathArgs& args) {
  ...
  if (!is_visible(pathDevBounds, args.fClip->getConservativeBounds())) {
    ...
    if (path.isInverseFillType()) {
      args.fSurfaceDrawContext->drawPaint(args.fClip, std::move(args.fPaint), *args.fViewMatrix);
    }
    return true;
  }
  ...
  SkAssertResult(this->addPathToAtlas(
      args.fContext, *args.fViewMatrix, path, pathDevBounds, &devIBounds, &locationInAtlas,
      &transposedInAtlas, nullptr /*DrawRefsAtlasCallback -- see onCanDrawPath()*/));

  const SkIRect& fillBounds =
      args.fShape->inverseFilled()
          ? (args.fClip ? args.fClip->getConservativeBounds()
                        : args.fSurfaceDrawContext->asSurfaceProxy()->backingStoreBoundsIRect())
          : devIBounds;
  const GrCaps& caps = *args.fSurfaceDrawContext->caps();
  auto op = GrOp::Make<DrawAtlasPathOp>(
      args.fContext, args.fSurfaceDrawContext->arenaAlloc(), fillBounds, *args.fViewMatrix,
      std::move(args.fPaint), locationInAtlas, devIBounds, transposedInAtlas,
      fAtlasRenderTasks.back()->readView(caps), args.fShape->inverseFilled());
  args.fSurfaceDrawContext->addDrawOp(args.fClip, std::move(op));
  return true;
}

GrFPResult AtlasPathRenderer::makeAtlasClipEffect(
    const SurfaceDrawContext* sdc, const GrOp* opBeingClipped,
    std::unique_ptr<GrFragmentProcessor> inputFP, const SkIRect& drawBounds,
  ...
  auto fallbackAAType =
      (sdc->numSamples() > 1 || sdc->canUseDynamicMSAA()) ? GrAAType::kMSAA : GrAAType::kCoverage;
  ...
  auto drawRefsAtlasCallback = [opBeingClipped, &inputFP](const GrSurfaceProxy* atlasProxy) {
    return refs_atlas(opBeingClipped, atlasProxy) || refs_atlas(inputFP.get(), atlasProxy);
  };
  // addPathToAtlas() ignores inverseness of the fill. See GrAtlasRenderTask::getAtlasUberPath().
  if (!this->addPathToAtlas(
          sdc->recordingContext(), viewMatrix, path, pathDevBounds, &devIBounds, &locationInAtlas,
          &transposedInAtlas, drawRefsAtlasCallback)) {
  ...
  if (!transposedInAtlas) {
    ...
    atlasMatrix.setAll(0, 1, atlasX - devIBounds.top(), 1, 0, atlasY - devIBounds.left(), 0, 0, 1);
  }
  ...
  return GrFPSuccess(std::make_unique<GrModulateAtlasCoverageEffect>(
      flags, std::move(inputFP), std::move(atlasView), atlasMatrix, devIBounds));
}

void AtlasPathRenderer::preFlush(
  ...
  for (int i = 1; i < fAtlasRenderTasks.count(); ++i) {
    ...
    if (atlasTask->atlasProxy()->backingStoreDimensions() == firstAtlasTexture->dimensions()) {
      ...
      SkASSERT(
          atlasTask->atlasProxy()->backingStoreDimensions().area() <
