...
void GrAtlasInstancedHelper::writeInstanceData(
    GrVertexWriter* instanceWriter, const Instance* i) const {
  ...
  instanceWriter->write(
      ...
      (float)i->fLocationInAtlas.y(), (float)i->fPathDevIBounds.left(),
      (float)i->fPathDevIBounds.top(),
      GrVertexWriter::If(
          fShaderFlags & ShaderFlags::kCheckBounds, SkSize::Make(i->fPathDevIBounds.size())));
}

void GrAtlasInstancedHelper::injectShaderCode(
    const GrGeometryProcessor::ProgramImpl::EmitArgs& args, const GrShaderVar& devCoord,
  ...
  *atlasAdjustUniformHandle = args.fUniformHandler->addUniform(
  ...
  if (fShaderFlags & ShaderFlags::kCheckBounds) {
    ...
    args.fVaryingHandler->addVarying(
        "atlasbounds", &atlasBounds, GrGLSLVaryingHandler::Interpolation::kCanBeFlat);
    args.fVertBuilder->codeAppendf(
        R"(
