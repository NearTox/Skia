...
GrAtlasTextOp::GrAtlasTextOp(
    MaskType maskType, GrPaint&& paint, GrTextBlob::SubRun* subrun, const SkMatrix& drawMatrix,
  ...
  geometry.fColor = subrun->maskFormat() == kARGB_GrMaskFormat ? SK_PMColor4fWHITE : filteredColor;
  fGeoCount = 1;
...
std::unique_ptr<GrAtlasTextOp> GrAtlasTextOp::MakeBitmap(
    GrRecordingContext* context, GrPaint&& paint, GrTextBlob::SubRun* subrun,
  ...
  return pool->allocate<GrAtlasTextOp>(
      maskType, std::move(paint), subrun, drawMatrix, drawOrigin, clipRect, filteredColor, 0, false,
      0);
}
...
std::unique_ptr<GrAtlasTextOp> GrAtlasTextOp::MakeDistanceField(
    GrRecordingContext* context, GrPaint&& paint, GrTextBlob::SubRun* subrun,
    const SkMatrix& drawMatrix, SkPoint drawOrigin, const SkIRect& clipRect,
    const SkPMColor4f& filteredColor, bool useGammaCorrectDistanceTable, SkColor luminanceColor,
  ...
  return pool->allocate<GrAtlasTextOp>(
      maskType, std::move(paint), subrun, drawMatrix, drawOrigin, clipRect, filteredColor,
      luminanceColor, useGammaCorrectDistanceTable, DFGPFlags);
}
...
void GrAtlasTextOp::onPrepareDraws(Target* target) {
  ...
  if (this->usesDistanceFields()) {
    ...
    auto filter =
        fNeedsGlyphTransform ? GrSamplerState::Filter::kBilerp : GrSamplerState::Filter::kNearest;
  ...
  void* vertices = target->makeVertexSpace(
      vertexStride, kVerticesPerGlyph * (quadBufferEnd - quadBufferBegin), &flushInfo.fVertexBuffer,
  ...
  for (int i = 0; i < fGeoCount; i++) {
    ...
    GrTextBlob::VertexRegenerator regenerator(
        resourceProvider, subRun, target->deferredUploadTarget(), atlasManager);
    ...
    while (totalGlyphsRegened < subRunEnd) {
      ...
      if (glyphsRegenerated > 0) {
        ...
        if (args.fClipRect.isEmpty()) {
          memcpy(
              regeneratedQuadBuffer, subRun->quadStart(subRunIndex), glyphsRegenerated * quadSize);
        } else {
          SkASSERT(!vmPerspective);
          clip_quads(
              args.fClipRect, regeneratedQuadBuffer, subRun->quadStart(subRunIndex), vertexStride,
              glyphsRegenerated);
        }
        if (fNeedsGlyphTransform && !args.fDrawMatrix.isIdentity()) {
          ...
          if (args.fDrawMatrix.hasPerspective()) {
            ...
            SkMatrixPriv::MapPointsWithStride(
                args.fDrawMatrix, pos, vertexStride, glyphsRegenerated * kVerticesPerGlyph);
          }
        }
      }
      ...
      if (totalGlyphsRegened != subRunEnd) {
        ...
        if (totalGlyphsRegened == quadBufferEnd) {
          ...
          int quadBufferSize = std::min(maxQuadsPerBuffer, this->numGlyphs() - totalGlyphsRegened);
          quadBufferEnd = quadBufferBegin + quadBufferSize;

          vertices = target->makeVertexSpace(
              vertexStride, kVerticesPerGlyph * quadBufferSize, &flushInfo.fVertexBuffer,
              &flushInfo.fVertexOffset);
          if (!vertices || !flushInfo.fVertexBuffer) {
            SkDebugf("Could not allocate vertices\n");
            return;
...
void GrAtlasTextOp::createDrawForGeneratedGlyphs(
  ...
  mesh->setIndexedPatterned(
      flushInfo->fIndexBuffer, kIndicesPerGlyph, flushInfo->fGlyphsToFlush, maxGlyphsPerDraw,
...
GrGeometryProcessor* GrAtlasTextOp::setupDfProcessor(
  ...
  if (isLCD) {
    ...
    return GrDistanceFieldLCDTextGeoProc::Make(
        arena, caps, views, numActiveViews, GrSamplerState::Filter::kBilerp, widthAdjust,
    ...
    return GrDistanceFieldA8TextGeoProc::Make(
        arena, caps, views, numActiveViews, GrSamplerState::Filter::kBilerp, correction, fDFGPFlags,
        localMatrix);
#else
    return GrDistanceFieldA8TextGeoProc::Make(
        arena, caps, views, numActiveViews, GrSamplerState::Filter::kBilerp, fDFGPFlags,
        localMatrix);
#endif
  }
}
