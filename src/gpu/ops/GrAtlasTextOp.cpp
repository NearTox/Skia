...
std::unique_ptr<GrDrawOp> GrAtlasTextOp::CreateOpTestingOnly(
  ...
  painter->processGlyphRunList(
      ...
      contextPriv.caps()->shaderCaps()->supportsDistanceFieldText(), SDFOptions, blob.get());

  return blob->firstSubRun()->makeOp(
      mtxProvider, drawOrigin, SkIRect::MakeEmpty(), skPaint, surfaceProps, rtc->textTarget());
}
