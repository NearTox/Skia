...
std::unique_ptr<GrAtlasTextOp> GrAtlasTextOp::MakeBitmap(
    GrRecordingContext* context, GrPaint&& paint, GrMaskFormat maskFormat, int glyphCount,
    bool needsTransform) {
  ...
  switch (maskFormat) {
    case kA8_GrMaskFormat: op->fMaskType = kGrayscaleCoverageMask_MaskType; break;
    case kA565_GrMaskFormat: op->fMaskType = kLCDCoverageMask_MaskType; break;
    case kARGB_GrMaskFormat: op->fMaskType = kColorBitmapMask_MaskType; break;
  }
...
std::unique_ptr<GrAtlasTextOp> GrAtlasTextOp::MakeDistanceField(
    GrRecordingContext* context, GrPaint&& paint, int glyphCount,
    const GrDistanceFieldAdjustTable* distanceAdjustTable, bool useGammaCorrectDistanceTable,
    SkColor luminanceColor, const SkSurfaceProps& props, bool isAntiAliased, bool useLCD) {
  GrOpMemoryPool* pool = context->priv().opMemoryPool();
  ...
  op->fMaskType = !isAntiAliased
                      ? kAliasedDistanceField_MaskType
                      : isLCD ? (isBGR ? kLCDBGRDistanceField_MaskType : kLCDDistanceField_MaskType)
...
void GrAtlasTextOp::init() {
  ...
  if (this->usesDistanceFields()) {
    ...
    fDFGPFlags |=
        (kAliasedDistanceField_MaskType == fMaskType) ? kAliased_DistanceFieldEffectFlag : 0;

    if (isLCD) {
      fDFGPFlags |= kUseLCD_DistanceFieldEffectFlag;
      fDFGPFlags |= (kLCDBGRDistanceField_MaskType == fMaskType) ? kBGR_DistanceFieldEffectFlag : 0;
    }
  ...
  geo.fBlob->computeSubRunBounds(
      &bounds, *geo.fSubRunPtr, geo.fViewMatrix, geo.fX, geo.fY, fNeedsGlyphTransform);
...
static void clip_quads(
  ...
  for (int i = 0; i < glyphCount; ++i) {
    ...
    SkIRect positionRect = SkIRect::MakeLTRB(
        SkScalarRoundToInt(blobPositionLT->fX), SkScalarRoundToInt(blobPositionLT->fY),
        SkScalarRoundToInt(blobPositionRB->fX), SkScalarRoundToInt(blobPositionRB->fY));
    if (clipRect.contains(positionRect)) {
      ...
      auto* blobCoordsRB =
          reinterpret_cast<const uint16_t*>(blobVertices + 3 * vertexStride + coordOffset);
...
void GrAtlasTextOp::onPrepareDraws(Target* target) {
  ...
  if (this->usesDistanceFields()) {
    flushInfo.fGeometryProcessor = this->setupDfProcessor(
        target->allocator(), *target->caps().shaderCaps(), proxies, numActiveProxies);
  } else {
    GrSamplerState samplerState =
        fNeedsGlyphTransform ? GrSamplerState::ClampBilerp() : GrSamplerState::ClampNearest();
    flushInfo.fGeometryProcessor = GrBitmapTextGeoProc::Make(
        target->allocator(), *target->caps().shaderCaps(), this->color(), false, proxies,
        numActiveProxies, samplerState, maskFormat, localMatrix, vmPerspective);
  }
  ...
  void* vertices = target->makeVertexSpace(
      vertexStride, glyphCount * kVerticesPerGlyph, &flushInfo.fVertexBuffer,
  ...
  for (int i = 0; i < fGeoCount; i++) {
    ...
    GrTextBlob::VertexRegenerator regenerator(
        ...
        args.fColor.toBytes_RGBA(), target->deferredUploadTarget(), glyphCache, atlasManager);
    bool done = false;
    while (!done) {
      ...
      if (args.fClipRect.isEmpty()) {
        ...
        clip_quads(
            args.fClipRect, currVertex, result.fFirstVertex, vertexStride,
      ...
      if (fNeedsGlyphTransform && !args.fViewMatrix.isIdentity()) {
        ...
        if (args.fViewMatrix.hasPerspective()) {
          ...
          SkMatrixPriv::MapPointsWithStride(
              args.fViewMatrix, pos, vertexStride, result.fGlyphsRegenerated * kVerticesPerGlyph);
        }
      }
  ...
  if (gp->numTextureSamplers() != (int)numActiveProxies) {
    ...
    if (this->usesDistanceFields()) {
      ...
      reinterpret_cast<GrBitmapTextGeoProc*>(gp)->addNewProxies(
          proxies, numActiveProxies, samplerState);
    }
  }
  int maxGlyphsPerDraw = static_cast<int>(flushInfo->fIndexBuffer->size() / sizeof(uint16_t) / 6);
  GrMesh* mesh = target->allocMesh(GrPrimitiveType::kTriangles);
  mesh->setIndexedPatterned(
      flushInfo->fIndexBuffer, kIndicesPerGlyph, kVerticesPerGlyph, flushInfo->fGlyphsToFlush,
      maxGlyphsPerDraw);
...
GrGeometryProcessor* GrAtlasTextOp::setupDfProcessor(
  ...
  if (isLCD) {
    ...
    float greenCorrection = fDistanceAdjustTable->getAdjustment(
        SkColorGetG(fLuminanceColor) >> kDistanceAdjustLumShift, fUseGammaCorrectDistanceTable);
    float blueCorrection = fDistanceAdjustTable->getAdjustment(
        SkColorGetB(fLuminanceColor) >> kDistanceAdjustLumShift, fUseGammaCorrectDistanceTable);
    GrDistanceFieldLCDTextGeoProc::DistanceAdjust widthAdjust =
        GrDistanceFieldLCDTextGeoProc::DistanceAdjust::Make(
            redCorrection, greenCorrection, blueCorrection);
    return GrDistanceFieldLCDTextGeoProc::Make(
        arena, caps, proxies, numActiveProxies, GrSamplerState::ClampBilerp(), widthAdjust,
    ...
    if (kAliasedDistanceField_MaskType != fMaskType) {
      ...
      correction = fDistanceAdjustTable->getAdjustment(
          lum >> kDistanceAdjustLumShift, fUseGammaCorrectDistanceTable);
    }
    return GrDistanceFieldA8TextGeoProc::Make(
        arena, caps, proxies, numActiveProxies, GrSamplerState::ClampBilerp(), correction,
        fDFGPFlags, localMatrix);
#else
    return GrDistanceFieldA8TextGeoProc::Make(
        arena, caps, proxies, numActiveProxies, GrSamplerState::ClampBilerp(), fDFGPFlags,
        localMatrix);
#endif
  }
}
