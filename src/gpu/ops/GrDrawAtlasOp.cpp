...
static sk_sp<GrGeometryProcessor> make_gp(
    const GrShaderCaps* shaderCaps, bool hasColors, const SkPMColor4f& color,
  ...
  return GrDefaultGeoProcFactory::Make(
      shaderCaps, gpColor, Coverage::kSolid_Type, LocalCoords::kHasExplicit_Type, viewMatrix);
}
...
void DrawAtlasOp::onPrepareDraws(Target* target) {
  ...
  sk_sp<GrGeometryProcessor> gp(
      make_gp(target->caps().shaderCaps(), this->hasColors(), this->color(), this->viewMatrix()));
...
std::unique_ptr<GrDrawOp> GrDrawAtlasOp::Make(
