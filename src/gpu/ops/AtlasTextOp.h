...
class AtlasTextOp final : public GrMeshDrawOp {
 public:
  ...
  struct Geometry {
    Geometry(
        const GrAtlasSubRun& subRun, const SkMatrix& drawMatrix, SkPoint drawOrigin,
        SkIRect clipRect, sk_sp<GrTextBlob> blob, GrAtlasSubRunOwner subRunOwner,
    ...
    static Geometry* MakeForBlob(
        const GrAtlasSubRun& subRun, const SkMatrix& drawMatrix, SkPoint drawOrigin,
        SkIRect clipRect, sk_sp<GrTextBlob> blob, const SkPMColor4f& color, SkArenaAlloc* alloc);
  ...
  static GrOp::Owner CreateOpTestingOnly(
      skgpu::v1::SurfaceDrawContext*, const SkPaint&, const SkFont&, const SkMatrixProvider&,
 ...
 private:
  ...
  GrMaskFormat maskFormat() const {
    switch (this->maskType()) {
      case MaskType::kLCDCoverage: return kA565_GrMaskFormat;
      case MaskType::kColorBitmap: return kARGB_GrMaskFormat;
      ...
      case MaskType::kLCDBGRDistanceField: return kA8_GrMaskFormat;
    }
    // SkUNREACHABLE;
  ...
  inline void createDrawForGeneratedGlyphs(GrMeshDrawTarget* target, FlushInfo* flushInfo) const;
  ...
  GrGeometryProcessor* setupDfProcessor(
      SkArenaAlloc*, const GrShaderCaps&, const SkMatrix& localMatrix,
