...
class StaticVertexAllocator : public GrEagerVertexAllocator {
 public:
  StaticVertexAllocator(GrResourceProvider* resourceProvider, bool canMapVB)
      : fResourceProvider(resourceProvider), fCanMapVB(canMapVB) {}
  ...
  void unlock(int actualCount) override {
    ...
    fVertexData =
        GrThreadSafeCache::MakeVertexData(std::move(fVertexBuffer), actualCount, fLockStride);
...
#ifdef SK_DEBUG
  ~CpuVertexAllocator() override { SkASSERT(!fLockStride && !fVertices && !fVertexData); }
#endif
...
GrPathRenderer::CanDrawPath GrTriangulatingPathRenderer::onCanDrawPath(
...
class TriangulatingPathOp final : public GrMeshDrawOp {
 ...
 public:
  ...
  GrProcessorSet::Analysis finalize(
    ...
    GrProcessorAnalysisCoverage coverage = fAntiAlias ? GrProcessorAnalysisCoverage::kSingleChannel
 ...
 private:
  ...
  static void CreateKey(
      GrUniqueKey* key, const GrStyledShape& shape, const SkIRect& devClipBounds) {
  ...
  static int Triangulate(
      GrEagerVertexAllocator* allocator, const SkMatrix& viewMatrix, const GrStyledShape& shape,
      const SkIRect& devClipBounds, SkScalar tol, bool* isLinear) {
    ...
    return GrTriangulator::PathToTriangles(
        path, tol, clipBounds, allocator, GrTriangulator::Mode::kNormal, isLinear);
  }
  ...
  void createNonAAMesh(Target* target) {
    ...
    SkScalar tol = GrPathUtils::scaleToleranceToSrc(
        GrPathUtils::kDefaultTolerance, fViewMatrix, fShape.bounds());
    ...
    if (fVertexData) {
      if (!fVertexData->gpuBuffer()) {
        sk_sp<GrGpuBuffer> buffer = rp->createBuffer(
    ...
    bool isLinear;
    int vertexCount = Triangulate(&allocator, fViewMatrix, fShape, fDevClipBounds, tol, &isLinear);
    if (vertexCount == 0) {
      return;
    }
    ...
    GrGeometryProcessor* gp;
    {
      ...
      if (fAntiAlias) {
        gp = GrDefaultGeoProcFactory::MakeForDeviceSpace(
            arena, color, coverageType, localCoordsType, fViewMatrix);
      } else {
        gp =
            GrDefaultGeoProcFactory::Make(arena, color, coverageType, localCoordsType, fViewMatrix);
      }
    }
    if (!gp) {
      return;
...
#ifdef SK_DEBUG
    auto mode = (fAntiAlias) ? GrTriangulator::Mode::kEdgeAntialias : GrTriangulator::Mode::kNormal;
  ...
  static GrSimpleMesh* CreateMesh(
      Target* target, sk_sp<const GrBuffer> vb, int firstVertex, int count) {
