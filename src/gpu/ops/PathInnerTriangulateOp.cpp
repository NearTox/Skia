 ...
 public:
  HullShader(const SkMatrix& viewMatrix, SkPMColor4f color, const GrShaderCaps& shaderCaps)
      : GrPathTessellationShader(
            kTessellate_HullShader_ClassID, GrPrimitiveType::kTriangleStrip, 0, viewMatrix, color) {
    ...
    if (!shaderCaps.vertexIDSupport()) {
      constexpr static Attribute kVertexIdxAttrib(
          "vertexidx", kFloat_GrVertexAttribType, kFloat_GrSLType);
...
void PathInnerTriangulateOp::prePreparePrograms(
  ...
  if (!isLinear) {
    fTessellator = GrPathCurveTessellator::Make(
        args.fArena, fViewMatrix, SK_PMColor4fTRANSPARENT,
        GrPathCurveTessellator::DrawInnerFan::kNo, fPath.countVerbs(), *pipelineForStencils,
        *args.fCaps);
    ...
    fStencilCurvesProgram = GrTessellationShader::MakeProgram(
        args, fTessellator->shader(), pipelineForStencils, stencilPathSettings);
  }
...
void PathInnerTriangulateOp::onPrepare(GrOpFlushState* flushState) {
  ...
  if (fTessellator) {
    ...
    fTessellator->prepare(
        flushState, this->bounds(), {SkMatrix::I(), fPath}, fPath.countVerbs(), &fFanBreadcrumbs);
  }
