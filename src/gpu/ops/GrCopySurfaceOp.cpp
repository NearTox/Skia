...
std::unique_ptr<GrOp> GrCopySurfaceOp::Make(
    GrRecordingContext* context, GrSurfaceProxy* dstProxy, GrSurfaceProxy* srcProxy,
  ...
  if (!clip_src_rect_and_dst_point(
          dstProxy, srcProxy, srcRect, dstPoint, &clippedSrcRect, &clippedDstPoint)) {
    return nullptr;
  }
...
void GrCopySurfaceOp::onExecute(GrOpFlushState* state, const SkRect& chainBounds) {
  ...
  state->commandBuffer()->copy(
      fSrc.get()->peekSurface(), fSrc.get()->origin(), fSrcRect, fDstPoint);
}
