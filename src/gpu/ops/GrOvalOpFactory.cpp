...
static inline GrVertexWriter::TriStrip<float> origin_centered_tri_strip(float x, float y) noexcept {
...
class CircleGeometryProcessor : public GrGeometryProcessor {
 public:
  ...
  const char* name() const noexcept override { return "CircleGeometryProcessor"; }
 ...
 private:
  ...
  CircleGeometryProcessor(
      ...
      const SkMatrix& localMatrix) noexcept
      : INHERITED(kCircleGeometryProcessor_ClassID), fLocalMatrix(localMatrix), fStroke(stroke) {
    ...
    if (roundCaps) {
      SkASSERT(stroke);
      SkASSERT(clipPlane);
      fInRoundCapCenters = {"inRoundCapCenters", kFloat4_GrVertexAttribType, kFloat4_GrSLType};
    }
    ...
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
      ...
      if (cgp.fInRoundCapCenters.isInitialized()) {
        ...
        varyingHandler->addVarying(
            "capRadius", &capRadius, GrGLSLVaryingHandler::Interpolation::kCanBeFlat);
        ...
        vertBuilder->codeAppendf(
            "%s = (1.0 - %s.w) / 2.0;", capRadius.vsOut(), cgp.fInCircleEdge.name());
      }
      ...
      // emit transforms
      this->emitTransforms(
          vertBuilder, varyingHandler, uniformHandler, cgp.fInPosition.asShaderVar(),
          cgp.fLocalMatrix, args.fFPCoordTransformHandler);
...
class ButtCapDashedCircleGeometryProcessor : public GrGeometryProcessor {
 ...
 private:
  ...
  ButtCapDashedCircleGeometryProcessor(bool wideColor, const SkMatrix& localMatrix) noexcept
   ...
   public:
    ...
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
      ...
      varyingHandler->addPassThroughAttribute(
          bcscgp.fInDashParams, "dashParams", GrGLSLVaryingHandler::Interpolation::kCanBeFlat);
      GrGLSLVarying wrapDashes(kHalf4_GrSLType);
      varyingHandler->addVarying(
          "wrapDashes", &wrapDashes, GrGLSLVaryingHandler::Interpolation::kCanBeFlat);
      ...
      // setup pass through color
      varyingHandler->addPassThroughAttribute(
          bcscgp.fInColor, args.fOutputColor, GrGLSLVaryingHandler::Interpolation::kCanBeFlat);
      ...
      // emit transforms
      this->emitTransforms(
          vertBuilder, varyingHandler, uniformHandler, bcscgp.fInPosition.asShaderVar(),
          bcscgp.fLocalMatrix, args.fFPCoordTransformHandler);
      GrShaderVar fnArgs[] = {
      ...
      fragBuilder->emitFunction(
          kFloat_GrSLType, "coverage_from_dash_edge", SK_ARRAY_COUNT(fnArgs), fnArgs, R"(
                    float linearDist;
...
class EllipseGeometryProcessor : public GrGeometryProcessor {
 public:
  ...
  const char* name() const noexcept override { return "EllipseGeometryProcessor"; }
 ...
 private:
  ...
  EllipseGeometryProcessor(
      bool stroke, bool wideColor, bool useScale, const SkMatrix& localMatrix) noexcept
  ...
  class GLSLProcessor : public GrGLSLGeometryProcessor {
   public:
    ...
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
      ...
      vertBuilder->codeAppendf("%s = %s;", ellipseOffsets.vsOut(), egp.fInEllipseOffset.name());
      ...
      // emit transforms
      this->emitTransforms(
          vertBuilder, varyingHandler, uniformHandler, egp.fInPosition.asShaderVar(),
          egp.fLocalMatrix, args.fFPCoordTransformHandler);
      ...
      if (egp.fUseScale) {
        fragBuilder->codeAppendf(
            "float2 grad = 2.0*offset*(%s.z*%s.xy);", ellipseOffsets.fsIn(), ellipseRadii.fsIn());
      } else {
      ...
      if (egp.fUseScale) {
        fragBuilder->codeAppendf(
            "float invlen = %s.z*inversesqrt(grad_dot);", ellipseOffsets.fsIn());
      } else {
      ...
      if (egp.fStroke) {
        fragBuilder->codeAppendf(
            "offset = %s.xy*%s.zw;", ellipseOffsets.fsIn(), ellipseRadii.fsIn());
        fragBuilder->codeAppend("test = dot(offset, offset) - 1.0;");
        if (egp.fUseScale) {
          fragBuilder->codeAppendf(
              "grad = 2.0*offset*(%s.z*%s.zw);", ellipseOffsets.fsIn(), ellipseRadii.fsIn());
        } else {
        ...
        if (egp.fUseScale) {
          fragBuilder->codeAppendf("invlen = %s.z*inversesqrt(grad_dot);", ellipseOffsets.fsIn());
        } else {
...
class DIEllipseGeometryProcessor : public GrGeometryProcessor {
 public:
  ...
  const char* name() const noexcept override { return "DIEllipseGeometryProcessor"; }
 ...
 private:
  ...
  DIEllipseGeometryProcessor(
      bool wideColor, bool useScale, const SkMatrix& viewMatrix, DIEllipseStyle style) noexcept
  ...
  class GLSLProcessor : public GrGLSLGeometryProcessor {
   public:
    ...
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
      ...
      // Setup position
      this->writeOutputPosition(
          vertBuilder, uniformHandler, gpArgs, diegp.fInPosition.name(), diegp.fViewMatrix,
          &fViewMatrixUniform);

      // emit transforms
      this->emitTransforms(
          vertBuilder, varyingHandler, uniformHandler, diegp.fInPosition.asShaderVar(),
...
static constexpr SkPoint kOctagonOuter[] = {
    SkPoint::Make(-kOctOffset, -1), SkPoint::Make(kOctOffset, -1),  SkPoint::Make(1, -kOctOffset),
    SkPoint::Make(1, kOctOffset),   SkPoint::Make(kOctOffset, 1),   SkPoint::Make(-kOctOffset, 1),
    SkPoint::Make(-1, kOctOffset),  SkPoint::Make(-1, -kOctOffset),
};
...
static constexpr SkPoint kOctagonInner[] = {
    SkPoint::Make(-kSinPi8, -kCosPi8), SkPoint::Make(kSinPi8, -kCosPi8),
    SkPoint::Make(kCosPi8, -kSinPi8),  SkPoint::Make(kCosPi8, kSinPi8),
    SkPoint::Make(kSinPi8, kCosPi8),   SkPoint::Make(-kSinPi8, kCosPi8),
    SkPoint::Make(-kCosPi8, kSinPi8),  SkPoint::Make(-kCosPi8, -kSinPi8),
};

static constexpr int kIndicesPerFillCircle = SK_ARRAY_COUNT(gFillCircleIndices);
static constexpr int kIndicesPerStrokeCircle = SK_ARRAY_COUNT(gStrokeCircleIndices);
static constexpr int kVertsPerStrokeCircle = 16;
static constexpr int kVertsPerFillCircle = 9;

static int circle_type_to_vert_count(bool stroked) noexcept {
  return stroked ? kVertsPerStrokeCircle : kVertsPerFillCircle;
}

static int circle_type_to_index_count(bool stroked) noexcept {
  return stroked ? kIndicesPerStrokeCircle : kIndicesPerFillCircle;
}

static const uint16_t* circle_type_to_indices(bool stroked) noexcept {
...
class CircleOp final : public GrMeshDrawOp {
 ...
 public:
  ...
  static std::unique_ptr<GrDrawOp> Make(
      GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, SkPoint center,
      SkScalar radius, const GrStyle& style, const ArcParams* arcParams = nullptr) {
    ...
    return Helper::FactoryHelper<CircleOp>(
        context, std::move(paint), viewMatrix, center, radius, style, arcParams);
  }
  ...
  CircleOp(
    ...
    SkRect devBounds = SkRect::MakeLTRB(
        center.fX - outerRadius, center.fY - outerRadius, center.fX + outerRadius,
    ...
    if (arcParams) {
      ...
      fRoundCaps =
          style.strokeRec().getWidth() > 0 && style.strokeRec().getCap() == SkPaint::kRound_Cap;
      SkPoint roundCaps[2];
      if (fRoundCaps) {
      ...
      bool useCenter =
          (arcParams->fUseCenter || isStrokeOnly) && !SkScalarNearlyEqual(absSweep, SK_ScalarPI);
      if (useCenter) {
 ...
 private:
  ...
  void onPrepareDraws(Target* target) override {
    ...
    GrVertexWriter vertices{target->makeVertexSpace(
        fProgramInfo->primProc().vertexStride(), fVertCount, &vertexBuffer, &firstVertex)};
    if (!vertices.fPtr) {
    ...
    for (const auto& circle : fCircles) {
      ...
      if (!circle.fStroked && fClipPlane && fClipPlaneIsect &&
        ...
        geoClipPlane.set(
            circle.fClipPlane[1] - circle.fIsectPlane[1],
      ...
      for (int i = 0; i < 8; ++i) {
        ...
        vertices.write(center + offset * halfWidth, color, offset, radii);
        if (fClipPlane) {
      ...
      if (circle.fStroked) {
        ...
        for (int i = 0; i < 8; ++i) {
          vertices.write(
              center + kOctagonInner[i] * circle.fInnerRadius, color,
              kOctagonInner[i] * innerRadius, radii);
          if (fClipPlane) {
            vertices.write(circle.fClipPlane);
          }
          if (fClipPlaneIsect) {
...
class ButtCapDashedCircleOp final : public GrMeshDrawOp {
 ...
 public:
  ...
  static std::unique_ptr<GrDrawOp> Make(
      GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, SkPoint center,
      SkScalar radius, SkScalar strokeWidth, SkScalar startAngle, SkScalar onAngle,
      SkScalar offAngle, SkScalar phaseAngle) {
    SkASSERT(circle_stays_circle(viewMatrix));
    SkASSERT(strokeWidth < 2 * radius);
    return Helper::FactoryHelper<ButtCapDashedCircleOp>(
        context, std::move(paint), viewMatrix, center, radius, strokeWidth, startAngle, onAngle,
        offAngle, phaseAngle);
  }

  ButtCapDashedCircleOp(
      const Helper::MakeArgs& helperArgs, const SkPMColor4f& color, const SkMatrix& viewMatrix,
      SkPoint center, SkScalar radius, SkScalar strokeWidth, SkScalar startAngle, SkScalar onAngle,
      SkScalar offAngle, SkScalar phaseAngle)
      : GrMeshDrawOp(ClassID()), fHelper(helperArgs, GrAAType::kCoverage) {
    SkASSERT(circle_stays_circle(viewMatrix));
    viewMatrix.mapPoints(&center, 1);
 ...
 private:
  ...
  void onPrepareDraws(Target* target) override {
    ...
    for (const auto& circle : fCircles) {
      ...
      bool reflect = false;
      struct {
        float onAngle, totalAngle, startAngle, phaseAngle;
      } dashParams = {circle.fOnAngle, circle.fTotalAngle, circle.fStartAngle, circle.fPhaseAngle};
      ...
      auto reflectY = [=](const SkPoint& p) { return SkPoint{p.fX, reflect ? -p.fY : p.fY}; };

      for (int i = 0; i < 8; ++i) {
        vertices.write(
            center + kOctagonOuter[i] * halfWidth, color, reflectY(kOctagonOuter[i]),
            circle.fOuterRadius, normInnerRadius, dashParams);
      }
      ...
      for (int i = 0; i < 8; ++i) {
        vertices.write(
            center + kOctagonInner[i] * circle.fInnerRadius, color,
            reflectY(kOctagonInner[i]) * normInnerRadius, circle.fOuterRadius, normInnerRadius,
            dashParams);
      }
...
class EllipseOp : public GrMeshDrawOp {
 ...
 public:
  ...
  static std::unique_ptr<GrDrawOp> Make(
    ...
    params.fXRadius = SkScalarAbs(
        viewMatrix[SkMatrix::kMScaleX] * ellipseXRadius +
        viewMatrix[SkMatrix::kMSkewX] * ellipseYRadius);
    params.fYRadius = SkScalarAbs(
        viewMatrix[SkMatrix::kMSkewY] * ellipseXRadius +
    ...
    scaledStroke.fX =
        SkScalarAbs(strokeWidth * (viewMatrix[SkMatrix::kMScaleX] + viewMatrix[SkMatrix::kMSkewY]));
    scaledStroke.fY =
        SkScalarAbs(strokeWidth * (viewMatrix[SkMatrix::kMSkewX] + viewMatrix[SkMatrix::kMScaleY]));
    ...
    if (hasStroke) {
      ...
      if (scaledStroke.length() > SK_ScalarHalf &&
          (0.5f * params.fXRadius > params.fYRadius || 0.5f * params.fYRadius > params.fXRadius)) {
        return nullptr;
      }
    ...
    constexpr SkScalar kMaxOvalRadius = 16384;
    if (!context->priv().caps()->shaderCaps()->floatIs32Bits() &&
        (params.fXRadius >= kMaxOvalRadius || params.fYRadius >= kMaxOvalRadius)) {
      return nullptr;
    }

    return Helper::FactoryHelper<EllipseOp>(context, std::move(paint), viewMatrix, params, stroke);
  }

  EllipseOp(
      const Helper::MakeArgs& helperArgs, const SkPMColor4f& color, const SkMatrix& viewMatrix,
      const DeviceSpaceParams& params, const SkStrokeRec& stroke)
      : INHERITED(ClassID()), fHelper(helperArgs, GrAAType::kCoverage), fUseScale(false) {
    SkStrokeRec::Style style = stroke.getStyle();
    bool isStrokeOnly =
        SkStrokeRec::kStroke_Style == style || SkStrokeRec::kHairline_Style == style;
  ...
  GrProcessorSet::Analysis finalize(
      ...
      GrClampType clampType) override {
    fUseScale =
        !caps.shaderCaps()->floatIs32Bits() && !caps.shaderCaps()->hasLowFragmentPrecision();
 ...
 private:
  ...
  void onPrepareDraws(Target* target) override {
    ...
    for (const auto& ellipse : fEllipses) {
      ...
      // The inner radius in the vertex data must be specified in normalized space.
      verts.writeQuad(
          GrVertexWriter::TriStripFromRect(ellipse.fDevBounds), color,
          origin_centered_tri_strip(xMaxOffset, yMaxOffset),
          GrVertexWriter::If(fUseScale, std::max(xRadius, yRadius)), invRadii);
    }
    fMesh = helper.mesh();
  }
...
class DIEllipseOp : public GrMeshDrawOp {
 ...
 public:
  ...
  static std::unique_ptr<GrDrawOp> Make(
    ...
    SkStrokeRec::Style style = stroke.getStyle();
    params.fStyle = (SkStrokeRec::kStroke_Style == style)     ? DIEllipseStyle::kStroke
                    : (SkStrokeRec::kHairline_Style == style) ? DIEllipseStyle::kHairline
    ...
    if (SkStrokeRec::kFill_Style != style && SkStrokeRec::kHairline_Style != style) {
      ...
      // we only handle thick strokes for near-circular ellipses
      if (strokeWidth > SK_ScalarHalf && (SK_ScalarHalf * params.fXRadius > params.fYRadius ||
  ...
  DIEllipseOp(
      Helper::MakeArgs& helperArgs, const SkPMColor4f& color, const DeviceSpaceParams& params,
 ...
 private:
  ...
  void onPrepareDraws(Target* target) override {
    ...
    for (const auto& ellipse : fEllipses) {
      ...
      verts.writeQuad(
          GrVertexWriter::TriStripFromRect(ellipse.fBounds), color,
  ...
  const SkMatrix& viewMatrix() const noexcept { return fEllipses[0].fViewMatrix; }
  DIEllipseStyle style() const noexcept { return fEllipses[0].fStyle; }
...
static constexpr uint16_t gOverstrokeRRectIndices[] = {
    // clang-format off
        // overstroke quads
...
static constexpr int kIndicesPerOverstrokeRRect = SK_ARRAY_COUNT(gOverstrokeRRectIndices) - 6;
// fill count skips overstroke indices and includes center
static constexpr int kIndicesPerFillRRect = kIndicesPerOverstrokeRRect - 6 * 4 + 6;
// stroke count is fill count minus center indices
static constexpr int kIndicesPerStrokeRRect = kIndicesPerFillRRect - 6;
static constexpr int kVertsPerStandardRRect = 16;
static constexpr int kVertsPerOverstrokeRRect = 24;
...
static int rrect_type_to_vert_count(RRectType type) noexcept {
  switch (type) {
    case kFill_RRectType:
    case kStroke_RRectType: return kVertsPerStandardRRect;
    case kOverstroke_RRectType: return kVertsPerOverstrokeRRect;
  }
  SK_ABORT("Invalid type");
}

static int rrect_type_to_index_count(RRectType type) noexcept {
  switch (type) {
...
static const uint16_t* rrect_type_to_indices(RRectType type) noexcept {
  switch (type) {
    case kFill_RRectType:
    case kStroke_RRectType: return gStandardRRectIndices;
...
class CircularRRectOp : public GrMeshDrawOp {
 ...
 public:
  ...
  static std::unique_ptr<GrDrawOp> Make(
    ...
    return Helper::FactoryHelper<CircularRRectOp>(
        context, std::move(paint), viewMatrix, devRect, devRadius, devStrokeWidth, strokeOnly);
  }
  CircularRRectOp(
 ...
 private:
  static void FillInOverstrokeVerts(
      GrVertexWriter& verts, const SkRect& bounds, SkScalar smInset, SkScalar bigInset,
      SkScalar xOffset, SkScalar outerRadius, SkScalar innerRadius,
      const GrVertexColor& color) noexcept {
    SkASSERT(smInset < bigInset);

    // TL
    verts.write(
        bounds.fLeft + smInset, bounds.fTop + smInset, color, xOffset, 0.0f, outerRadius,
        innerRadius);

    // TR
    verts.write(
        bounds.fRight - smInset, bounds.fTop + smInset, color, xOffset, 0.0f, outerRadius,
        innerRadius);

    verts.write(
        bounds.fLeft + bigInset, bounds.fTop + bigInset, color, 0.0f, 0.0f, outerRadius,
        innerRadius);

    verts.write(
        bounds.fRight - bigInset, bounds.fTop + bigInset, color, 0.0f, 0.0f, outerRadius,
        innerRadius);

    verts.write(
        bounds.fLeft + bigInset, bounds.fBottom - bigInset, color, 0.0f, 0.0f, outerRadius,
        innerRadius);

    verts.write(
        bounds.fRight - bigInset, bounds.fBottom - bigInset, color, 0.0f, 0.0f, outerRadius,
        innerRadius);

    // BL
    verts.write(
        bounds.fLeft + smInset, bounds.fBottom - smInset, color, xOffset, 0.0f, outerRadius,
        innerRadius);

    // BR
    verts.write(
        bounds.fRight - smInset, bounds.fBottom - smInset, color, xOffset, 0.0f, outerRadius,
        innerRadius);
  }
  ...
  void onPrepareDraws(Target* target) override {
    ...
    GrVertexWriter verts{target->makeVertexSpace(
        fProgramInfo->primProc().vertexStride(), fVertCount, &vertexBuffer, &firstVertex)};
    if (!verts.fPtr) {
      SkDebugf("Could not allocate vertices\n");
      return;
    ...
    for (const auto& rrect : fRRects) {
      ...
      for (int i = 0; i < 4; ++i) {
        verts.write(
            bounds.fLeft, yCoords[i], color, -1.0f, yOuterRadii[i], outerRadius, innerRadius);

        verts.write(
            bounds.fLeft + outerRadius, yCoords[i], color, 0.0f, yOuterRadii[i], outerRadius,
            innerRadius);

        verts.write(
            bounds.fRight - outerRadius, yCoords[i], color, 0.0f, yOuterRadii[i], outerRadius,
            innerRadius);

        verts.write(
            bounds.fRight, yCoords[i], color, 1.0f, yOuterRadii[i], outerRadius, innerRadius);
      }
      ...
      if (kOverstroke_RRectType == rrect.fType) {
        ...
        FillInOverstrokeVerts(
            verts, bounds, outerRadius, overstrokeOuterRadius, maxOffset, overstrokeOuterRadius,
            0.0f, color);
      }
...
static sk_sp<const GrBuffer> get_rrect_index_buffer(
    RRectType type, GrResourceProvider* resourceProvider) {
 ...
 public:
  ...
  static std::unique_ptr<GrDrawOp> Make(
    ...
    return Helper::FactoryHelper<EllipticalRRectOp>(
        context, std::move(paint), viewMatrix, devRect, devXRadius, devYRadius, devStrokeWidths,
        strokeOnly);
  }

  EllipticalRRectOp(
      Helper::MakeArgs helperArgs, const SkPMColor4f& color, const SkMatrix& viewMatrix,
      const SkRect& devRect, float devXRadius, float devYRadius, SkVector devStrokeHalfWidths,
      bool strokeOnly)
      : INHERITED(ClassID()), fHelper(helperArgs, GrAAType::kCoverage), fUseScale(false) {
 ...
 private:
  ...
  void onPrepareDraws(Target* target) override {
    ...
    PatternHelper helper(
        target, GrPrimitiveType::kTriangles, fProgramInfo->primProc().vertexStride(),
        std::move(indexBuffer), kVertsPerStandardRRect, indicesPerInstance, fRRects.count(),
    ...
    for (const auto& rrect : fRRects) {
      ...
      for (int i = 0; i < 4; ++i) {
        verts.write(
            bounds.fLeft, yCoords[i], color, xMaxOffset, yOuterOffsets[i], maybeScale,
            reciprocalRadii);

        verts.write(
            bounds.fLeft + xOuterRadius, yCoords[i], color, SK_ScalarNearlyZero, yOuterOffsets[i],
            maybeScale, reciprocalRadii);

        verts.write(
            bounds.fRight - xOuterRadius, yCoords[i], color, SK_ScalarNearlyZero, yOuterOffsets[i],
            maybeScale, reciprocalRadii);

        verts.write(
            bounds.fRight, yCoords[i], color, xMaxOffset, yOuterOffsets[i], maybeScale,
            reciprocalRadii);
      }
    }
    fMesh = helper.mesh();
...
std::unique_ptr<GrDrawOp> GrOvalOpFactory::MakeCircularRRectOp(
    GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRRect& rrect,
  ...
  SkScalar scaledRadius =
      SkScalarAbs(radius * (viewMatrix[SkMatrix::kMScaleX] + viewMatrix[SkMatrix::kMSkewY]));
  ...
  return CircularRRectOp::Make(
      context, std::move(paint), viewMatrix, bounds, scaledRadius, scaledStroke, isStrokeOnly);
}

static std::unique_ptr<GrDrawOp> make_rrect_op(
    GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRRect& rrect,
  ...
  SkScalar xRadius = SkScalarAbs(
      viewMatrix[SkMatrix::kMScaleX] * radii.fX + viewMatrix[SkMatrix::kMSkewY] * radii.fY);
  SkScalar yRadius = SkScalarAbs(
      viewMatrix[SkMatrix::kMSkewX] * radii.fX + viewMatrix[SkMatrix::kMScaleY] * radii.fY);
  ...
  return EllipticalRRectOp::Make(
      context, std::move(paint), viewMatrix, bounds, xRadius, yRadius, scaledStroke, isStrokeOnly);
}

std::unique_ptr<GrDrawOp> GrOvalOpFactory::MakeRRectOp(
    GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRRect& rrect,
    const SkStrokeRec& stroke, const GrShaderCaps* shaderCaps) {
  if (rrect.isOval()) {
    return MakeOvalOp(
        context, std::move(paint), viewMatrix, rrect.getBounds(), GrStyle(stroke, nullptr),
        shaderCaps);
  }
...
std::unique_ptr<GrDrawOp> GrOvalOpFactory::MakeCircleOp(
    GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRect& oval,
    const GrStyle& style, const GrShaderCaps* shaderCaps) {
  SkScalar width = oval.width();
  SkASSERT(
      width > SK_ScalarNearlyZero && SkScalarNearlyEqual(width, oval.height()) &&
  ...
  if (style.hasNonDashPathEffect()) {
    ...
    if (style.strokeRec().getCap() != SkPaint::kButt_Cap || style.dashIntervalCnt() != 2 ||
        style.strokeRec().getWidth() >= width) {
      return nullptr;
    }
    ...
    if (offInterval == 0) {
      ...
      return MakeOvalOp(context, std::move(paint), viewMatrix, oval, strokeStyle, shaderCaps);
    ...
    // it could.
    static constexpr SkScalar kStartAngle = 0.f;
    return ButtCapDashedCircleOp::Make(
        context, std::move(paint), viewMatrix, center, r, style.strokeRec().getWidth(), kStartAngle,
        angularOnInterval, angularOffInterval, phaseAngle);
  }
  return CircleOp::Make(context, std::move(paint), viewMatrix, center, r, style);
}

std::unique_ptr<GrDrawOp> GrOvalOpFactory::MakeOvalOp(
    GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRect& oval,
    const GrStyle& style, const GrShaderCaps* shaderCaps) {
  // we can draw circles
  SkScalar width = oval.width();
...
std::unique_ptr<GrDrawOp> GrOvalOpFactory::MakeArcOp(
    GrRecordingContext* context, GrPaint&& paint, const SkMatrix& viewMatrix, const SkRect& oval,
...
GR_DRAW_OP_TEST_DEFINE(CircleOp) {
  do {
    ...
    std::unique_ptr<GrDrawOp> op = CircleOp::Make(
        context, std::move(paint), viewMatrix, center, radius, GrStyle(stroke, nullptr), arcParams);
    if (op) {
      return op;
    }
    assert_alive(paint);
...
GR_DRAW_OP_TEST_DEFINE(EllipseOp) {
  ...
  return EllipseOp::Make(
      context, std::move(paint), viewMatrix, ellipse, GrTest::TestStrokeRec(random));
}

GR_DRAW_OP_TEST_DEFINE(DIEllipseOp) {
  SkMatrix viewMatrix = GrTest::TestMatrix(random);
  SkRect ellipse = GrTest::TestSquare(random);
  return DIEllipseOp::Make(
      context, std::move(paint), viewMatrix, ellipse, GrTest::TestStrokeRec(random));
}
...
GR_DRAW_OP_TEST_DEFINE(RRectOp) {
  ...
  return make_rrect_op(context, std::move(paint), viewMatrix, rrect, GrTest::TestStrokeRec(random));
}
