   ...
   private:
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
      ...
      fragBuilder->emitFunction(
          kFloat_GrSLType, fnName.c_str(), {fnArgs, SK_ARRAY_COUNT(fnArgs)}, R"(
                    float linearDist;
 ...
 private:
  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    for (const auto& ellipse : fEllipses) {
      ...
      // The inner radius in the vertex data must be specified in normalized space.
      verts.writeQuad(
          GrVertexWriter::TriStripFromRect(ellipse.fDevBounds.makeOutset(aaBloat, aaBloat)), color,
 ...
 private:
  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    for (const auto& ellipse : fEllipses) {
      ...
      verts.writeQuad(
          GrVertexWriter::TriStripFromRect(drawBounds), color,
...
class CircularRRectOp : public GrMeshDrawOp {
 ...
 private:
  static void FillInOverstrokeVerts(
      ...
      SkScalar xOffset, SkScalar outerRadius, SkScalar innerRadius, const GrVertexColor& color) {
