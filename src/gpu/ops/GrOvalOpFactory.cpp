 ...
 public:
  ...
  GrProcessorSet::Analysis finalize(
    ...
    return fHelper.finalizeProcessors(
        caps, clip, fsaaType, clampType, GrProcessorAnalysisCoverage::kSingleChannel, color,
        &fWideColor);
  }
...
static std::unique_ptr<GrDrawOp> make_rrect_op(
  ...
  if (isCircular) {
    return CircularRRectOp::Make(
        context, std::move(paint), viewMatrix, bounds, xRadius, scaledStroke.fX, isStrokeOnly);
    // otherwise we use the ellipse renderer
  } else {
    return EllipticalRRectOp::Make(
        context, std::move(paint), viewMatrix, bounds, xRadius, yRadius, scaledStroke,
        isStrokeOnly);
  }
}
  ...
  if (width > SK_ScalarNearlyZero && SkScalarNearlyEqual(width, oval.height()) &&
    ...
    if (style.hasNonDashPathEffect()) {
      ...
      return ButtCapDashedCircleOp::Make(
          context, std::move(paint), viewMatrix, center, r, style.strokeRec().getWidth(),
          kStartAngle, angularOnInterval, angularOffInterval, phaseAngle);
    }
