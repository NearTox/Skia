...
void AAStrokeRectOp::generateAAStrokeRectGeometry(
  ...
  if (!degenerate) {
    ...
    GrVertexColor interiorColor(
        tweakAlphaForCoverage ? color * interiorCoverage : color, wideColor);
    vertices.writeQuad(
        GrVertexWriter::TriFanFromRect(interiorAABoundary), interiorColor,
    ...
    vertices.writeQuad(
        GrVertexWriter::TriFanFromRect(devInside), innerColor, maybe_coverage(innerCoverage));
