 ...
 public:
  ...
  GrProcessorSet::Analysis finalize(
    ...
    return fHelper.finalizeProcessors(
        caps, clip, hasMixedSampledCoverage, clampType, GrProcessorAnalysisCoverage::kNone, &fColor,
        nullptr);
  }
  ...
  GrProgramInfo* createProgramInfo(
    ...
    GrGeometryProcessor* gp;
    {
      ...
      gp = GrDefaultGeoProcFactory::Make(
          arena, caps->shaderCaps(), color, Coverage::kSolid_Type, localCoordsType, fViewMatrix);
    }

    return fHelper.createProgramInfo(
        caps, arena, outputView, std::move(clip), dstProxyView, gp,
...
static void compute_aa_rects(
    SkRect* devOutside, SkRect* devOutsideAssist, SkRect* devInside, bool* isDegenerate,
    const SkMatrix& viewMatrix, const SkRect& rect, SkScalar strokeWidth, bool miterStroke,
...
static GrGeometryProcessor* create_aa_stroke_rect_gp(
    SkArenaAlloc* arena, const GrShaderCaps* shaderCaps, bool tweakAlphaForCoverage,
  ...
  return MakeForDeviceSpace(
      arena, shaderCaps, colorType, coverageType, localCoordsType, viewMatrix);
}
...
class AAStrokeRectOp final : public GrMeshDrawOp {
 ...
 public:
  ...
  static std::unique_ptr<GrDrawOp> Make(
    ...
    return Helper::FactoryHelper<AAStrokeRectOp>(
        context, std::move(paint), viewMatrix, rect, stroke, isMiter);
  }
  ...
  AAStrokeRectOp(
    ...
    compute_aa_rects(
        &info.fDevOutside, &info.fDevOutsideAssist, &info.fDevInside, &info.fDegenerate, viewMatrix,
        rect, stroke.getWidth(), isMiter, &info.fDevHalfStrokeSize);
    info.fColor = color;
    if (isMiter) {
 ...
 private:
  ...
  void generateAAStrokeRectGeometry(
      ...
      bool tweakAlphaForCoverage, const SkVector& devHalfStrokeSize) const;
...
GrProgramInfo* AAStrokeRectOp::createProgramInfo(
  ...
  GrGeometryProcessor* gp = create_aa_stroke_rect_gp(
      arena, caps->shaderCaps(), fHelper.compatibleWithCoverageAsAlpha(), this->viewMatrix(),
      fHelper.usesLocalCoords(), fWideColor);
  if (!gp) {
...
void AAStrokeRectOp::onPrepareDraws(Target* target) {
  ...
  for (int i = 0; i < instanceCount; i++) {
    ...
    this->generateAAStrokeRectGeometry(
        ...
        info.fDevInside, fMiterStroke, info.fDegenerate, fHelper.compatibleWithCoverageAsAlpha(),
...
void AAStrokeRectOp::generateAAStrokeRectGeometry(
  ...
  // Outermost rect
  vertices.writeQuad(inset_fan(devOutside, -outsetX, -outsetY), outerColor, maybe_coverage(0.0f));

  if (!miterStroke) {
    // Second outermost
    vertices.writeQuad(
        inset_fan(devOutsideAssist, -outsetX, -outsetY), outerColor, maybe_coverage(0.0f));
  }

  float innerCoverage =
      compute_inner_coverage(std::max(devHalfStrokeSize.fX, devHalfStrokeSize.fY));
  ...
  // Inner rect
  vertices.writeQuad(
      inset_fan(devOutside, insetX, insetY), innerColor, maybe_coverage(innerCoverage));

  if (!miterStroke) {
    // Second inner
    vertices.writeQuad(
        inset_fan(devOutsideAssist, insetX, insetY), innerColor, maybe_coverage(innerCoverage));
  }

  if (!degenerate) {
    vertices.writeQuad(
        inset_fan(devInside, -insetX, -insetY), innerColor, maybe_coverage(innerCoverage));

    // The innermost rect has 0 coverage...
    vertices.writeQuad(inset_fan(devInside, outsetX, outsetY), outerColor, maybe_coverage(0.0f));
  } else {
...
std::unique_ptr<GrDrawOp> MakeNested(
  ...
  SkVector devHalfStrokeSize{
      SkScalarHalf(devOutside.fRight - devInside.fRight),
