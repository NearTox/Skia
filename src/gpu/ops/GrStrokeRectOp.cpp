 ...
 public:
  ...
  GrProcessorSet::Analysis finalize(
    ...
    return fHelper.finalizeProcessors(
        caps, clip, fsaaType, clampType, GrProcessorAnalysisCoverage::kNone, &fColor, nullptr);
  }
  ...
  void onPrepareDraws(Target* target) override {
    sk_sp<GrGeometryProcessor> gp;
    {
      ...
      gp = GrDefaultGeoProcFactory::Make(
          target->caps().shaderCaps(), color, Coverage::kSolid_Type, localCoordsType, fViewMatrix);
    }
...
static sk_sp<GrGeometryProcessor> create_aa_stroke_rect_gp(
    const GrShaderCaps* shaderCaps, bool tweakAlphaForCoverage, const SkMatrix& viewMatrix,
 ...
 public:
  ...
  static std::unique_ptr<GrDrawOp> Make(
    ...
    return Helper::FactoryHelper<AAStrokeRectOp>(
        context, std::move(paint), viewMatrix, devOutside, devInside);
  }
...
void AAStrokeRectOp::onPrepareDraws(Target* target) {
  sk_sp<GrGeometryProcessor> gp(create_aa_stroke_rect_gp(
      target->caps().shaderCaps(), fHelper.compatibleWithCoverageAsAlpha(), this->viewMatrix(),
...
void AAStrokeRectOp::generateAAStrokeRectGeometry(
  ...
  if (!degenerate) {
    ...
    inset =
        SK_ScalarHalf * SkMinScalar(inset, SkTMax(devOutside.height(), devOutsideAssist.height()));
  }
  ...
  // Outermost rect
  vertices.writeQuad(
      inset_fan(devOutside, -SK_ScalarHalf, -SK_ScalarHalf), outerColor, maybe_coverage(0.0f));

  if (!miterStroke) {
    // Second outermost
    vertices.writeQuad(
        inset_fan(devOutsideAssist, -SK_ScalarHalf, -SK_ScalarHalf), outerColor,
        maybe_coverage(0.0f));
  }
...
std::unique_ptr<GrDrawOp> MakeNested(
  ...
  if (devInside.isEmpty()) {
    ...
    return GrFillRectOp::MakeGeneric(
        context, std::move(paint), GrAAType::kCoverage, GrQuadAAFlags::kAll,
        GrPerspQuad::MakeFromRect(rects[0], viewMatrix), GrPerspQuad(rects[0]));
  }
