...
class StaticVertexAllocator : public GrEagerVertexAllocator {
 public:
  StaticVertexAllocator(GrResourceProvider* resourceProvider, bool canMapVB)
      : fResourceProvider(resourceProvider), fCanMapVB(canMapVB), fVertices(nullptr) {}
#ifdef SK_DEBUG
  ~StaticVertexAllocator() override { SkASSERT(!fLockStride); }
#endif
...
class TessellatingPathOp final : public GrMeshDrawOp {
 ...
 private:
  ...
  void draw(Target* target, const GrGeometryProcessor* gp) {
    ...
    int count = GrTessellator::PathToTriangles(
        getPath(), tol, clipBounds, &allocator, GrTessellator::Mode::kNormal, &isLinear);
    if (count == 0) {
      return;
    }
  ...
  void drawAA(Target* target, const GrGeometryProcessor* gp) {
    ...
    int count = GrTessellator::PathToTriangles(
        path, tol, clipBounds, &allocator, GrTessellator::Mode::kEdgeAntialias, &isLinear);
    if (count == 0) {
      return;
    }
