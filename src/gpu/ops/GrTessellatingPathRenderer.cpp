...
GrPathRenderer::CanDrawPath GrTessellatingPathRenderer::onCanDrawPath(
...
class TessellatingPathOp final : public GrMeshDrawOp {
 ...
 public:
  ...
  static std::unique_ptr<GrDrawOp> Make(
    ...
    return Helper::FactoryHelper<TessellatingPathOp>(
  ...
  GrProcessorSet::Analysis finalize(
    ...
    return fHelper.finalizeProcessors(caps, clip, fsaaType, clampType, coverage, &fColor, nullptr);
  }
 ...
 private:
  ...
  void draw(Target* target, sk_sp<const GrGeometryProcessor> gp, size_t vertexStride) {
    ...
    if (cache_match(cachedVertexBuffer.get(), tol, &actualCount)) {
      this->drawVertices(target, std::move(gp), std::move(cachedVertexBuffer), 0, actualCount);
      return;
    }
    ...
    int count =
        GrTessellator::PathToTriangles(getPath(), tol, clipBounds, &allocator, false, &isLinear);
    if (count == 0) {
      return;
    }
  ...
  void drawAA(Target* target, sk_sp<const GrGeometryProcessor> gp, size_t vertexStride) {
    ...
    int count = GrTessellator::PathToTriangles(path, tol, clipBounds, &allocator, true, &isLinear);
    if (count == 0) {
      return;
    }
    this->drawVertices(
        target, std::move(gp), allocator.detachVertexBuffer(), allocator.firstVertex(), count);
  }
    ...
    sk_sp<GrGeometryProcessor> gp;
    {
      ...
      if (fAntiAlias) {
        gp = GrDefaultGeoProcFactory::MakeForDeviceSpace(
            target->caps().shaderCaps(), color, coverageType, localCoordsType, fViewMatrix);
      } else {
  ...
  void drawVertices(
      Target* target, sk_sp<const GrGeometryProcessor> gp, sk_sp<const GrBuffer> vb,
...
bool GrTessellatingPathRenderer::onDrawPath(const DrawPathArgs& args) {
  GR_AUDIT_TRAIL_AUTO_FRAME(
      args.fRenderTargetContext->auditTrail(), "GrTessellatingPathRenderer::onDrawPath");
  SkIRect clipBoundsI;
  ...
  std::unique_ptr<GrDrawOp> op = TessellatingPathOp::Make(
      args.fContext, std::move(args.fPaint), *args.fShape, *args.fViewMatrix, clipBoundsI, aaType,
...
GR_DRAW_OP_TEST_DEFINE(TesselatingPathOp) {
  ...
  return TessellatingPathOp::Make(
