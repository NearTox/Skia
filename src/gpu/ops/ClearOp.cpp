...
GrOp::Owner ClearOp::MakeColor(
    GrRecordingContext* context, const GrScissorState& scissor, std::array<float, 4> color) {
  return GrOp::Make<ClearOp>(context, Buffer::kColor, scissor, color, false);
}

GrOp::Owner ClearOp::MakeStencilClip(
    GrRecordingContext* context, const GrScissorState& scissor, bool insideMask) {
  return GrOp::Make<ClearOp>(
      context, Buffer::kStencilClip, scissor, std::array<float, 4>(), insideMask);
}

ClearOp::ClearOp(
    Buffer buffer, const GrScissorState& scissor, std::array<float, 4> color, bool insideMask)
    : GrOp(ClassID()),
      fScissor(scissor),
      fColor(color),
  ...
  if (other->fBuffer == fBuffer) {
    ...
    if (contains_scissor(other->fScissor, fScissor)) {
      ...
      return CombineResult::kMerged;
        ...
        other->fColor == fColor && other->fStencilInsideMask == fStencilInsideMask &&
