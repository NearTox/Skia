 ...
 public:
  ...
  GrProcessorSet::Analysis finalizeProcessors(
    ...
    return this->finalizeProcessors(
        caps, clip, &GrUserStencilSettings::kUnused, fsaaType, clampType, geometryCoverage,
        geometryColor);
  }
...
std::unique_ptr<GrDrawOp> GrSimpleMeshDrawOpHelper::FactoryHelper(
  ...
  if (paint.isTrivial()) {
    ...
    return std::unique_ptr<GrDrawOp>(new (mem)
                                         Op(makeArgs, color, std::forward<OpArgs>(opArgs)...));
  }
}
