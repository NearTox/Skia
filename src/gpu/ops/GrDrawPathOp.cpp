...
GrDrawPathOpBase::GrDrawPathOpBase(
    uint32_t classID, const SkMatrix& viewMatrix, GrPaint&& paint, GrPathRendering::FillType fill,
    GrAA aa)
    : INHERITED(classID),
...
const GrProcessorSet::Analysis& GrDrawPathOpBase::doProcessorAnalysis(
  ...
  fAnalysis = fProcessorSet.finalize(
      fInputColor, GrProcessorAnalysisCoverage::kNone, clip, &kCoverPass, hasMixedSampledCoverage,
      caps, clampType, &fInputColor);
  return fAnalysis;
}
...
std::unique_ptr<GrDrawOp> GrDrawPathOp::Make(
    GrRecordingContext* context, const SkMatrix& viewMatrix, GrPaint&& paint, GrAA aa,
...
void GrDrawPathOp::onExecute(GrOpFlushState* flushState, const SkRect& chainBounds) {
  ...
  auto pipeline = GrSimpleMeshDrawOpHelper::CreatePipeline(
      flushState, this->detachProcessorSet(), pipelineFlags, &kCoverPass);
...
inline void pre_translate_transform_values(
    const float* xforms, GrPathRendering::PathTransformType type, int count, SkScalar x, SkScalar y,
    float* dst) {
  if (0 == x && 0 == y) {
  ...
  switch (type) {
    ...
    case GrPathRendering::kAffine_PathTransformType:
      ...
      break;
    default: SK_ABORT("Unknown transform type."); break;
  }
}
