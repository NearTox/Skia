...
static inline bool get_direction(const SkPath& path, const SkMatrix& m, SkPathFirstDirection* dir) {
...
static inline void add_cubic_segments(
    ...
    *i += 6;
    ...
    if (Segment::kLine == segb.fType) {
      ...
      *v += 5;
      ...
      verts.write(
          fanPt, color, skipUVs, -segb.fNorms[0].dot(fanPt) + c0, -segb.fNorms[1].dot(fanPt) + c1);
...
bool GrAAConvexPathRenderer::onDrawPath(const DrawPathArgs& args) {
  GR_AUDIT_TRAIL_AUTO_FRAME(
      args.fRenderTargetContext->auditTrail(), "GrAAConvexPathRenderer::onDrawPath");
