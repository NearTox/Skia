...
static void write_quad(
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    vb->write(
        quad.fX[i], quad.fY[i], If(spec.deviceQuadType() == GrQuadType::kPerspective, quad.fW[i]),
    ...
    if (spec.hasLocalCoords()) {
      vb->write(
          quad.fU[i], quad.fV[i], If(spec.localQuadType() == GrQuadType::kPerspective, quad.fR[i]));
    }
...
void* Tessellate(
    void* vertices, const VertexSpec& spec, const GrPerspQuad& deviceQuad,
  ...
  if (spec.usesCoverageAA()) {
    ...
    if (spec.deviceQuadType() == GrQuadType::kPerspective) {
      ...
      maxCoverage = compute_nested_quad_vertices(
          aaFlags, spec.deviceQuadType() <= GrQuadType::kRectilinear, &inner, &outer, &geomDomain);
...
class QuadPerEdgeAAGeometryProcessor : public GrGeometryProcessor {
 public:
  ...
  static sk_sp<GrGeometryProcessor> Make(
      ...
      GrPixelConfig textureConfig, const GrSamplerState& samplerState, uint32_t extraSamplerKey,
...
sk_sp<GrGeometryProcessor> MakeTexturedProcessor(
  ...
  return QuadPerEdgeAAGeometryProcessor::Make(
      spec, caps, textureType, textureConfig, samplerState, extraSamplerKey,
      std::move(textureColorSpaceXform));
}
