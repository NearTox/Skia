...
static AI V4f fma(const V4f& f, const V4f& m, const V4f& a) { return mad(f, m, a); }

// These rotate the points/edge values either clockwise or counterclockwise assuming tri strip
// order.
static AI V4f nextCW(const V4f& v) { return skvx::shuffle<2, 0, 3, 1>(v); }

static AI V4f nextCCW(const V4f& v) { return skvx::shuffle<1, 3, 0, 2>(v); }
...
static bool get_optimized_outset(const QuadMetadata& metadata, bool rectilinear, V4f* outset) {
  ...
  *outset = 0.5f * rsqrt(1.f - cosTheta * cosTheta);  // 1/2sin(theta)
  ...
  V4f edgeAdjust = metadata.fMask * (halfTanTheta + nextCCW(halfTanTheta)) +
                   nextCCW(metadata.fMask) * nextCCW(*outset) + nextCW(metadata.fMask) * (*outset);
...
static void outset_projected_vertices(
    const V4f& x2d, const V4f& y2d, GrQuadAAFlags aaFlags, Vertices* quad) {
...
static float get_exact_coverage(
    const SkPoint& pixelCenter, const Vertices& quad, const Edges& edges) {
...
static V4f compute_degenerate_quad(
    GrQuadAAFlags aaFlags, const V4f& mask, const Edges& edges, bool outset, Vertices* quad) {
  ...
  // p(3) to e0,e2
  V4f dists1 = px * skvx::shuffle<3, 3, 0, 0>(edges.fA) + py * skvx::shuffle<3, 3, 0, 0>(edges.fB) +
               skvx::shuffle<3, 3, 0, 0>(oc);
  V4f dists2 = px * skvx::shuffle<1, 2, 1, 2>(edges.fA) + py * skvx::shuffle<1, 2, 1, 2>(edges.fB) +
  ...
  if (!any(d1Or2)) {
    ...
    V2f ex = (skvx::shuffle<0, 1>(edges.fB) * skvx::shuffle<3, 2>(oc) -
              skvx::shuffle<0, 1>(oc) * skvx::shuffle<3, 2>(edges.fB)) /
             eDenom;
    V2f ey = (skvx::shuffle<0, 1>(oc) * skvx::shuffle<3, 2>(edges.fA) -
              skvx::shuffle<0, 1>(edges.fA) * skvx::shuffle<3, 2>(oc)) /
             eDenom;
...
static V4f compute_nested_quad_vertices(
    GrQuadAAFlags aaFlags, bool rectilinear, Vertices* inner, Vertices* outer, SkRect* domain) {
...
static V4f compute_nested_persp_quad_vertices(
    const GrQuadAAFlags aaFlags, Vertices* inner, Vertices* outer, SkRect* domain) {
  SkASSERT(inner->fUVRCount == 0 || inner->fUVRCount == 2 || inner->fUVRCount == 3);
  SkASSERT(outer->fUVRCount == inner->fUVRCount);
  ...
  V4f coverage =
      compute_nested_quad_vertices(aaFlags, /* rect */ false, &inner2D, &outer2D, domain);
...
static void write_quad(
    GrVertexWriter* vb, const GrQuadPerEdgeAA::VertexSpec& spec, CoverageMode mode,
    const V4f& coverage, SkPMColor4f color4f, const SkRect& geomDomain, const SkRect& texDomain,
    const Vertices& quad) {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    vb->write(
        quad.fX[i], quad.fY[i], If(spec.deviceQuadType() == GrQuad::Type::kPerspective, quad.fW[i]),
    ...
    if (spec.hasVertexColors()) {
      ...
      vb->write(
          GrVertexColor(color4f * (mode == CoverageMode::kWithColor ? coverage[i] : 1.f), wide));
    }
...
ColorType MinColorType(SkPMColor4f color, GrClampType clampType, const GrCaps& caps) {
  if (color == SK_PMColor4fWHITE) {
    ...
    return SkPMColor4fNeedsWideColor(color, clampType, caps) ? ColorType::kHalf : ColorType::kByte;
  }
}
...
void* Tessellate(
    void* vertices, const VertexSpec& spec, const GrQuad& deviceQuad, const SkPMColor4f& color4f,
    const GrQuad& localQuad, const SkRect& domain, GrQuadAAFlags aaFlags) {
...
bool ConfigureMeshIndices(
    GrMeshDrawOp::Target* target, GrMesh* mesh, const VertexSpec& spec, int quadCount) {
  if (spec.usesCoverageAA()) {
    ...
    mesh->setIndexedPatterned(
        std::move(ibuffer), kIndicesPerAAFillRect, kVertsPerAAFillRect, quadCount,
...
class QuadPerEdgeAAGeometryProcessor : public GrGeometryProcessor {
 public:
  ...
  static sk_sp<GrGeometryProcessor> Make(
      const VertexSpec& vertexSpec, const GrShaderCaps& caps, GrTextureType textureType,
      GrPixelConfig textureConfig, const GrSamplerState& samplerState, const GrSwizzle& swizzle,
      uint32_t extraSamplerKey, sk_sp<GrColorSpaceXform> textureColorSpaceXform) {
    return sk_sp<QuadPerEdgeAAGeometryProcessor>(new QuadPerEdgeAAGeometryProcessor(
        vertexSpec, caps, textureType, textureConfig, samplerState, swizzle, extraSamplerKey,
  ...
  void getGLSLProcessorKey(const GrShaderCaps&, GrProcessorKeyBuilder* b) const override {
    ...
    if (fCoverageMode != CoverageMode::kNone) {
      x |= fGeomDomain.isInitialized() ? 384
                                       : (CoverageMode::kWithPosition == fCoverageMode ? 128 : 256);
    }
      ...
      void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
        ...
        if (gp.fSampler.isInitialized()) {
          ...
          if (gp.fTexDomain.isInitialized()) {
            ...
            args.fVaryingHandler->addPassThroughAttribute(
                gp.fTexDomain, "domain", Interpolation::kCanBeFlat);
            args.fFragBuilder->codeAppend("texCoord = clamp(texCoord, domain.xy, domain.zw);");
          }
        ...
        if (gp.fCoverageMode == CoverageMode::kWithPosition) {
          ...
          if (gp.fGeomDomain.isInitialized()) {
            ...
            args.fVaryingHandler->addPassThroughAttribute(
                gp.fGeomDomain, "geoDomain", Interpolation::kCanBeFlat);
            args.fFragBuilder->codeAppend(
                "if (coverage < 0.5) {"
          ...
          args.fFragBuilder->codeAppendf("%s = half4(half(coverage));", args.fOutputCoverage);
        } else {
...
sk_sp<GrGeometryProcessor> MakeTexturedProcessor(
  ...
  return QuadPerEdgeAAGeometryProcessor::Make(
      spec, caps, textureType, textureConfig, samplerState, swizzle, extraSamplerKey,
      std::move(textureColorSpaceXform));
}
