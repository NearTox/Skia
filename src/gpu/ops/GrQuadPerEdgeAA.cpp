...
static void outset_projected_vertices(const V4f& x2d, const V4f& y2d, GrQuadAAFlags aaFlags,
                                      Vertices* quad) {
...
static V4f compute_degenerate_quad(GrQuadAAFlags aaFlags, const V4f& mask, const Edges& edges,
    ...
    V4f dists1 = px * skvx::shuffle<3, 3, 0, 0>(edges.fA) +
                 py * skvx::shuffle<3, 3, 0, 0>(edges.fB) + skvx::shuffle<3, 3, 0, 0>(oc);
    V4f dists2 = px * skvx::shuffle<1, 2, 1, 2>(edges.fA) +
                 py * skvx::shuffle<1, 2, 1, 2>(edges.fB) + skvx::shuffle<1, 2, 1, 2>(oc);
...
static V4f compute_nested_quad_vertices(GrQuadAAFlags aaFlags, bool rectilinear, Vertices* inner,
            ...
            void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
                ...
                if (gp.fSampler.isInitialized()) {
                    ...
                    if (gp.fLocalCoord.cpuType() == kFloat3_GrVertexAttribType) {
                        ...
                        args.fVertBuilder->codeAppendf("%s = %s;", v.vsOut(),
                                                       gp.fLocalCoord.name());
                        args.fFragBuilder->codeAppendf("texCoord = %s.xy / %s.z;", v.fsIn(),
                                                       v.fsIn());
                    } else {
                ...
                if (gp.fCoverageMode == CoverageMode::kWithPosition) {
                    ...
                    if (gp.fNeedsPerspective) {
                        ...
                        args.fVertBuilder->codeAppendf("%s = %s.w * %s.z;", coverage.vsOut(),
                                                       gp.fPosition.name(), gp.fPosition.name());
                        args.fFragBuilder->codeAppendf("float coverage = %s * sk_FragCoord.w;",
                                                       coverage.fsIn());
                    } else {
                        args.fVertBuilder->codeAppendf("%s = %s;", coverage.vsOut(),
                                                       gp.fCoverage.name());
