...
static void write_quad(
    GrVertexWriter* vb, const GrQuadPerEdgeAA::VertexSpec& spec, GrQuadPerEdgeAA::CoverageMode mode,
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    if (spec.hasVertexColors()) {
      ...
      vb->write(GrVertexColor(
          color4f * (mode == GrQuadPerEdgeAA::CoverageMode::kWithColor ? coverage[i] : 1.f), wide));
    }
...
class QuadPerEdgeAAGeometryProcessor : public GrGeometryProcessor {
 public:
  ...
  void getGLSLProcessorKey(const GrShaderCaps&, GrProcessorKeyBuilder* b) const override {
    ...
    uint32_t x = (fTexDomain.isInitialized() ? 0 : 0x1) | (fSampler.isInitialized() ? 0 : 0x2) |
                 (fNeedsPerspective ? 0 : 0x4) | (fSaturate == Saturate::kNo ? 0 : 0x8);
