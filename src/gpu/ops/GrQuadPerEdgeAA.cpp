...
static void write_quad_generic(
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    if (spec.hasVertexColors()) {
      ...
      vb->write(GrVertexColor(
          color * (mode == GrQuadPerEdgeAA::CoverageMode::kWithColor ? coverage[i] : 1.f), wide));
    }
...
static void write_2d_color(
  ...
  SkASSERT(
      spec.coverageMode() == GrQuadPerEdgeAA::CoverageMode::kNone ||
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    SkASSERT(
        spec.coverageMode() == GrQuadPerEdgeAA::CoverageMode::kWithColor || coverage[i] == 1.f);
...
static void write_2d_color_uv(
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    SkASSERT(
        spec.coverageMode() == GrQuadPerEdgeAA::CoverageMode::kWithColor || coverage[i] == 1.f);
    vb->write(
        deviceQuad->x(i), deviceQuad->y(i), GrVertexColor(color * coverage[i], wide),
        localQuad->x(i), localQuad->y(i));
  }
}
...
static void write_2d_uv_strict(
    GrVertexWriter* vb, const GrQuadPerEdgeAA::VertexSpec& spec, const GrQuad* deviceQuad,
    const GrQuad* localQuad, const float coverage[4], const SkPMColor4f& color,
    const SkRect& geomSubset, const SkRect& texSubset) {
  // Assert assumptions about VertexSpec
...
static void write_2d_color_uv_strict(
    GrVertexWriter* vb, const GrQuadPerEdgeAA::VertexSpec& spec, const GrQuad* deviceQuad,
    const GrQuad* localQuad, const float coverage[4], const SkPMColor4f& color,
    const SkRect& geomSubset, const SkRect& texSubset) {
  // Assert assumptions about VertexSpec
...
static void write_2d_cov_uv_strict(
    GrVertexWriter* vb, const GrQuadPerEdgeAA::VertexSpec& spec, const GrQuad* deviceQuad,
    const GrQuad* localQuad, const float coverage[4], const SkPMColor4f& color,
    const SkRect& geomSubset, const SkRect& texSubset) {
  // Assert assumptions about VertexSpec
  ...
  for (int i = 0; i < 4; ++i) {
    vb->write(
        deviceQuad->x(i), deviceQuad->y(i), coverage[i], localQuad->x(i), localQuad->y(i),
        texSubset);
  }
}
     ...
     private:
      void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
        ...
        if (gp.fCoverageMode == CoverageMode::kWithPosition) {
          ...
          gpArgs->fPositionVar = {
              "position", gp.fNeedsPerspective ? kFloat3_GrSLType : kFloat2_GrSLType,
