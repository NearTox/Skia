...
void ConfigureMesh(
    const GrCaps& caps, GrMesh* mesh, const VertexSpec& spec, int runningQuadCount, int quadsInDraw,
    int maxVerts, sk_sp<const GrBuffer> vertexBuffer, sk_sp<const GrBuffer> indexBuffer,
  ...
  if (caps.avoidLargeIndexBufferDraws()) {
    ...
    mesh->setIndexedPatterned(
        std::move(indexBuffer), numIndicesPerQuad, numVertsPerQuad, quadsInDraw, maxNumQuads);
    mesh->setVertexData(std::move(vertexBuffer), offset);
  } else {
    ...
    mesh->setIndexed(
        std::move(indexBuffer), numIndicesToDraw, baseIndex, minVertex, maxVertex,
