...
sk_sp<const GrBuffer> GetIndexBuffer(
    GrMeshDrawOp::Target* target, IndexBufferOption indexBufferOption) {
...
class QuadPerEdgeAAGeometryProcessor : public GrGeometryProcessor {
 public:
  ...
  void getGLSLProcessorKey(const GrShaderCaps&, GrProcessorKeyBuilder* b) const override {
    ...
    uint32_t x = (fTexSubset.isInitialized() ? 0 : 0x1) | (fSampler.isInitialized() ? 0 : 0x2) |
      ...
      void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
        ...
        if (gp.fSampler.isInitialized()) {
          ...
          if (gp.fTexSubset.isInitialized()) {
            ...
            args.fVaryingHandler->addPassThroughAttribute(
                gp.fTexSubset, "subset", Interpolation::kCanBeFlat);
            args.fFragBuilder->codeAppend("texCoord = clamp(texCoord, subset.xy, subset.zw);");
          }
        ...
        if (gp.fCoverageMode == CoverageMode::kWithPosition) {
          ...
          if (gp.fGeomSubset.isInitialized()) {
            ...
            args.fVaryingHandler->addPassThroughAttribute(
                gp.fGeomSubset, "geoSubset", Interpolation::kCanBeFlat);
            args.fFragBuilder->codeAppend(
                "if (coverage < 0.5) {"
