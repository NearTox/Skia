...
static GrVertexAttribType SkVerticesAttributeToGrVertexAttribType(
    const SkVertices::Attribute& a) noexcept {
  switch (a.fType) {
...
static GrSLType SkVerticesAttributeToGrSLType(const SkVertices::Attribute& a) noexcept {
  switch (a.fType) {
...
class MarkedMatrices {
 public:
  ...
  void gather(const SkVerticesPriv& info, const SkMatrixProvider& matrixProvider) {
    for (int i = 0; i < info.attributeCount(); ++i) {
      if (uint32_t id = info.attributes()[i].fMarkerID) {
        if (std::none_of(fMatrices.begin(), fMatrices.end(), [id](const auto& m) {
              return m.first == id;
            })) {
          SkM44 matrix;
  ...
  SkM44 get(uint32_t id) const noexcept {
...
class VerticesGP : public GrGeometryProcessor {
 public:
  static GrGeometryProcessor* Make(
      SkArenaAlloc* arena, LocalCoordsType localCoordsType, ColorArrayType colorArrayType,
      const SkPMColor4f& color, sk_sp<GrColorSpaceXform> colorSpaceXform,
      const SkMatrix& viewMatrix, const SkVertices::Attribute* attrs, int attrCount,
      const MarkedMatrices* customMatrices) {
    return arena->make<VerticesGP>(
        localCoordsType, colorArrayType, color, std::move(colorSpaceXform), viewMatrix, attrs,
  ...
  const Attribute& positionAttr() const noexcept { return fAttributes[kPositionIndex]; }
  const Attribute& colorAttr() const noexcept { return fAttributes[kColorIndex]; }
  const Attribute& localCoordsAttr() const noexcept { return fAttributes[kLocalCoordsIndex]; }

  class GLSLProcessor : public GrGLSLGeometryProcessor {
   public:
    GLSLProcessor() : fViewMatrix(SkMatrix::InvalidMatrix()), fColor(SK_PMColor4fILLEGAL) {}
    ...
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
      ...
      fColorSpaceHelper.emitCode(uniformHandler, gp.fColorSpaceXform.get(), kVertex_GrShaderFlag);
      ...
      this->writeOutputPosition(
          vertBuilder, uniformHandler, gpArgs, gp.positionAttr().name(), gp.viewMatrix(),
          &fViewMatrixUniform);

      // emit transforms using either explicit local coords or positions
      const auto& coordsAttr =
          gp.localCoordsAttr().isInitialized() ? gp.localCoordsAttr() : gp.positionAttr();
      this->emitTransforms(
          vertBuilder, varyingHandler, uniformHandler, coordsAttr.asShaderVar(), SkMatrix::I(),
      ...
      for (size_t i = kFirstCustomIndex; i < gp.fAttributes.size(); ++i) {
        ...
        if (customAttr.fMarkerID) {
          ...
          if (!matrixHandle.isValid()) {
            SkString uniName =
                SkStringPrintf("customMatrix_%x%s", customAttr.fMarkerID, normal ? "_IT" : "");
            matrixHandle = uniformHandler->addUniform(
                nullptr, kVertex_GrShaderFlag, normal ? kFloat3x3_GrSLType : kFloat4x4_GrSLType,
                uniName.c_str());
            fCustomMatrixUniforms.push_back({customAttr.fMarkerID, normal, matrixHandle});
          }
        }
        ...
        switch (customAttr.fUsage) {
          ...
          case Usage::kColor: {
            ...
            SkString xformedColor;
            vertBuilder->appendColorGamutXform(
                &xformedColor, varyingIn.c_str(), &fColorSpaceHelper);
            // Store the result of the transform in a temporary
            vertBuilder->codeAppendf("half4 _tmp_clr_%d = %s;", customIdx, xformedColor.c_str());
            // Finally, premultiply
            varyingIn = SkStringPrintf(
                "half4(_tmp_clr_%d.rgb * _tmp_clr_%d.a, _tmp_clr_%d.a)", customIdx, customIdx,
                customIdx);
          ...
          case Usage::kVector: {
            ...
            if (matrixHandle.isValid()) {
              varyingIn = SkStringPrintf(
                  "(%s * %s.xyz0).xyz", uniformHandler->getUniformCStr(matrixHandle),
                  varyingIn.c_str());
            }
          ...
          case Usage::kNormalVector: {
            ...
            if (matrixHandle.isValid()) {
              varyingIn = SkStringPrintf(
                  "(%s * %s)", uniformHandler->getUniformCStr(matrixHandle), varyingIn.c_str());
            }
            varyingIn = SkStringPrintf("normalize(%s)", varyingIn.c_str());
            varyingType = kFloat3_GrSLType;
            break;
          ...
          case Usage::kPosition: {
            ...
            if (matrixHandle.isValid()) {
              vertBuilder->codeAppendf(
                  "float4 _tmp_pos_%d = %s * %s.xyz1;", customIdx,
                  uniformHandler->getUniformCStr(matrixHandle), varyingIn.c_str());
              varyingIn = SkStringPrintf("_tmp_pos_%d.xyz / _tmp_pos_%d.w", customIdx, customIdx);
            }
            varyingType = kFloat3_GrSLType;
          }
        }
    ...
    void setData(
      ...
      for (const auto& matrixUni : fCustomMatrixUniforms) {
        ...
        if (matrixUni.fNormal) {
          ...
          // row order.
          float mtxIT[9] = {mtx.rc(0, 0), mtx.rc(0, 1), mtx.rc(0, 2), mtx.rc(1, 0), mtx.rc(1, 1),
                            mtx.rc(1, 2), mtx.rc(2, 0), mtx.rc(2, 1), mtx.rc(2, 2)};
 ...
 private:
  ...
  VerticesGP(
      LocalCoordsType localCoordsType, ColorArrayType colorArrayType, const SkPMColor4f& color,
      sk_sp<GrColorSpaceXform> colorSpaceXform, const SkMatrix& viewMatrix,
      const SkVertices::Attribute* attrs, int attrCount, const MarkedMatrices* customMatrices)
    ...
    fAttributes.push_back(
        fColorArrayType != ColorArrayType::kUnused ? MakeColorAttribute("inColor", false)
                                                   : missingAttr);
    fAttributes.push_back(
        localCoordsType == LocalCoordsType::kExplicit
    ...
    for (int i = 0; i < attrCount; ++i) {
      ...
      fAttributes.push_back(
          {fAttrNames.back().c_str(), SkVerticesAttributeToGrVertexAttribType(attrs[i]),
...
class DrawVerticesOp final : public GrMeshDrawOp {
 ...
 private:
  ...
  bool combinablePrimitive() const noexcept {
  ...
  struct Mesh {
    ...
    bool hasPerVertexColors() const noexcept {
  ...
  bool requiresPerVertexColors() const noexcept {
    return fColorArrayType != ColorArrayType::kUnused;
  }

  bool requiresPerVertexLocalCoords() const noexcept {
    return fLocalCoordsType == LocalCoordsType::kExplicit;
  }

  size_t vertexStride() const noexcept {
    return sizeof(SkPoint) + (this->requiresPerVertexColors() ? sizeof(uint32_t) : 0) +
...
DrawVerticesOp::DrawVerticesOp(
  ...
  fColorArrayType = info.hasColors() ? ColorArrayType::kSkColor : ColorArrayType::kUnused;
  fLocalCoordsType =
      info.hasTexCoords() ? LocalCoordsType::kExplicit : LocalCoordsType::kUsePosition;
...
GrGeometryProcessor* DrawVerticesOp::makeGP(SkArenaAlloc* arena) {
  ...
  auto gp = VerticesGP::Make(
      arena, fLocalCoordsType, fColorArrayType, fMeshes[0].fColor, std::move(csxform), vm,
      info.attributes(), info.attributeCount(), &fCustomMatrices);
...
GrOp::CombineResult DrawVerticesOp::onCombineIfPossible(
  ...
  if (vThis.attributeCount() != vThat.attributeCount() ||
      !std::equal(
          vThis.attributes(), vThis.attributes() + vThis.attributeCount(), vThat.attributes())) {
    return CombineResult::kCannotCombine;
  }
...
std::unique_ptr<GrDrawOp> GrDrawVerticesOp::Make(
  ...
  GrPrimitiveType primType =
      overridePrimType ? *overridePrimType : SkVertexModeToGrPrimitiveType(vertices->priv().mode());
...
GR_DRAW_OP_TEST_DEFINE(DrawVerticesOp) {
  ...
  return GrDrawVerticesOp::Make(
      context, std::move(paint), std::move(vertices), matrixProvider, aaType,
      std::move(colorSpaceXform), &type, nullptr);
}
