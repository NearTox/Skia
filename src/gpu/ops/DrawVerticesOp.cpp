  ...
  static GrGeometryProcessor* Make(
    ...
    return arena->make([&](void* ptr) {
      return new (ptr) VerticesGP(
          localCoordsType, colorArrayType, color, std::move(colorSpaceXform), viewMatrix);
    });
  }
  ...
  class Impl : public ProgramImpl {
   ...
   private:
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
      ...
      // Setup position
      WriteOutputPosition(
          vertBuilder, uniformHandler, *args.fShaderCaps, gpArgs, gp.positionAttr().name(),
          gp.fViewMatrix, &fViewMatrixUniform);
...
DrawVerticesOpImpl::DrawVerticesOpImpl(
...
GrProcessorSet::Analysis DrawVerticesOpImpl::finalize(
...
void DrawVerticesOpImpl::onCreateProgramInfo(
  ...
  fProgramInfo = fHelper.createProgramInfo(
      ...
      this->primitiveType(), renderPassXferBarriers, colorLoadOp);
}
  ...
  for (const auto& mesh : fMeshes) {
    ...
    if (fMultipleViewMatrices) {
      ...
      SkMatrixPriv::MapPointsWithStride(
          mesh.fViewMatrix, posBase, vertexStride, positions, sizeof(SkPoint), vertexCount);
    }
...
GrOp::CombineResult DrawVerticesOpImpl::onCombineIfPossible(
...
GR_DRAW_OP_TEST_DEFINE(DrawVerticesOp) {
  ...
  return skgpu::v1::DrawVerticesOp::Make(
