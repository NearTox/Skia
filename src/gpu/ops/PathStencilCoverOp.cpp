...
void PathStencilCoverOp::prePreparePrograms(
  ...
  if (fTotalCombinedPathVerbCnt > 50 &&
    ...
    fTessellator = GrPathCurveTessellator::Make(
        args.fArena, shaderMatrix, SK_PMColor4fTRANSPARENT,
        GrPathCurveTessellator::DrawInnerFan::kNo, fTotalCombinedPathVerbCnt, *stencilPipeline,
        *args.fCaps);
  } else {
    fTessellator = GrPathWedgeTessellator::Make(
        args.fArena, shaderMatrix, SK_PMColor4fTRANSPARENT, fTotalCombinedPathVerbCnt,
        *stencilPipeline, *args.fCaps);
  }
  fStencilPathProgram = GrTessellationShader::MakeProgram(
      args, fTessellator->shader(), stencilPipeline, stencilSettings);
...
void PathStencilCoverOp::onPrepare(GrOpFlushState* flushState) {
  ...
  if (fStencilFanProgram) {
    ...
    for (auto [pathMatrix, path] : *fPathDrawList) {
      ...
      triangleVertexWriter = GrMiddleOutPolygonTriangulator::WritePathInnerFan(
          std::move(triangleVertexWriter), 0, 0, pathMatrix, path, &numTrianglesWritten);
  ...
  if (fCoverBBoxProgram) {
    ...
    GrVertexWriter vertexWriter =
        flushState->makeVertexSpace(instanceStride, fPathCount, &fBBoxBuffer, &fBBoxBaseInstance);
    SkDEBUGCODE(int pathCount = 0;)
    for (auto [pathMatrix, path] : *fPathDrawList) {
      SkDEBUGCODE(auto end = vertexWriter.makeOffset(instanceStride));
      vertexWriter.write(
          pathMatrix.getScaleX(), pathMatrix.getSkewY(), pathMatrix.getSkewX(),
          pathMatrix.getScaleY(), pathMatrix.getTranslateX(), pathMatrix.getTranslateY());
