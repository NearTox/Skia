...
// 1, 1, and height.
static void compute_domain(
    Domain domain, GrSamplerState::Filter filter, GrSurfaceOrigin origin, const SkRect& domainRect,
    float iw, float ih, float h, SkRect* out) {
...
static void compute_src_quad(
    GrSurfaceOrigin origin, const GrQuad& srcQuad, float iw, float ih, float h, GrQuad* out) {
...
class TextureOp final : public GrMeshDrawOp {
    ...
    for (unsigned p = 0; p < fProxyCnt; ++p) {
      ...
      while (i < fProxies[p].fQuadCnt && iter.next()) {
        ...
        str.appendf(
            ...
            quad.point(2).fX, quad.point(2).fY, quad.point(3).fX, quad.point(3).fY, uv.point(0).fX,
            uv.point(0).fY, uv.point(1).fX, uv.point(1).fY, uv.point(2).fX, uv.point(2).fY,
            uv.point(3).fX, uv.point(3).fY);
 ...
 private:
  ...
  void tess(
      void* v, const VertexSpec& spec, const GrTextureProxy* proxy,
    ...
    while (i < cnt && iter->next()) {
      ...
      compute_domain(info.domain(), this->filter(), origin, info.fDomainRect, iw, ih, h, &domain);
      v = GrQuadPerEdgeAA::Tessellate(
          v, spec, iter->deviceQuad(), info.fColor, srcQuad, domain, info.aaFlags());
      i++;
    }
  }
  ...
  void onPrepareDraws(Target* target) override {
    ...
    sk_sp<GrGeometryProcessor> gp = GrQuadPerEdgeAA::MakeTexturedProcessor(
        vertexSpec, *target->caps().shaderCaps(), textureType, config, samplerState, swizzle,
