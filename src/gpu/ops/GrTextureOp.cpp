...
class TextureOp final : public GrMeshDrawOp {
 ...
 private:
  ...
  struct Metadata {
    ...
    Metadata(
        const GrSwizzle& swizzle, GrSamplerState::Filter filter, GrQuadPerEdgeAA::Subset subset,
  ...
  TextureOp(
      ...
      GrAAType aaType, DrawQuad* quad, const SkRect* subsetRect)
      : INHERITED(ClassID()),
  ...
  TextureOp(
      ...
      : INHERITED(ClassID()),
        ...
        fDesc(nullptr),
        fMetadata(
            set[0].fProxyView.swizzle(), GrSamplerState::Filter::kNearest, Subset::kNo, saturate) {
    ...
    for (int q = 0; q < cnt; ++q) {
      ...
      fViewCountPairs[p].fQuadCnt += this->appendQuad(&quad, {alpha, alpha, alpha, alpha}, subset);
    }
  ...
  if (blendMode != SkBlendMode::kSrcOver ||
    ...
    for (int i = 0; i < cnt; ++i) {
      ...
      auto op = Make(
          ...
          {alpha, alpha, alpha, alpha}, saturate, blendMode, aaType, &quad, subset);
