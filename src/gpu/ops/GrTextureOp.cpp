...
static bool filter_has_effect(const GrQuad& srcQuad, const GrQuad& dstQuad) {
  ...
  if (srcQuad.asRect(&srcRect) && dstQuad.asRect(&dstRect)) {
    ...
    if (SkScalarIsInt(srcQuad.x(0)) && SkScalarIsInt(srcQuad.y(0)) && SkScalarIsInt(dstQuad.x(0)) &&
...
// domains overall.
static SkRect normalize_domain(
    GrSamplerState::Filter filter, const NormalizationParams& params, const SkRect* domainRect) {
...
class TextureOp final : public GrMeshDrawOp {
 public:
  static std::unique_ptr<GrDrawOp> Make(
      ...
      const SkPMColor4f& color, GrTextureOp::Saturate saturate, GrAAType aaType, DrawQuad* quad,
      const SkRect* domain) {
    GrOpMemoryPool* pool = context->priv().opMemoryPool();
    return pool->allocate<TextureOp>(
        std::move(proxyView), std::move(textureXform), filter, color, saturate, aaType, quad,
        domain);
  }

  static std::unique_ptr<GrDrawOp> Make(
      GrRecordingContext* context, GrRenderTargetContext::TextureSetEntry set[], int cnt,
      int proxyRunCnt, GrSamplerState::Filter filter, GrTextureOp::Saturate saturate,
    ...
    return std::unique_ptr<GrDrawOp>(new (mem) TextureOp(
        set, cnt, proxyRunCnt, filter, saturate, aaType, constraint, viewMatrix,
 ...
 private:
  ...
  struct ColorDomainAndAA {
    ColorDomainAndAA(const SkPMColor4f& color, const SkRect& domainRect, GrQuadAAFlags aaFlags)
        : fColor(color), fDomainRect(domainRect), fAAFlags(static_cast<uint16_t>(aaFlags)) {
  ...
  struct Metadata {
    ...
    Metadata(
        const GrSwizzle& swizzle, GrSamplerState::Filter filter, GrQuadPerEdgeAA::Domain domain,
    ...
    GrTextureOp::Saturate saturate() const { return static_cast<GrTextureOp::Saturate>(fSaturate); }
  ...
  TextureOp(
      ...
      GrAAType aaType, DrawQuad* quad, const SkRect* domainRect)
      : INHERITED(ClassID()),
    ...
    SkASSERT(!domainRect || !domainRect->contains(proxyView.proxy()->backingStoreBoundsRect()));
    ...
    // domain.
    if (domainRect && filter == GrSamplerState::Filter::kNearest && aaType != GrAAType::kCoverage) {
      domainRect = nullptr;
    ...
    this->setBounds(
        quad->fDevice.bounds(), HasAABloat(aaType == GrAAType::kCoverage), IsHairline::kNo);
  ...
  TextureOp(
      ...
      : INHERITED(ClassID()),
        ...
        fPrePreparedDesc(nullptr),
        fMetadata(
            set[0].fProxyView.swizzle(), GrSamplerState::Filter::kNearest, Domain::kNo, saturate) {
    ...
    for (int q = 0; q < cnt; ++q) {
      ...
      if (netFilter != filter && filter_has_effect(quad.fLocal, quad.fDevice)) {
        ...
        SkASSERT(
            netFilter == GrSamplerState::Filter::kNearest &&
      ...
      fViewCountPairs[p].fQuadCnt += this->appendQuad(&quad, {alpha, alpha, alpha, alpha}, domain);
    }
  ...
  int appendQuad(DrawQuad* quad, const SkPMColor4f& color, const SkRect& domain) {
    ...
    int quadCount =
        quad->fEdgeFlags != GrQuadAAFlags::kNone ? GrQuadUtils::ClipToW0(quad, &extra) : 1;
    if (quadCount == 0) {
  ...
  void onPrePrepareDraws(
    ...
    SkDEBUGCODE(this->validate();) SkASSERT(!fPrePreparedDesc);
    ...
    SkAssertResult(FillInData(
        *context->priv().caps(), this, fPrePreparedDesc, fPrePreparedDesc->fVertices, nullptr, 0,
        nullptr, nullptr));
  }

  static bool FillInData(
      const GrCaps& caps, TextureOp* texOp, PrePreparedDesc* desc, char* pVertexData,
      GrMesh* meshes, int absBufferOffset, sk_sp<const GrBuffer> vertexBuffer,
    ...
    for (const auto& op : ChainRange<TextureOp>(texOp)) {
      ...
      for (unsigned p = 0; p < op.fMetadata.fProxyCount; ++p) {
        ...
        if (pVertexData) {
          ...
          for (int i = 0; i < quadCnt && iter.next(); ++i) {
            ...
            tessellator.append(
                ...
                inset ? inset_domain_for_bilerp(params, info.fDomainRect) : info.fDomainRect,
                info.aaFlags());
          }
        ...
        if (meshes) {
          GrQuadPerEdgeAA::ConfigureMesh(
              caps, &(meshes[meshIndex]), desc->fVertexSpec, totQuadsSeen, quadCnt,
              desc->totalNumVertices(), vertexBuffer, indexBuffer, absBufferOffset);
        }
  ...
  void validate() const override {
    ...
    for (const auto& op : ChainRange<TextureOp>(this)) {
      ...
      if (aaType == GrAAType::kCoverage || aaType == GrAAType::kNone) {
        SkASSERT(
            op.fMetadata.aaType() == GrAAType::kCoverage ||
  ...
  void onPrepareDraws(Target* target) override {
    ...
    void* vdata = target->makeVertexSpace(
        vertexSize, desc.totalNumVertices(), &vbuffer, &vertexOffsetInBuffer);
    if (!vdata) {
    ...
    if (desc.fVertexSpec.needsIndexBuffer()) {
      indexBuffer = GrQuadPerEdgeAA::GetIndexBuffer(target, desc.fVertexSpec.indexBufferOption());
      if (!indexBuffer) {
    ...
    if (fPrePreparedDesc) {
      ...
      // mesh data
      result = FillInData(
          target->caps(), this, &desc, nullptr, meshes, vertexOffsetInBuffer, std::move(vbuffer),
          std::move(indexBuffer));
    } else {
      // Fills in both vertex data and mesh data
      result = FillInData(
          target->caps(), this, &desc, (char*)vdata, meshes, vertexOffsetInBuffer,
...

    {
      ...
      gp = GrQuadPerEdgeAA::MakeTexturedProcessor(
          target->allocator(), desc.fVertexSpec, *target->caps().shaderCaps(), backendFormat,
    ...
    target->recordDraw(
        gp, meshes, desc.fNumProxies, desc.fFixedDynamicState, desc.fDynamicStateArrays,
  ...
  void onExecute(GrOpFlushState* flushState, const SkRect& chainBounds) override {
    ...
    auto pipeline = GrSimpleMeshDrawOpHelper::CreatePipeline(
        flushState, GrProcessorSet::MakeEmptySet(), pipelineFlags);
...
#if GR_TEST_UTILS
uint32_t GrTextureOp::ClassID() { return TextureOp::ClassID(); }
#endif

std::unique_ptr<GrDrawOp> GrTextureOp::Make(
    ...
    Saturate saturate, SkBlendMode blendMode, GrAAType aaType, DrawQuad* quad,
    const SkRect* domain) {
  ...
  if (blendMode == SkBlendMode::kSrcOver) {
    return TextureOp::Make(
        context, std::move(proxyView), std::move(textureXform), filter, color, saturate, aaType,
        std::move(quad), domain);
  } else {
    ...
    if (domain) {
      ...
      if (quad->fLocal.asRect(&localRect)) {
        fp = GrTextureEffect::MakeSubset(
            std::move(proxyView), alphaType, SkMatrix::I(), filter, *domain, localRect, caps);
      } else {
        fp = GrTextureEffect::MakeSubset(
            std::move(proxyView), alphaType, SkMatrix::I(), filter, *domain, caps);
      }
    } else {
...
class GrTextureOp::BatchSizeLimiter {
 public:
  ...
  void createOp(GrRenderTargetContext::TextureSetEntry set[], int clumpSize, GrAAType aaType) {
    ...
    std::unique_ptr<GrDrawOp> op = TextureOp::Make(
        fContext, &set[fNumClumped], clumpSize, clumpProxyCount, fFilter, fSaturate, aaType,
        fConstraint, fViewMatrix, fTextureColorSpaceXform);
...
void GrTextureOp::AddTextureSetOps(
  ...
  if (blendMode != SkBlendMode::kSrcOver ||
    ...
    for (int i = 0; i < cnt; ++i) {
      ...
      auto op = Make(
          context, set[i].fProxyView, set[i].fSrcAlphaType, textureColorSpaceXform, filter,
          {alpha, alpha, alpha, alpha}, saturate, blendMode, aaType, &quad, domain);
  ...
  if (cnt <=
    ...
    auto op = TextureOp::Make(
        context, set, cnt, proxyRunCnt, filter, saturate, aaType, constraint, viewMatrix,
        std::move(textureColorSpaceXform));
    rtc->addDrawOp(clip, std::move(op));
    return;
  }
  ...
  BatchSizeLimiter state(
      rtc, clip, context, cnt, filter, saturate, constraint, viewMatrix,
      std::move(textureColorSpaceXform));
...
GR_DRAW_OP_TEST_DEFINE(TextureOp) {
  ...
  return GrTextureOp::Make(
      context, std::move(proxyView), alphaType, std::move(texXform), filter, color, saturate,
      SkBlendMode::kSrcOver, aaType, &quad, useDomain ? &srcRect : nullptr);
}
