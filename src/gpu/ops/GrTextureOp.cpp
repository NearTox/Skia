...
static GrPerspQuad compute_src_quad(GrSurfaceOrigin origin, const GrPerspQuad& srcQuad, float iw,
                                    float ih, float h) {
...
public:
    static std::unique_ptr<GrDrawOp> Make(GrRecordingContext* context,
        ...
        return pool->allocate<TextureOp>(std::move(proxy), filter, color, dstQuad, dstQuadType,
                                         srcRect, constraint, nullptr, GrQuadType::kRect, aaType,
    ...
    static std::unique_ptr<GrDrawOp> Make(GrRecordingContext* context,
        ...
        return pool->allocate<TextureOp>(std::move(proxy), filter, color, grDstQuad, dstQuadType,
                                         srcRect, constraint, &grSrcQuad, srcQuadType, aaType,
                                         aaFlags, std::move(textureColorSpaceXform));
    }
    static std::unique_ptr<GrDrawOp> Make(GrRecordingContext* context,
        ...
        return std::unique_ptr<GrDrawOp>(new (mem) TextureOp(set, cnt, filter, aaType, constraint,
                                                             viewMatrix,
        ...
        for (unsigned p = 0; p < fProxyCnt; ++p) {
            ...
            for (int i = 0; i < fProxies[p].fQuadCnt; ++i, ++q) {
                ...
                str.appendf(
                        ...
                        quad.point(0).fY, quad.point(1).fX, quad.point(1).fY, quad.point(2).fX,
                        quad.point(2).fY, quad.point(3).fX, quad.point(3).fY);
            }
        }
    ...
    GrProcessorSet::Analysis finalize(const GrCaps& caps, const GrAppliedClip*, GrFSAAType,
...
private:
    ...
    TextureOp(sk_sp<GrTextureProxy> proxy, GrSamplerState::Filter filter, const SkPMColor4f& color,
              const GrPerspQuad& dstQuad, GrQuadType dstQuadType, const SkRect& srcRect,
              SkCanvas::SrcRectConstraint constraint, const GrPerspQuad* srcQuad,
              GrQuadType srcQuadType, GrAAType aaType, GrQuadAAFlags aaFlags,
              sk_sp<GrColorSpaceXform> textureColorSpaceXform)
            : INHERITED(ClassID())
        ...
        Domain domain =
                constraint == SkCanvas::kStrict_SrcRectConstraint ? Domain::kYes : Domain::kNo;
    ...
    TextureOp(const GrRenderTargetContext::TextureSetEntry set[], int cnt,
        ...
        GrAAType overallAAType =
                GrAAType::kNone;  // aa type maximally compatible with all dst rects
        ...
        for (unsigned p = 0; p < fProxyCnt; ++p) {
            ...
            GrQuadType quadType = set[p].fDstClipQuad
                                          ? GrQuadTypeForPoints(set[p].fDstClipQuad, ctm)
    ...
    void onPrepareDraws(Target* target) override {
        ...
        target->recordDraw(std::move(gp), meshes, numProxies, fixedDynamicState,
                           dynamicStateArrays);
    }
