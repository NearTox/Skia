 ...
 private:
  ...
  struct ColorDomainAndAA {
    ColorDomainAndAA(const SkPMColor4f& color, const SkRect& domainRect, GrQuadAAFlags aaFlags)
        : fColor(color), fDomainRect(domainRect), fAAFlags(static_cast<unsigned>(aaFlags)) {
  ...
  TextureOp(
      GrRenderTargetContext::TextureSetEntry set[], int cnt, GrSamplerState::Filter filter,
    ...
    for (unsigned p = 0; p < fMetadata.fProxyCount; ++p) {
      ...
      SkASSERT(
          curProxy->backendFormat().textureType() ==
      ...
      NormalizationParams proxyParams =
          proxy_normalization_params(curProxy, set[p].fProxyView.origin());
    ...
    if (netDomain == Domain::kYes && correctDomainUpToIndex > 0) {
      ...
      while (p < correctDomainUpToIndex && iter.next()) {
        NormalizationParams proxyParams =
            proxy_normalization_params(fViewCountPairs[p].fProxy.get(), set[p].fProxyView.origin());
  ...
  if (blendMode == SkBlendMode::kSrcOver) {
    ...
    if (domain) {
      ...
      fp = GrDomainEffect::Make(
          std::move(fp), correctedDomain, GrTextureDomain::kClamp_Mode, filter);
    }
 ...
 public:
  ...
  void createOp(GrRenderTargetContext::TextureSetEntry set[], int clumpSize, GrAAType aaType) {
    std::unique_ptr<GrDrawOp> op = TextureOp::Make(
        fContext, &set[fNumClumped], clumpSize, fFilter, fSaturate, aaType, fConstraint,
        fViewMatrix, fTextureColorSpaceXform);
...
void GrTextureOp::AddTextureSetOps(
    ...
    GrRenderTargetContext::TextureSetEntry set[], int cnt, GrSamplerState::Filter filter,
    Saturate saturate, SkBlendMode blendMode, GrAAType aaType,
    SkCanvas::SrcRectConstraint constraint, const SkMatrix& viewMatrix,
    sk_sp<GrColorSpaceXform> textureColorSpaceXform) {
  ...
  SkASSERT(
      GrResourceProvider::MaxNumNonAAQuads() <= UINT16_MAX &&
...
GR_DRAW_OP_TEST_DEFINE(TextureOp) {
  ...
  sk_sp<GrTextureProxy> proxy = proxyProvider->createProxy(
      format, desc, GrRenderable::kNo, 1, origin, mipMapped, fit, SkBudgeted::kNo, GrProtected::kNo,
