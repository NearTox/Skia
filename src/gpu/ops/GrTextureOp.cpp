...
static std::tuple<bool /* filter */, bool /* mipmap */> filter_and_mm_have_effect(
...
class TextureOp final : public GrMeshDrawOp {
 public:
  static std::unique_ptr<GrDrawOp> Make(
      ...
      GrSamplerState::MipmapMode mm, const SkPMColor4f& color, GrTextureOp::Saturate saturate,
      GrAAType aaType, DrawQuad* quad, const SkRect* subset) {
    GrOpMemoryPool* pool = context->priv().opMemoryPool();
    return pool->allocate<TextureOp>(
        std::move(proxyView), std::move(textureXform), filter, mm, color, saturate, aaType, quad,
        subset);
  }
  ...
  static std::unique_ptr<GrDrawOp> Make(
    ...
    return std::unique_ptr<GrDrawOp>(new (mem) TextureOp(
        set, cnt, proxyRunCnt, filter, mm, saturate, aaType, constraint, viewMatrix,
 ...
 private:
  ...
  TextureOp(
      ...
      GrTextureOp::Saturate saturate, GrAAType aaType, DrawQuad* quad, const SkRect* subsetRect)
      : INHERITED(ClassID()),
  ...
  static int validate_op(
      GrTextureType textureType, GrAAType aaType, GrSwizzle swizzle, const TextureOp* op) {
  ...
  void validate() const override {
    ...
    for (const GrOp* tmp = this->prevInChain(); tmp; tmp = tmp->prevInChain()) {
      quadCount += validate_op(textureType, aaType, swizzle, static_cast<const TextureOp*>(tmp));
    }
...
void GrTextureOp::AddTextureSetOps(
  ...
  BatchSizeLimiter state(
      rtc, clip, context, cnt, filter, mm, saturate, constraint, viewMatrix,
      std::move(textureColorSpaceXform));
