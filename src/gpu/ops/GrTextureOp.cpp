...
// 1, 1, and height.
static SkRect compute_domain(
    Domain domain, GrSamplerState::Filter filter, GrSurfaceOrigin origin, const SkRect& srcRect,
...
// 1, 1, and height.
static GrPerspQuad compute_src_quad_from_rect(
    GrSurfaceOrigin origin, const SkRect& srcRect, float iw, float ih, float h) {
  // Convert the pixel-space src rectangle into normalized texture coordinates
  SkRect texRect = {iw * srcRect.fLeft, ih * srcRect.fTop, iw * srcRect.fRight,
                    ih * srcRect.fBottom};
  if (origin == kBottomLeft_GrSurfaceOrigin) {
...
static GrPerspQuad compute_src_quad(
    GrSurfaceOrigin origin, const GrPerspQuad& srcQuad, float iw, float ih, float h) {
...
class TextureOp final : public GrMeshDrawOp {
 public:
  static std::unique_ptr<GrDrawOp> Make(
      GrRecordingContext* context, sk_sp<GrTextureProxy> proxy, GrSamplerState::Filter filter,
      const SkPMColor4f& color, const SkRect& srcRect, const SkRect& dstRect, GrAAType aaType,
      GrQuadAAFlags aaFlags, SkCanvas::SrcRectConstraint constraint, const SkMatrix& viewMatrix,
    ...
    return pool->allocate<TextureOp>(
        std::move(proxy), filter, color, dstQuad, srcRect, constraint, nullptr, aaType, aaFlags,
        std::move(textureColorSpaceXform));
  }
  static std::unique_ptr<GrDrawOp> Make(
      GrRecordingContext* context, sk_sp<GrTextureProxy> proxy, GrSamplerState::Filter filter,
      const SkPMColor4f& color, const SkPoint srcQuad[4], const SkPoint dstQuad[4], GrAAType aaType,
      GrQuadAAFlags aaFlags, const SkRect* domain, const SkMatrix& viewMatrix,
      sk_sp<GrColorSpaceXform> textureColorSpaceXform) {
    ...
    return pool->allocate<TextureOp>(
        std::move(proxy), filter, color, grDstQuad, srcRect, constraint, &grSrcQuad, aaType,
        aaFlags, std::move(textureColorSpaceXform));
  }
  static std::unique_ptr<GrDrawOp> Make(
    ...
    for (unsigned p = 0; p < fProxyCnt; ++p) {
      ...
      for (int i = 0; i < fProxies[p].fQuadCnt; ++i, ++q) {
        ...
        str.appendf(
            ...
            info.fSrcRect.fRight, info.fSrcRect.fBottom, quad.point(0).fX, quad.point(0).fY,
            quad.point(1).fX, quad.point(1).fY, quad.point(2).fX, quad.point(2).fY,
            quad.point(3).fX, quad.point(3).fY);
      }
    }
 ...
 private:
  ...
  TextureOp(
      sk_sp<GrTextureProxy> proxy, GrSamplerState::Filter filter, const SkPMColor4f& color,
      const GrPerspQuad& dstQuad, const SkRect& srcRect, SkCanvas::SrcRectConstraint constraint,
      const GrPerspQuad* srcQuad, GrAAType aaType, GrQuadAAFlags aaFlags,
      sk_sp<GrColorSpaceXform> textureColorSpaceXform)
      : INHERITED(ClassID()),
    ...
    if (constraint == SkCanvas::kStrict_SrcRectConstraint &&
        this->filter() == GrSamplerState::Filter::kNearest && aaType != GrAAType::kCoverage) {
      constraint = SkCanvas::kFast_SrcRectConstraint;
    }

    Domain domain = constraint == SkCanvas::kStrict_SrcRectConstraint ? Domain::kYes : Domain::kNo;
  ...
  void tess(
      void* v, const VertexSpec& spec, const GrTextureProxy* proxy, int start, int cnt) const {
    ...
    for (int i = start; i < start + cnt; ++i) {
      ...
      v = GrQuadPerEdgeAA::Tessellate(
          v, spec, device, info.fColor, srcQuad, domain, info.aaFlags());
    }
  }
  ...
  void onPrepareDraws(Target* target) override {
    ...
    sk_sp<GrGeometryProcessor> gp = GrQuadPerEdgeAA::MakeTexturedProcessor(
        vertexSpec, *target->caps().shaderCaps(), textureType, config, samplerState,
      ...
      for (unsigned p = 0; p < op.fProxyCnt; ++p) {
        ...
        if (numAllocatedVertices < meshVertexCnt) {
          vdata = target->makeVertexSpaceAtLeast(
              vertexSize, meshVertexCnt, numQuadVerticesLeft, &vbuffer, &vertexOffsetInBuffer,
        ...
        if (!GrQuadPerEdgeAA::ConfigureMeshIndices(target, &(meshes[m]), vertexSpec, quadCnt)) {
  ...
  void onExecute(GrOpFlushState* flushState, const SkRect& chainBounds) override {
    auto pipelineFlags = (GrAAType::kMSAA == this->aaType()) ? GrPipeline::InputFlags::kHWAntialias
  ...
  CombineResult onCombineIfPossible(GrOp* t, const GrCaps& caps) override {
    ...
    if (fProxyCnt > 1 || that->fProxyCnt > 1 || thisProxy->uniqueID() != thatProxy->uniqueID()) {
  ...
  struct ColorDomainAndAA {
    ...
    ColorDomainAndAA(
        const SkPMColor4f& color, const SkRect& srcRect, int srcQuadIndex, Domain hasDomain,
        GrQuadAAFlags aaFlags)
        : fColor(color),
          fSrcRect(srcRect),
...
std::unique_ptr<GrDrawOp> Make(
  ...
  return TextureOp::Make(
      context, std::move(proxy), filter, color, srcRect, dstRect, aaType, aaFlags, constraint,
      viewMatrix, std::move(textureColorSpaceXform));
}

std::unique_ptr<GrDrawOp> MakeQuad(
    GrRecordingContext* context, sk_sp<GrTextureProxy> proxy, GrSamplerState::Filter filter,
    const SkPMColor4f& color, const SkPoint srcQuad[4], const SkPoint dstQuad[4], GrAAType aaType,
    GrQuadAAFlags aaFlags, const SkRect* domain, const SkMatrix& viewMatrix,
    sk_sp<GrColorSpaceXform> textureXform) {
  return TextureOp::Make(
      context, std::move(proxy), filter, color, srcQuad, dstQuad, aaType, aaFlags, domain,
      viewMatrix, std::move(textureXform));
}
...
bool GetFilterHasEffect(const SkMatrix& viewMatrix, const SkRect& srcRect, const SkRect& dstRect) {
  ...
  if (!viewMatrix.isScaleTranslate() || viewMatrix.getScaleX() < 0.0f ||
      viewMatrix.getScaleY() < 0.0f) {
    return true;
  }
...
GR_DRAW_OP_TEST_DEFINE(TextureOp) {
  ...
  sk_sp<GrTextureProxy> proxy = proxyProvider->createProxy(
      format, desc, origin, mipMapped, fit, SkBudgeted::kNo, GrInternalSurfaceFlags::kNone);
  ...
  auto constraint =
      random->nextBool() ? SkCanvas::kStrict_SrcRectConstraint : SkCanvas::kFast_SrcRectConstraint;
  return GrTextureOp::Make(
      context, std::move(proxy), filter, color, srcRect, rect, aaType, aaFlags, constraint,
