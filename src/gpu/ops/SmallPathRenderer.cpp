...
class SmallPathOp final : public GrMeshDrawOp {
 ...
 public:
  ...
  static GrOp::Owner Make(
      ...
      const SkMatrix& viewMatrix, bool gammaCorrect, const GrUserStencilSettings* stencilSettings) {
    return Helper::FactoryHelper<SmallPathOp>(
        context, std::move(paint), shape, viewMatrix, gammaCorrect, stencilSettings);
  }

  SmallPathOp(
      GrProcessorSet* processorSet, const SkPMColor4f& color, const GrStyledShape& shape,
      const SkMatrix& viewMatrix, bool gammaCorrect, const GrUserStencilSettings* stencilSettings)
  ...
  void visitProxies(const GrVisitProxyFunc& func) const override { fHelper.visitProxies(func); }
  ...
  GrProcessorSet::Analysis finalize(
    ...
    return fHelper.finalizeProcessors(
        caps, clip, clampType, GrProcessorAnalysisCoverage::kSingleChannel, &fShapes.front().fColor,
        &fWideColor);
  }
 ...
 private:
  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    if (fUsesDistanceField) {
      ...
      flushInfo.fGeometryProcessor = GrDistanceFieldPathGeoProc::Make(
          target->allocator(), *target->caps().shaderCaps(), *matrix, fWideColor, views,
          numActiveProxies, GrSamplerState::Filter::kLinear, flags);
    } else {
      SkMatrix invert;
      ...
      flushInfo.fGeometryProcessor = GrBitmapTextGeoProc::Make(
          target->allocator(), *target->caps().shaderCaps(), this->color(), fWideColor, views,
          numActiveProxies, GrSamplerState::Filter::kNearest, kA8_GrMaskFormat, invert, false);
    }
    ...
    for (int i = 0; i < instanceCount; i++) {
      ...
      if (fUsesDistanceField) {
        ...
        if (!shapeData->fAtlasLocator.plotLocator().isValid()) {
          ...
          if (!this->addDFPathToAtlas(
                  target, &flushInfo, atlasMgr, shapeData, args.fShape, ceilDesiredDimension,
                  scale)) {
        ...
        if (!shapeData->fAtlasLocator.plotLocator().isValid()) {
          if (!this->addBMPathToAtlas(
                  target, &flushInfo, atlasMgr, shapeData, args.fShape, args.fViewMatrix)) {
            atlasMgr->deleteCacheEntry(shapeData);
            continue;
          }
        }
      ...
      this->writePathVertices(
          vertices, GrVertexColor(args.fColor, fWideColor), args.fViewMatrix, shapeData);
  ...
  bool addToAtlasWithRetry(
      GrMeshDrawTarget* target, FlushInfo* flushInfo, GrSmallPathAtlasMgr* atlasMgr, int width,
      int height, const void* image, const SkRect& bounds, int srcInset,
      GrSmallPathShapeData* shapeData) const {
    auto resourceProvider = target->resourceProvider();
    auto uploadTarget = target->deferredUploadTarget();

    auto code = atlasMgr->addToAtlas(
        resourceProvider, uploadTarget, width, height, image, &shapeData->fAtlasLocator);
  ...
  bool addDFPathToAtlas(
      GrMeshDrawTarget* target, FlushInfo* flushInfo, GrSmallPathAtlasMgr* atlasMgr,
      GrSmallPathShapeData* shapeData, const GrStyledShape& shape, uint32_t dimension,
    ...
    return this->addToAtlasWithRetry(
        target, flushInfo, atlasMgr, width, height, dfStorage.get(), drawBounds,
        SK_DistanceFieldPad, shapeData);
  }

  bool addBMPathToAtlas(
      GrMeshDrawTarget* target, FlushInfo* flushInfo, GrSmallPathAtlasMgr* atlasMgr,
      GrSmallPathShapeData* shapeData, const GrStyledShape& shape, const SkMatrix& ctm) const {
    const SkRect& bounds = shape.bounds();
    if (bounds.isEmpty()) {
      return false;
    }
    ...
    return this->addToAtlasWithRetry(
        target, flushInfo, atlasMgr, dst.width(), dst.height(), dst.addr(), drawBounds, 0,
        shapeData);
  }

  void writePathVertices(
      GrVertexWriter& vertices, const GrVertexColor& color, const SkMatrix& ctm,
...
GR_DRAW_OP_TEST_DEFINE(SmallPathOp) {
  ...
  return SmallPathOp::Make(
      context, std::move(paint), shape, viewMatrix, gammaCorrect,
...
bool SmallPathRenderer::onDrawPath(const DrawPathArgs& args) {
  GR_AUDIT_TRAIL_AUTO_FRAME(args.fContext->priv().auditTrail(), "SmallPathRenderer::onDrawPath");
  ...
  GrOp::Owner op = SmallPathOp::Make(
      args.fContext, std::move(args.fPaint), *args.fShape, *args.fViewMatrix, args.fGammaCorrect,
