...
class GrSmallPathRenderer::SmallPathOp final : public GrMeshDrawOp {
 ...
 public:
  ...
  static std::unique_ptr<GrDrawOp> Make(
      ...
      ShapeDataList* shapeList, bool gammaCorrect, const GrUserStencilSettings* stencilSettings) {
    return Helper::FactoryHelper<SmallPathOp>(
        context, std::move(paint), shape, viewMatrix, atlas, shapeCache, shapeList, gammaCorrect,
        stencilSettings);
  }
 ...
 private:
  ...
  void onPrepareDraws(Target* target) override {
    ...
    if (fUsesDistanceField) {
      ...
      flushInfo.fGeometryProcessor = GrBitmapTextGeoProc::Make(
          *target->caps().shaderCaps(), this->color(), fWideColor, fAtlas->getProxies(),
          fAtlas->numActivePages(), GrSamplerState::ClampNearest(), kA8_GrMaskFormat, invert,
          false);
    }
    ...
    GrVertexWriter vertices{target->makeVertexSpace(
        kVertexStride, kVerticesPerQuad * instanceCount, &flushInfo.fVertexBuffer,
    ...
    for (int i = 0; i < instanceCount; i++) {
      ...
      if (fUsesDistanceField) {
        ...
        // For sizes less than kIdealMinMIP we want to use as large a distance field as we
        // can so we can preserve as much detail as possible. However, we can't scale down
        // more than a 1/4 of the size without artifacts. So the idea is that we pick the
        // mipsize just bigger than the ideal, and then scale down until we are no more than
        // 4x the original mipsize.
        ...
        if (nullptr == shapeData || !fAtlas->hasID(shapeData->fID)) {
          ...
          if (!this->addDFPathToAtlas(
                  target, &flushInfo, fAtlas, shapeData, args.fShape,
                  SkScalarCeilToInt(desiredDimension), scale)) {
            delete shapeData;
            continue;
          }
        }
        ...
        if (nullptr == shapeData || !fAtlas->hasID(shapeData->fID)) {
          ...
          shapeData = new ShapeData;
          if (!this->addBMPathToAtlas(
                  target, &flushInfo, fAtlas, shapeData, args.fShape, args.fViewMatrix)) {
            delete shapeData;
            continue;
          }
        }
      ...
      this->writePathVertices(
          fAtlas, vertices, GrVertexColor(args.fColor, fWideColor), args.fViewMatrix, shapeData);
  ...
  bool addToAtlas(
      GrMeshDrawOp::Target* target, FlushInfo* flushInfo, GrDrawOpAtlas* atlas, int width,
      int height, const void* image, GrDrawOpAtlas::AtlasID* id, SkIPoint16* atlasLocation) const {
    auto resourceProvider = target->resourceProvider();
    auto uploadTarget = target->deferredUploadTarget();

    GrDrawOpAtlas::ErrorCode code =
        atlas->addToAtlas(resourceProvider, id, uploadTarget, width, height, image, atlasLocation);
    ...
    if (GrDrawOpAtlas::ErrorCode::kTryAgain == code) {
      ...
      code = atlas->addToAtlas(
          resourceProvider, id, uploadTarget, width, height, image, atlasLocation);
    }

    return GrDrawOpAtlas::ErrorCode::kSucceeded == code;
  }

  bool addDFPathToAtlas(
      GrMeshDrawOp::Target* target, FlushInfo* flushInfo, GrDrawOpAtlas* atlas,
      ShapeData* shapeData, const GrShape& shape, uint32_t dimension, SkScalar scale) const {
    ...
    bool succeed = GrGenerateDistanceFieldFromPath(
        (unsigned char*)dfStorage.get(), path, drawMatrix, width, height,
    ...
    if (!succeed) {
      ...
      SkGenerateDistanceFieldFromA8Image(
          (unsigned char*)dfStorage.get(), (const unsigned char*)dst.addr(), dst.width(),
          dst.height(), dst.rowBytes());
#ifndef SK_USE_LEGACY_DISTANCE_FIELDS
    }
#endif
    ...
    if (!this->addToAtlas(
            target, flushInfo, atlas, width, height, dfStorage.get(), &id, &atlasLocation)) {
      return false;
    }
  ...
  bool addBMPathToAtlas(
    ...
    if (!this->addToAtlas(
            target, flushInfo, atlas, dst.width(), dst.height(), dst.addr(), &id, &atlasLocation)) {
      return false;
    }
  ...
  void writePathVertices(
      GrDrawOpAtlas* atlas, GrVertexWriter& vertices, const GrVertexColor& color,
      const SkMatrix& ctm, const ShapeData* shapeData) const {
    SkRect translatedBounds(shapeData->fBounds);
    if (!fUsesDistanceField) {
      translatedBounds.offset(
          SkScalarFloorToScalar(ctm.get(SkMatrix::kMTransX)),
          SkScalarFloorToScalar(ctm.get(SkMatrix::kMTransY)));
    }

    // set up texture coordinates
    GrVertexWriter::TriStrip<uint16_t> texCoords{
        (uint16_t)shapeData->fTextureCoords.fLeft, (uint16_t)shapeData->fTextureCoords.fTop,
        (uint16_t)shapeData->fTextureCoords.fRight, (uint16_t)shapeData->fTextureCoords.fBottom};

    if (fUsesDistanceField && !ctm.hasPerspective()) {
      vertices.writeQuad(GrQuad::MakeFromRect(translatedBounds, ctm), color, texCoords);
    } else {
      vertices.writeQuad(GrVertexWriter::TriStripFromRect(translatedBounds), color, texCoords);
    }
  }
  ...
  void flush(GrMeshDrawOp::Target* target, FlushInfo* flushInfo) const {
    ...
    if (flushInfo->fInstancesToFlush) {
      ...
      mesh->setIndexedPatterned(
          flushInfo->fIndexBuffer, kIndicesPerQuad, kVerticesPerQuad, flushInfo->fInstancesToFlush,
          maxInstancesPerDraw);
  ...
  CombineResult onCombineIfPossible(GrOp* t, const GrCaps& caps) override {
    ...
    if ((thisCtm.hasPerspective() || fHelper.usesLocalCoords()) && !thisCtm.cheapEqualTo(thatCtm)) {
      return CombineResult::kCannotCombine;
    }
...
bool GrSmallPathRenderer::onDrawPath(const DrawPathArgs& args) {
  GR_AUDIT_TRAIL_AUTO_FRAME(
      args.fRenderTargetContext->auditTrail(), "GrSmallPathRenderer::onDrawPath");
  ...
  if (!fAtlas) {
    const GrBackendFormat format =
        args.fContext->priv().caps()->getBackendFormatFromColorType(kAlpha_8_SkColorType);
    fAtlas = GrDrawOpAtlas::Make(
        args.fContext->priv().proxyProvider(), format, GrColorType::kAlpha_8, ATLAS_TEXTURE_WIDTH,
        ATLAS_TEXTURE_HEIGHT, PLOT_WIDTH, PLOT_HEIGHT, GrDrawOpAtlas::AllowMultitexturing::kYes,
        &GrSmallPathRenderer::HandleEviction, (void*)this);
    if (!fAtlas) {
      return false;
    }
  }
...
std::unique_ptr<GrDrawOp> GrSmallPathRenderer::createOp_TestingOnly(
    GrRecordingContext* context, GrPaint&& paint, const GrShape& shape, const SkMatrix& viewMatrix,
    GrDrawOpAtlas* atlas, ShapeCache* shapeCache, ShapeDataList* shapeList, bool gammaCorrect,
...
GR_DRAW_OP_TEST_DEFINE(SmallPathOp) {
  ...
  if (context->priv().contextID() != gTestStruct.fContextID) {
    ...
    gTestStruct.fAtlas = GrDrawOpAtlas::Make(
        context->priv().proxyProvider(), format, GrColorType::kAlpha_8, ATLAS_TEXTURE_WIDTH,
        ATLAS_TEXTURE_HEIGHT, PLOT_WIDTH, PLOT_HEIGHT, GrDrawOpAtlas::AllowMultitexturing::kYes,
  ...
  return GrSmallPathRenderer::createOp_TestingOnly(
      context, std::move(paint), shape, viewMatrix, gTestStruct.fAtlas.get(),
