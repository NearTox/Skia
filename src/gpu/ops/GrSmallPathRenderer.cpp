...
class GrSmallPathRenderer::SmallPathOp final : public GrMeshDrawOp {
 ...
 private:
  ...
  bool addToAtlasWithRetry(
      GrMeshDrawOp::Target* target, FlushInfo* flushInfo, GrSmallPathAtlasMgr* atlasMgr, int width,
  ...
  bool addDFPathToAtlas(
      GrMeshDrawOp::Target* target, FlushInfo* flushInfo, GrSmallPathAtlasMgr* atlasMgr,
...
GrOp::Owner GrSmallPathRenderer::createOp_TestingOnly(
    GrRecordingContext* context, GrPaint&& paint, const GrStyledShape& shape,
    const SkMatrix& viewMatrix, bool gammaCorrect, const GrUserStencilSettings* stencil) {
  return GrSmallPathRenderer::SmallPathOp::Make(
      context, std::move(paint), shape, viewMatrix, gammaCorrect, stencil);
}
