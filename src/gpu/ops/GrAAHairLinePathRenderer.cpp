 ...
 public:
  ...
  GrProcessorSet::Analysis finalize(
    ...
    return fHelper.finalizeProcessors(
        caps, clip, fsaaType, clampType, GrProcessorAnalysisCoverage::kSingleChannel, &fColor,
        nullptr);
  }
...
void AAHairlineOp::onPrepareDraws(Target* target) {
  ...
  if (lineCount) {
    sk_sp<GrGeometryProcessor> lineGP;
    {
      ...
      lineGP = GrDefaultGeoProcFactory::Make(
          target->caps().shaderCaps(), color, Coverage::kAttribute_Type, localCoords,
    ...
    LineVertex* verts = reinterpret_cast<LineVertex*>(
        target->makeVertexSpace(sizeof(LineVertex), vertexCount, &vertexBuffer, &firstVertex));
    ...
    mesh->setIndexedPatterned(
        std::move(linesIndexBuffer), kIdxsPerLineSeg, kLineSegNumVertices, lineCount,
  ...
  if (quadCount || conicCount) {
    sk_sp<GrGeometryProcessor> quadGP(GrQuadEffect::Make(
        this->color(), *geometryProcessorViewM, GrClipEdgeType::kHairlineAA, target->caps(),
        *geometryProcessorLocalM, fHelper.usesLocalCoords(), this->coverage()));

    sk_sp<GrGeometryProcessor> conicGP(GrConicEffect::Make(
        this->color(), *geometryProcessorViewM, GrClipEdgeType::kHairlineAA, target->caps(),
        *geometryProcessorLocalM, fHelper.usesLocalCoords(), this->coverage()));
