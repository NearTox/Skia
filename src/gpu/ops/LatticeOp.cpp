  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    for (int i = 0; i < patchCnt; i++) {
      ...
      while (patch.fIter->next(&srcR, &dstR)) {
        ...
        if (isScaleTranslate) {
          vertices.writeQuad(
              GrVertexWriter::TriStripFromRect(dstR), GrVertexWriter::TriStripFromRect(texCoords),
          ...
          vertices.write(
              mappedPts[0], SkPoint::Make(texCoords.fLeft, texCoords.fTop), texDomain, patchColor);
          vertices.write(
              mappedPts[3], SkPoint::Make(texCoords.fLeft, texCoords.fBottom), texDomain,
              patchColor);
          vertices.write(
              mappedPts[1], SkPoint::Make(texCoords.fRight, texCoords.fTop), texDomain, patchColor);
          vertices.write(
              mappedPts[2], SkPoint::Make(texCoords.fRight, texCoords.fBottom), texDomain,
              patchColor);
        }
      }
    }
