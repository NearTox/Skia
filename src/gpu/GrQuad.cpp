...
static void map_quad_general(
    const V4f& qx, const V4f& qy, const SkMatrix& m, V4f* xs, V4f* ys, V4f* ws) {
  *xs = mad(m.getScaleX(), qx, mad(m.getSkewX(), qy, m.getTranslateX()));
  *ys = mad(m.getSkewY(), qx, mad(m.getScaleY(), qy, m.getTranslateY()));
  if (m.hasPerspective()) {
    V4f w = mad(m.getPerspX(), qx, mad(m.getPerspY(), qy, m.get(SkMatrix::kMPersp2)));
    if (ws) {
...
static void map_rect_general(
    const SkRect& rect, const SkMatrix& matrix, V4f* xs, V4f* ys, V4f* ws) {
...
template <typename Q>
void GrResolveAATypeForQuad(
    GrAAType requestedAAType, GrQuadAAFlags requestedEdgeFlags, const Q& quad, GrAAType* outAAType,
...
template void GrResolveAATypeForQuad(
    GrAAType, GrQuadAAFlags, const GrQuad&, GrAAType*, GrQuadAAFlags*);
template void GrResolveAATypeForQuad(
    GrAAType, GrQuadAAFlags, const GrPerspQuad&, GrAAType*, GrQuadAAFlags*);
...
bool GrQuad::aaHasEffectOnRect() const { return aa_affects_rect(fX[0], fY[0], fX[3], fY[3]); }
