...
sk_sp<GrTexture> GrResourceProvider::getExactScratch(
    const GrSurfaceDesc& desc, SkBudgeted budgeted, Flags flags) {
...
sk_sp<GrTexture> GrResourceProvider::createTexture(
    const GrSurfaceDesc& desc, SkBudgeted budgeted, SkBackingFit fit, const GrMipLevel& mipLevel,
    Flags flags) {
  ASSERT_SINGLE_OWNER
  ...
  if (GrPixelConfigToColorType(desc.fConfig, &colorType)) {
    ...
    sk_sp<GrTextureProxy> proxy =
        proxyProvider->createWrapped(std::move(tex), kTopLeft_GrSurfaceOrigin);
    if (!proxy) {
      return nullptr;
    }
    auto srcInfo = SkImageInfo::Make(desc.fWidth, desc.fHeight, colorType, kUnknown_SkAlphaType);
...
sk_sp<GrTexture> GrResourceProvider::createApproxTexture(const GrSurfaceDesc& desc, Flags flags) {
  ASSERT_SINGLE_OWNER
...
sk_sp<GrTexture> GrResourceProvider::refScratchTexture(const GrSurfaceDesc& desc, Flags flags) {
  ...
  if (!SkToBool(desc.fFlags & kPerformInitialClear_GrSurfaceFlag) &&
    ...
    GrGpuResource* resource =
        fCache->findAndRefScratchResource(key, GrSurface::WorstCaseSize(desc), scratchFlags);
    if (resource) {
...
bool GrResourceProvider::attachStencilAttachment(GrRenderTarget* rt) {
  ...
  if (!rt->wasDestroyed() && rt->canAttemptStencilAttachment()) {
    ...
    GrStencilAttachment::ComputeSharedStencilAttachmentKey(
        width, height, rt->numStencilSamples(), &sbKey);
