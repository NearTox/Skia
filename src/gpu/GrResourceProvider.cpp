...
sk_sp<GrTexture> GrResourceProvider::createTexture(
  ...
  return fGpu->createTexture(
      ...
      colorType, tempColorType, tmpTexels.get(), numMipLevels);
}
...
sk_sp<GrTexture> GrResourceProvider::createTexture(
    SkISize dimensions, const GrBackendFormat& format, GrTextureType textureType,
    GrColorType colorType, GrRenderable renderable, int renderTargetSampleCnt, SkBudgeted budgeted,
    SkBackingFit fit, GrProtected isProtected, const GrMipLevel& mipLevel) {
  ASSERT_SINGLE_OWNER
  ...
  if (SkBackingFit::kApprox == fit) {
    ...
    auto tex = this->createApproxTexture(
        dimensions, format, textureType, renderable, renderTargetSampleCnt, isProtected);
    if (!tex) {
      return nullptr;
    }
    ...
    return this->createTexture(
        ...
        GrMipmapped::kNo, isProtected, &mipLevel);
  }
}
...
void GrResourceProvider::assignUniqueKeyToResource(
    const GrUniqueKey& key, GrGpuResource* resource) {
  ASSERT_SINGLE_OWNER
...
sk_sp<const GrGpuBuffer> GrResourceProvider::findOrMakeStaticBuffer(
    GrGpuBufferType intendedType, size_t size, const void* staticData, const GrUniqueKey& key) {
...
sk_sp<const GrGpuBuffer> GrResourceProvider::findOrMakeStaticBuffer(
    GrGpuBufferType intendedType, size_t size, const GrUniqueKey& uniqueKey,
