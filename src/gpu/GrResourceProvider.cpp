...
sk_sp<GrTexture> GrResourceProvider::createTexture(
  ...
  return fGpu->createTexture(
      dimensions, format, renderable, renderTargetSampleCnt, budgeted, isProtected, colorType,
      tempColorType, tmpTexels.get(), numMipLevels);
}
...
sk_sp<GrTexture> GrResourceProvider::createTexture(
  ...
  if (SkBackingFit::kApprox == fit) {
    ...
    return this->createTexture(
        dimensions, format, colorType, renderable, renderTargetSampleCnt, budgeted,
...
sk_sp<GrTexture> GrResourceProvider::createTexture(
  ...
  return fGpu->createTexture(
      dimensions, format, renderable, renderTargetSampleCnt, mipMapped, budgeted, isProtected);
}
  ...
  if (!rt->wasDestroyed() && rt->canAttemptStencilAttachment()) {
    ...
    GrAttachment::ComputeSharedAttachmentUniqueKey(
        *this->caps(), stencilFormat, rt->dimensions(),
        GrAttachment::UsageFlags::kStencilAttachment, numStencilSamples, isProtected, &sbKey);
    auto stencil = this->findByUniqueKey<GrAttachment>(sbKey);
    if (!stencil) {
      // Need to try and create a new stencil
      stencil = this->gpu()->makeStencilAttachmentForRenderTarget(
