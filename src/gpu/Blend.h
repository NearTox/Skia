...
struct BlendInfo {
  ...
  bool operator==(const BlendInfo& other) const noexcept {
    return fEquation == other.fEquation && fSrcBlend == other.fSrcBlend &&
           fDstBlend == other.fDstBlend && fBlendConstant == other.fBlendConstant &&
...
static constexpr bool BlendCoeffRefsSrc(const BlendCoeff coeff) noexcept {
  return BlendCoeff::kSC == coeff || BlendCoeff::kISC == coeff || BlendCoeff::kSA == coeff ||
         BlendCoeff::kISA == coeff;
}

static constexpr bool BlendCoeffRefsDst(const BlendCoeff coeff) noexcept {
  return BlendCoeff::kDC == coeff || BlendCoeff::kIDC == coeff || BlendCoeff::kDA == coeff ||
         BlendCoeff::kIDA == coeff;
}

static constexpr bool BlendCoeffRefsSrc2(const BlendCoeff coeff) noexcept {
  return BlendCoeff::kS2C == coeff || BlendCoeff::kIS2C == coeff || BlendCoeff::kS2A == coeff ||
         BlendCoeff::kIS2A == coeff;
}

static constexpr bool BlendCoeffsUseSrcColor(BlendCoeff srcCoeff, BlendCoeff dstCoeff) noexcept {
  return BlendCoeff::kZero != srcCoeff || BlendCoeffRefsSrc(dstCoeff);
}

static constexpr bool BlendCoeffsUseDstColor(
    BlendCoeff srcCoeff, BlendCoeff dstCoeff, bool srcColorIsOpaque) noexcept {
  return BlendCoeffRefsDst(srcCoeff) ||
         (dstCoeff != BlendCoeff::kZero && !(dstCoeff == BlendCoeff::kISA && srcColorIsOpaque));
}

static constexpr bool BlendEquationIsAdvanced(BlendEquation equation) noexcept {
  return equation >= BlendEquation::kFirstAdvanced && equation != BlendEquation::kIllegal;
}

static constexpr bool BlendModifiesDst(
    BlendEquation equation, BlendCoeff srcCoeff, BlendCoeff dstCoeff) noexcept {
...
static constexpr bool BlendCoeffRefsConstant(const BlendCoeff coeff) noexcept {
  return coeff == BlendCoeff::kConstC || coeff == BlendCoeff::kIConstC;
}

static constexpr bool BlendShouldDisable(
    BlendEquation equation, BlendCoeff srcCoeff, BlendCoeff dstCoeff) noexcept {
 ...
 */
static constexpr bool BlendAllowsCoverageAsAlpha(
    BlendEquation equation, BlendCoeff srcCoeff, BlendCoeff dstCoeff) noexcept {
  return BlendEquationIsAdvanced(equation) || !BlendModifiesDst(equation, srcCoeff, dstCoeff) ||
