...
bool GrCaps::validateSurfaceParams(
    const SkISize& dimensions, const GrBackendFormat& format, GrPixelConfig config,
    GrRenderable renderable, int renderTargetSampleCnt, GrMipMapped mipped) const {
...
GrCaps::SupportedRead GrCaps::supportedReadPixelsColorType(
  ...
  if ((componentFlags == kRGBA_SkColorTypeComponentFlags ||
    ...
    switch (read.fOffsetAlignmentForTransferBuffer & 0b11) {
      ...
      case 2: read.fOffsetAlignmentForTransferBuffer *= 2;
      // offset alignment is not a multiple of 2.
      default: read.fOffsetAlignmentForTransferBuffer *= 4;
    }
  }
  return read;
}
...
bool GrCaps::AreConfigsCompatible(GrPixelConfig genericConfig, GrPixelConfig specificConfig) {
  ...
  switch (genericConfig) {
    ...
    case kRGB_888_GrPixelConfig:
      compatible =
          kRGB_888_GrPixelConfig == specificConfig || kRGB_888X_GrPixelConfig == specificConfig;
      break;
    case kRGBA_8888_GrPixelConfig:
      compatible =
          kRGBA_8888_GrPixelConfig == specificConfig || kBGRA_8888_GrPixelConfig == specificConfig;
      break;
    default: compatible = genericConfig == specificConfig; break;
  }
