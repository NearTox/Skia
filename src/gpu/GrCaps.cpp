...
void GrCaps::dumpJSON(SkJSONWriter* writer) const {
  ...
  writer->appendBool(
      "Two-sided Stencil Refs And Masks Must Match", fTwoSidedStencilRefsAndMasksMustMatch);
  ...
  writer->appendBool(
      "Supports transfers from buffers to textures", fTransferFromBufferToTextureSupport);
  writer->appendBool(
  ...
  writer->appendBool(
      "Disable msaa clip mask atlas on current driver [workaround]", fDriverDisableMSAAClipAtlas);
  writer->appendBool(
  ...
  writer->appendBool(
      "Requires manual FB barrier after tessellated stencilDraw [workaround]",
  ...
  writer->appendString("Blend Equation Support", kBlendEquationSupportNames[fBlendEquationSupport]);
...
bool GrCaps::validateSurfaceParams(
    ...
    int renderTargetSampleCnt, GrMipmapped mipped, GrTextureType textureType) const {
...
GrCaps::SupportedRead GrCaps::supportedReadPixelsColorType(
    GrColorType srcColorType, const GrBackendFormat& srcFormat, GrColorType dstColorType) const {
  SupportedRead read = this->onSupportedReadPixelsColorType(srcColorType, srcFormat, dstColorType);
  ...
  if ((channelFlags == kRGBA_SkColorChannelFlags || channelFlags == kRGB_SkColorChannelFlags ||
    ...
    switch (read.fOffsetAlignmentForTransferBuffer & 0b11) {
      ...
      case 2: read.fOffsetAlignmentForTransferBuffer *= 2; break;
      // offset alignment is not a multiple of 2.
      default: read.fOffsetAlignmentForTransferBuffer *= 4; break;
    }
  }
  return read;
}
...
bool GrCaps::areColorTypeAndFormatCompatible(
...
GrSwizzle GrCaps::getReadSwizzle(const GrBackendFormat& format, GrColorType colorType) const {
  ...
  if (compression != SkImage::CompressionType::kNone) {
    ...
    SkDEBUGFAILF(
        "Illegal color type (%d) and compressed format (%d) combination.", (int)colorType,
        (int)compression);
    return {};
  }
...
GrDstSampleFlags GrCaps::getDstSampleFlagsForProxy(
    const GrRenderTargetProxy* rt, bool drawUsesMSAA) const {
  SkASSERT(rt);
...
std::tuple<GrColorType, GrBackendFormat> GrCaps::getFallbackColorTypeAndFormat(
    GrColorType ct, int sampleCnt) const {
  do {
