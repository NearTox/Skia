...
class ResourceKey {
 public:
  ...
  class Builder {
   ...
   protected:
    Builder(ResourceKey* key, uint32_t domain, int data32Count) noexcept : fKey(key) {
 ...
 protected:
  ...
  ResourceKey() noexcept { this->reset(); }

  bool operator==(const ResourceKey& that) const noexcept {
  ...
  ResourceKey& operator=(const ResourceKey& that) noexcept {
    if (this != &that) {
      if (!that.isValid()) {
        this->reset();
      } else {
 ...
 private:
  ...
  void validate() const noexcept {
    ...
    SkASSERT(
        ...
        ResourceKeyHash(&fKey[kHash_MetaDataIdx] + 1, this->internalSize() - sizeof(uint32_t)));
...
class ScratchKey : public ResourceKey {
 public:
  ...
  ScratchKey() noexcept = default;

  ScratchKey(const ScratchKey& that) noexcept { *this = that; }

  ResourceType resourceType() const noexcept { return this->domain(); }

  ScratchKey& operator=(const ScratchKey& that) noexcept {
  ...
  class Builder : public ResourceKey::Builder {
   public:
    Builder(ScratchKey* key, ResourceType type, int data32Count) noexcept
...
class UniqueKey : public ResourceKey {
 public:
  ...
  UniqueKey() noexcept : fTag(nullptr) {}

  UniqueKey(const UniqueKey& that) noexcept { *this = that; }

  UniqueKey& operator=(const UniqueKey& that) noexcept {
    this->ResourceKey::operator=(that);
  ...
  sk_sp<SkData> refCustomData() const noexcept { return fData; }
  ...
  class Builder : public ResourceKey::Builder {
   public:
    Builder(UniqueKey* key, Domain type, int data32Count, const char* tag = nullptr) noexcept
        : ResourceKey::Builder(key, type, data32Count) {
      key->fTag = tag;
    }
    ...
    Builder(
        UniqueKey* key, const UniqueKey& innerKey, Domain domain, int extraData32Cnt,
        const char* tag = nullptr)
        : ResourceKey::Builder(key, domain, Data32CntForInnerKey(innerKey) + extraData32Cnt) {
...
#define SKGPU_DEFINE_STATIC_UNIQUE_KEY(name)                              \
  ...
  static const skgpu::UniqueKey& name = *reinterpret_cast<skgpu::UniqueKey*>(name##_storage.get())
...
class UniqueKeyInvalidatedMessage {
 public:
  ...
  UniqueKeyInvalidatedMessage(
      const UniqueKey& key, uint32_t contextUniqueID, bool inThreadSafeCache = false)
...
static inline bool SkShouldPostMessageToBus(
    const UniqueKeyInvalidatedMessage& msg, uint32_t msgBusUniqueID) {
