...
class GrTriangulator {
 public:
  ...
  static int TriangulateSimpleInnerPolygons(
      const SkPath& path, GrEagerVertexAllocator* vertexAllocator, bool* isLinear) {
    GrTriangulator triangulator(path);
 ...
 protected:
  ...
  bool splitEdge(Edge* edge, Vertex* v, EdgeList* activeEdges, Vertex** current, const Comparator&);
  bool intersectEdgePair(
      Edge* left, Edge* right, EdgeList* activeEdges, Vertex** current, const Comparator&);
  Vertex* makeSortedVertex(
      const SkPoint&, uint8_t alpha, VertexList* mesh, Vertex* reference, const Comparator&);
...
struct GrTriangulator::MonotonePoly {
  ...
  void* emitTriangle(Vertex* prev, Vertex* curr, Vertex* next, bool emitCoverage, void* data) const;
};
