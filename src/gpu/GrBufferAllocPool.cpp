...
sk_sp<GrCpuBuffer> GrBufferAllocPool::CpuBufferCache::makeBuffer(
    size_t size, bool mustBeInitialized) {
  SkASSERT(size > 0);
...
#define UNMAP_BUFFER(block)                                                          \
  do {                                                                               \
    TRACE_EVENT_INSTANT1(                                                            \
        "skia.gpu", "GrBufferAllocPool Unmapping Buffer", TRACE_EVENT_SCOPE_THREAD,  \
        "percent_unwritten", (float)((block).fBytesFree) / (block).fBuffer->size()); \
...
void* GrBufferAllocPool::makeSpaceAtLeast(
    size_t minSize, size_t fallbackSize, size_t alignment, sk_sp<const GrBuffer>* buffer,
    size_t* offset, size_t* actualSize) {
  VALIDATE();
  ...
  size_t usedBytes =
      (fBlocks.empty()) ? 0 : fBlocks.back().fBuffer->size() - fBlocks.back().fBytesFree;
  size_t pad = align_up_pad(usedBytes, alignment);
...
void* GrVertexBufferAllocPool::makeSpace(
  ...
  void* ptr =
      INHERITED::makeSpace(SkSafeMath::Mul(vertexSize, vertexCount), vertexSize, buffer, &offset);
...
void* GrVertexBufferAllocPool::makeSpaceAtLeast(
  ...
  void* ptr = INHERITED::makeSpaceAtLeast(
      SkSafeMath::Mul(vertexSize, minVertexCount), SkSafeMath::Mul(vertexSize, fallbackVertexCount),
      vertexSize, buffer, &offset, &actualSize);
...
void* GrIndexBufferAllocPool::makeSpace(
  ...
  size_t offset SK_INIT_TO_AVOID_WARNING;
  void* ptr = INHERITED::makeSpace(
      SkSafeMath::Mul(indexCount, sizeof(uint16_t)), sizeof(uint16_t), buffer, &offset);
...
void* GrIndexBufferAllocPool::makeSpaceAtLeast(
  ...
  size_t actualSize SK_INIT_TO_AVOID_WARNING;
  void* ptr = INHERITED::makeSpaceAtLeast(
      SkSafeMath::Mul(minIndexCount, sizeof(uint16_t)),
      SkSafeMath::Mul(fallbackIndexCount, sizeof(uint16_t)), sizeof(uint16_t), buffer, &offset,
      &actualSize);
