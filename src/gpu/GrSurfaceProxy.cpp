...
static bool is_valid_fully_lazy(const GrSurfaceDesc& desc, SkBackingFit fit) {
  return desc.fWidth <= 0 && desc.fHeight <= 0 && desc.fConfig != kUnknown_GrPixelConfig &&
         desc.fSampleCnt == 1 && SkBackingFit::kApprox == fit;
}
...
bool GrSurfaceProxyPriv::AttachStencilIfNeeded(
    GrResourceProvider* resourceProvider, GrSurface* surface, bool needsStencil) {
  if (needsStencil) {
...
bool GrSurfaceProxy::instantiateImpl(
    GrResourceProvider* resourceProvider, int sampleCnt, bool needsStencil,
    GrSurfaceDescFlags descFlags, GrMipMapped mipMapped, const GrUniqueKey* uniqueKey) {
  ...
  sk_sp<GrSurface> surface =
      this->createSurfaceImpl(resourceProvider, sampleCnt, needsStencil, descFlags, mipMapped);
  if (!surface) {
    return false;
  }
...
sk_sp<GrSurfaceContext> GrSurfaceProxy::TestCopy(
    GrRecordingContext* context, const GrSurfaceDesc& dstDesc, GrSurfaceOrigin origin,
...
bool GrSurfaceProxyPriv::doLazyInstantiation(GrResourceProvider* resourceProvider) {
  ...
  bool needsStencil =
      fProxy->asRenderTargetProxy() ? fProxy->asRenderTargetProxy()->needsStencil() : false;
