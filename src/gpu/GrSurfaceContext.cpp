...
#define RETURN_FALSE_IF_ABANDONED    \
  if (this->fContext->abandoned()) { \
    return false;                    \
  }

std::unique_ptr<GrSurfaceContext> GrSurfaceContext::Make(
    GrRecordingContext* context, GrSurfaceProxyView readView, const GrColorInfo& info) {
  ...
  if (proxy->asRenderTargetProxy()) {
    ...
    if (info.colorType() != GrColorType::kUnknown) {
      writeSwizzle =
          context->priv().caps()->getWriteSwizzle(proxy->backendFormat(), info.colorType());
    }
    ...
    if (info.alphaType() == kPremul_SkAlphaType || info.alphaType() == kOpaque_SkAlphaType) {
      surfaceContext = std::make_unique<GrSurfaceDrawContext>(
          context, std::move(readView), std::move(writeView), info.colorType(),
          info.refColorSpace(),
          /*surface props*/ nullptr);
    } else {
      surfaceContext = std::make_unique<GrSurfaceFillContext>(
          context, std::move(readView), std::move(writeView), info);
    }
  } else {
...
std::unique_ptr<GrSurfaceContext> GrSurfaceContext::Make(
    GrRecordingContext* context, const GrImageInfo& info, const GrBackendFormat& format,
    SkBackingFit fit, GrSurfaceOrigin origin, GrRenderable renderable, int sampleCount,
    GrMipmapped mipmapped, GrProtected isProtected, SkBudgeted budgeted) {
  SkASSERT(context);
  ...
  sk_sp<GrTextureProxy> proxy = context->priv().proxyProvider()->createProxy(
      format, info.dimensions(), renderable, sampleCount, mipmapped, fit, budgeted, isProtected);
  if (!proxy) {
    return nullptr;
  }
...
std::unique_ptr<GrSurfaceContext> GrSurfaceContext::Make(
    GrRecordingContext* context, const GrImageInfo& info, SkBackingFit fit, GrSurfaceOrigin origin,
    GrRenderable renderable, int sampleCount, GrMipmapped mipmapped, GrProtected isProtected,
    SkBudgeted budgeted) {
  GrBackendFormat format =
      context->priv().caps()->getDefaultBackendFormat(info.colorType(), renderable);
  return Make(
      context, info, format, fit, origin, renderable, sampleCount, mipmapped, isProtected,
      budgeted);
}
...
bool GrSurfaceContext::readPixels(GrDirectContext* dContext, GrPixmap dst, SkIPoint pt) {
  ...
  SkColorSpaceXformSteps::Flags flags = SkColorSpaceXformSteps{this->colorInfo(), dst.info()}.flags;
  ...
  GrBackendFormat defaultRGBAFormat =
  ...
  bool canvas2DFastPath =
      unpremul && !needColorConversion &&
      (GrColorType::kRGBA_8888 == dst.colorType() || GrColorType::kBGRA_8888 == dst.colorType()) &&
      SkToBool(srcProxy->asTextureProxy()) &&
      (srcColorType == GrColorType::kRGBA_8888 || srcColorType == GrColorType::kBGRA_8888) &&
      defaultRGBAFormat.isValid() && dContext->priv().validPMUPMConversionExists();
  ...
  if (readFlag == GrCaps::SurfaceReadPixelsSupport::kCopyToTexture2D || canvas2DFastPath) {
    ...
    if (this->asTextureProxy()) {
      GrColorType colorType = (canvas2DFastPath || srcIsCompressed) ? GrColorType::kRGBA_8888
                                                                    : this->colorInfo().colorType();
      SkAlphaType alphaType = canvas2DFastPath ? dst.alphaType() : this->colorInfo().alphaType();
      GrImageInfo tempInfo(
          colorType, alphaType, this->colorInfo().refColorSpace(), dst.dimensions());
      ...
      if (canvas2DFastPath) {
        ...
        if (dst.colorType() == GrColorType::kBGRA_8888) {
          ...
          dst = GrPixmap(
              dst.info().makeColorType(GrColorType::kRGBA_8888), dst.addr(), dst.rowBytes());
        }
      } else {
      ...
      sfc->fillRectToRectWithFP(
          SkIRect::MakePtSize(pt, dst.dimensions()), SkIRect::MakeSize(dst.dimensions()),
          std::move(fp));
      pt = {0, 0};
      tempCtx = std::move(sfc);
    } else {
      auto restrictions = this->caps()->getDstCopyRestrictions(
      ...
      if (restrictions.fMustCopyWholeSrc) {
        copy =
            GrSurfaceProxy::Copy(fContext, srcProxy, this->origin(), kMipMapped, kFit, kBudgeted);
      } else {
        auto srcRect = SkIRect::MakePtSize(pt, dst.dimensions());
        copy = GrSurfaceProxy::Copy(
            fContext, srcProxy, this->origin(), kMipMapped, srcRect, kFit, kBudgeted,
  ...
  bool makeTight = !caps->readPixelsRowBytesSupport() && dst.rowBytes() != dst.info().minRowBytes();
  ...
  if (convert) {
    GrImageInfo tmpInfo(
        supportedRead.fColorType, this->colorInfo().alphaType(), this->colorInfo().refColorSpace(),
        dst.dimensions());
  ...
  dContext->submit();
  if (!dContext->priv().getGpu()->readPixels(
          srcSurface, pt.fX, pt.fY, dst.width(), dst.height(), this->colorInfo().colorType(),
...
bool GrSurfaceContext::writePixels(GrDirectContext* dContext, GrPixmap src, SkIPoint pt) {
  ...
  SkColorSpaceXformSteps::Flags flags = SkColorSpaceXformSteps{src.info(), this->colorInfo()}.flags;
  bool unpremul = flags.unpremul,
       needColorConversion = flags.linearize || flags.gamut_transform || flags.encode,
       premul = flags.premul;
  ...
  bool canvas2DFastPath =
      ...
      (src.colorType() == GrColorType::kRGBA_8888 || src.colorType() == GrColorType::kBGRA_8888) &&
      this->asFillContext() &&
      (dstColorType == GrColorType::kRGBA_8888 || dstColorType == GrColorType::kBGRA_8888) &&
      rgbaDefaultFormat.isValid() && dContext->priv().validPMUPMConversionExists();
  ...
  if (!caps->surfaceSupportsWritePixels(dstSurface) || canvas2DFastPath) {
    ...
    if (canvas2DFastPath) {
      tempColorInfo = {
          GrColorType::kRGBA_8888, kUnpremul_SkAlphaType, this->colorInfo().refColorSpace()};
    ...
    GrSurfaceOrigin tempOrigin = this->asFillContext() ? kTopLeft_GrSurfaceOrigin : this->origin();
    auto tempProxy = dContext->priv().proxyProvider()->createProxy(
        format, src.dimensions(), GrRenderable::kNo, 1, GrMipmapped::kNo, SkBackingFit::kApprox,
    ...
    if (this->asFillContext()) {
      ...
      this->asFillContext()->fillRectToRectWithFP(
          SkIRect::MakeSize(src.dimensions()), SkIRect::MakePtSize(pt, src.dimensions()),
          std::move(fp));
    } else {
  ...
  GrColorType allowedColorType =
      caps->supportedWritePixelsColorType(
              this->colorInfo().colorType(), dstProxy->backendFormat(), src.colorType())
          .fColorType;
  bool flip = this->origin() == kBottomLeft_GrSurfaceOrigin;
  bool makeTight =
      !caps->writePixelsRowBytesSupport() && src.rowBytes() != src.info().minRowBytes();
  ...
  if (convert) {
    GrImageInfo tmpInfo(
        allowedColorType, this->colorInfo().alphaType(), this->colorInfo().refColorSpace(),
        src.dimensions());
  ...
  return dContext->priv().getGpu()->writePixels(
      dstSurface, pt.fX, pt.fY, src.width(), src.height(), this->colorInfo().colorType(),
      src.colorType(), src.addr(), src.rowBytes());
}

void GrSurfaceContext::asyncRescaleAndReadPixels(
    GrDirectContext* dContext, const SkImageInfo& info, const SkIRect& srcRect,
    RescaleGamma rescaleGamma, RescaleMode rescaleMode, ReadPixelsCallback callback,
  ...
  if (needsRescale) {
    ...
    if (this->origin() == kBottomLeft_GrSurfaceOrigin || xform) {
      ...
      tempRTC = GrSurfaceDrawContext::Make(
          dContext, this->colorInfo().colorType(), info.refColorSpace(), SkBackingFit::kApprox,
  ...
  return rtc->asyncReadPixels(
      dContext, SkIRect::MakeXYWH(x, y, info.width(), info.height()), info.colorType(), callback,
      callbackContext);
}
...
void GrSurfaceContext::asyncReadPixels(
    GrDirectContext* dContext, const SkIRect& rect, SkColorType colorType,
    ReadPixelsCallback callback, ReadPixelsContext callbackContext) {
  ...
  auto* finishContext =
      new FinishContext{callback,  callbackContext,     rect.size(),
                        colorType, mappedBufferManager, std::move(transferResult)};
  ...
  dContext->priv().flushSurface(
      this->asSurfaceProxy(), SkSurface::BackendSurfaceAccess::kNoAccess, flushInfo);
}

void GrSurfaceContext::asyncRescaleAndReadPixelsYUV420(
    GrDirectContext* dContext, SkYUVColorSpace yuvColorSpace, sk_sp<SkColorSpace> dstColorSpace,
    const SkIRect& srcRect, SkISize dstSize, RescaleGamma rescaleGamma, RescaleMode rescaleMode,
    ReadPixelsCallback callback, ReadPixelsContext callbackContext) {
  ...
  if (needsRescale) {
    ...
    auto tempRTC =
        this->rescale(info, kTopLeft_GrSurfaceOrigin, srcRect, rescaleGamma, rescaleMode);
    if (!tempRTC) {
      callback(callbackContext, nullptr);
      return;
    }
    ...
    if (xform) {
      ...
      auto tempRTC = GrSurfaceDrawContext::Make(
          dContext, this->colorInfo().colorType(), dstColorSpace, SkBackingFit::kApprox, dstSize, 1,
  ...
  auto uRTC = GrSurfaceDrawContext::MakeWithFallback(
      dContext, GrColorType::kAlpha_8, dstColorSpace, SkBackingFit::kApprox, {halfW, halfH}, 1,
      GrMipmapped::kNo, GrProtected::kNo, kTopLeft_GrSurfaceOrigin);
  ...
  auto uTexFP = GrTextureEffect::Make(
      srcView, this->colorInfo().alphaType(), texMatrix, GrSamplerState::Filter::kLinear);
  ...
  if (doSynchronousRead) {
    ...
    if (!yRTC->readPixels(dContext, yPmp, {0, 0}) || !uRTC->readPixels(dContext, uPmp, {0, 0}) ||
...
std::unique_ptr<GrSurfaceDrawContext> GrSurfaceContext::rescale(
    const GrImageInfo& info, GrSurfaceOrigin origin, SkIRect srcRect, RescaleGamma rescaleGamma,
  ...
  auto sdc = GrSurfaceDrawContext::MakeWithFallback(
      fContext, info.colorType(), info.refColorSpace(), SkBackingFit::kExact, info.dimensions(), 1,
      GrMipmapped::kNo, this->asSurfaceProxy()->isProtected(), origin);
  if (!sdc ||
      !this->rescaleInto(
          sdc.get(), SkIRect::MakeSize(sdc->dimensions()), srcRect, rescaleGamma, rescaleMode)) {
    return nullptr;
  }
  return sdc;
}

bool GrSurfaceContext::rescaleInto(
    GrSurfaceDrawContext* dst, SkIRect dstRect, SkIRect srcRect, RescaleGamma rescaleGamma,
    RescaleMode rescaleMode) {
  SkASSERT(dst);
  ...
  if (!texView.asTextureProxy()) {
    texView = GrSurfaceProxyView::Copy(
        fContext, std::move(texView), GrMipmapped::kNo, srcRect, SkBackingFit::kApprox,
        SkBudgeted::kNo);
    if (!texView) {
      return false;
    }
  ...
  if (rescaleGamma == RescaleGamma::kLinear && this->colorInfo().colorSpace() &&
    ...
    auto xform = GrColorSpaceXform::Make(
        this->colorInfo().colorSpace(), srcAlphaType, cs.get(), kPremul_SkAlphaType);
    ...
    linearRTC->drawTexture(
        nullptr, std::move(texView), srcAlphaType, GrSamplerState::Filter::kNearest,
        GrSamplerState::MipmapMode::kNone, SkBlendMode::kSrc, SK_PMColor4fWHITE,
        SkRect::Make(srcRect), SkRect::Make(srcRect.size()), GrAA::kNo, GrQuadAAFlags::kNone,
  ...
  while (srcRect.size() != finalSize) {
    ...
    if (nextDims == finalSize) {
      ...
      xform = GrColorSpaceXform::Make(
          input->colorInfo().colorSpace(), input->colorInfo().alphaType(),
          dst->colorInfo().colorSpace(), dst->colorInfo().alphaType());
      stepDst = dst;
      stepDstRect = dstRect;
    } else {
      tempB = GrSurfaceDrawContext::MakeWithFallback(
          fContext, input->colorInfo().colorType(), input->colorInfo().refColorSpace(),
          SkBackingFit::kApprox, nextDims, 1, GrMipmapped::kNo, GrProtected::kNo, dst->origin());
      if (!tempB) {
        return false;
      }
      stepDst = tempB.get();
    ...
    if (rescaleMode == RescaleMode::kRepeatedCubic) {
      ...
      matrix.setScaleTranslate(
          (float)srcRect.width() / nextDims.width(), (float)srcRect.height() / nextDims.height(),
          srcRect.x(), srcRect.y());
      std::unique_ptr<GrFragmentProcessor> fp;
      ...
      fp = GrBicubicEffect::MakeSubset(
          std::move(texView), input->colorInfo().alphaType(), matrix, kWM, kWM,
      ...
      stepDst->fillRectToRect(
          nullptr, std::move(paint), GrAA::kNo, SkMatrix::I(), SkRect::Make(stepDstRect),
      ...
      stepDst->drawTexture(
          nullptr, std::move(texView), srcAlphaType, filter, GrSamplerState::MipmapMode::kNone,
          SkBlendMode::kSrc, SK_PMColor4fWHITE, SkRect::Make(srcRect), SkRect::Make(stepDstRect),
...
GrSurfaceContext::PixelTransferResult GrSurfaceContext::transferPixels(
  ...
  this->drawingManager()->newTransferFromRenderTask(
      this->asSurfaceProxyRef(), srcRect, this->colorInfo().colorType(), supportedRead.fColorType,
      buffer, 0);
