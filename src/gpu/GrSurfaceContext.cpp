...
GrSurfaceContext::GrSurfaceContext(
    GrRecordingContext* context, GrColorType colorType, SkAlphaType alphaType,
    sk_sp<SkColorSpace> colorSpace, GrSurfaceOrigin origin, GrSwizzle texSwizzle)
    : fContext(context),
      fOrigin(origin),
...
bool GrSurfaceContext::readPixels(
  ...
  if (readFlag == GrCaps::SurfaceReadPixelsSupport::kCopyToTexture2D || canvas2DFastPath) {
    ...
    auto tempCtx = direct->priv().makeDeferredRenderTargetContext(
        SkBackingFit::kApprox, dstInfo.width(), dstInfo.height(), colorType, std::move(cs), 1,
    ...
    if (canvas2DFastPath) {
      fp = direct->priv().createPMToUPMEffect(GrSimpleTextureEffect::Make(
          sk_ref_sp(srcProxy->asTextureProxy()), this->colorInfo().alphaType(), SkMatrix::I()));
...
bool GrSurfaceContext::writePixels(
  ...
  if (!caps->surfaceSupportsWritePixels(dstSurface) || canvas2DFastPath) {
    ...
    auto tempProxy = direct->priv().proxyProvider()->createProxy(
        format, desc, GrRenderable::kNo, 1, tempOrigin, GrMipMapped::kNo, SkBackingFit::kApprox,
    ...
    if (this->asRenderTargetContext()) {
      ...
      if (canvas2DFastPath) {
        fp = direct->priv().createUPMToPMEffect(
            GrSimpleTextureEffect::Make(std::move(tempProxy), alphaType, SkMatrix::I()));
...
bool GrSurfaceContext::copy(GrSurfaceProxy* src, const SkIRect& srcRect, const SkIPoint& dstPoint) {
  ...
  SkASSERT(
      caps->makeConfigSpecific(src->config(), src->backendFormat()) ==
      caps->makeConfigSpecific(
          this->asSurfaceProxy()->config(), this->asSurfaceProxy()->backendFormat()));
...
std::unique_ptr<GrRenderTargetContext> GrSurfaceContext::rescale(
  ...
  if (!texProxy) {
    texProxy = GrSurfaceProxy::Copy(
        fContext, this->asSurfaceProxy(), GrMipMapped::kNo, srcRect, SkBackingFit::kApprox,
        SkBudgeted::kNo);
    if (!texProxy) {
      return nullptr;
    }
...
GrSurfaceContext::PixelTransferResult GrSurfaceContext::transferPixels(
  ...
  if (!this->caps()->transferBufferSupport() || !supportedRead.fOffsetAlignmentForTransferBuffer) {
    return {};
  }
