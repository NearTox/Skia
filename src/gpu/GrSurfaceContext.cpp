...
std::unique_ptr<GrSurfaceContext> GrSurfaceContext::Make(
    GrRecordingContext* context, GrSurfaceProxyView readView, GrColorType colorType,
  ...
  if (proxy->asRenderTargetProxy()) {
    ...
    if (colorType != GrColorType::kUnknown) {
      writeSwizzle = context->priv().caps()->getWriteSwizzle(proxy->backendFormat(), colorType);
    }
    GrSurfaceProxyView writeView(readView.refProxy(), readView.origin(), writeSwizzle);
    surfaceContext = std::make_unique<GrRenderTargetContext>(
...
std::unique_ptr<GrSurfaceContext> GrSurfaceContext::Make(
    GrRecordingContext* context, SkISize dimensions, const GrBackendFormat& format,
    GrRenderable renderable, int renderTargetSampleCnt, GrMipmapped mipMapped,
    GrProtected isProtected, GrSurfaceOrigin origin, GrColorType colorType, SkAlphaType alphaType,
    sk_sp<SkColorSpace> colorSpace, SkBackingFit fit, SkBudgeted budgeted) {
  GrSwizzle swizzle;
  ...
  return GrSurfaceContext::Make(
      context, std::move(view), colorType, alphaType, std::move(colorSpace));
}
...
bool GrSurfaceContext::readPixels(
    GrDirectContext* dContext, const GrImageInfo& origDstInfo, void* dst, size_t rowBytes,
    SkIPoint pt) {
  ASSERT_SINGLE_OWNER
  ...
  if (readFlag == GrCaps::SurfaceReadPixelsSupport::kCopyToTexture2D || canvas2DFastPath) {
    ...
    if (this->asTextureProxy()) {
      ...
      tempCtx = GrRenderTargetContext::Make(
          dContext, colorType, std::move(cs), SkBackingFit::kApprox, dstInfo.dimensions(), 1,
      ...
      tempCtx = GrSurfaceContext::Make(
          dContext, std::move(view), this->colorInfo().colorType(), this->colorInfo().alphaType(),
  ...
  if (!dContext->priv().getGpu()->readPixels(
          srcSurface, pt.fX, pt.fY, dstInfo.width(), dstInfo.height(),
          this->colorInfo().colorType(), supportedRead.fColorType, readDst, readRB)) {
    return false;
  }
...
bool GrSurfaceContext::writePixels(
    GrDirectContext* dContext, const GrImageInfo& origSrcInfo, const void* src, size_t rowBytes,
    SkIPoint pt) {
  ASSERT_SINGLE_OWNER
  RETURN_FALSE_IF_ABANDONED
  SkDEBUGCODE(this->validate();)
  ...
  if (!caps->surfaceSupportsWritePixels(dstSurface) || canvas2DFastPath) {
    ...
    GrSurfaceContext tempCtx(
        dContext, tempView, colorType, alphaType, this->colorInfo().refColorSpace());
  ...
  GrColorType allowedColorType =
      caps->supportedWritePixelsColorType(
              this->colorInfo().colorType(), dstProxy->backendFormat(), srcInfo.colorType())
          .fColorType;
  bool flip = this->origin() == kBottomLeft_GrSurfaceOrigin;
  ...
  return dContext->priv().getGpu()->writePixels(
      dstSurface, pt.fX, pt.fY, srcInfo.width(), srcInfo.height(), this->colorInfo().colorType(),
...
void GrSurfaceContext::asyncRescaleAndReadPixels(
    ...
    RescaleGamma rescaleGamma, SkFilterQuality rescaleQuality, ReadPixelsCallback callback,
...
void GrSurfaceContext::asyncRescaleAndReadPixelsYUV420(
    ...
    const SkIRect& srcRect, SkISize dstSize, RescaleGamma rescaleGamma,
  ...
  while (srcRect.size() != info.dimensions()) {
    ...
    if (nextDims == info.dimensions()) {
      ...
      xform = GrColorSpaceXform::Make(
          input->colorInfo().colorSpace(), input->colorInfo().alphaType(), cs.get(),
          info.alphaType());
    }
    tempB = GrRenderTargetContext::MakeWithFallback(
        fContext, colorType, std::move(cs), SkBackingFit::kApprox, nextDims, 1, GrMipmapped::kNo,
    ...
    if (rescaleQuality == kHigh_SkFilterQuality) {
      ...
      paint.setColorFragmentProcessor(std::move(fp));
      paint.setPorterDuffXPFactory(SkBlendMode::kSrc);
      tempB->fillRectToRect(nullptr, std::move(paint), GrAA::kNo, SkMatrix::I(), dstRect, dstRect);
    } else {
      ...
      tempB->drawTexture(
          ...
          SkBlendMode::kSrc, SK_PMColor4fWHITE, SkRect::Make(srcRect), dstRect, GrAA::kNo,
...
GrSemaphoresSubmitted GrSurfaceContext::flush(
