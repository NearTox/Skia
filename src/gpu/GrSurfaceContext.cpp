...
bool GrSurfaceContext::readPixels(
  ...
  if (!direct->priv().getGpu()->readPixels(
          srcSurface, pt.fX, pt.fY, dstInfo.width(), dstInfo.height(), supportedRead.fColorType,
          readDst, readRB)) {
    return false;
  }
  ...
  if (convert) {
    return GrConvertPixels(
        dstInfo, dst, rowBytes, tmpInfo, readDst, readRB, flip, supportedRead.fSwizzle);
  }
  return true;
}
...
bool GrSurfaceContext::writePixels(
  ...
  if (!caps->surfaceSupportsWritePixels(dstSurface) || canvas2DFastPath) {
    ...
    auto tempProxy = direct->priv().proxyProvider()->createProxy(
        format, desc, GrRenderable::kNo, 1, tempOrigin, SkBackingFit::kApprox, SkBudgeted::kYes,
        GrProtected::kNo);
  ...
  return direct->priv().getGpu()->writePixels(
      dstSurface, pt.fX, pt.fY, srcInfo.width(), srcInfo.height(), srcColorType, src, rowBytes);
}
...
sk_sp<GrRenderTargetContext> GrSurfaceContext::rescale(
  ...
  while (stepsX || stepsY) {
    ...
    if (!stepsX && !stepsY) {
      ...
      xform = GrColorSpaceXform::Make(
          currCtx->colorSpaceInfo().colorSpace(), currCtx->colorSpaceInfo().alphaType(), cs.get(),
          info.alphaType());
    }
