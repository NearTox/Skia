...
std::unique_ptr<GrSurfaceContext> GrSurfaceContext::Make(
  ...
  if (proxy->asRenderTargetProxy()) {
    ...
    surfaceContext.reset(new GrSurfaceContext(
        context, std::move(readView), colorType, alphaType, std::move(colorSpace)));
  }
  SkDEBUGCODE(surfaceContext->validate();) return surfaceContext;
}
...
bool GrSurfaceContext::readPixels(
  ...
  bool canvas2DFastPath =
      ...
      defaultRGBAFormat.isValid() && direct->priv().validPMUPMConversionExists();
  ...
  if (readFlag == GrCaps::SurfaceReadPixelsSupport::kCopyToTexture2D || canvas2DFastPath) {
    ...
    auto tempCtx = GrRenderTargetContext::Make(
        direct, colorType, std::move(cs), SkBackingFit::kApprox, dstInfo.dimensions(), 1,
  ...
  direct->submit();
  if (!direct->priv().getGpu()->readPixels(
...
bool GrSurfaceContext::writePixels(
  ...
  bool canvas2DFastPath =
      ...
      rgbaDefaultFormat.isValid() && direct->priv().validPMUPMConversionExists();
  ...
  if (!caps->surfaceSupportsWritePixels(dstSurface) || canvas2DFastPath) {
    ...
    GrSurfaceContext tempCtx(
        direct, tempView, colorType, alphaType, this->colorInfo().refColorSpace());
...
std::unique_ptr<GrRenderTargetContext> GrSurfaceContext::rescale(
    const GrImageInfo& info, GrSurfaceOrigin origin, SkIRect srcRect,
    SkSurface::RescaleGamma rescaleGamma, SkFilterQuality rescaleQuality) {
  ...
  if (rescaleGamma == SkSurface::kLinear && this->colorInfo().colorSpace() &&
    ...
    linearRTC->drawTexture(
        ...
        SkBlendMode::kSrc, SK_PMColor4fWHITE, SkRect::Make(srcRect), SkRect::Make(srcRect.size()),
  ...
  while (srcRect.size() != info.dimensions()) {
    ...
    if (rescaleQuality == kHigh_SkFilterQuality) {
      ...
      fp = GrBicubicEffect::MakeSubset(
          std::move(texView), prevAlphaType, matrix, kWM, kWM, SkRect::Make(srcRect), dir,
          *this->caps());
      if (xform) {
      ...
      tempB->drawTexture(
          nullptr, std::move(texView), srcAlphaType, filter, SkBlendMode::kSrc, SK_PMColor4fWHITE,
          SkRect::Make(srcRect), dstRect, GrAA::kNo, GrQuadAAFlags::kNone, constraint,
