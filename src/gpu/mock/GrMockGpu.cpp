...
void GrMockGpu::querySampleLocations(GrRenderTarget* rt, SkTArray<SkPoint>* sampleLocations) {
  ...
  while (numRemainingSamples > 0) {
    ...
    switch (numRemainingSamples) {
      case 0:
      case 1: sampleLocations->push_back().set(.5, .5); break;
      case 2:
...
sk_sp<GrAttachment> GrMockGpu::makeStencilAttachmentForRenderTarget(
  ...
  return sk_sp<GrAttachment>(new GrMockAttachment(
      this, dimensions, GrAttachment::UsageFlags::kStencilAttachment, rt->numSamples()));
}
