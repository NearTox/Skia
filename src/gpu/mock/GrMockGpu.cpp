...
sk_sp<GrGpu> GrMockGpu::Make(
    const GrMockOptions* mockOptions, const GrContextOptions& contextOptions, GrContext* context) {
...
void GrMockGpu::submit(GrGpuCommandBuffer* buffer) {
  if (buffer->asRTCommandBuffer()) {
    this->submitCommandBuffer(static_cast<GrMockGpuRTCommandBuffer*>(buffer->asRTCommandBuffer()));
  }
...
GrMockGpu::GrMockGpu(
    GrContext* context, const GrMockOptions& options, const GrContextOptions& contextOptions)
...
sk_sp<GrTexture> GrMockGpu::onCreateTexture(
  ...
  if (renderable == GrRenderable::kYes) {
    ...
    return sk_sp<GrTexture>(new GrMockTextureRenderTarget(
        this, budgeted, desc, renderTargetSampleCnt, isProtected, mipMapsStatus, texInfo, rtInfo));
  }
...
sk_sp<GrRenderTarget> GrMockGpu::onWrapBackendRenderTarget(
  ...
  return sk_sp<GrRenderTarget>(new GrMockRenderTarget(
      this, GrMockRenderTarget::kWrapped, desc, rt.sampleCnt(), isProtected, info));
}
