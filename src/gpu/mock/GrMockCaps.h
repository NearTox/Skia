...
class GrMockCaps : public GrCaps {
 public:
  ...
  bool isFormatCopyable(GrColorType, const GrBackendFormat& format) const override { return false; }
  ...
  int getRenderTargetSampleCount(
      int requestCount, GrColorType, const GrBackendFormat& format) const override {
    if (!format.getMockColorType()) {
      return 0;
    }
