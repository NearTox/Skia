...
sk_sp<const GrCaps> GrContextPriv::refCaps() const { return fContext->refCaps(); }

sk_sp<GrSkSLFPFactoryCache> GrContextPriv::fpFactoryCache() { return fContext->fpFactoryCache(); }

sk_sp<GrOpMemoryPool> GrContextPriv::refOpMemoryPool() { return fContext->refOpMemoryPool(); }
...
sk_sp<GrSurfaceContext> GrContextPriv::makeDeferredSurfaceContext(
    const GrBackendFormat& format, const GrSurfaceDesc& dstDesc, GrSurfaceOrigin origin,
    GrMipMapped mipMapped, SkBackingFit fit, SkBudgeted isDstBudgeted,
    sk_sp<SkColorSpace> colorSpace, const SkSurfaceProps* props) {
  return fContext->makeDeferredSurfaceContext(
      format, dstDesc, origin, mipMapped, fit, isDstBudgeted, std::move(colorSpace), props);
}

sk_sp<GrRenderTargetContext> GrContextPriv::makeDeferredRenderTargetContext(
    const GrBackendFormat& format, SkBackingFit fit, int width, int height, GrPixelConfig config,
    sk_sp<SkColorSpace> colorSpace, int sampleCnt, GrMipMapped mipMapped, GrSurfaceOrigin origin,
    const SkSurfaceProps* surfaceProps, SkBudgeted budgeted) {
  return fContext->makeDeferredRenderTargetContext(
      format, fit, width, height, config, std::move(colorSpace), sampleCnt, mipMapped, origin,
...
sk_sp<GrTextureContext> GrContextPriv::makeBackendTextureContext(
    const GrBackendTexture& tex, GrSurfaceOrigin origin, sk_sp<SkColorSpace> colorSpace) {
  ASSERT_SINGLE_OWNER
...
sk_sp<GrRenderTargetContext> GrContextPriv::makeBackendTextureRenderTargetContext(
  ...
  return this->drawingManager()->makeRenderTargetContext(
      std::move(proxy), std::move(colorSpace), props);
}

sk_sp<GrRenderTargetContext> GrContextPriv::makeBackendRenderTargetRenderTargetContext(
    const GrBackendRenderTarget& backendRT, GrSurfaceOrigin origin, sk_sp<SkColorSpace> colorSpace,
    const SkSurfaceProps* surfaceProps, ReleaseProc releaseProc, ReleaseContext releaseCtx) {
  ASSERT_SINGLE_OWNER

  sk_sp<GrSurfaceProxy> proxy =
      this->proxyProvider()->wrapBackendRenderTarget(backendRT, origin, releaseProc, releaseCtx);
  if (!proxy) {
    return nullptr;
  }

  return this->drawingManager()->makeRenderTargetContext(
      std::move(proxy), std::move(colorSpace), surfaceProps);
}
...
sk_sp<GrRenderTargetContext> GrContextPriv::makeVulkanSecondaryCBRenderTargetContext(
  ...
  return this->drawingManager()->makeRenderTargetContext(
      std::move(proxy), imageInfo.refColorSpace(), props);
}

GrSemaphoresSubmitted GrContextPriv::flushSurfaces(
    GrSurfaceProxy* proxies[], int numProxies, const GrFlushInfo& info) {
  ASSERT_SINGLE_OWNER
...
bool GrContextPriv::readSurfacePixels(
    GrSurfaceContext* src, int left, int top, int width, int height, GrColorType dstColorType,
    SkColorSpace* dstColorSpace, void* buffer, size_t rowBytes, uint32_t pixelOpsFlags) {
  ASSERT_SINGLE_OWNER
  ...
  if (!GrSurfacePriv::AdjustReadPixelParams(
          srcSurface->width(), srcSurface->height(), GrColorTypeBytesPerPixel(dstColorType), &left,
  ...
  if (!fContext->priv().caps()->surfaceSupportsReadPixels(srcSurface) || canvas2DFastPath) {
    ...
    sk_sp<GrRenderTargetContext> tempCtx = this->makeDeferredRenderTargetContext(
        format, SkBackingFit::kApprox, width, height, config, std::move(cs), 1, GrMipMapped::kNo,
    ...
    if (canvas2DFastPath) {
      fp = fContext->createPMToUPMEffect(
          GrSimpleTextureEffect::Make(sk_ref_sp(srcProxy->asTextureProxy()), SkMatrix::I()));
    ...
    tempCtx->asRenderTargetContext()->fillRectToRect(
        GrNoClip(), std::move(paint), GrAA::kNo, SkMatrix::I(), SkRect::MakeWH(width, height),
        SkRect::MakeXYWH(left, top, width, height));

    uint32_t flags = canvas2DFastPath ? 0 : pixelOpsFlags;
    return this->readSurfacePixels(
        tempCtx.get(), 0, 0, width, height, dstColorType, dstColorSpace, buffer, rowBytes, flags);
  }
  ...
  GrColorType allowedColorType =
      fContext->priv().caps()->supportedReadPixelsColorType(srcProxy->config(), dstColorType);
  ...
  if (convert) {
    ...
    auto tempII = SkImageInfo::Make(
        width, height, srcSkColorType, tempAT, src->colorSpaceInfo().refColorSpace());
  ...
  if (!fContext->fGpu->readPixels(
          srcSurface, left, top, width, height, allowedColorType, buffer, rowBytes)) {
    return false;
  }
...
bool GrContextPriv::writeSurfacePixels(
    GrSurfaceContext* dst, int left, int top, int width, int height, GrColorType srcColorType,
    SkColorSpace* srcColorSpace, const void* buffer, size_t rowBytes, uint32_t pixelOpsFlags) {
  ASSERT_SINGLE_OWNER
  RETURN_VALUE_IF_ABANDONED(false)
  SkASSERT(dst);
  SkASSERT(buffer);
  ...
  if (!GrSurfacePriv::AdjustWritePixelParams(
          dstSurface->width(), dstSurface->height(), GrColorTypeBytesPerPixel(srcColorType), &left,
          &top, &width, &height, &buffer, &rowBytes)) {
    return false;
  }
  ...
  bool canvas2DFastPath =
      !fContext->priv().caps()->avoidWritePixelsFastPath() && premul && !needColorConversion &&
  ...
  if (!caps->surfaceSupportsWritePixels(dstSurface) || canvas2DFastPath) {
    ...
    if (!this->writeSurfacePixels(
            tempCtx.get(), 0, 0, width, height, tmpColorType, srcColorSpace, buffer, rowBytes,
            flags)) {
      return false;
    }
    ...
    if (dst->asRenderTargetContext()) {
      ...
      if (!this->caps()->canCopySurface(
              dst->asSurfaceProxy(), tempProxy.get(), srcRect, dstPoint)) {
        return false;
      }
  ...
  GrColorType allowedColorType =
      fContext->priv().caps()->supportedWritePixelsColorType(dstProxy->config(), srcColorType);
  ...
  if (convert) {
    ...
    SkPixmap src(
        SkImageInfo::Make(width, height, srcSkColorType, srcAlphaType, sk_ref_sp(srcColorSpace)),
        buffer, rowBytes);
    auto tempSrcII = SkImageInfo::Make(
        width, height, dstSkColorType, kPremul_SkAlphaType, dst->colorSpaceInfo().refColorSpace());
  ...
  return this->getGpu()->writePixels(
      dstSurface, left, top, width, height, srcColorType, buffer, rowBytes);
}
...
void GrContextPriv::dumpCacheStatsKeyValuePairs(
    SkTArray<SkString>* keys, SkTArray<double>* values) const {
#if GR_CACHE_STATS
...
void GrContextPriv::dumpGpuStatsKeyValuePairs(
    SkTArray<SkString>* keys, SkTArray<double>* values) const {
#if GR_GPU_STATS
...
sk_sp<SkImage> GrContextPriv::testingOnly_getFontAtlasImage(
  ...
  sk_sp<SkImage> image(new SkImage_Gpu(
      sk_ref_sp(fContext), kNeedNewImageUniqueID, kPremul_SkAlphaType, proxies[index], nullptr));
  return image;
}
...
//////////////////////////////////////////////////////////////////////////////
GrBackendTexture GrContextPriv::createBackendTexture(
    int width, int height, GrBackendFormat backendFormat, const SkColor4f& color,
  ...
  return gpu->createBackendTexture(
      width, height, backendFormat, mipMapped, renderable, nullptr, 0, color);
}

GrBackendTexture GrContextPriv::createBackendTexture(
    int width, int height, SkColorType colorType, const SkColor4f& color, GrMipMapped mipMapped,
