...
std::unique_ptr<GrTextureContext> GrContextPriv::makeDeferredTextureContext(
  ...
  return fContext->makeDeferredTextureContext(
      fit, width, height, colorType, alphaType, std::move(colorSpace), mipMapped, origin, budgeted,
      isProtected);
}
...
std::unique_ptr<GrTextureContext> GrContextPriv::makeBackendTextureContext(
  ...
  return this->drawingManager()->makeTextureContext(
...
GrContextPriv::makeBackendTextureAsRenderTargetRenderTargetContext(
  ...
  sk_sp<GrSurfaceProxy> proxy(
      this->proxyProvider()->wrapBackendTextureAsRenderTarget(tex, colorType, origin, sampleCnt));
  if (!proxy) {
    return nullptr;
  }
...
std::unique_ptr<GrRenderTargetContext> GrContextPriv::makeVulkanSecondaryCBRenderTargetContext(
  ...
  return this->drawingManager()->makeRenderTargetContext(
      std::move(proxy), SkColorTypeToGrColorType(imageInfo.colorType()), imageInfo.refColorSpace(),
      props);
}
...
GrBackendTexture GrContextPriv::createBackendTexture(
    const SkPixmap srcData[], int numLevels, GrRenderable renderable, GrProtected isProtected) {
  ...
  return gpu->createBackendTexture(
      baseWidth, baseHeight, backendFormat,
      GrMipMapped::kNo,  // TODO: use real mipmap setting here
      renderable, srcData[0].addr(), srcData[0].rowBytes(), nullptr, isProtected);
}
