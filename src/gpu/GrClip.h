...
class GrClip {
 public:
   ...
   */
  virtual Effect apply(
      GrRecordingContext*, GrRenderTargetContext*, GrAAType, bool hasUserStencilSettings,
   ...
   */
  static SkIRect GetPixelIBounds(
      const SkRect& bounds, GrAA aa, BoundsType mode = BoundsType::kExterior) {
    ...
    if (mode == BoundsType::kExterior) {
      return SkIRect::MakeLTRB(
          roundLow(bounds.fLeft), roundLow(bounds.fTop), roundHigh(bounds.fRight),
          roundHigh(bounds.fBottom));
    } else {
      return SkIRect::MakeLTRB(
          roundHigh(bounds.fLeft), roundHigh(bounds.fTop), roundLow(bounds.fRight),
          roundLow(bounds.fBottom));
    }
  }
...
class GrHardClip : public GrClip {
 ...
 private:
  Effect apply(
      GrRecordingContext*, GrRenderTargetContext* rtc, GrAAType aa, bool hasUserStencilSettings,
