...
class GrContextPriv {
 public:
  ...
  sk_sp<GrSurfaceContext> makeWrappedSurfaceContext(
      sk_sp<GrSurfaceProxy>, sk_sp<SkColorSpace> = nullptr, const SkSurfaceProps* = nullptr);

  sk_sp<GrSurfaceContext> makeDeferredSurfaceContext(
      const GrBackendFormat&, const GrSurfaceDesc&, GrSurfaceOrigin, GrMipMapped, SkBackingFit,
      SkBudgeted, sk_sp<SkColorSpace> colorSpace = nullptr, const SkSurfaceProps* = nullptr);
  ...
  sk_sp<GrRenderTargetContext> makeBackendRenderTargetRenderTargetContext(
      const GrBackendRenderTarget&, GrSurfaceOrigin origin, sk_sp<SkColorSpace> colorSpace,
      const SkSurfaceProps* = nullptr, ReleaseProc = nullptr, ReleaseContext = nullptr);
   ...
   *         pixel configs
   */
  bool readSurfacePixels(
      GrSurfaceContext* src, int left, int top, int width, int height, GrColorType dstColorType,
      SkColorSpace* dstColorSpace, void* buffer, size_t rowBytes = 0, uint32_t pixelOpsFlags = 0);
   ...
   */
  bool writeSurfacePixels(
      GrSurfaceContext* dst, int left, int top, int width, int height, GrColorType srcColorType,
      SkColorSpace* srcColorSpace, const void* buffer, size_t rowBytes, uint32_t pixelOpsFlags = 0);
  ...
  GrBackendTexture createBackendTexture(
      int width, int height, GrBackendFormat, const SkColor4f& color, GrMipMapped, GrRenderable);
 ...
 private:
  constexpr explicit GrContextPriv(GrContext* context) noexcept : fContext(context) {}
