...
size_t GrTexture::onGpuMemorySize() const {
  return GrSurface::ComputeSize(
      this->config(), this->width(), this->height(), 1, this->texturePriv().mipMapped(), false);
}

/////////////////////////////////////////////////////////////////////////////
GrTexture::GrTexture(
    GrGpu* gpu, const GrSurfaceDesc& desc, GrTextureType textureType, GrMipMapsStatus mipMapsStatus)
...
void GrTexture::computeScratchKey(GrScratchKey* key) const {
  if (!GrPixelConfigIsCompressed(this->config())) {
    ...
    GrTexturePriv::ComputeScratchKey(
        this->config(), this->width(), this->height(), SkToBool(rt), sampleCount,
        this->texturePriv().mipMapped(), key);
  }
}

void GrTexturePriv::ComputeScratchKey(
    GrPixelConfig config, int width, int height, bool isRenderTarget, int sampleCnt,
...
void GrTexturePriv::ComputeScratchKey(const GrSurfaceDesc& desc, GrScratchKey* key) {
  ...
  return ComputeScratchKey(
      desc.fConfig, desc.fWidth, desc.fHeight, SkToBool(desc.fFlags & kRenderTarget_GrSurfaceFlag),
      desc.fSampleCnt, GrMipMapped::kNo, key);
}
