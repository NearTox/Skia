...
size_t GrTexture::onGpuMemorySize() const {
  ...
  return GrSurface::ComputeSize(
      caps, this->backendFormat(), this->dimensions(), 1, this->mipmapped());
}
...
bool GrTexture::StealBackendTexture(
    sk_sp<GrTexture> texture, GrBackendTexture* backendTexture,
...
void GrTexture::computeScratchKey(GrScratchKey* key) const {
  if (!this->getGpu()->caps()->isFormatCompressed(this->backendFormat())) {
    ...
    ComputeScratchKey(
        *this->getGpu()->caps(), this->backendFormat(), this->dimensions(), renderable, sampleCount,
        this->mipmapped(), isProtected, key);
  }
}

void GrTexture::ComputeScratchKey(
    const GrCaps& caps, const GrBackendFormat& format, SkISize dimensions, GrRenderable renderable,
  ...
  builder[4] = (static_cast<uint32_t>(mipMapped) << 0) | (static_cast<uint32_t>(isProtected) << 1) |
               (static_cast<uint32_t>(renderable) << 2) | (static_cast<uint32_t>(sampleCnt) << 3);
}
