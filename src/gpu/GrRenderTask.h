...
class GrRenderTask : public SkRefCnt {
 ...
 private:
  ...
  struct TopoSortTraits {
    static uint32_t GetIndex(GrRenderTask* renderTask) { return renderTask->getIndex(); }
  ...
  struct ClusterTraits {
    static int NumTargets(GrRenderTask* renderTask) { return renderTask->numTargets(); }
    ...
    static uint32_t GetID(GrRenderTask* renderTask) { return renderTask->uniqueID(); }
  };
