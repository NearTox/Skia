...
GrBackendFormat::GrBackendFormat(const GrBackendFormat& that) noexcept
  ...
  switch (fBackend) {
#ifdef SK_GL
    case GrBackendApi::kOpenGL: fGLFormat = that.fGLFormat; break;
#endif
#ifdef SK_VULKAN
    case GrBackendApi::kVulkan: fVk = that.fVk; break;
#endif
#ifdef SK_METAL
    case GrBackendApi::kMetal: fMtlFormat = that.fMtlFormat; break;
#endif
#ifdef SK_DIRECT3D
    case GrBackendApi::kDirect3D: fDxgiFormat = that.fDxgiFormat; break;
#endif
#ifdef SK_DAWN
    case GrBackendApi::kDawn: fDawnFormat = that.fDawnFormat; break;
#endif
    case GrBackendApi::kMock: fMock = that.fMock; break;
    default: SK_ABORT("Unknown GrBackend");
  }
}

GrBackendFormat& GrBackendFormat::operator=(const GrBackendFormat& that) noexcept {
  if (this != &that) {
...
uint32_t GrBackendFormat::channelMask() const {
  ...
  switch (fBackend) {
#ifdef SK_GL
    case GrBackendApi::kOpenGL: return GrGLFormatChannels(GrGLFormatFromGLEnum(fGLFormat));
#endif
#ifdef SK_VULKAN
    case GrBackendApi::kVulkan: return GrVkFormatChannels(fVk.fFormat);
#endif
#ifdef SK_METAL
    case GrBackendApi::kMetal: return GrMtlFormatChannels(fMtlFormat);
#endif
#ifdef SK_DAWN
    case GrBackendApi::kDawn: return GrDawnFormatChannels(fDawnFormat);
#endif
#ifdef SK_DIRECT3D
    case GrBackendApi::kDirect3D: return GrDxgiFormatChannels(fDxgiFormat);
#endif
...
GrColorType GrBackendFormat::asMockColorType() const noexcept {
  if (this->isValid() && GrBackendApi::kMock == fBackend) {
    SkASSERT(
        fMock.fCompressionType == SkImage::CompressionType::kNone ||
...
SkImage::CompressionType GrBackendFormat::asMockCompressionType() const noexcept {
  if (this->isValid() && GrBackendApi::kMock == fBackend) {
    SkASSERT(
        fMock.fCompressionType == SkImage::CompressionType::kNone ||
        fMock.fColorType == GrColorType::kUnknown);
...
GrBackendFormat GrBackendFormat::MakeMock(
    GrColorType colorType, SkImage::CompressionType compression) noexcept {
...
bool GrBackendFormat::operator==(const GrBackendFormat& that) const noexcept {
  ...
  switch (fBackend) {
#ifdef SK_GL
    case GrBackendApi::kOpenGL: return fGLFormat == that.fGLFormat; break;
#endif
#ifdef SK_VULKAN
...
#endif
#ifdef SK_METAL
    case GrBackendApi::kMetal: return fMtlFormat == that.fMtlFormat; break;
#endif
#ifdef SK_DAWN
    case GrBackendApi::kDawn: return fDawnFormat == that.fDawnFormat; break;
#endif
    case GrBackendApi::kMock:
      return fMock.fColorType == that.fMock.fColorType &&
             fMock.fCompressionType == that.fMock.fCompressionType;
#ifdef SK_DIRECT3D
    case GrBackendApi::kDirect3D: return fDxgiFormat == that.fDxgiFormat;
#endif
    default: SK_ABORT("Unknown GrBackend");
  }
  return false;
...
sk_sp<GrGLTextureParameters> GrBackendTexture::getGLTextureParams() const noexcept {
...
GrBackendTexture::GrBackendTexture(int width, int height, const GrD3DTextureResourceInfo& d3dInfo)
    : GrBackendTexture(
          ...
          sk_sp<GrD3DResourceState>(
              new GrD3DResourceState(static_cast<D3D12_RESOURCE_STATES>(d3dInfo.fResourceState)))) {
}

GrBackendTexture::GrBackendTexture(
    int width, int height, const GrD3DTextureResourceInfo& d3dInfo, sk_sp<GrD3DResourceState> state)
    : fIsValid(true),
      fWidth(width),
      fHeight(height),
...
void GrBackendTexture::cleanup() noexcept {
#ifdef SK_GL
  if (this->isValid() && GrBackendApi::kOpenGL == fBackend) {
    fGLInfo.cleanup();
  }
...
GrBackendTexture::GrBackendTexture(const GrBackendTexture& that) noexcept : fIsValid(false) {
  *this = that;
}

GrBackendTexture& GrBackendTexture::operator=(const GrBackendTexture& that) noexcept {
  if (!that.isValid()) {
    this->cleanup();
    fIsValid = false;
    return *this;
...
#ifdef SK_GL
    case GrBackendApi::kOpenGL: fGLInfo.assign(that.fGLInfo, this->isValid()); break;
#endif
#ifdef SK_VULKAN
    case GrBackendApi::kVulkan: fVkInfo.assign(that.fVkInfo, this->isValid()); break;
#endif
#ifdef SK_METAL
    case GrBackendApi::kMetal: fMtlInfo = that.fMtlInfo; break;
#endif
#ifdef SK_DIRECT3D
    case GrBackendApi::kDirect3D: fD3DInfo.assign(that.fD3DInfo, this->isValid()); break;
#endif
#ifdef SK_DAWN
    case GrBackendApi::kDawn: fDawnInfo = that.fDawnInfo; break;
#endif
    case GrBackendApi::kMock: fMockInfo = that.fMockInfo; break;
    default: SK_ABORT("Unknown GrBackend");
  }
...
sk_sp<GrBackendSurfaceMutableStateImpl> GrBackendTexture::getMutableState() const noexcept {
...
bool GrBackendTexture::getGLTextureInfo(GrGLTextureInfo* outInfo) const {
  ...
  if (this->isValid() && GrBackendApi::kOpenGL == fBackend) {
    ...
    *outInfo =
        GrGLTextureInfo{GR_GL_TEXTURE_2D, static_cast<GrGLuint>(fMockInfo.id()), GR_GL_RGBA8};
    return true;
  }
#endif
  return false;
...
bool GrBackendTexture::isSameTexture(const GrBackendTexture& that) {
  ...
  switch (fBackend) {
#ifdef SK_GL
    case GrBackendApi::kOpenGL: return fGLInfo.info().fID == that.fGLInfo.info().fID;
#endif
#ifdef SK_VULKAN
    case GrBackendApi::kVulkan:
...
#endif
#ifdef SK_METAL
    case GrBackendApi::kMetal: return this->fMtlInfo.fTexture == that.fMtlInfo.fTexture;
#endif
#ifdef SK_DIRECT3D
    case GrBackendApi::kDirect3D:
...
#endif
    case GrBackendApi::kMock: return fMockInfo.id() == that.fMockInfo.id();
...
#endif
    case GrBackendApi::kMock: return fMockInfo.getBackendFormat();
...
bool GrBackendTexture::TestingOnly_Equals(const GrBackendTexture& t0, const GrBackendTexture& t1) {
  ...
  switch (t0.fBackend) {
#  ifdef SK_GL
    case GrBackendApi::kOpenGL: return t0.fGLInfo.info() == t1.fGLInfo.info();
#  endif
    case GrBackendApi::kMock: return t0.fMockInfo == t1.fMockInfo;
#  ifdef SK_VULKAN
    case GrBackendApi::kVulkan: return t0.fVkInfo == t1.fVkInfo;
#  endif
#  ifdef SK_METAL
    case GrBackendApi::kMetal: return t0.fMtlInfo == t1.fMtlInfo;
#  endif
#  ifdef SK_DIRECT3D
    case GrBackendApi::kDirect3D: return t0.fD3DInfo == t1.fD3DInfo;
#  endif
#  ifdef SK_DAWN
    case GrBackendApi::kDawn: return t0.fDawnInfo == t1.fDawnInfo;
#  endif
    default: return false;
  }
}
...
GrBackendRenderTarget::GrBackendRenderTarget(
    int width, int height, int sampleCnt, int stencilBits, const GrDawnRenderTargetInfo& dawnInfo)
    : fIsValid(true),
...
#ifdef SK_METAL
GrBackendRenderTarget::GrBackendRenderTarget(
    int width, int height, int sampleCnt, const GrMtlTextureInfo& mtlInfo)
    : fIsValid(true),
...
GrBackendRenderTarget& GrBackendRenderTarget::operator=(
  ...
  switch (that.fBackend) {
#ifdef SK_GL
    case GrBackendApi::kOpenGL: fGLInfo = that.fGLInfo; break;
#endif
#ifdef SK_VULKAN
    case GrBackendApi::kVulkan: fVkInfo.assign(that.fVkInfo, this->isValid()); break;
#endif
...
GrBackendFormat GrBackendRenderTarget::getBackendFormat() const {
  ...
  switch (fBackend) {
#ifdef SK_GL
    case GrBackendApi::kOpenGL: return GrBackendFormat::MakeGL(fGLInfo.fFormat, GR_GL_TEXTURE_NONE);
#endif
#ifdef SK_VULKAN
    case GrBackendApi::kVulkan: {
      auto info = fVkInfo.snapImageInfo(fMutableState.get());
...
void GrBackendRenderTarget::setMutableState(const GrBackendSurfaceMutableState& state) noexcept {
  fMutableState->set(state);
}
...
#if GR_TEST_UTILS
bool GrBackendRenderTarget::TestingOnly_Equals(
    const GrBackendRenderTarget& r0, const GrBackendRenderTarget& r1) {
  if (!r0.isValid() || !r1.isValid()) {
    return false;  // two invalid backend rendertargets are not considered equal
  }

  if (r0.fWidth != r1.fWidth || r0.fHeight != r1.fHeight || r0.fSampleCnt != r1.fSampleCnt ||
      r0.fStencilBits != r1.fStencilBits || r0.fBackend != r1.fBackend) {
    return false;
  }

  switch (r0.fBackend) {
#  ifdef SK_GL
    case GrBackendApi::kOpenGL: return r0.fGLInfo == r1.fGLInfo;
#  endif
    case GrBackendApi::kMock: return r0.fMockInfo == r1.fMockInfo;
#  ifdef SK_VULKAN
    case GrBackendApi::kVulkan: return r0.fVkInfo == r1.fVkInfo;
#  endif
#  ifdef SK_METAL
    case GrBackendApi::kMetal: return r0.fMtlInfo == r1.fMtlInfo;
#  endif
#  ifdef SK_DIRECT3D
    case GrBackendApi::kDirect3D: return r0.fD3DInfo == r1.fD3DInfo;
#  endif
#  ifdef SK_DAWN
    case GrBackendApi::kDawn: return r0.fDawnInfo == r1.fDawnInfo;
#  endif
    default: return false;
  }
