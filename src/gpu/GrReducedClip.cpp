  ...
  while (InitialTriState::kUnknown == initialTriState) {
    ...
    switch (element->getOp()) {
      ...
      case kReplace_SkClipOp:
        ...
        break;
      default: SkDEBUGFAIL("Unexpected op."); break;
    }
    if (!skippable) {
      if (fMaskElements.isEmpty()) {
      ...
      if (isFlip) {
        SkASSERT(
            kXOR_SkClipOp == element->getOp() || kReverseDifference_SkClipOp == element->getOp());
        fMaskElements.addToHead(
            SkRect::Make(fScissor), SkMatrix::I(), kReverseDifference_SkClipOp, false);
      } else {
...
GrReducedClip::ClipResult GrReducedClip::clipInsideElement(const Element* element) {
  ...
  switch (element->getDeviceSpaceType()) {
    case Element::DeviceSpaceType::kEmpty: return ClipResult::kMadeEmpty;
...
GrReducedClip::ClipResult GrReducedClip::clipOutsideElement(const Element* element) {
  switch (element->getDeviceSpaceType()) {
    ...
    case Element::DeviceSpaceType::kRRect: {
      ...
      SkRect horzRect = SkRect::MakeLTRB(
          bounds.left(), bounds.top() + insetTL.y(), bounds.right(), bounds.bottom() - insetBR.y());
      this->addWindowRectangle(horzRect, element->isAA());

      if (fWindowRects.count() < fMaxWindowRectangles) {
        SkRect vertRect = SkRect::MakeLTRB(
            bounds.left() + insetTL.x(), bounds.top(), bounds.right() - insetBR.x(),
            bounds.bottom());
...
GrReducedClip::ClipResult GrReducedClip::addAnalyticFP(
  ...
  if (auto fp = GrRRectEffect::Make(
          GetClipEdgeType(invert, aa), deviceSpaceRRect, *fCaps->shaderCaps())) {
...
static void stencil_device_rect(
    GrRenderTargetContext* rtc, const GrHardClip& clip, const GrUserStencilSettings* ss, GrAA aa,
    const SkRect& rect) {
  GrPaint paint;
...
bool GrReducedClip::drawAlphaClipMask(GrRenderTargetContext* rtc) const {
  ...
  if (!fWindowRects.empty()) {
    clip.setWindowRectangles(
        fWindowRects.makeOffset(-fScissor.left(), -fScissor.top()),
    ...
    if (invert || SkRegion::kIntersect_Op == op || SkRegion::kReverseDifference_Op == op) {
      ...
      static constexpr GrUserStencilSettings kStencilInElement(
          GrUserStencilSettings::StaticInit<
              0xffff, GrUserStencilTest::kAlways, 0xffff, GrUserStencilOp::kReplace,
      ...
      static constexpr GrUserStencilSettings kDrawOutsideElement(
          GrUserStencilSettings::StaticInit<
              0x0000, GrUserStencilTest::kEqual, 0xffff, GrUserStencilOp::kZero,
...
bool GrReducedClip::drawStencilClipMask(
  ...
  if (!fWindowRects.empty()) {
    stencilClip.fixedClip().setWindowRectangles(fWindowRects, GrWindowRectsState::Mode::kExclusive);
  }
  ...
  for (ElementList::Iter iter(fMaskElements); iter.get(); iter.next()) {
    ...
    const Element* element = iter.get();
    bool doStencilMSAA = element->isAA() && GrFSAAType::kNone != renderTargetContext->fsaaType();
    ...
    if (!drawDirectToClip) {
      ...
      if (Element::DeviceSpaceType::kRect == element->getDeviceSpaceType()) {
        stencil_device_rect(
            renderTargetContext, stencilClip.fixedClip(), &kDrawToStencil, GrAA(doStencilMSAA),
    ...
    for (GrUserStencilSettings const* const* pass = stencilPasses; *pass; ++pass) {
      if (drawDirectToClip) {
        if (Element::DeviceSpaceType::kRect == element->getDeviceSpaceType()) {
          stencil_device_rect(
              renderTargetContext, stencilClip, *pass, GrAA(doStencilMSAA),
              element->getDeviceSpaceRect());
        } else {
          GrShape shape(clipPath, GrStyle::SimpleFill());
          GrPaint paint;
          paint.setXPFactory(GrDisableColorXPFactory::Get());
          GrPathRenderer::DrawPathArgs args{context,         std::move(paint),
                                            *pass,           renderTargetContext,
                                            &stencilClip,    &stencilClip.fixedClip().scissorRect(),
                                            &SkMatrix::I(),  &shape,
                                            pathAATypeFlags, false};
          pr->drawPath(args);
        }
      } else {
        ...
        stencil_device_rect(
            renderTargetContext, stencilClip, *pass, GrAA(doStencilMSAA), SkRect::Make(fScissor));
      }
    }
  }
  return true;
...
std::unique_ptr<GrFragmentProcessor> GrReducedClip::finishAndDetachAnalyticFPs(
  ...
  SkDEBUGCODE(for (const auto& fp
    ...
    for (const SkPath& ccprClipPath : fCCPRClipPaths) {
      ...
      SkASSERT(fHasScissor);
      auto fp =
          ccpr->makeClipProcessor(opListID, ccprClipPath, fScissor, rtWidth, rtHeight, *fCaps);
      fAnalyticFPs.push_back(std::move(fp));
    }
