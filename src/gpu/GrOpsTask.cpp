...
std::unique_ptr<GrOp> GrOpsTask::OpChain::appendOp(
    std::unique_ptr<GrOp> op, GrProcessorSet::Analysis processorAnalysis, const DstProxy* dstProxy,
...
GrOpsTask::GrOpsTask(
    sk_sp<GrOpMemoryPool> opMemoryPool, sk_sp<GrRenderTargetProxy> rtProxy,
    GrAuditTrail* auditTrail)
  ...
  for (const auto& chain : fOpChains) {
    if (chain.shouldExecute()) {
      ...
      GrOpFlushState::OpArgs opArgs(
          chain.head(), fTarget->asRenderTargetProxy(), chain.appliedClip(), chain.dstProxy());
...
static GrOpsRenderPass* create_render_pass(
  ...
  const GrOpsRenderPass::LoadAndStoreInfo kColorLoadStoreInfo{colorLoadOp, GrStoreOp::kStore,
                                                              loadClearColor};
...
bool GrOpsTask::onExecute(GrOpFlushState* flushState) {
  ...
  GrOpsRenderPass* renderPass = create_render_pass(
      flushState->gpu(), fTarget->peekRenderTarget(), fTarget->origin(), fClippedContentBounds,
...
void GrOpsTask::visitProxies_debugOnly(const VisitSurfaceProxyFunc& func) const {
  auto textureFunc = [func](GrTextureProxy* tex, GrMipMapped mipmapped) { func(tex, mipmapped); };
...
void GrOpsTask::gatherProxyIntervals(GrResourceAllocator* alloc) const {
  ...
  if (fOpChains.count()) {
    ...
    alloc->addInterval(
        fTarget.get(), cur, cur + fOpChains.count() - 1, GrResourceAllocator::ActualUse::kYes);
  } else {
  ...
  auto gather = [alloc SkDEBUGCODE(, this)](GrSurfaceProxy* p, GrMipMapped) {
    alloc->addInterval(
        ...
        GrResourceAllocator::ActualUse::kYes SkDEBUGCODE(, fTarget.get() == p));
  };
  for (const OpChain& recordedOp : fOpChains) {
  ...
  if (maxCandidates) {
    ...
    while (true) {
      ...
      op = candidate.appendOp(
          std::move(op), processorAnalysis, dstProxy, clip, caps, fOpMemoryPool.get(), fAuditTrail);
      if (!op) {
        return;
      }
