...
GrOpsTask::OpChain::List GrOpsTask::OpChain::DoConcat(
    List chainA, List chainB, const GrCaps& caps, GrOpMemoryPool* pool, GrAuditTrail* auditTrail) {
  ...
  SkDEBUGCODE(bool first = true;) do {
    switch (fList.tail()->combineIfPossible(list->head(), caps)) {
      ...
      case GrOp::CombineResult::kMayChain:
        fList = DoConcat(std::move(fList), skstd::exchange(*list, List()), caps, pool, auditTrail);
...
bool GrOpsTask::OpChain::prependChain(
    OpChain* that, const GrCaps& caps, GrOpMemoryPool* pool, GrAuditTrail* auditTrail) {
  if (!that->tryConcat(
          &fList, fProcessorAnalysis, fDstProxyView, fAppliedClip, fBounds, caps, pool,
          auditTrail)) {
    this->validate();
    // append failed
    return false;
...
GrOpsTask::GrOpsTask(
    sk_sp<GrOpMemoryPool> opMemoryPool, GrSurfaceProxyView view, GrAuditTrail* auditTrail)
...
void GrOpsTask::onPrePrepare(GrRecordingContext* context) {
  ...
  for (const auto& chain : fOpChains) {
    if (chain.shouldExecute()) {
      chain.head()->prePrepare(context, &fTargetView, chain.appliedClip(), chain.dstProxyView());
    }
  }
}
  ...
  for (const auto& chain : fOpChains) {
    if (chain.shouldExecute()) {
      ...
      GrOpFlushState::OpArgs opArgs(
          chain.head(), &fTargetView, chain.appliedClip(), chain.dstProxyView());
...
static GrOpsRenderPass* create_render_pass(
  ...
  return gpu->getOpsRenderPass(
      rt, origin, bounds, kColorLoadStoreInfo, stencilLoadAndStoreInfo, sampledProxies);
}
...
bool GrOpsTask::onExecute(GrOpFlushState* flushState) {
  ...
  GrOpsRenderPass* renderPass = create_render_pass(
      flushState->gpu(), proxy->peekRenderTarget(), fTargetView.origin(), fClippedContentBounds,
      fColorLoadOp, fLoadClearColor, stencilLoadOp, stencilStoreOp, fSampledProxies);
  if (!renderPass) {
    return false;
  }
...
void GrOpsTask::dump(bool printDependencies) const {
  ...
  switch (fColorLoadOp) {
    ...
    case GrLoadOp::kClear: SkDebugf("kClear (0x%x)\n", fLoadClearColor.toBytes_RGBA()); break;
  ...
  for (int i = 0; i < fOpChains.count(); ++i) {
    ...
    if (!fOpChains[i].head()) {
      ...
      SkDebugf(
          "ClippedBounds: [L: %.2f, T: %.2f, R: %.2f, B: %.2f]\n", bounds.fLeft, bounds.fTop,
...
void GrOpsTask::gatherProxyIntervals(GrResourceAllocator* alloc) const {
  ...
  auto gather = [alloc SkDEBUGCODE(, this)](GrSurfaceProxy* p, GrMipMapped) {
    alloc->addInterval(
        ...
        GrResourceAllocator::ActualUse::kYes SkDEBUGCODE(, fTargetView.proxy() == p));
  };
  for (const OpChain& recordedOp : fOpChains) {
  ...
  if (maxCandidates) {
    ...
    while (true) {
      ...
      op = candidate.appendOp(
          std::move(op), processorAnalysis, dstProxyView, clip, caps, fOpMemoryPool.get(),
          fAuditTrail);
      if (!op) {
        return;
      }
