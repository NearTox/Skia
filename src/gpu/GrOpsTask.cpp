...
inline GrOpsTask::OpChain::List::List(GrOp::Owner op) : fHead(std::move(op)), fTail(fHead.get()) {
  this->validate();
}
  ...
  do {
    ...
    while (a) {
      ...
      if (canForwardMerge || canBackwardMerge) {
        auto result = a->combineIfPossible(chainB.head(), arenas->recordTimeAllocator(), caps);
...
bool GrOpsTask::OpChain::prependChain(
    OpChain* that, const GrCaps& caps, GrRecordingContext::Arenas* arenas,
    GrAuditTrail* auditTrail) {
  if (!that->tryConcat(
          &fList, fProcessorAnalysis, fDstProxyView, fAppliedClip, fBounds, caps, arenas,
          auditTrail)) {
    this->validate();
    // append failed
    return false;
...
GrOp::Owner GrOpsTask::OpChain::appendOp(
    GrOp::Owner op, GrProcessorSet::Analysis processorAnalysis, const DstProxyView* dstProxyView,
    const GrAppliedClip* appliedClip, const GrCaps& caps, GrRecordingContext::Arenas* arenas,
    GrAuditTrail* auditTrail) {
  ...
  if (!this->tryConcat(
          &chain, processorAnalysis, *dstProxyView, appliedClip, opBounds, caps, arenas,
          auditTrail)) {
...
void GrOpsTask::addDrawOp(
    GrDrawingManager* drawingMgr, GrOp::Owner op, const GrProcessorSet::Analysis& processorAnalysis,
  ...
  if (dstProxyView.proxy()) {
    ...
    SkASSERT(
        dstProxyView.dstSampleType() != GrDstSampleType::kAsInputAttachment ||
  ...
  for (const auto& chain : fOpChains) {
    if (chain.shouldExecute()) {
      chain.head()->prePrepare(
          context, this->target(0), chain.appliedClip(), chain.dstProxyView(),
  ...
  for (const auto& chain : fOpChains) {
    if (chain.shouldExecute()) {
      ...
      GrOpFlushState::OpArgs opArgs(
          chain.head(), this->target(0), chain.appliedClip(), chain.dstProxyView(),
          fRenderPassXferBarriers, fColorLoadOp);
...
static GrOpsRenderPass* create_render_pass(
    GrGpu* gpu, GrRenderTarget* rt, GrAttachment* stencil, GrSurfaceOrigin origin,
    const SkIRect& bounds, GrLoadOp colorLoadOp, const std::array<float, 4>& loadClearColor,
...
bool GrOpsTask::onExecute(GrOpFlushState* flushState) {
  ...
  if (int numStencilSamples = proxy->numStencilSamples()) {
    if (!flushState->resourceProvider()->attachStencilAttachment(renderTarget, numStencilSamples)) {
  ...
  if (maxCandidates) {
    ...
    while (true) {
      ...
      op = candidate.appendOp(
          std::move(op), processorAnalysis, dstProxyView, clip, caps, &fArenas, fAuditTrail);
      if (!op) {
        return;
      }
