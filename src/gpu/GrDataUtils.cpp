...
static bool fill_buffer_with_color(
    GrColorType colorType, int width, int height, const SkColor4f& colorf, void* dest) {
  ...
  switch (colorType) {
    ...
    case GrColorType::kABGR_4444: {
      ...
      uint16_t rgba4444 =
          r4 << SK_R4444_SHIFT | g4 << SK_G4444_SHIFT | b4 << SK_B4444_SHIFT | a4 << SK_A4444_SHIFT;
...
size_t GrComputeTightCombinedBufferSize(
    size_t bytesPerPixel, int baseWidth, int baseHeight, SkTArray<size_t>* individualMipOffsets,
    int mipLevelCount) {
...
void GrFillInData(
    GrColorType colorType, int baseWidth, int baseHeight,
    const SkTArray<size_t>& individualMipOffsets, char* dstPixels, const SkColor4f& colorf) {
  ...
  for (int currentMipLevel = 0; currentMipLevel < mipLevels; ++currentMipLevel) {
    ...
    fill_buffer_with_color(colorType, currentWidth, currentHeight, colorf, &(dstPixels[offset]));
    currentWidth = SkTMax(1, currentWidth / 2);
    currentHeight = SkTMax(1, currentHeight / 2);
  }
}
...
void GrFillInCompressedData(
    SkImage::CompressionType type, int baseWidth, int baseHeight, char* dstPixels,
...
static GrSwizzle get_load_and_get_swizzle(
...
static GrSwizzle get_dst_swizzle_and_store(
    GrColorType ct, SkRasterPipeline::StockStage* store, bool* isNormalized, bool* isSRGB) {
  GrSwizzle swizzle("rgba");
  *isNormalized = true;
  *isSRGB = false;
  switch (ct) {
...
bool GrConvertPixels(
  ...
  if (alphaOrCSConversion) {
    ...
    clampGamut = dstIsNormalized && !srcIsNormalized && dstInfo.alphaType() == kPremul_SkAlphaType;
    if (!clampGamut) {
