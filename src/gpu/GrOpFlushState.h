...
class GrOpFlushState final : public GrDeferredUploadTarget, public GrMeshDrawOp::Target {
 public:
  ...
  struct OpArgs {
    explicit OpArgs(
        GrOp* op, GrRenderTargetProxy* proxy, GrAppliedClip* appliedClip,
        const GrXferProcessor::DstProxy& dstProxy)
        : fOp(op), fProxy(proxy), fAppliedClip(appliedClip), fDstProxy(dstProxy) {}
  ...
  SkTArray<GrTextureProxy*, true>* sampledProxyArray() override { return fSampledProxies; }
