...
struct BufferWriter {
 public:
  ...
  struct Mark {
   public:
    Mark() noexcept : Mark(nullptr) {}
    Mark(void* ptr, size_t offset = 0) noexcept : fMark(reinterpret_cast<uintptr_t>(ptr) + offset) {
      SkASSERT(ptr || offset == 0);
    }

    bool operator<(const Mark& o) const noexcept { return fMark < o.fMark; }
    bool operator<=(const Mark& o) const noexcept { return fMark <= o.fMark; }
    bool operator==(const Mark& o) const noexcept { return fMark == o.fMark; }
    bool operator!=(const Mark& o) const noexcept { return fMark != o.fMark; }
    bool operator>=(const Mark& o) const noexcept { return fMark >= o.fMark; }
    ...
    ptrdiff_t operator-(const Mark& o) const noexcept { return fMark - o.fMark; }

    explicit operator bool() const noexcept { return *this != Mark(); }
  ...
  Mark mark(size_t offset = 0) const noexcept {
    this->validate(offset);
    return Mark(fPtr, offset);
  }

 protected:
  constexpr BufferWriter() noexcept = default;
  BufferWriter(void* ptr, size_t size) noexcept : fPtr(ptr) {
    SkDEBUGCODE(fEnd = Mark(ptr, ptr ? size : 0);)
  }
  BufferWriter(void* ptr, Mark end = {}) noexcept : fPtr(ptr) { SkDEBUGCODE(fEnd = end;) }

  BufferWriter& operator=(const BufferWriter&) = delete;
  BufferWriter& operator=(BufferWriter&& that) noexcept {
    fPtr = that.fPtr;
    that.fPtr = nullptr;
  ...
  void validate(size_t bytesToWrite) const noexcept {
...
struct VertexWriter : public BufferWriter {
  ...
  explicit VertexWriter(void* ptr) noexcept : BufferWriter(ptr, Mark()) {}

  VertexWriter(void* ptr, size_t size) noexcept : BufferWriter(ptr, size) {}
  VertexWriter(void* ptr, Mark end) noexcept : BufferWriter(ptr, end) {}

  VertexWriter(const VertexWriter&) = delete;
  VertexWriter(VertexWriter&& that) noexcept { *this = std::move(that); }

  VertexWriter& operator=(const VertexWriter&) = delete;
  VertexWriter& operator=(VertexWriter&& that) noexcept {
 ...
 private:
  ...
  std::enable_if_t<!is_quad<T>::value, void> writeQuadVertex(
      const T& val, const Args&... remainder) {
  ...
  std::enable_if_t<is_quad<Q>::value, void> writeQuadVertex(
      const Q& quad, const Args&... remainder) {
...
template <int kCount, typename T>
inline VertexWriter& operator<<(
    VertexWriter& w, const VertexWriter::RepeatDesc<kCount, T>& repeat) {
...
class VertexColor {
 public:
  ...
  explicit VertexColor(const SkPMColor4f& color, bool wideColor) { this->set(color, wideColor); }
...
struct IndexWriter : public BufferWriter {
  ...
  IndexWriter(void* ptr, size_t size) noexcept : BufferWriter(ptr, size) {}
  IndexWriter(void* ptr, Mark end) noexcept : BufferWriter(ptr, end) {}

  IndexWriter(const IndexWriter&) = delete;
  IndexWriter(IndexWriter&& that) noexcept { *this = std::move(that); }

  IndexWriter& operator=(const IndexWriter&) = delete;
  IndexWriter& operator=(IndexWriter&& that) noexcept {
    BufferWriter::operator=(std::move(that));
    return *this;
  }
  ...
  void writeArray(const uint16_t* array, int count) noexcept {
...
template <typename T>
inline IndexWriter& operator<<(IndexWriter&& w, const T& val) {
  return w << val;
}
...
struct UniformWriter : public BufferWriter {
  ...
  UniformWriter(void* ptr, size_t size) noexcept : BufferWriter(ptr, size) {}
  UniformWriter(void* ptr, Mark end) noexcept : BufferWriter(ptr, end) {}

  UniformWriter(const UniformWriter&) = delete;
  UniformWriter(UniformWriter&& that) noexcept { *this = std::move(that); }

  UniformWriter& operator=(const UniformWriter&) = delete;
  UniformWriter& operator=(UniformWriter&& that) noexcept {
    BufferWriter::operator=(std::move(that));
    return *this;
  }
...
struct UploadWriter : public BufferWriter {
  ...
  UploadWriter(void* ptr, size_t size) noexcept : BufferWriter(ptr, size) {}

  UploadWriter(const UploadWriter&) = delete;
  UploadWriter(UploadWriter&& that) noexcept { *this = std::move(that); }

  UploadWriter& operator=(const UploadWriter&) = delete;
  UploadWriter& operator=(UploadWriter&& that) noexcept {
    BufferWriter::operator=(std::move(that));
    return *this;
  }
