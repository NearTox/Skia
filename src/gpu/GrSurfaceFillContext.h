...
class GrSurfaceFillContext : public GrSurfaceContext {
 public:
  ...
  static std::tuple<GrColorType, GrBackendFormat> GetFallbackColorTypeAndFormat(
      GrImageContext*, GrColorType, int sampleCount);

  GrSurfaceFillContext(
      GrRecordingContext*, GrSurfaceProxyView readView, GrSurfaceProxyView writeView,
      const GrColorInfo&, bool flushTimeOpsTask = false);

  /**
   * Uses GrImageInfo's color type to pick the default texture format. Will return a
   * GrSurfaceDrawContext if possible.
   */
  static std::unique_ptr<GrSurfaceFillContext> Make(
      GrRecordingContext*, GrImageInfo, SkBackingFit = SkBackingFit::kExact, int sampleCount = 1,
   ...
   */
  static std::unique_ptr<GrSurfaceFillContext> Make(
      GrRecordingContext*, SkAlphaType, sk_sp<SkColorSpace>, SkISize dimensions, SkBackingFit,
      const GrBackendFormat&, int sampleCount, GrMipmapped, GrProtected, GrSwizzle readSwizzle,
      GrSwizzle writeSwizzle, GrSurfaceOrigin, SkBudgeted);
  ...
  static std::unique_ptr<GrSurfaceFillContext> MakeFromBackendTexture(
      GrRecordingContext*, GrColorInfo, const GrBackendTexture&, int sampleCount, GrSurfaceOrigin,
  ...
  void clearAtLeast(const SkIRect& scissor, const SkRGBA4f<AlphaType>& color) {
    this->internalClear(
        &scissor, this->adjustColorAlphaType(color),
   ...
   */
  void fillRectWithFP(
      const SkIRect& dstRect, const SkMatrix& localMatrix,
  ...
  void fillRectToRectWithFP(
      const SkRect& srcRect, const SkIRect& dstRect, std::unique_ptr<GrFragmentProcessor> fp) {
    SkMatrix lm;
  ...
  /** Fills 'dstRect' with 'fp' using a local matrix that maps 'srcRect' to 'dstRect' */
  void fillRectToRectWithFP(
      const SkIRect& srcRect, const SkIRect& dstRect, std::unique_ptr<GrFragmentProcessor> fp) {
 ...
 private:
  ...
  void internalClear(
      const SkIRect* scissor, std::array<float, 4> color, bool upgradePartialToFull = false);
...
std::array<float, 4> GrSurfaceFillContext::adjustColorAlphaType(SkRGBA4f<AlphaType> color) const {
  if (AlphaType == kUnknown_SkAlphaType || this->colorInfo().alphaType() == kUnknown_SkAlphaType) {
    return color.array();
  }
