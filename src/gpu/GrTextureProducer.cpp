...
sk_sp<GrTextureProxy> GrTextureProducer::CopyOnGpu(
    GrRecordingContext* context, sk_sp<GrTextureProxy> inputProxy, GrColorType colorType,
  ...
  if (copyParams.fFilter == GrSamplerState::Filter::kNearest && !resizing &&
    ...
    sk_sp<GrTextureProxy> proxy =
        GrCopyBaseMipMapToTextureProxy(context, inputProxy.get(), colorType);
    if (proxy) {
      return proxy;
    }
  }
  ...
  auto copyRTC = GrRenderTargetContext::MakeWithFallback(
      context, colorType, nullptr, SkBackingFit::kExact, copyParams.fDimensions, 1, mipMapped,
      inputProxy->isProtected(), inputProxy->origin());
  if (!copyRTC) {
    return nullptr;
  }
  ...
  auto fp = GrTextureEffect::MakeTexelSubset(
      std::move(inputProxy), kUnknown_SkAlphaType, SkMatrix::I(), sampler, boundsRect, localRect,
      caps);
  paint.addColorFragmentProcessor(std::move(fp));
  paint.setPorterDuffXPFactory(SkBlendMode::kSrc);

  copyRTC->fillRectToRect(
      GrNoClip(), std::move(paint), GrAA::kNo, SkMatrix::I(), dstRect, localRect);
  ...
  if (filterModeOrNullForBicubic) {
    switch (*filterModeOrNullForBicubic) {
      case GrSamplerState::Filter::kNearest:
        ...
        break;
      case GrSamplerState::Filter::kBilerp: filterHalfWidth = .5f; break;
...
std::unique_ptr<GrFragmentProcessor> GrTextureProducer::createFragmentProcessorForDomainAndFilter(
    sk_sp<GrTextureProxy> proxy, const SkMatrix& textureMatrix, DomainMode domainMode,
    const SkRect& domain, const GrSamplerState::Filter* filterOrNullForBicubic) {
  SkASSERT(kTightCopy_DomainMode != domainMode);
  const auto& caps = *fContext->priv().caps();
  SkAlphaType srcAlphaType = this->alphaType();
  if (filterOrNullForBicubic) {
    GrSamplerState::WrapMode wrapMode = fDomainNeedsDecal ? GrSamplerState::WrapMode::kClampToBorder
    ...
    if (kNoDomain_DomainMode == domainMode) {
      return GrTextureEffect::Make(
          std::move(proxy), srcAlphaType, textureMatrix, samplerState, caps);
    }
    return GrTextureEffect::MakeSubset(
        std::move(proxy), srcAlphaType, textureMatrix, samplerState, domain, caps);
  } else {
    ...
    if (kDomain_DomainMode == domainMode || (fDomainNeedsDecal && !clampToBorderSupport)) {
      GrTextureDomain::Mode wrapMode =
          fDomainNeedsDecal ? GrTextureDomain::kDecal_Mode : GrTextureDomain::kClamp_Mode;
      return GrBicubicEffect::Make(
          std::move(proxy), textureMatrix, kClampClamp, wrapMode, wrapMode, kDir, srcAlphaType,
          kDomain_DomainMode == domainMode ? &domain : nullptr);
    } else {
      return GrBicubicEffect::Make(
          std::move(proxy), textureMatrix, fDomainNeedsDecal ? kDecalDecal : kClampClamp, kDir,
          srcAlphaType);
    }
  }
}
...
sk_sp<GrTextureProxy> GrTextureProducer::refTextureProxyForParams(
    GrSamplerState sampler, SkScalar scaleAdjust[2]) {
  ...
  bool willBeMipped =
      GrSamplerState::Filter::kMipMap == sampler.filter() && mipCount && caps->mipMapSupport();
  ...
  SkDEBUGCODE(
      bool expectNoScale =
          (sampler.filter() != GrSamplerState::Filter::kMipMap && !sampler.isRepeated()));
...
std::pair<sk_sp<GrTextureProxy>, GrColorType> GrTextureProducer::refTextureProxy(
  ...
  GrSamplerState::Filter filter = GrMipMapped::kNo == willNeedMips
  ...
  SkASSERT(
      !result || result->isFormatCompressed(caps) ||
