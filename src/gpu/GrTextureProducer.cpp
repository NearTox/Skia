...
GrTextureProducer::DomainMode GrTextureProducer::DetermineDomainMode(
    ...
    bool coordsLimitedToConstraintRect, GrSurfaceProxy* proxy,
...
std::unique_ptr<GrFragmentProcessor> GrTextureProducer::createFragmentProcessorForSubsetAndFilter(
    GrSurfaceProxyView view, const SkMatrix& textureMatrix, DomainMode domainMode,
  ...
  if (filterOrNullForBicubic) {
    ...
    if (kNoDomain_DomainMode == domainMode) {
      return GrTextureEffect::Make(
          std::move(view), srcAlphaType, textureMatrix, samplerState, caps);
    }
    return GrTextureEffect::MakeSubset(
        std::move(view), srcAlphaType, textureMatrix, samplerState, domain, caps);
  } else {
    static constexpr auto kDir = GrBicubicEffect::Direction::kXY;
    const auto& caps = *fContext->priv().caps();
    if (kDomain_DomainMode == domainMode) {
      return GrBicubicEffect::MakeSubset(
          std::move(view), srcAlphaType, textureMatrix, wrapX, wrapY, domain, kDir, caps);
    } else {
      return GrBicubicEffect::Make(
          std::move(view), srcAlphaType, textureMatrix, wrapX, wrapY, kDir, caps);
    }
  }
}
...
GrSurfaceProxyView GrTextureProducer::view(GrSamplerState::Filter filter) {
  auto mipMapped = filter == GrSamplerState::Filter::kMipMap ? GrMipMapped::kYes : GrMipMapped::kNo;
