...
std::unique_ptr<GrFragmentProcessor> GrTextureProducer::createFragmentProcessorForView(
    ...
    const SkRect* domain, GrSamplerState samplerState) {
  if (!view) {
    return nullptr;
  }
  SkRect tempSubset;
  ...
  if (subset) {
    if (domain) {
      return GrTextureEffect::MakeSubset(
          std::move(view), this->alphaType(), textureMatrix, samplerState, *subset, *domain, caps);
    } else {
      return GrTextureEffect::MakeSubset(
          std::move(view), this->alphaType(), textureMatrix, samplerState, *subset, caps);
    }
  } else {
    return GrTextureEffect::Make(
        std::move(view), this->alphaType(), textureMatrix, samplerState, caps);
  }
}
...
GrSurfaceProxyView GrTextureProducer::view(GrMipmapped mipMapped) {
  ...
  // is not copyable.
  SkASSERT(
      !result || mipMapped == GrMipmapped::kNo ||
