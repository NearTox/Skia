...
class GrVkAttachment : public GrAttachment, public GrVkImage {
 public:
  ...
  static sk_sp<GrVkAttachment> MakeWrapped(
 ...
 private:
  static sk_sp<GrVkAttachment> Make(
      GrVkGpu* gpu, SkISize dimensions, UsageFlags attachmentUsages, int sampleCnt, VkFormat format,
      uint32_t mipLevels, VkImageUsageFlags vkUsageFlags, GrProtected isProtected, SkBudgeted);
  ...
  GrVkAttachment(
      ...
      sk_sp<GrBackendSurfaceMutableStateImpl> mutableState,
      sk_sp<const GrVkImageView> framebufferView, sk_sp<const GrVkImageView> textureView,
      GrBackendObjectOwnership, GrWrapCacheable, bool forSecondaryCB);
