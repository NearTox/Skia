...
class Copy : public GrVkPrimaryCommandBufferTask {
 public:
  Copy(GrSurface* src, const SkIRect& srcRect, const SkIPoint& dstPoint, bool shouldDiscardDst)
      : fSrc(src), fSrcRect(srcRect), fDstPoint(dstPoint), fShouldDiscardDst(shouldDiscardDst) {}
...
class TransferFrom : public GrVkPrimaryCommandBufferTask {
 public:
  ...
  void execute(const Args& args) override {
    args.fGpu->transferPixelsFrom(
        args.fSurface, fSrcRect.fLeft, fSrcRect.fTop, fSrcRect.width(), fSrcRect.height(),
...
void GrVkGpuTextureCommandBuffer::copy(
    GrSurface* src, const SkIRect& srcRect, const SkIPoint& dstPoint) {
  SkASSERT(!src->isProtected() || (fTexture->isProtected() && fGpu->protectedContext()));
  fTasks.emplace<Copy>(src, srcRect, dstPoint, false);
}

void GrVkGpuTextureCommandBuffer::transferFrom(
    const SkIRect& srcRect, GrColorType bufferColorType, GrGpuBuffer* transferBuffer,
    size_t offset) {
...
void get_vk_load_store_ops(
    GrLoadOp loadOpIn, GrStoreOp storeOpIn, VkAttachmentLoadOp* loadOp,
    VkAttachmentStoreOp* storeOp) {
  switch (loadOpIn) {
    case GrLoadOp::kLoad: *loadOp = VK_ATTACHMENT_LOAD_OP_LOAD; break;
    case GrLoadOp::kClear: *loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR; break;
    case GrLoadOp::kDiscard: *loadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE; break;
    default: SK_ABORT("Invalid LoadOp"); *loadOp = VK_ATTACHMENT_LOAD_OP_LOAD;
  }

  switch (storeOpIn) {
    case GrStoreOp::kStore: *storeOp = VK_ATTACHMENT_STORE_OP_STORE; break;
    case GrStoreOp::kDiscard: *storeOp = VK_ATTACHMENT_STORE_OP_DONT_CARE; break;
    default: SK_ABORT("Invalid StoreOp"); *storeOp = VK_ATTACHMENT_STORE_OP_STORE;
  }
}
...
void GrVkGpuRTCommandBuffer::init() {
  ...
  if (rpHandle.isValid()) {
    ...
    cbInfo.fRenderPass = fGpu->resourceProvider().findRenderPass(*vkRT, vkColorOps, vkStencilOps);
  }
  ...
  if (VK_ATTACHMENT_LOAD_OP_CLEAR == fVkColorLoadOp) {
    cbInfo.fLoadStoreState = LoadStoreState::kStartsWithClear;
      ...
      VK_ATTACHMENT_LOAD_OP_LOAD == fVkColorLoadOp &&
...
GrVkGpuRTCommandBuffer::~GrVkGpuRTCommandBuffer() { this->reset(); }
...
void GrVkGpuRTCommandBuffer::submit() {
  ...
  for (int i = 0; i < fCommandBufferInfos.count(); ++i) {
    ...
    if (cbInfo.fIsEmpty && cbInfo.fLoadStoreState != LoadStoreState::kStartsWithClear &&
        cbInfo.fLoadStoreState != LoadStoreState::kStartsWithDiscard) {
    ...
    if (cbInfo.fBounds.intersect(
            0, 0, SkIntToScalar(fRenderTarget->width()), SkIntToScalar(fRenderTarget->height()))) {
      ...
      if (stencil) {
        ...
        // wait there.
        vkStencil->setImageLayout(
            fGpu, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
            VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT |
                VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT,
            VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT, false);
      }
      ...
      fGpu->submitSecondaryCommandBuffer(
          cbInfo.fCommandBuffers, cbInfo.fRenderPass, &cbInfo.fColorClearValue, vkRT, fOrigin,
          iBounds);
    }
  }
...
void GrVkGpuRTCommandBuffer::set(
  ...
  get_vk_load_store_ops(colorInfo.fLoadOp, colorInfo.fStoreOp, &fVkColorLoadOp, &fVkColorStoreOp);

  get_vk_load_store_ops(
      stencilInfo.fLoadOp, stencilInfo.fStoreOp, &fVkStencilLoadOp, &fVkStencilStoreOp);
...
void GrVkGpuRTCommandBuffer::onClearStencilClip(const GrFixedClip& clip, bool insideStencilMask) {
  ...
  if (!clip.scissorEnabled()) {
    ...
    vkRect.setLTRB(
        scissor.fLeft, fRenderTarget->height() - scissor.fBottom, scissor.fRight,
...
void GrVkGpuRTCommandBuffer::onClear(const GrFixedClip& clip, const SkPMColor4f& color) {
  ...
  if (cbInfo.fIsEmpty && !clip.scissorEnabled()) {
    ...
    GrVkRenderPass::LoadStoreOps vkColorOps(
...
void GrVkGpuRTCommandBuffer::addAdditionalRenderPass() {
  ...
  GrVkRenderPass::LoadStoreOps vkColorOps(VK_ATTACHMENT_LOAD_OP_LOAD, VK_ATTACHMENT_STORE_OP_STORE);
  GrVkRenderPass::LoadStoreOps vkStencilOps(
...
void GrVkGpuRTCommandBuffer::inlineUpload(
    GrOpFlushState* state, GrDeferredTextureUploadFn& upload) {
...
void GrVkGpuRTCommandBuffer::copy(
    GrSurface* src, const SkIRect& srcRect, const SkIPoint& dstPoint) {
  CommandBufferInfo& cbInfo = fCommandBufferInfos[fCurrentCmdInfo];
...
void GrVkGpuRTCommandBuffer::transferFrom(
    const SkIRect& srcRect, GrColorType bufferColorType, GrGpuBuffer* transferBuffer,
    size_t offset) {
  CommandBufferInfo& cbInfo = fCommandBufferInfos[fCurrentCmdInfo];
  if (!cbInfo.fIsEmpty) {
...
void GrVkGpuRTCommandBuffer::bindGeometry(
  ...
  if (vertexBuffer) {
    ...
    currCmdBuf->bindInputBuffer(
        fGpu, binding++, static_cast<const GrVkVertexBuffer*>(vertexBuffer));
  }
  ...
  if (instanceBuffer) {
    ...
    currCmdBuf->bindInputBuffer(
        fGpu, binding++, static_cast<const GrVkVertexBuffer*>(instanceBuffer));
  }
  if (indexBuffer) {
...
GrVkPipelineState* GrVkGpuRTCommandBuffer::prepareDrawState(
    ...
    const GrPipeline::DynamicStateArrays* dynamicStateArrays, GrPrimitiveType primitiveType) {
  CommandBufferInfo& cbInfo = fCommandBufferInfos[fCurrentCmdInfo];
  ...
  GrVkPipelineState* pipelineState = fGpu->resourceProvider().findOrCreateCompatiblePipelineState(
      fRenderTarget, fOrigin, pipeline, primProc, primProcProxies, primitiveType,
      compatibleRenderPass);
  if (!pipelineState) {
    return pipelineState;
  }

  if (!cbInfo.fIsEmpty && fLastPipelineState && fLastPipelineState != pipelineState &&
  ...
  pipelineState->setAndBindUniforms(
      fGpu, fRenderTarget, fOrigin, primProc, pipeline, cbInfo.currentCmdBuf());
  ...
  if (setTextures) {
    pipelineState->setAndBindTextures(
        fGpu, primProc, pipeline, primProcProxies, cbInfo.currentCmdBuf());
  }

  if (!pipeline.isScissorEnabled()) {
    GrVkPipeline::SetDynamicScissorRectState(
        fGpu, cbInfo.currentCmdBuf(), fRenderTarget, fOrigin,
    ...
    GrVkPipeline::SetDynamicScissorRectState(
        fGpu, cbInfo.currentCmdBuf(), fRenderTarget, fOrigin, fixedDynamicState->fScissorRect);
  }
  GrVkPipeline::SetDynamicViewportState(fGpu, cbInfo.currentCmdBuf(), fRenderTarget);
  GrVkPipeline::SetDynamicBlendConstantState(
      fGpu, cbInfo.currentCmdBuf(), pipeline.outputSwizzle(), pipeline.getXferProcessor());

  return pipelineState;
}

void GrVkGpuRTCommandBuffer::onDraw(
    const GrPrimitiveProcessor& primProc, const GrPipeline& pipeline,
    const GrPipeline::FixedDynamicState* fixedDynamicState,
  ...
  auto prepareSampledImage = [&](GrTexture* texture, GrSamplerState::Filter filter) {
    ...
    SkASSERT(!texture->isProtected() || (fRenderTarget->isProtected() && fGpu->protectedContext()));
  };
  ...
  GrVkPipelineState* pipelineState = this->prepareDrawState(
      primProc, pipeline, fixedDynamicState, dynamicStateArrays, primitiveType);
  if (!pipelineState) {
    return;
  }
  ...
  for (int i = 0; i < meshCount; ++i) {
    ...
    if (dynamicTextures) {
      GrTextureProxy* const* meshProxies =
          dynamicStateArrays->fPrimitiveProcessorTextures + primProc.numTextureSamplers() * i;
      pipelineState->setAndBindTextures(
          fGpu, primProc, pipeline, meshProxies, cbInfo.currentCmdBuf());
    }
...
void GrVkGpuRTCommandBuffer::sendInstancedMeshToGpu(
...
void GrVkGpuRTCommandBuffer::sendIndexedInstancedMeshToGpu(
    ...
    int baseInstance, GrPrimitiveRestart restart) {
  ...
  cbInfo.currentCmdBuf()->drawIndexed(
      fGpu, indexCount, instanceCount, baseIndex, baseVertex, baseInstance);
  fGpu->stats()->incNumDraws();
}
...
void GrVkGpuRTCommandBuffer::executeDrawable(std::unique_ptr<SkDrawable::GpuDrawHandler> drawable) {
  ...
  if (bounds.extent.width == 0 || bounds.extent.height == 0) {
    ...
    cbInfo.fBounds.join(SkRect::MakeXYWH(
