...
GrVkTextureRenderTarget::GrVkTextureRenderTarget(
    GrVkGpu* gpu, SkBudgeted budgeted, SkISize dimensions, sk_sp<GrVkAttachment> texture,
    sk_sp<GrVkAttachment> colorAttachment, sk_sp<GrVkAttachment> resolveAttachment,
...
GrVkTextureRenderTarget::GrVkTextureRenderTarget(
    GrVkGpu* gpu, SkISize dimensions, sk_sp<GrVkAttachment> texture,
    sk_sp<GrVkAttachment> colorAttachment, sk_sp<GrVkAttachment> resolveAttachment,
...
bool create_rt_attachments(
    ...
    sk_sp<GrVkAttachment> texture, sk_sp<GrVkAttachment>* colorAttachment,
    sk_sp<GrVkAttachment>* resolveAttachment) {
  if (sampleCnt > 1) {
    auto rp = gpu->getContext()->priv().resourceProvider();
    sk_sp<GrAttachment> msaaAttachment =
        rp->makeMSAAAttachment(dimensions, GrBackendFormat::MakeVk(format), sampleCnt, isProtected);
    if (!msaaAttachment) {
      return false;
    }
    *colorAttachment =
...
sk_sp<GrVkTextureRenderTarget> GrVkTextureRenderTarget::MakeNewTextureRenderTarget(
    ...
    int sampleCnt, GrMipmapStatus mipmapStatus, GrProtected isProtected) {
