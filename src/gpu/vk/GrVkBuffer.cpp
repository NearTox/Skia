...
GrVkBuffer::GrVkBuffer(
    GrVkGpu* gpu, size_t sizeInBytes, GrGpuBufferType bufferType, GrAccessPattern accessPattern,
    VkBuffer buffer, const GrVkAlloc& alloc, const GrVkDescriptorSet* uniformDescriptorSet)
...
static const GrVkDescriptorSet* make_uniform_desc_set(GrVkGpu* gpu, VkBuffer buffer, size_t size) {
  ...
  GR_VK_CALL(
      gpu->vkInterface(), UpdateDescriptorSets(gpu->device(), 1, &descriptorWrite, 0, nullptr));
  return descriptorSet;
}

sk_sp<GrVkBuffer> GrVkBuffer::Make(
    GrVkGpu* gpu, size_t size, GrGpuBufferType bufferType, GrAccessPattern accessPattern) {
  VkBuffer buffer;
  GrVkAlloc alloc;
  ...
  bool requiresMappable = gpu->protectedContext() || accessPattern == kDynamic_GrAccessPattern ||
  ...
  return sk_sp<GrVkBuffer>(
      new GrVkBuffer(gpu, size, bufferType, accessPattern, buffer, alloc, uniformDescSet));
}
...
static VkAccessFlags buffer_type_to_access_flags(GrGpuBufferType type) {
  switch (type) {
    case GrGpuBufferType::kIndex: return VK_ACCESS_INDEX_READ_BIT;
    case GrGpuBufferType::kVertex: return VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT;
    default:
...
void GrVkBuffer::copyCpuDataToGpuBuffer(const void* src, size_t size) {
  ...
  if ((size <= 65536) && (0 == (size & 0x3)) && !gpu->vkCaps().avoidUpdateBuffers()) {
    ...
    gpu->copyBuffer(
        std::move(transferBuffer), sk_ref_sp(this), /*srcOffset=*/0,
        /*dstOffset=*/0, size);
  }

  this->addMemoryBarrier(
      VK_ACCESS_TRANSFER_WRITE_BIT, buffer_type_to_access_flags(this->intendedType()),
      VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_VERTEX_INPUT_BIT,
      /*byRegion=*/false);
}

void GrVkBuffer::addMemoryBarrier(
    VkAccessFlags srcAccessMask, VkAccessFlags dstAccesMask, VkPipelineStageFlags srcStageMask,
  ...
  this->getVkGpu()->addBufferMemoryBarrier(
      srcStageMask, dstStageMask, byRegion, &bufferMemoryBarrier);
}
