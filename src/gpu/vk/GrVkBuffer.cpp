...
const GrVkBuffer::Resource* GrVkBuffer::Create(GrVkGpu* gpu, const Desc& desc) {
  ...
  switch (desc.fType) {
    case kVertex_Type: bufInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT; break;
    case kIndex_Type: bufInfo.usage = VK_BUFFER_USAGE_INDEX_BUFFER_BIT; break;
    case kIndirect_Type: bufInfo.usage = VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT; break;
    case kUniform_Type: bufInfo.usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT; break;
    case kCopyRead_Type: bufInfo.usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT; break;
    case kCopyWrite_Type: bufInfo.usage = VK_BUFFER_USAGE_TRANSFER_DST_BIT; break;
    case kTexel_Type: bufInfo.usage = VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT;
  }
  if (!desc.fDynamic) {
...
void GrVkBuffer::addMemoryBarrier(
    const GrVkGpu* gpu, VkAccessFlags srcAccessMask, VkAccessFlags dstAccesMask,
    VkPipelineStageFlags srcStageMask, VkPipelineStageFlags dstStageMask, bool byRegion) const {
  ...
  gpu->addBufferMemoryBarrier(
      this->resource(), srcStageMask, dstStageMask, byRegion, &bufferMemoryBarrier);
}
...
VkAccessFlags buffer_type_to_access_flags(GrVkBuffer::Type type) {
  switch (type) {
    ...
    case GrVkBuffer::kVertex_Type: return VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT;
    default:
  ...
  if (!fResource->unique()) {
    if (fDesc.fDynamic) {
      ...
      this->addMemoryBarrier(
          gpu, buffer_type_to_access_flags(fDesc.fType), VK_ACCESS_TRANSFER_WRITE_BIT,
          VK_PIPELINE_STAGE_VERTEX_INPUT_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, false);
    }
  }
...
bool GrVkBuffer::vkUpdateData(
    GrVkGpu* gpu, const void* src, size_t srcSizeInBytes, bool* createdNewBuffer) {
...
void GrVkBuffer::validate() const {
  SkASSERT(
      !fResource || kVertex_Type == fDesc.fType || kIndex_Type == fDesc.fType ||
