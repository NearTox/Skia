...
sk_sp<GrGpu> GrVkGpu::Make(
  ...
  sk_sp<GrVkGpu> vkGpu(new GrVkGpu(
      direct, options, backendContext, interface, instanceVersion, physDevVersion,
...
GrVkGpu::GrVkGpu(
    GrDirectContext* direct, const GrContextOptions& options,
...
GrOpsRenderPass* GrVkGpu::getOpsRenderPass(
  ...
  if (!fCachedOpsRenderPass->set(
          rt, stencil, origin, bounds, colorInfo, stencilInfo, sampledProxies, usesXferBarriers)) {
    return nullptr;
  }
...
bool GrVkGpu::uploadTexDataCompressed(
  ...
  SkDEBUGCODE(size_t combinedBufferSize =) fill_in_regions(
...
bool GrVkGpu::setBackendTextureState(
  ...
  return this->setBackendSurfaceState(
      info, std::move(currentState), backendTeture.dimensions(), newState.fVkState);
}
...
void GrVkGpu::xferBarrier(GrRenderTarget* rt, GrXferBarrierType barrierType) {
  ...
  barrier.srcAccessMask =
      VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT | VK_ACCESS_COLOR_ATTACHMENT_READ_NONCOHERENT_BIT_EXT;
  ...
  this->addImageMemoryBarrier(
      vkRT->resource(), VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
...
bool GrVkGpu::compile(const GrProgramDesc& desc, const GrProgramInfo& programInfo) {
  ...
  sk_sp<const GrVkRenderPass> renderPass(this->resourceProvider().findCompatibleRenderPass(
      &attachmentsDescriptor, attachmentFlags, willReadDst));
  if (!renderPass) {
    return false;
  }
