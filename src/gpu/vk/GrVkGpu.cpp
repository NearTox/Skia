...
sk_sp<GrGpu> GrVkGpu::Make(
    const GrVkBackendContext& backendContext, const GrContextOptions& options, GrContext* context) {
  ...
  return sk_sp<GrGpu>(
      new GrVkGpu(context, options, backendContext, interface, instanceVersion, physDevVersion));
}
...
GrVkGpu::GrVkGpu(
  ...
  if (!fMemoryAllocator) {
    ...
    fMemoryAllocator.reset(
        new GrVkAMDMemoryAllocator(backendContext.fPhysicalDevice, fDevice, fInterface));
  }

  fCompiler = new SkSL::Compiler();

  if (backendContext.fDeviceFeatures2) {
    fVkCaps.reset(new GrVkCaps(
        options, this->vkInterface(), backendContext.fPhysicalDevice,
        *backendContext.fDeviceFeatures2, instanceVersion, physicalDeviceVersion,
    ...
    fVkCaps.reset(new GrVkCaps(
        options, this->vkInterface(), backendContext.fPhysicalDevice, features2, instanceVersion,
    ...
    fVkCaps.reset(new GrVkCaps(
        options, this->vkInterface(), backendContext.fPhysicalDevice, features, instanceVersion,
        physicalDeviceVersion, extensions));
  }
...
void GrVkGpu::submitCommandBuffer(
    SyncQueue sync, GrGpuFinishedProc finishedProc, GrGpuFinishedContext finishedContext) {
  SkASSERT(fCurrentCmdBuffer);

  if (!fCurrentCmdBuffer->hasWork() && kForce_SyncQueue != sync && !fSemaphoresToSignal.count() &&
...
sk_sp<GrGpuBuffer> GrVkGpu::onCreateBuffer(
  ...
  switch (type) {
    case GrGpuBufferType::kVertex:
      SkASSERT(
          kDynamic_GrAccessPattern == accessPattern || kStatic_GrAccessPattern == accessPattern);
    ...
    case GrGpuBufferType::kXferCpuToGpu:
      SkASSERT(
          kDynamic_GrAccessPattern == accessPattern || kStream_GrAccessPattern == accessPattern);
    ...
    case GrGpuBufferType::kXferGpuToCpu:
      ...
      break;
    default: SK_ABORT("Unknown buffer type."); return nullptr;
  }
  if (data && buff) {
...
bool GrVkGpu::onWritePixels(
  ...
  if (linearTiling) {
    ...
    success = this->uploadTexDataLinear(
        vkTex, left, top, width, height, srcColorType, texels[0].fPixels, texels[0].fRowBytes);
  } else {
    SkASSERT(mipLevelCount <= vkTex->texturePriv().maxMipMapLevel() + 1);
    success = this->uploadTexDataOptimal(
        vkTex, left, top, width, height, srcColorType, texels, mipLevelCount);
  }
...
bool GrVkGpu::onTransferPixelsTo(
    ...
    GrGpuBuffer* transferBuffer, size_t bufferOffset, size_t rowBytes) {
  ...
  fCurrentCmdBuffer->copyBufferToImage(
...
bool GrVkGpu::onTransferPixelsFrom(
    GrSurface* surface, int left, int top, int width, int height, GrColorType bufferColorType,
  ...
  if (GrVkRenderTarget* rt = static_cast<GrVkRenderTarget*>(surface->asRenderTarget())) {
    ...
    switch (rt->getResolveType()) {
      case GrVkRenderTarget::kCantResolve_ResolveType: return false;
      case GrVkRenderTarget::kAutoResolves_ResolveType: break;
      case GrVkRenderTarget::kCanResolve_ResolveType: this->resolveRenderTargetNoFlush(rt); break;
  ...
  fCurrentCmdBuffer->copyImageToBuffer(
  ...
  vkBuffer->addMemoryBarrier(
      this, VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_HOST_READ_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
...
bool GrVkGpu::uploadTexDataLinear(
    GrVkTexture* tex, int left, int top, int width, int height, GrColorType dataColorType,
    const void* data, size_t rowBytes) {
  SkASSERT(data);
  SkASSERT(tex->isLinearTiled());

  // If we're uploading compressed data then we should be using uploadCompressedTexData
  SkASSERT(!GrPixelConfigIsCompressed(GrColorTypeToPixelConfig(dataColorType, GrSRGBEncoded::kNo)));
  ...
  SkASSERT(
      VK_IMAGE_LAYOUT_PREINITIALIZED == tex->currentLayout() ||
  ...
  SkRectMemcpy(mapPtr, static_cast<size_t>(layout.rowPitch), data, rowBytes, trimRowBytes, height);
...
bool GrVkGpu::uploadTexDataOptimal(
  ...
  if (dataColorType == GrColorType::kRGB_888x && tex->imageFormat() == VK_FORMAT_R8G8B8_UNORM) {
    ...
    if (!this->vkCaps().canCopyAsBlit(
            tex->config(), 1, false, dstHasYcbcr, copyTexture->config(), 1, false, false) &&
        !this->vkCaps().canCopyAsDraw(
            tex->config(), SkToBool(tex->asRenderTarget()), dstHasYcbcr, copyTexture->config(),
            true, false)) {
      return false;
    }
  ...
  // Copy the buffer to the image
  fCurrentCmdBuffer->copyBufferToImage(
      this, transferBuffer.get(), uploadTexture, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
  ...
  if (copyTexture.get()) {
    ...
    SkAssertResult(this->copySurface(
        tex, kOrigin, copyTexture.get(), kOrigin, SkIRect::MakeWH(width, height),
  ...
  for (int currentMipLevel = 1; currentMipLevel < mipLevelCount; currentMipLevel++) {
    ...
    if (texels[currentMipLevel].fPixels) {
      const size_t dataSize =
          GrVkFormatCompressedDataSize(tex->imageFormat(), currentWidth, currentHeight);
      individualMipOffsets.push_back(combinedBufferSize);
...
sk_sp<GrTexture> GrVkGpu::onCreateTexture(
  ...
  if (renderTarget) {
    tex = GrVkTextureRenderTarget::MakeNewTextureRenderTarget(
        this, budgeted, desc, imageDesc, mipMapsStatus);
  } else {
  ...
  if (mipLevelCount) {
    bool success;
    if (isCompressed) {
      success = this->uploadTexDataCompressed(
          tex.get(), 0, 0, desc.fWidth, desc.fHeight, colorType, texels, mipLevelCount);
    } else {
      success = this->uploadTexDataOptimal(
          tex.get(), 0, 0, desc.fWidth, desc.fHeight, colorType, texels, mipLevelCount);
    }
...
void GrVkGpu::copyBuffer(
    GrVkBuffer* srcBuffer, GrVkBuffer* dstBuffer, VkDeviceSize srcOffset, VkDeviceSize dstOffset,
    VkDeviceSize size) {
...
bool GrVkGpu::updateBuffer(
    GrVkBuffer* buffer, const void* src, VkDeviceSize offset, VkDeviceSize size) {
  // Update the buffer
...
static bool check_image_info(
    const GrVkCaps& caps, const GrVkImageInfo& info, GrPixelConfig config, bool isWrappedRT) {
...
sk_sp<GrRenderTarget> GrVkGpu::onWrapBackendRenderTarget(const GrBackendRenderTarget& backendRT) {
  ...
  sk_sp<GrVkRenderTarget> tgt =
      GrVkRenderTarget::MakeWrappedRenderTarget(this, desc, info, std::move(layout));
...
sk_sp<GrRenderTarget> GrVkGpu::onWrapVulkanSecondaryCBAsRenderTarget(
  ...
  GrPixelConfig config =
      this->caps()->getConfigFromBackendFormat(backendFormat, imageInfo.colorType());
...
GrStencilAttachment* GrVkGpu::createStencilAttachmentForRenderTarget(
  ...
  GrVkStencilAttachment* stencil(GrVkStencilAttachment::Create(this, width, height, samples, sFmt));
...
bool copy_testing_data(
    GrVkGpu* gpu, const void* srcData, const GrVkAlloc& alloc, size_t bufferOffset,
    size_t srcRowBytes, size_t dstRowBytes, size_t trimRowBytes, int h) {
...
static size_t compute_combined_buffer_size(
    VkFormat format, size_t bpp, int w, int h, SkTArray<size_t>* individualMipOffsets,
    uint32_t mipLevels) {
...
bool GrVkGpu::createTestingOnlyVkImage(
    GrPixelConfig config, int w, int h, bool texturable, bool renderable, GrMipMapped mipMapped,
    const void* srcData, size_t srcRowBytes, GrVkImageInfo* info) {
  ...
  GR_VK_CALL_ERRCHECK(
      this->vkInterface(), CreateImage(this->device(), &imageCreateInfo, nullptr, &image));
  ...
  size_t combinedBufferSize =
      compute_combined_buffer_size(vkFormat, bpp, w, h, &individualMipOffsets, mipLevels);
  ...
  for (uint32_t currentMipLevel = 0; currentMipLevel < mipLevels; currentMipLevel++) {
    ...
    if (GrVkFormatIsCompressed(vkFormat)) {
      ...
      result = copy_testing_data(
          this, srcData, bufferAlloc, bufferOffset, currentRowBytes, currentRowBytes,
          currentRowBytes, currentHeight);
    } else {
      size_t currentRowBytes = bpp * currentWidth;
      result = copy_testing_data(
          this, srcData, bufferAlloc, bufferOffset, srcRowBytes, currentRowBytes, trimRowBytes,
          currentHeight);
    }
    if (!result) {
      GrVkMemory::FreeImageMemory(this, false, alloc);
  ...
  VK_CALL(CmdPipelineBarrier(
      cmdBuffer, GrVkImage::LayoutToPipelineSrcStageFlags(initialLayout),
  ...
  VK_CALL(CmdCopyBufferToImage(
      cmdBuffer, buffer, image, initialLayout, regions.count(), regions.begin()));
...
static bool vk_format_to_pixel_config(VkFormat format, GrPixelConfig* config) {
  ...
  switch (format) {
    case VK_FORMAT_UNDEFINED: *config = kUnknown_GrPixelConfig; return false;
    case VK_FORMAT_R8G8B8A8_UNORM: *config = kRGBA_8888_GrPixelConfig; return true;
    case VK_FORMAT_R8G8B8_UNORM: *config = kRGB_888_GrPixelConfig; return true;
    case VK_FORMAT_R8G8_UNORM: *config = kRG_88_GrPixelConfig; return true;
    case VK_FORMAT_B8G8R8A8_UNORM: *config = kBGRA_8888_GrPixelConfig; return true;
    case VK_FORMAT_R8G8B8A8_SRGB: *config = kSRGBA_8888_GrPixelConfig; return true;
    case VK_FORMAT_B8G8R8A8_SRGB: *config = kSBGRA_8888_GrPixelConfig; return true;
    case VK_FORMAT_A2B10G10R10_UNORM_PACK32: *config = kRGBA_1010102_GrPixelConfig; return true;
    case VK_FORMAT_R5G6B5_UNORM_PACK16: *config = kRGB_565_GrPixelConfig; return true;
    case VK_FORMAT_B4G4R4A4_UNORM_PACK16:
      *config = kRGBA_4444_GrPixelConfig;  // we're swizzling in this case
      return true;
    case VK_FORMAT_R4G4B4A4_UNORM_PACK16: *config = kRGBA_4444_GrPixelConfig; return true;
    case VK_FORMAT_R8_UNORM: *config = kAlpha_8_GrPixelConfig; return true;
    case VK_FORMAT_R32G32B32A32_SFLOAT: *config = kRGBA_float_GrPixelConfig; return true;
    case VK_FORMAT_R32G32_SFLOAT: *config = kRG_float_GrPixelConfig; return true;
    case VK_FORMAT_R16G16B16A16_SFLOAT: *config = kRGBA_half_GrPixelConfig; return true;
    case VK_FORMAT_ETC2_R8G8B8_UNORM_BLOCK: *config = kRGB_ETC1_GrPixelConfig; return true;
    case VK_FORMAT_R16_SFLOAT: *config = kAlpha_half_GrPixelConfig; return true;
...
GrBackendTexture GrVkGpu::createBackendTexture(
    int w, int h, const GrBackendFormat& format, GrMipMapped mipMapped, GrRenderable renderable,
    const void* srcData, size_t rowBytes, const SkColor4f& color) {
  ...
  GrVkImageInfo info;
  if (!this->createTestingOnlyVkImage(
          config, w, h, true, GrRenderable::kYes == renderable, mipMapped, srcData, rowBytes,
          &info)) {
    return {};
  }
...
GrBackendRenderTarget GrVkGpu::createTestingOnlyBackendRenderTarget(int w, int h, GrColorType ct) {
  ...
  if (!this->createTestingOnlyVkImage(
          config, w, h, false, true, GrMipMapped::kNo, nullptr, 0, &info)) {
    return {};
  }
...
void GrVkGpu::testingOnly_flushGpuAndSync() { this->submitCommandBuffer(kForce_SyncQueue); }
#endif
...
void GrVkGpu::onFinishFlush(
    GrSurfaceProxy* proxies[], int n, SkSurface::BackendSurfaceAccess access,
...
void GrVkGpu::copySurfaceAsCopyImage(
    ...
    GrVkImage* dstImage, GrVkImage* srcImage, const SkIRect& srcRect, const SkIPoint& dstPoint) {
#ifdef SK_DEBUG
  ...
  SkASSERT(this->vkCaps().canCopyImage(
      dst->config(), dstSampleCnt, dstOrigin, dstHasYcbcr, src->config(), srcSampleCnt, srcOrigin,
      srcHasYcbcr));
  ...
  fCurrentCmdBuffer->copyImage(
      this, srcImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, dstImage,
...
void GrVkGpu::copySurfaceAsBlit(
  ...
  SkASSERT(this->vkCaps().canCopyAsBlit(
      dst->config(), dstSampleCnt, dstImage->isLinearTiled(), dstHasYcbcr, src->config(),
      srcSampleCnt, srcImage->isLinearTiled(), srcHasYcbcr));
  ...
  fCurrentCmdBuffer->blitImage(
      this, *srcImage, *dstImage, 1, &blitRegion,
...
void GrVkGpu::copySurfaceAsResolve(
    GrSurface* dst, GrSurfaceOrigin dstOrigin, GrSurface* src, GrSurfaceOrigin srcOrigin,
    const SkIRect& origSrcRect, const SkIPoint& origDstPoint) {
  ...
  if (kBottomLeft_GrSurfaceOrigin == srcOrigin) {
    SkASSERT(kBottomLeft_GrSurfaceOrigin == dstOrigin);
    srcRect = {origSrcRect.fLeft, src->height() - origSrcRect.fBottom, origSrcRect.fRight,
               src->height() - origSrcRect.fTop};
    dstPoint.fY = dst->height() - dstPoint.fY - srcRect.height();
  }
  this->resolveImage(dst, srcRT, srcRect, dstPoint);
  SkIRect dstRect =
      SkIRect::MakeXYWH(origDstPoint.fX, origDstPoint.fY, srcRect.width(), srcRect.height());
  this->didWriteToSurface(dst, dstOrigin, &dstRect);
}
...
bool GrVkGpu::onCopySurface(
  ...
  if (this->vkCaps().canCopyAsResolve(
          dstConfig, dstSampleCnt, dstOrigin, dstHasYcbcr, srcConfig, srcSampleCnt, srcOrigin,
          srcHasYcbcr)) {
    this->copySurfaceAsResolve(dst, dstOrigin, src, srcOrigin, srcRect, dstPoint);
    return true;
  }

  if (this->vkCaps().canCopyAsDraw(
          dstConfig, SkToBool(dst->asRenderTarget()), dstHasYcbcr, srcConfig,
          SkToBool(src->asTexture()), srcHasYcbcr)) {
    SkAssertResult(fCopyManager.copySurfaceAsDraw(
        this, dst, dstOrigin, src, srcOrigin, srcRect, dstPoint, canDiscardOutsideDstRect));
  ...
  if (this->vkCaps().canCopyImage(
          dstConfig, dstSampleCnt, dstOrigin, dstHasYcbcr, srcConfig, srcSampleCnt, srcOrigin,
          srcHasYcbcr)) {
    this->copySurfaceAsCopyImage(
        dst, dstOrigin, src, srcOrigin, dstImage, srcImage, srcRect, dstPoint);
    return true;
  }

  if (this->vkCaps().canCopyAsBlit(
          dstConfig, dstSampleCnt, dstImage->isLinearTiled(), dstHasYcbcr, srcConfig, srcSampleCnt,
          srcImage->isLinearTiled(), srcHasYcbcr)) {
    this->copySurfaceAsBlit(dst, dstOrigin, src, srcOrigin, dstImage, srcImage, srcRect, dstPoint);
    return true;
  }
...
bool GrVkGpu::onReadPixels(
  ...
  if (dstColorType == GrColorType::kRGB_888x && image->imageFormat() == VK_FORMAT_R8G8B8_UNORM) {
    ...
    if (!this->vkCaps().canCopyAsBlit(
            copySurface->config(), 1, false, false, surface->config(), srcSampleCount,
            image->isLinearTiled(), srcHasYcbcr) &&
        !this->vkCaps().canCopyAsDraw(
            copySurface->config(), false, false, surface->config(), SkToBool(surface->asTexture()),
            srcHasYcbcr)) {
      return false;
    }
    ...
    static const GrSurfaceOrigin kOrigin = kTopLeft_GrSurfaceOrigin;
    if (!this->copySurface(
            copySurface.get(), kOrigin, surface, kOrigin, srcRect, SkIPoint::Make(0, 0))) {
      return false;
    }
    top = 0;
    left = 0;
  ...
  auto transferBuffer = sk_sp<GrVkTransferBuffer>(static_cast<GrVkTransferBuffer*>(
      this->createBuffer(
  ...
  // make sure the copy to buffer has finished
  transferBuffer->addMemoryBarrier(
      this, VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_HOST_READ_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
      VK_PIPELINE_STAGE_HOST_BIT, false);
...
void GrVkGpu::submitSecondaryCommandBuffer(
    const SkTArray<GrVkSecondaryCommandBuffer*>& buffers, const GrVkRenderPass* renderPass,
    const VkClearValue* colorClear, GrVkRenderTarget* target, GrSurfaceOrigin origin,
  ...
  if ((0 != granularity.width && 1 != granularity.width) ||
    ...
    adjust_bounds_to_granularity(
        &adjustedBounds, *pBounds, granularity, target->width(), target->height());
...
sk_sp<GrSemaphore> GrVkGpu::wrapBackendSemaphore(
...
uint32_t GrVkGpu::getExtraSamplerKeyForProgram(
    const GrSamplerState& samplerState, const GrBackendFormat& format) {
  ...
  const GrVkSampler* sampler =
      this->resourceProvider().findOrCreateCompatibleSampler(samplerState, *ycbcrInfo);
