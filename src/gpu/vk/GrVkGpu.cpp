...
static size_t fill_in_regions(
    GrVkCaps* vkCaps, GrStagingBufferManager* stagingBufferManager,
    SkTArray<VkBufferImageCopy>* regions, SkTArray<size_t>* individualMipOffsets,
    GrStagingBufferManager::Slice* slice, SkImage::CompressionType compression, VkFormat vkFormat,
...
bool GrVkGpu::uploadTexDataCompressed(
  ...
  fill_in_regions(
      fVkCaps.get(), &fStagingBufferManager, &regions, &individualMipOffsets, &slice, compression,
...
sk_sp<GrRenderTarget> GrVkGpu::onWrapBackendTextureAsRenderTarget(
  ...
  return GrVkRenderTarget::MakeWrappedRenderTarget(
      this, tex.dimensions(), sampleCnt, imageInfo, std::move(mutableState));
}
...
GrStencilAttachment* GrVkGpu::createStencilAttachmentForRenderTarget(
  ...
  GrVkStencilAttachment* stencil(
      GrVkStencilAttachment::Create(this, dimensions, numStencilSamples, sFmt));
...
bool copy_src_data(
    GrVkGpu* gpu, char* mapPtr, VkFormat vkFormat, const SkTArray<size_t>& individualMipOffsets,
...
bool GrVkGpu::createVkImageForBackendSurface(
    VkFormat vkFormat, SkISize dimensions, GrTexturable texturable, GrRenderable renderable,
    GrMipmapped mipMapped, GrVkImageInfo* info, GrProtected isProtected) {
...
bool GrVkGpu::onUpdateBackendTexture(
  ...
  if (fastPath) {
    ...
    fill_in_regions(
        fVkCaps.get(), &fStagingBufferManager, &regions, &individualMipOffsets, &slice, compression,
        info.fFormat, backendTexture.dimensions(), backendTexture.fMipmapped);
    ...
    if (data->type() == BackendTextureData::Type::kPixmaps) {
      result = copy_src_data(
          this, (char*)slice.fOffsetMapPtr, info.fFormat, individualMipOffsets, data->pixmaps(),
          info.fLevelCount);
...
GrBackendRenderTarget GrVkGpu::createTestingOnlyBackendRenderTarget(int w, int h, GrColorType ct) {
  ...
  GrVkImageInfo info;
  if (!this->createVkImageForBackendSurface(
          vkFormat, {w, h}, GrTexturable::kNo, GrRenderable::kYes, GrMipmapped::kNo, &info,
          GrProtected::kNo)) {
    return {};
  }
...
void GrVkGpu::addFinishedProc(
  ...
  sk_sp<GrRefCntedCallback> finishedCallback(new GrRefCntedCallback(finishedProc, finishedContext));
  this->addFinishedCallback(std::move(finishedCallback));
}
