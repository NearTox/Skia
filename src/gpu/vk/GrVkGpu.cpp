...
sk_sp<GrGpu> GrVkGpu::Make(
    const GrVkBackendContext& backendContext, const GrContextOptions& options,
  ...
  PFN_vkEnumerateInstanceVersion localEnumerateInstanceVersion =
      reinterpret_cast<PFN_vkEnumerateInstanceVersion>(
          backendContext.fGetProc("vkEnumerateInstanceVersion", VK_NULL_HANDLE, VK_NULL_HANDLE));
  ...
  if (backendContext.fVkExtensions) {
    interface.reset(new GrVkInterface(
        backendContext.fGetProc, backendContext.fInstance, backendContext.fDevice, instanceVersion,
        physDevVersion, backendContext.fVkExtensions));
    ...
    if (backendContext.fExtensions & kKHR_swapchain_GrVkExtensionFlag) {
      ...
      extensions.init(
          backendContext.fGetProc, backendContext.fInstance, backendContext.fPhysicalDevice, 0,
          nullptr, 1, &swapChainExtName);
    }
    interface.reset(new GrVkInterface(
        backendContext.fGetProc, backendContext.fInstance, backendContext.fDevice, instanceVersion,
        physDevVersion, &extensions));
  ...
  if (backendContext.fDeviceFeatures2) {
    caps.reset(new GrVkCaps(
        options, interface.get(), backendContext.fPhysicalDevice, *backendContext.fDeviceFeatures2,
        instanceVersion, physDevVersion, *backendContext.fVkExtensions,
    ...
    caps.reset(new GrVkCaps(
        options, interface.get(), backendContext.fPhysicalDevice, features2, instanceVersion,
        physDevVersion, *backendContext.fVkExtensions, backendContext.fProtectedContext));
  } else {
    ...
    caps.reset(new GrVkCaps(
        options, interface.get(), backendContext.fPhysicalDevice, features, instanceVersion,
        physDevVersion, extensions, backendContext.fProtectedContext));
  }
  ...
  if (!memoryAllocator) {
    ...
    memoryAllocator = GrVkAMDMemoryAllocator::Make(
        ...
        physDevVersion, backendContext.fVkExtensions, interface, caps.get());
  }
  if (!memoryAllocator) {
  ...
  sk_sp<GrVkGpu> vkGpu(new GrVkGpu(
      direct, backendContext, std::move(caps), interface, instanceVersion, physDevVersion,
...
GrVkGpu::GrVkGpu(
    GrDirectContext* direct, const GrVkBackendContext& backendContext, sk_sp<GrVkCaps> caps,
    sk_sp<const GrVkInterface> interface, uint32_t instanceVersion, uint32_t physicalDeviceVersion,
...
GrOpsRenderPass* GrVkGpu::onGetOpsRenderPass(
    GrRenderTarget* rt, bool useMSAASurface, GrAttachment* stencil, GrSurfaceOrigin origin,
  ...
  SkASSERT(
      ...
      (this->vkCaps().supportsDiscardableMSAAForDMSAA() && vkRT->resolveAttachment() &&
  ...
  if (vkRT->wrapsSecondaryCommandBuffer()) {
    ...
    auto fb = vkRT->getFramebuffer(withResolve, SkToBool(stencil), selfDepFlags, loadFromResolve);
  ...
  if (!fCachedOpsRenderPass->set(
          rt, std::move(framebuffer), origin, bounds, localColorInfo, stencilInfo, resolveInfo,
          selfDepFlags, loadFromResolve, sampledProxies)) {
    return nullptr;
  }
...
///////////////////////////////////////////////////////////////////////////////
sk_sp<GrGpuBuffer> GrVkGpu::onCreateBuffer(
  ...
  switch (type) {
    ...
    case GrGpuBufferType::kDrawIndirect:
      SkASSERT(
          accessPattern == kDynamic_GrAccessPattern || accessPattern == kStatic_GrAccessPattern);
      break;
    case GrGpuBufferType::kXferCpuToGpu: SkASSERT(accessPattern == kDynamic_GrAccessPattern); break;
    case GrGpuBufferType::kXferGpuToCpu:
      SkASSERT(
          accessPattern == kDynamic_GrAccessPattern || accessPattern == kStream_GrAccessPattern);
      break;
    case GrGpuBufferType::kUniform: SkASSERT(accessPattern == kDynamic_GrAccessPattern); break;
  }
#endif
...
bool GrVkGpu::onWritePixels(
  ...
  if (linearTiling) {
    ...
    if (VK_IMAGE_LAYOUT_PREINITIALIZED != texAttachment->currentLayout()) {
      ...
      texAttachment->setImageLayout(
          this, VK_IMAGE_LAYOUT_GENERAL, VK_ACCESS_HOST_WRITE_BIT, VK_PIPELINE_STAGE_HOST_BIT,
          false);
    ...
    success = this->uploadTexDataLinear(
        texAttachment, rect, srcColorType, texels[0].fPixels, texels[0].fRowBytes);
  } else {
    SkASSERT(mipLevelCount <= (int)texAttachment->mipLevels());
    success = this->uploadTexDataOptimal(texAttachment, rect, srcColorType, texels, mipLevelCount);
    if (1 == mipLevelCount) {
  ...
  if (prepForTexSampling) {
    texAttachment->setImageLayout(
        this, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_ACCESS_SHADER_READ_BIT,
...
bool GrVkGpu::onTransferPixelsTo(
    GrTexture* texture, SkIRect rect, GrColorType surfaceColorType, GrColorType bufferColorType,
    sk_sp<GrGpuBuffer> transferBuffer, size_t bufferOffset, size_t rowBytes) {
  if (!this->currentCommandBuffer()) {
    return false;
  }
  ...
  vkTex->setImageLayout(
      this, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_TRANSFER_WRITE_BIT,
  ...
  this->currentCommandBuffer()->copyBufferToImage(
      this, vkBuffer->vkBuffer(), vkTex, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
...
bool GrVkGpu::onTransferPixelsFrom(
    GrSurface* surface, SkIRect rect, GrColorType surfaceColorType, GrColorType bufferColorType,
...
void GrVkGpu::resolveImage(
  ...
  dstImage->setImageLayout(
      this, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_TRANSFER_WRITE_BIT,
      VK_PIPELINE_STAGE_TRANSFER_BIT, false);

  src->colorAttachment()->setImageLayout(
      this, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_ACCESS_TRANSFER_READ_BIT,
      VK_PIPELINE_STAGE_TRANSFER_BIT, false);
  ...
  this->currentCommandBuffer()->resolveImage(
      this, *src->colorAttachment(), *dstImage, 1, &resolveInfo);
}
...
bool GrVkGpu::uploadTexDataLinear(
    GrVkAttachment* texAttachment, SkIRect rect, GrColorType dataColorType, const void* data,
    size_t rowBytes) {
  SkASSERT(data);
  ...
  SkASSERT(
      VK_IMAGE_LAYOUT_PREINITIALIZED == texAttachment->currentLayout() ||
  ...
  GR_VK_CALL(
      interface, GetImageSubresourceLayout(fDevice, texAttachment->image(), &subres, &layout));
  ...
  SkRectMemcpy(
      mapPtr, static_cast<size_t>(layout.rowPitch), data, rowBytes, trimRowBytes, rect.height());
...
static size_t fill_in_compressed_regions(
    GrStagingBufferManager* stagingBufferManager, SkTArray<VkBufferImageCopy>* regions,
    SkTArray<size_t>* individualMipOffsets, GrStagingBufferManager::Slice* slice,
    SkImage::CompressionType compression, VkFormat vkFormat, SkISize dimensions,
...
bool GrVkGpu::uploadTexDataOptimal(
    GrVkAttachment* texAttachment, SkIRect rect, GrColorType dataColorType,
  ...
  if (mipLevelCount > 1) {
    combinedBufferSize =
        GrComputeTightCombinedBufferSize(bpp, rect.size(), &individualMipOffsets, mipLevelCount);
  } else {
  ...
  this->currentCommandBuffer()->copyBufferToImage(
      this, vkBuffer->vkBuffer(), texAttachment, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
bool GrVkGpu::uploadTexDataCompressed(
    GrVkAttachment* uploadTexture, SkImage::CompressionType compression, VkFormat vkFormat,
    SkISize dimensions, GrMipmapped mipMapped, const void* data, size_t dataSize) {
  if (!this->currentCommandBuffer()) {
    return false;
  }
  SkASSERT(data);
  ...
  SkASSERT(
      dimensions.fWidth == uploadTexture->width() && dimensions.fHeight == uploadTexture->height());
  ...
  fill_in_compressed_regions(
      &fStagingBufferManager, &regions, &individualMipOffsets, &slice, compression, vkFormat,
  ...
  GrVkBuffer* vkBuffer = static_cast<GrVkBuffer*>(slice.fBuffer);
  this->currentCommandBuffer()->copyBufferToImage(
      this, vkBuffer->vkBuffer(), uploadTexture, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
      regions.count(), regions.begin());
...
sk_sp<GrTexture> GrVkGpu::onCreateTexture(
  ...
  if (renderable == GrRenderable::kYes) {
    tex = GrVkTextureRenderTarget::MakeNewTextureRenderTarget(
        this, budgeted, dimensions, pixelFormat, mipLevelCount, renderTargetSampleCnt, mipmapStatus,
        isProtected);
  } else {
    tex = GrVkTexture::MakeNewTexture(
        this, budgeted, dimensions, pixelFormat, mipLevelCount, isProtected, mipmapStatus);
  }
  ...
  if (levelClearMask) {
    ...
    texImage->setImageLayout(
        this, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_TRANSFER_WRITE_BIT,
        VK_PIPELINE_STAGE_TRANSFER_BIT, false);
    this->currentCommandBuffer()->clearColorImage(
        this, texImage, &kZeroClearColor, ranges.count(), ranges.begin());
  }
  return std::move(tex);
}
...
sk_sp<GrTexture> GrVkGpu::onCreateCompressedTexture(
  ...
  auto tex = GrVkTexture::MakeNewTexture(
      this, budgeted, dimensions, pixelFormat, numMipLevels, isProtected, mipmapStatus);
  if (!tex) {
    return nullptr;
  }

  SkImage::CompressionType compression = GrBackendFormatToCompressionType(format);
  if (!this->uploadTexDataCompressed(
          tex->textureAttachment(), compression, pixelFormat, dimensions, mipMapped, data,
          dataSize)) {
    return nullptr;
  }
...
void GrVkGpu::copyBuffer(
    sk_sp<GrGpuBuffer> srcBuffer, sk_sp<GrGpuBuffer> dstBuffer, VkDeviceSize srcOffset,
  ...
  this->currentCommandBuffer()->copyBuffer(
      this, std::move(srcBuffer), std::move(dstBuffer), 1, &copyRegion);
}

bool GrVkGpu::updateBuffer(
...
sk_sp<GrTexture> GrVkGpu::onWrapBackendTexture(
  ...
  if (!check_image_info(
          this->vkCaps(), imageInfo, kAdopt_GrWrapOwnership == ownership, this->queueIndex())) {
    return nullptr;
  }
  ...
  return GrVkTexture::MakeWrappedTexture(
      this, backendTex.dimensions(), ownership, cacheable, ioType, imageInfo,
...
bool GrVkGpu::loadMSAAFromResolve(
    GrVkCommandBuffer* commandBuffer, const GrVkRenderPass& renderPass, GrAttachment* dst,
    GrVkAttachment* src, const SkIRect& srcRect) {
...
bool GrVkGpu::onRegenerateMipMapLevels(GrTexture* tex) {
  ...
  while (mipLevel < levelCount) {
    ...
    this->addImageMemoryBarrier(
        vkTex->resource(), VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, false,
        &imageMemoryBarrier);
    ...
    this->currentCommandBuffer()->blitImage(
        this, vkTex->resource(), vkTex->image(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
        vkTex->resource(), vkTex->image(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &blitRegion,
        VK_FILTER_LINEAR);
    ++mipLevel;
  }
  if (levelCount > 1) {
...
sk_sp<GrAttachment> GrVkGpu::makeStencilAttachment(
...
sk_sp<GrAttachment> GrVkGpu::makeMSAAAttachment(
...
bool GrVkGpu::createVkImageForBackendSurface(
    VkFormat vkFormat, SkISize dimensions, int sampleCnt, GrTexturable texturable,
    GrRenderable renderable, GrMipmapped mipMapped, GrVkImageInfo* info, GrProtected isProtected) {
...
bool GrVkGpu::onClearBackendTexture(
...
GrBackendTexture GrVkGpu::onCreateBackendTexture(
    SkISize dimensions, const GrBackendFormat& format, GrRenderable renderable,
  ...
  if (!this->createVkImageForBackendSurface(
          vkFormat, dimensions, 1, GrTexturable::kYes, renderable, mipMapped, &info, isProtected)) {
    return {};
  }
...
GrBackendTexture GrVkGpu::onCreateCompressedBackendTexture(
  ...
  return this->onCreateBackendTexture(
      dimensions, format, GrRenderable::kNo, mipMapped, isProtected);
}

bool GrVkGpu::onUpdateCompressedBackendTexture(
    const GrBackendTexture& backendTexture, sk_sp<GrRefCntedCallback> finishedCallback,
  ...
  fill_in_compressed_regions(
      &fStagingBufferManager, &regions, &individualMipOffsets, &slice, compression, info.fFormat,
      backendTexture.dimensions(), backendTexture.fMipmapped);
  ...
  cmdBuffer->copyBufferToImage(
      this, static_cast<GrVkBuffer*>(slice.fBuffer)->vkBuffer(), attachment,
      attachment->currentLayout(), regions.count(), regions.begin());
...
void set_layout_and_queue_from_mutable_state(
    GrVkGpu* gpu, GrVkImage* image, const GrVkSharedImageInfo& newInfo) {
  ...
  image->setImageLayoutAndQueueIndex(
      gpu, newLayout, dstAccess, dstStage, false, newQueueFamilyIndex);
}

bool GrVkGpu::setBackendSurfaceState(
  ...
  sk_sp<GrVkAttachment> texture = GrVkAttachment::MakeWrapped(
      this, dimensions, info, std::move(currentState), GrVkAttachment::UsageFlags::kColorAttachment,
      kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, /*forSecondaryCB=*/false);
  SkASSERT(texture);
  if (!texture) {
    return false;
  }
  if (previousState) {
    previousState->setVulkanState(texture->currentLayout(), texture->currentQueueFamilyIndex());
  }
...
bool GrVkGpu::setBackendTextureState(
    const GrBackendTexture& backendTeture, const GrBackendSurfaceMutableState& newState,
    GrBackendSurfaceMutableState* previousState, sk_sp<GrRefCntedCallback> finishedCallback) {
  GrVkImageInfo info;
  ...
  return this->setBackendSurfaceState(
      info, std::move(currentState), backendTeture.dimensions(), newState.fVkState, previousState,
      std::move(finishedCallback));
}

bool GrVkGpu::setBackendRenderTargetState(
    const GrBackendRenderTarget& backendRenderTarget, const GrBackendSurfaceMutableState& newState,
    GrBackendSurfaceMutableState* previousState, sk_sp<GrRefCntedCallback> finishedCallback) {
  GrVkImageInfo info;
  ...
  return this->setBackendSurfaceState(
      info, std::move(currentState), backendRenderTarget.dimensions(), newState.fVkState,
...
void GrVkGpu::xferBarrier(GrRenderTarget* rt, GrXferBarrierType barrierType) {
  ...
  this->addImageMemoryBarrier(
      colorAttachment->resource(), VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, dstStage, true,
      &barrier);
}
...
bool GrVkGpu::compile(const GrProgramDesc& desc, const GrProgramInfo& programInfo) {
  ...
  GrVkRenderTarget::ReconstructAttachmentsDescriptor(
      this->vkCaps(), programInfo, &attachmentsDescriptor, &attachmentFlags);
...
GrBackendRenderTarget GrVkGpu::createTestingOnlyBackendRenderTarget(
  ...
  GrVkImageInfo info;
  if (!this->createVkImageForBackendSurface(
          vkFormat, dimensions, sampleCnt, GrTexturable::kNo, GrRenderable::kYes, GrMipmapped::kNo,
          &info, isProtected)) {
    return {};
  }
...
void GrVkGpu::addBufferMemoryBarrier(
  ...
  SkASSERT(resource);
  this->currentCommandBuffer()->pipelineBarrier(
      this, resource, srcStageMask, dstStageMask, byRegion,
      GrVkCommandBuffer::kBufferMemory_BarrierType, barrier);
}
void GrVkGpu::addBufferMemoryBarrier(
    VkPipelineStageFlags srcStageMask, VkPipelineStageFlags dstStageMask, bool byRegion,
  ...
  this->currentCommandBuffer()->pipelineBarrier(
      this,
      /*resource=*/nullptr, srcStageMask, dstStageMask, byRegion,
      GrVkCommandBuffer::kBufferMemory_BarrierType, barrier);
}
...
void GrVkGpu::addImageMemoryBarrier(
  ...
  this->currentCommandBuffer()->pipelineBarrier(
      this, resource, srcStageMask, dstStageMask, byRegion,
      GrVkCommandBuffer::kImageMemory_BarrierType, barrier);
}
...
void GrVkGpu::copySurfaceAsCopyImage(
    GrSurface* dst, GrSurface* src, GrVkAttachment* dstAttachment, GrVkAttachment* srcAttachment,
  ...
  SkASSERT(this->vkCaps().canCopyImage(
      dstFormat, dstSampleCnt, dstHasYcbcr, srcFormat, srcSampleCnt, srcHasYcbcr));
#endif
  ...
  dstAttachment->setImageLayout(
      this, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_TRANSFER_WRITE_BIT,
      VK_PIPELINE_STAGE_TRANSFER_BIT, false);

  srcAttachment->setImageLayout(
      this, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_ACCESS_TRANSFER_READ_BIT,
      VK_PIPELINE_STAGE_TRANSFER_BIT, false);
  ...
  this->currentCommandBuffer()->addGrSurface(sk_ref_sp<const GrSurface>(dst));
  this->currentCommandBuffer()->copyImage(
      this, srcAttachment, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, dstAttachment,
...
void GrVkGpu::copySurfaceAsBlit(
    GrSurface* dst, GrSurface* src, GrVkAttachment* dstAttachment, GrVkAttachment* srcAttachment,
    const SkIRect& srcRect, const SkIPoint& dstPoint) {
  if (!this->currentCommandBuffer()) {
    return;
  ...
  VkFormat srcFormat;
  SkAssertResult(dst->backendFormat().asVkFormat(&srcFormat));
  SkASSERT(this->vkCaps().canCopyAsBlit(
      dstFormat, dstSampleCnt, dstAttachment->isLinearTiled(), dstHasYcbcr, srcFormat, srcSampleCnt,
      srcAttachment->isLinearTiled(), srcHasYcbcr));
  ...
  this->currentCommandBuffer()->addGrSurface(sk_ref_sp<const GrSurface>(dst));
  this->currentCommandBuffer()->blitImage(
      this, *srcAttachment, *dstAttachment, 1, &blitRegion,
...
void GrVkGpu::copySurfaceAsResolve(
...
bool GrVkGpu::onCopySurface(
  ...
  if (this->vkCaps().canCopyAsResolve(
          dstFormat, dstSampleCnt, dstHasYcbcr, srcFormat, srcSampleCnt, srcHasYcbcr)) {
    this->copySurfaceAsResolve(dst, src, srcRect, dstPoint);
    return true;
  }

  if (this->vkCaps().canCopyImage(
          dstFormat, dstSampleCnt, dstHasYcbcr, srcFormat, srcSampleCnt, srcHasYcbcr)) {
    this->copySurfaceAsCopyImage(dst, src, dstAttachment, srcAttachment, srcRect, dstPoint);
    return true;
  }

  if (this->vkCaps().canCopyAsBlit(
          dstFormat, dstSampleCnt, dstAttachment->isLinearTiled(), dstHasYcbcr, srcFormat,
          srcSampleCnt, srcAttachment->isLinearTiled(), srcHasYcbcr)) {
...
bool GrVkGpu::onReadPixels(
  ...
  sk_sp<GrGpuBuffer> transferBuffer = resourceProvider->createBuffer(
      transBufferRowBytes * imageRows, GrGpuBufferType::kXferGpuToCpu, kDynamic_GrAccessPattern);
  ...
  this->currentCommandBuffer()->copyImageToBuffer(
      this, image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, transferBuffer, 1, &region);
...
bool GrVkGpu::beginRenderPass(
    const GrVkRenderPass* renderPass, sk_sp<const GrVkFramebuffer> framebuffer,
    const VkClearValue* colorClear, const GrSurface* target, const SkIRect& renderPassBounds,
    bool forSecondaryCB) {
  if (!this->currentCommandBuffer()) {
    return false;
  }
...
bool GrVkGpu::checkVkResult(VkResult result) {
  switch (result) {
    ...
    case VK_ERROR_DEVICE_LOST: fDeviceIsLost = true; return false;
    case VK_ERROR_OUT_OF_DEVICE_MEMORY:
    case VK_ERROR_OUT_OF_HOST_MEMORY: this->setOOMed(); return false;
...
std::unique_ptr<GrSemaphore> GrVkGpu::wrapBackendSemaphore(
    const GrBackendSemaphore& semaphore, GrSemaphoreWrapType wrapType, GrWrapOwnership ownership) {
