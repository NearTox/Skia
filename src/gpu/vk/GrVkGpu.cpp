...
GrVkGpu::GrVkGpu(
  ...
  if (backendContext.fDeviceFeatures2) {
    ...
    fVkCaps.reset(new GrVkCaps(
        ...
        physicalDeviceVersion, *backendContext.fVkExtensions, fProtectedContext));
  } else {
    ...
    fVkCaps.reset(new GrVkCaps(
        ...
        physicalDeviceVersion, extensions, fProtectedContext));
  }
...
bool GrVkGpu::onTransferPixelsTo(
  ...
  SkDEBUGCODE(SkIRect subRect = SkIRect::MakeXYWH(left, top, width, height);
              ...
              SkASSERT(bounds.contains(subRect));) int bpp =
...
bool GrVkGpu::uploadTexDataOptimal(
  ...
  if (dataColorType == GrColorType::kRGB_888x && tex->imageFormat() == VK_FORMAT_R8G8B8_UNORM) {
    ...
    if (!this->vkCaps().canCopyAsBlit(
            tex->config(), 1, false, dstHasYcbcr, kRGBA_8888_GrPixelConfig, 1, false, false)) {
      return false;
    }
  ...
  if (copyTexture.get()) {
    ...
    SkAssertResult(this->copySurface(
        tex, copyTexture.get(), SkIRect::MakeWH(width, height), SkIPoint::Make(left, top), false));
  }
  if (1 == mipLevelCount) {
...
bool GrVkGpu::uploadTexDataCompressed(
    GrVkTexture* tex, int left, int top, int width, int height,
  ...
  // Copy the buffer to the image
  fCurrentCmdBuffer->copyBufferToImage(
      this, transferBuffer.get(), uploadTexture, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
...
sk_sp<GrTexture> GrVkGpu::onCreateCompressedTexture(
    int width, int height, SkImage::CompressionType compressionType, SkBudgeted budgeted,
    const void* data) {
  ...
  desc.fHeight = height;
  auto tex =
      GrVkTexture::MakeNewTexture(this, budgeted, desc, imageDesc, GrMipMapsStatus::kNotAllocated);
  if (!tex) {
    return nullptr;
  }

  if (!this->uploadTexDataCompressed(
          tex.get(), 0, 0, desc.fWidth, desc.fHeight, compressionType, data)) {
    return nullptr;
  }
...
sk_sp<GrTexture> GrVkGpu::onWrapBackendTexture(
  ...
  GrPixelConfig config =
      this->caps()->getConfigFromBackendFormat(backendTex.getBackendFormat(), colorType);
...
sk_sp<GrTexture> GrVkGpu::onWrapRenderableBackendTexture(
  ...
  surfDesc.fConfig = config;
  sampleCnt =
      this->caps()->getRenderTargetSampleCount(sampleCnt, colorType, backendTex.getBackendFormat());
...
sk_sp<GrRenderTarget> GrVkGpu::onWrapBackendRenderTarget(
  ...
  GrPixelConfig config =
      this->caps()->getConfigFromBackendFormat(backendRT.getBackendFormat(), colorType);
  SkASSERT(kUnknown_GrPixelConfig != config);
...
bool copy_src_data(
    GrVkGpu* gpu, const GrVkAlloc& alloc, VkFormat vkFormat, int width, int height,
...
bool copy_compressed_src_data(
    GrVkGpu* gpu, const GrVkAlloc& alloc, SkImage::CompressionType compressionType, int width,
...
static void set_image_layout(
    const GrVkInterface* vkInterface, VkCommandBuffer cmdBuffer, GrVkImageInfo* info,
    VkImageLayout newLayout, uint32_t mipLevels, VkAccessFlags dstAccessMask,
    VkPipelineStageFlagBits dstStageMask) {
  VkAccessFlags srcAccessMask = GrVkImage::LayoutToSrcAccessMask(info->fImageLayout);
  VkPipelineStageFlags srcStageMask = GrVkImage::LayoutToPipelineSrcStageFlags(info->fImageLayout);
...
bool GrVkGpu::createVkImageForBackendSurface(
  ...
  set_image_layout(
      this->vkInterface(), cmdBuffer, info, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels,
  ...
  if (srcData) {
    ...
    if (isCompressed) {
      ...
      combinedBufferSize =
          GrComputeTightCombinedBufferSize(bytesPerPixel, w, h, &individualMipOffsets, mipLevels);
    }
    ...
    VK_CALL(CmdCopyBufferToImage(
        cmdBuffer, buffer, info->fImage, info->fImageLayout, regions.count(), regions.begin()));
  } else {
    SkASSERT(color);
    ...
    VK_CALL(CmdClearColorImage(
        cmdBuffer, info->fImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &vkColor, 1, &range));
  }
  ...
  if (!srcData && renderable) {
    ...
    set_image_layout(
        this->vkInterface(), cmdBuffer, info, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, mipLevels,
    ...
    set_image_layout(
        this->vkInterface(), cmdBuffer, info, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, mipLevels,
...
static bool vk_format_to_pixel_config(VkFormat format, GrPixelConfig* config) {
  ...
  switch (format) {
    ...
    case VK_FORMAT_R16_SFLOAT: *config = kAlpha_half_as_Red_GrPixelConfig; return true;
    case VK_FORMAT_R16_UNORM: *config = kR_16_GrPixelConfig; return true;
    case VK_FORMAT_R16G16_UNORM: *config = kRG_1616_GrPixelConfig; return true;
    // Experimental (for Y416 and mutant P016/P010)
    case VK_FORMAT_R16G16B16A16_UNORM: *config = kRGBA_16161616_GrPixelConfig; return true;
    case VK_FORMAT_R16G16_SFLOAT: *config = kRG_half_GrPixelConfig; return true;
...
GrBackendTexture GrVkGpu::createBackendTexture(
    ...
    const void* srcData, size_t rowBytes, const SkColor4f* color, GrProtected isProtected) {
  const GrVkCaps& caps = this->vkCaps();
  ...
  if (!this->createVkImageForBackendSurface(
          *vkFormat, w, h, true, GrRenderable::kYes == renderable, mipMapped, srcData, rowBytes,
...
GrBackendRenderTarget GrVkGpu::createTestingOnlyBackendRenderTarget(int w, int h, GrColorType ct) {
  ...
  GrVkImageInfo info;
  if (!this->createVkImageForBackendSurface(
          vkFormat, w, h, false, true, GrMipMapped::kNo, nullptr, 0, &SkColors::kTransparent, &info,
          GrProtected::kNo)) {
    return {};
  }
...
void GrVkGpu::copySurfaceAsCopyImage(
  ...
  SkASSERT(this->vkCaps().canCopyImage(
      dst->config(), dstSampleCnt, dstHasYcbcr, src->config(), srcSampleCnt, srcHasYcbcr));
#endif
...
bool GrVkGpu::onCopySurface(
  ...
  if (this->vkCaps().canCopyAsResolve(
          dstConfig, dstSampleCnt, dstHasYcbcr, srcConfig, srcSampleCnt, srcHasYcbcr)) {
...
bool GrVkGpu::onReadPixels(
  ...
  if (dstColorType == GrColorType::kRGB_888x && image->imageFormat() == VK_FORMAT_R8G8B8_UNORM) {
    ...
    if (!this->vkCaps().canCopyAsBlit(
            kRGBA_8888_GrPixelConfig, 1, false, false, surface->config(), srcSampleCount,
