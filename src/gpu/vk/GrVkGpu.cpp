...
GrOpsRenderPass* GrVkGpu::onGetOpsRenderPass(
  ...
  if (!fCachedOpsRenderPass->set(
          rt, stencil, origin, bounds, colorInfo, stencilInfo, sampledProxies,
...
sk_sp<GrGpuBuffer> GrVkGpu::onCreateBuffer(
  ...
  switch (type) {
    ...
    case GrGpuBufferType::kDrawIndirect:
      ...
      buff = GrVkMeshBuffer::Make(this, type, size, kDynamic_GrAccessPattern == accessPattern);
      break;
    ...
    case GrGpuBufferType::kXferGpuToCpu:
      ...
      break;
    default: SK_ABORT("Unknown buffer type.");
  }
  if (data && buff) {
...
bool GrVkGpu::onTransferPixelsTo(
    ...
    GrColorType bufferColorType, GrGpuBuffer* transferBuffer, size_t bufferOffset,
    size_t rowBytes) {
  if (!this->currentCommandBuffer()) {
    return false;
  }
...
static size_t fill_in_regions(
  ...
  if (compression == SkImage::CompressionType::kNone) {
    combinedBufferSize = GrComputeTightCombinedBufferSize(
        bytesPerBlock, dimensions, individualMipOffsets, numMipLevels);
  } else {
    combinedBufferSize = SkCompressedDataSize(
...
bool GrVkGpu::uploadTexDataOptimal(
  ...
  if (dataColorType == GrColorType::kRGB_888x && tex->imageFormat() == VK_FORMAT_R8G8B8_UNORM) {
    ...
    copyTexture = GrVkTexture::MakeNewTexture(
        this, SkBudgeted::kYes, {width, height}, imageDesc, GrMipmapStatus::kNotAllocated);
    if (!copyTexture) {
      return false;
    }
  ...
  // Copy the buffer to the image
  this->currentCommandBuffer()->copyBufferToImage(
      this, static_cast<GrVkTransferBuffer*>(slice.fBuffer), uploadTexture,
      VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, regions.count(), regions.begin());
  ...
  if (copyTexture) {
    ...
    SkAssertResult(this->copySurface(
        tex, copyTexture.get(), SkIRect::MakeWH(width, height), SkIPoint::Make(left, top)));
  }
  if (1 == mipLevelCount) {
...
bool GrVkGpu::uploadTexDataCompressed(
    GrVkTexture* uploadTexture, SkImage::CompressionType compression, VkFormat vkFormat,
...
sk_sp<GrTexture> GrVkGpu::onCreateCompressedTexture(
  ...
  if (!this->uploadTexDataCompressed(
          tex.get(), compression, pixelFormat, dimensions, mipMapped, data, dataSize)) {
    return nullptr;
  }
...
sk_sp<GrAttachment> GrVkGpu::makeStencilAttachmentForRenderTarget(
...
bool copy_compressed_data(GrVkGpu* gpu, char* mapPtr, const void* rawData, size_t dataSize) {
  SkASSERT(mapPtr);
  memcpy(mapPtr, rawData, dataSize);
  return true;
}

bool generate_compressed_data(
    GrVkGpu* gpu, char* mapPtr, SkImage::CompressionType compression, SkISize dimensions,
...
bool GrVkGpu::onUpdateBackendTexture(
  ...
  bool fastPath =
      data->type() == BackendTextureData::Type::kColor && !GrVkFormatIsCompressed(info.fFormat);
  ...
  if (fastPath) {
    ...
    if (data->type() == BackendTextureData::Type::kPixmaps) {
      result = copy_src_data(
          (char*)slice.fOffsetMapPtr, info.fFormat, individualMipOffsets, data->pixmaps(),
          info.fLevelCount);
    } else if (data->type() == BackendTextureData::Type::kCompressed) {
      result = copy_compressed_data(
          this, (char*)slice.fOffsetMapPtr, data->compressedData(), data->compressedSize());
    } else {
      SkASSERT(data->type() == BackendTextureData::Type::kColor);
      result = generate_compressed_data(
          this, (char*)slice.fOffsetMapPtr, compression, backendTexture.dimensions(),
          backendTexture.fMipmapped, data->color());
    }

    cmdBuffer->addGrSurface(texture);
    cmdBuffer->copyBufferToImage(
        this, static_cast<GrVkTransferBuffer*>(slice.fBuffer), texture.get(),
...
void GrVkGpu::querySampleLocations(
  ...
  switch (numSamples) {
    case 1: sampleLocations->push_back_n(1, kStandardSampleLocations_1); break;
    case 2: sampleLocations->push_back_n(2, kStandardSampleLocations_2); break;
    case 4: sampleLocations->push_back_n(4, kStandardSampleLocations_4); break;
    case 8: sampleLocations->push_back_n(8, kStandardSampleLocations_8); break;
    case 16: sampleLocations->push_back_n(16, kStandardSampleLocations_16); break;
    default: SK_ABORT("Invalid vulkan sample count."); break;
  }
}
...
void GrVkGpu::xferBarrier(GrRenderTarget* rt, GrXferBarrierType barrierType) {
  ...
  this->addImageMemoryBarrier(
      vkRT->resource(), VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, dstStage, true, &barrier);
}
...
bool GrVkGpu::onReadPixels(
  ...
  if (dstColorType == GrColorType::kRGB_888x && image->imageFormat() == VK_FORMAT_R8G8B8_UNORM) {
    ...
    if (!this->vkCaps().canCopyAsBlit(
            VK_FORMAT_R8G8B8A8_UNORM, 1, false, false, image->imageFormat(), srcSampleCount,
    ...
    copySurface = GrVkTextureRenderTarget::MakeNewTextureRenderTarget(
        this, SkBudgeted::kYes, {width, height}, 1, imageDesc, GrMipmapStatus::kNotAllocated);
    if (!copySurface) {
      return false;
    }
