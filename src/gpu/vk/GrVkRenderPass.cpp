...
void setup_vk_attachment_description(
    VkAttachmentDescription* attachment, const AttachmentDesc& desc, VkImageLayout startLayout,
  ...
  switch (startLayout) {
    ...
    case VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL:
      ...
      break;
    default: SK_ABORT("Unexpected attachment layout");
  }
  ...
  switch (loadFromResolve) {
    case LoadFromResolve::kNo:
      return Create(
          ...
          kBasicLoadStoreOps, selfDepFlags, loadFromResolve);
    case LoadFromResolve::kLoad: {
      static const GrVkRenderPass::LoadStoreOps kDiscardLoadStoreOps(
          VK_ATTACHMENT_LOAD_OP_DONT_CARE, VK_ATTACHMENT_STORE_OP_DONT_CARE);
      return Create(
          gpu, attachmentFlags, attachmentsDescriptor, kDiscardLoadStoreOps, kBasicLoadStoreOps,
          kBasicLoadStoreOps, selfDepFlags, loadFromResolve);
    }
  }
  SkUNREACHABLE;
...
GrVkRenderPass* GrVkRenderPass::Create(
    ...
    const LoadStoreOps& resolveOp, const LoadStoreOps& stencilOp) {
  ...
  return Create(
      gpu, attachmentFlags, &attachmentsDescriptor, colorOp, resolveOp, stencilOp, selfDepFlags,
      loadFromResolve);
}

GrVkRenderPass* GrVkRenderPass::Create(
    GrVkGpu* gpu, AttachmentFlags attachmentFlags, AttachmentsDescriptor* attachmentsDescriptor,
    const LoadStoreOps& colorOp, const LoadStoreOps& resolveOp, const LoadStoreOps& stencilOp,
    SelfDependencyFlags selfDepFlags, LoadFromResolve loadFromResolve) {
  SkASSERT(
      !SkToBool(selfDepFlags & SelfDependencyFlags::kForNonCoherentAdvBlend) ||
  ...
  SkASSERT(
      loadFromResolve == LoadFromResolve::kNo ||
  ...
  if (attachmentFlags & kColor_AttachmentFlag) {
    ...
    VkImageLayout layout =
        needsGeneralLayout ? VK_IMAGE_LAYOUT_GENERAL : VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;

    attachmentsDescriptor->fColor.fLoadStoreOps = colorOp;

    setup_vk_attachment_description(
        &attachments[currentAttachment], attachmentsDescriptor->fColor, layout, layout);
  ...
  if (attachmentFlags & kResolve_AttachmentFlag) {
    ...
    setup_vk_attachment_description(
        &attachments[currentAttachment], attachmentsDescriptor->fResolve, layout,
        VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
  ...
  GR_VK_CALL_RESULT(
      gpu, result, CreateRenderPass(gpu->device(), &createInfo, nullptr, &renderPass));
  ...
  return new GrVkRenderPass(
      gpu, renderPass, attachmentFlags, *attachmentsDescriptor, selfDepFlags, loadFromResolve,
      granularity, clearValueCount);
}

GrVkRenderPass::GrVkRenderPass(
    const GrVkGpu* gpu, VkRenderPass renderPass, AttachmentFlags flags,
    const AttachmentsDescriptor& descriptor, SelfDependencyFlags selfDepFlags,
    LoadFromResolve loadFromResolve, const VkExtent2D& granularity, uint32_t clearValueCount)
    : INHERITED(gpu),
...
bool GrVkRenderPass::colorAttachmentIndex(uint32_t* index) const {
  *index = fColorAttachmentIndex;
  if ((fAttachmentFlags & kColor_AttachmentFlag) || (fAttachmentFlags & kExternal_AttachmentFlag)) {
    return true;
  }
  return false;
}
...
bool GrVkRenderPass::isCompatible(
    const AttachmentsDescriptor& desc, const AttachmentFlags& flags,
    SelfDependencyFlags selfDepFlags, LoadFromResolve loadFromResolve) const {
...
bool GrVkRenderPass::isCompatible(const GrVkRenderPass& renderPass) const {
  ...
  return this->isCompatible(
      renderPass.fAttachmentsDescriptor, renderPass.fAttachmentFlags, renderPass.fSelfDepFlags,
      renderPass.fLoadFromResolve);
}
...
bool GrVkRenderPass::equalLoadStoreOps(
    const LoadStoreOps& colorOps, const LoadStoreOps& resolveOps,
...
void GrVkRenderPass::genKey(GrProcessorKeyBuilder* b) const {
  GenKey(
      b, fAttachmentFlags, fAttachmentsDescriptor, fSelfDepFlags, fLoadFromResolve,
      (uint64_t)fRenderPass);
}

void GrVkRenderPass::GenKey(
    GrProcessorKeyBuilder* b, AttachmentFlags attachmentFlags,
    const AttachmentsDescriptor& attachmentsDescriptor, SelfDependencyFlags selfDepFlags,
    LoadFromResolve loadFromResolve, uint64_t externalRenderPass) {
