...
GrVkRenderPass* GrVkRenderPass::Create(
    ...
    const LoadStoreOps& colorOp, const LoadStoreOps& stencilOp, SelfDependencyFlags selfDepFlags) {
  SkASSERT(
  ...
  return new GrVkRenderPass(
      gpu, renderPass, attachmentFlags, *attachmentsDescriptor, selfDepFlags, granularity,
      clearValueCount);
}
...
bool GrVkRenderPass::isCompatible(
    const GrVkRenderTarget& target, SelfDependencyFlags selfDepFlags) const {
  SkASSERT(!(fAttachmentFlags & kExternal_AttachmentFlag));
