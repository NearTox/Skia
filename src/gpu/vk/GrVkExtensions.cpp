...
static int find_info(const SkTArray<GrVkExtensions::Info>& infos, const char ext[]) {
  ...
  int idx = SkTSearch<GrVkExtensions::Info, SkString, GrVkExtensions::Info::Less>(
      &infos.front(), infos.count(), extensionStr, sizeof(GrVkExtensions::Info), less);
  return idx;
}
...
void GrVkExtensions::init(
    GrVkGetProc getProc, VkInstance instance, VkPhysicalDevice physDev,
...
void GrVkExtensions::getSpecVersions(
    GrVkGetProc getProc, VkInstance instance, VkPhysicalDevice physDevice) {
