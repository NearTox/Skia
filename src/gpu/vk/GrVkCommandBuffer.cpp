...
void GrVkCommandBuffer::freeGPUData(GrVkGpu* gpu) const {
  ...
  if (!this->isWrapped()) {
    GR_VK_CALL(
        gpu->vkInterface(),
        FreeCommandBuffers(gpu->device(), fCmdPool->vkCommandPool(), 1, &fCmdBuffer));
  }
...
void GrVkCommandBuffer::pipelineBarrier(
    const GrVkGpu* gpu, const GrVkResource* resource, VkPipelineStageFlags srcStageMask,
...
void GrVkCommandBuffer::submitPipelineBarriers(const GrVkGpu* gpu) {
  ...
  if (fBufferBarriers.count() || fImageBarriers.count()) {
    ...
    GR_VK_CALL(
        gpu->vkInterface(), CmdPipelineBarrier(
                                fCmdBuffer, fSrcStageMask, fDstStageMask, dependencyFlags, 0,
                                nullptr, fBufferBarriers.count(), fBufferBarriers.begin(),
...
void GrVkCommandBuffer::bindInputBuffer(
    GrVkGpu* gpu, uint32_t binding, const GrVkVertexBuffer* vbuffer) {
  ...
  if (vkBuffer != fBoundInputBuffers[binding]) {
    ...
    GR_VK_CALL(
        gpu->vkInterface(), CmdBindVertexBuffers(fCmdBuffer, binding, 1, &vkBuffer, &offset));
...
void GrVkCommandBuffer::bindIndexBuffer(GrVkGpu* gpu, const GrVkIndexBuffer* ibuffer) {
  ...
  if (vkBuffer != fBoundIndexBuffer) {
    GR_VK_CALL(
        gpu->vkInterface(),
        CmdBindIndexBuffer(fCmdBuffer, vkBuffer, ibuffer->offset(), VK_INDEX_TYPE_UINT16));
    fBoundIndexBuffer = vkBuffer;
    this->addResource(ibuffer->resource());
  }
}

void GrVkCommandBuffer::clearAttachments(
    const GrVkGpu* gpu, int numAttachments, const VkClearAttachment* attachments, int numRects,
...
void GrVkCommandBuffer::bindDescriptorSets(
    const GrVkGpu* gpu, GrVkPipelineState* pipelineState, GrVkPipelineLayout* layout,
  ...
  GR_VK_CALL(
      ...
      CmdBindDescriptorSets(
          fCmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, layout->layout(), firstSet, setCount,
          descriptorSets, dynamicOffsetCount, dynamicOffsets));
  this->addRecordingResource(layout);
}

void GrVkCommandBuffer::bindDescriptorSets(
    const GrVkGpu* gpu, const SkTArray<const GrVkRecycledResource*>& recycled,
    const SkTArray<const GrVkResource*>& resources, GrVkPipelineLayout* layout, uint32_t firstSet,
...
void GrVkCommandBuffer::drawIndexed(
    const GrVkGpu* gpu, uint32_t indexCount, uint32_t instanceCount, uint32_t firstIndex,
  ...
  GR_VK_CALL(
      gpu->vkInterface(),
      CmdDrawIndexed(
          fCmdBuffer, indexCount, instanceCount, firstIndex, vertexOffset, firstInstance));
}

void GrVkCommandBuffer::draw(
    const GrVkGpu* gpu, uint32_t vertexCount, uint32_t instanceCount, uint32_t firstVertex,
...
void GrVkCommandBuffer::setViewport(
    const GrVkGpu* gpu, uint32_t firstViewport, uint32_t viewportCount,
    const VkViewport* viewports) {
  SkASSERT(fIsActive);
  SkASSERT(1 == viewportCount);
  if (memcmp(viewports, &fCachedViewport, sizeof(VkViewport))) {
    GR_VK_CALL(
        gpu->vkInterface(), CmdSetViewport(fCmdBuffer, firstViewport, viewportCount, viewports));
    fCachedViewport = viewports[0];
  }
}

void GrVkCommandBuffer::setScissor(
    const GrVkGpu* gpu, uint32_t firstScissor, uint32_t scissorCount, const VkRect2D* scissors) {
  SkASSERT(fIsActive);
...
GrVkPrimaryCommandBuffer* GrVkPrimaryCommandBuffer::Create(
    const GrVkGpu* gpu, GrVkCommandPool* cmdPool) {
  ...
  VkResult err =
      GR_VK_CALL(gpu->vkInterface(), AllocateCommandBuffers(gpu->device(), &cmdInfo, &cmdBuffer));
  if (err) {
    return nullptr;
  }
...
void GrVkPrimaryCommandBuffer::beginRenderPass(
    const GrVkGpu* gpu, const GrVkRenderPass* renderPass, const VkClearValue clearValues[],
    const GrVkRenderTarget& target, const SkIRect& bounds, bool forSecondaryCB) {
  SkASSERT(fIsActive);
  SkASSERT(!fActiveRenderPass);
...
void GrVkPrimaryCommandBuffer::executeCommands(
    const GrVkGpu* gpu, GrVkSecondaryCommandBuffer* buffer) {
...
static void submit_to_queue(
    const GrVkInterface* interface, VkQueue queue, VkFence fence, uint32_t waitCount,
    const VkSemaphore* waitSemaphores, const VkPipelineStageFlags* waitStages,
    uint32_t commandBufferCount, const VkCommandBuffer* commandBuffers, uint32_t signalCount,
    const VkSemaphore* signalSemaphores, GrProtected protectedContext) {
...
void GrVkPrimaryCommandBuffer::submitToQueue(
    const GrVkGpu* gpu, VkQueue queue, GrVkGpu::SyncQueue sync,
  ...
  if (VK_NULL_HANDLE == fSubmitFence) {
    ...
    err = GR_VK_CALL(
        gpu->vkInterface(), CreateFence(gpu->device(), &fenceInfo, nullptr, &fSubmitFence));
    SkASSERT(!err);
  } else {
  ...
  if (0 == signalCount && 0 == waitCount) {
    ...
    submit_to_queue(
        gpu->vkInterface(), queue, fSubmitFence, 0, nullptr, nullptr, 1, &fCmdBuffer, 0, nullptr,
    ...
    submit_to_queue(
        gpu->vkInterface(), queue, fSubmitFence, vkWaitSems.count(), vkWaitSems.begin(),
        vkWaitStages.begin(), 1, &fCmdBuffer, vkSignalSems.count(), vkSignalSems.begin(),
        gpu->protectedContext() ? GrProtected::kYes : GrProtected::kNo);
    for (int i = 0; i < signalCount; ++i) {
  ...
  if (GrVkGpu::kForce_SyncQueue == sync) {
    err = GR_VK_CALL(
        gpu->vkInterface(), WaitForFences(gpu->device(), 1, &fSubmitFence, true, UINT64_MAX));
    if (VK_TIMEOUT == err) {
...
void GrVkPrimaryCommandBuffer::copyImage(
    const GrVkGpu* gpu, GrVkImage* srcImage, VkImageLayout srcLayout, GrVkImage* dstImage,
    VkImageLayout dstLayout, uint32_t copyRegionCount, const VkImageCopy* copyRegions) {
  SkASSERT(fIsActive);
  SkASSERT(!fActiveRenderPass);
  this->addingWork(gpu);
  ...
  GR_VK_CALL(
      gpu->vkInterface(), CmdCopyImage(
                              fCmdBuffer, srcImage->image(), srcLayout, dstImage->image(),
                              dstLayout, copyRegionCount, copyRegions));
}

void GrVkPrimaryCommandBuffer::blitImage(
    const GrVkGpu* gpu, const GrVkResource* srcResource, VkImage srcImage, VkImageLayout srcLayout,
  ...
  GR_VK_CALL(
      gpu->vkInterface(), CmdBlitImage(
                              fCmdBuffer, srcImage, srcLayout, dstImage, dstLayout, blitRegionCount,
                              blitRegions, filter));
}

void GrVkPrimaryCommandBuffer::blitImage(
    const GrVkGpu* gpu, const GrVkImage& srcImage, const GrVkImage& dstImage,
    uint32_t blitRegionCount, const VkImageBlit* blitRegions, VkFilter filter) {
  this->blitImage(
      gpu, srcImage.resource(), srcImage.image(), srcImage.currentLayout(), dstImage.resource(),
      dstImage.image(), dstImage.currentLayout(), blitRegionCount, blitRegions, filter);
}

void GrVkPrimaryCommandBuffer::copyImageToBuffer(
    const GrVkGpu* gpu, GrVkImage* srcImage, VkImageLayout srcLayout, GrVkTransferBuffer* dstBuffer,
    uint32_t copyRegionCount, const VkBufferImageCopy* copyRegions) {
  SkASSERT(fIsActive);
  SkASSERT(!fActiveRenderPass);
  this->addingWork(gpu);
  this->addResource(srcImage->resource());
  ...
  GR_VK_CALL(
      gpu->vkInterface(), CmdCopyImageToBuffer(
                              fCmdBuffer, srcImage->image(), srcLayout, dstBuffer->buffer(),
                              copyRegionCount, copyRegions));
}

void GrVkPrimaryCommandBuffer::copyBufferToImage(
    const GrVkGpu* gpu, GrVkTransferBuffer* srcBuffer, GrVkImage* dstImage, VkImageLayout dstLayout,
    uint32_t copyRegionCount, const VkBufferImageCopy* copyRegions) {
  SkASSERT(fIsActive);
  SkASSERT(!fActiveRenderPass);
  this->addingWork(gpu);
  ...
  this->addResource(dstImage->resource());
  GR_VK_CALL(
      gpu->vkInterface(), CmdCopyBufferToImage(
                              fCmdBuffer, srcBuffer->buffer(), dstImage->image(), dstLayout,
                              copyRegionCount, copyRegions));
}

void GrVkPrimaryCommandBuffer::copyBuffer(
    GrVkGpu* gpu, GrVkBuffer* srcBuffer, GrVkBuffer* dstBuffer, uint32_t regionCount,
  ...
  GR_VK_CALL(
      gpu->vkInterface(),
      CmdCopyBuffer(fCmdBuffer, srcBuffer->buffer(), dstBuffer->buffer(), regionCount, regions));
}

void GrVkPrimaryCommandBuffer::updateBuffer(
    GrVkGpu* gpu, GrVkBuffer* dstBuffer, VkDeviceSize dstOffset, VkDeviceSize dataSize,
    const void* data) {
  SkASSERT(fIsActive);
  SkASSERT(!fActiveRenderPass);
  ...
  GR_VK_CALL(
      gpu->vkInterface(),
      CmdUpdateBuffer(fCmdBuffer, dstBuffer->buffer(), dstOffset, dataSize, (const uint32_t*)data));
}

void GrVkPrimaryCommandBuffer::clearColorImage(
    const GrVkGpu* gpu, GrVkImage* image, const VkClearColorValue* color, uint32_t subRangeCount,
  ...
  GR_VK_CALL(
      gpu->vkInterface(),
      CmdClearColorImage(
          fCmdBuffer, image->image(), image->currentLayout(), color, subRangeCount, subRanges));
}

void GrVkPrimaryCommandBuffer::clearDepthStencilImage(
    const GrVkGpu* gpu, GrVkImage* image, const VkClearDepthStencilValue* color,
...
void GrVkPrimaryCommandBuffer::resolveImage(
    GrVkGpu* gpu, const GrVkImage& srcImage, const GrVkImage& dstImage, uint32_t regionCount,
  ...
  GR_VK_CALL(
      gpu->vkInterface(), CmdResolveImage(
                              fCmdBuffer, srcImage.image(), srcImage.currentLayout(),
...
void GrVkSecondaryCommandBuffer::begin(
    const GrVkGpu* gpu, const GrVkFramebuffer* framebuffer,
