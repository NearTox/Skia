...
std::unique_ptr<GrVkSemaphore> GrVkSemaphore::Make(GrVkGpu* gpu, bool isOwned) {
  ...
  VkResult result;
  GR_VK_CALL_RESULT(gpu, result, CreateSemaphore(gpu->device(), &createInfo, nullptr, &semaphore));
...
std::unique_ptr<GrVkSemaphore> GrVkSemaphore::MakeWrapped(
    GrVkGpu* gpu, VkSemaphore semaphore, GrSemaphoreWrapType wrapType, GrWrapOwnership ownership) {
  ...
  return std::unique_ptr<GrVkSemaphore>(new GrVkSemaphore(
      gpu, semaphore, prohibitSignal, prohibitWait, kBorrow_GrWrapOwnership != ownership));
}
...
void GrVkSemaphore::Resource::freeGPUData() const {
  if (fIsOwned) {
    GR_VK_CALL(fGpu->vkInterface(), DestroySemaphore(fGpu->device(), fSemaphore, nullptr));
  }
}
