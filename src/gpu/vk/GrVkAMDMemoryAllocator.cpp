...
GrVkAMDMemoryAllocator::GrVkAMDMemoryAllocator(
    VkPhysicalDevice physicalDevice, VkDevice device, sk_sp<const GrVkInterface> interface)
    : fAllocator(VK_NULL_HANDLE), fInterface(std::move(interface)), fDevice(device) {
...
bool GrVkAMDMemoryAllocator::allocateMemoryForImage(
    VkImage image, AllocationPropertyFlags flags, GrVkBackendMemory* backendMemory) {
...
bool GrVkAMDMemoryAllocator::allocateMemoryForBuffer(
    VkBuffer buffer, BufferUsage usage, AllocationPropertyFlags flags,
...
void GrVkAMDMemoryAllocator::getAllocInfo(
    const GrVkBackendMemory& memoryHandle, GrVkAlloc* alloc) const {
  const VmaAllocation allocation = (const VmaAllocation)memoryHandle;
...
void GrVkAMDMemoryAllocator::flushMappedMemory(
    const GrVkBackendMemory& memoryHandle, VkDeviceSize offset, VkDeviceSize size) {
  GrVkAlloc info;
  ...
  if (GrVkAlloc::kNoncoherent_Flag & info.fFlags) {
    ...
    GrVkMemory::GetNonCoherentMappedMemoryRange(info, offset, size, alignment, &mappedMemoryRange);
