...
sk_sp<GrVkSecondaryCBDrawContext> GrVkSecondaryCBDrawContext::Make(
    GrRecordingContext* rContext, const SkImageInfo& imageInfo, const GrVkDrawableInfo& vkInfo,
  ...
  sk_sp<GrSurfaceProxy> proxy(
      rContext->priv().proxyProvider()->wrapVulkanSecondaryCBAsRenderTarget(imageInfo, vkInfo));
  if (!proxy) {
    return nullptr;
  }
  ...
  auto device = rContext->priv().createDevice(
      SkColorTypeToGrColorType(imageInfo.colorType()), std::move(proxy), imageInfo.refColorSpace(),
      kTopLeft_GrSurfaceOrigin, SkSurfacePropsCopyOrDefault(props),
...
GrVkSecondaryCBDrawContext::GrVkSecondaryCBDrawContext(
    sk_sp<skgpu::BaseDevice> device, const SkSurfaceProps* props)
...
bool GrVkSecondaryCBDrawContext::characterize(SkSurfaceCharacterization* characterization) const {
  ...
  characterization->set(
      ...
      SkSurfaceCharacterization::VulkanSecondaryCBCompatible(true), isProtected, this->props());
