...
class GrVkUniformBuffer : public GrVkBuffer {
 ...
 private:
  class Resource : public GrVkBuffer::Resource {
   public:
    Resource(GrVkGpu* gpu, VkBuffer buf, const GrVkAlloc& alloc, const GrVkDescriptorSet* descSet)
        : INHERITED(gpu, buf, alloc, kUniform_Type), fDescriptorSet(descSet) {}
  ...
  const GrVkBuffer::Resource* createResource(
      GrVkGpu* gpu, const GrVkBuffer::Desc& descriptor) override;
