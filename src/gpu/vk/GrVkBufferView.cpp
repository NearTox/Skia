...
const GrVkBufferView* GrVkBufferView::Create(
    const GrVkGpu* gpu, VkBuffer buffer, VkFormat format, VkDeviceSize offset, VkDeviceSize range) {
  ...
  VkResult err = GR_VK_CALL(
      gpu->vkInterface(), CreateBufferView(gpu->device(), &viewInfo, nullptr, &bufferView));
  if (err) {
    return nullptr;
  }
