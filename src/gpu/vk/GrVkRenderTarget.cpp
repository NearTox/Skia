...
sk_sp<GrVkRenderTarget> GrVkRenderTarget::MakeWrappedRenderTarget(
  ...
  if (sampleCnt > 1) {
    vkRT = new GrVkRenderTarget(
        ...
        std::move(msMutableState), std::move(colorAttachmentView),
...
const GrVkFramebuffer* GrVkRenderTarget::createFramebuffer(
  ...
  fCachedFramebuffers[cacheIndex] = GrVkFramebuffer::Create(
      gpu, this->width(), this->height(), renderPass, fColorAttachmentView.get(), stencilView);
...
void GrVkRenderTarget::addResources(
    GrVkCommandBuffer& commandBuffer, bool withStencil, SelfDependencyFlags selfDepFlags) {
