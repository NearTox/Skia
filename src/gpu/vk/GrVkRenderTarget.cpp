...
GrVkRenderTarget::GrVkRenderTarget(
    GrVkGpu* gpu, SkISize dimensions, sk_sp<GrVkAttachment> colorAttachment,
    sk_sp<GrVkAttachment> resolveAttachment, CreateType createType)
    : GrSurface(
...
sk_sp<GrVkRenderTarget> GrVkRenderTarget::MakeWrappedRenderTarget(
  ...
  sk_sp<GrVkAttachment> wrappedAttachment = GrVkAttachment::MakeWrapped(
...
sk_sp<GrVkRenderTarget> GrVkRenderTarget::MakeSecondaryCBRenderTarget(
  ...
  sk_sp<GrVkAttachment> colorAttachment = GrVkAttachment::MakeWrapped(
      gpu, dimensions, info, std::move(mutableState), GrAttachment::UsageFlags::kColorAttachment,
...
void GrVkRenderTarget::createFramebuffer(
  ...
  GrVkAttachment* colorAttachment = withResolve ? this->msaaAttachment() : this->colorAttachment();

  // Stencil attachment view is stored in the base RT stencil attachment
  bool useMSAA = this->numSamples() > 1 || withResolve;
  GrVkAttachment* stencil =
      withStencil ? static_cast<GrVkAttachment*>(this->getStencilAttachment(useMSAA)) : nullptr;
