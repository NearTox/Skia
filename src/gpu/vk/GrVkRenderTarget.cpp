...
GrVkRenderTarget::GrVkRenderTarget(
    ...
    : GrSurface(gpu, dimensions, info.fProtected),
      ...
      fMSAAImage(new GrVkImage(
          gpu, msaaInfo, std::move(msaaMutableState), GrBackendObjectOwnership::kOwned)),
...
GrVkRenderTarget::GrVkRenderTarget(
    ...
    sk_sp<GrBackendSurfaceMutableStateImpl> mutableState, const GrVkImageView* colorAttachmentView)
    : GrSurface(gpu, dimensions, info.fProtected),
      GrVkImage(gpu, info, std::move(mutableState), GrBackendObjectOwnership::kBorrowed),
...
sk_sp<GrVkRenderTarget> GrVkRenderTarget::MakeWrappedRenderTarget(
  ...
  if (sampleCnt > 1) {
    ...
    resolveAttachmentView = GrVkImageView::Create(
        gpu, info.fImage, pixelFormat, GrVkImageView::kColor_Type, 1, GrVkYcbcrConversionInfo());
    if (!resolveAttachmentView) {
      GrVkImage::DestroyImageInfo(gpu, &msInfo);
      return nullptr;
    }
    msMutableState.reset(
        new GrBackendSurfaceMutableStateImpl(msInfo.fImageLayout, msInfo.fCurrentQueueFamily));
  } else {
    // Set color attachment image
  ...
  if (sampleCnt > 1) {
    vkRT = new GrVkRenderTarget(
        gpu, dimensions, sampleCnt, info, std::move(mutableState), msInfo,
        std::move(msMutableState), colorAttachmentView, resolveAttachmentView);
  } else {
    vkRT =
        new GrVkRenderTarget(gpu, dimensions, info, std::move(mutableState), colorAttachmentView);
  }
...
const GrVkRenderPass* GrVkRenderTarget::getSimpleRenderPass(
    bool withStencil, bool needsXferBarrier) {
  if (withStencil) {
    if (needsXferBarrier) {
  ...
  if (withStencil) {
    ...
    if (needsXferBarrier) {
      ...
      fCachedSimpleRenderPass =
          rp.findCompatibleRenderPass(*this, &fCompatibleRPHandle, withStencil, needsXferBarrier);
      return fCachedSimpleRenderPass;
    }
  }
}
...
const GrVkFramebuffer* GrVkRenderTarget::createFramebuffer(
  ...
  GrVkFramebuffer* frameBuffer = GrVkFramebuffer::Create(
      gpu, this->width(), this->height(), renderPass, fColorAttachmentView, stencilView);
...
void GrVkRenderTarget::addResources(
    GrVkCommandBuffer& commandBuffer, bool withStencil, bool needsXferBarrier) {
  ...
  commandBuffer.addResource(
      this->msaaImageResource() ? this->msaaImageResource() : this->resource());
...
GrBackendRenderTarget GrVkRenderTarget::getBackendRenderTarget() const {
  ...
  return GrBackendRenderTarget(
      this->width(), this->height(), this->numSamples(), fInfo, this->getMutableState());
}
