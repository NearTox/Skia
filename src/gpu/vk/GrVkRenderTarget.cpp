...
GrVkRenderTarget::GrVkRenderTarget(
    ...
    sk_sp<GrBackendSurfaceMutableStateImpl> mutableState, sk_sp<GrVkAttachment> msaaAttachment,
...
GrVkRenderTarget::GrVkRenderTarget(
    ...
    sk_sp<GrBackendSurfaceMutableStateImpl> mutableState, sk_sp<GrVkAttachment> msaaAttachment,
    sk_sp<const GrVkImageView> colorAttachmentView,
    sk_sp<const GrVkImageView> resolveAttachmentView, GrBackendObjectOwnership ownership)
    : GrSurface(gpu, dimensions, info.fProtected),
...
GrVkRenderTarget::GrVkRenderTarget(
    GrVkGpu* gpu, SkISize dimensions, const GrVkImageInfo& info,
    sk_sp<GrBackendSurfaceMutableStateImpl> mutableState,
    sk_sp<const GrVkImageView> colorAttachmentView, GrBackendObjectOwnership ownership)
    : GrSurface(gpu, dimensions, info.fProtected),
      GrVkImage(gpu, info, std::move(mutableState), ownership),
      GrRenderTarget(gpu, dimensions, info.fSampleCount, info.fProtected),
      fColorAttachmentView(std::move(colorAttachmentView)),
...
GrVkRenderTarget::GrVkRenderTarget(
  ...
  // render pass.
  int exteralRPIndex =
      renderpass_features_to_index(false, false, SelfDependencyFlags::kNone, LoadFromResolve::kNo);
...
sk_sp<GrVkRenderTarget> GrVkRenderTarget::MakeWrappedRenderTarget(
  ...
  if (sampleCnt != wrappedImageSampleCnt) {
    ...
    // level.
    resolveAttachmentView = GrVkImageView::Make(
        gpu, info.fImage, pixelFormat, GrVkImageView::kColor_Type,
    ...
    colorAttachmentView = GrVkImageView::Make(
  ...
  if (resolveAttachmentView) {
    vkRT = new GrVkRenderTarget(
        gpu, dimensions, sampleCnt, info, std::move(mutableState), std::move(vkMSAAAttachment),
        std::move(colorAttachmentView), std::move(resolveAttachmentView));
  } else {
    vkRT = new GrVkRenderTarget(
        gpu, dimensions, info, std::move(mutableState), std::move(colorAttachmentView));
  }
...
sk_sp<GrVkRenderTarget> GrVkRenderTarget::MakeSecondaryCBRenderTarget(
  ...
  GrVkRenderTarget* vkRT = new GrVkRenderTarget(
      gpu, dimensions, info, std::move(mutableState), rp, vkInfo.fSecondaryCommandBuffer);
...
const GrVkFramebuffer* GrVkRenderTarget::createFramebuffer(
  ...
  fCachedFramebuffers[cacheIndex] = GrVkFramebuffer::Create(
      gpu, this->width(), this->height(), renderPass, fColorAttachmentView.get(), resolveView,
      stencilView);
...
void GrVkRenderTarget::getAttachmentsDescriptor(
    ...
    bool withResolve, bool withStencil) const {
  SkASSERT(!this->wrapsSecondaryCommandBuffer());
...
const GrVkDescriptorSet* GrVkRenderTarget::inputDescSet(GrVkGpu* gpu, bool forResolve) {
  ...
  imageInfo.imageLayout =
      forResolve ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL : VK_IMAGE_LAYOUT_GENERAL;
...
void GrVkRenderTarget::addResources(
    GrVkCommandBuffer& commandBuffer, const GrVkRenderPass& renderPass) {
