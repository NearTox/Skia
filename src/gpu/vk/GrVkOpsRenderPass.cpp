  ...
  if (withResolve) {
    ...
    if (loadFromResolve == LoadFromResolve::kLoad) {
      resolveAttachment->setImageLayout(
          fGpu, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_ACCESS_INPUT_ATTACHMENT_READ_BIT,
...
bool GrVkOpsRenderPass::beginRenderPass(
  ...
  auto nativeBounds = GrNativeRect::MakeIRectRelativeTo(
      fOrigin, dimensions.height(), useFullBounds ? SkIRect::MakeSize(dimensions) : fBounds);
  ...
  if ((0 != granularity.width && 1 != granularity.width) ||
    ...
    adjust_bounds_to_granularity(
        &adjustedBounds, nativeBounds, granularity, dimensions.width(), dimensions.height());
  } else {
    adjustedBounds = nativeBounds;
  }

  if (!fGpu->beginRenderPass(
          fCurrentRenderPass, fFramebuffer, &clearColor, fRenderTarget, adjustedBounds,
...
bool GrVkOpsRenderPass::init(
  ...
  fCurrentRenderPass =
      fGpu->resourceProvider().findRenderPass(rpHandle, vkColorOps, vkResolveOps, vkStencilOps);
...
void GrVkOpsRenderPass::loadResolveIntoMSAA(const SkIRect& nativeBounds) {
  fGpu->loadMSAAFromResolve(
      this->currentCommandBuffer(), *fCurrentRenderPass, fFramebuffer->colorAttachment(),
      fFramebuffer->resolveAttachment(), nativeBounds);
...
void GrVkOpsRenderPass::submit() {
  ...
  if (this->wrapsSecondaryCommandBuffer()) {
    ...
    fFramebuffer->returnExternalGrSecondaryCommandBuffer(std::move(fCurrentSecondaryCommandBuffer));
    return;
  }
...
bool GrVkOpsRenderPass::set(
    GrRenderTarget* rt, sk_sp<GrVkFramebuffer> framebuffer, GrSurfaceOrigin origin,
...
bool GrVkOpsRenderPass::wrapsSecondaryCommandBuffer() const { return fFramebuffer->isExternal(); }
...
void GrVkOpsRenderPass::onClearStencilClip(const GrScissorState& scissor, bool insideStencilMask) {
  ...
  if (!scissor.enabled()) {
    ...
    vkRect.setLTRB(
        scissor.rect().fLeft, sb->height() - scissor.rect().fBottom, scissor.rect().fRight,
        sb->height() - scissor.rect().fTop);
  }
...
void GrVkOpsRenderPass::onClear(const GrScissorState& scissor, std::array<float, 4> color) {
  ...
  if (!scissor.enabled()) {
    ...
    vkRect.setLTRB(
        scissor.rect().fLeft, dimensions.height() - scissor.rect().fBottom, scissor.rect().fRight,
        dimensions.height() - scissor.rect().fTop);
  }
  clearRect.rect.offset = {vkRect.fLeft, vkRect.fTop};
  clearRect.rect.extent = {(uint32_t)vkRect.width(), (uint32_t)vkRect.height()};
  clearRect.baseArrayLayer = 0;
...
void GrVkOpsRenderPass::addAdditionalRenderPass(bool mustUseSecondaryCommandBuffer) {
  ...
  GrVkRenderPass::LoadStoreOps vkResolveOps(
...
bool GrVkOpsRenderPass::onBindPipeline(const GrProgramInfo& programInfo, const SkRect& drawBounds) {
  ...
  if (!fCurrentPipelineState->setAndBindUniforms(
          fGpu, colorAttachment->dimensions(), programInfo, currentCB)) {
    return false;
  }
  ...
  if (!programInfo.pipeline().isScissorTestEnabled()) {
    ...
    GrVkPipeline::SetDynamicScissorRectState(
        fGpu, currentCB, colorAttachment->dimensions(), fOrigin, fCurrentPipelineBounds);
  }
  GrVkPipeline::SetDynamicViewportState(fGpu, currentCB, colorAttachment->dimensions());
  GrVkPipeline::SetDynamicBlendConstantState(
      fGpu, currentCB, programInfo.pipeline().writeSwizzle(),
...
void GrVkOpsRenderPass::onSetScissorRect(const SkIRect& scissor) {
  ...
  GrVkPipeline::SetDynamicScissorRectState(
      fGpu, this->currentCommandBuffer(), fFramebuffer->colorAttachment()->dimensions(), fOrigin,
      combinedScissorRect);
}
...
bool GrVkOpsRenderPass::onBindTextures(
  ...
  if (!fCurrentPipelineState->setAndBindTextures(
          fGpu, geomProc, pipeline, geomProcTextures, this->currentCommandBuffer())) {
    return false;
  }
  if (fSelfDependencyFlags == SelfDependencyFlags::kForInputAttachment) {
    ...
    return fCurrentPipelineState->setAndBindInputAttachment(
        fGpu, std::move(ds), this->currentCommandBuffer());
  }
  return true;
}
...
void GrVkOpsRenderPass::onDrawInstanced(
    int instanceCount, int baseInstance, int vertexCount, int baseVertex) {
  if (!fCurrentRenderPass) {
    SkASSERT(fGpu->isDeviceLost());
    return;
  }
...
void GrVkOpsRenderPass::onDrawIndirect(
...
void GrVkOpsRenderPass::onDrawIndexedIndirect(
    const GrBuffer* drawIndirectBuffer, size_t offset, int drawCount) {
  SkASSERT(!drawIndirectBuffer->isCpuBuffer());
  if (!fCurrentRenderPass) {
    SkASSERT(fGpu->isDeviceLost());
