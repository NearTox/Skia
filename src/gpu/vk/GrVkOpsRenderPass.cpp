...
GrVkPipelineState* GrVkOpsRenderPass::prepareDrawState(
  ...
  if (!programInfo.pipeline().isScissorEnabled()) {
    GrVkPipeline::SetDynamicScissorRectState(
        fGpu, currentCB, fRenderTarget, fOrigin, renderPassScissorRect);
...
void GrVkOpsRenderPass::onDraw(
  ...
  for (int i = 0; i < meshCount; ++i) {
    ...
    if (hasDynamicScissors) {
      SkIRect combinedScissorRect;
      if (!combinedScissorRect.intersect(renderPassScissorRect, programInfo.dynamicScissor(i))) {
        combinedScissorRect = SkIRect::MakeEmpty();
      }
      GrVkPipeline::SetDynamicScissorRectState(
