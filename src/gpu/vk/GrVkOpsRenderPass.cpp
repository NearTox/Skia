...
bool GrVkOpsRenderPass::beginRenderPass(
  ...
  if ((0 != granularity.width && 1 != granularity.width) ||
    ...
    adjust_bounds_to_granularity(
        &adjustedBounds, nativeBounds.asSkIRect(), granularity, vkRT->width(), vkRT->height());
  } else {
    adjustedBounds = nativeBounds.asSkIRect();
  }

  if (!fGpu->beginRenderPass(
          fCurrentRenderPass, &clearColor, vkRT, adjustedBounds, firstSubpassUsesSecondaryCB)) {
...
bool GrVkOpsRenderPass::init(
    ...
    const GrOpsRenderPass::StencilLoadAndStoreInfo& stencilInfo, std::array<float, 4> clearColor,
  ...
  if (rpHandle.isValid()) {
    ...
    fCurrentRenderPass = fGpu->resourceProvider().findRenderPass(
        vkRT, vkColorOps, vkResolveOps, vkStencilOps, nullptr, withResolve, withStencil,
        fSelfDependencyFlags, fLoadFromResolve);
  }
  ...
  if (!fGpu->vkCaps().preferPrimaryOverSecondaryCommandBuffers()) {
    ...
    const GrVkFramebuffer* framebuffer =
        vkRT->getFramebuffer(withResolve, withStencil, fSelfDependencyFlags, fLoadFromResolve);
...
void GrVkOpsRenderPass::loadResolveIntoMSAA(const SkIRect& nativeBounds) {
  fGpu->loadMSAAFromResolve(
      this->currentCommandBuffer(), *fCurrentRenderPass, fRenderTarget, fRenderTarget,
      nativeBounds);
...
bool GrVkOpsRenderPass::set(
  ...
  return this->init(
      localColorInfo, resolveInfo, stencilInfo, colorInfo.fClearColor, withResolve,
      SkToBool(stencil));
}
...
void GrVkOpsRenderPass::onClearStencilClip(const GrScissorState& scissor, bool insideStencilMask) {
  ...
  if (!scissor.enabled()) {
    ...
    vkRect.setLTRB(
        scissor.rect().fLeft, fRenderTarget->height() - scissor.rect().fBottom,
...
void GrVkOpsRenderPass::addAdditionalRenderPass(bool mustUseSecondaryCommandBuffer) {
  ...
  if (rpHandle.isValid()) {
    ...
    fCurrentRenderPass = fGpu->resourceProvider().findRenderPass(
        vkRT, vkColorOps, vkResolveOps, vkStencilOps, nullptr, withResolve, withStencil,
        fSelfDependencyFlags, loadFromResolve);
  }
  if (!fCurrentRenderPass) {
    return;
  }
  ...
  if (!fGpu->vkCaps().preferPrimaryOverSecondaryCommandBuffers() || mustUseSecondaryCommandBuffer) {
    ...
    const GrVkFramebuffer* framebuffer =
        vkRT->getFramebuffer(withResolve, withStencil, fSelfDependencyFlags, loadFromResolve);
    fCurrentSecondaryCommandBuffer->begin(fGpu, framebuffer, fCurrentRenderPass);
  }
...
bool GrVkOpsRenderPass::onBindPipeline(const GrProgramInfo& programInfo, const SkRect& drawBounds) {
  ...
  if (!programInfo.pipeline().isScissorTestEnabled()) {
    ...
    GrVkPipeline::SetDynamicScissorRectState(
        fGpu, currentCB, fRenderTarget, fOrigin, fCurrentPipelineBounds);
  }
...
void GrVkOpsRenderPass::onSetScissorRect(const SkIRect& scissor) {
  ...
  GrVkPipeline::SetDynamicScissorRectState(
      fGpu, this->currentCommandBuffer(), fRenderTarget, fOrigin, combinedScissorRect);
}
