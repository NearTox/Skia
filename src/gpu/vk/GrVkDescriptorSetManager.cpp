...
GrVkDescriptorSetManager* GrVkDescriptorSetManager::CreateUniformManager(GrVkGpu* gpu) {
  ...
  return new GrVkDescriptorSetManager(
      gpu, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, visibilities, samplers);
}
...
GrVkDescriptorSetManager::DescriptorPoolManager::DescriptorPoolManager(
    ...
    const SkTArray<const GrVkSampler*>& immutableSamplers)
    : fDescType(type), fCurrentDescriptorCount(0), fPool(nullptr) {
  if (VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER == type ||
      VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER == type) {
...
#endif
    GR_VK_CALL_ERRCHECK(
        gpu->vkInterface(), CreateDescriptorSetLayout(
                                gpu->device(), &dsSamplerLayoutCreateInfo, nullptr, &fDescLayout));
    ...
    static const uint32_t bindings[kUniformDescPerSet] = {GrVkUniformHandler::kGeometryBinding,
    ...
    GR_VK_CALL_ERRCHECK(
        gpu->vkInterface(),
        CreateDescriptorSetLayout(gpu->device(), &uniformLayoutCreateInfo, nullptr, &fDescLayout));
...
void GrVkDescriptorSetManager::DescriptorPoolManager::getNewDescriptorSet(
  ...
  GR_VK_CALL_ERRCHECK(
      gpu->vkInterface(), AllocateDescriptorSets(gpu->device(), &dsAllocateInfo, ds));
}
