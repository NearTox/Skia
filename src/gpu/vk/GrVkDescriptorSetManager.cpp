...
static bool get_layout_and_desc_count(
    GrVkGpu* gpu, VkDescriptorType type, const SkTArray<uint32_t>& visibilities,
    const SkTArray<const GrVkSampler*>& immutableSamplers, VkDescriptorSetLayout* descSetLayout,
  ...
  if (VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER == type ||
    ...
    *descCountPerSet = 0;
    ...
    GR_VK_CALL_RESULT(
        ...
        CreateDescriptorSetLayout(
            gpu->device(), &dsSamplerLayoutCreateInfo, nullptr, descSetLayout));
    ...
    GR_VK_CALL_RESULT(
        gpu, result,
        CreateDescriptorSetLayout(gpu->device(), &uniformLayoutCreateInfo, nullptr, descSetLayout));
    if (result != VK_SUCCESS) {
      return false;
    }
    ...
    *descCountPerSet = kUniformDescPerSet;
    ...
    GR_VK_CALL_RESULT(
        gpu, result,
        CreateDescriptorSetLayout(gpu->device(), &inputLayoutCreateInfo, nullptr, descSetLayout));
    if (result != VK_SUCCESS) {
      return false;
    }
...
GrVkDescriptorSetManager* GrVkDescriptorSetManager::Create(
  ...
  uint32_t descCountPerSet;
  if (!get_layout_and_desc_count(
          gpu, type, visibilities, immutableSamplers, &descSetLayout, &descCountPerSet)) {
    return nullptr;
  }
  return new GrVkDescriptorSetManager(
      gpu, type, descSetLayout, descCountPerSet, visibilities, immutableSamplers);
}
...
const GrVkDescriptorSet* GrVkDescriptorSetManager::getDescriptorSet(
    GrVkGpu* gpu, const Handle& handle) {
...
GrVkDescriptorSetManager::DescriptorPoolManager::DescriptorPoolManager(
    VkDescriptorSetLayout layout, VkDescriptorType type, uint32_t descCountPerSet)
    : fDescLayout(layout),
      fDescType(type),
...
bool GrVkDescriptorSetManager::DescriptorPoolManager::getNewPool(GrVkGpu* gpu) {
  ...
  fPool = gpu->resourceProvider().findOrCreateCompatibleDescriptorPool(fDescType, fMaxDescriptors);
  return SkToBool(fPool);
}

bool GrVkDescriptorSetManager::DescriptorPoolManager::getNewDescriptorSet(
  ...
  VkResult result;
  GR_VK_CALL_RESULT(gpu, result, AllocateDescriptorSets(gpu->device(), &dsAllocateInfo, ds));
