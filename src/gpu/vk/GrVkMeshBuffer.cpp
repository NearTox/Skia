...
GrVkMeshBuffer::GrVkMeshBuffer(
    GrVkGpu* gpu, GrGpuBufferType bufferType, const GrVkBuffer::Desc& desc,
    const GrVkBuffer::Resource* bufferResource)
    : GrGpuBuffer(
          gpu, desc.fSizeInBytes, bufferType,
...
static GrVkBuffer::Type vk_mesh_buffer_type_from_gr_bufer_type(GrGpuBufferType bufferType) {
  switch (bufferType) {
    case GrGpuBufferType::kVertex: return GrVkBuffer::kVertex_Type;
    case GrGpuBufferType::kIndex: return GrVkBuffer::kIndex_Type;
    case GrGpuBufferType::kDrawIndirect: return GrVkBuffer::kIndirect_Type;
    case GrGpuBufferType::kXferCpuToGpu:
    case GrGpuBufferType::kXferGpuToCpu: SK_ABORT("Invalid mesh buffer type.");
  }
  SkUNREACHABLE;
}

sk_sp<GrVkMeshBuffer> GrVkMeshBuffer::Make(
    GrVkGpu* gpu, GrGpuBufferType bufferType, size_t size, bool dynamic) {
