...
GrVkPipelineState* GrVkResourceProvider::PipelineStateCache::findOrCreatePipelineState(
  ...
  if (programInfo.isStencilEnabled()) {
    ...
    SkASSERT(
        renderTarget->getStencilAttachment(programInfo.numSamples() > 1)->numSamples() ==
  ...
  auto tmp = this->findOrCreatePipelineStateImpl(
      desc, programInfo, compatibleRenderPass, overrideSubpassForResolveLoad, &stat);
  if (!tmp) {
...
GrVkPipelineState* GrVkResourceProvider::PipelineStateCache::findOrCreatePipelineStateImpl(
    ...
    bool overrideSubpassForResolveLoad, Stats::ProgramCacheResult* stat) {
  if (stat) {
