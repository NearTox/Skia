...
GrVkStencilAttachment::GrVkStencilAttachment(
    GrVkGpu* gpu, SkISize dimensions, const Format& format, const GrVkImage::ImageDesc& desc,
...
GrVkStencilAttachment* GrVkStencilAttachment::Create(
  ...
  sk_sp<const GrVkImageView> imageView = GrVkImageView::Make(
      gpu, info.fImage, format.fInternalFormat, GrVkImageView::kStencil_Type, 1,
  ...
  GrVkStencilAttachment* stencil = new GrVkStencilAttachment(
      gpu, dimensions, format, imageDesc, info, std::move(mutableState), std::move(imageView));
