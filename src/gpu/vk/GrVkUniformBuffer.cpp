...
const GrManagedResource* GrVkUniformBuffer::CreateResource(GrVkGpu* gpu, size_t size) {
  ...
  const GrManagedResource* resource =
      new GrVkUniformBuffer::Resource(gpu, buffer, alloc, descriptorSet);
  return resource;
}

const GrVkBuffer::Resource* GrVkUniformBuffer::createResource(
    GrVkGpu* gpu, const GrVkBuffer::Desc& descriptor) {
