...
GrVkPipelineStateDataManager::GrVkPipelineStateDataManager(
    const UniformInfoArray& uniforms, uint32_t uniformSize, bool usePushConstants)
    : INHERITED(uniforms.count(), uniformSize), fUsePushConstants(usePushConstants) {
  ...
  GrVkUniformHandler::Layout memLayout =
      usePushConstants ? GrVkUniformHandler::kStd430Layout : GrVkUniformHandler::kStd140Layout;
...
std::pair<sk_sp<GrGpuBuffer>, bool> GrVkPipelineStateDataManager::uploadUniforms(
  ...
  if (fUsePushConstants) {
    commandBuffer->pushConstants(
        gpu, layout, gpu->vkCaps().getPushConstantStageFlags(), 0, fUniformSize,
        fUniformData.get());
    ...
    if (fUniformsDirty) {
      ...
      fUniformBuffer = resourceProvider->createBuffer(
          fUniformSize, GrGpuBufferType::kUniform, kDynamic_GrAccessPattern, fUniformData.get());
      if (!fUniformBuffer) {
