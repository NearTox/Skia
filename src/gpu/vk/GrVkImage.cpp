...
void GrVkImage::setImageLayout(
    const GrVkGpu* gpu, VkImageLayout newLayout, VkAccessFlags dstAccessMask,
    VkPipelineStageFlags dstStageMask, bool byRegion, bool releaseFamilyQueue) {
...
bool GrVkImage::InitImageInfo(const GrVkGpu* gpu, const ImageDesc& imageDesc, GrVkImageInfo* info) {
  ...
  GR_VK_CALL_ERRCHECK(
      gpu->vkInterface(), CreateImage(gpu->device(), &imageCreateInfo, nullptr, &image));
...
void GrVkImage::prepareForExternal(GrVkGpu* gpu) {
  this->setImageLayout(
      gpu, this->currentLayout(), 0, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, false, true);
}
...
void GrVkImage::releaseImage(GrVkGpu* gpu) {
  if (fInfo.fCurrentQueueFamily != fInitialQueueFamily) {
    ...
    this->setImageLayout(
        gpu, this->currentLayout(), 0, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, false, true);
  }
  if (fResource) {
...
void GrVkImage::BorrowedResource::freeGPUData(GrVkGpu* gpu) const { this->invokeReleaseProc(); }

void GrVkImage::BorrowedResource::abandonGPUData() const { this->invokeReleaseProc(); }
