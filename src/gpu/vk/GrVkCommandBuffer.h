...
class GrVkCommandBuffer : public GrVkResource {
 public:
  ...
  void bindDescriptorSets(
      const GrVkGpu* gpu, GrVkPipelineState*, GrVkPipelineLayout* layout, uint32_t firstSet,
      uint32_t setCount, const VkDescriptorSet* descriptorSets, uint32_t dynamicOffsetCount,
      const uint32_t* dynamicOffsets);

  void bindDescriptorSets(
      const GrVkGpu* gpu, const SkTArray<const GrVkRecycledResource*>&,
      const SkTArray<const GrVkResource*>&, GrVkPipelineLayout* layout, uint32_t firstSet,
      uint32_t setCount, const VkDescriptorSet* descriptorSets, uint32_t dynamicOffsetCount,
      const uint32_t* dynamicOffsets);
 ...
 protected:
  GrVkCommandBuffer(
      VkCommandBuffer cmdBuffer, GrVkCommandPool* cmdPool, const GrVkRenderPass* rp = nullptr)
      : fIsActive(false),
...
class GrVkPrimaryCommandBuffer : public GrVkCommandBuffer {
 public:
  ...
  void blitImage(
      const GrVkGpu* gpu, const GrVkResource* srcResource, VkImage srcImage,
  ...
  void copyBufferToImage(
      const GrVkGpu* gpu, GrVkTransferBuffer* srcBuffer, GrVkImage* dstImage,
      VkImageLayout dstLayout, uint32_t copyRegionCount, const VkBufferImageCopy* copyRegions);
 ...
 private:
  explicit GrVkPrimaryCommandBuffer(VkCommandBuffer cmdBuffer, GrVkCommandPool* cmdPool)
      : INHERITED(cmdBuffer, cmdPool), fSubmitFence(VK_NULL_HANDLE) {}
