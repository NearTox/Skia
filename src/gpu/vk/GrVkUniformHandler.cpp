...
static uint32_t grsltype_to_alignment_mask(GrSLType type) {
  switch (type) {
    ...
    case kTexture2DRectSampler_GrSLType: break;
  }
...
static void get_ubo_aligned_offset(
    uint32_t* uniformOffset, uint32_t* currentOffset, GrSLType type, int arrayCount) {
...
GrGLSLUniformHandler::UniformHandle GrVkUniformHandler::internalAddUniformArray(
  ...
  SkASSERT(
      kVertex_GrShaderFlag == visibility || kGeometry_GrShaderFlag == visibility ||
...
void GrVkUniformHandler::appendUniformDecls(GrShaderFlags visibility, SkString* out) const {
  ...
  if (!uniformsString.isEmpty()) {
    ...
    out->appendf(
        "layout (set=%d, binding=%d) uniform %sUniformBuffer\n{\n", kUniformBufferDescSet,
