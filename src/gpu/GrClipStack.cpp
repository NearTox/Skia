...
static GrFPResult analytic_clip_fp(
    const GrClipStack::Element& e, const GrShaderCaps& caps,
...
static std::unique_ptr<GrFragmentProcessor> clip_atlas_fp(
    GrCoverageCountingPathRenderer* ccpr, uint32_t opsTaskID, const SkIRect& bounds,
    const GrClipStack::Element& e, SkPath* devicePath, const GrCaps& caps,
    std::unique_ptr<GrFragmentProcessor> fp) {
...
static GrSurfaceProxyView render_sw_mask(
    GrRecordingContext* context, const SkIRect& bounds, const GrClipStack::Element** elements,
    int count) {
  SkASSERT(count > 0);
...
GrClipStack::RawElement::RawElement(
...
GrClipStack::SaveRecord::SaveRecord(
...
void GrClipStack::SaveRecord::invalidateMasks(GrProxyProvider* proxyProvider, Mask::Stack* masks) {
...
GrClip::Effect GrClipStack::apply(
  ...
  if (cs.op() == SkClipOp::kIntersect && (!elementsForMask.empty() || !elementsForAtlas.empty())) {
  ...
  if (!elementsForMask.empty()) {
    bool stencilUnavailable =
        context->priv().caps()->avoidStencilBuffers() || rtc->wrapsVkSecondaryCB();
    ...
    if ((rtc->numSamples() <= 1 && maskRequiresAA) || stencilUnavailable) {
      ...
      std::tie(hasSWMask, clipFP) = GetSWMaskFP(
          context, &fMasks, cs, scissorBounds, elementsForMask.begin(), elementsForMask.count(),
          std::move(clipFP));
    }
    ...
    if (!hasSWMask) {
      if (stencilUnavailable) {
        ...
        render_stencil_mask(
            context, rtc, cs.genID(), scissorBounds, elementsForMask.begin(),
  ...
  if (!elementsForAtlas.empty()) {
    ...
    for (int i = 0; i < elementsForAtlas.count(); ++i) {
      ...
      clipFP = clip_atlas_fp(
          ccpr, opsTaskID, scissorBounds, elementsForAtlas[i]->asElement(),
...
GrFPResult GrClipStack::GetSWMaskFP(
