...
ProgramImpl::FPCoordsMap ProgramImpl::collectTransforms(
    ...
    GrGLSLUniformHandler* uniformHandler, const GrShaderVar& localCoordsVar,
    const GrShaderVar& positionVar, const GrPipeline& pipeline) {
  SkASSERT(
      localCoordsVar.getType() == kFloat2_GrSLType ||
      localCoordsVar.getType() == kFloat3_GrSLType || localCoordsVar.getType() == kVoid_GrSLType);
  SkASSERT(
      positionVar.getType() == kFloat2_GrSLType || positionVar.getType() == kFloat3_GrSLType ||
...
static void write_passthrough_vertex_position(
  ...
  vertBuilder->codeAppendf(
      "float%d %s = %s;", GrSLTypeVecLength(inPos.getType()), outName.c_str(),
      inPos.getName().c_str());
}
...
static void write_vertex_position(
  ...
  *matrixUniform = uniformHandler->addUniform(
      nullptr, kVertex_GrShaderFlag, useCompactTransform ? kFloat4_GrSLType : kFloat3x3_GrSLType,
