...
void GrFillInData(
    GrPixelConfig, int baseWidth, int baseHeight, const SkTArray<size_t>& individualMipOffsets,
...
class GrPixelInfo {
 public:
  ...
  GrPixelInfo(const SkImageInfo& info)
      : fColorInfo(
            SkColorTypeToGrColorType(info.colorType()), info.alphaType(), info.refColorSpace()),
        fWidth(info.width()),
        fHeight(info.height()) {}
...
bool GrConvertPixels(
    ...
    const void* src, size_t srcRB, bool flipY = false, GrSwizzle swizzle = GrSwizzle{});
