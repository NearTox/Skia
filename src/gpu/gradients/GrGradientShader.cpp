...
static std::unique_ptr<GrFragmentProcessor> make_dual_interval_colorizer(
  ...
  return GrSkSLFP::Make(
      effect, "DualIntervalColorizer", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "scale01",
      scale01, "bias01", c0, "scale23", scale23, "bias23", bias23, "threshold", threshold);
}
...
static std::unique_ptr<GrFragmentProcessor> make_unrolled_colorizer(
  ...
  once[intervalCount - 1]([intervalCount] {
    ...
    sksl.appendf(
        ...
        intervalCount, intervalCount, intervalCount, intervalCount, intervalCount, intervalCount,
        intervalCount);
...
#define ADD_STOP(N, suffix)                                                     \
  "scale" #suffix, GrSkSLFP::When(intervalCount > N, scale[N]), "bias" #suffix, \
      GrSkSLFP::When(intervalCount > N, bias[N])

  return GrSkSLFP::Make(
      effects[intervalCount - 1], "UnrolledBinaryColorizer",
      /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "thresholds1_7", thresholds1_7,
      "thresholds9_13", thresholds9_13, "scale0_1", scale[0], "bias0_1", bias[0], ADD_STOP(1, 2_3),
      ADD_STOP(2, 4_5), ADD_STOP(3, 6_7), ADD_STOP(4, 8_9), ADD_STOP(5, 10_11), ADD_STOP(6, 12_13),
      ADD_STOP(7, 14_15));
#undef ADD_STOP
}
...
static std::unique_ptr<GrFragmentProcessor> make_colorizer(
  ...
  if (tryAnalyticColorizer) {
    if (count == 3) {
      ...
      return make_dual_interval_colorizer(
          colors[offset], colors[offset + 1], colors[offset + 1], colors[offset + 2],
          positions[offset + 1]);
