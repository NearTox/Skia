...
static std::unique_ptr<GrFragmentProcessor> make_colorizer(
  ...
  if (tryAnalyticColorizer) {
    if (count == 3) {
      ...
      return GrDualIntervalGradientColorizer::Make(
    ...
    std::unique_ptr<GrFragmentProcessor> unrolled =
        GrUnrolledBinaryGradientColorizer::Make(colors + offset, positions + offset, count);
    if (unrolled) {
      return unrolled;
    }
  }
...
static std::unique_ptr<GrFragmentProcessor> make_gradient(
  ...
  switch (shader.getTileMode()) {
    case SkTileMode::kRepeat:
      gradient = GrTiledGradientEffect::Make(
    ...
    case SkTileMode::kClamp:
      ...
      // border.
      gradient = GrClampedGradientEffect::Make(
