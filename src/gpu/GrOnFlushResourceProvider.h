...
class GrOnFlushCallbackObject {
 public:
  ...
  virtual void preFlush(
      GrOnFlushResourceProvider*, const uint32_t* opsTaskIDs, int numOpsTaskIDs) = 0;
  ...
  virtual void postFlush(
      GrDeferredUploadToken startTokenForNextFlush, const uint32_t* opsTaskIDs, int numOpsTaskIDs) {
  }
...
class GrOnFlushResourceProvider {
 public:
  ...
  sk_sp<const GrGpuBuffer> findOrMakeStaticBuffer(
      GrGpuBufferType, size_t, const void* data, const GrUniqueKey&);
