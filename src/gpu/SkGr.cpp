...
sk_sp<GrTextureProxy> GrCopyBaseMipMapToTextureProxy(
    GrRecordingContext* ctx, GrSurfaceProxy* baseProxy, GrColorType srcColorType) {
  SkASSERT(baseProxy);
...
sk_sp<GrTextureProxy> GrRefCachedBitmapTextureProxy(
    GrRecordingContext* ctx, const SkBitmap& bitmap, GrSamplerState params,
...
static inline bool skpaint_to_grpaint_impl(
  ...
  if (SkPaintPriv::ShouldDither(skPaint, GrColorTypeToSkColorType(ct)) &&
    ...
    if (ditherRange >= 0) {
      ...
      auto ditherFP = GrSkSLFP::Make(context, effect, "Dither", &ditherRange, sizeof(ditherRange));
      if (ditherFP) {
