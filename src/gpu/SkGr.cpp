...
static inline float dither_range_for_config(GrColorType dstColorType) {
  ...
  switch (dstColorType) {
    // 4 bit
    case GrColorType::kABGR_4444: return 1 / 15.f;
    // 6 bit
...
static inline bool skpaint_to_grpaint_impl(
  ...
  if (paintFP) {
    if (primColorMode) {
      ...
      paintFP =
          GrXfermodeFragmentProcessor::MakeFromSrcProcessor(std::move(paintFP), *primColorMode);
    ...
    if (primColorMode) {
      ...
      paintFP = GrConstColorProcessor::Make(
          /*inputFP=*/nullptr, opaqueColor, GrConstColorProcessor::InputMode::kIgnore);
      paintFP =
          GrXfermodeFragmentProcessor::MakeFromSrcProcessor(std::move(paintFP), *primColorMode);
  ...
  if (SkPaintPriv::ShouldDither(skPaint, GrColorTypeToSkColorType(ct)) &&
    ...
    if (ditherRange > 0.f) {
      ...
      auto ditherFP = GrSkSLFP::Make(
          context, effect, "Dither", SkData::MakeWithCopy(&ditherRange, sizeof(ditherRange)));
      if (ditherFP) {
