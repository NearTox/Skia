...
GrSurfaceProxyView GrRefCachedBitmapView(
    GrRecordingContext* ctx, const SkBitmap& bitmap, GrMipmapped mipMapped) {
  GrBitmapTextureMaker maker(ctx, bitmap, GrImageTexGenPolicy::kDraw);
  return maker.view(mipMapped);
}

GrSurfaceProxyView GrMakeCachedBitmapProxyView(
...
static inline bool skpaint_to_grpaint_impl(
  ...
  SkSamplingOptions sampling(
      SkPaintPriv::GetFQ(skPaint), SkSamplingOptions::kMedium_asMipmapLinear);
  ...
  if (paintFP) {
    if (primColorMode) {
      ...
      paintFP = GrBlendFragmentProcessor::Make(std::move(paintFP), /*dst=*/nullptr, *primColorMode);
...
bool SkPaintToGrPaintWithTexture(
  ...
  // TODO: take sampling directly
  SkSamplingOptions sampling(SkPaintPriv::GetFQ(paint), SkSamplingOptions::kMedium_asMipmapLinear);

  std::unique_ptr<GrFragmentProcessor> shaderFP;
  if (textureIsAlphaOnly) {
    if (const auto* shader = as_SB(paint.getShader())) {
      shaderFP =
          shader->asFragmentProcessor(GrFPArgs(context, matrixProvider, sampling, &dstColorInfo));
      if (!shaderFP) {
        return false;
      }
...
std::tuple<GrSamplerState::Filter, GrSamplerState::MipmapMode, SkCubicResampler>
GrInterpretSamplingOptions(
    SkISize imageDims, const SkSamplingOptions& sampling, const SkMatrix& viewM,
    ...
    if (allowFilterQualityReduction) {
      ...
      switch (SkMatrixPriv::AdjustHighQualityFilterLevel(matrix)) {
        case kNone_SkFilterQuality:
          return {Filter::kNearest, MipmapMode::kNone, kInvalidCubicResampler};
        case kLow_SkFilterQuality:
          return {Filter::kLinear, MipmapMode::kNone, kInvalidCubicResampler};
        case kMedium_SkFilterQuality:
          return {Filter::kLinear, MipmapMode::kLinear, kInvalidCubicResampler};
