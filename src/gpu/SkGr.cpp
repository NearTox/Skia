...
sk_sp<SkIDChangeListener> GrMakeUniqueKeyInvalidationListener(
    GrUniqueKey* key, uint32_t contextID) {
...
static std::unique_ptr<GrFragmentProcessor> make_dither_effect(
  ...
  static auto effect = SkMakeRuntimeEffect(
...
bool SkPaintToGrPaintReplaceShader(
  ...
  return skpaint_to_grpaint_impl(
      context, dstColorInfo, skPaint, matrixProvider, &shaderFP,
...
bool SkPaintToGrPaintNoShader(
  ...
  return skpaint_to_grpaint_impl(
      context, dstColorInfo, skPaint, matrixProvider, &nullShaderFP,
      /*primColorMode=*/nullptr, grPaint);
}
...
bool SkPaintToGrPaintWithBlendReplaceShader(
  ...
  return skpaint_to_grpaint_impl(
      context, dstColorInfo, skPaint, matrixProvider, &shaderFP, &primColorMode, grPaint);
}
...
bool SkPaintToGrPaintWithTexture(
  ...
  if (textureIsAlphaOnly) {
    if (const auto* shader = as_SB(paint.getShader())) {
      shaderFP = shader->asFragmentProcessor(GrFPArgs(context, matrixProvider, &dstColorInfo));
      if (!shaderFP) {
        return false;
      }
