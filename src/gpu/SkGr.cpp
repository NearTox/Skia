...
void GrInstallBitmapUniqueKeyInvalidator(
...
sk_sp<GrTextureProxy> GrRefCachedBitmapTextureProxy(
    GrRecordingContext* ctx, const SkBitmap& bitmap, const GrSamplerState& params,
...
sk_sp<GrTextureProxy> GrMakeCachedBitmapProxy(
    GrProxyProvider* proxyProvider, const SkBitmap& bitmap, SkBackingFit fit) {
  ...
  SkCopyPixelsMode cpyMode =
      proxyProvider->renderingDirectly() ? kNever_SkCopyPixelsMode : kIfMutable_SkCopyPixelsMode;
...
sk_sp<GrTextureProxy> GrMakeCachedImageProxy(
    GrProxyProvider* proxyProvider, sk_sp<SkImage> srcImage, SkBackingFit fit) {
  sk_sp<GrTextureProxy> proxy;
  ...
  if (!proxy) {
    proxy =
        proxyProvider->createTextureProxy(srcImage, kNone_GrSurfaceFlags, 1, SkBudgeted::kYes, fit);
    if (proxy && originalKey.isValid()) {
      ...
      if (bm && proxyProvider->renderingDirectly()) {
        GrInstallBitmapUniqueKeyInvalidator(
            originalKey, proxyProvider->contextID(), bm->pixelRef());
      }
    }
  }
...
GrPixelConfig SkColorType2GrPixelConfig(const SkColorType type) {
  switch (type) {
    ...
    case kRGB_565_SkColorType: return kRGB_565_GrPixelConfig;
    case kARGB_4444_SkColorType: return kRGBA_4444_GrPixelConfig;
    case kRGBA_8888_SkColorType: return kRGBA_8888_GrPixelConfig;
    case kRGB_888x_SkColorType: return kRGB_888_GrPixelConfig;
    case kBGRA_8888_SkColorType: return kBGRA_8888_GrPixelConfig;
    ...
    case kGray_8_SkColorType: return kGray_8_GrPixelConfig;
    case kRGBA_F16Norm_SkColorType: return kRGBA_half_Clamped_GrPixelConfig;
    case kRGBA_F16_SkColorType: return kRGBA_half_GrPixelConfig;
    case kRGBA_F32_SkColorType: return kRGBA_float_GrPixelConfig;
  }
...
static inline int32_t dither_range_type_for_config(GrPixelConfig dstConfig) {
  switch (dstConfig) {
    ...
    case kBGRA_8888_GrPixelConfig: return 0;
    case kRGB_565_GrPixelConfig: return 1;
    case kRGBA_4444_GrPixelConfig: return 2;
    ...
    case kAlpha_8_as_Red_GrPixelConfig: return -1;
  }
  SkASSERT(false);
  return 0;
}
...
static inline bool skpaint_to_grpaint_impl(
    GrRecordingContext* context, const GrColorSpaceInfo& colorSpaceInfo, const SkPaint& skPaint,
  ...
  if (shaderFP) {
    if (primColorMode) {
      ...
      shaderFP =
          GrXfermodeFragmentProcessor::MakeFromSrcProcessor(std::move(shaderFP), *primColorMode);
    ...
    if (primColorMode) {
      ...
      auto processor =
          GrConstColorProcessor::Make(opaqueColor, GrConstColorProcessor::InputMode::kIgnore);
      processor =
          GrXfermodeFragmentProcessor::MakeFromSrcProcessor(std::move(processor), *primColorMode);
      if (processor) {
  ...
  if (SkPaintPriv::ShouldDither(skPaint, ct) && grPaint->numColorFragmentProcessors() > 0) {
    ...
    if (ditherRange >= 0) {
      ...
      auto ditherFP = GrSkSLFP::Make(
          context, ditherIndex, "Dither", SKSL_DITHER_SRC, &ditherRange, sizeof(ditherRange));
      if (ditherFP) {
...
bool SkPaintToGrPaint(
  ...
  return skpaint_to_grpaint_impl(
      context, colorSpaceInfo, skPaint, viewM, nullptr, nullptr, grPaint);
}
...
bool SkPaintToGrPaintReplaceShader(
  ...
  return skpaint_to_grpaint_impl(
      context, colorSpaceInfo, skPaint, SkMatrix::I(), &shaderFP, nullptr, grPaint);
}
...
bool SkPaintToGrPaintNoShader(
  ...
  return skpaint_to_grpaint_impl(
      context, colorSpaceInfo, skPaint, SkMatrix::I(), &nullShaderFP, nullptr, grPaint);
}
...
bool SkPaintToGrPaintWithXfermode(
  ...
  return skpaint_to_grpaint_impl(
      context, colorSpaceInfo, skPaint, viewM, nullptr, &primColorMode, grPaint);
}

bool SkPaintToGrPaintWithTexture(
    GrRecordingContext* context, const GrColorSpaceInfo& colorSpaceInfo, const SkPaint& paint,
  ...
  if (textureIsAlphaOnly) {
    if (const auto* shader = as_SB(paint.getShader())) {
      shaderFP = shader->asFragmentProcessor(
          GrFPArgs(context, &viewM, paint.getFilterQuality(), &colorSpaceInfo));
      if (!shaderFP) {
        return false;
      }
