...
void GrTCluster_Validate(SkSpan<const sk_sp<T>> input, const SkTInternalLList<T>& llist) {
  ...
  for (T* t : llist) {
    ...
    for (int i = 0; i < Traits::NumDependencies(t); i++) {
      ...
      SkASSERTF(
          seen.contains(dep), "%s came before dependency %s",
          GrTCluster_DebugStr<T, Traits>(t).c_str(), GrTCluster_DebugStr<T, Traits>(dep).c_str());
    }
  }
...
template <typename T, typename Traits>
bool GrTCluster_Visit(T* task, SkTInternalLList<T>* llist, SkTHashMap<uint32_t, T*>* lastTaskMap) {
  SkScopeExit exit([&] {
  ...
  while (clusterHead->fPrev && 1 == Traits::NumTargets(clusterHead->fPrev) &&
