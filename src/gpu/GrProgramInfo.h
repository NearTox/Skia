...
class GrProgramInfo {
 public:
  GrProgramInfo(
      const GrCaps& caps, const GrSurfaceProxyView& targetView, bool usesMSAASurface,
      const GrPipeline* pipeline, const GrUserStencilSettings* userStencilSettings,
      const GrGeometryProcessor* geomProc, GrPrimitiveType primitiveType,
      uint8_t tessellationPatchVertexCount, GrXferBarrierFlags renderPassXferBarriers,
      ...
      : fNeedsStencil(targetView.asRenderTargetProxy()->needsStencil()),
        ...
        fTargetHasVkResolveAttachmentWithInput(
            ...
            ((targetView.asRenderTargetProxy()->numSamples() > 1 && targetView.asTextureProxy()) ||
  ...
  bool isStencilEnabled() const {
    return fUserStencilSettings != &GrUserStencilSettings::kUnused || fPipeline->hasStencilClip();
  }
