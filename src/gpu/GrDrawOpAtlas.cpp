...
std::unique_ptr<GrDrawOpAtlas> GrDrawOpAtlas::Make(
    ...
    AllowMultitexturing allowMultitexturing, EvictionCallback* evictor) {
  if (!format.isValid()) {
    return nullptr;
  }

  std::unique_ptr<GrDrawOpAtlas> atlas(new GrDrawOpAtlas(
      proxyProvider, format, colorType, width, height, plotWidth, plotHeight, generationCounter,
      allowMultitexturing));
...
GrDrawOpAtlas::Plot::Plot(
    int pageIndex, int plotIndex, GenerationCounter* generationCounter, int offX, int offY,
...
GrDrawOpAtlas::Plot::~Plot() { sk_free(fData); }
...
void GrDrawOpAtlas::Plot::uploadToTexture(
    GrDeferredTextureUploadWritePixelsFn& writePixels, GrTextureProxy* proxy) {
  ...
  writePixels(proxy, fDirtyRect.makeOffset(fOffset.fX, fOffset.fY), fColorType, dataPtr, rowBytes);
  fDirtyRect.setEmpty();
...
inline bool GrDrawOpAtlas::updatePlot(
    GrDeferredUploadTarget* target, AtlasLocator* atlasLocator, Plot* plot) {
  ...
  if (plot->lastUploadToken() < target->tokenTracker()->nextTokenToFlush()) {
    ...
    GrDeferredUploadToken lastUploadToken =
        target->addASAPUpload([plotsp, proxy](GrDeferredTextureUploadWritePixelsFn& writePixels) {
...
bool GrDrawOpAtlas::uploadToPage(
    unsigned int pageIdx, GrDeferredUploadTarget* target, int width, int height, const void* image,
...
GrDrawOpAtlas::ErrorCode GrDrawOpAtlas::addToAtlas(
  ...
  if (fNumActivePages == this->maxPages()) {
    for (unsigned int pageIdx = 0; pageIdx < fNumActivePages; ++pageIdx) {
      ...
      if (plot->lastUseToken() < target->tokenTracker()->nextTokenToFlush()) {
        ...
        SkASSERT(
            GrBackendFormatBytesPerPixel(fViews[pageIdx].proxy()->backendFormat()) == plot->bpp());
        SkDEBUGCODE(bool verify =)
        plot->addSubImage(width, height, image, atlasLocator);
        SkASSERT(verify);
  ...
  SkASSERT(
      GrBackendFormatBytesPerPixel(fViews[pageIdx].proxy()->backendFormat()) == newPlot->bpp());
  SkDEBUGCODE(bool verify =)
  newPlot->addSubImage(width, height, image, atlasLocator);
  SkASSERT(verify);
  ...
  GrDeferredUploadToken lastUploadToken =
      target->addInlineUpload([plotsp, proxy](GrDeferredTextureUploadWritePixelsFn& writePixels) {
        plotsp->uploadToTexture(writePixels, proxy);
      });
  ...
  for (uint32_t i = 0; i < this->maxPages(); ++i) {
    ...
    for (int y = numPlotsY - 1, r = 0; y >= 0; --y, ++r) {
      for (int x = numPlotsX - 1, c = 0; x >= 0; --x, ++c) {
        ...
        currPlot->reset(
            new Plot(i, plotIndex, generationCounter, x, y, fPlotWidth, fPlotHeight, fColorType));
...
GrDrawOpAtlasConfig::GrDrawOpAtlasConfig(int maxTextureSize, size_t maxBytes) {
  ...
  int index =
      maxBytes > 0 ? SkTPin<int>(SkPrevLog2(maxBytes), 0, SK_ARRAY_COUNT(kARGBDimensions) - 1) : 0;
