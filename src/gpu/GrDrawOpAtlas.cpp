...
GrDrawOpAtlas::Plot::Plot(
    int pageIndex, int plotIndex, uint64_t genID, int offX, int offY, int width, int height,
...
bool GrDrawOpAtlas::uploadToPage(
    const GrCaps& caps, unsigned int pageIdx, AtlasID* id, GrDeferredUploadTarget* target,
...
GrDrawOpAtlas::ErrorCode GrDrawOpAtlas::addToAtlas(
    GrResourceProvider* resourceProvider, AtlasID* id, GrDeferredUploadTarget* target, int width,
  ...
  for (uint32_t i = 0; i < this->maxPages(); ++i) {
    ...
    for (int y = numPlotsY - 1, r = 0; y >= 0; --y, ++r) {
      for (int x = numPlotsX - 1, c = 0; x >= 0; --x, ++c) {
        ...
        currPlot->reset(new Plot(i, plotIndex, 1, x, y, fPlotWidth, fPlotHeight, fColorType));
