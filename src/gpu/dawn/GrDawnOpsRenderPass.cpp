...
static dawn::LoadOp to_dawn_load_op(GrLoadOp loadOp) {
  switch (loadOp) {
    case GrLoadOp::kLoad: return dawn::LoadOp::Load;
    ...
    case GrLoadOp::kClear: return dawn::LoadOp::Clear;
    default: SK_ABORT("Invalid LoadOp");
  }
}

GrDawnOpsRenderPass::GrDawnOpsRenderPass(
    GrDawnGpu* gpu, GrRenderTarget* rt, GrSurfaceOrigin origin, const LoadAndStoreInfo& colorInfo,
    const StencilLoadAndStoreInfo& stencilInfo)
    : INHERITED(rt, origin), fGpu(gpu), fColorInfo(colorInfo) {
...
dawn::RenderPassEncoder GrDawnOpsRenderPass::beginRenderPass(
    dawn::LoadOp colorOp, dawn::LoadOp stencilOp) {
...
void GrDawnOpsRenderPass::end() { fPassEncoder.EndPass(); }

void GrDawnOpsRenderPass::submit() { fGpu->appendCommandBuffer(fEncoder.Finish()); }

void GrDawnOpsRenderPass::insertEventMarker(const char* msg) { SkASSERT(!"unimplemented"); }
...
void GrDawnOpsRenderPass::inlineUpload(GrOpFlushState* state, GrDeferredTextureUploadFn& upload) {
...
void GrDawnOpsRenderPass::setScissorState(
  ...
  if (pipeline.isScissorEnabled()) {
    ...
    if (kBottomLeft_GrSurfaceOrigin == fOrigin) {
      rect.setXYWH(rect.x(), fRenderTarget->height() - rect.bottom(), rect.width(), rect.height());
    }
  } else {
...
void GrDawnOpsRenderPass::applyState(
    ...
    bool hasPoints) {
  sk_sp<GrDawnProgram> program = fGpu->getOrCreateRenderPipeline(
      fRenderTarget, fOrigin, pipeline, primProc, primProcProxies, hasPoints, primitiveType);
  auto bindGroup =
      program->setData(fGpu, fRenderTarget, fOrigin, primProc, pipeline, primProcProxies);
...
void GrDawnOpsRenderPass::onDraw(
  ...
  for (int i = 0; i < meshCount; ++i) {
    applyState(
        pipeline, primProc, primProcProxies, fixedDynamicState, dynamicStateArrays,
        meshes[0].primitiveType(), hasPoints);
    meshes[i].sendToGpu(this);
  }
}

void GrDawnOpsRenderPass::sendInstancedMeshToGpu(
...
void GrDawnOpsRenderPass::sendIndexedInstancedMeshToGpu(
