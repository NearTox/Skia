...
GrDawnTexture::GrDawnTexture(
    GrDawnGpu* gpu, const SkISize& size, GrPixelConfig config, dawn::TextureView textureView,
    const GrDawnImageInfo& info, GrMipMapsStatus mipMapsStatus)
...
sk_sp<GrDawnTexture> GrDawnTexture::Make(
    GrDawnGpu* gpu, const SkISize& size, GrPixelConfig config, dawn::TextureFormat format,
    GrRenderable renderable, int sampleCnt, SkBudgeted budgeted, int mipLevels,
    GrMipMapsStatus status) {
  bool renderTarget = renderable == GrRenderable::kYes;
  dawn::TextureDescriptor textureDesc;

  textureDesc.usage = dawn::TextureUsageBit::Sampled | dawn::TextureUsageBit::CopySrc |
  ...
  if (renderTarget) {
    result = sk_sp<GrDawnTextureRenderTarget>(
        new GrDawnTextureRenderTarget(gpu, size, config, textureView, sampleCnt, info, status));
  } else {
    result = sk_sp<GrDawnTexture>(new GrDawnTexture(gpu, size, config, textureView, info, status));
  }
...
sk_sp<GrDawnTexture> GrDawnTexture::MakeWrapped(
    GrDawnGpu* gpu, const SkISize& size, GrPixelConfig config, GrRenderable renderable,
    int sampleCnt, GrMipMapsStatus status, GrWrapCacheable cacheable, const GrDawnImageInfo& info) {
...
void GrDawnTexture::onRelease() { INHERITED::onRelease(); }

void GrDawnTexture::onAbandon() { INHERITED::onAbandon(); }

GrBackendTexture GrDawnTexture::getBackendTexture() const {
  return GrBackendTexture(this->width(), this->height(), fInfo);
}

void GrDawnTexture::upload(
    const GrMipLevel texels[], int mipLevels, dawn::CommandEncoder copyEncoder) {
  this->upload(texels, mipLevels, SkIRect::MakeWH(width(), height()), copyEncoder);
}

void GrDawnTexture::upload(
    const GrMipLevel texels[], int mipLevels, const SkIRect& rect,
  ...
  for (int i = 0; i < mipLevels; i++) {
    ...
    SkPixmap pixmap;
    const char* src;
    if (kRGBA_4444_GrPixelConfig == this->config() || kRGB_565_GrPixelConfig == this->config() ||
    ...
    if (rowBytes == origRowBytes) {
      stagingBuffer.SetSubData(0, size, static_cast<const uint8_t*>(static_cast<const void*>(src)));
    } else {
      ...
      stagingBuffer.SetSubData(0, size, static_cast<const uint8_t*>(static_cast<const void*>(buf)));
      delete[] buf;
    }
