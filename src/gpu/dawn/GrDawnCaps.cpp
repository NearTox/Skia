...
bool GrDawnCaps::isConfigTexturable(GrPixelConfig config) const {
  switch (config) {
    ...
    case kAlpha_8_GrPixelConfig: return true;
...
GrPixelConfig GrDawnCaps::getYUVAConfigFromBackendFormat(
  ...
  switch (*format) {
    case dawn::TextureFormat::R8Unorm: return kAlpha_8_as_Red_GrPixelConfig; break;
    case dawn::TextureFormat::RGBA8Unorm: return kRGBA_8888_GrPixelConfig; break;
    case dawn::TextureFormat::BGRA8Unorm: return kBGRA_8888_GrPixelConfig; break;
...
int GrDawnCaps::getRenderTargetSampleCount(
    int requestedCount, GrColorType ct, const GrBackendFormat& format) const {
  GrPixelConfig config = this->getConfigFromBackendFormat(format, ct);
  if (kUnknown_GrPixelConfig == config) {
    return 0;
  }
