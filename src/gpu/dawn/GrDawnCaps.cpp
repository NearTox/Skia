...
bool GrDawnCaps::isFormatCompressed(
...
GrPixelConfig GrDawnCaps::onGetConfigFromBackendFormat(
  ...
  switch (colorType) {
    ...
    case GrColorType::kRGBA_8888:
      ...
      break;
    case GrColorType::kRGB_888x: break;
...
bool GrDawnCaps::isFormatTexturableAndUploadable(
  ...
  switch (ct) {
    case GrColorType::kAlpha_8: return wgpu::TextureFormat::R8Unorm == dawnFormat;
    case GrColorType::kRGBA_8888:
    case GrColorType::kRGB_888x:
    case GrColorType::kBGRA_8888:
...
bool GrDawnCaps::isFormatRenderable(const GrBackendFormat& format, int sampleCount) const {
  wgpu::TextureFormat dawnFormat;
...
int GrDawnCaps::getRenderTargetSampleCount(
    int requestedCount, const GrBackendFormat& backendFormat) const {
  wgpu::TextureFormat dawnFormat;
  if (!backendFormat.asDawnFormat(&dawnFormat)) {
    return 0;
  }
...
GrBackendFormat GrDawnCaps::onGetDefaultBackendFormat(
...
GrBackendFormat GrDawnCaps::getBackendFormatFromCompressionType(
    SkImage::CompressionType type) const {
  return GrBackendFormat();
}

GrSwizzle GrDawnCaps::getTextureSwizzle(
    const GrBackendFormat& format, GrColorType colorType) const {
...
bool GrDawnCaps::onAreColorTypeAndFormatCompatible(
    GrColorType ct, const GrBackendFormat& format) const {
  return true;
}

GrColorType GrDawnCaps::getYUVAColorTypeFromBackendFormat(
    const GrBackendFormat& backendFormat, bool isAlphaChannel) const {
...
GrProgramDesc GrDawnCaps::makeDesc(
