...
GrDawnRenderTarget::GrDawnRenderTarget(
    GrDawnGpu* gpu, const GrSurfaceDesc& desc, int sampleCnt, const GrDawnImageInfo& info,
...
GrDawnRenderTarget* GrDawnRenderTarget::Create(
    GrDawnGpu* gpu, const GrSurfaceDesc& desc, int sampleCnt, const GrDawnImageInfo& info,
    GrBackendObjectOwnership ownership) {
...
sk_sp<GrDawnRenderTarget> GrDawnRenderTarget::MakeWrapped(
  ...
  return sk_sp<GrDawnRenderTarget>(
      GrDawnRenderTarget::Create(gpu, desc, sampleCnt, info, GrBackendObjectOwnership::kBorrowed));
}
...
void GrDawnRenderTarget::onAbandon() { INHERITED::onAbandon(); }

GrBackendRenderTarget GrDawnRenderTarget::getBackendRenderTarget() const {
  return GrBackendRenderTarget(
      this->width(), this->height(), this->numSamples(), this->numSamples(), fInfo);
}
