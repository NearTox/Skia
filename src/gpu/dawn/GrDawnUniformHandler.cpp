...
static inline uint32_t grsltype_to_size(GrSLType type) {
  switch (type) {
    ...
    case kUByte2_GrSLType: return 2;
    case kByte3_GrSLType:
    case kUByte3_GrSLType: return 3;
    case kByte4_GrSLType:
    case kUByte4_GrSLType: return 4;
...
GrGLSLUniformHandler::SamplerHandle GrDawnUniformHandler::addSampler(
  ...
  UniformInfo& info = fSamplers.push_back(GrDawnUniformHandler::UniformInfo{
      GrShaderVar{
          std::move(mangleName), kSampler_GrSLType, GrShaderVar::TypeModifier::Uniform,
  ...
  UniformInfo& texInfo = fTextures.push_back(GrDawnUniformHandler::UniformInfo{
      GrShaderVar{
          std::move(mangleTexName), kTexture2D_GrSLType, GrShaderVar::TypeModifier::Uniform,
          GrShaderVar::kNonArray, std::move(texLayoutQualifier), SkString()},
      0, kFragment_GrShaderFlag});

  SkString reference;
  reference.printf(
      "makeSampler2D(%s, %s)", texInfo.fVar.getName().c_str(), info.fVar.getName().c_str());
