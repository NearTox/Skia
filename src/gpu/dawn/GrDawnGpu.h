...
class GrDawnGpu : public GrGpu {
 public:
  ...
  sk_sp<GrDawnProgram> getOrCreateRenderPipeline(
      GrRenderTarget*, GrSurfaceOrigin origin, const GrPipeline&, const GrPrimitiveProcessor&,
      const GrTextureProxy* const* primProcProxies, bool hasPoints, GrPrimitiveType primitiveType);
 ...
 private:
  ...
  sk_sp<GrTexture> onCreateTexture(
      ...
      SkBudgeted, GrProtected, const GrMipLevel texels[], int mipLevelCount) override;
  ...
  bool onWritePixels(
      GrSurface* surface, int left, int top, int width, int height, GrColorType surfaceColorType,
      GrColorType dstColorType, const GrMipLevel texels[], int mipLevelCount) override;
