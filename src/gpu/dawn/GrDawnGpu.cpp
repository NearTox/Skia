...
GrDawnGpu::~GrDawnGpu() { this->waitOnAllBusyStagingBuffers(); }
...
bool GrDawnGpu::onWritePixels(
  ...
  this->uploadTextureData(
      srcColorType, texels, mipLevelCount, SkIRect::MakeXYWH(left, top, width, height),
      texture->texture());
...
bool GrDawnGpu::onUpdateBackendTexture(
...
GrBackendRenderTarget GrDawnGpu::createTestingOnlyBackendRenderTarget(
  ...
  wgpu::TextureDescriptor desc;
  desc.usage = wgpu::TextureUsage::CopySrc | wgpu::TextureUsage::OutputAttachment;
...
void GrDawnGpu::testingOnly_flushGpuAndSync() { this->submitToGpu(true); }
...
bool GrDawnGpu::onRegenerateMipMapLevels(GrTexture* tex) {
  ...
  const char* vs =
      ...
      "}\n";
  SkSL::String vsSPIRV = this->SkSLToSPIRV(vs, SkSL::Program::kVertex_Kind, false, 0, nullptr);
  ...
  const char* fs =
      ...
      "}\n";
  SkSL::String fsSPIRV = this->SkSLToSPIRV(fs, SkSL::Program::kFragment_Kind, false, 0, nullptr);
...
SkSL::String GrDawnGpu::SkSLToSPIRV(
    const char* shaderString, SkSL::Program::Kind kind, bool flipY, uint32_t rtHeightOffset,
