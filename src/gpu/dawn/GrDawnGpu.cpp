...
class Fence {
 public:
  Fence(const wgpu::Device& device) : fDevice(device), fCalled(false) {
...
bool GrDawnGpu::onUpdateCompressedBackendTexture(
    const GrBackendTexture&, sk_sp<GrRefCntedCallback> finishedCallback, const void* data,
    size_t size) {
  return false;
}
...
bool GrDawnGpu::onRegenerateMipMapLevels(GrTexture* tex) {
  ...
  const char* vs =
      ...
      "}\n";
  SkSL::String vsSPIRV = this->SkSLToSPIRV(
  ...
  const char* fs =
      ...
      "}\n";
  SkSL::String fsSPIRV = this->SkSLToSPIRV(
...
bool GrDawnGpu::waitFence(GrFence fence) { return reinterpret_cast<Fence*>(fence)->check(); }

void GrDawnGpu::deleteFence(GrFence fence) const { delete reinterpret_cast<Fence*>(fence); }
...
void GrDawnGpu::finishOutstandingGpuWork() { this->waitOnAllBusyStagingBuffers(); }
...
SkSL::String GrDawnGpu::SkSLToSPIRV(
