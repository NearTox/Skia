...
GrDawnProgramDataManager::GrDawnProgramDataManager(
    const UniformInfoArray& uniforms, uint32_t uniformBufferSize)
    : fUniformBufferSize(uniformBufferSize), fUniformsDirty(false) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    SkDEBUGCODE(uniform.fArrayCount = uniformInfo.fVar.getArrayCount();
                uniform.fType = uniformInfo.fVar.getType();) uniform.fOffset =
        uniformInfo.fUBOOffset;
  }
}
...
void GrDawnProgramDataManager::set4i(
...
void GrDawnProgramDataManager::set4f(
...
void GrDawnProgramDataManager::setMatrix2fv(
...
void GrDawnProgramDataManager::setMatrix3fv(
    UniformHandle u, int arrayCount, const float m[]) const {
...
void GrDawnProgramDataManager::setMatrix4fv(
    UniformHandle u, int arrayCount, const float m[]) const {
...
template <int N>
inline void GrDawnProgramDataManager::setMatrices(
...
void GrDawnProgramDataManager::uploadUniformBuffers(
    GrDawnGpu* gpu, GrDawnRingBuffer::Slice slice) const {
