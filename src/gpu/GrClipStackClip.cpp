...
bool GrClipStackClip::quickContains(const SkRect& rect) const noexcept {
...
bool GrClipStackClip::quickContains(const SkRRect& rrect) const noexcept {
  if (!fStack || fStack->isWideOpen()) {
    return true;
  }
...
bool GrClipStackClip::isRRect(const SkRect& origRTBounds, SkRRect* rr, GrAA* aa) const noexcept {
  if (!fStack) {
    return false;
  }
...
SkIRect GrClipStackClip::getConservativeBounds(int width, int height) const noexcept {
  if (fStack) {
    SkRect devBounds;
...
static std::unique_ptr<GrFragmentProcessor> create_fp_for_mask(
    GrSurfaceProxyView mask, const SkIRect& devBound, const GrCaps& caps) {
...
bool GrClipStackClip::PathNeedsSWRenderer(
    ...
    const Element* element, GrPathRenderer** prOut, bool needsStencil) {
...
bool GrClipStackClip::apply(
    GrRecordingContext* context, GrRenderTargetContext* renderTargetContext, bool useHWAA,
  ...
  GrReducedClip reducedClip(
      *fStack, devBounds, context->priv().caps(), maxWindowRectangles, maxAnalyticFPs,
      ccpr ? maxAnalyticFPs : 0);
  if (InitialState::kAllOut == reducedClip.initialState() && reducedClip.maskElements().isEmpty()) {
    return false;
  }
  ...
  if (!reducedClip.windowRectangles().empty()) {
    out->hardClip().addWindowRectangles(
        reducedClip.windowRectangles(), GrWindowRectsState::Mode::kExclusive);
  }
...
bool GrClipStackClip::applyClipMask(
  ...
  if ((renderTargetContext->numSamples() <= 1 && reducedClip.maskRequiresAA()) ||
    ...
    if (result) {
      ...
      out->addCoverageFP(
          create_fp_for_mask(std::move(result), reducedClip.scissor(), *context->priv().caps()));
      return true;
    }
...
static void create_clip_mask_key(
    uint32_t clipGenID, const SkIRect& bounds, int numAnalyticFPs, GrUniqueKey* key) {
...
static void add_invalidate_on_pop_message(
    GrRecordingContext* context, const SkClipStack& stack, uint32_t clipGenID,
...
GrSurfaceProxyView GrClipStackClip::createAlphaClipMask(
    GrRecordingContext* context, const GrReducedClip& reducedClip) const {
  GrProxyProvider* proxyProvider = context->priv().proxyProvider();
  GrUniqueKey key;
...
class ClipMaskData {
 public:
  ClipMaskData(const GrReducedClip& reducedClip)
      : fScissor(reducedClip.scissor()), fInitialState(reducedClip.initialState()) {
    for (ElementList::Iter iter(reducedClip.maskElements()); iter.get(); iter.next()) {
      fElements.addToTail(*iter.get());
    }
  }

  const SkIRect& scissor() const noexcept { return fScissor; }
  InitialState initialState() const noexcept { return fInitialState; }
  const ElementList& elements() const noexcept { return fElements; }
...
static void draw_clip_elements_to_mask_helper(
    GrSWMaskHelper& helper, const ElementList& elements, const SkIRect& scissor,
    ...
    auto drawAndUploadMask = [uploaderRaw, maskSpaceIBounds] {
      ...
      if (helper.init(maskSpaceIBounds)) {
        draw_clip_elements_to_mask_helper(
            helper, uploaderRaw->data().elements(), uploaderRaw->data().scissor(),
