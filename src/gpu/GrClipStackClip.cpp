...
GrClip::Effect GrClipStackClip::apply(
  ...
  GrReducedClip reducedClip(
      *fStack, devBounds, context->priv().caps(), maxWindowRectangles, maxAnalyticElements,
      ccpr ? maxAnalyticElements : 0);
...
static void create_clip_mask_key(
    uint32_t clipGenID, const SkIRect& bounds, int numAnalyticElements, GrUniqueKey* key) {
