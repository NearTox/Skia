...
GrImageTextureMaker::GrImageTextureMaker(
    ...
    : INHERITED(
          context, client->width(), client->height(), client->imageInfo().colorInfo(), useDecal),
      fImage(static_cast<const SkImage_Lazy*>(client)),
      fCachingHint(chint) {
  SkASSERT(client->isLazyGenerated());
  GrMakeKeyFromImageID(
      &fOriginalKey, client->uniqueID(), SkIRect::MakeWH(this->width(), this->height()));
}
...
std::unique_ptr<GrFragmentProcessor> GrYUVAImageTextureMaker::createFragmentProcessor(
  ...
  auto fp = GrYUVtoRGBEffect::Make(
      fImage->fProxies, fImage->fYUVAIndices, fImage->fYUVColorSpace, filter, textureMatrix,
      domain);
