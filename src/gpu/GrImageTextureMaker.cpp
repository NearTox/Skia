...
GrImageTextureMaker::GrImageTextureMaker(
    GrRecordingContext* context, const SkImage* client, GrImageTexGenPolicy texGenPolicy)
...
std::unique_ptr<GrFragmentProcessor> GrYUVAImageTextureMaker::createFragmentProcessor(
  ...
  if (fImage->fRGBView.proxy()) {
    return this->INHERITED::createFragmentProcessor(textureMatrix, subset, domain, samplerState);
  }
  ...
  auto fp = GrYUVtoRGBEffect::Make(
      fImage->fYUVAProxies, samplerState, caps, textureMatrix, subset, domain);
  if (fImage->fFromColorSpace) {
    fp = GrColorSpaceXformEffect::Make(
        std::move(fp), fImage->fFromColorSpace.get(), fImage->alphaType(), fImage->colorSpace(),
        kPremul_SkAlphaType);
  }
  return fp;
}
...
std::unique_ptr<GrFragmentProcessor> GrYUVAImageTextureMaker::createBicubicFragmentProcessor(
  ...
  auto fp = GrYUVtoRGBEffect::Make(
      fImage->fYUVAProxies, samplerState, caps, SkMatrix::I(), subset, domain);
  fp = GrBicubicEffect::Make(
      std::move(fp), fImage->alphaType(), textureMatrix, kernel, GrBicubicEffect::Direction::kXY);
  if (fImage->fFromColorSpace) {
    fp = GrColorSpaceXformEffect::Make(
        std::move(fp), fImage->fFromColorSpace.get(), fImage->alphaType(), fImage->colorSpace(),
        kPremul_SkAlphaType);
