...
class GrTextureProducer : public SkNoncopyable {
 ...
 protected:
  ...
  /**
   *  If we need to make a copy in order to be compatible with GrTextureParams producer is asked
   * to return a key that identifies its original content + the CopyParms parameter. If the
   * producer does not want to cache the stretched version (e.g. the producer is volatile), this
   * should simply return without initializing the copyKey. If the texture generated by this
   * producer depends on the destination color space, then that information should also be
