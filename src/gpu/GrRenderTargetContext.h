...
class GrRenderTargetContext : public GrSurfaceContext, public GrOpsTaskClosedObserver {
 public:
  ...
  static std::unique_ptr<GrRenderTargetContext> Make(
      ...
      const GrBackendFormat&, int sampleCnt, GrMipmapped, GrProtected, GrSurfaceOrigin, SkBudgeted,
  ...
  static std::tuple<GrColorType, GrBackendFormat> GetFallbackColorTypeAndFormat(
      GrImageContext*, GrColorType, int sampleCnt);
   ...
   */
  void clear(const SkIRect& rect, const SkPMColor4f& color) { this->internalClear(&rect, color); }
  // Clears the entire render target to the color.
  void clear(const SkPMColor4f& color) { this->internalClear(nullptr, color); }
 ...
 private:
  ...
  void internalClear(const SkIRect* scissor, const SkPMColor4f&, bool upgradePartialToFull = false);
