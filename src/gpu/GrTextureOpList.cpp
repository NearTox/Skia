...
GrTextureOpList::GrTextureOpList(
    sk_sp<GrOpMemoryPool> opMemoryPool, sk_sp<GrTextureProxy> proxy, GrAuditTrail* auditTrail)
...
GrTextureOpList::~GrTextureOpList() { this->deleteOps(); }
...
void GrTextureOpList::onPrepare(GrOpFlushState* flushState) {
  ...
  for (int i = 0; i < fRecordedOps.count(); ++i) {
    if (fRecordedOps[i]) {
      ...
      GrOpFlushState::OpArgs opArgs = {fRecordedOps[i].get(), nullptr, nullptr, GrSwizzle(),
...
bool GrTextureOpList::onExecute(GrOpFlushState* flushState) {
  ...
  GrGpuTextureCommandBuffer* commandBuffer(
      flushState->gpu()->getCommandBuffer(fTarget->peekTexture(), fTarget->origin()));
...
bool GrTextureOpList::copySurface(
...
void GrTextureOpList::gatherProxyIntervals(GrResourceAllocator* alloc) const {
  ...
  if (fRecordedOps.count()) {
    ...
    alloc->addInterval(
        fTarget.get(), cur, cur + fRecordedOps.count() - 1, GrResourceAllocator::ActualUse::kYes);
  } else {
  ...
  auto gather = [alloc SkDEBUGCODE(, this)](GrSurfaceProxy* p, GrMipMapped) {
    alloc->addInterval(
        ...
        GrResourceAllocator::ActualUse::kYes SkDEBUGCODE(, p == fTarget.get()));
  };
  for (int i = 0; i < fRecordedOps.count(); ++i) {
    const GrOp* op = fRecordedOps[i].get();  // only diff from the GrRenderTargetOpList version
    if (op) {
...
void GrTextureOpList::recordOp(std::unique_ptr<GrOp> op) {
  ...
  GrOP_INFO(
      ...
      "\tBounds LRTB (%f, %f, %f, %f)\n",
      op->name(), op->uniqueID(), op->bounds().fLeft, op->bounds().fRight, op->bounds().fTop,
      op->bounds().fBottom);
