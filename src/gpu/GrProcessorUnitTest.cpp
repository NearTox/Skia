...
GrProcessorTestData::GrProcessorTestData(
    SkRandom* random, GrRecordingContext* context, int maxTreeDepth, int numViews,
    const ViewInfo views[])
    : GrProcessorTestData(
          random, context, maxTreeDepth, numViews, views,
          /*inputFP=*/nullptr) {}

GrProcessorTestData::GrProcessorTestData(
    SkRandom* random, GrRecordingContext* context, int maxTreeDepth, int numViews,
    const ViewInfo views[], std::unique_ptr<GrFragmentProcessor> inputFP)
    : fRandom(random),
...
template <class ProcessorSmartPtr>
ProcessorSmartPtr GrProcessorTestFactory<ProcessorSmartPtr>::MakeIdx(
    int idx, GrProcessorTestData* data) {
...
std::unique_ptr<GrFragmentProcessor> GrProcessorUnitTest::MakeChildFP(GrProcessorTestData* data) {
  ...
  if (data->fCurrentTreeDepth > data->fMaxTreeDepth) {
    ...
    for (;;) {
      ...
      if (data->fCurrentTreeDepth < data->fMaxTreeDepth || fp->numNonNullChildProcessors() == 0) {
        break;
      }
    }
  }
