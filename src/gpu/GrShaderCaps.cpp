...
void GrShaderCaps::dumpJSON(SkJSONWriter* writer) const {
  ...
  writer->appendBool(
      "Must force negated ldexp param to multiply", fMustForceNegatedLdexpParamToMultiply);
...
void GrShaderCaps::applyOptionsOverrides(const GrContextOptions& options) {
  ...
  if (options.fMaxTessellationSegmentsOverride > 0) {
    fMaxTessellationSegments =
        std::min(options.fMaxTessellationSegmentsOverride, fMaxTessellationSegments);
  }
#endif
}
