...
GrRenderTask::GrRenderTask(sk_sp<GrSurfaceProxy> target)
    : fTarget(std::move(target)), fUniqueID(CreateUniqueID()), fFlags(0) {}
...
void GrRenderTask::addDependency(
  ...
  if (dependedOnTask == this) {
    ...
    SkASSERT(
        !dependedOn->asTextureProxy() || !dependedOn->asTextureProxy()->texPriv().isDeferred());
    return;
  }
...
void GrRenderTask::addDependent(GrRenderTask* dependent) { fDependents.push_back(dependent); }
...
bool GrRenderTask::isInstantiated() const {
  ...
  int minStencilSampleCount =
      (fTarget->asRenderTargetProxy()) ? fTarget->asRenderTargetProxy()->numStencilSamples() : 0;
...
void GrRenderTask::dump(bool printDependencies) const {
  ...
  SkDebugf(
      ...
      fTarget && fTarget->peekSurface() ? fTarget->peekSurface()->uniqueID().asUInt() : -1);
