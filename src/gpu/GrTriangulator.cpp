...
struct VertexList {
  ...
  void remove(Vertex* v) { list_remove<Vertex, &Vertex::fPrev, &Vertex::fNext>(v, &fHead, &fTail); }
  void close() {
    if (fHead && fTail) {
      fTail->fNext = fHead;
      fHead->fPrev = fTail;
...
struct EdgeList {
  ...
  void remove(Edge* edge) { list_remove<Edge, &Edge::fLeft, &Edge::fRight>(edge, &fHead, &fTail); }
  void removeAll() {
    while (fHead) {
      this->remove(fHead);
    }
...
void create_event(
    SSEdge* edge, Vertex* v, SSEdge* other, Vertex* dest, EventList* events, Comparator& c,
    SkArenaAlloc& alloc) {
  if (!v->fPartner) {
    return;
  }
...
struct Poly {
  ...
  struct MonotonePoly {
    ...
    void* emitTriangle(
        Vertex* prev, Vertex* curr, Vertex* next, bool emitCoverage, void* data) const {
      if (fWinding < 0) {
...
void append_quadratic_to_contour(
    const SkPoint pts[3], SkScalar toleranceSqd, VertexList* contour, SkArenaAlloc& alloc) {
  SkQuadCoeff quad(pts);
...
void path_to_contours(
    const SkPath& path, SkScalar tolerance, const SkRect& clipBounds, VertexList* contours,
    SkArenaAlloc& alloc, Mode mode, int* numCountedCurves) {
    ...
    switch (verb) {
      ...
      case SkPath::kCubic_Verb: {
        ...
        generate_cubic_points(
            pts[0], pts[1], pts[2], pts[3], toleranceSqd, contour, pointsLeft, alloc);
        break;
      }
      case SkPath::kClose_Verb:
      case SkPath::kDone_Verb: break;
    }
  }
...
inline bool apply_fill_type(SkPathFillType fillType, int winding) {
  switch (fillType) {
    case SkPathFillType::kWinding: return winding != 0;
    case SkPathFillType::kEvenOdd: return (winding & 1) != 0;
    case SkPathFillType::kInverseWinding: return winding == 1;
    case SkPathFillType::kInverseEvenOdd: return (winding & 1) == 1;
  ...
  if (edge->fRight) {
    ...
    if (c.sweep_lt(rightTop->fPoint, top->fPoint) && !edge->fRight->isRightOf(top)) {
      ...
      rewind(activeEdges, current, top, c);
        ...
        c.sweep_lt(bottom->fPoint, rightBottom->fPoint) && !edge->fRight->isRightOf(bottom)) {
      rewind(activeEdges, current, rightTop, c);
...
void merge_edges_above(
    Edge* edge, Edge* other, EdgeList* activeEdges, Vertex** current, Comparator& c) {
...
bool split_edge(
    Edge* edge, Vertex* v, EdgeList* activeEdges, Vertex** current, Comparator& c,
    SkArenaAlloc& alloc) {
...
bool intersect_edge_pair(
    Edge* left, Edge* right, EdgeList* activeEdges, Vertex** current, Comparator& c,
    SkArenaAlloc& alloc) {
...
Edge* connect(
    Vertex* prev, Vertex* next, Edge::Type type, Comparator& c, SkArenaAlloc& alloc,
    int winding_scale = 1) {
...
void merge_vertices(
    Vertex* src, Vertex* dst, VertexList* mesh, Comparator& c, SkArenaAlloc& alloc) {
  TESS_LOG(
...
Vertex* create_sorted_vertex(
    const SkPoint& p, uint8_t alpha, VertexList* mesh, Vertex* reference, Comparator& c,
    SkArenaAlloc& alloc) {
...
bool check_for_intersection(
    Edge* left, Edge* right, EdgeList* activeEdges, Vertex** current, VertexList* mesh,
    ...
    for (Vertex* v = contour->fHead; v;) {
      ...
      if (coincident(prev->fPoint, v->fPoint)) {
        ...
        contour->remove(v);
          ...
          removeCollinearVertices && Line(prev->fPoint, nextWrap->fPoint).dist(v->fPoint) == 0.0) {
...
void build_edges(
    VertexList* contours, int contourCnt, VertexList* mesh, Comparator& c, SkArenaAlloc& alloc) {
  for (VertexList* contour = contours; contourCnt > 0; --contourCnt, ++contour) {
    Vertex* prev = contour->fTail;
    for (Vertex* v = contour->fHead; v;) {
      Vertex* next = v->fNext;
...
void dump_skel(const SSEdgeList& ssEdges) {
  ...
  for (SSEdge* edge : ssEdges) {
    if (edge->fEdge) {
      TESS_LOG("skel edge %g -> %g", edge->fPrev->fVertex->fID, edge->fNext->fVertex->fID);
      if (edge->fEdge->fTop && edge->fEdge->fBottom) {
        TESS_LOG(" (original %g -> %g)\n", edge->fEdge->fTop->fID, edge->fEdge->fBottom->fID);
      } else {
        TESS_LOG("\n");
      }
    }
...
bool connected(Vertex* v) { return v->fFirstEdgeAbove || v->fFirstEdgeBelow; }

enum class SimplifyResult { kAlreadySimple, kFoundSelfIntersection, kAbort };
    ...
    do {
      ...
      if (v->fFirstEdgeBelow) {
        for (Edge* edge = v->fFirstEdgeBelow; edge; edge = edge->fNextEdgeBelow) {
          if (check_for_intersection(leftEnclosingEdge, edge, &activeEdges, &v, mesh, c, alloc) ||
              check_for_intersection(edge, rightEnclosingEdge, &activeEdges, &v, mesh, c, alloc)) {
        ...
        if (check_for_intersection(
                leftEnclosingEdge, rightEnclosingEdge, &activeEdges, &v, mesh, c, alloc)) {
          if (Mode::kSimpleInnerPolygons == mode) {
            return SimplifyResult::kAbort;
          }
          result = SimplifyResult::kFoundSelfIntersection;
...
Poly* tessellate(
    SkPathFillType fillType, Mode mode, const VertexList& vertices, SkArenaAlloc& alloc) {
        ...
        if (leftPoly && rightPoly) {
          if (leftPoly == rightPoly) {
            if (leftPoly->fTail && leftPoly->fTail->fSide == Poly::kLeft_Side) {
              leftPoly = new_poly(&polys, leftPoly->lastVertex(), leftPoly->fWinding, alloc);
              leftEnclosingEdge->fRightPoly = leftPoly;
            } else {
              rightPoly = new_poly(&polys, rightPoly->lastVertex(), rightPoly->fWinding, alloc);
...
void remove_non_boundary_edges(
    const VertexList& mesh, SkPathFillType fillType, SkArenaAlloc& alloc) {
  ...
  for (Vertex* v = mesh.fHead; v != nullptr; v = v->fNext) {
    ...
    Edge* rightEnclosingEdge;
    find_enclosing_edges(v, &activeEdges, &leftEnclosingEdge, &rightEnclosingEdge);
    bool prevFilled = leftEnclosingEdge && apply_fill_type(fillType, leftEnclosingEdge->fWinding);
...
void get_edge_normal(const Edge* e, SkVector* normal) {
  normal->set(SkDoubleToScalar(e->fLine.fA), SkDoubleToScalar(e->fLine.fB));
}
...
void Event::apply(VertexList* mesh, Comparator& c, EventList* events, SkArenaAlloc& alloc) {
  ...
  TESS_LOG(
      ...
      fEdge->fEdge->fTop->fID, fEdge->fEdge->fBottom->fID, dest->fID, fPoint.fX, fPoint.fY, fAlpha);
  fEdge->fEdge = nullptr;
...
bool collapse_overlap_regions(
    VertexList* mesh, Comparator& c, SkArenaAlloc& alloc, EventComparator comp) {
  ...
  for (Vertex* v = mesh->fHead; v != nullptr; v = v->fNext) {
    ...
    for (Edge* e = v->fLastEdgeAbove; e && e != leftEnclosingEdge;) {
      ...
      bool isOuterBoundary =
          e->fType == Edge::Type::kOuter && (!prev || prev->fWinding == 0 || e->fWinding == 0);
      if (prev) {
        e->fWinding -= prev->fWinding;
      }
...
void stroke_boundary(
    EdgeList* boundary, VertexList* innerMesh, VertexList* outerMesh, Comparator& c,
    SkArenaAlloc& alloc) {
  TESS_LOG("\nstroking boundary\n");
    ...
    if (!prevEdge->fLine.nearParallel(e->fLine) && prevInner.intersect(inner, &innerPoint) &&
      ...
      if (cosAngle < -kCosMiterAngle) {
        ...
        if (prevNormal.cross(normal) > 0) {
          ...
          Line prevTangent(
              prevV->fPoint, prevV->fPoint + SkVector::Make(prevOuter.fA, prevOuter.fB));
          Line nextTangent(nextV->fPoint, nextV->fPoint + SkVector::Make(outer.fA, outer.fB));
          ...
          Line prevTangent(
              prevV->fPoint, prevV->fPoint + SkVector::Make(prevInner.fA, prevInner.fB));
          Line nextTangent(nextV->fPoint, nextV->fPoint + SkVector::Make(inner.fA, inner.fB));
        ...
        if (!innerPoint1.isFinite() || !innerPoint2.isFinite() || !outerPoint1.isFinite() ||
            !outerPoint2.isFinite()) {
          continue;
        }
        TESS_LOG(
            "inner (%g, %g), (%g, %g), ", innerPoint1.fX, innerPoint1.fY, innerPoint2.fX,
            innerPoint2.fY);
        TESS_LOG(
            "outer (%g, %g), (%g, %g)\n", outerPoint1.fX, outerPoint1.fY, outerPoint2.fX,
            outerPoint2.fY);
...
void extract_boundaries(
    const VertexList& inMesh, VertexList* innerVertices, VertexList* outerVertices,
    SkPathFillType fillType, Comparator& c, SkArenaAlloc& alloc) {
...
void contours_to_mesh(
    VertexList* contours, int contourCnt, Mode mode, VertexList* mesh, Comparator& c,
    SkArenaAlloc& alloc) {
#if LOGGING_ENABLED
...
Poly* contours_to_polys(
    VertexList* contours, int contourCnt, SkPathFillType fillType, const SkRect& pathBounds,
    Mode mode, VertexList* outerMesh, SkArenaAlloc& alloc) {
  Comparator c(
      pathBounds.width() > pathBounds.height() ? Comparator::Direction::kHorizontal
...
Poly* path_to_polys(
    const SkPath& path, SkScalar tolerance, const SkRect& clipBounds, int contourCnt,
    SkArenaAlloc& alloc, Mode mode, int* numCountedCurves, VertexList* outerMesh) {
  ...
  return contours_to_polys(
      contours.get(), contourCnt, path.getFillType(), path.getBounds(), mode, outerMesh, alloc);
}
...
int get_contour_count(const SkPath& path, SkScalar tolerance) {
  ...
  while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
    switch (verb) {
      ...
      case SkPath::kConic_Verb:
      case SkPath::kQuad_Verb:
      case SkPath::kCubic_Verb: hasPoints = true; break;
      default: break;
    }
    first = false;
  }
...
int PathToTriangles(
  ...
  Poly* polys = path_to_polys(
      path, tolerance, clipBounds, contourCnt, alloc, mode, numCountedCurves, &outerMesh);
  SkPathFillType fillType =
      (Mode::kEdgeAntialias == mode) ? SkPathFillType::kWinding : path.getFillType();
  ...
  int actualCount =
      static_cast<int>((static_cast<uint8_t*>(end) - static_cast<uint8_t*>(verts)) / vertexStride);
...
int PathToVertices(
    const SkPath& path, SkScalar tolerance, const SkRect& clipBounds, WindingVertex** verts) {
  int contourCnt = get_contour_count(path, tolerance);
  if (contourCnt <= 0) {
    *verts = nullptr;
    return 0;
  ...
  Poly* polys = path_to_polys(
      path, tolerance, clipBounds, contourCnt, alloc, Mode::kNormal, &numCountedCurves, nullptr);
  SkPathFillType fillType = path.getFillType();
  int64_t count64 = count_points(polys, fillType);
  if (0 == count64 || count64 > SK_MaxS32) {
    *verts = nullptr;
