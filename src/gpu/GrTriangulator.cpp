...
void GrTriangulator::appendQuadraticToContour(
    const SkPoint pts[3], SkScalar toleranceSqd, VertexList* contour) {
  SkQuadCoeff quad(pts);
...
void GrTriangulator::generateCubicPoints(
    ...
    VertexList* contour, int pointsLeft) {
...
void GrTriangulator::pathToContours(
    float tolerance, const SkRect& clipBounds, VertexList* contours) {
...
static void merge_collinear_edges(
...
static void merge_edges_above(
    Edge* edge, Edge* other, EdgeList* activeEdges, Vertex** current, const Comparator& c) {
...
Vertex* GrTriangulator::makeSortedVertex(
    const SkPoint& p, uint8_t alpha, VertexList* mesh, Vertex* reference, const Comparator& c) {
    ...
    for (Vertex* v = contour->fHead; v;) {
      ...
      if (coincident(prev->fPoint, v->fPoint)) {
        ...
        contour->remove(v);
          ...
          fCullCollinearVertices && Line(prev->fPoint, nextWrap->fPoint).dist(v->fPoint) == 0.0) {
...
void* GrTriangulator::polysToTrianglesImpl(
    Poly* polys, void* data, SkPathFillType overrideFillType) {
...
int GrTriangulator::pathToTriangles(
    float tolerance, const SkRect& clipBounds, GrEagerVertexAllocator* vertexAllocator) {
...
int GrTriangulator::PathToVertices(
