...
void GrFragmentProcessor::registerChild(
  ...
  // strategy set on it.
  SkASSERT(
      !child->fParent && !child->sampleUsage().isSampled() &&
  ...
  SkASSERT(
      !this->isSampledWithExplicitCoords() && !this->hasPerspectiveTransform() &&
...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::Compose(
  ...
  switch (leadingFPsToEliminate) {
    ...
    case 1:
      ...
      return ComposeProcessor::Make(GrConstColorProcessor::Make(knownColor), std::move(series[1]));
    case 2:
