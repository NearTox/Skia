...
static void recursive_dump_tree_info(
    const GrFragmentProcessor& fp, SkString indent, SkString* text) {
...
std::unique_ptr<GrFragmentProcessor::ProgramImpl> GrFragmentProcessor::makeProgramImpl() const {
  ...
  for (int i = 0; i < fChildProcessors.count(); ++i) {
    impl->fChildProcessors[i] =
        fChildProcessors[i] ? fChildProcessors[i]->makeProgramImpl() : nullptr;
  }
  return impl;
}
...
void GrFragmentProcessor::registerChild(
    std::unique_ptr<GrFragmentProcessor> child, SkSL::SampleUsage sampleUsage) {
...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::MakeColor(SkPMColor4f color) {
  ...
  return GrSkSLFP::Make(
      effect, "color_fp", /*inputFP=*/nullptr,
      color.isOpaque() ? GrSkSLFP::OptFlags::kPreservesOpaqueInput : GrSkSLFP::OptFlags::kNone,
      "color", color);
}
...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::MulChildByInputAlpha(
  ...
  return GrBlendFragmentProcessor::Make(
      /*src=*/nullptr, OverrideInput(std::move(fp), SK_PMColor4fWHITE), SkBlendMode::kDstIn);
}
...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::MulInputByChildAlpha(
  ...
  return GrBlendFragmentProcessor::Make(
      /*src=*/nullptr, OverrideInput(std::move(fp), SK_PMColor4fWHITE), SkBlendMode::kSrcIn);
}
...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::MakeInputPremulAndMulByOutput(
  ...
  class PremulFragmentProcessor : public GrFragmentProcessor {
   ...
   private:
    ...
    SkPMColor4f constantOutputForConstantInput(const SkPMColor4f& input) const override {
      SkPMColor4f childColor =
          ConstantOutputForConstantInput(this->childProcessor(0), SK_PMColor4fWHITE);
...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::OverrideInput(
  ...
  SkASSERT(SkRuntimeEffectPriv::SupportsConstantOutputForConstantInput(effect));
  return GrSkSLFP::Make(
      effect, "OverrideInput", /*inputFP=*/nullptr,
      color.isOpaque() ? GrSkSLFP::OptFlags::kPreservesOpaqueInput : GrSkSLFP::OptFlags::kNone,
      "fp", std::move(fp), "color", GrSkSLFP::SpecializeIf(!useUniform, color));
}
...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::UseDestColorAsInput(
  ...
  return GrSkSLFP::Make(
      effect, "UseDestColorAsInput", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "fp",
      std::move(fp));
}
...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::MakeInputOpaqueAndPostApplyAlpha(
  ...
  return GrSkSLFP::Make(
      effect, "MakeInputOpaque",
      /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kPreservesOpaqueInput, "fp", std::move(fp));
}
  ...
  class ComposeProcessor : public GrFragmentProcessor {
   ...
   private:
    ...
    ComposeProcessor(std::unique_ptr<GrFragmentProcessor> f, std::unique_ptr<GrFragmentProcessor> g)
        : INHERITED(
              kSeriesFragmentProcessor_ClassID, f->optimizationFlags() & g->optimizationFlags()) {
...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::ColorMatrix(
    std::unique_ptr<GrFragmentProcessor> child, const float matrix[20], bool unpremulInput,
  ...
  SkM44 m44(
      matrix[0], matrix[1], matrix[2], matrix[3], matrix[5], matrix[6], matrix[7], matrix[8],
      matrix[10], matrix[11], matrix[12], matrix[13], matrix[15], matrix[16], matrix[17],
      matrix[18]);
  SkV4 v4 = {matrix[4], matrix[9], matrix[14], matrix[19]};
  return GrSkSLFP::Make(
      effect, "ColorMatrix", std::move(child), GrSkSLFP::OptFlags::kNone, "m", m44, "v", v4,
      "unpremulInput", GrSkSLFP::Specialize(unpremulInput ? 1 : 0), "clampRGBOutput",
      GrSkSLFP::Specialize(clampRGBOutput ? 1 : 0), "premulOutput",
      GrSkSLFP::Specialize(premulOutput ? 1 : 0));
}
...
std::unique_ptr<GrFragmentProcessor> GrFragmentProcessor::Rect(
  ...
  return GrSkSLFP::Make(
      effect, "Rect", std::move(inputFP), GrSkSLFP::OptFlags::kCompatibleWithCoverageAsAlpha,
      "edgeType", GrSkSLFP::Specialize(static_cast<int>(edgeType)), "rectUniform", rectUniform);
}

GrFPResult GrFragmentProcessor::Circle(
  ...
  return GrFPSuccess(GrSkSLFP::Make(
      effect, "Circle", std::move(inputFP), GrSkSLFP::OptFlags::kCompatibleWithCoverageAsAlpha,
      "edgeType", GrSkSLFP::Specialize(static_cast<int>(edgeType)), "circle", circle));
}

GrFPResult GrFragmentProcessor::Ellipse(
    std::unique_ptr<GrFragmentProcessor> inputFP, GrClipEdgeType edgeType, SkPoint center,
  ...
  return GrFPSuccess(GrSkSLFP::Make(
      effect, "Ellipse", std::move(inputFP), GrSkSLFP::OptFlags::kCompatibleWithCoverageAsAlpha,
...
SkString ProgramImpl::invokeChild(
  ...
  auto invocation =
      SkStringPrintf("%s(%s", this->childProcessor(childIndex)->functionName(), inputColor);
