...
class GrPerspQuad {
 public:
  ...
  GrPerspQuad(const skvx::Vec<4, float>& xs, const skvx::Vec<4, float>& ys, GrQuadType type)
      : fType(type) {
...
class GrQuadList : public GrQuadListBase<void> {
 public:
  ...
  void push_back(const GrQuad& quad) { this->pushBackImpl(quad); }

  void push_back(const GrPerspQuad& quad) { this->pushBackImpl(quad); }
