...
class GrTriangulator {
 ...
 protected:
  ...
  virtual ~GrTriangulator() = default;
  ...
  void contoursToMesh(
      VertexList* contours, int contourCnt, VertexList* mesh, const Comparator&) const;

  // 3) Sort the vertices in Y (and secondarily in X):
  static void SortedMerge(
      VertexList* front, VertexList* back, VertexList* result, const Comparator&);
  ...
  void generateCubicPoints(
      const SkPoint&, const SkPoint&, const SkPoint&, const SkPoint&, SkScalar tolSqd,
  ...
  void mergeCollinearEdges(
      Edge* edge, EdgeList* activeEdges, Vertex** current, const Comparator&) const;
  bool splitEdge(
      Edge* edge, Vertex* v, EdgeList* activeEdges, Vertex** current, const Comparator&) const;
  bool intersectEdgePair(
      Edge* left, Edge* right, EdgeList* activeEdges, Vertex** current, const Comparator&) const;
  Vertex* makeSortedVertex(
      const SkPoint&, uint8_t alpha, VertexList* mesh, Vertex* reference, const Comparator&) const;
  ...
  void buildEdges(VertexList* contours, int contourCnt, VertexList* mesh, const Comparator&) const;
