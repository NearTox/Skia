...
static bool is_inverted(bool originalIsInverted, GrStyledShape::FillInversion inversion) noexcept {
  switch (inversion) {
    ...
    case GrStyledShape::FillInversion::kForceInverted: return true;
    case GrStyledShape::FillInversion::kForceNoninverted: return false;
  }
  return false;
}
...
void GrStyledShape::writeUnstyledKey(uint32_t* key) const {
  ...
  SkDEBUGCODE(uint32_t* origKey = key;) if (fInheritedKey.count()) {
    ...
    SkASSERT(
        (fShape.isRect() || fShape.isRRect()) ||
        (fShape.dir() == GrShape::kDefaultDir && fShape.startIndex() == GrShape::kDefaultStart));
...
void GrStyledShape::setInheritedKey(
    const GrStyledShape& parent, GrStyle::Apply apply, SkScalar scale) {
...
const SkPath* GrStyledShape::originalPathForListeners() const noexcept {
...
GrStyledShape GrStyledShape::MakeArc(
...
GrStyledShape::GrStyledShape(const GrStyledShape& parent, GrStyle::Apply apply, SkScalar scale) {
  ...
  if (pe) {
    ...
    if (!parent.fStyle.applyPathEffectToPath(
            &fShape.path(), &strokeRec, *srcForPathEffect, scale)) {
    ...
    if (GrStyle::Apply::kPathEffectAndStrokeRec == apply && strokeRec.needToApply()) {
      ...
      if (tmpParent.get()->style().applies()) {
        SkAssertResult(tmpParent.get()->style().applyToPath(
            &fShape.path(), &fillOrHairline, *tmpPath.get(), scale));
    ...
    SkAssertResult(
        parent.fStyle.applyToPath(&fShape.path(), &fillOrHairline, *srcForParentStyle, scale));
    fStyle.resetToInitStyle(fillOrHairline);
  }
  ...
  if (fShape.isRect()) {
    ...
    if (!fStyle.hasPathEffect()) {
      ...
      if (fShape.rect().fLeft > fShape.rect().fRight) {
        ...
        rectDir = rectDir == SkPathDirection::kCCW ? SkPathDirection::kCW : SkPathDirection::kCCW;
...
bool GrStyledShape::asLine(SkPoint pts[2], bool* inverted) const noexcept {
  if (!fShape.isLine()) {
    return false;
  }
...
bool GrStyledShape::simplifyStroke(bool originallyClosed) {
  ...
  if (fStyle.isDashed()) {
    ...
    bool dropDash = false;
    if (fShape.isPoint()) {
      dropDash = fStyle.dashIntervalCnt() > 0 && SkToBool(fStyle.dashIntervals()[0]);
    } else {
      dropDash = true;
  ...
  if (originallyClosed) {
    ...
    if (fShape.isLine() && fStyle.strokeRec().getJoin() == SkPaint::kRound_Join) {
      ...
      cap = SkPaint::kRound_Cap;
        ...
        fShape.isPoint() && fStyle.strokeRec().getJoin() == SkPaint::kMiter_Join &&
        !strokeAndFilled) {
