...
void GrAATriangulator::makeEvent(
    SSEdge* edge, Vertex* v, SSEdge* other, Vertex* dest, EventList* events,
...
void GrAATriangulator::Event::apply(
    VertexList* mesh, const Comparator& c, EventList* events,
...
bool GrAATriangulator::collapseOverlapRegions(
    VertexList* mesh, const Comparator& c, EventComparator comp) const {
  ...
  for (Vertex* v = mesh->fHead; v != nullptr; v = v->fNext) {
    ...
    for (Edge* e = v->fLastEdgeAbove; e && e != leftEnclosingEdge;) {
      ...
      bool isOuterBoundary =
          e->fType == EdgeType::kOuter && (!prev || prev->fWinding == 0 || e->fWinding == 0);
      if (prev) {
        e->fWinding -= prev->fWinding;
      }
      if (leftOverlap && rightOverlap) {
        ...
        if (!isOuterBoundary) {
          ...
          e->fWinding = SkScalarCopySign(e->fType == EdgeType::kInner ? 2 : 1, e->fWinding);
        }
      }
      e = prev;
    }
...
void GrAATriangulator::strokeBoundary(
    EdgeList* boundary, VertexList* innerMesh, const Comparator& c) const {
  TESS_LOG("\nstroking boundary\n");
  ...
  this->makeConnectingEdge(
      innerVertices.fTail, innerVertices.fHead, EdgeType::kInner, c, innerWinding);
  for (Vertex* v = outerVertices.fHead; v && v->fNext; v = v->fNext) {
    this->makeConnectingEdge(v, v->fNext, EdgeType::kOuter, c, outerWinding);
  }
  this->makeConnectingEdge(
      outerVertices.fTail, outerVertices.fHead, EdgeType::kOuter, c, outerWinding);
...
void GrAATriangulator::extractBoundaries(
    const VertexList& inMesh, VertexList* innerVertices, const Comparator& c) const {
...
int GrAATriangulator::polysToAATriangles(
    Poly* polys, GrEagerVertexAllocator* vertexAllocator) const {
