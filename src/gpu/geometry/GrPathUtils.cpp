...
SkScalar GrPathUtils::scaleToleranceToSrc(
    SkScalar devTol, const SkMatrix& viewM, const SkRect& pathBounds) {
  ...
  if (stretch < 0) {
    ...
    for (int i = 0; i < 4; ++i) {
      SkMatrix mat;
      mat.setTranslate(
          (i % 2) ? pathBounds.fLeft : pathBounds.fRight,
...
uint32_t GrPathUtils::generateQuadraticPoints(
...
int GrPathUtils::worstCasePointCount(const SkPath& path, int* subpaths, SkScalar tol) {
  ...
  *subpaths = 1;
  ...
  while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
    switch (verb) {
      case SkPath::kLine_Verb: pointCount += 1; break;
...
void convert_noninflect_cubic_to_quads(
    const SkPoint p[4], SkScalar toleranceSqd, SkTArray<SkPoint, true>* quads, int sublevel = 0,
    bool preserveFirstTangent = true, bool preserveLastTangent = true) {
...
void convert_noninflect_cubic_to_quads_with_constraint(
  ...
  if (!doQuads) {
    ...
    if (invDALengthSqd > SK_ScalarNearlyZero) {
      ...
      if (detABSqd * invDALengthSqd < toleranceSqd && detDCSqd * invDALengthSqd < toleranceSqd) {
        doQuads = true;
      }
    }
  }
...
ExcludedTerm GrPathUtils::calcCubicInverseTransposePowerBasisMatrix(
    const SkPoint p[4], SkMatrix* out) {
  ...
  D.store(dets);
  ExcludedTerm skipTerm = SkScalarAbs(dets[0]) > SkScalarAbs(dets[1]) ? ExcludedTerm::kQuadraticTerm
  ...
  out->setAll(
      y[middleRow] * rdet, -x[middleRow] * rdet, z[middleRow] * rdet, -y[0] * rdet, x[0] * rdet,
      -z[0] * rdet, 0, 0, 1);

  return skipTerm;
}

inline static void calc_serp_kcoeffs(
    SkScalar tl, SkScalar sl, SkScalar tm, SkScalar sm, ExcludedTerm skipTerm,
...
inline static void calc_serp_lmcoeffs(
    SkScalar t, SkScalar s, ExcludedTerm skipTerm, SkScalar outCoeffs[3]) {
  SkASSERT(ExcludedTerm::kQuadraticTerm == skipTerm || ExcludedTerm::kLinearTerm == skipTerm);
  outCoeffs[0] = -s * s * s;
...
inline static void calc_loop_kcoeffs(
    SkScalar td, SkScalar sd, SkScalar te, SkScalar se, SkScalar tdse, SkScalar tesd,
...
inline static void calc_loop_lmcoeffs(
    SkScalar t2, SkScalar s2, SkScalar t1, SkScalar s1, SkScalar t2s1, SkScalar t1s2,
    ExcludedTerm skipTerm, SkScalar outCoeffs[3]) {
  SkASSERT(ExcludedTerm::kQuadraticTerm == skipTerm || ExcludedTerm::kLinearTerm == skipTerm);
  outCoeffs[0] = -s2 * s2 * s1;
  outCoeffs[1] =
      (ExcludedTerm::kLinearTerm == skipTerm) ? s2 * (2 * t2s1 + t1s2) : -t2 * (t2s1 + 2 * t1s2);
...
static void calc_quadratic_klm(const SkPoint pts[4], double d3, SkMatrix* klm) {
  ...
  SkMatrix inversePts;
  inversePts.setAll(
      pts[0].x(), pts[1].x(), pts[3].x(), pts[0].y(), pts[1].y(), pts[3].y(), 1, 1, 1);
...
SkCubicType GrPathUtils::getCubicKLM(
    const SkPoint src[4], SkMatrix* klm, double tt[2], double ss[2]) {
  double d[4];
  ...
  switch (type) {
    ...
    default: SK_ABORT("Unexpected cubic type."); break;
  }
...
int GrPathUtils::chopCubicAtLoopIntersection(
