...
int TessellationHelper::EdgeEquations::computeDegenerateQuad(
  ...
  *aaMask = signedEdgeDistances != 0.f;
  ...
  // p(3) to e0,e2
  V4f dists1 = px * skvx::shuffle<3, 3, 0, 0>(fA) + py * skvx::shuffle<3, 3, 0, 0>(fB) +
               skvx::shuffle<3, 3, 0, 0>(oc);
  V4f dists2 = px * skvx::shuffle<1, 2, 1, 2>(fA) + py * skvx::shuffle<1, 2, 1, 2>(fB) +
...
void TessellationHelper::Vertices::moveAlong(
    const EdgeVectors& edgeVectors, const V4f& signedEdgeDistances) {
