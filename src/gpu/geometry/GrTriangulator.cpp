...
void* GrTriangulator::emitTriangle(
    Vertex* prev, Vertex* curr, Vertex* next, int winding, void* data) const {
  if (winding > 0) {
...
Edge* GrTriangulator::makeEdge(
    Vertex* prev, Vertex* next, EdgeType type, const Comparator& c) const {
...
bool GrTriangulator::intersectEdgePair(
    Edge* left, Edge* right, EdgeList* activeEdges, Vertex** current, const Comparator& c) const {
...
void GrTriangulator::buildEdges(
    VertexList* contours, int contourCnt, VertexList* mesh, const Comparator& c) const {
  for (VertexList* contour = contours; contourCnt > 0; --contourCnt, ++contour) {
    Vertex* prev = contour->fTail;
    for (Vertex* v = contour->fHead; v;) {
      Vertex* next = v->fNext;
...
GrTriangulator::SimplifyResult GrTriangulator::simplify(
    VertexList* mesh, const Comparator& c) const {
...
void* GrTriangulator::polysToTriangles(
    Poly* polys, void* data, SkPathFillType overrideFillType) const {
