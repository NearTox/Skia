...
GrShape& GrShape::operator=(const GrShape& that) {
  ...
  switch (fType) {
    case Type::kEmpty: break;
    case Type::kInvertedEmpty: break;
    case Type::kRRect: fRRectData = that.fRRectData; break;
    case Type::kArc: fArcData = that.fArcData; break;
    case Type::kLine: fLineData = that.fLineData; break;
    case Type::kPath: fPathData.fGenID = that.fPathData.fGenID; break;
  }
  fInheritedKey.reset(that.fInheritedKey.count());
  sk_careful_memcpy(
      fInheritedKey.get(), that.fInheritedKey.get(), sizeof(uint32_t) * fInheritedKey.count());
...
static bool flip_inversion(bool originalIsInverted, GrShape::FillInversion inversion) {
  switch (inversion) {
    ...
    case GrShape::FillInversion::kForceInverted: return !originalIsInverted;
    case GrShape::FillInversion::kForceNoninverted: return originalIsInverted;
  }
  return false;
}
...
static bool is_inverted(bool originalIsInverted, GrShape::FillInversion inversion) {
  switch (inversion) {
    ...
    case GrShape::FillInversion::kForceInverted: return true;
    case GrShape::FillInversion::kForceNoninverted: return false;
  }
  return false;
}
...
SkRect GrShape::bounds() const {
  ...
  switch (fType) {
    ...
    case Type::kInvertedEmpty: return kInverted;
    case Type::kLine: {
      SkRect bounds;
    ...
    case Type::kRRect: return fRRectData.fRRect.getBounds();
    case Type::kArc:
...
int GrShape::unstyledKeySize() const {
  ...
  switch (fType) {
    case Type::kEmpty: return 1;
    case Type::kInvertedEmpty: return 1;
    case Type::kRRect:
...
void GrShape::setInheritedKey(const GrShape& parent, GrStyle::Apply apply, SkScalar scale) {
  ...
  if (Type::kPath == fType) {
    ...
    GrStyle::WriteKey(fInheritedKey.get() + parentCnt, parent.fStyle, apply, scale, styleKeyFlags);
  }
}
...
GrShape::GrShape(const GrShape& parent, GrStyle::Apply apply, SkScalar scale) {
  ...
  if (pe) {
    ...
    if (!parent.fStyle.applyPathEffectToPath(&this->path(), &strokeRec, *srcForPathEffect, scale)) {
    ...
    if (GrStyle::Apply::kPathEffectAndStrokeRec == apply && strokeRec.needToApply()) {
      ...
      if (tmpParent.get()->style().applies()) {
        SkAssertResult(tmpParent.get()->style().applyToPath(
            &this->path(), &fillOrHairline, *tmpPath.get(), scale));
    ...
    SkAssertResult(
        parent.fStyle.applyToPath(&this->path(), &fillOrHairline, *srcForParentStyle, scale));
    fStyle.resetToInitStyle(fillOrHairline);
  }
...
void GrShape::attemptToSimplifyPath() {
  ...
  if (this->path().isEmpty()) {
    ...
    this->changeType(inverted && !this->style().isDashed() ? Type::kInvertedEmpty : Type::kEmpty);
