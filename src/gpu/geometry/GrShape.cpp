...
GrShape& GrShape::operator=(const GrShape& shape) noexcept {
  switch (shape.type()) {
    ...
    case Type::kRect: this->setRect(shape.fRect); break;
    case Type::kRRect: this->setRRect(shape.fRRect); break;
...
bool GrShape::simplifyArc(unsigned flags) {
  ...
  if (fArc.fOval.isEmpty() || !fArc.fSweepAngle) {
    if (flags & kSimpleFill_Flag) {
      ...
      SkPoint start = {
          center.fX + 0.5f * fArc.fOval.width() * SkScalarCos(startRad),
    ...
    if ((flags & kSimpleFill_Flag) || ((flags & kIgnoreWinding_Flag) && !fArc.fUseCenter)) {
      ...
      if (fArc.fSweepAngle <= -360.f || fArc.fSweepAngle >= 360.f) {
        this->simplifyRRect(SkRRect::MakeOval(fArc.fOval), kDefaultDir, kDefaultStart, flags);
        return true;
      }
    }
...
void GrShape::simplifyRRect(
    const SkRRect& rrect, SkPathDirection dir, unsigned start, unsigned flags) {
...
bool GrShape::simplify(unsigned flags) {
  ...
  switch (fType) {
    case Type::kEmpty:
      // do nothing
      break;
    case Type::kPoint: this->simplifyPoint(fPoint, flags); break;
    case Type::kLine: this->simplifyLine(fLine.fP1, fLine.fP2, flags); break;
    case Type::kRect:
    ...
    case Type::kRRect:
      ...
      wasClosed = true;
      break;
    case Type::kPath: wasClosed = this->simplifyPath(flags); break;
    case Type::kArc: wasClosed = this->simplifyArc(flags); break;
...
SkRect GrShape::bounds() const noexcept {
  ...
  switch (this->type()) {
    ...
    case Type::kPoint: return {fPoint.fX, fPoint.fY, fPoint.fX, fPoint.fY};
    case Type::kRect: return fRect.makeSorted();
    case Type::kRRect: return fRRect.getBounds();
    case Type::kPath: return fPath.getBounds();
    case Type::kArc: return fArc.fOval;
    case Type::kLine: {
      SkRect b = SkRect::MakeLTRB(fLine.fP1.fX, fLine.fP1.fY, fLine.fP2.fX, fLine.fP2.fY);
      b.sort();
      return b;
    }
    default: SkUNREACHABLE;
...
uint32_t GrShape::segmentMask() const noexcept {
  ...
  switch (this->type()) {
    ...
    case Type::kPath: return fPath.getSegmentMasks();
    case Type::kArc:
      if (fArc.fUseCenter) {
        return SkPath::kConic_SegmentMask | SkPath::kLine_SegmentMask;
      } else {
...
void GrShape::asPath(SkPath* out, bool simpleFill) const {
  ...
  switch (this->type()) {
    ...
    case Type::kPoint:
      ...
      out->lineTo(fPoint);
      return;
    case Type::kRect: out->addRect(fRect, this->dir(), this->startIndex()); return;
    case Type::kRRect: out->addRRect(fRRect, this->dir(), this->startIndex()); return;
    case Type::kPath: *out = fPath; return;
    case Type::kArc:
      SkPathPriv::CreateDrawArcPath(
          out, fArc.fOval, fArc.fStartAngle, fArc.fSweepAngle, fArc.fUseCenter, simpleFill);
