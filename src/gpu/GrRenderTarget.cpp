...
GrRenderTarget::GrRenderTarget(
    GrGpu* gpu, const SkISize& size, GrPixelConfig config, int sampleCount, GrProtected isProtected,
...
void GrRenderTargetPriv::attachStencilAttachment(sk_sp<GrStencilAttachment> stencil) {
  ...
  if (1 == fRenderTarget->fSampleCnt) {
    ...
    SkASSERT(
        GrSamplePatternDictionary::kInvalidSamplePatternKey == fRenderTarget->fSamplePatternKey);
  } else {
...
int GrRenderTargetPriv::getSamplePatternKey() const {
  ...
  SkASSERT(GrSamplePatternDictionary::kInvalidSamplePatternKey != fRenderTarget->fSamplePatternKey);
