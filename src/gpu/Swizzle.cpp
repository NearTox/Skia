...
void Swizzle::apply(SkRasterPipeline* pipeline) const {
  SkASSERT(pipeline);
  switch (fKey) {
    case Swizzle("rgba").asKey(): return;
    case Swizzle("bgra").asKey(): pipeline->append(SkRasterPipeline::swap_rb); return;
    case Swizzle("aaa1").asKey(): pipeline->append(SkRasterPipeline::alpha_to_gray); return;
    case Swizzle("rgb1").asKey(): pipeline->append(SkRasterPipeline::force_opaque); return;
    case Swizzle("a001").asKey(): pipeline->append(SkRasterPipeline::alpha_to_red); return;
    default: {
