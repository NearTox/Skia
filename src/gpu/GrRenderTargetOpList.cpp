...
bool GrRenderTargetOpList::OpChain::prependChain(
...
GrRenderTargetOpList::GrRenderTargetOpList(
    sk_sp<GrOpMemoryPool> opMemoryPool, sk_sp<GrRenderTargetProxy> proxy, GrAuditTrail* auditTrail)
...
GrRenderTargetOpList::~GrRenderTargetOpList() { this->deleteOps(); }
  ...
  for (const auto& chain : fOpChains) {
    if (chain.head()) {
      ...
      GrOpFlushState::OpArgs opArgs = {
          chain.head(), fTarget->asRenderTargetProxy(), chain.appliedClip(),
          fTarget.get()->asRenderTargetProxy()->outputSwizzle(), chain.dstProxy()};
...
static GrGpuRTCommandBuffer* create_command_buffer(
  ...
  const GrGpuRTCommandBuffer::LoadAndStoreInfo kColorLoadStoreInfo{colorLoadOp, GrStoreOp::kStore,
                                                                   loadClearColor};
...
bool GrRenderTargetOpList::onExecute(GrOpFlushState* flushState) {
  ...
  if (fOpChains.empty() && GrLoadOp::kClear != fColorLoadOp && GrLoadOp::kDiscard != fColorLoadOp) {
    return false;
  }
  ...
  GrGpuRTCommandBuffer* commandBuffer = create_command_buffer(
      flushState->gpu(), fTarget->peekRenderTarget(), fTarget->origin(), fTarget->getBoundsRect(),
      fColorLoadOp, fLoadClearColor, fStencilLoadOp);
  ...
  for (const auto& chain : fOpChains) {
    ...
    GrOpFlushState::OpArgs opArgs{chain.head(), fTarget->asRenderTargetProxy(), chain.appliedClip(),
...
bool GrRenderTargetOpList::copySurface(
    GrRecordingContext* context, GrSurfaceProxy* dst, GrSurfaceProxy* src, const SkIRect& srcRect,
