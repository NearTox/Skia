...
#endif
      GrOpFlushState::OpArgs opArgs = {chain.head(), fTarget.get()->asRenderTargetProxy(),
                                       chain.appliedClip(), chain.dstProxy()};
...
bool GrRenderTargetOpList::onExecute(GrOpFlushState* flushState) {
  ...
  GrGpuRTCommandBuffer* commandBuffer = create_command_buffer(
      flushState->gpu(), fTarget.get()->peekRenderTarget(), fTarget.get()->origin(),
      fTarget.get()->getBoundsRect(), fColorLoadOp, fLoadClearColor, fStencilLoadOp);
