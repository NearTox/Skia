...
sk_sp<GrTextureProxy> GrRefCachedBitmapTextureProxy(
    GrRecordingContext*, const SkBitmap&, GrSamplerState, SkScalar scaleAdjust[2]);

/**
 * Creates a new texture with mipmap levels and copies the baseProxy into the base layer.
 */
sk_sp<GrTextureProxy> GrCopyBaseMipMapToTextureProxy(
    GrRecordingContext*, GrSurfaceProxy* baseProxy, GrColorType srcColorType);
...
GrSurfaceProxyView GrMakeCachedBitmapProxyView(
    GrRecordingContext*, const SkBitmap& bitmap, SkBackingFit fit = SkBackingFit::kExact);
...
void GrInstallBitmapUniqueKeyInvalidator(
    const GrUniqueKey& key, uint32_t contextID, SkPixelRef* pixelRef);
