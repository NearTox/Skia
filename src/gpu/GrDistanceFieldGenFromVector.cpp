...
class DAffineMatrix {
 public:
  ...
  DPoint mapPoint(const DPoint& src) const {
    return DPoint::Make(
        fMat[0] * src.x() + fMat[1] * src.y() + fMat[2],
...
static inline bool between_closed_open(
    double a, double b, double c, double tolerance = 0.0, bool xformToleranceToX = false) {
...
static inline bool between_closed(
    double a, double b, double c, double tolerance = 0.0, bool xformToleranceToX = false) {
  SkASSERT(tolerance >= 0.0);
  double tolB = tolerance;
  double tolC = tolerance;
...
static inline bool nearly_equal(
    double x, double y, double tolerance = kNearlyZero, bool xformToleranceToX = false) {
  SkASSERT(tolerance >= 0.0);
  if (xformToleranceToX) {
...
static bool is_colinear(const SkPoint pts[3]) {
  return nearly_zero(
      (pts[1].y() - pts[0].y()) * (pts[1].x() - pts[2].x()) -
...
void PathSegment::init() {
  ...
  if (fType == PathSegment::kLine) {
    ...
    fXformMatrix.setAffine(
        cosTheta, sinTheta, -(cosTheta * p0x) - (sinTheta * p0y), -sinTheta, cosTheta,
        (sinTheta * p0x) - (cosTheta * p0y));
  } else {
    ...
    const double c = (p0xSqd * p2ySqd) - (4.0 * p01xProd * b12yProd) - (2.0 * p02xProd * p02yProd) +
                     (4.0 * p02xProd * p1ySqd) + (4.0 * p1xSqd * p02yProd) -
                     (4.0 * b12xProd * p01yProd) + (p2xSqd * p0ySqd);
    ...
    const double f =
        -((p0xSqd * p2y) - (2.0 * p01xProd * p1y) - (2.0 * p01xProd * p2y) - (p02xProd * p0y) +
          (4.0 * p02xProd * p1y) - (p02xProd * p2y) + (2.0 * p1xSqd * p0y) + (2.0 * p1xSqd * p2y) -
          (2.0 * b12xProd * p0y) - (2.0 * b12xProd * p1y) + (p2xSqd * p0y));
    ...
    fXformMatrix.setAffine(
        lambda_cosTheta, -lambda_sinTheta, lambda * x0, lambda_sinTheta, lambda_cosTheta,
        lambda * y0);
  }
...
void precomputation_for_row(
    RowData* rowData, const PathSegment& segment, const SkPoint& pointLeft,
...
SegSide calculate_side_of_quad(
    const PathSegment& segment, const SkPoint& point, const DPoint& xFormPt,
    const RowData& rowData) {
  SegSide side = kNA_SegSide;

  if (RowData::kVerticalLine == rowData.fIntersectionType) {
    side =
        (SegSide)(int)(sign_of(xFormPt.y() - rowData.fYAtIntersection) * rowData.fQuadXDirection);
    ...
    if (includeP1 &&
        between_closed(p1, segment.fP0T.x(), segment.fP2T.x(), segment.fNearlyZeroScaled, true)) {
      side = (SegSide)(signP1 * rowData.fQuadXDirection);
    }
    if (includeP2 &&
        between_closed(p2, segment.fP0T.x(), segment.fP2T.x(), segment.fNearlyZeroScaled, true)) {
...
static float distance_to_segment(
    const SkPoint& point, const PathSegment& segment, const RowData& rowData, SegSide* side) {
  SkASSERT(side);
  ...
  if (segment.fType == PathSegment::kLine) {
    ...
    if (between_closed_open(point.y(), segment.fBoundingBox.top(), segment.fBoundingBox.bottom())) {
...
static void calculate_distance_field_data(
    PathSegmentArray* segments, DFData* dataPtr, int width, int height) {
  int count = segments->count();
  for (int a = 0; a < count; ++a) {
    PathSegment& segment = (*segments)[a];
    const SkRect& segBB = segment.fBoundingBox.makeOutset(SK_DistanceFieldPad, SK_DistanceFieldPad);
    ...
    for (int row = startRow; row < endRow; ++row) {
      ...
      if (between_closed_open(pY, segment.fBoundingBox.top(), segment.fBoundingBox.bottom())) {
      ...
      for (int col = startColumn; col < endColumn; ++col) {
        ...
        int dilation = distSq < 1.5 * 1.5
                           ? 1
                           : distSq < 2.5 * 2.5 ? 2 : distSq < 3.5 * 3.5 ? 3 : SK_DistanceFieldPad;
        ...
        if (dilation != SK_DistanceFieldPad &&
            !segment.fBoundingBox.roundOut().makeOutset(dilation, dilation).contains(col, row)) {
          continue;
        }
...
bool GrGenerateDistanceFieldFromPath(
    unsigned char* distanceField, const SkPath& path, const SkMatrix& drawMatrix, int width,
  ...
  SkASSERT(expectPathBounds.isEmpty() || expectPathBounds.contains(pathBounds.x(), pathBounds.y()));
  SkASSERT(
      expectPathBounds.isEmpty() || pathBounds.isEmpty() || expectPathBounds.contains(pathBounds));
  ...
  while (auto e = iter.next()) {
    switch (e.fEdge) {
      ...
      case SkPathEdgeIter::Edge::kQuad: add_quad_segment(e.fPts, &segments); break;
  ...
  for (int row = 0; row < height; ++row) {
    ...
    for (int col = 0; col < width; ++col) {
      ...
      switch (workingPath.getNewFillType()) {
        ...
        case SkPathFillType::kInverseWinding: dfSign = windingNumber ? kOutside : kInside; break;
        case SkPathFillType::kEvenOdd: dfSign = (windingNumber % 2) ? kInside : kOutside; break;
