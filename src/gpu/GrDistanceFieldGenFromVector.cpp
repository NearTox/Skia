...
class DAffineMatrix {
 public:
  ...
  SkDPoint mapPoint(const SkDPoint& src) const {
    return {
        fMat[0] * src.fX + fMat[1] * src.fY + fMat[2],
...
static inline double sign_of(const double& val) { return std::copysign(1, val); }

static bool is_colinear(const SkPoint pts[3]) {
  return nearly_zero(
      (pts[1].fY - pts[0].fY) * (pts[1].fX - pts[2].fX) -
          (pts[1].fY - pts[2].fY) * (pts[1].fX - pts[0].fX),
      kCloseSqd);
}
...
static inline void add_cubic(const SkPoint pts[4], PathSegmentArray* segments) {
...
static float calculate_nearest_point_for_quad(const PathSegment& segment, const SkDPoint& xFormPt) {
...
void precomputation_for_row(
  ...
  if ((rowData->fScanlineXDirection == 1 &&
       (segment.fPts[0].fY == pointLeft.fY || segment.fPts[2].fY == pointLeft.fY)) &&
      nearly_zero(c, tol)) {
...
SegSide calculate_side_of_quad(
    const PathSegment& segment, const SkPoint& point, const SkDPoint& xFormPt,
  ...
  if (RowData::kVerticalLine == rowData.fIntersectionType) {
    ...
    if (includeP1 &&
        between_closed(p1, segment.fP0T.fX, segment.fP2T.fX, segment.fNearlyZeroScaled, true)) {
      side = (SegSide)(signP1 * rowData.fQuadXDirection);
    }
    if (includeP2 &&
        between_closed(p2, segment.fP0T.fX, segment.fP2T.fX, segment.fNearlyZeroScaled, true)) {
...
static float distance_to_segment(
  ...
  if (segment.fType == PathSegment::kLine) {
    ...
    if (between_closed_open(point.fY, segment.fBoundingBox.fTop, segment.fBoundingBox.fBottom)) {
...
bool GrGenerateDistanceFieldFromPath(
  ...
  SkASSERT(
      expectPathBounds.isEmpty() || expectPathBounds.contains(pathBounds.fLeft, pathBounds.fTop));
  SkASSERT(
  ...
  while (auto e = iter.next()) {
    switch (e.fEdge) {
      ...
      case SkPathEdgeIter::Edge::kQuad: add_quad(e.fPts, &segments); break;
