...
SkString GrGLSLFragmentProcessor::invokeChild(
  ...
  if (childProc->isSampledWithExplicitCoords()) {
    ...
    return SkStringPrintf(
        "%s(%s, %s)", fFunctionNames[childIndex].c_str(), inputColor, skslCoords.c_str());
  } else {
...
SkString GrGLSLFragmentProcessor::invokeChildWithMatrix(
  ...
  if (childProc->isSampledWithExplicitCoords()) {
    ...
    if (childProc->sampleUsage().fHasPerspective) {
      return SkStringPrintf(
          "%s(%s, proj((%s) * %s.xy1))", fFunctionNames[childIndex].c_str(), inputColor,
          skslMatrix.c_str(), args.fSampleCoord);
    } else {
      return SkStringPrintf(
          "%s(%s, ((%s) * %s.xy1).xy)", fFunctionNames[childIndex].c_str(), inputColor,
          skslMatrix.c_str(), args.fSampleCoord);
    }
  } else {
...
GrGLSLFragmentProcessor::ParallelIter::ParallelIter(
    const GrFragmentProcessor& fp, GrGLSLFragmentProcessor& glslFP)
...
GrGLSLFragmentProcessor::ParallelRange::ParallelRange(
    const GrFragmentProcessor& fp, GrGLSLFragmentProcessor& glslFP)
