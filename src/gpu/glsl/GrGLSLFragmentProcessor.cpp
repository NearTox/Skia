...
SkString GrGLSLFragmentProcessor::invokeChild(
  ...
  if (skslCoords.length() == 0) {
    switch (childProc.sampleMatrix().fKind) {
      ...
      case SkSL::SampleMatrix::Kind::kVariable: skslCoords = "_matrix"; break;
      default: break;
    }
  }
  ...
  if (fFunctionNames[childIndex].size() == 0) {
    ...
    EmitArgs childArgs(
        fragBuilder, args.fUniformHandler, args.fShaderCaps, childProc, "_output", "_input",
  ...
  SkString result = SkStringPrintf(
      "%s(%s", fFunctionNames[childIndex].c_str(), inputColor ? inputColor : "half4(1)");
...
SkString GrGLSLFragmentProcessor::invokeChildWithMatrix(
    int childIndex, const char* inputColor, EmitArgs& args, SkSL::String skslMatrix) {
  GrGLSLFPFragmentBuilder* fragBuilder = args.fFragBuilder;
  while (childIndex >= (int)fFunctionNames.size()) {
    fFunctionNames.emplace_back();
  }
  ...
  return SkStringPrintf(
      "%s(%s, %s)", fFunctionNames[childIndex].c_str(), inputColor ? inputColor : "half4(1)",
      skslMatrix.c_str());
}

//////////////////////////////////////////////////////////////////////////////

GrGLSLFragmentProcessor::Iter::Iter(
    std::unique_ptr<GrGLSLFragmentProcessor> fps[], int cnt) noexcept {
  for (int i = cnt - 1; i >= 0; --i) {
    fFPStack.push_back(fps[i].get());
  }
}

GrGLSLFragmentProcessor& GrGLSLFragmentProcessor::Iter::operator*() const noexcept {
