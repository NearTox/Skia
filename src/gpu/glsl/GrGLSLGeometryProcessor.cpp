...
void GrGLSLGeometryProcessor::emitCode(EmitArgs& args) {
  ...
  if (!args.fGP.willUseGeoShader()) {
    ...
    switch (gpArgs.fPositionVar.getType()) {
      case kFloat_GrSLType: vBuilder->codeAppend(", 0");   // fallthru.
      case kFloat2_GrSLType: vBuilder->codeAppend(", 0");  // fallthru.
      case kFloat3_GrSLType: vBuilder->codeAppend(", 1");  // fallthru.
...
void GrGLSLGeometryProcessor::emitTransforms(
    ...
    const SkMatrix& localMatrix, FPCoordTransformHandler* handler) {
  ...
  for (int i = 0; *handler; ++*handler, ++i) {
    ...
    if (!fp.isSampledWithExplicitCoords() || !coordTransform.isNoOp()) {
      ...
      fInstalledTransforms.push_back().fHandle =
          uniformHandler
              ->addUniform(
                  kVertex_GrShaderFlag, kFloat3x3_GrSLType, strUniName.c_str(), &matrixName)
              .toIndex();
    } else {
    ...
    if (!fp.isSampledWithExplicitCoords()) {
      ...
      handler->specifyCoordsForCurrCoordTransform(
          SkString(matrixName), fInstalledTransforms.back().fHandle, fsVar);
    } else {
      handler->specifyCoordsForCurrCoordTransform(
          SkString(matrixName), fInstalledTransforms.back().fHandle);
    }
  }
}

void GrGLSLGeometryProcessor::setTransformDataHelper(
    const SkMatrix& localMatrix, const GrGLSLProgramDataManager& pdman,
    ...
    *viewMatrixUniform = uniformHandler->addUniform(
    ...
    if (!mat.hasPerspective()) {
      ...
      vertBuilder->codeAppendf(
          "float2 %s = (%s * float3(%s, 1)).xy;", gpArgs->fPositionVar.c_str(), viewMatrixName,
          posName);
    } else {
      gpArgs->fPositionVar.set(kFloat3_GrSLType, "pos3");
      vertBuilder->codeAppendf(
          "float3 %s = %s * float3(%s, 1);", gpArgs->fPositionVar.c_str(), viewMatrixName, posName);
    }
  }
}
