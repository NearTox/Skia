...
void GrGLSLGeometryProcessor::emitCode(EmitArgs& args) {
  ...
  if (!args.fGP.willUseGeoShader()) {
    ...
    switch (gpArgs.fPositionVar.getType()) {
      case kFloat_GrSLType: vBuilder->codeAppend(", 0"); [[fallthrough]];
      case kFloat2_GrSLType: vBuilder->codeAppend(", 0"); [[fallthrough]];
      case kFloat3_GrSLType: vBuilder->codeAppend(", 1"); [[fallthrough]];
      case kFloat4_GrSLType: vBuilder->codeAppend(");"); break;
      default: SK_ABORT("Invalid position var type"); break;
    }
  }
}

void GrGLSLGeometryProcessor::collectTransforms(
  ...
  auto getLocalCoords = [&localCoordsVar, localCoords = SkString(),
  ...
  for (int i = 0; *handler; ++*handler, ++i) {
    ...
    if (!fp.isSampledWithExplicitCoords() || !coordTransform.isNoOp()) {
      ...
      auto flag = fp.isSampledWithExplicitCoords() ? kFragment_GrShaderFlag : kVertex_GrShaderFlag;
    ...
    switch (tr.fFP->sampleMatrix().fKind) {
      case SkSL::SampleMatrix::Kind::kConstantOrUniform: {
        ...
        if (tr.fFP->sampleMatrix().fBase) {
          ...
          localCoords =
              SkStringPrintf("float3(%s, 1)", localCoordsMap[tr.fFP->sampleMatrix().fBase]);
        } else {
        ...
        if (tr.fType == kFloat2_GrSLType) {
          vb->codeAppendf(
              ...
              tr.fMatrix.c_str(), localCoords.c_str());
        } else {
...
void GrGLSLGeometryProcessor::setTransformDataHelper(
    const GrGLSLProgramDataManager& pdman, const CoordTransformRange& transformRange) {
  int i = 0;
    ...
    if (fInstalledTransforms[i].fHandle.isValid()) {
      ...
      if (!SkMatrixPriv::CheapEqual(fInstalledTransforms[i].fCurrentValue, m)) {
        if (fInstalledTransforms[i].fType == kFloat4_GrSLType) {
          float values[4] = {m.getScaleX(), m.getTranslateX(), m.getScaleY(), m.getTranslateY()};
...
void GrGLSLGeometryProcessor::setTransform(
    const GrGLSLProgramDataManager& pdman, const UniformHandle& uniform, const SkMatrix& matrix,
...
static void write_vertex_position(
    ...
    const GrShaderVar& inPos, const SkMatrix& matrix, const char* matrixName, GrShaderVar* outPos,
  ...
  if (matrix.isIdentity()) {
    ...
    *matrixUniform = uniformHandler->addUniform(
    ...
    if (inPos.getType() == kFloat3_GrSLType) {
      ...
      vertBuilder->codeAppendf(
          "float3 %s = (%s * %s.xy1);", outName.c_str(), mangledMatrixName,
          inPos.getName().c_str());
      outPos->set(kFloat3_GrSLType, outName.c_str());
    } else {
      if (useCompactTransform) {
...
void GrGLSLGeometryProcessor::writeOutputPosition(
  ...
  write_vertex_position(
      vertBuilder, nullptr, inPos, SkMatrix::I(), "viewMatrix", &gpArgs->fPositionVar, nullptr);
}

void GrGLSLGeometryProcessor::writeOutputPosition(
    GrGLSLVertexBuilder* vertBuilder, GrGLSLUniformHandler* uniformHandler, GrGPArgs* gpArgs,
    const char* posName, const SkMatrix& mat, UniformHandle* viewMatrixUniform) {
  GrShaderVar inPos(posName, kFloat2_GrSLType);
  write_vertex_position(
      vertBuilder, uniformHandler, inPos, mat, "viewMatrix", &gpArgs->fPositionVar,
      viewMatrixUniform);
}

void GrGLSLGeometryProcessor::writeLocalCoord(
    GrGLSLVertexBuilder* vertBuilder, GrGLSLUniformHandler* uniformHandler, GrGPArgs* gpArgs,
    GrShaderVar localVar, const SkMatrix& localMatrix, UniformHandle* localMatrixUniform) {
  write_vertex_position(
      vertBuilder, uniformHandler, localVar, localMatrix, "localMatrix", &gpArgs->fLocalCoordVar,
      localMatrixUniform);
}
