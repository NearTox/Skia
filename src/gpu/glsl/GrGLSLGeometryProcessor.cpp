...
void GrGLSLGeometryProcessor::emitTransforms(
  ...
  SkASSERT(
      2 == GrSLTypeVecLength(localCoordsVar.getType()) ||
  ...
  while (const GrCoordTransform* coordTransform = handler->nextCoordTransform()) {
    ...
    fInstalledTransforms.push_back().fHandle =
        uniformHandler
            ->addUniform(kVertex_GrShaderFlag, kFloat3x3_GrSLType, strUniName.c_str(), &uniName)
            .toIndex();
    ...
    handler->specifyCoordsForCurrCoordTransform(
        SkString(uniName), fInstalledTransforms.back().fHandle,
