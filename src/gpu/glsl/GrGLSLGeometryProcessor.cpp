...
void GrGLSLGeometryProcessor::collectTransforms(
  ...
  SkASSERT(
      localCoordsVar.getType() == kFloat2_GrSLType || localCoordsVar.getType() == kFloat3_GrSLType);
  ...
  auto getBaseLocalCoord = [&baseLocalCoord, &localCoordsVar, vb, varyingHandler]() {
    ...
    return GrShaderVar(
        SkString(baseLocalCoord.fsIn()), baseLocalCoord.type(), GrShaderVar::TypeModifier::In);
  };
  ...
  for (int i = 0; *handler; ++*handler, ++i) {
    ...
    while (node) {
      SkASSERT(!node->isSampledWithExplicitCoords() && !node->sampleUsage().hasVariableMatrix());
    ...
    if (coordOwner) {
      ...
      if (transformedLocalCoord.getType() == kVoid_GrSLType) {
        ...
        fTransformInfos.push_back({GrShaderVar(v.vsOut(), v.type()), localCoordsVar, coordOwner});
...
void GrGLSLGeometryProcessor::emitTransformCode(
  ...
  for (const auto& tr : fTransformInfos) {
    ...
    if (tr.fOutputCoords.getType() == kFloat2_GrSLType) {
      vb->codeAppendf(
          "%s = ((%s) * %s).xy", tr.fOutputCoords.getName().c_str(), transformExpression.c_str(),
          localCoords.c_str());
    } else {
      SkASSERT(tr.fOutputCoords.getType() == kFloat3_GrSLType);
      vb->codeAppendf(
          "%s = (%s) * %s", tr.fOutputCoords.getName().c_str(), transformExpression.c_str(),
          localCoords.c_str());
    }
    vb->codeAppend(";\n");
    vb->codeAppend("}\n");
