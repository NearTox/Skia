...
SkMatrix GrGLSLPrimitiveProcessor::GetTransformMatrix(
    const GrCoordTransform& coordTransform, const SkMatrix& preMatrix) {
  SkMatrix combined;
  combined.setConcat(coordTransform.matrix(), preMatrix);
  if (coordTransform.normalize()) {
    SkMatrixPriv::PostIDiv(
        &combined, coordTransform.peekTexture()->width(), coordTransform.peekTexture()->height());
  }
...
void GrGLSLPrimitiveProcessor::setupUniformColor(
