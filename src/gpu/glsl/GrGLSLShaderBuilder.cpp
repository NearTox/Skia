...
void GrGLSLShaderBuilder::appendFunctionDecl(
    GrSLType returnType, const char* mangledName, SkSpan<const GrShaderVar> args,
    bool forceInline) {
  this->functions().appendf(
      "%s%s %s(", forceInline ? "inline " : "", GrGLSLTypeString(returnType), mangledName);
...
void GrGLSLShaderBuilder::emitFunction(
    GrSLType returnType, const char* mangledName, SkSpan<const GrShaderVar> args, const char* body,
    bool forceInline) {
...
void GrGLSLShaderBuilder::emitFunctionPrototype(
    GrSLType returnType, const char* mangledName, SkSpan<const GrShaderVar> args,
    bool forceInline) {
  this->appendFunctionDecl(returnType, mangledName, args, forceInline);
  this->functions().append(";\n");
...
void GrGLSLShaderBuilder::appendTextureLookup(
    SkString* out, SamplerHandle samplerHandle, const char* coordName) const {
...
void GrGLSLShaderBuilder::appendTextureLookup(
    SamplerHandle samplerHandle, const char* coordName,
...
void GrGLSLShaderBuilder::appendTextureLookupAndBlend(
    const char* dst, SkBlendMode mode, SamplerHandle samplerHandle, const char* coordName,
    GrGLSLColorSpaceXformHelper* colorXformHelper) {
  if (!dst) {
    dst = "half4(1)";
  }
...
void GrGLSLShaderBuilder::appendColorGamutXform(
    SkString* out, const char* srcColor, GrGLSLColorSpaceXformHelper* colorXformHelper) {
  ...
  auto emitTFFunc = [=](const char* name, GrGLSLProgramDataManager::UniformHandle uniform,
    ...
    this->emitFunction(
        kHalf_GrSLType, funcName.c_str(), {gTFArgs, SK_ARRAY_COUNT(gTFArgs)}, body.c_str());
    return funcName;
  };
  ...
  if (colorXformHelper->applySrcTF()) {
    srcTFFuncName =
        emitTFFunc("src_tf", colorXformHelper->srcTFUniform(), colorXformHelper->srcTFKind());
  }

  SkString dstTFFuncName;
  if (colorXformHelper->applyDstTF()) {
    dstTFFuncName =
        emitTFFunc("dst_tf", colorXformHelper->dstTFUniform(), colorXformHelper->dstTFKind());
  }
  ...
  if (colorXformHelper->applyGamutXform()) {
    ...
    this->emitFunction(
        kHalf4_GrSLType, gamutXformFuncName.c_str(),
  ...
  // Now define a wrapper function that applies all the intermediate steps
  {
    ...
    this->emitFunction(
        kHalf4_GrSLType, colorXformFuncName.c_str(),
...
void GrGLSLShaderBuilder::appendColorGamutXform(
    const char* srcColor, GrGLSLColorSpaceXformHelper* colorXformHelper) {
  SkString xform;
...
void GrGLSLShaderBuilder::addLayoutQualifier(const char* param, InterfaceQualifier interface) {
  SkASSERT(
      fProgramBuilder->shaderCaps()->generation() >= k330_GrGLSLGeneration ||
