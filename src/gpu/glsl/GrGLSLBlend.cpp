...
static void hard_light(GrGLSLFragmentBuilder* fsBuilder,
    ...
    for (size_t i = 0; i < SK_ARRAY_COUNT(kComponents); ++i) {
        ...
        fsBuilder->codeAppendf("%s.%c = 2.0 * %s.%c * %s.%c;", final, component, src, component,
                               dst, component);
...
static void color_dodge_component(GrGLSLFragmentBuilder* fsBuilder,
    ...
    fsBuilder->codeAppend("} else {");
    fsBuilder->codeAppendf("d = min(%s.a, %s.%c * %s.a / (d %s));", dst, dst, component, src,
                           divisorGuard);
...
static void color_burn_component(GrGLSLFragmentBuilder* fsBuilder,
    ...
    fsBuilder->codeAppend("} else {");
    fsBuilder->codeAppendf("half d = max(0.0, %s.a - (%s.a - %s.%c) * %s.a / (%s.%c %s));", dst,
                           dst, dst, component, src, src, component, divisorGuard);
...
static void soft_light_component_pos_dst_alpha(GrGLSLFragmentBuilder* fsBuilder,
    ...
    fsBuilder->codeAppendf(
            ...
            final, component, dst, component, dst, component, src, src, component, dst,
            divisorGuard, dst, src, component, dst, component, src, src, component);
    // else if (4D < Da)
    ...
    fsBuilder->codeAppendf(
            ...
            final, component, dst, component, src, src, component, src, component, dst, dst,
            component, src, src, component, dst, src, component);
    fsBuilder->codeAppendf("}");
}
...
static void add_lum_function(GrGLSLFragmentBuilder* fsBuilder, SkString* setLumFunction) {
    ...
    fsBuilder->emitFunction(kHalf_GrSLType, "luminance", SK_ARRAY_COUNT(getLumArgs), getLumArgs,
                            getLumBody.c_str(), &getFunction);
    ...
    setLumBody.append(
            ...
            "}"
            "return outColor;");
    fsBuilder->emitFunction(kHalf3_GrSLType, "set_luminance", SK_ARRAY_COUNT(setLumArgs),
                            setLumArgs, setLumBody.c_str(), setLumFunction);
}
...
static void add_sat_function(GrGLSLFragmentBuilder* fsBuilder, SkString* setSatFunction) {
    ...
    fsBuilder->emitFunction(kHalf_GrSLType, "saturation", SK_ARRAY_COUNT(getSatArgs), getSatArgs,
                            getSatBody.c_str(), &getFunction);
    ...
    static const char kHelperBody[] =
            ...
            "return half3(0, 0, 0);"
            "}";
    fsBuilder->emitFunction(kHalf3_GrSLType, "set_saturation_helper", SK_ARRAY_COUNT(helperArgs),
                            helperArgs, kHelperBody, &helperFunction);
    ...
    fsBuilder->emitFunction(kHalf3_GrSLType, "set_saturation", SK_ARRAY_COUNT(setSatArgs),
                            setSatArgs, setSatBody.c_str(), setSatFunction);
}
...
static void emit_advanced_xfermode_code(GrGLSLFragmentBuilder* fsBuilder, const char* srcColor,
    ...
    fsBuilder->codeAppendf("%s.a = %s.a + (1.0 - %s.a) * %s.a;", outputColor, srcColor, srcColor,
                           dstColor);
