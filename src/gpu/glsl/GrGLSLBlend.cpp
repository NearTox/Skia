...
static void hard_light(
    GrGLSLFragmentBuilder* fsBuilder, const char* final, const char* src, const char* dst) {
  ...
  for (size_t i = 0; i < SK_ARRAY_COUNT(kComponents); ++i) {
    ...
    fsBuilder->codeAppendf(
        "%s.%c = 2.0 * %s.%c * %s.%c;", final, component, src, component, dst, component);
    fsBuilder->codeAppend("} else {");
    fsBuilder->codeAppendf(
        "%s.%c = %s.a * %s.a - 2.0 * (%s.a - %s.%c) * (%s.a - %s.%c);", final, component, src, dst,
        dst, dst, component, src, src, component);
    fsBuilder->codeAppend("}");
  }
  fsBuilder->codeAppendf(
      "%s.rgb += %s.rgb * (1.0 - %s.a) + %s.rgb * (1.0 - %s.a);", final, src, dst, dst, src);
}

// Does one component of color-dodge
static void color_dodge_component(
    GrGLSLFragmentBuilder* fsBuilder, const char* final, const char* src, const char* dst,
  ...
  fsBuilder->codeAppendf(
      "%s.%c = %s.a * %s.a + %s.%c * (1.0 - %s.a) + %s.%c * (1.0 - %s.a);", final, component, src,
      dst, src, component, dst, dst, component, src);
  fsBuilder->codeAppend("} else {");
  fsBuilder->codeAppendf(
      "d = min(%s.a, %s.%c * %s.a / (d %s));", dst, dst, component, src, divisorGuard);
  fsBuilder->codeAppendf(
      "%s.%c = d * %s.a + %s.%c * (1.0 - %s.a) + %s.%c * (1.0 - %s.a);", final, component, src, src,
      component, dst, dst, component, src);
  fsBuilder->codeAppend("}");
  fsBuilder->codeAppend("}");
}

// Does one component of color-burn
static void color_burn_component(
    GrGLSLFragmentBuilder* fsBuilder, const char* final, const char* src, const char* dst,
    const char component) {
  const char* divisorGuard = "";
  const GrShaderCaps* shaderCaps = fsBuilder->getProgramBuilder()->shaderCaps();
  ...
  fsBuilder->codeAppend("} else {");
  fsBuilder->codeAppendf(
      "half d = max(0.0, %s.a - (%s.a - %s.%c) * %s.a / (%s.%c %s));", dst, dst, dst, component,
      src, src, component, divisorGuard);
  fsBuilder->codeAppendf(
      "%s.%c = %s.a * d + %s.%c * (1.0 - %s.a) + %s.%c * (1.0 - %s.a);", final, component, src, src,
      component, dst, dst, component, src);
  fsBuilder->codeAppend("}");
}

// Does one component of soft-light. Caller should have already checked that dst alpha > 0.
static void soft_light_component_pos_dst_alpha(
    GrGLSLFragmentBuilder* fsBuilder, const char* final, const char* src, const char* dst,
    const char component) {
  const char* divisorGuard = "";
  const GrShaderCaps* shaderCaps = fsBuilder->getProgramBuilder()->shaderCaps();
  ...
  fsBuilder->codeAppendf(
      "%s.%c = (%s.%c*%s.%c*(%s.a - 2.0*%s.%c)) / (%s.a %s) +"
      "(1.0 - %s.a) * %s.%c + %s.%c*(-%s.a + 2.0*%s.%c + 1.0);",
      final, component, dst, component, dst, component, src, src, component, dst, divisorGuard, dst,
      src, component, dst, component, src, src, component);
  // else if (4D < Da)
  fsBuilder->codeAppendf("} else if (4.0 * %s.%c <= %s.a) {", dst, component, dst);
  fsBuilder->codeAppendf("half DSqd = %s.%c * %s.%c;", dst, component, dst, component);
  ...
  fsBuilder->codeAppendf(
      ...
      component, src, src, component, src, component, divisorGuard);
...
static void add_lum_function(GrGLSLFragmentBuilder* fsBuilder, SkString* setLumFunction) {
  ...
  fsBuilder->emitFunction(
      kHalf_GrSLType, "luminance", SK_ARRAY_COUNT(getLumArgs), getLumArgs, getLumBody.c_str(),
      &getFunction);
  ...
  setLumBody.append(
      ...
      "return outColor;");
  fsBuilder->emitFunction(
      kHalf3_GrSLType, "set_luminance", SK_ARRAY_COUNT(setLumArgs), setLumArgs, setLumBody.c_str(),
      setLumFunction);
}
...
static void add_sat_function(GrGLSLFragmentBuilder* fsBuilder, SkString* setSatFunction) {
  ...
  fsBuilder->emitFunction(
      kHalf_GrSLType, "saturation", SK_ARRAY_COUNT(getSatArgs), getSatArgs, getSatBody.c_str(),
      &getFunction);
  ...
  static const char kHelperBody[] =
      ...
      "}";
  fsBuilder->emitFunction(
      kHalf3_GrSLType, "set_saturation_helper", SK_ARRAY_COUNT(helperArgs), helperArgs, kHelperBody,
      &helperFunction);
  ...
  setSatBody.appendf(
      ...
      "}"
      "return hueLumColor;",
      getFunction.c_str(), helpFunc, helpFunc, helpFunc, helpFunc, helpFunc, helpFunc);
  fsBuilder->emitFunction(
      kHalf3_GrSLType, "set_saturation", SK_ARRAY_COUNT(setSatArgs), setSatArgs, setSatBody.c_str(),
      setSatFunction);
}

static void emit_advanced_xfermode_code(
    GrGLSLFragmentBuilder* fsBuilder, const char* srcColor, const char* dstColor,
    const char* outputColor, SkBlendMode mode) {
  SkASSERT(srcColor);
  SkASSERT(dstColor);
  SkASSERT(outputColor);
  // These all perform src-over on the alpha channel.
  fsBuilder->codeAppendf(
      "%s.a = %s.a + (1.0 - %s.a) * %s.a;", outputColor, srcColor, srcColor, dstColor);
  ...
  switch (mode) {
    ...
    case SkBlendMode::kDarken:
      fsBuilder->codeAppendf(
          "%s.rgb = min((1.0 - %s.a) * %s.rgb + %s.rgb, "
          "(1.0 - %s.a) * %s.rgb + %s.rgb);",
          outputColor, srcColor, dstColor, srcColor, dstColor, srcColor, dstColor);
      break;
    case SkBlendMode::kLighten:
      fsBuilder->codeAppendf(
          "%s.rgb = max((1.0 - %s.a) * %s.rgb + %s.rgb, "
          "(1.0 - %s.a) * %s.rgb + %s.rgb);",
          outputColor, srcColor, dstColor, srcColor, dstColor, srcColor, dstColor);
      break;
    ...
    case SkBlendMode::kColorBurn:
      ...
      break;
    case SkBlendMode::kHardLight: hard_light(fsBuilder, outputColor, srcColor, dstColor); break;
    ...
    case SkBlendMode::kMultiply:
      fsBuilder->codeAppendf(
          ...
          "(1.0 - %s.a) * %s.rgb + "
          "%s.rgb * %s.rgb;",
          outputColor, srcColor, dstColor, dstColor, srcColor, srcColor, dstColor);
      break;
    case SkBlendMode::kHue: {
      ...
      fsBuilder->codeAppendf("half4 dstSrcAlpha = %s * %s.a;", dstColor, srcColor);
      fsBuilder->codeAppendf(
          "%s.rgb = %s(%s(%s.rgb * %s.a, dstSrcAlpha.rgb),"
          "dstSrcAlpha.a, dstSrcAlpha.rgb);",
          outputColor, setLum.c_str(), setSat.c_str(), srcColor, dstColor);
      fsBuilder->codeAppendf(
          "%s.rgb += (1.0 - %s.a) * %s.rgb + (1.0 - %s.a) * %s.rgb;", outputColor, srcColor,
    ...
    case SkBlendMode::kSaturation: {
      ...
      fsBuilder->codeAppendf("half4 dstSrcAlpha = %s * %s.a;", dstColor, srcColor);
      fsBuilder->codeAppendf(
          "%s.rgb = %s(%s(dstSrcAlpha.rgb, %s.rgb * %s.a),"
          "dstSrcAlpha.a, dstSrcAlpha.rgb);",
          outputColor, setLum.c_str(), setSat.c_str(), srcColor, dstColor);
      fsBuilder->codeAppendf(
          "%s.rgb += (1.0 - %s.a) * %s.rgb + (1.0 - %s.a) * %s.rgb;", outputColor, srcColor,
    ...
    case SkBlendMode::kColor: {
      ...
      SkString setLum;
      add_lum_function(fsBuilder, &setLum);
      fsBuilder->codeAppendf("half4 srcDstAlpha = %s * %s.a;", srcColor, dstColor);
      fsBuilder->codeAppendf(
          "%s.rgb = %s(srcDstAlpha.rgb, srcDstAlpha.a, %s.rgb * %s.a);", outputColor,
    ...
    case SkBlendMode::kLuminosity: {
      ...
      fsBuilder->codeAppendf("half4 srcDstAlpha = %s * %s.a;", srcColor, dstColor);
      fsBuilder->codeAppendf(
          "%s.rgb = %s(%s.rgb * %s.a, srcDstAlpha.a, srcDstAlpha.rgb);", outputColor,
          setLum.c_str(), dstColor, srcColor);
      fsBuilder->codeAppendf(
          "%s.rgb += (1.0 - %s.a) * %s.rgb + (1.0 - %s.a) * %s.rgb;", outputColor, srcColor,
          dstColor, dstColor, srcColor);
    ...
    default: SK_ABORT("Unknown Custom Xfer mode."); break;
  }
}
...
static bool append_porterduff_term(
    GrGLSLFragmentBuilder* fsBuilder, SkBlendModeCoeff coeff, const char* colorName,
    const char* srcColorName, const char* dstColorName, bool hasPrevious) {
  if (SkBlendModeCoeff::kZero == coeff) {
    ...
    switch (coeff) {
      case SkBlendModeCoeff::kOne: break;
      case SkBlendModeCoeff::kSC: fsBuilder->codeAppendf(" * %s", srcColorName); break;
      case SkBlendModeCoeff::kISC:
        fsBuilder->codeAppendf(" * (half4(1.0) - %s)", srcColorName);
        break;
      case SkBlendModeCoeff::kDC: fsBuilder->codeAppendf(" * %s", dstColorName); break;
      case SkBlendModeCoeff::kIDC:
        fsBuilder->codeAppendf(" * (half4(1.0) - %s)", dstColorName);
        break;
      case SkBlendModeCoeff::kSA: fsBuilder->codeAppendf(" * %s.a", srcColorName); break;
      case SkBlendModeCoeff::kISA: fsBuilder->codeAppendf(" * (1.0 - %s.a)", srcColorName); break;
      case SkBlendModeCoeff::kDA: fsBuilder->codeAppendf(" * %s.a", dstColorName); break;
      case SkBlendModeCoeff::kIDA: fsBuilder->codeAppendf(" * (1.0 - %s.a)", dstColorName); break;
      default: SK_ABORT("Unsupported Blend Coeff");
    }
    return true;
  }
}
...
void GrGLSLBlend::AppendMode(
  ...
  if (SkBlendMode_AsCoeff(mode, &srcCoeff, &dstCoeff)) {
    ...
    // append src blend
    bool didAppend =
        append_porterduff_term(fsBuilder, srcCoeff, srcColor, srcColor, dstColor, false);
    // append dst blend
...
void GrGLSLBlend::AppendRegionOp(
    GrGLSLFragmentBuilder* fsBuilder, const char* srcColor, const char* dstColor,
  ...
  fsBuilder->codeAppendf("%s = ", outColor);
  // append src blend
  bool didAppend = append_porterduff_term(fsBuilder, srcCoeff, srcColor, srcColor, dstColor, false);
  // append dst blend
  if (!append_porterduff_term(fsBuilder, dstCoeff, dstColor, srcColor, dstColor, didAppend)) {
    fsBuilder->codeAppend("half4(0, 0, 0, 0)");
  }
