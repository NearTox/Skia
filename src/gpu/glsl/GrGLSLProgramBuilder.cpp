...
SkString GrGLSLProgramBuilder::emitFragProc(
...
void GrGLSLProgramBuilder::writeFPFunction(
  ...
  if (this->fragmentProcessorHasCoordsParam(&fp)) {
    ...
    switch (varying.getType()) {
      case kVoid_GrSLType: SkASSERT(!fp.usesSampleCoordsDirectly()); break;
  ...
  fFS.emitFunction(
      kHalf4_GrSLType, impl.functionName(), SkMakeSpan(params, numParams), fFS.code().c_str());
  fFS.deleteStage();
}
...
bool GrGLSLProgramBuilder::emitAndInstallDstTexture() {
  ...
  if (this->pipeline().usesDstTexture()) {
    ...
    fUniformHandles.fDstTextureCoordsUni = this->uniformHandler()->addUniform(
        /*owner=*/nullptr, kFragment_GrShaderFlag, kHalf4_GrSLType, "DstTextureCoords",
...
void GrGLSLProgramBuilder::addRTFlipUniform(const char* name) {
  ...
  fUniformHandles.fRTFlipUni = uniformHandler->internalAddUniformArray(
      nullptr, kFragment_GrShaderFlag, kHalf2_GrSLType, name, false, 0, nullptr);
}
