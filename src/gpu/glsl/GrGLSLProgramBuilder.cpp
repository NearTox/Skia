...
void GrGLSLProgramBuilder::emitAndInstallPrimProc(SkString* outputColor, SkString* outputCoverage) {
  ...
  for (int i = 0; i < proc.numTextureSamplers(); ++i) {
    ...
    texSamplers[i] = this->emitSampler(
        proc.textureSampler(i).backendFormat(), sampler.samplerState(), sampler.swizzle(),
        name.c_str());
  }

  GrGLSLPrimitiveProcessor::FPCoordTransformHandler transformHandler(
      this->pipeline(), &fTransformedCoordVars);
  GrGLSLGeometryProcessor::EmitArgs args(
      ...
      outputCoverage->c_str(), rtAdjustName, texSamplers.get(), &transformHandler);
...
void GrGLSLProgramBuilder::emitAndInstallFragProcs(SkString* color, SkString* coverage) {
  ...
  for (int i = 0; i < this->pipeline().numFragmentProcessors(); ++i) {
    ...
    output = this->emitAndInstallFragProc(
        fp, i, transformedCoordVarsIdx, **inOut, output, &glslFragmentProcessors);
...
SkString GrGLSLProgramBuilder::emitAndInstallFragProc(
    const GrFragmentProcessor& fp, int index, int transformedCoordVarsIdx, const SkString& input,
    SkString output, SkTArray<std::unique_ptr<GrGLSLFragmentProcessor>>* glslFragmentProcessors) {
  SkASSERT(input.size());
  // Program builders have a bit of state we need to clear with each effect
  AutoStageAdvance adv(this);
  ...
  for (const auto& subFP : GrFragmentProcessor::FPCRange(fp)) {
    for (int i = 0; i < subFP.numTextureSamplers(); ++i) {
      ...
      texSamplers.emplace_back(this->emitSampler(
          sampler.view().proxy()->backendFormat(), sampler.samplerState(), sampler.view().swizzle(),
          name.c_str()));
    }
  }
  const GrGLSLPrimitiveProcessor::TransformVar* coordVars =
      fTransformedCoordVars.begin() + transformedCoordVarsIdx;
  GrGLSLFragmentProcessor::TransformedCoordVars coords(&fp, coordVars);
  GrGLSLFragmentProcessor::TextureSamplers textureSamplers(&fp, texSamplers.begin());
  GrGLSLFragmentProcessor::EmitArgs args(
      &fFS, this->uniformHandler(), this->shaderCaps(), fp, output.c_str(), input.c_str(), coords,
      textureSamplers);
...
void GrGLSLProgramBuilder::emitAndInstallXferProc(
  ...
  if (GrTextureProxy* dstTextureProxy = dstView.asTextureProxy()) {
    ...
    dstTextureSamplerHandle = this->emitSampler(
