...
void GrGLSLVaryingHandler::finalize() {
  for (int i = 0; i < fVaryings.count(); ++i) {
    ...
    if (v.fVisibility & kVertex_GrShaderFlag) {
      fVertexOutputs.push_back().set(
          v.fType, v.fVsOut, GrShaderVar::kOut_TypeModifier, nullptr, modifier);
      if (v.fVisibility & kGeometry_GrShaderFlag) {
        fGeomInputs.push_back().set(
            v.fType, v.fVsOut, GrShaderVar::kUnsizedArray, GrShaderVar::kIn_TypeModifier, nullptr,
            modifier);
      }
    }
    if (v.fVisibility & kFragment_GrShaderFlag) {
      ...
      if (v.fVisibility & kGeometry_GrShaderFlag) {
        fGeomOutputs.push_back().set(
            v.fType, v.fGsOut, GrShaderVar::kOut_TypeModifier, nullptr, modifier);
        fsIn = v.fGsOut.c_str();
      }
      fFragInputs.push_back().set(v.fType, fsIn, GrShaderVar::kIn_TypeModifier, nullptr, modifier);
    }
  }
  this->onFinalize();
}
