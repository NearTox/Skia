...
void GrGLSLVaryingHandler::addPassThroughAttribute(
    const GrGeometryProcessor::Attribute& input, const char* output, Interpolation interpolation) {
...
static bool use_flat_interpolation(
    GrGLSLVaryingHandler::Interpolation interpolation, const GrShaderCaps& shaderCaps) {
  switch (interpolation) {
    using Interpolation = GrGLSLVaryingHandler::Interpolation;
    case Interpolation::kInterpolated: return false;
    case Interpolation::kCanBeFlat:
      SkASSERT(!shaderCaps.preferFlatInterpolation() || shaderCaps.flatInterpolationSupport());
...
void GrGLSLVaryingHandler::addVarying(
    const char* name, GrGLSLVarying* varying, Interpolation interpolation) {
...
void GrGLSLVaryingHandler::finalize() {
  for (const VaryingInfo& v : fVaryings.items()) {
    ...
    if (v.fVisibility & kVertex_GrShaderFlag) {
      fVertexOutputs.emplace_back(
          v.fVsOut, v.fType, GrShaderVar::TypeModifier::Out, GrShaderVar::kNonArray, SkString(),
          SkString(modifier));
      if (v.fVisibility & kGeometry_GrShaderFlag) {
        fGeomInputs.emplace_back(
            v.fVsOut, v.fType, GrShaderVar::TypeModifier::In, GrShaderVar::kUnsizedArray,
            SkString(), SkString(modifier));
      }
    }
    if (v.fVisibility & kFragment_GrShaderFlag) {
      const char* fsIn = v.fVsOut.c_str();
      if (v.fVisibility & kGeometry_GrShaderFlag) {
        fGeomOutputs.emplace_back(
            v.fGsOut, v.fType, GrShaderVar::TypeModifier::Out, GrShaderVar::kNonArray, SkString(),
            SkString(modifier));
        fsIn = v.fGsOut.c_str();
      }
      fFragInputs.emplace_back(
          SkString(fsIn), v.fType, GrShaderVar::TypeModifier::In, GrShaderVar::kNonArray,
          SkString(), SkString(modifier));
    }
  }
  this->onFinalize();
...
void GrGLSLVaryingHandler::getFragDecls(SkString* inputDecls, SkString* outputDecls) const {
  ...
  SkASSERT(
      k110_GrGLSLGeneration != fProgramBuilder->shaderCaps()->generation() || fFragOutputs.empty());
