...
void GrGLSLFragmentShaderBuilder::enableCustomOutput() {
  if (!fHasCustomColorOutput) {
    ...
    fOutputs.push_back().set(
        kHalf4_GrSLType, DeclaredColorOutputName(), GrShaderVar::kOut_TypeModifier);
...
void GrGLSLFragmentShaderBuilder::enableSecondaryOutput() {
  ...
  if (caps.mustDeclareFragmentShaderOutput()) {
    fOutputs.push_back().set(
        kHalf4_GrSLType, DeclaredSecondaryColorOutputName(), GrShaderVar::kOut_TypeModifier);
...
void GrGLSLFragmentShaderBuilder::onFinalize() {
  SkASSERT(
      fProgramBuilder->header().processorFeatures() == fUsedProcessorFeaturesAllStages_DebugOnly);
