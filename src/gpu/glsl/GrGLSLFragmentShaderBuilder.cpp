...
static const char* specific_layout_qualifier_name(GrBlendEquation equation) {
  ...
  static const char* kLayoutQualifierNames[] = {
      ...
      "blend_support_exclusion",      "blend_support_multiply",   "blend_support_hsl_hue",
  ...
  static_assert(
      SK_ARRAY_COUNT(kLayoutQualifierNames) ==
...
SkString GrGLSLFragmentShaderBuilder::ensureCoords2D(
    const GrShaderVar& coords, const SkSL::SampleMatrix& matrix) {
  SkString result;
  if (!coords.getName().size()) {
    ...
    this->codeAppendf(
        "\tfloat2 %s = %s.xy / %s.z;", coords2D.c_str(), coords.c_str(), coords.c_str());
    result = coords2D;
  }
  switch (matrix.fKind) {
...
SkString GrGLSLFPFragmentBuilder::writeProcessorFunction(
  ...
  if (args.fFp.isSampledWithExplicitCoords() && args.fTransformedCoords.count() > 0) {
    ...
    switch (transform.getType()) {
      case kFloat4_GrSLType:
        ...
        this->codeAppendf(
            "_coords = _coords * %s.xz + %s.yw;\n", transform.c_str(), transform.c_str());
        break;
      case kFloat3x3_GrSLType:
        ...
        this->codeAppend("}\n");
        break;
      default: SkASSERT(transform.getType() == kVoid_GrSLType); break;
    }
  ...
  SkString result;
  this->emitFunction(
      kHalf4_GrSLType, args.fFp.name(),
      args.fFp.isSampledWithExplicitCoords() || hasVariableMatrix ? 2 : 1, params,
...
void GrGLSLFragmentShaderBuilder::enableAdvancedBlendEquationIfNeeded(GrBlendEquation equation) {
  ...
  this->addFeature(
      1 << kBlendEquationAdvanced_GLSLPrivateFeature, "GL_KHR_blend_equation_advanced");
...
void GrGLSLFragmentShaderBuilder::enableCustomOutput() {
  if (!fCustomColorOutput) {
    fCustomColorOutput = &fOutputs.emplace_back(
        DeclaredColorOutputName(), kHalf4_GrSLType, GrShaderVar::TypeModifier::Out);
...
void GrGLSLFragmentShaderBuilder::enableSecondaryOutput() {
  ...
  if (caps.mustDeclareFragmentShaderOutput()) {
    fOutputs.emplace_back(
        DeclaredSecondaryColorOutputName(), kHalf4_GrSLType, GrShaderVar::TypeModifier::Out);
