...
GrShaderVar GrGLSLUniformHandler::getUniformMapping(
    const GrFragmentProcessor& owner, SkString rawName) const {
...
GrShaderVar GrGLSLUniformHandler::liftUniformToVertexShader(
    const GrFragmentProcessor& owner, SkString rawName) {
  for (int i = this->numUniforms() - 1; i >= 0; i--) {
