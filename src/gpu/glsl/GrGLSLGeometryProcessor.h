...
class GrGLSLGeometryProcessor : public GrGLSLPrimitiveProcessor {
 public:
  ...
  void emitTransformCode(GrGLSLVertexBuilder* vb, GrGLSLUniformHandler* uniformHandler) override;

 protected:
  // A helper which subclasses can use to upload coord transform matrices in setData().
  void setTransformDataHelper(const GrGLSLProgramDataManager& pdman, const CoordTransformRange&);
  ...
  void writeLocalCoord(
      GrGLSLVertexBuilder*, GrGLSLUniformHandler*, GrGPArgs*, GrShaderVar localVar,
  ...
  static uint32_t AddMatrixKeys(
      uint32_t flags, const SkMatrix& viewMatrix, const SkMatrix& localMatrix) {
