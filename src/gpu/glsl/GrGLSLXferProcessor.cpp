...
static void adjust_for_lcd_coverage(GrGLSLXPFragmentBuilder* fragBuilder,
    ...
    if (srcCoverage && proc.isLCD()) {
        fragBuilder->codeAppendf("%s.a = max(max(%s.r, %s.g), %s.b);", srcCoverage, srcCoverage,
                                 srcCoverage, srcCoverage);
    }
}
...
void GrGLSLXferProcessor::emitCode(const EmitArgs& args) {
    ...
    this->emitOutputSwizzle(args.fXPFragBuilder, args.fOutputSwizzle, args.fOutputPrimary,
                            args.fOutputSecondary);
}

void GrGLSLXferProcessor::emitOutputSwizzle(GrGLSLXPFragmentBuilder* x, const GrSwizzle& swizzle,
                                            const char* outColor,
...
void GrGLSLXferProcessor::DefaultCoverageModulation(GrGLSLXPFragmentBuilder* fragBuilder,
    ...
    if (proc.dstReadUsesMixedSamples()) {
        ...
        if (proc.isLCD()) {
            fragBuilder->codeAppendf("half lerpRed = mix(%s.a, %s.a, %s.r);", dstColor, outColor,
                                     srcCoverage);
            fragBuilder->codeAppendf("half lerpBlue = mix(%s.a, %s.a, %s.g);", dstColor, outColor,
                                     srcCoverage);
            fragBuilder->codeAppendf("half lerpGreen = mix(%s.a, %s.a, %s.b);", dstColor, outColor,
                                     srcCoverage);
        }
        fragBuilder->codeAppendf("%s = %s * %s + (half4(1.0) - %s) * %s;", outColor, srcCoverage,
