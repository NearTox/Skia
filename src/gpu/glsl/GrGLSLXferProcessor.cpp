    ...
    if (args.fDstTextureSamplerHandle.isValid()) {
      ...
      if (GrDstSampleTypeUsesTexture(args.fDstSampleType)) {
        ...
        fDstTopLeftUni = uniformHandler->addUniform(
            nullptr, kFragment_GrShaderFlag, kHalf2_GrSLType, "DstTextureUpperLeft",
            &dstTopLeftName);
        ...
        fragBuilder->codeAppendf(
            "half2 _dstTexCoord = (half2(sk_FragCoord.xy) - %s) * %s;", dstTopLeftName,
            dstCoordScaleName);
...
void GrGLSLXferProcessor::setData(
    const GrGLSLProgramDataManager& pdm, const GrXferProcessor& xp, const GrTexture* dstTexture,
    const SkIPoint& dstTextureOffset) {
  if (dstTexture) {
    if (fDstTopLeftUni.isValid()) {
      pdm.set2f(
          fDstTopLeftUni, static_cast<float>(dstTextureOffset.fX),
...
void GrGLSLXferProcessor::DefaultCoverageModulation(
  ...
  if (proc.dstReadUsesMixedSamples()) {
    ...
    if (proc.isLCD()) {
      fragBuilder->codeAppendf(
          "half lerpRed = mix(%s.a, %s.a, %s.r);", dstColor, outColor, srcCoverage);
      fragBuilder->codeAppendf(
          "half lerpBlue = mix(%s.a, %s.a, %s.g);", dstColor, outColor, srcCoverage);
      fragBuilder->codeAppendf(
          "half lerpGreen = mix(%s.a, %s.a, %s.b);", dstColor, outColor, srcCoverage);
    }
    fragBuilder->codeAppendf(
