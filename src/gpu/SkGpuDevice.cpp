...
sk_sp<SkGpuDevice> SkGpuDevice::Make(
    GrRecordingContext* context, std::unique_ptr<GrRenderTargetContext> renderTargetContext,
    InitContents init) {
...
bool SkGpuDevice::onReadPixels(const SkPixmap& pm, int x, int y) {
  ...
  return fRenderTargetContext->readPixels(
      dContext, pm.info(), pm.writable_addr(), pm.rowBytes(), {x, y});
}
...
void SkGpuDevice::replaceRenderTargetContext(SkSurface::ContentChangeMode mode) {
  ...
  auto newRTC = MakeRenderTargetContext(
      this->recordingContext(), budgeted, this->imageInfo(), fRenderTargetContext->numSamples(),
...
void SkGpuDevice::drawPoints(
  ...
  GrPaint grPaint;
  if (!SkPaintToGrPaint(
          this->recordingContext(), fRenderTargetContext->colorInfo(), paint, *matrixProvider,
          &grPaint)) {
    return;
  }
...
void SkGpuDevice::drawStrokedLine(const SkPoint points[2], const SkPaint& origPaint) {
  ...
  GrPaint grPaint;
  if (!SkPaintToGrPaint(
          this->recordingContext(), fRenderTargetContext->colorInfo(), newPaint,
          this->asMatrixProvider(), &grPaint)) {
    return;
  }
...
bool SkGpuDevice::wait(
  ...
  return fRenderTargetContext->waitOnSemaphores(
