...
void SkGpuDevice::replaceRenderTargetContext(
    std::unique_ptr<GrRenderTargetContext> rtc, SkSurface::ContentChangeMode mode) {
  ...
  if (mode == SkSurface::kRetain_ContentChangeMode) {
    ...
    SkAssertResult(rtc->blitTexture(
        fRenderTargetContext->readSurfaceView(), SkIRect::MakeWH(this->width(), this->height()),
        SkIPoint::Make(0, 0)));
  }
...
void SkGpuDevice::drawPaint(const SkPaint& paint) {
  ...
  GrPaint grPaint;
  if (!SkPaintToGrPaint(
          this->context(), fRenderTargetContext->colorInfo(), paint, this->asMatrixProvider(),
          &grPaint)) {
    return;
  }
...
void SkGpuDevice::drawPoints(
  ...
  if (GrIsPrimTypeLines(primitiveType) || GrPrimitiveType::kPoints == primitiveType) {
    ...
    matrixProvider =
        postConcatMatrixProvider.init(*matrixProvider, SkMatrix::MakeTrans(kOffset, kOffset));
  }
#endif

  GrPaint grPaint;
  if (!SkPaintToGrPaint(
          this->context(), fRenderTargetContext->colorInfo(), paint, *matrixProvider, &grPaint)) {
    return;
  }
...
void SkGpuDevice::drawStrokedLine(const SkPoint points[2], const SkPaint& origPaint) {
  ...
  GrPaint grPaint;
  if (!SkPaintToGrPaint(
          this->context(), fRenderTargetContext->colorInfo(), newPaint, matrixProvider, &grPaint)) {
    return;
  }

  fRenderTargetContext->fillRectWithLocalMatrix(
      this->clip(), std::move(grPaint), GrAA(newPaint.isAntiAlias()),
...
void SkGpuDevice::drawSpecial(
  ...
  if (!SkPaintToGrPaintReplaceShader(
          this->context(), fRenderTargetContext->colorInfo(), tmpUnfiltered,
  ...
  if (clipImage) {
    ...
    if (clipView && ctm.invert(&inverseClipMatrix)) {
      ...
      cfp = GrTextureEffect::Make(
          std::move(clipView), clipImage->alphaType(), inverseClipMatrix, filter);
...
void SkGpuDevice::drawImageNine(
  ...
  if (GrSurfaceProxyView view = as_IB(image)->refPinnedView(this->context(), &pinnedUniqueID)) {
    GrTextureAdjuster adjuster(
        this->context(), std::move(view), image->imageInfo().colorInfo(), pinnedUniqueID);
...
static bool init_vertices_paint(
    ...
    const SkMatrixProvider& matrixProvider, SkBlendMode bmode, bool hasTexs, bool hasColors,
    GrPaint* grPaint) {
  if (hasTexs && skPaint.getShader()) {
    if (hasColors) {
      ...
      return SkPaintToGrPaintWithXfermode(
          context, colorInfo, skPaint, matrixProvider, bmode, grPaint);
    } else {
...
void SkGpuDevice::drawVertices(const SkVertices* vertices, SkBlendMode mode, const SkPaint& paint) {
  ...
  if (!init_vertices_paint(
          fContext.get(), fRenderTargetContext->colorInfo(), paint, this->asMatrixProvider(), mode,
...
void SkGpuDevice::drawAtlas(
  ...
  if (colors) {
    if (!SkPaintToGrPaintWithXfermode(
            this->context(), fRenderTargetContext->colorInfo(), p, this->asMatrixProvider(),
