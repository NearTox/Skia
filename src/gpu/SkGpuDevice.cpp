...
void SkGpuDevice::drawPoints(
  ...
  fRenderTargetContext->drawVertices(
      this->clip(), std::move(grPaint), *viewMatrix, std::move(vertices), &primitiveType);
}
...
sk_sp<SkSpecialImage> SkGpuDevice::snapSpecial(const SkIRect& subset, bool forceCopy) {
  ...
  if (forceCopy || !view.asTextureProxy()) {
    ...
    view = GrSurfaceProxy::Copy(
        fContext.get(), rtc->asSurfaceProxy(), view.origin(), rtc->colorInfo().colorType(),
...
void SkGpuDevice::wireframeVertices(
    SkVertices::VertexMode vmode, int vertexCount, const SkPoint vertices[], SkBlendMode bmode,
  ...
  fRenderTargetContext->drawVertices(
      this->clip(), std::move(grPaint), this->localToDevice(), builder.detach(), &primitiveType);
}
