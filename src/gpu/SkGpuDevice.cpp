...
sk_sp<SkGpuDevice> SkGpuDevice::Make(
    GrContext* context, std::unique_ptr<GrRenderTargetContext> renderTargetContext,
    InitContents init) {
...
SkGpuDevice::SkGpuDevice(
    GrContext* context, std::unique_ptr<GrRenderTargetContext> renderTargetContext, unsigned flags)
    : INHERITED(
...
void SkGpuDevice::replaceRenderTargetContext(
    std::unique_ptr<GrRenderTargetContext> rtc, bool shouldRetainContent) {
...
void SkGpuDevice::replaceRenderTargetContext(bool shouldRetainContent) {
  ...
  auto newRTC = MakeRenderTargetContext(
...
void SkGpuDevice::drawPaint(const SkPaint& paint) {
  ...
  GrPaint grPaint;
  if (!SkPaintToGrPaint(
          this->context(), fRenderTargetContext->colorInfo(), paint, this->localToDevice(),
          &grPaint)) {
    return;
  }
...
void SkGpuDevice::drawPoints(
  ...
  if (paint.getPathEffect() && 2 == count && SkCanvas::kLines_PointMode == mode) {
    ...
    fRenderTargetContext->drawPath(
        this->clip(), std::move(grPaint), GrAA(paint.isAntiAlias()), this->localToDevice(), path,
        style);
    return;
  }
  ...
  SkScalar scales[2];
  bool isHairline =
      (0 == width) || (1 == width && this->localToDevice().getMinMaxScales(scales) &&
...
void SkGpuDevice::drawRect(const SkRect& rect, const SkPaint& paint) {
  ...
  if (paint.getMaskFilter() || paint.getPathEffect()) {
    ...
    GrBlurUtils::drawShapeWithMaskFilter(
        fContext.get(), fRenderTargetContext.get(), this->clip(), paint, this->localToDevice(),
        shape);
    return;
  }
  ...
  fRenderTargetContext->drawRect(
      this->clip(), std::move(grPaint), GrAA(paint.isAntiAlias()), this->localToDevice(), rect,
      &style);
}
...
void SkGpuDevice::drawEdgeAAQuad(
  ...
  if (clip) {
    ...
    fRenderTargetContext->fillQuadWithEdgeAA(
        this->clip(), std::move(grPaint), GrAA::kYes, grAA, this->localToDevice(), clip, nullptr);
  } else {
    ...
    fRenderTargetContext->fillRectWithEdgeAA(
        this->clip(), std::move(grPaint), GrAA::kYes, grAA, this->localToDevice(), rect);
  }
}
...
void SkGpuDevice::drawRRect(const SkRRect& rrect, const SkPaint& paint) {
  ...
  fRenderTargetContext->drawRRect(
      this->clip(), std::move(grPaint), GrAA(paint.isAntiAlias()), this->localToDevice(), rrect,
      style);
}
...
void SkGpuDevice::drawDRRect(const SkRRect& outer, const SkRRect& inner, const SkPaint& paint) {
  ...
  if (stroke.isFillStyle() && !paint.getMaskFilter() && !paint.getPathEffect()) {
    ...
    fRenderTargetContext->drawDRRect(
        this->clip(), std::move(grPaint), GrAA(paint.isAntiAlias()), this->localToDevice(), outer,
        inner);
    return;
  }
...
void SkGpuDevice::drawRegion(const SkRegion& region, const SkPaint& paint) {
  ...
  fRenderTargetContext->drawRegion(
      this->clip(), std::move(grPaint), GrAA(paint.isAntiAlias()), this->localToDevice(), region,
      GrStyle(paint));
}
...
void SkGpuDevice::drawOval(const SkRect& oval, const SkPaint& paint) {
  ...
  fRenderTargetContext->drawOval(
      this->clip(), std::move(grPaint), GrAA(paint.isAntiAlias()), this->localToDevice(), oval,
      GrStyle(paint));
}
...
void SkGpuDevice::drawPath(const SkPath& origSrcPath, const SkPaint& paint, bool pathIsMutable) {
  ...
  if (!paint.getMaskFilter()) {
    ...
    fRenderTargetContext->drawPath(
        this->clip(), std::move(grPaint), GrAA(paint.isAntiAlias()), this->localToDevice(),
...
bool SkGpuDevice::shouldTileImageID(
    ...
    const SkMatrix& srcToDstRect, GrSamplerState params, const SkRect* srcRectPtr, int maxTileSize,
    int* tileSize, SkIRect* clippedSubset) const {
  ASSERT_SINGLE_OWNER
...
void SkGpuDevice::drawTiledBitmap(
    ...
    const SkRect& srcRect, const SkIRect& clippedSrcIRect, GrSamplerState params,
...
void SkGpuDevice::drawBitmapTile(
    ...
    const SkRect& srcRect, GrSamplerState samplerState, const SkPaint& paint,
  ...
  if (needsTextureDomain && (SkCanvas::kStrict_SrcRectConstraint == constraint)) {
    ...
    if (bicubic) {
      ...
      fp = GrTextureEffect::MakeSubset(
          std::move(proxy), srcAlphaType, texMatrix, samplerState, domain, caps);
    }
  } else if (bicubic) {
...
void SkGpuDevice::drawSpecial(
  ...
  if (clipImage) {
    ...
    if (clipProxy && ctm.invert(&inverseClipMatrix)) {
      ...
      cfp = GrTextureEffect::Make(
          std::move(clipProxy), clipImage->alphaType(), inverseClipMatrix, filter);
...
void SkGpuDevice::drawBitmapRect(
  ...
  if (!skipTileCheck) {
    ...
    if (this->shouldTileImageID(
            bitmap.getGenerationID(), bitmap.getSubset(), this->localToDevice(), srcToDstMatrix,
            sampleState, src, maxTileSizeForFilter, &tileSize, &clippedSrcRect)) {
      this->drawTiledBitmap(
          bitmap, this->localToDevice(), srcToDstMatrix, *src, clippedSrcRect, sampleState, paint,
...
sk_sp<SkSpecialImage> SkGpuDevice::makeSpecial(const SkBitmap& bitmap) {
  ...
  return SkSpecialImage::MakeDeferredFromGpu(
      fContext.get(), rect, bitmap.getGenerationID(), std::move(view),
...
sk_sp<SkSpecialImage> SkGpuDevice::makeSpecial(const SkImage* image) {
  ...
  if (image->isTextureBacked()) {
    ...
    return SkSpecialImage::MakeDeferredFromGpu(
        ...
        std::move(view), SkColorTypeToGrColorType(image->colorType()), image->refColorSpace(),
        &this->surfaceProps());
...
sk_sp<SkSpecialImage> SkGpuDevice::snapSpecial(const SkIRect& subset, bool forceCopy) {
  ...
  if (forceCopy || !view.asTextureProxy()) {
    ...
    auto proxy = GrSurfaceProxy::Copy(
  ...
  return SkSpecialImage::MakeDeferredFromGpu(
      fContext.get(), finalSubset, kNeedNewImageUniqueID_SpecialImage, std::move(view), ct,
...
void SkGpuDevice::drawImageNine(
  ...
  if (sk_sp<GrTextureProxy> proxy =
    ...
    GrTextureAdjuster adjuster(
        this->context(), std::move(proxy), image->imageInfo().colorInfo(), pinnedUniqueID);
...
void SkGpuDevice::drawProducerLattice(
  ...
  fRenderTargetContext->drawImageLattice(
      this->clip(), std::move(grPaint), this->localToDevice(), std::move(view),
      producer->alphaType(), std::move(csxf), filter, std::move(iter), dst);
}
...
void SkGpuDevice::wireframeVertices(
  ...
  fRenderTargetContext->drawVertices(
      this->clip(), std::move(grPaint), this->localToDevice(), builder.detach(), bones, boneCount,
      &primitiveType);
}
...
void SkGpuDevice::drawVertices(
  ...
  if (!init_vertices_paint(
          fContext.get(), fRenderTargetContext->colorInfo(), paint, this->localToDevice(), mode,
          hasTexs, hasColors, &grPaint)) {
    return;
  }
  fRenderTargetContext->drawVertices(
      this->clip(), std::move(grPaint), this->localToDevice(),
      sk_ref_sp(const_cast<SkVertices*>(vertices)), bones, boneCount);
}
...
void SkGpuDevice::drawAtlas(
  ...
  if (colors) {
    if (!SkPaintToGrPaintWithXfermode(
            this->context(), fRenderTargetContext->colorInfo(), p, this->localToDevice(),
...
bool SkGpuDevice::android_utils_clipWithStencil() {
  ...
  static constexpr GrUserStencilSettings kDrawToStencil(
      GrUserStencilSettings::StaticInit<
          0x1, GrUserStencilTest::kAlways, 0x1, GrUserStencilOp::kReplace,
          GrUserStencilOp::kReplace, 0x1>());
  rtc->drawRegion(
      noClip, std::move(grPaint), GrAA::kNo, SkMatrix::I(), clipRegion, GrStyle::SimpleFill(),
      &kDrawToStencil);
  return true;
}
