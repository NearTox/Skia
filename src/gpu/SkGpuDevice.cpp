...
sk_sp<SkGpuDevice> SkGpuDevice::Make(
    GrRecordingContext* context, SkBudgeted budgeted, const SkImageInfo& info, int sampleCount,
...
std::unique_ptr<GrSurfaceDrawContext> SkGpuDevice::MakeSurfaceDrawContext(
    GrRecordingContext* context, SkBudgeted budgeted, const SkImageInfo& origInfo, int sampleCount,
...
void SkGpuDevice::replaceSurfaceDrawContext(
    std::unique_ptr<GrSurfaceDrawContext> sdc, SkSurface::ContentChangeMode mode) {
...
void SkGpuDevice::replaceSurfaceDrawContext(SkSurface::ContentChangeMode mode) {
  ...
  auto newSDC = MakeSurfaceDrawContext(
      this->recordingContext(), budgeted, this->imageInfo(), fSurfaceDrawContext->numSamples(),
      fSurfaceDrawContext->origin(), &this->surfaceProps(), fSurfaceDrawContext->mipmapped());
  if (!newSDC) {
    return;
  }
...
void SkGpuDevice::drawDRRect(const SkRRect& outer, const SkRRect& inner, const SkPaint& paint) {
  ...
  if (stroke.isFillStyle() && !paint.getMaskFilter() && !paint.getPathEffect()) {
    ...
    fSurfaceDrawContext->drawDRRect(
...
void SkGpuDevice::drawStrokedLine(const SkPoint points[2], const SkPaint& origPaint) {
  ...
  GrPaint grPaint;
  if (!SkPaintToGrPaint(
          this->recordingContext(), fSurfaceDrawContext->colorInfo(), newPaint,
          this->asMatrixProvider(), &grPaint)) {
    return;
  }
  ...
  fSurfaceDrawContext->fillQuadWithEdgeAA(
      this->clip(), std::move(grPaint), aa, edgeAA, this->localToDevice(), corners, nullptr);
}
    ...
    if (origSrcPath.isLine(points)) {
      ...
      if ((paint.isAntiAlias() && fSurfaceDrawContext->numSamples() == 1) ||
          !SkDrawTreatAAStrokeAsHairline(
              paint.getStrokeWidth(), this->localToDevice(), &coverage)) {
...
sk_sp<SkSpecialImage> SkGpuDevice::makeSpecial(const SkImage* image) {
  ...
  if (image->isTextureBacked()) {
    ...
    return SkSpecialImage::MakeDeferredFromGpu(
        fContext.get(), SkIRect::MakeWH(image->width(), image->height()), image->uniqueID(), *view,
        SkColorTypeToGrColorType(image->colorType()), image->refColorSpace(),
        &this->surfaceProps());
...
void SkGpuDevice::drawImageRect(
  ...
  this->drawImageQuad(
      image, src, &dst, nullptr, GrAA(paint.isAntiAlias()), aaFlags, nullptr, sampling, paint,
      constraint);
}
...
void SkGpuDevice::drawImageLattice(
  ...
  if (GrSurfaceProxyView view =
          as_IB(image)->refPinnedView(this->recordingContext(), &pinnedUniqueID)) {
    GrTextureAdjuster adjuster(
        this->recordingContext(), std::move(view), image->imageInfo().colorInfo(), pinnedUniqueID);
...
static bool init_vertices_paint(
    GrRecordingContext* context, const GrColorInfo& colorInfo, const SkPaint& skPaint,
  ...
  if (skPaint.getShader()) {
    if (hasColors) {
      ...
      return SkPaintToGrPaintWithBlend(context, colorInfo, skPaint, matrixProvider, bmode, grPaint);
    } else {
    ...
    if (hasColors) {
      ...
      return SkPaintToGrPaintWithPrimitiveColor(
          context, colorInfo, skPaint, matrixProvider, grPaint);
    } else {
...
void SkGpuDevice::drawAtlas(
  ...
  GrFPArgs fpArgs(
      fContext.get(), this->asMatrixProvider(), sampling, &fSurfaceDrawContext->colorInfo());
...
bool SkGpuDevice::android_utils_clipWithStencil() {
  ...
  sdc->drawRegion(
      nullptr, std::move(grPaint), GrAA::kNo, SkMatrix::I(), clipRegion, GrStyle::SimpleFill(),
      &kDrawToStencil);
  return true;
}
