  ...
  *flags = 0;
  if (info) {
    switch (info->alphaType()) {
      case kPremul_SkAlphaType: break;
      case kOpaque_SkAlphaType: *flags |= SkGpuDevice::kIsOpaque_Flag; break;
...
sk_sp<SkGpuDevice> SkGpuDevice::Make(
    GrContext* context, sk_sp<GrRenderTargetContext> renderTargetContext, int width, int height,
    InitContents init) {
  ...
  return sk_sp<SkGpuDevice>(
      new SkGpuDevice(context, std::move(renderTargetContext), width, height, flags));
}
...
sk_sp<SkGpuDevice> SkGpuDevice::Make(
  ...
  return sk_sp<SkGpuDevice>(
      new SkGpuDevice(context, std::move(renderTargetContext), info.width(), info.height(), flags));
}
...
static SkImageInfo make_info(GrRenderTargetContext* context, int w, int h, bool opaque) {
  ...
  return SkImageInfo::Make(
      w, h, colorType, opaque ? kOpaque_SkAlphaType : kPremul_SkAlphaType,
...
SkGpuDevice::SkGpuDevice(
    ...
    unsigned flags)
    : INHERITED(
          make_info(renderTargetContext.get(), width, height, SkToBool(flags & kIsOpaque_Flag)),
...
sk_sp<GrRenderTargetContext> SkGpuDevice::MakeRenderTargetContext(
    ...
    GrSurfaceOrigin origin, const SkSurfaceProps* surfaceProps, GrMipMapped mipMapped) {
  if (kUnknown_SkColorType == origInfo.colorType() || origInfo.width() < 0 ||
      origInfo.height() < 0) {
    return nullptr;
  }
  ...
  return context->priv().makeDeferredRenderTargetContext(
      ...
      origInfo.refColorSpace(), sampleCount, mipMapped, origin, surfaceProps, budgeted);
}
...
void SkGpuDevice::clearAll() {
  ...
  fRenderTargetContext->clear(
      &rect, SK_PMColor4fTRANSPARENT, GrRenderTargetContext::CanClearFullscreen::kYes);
}

void SkGpuDevice::replaceRenderTargetContext(
    sk_sp<GrRenderTargetContext> rtc, bool shouldRetainContent) {
...
void SkGpuDevice::replaceRenderTargetContext(bool shouldRetainContent) {
  ...
  sk_sp<GrRenderTargetContext> newRTC(MakeRenderTargetContext(
      this->context(), budgeted, this->imageInfo(), fRenderTargetContext->numColorSamples(),
      fRenderTargetContext->origin(), &this->surfaceProps(), fRenderTargetContext->mipMapped()));
  if (!newRTC) {
    return;
  }
...
static inline GrPrimitiveType point_mode_to_primitive_type(SkCanvas::PointMode mode) {
  switch (mode) {
    ...
    case SkCanvas::kPolygon_PointMode: return GrPrimitiveType::kLineStrip;
  }
  SK_ABORT("Unexpected mode");
  return GrPrimitiveType::kPoints;
}

void SkGpuDevice::drawPoints(
    SkCanvas::PointMode mode, size_t count, const SkPoint pts[], const SkPaint& paint) {
  ASSERT_SINGLE_OWNER
  ...
  if (paint.getPathEffect() && 2 == count && SkCanvas::kLines_PointMode == mode) {
    ...
    fRenderTargetContext->drawPath(
        this->clip(), std::move(grPaint), GrAA(paint.isAntiAlias()), this->ctm(), path, style);
    return;
  }

  SkScalar scales[2];
  bool isHairline =
      (0 == width) || (1 == width && this->ctm().getMinMaxScales(scales) &&
  ...
  sk_sp<SkVertices> vertices =
      SkVertices::MakeCopy(kIgnoredMode, SkToS32(count), pts, nullptr, nullptr);

  fRenderTargetContext->drawVertices(
      this->clip(), std::move(grPaint), *viewMatrix, std::move(vertices), nullptr, 0,
      &primitiveType);
}
...
void SkGpuDevice::drawRect(const SkRect& rect, const SkPaint& paint) {
  ...
  if (paint.getMaskFilter() || paint.getPathEffect()) {
    ...
    GrBlurUtils::drawShapeWithMaskFilter(
        fContext.get(), fRenderTargetContext.get(), this->clip(), paint, this->ctm(), shape);
    return;
  }
  ...
  fRenderTargetContext->drawRect(
      this->clip(), std::move(grPaint), GrAA(paint.isAntiAlias()), this->ctm(), rect, &style);
}

void SkGpuDevice::drawEdgeAAQuad(
  ...
  SkPMColor4f dstColor =
      SkColor4fPrepForDst(SkColor4f::FromColor(color), fRenderTargetContext->colorSpaceInfo())
          .premul();
  ...
  if (clip) {
    ...
    fRenderTargetContext->fillQuadWithEdgeAA(
        this->clip(), std::move(grPaint), GrAA::kYes, grAA, this->ctm(), clip, nullptr);
  } else {
    // Use fillRectWithEdgeAA to preserve mathematical properties of dst being rectangular
    fRenderTargetContext->fillRectWithEdgeAA(
        this->clip(), std::move(grPaint), GrAA::kYes, grAA, this->ctm(), rect);
  }
}
...
void SkGpuDevice::drawRRect(const SkRRect& rrect, const SkPaint& paint) {
  ...
  fRenderTargetContext->drawRRect(
      this->clip(), std::move(grPaint), GrAA(paint.isAntiAlias()), this->ctm(), rrect, style);
}
...
void SkGpuDevice::drawDRRect(const SkRRect& outer, const SkRRect& inner, const SkPaint& paint) {
  ...
  if (stroke.isFillStyle() && !paint.getMaskFilter() && !paint.getPathEffect()) {
    ...
    fRenderTargetContext->drawDRRect(
        this->clip(), std::move(grPaint), GrAA(paint.isAntiAlias()), this->ctm(), outer, inner);
    return;
  }
...
void SkGpuDevice::drawRegion(const SkRegion& region, const SkPaint& paint) {
  ...
  fRenderTargetContext->drawRegion(
      this->clip(), std::move(grPaint), GrAA(paint.isAntiAlias()), this->ctm(), region,
      GrStyle(paint));
}
...
void SkGpuDevice::drawOval(const SkRect& oval, const SkPaint& paint) {
  ...
  fRenderTargetContext->drawOval(
      this->clip(), std::move(grPaint), GrAA(paint.isAntiAlias()), this->ctm(), oval,
      GrStyle(paint));
}

void SkGpuDevice::drawArc(
  ...
  fRenderTargetContext->drawArc(
      this->clip(), std::move(grPaint), GrAA(paint.isAntiAlias()), this->ctm(), oval, startAngle,
      sweepAngle, useCenter, GrStyle(paint));
}

#include "include/core/SkMaskFilter.h"

///////////////////////////////////////////////////////////////////////////////
void SkGpuDevice::drawStrokedLine(const SkPoint points[2], const SkPaint& origPaint) {
  ASSERT_SINGLE_OWNER
  ...
  SkRect rect = SkRect::MakeLTRB(
      mid.fX - halfWidth, mid.fY - 0.5f * length - xtraLength, mid.fX + halfWidth,
      mid.fY + 0.5f * length + xtraLength);
  SkMatrix m;
...
void SkGpuDevice::drawPath(const SkPath& origSrcPath, const SkPaint& paint, bool pathIsMutable) {
  ...
  if (!paint.getMaskFilter()) {
    ...
    fRenderTargetContext->drawPath(
        this->clip(), std::move(grPaint), GrAA(paint.isAntiAlias()), this->ctm(), origSrcPath,
        GrStyle(paint));
    return;
  }
...
static void determine_clipped_src_rect(
    int width, int height, const GrClip& clip, const SkMatrix& viewMatrix,
    const SkMatrix& srcToDstRect, const SkISize& imageSize, const SkRect* srcRectPtr,
...
const GrCaps* SkGpuDevice::caps() const { return fContext->priv().caps(); }

bool SkGpuDevice::shouldTileImageID(
    uint32_t imageID, const SkIRect& imageRect, const SkMatrix& viewMatrix,
    const SkMatrix& srcToDstRect, const GrSamplerState& params, const SkRect* srcRectPtr,
  ...
  if (imageRect.width() > maxTileSize || imageRect.height() > maxTileSize) {
    determine_clipped_src_rect(
        fRenderTargetContext->width(), fRenderTargetContext->height(), this->clip(), viewMatrix,
  ...
  *tileSize = kBmpSmallTileSize;  // already know whole bitmap fits in one max sized tile.
  size_t usedTileBytes = get_tile_count(*clippedSubset, kBmpSmallTileSize) * kBmpSmallTileSize *
                         kBmpSmallTileSize * sizeof(SkPMColor);  // assume 32bit pixels;

  return usedTileBytes * 2 < bmpSize;
}

bool SkGpuDevice::shouldTileImage(
    const SkImage* image, const SkRect* srcRectPtr, SkCanvas::SrcRectConstraint constraint,
    SkFilterQuality quality, const SkMatrix& viewMatrix, const SkMatrix& srcToDstRect) const {
  ASSERT_SINGLE_OWNER
  ...
  GrSamplerState::Filter textureFilterMode = GrSkFilterQualityToGrFilterMode(
      quality, viewMatrix, srcToDstRect, fContext->priv().options().fSharpenMipmappedTextures,
      &doBicubic);
  ...
  return this->shouldTileImageID(
      image->unique(), image->bounds(), viewMatrix, srcToDstRect, samplerState, srcRectPtr,
...
static inline void clamped_outset_with_offset(
    SkIRect* iRect, int outset, SkPoint* offset, const SkIRect& clamp) {
...
void SkGpuDevice::drawTiledBitmap(
    const SkBitmap& bitmap, const SkMatrix& viewMatrix, const SkMatrix& dstMatrix,
    const SkRect& srcRect, const SkIRect& clippedSrcIRect, const GrSamplerState& params,
    const SkPaint& origPaint, SkCanvas::SrcRectConstraint constraint, int tileSize, bool bicubic) {
  ASSERT_SINGLE_OWNER
  ...
  for (int x = 0; x <= nx; x++) {
    for (int y = 0; y <= ny; y++) {
      SkRect tileR;
      tileR.set(
          SkIntToScalar(x * tileSize), SkIntToScalar(y * tileSize),
      ...
      SkVector offset = SkPoint::Make(SkIntToScalar(iTileR.fLeft), SkIntToScalar(iTileR.fTop));
      ...
      if (bitmap.extractSubset(&tmpB, iTileR)) {
        ...
        this->drawBitmapTile(
            tmpB, viewMatrix, rectToDraw, tileR, params, *paint, constraint, bicubic,
            needsTextureDomain);
      }
    }
  }
...
void SkGpuDevice::drawBitmapTile(
    const SkBitmap& bitmap, const SkMatrix& viewMatrix, const SkRect& dstRect,
    const SkRect& srcRect, const GrSamplerState& samplerState, const SkPaint& paint,
    SkCanvas::SrcRectConstraint constraint, bool bicubic, bool needsTextureDomain) {
  // We should have already handled bitmaps larger than the max texture size.
  SkASSERT(
      bitmap.width() <= this->caps()->maxTextureSize() &&
      bitmap.height() <= this->caps()->maxTextureSize());
  // We should be respecting the max tile size by the time we get here.
  SkASSERT(
      bitmap.width() <= this->caps()->maxTileSize() &&
  ...
  GrAA aa = GrAA(paint.isAntiAlias() && GrFSAAType::kNone != fRenderTargetContext->fsaaType());
...
void SkGpuDevice::drawSpecial(
  ...
  if (clipImage) {
    ...
    sk_sp<GrTextureProxy> clipProxy =
        as_IB(clipImage)->asTextureProxyRef(this->context(), sampler, nullptr);
    ...
    if (cfp) {
      ...
      if (!clipGeometry.contains(inverseClipMatrix.mapRect(dstRect))) {
        ...
        clip.clipDevRect(
            SkIRect::MakeXYWH(left, top, subset.width(), subset.height()), SkClipOp::kIntersect);
        SkMatrix local = SkMatrix::Concat(
            SkMatrix::MakeRectToRect(dstRect, srcRect, SkMatrix::kFill_ScaleToFit), ctm);
        fRenderTargetContext->fillRectWithLocalMatrix(
            GrClipStackClip(&clip), std::move(grPaint), GrAA(paint.isAntiAlias()), ctm,
            clipGeometry, local);
        return;
      }
...
void SkGpuDevice::drawBitmapRect(
  ...
  if (!skipTileCheck) {
    ...
    if (this->shouldTileImageID(
            bitmap.getGenerationID(), bitmap.getSubset(), this->ctm(), srcToDstMatrix, sampleState,
            src, maxTileSizeForFilter, &tileSize, &clippedSrcRect)) {
      this->drawTiledBitmap(
          bitmap, this->ctm(), srcToDstMatrix, *src, clippedSrcRect, sampleState, paint, constraint,
          tileSize, doBicubic);
      return;
    }
  }
...
sk_sp<SkSpecialImage> SkGpuDevice::makeSpecial(const SkBitmap& bitmap) {
  ...
  sk_sp<GrTextureProxy> proxy = GrMakeCachedBitmapProxy(fContext->priv().proxyProvider(), bitmap);
  if (!proxy) {
    return nullptr;
  }
...
sk_sp<SkSpecialImage> SkGpuDevice::snapSpecial() {
  ...
  if (!proxy) {
    ...
    proxy = GrSurfaceProxy::Copy(
        fContext.get(), this->accessRenderTargetContext()->asSurfaceProxy(), GrMipMapped::kNo,
  ...
  return SkSpecialImage::MakeDeferredFromGpu(
      fContext.get(), srcRect, kNeedNewImageUniqueID_SpecialImage, std::move(proxy),
      ii.refColorSpace(), &this->surfaceProps());
}
...
sk_sp<SkSpecialImage> SkGpuDevice::snapBackImage(const SkIRect& subset) {
  ...
  auto srcProxy = GrSurfaceProxy::Copy(
      ctx, rtc->asSurfaceProxy(), rtc->mipMapped(), subset, SkBackingFit::kApprox,
  ...
  return SkSpecialImage::MakeDeferredFromGpu(
      fContext.get(), SkIRect::MakeSize(srcProxy->isize()), kNeedNewImageUniqueID_SpecialImage,
      srcProxy, this->imageInfo().refColorSpace(), &this->surfaceProps());
}
...
void SkGpuDevice::drawImageRect(
  ...
  this->drawImageQuad(
      image, src, &dst, nullptr, GrAA(paint.isAntiAlias()), aaFlags, nullptr, paint, constraint);
}
...
void SkGpuDevice::drawImageNine(
  ...
  if (sk_sp<GrTextureProxy> proxy =
    ...
    GrTextureAdjuster adjuster(
        this->context(), std::move(proxy), image->alphaType(), pinnedUniqueID, image->colorSpace());
...
void SkGpuDevice::drawProducerLattice(
    GrTextureProducer* producer, std::unique_ptr<SkLatticeIter> iter, const SkRect& dst,
  ...
  auto csxf = GrColorSpaceXform::Make(
...
void SkGpuDevice::wireframeVertices(
    SkVertices::VertexMode vmode, int vertexCount, const SkPoint vertices[],
    const SkVertices::Bone bones[], int boneCount, SkBlendMode bmode, const uint16_t indices[],
  ...
  switch (vmode) {
    case SkVertices::kTriangles_VertexMode: triangleCount = n / 3; break;
    case SkVertices::kTriangleStrip_VertexMode: triangleCount = n - 2; break;
    case SkVertices::kTriangleFan_VertexMode: SK_ABORT("Unexpected triangle fan."); break;
  }
  ...
  fRenderTargetContext->drawVertices(
      this->clip(), std::move(grPaint), this->ctm(), builder.detach(), bones, boneCount,
      &primitiveType);
}

void SkGpuDevice::drawVertices(
  ...
  if ((!hasTexs || !paint.getShader()) && !hasColors) {
    ...
    this->wireframeVertices(
        vertices->mode(), vertices->vertexCount(), vertices->positions(), bones, boneCount, mode,
  ...
  fRenderTargetContext->drawVertices(
      this->clip(), std::move(grPaint), this->ctm(), sk_ref_sp(const_cast<SkVertices*>(vertices)),
      bones, boneCount);
}
...
void SkGpuDevice::drawAtlas(
    const SkImage* atlas, const SkRSXform xform[], const SkRect texRect[], const SkColor colors[],
    int count, SkBlendMode mode, const SkPaint& paint) {
  ASSERT_SINGLE_OWNER
...
void SkGpuDevice::drawDrawable(SkDrawable* drawable, const SkMatrix* matrix, SkCanvas* canvas) {
  ...
  if (GrBackendApi::kVulkan == api) {
    ...
    std::unique_ptr<SkDrawable::GpuDrawHandler> gpuDraw = drawable->snapGpuDrawHandler(
        api, combinedMatrix, canvas->getDeviceClipBounds(), this->imageInfo());
    if (gpuDraw) {
...
GrSemaphoresSubmitted SkGpuDevice::flush(
...
SkBaseDevice* SkGpuDevice::onCreateDevice(const CreateInfo& cinfo, const SkPaint*) {
  ...
  sk_sp<GrRenderTargetContext> rtc(fContext->priv().makeDeferredRenderTargetContext(
      ...
      fRenderTargetContext->numStencilSamples(), GrMipMapped::kNo, kBottomLeft_GrSurfaceOrigin,
      &props));
  if (!rtc) {
    return nullptr;
  }
  ...
  return SkGpuDevice::Make(
             fContext.get(), std::move(rtc), cinfo.fInfo.width(), cinfo.fInfo.height(), init)
      .release();
}
