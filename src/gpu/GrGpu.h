...
class GrGpu : public SkRefCnt {
 public:
  ...
  virtual sk_sp<GrSemaphore> wrapBackendSemaphore(
  ...
  GrBackendTexture createTestingOnlyBackendTexture(
      int w, int h, SkColorType, GrMipMapped, GrRenderable, const void* pixels = nullptr,
      size_t rowBytes = 0);
  ...
  virtual GrBackendTexture createBackendTexture(
      ...
      size_t rowBytes, const SkColor4f& = SkColors::kTransparent) = 0;
  ...
  virtual GrStencilAttachment* createStencilAttachmentForRenderTarget(
      const GrRenderTarget*, int width, int height) = 0;
 ...
 private:
  ...
  virtual sk_sp<GrTexture> onCreateTexture(
      const GrSurfaceDesc&, SkBudgeted, const GrMipLevel texels[], int mipLevelCount) = 0;
  ...
  virtual bool onWritePixels(
      GrSurface*, int left, int top, int width, int height, GrColorType, const GrMipLevel texels[],
      int mipLevelCount) = 0;

  // overridden by backend-specific derived class to perform the texture transfer
  virtual bool onTransferPixelsTo(
      GrTexture*, int left, int top, int width, int height, GrColorType colorType,
      GrGpuBuffer* transferBuffer, size_t offset, size_t rowBytes) = 0;
  // overridden by backend-specific derived class to perform the surface transfer
  virtual bool onTransferPixelsFrom(
      GrSurface*, int left, int top, int width, int height, GrColorType colorType,
  ...
  virtual bool onCopySurface(
      GrSurface* dst, GrSurfaceOrigin dstOrigin, GrSurface* src, GrSurfaceOrigin srcOrigin,
      const SkIRect& srcRect, const SkIPoint& dstPoint, bool canDiscardOutsideDstRect) = 0;

  virtual void onFinishFlush(
      GrSurfaceProxy*[], int n, SkSurface::BackendSurfaceAccess access, const GrFlushInfo&,
