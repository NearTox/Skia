...
void GrMtlPipelineState::setData(
...
void GrMtlPipelineState::setDrawState(
    id<MTLRenderCommandEncoder> renderCmdEncoder, const GrSwizzle& outputSwizzle,
...
void GrMtlPipelineState::bindTextures(id<MTLRenderCommandEncoder> renderCmdEncoder) {
  ...
  for (int index = 0; index < fNumSamplers; ++index) {
    [renderCmdEncoder setFragmentTexture:fSamplerBindings[index].fTexture atIndex:index];
...
static bool blend_coeff_refs_constant(GrBlendCoeff coeff) {
  switch (coeff) {
    ...
    case kIConstA_GrBlendCoeff: return true;
    default: return false;
  }
}

void GrMtlPipelineState::setBlendConstants(
    id<MTLRenderCommandEncoder> renderCmdEncoder, const GrSwizzle& swizzle,
    const GrXferProcessor& xferProcessor) {
  if (!renderCmdEncoder) {
    return;
  }
  ...
  if (!fStencil.isDisabled()) {
    if (fStencil.isTwoSided()) {
      if (@available(macOS 10.11, iOS 9.0, *)) {
        [renderCmdEncoder setStencilFrontReferenceValue:fStencil.postOriginCCWFace(origin).fRef
