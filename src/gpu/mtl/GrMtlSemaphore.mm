...
sk_sp<GrMtlSemaphore> GrMtlSemaphore::MakeWrapped(
    GrMtlGpu* gpu, GrMTLHandle event, uint64_t value, GrWrapOwnership ownership) {
  ...
  if (@available(macOS 10.14, iOS 12.0, *)) {
    ...
    auto sema = sk_sp<GrMtlSemaphore>(
        new GrMtlSemaphore(gpu, mtlEvent, value, kBorrow_GrWrapOwnership != ownership));
    return sema;
  } else {
    return nullptr;
  }
