...
bool GrPixelConfigToMTLFormat(GrPixelConfig config, MTLPixelFormat* format) {
  ...
  switch (config) {
    ...
    case kRGBA_8888_GrPixelConfig: *format = MTLPixelFormatRGBA8Unorm; return true;
    case kRGB_888_GrPixelConfig: *format = MTLPixelFormatRGBA8Unorm; return true;
    case kRGB_888X_GrPixelConfig: *format = MTLPixelFormatRGBA8Unorm; return true;
    case kRG_88_GrPixelConfig: *format = MTLPixelFormatRG8Unorm; return true;
    case kBGRA_8888_GrPixelConfig: *format = MTLPixelFormatBGRA8Unorm; return true;
    case kSRGBA_8888_GrPixelConfig: *format = MTLPixelFormatRGBA8Unorm_sRGB; return true;
    case kRGBA_1010102_GrPixelConfig: *format = MTLPixelFormatRGB10A2Unorm; return true;
    ...
    case kRGBA_half_GrPixelConfig: *format = MTLPixelFormatRGBA16Float; return true;
    case kRGBA_half_Clamped_GrPixelConfig: *format = MTLPixelFormatRGBA16Float; return true;
    case kAlpha_half_GrPixelConfig:  // fall through
    case kAlpha_half_as_Red_GrPixelConfig: *format = MTLPixelFormatR16Float; return true;
    ...
    case kRGB_ETC1_GrPixelConfig:
      ...
      *format = MTLPixelFormatETC2_RGB8;
      ...
      return false;
#endif
    case kAlpha_16_GrPixelConfig: *format = MTLPixelFormatR16Unorm; return true;
    case kRG_1616_GrPixelConfig: *format = MTLPixelFormatRG16Unorm; return true;
    case kRGBA_16161616_GrPixelConfig: *format = MTLPixelFormatRGBA16Unorm; return true;
...
id<MTLLibrary> GrCompileMtlShaderLibrary(
    const GrMtlGpu* gpu, const char* shaderString, SkSL::Program::Kind kind,
    const SkSL::Program::Settings& settings, SkSL::Program::Inputs* outInputs) {
  std::unique_ptr<SkSL::Program> program =
      gpu->shaderCompiler()->convertProgram(kind, SkSL::String(shaderString), settings);
  ...
  *outInputs = program->fInputs;
  ...
  id<MTLLibrary> compiledLibrary =
      GrMtlNewLibraryWithSource(gpu->device(), mtlCode, defaultOptions, &timedout);
  if (timedout) {
    // try again
    compiledLibrary = GrMtlNewLibraryWithSource(gpu->device(), mtlCode, defaultOptions, &timedout);
  }
#else
  NSError* error = nil;
...
id<MTLLibrary> GrMtlNewLibraryWithSource(
    id<MTLDevice> device, NSString* mslCode, MTLCompileOptions* options, bool* timedout) {
...
bool GrMtlFormatIsCompressed(MTLPixelFormat mtlFormat) {
  switch (mtlFormat) {
#ifdef SK_BUILD_FOR_IOS
    case MTLPixelFormatETC2_RGB8: return true;
#endif
    default: return false;
  }
}

bool GrMtlFormatToCompressionType(
    MTLPixelFormat mtlFormat, SkImage::CompressionType* compressionType) {
  switch (mtlFormat) {
#ifdef SK_BUILD_FOR_IOS
    case MTLPixelFormatETC2_RGB8: *compressionType = SkImage::kETC1_CompressionType; return true;
#endif
    default: return false;
  }
}
...
#if GR_TEST_UTILS
bool GrMtlFormatIsBGRA(GrMTLPixelFormat mtlFormat) { return mtlFormat == MTLPixelFormatBGRA8Unorm; }
