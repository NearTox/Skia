...
GrMtlDepthStencil* GrMtlDepthStencil::Create(
    const GrMtlGpu* gpu, const GrStencilSettings& stencil, GrSurfaceOrigin origin) {
  ...
  return new GrMtlDepthStencil(
      [gpu->device() newDepthStencilStateWithDescriptor:desc], GenerateKey(stencil, origin));
}
