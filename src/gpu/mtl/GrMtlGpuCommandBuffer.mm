...
GrMtlGpuRTCommandBuffer::~GrMtlGpuRTCommandBuffer() { SkASSERT(nil == fActiveRenderCmdEncoder); }
...
void GrMtlGpuRTCommandBuffer::copy(
...
void GrMtlGpuRTCommandBuffer::transferFrom(
...
GrMtlPipelineState* GrMtlGpuRTCommandBuffer::prepareDrawState(
  ...
  GrMtlPipelineState* pipelineState = fGpu->resourceProvider().findOrCreateCompatiblePipelineState(
      fRenderTarget, fOrigin, pipeline, primProc, primProcProxies, primType);
  if (!pipelineState) {
    return nullptr;
  }
...
void GrMtlGpuRTCommandBuffer::onDraw(
    const GrPrimitiveProcessor& primProc, const GrPipeline& pipeline,
  ...
  GrMtlPipelineState* pipelineState =
      this->prepareDrawState(primProc, pipeline, fixedDynamicState, primitiveType);
  if (!pipelineState) {
    return;
  }
  ...
  pipelineState->setDrawState(
      fActiveRenderCmdEncoder, pipeline.outputSwizzle(), pipeline.getXferProcessor());

  bool dynamicScissor =
      pipeline.isScissorEnabled() && dynamicStateArrays && dynamicStateArrays->fScissorRects;
  if (!pipeline.isScissorEnabled()) {
    GrMtlPipelineState::SetDynamicScissorRectState(
        fActiveRenderCmdEncoder, fRenderTarget, fOrigin,
        SkIRect::MakeWH(fRenderTarget->width(), fRenderTarget->height()));
  } else if (!dynamicScissor) {
    SkASSERT(fixedDynamicState);
    GrMtlPipelineState::SetDynamicScissorRectState(
        fActiveRenderCmdEncoder, fRenderTarget, fOrigin, fixedDynamicState->fScissorRect);
  }
  ...
  for (int i = 0; i < meshCount; ++i) {
    ...
    if (mesh.primitiveType() != primitiveType) {
      ...
      pipelineState = this->prepareDrawState(primProc, pipeline, fixedDynamicState, primitiveType);
      if (!pipelineState) {
        return;
      }
    ...
    if (dynamicScissor) {
      GrMtlPipelineState::SetDynamicScissorRectState(
          fActiveRenderCmdEncoder, fRenderTarget, fOrigin, dynamicStateArrays->fScissorRects[i]);
    }
...
void GrMtlGpuRTCommandBuffer::onClear(const GrFixedClip& clip, const SkPMColor4f& color) {
  ...
  fRenderPassDesc.colorAttachments[0].clearColor =
      MTLClearColorMake(color.fR, color.fG, color.fB, color.fA);
...
void GrMtlGpuRTCommandBuffer::initRenderState(id<MTLRenderCommandEncoder> encoder) {
  ...
  MTLViewport viewport = {0.0, 0.0, (double)fRenderTarget->width(), (double)fRenderTarget->height(),
                          0.0, 1.0};
...
void GrMtlGpuRTCommandBuffer::setupRenderPass(
    const GrGpuRTCommandBuffer::LoadAndStoreInfo& colorInfo,
    const GrGpuRTCommandBuffer::StencilLoadAndStoreInfo& stencilInfo) {
  const static MTLLoadAction mtlLoadAction[]{MTLLoadActionLoad, MTLLoadActionClear,
  ...
  if (colorInfo.fLoadOp == GrLoadOp::kClear || stencilInfo.fLoadOp == GrLoadOp::kClear) {
    fBounds = SkRect::MakeWH(fRenderTarget->width(), fRenderTarget->height());
    this->precreateCmdEncoder();
...
void GrMtlGpuRTCommandBuffer::sendMeshToGpu(
...
void GrMtlGpuRTCommandBuffer::sendIndexedMeshToGpu(
    ...
    int baseVertex, GrPrimitiveRestart restart) {
  ...
  size_t indexOffset =
      static_cast<const GrMtlBuffer*>(indexBuffer)->offset() + sizeof(uint16_t) * baseIndex;
...
void GrMtlGpuRTCommandBuffer::sendInstancedMeshToGpu(
    GrPrimitiveType primitiveType, const GrBuffer* vertexBuffer, int vertexCount, int baseVertex,
...
void GrMtlGpuRTCommandBuffer::sendIndexedInstancedMeshToGpu(
    GrPrimitiveType primitiveType, const GrBuffer* indexBuffer, int indexCount, int baseIndex,
...
void GrMtlGpuRTCommandBuffer::setVertexBuffer(
    id<MTLRenderCommandEncoder> encoder, const GrMtlBuffer* buffer, size_t vertexOffset,
    size_t index) {
  SkASSERT(index < 4);
  ...
  if (fBufferBindings[index].fBuffer != mtlVertexBuffer) {
    [encoder setVertexBuffer:mtlVertexBuffer offset:offset atIndex:index];
