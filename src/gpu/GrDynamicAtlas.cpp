...
sk_sp<GrTextureProxy> GrDynamicAtlas::MakeLazyAtlasProxy(
  ...
  sk_sp<GrTextureProxy> proxy = GrProxyProvider::MakeFullyLazyProxy(
      std::move(instantiate), format, GrRenderable::kYes, sampleCount, GrProtected::kNo, caps,
      useAllocator);
...
GrDynamicAtlas::GrDynamicAtlas(
    GrColorType colorType, InternalMultisample internalMultisample, SkISize initialSize,
    int maxAtlasSize, const GrCaps& caps)
...
void GrDynamicAtlas::reset(SkISize initialSize, const GrCaps& caps) {
  ...
  fTextureProxy = MakeLazyAtlasProxy(
      [this](GrResourceProvider* resourceProvider, const GrBackendFormat& format, int sampleCount) {
        if (!fBackingTexture) {
          fBackingTexture = resourceProvider->createTexture(
              {fWidth, fHeight}, format, GrRenderable::kYes, sampleCount, GrMipMapped::kNo,
...
std::unique_ptr<GrRenderTargetContext> GrDynamicAtlas::instantiate(
  ...
  auto rtc = onFlushRP->makeRenderTargetContext(
      fTextureProxy, kTextureOrigin, fColorType, nullptr, nullptr);
  if (!rtc) {
    onFlushRP->printWarningMessage(
        SkStringPrintf(
  ...
  rtc->clear(&clearRect, SK_PMColor4fTRANSPARENT, GrRenderTargetContext::CanClearFullscreen::kYes);
  return rtc;
}
