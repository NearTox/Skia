...
sk_sp<GrTextureProxy> GrDynamicAtlas::MakeLazyAtlasProxy(
    LazyInstantiateAtlasCallback&& callback, GrColorType colorType,
  ...
  sk_sp<GrTextureProxy> proxy = GrProxyProvider::MakeFullyLazyProxy(
      std::move(callback), format, GrRenderable::kYes, sampleCount, GrProtected::kNo, caps,
      useAllocator);
...
GrDynamicAtlas::GrDynamicAtlas(
    ...
    int maxAtlasSize, const GrCaps& caps, RectanizerAlgorithm algorithm)
      ...
      [this](GrResourceProvider* resourceProvider, const LazyAtlasDesc& desc) {
        if (!fBackingTexture) {
          fBackingTexture = resourceProvider->createTexture(
              fTextureProxy->backingStoreDimensions(), desc.fFormat, desc.fTextureType,
              desc.fRenderable, desc.fSampleCnt, desc.fMipmapped, desc.fBudgeted, desc.fProtected);
        }
...
void GrDynamicAtlas::instantiate(
