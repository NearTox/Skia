...
template <
    ...
    ItemFn<
        T, typename std::conditional<
               Const, const GrBlockAllocator::Block, GrBlockAllocator::Block>::type>
        Resolve>
...
class GrTBlockList {
 public:
   ...
   */
  explicit GrTBlockList(
      int itemsPerBlock,
      GrBlockAllocator::GrowthPolicy policy = GrBlockAllocator::GrowthPolicy::kFixed)
      : fAllocator(
            policy, GrBlockAllocator::BlockOverhead<alignof(T)>() + sizeof(T) * itemsPerBlock) {}
  ...
  const T& item(int i) const { return const_cast<GrTBlockList*>(this)->item(i); }
  ...
  if (headBlock->metadata() > 0) {
    ...
    if (headItemCount > avail) {
      ...
      fAllocator->template reserve<alignof(T)>(
          ...
          GrBlockAllocator::kIgnoreExistingBytes_Flag | GrBlockAllocator::kIgnoreGrowthPolicy_Flag);
    }
