...
class GrStagingBuffer {
 public:
  GrStagingBuffer(GrGpu* gpu, size_t size, void* data) noexcept
      : fGpu(gpu), fSize(size), fData(data) {}
  virtual ~GrStagingBuffer() { fGpu = nullptr; }
  void markAvailable(void* data);
  struct Slice {
    Slice(GrStagingBuffer* buffer, int offset, void* data) noexcept
        : fBuffer(buffer), fOffset(offset), fData(data) {}
    GrStagingBuffer* fBuffer;
    int fOffset;
    void* fData;
  };
  size_t remaining() const noexcept { return fSize - fOffset; }
