...
GrPipeline::GrPipeline(
  ...
  SkASSERT(
      (args.fDstProxyView.dstSampleType() != GrDstSampleType::kNone) ==
...
void GrPipeline::genKey(GrProcessorKeyBuilder* b, const GrCaps& caps) const {
  ...
  static_assert(
      kBlendWriteShift + 2 * kBlendCoeffShift + kBlendEquationShift + kDstSampleTypeInputShift <=
      32);
  ...
  blendKey |=
      ((fDstSampleType == GrDstSampleType::kAsInputAttachment)
