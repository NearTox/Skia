...
GrDrawingManager::GrDrawingManager(
    GrRecordingContext* rContext, const PathRendererChain::Options& optionsForPathRendererChain,
...
GrDrawingManager::GrDrawingManager(GrRecordingContext* rContext, bool reduceOpsTaskSplitting)
    : fContext(rContext), fReduceOpsTaskSplitting(reduceOpsTaskSplitting) {}
...
bool GrDrawingManager::wasAbandoned() const { return fContext->abandoned(); }
...
bool GrDrawingManager::flush(
  ...
  if (!proxies.empty() && !info.fNumSemaphores && !info.fFinishedProc &&
    ...
    bool allUnused = std::all_of(proxies.begin(), proxies.end(), [&](GrSurfaceProxy* proxy) {
      bool used = std::any_of(
          fDAG.begin(), fDAG.end(), [&](auto& task) { return task && task->isUsed(proxy); });
      return !used;
    });
    if (allUnused) {
      if (info.fSubmittedProc) {
  ...
  bool flushed = !resourceAllocator.failedInstantiation() && this->executeRenderTasks(&flushState);
  this->removeRenderTasks();
  ...
  for (GrOnFlushCallbackObject* onFlushCBObject : fOnFlushCBObjects) {
    onFlushCBObject->postFlush(
        fTokenTracker.nextTokenToFlush(), SkMakeSpan(fFlushingRenderTaskIDs));
    flushed = true;
  }
  if (flushed) {
    resourceCache->purgeAsNeeded();
...
void GrDrawingManager::testingOnly_removeOnFlushCallbackObject(GrOnFlushCallbackObject* cb) {
  int n =
      std::find(fOnFlushCBObjects.begin(), fOnFlushCBObjects.end(), cb) - fOnFlushCBObjects.begin();
...
void GrDrawingManager::createDDLTask(
    sk_sp<const SkDeferredDisplayList> ddl, sk_sp<GrRenderTargetProxy> newDest, SkIPoint offset) {
  SkDEBUGCODE(this->validate());
  ...
  SkDEBUGCODE(auto ddlTask =)
  this->appendTask(sk_make_sp<GrDDLTask>(this, std::move(newDest), std::move(ddl), offset));
...
#if SK_GPU_V1
sk_sp<skgpu::v1::OpsTask> GrDrawingManager::newOpsTask(
    GrSurfaceProxyView surfaceView, sk_sp<GrArenas> arenas, bool flushTimeOpsTask) {
  SkDEBUGCODE(this->validate());
  SkASSERT(fContext);

  this->closeActiveOpsTask();

  sk_sp<skgpu::v1::OpsTask> opsTask(new skgpu::v1::OpsTask(
...
void GrDrawingManager::addAtlasTask(
    sk_sp<GrRenderTask> atlasTask, GrRenderTask* previousAtlasTask) {
  SkDEBUGCODE(this->validate());
  SkASSERT(fContext);
...
void GrDrawingManager::newTransferFromRenderTask(
  ...
  task->addDependency(this, srcProxy.get(), GrMipmapped::kNo, GrTextureResolveManager(this), caps);
  task->makeClosed(fContext);
...
sk_sp<GrRenderTask> GrDrawingManager::newCopyRenderTask(
    sk_sp<GrSurfaceProxy> src, SkIRect srcRect, sk_sp<GrSurfaceProxy> dst, SkIPoint dstPoint,
  ...
  sk_sp<GrRenderTask> task =
      GrCopyRenderTask::Make(this, src, srcRect, std::move(dst), dstPoint, origin);
  if (!task) {
    return nullptr;
  }
...
bool GrDrawingManager::newWritePixelsTask(
    sk_sp<GrSurfaceProxy> dst, SkIRect rect, GrColorType srcColorType, GrColorType dstColorType,
    const GrMipLevel levels[], int levelCount) {
  SkDEBUGCODE(this->validate());
  SkASSERT(fContext);
  ...
  if (!caps.preferVRAMUseOverFlushes()) {
    this->flushSurfaces(
        SkSpan<GrSurfaceProxy*>{}, SkSurface::BackendSurfaceAccess::kNoAccess, GrFlushInfo{},
        nullptr);
  }

  GrRenderTask* task = this->appendTask(GrWritePixelsTask::Make(
      this, std::move(dst), rect, srcColorType, dstColorType, levels, levelCount));
  if (!task) {
    return false;
  }
...
skgpu::v1::PathRenderer* GrDrawingManager::getPathRenderer(
    const PathRenderer::CanDrawPathArgs& args, bool allowSW, PathRendererChain::DrawType drawType,
