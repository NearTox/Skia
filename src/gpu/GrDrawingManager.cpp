...
GrDrawingManager::GrDrawingManager(
    GrRecordingContext* context, const GrPathRendererChain::Options& optionsForPathRendererChain,
    const GrTextContext::Options& optionsForTextContext, bool sortOpLists,
...
bool GrDrawingManager::wasAbandoned() const { return fContext->priv().abandoned(); }
...
GrSemaphoresSubmitted GrDrawingManager::flush(
  ...
  if (!fOnFlushCBObjects.empty()) {
    ...
    for (GrOnFlushCallbackObject* onFlushCBObject : fOnFlushCBObjects) {
      onFlushCBObject->preFlush(
          &onFlushProvider, fFlushingOpListIDs.begin(), fFlushingOpListIDs.count(),
...

  {
    GrResourceAllocator alloc(
        resourceProvider, flushState.deinstantiateProxyTracker() SkDEBUGCODE(, fDAG.numOpLists()));
  ...
  for (GrOnFlushCallbackObject* onFlushCBObject : fOnFlushCBObjects) {
    onFlushCBObject->postFlush(
        fTokenTracker.nextTokenToFlush(), fFlushingOpListIDs.begin(), fFlushingOpListIDs.count());
    flushed = true;
  }
  if (flushed) {
    resourceCache->purgeAsNeeded();
...
bool GrDrawingManager::executeOpLists(
    int startIndex, int stopIndex, GrOpFlushState* flushState, int* numOpListsExecuted) {
  SkASSERT(startIndex <= stopIndex && stopIndex <= fDAG.numOpLists());

#if GR_FLUSH_TIME_OP_SPEW
  SkDebugf(
      "Flushing opLists: %d to %d out of [%d, %d]\n", startIndex, stopIndex, 0, fDAG.numOpLists());
  for (int i = startIndex; i < stopIndex; ++i) {
    if (fDAG.opList(i)) {
...
void GrDrawingManager::copyOpListsFromDDL(
...
sk_sp<GrRenderTargetOpList> GrDrawingManager::newRTOpList(
    sk_sp<GrRenderTargetProxy> rtp, bool managedOpList) {
  SkDEBUGCODE(this->validate());
  SkASSERT(fContext);
  ...
  sk_sp<GrRenderTargetOpList> opList(new GrRenderTargetOpList(
      fContext->priv().refOpMemoryPool(), rtp, fContext->priv().auditTrail()));
...
sk_sp<GrTextureOpList> GrDrawingManager::newTextureOpList(sk_sp<GrTextureProxy> textureProxy) {
  ...
  sk_sp<GrTextureOpList> opList(new GrTextureOpList(
      fContext->priv().refOpMemoryPool(), textureProxy, fContext->priv().auditTrail()));
 ...
 */
GrPathRenderer* GrDrawingManager::getPathRenderer(
    const GrPathRenderer::CanDrawPathArgs& args, bool allowSW,
    GrPathRendererChain::DrawType drawType, GrPathRenderer::StencilSupport* stencilSupport) {
  if (!fPathRendererChain) {
...
GrPathRenderer* GrDrawingManager::getSoftwarePathRenderer() {
  if (!fSoftwarePathRenderer) {
    fSoftwarePathRenderer.reset(new GrSoftwarePathRenderer(
...
sk_sp<GrRenderTargetContext> GrDrawingManager::makeRenderTargetContext(
    sk_sp<GrSurfaceProxy> sProxy, GrColorType colorType, sk_sp<SkColorSpace> colorSpace,
...
sk_sp<GrTextureContext> GrDrawingManager::makeTextureContext(
    sk_sp<GrSurfaceProxy> sProxy, GrColorType colorType, SkAlphaType alphaType,
  ...
  return sk_sp<GrTextureContext>(new GrTextureContext(
