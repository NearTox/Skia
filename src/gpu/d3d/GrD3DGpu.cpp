...
void GrD3DGpu::querySampleLocations(
  ...
  switch (numSamples) {
    ...
    default: SK_ABORT("Invalid sample count."); break;
  }
}
...
bool GrD3DGpu::onReadPixels(
  ...
  fDevice->GetCopyableFootprints(
      &desc, 0, 1, baseOffset, &dstLocation.PlacedFootprint, nullptr, nullptr, &transferTotalBytes);
  ...
  SkRectMemcpy(
      buffer, rowBytes, mappedMemory, dstLocation.PlacedFootprint.Footprint.RowPitch, tightRowBytes,
      height);
...
bool GrD3DGpu::onWritePixels(
  ...
  success =
      this->uploadToTexture(d3dTex, left, top, width, height, srcColorType, texels, mipLevelCount);
...
bool GrD3DGpu::uploadToTexture(
  ...
  ID3D12Resource* d3dBuffer = static_cast<GrD3DBuffer*>(slice.fBuffer)->d3dResource();
  fCurrentDirectCommandList->copyBufferToTexture(
      d3dBuffer, tex, mipLevelCount, placedFootprints.get(), left, top);
...
sk_sp<GrRenderTarget> GrD3DGpu::onWrapBackendRenderTarget(const GrBackendRenderTarget& rt) {
  ...
  sk_sp<GrD3DRenderTarget> tgt =
      GrD3DRenderTarget::MakeWrappedRenderTarget(this, rt.dimensions(), 1, info, std::move(state));
...
sk_sp<GrRenderTarget> GrD3DGpu::onWrapBackendTextureAsRenderTarget(
  ...
  return GrD3DRenderTarget::MakeWrappedRenderTarget(
      this, tex.dimensions(), sampleCnt, textureInfo, std::move(state));
}
...
GrStencilAttachment* GrD3DGpu::createStencilAttachmentForRenderTarget(
  ...
  GrD3DStencilAttachment* stencil(
      GrD3DStencilAttachment::Make(this, dimensions, numStencilSamples, sFmt));
...
GrBackendTexture GrD3DGpu::onCreateBackendTexture(
  ...
  if (!this->createTextureResourceForBackendSurface(
          dxgiFormat, dimensions, GrTexturable::kYes, renderable, mipMapped, &info, isProtected)) {
    return {};
  }
...
static void copy_src_data(
    GrD3DGpu* gpu, char* mapPtr, DXGI_FORMAT dxgiFormat,
    D3D12_PLACED_SUBRESOURCE_FOOTPRINT* placedFootprints, const SkPixmap srcData[],
    int numMipLevels) {
  ...
  for (int currentMipLevel = 0; currentMipLevel < numMipLevels; currentMipLevel++) {
    ...
    char* dst = mapPtr + placedFootprints[currentMipLevel].Offset;
    SkRectMemcpy(
        dst, placedFootprints[currentMipLevel].Footprint.RowPitch, srcData[currentMipLevel].addr(),
        srcData[currentMipLevel].rowBytes(), trimRowBytes, srcData[currentMipLevel].height());
  }
}
...
bool GrD3DGpu::onUpdateBackendTexture(
  ...
  fDevice->GetCopyableFootprints(
      ...
      &combinedBufferSize);
  SkASSERT(combinedBufferSize);
  if (data->type() == BackendTextureData::Type::kColor && !GrDxgiFormatIsCompressed(info.fFormat) &&
      mipLevelCount > 1) {
  ...
  if (data->type() == BackendTextureData::Type::kPixmaps) {
    copy_src_data(
        this, bufferData, info.fFormat, placedFootprints.get(), data->pixmaps(), info.fLevelCount);
  } else if (data->type() == BackendTextureData::Type::kCompressed) {
    copy_compressed_data(
        bufferData, info.fFormat, placedFootprints.get(), numRows.get(), rowSizeInBytes.get(),
        data->compressedData(), info.fLevelCount);
  } else {
    ...
    if (SkImage::CompressionType::kNone == compression) {
      if (!copy_color_data(
              this->d3dCaps(), bufferData, info.fFormat, backendTexture.dimensions(),
              placedFootprints, data->color())) {
        return false;
      }
    } else {
      size_t totalCompressedSize = SkCompressedFormatDataSize(
          compression, backendTexture.dimensions(), backendTexture.hasMipmaps());
      SkAutoTMalloc<char> tempData(totalCompressedSize);
      GrFillInCompressedData(
          compression, backendTexture.dimensions(), backendTexture.fMipmapped, tempData,
          data->color());
      copy_compressed_data(
          bufferData, info.fFormat, placedFootprints.get(), numRows.get(), rowSizeInBytes.get(),
          tempData.get(), info.fLevelCount);
    }
  }
...
GrBackendRenderTarget GrD3DGpu::createTestingOnlyBackendRenderTarget(
  ...
  GrD3DTextureResourceInfo info;
  if (!this->createTextureResourceForBackendSurface(
          dxgiFormat, {w, h}, GrTexturable::kNo, GrRenderable::kYes, GrMipmapped::kNo, &info,
          GrProtected::kNo)) {
    return {};
  }
...
std::unique_ptr<GrSemaphore> GrD3DGpu::wrapBackendSemaphore(
    const GrBackendSemaphore& semaphore, GrResourceProvider::SemaphoreWrapType, GrWrapOwnership) {
