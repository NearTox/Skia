...
sk_sp<GrTexture> GrD3DGpu::onCreateTexture(
  ...
  if (renderable == GrRenderable::kYes) {
    ...
    tex = GrD3DTexture::MakeNewTexture(
        this, budgeted, dimensions, resourceDesc, isProtected, mipMapsStatus);
  }
...
void GrD3DGpu::copySurfaceAsCopyTexture(
  ...
  fCurrentDirectCommandList->copyTextureRegion(
...
bool GrD3DGpu::onReadPixels(
  ...
  fCurrentDirectCommandList->copyTextureRegion(
      d3dBuf->resource(), &dstLocation, 0, 0, texResource->resource(), &srcLocation, &srcBox);
...
bool GrD3DGpu::uploadToTexture(
  ...
  sk_sp<GrGpuBuffer> transferBuffer = this->createBuffer(
      combinedBufferSize, GrGpuBufferType::kXferCpuToGpu, kDynamic_GrAccessPattern);
  if (!transferBuffer) {
    return false;
  }
...
sk_sp<GrTexture> GrD3DGpu::onWrapCompressedBackendTexture(
    const GrBackendTexture& tex, GrWrapOwnership, GrWrapCacheable wrapType) {
  GrD3DTextureResourceInfo textureInfo;
  if (!tex.getD3DTextureResourceInfo(&textureInfo)) {
    return nullptr;
  }
  ...
  SkASSERT(state);
  return GrD3DTexture::MakeWrappedTexture(
      this, tex.dimensions(), wrapType, kRead_GrIOType, textureInfo, std::move(state));
}
...
bool copy_color_data(
    char* mapPtr, DXGI_FORMAT dxgiFormat, SkISize dimensions,
...
GrBackendTexture GrD3DGpu::onCreateCompressedBackendTexture(
  ...
  GrD3DTextureResourceInfo info;
  if (!this->createTextureResourceForBackendSurface(
          dxgiFormat, dimensions, GrTexturable::kYes, GrRenderable::kNo, mipMapped, &info,
          isProtected)) {
    return {};
  }
