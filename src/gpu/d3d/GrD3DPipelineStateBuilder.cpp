...
void GrD3DPipelineStateBuilder::compileD3DProgram(
    SkSL::Program::Kind kind, const SkSL::String& sksl, const SkSL::Program::Settings& settings,
    ID3DBlob** shader, SkSL::Program::Inputs* outInputs) {
  ...
  *outInputs = program->fInputs;
  ...
  HRESULT hr = D3DCompile(
      outHLSL.c_str(), outHLSL.length(), nullptr, nullptr, nullptr, "main", compileTarget,
      compileFlags, 0, shader, &errors);
  if (!SUCCEEDED(hr)) {
    errorHandler->compileError(
        outHLSL.c_str(), reinterpret_cast<char*>(errors->GetBufferPointer()));
  }
}
...
sk_sp<GrD3DPipelineState> GrD3DPipelineStateBuilder::finalize() {
  ...
  this->compileD3DProgram(
      SkSL::Program::kVertex_Kind, fVS.fCompilerString, settings, &vertexShader, &vertInputs);
  this->compileD3DProgram(
      SkSL::Program::kFragment_Kind, fFS.fCompilerString, settings, &pixelShader, &fragInputs);
  ...
  if (this->primitiveProcessor().willUseGeoShader()) {
    this->compileD3DProgram(
        SkSL::Program::kGeometry_Kind, fGS.fCompilerString, settings, &geometryShader, &geomInputs);
  ...
  return GrD3DPipelineState::Make(
      fGpu, fProgramInfo, std::move(rootSig), std::move(vertexShader), std::move(geometryShader),
      std::move(pixelShader), rt->dxgiFormat(), rt->stencilDxgiFormat(), rt->sampleQualityLevel());
}
