...
sk_sp<GrD3DPipelineState> GrD3DPipelineStateBuilder::MakePipelineState(
    GrD3DGpu* gpu, GrRenderTarget* renderTarget, const GrProgramDesc& desc,
...
static gr_cp<ID3DBlob> GrCompileHLSLShader(
    GrD3DGpu* gpu, const SkSL::String& hlsl, SkSL::Program::Kind kind) {
  const char* compileTarget = nullptr;
  switch (kind) {
    case SkSL::Program::kVertex_Kind: compileTarget = "vs_5_1"; break;
    case SkSL::Program::kGeometry_Kind: compileTarget = "gs_5_1"; break;
    case SkSL::Program::kFragment_Kind: compileTarget = "ps_5_1"; break;
...
gr_cp<ID3DBlob> GrD3DPipelineStateBuilder::compileD3DProgram(
    SkSL::Program::Kind kind, const SkSL::String& sksl, const SkSL::Program::Settings& settings,
    SkSL::Program::Inputs* outInputs, SkSL::String* outHLSL) {
  auto errorHandler = fGpu->getContext()->priv().getShaderErrorHandler();
  std::unique_ptr<SkSL::Program> program =
      fGpu->shaderCompiler()->convertProgram(kind, sksl, settings);
  if (!program) {
    errorHandler->compileError(sksl.c_str(), fGpu->shaderCompiler()->errorText().c_str());
...
static void setup_vertex_input_layout(
    const GrPrimitiveProcessor& primProc, D3D12_INPUT_ELEMENT_DESC* inputElements) {
...
static void fill_in_rasterizer_state(
    const GrPipeline& pipeline, const GrCaps* caps, D3D12_RASTERIZER_DESC* rasterizer) {
...
gr_cp<ID3D12PipelineState> create_pipeline_state(
  ...
  if (geometryShader.get()) {
    psoDesc.GS = {
        reinterpret_cast<UINT8*>(geometryShader->GetBufferPointer()),
