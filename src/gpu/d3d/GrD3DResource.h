...
class GrD3DResource : SkNoncopyable {
 public:
  GrD3DResource(
      ID3D12Resource* resource, const D3D12_RESOURCE_DESC& desc, sk_sp<GrD3DResourceState> state,
  ...
  GrBackendFormat getBackendFormat() const { return GrBackendFormat::MakeDxgi(this->dxgiFormat()); }
