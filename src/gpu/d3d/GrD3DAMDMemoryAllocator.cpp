...
sk_sp<GrD3DMemoryAllocator> GrD3DAMDMemoryAllocator::Make(
    IDXGIAdapter* adapter, ID3D12Device* device) {
...
gr_cp<ID3D12Resource> GrD3DAMDMemoryAllocator::createResource(
  ...
  HRESULT hr = fAllocator->CreateResource(
      &allocationDesc, resourceDesc, initialResourceState, clearValue, &d3d12maAllocation,
