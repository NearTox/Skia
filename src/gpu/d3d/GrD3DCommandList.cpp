...
void GrD3DCommandList::copyBufferToTexture(
    const GrD3DBuffer* srcBuffer, const GrD3DTextureResource* dstTexture, uint32_t subresourceCount,
...
void GrD3DCommandList::copyTextureRegion(
...
void GrD3DCommandList::copyBufferToBuffer(
    sk_sp<GrManagedResource> dst, ID3D12Resource* dstBuffer, uint64_t dstOffset,
    sk_sp<GrManagedResource> src, ID3D12Resource* srcBuffer, uint64_t srcOffset,
    uint64_t numBytes) {
  SkASSERT(fIsActive);
...
std::unique_ptr<GrD3DDirectCommandList> GrD3DDirectCommandList::Make(ID3D12Device* device) {
  ...
  SkDEBUGCODE(hr =) device->CreateCommandList(
...
void GrD3DDirectCommandList::setVertexBuffers(
    unsigned int startSlot, const GrD3DBuffer* vertexBuffer, size_t vertexStride,
