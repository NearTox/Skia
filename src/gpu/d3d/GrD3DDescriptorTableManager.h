...
class GrD3DDescriptorTable {
 public:
  GrD3DDescriptorTable(
      D3D12_CPU_DESCRIPTOR_HANDLE baseCPU, D3D12_GPU_DESCRIPTOR_HANDLE baseGPU,
      D3D12_DESCRIPTOR_HEAP_TYPE type)
      : fDescriptorTableCpuStart(baseCPU), fDescriptorTableGpuStart(baseGPU), fType(type) {}

  const D3D12_CPU_DESCRIPTOR_HANDLE* baseCpuDescriptorPtr() { return &fDescriptorTableCpuStart; }

  const D3D12_GPU_DESCRIPTOR_HANDLE baseGpuDescriptor() { return fDescriptorTableGpuStart; }
...
class GrD3DDescriptorTableManager {
 public:
  ...
  std::unique_ptr<GrD3DDescriptorTable> createShaderOrConstantResourceTable(
      GrD3DGpu*, unsigned int count);
 ...
 private:
  class Heap : public GrRecycledResource {
   public:
    ...
    void reset() { fNextAvailable = 0; }

   private:
    Heap(
        GrD3DGpu* gpu, std::unique_ptr<GrD3DDescriptorHeap>& heap, D3D12_DESCRIPTOR_HEAP_TYPE type,
