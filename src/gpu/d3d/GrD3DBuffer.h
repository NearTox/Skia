...
class GrD3DBuffer : public GrGpuBuffer {
 ...
 protected:
  GrD3DBuffer(
      GrD3DGpu*, size_t size, GrGpuBufferType, GrAccessPattern, const sk_sp<Resource>&,
 ...
 private:
  ...
  class Resource : public GrRecycledResource {
   public:
    explicit Resource() : fD3DResource(nullptr) {}

    Resource(const gr_cp<ID3D12Resource>& bufferResource) : fD3DResource(bufferResource) {
      SkASSERT(
          bufferResource.get() &&
          bufferResource->GetDesc().Dimension == D3D12_RESOURCE_DIMENSION_BUFFER);
    }

    static sk_sp<Resource> Make(
        GrD3DGpu*, size_t size, GrGpuBufferType, GrAccessPattern, D3D12_RESOURCE_STATES*);
   ...
   private:
    void freeGPUData() const override { fD3DResource.reset(); }
