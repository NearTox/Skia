...
sk_sp<GrD3DBuffer::Resource> GrD3DBuffer::Resource::Make(
    GrD3DGpu* gpu, size_t size, GrGpuBufferType intendedType, GrAccessPattern accessPattern,
  ...
  HRESULT hr = gpu->device()->CreateCommittedResource(
      &heapProperties, D3D12_HEAP_FLAG_NONE, &bufferDesc, *resourceState, nullptr,
...
sk_sp<GrD3DBuffer> GrD3DBuffer::Make(
    GrD3DGpu* gpu, size_t size, GrGpuBufferType intendedType, GrAccessPattern accessPattern) {
  SkASSERT(!gpu->protectedContext() || (accessPattern != kStatic_GrAccessPattern));
  D3D12_RESOURCE_STATES resourceState;
  sk_sp<Resource> resource = Resource::Make(gpu, size, intendedType, accessPattern, &resourceState);
  if (!resource) {
    return nullptr;
  }

  return sk_sp<GrD3DBuffer>(
      new GrD3DBuffer(gpu, size, intendedType, accessPattern, std::move(resource), resourceState));
}

GrD3DBuffer::GrD3DBuffer(
    GrD3DGpu* gpu, size_t size, GrGpuBufferType intendedType, GrAccessPattern accessPattern,
    const sk_sp<Resource>& bufferResource, D3D12_RESOURCE_STATES resourceState)
...
void GrD3DBuffer::setResourceState(const GrD3DGpu* gpu, D3D12_RESOURCE_STATES newResourceState) {
...
void GrD3DBuffer::onMap() { this->internalMap(this->size()); }

void GrD3DBuffer::onUnmap() { this->internalUnmap(this->size()); }
...
void GrD3DBuffer::internalMap(size_t size) {
  ...
  if (this->accessPattern() == kStatic_GrAccessPattern) {
    ...
    fMappedResource = Resource::Make(
        this->getD3DGpu(), size, GrGpuBufferType::kXferCpuToGpu,
        GrAccessPattern::kDynamic_GrAccessPattern, &resourceState);
...
void GrD3DBuffer::internalUnmap(size_t size) {
  ...
  if (this->accessPattern() == kStatic_GrAccessPattern) {
    ...
    this->getD3DGpu()->currentCommandList()->copyBufferToBuffer(
        fResource, fResource->fD3DResource.get(), 0, fMappedResource,
        fMappedResource->fD3DResource.get(), 0, size);
  } else {
    D3D12_RANGE range;
    range.Begin = 0;
...
void GrD3DBuffer::validate() const {
  SkASSERT(
      !fResource || this->intendedType() == GrGpuBufferType::kVertex ||
  ...
  SkASSERT(
      !fMappedResource || !fResource ||
      (fResource->size() == this->size() && fMappedResource->size() <= fResource->size()));
}
#endif
