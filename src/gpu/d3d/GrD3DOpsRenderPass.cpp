...
void set_stencil_ref(GrD3DGpu* gpu, const GrProgramInfo& info) {
  ...
  if (!stencilSettings.isDisabled()) {
    ...
    if (stencilSettings.isTwoSided()) {
      SkASSERT(
          stencilSettings.postOriginCCWFace(info.origin()).fRef ==
...
void set_primitive_topology(GrD3DGpu* gpu, const GrProgramInfo& info) {
  ...
  switch (info.primitiveType()) {
    case GrPrimitiveType::kTriangles: topology = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST; break;
    case GrPrimitiveType::kTriangleStrip: topology = D3D_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP; break;
    case GrPrimitiveType::kPoints: topology = D3D_PRIMITIVE_TOPOLOGY_POINTLIST; break;
    case GrPrimitiveType::kLines: topology = D3D_PRIMITIVE_TOPOLOGY_LINELIST; break;
    case GrPrimitiveType::kLineStrip: topology = D3D_PRIMITIVE_TOPOLOGY_LINESTRIP; break;
...
void set_scissor_rects(
    GrD3DGpu* gpu, const GrRenderTarget* renderTarget, GrSurfaceOrigin rtOrigin,
...
bool GrD3DOpsRenderPass::onBindTextures(
  ...
  pipeline.visitTextureEffects(
      [&](const GrTextureEffect& te) { update_resource_state(te.texture(), fRenderTarget, fGpu); });
...
void GrD3DOpsRenderPass::onBindBuffers(
  ...
  fCurrentPipelineState->bindBuffers(
      fGpu, std::move(indexBuffer), std::move(instanceBuffer), std::move(vertexBuffer),
      currCmdList);
}

void GrD3DOpsRenderPass::onDrawInstanced(
...
void GrD3DOpsRenderPass::onDrawIndexedInstanced(
    int indexCount, int baseIndex, int instanceCount, int baseInstance, int baseVertex) {
  SkASSERT(fCurrentPipelineState);
  fGpu->currentCommandList()->drawIndexedInstanced(
...
void GrD3DOpsRenderPass::onDrawIndirect(const GrBuffer* buffer, size_t offset, int drawCount) {
  ...
  fGpu->currentCommandList()->executeIndirect(
      cmdSig, drawCount, static_cast<const GrD3DBuffer*>(buffer), offset);
  fGpu->stats()->incNumDraws();
}

void GrD3DOpsRenderPass::onDrawIndexedIndirect(
    const GrBuffer* buffer, size_t offset, int drawCount) {
  constexpr unsigned int kSlot = 0;
  sk_sp<GrD3DCommandSignature> cmdSig = fGpu->resourceProvider().findOrCreateCommandSignature(
...
static D3D12_RECT scissor_to_d3d_clear_rect(
    const GrScissorState& scissor, const GrSurface* surface, GrSurfaceOrigin origin) {
  ...
  if (!scissor.enabled()) {
    ...
    d3dRect.setLTRB(
        scissor.rect().fLeft, surface->height() - scissor.rect().fBottom, scissor.rect().fRight,
        surface->height() - scissor.rect().fTop);
  }
