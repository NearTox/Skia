...
std::unique_ptr<GrD3DDirectCommandList> GrD3DResourceProvider::findOrCreateDirectCommandList() {
  if (fAvailableDirectCommandLists.count()) {
    std::unique_ptr<GrD3DDirectCommandList> list = std::move(fAvailableDirectCommandLists.back());
...
sk_sp<GrD3DRootSignature> GrD3DResourceProvider::findOrCreateRootSignature(
    int numTextureSamplers, int numUAVs) {
...
D3D12_CPU_DESCRIPTOR_HANDLE GrD3DResourceProvider::findOrCreateCompatibleSampler(
  ...
  float maxLOD = params.mipmapped() == GrMipmapped::kYes ? std::numeric_limits<float>::max() : 0.f;
  ...
  D3D12_CPU_DESCRIPTOR_HANDLE sampler =
      fCpuDescriptorManager.createSampler(fGpu, filter, maxLOD, addressModeU, addressModeV).fHandle;
...
sk_sp<GrD3DPipeline> GrD3DResourceProvider::findOrCreateMipmapPipeline() {
  if (!fMipmapPipeline) {
    ...
    fMipmapPipeline = GrD3DPipelineStateBuilder::MakeComputePipeline(fGpu, rootSig.get(), shader);
  }
...
void GrD3DResourceProvider::PipelineStateCache::release() { fMap.reset(); }
...
GrD3DPipelineState* GrD3DResourceProvider::PipelineStateCache::refPipelineState(
  ...
  if (!entry) {
    ...
    entry = fMap.insert(desc, std::unique_ptr<Entry>(new Entry(fGpu, std::move(pipelineState))));
...
void GrD3DResourceProvider::DescriptorTableCache::release() { fMap.reset(); }
...
sk_sp<GrD3DDescriptorTable> GrD3DResourceProvider::DescriptorTableCache::findOrCreateDescTable(
  ...
  fGpu->device()->CopyDescriptors(
      1, descTable->baseCpuDescriptorPtr(), &numDescriptors, numDescriptors, cpuDescriptors.data(),
      fRangeSizes, descTable->type());
