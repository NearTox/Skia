...
GrD3DPipelineState::GrD3DPipelineState(
    gr_cp<ID3D12PipelineState> pipelineState, sk_sp<GrD3DRootSignature> rootSignature,
    const GrGLSLBuiltinUniformHandles& builtinUniformHandles, const UniformInfoArray& uniforms,
    uint32_t uniformSize, uint32_t numSamplers,
...
void GrD3DPipelineState::setAndBindConstants(
    GrD3DGpu* gpu, const GrRenderTarget* renderTarget, const GrProgramInfo& programInfo) {
...

  {
    ...
    fXferProcessor->setData(
        fDataManager, programInfo.pipeline().getXferProcessor(), dstTexture, offset);
  }
...
void GrD3DPipelineState::setAndBindTextures(
    GrD3DGpu* gpu, const GrPrimitiveProcessor& primProc,
  ...
  if (GrTexture* dstTexture = pipeline.peekDstTexture()) {
    ...
    shaderResourceViews[currTextureBinding] = texture->shaderResourceView();
    samplers[currTextureBinding] =
        gpu->resourceProvider().findOrCreateCompatibleSampler(GrSamplerState::Filter::kNearest);
    gpu->currentCommandList()->addSampledTextureRef(texture);
    rangeSizes[currTextureBinding++] = 1;
  }
  ...
  if (fNumSamplers > 0) {
    ...
    gpu->device()->CopyDescriptors(
        1, srvTable->baseCpuDescriptorPtr(), &fNumSamplers, fNumSamplers, shaderResourceViews.get(),
        rangeSizes.get(), srvTable->type());
    ...
    gpu->device()->CopyDescriptors(
        1, samplerTable->baseCpuDescriptorPtr(), &fNumSamplers, fNumSamplers, samplers.get(),
        rangeSizes.get(), samplerTable->type());
    gpu->currentCommandList()->setGraphicsRootDescriptorTable(
...
void GrD3DPipelineState::bindBuffers(
    GrD3DGpu* gpu, const GrBuffer* indexBuffer, const GrBuffer* instanceBuffer,
  ...
  if (auto* d3dVertexBuffer = static_cast<const GrD3DBuffer*>(vertexBuffer)) {
    ...
    commandList->setVertexBuffers(
        0, d3dVertexBuffer, fVertexStride, d3dInstanceBuffer, fInstanceStride);
  }
