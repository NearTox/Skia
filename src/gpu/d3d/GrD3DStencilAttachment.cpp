...
GrD3DStencilAttachment::GrD3DStencilAttachment(
    GrD3DGpu* gpu, const Format& format, const D3D12_RESOURCE_DESC& desc,
    const GrD3DTextureResourceInfo& info, sk_sp<GrD3DResourceState> state,
    const GrD3DDescriptorHeap::CPUHandle& view)
    : GrStencilAttachment(gpu, desc.Width, desc.Height, format.fStencilBits, desc.SampleDesc.Count),
...
GrD3DStencilAttachment* GrD3DStencilAttachment::Make(
  ...
  GrD3DStencilAttachment* stencil =
      new GrD3DStencilAttachment(gpu, format, resourceDesc, info, std::move(state), view);
  return stencil;
}
