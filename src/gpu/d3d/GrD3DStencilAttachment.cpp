...
GrD3DStencilAttachment::GrD3DStencilAttachment(
    GrD3DGpu* gpu, SkISize dimensions, const Format& format, const D3D12_RESOURCE_DESC& desc,
    const GrD3DTextureResourceInfo& info, sk_sp<GrD3DResourceState> state,
    const GrD3DDescriptorHeap::CPUHandle& view)
    : GrStencilAttachment(
          gpu, dimensions, format.fStencilBits, desc.SampleDesc.Count, GrProtected::kNo),
...
GrD3DStencilAttachment* GrD3DStencilAttachment::Make(
  ...
  if (!GrD3DTextureResource::InitTextureResourceInfo(
          gpu, resourceDesc, D3D12_RESOURCE_STATE_DEPTH_WRITE, GrProtected::kNo, &clearValue,
          &info)) {
    return nullptr;
  }
  ...
  GrD3DStencilAttachment* stencil = new GrD3DStencilAttachment(
      gpu, dimensions, format, resourceDesc, info, std::move(state), view);
  return stencil;
}
