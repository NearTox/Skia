...
GrD3DDescriptorHeap::CPUHandle GrD3DCpuDescriptorManager::createConstantBufferView(
  ...
  const GrD3DDescriptorHeap::CPUHandle& descriptor = fShaderViewDescriptorPool.allocateHandle(gpu);
...
GrD3DDescriptorHeap::CPUHandle GrD3DCpuDescriptorManager::createShaderResourceView(
    GrD3DGpu* gpu, ID3D12Resource* resource, unsigned int mostDetailedMip, unsigned int mipLevels) {
  const GrD3DDescriptorHeap::CPUHandle& descriptor = fShaderViewDescriptorPool.allocateHandle(gpu);
...
GrD3DDescriptorHeap::CPUHandle GrD3DCpuDescriptorManager::createSampler(
    GrD3DGpu* gpu, D3D12_FILTER filter, float maxLOD, D3D12_TEXTURE_ADDRESS_MODE addressModeU,
...
GrD3DCpuDescriptorManager::HeapPool::HeapPool(GrD3DGpu* gpu, D3D12_DESCRIPTOR_HEAP_TYPE heapType)
    : fMaxAvailableDescriptors(32), fHeapType(heapType) {
...
GrD3DDescriptorHeap::CPUHandle GrD3DCpuDescriptorManager::HeapPool::allocateHandle(GrD3DGpu* gpu) {
