...
sk_sp<GrD3DRenderTarget> GrD3DRenderTarget::MakeWrappedRenderTarget(
  ...
  if (sampleCnt > 1) {
    ...
    d3dRT = new GrD3DRenderTarget(
        ...
        msaaRenderTargetView, renderTargetView, kWrapped);
  } else {
    d3dRT =
        new GrD3DRenderTarget(gpu, dimensions, info, std::move(state), renderTargetView, kWrapped);
  }
...
GrBackendRenderTarget GrD3DRenderTarget::getBackendRenderTarget() const {
  return GrBackendRenderTarget(
      this->width(), this->height(), this->numSamples(), fInfo, this->grD3DResourceState());
}
