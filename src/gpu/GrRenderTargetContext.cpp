...
GrRenderTargetContext::GrRenderTargetContext(
    ...
    sk_sp<SkColorSpace> colorSpace, const SkSurfaceProps* surfaceProps, bool managedOpList)
...
void GrRenderTargetContext::drawTexturedQuad(
    const GrClip& clip, sk_sp<GrTextureProxy> proxy, sk_sp<GrColorSpaceXform> textureXform,
    GrSamplerState::Filter filter, const SkPMColor4f& color, SkBlendMode blendMode, GrAA aa,
    GrQuadAAFlags edgeFlags, const GrQuad& deviceQuad, const GrQuad& localQuad,
    const SkRect* domain) {
  ASSERT_SINGLE_OWNER
  RETURN_IF_ABANDONED
  SkDEBUGCODE(this->validate();)
  ...
  if (opt != QuadOptimization::kDiscarded) {
    ...
    this->addDrawOp(
        finalClip, GrTextureOp::Make(
                       fContext, std::move(proxy), std::move(textureXform), filter, color,
                       blendMode, aaType, edgeFlags, croppedDeviceQuad, croppedLocalQuad, domain));
  }
}
...
GrRenderTargetOpList::CanDiscardPreviousOps
GrRenderTargetContext::canDiscardPreviousOpsOnFullClear() const {
#if GR_TEST_UTILS
...
void GrRenderTargetContext::setNeedsStencil(bool multisampled) {
  ...
  if (multisampled && 1 == numRequiredSamples) {
    ...
    numRequiredSamples = this->caps()->internalMultisampleCount(this->asSurfaceProxy()->config());
  }
  ...
  if (mode != SkBlendMode::kSrcOver ||
    ...
    for (int i = 0; i < cnt; ++i) {
      ...
      this->drawTexturedQuad(
          clip, set[i].fProxy, texXform, filter, {alpha, alpha, alpha, alpha}, mode, aa,
...
void GrRenderTargetContext::asyncRescaleAndReadPixels(
  ...
  if (needsRescale) {
    ...
    sk_sp<GrColorSpaceXform> xform = GrColorSpaceXform::Make(
        this->colorSpaceInfo().colorSpace(), this->colorSpaceInfo().alphaType(), info.colorSpace(),
        info.alphaType());
    ...
    if (this->origin() == kBottomLeft_GrSurfaceOrigin || xform) {
      ...
      if (!texProxy) {
        texProxy = GrSurfaceProxy::Copy(
            fContext, fRenderTargetProxy.get(), GrMipMapped::kNo, srcRect, SkBackingFit::kApprox,
            SkBudgeted::kNo);
        if (!texProxy) {
          callback(context, nullptr, 0);
          return;
...
GrRenderTargetContext::PixelTransferResult GrRenderTargetContext::transferPixels(
  ...
  if (flip) {
    srcRect = SkIRect::MakeLTRB(
        rect.fLeft, this->height() - rect.fBottom, rect.fRight, this->height() - rect.fTop);
  }
  ...
  if (supportedRead.fColorType != dstCT || supportedRead.fSwizzle != GrSwizzle("rgba") || flip) {
    result.fPixelConverter = [w = rect.width(), h = rect.height(), dstCT, supportedRead](
      ...
      GrConvertPixels(
          dstInfo, dst, dstInfo.minRowBytes(), srcInfo, src, srcInfo.minRowBytes(),
...
void GrRenderTargetContext::asyncReadPixels(
  ...
  if (!transferResult.fTransferBuffer) {
    ...
    auto ii = SkImageInfo::Make(
        rect.width(), rect.height(), colorType, this->colorSpaceInfo().alphaType(),
...
void GrRenderTargetContext::asyncRescaleAndReadPixelsYUV420(
  ...
  if (needsRescale) {
    ...
    auto info =
        SkImageInfo::Make(dstW, dstH, kRGBA_8888_SkColorType, kPremul_SkAlphaType, dstColorSpace);
    ...
    if (xform) {
      ...
      SkRect srcRectToDraw = SkRect::Make(srcRect);
      rtc = direct->priv().makeDeferredRenderTargetContext(
          SkBackingFit::kApprox, dstW, dstH, this->colorSpaceInfo().colorType(), dstColorSpace, 1,
  ...
  auto yRTC = direct->priv().makeDeferredRenderTargetContextWithFallback(
      SkBackingFit::kApprox, dstW, dstH, GrColorType::kAlpha_8, dstColorSpace, 1, GrMipMapped::kNo,
      kTopLeft_GrSurfaceOrigin);
...
bool GrRenderTargetContext::setupDstProxy(
  ...
  sk_sp<GrTextureProxy> newProxy = GrSurfaceProxy::Copy(
      fContext, rtProxy, GrMipMapped::kNo, copyRect, fit, SkBudgeted::kYes,
...
bool GrRenderTargetContext::blitTexture(
  ...
  GrPaint paint;
  paint.setPorterDuffXPFactory(SkBlendMode::kSrc);
  auto fp = GrSimpleTextureEffect::Make(sk_ref_sp(src->asTextureProxy()), SkMatrix::I());
  if (!fp) {
    return false;
  }
