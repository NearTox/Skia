...
void GrRenderTargetContext::drawGlyphRunList(
  ...
  if (blob == nullptr || !blob->canReuse(blobPaint, drawMatrix)) {
    ...
    fGlyphPainter.processGlyphRunList(
        ...
        fSurfaceProps, supportsSDFT, options, blob.get());
  }
...
void GrRenderTargetContext::drawRect(
  ...
  if (stroke.getStyle() == SkStrokeRec::kFill_Style) {
    ...
    GrAAType aaType = this->chooseAAType(aa);
    GrOp::Owner op =
        GrStrokeRectOp::Make(fContext, std::move(paint), aaType, viewMatrix, rect, stroke);
...
void GrRenderTargetContextPriv::stencilPath(
  ...
  GrOp::Owner op = GrStencilPathOp::Make(
...
void GrRenderTargetContext::drawVertices(
  ...
  GrOp::Owner op = GrDrawVerticesOp::Make(
...
void GrRenderTargetContext::drawAtlas(
  ...
  GrAAType aaType = this->chooseAAType(GrAA::kNo);
  GrOp::Owner op = GrDrawAtlasOp::Make(
...
bool GrRenderTargetContext::drawFastShadow(
  ...
  if (SkColorGetA(rec.fAmbientColor) > 0) {
    ...
    GrOp::Owner op = GrShadowRRectOp::Make(
...
void GrRenderTargetContext::drawImageLattice(
  ...
  GrOp::Owner op = GrLatticeOp::MakeNonAA(
  ...
  if (!shape.style().hasPathEffect()) {
    ...
    if (shape.asRRect(&rrect, nullptr, nullptr, &inverted) && !inverted) {
      ...
      if (shape.asNestedRects(rects)) {
        ...
        GrOp::Owner op = GrStrokeRectOp::MakeNested(fContext, std::move(paint), viewMatrix, rects);
        if (op) {
          this->addDrawOp(clip, std::move(op));
        }
...
void GrRenderTargetContext::addDrawOp(
  ...
  if (clip) {
    ...
    GrAAType aaType =
        usesHWAA ? GrAAType::kMSAA : (op->hasAABloat() ? GrAAType::kCoverage : GrAAType::kNone);
    skipDraw = clip->apply(fContext, this, aaType, usesUserStencilBits, &appliedClip, &bounds) ==
  ...
  GrProcessorSet::Analysis analysis =
      drawOp->finalize(*this->caps(), &appliedClip, hasMixedSampledCoverage, clampType);
