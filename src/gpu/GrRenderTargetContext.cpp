...
GrRenderTargetContext::GrRenderTargetContext(
    GrRecordingContext* context, sk_sp<GrRenderTargetProxy> rtp, sk_sp<SkColorSpace> colorSpace,
    const SkSurfaceProps* surfaceProps, bool managedOpList)
...
inline GrAAType GrRenderTargetContext::chooseAAType(GrAA aa) {
  ...
  switch (fsaaType) {
    ...
    case GrFSAAType::kMixedSamples: return GrAAType::kCoverage;
    case GrFSAAType::kUnifiedMSAA: return GrAAType::kMSAA;
  }
...
static inline GrPathRenderer::AATypeFlags choose_path_aa_type_flags(
  ...
  switch (fsaaType) {
    case GrFSAAType::kNone: return AATypeFlags::kCoverage;
    case GrFSAAType::kMixedSamples:
      return AATypeFlags::kCoverage | AATypeFlags::kMixedSampledStencilThenCover;
    case GrFSAAType::kUnifiedMSAA: return AATypeFlags::kMSAA;
  }
...
GrOpList* GrRenderTargetContext::getOpList() { return this->getRTOpList(); }
  ...
  if (isFull) {
    ...
    if (this->caps()->performColorClearsAsDraws()) {
      ...
      this->getRTOpList()->addOp(
          GrClearOp::Make(
              fContext, SkIRect::MakeEmpty(), color,
              /* fullscreen */ true),
          *this->caps());
    }
  } else {
...
void GrRenderTargetContextPriv::absClear(const SkIRect* clearRect, const SkPMColor4f& color) {
  ...
  SkIRect rtRect = SkIRect::MakeWH(
      fRenderTargetContext->fRenderTargetProxy->worstCaseWidth(),
  ...
  if (clearRect) {
    if (fRenderTargetContext->caps()->performPartialClearsAsDraws()) {
      ...
      fRenderTargetContext->addDrawOp(
          GrFixedClip::Disabled(), GrFillRectOp::MakeNonAARect(
                                       fRenderTargetContext->fContext, std::move(paint),
      ...
      fRenderTargetContext->getRTOpList()->addOp(
          GrClearOp::Make(
              fRenderTargetContext->fContext, rtRect, color,
              /* fullscreen */ false),
...
void GrRenderTargetContext::drawPaint(
  ...
  if (!paint.numTotalFragmentProcessors()) {
    ...
    if (clip.isRRect(r, &rrect, &aa)) {
      if (rrect.isRect()) {
        ...
        this->drawRRect(
            GrNoClip(), std::move(paint), aa, SkMatrix::I(), rrect, GrStyle::SimpleFill());
      }
    } else {
  ...
  std::unique_ptr<GrDrawOp> op = GrFillRectOp::MakeGeneric(
      fContext, std::move(paint), GrAAType::kNone, GrQuadAAFlags::kNone, GrPerspQuad(r),
      GrPerspQuad::MakeFromRect(r, localMatrix));
...
static bool crop_filled_rect(
    int width, int height, const GrClip& clip, const SkMatrix& viewMatrix, SkRect* rect,
...
bool GrRenderTargetContext::drawFilledRectAsClear(
  ...
  if (combinedRect.contains(rtRect)) {
    ...
    return true;
      ...
      GrClip::IsPixelAligned(combinedRect) && combinedRect.width() > 256 &&
      combinedRect.height() > 256) {
  ...
  this->addDrawOp(
      GrFixedClip::Disabled(), GrFillRectOp::MakeGeneric(
                                   fContext, std::move(paint), aaType, edgeFlags,
...
void GrRenderTargetContext::drawFilledRect(
  ...
  GrAAType aaType = this->chooseAAType(aa);
  this->addDrawOp(
      clip, GrFillRectOp::MakeGeneric(
                fContext, std::move(paint), aaType,
...
void GrRenderTargetContext::drawQuadSet(
  ...
  GrAAType aaType = this->chooseAAType(aa);
  this->addDrawOp(
      clip, GrFillRectOp::MakeSet(fContext, std::move(paint), aaType, viewMatrix, quads, cnt));
}
...
void GrRenderTargetContext::internalStencilClear(const GrFixedClip& clip, bool insideStencilMask) {
  if (this->caps()->performStencilClearsAsDraws()) {
    ...
    std::unique_ptr<GrOp> op(
        GrClearStencilClipOp::Make(fContext, clip, insideStencilMask, fRenderTargetProxy.get()));
    if (!op) {
      return;
    }
    this->getRTOpList()->addOp(std::move(op), *this->caps());
...
void GrRenderTargetContextPriv::stencilPath(
  ...
  std::unique_ptr<GrOp> op = GrStencilPathOp::Make(
      fRenderTargetContext->fContext, viewMatrix, GrAA::kYes == doStencilMSAA, path->getFillType(),
      appliedClip.hasStencilClip(), appliedClip.scissorState(), path);
  if (!op) {
    return;
  }
...
void GrRenderTargetContextPriv::stencilRect(
    const GrClip& clip, const GrUserStencilSettings* ss, GrPaint&& paint, GrAA doStencilMSAA,
  ...
  GrPerspQuad localQuad =
      localMatrix ? GrPerspQuad::MakeFromRect(rect, *localMatrix) : GrPerspQuad(rect);
  std::unique_ptr<GrDrawOp> op = GrFillRectOp::MakeGeneric(
...
void GrRenderTargetContext::fillRectWithEdgeAA(
    const GrClip& clip, GrPaint&& paint, GrAA aa, GrQuadAAFlags edgeAA, const SkMatrix& viewMatrix,
  ...
  if (localRect) {
    ...
    if (!crop_filled_rect(
            this->width(), this->height(), clip, viewMatrix, &croppedRect, &croppedLocalRect)) {
      return;
    }
    op = GrFillRectOp::MakeGeneric(
        fContext, std::move(paint), aaType, edgeAA,
        GrPerspQuad::MakeFromRect(croppedRect, viewMatrix), GrPerspQuad(croppedLocalRect));
  } else {
    ...
    if (aaType != GrAAType::kMSAA &&
      ...
      this->drawFilledRect(
          clip, std::move(paint), GrAA(edgeAA == GrQuadAAFlags::kAll), viewMatrix, rect);
      return;
    }
...
void GrRenderTargetContext::fillQuadWithEdgeAA(
    const GrClip& clip, GrPaint&& paint, GrAA aa, GrQuadAAFlags edgeAA, const SkMatrix& viewMatrix,
  ...
  this->addDrawOp(
      clip,
      GrFillRectOp::MakeGeneric(
          fContext, std::move(paint), aaType, edgeAA, GrPerspQuad::MakeFromSkQuad(quad, viewMatrix),
          GrPerspQuad::MakeFromSkQuad(localPoints, SkMatrix::I())));
}

// Creates a paint for GrFillRectOp that matches behavior of GrTextureOp
static void draw_texture_to_grpaint(
    sk_sp<GrTextureProxy> proxy, const SkRect* domain, GrSamplerState::Filter filter,
    SkBlendMode mode, const SkPMColor4f& color, sk_sp<GrColorSpaceXform> csXform, GrPaint* paint) {
  paint->setColor4f(color);
...
void GrRenderTargetContext::drawTexture(
    const GrClip& clip, sk_sp<GrTextureProxy> proxy, GrSamplerState::Filter filter,
    SkBlendMode mode, const SkPMColor4f& color, const SkRect& srcRect, const SkRect& dstRect,
  ...
  if (!crop_filled_rect(
          this->width(), this->height(), clip, viewMatrix, &clippedDstRect, &clippedSrcRect)) {
    return;
  }
  ...
  if (mode != SkBlendMode::kSrcOver) {
    ...
    draw_texture_to_grpaint(
        std::move(proxy), constraint == SkCanvas::kStrict_SrcRectConstraint ? &srcRect : nullptr,
        filter, mode, color, std::move(textureColorSpaceXform), &paint);
    op = GrFillRectOp::MakeGeneric(
        fContext, std::move(paint), aaType, aaFlags,
    ...
    op = GrTextureOp::Make(
        fContext, std::move(proxy), filter, color, clippedSrcRect, clippedDstRect, aaType, aaFlags,
...
void GrRenderTargetContext::drawTextureQuad(
    const GrClip& clip, sk_sp<GrTextureProxy> proxy, GrSamplerState::Filter filter,
    SkBlendMode mode, const SkPMColor4f& color, const SkPoint srcQuad[4], const SkPoint dstQuad[4],
    GrAA aa, GrQuadAAFlags aaFlags, const SkRect* domain, const SkMatrix& viewMatrix,
  ...
  if (mode != SkBlendMode::kSrcOver) {
    ...
    op = GrTextureOp::MakeQuad(
        fContext, std::move(proxy), filter, color, srcQuad, dstQuad, aaType, aaFlags, domain,
...
void GrRenderTargetContext::drawTextureSet(
    const GrClip& clip, const TextureSetEntry set[], int cnt, GrSamplerState::Filter filter,
    SkBlendMode mode, GrAA aa, SkCanvas::SrcRectConstraint constraint, const SkMatrix& viewMatrix,
    sk_sp<GrColorSpaceXform> texXform) {
  ASSERT_SINGLE_OWNER
  RETURN_IF_ABANDONED
  SkDEBUGCODE(this->validate());
  ...
  if (mode != SkBlendMode::kSrcOver ||
    ...
    for (int i = 0; i < cnt; ++i) {
      ...
      if (set[i].fDstClipQuad == nullptr) {
        ...
        // being drawn.
        this->drawTexture(
            clip, set[i].fProxy, filter, mode, {alpha, alpha, alpha, alpha}, set[i].fSrcRect,
            set[i].fDstRect, aa, set[i].fAAFlags, constraint, ctm, texXform);
      } else {
        ...
        const SkRect* domain =
            constraint == SkCanvas::kStrict_SrcRectConstraint ? &set[i].fSrcRect : nullptr;
        this->drawTextureQuad(
            clip, set[i].fProxy, filter, mode, {alpha, alpha, alpha, alpha}, srcQuad,
            set[i].fDstClipQuad, aa, set[i].fAAFlags, domain, ctm, texXform);
      }
    }
  } else {
    ...
    AutoCheckFlush acf(this->drawingManager());
    GrAAType aaType = this->chooseAAType(aa);
    auto op = GrTextureOp::MakeSet(
        fContext, set, cnt, filter, aaType, constraint, viewMatrix, std::move(texXform));
    this->addDrawOp(clip, std::move(op));
  }
}

void GrRenderTargetContext::fillRectWithLocalMatrix(
    const GrClip& clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix,
...
void GrRenderTargetContext::drawImageLattice(
    const GrClip& clip, GrPaint&& paint, const SkMatrix& viewMatrix, sk_sp<GrTextureProxy> image,
    sk_sp<GrColorSpaceXform> csxf, GrSamplerState::Filter filter,
  ...
  std::unique_ptr<GrDrawOp> op = GrLatticeOp::MakeNonAA(
      fContext, std::move(paint), viewMatrix, std::move(image), std::move(csxf), filter,
      std::move(iter), dst);
  this->addDrawOp(clip, std::move(op));
}
...
sk_sp<GrRenderTargetContext> GrRenderTargetContext::rescale(
  ...
  if (!this->asTextureProxy()) {
    ...
    auto sContext = direct->priv().makeDeferredSurfaceContext(
        fRenderTargetProxy->backendFormat().makeTexture2D(), desc, this->origin(), GrMipMapped::kNo,
        SkBackingFit::kApprox, SkBudgeted::kNo, this->colorSpaceInfo().refColorSpace());
    if (!sContext) {
      return nullptr;
    }
  ...
  if (rescaleQuality > kNone_SkFilterQuality) {
    stepsX = static_cast<int>((sx > 1.f) ? std::ceil(std::log2f(sx)) : std::floor(std::log2f(sx)));
    stepsY = static_cast<int>((sy > 1.f) ? std::ceil(std::log2f(sy)) : std::floor(std::log2f(sy)));
  } else {
    stepsX = sx != 1.f;
    stepsY = sy != 1.f;
  }
  ...
  if (rescaleGamma == SkSurface::RescaleGamma::kLinear &&
    ...
    auto backendFormat =
        this->caps()->getBackendFormatFromGrColorType(GrColorType::kRGBA_F16, GrSRGBEncoded::kNo);
    auto xform = GrColorSpaceXform::Make(
        srcContext->colorSpaceInfo().colorSpace(), kPremul_SkAlphaType, cs.get(),
        kPremul_SkAlphaType);
    ...
    auto linearRTC = fContext->priv().makeDeferredRenderTargetContextWithFallback(
        backendFormat, SkBackingFit::kExact, srcW, srcH, kRGBA_half_GrPixelConfig, std::move(cs), 1,
        GrMipMapped::kNo, kTopLeft_GrSurfaceOrigin);
    if (!linearRTC) {
      return nullptr;
    }
    linearRTC->drawTexture(
        GrNoClip(), srcContext->asTextureProxyRef(), GrSamplerState::Filter::kNearest,
        SkBlendMode::kSrc, SK_PMColor4fWHITE, SkRect::Make(srcRect), SkRect::MakeWH(srcW, srcH),
        GrAA::kNo, GrQuadAAFlags::kNone, constraint, SkMatrix::I(), std::move(xform));
  ...
  while (stepsX || stepsY) {
    ...
    GrBackendFormat backendFormat = srcContext->asSurfaceProxy()->backendFormat().makeTexture2D();
    ...
    if (rescaleQuality == kHigh_SkFilterQuality) {
      ...
      if (srcW != srcContext->width() || srcH != srcContext->height()) {
        ...
        fp = GrBicubicEffect::Make(
            srcContext->asTextureProxyRef(), matrix, domain, dir, kPremul_SkAlphaType);
      } else {
        fp = GrBicubicEffect::Make(
            srcContext->asTextureProxyRef(), matrix, dir, kPremul_SkAlphaType);
      }
      GrPaint paint;
      paint.addColorFragmentProcessor(std::move(fp));
      paint.setPorterDuffXPFactory(SkBlendMode::kSrc);
      nextRTC->drawFilledRect(GrNoClip(), std::move(paint), GrAA::kNo, SkMatrix::I(), dstRect);
    } else {
      ...
      nextRTC->drawTexture(
          GrNoClip(), srcContext->asTextureProxyRef(), filter, SkBlendMode::kSrc, SK_PMColor4fWHITE,
          srcSubset, dstRect, GrAA::kNo, GrQuadAAFlags::kNone, constraint, SkMatrix::I(), nullptr);
    }
...
void GrRenderTargetContext::asyncRescaleAndReadPixels(
  ...
  if (needsRescale) {
    ...
    sk_sp<GrColorSpaceXform> xform = GrColorSpaceXform::Make(
        this->colorSpaceInfo().colorSpace(), kPremul_SkAlphaType, info.colorSpace(),
        info.alphaType());
    ...
    if (this->origin() == kBottomLeft_GrSurfaceOrigin || xform) {
      ...
      rtc->drawTexture(
          GrNoClip(), std::move(texProxy), GrSamplerState::Filter::kNearest, SkBlendMode::kSrc,
...
void GrRenderTargetContext::asyncReadPixels(
    const SkImageInfo& info, int x, int y, ReadPixelsCallback callback, ReadPixelsContext context) {
  ...
  this->getRTOpList()->addOp(
      GrTransferFromOp::Make(
          fContext, SkIRect::MakeXYWH(x, y, info.width(), info.height()), readCT, buffer, 0),
      *this->caps());
  ...
  auto finishCallback = [](GrGpuFinishedContext c) {
    ...
    if (context->fDstInfo != context->fReadInfo) {
      ...
      SkConvertPixels(
          context->fDstInfo, pm.writable_addr(), pm.rowBytes(), context->fReadInfo, data,
          context->fRowBytes);
...
bool GrRenderTargetContext::waitOnSemaphores(
  ...
  for (int i = 0; i < numSemaphores; ++i) {
    ...
    std::unique_ptr<GrOp> waitOp(
        GrSemaphoreOp::MakeWait(fContext, std::move(sema), fRenderTargetProxy.get()));
...
const GrCaps* GrRenderTargetContext::caps() const { return fContext->priv().caps(); }
...
void GrRenderTargetContext::addDrawOp(
  ...
  if (!clip.apply(
          fContext, this, fixedFunctionFlags & GrDrawOp::FixedFunctionFlags::kUsesHWAA,
          fixedFunctionFlags & GrDrawOp::FixedFunctionFlags::kUsesStencil, &appliedClip, &bounds)) {
    fContext->priv().opMemoryPool()->release(std::move(op));
    return;
  }
  ...
  GrProcessorSet::Analysis analysis =
      op->finalize(*this->caps(), &appliedClip, this->fsaaType(), clampType);
...
bool GrRenderTargetContext::setupDstProxy(
    GrRenderTargetProxy* rtProxy, const GrClip& clip, const GrOp& op,
    GrXferProcessor::DstProxy* dstProxy) {
  ...
  if (!this->caps()->initDescForDstCopy(
          rtProxy, &desc, &origin, &rectsMustMatch, &disallowSubrect)) {
