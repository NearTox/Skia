...
class GrRenderTargetContext::TextTarget : public GrTextTarget {
 public:
  ...
  void drawShape(
      const GrClip& clip, const SkPaint& paint, const SkMatrix& viewMatrix,
      const GrShape& shape) override {
    GrBlurUtils::drawShapeWithMaskFilter(
        fRenderTargetContext->fContext, fRenderTargetContext, clip, paint, viewMatrix, shape);
  }

  void makeGrPaint(
      GrMaskFormat maskFormat, const SkPaint& skPaint, const SkMatrix& viewMatrix,
...
std::unique_ptr<GrRenderTargetContext> GrRenderTargetContext::Make(
    GrRecordingContext* context, GrColorType colorType, sk_sp<SkColorSpace> colorSpace,
    SkBackingFit fit, const SkISize& dimensions, const GrBackendFormat& format, int sampleCnt,
...
std::unique_ptr<GrRenderTargetContext> GrRenderTargetContext::Make(
    GrRecordingContext* context, GrColorType colorType, sk_sp<SkColorSpace> colorSpace,
    SkBackingFit fit, const SkISize& dimensions, int sampleCnt, GrMipMapped mipMapped,
...
std::unique_ptr<GrRenderTargetContext> GrRenderTargetContext::MakeWithFallback(
  ...
  do {
    rtc = GrRenderTargetContext::Make(
        context, colorType, colorSpace, fit, dimensions, sampleCnt, mipMapped, isProtected, origin,
...
std::unique_ptr<GrRenderTargetContext> GrRenderTargetContext::MakeFromBackendTexture(
  ...
  sk_sp<GrTextureProxy> proxy(context->priv().proxyProvider()->wrapRenderableBackendTexture(
      tex, origin, sampleCnt, colorType, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, releaseProc,
      releaseCtx));
  if (!proxy) {
    return nullptr;
  }
...
GrRenderTargetContext::GrRenderTargetContext(
    GrRecordingContext* context, GrSurfaceProxyView readView, GrSurfaceProxyView outputView,
...
GrRenderTargetContext::~GrRenderTargetContext() { ASSERT_SINGLE_OWNER }
...
void GrRenderTargetContext::drawGlyphRunList(
  ...
  atlasTextContext->drawGlyphRunList(
      fContext, fTextTarget.get(), clip, viewMatrix, fSurfaceProps, blob);
}
  ...
  if (axisAlignedClip && (!stencilSettings || clipAA == *aa)) {
    ...
    if (clipRRect.isRect()) {
      ...
      if (GrQuadUtils::CropToRect(clipBounds, clipAA, &newFlags, deviceQuad, localQuad)) {
        ...
        *edgeFlags = newFlags;
        if (*aa == GrAA::kNo && clipAA == GrAA::kYes && newFlags != GrQuadAAFlags::kNone) {
...
void GrRenderTargetContext::drawFilledQuad(
    const GrClip& clip, GrPaint&& paint, GrAA aa, GrQuadAAFlags edgeFlags, const GrQuad& deviceQuad,
    const GrQuad& localQuad, const GrUserStencilSettings* ss) {
  ASSERT_SINGLE_OWNER
  RETURN_IF_ABANDONED
  SkDEBUGCODE(this->validate();)
  ...
  QuadOptimization opt = this->attemptQuadOptimization(
      clip, constColor, ss, &aa, &edgeFlags, &croppedDeviceQuad, &croppedLocalQuad);
  if (opt >= QuadOptimization::kClipApplied) {
    ...
    this->addDrawOp(
        finalClip, GrFillRectOp::Make(
                       fContext, std::move(paint), aaType, edgeFlags, croppedDeviceQuad,
                       croppedLocalQuad, ss));
  }
...
void GrRenderTargetContext::drawTexturedQuad(
    ...
    SkBlendMode blendMode, GrAA aa, GrQuadAAFlags edgeFlags, const GrQuad& deviceQuad,
  ...
  GrQuad croppedLocalQuad = localQuad;
  QuadOptimization opt = this->attemptQuadOptimization(
      clip, nullptr, nullptr, &aa, &edgeFlags, &croppedDeviceQuad, &croppedLocalQuad);
  ...
  if (opt != QuadOptimization::kDiscarded) {
    ...
    this->addDrawOp(
        finalClip,
        GrTextureOp::Make(
            fContext, std::move(proxyView), srcAlphaType, std::move(textureXform), filter, color,
            saturate, blendMode, aaType, edgeFlags, croppedDeviceQuad, croppedLocalQuad, domain));
  }
}
...
void GrRenderTargetContext::drawRect(
  ...
  if (stroke.getStyle() == SkStrokeRec::kFill_Style) {
    ...
    if ((!rect.width() || !rect.height()) && SkStrokeRec::kHairline_Style != stroke.getStyle()) {
      ...
      switch (stroke.getJoin()) {
        ...
        case SkPaint::kRound_Join:
          ...
          if (rect.width() || rect.height()) {
            ...
            this->drawRRect(clip, std::move(paint), aa, viewMatrix, rrect, GrStyle::SimpleFill());
            return;
          }
        case SkPaint::kBevel_Join:
          if (!rect.width()) {
            this->drawRect(
                clip, std::move(paint), aa, viewMatrix,
                {rect.fLeft - r, rect.fTop, rect.fRight + r, rect.fBottom}, &GrStyle::SimpleFill());
          } else {
            this->drawRect(
                clip, std::move(paint), aa, viewMatrix,
                {rect.fLeft, rect.fTop - r, rect.fRight, rect.fBottom + r}, &GrStyle::SimpleFill());
          }
          return;
      }
    }
...
void GrRenderTargetContext::asyncRescaleAndReadPixels(
  ...
  if (needsRescale) {
    ...
    if (this->origin() == kBottomLeft_GrSurfaceOrigin || xform) {
      ...
      if (!texProxy) {
        texProxy = GrSurfaceProxy::Copy(
            fContext, this->asSurfaceProxy(), this->colorInfo().colorType(), GrMipMapped::kNo,
      ...
      tempRTC->drawTexture(
          GrNoClip(), std::move(texProxy), this->colorInfo().colorType(),
          this->colorInfo().alphaType(), GrSamplerState::Filter::kNearest, SkBlendMode::kSrc,
          SK_PMColor4fWHITE, srcRectToDraw, SkRect::MakeWH(srcRect.width(), srcRect.height()),
          GrAA::kNo, GrQuadAAFlags::kNone, SkCanvas::kFast_SrcRectConstraint, SkMatrix::I(),
          std::move(xform));
      x = y = 0;
    }
  }
  ...
  if (needsRescale) {
    ...
    if (!srcProxy) {
      srcProxy = GrSurfaceProxy::Copy(
          fContext, fReadView.proxy(), this->colorInfo().colorType(), GrMipMapped::kNo, srcRect,
    ...
    if (xform) {
      ...
      tempRTC->drawTexture(
          GrNoClip(), std::move(srcProxy), this->colorInfo().colorType(),
          this->colorInfo().alphaType(), GrSamplerState::Filter::kNearest, SkBlendMode::kSrc,
          SK_PMColor4fWHITE, srcRectToDraw, SkRect::Make(srcRect.size()), GrAA::kNo,
          GrQuadAAFlags::kNone, SkCanvas::kFast_SrcRectConstraint, SkMatrix::I(), std::move(xform));
      srcProxy = tempRTC->asTextureProxyRef();
      SkASSERT(srcProxy);
      x = y = 0;
    }
...
bool GrRenderTargetContextPriv::drawAndStencilPath(
  ...
  clip.getConservativeBounds(
      fRenderTargetContext->width(), fRenderTargetContext->height(), &clipConservativeBounds,
      nullptr);
...
SkBudgeted GrRenderTargetContextPriv::isBudgeted() const {
  ...
  SkDEBUGCODE(fRenderTargetContext->validate();)
      ...
      return fRenderTargetContext->asSurfaceProxy()
          ->isBudgeted();
}
...
bool GrRenderTargetContext::setupDstProxyView(
  ...
  SkIRect clippedRect;
  clip.getConservativeBounds(this->width(), this->height(), &clippedRect);
  ...
  sk_sp<GrTextureProxy> newProxy = GrSurfaceProxy::Copy(
      fContext, this->asSurfaceProxy(), colorType, GrMipMapped::kNo, copyRect, fit,
...
bool GrRenderTargetContext::blitTexture(
    GrTextureProxy* src, const SkIRect& srcRect, const SkIPoint& dstPoint) {
  SkIRect clippedSrcRect;
  SkIPoint clippedDstPoint;
  if (!GrClipSrcRectAndDstPoint(
          this->asSurfaceProxy()->dimensions(), src->dimensions(), srcRect, dstPoint,
