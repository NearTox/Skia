...
class GrRenderTargetContext::TextTarget : public GrTextTarget {
 public:
  TextTarget(GrRenderTargetContext* renderTargetContext)
      : GrTextTarget(
            renderTargetContext->width(), renderTargetContext->height(),
  ...
  void drawShape(
      const GrClip& clip, const SkPaint& paint, const SkMatrixProvider& matrixProvider,
      const GrStyledShape& shape) override {
    GrBlurUtils::drawShapeWithMaskFilter(
        fRenderTargetContext->fContext, fRenderTargetContext, clip, paint, matrixProvider, shape);
  }

  void makeGrPaint(
      GrMaskFormat maskFormat, const SkPaint& skPaint, const SkMatrixProvider& matrixProvider,
    ...
    if (kARGB_GrMaskFormat == maskFormat) {
      SkPaintToGrPaintWithPrimitiveColor(context, colorInfo, skPaint, matrixProvider, grPaint);
    } else {
...
std::unique_ptr<GrRenderTargetContext> GrRenderTargetContext::Make(
    ...
    sk_sp<GrSurfaceProxy> proxy, GrSurfaceOrigin origin, const SkSurfaceProps* surfaceProps,
    bool managedOps) {
  if (!proxy) {
    return nullptr;
  }
  ...
  return std::make_unique<GrRenderTargetContext>(
      context, std::move(readView), std::move(writeView), colorType, std::move(colorSpace),
      surfaceProps, managedOps);
}

std::unique_ptr<GrRenderTargetContext> GrRenderTargetContext::Make(
    GrRecordingContext* context, GrColorType colorType, sk_sp<SkColorSpace> colorSpace,
    SkBackingFit fit, SkISize dimensions, const GrBackendFormat& format, int sampleCnt,
    GrMipMapped mipMapped, GrProtected isProtected, GrSurfaceOrigin origin, SkBudgeted budgeted,
  ...
  auto rtc = GrRenderTargetContext::Make(
      context, colorType, std::move(colorSpace), std::move(proxy), origin, surfaceProps, true);
  if (!rtc) {
    return nullptr;
  }
  rtc->discard();
...
std::unique_ptr<GrRenderTargetContext> GrRenderTargetContext::Make(
    GrRecordingContext* context, GrColorType colorType, sk_sp<SkColorSpace> colorSpace,
    SkBackingFit fit, SkISize dimensions, int sampleCnt, GrMipMapped mipMapped,
    GrProtected isProtected, GrSurfaceOrigin origin, SkBudgeted budgeted,
    const SkSurfaceProps* surfaceProps) {
  ...
  return GrRenderTargetContext::Make(
      context, colorType, std::move(colorSpace), fit, dimensions, format, sampleCnt, mipMapped,
      isProtected, origin, budgeted, surfaceProps);
}
...
static inline GrColorType color_type_fallback(GrColorType ct) noexcept {
  switch (ct) {
    ...
    case GrColorType::kRGBA_F16_Clamped: return GrColorType::kRGBA_8888;
    case GrColorType::kAlpha_F16: return GrColorType::kRGBA_F16;
    case GrColorType::kGray_8: return GrColorType::kRGB_888x;
...
std::unique_ptr<GrRenderTargetContext> GrRenderTargetContext::MakeWithFallback(
  ...
  return GrRenderTargetContext::Make(
      context, ct, colorSpace, fit, dimensions, sampleCnt, mipMapped, isProtected, origin, budgeted,
      surfaceProps);
}

std::unique_ptr<GrRenderTargetContext> GrRenderTargetContext::MakeFromBackendTexture(
    GrRecordingContext* context, GrColorType colorType, sk_sp<SkColorSpace> colorSpace,
    const GrBackendTexture& tex, int sampleCnt, GrSurfaceOrigin origin,
  ...
  return GrRenderTargetContext::Make(
      context, colorType, std::move(colorSpace), std::move(proxy), origin, surfaceProps);
}
...
std::unique_ptr<GrRenderTargetContext> GrRenderTargetContext::MakeFromVulkanSecondaryCB(
    GrRecordingContext* context, const SkImageInfo& imageInfo, const GrVkDrawableInfo& vkInfo,
    const SkSurfaceProps* props) {
  sk_sp<GrSurfaceProxy> proxy(
      context->priv().proxyProvider()->wrapVulkanSecondaryCBAsRenderTarget(imageInfo, vkInfo));
  if (!proxy) {
    return nullptr;
  }

  return GrRenderTargetContext::Make(
      context, SkColorTypeToGrColorType(imageInfo.colorType()), imageInfo.refColorSpace(),
      std::move(proxy), kTopLeft_GrSurfaceOrigin, props);
}
...
GrRenderTargetContext::GrRenderTargetContext(
    GrRecordingContext* context, GrSurfaceProxyView readView, GrSurfaceProxyView writeView,
    GrColorType colorType, sk_sp<SkColorSpace> colorSpace, const SkSurfaceProps* surfaceProps,
    bool managedOpsTask)
    : GrSurfaceContext(
          context, std::move(readView), colorType, kPremul_SkAlphaType, std::move(colorSpace)),
...
void GrRenderTargetContext::drawGlyphRunList(
    const GrClip& clip, const SkMatrixProvider& matrixProvider, const SkGlyphRunList& blob) {
  ASSERT_SINGLE_OWNER
  RETURN_IF_ABANDONED
  SkDEBUGCODE(this->validate());
  ...
  atlasTextContext->drawGlyphRunList(
      fContext, fTextTarget.get(), clip, matrixProvider, fSurfaceProps, blob);
}
...
void GrRenderTargetContext::clear(
  ...
  AutoCheckFlush acf(this->drawingManager());
  this->internalClear(
      rect ? GrFixedClip(*rect) : GrFixedClip::Disabled(), color, canClearFullscreen);
}

void GrRenderTargetContextPriv::clear(
...
void GrRenderTargetContext::internalClear(
    const GrFixedClip& clip, const SkPMColor4f& color, CanClearFullscreen canClearFullscreen) {
  bool isFull = false;
  ...
  if (isFull) {
    ...
    if (this->caps()->performColorClearsAsDraws()) {
      ...
      this->addDrawOp(
          ...
          GrFillRectOp::MakeNonAARect(fContext, std::move(paint), SkMatrix::I(), rtRect));
    } else {
      this->addOp(GrClearOp::Make(fContext, SkIRect::MakeEmpty(), color, /* fullscreen */ true));
    }
  } else {
    if (this->caps()->performPartialClearsAsDraws()) {
      ...
      this->addDrawOp(
          clip, GrFillRectOp::MakeNonAARect(
                    fContext, std::move(paint), SkMatrix::I(), SkRect::Make(clip.scissorRect())));
    } else {
...
void GrRenderTargetContext::drawPaint(
  ...
  if (!paint.numTotalFragmentProcessors()) {
    ...
    this->fillRectWithLocalMatrix(clip, std::move(paint), GrAA::kNo, SkMatrix::I(), r, localMatrix);
  }
}
...
static bool make_vertex_finite(float* value) noexcept {
...
GrRenderTargetContext::QuadOptimization GrRenderTargetContext::attemptQuadOptimization(
    const GrClip& clip, const SkPMColor4f* constColor, const GrUserStencilSettings* stencilSettings,
    GrAA* aa, DrawQuad* quad) {
  ...
  if (axisAlignedClip && (!stencilSettings || clipAA == *aa)) {
    ...
    if (clipRRect.isRect()) {
      ...
      if (GrQuadUtils::CropToRect(clipBounds, clipAA, quad, /*compute local*/ !constColor)) {
        if (!stencilSettings && constColor &&
          ...
          if (drawBounds.contains(rtRect)) {
            ...
            return QuadOptimization::kSubmitted;
              ...
              GrClip::IsPixelAligned(drawBounds) && drawBounds.width() > 256 &&
        ...
        if (*aa == GrAA::kNo && clipAA == GrAA::kYes && quad->fEdgeFlags != GrQuadAAFlags::kNone) {
      ...
      if (GrQuadUtils::CropToRect(clipBounds, clipAA, quad, /* compute local */ false) &&
        ...
        this->drawRRect(
            GrFixedClip::Disabled(), std::move(paint), clipAA, SkMatrix::I(), clipRRect,
            GrStyle::SimpleFill());
        return QuadOptimization::kSubmitted;
      } else {
...
void GrRenderTargetContext::drawFilledQuad(
    const GrClip& clip, GrPaint&& paint, GrAA aa, DrawQuad* quad, const GrUserStencilSettings* ss) {
  ASSERT_SINGLE_OWNER
  RETURN_IF_ABANDONED
  SkDEBUGCODE(this->validate());
  ...
  if (!ss && !paint.numCoverageFragmentProcessors() && paint.isConstantBlendedColor(&paintColor)) {
  ...
  if (opt >= QuadOptimization::kClipApplied) {
    ...
    const GrClip& finalClip =
        opt == QuadOptimization::kClipApplied ? GrFixedClip::Disabled() : clip;
    GrAAType aaType =
        ss ? (aa == GrAA::kYes ? GrAAType::kMSAA : GrAAType::kNone) : this->chooseAAType(aa);
    this->addDrawOp(finalClip, GrFillRectOp::Make(fContext, std::move(paint), aaType, quad, ss));
  }
  // All other optimization levels were completely handled inside attempt(), so no extra op needed
}

void GrRenderTargetContext::drawTexturedQuad(
    const GrClip& clip, GrSurfaceProxyView proxyView, SkAlphaType srcAlphaType,
    sk_sp<GrColorSpaceXform> textureXform, GrSamplerState::Filter filter, const SkPMColor4f& color,
    SkBlendMode blendMode, GrAA aa, DrawQuad* quad, const SkRect* subset) {
  ASSERT_SINGLE_OWNER
  RETURN_IF_ABANDONED
  ...
  if (opt != QuadOptimization::kDiscarded) {
    ...
    this->addDrawOp(
        finalClip, GrTextureOp::Make(
                       fContext, std::move(proxyView), srcAlphaType, std::move(textureXform),
                       filter, color, saturate, blendMode, aaType, quad, subset));
  }
}

void GrRenderTargetContext::drawRect(
    const GrClip& clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix, const SkRect& rect,
    const GrStyle* style) {
  if (!style) {
  ...
  if (stroke.getStyle() == SkStrokeRec::kFill_Style) {
    ...
    return;
      ...
      (stroke.getStyle() == SkStrokeRec::kStroke_Style ||
  ...
  this->drawShapeUsingPathRenderer(
      clip, std::move(paint), aa, viewMatrix, GrStyledShape(rect, *style));
}

void GrRenderTargetContext::drawQuadSet(
    const GrClip& clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix,
    const QuadSetEntry quads[], int cnt) {
  GrAAType aaType = this->chooseAAType(aa);

  GrFillRectOp::AddFillRectOps(
      this, clip, fContext, std::move(paint), aaType, viewMatrix, quads, cnt);
}
...
void GrRenderTargetContext::setNeedsStencil(bool useMixedSamplesIfNotMSAA) {
  ...
  if (!hasInitializedStencil) {
    if (this->caps()->performStencilClearsAsDraws()) {
      ...
      this->getOpsTask()->setInitialStencilContent(GrOpsTask::StencilContent::kUserBitsCleared);
    }
  }
}
...
void GrRenderTargetContext::internalStencilClear(const GrFixedClip& clip, bool insideStencilMask) {
  ...
  if (this->caps()->performStencilClearsAsDraws()) {
    ...
    this->addDrawOp(
        clip, GrFillRectOp::MakeNonAARect(fContext, std::move(paint), SkMatrix::I(), rtRect, ss));
  } else {
    std::unique_ptr<GrOp> op(
        GrClearStencilClipOp::Make(fContext, clip, insideStencilMask, this->asRenderTargetProxy()));
    if (!op) {
      return;
    }
    this->addOp(std::move(op));
...
void GrRenderTargetContextPriv::stencilPath(
    const GrHardClip& clip, GrAA doStencilMSAA, const SkMatrix& viewMatrix,
  ...
  if (!clip.apply(
          fRenderTargetContext->width(), fRenderTargetContext->height(), &appliedClip, &bounds)) {
    return;
  }

  std::unique_ptr<GrOp> op = GrStencilPathOp::Make(
      fRenderTargetContext->fContext, viewMatrix, GrAA::kYes == doStencilMSAA,
      appliedClip.hasStencilClip(), appliedClip.scissorState(), std::move(path));
  if (!op) {
    return;
  }
...
void GrRenderTargetContext::drawTextureSet(
    const GrClip& clip, TextureSetEntry set[], int cnt, int proxyRunCnt,
    GrSamplerState::Filter filter, SkBlendMode mode, GrAA aa,
  ...
  auto clampType = GrColorTypeClampType(this->colorInfo().colorType());
  auto saturate =
      clampType == GrClampType::kManual ? GrTextureOp::Saturate::kYes : GrTextureOp::Saturate::kNo;
  GrTextureOp::AddTextureSetOps(
      this, clip, fContext, set, cnt, proxyRunCnt, filter, saturate, mode, aaType, constraint,
      viewMatrix, std::move(texXform));
}

void GrRenderTargetContext::drawVertices(
    const GrClip& clip, GrPaint&& paint, const SkMatrixProvider& matrixProvider,
    sk_sp<SkVertices> vertices, GrPrimitiveType* overridePrimType, const SkRuntimeEffect* effect) {
  ASSERT_SINGLE_OWNER
  RETURN_IF_ABANDONED
  SkDEBUGCODE(this->validate());
  ...
  std::unique_ptr<GrDrawOp> op = GrDrawVerticesOp::Make(
      fContext, std::move(paint), std::move(vertices), matrixProvider, aaType,
...
void GrRenderTargetContext::drawAtlas(
    const GrClip& clip, GrPaint&& paint, const SkMatrix& viewMatrix, int spriteCount,
  ...
  GrAAType aaType = this->chooseAAType(GrAA::kNo);
  std::unique_ptr<GrDrawOp> op = GrDrawAtlasOp::Make(
      fContext, std::move(paint), viewMatrix, aaType, spriteCount, xform, texRect, colors);
  this->addDrawOp(clip, std::move(op));
}

///////////////////////////////////////////////////////////////////////////////

void GrRenderTargetContext::drawRRect(
    const GrClip& origClip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix,
  ...
  if (GrAAType::kCoverage == aaType && rrect.isSimple() &&
      rrect.getSimpleRadii().fX == rrect.getSimpleRadii().fY && viewMatrix.rectStaysRect() &&
  ...
  assert_alive(paint);
  this->drawShapeUsingPathRenderer(
      *clip, std::move(paint), aa, viewMatrix, GrStyledShape(rrect, style));
}
...
bool GrRenderTargetContext::drawFastShadow(
    const GrClip& clip, const SkMatrix& viewMatrix, const SkPath& path,
  ...
  bool tiltZPlane = SkToBool(
      !SkScalarNearlyZero(rec.fZPlaneParams.fX) || !SkScalarNearlyZero(rec.fZPlaneParams.fY));
  ...
  if (!isRRect && path.isOval(&rect) && SkScalarNearlyEqual(rect.width(), rect.height()) &&
  ...
  SkScalar devToSrcScale =
      viewMatrix.isScaleTranslate()
          ...
          : sk_float_rsqrt(
                viewMatrix[SkMatrix::kMScaleX] * viewMatrix[SkMatrix::kMScaleX] +
  ...
  if (SkColorGetA(rec.fAmbientColor) > 0) {
    ...
    std::unique_ptr<GrDrawOp> op = GrShadowRRectOp::Make(
        fContext, ambientColor, viewMatrix, ambientRRect, devSpaceAmbientBlur, devSpaceInsetWidth);
    if (op) {
      this->addDrawOp(clip, std::move(op));
    }
  }
  ...
  if (SkColorGetA(rec.fSpotColor) > 0) {
    ...
    SkScalar spotScale;
    SkVector spotOffset;
    SkDrawShadowMetrics::GetSpotParams(
        occluderHeight, devLightPos.fX, devLightPos.fY, devLightPos.fZ, rec.fLightRadius,
    ...
    if (transparent) {
      ...
      if (rrect.isRect()) {
        ...
        maxOffset = std::max(
            std::max(
                SkTAbs(spotShadowRRect.rect().fLeft - rrect.rect().fLeft),
                SkTAbs(spotShadowRRect.rect().fTop - rrect.rect().fTop)),
            std::max(
                SkTAbs(spotShadowRRect.rect().fRight - rrect.rect().fRight),
                SkTAbs(spotShadowRRect.rect().fBottom - rrect.rect().fBottom)));
      } else {
        SkScalar dr = spotRadius - SkRRectPriv::GetSimpleRadii(rrect).fX;
        SkPoint upperLeftOffset = SkPoint::Make(
            spotShadowRRect.rect().fLeft - rrect.rect().fLeft + dr,
            spotShadowRRect.rect().fTop - rrect.rect().fTop + dr);
        SkPoint lowerRightOffset = SkPoint::Make(
            spotShadowRRect.rect().fRight - rrect.rect().fRight - dr,
            spotShadowRRect.rect().fBottom - rrect.rect().fBottom - dr);
        maxOffset = SkScalarSqrt(std::max(
                        SkPointPriv::LengthSqd(upperLeftOffset),
                        SkPointPriv::LengthSqd(lowerRightOffset))) +
                    dr;
      }
    ...
    std::unique_ptr<GrDrawOp> op = GrShadowRRectOp::Make(
        fContext, spotColor, viewMatrix, spotShadowRRect, 2.0f * devSpaceSpotBlur, insetWidth);
    if (op) {
      this->addDrawOp(clip, std::move(op));
    }
  }
...
bool GrRenderTargetContext::drawFilledDRRect(
    const GrClip& clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix,
  ...
  if (GrAAType::kCoverage == aaType && SkRRectPriv::IsCircle(*inner) &&
    ...
    if (SkScalarNearlyEqual(cx, inner->getBounds().fLeft + innerR) &&
      ...
      auto op = GrOvalOpFactory::MakeOvalOp(
          fContext, std::move(paint), viewMatrix, circleBounds, GrStyle(stroke, nullptr),
  ...
  this->fillRectWithLocalMatrix(
      clip, std::move(paint), GrAA::kNo, SkMatrix::I(), bounds, inverseVM);
  return true;
}

void GrRenderTargetContext::drawDRRect(
    const GrClip& clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix, const SkRRect& outer,
    const SkRRect& inner) {
  ASSERT_SINGLE_OWNER
  RETURN_IF_ABANDONED
  SkDEBUGCODE(this->validate());
...
void GrRenderTargetContext::drawRegion(
    const GrClip& clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix,
  ...
  if (GrAA::kYes == aa) {
    ...
    if (!SkToBool(viewMatrix.getType() & ~(SkMatrix::kTranslate_Mask)) &&
        SkScalarIsInt(viewMatrix.getTranslateX()) && SkScalarIsInt(viewMatrix.getTranslateY())) {
      aa = GrAA::kNo;
    }
  }
  ...
  GrAAType aaType = this->chooseAAType(GrAA::kNo);
  std::unique_ptr<GrDrawOp> op =
      GrRegionOp::Make(fContext, std::move(paint), viewMatrix, region, aaType, ss);
  this->addDrawOp(clip, std::move(op));
}

void GrRenderTargetContext::drawOval(
    const GrClip& clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix, const SkRect& oval,
    const GrStyle& style) {
  ASSERT_SINGLE_OWNER
  RETURN_IF_ABANDONED
  SkDEBUGCODE(this->validate());
  ...
  if (GrAAType::kCoverage == aaType && oval.width() > SK_ScalarNearlyZero &&
    ...
    assert_alive(paint);
    op = GrOvalOpFactory::MakeCircleOp(
        fContext, std::move(paint), viewMatrix, oval, style, this->caps()->shaderCaps());
  }
  if (!op && style.isSimpleFill()) {
...
void GrRenderTargetContext::drawArc(
    const GrClip& clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix, const SkRect& oval,
  ...
  if (GrAAType::kCoverage == aaType) {
    ...
    std::unique_ptr<GrDrawOp> op = GrOvalOpFactory::MakeArcOp(
        fContext, std::move(paint), viewMatrix, oval, startAngle, sweepAngle, useCenter, style,
        shaderCaps);
    if (op) {
      this->addDrawOp(clip, std::move(op));
      return;
...
void GrRenderTargetContext::drawImageLattice(
    const GrClip& clip, GrPaint&& paint, const SkMatrix& viewMatrix, GrSurfaceProxyView view,
    SkAlphaType alphaType, sk_sp<GrColorSpaceXform> csxf, GrSamplerState::Filter filter,
  ...
  std::unique_ptr<GrDrawOp> op = GrLatticeOp::MakeNonAA(
      fContext, std::move(paint), viewMatrix, std::move(view), alphaType, std::move(csxf), filter,
      std::move(iter), dst);
  this->addDrawOp(clip, std::move(op));
}

void GrRenderTargetContext::drawDrawable(
...
void GrRenderTargetContext::asyncRescaleAndReadPixels(
  ...
  if (needsRescale) {
    ...
    backendFormatOfFinalContext = this->caps()->getDefaultBackendFormat(dstCT, GrRenderable::kYes);
  }
  auto readInfo = this->caps()->supportedReadPixelsColorType(
      colorTypeOfFinalContext, backendFormatOfFinalContext, dstCT);
  ...
  if (needsRescale) {
    ...
    sk_sp<GrColorSpaceXform> xform = GrColorSpaceXform::Make(
        this->colorInfo().colorSpace(), this->colorInfo().alphaType(), info.colorSpace(),
        info.alphaType());
    ...
    if (this->origin() == kBottomLeft_GrSurfaceOrigin || xform) {
      ...
      if (!texProxyView.asTextureProxy()) {
        texProxyView = GrSurfaceProxyView::Copy(
            fContext, texProxyView, GrMipMapped::kNo, srcRect, SkBackingFit::kApprox,
            SkBudgeted::kNo);
        if (!texProxyView) {
          callback(context, nullptr);
          return;
      ...
      tempRTC = GrRenderTargetContext::Make(
          direct, this->colorInfo().colorType(), info.refColorSpace(), SkBackingFit::kApprox,
          srcRect.size(), 1, GrMipMapped::kNo, GrProtected::kNo, kTopLeft_GrSurfaceOrigin);
      if (!tempRTC) {
        callback(context, nullptr);
        return;
      }
      tempRTC->drawTexture(
          GrNoClip(), std::move(texProxyView), this->colorInfo().alphaType(),
          GrSamplerState::Filter::kNearest, SkBlendMode::kSrc, SK_PMColor4fWHITE, srcRectToDraw,
          SkRect::MakeWH(srcRect.width(), srcRect.height()), GrAA::kNo, GrQuadAAFlags::kNone,
          SkCanvas::kFast_SrcRectConstraint, SkMatrix::I(), std::move(xform));
      x = y = 0;
    }
  }
  auto rtc = tempRTC ? tempRTC.get() : this;
  return rtc->asyncReadPixels(
      SkIRect::MakeXYWH(x, y, info.width(), info.height()), info.colorType(), callback, context);
}

class GrRenderTargetContext::AsyncReadResult : public SkSurface::AsyncReadResult {
 public:
  AsyncReadResult(uint32_t inboxID) noexcept : fInboxID(inboxID) {}
  ...
  int count() const noexcept override { return fPlanes.count(); }
  const void* data(int i) const noexcept override { return fPlanes[i].fData; }
  size_t rowBytes(int i) const noexcept override { return fPlanes[i].fRowBytes; }

  bool addTransferResult(
      const PixelTransferResult& result, SkISize dimensions, size_t rowBytes,
 ...
 private:
  ...
  struct Plane {
    Plane(const void* data, size_t rowBytes, sk_sp<GrGpuBuffer> buffer) noexcept
...
void GrRenderTargetContext::asyncReadPixels(
    const SkIRect& rect, SkColorType colorType, ReadPixelsCallback callback,
  ...
  if (!transferResult.fTransferBuffer) {
    auto ii = SkImageInfo::Make(
        rect.size(), colorType, this->colorInfo().alphaType(), this->colorInfo().refColorSpace());
  ...
  auto* finishContext = new FinishContext{
      callback, context, rect.size(), colorType, mappedBufferManager, std::move(transferResult)};
  auto finishCallback = [](GrGpuFinishedContext c) {
    ...
    if (!result->addTransferResult(
            context->fTransferResult, context->fSize, rowBytes, context->fMappedBufferManager)) {
      result.reset();
    }
...
void GrRenderTargetContext::asyncRescaleAndReadPixelsYUV420(
    ...
    SkISize dstSize, RescaleGamma rescaleGamma, SkFilterQuality rescaleQuality,
  ...
  if (needsRescale) {
    ...
    auto info =
        SkImageInfo::Make(dstSize, kRGBA_8888_SkColorType, kPremul_SkAlphaType, dstColorSpace);
    ...
    if (!srcView.asTextureProxy()) {
      srcView = GrSurfaceProxyView::Copy(
          fContext, std::move(srcView), GrMipMapped::kNo, srcRect, SkBackingFit::kApprox,
          SkBudgeted::kYes);
      if (!srcView) {
    ...
    if (xform) {
      ...
      auto tempRTC = GrRenderTargetContext::Make(
          direct, this->colorInfo().colorType(), dstColorSpace, SkBackingFit::kApprox, dstSize, 1,
      ...
      tempRTC->drawTexture(
          GrNoClip(), std::move(srcView), this->colorInfo().alphaType(),
          GrSamplerState::Filter::kNearest, SkBlendMode::kSrc, SK_PMColor4fWHITE, srcRectToDraw,
  ...
  yRTC->fillRectToRect(GrNoClip(), std::move(yPaint), GrAA::kNo, SkMatrix::I(), dstRectY, dstRectY);
  if (!doSynchronousRead) {
    yTransfer =
        yRTC->transferPixels(GrColorType::kAlpha_8, SkIRect::MakeWH(yRTC->width(), yRTC->height()));
  ...
  uRTC->fillRectToRect(
      GrNoClip(), std::move(uPaint), GrAA::kNo, SkMatrix::I(), dstRectUV, dstRectUV);
  if (!doSynchronousRead) {
    uTransfer =
        uRTC->transferPixels(GrColorType::kAlpha_8, SkIRect::MakeWH(uRTC->width(), uRTC->height()));
  ...
  vPaint.addColorFragmentProcessor(GrTextureEffect::Make(
      std::move(srcView), this->colorInfo().alphaType(), texMatrix,
  ...
  vRTC->fillRectToRect(
      GrNoClip(), std::move(vPaint), GrAA::kNo, SkMatrix::I(), dstRectUV, dstRectUV);
  if (!doSynchronousRead) {
    vTransfer =
        vRTC->transferPixels(GrColorType::kAlpha_8, SkIRect::MakeWH(vRTC->width(), vRTC->height()));
...
bool GrRenderTargetContext::waitOnSemaphores(
  ...
  this->drawingManager()->newWaitRenderTask(
      this->asSurfaceProxyRef(), std::move(grSemaphores), numSemaphores);
  return true;
}

void GrRenderTargetContext::drawPath(
    const GrClip& clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix, const SkPath& path,
    const GrStyle& style) {
  ASSERT_SINGLE_OWNER
  RETURN_IF_ABANDONED
  SkDEBUGCODE(this->validate());
...
void GrRenderTargetContext::drawShape(
    const GrClip& clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix,
  ...
  if (!shape.style().hasPathEffect()) {
    ...
    if (shape.asRRect(&rrect, nullptr, nullptr, &inverted) && !inverted) {
      if (rrect.isRect()) {
        this->drawRect(clip, std::move(paint), aa, viewMatrix, rrect.rect(), &shape.style());
        return;
      ...
      return;
        ...
        GrAAType::kCoverage == aaType && shape.style().isSimpleFill() &&
      ...
      if (shape.asNestedRects(rects)) {
        ...
        std::unique_ptr<GrDrawOp> op =
            GrStrokeRectOp::MakeNested(fContext, std::move(paint), viewMatrix, rects);
        if (op) {
          this->addDrawOp(clip, std::move(op));
        }
  ...
  this->drawShapeUsingPathRenderer(
      clip, std::move(paint), aa, viewMatrix, shape,
      /* attempt fallback */ false);
}

bool GrRenderTargetContextPriv::drawAndStencilPath(
    const GrHardClip& clip, const GrUserStencilSettings* ss, SkRegion::Op op, bool invert, GrAA aa,
  ...
  if (path.isEmpty() && path.isInverseFillType()) {
    ...
    this->stencilRect(
        clip, ss, std::move(paint), GrAA::kNo, SkMatrix::I(),
        SkRect::MakeIWH(fRenderTargetContext->width(), fRenderTargetContext->height()));
    return true;
  }
...
void GrRenderTargetContext::drawShapeUsingPathRenderer(
    const GrClip& clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix,
    const GrStyledShape& originalShape, bool attemptShapeFallback) {
  ASSERT_SINGLE_OWNER
  RETURN_IF_ABANDONED
  ...
  if (!pr) {
    if (canDrawArgs.fShape->style().applies()) {
      tempShape =
          canDrawArgs.fShape->applyStyle(GrStyle::Apply::kPathEffectAndStrokeRec, styleScale);
      if (tempShape.isEmpty()) {
        return;
      }
      canDrawArgs.fShape = &tempShape;
  ...
  GrPathRenderer::DrawPathArgs args{
      this->drawingManager()->getContext(),
      std::move(paint),
...
void GrRenderTargetContext::addDrawOp(
    const GrClip& clip, std::unique_ptr<GrDrawOp> op, const std::function<WillAddOpFn>& willAddFn) {
  ASSERT_SINGLE_OWNER
  if (fContext->priv().abandoned()) {
  ...
  SkASSERT(
      !hasMixedSampledCoverage || this->asRenderTargetProxy()->canUseMixedSamples(*this->caps()));

  GrClampType clampType = GrColorTypeClampType(this->colorInfo().colorType());
  GrProcessorSet::Analysis analysis =
      op->finalize(*this->caps(), &appliedClip, hasMixedSampledCoverage, clampType);
  ...
  opsTask->addDrawOp(
      std::move(op), analysis, std::move(appliedClip), dstProxyView,
      GrTextureResolveManager(this->drawingManager()), *this->caps());
}

bool GrRenderTargetContext::setupDstProxyView(
    const GrClip& clip, const GrOp& op, GrXferProcessor::DstProxyView* dstProxyView) {
  ...
  GrCaps::DstCopyRestrictions restrictions =
      this->caps()->getDstCopyRestrictions(this->asRenderTargetProxy(), colorType);
  ...
  auto copy = GrSurfaceProxy::Copy(
      fContext, this->asSurfaceProxy(), this->origin(), GrMipMapped::kNo, copyRect, fit,
...
bool GrRenderTargetContext::blitTexture(
    GrSurfaceProxyView view, const SkIRect& srcRect, const SkIPoint& dstPoint) {
  SkASSERT(view.asTextureProxy());
  SkIRect clippedSrcRect;
  SkIPoint clippedDstPoint;
  if (!GrClipSrcRectAndDstPoint(
          this->asSurfaceProxy()->dimensions(), view.proxy()->dimensions(), srcRect, dstPoint,
  ...
  this->fillRectToRect(
      ...
      SkRect::MakeXYWH(
          clippedDstPoint.fX, clippedDstPoint.fY, clippedSrcRect.width(), clippedSrcRect.height()),
