  ...
  if (opt != QuadOptimization::kDiscarded) {
    ...
    this->addDrawOp(
        finalClip, GrTextureOp::Make(
                       ...
                       filter, color, saturate, blendMode, aaType, quad, domain));
  }
}
  ...
  if (needsRescale) {
    ...
    if (this->origin() == kBottomLeft_GrSurfaceOrigin || xform) {
      ...
      if (!texProxyView.asTextureProxy()) {
        texProxyView = GrSurfaceProxy::Copy(
            fContext, this->asSurfaceProxy(), this->origin(), this->colorInfo().colorType(),
...
void GrRenderTargetContext::asyncRescaleAndReadPixelsYUV420(
  ...
  if (needsRescale) {
    ...
    if (!srcView.asTextureProxy()) {
      srcView = GrSurfaceProxy::Copy(
          fContext, fReadView.proxy(), this->origin(), this->colorInfo().colorType(),
          GrMipMapped::kNo, srcRect, SkBackingFit::kApprox, SkBudgeted::kYes);
      if (!srcView.asTextureProxy()) {
...
bool GrRenderTargetContext::setupDstProxyView(
  ...
  GrSurfaceProxyView newProxyView = GrSurfaceProxy::Copy(
      fContext, this->asSurfaceProxy(), this->origin(), colorType, GrMipMapped::kNo, copyRect, fit,
