...
class GrCaps : public SkRefCnt {
 public:
  ...
  virtual int getRenderTargetSampleCount(
      int requestedCount, GrColorType, const GrBackendFormat&) const = 0;
   ...
   */
  virtual GrColorType supportedWritePixelsColorType(
      GrPixelConfig config, GrColorType srcColorType) const {
   ...
   */
  virtual SupportedRead supportedReadPixelsColorType(
  ...
  bool validateSurfaceDesc(
      const GrSurfaceDesc&, GrRenderable renderable, int renderTargetSampleCnt, GrMipMapped) const;
   ...
   * returned.
   */
  virtual GrPixelConfig validateBackendRenderTarget(
      const GrBackendRenderTarget&, GrColorType) const = 0;
