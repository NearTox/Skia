...
class GrCaps : public SkRefCnt {
 public:
   ...
   */
  virtual GrColorType supportedReadPixelsColorType(
   ...
   */
  virtual bool initDescForDstCopy(
  ...
  virtual GrPixelConfig validateBackendRenderTarget(
      const GrBackendRenderTarget&, SkColorType) const = 0;
  ...
  virtual GrPixelConfig getConfigFromBackendFormat(
      const GrBackendFormat& format, SkColorType ct) const = 0;
  ...
  virtual GrBackendFormat getBackendFormatFromGrColorType(
      GrColorType ct, GrSRGBEncoded srgbEncoded) const = 0;
