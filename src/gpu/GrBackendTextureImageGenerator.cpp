...
GrBackendTextureImageGenerator::RefHelper::RefHelper(
    GrTexture* texture, GrDirectContext::DirectContextID owningContextID,
...
std::unique_ptr<SkImageGenerator> GrBackendTextureImageGenerator::Make(
    sk_sp<GrTexture> texture, GrSurfaceOrigin origin, std::unique_ptr<GrSemaphore> semaphore,
  ...
  SkImageInfo info = SkImageInfo::Make(
      texture->width(), texture->height(), colorType, alphaType, std::move(colorSpace));
  return std::unique_ptr<SkImageGenerator>(new GrBackendTextureImageGenerator(
      info, texture.get(), origin, dContext->directContextID(), std::move(semaphore),
      backendTexture));
}

GrBackendTextureImageGenerator::GrBackendTextureImageGenerator(
    const SkImageInfo& info, GrTexture* texture, GrSurfaceOrigin origin,
    GrDirectContext::DirectContextID owningContextID, std::unique_ptr<GrSemaphore> semaphore,
...
GrBackendTextureImageGenerator::~GrBackendTextureImageGenerator() { fRefHelper->unref(); }
...
GrSurfaceProxyView GrBackendTextureImageGenerator::onGenerateTexture(
  ...
  if (fRefHelper->fBorrowingContextID.isValid()) {
    ...
    // GrRefCntedCallback.
    fRefHelper->ref();
    releaseProcHelper = GrRefCntedCallback::Make(ReleaseRefHelper_TextureReleaseProc, fRefHelper);
  ...
  GrMipmapped textureIsMipMapped =
      fBackendTexture.hasMipmaps() ? GrMipmapped::kYes : GrMipmapped::kNo;

  // Ganesh assumes that, when wrapping a mipmapped backend texture from a client, that its
  // mipmaps are fully fleshed out.
  GrMipmapStatus mipmapStatus =
      fBackendTexture.hasMipmaps() ? GrMipmapStatus::kValid : GrMipmapStatus::kNotAllocated;
  ...
  sk_sp<GrTextureProxy> proxy = proxyProvider->createLazyProxy(
      [refHelper = fRefHelper, releaseProcHelper, backendTexture = fBackendTexture](
        ...
        auto surf = resourceProvider->findByUniqueKey<GrSurface>(refHelper->fBorrowedTextureKey);
        if (surf) {
  ...
  if (texGenPolicy == GrImageTexGenPolicy::kDraw && origin.isZero() &&
    ...
    auto copy = GrSurfaceProxy::Copy(
        dContext, std::move(proxy), fSurfaceOrigin, mipMapped, subset, SkBackingFit::kExact,
        budgeted);
