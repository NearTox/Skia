...
static void draw_stencil_rect(
    GrRenderTargetContext* rtc, const GrHardClip& clip, const GrUserStencilSettings* ss,
...
static void draw_path(
    GrRecordingContext* context, GrRenderTargetContext* rtc, GrPathRenderer* pr,
  ...
  GrPathRenderer::DrawPathArgs args{context, std::move(paint), ss,     rtc,        &clip,
                                    &bounds, &matrix,          &shape, pathAAType, false};
  pr->drawPath(args);
}

static void stencil_path(
    GrRecordingContext* context, GrRenderTargetContext* rtc, GrPathRenderer* pr,
...
void GrStencilMaskHelper::drawRect(
  ...
  bool drawDirectToClip;
  auto passes = get_stencil_passes(
      op, GrPathRenderer::kNoRestriction_StencilSupport, false, &drawDirectToClip);
  ...
  for (GrUserStencilSettings const* const* pass = passes; *pass; ++pass) {
    if (drawDirectToClip) {
      ...
      draw_stencil_rect(
          fRTC, fClip, *pass, SkMatrix::I(), SkRect::Make(fClip.fixedClip().scissorRect()), aa);
    }
  }
}

bool GrStencilMaskHelper::drawPath(
  ...
  if (!drawDirectToClip) {
    if (stencilSupport == GrPathRenderer::kNoRestriction_StencilSupport) {
      draw_path(
          fContext, fRTC, pr, fClip.fixedClip(), fClip.fixedClip().scissorRect(), &gDrawToStencil,
          matrix, shape, aa);
    } else {
  ...
  for (GrUserStencilSettings const* const* pass = passes; *pass; ++pass) {
    if (drawDirectToClip) {
      draw_path(
          fContext, fRTC, pr, fClip, fClip.fixedClip().scissorRect(), *pass, matrix, shape, aa);
    } else {
      draw_stencil_rect(
          fRTC, fClip, *pass, SkMatrix::I(), SkRect::Make(fClip.fixedClip().scissorRect()), aa);
    }
...
bool GrStencilMaskHelper::drawShape(
