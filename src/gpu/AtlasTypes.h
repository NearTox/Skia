...
struct IRect16 {
  ...
  static IRect16 SK_WARN_UNUSED_RESULT MakeEmpty() noexcept {
    IRect16 r;
    r.setEmpty();
    return r;
  }

  static IRect16 SK_WARN_UNUSED_RESULT MakeWH(int16_t w, int16_t h) noexcept {
    IRect16 r;
    r.set(0, 0, w, h);
    return r;
  }

  static IRect16 SK_WARN_UNUSED_RESULT
  ...
  static IRect16 SK_WARN_UNUSED_RESULT Make(const SkIRect& ir) noexcept {
    IRect16 r;
    r.set(ir);
    return r;
  }
...
static constexpr int kMaskFormatCount = static_cast<int>(MaskFormat::kLast) + 1;

/**
 *  Return the number of bytes-per-pixel for the specified mask format.
 */
inline constexpr int MaskFormatBytesPerPixel(MaskFormat format) noexcept {
...
static constexpr SkColorType MaskFormatToColorType(MaskFormat format) noexcept {
  switch (format) {
...
class DrawToken {
 public:
  static DrawToken AlreadyFlushedToken() noexcept { return DrawToken(0); }

  DrawToken(const DrawToken&) noexcept = default;
  DrawToken& operator=(const DrawToken&) noexcept = default;

  bool operator==(const DrawToken& that) const noexcept {
    return fSequenceNumber == that.fSequenceNumber;
  }
  bool operator!=(const DrawToken& that) const noexcept { return !(*this == that); }
  bool operator<(const DrawToken that) const noexcept {
    return fSequenceNumber < that.fSequenceNumber;
  }
  bool operator<=(const DrawToken that) const noexcept {
  ...
  DrawToken& operator++() noexcept {
    ++fSequenceNumber;
    return *this;
  }
  DrawToken operator++(int) noexcept {
    auto old = fSequenceNumber;
    ++fSequenceNumber;
    return DrawToken(old);
  }

  DrawToken next() const noexcept { return DrawToken(fSequenceNumber + 1); }

  /** Is this token in the [start, end] inclusive interval? */
  bool inInterval(const DrawToken& start, const DrawToken& end) noexcept {
    return *this >= start && *this <= end;
  }

 private:
  DrawToken() = delete;
  constexpr explicit DrawToken(uint64_t sequenceNumber) noexcept
...
class PlotLocator {
 public:
  ...
  PlotLocator()
      : fGenID(AtlasGenerationCounter::kInvalidGeneration), fPlotIndex(0), fPageIndex(0) {}
...
class AtlasLocator {
 public:
  std::array<uint16_t, 4> getUVs() const noexcept { return fUVs; }
  ...
  SkIPoint topLeft() const noexcept { return {fUVs[0] & 0x1FFF, fUVs[1]}; }

  uint16_t width() const noexcept { return fUVs[2] - fUVs[0]; }

  uint16_t height() const noexcept { return fUVs[3] - fUVs[1]; }
  ...
  void updatePlotLocator(PlotLocator p) noexcept {
    fPlotLocator = p;
  ...
  void updateRect(skgpu::IRect16 rect) noexcept {
...
class BulkUsePlotUpdater {
 public:
  ...
  int count() const noexcept { return fPlotsToUpdate.count(); }

  const PlotData& plotData(int index) const noexcept { return fPlotsToUpdate[index]; }
...
class Plot : public SkRefCnt {
 ...
 public:
  Plot(
      int pageIndex, int plotIndex, AtlasGenerationCounter* generationCounter, int offX, int offY,
      int width, int height, SkColorType colorType, size_t bpp);
   ...
   */
  skgpu::DrawToken lastUploadToken() const noexcept { return fLastUpload; }
  skgpu::DrawToken lastUseToken() const noexcept { return fLastUse; }
  void setLastUploadToken(skgpu::DrawToken token) noexcept { fLastUpload = token; }
  void setLastUseToken(skgpu::DrawToken token) noexcept { fLastUse = token; }
