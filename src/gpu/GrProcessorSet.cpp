...
bool GrProcessorSet::operator==(const GrProcessorSet& that) const {
  ...
  const GrXferProcessor& thisXP =
      this->xferProcessor() ? *this->xferProcessor() : GrPorterDuffXPFactory::SimpleSrcOverXP();
  const GrXferProcessor& thatXP =
      that.xferProcessor() ? *that.xferProcessor() : GrPorterDuffXPFactory::SimpleSrcOverXP();
...
GrProcessorSet::Analysis GrProcessorSet::finalize(
    ...
    const GrAppliedClip* clip, const GrUserStencilSettings* userStencil, const GrCaps& caps,
    GrClampType clampType, SkPMColor4f* overrideInputColor) {
  SkASSERT(!this->isFinalized());
  ...
  GrColorFragmentProcessorAnalysis colorAnalysis(
      colorInput, &fColorFragmentProcessor, this->hasColorFragmentProcessor() ? 1 : 0);
...
#ifdef SK_DEBUG
  bool hasXferBarrier = fXP.fProcessor && GrXferBarrierType::kNone_GrXferBarrierType !=
                                              fXP.fProcessor->xferBarrierType(caps);
