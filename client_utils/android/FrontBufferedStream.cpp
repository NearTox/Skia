...
std::unique_ptr<SkStreamRewindable> FrontBufferedStream::Make(
    std::unique_ptr<SkStream> stream, size_t bufferSize) {
  if (!stream) {
    return nullptr;
  }
  auto frontBufferedStream = std::make_unique<::FrontBufferedStream>(std::move(stream), bufferSize);
...
FrontBufferedStream::FrontBufferedStream(std::unique_ptr<SkStream> stream, size_t bufferSize)
    : fStream(std::move(stream)),
      ...
      fBuffer(bufferSize <= kStorageSize ? fStorage : reinterpret_cast<char*>(malloc(bufferSize))) {
}
