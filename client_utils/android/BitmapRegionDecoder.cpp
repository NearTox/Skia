...
std::unique_ptr<BitmapRegionDecoder> BitmapRegionDecoder::Make(sk_sp<SkData> data) {
  ...
  switch (codec->getEncodedFormat()) {
    ...
    case SkEncodedImageFormat::kHEIF: break;
...
int BitmapRegionDecoder::width() const { return fCodec->getInfo().width(); }

int BitmapRegionDecoder::height() const { return fCodec->getInfo().height(); }

bool BitmapRegionDecoder::decodeRegion(
    SkBitmap* bitmap, BRDAllocator* allocator, const SkIRect& desiredSubset, int sampleSize,
    SkColorType dstColorType, bool requireUnpremul, sk_sp<SkColorSpace> dstColorSpace) {
  ...
  SkImageInfo decodeInfo = SkImageInfo::Make(scaledSize, dstColorType, dstAlphaType, dstColorSpace);
  ...
  SkCodec::Result result = fCodec->getAndroidPixels(decodeInfo, dst, bitmap->rowBytes(), &options);
  switch (result) {
