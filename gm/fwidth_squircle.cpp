...
class FwidthSquircleTestProcessor : public GrGeometryProcessor {
 public:
  static GrGeometryProcessor* Make(SkArenaAlloc* arena, const SkMatrix& viewMatrix) {
    return arena->make(
        [&](void* ptr) { return new (ptr) FwidthSquircleTestProcessor(viewMatrix); });
  }
 ...
 private:
  FwidthSquircleTestProcessor(const SkMatrix& viewMatrix)
      : GrGeometryProcessor(kFwidthSquircleTestProcessor_ClassID), fViewMatrix(viewMatrix) {
  ...
  class Impl : public ProgramImpl {
   ...
   private:
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
      ...
      fViewMatrixHandle =
          uniforms->addUniform(nullptr, kVertex_GrShaderFlag, SkSLType::kFloat3x3, "viewmatrix");
      ...
      f->codeAppendf(
          "float x = abs(%s.x), y = abs(%s.y);", squircleCoord.fsIn(), squircleCoord.fsIn());
...
class FwidthSquircleTestOp : public GrDrawOp {
 ...
 private:
  FwidthSquircleTestOp(const SkMatrix& viewMatrix) : GrDrawOp(ClassID()), fViewMatrix(viewMatrix) {
