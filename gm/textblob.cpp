 ...
 protected:
  ...
  void onDraw(SkCanvas* canvas) override {
    for (unsigned b = 0; b < SK_ARRAY_COUNT(blobConfigs); ++b) {
      ...
      p.setAntiAlias(true);
      SkPoint offset =
          SkPoint::Make(SkIntToScalar(10 + 300 * (b % 2)), SkIntToScalar(20 + 150 * (b / 2)));
  ...
  sk_sp<SkTextBlob> makeBlob(unsigned blobIndex) {
    ...
    for (unsigned l = 0; l < SK_ARRAY_COUNT(blobConfigs[blobIndex]); ++l) {
      ...
      for (unsigned c = 0; c < SK_ARRAY_COUNT(blobConfigs[blobIndex][l]); ++c) {
        ...
        SkPoint offset = SkPoint::Make(currentGlyph * advanceX + c * advanceX, advanceY * l);
        switch (cfg->pos) {
          case kDefault_Pos: {
          ...
          case kScalar_Pos: {
            const SkTextBlobBuilder::RunBuffer& buf = builder.allocRunPosH(font, count, offset.y());
          ...
          default: SK_ABORT("unhandled pos value");
        }
