...
class TexelSubset : public GpuGM {
 ...
 protected:
  SkString onShortName() override {
    ...
    switch (fFilter) {
      case Filter::kNearest: name.append("_nearest"); break;
      case Filter::kLinear: name.append("_linear"); break;
    }
    switch (fMipmapMode) {
      case MipmapMode::kNone: break;
      case MipmapMode::kNearest: name.append("_mipmap_nearest"); break;
      case MipmapMode::kLinear: name.append("_mipmap_linear"); break;
    }
  ...
  DrawResult onDraw(GrRecordingContext* rContext, SkCanvas* canvas, SkString* errorMsg) override {
    ...
    // will draw.
    if (fUpscale) {
      texelSubset = SkIRect::MakeXYWH(
          fBitmap.width() / 3 - 1, 2 * fBitmap.height() / 5 - 1, fBitmap.width() / 4 + 2,
          fBitmap.height() / 5 + 2);
    } else {
      texelSubset = SkIRect::MakeXYWH(
          fBitmap.width() / 8 - 1, fBitmap.height() / 7 - 1, 3 * fBitmap.width() / 5 + 2,
          4 * fBitmap.height() / 5 + 2);
    }
    ...
    for (int tm = 0; tm < textureMatrices.count(); ++tm) {
      for (int my = 0; my < GrSamplerState::kWrapModeCount; ++my) {
        ...
        for (int mx = 0; mx < GrSamplerState::kWrapModeCount; ++mx) {
          ...
          fp1 = GrTextureEffect::MakeSubset(
              view, fBitmap.alphaType(), textureMatrices[tm], sampler, SkRect::Make(texelSubset),
              caps);
          if (!fp1) {
            continue;
          }
          ...
          if (auto op = sk_gpu_test::test_ops::MakeRect(
                  rContext, std::move(fp1), drawRect, localRect.makeOffset(kT),
          ...
          if (auto op =
                  sk_gpu_test::test_ops::MakeRect(rContext, std::move(fp2), drawRect, localRect)) {
            sdc->addDrawOp(std::move(op));
          }
