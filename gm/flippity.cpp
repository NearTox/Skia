...
static const SkMatrix kUVMatrices[kNumMatrices] = {
    SkMatrix::MakeAll(0, -1, 1, -1, 0, 1, 0, 0, 1), SkMatrix::MakeAll(1, 0, 0, 0, -1, 1, 0, 0, 1),
    // flip x
    SkMatrix::MakeAll(-1, 0, 1, 0, 1, 0, 0, 0, 1), SkMatrix::MakeAll(0, 1, 0, -1, 0, 1, 0, 0, 1),
...
static sk_sp<SkImage> make_text_image(GrContext* context, const char* text, SkColor color) {
  ...
  const SkMatrix mat = SkMatrix::MakeRectToRect(
      bounds, SkRect::MakeWH(kLabelSize, kLabelSize), SkMatrix::kFill_ScaleToFit);
...
static sk_sp<SkImage> make_reference_image(
  ...
  for (int i = 0; i < kNumLabels; ++i) {
    canvas.drawImage(
        labels[i], 0.0 != kPoints[i].fX ? kPoints[i].fX - kLabelSize - kInset : kInset,
  ...
  auto proxy = sk_gpu_test::MakeTextureProxyFromData(
      context, GrRenderable::kNo, kImageSize, kImageSize, bm.colorType(), origin, bm.getPixels(),
      bm.rowBytes());
  if (!proxy) {
    return nullptr;
  }
...
class FlippityGM : public skiagm::GpuGM {
 ...
 protected:
  ...
  void drawImageWithMatrixAndLabels(
      SkCanvas* canvas, SkImage* image, int matIndex, bool drawSubset, bool drawScaled) {
    ...
    if (drawSubset) {
      canvas->drawImageRect(
          ...
          drawScaled ? SkRect::MakeWH(kImageSize, kImageSize) : kSubsets[matIndex], nullptr,
    ...
    for (int i = 0; i < kNumLabels; ++i) {
      canvas->drawImage(
          fLabels[i], 0.0f == kPoints[i].fX ? -kLabelSize : kPoints[i].fX,
  ...
  void drawRow(
      GrContext* context, SkCanvas* canvas, bool bottomLeftImage, bool drawSubset,
      bool drawScaled) {
    ...
    for (int i = 0; i < kNumMatrices; ++i) {
      this->drawImageWithMatrixAndLabels(canvas, referenceImage.get(), i, drawSubset, drawScaled);
