...
class PixelSnapGM : public skiagm::GM {
 ...
 protected:
  ...
  SkISize onISize() override {
    return SkISize::Make(
        (kSubPixelSteps + 1) * kTrans + kLabelOffsetX + kLabelPad,
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    for (int i = 0; i <= kSubPixelSteps; ++i) {
      offset.printf("%d", i);
      canvas->drawString(
          offset, i * SkIntToScalar(kTrans), SkIntToScalar(kLabelTextSize), font, labelPaint);
    }
    canvas->restore();
    ...
    for (int i = 0; i <= kSubPixelSteps + 1; ++i) {
      canvas->drawLine(
          0.5f, i * SkIntToScalar(kTrans) + 0.5f,
          SkIntToScalar(kTrans) * (kSubPixelSteps + 1) + 0.5f, i * SkIntToScalar(kTrans) + 0.5f,
          linePaint);
      canvas->drawLine(
          i * SkIntToScalar(kTrans) + 0.5f, 0.5f, i * SkIntToScalar(kTrans) + 0.5f,
          SkIntToScalar(kTrans) * (kSubPixelSteps + 1) + 0.5f, linePaint);
    }
    ...
    for (int i = 0; i <= kSubPixelSteps; ++i) {
      for (int j = 0; j <= kSubPixelSteps; ++j) {
        ...
        canvas->translate(
            j * (kTrans + 1.f / kSubPixelSteps) + 1, i * (kTrans + 1.f / kSubPixelSteps) + 1);
