...
static void do_unary(
  ...
  canvas->drawSimpleText(
      fn, strlen(fn), SkTextEncoding::kUTF8, 0.0f, (kBoxSize + bounds.height()) * 0.5f, font, p);
  canvas->translate(kLabelWidth, 0);

  const float xScale = xMax - xMin, xBias = xMin, yScale = 1.0f / (yMax - yMin),
...
DEF_SIMPLE_GM_BG(
    runtime_intrinsics_trig, canvas,
    /*width=*/kPadding + kLabelWidth + kBoxSize + kPadding,
    /*height=*/8 * (kPadding + kBoxSize) + kPadding, SK_ColorWHITE) {
