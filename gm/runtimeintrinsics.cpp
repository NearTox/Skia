...
static void next_column(SkCanvas* canvas) { canvas->translate(kBoxSize + kPadding, 0); }
...
static void draw_label(SkCanvas* canvas, const char* label) {
  ...
  canvas->drawSimpleText(
      label, strlen(label), SkTextEncoding::kUTF8, (kBoxSize - bounds.width()) * 0.5f,
      (kLabelHeight + bounds.height()) * 0.5f, font, p);
  canvas->translate(0, kLabelHeight);
}

static SkBitmap draw_shader(
    SkCanvas* canvas, sk_sp<SkShader> shader, bool allowRasterFallback = true) {
  SkPaint paint;
...
static void plot(
    ...
    const char* label = nullptr, bool requireES3 = false) {
  canvas->save();
  ...
  SkBitmap bitmap = draw_shader(
      canvas, builder.makeShader(/*localMatrix=*/nullptr, /*isOpaque=*/false),
...
DEF_SIMPLE_GPU_GM_CAN_FAIL(
    runtime_intrinsics_trig_es3, ctx, canvas, errorMsg, columns_to_width(3), rows_to_height(2)) {
...
// The OpenGL ES Shading Language, Version 3.00, Section 8.1
DEF_SIMPLE_GPU_GM_CAN_FAIL(
    runtime_intrinsics_common_es3, ctx, canvas, errorMsg, columns_to_width(6), rows_to_height(5)) {
  if (!ctx->priv().caps()->shaderCaps()->supportsSkSLES3()) {
    *errorMsg = "SkSL ES3 is not supported.";
    return skiagm::DrawResult::kSkip;
  }
  ...
  plot_es3(canvas, "floatBitsToInt(x)", -2, 2, -2'000'000'000, 2'000'000'000, "floatBitsToInt(s)");
  plot_es3(
      canvas, "floatBitsToInt(p).x", -2, 2, -2'000'000'000, 2'000'000'000, "floatBitsToInt(v)");
  plot_es3(canvas, "floatBitsToUint(x)", -2, 2, 0, 4'000'000'000, "floatBitsToUint(s)");
  plot_es3(canvas, "floatBitsToUint(p).x", -2, 2, 0, 4'000'000'000, "floatBitsToUint(v)");
  next_row(canvas);

  plot_es3(canvas, "intBitsToFloat(xi)", -2'000'000'000, 2'000'000'000, -2, 2, "intBitsToFloat(s)");
  plot_es3(
      canvas, "intBitsToFloat(pi).x", -2'000'000'000, 2'000'000'000, -2, 2, "intBitsToFloat(v)");
  plot_es3(canvas, "uintBitsToFloat(uint(xi))", 0, 4'000'000'000, -2, 2, "uintBitsToFloat(s)");
  plot_es3(canvas, "uintBitsToFloat(uint2(pi)).x", 0, 4'000'000'000, -2, 2, "uintBitsToFloat(v)");
  next_row(canvas);
...
template <int N>
static void plot_matrix_comp_mult(
    SkCanvas* canvas, std::array<float, N * N> mtx1, std::array<float, N * N> mtx2,
    const char* label) {
  canvas->save();
...
DEF_SIMPLE_GM(runtime_intrinsics_matrix, canvas, columns_to_width(3), rows_to_height(2)) {
  ...
  plot_matrix_comp_mult<2>(
      canvas, {1.00f, 0.0f, 2.0f, 0.5f}, {0.75f, 2.0f, 0.2f, 1.2f}, "compMult(2x2)");

  plot_matrix_comp_mult<3>(
      canvas, {1.00f, 0.0f, 2.0f, 0.5f, -1.0f, -2.0f, -0.5f, 4.00f, 0.25f},
      {0.75f, 2.0f, 0.2f, 1.2f, -0.8f, -0.1f, -1.8f, 0.25f, 2.00f}, "compMult(3x3)");

  plot_matrix_comp_mult<4>(
      canvas,
      {1.00f, 0.0f, 2.0f, 0.5f, -1.0f, -2.0f, -0.5f, 4.00f, 0.25f, 0.05f, 10.00f, -0.66f, -1.0f,
       -0.5f, 0.5f, 0.66f},
      {0.75f, 2.0f, 0.2f, 1.2f, -0.8f, -0.1f, -1.8f, 0.25f, 2.00f, 2.00f, 0.03f, -1.00f, -1.0f,
       -0.5f, 1.7f, 0.66f},
      "compMult(4x4)");
  next_row(canvas);

  // Random, invertible matrices where the elements of inverse(m) lie in [-1, 1]
  plot_matrix_inverse<2>(canvas, {1.20f, 0.68f, -0.27f, -1.55f}, "inverse(2x2)");

  plot_matrix_inverse<3>(
      canvas, {-1.13f, -2.96f, -0.14f, 1.45f, -1.88f, -1.02f, -2.54f, -2.58f, -1.17f},
      "inverse(3x3)");

  plot_matrix_inverse<4>(
      canvas,
      {-1.51f, -3.95f, -0.19f, 1.93f, -2.51f, -1.35f, -3.39f, -3.45f, -1.56f, 1.61f, -0.22f, -1.08f,
