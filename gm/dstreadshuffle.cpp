  ...
  void drawShape(SkCanvas* canvas, SkPaint* paint, ShapeType type) {
    ...
    switch (type) {
      case kCircle_ShapeType:
        ...
        break;
      case kRoundRect_ShapeType: canvas->drawRoundRect(kRect, 15.f, 15.f, *paint); break;
      case kRect_ShapeType: canvas->drawRect(kRect, *paint); break;
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    if (SkColorType::kUnknown_SkColorType == canvas->imageInfo().colorType()) {
      ...
      info = SkImageInfo::Make(
          35, 35, canvas->imageInfo().colorType(), canvas->imageInfo().alphaType(),
    ...
    if (!surf) {
      ...
      if ((info.colorType() == kRGBA_8888_SkColorType ||
        ...
        info = SkImageInfo::Make(
            35, 35, kN32_SkColorType, canvas->imageInfo().alphaType(),
            canvas->imageInfo().refColorSpace());
      }
