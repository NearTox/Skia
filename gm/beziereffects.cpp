...
class BezierConicTestOp : public BezierTestOp {
 ...
 private:
  ...
  BezierConicTestOp(
      GrClipEdgeType et, const SkRect& rect, const SkPMColor4f& color, const SkMatrix& klm)
  ...
  void onPrepareDraws(Target* target) override {
    GrGeometryProcessor* gp = GrConicEffect::Make(
        target->allocator(), this->color(), SkMatrix::I(), this->edgeType(), target->caps(),
        SkMatrix::I(), false);
    if (!gp) {
      return;
    }
      ...
      for (int col = 0; col < kGrClipEdgeTypeCnt; ++col) {
        ...
        for (int c = 0; c < cnt; ++c) {
          ...
          std::unique_ptr<GrDrawOp> op =
              BezierConicTestOp::Make(context, et, bounds, kOpaqueBlack, klm);
...
class BezierQuadTestOp : public BezierTestOp {
 ...
 private:
  ...
  BezierQuadTestOp(
      GrClipEdgeType et, const SkRect& rect, const SkPMColor4f& color,
      const GrPathUtils::QuadUVMatrix& devToUV)
  ...
  void onPrepareDraws(Target* target) override {
    GrGeometryProcessor* gp = GrQuadEffect::Make(
        target->allocator(), this->color(), SkMatrix::I(), this->edgeType(), target->caps(),
        SkMatrix::I(), false);
    if (!gp) {
      return;
    ...
    for (int row = 0; row < kNumQuads; ++row) {
      for (int col = 0; col < kGrClipEdgeTypeCnt; ++col) {
        ...
        for (int c = 0; c < cnt; ++c) {
          ...
          std::unique_ptr<GrDrawOp> op =
              BezierQuadTestOp::Make(context, et, bounds, kOpaqueBlack, DevToUV);
          renderTargetContext->priv().testingOnly_addDrawOp(std::move(op));
        }
      }
    }
