...
class BezierTestOp : public GrMeshDrawOp {
 ...
 protected:
  BezierTestOp(
      sk_sp<const GrGeometryProcessor> gp, const SkRect& rect, const SkPMColor4f& color,
      int32_t classID)
      : INHERITED(classID),
        fRect(rect),
        fColor(color),
...
class BezierConicTestOp : public BezierTestOp {
 public:
  ...
  static std::unique_ptr<GrDrawOp> Make(
      GrRecordingContext* context, sk_sp<const GrGeometryProcessor> gp, const SkRect& rect,
 ...
 private:
  ...
  void onPrepareDraws(Target* target) override {
    ...
    SkPointPriv::SetRectTriStrip(
        &verts[0].fPosition, rect.fLeft, rect.fTop, rect.fRight, rect.fBottom, sizeof(Vertex));
    for (int v = 0; v < 4; ++v) {
...
class BezierConicEffects : public GpuGM {
 public:
  BezierConicEffects() { this->setBGColor(0xFFFFFFFF); }

 protected:
  SkString onShortName() override { return SkString("bezier_conic_effects"); }
  ...
  void onDraw(
    ...
    for (int i = 0; i < kNumConics; ++i) {
      SkPoint baseControlPts[] = {{rand.nextRangeF(0.f, w), rand.nextRangeF(0.f, h)},
      ...
      for (int edgeType = 0; edgeType < kGrClipEdgeTypeCnt; ++edgeType) {
        ...
        gp = GrConicEffect::Make(
            color, SkMatrix::I(), et, *context->priv().caps(), SkMatrix::I(), false);
        if (!gp) {
          continue;
        }
        ...
        SkScalar x = col * w;
        SkScalar y = row * h;
        SkPoint controlPts[] = {{x + baseControlPts[0].fX, y + baseControlPts[0].fY},
        ...
        for (int c = 0; c < cnt; ++c) {
          ...
          std::unique_ptr<GrDrawOp> op = BezierConicTestOp::Make(context, gp, bounds, color, klm);
...
class BezierQuadTestOp : public BezierTestOp {
 public:
  ...
  static std::unique_ptr<GrDrawOp> Make(
      GrContext* context, sk_sp<const GrGeometryProcessor> gp, const SkRect& rect,
...
class BezierQuadEffects : public GpuGM {
 public:
  BezierQuadEffects() { this->setBGColor(0xFFFFFFFF); }

 protected:
  SkString onShortName() override { return SkString("bezier_quad_effects"); }
      ...
      for (int edgeType = 0; edgeType < kGrClipEdgeTypeCnt; ++edgeType) {
        ...
        for (int c = 0; c < cnt; ++c) {
          ...
          std::unique_ptr<GrDrawOp> op =
              BezierQuadTestOp::Make(context, gp, bounds, color, DevToUV);
          renderTargetContext->priv().testingOnly_addDrawOp(std::move(op));
        }
        ++col;
        if (numCols == col) {
