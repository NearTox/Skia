...
static sk_sp<SkImage> data_to_img(
    GrDirectContext* direct, sk_sp<SkData> data, SkImage::CompressionType compression) {
  if (direct) {
...
static void draw_image(SkCanvas* canvas, sk_sp<SkImage> image, int x, int y) {
  ...
  if (image && image->isTextureBacked()) {
    ...
    GrTextureProxy* proxy =
        sk_gpu_test::GetTextureImageProxy(image.get(), canvas->recordingContext());
...
class BC1TransparencyGM : public GM {
 ...
 protected:
  ...
  DrawResult onGpuSetup(GrDirectContext* dContext, SkString* errorMsg) override {
    ...
    fRGBAImage =
        data_to_img(dContext, std::move(bc1Data), SkImage::CompressionType::kBC1_RGBA8_UNORM);
