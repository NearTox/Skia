...
static sk_sp<SkData> make_compressed_data() {
  ...
  size_t totalSize =
      SkCompressedDataSize(SkImage::CompressionType::kBC1_RGB8_UNORM, dim, nullptr, false);
...
static sk_sp<SkImage> data_to_img(
    GrContext* context, sk_sp<SkData> data, SkImage::CompressionType compression) {
  if (context) {
    return SkImage::MakeTextureFromCompressed(
        context, std::move(data), kImgWidth, kImgHeight, compression, GrMipMapped::kNo);
  } else {
    return SkImage::MakeRasterFromCompressed(std::move(data), kImgWidth, kImgHeight, compression);
  }
}
...
class BC1TransparencyGM : public GM {
 public:
  BC1TransparencyGM() { this->setBGColor(SK_ColorGREEN); }

 protected:
  SkString onShortName() override { return SkString("bc1_transparency"); }

  SkISize onISize() override {
    return SkISize::Make(kImgWidth + 2 * kPad, 2 * kImgHeight + 3 * kPad);
  }

  void onOnceBeforeDraw() override { fBC1Data = make_compressed_data(); }

  void onDraw(SkCanvas* canvas) override {
    GrContext* context = canvas->getGrContext();

    sk_sp<SkImage> rgbImg =
        data_to_img(context, fBC1Data, SkImage::CompressionType::kBC1_RGB8_UNORM);

    sk_sp<SkImage> rgbaImg =
        data_to_img(context, fBC1Data, SkImage::CompressionType::kBC1_RGBA8_UNORM);
