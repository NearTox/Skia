...
static void draw_contents(SkSurface* surface, SkColor fillC) {
  SkSize size = SkSize::Make(SkIntToScalar(surface->width()), SkIntToScalar(surface->height()));
...
static void test_surface(SkCanvas* canvas, SkSurface* surf, bool usePaint) {
  ...
  src2.setLTRB(
      SkIntToScalar(-surf->width() / 2), SkIntToScalar(-surf->height() / 2),
  ...
  canvas->drawImageRect(
      imgR, src1, dst1, sampling, usePaint ? &paint : nullptr, SkCanvas::kStrict_SrcRectConstraint);
  canvas->drawImageRect(
      imgG, src2, dst2, sampling, usePaint ? &paint : nullptr, SkCanvas::kStrict_SrcRectConstraint);
  canvas->drawImageRect(
      imgR, src3, dst3, sampling, usePaint ? &paint : nullptr, SkCanvas::kStrict_SrcRectConstraint);
...
class ImageGM : public skiagm::GM {
 ...
 public:
  ...
  ~ImageGM() override { sk_free(fBuffer); }

 protected:
  SkString onShortName() override { return SkString("image-surface"); }

  SkISize onISize() override { return SkISize::Make(960, 1200); }
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    sk_sp<SkSurface> surf2(
        SkSurface::MakeRenderTarget(canvas->recordingContext(), SkBudgeted::kNo, info));
...
static sk_sp<SkImage> make_raster(
    const SkImageInfo& info, GrRecordingContext*, void (*draw)(SkCanvas*)) {
  auto surface(SkSurface::MakeRaster(info));
  draw(surface->getCanvas());
  return surface->makeImageSnapshot();
}

static sk_sp<SkImage> make_picture(
    const SkImageInfo& info, GrRecordingContext*, void (*draw)(SkCanvas*)) {
  SkPictureRecorder recorder;
  draw(recorder.beginRecording(SkRect::MakeIWH(info.width(), info.height())));
  return SkImage::MakeFromPicture(
      recorder.finishRecordingAsPicture(), info.dimensions(), nullptr, nullptr,
      SkImage::BitDepth::kU8, SkColorSpace::MakeSRGB());
}

static sk_sp<SkImage> make_codec(
    const SkImageInfo& info, GrRecordingContext*, void (*draw)(SkCanvas*)) {
  sk_sp<SkImage> image(make_raster(info, nullptr, draw));
  return SkImage::MakeFromEncoded(image->encodeToData());
}

static sk_sp<SkImage> make_gpu(
    const SkImageInfo& info, GrRecordingContext* ctx, void (*draw)(SkCanvas*)) {
  if (!ctx) {
    return nullptr;
  }
...
typedef sk_sp<SkImage> (*ImageMakerProc)(
    const SkImageInfo&, GrRecordingContext*, void (*)(SkCanvas*));
...
class ScalePixelsGM : public skiagm::GM {
 ...
 protected:
  SkString onShortName() override { return SkString("scale-pixels"); }
...
DEF_SIMPLE_GPU_GM(new_texture_image, rContext, canvas, 280, 60) {
  ...
  std::function<sk_sp<SkImage>()> imageFactories[] = {
      ...
      [render_image] {
        ...
        return SkImage::MakeFromPicture(
            recorder.finishRecordingAsPicture(), SkISize::Make(kSize, kSize), nullptr, nullptr,
            SkImage::BitDepth::kU8, srgbColorSpace);
      },
      // Create a texture image
      [rContext, render_image]() -> sk_sp<SkImage> {
        auto surface(SkSurface::MakeRenderTarget(
            rContext, SkBudgeted::kYes, SkImageInfo::MakeS32(kSize, kSize, kPremul_SkAlphaType)));
        if (!surface) {
          return nullptr;
        }
