  ...
  void drawShapes(SkCanvas* canvas) const override {
    ...
    for (int i = 0; i < fShapes.count(); i++) {
      ...
      switch (shape.getType()) {
        case SkRRect::kRect_Type: canvas->drawRect(shape.rect(), paint); break;
        case SkRRect::kOval_Type: canvas->drawOval(shape.rect(), paint); break;
        default: canvas->drawRRect(shape, paint); break;
      }
      canvas->restore();
    }
  }
