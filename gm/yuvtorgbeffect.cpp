  ...
  DrawResult onDraw(
    ...
    for (int i = 0; i < 3; ++i) {
      proxies[i] = proxyProvider->createTextureProxy(
          fImage[i], kNone_GrSurfaceFlags, 1, SkBudgeted::kYes, SkBackingFit::kExact);
      if (!proxies[i]) {
    ...
    for (int space = kJPEG_SkYUVColorSpace; space <= kLastEnum_SkYUVColorSpace; ++space) {
      SkRect renderRect =
          SkRect::MakeWH(SkIntToScalar(fImage[0]->width()), SkIntToScalar(fImage[0]->height()));
  ...
  DrawResult onDraw(
    ...
    for (uint32_t i = 0; i < SK_ARRAY_COUNT(kFilters); ++i) {
      ...
      for (uint32_t j = 0; j < 2; ++j) {
        ...
        std::unique_ptr<GrFragmentProcessor> fp(GrYUVtoRGBEffect::Make(
            proxies, yuvaIndices, kJPEG_SkYUVColorSpace, kFilters[i], SkMatrix::I(), domainPtr));
        if (fp) {
          GrPaint grPaint;
          grPaint.addColorFragmentProcessor(std::move(fp));
