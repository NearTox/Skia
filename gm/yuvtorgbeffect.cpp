 ...
 protected:
  ...
  DrawResult onDraw(
    ...
    for (int space = kJPEG_SkYUVColorSpace; space <= kLastEnum_SkYUVColorSpace; ++space) {
      SkRect renderRect =
          SkRect::MakeWH(SkIntToScalar(fBitmaps[0].width()), SkIntToScalar(fBitmaps[0].height()));
  ...
  DrawResult onDraw(
    ...
    for (int space = kJPEG_SkYUVColorSpace; space <= kLastEnum_SkYUVColorSpace; ++space) {
      ...
      auto fp = GrYUVtoRGBEffect::Make(
          views, yuvaIndices, static_cast<SkYUVColorSpace>(space), GrSamplerState::Filter::kNearest,
          caps);
      if (fp) {
        SkMatrix viewMatrix;
        viewMatrix.setTranslate(x, y);
...
class YUVtoRGBSubsetEffect : public GpuGM {
 public:
  YUVtoRGBSubsetEffect() { this->setBGColor(0xFFFFFFFF); }

 protected:
  SkString onShortName() override { return SkString("yuv_to_rgb_subset_effect"); }
  ...
  void onOnceBeforeDraw() override {
    ...
    unsigned char innerY[16] = {149, 160, 130, 105, 160, 130, 105, 149,
                                130, 105, 149, 160, 105, 149, 160, 130};
  ...
  DrawResult onDraw(
    ...
    for (uint32_t i = 0; i < SK_ARRAY_COUNT(kFilters); ++i) {
      ...
      for (uint32_t j = 0; j < GrSamplerState::kWrapModeCount + 1; ++j) {
        ...
        const auto& caps = *context->priv().caps();
        std::unique_ptr<GrFragmentProcessor> fp(GrYUVtoRGBEffect::Make(
            views, yuvaIndices, kJPEG_SkYUVColorSpace, samplerState, caps, SkMatrix::I(), subset));
        if (fp) {
          GrPaint grPaint;
          grPaint.setColorFragmentProcessor(std::move(fp));
