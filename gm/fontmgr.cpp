...
static SkScalar drawString(
    SkCanvas* canvas, const SkString& text, SkScalar x, SkScalar y, const SkFont& font) {
  canvas->drawString(text, x, y, font, SkPaint());
  return x + font.measureText(text.c_str(), text.size(), SkTextEncoding::kUTF8);
}

static SkScalar drawCharacter(
    SkCanvas* canvas, uint32_t character, SkScalar x, SkScalar y, const SkFont& origFont,
    SkFontMgr* fm, const char* fontName, const char* bcp47[], int bcp47Count,
  ...
  sk_sp<SkTypeface> typeface(
      fm->matchFamilyStyleCharacter(fontName, fontStyle, bcp47, bcp47Count, character));
...
class FontMgrMatchGM : public skiagm::GM {
  ...
  DrawResult onDraw(SkCanvas* canvas, SkString* errorMsg) override {
    ...
    const char* gNames[] = {"Helvetica Neue", "Arial", "sans"};
...
class FontMgrBoundsGM : public skiagm::GM {
 ...
 private:
  ...
  static SkRect show_bounds(
      SkCanvas* canvas, const SkFont& font, SkScalar x, SkScalar y, SkColor boundsColor,
      bool labelBounds) {
    SkGlyphID left = 0, right = 0, top = 0, bottom = 0;
    SkRect min = SkRect::MakeLTRB(
        SK_ScalarInfinity, SK_ScalarInfinity, SK_ScalarNegativeInfinity, SK_ScalarNegativeInfinity);
    {
      ...
      for (int i = 0; i < numGlyphs; ++i) {
        ...
        if (cur.fLeft < min.fLeft) {
          min.fLeft = cur.fLeft;
          left = i;
        }
        if (cur.fTop < min.fTop) {
          min.fTop = cur.fTop;
          top = i;
        }
        if (min.fRight < cur.fRight) {
          min.fRight = cur.fRight;
          right = i;
        }
        if (min.fBottom < cur.fBottom) {
          min.fBottom = cur.fBottom;
          bottom = i;
        }
      }
    }
    ...
    if ((fm.fFlags & SkFontMetrics::kUnderlinePositionIsValid_Flag) &&
      ...
      SkRect underline{
          min.fLeft, fm.fUnderlinePosition, min.fRight,
          fm.fUnderlinePosition + fm.fUnderlineThickness};
    ...
    if ((fm.fFlags & SkFontMetrics::kStrikeoutPositionIsValid_Flag) &&
      ...
      SkRect strikeout{
          min.fLeft, fm.fStrikeoutPosition - fm.fStrikeoutThickness, min.fRight,
    ...
    for (const GlyphToDraw& glyphToDraw : glyphsToDraw) {
      ...
      canvas->drawSimpleText(
          &glyphToDraw.id, sizeof(glyphToDraw.id), SkTextEncoding::kGlyphID, 0, 0, font,
          glyphPaint);
