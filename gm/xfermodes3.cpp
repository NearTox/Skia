...
class Xfermodes3GM : public GM {
 ...
 protected:
  ...
  SkISize onISize() override { return SkISize::Make(630, 1215); }
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    constexpr SkColor kSolidColors[] = {SK_ColorTRANSPARENT, SK_ColorBLUE, 0x80808000};
    ...
    for (size_t s = 0; s < SK_ARRAY_COUNT(kStrokes); ++s) {
      for (size_t m = 0; m < kSkBlendModeCount; ++m) {
        ...
        canvas->drawString(
            SkBlendMode_Name(mode), SkIntToScalar(x), SkIntToScalar(y + kSize + 3) + font.getSize(),
            font, labelP);
 ...
 private:
  ...
  sk_sp<SkSurface> makeTempSurface(SkCanvas* baseCanvas, int w, int h) {
    ...
    SkImageInfo info = SkImageInfo::Make(
        w, h, baseInfo.colorType(), baseInfo.alphaType(), baseInfo.refColorSpace());
    return baseCanvas->makeSurface(info);
  }

  void drawMode(
      SkCanvas* canvas, int x, int y, int w, int h, const SkPaint& modePaint, SkSurface* surface) {
    canvas->save();
  ...
  void onOnceBeforeDraw() override {
    ...
    const SkColor kColors[] = {SK_ColorTRANSPARENT, 0x80800000, 0xF020F060, SK_ColorWHITE};
    bmpPaint.setShader(SkGradientShader::MakeRadial(
        kCenter, 3 * SkIntToScalar(kSize) / 4, kColors, nullptr, SK_ARRAY_COUNT(kColors),
        SkTileMode::kRepeat));
