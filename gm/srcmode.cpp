...
static void identity_paintproc(SkPaint* paint) { paint->setShader(nullptr); }
...
public:
    SrcModeGM() { this->setBGColor(SK_ColorBLACK); }

protected:
    virtual SkString onShortName() { return SkString("srcmode"); }

    virtual SkISize onISize() { return SkISize::Make(640, 760); }
    ...
    void drawContent(SkCanvas* canvas) {
        ...
        const Proc procs[] = {draw_hair, draw_thick, draw_rect, draw_oval, draw_text};

        const SkBlendMode modes[] = {SkBlendMode::kSrcOver, SkBlendMode::kSrc, SkBlendMode::kClear};

        const PaintProc paintProcs[] = {identity_paintproc, gradient_paintproc};
