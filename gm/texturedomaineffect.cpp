    ...
    for (int tm = 0; tm < textureMatrices.count(); ++tm) {
      for (size_t d = 0; d < SK_ARRAY_COUNT(texelDomains); ++d) {
        ...
        for (int m = 0; m < GrSamplerState::kWrapModeCount; ++m) {
          ...
          auto fp1 = GrTextureEffect::MakeTexelSubset(
              proxy, fBitmap.alphaType(), textureMatrices[tm], sampler, texelDomains[d], caps);
          if (!fp1) {
            continue;
          }
          ...
          auto fp2 = GrTextureEffect::Make(
              subsetProxies[d], fBitmap.alphaType(), subsetTextureMatrix,
