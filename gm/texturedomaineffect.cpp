...
class TextureDomainEffect : public GpuGM {
 ...
 protected:
  ...
  void onOnceBeforeDraw() override {
    ...
    paint.setShader(
        SkGradientShader::MakeSweep(65.f, 75.f, colors1, nullptr, SK_ARRAY_COUNT(colors1)));
    canvas.drawOval(SkRect::MakeXYWH(-5.f, -5.f, kTargetWidth + 10.f, kTargetHeight + 10.f), paint);

    SkColor colors2[] = {SK_ColorMAGENTA, SK_ColorLTGRAY, SK_ColorYELLOW};
    paint.setShader(
        SkGradientShader::MakeSweep(45.f, 55.f, colors2, nullptr, SK_ARRAY_COUNT(colors2)));
    paint.setBlendMode(SkBlendMode::kDarken);
    canvas.drawOval(SkRect::MakeXYWH(-5.f, -5.f, kTargetWidth + 10.f, kTargetHeight + 10.f), paint);

    SkColor colors3[] = {SK_ColorBLUE, SK_ColorLTGRAY, SK_ColorGREEN};
    paint.setShader(
        SkGradientShader::MakeSweep(25.f, 35.f, colors3, nullptr, SK_ARRAY_COUNT(colors3)));
    paint.setBlendMode(SkBlendMode::kLighten);
    canvas.drawOval(SkRect::MakeXYWH(-5.f, -5.f, kTargetWidth + 10.f, kTargetHeight + 10.f), paint);
  }

  DrawResult onDraw(
      GrContext* context, GrRenderTargetContext* renderTargetContext, SkCanvas* canvas,
    ...
    const SkIRect texelDomains[] = {
        fBitmap.bounds(),
        SkIRect::MakeXYWH(
            fBitmap.width() / 4 - 1, fBitmap.height() / 4 - 1, fBitmap.width() / 2 + 2,
            fBitmap.height() / 2 + 2),
    };
    ...
    for (int tm = 0; tm < textureMatrices.count(); ++tm) {
      for (size_t d = 0; d < SK_ARRAY_COUNT(texelDomains); ++d) {
        ...
        for (int m = 0; m < GrTextureDomain::kModeCount; ++m) {
          ...
          auto fp1 =
              GrSimpleTextureEffect::Make(proxy, fBitmap.alphaType(), textureMatrices[tm], fFilter);
          fp1 = GrDomainEffect::Make(
              std::move(fp1), GrTextureDomain::MakeTexelDomain(texelDomains[d], mode), mode,
              fFilter);
          if (!fp1) {
            continue;
          }
          ...
          if (auto op =
                  sk_gpu_test::test_ops::MakeRect(context, std::move(fp1), drawRect, localRect)) {
          ...
          if (auto op = sk_gpu_test::test_ops::MakeRect(
                  context, std::move(fp2), drawRect, localRect.makeOffset(kT),
