...
static sk_sp<SkImage> make_src(int w, int h) {
  ...
  SkColor colors[] = {
      SK_ColorTRANSPARENT, SK_ColorGREEN, SK_ColorCYAN, SK_ColorRED, SK_ColorMAGENTA, SK_ColorWHITE,
  };
...
class ArithmodeGM : public skiagm::GM {
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    constexpr SkScalar K[] = {
        0,       0,       0,        0,       0,       0,       0,   one,      0,       one,     0,
        0,       0,       0,        one,     0,       0,       one, one,      0,       0,       one,
        -one,    0,       0,        one / 2, one / 2, 0,       0,   one / 2,  one / 2, one / 4, 0,
        one / 2, one / 2, -one / 4, one / 4, one / 2, one / 2, 0,   -one / 4, one / 2, one / 2, 0,
    };
    ...
    while (k < stop) {
      {
        ...
        paint.setImageFilter(SkImageFilters::Arithmetic(
            k[0], k[1], k[2], k[3], true, dstFilter, srcFilter, nullptr));
    ...
    for (int i = 0; i < 2; i++) {
...

      {
        ...
        sk_sp<SkImageFilter> bg = SkImageFilters::Arithmetic(
            0, 0, -one / 2, 1, enforcePMColor, dstFilter, nullptr, nullptr);
        SkPaint p;
        p.setImageFilter(SkImageFilters::Arithmetic(
            0, one / 2, -one, 1, true, std::move(bg), dstFilter, nullptr));
...
class Arithmode2GM : public skiagm::GM {
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    paint.setImageFilter(SkImageFilters::Blend(
        blender,
        /* dst imagefilter */ nullptr, SkImageFilters::Image(fSrc, sampling)));
