...
class HairlinesGM : public GM {
 protected:
  ...
  void onOnceBeforeDraw() override {
    ...
    fPaths.push_back(SkPathBuilder().moveTo(0, -10).quadTo(100, 100, -10, 0).detach());

    fPaths.push_back(SkPathBuilder().moveTo(0, -5).quadTo(100, 100, -5, 0).detach());

    fPaths.push_back(SkPathBuilder().moveTo(0, -2).quadTo(100, 100, -2, 0).detach());

    fPaths.push_back(SkPathBuilder().moveTo(0, -1).quadTo(100, 100, -2 + 306.0f / 4, 75).detach());

    fPaths.push_back(SkPathBuilder().moveTo(0, -1).quadTo(100, 100, -1, 0).detach());

    fPaths.push_back(SkPathBuilder().moveTo(0, -0).quadTo(100, 100, 0, 0).detach());

    fPaths.push_back(SkPathBuilder().moveTo(0, -0).quadTo(100, 100, 75, 75).detach());
    ...
    fPaths.push_back(SkPathBuilder().moveTo(4, 6).cubicTo(5, 6, 5, 4, 4, 0).close().detach());
    ...
    fPaths.push_back(
        SkPathBuilder()
            ...
            .cubicTo(4.66472578f, 3.888381f, 5.f, 2.875f, 5.f, 1.f)
            .close()
            .detach());

    // Three paths that show the same bug (missing end caps)

    fPaths.push_back(SkPathBuilder()
                         .moveTo(6.5f, 5.5f)
                         .lineTo(3.5f, 0.5f)
                         .moveTo(0.5f, 5.5f)
                         .lineTo(3.5f, 0.5f)
    ...
    fPaths.push_back(SkPathBuilder().moveTo(1, 1).lineTo(6, 6).moveTo(1, 6).lineTo(6, 1).detach());

    // A right angle (crbug.com/137465 and crbug.com/256776)
    fPaths.push_back(
        SkPathBuilder().moveTo(5.5f, 5.5f).lineTo(5.5f, 0.5f).lineTo(0.5f, 0.5f).detach());
...
static void draw_subdivided_quad(SkCanvas* canvas, int x0, int y0, int x1, int y1, SkColor color) {
  ...
  SkPath quad;
  quad.moveTo(0, 0);
  quad.quadTo(SkIntToScalar(x0), SkIntToScalar(y0), SkIntToScalar(x1), SkIntToScalar(y1));
