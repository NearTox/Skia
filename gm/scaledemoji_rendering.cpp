...
class ScaledEmojiRenderingGM : public GM {
 ...
 protected:
  ...
  SkString onShortName() override { return SkString("scaledemoji_rendering"); }
    ...
    for (auto&& [i, test] : SkMakeEnumerate(tests)) {
      ...
      for (SkScalar textSize : {70, 150}) {
        ...
        for (bool fakeBold : {false, true}) {
          ...
          SkRect bounds;
          SkScalar advance =
              font.measureText(text, strlen(text), SkTextEncoding::kUTF8, &bounds, &textPaint);
          canvas->drawSimpleText(text, strlen(text), SkTextEncoding::kUTF8, x, y, font, textPaint);
          if ((false)) {
            bounds.offset(x, y);
