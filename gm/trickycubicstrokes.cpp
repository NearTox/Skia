...
enum class CellFillMode { kStretch, kCenter };
...
static const TrickyCubic kTrickyCubics[] = {
    ...
    {{{1, 1}, {100, 1}, {25, 1}, {1.5f, std::numeric_limits<float>::quiet_NaN()}},
     3,
     CellFillMode::kStretch},  // Flat CONIC with a cusp
};
...
enum class FillMode { kCenter, kScale };
...
static void draw_test(SkCanvas* canvas, const SkColor strokeColor) {
  ...
  for (size_t i = 0; i < SK_ARRAY_COUNT(kTrickyCubics); ++i) {
    ...
    auto cellRect = SkRect::MakeXYWH(
        (i % kNumCols) * kCellSize, (i / kNumCols) * kCellSize, kCellSize, kCellSize);
...
class TrickyCubicStrokes_tess_segs_5 : public skiagm::GpuGM {
  SkString onShortName() override { return SkString("trickycubicstrokes_tess_segs_5"); }
  ...
  DrawResult onDraw(
    ...
    if (context->priv().caps()->shaderCaps()->maxTessellationSegments() !=
      ...
      errorMsg->set(
          "modifyGrContextOptions did not affect maxTessellationSegments. "
