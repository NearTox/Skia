...
DEF_SIMPLE_GM(arcto, canvas, 500, 600) {
  ...
  SkPathBuilder path;
  path.moveTo(100, 100).arcTo(
      {0, 0}, 0, SkPathBuilder::kLarge_ArcSize, SkPathDirection::kCW, {200, 200});
  canvas->drawPath(path.detach(), paint);

  path.moveTo(200, 100).arcTo(
      {80, 80}, 0, SkPathBuilder::kLarge_ArcSize, SkPathDirection::kCW, {200, 100});
  canvas->drawPath(path.detach(), paint);
}
...
DEF_SIMPLE_GM(parsedpaths, canvas, kParsePathTestDimension, kParsePathTestDimension) {
  ...
  if (GENERATE_SVG_REFERENCE) {
    ...
    str.printf(
        "<svg width=\"%d\" height=\"%d\">\n", kParsePathTestDimension, kParsePathTestDimension);
