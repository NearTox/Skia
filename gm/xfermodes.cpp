...
const struct {
  ...
  int fSourceTypeMask;  // The source types to use this
    ...
    {SkBlendMode::kClear, kAll_SrcType},       {SkBlendMode::kSrc, kAll_SrcType},
    {SkBlendMode::kDst, kAll_SrcType},         {SkBlendMode::kSrcOver, kAll_SrcType},
    {SkBlendMode::kDstOver, kAll_SrcType},     {SkBlendMode::kSrcIn, kAll_SrcType},
    {SkBlendMode::kDstIn, kAll_SrcType},       {SkBlendMode::kSrcOut, kAll_SrcType},
    {SkBlendMode::kDstOut, kAll_SrcType},      {SkBlendMode::kSrcATop, kAll_SrcType},
    {SkBlendMode::kDstATop, kAll_SrcType},

    {SkBlendMode::kXor, kBasic_SrcType},       {SkBlendMode::kPlus, kBasic_SrcType},
    {SkBlendMode::kModulate, kAll_SrcType},    {SkBlendMode::kScreen, kBasic_SrcType},
    {SkBlendMode::kOverlay, kBasic_SrcType},   {SkBlendMode::kDarken, kBasic_SrcType},
    {SkBlendMode::kLighten, kBasic_SrcType},   {SkBlendMode::kColorDodge, kBasic_SrcType},
    {SkBlendMode::kColorBurn, kBasic_SrcType}, {SkBlendMode::kHardLight, kBasic_SrcType},
    {SkBlendMode::kSoftLight, kBasic_SrcType}, {SkBlendMode::kDifference, kBasic_SrcType},
    {SkBlendMode::kExclusion, kBasic_SrcType}, {SkBlendMode::kMultiply, kAll_SrcType},
    {SkBlendMode::kHue, kBasic_SrcType},       {SkBlendMode::kSaturation, kBasic_SrcType},
    {SkBlendMode::kColor, kBasic_SrcType},     {SkBlendMode::kLuminosity, kBasic_SrcType},
};
...
class XfermodesGM : public skiagm::GM {
  ...
  void draw_mode(SkCanvas* canvas, SkBlendMode mode, SrcType srcType, SkScalar x, SkScalar y) {
    ...
    switch (srcType) {
      ...
      case kQuarterClearInLayer_SrcType: {
        SkRect bounds = SkRect::MakeXYWH(x, y, SkIntToScalar(W), SkIntToScalar(H));
      ...
      case kQuarterClear_SrcType: {
        ...
        p.setColor(ToolUtils::color_to_565(0xFF66AAFF));
        SkRect r = SkRect::MakeXYWH(x + halfW, y, halfW, SkIntToScalar(H));
        canvas->drawRect(r, p);
      ...
      case kRectangle_SrcType: {
        SkScalar w = SkIntToScalar(W);
        SkScalar h = SkIntToScalar(H);
        SkRect r = SkRect::MakeXYWH(x + w / 3, y + h / 3, w * 37 / 60, h * 37 / 60);
        p.setColor(ToolUtils::color_to_565(0xFF66AAFF));
        canvas->drawRect(r, p);
        break;
      }
      case kSmallRectangleImageWithAlpha_SrcType:
        m.postScale(SK_ScalarHalf, SK_ScalarHalf, x, y);
        [[fallthrough]];
      case kRectangleImageWithAlpha_SrcType: p.setAlpha(0x88); [[fallthrough]];
 ...
 protected:
  SkString onShortName() override { return SkString("xfermodes"); }

  SkISize onISize() override { return SkISize::Make(1990, 570); }
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    for (int sourceType = 1; sourceType & kAll_SrcType; sourceType <<= 1) {
      ...
      for (size_t i = 0; i < SK_ARRAY_COUNT(gModes); i++) {
        ...
        draw_mode(canvas, gModes[i].fMode, static_cast<SrcType>(sourceType), r.fLeft, r.fTop);
        canvas->restore();
        ...
        SkTextUtils::DrawString(
            canvas, label, x + w / 2, y - font.getSize() / 2, font, labelP,
