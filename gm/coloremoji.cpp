...
static sk_sp<SkShader> MakeLinear() {
  ...
  return SkGradientShader::MakeLinear(
      kPts, kColors, kPos, SK_ARRAY_COUNT(kColors), SkTileMode::kClamp);
}
...
static sk_sp<SkColorFilter> make_color_filter() {
  return SkColorMatrixFilter::MakeLightingFilter(
      SkColorSetRGB(0x00, 0x80, 0xFF), SkColorSetRGB(0xFF, 0x20, 0x00));
}
...
class ColorEmojiGM : public GM {
 ...
 protected:
  ...
  SkString onShortName() override { return SkString("coloremoji"); }
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    for (const bool& fakeBold : {false, true}) {
      ...
      for (const SkScalar& textSize : textSizes) {
        ...
        y += -metrics.fAscent;
        canvas->drawSimpleText(text, textLen, SkTextEncoding::kUTF8, 10, y, font, SkPaint());
          ...
          for (int makeMode = 0; makeMode < 2; ++makeMode) {
            for (int alpha = 0; alpha < 2; ++alpha) {
              ...
              canvas->drawSimpleText(
                  text, textLen, SkTextEncoding::kUTF8, 380, y, shaderFont, shaderPaint);
              y += metrics.fDescent + metrics.fLeading;
            }
          }
        }
    ...
    SkRect lowerRightClip =
        SkRect::MakeXYWH(bounds.centerX(), bounds.centerY(), boundsHalfWidth, boundsHalfHeight);
