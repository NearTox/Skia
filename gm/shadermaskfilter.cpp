...
static void draw_masked_image(
    SkCanvas* canvas, const SkImage* image, SkScalar x, SkScalar y, const SkImage* mask,
    sk_sp<SkMaskFilter> outer, SkBlendMode mode) {
  SkMatrix matrix = SkMatrix::MakeScale(
      SkIntToScalar(image->width()) / mask->width(),
...
DEF_SIMPLE_GM(combinemaskfilter, canvas, 560, 510) {
  ...
  for (int i = 0; i < 5; ++i) {
    SkTextUtils::DrawString(
        canvas, gCoverageName[i], r.width() * 0.5f, -10, font, SkPaint(),
...
DEF_SIMPLE_GM(shadermaskfilter_localmatrix, canvas, 1500, 1000) {
  ...
  static const ShaderMakerT gShaderMakers[] = {
      [](SkCanvas* canvas, const SkMatrix& lm) -> sk_sp<SkShader> {
        auto surface = ToolUtils::makeSurface(canvas, SkImageInfo::MakeN32Premul(kSize, kSize));
  ...
  struct Config {
    SkMatrix fCanvasMatrix, fMaskMatrix, fShaderMatrix;
      ...
      {SkMatrix::Concat(SkMatrix::MakeScale(2, 2), SkMatrix::MakeTrans(10, 10)), SkMatrix::I(),
       SkMatrix::I()},
      {SkMatrix::I(), SkMatrix::Concat(SkMatrix::MakeScale(2, 2), SkMatrix::MakeTrans(10, 10)),
       SkMatrix::I()},
