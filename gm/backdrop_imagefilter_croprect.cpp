...
static void draw_backdrop_filter_gm(
    SkCanvas* canvas, float outsetX, float outsetY, FilterFactory factory) {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (i == 1) {
      ...
      canvas->saveLayer(SkCanvas::SaveLayerRec(
          nullptr, nullptr, imageFilter.get(), SkCanvas::kInitWithPrevious_SaveLayerFlag));
    }
...
static sk_sp<SkImageFilter> make_invert_filter(const SkIRect* crop) {
  static const float matrix[20] = {-1.f, 0.f, 0.f,  0.f, 1.f, 0.f, -1.f, 0.f, 0.f, 1.f,
