...
static void test_hittest(SkCanvas* canvas, const SkPath& path) {
    ...
    for (SkScalar y = r.fTop + SK_ScalarHalf - MARGIN; y < r.fBottom + MARGIN; y += SK_Scalar1) {
        for (SkScalar x = r.fLeft + SK_ScalarHalf - MARGIN; x < r.fRight + MARGIN;
             x += SK_Scalar1) {
...
DEF_SIMPLE_GM(hittestpath, canvas, 700, 460) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        path.quadTo(randoms[2] * scale, randoms[3] * scale, randoms[4] * scale, randoms[5] * scale);
        path.cubicTo(randoms[6] * scale, randoms[7] * scale, randoms[8] * scale, randoms[9] * scale,
