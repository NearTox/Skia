...
static SkPath create_underline(const SkTDArray<SkScalar>& intersections, SkScalar last,
                               SkScalar finalPos, SkScalar uPos, SkScalar uWidth,
                               SkScalar textSize) {
    SkPath underline;
    SkScalar end = last;
...
sk_sp<SkTextBlob> MakeFancyBlob(const SkPaint& paint, const SkFont& font, const char* text) {
    ...
    // Default-positioned run.
    {
        ...
        const SkTextBlobBuilder::RunBuffer& buf =
                blobBuilder.allocRun(font, defaultRunLen, advance, 0);
    ...
    // Horizontal-positioned run.
    {
        ...
        const SkTextBlobBuilder::RunBuffer& buf =
                blobBuilder.allocRunPosH(font, horizontalRunLen, 0);
