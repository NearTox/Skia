...
static SkPath create_underline(
    const SkTDArray<SkScalar>& intersections, SkScalar last, SkScalar finalPos, SkScalar uPos,
    SkScalar uWidth, SkScalar textSize) {
  SkPath underline;
  SkScalar end = last;
...
sk_sp<SkTextBlob> MakeFancyBlob(const SkPaint& paint, const SkFont& font, const char* text) {
  ...
  // Default-positioned run.
  {
    ...
    const SkTextBlobBuilder::RunBuffer& buf = blobBuilder.allocRun(font, defaultRunLen, advance, 0);
  ...
  // Horizontal-positioned run.
  {
    ...
    const SkTextBlobBuilder::RunBuffer& buf = blobBuilder.allocRunPosH(font, horizontalRunLen, 0);
    memcpy(
        buf.glyphs, glyphs.get() + glyphIndex,
  ...
  // Full-positioned run.
  {
    ...
    memcpy(buf.glyphs, glyphs.get() + glyphIndex, SkTo<uint32_t>(fullRunLen) * sizeof(SkGlyphID));
...
static sk_sp<SkTextBlob> make_text(const SkFont& font, const SkGlyphID glyphs[], int count) {
  return SkTextBlob::MakeFromText(
      glyphs, count * sizeof(SkGlyphID), font, SkTextEncoding::kGlyphID);
}

static sk_sp<SkTextBlob> make_posh(
    const SkFont& font, const SkGlyphID glyphs[], int count, SkScalar spacing) {
  ...
  return SkTextBlob::MakeFromPosTextH(
      glyphs, count * sizeof(SkGlyphID), xpos.get(), 0, font, SkTextEncoding::kGlyphID);
}

static sk_sp<SkTextBlob> make_pos(
    const SkFont& font, const SkGlyphID glyphs[], int count, SkScalar spacing) {
  ...
  return SkTextBlob::MakeFromPosText(
      glyphs, count * sizeof(SkGlyphID), pos.get(), font, SkTextEncoding::kGlyphID);
}
