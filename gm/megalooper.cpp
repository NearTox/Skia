...
class MegaLooperGM : public skiagm::GM {
 ...
 protected:
  virtual SkString onShortName() {
    switch (fType) {
      ...
      default: return SkString("megalooper_1x4"); break;
    }
  }
  ...
  virtual void onDraw(SkCanvas* canvas) {
    for (int y = 100; y < kHeight; y += 200) {
      for (int x = 100; x < kWidth; x += 200) {
        switch (fType) {
          case k0x0_Type: draw0x0(canvas, SkIntToScalar(x), SkIntToScalar(y)); break;
          case k4x1_Type: draw4x1(canvas, SkIntToScalar(x), SkIntToScalar(y)); break;
          case k1x4_Type:  // fall through
          default: draw1x4(canvas, SkIntToScalar(x), SkIntToScalar(y)); break;
        }
      }
    }
  }
 ...
 private:
  ...
  void draw0x0(SkCanvas* canvas, SkScalar x, SkScalar y) {
    ...
    SkRect outerClip = {-kHalfOuterClipSize - kHalfSquareSize,
                        -kHalfOuterClipSize - kHalfSquareSize, kHalfOuterClipSize + kHalfSquareSize,
                        kHalfOuterClipSize + kHalfSquareSize};
  ...
  void draw4x1(SkCanvas* canvas, SkScalar x, SkScalar y) {
    for (int i = 0; i < 4; ++i) {
      ...
      SkRect outerClip = {-kHalfOuterClipSize, -kHalfOuterClipSize, kHalfOuterClipSize,
                          kHalfOuterClipSize};
      outerClip.offset(x, y);
