...
static void show_image(SkCanvas* canvas, SkImage* image, sk_sp<SkImageFilter> filter) {
  ...
  SkRect r =
      SkRect::MakeIWH(image->width(), image->height()).makeOutset(SK_ScalarHalf, SK_ScalarHalf);
...
template <typename T>
ImageFilterFactory IFCCast(T arg) {
  return arg;
}
...
DEF_SIMPLE_GM(localmatriximagefilter, canvas, 640, 640) {
  ...
  const SkMatrix matrices[] = {SkMatrix::MakeScale(SK_ScalarHalf, SK_ScalarHalf),
                               SkMatrix::MakeScale(2, 2), SkMatrix::MakeTrans(10, 10)};
