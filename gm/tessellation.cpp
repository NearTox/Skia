...
void TessellationTestRectShader::Impl::writeFragmentShader(
    GrGLSLFPFragmentBuilder* f, const char* color, const char* coverage) {
  f->declareGlobal(
      GrShaderVar("barycentric_coord", kFloat4_GrSLType, GrShaderVar::kIn_TypeModifier));
  f->codeAppendf(
      R"(
...
class TessellationTestOp : public GrDrawOp {
 ...
 private:
  ...
  void onExecute(GrOpFlushState* state, const SkRect& chainBounds) override {
    GrPipeline pipeline(
        GrScissorTest::kDisabled, SkBlendMode::kSrc, state->drawOpArgs().outputSwizzle());
    ...
    GrProgramInfo programInfo(
        ...
        state->proxy()->backendFormat(), state->view()->origin(), &pipeline, shader.get(),
        &fixedDynamicState, nullptr, 0, GrPrimitiveType::kPatches,
        mesh.tessellationPatchVertexCount());
