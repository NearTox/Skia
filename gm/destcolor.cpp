...
class DestColorTestFP : public GrFragmentProcessor {
 ...
 private:
  ...
  explicit DestColorTestFP(const DestColorTestFP& that) : INHERITED(that) {}
...
DEF_SIMPLE_GPU_GM_CAN_FAIL(destcolor, rContext, canvas, errorMsg, 640, 640) {
  ...
  // Draw the mandrill.
  SkPaint p;
  p.setImageFilter(SkImageFilters::Image(
      GetResourceAsImage("images/mandrill_512.png"), bounds, bounds, SkSamplingOptions()));
  canvas->drawPaint(p);
  ...
  invertPaint.setColorFragmentProcessor(DestColorTestFP::Make(GrFragmentProcessor::SurfaceColor()));
  sdc->drawOval(
      /*clip*/ nullptr, std::move(invertPaint), GrAA::kYes, SkMatrix::I(),
