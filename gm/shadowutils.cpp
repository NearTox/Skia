...
void draw_shadow(
    SkCanvas* canvas, const SkPath& path, SkScalar height, SkColor color, SkPoint3 lightPos,
    SkScalar lightR, bool isAmbient, uint32_t flags) {
  SkScalar ambientAlpha = isAmbient ? .5f : 0.f;
  SkScalar spotAlpha = isAmbient ? 0.f : .5f;
  SkColor ambientColor = SkColorSetARGB(
      ambientAlpha * SkColorGetA(color), SkColorGetR(color), SkColorGetG(color),
      SkColorGetB(color));
  SkColor spotColor = SkColorSetARGB(
      spotAlpha * SkColorGetA(color), SkColorGetR(color), SkColorGetG(color), SkColorGetB(color));
  SkShadowUtils::DrawShadow(
      canvas, path, SkPoint3{0, 0, height}, lightPos, lightR, ambientColor, spotColor, flags);
}
...
void draw_paths(SkCanvas* canvas, ShadowMode mode) {
    ...
    for (int flags : {kNone_ShadowFlag, kTransparentOccluder_ShadowFlag}) {
      ...
      for (const auto& path : paths) {
        ...
        if (kDebugColorNoOccluders == mode || kDebugColorOccluders == mode) {
          ...
          SkShadowUtils::DrawShadow(
              canvas, path, SkPoint3{0, 0, kHeight}, lightPos, kLightR, ambientColor, spotColor,
              flags);
        }
  ...
  for (auto& m : matrices) {
    ...
    for (const auto& path : concavePaths) {
      ...
      if (kDebugColorNoOccluders == mode || kDebugColorOccluders == mode) {
        draw_shadow(canvas, path, kHeight, SK_ColorRED, lightPos, kLightR, true, kNone_ShadowFlag);
        draw_shadow(
            canvas, path, kHeight, SK_ColorBLUE, lightPos, kLightR, false, kNone_ShadowFlag);
      } else if (kGrayscale == mode) {
        SkColor ambientColor = SkColorSetARGB(0.1f * 255, 0, 0, 0);
        SkColor spotColor = SkColorSetARGB(0.25f * 255, 0, 0, 0);
        SkShadowUtils::DrawShadow(
...
DEF_SIMPLE_GM(shadow_utils_gaussian_colorfilter, canvas, 512, 256) {
  ...
  auto sh = SkGradientShader::MakeRadial(
      {r.centerX(), r.centerY()}, r.width(), colors, nullptr, SK_ARRAY_COUNT(colors),
      SkTileMode::kClamp);
...
DEF_SIMPLE_GM(shadow_utils_directional, canvas, 256, 384) {
  ...
  for (int i = 0; i < 3; ++i) {
    SkShadowUtils::DrawShadow(
        canvas, rrect, SkPoint3{0, 0, kHeight}, lightPos, kLightR, ambientColor, spotColor,
