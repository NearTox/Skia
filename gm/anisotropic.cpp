...
class AnisotropicGM : public GM {
 public:
  ...
  AnisotropicGM(Mode mode) : fMode(mode) {
    switch (fMode) {
      case Mode::kLinear: fSampling = SkSamplingOptions(SkFilterMode::kLinear); break;
      case Mode::kMip:
        fSampling = SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kLinear);
        break;
      case Mode::kAniso: fSampling = SkSamplingOptions::Aniso(16); break;
    }
 ...
 protected:
  SkString onShortName() override {
    ...
    switch (fMode) {
      case Mode::kLinear: name += "linear"; break;
      case Mode::kMip: name += "mip"; break;
      case Mode::kAniso: name += "aniso"; break;
    }
    return name;
  }
  ...
  SkISize onISize() override {
    return SkISize::Make(
        2 * kImageSize + 3 * kSpacer, kNumVertImages * kImageSize + (kNumVertImages + 1) * kSpacer);
  }
  ...
  void onOnceBeforeDraw() override {
    ...
    for (int i = 0; i < kNumLines; ++i, angle += kAngleStep) {
      ...
      canvas->drawLine(
          cos * kInnerOffset, sin * kInnerOffset, cos * kImageSize / 2, sin * kImageSize / 2, p);
    }
 ...
 protected:
  ...
  sk_sp<SkImage> updateImage(SkSurface* surf, SkColor color) {
    ...
    surf->getCanvas()->drawRect(
        SkRect::MakeLTRB(
            surf->width() * 2 / 5.f, surf->height() * 2 / 5.f, surf->width() * 3 / 5.f,
            surf->height() * 3 / 5.f),
        paint);
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    auto ii = SkImageInfo::Make(
        kImageSize, kImageSize, ct, kPremul_SkAlphaType, canvas->imageInfo().refColorSpace());
    ...
    if (auto rc = canvas->recordingContext()) {
      surface = SkSurface::MakeRenderTarget(
          rc, SkBudgeted::kYes, ii, 1, kTopLeft_GrSurfaceOrigin,
