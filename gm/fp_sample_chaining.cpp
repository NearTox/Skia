...
class UniformMatrixEffect : public GrFragmentProcessor {
 public:
  ...
  UniformMatrixEffect(std::unique_ptr<GrFragmentProcessor> child)
    ...
    this->registerChild(
        std::move(child), SkSL::SampleUsage::UniformMatrix(/*hasPerspective=*/false));
  }
     ...
     public:
      void emitCode(EmitArgs& args) override {
        fMatrixVar = args.fUniformHandler->addUniform(
            &args.fFp, kFragment_GrShaderFlag, SkSLType::kFloat3x3,
...
static std::unique_ptr<GrFragmentProcessor> wrap(
    std::unique_ptr<GrFragmentProcessor> fp, EffectType effectType, int drawX, int drawY) {
  switch (effectType) {
