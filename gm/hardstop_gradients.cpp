...
static void shade_rect(SkCanvas* canvas, sk_sp<SkShader> shader, int cellRow, int cellCol) {
  ...
  SkRect rect = SkRect::MakeXYWH(
      ...
      SkIntToScalar(cellRow * CELL_HEIGHT + PAD_HEIGHT), SkIntToScalar(RECT_WIDTH),
...
class HardstopGradientShaderGM : public skiagm::GM {
 ...
 protected:
  SkString onShortName() override { return SkString("hardstop_gradients"); }
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    SkColor colors[] = {
        SK_ColorRED, SK_ColorGREEN, SK_ColorBLUE, SK_ColorYELLOW, SK_ColorMAGENTA,
    };
    ...
    SkScalar* positions[NUM_ROWS] = {
        nullptr, nullptr, row3, row4, row5, row6, row7, row8,
    };
    ...
    for (int cellRow = 0; cellRow < NUM_ROWS; cellRow++) {
      for (int cellCol = 0; cellCol < NUM_COLS; cellCol++) {
        ...
        auto shader = SkGradientShader::MakeLinear(
            points, colors, positions[cellRow], numGradientColors[cellRow], tilemodes[cellCol], 0,
            nullptr);
