...
class BlurCircles2GM : public skiagm::GM {
 public:
  BlurCircles2GM() {
    ...
    fAnimBlurRadius = TimeUtils::PingPong(
        0, kBlurRadiusPingPoingPeriod, kBlurRadiusPingPoingShift, kMinBlurRadius, kMaxBlurRadius);
  }
 ...
 protected:
  ...
  SkISize onISize() override { return SkISize::Make(730, 1350); }
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    auto almostCircleMaker = [](SkScalar radius) {
      return SkPathBuilder()
          .addArc(SkRect::MakeXYWH(-radius, -radius, 2 * radius, 2 * radius), 0, 355)
          .setIsVolatile(true)
          .close()
          .detach();
    };
    ...
    auto blurMaker = [](SkScalar radius) -> sk_sp<SkMaskFilter> {
      return SkMaskFilter::MakeBlur(kNormal_SkBlurStyle, SkBlurMask::ConvertRadiusToSigma(radius));
    };
    ...
    if (this->getMode() == kSample_Mode) {
      ...
      constexpr SkScalar kDeltaBlurRadius = (kMaxBlurRadius - kMinBlurRadius) / kBlurRadiusSteps;
  ...
  bool onAnimate(double nanos) override {
    fAnimRadius = TimeUtils::PingPong(
        1e-9 * nanos, kRadiusPingPoingPeriod, kRadiusPingPoingShift, kMinRadius, kMaxRadius);
    fAnimBlurRadius = TimeUtils::PingPong(
        1e-9 * nanos, kBlurRadiusPingPoingPeriod, kBlurRadiusPingPoingShift, kMinBlurRadius,
        kMaxBlurRadius);
    return true;
  }
