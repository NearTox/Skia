...
static sk_sp<SkShader> make_trans_black_shader() { return make_color_shader(0x0); }
...
static sk_sp<SkShader> make_bg_shader(int checkSize) {
  ...
  SkRect rect0 = SkRect::MakeXYWH(0, 0, SkIntToScalar(checkSize), SkIntToScalar(checkSize));
  SkRect rect1 = SkRect::MakeXYWH(
      SkIntToScalar(checkSize), SkIntToScalar(checkSize), SkIntToScalar(checkSize),
...
class ModeColorFilterGM : public GM {
 public:
  ModeColorFilterGM() { this->setBGColor(0xFF303030); }

 protected:
  SkString onShortName() override { return SkString("modecolorfilters"); }
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    // used without shader
    SkColor colors[] = {
        SkColorSetARGB(0xFF, 0xFF, 0xFF, 0xFF), SkColorSetARGB(0xFF, 0x00, 0x00, 0x00),
        SkColorSetARGB(0x00, 0x00, 0x00, 0x00), SkColorSetARGB(0xFF, 0x10, 0x20, 0x42),
        ...
        for (size_t s = 0; s < SK_ARRAY_COUNT(shaders); ++s) {
          ...
          for (int pc = 0; pc < paintColorCnt; ++pc) {
            ...
            SkRect rect = SkRect::MakeXYWH(
                x * kRectWidth, y * kRectHeight, SkIntToScalar(kRectWidth),
