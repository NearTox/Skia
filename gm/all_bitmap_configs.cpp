...
static void draw_center_letter(char c, const SkFont& font, SkColor color, SkScalar x, SkScalar y,
                               SkCanvas* canvas) {
    SkRect bounds;
    font.measureText(&c, 1, SkTextEncoding::kUTF8, &bounds);
    canvas->drawSimpleText(&c, 1, SkTextEncoding::kUTF8, x - bounds.centerX(), y - bounds.centerY(),
...
template <typename T> int find(T* array, int N, T item) {
    for (int i = 0; i < N; ++i) {
        if (array[i] == item) {
            return i;
        }
