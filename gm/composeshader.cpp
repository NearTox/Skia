...
static void draw_color_bm(SkBitmap* bm, int length) {
    ...
    SkCanvas canvas(*bm);
    canvas.drawCircle(SkIntToScalar(length / 2), SkIntToScalar(length / 2),
                      SkIntToScalar(length / 2), paint);
}
...
static void draw_alpha8_bm(SkBitmap* bm, int length) {
    ...
    SkCanvas canvas(*bm);
    canvas.drawCircle(SkIntToScalar(length / 2), SkIntToScalar(length / 2),
                      SkIntToScalar(length / 4), circlePaint);
}
...
protected:
    void onOnceBeforeDraw() override {
        ...
        fAlpha8BitmapShader =
                fAlpha8Bitmap.makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, &s);
...
DEF_SIMPLE_GM(composeshader_bitmap2, canvas, 200, 200) {
    ...
    skBitmap.installPixels(imageInfo, dst32Storage.begin(), width * sizeof(int32_t), nullptr,
                           nullptr);
    imageInfo = SkImageInfo::Make(width, height, SkColorType::kAlpha_8_SkColorType,
                                  kPremul_SkAlphaType);
...
static void draw_cell(SkCanvas* canvas, sk_sp<SkShader> src, sk_sp<SkShader> dst, SkBlendMode mode,
                      SkAlpha alpha) {
