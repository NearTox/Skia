...
class ComposeShaderAlphaGM : public skiagm::GM {
 ...
 protected:
  SkString onShortName() override { return SkString("composeshader_alpha"); }

  SkISize onISize() override { return SkISize::Make(750, 220); }
...
static void draw_color_bm(SkBitmap* bm, int length) {
  ...
  SkCanvas canvas(*bm);
  canvas.drawCircle(
      SkIntToScalar(length / 2), SkIntToScalar(length / 2), SkIntToScalar(length / 2), paint);
}
...
static void draw_alpha8_bm(SkBitmap* bm, int length) {
  ...
  SkCanvas canvas(*bm);
  canvas.drawCircle(
      SkIntToScalar(length / 2), SkIntToScalar(length / 2), SkIntToScalar(length / 4), circlePaint);
}
...
class ComposeShaderBitmapGM : public skiagm::GM {
 ...
 protected:
  void onOnceBeforeDraw() override {
    ...
    s.reset();
    fColorBitmapShader =
        fColorBitmap.makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions(), s);
    fAlpha8BitmapShader =
        fAlpha8Bitmap.makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions(), s);
...
DEF_SIMPLE_GM(composeshader_bitmap2, canvas, 200, 200) {
  ...
  SkImageInfo imageInfo =
      SkImageInfo::Make(width, height, SkColorType::kN32_SkColorType, kPremul_SkAlphaType);
  skBitmap.installPixels(
      imageInfo, dst32Storage.begin(), width * sizeof(int32_t), nullptr, nullptr);
  imageInfo =
      SkImageInfo::Make(width, height, SkColorType::kAlpha_8_SkColorType, kPremul_SkAlphaType);
  ...
  paint.setShader(SkShaders::Blend(
      SkBlendMode::kSrcIn, skMaskImage->makeShader(SkSamplingOptions()),
...
static void draw_cell(
    SkCanvas* canvas, sk_sp<SkShader> src, sk_sp<SkShader> dst, SkBlendMode mode, SkAlpha alpha) {
...
static void draw_pair(
    SkCanvas* canvas, sk_sp<SkShader> src, sk_sp<SkShader> dst, SkBlendMode mode) {
