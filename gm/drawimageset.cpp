...
static void make_image_tiles(
    int tileW, int tileH, int m, int n, const SkColor colors[4], SkCanvas::ImageSetEntry set[],
    const SkColor bgColor = SK_ColorLTGRAY) {
  const int w = tileW * m;
  const int h = tileH * n;
  auto surf =
      SkSurface::MakeRaster(SkImageInfo::Make(w, h, kRGBA_8888_SkColorType, kPremul_SkAlphaType));
  ...
  for (int y = 0; y < n; ++y) {
    for (int x = 0; x < m; ++x) {
      ...
      set[y * m + x].fSrcRect = SkRect::MakeXYWH(x == 0 ? 0 : 1, y == 0 ? 0 : 1, tileW, tileH);
    ...
    for (auto fm : {SkFilterMode::kNearest, SkFilterMode::kLinear}) {
      ...
      for (size_t m = 0; m < SK_ARRAY_COUNT(matrices); ++m) {
        ...
        canvas->experimental_DrawEdgeAAImageSet(
            fSet, kM * kN, nullptr, nullptr, sampling, &setPaint,
      ...
      canvas->experimental_DrawEdgeAAImageSet(
          &entry, 1, nullptr, nullptr, sampling, &setPaint, SkCanvas::kFast_SrcRectConstraint);
      ...
      canvas->experimental_DrawEdgeAAImageSet(
          &entry, 1, nullptr, nullptr, sampling, &cfPaint, SkCanvas::kFast_SrcRectConstraint);
      canvas->restore();
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    for (SkScalar frac : {0.f, 0.5f}) {
      ...
      for (size_t m = 0; m < SK_ARRAY_COUNT(matrices); ++m) {
        ...
        canvas->concat(matrices[m]);
        canvas->experimental_DrawEdgeAAImageSet(
            fSet, kM * kN, nullptr, nullptr, SkSamplingOptions(SkFilterMode::kLinear), &paint,
            SkCanvas::kFast_SrcRectConstraint);
        canvas->restore();
    ...
    for (SkVector scale : {SkVector{2.f, 0.5f}, SkVector{0.5, 2.f}}) {
      ...
      for (size_t m = 0; m < SK_ARRAY_COUNT(matrices); ++m) {
        ...
        canvas->concat(matrices[m]);
        canvas->experimental_DrawEdgeAAImageSet(
            scaledSet, kM * kN, nullptr, nullptr, SkSamplingOptions(SkFilterMode::kLinear), &paint,
            SkCanvas::kFast_SrcRectConstraint);
        canvas->restore();
        canvas->translate(kTranslate, 0);
      }
...
class DrawImageSetAlphaOnlyGM : public GM {
 private:
  ...
  DrawResult onGpuSetup(GrDirectContext* direct, SkString*) override {
    static constexpr SkColor kColors[] = {
        SK_ColorBLUE, SK_ColorTRANSPARENT, SK_ColorRED, SK_ColorTRANSPARENT};
    ...
    for (int y = 0; y < kN; ++y) {
      for (int x = 0; x < kM; ++x) {
        ...
        if (y % 2 == 0) {
          fSet[i].fImage =
              fSet[i].fImage->makeColorTypeAndColorSpace(kAlpha_8_SkColorType, alphaSpace, direct);
        }
      }
    }
    ...
    for (int y = 0; y < kN; ++y) {
      for (int x = 0; x < kM; ++x) {
        ...
        canvas->drawImageRect(
            ToolUtils::MakeTextureImage(canvas, std::move(orig)), fSet[i].fSrcRect,
            fSet[i].fDstRect, SkSamplingOptions(), &entryPaint, SkCanvas::kFast_SrcRectConstraint);
      }
    }
  }
