...
int make_bm(SkBitmap* bm, int height) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    SkBitmap smallBM;
    SkIRect subRect = SkIRect::MakeXYWH(
        0, i * (kMargin + 2 * kRadius), 2 * kRadius + kMargin, 2 * kRadius + kMargin);
...
class TallStretchedBitmapsGM : public skiagm::GM {
 ...
 protected:
  SkString onShortName() override { return SkString("tall_stretched_bitmaps"); }

  SkISize onISize() override { return SkISize::Make(730, 690); }
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    for (size_t i = 0; i < SK_ARRAY_COUNT(fTallBmps); ++i) {
      ...
      SkIRect subRect = SkIRect::MakeLTRB(0, startItem * itemHeight, bmp.width(), bmp.height());
      SkRect dstRect = SkRect::MakeWH(SkIntToScalar(bmp.width()), 10.f * itemHeight);
      canvas->drawImageRect(
          ToolUtils::MakeTextureImage(canvas, bmp.asImage()), SkRect::Make(subRect), dstRect,
          SkSamplingOptions(SkFilterMode::kLinear), nullptr, SkCanvas::kStrict_SrcRectConstraint);
