...
class LcdTextGM : public skiagm::GM {
  ...
  void drawText(
      SkCanvas* canvas, const SkString& string, bool subpixelTextEnabled,
...
class SaveLayerPreserveLCDTextGM : public skiagm::GM {
  ...
  void onDraw(SkCanvas* canvas) override {
    drawText(
        canvas, SkString("SaveLayer PreserveLCDText"), 50,
        SkCanvas::kPreserveLCDText_SaveLayerFlag);
    drawText(canvas, SkString("SaveLayer Default (LCDText not preserved)"), 150, 0);
  }

  void drawText(
      SkCanvas* canvas, const SkString& string, int y, SkCanvas::SaveLayerFlags saveLayerFlags) {
