 ...
 protected:
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    mandoline.sliceCubic(
        {472.750000f, -71.950012f}, {639.750000f, 531.950012f}, {309.049988f, 347.950012f});
    ...
    mandoline.sliceConic(
        {-2, 0}, {-cosf(SkDegreesToRadians(60)), sinf(SkDegreesToRadians(60))}, .5f);
    mandoline.sliceConic(
        {-cosf(SkDegreesToRadians(120)) * 2, sinf(SkDegreesToRadians(120)) * 2}, {1, 0}, .5f);
    ...
    for (int i = 0; i < nquads; ++i) {
      ...
      mandoline.sliceQuadratic({cosf(theta1) * 2, sinf(theta1) * 2}, {cosf(theta2), sinf(theta2)});
    }
    canvas->drawPath(mandoline.path(), paint);
    canvas->restore();
  }
