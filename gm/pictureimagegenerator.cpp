...
static void draw_vector_logo(SkCanvas* canvas, const SkRect& viewBox) {
    ...
    const SkScalar underlineY =
            iBox.bottom() + (kVerticalSpacing + SkScalarSqrt(3) / 2) * accentSize;
    SkMatrix m;
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        for (size_t i = 0; i < SK_ARRAY_COUNT(configs); ++i) {
            ...
            std::unique_ptr<SkImageGenerator> gen = SkImageGenerator::MakeFromPicture(
                    configs[i].size, fPicture, &m, p.getAlpha() != 255 ? &p : nullptr,
            ...
            p.setAlphaf(1.0f);
            canvas->drawRect(
                    SkRect::MakeXYWH(x, y, SkIntToScalar(bm.width()), SkIntToScalar(bm.height())),
                    p);
