...
static void draw_vector_logo(SkCanvas* canvas, const SkRect& viewBox) {
  ...
  canvas->drawCircle(
      iBox.centerX(), iBox.y() - (0.5f + kVerticalSpacing) * accentSize, accentSize / 2, paint);
  ...
  SkRect underlineRect = SkRect::MakeLTRB(
      iBox.centerX() - iBox.width() * accentSize * 3, underlineY, iBox.centerX(),
      underlineY + accentSize / 10);
  const SkPoint pts1[] = {SkPoint::Make(underlineRect.x(), 0), SkPoint::Make(iBox.centerX(), 0)};
  ...
  paint.setShader(
      SkGradientShader::MakeLinear(pts2, colors2, pos2, SK_ARRAY_COUNT(pos2), SkTileMode::kClamp));
...
class PictureGeneratorGM : public skiagm::GM {
 protected:
  SkString onShortName() override { return SkString("pictureimagegenerator"); }

  SkISize onISize() override { return SkISize::Make(1160, 860); }
  ...
  void onDraw(SkCanvas* canvas) override {
    const struct {
      ...
      SkScalar opacity;
        ...
        {SkISize::Make(200, 100), 1, 1, 1},       {SkISize::Make(200, 200), 1, 1, 1},
        {SkISize::Make(200, 200), 1, 2, 1},       {SkISize::Make(400, 200), 2, 2, 1},

        {SkISize::Make(200, 100), 1, 1, 0.9f},    {SkISize::Make(200, 200), 1, 1, 0.75f},
        {SkISize::Make(200, 200), 1, 2, 0.5f},    {SkISize::Make(400, 200), 2, 2, 0.25f},

        {SkISize::Make(200, 200), 0.5f, 1, 1},    {SkISize::Make(200, 200), 1, 0.5f, 1},
        {SkISize::Make(200, 200), 0.5f, 0.5f, 1}, {SkISize::Make(200, 200), 2, 2, 1},

        {SkISize::Make(200, 100), -1, 1, 1},      {SkISize::Make(200, 100), 1, -1, 1},
        {SkISize::Make(200, 100), -1, -1, 1},     {SkISize::Make(200, 100), -1, -1, 0.5f},
    };
    ...
    for (size_t i = 0; i < SK_ARRAY_COUNT(configs); ++i) {
      ...
      std::unique_ptr<SkImageGenerator> gen = SkImageGenerator::MakeFromPicture(
          configs[i].size, fPicture, &m, p.getAlpha() != 255 ? &p : nullptr, SkImage::BitDepth::kU8,
          srgbColorSpace);
      ...
      p.setAlphaf(1.0f);
      canvas->drawRect(
          SkRect::MakeXYWH(x, y, SkIntToScalar(bm.width()), SkIntToScalar(bm.height())), p);
