...
class EncodeColorTypesGM : public GM {
 public:
  EncodeColorTypesGM(SkEncodedImageFormat format, int quality, Variant variant, const char* name)
      : fFormat(format), fQuality(quality), fVariant(variant), fName(name) {}
  ...
  DrawResult onDraw(SkCanvas* canvas, SkString* errorMsg) override {
    ...
    switch (fVariant) {
      ...
      case Variant::kOpaque:
        if (colorType != kRGB_565_SkColorType && colorType != kRGB_888x_SkColorType &&
            colorType != kRGB_101010x_SkColorType && colorType != kBGR_101010x_SkColorType) {
          return DrawResult::kSkip;
        }
        break;
      case Variant::kNormal:
        if (colorType != kARGB_4444_SkColorType && colorType != kRGBA_8888_SkColorType &&
            colorType != kBGRA_8888_SkColorType && colorType != kRGBA_1010102_SkColorType &&
            colorType != kBGRA_1010102_SkColorType && colorType != kRGBA_F16Norm_SkColorType &&
            colorType != kRGBA_F16_SkColorType && colorType != kRGBA_F32_SkColorType) {
          return DrawResult::kSkip;
        }
        break;
    }
    ...
    for (SkAlphaType alphaType : alphaTypes) {
      ...
      auto decoded = src ? SkImage::MakeFromEncoded(src->encodeToData(fFormat, fQuality)) : nullptr;
      if (!src || !decoded) {
        break;
      }
