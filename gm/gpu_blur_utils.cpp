  ...
  for (const auto& dstRectSet : dstRectSets) {
    for (int t = 0; t < kSkTileModeCount; ++t) {
      ...
      if (ref) {
        ...
        refSrc = GrRenderTargetContext::Make(
            ctx, GrColorType::kRGBA_8888, nullptr, SkBackingFit::kApprox, refRect.size());
      ...
      for (const auto& dstRect : dstRectSet) {
        ...
        if (auto blurView =
                blur(ctx, blurSrc, blurDstRect, blurSrcRect, sigmaX, sigmaY, blurMode)) {
          auto fp =
