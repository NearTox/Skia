...
static GrSurfaceProxyView blur(
    GrRecordingContext* ctx, GrSurfaceProxyView src, SkIRect dstB, SkIRect srcB, float sigmaX,
    float sigmaY, SkTileMode mode) {
  auto resultSDC = SkGpuBlurUtils::GaussianBlur(
      ctx, src, GrColorType::kRGBA_8888, kPremul_SkAlphaType, nullptr, dstB, srcB, sigmaX, sigmaY,
      mode);
  if (!resultSDC) {
    return {};
  }
...
static GrSurfaceProxyView slow_blur(
    GrRecordingContext* rContext, GrSurfaceProxyView src, SkIRect dstB, SkIRect srcB, float sigmaX,
    float sigmaY, SkTileMode mode) {
  auto tileInto = [rContext](
                      GrSurfaceProxyView src, SkIRect srcTileRect, SkISize resultSize,
                      SkIPoint offset, SkTileMode mode) {
    ...
    auto fp = GrTextureEffect::MakeSubset(
        src, kPremul_SkAlphaType, SkMatrix::Translate(-offset.x(), -offset.y()), sampler,
        SkRect::Make(srcTileRect), *rContext->priv().caps());
  ...
  while (sigmaX || sigmaY) {
    ...
    auto sdc = SkGpuBlurUtils::GaussianBlur(
        rContext, std::move(src), GrColorType::kRGBA_8888, kPremul_SkAlphaType, nullptr, bounds,
        bounds, stepX, stepY, SkTileMode::kClamp);
    if (!sdc) {
      return {};
    }
...
GrSurfaceProxyView make_src_image(
    GrRecordingContext* rContext, SkISize dimensions, const SkIRect* contentArea = nullptr) {
...
static GM::DrawResult run(
    GrRecordingContext* rContext, SkCanvas* canvas, SkString* errorMsg, bool subsetSrc, bool ref) {
  ...
  if (subsetSrc) {
    srcRect = SkIRect::MakeXYWH(
        2.f * srcRect.width() / 8.f, 1.f * srcRect.height() / 8.f, 5.f * srcRect.width() / 8.f,
  ...
  const std::vector<SkIRect> dstRectSets[] = {
      ...
      {
          SkIRect::MakeXYWH(
              -6.f * srcW / 8.f, -7.f * srcH / 8.f, 4.f * srcW / 8.f, 20.f * srcH / 8.f)
              .makeOffset(srcRect.topLeft()),
          SkIRect::MakeXYWH(
              -1.f * srcW / 8.f, -7.f * srcH / 8.f, 16.f * srcW / 8.f, 2.f * srcH / 8.f)
              .makeOffset(srcRect.topLeft()),
          SkIRect::MakeXYWH(
              10.f * srcW / 8.f, -3.f * srcH / 8.f, 4.f * srcW / 8.f, 16.f * srcH / 8.f)
              .makeOffset(srcRect.topLeft()),
          SkIRect::MakeXYWH(
              -7.f * srcW / 8.f, 14.f * srcH / 8.f, 18.f * srcW / 8.f, 1.f * srcH / 8.f)
              .makeOffset(srcRect.topLeft()),
      },
      ...
      {
          SkIRect::MakeXYWH(
              -5.f * srcW / 8.f, -5.f * srcH / 8.f, 2.f * srcW / 8.f, 2.f * srcH / 8.f)
              .makeOffset(srcRect.topLeft()),
          SkIRect::MakeXYWH(
              -5.f * srcW / 8.f, 12.f * srcH / 8.f, 2.f * srcW / 8.f, 2.f * srcH / 8.f)
              .makeOffset(srcRect.topLeft()),
          SkIRect::MakeXYWH(
              12.f * srcW / 8.f, -5.f * srcH / 8.f, 2.f * srcW / 8.f, 2.f * srcH / 8.f)
              .makeOffset(srcRect.topLeft()),
          SkIRect::MakeXYWH(
              12.f * srcW / 8.f, 12.f * srcH / 8.f, 2.f * srcW / 8.f, 2.f * srcH / 8.f)
              .makeOffset(srcRect.topLeft()),
      },
  };
  ...
  for (const auto& dstRectSet : dstRectSets) {
    for (int t = 0; t < kSkTileModeCount; ++t) {
      ...
      // on top.
      {
        ...
        auto fp = GrTextureEffect::MakeSubset(
            src, kPremul_SkAlphaType, SkMatrix::I(), sampler, SkRect::Make(srcRect), caps);
        fp = GrFragmentProcessor::ModulateRGBA(std::move(fp), {kAlpha, kAlpha, kAlpha, kAlpha});
        GrPaint paint;
      ...
      for (const auto& dstRect : dstRectSet) {
        ...
        if (auto blurView = blurFn(rContext, src, dstRect, srcRect, sigmaX, sigmaY, mode)) {
          auto fp =
              GrTextureEffect::Make(blurView, kPremul_SkAlphaType, SkMatrix::I(), sampler, caps);
          // Compose against white (default paint color)
          fp = GrBlendFragmentProcessor::Make<SkBlendMode::kSrcOver>(
              std::move(fp),
              /*dst=*/nullptr);
          GrPaint paint;
          ...
          sdc->fillRectToRect(
              nullptr, std::move(paint), GrAA::kNo, m, SkRect::Make(dstRect),
...
static DrawResult do_very_large_blur_gm(
    GrRecordingContext* rContext, SkCanvas* canvas, SkString* errorMsg, GrSurfaceProxyView src,
    SkIRect srcB) {
  auto sdc = SkCanvasPriv::TopDeviceSurfaceDrawContext(canvas);
  if (!sdc) {
    *errorMsg = GM::kErrorMsg_DrawSkippedGpuOnly;
      ...
      for (float sigma : {0.f, 5.f, 25.f, 80.f}) {
        ...
        for (auto b : blurs) {
          ...
          sdc->drawRect(
              ...
              SkRect::Make(dstRect).makeOutset(0.5, 0.5), &GrStyle::SimpleHairline());
          if (result) {
            ...
            fp = GrBlendFragmentProcessor::Make<SkBlendMode::kSrcOver>(
                std::move(fp),
                /*dst=*/nullptr);
            sdc->fillRectToRectWithFP(SkIRect::MakeSize(dstB.size()), dstRect, std::move(fp));
          }
          x += dstB.width() + 10;
        }
      }
...
DEF_SIMPLE_GPU_GM_CAN_FAIL(
    very_large_sigma_gpu_blur_subset, rContext, canvas, errorMsg, 350, 1030) {
...
DEF_SIMPLE_GPU_GM_CAN_FAIL(
    very_large_sigma_gpu_blur_subset_transparent_border, rContext, canvas, errorMsg, 355, 1055) {
