...
static GrSurfaceProxyView blur(
  ...
  auto resultRTC = SkGpuBlurUtils::GaussianBlur(
  ...
  for (const auto& dstRectSet : dstRectSets) {
    for (int t = 0; t < kSkTileModeCount; ++t) {
      ...
      if (ref) {
        ...
        refSrc = GrSurfaceDrawContext::Make(
            rContext, GrColorType::kRGBA_8888, nullptr, SkBackingFit::kApprox, refRect.size());
        ...
        auto fp = GrTextureEffect::MakeSubset(
            src, kPremul_SkAlphaType, tm, sampler, SkRect::Make(srcRect), caps);
        GrPaint paint;
        paint.setColorFragmentProcessor(std::move(fp));
        refSrc->drawRect(
            nullptr, std::move(paint), GrAA::kNo, SkMatrix::I(), SkRect::Make(refRect.size()));
      }
      ...
      for (const auto& dstRect : dstRectSet) {
        ...
        if (auto blurView =
                blur(rContext, blurSrc, blurDstRect, blurSrcRect, sigmaX, sigmaY, blurMode)) {
          auto fp =
          ...
          // (SkBlendMode::kSrc).
          fp =
              GrBlendFragmentProcessor::Make(std::move(fp), /*dst=*/nullptr, SkBlendMode::kSrcOver);
          paint.setColorFragmentProcessor(std::move(fp));
