...
static GrSurfaceProxyView blur(
    GrContext* ctx, GrSurfaceProxyView src, SkIRect dstB, SkIRect srcB, float sigmaX, float sigmaY,
    SkTileMode mode) {
    ...
    for (int t = 0; t < kSkTileModeCount; ++t) {
      ...
      for (const auto& dstRect : dstRectSet) {
        ...
        if (auto blurView =
          ...
          // (SkBlendMode::kSrc).
          fp = GrXfermodeFragmentProcessor::MakeFromSrcProcessor(
              std::move(fp), SkBlendMode::kSrcOver);
          paint.addColorFragmentProcessor(std::move(fp));
