...
static sk_sp<SkShader> make_shader0(int w, int h) {
  ...
  SkColor colors[] = {SK_ColorBLACK, SK_ColorGREEN, SK_ColorCYAN, SK_ColorRED, 0,
...
static sk_sp<SkShader> make_shader1(int w, int h) {
  ...
  return SkGradientShader::MakeRadial(
      SkPoint::Make(cx, cy), cx, colors, nullptr, SK_ARRAY_COUNT(colors), SkTileMode::kClamp);
}
...
static sk_sp<SkColorFilter> make_null_cf() { return nullptr; }
  ...
  virtual void onDraw(SkCanvas* canvas) {
    ...
    for (size_t bitmapMaker = 0; bitmapMaker < SK_ARRAY_COUNT(gBitmapMakers); ++bitmapMaker) {
      ...
      for (unsigned i = 0; i < SK_ARRAY_COUNT(gColorFilterMakers); ++i) {
        ...
        sk_sp<SkImageFilter> imageFilter1(
            SkColorFilterImageFilter::Make(std::move(colorFilter1), nullptr));
        ...
        for (unsigned j = 1; j < SK_ARRAY_COUNT(gColorFilterMakers); ++j) {
          ...
          sk_sp<SkImageFilter> imageFilter2(
              SkColorFilterImageFilter::Make(std::move(colorFilter2), imageFilter1, nullptr));
