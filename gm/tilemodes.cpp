...
static void makebm(SkBitmap* bm, SkColorType ct, int w, int h) {
  ...
  paint.setShader(
      SkGradientShader::MakeLinear(pts, colors, pos, SK_ARRAY_COUNT(colors), SkTileMode::kClamp));
...
class TilingGM : public skiagm::GM {
 public:
  TilingGM(bool powerOfTwoSize) : fPowerOfTwoSize(powerOfTwoSize) {}
    ...
    for (size_t kx = 0; kx < SK_ARRAY_COUNT(gModes); kx++) {
      for (size_t ky = 0; ky < SK_ARRAY_COUNT(gModes); ky++) {
        ...
        SkTextUtils::DrawString(
            canvas, str.c_str(), x + r.width() / 2, y, font, p, SkTextUtils::kCenter_Align);
    ...
    for (size_t i = 0; i < SK_ARRAY_COUNT(gColorTypes); i++) {
      for (size_t j = 0; j < SK_ARRAY_COUNT(gFilters); j++) {
       ...
       // on SGX where resizing NPOT textures to POT textures exhibits a driver bug.
        ...
        canvas->drawString(
            SkStringPrintf("%s, %s", gConfigNames[i], gFilterNames[j]), x, y + r.height() * 2 / 3,
            font, textPaint);
...
static sk_sp<SkShader> make_grad(SkTileMode tx, SkTileMode ty) {
  ...
  int index = (int)ty;
  switch (index % 3) {
    case 0: return SkGradientShader::MakeLinear(pts, colors, nullptr, SK_ARRAY_COUNT(colors), tx);
    case 1:
      return SkGradientShader::MakeRadial(center, rad, colors, nullptr, SK_ARRAY_COUNT(colors), tx);
    case 2:
      return SkGradientShader::MakeSweep(
          center.fX, center.fY, colors, nullptr, SK_ARRAY_COUNT(colors), tx, 135, 225, 0, nullptr);
  }
  return nullptr;
}
...
class Tiling2GM : public skiagm::GM {
 ...
 public:
  Tiling2GM(ShaderProc proc, const char name[]) : fProc(proc) { fName.printf("tilemode_%s", name); }
 ...
 protected:
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    constexpr SkTileMode gModes[] = {SkTileMode::kClamp, SkTileMode::kRepeat, SkTileMode::kMirror};
    const char* gModeNames[] = {"Clamp", "Repeat", "Mirror"};
    ...
    for (size_t kx = 0; kx < SK_ARRAY_COUNT(gModes); kx++) {
      ...
      SkTextUtils::DrawString(
          canvas, str.c_str(), x + r.width() / 2, y, font, SkPaint(), SkTextUtils::kCenter_Align);
      x += r.width() * 4 / 3;
    }
    ...
    for (size_t ky = 0; ky < SK_ARRAY_COUNT(gModes); ky++) {
      ...
      SkTextUtils::DrawString(
          canvas, str.c_str(), x, y + h / 2, font, SkPaint(), SkTextUtils::kRight_Align);
  ...
  std::function<void(SkPaint*, SkTileMode, SkTileMode)> shader_procs[] = {
      ...
      [img](SkPaint* paint, SkTileMode tx, SkTileMode ty) {
        ...
        paint->setShader(SkGradientShader::MakeRadial(
            {img->width() * 0.5f, img->width() * 0.5f}, img->width() * 0.5f, colors, pos, count,
            tx));
      },
  };
