...
class AAXfermodesGM : public skiagm::GM {
 ...
 protected:
  ...
  void onOnceBeforeDraw() override {
    ...
    fOval = SkPathBuilder().moveTo(pts[0]).quadTo(pts[1], pts[2]).quadTo(pts[3], pts[0]).detach();

    fConcave = SkPathBuilder()
                   .moveTo(-radius, 0)
                   .quadTo(0, 0, 0, -radius)
                   .quadTo(0, 0, radius, 0)
                   .quadTo(0, 0, 0, radius)
    ...
    for (size_t xfermodeSet = 0; xfermodeSet < 2; xfermodeSet++) {
      ...
      if (kShape_Pass == drawingPass) {
        SkTextUtils::DrawString(
            canvas, "Src Unknown", kLabelSpacing + kShapeTypeSpacing * 1.5f + kShapeSpacing / 2,
            kSubtitleSpacing / 2 + fLabelFont.getSize() / 3, fLabelFont, SkPaint(),
            SkTextUtils::kCenter_Align);
        SkTextUtils::DrawString(
            canvas, "Src Opaque",
            kLabelSpacing + kShapeTypeSpacing * 1.5f + kShapeSpacing / 2 + kPaintSpacing,
            kSubtitleSpacing / 2 + fLabelFont.getSize() / 3, fLabelFont, SkPaint(),
            SkTextUtils::kCenter_Align);
      }
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    SkTextUtils::DrawString(
        canvas, "Porter Duff", kLabelSpacing + 4 * kShapeTypeSpacing,
        kTitleSpacing / 2 + titleFont.getSize() / 3, titleFont, SkPaint(),
        SkTextUtils::kCenter_Align);
    SkTextUtils::DrawString(
        canvas, "Advanced", kXfermodeTypeSpacing + kLabelSpacing + 4 * kShapeTypeSpacing,
        kTitleSpacing / 2 + titleFont.getSize() / 3, titleFont, SkPaint(),
        SkTextUtils::kCenter_Align);
  ...
  void drawModeName(SkCanvas* canvas, SkBlendMode mode) {
    ...
    SkTextUtils::DrawString(
        canvas, modeName, kLabelSpacing - kShapeSize / 4, fLabelFont.getSize() / 4, fLabelFont,
        SkPaint(), SkTextUtils::kRight_Align);
  }
  ...
  void setupShapePaint(SkCanvas* canvas, SkColor color, SkBlendMode mode, SkPaint* paint) {
    ...
    if (mode == SkBlendMode::kPlus) {
      ...
      if (maxSum > 255) {
        ...
        canvas->drawRect(
            {-kShapeSpacing / 2, -kShapeSpacing / 2, kShapeSpacing / 2 + 3 * kShapeTypeSpacing,
             kShapeSpacing / 2},
            dimPaint);
      }
    }
  ...
  void drawShape(SkCanvas* canvas, Shape shape, const SkPaint& paint, SkBlendMode mode) {
    ...
    switch (shape) {
      case kSquare_Shape:
        canvas->drawRect(
            {-kShapeSize / 2, -kShapeSize / 2, kShapeSize / 2, kShapeSize / 2}, shapePaint);
        break;
      ...
      case kConcave_Shape: canvas->drawPath(fConcave, shapePaint); break;

      default: SK_ABORT("Invalid shape.");
    }
  }
