...
static void install_detector_image_filter(SkPaint* drawPaint) {
  drawPaint->setImageFilter(
      SkColorFilterImageFilter::Make(make_detector_color_filter(), drawPaint->refImageFilter()));
}
...
static void draw_save_layer_draw_rect_restore_sequence(
    SkCanvas* canvas, SkColor shapeColor, InstallDetectorFunc installDetector) {
...
static void draw_save_layer_draw_bitmap_restore_sequence(
    SkCanvas* canvas, SkColor shapeColor, InstallDetectorFunc installDetector) {
  SkBitmap bitmap;
...
static void draw_svg_opacity_and_filter_layer_sequence(
    SkCanvas* canvas, SkColor shapeColor, InstallDetectorFunc installDetector) {
  SkRect targetRect(SkRect::MakeWH(SkIntToScalar(kTestRectSize), SkIntToScalar(kTestRectSize)));
...
DEF_SIMPLE_GM(recordopts, canvas, (kTestRectSize + 1) * 2, (kTestRectSize + 1) * 15) {
  ...
  for (size_t k = 0; k < SK_ARRAY_COUNT(funcs); ++k) {
    ...
    canvas->translate(SkIntToScalar(kTestRectSize) + SkIntToScalar(1), SkIntToScalar(0));
    {
      SkPictureRecorder recorder;
      drawTestSequence(
          recorder.beginRecording(SkIntToScalar(kTestRectSize), SkIntToScalar(kTestRectSize)),
