...
static sk_sp<SkShader> make_sweep(SkTileMode mode) {
    ...
    return SkGradientShader::MakeSweep(CENTER.fX, CENTER.fY, COLORS, POS, COLOR_CT, mode, SWEEP_ANG,
                                       SWEEP_ANG, 0, nullptr);
}

static sk_sp<SkShader> make_sweep_zero_ang(SkTileMode mode) {
    // Start and end angles at 0
    return SkGradientShader::MakeSweep(CENTER.fX, CENTER.fY, COLORS, POS, COLOR_CT, mode, 0.0, 0.0,
                                       0, nullptr);
}
...
static sk_sp<SkShader> make_2pt_conic_zero_rad(SkTileMode mode) {
    ...
    return SkGradientShader::MakeTwoPointConical(CENTER, 0.0, CENTER, 0.0, COLORS, POS, COLOR_CT,
                                                 mode);
}
