...
class MorphologyGM : public GM {
 public:
  MorphologyGM() { this->setBGColor(0xFF000000); }
  ...
  void onDraw(SkCanvas* canvas) override {
    struct {
      ...
      int fRadiusX, fRadiusY;
        ...
        {140, 140, 0, 0}, {140, 140, 0, 2}, {140, 140, 2, 0}, {140, 140, 2, 2}, {24, 24, 25, 25},
    };
    SkPaint paint;
      ...
      for (unsigned i = 0; i < SK_ARRAY_COUNT(samples); ++i) {
        ...
        if (j & 0x01) {
          paint.setImageFilter(
              SkErodeImageFilter::Make(samples[i].fRadiusX, samples[i].fRadiusY, nullptr, cr));
        } else {
          paint.setImageFilter(
              SkDilateImageFilter::Make(samples[i].fRadiusX, samples[i].fRadiusY, nullptr, cr));
        }
