  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    for (SkBlendMode mode :
         {SkBlendMode::kPlus /* add */, SkBlendMode::kClear, SkBlendMode::kDarken,
          ...
          SkBlendMode::kSrcIn, SkBlendMode::kSrcOut, SkBlendMode::kSrcOver, SkBlendMode::kXor}) {
      int saveCount = canvas->save();
      this->drawTile(canvas, xOffset, yOffset, mode);
      canvas->restoreToCount(saveCount);

      SkTextUtils::DrawString(
          canvas, SkBlendMode_Name(mode), xOffset + kBitmapSize / 2.0f, yOffset + kBitmapSize, font,
