...
static sk_sp<SkSurface> make_surface(
    GrRecordingContext* ctx, const SkImageInfo& info, SkPixelGeometry geo) {
...
static void test_draw(SkCanvas* canvas, const char label[]) {
  ...
  SkTextUtils::DrawString(canvas, label, W / 2, H * 3 / 4, font, paint, SkTextUtils::kCenter_Align);
}
...
namespace {
enum SurfaceType { kManaged, kBackendTexture, kBackendRenderTarget };
}
...
static sk_sp<SkSurface> make_surface(const SkImageInfo& ii, SkCanvas* canvas, SurfaceType type) {
  ...
  switch (type) {
    case kManaged: return ToolUtils::makeSurface(canvas, ii);
    ...
    case kBackendRenderTarget:
      return sk_gpu_test::MakeBackendRenderTargetSurface(direct, ii, kTopLeft_GrSurfaceOrigin, 1);
  }
  return nullptr;
}
...
#define DEF_BET_SURFACE_TEST(name, canvas, main, W, H) \
  DEF_BACKEND_SURFACE_TEST(                            \
      SK_MACRO_CONCAT(name, _bet), canvas, main, SurfaceType::kBackendTexture, W, H)

#define DEF_BERT_SURFACE_TEST(name, canvas, main, W, H) \
  DEF_BACKEND_SURFACE_TEST(                             \
      SK_MACRO_CONCAT(name, _bert), canvas, main, SurfaceType::kBackendRenderTarget, W, H)
...
DEF_SIMPLE_GM(snap_with_mips, canvas, 80, 75) {
  ...
  auto ii =
      SkImageInfo::Make({32, 32}, ct, kPremul_SkAlphaType, canvas->imageInfo().refColorSpace());
  ...
  auto nextImage = [&](SkColor color) {
    ...
    surface->getCanvas()->drawRect(
        SkRect::MakeLTRB(
            surface->width() * 2 / 5.f, surface->height() * 2 / 5.f, surface->width() * 3 / 5.f,
...
DEF_SURFACE_TESTS(surface_underdraw, canvas, 256, 256) {
  ...
  // restore the original strip, drawing it "under" the current foreground
  {
    ...
    surf->getCanvas()->drawImage(
        saveImg, SkIntToScalar(subset.left()), SkIntToScalar(subset.top()), SkSamplingOptions(),
        &paint);
  }
