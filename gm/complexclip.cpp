...
protected:
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        if (fDoSaveLayer) {
            ...
            // scale and translate
            SkRect bounds = SkRect::MakeLTRB(4.0f / 3.0f * -20,
                                             4.0f / 3.0f * -20,
        ...
        for (int invBits = 0; invBits < 4; ++invBits) {
            ...
            for (size_t op = 0; op < SK_ARRAY_COUNT(gOps); ++op) {
                ...
                canvas->drawSimpleText(aTxt, strlen(aTxt), SkTextEncoding::kUTF8, txtX, 220, font,
                                       paint);
                ...
                canvas->drawSimpleText(bTxt, strlen(bTxt), SkTextEncoding::kUTF8, txtX, 220, font,
                                       paint);
...
private:
    void drawHairlines(SkCanvas* canvas, const SkPath& path, const SkPath& clipA,
                       const SkPath& clipB) {
        SkPaint paint;
