...
class ComplexClipGM : public GM {
 public:
  ComplexClipGM(bool aaclip, bool saveLayer, bool invertDraw)
      : fDoAAClip(aaclip), fDoSaveLayer(saveLayer), fInvertDraw(invertDraw) {
 ...
 protected:
  SkString onShortName() override {
    SkString str;
    str.printf(
        "complexclip_%s%s%s", fDoAAClip ? "aa" : "bw", fDoSaveLayer ? "_layer" : "",
        fInvertDraw ? "_invert" : "");
    return str;
  }
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    if (fDoSaveLayer) {
      ...
      SkRect bounds = SkRect::MakeLTRB(
          4.0f / 3.0f * -20, 4.0f / 3.0f * -20, 4.0f / 3.0f * (this->getISize().fWidth - 20),
    ...
    for (int invBits = 0; invBits < 4; ++invBits) {
      ...
      for (size_t op = 0; op < SK_ARRAY_COUNT(gOps); ++op) {
        ...
        canvas->save();
        // set clip
        clipA.setFillType(doInvA ? SkPathFillType::kInverseEvenOdd : SkPathFillType::kEvenOdd);
        clipB.setFillType(doInvB ? SkPathFillType::kInverseEvenOdd : SkPathFillType::kEvenOdd);
        ...
        canvas->drawSimpleText(
            gOps[op].fName, strlen(gOps[op].fName), SkTextEncoding::kUTF8, txtX, 220, font, paint);
 ...
 private:
  void drawHairlines(
      SkCanvas* canvas, const SkPath& path, const SkPath& clipA, const SkPath& clipB) {
    SkPaint paint;
...
DEF_SIMPLE_GM(clip_shader_nested, canvas, 256, 256) {
  ...
  auto s = SkGradientShader::MakeRadial(
      {0.5f * w, 0.5f * h}, 0.1f * w, gradColors, nullptr, 2, SkTileMode::kRepeat, 0, nullptr);
...
enum ConcatPerspective { kConcatBeforeClips, kConcatAfterClips, kConcatBetweenClips };
...
enum LocalMatrix { kNoLocalMat, kImageWithLocalMat, kGradientWithLocalMat, kBothWithLocalMat };
struct Config {
...
static void draw_banner(SkCanvas* canvas, Config config) {
  ...
  if (config.fConcat == kConcatBeforeClips || config.fLM == kBothWithLocalMat) {
    ...
    SkASSERT(
        (config.fConcat == kConcatBetweenClips && config.fLM == kNoLocalMat) ||
        (config.fConcat == kConcatAfterClips &&
         (config.fLM == kImageWithLocalMat || config.fLM == kGradientWithLocalMat)));
...
DEF_SIMPLE_GM(clip_shader_persp, canvas, 1370, 1030) {
  ...
  Config matches[][2] = {// Everything has perspective
  ...
  auto drawConfig = [&](Config config) {
    ...
    const SkColor gradColors[] = {SK_ColorBLACK, SkColorSetARGB(128, 128, 128, 128)};
    auto gradShader = SkGradientShader::MakeRadial(
