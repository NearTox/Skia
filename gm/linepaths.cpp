...
static void drawPath(SkPath& path, SkCanvas* canvas, SkColor color, const SkRect& clip,
                     SkPaint::Cap cap, SkPaint::Join join, SkPaint::Style style,
                     SkPath::FillType fill, SkScalar strokeWidth) {
    ...
    for (size_t cap = 0; cap < SK_ARRAY_COUNT(gCaps); ++cap) {
        ...
        for (size_t fill = 0; fill < SK_ARRAY_COUNT(gFills); ++fill) {
            ...
            for (size_t style = 0; style < SK_ARRAY_COUNT(gStyles); ++style) {
                ...
                drawPath(path.fPath, canvas, color, rect, gCaps[cap].fCap, gCaps[cap].fJoin,
                ...
                font.setSize(10);
                canvas->drawString(gStyles[style].fName, 0, rect.height() + 12.0f, font,
                                   labelPaint);
                canvas->drawString(gFills[fill].fName, 0, rect.height() + 24.0f, font, labelPaint);
                canvas->drawString(gCaps[cap].fName, 0, rect.height() + 36.0f, font, labelPaint);
            }
            canvas->restore();
        }
        canvas->restore();
...
DEF_SIMPLE_GM(linepath, canvas, 1240, 390) { draw(canvas, false); }
DEF_SIMPLE_GM(lineclosepath, canvas, 1240, 390) { draw(canvas, true); }
