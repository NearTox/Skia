...
static void getGlyphPositions(const SkFont& font, const uint16_t glyphs[], int count, SkScalar x,
                              SkScalar y, SkPoint pos[]) {
...
static void drawKernText(SkCanvas* canvas, const void* text, size_t len, SkScalar x, SkScalar y,
...
static void draw_typeface_rendering_gm(SkCanvas* canvas, sk_sp<SkTypeface> face,
    ...
    struct AliasType {
        ...
        bool inLayer;
            ...
            // This gm crashes on iOS when drawing an embedded bitmap when requesting aliased
            // rendering.
                    ...
                    for (const SkFontHinting& hinting : hintingTypes) {
                        ...
                        for (const bool& rotateABit : rotateABitTypes) {
                            ...
                            canvas->drawSimpleText(&character, 1, SkTextEncoding::kUTF8,
                                                   x + subpixel.offset.x(), y + subpixel.offset.y(),
                                                   font, paint);

                            SkScalar dx = SkScalarCeilToScalar(font.measureText(
                                                  &character, 1, SkTextEncoding::kUTF8)) +
                                          5;
                            x += dx;
                            xMax = SkTMax(x, xMax);
                        }
            ...
            for (const AliasType& alias : aliasTypes) {
                ...
                for (const StyleTests& style : styleTypes) {
                    ...
                    SkScalar dx = SkScalarCeilToScalar(
                                          font.measureText(&character, 1, SkTextEncoding::kUTF8)) +
                                  5;
                    x += dx;
                }
            }
