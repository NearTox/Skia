...
static void draw_typeface_rendering_gm(
    SkCanvas* canvas, sk_sp<SkTypeface> face, char character = 'A') {
  struct AliasType {
          ...
          for (const SkFontHinting& hinting : hintingTypes) {
            ...
            for (const bool& rotateABit : rotateABitTypes) {
              ...
              canvas->drawSimpleText(
                  &character, 1, SkTextEncoding::kUTF8, x + subpixel.offset.x(),
                  y + subpixel.offset.y(), font, paint);

              SkScalar dx =
                  SkScalarCeilToScalar(font.measureText(&character, 1, SkTextEncoding::kUTF8)) + 5;
              x += dx;
              xMax = std::max(x, xMax);
            }
          }
