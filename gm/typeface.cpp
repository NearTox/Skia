...
static void getGlyphPositions(
    const SkFont& font, const uint16_t glyphs[], int count, SkScalar x, SkScalar y, SkPoint pos[]) {
...
static void applyKerning(
    SkPoint pos[], const int32_t adjustments[], int count, const SkFont& font) {
...
static void drawKernText(
    SkCanvas* canvas, const void* text, size_t len, SkScalar x, SkScalar y, const SkFont& font,
    const SkPaint& paint) {
...
static void draw_typeface_rendering_gm(SkCanvas* canvas, sk_sp<SkTypeface> face, SkGlyphID glyph) {
...
#endif
      {SkFont::Edging::kAntiAlias, false}, {SkFont::Edging::kSubpixelAntiAlias, false},
      {SkFont::Edging::kAntiAlias, true},  {SkFont::Edging::kSubpixelAntiAlias, true},
  };
        ...
        for (const SkScalar& textSize : textSizes) {
          ...
          for (const SkFontHinting& hinting : hintingTypes) {
            ...
            for (const bool& rotateABit : rotateABitTypes) {
              ...
              if (rotateABit) {
                canvas->rotate(2, x + subpixel.offset.x(), y + subpixel.offset.y());
              }
              canvas->drawSimpleText(
                  &glyph, sizeof(glyph), SkTextEncoding::kGlyphID, x + subpixel.offset.x(),
  ...
  constexpr struct MaskTests {
    ...
    SkScalar sigma;
      ...
      {SkBlurStyle::kNormal_SkBlurStyle, 0.0f}, {SkBlurStyle::kSolid_SkBlurStyle, 0.0f},
      {SkBlurStyle::kOuter_SkBlurStyle, 0.0f},  {SkBlurStyle::kInner_SkBlurStyle, 0.0f},

      {SkBlurStyle::kNormal_SkBlurStyle, 0.5f}, {SkBlurStyle::kSolid_SkBlurStyle, 0.5f},
      {SkBlurStyle::kOuter_SkBlurStyle, 0.5f},  {SkBlurStyle::kInner_SkBlurStyle, 0.5f},

      {SkBlurStyle::kNormal_SkBlurStyle, 2.0f}, {SkBlurStyle::kSolid_SkBlurStyle, 2.0f},
...
DEF_SIMPLE_GM(typeface_styling, canvas, 710, 360) {
  ...
  const struct {
    ...
    bool usePE;
      ...
      {SkPaint::kFill_Style, 0, false},          {SkPaint::kStroke_Style, 0, false},
      {SkPaint::kStroke_Style, 3, false},        {SkPaint::kStrokeAndFill_Style, 0, false},
      {SkPaint::kStrokeAndFill_Style, 3, false}, {SkPaint::kStroke_Style, 0, true},
