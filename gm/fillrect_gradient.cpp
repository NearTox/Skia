...
class FillrectGradientGM : public skiagm::GM {
 ...
 protected:
  ...
  SkString onShortName() override { return SkString("fillrect_gradient"); }

  SkISize onISize() override {
    return SkISize::Make(kNumColumns * (kCellSize + kPadSize), kNumRows * (kCellSize + kPadSize));
  }
  ...
  void drawGradient(SkCanvas* canvas, std::initializer_list<GradientStop> stops) {
    ...
    sk_sp<SkShader> shader = SkGradientShader::MakeLinear(
        points, colors.data(), positions.data(), colors.size(), SkTileMode::kClamp);
    SkPaint paint;
    ...
    shader = SkGradientShader::MakeRadial(
        SkPoint::Make(kCellSize / 2, kCellSize / 2), kCellSize / 2, colors.data(), positions.data(),
        colors.size(), SkTileMode::kClamp);
    paint.setShader(shader);
    canvas->drawRect(SkRect::MakeXYWH(0, 0, kCellSize, kCellSize), paint);
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    this->drawGradient(canvas, {{0.0f, SK_ColorGREEN}, {0.5f, SK_ColorWHITE}, {1.0f, SK_ColorRED}});

    // No stops at 0.0 or 1.0: Larger green to white to larger red
    this->drawGradient(canvas, {{0.4f, SK_ColorGREEN}, {0.5f, SK_ColorWHITE}, {0.6f, SK_ColorRED}});
    ...
    this->drawGradient(
        canvas,
        {{0.0f, SK_ColorBLUE}, {0.5f, SK_ColorWHITE}, {0.5f, SK_ColorRED}, {1.0f, SK_ColorYELLOW}});

    // Ignored stops: Blue to white, red to yellow (same as previous)
    this->drawGradient(
        canvas, {{0.0f, SK_ColorBLUE},
                 {0.5f, SK_ColorWHITE},
                 {0.5f, SK_ColorGRAY},
                 {0.5f, SK_ColorCYAN},
                 {0.5f, SK_ColorRED},
