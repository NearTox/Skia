...
class PosterCircleGM : public skiagm::GM {
 ...
 protected:
  SkString onShortName() override { return SkString("poster_circle"); }

  SkISize onISize() override { return SkISize::Make(kStageWidth, kStageHeight + 50); }
  ...
  void onOnceBeforeDraw() override {
    ...
    for (int i = 0; i < kNumAngles; ++i) {
      ...
      fillPaint.setColor(
          i % 2 == 0 ? SkColorSetRGB(0x99, 0x5C, 0x7F) : SkColorSetRGB(0x83, 0x5A, 0x99));
      canvas->drawRRect(
          SkRRect::MakeRectXY(SkRect::MakeWH(kPosterSize, kPosterSize), 10.f, 10.f), fillPaint);
    ...
    for (int pass = 0; pass < 2; ++pass) {
      ...
      for (int y = 0; y < 3; ++y) {
        ...
        for (int i = 0; i < kNumAngles; ++i) {
          ...
          SkScalar yRotation = SkScalarMod(
              kAngleStep * i + 360.f * SkScalarMod(fTime / yDuration, yDuration), 360.f);
          ...
          SkM44 model = SkM44::Translate(kStageWidth / 2, kStageHeight / 2 + 25, 0) * proj *
                        SkM44::Translate(0, ringY, 0) *
                        SkM44::Rotate({0, 1, 0}, SkDegreesToRadians(yRotation)) *
                        SkM44::Translate(0, 0, kRingRadius);
          canvas->concat(model);

          SkRect poster = SkRect::MakeLTRB(
              -0.5f * kPosterSize, -0.5f * kPosterSize, 0.5f * kPosterSize, 0.5f * kPosterSize);
          SkPaint fillPaint;
          fillPaint.setAntiAlias(true);
          fillPaint.setAlphaf(0.7f);
          canvas->drawImageRect(
              fPosterImages[i], poster, SkSamplingOptions(SkFilterMode::kLinear), &fillPaint);
