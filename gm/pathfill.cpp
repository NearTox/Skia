...
static PathDY make_rect() {
  ...
  return {SkPathBuilder().addRect(r).offset(10, 0).detach(), 30};
}

static PathDY make_oval() {
  SkRect r = {SkIntToScalar(10), SkIntToScalar(10), SkIntToScalar(30), SkIntToScalar(30)};
  return {SkPathBuilder().addOval(r).offset(10, 0).detach(), 30};
}
...
class PathInverseFillGM : public skiagm::GM {
 ...
 protected:
  ...
  static void show(
      SkCanvas* canvas, const SkPath& path, const SkPaint& paint, const SkRect* clip, SkScalar top,
  ...
  void onDraw(SkCanvas* canvas) override {
    SkPath path = SkPathBuilder().addCircle(50, 50, 40).toggleInverseFillType().detach();
...
DEF_SIMPLE_GM(path_stroke_clip_crbug1070835, canvas, 25, 50) {
  ...
  SkPoint pts[] = {
      {11, 12},           {11, 18.0751324f},   {6.07513189f, 23}, {-4.80825292E-7f, 23},
      {-6.07513332f, 23}, {-11, 18.0751324f},  {-11, 11.999999f}, {-10.999999f, 5.92486763f},
      {-6.07513189f, 1},  {1.31173692E-7f, 1}, {6.07513141f, 1},  {10.9999981f, 5.92486572f},
      {11, 11.9999971f},
  };
  path.moveTo(pts[0])
