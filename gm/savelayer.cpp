...
DEF_SIMPLE_GM(picture_savelayer, canvas, 320, 640) {
  ...
  for (int i = 1; i < 2; ++i) {
    ...
    auto flag = i ? (SkCanvas::SaveLayerFlags)SkCanvasPriv::kDontClipToLayer_SaveLayerFlag : 0;
...
static void draw_mask(SkCanvas* canvas, int size) {
  const SkScalar cx = size * SK_ScalarHalf, cy = cx;
  ...
  paint.setShader(
      SkGradientShader::MakeRadial({cx, cy}, size / 4, colors, nullptr, 2, SkTileMode::kClamp));
  ...
  static const MaskMakerFunc kMaskMakers[] = {
      ...
      [](int size) -> sk_sp<SkImage> {
        ...
        return SkImage::MakeFromPicture(
            recorder.finishRecordingAsPicture(), SkISize::Make(size, size), nullptr, nullptr,
...
static void draw_cell(
    SkCanvas* canvas, sk_sp<SkTextBlob> blob, SkColor c, SkScalar w, SkScalar h,
    bool useDrawBehind) {
...
DEF_SIMPLE_GM(savelayer_f16, canvas, 900, 300) {
  ...
  paint.setShader(SkGradientShader::MakeSweep(
      r.centerX(), r.centerY(), colors, nullptr, SK_ARRAY_COUNT(colors)));
