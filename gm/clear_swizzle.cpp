...
DEF_SIMPLE_GPU_GM(clear_swizzle, ctx, rtCtx, canvas, 6 * kSize, 2 * kSize) {
  ...
  auto make_offscreen = [&](const SkISize dimensions) {
    ...
    return GrSurfaceFillContext::Make(
        ctx, kPremul_SkAlphaType, rtCtx->colorInfo().refColorSpace(), dimensions,
        SkBackingFit::kExact, rtCtx->asSurfaceProxy()->backendFormat(),
        /* sample count*/ 1, GrMipmapped::kNo, rtCtx->asSurfaceProxy()->isProtected(), readSwizzle,
