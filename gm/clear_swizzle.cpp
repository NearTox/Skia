...
DEF_SIMPLE_GPU_GM_CAN_FAIL(clear_swizzle, rContext, canvas, errorMsg, 6 * kSize, 2 * kSize) {
  ...
  auto make_offscreen = [&](const SkISize dimensions) {
    skgpu::Swizzle readSwizzle = skgpu::Swizzle::Concat(sfc->readSwizzle(), skgpu::Swizzle{"bgra"});
    skgpu::Swizzle writeSwizzle =
        skgpu::Swizzle::Concat(sfc->readSwizzle(), skgpu::Swizzle{"bgra"});
    return rContext->priv().makeSFC(
        kPremul_SkAlphaType, sfc->colorInfo().refColorSpace(), dimensions, SkBackingFit::kExact,
        sfc->asSurfaceProxy()->backendFormat(),
        /* sample count*/ 1, GrMipmapped::kNo, sfc->asSurfaceProxy()->isProtected(), readSwizzle,
        writeSwizzle, kTopLeft_GrSurfaceOrigin, SkBudgeted::kYes);
  };
  ...
  sfc->blitTexture(
      offscreen->readSurfaceView(), SkIRect::MakeSize({2 * kSize, 2 * kSize}),
      SkIPoint{2 * kSize, 0});
  ...
  for (const auto& c : clears) {
    ...
    sfc->blitTexture(
        offscreen->readSurfaceView(), SkIRect::MakeSize(offscreen->dimensions()),
