...
DEF_SIMPLE_GM(analytic_antialias_convex, canvas, W, H) {
  ...
  path.cubicTo(
      SkBits2Float(0x429b9d71), SkBits2Float(0x4367a022), SkBits2Float(0x429b9d64),
  ...
  path.conicTo(
      SkBits2Float(0x4348ebaf), SkBits2Float(0x43981a75), SkBits2Float(0x4344f079),
...
DEF_SIMPLE_GM(analytic_antialias_general, canvas, W, H) {
  ...
  canvas->drawPath(
      SkPathBuilder().addRect({20, 20, 100.4999f, 100}).addRect({100.5001f, 20, 200, 100}).detach(),
      p);

  canvas->translate(300, 0);
  canvas->drawPath(
      SkPathBuilder().addRect({20, 20, 100.1f, 100}).addRect({100.9f, 20, 200, 100}).detach(), p);
}
