...
static void draw_text_on_path(
    SkCanvas* canvas, const void* text, size_t length, const SkPoint xy[], const SkPath& path,
    const SkFont& font, const SkPaint& paint, float baseline_offset) {
  ...
  canvas->drawTextBlob(
      SkTextBlob::MakeFromRSXform(
          glyphs.get(), count * sizeof(SkGlyphID), &xform[0], font, SkTextEncoding::kGlyphID),
      0, 0, paint);
