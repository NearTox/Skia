...
static sk_sp<SkColorFilter> MakeTintColorFilter(SkColor lo, SkColor hi) {
  const auto r_lo = SkColorGetR(lo), g_lo = SkColorGetG(lo), b_lo = SkColorGetB(lo),
             a_lo = SkColorGetA(lo), r_hi = SkColorGetR(hi), g_hi = SkColorGetG(hi),
             b_hi = SkColorGetB(hi), a_hi = SkColorGetA(hi);
...
class MixerCFGM final : public skiagm::GM {
 public:
  MixerCFGM(const SkSize& tileSize, size_t tileCount)
      : fTileSize(tileSize), fTileCount(tileCount) {}

 protected:
  SkString onShortName() override { return SkString("mixerCF"); }

  SkISize onISize() override {
    return SkISize::Make(
        fTileSize.width() * 1.2f * fTileCount,
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    paint.setShader(SkGradientShader::MakeSweep(
        fTileSize.width() / 2, fTileSize.height() / 2, gradient_colors, nullptr,
 ...
 private:
  ...
  void mixRow(
      SkCanvas* canvas, SkPaint& paint, sk_sp<SkColorFilter> cf0, sk_sp<SkColorFilter> cf1) {
...
static sk_sp<SkShader> make_grad(int size, float t) {
  ...
  return SkGradientShader::MakeLinear(pts, colors, pos, SK_ARRAY_COUNT(colors), SkTileMode::kClamp);
}
...
class ShaderMixerGM final : public skiagm::GM {
 ...
 protected:
  SkString onShortName() override { return SkString("mixershader_shadermixer"); }
