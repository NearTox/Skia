...
static sk_sp<SkShader> MakeLinear4f(const SkPoint pts[2], const GradData& data, SkTileMode tm,
                                    const SkMatrix& localMatrix) {
...
static sk_sp<SkShader> MakeRadial4f(const SkPoint pts[2], const GradData& data, SkTileMode tm,
                                    const SkMatrix& localMatrix) {
    SkPoint center;
    center.set(SkScalarAve(pts[0].fX, pts[1].fX), SkScalarAve(pts[0].fY, pts[1].fY));
    auto srgb = SkColorSpace::MakeSRGB();
...
static sk_sp<SkShader> MakeSweep4f(const SkPoint pts[2], const GradData& data, SkTileMode,
                                   const SkMatrix& localMatrix) {
    SkPoint center;
    center.set(SkScalarAve(pts[0].fX, pts[1].fX), SkScalarAve(pts[0].fY, pts[1].fY));
    auto srgb = SkColorSpace::MakeSRGB();
...
static sk_sp<SkShader> Make2Radial(const SkPoint pts[2], const GradData& data, SkTileMode tm,
    ...
    return SkGradientShader::MakeTwoPointConical(center1, (pts[1].fX - pts[0].fX) / 7, center0,
                                                 (pts[1].fX - pts[0].fX) / 2, data.fColors,
                                                 data.fPos, data.fCount, tm, 0, &localMatrix);
}

static sk_sp<SkShader> Make2Radial4f(const SkPoint pts[2], const GradData& data, SkTileMode tm,
                                     const SkMatrix& localMatrix) {
    SkPoint center0, center1;
    center0.set(SkScalarAve(pts[0].fX, pts[1].fX), SkScalarAve(pts[0].fY, pts[1].fY));
    center1.set(SkScalarInterp(pts[0].fX, pts[1].fX, SkIntToScalar(3) / 5),
    ...
    auto srgb = SkColorSpace::MakeSRGB();
    return SkGradientShader::MakeTwoPointConical(center1, (pts[1].fX - pts[0].fX) / 7, center0,
                                                 (pts[1].fX - pts[0].fX) / 2, data.fColors4f, srgb,
                                                 data.fPos, data.fCount, tm, 0, &localMatrix);
}
...
static sk_sp<SkShader> Make2Conical4f(const SkPoint pts[2], const GradData& data, SkTileMode tm,
                                      const SkMatrix& localMatrix) {
    SkPoint center0, center1;
    SkScalar radius0 = (pts[1].fX - pts[0].fX) / 10;
    SkScalar radius1 = (pts[1].fX - pts[0].fX) / 3;
    ...
    center1.set(pts[1].fX - radius1, pts[1].fY - radius1);
    auto srgb = SkColorSpace::MakeSRGB();
    return SkGradientShader::MakeTwoPointConical(center1, radius1, center0, radius0, data.fColors4f,
...
protected:
    ...
    virtual void onDraw(SkCanvas* canvas) {
        ...
        center.iset(0, 300);
        canvas->translate(SkIntToScalar(20), SkIntToScalar(20));
        paint.setShader(SkGradientShader::MakeRadial(SkPoint(center), SkIntToScalar(200), gColors,
                                                     nullptr, 5, SkTileMode::kClamp));
        canvas->drawRect(r, paint);
    }
...
protected:
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        for (size_t i = 0; i < SK_ARRAY_COUNT(flags); i++) {
            paint1.setShader(SkGradientShader::MakeSweep(cx, cy, sweep_colors, nullptr,
                                                         SK_ARRAY_COUNT(sweep_colors), flags[i],
                                                         nullptr));
            paint2.setShader(SkGradientShader::MakeRadial(center, radius, colors1, nullptr,
                                                          SK_ARRAY_COUNT(colors1),
                                                          SkTileMode::kClamp, flags[i], nullptr));
            paint3.setShader(SkGradientShader::MakeRadial(center, radius, colors2, nullptr,
...
protected:
    ...
    void onOnceBeforeDraw() override {
        ...
        for (int index = 0; index < (int)SK_ARRAY_COUNT(fShader); ++index) {
            ...
            fShader[index] = SkGradientShader::MakeLinear(pts, colors, pos, SK_ARRAY_COUNT(gColors),
                                                          SkTileMode::kClamp);
        }
    }
    ...
    void onDraw(SkCanvas* canvas) override {
        ...
        for (unsigned i = 0; i < SK_ARRAY_COUNT(configs); ++i) {
            ...
            paint.setShader(SkGradientShader::MakeLinear(configs[i].pts, colors, configs[i].pos,
                                                         kStopCount, SkTileMode::kClamp, fFlags,
                                                         nullptr));
...
static sk_sp<SkShader> make_radial(const GradRun& run, SkTileMode mode) {
    ...
    return SkGradientShader::MakeRadial({half, half}, half - 10, run.fColors, run.fPos, run.fCount,
                                        mode);
}
...
static sk_sp<SkShader> make_conical(const GradRun& run, SkTileMode mode) {
    ...
    return SkGradientShader::MakeTwoPointConical(center, 20, center, half - 10, run.fColors,
                                                 run.fPos, run.fCount, mode);
}
...
DEF_SIMPLE_GM(gradients_dup_color_stops, canvas, 704, 564) {
    ...
    sk_sp<SkShader> (*factories[])(const GradRun&, SkTileMode){make_linear, make_radial,
                                                               make_conical, make_sweep};
...
DEF_SIMPLE_GM(fancy_gradients, canvas, 800, 300) {
    draw_circle_shader(canvas, 150, 150, 100, []() -> sk_sp<SkShader> {
        ...
        SkColor colors1[] = {0xff000000, 0xff000000, 0xffffffff,
                             0xffffffff, 0xff000000, 0xff000000};
        SkColor colors2[] = {0xff000000, 0xff000000, 0x00000000,
    ...
    draw_circle_shader(canvas, 400, 150, 100, []() -> sk_sp<SkShader> {
        ...
        SkPaint p;
        p.setShader(SkGradientShader::MakeSweep(kTileSize / 2, kTileSize / 2, colors, pos,
    ...
    draw_circle_shader(canvas, 650, 150, 100, []() -> sk_sp<SkShader> {
        ...
        return SkShaders::Blend(
                ...
                SkGradientShader::MakeRadial(center, 100, colors, radialPos,
                                             SK_ARRAY_COUNT(radialPos), SkTileMode::kClamp));
    });
}
    ...
    for (auto mode : modes) {
        {
            ...
            for (auto angle : angles) {
                p.setShader(SkGradientShader::MakeSweep(size / 2, size / 2, colors, pos,
                                                        SK_ARRAY_COUNT(colors), mode, angle.start,
                                                        angle.end, 0, nullptr));
    ...
    for (const auto& cfg : configs) {
        {
            ...
            for (auto mode : modes) {
                p.setShader(
                        SkGradientShader::MakeLinear(pts, cfg.colors, cfg.pos, cfg.count, mode));
