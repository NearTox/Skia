...
protected:
    ...
    static void DrawAndFrame(SkCanvas* canvas, const SkBitmap& orig, SkScalar x, SkScalar y) {
        ...
        canvas->drawRect(
                SkRect::MakeIWH(bm.width(), bm.height()).makeOffset(x, y).makeOutset(0.5f, 0.5f),
                paint);
    }
...
protected:
    ...
    SkISize onISize() override { return {824, 862}; }
    ...
    void drawSet(SkCanvas* canvas, const SkBitmap& orig) {
        const SkColorType ctypes[] = {kN32_SkColorType, kRGB_565_SkColorType,
                                      kARGB_4444_SkColorType, kGray_8_SkColorType};
