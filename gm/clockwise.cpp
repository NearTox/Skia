...
class ClockwiseTestProcessor : public GrGeometryProcessor {
 public:
  static GrGeometryProcessor* Make(SkArenaAlloc* arena, bool readSkFragCoord) {
    return arena->make(
        [&](void* ptr) { return new (ptr) ClockwiseTestProcessor(readSkFragCoord); });
  }
 ...
 private:
  ClockwiseTestProcessor(bool readSkFragCoord)
      : GrGeometryProcessor(kClockwiseTestProcessor_ClassID), fReadSkFragCoord(readSkFragCoord) {
   ...
   private:
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
      ...
      args.fFragBuilder->codeAppendf(
          "half4 %s = sk_Clockwise ? half4(0,1,0,1) : half4(1,0,0,1);", args.fOutputColor);
      if (!proc.readSkFragCoord()) {
        args.fFragBuilder->codeAppendf("const half4 %s = half4(1);", args.fOutputCoverage);
      } else {
        // Verify layout(origin_upper_left) on gl_FragCoord does not affect gl_FrontFacing.
        args.fFragBuilder->codeAppendf(
            "half4 %s = half4(min(half(sk_FragCoord.y), 1));", args.fOutputCoverage);
      }
    }
  };
...
class ClockwiseTestOp : public GrDrawOp {
 public:
  ...
  static GrOp::Owner Make(GrRecordingContext* context, bool readSkFragCoord, int y = 0) {
 ...
 private:
  ...
  GrProgramInfo* createProgramInfo(
    ...
    return sk_gpu_test::CreateProgramInfo(
        caps, arena, writeView, usesMSAASurface, std::move(appliedClip), dstProxyView, geomProc,
        SkBlendMode::kPlus, GrPrimitiveType::kTriangleStrip, renderPassXferBarriers, colorLoadOp);
  }
  ...
  void onPrePrepare(
    ...
    fProgramInfo = this->createProgramInfo(
        context->priv().caps(), arena, writeView, usesMSAASurface, std::move(appliedClip),
  ...
  void onPrepare(GrOpFlushState* flushState) override {
    ...
    fVertexBuffer = flushState->resourceProvider()->createBuffer(
        vertices, sizeof(vertices), GrGpuBufferType::kVertex, kStatic_GrAccessPattern);
  }
...
DrawResult ClockwiseGM::onDraw(GrRecordingContext* rContext, SkCanvas* canvas, SkString* errorMsg) {
  ...
  if (auto topLeftSDC = skgpu::v1::SurfaceDrawContext::Make(
          rContext, sdcColorType, nullptr, SkBackingFit::kExact, {100, 200}, SkSurfaceProps(), 1,
    ...
    sdc->drawTexture(
        nullptr, topLeftSDC->readSurfaceView(), sdc->colorInfo().alphaType(),
        GrSamplerState::Filter::kNearest, GrSamplerState::MipmapMode::kNone, SkBlendMode::kSrcOver,
        SK_PMColor4fWHITE, {0, 0, 100, 200}, {100, 0, 200, 200}, GrQuadAAFlags::kNone,
  ...
  if (auto topLeftSDC = skgpu::v1::SurfaceDrawContext::Make(
    ...
    sdc->drawTexture(
        nullptr, topLeftSDC->readSurfaceView(), sdc->colorInfo().alphaType(),
        GrSamplerState::Filter::kNearest, GrSamplerState::MipmapMode::kNone, SkBlendMode::kSrcOver,
        SK_PMColor4fWHITE, {0, 0, 100, 200}, {200, 0, 300, 200}, GrQuadAAFlags::kNone,
        SkCanvas::SrcRectConstraint::kStrict_SrcRectConstraint, SkMatrix::I(), nullptr);
  }
