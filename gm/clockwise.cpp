...
class GLSLClockwiseTestProcessor : public GrGLSLGeometryProcessor {
  ...
  void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
    ...
    if (!proc.fReadSkFragCoord) {
      ...
      args.fFragBuilder->codeAppendf(
          "%s = half4(min(half(sk_FragCoord.y), 1));", args.fOutputCoverage);
    }
  }
};

GrGLSLPrimitiveProcessor* ClockwiseTestProcessor::createGLSLInstance(const GrShaderCaps&) const {
...
class ClockwiseTestOp : public GrDrawOp {
 public:
  ...
  static std::unique_ptr<GrDrawOp> Make(
      GrRecordingContext* context, bool readSkFragCoord, int y = 0) {
 ...
 private:
  ...
  void onExecute(GrOpFlushState* flushState, const SkRect& chainBounds) override {
    ...
    GrPipeline pipeline(
        GrScissorTest::kDisabled, SkBlendMode::kPlus, flushState->drawOpArgs().fOutputSwizzle);
    GrMesh mesh(GrPrimitiveType::kTriangleStrip);
    mesh.setNonIndexedNonInstanced(4);
    mesh.setVertexData(std::move(vertexBuffer));
    flushState->rtCommandBuffer()->draw(
        ClockwiseTestProcessor(fReadSkFragCoord), pipeline, nullptr, nullptr, &mesh, 1,
...
void ClockwiseGM::onDraw(GrContext* ctx, GrRenderTargetContext* rtc, SkCanvas* canvas) {
  ...
  if (auto topLeftRTC = ctx->priv().makeDeferredRenderTargetContext(
    ...
    rtc->drawTexture(
        GrNoClip(), sk_ref_sp(topLeftRTC->asTextureProxy()), GrSamplerState::Filter::kNearest,
        SkBlendMode::kSrcOver, SK_PMColor4fWHITE, {0, 0, 100, 200}, {100, 0, 200, 200}, GrAA::kNo,
  ...
  if (auto topLeftRTC = ctx->priv().makeDeferredRenderTargetContext(
    ...
    rtc->drawTexture(
        GrNoClip(), sk_ref_sp(topLeftRTC->asTextureProxy()), GrSamplerState::Filter::kNearest,
        SkBlendMode::kSrcOver, SK_PMColor4fWHITE, {0, 0, 100, 200}, {200, 0, 300, 200}, GrAA::kNo,
        GrQuadAAFlags::kNone, SkCanvas::SrcRectConstraint::kStrict_SrcRectConstraint, SkMatrix::I(),
        nullptr);
  }
}
