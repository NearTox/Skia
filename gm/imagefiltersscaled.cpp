...
class ImageFiltersScaledGM : public GM {
 public:
  ImageFiltersScaledGM() { this->setBGColor(0x00000000); }

 protected:
  SkString onShortName() override { return SkString("imagefiltersscaled"); }

  SkISize onISize() override { return SkISize::Make(1428, 500); }
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    noisePaint.setShader(
        SkPerlinNoiseShader::MakeFractalNoise(SkDoubleToScalar(0.1), SkDoubleToScalar(0.05), 1, 0));

    SkPoint3 pointLocation = SkPoint3::Make(0, 0, SkIntToScalar(10));
    SkPoint3 spotLocation =
        SkPoint3::Make(SkIntToScalar(-10), SkIntToScalar(-10), SkIntToScalar(20));
    ...
    sk_sp<SkImageFilter> filters[] = {
        ...
        SkDropShadowImageFilter::Make(
            SkIntToScalar(5), SkIntToScalar(10), SkIntToScalar(3), SkIntToScalar(3), SK_ColorYELLOW,
            SkDropShadowImageFilter::kDrawShadowAndForeground_ShadowMode, nullptr),
        SkDisplacementMapEffect::Make(
            SkDisplacementMapEffect::kR_ChannelSelectorType,
            SkDisplacementMapEffect::kR_ChannelSelectorType, SkIntToScalar(12), std::move(gradient),
            checkerboard),
        ...
        SkLightingImageFilter::MakePointLitDiffuse(pointLocation, white, surfaceScale, kd, nullptr),
        SkLightingImageFilter::MakeSpotLitDiffuse(
            spotLocation, spotTarget, spotExponent, cutoffAngle, white, surfaceScale, kd, nullptr),
    };
    ...
    for (size_t j = 0; j < SK_ARRAY_COUNT(scales); ++j) {
      ...
      for (size_t i = 0; i < SK_ARRAY_COUNT(filters); ++i) {
        ...
        if (5 == i) {
          ...
          canvas->scale(SkScalarInvert(RESIZE_FACTOR), SkScalarInvert(RESIZE_FACTOR));
        }
