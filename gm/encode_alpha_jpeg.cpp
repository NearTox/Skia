...
static inline void read_into_pixmap(
    SkPixmap* dst, SkImageInfo dstInfo, void* dstPixels, sk_sp<SkImage> src) {
  dst->reset(dstInfo, dstPixels, dstInfo.minRowBytes());
  src->readPixels(nullptr, *dst, 0, 0, SkImage::CachingHint::kDisallow_CachingHint);
}

static inline sk_sp<SkImage> encode_pixmap_and_make_image(
    const SkPixmap& src, SkJpegEncoder::AlphaOption alphaOption) {
...
class EncodeJpegAlphaOptsGM : public GM {
 ...
 protected:
  SkString onShortName() override { return SkString("encode-alpha-jpeg"); }
  ...
  DrawResult onDraw(SkCanvas* canvas, SkString* errorMsg) override {
    ...
    fStorage.reset(
        srcImg->width() * srcImg->height() * SkColorTypeBytesPerPixel(kRGBA_F16_SkColorType));
    ...
    info = SkImageInfo::Make(
        srcImg->width(), srcImg->height(), kRGBA_F16_SkColorType, kPremul_SkAlphaType,
