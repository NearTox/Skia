...
class PolyBoundsOp : public GrMeshDrawOp {
 public:
  ...
  static std::unique_ptr<GrDrawOp> Make(
      GrRecordingContext* context, GrPaint&& paint, const SkRect& rect) {
 ...
 private:
  ...
  void onPrepareDraws(Target* target) override {
    ...
    sk_sp<GrGeometryProcessor> gp(GrDefaultGeoProcFactory::Make(
        target->caps().shaderCaps(), color, Coverage::kSolid_Type, LocalCoords::kUnused_Type,
        SkMatrix::I()));
...
class ConvexPolyEffect : public GpuGM {
 ...
 protected:
  ...
  void onDraw(
    ...
    for (PathList::Iter iter(fPaths, PathList::Iter::kHead_IterStart); iter.get(); iter.next()) {
    ...
    for (RectList::Iter iter(fRects, RectList::Iter::kHead_IterStart); iter.get(); iter.next()) {
      SkScalar x = 0;
      ...
      for (int et = 0; et < kGrClipEdgeTypeCnt; ++et) {
        ...
        std::unique_ptr<GrDrawOp> op = PolyBoundsOp::Make(context, std::move(grPaint), rect);
        renderTargetContext->priv().testingOnly_addDrawOp(std::move(op));
