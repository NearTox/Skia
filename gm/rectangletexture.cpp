...
class RectangleTexture : public GpuGM {
 ...
 private:
  ...
  DrawResult onDraw(
    ...
    for (size_t i = 0; i < SK_ARRAY_COUNT(gradImgs); ++i) {
      ...
      for (auto s : kScales) {
        ...
        for (auto q : kQualities) {
          ...
          repeatPaint.setShader(gradImgs[i]->makeShader(SkTileMode::kRepeat, SkTileMode::kMirror));
          ...
          canvas->drawImageRect(
              gradImgs[i], srcRect, dstRect, &plainPaint, SkCanvas::kStrict_SrcRectConstraint);
    ...
    auto smallImg = this->createRectangleTextureImg(
    ...
    for (int fq = kNone_SkFilterQuality; fq <= kLast_SkFilterQuality; ++fq) {
      ...
      for (int ty = 0; ty < kSkTileModeCount; ++ty) {
        ...
        for (int tx = 0; tx < kSkTileModeCount; ++tx) {
          ...
          auto shader =
              smallImg->makeShader(static_cast<SkTileMode>(tx), static_cast<SkTileMode>(ty), &lm);
          SkPaint paint;
