...
class RectangleTexture : public GpuGM {
 ...
 protected:
  SkString onShortName() override { return SkString("rectangle_texture"); }
  ...
  void fillPixels(int width, int height, void* pixels) {
    ...
    canvas.drawCircle(
        SkIntToScalar(width) / 2, SkIntToScalar(height) / 2, SkIntToScalar(width + height) / 5,
        paint);
  }
  ...
  sk_sp<SkImage> createRectangleTextureImg(
    ...
    GR_GL_CALL(
        gl, TexImage2D(
                kTarget, 0, GR_GL_RGBA, width, height, 0, GR_GL_RGBA, GR_GL_UNSIGNED_BYTE, pixels));
    ...
    if (sk_sp<SkImage> image = SkImage::MakeFromAdoptedTexture(
            context, rectangleTex, origin, kRGBA_8888_SkColorType)) {
      return image;
    }
  ...
  DrawResult onDraw(
    ...
    sk_sp<SkImage> rectImgs[] = {
        this->createRectangleTextureImg(context, kTopLeft_GrSurfaceOrigin, kWidth, kHeight, pixels),
        this->createRectangleTextureImg(
            context, kBottomLeft_GrSurfaceOrigin, kWidth, kHeight, pixels),
    };
    ...
    for (size_t i = 0; i < SK_ARRAY_COUNT(rectImgs); ++i) {
      for (auto s : kScales) {
        ...
        for (auto q : kQualities) {
          ...
          repeatPaint.setShader(rectImgs[i]->makeShader(SkTileMode::kRepeat, SkTileMode::kMirror));
          ...
          auto srcRect = SkRect::MakeXYWH(
              .25f * rectImgs[i]->width(), .25f * rectImgs[i]->height(),
              .50f * rectImgs[i]->width(), .50f * rectImgs[i]->height());
          auto dstRect =
              SkRect::MakeXYWH(0, 0, .50f * rectImgs[i]->width(), .50f * rectImgs[i]->height());
          canvas->drawImageRect(
              rectImgs[i], srcRect, dstRect, &plainPaint, SkCanvas::kStrict_SrcRectConstraint);
