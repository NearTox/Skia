...
class RectangleTexture : public GpuGM {
 ...
 private:
  enum class ImageType { kGradientCircle, k2x2 };
  ...
  sk_sp<SkImage> createRectangleTextureImg(
      GrDirectContext* dContext, GrSurfaceOrigin origin, const SkBitmap content) {
    SkASSERT(content.colorType() == kRGBA_8888_SkColorType);
    auto format = GrBackendFormat::MakeGL(GR_GL_RGBA8, GR_GL_TEXTURE_RECTANGLE);
    auto bet = dContext->createBackendTexture(
        content.width(), content.height(), format, GrMipmapped::kNo, GrRenderable::kNo);
    if (!bet.isValid()) {
      return nullptr;
    }
    ...
    if (origin == kBottomLeft_GrSurfaceOrigin) {
      ...
      for (int y = 0; y < content.height();
           ++y, src += pm->rowBytesAsPixels(), dst -= tempPM.rowBytesAsPixels()) {
  ...
  DrawResult onGpuSetup(GrDirectContext* context, SkString* errorMsg) override {
    ...
    fGradImgs[0] = this->createRectangleTextureImg(context, kTopLeft_GrSurfaceOrigin, gradCircle);
    fGradImgs[1] =
    ...
    fSmallImg = this->createRectangleTextureImg(
        context, kTopLeft_GrSurfaceOrigin, this->makeImagePixels(2, ImageType::k2x2));
    if (!fSmallImg) {
    ...
    for (size_t i = 0; i < kNumGradImages; ++i) {
      ...
      for (auto s : kScales) {
        ...
        for (auto q : kQualities) {
          ...
          repeatPaint.setShader(fGradImgs[i]->makeShader(SkTileMode::kRepeat, SkTileMode::kMirror));
    ...
    for (int fq = kNone_SkFilterQuality; fq <= kLast_SkFilterQuality; ++fq) {
      ...
      for (int ty = 0; ty < kSkTileModeCount; ++ty) {
        ...
        for (int tx = 0; tx < kSkTileModeCount; ++tx) {
          ...
          auto shader =
              fSmallImg->makeShader(static_cast<SkTileMode>(tx), static_cast<SkTileMode>(ty), &lm);
          SkPaint paint;
