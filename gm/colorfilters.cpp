...
static void install_nothing(SkPaint* paint, uint32_t, uint32_t) { paint->setColorFilter(nullptr); }
...
class HSLColorFilterGM : public skiagm::GM {
 protected:
  ...
  void onOnceBeforeDraw() override {
    ...
    const auto lm = SkMatrix::MakeRectToRect(
        SkRect::MakeWH(mandrill->width(), mandrill->height()),
        SkRect::MakeWH(kWheelSize, kWheelSize), SkMatrix::kFill_ScaleToFit);
    ...
    for (const auto& cols : gGrads) {
      fShaders.push_back(SkGradientShader::MakeSweep(
          kWheelSize / 2, kWheelSize / 2, cols, nullptr, SK_ARRAY_COUNT(cols), SkTileMode::kRepeat,
          -90, 270, 0, nullptr));
    }
  }
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    for (const auto& shader : fShaders) {
      ...
      for (const auto& tst : gTests) {
        ...
        auto h = std::get<0>(tst.h), s = std::get<0>(tst.s), l = std::get<0>(tst.l);
        {
 ...
 private:
  ...
  static sk_sp<SkColorFilter> make_filter(float h, float s, float l) {
    ...
    const auto h_bias = h, h_scale = 1.0f, s_bias = std::max(s, 0.0f), s_scale = 1 - std::abs(s),
               l_bias = std::max(l, 0.0f), l_scale = 1 - std::abs(l);

    const float cm[20] = {
        h_scale, 0, 0, 0, h_bias, 0, s_scale, 0, 0, s_bias, 0, 0, l_scale, 0, l_bias, 0, 0, 0, 1, 0,
    };
