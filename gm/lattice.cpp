...
static sk_sp<SkSurface> make_surface(
    SkCanvas* root, int N, int padLeft, int padTop, int padRight, int padBottom) {
...
static sk_sp<SkImage> make_image(
    SkCanvas* root, int* xDivs, int* yDivs, int padLeft, int padTop, int padRight, int padBottom) {
  const int kCap = 28;
  const int kMid = 8;
...
class LatticeGM : public skiagm::GM {
 ...
 protected:
  ...
  void onDrawHelper(
      GrDirectContext* dContext, SkCanvas* canvas, int padLeft, int padTop, int padRight,
      int padBottom) {
    canvas->save();
    ...
    SkIRect bounds =
        SkIRect::MakeLTRB(padLeft, padTop, image->width() - padRight, image->height() - padBottom);
    lattice.fBounds =
...
class LatticeGM2 : public skiagm::GM {
 public:
  ...
  void onDrawHelper(
      SkCanvas* canvas, int padLeft, int padTop, int padRight, int padBottom, SkPaint& paint) {
    int xDivs[2] = {4, 5};
    int yDivs[2] = {1, 2};
