...
static void paint_rgn(SkCanvas* canvas, const SkAAClip& clip, const SkPaint& paint) {
  ...
  canvas->drawBitmap(bm2, SK_Scalar1 * mask.fBounds.fLeft, SK_Scalar1 * mask.fBounds.fTop, &paint);
}
...
class SimpleClipGM : public GM {
 ...
 protected:
  void onOnceBeforeDraw() override {
    ...
    fBase.setLTRB(100.65f, 100.65f, 150.65f, 150.65f);
    fRect = fBase;
    fRect.inset(5, 5);
    fRect.offset(25, 25);
  ...
  void drawPathsOped(SkCanvas* canvas, SkClipOp op, SkColor color) {
    ...
    SkRect r = SkRect::MakeLTRB(
        SkIntToScalar(90), SkIntToScalar(90), SkIntToScalar(180), SkIntToScalar(180));
