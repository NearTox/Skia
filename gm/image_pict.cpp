...
class TextureGenerator : public SkImageGenerator {
 public:
  TextureGenerator(GrContext* ctx, const SkImageInfo& info, sk_sp<SkPicture> pic)
    ...
    sk_sp<SkSurface> surface(SkSurface::MakeRenderTarget(
        ctx, SkBudgeted::kYes, info, 0, kTopLeft_GrSurfaceOrigin, nullptr));
    if (surface) {
 ...
 protected:
  sk_sp<GrTextureProxy> onGenerateTexture(
    ...
    if (!dstContext->copy(
            fProxy.get(), SkIRect::MakeXYWH(origin.x(), origin.y(), info.width(), info.height()),
            SkIPoint::Make(0, 0))) {
      return nullptr;
    }
 ...
 protected:
  ...
  static void draw_as_tex(SkCanvas* canvas, SkImage* image, SkScalar x, SkScalar y) {
    ...
    if (!proxy) {
      ...
      paint.setStyle(SkPaint::kStroke_Style);
      SkRect r =
          SkRect::MakeXYWH(x, y, SkIntToScalar(image->width()), SkIntToScalar(image->width()));
    ...
    sk_sp<SkImage> texImage(new SkImage_Gpu(
        sk_ref_sp(canvas->getGrContext()), image->uniqueID(), kPremul_SkAlphaType, std::move(proxy),
        image->refColorSpace()));
