...
class SharedCornersGM : public GM {
 ...
 protected:
  ...
  SkISize onISize() override {
    ...
    return SkISize::Make(
        numCols * (kBoxSize + kPadSize) + kPadSize, numRows * (kBoxSize + kPadSize) + kPadSize);
  }
  ...
  void drawTriangleBoxes(
      SkCanvas* canvas, const std::vector<SkPoint>& points,
  ...
  void drawRow(SkCanvas* canvas, const SkPath& path) {
    ...
    canvas->translate(
        (kBoxSize - bounds.width()) / 2 - bounds.left(),
