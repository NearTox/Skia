...
DEF_SIMPLE_GPU_GM(sample_matrix_constant, ctx, rtCtx, canvas, 512, 256) {
  {
    ...
    auto fp =
        std::unique_ptr<GrFragmentProcessor>(new SampleMatrixConstantEffect(std::move(imgFP)));
...

  {
    ...
    auto fp =
        std::unique_ptr<GrFragmentProcessor>(new SampleMatrixConstantEffect(std::move(gradientFP)));
    paint.addCoverageFragmentProcessor(std::move(fp));
    rtCtx->drawRect(GrNoClip(), std::move(paint), GrAA::kNo, SkMatrix::I(), bounds);
  }
}
