...
static void draw_clipped_filter(
    SkCanvas* canvas, sk_sp<SkImageFilter> filter, size_t i, const SkRect& primBounds,
  ...
  canvas->drawCircle(primBounds.centerX(), primBounds.centerY(), primBounds.width() * 2 / 5, paint);
  canvas->restore();
}
...
class ImageFiltersClippedGM : public GM {
 public:
  ImageFiltersClippedGM() { this->setBGColor(0x00000000); }

 protected:
  SkString onShortName() override { return SkString("imagefiltersclipped"); }
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    sk_sp<SkImageFilter> filters[] = {
        ...
        SkImageFilters::DropShadow(
            SkIntToScalar(10), SkIntToScalar(10), SkIntToScalar(3), SkIntToScalar(3), SK_ColorGREEN,
            nullptr),
        SkImageFilters::DisplacementMap(
            SkColorChannel::kR, SkColorChannel::kR, SkIntToScalar(12), std::move(gradient),
            checkerboard),
    ...
    sk_sp<SkImageFilter> rectFilter(
        SkImageFilters::Shader(SkPerlinNoiseShader::MakeFractalNoise(0.1f, 0.05f, 1, 0)));
