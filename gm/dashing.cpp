...
static void drawline(
    SkCanvas* canvas, int on, int off, const SkPaint& paint, SkScalar finalX = SkIntToScalar(600),
    SkScalar finalY = SkIntToScalar(0), SkScalar phase = SkIntToScalar(0),
    ...
    for (int width = 0; width <= 2; ++width) {
      for (const Intervals& data : {Intervals{1, 1}, Intervals{4, 1}}) {
        for (bool aa : {false, true}) {
          ...
          drawline(canvas, data.fOnInterval * scale, data.fOffInterval * scale, paint);
          canvas->translate(0, SkIntToScalar(20));
        }
      }
    }
...
static SkPath make_path_rect(const SkRect& bounds) { return SkPath::Rect(bounds); }

static SkPath make_path_oval(const SkRect& bounds) { return SkPath::Oval(bounds); }
...
class Dashing2GM : public skiagm::GM {
  ...
  void onDraw(SkCanvas* canvas) override {
    constexpr int gIntervals[] = {3,  // 3 dashes: each count [0] followed by intervals [1..count]
...
class Dashing3GM : public skiagm::GM {
  ...
  void drawDashedLines(
      SkCanvas* canvas, SkScalar lineLength, SkScalar phase, SkScalar dashLength, int strokeWidth,
      bool circles) {
    SkPaint p;
 ...
 private:
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    for (int x = 0; x < 200; x += 10) {
      ...
      sign = (x % 20) ? 1 : -1;
      drawline(
          canvas, kOn, kOff, paint, SkIntToScalar(x), -sign * SkIntToScalar(10003),
    ...
    for (int y = -400; y < 0; y += 10) {
      ...
      sign = (y % 20) ? 1 : -1;
      drawline(
          canvas, kOn, kOff, paint, -sign * SkIntToScalar(10003), SkIntToScalar(y),
