...
class BigBlursGM : public GM {
 public:
  BigBlursGM() { this->setBGColor(0xFFDDDDDD); }

 protected:
  SkString onShortName() override { return SkString("bigblurs"); }
      ...
      for (int j = 0; j <= kLastEnum_SkBlurStyle; ++j) {
        ...
        for (int k = 0; k < (int)SK_ARRAY_COUNT(origins); ++k) {
          ...
          SkRect clipRect = SkRect::MakeXYWH(
              SkIntToScalar(desiredX), SkIntToScalar(desiredY), SkIntToScalar(kCloseUpSize),
              SkIntToScalar(kCloseUpSize));

          canvas->clipRect(clipRect);

          canvas->translate(desiredX - origins[k].fX, desiredY - origins[k].fY);
