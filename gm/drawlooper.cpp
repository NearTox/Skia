...
public:
    DrawLooperGM() : fLooper(nullptr) { this->setBGColor(0xFFDDDDDD); }

protected:
    virtual SkISize onISize() override { return SkISize::Make(520, 160); }

    SkString onShortName() override { return SkString("drawlooper"); }
...
private:
    ...
    void init() {
        ...
        constexpr struct {
            ...
            SkScalar fOffset;
            SkScalar fBlur;
        } gParams[] = {{SK_ColorWHITE, SkPaint::kStroke_Style, SkIntToScalar(1) * 3 / 4, 0, 0},
        ...
        for (size_t i = 0; i < SK_ARRAY_COUNT(gParams); i++) {
            ...
            if (gParams[i].fBlur > 0) {
                paint->setMaskFilter(SkMaskFilter::MakeBlur(
                        kNormal_SkBlurStyle, SkBlurMask::ConvertRadiusToSigma(gParams[i].fBlur)));
            }
        }
