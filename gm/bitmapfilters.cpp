...
static void make_bm(SkBitmap* bm) {
  const SkColor colors[4] = {SK_ColorRED, SK_ColorGREEN, SK_ColorBLUE, SK_ColorWHITE};
...
static SkScalar draw_bm(
    SkCanvas* canvas, const SkBitmap& bm, SkScalar x, SkScalar y, SkPaint* paint) {
...
static SkScalar draw_row(SkCanvas* canvas, const SkBitmap& bm) {
  ...
  canvas->drawString(name, x, SkIntToScalar(bm.height()) * scale * 5 / 8, font, paint);
...
class FilterGM : public skiagm::GM {
 ...
 public:
  ...
  FilterGM() { this->setBGColor(0xFFDDDDDD); }

 protected:
  SkString onShortName() override { return SkString("bitmapfilters"); }

  SkISize onISize() override { return SkISize::Make(540, 250); }
...
class TestExtractAlphaGM : public skiagm::GM {
 ...
 protected:
  SkString onShortName() override { return SkString("extractalpha"); }

  SkISize onISize() override { return SkISize::Make(540, 330); }
