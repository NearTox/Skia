...
static sk_sp<SkShader> make_opaque_color() { return SkShaders::Color(0xFFFF0000); }

static sk_sp<SkShader> make_alpha_color() { return SkShaders::Color(0x80FF0000); }

static sk_sp<SkColorFilter> make_cf_null() { return nullptr; }
...
DEF_SIMPLE_GM(color4f, canvas, 1024, 260) {
  ...
  sk_sp<SkColorSpace> colorSpaces[]{nullptr, SkColorSpace::MakeSRGB()};
  for (const sk_sp<SkColorSpace>& colorSpace : colorSpaces) {
    const SkImageInfo info =
        SkImageInfo::Make(1024, 100, kN32_SkColorType, kPremul_SkAlphaType, colorSpace);
