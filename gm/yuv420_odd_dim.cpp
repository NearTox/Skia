...
DEF_SIMPLE_GM_CAN_FAIL(
    yuv420_odd_dim, canvas, errMsg, kScale* kImageDim.width(), kScale* kImageDim.height()) {
...
DEF_SIMPLE_GM_CAN_FAIL(yuv420_odd_dim_repeat, canvas, errMsg, 1000, 500) {
  ...
  auto [planes, yuvaInfo] = sk_gpu_test::MakeYUVAPlanesAsA8(
      image.get(), kJPEG_SkYUVColorSpace, SkYUVAInfo::Subsampling::k420, nullptr);
  SkPixmap pixmaps[4];
  ...
  for (SkMipmapMode mm : {SkMipmapMode::kNone, SkMipmapMode::kLinear}) {
    ...
    for (SkFilterMode filter : {SkFilterMode::kNearest, SkFilterMode::kLinear}) {
      ...
      auto shader = image->makeShader(
          SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions(filter, mm));
      SkPaint paint;
