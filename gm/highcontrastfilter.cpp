...
static void draw_label(SkCanvas* canvas, const SkHighContrastConfig& config) {
  ...
  canvas->drawSimpleText(
      labelBuffer, len, SkTextEncoding::kUTF8, 0.5f - width / 2, 0.16f, font, SkPaint());
}
...
static void draw_scene(SkCanvas* canvas, const SkHighContrastConfig& config) {
  ...
  paint.setShader(
      SkGradientShader::MakeLinear(pts, colors2, pos, SK_ARRAY_COUNT(colors2), SkTileMode::kClamp));
  canvas->drawRect(bounds, paint);
 ...
 public:
  HighContrastFilterGM() {
    ...
    fGr1 = SkGradientShader::MakeLinear(
        g1Points, g1Colors, pos, SK_ARRAY_COUNT(g1Colors), SkTileMode::kClamp);
 ...
 protected:
  ...
  void onDraw(SkCanvas* canvas) override {
    SkHighContrastConfig configs[] = {
        {false, InvertStyle::kNoInvert, 0.0f},        {false, InvertStyle::kInvertBrightness, 0.0f},
        {false, InvertStyle::kInvertLightness, 0.0f}, {false, InvertStyle::kInvertLightness, 0.2f},
        {true, InvertStyle::kNoInvert, 0.0f},         {true, InvertStyle::kInvertBrightness, 0.0f},
        {true, InvertStyle::kInvertLightness, 0.0f},  {true, InvertStyle::kInvertLightness, 0.2f},
    };
