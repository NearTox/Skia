...
class ComplexClip2GM : public GM {
 ...
 protected:
  ...
  static const char* ClipStr(Clip clip) {
    switch (clip) {
      case kRect_Clip: return "rect";
      case kRRect_Clip: return "rrect";
      case kPath_Clip: return "path";
    }
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    for (int i = 0; i < kRows; ++i) {
      for (int j = 0; j < kCols; ++j) {
        ...
        canvas->translate(
            kPadX * SK_Scalar1 + (fWidth + kPadX * SK_Scalar1) * j,
        ...
        for (int k = 0; k < 5; ++k) {
          ...
          switch (fClip) {
            case kRect_Clip: canvas->drawRect(fRects[k], rectPaint); break;
            case kRRect_Clip: canvas->drawRRect(fRRects[k], rectPaint); break;
            case kPath_Clip: canvas->drawPath(fPaths[k], rectPaint); break;
          }
        }

        for (int k = 0; k < 5; ++k) {
          switch (fClip) {
            case kRect_Clip: canvas->clipRect(fRects[k], fOps[j * kRows + i][k], fAntiAlias); break;
            case kRRect_Clip:
              canvas->clipRRect(fRRects[k], fOps[j * kRows + i][k], fAntiAlias);
              break;
            case kPath_Clip: canvas->clipPath(fPaths[k], fOps[j * kRows + i][k], fAntiAlias); break;
          }
        }
