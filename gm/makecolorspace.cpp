...
DEF_SIMPLE_GM_BG(makecolortypeandspace, canvas, 128 * 3, 128 * 4, SK_ColorWHITE) {
  ...
  for (bool lazy : {true, false}) {
    for (int j = 0; j < 2; ++j) {
      ...
      auto image565 = image->makeColorTypeAndColorSpace(kRGB_565_SkColorType, rec2020, direct);
      ...
      auto imageGray =
          image->makeColorTypeAndColorSpace(kGray_8_SkColorType, image->refColorSpace(), direct);
...
DEF_SIMPLE_GM_CAN_FAIL(reinterpretcolorspace, canvas, errorMsg, 128 * 3, 128 * 3) {
  ...
  canvas->drawImage(
      image->makeColorSpace(spin, nullptr)->reinterpretColorSpace(srgb), 256.0f, 0.0f);
  ...
  canvas->drawImage(image, 0.0f, 0.0f);
  canvas->drawImage(image->reinterpretColorSpace(spin), 128.0f, 0.0f);
  canvas->drawImage(image->makeColorSpace(spin, direct)->reinterpretColorSpace(srgb), 256.0f, 0.0f);
