...
static void extract_planes(const SkBitmap& bm, SkYUVColorSpace yuvColorSpace, PlaneData* planes) {
  ...
  planes->fYFull.allocPixels(
      SkImageInfo::Make(bm.width(), bm.height(), kGray_8_SkColorType, kUnpremul_SkAlphaType));
...
static SkBitmap make_quarter_2_channel(
  ...
  result.allocPixels(SkImageInfo::Make(
      fullY.width() / 2, fullY.height() / 2, kRGBA_8888_SkColorType, kUnpremul_SkAlphaType));
...
static uint8_t look_up(float x1, float y1, const SkBitmap& bm, SkColorChannel channel) {
  ...
  if (kAlpha_8_SkColorType == bm.colorType() || kGray_8_SkColorType == bm.colorType()) {
    ...
    switch (channel) {
      case SkColorChannel::kR: result = SkColorGetB(c); break;
      case SkColorChannel::kG: result = SkColorGetG(c); break;
      case SkColorChannel::kB: result = SkColorGetR(c); break;
...
static void draw_row_label(SkCanvas* canvas, int y, int yuvFormat) {
  static const char* kYUVFormatNames[] = {"P016", "P010", "Y416", "AYUV", "Y410",
                                          "NV12", "NV21", "I420", "YV12"};
...
static void make_RG_88(
    const GrCaps* caps, const SkBitmap& bm, YUVFormat yuvFormat, SkAutoTMalloc<uint8_t>* pixels,
...
static GrBackendTexture create_yuva_texture(
  ...
  if (format_uses_16_bpp(yuvFormat) || 2 == channelCount) {
    ...
    tex = gpu->createBackendTexture(
        bm.width(), bm.height(), format, GrMipMapped::kNo, GrRenderable::kNo, pixels, rowBytes,
        nullptr, GrProtected::kNo);
  } else {
    tex =
        context->priv().createBackendTexture(&bm.pixmap(), 1, GrRenderable::kNo, GrProtected::kNo);
  }
      ...
      for (int cs = kJPEG_SkYUVColorSpace; cs <= kLastEnum_SkYUVColorSpace; ++cs) {
        ...
        for (int format = kP016_YUVFormat; format <= kLast_YUVFormat; ++format) {
          ...
          if (context) {
            ...
            if (format_cant_be_represented_with_pixmaps((YUVFormat)format) && counterMod == 2) {
