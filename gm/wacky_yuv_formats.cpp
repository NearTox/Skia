...
static void create_YUV(
  ...
  switch (yuvFormat) {
    ...
    case kNV12_YUVFormat: {
      ...
      uvQuarter.allocPixels(SkImageInfo::Make(
          planes.fYFull.width() / 2, planes.fYFull.height() / 2, kRGBA_8888_SkColorType,
          kUnpremul_SkAlphaType));
    ...
    case kNV21_YUVFormat: {
      ...
      // There isn't a RG color type. Approx w/ RGBA.
      vuQuarter.allocPixels(SkImageInfo::Make(
          planes.fYFull.width() / 2, planes.fYFull.height() / 2, kRGBA_8888_SkColorType,
          kUnpremul_SkAlphaType));
...
static GrBackendTexture create_yuva_texture(
    GrGpu* gpu, const SkBitmap& bm, SkYUVAIndex yuvaIndices[4], int texIndex) {
  ...
  if (2 == channelCount) {
    ...
    tex = gpu->createBackendTexture(
        bm.width(), bm.height(), format, GrMipMapped::kNo, GrRenderable::kNo, pixels,
        2 * bm.width());
  }
  if (!tex.isValid()) {
