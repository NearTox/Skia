...
static bool format_uses_16_bpp(YUVFormat yuvFormat) {
  return kP016_YUVFormat == yuvFormat || kP010_YUVFormat == yuvFormat ||
         kP016F_YUVFormat == yuvFormat || kY416_YUVFormat == yuvFormat;
}
#endif

static bool format_has_builtin_alpha(YUVFormat yuvFormat) {
  return kY416_YUVFormat == yuvFormat || kAYUV_YUVFormat == yuvFormat ||
...
static void create_YUV(
    const PlaneData& planes, YUVFormat yuvFormat, SkBitmap resultBMs[], SkYUVAIndex yuvaIndices[4],
    bool opaque) {
  int nextLayer = 0;
...
static uint8_t look_up(float x1, float y1, const SkBitmap& bm, SkColorChannel channel) {
  ...
  if (kGray_8_SkColorType == bm.colorType()) {
    ...
    result = *bm.getAddr8(x, y);
      ...
      kAlpha_8_SkColorType == bm.colorType() || kA16_unorm_SkColorType == bm.colorType() ||
      kA16_float_SkColorType == bm.colorType()) {
    SkASSERT(SkColorChannel::kA == channel);
    SkColor c = bm.getColor(x, y);
    result = SkColorGetA(c);
  } else if (
      kR8G8_unorm_SkColorType == bm.colorType() || kR16G16_unorm_SkColorType == bm.colorType() ||
      kR16G16_float_SkColorType == bm.colorType()) {
    SkASSERT(SkColorChannel::kR == channel || SkColorChannel::kG == channel);
    SkColor c = bm.getColor(x, y);

    switch (channel) {
      case SkColorChannel::kR: result = SkColorGetR(c); break;
      case SkColorChannel::kG: result = SkColorGetG(c); break;
      case SkColorChannel::kB: result = 0; break;
      case SkColorChannel::kA: result = 255; break;
    }
  } else {
    SkASSERT(
        kRGBA_1010102_SkColorType == bm.colorType() || kRGBA_8888_SkColorType == bm.colorType() ||
    ...
    switch (channel) {
      case SkColorChannel::kR: result = SkColorGetR(c); break;
      case SkColorChannel::kG: result = SkColorGetG(c); break;
      case SkColorChannel::kB: result = SkColorGetB(c); break;
      case SkColorChannel::kA: result = SkColorGetA(c); break;
    }
  }
...
class YUVGenerator : public SkImageGenerator {
 public:
  YUVGenerator(
      const SkImageInfo& ii, SkYUVColorSpace yuvColorSpace,
      SkYUVAIndex yuvaIndices[SkYUVAIndex::kIndexCount],
      SkBitmap bitmaps[SkYUVASizeInfo::kMaxCount])
      : SkImageGenerator(ii), fYUVColorSpace(yuvColorSpace), fAllA8(true) {
    ...
    if (kUnknown_SkColorType == fFlattened.colorType()) {
      ...
      for (int y = 0; y < info.height(); ++y) {
        for (int x = 0; x < info.width(); ++x) {
          ...
          uint8_t Y =
              look_up(x1, y1, fYUVBitmaps[fYUVAIndices[0].fIndex], fYUVAIndices[0].fChannel);

          uint8_t U =
              look_up(x1, y1, fYUVBitmaps[fYUVAIndices[1].fIndex], fYUVAIndices[1].fChannel);

          uint8_t V =
              look_up(x1, y1, fYUVBitmaps[fYUVAIndices[2].fIndex], fYUVAIndices[2].fChannel);

          uint8_t A = 255;
          if (fYUVAIndices[3].fIndex >= 0) {
            A = look_up(x1, y1, fYUVBitmaps[fYUVAIndices[3].fIndex], fYUVAIndices[3].fChannel);
          }
...
static sk_sp<SkImage> make_yuv_gen_image(
    const SkImageInfo& ii, SkYUVColorSpace yuvColorSpace,
    SkYUVAIndex yuvaIndices[SkYUVAIndex::kIndexCount], SkBitmap bitmaps[]) {
  std::unique_ptr<SkImageGenerator> gen(new YUVGenerator(ii, yuvColorSpace, yuvaIndices, bitmaps));
...
static GrBackendTexture create_yuva_texture(
    GrContext* context, const SkBitmap& bm, SkYUVAIndex yuvaIndices[4], int texIndex,
    YUVFormat yuvFormat) {
#ifdef SK_DEBUG
  ...
  sk_sp<SkImage> resizeOnGpu(
    ...
    for (int i = 0; i < numTextures; ++i) {
      ...
      sk_sp<SkImage> wrappedOrig = SkImage::MakeFromTexture(
          context, yuvaTextures[i], kTopLeft_GrSurfaceOrigin, ct, kPremul_SkAlphaType, nullptr);

      shrunkTextures[i] = context->createBackendTexture(
          shrunkPlaneSize.width(), shrunkPlaneSize.height(), yuvaTextures[i].getBackendFormat(),
      ...
      sk_sp<SkSurface> s = SkSurface::MakeFromBackendTexture(
          context, shrunkTextures[i], kTopLeft_GrSurfaceOrigin, 0, ct, nullptr, nullptr);
      if (!s) {
        return nullptr;
      }
    ...
    return SkImage::MakeFromYUVATextures(
        context, yuvColorSpace, shrunkTextures, yuvaIndices, shrunkImageSize,
      ...
      for (int cs = kJPEG_SkYUVColorSpace; cs <= kLastEnum_SkYUVColorSpace; ++cs) {
        ...
        for (int format = kP016_YUVFormat; format <= kLast_YUVFormat; ++format) {
          ...
          if (context) {
            ...
            for (int i = 0; i < numTextures; ++i) {
              yuvaTextures[i] =
                  create_yuva_texture(context, resultBMs[i], yuvaIndices, i, (YUVFormat)format);
            ...
            if (fQuarterSize) {
              fImages[opaque][cs][format] = this->resizeOnGpu(
                  context, (YUVFormat)format, (SkYUVColorSpace)cs, opaque, yuvaTextures,
              ...
              switch (counterMod) {
                case 0:
                  fImages[opaque][cs][format] = SkImage::MakeFromYUVATexturesCopy(
                      context, (SkYUVColorSpace)cs, yuvaTextures, yuvaIndices,
                ...
                default:
                  fImages[opaque][cs][format] = SkImage::MakeFromYUVAPixmaps(
                      context, (SkYUVColorSpace)cs, yuvaPixmaps, yuvaIndices,
                      {fOriginalBMs[opaque].width(), fOriginalBMs[opaque].height()},
 ...
 protected:
  ...
  void createImages(GrContext* context) {
    for (bool opaque : {false, true}) {
      ...
      for (int i = 0; i < numTextures; ++i) {
        yuvaTextures[i] =
            create_yuva_texture(context, resultBMs[i], yuvaIndices, i, kAYUV_YUVFormat);
        if (yuvaTextures[i].isValid()) {
          fBackendTextures.push_back(yuvaTextures[i]);
        }
      }
      ...
      fImages[opaque][0] = SkImage::MakeFromYUVATextures(
          context, kJPEG_SkYUVColorSpace, yuvaTextures, yuvaIndices,
