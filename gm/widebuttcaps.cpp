...
static void draw_strokes(
    SkCanvas* canvas, SkColor strokeColor, const SkPath& path, const SkPath& cubic) {
  SkPaint strokePaint;
...
static void draw_test(SkCanvas* canvas, SkColor strokeColor) {
  ...
  draw_strokes(
      canvas, strokeColor, SkPath().lineTo(10, 0).lineTo(10, 10),
      SkPath().cubicTo(10, 0, 10, 0, 10, 10));
  canvas->translate(0, 120);

  draw_strokes(
      canvas, strokeColor, SkPath().lineTo(0, -10).lineTo(0, 10),
      SkPath().cubicTo(0, -10, 0, -10, 0, 10));
  canvas->translate(0, 120);

  draw_strokes(
      canvas, strokeColor, SkPath().lineTo(0, -10).lineTo(10, -10).lineTo(10, 10).lineTo(0, 10),
      SkPath().cubicTo(0, -10, 10, 10, 0, 10));
  canvas->translate(0, 140);

  draw_strokes(
      canvas, strokeColor, SkPath().lineTo(0, -10).lineTo(10, -10).lineTo(10, 0).lineTo(0, 0),
      SkPath().cubicTo(0, -10, 10, 0, 0, 0));
  canvas->translate(0, 120);
}

DEF_SIMPLE_GM(widebuttcaps, canvas, kTestWidth, kTestHeight) { draw_test(canvas, SK_ColorGREEN); }

class WideButtCaps_tess_segs_5 : public skiagm::GpuGM {
  SkString onShortName() override { return SkString("widebuttcaps_tess_segs_5"); }

  SkISize onISize() override { return SkISize::Make(kTestWidth, kTestHeight); }
  ...
  void modifyGrContextOptions(GrContextOptions* options) override {
    ...
    options->fGpuPathRenderers =
        (GpuPathRenderers)((int)options->fGpuPathRenderers & (int)GpuPathRenderers::kTessellation);
  }

  DrawResult onDraw(
      GrRecordingContext* context, GrRenderTargetContext* rtc, SkCanvas* canvas,
    ...
    if (context->priv().caps()->shaderCaps()->maxTessellationSegments() !=
      ...
      errorMsg->set(
          "modifyGrContextOptions() did not limit maxTessellationSegments. "
