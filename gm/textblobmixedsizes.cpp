...
class TextBlobMixedSizes : public GM {
 ...
 protected:
  ...
  SkString onShortName() override {
    return SkStringPrintf("textblobmixedsizes%s", fUseDFT ? "_df" : "");
  }
  ...
  void onDraw(SkCanvas* inputCanvas) override {
    ...
    if (fUseDFT) {
      ...
      SkImageInfo info =
          SkImageInfo::MakeN32(size.width(), size.height(), kPremul_SkAlphaType, colorSpace);
    ...
    for (int i = 0; i < 4; i++) {
      canvas->save();
      switch (i % 2) {
        case 0: canvas->rotate(random.nextF() * 45.f); break;
        case 1: canvas->rotate(-random.nextF() * 45.f); break;
      }
      if (!fUseDFT) {
