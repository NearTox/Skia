...
class DFTextBlobPerspGM : public skiagm::GM {
 ...
 protected:
  ...
  void onOnceBeforeDraw() override {
    for (int i = 0; i < 3; ++i) {
      ...
      font.setSize(32);
      font.setEdging(
          i == 0 ? SkFont::Edging::kAlias
                 : (i == 1 ? SkFont::Edging::kAntiAlias : SkFont::Edging::kSubpixelAntiAlias));
  ...
  void onDraw(SkCanvas* inputCanvas) override {
    ...
    SkImageInfo info = SkImageInfo::MakeN32(
        size.width(), size.height(), kPremul_SkAlphaType, inputCanvas->imageInfo().refColorSpace());
    SkSurfaceProps props(
        SkSurfaceProps::kUseDeviceIndependentFonts_Flag, SkSurfaceProps::kLegacyFontHost_InitType);
      ...
      for (auto pm : {PerspMode::kNone, PerspMode::kX, PerspMode::kY, PerspMode::kXY}) {
        for (auto& blob : fBlobs) {
          for (bool clip : {false, true}) {
            ...
            if (clip) {
              auto rect = SkRect::MakeXYWH(x + 5, y + 5, w * 3.f / 4.f, h * 3.f / 4.f);
 ...
 private:
  ...
  void drawBlob(
      SkCanvas* canvas, SkTextBlob* blob, SkColor color, SkScalar x, SkScalar y,
      PerspMode perspMode, TranslateWithMatrix translateWithMatrix) {
    canvas->save();
    SkMatrix persp = SkMatrix::I();
    switch (perspMode) {
      case PerspMode::kNone: break;
      case PerspMode::kX: persp.setPerspX(0.005f); break;
      case PerspMode::kY: persp.setPerspY(00.005f); break;
      case PerspMode::kXY:
