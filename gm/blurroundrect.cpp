...
class BlurRoundRectGM : public skiagm::GM {
 public:
  BlurRoundRectGM(int width, int height) : fName("blurroundrect"), fWidth(width), fHeight(height) {
  ...
  void onDraw(SkCanvas* canvas) override {
    SkLayerDrawLooper::Builder looperBuilder;
    {
      ...
      info.fPaintBits = SkLayerDrawLooper::kMaskFilter_Bit | SkLayerDrawLooper::kColorFilter_Bit;
      info.fColorMode = SkBlendMode::kSrc;
      info.fOffset = SkPoint::Make(SkIntToScalar(-1), SkIntToScalar(0));
      info.fPostTranslate = false;
      SkPaint* paint = looperBuilder.addLayerOnTop(info);
      paint->setMaskFilter(SkMaskFilter::MakeBlur(
          kNormal_SkBlurStyle, SkBlurMask::ConvertRadiusToSigma(SK_ScalarHalf)));
...
class SimpleBlurRoundRectGM : public skiagm::GM {
 public:
  SimpleBlurRoundRectGM() : fName("simpleblurroundrect") {}

 protected:
  SkString onShortName() override { return fName; }

  SkISize onISize() override { return SkISize::Make(1000, 500); }
    ...
    for (size_t i = 0; i < SK_ARRAY_COUNT(blurRadii); ++i) {
      ...
      for (size_t j = 0; j < SK_ARRAY_COUNT(cornerRadii); ++j) {
        for (int k = 0; k <= 1; k++) {
          ...
          paint.setMaskFilter(SkMaskFilter::MakeBlur(
              kNormal_SkBlurStyle, SkBlurMask::ConvertRadiusToSigma(SkIntToScalar(blurRadii[i]))));
