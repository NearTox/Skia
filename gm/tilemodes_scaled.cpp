...
class ScaledTilingGM : public skiagm::GM {
 public:
  ScaledTilingGM(bool powerOfTwoSize) : fPowerOfTwoSize(powerOfTwoSize) {}
    ...
    for (size_t kx = 0; kx < SK_ARRAY_COUNT(gModes); kx++) {
      for (size_t ky = 0; ky < SK_ARRAY_COUNT(gModes); ky++) {
        ...
        SkTextUtils::DrawString(
            canvas, str.c_str(), scale * (x + r.width() / 2), y, font, SkPaint(),
    ...
    for (size_t i = 0; i < SK_ARRAY_COUNT(gColorTypes); i++) {
      for (size_t j = 0; j < SK_ARRAY_COUNT(gFilterQualitys); j++) {
       ...
       // on SGX where resizing NPOT textures to POT textures exhibits a driver bug.
        ...
        canvas->drawString(
            SkStringPrintf("%s, %s", gColorTypeNames[i], gFilterNames[j]), scale * x,
            scale * (y + r.height() * 2 / 3), font, textPaint);
  ...
  switch (index % 3) {
    ...
    case 2:
      return SkGradientShader::MakeSweep(
          center.fX, center.fY, colors, nullptr, SK_ARRAY_COUNT(colors));
  }
