...
static void show_bounds(
    SkCanvas* canvas, const SkIRect* clip, const SkIRect* inSubset, const SkIRect* outSubset) {
...
static sk_sp<SkImageFilter> drop_shadow_factory(sk_sp<SkImage> auxImage, const SkIRect* cropRect) {
  ...
  return SkDropShadowImageFilter::Make(
      ...
      SkDropShadowImageFilter::kDrawShadowAndForeground_ShadowMode, nullptr, &crop);
}
...
static sk_sp<SkImageFilter> displacement_factory(sk_sp<SkImage> auxImage, const SkIRect* cropRect) {
  ...
  return SkDisplacementMapEffect::Make(
      ...
      SkDisplacementMapEffect::kG_ChannelSelectorType, 40.f, std::move(displacement), nullptr,
      &crop);
}
...
static sk_sp<SkImageFilter> arithmetic_factory(sk_sp<SkImage> auxImage, const SkIRect* cropRect) {
  ...
  return SkArithmeticImageFilter::Make(
      0.0f, .6f, 1.f, 0.f, false, std::move(background), nullptr, &crop);
}
...
static sk_sp<SkImageFilter> xfermode_factory(sk_sp<SkImage> auxImage, const SkIRect* cropRect) {
  ...
  sk_sp<SkImageFilter> background = SkImageSource::Make(std::move(auxImage));
  return SkXfermodeImageFilter::Make(SkBlendMode::kModulate, std::move(background), nullptr, &crop);
}
...
static sk_sp<SkImageFilter> convolution_factory(sk_sp<SkImage> auxImage, const SkIRect* cropRect) {
  ...
  SkScalar kernel[9] = {-1.f, -1.f, -1.f, -1.f, 8.f, -1.f, -1.f, -1.f, -1.f};
  return SkMatrixConvolutionImageFilter::Make(
      kernelSize, kernel, 1.f, 0.f, kernelOffset, SkMatrixConvolutionImageFilter::kClamp_TileMode,
      false, nullptr, &crop);
}
...
static sk_sp<SkImageFilter> alpha_threshold_factory(
    sk_sp<SkImage> auxImage, const SkIRect* cropRect) {
  auto crop = make_crop(cropRect);
...
static sk_sp<SkImageFilter> lighting_factory(sk_sp<SkImage> auxImage, const SkIRect* cropRect) {
  ...
  static const float kMatrix[20] = {0.f, 0.f, 0.f, 0.f, 1.f, 0.f,     0.f,     0.f,     0.f, 1.f,
                                    0.f, 0.f, 0.f, 0.f, 1.f, 0.2126f, 0.7152f, 0.0722f, 0.f, 0.f};
  sk_sp<SkImageFilter> srcToAlpha =
      SkColorFilterImageFilter::Make(SkColorFilters::Matrix(kMatrix), nullptr);
  ...
  SkPoint3 diffLocation = SkPoint3::Make(
      spotTarget.fX + 50 * cosAzimuth, spotTarget.fY + 50 * sinAzimuth, SkIntToScalar(10));
  SkPoint3 specLocation = SkPoint3::Make(
      spotTarget.fX - 50 * sinAzimuth, spotTarget.fY + 50 * cosAzimuth, SkIntToScalar(10));
  ...
  sk_sp<SkImageFilter> specular = SkLightingImageFilter::MakePointLitSpecular(
      specLocation, SK_ColorRED, /* scale */ 1.f, /* ks */ 1.f, /* shine */ 8.f, srcToAlpha, &crop);
...
static sk_sp<SkImageFilter> tile_factory(sk_sp<SkImage> auxImage, const SkIRect* cropRect) {
  ...
  return SkTileImageFilter::Make(
      SkRect::MakeLTRB(25, 25, 75, 75), SkRect::MakeWH(100, 100), nullptr);
}
...
class ImageMakeWithFilterGM : public skiagm::GM {
 ...
 protected:
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    SkIRect clipBounds[]{{-20, -20, 100, 100}, {0, 0, 75, 75},   {20, 20, 100, 100},
                         {-20, -20, 50, 50},   {20, 20, 50, 50}, {30, 30, 75, 75}};
 ...
 private:
  ...
  void drawImageWithFilter(
      SkCanvas* canvas, sk_sp<SkImage> mainImage, sk_sp<SkImage> auxImage,
      FilterFactory filterFactory, const SkIRect& clip, const SkIRect& subset, SkIRect* dstRect) {
    ...
    sk_sp<SkImageFilter> filter = filterFactory(auxImage, fFilterWithCropRect ? &clip : nullptr);
