...
static sk_sp<SkImage> make_compressed_image(
    GrDirectContext* dContext, const SkISize dimensions, SkColorType colorType, bool opaque,
  ...
  if (dContext) {
    image = SkImage::MakeTextureFromCompressed(
        dContext, std::move(tmp), dimensions.width(), dimensions.height(), compression,
        GrMipmapped::kYes);
  } else {
 ...
 protected:
  ...
  DrawResult onGpuSetup(GrDirectContext* dContext, SkString* errorMsg) override {
    ...
    if (dContext && dContext->backend() == GrBackendApi::kDirect3D &&
        fType == Type::kNonMultipleOfFour) {
      // skbug.com/10541 - Are non-multiple-of-four BC1 textures supported in D3D?
      return DrawResult::kSkip;
    }

    fOpaqueETC2Image = make_compressed_image(
        dContext, fImgDimensions, kRGB_565_SkColorType, true,
        SkImage::CompressionType::kETC2_RGB8_UNORM);

    fOpaqueBC1Image = make_compressed_image(
        dContext, fImgDimensions, kRGBA_8888_SkColorType, true,
        SkImage::CompressionType::kBC1_RGB8_UNORM);

    fTransparentBC1Image = make_compressed_image(
        dContext, fImgDimensions, kRGBA_8888_SkColorType, false,
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    this->drawCell(
        canvas, fTransparentBC1Image.get(), {2 * kPad + kCellWidth, 2 * kPad + kBaseTexHeight});
  }
 ...
 private:
  void drawCell(SkCanvas* canvas, SkImage* image, SkIVector offset) {
    ...
    if (image->isTextureBacked()) {
      ...
      GrTextureProxy* proxy = sk_gpu_test::GetTextureImageProxy(image, canvas->recordingContext());
