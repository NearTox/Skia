...
class SrcRectConstraintGM : public skiagm::GM {
 public:
  SrcRectConstraintGM(const char* shortName, SkCanvas::SrcRectConstraint constraint, bool batch)
      : fShortName(shortName), fConstraint(constraint), fBatch(batch) {
    // Make sure GPU SkSurfaces can be created for this GM.
    SkASSERT(
        this->onISize().width() <= kMaxTextureSize && this->onISize().height() <= kMaxTextureSize);
  }
 ...
 protected:
  ...
  void drawImage(
    ...
    if (fBatch) {
      ...
      imageSetEntry[0].fAAFlags =
          paint->isAntiAlias() ? SkCanvas::kAll_QuadAAFlags : SkCanvas::kNone_QuadAAFlags;
      canvas->experimental_DrawEdgeAAImageSet(
          imageSetEntry, SK_ARRAY_COUNT(imageSetEntry),
          /*dstClips=*/nullptr,
          /*preViewMatrices=*/nullptr, sampling, paint, fConstraint);
    } else {
  ...
  void drawCase1(
      SkCanvas* canvas, int transX, int transY, bool aa, const SkSamplingOptions& sampling) {
  ...
  void drawCase3(
    ...
    SkRect src = SkRect::MakeXYWH(
        fBigSrcRect.fLeft, fBigSrcRect.fTop, fBigSrcRect.width() / 2, fBigSrcRect.height() / 2);
    SkRect dst = SkRect::MakeXYWH(
        SkIntToScalar(transX), SkIntToScalar(transY), SkIntToScalar(kBlockSize),
        SkIntToScalar(kBlockSize));
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    m.setTranslate(
        std::max({corners[0].fX, corners[1].fX, corners[2].fX, corners[3].fX}),
    ...
    for (bool antiAlias : {false, true}) {
      ...
      for (const SkMatrix& matrix : matrices) {
        ...
        SkScalar x = kBlockSize + std::max(
                                      {innerCorners[0].fX, innerCorners[1].fX, innerCorners[2].fX,
                                       innerCorners[3].fX});
        maxX = std::max(maxX, x);
        canvas->restore();
      }
...
DEF_GM(return new SrcRectConstraintGM(
                  "strict_constraint_no_red_allowed", SkCanvas::kStrict_SrcRectConstraint,
                  /*batch=*/false););
DEF_GM(return new SrcRectConstraintGM(
                  "strict_constraint_batch_no_red_allowed", SkCanvas::kStrict_SrcRectConstraint,
                  /*batch=*/true););
DEF_GM(return new SrcRectConstraintGM(
                  "fast_constraint_red_is_allowed", SkCanvas::kFast_SrcRectConstraint,
                  /*batch=*/false););
  ...
  for (auto constraint : constraints) {
    ...
    for (auto sampling : samplings) {
      ...
      surf->getCanvas()->drawImageRect(
          img, src, SkRect::MakeWH(1, 1), sampling, nullptr, constraint);
      // now blow up the 1 pixel result
      canvas->drawImageRect(
          surf->makeImageSnapshot(), SkRect::MakeWH(100, 100), SkSamplingOptions());
