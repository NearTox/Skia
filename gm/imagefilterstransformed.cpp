  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    sk_sp<SkImageFilter> filters[] = {
        ...
        SkImageFilters::DisplacementMap(
            SkColorChannel::kR, SkColorChannel::kR, 12, std::move(gradient), checkerboard),
    ...
    for (size_t j = 0; j < 3; j++) {
      ...
      for (size_t i = 0; i < SK_ARRAY_COUNT(filters); ++i) {
        ...
        canvas->translate(-size * SK_ScalarHalf, -size * SK_ScalarHalf);
        canvas->drawOval(
            SkRect::MakeXYWH(0, size * SkDoubleToScalar(0.1), size, size * SkDoubleToScalar(0.6)),
            paint);
        canvas->restore();
...
class ImageFilterMatrixWLocalMatrix : public skiagm::GM {
 ...
 protected:
  SkString onShortName() override { return SkString("imagefilter_matrix_localmatrix"); }
  ...
  void onOnceBeforeDraw() override { fImage = GetResourceAsImage("images/mandrill_256.png"); }
...
class ImageFilterComposedTransform : public skiagm::GM {
 ...
 protected:
  SkString onShortName() override { return SkString("imagefilter_composed_transform"); }
 ...
 private:
  ...
  sk_sp<SkImageFilter> makeDirectFilter(const SkMatrix& matrix) const {
    ...
    filter = SkImageFilters::MatrixTransform(
        matrix, SkSamplingOptions(SkFilterMode::kLinear), std::move(filter));
...
DEF_SIMPLE_GM(imagefilter_transformed_image, canvas, 256, 256) {
  ...
  SkM44 m1 =
      SkM44::Translate(0.9f * image->width(), 0.1f * image->height()) * SkM44::Scale(-.8f, .8f);

  SkM44 m2 = SkM44::RectToRect({-1.f, -1.f, 1.f, 1.f}, imageRect) *
             SkM44::Perspective(0.01f, 100.f, SK_ScalarPI / 3.f) *
             SkM44::Translate(0.f, 0.f, -2.f) * SkM44::Rotate({0.f, 1.f, 0.f}, SK_ScalarPI / 6.f) *
