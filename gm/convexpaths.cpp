...
class ConvexPathsGM : public skiagm::GM {
  ...
  void makePaths() {
    ...
    fPaths.push_back(
        SkPath::RRect(SkRRect::MakeRectXY({0, 0, 100, 100}, 40, 20), SkPathDirection::kCW));
    ...
    fPaths.push_back(b.arcTo(SkRect::MakeXYWH(0, 0, 50, 100), 25, 130, false).detach());
    ...
    // moveTo only paths
    fPaths.push_back(b.moveTo(0, 0).moveTo(0, 0).moveTo(1, 1).moveTo(1, 1).moveTo(10, 10).detach());
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    for (int i = 0; i < fPaths.count(); ++i) {
      ...
      canvas->translate(200.0f * (i % 5) + 1.0f / 10, 200.0f * (i / 5) + 9.0f / 10);
