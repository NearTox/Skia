...
class CrBug224618GM : public skiagm::GM {
 ...
 protected:
  SkString onShortName() override { return SkString("crbug_224618"); }

  SkISize onISize() override { return SkISize::Make(kMaxVW, kMaxVW); }
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    SkM44 proj{1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f,           0.f,
               0.f, 0.f, 1.f, 0.f, 0.f, 0.f, -1.f / radius, 1.f};
    ...
    for (int i = 0; i < 6; ++i) {
      SkM44 model = SkM44::Rotate(
          {axisAngles[i].x, axisAngles[i].y, axisAngles[i].z}, SkDegreesToRadians(axisAngles[i].w));
      ...
      canvas->drawImageRect(
          fCubeImage.get(), SkRect::MakeWH(fCubeImage->width(), fCubeImage->height()),
          SkRect::MakeWH(viewportWidth, viewportWidth), SkSamplingOptions(SkFilterMode::kLinear),
          &fillPaint, SkCanvas::kFast_SrcRectConstraint);
