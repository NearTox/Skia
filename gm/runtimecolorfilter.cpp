...
DEF_SIMPLE_GM(runtimecolorfilter_vertices_atlas_and_patch, canvas, 404, 404) {
  ...
  auto info = SkImageInfo::Make(
      {128, 128}, kRGBA_8888_SkColorType, kPremul_SkAlphaType, canvas->imageInfo().refColorSpace());
  ...
  const SkPoint cubics[12] = {pos[0], pos[0] + vx, pos[1] - vx, pos[1], pos[1] + vy, pos[2] - vy,
                              pos[2], pos[2] - vx, pos[3] + vx, pos[3], pos[3] - vy, pos[0] + vy};
  ...
  auto drawAtlas = [&](float x, bool useCF) {
    ...
    canvas->drawAtlas(
        atlas.get(), &xform, &r, &kColor, 1, SkBlendMode::kModulate, SkSamplingOptions{}, nullptr,
        &paint);
  };
