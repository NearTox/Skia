...
class ImageFromYUVTextures : public GpuGM {
 public:
  ImageFromYUVTextures() { this->setBGColor(0xFFFFFFFF); }
 ...
 protected:
  ...
  void onOnceBeforeDraw() override {
    ...
    SkPaint paint;
    constexpr SkColor kColors[] = {SK_ColorBLUE, SK_ColorYELLOW, SK_ColorGREEN, SK_ColorWHITE};
    paint.setShader(SkGradientShader::MakeRadial(
        SkPoint::Make(0, 0), kBmpSize / 2.f, kColors, nullptr, SK_ARRAY_COUNT(kColors),
        SkTileMode::kMirror));
    SkBitmap rgbBmp;
    ...
    SkImageInfo yinfo =
        SkImageInfo::Make(kBmpSize, kBmpSize, kGray_8_SkColorType, kUnpremul_SkAlphaType);
    fYUVBmps[0].allocPixels(yinfo);
    SkImageInfo uinfo =
        SkImageInfo::Make(kBmpSize / 2, kBmpSize / 2, kGray_8_SkColorType, kUnpremul_SkAlphaType);
    ...
    for (int i = 0; i < kBmpSize * kBmpSize; ++i) {
      yPixels[i] = static_cast<unsigned char>(
          0.299f * SkGetPackedR32(rgbColors[i]) + 0.587f * SkGetPackedG32(rgbColors[i]) +
    ...
    for (int j = 0; j < kBmpSize / 2; ++j) {
      for (int i = 0; i < kBmpSize / 2; ++i) {
        ...
        uvPixels[1][uvIndex] =
            static_cast<signed char>(((112 * rgb[0] - 94 * rgb[1] - 18 * rgb[2] + 128) >> 8) + 128);
      }
    }
  ...
  void createYUVTextures(GrContext* context, GrBackendTexture yuvTextures[3]) {
    for (int i = 0; i < 3; ++i) {
      ...
      yuvTextures[i] = context->priv().createBackendTexture(
          &fYUVBmps[i].pixmap(), 1, GrRenderable::kNo, GrProtected::kNo);
    }
  }

  void createResultTexture(
      GrContext* context, int width, int height, GrBackendTexture* resultTexture) {
    *resultTexture = context->createBackendTexture(
        width, height, kRGBA_8888_SkColorType, SkColors::kTransparent, GrMipMapped::kNo,
  ...
  void onDraw(GrContext* context, GrRenderTargetContext*, SkCanvas* canvas) override {
    ...
    for (int space = kJPEG_SkYUVColorSpace; space <= kLastEnum_SkYUVColorSpace; ++space) {
      ...
      auto image = SkImage::MakeFromYUVTexturesCopy(
          context, static_cast<SkYUVColorSpace>(space), yuvTextures, kTopLeft_GrSurfaceOrigin);
