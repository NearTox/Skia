...
class ImageFromYUVTextures : public GpuGM {
 ...
 protected:
  ...
  static SkBitmap CreateBmpAndPlanes(const char* name, SkBitmap yuvaBmps[4]) {
    ...
    SkImageInfo yaInfo =
        SkImageInfo::Make(rgbaBmp.dimensions(), kAlpha_8_SkColorType, kUnpremul_SkAlphaType);
    ...
    for (int j = 0; j < yaInfo.height(); ++j) {
      for (int i = 0; i < yaInfo.width(); ++i) {
        ...
        yuvPixels[0][j * yaInfo.width() + i] =
            SkToU8(sk_float_round2int(m[0] * r + m[1] * g + m[2] * b + m[3] * a + 255 * m[4]));
        yuvPixels[3][j * yaInfo.width() + i] =
            SkToU8(sk_float_round2int(m[15] * r + m[16] * g + m[17] * b + m[18] * a + 255 * m[19]));
      }
    }
  ...
  static bool CreateYUVBackendTextures(
      GrDirectContext* context, SkBitmap bmps[4], SkYUVAIndex indices[4],
    ...
    for (int i = 0; i < 4; ++i) {
      GrBackendTexture tmp = context->createBackendTexture(
          ...
          YUVABackendReleaseContext::CreationCompleteProc(i), beContext);
      if (!tmp.isValid()) {
        return false;
      }
  ...
  sk_sp<SkImage> makeYUVAImage(GrDirectContext* context) {
    ...
    return SkImage::MakeFromYUVATextures(
        context, kJPEG_SkYUVColorSpace, releaseContext->beTextures(), indices,
        fRGBABmp.dimensions(), kTopLeft_GrSurfaceOrigin, nullptr,
  ...
  sk_sp<SkImage> createReferenceImage(GrDirectContext* dContext) {
    ...
    GrBackendTexture resultTexture = dContext->createBackendTexture(
        fRGBABmp.dimensions().width(), fRGBABmp.dimensions().height(), kRGBA_8888_SkColorType,
        SkColors::kTransparent, GrMipmapped::kNo, GrRenderable::kYes, GrProtected::kNo,
        YUVABackendReleaseContext::CreationCompleteProc(0), rgbaReleaseContext);
    ...
    auto tmp = SkImage::MakeFromYUVATexturesCopyWithExternalBackend(
        dContext, kJPEG_SkYUVColorSpace, planeReleaseContext->beTextures(), indices,
        fRGBABmp.dimensions(), kTopLeft_GrSurfaceOrigin, resultTexture, nullptr,
        YUVABackendReleaseContext::Release, rgbaReleaseContext);
