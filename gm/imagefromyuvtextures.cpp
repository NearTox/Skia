...
class ImageFromYUVTextures : public GM {
 ...
 protected:
  ...
  static std::unique_ptr<sk_gpu_test::LazyYUVImage> CreatePlanes(const char* name) {
    ...
    SkYUVAPixmapInfo pixmapInfo(
        {bmp.dimensions(), SkYUVAInfo::PlaneConfig::kY_U_V_A, SkYUVAInfo::Subsampling::k420,
    ...
    for (int j = 0; j < pixmaps.planes()[0].height(); ++j) {
      for (int i = 0; i < pixmaps.planes()[0].width(); ++i) {
        ...
        yuvPixels[0][j * pixmaps.planes()[0].width() + i] =
            SkToU8(sk_float_round2int(m[0] * r + m[1] * g + m[2] * b + m[3] * a + 255 * m[4]));
        yuvPixels[3][j * pixmaps.planes()[0].width() + i] =
  ...
  sk_sp<SkImage> createReferenceImage(GrDirectContext* dContext) {
    ...
    auto resultInfo =
        SkImageInfo::Make(fLazyYUVImage->dimensions(), kRGBA_8888_SkColorType, kPremul_SkAlphaType);
    auto resultSurface = SkSurface::MakeRenderTarget(
        dContext, SkBudgeted::kYes, resultInfo, 1, kTopLeft_GrSurfaceOrigin, nullptr);
    if (!resultSurface) {
      return nullptr;
    }
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    auto draw_image_rect = [canvas](SkImage* image, const SkSamplingOptions& sampling) -> SkSize {
      if (!image) {
        return {0, 0};
      }
      ...
      canvas->drawImageRect(
          image, subset, dst, sampling, nullptr, SkCanvas::kStrict_SrcRectConstraint);
      return {dst.width(), dst.height()};
    };

    auto draw_image_shader = [canvas](SkImage* image, const SkSamplingOptions& sampling) -> SkSize {
      if (!image) {
        return {0, 0};
      }
      SkMatrix m;
      ...
      SkPaint paint;
      paint.setShader(image->makeShader(SkTileMode::kMirror, SkTileMode::kDecal, sampling, m));
