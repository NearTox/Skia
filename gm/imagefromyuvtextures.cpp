 ...
 protected:
  ...
  void createYUVTextures(SkBitmap bmps[4], GrContext* context, GrBackendTexture textures[4]) {
    for (int i = 0; i < 4; ++i) {
      ...
      textures[i] =
          context->createBackendTexture(tmp.pixmap(), GrRenderable::kNo, GrProtected::kNo);
    }
  }
  ...
  void onDraw(GrContext* context, GrRenderTargetContext*, SkCanvas* canvas) override {
    ...
    auto image1 = SkImage::MakeFromYUVATextures(
        context, kJPEG_SkYUVColorSpace, yuvaTextures, kIndices, fRGBABmp.dimensions(),
        kTopLeft_GrSurfaceOrigin);

    GrBackendTexture resultTexture;
    this->createResultTexture(context, fRGBABmp.dimensions(), &resultTexture);
    auto image2 = SkImage::MakeFromYUVTexturesCopyWithExternalBackend(
        context, kJPEG_SkYUVColorSpace, yuvaTextures, kTopLeft_GrSurfaceOrigin, resultTexture);
