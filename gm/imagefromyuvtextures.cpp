...
class ImageFromYUVTextures : public GpuGM {
 ...
 protected:
  ...
  static bool CreateYUVBackendTextures(
      GrContext* context, SkBitmap bmps[4], SkYUVAIndex indices[4],
      YUVABackendReleaseContext* beContext) {
    for (int i = 0; i < 4; ++i) {
      GrBackendTexture tmp =
          context->createBackendTexture(bmps[i].pixmap(), GrRenderable::kNo, GrProtected::kNo);
      if (!tmp.isValid()) {
        return false;
      }
  ...
  sk_sp<SkImage> createReferenceImage(GrContext* context) {
    ...
    auto tmp = SkImage::MakeFromYUVATexturesCopyWithExternalBackend(
        context, kJPEG_SkYUVColorSpace, planeReleaseContext->beTextures(), indices,
