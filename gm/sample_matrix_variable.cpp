...
class GLSLSampleMatrixVariableEffect : public GrGLSLFragmentProcessor {
  void emitCode(EmitArgs& args) override {
    ...
    SkString sample2 = this->invokeChildWithMatrix(
        0, args,
        SkStringPrintf(
            "float3x3(1, -1, 0, 1, 0, 0, "
            "%g, %g, 1)",
            smve.fXOffset, smve.fYOffset)
            .c_str());
    fragBuilder->codeAppendf(
        "%s = (%s + %s) / 2;\n", args.fOutputColor, sample1.c_str(), sample2.c_str());
  }
};
...
DEF_SIMPLE_GPU_GM(sample_matrix_variable, ctx, rtCtx, canvas, 512, 256) {
  auto draw =
      [rtCtx](std::unique_ptr<GrFragmentProcessor> baseFP, float ofsX, float ofsY, int tx, int ty) {
...

  {
    ...
    auto shader = SkGradientShader::MakeLinear(
        pts, colors, nullptr, SK_ARRAY_COUNT(colors), SkTileMode::kRepeat);
    SkMatrix matrix;
