...
class ImageScaleAlignedGM : public skiagm::GM {
 ...
 private:
  ...
  static sk_sp<SkImage> MakeImage(const SkVector& vec, SkColor color) {
    ...
    const SkPoint end =
        SkPoint::Make(start.x() + vec.x() * (kSegLen - 1), start.y() + vec.y() * (kSegLen - 1));
    ...
    for (int i = 0; i < fSets.count(); ++i) {
      ...
      for (size_t j = 0; j < SK_ARRAY_COUNT(AAs); ++j) {
        ...
        for (size_t k = 0; k < SK_ARRAY_COUNT(samplings); ++k) {
          ...
          canvas->translate((kSegLen + 4) * set.fVector.y(), (kSegLen + 4) * set.fVector.x());
        }
      }
      canvas->translate(
          lastPt.x() + kSegLen,
  ...
  SkPoint drawSet(
      SkCanvas* canvas, const ImageSet& set, const SkSamplingOptions& sampling,
