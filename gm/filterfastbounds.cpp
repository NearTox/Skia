...
constexpr drawMth gDrawMthds[] = {draw_rect, draw_oval,   draw_rrect, draw_drrect,
                                  draw_path, draw_points, draw_bitmap};
...
static void create_paints(SkTArray<SkPaint>* paints, sk_sp<SkImageFilter> source) {
...

  {
    ...
    sk_sp<SkImageFilter> rotMIF(SkImageFilter::MakeMatrixFilter(rot, kLow_SkFilterQuality, source));
...

  {
    ...
    sk_sp<SkImageFilter> dsif(
        SkDropShadowImageFilter::Make(10.0f, 10.0f, 3.0f, 3.0f, SK_ColorRED, kBoth, source));

    add_paint(paints, std::move(dsif));
  }

  {
    sk_sp<SkImageFilter> dsif(SkDropShadowImageFilter::Make(
        27.0f, 27.0f, 3.0f, 3.0f, SK_ColorRED, SkDropShadowImageFilter::kDrawShadowOnly_ShadowMode,
        source));
...
class ImageFilterFastBoundGM : public GM {
 public:
  ImageFilterFastBoundGM() { this->setBGColor(0xFFCCCCCC); }
 ...
 protected:
  ...
  SkISize onISize() override {
    return SkISize::Make(
        (SK_ARRAY_COUNT(gDrawMthds) + kNumXtraCols) * kTileWidth, kNumVertTiles * kTileHeight);
  }

  static void draw_geom_with_paint(
      drawMth draw, const SkIPoint& off, SkCanvas* canvas, const SkPaint& p) {
    SkPaint redStroked;
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    for (int i = 1; i < paints.count(); ++i) {
      canvas->drawLine(
          0, i * SkIntToScalar(kTileHeight),
          SkIntToScalar((SK_ARRAY_COUNT(gDrawMthds) + kNumXtraCols) * kTileWidth),
          i * SkIntToScalar(kTileHeight), blackFill);
    }
    // vertical separators
    for (int i = 0; i < (int)SK_ARRAY_COUNT(gDrawMthds) + kNumXtraCols; ++i) {
      canvas->drawLine(
          SkIntToScalar(i * kTileWidth), 0, SkIntToScalar(i * kTileWidth),
          SkIntToScalar(paints.count() * kTileWidth), blackFill);
    }

    // A column of saveLayers with PictureImageFilters
    for (int i = 0; i < pifPaints.count(); ++i) {
      draw_savelayer_with_paint(SkIPoint::Make(0, i * kTileHeight), canvas, pifPaints[i]);
    }

    // A column of saveLayers with BitmapSources
    for (int i = 0; i < pifPaints.count(); ++i) {
      draw_savelayer_with_paint(SkIPoint::Make(kTileWidth, i * kTileHeight), canvas, bmsPaints[i]);
    }
    ...
    for (int i = 0; i < (int)SK_ARRAY_COUNT(gDrawMthds); ++i) {
      for (int j = 0; j < paints.count(); ++j) {
        draw_geom_with_paint(
            *gDrawMthds[i], SkIPoint::Make((i + kNumXtraCols) * kTileWidth, j * kTileHeight),
            canvas, paints[j]);
      }
    }
  }
