...
static SkScalar draw_string(
    SkAtlasTextTarget* target, const SkString& text, SkScalar x, SkScalar y, uint32_t color,
    sk_sp<SkTypeface> typeface, float size) {
  if (!text.size()) {
    return x;
  }
...
class AtlasTextGM : public skiagm::GpuGM {
 ...
 protected:
  ...
  DrawResult onDraw(
      GrContext*, GrRenderTargetContext*, SkCanvas* canvas, SkString* errorMsg) override {
    if (!fRenderer) {
      ...
      for (auto s : kSizes) {
        ...
        for (const auto& typeface : fTypefaces) {
          for (const auto& text : kTexts) {
            ...
            x += size + draw_string(fTarget.get(), text, drawX, drawY, color, typeface, size);
