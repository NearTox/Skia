...
static void draw_label(SkCanvas* canvas, const char* label, const SkPoint& offset) {
  ...
  canvas->drawSimpleText(
      label, len, SkTextEncoding::kUTF8, offset.x() - width / 2, offset.y(), font, SkPaint());
}

static void draw_scene(
    SkCanvas* canvas, const sk_sp<SkColorFilter>& filter, SkBlendMode mode,
 ...
 protected:
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    for (size_t i = 0; i < SK_ARRAY_COUNT(modes); ++i) {
      draw_label(canvas, SkBlendMode_Name(modes[i]), SkPoint::Make(gridStep * (0.5f + i), 20));
    }
...
DEF_SIMPLE_GM(AlternateLuma, canvas, 384, 128) {
  ...
  auto [effect, err] = SkRuntimeEffect::MakeForColorFilter(
      SkString{"half4 main(half4 inColor) { return inColor.ggga; }"});
  ...
  paint.setColorFilter(SkColorFilterPriv::WithWorkingFormat(
      std::move(filter), &SkNamedTransferFn::kLinear, &SkNamedGamut::kXYZ, &unpremul));
  canvas->drawImage(img, 0, 0, SkSamplingOptions{}, &paint);
}
