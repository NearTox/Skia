...
static bool nearly_equal(SkColor4f x, SkColor4f y) {
    const float K = 0.01f;
    return fabsf(x.fR - y.fR) < K && fabsf(x.fG - y.fG) < K && fabsf(x.fB - y.fB) < K &&
           fabsf(x.fA - y.fA) < K;
}
...
static SkColor4f transform(SkColor4f c, SkColorSpace* src, SkColorSpace* dst) {
    SkColorSpaceXformSteps(src, kUnpremul_SkAlphaType, dst, kUnpremul_SkAlphaType).apply(c.vec());
    return c;
}

static void compare_pixel(const char* label, SkCanvas* canvas, int x, int y, SkColor4f color,
                          SkColorSpace* cs) {
    SkPaint paint;
    SkFont font;
    ...
    SkBitmap bm;
    bm.allocPixels(
            SkImageInfo::Make(1, 1, kRGBA_F32_SkColorType, kUnpremul_SkAlphaType, canvas_cs));
...
DEF_SIMPLE_GM(p3, canvas, 450, 1300) {
    ...
    // Draw a P3 red rectangle and check the corner.
    {
        ...
        compare_pixel("drawRect P3 red ", canvas, 10, 10, {1, 0, 0, 1}, p3.get());
    }
    ...
    // Draw a P3 red bitmap, using a draw.
    {
        ...
        compare_pixel("drawBitmap P3 red, from drawPaint", canvas, 10, 10, {1, 0, 0, 1}, p3.get());
    }
    ...
    // Draw a P3 red bitmap, using SkBitmap::eraseColor().
    {
        ...
        canvas->drawBitmap(bm, 10, 10);
        compare_pixel("drawBitmap P3 red, from SkBitmap::eraseColor()", canvas, 10, 10,
                      {1, 0, 0, 1}, p3.get());
    }
    ...
    // Draw a P3 red bitmap, using SkPixmap::erase().
    {
        ...
        canvas->drawBitmap(bm, 10, 10);
        compare_pixel("drawBitmap P3 red, from SkPixmap::erase", canvas, 10, 10, {1, 0, 0, 1},
                      p3.get());
    }
    ...
    // Draw a P3 red bitmap wrapped in a shader, using SkPixmap::erase().
    {
        ...
        canvas->drawRect({10, 10, 70, 70}, paint);
        compare_pixel("drawBitmapAsShader P3 red, from SkPixmap::erase", canvas, 10, 10,
                      {1, 0, 0, 1}, p3.get());
    }
    ...
    // Draw a gradient from P3 red to P3 green interpolating in unpremul P3, checking the corners.
    {
        ...
        SkPaint paint;
        paint.setShader(SkGradientShader::MakeLinear(points, colors, p3, nullptr,
                                                     SK_ARRAY_COUNT(colors), SkTileMode::kClamp));
        canvas->drawRect({10, 10, 70, 70}, paint);
        canvas->save();
        compare_pixel("UPM P3 gradient, P3 red", canvas, 10, 10, {1, 0, 0, 1}, p3.get());

        canvas->translate(180, 0);

        compare_pixel("UPM P3 gradient, P3 green", canvas, 69, 69, {0, 1, 0, 1}, p3.get());
        canvas->restore();
    }
    ...
    // Draw a gradient from P3 red to P3 green interpolating in premul P3, checking the corners.
    {
        ...
        paint.setShader(SkGradientShader::MakeLinear(
                points, colors, p3, nullptr, SK_ARRAY_COUNT(colors), SkTileMode::kClamp,
                SkGradientShader::kInterpolateColorsInPremul_Flag, nullptr /*local matrix*/));
        canvas->drawRect({10, 10, 70, 70}, paint);
        canvas->save();
        compare_pixel("PM P3 gradient, P3 red", canvas, 10, 10, {1, 0, 0, 1}, p3.get());

        canvas->translate(180, 0);

        compare_pixel("PM P3 gradient, P3 green", canvas, 69, 69, {0, 1, 0, 1}, p3.get());
        canvas->restore();
    }
    ...
    // Draw a gradient from P3 red to P3 green interpolating in unpremul sRGB, checking the corners.
    {
        ...
        SkPaint paint;
        paint.setShader(SkGradientShader::MakeLinear(points, colors, srgb, nullptr,
                                                     SK_ARRAY_COUNT(colors), SkTileMode::kClamp));
        canvas->drawRect({10, 10, 70, 70}, paint);
        canvas->save();
        compare_pixel("UPM sRGB gradient, P3 red", canvas, 10, 10, {1, 0, 0, 1}, p3.get());

        canvas->translate(180, 0);

        compare_pixel("UPM sRGB gradient, P3 green", canvas, 69, 69, {0, 1, 0, 1}, p3.get());
        canvas->restore();
    }
    ...
    // Draw a gradient from P3 red to P3 green interpolating in premul sRGB, checking the corners.
    {
        ...
        SkPaint paint;
        paint.setShader(SkGradientShader::MakeLinear(
                points, colors, srgb, nullptr, SK_ARRAY_COUNT(colors), SkTileMode::kClamp,
                SkGradientShader::kInterpolateColorsInPremul_Flag, nullptr /*local matrix*/));
        canvas->drawRect({10, 10, 70, 70}, paint);
        canvas->save();
        compare_pixel("PM sRGB gradient, P3 red", canvas, 10, 10, {1, 0, 0, 1}, p3.get());

        canvas->translate(180, 0);

        compare_pixel("PM sRGB gradient, P3 green", canvas, 69, 69, {0, 1, 0, 1}, p3.get());
        canvas->restore();
    }
    ...
    // Leon's blue -> green -> red gradient, interpolating in premul.
    {
        ...
        canvas->drawRect({10, 10, 70, 70}, paint);
        canvas->save();
        compare_pixel("Leon's gradient, P3 blue", canvas, 10, 10, {0, 0, 1, 1}, p3.get());

        canvas->translate(180, 0);

        compare_pixel("Leon's gradient, P3 red", canvas, 10, 69, {1, 0, 0, 1}, p3.get());
        canvas->restore();
    }
    ...
    // Draw an A8 image with a P3 red, scaled and not, as a shader or bitmap.
    {
        ...
        compare_pixel("A8 sprite bitmap P3 red", canvas, 10, 10, {1, 0, 0, 1}, p3.get());
        ...
        canvas->restore();
        compare_pixel("A8 sprite shader P3 red", canvas, 10, 10, {1, 0, 0, 1}, p3.get());

        canvas->translate(0, 80);

        canvas->drawBitmapRect(bm, {10, 10, 70, 70}, &as_bitmap);
        compare_pixel("A8 scaled bitmap P3 red", canvas, 10, 10, {1, 0, 0, 1}, p3.get());
        ...
        canvas->drawRect({0, 0, 16, 16}, as_shader);
        canvas->restore();
        compare_pixel("A8 scaled shader P3 red", canvas, 10, 10, {1, 0, 0, 1}, p3.get());
    }
...
DEF_SIMPLE_GM(p3_ovals, canvas, 450, 320) {
    ...
    // Draw a circle and check the center (CircleOp)
    {
        ...
        compare_pixel("drawCircle P3 red ", canvas, 40, 40, {1, 0, 0, 1}, p3.get());
    }
    ...
    // Draw an oval and check the center (EllipseOp)
    {
        ...
        compare_pixel("drawOval P3 red ", canvas, 40, 40, {1, 0, 0, 1}, p3.get());
    }
    ...
    // Draw a butt-capped dashed circle and check the top of the stroke (ButtCappedDashedCircleOp)
    {
        ...
        canvas->drawCircle(40, 40, 30, paint);
        compare_pixel("drawDashedCircle P3 red ", canvas, 40, 10, {1, 0, 0, 1}, p3.get());
    }
    ...
    // Draw an oval with rotation and check the center (DIEllipseOp)
    {
        ...
        canvas->restore();
        compare_pixel("drawRotatedOval P3 red ", canvas, 40, 40, {1, 0, 0, 1}, p3.get());
    }
