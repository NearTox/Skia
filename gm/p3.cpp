...
static void compare_pixel(
    const char* label, SkCanvas* canvas, int x, int y, SkColor4f color, SkColorSpace* cs) {
  SkPaint paint;
  SkFont font;
...
DEF_SIMPLE_GM(p3, canvas, 450, 1300) {
  ...
  // Draw a P3 red bitmap, using SkPixmap::erase().
  {
    ...
    canvas->drawBitmap(bm, 10, 10);
    compare_pixel(
        "drawBitmap P3 red, from SkPixmap::erase", canvas, 10, 10, {1, 0, 0, 1}, p3.get());
  }
  ...
  // Draw a P3 red bitmap wrapped in a shader, using SkPixmap::erase().
  {
    ...
    SkPaint paint;
    paint.setShader(bm.makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions()));

    canvas->drawRect({10, 10, 70, 70}, paint);
    compare_pixel(
        "drawBitmapAsShader P3 red, from SkPixmap::erase", canvas, 10, 10, {1, 0, 0, 1}, p3.get());
  }
  ...
  // Draw an A8 image with a P3 red, scaled and not, as a shader or bitmap.
  {
    ...
    as_shader.setShader(
        bm.makeShader(SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kNone)));
