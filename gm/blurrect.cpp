...
static sk_sp<SkShader> make_radial() {
  ...
  return SkGradientShader::MakeTwoPointConical(
      center1, (pts[1].fX - pts[0].fX) / 7, center0, (pts[1].fX - pts[0].fX) / 2, colors, pos,
      SK_ARRAY_COUNT(colors), tm, 0, &scale);
}
  ...
  void onOnceBeforeDraw() override {
    for (int i = 0; i <= kLastEnum_SkBlurStyle; ++i) {
      fMaskFilters[i] = SkMaskFilter::MakeBlur(
          (SkBlurStyle)i, SkBlurMask::ConvertRadiusToSigma(SkIntToScalar(STROKE_WIDTH / 2)));
    }
  }
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    for (size_t s = 0; s < SK_ARRAY_COUNT(scales); ++s) {
      ...
      for (size_t f = 0; f < SK_ARRAY_COUNT(fMaskFilters); ++f) {
        ...
        constexpr Proc procs[] = {fill_rect, draw_donut, draw_donut_skewed};
  ...
  for (size_t i = 0; i < SK_ARRAY_COUNT(widths); i++) {
    ...
    for (size_t j = 0; j < SK_ARRAY_COUNT(radii); j++) {
      ...
      for (size_t k = 0; k < SK_ARRAY_COUNT(styles); k++) {
        ...
        SkMask mask;
        if (!SkBlurMask::BlurRect(SkBlurMask::ConvertRadiusToSigma(radius), &mask, r, style)) {
          continue;
        }
        ...
        if (bm.height() > max_height) max_height = bm.height();
      }
    }
  }
}
...
class BlurRectCompareGM : public GM {
 protected:
  ...
  DrawResult onDraw(SkCanvas* canvas, SkString* errorMsg) override {
    ...
    int32_t ctxID = canvas->recordingContext() ? canvas->recordingContext()->priv().contextID() : 0;
      ...
      for (size_t sigmaIdx = 0; sigmaIdx < kNumSigmas; ++sigmaIdx) {
        ...
        for (size_t heightIdx = 0; heightIdx < kNumSizes; ++heightIdx) {
          ...
          for (size_t widthIdx = 0; widthIdx < kNumSizes; ++widthIdx) {
            ...
            SkImage* img;
            switch (mode) {
              case 0: img = fReferenceMasks[sigmaIdx][heightIdx][widthIdx].get(); break;
              case 1: img = fActualMasks[sigmaIdx][heightIdx][widthIdx].get(); break;
              case 2:
      ...
      for (size_t heightIdx = 0; heightIdx < kNumSizes; ++heightIdx) {
        for (size_t widthIdx = 0; widthIdx < kNumSizes; ++widthIdx) {
          ...
          auto ii = SkImageInfo::Make(
              r->width(), r->height(), kRGBA_8888_SkColorType, kPremul_SkAlphaType);
          auto surf = canvas->makeSurface(ii);
          if (!surf) {
            return;
          }
