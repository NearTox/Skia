...
static sk_sp<SkShader> make_radial() {
    ...
    return SkGradientShader::MakeTwoPointConical(center1, (pts[1].fX - pts[0].fX) / 7, center0,
                                                 (pts[1].fX - pts[0].fX) / 2, colors, pos,
...
private:
    void drawProcs(SkCanvas* canvas, const SkRect& r, const SkPaint& paint, bool doClip,
                   const Proc procs[], size_t procsCount) {
    ...
    for (size_t i = 0; i < SK_ARRAY_COUNT(widths); i++) {
        ...
        for (size_t j = 0; j < SK_ARRAY_COUNT(radii); j++) {
            ...
            for (size_t k = 0; k < SK_ARRAY_COUNT(styles); k++) {
                ...
                SkMask mask;
                if (!SkBlurMask::BlurRect(SkBlurMask::ConvertRadiusToSigma(radius), &mask, r,
                                          style)) {
                    continue;
                }
