...
static bool intersect_line_segments(
    const SkPoint& p0, const SkPoint& p1, const SkPoint& l0, const SkPoint& l1,
    SkPoint* intersect) {
...
static void draw_outset_line(
    SkCanvas* canvas, const SkMatrix& local, const SkPoint pts[2], const SkPaint& paint) {
...
class ClipTileRenderer : public SkRefCntBase {
 public:
  ...
  virtual int drawTile(
      SkCanvas* canvas, const SkRect& rect, const SkPoint clip[4], const bool edgeAA[4], int tileID,
      int quadID) = 0;
  ...
  virtual int drawTiles(SkCanvas* canvas) {
    ...
    for (int i = 0; i < kRowCount; ++i) {
      for (int j = 0; j < kColCount; ++j) {
        ...
        // quads as needed.
        int quadCount = 0;
        drawCount += this->clipTile(canvas, tileID, tile, nullptr, edgeAA, lines, 3, &quadCount);
        tileID++;
      }
    }
 ...
 protected:
  SkCanvas::QuadAAFlags maskToFlags(const bool edgeAA[4]) const {
    unsigned flags =
        (edgeAA[0] * SkCanvas::kTop_QuadAAFlag) | (edgeAA[1] * SkCanvas::kRight_QuadAAFlag) |
        (edgeAA[2] * SkCanvas::kBottom_QuadAAFlag) | (edgeAA[3] * SkCanvas::kLeft_QuadAAFlag);
  ...
  // drawTile at leaves.
  int clipTile(
      SkCanvas* canvas, int tileID, const SkRect& baseRect, const SkPoint quad[4],
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      if (intersect_line_segments(
              points[i], points[i == 3 ? 0 : i + 1], lines[0], lines[1], &intersect)) {
    ...
    for (int i = 0; i < subtiles.count(); ++i) {
      ...
      for (int j = 0; j < 4; ++j) {
        ...
        if ((p >= kS0 && (np == s2 || np >= kS0)) || ((np >= kS0) && (p == s2 || p >= kS0))) {
          // New edge
          subAA[j] = false;
        } else {
      ...
      draws +=
          this->clipTile(canvas, tileID, baseRect, sub, subAA, lines + 2, lineCount - 1, quadCount);
    }
    return draws;
  }
};
...
class CompositorGM : public skiagm::GM {
 public:
  CompositorGM(const char* name, std::function<ClipTileRendererArray()> makeRendererFn)
      : fMakeRendererFn(std::move(makeRendererFn)), fName(name) {}
 ...
 protected:
  SkISize onISize() override {
    ...
    return SkISize::Make(
        SkScalarRoundToInt(kCellWidth * kMatrixCount + 175.f),
...
class DebugTileRenderer : public ClipTileRenderer {
 public:
  ...
  int drawTile(
    ...
    canvas->experimental_DrawEdgeAAQuad(rect, clip, aaFlags, c.toSkColor(), SkBlendMode::kSrcOver);
    return 1;
  }
  ...
  void drawBanner(SkCanvas* canvas) override {
    ...
    if (fEnableAAOverride) {
      SkASSERT(
          fAAOverride == SkCanvas::kAll_QuadAAFlags || fAAOverride == SkCanvas::kNone_QuadAAFlags);
 ...
 public:
  ...
  int drawTile(
    ...
    canvas->experimental_DrawEdgeAAQuad(
        rect, clip, this->maskToFlags(edgeAA), fColor.toSkColor(), SkBlendMode::kSrcOver);
    return 1;
  }
...
class TextureSetRenderer : public ClipTileRenderer {
 public:
  static sk_sp<ClipTileRenderer> MakeUnbatched(sk_sp<SkImage> image) {
    return Make("Texture", "", std::move(image), nullptr, nullptr, nullptr, nullptr, 1.f, true, 0);
  }
  ...
  static sk_sp<ClipTileRenderer> MakeBatched(sk_sp<SkImage> image, int transformCount) {
    ...
    return Make(
        "Texture Set", subtitle, std::move(image), nullptr, nullptr, nullptr, nullptr, 1.f, false,
        transformCount);
  }

  static sk_sp<ClipTileRenderer> MakeShader(
      const char* name, sk_sp<SkImage> image, sk_sp<SkShader> shader, bool local) {
    return Make(
        "Shader", name, std::move(image), std::move(shader), nullptr, nullptr, nullptr, 1.f, local,
        0);
  }

  static sk_sp<ClipTileRenderer> MakeColorFilter(
      const char* name, sk_sp<SkImage> image, sk_sp<SkColorFilter> filter) {
    return Make(
        "Color Filter", name, std::move(image), nullptr, std::move(filter), nullptr, nullptr, 1.f,
        false, 0);
  }

  static sk_sp<ClipTileRenderer> MakeImageFilter(
      const char* name, sk_sp<SkImage> image, sk_sp<SkImageFilter> filter) {
    return Make(
        "Image Filter", name, std::move(image), nullptr, nullptr, std::move(filter), nullptr, 1.f,
        false, 0);
  }

  static sk_sp<ClipTileRenderer> MakeMaskFilter(
      const char* name, sk_sp<SkImage> image, sk_sp<SkMaskFilter> filter) {
    return Make(
        "Mask Filter", name, std::move(image), nullptr, nullptr, nullptr, std::move(filter), 1.f,
        false, 0);
  }

  static sk_sp<ClipTileRenderer> MakeAlpha(sk_sp<SkImage> image, SkScalar alpha) {
    return Make(
        "Alpha", SkStringPrintf("a = %.2f", alpha).c_str(), std::move(image), nullptr, nullptr,
        nullptr, nullptr, alpha, false, 0);
  }

  static sk_sp<ClipTileRenderer> Make(
      const char* topBanner, const char* bottomBanner, sk_sp<SkImage> image, sk_sp<SkShader> shader,
      sk_sp<SkColorFilter> colorFilter, sk_sp<SkImageFilter> imageFilter,
      sk_sp<SkMaskFilter> maskFilter, SkScalar paintAlpha, bool resetAfterEachQuad,
      int transformCount) {
    return sk_sp<ClipTileRenderer>(new TextureSetRenderer(
        topBanner, bottomBanner, std::move(image), std::move(shader), std::move(colorFilter),
        std::move(imageFilter), std::move(maskFilter), paintAlpha, resetAfterEachQuad,
        transformCount));
  }
  ...
  int drawTile(
    ...
    SkMatrix gridToImage = SkMatrix::RectToRect(
 ...
 private:
  ...
  TextureSetRenderer(
      const char* topBanner, const char* bottomBanner, sk_sp<SkImage> image, sk_sp<SkShader> shader,
      sk_sp<SkColorFilter> colorFilter, sk_sp<SkImageFilter> imageFilter,
      sk_sp<SkMaskFilter> maskFilter, SkScalar paintAlpha, bool resetEachQuad,
  ...
  int drawAndReset(SkCanvas* canvas) {
    ...
    canvas->experimental_DrawEdgeAAImageSet(
        fSetEntries.begin(), fSetEntries.count(), fDstClips.begin(), fPreViewMatrices.begin(),
        SkSamplingOptions(SkFilterMode::kLinear), &paint, SkCanvas::kFast_SrcRectConstraint);
...
class YUVTextureSetRenderer : public ClipTileRenderer {
 public:
  ...
  int drawTile(
    ...
    fSetEntries.push_back({fImage, localRect, rect, -1, .5f, this->maskToFlags(edgeAA), hasClip});
    return 0;
  }
...
static ClipTileRendererArray make_debug_renderers() {
  return ClipTileRendererArray{
      DebugTileRenderer::Make(), DebugTileRenderer::MakeAA(), DebugTileRenderer::MakeNonAA()};
}
...
static ClipTileRendererArray make_image_renderers() {
  ...
  return ClipTileRendererArray{
      TextureSetRenderer::MakeUnbatched(mandrill), TextureSetRenderer::MakeBatched(mandrill, 0),
