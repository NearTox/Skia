...
DEF_SIMPLE_GM(colorwheel_alphatypes, canvas, 256, 128) {
  ...
  auto pmImg =
      ToolUtils::MakeTextureImage(canvas, SkImage::MakeFromEncoded(imgData, kPremul_SkAlphaType));
  auto upmImg =
      ToolUtils::MakeTextureImage(canvas, SkImage::MakeFromEncoded(imgData, kUnpremul_SkAlphaType));
  ...
  canvas->drawImageRect(
      pmImg, srcRect, dstRect, linear, nullptr, SkCanvas::kFast_SrcRectConstraint);
  ...
  canvas->drawImageRect(
      upmImg, srcRect, dstRect.makeOffset(128, 0), linear, nullptr,
