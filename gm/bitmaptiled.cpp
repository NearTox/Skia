...
static void draw_tile_bitmap_with_fractional_offset(
    GrRecordingContext* rContext, SkCanvas* canvas, bool vertical) {
  ...
  SkBitmap bmp;
  bmp.allocN32Pixels(
      vertical ? kBitmapShortEdge : kBitmapLongEdge, vertical ? kBitmapLongEdge : kBitmapShortEdge,
      true);
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    if (vertical) {
      canvas->drawImageRect(
          bmp.asImage(), SkRect::MakeXYWH(0, (kTileSize - 50) + offset, 32, 1124.0f),
          SkRect::MakeXYWH(37.0f * i, 0.0f, 32.0f, 1124.0f), SkSamplingOptions(), nullptr,
          SkCanvas::kStrict_SrcRectConstraint);
    } else {
      canvas->drawImageRect(
          bmp.asImage(), SkRect::MakeXYWH((kTileSize - 50) + offset, 0, 1124, 32),
          SkRect::MakeXYWH(0.0f, 37.0f * i, 1124.0f, 32.0f), SkSamplingOptions(), nullptr,
          SkCanvas::kStrict_SrcRectConstraint);
    }
  }
}
