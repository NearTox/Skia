...
class LightingShader2GM : public GM {
 ...
 protected:
  ...
  SkISize onISize() override { return SkISize::Make(600, 740); }
  ...
  void onOnceBeforeDraw() override {
    ...
    // Standard set of lights
    {
      ...
      builder.add(SkLights::Light::MakeDirectional(
          SkColor3f::Make(1.0f, 1.0f, 1.0f), kLightFromUpperRight));
    ...
    // Two directional lights
    {
      SkLights::Builder builder;
      builder.add(SkLights::Light::MakeDirectional(
          SkColor3f::Make(1.0f, 0.0f, 0.0f), kLightFromUpperRight));
      builder.add(
          SkLights::Light::MakeDirectional(SkColor3f::Make(0.0f, 1.0f, 0.0f), kLightFromUpperLeft));
      builder.setAmbientLightColor(SkColor3f::Make(0.2f, 0.2f, 0.2f));
    ...
    SkBitmap opaqueDiffuseMap =
        ToolUtils::create_checkerboard_bitmap(kTexSize, kTexSize, SK_ColorBLACK, 0xFF808080, 8);
    fOpaqueDiffuse = opaqueDiffuseMap.makeShader(&matrix);

    SkBitmap translucentDiffuseMap = ToolUtils::create_checkerboard_bitmap(
        kTexSize, kTexSize, SkColorSetARGB(0x55, 0x00, 0x00, 0x00),
        SkColorSetARGB(0x55, 0x80, 0x80, 0x80), 8);
  ...
  void drawRect(
      SkCanvas* canvas, SkScalar scaleX, SkScalar scaleY, SkScalar rotate, bool useNormalSource,
      bool useDiffuseShader, bool useTranslucentPaint, bool useTranslucentShader,
    ...
    paint.setShader(SkLightingShader::Make(
      ...
      for (bool useDiffuseShader : {true, false}) {
        for (bool useTranslucentPaint : {true, false}) {
          for (bool useTranslucentShader : {true, false}) {
            ...
            this->drawRect(
                canvas, 1.0f, 1.0f, 0.f, useNormalSource, useDiffuseShader, useTranslucentPaint,
                useTranslucentShader, fLights);
            // Drawing labels
