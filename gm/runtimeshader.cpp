...
class RuntimeShader : public skiagm::GM {
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    auto shader = gEffect->makeShader(
        SkData::MakeWithCopy(&inputColor, sizeof(inputColor)), nullptr, 0, &localM, true);
    SkPaint p;
...
static sk_sp<SkShader> make_shader(sk_sp<SkImage> img, SkISize size) {
  SkMatrix scale =
      SkMatrix::MakeScale(size.width() / (float)img->width(), size.height() / (float)img->height());
...
class ThresholdRT : public skiagm::GM {
  ...
  DrawResult onDraw(SkCanvas* canvas, SkString* errorMsg) override {
    ...
    SkPaint paint;
    paint.setShader(fEffect->makeShader(data, children, SK_ARRAY_COUNT(children), nullptr, true));
...
class SpiralRT : public skiagm::GM {
  ...
  void onDraw(SkCanvas* canvas) override {
    ...
    SkPaint paint;
    paint.setShader(
        fEffect->makeShader(SkData::MakeWithCopy(&uni, sizeof(uni)), nullptr, 0, nullptr, true));
