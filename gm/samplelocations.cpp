...
class SampleLocationsTestProcessor::Impl : public GrGLSLGeometryProcessor {
    void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override {
        ...
        if (GradType::kHW == proc.fGradType) {
            f->codeAppendf("float2x2 grad = float2x2(dFdx(%s), dFdy(%s));", coord.fsIn(),
                           coord.fsIn());
        } else {
        ...
        f->maskOffMultisampleCoverage("~(1 << i)",
                                      GrGLSLFPFragmentBuilder::ScopeFlags::kInsideLoop);
        f->codeAppendf("}");
        f->codeAppendf("}");
    }
...
private:
    ...
    void onExecute(GrOpFlushState* flushState, const SkRect& chainBounds) override {
        static constexpr GrUserStencilSettings kStencilWrite(
                GrUserStencilSettings::StaticInit<0x0001, GrUserStencilTest::kAlways, 0xffff,
                                                  GrUserStencilOp::kReplace, GrUserStencilOp::kKeep,
                                                  0xffff>());
        ...
        flushState->rtCommandBuffer()->draw(SampleLocationsTestProcessor(fGradType), pipeline,
                                            nullptr, nullptr, &mesh, 1, SkRect::MakeIWH(200, 200));
    }
...
DrawResult SampleLocationsGM::onDraw(GrContext* ctx, GrRenderTargetContext* rtc, SkCanvas* canvas,
                                     SkString* errorMsg) {
    ...
    if (auto offscreenRTC = ctx->priv().makeDeferredRenderTargetContext(
        ...
        rtc->priv().drawFilledRect(GrNoClip(), std::move(coverPaint), GrAA::kNo, SkMatrix::I(),
