...
static sk_sp<SkShader> make_bg_shader() {
  ...
  *bm.getAddr32(1, 0) = *bm.getAddr32(0, 1) = SkPackARGB32(0xFF, 0xCE, 0xCF, 0xCE);
  ...
  return bm.makeShader(
      SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions(), SkMatrix::Scale(6, 6));
}
...
DEF_SIMPLE_GM(aarectmodes, canvas, 640, 480) {
  ...
  for (int alpha = 0; alpha < 4; ++alpha) {
    ...
    for (size_t i = 0; i < SK_ARRAY_COUNT(gModes); ++i) {
      ...
      SkScalar dy = drawCell(canvas, gModes[i], gAlphaValue[alpha & 1], gAlphaValue[alpha & 2]);
      canvas->restore();
