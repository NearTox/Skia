...
DrawResult WindowRectanglesBaseGM::onDraw(SkCanvas* canvas, SkString* errorMsg) {
  ...
  SkClipStack stack;
  stack.clipRect(
      SkRect::MakeXYWH(370.75, 80.25, 149, 100), SkMatrix::I(), kDifference_SkClipOp, false);
  stack.clipRect(
      SkRect::MakeXYWH(80.25, 420.75, 150, 100), SkMatrix::I(), kDifference_SkClipOp, true);
  stack.clipRRect(
      SkRRect::MakeRectXY(SkRect::MakeXYWH(200, 200, 200, 200), 60, 45), SkMatrix::I(),
...
DrawResult WindowRectanglesGM::onCoverClipStack(
...
class WindowRectanglesMaskGM : public WindowRectanglesBaseGM {
 private:
  ...
  void visualizeAlphaMask(
      GrRecordingContext*, GrSurfaceDrawContext*, const GrReducedClip&, GrPaint&&);
...
class AlphaOnlyClip final : public GrClip {
 ...
 private:
  ...
  Effect apply(
      GrRecordingContext* ctx, GrSurfaceDrawContext*, GrAAType, bool, GrAppliedClip* out,
...
DrawResult WindowRectanglesMaskGM::onCoverClipStack(
  ...
  if (sdc->maxWindowRectangles() < kNumWindows) {
    *errorMsg =
        "Requires at least 8 window rectangles. "
...
void WindowRectanglesMaskGM::visualizeAlphaMask(
    GrRecordingContext* ctx, GrSurfaceDrawContext* rtc, const GrReducedClip& reducedClip,
    GrPaint&& paint) {
  ...
  maskRTC->stencilRect(
      &stencilClip, &GrUserStencilSettings::kUnused, std::move(stencilPaint), GrAA::kNo,
      SkMatrix::I(), SkRect::Make(maskRTC->dimensions()));
  reducedClip.drawAlphaClipMask(maskRTC.get());

  int x = kCoverRect.x() - kDeviceRect.x(), y = kCoverRect.y() - kDeviceRect.y();
  ...
  rtc->drawRect(
      &alphaClip, std::move(paint), GrAA::kYes, SkMatrix::I(),
...
void WindowRectanglesMaskGM::stencilCheckerboard(GrSurfaceDrawContext* rtc, bool flip) {
  constexpr static GrUserStencilSettings kSetClip(
      GrUserStencilSettings::StaticInit<
          0, GrUserStencilTest::kAlways, 0, GrUserStencilOp::kSetClipBit, GrUserStencilOp::kKeep,
          0>());

  rtc->clearStencilClip(SkIRect::MakeSize(rtc->dimensions()), false);

  for (int y = 0; y < kDeviceRect.height(); y += kMaskCheckerSize) {
    for (int x = (y & 1) == flip ? 0 : kMaskCheckerSize; x < kDeviceRect.width();
         x += 2 * kMaskCheckerSize) {
      SkIRect checker = SkIRect::MakeXYWH(x, y, kMaskCheckerSize, kMaskCheckerSize);
      GrPaint paint;
      paint.setXPFactory(GrDisableColorXPFactory::Get());
      rtc->stencilRect(
          nullptr, &kSetClip, std::move(paint), GrAA::kNo, SkMatrix::I(), SkRect::Make(checker));
    }
  }
}
