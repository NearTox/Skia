...
DEF_SIMPLE_GPU_GM_BG(
    fpcoordinateoverride, ctx, rtCtx, canvas, 512, 512, ToolUtils::color_to_565(0xFF66AA99)) {
  ...
  std::unique_ptr<GrFragmentProcessor> imgFP =
      GrSimpleTextureEffect::Make(texture, srcColorType, SkMatrix());
  ...
  rtCtx->priv().testingOnly_addDrawOp(
      GrFillRectOp::MakeNonAARect(ctx, std::move(grPaint), SkMatrix::I(), bounds));
}
