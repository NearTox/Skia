...
static void html_canvas_arc(
    SkPathBuilder* path, SkScalar x, SkScalar y, SkScalar r, SkScalar start, SkScalar end, bool ccw,
    bool callArcTo) {
...
DEF_SIMPLE_GM(manyarcs, canvas, 620, 330) {
  ...
  // 20 angles.
  SkScalar sweepAngles[] = {-123.7f,    -2.3f,    -2,        -1,   -0.3f,
  ...
  for (size_t i = 0; i < SK_ARRAY_COUNT(startAngles) * 2; ++i) {
    ...
    for (size_t j = 0; j < SK_ARRAY_COUNT(sweepAngles); ++j) {
      ...
      path.moveTo(0, 2);
      html_canvas_arc(
          &path, 18, 15, 10, startAngle, startAngle + (sweepAngles[j] * sign), anticlockwise, true);
      path.lineTo(0, 28);
      canvas->drawPath(path.detach(), paint);
...
DEF_SIMPLE_GM(tinyanglearcs, canvas, 620, 330) {
  ...
  for (size_t i = 0; i < SK_ARRAY_COUNT(startAngles); ++i) {
    ...
    path.moveTo(
        centerX + innerRadius * sk_float_cos(startAngles[i]),
        centerY + innerRadius * sk_float_sin(startAngles[i]));
    path.lineTo(
        centerX + outerRadius * sk_float_cos(startAngles[i]),
        centerY + outerRadius * sk_float_sin(startAngles[i]));
    // A combination of tiny sweepAngle + large radius, we should draw a line.
    html_canvas_arc(
        &path, centerX, outerRadius, outerRadius, startAngles[i] * 180 / SK_ScalarPI,
        endAngle * 180 / SK_ScalarPI, true, true);
    path.lineTo(
        centerX + innerRadius * sk_float_cos(endAngle),
        centerY + innerRadius * sk_float_sin(endAngle));
    html_canvas_arc(
        &path, centerX, outerRadius, innerRadius, endAngle * 180 / SK_ScalarPI,
        startAngles[i] * 180 / SK_ScalarPI, true, false);
    canvas->drawPath(path.detach(), paint);
