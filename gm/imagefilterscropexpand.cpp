...
DEF_SIMPLE_GM(imagefilterscropexpand, canvas, 730, 650) {
    ...
    for (int outset = -15; outset <= 20; outset += 5) {
        ...
        draw(canvas, checkerboard, rect,
             SkBlurImageFilter::Make(0.3f, 0.3f, noopCropped, &bigRect));

        draw(canvas, checkerboard, rect,
             SkBlurImageFilter::Make(8.0f, 8.0f, noopCropped, &bigRect));
        ...
        draw(canvas, checkerboard, rect,
             SkOffsetImageFilter::Make(SkIntToScalar(-8), SkIntToScalar(16), noopCropped,
                                       &bigRect));

        draw(canvas, checkerboard, rect,
             SkLightingImageFilter::MakePointLitDiffuse(pointLocation, SK_ColorWHITE, surfaceScale,
                                                        kd, noopCropped, &bigRect));
