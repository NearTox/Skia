...
void draw(
    SkCanvas* canvas, const SkBitmap& bitmap, const SkIRect& rect, sk_sp<SkImageFilter> filter);
...
DEF_SIMPLE_GM(imagefilterscropexpand, canvas, 730, 650) {
  ...
  for (int outset = -15; outset <= 20; outset += 5) {
    ...
    draw(
        canvas, checkerboard, bigRect,
        SkImageFilters::ColorFilter(cfAlphaTrans, noopCropped, &bigRect));

    draw(canvas, checkerboard, bigRect, SkImageFilters::Blur(0.3f, 0.3f, noopCropped, &bigRect));

    draw(canvas, checkerboard, bigRect, SkImageFilters::Blur(8.0f, 8.0f, noopCropped, &bigRect));

    draw(canvas, checkerboard, bigRect, SkImageFilters::Dilate(2, 2, noopCropped, &bigRect));

    draw(canvas, checkerboard, bigRect, SkImageFilters::Erode(2, 2, noopCropped, &bigRect));
    ...
    draw(
        ...
        SkImageFilters::DisplacementMap(
            SkColorChannel::kR, SkColorChannel::kR, 12, gradientCircleSource, noopCropped,
            &bigRect));
...
void draw(
    SkCanvas* canvas, const SkBitmap& bitmap, const SkIRect& layerRect,
