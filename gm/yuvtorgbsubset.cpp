...
class YUVtoRGBSubsetEffect : public GpuGM {
 ...
 protected:
  ...
  void makePixmaps() {
    SkYUVAInfo yuvaInfo = SkYUVAInfo(
        {8, 8}, SkYUVAInfo::PlaneConfig::kY_U_V, SkYUVAInfo::Subsampling::k420,
        kJPEG_Full_SkYUVColorSpace);
    SkColorType colorTypes[] = {kAlpha_8_SkColorType, kAlpha_8_SkColorType, kAlpha_8_SkColorType};
    ...
    for (uint32_t i = 0; i < SK_ARRAY_COUNT(kFilters); ++i) {
      ...
      for (uint32_t j = 0; j < GrSamplerState::kWrapModeCount + 1; ++j) {
        ...
        if (fp) {
          ...
          surfaceDrawContext->drawRect(nullptr, std::move(grPaint), GrAA::kYes, ctm, rect);
        }
