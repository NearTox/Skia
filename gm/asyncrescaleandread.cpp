...
static sk_sp<SkImage> do_read_and_scale_yuv(
  ...
  GrBackendFormat format = gr->priv().caps()->getBackendFormatFromColorType(GrColorType::kAlpha_8);
  ...
  backendTextures[1] = gr->priv().getGpu()->createBackendTexture(
      dstW / 2, dstH / 2, format, GrMipMapped::kNo, GrRenderable::kNo, uData.get(), dstW / 2,
      nullptr, GrProtected::kNo);
  backendTextures[2] = gr->priv().getGpu()->createBackendTexture(
      dstW / 2, dstH / 2, format, GrMipMapped::kNo, GrRenderable::kNo, vData.get(), dstW / 2,
      nullptr, GrProtected::kNo);
