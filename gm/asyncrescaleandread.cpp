...
static sk_sp<SkImage> do_read_and_scale(
    SkSurface* surface, const SkIRect& srcRect, const SkImageInfo& ii,
  ...
  auto callback = [](void* c, const void* data, size_t rowBytes) {
    ...
    SkRectMemcpy(
        context->fPixmap.writable_addr(), context->fPixmap.rowBytes(), data, rowBytes,
...
static sk_sp<SkImage> do_read_and_scale_yuv(
    SkSurface* surface, SkYUVColorSpace yuvCS, const SkIRect& srcRect, int dstW, int dstH,
    SkSurface::RescaleGamma rescaleGamma, SkFilterQuality quality, SkScopeExit* cleanup) {
  SkASSERT(!(dstW & 0b1) && !(dstH & 0b1));

  struct Context {
    Context(int w, int h) {
      SkImageInfo yII = SkImageInfo::Make(w, h, kGray_8_SkColorType, kPremul_SkAlphaType);
      SkImageInfo uvII = SkImageInfo::Make(w / 2, h / 2, kGray_8_SkColorType, kPremul_SkAlphaType);
  ...
  auto callback = [](void* c, const void* data[3], size_t rowBytes[3]) {
    ...
    context->fSucceeded = true;
    SkRectMemcpy(
        context->fYData.writable_addr(), context->fYData.rowBytes(), data[0], rowBytes[0],
        context->fYData.width(), context->fYData.height());
    SkRectMemcpy(
        context->fUData.writable_addr(), context->fUData.rowBytes(), data[1], rowBytes[1],
        context->fUData.width(), context->fUData.height());
    SkRectMemcpy(
        context->fVData.writable_addr(), context->fVData.rowBytes(), data[2], rowBytes[2],
        context->fVData.width(), context->fVData.height());
  };
  surface->asyncRescaleAndReadPixelsYUV420(
      yuvCS, SkColorSpace::MakeSRGB(), srcRect, dstW, dstH, rescaleGamma, quality, callback,
      &context);
  while (!context.fCalled) {
    // Only GPU should actually be asynchronous.
    SkASSERT(surface->getCanvas()->getGrContext());
  ...
  backendTextures[0] =
      gr->priv().createBackendTexture(&context.fYData, 1, GrRenderable::kNo, GrProtected::kNo);
  backendTextures[1] =
      gr->priv().createBackendTexture(&context.fUData, 1, GrRenderable::kNo, GrProtected::kNo);
  backendTextures[2] =
      gr->priv().createBackendTexture(&context.fVData, 1, GrRenderable::kNo, GrProtected::kNo);
  ...
  *cleanup = {[gr, backendTextures] {
  ...
  return SkImage::MakeFromYUVATextures(
      gr, yuvCS, backendTextures, indices, {dstW, dstH}, kTopLeft_GrSurfaceOrigin,
...
static skiagm::DrawResult do_rescale_grid(
    SkCanvas* canvas, SkSurface* surface, const SkIRect& srcRect, int newW, int newH, bool doYUV420,
    ...
    for (auto quality : {kNone_SkFilterQuality, kLow_SkFilterQuality, kHigh_SkFilterQuality}) {
      ...
      if (doYUV420) {
        result = do_read_and_scale_yuv(
            surface, yuvColorSpace, srcRect, newW, newH, gamma, quality, &cleanup);
        if (!result) {
...
static skiagm::DrawResult do_rescale_image_grid(
    SkCanvas* canvas, const char* imageFile, const SkIRect& srcRect, int newW, int newH,
...
DEF_SIMPLE_GM_CAN_FAIL(async_rescale_and_read_no_bleed, canvas, errorMsg, 60, 60) {
  ...
  auto surfaceII = SkImageInfo::Make(
      kInner + 2 * kBorder, kInner + 2 * kBorder, kRGBA_8888_SkColorType, kPremul_SkAlphaType,
      SkColorSpace::MakeSRGB());
