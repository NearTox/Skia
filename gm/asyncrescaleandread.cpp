...
static sk_sp<SkImage> do_read_and_scale_yuv(
  ...
  *cleanup = {[direct, backendTextures] {
  ...
  return SkImage::MakeFromYUVATextures(
      direct, yuvCS, backendTextures, indices, size, kTopLeft_GrSurfaceOrigin,
...
static skiagm::DrawResult do_rescale_image_grid(
  ...
  if (doSurface) {
    ...
    return do_rescale_grid(canvas, surface.get(), direct, srcRect, newSize, doYUV420, errorMsg);
  } else if (direct) {
  ...
  return do_rescale_grid(canvas, image.get(), direct, srcRect, newSize, doYUV420, errorMsg);
}
...
DEF_SIMPLE_GM_CAN_FAIL(async_yuv_no_scale, canvas, errorMsg, 400, 300) {
  ...
  auto yuvImage = do_read_and_scale_yuv(
      surface, direct, kRec601_SkYUVColorSpace, SkIRect::MakeWH(400, 300), {400, 300},
...
DEF_SIMPLE_GM_CAN_FAIL(async_rescale_and_read_no_bleed, canvas, errorMsg, 60, 60) {
  ...
  auto direct = GrAsDirectContext(canvas->recordingContext());
  result = do_rescale_grid(canvas, surface.get(), direct, srcRect, downSize, false, errorMsg, kPad);
