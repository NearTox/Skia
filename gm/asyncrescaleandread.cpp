...
static sk_sp<SkImage> do_read_and_scale_yuv(
  ...
  SkYUVAInfo yuvaInfo(size, SkYUVAInfo::PlaneConfig::kY_U_V, SkYUVAInfo::Subsampling::k420, yuvCS);
  SkPixmap yuvPMs[] = {
...
static skiagm::DrawResult do_rescale_grid(
    SkCanvas* canvas, Src* src, GrDirectContext* direct, const SkIRect& srcRect, SkISize newSize,
    bool doYUV420, SkString* errorMsg, int pad = 0) {
  if (doYUV420 && !direct) {
...
static skiagm::DrawResult do_rescale_image_grid(
    SkCanvas* canvas, const char* imageFile, const SkIRect& srcRect, SkISize newSize,
    bool doSurface, bool doYUV420, SkString* errorMsg) {
  ...
  if (doSurface) {
    ...
    return do_rescale_grid(canvas, surface.get(), dContext, srcRect, newSize, doYUV420, errorMsg);
  } else if (dContext) {
  ...
  return do_rescale_grid(canvas, image.get(), dContext, srcRect, newSize, doYUV420, errorMsg);
}
...
#define DEF_RESCALE_AND_READ_SURF_GM(IMAGE_FILE, TAG, SRC_RECT, W, H)                           \
  DEF_SIMPLE_GM_CAN_FAIL(async_rescale_and_read_##TAG, canvas, errorMsg, 3 * W, 2 * H) {        \
    ...
    return do_rescale_image_grid(canvas, #IMAGE_FILE, SRC_RECT, {W, H}, true, false, errorMsg); \
  }
...
#define DEF_RESCALE_AND_READ_IMG_GM(IMAGE_FILE, TAG, SRC_RECT, W, H)                             \
  DEF_SIMPLE_GM_CAN_FAIL(async_rescale_and_read_##TAG, canvas, errorMsg, 3 * W, 2 * H) {         \
    ToolUtils::draw_checkerboard(canvas, SK_ColorDKGRAY, SK_ColorLTGRAY, 25);                    \
    return do_rescale_image_grid(canvas, #IMAGE_FILE, SRC_RECT, {W, H}, false, false, errorMsg); \
  }
...
DEF_SIMPLE_GM_CAN_FAIL(async_rescale_and_read_no_bleed, canvas, errorMsg, 60, 60) {
  ...
  result =
      do_rescale_grid(canvas, surface.get(), dContext, srcRect, downSize, false, errorMsg, kPad);
...
DEF_SIMPLE_GM_CAN_FAIL(async_rescale_and_read_alpha_type, canvas, errorMsg, 512, 512) {
  ...
  auto shader = SkGradientShader::MakeRadial(
      {100, 100}, 230, colors, nullptr, nullptr, SK_ARRAY_COUNT(colors), SkTileMode::kRepeat);
  SkPaint paint;
  ...
  for (const auto& img : {pmImg, upmImg}) {
    ...
    for (auto readAT : {kPremul_SkAlphaType, kUnpremul_SkAlphaType}) {
      ...
      auto* asyncContext = new AsyncContext();
      img->asyncRescaleAndReadPixels(
          readInfo, SkIRect::MakeSize(img->dimensions()), SkImage::RescaleGamma::kSrc,
          SkImage::RescaleMode::kRepeatedCubic, async_callback, asyncContext);
      if (dContext) {
        dContext->submit();
      }
      while (!asyncContext->fCalled) {
      ...
      if (asyncContext->fResult) {
        SkPixmap pixmap(
            readInfo, asyncContext->fResult->data(0), asyncContext->fResult->rowBytes(0));
        auto releasePixels = [](const void*, void* c) { delete static_cast<AsyncContext*>(c); };
