...
static sk_sp<SkImage> do_read_and_scale_yuv(
    SkSurface* surface, SkYUVColorSpace yuvCS, const SkIRect& srcRect, SkISize size,
  ...
  *cleanup = {[gr, backendTextures] {
  ...
  return SkImage::MakeFromYUVATextures(
      gr, yuvCS, backendTextures, indices, size, kTopLeft_GrSurfaceOrigin,
...
static skiagm::DrawResult do_rescale_grid(
    SkCanvas* canvas, SkSurface* surface, const SkIRect& srcRect, SkISize newSize, bool doYUV420,
    ...
    for (auto quality : {kNone_SkFilterQuality, kLow_SkFilterQuality, kHigh_SkFilterQuality}) {
      ...
      if (doYUV420) {
        result = do_read_and_scale_yuv(
            surface, yuvColorSpace, srcRect, newSize, gamma, quality, &cleanup);
        if (!result) {
...
static skiagm::DrawResult do_rescale_image_grid(
    SkCanvas* canvas, const char* imageFile, const SkIRect& srcRect, SkISize newSize, bool doYUV420,
    SkString* errorMsg) {
