...
sk_sp<SkShader> create_gradient_shader(SkRect r) {
  ...
  return SkGradientShader::MakeLinear(
      pts, colors, offsets, SK_ARRAY_COUNT(colors), SkTileMode::kClamp);
}
...
sk_sp<SkShader> create_blend_shader(SkCanvas* destCanvas, SkBlendMode bm) {
  ...
  return SkShaders::Blend(
      bm, std::move(dst),
      create_image_shader(destCanvas, SkTileMode::kRepeat, SkTileMode::kRepeat));
}
...
class GraphiteStartGM : public GM {
 ...
 protected:
  ...
  SkString onShortName() override { return SkString("graphitestart"); }
