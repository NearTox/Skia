...
class AnimatedGifGM : public skiagm::GM {
 private:
  ...
  void drawFrame(SkCanvas* canvas, int frameIndex) {
    ...
    if (!bm.getPixels()) {
      ...
      if (SkCodec::kSuccess != fCodec->getPixels(info, bm.getPixels(), bm.rowBytes(), &opts)) {
 ...
 public:
  AnimatedGifGM() : fFrame(0), fNextUpdate(-1), fTotalFrames(-1) {}

 private:
  SkString onShortName() override { return SkString("animatedGif"); }
...
class AnimCodecPlayerGM : public skiagm::GM {
 ...
 private:
  SkString onShortName() override { return SkString("AnimCodecPlayer"); }
...
class AnimCodecPlayerExifGM : public skiagm::GM {
 ...
 public:
  AnimCodecPlayerExifGM(const char* path) : fPath(path) {}
