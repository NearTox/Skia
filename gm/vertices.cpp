...
static void fill_mesh(
    SkPoint pts[kMeshVertexCnt], SkPoint texs[kMeshVertexCnt], SkColor colors[kMeshVertexCnt],
          ...
          for (const auto& shader : {fShader1, fShader2}) {
            ...
            for (auto attrs : kAttrs) {
              ...
              auto v = SkVertices::MakeCopy(
                  SkVertices::kTriangleFan_VertexMode, kMeshVertexCnt, fPts, texs, colors,
  ...
  for (bool useShader : {false, true}) {
    for (bool useTex : {false, true}) {
      for (const auto& m : matrices) {
        ...
        auto v = SkVertices::MakeCopy(
            SkVertices::kTriangles_VertexMode, kMeshVertexCnt, pts, t, colors, kNumTris * 3,
            indices);
...
static constexpr SkPoint kHexVerts[] = {
    {0, 0}, {0, -1},         {kSin60, -0.5f},  {kSin60, 0.5f},
    {0, 1}, {-kSin60, 0.5f}, {-kSin60, -0.5f}, {0, -1},
};

static constexpr SkColor4f kColors[] = {
    SkColors::kWhite, SkColors::kRed,  SkColors::kYellow,  SkColors::kGreen,
...
DEF_SIMPLE_GM(vertices_custom_colors, canvas, 400, 200) {
  ...
  {
    SkVertices::Builder builder(
        SkVertices::kTriangleFan_VertexMode, 8, 0, SkVertices::kHasColors_BuilderFlag);
    for (int i = 0; i < 8; ++i) {
