...
VulkanWindowContext::VulkanWindowContext(
    const DisplayParams& params, CreateVkSurfaceFn createVkSurface, CanPresentFn canPresent,
...
void VulkanWindowContext::initializeContext() {
  ...
  auto getProc = [getInstanceProc, getDeviceProc](
  ...
  if (!sk_gpu_test::CreateVkBackendContext(
          getProc, &backendContext, &extensions, &features, &fDebugCallback, &fPresentQueueIndex,
          fCanPresentFn)) {
  ...
  fInterface.reset(new GrVkInterface(
      backendContext.fGetProc, fInstance, fDevice, backendContext.fInstanceVersion, physDevVersion,
      &extensions));
...
bool VulkanWindowContext::createSwapchain(int width, int height, const DisplayParams& params) {
  ...
  res =
      fGetPhysicalDeviceSurfaceFormatsKHR(fPhysicalDevice, fSurface, &surfaceFormatCount, nullptr);
  if (VK_SUCCESS != res) {
    return false;
  }
  ...
  switch (surfaceFormat) {
    ...
    case VK_FORMAT_R8G8B8A8_SRGB: colorType = kRGBA_8888_SkColorType; break;
...
void VulkanWindowContext::destroyBuffers() {
  if (fBackbuffers) {
    for (uint32_t i = 0; i < fImageCount + 1; ++i) {
      fBackbuffers[i].fImageIndex = -1;
      GR_VK_CALL(fInterface, DestroySemaphore(fDevice, fBackbuffers[i].fRenderSemaphore, nullptr));
    }
  }
...
VulkanWindowContext::~VulkanWindowContext() { this->destroyContext(); }
...
sk_sp<SkSurface> VulkanWindowContext::getBackbufferSurface() {
  ...
  GR_VK_CALL_ERRCHECK(fInterface, CreateSemaphore(fDevice, &semaphoreInfo, nullptr, &semaphore));

  // acquire the image
  VkResult res = fAcquireNextImageKHR(
      fDevice, fSwapchain, UINT64_MAX, semaphore, VK_NULL_HANDLE, &backbuffer->fImageIndex);
