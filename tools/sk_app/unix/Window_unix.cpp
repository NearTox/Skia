    ...
    switch (event.type) {
        ...
        case SelectionRequest: {
          ...
          if (fClipboardText.empty() || xsr->target != UTF8 || xsr->property == None) {
            ...
            XChangeProperty(
                fDisplay, xsr->requestor, xsr->property, UTF8, /*format=*/8, PropModeReplace,
                (unsigned char*)fClipboardText.data(), fClipboardText.length());
            XSendEvent(fDisplay, xsr->requestor, True, NoEventMask, (XEvent*)&xsel);
          }
        } break;
...
const char* Window_unix::getClipboardText() {
  ...
  if (event.type == SelectionNotify && event.xselection.selection == CLIPBOARD &&
    ...
    XGetWindowProperty(
        xsel.display, xsel.requestor, xsel.property, /*offset=*/0,
        /*length=*/~0L, /*delete=*/False, AnyPropertyType, &type, &format, &nitems, &bytes_after,
        (unsigned char**)&data);
