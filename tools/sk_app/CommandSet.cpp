...
CommandSet::CommandSet() : fHelpMode(kNone_HelpMode) {
  this->addCommand('h', "Overlays", "Show help screen", [this]() {
    switch (this->fHelpMode) {
      case kNone_HelpMode: this->fHelpMode = kGrouped_HelpMode; break;
      case kGrouped_HelpMode: this->fHelpMode = kAlphabetical_HelpMode; break;
      case kAlphabetical_HelpMode: this->fHelpMode = kNone_HelpMode; break;
    }
    fWindow->inval();
  });
}
...
void CommandSet::drawHelp(SkCanvas* canvas) {
  ...
  SkTQSort(
      fCommands.begin(), fCommands.end() - 1,
  ...
  SkScalar keyWidth = 0;
  for (Command& cmd : fCommands) {
    keyWidth = SkMaxScalar(
        keyWidth,
        font.measureText(cmd.fKeyName.c_str(), cmd.fKeyName.size(), SkTextEncoding::kUTF8));
  }
  ...
  for (Command& cmd : fCommands) {
    if (kGrouped_HelpMode == fHelpMode && lastGroup != cmd.fGroup) {
      ...
      canvas->drawSimpleText(
          cmd.fGroup.c_str(), cmd.fGroup.size(), SkTextEncoding::kUTF8, x, y, groupFont,
          groupPaint);
    ...
    canvas->drawSimpleText(
        cmd.fKeyName.c_str(), cmd.fKeyName.size(), SkTextEncoding::kUTF8, x, y, font, paint);
