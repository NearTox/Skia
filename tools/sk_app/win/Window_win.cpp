...
Window_win::~Window_win() { this->closeWindow(); }
...
bool Window_win::init(HINSTANCE hInstance) {
  ...
  fHWnd = CreateWindow(
      gSZWindowClass, nullptr, WS_OVERLAPPEDWINDOW, gWindowX, gWindowY, gWindowWidth, gWindowHeight,
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
  ...
  switch (message) {
    ...
    case WM_CHAR: {
      ...
      eventHandled =
          window->onChar(SkUTF16_NextUnichar(&c), get_modifiers(message, wParam, lParam));
    } break;

    case WM_KEYDOWN:
    case WM_SYSKEYDOWN:
      eventHandled = window->onKey(
          get_key(wParam), Window::kDown_InputState, get_modifiers(message, wParam, lParam));
      break;

    case WM_KEYUP:
    case WM_SYSKEYUP:
      eventHandled = window->onKey(
          get_key(wParam), Window::kUp_InputState, get_modifiers(message, wParam, lParam));
      break;
    ...
    case WM_LBUTTONUP: {
      ...
      Window::InputState istate =
          ((wParam & MK_LBUTTON) != 0) ? Window::kDown_InputState : Window::kUp_InputState;
    ...
    case WM_MOUSEMOVE: {
      ...
      eventHandled = window->onMouse(
          xPos, yPos, Window::kMove_InputState, get_modifiers(message, wParam, lParam));
    } break;
...
void Window_win::setTitle(const char* title) { SetWindowTextA(fHWnd, title); }

void Window_win::show() { ShowWindow(fHWnd, SW_SHOW); }
...
bool Window_win::attach(BackendType attachType) {
  ...
  switch (attachType) {
    ...
    case kRaster_BackendType:
      fWindowContext = window_context_factory::NewRasterForWin(fHWnd, fRequestedDisplayParams);
      break;
#ifdef SK_VULKAN
    case kVulkan_BackendType:
      fWindowContext = window_context_factory::NewVulkanForWin(fHWnd, fRequestedDisplayParams);
      break;
#endif
  }
...
void Window_win::onInval() { InvalidateRect(fHWnd, nullptr, false); }
