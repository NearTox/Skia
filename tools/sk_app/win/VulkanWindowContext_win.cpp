...
std::unique_ptr<WindowContext> MakeVulkanForWin(HWND hwnd, const DisplayParams& params) {
  ...
  auto createVkSurface = [hwnd, instProc](VkInstance instance) -> VkSurfaceKHR {
    ...
    if (!createWin32SurfaceKHR) {
      createWin32SurfaceKHR =
          (PFN_vkCreateWin32SurfaceKHR)instProc(instance, "vkCreateWin32SurfaceKHR");
    }
  ...
  auto canPresent = [instProc](
                        VkInstance instance, VkPhysicalDevice physDev, uint32_t queueFamilyIndex) {
