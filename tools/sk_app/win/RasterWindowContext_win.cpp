...
void RasterWindowContext_win::resize(int w, int h) {
  ...
  SkImageInfo info = SkImageInfo::Make(
      w, h, fDisplayParams.fColorType, kPremul_SkAlphaType, fDisplayParams.fColorSpace);
...
void RasterWindowContext_win::swapBuffers() {
  ...
  HDC dc = GetDC(fWnd);
  StretchDIBits(
      dc, 0, 0, fWidth, fHeight, 0, 0, fWidth, fHeight, bmpInfo->bmiColors, bmpInfo, DIB_RGB_COLORS,
      SRCCOPY);
  ReleaseDC(fWnd, dc);
}
