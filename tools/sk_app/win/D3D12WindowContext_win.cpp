...
D3D12WindowContext::~D3D12WindowContext() { this->destroyContext(); }
...
void D3D12WindowContext::initializeContext() {
  ...
  GR_D3D_CALL_ERRCHECK(fDevice->CreateFence(
      fFenceValues[fBufferIndex], D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&fFence)));
...
void D3D12WindowContext::setupSurfaces(int width, int height) {
  ...
  for (int i = 0; i < kNumFrames; ++i) {
    ...
    SkASSERT(
        fBuffers[i]->GetDesc().Width == (UINT64)width &&
...
void D3D12WindowContext::resize(int width, int height) {
  ...
  GR_D3D_CALL_ERRCHECK(fSwapChain->ResizeBuffers(0, width, height, DXGI_FORMAT_R8G8B8A8_UNORM, 0));
