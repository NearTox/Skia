...
float Window_mac::scaleFactor() const { return sk_app::GetBackingScaleFactor(fWindow.contentView); }
...
- (skui::ModifierKey)updateModifierKeys:(NSEvent*)event {
  ...
  for (const ModMap& cur :
       {ModMap{skui::ModifierKey::kCommand, skui::Key::kSuper},
    ...
    if (Any(changed & cur.modifier)) {
      const skui::InputState state =
          Any(modifiers & cur.modifier) ? skui::InputState::kDown : skui::InputState::kUp;
...
- (void)mouseDown:(NSEvent *)event {
  ...
  fWindow->onMouse(
      pos.x * backingScaleFactor, (rect.size.height - pos.y) * backingScaleFactor,
