...
static bool detect_shader_settings(
    const std::string& text, SkSL::Program::Settings* settings, const SkSL::ShaderCaps** caps,
...
ResultCode processCommand(const std::vector<std::string>& args) {
  ...
  if (skstd::ends_with(outputPath, ".spirv")) {
    return compileProgram([](SkSL::Compiler& compiler, SkSL::Program& program,
      ...
      skstd::ends_with(outputPath, ".asm.frag") || skstd::ends_with(outputPath, ".asm.vert")) {
    return compileProgram(
        [](SkSL::Compiler& compiler, SkSL::Program& program, SkSL::OutputStream& out) {
          ...
          if (!tools.Disassemble((const uint32_t*)spirv.data(), spirv.size() / 4, &disassembly)) {
            return false;
          }
    ...
    return compileProgram([](SkSL::Compiler& compiler, SkSL::Program& program,
                             SkSL::OutputStream& out) { return compiler.toHLSL(program, out); });
  } else if (skstd::ends_with(outputPath, ".wgsl")) {
    return compileProgram([](SkSL::Compiler& compiler, SkSL::Program& program,
                             SkSL::OutputStream& out) { return compiler.toWGSL(program, out); });
    ...
    return compileProgram([](SkSL::Compiler&, SkSL::Program& program, SkSL::OutputStream& out) {
      class Callbacks : public SkSL::PipelineStage::Callbacks {
       public:
        std::string getMangledName(const char* name) override { return std::string(name) + "_0"; }
        ...
        void declareFunction(const char* decl) override { fOutput += std::string(decl) + ";"; }
