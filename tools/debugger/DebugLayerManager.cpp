...
void DebugLayerManager::storeSkPicture(
    int nodeId, int frame, sk_sp<SkPicture> picture, SkIRect dirty) {
...
sk_sp<SkImage> DebugLayerManager::getLayerAsImage(const int nodeId, const int frame) {
  ...
  while (i > 0 && !(fDraws[{relevantFrames[i], nodeId}].fullRedraw)) {
    i--;
  }
...
DebugLayerManager::DrawEventSummary DebugLayerManager::event(int nodeId, int frame) const {
  ...
  return {true, evt->debugCanvas->getSize(), evt->layerBounds.width(), evt->layerBounds.height()};
}
...
std::vector<DebugLayerManager::LayerSummary> DebugLayerManager::summarizeLayers(int frame) const {
  ...
  for (const auto& key : keys) {
    ...
    if (found != summaryMap.end()) {
      ...
      summaryMap.insert(
          {key.nodeId,
           {key.nodeId, lastUpdate, evt->fullRedraw, evt->layerBounds.width(),
            evt->layerBounds.height()}});
    }
  }
