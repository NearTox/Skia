...
void xlate_and_scale_to_bounds(SkCanvas* canvas, const SkRect& bounds) {
  ...
  if (bounds.width() > bounds.height()) {
    canvas->scale(
        SkDoubleToScalar((kInsetFrac * size.fWidth) / bounds.width()),
        SkDoubleToScalar((kInsetFrac * size.fHeight) / bounds.width()));
  } else {
    canvas->scale(
        SkDoubleToScalar((kInsetFrac * size.fWidth) / bounds.height()),
...
void render_bitmap(SkCanvas* canvas, const SkBitmap& input, const SkRect* srcRect = nullptr) {
  ...
  SkRect dst =
      SkRect::MakeXYWH(SK_Scalar1, SK_Scalar1, xScale * input.width(), yScale * input.height());

  static const int kNumBlocks = 8;

  canvas->clear(0xFFFFFFFF);
  SkISize block = {
      canvas->imageInfo().width() / kNumBlocks, canvas->imageInfo().height() / kNumBlocks};
  for (int y = 0; y < kNumBlocks; ++y) {
    for (int x = 0; x < kNumBlocks; ++x) {
      ...
      SkRect r = SkRect::MakeXYWH(
          SkIntToScalar(x * block.width()), SkIntToScalar(y * block.height()),
          SkIntToScalar(block.width()), SkIntToScalar(block.height()));
  ...
  if (srcRect) {
    SkRect r = SkRect::MakeLTRB(
        srcRect->fLeft * xScale + SK_Scalar1, srcRect->fTop * yScale + SK_Scalar1,
        srcRect->fRight * xScale + SK_Scalar1, srcRect->fBottom * yScale + SK_Scalar1);
    SkPaint p;
...
static_assert(
    SK_ARRAY_COUNT(gBlendModeMap) == static_cast<size_t>(SkBlendMode::kLastMode) + 1,
    "blendMode mismatch");
static_assert(
    SK_ARRAY_COUNT(gBlendModeMap) == static_cast<size_t>(SkBlendMode::kLuminosity) + 1,
    "blendMode mismatch");
...
void apply_paint_blend_mode(const SkPaint& paint, SkJSONWriter& writer) {
  ...
  if (mode != SkBlendMode::kSrcOver) {
    ...
    writer.appendString(DEBUGCANVAS_ATTRIBUTE_BLENDMODE, gBlendModeMap[static_cast<size_t>(mode)]);
  }
}
...
void DrawCommand::MakeJsonPath(SkJSONWriter& writer, const SkPath& path) {
  ...
  switch (path.getFillType()) {
    ...
    case SkPathFillType::kInverseWinding:
      writer.appendString(DEBUGCANVAS_ATTRIBUTE_FILLTYPE, DEBUGCANVAS_FILLTYPE_INVERSEWINDING);
      break;
    case SkPathFillType::kInverseEvenOdd:
      writer.appendString(DEBUGCANVAS_ATTRIBUTE_FILLTYPE, DEBUGCANVAS_FILLTYPE_INVERSEEVENODD);
      break;
  }
...
static void store_scalar(
    SkJSONWriter& writer, const char* key, SkScalar value, SkScalar defaultValue) {
...
static SkString encode_data(
    const void* bytes, size_t count, const char* contentType, UrlDataManager& urlDataManager) {
  sk_sp<SkData> data(SkData::MakeWithCopy(bytes, count));
  return urlDataManager.addData(data.get(), contentType);
}

void DrawCommand::flatten(
    const SkFlattenable* flattenable, SkJSONWriter& writer, UrlDataManager& urlDataManager) {
...
bool DrawCommand::flatten(
    const SkImage& image, SkJSONWriter& writer, UrlDataManager& urlDataManager) {
...
bool DrawCommand::flatten(
    const SkBitmap& bitmap, SkJSONWriter& writer, UrlDataManager& urlDataManager) {
...
static void apply_paint_filterquality(const SkPaint& paint, SkJSONWriter& writer) {
  ...
  switch (quality) {
    ...
    case kMedium_SkFilterQuality:
      writer.appendString(DEBUGCANVAS_ATTRIBUTE_FILTERQUALITY, DEBUGCANVAS_FILTERQUALITY_MEDIUM);
      break;
    case kHigh_SkFilterQuality:
      writer.appendString(DEBUGCANVAS_ATTRIBUTE_FILTERQUALITY, DEBUGCANVAS_FILTERQUALITY_HIGH);
      break;
  }
}

static void apply_paint_maskfilter(
    const SkPaint& paint, SkJSONWriter& writer, UrlDataManager& urlDataManager) {
...
static void apply_font_typeface(
    const SkFont& font, SkJSONWriter& writer, UrlDataManager& urlDataManager) {
...
static void apply_flattenable(
    const char* key, SkFlattenable* flattenable, SkJSONWriter& writer,
...
void DrawCommand::MakeJsonPaint(
    SkJSONWriter& writer, const SkPaint& paint, UrlDataManager& urlDataManager) {
  writer.beginObject();
  store_scalar(writer, DEBUGCANVAS_ATTRIBUTE_STROKEWIDTH, paint.getStrokeWidth(), 0.0f);
  store_scalar(
      writer, DEBUGCANVAS_ATTRIBUTE_STROKEMITER, paint.getStrokeMiter(),
...
DrawAnnotationCommand::DrawAnnotationCommand(
    const SkRect& rect, const char key[], sk_sp<SkData> value)
...
DrawBitmapCommand::DrawBitmapCommand(
...
DrawBitmapLatticeCommand::DrawBitmapLatticeCommand(
...
DrawBitmapNineCommand::DrawBitmapNineCommand(
...
DrawBitmapRectCommand::DrawBitmapRectCommand(
...
DrawImageCommand::DrawImageCommand(
    ...
    : INHERITED(kDrawImage_OpType), fImage(SkRef(image)), fLeft(left), fTop(top), fPaint(paint) {}
...
void DrawImageCommand::toJSON(SkJSONWriter& writer, UrlDataManager& urlDataManager) const {
  ...
  switch (fImage->alphaType()) {
    ...
    case kUnpremul_SkAlphaType:
      ...
      break;
    default: writer.appendString(DEBUGCANVAS_ATTRIBUTE_ALPHA, DEBUGCANVAS_ALPHATYPE_UNKNOWN); break;
  }
}

DrawImageLatticeCommand::DrawImageLatticeCommand(
...
DrawImageRectCommand::DrawImageRectCommand(
...
DrawImageRectLayerCommand::DrawImageRectLayerCommand(
...
DrawImageNineCommand::DrawImageNineCommand(
...
BeginDrawPictureCommand::BeginDrawPictureCommand(
...
DrawPointsCommand::DrawPointsCommand(
...
DrawTextBlobCommand::DrawTextBlobCommand(
    sk_sp<SkTextBlob> blob, SkScalar x, SkScalar y, const SkPaint& paint)
    : INHERITED(kDrawTextBlob_OpType), fBlob(std::move(blob)), fXPos(x), fYPos(y), fPaint(paint) {}
...
DrawPatchCommand::DrawPatchCommand(
...
DrawDRRectCommand::DrawDRRectCommand(
...
DrawEdgeAAImageSetCommand::DrawEdgeAAImageSetCommand(
    const SkCanvas::ImageSetEntry set[], int count, const SkPoint dstClips[],
    const SkMatrix preViewMatrices[], const SkPaint* paint, SkCanvas::SrcRectConstraint constraint)
...
void DrawEdgeAAImageSetCommand::execute(SkCanvas* canvas) const {
  canvas->experimental_DrawEdgeAAImageSet(
      fSet.get(), fCount, fDstClips.get(), fPreViewMatrices.get(), fPaint.getMaybeNull(),
      fConstraint);
}
...
DrawVerticesCommand::DrawVerticesCommand(
    sk_sp<SkVertices> vertices, SkBlendMode bmode, const SkPaint& paint)
...
DrawAtlasCommand::DrawAtlasCommand(
...
void DrawAtlasCommand::execute(SkCanvas* canvas) const {
  canvas->drawAtlas(
      fImage.get(), fXform.begin(), fTex.begin(), fColors.isEmpty() ? nullptr : fColors.begin(),
      fXform.count(), fBlendMode, fCull.getMaybeNull(), fPaint.getMaybeNull());
}
