...
struct DiffSummary {
    ...
    void add(const DiffRecord& drp) {
      ...
      switch (drp.fResult) {
        case DiffRecord::kEqualBits_Result: fNumMatches++; break;
        case DiffRecord::kEqualPixels_Result: fNumMatches++; break;
        case DiffRecord::kDifferentSizes_Result: fNumMismatches++; break;
        case DiffRecord::kDifferentPixels_Result:
          ...
          mismatchValue = MAX3(drp.fMaxMismatchR, drp.fMaxMismatchG, drp.fMaxMismatchB);
        ...
        case DiffRecord::kCouldNotCompare_Result:
          fNumMismatches++;
          fStatusOfType[drp.fBase.fStatus][drp.fComparison.fStatus].push_back(drp.fBase.fFilename);
          break;
        case DiffRecord::kUnknown_Result: SkDEBUGFAIL("adding uncategorized DiffRecord"); break;
        default: SkDEBUGFAIL("adding DiffRecord with unhandled fResult value"); break;
      }

      switch (drp.fResult) {
        case DiffRecord::kEqualBits_Result:
        case DiffRecord::kEqualPixels_Result: break;
        default: add_unique_basename(&fFailedBaseNames[drp.fResult], drp.fBase.fFilename); break;
      }
    }
};
...
static void create_diff_images (DiffMetricProc dmp,
    ...
    if (!baseFiles.empty()) {
      qsort(
          baseFiles.begin(), baseFiles.count(), sizeof(SkString),
          SkCastForQSort(compare_file_name_metrics));
    }
    if (!comparisonFiles.empty()) {
      qsort(
          comparisonFiles.begin(), comparisonFiles.count(), sizeof(SkString),
          SkCastForQSort(compare_file_name_metrics));
    }
            ...
            if (nullptr == baseFileBits || nullptr == comparisonFileBits) {
              ...
              if (DiffResource::kDecoded_Status == drp.fBase.fStatus &&
                ...
                create_and_write_diff_image(
                    &drp, dmp, colorThreshold, outputDir, drp.fBase.fFilename);
              } else {
                drp.fResult = DiffRecord::kCouldNotCompare_Result;
              }
            }
    ...
    for (int i = 1; i < argc; i++) {
      ...
      if (argv[i][0] != '-') {
        switch (numUnflaggedArguments++) {
          case 0: baseDir.set(argv[i]); continue;
          case 1: comparisonDir.set(argv[i]); continue;
          case 2: outputDir.set(argv[i]); continue;
          default:
