...
static const char* get_sksl_error_name(const SkQP::SkSLErrorTest& t) { return t.name.c_str(); }
...
void Java_org_skia_skqp_SkQP_nInit(JNIEnv* env, jobject object, jobject assetManager,
    ...
    env->SetObjectField(
        object, env->GetFieldID(SkQP_class, "mUnitTests", kStringArrayType),
        to_java_string_array(env, unitTests, &SkQP::GetUnitTestName));
    env->SetObjectField(
        object, env->GetFieldID(SkQP_class, "mSkSLErrorTestName", kStringArrayType),
        to_java_string_array(env, skslErrorTests, get_sksl_error_name));
    env->SetObjectField(
        object, env->GetFieldID(SkQP_class, "mSkSLErrorTestShader", kStringArrayType),
