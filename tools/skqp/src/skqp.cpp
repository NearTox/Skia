...
static void read_lines(
    const void* data, size_t size, const std::function<void(std::string_view)>& lineFn) {
...
static std::vector<SkQP::SkSLErrorTest> get_sksl_error_tests(
    SkQPAssetManager* assetManager, const ExclusionList& exclusionList) {
...
void ExclusionList::initialize(
    SkQPAssetManager* assetManager, sk_sp<SkData> dat, int enforcedAndroidAPILevel) {
  ...
  const std::vector<SkQP::SkSLErrorTest> skslTestList =
      get_sksl_error_tests(assetManager, noExclusions);
  ...
  read_lines(dat->data(), dat->size(), [this, &testExists](std::string_view line) {
    ...
    if (!testExists(exclusionRegex)) {
      SK_ABORT(
          "Exclusion list contains tests not found in the test registry: %s",
...
TestHarness CurrentTestHarness() { return TestHarness::kSkQP; }
