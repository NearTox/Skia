...
SkEventTracer::Handle SkDebugfTracer::addTraceEvent(
    char phase, const uint8_t* categoryEnabledFlag, const char* name, uint64_t id, int numArgs,
    const char** argNames, const uint8_t* argTypes, const uint64_t* argValues, uint8_t flags) {
  SkString args;
  for (int i = 0; i < numArgs; ++i) {
    ...
    switch (argTypes[i]) {
      ...
      case TRACE_VALUE_TYPE_INT:
        ...
        break;
      case TRACE_VALUE_TYPE_DOUBLE: args.appendf("%s=%g", argNames[i], value.as_double); break;
      case TRACE_VALUE_TYPE_POINTER: args.appendf("%s=0x%p", argNames[i], value.as_pointer); break;
      ...
      default: args.appendf("%s=<unknown type>", argNames[i]); break;
    }
  }
  ...
  if (open) {
    ...
    SkDebugf(
        "[% 2d]%s <%s> %s%s #%d {\n", fIndent.size(), fIndent.c_str(), category, name, args.c_str(),
        fCnt);
    fIndent.append(" ");
  } else {
...
void SkDebugfTracer::updateTraceEventDuration(
    const uint8_t* categoryEnabledFlag, const char* name, SkEventTracer::Handle handle) {
