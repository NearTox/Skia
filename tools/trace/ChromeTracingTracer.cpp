...
SkEventTracer::Handle ChromeTracingTracer::addTraceEvent(
...
void ChromeTracingTracer::updateTraceEventDuration(
...
static void trace_value_to_json(
    SkJSONWriter* writer, uint64_t argValue, uint8_t argType, const char* stringTableBase) {
  skia::tracing_internals::TraceValueUnion value;
  ...
  switch (argType) {
    ...
    case TRACE_VALUE_TYPE_COPY_STRING: writer->appendString(stringTableBase + value.as_uint); break;
...
static void trace_event_to_json(
    SkJSONWriter* writer, TraceEvent* traceEvent,
