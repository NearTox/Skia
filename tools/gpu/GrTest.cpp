...
bool GrSurfaceProxy::isWrapped_ForTesting() const { return SkToBool(fTarget); }
...
void GrRenderTargetContextPriv::testingOnly_addDrawOp(
    const GrClip& clip, std::unique_ptr<GrDrawOp> op,
  ...
  GR_AUDIT_TRAIL_AUTO_FRAME(
      fRenderTargetContext->auditTrail(), "GrRenderTargetContext::testingOnly_addDrawOp");
...
#define DRAW_OP_TEST_EXTERN(Op) \
  extern std::unique_ptr<GrDrawOp> Op##__Test(GrPaint&&, SkRandom*, GrRecordingContext*, GrFSAAType)
...
void GrDrawRandomOp(SkRandom* random, GrRenderTargetContext* renderTargetContext, GrPaint&& paint) {
  ...
  using MakeDrawOpFn =
      std::unique_ptr<GrDrawOp>(GrPaint&&, SkRandom*, GrRecordingContext*, GrFSAAType);
  static constexpr MakeDrawOpFn* gFactories[] = {
      ...
      DRAW_OP_TEST_ENTRY(CircleOp),          DRAW_OP_TEST_ENTRY(DashOp),
      DRAW_OP_TEST_ENTRY(DefaultPathOp),     DRAW_OP_TEST_ENTRY(DIEllipseOp),
      DRAW_OP_TEST_ENTRY(EllipseOp),         DRAW_OP_TEST_ENTRY(FillRectOp),
      DRAW_OP_TEST_ENTRY(GrAtlasTextOp),     DRAW_OP_TEST_ENTRY(DrawAtlasOp),
  ...
  auto op = gFactories[index](std::move(paint), random, context, renderTargetContext->fsaaType());
  SkASSERT(op);
