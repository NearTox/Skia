...
class GLTestAtlasTextRenderer : public sk_gpu_test::TestAtlasTextRenderer {
 public:
  ...
  void setTextureData(
      void* textureHandle, const void* data, int x, int y, int width, int height,
      size_t rowBytes) override;

  void drawSDFGlyphs(
      void* targetHandle, void* textureHandle, const SDFVertex vertices[], int quadCnt) override;
...
GLTestAtlasTextRenderer::GLTestAtlasTextRenderer(std::unique_ptr<GLTestContext> context)
  ...
  switch (standard) {
    ...
    case kNone_GrGLStandard: return;
...
inline bool atlas_format_to_gl_types(
    SkAtlasTextRenderer::AtlasFormat format, GrGLenum* internalFormat, GrGLenum* externalFormat,
    GrGLenum* type) {
  switch (format) {
...
inline int atlas_format_bytes_per_pixel(SkAtlasTextRenderer::AtlasFormat format) {
  switch (format) {
    case SkAtlasTextRenderer::AtlasFormat::kA8: return 1;
  }
  return 0;
}
...
void* GLTestAtlasTextRenderer::createTexture(AtlasFormat format, int width, int height) {
  ...
  callgl(
      TexImage2D, GR_GL_TEXTURE_2D, 0, internalFormat, width, height, 0, externalFormat, type,
      nullptr);
  checkgl();
...
void GLTestAtlasTextRenderer::setTextureData(
    void* textureHandle, const void* data, int x, int y, int width, int height, size_t rowBytes) {
  auto restore = fContext->makeCurrentAndAutoRestore();
...
void GLTestAtlasTextRenderer::drawSDFGlyphs(
    void* targetHandle, void* textureHandle, const SDFVertex vertices[], int quadCnt) {
  auto restore = fContext->makeCurrentAndAutoRestore();
  ...
  callgl(
      VertexAttribPointer, 1, 4, GR_GL_UNSIGNED_BYTE, GR_GL_TRUE, sizeof(SDFVertex),
      reinterpret_cast<const char*>(vertices) + colorOffset);
  size_t texOffset = colorOffset + sizeof(uint32_t);
  callgl(
      VertexAttribIPointer, 2, 2, GR_GL_UNSIGNED_SHORT, sizeof(SDFVertex),
...
void* GLTestAtlasTextRenderer::makeTargetHandle(int width, int height) {
  ...
  callgl(
      FramebufferRenderbuffer, GR_GL_FRAMEBUFFER, GR_GL_COLOR_ATTACHMENT0, GR_GL_RENDERBUFFER, rb);
...
SkBitmap GLTestAtlasTextRenderer::readTargetHandle(void* targetHandle) {
  ...
  auto info = SkImageInfo::Make(
      target->fWidth, target->fHeight, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
  SkBitmap bmp;
  ...
  callgl(BindFramebuffer, GR_GL_FRAMEBUFFER, target->fFBOID);
  callgl(
      ReadPixels, 0, 0, target->fWidth, target->fHeight, GR_GL_RGBA, GR_GL_UNSIGNED_BYTE,
      bmp.getPixels());
  checkgl();
  return bmp;
}
