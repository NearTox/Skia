...
sk_sp<GrTextureProxy> MakeTextureProxyFromData(
    ...
    SkAlphaType alphaType, GrSurfaceOrigin origin, const void* data, size_t rowBytes) {
  ...
  if (kBottomLeft_GrSurfaceOrigin == origin) {
    ...
    if (GrRenderable::kYes == renderable) {
      proxy = context->priv().proxyProvider()->wrapRenderableBackendTexture(
          backendTex, origin, 1, colorType, kAdopt_GrWrapOwnership, GrWrapCacheable::kNo, nullptr,
          nullptr);
    } else {
      proxy = context->priv().proxyProvider()->wrapBackendTexture(
          backendTex, colorType, origin, kAdopt_GrWrapOwnership, GrWrapCacheable::kNo,
          kRW_GrIOType);
    }
    ...
    proxy = context->priv().proxyProvider()->createProxy(
        format, desc, renderable, 1, origin, GrMipMapped::kNo, SkBackingFit::kExact,
  ...
  if (!sContext->writePixels(
          {colorType, alphaType, nullptr, width, height}, data, rowBytes, {0, 0}, context)) {
    return nullptr;
  }
  return proxy;
}
