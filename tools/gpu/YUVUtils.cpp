...
class Generator : public SkImageGenerator {
 public:
  Generator(SkYUVAPixmaps pixmaps, sk_sp<SkColorSpace> cs)
      : SkImageGenerator(SkImageInfo::Make(
            pixmaps.yuvaInfo().dimensions(), kN32_SkColorType, kPremul_SkAlphaType, std::move(cs))),
    ...
    if (kUnknown_SkColorType == fFlattened.colorType()) {
      ...
      for (int y = 0; y < info.height(); ++y) {
        for (int x = 0; x < info.width(); ++x) {
          ...
          for (auto c : {SkYUVAIndex::kY_Index, SkYUVAIndex::kU_Index, SkYUVAIndex::kV_Index}) {
          ...
          if (yuvaIndices[SkYUVAIndex::kA_Index].fIndex >= 0) {
            const auto& pmap = fPixmaps.plane(yuvaIndices[SkYUVAIndex::kA_Index].fIndex);
            yuva[3] = look_up(x1, y1, pmap, yuvaIndices[SkYUVAIndex::kA_Index].fChannel);
          }
...
std::unique_ptr<LazyYUVImage> LazyYUVImage::Make(
    SkYUVAPixmaps pixmaps, GrMipmapped mipmapped, sk_sp<SkColorSpace> cs) {
  std::unique_ptr<LazyYUVImage> image(new LazyYUVImage());
...
bool LazyYUVImage::ensureYUVImage(GrRecordingContext* rContext, Type type) {
  ...
  switch (type) {
    case Type::kFromPixmaps:
      ...
      fYUVImage[idx] = SkImage::MakeFromYUVAPixmaps(
          rContext, fPixmaps, fMipmapped,
          /*limit to max tex size*/ false, fColorSpace);
      break;
    ...
    case Type::kFromTexturesCopyToExternal:
      ...
      if (auto direct = rContext->asDirectContext()) {
        ...
        GrYUVABackendTextures yuvaTextures(fPixmaps.yuvaInfo(), textures, kTopLeft_GrSurfaceOrigin);
        if (!yuvaTextures.isValid()) {
          return false;
        }
        if (type == Type::kFromTextures) {
          void* planeRelContext = sk_gpu_test::ManagedBackendTexture::MakeYUVAReleaseContext(mbets);
          fYUVImage[idx] = SkImage::MakeFromYUVATextures(
              direct, yuvaTextures, fColorSpace, sk_gpu_test::ManagedBackendTexture::ReleaseProc,
              planeRelContext);
        } else {
          SkASSERT(type == Type::kFromTexturesCopyToExternal);
          sk_sp<sk_gpu_test::ManagedBackendTexture> rgbaMBET;
          for (auto ct :
               {SkYUVAPixmaps::RecommendedRGBAColorType(fPixmaps.dataType()),
                kRGBA_8888_SkColorType}) {
            rgbaMBET = sk_gpu_test::ManagedBackendTexture::MakeWithoutData(
                direct, fPixmaps.yuvaInfo().width(), fPixmaps.yuvaInfo().height(),
                kUnknown_SkColorType, GrMipmapped::kNo, GrRenderable::kYes);
            if (rgbaMBET) {
              ...
              fYUVImage[idx] = SkImage::MakeFromYUVATexturesCopyToExternal(
                  direct, yuvaTextures, rgbaMBET->texture(), ct, fColorSpace,
                  sk_gpu_test::ManagedBackendTexture::ReleaseProc, planeRelContext,
                  sk_gpu_test::ManagedBackendTexture::ReleaseProc, rgbaMBET->releaseContext());
              if (fYUVImage[idx]) {
                break;
              }
            }
