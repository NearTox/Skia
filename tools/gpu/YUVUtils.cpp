...
bool LazyYUVImage::reset(sk_sp<SkData> data, GrMipmapped mipmapped) {
  ...
  for (int i = 0; i < SkYUVASizeInfo::kMaxCount; ++i) {
    if (fSizeInfo.fSizes[i].isEmpty()) {
      ...
      SkASSERT(planes[i]);
      auto planeInfo = SkImageInfo::Make(
          fSizeInfo.fSizes[i].fWidth, fSizeInfo.fSizes[i].fHeight, kGray_8_SkColorType,
...
bool LazyYUVImage::ensureYUVImage(GrRecordingContext* rContext) {
  ...
  fYUVImage = SkImage::MakeFromYUVAPixmaps(
      rContext->priv().backdoor(), fColorSpace, fPlanes, fComponents, fSizeInfo.fSizes[0],
      kTopLeft_GrSurfaceOrigin, static_cast<bool>(fMipmapped), false);
  return fYUVImage != nullptr;
}

///////////////////////////////////////////////////////////////////////////////////////////////////
void YUVABackendReleaseContext::Unwind(
    GrDirectContext* dContext, YUVABackendReleaseContext* beContext, bool fullFlush) {
...
template <int I>
static void CreationComplete(void* releaseContext) {
