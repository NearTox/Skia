...
bool LoadVkLibraryAndGetProcAddrFuncs(
  ...
  if (!vkLib) {
    ...
    localInstProc = (PFN_vkGetInstanceProcAddr)GetProcedureAddress(vkLib, "vkGetInstanceProcAddr");
    localDevProc = (PFN_vkGetDeviceProcAddr)GetProcedureAddress(vkLib, "vkGetDeviceProcAddr");
  }
...
static int should_include_debug_layer(
    const char* layerName, uint32_t layerCount, VkLayerProperties* layers, uint32_t version) {
...
#  ifdef SK_ENABLE_VK_LAYERS
  ...
  for (size_t i = 0; i < SK_ARRAY_COUNT(kDebugLayerNames); ++i) {
    int idx = should_include_debug_layer(kDebugLayerNames[i], layerCount, layers, nonPatchVersion);
    if (idx != -1) {
...
static bool init_device_extensions_and_layers(
    ...
    SkTArray<VkExtensionProperties>* deviceExtensions, SkTArray<VkLayerProperties>* deviceLayers) {
  if (getProc == nullptr) {
    return false;
  }
...
#  endif
  ...
  for (uint32_t layerIndex = 0; layerIndex < layerCount; ++layerIndex) {
    ...
    res = EnumerateDeviceExtensionProperties(
        physDev, (*deviceLayers)[layerIndex].layerName, &extensionCount, nullptr);
    if (VK_SUCCESS != res) {
      return false;
    }
    VkExtensionProperties* extensions = new VkExtensionProperties[extensionCount];
    res = EnumerateDeviceExtensionProperties(
        physDev, (*deviceLayers)[layerIndex].layerName, &extensionCount, extensions);
    if (VK_SUCCESS != res) {
      delete[] extensions;
      return false;
    }
...
static bool setup_features(
    GrVkGetProc getProc, VkInstance inst, VkPhysicalDevice physDev, uint32_t physDeviceVersion,
    GrVkExtensions* extensions, VkPhysicalDeviceFeatures2* features, bool isProtected) {
  SkASSERT(
      physDeviceVersion >= VK_MAKE_VERSION(1, 1, 0) ||
  ...
  if (isProtected) {
    ...
    protectedMemoryFeatures = (VkPhysicalDeviceProtectedMemoryFeatures*)sk_malloc_throw(
  ...
  if (physDeviceVersion >= VK_MAKE_VERSION(1, 1, 0)) {
    ...
    SkASSERT(extensions->hasExtension(VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME, 1));
...
#  endif
  ...
  extensions->init(
      getProc, inst, physDev, (uint32_t)instanceExtensionNames.count(),
      instanceExtensionNames.begin(), (uint32_t)deviceExtensionNames.count(),
