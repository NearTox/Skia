...
GrVkGpu* VkYcbcrSamplerHelper::vkGpu() { return (GrVkGpu*)fDContext->priv().getGpu(); }
...
bool VkYcbcrSamplerHelper::createBackendTexture(uint32_t width, uint32_t height) {
  ...
  GR_VK_CALL(
      ...
      GetPhysicalDeviceFormatProperties(
          vkGpu->physicalDevice(), VK_FORMAT_G8_B8R8_2PLANE_420_UNORM, &formatProperties));
  ...
  GR_VK_CALL(
      vkGpu->vkInterface(), GetImageMemoryRequirements(vkGpu->device(), fImage, &requirements));
  ...
  GR_VK_CALL(
      vkGpu->vkInterface(),
      GetPhysicalDeviceMemoryProperties(vkGpu->physicalDevice(), &phyDevMemProps));
  ...
  GR_VK_CALL_RESULT(
      vkGpu, result, AllocateMemory(vkGpu->device(), &allocInfo, nullptr, &fImageMemory));
  if (result != VK_SUCCESS) {
    return false;
  }
  ...
  GR_VK_CALL_RESULT(
      vkGpu, result,
      MapMemory(vkGpu->device(), fImageMemory, 0u, requirements.size, 0u, &mappedBuffer));
  if (result != VK_SUCCESS) {
    return false;
  }
  ...
  GR_VK_CALL(
      vkGpu->vkInterface(),
      GetImageSubresourceLayout(vkGpu->device(), fImage, &subresource, &yLayout));
  ...
  GR_VK_CALL(
      vkGpu->vkInterface(),
      GetImageSubresourceLayout(vkGpu->device(), fImage, &subresource, &uvLayout));
