...
class VkFenceSync : public sk_gpu_test::FenceSync {
 public:
  VkFenceSync(GrVkGetProc getProc, VkDevice device, VkQueue queue, uint32_t queueFamilyIndex)
    ...
    VkResult result;
    SkDEBUGCODE(fUnfinishedSyncs = 0;) VkCommandPoolCreateInfo createInfo;
  ...
  sk_gpu_test::PlatformFence SK_WARN_UNUSED_RESULT insertFence() const override {
    ...
    SkDEBUGCODE(++fUnfinishedSyncs;) return (sk_gpu_test::PlatformFence)fence;
  }
  ...
  static VkTestContext* Create(VkTestContext* sharedContext) {
    ...
    if (sharedContext) {
      ...
      if (!sk_gpu_test::CreateVkBackendContext(
              getProc, &backendContext, extensions, features, &debugCallback)) {
    ...
    return new VkTestContextImpl(
        backendContext, extensions, features, ownsContext, debugCallback, destroyCallback);
  }
...
#  endif
 ...
 private:
  VkTestContextImpl(
      const GrVkBackendContext& backendContext, const GrVkExtensions* extensions,
      VkPhysicalDeviceFeatures2* features, bool ownsContext, VkDebugReportCallbackEXT debugCallback,
      PFN_vkDestroyDebugReportCallbackEXT destroyCallback)
      : VkTestContext(
            backendContext, extensions, features, ownsContext, debugCallback, destroyCallback) {
    fFenceSync.reset(
        new VkFenceSync(fVk.fGetProc, fVk.fDevice, fVk.fQueue, fVk.fGraphicsQueueIndex));
  }
