...
WinGLTestContext::WinGLTestContext(GrGLStandard forcedGpuAPI, WinGLTestContext* shareContext)
  ...
  if (!(fWindow = CreateWindow(
            TEXT("Griffin"), TEXT("The Invisible Man"), WS_OVERLAPPEDWINDOW, 0, 0, 1, 1, nullptr,
  ...
  SkWGLContextRequest contextType = kGLES_GrGLStandard == forcedGpuAPI
  ...
  if (nullptr == fPbufferContext) {
    if (!(fGlRenderContext =
              SkCreateWGLContext(fDeviceContext, 0, false, contextType, winShareContext))) {
