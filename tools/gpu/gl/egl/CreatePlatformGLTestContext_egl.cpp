...
void EGLGLTestContext::setupFenceSync(sk_sp<const GrGLInterface> interface) {
  ...
  auto grEGLGetSyncAttribKHR = (PFNEGLGETSYNCATTRIBKHRPROC)eglGetProcAddress("eglGetSyncAttribKHR");
  SkASSERT(
      grEGLCreateSyncKHR && grEGLClientWaitSyncKHR && grEGLDestroySyncKHR && grEGLGetSyncAttribKHR);
  ...
  glInt->fFunctions.fFenceSync = [grEGLCreateSyncKHR, display = fDisplay](
                                     GrGLenum condition, GrGLbitfield flags) {
  ...
  glInt->fFunctions.fClientWaitSync = [grEGLClientWaitSyncKHR, display = fDisplay](
    ...
    switch (result) {
      case EGL_CONDITION_SATISFIED_KHR: return GR_GL_CONDITION_SATISFIED;
      case EGL_TIMEOUT_EXPIRED_KHR: return GR_GL_TIMEOUT_EXPIRED;
      case EGL_FALSE: return GR_GL_WAIT_FAILED;
    }
    SkUNREACHABLE;
  };

  glInt->fFunctions.fWaitSync = [grEGLClientWaitSyncKHR, grEGLWaitSyncKHR, display = fDisplay](
  ...
  glInt->fFunctions.fIsSync = [grEGLGetSyncAttribKHR,
                               display = fDisplay](GrGLsync sync) -> GrGLboolean {
    EGLSyncKHR eglSync = reinterpret_cast<EGLSyncKHR>(sync);
    EGLint value;
