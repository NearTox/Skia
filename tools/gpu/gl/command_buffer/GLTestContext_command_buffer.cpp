...
typedef EGLContext (*CreateContextProc)(
...
typedef EGLBoolean (*MakeCurrentProc)(
...
#  endif  // defined _WIN32
    ...
    if (gLibrary) {
      ...
      gfCreateWindowSurface =
          (CreateWindowSurfaceProc)SkGetProcedureAddress(gLibrary, "eglCreateWindowSurface");
      gfCreatePbufferSurface =
          (CreatePbufferSurfaceProc)SkGetProcedureAddress(gLibrary, "eglCreatePbufferSurface");
      ...
      gfFunctionsLoadedSuccessfully =
          gfGetDisplay && gfInitialize && gfTerminate && gfChooseConfig && gfCreateWindowSurface &&
...
class TLSCurrentObjects {
 public:
  ...
  static EGLSurface CurrentSurface(EGLint readdraw) {
    if (auto objects = Get()) {
      switch (readdraw) {
        case EGL_DRAW: return objects->fDrawSurface;
        case EGL_READ: return objects->fReadSurface;
  ...
  static EGLBoolean MakeCurrent(
      EGLDisplay display, EGLSurface draw, EGLSurface read, EGLContext ctx) {
...
CommandBufferGLTestContext::CommandBufferGLTestContext(
    int version, CommandBufferGLTestContext* shareContext)
    : fContext(EGL_NO_CONTEXT), fDisplay(EGL_NO_DISPLAY), fSurface(EGL_NO_SURFACE) {
  EGLint renderableType;
  switch (version) {
    case 2: renderableType = EGL_OPENGL_ES2_BIT; break;
  ...
  EGLint numConfigs;
  if (!gfChooseConfig(fDisplay, configAttribs, static_cast<EGLConfig*>(&fConfig), 1, &numConfigs) ||
      numConfigs != 1) {
  ...
  fSurface = gfCreatePbufferSurface(fDisplay, static_cast<EGLConfig>(fConfig), surfaceAttribs);
  ...
  EGLContext eglShareContext =
      shareContext ? reinterpret_cast<EGLContext>(shareContext->fContext) : nullptr;
  fContext =
      gfCreateContext(fDisplay, static_cast<EGLConfig>(fConfig), eglShareContext, contextAttribs);
  ...
  if (strstr(versionString, expectedVersion) != versionString) {
    SkDebugf(
        "Command Buffer: Unexpected version.\n"
        "           Got: \"%s\".\n"
        "      Expected: \"%s ...\".\n",
