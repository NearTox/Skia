...
DDLTileHelper::DDLTileHelper(
    ...
    int numDivisions, bool addRandomPaddingToDst)
  ...
  for (int y = 0, yOff = 0; y < fNumDivisions; ++y, yOff += yTileSize) {
    ...
    for (int x = 0, xOff = 0; x < fNumDivisions; ++x, xOff += xTileSize) {
      ...
      fTiles[y * fNumDivisions + x].init(
          y * fNumDivisions + x, direct, dstChar, clip, {lPad, tPad, rPad, bPad});
    }
  }
}

void DDLTileHelper::createSKPPerTile(
    SkData* compressedPictureData, const DDLPromiseImageHelper& helper) {
  for (int i = 0; i < this->numTiles(); ++i) {
...
void DDLTileHelper::kickOffThreadedWork(
    SkTaskGroup* recordingTaskGroup, SkTaskGroup* gpuTaskGroup, GrDirectContext* direct) {
  ...
  for (int i = 0; i < this->numTiles(); ++i) {
    ...
    recordingTaskGroup->add([tile, gpuTaskGroup, direct]() {
      ...
      gpuTaskGroup->add([direct, tile]() { do_gpu_stuff(direct, tile); });
    });
  }
