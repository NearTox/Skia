...
void DDLTileHelper::TileData::init(
    int id, GrContext* context, const SkSurfaceCharacterization& dstSurfaceCharacterization,
    const SkIRect& clip) {
  fID = id;
  fClip = clip;
  ...
  GrBackendFormat backendFormat =
      context->defaultBackendFormat(fCharacterization.colorType(), GrRenderable::kYes);
...
void DDLTileHelper::kickOffThreadedWork(
  ...
  for (int i = 0; i < this->numTiles(); ++i) {
    ...
    recordingTaskGroup->add([tile, gpuTaskGroup, gpuThreadContext]() {
      ...
      gpuTaskGroup->add([gpuThreadContext, tile]() { do_gpu_stuff(gpuThreadContext, tile); });
    });
  }
