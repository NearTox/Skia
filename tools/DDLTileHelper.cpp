...
void DDLTileHelper::TileData::init(
    int id, GrDirectContext* direct, const SkSurfaceCharacterization& dstSurfaceCharacterization,
    const SkIRect& clip, const SkIRect& paddingOutsets) {
  fID = id;
  fClip = clip;
  ...
  SkASSERT(caps->isFormatTexturable(
      fPlaybackChar.backendFormat(), fPlaybackChar.backendFormat().textureType()));
...
void DDLTileHelper::createComposeDDL() {
  ...
  for (int i = 0; i < this->numTiles(); ++i) {
    ...
    sk_sp<SkImage> promiseImage =
        tile->makePromiseImageForDst(recordingCanvas->recordingContext()->threadSafeProxy());
    ...
    recordingCanvas->drawImageRect(
        promiseImage.get(), SkRect::Make(srcRect), dstRect, SkSamplingOptions(), nullptr,
...
sk_sp<SkSurface> DDLTileHelper::TileData::makeWrappedTileDest(GrRecordingContext* rContext) {
  ...
  return SkSurface::MakeFromBackendTexture(
      rContext, promiseImageTexture->backendTexture(), fPlaybackChar.origin(),
      fPlaybackChar.sampleCount(), fPlaybackChar.colorType(), fPlaybackChar.refColorSpace(),
      &fPlaybackChar.surfaceProps());
}

void DDLTileHelper::TileData::drawSKPDirectly(GrDirectContext* dContext, const SkPicture* picture) {
...
sk_sp<SkImage> DDLTileHelper::TileData::makePromiseImageForDst(
  ...
  sk_sp<SkImage> promiseImage = SkImage::MakePromiseTexture(
      std::move(threadSafeProxy), fCallbackContext->backendFormat(), this->paddedRectSize(),
      GrMipmapped::kNo, GrSurfaceOrigin::kBottomLeft_GrSurfaceOrigin, fPlaybackChar.colorType(),
      kPremul_SkAlphaType, fPlaybackChar.refColorSpace(),
...
void DDLTileHelper::TileData::DeleteBackendTexture(GrDirectContext*, TileData* tile) {
  ...
  SkASSERT(
      !tile->fCallbackContext->promiseImageTexture() ||
...
DDLTileHelper::DDLTileHelper(
    GrDirectContext* direct, const SkSurfaceCharacterization& dstChar, const SkIRect& viewport,
    int numXDivisions, int numYDivisions, bool addRandomPaddingToDst)
  ...
  for (int y = 0, yOff = 0; y < fNumYDivisions; ++y, yOff += yTileSize) {
    ...
    for (int x = 0, xOff = 0; x < fNumXDivisions; ++x, xOff += xTileSize) {
      ...
      fTiles[y * fNumXDivisions + x].init(
          y * fNumXDivisions + x, direct, dstChar, clip, {lPad, tPad, rPad, bPad});
    }
  }
}
...
#if 1
  SkTaskGroup().batch(this->numTiles(), [&](int i) { fTiles[i].createDDL(picture); });
...
void DDLTileHelper::kickOffThreadedWork(
    SkTaskGroup* recordingTaskGroup, SkTaskGroup* gpuTaskGroup, GrDirectContext* dContext,
    SkPicture* picture) {
  ...
  for (int i = 0; i < this->numTiles(); ++i) {
    ...
    recordingTaskGroup->add([tile, gpuTaskGroup, dContext, picture]() {
      ...
      gpuTaskGroup->add([dContext, tile]() { do_gpu_stuff(dContext, tile); });
    });
  }
