...
void DDLTileHelper::TileData::init(
    int id, sk_sp<SkSurface> dstSurface,
    const SkSurfaceCharacterization& dstSurfaceCharacterization, const SkIRect& clip) {
  fID = id;
...
void DDLTileHelper::TileData::drawSKPDirectly(GrContext* context) {
  ...
  sk_sp<SkSurface> tileSurface =
      SkSurface::MakeRenderTarget(context, fCharacterization, SkBudgeted::kYes);
  if (tileSurface) {
...
DDLTileHelper::DDLTileHelper(
    sk_sp<SkSurface> dstSurface, const SkSurfaceCharacterization& dstChar, const SkIRect& viewport,
    int numDivisions)
