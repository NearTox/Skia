...
void MSKPPlayer::DrawLayerCmd::draw(
    SkCanvas* canvas, const LayerMap& layerMap, LayerStateMap* layerStateMap) const {
  ...
  canvas->drawImageRect(
      layerState->fSurface->makeImageSnapshot(), fSrcRect, fDstRect, fSampling, paint, fConstraint);
}
...
class MSKPPlayer::CmdRecordCanvas : public SkCanvasVirtualEnforcer<SkCanvas> {
 ...
 protected:
  ...
  void onDrawArc(
      const SkRect& rect, SkScalar startAngle, SkScalar sweepAngle, bool useCenter,
  ...
  void onDrawPoints(
      SkCanvas::PointMode mode, size_t count, const SkPoint pts[], const SkPaint& paint) override {
    fRecorder.getRecordingCanvas()->drawPoints(mode, count, pts, paint);
  }

  void onDrawImage2(
      const SkImage* image, SkScalar dx, SkScalar dy, const SkSamplingOptions& sampling,
      const SkPaint* paint) override {
  ...
  void onDrawImageLattice2(
      const SkImage* image, const Lattice& lattice, const SkRect& dst, SkFilterMode mode,
      const SkPaint* paint) override {
    fRecorder.getRecordingCanvas()->drawImageLattice(image, lattice, dst, mode, paint);
  }

  void onDrawAtlas2(
      const SkImage* image, const SkRSXform rsxForms[], const SkRect src[], const SkColor colors[],
      int count, SkBlendMode mode, const SkSamplingOptions& sampling, const SkRect* cull,
      const SkPaint* paint) override {
    fRecorder.getRecordingCanvas()->drawAtlas(
        image, rsxForms, src, colors, count, mode, sampling, cull, paint);
  }
  ...
  void onDrawEdgeAAImageSet2(
    ...
    fRecorder.getRecordingCanvas()->experimental_DrawEdgeAAImageSet(
        imageSet, count, dstClips, preViewMatrices, sampling, paint, constraint);
  }
  ...
  void onDrawEdgeAAQuad(
      const SkRect& rect, const SkPoint clip[4], SkCanvas::QuadAAFlags aaFlags,
      const SkColor4f& color, SkBlendMode mode) override {
    fRecorder.getRecordingCanvas()->experimental_DrawEdgeAAQuad(rect, clip, aaFlags, color, mode);
  }
#endif
    ...
    if (tokens.size() == 2) {
      if (tokens[0].equals(kOffscreenLayerDraw)) {
        ...
        if (fOffscreenLayers->find(fNextDrawPictureToLayerID) == fOffscreenLayers->end()) {
          SkASSERT(
              fNextDrawPictureToLayerClipRect.left() == 0 &&
  ...
  void onDrawPicture(
      const SkPicture* picture, const SkMatrix* matrix, const SkPaint* paint) override {
...
std::unique_ptr<MSKPPlayer> MSKPPlayer::Make(SkStreamSeekable* stream) {
  ...
  for (const auto& page : pages) {
    SkISize dims = {SkScalarCeilToInt(page.fSize.width()), SkScalarCeilToInt(page.fSize.height())};
...
sk_sp<SkSurface> MSKPPlayer::MakeSurfaceForLayer(const LayerCmds& layer, SkCanvas* rootCanvas) {
  ...
  return rootCanvas->makeSurface(rootCanvas->imageInfo().makeDimensions(layer.fDimensions), &props);
}
