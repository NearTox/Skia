...
SkColor color_to_565(SkColor color) {
  ...
  SkASSERT(
      SkColorGetR(color) != SkColorGetG(color) || SkColorGetR(color) != SkColorGetB(color) ||
...
SkBitmap create_string_bitmap(
    int w, int h, SkColor c, int x, int y, int textSize, const char* str) {
  SkBitmap bitmap;
  ...
  canvas.drawSimpleText(
      str, strlen(str), SkTextEncoding::kUTF8, SkIntToScalar(x), SkIntToScalar(y), font, paint);
...
void add_to_text_blob_w_len(
    SkTextBlobBuilder* builder, const char* text, size_t len, SkTextEncoding encoding,
...
void get_text_path(
    const SkFont& font, const void* text, size_t length, SkTextEncoding encoding, SkPath* dst,
    const SkPoint pos[]) {
...
void copy_to_g8(SkBitmap* dst, const SkBitmap& src) {
  SkASSERT(kBGRA_8888_SkColorType == src.colorType() || kRGBA_8888_SkColorType == src.colorType());
...
sk_sp<SkSurface> makeSurface(
    SkCanvas* canvas, const SkImageInfo& info, const SkSurfaceProps* props) {
