...
static const Result ok = {Result::Ok, {}}, skip = {Result::Skip, {}};

static Result fail(SkString why) { return {Result::Fail, why}; }
    ...
    source->draw = [animation](SkCanvas* canvas) {
        ...
        for (int x : order) {
            ...
            canvas->concat(SkMatrix::RectToRect(
                SkRect::MakeSize(animation->size()), dst, SkMatrix::kCenter_ScaleToFit));
    ...
    switch (surfaceType) {
        ...
        case SurfaceType::kBackendTexture:
          surface = sk_gpu_test::MakeBackendTextureSurface(
              context, info, kTopLeft_GrSurfaceOrigin, FLAGS_samples, GrMipmapped::kNo,
              GrProtected::kNo, &props);
          break;

        case SurfaceType::kBackendRenderTarget:
          surface = sk_gpu_test::MakeBackendRenderTargetSurface(
              context, info, kBottomLeft_GrSurfaceOrigin, FLAGS_samples, GrProtected::kNo, &props);
          break;
    }
...
TestHarness CurrentTestHarness() { return TestHarness::kFM; }
...
int main(int argc, char** argv) {
    ...
    const FlagOption<skcms_Matrix3x3> kGamuts[] = {
        {"srgb", SkNamedGamut::kSRGB},       {"p3", SkNamedGamut::kDisplayP3},
        {"rec2020", SkNamedGamut::kRec2020}, {"adobe", SkNamedGamut::kAdobeRGB},
      ...
      SkTaskGroup{}.batch(replicas, [=](int replica) {
        ...
        auto draw = [&source](SkCanvas* canvas) {
            ...
            switch (result.status) {
                ...
                case Result::Fail: SK_ABORT("%s", result.failure.c_str());
            }
            return true;
        };
        ...
        if (!FLAGS_quick) {
          ...
          if (!FLAGS_writePath.isEmpty()) {
            ...
            if (image) {
              if (!hashAndEncode.encodePNG(&file, md5.c_str(), FLAGS_key, FLAGS_properties)) {
