...
static sk_sp<SkSurface> make_gl_surface(GrDirectContext* dContext, int width, int height) {
  ...
  return SkSurface::MakeFromBackendRenderTarget(
      ...
      GrBackendRenderTarget(
          width, height, kSampleCount, kStencilBits,
          GrGLFramebufferInfo{(GrGLuint)fboid, GL_RGBA8}),
      kBottomLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, nullptr, &surfaceProps);
}
...
- (void)drawRect:(CGRect)rect {
  ...
  int width = (int)[self drawableWidth], height = (int)[self drawableHeight];
  if (!(fDContext)) {
  ...
  if (sk_sp<SkSurface> surface = make_gl_surface(fDContext, width, height)) {
    [viewController draw:rect
                toCanvas:(surface->getCanvas())atSize:CGSize{(CGFloat)width, (CGFloat)height}];
