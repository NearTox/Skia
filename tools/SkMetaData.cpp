...
void* SkMetaData::set(const char name[], const void* data, size_t dataSize, Type type, int count) {
  ...
  Rec* rec;
  bool reuseRec = result.rec && result.rec->fDataLen == dataSize && result.rec->fDataCount == count;
  if (reuseRec) {
    rec = result.rec;
  } else {
...
SkMetaData::FindResult SkMetaData::findWithPrev(const char name[], Type type) const {
  ...
  while (current.rec) {
    if (current.rec->fType == type && !strcmp(current.rec->name(), name)) return current;
