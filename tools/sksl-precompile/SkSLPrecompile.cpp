 ...
 */
static void show_usage() { printf("usage: sksl-precompile <output> <input>\n"); }
...
ResultCode processCommand(const std::vector<std::string>& paths) {
  ...
  for (int inputIdx = paths.size() - 1; inputIdx >= 1; --inputIdx) {
    ...
    modules.push_front(compiler.loadModule(
        SkSL::ProgramKind::kFragment, SkSL::Compiler::MakeModulePath(modulePath.c_str()),
