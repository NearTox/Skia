...
class RandomScalerContext : public SkScalerContext {
 public:
  RandomScalerContext(
      sk_sp<SkRandomTypeface>, const SkScalerContextEffects&, const SkDescriptor*, bool fFakeIt);
...
RandomScalerContext::RandomScalerContext(
    sk_sp<SkRandomTypeface> face, const SkScalerContextEffects& effects, const SkDescriptor* desc,
    bool fakeIt)
...
void RandomScalerContext::generateImage(const SkGlyph& glyph) {
  ...
  */
  ...
  SkBitmap bm;
  bm.installPixels(
      SkImageInfo::MakeN32Premul(glyph.fWidth, glyph.fHeight), glyph.fImage, glyph.rowBytes());
  bm.eraseColor(0);
