...
struct DragCanvas {
  DragCanvas(
      const void* id, SkPoint tl = {0.0f, 0.0f}, SkPoint br = {1.0f, 1.0f}, float aspect = -1.0f)
      : fID(0), fDragging(false) {
    ImGui::PushID(id);
    fDrawList = ImGui::GetWindowDrawList();

    // Logical size
    SkScalar w = SkTAbs(br.fX - tl.fX), h = SkTAbs(br.fY - tl.fY);
  ...
  void dragPoint(SkPoint* p, bool tooltip = false, ImU32 color = 0xFFFFFFFF) {
    ...
    if (ImGui::IsItemActive() && ImGui::IsMouseDragging()) {
      ...
      center.set(
          SkTPin(io.MousePos.x, fPos.x, fPos.x + fSize.x),
