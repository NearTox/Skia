...
static double getseconds() { return SkTime::GetMSecs() * 0.001; }
...
TouchGesture::TouchGesture() { this->reset(); }
...
static SkScalar center(float pos0, float pos1) { return (pos0 + pos1) * 0.5f; }
...
bool TouchGesture::handleDblTap(float x, float y) {
  ...
  if (now - fLastUpMillis <= MAX_DBL_TAP_INTERVAL) {
    if (SkPoint::Length(fLastUpP.fX - x, fLastUpP.fY - y) <= MAX_DBL_TAP_DISTANCE) {
      fFlinger.stop();
      fLocalM.reset();
      fGlobalM.reset();
      fTouches.reset();
...
void TouchGesture::setTransLimit(
    const SkRect& contentRect, const SkRect& windowRect, const SkMatrix& preTouchMatrix) {
