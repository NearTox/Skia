...
void SkSLDebuggerSlide::showCodeTable() {
  constexpr ImGuiTableFlags kTableFlags = ImGuiTableFlags_ScrollY | ImGuiTableFlags_RowBg |
...
void SkSLDebuggerSlide::showStackTraceTable() {
  ...
  constexpr ImGuiTableColumnFlags kColumnFlags =
      ImGuiTableColumnFlags_NoReorder | ImGuiTableColumnFlags_NoHide | ImGuiTableColumnFlags_NoSort;

  std::vector<int> callStack = fPlayer.getCallStack();

  ImVec2 contentRect = ImGui::GetContentRegionAvail();
  ImVec2 stackViewSize =
      ImVec2(contentRect.x / 3.0f, ImGui::GetTextLineHeightWithSpacing() * kNumTopRows);
      ...
      while (clipper.Step()) {
        for (int row = clipper.DisplayStart; row < clipper.DisplayEnd; row++) {
          ...
          if (var.fDirty) {
            ...
            ImGui::TableSetBgColor(
                ImGuiTableBgTarget_RowBg1, ImGui::GetColorU32(ImVec4{0.0f, 1.0f, 0.0f, 0.20f}));
          }
          ImGui::TableSetColumnIndex(0);
          ImGui::Text(
              "%s%s", slotInfo.name.c_str(),
              fTrace->getSlotComponentSuffix(var.fSlotIndex).c_str());
          ImGui::TableSetColumnIndex(1);
          ImGui::Text("%s", fTrace->slotValueToString(var.fSlotIndex, var.fValue).c_str());
        }
      }
    }
    ImGui::EndTable();
...
bool SkSLDebuggerSlide::animate(double nanos) { return true; }
