...
void ImGuiLayer::onAttach(Window* window) { fWindow = window; }
      ...
      if (drawCmd->UserCallback) {
        ...
        if (idIndex < fSkiaWidgetFuncs.count()) {
          ...
          canvas->clipRect(SkRect::MakeLTRB(
              drawCmd->ClipRect.x, drawCmd->ClipRect.y, drawCmd->ClipRect.z, drawCmd->ClipRect.w));
          auto vertices = SkVertices::MakeCopy(
              SkVertices::kTriangles_VertexMode, drawList->VtxBuffer.size(), pos.begin(),
              uv.begin(), color.begin(), drawCmd->ElemCount,
...
bool ImGuiLayer::onKey(
    sk_app::Window::Key key, sk_app::Window::InputState state, uint32_t modifiers) {
  ImGuiIO& io = ImGui::GetIO();
