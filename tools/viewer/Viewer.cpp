...
#define BACKENDS_STR                                                              \
  BACKENDS_STR_EVALUATOR(                                                         \
      "\"sw\"", GL_BACKEND_STR, VK_BACKEND_STR, MTL_BACKEND_STR, D3D_BACKEND_STR, \
      DAWN_BACKEND_STR)
...
static SkColorSpacePrimaries gSrgbPrimaries = {0.64f, 0.33f, 0.30f,   0.60f,
                                               0.15f, 0.06f, 0.3127f, 0.3290f};

static SkColorSpacePrimaries gAdobePrimaries = {0.64f, 0.33f, 0.21f,   0.71f,
                                                0.15f, 0.06f, 0.3127f, 0.3290f};

static SkColorSpacePrimaries gP3Primaries = {0.680f, 0.320f, 0.265f,  0.690f,
                                             0.150f, 0.060f, 0.3127f, 0.3290f};

static SkColorSpacePrimaries gRec2020Primaries = {0.708f, 0.292f, 0.170f,  0.797f,
...
class NullSlide : public Slide {
  SkISize getDimensions() const override { return SkISize::Make(640, 480); }
...
Viewer::Viewer(int argc, char** argv, void* platformData)
  ...
  fCommands.addCommand('c', "Modes", "Cycle color mode", [this]() {
    switch (fColorMode) {
      case ColorMode::kLegacy: this->setColorMode(ColorMode::kColorManaged8888); break;
      case ColorMode::kColorManaged8888: this->setColorMode(ColorMode::kColorManagedF16); break;
      case ColorMode::kColorManagedF16: this->setColorMode(ColorMode::kColorManagedF16Norm); break;
      case ColorMode::kColorManagedF16Norm: this->setColorMode(ColorMode::kLegacy); break;
    }
  });
  ...
  fCommands.addCommand('d', "Modes", "Change rendering backend", [this]() {
    sk_app::Window::BackendType newBackend =
        (sk_app::Window::BackendType)((fBackendType + 1) % sk_app::Window::kBackendTypeCount);
    ...
    if (newBackend == sk_app::Window::kVulkan_BackendType) {
      newBackend =
          (sk_app::Window::BackendType)((newBackend + 1) % sk_app::Window::kBackendTypeCount);
  ...
  fCommands.addCommand('H', "Font", "Hinting mode", [this]() {
    if (!fFontOverrides.fHinting) {
      ...
      switch (fFont.getHinting()) {
        case SkFontHinting::kNone: fFont.setHinting(SkFontHinting::kSlight); break;
        case SkFontHinting::kSlight: fFont.setHinting(SkFontHinting::kNormal); break;
        case SkFontHinting::kNormal: fFont.setHinting(SkFontHinting::kFull); break;
  ...
  fCommands.addCommand('L', "Font", "Subpixel Antialias Mode", [this]() {
    if (!fFontOverrides.fEdging) {
      ...
      switch (fFont.getEdging()) {
        case SkFont::Edging::kAlias: fFont.setEdging(SkFont::Edging::kAntiAlias); break;
        case SkFont::Edging::kAntiAlias: fFont.setEdging(SkFont::Edging::kSubpixelAntiAlias); break;
  ...
  fCommands.addCommand('p', "Transform", "Toggle Perspective Mode", [this]() {
    fPerspectiveMode =
        (kPerspective_Real == fPerspectiveMode) ? kPerspective_Fake : kPerspective_Real;
    this->updateTitle();
    fWindow->inval();
  });
  fCommands.addCommand('P', "Transform", "Toggle Perspective", [this]() {
    fPerspectiveMode =
        (kPerspective_Off == fPerspectiveMode) ? kPerspective_Real : kPerspective_Off;
    this->updateTitle();
    fWindow->inval();
  });
  fCommands.addCommand(
      'a', "Transform", "Toggle Animation", [this]() { fAnimTimer.togglePauseResume(); });
...
void Viewer::initSlides() {
  ...
  const auto addSlide = [&](const SkString& name, const SkString& path, const SlideFactory& fact) {
  ...
  for (const auto& info : gExternalSlidesInfo) {
    for (const auto& flag : info.fFlags) {
      ...
      if (!dirSlides.empty()) {
        fSlides.push_back(sk_make_sp<SlideDir>(
            SkStringPrintf("%s[%s]", info.fDirName, flag.c_str()), std::move(dirSlides)));
...
void Viewer::updateTitle() {
  ...
  auto paintFlag = [this, &paintTitle](
                       bool SkPaintFields::*flag, bool (SkPaint::*isFlag)() const, const char* on,
                       const char* off) {
  ...
  auto fontFlag = [this, &paintTitle](
                      bool SkFontFields::*flag, bool (SkFont::*isFlag)() const, const char* on,
                      const char* off) {
  ...
  fontFlag(
      &SkFontFields::fForceAutoHinting, &SkFont::isForceAutoHinting, "Force Autohint",
      "No Force Autohint");
  ...
  fontFlag(
      &SkFontFields::fLinearMetrics, &SkFont::isLinearMetrics, "Linear Metrics",
      "Non-Linear Metrics");
  fontFlag(
      &SkFontFields::fEmbeddedBitmaps, &SkFont::isEmbeddedBitmaps, "Bitmap Text", "No Bitmap Text");
  fontFlag(&SkFontFields::fSubpixel, &SkFont::isSubpixel, "Subpixel Text", "Pixel Text");

  if (fFontOverrides.fEdging) {
    switch (fFont.getEdging()) {
      case SkFont::Edging::kAlias: paintTitle.append("Alias Text"); break;
      case SkFont::Edging::kAntiAlias: paintTitle.append("Antialias Text"); break;
      case SkFont::Edging::kSubpixelAntiAlias: paintTitle.append("Subpixel Antialias Text"); break;
    }
  }

  if (fFontOverrides.fHinting) {
    switch (fFont.getHinting()) {
      case SkFontHinting::kNone: paintTitle.append("No Hinting"); break;
      case SkFontHinting::kSlight: paintTitle.append("Slight Hinting"); break;
      case SkFontHinting::kNormal: paintTitle.append("Normal Hinting"); break;
      case SkFontHinting::kFull: paintTitle.append("Full Hinting"); break;
    }
  }
  paintTitle.done();

  switch (fColorMode) {
    case ColorMode::kLegacy: title.append(" Legacy 8888"); break;
    case ColorMode::kColorManaged8888: title.append(" ColorManaged 8888"); break;
    case ColorMode::kColorManagedF16: title.append(" ColorManaged F16"); break;
    case ColorMode::kColorManagedF16Norm: title.append(" ColorManaged F16 Norm"); break;
  }
  ...
  if (ColorMode::kLegacy != fColorMode) {
    ...
    title.appendf(
        " %s Gamma %f", curPrimaries >= 0 ? gNamedPrimaries[curPrimaries].fName : "Custom",
  ...
  if (fDisplayOverrides.fSurfaceProps.fPixelGeometry) {
    switch (params.fSurfaceProps.pixelGeometry()) {
      ...
      case kBGR_H_SkPixelGeometry: title.append(" BGR"); break;
      case kRGB_V_SkPixelGeometry: title.append(" RGBV"); break;
      case kBGR_V_SkPixelGeometry: title.append(" BGRV"); break;
    }
  }
...
void Viewer::setCurrentSlide(int slide) {
  ...
  fSlides[slide]->load(
      SkIntToScalar(fWindow->width()) / scaleFactor,
  ...
  if (fCurrentSlide >= 0) {
    ...
    if (fWindow->scaleContentToFit()) {
      if (windowRect.width() > 0 && windowRect.height() > 0) {
        fDefaultMatrix = SkMatrix::RectToRect(slideBounds, windowRect, SkMatrix::kStart_ScaleToFit);
      }
    }
...
class OveridePaintFilterCanvas : public SkPaintFilterCanvas {
 public:
  OveridePaintFilterCanvas(
      SkCanvas* canvas, SkPaint* paint, Viewer::SkPaintFields* pfields, SkFont* font,
  ...
  const SkTextBlob* filterTextBlob(
      const SkPaint& paint, const SkTextBlob* blob, sk_sp<SkTextBlob>* cache) {
      ...
      const SkTextBlobBuilder::RunBuffer& runBuffer =
          it.positioning() == SkTextBlobRunIterator::kDefault_Positioning
              ? builder.allocRunText(
                    font, it.glyphCount(), it.offset().x(), it.offset().y(), it.textSize())
          : it.positioning() == SkTextBlobRunIterator::kHorizontal_Positioning
              ? builder.allocRunTextPosH(font, it.glyphCount(), it.offset().y(), it.textSize())
...
void Viewer::drawSlide(SkSurface* surface) {
  ...
  if (fSaveToSKP) {
    ...
    SkCanvas* recorderCanvas =
        recorder.beginRecording(SkRect::Make(fSlides[fCurrentSlide]->getDimensions()));
  ...
  switch (fColorMode) {
    ...
    case ColorMode::kColorManaged8888: colorType = kN32_SkColorType; break;
    case ColorMode::kColorManagedF16: colorType = kRGBA_F16_SkColorType; break;
    case ColorMode::kColorManagedF16Norm: colorType = kRGBA_F16Norm_SkColorType; break;
  }
  ...
  if (kPerspective_Fake == fPerspectiveMode || fShowZoomWindow || fShowHistogramWindow ||
      Window::kRaster_BackendType == fBackendType || colorSpace != nullptr || FLAGS_offscreen) {
...
static void ImGui_Primaries(SkColorSpacePrimaries* primaries, SkPaint* gamutPaint) {
  ...
  dc.fDrawList->AddImage(
      gamutPaint, dc.fPos, ImVec2(dc.fPos.x + dc.fSize.x, dc.fPos.y + dc.fSize.y), ImVec2(242, 61),
      ImVec2(1897, 1922));
...
void Viewer::drawImGui() {
  ...
  if (fShowImGuiDebugWindow) {
    ...
    if (ImGui::Begin("Tools", &fShowImGuiDebugWindow, ImGuiWindowFlags_AlwaysVerticalScrollbar)) {
...
#  if defined(SK_GRAPHITE_ENABLED)
        ImGui::SameLine();
        ImGui::RadioButton(
            "Metal (Graphite)", &newBackend, sk_app::Window::kGraphiteMetal_BackendType);
#  endif
#endif
      ...
      if (ImGui::CollapsingHeader("Paint")) {
        ...
        if (ImGui::Combo(
          ...
          if (aliasIdx == 0) {
            ...
            switch (fPaintOverrides.fAntiAliasState) {
              case SkPaintFields::AntiAliasState::Alias: break;
              case SkPaintFields::AntiAliasState::Normal: break;
              case SkPaintFields::AntiAliasState::AnalyticAAEnabled:
                gSkUseAnalyticAA = true;
                gSkForceAnalyticAA = false;
                break;
        ...
        auto paintFlag = [this, &uiParamsChanged](
                             const char* label, const char* items, bool SkPaintFields::*flag,
                             bool (SkPaint::*isFlag)() const, void (SkPaint::*setFlag)(bool)) {
          int itemIndex = 0;
          if (fPaintOverrides.*flag) {
        ...
        paintFlag(
            "Dither", "Default\0No Dither\0Dither\0\0", &SkPaintFields::fDither, &SkPaint::isDither,
            &SkPaint::setDither);
        ...
        if (ImGui::Combo("Style", &styleIdx, "Default\0Fill\0Stroke\0Stroke and Fill\0\0")) {
          if (styleIdx == 0) {
        ...
        if (ImGui::Combo("Cap Type", &capIdx, "Default\0Butt\0Round\0Square\0\0")) {
          if (capIdx == 0) {
        ...
        if (ImGui::Combo("Join Type", &joinIdx, "Default\0Miter\0Round\0Bevel\0\0")) {
          if (joinIdx == 0) {
      ...
      if (ImGui::CollapsingHeader("Font")) {
        ...
        if (ImGui::Combo("Hinting", &hintingIdx, "Default\0None\0Slight\0Normal\0Full\0\0")) {
          if (hintingIdx == 0) {
            fFontOverrides.fHinting = false;
            fFont.setHinting(SkFontHinting::kNone);
          } else {
        ...
        auto fontFlag = [this, &uiParamsChanged](
                            const char* label, const char* items, bool SkFontFields::*flag,
                            bool (SkFont::*isFlag)() const, void (SkFont::*setFlag)(bool)) {
          int itemIndex = 0;
          if (fFontOverrides.*flag) {
        ...
        fontFlag(
            "Fake Bold Glyphs", "Default\0No Fake Bold\0Fake Bold\0\0", &SkFontFields::fEmbolden,
            &SkFont::isEmbolden, &SkFont::setEmbolden);

        fontFlag(
            "Baseline Snapping", "Default\0No Baseline Snapping\0Baseline Snapping\0\0",
            &SkFontFields::fBaselineSnap, &SkFont::isBaselineSnap, &SkFont::setBaselineSnap);

        fontFlag(
            "Linear Text", "Default\0No Linear Text\0Linear Text\0\0",
            &SkFontFields::fLinearMetrics, &SkFont::isLinearMetrics, &SkFont::setLinearMetrics);

        fontFlag(
            "Subpixel Position Glyphs", "Default\0Pixel Text\0Subpixel Text\0\0",
            &SkFontFields::fSubpixel, &SkFont::isSubpixel, &SkFont::setSubpixel);

        fontFlag(
            "Embedded Bitmap Text", "Default\0No Embedded Bitmaps\0Embedded Bitmaps\0\0",
            &SkFontFields::fEmbeddedBitmaps, &SkFont::isEmbeddedBitmaps,
            &SkFont::setEmbeddedBitmaps);

        fontFlag(
            "Force Auto-Hinting", "Default\0No Force Auto-Hinting\0Force Auto-Hinting\0\0",
            &SkFontFields::fForceAutoHinting, &SkFont::isForceAutoHinting,
        ...
        if (ImGui::Combo(
                "Edging", &edgingIdx, "Default\0Alias\0Antialias\0Subpixel Antialias\0\0")) {
          if (edgingIdx == 0) {
            fFontOverrides.fEdging = false;
            fFont.setEdging(SkFont::Edging::kAlias);
          } else {
        ...
        if (fFontOverrides.fSize) {
          ImGui::DragFloat2(
              "TextRange", fFontOverrides.fSizeRange, 0.001f, -10.0f, 300.0f, "%.6f",
              ImGuiSliderFlags_Logarithmic);
          float textSize = fFont.getSize();
          if (ImGui::DragFloat(
                  "TextSize", &textSize, 0.001f, fFontOverrides.fSizeRange[0],
                  fFontOverrides.fSizeRange[1], "%.6f", ImGuiSliderFlags_Logarithmic)) {
      ...
      if (ImGui::CollapsingHeader("Slide")) {
        ...
        if (ImGui::ListBox(
                "", &filteredIndex, filteredSlideNames.begin(), filteredSlideNames.size(), 20)) {
      ...
      if (ImGui::CollapsingHeader("Color Mode")) {
        ...
        if (ImGui::Combo("Primaries", &primariesIdx, "sRGB\0AdobeRGB\0P3\0Rec. 2020\0Custom\0\0")) {
        ...
        if (ImGui::Button("Spin")) {
          float rx = fColorSpacePrimaries.fRX, ry = fColorSpacePrimaries.fRY;
      ...
      if (ImGui::CollapsingHeader("Shaders")) {
        ...
        if (gLoadPending) {
          auto collectShaders = [this](
                                    sk_sp<const SkData> key, sk_sp<SkData> data,
          ...
          if (isVulkan && !sksl) {
            ...
            for (auto& entry : fCachedShaders) {
              for (int i = 0; i < kGrShaderTypeCount; ++i) {
                ...
                std::string disasm;
                tools.Disassemble((const uint32_t*)spirv.c_str(), spirv.size() / 4, &disasm);
        ...
        bool doView = ImGui::Button("View");
        ImGui::SameLine();
        bool doApply = ImGui::Button("Apply Changes");
        ImGui::SameLine();
        ...
        if (doView || doDump || newOptLevel != fOptLevel) {
          ...
          fDeferredActions.push_back([=]() {
            ...
            if (doDump) {
              fDeferredActions.push_back([this]() { this->dumpShadersToResources(); });
            }
          });
        }
        ...
        if (doApply) {
          ...
          for (auto& entry : fCachedShaders) {
            ...
            if (entry.fHovered) {
              ...
              switch (entry.fShaderType) {
                ...
                case SkSetFourByteTag('G', 'L', 'S', 'L'): {
                  fragShader = build_glsl_highlight_shader(*ctx->priv().caps()->shaderCaps());
                  break;
                }
            ...
            auto data = GrPersistentCacheUtils::PackCachedShaders(
                entry.fShaderType, entry.fShader, entry.fInputs, kGrShaderTypeCount);
      ...
      if (ImGui::CollapsingHeader("SkVM")) {
        ...
        if (!fHoveredProgram.empty()) {
          auto restoreHoveredProgram = [this](const SkVMBlitter::Key* key, skvm::Program* program) {
            if (*key == fHoveredKey) {
          ...
          if (inTreeNode) {
            ...
            if (!asmString) {
              ...
              asmString = fDisassemblyCache.set(
                  *key, std::string((const char*)asmData->data(), asmData->size()));
            }
  ...
  if (fShowZoomWindow && fLastImage) {
    ...
    if (ImGui::Begin("Zoom", &fShowZoomWindow)) {
      ...
      if (fLastImage->readPixels(dContext, info, &pixel, info.minRowBytes(), xInt, yInt)) {
        ImGui::SameLine();
        ImGui::Text(
            "(X, Y): %d, %d RGBA: %X %X %X %X", xInt, yInt, SkGetPackedR32(pixel),
  ...
  if (fShowHistogramWindow && fLastImage) {
    ...
    if (ImGui::Begin("Color Histogram (R,G,B)", &fShowHistogramWindow)) {
      ...
      if (fLastImage->readPixels(
              fWindow->directContext(), info, pixmap.writable_addr(), info.minRowBytes(), 0, 0)) {
        ...
        ImGui::PlotHistogram("R", r.data(), r.size(), 0, nullptr, FLT_MAX, FLT_MAX, ImVec2(0, 150));
        ImGui::PlotHistogram("G", g.data(), g.size(), 0, nullptr, FLT_MAX, FLT_MAX, ImVec2(0, 150));
        ImGui::PlotHistogram("B", b.data(), b.size(), 0, nullptr, FLT_MAX, FLT_MAX, ImVec2(0, 150));
        ImGui::PopItemWidth();
      }
    }
...
void Viewer::dumpShadersToResources() {
  ...
  SkString directory = SkStringPrintf(
      "%ssksl/%s", GetResourcePath().c_str(), fSlides[fCurrentSlide]->getName().c_str());
...
void Viewer::onIdle() {
  ...
  if (animateWantsInval || fStatsLayer.getActive() || fRefresh || io.MetricsActiveWindows > 1 ||
...
static void WriteStateObject(
    SkJSONWriter& writer, const char* name, const char* value, OptionsFunc&& optionsFunc) {
  writer.beginObject();
  {
...
void Viewer::updateUIState() {
  ...
  // Slide state
  WriteStateObject(
      writer, kSlideStateName, fSlides[fCurrentSlide]->getName().c_str(),
  ...
  // Backend state
  WriteStateObject(
      writer, kBackendStateName, kBackendTypeStrings[fBackendType], [](SkJSONWriter& writer) {
  ...
  WriteStateObject(writer, kMSAAStateName, countString.c_str(), [this](SkJSONWriter& writer) {
    writer.appendS32(0);
  ...
  GpuPathRenderers pr = fWindow->getRequestedDisplayParams().fGrContextOptions.fGpuPathRenderers;
  WriteStateObject(
      writer, kPathRendererStateName, gPathRendererNames[pr].c_str(), [this](SkJSONWriter& writer) {
        auto ctx = fWindow->directContext();
        if (!ctx) {
          ...
          if (fWindow->sampleCount() > 1 || FLAGS_dmsaa) {
            const auto* caps = ctx->priv().caps();
            if (skgpu::v1::AtlasPathRenderer::IsSupported(ctx)) {
              writer.appendString(gPathRendererNames[GpuPathRenderers::kAtlas].c_str());
            }
            if (skgpu::v1::TessellationPathRenderer::IsSupported(*caps)) {
  ...
  // Softkey state
  WriteStateObject(writer, kSoftkeyStateName, kSoftkeyHint, [this](SkJSONWriter& writer) {
