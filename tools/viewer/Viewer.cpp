...
static DEFINE_int(
    internalSamples, 4, "Number of samples for internal draws that use MSAA or mixed samples.");
          ...
          it.positioning() == SkTextBlobRunIterator::kDefault_Positioning
              ...
              : it.positioning() == SkTextBlobRunIterator::kHorizontal_Positioning
                    ? SkTextBlobBuilderPriv::AllocRunTextPosH(
                          &builder, font, it.glyphCount(), it.offset().y(), it.textSize(),
                          SkString())
...
void Viewer::drawSlide(SkSurface* surface) {
  ...
  if (kPerspective_Fake == fPerspectiveMode || fShowZoomWindow ||
      Window::kRaster_BackendType == fBackendType || colorSpace != nullptr) {
    ...
    for (int y = 0; y < fWindow->height(); y += tileH) {
      for (int x = 0; x < fWindow->width(); x += tileW) {
        ...
        OveridePaintFilterCanvas filterCanvas(
            tileCanvas, &fPaint, &fPaintOverrides, &fFont, &fFontOverrides);
...
void Viewer::drawImGui() {
  ...
  if (fShowImGuiDebugWindow) {
    ...
    if (ImGui::Begin("Tools", &fShowImGuiDebugWindow, ImGuiWindowFlags_AlwaysVerticalScrollbar)) {
      if (ImGui::CollapsingHeader("Backend")) {
        ...
        if (ImGui::TreeNode("Path Renderers")) {
          ...
          if (!ctx) {
            ...
            prButton(GpuPathRenderers::kAll);
            if (GrCoverageCountingPathRenderer::IsSupported(*ctx->priv().caps())) {
      ...
      if (!backendIsGL && params.fGrContextOptions.fShaderCacheStrategy !=
