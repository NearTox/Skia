...
class SlideAdapter final : public sksg::RenderNode {
 public:
  explicit SlideAdapter(sk_sp<Slide> slide) : fSlide(std::move(slide)) { SkASSERT(fSlide); }
  ...
  std::unique_ptr<sksg::Animator> makeForwardingAnimator() {
    ...
    class ForwardingAnimator final : public sksg::Animator {
     public:
      explicit ForwardingAnimator(sk_sp<SlideAdapter> adapter) : fAdapter(std::move(adapter)) {}

     protected:
      void onTick(float t) override { fAdapter->tick(SkScalarRoundToInt(t)); }
...
class SlideDir::FocusController final : public sksg::Animator {
 ...
 protected:
  void onTick(float t) {
    ...
    switch (fState) {
      case State::kFocusing: fState = State::kFocused; break;
      ...
      case State::kIdle:
      case State::kFocused: SkASSERT(false); break;
    }
  }
 ...
 private:
  ...
  SkMatrix fM0 = SkMatrix::I(), fM1 = SkMatrix::I();
  float fOpacity0 = 0, fOpacity1 = 1, fTimeBase = 0;
...
SlideDir::SlideDir(const SkString& name, SkTArray<sk_sp<Slide>>&& slides, int columns)
    : fSlides(std::move(slides)), fColumns(columns) {
  fName = name;
}

static sk_sp<sksg::RenderNode> MakeLabel(
    const SkString& txt, const SkPoint& pos, const SkMatrix& dstXform) {
...
void SlideDir::load(SkScalar winWidth, SkScalar winHeight) {
  ...
  for (int i = 0; i < fSlides.count(); ++i) {
    ...
    const auto slideSize = slide->getDimensions();
    const auto cell = SkRect::MakeXYWH(
                   fCellSize.width() * (i % fColumns), fCellSize.height() * (i / fColumns),
                   fCellSize.width(), fCellSize.height()),
    ...
    slideGrp->addChild(sksg::Draw::Make(
        sksg::Rect::Make(SkRect::MakeIWH(slideSize.width(), slideSize.height())),
        sksg::Color::Make(0xfff0f0f0)));
    slideGrp->addChild(adapter);
    slideGrp->addChild(MakeLabel(
        slide->getName(), SkPoint::Make(slideSize.width() / 2, slideSize.height()),
  ...
  const auto focusRect =
      SkRect::MakeSize(fWinSize).makeInset(kFocusInset.width(), kFocusInset.height());
...
SkISize SlideDir::getDimensions() const {
  return SkSize::Make(fWinSize.width(), fCellSize.height() * (1 + (fSlides.count() - 1) / fColumns))
      .toCeil();
}

void SlideDir::draw(SkCanvas* canvas) { fScene->render(canvas); }
...
bool SlideDir::onMouse(
    SkScalar x, SkScalar y, sk_app::Window::InputState state, uint32_t modifiers) {
  if (state == sk_app::Window::kMove_InputState || modifiers) return false;
...
const SlideDir::Rec* SlideDir::findCell(float x, float y) const {
  ...
  const int col = static_cast<int>(x / fCellSize.width()),
            row = static_cast<int>(y / fCellSize.height()), idx = row * fColumns + col;
