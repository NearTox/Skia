...
#define IF_OPEN(WIDGET)    \
  if (fTreeStack.back()) { \
    WIDGET;                \
  }

  void visit(const char* name, float& f) override { IF_OPEN(ImGui::DragFloat(item(name), &f)) }
  void visit(const char* name, int& i) override { IF_OPEN(ImGui::DragInt(item(name), &i)) }
  void visit(const char* name, bool& b) override { IF_OPEN(ImGui::Checkbox(item(name), &b)) }
    ...
    if (fTreeStack.back()) {
      ...
      if (lines > 1) {
        ...
        ImGui::InputTextMultiline(
            item(name), s.writable_str(), s.size() + 1, boxSize, flags, InputTextCallback, &s);
      } else {
        ImGui::InputText(item(name), s.writable_str(), s.size() + 1, flags, InputTextCallback, &s);
      }
    }
  }
...
void ParticlesSlide::draw(SkCanvas* canvas) {
  ...
  if (ImGui::Begin("Running")) {
    for (int i = 0; i < fRunning.count(); ++i) {
      ...
      ImGui::SameLine();
      ImGui::Text(
          "%4g, %4g %5d %s", fRunning[i].fPosition.fX, fRunning[i].fPosition.fY, effect->getCount(),
          fRunning[i].fName.c_str());
      ...
      auto uniformsGui = [](const SkSL::ByteCode* code, float* data, SkPoint spawnPos) {
        ...
        for (int i = 0; i < code->getUniformCount(); ++i) {
          ...
          if (uni.fName == "mouse_pos" && uni.fType == SkSL::TypeCategory::kFloat &&
