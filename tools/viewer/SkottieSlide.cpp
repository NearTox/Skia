...
static void draw_stats_box(SkCanvas* canvas, const skottie::Animation::Builder::Stats& stats) {
  ...
  const auto json_size = SkStringPrintf("Json size: %lu bytes", stats.fJsonSize);
  canvas->drawString(json_size, kR.x() + 10, kR.y() + kTextSize * 1, font, paint);
  const auto animator_count = SkStringPrintf("Animator count: %lu", stats.fAnimatorCount);
...
void SkottieSlide::load(SkScalar w, SkScalar h) {
  ...
  fAnimation = builder.setLogger(logger)
                   .setResourceProvider(skresources::DataURIResourceProviderProxy::Make(
