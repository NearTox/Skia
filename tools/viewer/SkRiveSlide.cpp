...
SkRiveSlide::SkRiveSlide(const SkString& name, const SkString& path) : fPath(path) { fName = name; }
...
void SkRiveSlide::unload() { fRive.reset(); }

void SkRiveSlide::resize(SkScalar w, SkScalar h) { fWinSize = {w, h}; }
...
void SkRiveSlide::draw(SkCanvas* canvas) {
  ...
  canvas->concat(
      SkMatrix::RectToRect(fRiveBounds, SkRect::MakeSize(fWinSize), SkMatrix::kCenter_ScaleToFit));
