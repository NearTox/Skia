...
void DDLPromiseImageHelper::createCallbackContexts(GrContext* context) {
  ...
  for (int i = 0; i < fImageInfo.count(); ++i) {
    ...
    if (info.isYUV()) {
      ...
      GrBackendFormat backendFormat =
          context->defaultBackendFormat(bm.pixmap().colorType(), GrRenderable::kNo);
...
sk_sp<SkImage> DDLPromiseImageHelper::CreatePromiseImages(
  ...
  if (curImage.isYUV()) {
    ...
    image = recorder->makeYUVAPromiseTexture(
        ...
        DDLPromiseImageHelper::PromiseImageReleaseProc, DDLPromiseImageHelper::PromiseImageDoneProc,
        contexts, SkDeferredDisplayListRecorder::PromiseImageApiVersion::kNew);
    for (int i = 0; i < textureCount; ++i) {
    ...
    image = recorder->makePromiseTexture(
        backendFormat, curImage.overallWidth(), curImage.overallHeight(), GrMipMapped::kNo,
