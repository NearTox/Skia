...
DDLPromiseImageHelper::PromiseImageInfo::PromiseImageInfo(
    int index, uint32_t originalUniqueID, const SkImageInfo& ii)
    : fIndex(index), fOriginalUniqueID(originalUniqueID), fImageInfo(ii) {}
...
void DDLPromiseImageHelper::PromiseImageInfo::setMipLevels(
...
static GrBackendTexture create_yuva_texture(
    GrDirectContext* direct, const SkPixmap& pm, const SkYUVAIndex yuvaIndices[4], int texIndex) {
 ...
 */
void DDLPromiseImageHelper::CreateBETexturesForPromiseImage(
    GrDirectContext* direct, PromiseImageInfo* info) {
  if (info->isYUV()) {
    int numPixmaps;
    ...
    for (int j = 0; j < numPixmaps; ++j) {
      ...
      callbackContext->setBackendTexture(
          create_yuva_texture(direct, yuvPixmap, info->yuvaIndices(), j));
    ...
    bool finishedBECreate = false;
    auto markFinished = [](void* context) { *(bool*)context = true; };
    auto backendTex = direct->createBackendTexture(
...
void DDLPromiseImageHelper::DeleteBETexturesForPromiseImage(
    GrDirectContext* direct, PromiseImageInfo* info) {
  if (info->isYUV()) {
    int numPixmaps;
    SkAssertResult(SkYUVAIndex::AreValidIndices(info->yuvaIndices(), &numPixmaps));
    ...
    if (info.isYUV()) {
      ...
      for (int j = 0; j < numPixmaps; ++j) {
        ...
        GrBackendFormat backendFormat =
            direct->defaultBackendFormat(yuvPixmap.colorType(), GrRenderable::kNo);
      ...
      GrBackendFormat backendFormat =
          direct->defaultBackendFormat(baseLevel.colorType(), GrRenderable::kNo);
...
sk_sp<SkPicture> DDLPromiseImageHelper::reinflateSKP(
    SkDeferredDisplayListRecorder* recorder, SkData* compressedPictureData,
...
sk_sp<SkImage> DDLPromiseImageHelper::CreatePromiseImages(
    const void* rawData, size_t length, void* ctxIn) {
  ...
  if (curImage.isYUV()) {
    ...
    image = recorder->makeYUVAPromiseTexture(
        curImage.yuvColorSpace(), backendFormats, sizes, curImage.yuvaIndices(),
        curImage.overallWidth(), curImage.overallHeight(),
        GrSurfaceOrigin::kTopLeft_GrSurfaceOrigin, curImage.refOverallColorSpace(),
        PromiseImageCallbackContext::PromiseImageFulfillProc,
        PromiseImageCallbackContext::PromiseImageReleaseProc,
        PromiseImageCallbackContext::PromiseImageDoneProc, contexts,
    ...
    image = recorder->makePromiseTexture(
        backendFormat, curImage.overallWidth(), curImage.overallHeight(), curImage.mipMapped(0),
        GrSurfaceOrigin::kTopLeft_GrSurfaceOrigin, curImage.overallColorType(),
        curImage.overallAlphaType(), curImage.refOverallColorSpace(),
        PromiseImageCallbackContext::PromiseImageFulfillProc,
        PromiseImageCallbackContext::PromiseImageReleaseProc,
...
int DDLPromiseImageHelper::addImage(SkImage* image) {
  ...
  SkImageInfo overallII = SkImageInfo::Make(
      image->width(), image->height(),
      image->colorType() == kBGRA_8888_SkColorType ? kRGBA_8888_SkColorType : image->colorType(),
      image->alphaType(), image->refColorSpace());

  PromiseImageInfo& newImageInfo =
      fImageInfo.emplace_back(fImageInfo.count(), image->uniqueID(), overallII);
