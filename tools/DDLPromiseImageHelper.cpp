...
static GrBackendTexture create_yuva_texture(
    GrDirectContext* direct, const SkPixmap& pm, int texIndex) {
  ...
  auto beTex = direct->createBackendTexture(
      pm, kTopLeft_GrSurfaceOrigin, GrRenderable::kNo, GrProtected::kNo, markFinished,
      &finishedBECreate);
  if (beTex.isValid()) {
    direct->submit();
...
void DDLPromiseImageHelper::CreateBETexturesForPromiseImage(
  ...
  if (info->isYUV()) {
    ...
    auto backendTex = direct->createBackendTexture(
        mipLevels.get(), info->numMipLevels(), kTopLeft_GrSurfaceOrigin, GrRenderable::kNo,
...
sk_sp<SkImage> DDLPromiseImageHelper::CreatePromiseImages(
  ...
  if (curImage.isYUV()) {
    ...
    GrYUVABackendTextureInfo yuvaBackendTextures(
        yuvaInfo, backendFormats, GrMipmapped::kNo, kTopLeft_GrSurfaceOrigin);

    image = recorder->makeYUVAPromiseTexture(
        yuvaBackendTextures, curImage.refOverallColorSpace(),
        PromiseImageCallbackContext::PromiseImageFulfillProc,
        PromiseImageCallbackContext::PromiseImageReleaseProc, contexts);
    if (!image) {
      return nullptr;
    }
    for (int i = 0; i < textureCount; ++i) {
