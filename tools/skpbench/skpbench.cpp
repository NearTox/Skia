...
public:
    ...
    int drawAndFlushAndSync(
        GrDirectContext* context, SkSurface* surface, GpuSync& gpuSync) override {
...
static void run_benchmark(
    GrDirectContext* context, SkSurface* surface, SkpProducer* skpp, std::vector<Sample>* samples) {
  using clock = std::chrono::high_resolution_clock;
  const Sample::duration sampleDuration = std::chrono::milliseconds(FLAGS_sampleMs);
  const clock::duration benchDuration = std::chrono::milliseconds(FLAGS_duration);
...
static void run_gpu_time_benchmark(
    sk_gpu_test::GpuTimer* gpuTimer, GrDirectContext* context, SkSurface* surface,
...
static void draw_skp_and_flush_with_sync(
    GrDirectContext* context, SkSurface* surface, const SkPicture* skp, GpuSync& gpuSync) {
