...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < frameCount; i++) {
    ...
    if (!stillImage) {
      ...
      if ((cachedFrame != SkCodec::kNoFrame) && (cachedFrame == frameInfos[i].fRequiredFrame) &&
          cachedFramePixels.get()) {
    ...
    if (!firstFrameOnly) {
      if (i == 0) {
        write_nix_header(
            0x41AFC36E,  // "nïA" magic string as a u32le.
            bm.width(), bm.height());
      }
    ...
    write_nix_header(
        0x45AFC36E,  // "nïE" magic string as a u32le.
        width, height);
