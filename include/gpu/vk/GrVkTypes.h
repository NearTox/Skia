...
struct GrVkYcbcrConversionInfo {
  ...
  bool operator==(const GrVkYcbcrConversionInfo& that) const noexcept {
    ...
    return this->fFormat == that.fFormat && this->fExternalFormat == that.fExternalFormat &&
           this->fYcbcrModel == that.fYcbcrModel && this->fYcbcrRange == that.fYcbcrRange &&
...
struct GrVkImageInfo {
  ...
  GrVkImageInfo(const GrVkImageInfo& info, VkImageLayout layout, uint32_t familyQueueIndex) noexcept
      : fImage(info.fImage),
        fAlloc(info.fAlloc),
