...
class GrVkMemoryAllocator : public SkRefCnt {
 public:
  ...
  virtual bool allocateMemoryForImage(
      VkImage image, AllocationPropertyFlags flags, GrVkBackendMemory*) = 0;

  virtual bool allocateMemoryForBuffer(
      VkBuffer buffer, BufferUsage usage, AllocationPropertyFlags flags, GrVkBackendMemory*) = 0;
  ...
  virtual void flushMappedMemory(
      const GrVkBackendMemory&, VkDeviceSize offset, VkDeviceSize size) = 0;
