...
#  ifdef SK_VULKAN
  ...
  static GrBackendFormat MakeVk(
      const GrVkYcbcrConversionInfo& ycbcrInfo, bool willUseDRMFormatModifiers = false);
#  endif
...
#  ifdef SK_VULKAN
  GrBackendFormat(
      const VkFormat vkFormat, const GrVkYcbcrConversionInfo&, bool willUseDRMFormatModifiers);
#  endif
...
#  endif
  ...
  GrMipmapped mipmapped() const noexcept { return fMipmapped; }
...
#  ifdef SK_GL
  ...
  GrBackendTexture(
      int width, int height, GrMipmapped, const GrGLTextureInfo, sk_sp<GrGLTextureParameters>);
...
#  ifdef SK_METAL
  GrBackendRenderTarget(int width, int height, const GrMtlTextureInfo& mtlInfo);
...
#  ifdef SK_DIRECT3D
  GrBackendRenderTarget(int width, int height, const GrD3DTextureResourceInfo& d3dInfo);
#  endif
