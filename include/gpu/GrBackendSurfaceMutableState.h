...
class GrBackendSurfaceMutableState {
 public:
#ifdef SK_VULKAN
  GrBackendSurfaceMutableState(VkImageLayout layout, uint32_t queueFamilyIndex) noexcept
      : fVkState(layout, queueFamilyIndex), fBackend(GrBackend::kVulkan) {}
#endif

  GrBackendSurfaceMutableState& operator=(const GrBackendSurfaceMutableState& that) noexcept {
    switch (fBackend) {
      ...
      default: (void)that; SkUNREACHABLE;
    }
