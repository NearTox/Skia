 ...
 *   (src, dst, 0xFF) returns src
 */
static constexpr inline SkPMColor SkFourByteInterp(
...
static inline Sk4f swizzle_rb(const Sk4f& x) noexcept { return SkNx_shuffle<2, 1, 0, 3>(x); }

static inline Sk4f swizzle_rb_if_bgra(const Sk4f& x) noexcept {
#ifdef SK_PMCOLOR_IS_BGRA
  return swizzle_rb(x);
#else
  return x;
