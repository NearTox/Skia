...
static constexpr unsigned SkR16ToR32(unsigned r) noexcept {
  return (r << (8 - SK_R16_BITS)) | (r >> (2 * SK_R16_BITS - 8));
}

static constexpr unsigned SkG16ToG32(unsigned g) noexcept {
  return (g << (8 - SK_G16_BITS)) | (g >> (2 * SK_G16_BITS - 8));
}

static constexpr unsigned SkB16ToB32(unsigned b) noexcept {
...
static constexpr uint32_t SkPackARGB_as_RGBA(U8CPU a, U8CPU r, U8CPU g, U8CPU b) noexcept {
  SkASSERT_IS_BYTE(a);
  SkASSERT_IS_BYTE(r);
  SkASSERT_IS_BYTE(g);
  SkASSERT_IS_BYTE(b);
...
static constexpr uint32_t SkPackARGB_as_BGRA(U8CPU a, U8CPU r, U8CPU g, U8CPU b) noexcept {
  SkASSERT_IS_BYTE(a);
  SkASSERT_IS_BYTE(r);
  SkASSERT_IS_BYTE(g);
  SkASSERT_IS_BYTE(b);
...
*/
static constexpr U8CPU SkComputeLuminance(U8CPU r, U8CPU g, U8CPU b) noexcept {
  // The following is
 ...
 */
static constexpr U16CPU SkAlphaMulInv256(U16CPU value, U16CPU alpha256) noexcept {
...
static constexpr int SkAlphaBlend(int src, int dst, int scale256) noexcept {
  SkASSERT((unsigned)scale256 <= 256);
  return dst + SkAlphaMul(src - dst, scale256);
}

static constexpr uint16_t SkPackRGB16(unsigned r, unsigned g, unsigned b) {
 ...
 */
static constexpr SkPMColor SkFourByteInterp256(SkPMColor src, SkPMColor dst, unsigned scale) {
 ...
 * (note, ARGB -> AGRB)
 */
static constexpr uint64_t SkSplay(uint32_t color) noexcept {
  constexpr uint32_t mask = 0x00FF00FF;
 ...
 */
static constexpr uint32_t SkUnsplay(uint32_t ag, uint32_t rb) noexcept {
 ...
 * (note, AGRB -> ARGB)
 */
static constexpr uint32_t SkUnsplay(uint64_t agrb) noexcept {
  constexpr uint32_t mask = 0xFF00FF00;
  return SkPMColor(
...
static constexpr U16CPU SkPixel32ToPixel16(SkPMColor c) noexcept {
...
static constexpr U16CPU SkPack888ToRGB16(U8CPU r, U8CPU g, U8CPU b) noexcept {
...
static constexpr SkColor SkPixel16ToColor(U16CPU src) noexcept {
...
static constexpr U8CPU SkReplicateNibble(unsigned nib) noexcept {
  SkASSERT(nib <= 0xF);
  return (nib << 4) | nib;
}
...
static constexpr SkPMColor SkPixel4444ToPixel32(U16CPU c) noexcept {
