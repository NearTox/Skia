...
static constexpr inline unsigned SkR16ToR32(unsigned r) noexcept {
    return (r << (8 - SK_R16_BITS)) | (r >> (2 * SK_R16_BITS - 8));
}

static constexpr inline unsigned SkG16ToG32(unsigned g) noexcept {
    return (g << (8 - SK_G16_BITS)) | (g >> (2 * SK_G16_BITS - 8));
}

static constexpr inline unsigned SkB16ToB32(unsigned b) noexcept {
...
static constexpr inline uint32_t SkSwizzle_RB(uint32_t c) noexcept {
    constexpr uint32_t kRBMask = (0xFF << SK_R32_SHIFT) | (0xFF << SK_B32_SHIFT);
...
static constexpr inline uint32_t SkPackARGB_as_RGBA(U8CPU a, U8CPU r, U8CPU g, U8CPU b) noexcept {
    SkASSERT_IS_BYTE(a);
    SkASSERT_IS_BYTE(r);
    SkASSERT_IS_BYTE(g);
    SkASSERT_IS_BYTE(b);
    return (a << SK_RGBA_A32_SHIFT) | (r << SK_RGBA_R32_SHIFT) | (g << SK_RGBA_G32_SHIFT) |
           (b << SK_RGBA_B32_SHIFT);
}

static constexpr inline uint32_t SkPackARGB_as_BGRA(U8CPU a, U8CPU r, U8CPU g, U8CPU b) noexcept {
    SkASSERT_IS_BYTE(a);
    SkASSERT_IS_BYTE(r);
    SkASSERT_IS_BYTE(g);
    SkASSERT_IS_BYTE(b);
    return (a << SK_BGRA_A32_SHIFT) | (r << SK_BGRA_R32_SHIFT) | (g << SK_BGRA_G32_SHIFT) |
           (b << SK_BGRA_B32_SHIFT);
}

static constexpr inline SkPMColor SkSwizzle_RGBA_to_PMColor(uint32_t c) {
...
static constexpr inline int SkAlphaBlend(int src, int dst, int scale256) noexcept {
    SkASSERT((unsigned)scale256 <= 256);
    return dst + SkAlphaMul(src - dst, scale256);
}

static constexpr inline uint16_t SkPackRGB16(unsigned r, unsigned g, unsigned b) noexcept {
 ...
 */
static constexpr inline SkPMColor SkFourByteInterp256(SkPMColor src,
                                                      SkPMColor dst,
 ...
 *   (src, dst, 0xFF) returns src
 */
static constexpr inline SkPMColor SkFourByteInterp(SkPMColor src,
                                                   SkPMColor dst,
                                                   U8CPU srcWeight) noexcept {
 ...
 * (note, ARGB -> AGRB)
 */
static constexpr inline uint64_t SkSplay(uint32_t color) noexcept {
    const uint32_t mask = 0x00FF00FF;
 ...
 */
static constexpr inline uint32_t SkUnsplay(uint32_t ag, uint32_t rb) noexcept {
 ...
 * (note, AGRB -> ARGB)
 */
static constexpr inline uint32_t SkUnsplay(uint64_t agrb) noexcept {
    const uint32_t mask = 0xFF00FF00;
    return SkPMColor(((agrb & mask) >> 8) |   // 0x00RR00BB
                     ((agrb >> 32) & mask));  // 0xAARRGGBB
}

static constexpr inline SkPMColor SkFastFourByteInterp256_32(SkPMColor src,
                                                             SkPMColor dst,
                                                             unsigned scale) noexcept {
    SkASSERT(scale <= 256);

    // Two 8-bit blends per two 32-bit registers, with space to make sure the math doesn't collide.
    uint32_t src_ag = 0, src_rb = 0, dst_ag = 0, dst_rb = 0;
...
static constexpr inline SkPMColor SkFastFourByteInterp256_64(SkPMColor src,
                                                             SkPMColor dst,
                                                             unsigned scale) noexcept {
    SkASSERT(scale <= 256);
...
static constexpr inline SkPMColor SkBlendARGB32(SkPMColor src, SkPMColor dst, U8CPU aa) noexcept {
...
static constexpr inline U16CPU SkPixel32ToPixel16(SkPMColor c) noexcept {
...
static constexpr inline U16CPU SkPack888ToRGB16(U8CPU r, U8CPU g, U8CPU b) noexcept {
...
static constexpr inline SkColor SkPixel16ToColor(U16CPU src) noexcept {
...
static constexpr inline U8CPU SkReplicateNibble(unsigned nib) noexcept {
    SkASSERT(nib <= 0xF);
    return (nib << 4) | nib;
}
...
static constexpr inline SkPMColor SkPixel4444ToPixel32(U16CPU c) noexcept {
...
static inline Sk4f Sk4f_fromL32(uint32_t px) noexcept {
    return SkNx_cast<float>(Sk4b::Load(&px)) * (1 / 255.0f);
}

static inline uint32_t Sk4f_toL32(const Sk4f& px) noexcept {
    Sk4f v = px;
...
constexpr SkPMColor4f SK_PMColor4fILLEGAL = {SK_FloatNegativeInfinity, SK_FloatNegativeInfinity,
