...
static constexpr inline unsigned SkR16ToR32(unsigned r) {
  return (r << (8 - SK_R16_BITS)) | (r >> (2 * SK_R16_BITS - 8));
}

static constexpr inline unsigned SkG16ToG32(unsigned g) {
  return (g << (8 - SK_G16_BITS)) | (g >> (2 * SK_G16_BITS - 8));
}

static constexpr inline unsigned SkB16ToB32(unsigned b) {
...
static constexpr inline uint32_t SkPackARGB_as_RGBA(U8CPU a, U8CPU r, U8CPU g, U8CPU b) {
  SkASSERT_IS_BYTE(a);
  SkASSERT_IS_BYTE(r);
  SkASSERT_IS_BYTE(g);
  SkASSERT_IS_BYTE(b);
...
static constexpr inline uint32_t SkPackARGB_as_BGRA(U8CPU a, U8CPU r, U8CPU g, U8CPU b) {
  SkASSERT_IS_BYTE(a);
  SkASSERT_IS_BYTE(r);
  SkASSERT_IS_BYTE(g);
  SkASSERT_IS_BYTE(b);
...
static constexpr inline SkPMColor SkSwizzle_BGRA_to_PMColor(uint32_t c) {
...
*/
static constexpr inline U8CPU SkComputeLuminance(U8CPU r, U8CPU g, U8CPU b) {
  // The following is
 ...
 */
static constexpr inline U16CPU SkAlphaMulInv256(U16CPU value, U16CPU alpha256) {
...
static constexpr inline int SkAlphaBlend(int src, int dst, int scale256) {
  SkASSERT((unsigned)scale256 <= 256);
  return dst + SkAlphaMul(src - dst, scale256);
}

static constexpr inline uint16_t SkPackRGB16(unsigned r, unsigned g, unsigned b) {
 ...
 */
static constexpr inline SkPMColor SkFourByteInterp256(
 ...
 *   (src, dst, 0xFF) returns src
 */
static constexpr inline SkPMColor SkFourByteInterp(SkPMColor src, SkPMColor dst, U8CPU srcWeight) {
 ...
 * (note, ARGB -> AGRB)
 */
static constexpr inline uint64_t SkSplay(uint32_t color) {
  const uint32_t mask = 0x00FF00FF;
 ...
 */
static constexpr inline uint32_t SkUnsplay(uint32_t ag, uint32_t rb) {
 ...
 * (note, AGRB -> ARGB)
 */
static constexpr inline uint32_t SkUnsplay(uint64_t agrb) {
  const uint32_t mask = 0xFF00FF00;
  return SkPMColor(
...
static constexpr inline SkPMColor SkBlendARGB32(SkPMColor src, SkPMColor dst, U8CPU aa) {
...
static constexpr inline U16CPU SkPixel32ToPixel16(SkPMColor c) {
...
static constexpr inline U16CPU SkPack888ToRGB16(U8CPU r, U8CPU g, U8CPU b) {
...
*/
static constexpr inline U16CPU SkSrcOver32To16(SkPMColor src, uint16_t dst) {
...
static constexpr inline SkColor SkPixel16ToColor(U16CPU src) {
...
static constexpr inline U8CPU SkReplicateNibble(unsigned nib) {
  SkASSERT(nib <= 0xF);
  return (nib << 4) | nib;
}
...
static constexpr inline SkPMColor SkPixel4444ToPixel32(U16CPU c) {
