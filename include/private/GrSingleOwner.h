...
class GrSingleOwner {
 public:
  ...
  struct AutoEnforce {
    AutoEnforce(GrSingleOwner* so, const char* file, int line) : fFile(file), fLine(line), fSO(so) {
      fSO->enter(file, line);
    }
 ...
 private:
  void enter(const char* file, int line) {
    ...
    SkASSERTF(
        fOwner == self || fOwner == kIllegalThreadID, "%s:%d Single owner failure.", file, line);
    fReentranceCount++;
    fOwner = self;
  }
