...
// Seems plenty big.
AI static __m128 emulate_mm_floor_ps(__m128 v) noexcept {
...
public:
    ...
    AI SkNx(float a, float b) noexcept : fVec(_mm_setr_ps(a, b, 0, 0)) {}

    AI void store(void* ptr) const noexcept { _mm_storel_pi((__m64*)ptr, fVec); }

    AI static void Load2(const void* ptr, SkNx* x, SkNx* y) noexcept {
        const float* m = (const float*)ptr;
        *x = SkNx{m[0], m[2]};
        *y = SkNx{m[1], m[3]};
    }

    AI static void Store2(void* dst, const SkNx& a, const SkNx& b) noexcept {
        auto vals = _mm_unpacklo_ps(a.fVec, b.fVec);
        _mm_storeu_ps((float*)dst, vals);
    }

    AI static void Store3(void* dst, const SkNx& a, const SkNx& b, const SkNx& c) noexcept {
        auto lo = _mm_setr_ps(a[0], b[0], c[0], a[1]), hi = _mm_setr_ps(b[1], c[1], 0, 0);
        _mm_storeu_ps((float*)dst, lo);
        _mm_storel_pi(((__m64*)dst) + 2, hi);
    }

    AI static void Store4(void* dst, const SkNx& a, const SkNx& b, const SkNx& c,
                          const SkNx& d) noexcept {
    ...
    AI SkNx abs() const noexcept { return _mm_andnot_ps(_mm_set1_ps(-0.0f), fVec); }
    AI SkNx floor() const noexcept {
#if SK_CPU_SSE_LEVEL >= SK_CPU_SSE_LEVEL_SSE41
    ...
    AI bool allTrue() const noexcept {
        return 0xff == (_mm_movemask_epi8(_mm_castps_si128(fVec)) & 0xff);
    }
    AI bool anyTrue() const noexcept {
...
public:
    ...
    AI void store(void* ptr) const noexcept { _mm_storeu_ps((float*)ptr, fVec); }
    ...
    AI static void Load4(const void* ptr, SkNx* r, SkNx* g, SkNx* b, SkNx* a) noexcept {
        __m128 v0 = _mm_loadu_ps(((float*)ptr) + 0), v1 = _mm_loadu_ps(((float*)ptr) + 4),
               v2 = _mm_loadu_ps(((float*)ptr) + 8), v3 = _mm_loadu_ps(((float*)ptr) + 12);
    ...
    AI static void Store4(void* dst, const SkNx& r, const SkNx& g, const SkNx& b,
                          const SkNx& a) noexcept {
...
AI static __m128i mullo32(__m128i a, __m128i b) noexcept {
#if SK_CPU_SSE_LEVEL >= SK_CPU_SSE_LEVEL_SSE41
...
public:
    ...
    AI SkNx(int32_t a, int32_t b, int32_t c, int32_t d) noexcept
            : fVec(_mm_setr_epi32(a, b, c, d)) {}

    AI void store(void* ptr) const noexcept { _mm_storeu_si128((__m128i*)ptr, fVec); }

    AI SkNx operator+(const SkNx& o) const noexcept { return _mm_add_epi32(fVec, o.fVec); }
    AI SkNx operator-(const SkNx& o) const noexcept { return _mm_sub_epi32(fVec, o.fVec); }
    AI SkNx operator*(const SkNx& o) const noexcept { return mullo32(fVec, o.fVec); }

    AI SkNx operator&(const SkNx& o) const noexcept { return _mm_and_si128(fVec, o.fVec); }
    AI SkNx operator|(const SkNx& o) const noexcept { return _mm_or_si128(fVec, o.fVec); }
    AI SkNx operator^(const SkNx& o) const noexcept { return _mm_xor_si128(fVec, o.fVec); }

    AI SkNx operator<<(int bits) const noexcept { return _mm_slli_epi32(fVec, bits); }
    AI SkNx operator>>(int bits) const noexcept { return _mm_srai_epi32(fVec, bits); }

    AI SkNx operator==(const SkNx& o) const noexcept { return _mm_cmpeq_epi32(fVec, o.fVec); }
    AI SkNx operator<(const SkNx& o) const noexcept { return _mm_cmplt_epi32(fVec, o.fVec); }
    AI SkNx operator>(const SkNx& o) const noexcept { return _mm_cmpgt_epi32(fVec, o.fVec); }

    AI int32_t operator[](int k) const noexcept {
        SkASSERT(0 <= k && k < 4);
        union {
            __m128i v;
            int32_t is[4];
...
template <> class SkNx<2, uint32_t> {
public:
    AI SkNx(const __m128i& vec) noexcept : fVec(vec) {}
    ...
    AI void store(void* ptr) const noexcept { _mm_storel_epi64((__m128i*)ptr, fVec); }
    ...
    AI SkNx operator<<(int bits) const noexcept { return _mm_slli_epi32(fVec, bits); }
    AI SkNx operator>>(int bits) const noexcept { return _mm_srli_epi32(fVec, bits); }

    AI SkNx operator==(const SkNx& o) const noexcept { return _mm_cmpeq_epi32(fVec, o.fVec); }
    AI SkNx operator!=(const SkNx& o) const noexcept { return (*this == o) ^ 0xffffffff; }
    // operator < and > take a little extra fiddling to make work for unsigned ints.

    AI uint32_t operator[](int k) const noexcept {
        SkASSERT(0 <= k && k < 2);
        union {
            __m128i v;
            uint32_t us[4];
    ...
    AI bool allTrue() const noexcept { return 0xff == (_mm_movemask_epi8(fVec) & 0xff); }

    __m128i fVec;
};

template <> class SkNx<4, uint32_t> {
public:
    AI SkNx(const __m128i& vec) noexcept : fVec(vec) {}
    ...
    AI SkNx mulHi(SkNx m) const noexcept {
...
template <> class SkNx<4, uint16_t> {
public:
    AI SkNx(const __m128i& vec) noexcept : fVec(vec) {}
    ...
    AI static void Load3(const void* ptr, SkNx* r, SkNx* g, SkNx* b) noexcept {
    ...
    AI SkNx operator+(const SkNx& o) const noexcept { return _mm_add_epi16(fVec, o.fVec); }
    AI SkNx operator-(const SkNx& o) const noexcept { return _mm_sub_epi16(fVec, o.fVec); }
    AI SkNx operator*(const SkNx& o) const noexcept { return _mm_mullo_epi16(fVec, o.fVec); }
    AI SkNx operator&(const SkNx& o) const noexcept { return _mm_and_si128(fVec, o.fVec); }
    AI SkNx operator|(const SkNx& o) const noexcept { return _mm_or_si128(fVec, o.fVec); }

    AI SkNx operator<<(int bits) const noexcept { return _mm_slli_epi16(fVec, bits); }
    AI SkNx operator>>(int bits) const noexcept { return _mm_srli_epi16(fVec, bits); }

    AI uint16_t operator[](int k) const noexcept {
        SkASSERT(0 <= k && k < 4);
        union {
            __m128i v;
            uint16_t us[8];
...
template <> class SkNx<8, uint16_t> {
public:
    AI SkNx(const __m128i& vec) noexcept : fVec(vec) {}

    AI SkNx() noexcept {}
    AI SkNx(uint16_t val) noexcept : fVec(_mm_set1_epi16(val)) {}
    AI SkNx(uint16_t a, uint16_t b, uint16_t c, uint16_t d, uint16_t e, uint16_t f, uint16_t g,
            uint16_t h) noexcept
    ...
    AI static void Store4(void* ptr, const SkNx& r, const SkNx& g, const SkNx& b,
                          const SkNx& a) noexcept {
    ...
    AI static SkNx Min(const SkNx& a, const SkNx& b) noexcept {
    ...
    AI SkNx mulHi(const SkNx& m) const noexcept { return _mm_mulhi_epu16(fVec, m.fVec); }
...
public:
    ...
    AI static SkNx Load(const void* ptr) noexcept { return _mm_cvtsi32_si128(*(const int*)ptr); }
    AI void store(void* ptr) const noexcept { *(int*)ptr = _mm_cvtsi128_si32(fVec); }

    AI uint8_t operator[](int k) const noexcept {
        SkASSERT(0 <= k && k < 4);
        union {
            __m128i v;
            uint8_t us[16];
...
public:
    ...
    AI static SkNx Load(const void* ptr) noexcept { return _mm_loadl_epi64((const __m128i*)ptr); }
    AI SkNx(uint8_t a, uint8_t b, uint8_t c, uint8_t d, uint8_t e, uint8_t f, uint8_t g,
            uint8_t h) noexcept
            : fVec(_mm_setr_epi8(a, b, c, d, e, f, g, h, 0, 0, 0, 0, 0, 0, 0, 0)) {}

    AI void store(void* ptr) const noexcept { _mm_storel_epi64((__m128i*)ptr, fVec); }

    AI SkNx saturatedAdd(const SkNx& o) const noexcept { return _mm_adds_epu8(fVec, o.fVec); }

    AI SkNx operator+(const SkNx& o) const noexcept { return _mm_add_epi8(fVec, o.fVec); }
    AI SkNx operator-(const SkNx& o) const noexcept { return _mm_sub_epi8(fVec, o.fVec); }

    AI static SkNx Min(const SkNx& a, const SkNx& b) noexcept {
        return _mm_min_epu8(a.fVec, b.fVec);
    }
    AI SkNx operator<(const SkNx& o) const noexcept {
...
template <> class SkNx<16, uint8_t> {
public:
    AI SkNx(const __m128i& vec) noexcept : fVec(vec) {}
...
template <> AI /*static*/ Sk4f SkNx_cast<float, int32_t>(const Sk4i& src) noexcept {
    return _mm_cvtepi32_ps(src.fVec);
}

template <> AI /*static*/ Sk4f SkNx_cast<float, uint32_t>(const Sk4u& src) noexcept {
    return SkNx_cast<float>(Sk4i::Load(&src));
}

template <> AI /*static*/ Sk4i SkNx_cast<int32_t, float>(const Sk4f& src) noexcept {
    return _mm_cvttps_epi32(src.fVec);
}

template <> AI /*static*/ Sk4h SkNx_cast<uint16_t, int32_t>(const Sk4i& src) noexcept {
    ...
    const int _ = ~0;
    return _mm_shuffle_epi8(src.fVec,
                            _mm_setr_epi8(0, 1, 4, 5, 8, 9, 12, 13, _, _, _, _, _, _, _, _));
#else
...
template <> AI /*static*/ Sk4h SkNx_cast<uint16_t, float>(const Sk4f& src) noexcept {
    return SkNx_cast<uint16_t>(SkNx_cast<int32_t>(src));
}

template <> AI /*static*/ Sk4b SkNx_cast<uint8_t, float>(const Sk4f& src) noexcept {
...
template <> AI /*static*/ Sk4u SkNx_cast<uint32_t, uint8_t>(const Sk4b& src) noexcept {
#if SK_CPU_SSE_LEVEL >= SK_CPU_SSE_LEVEL_SSSE3
    const int _ = ~0;
    return _mm_shuffle_epi8(src.fVec,
...
template <> AI /*static*/ Sk4i SkNx_cast<int32_t, uint8_t>(const Sk4b& src) noexcept {
    return SkNx_cast<uint32_t>(src).fVec;
}

template <> AI /*static*/ Sk4f SkNx_cast<float, uint8_t>(const Sk4b& src) noexcept {
    return _mm_cvtepi32_ps(SkNx_cast<int32_t>(src).fVec);
}

template <> AI /*static*/ Sk4f SkNx_cast<float, uint16_t>(const Sk4h& src) noexcept {
    auto _32 = _mm_unpacklo_epi16(src.fVec, _mm_setzero_si128());
    return _mm_cvtepi32_ps(_32);
}

template <> AI /*static*/ Sk8b SkNx_cast<uint8_t, int32_t>(const Sk8i& src) noexcept {
    Sk4i lo, hi;
...
template <> AI /*static*/ Sk16b SkNx_cast<uint8_t, float>(const Sk16f& src) noexcept {
    Sk8f ab, cd;
    ...
    return _mm_packus_epi16(_mm_packus_epi16(_mm_cvttps_epi32(a.fVec), _mm_cvttps_epi32(b.fVec)),
                            _mm_packus_epi16(_mm_cvttps_epi32(c.fVec), _mm_cvttps_epi32(d.fVec)));
}

template <> AI /*static*/ Sk4h SkNx_cast<uint16_t, uint8_t>(const Sk4b& src) noexcept {
    return _mm_unpacklo_epi8(src.fVec, _mm_setzero_si128());
}

template <> AI /*static*/ Sk8h SkNx_cast<uint16_t, uint8_t>(const Sk8b& src) noexcept {
    return _mm_unpacklo_epi8(src.fVec, _mm_setzero_si128());
}

template <> AI /*static*/ Sk4b SkNx_cast<uint8_t, uint16_t>(const Sk4h& src) noexcept {
    return _mm_packus_epi16(src.fVec, src.fVec);
}

template <> AI /*static*/ Sk8b SkNx_cast<uint8_t, uint16_t>(const Sk8h& src) noexcept {
    return _mm_packus_epi16(src.fVec, src.fVec);
}

template <> AI /*static*/ Sk4i SkNx_cast<int32_t, uint16_t>(const Sk4h& src) noexcept {
    return _mm_unpacklo_epi16(src.fVec, _mm_setzero_si128());
}

template <> AI /*static*/ Sk4b SkNx_cast<uint8_t, int32_t>(const Sk4i& src) noexcept {
    return _mm_packus_epi16(_mm_packus_epi16(src.fVec, src.fVec), src.fVec);
}

template <> AI /*static*/ Sk4b SkNx_cast<uint8_t, uint32_t>(const Sk4u& src) noexcept {
    return _mm_packus_epi16(_mm_packus_epi16(src.fVec, src.fVec), src.fVec);
}

template <> AI /*static*/ Sk4i SkNx_cast<int32_t, uint32_t>(const Sk4u& src) noexcept {
    return src.fVec;
}

AI static Sk4i Sk4f_round(const Sk4f& x) noexcept { return _mm_cvtps_epi32(x.fVec); }
