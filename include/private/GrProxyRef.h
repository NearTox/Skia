...
class GrProxyRef {
 public:
  ...
  GrProxyRef(sk_sp<T> proxy, GrIOType ioType) noexcept { this->setProxy(std::move(proxy), ioType); }
  ...
  void reset() noexcept {
    if (fPendingIO) {
      SkASSERT(fProxy);
      switch (fIOType) {
        case kRead_GrIOType: fProxy->completedRead(); break;
        case kWrite_GrIOType: fProxy->completedWrite(); break;
        case kRW_GrIOType:
  ...
  void markPendingIO() const {
    ...
    fPendingIO = true;
    switch (fIOType) {
      case kRead_GrIOType: fProxy->addPendingRead(); break;
      case kWrite_GrIOType: fProxy->addPendingWrite(); break;
      case kRW_GrIOType:
