...
#else
        float32x2_t est0 = vrecpe_f32(o.fVec), est1 = vmul_f32(vrecps_f32(est0, o.fVec), est0),
...
public:
...
#else
        float32x4_t est0 = vrecpeq_f32(o.fVec), est1 = vmulq_f32(vrecpsq_f32(est0, o.fVec), est0),
    ...
    AI bool allTrue() const {
        ...
        return vgetq_lane_u32(v, 0) && vgetq_lane_u32(v, 1) && vgetq_lane_u32(v, 2) &&
               vgetq_lane_u32(v, 3);
#endif
    }
    AI bool anyTrue() const {
...
public:
    ...
    AI SkNx(uint16_t a, uint16_t b, uint16_t c, uint16_t d) { fVec = (uint16x4_t){a, b, c, d}; }
    ...
    AI SkNx thenElse(const SkNx& t, const SkNx& e) const { return vbsl_u16(fVec, t.fVec, e.fVec); }
...
public:
    ...
    AI SkNx thenElse(const SkNx& t, const SkNx& e) const { return vbslq_u16(fVec, t.fVec, e.fVec); }
...
public:
    ...
    AI SkNx thenElse(const SkNx& t, const SkNx& e) const { return vbslq_u8(fVec, t.fVec, e.fVec); }
...
public:
    ...
    AI SkNx() {}
    AI SkNx(int32_t v) { fVec = vdupq_n_s32(v); }
    AI SkNx(int32_t a, int32_t b, int32_t c, int32_t d) { fVec = (int32x4_t){a, b, c, d}; }
    AI static SkNx Load(const void* ptr) { return vld1q_s32((const int32_t*)ptr); }
    AI void store(void* ptr) const { return vst1q_s32((int32_t*)ptr, fVec); }
...
public:
    ...
    AI SkNx() {}
    AI SkNx(uint32_t v) { fVec = vdupq_n_u32(v); }
    AI SkNx(uint32_t a, uint32_t b, uint32_t c, uint32_t d) { fVec = (uint32x4_t){a, b, c, d}; }
    AI static SkNx Load(const void* ptr) { return vld1q_u32((const uint32_t*)ptr); }
    AI void store(void* ptr) const { return vst1q_u32((uint32_t*)ptr, fVec); }
    ...
    AI SkNx thenElse(const SkNx& t, const SkNx& e) const { return vbslq_u32(fVec, t.fVec, e.fVec); }
...
template <> AI /*static*/ Sk16b SkNx_cast<uint8_t, float>(const Sk16f& src) {
    ...
    SkNx_split(ab, &a, &b);
    SkNx_split(cd, &c, &d);
    return vuzpq_u8(vuzpq_u8((uint8x16_t)vcvtq_u32_f32(a.fVec), (uint8x16_t)vcvtq_u32_f32(b.fVec))
                            .val[0],
                    vuzpq_u8((uint8x16_t)vcvtq_u32_f32(c.fVec), (uint8x16_t)vcvtq_u32_f32(d.fVec))
                            .val[0])
            .val[0];
}
...
AI static Sk4i Sk4f_round(const Sk4f& x) { return vcvtq_s32_f32((x + 0.5f).fVec); }
