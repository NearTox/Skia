...
struct GrGLTextureSpec {
  constexpr GrGLTextureSpec() noexcept : fTarget(0), fFormat(0) {}
  GrGLTextureSpec(const GrGLSurfaceInfo& info) noexcept
      : fTarget(info.fTarget), fFormat(info.fFormat) {}
...
GrGLSurfaceInfo GrGLTextureSpecToSurfaceInfo(
    const GrGLTextureSpec& glSpec, uint32_t sampleCount, uint32_t levelCount,
