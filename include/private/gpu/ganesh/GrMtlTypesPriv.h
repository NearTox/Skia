...
#    define GR_NORETAIN_BEGIN                                                 \
      _Pragma(                                                                \
          "clang attribute push (__attribute__((objc_externally_retained)), " \
          "apply_to=any(function,objc_method))")
...
struct GrMtlTextureSpec {
  GrMtlTextureSpec() : fFormat(0), fUsage(0), fStorageMode(0) {}
  GrMtlTextureSpec(const GrMtlSurfaceInfo& info)
      : fFormat(info.fFormat), fUsage(info.fUsage), fStorageMode(info.fStorageMode) {}
...
GrMtlSurfaceInfo GrMtlTextureSpecToSurfaceInfo(
    const GrMtlTextureSpec& mtlSpec, uint32_t sampleCount, uint32_t levelCount,
