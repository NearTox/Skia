...
template <int N, typename T> struct SKVX_ALIGNMENT Vec {
...
template <typename D, typename S> static inline D bit_pun(const S& s) {
...
template <int N, typename T> using VExt = T __attribute__((ext_vector_type(N)));

#elif defined(__GNUC__)
template <int N, typename T> struct VExtHelper {
    typedef T __attribute__((vector_size(N * sizeof(T)))) type;
};

template <int N, typename T> using VExt = typename VExtHelper<N, T>::type;
...
template <typename D, typename S> static inline Vec<1, D> cast(const Vec<1, S>& src) {
    return (D)src.val;
}

template <typename D, int N, typename S> static inline Vec<N, D> cast(const Vec<N, S>& src) {
...
template <int N> static inline Vec<N, uint8_t> div255(const Vec<N, uint16_t>& x) {
...
static inline Vec<4, float> if_then_else(const Vec<4, int>& c,
                                         const Vec<4, float>& t,
                                         const Vec<4, float>& e) {
    return bit_pun<Vec<4, float>>(_mm_or_ps(_mm_and_ps(bit_pun<__m128>(c), bit_pun<__m128>(t)),
