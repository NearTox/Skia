...
#if defined(__SSE4_1__)
  if /*constexpr*/ (N * sizeof(T) == 16) {
    return _mm_testc_si128(unchecked_bit_pun<__m128i>(x), _mm_set1_epi32(-1));
  }
#endif
#if defined(__wasm_simd128__)
  if /*constexpr*/ (N == 4 && sizeof(T) == 4) {
...
#endif
SI auto
map(const skvx::Vec<N, T>& x, Fn&& fn, std::index_sequence<I...> ix = {})
    -> skvx::Vec<N, decltype(fn(x[0]))> {
...
SI Vec<2, float> rcp(const Vec<2, float>& x) { return shuffle<0, 1>(rcp(shuffle<0, 1, 0, 1>(x))); }
#  endif
