...
SI Vec<sizeof...(Ix), T> shuffle(const Vec<N, T>&) noexcept;

template <typename D, typename S>
SI D bit_pun(const S&) noexcept;

// All Vec have the same simple memory layout, the same as `T vec[N]`.
template <int N, typename T>
struct alignas(N * sizeof(T)) VecStorage {
  SKVX_ALWAYS_INLINE VecStorage() noexcept = default;
  SKVX_ALWAYS_INLINE VecStorage(T s) noexcept : lo(s), hi(s) {}
...
struct VecStorage<4, T> {
  SKVX_ALWAYS_INLINE VecStorage() noexcept = default;
  SKVX_ALWAYS_INLINE VecStorage(T s) noexcept : lo(s), hi(s) {}
  SKVX_ALWAYS_INLINE VecStorage(T x, T y, T z, T w) noexcept : lo(x, y), hi(z, w) {}
  SKVX_ALWAYS_INLINE VecStorage(Vec<2, T> xy, T z, T w) noexcept : lo(xy), hi(z, w) {}
  SKVX_ALWAYS_INLINE VecStorage(T x, T y, Vec<2, T> zw) noexcept : lo(x, y), hi(zw) {}
  SKVX_ALWAYS_INLINE VecStorage(Vec<2, T> xy, Vec<2, T> zw) noexcept : lo(xy), hi(zw) {}

  SKVX_ALWAYS_INLINE Vec<2, T>& xy() noexcept { return lo; }
  SKVX_ALWAYS_INLINE Vec<2, T>& zw() noexcept { return hi; }
  SKVX_ALWAYS_INLINE T& x() noexcept { return lo.lo.val; }
  SKVX_ALWAYS_INLINE T& y() noexcept { return lo.hi.val; }
  SKVX_ALWAYS_INLINE T& z() noexcept { return hi.lo.val; }
  SKVX_ALWAYS_INLINE T& w() noexcept { return hi.hi.val; }

  SKVX_ALWAYS_INLINE Vec<2, T> xy() const noexcept { return lo; }
  SKVX_ALWAYS_INLINE Vec<2, T> zw() const noexcept { return hi; }
  SKVX_ALWAYS_INLINE T x() const noexcept { return lo.lo.val; }
  SKVX_ALWAYS_INLINE T y() const noexcept { return lo.hi.val; }
  SKVX_ALWAYS_INLINE T z() const noexcept { return hi.lo.val; }
  SKVX_ALWAYS_INLINE T w() const noexcept { return hi.hi.val; }

  // Exchange-based swizzles. These should take 1 cycle on NEON and 3 (pipelined) cycles on SSE.
  SKVX_ALWAYS_INLINE Vec<4, T> yxwz() const noexcept {
    return shuffle<1, 0, 3, 2>(bit_pun<Vec<4, T>>(*this));
  }
  SKVX_ALWAYS_INLINE Vec<4, T> zwxy() const noexcept {
...
struct VecStorage<2, T> {
  SKVX_ALWAYS_INLINE VecStorage() noexcept = default;
  SKVX_ALWAYS_INLINE VecStorage(T s) noexcept : lo(s), hi(s) {}
  SKVX_ALWAYS_INLINE VecStorage(T x, T y) noexcept : lo(x), hi(y) {}

  SKVX_ALWAYS_INLINE T& x() noexcept { return lo.val; }
  SKVX_ALWAYS_INLINE T& y() noexcept { return hi.val; }

  SKVX_ALWAYS_INLINE T x() const noexcept { return lo.val; }
  SKVX_ALWAYS_INLINE T y() const noexcept { return hi.val; }

  // This exchange-based swizzle should take 1 cycle on NEON and 3 (pipelined) cycles on SSE.
  SKVX_ALWAYS_INLINE Vec<2, T> yx() const noexcept {
    return shuffle<1, 0>(bit_pun<Vec<2, T>>(*this));
  }

  SKVX_ALWAYS_INLINE Vec<4, T> xyxy() const noexcept {
...
struct alignas(N * sizeof(T)) Vec : public VecStorage<N, T> {
  ...
  SKVX_ALWAYS_INLINE T& operator[](int i) noexcept {
    return i < N / 2 ? this->lo[i] : this->hi[i - N / 2];
  }
...
SINT Vec<N, T> if_then_else(
  ...
  if constexpr (N * sizeof(T) == 32) {
    return unchecked_bit_pun<Vec<N, T>>(_mm256_blendv_epi8(
        unchecked_bit_pun<__m256i>(e), unchecked_bit_pun<__m256i>(t),
        unchecked_bit_pun<__m256i>(cond)));
  }
#endif
#if SKVX_USE_SIMD && defined(__SSE4_1__)
  if constexpr (N * sizeof(T) == 16) {
    return unchecked_bit_pun<Vec<N, T>>(_mm_blendv_epi8(
        unchecked_bit_pun<__m128i>(e), unchecked_bit_pun<__m128i>(t),
        unchecked_bit_pun<__m128i>(cond)));
  }
#endif
#if SKVX_USE_SIMD && defined(__ARM_NEON)
  if constexpr (N * sizeof(T) == 16) {
    return unchecked_bit_pun<Vec<N, T>>(vbslq_u8(
        unchecked_bit_pun<uint8x16_t>(cond), unchecked_bit_pun<uint8x16_t>(t),
...
SINT bool any(const Vec<N, T>& x) noexcept {
  ...
  if constexpr (N * sizeof(T) == 16) {
    ...
    return _mm_movemask_ps(_mm_cmpneq_ps(unchecked_bit_pun<__m128>(x), _mm_set1_ps(0))) != 0b0000;
  }
#endif
  ...
  if constexpr (N * sizeof(T) == 8) {
    return vmaxv_u8(unchecked_bit_pun<uint8x8_t>(x)) > 0;
  }
  if constexpr (N * sizeof(T) == 16) {
    return vmaxvq_u8(unchecked_bit_pun<uint8x16_t>(x)) > 0;
  }
#endif
...
// true lane has a mix of 0 and 1 bits.
  ...
  if constexpr (N == 4 && sizeof(T) == 4) {
    return _mm_movemask_ps(_mm_cmpneq_ps(unchecked_bit_pun<__m128>(x), _mm_set1_ps(0))) == 0b1111;
  }
#endif
#if SKVX_USE_SIMD && defined(__aarch64__)
  // On 64-bit NEON, take the min across the lanes, which will be non-zero if all lanes are != 0.
  if constexpr (sizeof(T) == 1 && N == 8) {
    return vminv_u8(unchecked_bit_pun<uint8x8_t>(x)) > 0;
  }
  if constexpr (sizeof(T) == 1 && N == 16) {
    return vminvq_u8(unchecked_bit_pun<uint8x16_t>(x)) > 0;
  }
  if constexpr (sizeof(T) == 2 && N == 4) {
    return vminv_u16(unchecked_bit_pun<uint16x4_t>(x)) > 0;
  }
  if constexpr (sizeof(T) == 2 && N == 8) {
    return vminvq_u16(unchecked_bit_pun<uint16x8_t>(x)) > 0;
  }
  if constexpr (sizeof(T) == 4 && N == 2) {
    return vminv_u32(unchecked_bit_pun<uint32x2_t>(x)) > 0;
  }
  if constexpr (sizeof(T) == 4 && N == 4) {
    return vminvq_u32(unchecked_bit_pun<uint32x4_t>(x)) > 0;
  }
#endif
#if SKVX_USE_SIMD && defined(__wasm_simd128__)
  if constexpr (N == 4 && sizeof(T) == 4) {
...
SIN Vec<N, uint16_t> to_half_finite_ftz(const Vec<N, float>& x) noexcept {
  Vec<N, uint32_t> sem = bit_pun<Vec<N, uint32_t>>(x), s = sem & 0x8000'0000, em = sem ^ s,
                   is_norm =
                       em > 0x387f'd000,  // halfway between largest f16 denorm and smallest norm
...
SIN Vec<N, float> from_half_finite_ftz(const Vec<N, uint16_t>& x) noexcept {
  Vec<N, uint32_t> wide = cast<uint32_t>(x), s = wide & 0x8000, em = wide ^ s, is_norm = em > 0x3ff,
...
SINT std::enable_if_t<std::is_unsigned_v<T>, Vec<N, T>> saturated_add(
    const Vec<N, T>& x, const Vec<N, T>& y) noexcept {
...
#  if defined(__SSE__)
    return unchecked_bit_pun<Vec<N, T>>(
        _mm_adds_epu8(unchecked_bit_pun<__m128i>(x), unchecked_bit_pun<__m128i>(y)));
#  else  // __ARM_NEON
    return unchecked_bit_pun<Vec<N, T>>(
        vqaddq_u8(unchecked_bit_pun<uint8x16_t>(x), unchecked_bit_pun<uint8x16_t>(y)));
#  endif
...
class ScaledDividerU32 {
 public:
  explicit ScaledDividerU32(uint32_t divisor) noexcept
      : fDivisorFactor{(uint32_t)(std::round((1.0 / divisor) * (1ull << 32)))},
        fHalf{(divisor + 1) >> 1} {
    assert(divisor > 1);
  }

  Vec<4, uint32_t> divide(const Vec<4, uint32_t>& numerator) const noexcept {
#if SKVX_USE_SIMD && defined(__ARM_NEON)
  ...
  uint32_t half() const noexcept { return fHalf; }
...
SIN std::enable_if_t<(N < 8), Vec<N, uint16_t>> mull(
...
SI Vec<4, uint32_t> mull(const Vec<4, uint16_t>& x, const Vec<4, uint16_t>& y) noexcept {
  return to_vec<4, uint32_t>(vmull_u16(to_vext(x), to_vext(y)));
}

SIN std::enable_if_t<(N < 4), Vec<N, uint32_t>> mull(
    const Vec<N, uint16_t>& x, const Vec<N, uint16_t>& y) noexcept {
...
SIN Vec<N, uint32_t> mull(const Vec<N, uint16_t>& x, const Vec<N, uint16_t>& y) noexcept {
  return cast<uint32_t>(x) * cast<uint32_t>(y);
}
#endif

SINT T dot(const Vec<N, T>& a, const Vec<N, T>& b) noexcept {
  auto ab = a * b;
  if constexpr (N == 2) {
    return ab[0] + ab[1];
...
SI float cross(const Vec<2, float>& a, const Vec<2, float>& b) noexcept {
...
SIT void strided_load4(
    const T* v, Vec<1, T>& a, Vec<1, T>& b, Vec<1, T>& c, Vec<1, T>& d) noexcept {
  a.val = v[0];
  b.val = v[1];
  c.val = v[2];
  d.val = v[3];
}
SINT void strided_load4(
    const T* v, Vec<N, T>& a, Vec<N, T>& b, Vec<N, T>& c, Vec<N, T>& d) noexcept {
  strided_load4(v, a.lo, b.lo, c.lo, d.lo);
  strided_load4(v + 4 * (N / 2), a.hi, b.hi, c.hi, d.hi);
}
#if SKVX_USE_SIMD && defined(__ARM_NEON)
#  define IMPL_LOAD4_TRANSPOSED(N, T, VLD)                                             \
    SI void strided_load4(                                                             \
        const T* v, Vec<N, T>& a, Vec<N, T>& b, Vec<N, T>& c, Vec<N, T>& d) noexcept { \
      auto mat = VLD(v);                                                               \
...
SI void strided_load4(
    const float* v, Vec<4, float>& a, Vec<4, float>& b, Vec<4, float>& c,
    Vec<4, float>& d) noexcept {
...
// WARNING: These are really only supported well on NEON. Consider restructuring your data before
// resorting to these methods.
SIT void strided_load2(const T* v, Vec<1, T>& a, Vec<1, T>& b) noexcept {
  a.val = v[0];
  b.val = v[1];
}
SINT void strided_load2(const T* v, Vec<N, T>& a, Vec<N, T>& b) noexcept {
  strided_load2(v, a.lo, b.lo);
  strided_load2(v + 2 * (N / 2), a.hi, b.hi);
}
#if SKVX_USE_SIMD && defined(__ARM_NEON)
#  define IMPL_LOAD2_TRANSPOSED(N, T, VLD)                                   \
    SI void strided_load2(const T* v, Vec<N, T>& a, Vec<N, T>& b) noexcept { \
      auto mat = VLD(v);                                                     \
      a = bit_pun<Vec<N, T>>(mat.val[0]);                                    \
      b = bit_pun<Vec<N, T>>(mat.val[1]);                                    \
    }
