...
constexpr auto data(Array (&a)[N]) -> decltype(a) {
  return a;
}
template <typename T>
constexpr const T* data(std::initializer_list<T> i) {
  return i.begin();
}

template <typename Container>
constexpr auto size(Container& c) -> decltype(c.size()) {
  return c.size();
}
template <typename Array, size_t N>
constexpr size_t size(Array (&)[N]) {
  return N;
}
template <typename T>
constexpr const T* size(std::initializer_list<T> i) {
