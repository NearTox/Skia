 ...
 */
template <typename Message> class SkMessageBus : SkNoncopyable {
public:
...
#define DECLARE_SKMESSAGEBUS_MESSAGE(Message)                         \
    template <> SkMessageBus<Message>* SkMessageBus<Message>::Get() { \
        static SkOnce once;                                           \
...
template <typename Message> void SkMessageBus<Message>::Inbox::receive(const Message& m) {
    SkAutoMutexExclusive lock(fMessagesMutex);
    fMessages.push_back(m);
}

template <typename Message> void SkMessageBus<Message>::Inbox::poll(SkTArray<Message>* messages) {
    SkASSERT(messages);
    messages->reset();
    SkAutoMutexExclusive lock(fMessagesMutex);
