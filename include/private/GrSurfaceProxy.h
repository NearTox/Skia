...
protected:
    GrIORefProxy() noexcept : fTarget(nullptr), fRefCnt(1), fPendingReads(0), fPendingWrites(0) {}
    ...
    int32_t internalGetTotalRefs() const noexcept {
...
public:
    ...
    static sk_sp<GrSurfaceContext> TestCopy(GrRecordingContext* context,
                                            const GrSurfaceDesc& dstDesc, GrSurfaceOrigin,
...
protected:
    // Deferred version
    GrSurfaceProxy(const GrBackendFormat& format, const GrSurfaceDesc& desc, GrSurfaceOrigin origin,
...
private:
    ...
    FirstRefAccess(GrSurfaceProxy* proxy) noexcept : fProxy(proxy) {}
