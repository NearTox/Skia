...
template <int N, typename T> struct SkNx {
...
template <typename T> struct SkNx<1, T> {
    T fVal;
...
private:
    ...
    template <typename U> AI static U ToBits(U v) { return v; }
    ...
    template <typename Bits> AI static T FromBits(Bits bits) {
        static_assert(
...
template <typename Dst, typename Src> AI static SkNx<1, Dst> SkNx_cast(const SkNx<1, Src>& v) {
...
AI static void Sk4f_ToBytes(uint8_t p[16], const Sk4f& a, const Sk4f& b, const Sk4f& c,
                            const Sk4f& d) noexcept {
