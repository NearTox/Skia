...
class SkAutoTArray {
 public:
   ...
   */
  explicit SkAutoTArray(int count) noexcept {
    SkASSERT(count >= 0);
    if (count) {
   ...
   */
  void reset(int count) noexcept { *this = SkAutoTArray(count); }
...
constexpr auto SkMakeArray(C c) noexcept -> std::array<std::invoke_result_t<C(std::size_t)>, N> {
