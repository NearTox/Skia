...
class SK_API SkRuntimeEffect : public SkRefCnt {
 ...
 private:
  ...
  SpecializeResult specialize(
      SkSL::Program& baseProgram, const void* inputs, const SkSL::SharedCompiler&) const;
  ...
  size_t uniformSize() const noexcept { return fUniformSize; }
...
struct SkRuntimeShaderBuilder {
  ...
  struct BuilderInput {
    ...
    template <typename T>
    std::enable_if_t<std::is_trivially_copyable<T>::value, BuilderInput&> operator=(const T& val) {
      if (!fVar) {
        ...
        memcpy(
            SkTAddOffset<void>(fOwner->fInputs->writable_data(), fVar->fOffset), &val, sizeof(val));
      }
      return *this;
    }
