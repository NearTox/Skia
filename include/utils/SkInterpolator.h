...
class SK_API SkInterpolatorBase : SkNoncopyable {
 ...
 protected:
  ...
  static SkScalar ComputeRelativeT(
      SkMSec time, SkMSec prevTime, SkMSec nextTime, const SkScalar blend[4] = nullptr);
  int16_t fFrameCount;
  uint8_t fElemCount;
  uint8_t fFlags;
  SkScalar fRepeat;
...
class SK_API SkInterpolator : public SkInterpolatorBase {
  ...
  */
  bool setKeyFrame(
      int index, SkMSec time, const SkScalar values[], const SkScalar blend[4] = nullptr);
