...
class SkRandom {
 public:
  ...
  constexpr SkRandom(const SkRandom& rand) noexcept : fK(rand.fK), fJ(rand.fJ) {}

  SkRandom& operator=(const SkRandom& rand) noexcept {
    fK = rand.fK;
    fJ = rand.fJ;
   ...
   */
  int32_t nextS() noexcept { return (int32_t)this->nextU(); }
   ...
   */
  float nextRangeF(float min, float max) noexcept { return min + this->nextF() * (max - min); }

  /** Return the next pseudo random number, as an unsigned value of
   at most bitCount bits.
   @param bitCount The maximum number of bits to be returned
   */
  uint32_t nextBits(unsigned bitCount) noexcept {
  ...
  /** Return the next pseudo random unsigned number, mapped to lie within
   [min, max] inclusive.
   */
  uint32_t nextRangeU(uint32_t min, uint32_t max) noexcept {
    SkASSERT(min <= max);
  ...
  /** Return the next pseudo random unsigned number, mapped to lie within
   [0, count).
   */
  uint32_t nextULessThan(uint32_t count) noexcept {
    SkASSERT(count > 0);
   ...
   */
  SkScalar nextUScalar1() noexcept { return SkFixedToScalar(this->nextUFixed1()); }

  /** Return the next pseudo random number expressed as a SkScalar
   in the range [min..max).
   */
  SkScalar nextRangeScalar(SkScalar min, SkScalar max) noexcept {
   ...
   */
  SkScalar nextSScalar1() noexcept { return SkFixedToScalar(this->nextSFixed1()); }

  /** Return the next pseudo random number as a bool.
   */
  bool nextBool() noexcept { return this->nextU() >= 0x80000000; }

  /** A biased version of nextBool().
   */
  bool nextBiasedBool(SkScalar fractionTrue) noexcept {
   ...
   */
  void setSeed(uint32_t seed) noexcept { init(seed); }
 ...
 private:
  ...
  static constexpr uint32_t NextLCG(uint32_t seed) noexcept { return kMul * seed + kAdd; }

  /** Return the next pseudo random number expressed as an unsigned SkFixed
   in the range [0..SK_Fixed1).
   */
  SkFixed nextUFixed1() noexcept { return this->nextU() >> 16; }

  /** Return the next pseudo random number expressed as a signed SkFixed
   in the range [-SK_Fixed1..SK_Fixed1).
   */
  SkFixed nextSFixed1() noexcept { return this->nextS() >> 15; }
