...
DSLStatement For(
    ...
    Position pos = {}, ForLoopPositions positions = {});
...
DSLGlobalVar InterfaceBlock(
    ...
    std::string_view varName = "", int arraySize = 0, Position pos = {});

/**
 * return [value];
 */
DSLStatement Return(DSLExpression value = DSLExpression(), Position pos = {});

/**
 * test ? ifTrue : ifFalse
 */
DSLExpression Select(
    DSLExpression test, DSLExpression ifTrue, DSLExpression ifFalse, Position = {});
...
DSLExpression Smoothstep(
    DSLExpression edge1, DSLExpression edge2, DSLExpression x, Position pos = {});
