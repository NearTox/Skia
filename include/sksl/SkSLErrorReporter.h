...
class PositionInfo {
 public:
  PositionInfo(const char* file = nullptr, int line = -1) : fFile(file), fLine(line) {}
  ...
  const char* file_name() const { return fFile; }
...
class ErrorReporter {
 public:
  ...
  virtual ~ErrorReporter() { SkASSERT(fPendingErrors.empty()); }
  ...
  void resetErrorCount() { fErrorCount = 0; }
