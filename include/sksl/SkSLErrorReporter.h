...
class ErrorReporter {
 public:
  ErrorReporter() noexcept = default;

  virtual ~ErrorReporter() = default;

  void error(Position position, std::string_view msg);

  std::string_view source() const noexcept { return fSource; }

  void setSource(std::string_view source) noexcept { fSource = source; }

  int errorCount() const noexcept { return fErrorCount; }

  void resetErrorCount() noexcept { fErrorCount = 0; }
