...
class DSLStatement {
 ...
 private:
  ...
  std::unique_ptr<SkSL::Statement> releaseIfPossible() { return std::move(fStatement); }
...
class DSLPossibleStatement {
 public:
  ...
  std::unique_ptr<SkSL::Statement> release() { return DSLStatement(std::move(*this)).release(); }
