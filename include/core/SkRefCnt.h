 ...
 */
template <typename T>
static inline T* SkRef(T* obj) noexcept(noexcept(obj->ref())) {
  SkASSERT(obj);
  obj->ref();
  return obj;
}
 ...
 */
template <typename T>
static inline T* SkSafeRef(T* obj) noexcept(noexcept(obj->ref())) {
  if (obj) {
    obj->ref();
  }
  return obj;
...
class sk_sp {
 public:
   ...
   */
  sk_sp(const sk_sp<T>& that) noexcept(noexcept(SkSafeUnref(that.get())))
      : fPtr(SkSafeRef(that.get())) {}
  template <
      typename U, typename = typename std::enable_if<std::is_convertible<U*, T*>::value>::type>
  sk_sp(const sk_sp<U>& that) noexcept(noexcept(SkSafeUnref(that.get())))
      : fPtr(SkSafeRef(that.get())) {}
   ...
   *  will be made.
   */
  sk_sp<T>& operator=(sk_sp<T>&& that) noexcept(noexcept(this->reset())) {
    this->reset(that.release());
    return *this;
  }
  template <
      typename U, typename = typename std::enable_if<std::is_convertible<U*, T*>::value>::type>
  sk_sp<T>& operator=(sk_sp<U>&& that) noexcept(noexcept(this->reset())) {
    this->reset(that.release());
    return *this;
  }
   ...
   */
  void reset(T* ptr = nullptr) noexcept(noexcept(SkSafeUnref(fPtr))) {
 ...
 */
template <typename T>
sk_sp<T> sk_ref_sp(T* obj) noexcept(noexcept(SkSafeRef(obj))) {
  return sk_sp<T>(SkSafeRef(obj));
}

template <typename T>
sk_sp<T> sk_ref_sp(const T* obj) noexcept(noexcept(SkSafeRef(obj))) {
