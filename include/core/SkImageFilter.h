...
class SK_API SkImageFilter : public SkFlattenable {
 public:
  ...
  class CropRect {
   public:
     ...
     */
    void applyTo(
        const SkIRect& imageBounds, const SkMatrix& matrix, bool embiggen, SkIRect* cropped) const;
   ...
   */
  sk_sp<SkSpecialImage> filterImage(
  ...
  bool asColorFilter(SkColorFilter** filterPtr) const { return this->isColorFilterNode(filterPtr); }
 ...
 protected:
   ...
   */
  virtual sk_sp<SkSpecialImage> onFilterImage(
      SkSpecialImage* src, const Context&, SkIPoint* offset) const = 0;
   ...
   */
  virtual SkIRect onFilterNodeBounds(
      const SkIRect&, const SkMatrix& ctm, MapDirection, const SkIRect* inputRect) const;
  ...
  sk_sp<SkSpecialImage> filterInput(
      int index, SkSpecialImage* src, const Context&, SkIPoint* offset) const;
   ...
   *  larger destination.
   */
  sk_sp<SkSpecialImage> applyCropRectAndPad(
      const Context&, SkSpecialImage* src, SkIPoint* srcOffset, SkIRect* bounds) const;
 ...
 private:
  ...
  friend bool SkIsSameFilter(const SkImageFilter* a, const SkImageFilter* b) noexcept;

  static void PurgeCache();

  void init(sk_sp<SkImageFilter> const* inputs, int inputCount, const CropRect* cropRect) noexcept;
