 ...
 */
template <typename T> static constexpr bool SkToBool(const T& x) noexcept { return 0 != x; }
...
template <typename T> static constexpr inline T SkTAbs(T value) noexcept {
    if (value < 0) {
        value = -value;
    }
    return value;
...
template <typename T> constexpr const T& SkTMin(const T& a, const T& b) { return (a < b) ? a : b; }

template <typename T> constexpr const T& SkTMax(const T& a, const T& b) { return (b < a) ? a : b; }
 ...
 */
enum class SkBudgeted : bool { kNo = false, kYes = true };
