...
class SkMeshSpecification : public SkNVRefCnt<SkMeshSpecification> {
 ...
 private:
  ...
  static Result MakeFromSourceWithStructs(
      SkSpan<const Attribute> attributes, size_t stride, SkSpan<const Varying> varyings,
      const SkString& vs, const SkString& fs, sk_sp<SkColorSpace> cs, SkAlphaType at);

  SkMeshSpecification(
      SkSpan<const Attribute>, size_t, SkSpan<const Varying>, std::vector<Uniform> uniforms,
      std::unique_ptr<const SkSL::Program>, std::unique_ptr<const SkSL::Program>, ColorType,
      bool hasLocalCoords, sk_sp<SkColorSpace>, SkAlphaType);
...
class SkMesh {
 public:
  ...
  static SkMesh Make(
      sk_sp<SkMeshSpecification>, Mode, sk_sp<VertexBuffer>, size_t vertexCount,
      size_t vertexOffset, sk_sp<const SkData> uniforms, const SkRect& bounds);

  static SkMesh MakeIndexed(
      sk_sp<SkMeshSpecification>, Mode, sk_sp<VertexBuffer>, size_t vertexCount,
      size_t vertexOffset, sk_sp<IndexBuffer>, size_t indexCount, size_t indexOffset,
      sk_sp<const SkData> uniforms, const SkRect& bounds);

  sk_sp<SkMeshSpecification> spec() const noexcept { return fSpec; }

  Mode mode() const noexcept { return fMode; }

  sk_sp<VertexBuffer> vertexBuffer() const noexcept { return fVB; }

  size_t vertexOffset() const noexcept { return fVOffset; }
  size_t vertexCount() const noexcept { return fVCount; }

  sk_sp<IndexBuffer> indexBuffer() const noexcept { return fIB; }

  size_t indexOffset() const noexcept { return fIOffset; }
  size_t indexCount() const noexcept { return fICount; }

  sk_sp<const SkData> uniforms() const noexcept { return fUniforms; }

  SkRect bounds() const noexcept { return fBounds; }
