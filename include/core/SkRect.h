...
struct SK_API SkRect {
  ...
  */
  ...
  // Experimental
  SkPoint topLeft() const noexcept { return {fLeft, fTop}; }
  /** Returns spans on the x-axis and y-axis. This does not check if SkRect is sorted,
      or if result fits in 32-bit signed integer; result may be negative.

      @return  SkISize (width, height)
  */
  SkSize size() const noexcept { return SkSize::Make(this->width(), this->height()); }
