...
struct SK_API SkFontMetrics {
  bool operator==(const SkFontMetrics& that) noexcept {
    return this->fFlags == that.fFlags && this->fTop == that.fTop &&
           ...
           this->fAvgCharWidth == that.fAvgCharWidth && this->fMaxCharWidth == that.fMaxCharWidth &&
           this->fXMin == that.fXMin && this->fXMax == that.fXMax &&
           this->fXHeight == that.fXHeight && this->fCapHeight == that.fCapHeight &&
