...
class SK_API SkVertices : public SkNVRefCnt<SkVertices> {
 public:
   ...
   */
  static sk_sp<SkVertices> MakeCopy(
      VertexMode mode, int vertexCount, const SkPoint positions[], const SkPoint texs[],
      const SkColor colors[], const BoneIndices boneIndices[], const BoneWeights boneWeights[],
      int indexCount, const uint16_t indices[], bool isVolatile = true);
  ...
  static sk_sp<SkVertices> MakeCopy(
    ...
    return MakeCopy(
        mode, vertexCount, positions, texs, colors, boneIndices, boneWeights, 0, nullptr,
        isVolatile);
  }

  static sk_sp<SkVertices> MakeCopy(
      VertexMode mode, int vertexCount, const SkPoint positions[], const SkPoint texs[],
      const SkColor colors[], int indexCount, const uint16_t indices[], bool isVolatile = true) {
    return MakeCopy(
        mode, vertexCount, positions, texs, colors, nullptr, nullptr, indexCount, indices,
        isVolatile);
  }

  static sk_sp<SkVertices> MakeCopy(
      VertexMode mode, int vertexCount, const SkPoint positions[], const SkPoint texs[],
      const SkColor colors[], bool isVolatile = true) {
  ...
  class Builder {
   public:
    ...
    BoneIndices* boneIndices() noexcept;  // returns null if there are no bone indices
    BoneWeights* boneWeights() noexcept;  // returns null if there are no bone weights
