...
class SK_API SkYUVAPixmapInfo {
 public:
   ...
   */
  SkYUVAPixmapInfo(
      const SkYUVAInfo&, const SkColorType[kMaxPlanes], const size_t rowBytes[kMaxPlanes]);
  /**
...
constexpr SkYUVAPixmapInfo::SupportedDataTypes SkYUVAPixmapInfo::SupportedDataTypes::All() {
  ...
  for (ULL c = 1; c <= 4; ++c) {
    for (ULL dt = 0; dt <= ULL(kDataTypeCnt); ++dt) {
      if (DefaultColorTypeForDataType(static_cast<DataType>(dt), static_cast<int>(c)) !=
...
constexpr bool SkYUVAPixmapInfo::SupportedDataTypes::supported(
  ...
  for (int i = 0; i < n; ++i) {
    ...
    SkASSERT(c >= 1 && c <= 4);
    if (!fDataTypeSupport
            [static_cast<size_t>(type) + (c - 1) * static_cast<size_t>(kDataTypeCnt)]) {
      return false;
    }
  }
  return true;
