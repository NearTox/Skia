...
class SK_API SkYUVAInfo {
 public:
  ...
  static constexpr std::tuple<PlaneConfig, Subsampling> PlanarConfigToPlaneConfigAndSubsampling(
      PlanarConfig);
   ...
   * dimensions).
   */
  static int PlaneDimensions(
      SkISize imageDimensions, PlaneConfig, Subsampling, SkEncodedOrigin,
   ...
   */
  static bool GetYUVAIndices(
      PlaneConfig, const uint32_t planeChannelFlags[kMaxPlanes],
   ...
   */
  SkYUVAInfo(
      SkISize dimensions, PlaneConfig, Subsampling, SkYUVColorSpace,
      SkEncodedOrigin origin = kTopLeft_SkEncodedOrigin, Siting sitingX = Siting::kCentered,
   ...
   */
  SkYUVAInfo(
      SkISize dimensions, PlanarConfig, SkYUVColorSpace,
      SkEncodedOrigin origin = kTopLeft_SkEncodedOrigin, Siting sitingX = Siting::kCentered,
      Siting sitingY = Siting::kCentered);
   ...
   */
  size_t computeTotalBytes(
      const size_t rowBytes[kMaxPlanes], size_t planeSizes[kMaxPlanes] = nullptr) const;
...
constexpr int SkYUVAInfo::NumChannelsInPlane(PlaneConfig config, int i) {
  switch (config) {
    ...
    case SkYUVAInfo::PlaneConfig::kY_V_U: return i >= 0 && i < 3 ? 1 : 0;
    ...
    case SkYUVAInfo::PlaneConfig::kUYV: return i == 0 ? 3 : 0;
    case SkYUVAInfo::PlaneConfig::kY_U_V_A:
    case SkYUVAInfo::PlaneConfig::kY_V_U_A: return i >= 0 && i < 4 ? 1 : 0;
    ...
    case SkYUVAInfo::PlaneConfig::kUYVA: return i == 0 ? 4 : 0;
  }
  return 0;
}
...
SkYUVAInfo::PlanarConfigToPlaneConfigAndSubsampling(PlanarConfig planarConfig) {
  switch (planarConfig) {
    case PlanarConfig::kUnknown: return {PlaneConfig::kUnknown, Subsampling ::kUnknown};
