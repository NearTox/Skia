 ...
 */
template <typename T>
static inline T SkCFSafeRetain(T obj) {
  if (obj) {
    CFRetain(obj);
  }
  return obj;
...
template <typename T>
static inline void SkCFSafeRelease(T obj) {
  if (obj) {
    CFRelease(obj);
  }
}
...
class sk_cfp {
 public:
   ...
   *  No call to CFRetain() or CFRelease() will be made.
   */
  explicit sk_cfp(T obj) { fObject = obj; }
...
template <typename T>
inline bool operator==(const sk_cfp<T>& a, const sk_cfp<T>& b) {
  return a.get() == b.get();
}
template <typename T>
inline bool operator==(const sk_cfp<T>& a, std::nullptr_t) {
  return !a;
}
template <typename T>
inline bool operator==(std::nullptr_t, const sk_cfp<T>& b) {
  return !b;
}
