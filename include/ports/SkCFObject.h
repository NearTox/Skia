...
class sk_cf_obj {
 public:
  ...
  sk_cf_obj<T>& operator=(std::nullptr_t) {
    this->reset();
    return *this;
  }
...
template <typename T>
inline bool operator==(const sk_cf_obj<T>& a, std::nullptr_t) {
  return !a;
}
template <typename T>
inline bool operator==(std::nullptr_t, const sk_cf_obj<T>& b) {
  return !b;
}
 ...
 */
template <typename T>
sk_cf_obj<T> sk_ret_cf_obj(T obj) {
