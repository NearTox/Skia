...
void SkGIFLZWContext::outputRow(const unsigned char* rowBegin) {
  ...
  m_client->haveDecodedRow(
      m_frameContext->frameId(), rowBegin, drowStart, drowEnd - drowStart + 1,
...
sk_sp<SkColorTable> SkGIFColorMap::buildTable(
    SkStreamBuffer* streamBuffer, SkColorType colorType, int transparentPixel) const {
  ...
  if (m_table && proc == m_packColorProc && m_transPixel == transparentPixel) {
    SkASSERT(
        transparentPixel == kNotFound || transparentPixel > m_table->count() ||
...
bool SkGIFFrameContext::decode(
    SkStreamBuffer* streamBuffer, SkGifCodec* client, bool* frameComplete) {
  *frameComplete = false;
  if (!m_lzwContext) {
    ...
    switch (m_state) {
      ...
      case SkGIFExtension: {
        ...
        switch (*currentComponent) {
          ...
          // and sanity-check the actual length of the application extension header before reading
          // it, we allow GIFs to deviate from these values in either direction. This is important
          // for real-world compatibility, as GIFs in the wild exist with application extension
          // headers that are both shorter and longer than 11 bytes.
          case 0x01:
            // ignoring plain text extension
            break;

          case 0xff: exceptionState = SkGIFApplicationExtension; break;

          case 0xfe: exceptionState = SkGIFConsumeComment; break;
        }
      ...
      case SkGIFNetscapeExtensionBlock: {
        ...
        // SkGIFConsumeNetscapeExtension always reads 3 bytes from the stream; we should at least
        // wait for this amount.
        if (currentComponent)
      ...
      case SkGIFConsumeNetscapeExtension: {
        ...
        // Loop entire animation specified # of times. Only read the loop count during the first
        // iteration.
      ...
      case SkGIFImageHeader: {
        ...
        if (currentFrameIsFirstFrame()) {
          const int transPix =
              m_frames.empty() ? SkGIFColorMap::kNotFound : m_frames[0]->transparentPixel();
          if (is_palette_index_valid(transPix)) {
            m_firstFrameHasAlpha = true;
          } else {
            const bool frameIsSubset =
                xOffset > 0 || yOffset > 0 || width < fScreenWidth || height < fScreenHeight;
