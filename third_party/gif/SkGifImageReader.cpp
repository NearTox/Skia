...
// Colormaps are directly copied in the resp. global_colormap or dynamically allocated
// local_colormap. So a fixed buffer in SkGifImageReader is good enough. This buffer is only needed
// to copy left-over data from one GifWrite call to the next
#define GETN(n, s)              \
    do {                        \
        m_bytesToConsume = (n); \
        m_state = (s);          \
...
namespace {
bool is_palette_index_valid(int transparentIndex) noexcept {
...
void SkGIFLZWContext::outputRow(const unsigned char* rowBegin) {
    ...
    if (m_frameContext->progressiveDisplay() && m_frameContext->interlaced() && ipass < 4) {
        ...
        if (drowStart < 0) drowStart = 0;

        if (drowEnd >= m_frameContext->height()) drowEnd = m_frameContext->height() - 1;
    }

    // Protect against too much image data.
    if (drowStart >= m_frameContext->height()) return;

    // CALLBACK: Let the client know we have decoded a row.
    const bool writeTransparentPixels = SkCodec::kNoFrame == m_frameContext->getRequiredFrame();
    m_client->haveDecodedRow(m_frameContext->frameId(), rowBegin, drowStart,
                             drowEnd - drowStart + 1, writeTransparentPixels);
...
// Returns true if decoding was successful. In this case the block will have been completely
// consumed and/or rowsRemaining will be 0. Otherwise, decoding failed; returns false in this case,
// which will always cause the SkGifImageReader to set the "decode failed" flag.
bool SkGIFLZWContext::doLZW(const unsigned char* block, size_t bytesInBlock) {
    if (rowIter == rowBuffer.end()) return true;
...
// Returns false if a decoding error occurred. This is a fatal error and causes the SkGifImageReader
// to set the "decode failed" flag. Otherwise, either not enough data is available to decode further
// than before, or the new data has been decoded successfully; returns true in this case.
bool SkGIFFrameContext::decode(SkStreamBuffer* streamBuffer, SkGifCodec* client,
    ...
    *frameComplete = false;
    if (!m_lzwContext) {
        ...
        if (!isDataSizeDefined() || !isHeaderDefined()) return true;
    ...
    // If this frame is data complete then the previous loop must have completely decoded all LZW
    // blocks. There will be no more decoding for this frame so it's time to cleanup.
    if (isComplete()) {
        *frameComplete = true;
        m_lzwContext.reset();
    }
...
// Decode a frame.
// This method uses SkGIFFrameContext:decode() to decode the frame; decoding error is reported to
// client as a critical failure. Return true if decoding has progressed. Return false if an error
// has occurred.
        ...
        switch (m_state) {
            ...
            case SkGIFExtension: {
                ...
                switch (*currentComponent) {
                    case 0xf9:
                        // The GIF spec mandates that the GIFControlExtension header block length is
                        // 4 bytes,
                        exceptionState = SkGIFControlExtension;
                        // and the parser for this block reads 4 bytes, so we must enforce that the
                        // buffer contains at least this many bytes. If the GIF specifies a
                    ...
                    // headers (as 12 and 11 bytes, respectively). Because we ignore the plain text
                    // extension entirely and sanity-check the actual length of the application
                    // extension header before reading it, we allow GIFs to deviate from these
                    // values in either direction. This is important for real-world compatibility,
                    // as GIFs in the wild exist with application extension headers that are both
                    // shorter and longer than 11 bytes.
                    case 0x01:
            ...
            case SkGIFControlExtension: {
                ...
                if (*currentComponent & 0x1) currentFrame->setTransparentPixel(currentComponent[3]);
                ...
                switch (rawDisposalMethod) {
                    ...
                    case 3:
                        currentFrame->setDisposalMethod(
                                (SkCodecAnimation::DisposalMethod)rawDisposalMethod);
                        break;
                    case 4:
                        // Some specs say that disposal method 3 is "overwrite previous", others
                        // that setting the third bit of the field (i.e. method 4) is. We map both
                        // to the same value.
                        currentFrame->setDisposalMethod(
            ...
            case SkGIFApplicationExtension: {
                ...
                if (m_state != SkGIFNetscapeExtensionBlock) GETN(1, SkGIFConsumeBlock);
                break;
            }
            ...
            case SkGIFNetscapeExtensionBlock: {
                ...
                // SkGIFConsumeNetscapeExtension always reads 3 bytes from the stream; we should at
                // least wait for this amount.
                if (currentComponent)
            ...
            case SkGIFConsumeNetscapeExtension: {
                ...
                // Loop entire animation specified # of times. Only read the loop count during the
                // first iteration.
                if (netscapeExtension == 1) {
                    m_loopCount = GETINT16(currentComponent + 1);

                    // Zero loop count is infinite animation loop request.
                    if (!m_loopCount) m_loopCount = SkCodec::kRepetitionCountInfinite;
                ...
                if (currentFrameIsFirstFrame()) {
                    ...
                    if (is_palette_index_valid(transPix)) {
                        ...
                        const bool frameIsSubset = xOffset > 0 || yOffset > 0 ||
                                                   width < fScreenWidth || height < fScreenHeight;
...
SkEncodedInfo::Alpha SkGIFFrameContext::onReportedAlpha() const noexcept {
...
bool SkGIFLZWContext::prepareToDecode() {
    ...
    if (m_frameContext->dataSize() >= SK_MAX_DICTIONARY_ENTRY_BITS) return false;
    ...
    constexpr size_t kMaxBytes =
            (kMaxSequence + SK_DICTIONARY_WORD_SIZE - 1) & ~(SK_DICTIONARY_WORD_SIZE - 1);
