...
#if defined(USING_NEON_FP16)
SI U16 swap_endian_16(U16 v) { return (U16)vrev16q_u8((uint8x16_t)v); }
...
static void clut(
    uint32_t input_channels, uint32_t output_channels, const uint8_t grid_points[4],
    const uint8_t* grid_8, const uint8_t* grid_16, F* r, F* g, F* b, F* a) {
  ...
  *r = *g = *b = F0;
  ...
  for (int combo = 0; combo < (1 << dim); combo++) {  // This loop can be done in any order.
    ...
    switch ((dim - 1) & 3) {  // This lets the compiler know there are no other cases to handle.
      ...
      case 1: ix += index[1 + (combo & 2) * 2]; w *= weight[1 + (combo & 2) * 2];
    }
...
static void clut(const skcms_A2B* a2b, F* r, F* g, F* b, F a) {
  clut(
      a2b->input_channels, a2b->output_channels, a2b->grid_points, a2b->grid_8, a2b->grid_16, r, g,
      b, &a);
}
static void clut(const skcms_B2A* b2a, F* r, F* g, F* b, F* a) {
  clut(
      b2a->input_channels, b2a->output_channels, b2a->grid_points, b2a->grid_8, b2a->grid_16, r, g,
      b, a);
}

static void exec_ops(const Op* ops, const void** args, const char* src, char* dst, int i) {
  ...
  while (true) {
    switch (*ops++) {
...
#if defined(USING_NEON_FP16)
        float32x4x3_t lo = vld3q_f32(rgb + 0), hi = vld3q_f32(rgb + 12);
...
#if defined(USING_NEON_FP16)
        float32x4x4_t lo = vld4q_f32(rgba + 0), hi = vld4q_f32(rgba + 16);
        r = (F)vcombine_f16(vcvt_f16_f32(lo.val[0]), vcvt_f16_f32(hi.val[0]));
        g = (F)vcombine_f16(vcvt_f16_f32(lo.val[1]), vcvt_f16_f32(hi.val[1]));
        b = (F)vcombine_f16(vcvt_f16_f32(lo.val[2]), vcvt_f16_f32(hi.val[2]));
      ...
      case Op_tf_r: {
        r = apply_tf((const skcms_TransferFunction*)*args++, r);
      } break;
      case Op_tf_g: {
        g = apply_tf((const skcms_TransferFunction*)*args++, g);
      } break;
      case Op_tf_b: {
        b = apply_tf((const skcms_TransferFunction*)*args++, b);
      } break;
      case Op_tf_a: {
        a = apply_tf((const skcms_TransferFunction*)*args++, a);
      } break;

      case Op_pq_r: {
        r = apply_pq((const skcms_TransferFunction*)*args++, r);
      } break;
      case Op_pq_g: {
        g = apply_pq((const skcms_TransferFunction*)*args++, g);
      } break;
      case Op_pq_b: {
        b = apply_pq((const skcms_TransferFunction*)*args++, b);
      } break;
      case Op_pq_a: {
        a = apply_pq((const skcms_TransferFunction*)*args++, a);
      } break;

      case Op_hlg_r: {
        r = apply_hlg((const skcms_TransferFunction*)*args++, r);
      } break;
      case Op_hlg_g: {
        g = apply_hlg((const skcms_TransferFunction*)*args++, g);
      } break;
      case Op_hlg_b: {
        b = apply_hlg((const skcms_TransferFunction*)*args++, b);
      } break;
      case Op_hlg_a: {
        a = apply_hlg((const skcms_TransferFunction*)*args++, a);
      } break;

      case Op_hlginv_r: {
        r = apply_hlginv((const skcms_TransferFunction*)*args++, r);
      } break;
      case Op_hlginv_g: {
        g = apply_hlginv((const skcms_TransferFunction*)*args++, g);
      } break;
      case Op_hlginv_b: {
        b = apply_hlginv((const skcms_TransferFunction*)*args++, b);
      } break;
      case Op_hlginv_a: {
        a = apply_hlginv((const skcms_TransferFunction*)*args++, a);
      } break;

      case Op_table_r: {
        r = table((const skcms_Curve*)*args++, r);
      } break;
      case Op_table_g: {
        g = table((const skcms_Curve*)*args++, g);
      } break;
      case Op_table_b: {
        b = table((const skcms_Curve*)*args++, b);
      } break;
      case Op_table_a: {
        a = table((const skcms_Curve*)*args++, a);
      } break;
      ...
      case Op_store_4444: {
        store<U16>(
            dst + 2 * i, cast<U16>(to_fixed(r * 15) << 12) | cast<U16>(to_fixed(g * 15) << 8) |
      ...
      case Op_store_565: {
        store<U16>(
            dst + 2 * i, cast<U16>(to_fixed(r * 31) << 0) | cast<U16>(to_fixed(g * 63) << 5) |
...
#if defined(USING_NEON_FP16)
        // See the explanation under USING_NEON below.  This is that doubled up.
        U16 R = to_fixed(r * 255), G = to_fixed(g * 255), B = to_fixed(b * 255);
      ...
      case Op_store_8888: {
        store(
            dst + 4 * i, cast<U32>(to_fixed(r * 255)) << 0 | cast<U32>(to_fixed(g * 255)) << 8 |
      ...
      case Op_store_1010102: {
        store(
            dst + 4 * i, cast<U32>(to_fixed(r * 1023)) << 0 | cast<U32>(to_fixed(g * 1023)) << 10 |
...
#else
        store(
            rgba, cast<U64>(R) << 0 | cast<U64>(G) << 16 | cast<U64>(B) << 32 | cast<U64>(A) << 48);
#endif
      }
        return;
...
static void run_program(
    const Op* program, const void** arguments, const char* src, char* dst, int n,
