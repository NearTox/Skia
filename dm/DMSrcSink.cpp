...
static DEFINE_bool(
    multiPage, false, "For document-type backends, render the source into multiple pages");
...
BRDSrc::BRDSrc(Path path, Mode mode, CodecSrc::DstColorType dstColorType, uint32_t sampleSize)
    : fPath(path), fMode(mode), fDstColorType(dstColorType), fSampleSize(sampleSize) {}
...
static inline void alpha8_to_gray8(SkBitmap* bitmap) {
  ...
  if (kAlpha_8_SkColorType == bitmap->info().colorType()) {
    SkImageInfo newInfo =
        bitmap->info().makeColorType(kGray_8_SkColorType).makeAlphaType(kOpaque_SkAlphaType);
...
Result BRDSrc::draw(GrDirectContext*, SkCanvas* canvas) const {
  ...
  switch (fDstColorType) {
    case CodecSrc::kGetFromCanvas_DstColorType: break;
    case CodecSrc::kGrayscale_Always_DstColorType: colorType = kGray_8_SkColorType; break;
  ...
  switch (fMode) {
    case kFullImage_Mode: {
      SkBitmap bitmap;
      if (!brd->decodeRegion(
              &bitmap, nullptr, SkIRect::MakeXYWH(0, 0, width, height), fSampleSize, colorType,
              false, colorSpace)) {
    ...
    case kDivisor_Mode: {
      ...
      for (uint32_t x = 0; x < divisor; x++) {
        for (uint32_t y = 0; y < divisor; y++) {
          ...
          if (!brd->decodeRegion(
                  &bitmap, nullptr,
                  SkIRect::MakeXYWH(decodeLeft, decodeTop, decodeWidth, decodeHeight), fSampleSize,
                  colorType, false, colorSpace)) {
          ...
          canvas->drawImageRect(
              bitmap.asImage().get(),
              SkRect::MakeXYWH(
                  (SkScalar)scaledBorder, (SkScalar)scaledBorder,
                  (SkScalar)(subsetWidth / fSampleSize), (SkScalar)(subsetHeight / fSampleSize)),
              SkRect::MakeXYWH(
                  (SkScalar)(left / fSampleSize), (SkScalar)(top / fSampleSize),
                  (SkScalar)(subsetWidth / fSampleSize), (SkScalar)(subsetHeight / fSampleSize)),
    ...
    default: SkASSERT(false); return Result::Fatal("Error: Should not be reached.");
  }
}
...
SkISize BRDSrc::size() const {
  ...
  if (brd) {
    return {
        std::max(1, brd->width() / (int)fSampleSize),
...
CodecSrc::CodecSrc(
    Path path, Mode mode, DstColorType dstColorType, SkAlphaType dstAlphaType, float scale)
    : fPath(path),
      fMode(mode),
...
static bool get_decode_info(
    SkImageInfo* decodeInfo, SkColorType canvasColorType, CodecSrc::DstColorType dstColorType,
  ...
  switch (dstColorType) {
    ...
    case CodecSrc::kNonNative8888_Always_DstColorType:
      if (kRGB_565_SkColorType == canvasColorType || kRGBA_F16_SkColorType == canvasColorType) {
        return false;
      }
...
static void draw_to_canvas(
    SkCanvas* canvas, const SkImageInfo& info, void* pixels, size_t rowBytes,
    CodecSrc::DstColorType dstColorType, SkScalar left = 0, SkScalar top = 0) {
  SkBitmap bitmap;
...
Result CodecSrc::draw(GrDirectContext*, SkCanvas* canvas) const {
  ...
  if (!get_decode_info(
          &decodeInfo, canvas->imageInfo().colorType(), fDstColorType, fDstAlphaType)) {
  ...
  switch (fMode) {
    case kAnimated_Mode: {
      ...
      std::vector<SkCodec::FrameInfo> frameInfos =
          androidCodec ? androidCodec->codec()->getFrameInfo() : codec->getFrameInfo();
      ...
      for (int i = 0; static_cast<size_t>(i) < frameInfos.size(); i++) {
        ...
        if (reqFrame != SkCodec::kNoFrame && reqFrame == cachedFrame && priorFramePixels.get()) {
          // Copy into pixels
        ...
        SkCodec::Result result =
            androidCodec ? androidCodec->getAndroidPixels(
                               decodeInfo, pixels.get(), rowBytes, &androidOptions)
        ...
        switch (result) {
          ...
          case SkCodec::kInvalidConversion:
            if (i > 0 && (decodeInfo.colorType() == kRGB_565_SkColorType)) {
              return Result::Skip("Cannot decode frame %i to 565 (%s).", i, fPath.c_str());
            }
            [[fallthrough]];
          default: return Result::Fatal("Couldn't getPixels for frame %i in %s.", i, fPath.c_str());
        }
      }
      break;
    }
    ...
    case kCodec_Mode: {
      switch (codec->getPixels(decodeInfo, pixels.get(), rowBytes, &options)) {
        case SkCodec::kSuccess:
          ...
          // available.
        case SkCodec::kErrorInInput:
        case SkCodec::kIncompleteInput: break;
        default:
        ...
        if (SkCodec::kSuccess ==
          ...
          if (SkCodec::kIncompleteInput == result || SkCodec::kErrorInInput == result) {
            codec->fillIncompleteImage(
                decodeInfo, dst, rowBytes, SkCodec::kNo_ZeroInitialized, height, rowsDecoded);
          }
        } else {
          if (useIncremental) {
    ...
    case kStripe_Mode: {
      ...
      for (int i = 0; i < numStripes; i += 2) {
        ...
        if (linesToRead > 0) {
          codec->getScanlines(SkTAddOffset<void>(dst, rowBytes * startY), linesToRead, rowBytes);
        }
      }
      ...
      for (int x = 0; x < W; x += w) {
        ...
        for (int y = 0; y < H; y += h) {
          ...
          const SkCodec::Result result =
              codec->getPixels(decodeInfo, dst, subsetRowBytes, &options);
          switch (result) {
            case SkCodec::kSuccess:
            case SkCodec::kErrorInInput:
            case SkCodec::kIncompleteInput: break;
            default:
              return Result::Fatal(
                  ...
                  x, y, decodeInfo.width(), decodeInfo.height(), fPath.c_str(), W, H, result);
          }
          draw_to_canvas(
              canvas, subsetBitmapInfo, dst, subsetRowBytes, fDstColorType, SkIntToScalar(left),
              SkIntToScalar(top));
    ...
    default: SkASSERT(false); return Result::Fatal("Invalid fMode");
  }
  return Result::Ok();
}
...
AndroidCodecSrc::AndroidCodecSrc(
    Path path, CodecSrc::DstColorType dstColorType, SkAlphaType dstAlphaType, int sampleSize)
    : fPath(path),
      fDstColorType(dstColorType),
      fDstAlphaType(dstAlphaType),
...
Result AndroidCodecSrc::draw(GrDirectContext*, SkCanvas* canvas) const {
  ...
  switch (codec->getAndroidPixels(decodeInfo, pixels.get(), rowBytes, &options)) {
    ...
    case SkCodec::kErrorInInput:
    case SkCodec::kIncompleteInput: break;
    default: return Result::Fatal("Couldn't getPixels %s.", fPath.c_str());
  }
  draw_to_canvas(canvas, bitmapInfo, pixels.get(), rowBytes, fDstColorType);
  return Result::Ok();
}
...
Result ImageGenSrc::draw(GrDirectContext*, SkCanvas* canvas) const {
  ...
  switch (fMode) {
    ...
    default: SkASSERT(false); return Result::Fatal("Invalid image generator mode");
  }
...
#endif
    return Result(
        status, SkStringPrintf("Image generator could not getPixels() for %s\n", fPath.c_str()));
  }

  set_bitmap_color_space(&decodeInfo);
  draw_to_canvas(canvas, decodeInfo, pixels.get(), rowBytes, CodecSrc::kGetFromCanvas_DstColorType);
  return Result::Ok();
}
...
Name ImageGenSrc::name() const { return SkOSPath::Basename(fPath.c_str()); }

/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/

ColorCodecSrc::ColorCodecSrc(Path path, bool decode_to_dst)
    : fPath(path), fDecodeToDst(decode_to_dst) {}
...
Result ColorCodecSrc::draw(GrDirectContext*, SkCanvas* canvas) const {
  ...
  switch (result) {
    case SkCodec::kSuccess:
    case SkCodec::kErrorInInput:
    case SkCodec::kIncompleteInput: canvas->drawImage(image, 0, 0); return Result::Ok();
    case SkCodec::kInvalidConversion:
      // TODO(mtklein): why are there formats we can't decode to?
      return Result::Skip("SkCodec can't decode to this format.");
    default: return Result::Fatal("Couldn't getPixels %s. Error code %d", fPath.c_str(), result);
  }
}
...
Name ColorCodecSrc::name() const { return SkOSPath::Basename(fPath.c_str()); }
...
Result BisectSrc::draw(GrDirectContext* context, SkCanvas* canvas) const {
  ...
  PathFindingCanvas pathFinder(
      canvas->getBaseLayerSize().width(), canvas->getBaseLayerSize().height());
...
#if defined(SK_ENABLE_SKOTTIE)
static DEFINE_bool(
    useLottieGlyphPaths, false, "Prioritize embedded glyph paths over native fonts.");
...
Result SkottieSrc::draw(GrDirectContext*, SkCanvas* canvas) const {
  ...
  auto precomp_interceptor = sk_make_sp<skottie_utils::ExternalAnimationPrecompInterceptor>(
      resource_provider, kInterceptPrefix);
  uint32_t flags = 0;
  ...
  for (int i = 0; i < kTileCount; ++i) {
    ...
    for (int j = 0; j < kTileCount; ++j) {
      ...
      const auto t = t_rate * (frameOrder[i] * kTileCount + frameOrder[j]);
      {
        ...
        canvas->concat(SkMatrix::RectToRect(
            SkRect::MakeSize(animation->size()), dest, SkMatrix::kCenter_ScaleToFit));
        animation->seek(t);
...
SkISize SkottieSrc::size() const { return SkISize::Make(kTargetSize, kTargetSize); }
...
bool SkottieSrc::veto(SinkFlags flags) const {
  ...
  bool type_ok = flags.type == SinkFlags::kRaster || flags.type == SinkFlags::kGPU ||
...
SVGSrc::SVGSrc(Path path) : fName(SkOSPath::Basename(path.c_str())), fScale(1) {
  ...
  if (sz.isEmpty()) {
    ...
    fScale = std::max(
        1.f,
        std::max(kMinimumSVGSize.width() / sz.width(), kMinimumSVGSize.height() / sz.height()));
  }
}
...
Result MSKPSrc::draw(int i, GrDirectContext*, SkCanvas* canvas) const {
  ...
  if (!page) {
    ...
    if (!SkMultiPictureDocumentRead(stream.get(), &fPages[0], fPages.count())) {
      return Result::Fatal("SkMultiPictureDocument reader failed on page %d: %s", i, fPath.c_str());
    }
...
static DEFINE_bool(
    preAbandonGpuContext, false, "Test abandoning the GrContext before running the test.");
static DEFINE_bool(
    abandonGpuContext, false, "Test abandoning the GrContext after running each test.");
static DEFINE_bool(
    releaseAndAbandonGpuContext, false,
...
GPUSink::GPUSink(const SkCommandLineConfigGpu* config, const GrContextOptions& grCtxOptions)
...
sk_sp<SkSurface> GPUSink::createDstSurface(GrDirectContext* context, SkISize size) const {
  ...
  switch (fSurfType) {
    case SkCommandLineConfigGpu::SurfType::kDefault:
      surface = SkSurface::MakeRenderTarget(context, SkBudgeted::kNo, info, fSampleCount, &props);
      break;
    case SkCommandLineConfigGpu::SurfType::kBackendTexture:
      surface = sk_gpu_test::MakeBackendTextureSurface(
          context, info, kTopLeft_GrSurfaceOrigin, fSampleCount, GrMipmapped::kNo, GrProtected::kNo,
          &props);
      break;
    case SkCommandLineConfigGpu::SurfType::kBackendRenderTarget:
      surface = sk_gpu_test::MakeBackendRenderTargetSurface(
          context, info, kBottomLeft_GrSurfaceOrigin, fSampleCount, GrProtected::kNo, &props);
      break;
  }
...
Result GPUSink::onDraw(
    const Src& src, SkBitmap* dst, SkWStream*, SkString* log, const GrContextOptions& baseOptions,
...
Result GPUSlugSink::draw(const Src& src, SkBitmap* dst, SkWStream* write, SkString* log) const {
  ...
  return onDraw(src, dst, write, log, grOptions, nullptr, [&](SkCanvas* canvas) {
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
GPUThreadTestingSink::GPUThreadTestingSink(
    const SkCommandLineConfigGpu* config, const GrContextOptions& grCtxOptions)
    : INHERITED(config, grCtxOptions), fExecutor(SkExecutor::MakeFIFOThreadPool(FLAGS_gpuThreads)) {
  SkASSERT(fExecutor);
}

Result GPUThreadTestingSink::draw(
    const Src& src, SkBitmap* dst, SkWStream* wStream, SkString* log) const {
...
GPUPersistentCacheTestingSink::GPUPersistentCacheTestingSink(
    const SkCommandLineConfigGpu* config, const GrContextOptions& grCtxOptions)
    : INHERITED(config, grCtxOptions), fCacheType(config->getTestPersistentCache()) {}

Result GPUPersistentCacheTestingSink::draw(
    const Src& src, SkBitmap* dst, SkWStream* wStream, SkString* log) const {
...
Result GPUPrecompileTestingSink::draw(
    const Src& src, SkBitmap* dst, SkWStream* wStream, SkString* log) const {
  // Three step process:
  ...
  auto precompileShaders = [&memoryCache](GrDirectContext* dContext) {
    memoryCache.foreach (
        [dContext](
            sk_sp<const SkData> key, sk_sp<SkData> data, const SkString& /*description*/,
            int /*count*/) { SkAssertResult(dContext->precompileShader(*key, *data)); });
  };
  ...
  SkDynamicMemoryWStream refStream;
  Result refResult =
      this->onDraw(src, &reference, &refStream, &refLog, replayOptions, precompileShaders);
  if (!refResult.isOk()) {
    return refResult;
  }
...
Result GPUOOPRSink::ooprDraw(
    const Src& src, sk_sp<SkSurface> dstSurface, GrDirectContext* context) const {
...
Result GPUDDLSink::ddlDraw(
    const Src& src, sk_sp<SkSurface> dstSurface, SkTaskGroup* recordingTaskGroup,
    SkTaskGroup* gpuTaskGroup, sk_gpu_test::TestContext* gpuTestCtx,
  ...
  Result result = src.draw(
      dContext, recorder.beginRecording(SkIntToScalar(size.width()), SkIntToScalar(size.height())));
  if (!result.isOk()) {
    return result;
  }
  ...
  DDLTileHelper tiles(
      dContext, dstCharacterization, viewport, kNumDivisions, kNumDivisions,
  ...
  gpuTaskGroup->add([dstSurface, ddl = tiles.composeDDL()]() { dstSurface->draw(ddl); });
...
Result GPUDDLSink::draw(const Src& src, SkBitmap* dst, SkWStream*, SkString* log) const {
  ...
  Result result =
      this->ddlDraw(src, surface, &recordingTaskGroup, &gpuTaskGroup, mainTestCtx, mainCtx);
  if (!result.isOk()) {
    return result;
  }
...
static SkTScopedComPtr<IXpsOMObjectFactory> make_xps_factory() {
  ...
  HRN(CoCreateInstance(
      CLSID_XpsOMObjectFactory, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&factory)));
...
Result SVGSink::draw(const Src& src, SkBitmap*, SkWStream* dst, SkString*) const {
  ...
  if (src.pageCount() > 1) {
    ...
    if (fPageIndex > pageCount - 1) {
      return Result::Fatal(
          "Page index %d too high for document with only %d pages.", fPageIndex, pageCount);
    }
  }
  return src.draw(
      fPageIndex, nullptr,
...
Result RasterSink::draw(const Src& src, SkBitmap* dst, SkWStream*, SkString*) const {
  ...
  dst->allocPixelsFlags(
      SkImageInfo::Make(size, fColorType, alphaType, fColorSpace), SkBitmap::kZeroPixels_AllocFlag);
...
Result GraphiteSink::draw(
    const Src& src, SkBitmap* dst, SkWStream* dstStream, SkString* log) const {
...

  {
    ...
    // that instead.
    SkPixmap pm;
    if (!dst->peekPixels(&pm) || !static_cast<skgpu::graphite::Surface*>(surface.get())
                                      ->onReadPixels(context, recorder.get(), pm, 0, 0)) {
      return Result::Fatal("Could not readback from surface.");
    }
  }
...
static Result draw_to_canvas(
    Sink* sink, SkBitmap* bitmap, SkWStream* stream, SkString* log, SkISize size,
...
Result ViaMatrix::draw(const Src& src, SkBitmap* bitmap, SkWStream* stream, SkString* log) const {
  ...
  return draw_to_canvas(
      fSink.get(), bitmap, stream, log, size, [&](GrDirectContext* context, SkCanvas* canvas) {
...
Result ViaSerialization::draw(
  ...
  SkPictureRecorder recorder;
  Result result = src.draw(
      nullptr, recorder.beginRecording(SkIntToScalar(size.width()), SkIntToScalar(size.height())));
  if (!result.isOk()) {
    return result;
  }
  ...
  result = draw_to_canvas(
      fSink.get(), bitmap, stream, log, size, [&](GrDirectContext*, SkCanvas* canvas) {
...
Result ViaPicture::draw(const Src& src, SkBitmap* bitmap, SkWStream* stream, SkString* log) const {
  auto size = src.size();
  Result result = draw_to_canvas(
      fSink.get(), bitmap, stream, log, size, [&](GrDirectContext* context, SkCanvas* canvas) {
        SkPictureRecorder recorder;
        sk_sp<SkPicture> pic;
        Result result = src.draw(
            context,
            recorder.beginRecording(SkIntToScalar(size.width()), SkIntToScalar(size.height())));
        if (!result.isOk()) {
          return result;
        }
...
Result ViaRuntimeBlend::draw(
  ...
  return draw_to_canvas(
      fSink.get(), bitmap, stream, log, src.size(),
...
Result ViaSVG::draw(const Src& src, SkBitmap* bitmap, SkWStream* stream, SkString* log) const {
  auto size = src.size();
  return draw_to_canvas(
      fSink.get(), bitmap, stream, log, size, [&](GrDirectContext*, SkCanvas* canvas) -> Result {
        SkDynamicMemoryWStream wstream;
