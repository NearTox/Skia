...
static DEFINE_string(
    src, "tests gm skp mskp lottie rive svg image colorImage", "Source types to test.");
static DEFINE_bool(
    nameByHash, false,
...
static DEFINE_string2(
    readPath, r, "", "If set check for equality with golden results in this directory.");
...
static DEFINE_bool(
    dryRun, false, "just print the tests that would be run, without actually running them.");
...
static DEFINE_string(
    key, "", "Space-separated key/value pairs to add to JSON identifying this builder.");
static DEFINE_string(
    properties, "", "Space-separated key/value pairs to add to JSON identifying this run.");

static DEFINE_bool(rasterize_pdf, false, "Rasterize PDFs when possible.");

static DEFINE_bool(
    runVerifiers, false, "if true, run SkQP-style verification of GM-produced images.");

#if defined(__MSVC_RUNTIME_CHECKS)
#  include <rtcapi.h>
int RuntimeCheckErrorFunc(
    int errorType, const char* filename, int linenumber, const char* moduleName, const char* fmt,
    ...) {
  va_list args;
  va_start(args, fmt);
  ...
  SkDebugf(
      "Line #%d\nFile: %s\nModule: %s\n", linenumber, filename ? filename : "Unknown",
...
static void done(const char* config, const char* src, const char* srcOptions, const char* name) {
  ...
  if (pending % 500 == 0) {
    ...
    int curr = sk_tools::getCurrResidentSetSizeMB(), peak = sk_tools::getMaxResidentSetSizeMB();

    SkAutoSpinlock lock(*gMutex);
    info(
        "\n%dMB RAM, %dMB peak, %d queued, %d active:\n", curr, peak, gPending - gRunning->count(),
        gRunning->count());
...
struct Gold : public SkString {
  Gold() : SkString("") {}
  Gold(
      const SkString& sink, const SkString& src, const SkString& srcOptions, const SkString& name,
      const SkString& md5)
      : SkString("") {
    this->append(sink);
    this->append(src);
  ...
  struct Hash {
    uint32_t operator()(const Gold& g) const { return SkGoodHash()((const SkString&)g); }
  };
};
...
static void push_codec_src(
    Path path, CodecSrc::Mode mode, CodecSrc::DstColorType dstColorType, SkAlphaType dstAlphaType,
    float scale) {
  ...
  SkString folder;
  switch (mode) {
    case CodecSrc::kCodec_Mode: folder.append("codec"); break;
    case CodecSrc::kCodecZeroInit_Mode: folder.append("codec_zero_init"); break;
    case CodecSrc::kScanline_Mode: folder.append("scanline"); break;
    case CodecSrc::kStripe_Mode: folder.append("stripe"); break;
    case CodecSrc::kCroppedScanline_Mode: folder.append("crop"); break;
    case CodecSrc::kSubset_Mode: folder.append("codec_subset"); break;
    case CodecSrc::kAnimated_Mode: folder.append("codec_animated"); break;
  }

  switch (dstColorType) {
    case CodecSrc::kGrayscale_Always_DstColorType: folder.append("_kGray8"); break;
    case CodecSrc::kNonNative8888_Always_DstColorType: folder.append("_kNonNative"); break;
    default: break;
  }

  switch (dstAlphaType) {
    case kPremul_SkAlphaType: folder.append("_premul"); break;
    case kUnpremul_SkAlphaType: folder.append("_unpremul"); break;
    default: break;
  }
...
static void push_image_gen_src(
    Path path, ImageGenSrc::Mode mode, SkAlphaType alphaType, bool isGpu) {
  SkString folder;
  switch (mode) {
    case ImageGenSrc::kCodec_Mode: folder.append("gen_codec"); break;
    case ImageGenSrc::kPlatform_Mode: folder.append("gen_platform"); break;
  }
  ...
  if (isGpu) {
    ...
    switch (alphaType) {
      case kOpaque_SkAlphaType: folder.append("_opaque"); break;
      case kPremul_SkAlphaType: folder.append("_premul"); break;
      case kUnpremul_SkAlphaType: folder.append("_unpremul"); break;
      default: break;
    }
...
static void push_brd_src(
    Path path, CodecSrc::DstColorType dstColorType, BRDSrc::Mode mode, uint32_t sampleSize) {
  SkString folder("brd_android_codec");
  switch (mode) {
    case BRDSrc::kFullImage_Mode: break;
    case BRDSrc::kDivisor_Mode: folder.append("_divisor"); break;
    default: SkASSERT(false); return;
  }

  switch (dstColorType) {
    case CodecSrc::kGetFromCanvas_DstColorType: break;
    case CodecSrc::kGrayscale_Always_DstColorType: folder.append("_kGray"); break;
    default: SkASSERT(false); return;
  }
...
static void push_codec_srcs(Path path) {
  ...
  switch (codec->getEncodedFormat()) {
    ...
    case SkEncodedImageFormat::kWEBP:
      ...
      supportsNativeScaling = true;
      break;
    case SkEncodedImageFormat::kDNG: break;
    default: nativeModes.push_back(CodecSrc::kScanline_Mode); break;
  }
  ...
  switch (codec->getInfo().colorType()) {
    case kGray_8_SkColorType: colorTypes.push_back(CodecSrc::kGrayscale_Always_DstColorType); break;
    default: break;
  }
...

  {
    ...
    if (frameInfos.size() > 1) {
      for (auto dstCT :
           {CodecSrc::kNonNative8888_Always_DstColorType, CodecSrc::kGetFromCanvas_DstColorType}) {
      ...
      for (float scale : {.5f, .33f}) {
        push_codec_src(
            path, CodecSrc::kAnimated_Mode, CodecSrc::kGetFromCanvas_DstColorType,
...
template <typename T>
void gather_file_srcs(
    const CommandLineFlags::StringArray& flags, const char* ext, const char* src_name = nullptr) {
  if (!src_name) {
...
static sk_sp<SkColorSpace> rgb_to_gbr() { return SkColorSpace::MakeSRGB()->makeColorSpin(); }
...
#define SINK(t, sink, ...) \
  ...
  if (FLAGS_cpu) {
    ...
    auto narrow = SkColorSpace::MakeRGB(SkNamedTransferFn::k2Dot2, gNarrow_toXYZD50),
         srgb = SkColorSpace::MakeSRGB(), srgbLinear = SkColorSpace::MakeSRGBLinear(),
...
static bool should_skip(
    const char* sink, const char* src, const char* srcOptions, const char* name) {
  for (int i = 0; i < FLAGS_skip.count() - 3; i += 4) {
    if (match(FLAGS_skip[i + 0], sink) && match(FLAGS_skip[i + 1], src) &&
        match(FLAGS_skip[i + 2], srcOptions) && match(FLAGS_skip[i + 3], name)) {
      return true;
    }
  }
  return false;
...
struct Task {
  ...
  static void Run(const Task& task) {
    ...
    if (!FLAGS_dryRun) {
      SkBitmap bitmap;
      SkDynamicMemoryWStream stream;
      start(task.sink.tag.c_str(), task.src.tag.c_str(), task.src.options.c_str(), name.c_str());
      Result result = task.sink->draw(*task.src, &bitmap, &stream, &log);
      if (!log.isEmpty()) {
        info(
            "%s %s %s %s:\n%s\n", task.sink.tag.c_str(), task.src.tag.c_str(),
            task.src.options.c_str(), name.c_str(), log.c_str());
      }
      if (result.isSkip()) {
      ...
      if (result.isFatal()) {
        fail(SkStringPrintf(
            ...
            task.src.options.c_str(), name.c_str(), result.c_str()));
      }
      ...
      gDefinitelyThreadSafeWork->add([task, name, bitmap, data] {
        ...
        if (!FLAGS_readPath.isEmpty() &&
            !gGold->contains(Gold(task.sink.tag, task.src.tag, task.src.options, name, md5))) {
          fail(SkStringPrintf(
              "%s not found for %s %s %s %s in %s", md5.c_str(), task.sink.tag.c_str(),
              task.src.tag.c_str(), task.src.options.c_str(), name.c_str(), FLAGS_readPath[0]));
        }
        ...
        if (ext && !FLAGS_writePath.isEmpty()) {
          ...
          if (FLAGS_rasterize_pdf && SkString("pdf").equals(ext)) {
            ...
            SkUniqueCFRef<CGDataProviderRef> provider{
                CGDataProviderCreateWithData(nullptr, blob->data(), blob->size(), nullptr)};

            SkUniqueCFRef<CGPDFDocumentRef> pdf{CGPDFDocumentCreateWithProvider(provider.get())};

            CGPDFPageRef page = CGPDFDocumentGetPage(pdf.get(), 1);

            CGRect bounds = CGPDFPageGetBoxRect(page, kCGPDFMediaBox);
            const int w = (int)CGRectGetWidth(bounds), h = (int)CGRectGetHeight(bounds);
    ...
    switch (classify_transfer_fn(tf)) {
      case sRGBish_TF:
        ...
        return SkStringPrintf(
            "%.3g %.3g %.3g %.3g %.3g %.3g %.3g", tf.g, tf.a, tf.b, tf.c, tf.d, tf.e, tf.f);
      ...
      case PQish_TF:
        ...
        return SkStringPrintf(
            "PQish %.3g %.3g %.3g %.3g %.3g %.3g", tf.a, tf.b, tf.c, tf.d, tf.e, tf.f);
      ...
      case HLGish_TF:
        ...
        return SkStringPrintf(
            "HLGish %.3g %.3g %.3g %.3g %.3g (%.3g)", tf.a, tf.b, tf.c, tf.d, tf.e, tf.f + 1);
  ...
  static void WriteToDisk(
      const Task& task, SkString md5, const char* ext, SkStream* data, size_t len,
      const SkBitmap* bitmap, const HashAndEncode* hashAndEncode) {
  ...
  static void RunGMVerifiers(const Task& task, const SkBitmap& actualBitmap) {
    ...
    skiagm::verifiers::VerifierResult res =
        verifierList->verifyAll(task.sink->colorInfo(), actualBitmap);
    if (!res.ok()) {
      fail(SkStringPrintf(
...
static void run_test(skiatest::Test test, const GrContextOptions& grCtxOptions) {
  struct : public skiatest::Reporter {
    void reportFailed(const skiatest::Failure& failure) override { fail(failure.toString()); }
    bool allowExtendedTest() const override { return FLAGS_pathOpsExtended; }
...
int main(int argc, char** argv) {
  ...
  info(
      "%d srcs * %d sinks + %d tests == %d tasks\n", gSrcs->count(), gSinks->count(),
      gParallelTests->count() + gSerialTests->count(), gPending);
  ...
  for (TaggedSink& sink : *gSinks) {
    for (TaggedSrc& src : *gSrcs) {
      if (src->veto(sink->flags()) ||
          should_skip(
              sink.tag.c_str(), src.tag.c_str(), src.options.c_str(), src->name().c_str())) {
        SkAutoSpinlock lock(*gMutex);
        gPending--;
        continue;
      }
  ...
  for (Task& task : serial) {
    Task::Run(task);
  }
  for (skiatest::Test& test : *gSerialTests) {
    run_test(test, grCtxOptions);
  }
