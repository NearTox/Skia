...
class Result {
 public:
  ...
  Result(Status status, const char* s, Args... args)
  ...
  template <typename... Args>
  static Result Fatal(const char* s, Args... args) {
  ...
  template <typename... Args>
  static Result Skip(const char* s, Args... args) {
...
struct Src {
  ...
  virtual Result SK_WARN_UNUSED_RESULT
  draw([[maybe_unused]] int page, GrDirectContext* context, SkCanvas* canvas) const {
  ...
  virtual std::unique_ptr<skiagm::verifiers::VerifierList> getVerifiers() const { return nullptr; }
};
...
struct Sink {
  ...
  virtual Result SK_WARN_UNUSED_RESULT
  draw(const Src&, SkBitmap*, SkWStream*, SkString* log) const = 0;
...
class GPUSink : public Sink {
 public:
  ...
  Result onDraw(
      const Src&, SkBitmap*, SkWStream*, SkString*, const GrContextOptions& baseOptions,
  ...
  SinkFlags flags() const override {
    SinkFlags::Multisampled ms =
        fSampleCount > 1 ? SinkFlags::kMultisampled : SinkFlags::kNotMultisampled;
...
class GPUDDLSink : public GPUSink {
 ...
 private:
  Result ddlDraw(
      const Src&, sk_sp<SkSurface> dstSurface, SkTaskGroup* recordingTaskGroup,
...
class Via : public Sink {
 public:
  ...
  void setColorSpace(sk_sp<SkColorSpace> colorSpace) override { fSink->setColorSpace(colorSpace); }
