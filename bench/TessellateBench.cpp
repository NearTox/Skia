...
DEF_PATH_TESS_BENCH(GrPathCurveTessellator, make_cubic_path(8), SkMatrix::I()) {
  ...
  GrPipeline noVaryingsPipeline(
      GrScissorTest::kDisabled, SkBlendMode::kSrcOver, skgpu::Swizzle::RGBA());
  auto tess = PathCurveTessellator::Make(&arena, fTarget->caps().shaderCaps()->fInfinitySupport);
  tess->prepare(
      fTarget.get(), fMatrix, {gAlmostIdentity, fPath, SK_PMColor4fTRANSPARENT},
      fPath.countVerbs());
}
...
DEF_PATH_TESS_BENCH(GrPathWedgeTessellator, make_cubic_path(8), SkMatrix::I()) {
  ...
  GrPipeline noVaryingsPipeline(
      GrScissorTest::kDisabled, SkBlendMode::kSrcOver, skgpu::Swizzle::RGBA());
  auto tess = PathWedgeTessellator::Make(&arena, fTarget->caps().shaderCaps()->fInfinitySupport);
  tess->prepare(
      fTarget.get(), fMatrix, {gAlmostIdentity, fPath, SK_PMColor4fTRANSPARENT},
      fPath.countVerbs());
}
...
DEF_PATH_TESS_BENCH(
  ...
  VertexWriter vertexWriter =
      fTarget->makeVertexWriter(sizeof(SkPoint), maxVerts, &buffer, &baseVertex);
...
class TessPrepareBench : public Benchmark {
 public:
  TessPrepareBench(
      MakePathStrokesFn makePathStrokesFn, PatchAttribs attribs, float matrixScale,
      const char* suffix)
      : fMakePathStrokesFn(makePathStrokesFn), fPatchAttribs(attribs), fMatrixScale(matrixScale) {
  ...
  void onDraw(int loops, SkCanvas*) final {
    for (int i = 0; i < loops; ++i) {
      fTessellator->prepare(
          fTarget.get(), SkMatrix::Scale(fMatrixScale, fMatrixScale), fPathStrokes.data(),
          fTotalVerbCount);
      fTarget->resetAllocator();
    }
  }
