...
class BenchmarkTarget : public GrMeshDrawOp::Target {
 public:
  ...
  void* makeVertexSpaceAtLeast(
      size_t vertexSize, int minVertexCount, int fallbackVertexCount, sk_sp<const GrBuffer>*,
...
#define UNIMPL(...) \
  __VA_ARGS__ override { SK_ABORT("unimplemented."); }
  UNIMPL(void recordDraw(
      const GrGeometryProcessor*, const GrSimpleMesh[], int, const GrSurfaceProxy* const[],
      GrPrimitiveType))
...
DEF_PATH_TESS_BENCH(
    prepareMiddleOutStencilGeometry_indirect, make_cubic_path(), SkMatrix::I(), target, op) {
...
DEF_PATH_TESS_BENCH(
    wangs_formula_cubic_log2_scale, make_cubic_path(), SkMatrix::Scale(1.1f, 0.9f), target, op) {
  benchmark_wangs_formula_cubic_log2(op->fViewMatrix, op->fPath);
}

DEF_PATH_TESS_BENCH(
    wangs_formula_cubic_log2_affine, make_cubic_path(),
...
DEF_PATH_TESS_BENCH(
    ...
    ToolUtils::make_star(SkRect::MakeWH(500, 500), kNumCubicsInChalkboard), SkMatrix::I(), target,
    op) {
  int baseVertex;
  auto vertexData = static_cast<SkPoint*>(
      target->makeVertexSpace(sizeof(SkPoint), kNumCubicsInChalkboard, nullptr, &baseVertex));
  ...
  for (auto [verb, pts, w] : SkPathPriv::Iterate(op->fPath)) {
    switch (verb) {
      ...
      case SkPathVerb::kCubic: SkUNREACHABLE;
    }
...
class StrokePatchBuilderBench : public Benchmark {
  ...
  void onDraw(int loops, SkCanvas*) final {
    ...
    for (int i = 0; i < loops; ++i) {
      fPatchChunks.reset();
      GrStrokePatchBuilder builder(&fTarget, &fPatchChunks, 1, fStrokeRec, fPath.countVerbs());
