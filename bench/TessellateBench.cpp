...
class GrStrokeTessellateOp::TestingOnly_Benchmark : public Benchmark {
 ...
 private:
  ...
  void onDraw(int loops, SkCanvas*) final {
    ...
    for (int i = 0; i < loops; ++i) {
      GrStrokeTessellateOp op(
          GrAAType::kMSAA, SkMatrix::Scale(fMatrixScale, fMatrixScale), fStrokeRec, fPath,
          GrPaint());
...
DEF_BENCH(return new StrokeIndirectBenchmark(
                     "_inflect2", SkPaint::kBevel_Join,
                     {{37, 162}, {412, 160}, {249, 65}, {112, 360}});)
 ...
 private:
  void setupPaths(SkTArray<SkPath>* paths) override {
    ...
    for (int i = 0; i < kNumCubicsInChalkboard; ++i) {
      switch (fVerb) {
        case SkPathVerb::kQuad:
          paths->push_back().quadTo(rand.nextF(), rand.nextF(), rand.nextF(), rand.nextF());
          break;
