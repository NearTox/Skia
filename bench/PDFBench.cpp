...
float next_common(SkRandom* random) { return random->nextRangeF(-500.0f, 1500.0f); }
...
class PDFCompressionBench : public Benchmark {
 ...
 protected:
  ...
  bool isSuitableFor(Backend backend) override { return backend == kNonRendering_Backend; }
  void onDelayedSetup() override { fAsset = GetResourceAsStream("pdf_command_stream.txt"); }
  void onDraw(int loops, SkCanvas*) override {
    SkASSERT(fAsset);
    if (!fAsset) {
      return;
...
struct PDFShaderBench : public Benchmark {
  ...
  void onDraw(int loops, SkCanvas*) final {
    ...
    while (loops-- > 0) {
      ...
      SkPDFDocument doc(&nullStream, SkPDF::Metadata());
      doc.beginPage(256, 256);
      (void)SkPDFMakeShader(&doc, fShader.get(), SkMatrix::I(), {0, 0, 400, 400}, SkColors::kBlack);
    }
  }
};
...
SkBitmap make_background() {
  ...
  SkPaint shader;
  shader.setShader(
      SkShader::MakeBitmapShader(bitmap, SkShader::kRepeat_TileMode, SkShader::kRepeat_TileMode));
