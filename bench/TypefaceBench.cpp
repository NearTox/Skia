...
class UtfToGlyph : public Benchmark {
 public:
  UtfToGlyph(SkTextEncoding encoding, const char*(*text), int lineCount, const char* name)
      : fEncoding{encoding}, fText{text}, fLineCount{lineCount}, fName{name} {}
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    ...
    for (int i = 0; i < loops * 3; ++i) {
      for (auto& line : fLines) {
        font.textToGlyphs(
            line->utf.data(), line->utf.size(), fEncoding, fGlyphIds.data(), line->glyphCount);
      }
    }
  }
...
DEF_BENCH(return new UtfToGlyph(
                     SkTextEncoding::kUTF32, ctext, SK_ARRAY_COUNT(ctext),
                     "SkTypefaceUTF32ToGlyphCN");)
DEF_BENCH(return new UtfToGlyph(
                     SkTextEncoding::kUTF16, ctext, SK_ARRAY_COUNT(ctext),
                     "SkTypefaceUTF16ToGlyphCN");)
DEF_BENCH(return new UtfToGlyph(
                     SkTextEncoding::kUTF8, ctext, SK_ARRAY_COUNT(ctext),
                     "SkTypefaceUTF8ToGlyphCN");)

DEF_BENCH(return new UtfToGlyph(
                     SkTextEncoding::kUTF32, atext, SK_ARRAY_COUNT(atext),
                     "SkTypefaceUTF32ToGlyphAscii");)
DEF_BENCH(return new UtfToGlyph(
                     SkTextEncoding::kUTF16, atext, SK_ARRAY_COUNT(atext),
                     "SkTypefaceUTF16ToGlyphAscii");)
DEF_BENCH(return new UtfToGlyph(
                     SkTextEncoding::kUTF8, atext, SK_ARRAY_COUNT(atext),
