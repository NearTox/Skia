...
class HardStopGradientBench_ScaleNumHardStops : public Benchmark {
 public:
  HardStopGradientBench_ScaleNumHardStops(int colorCount, int hardStopCount) {
    ...
    fName.printf(
        "hardstop_scale_num_hard_stops_%03d_colors_%03d_hard_stops", colorCount, hardStopCount);
  ...
  void onPreDraw(SkCanvas* canvas) override {
    ...
    fPaint.setShader(SkGradientShader::MakeLinear(
        points, colors.get(), positions.get(), fColorCount, SkTileMode::kClamp, 0, nullptr));
  }
