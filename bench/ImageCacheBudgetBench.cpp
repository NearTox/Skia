...
class ImageCacheBudgetBench : public Benchmark {
 public:
  ...
  ImageCacheBudgetBench(int budgetSize, bool shuffle)
      : fBudgetSize(budgetSize), fShuffle(shuffle), fIndices(nullptr) {
    ...
    // simulated frame.
    fName.printf("image_cache_budget_%.0f%s", imagesOverBudget * 100, (shuffle ? "_shuffle" : ""));
  }
...
class ImageCacheBudgetDynamicBench : public Benchmark {
 ...
 protected:
  const char* onGetName() override {
    switch (fMode) {
      case Mode::kPingPong: return "image_cache_budget_dynamic_ping_pong";
      case Mode::kFlipFlop: return "image_cache_budget_dynamic_flip_flop";
    }
    return "";
  }
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    ...
    switch (fMode) {
      ...
      case Mode::kFlipFlop: delta = kMaxImagesToDraw - kMinImagesToDraw; break;
    }
    for (int i = 0; i < loops; ++i) {
