...
class SkSLCompileBench : public Benchmark {
 public:
  ...
  SkSLCompileBench(std::string name, const char* src, bool optimize, Output output)
      : fName(
            std::string("sksl_") + (optimize ? "" : "unoptimized_") + output_string(output) + name),
        fSrc(src),
 ...
 protected:
  ...
  bool usesRuntimeShader() const { return fOutput >= Output::kSkVM; }
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    ...
    for (int i = 0; i < loops; i++) {
      std::unique_ptr<SkSL::Program> program = fCompiler.convertProgram(kind, fSrc, fSettings);
