...
class SkSLCompilerStartupBench : public Benchmark {
 protected:
  const char* onGetName() override { return "sksl_compiler_startup"; }
...
enum class Output { kNone, kGLSL, kMetal, kSPIRV };
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    for (int i = 0; i < loops; i++) {
      std::unique_ptr<SkSL::Program> program =
          fCompiler.convertProgram(SkSL::Program::kFragment_Kind, fSrc, fSettings);
...
class SkSLParseBench : public Benchmark {
 ...
 protected:
  ...
  void onDelayedSetup() override {
    SkSL::ParsedModule module = fCompiler.moduleForProgramKind(SkSL::Program::Kind::kFragment_Kind);
  ...
  void onDraw(int loops, SkCanvas*) override {
    for (int i = 0; i < loops; i++) {
      ...
      SkSL::Parser parser(
          fSrc.c_str(), fSrc.length(), *fCompiler.irGenerator().symbolTable(), fCompiler);
