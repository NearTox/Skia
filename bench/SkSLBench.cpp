  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    for (int i = 0; i < loops; i++) {
      ...
      std::unique_ptr<SkSL::Program> program =
          SkSL::DSLParser(&fCompiler, fSettings, SkSL::ProgramKind::kFragment, fSrc).program();
#else
      std::unique_ptr<SkSL::Program> program =
          fCompiler.convertProgram(SkSL::ProgramKind::kFragment, fSrc, fSettings);
 ...
 protected:
  ...
  void onDraw(int loops, SkCanvas*) override {
    for (int i = 0; i < loops; i++) {
      ...
      SkSL::Parser parser(fSrc, *fCompiler.irGenerator().symbolTable(), fCompiler.errorReporter());
