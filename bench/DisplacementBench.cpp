 ...
 protected:
  ...
  void makeBitmap() {
    ...
    canvas.drawSimpleText(
        "g", 1, SkTextEncoding::kUTF8, SkIntToScalar(15), SkIntToScalar(55), font, paint);
  }
  ...
  void drawClippedBitmap(SkCanvas* canvas, int x, int y, const SkPaint& paint) {
    ...
    canvas->clipRect(SkRect::MakeXYWH(
        SkIntToScalar(x), SkIntToScalar(y), SkIntToScalar(fBitmap.width()),
 ...
 protected:
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    ...
    paint.setImageFilter(SkDisplacementMapEffect::Make(
        ...
        SkDisplacementMapEffect::kG_ChannelSelectorType, 0.0f, std::move(displ), nullptr));
 ...
 protected:
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    ...
    paint.setImageFilter(SkDisplacementMapEffect::Make(
        ...
        SkDisplacementMapEffect::kA_ChannelSelectorType, 16.0f, std::move(displ), nullptr));
    for (int i = 0; i < loops; i++) {
...
class DisplacementFullBench : public DisplacementBaseBench {
 ...
 protected:
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    ...
    paint.setImageFilter(SkDisplacementMapEffect::Make(
        SkDisplacementMapEffect::kR_ChannelSelectorType,
        SkDisplacementMapEffect::kB_ChannelSelectorType, 32.0f, std::move(displ), nullptr));
    for (int i = 0; i < loops; ++i) {
