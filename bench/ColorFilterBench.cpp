...
class FilteredRectBench : public Benchmark {
 ...
 protected:
  ...
  void onDelayedSetup() override {
    switch (fType) {
      case kNoFilter_Type: break;
      case kColorFilter_Type: fPaint.setColorFilter(make_grayscale()); break;
...
class ColorMatrixBench final : public Benchmark {
 public:
  ...
  explicit ColorMatrixBench(const char* suffix, Factory f)
      : fFactory(f), fName(SkStringPrintf("colorfilter_%s", suffix)) {}

 private:
  const char* onGetName() override { return fName.c_str(); }

  SkIPoint onGetSize() override { return {256, 256}; }
...
static constexpr float gColorMatrix[] = {
    0.3f, 0.3f, 0.0f, 0.0f, 0.3f, 0.0f, 0.3f, 0.3f, 0.0f, 0.3f,
    0.0f, 0.0f, 0.3f, 0.3f, 0.3f, 0.3f, 0.0f, 0.3f, 0.3f, 0.0f,
};

}  // namespace

DEF_BENCH(return new ColorMatrixBench("none", []() { return sk_sp<SkColorFilter>(nullptr); });)
...
DEF_BENCH(return new ColorMatrixBench("lerp_src", []() {
                   return SkColorFilters::Lerp(
                       0.3f, SkColorFilters::Blend(0x80808080, SkBlendMode::kSrc),
                       SkColorFilters::Blend(0x80808080, SkBlendMode::kSrc));
                 });)

#ifdef SK_SUPPORT_GPU
DEF_BENCH(return new ColorMatrixBench("src_runtime", []() {
                   static SkRuntimeColorFilterFactory gRuntimeFact(
                       SkString(RuntimeNone_GPU_SRC), RuntimeNone_CPU);
                   return gRuntimeFact.make(
                       SkData::MakeWithCopy(gColorMatrix, sizeof(gColorMatrix)));
                 });)
DEF_BENCH(return new ColorMatrixBench("matrix_runtime", []() {
                   static SkRuntimeColorFilterFactory gRuntimeFact(
                       SkString(RuntimeColorMatrix_GPU_SRC), RuntimeColorMatrix_CPU);
                   return gRuntimeFact.make(
                       SkData::MakeWithCopy(gColorMatrix, sizeof(gColorMatrix)));
                 });)
#endif
