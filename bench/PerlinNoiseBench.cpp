...
class PerlinNoiseBench : public Benchmark {
 ...
 public:
  PerlinNoiseBench() { fSize = SkISize::Make(80, 80); }

 protected:
  const char* onGetName() override { return "perlinnoise"; }
 ...
 private:
  void drawClippedRect(SkCanvas* canvas, int x, int y, const SkPaint& paint) {
    ...
    canvas->clipRect(SkRect::MakeXYWH(
        SkIntToScalar(x), SkIntToScalar(y), SkIntToScalar(fSize.width()),
  ...
  void test(
      int loops, SkCanvas* canvas, int x, int y, float baseFrequencyX, float baseFrequencyY,
