...
class DirectMaskGlyphVertexFillBenchmark : public Benchmark {
  ...
  void onPerCanvasPreDraw(SkCanvas* canvas) override {
    ...
    SkGlyphRunListPainter painter{
        props, kUnknown_SkColorType, colorSpace.get(), SkStrikeCache::GlobalStrikeCache()};

    GrSDFTOptions options{256, 256};
    painter.processGlyphRunList(glyphRunList, view, props, false, options, fBlob.get());
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    ...
    for (int loop = 0; loop < loops; loop++) {
      subRun->fillVertexData(
          fVertices.get(), 0, subRun->glyphCount(), grColor, SkMatrix::I(), {100, 100}, clip);
    }
  }
