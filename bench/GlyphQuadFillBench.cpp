...
class SkTestCanvas<FillBench> {
 public:
  static SkBaseDevice* GetDevice(SkCanvas* canvas) { return canvas->topDevice(); }
};

class DirectMaskGlyphVertexFillBenchmark : public Benchmark {
  bool isSuitableFor(Backend backend) override { return backend == kGPU_Backend; }

  const char* onGetName() override { return "DirectMaskGlyphVertexFillBenchmark"; }
  ...
  void onPerCanvasPreDraw(SkCanvas* canvas) override {
    ...
    fBlob = sktext::gpu::TextBlob::Make(
        glyphRunList, paint, drawMatrix, device->strikeDeviceInfo(),
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    ...
    for (int loop = 0; loop < loops; loop++) {
      subRun->fillVertexData(
          fVertices.get(), 0, subRun->glyphCount(), grColor, positionMatrix, {0, 0}, clip);
    }
  }
