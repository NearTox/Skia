...
class GeometryBench : public Benchmark {
 public:
  GeometryBench(const char suffix[]) : fVolatileInt(0) { fName.printf("geo_%s", suffix); }
...
class ConvexityBench : public Benchmark {
 ...
 public:
  ConvexityBench(const char suffix[]) { fName.printf("convexity_%s", suffix); }
 ...
 protected:
  void onPreDraw(SkCanvas*) override { this->preparePath(&fPath); }
