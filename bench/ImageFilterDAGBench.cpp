...
class ImageFilterDAGBench : public Benchmark {
 ...
 protected:
  const char* onGetName() override { return "image_filter_dag"; }
...
class ImageMakeWithFilterDAGBench : public Benchmark {
 ...
 protected:
  const char* onGetName() override { return "image_make_with_filter_dag"; }

  void onDelayedSetup() override { fImage = GetResourceAsImage("images/mandrill_512.png"); }
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    ...
    for (int j = 0; j < loops; j++) {
      ...
      image = image->makeWithFilter(mergeFilter.get(), subset, subset, &discardSubset, &offset);
...
class ImageFilterDisplacedBlur : public Benchmark {
 ...
 protected:
  const char* onGetName() override { return "image_filter_displaced_blur"; }
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    for (int j = 0; j < loops; j++) {
      ...
      SkPaint paint;
      paint.setImageFilter(SkDisplacementMapEffect::Make(xSelector, ySelector, scale, blur, blur));
