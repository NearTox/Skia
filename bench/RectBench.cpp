...
class RectBench : public Benchmark {
 public:
  ...
  RectBench(int shift, int stroke = 0, bool aa = true, bool perspective = false)
      : fShift(shift), fStroke(stroke), fAA(aa), fPerspective(perspective) {}
 ...
 protected:
  virtual void drawThisRect(SkCanvas* c, const SkRect& r, const SkPaint& p) { c->drawRect(r, p); }
  ...
  void onDelayedSetup() override {
    ...
    for (int i = 0; i < N; i++) {
      ...
      x -= w / 2;
      y -= h / 2;
      fRects[i].setXYWH(SkIntToScalar(x), SkIntToScalar(y), SkIntToScalar(w), SkIntToScalar(h));
...
class SrcModeRectBench : public RectBench {
 public:
  SrcModeRectBench() : INHERITED(1, 0) { fMode = SkBlendMode::kSrc; }
...
class OvalBench : public RectBench {
 ...
 protected:
  void drawThisRect(SkCanvas* c, const SkRect& r, const SkPaint& p) override { c->drawOval(r, p); }
...
class PointsBench : public RectBench {
 public:
  ...
  PointsBench(SkCanvas::PointMode mode, const char* name) : RectBench(2), fMode(mode) {
    fName = name;
  }

 protected:
  void onDraw(int loops, SkCanvas* canvas) override {
    SkScalar gSizes[] = {SkIntToScalar(7), 0};
...
class BlitMaskBench : public RectBench {
 ...
 protected:
  void onDraw(int loops, SkCanvas* canvas) override {
    SkScalar gSizes[] = {SkIntToScalar(13), SkIntToScalar(24)};
    size_t sizes = SK_ARRAY_COUNT(gSizes);
 ...
 */
DEF_BENCH(return new BlitMaskBench(
                     SkCanvas::kPoints_PointMode, BlitMaskBench::kMaskOpaque, "maskopaque");)
DEF_BENCH(
    return new BlitMaskBench(SkCanvas::kPoints_PointMode, BlitMaskBench::kMaskBlack, "maskblack");)
DEF_BENCH(
    return new BlitMaskBench(SkCanvas::kPoints_PointMode, BlitMaskBench::kMaskColor, "maskcolor");)
DEF_BENCH(return new BlitMaskBench(
                     SkCanvas::kPoints_PointMode, BlitMaskBench::KMaskShader, "maskshader");)
