...
class BulkRectBench : public Benchmark {
 public:
  static_assert(
      kImageMode == ImageMode::kNone || kDrawMode != DrawMode::kQuad,
  ...
  inline static constexpr int kImageCount =
      kImageMode == ImageMode::kShared ? 1 : (kImageMode == ImageMode::kNone ? 0 : kRectCount);
 ...
 protected:
  ...
  void drawImagesBatch(SkCanvas* canvas) const {
    ...
    for (int i = 0; i < kRectCount; ++i) {
      ...
      batch[i].fSrcRect =
          SkRect::MakeIWH(fImages[imageIndex]->width(), fImages[imageIndex]->height());
    ...
    for (int i = 0; i < kRectCount; ++i) {
      if (kDrawMode == DrawMode::kRef) {
        ...
        canvas->experimental_DrawEdgeAAQuad(
            fRects[i], nullptr, SkCanvas::kAll_QuadAAFlags, fColors[i], SkBlendMode::kSrcOver);
      }
    }
  }
