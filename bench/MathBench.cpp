...
class MathBenchU32 : public MathBench {
 ...
 protected:
  virtual void performITest(
      uint32_t* SK_RESTRICT dst, const uint32_t* SK_RESTRICT src, int count) = 0;
...
class QMul64Bench : public MathBenchU32 {
 ...
 protected:
  void performITest(
      uint32_t* SK_RESTRICT dst, const uint32_t* SK_RESTRICT src, int count) override {
    for (int i = 0; i < count; ++i) {
...
static bool isFinite_float(float x) { return SkToBool(sk_float_isfinite(x)); }
...
static bool isfinite_and_int(const float data[4]) {
  return isFinite_int(data[0]) && isFinite_int(data[1]) && isFinite_int(data[2]) &&
         isFinite_int(data[3]);
}

static bool isfinite_and_float(const float data[4]) {
  return isFinite_float(data[0]) && isFinite_float(data[1]) && isFinite_float(data[2]) &&
         isFinite_float(data[3]);
}

static bool isfinite_and_mulzero(const float data[4]) {
  return isFinite_mulzero(data[0]) && isFinite_mulzero(data[1]) && isFinite_mulzero(data[2]) &&
         isFinite_mulzero(data[3]);
}

#define mulzeroadd(data) (data[0] * 0 + data[1] * 0 + data[2] * 0 + data[3] * 0)

static bool isfinite_plus_int(const float data[4]) { return isFinite_int(mulzeroadd(data)); }

static bool isfinite_plus_float(const float data[4]) { return !sk_float_isnan(mulzeroadd(data)); }
...
static const struct {
  ...
  const char* fName;
    ...
    MAKEREC(isfinite_and_int),  MAKEREC(isfinite_and_float),  MAKEREC(isfinite_and_mulzero),
    MAKEREC(isfinite_plus_int), MAKEREC(isfinite_plus_float), MAKEREC(isfinite_plus_mulzero),
};
...
class DivModBench : public Benchmark {
 ...
 public:
  explicit DivModBench(const char* name) { fName.printf("divmod_%s", name); }
