...
class GameBench : public Benchmark {
 public:
  ...
  GameBench(
      Type type, Clear clear, bool aligned = false, bool useAtlas = false,
    ...
    switch (fType) {
      ...
      case kRotate_Type: fName.append("_rot"); break;
    }
 ...
 protected:
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    ...
    p2.setShader(fAtlas.makeShader(SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kNone)));
    ...
    for (int i = 0; i < loops; ++i, ++fNumSaved) {
      ...
      if (fUseAtlas) {
        ...
        if (fUseDrawVertices) {
          ...
          canvas->drawVertices(
              SkVertices::MakeCopy(
                  SkVertices::kTriangles_VertexMode, 4, verts, uvs, nullptr, 6, indices),
              SkBlendMode::kModulate, p2);
        } else {
          canvas->drawBitmapRect(fAtlas, src, dst, &p, SkCanvas::kFast_SrcRectConstraint);
        }
      } else {
 ...
 private:
  ...
  static const int kTotAtlasWidth =
      kNumAtlasedX * kAtlasCellWidth + (kNumAtlasedX + 1) * kAtlasSpacer;
  static const int kTotAtlasHeight =
      kNumAtlasedY * kAtlasCellHeight + (kNumAtlasedY + 1) * kAtlasSpacer;
  ...
  void makeAtlas() {
    ...
    for (int y = 0; y < kNumAtlasedY; ++y) {
      for (int x = 0; x < kNumAtlasedX; ++x) {
        ...
        fAtlasRects[x][y] = SkIRect::MakeXYWH(
            kAtlasSpacer + x * (kAtlasCellWidth + kAtlasSpacer),
            kAtlasSpacer + y * (kAtlasCellHeight + kAtlasSpacer), kAtlasCellWidth,
            kAtlasCellHeight);
      }
    }
...
DEF_BENCH(
    return new GameBench(GameBench::kTranslate_Type, GameBench::kFull_Clear, false, true, true);)
