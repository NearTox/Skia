...
class ClearBench : public Benchmark {
 ...
 protected:
  const char* onGetName() override {
    switch (fType) {
      case kFull_ClearType: return "Clear-Full";
      case kPartial_ClearType: return "Clear-Partial";
      case kComplex_ClearType: return "Clear-Complex";
    }
    SkASSERT(false);
    return "Unreachable";
  }
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    ...
    for (int i = 0; i < loops; i++) {
      canvas->save();
      switch (fType) {
        case kPartial_ClearType: canvas->clipRect(kPartialClip); break;
        case kComplex_ClearType: canvas->clipRRect(kComplexClip); break;
