 ...
 protected:
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    ...
    auto verts = SkVertices::MakeCopy(
        SkVertices::kTriangles_VertexMode, PTS, fPts, nullptr, fColors, IDX, fIdx);
...
class AtlasBench : public Benchmark {
 ...
 public:
  AtlasBench(unsigned flags) : fFlags(flags) { fName.printf("drawAtlas_%d", flags); }
  ~AtlasBench() override {}
 ...
 protected:
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    ...
    for (int i = 0; i < loops; i++) {
      canvas->drawAtlas(
          atlas, fXforms, fRects, colors, N, SkBlendMode::kSrcOver, cullRect, paintPtr);
    }
  }
