...
class DiffCanvasBench : public Benchmark {
  ...
  void onDraw(int loops, SkCanvas* modelCanvas) override {
    ...
    std::unique_ptr<SkCanvas> canvas =
        fServer->makeAnalysisCanvas(1024, 1024, props, nullptr, true);
    loops *= 100;
    while (loops-- > 0) {
      for (const auto& record : fTrace) {
        canvas->drawTextBlob(record.blob.get(), record.offset.x(), record.offset.y(), record.paint);
      }
    }
  }
...
DEF_BENCH(return CreateDiffCanvasBench(SkString("SkDiffBench-lorem_ipsum"), []() {
