...
enum Flags { kStroke_Flag = 1 << 0, kBig_Flag = 1 << 1 };
...
class PathBench : public Benchmark {
 ...
 protected:
  const char* onGetName() override {
    fName.printf(
        "path_%s_%s_", fFlags & kStroke_Flag ? "stroke" : "fill",
...
class TrianglePathBench : public PathBench {
 public:
  ...
  void appendName(SkString* name) override { name->append("triangle"); }
  void makePath(SkPath* path) override {
    static const int gCoord[] = {10, 10, 15, 5, 20, 20};
...
class RectPathBench : public PathBench {
 public:
  ...
  void appendName(SkString* name) override { name->append("rect"); }
  void makePath(SkPath* path) override {
...
class OvalPathBench : public PathBench {
 public:
  ...
  void appendName(SkString* name) override { name->append("oval"); }
  void makePath(SkPath* path) override {
...
class CirclePathBench : public PathBench {
 public:
  ...
  void appendName(SkString* name) override { name->append("circle"); }
  void makePath(SkPath* path) override {
    path->addCircle(SkIntToScalar(20), SkIntToScalar(20), SkIntToScalar(10));
  }
...
class NonAACirclePathBench : public CirclePathBench {
 public:
  ...
  void appendName(SkString* name) override { name->append("nonaacircle"); }
...
class AAAConcavePathBench : public PathBench {
 public:
  ...
  void appendName(SkString* name) override { name->append("concave_aaa"); }
...
class AAAConvexPathBench : public PathBench {
 public:
  ...
  void appendName(SkString* name) override { name->append("convex_aaa"); }
...
class SawToothPathBench : public PathBench {
 public:
  ...
  void appendName(SkString* name) override { name->append("sawtooth"); }
  void makePath(SkPath* path) override {
...
class LongCurvedPathBench : public PathBench {
 public:
  ...
  void makePath(SkPath* path) override {
    ...
    for (i = 0; i < 100; i++) {
      path->quadTo(
          rand.nextUScalar1() * 640, rand.nextUScalar1() * 480, rand.nextUScalar1() * 640,
          rand.nextUScalar1() * 480);
    }
    path->close();
  }
...
class LongLinePathBench : public PathBench {
 public:
  ...
  void appendName(SkString* name) override { name->append("long_line"); }
  void makePath(SkPath* path) override {
    SkRandom rand;
 ...
 protected:
  void createData(int minVerbs, int maxVerbs, bool allowMoves = true, SkRect* bounds = nullptr) {
    ...
    for (int i = 0; i < kNumPoints; ++i) {
      fPoints[i].set(
          fRandom.nextRangeScalar(bounds->fLeft, bounds->fRight),
  ...
  void makePath(SkPath* path) {
    ...
    for (int v = 0; v < vCount; ++v) {
      ...
      switch (verb) {
        case SkPath::kMove_Verb: path->moveTo(fPoints[(fCurrPoint++) & (kNumPoints - 1)]); break;
        case SkPath::kLine_Verb: path->lineTo(fPoints[(fCurrPoint++) & (kNumPoints - 1)]); break;
        ...
        case SkPath::kConic_Verb:
          path->conicTo(
              fPoints[(fCurrPoint + 0) & (kNumPoints - 1)],
              fPoints[(fCurrPoint + 1) & (kNumPoints - 1)], SK_ScalarHalf);
          fCurrPoint += 2;
          break;
        ...
        default: SkDEBUGFAIL("Unexpected path verb"); break;
      }
    }
  }
...
class PathCreateBench : public RandomPathBench {
 ...
 protected:
  const char* onGetName() override { return "path_create"; }

  void onDelayedSetup() override { this->createData(10, 100); }
...
class PathCopyBench : public RandomPathBench {
 ...
 protected:
  const char* onGetName() override { return "path_copy"; }
...
class PathEqualityBench : public RandomPathBench {
 ...
 protected:
  const char* onGetName() override { return "path_equality_50%"; }
...
class SkBench_AddPathTest : public RandomPathBench {
 public:
  ...
  SkBench_AddPathTest(AddType type) : fType(type) { fMatrix.setRotate(60 * SK_Scalar1); }

 protected:
  const char* onGetName() override {
    switch (fType) {
      case kAdd_AddType: return "path_add_path";
      case kAddTrans_AddType: return "path_add_path_trans";
      case kAddMatrix_AddType: return "path_add_path_matrix";
      case kReverseAdd_AddType: return "path_reverse_add_path";
      case kReversePathTo_AddType: return "path_reverse_path_to";
      default: SkDEBUGFAIL("Bad add type"); return "";
    }
  }
...
class ArbRoundRectBench : public Benchmark {
 ...
 protected:
  ...
  static void make_arb_round_rect(
      SkPath* path, const SkRect& r, SkScalar xCorner, SkScalar yCorner) {
...
class ConicBench_EvalPos : public ConicBench_Chop {
 ...
 public:
  ConicBench_EvalPos(bool useV2) : fUseV2(useV2) { fName.printf("conic-eval-pos%d", useV2); }
  void onDraw(int loops, SkCanvas*) override {
    if (fUseV2) {
      for (int i = 0; i < loops; ++i) {
...
class ConicBench_EvalTan : public ConicBench_Chop {
 ...
 public:
  ConicBench_EvalTan(bool useV2) : fUseV2(useV2) { fName.printf("conic-eval-tan%d", useV2); }
  void onDraw(int loops, SkCanvas*) override {
    if (fUseV2) {
      for (int i = 0; i < loops; ++i) {
        for (int j = 0; j < 1000; ++j) {
...
class ConicBench_TinyError : public Benchmark {
 ...
 protected:
  ...
  void onDraw(int loops, SkCanvas*) override {
    ...
    const SkMatrix mtx = SkMatrix::MakeAll(
...
class ConicBench_ComputeError : public ConicBench {
 ...
 protected:
  const char* onGetName() override { return "conic-compute-error"; }
...
class ConicBench_asQuadTol : public ConicBench {
 ...
 protected:
  const char* onGetName() override { return "conic-asQuadTol"; }
...
class ConicBench_quadPow2 : public ConicBench {
 ...
 protected:
  const char* onGetName() override { return "conic-quadPow2"; }
...
const SkRect ConservativelyContainsBench::kBounds =
...
const SkRect ConservativelyContainsBench::kBaseRect =
    SkRect::MakeXYWH(SkIntToScalar(25), SkIntToScalar(25), SkIntToScalar(50), SkIntToScalar(50));
...
DEF_BENCH(return new TightBoundsBench(
                     [](const SkPath& path) { return path.computeTightBounds(); }, "priv");)
DEF_BENCH(return new TightBoundsBench(
                     [](const SkPath& path) {
                       SkRect bounds;
