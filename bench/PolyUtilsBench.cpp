...
class PolyUtilsBench : public Benchmark {
 ...
 protected:
  const char* onGetName() override {
    ...
    switch (fType) {
      case Type::kConvexCheck: fName.append("_c"); break;
      case Type::kSimpleCheck: fName.append("_s"); break;
      case Type::kInsetConvex: fName.append("_i"); break;
      case Type::kOffsetSimple: fName.append("_o"); break;
      case Type::kTessellateSimple: fName.append("_t"); break;
    }
    return fName.c_str();
  }
    ...
    switch (fType) {
      ...
      case Type::kOffsetSimple:
        if (SkIsSimplePolygon(poly.begin(), poly.count())) {
          ...
          for (int i = 0; i < loops; i++) {
            (void)SkOffsetSimplePolygon(poly.begin(), poly.count(), bounds, 10, &result);
            (void)SkOffsetSimplePolygon(poly.begin(), poly.count(), bounds, -10, &result);
          }
        }
        break;
      case Type::kTessellateSimple:
        if (SkIsSimplePolygon(poly.begin(), poly.count())) {
          ...
          for (int i = 0; i < loops; i++) {
            SkTriangulateSimplePolygon(poly.begin(), indexMap, poly.count(), &triangleIndices);
          }
        }
        break;
    }
...
class StarPolyUtilsBench : public PolyUtilsBench {
 public:
  ...
  void appendName(SkString* name) override { name->append("star"); }
...
class IntersectingPolyUtilsBench : public PolyUtilsBench {
 public:
  ...
  void appendName(SkString* name) override { name->append("intersecting"); }
  void makePoly(SkTDArray<SkPoint>* poly) override {
...
class NotchPolyUtilsBench : public PolyUtilsBench {
 public:
  ...
  void appendName(SkString* name) override { name->append("notch"); }
  void makePoly(SkTDArray<SkPoint>* poly) override {
...
class IceCreamPolyUtilsBench : public PolyUtilsBench {
 public:
  ...
  void appendName(SkString* name) override { name->append("icecream"); }
  void makePoly(SkTDArray<SkPoint>* poly) override {
    // create 3/4 circle with many vertices
    const SkScalar c = SkIntToScalar(45);
    const SkScalar r = SkIntToScalar(20);
