...
static DEFINE_string(
    intersectionTreeFile, "", "svg or skp for the IntersectionTree bench to sniff paths from.");
  ...
  void gatherRects(SkTArray<SkRect>* rects) override {
    ...
    for (int i = 0; i < fNumRandomRects; ++i) {
      rects->push_back(SkRect::MakeXYWH(
          rand.nextRangeF(0, 2000), rand.nextRangeF(0, 2000), rand.nextRangeF(0, 70),
...
class FileIntersectionBench : public IntersectionTreeBench {
 ...
 private:
  ...
  void gatherRects(SkTArray<SkRect>* rects) override {
    ...
    ToolUtils::sniff_paths(
        FLAGS_intersectionTreeFile[0],
        [&](const SkMatrix& matrix, const SkPath& path, const SkPaint& paint) {
