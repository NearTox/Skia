...
class BitmapRectBench : public Benchmark {
 ...
 protected:
  const char* onGetName() override {
    fName.printf(
        "bitmaprect_%02X_%sfilter_%s", fAlpha, kNone_SkFilterQuality == fFilterQuality ? "no" : "",
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    ...
    for (int i = 0; i < loops; i++) {
      canvas->drawBitmapRect(fBitmap, fSrcR, fDstR, &paint, SkCanvas::kStrict_SrcRectConstraint);
    }
  }
