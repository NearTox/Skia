...
enum Flags { kBig_Flag = 1 << 0, kAA_Flag = 1 << 1 };
...
static const int points[] = {10, 10, 15, 5,  20, 20, 30, 5,  25, 20, 15, 12,
                             21, 21, 30, 30, 12, 4,  32, 28, 20, 18, 12, 10};
...
class HairlinePathBench : public Benchmark {
 ...
 protected:
  const char* onGetName() override {
    fName.printf(
        "path_hairline_%s_%s_", fFlags & kBig_Flag ? "big" : "small",
...
class LinePathBench : public HairlinePathBench {
 public:
  ...
  void appendName(SkString* name) override { name->append("line"); }
  void makePath(SkPath* path) override {
    ...
    for (int i = 0; i < kMaxPathSize; ++i) {
      ...
      path->moveTo(
          SkIntToScalar(points[base1] + xTrans), SkIntToScalar(points[base1 + 1] + yTrans));
      path->lineTo(
          SkIntToScalar(points[base2] + xTrans), SkIntToScalar(points[base2 + 1] + yTrans));
      path->lineTo(
          SkIntToScalar(points[base3] + xTrans), SkIntToScalar(points[base3 + 1] + yTrans));
    }
  }
...
class QuadPathBench : public HairlinePathBench {
 public:
  ...
  void appendName(SkString* name) override { name->append("quad"); }
  void makePath(SkPath* path) override {
    SkRandom rand;
    int size = SK_ARRAY_COUNT(points);
    int hSize = size / 2;
...
class ConicPathBench : public HairlinePathBench {
 public:
  ...
  void appendName(SkString* name) override { name->append("conic"); }
  void makePath(SkPath* path) override {
    SkRandom rand;
    SkRandom randWeight;
    int size = SK_ARRAY_COUNT(points);
    ...
    for (int i = 0; i < kMaxPathSize; ++i) {
      ...
      path->conicTo(
          SkIntToScalar(points[base2] + xTrans), SkIntToScalar(points[base2 + 1] + yTrans),
          SkIntToScalar(points[base3] + xTrans), SkIntToScalar(points[base3 + 1] + yTrans), weight);
    }
  }
...
class CubicPathBench : public HairlinePathBench {
 public:
  ...
  void appendName(SkString* name) override { name->append("cubic"); }
  void makePath(SkPath* path) override {
    SkRandom rand;
    int size = SK_ARRAY_COUNT(points);
    int hSize = size / 2;
    for (int i = 0; i < kMaxPathSize; ++i) {
      ...
      path->cubicTo(
          SkIntToScalar(points[base2] + xTrans), SkIntToScalar(points[base2 + 1] + yTrans),
          SkIntToScalar(points[base3] + xTrans), SkIntToScalar(points[base3 + 1] + yTrans),
          SkIntToScalar(points[base4] + xTrans), SkIntToScalar(points[base4 + 1] + yTrans));
    }
  }
