...
class DashBench : public Benchmark {
 ...
 public:
  ...
  virtual void makePath(SkPath* path) { path_hline(path); }
...
class RectDashBench : public DashBench {
 ...
 protected:
  virtual void handlePath(
      SkCanvas* canvas, const SkPath& path, const SkPaint& paint, int N) override {
    SkPoint pts[2];
...
static void make_quad(SkPath* path) {
  ...
  path->moveTo(x0, y0);
  path->quadTo(x0, y0 + 400 * SK_Scalar1, x0 + 600 * SK_Scalar1, y0 + 400 * SK_Scalar1);
}
...
static void make_cubic(SkPath* path) {
  ...
  path->moveTo(x0, y0);
  path->cubicTo(
      x0, y0 + 400 * SK_Scalar1, x0 + 600 * SK_Scalar1, y0 + 400 * SK_Scalar1,
...
class DashLineBench : public Benchmark {
 ...
 protected:
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    ...
    for (int i = 0; i < loops; ++i) {
      canvas->drawLine(10 * SK_Scalar1, 10 * SK_Scalar1, 640 * SK_Scalar1, 10 * SK_Scalar1, paint);
    }
  }
...
class DrawPointsDashingBench : public Benchmark {
 ...
 protected:
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    ...
    SkPoint pts[2] = {{SkIntToScalar(10), 0}, {SkIntToScalar(640), 0}};
...
class GiantDashBench : public Benchmark {
 ...
 public:
  ...
  GiantDashBench(LineType lt, SkScalar width) {
    ...
    switch (lt) {
      case kHori_LineType: matrix.setIdentity(); break;
      case kVert_LineType: matrix.setRotate(90, cx, cy); break;
      case kDiag_LineType: matrix.setRotate(45, cx, cy); break;
      case kLineTypeCount:
    ...
    const SkPoint pts[2] = {{-overshoot, cy}, {640 + overshoot, cy}};
