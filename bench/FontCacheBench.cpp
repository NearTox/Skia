...
class FontCacheBench : public Benchmark {
 ...
 protected:
  const char* onGetName() override { return "fontcache"; }
...
class FontCacheEfficiency : public Benchmark {
 ...
 protected:
  const char* onGetName() override { return "fontefficiency"; }
      ...
      for (int limit = 32; limit <= 1024; limit <<= 1) {
        for (size_t i = 0; i < SK_ARRAY_COUNT(gRec); ++i) {
          ...
          SkDebugf(
              ...
              collisions, glyphs, collisions * 100.0 / glyphs, gRec[i].fName);
        }
      }
    }
  }
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    ...
    for (int loop = 0; loop < loops; ++loop) {
      if (fOneAtATime) {
        ...
        fFont.getPaths(
            fGlyphs, SK_ARRAY_COUNT(fGlyphs),
