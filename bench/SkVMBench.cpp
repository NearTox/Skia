...
class SkVMBench : public Benchmark {
 ...
 private:
  ...
  void onDelayedSetup() override {
    ...
    if (fMode == F32) {
      fProgram = SrcoverBuilder_F32{}.done();
    }
    if (fMode == I32_Naive) {
  ...
  Mode fMode;
  SkString fName;
  std::vector<uint32_t> fSrc, fDst;
  skvm::Program fProgram;

  SkRasterPipeline_MemoryCtx fSrcCtx, fDstCtx;
  ...
  void onDraw(int loops, SkCanvas*) override {
    ...
    if (fRP) {
      while (loops-- > 0) {
        ...
        SkRasterPipeline_MemoryCtx src = {&dummy, 0}, dst = {&dummy, 0};
