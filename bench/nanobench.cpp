    ...
    while (fCurrentScale < fScales.count()) {
      while (fCurrentSKP < fSKPs.count()) {
        ...
        if (FLAGS_bbh) {
          ...
          SkPictureRecorder recorder;
          pic->playback(
              recorder.beginRecording(pic->cullRect().width(), pic->cullRect().height(), &factory));
          pic = recorder.finishRecordingAsPicture();
        }
        SkString name = SkOSPath::Basename(path.c_str());
        fSourceType = "skp";
        fBenchType = "playback";
        return new SKPBench(name.c_str(), pic.get(), fClip, fScales[fCurrentScale], FLAGS_loopSKP);
      }
      ...
      while (fCurrentSVG < fSVGs.count()) {
        ...
        if (sk_sp<SkPicture> pic = ReadSVGPicture(path)) {
          ...
          fBenchType = "playback";
          return new SKPBench(
              SkOSPath::Basename(path).c_str(), pic.get(), fClip, fScales[fCurrentScale],
              FLAGS_loopSKP);
        }
      }
