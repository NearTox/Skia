...
static DEFINE_double(
    overheadGoal, 0.0001, "Loop until timer overhead is at most this fraction of our measurments.");
static DEFINE_double(gpuMs, 5, "Target bench time in millseconds for GPU.");
static DEFINE_int(
    gpuFrameLag, 5, "If unknown, estimated maximum number of frames GPU allows to lag.");
...
static DEFINE_string(
    sourceType, "", "Apply usual --match rules to source type: bench, gm, skp, image, etc.");
static DEFINE_string(
    benchType, "",
...
static DEFINE_bool(
    simpleCodec, false, "Runs of a subset of the codec tests, always N32, Premul or Opaque");
...
static DEFINE_bool(
    purgeBetweenBenches, false, "Call SkGraphics::PurgeAllCaches() between each benchmark?");
...
struct GPUTarget : public Target {
  ...
  bool init(SkImageInfo info, Benchmark* bench) override {
    ...
    this->surface = SkSurface::MakeRenderTarget(
        this->factory->get(this->config.ctxType, this->config.ctxOverrides), SkBudgeted::kNo, info,
  ...
  void fillOptions(NanoJSONResultsWriter& log) override {
    ...
    if (this->contextInfo.backend() == GrBackendApi::kOpenGL) {
      const GrGLInterface* gl =
          static_cast<GrGLGpu*>(this->contextInfo.directContext()->priv().getGpu())->glInterface();
    ...
    while (bench_plus_overhead < overhead) {
      if (round++ == FLAGS_maxCalibrationAttempts) {
        SkDebugf(
            ...
            HUMANIZE(bench_plus_overhead), HUMANIZE(overhead));
        return kFailedLoops;
      }
...
#define CPU_CONFIG(name, backend, color, alpha, colorSpace)                                  \
  if (config->getTag().equals(#name)) {                                                      \
    ...
    Config config = {SkString(#name),   Benchmark::backend,     color, alpha, colorSpace, 0, \
                     kBogusContextType, kBogusContextOverrides, false};                      \
    configs->push_back(config);                                                              \
    return;                                                                                  \
  }

  CPU_CONFIG(
      nonrendering, kNonRendering_Backend, kUnknown_SkColorType, kUnpremul_SkAlphaType, nullptr)
...
static Target* is_enabled(Benchmark* bench, const Config& config) {
  ...
  SkImageInfo info = SkImageInfo::Make(
      bench->getSize().fX, bench->getSize().fY, config.color, config.alpha, config.colorSpace);

  Target* target = nullptr;

  switch (config.backend) {
    case Benchmark::kGPU_Backend: target = new GPUTarget(config); break;
    default: target = new Target(config); break;
  }
...
static bool valid_brd_bench(
  ...
  if (sampleSize * minOutputSize > (uint32_t)brd->width() ||
      sampleSize * minOutputSize > (uint32_t)brd->height()) {
...
static void collect_files(
    const CommandLineFlags::StringArray& paths, const char* ext, SkTArray<SkString>* list) {
...
class BenchmarkStream {
 public:
  ...
  static sk_sp<SkPicture> ReadSVGPicture(const char* path) {
    ...
    svgDom->render(
        recorder.beginRecording(svgDom->containerSize().width(), svgDom->containerSize().height()));
  ...
  Benchmark* rawNext() {
    ...
    while (fCurrentScale < fScales.count()) {
      while (fCurrentSKP < fSKPs.count()) {
        ...
        while (fCurrentUseMPD < fUseMPDs.count()) {
          if (FLAGS_bbh) {
            ...
            SkPictureRecorder recorder;
            pic->playback(recorder.beginRecording(
                pic->cullRect().width(), pic->cullRect().height(), &factory, 0));
          ...
          fBenchType = "playback";
          return new SKPBench(
              name.c_str(), pic.get(), fClip, fScales[fCurrentScale], fUseMPDs[fCurrentUseMPD++],
              FLAGS_loopSKP);
        }
        fCurrentUseMPD = 0;
        fCurrentSKP++;
      ...
      while (fCurrentSVG++ < fSVGs.count()) {
        ...
        if (sk_sp<SkPicture> pic = ReadSVGPicture(path)) {
          ...
          fBenchType = "playback";
          return new SKPBench(
              SkOSPath::Basename(path).c_str(), pic.get(), fClip, fScales[fCurrentScale], false,
              FLAGS_loopSKP);
        }
      }
    ...
    if (fZoomMax != 1.0f && fZoomPeriodMs > 0) {
      while (fCurrentAnimSKP < fSKPs.count()) {
        ...
        return new SKPAnimationBench(
            name.c_str(), pic.get(), fClip, std::move(animation), FLAGS_loopSKP);
      }
    }
    ...
    for (; fCurrentCodec < fImages.count(); fCurrentCodec++) {
      ...
      while (fCurrentColorType < fColorTypes.count()) {
        ...
        const SkCodec::Result result = codec->getPixels(info, storage.get(), rowBytes);
        switch (result) {
          case SkCodec::kSuccess:
          case SkCodec::kIncompleteInput:
            return new CodecBench(
                SkOSPath::Basename(path.c_str()), encoded.get(), colorType, alphaType);
    ...
    for (; fCurrentAndroidCodec < fImages.count(); fCurrentAndroidCodec++) {
      ...
      while (fCurrentSampleSize < (int)SK_ARRAY_COUNT(sampleSizes)) {
        ...
        return new AndroidCodecBench(SkOSPath::Basename(path.c_str()), encoded.get(), sampleSize);
      }
      ...
      while (fCurrentColorType < fColorTypes.count()) {
        while (fCurrentSampleSize < (int)SK_ARRAY_COUNT(brdSampleSizes)) {
          while (fCurrentSubsetType <= kLastSingle_SubsetType) {
            ...
            int width = 0;
            int height = 0;
            if (!valid_brd_bench(encoded, colorType, sampleSize, minOutputSize, &width, &height)) {
              break;
            }
            ...
            switch (currentSubsetType) {
              ...
              case kTopRight_SubsetType:
                ...
                subset = SkIRect::MakeXYWH(width - subsetSize, 0, subsetSize, subsetSize);
                break;
              case kMiddle_SubsetType:
                basename.append("_Middle");
                subset = SkIRect::MakeXYWH(
                    (width - subsetSize) / 2, (height - subsetSize) / 2, subsetSize, subsetSize);
                break;
              case kBottomLeft_SubsetType:
                basename.append("_BottomLeft");
                subset = SkIRect::MakeXYWH(0, height - subsetSize, subsetSize, subsetSize);
                break;
              case kBottomRight_SubsetType:
                basename.append("_BottomRight");
                subset = SkIRect::MakeXYWH(
                    width - subsetSize, height - subsetSize, subsetSize, subsetSize);
                break;
              default: SkASSERT(false);
            }

            return new BitmapRegionDecoderBench(
                basename.c_str(), encoded.get(), colorType, sampleSize, subset);
          }
  ...
  void fillCurrentOptions(NanoJSONResultsWriter& log) const {
    ...
    if (0 == strcmp(fSourceType, "skp")) {
      log.appendString(
          "clip",
          SkStringPrintf("%d %d %d %d", fClip.fLeft, fClip.fTop, fClip.fRight, fClip.fBottom)
              .c_str());
      ...
      if (fCurrentUseMPD > 0) {
        ...
        log.appendString("multi_picture_draw", fUseMPDs[fCurrentUseMPD - 1] ? "true" : "false");
      }
    }
  }
...
int main(int argc, char** argv) {
  ...
  while (Benchmark* b = benchStream.next()) {
    ...
    for (int i = 0; i < configs.count(); ++i) {
      ...
      TRACE_EVENT2(
          "skia", "Benchmark", "name", TRACE_STR_COPY(bench->getUniqueName()), "config",
          TRACE_STR_COPY(config));
      ...
      if (kAutoTuneLoops != FLAGS_loops) {
        ...
        SkDebugf(
            ...
            sk_tools::getMaxResidentSetSizeMB(), bench->getUniqueName(), config);
      } else if (FLAGS_quiet) {
        const char* mark = " ";
        ...
        SkDebugf(
            ...
            stddev_percent, config, bench->getUniqueName());
      } else {
        const char* format = "%4d/%-4dMB\t%d\t%s\t%s\t%s\t%s\t%.0f%%\t%s\t%s\t%s\n";
        const double stddev_percent = sk_ieee_double_divide(100 * sqrt(stats.var), stats.mean);
        SkDebugf(
            format, sk_tools::getCurrResidentSetSizeMB(), sk_tools::getMaxResidentSetSizeMB(),
            loops, HUMANIZE(stats.min), HUMANIZE(stats.median), HUMANIZE(stats.mean),
            HUMANIZE(stats.max), stddev_percent,
            FLAGS_ms ? to_string(samples.count()).c_str() : stats.plot.c_str(), config,
            bench->getUniqueName());
      }
