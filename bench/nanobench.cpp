...
#ifdef SK_GRAPHITE_ENABLED
    case Benchmark::kGraphite_Backend: target = new GraphiteTarget(config); break;
#endif
  ...
  while (Benchmark* b = benchStream.next()) {
    ...
    for (int i = 0; i < configs.count(); ++i) {
      ...
      if (kAutoTuneLoops != FLAGS_loops) {
        ...
        SkDebugf(
            "%4d/%-4dMB\t%s\t%s ", sk_tools::getCurrResidentSetSizeMB(),
        ...
        SkDebugf(
            ...
            sk_tools::getCurrResidentSetSizeMB(), sk_tools::getMaxResidentSetSizeMB(), loops,
            HUMANIZE(stats.min), HUMANIZE(stats.median), HUMANIZE(stats.mean), HUMANIZE(stats.max),
            stddev_percent, FLAGS_ms ? to_string(samples.count()).c_str() : stats.plot.c_str(),
            config, bench->getUniqueName());
      }
