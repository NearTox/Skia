  ...
  GP(Mode mode, sk_sp<GrColorSpaceXform> colorSpaceXform)
    ...
    switch (fMode) {
      ...
      case kShader_Mode:
        ...
        break;
      case kFloat_Mode: fInColor = {"inColor", kFloat4_GrVertexAttribType, kHalf4_GrSLType}; break;
      case kHalf_Mode: fInColor = {"inColor", kHalf4_GrVertexAttribType, kHalf4_GrSLType}; break;
    }
...
class Op : public GrMeshDrawOp {
 ...
 private:
  ...
  void onPrepareDraws(GrMeshDrawTarget* target) override {
    ...
    int firstVertex = 0;
    void* verts = target->makeVertexSpace(vertexStride, kVertexCount, &vertexBuffer, &firstVertex);
    if (!verts) {
      return;
    }
    ...
    if (kFloat_Mode == fMode) {
      ...
      color = (uint64_t)halfColor[0] << 48 | (uint64_t)halfColor[1] << 32 |
              (uint64_t)halfColor[2] << 16 | (uint64_t)halfColor[3] << 0;
      V* v = (V*)verts;
      for (int i = 0; i < kVertexCount; i += 2) {
        v[i + 0].fPos.set(dx * i, 0.0f);
        v[i + 0].fColor = color;
...
class VertexColorSpaceBench : public Benchmark {
 public:
  ...
  void onDraw(int loops, SkCanvas* canvas) override {
    ...
    if (kHalf_Mode == fMode && !context->priv().caps()->halfFloatVertexAttributeSupport()) {
      return;
    }
