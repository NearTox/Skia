...
class BlurRectDirectBench : public BlurRectBench {
 ...
 protected:
  void makeBlurryRect(const SkRect& r) override {
    SkMask mask;
    if (!SkBlurMask::BlurRect(
            SkBlurMask::ConvertRadiusToSigma(this->radius()), &mask, r, kNormal_SkBlurStyle)) {
      return;
    }
...
class BlurRectSeparableBench : public BlurRectBench {
 public:
  ...
  ~BlurRectSeparableBench() override { SkMask::FreeImage(fSrcMask.fImage); }
...
class BlurRectBoxFilterBench : public BlurRectSeparableBench {
 ...
 protected:
  void makeBlurryRect(const SkRect&) override {
    SkMask mask;
    if (!SkBlurMask::BoxBlur(
            &mask, fSrcMask, SkBlurMask::ConvertRadiusToSigma(this->radius()),
            kNormal_SkBlurStyle)) {
      return;
    }
    SkMask::FreeImage(mask.fImage);
...
class BlurRectGaussianBench : public BlurRectSeparableBench {
 ...
 protected:
  void makeBlurryRect(const SkRect&) override {
    SkMask mask;
    if (!SkBlurMask::BlurGroundTruth(
            SkBlurMask::ConvertRadiusToSigma(this->radius()), &mask, fSrcMask,
            kNormal_SkBlurStyle)) {
      return;
    }
    SkMask::FreeImage(mask.fImage);
