...
static void LoadKnownDigest(std::string md5) { gKnownDigests.insert(md5); }
...
static JSObject RunGM(sk_sp<GrDirectContext> ctx, std::string name) {
  ...
  sk_sp<SkSurface> surface(SkSurface::MakeRenderTarget(
      ctx.get(), SkBudgeted::kYes, info, 0, kBottomLeft_GrSurfaceOrigin, nullptr, true));
  if (!surface) {
...
bool IsMockContextType(ContextType ct) { return ct == ContextType::kMock_ContextType; }
...
void RunWithGPUTestContexts(
    GrContextTestFn* test, GrContextTypeFilterFn* contextTypeFilter, Reporter* reporter,
...
class WasmWebGlTestContext : public sk_gpu_test::GLTestContext {
 public:
  ...
  ~WasmWebGlTestContext() override { this->teardown(); }
  ...
  GrGLFuncPtr onPlatformGetProcAddress(const char* procName) const override { return nullptr; }
};
}  // namespace
...
TestHarness CurrentTestHarness() { return TestHarness::kWasmGMTests; }
