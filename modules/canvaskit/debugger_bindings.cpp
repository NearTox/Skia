...
class SkpDebugPlayer {
 public:
  ...
  const SkIRect getBounds() { return getBoundsForFrame(fp); }
  ...
  void deleteCommand(int index) { visibleCanvas()->deleteDrawCommandAt(index); }
  ...
  void changeFrame(int index) { fp = index; }
  ...
  int getImageCount() { return fImages.size(); }
  ...
  void setInspectedLayer(int nodeId) { fInspectedLayer = nodeId; }
 ...
 private:
  ...
  std::string loadMultiFrame(SkMemoryStream* stream) {
    ...
    if (!page_count) {
      ...
      return "Not a MultiPictureDocument, MultiPictureDocument file version too old, or "
             "MultiPictureDocument contained 0 frames.";
    }
  ...
  // frame sizes are known to change in Android Skia RenderEngine because it interleves pictures
...
EMSCRIPTEN_BINDINGS(my_module) {
  ...
  class_<SkpDebugPlayer>("SkpDebugPlayer")
      ...
      .function(
          "imageUseInfoForFrameJs",
          optional_override([](SkpDebugPlayer& self, const int frame) -> JSObject {
