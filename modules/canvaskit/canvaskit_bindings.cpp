...
SkImageInfo toSkImageInfo(const SimpleImageInfo& sii) {
  return SkImageInfo::Make(
      sii.width, sii.height, sii.colorType, sii.alphaType,
...
sk_sp<SkSurface> MakeOnScreenGLSurface(
    sk_sp<GrDirectContext> dContext, int width, int height, sk_sp<SkColorSpace> colorSpace) {
  ...
  sk_sp<SkSurface> surface(SkSurface::MakeFromBackendRenderTarget(
      dContext.get(), target, kBottomLeft_GrSurfaceOrigin, colorSettings.colorType, colorSpace,
      nullptr));
  return surface;
}
...
sk_sp<SkSurface> MakeRenderTarget(sk_sp<GrDirectContext> dContext, int width, int height) {
  ...
  sk_sp<SkSurface> surface(SkSurface::MakeRenderTarget(
      dContext.get(), SkBudgeted::kYes, info, 0, kBottomLeft_GrSurfaceOrigin, nullptr, true));
  return surface;
}

sk_sp<SkSurface> MakeRenderTarget(sk_sp<GrDirectContext> dContext, SimpleImageInfo sii) {
  sk_sp<SkSurface> surface(SkSurface::MakeRenderTarget(
      dContext.get(), SkBudgeted::kYes, toSkImageInfo(sii), 0, kBottomLeft_GrSurfaceOrigin, nullptr,
      true));
  return surface;
}
#endif
...
void ApplyAddPath(
    SkPath& orig, const SkPath& newPath, SkScalar scaleX, SkScalar skewX, SkScalar transX,
    SkScalar skewY, SkScalar scaleY, SkScalar transY, SkScalar pers0, SkScalar pers1,
    SkScalar pers2, bool extendPath) {
  SkMatrix m = SkMatrix::MakeAll(scaleX, skewX, transX, skewY, scaleY, transY, pers0, pers1, pers2);
...
void ApplyArcToArcSize(
    SkPath& orig, SkScalar rx, SkScalar ry, SkScalar xAxisRotate, bool useSmallArc, bool ccw,
...
void ApplyRCubicTo(
    SkPath& p, SkScalar dx1, SkScalar dy1, SkScalar dx2, SkScalar dy2, SkScalar dx3, SkScalar dy3) {
...
void ApplyTransform(
    SkPath& orig, SkScalar scaleX, SkScalar skewX, SkScalar transX, SkScalar skewY, SkScalar scaleY,
...
Float32Array ToCmds(const SkPath& path) {
  ...
  for (auto [verb, pts, w] : SkPathPriv::Iterate(path)) {
    switch (verb) {
      case SkPathVerb::kMove: cmds.insert(cmds.end(), {MOVE, pts[0].x(), pts[0].y()}); break;
      case SkPathVerb::kLine: cmds.insert(cmds.end(), {LINE, pts[1].x(), pts[1].y()}); break;
      case SkPathVerb::kQuad:
        cmds.insert(cmds.end(), {QUAD, pts[1].x(), pts[1].y(), pts[2].x(), pts[2].y()});
        break;
      case SkPathVerb::kConic:
        cmds.insert(cmds.end(), {CONIC, pts[1].x(), pts[1].y(), pts[2].x(), pts[2].y(), *w});
        break;
      case SkPathVerb::kCubic:
        cmds.insert(
            cmds.end(),
            {CUBIC, pts[1].x(), pts[1].y(), pts[2].x(), pts[2].y(), pts[3].x(), pts[3].y()});
        break;
      case SkPathVerb::kClose: cmds.push_back(CLOSE); break;
    }
  }
...
void PathAddVerbsPointsWeights(
    SkPath& path, WASMPointerU8 verbsPtr, int numVerbs, WASMPointerF32 ptsPtr, int numPts,
  ...
  for (int v = 0; v < numVerbs; ++v) {
    switch (verbs[v]) {
      ...
      case CONIC:
        CHECK_NUM_POINTS(4);
        CHECK_NUM_WEIGHTS(1);
        path.conicTo(pts[ptIdx], pts[ptIdx + 1], pts[ptIdx + 2], pts[ptIdx + 3], weights[wtIdx]);
        ptIdx += 4;
        wtIdx++;
        break;
      case CUBIC:
        CHECK_NUM_POINTS(6);
        path.cubicTo(
            pts[ptIdx], pts[ptIdx + 1], pts[ptIdx + 2], pts[ptIdx + 3], pts[ptIdx + 4],
            pts[ptIdx + 5]);
        ptIdx += 6;
        break;
      case CLOSE: path.close(); break;
...
SkPath MakePathFromVerbsPointsWeights(
    WASMPointerU8 verbsPtr, int numVerbs, WASMPointerF32 ptsPtr, int numPts, WASMPointerF32 wtsPtr,
    int numWts) {
  SkPath path;
...
class WebGLTextureImageGenerator : public SkImageGenerator {
 public:
  WebGLTextureImageGenerator(SkImageInfo ii, JSObject callbackObj)
      : SkImageGenerator(ii), fCallback(callbackObj) {}
 ...
 protected:
  GrSurfaceProxyView onGenerateTexture(
    ...
    sk_sp<GrSurfaceProxy> proxy = ctx->priv().proxyProvider()->wrapBackendTexture(
        backendTexture, kBorrow_GrWrapOwnership, GrWrapCacheable::kYes, kRead_GrIOType,
...
EMSCRIPTEN_BINDINGS(Skia) {
  ...
  function(
      ...
      allow_raw_pointers());
  function(
      "_decodeImage", optional_override([](WASMPointerU8 iptr, size_t length) -> sk_sp<SkImage> {
        uint8_t* imgData = reinterpret_cast<uint8_t*>(iptr);
  ...
  function(
      "_MakeImage",
      optional_override(
          [](SimpleImageInfo ii, WASMPointerU8 pPtr, int plen, size_t rowBytes) -> sk_sp<SkImage> {
  ...
  function(
      "_getShadowLocalBounds",
      optional_override(
          [](WASMPointerF32 ctmPtr, const SkPath& path, WASMPointerF32 zPlaneParamPtr,
             WASMPointerF32 lightPosPtr, SkScalar lightRadius, uint32_t flags,
  ...
  function(
      "_MakePicture", optional_override([](WASMPointerU8 dPtr, size_t bytes) -> sk_sp<SkPicture> {
  ...
  class_<GrDirectContext>("GrDirectContext")
      ...
      .function(
          "getResourceCacheLimitBytes", optional_override([](GrDirectContext& self) -> size_t {
      ...
      .function(
          "getResourceCacheUsageBytes", optional_override([](GrDirectContext& self) -> size_t {
      ...
      .function(
          "releaseResourcesAndAbandonContext", &GrDirectContext::releaseResourcesAndAbandonContext)
      .function(
  ...
  class_<SkCanvas>("Canvas")
      ...
      .function(
          "clipPath", select_overload<void(const SkPath&, SkClipOp, bool)>(&SkCanvas::clipPath))
      .function(
          "_clipRRect",
          optional_override([](SkCanvas& self, WASMPointerF32 fPtr, SkClipOp op, bool doAntiAlias) {
      ...
      .function(
          "_drawArc",
          optional_override([](SkCanvas& self, WASMPointerF32 fPtr, SkScalar startAngle,
      ...
      .function(
          "_drawAtlasOptions",
          optional_override(
              [](SkCanvas& self, const sk_sp<SkImage>& atlas, WASMPointerF32 xptr,
                 WASMPointerF32 rptr, WASMPointerU32 cptr, int count, SkBlendMode mode,
                 SkFilterMode filter, SkMipmapMode mipmap, const SkPaint* paint) -> void {
                ...
                self.drawAtlas(
                    atlas.get(), dstXforms, srcRects, colors, count, mode, sampling, nullptr,
                    paint);
              }),
          allow_raw_pointers())
      .function(
          ...
          optional_override(
              [](SkCanvas& self, const sk_sp<SkImage>& atlas, WASMPointerF32 xptr,
                 WASMPointerF32 rptr, WASMPointerU32 cptr, int count, SkBlendMode mode, float B,
                 float C, const SkPaint* paint) -> void {
                const SkRSXform* dstXforms = reinterpret_cast<const SkRSXform*>(xptr);
                const SkRect* srcRects = reinterpret_cast<const SkRect*>(rptr);
                const SkColor* colors = nullptr;
                if (cptr) {
          ...
          allow_raw_pointers())
      .function(
          "_drawCircle", select_overload<void(SkScalar, SkScalar, SkScalar, const SkPaint& paint)>(
                             &SkCanvas::drawCircle))
      ...
      .function(
          "_drawColorInt", optional_override([](SkCanvas& self, SkColor color, SkBlendMode mode) {
            self.drawColor(color, mode);
          }))
      .function(
          "_drawDRRect", optional_override([](SkCanvas& self, WASMPointerF32 outerPtr,
      ...
      .function(
          "_drawGlyphs",
          optional_override(
              [](SkCanvas& self, int count, WASMPointerU16 glyphs, WASMPointerF32 positions,
                 float x, float y, const SkFont& font, const SkPaint& paint) -> void {
                self.drawGlyphs(
                    count, reinterpret_cast<const uint16_t*>(glyphs),
                    reinterpret_cast<const SkPoint*>(positions), {x, y}, font, paint);
              }))
      // TODO: deprecate this version, and require sampling
      .function(
          "_drawImage",
          optional_override([](SkCanvas& self, const sk_sp<SkImage>& image, SkScalar x, SkScalar y,
                               const SkPaint* paint) {
      ...
      .function(
          "_drawImageCubic",
          optional_override(
              [](SkCanvas& self, const sk_sp<SkImage>& img, SkScalar left, SkScalar top, float B,
      ...
      .function(
          "_drawImageOptions",
          optional_override(
              [](SkCanvas& self, const sk_sp<SkImage>& img, SkScalar left, SkScalar top,
                 SkFilterMode filter, SkMipmapMode mipmap, const SkPaint* paint) -> void {
      ...
      .function(
          "_drawImageNine",
          optional_override(
              [](SkCanvas& self, const sk_sp<SkImage>& image, WASMPointerU32 centerPtr,
                 WASMPointerF32 dstPtr, SkFilterMode filter, const SkPaint* paint) -> void {
      ...
      .function(
          "_drawImageRect",
          optional_override(
              [](SkCanvas& self, const sk_sp<SkImage>& image, WASMPointerF32 srcPtr,
                 WASMPointerF32 dstPtr, const SkPaint* paint, bool fastSample) -> void {
                const SkRect* src = reinterpret_cast<const SkRect*>(srcPtr);
                const SkRect* dst = reinterpret_cast<const SkRect*>(dstPtr);
                self.drawImageRect(
                    image, *src, *dst, SkSamplingOptions(), paint,
      ...
      .function(
          ...
          optional_override(
              [](SkCanvas& self, const sk_sp<SkImage>& image, WASMPointerF32 srcPtr,
                 WASMPointerF32 dstPtr, float B, float C,  // See SkSamplingOptions.h for docs.
                 const SkPaint* paint) -> void {
                const SkRect* src = reinterpret_cast<const SkRect*>(srcPtr);
                const SkRect* dst = reinterpret_cast<const SkRect*>(dstPtr);
                auto constraint = SkCanvas::kStrict_SrcRectConstraint;  // TODO: get from caller
                self.drawImageRect(
                    image.get(), *src, *dst, SkSamplingOptions({B, C}), paint, constraint);
              }),
          allow_raw_pointers())
      .function(
          "_drawImageRectOptions",
          optional_override(
              [](SkCanvas& self, const sk_sp<SkImage>& image, WASMPointerF32 srcPtr,
                 WASMPointerF32 dstPtr, SkFilterMode filter, SkMipmapMode mipmap,
                 const SkPaint* paint) -> void {
                const SkRect* src = reinterpret_cast<const SkRect*>(srcPtr);
                const SkRect* dst = reinterpret_cast<const SkRect*>(dstPtr);
                auto constraint = SkCanvas::kStrict_SrcRectConstraint;  // TODO: get from caller
      ...
      .function(
          "_drawOval",
          optional_override([](SkCanvas& self, WASMPointerF32 fPtr, const SkPaint& paint) -> void {
            const SkRect* oval = reinterpret_cast<const SkRect*>(fPtr);
      ...
      .function(
          "_drawParagraph",
          optional_override([](SkCanvas& self, skia::textlayout::Paragraph* p, SkScalar x,
      ...
      .function(
          "_drawPatch",
          optional_override(
              [](SkCanvas& self, WASMPointerF32 cubics, WASMPointerU32 colors, WASMPointerF32 texs,
                 SkBlendMode mode, const SkPaint& paint) -> void {
                self.drawPatch(
                    reinterpret_cast<const SkPoint*>(cubics),
                    reinterpret_cast<const SkColor*>(colors),
                    reinterpret_cast<const SkPoint*>(texs), mode, paint);
              }))
      ...
      .function(
          "_drawPicture", select_overload<void(const sk_sp<SkPicture>&)>(&SkCanvas::drawPicture))
      .function(
          "_drawPoints", optional_override(
                             [](SkCanvas& self, SkCanvas::PointMode mode, WASMPointerF32 pptr,
      ...
      .function(
          "_drawRRect",
          optional_override([](SkCanvas& self, WASMPointerF32 fPtr, const SkPaint& paint) {
      ...
      .function(
          "_drawRect4f", optional_override(
                             [](SkCanvas& self, SkScalar left, SkScalar top, SkScalar right,
                                SkScalar bottom, const SkPaint& paint) -> void {
                               const SkRect rect = SkRect::MakeLTRB(left, top, right, bottom);
                               self.drawRect(rect, paint);
                             }))
      .function(
          "_drawShadow",
          optional_override([](SkCanvas& self, const SkPath& path, WASMPointerF32 zPlaneParamPtr,
                               WASMPointerF32 lightPosPtr, SkScalar lightRadius,
                               WASMPointerF32 ambientColorPtr, WASMPointerF32 spotColorPtr,
                               uint32_t flags) {
            const SkVector3* zPlaneParams = reinterpret_cast<const SkVector3*>(zPlaneParamPtr);
            const SkVector3* lightPos = reinterpret_cast<const SkVector3*>(lightPosPtr);

            SkShadowUtils::DrawShadow(
                &self, path, *zPlaneParams, *lightPos, lightRadius,
                ptrToSkColor4f(ambientColorPtr).toSkColor(),
                ptrToSkColor4f(spotColorPtr).toSkColor(), flags);
          }))
#ifndef CK_NO_FONTS
      .function(
          "_drawSimpleText",
          optional_override([](SkCanvas& self, WASMPointerU8 sptr, size_t len, SkScalar x,
      ...
      .function(
          "_drawTextBlob",
          select_overload<void(const sk_sp<SkTextBlob>&, SkScalar, SkScalar, const SkPaint&)>(
      ...
      .function(
          "_getDeviceClipBounds", optional_override([](const SkCanvas& self, WASMPointerI32 iPtr) {
      ...
      .function(
          "_getLocalToDevice", optional_override([](const SkCanvas& self, WASMPointerF32 mPtr) {
      ...
      .function(
          "_readPixels",
          optional_override([](SkCanvas& self, SimpleImageInfo di, WASMPointerU8 pPtr,
      ...
      .function(
          "_saveLayer",
          optional_override(
              [](SkCanvas& self, const SkPaint* p, WASMPointerF32 fPtr,
  ...
  class_<SkColorFilter>("ColorFilter")
      ...
      .class_function(
          "_MakeBlend",
          optional_override([](WASMPointerF32 cPtr, SkBlendMode mode) -> sk_sp<SkColorFilter> {
  ...
  class_<SkContourMeasure>("ContourMeasure")
      ...
      .function(
          "_getPosTan",
          optional_override(
              [](SkContourMeasure& self, SkScalar distance, WASMPointerF32 oPtr) -> void {
      ...
      .function(
          "getSegment", optional_override(
                            [](SkContourMeasure& self, SkScalar startD, SkScalar stopD,
  ...
  class_<SkFont>("Font")
      ...
      .function(
          "_getGlyphWidthBounds",
          optional_override([](SkFont& self, WASMPointerU16 gPtr, int numGlyphs,
                               WASMPointerF32 wPtr, WASMPointerF32 rPtr, SkPaint* paint) {
          ...
          allow_raw_pointers())
      .function(
          "_getGlyphIDs", optional_override(
                              [](SkFont& self, WASMPointerU8 sptr, size_t strLen,
                                 size_t expectedCodePoints, WASMPointerU16 iPtr) -> int {
                                ...
                                int actualCodePoints = self.textToGlyphs(
                                    str, strLen, SkTextEncoding::kUTF8, glyphIDs,
                                    expectedCodePoints);
      ...
      .function(
          "_getGlyphIntercepts",
          optional_override(
              [](SkFont& self, WASMPointerU16 gPtr, size_t numGlyphs, bool ownGlyphs,
                 WASMPointerF32 pPtr, size_t numPos, bool ownPos, float top,
                ...
                auto sects = self.getIntercepts(
                    glyphs.data(), SkToInt(glyphs.size()), (const SkPoint*)pos.data(), top, bottom);
  ...
  class_<SkFontMgr>("FontMgr")
      ...
      .class_function(
          "_fromData",
          optional_override(
              [](WASMPointerU32 dPtr, WASMPointerU32 sPtr, int numFonts) -> sk_sp<SkFontMgr> {
...
#  endif
      .function(
          ...
          optional_override([](SkFontMgr& self, WASMPointerU8 fPtr, int flen) -> sk_sp<SkTypeface> {
  ...
  class_<SkImage>("Image")
      ...
      .function(
          "getColorSpace", optional_override([](sk_sp<SkImage> self) -> sk_sp<SkColorSpace> {
            return self->imageInfo().refColorSpace();
          }),
          allow_raw_pointers())
      .function("getImageInfo", optional_override([](sk_sp<SkImage> self) -> JSObject {
                  // We cannot return a SimpleImageInfo because the colorspace object would be
                  // leaked.
      ...
      .function(
          "encodeToBytes",
          optional_override(
              [](sk_sp<SkImage> self, SkEncodedImageFormat fmt, int quality) -> Uint8Array {
      ...
      .function(
          ...
          optional_override(
              [](sk_sp<SkImage> self) -> sk_sp<SkImage> { return self->withDefaultMipmaps(); }))
      .function(
          "_makeShaderCubic",
          optional_override(
              [](sk_sp<SkImage> self, SkTileMode tx, SkTileMode ty, float B,
                 float C,  // See SkSamplingOptions.h for docs.
      ...
      .function(
          "_makeShaderOptions",
          optional_override(
              [](sk_sp<SkImage> self, SkTileMode tx, SkTileMode ty, SkFilterMode filter,
                 SkMipmapMode mipmap, WASMPointerF32 mPtr) -> sk_sp<SkShader> {
      ...
      .function(
          "_readPixels",
          optional_override(
              [](sk_sp<SkImage> self, SimpleImageInfo sii, WASMPointerU8 pPtr, size_t dstRowBytes,
                 int srcX, int srcY) -> bool {
                uint8_t* pixels = reinterpret_cast<uint8_t*>(pPtr);
  ...
  class_<SkImageFilter>("ImageFilter")
      ...
      .class_function(
          "MakeColorFilter",
          optional_override(
              [](sk_sp<SkColorFilter> cf, sk_sp<SkImageFilter> input) -> sk_sp<SkImageFilter> {
      ...
      .class_function(
          ...
          optional_override(
              [](WASMPointerF32 mPtr, float B, float C,
                 sk_sp<SkImageFilter> input) -> sk_sp<SkImageFilter> {
      ...
      .class_function(
          ...
          optional_override(
              [](WASMPointerF32 mPtr, SkFilterMode fm, SkMipmapMode mm,
                 sk_sp<SkImageFilter> input) -> sk_sp<SkImageFilter> {
                OptionalMatrix matr(mPtr);
  ...
  class_<SkMaskFilter>("MaskFilter")
      ...
      .class_function(
          "MakeBlur",
          optional_override(
              [](SkBlurStyle style, SkScalar sigma, bool respectCTM) -> sk_sp<SkMaskFilter> {
  ...
  class_<SkPaint>("Paint")
      ...
      .function(
          "_setColor",
          optional_override([](SkPaint& self, WASMPointerF32 cPtr, sk_sp<SkColorSpace> colorSpace) {
      ...
      .function(
          "setColorInt",
          optional_override([](SkPaint& self, SkColor color, sk_sp<SkColorSpace> colorSpace) {
  ...
  class_<SkColorSpace>("ColorSpace")
      ...
      .class_function(
          "Equals", optional_override([](sk_sp<SkColorSpace> a, sk_sp<SkColorSpace> b) -> bool {
  ...
  class_<SkPathEffect>("PathEffect")
      ...
      .class_function(
          "_MakeDash",
          optional_override(
              [](WASMPointerF32 cptr, int count, SkScalar phase) -> sk_sp<SkPathEffect> {
      ...
      .class_function(
          "_MakeLine2D",
          optional_override([](SkScalar width, WASMPointerF32 mPtr) -> sk_sp<SkPathEffect> {
            SkMatrix matrix;
            const SkScalar* nineMatrixValues = reinterpret_cast<const SkScalar*>(mPtr);
  ...
  class_<SkPath>("Path")
      ...
      .function(
          "_addArc", optional_override(
                         [](SkPath& self, WASMPointerF32 fPtr, SkScalar startAngle,
      ...
      .function(
          "_addOval", optional_override(
                          [](SkPath& self, WASMPointerF32 fPtr, bool ccw, unsigned start) -> void {
                            const SkRect* oval = reinterpret_cast<const SkRect*>(fPtr);
                            self.addOval(
      ...
      .function(
          "_addPoly",
          optional_override([](SkPath& self, WASMPointerF32 fPtr, int count, bool close) -> void {
            const SkPoint* pts = reinterpret_cast<const SkPoint*>(fPtr);
            self.addPoly(pts, count, close);
          }))
      .function(
          "_addRect", optional_override([](SkPath& self, WASMPointerF32 fPtr, bool ccw) -> void {
            const SkRect* rect = reinterpret_cast<const SkRect*>(fPtr);
            self.addRect(*rect, ccw ? SkPathDirection::kCCW : SkPathDirection::kCW);
          }))
      .function(
          "_addRRect", optional_override([](SkPath& self, WASMPointerF32 fPtr, bool ccw) -> void {
      ...
      .function(
          "_getPoint", optional_override([](SkPath& self, int index, WASMPointerF32 oPtr) -> void {
      ...
      .function(
          "_transform", select_overload<void(
                            SkPath&, SkScalar, SkScalar, SkScalar, SkScalar, SkScalar, SkScalar,
                            SkScalar, SkScalar, SkScalar)>(&ApplyTransform))
      ...
      .function("_getBounds", optional_override([](SkPath& self, WASMPointerF32 fPtr) -> void {
                  SkRect* output = reinterpret_cast<SkRect*>(fPtr);
                  output[0] = self.getBounds();
                }))
      .function(
          "_computeTightBounds", optional_override([](SkPath& self, WASMPointerF32 fPtr) -> void {
            SkRect* output = reinterpret_cast<SkRect*>(fPtr);
  ...
  class_<SkPictureRecorder>("PictureRecorder")
      ...
      .function(
          "_beginRecording",
          optional_override([](SkPictureRecorder& self, WASMPointerF32 fPtr) -> SkCanvas* {
            SkRect* bounds = reinterpret_cast<SkRect*>(fPtr);
  ...
  class_<SkPicture>("Picture")
      ...
      .function(
          "_makeShader",
          optional_override(
              [](SkPicture& self, SkTileMode tmx, SkTileMode tmy, SkFilterMode mode,
      ...
      .function(
          "serialize", optional_override([](SkPicture& self) -> Uint8Array {
  ...
  class_<SkShader>("Shader")
      ...
      .class_function(
          "_MakeColor",
          optional_override(
              [](WASMPointerF32 cPtr, sk_sp<SkColorSpace> colorSpace) -> sk_sp<SkShader> {
                return SkShaders::Color(ptrToSkColor4f(cPtr), colorSpace);
              }))
      .class_function(
          "MakeFractalNoise", optional_override(
                                  [](SkScalar baseFreqX, SkScalar baseFreqY, int numOctaves,
                                     SkScalar seed, int tileW, int tileH) -> sk_sp<SkShader> {
                                    // if tileSize is empty (e.g. tileW <= 0 or tileH <= 0, it will
                                    // be ignored.
                                    SkISize tileSize = SkISize::Make(tileW, tileH);
                                    return SkPerlinNoiseShader::MakeFractalNoise(
      ...
      .class_function(
          "_MakeLinearGradient",
          optional_override(
              [](WASMPointerF32 fourFloatsPtr, WASMPointerF32 cPtr, SkColorType colorType,
                 WASMPointerF32 pPtr, int count, SkTileMode mode, uint32_t flags,
                 WASMPointerF32 mPtr, sk_sp<SkColorSpace> colorSpace) -> sk_sp<SkShader> {
                ...
                if (colorType == SkColorType::kRGBA_F32_SkColorType) {
                  ...
                  return SkGradientShader::MakeLinear(
                      points, colors, colorSpace, positions, count, mode, flags, &localMatrix);
                } else if (colorType == SkColorType::kRGBA_8888_SkColorType) {
                  const SkColor* colors = reinterpret_cast<const SkColor*>(cPtr);
                  return SkGradientShader::MakeLinear(
                      points, colors, positions, count, mode, flags, &localMatrix);
                }
      ...
      .class_function(
          "_MakeRadialGradient",
          optional_override(
              [](SkScalar cx, SkScalar cy, SkScalar radius, WASMPointerF32 cPtr,
                 SkColorType colorType, WASMPointerF32 pPtr, int count, SkTileMode mode,
                 uint32_t flags, WASMPointerF32 mPtr,
                ...
                if (colorType == SkColorType::kRGBA_F32_SkColorType) {
                  ...
                  return SkGradientShader::MakeRadial(
                      {cx, cy}, radius, colors, colorSpace, positions, count, mode, flags,
                      &localMatrix);
                } else if (colorType == SkColorType::kRGBA_8888_SkColorType) {
                  const SkColor* colors = reinterpret_cast<const SkColor*>(cPtr);
                  return SkGradientShader::MakeRadial(
                      {cx, cy}, radius, colors, positions, count, mode, flags, &localMatrix);
                }
                SkDebugf("%d is not an accepted colorType\n", colorType);
                return nullptr;
              }),
      ...
      .class_function(
          "_MakeSweepGradient",
          optional_override(
              [](SkScalar cx, SkScalar cy, WASMPointerF32 cPtr, SkColorType colorType,
                 WASMPointerF32 pPtr, int count, SkTileMode mode, SkScalar startAngle,
                 SkScalar endAngle, uint32_t flags, WASMPointerF32 mPtr,
                 sk_sp<SkColorSpace> colorSpace) -> sk_sp<SkShader> {
                const SkScalar* positions = reinterpret_cast<const SkScalar*>(pPtr);
                OptionalMatrix localMatrix(mPtr);
                if (colorType == SkColorType::kRGBA_F32_SkColorType) {
                  ...
                  return SkGradientShader::MakeSweep(
                      cx, cy, colors, colorSpace, positions, count, mode, startAngle, endAngle,
                      flags, &localMatrix);
                } else if (colorType == SkColorType::kRGBA_8888_SkColorType) {
                  const SkColor* colors = reinterpret_cast<const SkColor*>(cPtr);
                  return SkGradientShader::MakeSweep(
                      cx, cy, colors, positions, count, mode, startAngle, endAngle, flags,
                      &localMatrix);
                }
                SkDebugf("%d is not an accepted colorType\n", colorType);
                return nullptr;
      ...
      .class_function(
          "MakeTurbulence", optional_override(
                                [](SkScalar baseFreqX, SkScalar baseFreqY, int numOctaves,
                                  ...
                                  // ignored.
                                  SkISize tileSize = SkISize::Make(tileW, tileH);
                                  return SkPerlinNoiseShader::MakeTurbulence(
                                      baseFreqX, baseFreqY, numOctaves, seed, &tileSize);
                                }))
      .class_function(
          "_MakeTwoPointConicalGradient",
          optional_override(
              [](WASMPointerF32 fourFloatsPtr, SkScalar startRadius, SkScalar endRadius,
                 WASMPointerF32 cPtr, SkColorType colorType, WASMPointerF32 pPtr, int count,
                 SkTileMode mode, uint32_t flags, WASMPointerF32 mPtr,
                 sk_sp<SkColorSpace> colorSpace) -> sk_sp<SkShader> {
                ...
                if (colorType == SkColorType::kRGBA_F32_SkColorType) {
                  ...
                  const SkColor* colors = reinterpret_cast<const SkColor*>(cPtr);
                  return SkGradientShader::MakeTwoPointConical(
                      startAndEnd[0], startRadius, startAndEnd[1], endRadius, colors, positions,
...
#  endif
  ...
  class_<SkRuntimeEffect>("RuntimeEffect")
      ...
      .class_function(
          "_Make", optional_override(
                       [](std::string sksl, emscripten::val errHandler) -> sk_sp<SkRuntimeEffect> {
...
#  ifdef SKSL_ENABLE_TRACING
      .class_function(
          "MakeTraced", optional_override(
                            [](sk_sp<SkShader> shader, int traceCoordX,
...
#  endif
      .function(
          "_makeShader", optional_override(
                             [](SkRuntimeEffect& self, WASMPointerF32 fPtr, size_t fLen,
                                bool shouldOwnUniforms, WASMPointerF32 mPtr) -> sk_sp<SkShader> {
      ...
      .function(
          "_makeShaderWithChildren",
          optional_override(
              [](SkRuntimeEffect& self, WASMPointerF32 fPtr, size_t fLen, bool shouldOwnUniforms,
                 WASMPointerU32 cPtrs, size_t cLen, WASMPointerF32 mPtr) -> sk_sp<SkShader> {
                void* uniformData = reinterpret_cast<void*>(fPtr);
                castUniforms(uniformData, fLen, self);
                sk_sp<SkData> uniforms;
                if (shouldOwnUniforms) {
      ...
      .function(
          "getUniform", optional_override([](SkRuntimeEffect& self, int i) -> RuntimeEffectUniform {
            auto it = self.uniforms().begin() + i;
  ...
  class_<SkSurface>("Surface")
      ...
      .class_function(
          "_makeRasterDirect",
          optional_override(
              [](const SimpleImageInfo ii, WASMPointerU8 pPtr,
          ...
          allow_raw_pointers())
      .function("_flush", optional_override([](SkSurface& self) { self.flushAndSubmit(false); }))
      .function("_getCanvas", &SkSurface::getCanvas, allow_raw_pointers())
      .function(
          "imageInfo", optional_override([](SkSurface& self) -> SimpleImageInfo {
...
#ifdef SK_GL
      .function(
          "_makeImageFromTexture",
          optional_override(
              [](SkSurface& self, uint32_t webglHandle, uint32_t texHandle,
                 SimpleImageInfo ii) -> sk_sp<SkImage> {
                auto releaseCtx = new TextureReleaseContext{webglHandle, texHandle};
                GrGLTextureInfo gti = {
                    GR_GL_TEXTURE_2D, texHandle,
                ...
                return SkImage::MakeFromTexture(
                    dContext, gbt, GrSurfaceOrigin::kTopLeft_GrSurfaceOrigin, ii.colorType,
                    ii.alphaType, ii.colorSpace, deleteJSTexture, releaseCtx);
              }))
#endif
      .function(
          "_makeImageSnapshot",
      ...
      .function("sampleCnt", optional_override([](SkSurface& self) -> int {
                  auto backendRT =
                      self.getBackendRenderTarget(SkSurface::kFlushRead_BackendHandleAccess);
  ...
  class_<SkTextBlob>("TextBlob")
      ...
      .class_function(
          "_MakeFromRSXform",
          optional_override(
              [](WASMPointerU8 sptr, size_t strBtyes, WASMPointerF32 xptr,
          ...
          allow_raw_pointers())
      .class_function(
          "_MakeFromRSXformGlyphs",
          optional_override(
              [](WASMPointerU16 gPtr, size_t byteLen, WASMPointerF32 xptr,
                 const SkFont& font) -> sk_sp<SkTextBlob> {
                const SkGlyphID* glyphs = reinterpret_cast<const SkGlyphID*>(gPtr);
                const SkRSXform* xforms = reinterpret_cast<const SkRSXform*>(xptr);
      ...
      .class_function(
          "_MakeFromText",
          optional_override(
              [](WASMPointerU8 sptr, size_t len, const SkFont& font) -> sk_sp<SkTextBlob> {
                const char* str = reinterpret_cast<const char*>(sptr);
      ...
      .class_function(
          "_MakeFromGlyphs",
          optional_override(
              [](WASMPointerU16 gPtr, size_t byteLen, const SkFont& font) -> sk_sp<SkTextBlob> {
                const SkGlyphID* glyphs = reinterpret_cast<const SkGlyphID*>(gPtr);
  ...
  class_<SkTypeface>("Typeface")
      ...
      .function(
          "_getGlyphIDs", optional_override(
                              [](SkTypeface& self, WASMPointerU8 sptr, size_t strLen,
                                 size_t expectedCodePoints, WASMPointerU16 iPtr) -> int {
                                char* str = reinterpret_cast<char*>(sptr);
                                SkGlyphID* glyphIDs = reinterpret_cast<SkGlyphID*>(iPtr);
  ...
  class_<SkVertices>("Vertices")
      ...
      .function("_bounds", optional_override([](SkVertices& self, WASMPointerF32 fPtr) -> void {
                  SkRect* output = reinterpret_cast<SkRect*>(fPtr);
...
#ifdef CK_INCLUDE_PARAGRAPH
  constant(
      "_GlyphRunFlags_isWhiteSpace", (int)skia::textlayout::Paragraph::kWhiteSpace_VisitorFlag);
#endif
}
