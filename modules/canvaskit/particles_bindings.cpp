...
EMSCRIPTEN_BINDINGS(Particles) {
  class_<SkParticleEffect>("ParticleEffect")
      ...
      .function(
          "getUniform", optional_override([](SkParticleEffect& self, int i) -> ParticleUniform {
            ParticleUniform su;
            auto info = self.uniformInfo();
            if (!info) {
              return su;
      ...
      .function(
          "_setPosition",
          optional_override([](SkParticleEffect& self, SkScalar x, SkScalar y) -> void {
  ...
  function(
      ...
      optional_override(
          [](std::string json, size_t assetCount, WASMPointerU32 nptr, WASMPointerU32 dptr,
             WASMPointerU32 sptr) -> sk_sp<SkParticleEffect> {
