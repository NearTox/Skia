...
class WebTrack final : public skresources::ExternalTrackAsset {
 ...
 private:
  void seek(float t) override { fPlayer.call<void>("seek", val(t)); }
...
class SkottieAssetProvider : public skottie::ResourceProvider {
 public:
  ...
  static sk_sp<SkottieAssetProvider> Make(AssetVec assets, emscripten::val soundMap) {
    return sk_sp<SkottieAssetProvider>(
        new SkottieAssetProvider(std::move(assets), std::move(soundMap)));
  }
 ...
 private:
  explicit SkottieAssetProvider(AssetVec assets, emscripten::val soundMap)
      : fAssets(std::move(assets)), fSoundMap(std::move(soundMap)) {}
...
class JSLogger final : public skottie::Logger {
 public:
  static sk_sp<JSLogger> Make(emscripten::val logger) {
    return logger.as<bool>() && logger.hasOwnProperty(kWrnFunc) && logger.hasOwnProperty(kErrFunc)
 ...
 private:
  ...
  inline static constexpr char kWrnFunc[] = "onWarning", kErrFunc[] = "onError";
...
class ManagedAnimation final : public SkRefCnt {
 public:
  static sk_sp<ManagedAnimation> Make(
      const std::string& json, sk_sp<skottie::ResourceProvider> rp, std::string prop_prefix,
  ...
  bool setColor(const std::string& key, SkColor c) { return fPropMgr->setColor(key, c); }

  bool setOpacity(const std::string& key, float o) { return fPropMgr->setOpacity(key, o); }
 ...
 private:
  ManagedAnimation(
      ...
      : fAnimation(std::move(animation)), fPropMgr(std::move(propMgr)) {}
...
EMSCRIPTEN_BINDINGS(Skottie) {
  ...
  class_<skottie::Animation>("Animation")
      ...
      .function(
          "_size", optional_override([](skottie::Animation& self, WASMPointerF32 oPtr) -> void {
      ...
      .function(
          "_render",
          optional_override(
              [](skottie::Animation& self, SkCanvas* canvas, WASMPointerF32 fPtr) -> void {
  ...
  class_<ManagedAnimation>("ManagedAnimation")
      ...
      .function("_size", optional_override([](ManagedAnimation& self, WASMPointerF32 oPtr) -> void {
                  SkSize* output = reinterpret_cast<SkSize*>(oPtr);
                  *output = self.size();
                }))
      ...
      .function(
          "_render",
          optional_override(
              [](ManagedAnimation& self, SkCanvas* canvas, WASMPointerF32 fPtr) -> void {
                const SkRect* dst = reinterpret_cast<const SkRect*>(fPtr);
                self.render(canvas, dst);
              }),
          allow_raw_pointers())
      .function(
          "_seek", optional_override([](ManagedAnimation& self, SkScalar t, WASMPointerF32 fPtr) {
            SkRect* damageRect = reinterpret_cast<SkRect*>(fPtr);
            damageRect[0] = self.seek(t);
          }))
      .function(
          "_seekFrame",
          optional_override([](ManagedAnimation& self, double frame, WASMPointerF32 fPtr) {
            SkRect* damageRect = reinterpret_cast<SkRect*>(fPtr);
            damageRect[0] = self.seekFrame(frame);
          }))
      .function("seekFrame", &ManagedAnimation::seekFrame)
      .function(
          "_setColor", optional_override([](ManagedAnimation& self, const std::string& key,
                                            WASMPointerF32 cPtr) {
  ...
  function(
      ...
      optional_override(
          [](std::string json, size_t assetCount, WASMPointerU32 nptr, WASMPointerU32 dptr,
             WASMPointerU32 sptr, std::string prop_prefix, emscripten::val soundMap,
             emscripten::val logger) -> sk_sp<ManagedAnimation> {
