...
// 3. Run `./build_simd_test.sh simd_int_capabilities.cpp` to build and output WASM SIMD operations
// present
...
int main() {
  ...
  // vec1 = skvx::fma(vec1, vec2, vec1); //not available in wasm (no fused multiply-add is
  // available) vec1 = skvx::fract(vec1); //N/A
