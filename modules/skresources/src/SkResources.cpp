...
sk_sp<SkImage> ImageAsset::getFrame(float t) { return nullptr; }
  ...
  auto decode = [](sk_sp<SkImage> image) {
    ...
    if (image_area > kMaxArea) {
      ...
      if (bm.tryAllocPixels(info, info.minRowBytes()) &&
          image->scalePixels(
              bm.pixmap(), SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kNearest),
...
sk_sp<ExternalTrackAsset> ResourceProviderProxyBase::loadAudioAsset(
    const char path[], const char name[], const char id[]) {
...
sk_sp<SkTypeface> DataURIResourceProviderProxy::loadTypeface(
    const char name[], const char url[]) const {
