    ...
    void drawTest(SkCanvas* canvas, SkScalar w, SkScalar h, SkColor fg, SkColor bg) {
        ...
        for (auto i = 1; i < 5; ++i) {
            ...
            for (auto para : gParagraph) {
                ...
                name = " " + std::get<0>(para) + " " + (std::get<1>(para) ? ", bold" : "") +
...
protected:
    ...
    void onDrawContent(SkCanvas* canvas) override {
        ...
        auto rects =
            paragraph->getRectsForRange(7, 9, RectHeightStyle::kTight, RectWidthStyle::kTight);
        SkPaint paint;
        ...
        for (auto& query : hit1) {
          auto hitRects = paragraph->getRectsForRange(
    ...
    void onDrawContent(SkCanvas* canvas) override {
        ...
        auto draw = [&](SkScalar width, SkScalar height, TextDirection td, TextAlign ta, const char* t) {
            ...
            for (auto& line : impl->lines()) {
                if (this->isVerbose()) {
                  SkDebugf(
                      "line[%d]: %f + %f\n", (int)(&line - impl->lines().begin()), line.offset().fX,
                      line.shift());
                }
                line.iterateThroughVisualRuns(true,
                    ...
                    *width = line.measureTextInsideOneRun(textRange, run, runOffset, 0, true, false).clip.width();
                    if (this->isVerbose()) {
                      SkDebugf(
                          "%zu[%zu: %zu) @%f + %f %s\n", run->index(), textRange.start,
                          textRange.end, runOffset, *width, run->leftToRight() ? "left" : "right");
                    }
                    return true;
                });
            }
...
protected:
    ...
    void onDrawContent(SkCanvas* canvas) override {
        ...
        if (this->isVerbose()) {
          ...
          SkDebugf(
              "%d(%s) %d(%s) %d(%s)\n", f1Pos.position,
              f1Pos.affinity == Affinity::kUpstream ? "up" : "down", f2Pos.position,
              f2Pos.affinity == Affinity::kUpstream ? "up" : "down", iPos.position,
              iPos.affinity == Affinity::kUpstream ? "up" : "down");
          ...
          auto f2 =
              paragraph->getRectsForRange(1, 2, RectHeightStyle::kTight, RectWidthStyle::kTight);
          if (f2.empty()) {
          ...
          auto fi =
              paragraph->getRectsForRange(2, 3, RectHeightStyle::kTight, RectWidthStyle::kTight);
          if (fi.empty()) {
...
protected:
    ...
    void onDrawContent(SkCanvas* canvas) override {
      const std::u16string
          text =  // u"\U0001f600\U0001f1e6\U0001f1f9\U0001f601\U0001f9f1\U0001f61a\U0001f431\U0001f642\U0001f38e\U0001f60d\U0001f3b9\U0001f917\U0001f6bb\U0001f609\U0001f353\U0001f618\U0001f1eb\U0001f1f0\U0001f468\u200D\U0001f469\u200D\U0001f466\u200D\U0001f466\U0001f468\u200D\U0001f469\u200D\U0001f467\u200D\U0001f466\U0001f468\u200D\U0001f469\u200D\U0001f467\U0001f46a";
          u"\U0001f469\u200D\U0001f469\u200D\U0001f466\U0001f469\u200D\U0001f469\u200D\U0001f467"
          u"\u200D\U0001f467\U0001f1fa\U0001f1f8";
      canvas->drawColor(SK_ColorWHITE);
      ...
      std::pair<size_t, size_t> rects[] = {
          {0, 2},   {0, 4},   {0, 8},   {23, 25}, {23, 27}, {23, 31}, {23, 39}, {23, 55}, {21, 23},
          {1, 3},   {1, 5},   {1, 9},   {1, 17},  {1, 33},  {2, 4},   {2, 6},   {2, 10},  {2, 18},
          {2, 34},  {3, 5},   {3, 7},   {3, 11},  {3, 19},  {4, 6},   {4, 8},   {4, 12},  {4, 20},
          {5, 7},   {5, 9},   {5, 13},  {5, 21},  {6, 8},   {6, 10},  {6, 14},  {6, 22},  {7, 9},
          {7, 11},  {7, 15},  {7, 23},  {8, 10},  {8, 12},  {8, 16},  {8, 24},  {9, 11},  {9, 13},
          {9, 17},  {9, 25},  {10, 12}, {10, 14}, {10, 18}, {10, 26}, {11, 13}, {11, 15}, {11, 19},
          {11, 27}, {12, 14}, {12, 16}, {12, 20}, {12, 28}, {13, 15}, {13, 17}, {13, 21}, {14, 16},
          {14, 18}, {14, 22}, {15, 17}, {15, 19}, {15, 23}, {16, 18}, {16, 20}, {16, 24}, {17, 19},
          {17, 21}, {18, 20}, {18, 22}, {19, 21}, {20, 22}, {20, 24}, {21, 23}, {22, 24}, {22, 26},
          {22, 30}, {22, 38}, {22, 54}, {20, 22}, {18, 22},
      };
      for (auto rect : rects) {
 ...
 protected:
  ...
  void onDrawContent(SkCanvas* canvas) override {
    ...
    paragraph->visit([&](int, const skia::textlayout::Paragraph::VisitorInfo* info) {
      ...
      auto first = info->positions[0];
      first.offset(info->origin.fX, info->origin.fY);
      SkRect rect = SkRect::MakeXYWH(
          first.fX, first.fY + metrics.fAscent, info->advanceX - first.fX,
...
class ParagraphView60 : public ParagraphView_Base {
 protected:
  ...
  void onDrawContent(SkCanvas* canvas) override {
    ...
    SkDebugf(
        "getGlyphPositionAtCoordinate(20,2)=%d %s\n", result.position,
 ...
 protected:
  ...
  void onDrawContent(SkCanvas* canvas) override {
    ...
    for (auto x = 0.0f; x < paragraph->getMaxIntrinsicWidth(); x += 5.0f) {
      ...
      auto rects =
          paragraph->getRectsForRange(0, p, RectHeightStyle::kTight, RectWidthStyle::kTight);
      SkDebugf(
          "@x=%f [0:%d%s=%d) ", x, pos.position, pos.affinity == Affinity::kDownstream ? "D" : "U",
          p);
      for (auto& rect : rects) {
    ...
    // auto rects130 = paragraph->getRectsForRange(0.0f, 130.0f, RectHeightStyle::kTight,
    // RectWidthStyle::kTight); auto rects140 = paragraph->getRectsForRange(0.0f, 140.0f,
    // RectHeightStyle::kTight, RectWidthStyle::kTight);
  }
