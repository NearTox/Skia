...
class ParagraphGM : public skiagm::GM {
 ...
 protected:
  void onOnceBeforeDraw() override { this->buildParagraph(); }

  SkString onShortName() override {
    SkString name;
    name.printf(
        "paragraph%s_%s", fFlags & kTimeLayout ? "_layout" : "",
    ...
    para->visit([&](int, const skia::textlayout::Paragraph::VisitorInfo* info) {
      ...
      if (fFlags & kUseUnderline) {
        ...
        for (int i = 0; i < info->count; ++i) {
          pos.push_back(
              {info->origin.fX + info->positions[i].fX, info->origin.fY + info->positions[i].fY});
        }

        const SkScalar X0 = pos[0].fX;
        const SkScalar X1 = X0 + info->advanceX;
        const SkScalar Y = pos[0].fY;
        auto sects = info->font.getIntercepts(info->glyphs, info->count, pos.data(), Y + 1, Y + 3);
  ...
  bool onAnimate(double /*nanos*/) override { return false; }
