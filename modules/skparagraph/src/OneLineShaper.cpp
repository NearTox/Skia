  ...
  if (unresolved.fGlyphs.width() == fCurrentRun->size()) {
    ...
    if (lastUnresolved.fRun != nullptr && lastUnresolved.fRun->fIndex == fCurrentRun->fIndex) {
      if (lastUnresolved.fText.end == unresolved.fText.start) {
        ...
        lastUnresolved.fGlyphs.start = SkTMin(lastUnresolved.fGlyphs.start, glyphRange.start);
...
void OneLineShaper::iterateThroughFontStyles(
    SkSpan<Block> styleSpan, const ShapeSingleFontVisitor& visitor) {
  Block combinedBlock;
  ...
  for (auto& placeholder : fParagraph->fPlaceholders) {
    ...
    if (placeholder.fTextBefore.width() > 0) {
      ...
      SkSpan<Block> styleSpan(
          fParagraph->fTextStyles.begin() + placeholder.fBlocksBefore.start,
    ...
    auto& run = fParagraph->fRuns.emplace_back(
        this->fParagraph, runInfo, 0, 1.0f, fParagraph->fRuns.count(), advanceX);
...
bool OneLineShaper::shape() {
  ...
  uint8_t textDirection =
      fParagraph->fParagraphStyle.getTextDirection() == TextDirection::kLtr ? 2 : 1;
  auto limitlessWidth = std::numeric_limits<SkScalar>::max();

  auto result =
      iterateThroughShapingRegions([this, textDirection, limitlessWidth](
                                       SkSpan<const char> textSpan, SkSpan<Block> styleSpan,
        ...
        iterateThroughFontStyles(
            styleSpan, [this, &shaper, textDirection, limitlessWidth, &advanceX](Block block) {
              ...
              matchResolvedFonts(block.fStyle, [&](sk_sp<SkTypeface> typeface) {
                ...
                while (count-- > 0) {
                  ...
                  auto script = SkShaper::MakeHbIcuScriptRunIterator(
                  ...
                  shaper->shape(
                      unresolvedText.begin(), unresolvedText.size(), fontIter, *bidi, *script, lang,
                      limitlessWidth, this);
