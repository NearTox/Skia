...
void OneLineShaper::commitRunBuffer(const RunInfo&) {
  ...
  /*
      ...
      for (size_t i = 0; i < fCurrentRun->size(); ++i) {
          SkDebugf("[%zu] %hu %u %f\n", i, fCurrentRun->fGlyphs[i], fCurrentRun->fClusterIndexes[i],
     fCurrentRun->fPositions[i].fX);
      }
  */
...
void OneLineShaper::printState() {
  ...
  for (auto& resolved : fResolvedBlocks) {
    ...
    SkDebugf("[%zu:%zu) with %s\n", resolved.fText.start, resolved.fText.end, name.c_str());
  }
...
void OneLineShaper::finish(const Block& block, SkScalar height, SkScalar& advanceX) {
  ...
  std::sort(
      fResolvedBlocks.begin(), fResolvedBlocks.end(),
      [](const RunBlock& a, const RunBlock& b) { return a.fText.start < b.fText.start; });
  ...
  for (auto& resolvedBlock : fResolvedBlocks) {
    ...
    if (lastTextEnd != text.start) {
      SkDEBUGF(
          "Text ranges mismatch: ...:%zu] - [%zu:%zu] (%zu-%zu)\n", lastTextEnd, text.start,
          text.end, glyphs.start, glyphs.end);
      SkASSERT(false);
    }
    ...
    auto runAdvance =
        SkVector::Make(run->posX(glyphs.end) - run->posX(glyphs.start), run->fAdvance.fY);
    ...
    this->fParagraph->fRuns.emplace_back(
        this->fParagraph, info, run->fClusterStart, height, block.fStyle.getHalfLeading(),
        block.fStyle.getBaselineShift(), this->fParagraph->fRuns.count(), advanceX);
...
void OneLineShaper::addFullyResolved() {
  ...
  RunBlock resolved(
      fCurrentRun, this->fCurrentRun->fTextRange, GlyphRange(0, this->fCurrentRun->size()),
...
void OneLineShaper::sortOutGlyphs(std::function<void(GlyphRange)>&& sortOutUnresolvedBLock) {
  ...
  for (size_t i = 0; i < fCurrentRun->size(); ++i) {
    ...
    if ((fCurrentRun->leftToRight() ? gi > graphemeStart : gi < graphemeStart) ||
...
void OneLineShaper::iterateThroughFontStyles(
  ...
  auto addFeatures = [&features](const Block& block) {
    for (auto& ff : block.fStyle.getFontFeatures()) {
      ...
      SkShaper::Feature feature = {
          SkSetFourByteTag(ff.fName[0], ff.fName[1], ff.fName[2], ff.fName[3]), SkToU32(ff.fValue),
          block.fRange.start, block.fRange.end};
  ...
  for (auto& block : styleSpan) {
    BlockRange blockRange(
        std::max(block.fRange.start, textRange.start), std::min(block.fRange.end, textRange.end));
    if (blockRange.empty()) {
      continue;
    }
...
bool OneLineShaper::iterateThroughShapingRegions(const ShapeVisitor& shape) {
  ...
  for (auto& placeholder : fParagraph->fPlaceholders) {
    ...
    std::vector<sk_sp<SkTypeface>> typefaces = fParagraph->fFontCollection->findTypefaces(
        placeholder.fTextStyle.getFontFamilies(), placeholder.fTextStyle.getFontStyle(),
    ...
    auto& run = fParagraph->fRuns.emplace_back(
        this->fParagraph, runInfo, placeholder.fRange.start, 0.0f, 0.0f, false,
        fParagraph->fRuns.count(), advanceX);
...
bool OneLineShaper::shape() {
  ...
  auto result = iterateThroughShapingRegions([this, limitlessWidth](
                                                 TextRange textRange, SkSpan<Block> styleSpan,
                                                 SkScalar& advanceX, TextIndex textStart,
                                                 uint8_t defaultBidiLevel) {
    ...
    iterateThroughFontStyles(
        ...
        [this, &shaper, defaultBidiLevel, limitlessWidth, &advanceX](
          ...
          this->matchResolvedFonts(block.fStyle, [&](sk_sp<SkTypeface> typeface) {
            ...
            bool fakeBold = wantedWeight >= SkFontStyle::kSemiBold_Weight &&
                            wantedWeight - font.getTypeface()->fontStyle().weight() >= 200;
            bool fakeItalic = block.fStyle.getFontStyle().slant() == SkFontStyle::kItalic_Slant &&
            ...
            while (unresolvedCount-- > 0) {
              ...
              LangIterator langIter(
                  unresolvedText, blockSpan, fParagraph->paragraphStyle().getTextStyle());
              ...
              for (const SkShaper::Feature& feature : features) {
                ...
                if (unresolvedRange.intersects(featureRange)) {
                  ...
                  adjustedRange.Shift(
                      -static_cast<std::make_signed_t<size_t>>(unresolvedRange.start));
                  adjustedFeatures.push_back(
                      {feature.tag, feature.value, adjustedRange.start, adjustedRange.end});
                }
              }

              shaper->shape(
                  unresolvedText.begin(), unresolvedText.size(), fontIter, bidiIter, *scriptIter,
                  langIter, adjustedFeatures.data(), adjustedFeatures.size(), limitlessWidth, this);
...
size_t OneLineShaper::FontKey::Hasher::operator()(const OneLineShaper::FontKey& key) const {
  return SkGoodHash()(key.fUnicode) ^ SkGoodHash()(key.fFontStyle) ^
