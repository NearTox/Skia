...
void TextWrapper::trimEndSpaces(TextAlign align) {
  ...
  for (auto cluster = fEndLine.endCluster();
    ...
    if ((cluster->run()->leftToRight()) || (right && !cluster->run()->leftToRight()) ||
...
void TextWrapper::breakTextIntoLines(
  ...
  while (fEndLine.endCluster() != end) {
    ...
    parent->fMaxWidthWithTrailingSpaces =
        SkMaxScalar(parent->fMaxWidthWithTrailingSpaces, widthWithSpaces);
  ...
  if (fHardLineBreak) {
    ...
    TextRange hardBreak(
        fEndLine.breakCluster()->textRange().end, fEndLine.breakCluster()->textRange().end);
    ClusterRange clusters(fEndLine.breakCluster() - start, fEndLine.endCluster() - start);
    addLine(
        empty, hardBreak, clusters, clusters, 0, 0, 0, SkVector::Make(0, fHeight),
