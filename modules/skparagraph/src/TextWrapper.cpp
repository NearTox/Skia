...
struct LineBreakerWithLittleRounding {
  LineBreakerWithLittleRounding(SkScalar maxWidth)
      : fLower(maxWidth - 0.25f), fMaxWidth(maxWidth), fUpper(maxWidth + 0.25f) {}
      ...
      if (nextWordLength > maxWidth) {
        if (nextNonBreakingSpace != nullptr) {
          ...
          if (SkScalar shortLength = fWords.width() + nextShortWordLength;
            ...
            fClusters = TextStretch(
                fClusters.startCluster(), nextNonBreakingSpace,
...
void TextWrapper::trimEndSpaces(TextAlign align) {
  ...
  for (auto cluster = fEndLine.endCluster();
       cluster >= fEndLine.startCluster() && cluster->isWhitespaceBreak(); --cluster) {
...
void TextWrapper::breakTextIntoLines(
  ...
  while (fEndLine.endCluster() != end) {
    ...
    TextRange text(
        fEndLine.startCluster()->textRange().start, fEndLine.breakCluster()->textRange().start);
    TextRange textIncludingNewlines(
        fEndLine.startCluster()->textRange().start, startLine->textRange().start);
    if (startLine == end) {
    ...
    addLine(
        textExcludingSpaces, text, textIncludingNewlines, clusters, clustersWithGhosts,
        widthWithSpaces, fEndLine.startPos(), fEndLine.endPos(), SkVector::Make(0, fHeight),
        SkVector::Make(fEndLine.width(), lineHeight), fEndLine.metrics(),
    ...
    parent->fMaxWidthWithTrailingSpaces =
  ...
  if (fHardLineBreak) {
    ...
    addLine(
        fEndLine.breakCluster()->textRange(), fEndLine.breakCluster()->textRange(),
        fEndLine.endCluster()->textRange(), clusters, clusters, 0, 0, 0, SkVector::Make(0, fHeight),
        SkVector::Make(0, fEndLine.metrics().height()), fEndLine.metrics(), needEllipsis);
