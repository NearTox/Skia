...
void TextWrapper::trimEndSpaces() {
  ...
  for (auto cluster = fEndLine.endCluster();
       cluster >= fEndLine.startCluster() && cluster->isWhitespaces(); --cluster) {
...
void TextWrapper::breakTextIntoLines(
  ...
  while (fEndLine.endCluster() != end) {
    ...
    auto lastLine = maxLines == std::numeric_limits<size_t>::max() || fLineNumber >= maxLines;
    auto needEllipsis = lastLine && !fHardLineBreak && fEndLine.endCluster() < end - 1 &&
                        maxWidth != std::numeric_limits<SkScalar>::max() && !ellipsisStr.isEmpty();
    ...
    TextRange text(
        fEndLine.startCluster()->textRange().start, fEndLine.endCluster()->textRange().end);
    TextRange textWithSpaces(
        fEndLine.startCluster()->textRange().start, fEndLine.breakCluster()->textRange().end);
    ClusterRange clusters(
        fEndLine.startCluster() - parent->clusters().begin(),
        fEndLine.endCluster() - parent->clusters().begin() + 1);
    addLine(
        text, textWithSpaces, clusters, fEndLine.startPos(), fEndLine.endPos(),
        SkVector::Make(0, fHeight), SkVector::Make(fEndLine.width(), fEndLine.metrics().height()),
        fEndLine.metrics(), needEllipsis);
  ...
  if (fHardLineBreak) {
    ...
    TextRange empty(
        fEndLine.breakCluster()->textRange().start, fEndLine.breakCluster()->textRange().start);
    ClusterRange clusters(
        fEndLine.breakCluster() - parent->clusters().begin(),
        fEndLine.breakCluster() - parent->clusters().begin());
    addLine(
        empty, empty, clusters, 0, 0, SkVector::Make(0, fHeight),
        SkVector::Make(0, fEndLine.metrics().height()), fEndLine.metrics(), false);
  }
}
