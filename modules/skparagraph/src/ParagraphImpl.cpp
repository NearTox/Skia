...
Paragraph::Paragraph(ParagraphStyle style, sk_sp<FontCollection> fonts) noexcept
...
ParagraphImpl::ParagraphImpl(
    ...
    SkTArray<Placeholder, true> placeholders, sk_sp<FontCollection> fonts)
    : ParagraphImpl(
          SkStringFromU16String(utf16text), std::move(style), std::move(blocks),
          std::move(placeholders), std::move(fonts)) {}

ParagraphImpl::~ParagraphImpl() = default;

int32_t ParagraphImpl::unresolvedGlyphs() noexcept {
  if (fState < kShaped) {
    return -1;
  }
...
void ParagraphImpl::resetContext() noexcept {
  fAlphabeticBaseline = 0;
  fHeight = 0;
  fWidth = 0;
  fIdeographicBaseline = 0;
...
bool ParagraphImpl::computeCodeUnitProperties() {
  ...
  Direction textDirection =
      fParagraphStyle.getTextDirection() == TextDirection::kLtr ? Direction::kLTR : Direction::kRTL;
...
void ParagraphImpl::forEachCodeUnitPropertyRange(
    CodeUnitFlags property, CodeUnitRangeVisitor visitor) {
  size_t first = 0;
...
SkSpan<const char> ParagraphImpl::text(TextRange textRange) noexcept {
...
SkSpan<Cluster> ParagraphImpl::clusters(ClusterRange clusterRange) noexcept {
...
Cluster& ParagraphImpl::cluster(ClusterIndex clusterIndex) noexcept {
...
SkSpan<Block> ParagraphImpl::blocks(BlockRange blockRange) noexcept {
...
Block& ParagraphImpl::block(BlockIndex blockIndex) noexcept {
...
void ParagraphImpl::updateTextAlign(TextAlign textAlign) noexcept {
...
TextIndex ParagraphImpl::findGraphemeStart(TextIndex index) {
  ...
  while (index > 0 && (fCodeUnitProperties[index] & CodeUnitFlags::kGraphemeStart) == 0) {
    --index;
  }
  return index;
}
