...
void ParagraphImpl::layout(SkScalar rawWidth) {
  ...
  if (fState < kShaped) {
    ...
    // SkDebugf("layout('%s', %f): %f %f\n", fText.c_str(), rawWidth, fMinIntrinsicWidth,
    // fMaxIntrinsicWidth);
}
  ...
  for (RunIndex runIndex = 0; runIndex < fRuns.size(); ++runIndex) {
    ...
    if (run.isPlaceholder()) {
      ...
      auto& cluster = fClusters.emplace_back(
          this, runIndex, 0ul, 1ul, text, run.advance().fX, run.advance().fY);
      ...
      run.iterateThroughClustersInTextOrder([runIndex, this](
                                                size_t glyphStart, size_t glyphEnd,
                                                size_t charStart, size_t charEnd, SkScalar width,
                                                SkScalar height) {
...
void ParagraphImpl::markLineBreaks() {
  ...
  for (auto& run : fRuns) {
    ...
    for (size_t index = 0; index != run.clusterRange().width(); ++index) {
      auto correctIndex =
          run.leftToRight() ? index + run.clusterRange().start : run.clusterRange().end - index - 1;
...
void ParagraphImpl::breakShapedTextIntoLines(SkScalar maxWidth) {
  ...
  textWrapper.breakTextIntoLines(
      ...
      [&](TextRange text, TextRange textWithSpaces, ClusterRange clusters,
          ClusterRange clustersWithGhosts, SkScalar widthWithSpaces, size_t startPos, size_t endPos,
          SkVector offset, SkVector advance, InternalLineMetrics metrics, bool addEllipsis) {
        // TODO: Take in account clipped edges
        auto& line = this->addLine(
            offset, advance, text, textWithSpaces, clusters, clustersWithGhosts, widthWithSpaces,
            metrics);
        if (addEllipsis) {
        ...
        fLongestLine =
            std::max(fLongestLine, nearlyZero(advance.fX) ? widthWithSpaces : advance.fX);
      });
...
void ParagraphImpl::resolveStrut() {
  ...
  std::vector<sk_sp<SkTypeface>> typefaces =
      fFontCollection->findTypefaces(strutStyle.getFontFamilies(), strutStyle.getFontStyle());
  if (typefaces.empty()) {
...
TextLine& ParagraphImpl::addLine(
  ...
  return fLines.emplace_back(
      this, offset, advance, blocks, text, textWithSpaces, clusters, clustersWithGhosts,
      widthWithSpaces, sizes);
}
...
std::vector<TextBox> ParagraphImpl::getRectsForRange(
    unsigned start, unsigned end, RectHeightStyle rectHeightStyle, RectWidthStyle rectWidthStyle) {
  ...
  for (auto& line : fLines) {
    ...
    line.iterateThroughVisualRuns(
        true, [&](const Run* run, SkScalar runOffsetInLine, TextRange textRange,
          ...
          *runWidthInLine = line.iterateThroughSingleRunByStyles(
              ...
              [&](TextRange textRange, const TextStyle& style,
                ...
                auto context =
                    line.measureTextInsideOneRun(intersect, run, runOffsetInLine, 0, true, true);
                ...
                if (line.trimmedText().end <
                        line.textWithSpaces().end &&  // Line has trailing spaces
                    line.textWithSpaces().end ==
                    ...
                    line.trimmedText().end > intersect.start)  // Range has more than just spaces
                {
                  ...
                  if (this->paragraphStyle().getTextAlign() == TextAlign::kJustify &&
                      &line != &fLines.back()) {
                    // TODO: this is just a patch. Make it right later (when it's clear what and
                    // how)
                    trailingSpaces = clip;
                    trailingSpaces.fLeft = line.width();
                    clip.fRight = line.width();
                  } else if (
                      this->fParagraphStyle.getTextDirection() == TextDirection::kRtl &&
                      !run->leftToRight()) {
                    // Split
                    trailingSpaces = clip;
                    ...
                    clip.fLeft += delta;
                      ...
                      this->fParagraphStyle.getTextDirection() == TextDirection::kLtr &&
                      run->leftToRight()) {
                    // Split
                    trailingSpaces = clip;
                    trailingSpaces.fLeft = line.width();
                ...
                auto merge = [&lastRun, &context, &results](SkRect clip) {
                  ...
                  if (!results.empty() && lastRun != nullptr &&
...
std::vector<TextBox> ParagraphImpl::getRectsForPlaceholders() {
  ...
  for (auto& line : fLines) {
    line.iterateThroughVisualRuns(
        true,
        [&boxes, &line](const Run* run, SkScalar runOffset, TextRange textRange, SkScalar* width) {
              ...
              [this, line, dx, &result, &lookingForHit](
                ...
                auto findCodepointByTextIndex = [this](ClusterIndex clusterIndex8) {
                  auto codepoint = std::lower_bound(
                      fCodePoints.begin(), fCodePoints.end(), clusterIndex8,
                      [](const Codepoint& lhs, size_t rhs) -> bool {
                        return lhs.fTextIndex < rhs;
                      });
                ...
                if (context.run->leftToRight()) {
                  for (codepoints.end = codepointIndex; codepoints.end < fCodePoints.size();
                       ++codepoints.end) {
...
void ParagraphImpl::computeEmptyMetrics() {
  ...
  if (!paragraphStyle().getStrutStyle().getForceStrutHeight() &&
    ...
    fEmptyMetrics = InternalLineMetrics(
        fEmptyMetrics.ascent() * multiplier, fEmptyMetrics.descent() * multiplier,
...
bool ParagraphImpl::calculateBidiRegions(SkTArray<BidiRegion>* regions) {
  ...
  uint8_t bidiLevel =
      fParagraphStyle.getTextDirection() == TextDirection::kLtr ? UBIDI_LTR : UBIDI_RTL;
