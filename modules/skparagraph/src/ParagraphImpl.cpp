  ...
  for (RunIndex runIndex = 0; runIndex < fRuns.size(); ++runIndex) {
    ...
    if (run.isPlaceholder()) {
      ...
      auto& cluster = fClusters.emplace_back(
          this, runIndex, 0ul, 0ul, text, run.advance().fX, run.advance().fY);
...
bool ParagraphImpl::shapeTextIntoEndlessLine() {
  class ShapeHandler final : public SkShaper::RunHandler {
   public:
    explicit ShapeHandler(
        ParagraphImpl& paragraph, size_t firstChar, FontIterator* fontIterator, SkScalar advanceX)
   ...
   private:
    ...
    Buffer runBuffer(const RunInfo& info) override {
      auto& run = fParagraph->fRuns.emplace_back(
          fParagraph, info, fFirstChar, fFontIterator->currentLineHeight(),
          fParagraph->fRuns.count(), fAdvance.fX);
  ...
  if (!fFontCollection->getParagraphCache()->findParagraph(this)) {
    ...
    auto result = iterateThroughShapingRegions([this, textDirection, limitlessWidth](
                                                   SkSpan<const char> textSpan,
                                                   SkSpan<Block> styleSpan, SkScalar& advanceX,
                                                   size_t start) {
      ...
      auto bidi =
          SkShaper::MakeIcuBiDiRunIterator(textSpan.begin(), textSpan.size(), textDirection);
      if (bidi == nullptr) {
        return false;
      }
      ...
      shaper->shape(
          textSpan.begin(), textSpan.size(), font, *bidi, *script, lang, limitlessWidth, &handler);
  ...
  for (auto& placeholder : fPlaceholders) {
    ...
    if (placeholder.fTextBefore.width() > 0) {
      ...
      SkSpan<Block> styleSpan(
          fTextStyles.begin() + placeholder.fBlocksBefore.start, placeholder.fBlocksBefore.width());
    ...
    const SkShaper::RunHandler::RunInfo runInfo = {
        SkFont(), (uint8_t)2, SkPoint::Make(placeholder.fStyle.fWidth, placeholder.fStyle.fHeight),
        1, SkShaper::RunHandler::Range(placeholder.fRange.start, placeholder.fRange.width())};
    auto& run = fRuns.emplace_back(
        this, runInfo,
        0,  // placeholder.fRange.start,
        1, fRuns.count(), advanceX);
...
void ParagraphImpl::breakShapedTextIntoLines(SkScalar maxWidth) {
  ...
  textWrapper.breakTextIntoLines(
      ...
      [&](TextRange text, TextRange textWithSpaces, ClusterRange clusters,
          ...
          SkVector offset, SkVector advance, LineMetrics metrics, bool addEllipsis) {
        // Add the line
...
void ParagraphImpl::resolveStrut() {
  ...
  for (auto& fontFamily : strutStyle.getFontFamilies()) {
    typeface =
        fFontCollection->matchTypeface(fontFamily.c_str(), strutStyle.getFontStyle(), SkString());
...
std::vector<TextBox> ParagraphImpl::getRectsForRange(
  ...
  TextRange text;
  text.end = end >= fCodePoints.size() ? fText.size()
        ...
        true, [&](const Run* run, SkScalar runOffset, TextRange textRange, SkScalar* width) {
          ...
          *width += context.clip.width();
          ...
          if (rectHeightStyle == RectHeightStyle::kMax) {
            ...
            clip.fTop = line.offset().fY + line.sizes().baseline() -
                        line.getMaxRunMetrics().baseline() + line.getMaxRunMetrics().delta();
          ...
          if (!results.empty() && lastRun != nullptr && lastRun->placeholder() == nullptr &&
    ...
    line.iterateThroughVisualRuns(
        true, [this, &line, dx, &result](
          ...
          if (dx >= context.clip.fRight) {
            ...
            // so far
            result = {SkToS32(context.run->fClusterIndexes[context.pos + context.size - 1]) + 1,
                      kUpstream};
            return true;
          }
          ...
          if (graphemeSize > 1) {
            ...
            auto codepointStart =
                glyphStart + averageCodepoint * (codepointIndex - codepoints.start);
