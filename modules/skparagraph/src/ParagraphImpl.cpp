...
ParagraphImpl::ParagraphImpl(
    ...
    std::shared_ptr<SkUnicode> unicode)
    : ParagraphImpl(
          SkString(), std::move(style), std::move(blocks), std::move(placeholders),
          std::move(fonts), std::move(unicode)) {
  SkASSERT(fUnicode);
...
void ParagraphImpl::layout(SkScalar rawWidth) {
  ...
  if ((!SkScalarIsFinite(rawWidth) || fLongestLine <= floorWidth) && fState >= kLineBroken &&
...
bool ParagraphImpl::computeCodeUnitProperties() {
  ...
  for (auto& lineBreak : lineBreaks) {
    fCodeUnitProperties[lineBreak.pos] |=
  ...
  for (auto& run : fRuns) {
    ...
    if (run.isPlaceholder()) {
      ...
      fClusters.emplace_back(
          this, runIndex, 0ul, 1ul, this->text(run.textRange()), run.advance().fX,
          run.advance().fY);
...
void ParagraphImpl::breakShapedTextIntoLines(SkScalar maxWidth) {
  if (!fHasLineBreaks && !fHasWhitespacesInside && fPlaceholders.size() == 1 && fRuns.size() == 1 &&
      fRuns[0].fAdvance.fX <= maxWidth) {
    ...
    auto disableFirstAscent =
        this->paragraphStyle().getTextHeightBehavior() & TextHeightBehavior::kDisableFirstAscent;
    auto disableLastDescent =
        this->paragraphStyle().getTextHeightBehavior() & TextHeightBehavior::kDisableLastDescent;
    ...
    this->addLine(
        SkPoint::Make(0, 0), advance, textExcludingSpaces, textRange, textRange, clusterRange,
        clusterRangeWithGhosts, run.advance().x(), metrics);
  ...
  textWrapper.breakTextIntoLines(
      this, maxWidth,
      [&](TextRange textExcludingSpaces, TextRange text, TextRange textWithNewlines,
          ClusterRange clusters, ClusterRange clustersWithGhosts, SkScalar widthWithSpaces,
          size_t startPos, size_t endPos, SkVector offset, SkVector advance,
          InternalLineMetrics metrics, bool addEllipsis) {
        // TODO: Take in account clipped edges
        auto& line = this->addLine(
            offset, advance, textExcludingSpaces, text, textWithNewlines, clusters,
            clustersWithGhosts, widthWithSpaces, metrics);
        if (addEllipsis) {
...
void ParagraphImpl::resolveStrut() {
  ...
  std::vector<sk_sp<SkTypeface>> typefaces = fFontCollection->findTypefaces(
      strutStyle.getFontFamilies(), strutStyle.getFontStyle(), std::nullopt);
  if (typefaces.empty()) {
...
TextLine& ParagraphImpl::addLine(
  ...
  return fLines.emplace_back(
      this, offset, advance, blocks, textExcludingSpaces, text, textIncludingNewLines, clusters,
      clustersWithGhosts, widthWithSpaces, sizes);
}
...
void ParagraphImpl::setState(InternalState state) {
  ...
  switch (fState) {
    ...
    case kShaped: fClusters.reset(); [[fallthrough]];
    ...
    case kFormatted: fPicture = nullptr; [[fallthrough]];
...
void ParagraphImpl::computeEmptyMetrics() {
  ...
  if (!paragraphStyle().getStrutStyle().getForceStrutHeight() && textStyle.getHeightOverride()) {
    ...
    if (paragraphStyle().getStrutStyle().getHalfLeading()) {
      fEmptyMetrics.update(
          fEmptyMetrics.ascent(), fEmptyMetrics.descent(),
  ...
  if (emptyParagraph) {
    ...
    // In case of non-empty paragraph TextHeightBehaviour flags will be applied at the appropriate
    // place We have to do it here because we skip wrapping for an empty text
    auto disableFirstAscent =
    ...
    for (auto& rec : line.fTextBlobCache) {
      ...
      while (iter.experimentalNext(&run)) {
        const Paragraph::VisitorInfo info = {
            run.font,   rec.fOffset,   rec.fClipRect.fRight, run.count,
            run.glyphs, run.positions, clusterPtr,
            0,  // flags
        };
