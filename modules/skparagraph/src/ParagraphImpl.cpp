...
void ParagraphImpl::buildClusterTable() {
  ...
  for (RunIndex runIndex = 0; runIndex < fRuns.size(); ++runIndex) {
    ...
    run.iterateThroughClustersInTextOrder([runIndex, this](
                                              size_t glyphStart, size_t glyphEnd, size_t charStart,
                                              size_t charEnd, SkScalar width, SkScalar height) {
...
bool ParagraphImpl::shapeTextIntoEndlessLine() {
  class ShapeHandler final : public SkShaper::RunHandler {
   public:
    explicit ShapeHandler(ParagraphImpl& paragraph, FontIterator* fontIterator)
        : fParagraph(&paragraph), fFontIterator(fontIterator), fAdvance(SkVector::Make(0, 0)) {}
   ...
   private:
    ...
    Buffer runBuffer(const RunInfo& info) override {
      auto& run = fParagraph->fRuns.emplace_back(
          fParagraph, info, fFontIterator->currentLineHeight(), fParagraph->fRuns.count(),
          fAdvance.fX);
  ...
  shaper->shape(
      fTextSpan.begin(), fTextSpan.size(), font, *bidi, *script, lang,
...
void ParagraphImpl::breakShapedTextIntoLines(SkScalar maxWidth) {
  ...
  textWrapper.breakTextIntoLines(
      this, maxWidth,
      [&](TextRange text, TextRange textWithSpaces, ClusterRange clusters, size_t startPos,
          size_t endPos, SkVector offset, SkVector advance, LineMetrics metrics, bool addEllipsis) {
        // Add the line
...
void ParagraphImpl::resolveStrut() {
  ...
  fStrutMetrics = LineMetrics(
      metrics.fAscent * strutStyle.getHeight(), metrics.fDescent * strutStyle.getHeight(),
  ...
  for (auto& line : fLines) {
    ...
    if (lineText.start != intersect.start) {
      ...
      runOffset = line.iterateThroughRuns(
          before, 0, true, [](Run*, size_t, size_t, SkRect, SkScalar, bool) { return true; });
    }
    auto firstBox = results.size();
    line.iterateThroughRuns(
        intersect, runOffset, true,
        [&results, &line](
    ...
    if (rectWidthStyle == RectWidthStyle::kMax) {
      for (auto& i = firstBox; i < results.size(); ++i) {
        ...
        if (clip.fRight < line.offset().fX + line.width()) {
          SkRect right = SkRect::MakeXYWH(
              clip.fRight - line.offset().fX, clip.fTop,
              line.width() - (clip.fRight - line.offset().fX), clip.fBottom);
