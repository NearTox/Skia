    ...
    switch (textStyle.getDecorationStyle()) {
      ...
      case TextDecorationStyle::kDouble: {
        ...
        if (drawGaps) {
          ...
          canvas->drawPath(fPath, fPaint);
          calculateGaps(
              context, left, left + width, bottom, bottom + fThickness, baseline, fThickness);
          canvas->drawPath(fPath, fPaint);
        } else {
...
void Decorations::calculateGaps(
  ...
  context.run->copyTo(builder, SkToU32(context.pos), context.size, SkVector::Make(0, baseline));
