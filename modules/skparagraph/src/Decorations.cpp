...
static void draw_line_as_rect(
    SkCanvas* canvas, SkScalar x, SkScalar y, SkScalar width, const SkPaint& paint) {
    ...
    switch (textStyle.getDecorationStyle()) {
      ...
      case TextDecorationStyle::kDouble: {
        ...
        if (drawGaps) {
          ...
          calculateGaps(
              context, SkRect::MakeXYWH(left, y, width, fThickness), baseline, fThickness);
          canvas->drawPath(fPath, fPaint);
          calculateGaps(
              context, SkRect::MakeXYWH(left, bottom, width, fThickness), baseline, fThickness);
          canvas->drawPath(fPath, fPaint);
        } else {
...
void Decorations::calculatePosition(TextDecoration decoration, SkScalar ascent) {
  switch (decoration) {
    case TextDecoration::kUnderline:
      ...
      fPosition -= ascent;
      break;
    case TextDecoration::kOverline: fPosition = 0; break;
    case TextDecoration::kLineThrough: {
      fPosition =
          (fFontMetrics.fFlags & SkFontMetrics::FontMetricsFlags::kStrikeoutThicknessIsValid_Flag)
...
void Decorations::calculateWaves(const TextStyle& textStyle, SkRect clip) {
  ...
  if (remaining > 0) {
    ...
    double x2 = remaining;
    double y2 =
        (remaining - remaining * remaining / (quarterWave * 2)) * (wave_count % 2 == 0 ? -1 : 1);
