...
Run::Run(
    ParagraphImpl* master, const SkShaper::RunHandler::RunInfo& info, SkScalar lineHeight,
    size_t index, SkScalar offsetX)
    : fMaster(master),
...
SkScalar Run::calculateWidth(size_t start, size_t end, bool clip) const {
  ...
  auto correction =
      end > start ? fMaster->posShift(fIndex, end - 1) - fMaster->posShift(fIndex, start) : 0;
...
std::tuple<bool, ClusterIndex, ClusterIndex> Run::findLimitingClusters(TextRange text) {
  ...
  if (text.width() == 0) {
    auto found = std::lower_bound(
        fMaster->clusters().begin() + fClusterRange.start,
        fMaster->clusters().begin() + fClusterRange.end, Cluster(text), less);
    return std::make_tuple(
        found != nullptr, found - fMaster->clusters().begin(), found - fMaster->clusters().begin());
  }

  auto start = std::lower_bound(
      fMaster->clusters().begin() + fClusterRange.start,
      fMaster->clusters().begin() + fClusterRange.end,
      Cluster(TextRange(text.start, text.start + 1)), less);
  auto end = std::lower_bound(
      start, fMaster->clusters().begin() + fClusterRange.end,
      Cluster(TextRange(text.end - 1, text.end)), less);
  if (!leftToRight()) {
    std::swap(start, end);
  }
...
void Run::iterateThroughClustersInTextOrder(const ClusterVisitor& visitor) {
  ...
  if (leftToRight()) {
    ...
    for (size_t glyph = 1; glyph <= this->size(); ++glyph) {
      ...
      visitor(
          start, glyph, cluster, nextCluster, this->calculateWidth(start, glyph, glyph == size()),
    ...
    for (int32_t start = this->size() - 1; start >= 0; --start) {
      ...
      visitor(
          start, glyph, cluster, nextCluster, this->calculateWidth(start, glyph, glyph == 0),
          this->calculateHeight());
