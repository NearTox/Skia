...
Run::Run(
    ...
    SkScalar heightMultiplier, bool useHalfLeading, SkScalar baselineShift, size_t index,
    SkScalar offsetX)
    : fOwner(owner),
  ...
  fClusterIndexes[info.glyphCount] =
...
// Adjust the text to grapheme edges so the first grapheme start is in the text and the last
// grapheme start is in the text It actually means that the first grapheme is entirely in the text
// and the last grapheme does not have to be
...
void Run::updateMetrics(InternalLineMetrics* endlineMetrics) {
  ...
  switch (placeholderStyle->fAlignment) {
    ...
    case PlaceholderAlignment::kTop: fFontMetrics.fDescent = height + fFontMetrics.fAscent; break;
