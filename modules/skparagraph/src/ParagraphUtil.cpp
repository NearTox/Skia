...
SkString SkStringFromU16String(const std::u16string& utf16text) {
  ...
  status = U_ZERO_ERROR;
  u_strToUTF8(
      dst.writable_str(), dst.size(), nullptr, (UChar*)utf16text.data(), SkToS32(utf16text.size()),
      &status);
  if (U_FAILURE(status)) {
...
bool isControl(SkUnichar utf8) noexcept { return u_iscntrl(utf8); }

bool isWhitespace(SkUnichar utf8) noexcept { return u_isWhitespace(utf8); }
