...
TextLine::TextLine(
    ParagraphImpl* owner, SkVector offset, SkVector advance, BlockRange blocks,
...
void TextLine::ensureTextBlobCachePopulated() {
  ...
  if (fBlockRange.width() == 1 && fRunsInVisualOrder.size() == 1 && fEllipsis == nullptr &&
    ...
    auto clip = SkRect::MakeXYWH(
        0.0f, this->sizes().runTop(&run, this->fAscentStyle), fAdvance.fX,
    ...
    GlyphRange glyphs;
    if (run.leftToRight()) {
      glyphs = GlyphRange(start.startPos(), end.isHardBreak() ? end.startPos() : end.endPos());
    } else {
      glyphs = GlyphRange(end.startPos(), start.isHardBreak() ? start.startPos() : start.endPos());
    }
    ClipContext context = {
        /*run=*/&run,
        /*pos=*/glyphs.start,
...
SkScalar TextLine::metricsWithoutMultiplier(TextHeightBehavior correction) {
  ...
  this->iterateThroughVisualRuns(
      true, [&result](const Run* run, SkScalar runOffset, TextRange textRange, SkScalar* width) {
...
void TextLine::buildTextBlob(
  ...
  SkScalar correctedBaseline =
      SkScalarFloorToScalar(this->baseline() + style.getBaselineShift() + 0.5);
  ...
  record.fOffset =
      SkPoint::Make(this->offset().fX + context.fTextShift, this->offset().fY + correctedBaseline);
}
...
void TextLine::paintBackground(
    SkCanvas* canvas, SkScalar x, SkScalar y, TextRange textRange, const TextStyle& style,
    const ClipContext& context) const {
  if (style.hasBackground()) {
    canvas->drawRect(
        context.clip.makeOffset(this->offset() + SkPoint::Make(x, y)), style.getBackground());
  }
}

void TextLine::paintShadow(
    SkCanvas* canvas, SkScalar x, SkScalar y, TextRange textRange, const TextStyle& style,
    const ClipContext& context) const {
  SkScalar correctedBaseline =
      SkScalarFloorToScalar(this->baseline() + style.getBaselineShift() + 0.5);
  ...
  for (TextShadow shadow : style.getShadows()) {
    ...
    if (shadow.fBlurSigma != 0.0) {
      auto filter =
          SkMaskFilter::MakeBlur(kNormal_SkBlurStyle, SkDoubleToScalar(shadow.fBlurSigma), false);
    ...
    canvas->drawTextBlob(
        blob, x + this->offset().fX + shadow.fOffset.x() + context.fTextShift,
        y + this->offset().fY + shadow.fOffset.y() + correctedBaseline, paint);
    if (context.clippingNeeded) {
      canvas->restore();
    }
  }
...
void TextLine::paintDecorations(
    SkCanvas* canvas, SkScalar x, SkScalar y, TextRange textRange, const TextStyle& style,
    const ClipContext& context) const {
...
void TextLine::createEllipsis(SkScalar maxWidth, const SkString& ellipsis, bool) {
  ...
  iterateThroughClustersInGlyphsOrder(false, false, [&](const Cluster* cluster, bool ghost) {
    if (cluster->run().leftToRight()) {
      return false;
    }
  ...
  iterateThroughClustersInGlyphsOrder(true, false, [&](const Cluster* cluster, bool ghost) {
    // Shape the ellipsis if the run has changed
...
std::unique_ptr<Run> TextLine::shapeEllipsis(const SkString& ellipsis, const Run& run) {
  class ShapeHandler final : public SkShaper::RunHandler {
   public:
    ShapeHandler(
        SkScalar lineHeight, bool useHalfLeading, SkScalar baselineShift, const SkString& ellipsis)
        : fRun(nullptr),
   ...
   private:
    ...
    Buffer runBuffer(const RunInfo& info) override {
      SkASSERT(!fRun);
      fRun = std::make_unique<Run>(
          nullptr, info, 0, fLineHeight, fUseHalfLeading, fBaselineShift, 0, 0);
  ...
  shaper->shape(
      ellipsis.c_str(), ellipsis.size(), run.font(), true, std::numeric_limits<SkScalar>::max(),
      &handler);
...
TextLine::ClipContext TextLine::measureTextInsideOneRun(
    ...
    bool includeGhostSpaces, bool limitToGraphemes) const {
  ...
  if (run->fEllipsis) {
    ...
    result.clip = SkRect::MakeXYWH(
        runOffsetInLine, sizes().runTop(run, this->fAscentStyle), run->advance().fX,
        run->calculateHeight(this->fAscentStyle, this->fDescentStyle));
    return result;
    ...
    if (SkScalarIsFinite(run->fFontMetrics.fAscent)) {
      ...
      result.clip =
          SkRect::MakeXYWH(runOffsetInLine, run->fFontMetrics.fAscent, run->advance().fX, 0);
    }
    return result;
  }
  ...
  do {
    ...
    // SkDebugf("Correct range: [%d:%d) -> [%d:%d)\n", textRange.start, textRange.end, startIndex,
    // endIndex);
  ...
  */
  ...
  result.clip = SkRect::MakeXYWH(
      0, sizes().runTop(run, this->fAscentStyle),
  ...
  if (compareRound(result.clip.fRight, fAdvance.fX) > 0 && !includeGhostSpaces) {
    ...
    // The most important one: we measure the text with spaces at the end (or at the beginning in
    // RTL) and we should ignore these spaces
    if (run->leftToRight()) {
...
void TextLine::iterateThroughClustersInGlyphsOrder(
    bool reversed, bool includeGhosts, const ClustersVisitor& visitor) const {
...
SkScalar TextLine::iterateThroughSingleRunByStyles(
    const Run* run, SkScalar runOffset, TextRange textRange, StyleType styleType,
    const RunStyleVisitor& visitor) const {
  if (run->fEllipsis) {
    // Extra efforts to get the ellipsis text style
    ClipContext clipContext =
        this->measureTextInsideOneRun(run->textRange(), run, runOffset, 0, false, true);
  ...
  if (styleType == StyleType::kNone) {
    ClipContext clipContext =
        this->measureTextInsideOneRun(textRange, run, runOffset, 0, false, true);
  ...
  for (BlockIndex index = 0; index <= blockRangeSize; ++index) {
    ...
    // Measure the text
    ClipContext clipContext = this->measureTextInsideOneRun(
        runStyleTextRange, run, runOffset, textOffsetInRun, false, true);
...
void TextLine::iterateThroughVisualRuns(
    bool includingGhostSpaces, const RunVisitor& visitor) const {
  ...
  for (auto& runIndex : fRunsInVisualOrder) {
    ...
    if (!run->leftToRight() && runOffset == 0 && includingGhostSpaces) {
      ...
      if (whitespaces.width() > 0) {
        auto whitespacesLen =
            measureTextInsideOneRun(whitespaces, run, runOffset, 0, true, false).clip.width();
...
SkVector TextLine::offset() const { return fOffset + SkVector::Make(fShift, 0); }
...
bool TextLine::isFirstLine() { return this == &fOwner->lines().front(); }

bool TextLine::isLastLine() { return this == &fOwner->lines().back(); }
...
bool TextLine::endsWithHardLineBreak() const {
  ...
  //  To be removed...
  return fOwner->cluster(fGhostClusterRange.end - 1).isHardBreak() || fEllipsis != nullptr ||
         fGhostClusterRange.end == fOwner->clusters().size() - 1;
}

void TextLine::getRectsForRange(
    TextRange textRange0, RectHeightStyle rectHeightStyle, RectWidthStyle rectWidthStyle,
  ...
  this->iterateThroughVisualRuns(
      ...
      [textRange0, rectHeightStyle, rectWidthStyle, &boxes, &lastRun, startBox, this](
        ...
        *runWidthInLine = this->iterateThroughSingleRunByStyles(
            ...
            [run, runOffsetInLine, textRange0, rectHeightStyle, rectWidthStyle, &boxes, &lastRun,
             startBox, this](
                TextRange textRange, const TextStyle& style,
              ...
              auto context =
                  this->measureTextInsideOneRun(intersect, run, runOffsetInLine, 0, true, true);
              ...
              if (this->trimmedText()
                      .end<
                          this->textWithNewlines()
                              .end&&  // Line has trailing space
                          this->textWithNewlines()
                              .end == intersect
                                          .end&&  // Range is at the end of the line
                                      this->trimmedText()
                                          .end>
                          intersect.start)  // Range has more than just spaces
              {
                ...
                if (paragraphStyle.getTextAlign() == TextAlign::kJustify && isLastLine()) {
                  ...
                  if (run->leftToRight()) {
                    ...
                    paragraphStyle.getTextDirection() == TextDirection::kRtl &&
                    !run->leftToRight()) {
                  // Split
                  trailingSpaces = clip;
                  ...
                  clip.fLeft += delta;
                    ...
                    paragraphStyle.getTextDirection() == TextDirection::kLtr &&
                    run->leftToRight()) {
                  // Split
                  trailingSpaces = clip;
                  trailingSpaces.fLeft = this->width();
              ...
              if (rectWidthStyle == RectWidthStyle::kMax && !isLastLine()) {
                ...
                if (right.direction == TextDirection::kLtr && right.rect.fRight >= lineEnd &&
      ...
      [this, dx, &result](
        ...
        *runWidthInLine = this->iterateThroughSingleRunByStyles(
            ...
            [this, run, dx, &result, &keepLooking](
                TextRange textRange, const TextStyle& style,
                const TextLine::ClipContext& context0) {
              ...
              if (run->leftToRight()) {
                context.clip.fRight +=
                    context.fExcludedTrailingSpaces;  // extending clip to the right
              } else {
              ...
              if (dx <= context.clip.fLeft + offsetX) {
                ...
                auto utf16Index =
                    fOwner->getUTF16Index(context.run->globalClusterIndex(context.pos));
                if (run->leftToRight()) {
              ...
              for (size_t index = context.pos; index < context.pos + context.size; ++index) {
                ...
                auto end =
                    littleRound(context.run->positionX(index) + context.fTextShift + offsetX);
                if (end > dx) {
                  break;
                } else if (end == dx && !context.run->leftToRight()) {
                  // When we move RTL variable end points to the beginning of the code point which
                  // is included
                  found = index;
                  break;
                }
              ...
              SkScalar glyphemePosLeft =
                  context.run->positionX(found) + context.fTextShift + offsetX;
              SkScalar glyphemePosWidth =
                  context.run->positionX(found + 1) - context.run->positionX(found);
