...
TextLine::TextLine(
    ...
    TextRange textWithSpaces, ClusterRange clusters, LineMetrics sizes)
    : fMaster(master),
      fBlockRange(blocks),
      fTextRange(text),
...
SkScalar TextLine::paintText(
    SkCanvas* canvas, TextRange textRange, const TextStyle& style, SkScalar offsetX) const {
  SkPaint paint;
  ...
  return this->iterateThroughRuns(
      textRange, offsetX, false,
      [canvas, paint, shiftDown](
          Run* run, int32_t pos, size_t size, SkRect clip, SkScalar shift, bool clippingNeeded) {
...
SkScalar TextLine::paintBackground(
    SkCanvas* canvas, TextRange textRange, const TextStyle& style, SkScalar offsetX) const {
  return this->iterateThroughRuns(
      textRange, offsetX, false,
      [canvas, &style](
...
SkScalar TextLine::paintDecorations(
    SkCanvas* canvas, TextRange textRange, const TextStyle& style, SkScalar offsetX) const {
  return this->iterateThroughRuns(
      textRange, offsetX, false,
      [this, canvas, &style](
        ...
        for (auto decoration : AllTextDecorations) {
          ...
          switch (style.getDecorationType()) {
            case TextDecoration::kUnderline: position = -run->ascent() + thickness; break;
  ...
  switch (style.getDecorationStyle()) {
    ...
    case TextDecorationStyle::kWavy: {
      ...
      while (x_start + wavelength * 2 < width) {
        path.rQuadTo(wavelength, wave_count % 2 != 0 ? wavelength : -wavelength, wavelength * 2, 0);
...
void TextLine::justify(SkScalar maxWidth) {
  ...
  this->iterateThroughClustersInGlyphsOrder(
      false,
      [&whitespacePatches, &textLen, &whitespacePatch](const Cluster* cluster, ClusterIndex index) {
  ...
  class ShapeHandler final : public SkShaper::RunHandler {
   ...
   private:
    ...
    Buffer runBuffer(const RunInfo& info) override {
      fRun = fEllipsisCache.set(info.fFont, Run(nullptr, info, fLineHeight, 0, 0));
...
SkRect TextLine::measureTextInsideOneRun(
  ...
  SkRect clip = SkRect::MakeXYWH(
      run->positionX(start->startPos()) - run->positionX(0), sizes().runTop(run),
      run->calculateWidth(start->startPos(), end->endPos(), needsClipping), run->calculateHeight());
...
void TextLine::iterateThroughClustersInGlyphsOrder(
    bool reverse, const ClustersVisitor& visitor) const {
...
SkScalar TextLine::iterateThroughRuns(
    TextRange textRange, SkScalar runOffset, bool includeEmptyText,
...
void TextLine::iterateThroughStylesInTextOrder(
    StyleType styleType, const StyleVisitor& visitor) const {
