...
bool FontResolver::isEmpty() { return fFontIterator == fFontSwitches.end(); }
...
void FontResolver::findAllFontsForStyledBlock(const TextStyle& style, TextRange textRange) {
  ...
  for (auto& fontFamily : style.getFontFamilies()) {
    auto typeface =
        fFontCollection->matchTypeface(fontFamily.c_str(), style.getFontStyle(), style.getLocale());
...
void FontResolver::setLastResortFont() {
  ...
  for (auto& style : fStyles) {
    for (auto& fontFamily : style.fStyle.getFontFamilies()) {
      typeface = fFontCollection->matchTypeface(
          fontFamily.c_str(), style.fStyle.getFontStyle(), style.fStyle.getLocale());
      if (typeface.get() != nullptr) {
  ...
  if (typeface == nullptr) {
    for (auto& fontFamily : fDefaultStyle.getFontFamilies()) {
      typeface = fFontCollection->matchTypeface(
          fontFamily.c_str(), fDefaultStyle.getFontStyle(), fDefaultStyle.getLocale());
      if (typeface.get() != nullptr) {
  ...
  if (typeface == nullptr) {
    ...
    typeface =
        fFontCollection->defaultFallback(0, foundStyle.getFontStyle(), foundStyle.getLocale());
  }
