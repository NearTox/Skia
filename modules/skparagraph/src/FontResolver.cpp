...
void FontResolver::findAllFontsForStyledBlock(const TextStyle& style, TextRange textRange) {
  ...
  if (fResolvedFonts.count() == 0) {
    auto result = fFontCollection->defaultFallback(
        firstUnresolved(), style.getFontStyle(), style.getLocale());
    if (result == nullptr) {
...
void FontResolver::findAllFontsForAllStyledBlocks(
    SkSpan<const char> utf8, SkSpan<Block> styles, sk_sp<FontCollection> fontCollection) {
  ...
  for (auto& block : fStyles) {
    SkASSERT(combined.fRange.empty() || combined.fRange.end == block.fRange.start);
