...
DEF_TEST(SkParagraph_FontFallbackParagraph, reporter) {
    ...
    REPORTER_ASSERT(
        reporter,
        SkScalarNearlyEqual(impl->runs()[2 + spaceRun].advance().fX, 139.125f, EPSILON100));
    REPORTER_ASSERT(
        reporter,
        SkScalarNearlyEqual(impl->runs()[3 + spaceRun].advance().fX, 27.999f, EPSILON100));
    REPORTER_ASSERT(
        reporter,
        SkScalarNearlyEqual(impl->runs()[4 + spaceRun].advance().fX, 62.248f, EPSILON100));
    REPORTER_ASSERT(
        reporter,
        SkScalarNearlyEqual(impl->runs()[5 + spaceRun].advance().fX, 27.999f, EPSILON100));
...
DEF_TEST(SkParagraph_RTLGlyphPositionsForTrailingSpaces, reporter) {
  ...
  auto test = [&](const char* text, int whitespaces) {
    ...
    for (int i = 0; i < SkToInt(str.size()); ++i) {
      ...
      // SkDebugf("@%f[%d]: %d %s\n", pointX, i, res.position, res.affinity == Affinity::kDownstream
      // ? "D" : "U");
      //  At the beginning there is a control codepoint that makes the string RTL
      REPORTER_ASSERT(
          reporter, (pos.position + i) == SkToInt(str.size() - (pos.position > 0 ? 0 : 1)));
      // The ending looks slightly different...
      REPORTER_ASSERT(
          reporter, pos.affinity == (i == 0 ? Affinity::kUpstream : Affinity::kDownstream));
    }
  };
