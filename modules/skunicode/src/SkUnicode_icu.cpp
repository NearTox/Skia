...
static inline UBreakIterator* sk_ubrk_clone(const UBreakIterator* bi, UErrorCode* status) {
  ...
  return icu->f_ubrk_clone_ ? icu->f_ubrk_clone_(bi, status)
                            : icu->f_ubrk_safeClone_(bi, nullptr, nullptr, status);
}

static void ubidi_close_wrapper(UBiDi* bidi) { sk_ubidi_close(bidi); }
static UText* utext_close_wrapper(UText* ut) { return sk_utext_close(ut); }
static void ubrk_close_wrapper(UBreakIterator* bi) { sk_ubrk_close(bi); }

using SkUnicodeBidi =
    std::unique_ptr<UBiDi, SkFunctionWrapper<decltype(ubidi_close), ubidi_close_wrapper>>;
using ICUUText =
    std::unique_ptr<UText, SkFunctionWrapper<decltype(utext_close), utext_close_wrapper>>;
using ICUBreakIterator =
    std::unique_ptr<UBreakIterator, SkFunctionWrapper<decltype(ubrk_close), ubrk_close_wrapper>>;
...
class SkBreakIterator_icu : public SkBreakIterator {
 ...
 public:
  ...
  bool setText(const char16_t utftext16[], int utf16Units) override {
    UErrorCode status = U_ZERO_ERROR;
    ICUUText text(sk_utext_openUChars(
        nullptr, reinterpret_cast<const UChar*>(&utftext16[0]), utf16Units, &status));
 ...
 public:
  ...
  ICUBreakIterator makeBreakIterator(SkUnicode::BreakType type) {
    ...
    ICUBreakIterator* cachedIterator;
    {
      ...
      if (!cachedIterator) {
        ICUBreakIterator newIterator(
            sk_ubrk_open(convertType(type), sk_uloc_getDefault(), nullptr, 0, &status));
        if (U_FAILURE(status)) {
          SkDEBUGF("Break error: %s", sk_u_errorName(status));
        } else {
...
class SkUnicode_icu : public SkUnicode {
  static bool extractBidi(
    ...
    // It appears it isn't used after ubidi_setPara except through ubidi_getText.
    sk_ubidi_setPara(
        bidi.get(), (const UChar*)utf16.get(), utf16Units, bidiLevel, nullptr, &status);
    if (U_FAILURE(status)) {
      SkDEBUGF("Bidi error: %s", sk_u_errorName(status));
      return false;
    }
  ...
  static bool extractPositions(
    ...
    while (pos != UBRK_DONE) {
      auto status =
          type == SkUnicode::BreakType::kLines ? UBRK_LINE_SOFT : sk_ubrk_getRuleStatus(iter);
      setBreak(pos, status);
 ...
 public:
  ...
  std::unique_ptr<SkBreakIterator> makeBreakIterator(
    ...
    UErrorCode status = U_ZERO_ERROR;
    ICUBreakIterator iterator(sk_ubrk_open(convertType(breakType), locale, nullptr, 0, &status));
    if (U_FAILURE(status)) {
      SkDEBUGF("Break error: %s", sk_u_errorName(status));
      return nullptr;
    }
  ...
  bool isControl(SkUnichar utf8) override { return sk_u_iscntrl(utf8); }

  bool isWhitespace(SkUnichar utf8) override { return sk_u_isWhitespace(utf8); }

  bool isSpace(SkUnichar utf8) override { return sk_u_isspace(utf8); }
  ...
  SkString toUpper(const SkString& str) override {
    ...
    const auto upper16len =
        sk_u_strToUpper(nullptr, 0, (UChar*)(str16.get()), str16len, nullptr, &icu_err);
    ...
    sk_u_strToUpper(
        (UChar*)(upper16.get()), SkToS32(upper16.size()), (UChar*)(str16.get()), str16len, nullptr,
        &icu_err);
    ...
    return upper8len >= 0 ? SkString(upper8.get(), upper8len) : SkString();
  }
...
std::unique_ptr<SkUnicode> SkUnicode::Make() {
  ...
  return ICULib() ? std::make_unique<SkUnicode_icu>() : nullptr;
}
