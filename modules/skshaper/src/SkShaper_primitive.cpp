...
static size_t linebreak(
    const char text[], const char stop[], const SkFont& font, SkScalar width, SkScalar* advance,
    size_t* trailing) {
...
void SkShaperPrimitive::shape(
  ...
  while (0 < utf8Bytes) {
    ...
    size_t bytesConsumed = linebreak(
        utf8, utf8 + utf8Bytes, font, width, advances.get() + glyphOffset, &bytesCollapsed);
