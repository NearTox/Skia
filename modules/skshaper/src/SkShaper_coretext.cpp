...
class SkShaper_CoreText : public SkShaper {
 ...
 private:
  ...
  void shape(
      ...
      LanguageRunIterator&, SkScalar width, RunHandler*) const override;

  void shape(
      const char* utf8, size_t utf8Bytes, FontRunIterator&, BiDiRunIterator&, ScriptRunIterator&,
      LanguageRunIterator&, const Feature*, size_t featureSize, SkScalar width,
      RunHandler*) const override;
};

std::unique_ptr<SkShaper> SkShaper::MakeCoreText() { return std::make_unique<SkShaper_CoreText>(); }

void SkShaper_CoreText::shape(
    const char* utf8, size_t utf8Bytes, FontRunIterator& font, BiDiRunIterator& bidi,
    ScriptRunIterator&, LanguageRunIterator&, SkScalar width, RunHandler* handler) const {
  SkFont skfont;
  if (!font.atEnd()) {
    font.consume();
...
void SkShaper_CoreText::shape(
    const char* utf8, size_t utf8Bytes, FontRunIterator& font, BiDiRunIterator& bidi,
    ScriptRunIterator&, LanguageRunIterator&, const Feature*, size_t, SkScalar width,
    RunHandler* handler) const {
  font.consume();
  SkASSERT(font.currentFont().getTypeface());
  bidi.consume();
  return this->shape(
      utf8, utf8Bytes, font.currentFont(), (bidi.currentLevel() % 2) == 0, width, handler);
}
...
class LineBreakIter {
 ...
 public:
  LineBreakIter(CTTypesetterRef ts, SkScalar width) : fTypesetter(ts), fWidth(width) { fStart = 0; }
...
void SkShaper_CoreText::shape(
    const char* utf8, size_t utf8Bytes, const SkFont& font, bool /* leftToRight */, SkScalar width,
    RunHandler* handler) const {
  ...
  AutoCF<CFStringRef> textString = CFStringCreateWithBytes(
      nullptr, (const uint8_t*)utf8, utf8Bytes, kCFStringEncodingUTF8, false);

  AutoCF<CTFontRef> ctfont = create_ctfont_from_font(font);

  AutoCF<CFMutableDictionaryRef> attr = CFDictionaryCreateMutable(
...
#else
std::unique_ptr<SkShaper> SkShaper::MakeCoreText() { return nullptr; }
#endif
