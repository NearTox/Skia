...
class IcuBiDiRunIterator final : public SkShaper::BiDiRunIterator {
 public:
  IcuBiDiRunIterator(const char* utf8, const char* end, ICUBiDi bidi) noexcept
      : fBidi(std::move(bidi)),
...
ShaperHarfBuzz::ShaperHarfBuzz(
    HBBuffer buffer, ICUBrk line, ICUBrk grapheme, sk_sp<SkFontMgr> fontmgr)
  ...
  while (runSegmenter.advanceRuns()) {  // For each item
    ...
    while (utf8Start < utf8End) {  // While there are still code points left in this item
      ...
      for (int32_t breakIteratorCurrent = ubrk_next(&breakIterator);
           breakIteratorCurrent != UBRK_DONE; breakIteratorCurrent = ubrk_next(&breakIterator)) {
  ...
  SkTArray<ShapedRun> runs;
  {
    ...
    while (runSegmenter.advanceRuns()) {
      ...
      for (size_t i = 0; i < run.fNumGlyphs; ++i) {
        ...
        while (lineBreakIteratorCurrent != UBRK_DONE && lineBreakIteratorCurrent < glyphCluster) {
...
std::unique_ptr<SkShaper> SkShaper::MakeShapeDontWrapOrReorder(sk_sp<SkFontMgr> fontmgr) {
  ...
  return std::make_unique<ShapeDontWrapOrReorder>(
      std::move(buffer), nullptr, nullptr, std::move(fontmgr));
}
