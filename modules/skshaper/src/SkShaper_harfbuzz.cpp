...
HBBlob stream_to_blob(std::unique_ptr<SkStreamAsset> asset) {
    ...
    if (const void* base = asset->getMemoryBase()) {
        blob.reset(hb_blob_create((char*)base, SkToUInt(size), HB_MEMORY_MODE_READONLY,
                                  asset.release(), [](void* p) { delete (SkStreamAsset*)p; }));
    } else {
        // SkDebugf("Extra SkStreamAsset copy\n");
        void* ptr = size ? sk_malloc_throw(size) : nullptr;
        asset->read(ptr, size);
        blob.reset(
                hb_blob_create((char*)ptr, SkToUInt(size), HB_MEMORY_MODE_READONLY, ptr, sk_free));
    }
    SkASSERT(blob);
...
unsigned skhb_nominal_glyphs(hb_font_t* hb_font, void* font_data, unsigned int count,
                             const hb_codepoint_t* unicodes, unsigned int unicode_stride,
                             hb_codepoint_t* glyphs, unsigned int glyph_stride, void* user_data) {
    SkFont& font = *reinterpret_cast<SkFont*>(font_data);
    ...
    font.textToGlyphs(unicode.get(), count * sizeof(SkUnichar), SkTextEncoding::kUTF32, glyph.get(),
                      count);
...
hb_blob_t* skhb_get_table(hb_face_t* face, hb_tag_t tag, void* user_data) {
    ...
    return hb_blob_create(reinterpret_cast<char*>(buffer), tableSize, HB_MEMORY_MODE_WRITABLE,
                          buffer, sk_free);
}
...
private:
    ...
    static bool CompareRunIterator(SkShaper::RunIterator* const& a,
                                   SkShaper::RunIterator* const& b) {
...
void emit(const ShapedLine& line, SkShaper::RunHandler* handler) {
    ...
    for (int i = 0; i < numRuns; ++i) {
        ...
        const SkShaper::RunHandler::RunInfo info = {run.fFont, run.fLevel, run.fAdvance,
...
struct ShapedRunGlyphIterator {
    ...
    bool operator==(const ShapedRunGlyphIterator& that) const {
        return fRuns == that.fRuns && fRunIndex == that.fRunIndex &&
...
protected:
    ...
    ShapedRun shape(const char* utf8, size_t utf8Bytes, const char* utf8Start, const char* utf8End,
...
private:
    ...
    void shape(const char* utf8, size_t utf8Bytes, const SkFont&, bool leftToRight, SkScalar width,
               RunHandler*) const override;

    void shape(const char* utf8Text, size_t textBytes, FontRunIterator&, BiDiRunIterator&,
               ScriptRunIterator&, LanguageRunIterator&, SkScalar width,
               RunHandler*) const override;

    virtual void wrap(char const* const utf8, size_t utf8Bytes, const BiDiRunIterator&,
                      const LanguageRunIterator&, const ScriptRunIterator&, const FontRunIterator&,
                      RunIteratorQueue& runSegmenter, SkScalar width, RunHandler*) const = 0;
};
...
private:
    void wrap(char const* const utf8, size_t utf8Bytes, const BiDiRunIterator&,
              const LanguageRunIterator&, const ScriptRunIterator&, const FontRunIterator&,
              RunIteratorQueue& runSegmenter, SkScalar width, RunHandler*) const override;
};
...
static std::unique_ptr<SkShaper> MakeHarfBuzz(bool correct) {
    ...
    if (correct) {
        ...
        return skstd::make_unique<ShapeThenWrap>(std::move(buffer), std::move(lineBreakIterator),
...
void ShaperHarfBuzz::shape(const char* utf8, size_t utf8Bytes, const SkFont& srcFont,
                           bool leftToRight, SkScalar width, RunHandler* handler) const {
    SkASSERT(handler);
...
void ShaperHarfBuzz::shape(const char* utf8, size_t utf8Bytes, FontRunIterator& font,
                           BiDiRunIterator& bidi, ScriptRunIterator& script,
                           LanguageRunIterator& language, SkScalar width,
...
void ShaperDrivenWrapper::wrap(char const* const utf8, size_t utf8Bytes,
                               const BiDiRunIterator& bidi, const LanguageRunIterator& language,
                               const ScriptRunIterator& script, const FontRunIterator& font,
                               RunIteratorQueue& runSegmenter, SkScalar width,
                               RunHandler* handler) const {
    ShapedLine line;
    ...
    while (runSegmenter.advanceRuns()) {  // For each item
        ...
        while (utf8Start < utf8End) {  // While there are still code points left in this item
            ...
            UBreakIterator& breakIterator = *fLineBreakIterator;
            {
                ...
                std::unique_ptr<UText, SkFunctionWrapper<UText*, UText, utext_close>> autoClose(
                        &utf8UText);
                if (U_FAILURE(status)) {
            ...
            for (int32_t breakIteratorCurrent = ubrk_next(&breakIterator);
                ...
                ShapedRun candidate = [&](const TextProps& props) {
                    if (props.glyphLen) {
                        ...
                        return shape(utf8, utf8Bytes, utf8Start, utf8Start + breakIteratorCurrent,
...
void ShapeThenWrap::wrap(char const* const utf8, size_t utf8Bytes, const BiDiRunIterator& bidi,
                         const LanguageRunIterator& language, const ScriptRunIterator& script,
                         const FontRunIterator& font, RunIteratorQueue& runSegmenter,
    ...
    SkTArray<ShapedRun> runs;
    {
        ...
        while (runSegmenter.advanceRuns()) {
            ...
            runs.emplace_back(
                    shape(utf8, utf8Bytes, utf8Start, utf8End, bidi, language, script, font));
...
ShapedRun ShaperHarfBuzz::shape(char const* const utf8,
    ...
    size_t utf8runLength = utf8End - utf8Start;
    ShapedRun run(RunHandler::Range(utf8Start - utf8, utf8runLength), font.currentFont(),
                  bidi.currentLevel(), nullptr, 0);
    ...
    run = ShapedRun(RunHandler::Range(utf8Start - utf8, utf8runLength), font.currentFont(),
                    bidi.currentLevel(), std::unique_ptr<ShapedGlyph[]>(new ShapedGlyph[len]), len);
    int scaleX, scaleY;
