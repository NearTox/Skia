...
using resource = std::unique_ptr<T, SkFunctionWrapper<skstd::remove_pointer_t<decltype(P)>, P>>;
...
hb_position_t skhb_glyph_h_advance(
    hb_font_t* hb_font, void* font_data, hb_codepoint_t codepoint, void* user_data) {
  SkFont& font = *reinterpret_cast<SkFont*>(font_data);
...
hb_bool_t skhb_glyph_extents(
    hb_font_t* hb_font, void* font_data, hb_codepoint_t codepoint, hb_glyph_extents_t* extents,
    void* user_data) {
  SkFont& font = *reinterpret_cast<SkFont*>(font_data);
...
hb_blob_t* skhb_get_table(hb_face_t* face, hb_tag_t tag, void* user_data) {
  ...
  return hb_blob_create(
      ...
      rawData, [](void* ctx) { ((SkData*)ctx)->unref(); });
}
...
class RunIteratorQueue {
 public:
  void insert(SkShaper::RunIterator* runIterator) { fRunIterators.insert(runIterator); }
  ...
  size_t endOfCurrentRun() const { return fRunIterators.peek()->endOfCurrentRun(); }
...
void append(
  ...
  for (size_t i = 0; i < glyphLen; i++) {
    ...
    if (buffer.offsets) {
      ...
      buffer.positions[i] =
          advance + buffer.point + glyph.fOffset;  // TODO: invert glyph.fOffset.fY?
    }
    if (buffer.clusters) {
...
void emit(const ShapedLine& line, SkShaper::RunHandler* handler) {
  ...
  for (int i = 0; i < numRuns; ++i) {
    ...
    const SkShaper::RunHandler::RunInfo info = {run.fFont, run.fLevel, run.fAdvance, run.fNumGlyphs,
                                                run.fUtf8Range};
...
class ShaperDrivenWrapper : public ShaperHarfBuzz {
 ...
 private:
  void wrap(
      char const* const utf8, size_t utf8Bytes, const BiDiRunIterator&, const LanguageRunIterator&,
      const ScriptRunIterator&, const FontRunIterator&, RunIteratorQueue& runSegmenter,
      SkScalar width, RunHandler*) const override;
};
...
void ShaperDrivenWrapper::wrap(
    ...
    const FontRunIterator& font, RunIteratorQueue& runSegmenter, SkScalar width,
    RunHandler* handler) const {
  ShapedLine line;
  ...
  while (runSegmenter.advanceRuns()) {  // For each item
    ...
    while (utf8Start < utf8End) {  // While there are still code points left in this item
      ...
      if (modelNeedsRegenerated) {
        model = shape(utf8, utf8Bytes, utf8Start, utf8End, bidi, language, script, font);
        modelGlyphOffset = 0;
...
void ShapeThenWrap::wrap(
  ...
  SkTArray<ShapedRun> runs;
  {
    ...
    while (runSegmenter.advanceRuns()) {
      ...
      runs.emplace_back(shape(utf8, utf8Bytes, utf8Start, utf8End, bidi, language, script, font));
  ...
  // Reorder the runs and glyphs per line and write them out.
  {
    ...
    while (glyphIterator.current()) {
      ...
      for (int i = 0; i < numRuns; ++i) {
        ...
        const RunHandler::RunInfo info = {run.fFont, run.fLevel, run.fAdvance, run.fNumGlyphs,
                                          run.fUtf8Range};
        handler->runInfo(info);
      }
      handler->commitRunInfo();
      for (int i = 0; i < numRuns; ++i) {
        ...
        size_t endGlyphIndex = (logicalIndex == runIndex) ? glyphIndex + 1 : run.fNumGlyphs;
...
ShapedRun ShaperHarfBuzz::shape(
    ...
    const ScriptRunIterator& script, const FontRunIterator& font) const {
  size_t utf8runLength = utf8End - utf8Start;
  ShapedRun run(
...
std::unique_ptr<SkShaper> SkShaper::MakeShapeDontWrapOrReorder(sk_sp<SkFontMgr> fontmgr) {
  ...
  return skstd::make_unique<ShapeDontWrapOrReorder>(
