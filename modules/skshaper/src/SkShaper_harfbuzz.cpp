...
hb_bool_t skhb_glyph(
    hb_font_t* hb_font, void* font_data, hb_codepoint_t unicode, hb_codepoint_t variation_selector,
...
hb_bool_t skhb_nominal_glyph(
    hb_font_t* hb_font, void* font_data, hb_codepoint_t unicode, hb_codepoint_t* glyph,
    void* user_data) {
  return skhb_glyph(hb_font, font_data, unicode, 0, glyph, user_data);
}

unsigned skhb_nominal_glyphs(
    hb_font_t* hb_font, void* font_data, unsigned int count, const hb_codepoint_t* unicodes,
    unsigned int unicode_stride, hb_codepoint_t* glyphs, unsigned int glyph_stride,
    void* user_data) {
  SkFont& font = *reinterpret_cast<SkFont*>(font_data);
  ...
  font.textToGlyphs(
      unicode.get(), count * sizeof(SkUnichar), SkTextEncoding::kUTF32, glyph.get(), count);
...
hb_position_t skhb_glyph_h_advance(
    hb_font_t* hb_font, void* font_data, hb_codepoint_t hbGlyph, void* user_data) {
  SkFont& font = *reinterpret_cast<SkFont*>(font_data);
...
void skhb_glyph_h_advances(
    hb_font_t* hb_font, void* font_data, unsigned count, const hb_codepoint_t* glyphs,
    unsigned int glyph_stride, hb_position_t* advances, unsigned int advance_stride,
    void* user_data) {
  SkFont& font = *reinterpret_cast<SkFont*>(font_data);
...
hb_bool_t skhb_glyph_extents(
    hb_font_t* hb_font, void* font_data, hb_codepoint_t hbGlyph, hb_glyph_extents_t* extents,
    void* user_data) {
  SkFont& font = *reinterpret_cast<SkFont*>(font_data);
  SkASSERT(extents);
...
#define SK_HB_VERSION_CHECK(x, y, z)                                                 \
  (HB_VERSION_MAJOR > (x)) || (HB_VERSION_MAJOR == (x) && HB_VERSION_MINOR > (y)) || \
...
hb_blob_t* skhb_get_table(hb_face_t* face, hb_tag_t tag, void* user_data) {
  ...
  return hb_blob_create(
      reinterpret_cast<char*>(rawData->writable_data()), rawData->size(), HB_MEMORY_MODE_READONLY,
      rawData, [](void* ctx) { SkSafeUnref(((SkData*)ctx)); });
}
  ...
  if (axis_count > 0) {
    ...
    if (typeface.getVariationDesignPosition(axis_values, axis_count) == axis_count) {
      hb_font_set_variations(
          otFont.get(), reinterpret_cast<hb_variation_t*>(axis_values.get()), axis_count);
    }
  }
...
HBFont create_sub_hb_font(const SkFont& font, const HBFont& typefaceFont) {
  SkDEBUGCODE(hb_face_t* face = hb_font_get_face(typefaceFont.get());
...
class SkUnicodeBidiRunIterator final : public SkShaper::BiDiRunIterator {
 public:
  ...
  bool atEnd() const override { return fUTF16LogicalPosition == fBidi->getLength(); }
  SkBidiIterator::Level currentLevel() const override { return fLevel; }
...
class RunIteratorQueue {
 public:
  ...
  size_t endOfCurrentRun() const { return fEntries.peek().runIterator->endOfCurrentRun(); }
...
struct ShapedRun {
  ShapedRun(
      SkShaper::RunHandler::Range utf8Range, const SkFont& font, SkBidiIterator::Level level,
...
constexpr bool is_LTR(SkBidiIterator::Level level) { return (level & 1) == 0; }
...
void append(
  ...
  for (size_t i = 0; i < glyphLen; i++) {
    ...
    const ShapedGlyph& glyph =
        run.fGlyphs[is_LTR(run.fLevel) ? startGlyphIndex + i : endGlyphIndex - 1 - i];
...
void emit(const ShapedLine& line, SkShaper::RunHandler* handler) {
  ...
  for (int i = 0; i < numRuns; ++i) {
    ...
    const SkShaper::RunHandler::RunInfo info = {
        run.fFont, run.fLevel, run.fAdvance, run.fNumGlyphs, run.fUtf8Range};
...
class ShaperHarfBuzz : public SkShaper {
 public:
  ShaperHarfBuzz(
      std::unique_ptr<SkUnicode>, SkUnicodeBreak line, SkUnicodeBreak grapheme, HBBuffer,
      sk_sp<SkFontMgr>);
 ...
 protected:
  ...
  ShapedRun shape(
      const char* utf8, size_t utf8Bytes, const char* utf8Start, const char* utf8End,
      const BiDiRunIterator&, const LanguageRunIterator&, const ScriptRunIterator&,
      const FontRunIterator&, const Feature*, size_t featuresSize) const;
 ...
 private:
  ...
  void shape(
      const char* utf8, size_t utf8Bytes, const SkFont&, bool leftToRight, SkScalar width,
      RunHandler*) const override;

  void shape(
      const char* utf8Text, size_t textBytes, FontRunIterator&, BiDiRunIterator&,
      ScriptRunIterator&, LanguageRunIterator&, SkScalar width, RunHandler*) const override;

  void shape(
      const char* utf8Text, size_t textBytes, FontRunIterator&, BiDiRunIterator&,
      ScriptRunIterator&, LanguageRunIterator&, const Feature*, size_t featuresSize, SkScalar width,
      RunHandler*) const override;

  virtual void wrap(
      char const* const utf8, size_t utf8Bytes, const BiDiRunIterator&, const LanguageRunIterator&,
      const ScriptRunIterator&, const FontRunIterator&, RunIteratorQueue& runSegmenter,
      const Feature*, size_t featuresSize, SkScalar width, RunHandler*) const = 0;
};
...
class ShaperDrivenWrapper : public ShaperHarfBuzz {
 ...
 private:
  void wrap(
      char const* const utf8, size_t utf8Bytes, const BiDiRunIterator&, const LanguageRunIterator&,
      const ScriptRunIterator&, const FontRunIterator&, RunIteratorQueue& runSegmenter,
      const Feature*, size_t featuresSize, SkScalar width, RunHandler*) const override;
};
...
static std::unique_ptr<SkShaper> MakeHarfBuzz(sk_sp<SkFontMgr> fontmgr, bool correct) {
  ...
  if (correct) {
    return std::make_unique<ShaperDrivenWrapper>(
        std::move(unicode), std::move(lineIter), std::move(graphIter), std::move(buffer),
        std::move(fontmgr));
  } else {
...
ShaperHarfBuzz::ShaperHarfBuzz(
    std::unique_ptr<SkUnicode> unicode, SkUnicodeBreak lineIter, SkUnicodeBreak graphIter,
    HBBuffer buffer, sk_sp<SkFontMgr> fontmgr)
...
void ShaperHarfBuzz::shape(
  ...
  std::unique_ptr<BiDiRunIterator> bidi(
      MakeSkUnicodeBidiRunIterator(fUnicode.get(), utf8, utf8Bytes, defaultLevel));
  ...
  std::unique_ptr<FontRunIterator> font(MakeFontMgrRunIterator(
      utf8, utf8Bytes, srcFont, fFontMgr ? fFontMgr : SkFontMgr::RefDefault()));
  if (!font) {
    return;
  }
...
void ShaperHarfBuzz::shape(
    const char* utf8, size_t utf8Bytes, FontRunIterator& font, BiDiRunIterator& bidi,
    ScriptRunIterator& script, LanguageRunIterator& language, SkScalar width,
    RunHandler* handler) const {
...
void ShaperHarfBuzz::shape(
    const char* utf8, size_t utf8Bytes, FontRunIterator& font, BiDiRunIterator& bidi,
    ScriptRunIterator& script, LanguageRunIterator& language, const Feature* features,
    size_t featuresSize, SkScalar width, RunHandler* handler) const {
  SkASSERT(handler);
  ...
  this->wrap(
      utf8, utf8Bytes, bidi, language, script, font, runSegmenter, features, featuresSize, width,
      handler);
}

void ShaperDrivenWrapper::wrap(
    char const* const utf8, size_t utf8Bytes, const BiDiRunIterator& bidi,
    const LanguageRunIterator& language, const ScriptRunIterator& script,
    const FontRunIterator& font, RunIteratorQueue& runSegmenter, const Feature* features,
    size_t featuresSize, SkScalar width, RunHandler* handler) const {
  ShapedLine line;
  ...
  while (runSegmenter.advanceRuns()) {  // For each item
    ...
    while (utf8Start < utf8End) {  // While there are still code points left in this item
      ...
      if (modelNeedsRegenerated) {
        model = shape(
            utf8, utf8Bytes, utf8Start, utf8End, bidi, language, script, font, features,
            featuresSize);
        modelGlyphOffset = 0;
      ...
      ShapedRun best(
          RunHandler::Range(), SkFont(), 0, nullptr, 0,
          {SK_ScalarNegativeInfinity, SK_ScalarNegativeInfinity});
      bool bestIsInvalid = true;
      bool bestUsesModelForGlyphs = false;
      SkScalar widthLeft = width - line.fAdvance.fX;

      for (int32_t breakIteratorCurrent = breakIterator.next(); !breakIterator.isDone();
        ...
        ShapedRun candidate = [&](const TextProps& props) {
          if (props.glyphLen) {
            ...
            return ShapedRun(
                RunHandler::Range(utf8Start - utf8, breakIteratorCurrent), font.currentFont(),
                bidi.currentLevel(), std::unique_ptr<ShapedGlyph[]>(),
                props.glyphLen - modelGlyphOffset, props.advance - modelAdvanceOffset);
          } else {
            return shape(
                utf8, utf8Bytes, utf8Start, utf8Start + breakIteratorCurrent, bidi, language,
...
void ShapeThenWrap::wrap(
  ...
  SkTArray<ShapedRun> runs;
  {
    ...
    while (runSegmenter.advanceRuns()) {
      ...
      for (size_t i = 0; i < run.fNumGlyphs; ++i) {
        ...
        glyph.fMayLineBreakBefore =
            glyph.fCluster != previousCluster && lineBreakIteratorCurrent == glyphCluster;
        ...
        glyph.fGraphemeBreakBefore =
            glyph.fCluster != previousCluster && graphemeBreakIteratorCurrent == glyphCluster;
  ...
  // Iterate over the glyphs in logical order to find potential line lengths.
  {
    ...
    while (ShapedGlyph* glyph = current.current()) {
      ...
      if (width < currentWidth + glyphWidth && glyph->fHasVisual &&
  ...
  // Reorder the runs and glyphs per line and write them out.
  {
    ...
    while (glyphIterator.current()) {
      ...
      auto makeSubRun = [&runs, &previousBreak, &current, &logicalFromVisual](size_t visualIndex) {
        ...
        size_t startGlyphIndex =
            (logicalIndex == previousBreak.fRunIndex) ? previousBreak.fGlyphIndex : 0;
        size_t endGlyphIndex =
      ...
      auto makeRunInfo = [](const SubRun& sub) {
        ...
        return RunHandler::RunInfo{
            sub.run.fFont, sub.run.fLevel, advance, sub.endGlyphIndex - sub.startGlyphIndex,
...
void ShapeDontWrapOrReorder::wrap(
    char const* const utf8, size_t utf8Bytes, const BiDiRunIterator& bidi,
    const LanguageRunIterator& language, const ScriptRunIterator& script,
    const FontRunIterator& font, RunIteratorQueue& runSegmenter, const Feature* features,
    size_t featuresSize, SkScalar width, RunHandler* handler) const {
  ...
  while (runSegmenter.advanceRuns()) {
    ...
    runs.emplace_back(shape(
        utf8, utf8Bytes, utf8Start, utf8End, bidi, language, script, font, features, featuresSize));
  }
...
class HBLockedFaceCache {
 public:
  ...
  ~HBLockedFaceCache() { fMutex.release(); }

  HBFont* find(SkTypefaceID fontId) { return fLRUCache.find(fontId); }
  HBFont* insert(SkTypefaceID fontId, HBFont hbFont) {
    return fLRUCache.insert(fontId, std::move(hbFont));
  }
  void reset() { fLRUCache.reset(); }
...
ShapedRun ShaperHarfBuzz::shape(
    char const* const utf8, size_t const utf8Bytes, char const* const utf8Start,
    char const* const utf8End, const BiDiRunIterator& bidi, const LanguageRunIterator& language,
    const ScriptRunIterator& script, const FontRunIterator& font, Feature const* const features,
    size_t const featuresSize) const {
  size_t utf8runLength = utf8End - utf8Start;
  ShapedRun run(
      RunHandler::Range(utf8Start - utf8, utf8runLength), font.currentFont(), bidi.currentLevel(),
      nullptr, 0);
  ...
  for (const auto& feature : SkMakeSpan(features, featuresSize)) {
    ...
    if (feature.start <= SkTo<size_t>(utf8Start - utf8) &&
      ...
      hbFeatures.push_back(
          {(hb_tag_t)feature.tag, feature.value, HB_FEATURE_GLOBAL_START, HB_FEATURE_GLOBAL_END});
    } else {
      hbFeatures.push_back(
          {(hb_tag_t)feature.tag, feature.value, SkTo<unsigned>(feature.start),
  ...
  for (unsigned i = 0; i < len; i++) {
    ...
    glyph.fHasVisual =
        !glyphBounds[i].isEmpty();  //! font->currentTypeface()->glyphBoundsAreZero(glyph.fID);
...
std::unique_ptr<SkShaper::BiDiRunIterator> SkShaper::MakeSkUnicodeBidiRunIterator(
    SkUnicode* unicode, const char* utf8, size_t utf8Bytes, uint8_t bidiLevel) {
