...
class SkUnicode_icu : public SkUnicode {
  static UBreakIteratorType convertType(UBreakType type) noexcept {
    switch (type) {
  ...
  static int convertUtf8ToUtf16(
      const char* utf8, size_t utf8Units, std::unique_ptr<uint16_t[]>* utf16) {
  ...
  static bool extractBidi(
      const char utf8[], int utf8Units, Direction dir, std::vector<BidiRegion>* bidiRegions) {
 ...
 public:
  ...
  bool getBidiRegions(
      const char utf8[], int utf8Units, Direction dir, std::vector<BidiRegion>* results) override {
    return extractBidi(utf8, utf8Units, dir, results);
  }

  bool getLineBreaks(
      const char utf8[], int utf8Units, std::vector<LineBreakBefore>* results) override {
    return extractPositions(utf8, utf8Units, UBreakType::kLines, [results](int pos, int status) {
  ...
  bool getGraphemes(const char utf8[], int utf8Units, std::vector<Position>* results) override {
    return extractPositions(
        utf8, utf8Units, UBreakType::kGraphemes,
  ...
  void reorderVisual(
      const BidiLevel runLevels[], int levelsCount, int32_t logicalFromVisual[]) noexcept override {
