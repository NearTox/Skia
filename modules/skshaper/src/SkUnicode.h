...
enum class UtfFormat { kUTF8, kUTF16 };
// Bidi
...
struct BidiRegion {
  BidiRegion(Position start, Position end, BidiLevel level) noexcept
...
// LineBreaks
enum class LineBreakType { kSoftLineBreak, kHardLineBreak };
struct LineBreakBefore {
  LineBreakBefore(Position pos, LineBreakType breakType) noexcept
      : pos(pos), breakType(breakType) {}
  Position pos;
  LineBreakType breakType;
};
// Other breaks
enum class UBreakType { kWords, kGraphemes, kLines };
struct Range {
  Position start;
  Position end;
};
...
class SKUNICODE_API SkUnicode {
 public:
  ...
  virtual bool getGraphemes(const char utf8[], int utf8Units, std::vector<Position>* results) = 0;
  virtual bool getWhitespaces(const char utf8[], int utf8Units, std::vector<Position>* results) = 0;

  virtual void reorderVisual(
      const BidiLevel runLevels[], int levelsCount, int32_t logicalFromVisual[]) = 0;
