...
std::tuple<SkScalar, SkScalar> find_yintercept(
    const SkPoint& pos, const SkVector& spd, const SkRect& box) {
...
void update_pos(const sk_sp<sksg::RRect>& rr, const SkPoint& pos) {
  ...
  const auto offsetX = pos.x() - r.x(), offsetY = pos.y() - r.y();
...
class PongView final : public Sample {
 ...
 protected:
  void onOnceBeforeDraw() override {
    ...
    const SkRRect paddle = SkRRect::MakeRectXY(
        SkRect::MakeWH(kPaddleSize.width(), kPaddleSize.height()), kPaddleSize.width() / 2,
        kPaddleSize.width() / 2);
    fBall.initialize(
        ball, SkPoint::Make(kBounds.centerX(), kBounds.centerY()),
        SkVector::Make(
            fRand.nextRangeScalar(kBallSpeedMin, kBallSpeedMax),
            fRand.nextRangeScalar(kBallSpeedMin, kBallSpeedMax)));
    fPaddle0.initialize(
        paddle, SkPoint::Make(fieldBounds.left() - kPaddleSize.width() / 2, fieldBounds.centerY()),
        SkVector::Make(0, 0));
    fPaddle1.initialize(
        paddle, SkPoint::Make(fieldBounds.right() + kPaddleSize.width() / 2, fieldBounds.centerY()),
        SkVector::Make(0, 0));
    ...
    for (uint32_t i = 0; i < kBackgroundDashCount; ++i) {
      bgPath.moveTo(
          kBounds.centerX(), kBounds.top() + (i + 0.25f) * kBounds.height() / kBackgroundDashCount);
      bgPath.lineTo(
          kBounds.centerX(), kBounds.top() + (i + 0.75f) * kBounds.height() / kBackgroundDashCount);
    }
    ...
    fContentMatrix = sksg::Matrix<SkMatrix>::Make(SkMatrix::MakeRectToRect(
  ...
  bool onChar(SkUnichar uni) override {
    switch (uni) {
      case '[': fTimeScale = SkTPin(fTimeScale - 0.1f, kTimeScaleMin, kTimeScaleMax); return true;
      case ']': fTimeScale = SkTPin(fTimeScale + 0.1f, kTimeScaleMin, kTimeScaleMax); return true;
      case 'I': fShowInval = !fShowInval; return true;
      default: break;
    }
    return false;
  }
  ...
  void onSizeChange() override {
    if (fContentMatrix) {
      fContentMatrix->setMatrix(SkMatrix::MakeRectToRect(
          SkRect::MakeWH(1, 1), SkRect::MakeIWH(this->width(), this->height()),
          SkMatrix::kFill_ScaleToFit));
    }
 ...
 private:
  struct Object {
    ...
    void posTick(SkScalar dt) { pos += spd * dt; }
