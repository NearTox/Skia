...
static void check_inval(
    ...
    const SkRect& expected_inval_bounds, const std::vector<SkRect>* expected_damage) {
  sksg::InvalidationController ic;
  const auto bbox = root->revalidate(&ic, SkMatrix::I());

  if (0) {
    SkDebugf(
        "** bbox: [%f %f %f %f], ibbox: [%f %f %f %f]\n", bbox.fLeft, bbox.fTop, bbox.fRight,
        bbox.fBottom, ic.bounds().left(), ic.bounds().top(), ic.bounds().right(),
        ic.bounds().bottom());
  }
  ...
  if (expected_damage) {
    ...
    for (size_t i = 0; i < std::min(expected_damage->size(), damage_count); ++i) {
      const auto r1 = (*expected_damage)[i], r2 = ic.begin()[i];
      if (0) {
        SkDebugf(
            "*** expected inval: [%f %f %f %f], actual: [%f %f %f %f]\n", r1.left(), r1.top(),
            r1.right(), r1.bottom(), r2.left(), r2.top(), r2.right(), r2.bottom());
      }
...
static void inval_test1(skiatest::Reporter* reporter) {
  ...
  auto d1 = sksg::Draw::Make(r1, color), d2 = sksg::Draw::Make(r2, color);
...

  {
    ...
    check_inval(reporter, root, SkRect::MakeWH(100, 100), SkRectPriv::MakeLargeS32(), nullptr);
...

  {
    ...
    check_inval(reporter, root, SkRect::MakeWH(300, 200), SkRect::MakeWH(300, 200), &damage);
...

  {
    ...
    check_inval(reporter, root, SkRect::MakeWH(300, 200), SkRect::MakeWH(100, 100), &damage);
...

  {
    ...
    check_inval(reporter, root, SkRect::MakeWH(600, 400), SkRect::MakeWH(600, 400), &damage);
...

  {
    ...
    check_inval(
        reporter, root, SkRect::MakeWH(500, 400), SkRect::MakeLTRB(400, 200, 600, 400), &damage);
...
static void inval_test2(skiatest::Reporter* reporter) {
  ...
  auto t1 = sksg::TransformEffect::Make(
           sksg::Draw::Make(rect, color), sksg::Transform::MakeConcat(m1, m2)),
...

  {
    ...
    check_inval(reporter, root, SkRect::MakeWH(100, 100), SkRect::MakeWH(100, 100), &damage);
  }

  {
    ...
    check_inval(reporter, root, SkRect::MakeWH(200, 200), SkRect::MakeWH(200, 200), &damage);
  }
...
static void inval_test3(skiatest::Reporter* reporter) {
  auto color1 = sksg::Color::Make(0xff000000), color2 = sksg::Color::Make(0xff000000);
  auto group = sksg::Group::Make();

  group->addChild(sksg::Draw::Make(sksg::Rect::Make(SkRect::MakeWH(100, 100)), color1));
  group->addChild(sksg::Draw::Make(sksg::Rect::Make(SkRect::MakeXYWH(200, 0, 100, 100)), color2));
...

  {
    // Initial revalidation.
    check_inval(
        reporter, root, SkRect::MakeXYWH(0, 0, 350, 175), SkRectPriv::MakeLargeS32(), nullptr);
  }

  {
    ...
    check_inval(
        reporter, root, SkRect::MakeLTRB(50, 75, 350, 175), SkRect::MakeLTRB(0, 0, 350, 175),
        &damage);
  }
...
static void inval_group_remove(skiatest::Reporter* reporter) {
  auto draw = sksg::Draw::Make(
      sksg::Rect::Make(SkRect::MakeWH(100, 100)), sksg::Color::Make(SK_ColorBLACK));
  auto grp = sksg::Group::Make();
