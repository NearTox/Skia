...
static void check_inval(skiatest::Reporter* reporter, const sk_sp<sksg::Node>& root,
                        const SkRect& expected_bounds, const SkRect& expected_inval_bounds,
                        const std::vector<SkRect>* expected_damage) {
    sksg::InvalidationController ic;
    const auto bbox = root->revalidate(&ic, SkMatrix::I());

    if (0) {
        SkDebugf("** bbox: [%f %f %f %f], ibbox: [%f %f %f %f]\n", bbox.fLeft, bbox.fTop,
                 bbox.fRight, bbox.fBottom, ic.bounds().left(), ic.bounds().top(),
                 ic.bounds().right(), ic.bounds().bottom());
    }
        ...
        for (size_t i = 0; i < std::min(expected_damage->size(), damage_count); ++i) {
            ...
            if (0) {
                SkDebugf("*** expected inval: [%f %f %f %f], actual: [%f %f %f %f]\n", r1.left(),
                         r1.top(), r1.right(), r1.bottom(), r2.left(), r2.top(), r2.right(),
                         r2.bottom());
            }
...
static void inval_test1(skiatest::Reporter* reporter) {
...

    {
        ...
        check_inval(reporter, root, SkRect::MakeWH(500, 400), SkRect::MakeLTRB(400, 200, 600, 400),
                    &damage);
...
static void inval_test2(skiatest::Reporter* reporter) {
...

    {
        ...
        check_inval(reporter, root, SkRect::MakeLTRB(100, 100, 300, 300),
                    SkRect::MakeLTRB(0, 0, 300, 300), &damage);
    }

    {
        ...
        check_inval(reporter, root, SkRect::MakeLTRB(100, 100, 200, 300),
                    SkRect::MakeLTRB(100, 100, 300, 300), &damage);
    }
}
...
static void inval_test3(skiatest::Reporter* reporter) {
    ...
    {
        // Initial revalidation.
        check_inval(reporter, root, SkRect::MakeXYWH(0, 0, 350, 175), SkRectPriv::MakeLargeS32(),
                    nullptr);
    }

    {
        ...
        check_inval(reporter, root, SkRect::MakeLTRB(50, 75, 350, 175),
                    SkRect::MakeLTRB(0, 0, 350, 175), &damage);
    }

    {
        ...
        check_inval(reporter, root, SkRect::MakeLTRB(50, 75, 350, 175),
                    SkRect::MakeLTRB(50, 75, 350, 175), &damage);
    }
}
