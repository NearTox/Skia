...
void Animator::tick(float t) { this->onTick(t); }

GroupAnimator::GroupAnimator(AnimatorList&& animators) : fAnimators(std::move(animators)) {}
...
Scene::Scene(sk_sp<RenderNode> root, AnimatorList&& animators)
        : fRoot(std::move(root)), fAnimators(std::move(animators)) {}
...
const RenderNode* Scene::nodeAt(const SkPoint& p) const { return fRoot->nodeAt(p); }
