...
void RenderNode::RenderContext::modulatePaint(
    const SkMatrix& ctm, SkPaint* paint, bool is_layer_paint) const {
  ...
  if (!is_layer_paint && fMaskShader) {
    paint->setShader(SkShaders::Blend(
        SkBlendMode::kSrcIn, LocalShader(fMaskShader, fMaskCTM, ctm), paint->refShader()));
  }
}
...
RenderNode::ScopedRenderContext&& RenderNode::ScopedRenderContext::modulateColorFilter(
...
RenderNode::ScopedRenderContext&& RenderNode::ScopedRenderContext::modulateMaskShader(
    sk_sp<SkShader> ms, const SkMatrix& ctm) {
  if (fCtx.fMaskShader) {
    ...
    if (ms && fCtx.fMaskCTM.invert(&invMaskCTM)) {
      ...
      fCtx.fMaskShader = SkShaders::Blend(
          SkBlendMode::kSrcIn, std::move(fCtx.fMaskShader),
...
RenderNode::ScopedRenderContext&& RenderNode::ScopedRenderContext::setIsolation(
    const SkRect& bounds, const SkMatrix& ctm, bool isolation) {
