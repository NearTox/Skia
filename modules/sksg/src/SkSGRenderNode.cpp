...
bool RenderNode::RenderContext::requiresIsolation() const {
  ...
  return ScaleAlpha(SK_AlphaOPAQUE, fOpacity) != SK_AlphaOPAQUE || fColorFilter || fMaskFilter ||
...
RenderNode::ScopedRenderContext&& RenderNode::ScopedRenderContext::modulateMaskFilter(
    sk_sp<SkMaskFilter> mf, const SkMatrix& ctm) {
  if (fCtx.fMaskFilter) {
