...
ColorFilter::ColorFilter(sk_sp<RenderNode> child) : INHERITED(std::move(child)) {}
...
sk_sp<ExternalColorFilter> ExternalColorFilter::Make(sk_sp<RenderNode> child) {
    return child ? sk_sp<ExternalColorFilter>(new ExternalColorFilter(std::move(child))) : nullptr;
}
...
sk_sp<ModeColorFilter> ModeColorFilter::Make(sk_sp<RenderNode> child, sk_sp<Color> color,
                                             SkBlendMode mode) {
    return (child && color) ? sk_sp<ModeColorFilter>(
                                      new ModeColorFilter(std::move(child), std::move(color), mode))
                            : nullptr;
}

ModeColorFilter::ModeColorFilter(sk_sp<RenderNode> child, sk_sp<Color> color, SkBlendMode mode)
        : INHERITED(std::move(child)), fColor(std::move(color)), fMode(mode) {
    this->observeInval(fColor);
}

ModeColorFilter::~ModeColorFilter() { this->unobserveInval(fColor); }
...
sk_sp<GradientColorFilter> GradientColorFilter::Make(sk_sp<RenderNode> child, sk_sp<Color> c0,
                                                     sk_sp<Color> c1) {
    return Make(std::move(child), {std::move(c0), std::move(c1)});
}

sk_sp<GradientColorFilter> GradientColorFilter::Make(
        sk_sp<RenderNode> child, std::vector<sk_sp<Color>> colors) {
    return (child && colors.size() > 1) ? sk_sp<GradientColorFilter>(new GradientColorFilter(
                                                  std::move(child), std::move(colors)))
                                        : nullptr;
}

GradientColorFilter::GradientColorFilter(sk_sp<RenderNode> child, std::vector<sk_sp<Color>> colors)
        : INHERITED(std::move(child)), fColors(std::move(colors)) {
...
sk_sp<SkColorFilter> Make2ColorGradient(const sk_sp<Color>& color0, const sk_sp<Color>& color1) {
    ...
    const auto dR = c1.fR - c0.fR, dG = c1.fG - c0.fG, dB = c1.fB - c0.fB;
...
sk_sp<SkColorFilter> MakeNColorGradient(const std::vector<sk_sp<Color>>& colors) {
    ...
    for (size_t i = 0; i < span_count; ++i) {
        ...
        const SkColor c0 = colors[i]->getColor(), c1 = colors[i + 1]->getColor();
        float r = SkColorGetR(c0), g = SkColorGetG(c0), b = SkColorGetB(c0);
        const float dR = (SkColorGetR(c1) - r) / span_size, dG = (SkColorGetG(c1) - g) / span_size,
