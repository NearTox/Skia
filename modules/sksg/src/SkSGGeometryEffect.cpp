...
GeometryEffect::GeometryEffect(sk_sp<GeometryNode> child) : fChild(std::move(child)) {
  SkASSERT(fChild);

  this->observeInval(fChild);
}

GeometryEffect::~GeometryEffect() { this->unobserveInval(fChild); }
...
bool GeometryEffect::onContains(const SkPoint& p) const { return fPath.contains(p.x(), p.y()); }

SkPath GeometryEffect::onAsPath() const { return fPath; }
...
GeometryTransform::~GeometryTransform() { this->unobserveInval(fTransform); }
