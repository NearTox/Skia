...
MaskFilter::MaskFilter(sk_sp<SkMaskFilter> mf)
    : INHERITED(kBubbleDamage_Trait), fMaskFilter(std::move(mf)) {}
...
sk_sp<SkMaskFilter> MaskFilter::onRevalidateMask() { return fMaskFilter; }
...
MaskFilterEffect::MaskFilterEffect(sk_sp<RenderNode> child, sk_sp<MaskFilter> mf)
    ...
    : INHERITED(std::move(child), kOverrideDamage_Trait), fMaskFilter(std::move(mf)) {
  this->observeInval(fMaskFilter);
}

MaskFilterEffect::~MaskFilterEffect() { this->unobserveInval(fMaskFilter); }
...
void MaskFilterEffect::onRender(SkCanvas* canvas, const RenderContext* ctx) const {
  const auto local_ctx =
      ScopedRenderContext(canvas, ctx)
          .modulateMaskFilter(
              fMaskFilter ? fMaskFilter->getMaskFilter() : nullptr, canvas->getTotalMatrix());
...
ShaderEffect::ShaderEffect(sk_sp<RenderNode> child, sk_sp<Shader> shader)
    : INHERITED(std::move(child)), fShader(std::move(shader)) {
  if (fShader) {
...
ImageFilterEffect::ImageFilterEffect(sk_sp<RenderNode> child, sk_sp<ImageFilter> filter)
    ...
    : INHERITED(std::move(child), kOverrideDamage_Trait), fImageFilter(std::move(filter)) {
  this->observeInval(fImageFilter);
}

ImageFilterEffect::~ImageFilterEffect() { this->unobserveInval(fImageFilter); }
...
ImageFilter::ImageFilter(std::unique_ptr<InputsT> inputs)
    : INHERITED(kBubbleDamage_Trait), fInputs(std::move(inputs)) {
  if (fInputs) {
...
sk_sp<SkImageFilter> DropShadowImageFilter::onRevalidateFilter() {
  ...
  return SkDropShadowImageFilter::Make(
      fOffset.x(), fOffset.y(), fSigma.x(), fSigma.y(), fColor, mode, this->refInput(0));
}
...
BlurImageFilter::BlurImageFilter(sk_sp<ImageFilter> input) : INHERITED(std::move(input)) {}
...
sk_sp<BlendModeEffect> BlendModeEffect::Make(sk_sp<RenderNode> child, SkBlendMode mode) {
  return child ? sk_sp<BlendModeEffect>(new BlendModeEffect(std::move(child), mode)) : nullptr;
}
