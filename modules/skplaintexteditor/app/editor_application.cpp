...
struct EditorLayer : public sk_app::Window::Layer {
  ...
  void setFont() {
    fEditor.setFont(SkFont(
        SkTypeface::MakeFromName(
            kTypefaces[fTypefaceIndex], SkFontStyle(kFontWeight, kFontWidth, kFontSlant)),
        fFontSize));
  }
  ...
  void loadFile(const char* path) {
    if (sk_sp<SkData> data = SkData::MakeFromFileName(path)) {
      fPath = path;
      fEditor.insert(Editor::TextPosition{0, 0}, (const char*)data->data(), data->size());
    } else {
      fPath = "output.txt";
    }
  }
  ...
  bool onKey(skui::Key key, skui::InputState state, skui::ModifierKey modifiers) override {
    ...
    if (!Any(ctrlAltCmd)) {
      ...
      switch (key) {
        ...
        case skui::Key::kDown:
        case skui::Key::kHome:
        case skui::Key::kEnd: return this->moveCursor(convert(key), shift);
      ...
      switch (key) {
        ...
        case skui::Key::kRight: return this->moveCursor(Editor::Movement::kWordRight, shift);
        default: break;
      }
    }
#ifdef SK_EDITOR_DEBUG_OUT
