...
void RunHandler::commitRunInfo() { fCurrentPosition.fY -= fMaxRunAscent; }
...
SkShaper::RunHandler::Buffer RunHandler::runBuffer(const RunInfo& info) {
  ...
  const auto& runBuffer = SkTextBlobBuilderPriv::AllocRunTextPos(
      &fBuilder, info.fFont, glyphCount, utf8RangeSize, SkString());
...
void RunHandler::commitRunBuffer(const RunInfo& info) {
  ...
  if (fCallbackFunction) {
    fCallbackFunction(
        fCallbackContext, fUtf8Text, info.utf8Range.end(), info.glyphCount, fCurrentGlyphs,
        fCurrentPoints, fClusters, info.fFont);
  }
...
sk_sp<SkTextBlob> RunHandler::makeBlob() { return fBuilder.make(); }
...
static void set_character_bounds(
    void* context, const char* utf8Text, size_t utf8TextBytes, size_t glyphCount,
    const SkGlyphID* glyphs, const SkPoint* positions, const uint32_t* clusters,
    const SkFont& font) {
  SkASSERT(context);
...
ShapeResult SkPlainTextEditor::Shape(
    const char* utf8Text, size_t textByteLen, const SkFont& font, const char* locale, float width) {
  ShapeResult result;
