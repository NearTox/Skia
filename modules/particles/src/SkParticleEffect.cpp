...
class SkRandomExternalValue : public SkParticleExternalValue {
 public:
  ...
  void read(int index, float* target) override { *target = fRandom[index].nextF(); }
};
...
void SkParticleEffect::processEffectSpawnRequests(double now) {
  for (const auto& spawnReq : fSpawnRequests) {
    sk_sp<SkParticleEffect> newEffect(new SkParticleEffect(std::move(spawnReq.fParams), fRandom));
    fRandom.nextU();

    newEffect->start(
        now, spawnReq.fLoop, fState.fPosition, fState.fHeading, fState.fScale, fState.fVelocity,
        fState.fSpin, fState.fColor, fState.fFrame, fState.fFlags);
...
void SkParticleEffect::runEffectScript(double now, const char* entry) {
  if (const auto& byteCode = fParams->fEffectProgram.fByteCode) {
    if (auto fun = byteCode->getFunction(entry)) {
      ...
      SkAssertResult(byteCode->run(
          fun, &fState.fAge, sizeof(EffectState) / sizeof(float), nullptr, 0,
...
void SkParticleEffect::processParticleSpawnRequests(double now, int start) {
  ...
  for (const auto& spawnReq : fSpawnRequests) {
    ...
    sk_sp<SkParticleEffect> newEffect(
        new SkParticleEffect(std::move(spawnReq.fParams), fParticles.fRandom[idx]));
    newEffect->start(
        now, spawnReq.fLoop,
        {data[SkParticles::kPositionX][idx], data[SkParticles::kPositionY][idx]},
        {data[SkParticles::kHeadingX][idx], data[SkParticles::kHeadingY][idx]},
        data[SkParticles::kScale][idx],
        {data[SkParticles::kVelocityX][idx], data[SkParticles::kVelocityY][idx]},
        data[SkParticles::kVelocityAngular][idx],
        {data[SkParticles::kColorR][idx], data[SkParticles::kColorG][idx],
         data[SkParticles::kColorB][idx], data[SkParticles::kColorA][idx]},
        data[SkParticles::kSpriteFrame][idx], float_to_bits(data[SkParticles::kFlags][idx]));
    fSubEffects.push_back(std::move(newEffect));
  }
  fSpawnRequests.reset();
}
...
void SkParticleEffect::runParticleScript(double now, const char* entry, int start, int count) {
  if (const auto& byteCode = fParams->fParticleProgram.fByteCode) {
    if (auto fun = byteCode->getFunction(entry)) {
      ...
      SkAssertResult(byteCode->runStriped(
          fun, count, args, SkParticles::kNumChannels, nullptr, 0, fParticleUniforms.data(),
...
void SkParticleEffect::advanceTime(double now) {
  ...
  // Do integration of effect position and orientation
  {
    ...
    float s = sk_float_sin(fState.fSpin * deltaTime), c = sk_float_cos(fState.fSpin * deltaTime);
    // Using setNormalize to prevent scale drift
    fState.fHeading.setNormalize(
        fState.fHeading.fX * c - fState.fHeading.fY * s,
  ...
  for (int i = 0; i < fCount; ++i) {
    ...
    float s = sk_float_sin(spin * deltaTime), c = sk_float_cos(spin * deltaTime);
