...
void SkParticleEffectParams::prepare(const skresources::ResourceProvider* resourceProvider) {
  ...
  auto buildProgram = [this](const SkSL::String& code, Program* p) {
    ...
    auto program =
        compiler.convertProgram(SkSL::Program::kGeneric_Kind, code, settings, &externalValues);
    if (!program) {
...
// Numerical Recipes
static uint32_t nr_rand(uint32_t x) { return x * 1664525 + 1013904223; }
...
void SkParticleEffect::processEffectSpawnRequests(double now) {
  for (const auto& spawnReq : fSpawnRequests) {
    ...
    newEffect->start(
        ...
        fState.fSpin, fState.fColor, fState.fFrame, fState.fFlags, fState.fRandom);
