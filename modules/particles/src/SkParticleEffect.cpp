...
static bool source_needs_frame(int source) {
  switch (source) {
    ...
    case SkParticleValue::kVelocityY_Source: return true;
...
float SkParticleValue::getSourceValue(
    const SkParticleUpdateParams& params, SkParticleState& ps) const {
...
float SkParticleValue::eval(const SkParticleUpdateParams& params, SkParticleState& ps) const {
  ...
  switch (fTileMode) {
    case kClamp_TileMode: v = SkTPin(v, 0.0f, 1.0f); break;
