...
SkParticleEffectParams::SkParticleEffectParams()
    : fMaxCount(128), fEffectDuration(1.0f), fRate(8.0f), fDrawable(nullptr), fCode(kDefaultCode) {
  this->rebuild();
}
  ...
  auto runProgram = [](const SkParticleEffectParams* params, const char* entry,
    ...
    if (const auto& byteCode = params->fByteCode) {
      ...
      SkAssertResult(byteCode->runStriped(
          byteCode->getFunction(entry), args, SkParticles::kNumChannels, count, updateParams, 2,
          nullptr, 0));
    }
  };
