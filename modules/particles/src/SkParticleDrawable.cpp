...
static inline SkRSXform make_rsxform(
    SkPoint ofs, float posX, float posY, float dirX, float dirY, float scale) {
...
class SkCircleDrawable : public SkParticleDrawable {
 public:
  ...
  void visitFields(SkFieldVisitor* v) override { v->visit("Radius", fRadius); }
...
class SkImageDrawable : public SkParticleDrawable {
 public:
  SkImageDrawable(
      const char* imagePath = "", const char* imageName = "", int cols = 1, int rows = 1)
  ...
  void draw(SkCanvas* canvas, const SkParticles& particles, int count) override {
    ...
    canvas->drawAtlas(
        fImage.get(), arrays.fXforms.get(), arrays.fRects.get(), arrays.fColors.get(), count,
        SkBlendMode::kModulate, SkSamplingOptions(SkFilterMode::kLinear),
        /*cullRect=*/nullptr,
        /*paint=*/nullptr);
  }
...
class SkSkottieDrawable : public SkParticleDrawable {
 public:
  SkSkottieDrawable(const char* animationPath = "", const char* animationName = "")
      : fPath(animationPath), fName(animationName) {}
  ...
  void draw(SkCanvas* canvas, const SkParticles& particles, int count) override {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      auto mat = SkMatrix::Translate(
                     particles.fData[SkParticles::kPositionX][i],
                     particles.fData[SkParticles::kPositionY][i]) *
                 SkMatrix::Scale(s, s) * SkMatrix::RotateRad(rads) *
...
sk_sp<SkParticleDrawable> SkParticleDrawable::MakeImage(
    const char* imagePath, const char* imageName, int cols, int rows) {
  return sk_sp<SkParticleDrawable>(new SkImageDrawable(imagePath, imageName, cols, rows));
}

sk_sp<SkParticleDrawable> SkParticleDrawable::MakeSkottie(
    const char* animPath, const char* animName, int cols, int rows) {
