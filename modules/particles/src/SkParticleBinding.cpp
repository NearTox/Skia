...
void SkParticleBinding::visitFields(SkFieldVisitor* v) { v->visit("Name", fName); }
...
class SkPathExternalFunction : public SkParticleExternalFunction {
 public:
  SkPathExternalFunction(
      const char* name, SkSL::Compiler& compiler, const LinearizedPath& path,
  ...
  void call(skvm::Builder* builder, skvm::F32* arguments, skvm::F32* outResult, skvm::I32 mask)
      const override {
    if (fPath.empty()) {
      return;
    }
...
class SkPathBinding : public SkParticleBinding {
 public:
  ...
  std::unique_ptr<SkParticleExternalFunction> toFunction(
      SkSL::Compiler& compiler, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) override {
    return std::make_unique<SkPathExternalFunction>(
        fName.c_str(), compiler, fData, uniforms, alloc);
  }
...
class SkShaderExternalFunction : public SkParticleExternalFunction {
 public:
  SkShaderExternalFunction(
      const char* name, SkSL::Compiler& compiler, sk_sp<SkShader> shader, skvm::Uniforms* uniforms,
      SkArenaAlloc* alloc)
      : SkParticleExternalFunction(
            name, compiler, *compiler.context().fTypes.fFloat4, uniforms, alloc),
  ...
  void call(skvm::Builder* builder, skvm::F32* arguments, skvm::F32* outResult, skvm::I32 mask)
    ...
    skvm::Color result = as_SB(fShader)->program(
        ...
        matrixProvider, /*localM=*/nullptr, colorInfo, fUniforms, fAlloc);
    SkASSERT(result);
...
class SkImageBinding : public SkParticleBinding {
 public:
  ...
  std::unique_ptr<SkParticleExternalFunction> toFunction(
      SkSL::Compiler& compiler, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) override {
    return std::make_unique<SkShaderExternalFunction>(
        fName.c_str(), compiler, fShader, uniforms, alloc);
  }
...
sk_sp<SkParticleBinding> SkParticleBinding::MakeImage(
    const char* name, const char* imagePath, const char* imageName) {
  return sk_sp<SkParticleBinding>(new SkImageBinding(name, imagePath, imageName));
}

sk_sp<SkParticleBinding> SkParticleBinding::MakePath(
    const char* name, const char* pathPath, const char* pathName) {
