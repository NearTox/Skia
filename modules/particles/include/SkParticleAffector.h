...
class SkParticleAffector : public SkReflected {
 public:
  ...
  static sk_sp<SkParticleAffector> MakeLinearVelocity(
      const SkCurve& angle, const SkCurve& strength, bool force, SkParticleFrame frame);
  static sk_sp<SkParticleAffector> MakeAngularVelocity(const SkCurve& strength, bool force);

  // Set the orientation of a particle, relative to the world, local, or velocity frame.
  static sk_sp<SkParticleAffector> MakeOrientation(const SkCurve& angle, SkParticleFrame frame);

  static sk_sp<SkParticleAffector> MakePointForce(
