...
class FakeWebFontProvider final : public skresources::ResourceProvider {
 public:
  ...
  sk_sp<SkTypeface> loadTypeface(const char[], const char[]) const override { return fTypeface; }
...
class SkottieColorizeGM : public skiagm::GM {
 public:
  SkottieColorizeGM(const char* name, const char* resource) : fName(name), fResource(resource) {}

 protected:
  SkString onShortName() override { return SkStringPrintf("skottie_colorize_%s", fName); }
 ...
 private:
  ...
  std::vector<skottie_utils::CustomPropertyManager::PropKey> fColorProps, fTextProps;
