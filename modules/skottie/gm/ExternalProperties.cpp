...
class SkottieExternalPropsGM : public skiagm::GM {
 public:
 protected:
  SkString onShortName() override { return SkString("skottie_external_props"); }
  ...
  DrawResult onDraw(SkCanvas* canvas, SkString* errorMsg) override {
    ...
    for (size_t i = 0; i < 4; ++i) {
      this->update_props(i);
      auto dest =
          SkRect::MakeWH(kSize / 2, kSize / 2).makeOffset(kSize * (i & 1) / 2, kSize * (i & 2) / 4);
