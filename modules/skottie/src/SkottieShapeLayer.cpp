...
sk_sp<sksg::GeometryNode> AttachRRectGeometry(const skjson::ObjectValue& jrect,
    ...
    auto p_attached = abuilder->bindProperty<VectorValue>(
            jrect["p"], ascope, [adapter](const VectorValue& p) {
                adapter->setPosition(ValueTraits<VectorValue>::As<SkPoint>(p));
            });
    auto s_attached = abuilder->bindProperty<VectorValue>(
            jrect["s"], ascope, [adapter](const VectorValue& s) {
...
sk_sp<sksg::GeometryNode> AttachEllipseGeometry(const skjson::ObjectValue& jellipse,
    ...
    auto p_attached = abuilder->bindProperty<VectorValue>(
            jellipse["p"], ascope, [adapter](const VectorValue& p) {
                adapter->setPosition(ValueTraits<VectorValue>::As<SkPoint>(p));
            });
    auto s_attached = abuilder->bindProperty<VectorValue>(
            jellipse["s"], ascope, [adapter](const VectorValue& s) {
...
sk_sp<sksg::GeometryNode> AttachPolystarGeometry(const skjson::ObjectValue& jstar,
    ...
    abuilder->bindProperty<ScalarValue>(jstar["or"], ascope, [adapter](const ScalarValue& otr) {
        adapter->setOuterRadius(otr);
    });
    abuilder->bindProperty<ScalarValue>(jstar["is"], ascope, [adapter](const ScalarValue& is) {
        adapter->setInnerRoundness(is);
    });
    abuilder->bindProperty<ScalarValue>(jstar["os"], ascope, [adapter](const ScalarValue& os) {
...
std::vector<sk_sp<sksg::GeometryNode>> AttachRoundGeometryEffect(
    ...
    for (auto& g : geos) {
        ...
        abuilder->bindProperty<ScalarValue>(
                jtrim["r"], ascope,
                [roundEffect](const ScalarValue& r) { roundEffect->setRadius(r); });
    }
...
std::vector<sk_sp<sksg::RenderNode>> AttachRepeaterDrawEffect(
    ...
    if (const skjson::ObjectValue* jtransform = jrepeater["tr"]) {
        ...
        abuilder->bindProperty<ScalarValue>(
                (*jtransform)["r"], ascope,
                [adapter](const ScalarValue& r) { adapter->setRotation(r); });
        abuilder->bindProperty<ScalarValue>(
    ...
    for (auto rec = recs.rbegin(); rec != recs.rend(); ++rec) {
        ...
        switch (rec->fInfo.fShapeType) {
            ...
            case ShapeType::kGroup: {
                AttachShapeContext groupShapeCtx(ctx->fScope, &geos, ctx->fGeometryEffectStack,
            ...
            case ShapeType::kPaint: {
                ...
                auto paint =
                        gPaintAttachers[rec->fInfo.fAttacherIndex](rec->fJson, this, ctx->fScope);
