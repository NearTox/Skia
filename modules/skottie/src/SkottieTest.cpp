...
DEF_TEST(Skottie_Properties, reporter) {
  ...
  class TestPropertyObserver final : public PropertyObserver {
   public:
    ...
    void onLeavingNode(const char node_name[], PropertyObserver::NodeType node_type) override {
      ...
      fCurrentNode = fCurrentNode.length() > length
    ...
    const std::vector<ColorInfo>& colorsWithFullKeypath() const { return fColorsWithFullKeypath; }
  ...
  REPORTER_ASSERT(
      reporter, transforms[0].handle->get() == skottie::TransformPropertyValue(
                                                   {SkPoint::Make(0, 0), SkPoint::Make(0, 0),
                                                    SkVector::Make(100, 100), 0, 0, 0}));
  REPORTER_ASSERT(reporter, transforms[1].node_name.equals("layer_1"));
  REPORTER_ASSERT(
      reporter, transforms[1].handle->get() == skottie::TransformPropertyValue(
                                                   {SkPoint::Make(0, 0), SkPoint::Make(25, 25),
                                                    SkVector::Make(100, 100), 0, 0, 0}));
  REPORTER_ASSERT(reporter, transforms[2].node_name.equals("shape_transform_0"));
  REPORTER_ASSERT(
      reporter, transforms[2].handle->get() == skottie::TransformPropertyValue(
                                                   {SkPoint::Make(0, 0), SkPoint::Make(0, 0),
                                                    SkVector::Make(50, 50), 0, 0, 0}));
...
static SkRect ComputeBlobBounds(const sk_sp<SkTextBlob>& blob) {
  ...
  for (SkTextBlobRunIterator it(blob.get()); !it.done(); it.next()) {
    ...
    for (uint32_t i = 0; i < it.glyphCount(); ++i) {
      bounds.join(glyphBounds[i].makeOffset(it.pos()[i * 2], it.pos()[i * 2 + 1]));
    }
  }
...
DEF_TEST(Skottie_Shaper_HAlign, reporter) {
  ...
  static constexpr struct {
    ...
    SkScalar l_selector, r_selector;
  } kTestAligns[] = {
  ...
  for (const auto& tsize : kTestSizes) {
    for (const auto& talign : kTestAligns) {
      ...
      REPORTER_ASSERT(
          reporter, std::fabs(shape_bounds.left() - expected_l) < tsize.tolerance, "%f %f %f %f %d",
          shape_bounds.left(), expected_l, tsize.tolerance, tsize.text_size, talign.align);

      const auto expected_r = text_point.x() + shape_bounds.width() * talign.r_selector;
      REPORTER_ASSERT(
          reporter, std::fabs(shape_bounds.right() - expected_r) < tsize.tolerance,
          "%f %f %f %f %d", shape_bounds.right(), expected_r, tsize.tolerance, tsize.text_size,
          talign.align);
    }
  }
}
...
DEF_TEST(Skottie_Shaper_VAlign, reporter) {
  ...
  struct {
    ...
    SkScalar topFactor;
      ...
      {skottie::Shaper::VAlign::kVisualTop, 0.0f}, {skottie::Shaper::VAlign::kVisualCenter, 0.5f},
  ...
  for (const auto& tsize : kTestSizes) {
    for (const auto& talign : kTestAligns) {
      ...
      REPORTER_ASSERT(
          reporter, std::fabs(shape_bounds.top() - expected_t) < tsize.tolerance, "%f %f %f %f %d",
          shape_bounds.top(), expected_t, tsize.tolerance, tsize.text_size, SkToU32(talign.align));

      const auto expected_b = text_box.bottom() - v_diff * (1 - talign.topFactor);
      REPORTER_ASSERT(
          reporter, std::fabs(shape_bounds.bottom() - expected_b) < tsize.tolerance,
          "%f %f %f %f %d", shape_bounds.bottom(), expected_b, tsize.tolerance, tsize.text_size,
...
DEF_TEST(Skottie_Shaper_ExplicitFontMgr, reporter) {
...

  {
    ...
    const auto shape_result =
        skottie::Shaper::Shape(SkString("foo\U000DFFFFbar"), desc, text_box, fontmgr);
...
DEF_TEST(Skottie_Image_Loading, reporter) {
  class TestResourceProvider final : public skresources::ResourceProvider {
   public:
    TestResourceProvider(
        sk_sp<skresources::ImageAsset> single_asset, sk_sp<skresources::ImageAsset> multi_asset)
   ...
   private:
    ...
    const sk_sp<skresources::ImageAsset> fSingleFrameAsset, fMultiFrameAsset;
  };

  auto make_animation = [&reporter](
                            sk_sp<skresources::ImageAsset> single_asset,
    ...
    auto animation = skottie::Animation::Builder(flags)
                         .setResourceProvider(sk_make_sp<TestResourceProvider>(
                             std::move(single_asset), std::move(multi_asset)))
                         .make(&stream);
  ...
  {
    auto single_asset = sk_make_sp<TestAsset>(false), multi_asset = sk_make_sp<TestAsset>(true);
