...
DEF_TEST(Skottie_Shaper_HAlign, reporter) {
    ...
    for (const auto& tsize : kTestSizes) {
        for (const auto& talign : kTestAligns) {
            ...
            REPORTER_ASSERT(reporter, std::fabs(shape_bounds.left() - expected_l) < tsize.tolerance,
...
DEF_TEST(Skottie_Shaper_VAlign, reporter) {
    ...
    for (const auto& tsize : kTestSizes) {
        for (const auto& talign : kTestAligns) {
            const skottie::Shaper::TextDesc desc = {
                    typeface,        tsize.text_size,
                    tsize.text_size, SkTextUtils::Align::kCenter_Align,
                    talign.align,
            };
            ...
            REPORTER_ASSERT(reporter, std::fabs(shape_bounds.top() - expected_t) < tsize.tolerance,
