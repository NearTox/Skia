...
DEF_TEST(Skottie_Properties, reporter) {
  ...
  REPORTER_ASSERT(
      reporter, transforms[0].handle->get() ==
                    skottie::TransformPropertyValue({SkPoint::Make(0, 0), SkPoint::Make(0, 0),
  ...
  REPORTER_ASSERT(
      reporter,
      texts[0].handle->get() ==
          skottie::TextPropertyValue({test_typeface, SkString("inline_text"), 100, 0, 120, 0,
                                      SkTextUtils::kLeft_Align, Shaper::VAlign::kTopBaseline,
                                      SkRect::MakeEmpty(), SK_ColorTRANSPARENT, SK_ColorTRANSPARENT,
                                      false, false}));
}
...
DEF_TEST(Skottie_Shaper_HAlign, reporter) {
  ...
  for (const auto& tsize : kTestSizes) {
    for (const auto& talign : kTestAligns) {
      const skottie::Shaper::TextDesc desc = {typeface,
                                              tsize.text_size,
                                              tsize.text_size,
                                              0,
                                              talign.align,
  ...
  for (const auto& tsize : kTestSizes) {
    for (const auto& talign : kTestAligns) {
      const skottie::Shaper::TextDesc desc = {
          typeface,     tsize.text_size,     tsize.text_size, 0, SkTextUtils::Align::kCenter_Align,
          talign.align, Shaper::Flags::kNone};
...
DEF_TEST(Skottie_Shaper_FragmentGlyphs, reporter) {
  skottie::Shaper::TextDesc desc = {
      SkTypeface::MakeDefault(), 18, 18, 0, SkTextUtils::Align::kCenter_Align, Shaper::VAlign::kTop,
      Shaper::Flags::kNone};
...
DEF_TEST(Skottie_Shaper_ExplicitFontMgr, reporter) {
  ...
  skottie::Shaper::TextDesc desc = {ToolUtils::create_portable_typeface(),
                                    18,
                                    18,
                                    0,
