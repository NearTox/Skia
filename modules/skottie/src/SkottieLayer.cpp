...
const MaskInfo* GetMaskInfo(char mode) {
  ...
  static constexpr MaskInfo k_int_info = {SkBlendMode::kSrcIn, sksg::Merge::Mode::kIntersect,
                                          false};
  // AE 'subtract' is the same as 'intersect' + inverted geometry
  // (draws the opacity-adjusted paint *outside* the shape).
  static constexpr MaskInfo k_sub_info = {SkBlendMode::kSrcIn, sksg::Merge::Mode::kIntersect, true};
  static constexpr MaskInfo k_dif_info = {SkBlendMode::kDifference, sksg::Merge::Mode::kDifference,
                                          false};
...
sk_sp<sksg::RenderNode> AttachMask(
    const skjson::ArrayValue* jmask, const AnimationBuilder* abuilder, AnimatorScope* ascope,
  ...
  for (const skjson::ObjectValue* m : *jmask) {
    ...
    const auto inverted = (mask_info->fInvertGeometry != ParseDefault<bool>((*m)["inv"], false));
    mask_path->setFillType(inverted ? SkPath::kInverseWinding_FillType : SkPath::kWinding_FillType);

    auto mask_paint = sksg::Color::Make(SK_ColorBLACK);
    mask_paint->setAntiAlias(true);
    // First mask in the stack initializes the mask buffer.
    mask_paint->setBlendMode(mask_stack.empty() ? SkBlendMode::kSrc : mask_info->fBlendMode);

    has_effect |= abuilder->bindProperty<ScalarValue>(
        (*m)["o"], ascope,
        [mask_paint](const ScalarValue& o) { mask_paint->setOpacity(o * 0.01f); }, 100.0f);

    static const VectorValue default_feather = {0, 0};
    if (abuilder->bindProperty<VectorValue>(
            (*m)["f"], ascope,
            ...
            default_feather)) {
      has_effect = true;
      mask_stack.push_back({mask_path, mask_paint, std::move(blur_effect), mask_info->fMergeMode});
  ...
  if (mask_stack.empty()) return childNode;
  ...
  const auto make_mask = [](const MaskRecord& rec) {
    auto mask = sksg::Draw::Make(std::move(rec.mask_path), std::move(rec.mask_paint));
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachNestedAnimation(
    const char* name, AnimatorScope* ascope) const {
  class SkottieSGAdapter final : public sksg::RenderNode {
   public:
    explicit SkottieSGAdapter(sk_sp<Animation> animation) : fAnimation(std::move(animation)) {
      SkASSERT(fAnimation);
    }
  ...
  class SkottieAnimatorAdapter final : public sksg::Animator {
   public:
    SkottieAnimatorAdapter(sk_sp<Animation> animation, float time_scale)
        : fAnimation(std::move(animation)), fTimeScale(time_scale) {
      SkASSERT(fAnimation);
    }
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachAssetRef(
    ...
    const std::function<sk_sp<sksg::RenderNode>(const skjson::ObjectValue&, AnimatorScope*)>& func)
    const {
  ...
  if (asset_info->fIsAttaching) {
    this->log(Logger::Level::kError, nullptr, "Asset cycle detected for: '%s'", refId.c_str());
    return nullptr;
  }
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachSolidLayer(
    const skjson::ObjectValue& jlayer, const LayerInfo&, AnimatorScope*) const {
  const auto size = SkSize::Make(
      ParseDefault<float>(jlayer["sw"], 0.0f), ParseDefault<float>(jlayer["sh"], 0.0f));
  const skjson::StringValue* hex_str = jlayer["sc"];
  uint32_t c;
  if (size.isEmpty() || !hex_str || *hex_str->begin() != '#' ||
  ...
  return sksg::Draw::Make(sksg::Rect::Make(SkRect::MakeSize(size)), std::move(solid_paint));
}

const AnimationBuilder::ImageAssetInfo* AnimationBuilder::loadImageAsset(
  ...
  const auto name_cstr = name->begin(), path_cstr = path ? path->begin() : "";
  ...
  if (!asset) {
    this->log(
        Logger::Level::kError, nullptr, "Could not load image asset: %s/%s.", path_cstr, name_cstr);
    return nullptr;
  }

  const auto size =
      SkISize::Make(ParseDefault<int>(jimage["w"], 0), ParseDefault<int>(jimage["h"], 0));
  return fImageAssetCache.set(res_id, {std::move(asset), size});
}

sk_sp<sksg::RenderNode> AnimationBuilder::attachImageAsset(
    const skjson::ObjectValue& jimage, const LayerInfo& layer_info, AnimatorScope* ascope) const {
  ...
  if (asset_info->fAsset->isMultiFrame()) {
    class MultiFrameAnimator final : public sksg::Animator {
     public:
      MultiFrameAnimator(
          sk_sp<ImageAsset> asset, sk_sp<sksg::Image> image_node, float time_bias, float time_scale)
    ...
    ascope->push_back(skstd::make_unique<MultiFrameAnimator>(
  ...
  return sksg::TransformEffect::Make(
      ...
      SkMatrix::MakeRectToRect(
          SkRect::Make(image->bounds()), SkRect::Make(asset_size), SkMatrix::kCenter_ScaleToFit));
}

sk_sp<sksg::RenderNode> AnimationBuilder::attachImageLayer(
    const skjson::ObjectValue& jlayer, const LayerInfo& layer_info, AnimatorScope* ascope) const {
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachNullLayer(
    const skjson::ObjectValue& layer, const LayerInfo&, AnimatorScope*) const {
...
struct AnimationBuilder::AttachLayerContext {
  ...
  sk_sp<sksg::Transform> attachLayerTransform(
    ...
    if (layer_index < 0) return nullptr;

    if (auto* m = fLayerMatrixMap.find(layer_index)) return *m;
 ...
 private:
  sk_sp<sksg::Transform> attachParentLayerTransform(
    ...
    if (parent_index < 0 || parent_index == layer_index) return nullptr;

    if (auto* m = fLayerMatrixMap.find(parent_index)) return *m;
  ...
  sk_sp<sksg::Transform> attachTransformNode(
    ...
    if (type == TransformType::kCamera) {
      ...
      abuilder->bindProperty<ScalarValue>(
          jlayer["pe"], fScope, [camera_adapter](const ScalarValue& pe) {
      ...
      return abuilder->attachMatrix3D(
          *jtransform, fScope, std::move(parent_transform), std::move(camera_adapter),
  ...
  sk_sp<sksg::Transform> attachLayerTransformImpl(
    ...
    return *fLayerMatrixMap.set(
        layer_index, this->attachTransformNode(jlayer, abuilder, std::move(parent_matrix), type));
  }
};

sk_sp<sksg::RenderNode> AnimationBuilder::attachLayer(
  ...
  const LayerInfo layer_info = {ParseDefault<float>((*jlayer)["ip"], 0.0f),
  ...
  if (type == kCameraLayerType) {
    ...
    if (layerCtx->fCameraTransform) {
      ...
      layerCtx->fCameraTransform =
          layerCtx->attachLayerTransform(*jlayer, this, AttachLayerContext::TransformType::kCamera);
    }
    return nullptr;
  }
  ...
  if (Parse<float>((*jlayer)["w"], &w) && Parse<float>((*jlayer)["h"], &h)) {
    layer = sksg::ClipEffect::Make(std::move(layer), sksg::Rect::Make(SkRect::MakeWH(w, h)), true);
  }
  ...
  class LayerController final : public sksg::GroupAnimator {
   public:
    LayerController(
        sksg::AnimatorList&& layer_animators, sk_sp<sksg::OpacityEffect> controlNode, float in,
        float out)
  ...
  layerCtx->fScope->push_back(skstd::make_unique<LayerController>(
      std::move(layer_animators), controller_node, layer_info.fInPoint, layer_info.fOutPoint));
  ...
  if (layerCtx->fCurrentMatte) {
    ...
    if (matteType < SK_ARRAY_COUNT(gMaskModes)) {
      return sksg::MaskEffect::Make(
          std::move(controller_node), std::move(layerCtx->fCurrentMatte), gMaskModes[matteType]);
    }
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachComposition(
    const skjson::ObjectValue& jcomp, AnimatorScope* scope) const {
