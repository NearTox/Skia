...
class KeyframeAnimatorBase : public sksg::Animator {
 ...
 protected:
  ...
  void parseKeyFrames(const skjson::ArrayValue& jframes, const AnimationBuilder* abuilder) {
    ...
    for (const skjson::ObjectValue* jframe : jframes) {
      ...
      float t0;
      if (!Parse<float>((*jframe)["t"], &t0)) continue;
      ...
      if (!fRecs.empty()) {
        if (fRecs.back().t1 >= t0) {
          abuilder->log(
              Logger::Level::kWarning, nullptr, "Ignoring out-of-order key frame (t:%f < t:%f).",
              t0, fRecs.back().t1);
          continue;
        }
 ...
 private:
  const KeyframeRec* findFrame(float t) const {
    ...
    auto f0 = &fRecs.front(), f1 = &fRecs.back();
...
class KeyframeAnimator final : public KeyframeAnimatorBase {
 ...
 protected:
  void onTick(float t) override { fApplyFunc(*this->eval(this->frame(t), t, &fScratch)); }

 private:
  KeyframeAnimator(
      const skjson::ArrayValue& jframes, const AnimationBuilder* abuilder,
      std::function<void(const T&)>&& apply)
...
template <typename T>
static inline bool BindPropertyImpl(
    const skjson::ObjectValue* jprop, const AnimationBuilder* abuilder, AnimatorScope* ascope,
    std::function<void(const T&)>&& apply, const T* noop = nullptr) {
...
class SplitPointAnimator final : public sksg::Animator {
 public:
  static std::unique_ptr<SplitPointAnimator> Make(
    ...
    if (!BindPropertyImpl<ScalarValue>(
            (*jprop)["x"], abuilder, &split_animator->fAnimators,
            [split_animator_ptr](const ScalarValue& x) { split_animator_ptr->setX(x); }) ||
        !BindPropertyImpl<ScalarValue>(
            (*jprop)["y"], abuilder, &split_animator->fAnimators,
 ...
 private:
  ...
  ScalarValue fX = 0, fY = 0;

  using INHERITED = sksg::Animator;
};

bool BindSplitPositionProperty(
...
template <>
bool AnimationBuilder::bindProperty(
    const skjson::Value& jv, AnimatorScope* ascope, std::function<void(const ScalarValue&)>&& apply,
...
template <>
bool AnimationBuilder::bindProperty(
    const skjson::Value& jv, AnimatorScope* ascope, std::function<void(const VectorValue&)>&& apply,
...
template <>
bool AnimationBuilder::bindProperty(
    const skjson::Value& jv, AnimatorScope* ascope, std::function<void(const ShapeValue&)>&& apply,
...
template <>
bool AnimationBuilder::bindProperty(
    const skjson::Value& jv, AnimatorScope* ascope, std::function<void(const TextValue&)>&& apply,
