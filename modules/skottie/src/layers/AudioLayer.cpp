...
class ForwardingPlaybackController final : public Animator {
 public:
  ForwardingPlaybackController(
      sk_sp<skresources::ExternalTrackAsset> track, float in_point, float out_point, float fps)
 ...
 private:
  ...
  const float fInPoint, fOutPoint, fFps;
};

}  // namespace

sk_sp<sksg::RenderNode> AnimationBuilder::attachAudioLayer(
  ...
  if (audio_asset) {
    ...
    if (name && path && id) {
      auto track = fResourceProvider->loadAudioAsset(path->begin(), name->begin(), id->begin());
      if (track) {
        fCurrentAnimatorScope->push_back(sk_make_sp<ForwardingPlaybackController>(
            std::move(track), layer_info->fInPoint, layer_info->fOutPoint, fFrameRate));
      } else {
        this->log(
            Logger::Level::kWarning, nullptr, "Could not load audio asset '%s'.", name->begin());
      }
    }
  }
