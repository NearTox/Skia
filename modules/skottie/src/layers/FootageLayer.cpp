...
SkMatrix image_matrix(const ImageAsset::FrameData& frame_data, const SkISize& dest_size) {
  ...
  return frame_data.matrix * SkMatrix::MakeRectToRect(
                                 SkRect::Make(frame_data.image->bounds()), SkRect::Make(dest_size),
...
class FootageAnimator final : public Animator {
 public:
  FootageAnimator(
      sk_sp<ImageAsset> asset, sk_sp<sksg::Image> image_node,
      sk_sp<sksg::Matrix<SkMatrix>> image_transform_node, const SkISize& asset_size,
...
const AnimationBuilder::FootageAssetInfo* AnimationBuilder::loadFootageAsset(
  ...
  if (!asset) {
    this->log(
        Logger::Level::kError, nullptr, "Could not load image asset: %s/%s (id: '%s').",
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachFootageAsset(
  ...
  const auto requires_animator =
      (fFlags & Animation::Builder::kDeferImageLoading) || asset_info->fAsset->isMultiFrame();
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachFootageLayer(
    const skjson::ObjectValue& jlayer, LayerInfo* layer_info) const {
  const ScopedAssetRef footage_asset(this, jlayer);

  return footage_asset ? this->attachFootageAsset(*footage_asset, layer_info) : nullptr;
}
