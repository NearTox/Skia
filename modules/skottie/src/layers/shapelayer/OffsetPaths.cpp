...
class OffsetPathsAdapter final
    : public DiscardableAdapterBase<OffsetPathsAdapter, sksg::OffsetEffect> {
 public:
  OffsetPathsAdapter(
 ...
 private:
  ...
  ScalarValue fAmount = 0, fMiterLimit = 0;
...
std::vector<sk_sp<sksg::GeometryNode>> ShapeBuilder::AttachOffsetGeometryEffect(
  ...
  for (auto& g : geos) {
    offsetted.push_back(
        abuilder->attachDiscardableAdapter<OffsetPathsAdapter>(jround, *abuilder, std::move(g)));
  }
