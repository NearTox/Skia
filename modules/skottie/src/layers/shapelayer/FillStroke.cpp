...
class FillStrokeAdapter final : public DiscardableAdapterBase<FillStrokeAdapter, sksg::PaintNode> {
 public:
  ...
  FillStrokeAdapter(
      ...
      sk_sp<sksg::PaintNode> paint_node, sk_sp<AnimatablePropertyContainer> gradient_adapter,
      Type type)
    ...
    if (type == Type::kStroke) {
      ...
      this->node()->setStrokeJoin(gJoins[std::min<size_t>(
          ParseDefault<size_t>(jpaint["lj"], 1) - 1, SK_ARRAY_COUNT(gJoins) - 1)]);
      ...
      this->node()->setStrokeCap(gCaps[std::min<size_t>(
          ParseDefault<size_t>(jpaint["lc"], 1) - 1, SK_ARRAY_COUNT(gCaps) - 1)]);
    }
 ...
 private:
  ...
  VectorValue fColor;
  ScalarValue fOpacity = 100, fStrokeWidth = 1;
...
class DashAdapter final : public DiscardableAdapterBase<DashAdapter, sksg::DashEffect> {
 public:
  DashAdapter(
      const skjson::ArrayValue& jdash, const AnimationBuilder& abuilder,
    ...
    for (size_t i = 0; i < jdash.size(); ++i) {
      if (const skjson::ObjectValue* jint = jdash[i]) {
        auto* target = i < interval_count ? &fIntervals[i] : &fOffset;
