...
sk_sp<sksg::Merge> ShapeBuilder::MergeGeometry(
    std::vector<sk_sp<sksg::GeometryNode>>&& geos, sksg::Merge::Mode mode) {
...
std::vector<sk_sp<sksg::GeometryNode>> ShapeBuilder::AttachMergeGeometryEffect(
  ...
  const auto mode = gModes[std::min<size_t>(
      ParseDefault<size_t>(jmerge["mm"], 1) - 1, SK_ARRAY_COUNT(gModes) - 1)];

  std::vector<sk_sp<sksg::GeometryNode>> merged;
  merged.emplace_back(ShapeBuilder::MergeGeometry(std::move(geos), mode));
