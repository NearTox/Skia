...
static SkPoint lerp(const SkPoint& p0, const SkPoint& p1, SkScalar t) { return p0 + (p1 - p0) * t; }
  ...
  SkPath onRevalidateEffect(const sk_sp<GeometryNode>& geo) override {
    ...
    auto commit_contour = [&]() {
      ...
      for (const auto& c : cubics) {
        path.cubicTo(
            lerp(c.ctrl0, center, -fAmount), lerp(c.ctrl1, center, -fAmount),
    ...
    while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
      switch (verb) {
        ...
        case SkPath::kLine_Verb: {
          ...
          const auto line_start = pts[0], line_end = pts[1];
          cubics.push_back(
              {lerp(line_start, line_end, kCtrlPosFraction),
               lerp(line_start, line_end, 1 - kCtrlPosFraction), line_end});
        } break;
        ...
        case SkPath::kConic_Verb: {
          ...
          const auto conic_start = cubics.empty() ? contour_start : cubics.back().pt,
                     conic_end = pts[2];

          cubics.push_back(
              {lerp(pts[1], conic_start, kCubicCircleCoeff),
               lerp(pts[1], conic_end, kCubicCircleCoeff), conic_end});
        } break;
        case SkPath::kCubic_Verb: cubics.push_back({pts[1], pts[2], pts[3]}); break;
        case SkPath::kClose_Verb: commit_contour(); break;
        default: break;
      }
    }
...
class PuckerBloatAdapter final
    : public DiscardableAdapterBase<PuckerBloatAdapter, PuckerBloatEffect> {
 public:
  PuckerBloatAdapter(
      const skjson::ObjectValue& joffset, const AnimationBuilder& abuilder,
...
std::vector<sk_sp<sksg::GeometryNode>> ShapeBuilder::AttachPuckerBloatGeometryEffect(
  ...
  for (auto& g : geos) {
    bloated.push_back(
        abuilder->attachDiscardableAdapter<PuckerBloatAdapter>(jround, *abuilder, std::move(g)));
  }
