...
static constexpr GeometryEffectAttacherT gGeometryEffectAttachers[] = {
    ShapeBuilder::AttachMergeGeometryEffect,       ShapeBuilder::AttachTrimGeometryEffect,
    ShapeBuilder::AttachRoundGeometryEffect,       ShapeBuilder::AttachOffsetGeometryEffect,
...
sk_sp<sksg::GeometryNode> ShapeBuilder::AttachPathGeometry(
  ...
  for (auto rec = recs.rbegin(); rec != recs.rend(); ++rec) {
    ...
    switch (rec->fInfo.fShapeType) {
      ...
      case ShapeType::kGroup: {
        ...
        if (auto subgroup = this->attachShape(rec->fJson["it"], &groupShapeCtx, rec->fSuppressed)) {
      ...
      case ShapeType::kPaint: {
        ...
        if (!paint || geos.empty() || rec->fSuppressed) break;
