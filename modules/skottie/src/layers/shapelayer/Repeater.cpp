...
class RepeaterRenderNode final : public sksg::CustomRenderNode {
 ...
 private:
  ...
  SkMatrix instanceTransform(size_t i) const {
    ...
    return SkMatrix::Translate(t * fPosition.x + fAnchorPoint.x, t * fPosition.y + fAnchorPoint.y) *
  ...
  size_t fCount = 0;
  float fOffset = 0, fRotation = 0, fStartOpacity = 1, fEndOpacity = 1;
  SkV2 fAnchorPoint = {0, 0}, fPosition = {0, 0}, fScale = {1, 1};
...
class RepeaterAdapter final : public DiscardableAdapterBase<RepeaterAdapter, RepeaterRenderNode> {
 public:
  RepeaterAdapter(
      ...
      const AnimationBuilder& abuilder, std::vector<sk_sp<sksg::RenderNode>>&& draws)
      : INHERITED(sk_make_sp<RepeaterRenderNode>(
            std::move(draws), (ParseDefault(jrepeater["m"], 1) == 1)
 ...
 private:
  ...
  // Repeater props
  ScalarValue fCount = 0, fOffset = 0;

  // Transform props
  Vec2Value fAnchorPoint = {0, 0}, fPosition = {0, 0}, fScale = {100, 100};
  ScalarValue fRotation = 0, fStartOpacity = 100, fEndOpacity = 100;
...
std::vector<sk_sp<sksg::RenderNode>> ShapeBuilder::AttachRepeaterDrawEffect(
  ...
  if (const skjson::ObjectValue* jtransform = jrepeater["tr"]) {
    ...
    repeater_draws.push_back(abuilder->attachDiscardableAdapter<RepeaterAdapter>(
        jrepeater, *jtransform, *abuilder, std::move(draws)));
  } else {
