...
class GradientAdapter final : public AnimatablePropertyContainer {
 public:
  static sk_sp<GradientAdapter> Make(
    ...
    return sk_sp<GradientAdapter>(new GradientAdapter(
        std::move(gradient_node), type, SkToSizeT(stopCount), jgrad, *jstops, abuilder));
  }
 ...
 private:
  ...
  GradientAdapter(
      sk_sp<sksg::Gradient> gradient, Type type, size_t stop_count,
      const skjson::ObjectValue& jgrad, const skjson::ObjectValue& jstops,
  ...
  void onSync() override {
    ...
    const auto c_count = fStopCount, c_size = c_count * 4, o_count = (fStops.size() - c_size) / 2;
 ...
 private:
  ...
  VectorValue fStartPoint, fEndPoint, fStops;
};
...
sk_sp<sksg::PaintNode> ShapeBuilder::AttachGradientFill(
  ...
  return adapter ? AttachFill(jgrad, abuilder, sksg::ShaderPaint::Make(adapter->node()), adapter)
                 : nullptr;
}
...
sk_sp<sksg::PaintNode> ShapeBuilder::AttachGradientStroke(
  ...
  return adapter ? AttachStroke(jgrad, abuilder, sksg::ShaderPaint::Make(adapter->node()), adapter)
                 : nullptr;
}
