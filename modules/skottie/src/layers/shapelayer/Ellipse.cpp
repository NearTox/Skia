...
class EllipseGeometryAdapter final
 ...
 public:
  EllipseGeometryAdapter(const skjson::ObjectValue& jellipse, const AnimationBuilder* abuilder) {
    this->node()->setDirection(
 ...
 private:
  void onSync() override {
    const auto bounds =
        SkRect::MakeXYWH(fPosition.x - fSize.x / 2, fPosition.y - fSize.y / 2, fSize.x, fSize.y);

    this->node()->setRRect(SkRRect::MakeOval(bounds));
  }

  Vec2Value fSize = {0, 0}, fPosition = {0, 0};  // center
};
