...
bool parse_glyph_path(
    const skjson::ObjectValue* jdata, const AnimationBuilder* abuilder, SkPath* path) {
  // Glyph path encoding:
  //
  //   "data": {
...
void AnimationBuilder::parseFonts(
  ...
  //        ]
  //    },
  const skjson::ArrayValue* jlist =
      jfonts ? static_cast<const skjson::ArrayValue*>((*jfonts)["list"]) : nullptr;
  if (!jlist) {
    return;
  }
...
bool AnimationBuilder::resolveNativeTypefaces() {
  ...
  fFonts.foreach ([&](const SkString& name, FontInfo* finfo) {
    ...
    if (!finfo->fTypeface) {
      finfo->fTypeface.reset(
          fmgr->matchFamilyStyle(finfo->fFamily.c_str(), FontStyle(this, finfo->fStyle.c_str())));
      ...
      if (!finfo->fTypeface) {
        ...
        // Last resort.
        finfo->fTypeface =
            fmgr->legacyMakeTypeface(nullptr, FontStyle(this, finfo->fStyle.c_str()));
