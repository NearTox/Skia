...
sk_sp<sksg::RenderNode> AnimationBuilder::attachExternalPrecompLayer(
    const skjson::ObjectValue& jlayer, const LayerInfo& layer_info) const {
  ...
  auto external_layer =
      fPrecompInterceptor->onLoadPrecomp(id->begin(), nm->begin(), layer_info.fSize);
  if (!external_layer) {
    return nullptr;
  }
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachPrecompLayer(
  ...
  const auto requires_time_mapping =
      !SkScalarNearlyEqual(start_time, 0) || !SkScalarNearlyEqual(stretch_time, 1) || time_remapper;
  ...
  if (!precomp_layer) {
    ...
    if (precomp_asset) {
      ...
      precomp_layer = CompositionBuilder(*this, layer_info->fSize, *precomp_asset).build(*this);
    }
  }
