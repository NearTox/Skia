...
class LayerController final : public sksg::Animator {
 public:
  LayerController(
      sksg::AnimatorList&& layer_animators, sk_sp<sksg::RenderNode> layer, size_t tanim_count,
      float in, float out)
 ...
 protected:
  void onTick(float t) override {
    ...
    const auto dispatch_count = active ? fLayerAnimators.size() : fTransformAnimatorsCount;
...
class MotionBlurController final : public sksg::Animator {
 public:
  explicit MotionBlurController(sk_sp<MotionBlurEffect> mbe) : fMotionBlurEffect(std::move(mbe)) {}
 ...
 protected:
  ...
  void onTick(float t) override { fMotionBlurEffect->setT(t); }
...
AnimationBuilder::AttachLayerContext::attachLayerTransform(
...
sk_sp<sksg::Transform> AnimationBuilder::AttachLayerContext::attachParentLayerTransform(
  ...
  if (const auto* rec = fLayerTransformMap.find(parent_index)) return rec->fTransformNode;
...
sk_sp<sksg::Transform> AnimationBuilder::AttachLayerContext::attachTransformNode(
    const skjson::ObjectValue& jlayer, const AnimationBuilder* abuilder, AnimatorScope* ascope,
  ...
  if (type == TransformType::kCamera) {
    ...
    abuilder->bindProperty<ScalarValue>(
        jlayer["pe"], ascope, [camera_adapter](const ScalarValue& pe) {
    ...
    return abuilder->attachMatrix3D(
        *jtransform, ascope, std::move(parent_transform), std::move(camera_adapter),
...
AnimationBuilder::AttachLayerContext::TransformRec*
AnimationBuilder::AttachLayerContext::attachLayerTransformImpl(
    const skjson::ObjectValue& jlayer, const AnimationBuilder* abuilder, TransformType type,
    int layer_index) {
  ...
  auto transform =
      this->attachTransformNode(jlayer, abuilder, &ascope, std::move(parent_matrix), type);
...
bool AnimationBuilder::AttachLayerContext::hasMotionBlur(const skjson::ObjectValue& jlayer) const {
  return fMotionBlurSamples > 1 && fMotionBlurAngle > 0 && ParseDefault(jlayer["mb"], false);
}

sk_sp<sksg::RenderNode> AnimationBuilder::attachLayer(
    const skjson::ObjectValue* jlayer, AnimatorScope* ascope, AttachLayerContext* layerCtx) const {
  if (!jlayer) {
    return nullptr;
  }
  ...
  using LayerBuilder = sk_sp<sksg::RenderNode> (AnimationBuilder::*)(
      const skjson::ObjectValue&, LayerInfo*, AnimatorScope*) const;
  ...
  if (layer_transform_rec.fTransformNode && transform_effects) {
    layer = sksg::TransformEffect::Make(
        std::move(layer), std::move(layer_transform_rec.fTransformNode));
  }
  ...
  sk_sp<sksg::Animator> controller = sk_make_sp<LayerController>(
      std::move(layer_animators), layer, transform_animator_count, layer_info.fInPoint,
      layer_info.fOutPoint);
  ...
  if (has_animators && layerCtx->hasMotionBlur(*jlayer)) {
    ...
    auto motion_blur = MotionBlurEffect::Make(
        std::move(controller), std::move(layer), layerCtx->fMotionBlurSamples,
        layerCtx->fMotionBlurAngle, layerCtx->fMotionBlurPhase);
  ...
  if (layerCtx->fCurrentMatte) {
    ...
    if (matteType < SK_ARRAY_COUNT(gMaskModes)) {
      return sksg::MaskEffect::Make(
          std::move(layer), std::move(layerCtx->fCurrentMatte), gMaskModes[matteType]);
    }
