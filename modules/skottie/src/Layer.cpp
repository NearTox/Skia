...
class MaskAdapter final : public AnimatablePropertyContainer {
 public:
  ...
  bool hasEffect() const { return !this->isStatic() || fOpacity < 100 || fFeather != SkV2{0, 0}; }
 ...
 private:
  void onSync() override {
    ...
    if (fMaskFilter) {
      ...
      fMaskFilter->setSigma({fFeather.x * kFeatherToSigma, fFeather.y * kFeatherToSigma});
    }
  }
...
sk_sp<sksg::RenderNode> AttachMask(
  ...
  for (const skjson::ObjectValue* m : *jmask) {
    ...
    const auto blend_mode = mask_stack.empty() ? SkBlendMode::kSrc : mask_info->fBlendMode;
    ...
    mask_stack.push_back({std::move(mask_path), std::move(mask_adapter), mask_info->fMergeMode});
  }
...
class LayerController final : public Animator {
 public:
  LayerController(
      AnimatorScope&& layer_animators, sk_sp<sksg::RenderNode> layer, size_t tanim_count, float in,
      float out)
