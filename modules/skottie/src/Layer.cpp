...
sk_sp<sksg::RenderNode> AttachMask(
  ...
  for (const skjson::ObjectValue* m : *jmask) {
    ...
    mask_path->setFillType(inverted ? SkPathFillType::kInverseWinding : SkPathFillType::kWinding);
...
sk_sp<sksg::Transform> LayerBuilder::buildTransform(
  ...
  const auto transform_chain_type = this->is3D() ? TransformType::k3D : TransformType::k2D;
  fLayerTransform = this->getTransform(abuilder, cbuilder, transform_chain_type);

  return fLayerTransform;
}

sk_sp<sksg::Transform> LayerBuilder::getTransform(
    const AnimationBuilder& abuilder, CompositionBuilder* cbuilder, TransformType ttype) {
...
sk_sp<sksg::Transform> LayerBuilder::getParentTransform(
    const AnimationBuilder& abuilder, CompositionBuilder* cbuilder, TransformType ttype) {
...
sk_sp<sksg::Transform> LayerBuilder::doAttachTransform(
    const AnimationBuilder& abuilder, CompositionBuilder* cbuilder, TransformType ttype) {
  ...
  if (this->isCamera()) {
    ...
    abuilder.bindProperty<ScalarValue>(fJlayer["pe"], [camera_adapter](const ScalarValue& pe) {
...
bool LayerBuilder::hasMotionBlur(const CompositionBuilder* cbuilder) const {
  return cbuilder->fMotionBlurSamples > 1 && cbuilder->fMotionBlurAngle > 0 &&
...
sk_sp<sksg::RenderNode> LayerBuilder::buildRenderTree(
  ...
  using LayerBuilder = sk_sp<sksg::RenderNode> (AnimationBuilder::*)(
      const skjson::ObjectValue&, AnimationBuilder::LayerInfo*) const;
  ...
  sk_sp<sksg::Animator> controller = sk_make_sp<LayerController>(
      ascope.release(), layer, fTransformAnimatorCount, layer_info.fInPoint, layer_info.fOutPoint);
  ...
  if (layer && has_animators && this->hasMotionBlur(cbuilder)) {
    ...
    auto motion_blur = MotionBlurEffect::Make(
        std::move(controller), std::move(layer), cbuilder->fMotionBlurSamples,
        cbuilder->fMotionBlurAngle, cbuilder->fMotionBlurPhase);
  ...
  if (auto matte = cbuilder->popMatte()) {
    ...
    if (matteType < SK_ARRAY_COUNT(gMaskModes)) {
      layer = sksg::MaskEffect::Make(std::move(layer), std::move(matte), gMaskModes[matteType]);
    }
  }
