...
T ParseEnum(
  ...
  abuilder->log(
      Logger::Level::kWarning, nullptr, "Ignoring unknown range selector %s '%d'", warn_name, idx);
...
static const ShapeGenerator gShapeGenerators[] = {
    ...
    {0, 0,
     [](float t) -> float {
       static constexpr auto cx = 0.5f, cx2 = cx * cx;
...
sk_sp<RangeSelector> RangeSelector::Make(
    const skjson::ObjectValue* jrange, const AnimationBuilder* abuilder, AnimatorScope* ascope) {
  if (!jrange) {
    return nullptr;
  }
  ...
  abuilder->bindProperty<ScalarValue>(
      (*jrange)["s"], ascope, [selector](const ScalarValue& s) { selector->fStart = s; });
  abuilder->bindProperty<ScalarValue>(
      (*jrange)["e"], ascope, [selector](const ScalarValue& e) { selector->fEnd = e; });
  abuilder->bindProperty<ScalarValue>(
      (*jrange)["o"], ascope, [selector](const ScalarValue& o) { selector->fOffset = o; });
  abuilder->bindProperty<ScalarValue>(
      (*jrange)["a"], ascope, [selector](const ScalarValue& a) { selector->fAmount = a; });
...
void RangeSelector::modulateCoverage(
  ...
  if (SkScalarNearlyZero(range_span)) {
    ...
    SkASSERT(i0 == i1);
    const auto ratio = f0 - i0, coverage = Lerp(generator.lo, generator.hi, ratio);
  ...
  const auto partial_coverage = [&](float shape_val, float i) {
    ...
    const auto lo_weight = std::max(f0 - i, 0.0f), mi_weight = std::min(f1 - i, 1.0f) - lo_weight,
    ...
    return lo_weight * generator.lo + mi_weight * shape_val + hi_weight * generator.hi;
  };
