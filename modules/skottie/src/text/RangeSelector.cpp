...
T ParseEnum(
    const TArray& arr, const skjson::Value& jenum, const AnimationBuilder* abuilder,
  ...
  if (idx != 0) {
    abuilder->log(
        Logger::Level::kWarning, nullptr, "Ignoring unknown range selector %s '%d'", warn_name,
        idx);
  }
...
struct UnitTraits<RangeSelector::Units::kPercentage> {
  static constexpr auto Defaults() { return std::make_tuple<float, float, float>(0, 100, 0); }

  static auto Resolve(float s, float e, float o, size_t domain_size) {
    return std::make_tuple(domain_size * (s + o) / 100, domain_size * (e + o) / 100);
  }
};
...
class CoverageProcessor {
 public:
  CoverageProcessor(
      const TextAnimator::DomainMaps& maps, RangeSelector::Domain domain, RangeSelector::Mode mode,
    ...
    switch (domain) {
      case RangeSelector::Domain::kChars:
        ...
        break;
      case RangeSelector::Domain::kCharsExcludingSpaces: fMap = &maps.fNonWhitespaceMap; break;
      case RangeSelector::Domain::kWords: fMap = &maps.fWordsMap; break;
      case RangeSelector::Domain::kLines: fMap = &maps.fLinesMap; break;
    }
 ...
 private:
  ...
  ProcT fProc, fMappedProc = nullptr;
...
struct ShapeInfo {
  SkVector ctrl0, ctrl1;
  float e0, e1, crs;
};

SkVector EaseVec(float ease) { return (ease < 0) ? SkVector{0, -ease} : SkVector{ease, 0}; }

struct ShapeGenerator {
  SkCubicMap shape_mapper, ease_mapper;
  float e0, e1, crs;
...
sk_sp<RangeSelector> RangeSelector::Make(
    const skjson::ObjectValue* jrange, const AnimationBuilder* abuilder) {
  if (!jrange) {
    return nullptr;
  }
...

  {
    ...
    if (type != kRange_SelectorType) {
      abuilder->log(
          Logger::Level::kWarning, nullptr, "Ignoring unsupported selector type '%d'", type);
      return nullptr;
    }
  }
  ...
  auto selector = sk_sp<RangeSelector>(new RangeSelector(
      ParseEnum<Units>(gUnitMap, (*jrange)["r"], abuilder, "units"),
  ...
  abuilder->bindProperty<ScalarValue>(
      (*jrange)["s"], [raw_selector](const ScalarValue& s) { raw_selector->fStart = s; });
  abuilder->bindProperty<ScalarValue>(
      (*jrange)["e"], [raw_selector](const ScalarValue& e) { raw_selector->fEnd = e; });
  abuilder->bindProperty<ScalarValue>(
      (*jrange)["o"], [raw_selector](const ScalarValue& o) { raw_selector->fOffset = o; });
  abuilder->bindProperty<ScalarValue>(
      (*jrange)["a"], [raw_selector](const ScalarValue& a) { raw_selector->fAmount = a; });
  abuilder->bindProperty<ScalarValue>(
      (*jrange)["ne"], [raw_selector](const ScalarValue& ne) { raw_selector->fEaseLo = ne; });
  abuilder->bindProperty<ScalarValue>(
      (*jrange)["xe"], [raw_selector](const ScalarValue& xe) { raw_selector->fEaseHi = xe; });

  // Optional square "smoothness" prop.
  if (selector->fShape == Shape::kSquare) {
    abuilder->bindProperty<ScalarValue>(
        (*jrange)["sm"], [selector](const ScalarValue& sm) { selector->fSmoothness = sm; });
  }
...
std::tuple<float, float> RangeSelector::resolve(size_t len) const {
  ...
  const auto resolver = (fUnits == Units::kPercentage) ? UnitTraits<Units::kPercentage>::Resolve
 ...
 */
void RangeSelector::modulateCoverage(
    const TextAnimator::DomainMaps& maps, TextAnimator::ModulatorBuffer& mbuf) const {
