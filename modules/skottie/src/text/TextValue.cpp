...
bool Parse(const skjson::Value& jv, const internal::AnimationBuilder& abuilder, TextValue* v) {
  ...
  v->fHAlign = gAlignMap[std::min<size_t>(
      ParseDefault<size_t>((*jtxt)["j"], 0), SK_ARRAY_COUNT(gAlignMap) - 1)];
  ...
  v->fLineBreak =
      v->fBox.isEmpty() ? Shaper::LinebreakPolicy::kExplicit : Shaper::LinebreakPolicy::kParagraph;
  ...
  v->fCapitalization = gCapMap[std::min<size_t>(
      ParseDefault<size_t>((*jtxt)["ca"], 0), SK_ARRAY_COUNT(gCapMap) - 1)];
  ...
  if (v->fHasStroke) {
    ...
    v->fPaintOrder = ParseDefault((*jtxt)["of"], true) ? TextPaintOrder::kFillStroke
    ...
    v->fStrokeJoin = gJoins[std::min<size_t>(
        ParseDefault<size_t>((*jtxt)["lj"], 1) - 1, SK_ARRAY_COUNT(gJoins) - 1)];
  }
