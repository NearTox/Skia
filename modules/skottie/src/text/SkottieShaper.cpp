...
class BlobMaker final : public SkShaper::RunHandler {
 public:
  ...
  void runInfo(const RunInfo& info) override { fPendingLineAdvance += info.fAdvance; }
  ...
  Buffer runBuffer(const RunInfo& info) override {
    ...
    return {fLineGlyphs.get() + run_start_index, fLinePos.get() + run_start_index, nullptr, nullptr,
  ...
  void commitLine() override {
    ...
    for (const auto& rec : fLineRuns) {
      ...
      sk_careful_memcpy(
          blob_buffer.glyphs, fLineGlyphs.get() + run_offset, rec.fGlyphCount * sizeof(SkGlyphID));
      sk_careful_memcpy(
          blob_buffer.pos, fLinePos.get() + run_offset, rec.fGlyphCount * sizeof(SkPoint));
  ...
  Shaper::Result makeBlob() {
    ...
    switch (fDesc.fVAlign) {
      case Shaper::VAlign::kTop: pos.fY -= ComputeBlobBounds(blob).fTop; break;
      ...
      case Shaper::VAlign::kBottom:
        ...
        break;
      case Shaper::VAlign::kResizeToFit: SkASSERT(false); break;
    }
...
Shaper::Result Shaper::Shape(const SkString& txt, const TextDesc& desc, const SkRect& box) {
  return (desc.fVAlign == VAlign::kResizeToFit) ? ShapeToFit(txt, desc, box)
