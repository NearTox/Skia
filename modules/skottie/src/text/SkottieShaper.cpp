...
public:
    ...
    Buffer runBuffer(const RunInfo& info) override {
        ...
        return {blobBuffer.glyphs, blobBuffer.points(), nullptr, nullptr,
    ...
    void commitLine() override {
        ...
        const auto lh = fDesc.fLineHeight > 0 ? fDesc.fLineHeight
...
private:
    ...
    struct Run {
        ...
        Run(const SkFont& font, const SkShaper::RunHandler::RunInfo& info, int count)
                : fFont(font), fInfo(info), fGlyphs(count), fPositions(count) {
    ...
    for (size_t i = 0; i < kMaxIter; ++i) {
        ...
        if (res_height > box.height()) {
            ...
            try_size =
                    (in_size == 0)
                            ? try_size *
                                      0.5f  // initial in_size not found yet - search exponentially
            ...
            in_size = try_size;
            try_size =
                    (out_size == std::numeric_limits<float>::max())
