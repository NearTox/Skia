...
class BlobMaker final : public SkShaper::RunHandler {
 public:
  ...
  Buffer runBuffer(const RunInfo& info) override {
    ...
    return {fLineGlyphs.get() + run_start_index, fLinePos.get() + run_start_index, nullptr,
  ...
  Shaper::Result finalize(float* shaped_height) {
    ...
    switch (fDesc.fVAlign) {
      ...
      case Shaper::VAlign::kTopBaseline:
        // Default behavior.
        break;
      case Shaper::VAlign::kVisualTop: v_offset = fBox.fTop - extent_box().fTop; break;
      ...
      case Shaper::VAlign::kVisualDownscaleToFit: SkASSERT(false); break;
    }
  ...
  void shapeLine(const char* start, const char* end) {
    ...
    const auto shape_width = fBox.isEmpty() ? SK_ScalarMax : fBox.width();
