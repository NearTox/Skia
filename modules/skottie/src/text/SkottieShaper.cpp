...
class BlobMaker final : public SkShaper::RunHandler {
 public:
  ...
  Buffer runBuffer(const RunInfo& info) override {
    ...
    return {
        fLineGlyphs.get() + run_start_index, fLinePos.get() + run_start_index, nullptr,
  ...
  void commitLine() override {
    ...
    for (const auto& rec : fLineRuns) {
      ...
      (this->*commit_proc)(
          rec, fLineGlyphs.get() + run_offset, fLinePos.get() + run_offset,
          fLineClusters.get() + run_offset, fLineCount);
  ...
  void shapeLine(const char* start, const char* end) {
    ...
    const auto shape_width =
        fDesc.fLinebreak == Shaper::LinebreakPolicy::kExplicit ? SK_ScalarMax : fBox.width();
 ...
 private:
  ...
  void commitConsolidatedRun(
      const RunRec& rec, const SkGlyphID* glyphs, const SkPoint* pos, const uint32_t*, uint32_t) {
  ...
  float fFirstLineAscent = 0, fLastLineDescent = 0;
...
class AdjustedText {
 public:
  AdjustedText(const SkString& txt, const Shaper::TextDesc& desc) : fText(txt) {
    switch (desc.fCapitalization) {
      case Shaper::Capitalization::kNone: break;
...
Shaper::Result Shaper::Shape(
    const SkString& orig_txt, const TextDesc& desc, const SkPoint& point,
    const sk_sp<SkFontMgr>& fontmgr) {
...
Shaper::Result Shaper::Shape(
    const SkString& orig_txt, const TextDesc& desc, const SkRect& box,
    const sk_sp<SkFontMgr>& fontmgr) {
  const AdjustedText txt(orig_txt, desc);

  switch (desc.fResize) {
    case ResizePolicy::kNone: return ShapeImpl(txt, desc, box, fontmgr);
    case ResizePolicy::kScaleToFit: return ShapeToFit(txt, desc, box, fontmgr);
