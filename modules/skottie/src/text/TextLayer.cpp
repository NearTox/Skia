  ...
  if (jfonts) {
    if (const skjson::ArrayValue* jlist = (*jfonts)["list"]) {
      for (const skjson::ObjectValue* jfont : *jlist) {
        ...
        if (!jname || !jname->size() || !jfamily || !jfamily->size() || !jstyle ||
            !jstyle->size()) {
        ...
        sk_sp<SkTypeface> tf = fmgr->makeFromData(
            fResourceProvider->loadFont(jname->begin(), jpath ? jpath->begin() : nullptr));

        if (!tf) {
          tf.reset(fmgr->matchFamilyStyle(jfamily->begin(), FontStyle(this, jstyle->begin())));
        }

        if (!tf) {
          this->log(
              Logger::Level::kError, nullptr, "Could not create typeface for %s|%s.",
        ...
        fFonts.set(
            SkString(jname->begin(), jname->size()),
            {SkString(jfamily->begin(), jfamily->size()), SkString(jstyle->begin(), jstyle->size()),
             ParseDefault((*jfont)["ascent"], 0.0f), std::move(tf)});
      }
    }
  }
  ...
  if (jchars) {
    ...
    for (const skjson::ObjectValue* jchar : *jchars) {
      ...
      if (!current_font || !current_font->matches(family, style)) {
        ...
        if (!current_font) {
          this->log(
              Logger::Level::kError, nullptr, "Font not found for codepoint (%d, %s, %s).", uni,
              family, style);
          continue;
        }
      }
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachTextLayer(
  ...
  this->bindProperty<TextValue>(
      *jd, ascope, [adapter](const TextValue& txt) { adapter->setText(txt); });
