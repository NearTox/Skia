    ...
    if (jfonts) {
        if (const skjson::ArrayValue* jlist = (*jfonts)["list"]) {
            for (const skjson::ObjectValue* jfont : *jlist) {
                ...
                sk_sp<SkTypeface> tf = fmgr->makeFromData(fResourceProvider->loadFont(
                        jname->begin(), jpath ? jpath->begin() : nullptr));
                ...
                if (!tf) {
                    this->log(Logger::Level::kError, nullptr,
                              "Could not create typeface for %s|%s.", jfamily->begin(),
                              jstyle->begin());
                    // Last resort.
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachTextLayer(const skjson::ObjectValue& layer,
    ...
    this->bindProperty<TextValue>(*jd, ascope,
                                  [adapter](const TextValue& txt) { adapter->setText(txt); });
