...
sk_sp<TextAnimator> TextAnimator::Make(
  ...
  return sk_sp<TextAnimator>(new TextAnimator(std::move(selectors), *jprops, abuilder, acontainer));
}
...
TextAnimator::ResolvedProps TextAnimator::modulateProps(
    const ResolvedProps& props, float amount) const {
  ...
  const auto lerp = [](float v0, float v1, float t) { return v0 + (v1 - v0) * t; };
  ...
  if (fHasFillOpacity) {
    // 255-based
    const auto alpha =
        lerp(SkColorGetA(props.fill_color), fTextProps.fill_opacity * 2.55f, clamped_amount);
    modulated_props.fill_color =
        SkColorSetA(modulated_props.fill_color, static_cast<U8CPU>(std::round(alpha)));
  }
  if (fHasStrokeOpacity) {
    // 255-based
    const auto alpha =
        lerp(SkColorGetA(props.stroke_color), fTextProps.stroke_opacity * 2.55f, clamped_amount);
    modulated_props.stroke_color =
        SkColorSetA(modulated_props.stroke_color, static_cast<U8CPU>(std::round(alpha)));
  }
  if (fHasOpacity) {
