 ...
 * }
 */
sk_sp<TextAnimator> TextAnimator::Make(
    const skjson::ObjectValue* janimator, const AnimationBuilder* abuilder, AnimatorScope* ascope) {
  if (!janimator) {
    return nullptr;
  }
  ...
  return jprops ? sk_sp<TextAnimator>(
                      new TextAnimator(std::move(selectors), *jprops, abuilder, ascope))
                : nullptr;
}
...
TextAnimator::AnimatedProps TextAnimator::modulateProps(
    const AnimatedProps& props, float amount) const {
  ...
  if (fHasFillColor) {
    modulated_props.fill_color =
        lerp_color(props.fill_color, fTextProps.fill_color, clamped_amount);
  }
  if (fHasStrokeColor) {
    modulated_props.stroke_color =
        lerp_color(props.stroke_color, fTextProps.stroke_color, clamped_amount);
  }
...
TextAnimator::TextAnimator(
  ...
  abuilder->bindProperty<VectorValue>(jprops["p"], ascope, [animator](const VectorValue& p) {
    animator->fTextProps.position = ValueTraits<VectorValue>::As<SkPoint>(p);
  });
  abuilder->bindProperty<ScalarValue>(jprops["s"], ascope, [animator](const ScalarValue& s) {
    // Scale is 100-based.
    animator->fTextProps.scale = s * 0.01f;
  });
  abuilder->bindProperty<ScalarValue>(
      jprops["r"], ascope, [animator](const ScalarValue& r) { animator->fTextProps.rotation = r; });
  fHasFillColor =
      abuilder->bindProperty<VectorValue>(jprops["fc"], ascope, [animator](const VectorValue& fc) {
        animator->fTextProps.fill_color = ValueTraits<VectorValue>::As<SkColor>(fc);
      });
  fHasStrokeColor =
      abuilder->bindProperty<VectorValue>(jprops["sc"], ascope, [animator](const VectorValue& sc) {
        animator->fTextProps.stroke_color = ValueTraits<VectorValue>::As<SkColor>(sc);
      });
  abuilder->bindProperty<ScalarValue>(jprops["o"], ascope, [animator](const ScalarValue& o) {
    // Opacity is 100-based.
    animator->fTextProps.opacity = SkTPin<float>(o * 0.01f, 0, 1);
  });
  abuilder->bindProperty<ScalarValue>(
      jprops["t"], ascope, [animator](const ScalarValue& t) { animator->fTextProps.tracking = t; });
}

sk_sp<TextAnimatorList> TextAnimatorList::Make(
    const skjson::ArrayValue& janimators, const AnimationBuilder* abuilder,
  ...
  return sk_sp<TextAnimatorList>(new TextAnimatorList(
      std::move(adapter), std::move(local_animator_scope), std::move(animators)));
}

TextAnimatorList::TextAnimatorList(
    sk_sp<TextAdapter> adapter, sksg::AnimatorList&& alist,
