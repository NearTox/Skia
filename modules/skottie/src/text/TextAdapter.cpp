...
struct TextAdapter::PathInfo {
  ShapeValue fPath;
  ScalarValue fPathFMargin = 0, fPathLMargin = 0, fPathPerpendicular = 0, fPathReverse = 0;
  ...
  SkM44 getMatrix(float distance, SkTextUtils::Align alignment) const {
    ...
    switch (alignment) {
      ...
      case SkTextUtils::Align::kCenter_Align: distance += fPathFMargin + fPathLMargin; break;
    ...
    const auto underflow = std::min(0.0f, distance), overflow = std::max(0.0f, distance - path_len);
 ...
 private:
  ...
  bool fCurrentReversed = false, fCurrentClosed = false;
};

sk_sp<TextAdapter> TextAdapter::Make(
    const skjson::ObjectValue& jlayer, const AnimationBuilder* abuilder, sk_sp<SkFontMgr> fontmgr,
    sk_sp<Logger> logger) {
  ...
  const skjson::ObjectValue* jd =
      jt ? static_cast<const skjson::ObjectValue*>((*jt)["d"]) : nullptr;
  if (!jd) {
  ...
  const auto apg =
      jm ? SkTPin<int>(ParseDefault<int>((*jm)["g"], 1), 1, SK_ARRAY_COUNT(gGroupingMap)) : 1;

  auto adapter = sk_sp<TextAdapter>(
      new TextAdapter(std::move(fontmgr), std::move(logger), gGroupingMap[SkToSizeT(apg - 1)]));
  ...
  const auto attach_path = [&](const skjson::ObjectValue* jpath) -> std::unique_ptr<PathInfo> {
    ...
    const auto mask_index = ParseDefault<size_t>((*jpath)["m"], std::numeric_limits<size_t>::max());
...
void TextAdapter::buildDomainMaps(const Shaper::Result& shape_result) {
  ...
  size_t i = 0, line = 0, line_start = 0, word_start = 0;

  float word_advance = 0, word_ascent = 0, line_advance = 0, line_ascent = 0;
...
void TextAdapter::reshape() {
  const Shaper::TextDesc text_desc = {
      fText->fTypeface,       fText->fTextSize,  fText->fMinTextSize, fText->fMaxTextSize,
      fText->fLineHeight,     fText->fLineShift, fText->fAscent,      fText->fHAlign,
      fText->fVAlign,         fText->fResize,    fText->fLineBreak,   fText->fDirection,
      fText->fCapitalization, fText->fMaxLines,  this->shaperFlags(),
  };
  ...
  if (fLogger) {
    if (shape_result.fFragments.empty() && fText->fText.size() > 0) {
      const auto msg = SkStringPrintf("Text layout failed for '%s'.", fText->fText.c_str());
    ...
    if (shape_result.fMissingGlyphCount > 0) {
      const auto msg = SkStringPrintf(
          "Missing %zu glyphs for '%s'.", shape_result.fMissingGlyphCount, fText->fText.c_str());
  ...
  if (SHOW_LAYOUT_BOXES) {
    ...
    if (fPathInfo) {
      ...
      fRoot->addChild(sksg::Draw::Make(
          sksg::Path::Make(static_cast<SkPath>(fPathInfo->fPath)), std::move(path_color)));
    }
  }
}
...
void TextAdapter::onSync() {
  ...
  auto compute_linewide_props =
      [this](const TextAnimator::ModulatorBuffer& buf, const TextAnimator::DomainSpan& line_span) {
  ...
  for (const auto& line_span : fMaps.fLinesMap) {
    ...
    for (size_t i = line_span.fOffset; i < line_span.fOffset + line_span.fCount; ++i) {
      ...
      const auto track_before = i > line_span.fOffset ? props.tracking * 0.5f : 0.0f,
                 track_after =
                     i < line_span.fOffset + line_span.fCount - 1 ? props.tracking * 0.5f : 0.0f;

      const auto frag_offset =
          current_line_offset + SkV2{align_offset + tracking_acc + track_before, 0};

      tracking_acc += track_before + track_after;

      this->pushPropsToFragment(
          props, frag, frag_offset, fGroupingAlignment * .01f,  // %
          grouping_domain ? &(*grouping_domain)[grouping_span_index] : nullptr);
    }
  }
}

SkV2 TextAdapter::fragmentAnchorPoint(
    const FragmentRec& rec, const SkV2& grouping_alignment,
  ...
  auto anchor_box = [&]() -> SkRect {
    switch (fAnchorPointGrouping) {
      ...
      case AnchorPointGrouping::kLine: {
        ...
        return make_box(
            first_span_fragment.fOrigin, grouping_span->fAdvance, grouping_span->fAscent);
      }
  ...
  const auto ap = SkV2{
      ab.centerX() + ab.width() * 0.5f * grouping_alignment.x,
...
SkM44 TextAdapter::fragmentMatrix(
    const TextAnimator::ResolvedProps& props, const FragmentRec& rec,
    const SkV2& frag_offset) const {
  const SkV3 pos = {
      props.position.x + rec.fOrigin.fX + frag_offset.x,
      props.position.y + rec.fOrigin.fY + frag_offset.y, props.position.z};
...
void TextAdapter::pushPropsToFragment(
    const TextAnimator::ResolvedProps& props, const FragmentRec& rec, const SkV2& frag_offset,
    const SkV2& grouping_alignment, const TextAnimator::DomainSpan* grouping_span) const {
  ...
  if (rec.fBlur) {
    rec.fBlur->setSigma({props.blur.x * kBlurSizeToSigma, props.blur.y * kBlurSizeToSigma});
  }
}
