...
sk_sp<TextAdapter> TextAdapter::Make(
  ...
  auto adapter = sk_sp<TextAdapter>(
      new TextAdapter(std::move(fontmgr), std::move(logger), std::move(animators)));
  auto* raw_adapter = adapter.get();

  abuilder->bindProperty<TextValue>(
      *jd, [raw_adapter](const TextValue& txt) { raw_adapter->setText(txt); });
...
TextAdapter::TextAdapter(
    sk_sp<SkFontMgr> fontmgr, sk_sp<Logger> logger, std::vector<sk_sp<TextAnimator>>&& animators)
...
void TextAdapter::addFragment(const Shaper::Fragment& frag) {
  ...
  rec.fMatrixNode = sksg::Matrix<SkMatrix>::Make(SkMatrix::MakeTrans(frag.fPos.x(), frag.fPos.y()));
...
void TextAdapter::pushPropsToFragment(
    const TextAnimator::AnimatedProps& props, const FragmentRec& rec) const {
  // TODO: share this with TransformAdapter2D?
  auto t = SkMatrix::MakeTrans(
      rec.fOrigin.x() + props.position.x(), rec.fOrigin.y() + props.position.y());
...
void TextAdapter::adjustLineTracking(
  ...
  for (size_t i = line_span.fOffset; i < line_span.fOffset + line_span.fCount; ++i) {
    ...
    const auto& frag = fFragments[i];
    const auto m =
        SkMatrix::Concat(SkMatrix::MakeTrans(fragment_offset, 0), frag.fMatrixNode->getMatrix());
