...
void TextAdapter::reshape() {
  const Shaper::TextDesc text_desc = {
      ...
      fText->fCapitalization, this->shaperFlags(),
  };
  ...
  for (const auto& line_span : fMaps.fLinesMap) {
    ...
    for (size_t i = line_span.fOffset; i < line_span.fOffset + line_span.fCount; ++i) {
      ...
      this->pushPropsToFragment(
          props, frag, fGroupingAlignment * .01f,  // percentage
...
void TextAdapter::pushPropsToFragment(
  ...
  rec.fMatrixNode->setMatrix(
      SkM44::Translate(
          props.position.x + rec.fOrigin.x() + anchor_point.x,
          props.position.y + rec.fOrigin.y() + anchor_point.y, props.position.z) *
...
void TextAdapter::adjustLineProps(
    const TextAnimator::ModulatorBuffer& buf, const TextAnimator::DomainSpan& line_span,
    const SkV2& line_offset, float total_tracking) const {
  ...
  for (size_t i = line_span.fOffset; i < line_span.fOffset + line_span.fCount; ++i) {
    ...
    const auto& frag = fFragments[i];
    const auto m = SkM44::Translate(line_offset.x + fragment_offset, line_offset.y) *
