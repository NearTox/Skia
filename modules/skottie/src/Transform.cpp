...
TransformAdapter2D::TransformAdapter2D(
    const AnimationBuilder& abuilder, const skjson::ObjectValue* janchor_point,
    ...
    const skjson::ObjectValue* jskew_axis, bool auto_orient)
...
sk_sp<sksg::Transform> AnimationBuilder::attachMatrix2D(
    const skjson::ObjectValue& jtransform, sk_sp<sksg::Transform> parent, bool auto_orient) const {
  ...
  auto adapter = TransformAdapter2D::Make(
      *this, jtransform["a"], jtransform["p"], jtransform["s"], *jrotation, jtransform["sk"],
      jtransform["sa"], auto_orient);
  SkASSERT(adapter);
...
TransformAdapter3D::TransformAdapter3D(
    const skjson::ObjectValue& jtransform, const AnimationBuilder& abuilder)
...
sk_sp<sksg::Transform> AnimationBuilder::attachMatrix3D(
    const skjson::ObjectValue& jtransform, sk_sp<sksg::Transform> parent,
