...
std::unique_ptr<SkFILEWStream> MakeFrameStream(size_t idx, const char* ext) {
  ...
  auto stream = skstd::make_unique<SkFILEWStream>(
...
class NullSink final : public Sink {
 ...
 private:
  NullSink(sk_sp<SkSurface> surface, const SkMatrix& scale_matrix) : fSurface(std::move(surface)) {
    fSurface->getCanvas()->concat(scale_matrix);
  }
...
int main(int argc, char** argv) {
  ...
  auto rp = skottie_utils::CachingResourceProvider::Make(skottie_utils::FileResourceProvider::Make(
      SkOSPath::Dirname(FLAGS_input[0]),
      /*predecode=*/true));
  ...
  tg.batch(frame_count, [&](int i) {
    ...
    auto anim = skottie::Animation::Builder().setResourceProvider(rp).make(
        static_cast<const char*>(data->data()), data->size());
  ...
  if (FLAGS_format.contains("mp4")) {
    ...
    SkDebugf(
        "starved min %gms, med %gms, avg %gms, max %gms, sum %gms, first %gms (%s)\n",
  ...
  SkDebugf(
      "frame time min %gms, med %gms, avg %gms, max %gms, sum %gms\n", frames_ms[0],
