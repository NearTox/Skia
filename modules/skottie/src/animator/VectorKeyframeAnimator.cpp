...
class VectorKeyframeAnimator final : public KeyframeAnimator {
 ...
 private:
  StateChanged onSeek(float t) override {
    ...
    const auto is_constant = lerp_info.vrec0.equals(lerp_info.vrec1, Keyframe::Value::Type::kIndex);
    if (is_constant) {
    ...
    while (count >= 4) {
      const auto old_val = skvx::float4::Load(dst),
                 new_val = Lerp(skvx::float4::Load(v0), skvx::float4::Load(v1), lerp_info.weight);
