...
static bool parse_encoding_data(const skjson::Value& jv, size_t data_len, float data[]) {
  ...
  auto parse_optional_point = [&parse_point](
                                  const skjson::ArrayValue* ja, size_t i, float* x, float* y) {
    if (!ja || i >= ja->size()) {
...
ShapeValue::operator SkPath() const {
  ...
  auto addCubic = [&](size_t from_vertex, size_t to_vertex) {
    const auto from_index = kFloatsPerVertex * from_vertex, to_index = kFloatsPerVertex * to_vertex;

    const SkPoint p0 = SkPoint{fData[from_index + kX_Index], fData[from_index + kY_Index]},
                  p1 = SkPoint{fData[to_index + kX_Index], fData[to_index + kY_Index]},
                  c0 = SkPoint{fData[from_index + kOutX_Index], fData[from_index + kOutY_Index]} +
                       p0,
                  c1 = SkPoint{fData[to_index + kInX_Index], fData[to_index + kInY_Index]} + p1;
