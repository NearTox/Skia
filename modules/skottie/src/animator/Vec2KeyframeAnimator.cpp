...
class Vec2ExpressionAnimator final : public Animator {
 public:
  Vec2ExpressionAnimator(
      sk_sp<ExpressionEvaluator<std::vector<float>>> expression_evaluator, Vec2Value* target_value)
...
class Vec2AnimatorBuilder final : public AnimatorBuilder {
 ...
 private:
  void backfill_spatial(const Vec2KeyframeAnimator::SpatialValue& val) {
    ...
    p.cubicTo(
        prev_val.v2.x + fTo.x, prev_val.v2.y + fTo.y, val.v2.x + fTi.x, val.v2.y + fTi.y, val.v2.x,
        val.v2.y);
