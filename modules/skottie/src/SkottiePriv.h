...
class AnimationBuilder final : public SkNoncopyable {
 public:
  AnimationBuilder(
      ...
      sk_sp<MarkerObserver>, Animation::Builder::Stats*, const SkSize& size, float duration,
      float framerate);
  ...
  template <typename T>
  bool bindProperty(
      const skjson::Value&, AnimatorScope*, std::function<void(const T&)>&&,
      const T* default_igore = nullptr) const;

  template <typename T>
  bool bindProperty(
      const skjson::Value& jv, AnimatorScope* ascope, std::function<void(const T&)>&& apply,
  ...
  sk_sp<sksg::Color> attachColor(
      const skjson::ObjectValue&, AnimatorScope*, const char prop_name[]) const;
  sk_sp<sksg::Transform> attachMatrix2D(
      const skjson::ObjectValue&, AnimatorScope*, sk_sp<sksg::Transform>) const;
  sk_sp<sksg::Transform> attachMatrix3D(
      const skjson::ObjectValue&, AnimatorScope*, sk_sp<sksg::Transform>,
      sk_sp<TransformAdapter3D> = nullptr, bool precompose_parent = false) const;
  sk_sp<sksg::RenderNode> attachOpacity(
      const skjson::ObjectValue&, AnimatorScope*, sk_sp<sksg::RenderNode>) const;
 ...
 private:
  ...
  sk_sp<sksg::RenderNode> attachLayer(
      const skjson::ObjectValue*, AnimatorScope*, AttachLayerContext*) const;
  ...
  sk_sp<sksg::RenderNode> attachAssetRef(
      ...
      const std::function<sk_sp<sksg::RenderNode>(const skjson::ObjectValue&, AnimatorScope* ctx)>&)
      const;
  const ImageAssetInfo* loadImageAsset(const skjson::ObjectValue&) const;
  sk_sp<sksg::RenderNode> attachImageAsset(
  ...
  sk_sp<sksg::RenderNode> attachImageLayer(
      const skjson::ObjectValue&, LayerInfo*, AnimatorScope*) const;
  sk_sp<sksg::RenderNode> attachNullLayer(
      const skjson::ObjectValue&, LayerInfo*, AnimatorScope*) const;
  sk_sp<sksg::RenderNode> attachPrecompLayer(
      const skjson::ObjectValue&, LayerInfo*, AnimatorScope*) const;
  sk_sp<sksg::RenderNode> attachShapeLayer(
      const skjson::ObjectValue&, LayerInfo*, AnimatorScope*) const;
  sk_sp<sksg::RenderNode> attachSolidLayer(
      const skjson::ObjectValue&, LayerInfo*, AnimatorScope*) const;
  sk_sp<sksg::RenderNode> attachTextLayer(
      const skjson::ObjectValue&, LayerInfo*, AnimatorScope*) const;
  ...
  struct LayerInfo {
    SkSize fSize;
    const float fInPoint, fOutPoint;
  };
...
struct AnimationBuilder::AttachLayerContext {
 ...
 private:
  ...
  sk_sp<sksg::Transform> attachTransformNode(
      const skjson::ObjectValue& jlayer, const AnimationBuilder* abuilder, AnimatorScope*,
