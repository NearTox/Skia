...
sk_sp<sksg::RenderNode> AttachGradientLayerEffect(const skjson::ArrayValue& jprops,
    ...
    abuilder->bindProperty<ScalarValue>((*sc)["v"], ascope, [adapter](const ScalarValue& scatter) {
...
sk_sp<sksg::RenderNode> AttachFillLayerEffect(const skjson::ArrayValue& jprops,
    ...
    return sksg::ModeColorFilter::Make(std::move(layer), std::move(color_node),
                                       SkBlendMode::kSrcIn);
}
...
sk_sp<sksg::RenderNode> AttachLevelsLayerEffect(const skjson::ArrayValue& jprops,
    ...
    abuilder->bindProperty<ScalarValue>(
            (*iblack_prop)["v"], ascope,
            [adapter](const ScalarValue& ib) { adapter->setInBlack(ib); });
    abuilder->bindProperty<ScalarValue>(
            (*iwhite_prop)["v"], ascope,
            [adapter](const ScalarValue& iw) { adapter->setInWhite(iw); });
    abuilder->bindProperty<ScalarValue>(
            (*oblack_prop)["v"], ascope,
            [adapter](const ScalarValue& ob) { adapter->setOutBlack(ob); });
    abuilder->bindProperty<ScalarValue>(
            (*owhite_prop)["v"], ascope,
            [adapter](const ScalarValue& ow) { adapter->setOutWhite(ow); });
    abuilder->bindProperty<ScalarValue>((*gamma_prop)["v"], ascope,
                                        [adapter](const ScalarValue& g) { adapter->setGamma(g); });
