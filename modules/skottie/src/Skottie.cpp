...
sk_sp<sksg::Transform> AnimationBuilder::attachMatrix2D(
  ...
  return (bound || dispatched) ? sksg::Transform::MakeConcat(std::move(parent), std::move(matrix))
                               : parent;
}
...
sk_sp<SkData> ResourceProvider::load(const char[], const char[]) const { return nullptr; }

sk_sp<ImageAsset> ResourceProvider::loadImageAsset(
    const char path[], const char name[], const char id[]) const {
  return nullptr;
}

sk_sp<SkData> ResourceProvider::loadFont(const char[], const char[]) const { return nullptr; }
...
sk_sp<Animation> Animation::Builder::make(const char* data, size_t data_len) {
  ...
  internal::AnimationBuilder builder(
      ...
      std::move(fMarkerObserver), &fStats, size, duration, fps);
  ...
  return sk_sp<Animation>(new Animation(
      std::move(scene), std::move(version), size, inPoint, outPoint, duration, flags));
}
...
Animation::Animation(
    std::unique_ptr<sksg::Scene> scene, SkString version, const SkSize& size, SkScalar inPoint,
    SkScalar outPoint, SkScalar duration, uint32_t flags)
