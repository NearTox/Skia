...
void AnimationBuilder::log(
    Logger::Level lvl, const skjson::Value* json, const char fmt[], ...) const {
  if (!fLogger) {
    return;
  }
...
class OpacityAdapter final : public DiscardableAdapterBase<OpacityAdapter, sksg::OpacityEffect> {
 public:
  OpacityAdapter(
      const skjson::ObjectValue& jobject, sk_sp<sksg::RenderNode> child,
 ...
 private:
  void onSync() override { this->node()->setOpacity(fOpacity * 0.01f); }
...
static SkBlendMode GetBlendMode(
    const skjson::ObjectValue& jobject, const AnimationBuilder* abuilder) {
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachOpacity(
    const skjson::ObjectValue& jobject, sk_sp<sksg::RenderNode> child_node) const {
...
sk_sp<sksg::RenderNode> AnimationBuilder::attachBlendMode(
    const skjson::ObjectValue& jobject, sk_sp<sksg::RenderNode> child) const {
...
AnimationBuilder::AnimationBuilder(
    ...
    sk_sp<ExpressionManager> expressionmgr, Animation::Builder::Stats* stats,
...
void AnimationBuilder::dispatchMarkers(const skjson::ArrayValue* jmarkers) const {
  ...
  for (const skjson::ObjectValue* m : *jmarkers) {
    ...
    const auto time = ParseDefault((*m)["tm"], -1.0f), duration = ParseDefault((*m)["dr"], -1.0f);
...
sk_sp<ExpressionManager> AnimationBuilder::expression_manager() const { return fExpressionManager; }

void AnimationBuilder::AutoPropertyTracker::updateContext(
    PropertyObserver* observer, const skjson::ObjectValue& obj) {
...
sk_sp<Animation> Animation::Builder::make(const char* data, size_t data_len) {
  ...
  auto resolvedProvider =
      fResourceProvider ? fResourceProvider : sk_make_sp<NullResourceProvider>();
  ...
  if (size.isEmpty() || version.isEmpty() || fps <= 0 || !SkScalarIsFinite(inPoint) ||
      !SkScalarIsFinite(outPoint) || !SkScalarIsFinite(duration)) {
    if (fLogger) {
  ...
  internal::AnimationBuilder builder(
      ...
      std::move(fMarkerObserver), std::move(fPrecompInterceptor), std::move(fExpressionManager),
  ...
  return sk_sp<Animation>(new Animation(
      std::move(ainfo.fScene), std::move(ainfo.fAnimators), std::move(version), size, inPoint,
      outPoint, duration, fps, flags));
}

sk_sp<Animation> Animation::Builder::makeFromFile(const char path[]) {
  const auto data = SkData::MakeFromFileName(path);

  return data ? this->make(static_cast<const char*>(data->data()), data->size()) : nullptr;
}

Animation::Animation(
    std::unique_ptr<sksg::Scene> scene, std::vector<sk_sp<internal::Animator>>&& animators,
    SkString version, const SkSize& size, double inPoint, double outPoint, double duration,
...
sk_sp<Animation> Animation::Make(SkStream* stream) { return Builder().make(stream); }

sk_sp<Animation> Animation::MakeFromFile(const char path[]) { return Builder().makeFromFile(path); }
