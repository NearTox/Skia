...
sk_sp<sksg::RenderNode> AnimationBuilder::attachPrecompLayer(
    const skjson::ObjectValue& jlayer, const LayerInfo&, AnimatorScope* ascope) const {
  ...
  const auto requires_time_mapping =
      !SkScalarNearlyEqual(start_time, 0) || !SkScalarNearlyEqual(stretch_time, 1) || time_remap;

  AnimatorScope local_animators;
  auto precomp_layer = this->attachAssetRef(
      jlayer, requires_time_mapping ? &local_animators : ascope,
      [this](const skjson::ObjectValue& jcomp, AnimatorScope* ascope) {
  ...
  class CompTimeMapper final : public sksg::GroupAnimator {
   public:
    CompTimeMapper(sksg::AnimatorList&& layer_animators, float time_bias, float time_scale)
        : INHERITED(std::move(layer_animators)), fTimeBias(time_bias), fTimeScale(time_scale) {}
  ...
  if (requires_time_mapping) {
    const auto t_bias = -start_time, t_scale = sk_ieee_float_divide(1, stretch_time);
    auto time_mapper = skstd::make_unique<CompTimeMapper>(
        std::move(local_animators), t_bias, sk_float_isfinite(t_scale) ? t_scale : 0);
    if (time_remap) {
      ...
      this->bindProperty<ScalarValue>(
          *time_remap, ascope, [raw_mapper, frame_rate](const ScalarValue& t) {
