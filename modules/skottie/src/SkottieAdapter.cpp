...
RRectAdapter::RRectAdapter(sk_sp<sksg::RRect> wrapped_node) : fRRectNode(std::move(wrapped_node)) {}

RRectAdapter::~RRectAdapter() = default;

void RRectAdapter::apply() {
  // BM "position" == "center position"
  auto rr = SkRRect::MakeRectXY(
      SkRect::MakeXYWH(
          fPosition.x() - fSize.width() / 2, fPosition.y() - fSize.height() / 2, fSize.width(),
          fSize.height()),
      fRadius.width(), fRadius.height());
...
void TransformAdapter2D::apply() { fMatrixNode->setMatrix(this->totalMatrix()); }
...
sk_sp<sksg::Transform> TransformAdapter3D::refTransform() const { return fMatrixNode; }
...
void TransformAdapter3D::apply() { fMatrixNode->setMatrix(this->totalMatrix()); }

CameraAdapter::CameraAdapter(const SkSize& viewport_size) : fViewportSize(viewport_size) {}
...
SkMatrix44 CameraAdapter::totalMatrix() const {
  ...
  //
  SkPoint3 pos = {this->getPosition().fX, this->getPosition().fY, -this->getPosition().fZ},
           poi = {this->getAnchorPoint().fX, this->getAnchorPoint().fY, -this->getAnchorPoint().fZ},
           up = {0, 1, 0};
...
RepeaterAdapter::RepeaterAdapter(sk_sp<sksg::RenderNode> repeater_node, Composite composite)
    : fRepeaterNode(repeater_node), fComposite(composite), fRoot(sksg::Group::Make()) {}
...
void RepeaterAdapter::apply() {
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    fRoot->addChild(sksg::TransformEffect::Make(fRepeaterNode, compute_transform(insert_index)));
  }
}

PolyStarAdapter::PolyStarAdapter(sk_sp<sksg::Path> wrapped_node, Type t)
    : fPathNode(std::move(wrapped_node)), fType(t) {}
...
GradientAdapter::GradientAdapter(sk_sp<sksg::Gradient> grad, size_t stopCount)
    : fGradient(std::move(grad)), fStopCount(stopCount) {}
...
void DropShadowEffectAdapter::apply() {
  ...
  fDropShadow->setMode(
      fShadowOnly ? sksg::DropShadowImageFilter::Mode::kShadowOnly
...
void GaussianBlurEffectAdapter::apply() {
  ...
  const auto dim_index =
      SkTPin<size_t>(static_cast<size_t>(fDimensions), 1, SK_ARRAY_COUNT(kDimensionsMap)) - 1;
  ...
  const auto repeat_index =
      SkTPin<size_t>(static_cast<size_t>(fRepeatEdge), 0, SK_ARRAY_COUNT(kRepeatEdgeMap) - 1);
...
void LevelsEffectAdapter::apply() {
  ...
  auto in_0 = SkTPin(fInBlack, 0.0f, 1.0f), in_1 = SkTPin(fInWhite, 0.0f, 1.0f),
       out_0 = SkTPin(fOutBlack, 0.0f, 1.0f), out_1 = SkTPin(fOutWhite, 0.0f, 1.0f),
  ...
  auto dIn = in_1 - in_0, dOut = out_1 - out_0;
  ...
  auto t = -in_0 / dIn, dT = 1 / 255.0f / dIn;
