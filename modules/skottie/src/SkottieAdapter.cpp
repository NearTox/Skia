...
CameraAdapter::CameraAdapter(const SkSize& viewport_size, Type type)
    : fViewportSize(viewport_size), fType(type) {}
...
sk_sp<CameraAdapter> CameraAdapter::MakeDefault(const SkSize& viewport_size) {
  ...
  const auto center = SkVector::Make(viewport_size.width() * 0.5f, viewport_size.height() * 0.5f);
...
SkPoint3 CameraAdapter::poi() const {
  ...
  return fType == Type::kOneNode
             ? SkPoint3{this->getPosition().fX, this->getPosition().fY, -this->getPosition().fZ - 1}
             : SkPoint3{
                   this->getAnchorPoint().fX, this->getAnchorPoint().fY,
...
SkMatrix44 CameraAdapter::totalMatrix() const {
  ...
  //
  const auto pos =
                 SkPoint3{this->getPosition().fX, this->getPosition().fY, -this->getPosition().fZ},
             up = SkPoint3{0, 1, 0};
...
void GradientAdapter::apply() {
  ...
  const auto* c_rec = c_count > 0 ? reinterpret_cast<const ColorRec*>(fStops.data()) : nullptr;
  const auto* o_rec =
      o_count > 0 ? reinterpret_cast<const OpacityRec*>(fStops.data() + c_size) : nullptr;
  ...
  while (c_rec || o_rec) {
    ...
    const auto& cs = c_rec ? *c_rec
                           : ColorRec{
                                 o_rec->t, current_stop.fColor.fR, current_stop.fColor.fG,
                                 current_stop.fColor.fB};
    const auto& os = o_rec ? *o_rec : OpacityRec{c_rec->t, current_stop.fColor.fA};
