...
void RRectAdapter::apply() {
    ...
    auto rr = SkRRect::MakeRectXY(
            SkRect::MakeXYWH(fPosition.x() - fSize.width() / 2, fPosition.y() - fSize.height() / 2,
...
SkMatrix44 CameraAdapter::totalMatrix() const {
    ...
    SkPoint3 pos = {this->getPosition().fX, this->getPosition().fY, -this->getPosition().fZ},
             poi = {this->getAnchorPoint().fX, this->getAnchorPoint().fY,
...
void RepeaterAdapter::apply() {
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        fRoot->addChild(
                sksg::TransformEffect::Make(fRepeaterNode, compute_transform(insert_index)));
    }
}
