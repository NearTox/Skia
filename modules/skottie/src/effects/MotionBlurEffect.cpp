...
class MotionBlurEffect::AutoInvalBlocker {
 public:
  ...
  ~AutoInvalBlocker() { fMBNode->observeInval(fChild); }
...
sk_sp<MotionBlurEffect> MotionBlurEffect::Make(
  ...
  const auto samples_duration = shutter_angle / 360, phase = shutter_phase / 360,
             dt = samples_duration / (samples_per_frame - 1);

  return sk_sp<MotionBlurEffect>(
      new MotionBlurEffect(std::move(animator), std::move(child), samples_per_frame, phase, dt));
}
...
void MotionBlurEffect::renderToRaster8888Pow2Samples(
    SkCanvas* canvas, const RenderContext* ctx) const {
...
void MotionBlurEffect::onRender(SkCanvas* canvas, const RenderContext* ctx) const {
  ...
  if (canvas->peekPixels(&pm) &&
      (canvas->imageInfo().colorType() == kRGBA_8888_SkColorType ||
