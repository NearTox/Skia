...
class LevelsEffectAdapter final : public AnimatablePropertyContainer {
 public:
  static sk_sp<LevelsEffectAdapter> Make(
      const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer,
      const AnimationBuilder* abuilder) {
    return sk_sp<LevelsEffectAdapter>(new LevelsEffectAdapter(jprops, std::move(layer), abuilder));
  }
 ...
 private:
  LevelsEffectAdapter(
    ...
    this->bind(*abuilder, EffectBuilder::GetPropValue(jprops, kClipToOutBlack_Index), &fClipBlack);
    this->bind(*abuilder, EffectBuilder::GetPropValue(jprops, kClipToOutWhite_Index), &fClipWhite);
  }
  ...
  ScalarValue fChannel = 1,  // 1: RGB, 2: R, 3: G, 4: B, 5: A
      fInBlack = 0, fInWhite = 1, fOutBlack = 0, fOutWhite = 1, fGamma = 1,
...
sk_sp<sksg::RenderNode> EffectBuilder::attachLevelsEffect(
  ...
  return fBuilder->attachDiscardableAdapter<LevelsEffectAdapter>(
