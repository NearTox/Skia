...
struct ChannelMapper {
  ...
  const uint8_t* build_lut(std::array<uint8_t, 256>& lut_storage, const ClipInfo& clip_info) const noexcept {
    ...
    if (SkScalarNearlyEqual(in_0, out_0) && SkScalarNearlyEqual(in_1, out_1) &&
...
class EasyLevelsEffectAdapter final
    : public DiscardableAdapterBase<EasyLevelsEffectAdapter, sksg::ExternalColorFilter> {
 public:
...
class ProLevelsEffectAdapter final
    : public DiscardableAdapterBase<ProLevelsEffectAdapter, sksg::ExternalColorFilter> {
 public:
 ...
 private:
  void onSync() override {
    std::array<uint8_t, 256> a_lut_storage, r_lut_storage, g_lut_storage, b_lut_storage;

    auto cf = SkTableColorFilter::MakeARGB(
        fAMapper.build_lut(a_lut_storage, fClip), fRMapper.build_lut(r_lut_storage, fClip),
        fGMapper.build_lut(g_lut_storage, fClip), fBMapper.build_lut(b_lut_storage, fClip));

    // The RGB mapper composes outside individual channel mappers.
    if (const auto* rgb_lut = fRGBMapper.build_lut(a_lut_storage, fClip)) {
      cf = SkColorFilters::Compose(
          SkTableColorFilter::MakeARGB(nullptr, rgb_lut, rgb_lut, rgb_lut), std::move(cf));
    }

    this->node()->setColorFilter(std::move(cf));
  }

  ChannelMapper fRGBMapper, fRMapper, fGMapper, fBMapper, fAMapper;
...
sk_sp<sksg::RenderNode> EffectBuilder::attachEasyLevelsEffect(
  ...
  return fBuilder->attachDiscardableAdapter<EasyLevelsEffectAdapter>(
      jprops, std::move(layer), fBuilder);
}

sk_sp<sksg::RenderNode> EffectBuilder::attachProLevelsEffect(
    const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer) const {
  return fBuilder->attachDiscardableAdapter<ProLevelsEffectAdapter>(
      jprops, std::move(layer), fBuilder);
}
