...
class TileRenderNode final : public sksg::CustomRenderNode {
 ...
 protected:
  ...
  SkRect onRevalidate(sksg::InvalidationController* ic, const SkMatrix& ctm) override {
    ...
    const auto tile_size = SkSize::Make(std::max(tileW, 1.0f), std::max(tileH, 1.0f));
    const auto tile = SkRect::MakeXYWH(
    ...
    if (fPhase) {
      ...
      const auto phase_vec =
          fHorizontalPhase ? SkVector::Make(tile.width(), 0) : SkVector::Make(0, tile.height());
      const auto phase_shift = SkVector::Make(
      ...
      fPhasePassShader =
          SkShaders::Blend(SkBlendMode::kSrcOut, mask_shader, layer_shader, &phase_shader_matrix);
    } else {
    ...
    return SkRect::MakeXYWH(
        (fLayerSize.width() - outputW) * 0.5f, (fLayerSize.height() - outputH) * 0.5f, outputW,
        outputH);
  }
 ...
 private:
  ...
  SkScalar fTileW = 1, fTileH = 1, fOutputW = 1, fOutputH = 1, fPhase = 0;
...
class MotionTileAdapter final : public DiscardableAdapterBase<MotionTileAdapter, TileRenderNode> {
 public:
  MotionTileAdapter(
      ...
      const AnimationBuilder& abuilder, const SkSize& layer_size)
 ...
 private:
  ...
  ScalarValue fTileW = 1, fTileH = 1, fOutputW = 1, fOutputH = 1, fMirrorEdges = 0, fPhase = 0,
              fHorizontalPhase = 0;
...
sk_sp<sksg::RenderNode> EffectBuilder::attachMotionTileEffect(
  ...
  return fBuilder->attachDiscardableAdapter<MotionTileAdapter>(
      jprops, std::move(layer), *fBuilder, fLayerSize);
}
