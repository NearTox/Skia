...
class VenetialBlindsAdapter final : public MaskFilterEffectBase {
 ...
 private:
  MaskInfo onMakeMask() const override {
    ...
    return {SkShaderMaskFilter::Make(SkGradientShader::MakeLinear(
...
sk_sp<sksg::RenderNode> EffectBuilder::attachVenetianBlindsEffect(
  ...
  fBuilder->bindProperty<ScalarValue>(
      ...
      [adapter](const ScalarValue& d) { adapter->setDirection(d); });
  fBuilder->bindProperty<ScalarValue>(
      GetPropValue(jprops, kWidth_Index), fScope,
      [adapter](const ScalarValue& w) { adapter->setWidth(w); });
  fBuilder->bindProperty<ScalarValue>(
