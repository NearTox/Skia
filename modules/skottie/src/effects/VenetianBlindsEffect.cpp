...
class VenetianBlindsAdapter final : public MaskFilterEffectBase {
 public:
  static sk_sp<VenetianBlindsAdapter> Make(
 ...
 private:
  VenetianBlindsAdapter(
      const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer, const SkSize& ls,
      const AnimationBuilder* abuilder)
  ...
  MaskInfo onMakeMask() const override {
    ...
    const auto t = fCompletion * 0.01f, size = std::max(1.0f, fWidth),
        ...
        df0 = 0.5f * std::min(df, t), df1 = 0.5f * std::min(df, 1 - t);
    ...
    const auto center = SkPoint::Make(
                   0.5f * this->layerSize().width(), 0.5f * this->layerSize().height()),
               grad_vec = SkVector::Make(size * std::cos(angle), -size * std::sin(angle));
    ...
    return {
        SkShaderMaskFilter::Make(SkGradientShader::MakeLinear(
            pts, colors, pos, SK_ARRAY_COUNT(colors), SkTileMode::kRepeat, 0, nullptr)),
        true};
  }
...
sk_sp<sksg::RenderNode> EffectBuilder::attachVenetianBlindsEffect(
  ...
  return fBuilder->attachDiscardableAdapter<VenetianBlindsAdapter>(
