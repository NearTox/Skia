 ...
 private:
  FillAdapter(
      ...
      : fColorNode(sksg::Color::Make(SK_ColorBLACK)),
        fFilterNode(
            sksg::ModeColorFilter::Make(std::move(layer), fColorNode, SkBlendMode::kSrcIn)) {
    enum : size_t {
      // kFillMask_Index = 0,
      // kAllMasks_Index = 1,
      kColor_Index = 2,
    ...
    EffectBinder(jprops, abuilder, this).bind(kColor_Index, fColor).bind(kOpacity_Index, fOpacity);
  ...
  void onSync() override {
    ...
    const auto a = sk_float_round2int_no_saturate(SkColorGetA(c) * SkTPin(fOpacity, 0.0f, 1.0f));
...
sk_sp<sksg::RenderNode> EffectBuilder::attachFillEffect(
