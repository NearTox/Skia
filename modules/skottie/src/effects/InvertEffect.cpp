...
class InvertEffectAdapter final : public AnimatablePropertyContainer {
 public:
  static sk_sp<InvertEffectAdapter> Make(
 ...
 private:
  ...
  void onSync() override {
    ...
    SkColorMatrix m(
        stcm.scale[0], 0, 0, 0, stcm.trans[0], 0, stcm.scale[1], 0, 0, stcm.trans[1], 0, 0,
    ...
    if (stcm.cs == CS::kYIQ) {
      ...
      static constexpr SkColorMatrix RGB2YIQ(
          0.2990f, 0.5870f, 0.1140f, 0, 0, 0.5959f, -0.2746f, -0.3213f, 0, 0, 0.2115f, -0.5227f,
          0.3112f, 0, 0, 0, 0, 0, 1, 0);
      static constexpr SkColorMatrix YIQ2RGB(
          1, 0.9560f, 0.6190f, 0, 0, 1, -0.2720f, -0.6470f, 0, 0, 1, -1.1060f, 1.7030f, 0, 0, 0, 0,
          0, 1, 0);
    ...
    fColorFilter->setColorFilter(
        stcm.cs == CS::kHSL ? SkColorFilters::HSLAMatrix(m) : SkColorFilters::Matrix(m));
  }
...
sk_sp<sksg::RenderNode> EffectBuilder::attachInvertEffect(
  ...
  return fBuilder->attachDiscardableAdapter<InvertEffectAdapter>(
