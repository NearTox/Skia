...
class DisplacementNode final : public sksg::CustomRenderNode {
 public:
  ~DisplacementNode() override { this->unobserveInval(fDisplSource); }

  static sk_sp<DisplacementNode> Make(
      sk_sp<RenderNode> child, const SkSize& child_size, sk_sp<RenderNode> displ,
      const SkSize& displ_size) {
    if (!child || !displ) {
      return nullptr;
    }

    return sk_sp<DisplacementNode>(
        new DisplacementNode(std::move(child), child_size, std::move(displ), displ_size));
  }
 ...
 private:
  ...
  static bool IsConst(Selector s) {
    return s == Selector::kFull || s == Selector::kHalf || s == Selector::kOff;
  }
  ...
  sk_sp<SkShader> buildEffectShader(sksg::InvalidationController* ic, const SkMatrix& ctm) {
    ...
    auto child_shader = child_content->makeShader(
        fChildTileMode, fChildTileMode, SkFilterMode::kLinear, nullptr, &child_tile);
    ...
    auto displ_shader = displ_content->makeShader(
        displ_mode, displ_mode, SkFilterMode::kLinear, &displ_matrix, &displ_tile);
    ...
    const auto xc = Coeffs(fXSelector), yc = Coeffs(fYSelector);

    const auto s = fScale * 2;

    const float selector_m[] = {
        xc.dr * s.x, yc.dr * s.y, 0, 0, xc.dg * s.x, yc.dg * s.y, 0,          0,
        xc.db * s.x, yc.db * s.y, 0, 0, xc.da * s.x, yc.da * s.y, xc.c_scale, yc.c_scale,
  ...
  void onRender(SkCanvas* canvas, const RenderContext* ctx) const override {
    ...
    auto local_ctx = ScopedRenderContext(canvas, ctx)
  ...
  SkTileMode displacementTileMode() const {
    return fPos == Pos::kTile ? SkTileMode::kRepeat : SkTileMode::kClamp;
  }
    ...
    switch (fPos) {
      ...
      case Pos::kStretch:
        return SkMatrix::Scale(
            fChildSize.fWidth / fDisplSize.fWidth, fChildSize.fHeight / fDisplSize.fHeight);
        break;
      case Pos::kTile: return SkMatrix::I(); break;
    }
    SkUNREACHABLE;
  }
  ...
  const SkSize fDisplSize, fChildSize;
  ...
  Selector fXSelector = Selector::kR, fYSelector = Selector::kR;
  bool fExpandBounds = false;

  using INHERITED = sksg::CustomRenderNode;
};

class DisplacementMapAdapter final
    : public DiscardableAdapterBase<DisplacementMapAdapter, DisplacementNode> {
 public:
  ...
  static std::tuple<sk_sp<sksg::RenderNode>, SkSize> GetDisplacementSource(
      const skjson::ArrayValue& jprops, const EffectBuilder* ebuilder) {
 ...
 private:
  ...
  E ToEnum(float v) {
    ...
    const auto uv = std::min(static_cast<unsigned>(v) - 1, static_cast<unsigned>(E::kLast));
  ...
  void onSync() override {
    ...
    this->node()->setChildTileMode(fEdgeBehavior != 0 ? SkTileMode::kRepeat : SkTileMode::kDecal);
  ...
  ScalarValue fHorizontalSelector = 0, fVerticalSelector = 0, fMaxHorizontal = 0, fMaxVertical = 0,
              fMapBehavior = 0, fEdgeBehavior = 0, fExpandOutput = 0;
...
sk_sp<sksg::RenderNode> EffectBuilder::attachDisplacementMapEffect(
  ...
  return fBuilder->attachDiscardableAdapter<DisplacementMapAdapter>(
      jprops, fBuilder, std::move(displ_node));
#else
  // TODO(skia:12197)
  return layer;
#endif
