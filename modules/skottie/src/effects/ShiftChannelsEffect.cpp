...
class ShiftChannelsEffectAdapter final : public DiscardableAdaptorBase {
 public:
  static sk_sp<ShiftChannelsEffectAdapter> Make(
    ...
    auto adapter =
        sk_sp<ShiftChannelsEffectAdapter>(new ShiftChannelsEffectAdapter(std::move(layer)));
    ...
    abuilder->bindProperty<ScalarValue>(
        EffectBuilder::GetPropValue(jprops, kTakeRedFrom_Index),
        [raw_adapter](const ScalarValue& r) { raw_adapter->fR = r; });
    abuilder->bindProperty<ScalarValue>(
        EffectBuilder::GetPropValue(jprops, kTakeGreenFrom_Index),
        [raw_adapter](const ScalarValue& g) { raw_adapter->fG = g; });
    abuilder->bindProperty<ScalarValue>(
        EffectBuilder::GetPropValue(jprops, kTakeBlueFrom_Index),
        [raw_adapter](const ScalarValue& b) { raw_adapter->fB = b; });
    abuilder->bindProperty<ScalarValue>(
        EffectBuilder::GetPropValue(jprops, kTakeAlphaFrom_Index),
 ...
 private:
  ...
  void onSync() override {
    ...
    const float cm[] = {
        rc[0], rc[1], rc[2], rc[3], rc[4], gc[0], gc[1], gc[2], gc[3], gc[4],
        bc[0], bc[1], bc[2], bc[3], bc[4], ac[0], ac[1], ac[2], ac[3], ac[4],
    };
  ...
  float fR = static_cast<float>(Source::kRed), fG = static_cast<float>(Source::kGreen),
        fB = static_cast<float>(Source::kBlue), fA = static_cast<float>(Source::kAlpha);
