...
class GradientRampEffectAdapter final : public AnimatablePropertyContainer {
 public:
  static sk_sp<GradientRampEffectAdapter> Make(
      const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer,
      const AnimationBuilder* abuilder) {
    return sk_sp<GradientRampEffectAdapter>(
        new GradientRampEffectAdapter(jprops, std::move(layer), abuilder));
  }
 ...
 private:
  ...
  void onSync() override {
    ...
    auto update_gradient = [this](InstanceType new_type) {
      ...
      fGradient->setColorStops(
          {{0, ValueTraits<VectorValue>::As<SkColor4f>(fStartColor)},
...
sk_sp<sksg::RenderNode> EffectBuilder::attachGradientEffect(
  ...
  return fBuilder->attachDiscardableAdapter<GradientRampEffectAdapter>(
