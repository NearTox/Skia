...
EffectBuilder::EffectBuilder(
    const AnimationBuilder* abuilder, const SkSize& layer_size, CompositionBuilder* cbuilder)
...
EffectBuilder::EffectBuilderT EffectBuilder::findBuilder(const skjson::ObjectValue& jeffect) const {
  ...
  if (mn) {
    ...
    const auto* binfo = std::lower_bound(
        std::begin(gBuilderInfo), std::end(gBuilderInfo), key,
        [](const BuilderInfo& a, const BuilderInfo& b) { return strcmp(a.fName, b.fName) < 0; });
  ...
  fBuilder->log(
      Logger::Level::kWarning, &jeffect, "Unsupported layer effect: %s",
      mn ? mn->begin() : "(unknown)");

  return nullptr;
}

sk_sp<sksg::RenderNode> EffectBuilder::attachEffects(
    const skjson::ArrayValue& jeffects, sk_sp<sksg::RenderNode> layer) const {
  if (!layer) {
    return nullptr;
  }
  ...
  for (const skjson::ObjectValue* jeffect : jeffects) {
    ...
    const AnimationBuilder::AutoPropertyTracker apt(
        fBuilder, *jeffect, PropertyObserver::NodeType::EFFECT);
...
sk_sp<sksg::RenderNode> EffectBuilder::attachStyles(
    const skjson::ArrayValue& jstyles, sk_sp<sksg::RenderNode> layer) const {
  ...
  for (const skjson::ObjectValue* jstyle : jstyles) {
    ...
    auto builder =
        style_type < SK_ARRAY_COUNT(gStyleBuilders) ? gStyleBuilders[style_type] : nullptr;
...
const skjson::Value& EffectBuilder::GetPropValue(
    const skjson::ArrayValue& jprops, size_t prop_index) {
