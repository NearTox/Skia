...
class RWipeRenderNode final : public sksg::CustomRenderNode {
 public:
  explicit RWipeRenderNode(sk_sp<sksg::RenderNode> layer) : INHERITED({std::move(layer)}) {}
 ...
 protected:
  ...
  SkRect onRevalidate(sksg::InvalidationController* ic, const SkMatrix& ctm) override {
    ...
    if (fCompletion <= 0) {
      ...
      const SkColor c0 = fMaskSigma > 0 ? 0xffffffff : 0x00000000, c1 = 0xffffffff - c0;
      ...
      SkMatrix lm;
      lm.setRotate(fStartAngle - 90 + t * this->wipeAlignment(), fWipeCenter.x(), fWipeCenter.y());

      fMaskFilter = SkShaderMaskFilter::Make(SkGradientShader::MakeSweep(
          fWipeCenter.x(), fWipeCenter.y(), grad_colors, grad_pos, SK_ARRAY_COUNT(grad_colors), 0,
          &lm));

      // Edge feather requires a real blur.
      if (fMaskSigma > 0) {
        fMaskFilter = SkMaskFilter::MakeCompose(
            SkMaskFilter::MakeBlur(kNormal_SkBlurStyle, fMaskSigma), std::move(fMaskFilter));
      }
    }
 ...
 private:
  ...
  float fCompletion = 0, fStartAngle = 0, fWipe = 0, fFeather = 0;
...
sk_sp<sksg::RenderNode> EffectBuilder::attachRadialWipeEffect(
  ...
  fBuilder->bindProperty<ScalarValue>(
      ...
      [wiper](const ScalarValue& c) { wiper->setCompletion(c); });
  fBuilder->bindProperty<ScalarValue>(
      GetPropValue(jprops, kStartAngle_Index), fScope,
      [wiper](const ScalarValue& sa) { wiper->setStartAngle(sa); });
  fBuilder->bindProperty<VectorValue>(
      GetPropValue(jprops, kWipeCenter_Index), fScope, [wiper](const VectorValue& c) {
        wiper->setWipeCenter(ValueTraits<VectorValue>::As<SkPoint>(c));
      });
  fBuilder->bindProperty<ScalarValue>(
      GetPropValue(jprops, kWipe_Index), fScope,
      [wiper](const ScalarValue& w) { wiper->setWipe(w); });
  fBuilder->bindProperty<ScalarValue>(
      GetPropValue(jprops, kFeather_Index), fScope,
      [wiper](const ScalarValue& f) { wiper->setFeather(f); });
