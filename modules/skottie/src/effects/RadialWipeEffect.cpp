 ...
 protected:
  ...
  SkRect onRevalidate(sksg::InvalidationController* ic, const SkMatrix& ctm) override {
    ...
    if (fCompletion <= 0) {
      ...
      SkColor c0 = 0x00000000, c1 = 0xffffffff;
      ...
      fMaskShader = SkGradientShader::MakeSweep(
          fWipeCenter.x(), fWipeCenter.y(), grad_colors, grad_pos, SK_ARRAY_COUNT(grad_colors),
          SkTileMode::kClamp, a0, a1, 0, nullptr);
  ...
  void onRender(SkCanvas* canvas, const RenderContext* ctx) const override {
    ...
    const auto local_ctx =
        ScopedRenderContext(canvas, ctx).modulateMaskShader(fMaskShader, canvas->getTotalMatrix());
...
sk_sp<sksg::RenderNode> EffectBuilder::attachRadialWipeEffect(
  ...
  return fBuilder->attachDiscardableAdapter<RadialWipeAdapter>(jprops, std::move(layer), *fBuilder);
}
