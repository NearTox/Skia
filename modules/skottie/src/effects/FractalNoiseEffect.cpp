...
sk_sp<SkRuntimeEffect> make_noise_effect(unsigned loops, const char* filter, const char* fractal) {
  auto result =
      SkRuntimeEffect::MakeForShader(SkStringPrintf(gNoiseEffectSkSL, filter, fractal, loops), {});
...
sk_sp<SkRuntimeEffect> noise_effect() {
  static constexpr char const* gFilters[] = {
      gFilterNearestSkSL, gFilterLinearSkSL, gFilterSoftLinearSkSL};

  static constexpr char const* gFractals[] = {
      gFractalBasicSkSL, gFractalTurbulentBasicSkSL, gFractalTurbulentSmoothSkSL,
  ...
  static const SkRuntimeEffect* effect =
      make_noise_effect(
          LOOPS, gFilters[static_cast<size_t>(FILTER)], gFractals[static_cast<size_t>(FRACTAL)])
          .release();
...
class FractalNoiseNode final : public sksg::CustomRenderNode {
 ...
 private:
  ...
  sk_sp<SkRuntimeEffect> getEffect() const {
    switch (fFractal) {
      case NoiseFractal::kBasic: return this->getEffect<FI, NoiseFractal::kBasic>();
  ...
  void onRender(SkCanvas* canvas, const RenderContext* ctx) const override {
    ...
    const auto local_ctx =
        ScopedRenderContext(canvas, ctx).setIsolation(bounds, canvas->getTotalMatrix(), true);
  ...
  SkMatrix fMatrix, fSubMatrix;
  NoiseFilter fFilter = NoiseFilter::kNearest;
  NoiseFractal fFractal = NoiseFractal::kBasic;
  SkV2 fNoisePlanes = {0, 0};
  float fNoiseWeight = 0, fOctaves = 1, fPersistence = 1;

  using INHERITED = sksg::CustomRenderNode;
};

class FractalNoiseAdapter final
    : public DiscardableAdapterBase<FractalNoiseAdapter, FractalNoiseNode> {
 public:
  FractalNoiseAdapter(
 ...
 private:
  std::tuple<SkV2, float> noise() const {
    ...
    // must be integral.
    const float evo_rad = SkDegreesToRadians(fEvolution),
                rev_rad = std::max(fCycleRevolutions, 1.0f) * SK_FloatPI * 2,
                cycle = fCycleEvolution ? SkScalarRoundToScalar(rev_rad * kEvolutionScale)
                                        : SK_ScalarMax,
                // Adjust scale when cycling to ensure an integral period (post scaling).
        scale = fCycleEvolution ? cycle / rev_rad : kEvolutionScale,
                offset = SkRandom(static_cast<uint32_t>(fRandomSeed)).nextRangeU(0, 100),
                evo = evo_rad * scale, evo_ = std::floor(evo), weight = evo - evo_;

    // We want the GLSL mod() flavor.
    auto glsl_mod = [](float x, float y) { return x - y * std::floor(x / y); };
  ...
  SkMatrix shaderMatrix() const {
    ...
    const auto scale = (SkScalarRoundToInt(fUniformScaling) == 1) ? SkV2{fScale, fScale}
                                                                  : SkV2{fScaleWidth, fScaleHeight};

    return SkMatrix::Translate(fOffset.x, fOffset.y) *
           SkMatrix::Scale(
               SkTPin(scale.x, 1.0f, 10000.0f) * 0.01f, SkTPin(scale.y, 1.0f, 10000.0f) * 0.01f) *
           SkMatrix::RotateDeg(fRotation) * SkMatrix::Scale(kGridSize, kGridSize);
  }
  ...
  SkMatrix subMatrix() const {
    ...
    return SkMatrix::Translate(-fSubOffset.x * 0.01f, -fSubOffset.y * 0.01f) *
           SkMatrix::RotateDeg(-fSubRotation) * SkMatrix::Scale(scale, scale);
  }
  ...
  Vec2Value fOffset = {0, 0}, fSubOffset = {0, 0};

  ScalarValue fFractalType = 0, fNoiseType = 0,

              fRotation = 0, fUniformScaling = 0,
              fScale = 100,  // used when uniform scaling is selected
      fScaleWidth = 100,     // used when uniform scaling is not selected
      fScaleHeight = 100,    // ^

      fComplexity = 1, fSubInfluence = 100, fSubScale = 50, fSubRotation = 0,

              fEvolution = 0, fCycleEvolution = 0, fCycleRevolutions = 0, fRandomSeed = 0,
...
sk_sp<sksg::RenderNode> EffectBuilder::attachFractalNoiseEffect(
  ...
  return fBuilder->attachDiscardableAdapter<FractalNoiseAdapter>(
      jprops, fBuilder, std::move(fractal_noise));
#else
  // TODO(skia:12197)
  return layer;
#endif
