 ...
 private:
  sk_sp<SkShader> contentShader() {
    if (!fContentShader || this->hasChildrenInval()) {
      ...
      fContentShader = recorder.finishRecordingAsPicture()->makeShader(
          SkTileMode::kRepeat, SkTileMode::kRepeat, SkFilterMode::kLinear, nullptr, nullptr);
    }
  ...
  sk_sp<SkShader> buildEffectShader(float selector) {
    ...
    SkRuntimeShaderBuilder builder(
        has_fancy_light ? sphere_fancylight_effect() : sphere_basiclight_effect());
  ...
  SkRect onRevalidate(sksg::InvalidationController* ic, const SkMatrix& ctm) override {
    ...
    if (fSide != RenderSide::kInside) {
      ...
      fSphereShader = fSphereShader
                          ? SkShaders::Blend(
                                SkBlendMode::kSrcOver, std::move(fSphereShader), std::move(outside))
                          : std::move(outside);
    }
    SkASSERT(fSphereShader);

    return SkRect::MakeLTRB(
        fCenter.fX - fRadius, fCenter.fY - fRadius, fCenter.fX + fRadius, fCenter.fY + fRadius);
  }
...
class SphereAdapter final : public DiscardableAdapterBase<SphereAdapter, SphereNode> {
 public:
  SphereAdapter(
      const skjson::ArrayValue& jprops, const AnimationBuilder* abuilder, sk_sp<SphereNode> node)
 ...
 private:
  void onSync() override {
    ...
    const auto light_vec = [](float height, float direction) {
      float z = std::sin(height * SK_ScalarPI / 2), r = std::sqrt(1 - z * z),
            x = std::cos(direction) * r, y = std::sin(direction) * r;
    ...
    sph->setLightVec(light_vec(
        SkTPin(fLightHeight * 0.01f, -1.0f, 1.0f), SkDegreesToRadians(fLightDirection - 90)));
  ...
  ScalarValue fLightIntensity = 0, fLightHeight = 0, fLightDirection = 0, fAmbient = 100,
              fDiffuse = 0, fSpecular = 0, fRoughness = 0.5f;
