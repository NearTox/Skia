...
class TritoneAdapter final : public AnimatablePropertyContainer {
 public:
  static sk_sp<TritoneAdapter> Make(
 ...
 private:
  TritoneAdapter(
      ...
      : fLoColorNode(sksg::Color::Make(SK_ColorBLACK)),
        ...
        fCF(sksg::GradientColorFilter::Make(
            std::move(layer), {fLoColorNode, fMiColorNode, fHiColorNode})) {
    enum : size_t {
      kHiColor_Index = 0,
      kMiColor_Index = 1,
      kLoColor_Index = 2,
  ...
  const sk_sp<sksg::Color> fLoColorNode, fMiColorNode, fHiColorNode;
  const sk_sp<sksg::GradientColorFilter> fCF;

  VectorValue fLoColor, fMiColor, fHiColor;
