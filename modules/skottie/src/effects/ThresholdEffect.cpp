...
class ThresholdAdapter final
    : public DiscardableAdapterBase<ThresholdAdapter, sksg::ExternalColorFilter> {
 public:
  ThresholdAdapter(
 ...
 private:
  void onSync() override {
    auto cf = threshold_effect()->makeColorFilter(SkData::MakeWithCopy(&fLevel, sizeof(fLevel)));
...
sk_sp<sksg::RenderNode> EffectBuilder::attachThresholdEffect(
    const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer) const {
#ifdef SK_ENABLE_SKSL
  return fBuilder->attachDiscardableAdapter<ThresholdAdapter>(jprops, std::move(layer), *fBuilder);
#else
  // TODO(skia:12197)
  return layer;
#endif
