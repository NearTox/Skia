...
class BrightnessContrastAdapter final
    : public DiscardableAdapterBase<BrightnessContrastAdapter, sksg::ExternalColorFilter> {
 public:
 ...
 private:
  void onSync() override {
    this->node()->setColorFilter(
        SkScalarRoundToInt(fUseLegacy) ? this->makeLegacyCF() : this->makeCF());
  }
  ...
  sk_sp<SkColorFilter> makeLegacyCF() const {
    ...
    const auto S = contrast > 0 ? 1 / std::max(1 - contrast, SK_ScalarNearlyZero) : 1 + contrast;
    ...
    const float cm[] = {
        S, 0, 0, 0, B, 0, S, 0, 0, B, 0, 0, S, 0, B, 0, 0, 0, 1, 0,
    };
  ...
  sk_sp<SkColorFilter> makeCF() const {
    ...
    auto b_eff = SkScalarNearlyZero(brightness)
                     ? nullptr
                     : fBrightnessEffect->makeColorFilter(
                           make_brightness_coeffs(brightness), &input, 1),
  ...
  const sk_sp<SkRuntimeEffect> fBrightnessEffect, fContrastEffect;

  ScalarValue fBrightness = 0, fContrast = 0, fUseLegacy = 0;
...
sk_sp<sksg::RenderNode> EffectBuilder::attachBrightnessContrastEffect(
  ...
  return fBuilder->attachDiscardableAdapter<BrightnessContrastAdapter>(
