...
class CCTonerAdapter final
    : public DiscardableAdapterBase<CCTonerAdapter, sksg::GradientColorFilter> {
 public:
  CCTonerAdapter(
      const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer,
      const AnimationBuilder& abuilder, std::vector<sk_sp<sksg::Color>> colorNodes)
 ...
 private:
  ...
  void onSync() override {
    switch (SkScalarRoundToInt(fTone)) {
      // duotone
      case 1:
        fColorNodes.at(0)->setColor(fShadows);
      ...
      // solid
      default:
        fColorNodes.at(0)->setColor(fMidtones);
  ...
  ScalarValue fTone = 0;
  VectorValue fHighlights, fBrights, fMidtones, fDarktones, fShadows;
...
sk_sp<sksg::RenderNode> EffectBuilder::attachCCTonerEffect(
    const skjson::ArrayValue& jprops, sk_sp<sksg::RenderNode> layer) const {
  std::vector<sk_sp<sksg::Color>> colorNodes = {
      sksg::Color::Make(SK_ColorRED), sksg::Color::Make(SK_ColorRED),
      sksg::Color::Make(SK_ColorRED), sksg::Color::Make(SK_ColorRED),
      sksg::Color::Make(SK_ColorRED)};
  return fBuilder->attachDiscardableAdapter<CCTonerAdapter>(
      jprops, std::move(layer), *fBuilder, std::move(colorNodes));
}
