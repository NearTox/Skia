...
class HueSaturationEffectAdapter final : public DiscardableAdaptorBase {
 public:
  static sk_sp<HueSaturationEffectAdapter> Make(
    ...
    auto adapter =
        sk_sp<HueSaturationEffectAdapter>(new HueSaturationEffectAdapter(std::move(layer)));

    auto* raw_adapter = adapter.get();

    abuilder->bindProperty<ScalarValue>(
        EffectBuilder::GetPropValue(jprops, kChannelControl_Index),
        [raw_adapter](const ScalarValue& c) { raw_adapter->fChanCtrl = c; });
    abuilder->bindProperty<ScalarValue>(
        EffectBuilder::GetPropValue(jprops, kMasterHue_Index),
        [raw_adapter](const ScalarValue& h) { raw_adapter->fMasterHue = h; });
    abuilder->bindProperty<ScalarValue>(
        EffectBuilder::GetPropValue(jprops, kMasterSat_Index),
        [raw_adapter](const ScalarValue& s) { raw_adapter->fMasterSat = s; });
    abuilder->bindProperty<ScalarValue>(
        EffectBuilder::GetPropValue(jprops, kMasterLightness_Index),
        [raw_adapter](const ScalarValue& l) { raw_adapter->fMasterLight = l; });
 ...
 protected:
  void onSync() override { fColorFilter->setColorFilter(this->makeColorFilter()); }
 ...
 private:
  ...
  sk_sp<SkColorFilter> makeColorFilter() const {
    ...
    //                                 ( 0..100] -> (color.L .. 1]
    const auto h = fMasterHue / 360, s = SkTPin(fMasterSat / 100, -1.0f, 1.0f),
               l = SkTPin(fMasterLight / 100, -1.0f, 1.0f), h_bias = h, s_bias = std::max(s, 0.0f),
               s_scale = 1 - std::abs(s), l_bias = std::max(l, 0.0f), l_scale = 1 - std::abs(l);

    const float hsl_cm[20] = {
        1, 0, 0, 0, h_bias, 0, s_scale, 0, 0, s_bias, 0, 0, l_scale, 0, l_bias, 0, 0, 0, 1, 0,
    };
...
sk_sp<sksg::RenderNode> EffectBuilder::attachHueSaturationEffect(
  ...
  return fBuilder->attachDiscardableAdapter<HueSaturationEffectAdapter>(
      jprops, std::move(layer), fBuilder);
}
