...
class LinearWipeAdapter final : public MaskShaderEffectBase {
 public:
  static sk_sp<LinearWipeAdapter> Make(
    ...
    return sk_sp<LinearWipeAdapter>(
        new LinearWipeAdapter(jprops, std::move(layer), layer_size, abuilder));
  }
 ...
 private:
  ...
  MaskInfo onMakeMask() const override {
    ...
    const auto t = SkTPin(fCompletion * 0.01f, 0.0f, 1.0f), feather = std::max(fFeather, 0.0f),
               angle = SkDegreesToRadians(90 - fAngle), cos_ = std::cos(angle),
               sin_ = std::sin(angle);

    // Select the correct diagonal vector depending on quadrant.
    const SkVector angle_v = {cos_, sin_}, diag_v = {
                                               std::copysign(this->layerSize().width(), cos_),
    ...
    const SkVector grad_v = angle_v * grad_len,
        ...
        center_v = {0.5f * this->layerSize().width(), 0.5f * this->layerSize().height()};
    ...
    static constexpr SkColor colors[] = {0x00000000, 0xffffffff};
  ...
  ScalarValue fCompletion = 0, fAngle = 0, fFeather = 0;
...
sk_sp<sksg::RenderNode> EffectBuilder::attachLinearWipeEffect(
  ...
  return fBuilder->attachDiscardableAdapter<LinearWipeAdapter>(
      jprops, std::move(layer), fLayerSize, fBuilder);
}
