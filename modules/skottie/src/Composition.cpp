...
sk_sp<sksg::RenderNode> AnimationBuilder::attachNestedAnimation(const char* name) const {
  class SkottieSGAdapter final : public sksg::RenderNode {
   public:
    explicit SkottieSGAdapter(sk_sp<Animation> animation) noexcept : fAnimation(std::move(animation)) {
      SkASSERT(fAnimation);
    }
  ...
  class SkottieAnimatorAdapter final : public Animator {
   public:
    SkottieAnimatorAdapter(sk_sp<Animation> animation, float time_scale) noexcept
...
CompositionBuilder::CompositionBuilder(
    const AnimationBuilder& abuilder, const SkSize& size, const skjson::ObjectValue& jcomp)
    : fSize(size) {
  // Optional motion blur params.
  if (const skjson::ObjectValue* jmb = jcomp["mb"]) {
    static constexpr size_t kMaxSamplesPerFrame = 64;
    fMotionBlurSamples = std::min(ParseDefault<size_t>((*jmb)["spf"], 1ul), kMaxSamplesPerFrame);
