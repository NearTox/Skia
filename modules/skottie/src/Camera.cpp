...
SkM44 ComputeCameraMatrix(
    const SkV3& position, const SkV3& poi, const SkV3& rotation, const SkSize& viewport_size,
    float zoom) {
  ...
  const auto cam_t =
      ...
      SkM44::LookAt({position.x, position.y, -position.z}, {poi.x, poi.y, poi.z}, {0, 1, 0}) *
      SkM44::Scale(1, 1, -1);
  ...
  return SkM44::Translate(viewport_size.width() * 0.5f, viewport_size.height() * 0.5f, 0) *
         persp_t * cam_t;
}

}  // namespace

CameraAdaper::CameraAdaper(
    const skjson::ObjectValue& jlayer, const skjson::ObjectValue& jtransform,
...
SkM44 CameraAdaper::totalMatrix() const {
  ...
  return ComputeCameraMatrix(position, this->poi(position), this->rotation(), fViewportSize, fZoom);
}
...
sk_sp<sksg::Transform> CameraAdaper::DefaultCameraTransform(const SkSize& viewport_size) {
  ...
  const SkV3 pos = {center.fX, center.fY, -kDefaultAEZoom}, poi = {pos.x, pos.y, -pos.z - 1},
             rot = {0, 0, 0};
...
sk_sp<sksg::Transform> AnimationBuilder::attachCamera(
    const skjson::ObjectValue& jlayer, const skjson::ObjectValue& jtransform,
    sk_sp<sksg::Transform> parent, const SkSize& viewport_size) const {
