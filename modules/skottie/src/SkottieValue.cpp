...
template <>
void ValueTraits<ScalarValue>::Lerp(
    const ScalarValue& v0, const ScalarValue& v1, float t, ScalarValue* result) {
  *result = v0 + (v1 - v0) * t;
}
...
template <>
bool ValueTraits<VectorValue>::FromJSON(
    const skjson::Value& jv, const internal::AnimationBuilder*, VectorValue* v) {
  return Parse(jv, v);
}
...
template <>
void ValueTraits<VectorValue>::Lerp(
    const VectorValue& v0, const VectorValue& v1, float t, VectorValue* result) {
...
SkColor ValueTraits<VectorValue>::As<SkColor>(const VectorValue& v) {
  ...
  const auto r = v.size() > 0 ? v[0] : 0, g = v.size() > 1 ? v[1] : 0, b = v.size() > 2 ? v[2] : 0,
             a = v.size() > 3 ? v[3] : 1;
...
template <>
bool ValueTraits<ShapeValue>::FromJSON(
    const skjson::Value& jv, const internal::AnimationBuilder* abuilder, ShapeValue* v) {
...
template <>
void ValueTraits<ShapeValue>::Lerp(
    const ShapeValue& v0, const ShapeValue& v1, float t, ShapeValue* result) {
