...
template <> bool ValueTraits<ScalarValue>::CanLerp(const ScalarValue&, const ScalarValue&) {
    return true;
}
...
template <> template <> SkScalar ValueTraits<ScalarValue>::As<SkScalar>(const ScalarValue& v) {
    return v;
}
...
template <> bool ValueTraits<VectorValue>::CanLerp(const VectorValue& v1, const VectorValue& v2) {
...
template <> template <> SkColor ValueTraits<VectorValue>::As<SkColor>(const VectorValue& v) {
    // best effort to turn this into a color
    const auto r = v.size() > 0 ? v[0] : 0, g = v.size() > 1 ? v[1] : 0,
               b = v.size() > 2 ? v[2] : 0, a = v.size() > 3 ? v[3] : 1;
...
template <> template <> SkPoint ValueTraits<VectorValue>::As<SkPoint>(const VectorValue& vec) {
    // best effort to turn this into a point
    const auto x = vec.size() > 0 ? vec[0] : 0, y = vec.size() > 1 ? vec[1] : 0;
    return SkPoint::Make(x, y);
}

template <> template <> SkSize ValueTraits<VectorValue>::As<SkSize>(const VectorValue& vec) {
...
bool ParsePointVec(const skjson::Value& jv, std::vector<SkPoint>* pts) {
    if (!jv.is<skjson::ArrayValue>()) return false;
    ...
    for (size_t i = 0; i < av.size(); ++i) {
        if (!Parse(av[i], &vec) || vec.size() != 2) return false;
...
bool ValueTraits<ShapeValue>::FromJSON(const skjson::Value& jv,
    ...
    if (!jv.is<skjson::ObjectValue>()) return false;
...
template <> bool ValueTraits<ShapeValue>::CanLerp(const ShapeValue& v1, const ShapeValue& v2) {
    return v1.fVertices.size() == v2.fVertices.size() && v1.fClosed == v2.fClosed;
}

static SkPoint lerp_point(const SkPoint& v0, const SkPoint& v1, const Sk2f& t) {
    const auto v2f0 = Sk2f::Load(&v0), v2f1 = Sk2f::Load(&v1);
...
void ValueTraits<ShapeValue>::Lerp(const ShapeValue& v0, const ShapeValue& v1, float t,
    ...
    for (size_t i = 0; i < v0.fVertices.size(); ++i) {
        result->fVertices[i] =
                BezierVertex({lerp_point(v0.fVertices[i].fInPoint, v1.fVertices[i].fInPoint, t2f),
...
template <> template <> SkPath ValueTraits<ShapeValue>::As<SkPath>(const ShapeValue& shape) {
    SkPath path;
    ...
    const auto& addCubic = [&](size_t from, size_t to) {
        ...
        if (c0 == shape.fVertices[from].fVertex && c1 == shape.fVertices[to].fVertex) {
