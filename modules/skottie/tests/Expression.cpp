...
class FakePropertyObserver : public PropertyObserver {
 public:
  void onOpacityProperty(
      const char node_name[], const LazyHandle<OpacityPropertyHandle>& opacity_handle) override {
  ...
  void onColorProperty(
      const char node_name[], const LazyHandle<ColorPropertyHandle>& color_handle) override {
    color_handle_.reset(color_handle().release());
  }

  void onTextProperty(
      const char node_name[], const LazyHandle<TextPropertyHandle>& text_handle) override {
...
DEF_TEST(Skottie_Expression, r) {
  ...
  auto anim =
      Animation::Builder().setExpressionManager(em).setPropertyObserver(observer).make(&stream);
  ...
  REPORTER_ASSERT(
      r, (observer->color_handle_->get() == SkColor4f{0.1f, 0.2f, 0.3f, 1.0f}.toSkColor()));
}
