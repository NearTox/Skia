...
DEF_TEST(Skottie_AudioLayer, r) {
  ...
  class MockTracker final : public ExternalTrackAsset {
   ...
   private:
    void seek(float t) override { fCurrentTime = t; }
  ...
  class MockResourceProvider final : public ResourceProvider {
   ...
   private:
    sk_sp<ExternalTrackAsset> loadAudioAsset(
  ...
  auto skottie = skottie::Animation::Builder().setResourceProvider(rp).make(&stream);
