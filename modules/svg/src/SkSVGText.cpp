...
void SkSVGText::onRender(const SkSVGRenderContext& ctx) const {
  ...
  const auto text_align = [](const SkSVGTextAnchor& anchor) {
    switch (anchor.type()) {
      ...
      case SkSVGTextAnchor::Type::kInherit: SkASSERT(false); return SkTextUtils::Align::kLeft_Align;
    }
    SkUNREACHABLE;
  };
  ...
  if (const SkPaint* fillPaint = ctx.fillPaint()) {
    SkTextUtils::DrawString(
        ctx.canvas(), fText.c_str(), fX.value(), fY.value(), font, *fillPaint, align);
  }

  if (const SkPaint* strokePaint = ctx.strokePaint()) {
    SkTextUtils::DrawString(
        ctx.canvas(), fText.c_str(), fX.value(), fY.value(), font, *strokePaint, align);
  }
}
