...
static SkFont ResolveFont(const SkSVGRenderContext& ctx) {
  ...
  const SkFontStyle style(
      weight(*ctx.presentationContext().fInherited.fFontWeight), SkFontStyle::kNormal_Width,
      slant(*ctx.presentationContext().fInherited.fFontStyle));

  const auto size = ctx.lengthContext().resolve(
      ctx.presentationContext().fInherited.fFontSize->size(),
...
static std::vector<float> ResolveLengths(
    const SkSVGLengthContext& lctx, const std::vector<SkSVGLength>& lengths,
...
static float ComputeAlignmentFactor(const SkSVGPresentationContext& pctx) {
  switch (pctx.fInherited.fTextAnchor->type()) {
    ...
    case SkSVGTextAnchor::Type::kInherit: SkASSERT(false); return 0.0f;
  }
  SkUNREACHABLE;
}
...
SkSVGTextContext::ScopedPosResolver::ScopedPosResolver(
    const SkSVGTextContainer& txt, const SkSVGLengthContext& lctx, SkSVGTextContext* tctx,
...
SkSVGTextContext::ScopedPosResolver::~ScopedPosResolver() { fTextContext->fPosResolver = fParent; }
...
SkSVGTextContext::PosAttrs SkSVGTextContext::ScopedPosResolver::resolve(size_t charIndex) const {
  ...
  if (charIndex < fLastPosIndex) {
    ...
    const auto hasAllLocal = localCharIndex < fX.size() && localCharIndex < fY.size() &&
                             localCharIndex < fDx.size() && localCharIndex < fDy.size() &&
...
void SkSVGTextContext::shapePendingBuffer(const SkFont& font) {
  ...
  fShaper->shape(
      fShapeBuffer.fUtf8.data(), fShapeBuffer.fUtf8.size(), font, LTR, SK_ScalarMax, this);
  fShapeBuffer.reset();
}
  ...
  if (tpath) {
    ...
    auto resolve_offset = [this](const SkSVGLength& offset) {
      if (offset.unit() != SkSVGLength::Unit::kPercentage) {
        ...
        // system."
        return fRenderContext.lengthContext().resolve(
            offset, SkSVGLengthContext::LengthType::kHorizontal);
      }
...
SkSVGTextContext::~SkSVGTextContext() { this->flushChunk(fRenderContext); }

void SkSVGTextContext::appendFragment(
    const SkString& txt, const SkSVGRenderContext& ctx, SkSVGXmlSpace xs) {
  ...
  while (ch_ptr < ch_end) {
    ...
    ch = (xs == SkSVGXmlSpace::kDefault) ? filterWSDefault(ch) : filterWSPreserve(ch);
...
SkRSXform SkSVGTextContext::computeGlyphXform(
    SkGlyphID glyph, const SkFont& font, const SkPoint& glyph_pos,
...
void SkSVGTextContext::flushChunk(const SkSVGRenderContext& ctx) {
  ...
  for (const auto& run : fRuns) {
    ...
    for (size_t i = 0; i < run.glyphCount; ++i) {
      buf.xforms()[i] =
          this->computeGlyphXform(run.glyphs[i], run.font, run.glyphPos[i], run.glyhPosAdjust[i]);
    }
...
void SkSVGTextFragment::renderText(
    const SkSVGRenderContext& ctx, SkSVGTextContext* tctx, SkSVGXmlSpace xs) const {
...
void SkSVGTextContainer::onRenderText(
    const SkSVGRenderContext& ctx, SkSVGTextContext* tctx, SkSVGXmlSpace) const {
...
bool SkSVGTextContainer::parseAndSetAttribute(const char* name, const char* value) {
  return INHERITED::parseAndSetAttribute(name, value) ||
         ...
         this->setRotate(
             SkSVGAttributeParser::parse<std::vector<SkSVGNumberType>>("rotate", name, value)) ||
...
void SkSVGTextLiteral::onRenderText(
    const SkSVGRenderContext& ctx, SkSVGTextContext* tctx, SkSVGXmlSpace xs) const {
  SkASSERT(tctx);
...
void SkSVGText::onRenderText(
    const SkSVGRenderContext& ctx, SkSVGTextContext*, SkSVGXmlSpace xs) const {
...
void SkSVGTextPath::onRenderText(
    const SkSVGRenderContext& ctx, SkSVGTextContext*, SkSVGXmlSpace xs) const {
  // Root text nodes establish a new text layout context.
