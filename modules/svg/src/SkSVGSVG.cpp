...
static SkMatrix ViewboxMatrix(
    const SkRect& view_box, const SkRect& view_port, const SkSVGPreserveAspectRatio& par) {
  ...
  auto compute_scale = [&]() -> SkV2 {
    ...
    // isotropic scaling
    const auto s =
        par.fScale == SkSVGPreserveAspectRatio::kMeet ? std::min(sx, sy) : std::max(sx, sy);
    return {s, s};
  };
  ...
  auto compute_trans = [&](const SkV2& scale) -> SkV2 {
    ...
    const auto tx = -view_box.x() * scale.x, ty = -view_box.y() * scale.y,
...
void SkSVGSVG::setViewBox(const SkSVGViewBoxType& vb) { fViewBox.set(vb); }
