...
SkPath EMSCRIPTEN_KEEPALIVE NewPath() { return SkPath(); }
...
bool EMSCRIPTEN_KEEPALIVE Equals(const SkPath& a, const SkPath& b) { return a == b; }
...
bool EMSCRIPTEN_KEEPALIVE ApplySimplify(SkPath& path) { return Simplify(path, &path); }
...
SkPathOrNull EMSCRIPTEN_KEEPALIVE
MakeFromOp(const SkPath& pathOne, const SkPath& pathTwo, SkPathOp op) {
  SkPath out;
...
void EMSCRIPTEN_KEEPALIVE ToCanvas(const SkPath& path, emscripten::val /* Path2D or Canvas*/ ctx) {
  ...
  while ((verb = iter.next(pts)) != SkPath::kDone_Verb) {
    switch (verb) {
      case SkPath::kMove_Verb: ctx.call<void>("moveTo", pts[0].x(), pts[0].y()); break;
      case SkPath::kLine_Verb: ctx.call<void>("lineTo", pts[1].x(), pts[1].y()); break;
      ...
      case SkPath::kCubic_Verb:
        ctx.call<void>(
            "bezierCurveTo", pts[1].x(), pts[1].y(), pts[2].x(), pts[2].y(), pts[3].x(),
            pts[3].y());
        break;
      case SkPath::kClose_Verb: ctx.call<void>("closePath"); break;
...
void ApplyAddArc(
    SkPath& path, SkScalar x, SkScalar y, SkScalar radius, SkScalar startAngle, SkScalar endAngle,
    bool ccw) {
  SkPath temp;
...
void ApplyEllipse(
    SkPath& path, SkScalar x, SkScalar y, SkScalar radiusX, SkScalar radiusY, SkScalar rotation,
...
SkMatrix toSkMatrix(const SimpleMatrix& sm) {
  return SkMatrix::MakeAll(
      sm.scaleX, sm.skewX, sm.transX, sm.skewY, sm.scaleY, sm.transY, sm.pers0, sm.pers1, sm.pers2);
}

void ApplyTransform(SkPath& orig, const SimpleMatrix& sm) { orig.transform(toSkMatrix(sm)); }
...
float SkBits2FloatUnsigned(uint32_t floatAsBits) { return SkBits2Float((int32_t)floatAsBits); }

// Binds the classes to the JS
//
// See
// https://kripken.github.io/emscripten-site/docs/porting/connecting_cpp_and_javascript/embind.html#non-member-functions-on-the-javascript-prototype
...
EMSCRIPTEN_BINDINGS(skia) {
  class_<SkPath>("SkPath")
      ...
      .function(
          "_transform",
          select_overload<void(SkPath & orig, const SimpleMatrix& sm)>(&ApplyTransform))
      .function(
          "_transform", select_overload<void(
                            SkPath & orig, SkScalar, SkScalar, SkScalar, SkScalar, SkScalar,
                            SkScalar, SkScalar, SkScalar, SkScalar)>(&ApplyTransform))
  ...
  value_array<SkPoint>("SkPoint").element(&SkPoint::fX).element(&SkPoint::fY);
