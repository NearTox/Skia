...
void initGradientParams(
    Fuzz* fuzz, std::vector<SkColor>* colors, std::vector<SkScalar>* pos, SkTileMode* mode) {
  int count;
...
static void logLinearGradient(
    const SkPoint pts[2], const std::vector<SkColor>& colors, const std::vector<SkScalar> pos,
    SkTileMode mode, uint32_t flags, const SkMatrix* localMatrix, const SkMatrix* globalMatrix) {
  if (!FLAGS_verbose) {
    return;
  }
...
void fuzzLinearGradient(Fuzz* fuzz) {
  ...
  p.setShader(SkGradientShader::MakeLinear(
      pts, colors.data(), pos.data(), colors.size(), mode, flags, localMatrix.getMaybeNull()));
...
void fuzzRadialGradient(Fuzz* fuzz) {
  ...
  p.setShader(SkGradientShader::MakeRadial(
      center, radius, colors.data(), pos.data(), colors.size(), mode, flags,
...
void fuzzTwoPointConicalGradient(Fuzz* fuzz) {
  ...
  p.setShader(SkGradientShader::MakeTwoPointConical(
      start, startRadius, end, endRadius, colors.data(), pos.data(), colors.size(), mode, flags,
      localMatrix.getMaybeNull()));
...
void fuzzSweepGradient(Fuzz* fuzz) {
  ...
  if (useLocalMatrix) {
    ...
    p.setShader(
        SkGradientShader::MakeSweep(cx, cy, colors.data(), pos.data(), colors.size(), flags, &m));
  } else {
    p.setShader(SkGradientShader::MakeSweep(cx, cy, colors.data(), pos.data(), colors.size()));
  }
