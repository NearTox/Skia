...
static SkImageInfo gen_fuzzed_imageinfo(Fuzz* fuzz, SkColorType surfaceType) {
  ...
  return SkImageInfo::Make(
      width, height, surfaceType, alphaType, SkColorSpace::MakeRGB(skcmsFn, skcmsMat));
}

static SkSurfaceCharacterization make_characterization(
    Fuzz* fuzz, GrDirectContext* dContext, SkImageInfo& ii, SkColorType surfaceType,
  ...
  GrBackendFormat backendFormat = dContext->defaultBackendFormat(surfaceType, GrRenderable::kYes);
  if (!backendFormat.isValid()) {
    SkDebugf(
        "Color Type is not supported in the backend %s", GrBackendApiToStr(dContext->backend()));
    return {};
  }
  ...
  c = dContext->threadSafeProxy()->createCharacterization(
      maxResourceBytes, ii, backendFormat, kSampleCount, origin, gen_fuzzed_surface_props(fuzz),
      true, false, true, protect);
  if (!c.isValid()) {
    SkDebugf(
...
static sk_sp<SkDeferredDisplayList> make_ddl(
    Fuzz* fuzz, GrDirectContext* dContext, const SkSurfaceCharacterization& c) {
...
static sk_sp<SkSurface> make_surface(
    Fuzz* fuzz, GrDirectContext* dContext, const SkImageInfo& ii, GrSurfaceOrigin origin) {
  SkBudgeted budgeted;
  fuzz->nextEnum(&budgeted, SkBudgeted::kYes);
  SkSurfaceProps surfaceProps = gen_fuzzed_surface_props(fuzz);
  auto surface =
      SkSurface::MakeRenderTarget(dContext, budgeted, ii, kSampleCount, origin, &surfaceProps);
  return surface;
}
...
static SurfaceAndChar create_surface_and_characterization(
    Fuzz* fuzz, GrDirectContext* dContext, SkColorType surfaceType, GrSurfaceOrigin origin) {
