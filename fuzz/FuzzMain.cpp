...
static DEFINE_string2(
    dump, d, "",
    "If not empty, dump 'image*' or 'skp' types as a "
    "PNG with this name.");
  ...
  switch (mode) {
    case 0: {  // kCodecZeroInit_Mode, kCodec_Mode
      switch (codec->getPixels(decodeInfo, bitmap.getPixels(), bitmap.rowBytes(), &options)) {
        case SkCodec::kSuccess: SkDebugf("[terminated] Success!\n"); break;
        case SkCodec::kIncompleteInput: SkDebugf("[terminated] Partial Success\n"); break;
        case SkCodec::kErrorInInput: SkDebugf("[terminated] Partial Success with error\n"); break;
        case SkCodec::kInvalidConversion:
          SkDebugf("Incompatible colortype conversion\n");
          // Crash to allow afl-fuzz to know this was a bug.
          raise(SIGSEGV);
          break;
        default: SkDebugf("[terminated] Couldn't getPixels.\n"); return;
      }
      break;
    }
    ...
    case 3: {  // kSubset_Mode
      ...
      for (int x = 0; x < W; x += w) {
        for (int y = 0; y < H; y += h) {
          ...
          const SkCodec::Result result = codec->getPixels(decodeInfo, pixels, rowBytes, &opts);
          switch (result) {
            case SkCodec::kSuccess:
            case SkCodec::kIncompleteInput:
            case SkCodec::kErrorInInput: SkDebugf("okay\n"); break;
            case SkCodec::kInvalidConversion:
              if (0 == (x | y)) {
            ...
            default:
              SkDebugf(
                  ...
                  x, y, decodeInfo.width(), decodeInfo.height(), W, H, result);
              return;
          }
        }
      }
    ...
    case 4: {  // kAnimated_Mode
      ...
      for (size_t i = 0; i < frameInfos.size(); i++) {
        ...
        auto result = codec->startIncrementalDecode(
            decodeInfo, bitmap.getPixels(), bitmap.rowBytes(), &options);
    ...
    default: SkDebugf("[terminated] Mode not implemented yet\n");
  }
