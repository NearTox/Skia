...
void Path2DBuilder::AddToGlobal(Global* global) {
  ...
  context->Global()->Set(
      v8::String::NewFromUtf8(gGlobal->getIsolate(), "Path2DBuilder"), constructor->GetFunction());
}

Path2DBuilder* Path2DBuilder::Unwrap(const v8::FunctionCallbackInfo<v8::Value>& args) {
  v8::Handle<v8::External> field = v8::Handle<v8::External>::Cast(args.This()->GetInternalField(0));
...
void Path2DBuilder::QuadraticCurveTo(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  // the HTML 5 spec.
  path->fSkPath.quadTo(
      SkDoubleToScalar(cpx), SkDoubleToScalar(cpy), SkDoubleToScalar(x), SkDoubleToScalar(y));
}
...
void Path2DBuilder::BezierCurveTo(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  path->fSkPath.cubicTo(
      SkDoubleToScalar(cp1x), SkDoubleToScalar(cp1y), SkDoubleToScalar(cp2x),
...
void Path2DBuilder::ConicTo(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  path->fSkPath.conicTo(
      SkDoubleToScalar(x1), SkDoubleToScalar(y1), SkDoubleToScalar(x2), SkDoubleToScalar(y2),
      SkDoubleToScalar(w));
}
