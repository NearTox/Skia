...
void DrawingMethods::Rotate(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  if (args.Length() != 1) {
    args.GetIsolate()->ThrowException(
        v8::String::NewFromUtf8(args.GetIsolate(), "Error: 1 arguments required."));
    return;
  }
...
void DrawingMethods::DrawPath(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  if (args.Length() != 1) {
    args.GetIsolate()->ThrowException(
        v8::String::NewFromUtf8(args.GetIsolate(), "Error: 1 argument required."));
    return;
  }

  v8::Handle<v8::External> field =
      v8::Handle<v8::External>::Cast(args[0]->ToObject()->GetInternalField(0));
  void* ptr = field->Value();
...
void DrawingMethods::GetWidth(
  ...
  info.GetReturnValue().Set(
      v8::Int32::New(drawingMethods->fGlobal->getIsolate(), canvas->imageInfo().width()));
}

void DrawingMethods::GetHeight(
    v8::Local<v8::String> name, const v8::PropertyCallbackInfo<v8::Value>& info) {
  DrawingMethods* drawingMethods = Unwrap(info.This());
  SkCanvas* canvas = drawingMethods->getCanvas();
  if (NULL == canvas) {
  ...
  info.GetReturnValue().Set(
      v8::Int32::New(drawingMethods->fGlobal->getIsolate(), canvas->imageInfo().height()));
}

#define ADD_METHOD(name, fn)                                                                 \
  tmpl->Set(                                                                                 \
      v8::String::NewFromUtf8(fGlobal->getIsolate(), name, v8::String::kInternalizedString), \
...
void DrawingMethods::addAttributesAndMethods(v8::Handle<v8::ObjectTemplate> tmpl) {
  ...
  tmpl->SetAccessor(
      v8::String::NewFromUtf8(fGlobal->getIsolate(), "width", v8::String::kInternalizedString),
      GetWidth);
  tmpl->SetAccessor(
      v8::String::NewFromUtf8(fGlobal->getIsolate(), "height", v8::String::kInternalizedString),
      GetHeight);
