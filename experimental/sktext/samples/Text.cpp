...
class TextSample_Align_Dir : public Sample {
 ...
 protected:
  ...
  void drawLine(
    ...
    Paint::drawText(
        direction == TextDirection::kRtl ? mirror(text) : normal(text), canvas, direction, align,
        foregroundPaint, backgroundPaint, SkString("Roboto"), 12.0f, SkFontStyle::Normal(), 0, 0);
  }
...
class TextSample_LongLTR : public Sample {
 protected:
  ...
  void onDrawContent(SkCanvas* canvas) override {
    canvas->drawColor(SK_ColorWHITE);
    Paint::drawText(
        u"A very_very_very_very_very_very_very_very_very_very "
