...
class Visitor {
 public:
  ...
  virtual void onGlyphRun(
      const SkFont& font,
      TextRange
          textRange,  // Currently we make sure that the run edges are the grapheme cluster edges
      SkRect bounds,  // bounds contains the physical boundaries of the run
      int trailingSpaces,  // Depending of TextDirection it goes right to the end (LTR) or left to
                           // the start (RTL)
...
class WrappedText {
  ...
  */
  std::unique_ptr<DrawableText> prepareToDraw(
  ...
  */
  ...
  /** Breaks the text runs into smaller runs by given list of chunks to be used for styling.
      ...
      @param chunks         a range of text indices that cause an additional run breaking to be used
     for styling
  */
