...
class Range {
 public:
  ...
  bool leftToRight() const { return fEnd >= fStart; }
  ...
  int width() const { return leftToRight() ? SkToInt(fEnd - fStart) : SkToInt(fStart - fEnd); }
...
struct FontBlock {
  FontBlock(uint32_t count, sk_sp<FontChain> fontChain)
      : type(BlockType::kFont), charCount(count), chain(fontChain) {}
