...
enum class LogicalRunType { kText, kLineBreak };
...
class Range {
 public:
  ...
  bool operator==(Range<T> other) { return fStart == other.fStart && fEnd == other.fEnd; }
  ...
  bool isEmpty() const { return fEnd == fStart; }

  bool contains(T index) const { return index >= fStart && index < fEnd; }

  bool contains(Range<T> range) const { return range.fStart >= fStart && range.fEnd < fEnd; }

  // For RTL ranges start >= end
  int width() const { return SkToInt(fEnd - fStart); }
...
struct DirTextRange : public TextRange {
  DirTextRange(TextRange textRange, bool leftToRight)
      : TextRange(textRange), fLeftToRight(leftToRight) {}
  DirTextRange(TextIndex start, TextIndex end, bool leftToRight)
      : TextRange(start, end), fLeftToRight(leftToRight) {}

  bool before(TextIndex index) const { return fLeftToRight ? index >= fEnd : index < fStart; }
  bool after(TextIndex index) const { return fLeftToRight ? index < fEnd : index >= fStart; }
  bool left() const { return fLeftToRight ? fStart : fEnd; }
  bool right() const { return fLeftToRight ? fEnd : fStart; }
...
struct FontBlock {
  FontBlock(uint32_t count, sk_sp<FontChain> fontChain)
      : type(BlockType::kFontChain), charCount(count), chain(fontChain) {}
...
struct ResolvedFontBlock {
  ResolvedFontBlock(
      TextRange textRange, sk_sp<SkTypeface> typeface, SkScalar size, SkFontStyle fontStyle)
      : textRange(textRange), typeface(typeface), size(size), style(fontStyle) {}
...
namespace sknonstd {
template <>
struct is_bitmask_enum<skia::text::CodeUnitFlags> : std::true_type {};
template <>
struct is_bitmask_enum<skia::text::GlyphUnitFlags> : std::true_type {};
}  // namespace sknonstd
