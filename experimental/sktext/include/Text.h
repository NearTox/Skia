...
class UnicodeText {
  ...
  */
  ...
  SkSpan<const char16_t> getText16() const {
    return SkSpan<const char16_t>(fText16.data(), fText16.size());
  }
...
class FontResolvedText {
  ...
  */
  ...
  SkSpan<ResolvedFontBlock> resolvedFonts() {
    return SkSpan<ResolvedFontBlock>(fResolvedFonts.data(), fResolvedFonts.size());
  }
...
class ShapedText : public SkShaper::RunHandler {
 public:
  /** Break text by lines with a given width (and possible new lines).
      ...
      @return               an object that contains the result of shaping operations (wrapping and
     formatting).
  */
  std::unique_ptr<WrappedText> wrap(UnicodeText* unicodeText, float width, float height);

  ShapedText() : fCurrentRun(nullptr), fParagraphTextStart(0), fRunGlyphStart(0.0f) {}
  ...
  SkSpan<const LogicalRun> getLogicalRuns() const {
    return SkSpan<const LogicalRun>(fLogicalRuns.begin(), fLogicalRuns.size());
  }
...
class Visitor {
 public:
  ...
  virtual void onGlyphRun(
      const SkFont& font,
      DirTextRange
          dirTextRange,  // Currently we make sure that the run edges are the grapheme cluster edges
      SkRect bounds,     // bounds contains the physical boundaries of the run
      TextIndex trailingSpaces,    // Depending of TextDirection it goes right to the end (LTR) or
                                   // left to the start (RTL)
...
class WrappedText {
 public:
  /** Builds a list of SkTextBlobs to draw on a canvas.
      @param positionType   specifies a text adjustment granularity (grapheme cluster, grapheme,
     glypheme, glyph)
      @param blocks         a range of text indices that cause an additional run breaking to be used
     for styling
      @return               an object that contains a list of SkTextBlobs to draw on a canvas
  */
  template <class Drawable>
  std::unique_ptr<Drawable> prepareToDraw(
  ...
  */
  ...
  /** Walks though the data structures and makes certain callbacks on visitor so the visitor can
  ...
  */
  ...
  /** Walks though the data structures and makes certain callbacks on visitor so the visitor can
     collect all the information.
      ...
      @param visitor      a reference to Visitor object
      @param positionType specifies a text adjustment granularity (grapheme cluster, grapheme,
     glypheme, glyph) to map text blocks to glyph ranges.
      @param chunks       a range of widths that cause an additional run breaking to be used for
     styling
  */
  void visit(
...
struct Position {
  Position(
      PositionType positionType, size_t lineIndex, GlyphRange glyphRange, TextRange textRange,
      SkRect rect)
...
class SelectableText : public Visitor {
  ...
  */
  ...
  bool isFirstOnTheLine(Position element) const { return (element.fGlyphRange.fStart == 0); }
  ...
  void onEndLine(
      size_t index, TextRange lineText, GlyphRange trailingSpaces, size_t glyphCount) override;
 ...
 private:
  ...
  // Just in theory a random glyph range can be represented by multiple text ranges (because of
  // LTR/RTL) Currently we only support this method for a glyph, grapheme or grapheme cluster So
  // it's guaranteed to be one text range
