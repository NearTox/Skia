...
Editor::Editor(std::u16string text, SkSize size)
    : fDefaultPositionType(PositionType::kGraphemeCluster), fInsertMode(true) {
  fParent = nullptr;
  fCursor = Cursor::Make();
  fMouse = std::make_unique<Mouse>();
  {
    SkPaint foreground;
    foreground.setColor(DEFAULT_TEXT_FOREGROUND);
    SkPaint background;
    background.setColor(DEFAULT_TEXT_BACKGROUND);
    static FontBlock textBlock(
        text.size(), sk_make_sp<TrivialFontChain>("Roboto", 40, SkFontStyle::Normal()));
    ...
    fEditableText = std::make_unique<EditableText>(
        text, SkPoint::Make(0, 0), textSize, SkSpan<FontBlock>(&textBlock, 1),
        SkSpan<DecoratedBlock>(&textDecor, 1), DEFAULT_TEXT_DIRECTION, DEFAULT_TEXT_ALIGN);
  }
  {
    SkPaint foreground;
    foreground.setColor(DEFAULT_STATUS_FOREGROUND);
    SkPaint background;
    background.setColor(DEFAULT_STATUS_BACKGROUND);
    std::u16string status = u"This is the status line";
    static FontBlock statusBlock(
        status.size(), sk_make_sp<TrivialFontChain>("Roboto", 20, SkFontStyle::Normal()));
...
void Editor::update() {
  ...
  auto position =
      fEditableText->adjustedPosition(fDefaultPositionType, fCursor->getCenterPosition());
...
// We don't want to move by the input text indexes because then we will have to take in account
// LTR/RTL
...
bool Editor::deleteElement(skui::Key key) {
  ...
  // IMPORTANT: We assume that a single element (grapheme cluster) does not cross the run
  // boundaries; It's not exactly true but we are going to enforce in by breaking the grapheme by
  // the run boundaries
  ...
  position = fEditableText->adjustedPosition(
      fDefaultPositionType, SkPoint::Make(position.fBoundaries.fLeft, position.fBoundaries.fTop));
  fCursor->place(position.fBoundaries);
  this->invalidate();
...
bool Editor::onKey(skui::Key key, skui::InputState state, skui::ModifierKey modifiers) {
  ...
  if (!Any(ctrlAltCmd)) {
    ...
    switch (key) {
      ...
      case skui::Key::kDown:
      case skui::Key::kHome:
      case skui::Key::kEnd: this->moveCursor(key); break;
      case skui::Key::kDelete:
      case skui::Key::kBack: this->deleteElement(key); return true;
      case skui::Key::kOK: return this->onChar(0x000A, modifiers);
      default: break;
    }
  }
  return false;
...
void Editor::paint(SkCanvas* canvas) {
  ...
  background.setColor(DEFAULT_STATUS_BACKGROUND);
  canvas->drawRect(
      SkRect::MakeXYWH(0, fHeight - DEFAULT_STATUS_HEIGHT, fWidth, DEFAULT_STATUS_HEIGHT),
      background);
...
std::unique_ptr<Editor> Editor::MakeDemo(SkScalar width, SkScalar height) {
  std::u16string text0 =
      u"In a hole in the ground there lived a hobbit. Not a nasty, dirty, "
