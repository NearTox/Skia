...
class DynamicText {
 public:
  ...
  bool rebuild(std::u16string text) {
    ...
    this->fUnicodeText =
        Text::parse(SkSpan<uint16_t>((uint16_t*)this->fText.data(), this->fText.size()));
    this->fShapedText = fUnicodeText->shape(this->fFontBlocks, fTextDirection);
    this->fWrappedText = fShapedText->wrap(
        this->fRequiredSize.fWidth, this->fRequiredSize.fHeight, this->fUnicodeText->getUnicode());
...
class EditableText : public DynamicText {
 public:
  ...
  Position previousElement(Position element) const {
    return fFormattedText->previousElement(element);
  }
  ...
  Position firstElement(PositionType positionType) const {
    return fFormattedText->firstElement(positionType);
  }
  Position lastElement(PositionType positionType) const {
    return fFormattedText->lastElement(positionType);
  }

  bool isFirstOnTheLine(Position element) const {
    return fFormattedText->isFirstOnTheLine(element);
  }
