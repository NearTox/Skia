...
class StaticText {
 public:
  StaticText(
      std::u16string text, SkPoint offset, SkSize size, SkSpan<FontBlock> fontBlocks,
    ...
    fUnicodeText = std::make_unique<UnicodeText>(
        std::move(unicode), SkSpan<uint16_t>((uint16_t*)fText.data(), fText.size()));
...
class DynamicText {
 public:
  DynamicText(
      std::u16string text, SkPoint offset, SkSize size, SkSpan<FontBlock> fontBlocks,
      SkSpan<DecoratedBlock> decorations, TextDirection textDirection, TextAlign textAlign) {
    fOffset = offset;
    fRequiredSize = size;
    fFontBlocks = fontBlocks;
  ...
  bool contains(SkScalar x, SkScalar y) {
    return SkRect::MakeXYWH(fOffset.fX, fOffset.fY, fRequiredSize.fWidth, fRequiredSize.fHeight)
        .contains(x, y);
  }
  ...
  bool rebuild(std::u16string text) {
    ...
    fShapedText = fFontResolvedText->shape(fUnicodeText.get(), fTextDirection);
    fWrappedText = fShapedText->wrap(
        fUnicodeText.get(), this->fRequiredSize.fWidth, this->fRequiredSize.fHeight);
    fWrappedText->format(fTextAlign, fTextDirection);
...
class EditableText : public DynamicText {
 public:
  ...
  Position previousElement(Position element) const {
    return fSelectableText->previousPosition(element);
  }
  ...
  Position firstElement(PositionType positionType) const {
    return fSelectableText->firstPosition(positionType);
  }
  Position lastElement(PositionType positionType) const {
    return fSelectableText->lastPosition(positionType);
  }

  bool isFirstOnTheLine(Position element) const {
    return fSelectableText->isFirstOnTheLine(element);
  }
