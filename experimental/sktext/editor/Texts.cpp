...
void DynamicText::paint(SkCanvas* canvas) {
  if (!fDrawableText) {
    ...
    fDrawableText = fWrappedText->prepareToDraw<DrawableText>(
        fUnicodeText.get(), PositionType::kGraphemeCluster,
...
void EditableText::paint(SkCanvas* canvas) {
  if (fSelection->isEmpty()) {
    ...
    auto chunks =
        this->getDecorationChunks(SkSpan<DecoratedBlock>(decorations.data(), decorations.size()));
    fDrawableText = fWrappedText->prepareToDraw<DrawableText>(
        fUnicodeText.get(), PositionType::kGraphemeCluster,
        SkSpan<TextIndex>(chunks.data(), chunks.size()));
  }
...
SkTArray<DecoratedBlock> EditableText::mergeSelectionIntoDecorations() {
  ...
  for (auto& selected : fSelection->fTextRanges) {
    ...
    if (lastDecorPos + decor.charCount > selected.fEnd) {
      ...
      merged.emplace_back(
          lastDecorPos + decor.charCount - selected.fEnd, decor.foregroundPaint,
          decor.backgroundPaint);
