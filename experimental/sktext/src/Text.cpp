...
UnicodeText::UnicodeText(std::unique_ptr<SkUnicode> unicode, SkSpan<uint16_t> utf16)
    : fText16(std::u16string((char16_t*)utf16.data(), utf16.size())), fUnicode(std::move(unicode)) {
  initialize(utf16);
}
...
void UnicodeText::initialize(SkSpan<uint16_t> utf16) {
  ...
  this->fUnicode->forEachCodepoint(
      (char16_t*)utf16.data(), utf16.size(), [this](SkUnichar unichar, int32_t start, int32_t end) {
  ...
  // Get graphemes
  this->fUnicode->forEachBreak(
      (char16_t*)utf16.data(), utf16.size(), SkUnicode::BreakType::kGraphemes,
      [this](SkBreakIterator::Position pos, SkBreakIterator::Status) {
        fCodeUnitProperties[pos] |= CodeUnitFlags::kGraphemeStart;
      });
  // Get line breaks
  this->fUnicode->forEachBreak(
      (char16_t*)utf16.data(), utf16.size(), SkUnicode::BreakType::kLines,
...
std::unique_ptr<FontResolvedText> UnicodeText::resolveFonts(SkSpan<FontBlock> blocks) {
  ...
  for (auto& block : blocks) {
    ...
    while (adjustedBlock.fEnd < this->fText16.size() &&
           !this->hasProperty(adjustedBlock.fEnd, CodeUnitFlags::kGraphemeStart)) {
      ++adjustedBlock.fEnd;
    }
  ...
  std::sort(
      fontResolvedText->fResolvedFonts.begin(), fontResolvedText->fResolvedFonts.end(),
...
bool FontResolvedText::resolveChain(
    UnicodeText* unicodeText, TextRange textRange, const FontChain& fontChain) {
  ...
  for (auto fontIndex = 0; fontIndex < fontChain.count(); ++fontIndex) {
    ...
    while (!unresolvedTexts.empty()) {
      ...
      unicodeText->forEachGrapheme(unresolvedText, [&](TextRange grapheme) {
        auto count = typeface->textToGlyphs(
            unicodeText->getText16().data() + grapheme.fStart, grapheme.width() * 2,
            SkTextEncoding::kUTF16, nullptr, 0);
        SkAutoTArray<SkGlyphID> glyphs(count);
        typeface->textToGlyphs(
            unicodeText->getText16().data() + grapheme.fStart, grapheme.width() * 2,
            SkTextEncoding::kUTF16, glyphs.data(), count);
      ...
      for (auto newUnresolvedText : newUnresolvedTexts) {
        ...
        if (resolvedText.width() > 0) {
          ...
          fResolvedFonts.emplace_back(
              resolvedText, typeface, fontChain.fontSize(), SkFontStyle::Normal());
        }
...
class FormattingFontIterator final : public SkShaper::FontRunIterator {
 public:
  FormattingFontIterator(
      TextIndex textCount, SkSpan<ResolvedFontBlock> fontBlocks, SkSpan<TextIndex> marks)
...
std::unique_ptr<ShapedText> FontResolvedText::shape(
    UnicodeText* unicodeText, TextDirection textDirection) {
  ...
  unicodeText->getUnicode()->forEachCodepoint(
  ...
  // Shape the text
  FormattingFontIterator fontIter(
      text8.size(), SkSpan<ResolvedFontBlock>(fontBlocks8.data(), fontBlocks8.size()),
  ...
  std::unique_ptr<SkShaper::BiDiRunIterator> bidiIter(SkShaper::MakeSkUnicodeBidiRunIterator(
      unicodeText->getUnicode(), text8.c_str(), text8.size(),
      textDirection == TextDirection::kLtr ? 0 : 1));
  std::unique_ptr<SkShaper::ScriptRunIterator> scriptIter(
      SkShaper::MakeSkUnicodeHbScriptRunIterator(
          unicodeText->getUnicode(), text8.c_str(), text8.size()));
  ...
  shaper->shape(
      text8.c_str(), text8.size(), fontIter, *bidiIter, *scriptIter, langIter,
  ...
  if (shapedText->fLogicalRuns.empty()) {
    ...
    SkShaper::RunHandler::RunInfo emptyInfo{
        fontIter.createFont(fResolvedFonts.front()), 0, SkVector::Make(0.0f, 0.0f), 0,
  ...
  for (auto& logicalRun : shapedText->fLogicalRuns) {
    ...
    logicalRun.convertUtf16Range([&](unsigned long index8) { return UTF16FromUTF8[index8]; });
    // Convert all utf8 indexes into utf16 indexes (and also shift them to be on the entire text
    // scale, too)
    logicalRun.convertClusterIndexes(
        [&](TextIndex clusterIndex8) { return UTF16FromUTF8[clusterIndex8]; });
    // Detect and mark line break runs
    if (logicalRun.getTextRange().width() == 1 && logicalRun.size() == 1 &&
...
std::unique_ptr<WrappedText> ShapedText::wrap(UnicodeText* unicodeText, float width, float height) {
  ...
  for (size_t runIndex = 0; runIndex < this->fLogicalRuns.size(); ++runIndex) {
    ...
    for (size_t glyphIndex = 0; glyphIndex < run.fPositions.size(); ++glyphIndex) {
      ...
      cluster = Stretch(
          runIndex, clusterGlyphs, clusterText.normalized(), run.calculateWidth(clusterGlyphs),
          runMetrics);
...
SkTArray<int32_t> ShapedText::getVisualOrder(
    SkUnicode* unicode, RunIndex startRun, RunIndex endRun) {
...
void ShapedText::addLine(
    WrappedText* wrappedText, SkUnicode* unicode, Stretch& stretch, Stretch& spaces,
    bool hardLineBreak) {
  ...
  for (auto visualIndex : visualOrder) {
    ...
    if (glyphSpaces > glyphStart) {
      ...
      wrappedText->fVisualRuns.emplace_back(
          TextRange(textStart, textEnd), glyphSpaces - glyphStart, logicalRun.fFont,
  ...
  auto runRange =
      wrappedText->fVisualRuns.size() == runStart
          ? SkSpan<VisualRun>(nullptr, 0)
          : SkSpan<VisualRun>(
                &wrappedText->fVisualRuns[runStart], wrappedText->fVisualRuns.size() - runStart);
  wrappedText->fVisualLines.emplace_back(
      lineStretch.textRange(), hardLineBreak, wrappedText->fActualSize.fHeight, runRange);
...
void WrappedText::visit(Visitor* visitor) const {
  ...
  for (auto& line : fVisualLines) {
    visitor->onBeginLine(
        lineIndex, line.text(), line.isHardBreak(),
        SkRect::MakeXYWH(0, verticalOffset, line.fActualWidth, line.fTextMetrics.height()));
    ...
    for (auto& run : line.fRuns) {
      ...
      SkRect boundingRect = SkRect::MakeXYWH(
          line.fOffset.fX + run.fPositions[0].fX, line.fOffset.fY + diff, run.width(),
          run.fTextMetrics.height());
      visitor->onGlyphRun(
          run.fFont, run.dirTextRange(), boundingRect, run.trailingSpacesStart(), run.size(),
          run.fGlyphs.data(), run.fPositions.data(), run.fClusters.data());
      glyphCount += run.size();
    }
...
void WrappedText::visit(
    UnicodeText* unicodeText, Visitor* visitor, PositionType positionType,
    SkSpan<size_t> chunks) const {
  ...
  // The edges of the decor blocks don't have to match glyph, grapheme or even unicode code point
  // edges It's out responsibility to adjust them to some reasonable values [a:b) -> [c:d) where c
  // is closest GG cluster edge to a from the left and d is closest GG cluster edge to b from the
  // left
  ...
  for (auto& line : fVisualLines) {
    ...
    auto lineBlocks = WrappedText::limitBlocks(
        line.fText, SkSpan<TextIndex>(textBlocks.data(), textBlocks.size()));
    for (auto& run : fVisualRuns) {
      run.forEachTextBlockInGlyphRange(lineBlocks, [&](DirTextRange dirTextRange) {
        GlyphRange glyphRange =
            this->textToGlyphs(unicodeText, positionType, runIndex, dirTextRange);
        auto diff = line.fTextMetrics.above() - run.fTextMetrics.above();
        SkRect boundingRect = SkRect::MakeXYWH(
            line.fOffset.fX + run.fPositions[glyphRange.fStart].fX, line.fOffset.fY + diff,
            run.calculateWidth(glyphRange), run.fTextMetrics.height());
        visitor->onGlyphRun(
            run.fFont, dirTextRange, boundingRect, run.trailingSpacesStart(), glyphRange.width(),
            &run.fGlyphs[glyphRange.fStart], &run.fPositions[glyphRange.fStart],
...
GlyphRange WrappedText::textToGlyphs(
    UnicodeText* unicodeText, PositionType positionType, RunIndex runIndex,
    DirTextRange dirTextRange) const {
  ...
  for (GlyphIndex glyph = 0; glyph < run.size(); ++glyph) {
    ...
    if (positionType == PositionType::kGraphemeCluster &&
        unicodeText->hasProperty(textIndex, CodeUnitFlags::kGraphemeStart)) {
...
std::unique_ptr<SelectableText> WrappedText::prepareToEdit(UnicodeText* unicodeText) const {
  ...
  selectableText->fGlyphUnitProperties.push_back_n(
      unicodeText->getText16().size() + 1, GlyphUnitFlags::kNoGlyphUnitFlag);
...
void SelectableText::onEndLine(
    size_t index, TextRange lineText, GlyphRange trailingSpaces, size_t glyphCount) {
  ...
  line.fBoxGlyphs.emplace_back(
      SkRect::MakeXYWH(line.fBounds.fRight, line.fBounds.fTop, 0.0f, line.fBounds.height()));
...
void SelectableText::onGlyphRun(
  ...
  for (auto i = 0; i < glyphCount; ++i) {
    ...
    line.fBoxGlyphs[start + i] =
        SkRect::MakeXYWH(pos.fX, bounds.fTop, pos1.fX - pos.fX, bounds.height());
    line.fTextByGlyph[start + i] = clusters[i];
  }
}

// TODO: Do something (logN) that is not a linear search
Position SelectableText::findPosition(
    PositionType positionType, const BoxLine& line, SkScalar x) const {
...
Position SelectableText::previousPosition(Position current) const {
  ...
  auto position = this->findPosition(
      current.fPositionType, *currentLine,
      currentLine->fBoxGlyphs[current.fGlyphRange.fStart].centerX());
...
Position SelectableText::nextPosition(Position current) const {
  ...
  if (this->isLastOnTheLine(current)) {
    ...
    if (current.fLineIndex == this->fBoxLines.size() - 1) {
      // We reached the end; there is nowhere to move
      current.fGlyphRange =
          GlyphRange(currentLine->fBoxGlyphs.size(), currentLine->fBoxGlyphs.size());
      return current;
    } else {
      current.fLineIndex += 1;
      currentLine = &fBoxLines[current.fLineIndex];
...
Position SelectableText::upPosition(Position current) const {
  ...
  const BoxLine* currentLine = &fBoxLines[current.fLineIndex];
  auto position = this->findPosition(
      current.fPositionType, fBoxLines[current.fLineIndex - 1],
      currentLine->fBoxGlyphs[current.fGlyphRange.fStart].centerX());
  if (current.fPositionType == PositionType::kGraphemeCluster) {
    // Either way we found us a grapheme cluster (just make sure of it)
    SkASSERT(this->hasProperty(current.fTextRange.fEnd, GlyphUnitFlags::kGraphemeClusterStart));
...
Position SelectableText::firstPosition(PositionType positionType) const {
  ...
  beginningOfText.fBoundaries =
      SkRect::MakeXYWH(firstGlyph.fLeft, firstGlyph.fTop, 0, firstGlyph.height());
...
Position SelectableText::lastPosition(PositionType positionType) const {
  ...
  endOfText.fGlyphRange =
      GlyphRange(lastLine.fBoxGlyphs.size() - 1, lastLine.fBoxGlyphs.size() - 1);
