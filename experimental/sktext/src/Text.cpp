...
std::unique_ptr<UnicodeText> Text::parse(SkSpan<uint16_t> utf16) {
  ...
  unicodeText->fUnicode->forEachCodepoint(
      unicodeText->fText8.c_str(), unicodeText->fText8.size(),
  ...
  // Get graphemes
  unicodeText->fUnicode->forEachBreak(
...
std::unique_ptr<ShapedText> UnicodeText::shape(
    SkSpan<FontBlock> fontBlocks, TextDirection textDirection) {
  ...
  fShapedText->fGlyphUnitProperties.push_back_n(
      this->fCodeUnitProperties.size(), GlyphUnitFlags::kNoGlyphUnitFlag);
  ...
  FormattingFontIterator fontIter(
      fText8.size(), fontBlocks, SkSpan<TextIndex>(&formattingMarks[0], 1));
  ...
  std::unique_ptr<SkShaper::BiDiRunIterator> bidiIter(SkShaper::MakeSkUnicodeBidiRunIterator(
      this->fUnicode.get(), fText8.c_str(), fText8.size(),
  ...
  shaper->shape(
      fText8.c_str(), fText8.size(), fontIter, *bidiIter, *scriptIter, langIter,
  ...
  if (fShapedText->fRuns.empty()) {
    ...
    SkShaper::RunHandler::RunInfo emptyInfo{
        this->createFont(fontBlocks.front()), 0, SkVector::Make(0.0f, 0.0f), 0, Range(0, 0)};
...
void UnicodeText::commitRunBuffer(const RunInfo&) {
  ...
  fCurrentRun->convertUtf16Range(
      [this](unsigned long index) { return this->fUTF16FromUTF8[index + fParagraphTextStart]; });
...
std::unique_ptr<WrappedText> ShapedText::wrap(
    SkScalar width, SkScalar heightCurrentlyIgnored, SkUnicode* unicode) {
  ...
  for (size_t runIndex = 0; runIndex < this->fRuns.size(); ++runIndex) {
    ...
    for (size_t glyphIndex = 0; glyphIndex < run.fPositions.size(); ++glyphIndex) {
      ...
      auto isEndOfText = run.leftToRight() ? textIndex == run.fUtf16Range.fEnd
                                           : textIndex == run.fUtf16Range.fStart;
...
void WrappedText::addLine(
    Stretch& stretch, Stretch& spaces, SkUnicode* unicode, bool hardLineBreak) {
  ...
  this->fLines.emplace_back(
      stretch, spaces, std::move(visualOrder), fActualSize.fHeight, hardLineBreak);
  ...
  for (auto& line : this->fLines) {
    ...
    for (auto index = 0; index < line.runsNumber(); ++index) {
      ...
      GlyphRange glyphRange(
          line.glyphRange(runIndex, run.size(), false /* excluding trailing spaces */));
      // Update positions
      ...
      SkRect boundingRect = SkRect::MakeXYWH(
          shift.fX + offset.fX, offset.fY, run.fPositions[glyphRange.fEnd].fX,
          run.fTextMetrics.height());
      visitor->onGlyphRun(
          run.fFont, run.getTextRange(glyphRange), boundingRect, glyphRange.width(),
          run.fGlyphs.data() + glyphRange.fStart, positions.data(),
          run.fOffsets.data() + glyphRange.fStart);
...
void FormattedText::visit(Visitor* visitor, SkSpan<TextIndex> textChunks) const {
  ...
  for (auto& line : this->fLines) {
    ...
    for (auto index = 0; index < line.runsNumber(); ++index) {
      ...
      // chunks[text index] -> chunks [glyph index] -> walk through in glyph index order (the visual
      // order)
      run.forEachTextChunkInGlyphRange(
          textChunks, runGlyphRange, [&](TextRange textRange, GlyphRange glyphRange) {
            // Update positions & calculate the bounding rect
            SkAutoSTMalloc<256, SkPoint> positions(glyphRange.width() + 1);
            for (GlyphIndex i = glyphRange.fStart; i <= glyphRange.fEnd; ++i) {
              positions[i - glyphRange.fStart] =
                  run.fPositions[i] + shift + offset + SkPoint::Make(line.horizontalOffset(), 0.0f);
            }
            SkRect boundingRect = SkRect::MakeXYWH(
                positions[0].fX + line.horizontalOffset(), offset.fY,
                positions[glyphRange.width()].fX - positions[0].fX, run.fTextMetrics.height());
            visitor->onGlyphRun(
                run.fFont, run.getTextRange(glyphRange), boundingRect, glyphRange.width(),
                run.fGlyphs.data() + glyphRange.fStart, positions.data(),
                run.fOffsets.data() + glyphRange.fStart);
...
Position FormattedText::adjustedPosition(PositionType positionType, TextIndex textIndex) const {
  ...
  for (auto& line : fLines) {
    ...
    for (auto index = 0; index < line.runsNumber(); ++index) {
      ...
      GlyphRange runGlyphRange(
          line.glyphRange(runIndex, run.size(), true /* including trailing spaces */));
      ...
      position.fBoundaries.fLeft +=
          shift + run.calculateWidth(runGlyphRange.fStart, position.fGlyphRange.fStart);
      position.fBoundaries.fRight +=
          shift + run.calculateWidth(runGlyphRange.fStart, position.fGlyphRange.fEnd);
      return position;
    }
...
Position FormattedText::adjustedPosition(PositionType positionType, SkPoint xy) const {
  ...
  for (auto& line : fLines) {
    ...
    for (auto index = 0; index < line.runsNumber(); ++index) {
      auto runIndex = line.visualRun(index);
      auto& run = fRuns[runIndex];
      GlyphRange runGlyphRangeInLine =
          line.glyphRange(runIndex, run.size(), true /* including trailing space */);
      ...
      position.fGlyphRange =
          GlyphRange(found, found == runGlyphRangeInLine.fEnd ? found : found + 1);
      position.fTextRange = position.fRun->getTextRange(position.fGlyphRange);
      this->adjustTextRange(&position);
      position.fBoundaries.fLeft =
          runOffsetInLine +
          run.calculateWidth(runGlyphRangeInLine.fStart, position.fGlyphRange.fStart);
      position.fBoundaries.fRight =
          runOffsetInLine +
          run.calculateWidth(runGlyphRangeInLine.fStart, position.fGlyphRange.fEnd);
      return position;
    }
    // The cursor is not on the text anymore; position it after the last element of the last visual
    // run of the current line
    break;
  }

  position.fRun = this->visuallyLastRun(position.fLineIndex);
  auto line = this->line(position.fLineIndex);
  position.fGlyphRange.fStart = position.fGlyphRange.fEnd =
      line->glyphRange(
              this->runIndex(position.fRun), position.fRun->size(),
              true /* including trailing spaces */)
          .fEnd;
  position.fTextRange = position.fRun->getTextRange(position.fGlyphRange);
  this->adjustTextRange(&position);
  position.fBoundaries.fLeft = position.fBoundaries.fRight = line->withWithTrailingSpaces();
  return position;
}
...
void FormattedText::adjustTextRange(Position* position) const {
  ...
  position->fTextRange = TextRange(
      position->fRun->fClusters[position->fGlyphRange.fStart],
      position->fRun->fClusters[position->fGlyphRange.fEnd]);
}
...
bool FormattedText::recalculateBoundaries(Position& position) const {
  ...
  GlyphIndex start =
      runIndex == line->glyphStart().runIndex() ? line->glyphStart().glyphIndex() : 0;
  GlyphIndex end = runIndex == line->glyphTrailingEnd().runIndex()
                       ? line->glyphTrailingEnd().glyphIndex()
                       : position.fRun->fGlyphs.size();
  ...
  position.fBoundaries =
      SkRect::MakeXYWH(left, line->verticalOffset(), width, line->getMetrics().height());
  return true;
}
...
Position FormattedText::previousElement(Position element) const {
  ...
  element.fTextRange =
      TextRange(clusters[element.fGlyphRange.fStart], clusters[element.fGlyphRange.fStart]);
...
Position FormattedText::nextElement(Position element) const {
  ...
  element.fTextRange =
      TextRange(clusters[element.fGlyphRange.fEnd], clusters[element.fGlyphRange.fEnd]);
