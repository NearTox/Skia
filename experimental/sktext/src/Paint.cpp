...
bool Paint::drawText(std::u16string text, SkCanvas* canvas, SkScalar x, SkScalar y) {
  ...
  return drawText(
      std::move(text), canvas, TextDirection::kLtr, TextAlign::kLeft, foregroundPaint,
      backgroundPaint, SkString("Roboto"), 14, SkFontStyle::Normal(), x, y);
}
...
bool Paint::drawText(std::u16string text, SkCanvas* canvas, SkScalar width) {
  ...
  return drawText(
      std::move(text), canvas, TextDirection::kLtr, TextAlign::kLeft, foregroundPaint,
      backgroundPaint, SkString("Roboto"), 14, SkFontStyle::Normal(),
      SkSize::Make(width, SK_ScalarInfinity), 0, 0);
}

bool Paint::drawText(
    std::u16string text, SkCanvas* canvas, TextDirection textDirection, TextAlign textAlign,
    SkPaint foreground, SkPaint background, const SkString& fontFamily, SkScalar fontSize,
    SkFontStyle fontStyle, SkScalar x, SkScalar y) {
  return drawText(
      std::move(text), canvas, textDirection, textAlign, foreground, background, fontFamily,
      fontSize, fontStyle, SkSize::Make(SK_ScalarInfinity, SK_ScalarInfinity), x, y);
}
...
bool Paint::drawText(
  ...
  sk_sp<TrivialFontChain> fontChain =
      sk_make_sp<TrivialFontChain>(fontFamily.c_str(), fontSize, fontStyle);
  FontBlock fontBlock(text.size(), fontChain);
  if (fontChain->getTypeface() == nullptr || fontChain->getTypeface().get() == nullptr) {
    return false;
  }

  auto formattedText = Paint::layout(
      std::move(text), textDirection, textAlign, reqSize, SkSpan<FontBlock>(&fontBlock, 1));
  ...
  Paint paint;
  paint.paint(
      canvas, SkPoint::Make(x, y), nullptr, formattedText.get(),
      SkSpan<DecoratedBlock>(&decoratedBlock, 1));

  return true;
}

void Paint::onGlyphRun(
...
std::unique_ptr<WrappedText> Paint::layout(
    std::u16string text, TextDirection textDirection, TextAlign textAlign, SkSize reqSize,
    SkSpan<FontBlock> fontBlocks) {
  auto unicode = SkUnicode::Make();
  auto unicodeText = std::make_unique<UnicodeText>(
      std::move(unicode), SkSpan<uint16_t>((uint16_t*)text.data(), text.size()));
...
void Paint::paint(
  ...
  if (chunks.size() == 1) {
    ...
    wrappedText->visit(
        unicodeText, this, PositionType::kGraphemeCluster,
        SkSpan<size_t>(chunks.data(), chunks.size()));
  }
}
