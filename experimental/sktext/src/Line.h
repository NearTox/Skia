...
class TextMetrics {
 public:
  ...
  SkScalar height() const { return this->fDescent - this->fAscent + this->fLeading; }

  SkScalar baseline() const { return -this->fAscent + this->fLeading / 2; }
...
class Stretch {
 public:
  ...
  void moveTo(Stretch& tail) {
    ...
    SkASSERT(
        this->fGlyphEnd.runIndex() != tail.fGlyphStart.runIndex() ||
...
class Line {
 public:
  ...
  GlyphRange glyphRange(size_t runIndex, size_t runSize, bool includingTrailingSpaces) const {
    GlyphIndex start =
        runIndex != this->glyphStart().runIndex() ? 0 : this->glyphStart().glyphIndex();
    GlyphIndex end = runIndex != this->glyphTrailingEnd().runIndex()
                         ? runSize
                         : this->glyphTrailingEnd().glyphIndex();

    if (!includingTrailingSpaces) {
      // It's possible that the run in question consists of trailing spaces and therefore should not
      // be count
