...
struct TestRun {
  ...
  size_t trailingSpaces;  // Depending of TextDirection it goes right to the end (LTR) or left to
                          // the start (RTL)
...
class TestVisitor : public Visitor {
 public:
  void onBeginLine(size_t index, TextRange lineText, bool hardBreak, SkRect bounds) override {
    ...
    fTestLines.push_back(
        {index, lineText, hardBreak, bounds, EMPTY_RANGE,
         Range<RunIndex>(fTestRuns.size(), fTestRuns.size()), 0});
  }
  void onEndLine(
...
UNIX_ONLY_TEST(SkText_SelectableText_Bounds, reporter) {
  sk_sp<TrivialFontChain> fontChain =
      sk_make_sp<TrivialFontChain>("Roboto", 40.0f, SkFontStyle::Normal());
  if (fontChain->empty()) return;

  std::u16string utf16(
      u"    Leading spaces\nTrailing spaces    \nLong text with collapsed      spaces inside "
      u"wrapped into few lines");
  UnicodeText unicodeText(
      SkUnicode::Make(), SkSpan<uint16_t>((uint16_t*)utf16.data(), utf16.size()));
  ...
  for (LineIndex lineIndex = 0; lineIndex < selectableText->countLines(); ++lineIndex) {
    ...
    for (RunIndex runIndex = testLine.runRange.fEnd; runIndex > testLine.runRange.fStart;
         --runIndex) {
...
UNIX_ONLY_TEST(SkText_SelectableText_Navigation_FirstLast, reporter) {
  ...
  REPORTER_ASSERT(
      reporter, SkScalarNearlyEqual(firstPosition.fBoundaries.height(), firstLine.bounds.height()));
  ...
  REPORTER_ASSERT(
      reporter,
      lastPosition.fGlyphRange == GlyphRange(lastRun.glyphs.size(), lastRun.glyphs.size()));
  REPORTER_ASSERT(
      reporter, SkScalarNearlyEqual(
                    lastPosition.fBoundaries.fLeft, lastRun.positions[lastRun.glyphs.size()].fX));
  REPORTER_ASSERT(
      reporter, SkScalarNearlyEqual(lastPosition.fBoundaries.fTop, lastLine.bounds.fTop));
  REPORTER_ASSERT(reporter, SkScalarNearlyEqual(lastPosition.fBoundaries.width(), 0.0f));
  REPORTER_ASSERT(
      reporter, SkScalarNearlyEqual(lastPosition.fBoundaries.height(), lastLine.bounds.height()));
}
...
UNIX_ONLY_TEST(SkText_SelectableText_ScanRightByGraphemeClusters, reporter) {
  ...
  while (!(position.fGlyphRange == lastPosition.fGlyphRange)) {
    ...
    if (position.fLineIndex == next.fLineIndex - 1) {
      ...
      REPORTER_ASSERT(
          reporter, SkScalarNearlyEqual(next.fBoundaries.height(), line.fBounds.height()));
    } else {
      ...
      REPORTER_ASSERT(
          reporter, SkScalarNearlyEqual(position.fBoundaries.fRight, next.fBoundaries.fLeft));
      REPORTER_ASSERT(
          reporter, SkScalarNearlyEqual(position.fBoundaries.fTop, next.fBoundaries.fTop));
      REPORTER_ASSERT(
          reporter, SkScalarNearlyEqual(position.fBoundaries.height(), next.fBoundaries.height()));
    }
    position = next;
  }
}
...
UNIX_ONLY_TEST(SkText_SelectableText_ScanLeftByGraphemeClusters, reporter) {
  ...
  while (!(position.fGlyphRange == firstPosition.fGlyphRange)) {
    ...
    if (position.fLineIndex == prev.fLineIndex + 1) {
      ...
      REPORTER_ASSERT(
          reporter, SkScalarNearlyEqual(prev.fBoundaries.height(), line.fBounds.height()));
    } else {
      ...
      REPORTER_ASSERT(
          reporter, SkScalarNearlyEqual(position.fBoundaries.fLeft, prev.fBoundaries.fRight));
      REPORTER_ASSERT(
          reporter, SkScalarNearlyEqual(position.fBoundaries.fTop, prev.fBoundaries.fTop));
      REPORTER_ASSERT(
          reporter, SkScalarNearlyEqual(position.fBoundaries.height(), prev.fBoundaries.height()));
    }
    position = prev;
  }
}
...
UNIX_ONLY_TEST(SkText_SelectableText_Navigation_UpDown, reporter) {
  ...
  // Down position
  position = selectableText->lastInLinePosition(
      PositionType::kGraphemeCluster, selectableText->countLines() - 1);
