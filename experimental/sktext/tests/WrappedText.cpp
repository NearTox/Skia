  ...
  void onBeginLine(size_t index, TextRange lineText, bool hardBreak, SkRect bounds) override {
    ...
    fTestLines.push_back(
        {index, lineText, hardBreak, bounds, EMPTY_RANGE,
         Range<RunIndex>(fTestRuns.size(), fTestRuns.size())});
  }
  void onEndLine(
...
UNIX_ONLY_TEST(SkText_WrappedText_Spaces, reporter) {
  ...
  for (int lineIndex = 0; lineIndex < testVisitor.fTestLines.size(); ++lineIndex) {
    ...
    // There is only one line that is wrapped and it has enough trailing spaces to exceed the line
    // width
    REPORTER_ASSERT(
        reporter, (line.index == 2 ? line.bounds.width() > 440.0f : line.bounds.width() < 440.0f));
...
UNIX_ONLY_TEST(SkText_WrappedText_LongRTL, reporter) {
  sk_sp<TrivialFontChain> fontChain =
      sk_make_sp<TrivialFontChain>("Noto Naskh Arabic", 40.0f, SkFontStyle::Normal());
  if (fontChain->empty()) return;
