...
UNIX_ONLY_TEST(SkText_FontResolution1, reporter) {
  ...
  REPORTER_ASSERT(
      reporter, resolvedFonts.front().typeface->uniqueID() == fontChain->getTypeface()->uniqueID());
...
UNIX_ONLY_TEST(SkText_FontResolution3, reporter) {
  auto fontChain = sk_sp(new MultipleFontChain(
      {"Roboto", "Noto Color Emoji", "Noto Serif CJK JP"}, 40.0f, SkFontStyle::Normal()));
  ...
  REPORTER_ASSERT(
      reporter, resolvedFonts.size() == 8 /* 1English 3English spaces + 2Emoji + 2JP */);
