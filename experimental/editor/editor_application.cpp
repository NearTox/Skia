...
static const char* key_name(sk_app::Window::Key k) {
  switch (k) {
#define M(X) \
  case sk_app::Window::Key::k##X: return #X
    M(NONE);
    M(LeftSoftKey);
    M(RightSoftKey);
    M(Home);
...
static SkString modifiers_desc(uint32_t m) {
  SkString s;
#define M(X)                                    \
  if (m & sk_app::Window::k##X##_ModifierKey) { \
    s.append(" {" #X "}");                      \
  }
  M(Shift)
  M(Control)
...
struct EditorLayer : public sk_app::Window::Layer {
  ...
  EditorLayer() { fEditor.setFont(SkFont(nullptr, 24)); }
  ...
  bool onChar(SkUnichar c, uint32_t modifiers) override {
    if (0 ==
        (modifiers & (sk_app::Window::kControl_ModifierKey | sk_app::Window::kOption_ModifierKey |
  ...
  bool onKey(
      sk_app::Window::Key key, sk_app::Window::InputState state, uint32_t modifiers) override {
    if (state == sk_app::Window::kDown_InputState) {
      switch (key) {
        case sk_app::Window::Key::kPageDown: return this->scroll(fHeight * 4 / 5);
        case sk_app::Window::Key::kPageUp: return this->scroll(-fHeight * 4 / 5);
        ...
        case sk_app::Window::Key::kEnd:
          return this->moveCursor(convert(key), modifiers & sk_app::Window::kShift_ModifierKey);
        case sk_app::Window::Key::kDelete:
          if (fMarkPos != editor::Editor::TextPosition()) {
            ...
            fTextPos =
                fEditor.remove(fTextPos, fEditor.move(editor::Editor::Movement::kRight, fTextPos));
          }
          this->inval();
          return true;
        case sk_app::Window::Key::kBack:
          if (fMarkPos != editor::Editor::TextPosition()) {
            ...
            fTextPos =
                fEditor.remove(fTextPos, fEditor.move(editor::Editor::Movement::kLeft, fTextPos));
          }
          this->inval();
          return true;
        default: break;
