...
struct EditorLayer : public sk_app::Window::Layer {
  ...
  bool onChar(SkUnichar c, ModifierKey modifiers) override {
    if (!ModifierKeyIsSet(
            modifiers & (ModifierKey::kControl | ModifierKey::kOption | ModifierKey::kCommand))) {
    ...
    if (state == InputState::kDown) {
      switch (key) {
        ...
        case sk_app::Window::Key::kEnd:
          return this->moveCursor(convert(key), (int)(modifiers & ModifierKey::kShift));
        ...
        case sk_app::Window::Key::kBack:
          ...
          this->inval();
          return true;
        case sk_app::Window::Key::kOK: return this->onChar('\n', modifiers);
        default: break;
      }
#ifdef SK_EDITOR_DEBUG_OUT
