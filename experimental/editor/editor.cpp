...
static SkRect selection_box(const SkFontMetrics& metrics, float advance, SkPoint pos) {
  ...
  return SkRect{pos.x(), pos.y() + metrics.fAscent, pos.x() + advance, pos.y() + metrics.fDescent}
      .makeSorted();
}

void callback_fn(
    void* context, const char* utf8Text, size_t utf8TextBytes, size_t glyphCount,
    const SkGlyphID* glyphs, const SkPoint* positions, const uint32_t* clusters,
    const SkFont& font) {
  SkASSERT(context);
...
void Editor::Shape(
    TextLine* line, SkShaper* shaper, float width, const SkFont& font, SkRect space) {
  SkASSERT(line);
  SkASSERT(shaper);
  ...
  if (line->fText.size() > 0) {
    last.offset(
        line->fCursorPos[line->fText.size() - 1].fRight,
...
void Editor::setText(const char* data, size_t length) {
  ...
  if (data && length && valid_utf8(data, length)) {
    readlines(
        data, length, [&lines](const char* p, size_t s) { lines.push_back(remove_newline(p, s)); });
  }
...
StringSlice Editor::copy(TextPosition pos1, TextPosition pos2) const {
  ...
  if (start.fParagraphIndex == end.fParagraphIndex) {
    ...
    for (const TextLine* line = &fLines.begin()[start.fParagraphIndex + 1];
         line < &fLines.begin()[end.fParagraphIndex]; ++line) {
...
Editor::TextPosition Editor::move(Editor::Movement move, Editor::TextPosition pos) const {
  ...
  switch (move) {
    ...
    case Editor::Movement::kRight:
      ...
      break;
    case Editor::Movement::kHome: pos.fTextByteIndex = 0; break;
