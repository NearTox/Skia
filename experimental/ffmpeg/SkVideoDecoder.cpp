...
sk_sp<SkColorSpace> make_colorspace(
    AVColorPrimaries primaries, AVColorTransferCharacteristic transfer) {
...
static int64_t skstream_seek_packet(void* ctx, int64_t pos, int whence) {
  ...
  switch (whence) {
    case SEEK_SET: break;
    case SEEK_CUR: pos = (int64_t)stream->getPosition() + pos; break;
    case SEEK_END: pos = (int64_t)stream->getLength() + pos; break;
    default: return -1;
  }
...
void SkVideoDecoder::ConvertedColorSpace::update(
    AVColorPrimaries primaries, AVColorTransferCharacteristic transfer) {
...
sk_sp<SkImage> SkVideoDecoder::convertFrame(const AVFrame* frame) {
  ...
  switch (frame->format) {
    case AV_PIX_FMT_YUV420P:
      if (auto image = make_yuv_420(
              fRecordingContext, frame->width, frame->height, frame->data, frame->linesize,
  ...
  const auto info =
      SkImageInfo::MakeN32(frame->width, frame->height, SkAlphaType::kOpaque_SkAlphaType);
  ...
  auto* ctx = sws_getContext(
      frame->width, frame->height, (AVPixelFormat)frame->format, info.width(), info.height(), fmt,
...
SkVideoDecoder::~SkVideoDecoder() { this->reset(); }
...
bool SkVideoDecoder::loadStream(std::unique_ptr<SkStream> stream) {
  ...
  fStreamCtx = avio_alloc_context(
      buffer, bufferSize, 0, fStream.get(), skstream_read_packet, nullptr, skstream_seek_packet);
  if (!fStreamCtx) {
    av_freep(buffer);
    this->reset();
    return false;
