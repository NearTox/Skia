...
static int64_t sk_seek_packet(void* ctx, int64_t pos, int whence) {
  ...
  switch (whence) {
    case SEEK_SET: break;
    case SEEK_CUR: pos = (int64_t)stream->pos() + pos; break;
    case SEEK_END: pos = (int64_t)stream->size() + pos; break;
    default: return -1;
  }
...
bool SkVideoEncoder::init(int fps) {
  ...
  fStreamCtx = avio_alloc_context(
      buffer, bufferSize, AVIO_FLAG_WRITE, fWStream.get(), nullptr, sk_write_packet,
      sk_seek_packet);
  SkASSERT(fStreamCtx);
...
bool SkVideoEncoder::beginRecording(SkISize dim, int fps) {
  ...
  fSWScaleCtx = sws_getCachedContext(
      fSWScaleCtx, dim.width(), dim.height(), fmt, dim.width(), dim.height(), AV_PIX_FMT_YUV420P,
