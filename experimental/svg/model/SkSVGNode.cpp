...
bool SkSVGNode::onPrepareToRender(SkSVGRenderContext* ctx) const {
  ctx->applyPresentationAttributes(
      fPresentationAttributes, this->hasChildren() ? 0 : SkSVGRenderContext::kLeaf);
...
void SkSVGNode::setClipPath(const SkSVGClip& clip) { fPresentationAttributes.fClipPath.set(clip); }

void SkSVGNode::setClipRule(const SkSVGFillRule& clipRule) {
  fPresentationAttributes.fClipRule.set(clipRule);
}

void SkSVGNode::setFill(const SkSVGPaint& svgPaint) { fPresentationAttributes.fFill.set(svgPaint); }
...
void SkSVGNode::setOpacity(const SkSVGNumberType& opacity) {
  fPresentationAttributes.fOpacity.set(SkSVGNumberType(SkTPin<SkScalar>(opacity.value(), 0, 1)));
}
