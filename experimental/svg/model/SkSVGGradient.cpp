...
void SkSVGGradient::collectColorStops(
  ...
  for (const auto& child : fChildren) {
    ...
    colors->push_back(SkColorSetA(stop.stopColor(), SkScalarRoundToInt(stop.stopOpacity() * 255)));
    pos->push_back(
...
bool SkSVGGradient::onAsPaint(const SkSVGRenderContext& ctx, SkPaint* paint) const {
  ...
  paint->setShader(this->onMakeShader(
      ctx, colors.begin(), pos.begin(), colors.count(), tileMode, fGradientTransform.value()));
  return true;
}
