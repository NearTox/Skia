...
void SkSVGGradient::collectColorStops(const SkSVGRenderContext& ctx,
    ...
    for (const auto& child : fChildren) {
        ...
        colors->push_back(
                SkColorSetA(stop.stopColor(), SkScalarRoundToInt(stop.stopOpacity() * 255)));
...
bool SkSVGGradient::onAsPaint(const SkSVGRenderContext& ctx, SkPaint* paint) const {
    ...
    static_assert(static_cast<SkTileMode>(SkSVGSpreadMethod::Type::kPad) == SkTileMode::kClamp,
                  "SkSVGSpreadMethod::Type is out of sync");
    static_assert(static_cast<SkTileMode>(SkSVGSpreadMethod::Type::kRepeat) == SkTileMode::kRepeat,
                  "SkSVGSpreadMethod::Type is out of sync");
    static_assert(static_cast<SkTileMode>(SkSVGSpreadMethod::Type::kReflect) == SkTileMode::kMirror,
                  "SkSVGSpreadMethod::Type is out of sync");
