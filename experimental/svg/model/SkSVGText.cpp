...
void SkSVGText::setFontFamily(const SkSVGStringType& font_family) {
  fTypeface = SkTypeface::MakeFromName(font_family.value().c_str(), SkFontStyle());
}
...
void SkSVGText::onDraw(
  ...
  SkTextUtils::DrawString(
      canvas, fText.value().c_str(), fX.value(), fY.value(), font, paint, fTextAlign);
}
