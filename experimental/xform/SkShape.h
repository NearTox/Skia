...
class Shape : public SkRefCnt {
 ...
 public:
  ...
  void setXform(sk_sp<Xform> x) { fXform = std::move(x); }
...
class GroupShape : public Shape {
 ...
 public:
  ...
  ~GroupShape() override { fArray.unrefAll(); }

  int count() const { return fArray.count(); }
  Shape* get(int index) const { return fArray[index]; }
  void set(int index, sk_sp<Shape> s) { fArray[index] = s.release(); }

  void append(sk_sp<Shape> s) { *fArray.append() = s.release(); }
  void insert(int index, sk_sp<Shape> s) { *fArray.insert(index) = s.release(); }
