...
class Component : public SkRefCnt {
 public:
  ...
  template <typename T>
  std::enable_if_t<std::is_base_of<Component, T>::value, bool> is() const {
  ...
  void render(SkCanvas* canvas) const { this->onRender(canvas); }
...
class Paint : public Component {
 public:
  ...
  void apply(SkPaint* paint) const { this->onApply(paint); }
...
class Shape final : public Drawable {
 ...
 private:
  ...
  std::vector<const Paint*> fFills, fStrokes;
