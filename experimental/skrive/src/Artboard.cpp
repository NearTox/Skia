...
std::tuple<sk_sp<Component>, size_t> parse_component(StreamReader* sr) {
  ...
  switch (block.type()) {
    ...
    case StreamReader::BlockType::kActorEllipse: return make_from_stream<Ellipse>(sr);
    case StreamReader::BlockType::kActorRectangle: return make_from_stream<Rectangle>(sr);
    default: break;
  }
...
sk_sp<Artboard> parse_artboard(StreamReader* sr) {
  ...
  ab->setSize({sr->readFloat("width"), sr->readFloat("height")});
  ...
  for (;;) {
    ...
    switch (block.type()) {
      case StreamReader::BlockType::kComponents: ab->setRoot(parse_components(sr)); break;
      default: SkDebugf("!! Unsupported block type: %d\n", (int)block.type()); break;
    }
  }

  SkDebugf(
      ".. parsed artboard \"%s\" [%f x %f]\n", ab->getName().c_str(), ab->getSize().x,
      ab->getSize().y);
