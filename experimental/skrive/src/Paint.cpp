...
void parse_fill_stroke(StreamReader* sr, Paint* node) {
  if (node->style() == SkPaint::kFill_Style) {
    ...
    node->setFillRule(gFillTypeMap[std::min<size_t>(
        sr->readUInt8("fillRule"), SK_ARRAY_COUNT(gFillTypeMap) - 1)]);
  } else {
    ...
    node->setStrokeCap(
        gCapMap[std::min<size_t>(sr->readUInt8("cap"), SK_ARRAY_COUNT(gCapMap) - 1)]);
    ...
    node->setStrokeJoin(
        gJoinMap[std::min<size_t>(sr->readUInt8("join"), SK_ARRAY_COUNT(gJoinMap) - 1)]);
    ...
    node->setStrokeTrim(
        gTrimMap[std::min<size_t>(sr->readUInt8("trim"), SK_ARRAY_COUNT(gTrimMap) - 1)]);
