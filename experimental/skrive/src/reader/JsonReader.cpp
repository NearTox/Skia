...
StreamReader::BlockType block_type(const char* type_name) {
  ...
  const auto* map_entry = std::lower_bound(
      std::begin(gTypeMap), std::end(gTypeMap), key,
      [](const TypeMapEntry& a, const TypeMapEntry& b) { return strcmp(a.name, b.name) < 0; });
...
class JsonReader final : public StreamReader {
 public:
  explicit JsonReader(std::unique_ptr<skjson::DOM> dom) : fDom(std::move(dom)) {
    fContextStack.push_back({&fDom->root(), 0});
  }

  ~JsonReader() override { SkASSERT(fContextStack.size() == 1); }
 ...
 private:
  ...
  const T* readProp(const char label[]) {
    ...
    return ctx.fMemberIndex < jarr->size() ? static_cast<const T*>((*jarr)[ctx.fMemberIndex++])
                                           : nullptr;
  }
  ...
  uint8_t readLength8() override { return SkToU8(this->currentLength()); }

  uint16_t readLength16() override { return SkToU16(this->currentLength()); }
