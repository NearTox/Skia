...
static void test_expect_pass(skiatest::Reporter* r, SkSurface* surface, const char* testFile) {
  ...
  if (color != SkColorSetARGB(0xFF, 0x00, 0xFF, 0x00)) {
    ERRORF(
        ...
        "RRGGBBAA\n"
        "%02X%02X%02X%02X",
        testFile, SkColorGetR(color), SkColorGetG(color), SkColorGetB(color), SkColorGetA(color));
  }
}
...
DEF_TEST(SkSL_ES2Conformance_Fail, r) {
  iterate_dir("sksl/es2_conformance/fail/", [&](const char* path) { test_expect_fail(r, path); });
}
