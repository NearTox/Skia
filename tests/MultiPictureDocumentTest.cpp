...
DEF_TEST(SkMultiPictureDocument_Serialize_and_deserialize, reporter) {
  ...
  REPORTER_ASSERT(
      reporter, writtenStream->getLength() > 24, "Written data length too short (%zu)",
  ...
  REPORTER_ASSERT(
      reporter, frame_count == NUM_FRAMES,
  ...
  for (const auto& frame : frames) {
    ...
    REPORTER_ASSERT(
        reporter, bounds.width() == WIDTH, "Page width: expected (%d) got (%d)", WIDTH,
        (int)bounds.width());
    REPORTER_ASSERT(
        reporter, bounds.height() == HEIGHT, "Page height: expected (%d) got (%d)", HEIGHT,
        (int)bounds.height());
...
static sk_sp<SkImage> makeAHardwareBufferTestImage(
  ...
  hwbDesc.layers = 1;
  hwbDesc.usage = AHARDWAREBUFFER_USAGE_CPU_READ_OFTEN | AHARDWAREBUFFER_USAGE_CPU_WRITE_OFTEN |
  ...
  if (AHardwareBuffer_lock(
          buffer, AHARDWAREBUFFER_USAGE_CPU_WRITE_OFTEN, -1, nullptr, &bufferAddr)) {
...
// Test the onEndPage callback's intended use by processing an mskp containing
// AHardwareBuffer-backed SkImages Expected behavior is that the callback is called while the
// AHardwareBuffer is still valid and the images are copied so .close() can still access them.
// Confirm deserialized file contains images with correct data.
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SkMultiPictureDocument_AHardwarebuffer, reporter, ctx_info) {
  ...
  sk_sp<SkDocument> multipic =
      SkMakeMultiPictureDocument(&stream, &procs, [sharingCtx = &ctx](const SkPicture* pic) {
