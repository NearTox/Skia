...
static void test_erasecolor_premul(
    skiatest::Reporter* reporter, SkColorType ct, SkColor input, SkColor expected) {
  SkBitmap bm;
...
DEF_TEST(Bitmap_erase_srgb, r) {
  ...
  bm.allocPixels(SkImageInfo::Make(
      1, 1, kN32_SkColorType, kPremul_SkAlphaType, SkColorSpace::MakeSRGB()->makeColorSpin()));
...
DEF_TEST(Bitmap_erase, r) {
  SkColorType colorTypes[] = {
      kRGB_565_SkColorType,      kARGB_4444_SkColorType, kRGB_888x_SkColorType,
      kRGBA_8888_SkColorType,    kBGRA_8888_SkColorType, kRGB_101010x_SkColorType,
      kRGBA_1010102_SkColorType,
  };
...
DEF_TEST(getalphaf, reporter) {
  ...
  auto nearly = [](float expected, float actual) -> bool { return unit_compare(expected, actual); };
  ...
  auto opaque = [](float expected, float actual) -> bool { return actual == 1.0f; };
...
DEF_TEST(bitmap_zerowidth_crbug_1103827, reporter) {
  ...
  for (const auto& r : rec) {
    auto info = SkImageInfo::Make(r.width, r.height, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
