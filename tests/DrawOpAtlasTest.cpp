...
static bool fill_plot(
    ...
    GrDrawOpAtlas::AtlasID* atlasID, int alpha) {
  ...
  code = atlas->addToAtlas(
      resourceProvider, atlasID, target, kPlotSize, kPlotSize, data.getAddr(0, 0), &loc);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(BasicDrawOpAtlas, reporter, ctxInfo) {
  ...
  std::unique_ptr<GrDrawOpAtlas> atlas = GrDrawOpAtlas::Make(
      ...
      kAtlasSize / kNumPlots, GrDrawOpAtlas::AllowMultitexturing::kYes, EvictionFunc, nullptr);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrAtlasTextOpPreparation, reporter, ctxInfo) {
  ...
  auto rtc = context->priv().makeDeferredRenderTargetContext(
      SkBackingFit::kApprox, 32, 32, GrColorType::kRGBA_8888, nullptr);
  ...
  GrOpFlushState::OpArgs opArgs = {op.get(), rtc->asRenderTargetProxy(), nullptr,
