...
static bool fill_plot(
    ...
    GrDrawOpAtlas::PlotLocator* plotLocator, int alpha) {
  ...
  code = atlas->addToAtlas(
      resourceProvider, plotLocator, target, kPlotSize, kPlotSize, data.getAddr(0, 0), &loc);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(BasicDrawOpAtlas, reporter, ctxInfo) {
  ...
  for (int i = 0; i < kNumPlots * kNumPlots; ++i) {
    bool result = fill_plot(atlas.get(), resourceProvider, &uploadTarget, &plotLocators[i], i * 32);
