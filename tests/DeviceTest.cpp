...
class DeviceTestingAccess {
 public:
  ...
  static sk_sp<SkSpecialImage> SnapSpecial(SkBaseDevice* dev) { return dev->snapSpecial(); }
};
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SpecialImage_GPUDevice, reporter, ctxInfo) {
  ...
  auto device = dContext->priv().createDevice(
      SkBudgeted::kNo, ii, SkBackingFit::kExact, 1, GrMipmapped::kNo, GrProtected::kNo,
      kBottomLeft_GrSurfaceOrigin, SkSurfaceProps(), skgpu::BaseDevice::InitContents::kClear);
