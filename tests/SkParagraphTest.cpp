...
DEF_TEST(SkParagraph_SimpleParagraph, reporter) {
  ...
  for (auto& line : impl->lines()) {
    line.scanStyles(
        StyleType::kDecorations, [&index, reporter](TextRange text, TextStyle style, SkScalar) {
...
DEF_TEST(SkParagraph_JustifyAlignParagraph, reporter) {
  ...
  expected_y += 30 * 9;
  REPORTER_ASSERT(
      reporter, SkScalarNearlyEqual(impl->lines()[12].offset().fY, expected_y, epsilon));
...
DEF_TEST(SkParagraph_ItalicsParagraph, reporter) {
  ...
  size_t index = 0;
  line.scanStyles(
      StyleType::kForeground, [&index, reporter](TextRange textRange, TextStyle style, SkScalar) {
        switch (index) {
          case 0:
...
DEF_TEST(SkParagraph_GetGlyphPositionAtCoordinateParagraph, reporter) {
  ...
  REPORTER_ASSERT(
      reporter, paragraph->getGlyphPositionAtCoordinate(300, 2).position == 10);  // !!! 11
  ...
  REPORTER_ASSERT(
      reporter, paragraph->getGlyphPositionAtCoordinate(20, -80).position == 0);  // !!! 1
...
DEF_TEST(SkParagraph_GetRectsForRangeIncludeCombiningCharacter, reporter) {
  ...
  }
  {
    ...
    REPORTER_ASSERT(
        reporter, last[0].rect.fLeft = SkTMin(first[0].rect.fLeft, second[0].rect.fLeft));
    REPORTER_ASSERT(
        reporter, last[0].rect.fRight = SkTMax(first[0].rect.fRight, second[0].rect.fRight));
  }
  {
  ...
  }
  {
    ...
    REPORTER_ASSERT(
        reporter, first.size() == 1 && second.size() == 1 && third.size() == 1 && last.size() == 1);
    REPORTER_ASSERT(
        reporter, last[0].rect.fLeft = SkTMin(first[0].rect.fLeft, third[0].rect.fLeft));
    REPORTER_ASSERT(
        reporter, last[0].rect.fRight = SkTMax(first[0].rect.fRight, third[0].rect.fRight));
  }
}
...
DEF_TEST(SkParagraph_SpacingParagraph, reporter) {
  ...
  impl->lines().begin()->scanStyles(
      StyleType::kLetterSpacing, [&index](TextRange text, TextStyle style, SkScalar) {
        ++index;
        return true;
      });
  REPORTER_ASSERT(reporter, index == 4);
  index = 0;
  impl->lines().begin()->scanStyles(
      StyleType::kWordSpacing, [&index](TextRange text, TextStyle style, SkScalar) {
        ++index;
        return true;
      });
  REPORTER_ASSERT(reporter, index == 4);
...
DEF_TEST(SkParagraph_NewlineParagraph, reporter) {
  ...
  REPORTER_ASSERT(
      reporter, SkScalarNearlyEqual(impl->lines()[4].width(), 0, epsilon));  // Empty line
  ...
  REPORTER_ASSERT(reporter, impl1->lines().size() == 1);
  {
    ...
    line.scanStyles(
        StyleType::kDecorations, [&index, reporter](TextRange text, TextStyle style, SkScalar) {
          switch (index) {
            case 0:
              REPORTER_ASSERT(reporter, style.getDecorationType() == TextDecoration::kNoDecoration);
              break;
            case 1:
              REPORTER_ASSERT(reporter, style.getDecorationType() == TextDecoration::kUnderline);
              break;
            default: REPORTER_ASSERT(reporter, false); break;
          }
          ++index;
...
DEF_TEST(SkParagraph_BaselineParagraph, reporter) {
  ...
  SkScalar epsilon = 0.01f;
  REPORTER_ASSERT(
      reporter, SkScalarNearlyEqual(paragraph->getIdeographicBaseline(), 79.035f, epsilon));
  REPORTER_ASSERT(
      reporter, SkScalarNearlyEqual(paragraph->getAlphabeticBaseline(), 63.305f, epsilon));
}
