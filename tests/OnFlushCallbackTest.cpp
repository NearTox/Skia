...
class NonAARectOp : public GrMeshDrawOp {
 ...
 public:
  ...
  GrProcessorSet::Analysis finalize(
      const GrCaps& caps, const GrAppliedClip*, GrFSAAType fsaaType,
 ...
 private:
  void onPrepareDraws(Target* target) override {
    ...
    sk_sp<GrGeometryProcessor> gp = GrDefaultGeoProcFactory::Make(
        target->caps().shaderCaps(), Color::kPremulGrColorAttribute_Type, Coverage::kSolid_Type,
...
class AtlasedRectOp final : public NonAARectOp {
 public:
  ...
  static std::unique_ptr<AtlasedRectOp> Make(
      GrContext* context, GrPaint&& paint, const SkRect& r, int id) {
    GrDrawOp* op = Helper::FactoryHelper<AtlasedRectOp>(context, std::move(paint), r, id).release();
  ...
  void setNext(AtlasedRectOp* next) { fNext = next; }
...
const GrColor AtlasedRectOp::kColors[kMaxIDs] = {
    GrColorPackRGBA(255, 0, 0, 255),    GrColorPackRGBA(0, 255, 0, 255),
    GrColorPackRGBA(0, 0, 255, 255),    GrColorPackRGBA(0, 255, 255, 255),
    GrColorPackRGBA(255, 0, 255, 255),  GrColorPackRGBA(255, 255, 0, 255),
    GrColorPackRGBA(0, 0, 0, 255),      GrColorPackRGBA(128, 128, 128, 255),
...
class AtlasObject final : public GrOnFlushCallbackObject {
 public:
  ...
  ~AtlasObject() override { SkASSERT(fDone); }

  void markAsDone() { fDone = true; }
  ...
  sk_sp<GrTextureProxy> getAtlasProxy(GrProxyProvider* proxyProvider, const GrCaps* caps) {
    ...
    fAtlasProxy = GrProxyProvider::MakeFullyLazyProxy(
        [](GrResourceProvider* resourceProvider) -> GrSurfaceProxy::LazyInstantiationResult {
          GrSurfaceDesc desc;
        ...
        kRGBA_8888_GrPixelConfig, *proxyProvider->caps());
   ...
   */
  void preFlush(
      GrOnFlushResourceProvider* resourceProvider, const uint32_t* opListIDs, int numOpListIDs,
    ...
    SkASSERT(
        2 == fAtlasProxy->getPendingWriteCnt_TestOnly() ||
        0 == fAtlasProxy->getPendingWriteCnt_TestOnly());
    sk_sp<GrRenderTargetContext> rtc =
        resourceProvider->makeRenderTargetContext(fAtlasProxy, nullptr, nullptr);
    ...
    for (int i = 0; i < lists.count(); ++i) {
      for (AtlasedRectOp* op = lists[i]->fHead; op; op = op->next()) {
        SkIRect r =
            SkIRect::MakeXYWH(blocksInAtlas * kAtlasTileSize, 0, kAtlasTileSize, kAtlasTileSize);
        ...
        std::unique_ptr<GrDrawOp> drawOp(NonAARectOp::Make(std::move(paint), SkRect::Make(r)));
...
static sk_sp<GrTextureProxy> make_upstream_image(
  ...
  sk_sp<GrRenderTargetContext> rtc(context->priv().makeDeferredRenderTargetContext(
      format, SkBackingFit::kApprox, 3 * kDrawnTileSize, kDrawnTileSize, kRGBA_8888_GrPixelConfig,
      nullptr));
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    std::unique_ptr<AtlasedRectOp> op(AtlasedRectOp::Make(context, std::move(paint), r, start + i));

    AtlasedRectOp* sparePtr = op.get();

    uint32_t opListID;
    rtc->priv().testingOnly_addDrawOp(
        GrNoClip(), std::move(op), [&opListID](GrOp* op, uint32_t id) { opListID = id; });
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(OnFlushCallbackTest, reporter, ctxInfo) {
  ...
  sk_sp<GrRenderTargetContext> rtc(context->priv().makeDeferredRenderTargetContext(
      format, SkBackingFit::kApprox, kFinalWidth, kFinalHeight, kRGBA_8888_GrPixelConfig, nullptr));
  ...
  SkDEBUGCODE(bool result =)
      rtc->readPixels(readBack.info(), readBack.getPixels(), readBack.rowBytes(), 0, 0);
  SkASSERT(result);
