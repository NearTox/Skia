...
void test(
    skiatest::Reporter* r, const char* src, SkSL::Interpreter::Value* in, int expectedCount,
...
void test(
  ...
  if (program) {
    ...
    if (inoutColor[0] != expectedR || inoutColor[1] != expectedG || inoutColor[2] != expectedB ||
        inoutColor[3] != expectedA) {
      printf("for program: %s\n", src);
      printf(
          "    expected (%f, %f, %f, %f), but received (%f, %f, %f, %f)\n", expectedR, expectedG,
          expectedB, expectedA, inoutColor[0], inoutColor[1], inoutColor[2], inoutColor[3]);
      interpreter.disassemble(*main);
    }
...
DEF_TEST(SkSLInterpreterAdd, r) {
  ...
  test(
      ...
      1, 1.5, 2);
  test(
      r, "void main(inout half4 color) { int a = 1; int b = 3; color.r = a + b; }", 1, 2, 3, 4, 4,
      2, 3, 4);
}
...
DEF_TEST(SkSLInterpreterSubtract, r) {
  ...
  test(
      r, "void main(inout half4 color) { int a = 3; int b = 1; color.r = a - b; }", 0, 0, 0, 0, 2,
      0, 0, 0);
}
...
DEF_TEST(SkSLInterpreterMultiply, r) {
  ...
  test(
      r, "void main(inout half4 color) { int a = 3; int b = -2; color.r = a * b; }", 0, 0, 0, 0, -6,
      0, 0, 0);
}
...
DEF_TEST(SkSLInterpreterDivide, r) {
  ...
  test(
      r, "void main(inout half4 color) { int a = 8; int b = -2; color.r = a / b; }", 0, 0, 0, 0, -4,
      0, 0, 0);
}
...
DEF_TEST(SkSLInterpreterRemainder, r) {
  ...
  test(
      ...
      1.5, 0.5, 1.5);
  test(
      r, "void main(inout half4 color) { int a = 8; int b = 3; a %= b; color.r = a; }", 0, 0, 0, 0,
      2, 0, 0, 0);
  test(
      r, "void main(inout half4 color) { int a = 8; int b = 3; color.r = a % b; }", 0, 0, 0, 0, 2,
      0, 0, 0);
  test(
      r, "void main(inout half4 color) { int2 a = int2(8, 10); a %= 6; color.rg = a; }", 0, 0, 0, 0,
      2, 4, 0, 0);
}
...
DEF_TEST(SkSLInterpreterMatrix, r) {
  ...
  expected[0] = 6.0f;
  test(
      r, "float main(float4 v) { float2x2 m = float2x2(v); return m[0][1] + m[1][0]; }", in, 1,
      expected);
  ...
  // M*M
  {
    ...
    test(
        r, "float4x4 main(float4x4 m) { return m * float4x4(m[1], m[2], m[3], m[0]); }", in, 16,
        (SkSL::Interpreter::Value*)&m);
  }
}
...
DEF_TEST(SkSLInterpreterGeneric, r) {
  ...
  float expected1 = 25;
  test(
      r, "float main(float x) { return x * x; }", (SkSL::Interpreter::Value*)&value1, 1,
      (SkSL::Interpreter::Value*)&expected1);
  float value2[2] = {5, 25};
  float expected2[2] = {25, 625};
  test(
      r, "float2 main(float x, float y) { return float2(x * x, y * y); }",
      (SkSL::Interpreter::Value*)&value2, 2, (SkSL::Interpreter::Value*)expected2);
}
...
DEF_TEST(SkSLInterpreterSetInputs, r) {
  ...
  SkSL::Interpreter interpreter(
      std::move(program), std::move(byteCode), (SkSL::Interpreter::Value*)&x);
  float out = 0.0f;
  float in = 2.0f;
...
DEF_TEST(SkSLInterpreterCompound, r) {
  ...
  SkSL::Interpreter interpreter(
      std::move(program), std::move(byteCode), (SkSL::Interpreter::Value*)gRects);

  {
    SkIRect in = SkIRect::MakeXYWH(10, 10, 20, 30);
    int out = 0;
    interpreter.run(*rect_height, (SkSL::Interpreter::Value*)&in, (SkSL::Interpreter::Value*)&out);
...

  {
    ...
    RectAndColor out;
    interpreter.run(
        *make_blue_rect, (SkSL::Interpreter::Value*)in, (SkSL::Interpreter::Value*)&out);
...

  {
    ...
    int out = 0;
    interpreter.run(*median, (SkSL::Interpreter::Value*)in, (SkSL::Interpreter::Value*)&out);
...

  {
    ...
    float out[8] = {0};
    interpreter.run(*sums, (SkSL::Interpreter::Value*)in, (SkSL::Interpreter::Value*)out);
    for (int i = 0; i < 8; ++i) {
...

  {
    ...
    interpreter.run(*get_rect, (SkSL::Interpreter::Value*)&in, (SkSL::Interpreter::Value*)&out);
...

  {
    ...
    in.fNumRects = 2;
    interpreter.run(*fill_rects, (SkSL::Interpreter::Value*)&in, nullptr);
    ManyRects expected;
...
DEF_TEST(SkSLInterpreterMix, r) {
  ...
  test(
      r, "float4 main(float4 x, float4 y) { return mix(x, y, 0.5); }", valueVectors, 4,
      expectedVector);
}

DEF_TEST(SkSLInterpreterCross, r) {
  SkSL::Interpreter::Value args[] = {1.0f, 4.0f, -6.0f, -2.0f, 7.0f, -3.0f};
  SkPoint3 cross = SkPoint3::CrossProduct(
      SkPoint3::Make(args[0].fFloat, args[1].fFloat, args[2].fFloat),
      SkPoint3::Make(args[3].fFloat, args[4].fFloat, args[5].fFloat));
...
DEF_TEST(SkSLInterpreterDot, r) {
  ...
  SkSL::Interpreter::Value expected =
      args[0].fFloat * args[2].fFloat + args[1].fFloat * args[3].fFloat;
  test(r, "float main(float2 x, float2 y) { return dot(x, y); }", args, 1, &expected);

  expected = args[0].fFloat * args[3].fFloat + args[1].fFloat * args[4].fFloat +
             args[2].fFloat * args[5].fFloat;
  test(r, "float main(float3 x, float3 y) { return dot(x, y); }", args, 1, &expected);

  expected = args[0].fFloat * args[4].fFloat + args[1].fFloat * args[5].fFloat +
             args[2].fFloat * args[6].fFloat + args[3].fFloat * args[7].fFloat;
...
class PointerExternalValue : public SkSL::ExternalValue {
 public:
  ...
  void read(void* target) override { memcpy(target, fData, fSize); }

  void write(void* src) override { memcpy(fData, src, fSize); }
