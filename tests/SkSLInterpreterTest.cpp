...
struct ProgramBuilder {
  ProgramBuilder(skiatest::Reporter* r, const char* src) : fCompiler(&fCaps) {
    ...
    fProgram = fCompiler.convertProgram(SkSL::ProgramKind::kGeneric, std::string(src), settings);
    if (!fProgram) {
...
static void verify_values(
    skiatest::Reporter* r, const char* src, const float* actual, const float* expected, int N,
    bool exactCompare) {
  auto exact_equiv = [](float x, float y) { return x == y || (isnan(x) && isnan(y)); };
...
DEF_TEST(SkSLInterpreterAdd, r) {
  ...
  test(
      ...
      1, 1.5, 2);
  test(
      r, "void main(inout half4 color) { color.r = half(int(color.r) + int(color.g)); }", 1, 3, 0,
      0, 4, 3, 0, 0);
}
...
DEF_TEST(SkSLInterpreterSubtract, r) {
  ...
  test(
      r, "void main(inout half4 color) { color.r = half(int(color.r) - int(color.g)); }", 3, 1, 0,
      0, 2, 1, 0, 0);
}
...
DEF_TEST(SkSLInterpreterMultiply, r) {
  ...
  test(
      r, "void main(inout half4 color) { color.r = half(int(color.r) * int(color.g)); }", 3, -2, 0,
      0, -6, -2, 0, 0);
}
...
DEF_TEST(SkSLInterpreterDivide, r) {
  ...
  test(
      r, "void main(inout half4 color) { color.r = half(int(color.r) / int(color.g)); }", 8, -2, 0,
      0, -4, -2, 0, 0);
}
...
DEF_TEST(SkSLInterpreterAnd, r) {
  ...
  test(
      r,
      "half global; bool update() { global = 123; return true; }"
...
DEF_TEST(SkSLInterpreterFor, r) {
  test(
      r, "void main(inout half4 color) { for (int i = 1; i <= 10; ++i) color.r += half(i); }", 0, 0,
      0, 0, 55, 0, 0, 0);
  test(
      r,
...
DEF_TEST(SkSLInterpreterSwizzle, r) {
  ...
  test(
      ...
      4);
  test(r, "void main(inout half4 color) { color.bgr = half3(5, 6, 7); }", 1, 2, 3, 4, 7, 6, 5, 4);
}

DEF_TEST(SkSLInterpreterGlobal, r) {
  test(
      r, "int x; void main(inout half4 color) { x = 10; color.b = half(x); }", 1, 2, 3, 4, 1, 2, 10,
      4);
  test(
...
class ExternalSqrt : public SkSL::ExternalFunction {
 public:
  ExternalSqrt(const char* name, SkSL::Compiler& compiler)
      : INHERITED(name, *compiler.context().fTypes.fFloat), fCompiler(compiler) {}
...
DEF_TEST(SkSLInterpreterExternalFunction, r) {
  ...
  std::unique_ptr<SkSL::Program> program =
      compiler.convertProgram(SkSL::ProgramKind::kGeneric, std::string(src), settings);
  REPORTER_ASSERT(r, program);
...
int main() {
  ...
  skvm::Builder b;
  std::unique_ptr<SkSL::Program> program =
      compiler.convertProgram(SkSL::ProgramKind::kGeneric, std::string(kSrc), settings);
  REPORTER_ASSERT(r, program);
