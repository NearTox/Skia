...
struct ClipRectVisitor {
  ...
  SkRect operator()(const SkRecords::ClipRect& op) { return op.rect; }
};
...
DEF_TEST(canvas_unsorted_clip, r) {
  ...
  for (SkRect clip : {SkRect{0, 0, 5, 5}, SkRect{5, 5, 0, 0}}) {
    ...
    rec.beginRecording({0, 0, 10, 10})->clipRect(clip);
    ...
    REPORTER_ASSERT(r, record->visit(0, ClipRectVisitor{r}).isSorted());
  }
}
...
template <typename F>
static void multi_canvas_driver(int w, int h, F proc) {
...
DEF_TEST(canvas_clip_restriction, reporter) {
  multi_canvas_driver(
      gBaseRestrictedR.width(), gBaseRestrictedR.height(),
...
DEF_TEST(CanvasNewRasterTest, reporter) {
  ...
  // unaligned rowBytes
  REPORTER_ASSERT(reporter, nullptr == SkCanvas::MakeRasterDirect(info, baseAddr, minRowBytes + 1));
...
static CanvasTest kCanvasTests[] = {
    [](SkCanvas* c, skiatest::Reporter* r) { c->translate(SkIntToScalar(1), SkIntToScalar(2)); },
    [](SkCanvas* c, skiatest::Reporter* r) { c->scale(SkIntToScalar(1), SkIntToScalar(2)); },
    [](SkCanvas* c, skiatest::Reporter* r) { c->rotate(SkIntToScalar(1)); },
    [](SkCanvas* c, skiatest::Reporter* r) { c->skew(SkIntToScalar(1), SkIntToScalar(2)); },
    [](SkCanvas* c, skiatest::Reporter* r) { c->concat(SkMatrix::Scale(2, 3)); },
    [](SkCanvas* c, skiatest::Reporter* r) { c->setMatrix(SkMatrix::Scale(2, 3)); },
    [](SkCanvas* c, skiatest::Reporter* r) { c->clipRect(kRect); },
    ...
    [](SkCanvas* c, skiatest::Reporter* r) { c->clear(kColor); },
    [](SkCanvas* c, skiatest::Reporter* r) {
...
DEF_TEST(Canvas_pdf, reporter) {
  for (const CanvasTest& test : kCanvasTests) {
    ...
    if (auto doc = SkPDF::MakeDocument(&outStream)) {
      SkCanvas* canvas = doc->beginPage(SkIntToScalar(kWidth), SkIntToScalar(kHeight));
      REPORTER_ASSERT(reporter, canvas);
...
DEF_TEST(PaintFilterCanvas_ConsistentState, reporter) {
  ...
  REPORTER_ASSERT(reporter, canvas.getTotalMatrix() == filterCanvas.getTotalMatrix());
  REPORTER_ASSERT(
      reporter, filterCanvas.getLocalClipBounds().contains(canvas.getLocalClipBounds()));
}
...
DEF_TEST(Canvas_LegacyColorBehavior, r) {
  sk_sp<SkColorSpace> cs = SkColorSpace::MakeRGB(SkNamedTransferFn::kSRGB, SkNamedGamut::kAdobeRGB);
...
class ZeroBoundsImageFilter : public SkImageFilter_Base {
 ...
 protected:
  sk_sp<SkSpecialImage> onFilterImage(const Context&, SkIPoint*) const override { return nullptr; }
  SkIRect onFilterNodeBounds(
      const SkIRect&, const SkMatrix&, MapDirection, const SkIRect* inputRect) const override {
