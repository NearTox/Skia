...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(PreinstantiatedProxyConversionTest, reporter, ctxInfo) {
...

  {
    ...
    sk_sp<GrSurfaceProxy> sProxy(make_texture(proxyProvider, desc, kBottomLeft_GrSurfaceOrigin));
    if (sProxy) {
      // Both RenderTarget and Texture
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(DefferredProxyConversionTest, reporter, ctxInfo) {
  ...
  const GrBackendFormat format =
      ctxInfo.grContext()->priv().caps()->getBackendFormatFromColorType(kRGBA_8888_SkColorType);
  {
