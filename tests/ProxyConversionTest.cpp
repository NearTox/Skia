...
static sk_sp<GrSurfaceProxy> make_wrapped_rt(
    GrProxyProvider* provider, GrGpu* gpu, skiatest::Reporter* reporter, const GrSurfaceDesc& desc,
...
static sk_sp<GrSurfaceProxy> make_offscreen_rt(
    GrProxyProvider* provider, const GrSurfaceDesc& desc, GrSurfaceOrigin origin) {
  return provider->testingOnly_createInstantiatedProxy(
      desc, GrRenderable::kYes, 1, origin, SkBackingFit::kExact, SkBudgeted::kYes,
      GrProtected::kNo);
}

static sk_sp<GrSurfaceProxy> make_texture(
    GrProxyProvider* provider, const GrSurfaceDesc& desc, GrRenderable renderable,
    GrSurfaceOrigin origin) {
  return provider->testingOnly_createInstantiatedProxy(
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(DefferredProxyConversionTest, reporter, ctxInfo) {
  ...
  const GrBackendFormat format =
      ctxInfo.grContext()->priv().caps()->getBackendFormatFromColorType(GrColorType::kRGBA_8888);
  {
    sk_sp<GrTextureProxy> proxy = proxyProvider->createProxy(
        format, desc, GrRenderable::kYes, 1, kBottomLeft_GrSurfaceOrigin, SkBackingFit::kApprox,
