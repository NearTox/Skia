...
static void test_search(skiatest::Reporter* reporter) {
  ...
  for (i = 0; i < 10000; i++) {
    ...
    if (index >= 0) {
      REPORTER_ASSERT(reporter, index < kSEARCH_COUNT && array[index] == value);
    } else {
      index = ~index;
...
DEF_TEST(SkZip, reporter) {
  ...
  // Check SkZip calls
  SkZip<uint16_t, const float, int, int, int> z{4, &A[0], &B[0], C.data(), D.data(), S.data()};
...

  {
    ...
    auto ptrs = z.data();
    REPORTER_ASSERT(reporter, ptrs == std::make_tuple(&A[0], &B[0], C.data(), D.data(), S.data()));
  }
