...
DEF_TEST(FontConfigInterface_MatchStyleNamedInstance, reporter) {
  ...
  static constexpr const char* family_names[]{
      "Noto Sans CJK JP", "Noto Sans CJK HK", "Noto Sans CJK SC", "Noto Sans CJK TC",
      "Noto Sans CJK KR"};
  static constexpr const struct Test {
    int weight;
    bool highBitsExpectation;
  } tests[]{
      {100, false}, {300, true}, {350, true}, {400, true}, {500, true}, {700, true}, {900, true},
  };
  ...
  for (auto&& font_name : family_names) {
    for (auto&& [weight, highBitsExpectation] : tests) {
      ...
      REPORTER_ASSERT(
          reporter, (resultIdentity.fTTCIndex >> 16 > 0) == highBitsExpectation,
      ...
      REPORTER_ASSERT(
          reporter, family_from_typeface == SkString(font_name),
          "Matched font's family name should match the request.");

      SkFontStyle intrinsic_style = typeface->fontStyle();
      REPORTER_ASSERT(
          reporter, intrinsic_style.weight() == weight,
          "Matched font's weight should match request.");
      if (intrinsic_style.weight() != weight) {
        ERRORF(
            reporter, "Matched font had weight: %d, expected %d, family: %s",
            intrinsic_style.weight(), weight, family_from_typeface.c_str());
      }
      ...
      REPORTER_ASSERT(
          reporter, coords.size() == 1, "The font must only have one axis, the weight axis.");

      REPORTER_ASSERT(
          reporter, coords[0].axis == SkSetFourByteTag('w', 'g', 'h', 't'),
          "The weight axis must be present and configured.");

      REPORTER_ASSERT(
          reporter, static_cast<int>(coords[0].value) == weight,
