...
bool check_gamma(uint32_t src, uint32_t dst, bool toSRGB, float error, uint32_t* expected) {
  ...
  for (int c = 0; c < 3; ++c) {
    ...
    if (dstComponent < SkScalarFloorToInt(lower) || dstComponent > SkScalarCeilToInt(upper)) {
      result = false;
    }
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ApplyGamma, reporter, ctxInfo) {
  ...
  for (auto toSRGB : {false, true}) {
    ...
    gammaPaint.setColorFilter(
        toSRGB ? SkColorFilters::LinearToSRGBGamma() : SkColorFilters::SRGBToLinearGamma());
    ...
    for (int y = 0; y < kH && !abort; ++y) {
      for (int x = 0; x < kW && !abort; ++x) {
        ...
        if (!check_gamma(s, r, toSRGB, error, &expected)) {
          ERRORF(
              reporter,
              "Expected dst %d,%d to contain 0x%08x "
              "from src 0x%08x and mode %s. Got %08x",
              x, y, expected, s, toSRGB ? "ToSRGB" : "ToLinear", r);
          abort = true;
          break;
        }
      }
