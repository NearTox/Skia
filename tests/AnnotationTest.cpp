...
DEF_TEST(Annotation_PdfLink, reporter) {
  ...
  SkRect r =
      SkRect::MakeXYWH(SkIntToScalar(72), SkIntToScalar(72), SkIntToScalar(288), SkIntToScalar(72));
  sk_sp<SkData> data(SkData::MakeWithCString("http://www.gooogle.com"));
  SkAnnotateRectWithURL(canvas, r, data.get());
...
DEF_TEST(Annotation_PdfDefineNamedDestination, reporter) {
  ...
  REPORTER_ASSERT(reporter, ContainsString(rawOutput, out->size(), "/example "));
}
...
DEF_TEST(Annotation_SvgLink, reporter) {
  ...
  REPORTER_ASSERT(
      reporter, ContainsString(rawOutput, out->size(), "a xlink:href=\"http://www.gooogle.com\""));
}
...
DEF_TEST(Annotation_SvgLinkNamedDestination, reporter) {
  ...
  REPORTER_ASSERT(
      reporter,
      ContainsString(
          rawOutput, out->size(), "a xlink:href=\"http://www.gooogle.com/#NamedDestination\""));
}
#endif
