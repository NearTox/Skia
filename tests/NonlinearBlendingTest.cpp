...
DEF_TEST(SkColorSpaceXformSteps_vs_skcms, r) {
  ...
  SkColor colors[] = {
      0xffff0000, 0x7fff0000, 0x7f7f0000, 0xff00ff00, 0x7f00ff00, 0x7f007f00,
  };

  for (auto color : colors) {
    auto bgra = skcms_PixelFormat_BGRA_8888, f32 = skcms_PixelFormat_RGBA_ffff;
    auto unpremul = skcms_AlphaFormat_Unpremul, premul = skcms_AlphaFormat_PremulAsEncoded;

    float via_skcms[4];
    skcms_Transform(
        &color, bgra, unpremul, &srgb_profile, &via_skcms, f32, premul, &dp3_profile, 1);

    SkColorSpaceXformSteps steps(srgb.get(), kUnpremul_SkAlphaType, dp3.get(), kPremul_SkAlphaType);
    float via_steps[4] = {
    ...
    skcms_Transform(
        via_steps, f32, premul, &dp3_profile, steps_to_skcms, f32, premul, &srgb_profile, 1);

    float skcms_to_steps[4] = {via_skcms[0], via_skcms[1], via_skcms[2], via_skcms[3]};
    SkColorSpaceXformSteps inv_steps(
        dp3.get(), kPremul_SkAlphaType, srgb.get(), kPremul_SkAlphaType);
