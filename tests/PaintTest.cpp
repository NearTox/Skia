...
DEF_TEST(Paint_copy, reporter) {
  ...
  // set a few pointers
  paint.setMaskFilter(
      SkMaskFilter::MakeBlur(kNormal_SkBlurStyle, SkBlurMask::ConvertRadiusToSigma(1)));
...
DEF_TEST(Paint_regression_cubic, reporter) {
  ...
  path.moveTo(460.2881309415525f, 303.250847066498f);
  path.cubicTo(
      463.36378422175284f, 302.1169735073363f, 456.32239330810046f, 304.720354932878f,
      453.15255460013304f, 305.788586869862f);
  ...
  SkScalar inset = paint.getStrokeJoin() == SkPaint::kMiter_Join ? paint.getStrokeWidth() * miter
  ...
  for (bool subpix : {false, true}) {
    ...
    for (auto hint : {SkFontHinting::kNone, SkFontHinting::kSlight, SkFontHinting::kNormal,
      ...
      for (auto size : {1.0f, 12.0f, 100.0f}) {
        ...
        auto nearly_eq = [](SkScalar a, SkScalar b) { return SkScalarAbs(a - b) < 0.000001f; };
