...
class Reporter : SkNoncopyable {
 public:
  ...
  void push(const SkString& message) { fContextStack.push_back(message); }
  void pop() { fContextStack.pop_back(); }
...
struct Test {
  Test(
      const char* name, bool needsGpu, bool needsGraphite, TestProc proc,
...
void RunWithGPUTestContexts(
    GrContextTestFn*, GrContextTypeFilterFn*, Reporter*, const GrContextOptions&);
...
#define DEF_GRAPHITE_TEST(name, reporter)                                             \
  ...
  static void test_graphite_##name(                                                   \
      skiatest::Reporter* reporter, const GrContextOptions& /*unused*/) {             \
    test_##name(reporter);                                                            \
  }                                                                                   \
  skiatest::TestRegistry name##TestRegistry(                                          \
...
#define DEF_GRAPHITE_TEST_FOR_CONTEXTS(name, reporter, graphite_context)                       \
  ...
  static void test_graphite_contexts_##name(                                                   \
      skiatest::Reporter* _reporter, const GrContextOptions& /*unused*/) {                     \
    skiatest::graphite::RunWithGraphiteTestContexts(test_##name, _reporter);                   \
  }                                                                                            \
  skiatest::TestRegistry name##TestRegistry(                                                   \
      skiatest::Test(#name, /*gpu=*/false, /*graphite=*/true, test_graphite_contexts_##name)); \
...
#define DEF_CONDITIONAL_GPUTEST_FOR_CONTEXTS(                                                 \
    name, context_filter, reporter, context_info, options_filter, condition)                  \
  static void test_##name(skiatest::Reporter*, const sk_gpu_test::ContextInfo&);              \
  static void test_gpu_contexts_##name(                                                       \
      skiatest::Reporter* reporter, const GrContextOptions& options) {                        \
    skiatest::RunWithGPUTestContexts(test_##name, context_filter, reporter, options);         \
  }                                                                                           \
  skiatest::TestRegistry name##TestRegistry(                                                  \
      skiatest::Test(                                                                         \
          #name, /*gpu=*/true, /*graphite=*/false, test_gpu_contexts_##name, options_filter), \
      condition);                                                                             \
...
#define DEF_CONDITIONAL_GPUTEST_FOR_ALL_CONTEXTS(name, reporter, context_info, condition) \
  DEF_CONDITIONAL_GPUTEST_FOR_CONTEXTS(name, nullptr, reporter, context_info, nullptr, condition)
#define DEF_CONDITIONAL_GPUTEST_FOR_RENDERING_CONTEXTS(name, reporter, context_info, condition) \
  DEF_CONDITIONAL_GPUTEST_FOR_CONTEXTS(                                                         \
      name, sk_gpu_test::GrContextFactory::IsRenderingContext, reporter, context_info, nullptr, \
      condition)

#define DEF_GPUTEST_FOR_CONTEXTS(name, context_filter, reporter, context_info, options_filter) \
  DEF_CONDITIONAL_GPUTEST_FOR_CONTEXTS(                                                        \
      name, context_filter, reporter, context_info, options_filter, true)
...
#define DEF_GPUTEST_FOR_ALL_GL_CONTEXTS(name, reporter, context_info) \
  DEF_GPUTEST_FOR_CONTEXTS(name, &skiatest::IsGLContextType, reporter, context_info, nullptr)
#define DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(name, reporter, context_info) \
  DEF_GPUTEST_FOR_CONTEXTS(                                                 \
      name, &skiatest::IsRenderingGLContextType, reporter, context_info, nullptr)
#define DEF_GPUTEST_FOR_MOCK_CONTEXT(name, reporter, context_info) \
  DEF_GPUTEST_FOR_CONTEXTS(name, &skiatest::IsMockContextType, reporter, context_info, nullptr)
#define DEF_GPUTEST_FOR_VULKAN_CONTEXT(name, reporter, context_info) \
  DEF_GPUTEST_FOR_CONTEXTS(name, &skiatest::IsVulkanContextType, reporter, context_info, nullptr)
#define DEF_GPUTEST_FOR_METAL_CONTEXT(name, reporter, context_info) \
  DEF_GPUTEST_FOR_CONTEXTS(name, &skiatest::IsMetalContextType, reporter, context_info, nullptr)
#define DEF_GPUTEST_FOR_D3D_CONTEXT(name, reporter, context_info) \
  DEF_GPUTEST_FOR_CONTEXTS(name, &skiatest::IsDirect3DContextType, reporter, context_info, nullptr)
#define DEF_GPUTEST_FOR_DAWN_CONTEXT(name, reporter, context_info) \
  DEF_GPUTEST_FOR_CONTEXTS(name, &skiatest::IsDawnContextType, reporter, context_info, nullptr)
