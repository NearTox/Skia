...
static sk_sp<GrSurfaceProxy> make_deferred(
  ...
  return proxyProvider->createProxy(
      format, desc, p.fRenderable, p.fSampleCnt, p.fOrigin, p.fFit, p.fBudgeted, GrProtected::kNo);
}
...
static sk_sp<GrSurfaceProxy> make_backend(
  ...
  *backendTex = context->createBackendTexture(
      p.fSize, p.fSize, skColorType, SkColors::kTransparent, GrMipMapped::kNo, GrRenderable::kNo,
      GrProtected::kNo);
  ...
  return proxyProvider->wrapBackendTexture(
      *backendTex, p.fColorType, p.fOrigin, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo,
      kRead_GrIOType);
}
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ResourceAllocatorTest, reporter, ctxInfo) {
  ...
  int k2 =
      ctxInfo.grContext()->priv().caps()->getRenderTargetSampleCount(2, kRGBA_8888_GrPixelConfig);
  int k4 =
      ctxInfo.grContext()->priv().caps()->getRenderTargetSampleCount(4, kRGBA_8888_GrPixelConfig);
...
sk_sp<GrSurfaceProxy> make_lazy(
    GrProxyProvider* proxyProvider, const GrCaps* caps, const ProxyParams& p, bool deinstantiate) {
  GrPixelConfig config = GrColorTypeToPixelConfig(p.fColorType);
  ...
  auto callback = [fit, desc, p](GrResourceProvider* resourceProvider) {
    ...
    if (fit == SkBackingFit::kApprox) {
      ...
      texture = resourceProvider->createTexture(
          desc, p.fRenderable, p.fSampleCnt, SkBudgeted::kNo, GrProtected::kNo,
          GrResourceProvider::Flags::kNoPendingIO);
    }
  ...
  return proxyProvider->createLazyProxy(
      callback, format, desc, p.fRenderable, p.fSampleCnt, p.fOrigin, GrMipMapped::kNo, flags,
      p.fFit, p.fBudgeted, GrProtected::kNo, lazyType);
}
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ResourceAllocatorOverBudgetTest, reporter, ctxInfo) {
  ...
  const ProxyParams params = {
      ...
      1,  kTopLeft_GrSurfaceOrigin, SkBudgeted::kYes};
