...
static GrSurfaceProxy* make_deferred(
...
static GrSurfaceProxy* make_backend(
    GrContext* context, const ProxyParams& p, GrBackendTexture* backendTex) {
  GrProxyProvider* proxyProvider = context->priv().proxyProvider();

  *backendTex = context->createBackendTexture(
      p.fSize, p.fSize, p.fColorType, GrMipMapped::kNo, GrRenderable::kNo);
  if (!backendTex->isValid()) {
    return nullptr;
  }

  auto tmp = proxyProvider->wrapBackendTexture(
      *backendTex, p.fOrigin, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, kRead_GrIOType);
  if (!tmp) {
    return nullptr;
  }
  GrSurfaceProxy* ret = tmp.release();
...
static void overlap_test(
    skiatest::Reporter* reporter, GrResourceProvider* resourceProvider,
    GrResourceCache* resourceCache, GrSurfaceProxy* p1, GrSurfaceProxy* p2, bool expectedResult) {
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ResourceAllocatorTest, reporter, ctxInfo) {
...

  {
    ...
    TestCase t[1] = {{{64, kNotRT, kRGBA, kE, 0, kTL, kNotB},
                      {64, kNotRT, kRGBA, kE, 0, kTL, kNotB},
                      kDontShare}};
...
sk_sp<GrSurfaceProxy> make_lazy(
  ...
  auto callback = [fit, desc](GrResourceProvider* resourceProvider) {
    ...
    if (fit == SkBackingFit::kApprox) {
      texture =
          resourceProvider->createApproxTexture(desc, GrResourceProvider::Flags::kNoPendingIO);
    } else {
  ...
  return proxyProvider->createLazyProxy(
      callback, format, desc, p.fOrigin, GrMipMapped::kNo, flags, p.fFit, p.fBudgeted, lazyType);
}
