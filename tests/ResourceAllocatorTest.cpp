...
struct ProxyParams {
  ...
  enum Kind { kDeferred, kBackend, kFullyLazy, kLazy, kInstantiated };
  Kind fKind;
...
static sk_sp<GrSurfaceProxy> make_fully_lazy(
    GrProxyProvider* proxyProvider, const GrCaps* caps, const ProxyParams& p) {
  const GrBackendFormat format = caps->getDefaultBackendFormat(p.fColorType, p.fRenderable);
  auto cb = [p](GrResourceProvider* provider, const GrSurfaceProxy::LazySurfaceDesc& desc) {
    auto tex = provider->createTexture(
        {p.fSize, p.fSize}, desc.fFormat, desc.fTextureType, desc.fRenderable, desc.fSampleCnt,
  ...
  return GrProxyProvider::MakeFullyLazyProxy(
      std::move(cb), format, p.fRenderable, p.fSampleCnt, GrProtected::kNo, *caps,
      GrSurfaceProxy::UseAllocator::kYes);
}

static sk_sp<GrSurfaceProxy> make_lazy(
    GrProxyProvider* proxyProvider, const GrCaps* caps, const ProxyParams& p) {
  const GrBackendFormat format = caps->getDefaultBackendFormat(p.fColorType, p.fRenderable);
  auto cb = [](GrResourceProvider* provider, const GrSurfaceProxy::LazySurfaceDesc& desc) {
    auto tex = provider->createTexture(
        desc.fDimensions, desc.fFormat, desc.fTextureType, desc.fRenderable, desc.fSampleCnt,
        desc.fMipmapped, desc.fBudgeted, desc.fProtected,
        /*label=*/"ResourceAllocatorTest_Lazy");
    return GrSurfaceProxy::LazyCallbackResult(std::move(tex));
  };
  return proxyProvider->createLazyProxy(
      std::move(cb), format, {p.fSize, p.fSize}, GrMipmapped::kNo, GrMipmapStatus::kNotAllocated,
      GrInternalSurfaceFlags::kNone, p.fFit, p.fBudgeted, GrProtected::kNo,
      GrSurfaceProxy::UseAllocator::kYes);
}
...
static sk_sp<GrSurfaceProxy> make_proxy(GrDirectContext* dContext, const ProxyParams& p) {
  ...
  switch (p.fKind) {
    case ProxyParams::kDeferred: proxy = make_deferred(proxyProvider, caps, p); break;
    case ProxyParams::kBackend: proxy = make_backend(dContext, p); break;
    case ProxyParams::kFullyLazy: proxy = make_fully_lazy(proxyProvider, caps, p); break;
    case ProxyParams::kLazy: proxy = make_lazy(proxyProvider, caps, p); break;
...
static void overlap_test(
    skiatest::Reporter* reporter, GrDirectContext* dContext, sk_sp<GrSurfaceProxy> p1,
    sk_sp<GrSurfaceProxy> p2, bool expectedResult) {
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ResourceAllocatorTest, reporter, ctxInfo) {
  ...
  for (size_t i = 0; i < SK_ARRAY_COUNT(nonOverlappingTests); i++) {
    ...
    reporter->push(SkStringPrintf("case %d", SkToInt(i)));
    non_overlap_test(reporter, dContext, std::move(p1), std::move(p2), test.fExpectation);
    reporter->pop();
  }
}
...
static void draw(GrRecordingContext* rContext) {
  ...
  sk_sp<SkSurface> s = SkSurface::MakeRenderTarget(
      rContext, SkBudgeted::kYes, ii, 1, kTopLeft_GrSurfaceOrigin, nullptr);
...
static void memory_budget_test(
    skiatest::Reporter* reporter, GrDirectContext* dContext, const TestCase& test) {
  // Reset cache.
  ...
  REPORTER_ASSERT(
      reporter, expectedPurgeableBytes == cache->getPurgeableBytes(), "%zu",
  ...
  REPORTER_ASSERT(reporter, expectedUnpurgeableBytes == unpurgeableBytes, "%zu", unpurgeableBytes);
  ...
  for (auto& interval : test.fIntervals) {
    ...
    alloc.addInterval(
        interval.fProxy.get(), interval.fStart, interval.fEnd,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ResourceAllocatorMemoryBudgetTest, reporter, ctxInfo) {
  ...
  TestCase tests[] = {
      ...
      {"purge 4",
       ...
       kRGBA64Bytes,
       /* purgeable */
       {kProxy32Instantiated, kProxy32Instantiated, kProxy32Instantiated, kProxy32Instantiated},
       /* unpurgeable */ {},
       {{kProxy64, 0, 2}}},
