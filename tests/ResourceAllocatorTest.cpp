...
static sk_sp<GrSurfaceProxy> make_backend(
    GrContext* context, const ProxyParams& p, GrBackendTexture* backendTex) {
  ...
  CreateBackendTexture(
      context, backendTex, p.fSize, p.fSize, skColorType, SkColors::kTransparent, GrMipMapped::kNo,
      GrRenderable::kNo);
...
static void overlap_test(
    skiatest::Reporter* reporter, GrResourceProvider* resourceProvider, sk_sp<GrSurfaceProxy> p1,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ResourceAllocatorTest, reporter, ctxInfo) {
  ...
  TestCase gNonOverlappingTests[] = {
      ...
      {{64, kNotRT, kRGBA, kA, 1, kNotB}, {64, kNotRT, kRGBA, kA, 1, kNotB}, kConditionallyShare},
      // diffs sizes but still approx
      {{64, kRT, kRGBA, kA, 1, kNotB}, {50, kRT, kRGBA, kA, 1, kNotB}, kShare},
      {{64, kNotRT, kRGBA, kA, 1, kNotB}, {50, kNotRT, kRGBA, kA, 1, kNotB}, kConditionallyShare},
      // sames sizes but exact
      {{64, kRT, kRGBA, kE, 1, kNotB}, {64, kRT, kRGBA, kE, 1, kNotB}, kShare},
      {{64, kNotRT, kRGBA, kE, 1, kNotB}, {64, kNotRT, kRGBA, kE, 1, kNotB}, kConditionallyShare},
      //----------------------------------------------------------------------------------------
  ...
  for (auto test : gNonOverlappingTests) {
    ...
    non_overlap_test(reporter, resourceProvider, std::move(p1), std::move(p2), test.fExpectation);
  }

  {
    ...
    non_overlap_test(reporter, resourceProvider, std::move(p1), std::move(p2), t[0].fExpectation);
...
static void draw(GrContext* context) {
  ...
  sk_sp<SkSurface> s = SkSurface::MakeRenderTarget(
      context, SkBudgeted::kYes, ii, 1, kTopLeft_GrSurfaceOrigin, nullptr);
...
sk_sp<GrSurfaceProxy> make_lazy(
    GrProxyProvider* proxyProvider, const GrCaps* caps, const ProxyParams& p) {
  ...
  auto callback = [](GrResourceProvider* resourceProvider,
    ...
    if (desc.fFit == SkBackingFit::kApprox) {
      texture = resourceProvider->createApproxTexture(
          desc.fDimensions, desc.fFormat, desc.fRenderable, desc.fSampleCnt, desc.fProtected);
    } else {
      texture = resourceProvider->createTexture(
          desc.fDimensions, desc.fFormat, desc.fRenderable, desc.fSampleCnt, desc.fMipMapped,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ResourceAllocatorOverBudgetTest, reporter, ctxInfo) {
  ...
  const ProxyParams params = {64, GrRenderable::kNo, GrColorType::kRGBA_8888, SkBackingFit::kExact,
                              1,  SkBudgeted::kYes};
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ResourceAllocatorCurOpsTaskIndexTest, reporter, ctxInfo) {
  GrContext* context = ctxInfo.grContext();
  const GrCaps* caps = context->priv().caps();
  GrProxyProvider* proxyProvider = context->priv().proxyProvider();
  GrResourceProvider* resourceProvider = context->priv().resourceProvider();
  ...
  GrBackendTexture backEndTex;
  sk_sp<GrSurfaceProxy> proxyWrapped = make_backend(ctxInfo.grContext(), params, &backEndTex);
  if (!proxyWrapped) {
    return;
  }
  ...
  sk_sp<GrSurfaceProxy> proxyWrapped2 = make_backend(ctxInfo.grContext(), params, &backEndTex2);
  if (!proxyWrapped2) {
    cleanup_backend(ctxInfo.grContext(), backEndTex);
    return;
  }
