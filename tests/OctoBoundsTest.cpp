...
static void validate_octo_bounds(
  ...
  octoBounds.validateBoundsAreTight(
      [reporter](bool cond, const char* file, int line, const char* code) {
        if (!cond) {
...
static void test_octagon(Reporter* reporter, SkRandom* rand, float l, float t, float r, float b) {
  for (int i = 0; i < 20; ++i) {
    ...
    // Shuffle.
    for (int i = 0; i < 10; ++i) {
      std::swap(growOutDiagonals[rand->nextRangeU(0, 3)], growOutDiagonals[rand->nextRangeU(0, 3)]);
    }
    ...
    for (int j = 0; j < 20; ++j) {
      ...
      if (!octoBoundsClipped.clip(clipRect)) {
        ...
        REPORTER_ASSERT(
            reporter, GrOctoBounds::Get_x45(ir, it) <= l45 + kEpsilon ||
