...
static void test_stroke(
    skiatest::Reporter* r, GrDirectContext* ctx, GrMeshDrawOp::Target* target, const SkPath& path,
    SkRandom& rand) {
  ...
  for (auto join : {SkPaint::kMiter_Join, SkPaint::kRound_Join}) {
    ...
    for (int i = 0; i < 16; ++i) {
      ...
      auto op = GrOp::Make<GrStrokeIndirectOp>(
          ctx, GrAAType::kMSAA, SkMatrix::Scale(scale, scale), path, stroke, GrPaint());
...
DEF_TEST(tessellate_GrStrokeIndirectOp, r) {
  ...
  for (int i = 0; i < (1 << 8); ++i) {
    path.moveTo((i >> 0) & 1, (i >> 1) & 1);
    path.cubicTo(
        (i >> 2) & 1, (i >> 3) & 1, (i >> 4) & 1, (i >> 5) & 1, (i >> 6) & 1, (i >> 7) & 1);
    path.close();
  }
  test_stroke(r, ctx.get(), target.get(), path, rand);

  {
    ...
    test_stroke(
        r, ctx.get(), target.get(), SkPath().moveTo(pts[0]).cubicTo(pts[1], pts[2], pts[3]).close(),
        rand);
  }
  ...
  for (int j = 0; j < 50; ++j) {
    ...
    for (int i = 0; i < 100; ++i) {
      switch (rand.nextRangeU(0, 4)) {
        case 0: path.lineTo(rand.nextF(), rand.nextF()); break;
        case 1: path.quadTo(rand.nextF(), rand.nextF(), rand.nextF(), rand.nextF()); break;
        case 2:
        case 3:
        case 4:
          path.cubicTo(
              rand.nextF(), rand.nextF(), rand.nextF(), rand.nextF(), rand.nextF(), rand.nextF());
          break;
      ...
      if (i % 19 == 0) {
        switch (i / 19 % 4) {
          case 0: break;
          case 1: path.lineTo(startPoint); break;
          case 2: path.quadTo(SkPoint::Make(1.1f, 1.1f), startPoint); break;
          case 3:
            path.cubicTo(SkPoint::Make(1.1f, 1.1f), SkPoint::Make(1.1f, 1.1f), startPoint);
            break;
        }
        path.close();
...
static SkPoint get_contour_closing_control_point(
    SkPathPriv::RangeIter iter, const SkPathPriv::RangeIter& end) {
  auto [verb, p, w] = *iter;
  ...
  for (++iter; iter != end; ++iter) {
    auto [verb, p, w] = *iter;
    switch (verb) {
      case SkPathVerb::kMove: return startPoint;
      ...
      case SkPathVerb::kClose: return (p[0] == startPoint) ? lastControlPoint : p[0];
    }
  }
  return startPoint;
}

static bool check_resolve_level(
    skiatest::Reporter* r, float numCombinedSegments, int8_t actualLevel, float tolerance,
  ...
  if ((actualLevel > expectedLevel &&
    ...
    if (printError) {
      ERRORF(
          r, "expected %f segments => resolveLevel=%i (got %i)\n", numCombinedSegments,
...
static bool check_first_resolve_levels(
    skiatest::Reporter* r, const SkTArray<float>& firstNumSegments, int8_t** nextResolveLevel,
    float tolerance) {
...
void GrStrokeIndirectOp::verifyPrePrepareResolveLevels(
    skiatest::Reporter* r, GrMeshDrawOp::Target* target) {
      ...
      switch (verb) {
        ...
        case SkPathVerb::kLine:
          ...
          if (fStroke.getJoin() == SkPaint::kRound_Join) {
            float rotation =
                SkMeasureAngleBetweenVectors(pts[0] - lastControlPoint, pts[1] - pts[0]);
            float numSegments = rotation * tolerances.fNumRadialSegmentsPerRadian;
            if (isFirstStroke) {
              firstNumSegments.push_back(numSegments);
            } else if (!check_resolve_level(r, numSegments, *nextResolveLevel++, tolerance)) {
              return;
            }
          }
        ...
        case SkPathVerb::kQuad: {
          ...
          float numParametricSegments =
              (hasCusp) ? 0 : GrWangsFormula::quadratic(tolerances.fParametricIntolerance, pts);
          float rotation = (hasCusp) ? 0 : SkMeasureQuadRotation(pts);
          if (fStroke.getJoin() == SkPaint::kRound_Join) {
            SkVector controlPoint = (pts[0] == pts[1]) ? pts[2] : pts[1];
            rotation +=
                SkMeasureAngleBetweenVectors(pts[0] - lastControlPoint, controlPoint - pts[0]);
          }
        ...
        case SkPathVerb::kCubic: {
          ...
          lastControlPoint = (pts[3] == pts[2]) ? (pts[2] == pts[1]) ? pts[0] : pts[1] : pts[2];
          isFirstStroke = false;
          break;
        }
        case SkPathVerb::kConic: SkUNREACHABLE;
        case SkPathVerb::kClose:
          if (pts[0] != startPoint) {
            ...
            if (fStroke.getJoin() == SkPaint::kRound_Join) {
              ...
              float rotation =
                  SkMeasureAngleBetweenVectors(pts[0] - lastControlPoint, startPoint - pts[0]);
              if (!check_resolve_level(
                      r, rotation * tolerances.fNumRadialSegmentsPerRadian, *nextResolveLevel++,
                      tolerance)) {
                return;
              }
            }
...
void GrStrokeIndirectOp::verifyPrepareBuffers(skiatest::Reporter* r, GrMeshDrawOp::Target* target) {
  ...
  for (int i = 0; i < fDrawIndirectCount; ++i) {
    ...
    for (unsigned j = 0; j < indirect->fInstanceCount; ++j) {
      ...
      float numParametricSegments = GrWangsFormula::cubic(tolerances.fParametricIntolerance, p);
