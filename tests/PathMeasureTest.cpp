...
static void test_small_segment3() {
  SkPath path;
  const SkPoint pts[] = {
      {0, 0}, {100000000000.0f, 100000000000.0f}, {0, 0}, {10, 10}, {10, 10}, {0, 0}, {10, 10}};
...
static void test_small_segment2() {
  SkPath path;
  const SkPoint pts[] = {
      {0, 0}, {100000000000.0f, 100000000000.0f}, {0, 0}, {10, 10}, {0, 0},
  };
...
DEF_TEST(PathMeasure, reporter) {
  ...
  REPORTER_ASSERT(reporter, SkScalarNearlyEqual(position.fX, -SK_ScalarHalf, 0.0001f));
  ...
  path.cubicTo(SK_Scalar1, SK_Scalar1 * 2, SK_Scalar1, SK_Scalar1 * 2, SK_Scalar1, SK_Scalar1 * 2);
  path.cubicTo(
      SK_Scalar1 * 2, SK_Scalar1 * 2, SK_Scalar1 * 3, SK_Scalar1 * 2, SK_Scalar1 * 4,
      SK_Scalar1 * 2);
  meas.setPath(&path, false);
  length = meas.getLength();
  ...
  REPORTER_ASSERT(reporter, SkScalarNearlyEqual(position.fX, SK_Scalar1, 0.0001f));
  REPORTER_ASSERT(reporter, SkScalarNearlyEqual(position.fY, 1.5f));
  ...
  REPORTER_ASSERT(reporter, SkScalarNearlyEqual(position.fX, 2.5f, 0.0001f));
  REPORTER_ASSERT(reporter, SkScalarNearlyEqual(position.fY, 2.0f, 0.0001f));
  REPORTER_ASSERT(reporter, tangent.fX == SK_Scalar1);
  REPORTER_ASSERT(reporter, tangent.fY == 0);
  ...
  REPORTER_ASSERT(reporter, length == SK_Scalar1);
  REPORTER_ASSERT(reporter, meas.getPosTan(SK_ScalarHalf, &position, &tangent));
  REPORTER_ASSERT(reporter, SkScalarNearlyEqual(position.fX, 1.5f, 0.0001f));
  REPORTER_ASSERT(reporter, SkScalarNearlyEqual(position.fY, 2.0f, 0.0001f));
  REPORTER_ASSERT(reporter, tangent.fX == -SK_Scalar1);
  REPORTER_ASSERT(reporter, tangent.fY == 0);
...
static void test_90_degrees(
    sk_sp<SkContourMeasure> cm, SkScalar radius, skiatest::Reporter* reporter) {
  SkPoint pos;
  SkVector tan;
