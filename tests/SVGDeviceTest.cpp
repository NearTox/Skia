...
static std::unique_ptr<SkCanvas> MakeDOMCanvas(SkDOM* dom, uint32_t flags = 0) {
  auto svgDevice = SkSVGDevice::Make(
      SkISize::Make(100, 100), skstd::make_unique<SkXMLParserWriter>(dom->beginParsing()), flags);
  return svgDevice ? skstd::make_unique<SkCanvas>(svgDevice) : nullptr;
}

namespace {

void check_text_node(
    skiatest::Reporter* reporter, const SkDOM& dom, const SkDOM::Node* root, const SkPoint& offset,
    unsigned scalarsPerPos, const char* txt, const char* expected) {
  if (root == nullptr) {
...
void test_whitespace_pos(skiatest::Reporter* reporter, const char* txt, const char* expected) {
  ...
  {
    MakeDOMCanvas(&dom)->drawSimpleText(
        txt, len, SkTextEncoding::kUTF8, offset.x(), offset.y(), font, paint);
  }
...
void SetImageShader(
    SkPaint* paint, int imageWidth, int imageHeight, SkTileMode xTile, SkTileMode yTile) {
...
bool FindImageShaderNodes(
    skiatest::Reporter* reporter, const SkDOM* dom, const SkDOM::Node* root,
    const SkDOM::Node** patternOut, const SkDOM::Node** imageOut, const SkDOM::Node** rectOut) {
...
void ImageShaderTestSetup(
    SkDOM* dom, SkPaint* paint, int imageWidth, int imageHeight, int rectWidth, int rectHeight,
    SkTileMode xTile, SkTileMode yTile) {
...
DEF_TEST(SVGDevice_image_shader_norepeat, reporter) {
  ...
  ImageShaderTestSetup(
      &dom, &paint, imageWidth, imageHeight, rectWidth, rectHeight, SkTileMode::kClamp,
      SkTileMode::kClamp);
...
DEF_TEST(SVGDevice_image_shader_tilex, reporter) {
  ...
  int rectWidth = 10, rectHeight = 10;
  ImageShaderTestSetup(
      &dom, &paint, imageWidth, imageHeight, rectWidth, rectHeight, SkTileMode::kRepeat,
      SkTileMode::kClamp);
...
DEF_TEST(SVGDevice_image_shader_tiley, reporter) {
  ...
  ImageShaderTestSetup(
      &dom, &paint, imageNodeWidth, imageNodeHeight, rectNodeWidth, rectNodeHeight,
      SkTileMode::kClamp, SkTileMode::kRepeat);
...
DEF_TEST(SVGDevice_ColorFilters, reporter) {
  ...
  REPORTER_ASSERT(reporter, strcmp(dom.findAttr(floodElement, "flood-color"), "rgb(255,0,0)") == 0);
