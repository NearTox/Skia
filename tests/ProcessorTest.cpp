...
void test_draw_op(
    ...
    GrSurfaceProxyView inputDataView, SkAlphaType inputAlphaType) {
  GrPaint paint;
  paint.addColorFragmentProcessor(GrTextureEffect::Make(std::move(inputDataView), inputAlphaType));
...
void render_fp(
  ...
  rtc->readPixels(
      SkImageInfo::Make(
          inputDataView.proxy()->dimensions(), kRGBA_8888_SkColorType, kPremul_SkAlphaType),
      buffer, 0, {0, 0});
}

/** Initializes the two test texture proxies that are available to the FP test factories. */
bool init_test_textures(
    GrResourceProvider* resourceProvider, GrRecordingContext* context, SkRandom* random,
...
GrSurfaceProxyView make_input_texture(
    GrRecordingContext* context, int width, int height, SkScalar delta) {
...
bool log_texture_view(GrContext* context, GrSurfaceProxyView src, SkString* dst) {
  ...
  auto sContext = GrSurfaceContext::Make(
      context, std::move(src), GrColorType::kRGBA_8888, kLogAlphaType, nullptr);
  SkBitmap bm;
...
#  endif
    for (int j = 0; j < timesToInvokeFactory; ++j) {
      ...
      if (fp->compatibleWithCoverageAsAlpha()) {
        ...
        render_fp(
            context, rtc.get(), fp->clone(), inputTexture2, kPremul_SkAlphaType, readData2.get());
        render_fp(
            context, rtc.get(), fp->clone(), inputTexture3, kPremul_SkAlphaType, readData3.get());
      }

      // Draw base frame last so that rtc holds the original FP behavior if we need to
      // dump the image to the log.
      render_fp(
          context, rtc.get(), fp->clone(), inputTexture1, kPremul_SkAlphaType, readData1.get());
            ...
            if (rDiff > kTol || gDiff > kTol || bDiff > kTol || aDiff > kTol) {
              if (constMessage.isEmpty()) {
                ...
                constMessage.printf(
                    ...
                    "expected(%f, %f, %f, %f)",
                    fp->name(), std::max(rDiff, std::max(gDiff, std::max(bDiff, aDiff))), kTol,
...
static void describe_fp_children(
    const GrFragmentProcessor& fp, std::string indent, SkString* text) {
...
#  endif
  ...
  for (int i = 0; i < GrFragmentProcessorTestFactory::Count(); ++i) {
    ...
    for (int j = 0; j < kTimesToInvokeFactory; ++j) {
      ...
      REPORTER_ASSERT(
          reporter, !strcmp(fp->name(), clone->name()), "%s\n", describe_fp(*fp).c_str());
      REPORTER_ASSERT(
          reporter, fp->compatibleWithCoverageAsAlpha() == clone->compatibleWithCoverageAsAlpha(),
          "%s\n", describe_fp(*fp).c_str());
      REPORTER_ASSERT(reporter, fp->isEqual(*clone), "%s\n", describe_fp(*fp).c_str());
      REPORTER_ASSERT(
          reporter, fp->preservesOpaqueInput() == clone->preservesOpaqueInput(), "%s\n",
          describe_fp(*fp).c_str());
      REPORTER_ASSERT(
          reporter,
          ...
          "%s\n", describe_fp(*fp).c_str());
      REPORTER_ASSERT(
          reporter, fp->numChildProcessors() == clone->numChildProcessors(), "%s\n",
          describe_fp(*fp).c_str());
      REPORTER_ASSERT(
          reporter, fp->usesLocalCoords() == clone->usesLocalCoords(), "%s\n",
          describe_fp(*fp).c_str());
      // Draw with original and read back the results.
      render_fp(
          context, rtc.get(), std::move(fp), inputTexture, kPremul_SkAlphaType, readData1.get());

      // Draw with clone and read back the results.
      render_fp(
          context, rtc.get(), std::move(clone), inputTexture, kPremul_SkAlphaType, readData2.get());
