...
class TestOp : public GrMeshDrawOp {
 public:
  ...
  void visitProxies(const GrVisitProxyFunc& func) const override { fProcessors.visitProxies(func); }
  ...
  GrProcessorSet::Analysis finalize(
    ...
    return fProcessors.finalize(
        kUnknownColor, GrProcessorAnalysisCoverage::kNone, clip, &GrUserStencilSettings::kUnused,
 ...
 private:
  ...
  std::unique_ptr<ProgramImpl> onMakeProgramImpl() const override {
    class Impl : public ProgramImpl {
     public:
      void emitCode(EmitArgs& args) override { args.fFragBuilder->codeAppendf("return half4(1);"); }
...
DEF_GPUTEST_FOR_ALL_CONTEXTS(ProcessorRefTest, reporter, ctxInfo) {
  ...
  const GrBackendFormat format =
      dContext->priv().caps()->getDefaultBackendFormat(GrColorType::kRGBA_8888, GrRenderable::kNo);
  skgpu::Swizzle swizzle = dContext->priv().caps()->getReadSwizzle(format, GrColorType::kRGBA_8888);
...
static DEFINE_bool(
    randomProcessorTest, false, "Use non-deterministic seed for random processor tests?");
static DEFINE_int(
    processorSeed, 0,
...
static void render_fp(
    GrDirectContext* dContext, skgpu::v1::SurfaceDrawContext* sdc,
    std::unique_ptr<GrFragmentProcessor> fp, GrColor* outBuffer) {
 ...
 public:
  ...
  bool init() {
...

    {
      ...
      for (int y = 0; y < kTestTextureSize; ++y) {
        for (int x = 0; x < kTestTextureSize; ++x) {
          rgbaData[kTestTextureSize * y + x] =
              input_texel_color(random.nextULessThan(256), random.nextULessThan(256), 0.0f);
        }
      }

      SkImageInfo ii = SkImageInfo::Make(
          kTestTextureSize, kTestTextureSize, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
      SkBitmap bitmap;
      ...
      fTestViews[0] =
          GrProcessorTestData::ViewInfo{view, GrColorType::kRGBA_8888, kPremul_SkAlphaType};
    }

    {
      ...
      SkImageInfo ii = SkImageInfo::Make(
          kTestTextureSize, kTestTextureSize, kAlpha_8_SkColorType, kPremul_SkAlphaType);
      SkBitmap bitmap;
      bitmap.installPixels(
      ...
      fTestViews[1] =
          GrProcessorTestData::ViewInfo{view, GrColorType::kAlpha_8, kPremul_SkAlphaType};
    }
  ...
  std::unique_ptr<GrFragmentProcessor> make(
      int type, int randomTreeDepth, std::unique_ptr<GrFragmentProcessor> inputFP) {
    ...
    SkRandom random{fRandomSeed};
    GrProcessorTestData testData{&random,         fContext,
                                 randomTreeDepth, SK_ARRAY_COUNT(fTestViews),
                                 fTestViews,      std::move(inputFP)};
    return GrFragmentProcessorTestFactory::MakeIdx(type, &testData);
  }

  std::unique_ptr<GrFragmentProcessor> make(
      int type, int randomTreeDepth, GrSurfaceProxyView view,
...
static GrSurfaceProxyView make_input_texture(
    GrRecordingContext* context, int width, int height, GrColor* pixel) {
...
static bool log_texture_view(GrDirectContext* dContext, GrSurfaceProxyView src, SkString* dst) {
  SkImageInfo ii =
      SkImageInfo::Make(src.proxy()->dimensions(), kRGBA_8888_SkColorType, kLogAlphaType);
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(ProcessorOptimizationValidationTest, reporter, ctxInfo) {
  ...
  auto sdc = skgpu::v1::SurfaceDrawContext::Make(
      context, GrColorType::kRGBA_8888, nullptr, SkBackingFit::kExact, {kRenderSize, kRenderSize},
      SkSurfaceProps());
  ...
  std::vector<GrColor> inputPixels2 = make_input_pixels(kRenderSize, kRenderSize, 1 * kInputDelta);
  std::vector<GrColor> inputPixels3 = make_input_pixels(kRenderSize, kRenderSize, 2 * kInputDelta);
...
#  endif
    ...
    for (int trial = 0;; ++trial) {
      ...
      if (trial >= kMinimumTrials) {
        bool moreTrialsNeeded =
            (optimizedForOpaqueInput > 0 && optimizedForOpaqueInput < kExpectedSuccesses) ||
            (optimizedForCoverageAsAlpha > 0 && optimizedForCoverageAsAlpha < kExpectedSuccesses) ||
      ...
      if (fp->compatibleWithCoverageAsAlpha()) {
        ...
        render_fp(
            context, sdc.get(), fpGenerator.make(i, /*randomTreeDepth=*/1, inputTexture2),
            readData2.data());
        render_fp(
            context, sdc.get(), fpGenerator.make(i, /*randomTreeDepth=*/1, inputTexture3),
            readData3.data());
      ...
      render_fp(
          context, sdc.get(), fpGenerator.make(i, /*randomTreeDepth=*/1, inputTexture1),
          readData1.data());
            ...
            if (rDiff > kTol || gDiff > kTol || bDiff > kTol || aDiff > kTol) {
              if (constMessage.isEmpty()) {
                ...
                constMessage.printf(
                    ...
                    std::max(rDiff, std::max(gDiff, std::max(bDiff, aDiff))), kTol, input4f.fR,
                    input4f.fG, input4f.fB, input4f.fA, output4f.fR, output4f.fG, output4f.fB,
                    output4f.fA, expected4f.fR, expected4f.fG, expected4f.fB, expected4f.fA);
              }
            }
          }
...
static void assert_processor_equality(
    skiatest::Reporter* reporter, const GrFragmentProcessor& fp, const GrFragmentProcessor& clone) {
  REPORTER_ASSERT(reporter, !strcmp(fp.name(), clone.name()), "\n%s", fp.dumpTreeInfo().c_str());
  REPORTER_ASSERT(
      reporter, fp.compatibleWithCoverageAsAlpha() == clone.compatibleWithCoverageAsAlpha(), "\n%s",
      fp.dumpTreeInfo().c_str());
  REPORTER_ASSERT(reporter, fp.isEqual(clone), "\n%s", fp.dumpTreeInfo().c_str());
  REPORTER_ASSERT(
      reporter, fp.preservesOpaqueInput() == clone.preservesOpaqueInput(), "\n%s",
      fp.dumpTreeInfo().c_str());
  REPORTER_ASSERT(
      reporter, fp.hasConstantOutputForConstantInput() == clone.hasConstantOutputForConstantInput(),
      "\n%s", fp.dumpTreeInfo().c_str());
  REPORTER_ASSERT(
      reporter, fp.numChildProcessors() == clone.numChildProcessors(), "\n%s",
      fp.dumpTreeInfo().c_str());
  REPORTER_ASSERT(
      reporter, fp.sampleUsage() == clone.sampleUsage(), "\n%s", fp.dumpTreeInfo().c_str());
  REPORTER_ASSERT(
      reporter, fp.usesSampleCoords() == clone.usesSampleCoords(), "\n%s",
      fp.dumpTreeInfo().c_str());
}

static bool verify_identical_render(
    skiatest::Reporter* reporter, int renderSize, const char* processorType,
    ...
    for (int x = 0; x < renderSize; ++x, ++idx) {
      ...
      if (failedPixelCount > maxAcceptableFailedPixels) {
        ...
        ERRORF(
            ...
            processorType, firstWrongX, firstWrongY, input_texel_color(x, y, 0.0f), readData1[idx],
            readData2[idx]);
...
static void log_clone_failure(
    skiatest::Reporter* reporter, int renderSize, GrDirectContext* context,
    const GrSurfaceProxyView& inputTexture, GrColor pixelsFP[], GrColor pixelsClone[],
    GrColor pixelsRegen[]) {
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(ProcessorCloneTest, reporter, ctxInfo) {
  ...
  for (int i = 0; i < GrFragmentProcessorTestFactory::Count(); ++i) {
    ...
    for (int j = 0; j < kTimesToInvokeFactory; ++j) {
      ...
      if (!verify_identical_render(
              reporter, kRenderSize, "Processor clone", readDataFP.data(), readDataClone.data())) {
        ...
        if (!verify_identical_render(
                reporter, kRenderSize, "Regenerated processor", readDataFP.data(),
                readDataRegen.data())) {
        ...
        if (!loggedFirstFailure) {
          log_clone_failure(
              reporter, kRenderSize, context, inputTexture, readDataFP.data(), readDataClone.data(),
              readDataRegen.data());
          loggedFirstFailure = true;
        }
      }
