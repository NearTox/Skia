...
void test_draw_op(
  ...
  GrPaint paint;
  paint.addColorFragmentProcessor(GrTextureEffect::Make(std::move(inputDataProxy), inputAlphaType));
...
bool log_texture_proxy(GrContext* context, sk_sp<GrTextureProxy> src, SkString* dst) {
  ...
  GrSwizzle swizzle =
      context->priv().caps()->getReadSwizzle(src->backendFormat(), GrColorType::kRGBA_8888);
  GrSurfaceProxyView view(std::move(src), origin, swizzle);
  auto sContext = GrSurfaceContext::Make(
      context, std::move(view), GrColorType::kRGBA_8888, kLogAlphaType, nullptr);
  SkBitmap bm;
