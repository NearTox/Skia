    ...
    for (int parentCnt = 0; parentCnt < 2; parentCnt++) {
              ...
              SkBackingFit::kApprox, 1, 1, GrColorType::kRGBA_8888, nullptr));
      {
        sk_sp<GrTextureProxy> proxy = proxyProvider->createProxy(
            format, desc, GrRenderable::kNo, 1, kTopLeft_GrSurfaceOrigin, SkBackingFit::kExact,
...
bool init_test_textures(
...

  {
    ...
    proxies[0] = proxyProvider->createTextureProxy(
        img, GrRenderable::kNo, 1, SkBudgeted::kYes, SkBackingFit::kExact);
...
bool log_surface_proxy(GrContext* context, sk_sp<GrSurfaceProxy> src, SkString* dst) {
  ...
  sk_sp<GrSurfaceContext> sContext(
      context->priv().makeWrappedSurfaceContext(src, GrColorType::kRGBA_8888, kPremul_SkAlphaType));
