 ...
 public:
  ...
  GrProcessorSet::Analysis finalize(
    ...
    return fProcessors.finalize(
        ...
        fsaaType, caps, clampType, &overrideColor);
  }
    ...
    for (int parentCnt = 0; parentCnt < 2; parentCnt++) {
      sk_sp<GrRenderTargetContext> renderTargetContext(
          context->priv().makeDeferredRenderTargetContext(
              format, SkBackingFit::kApprox, 1, 1, kRGBA_8888_GrPixelConfig, nullptr));
      {
        sk_sp<GrTextureProxy> proxy1 = proxyProvider->createProxy(
            format, desc, kTopLeft_GrSurfaceOrigin, SkBackingFit::kExact, SkBudgeted::kYes);
...
void render_fp(
  ...
  rtc->readPixels(
      SkImageInfo::Make(width, height, kRGBA_8888_SkColorType, kPremul_SkAlphaType), buffer, 0, 0,
      0);
}
...
bool init_test_textures(
...

  {
    ...
    proxies[0] = proxyProvider->createTextureProxy(
        img, kNone_GrSurfaceFlags, 1, SkBudgeted::kYes, SkBackingFit::kExact);
...

  {
    ...
    sk_sp<SkImage> img = SkImage::MakeRasterCopy(pixmap);
    proxies[1] = proxyProvider->createTextureProxy(
        img, kNone_GrSurfaceFlags, 1, SkBudgeted::kYes, SkBackingFit::kExact);
...
sk_sp<GrTextureProxy> make_input_texture(
  ...
  sk_sp<SkImage> img = SkImage::MakeRasterCopy(pixmap);
  return proxyProvider->createTextureProxy(
      img, kNone_GrSurfaceFlags, 1, SkBudgeted::kYes, SkBackingFit::kExact);
}

bool log_surface_context(sk_sp<GrSurfaceContext> src, SkString* dst) {
  SkImageInfo ii =
      SkImageInfo::Make(src->width(), src->height(), kRGBA_8888_SkColorType, kPremul_SkAlphaType);
  SkBitmap bm;
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(ProcessorOptimizationValidationTest, reporter, ctxInfo) {
  ...
  sk_sp<GrRenderTargetContext> rtc = context->priv().makeDeferredRenderTargetContext(
      format, SkBackingFit::kExact, kRenderSize, kRenderSize, kRGBA_8888_GrPixelConfig, nullptr);
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(ProcessorCloneTest, reporter, ctxInfo) {
  ...
  std::unique_ptr<GrColor[]> readData2(new GrColor[kRenderSize * kRenderSize]);
  auto readInfo =
      SkImageInfo::Make(kRenderSize, kRenderSize, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
