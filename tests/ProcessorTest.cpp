...
class TestOp : public GrMeshDrawOp {
 public:
  ...
  static std::unique_ptr<GrDrawOp> Make(
      GrContext* context, std::unique_ptr<GrFragmentProcessor> fp) {
...
class TestFP : public GrFragmentProcessor {
 public:
  ...
  static std::unique_ptr<GrFragmentProcessor> Make(
      const SkTArray<sk_sp<GrTextureProxy>>& proxies, const SkTArray<sk_sp<GrGpuBuffer>>& buffers) {
...
void test_draw_op(
    GrContext* context, GrRenderTargetContext* rtc, std::unique_ptr<GrFragmentProcessor> fp,
    sk_sp<GrTextureProxy> inputDataProxy, GrColorType inputColorType) {
  GrPaint paint;
  ...
  auto op = GrFillRectOp::MakeNonAARect(
      context, std::move(paint), SkMatrix::I(), SkRect::MakeWH(rtc->width(), rtc->height()));
  rtc->addDrawOp(GrNoClip(), std::move(op));
}

// This assumes that the output buffer will be the same size as inputDataProxy
void render_fp(
    GrContext* context, GrRenderTargetContext* rtc, GrFragmentProcessor* fp,
  ...
  rtc->readPixels(
      SkImageInfo::Make(width, height, kRGBA_8888_SkColorType, kPremul_SkAlphaType), buffer, 0,
      {0, 0});
}

/** Initializes the two test texture proxies that are available to the FP test factories. */
bool init_test_textures(
    GrResourceProvider* resourceProvider, GrProxyProvider* proxyProvider, SkRandom* random,
    sk_sp<GrTextureProxy> proxies[2], GrColorType proxyColorTypes[2]) {
...

  {
    ...
    proxies[0] = proxyProvider->createTextureProxy(img, 1, SkBudgeted::kYes, SkBackingFit::kExact);
...

  {
    ...
    sk_sp<SkImage> img = SkImage::MakeRasterCopy(pixmap);
    proxies[1] = proxyProvider->createTextureProxy(img, 1, SkBudgeted::kYes, SkBackingFit::kExact);
...
sk_sp<GrTextureProxy> make_input_texture(
    GrProxyProvider* proxyProvider, int width, int height, SkScalar delta) {
...
bool legal_modulation(
    const GrColor& in1, const GrColor& in2, const GrColor& in3, const GrColor& out1,
    const GrColor& out2, const GrColor& out3, bool alphaModulation) {
  ...
  SkPMColor4f expected1 = alphaModulation ? (fpPreModulation * in1f.fA) : (fpPreModulation * in1f);
  SkPMColor4f expected2 = alphaModulation ? (fpPreModulation * in2f.fA) : (fpPreModulation * in2f);
  SkPMColor4f expected3 = alphaModulation ? (fpPreModulation * in3f.fA) : (fpPreModulation * in3f);

  return fuzzy_color_equals(out1f, expected1) && fuzzy_color_equals(out2f, expected2) &&
...
#  endif
    for (int j = 0; j < timesToInvokeFactory; ++j) {
      ...
      if (fp->compatibleWithCoverageAsAlpha()) {
        ...
        render_fp(
            context, rtc.get(), fp.get(), inputTexture2, GrColorType::kRGBA_8888, readData2.get());
        render_fp(
            context, rtc.get(), fp.get(), inputTexture3, GrColorType::kRGBA_8888, readData3.get());
      }
      // Draw base frame last so that rtc holds the original FP behavior if we need to
      // dump the image to the log.
      render_fp(
          context, rtc.get(), fp.get(), inputTexture1, GrColorType::kRGBA_8888, readData1.get());
...
#  endif
      ...
      for (int y = 0; y < kRenderSize; ++y) {
        for (int x = 0; x < kRenderSize; ++x) {
          ...
          if (fp->compatibleWithCoverageAsAlpha()) {
            ...
            bool legalAlphaModulation = legal_modulation(
                input, i2, i3, output, o2, o3,
                /* alpha */ true);
            bool legalColorModulation = legal_modulation(
                input, i2, i3, output, o2, o3,
                /* alpha */ false);
          ...
          if (fp->hasConstantOutputForConstantInput(input4f, &expected4f)) {
            ...
            if (rDiff > kTol || gDiff > kTol || bDiff > kTol || aDiff > kTol) {
              if (constMessage.isEmpty()) {
                ...
                constMessage.printf(
                    "Processor %s claimed output for const input "
                    ...
                    input4f.fR, input4f.fG, input4f.fB, input4f.fA, output4f.fR, output4f.fG,
                    output4f.fB, output4f.fA, expected4f.fR, expected4f.fG, expected4f.fB,
                    expected4f.fA);
              }
            }
          }
          if (input4f.isOpaque() && fp->preservesOpaqueInput() && !output4f.isOpaque()) {
            ...
            if (opaqueMessage.isEmpty()) {
              opaqueMessage.printf(
                  "Processor %s claimed opaqueness is preserved but "
      ...
      if (failedPixelCount > kMaxAcceptableFailedPixels) {
        ERRORF(
            reporter,
            "Processor violated %d of %d pixels, seed: 0x%08x, processor: %s"
            ", first failing pixel details are below:",
            failedPixelCount, kRenderSize * kRenderSize, seed, fp->dumpInfo().c_str());
...
#  endif
  ...
  for (int i = 0; i < GrFragmentProcessorTestFactory::Count(); ++i) {
    ...
    for (int j = 0; j < kTimesToInvokeFactory; ++j) {
      ...
      REPORTER_ASSERT(
          reporter,
          fp->hasConstantOutputForConstantInput() == clone->hasConstantOutputForConstantInput());
      REPORTER_ASSERT(reporter, fp->numChildProcessors() == clone->numChildProcessors());
      REPORTER_ASSERT(reporter, fp->usesLocalCoords() == clone->usesLocalCoords());
      // Draw with original and read back the results.
      render_fp(
          context, rtc.get(), fp.get(), inputTexture, GrColorType::kRGBA_8888, readData1.get());

      // Draw with clone and read back the results.
      render_fp(
          context, rtc.get(), clone.get(), inputTexture, GrColorType::kRGBA_8888, readData2.get());
        ...
        for (int x = 0; x < kRenderSize && passing; ++x) {
          ...
          if (failedPixelCount > kMaxAcceptableFailedPixels) {
            ...
            ERRORF(
                ...
                name, firstWrongX, firstWrongY, input_texel_color(x, y, 0.0f), readData1[idx],
                readData2[idx]);
            if (!loggedFirstFailure) {
              ...
              auto info = SkImageInfo::Make(
                  kRenderSize, kRenderSize, kRGBA_8888_SkColorType, kUnpremul_SkAlphaType);
