...
class TestOp : public GrMeshDrawOp {
 public:
  ...
  void visitProxies(const VisitProxyFunc& func) const override { fProcessors.visitProxies(func); }
...
static GrColor input_texel_color(int i, int j, SkScalar delta) {
  ...
  SkColor color = SkColorSetARGB(
      (uint8_t)(i & 0xFF), (uint8_t)(j & 0xFF), (uint8_t)((i + j) & 0xFF),
      (uint8_t)((2 * j - i) & 0xFF));
...
void test_draw_op(
    GrRecordingContext* rContext, GrRenderTargetContext* rtc,
    std::unique_ptr<GrFragmentProcessor> fp) {
  GrPaint paint;
  paint.setColorFragmentProcessor(std::move(fp));
  paint.setPorterDuffXPFactory(SkBlendMode::kSrc);

  auto op = GrFillRectOp::MakeNonAARect(
      rContext, std::move(paint), SkMatrix::I(), SkRect::MakeWH(rtc->width(), rtc->height()));
  rtc->priv().testingOnly_addDrawOp(std::move(op));
}

// The output buffer must be the same size as the render-target context.
void render_fp(
    GrDirectContext* dContext, GrRenderTargetContext* rtc, std::unique_ptr<GrFragmentProcessor> fp,
    GrColor* outBuffer) {
  test_draw_op(dContext, rtc, std::move(fp));
  std::fill_n(outBuffer, rtc->width() * rtc->height(), 0);
  rtc->readPixels(
      dContext,
      SkImageInfo::Make(rtc->width(), rtc->height(), kRGBA_8888_SkColorType, kPremul_SkAlphaType),
 ...
 public:
  ...
  bool init() {
...

    {
      ...
      bitmap.setImmutable();
      GrBitmapTextureMaker maker(fContext, bitmap, GrImageTexGenPolicy::kNew_Uncached_Budgeted);
...
GrSurfaceProxyView make_input_texture(
...
bool log_texture_view(GrDirectContext* dContext, GrSurfaceProxyView src, SkString* dst) {
  ...
  auto sContext = GrSurfaceContext::Make(
      dContext, std::move(src), GrColorType::kRGBA_8888, kLogAlphaType, nullptr);
  SkBitmap bm;
...
#  endif
    ...
    for (int trial = 0;; ++trial) {
      ...
      if (fp->compatibleWithCoverageAsAlpha()) {
        ...
        render_fp(
            context, rtc.get(), fpGenerator.make(i, /*randomTreeDepth=*/1, inputTexture2),
            readData2.data());
        render_fp(
            context, rtc.get(), fpGenerator.make(i, /*randomTreeDepth=*/1, inputTexture3),
            readData3.data());
      ...
      render_fp(
          context, rtc.get(), fpGenerator.make(i, /*randomTreeDepth=*/1, inputTexture1),
          readData1.data());
...
static void assert_processor_equality(
  ...
  REPORTER_ASSERT(
      ...
      fp.dumpTreeInfo().c_str());
  REPORTER_ASSERT(
      reporter, fp.usesVaryingCoords() == clone.usesVaryingCoords(), "\n%s",
      fp.dumpTreeInfo().c_str());
  REPORTER_ASSERT(
      reporter, fp.referencesSampleCoords() == clone.referencesSampleCoords(), "\n%s",
      fp.dumpTreeInfo().c_str());
}
