...
static bool check_points(const SkPath& path, const SkPoint expected[], size_t count) {
  ...
  for (auto [v, p, w] : SkPathPriv::Iterate(path)) {
    switch (v) {
      case SkPathVerb::kMove: iter_pts.push_back(p[0]); break;
      case SkPathVerb::kLine: iter_pts.push_back(p[1]); break;
...
DEF_TEST(pathbuilder_make, reporter) {
  ...
  b.moveTo(0, 0);
  pts[0] = {0, 0};
  vbs[0] = (uint8_t)SkPathVerb::kMove;
  for (int i = 1; i < N; ++i) {
    float x = rand.nextF();
    float y = rand.nextF();
    b.lineTo(x, y);
    pts[i] = {x, y};
    vbs[i] = (uint8_t)SkPathVerb::kLine;
  }
  auto p0 = b.detach();
...
DEF_TEST(pathbuilder_shrinkToFit, reporter) {
  ...
  for (int pts = 0; pts < 10; pts++) {
    ...
    SkPath s = b.snapshot(), d = b.detach();
