...
static void compare_blobs(
    const SkBitmap& expected, const SkBitmap& actual, skiatest::Reporter* reporter,
    int tolerance = 0) {
...
SkBitmap RasterBlob(
    sk_sp<SkTextBlob> blob, int width, int height, const SkPaint& paint,
    GrRecordingContext* rContext, const SkMatrix* matrix = nullptr, SkScalar x = 0) {
...
SkBitmap RasterBlobThroughSlug(
    sk_sp<SkTextBlob> blob, int width, int height, const SkPaint& paint,
    GrRecordingContext* rContext, const SkMatrix* matrix = nullptr, SkScalar x = 0) {
  auto surface = MakeSurface(width, height, rContext);
  if (matrix) {
...
SkBitmap RasterSlug(
    sk_sp<Slug> slug, int width, int height, const SkPaint& paint, GrRecordingContext* rContext,
    const SkMatrix* matrix = nullptr, SkScalar x = 0) {
  auto surface = MakeSurface(width, height, rContext);
  auto canvas = surface->getCanvas();
  if (matrix) {
...
DEF_TEST(SkRemoteGlyphCache_TypefaceSerialization, reporter) {
  ...
  REPORTER_ASSERT(
      reporter,
      static_cast<SkTypefaceProxy*>(client_tf.get())->remoteTypefaceID() == server_tf->uniqueID());
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SkRemoteGlyphCache_StrikeSerialization, reporter, ctxInfo) {
  ...
  std::unique_ptr<SkCanvas> cache_diff_canvas =
      server.makeAnalysisCanvas(10, 10, props, nullptr, dContext->supportsDistanceFieldText());
  ...
  REPORTER_ASSERT(
      reporter, client.readStrikeData(serverStrikeData.data(), serverStrikeData.size()));
...
DEF_GPUTEST_FOR_CONTEXTS(
    SkRemoteGlyphCache_StrikeSerializationSlug, sk_gpu_test::GrContextFactory::IsRenderingContext,
...
DEF_GPUTEST_FOR_CONTEXTS(
    SkRemoteGlyphCache_SlugSerialization, sk_gpu_test::GrContextFactory::IsRenderingContext,
    reporter, ctxInfo, use_padding_options) {
  auto dContext = ctxInfo.directContext();
  sk_sp<DiscardableManager> discardableManager = sk_make_sp<DiscardableManager>();
  SkStrikeServer server(discardableManager.get());
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SkRemoteGlyphCache_DrawTextAsPath, reporter, ctxInfo) {
  ...
  REPORTER_ASSERT(reporter, SkStrikeSpec::ShouldDrawAsPath(paint, SkFont(), SkMatrix::I()));
  ...
  std::unique_ptr<SkCanvas> cache_diff_canvas =
      server.makeAnalysisCanvas(10, 10, props, nullptr, direct->supportsDistanceFieldText());
  cache_diff_canvas->drawTextBlob(serverBlob.get(), 0, 0, paint);
...
sk_sp<SkTextBlob> make_blob_causing_fallback(
  ...
  REPORTER_ASSERT(reporter, !SkStrikeSpec::ShouldDrawAsPath(SkPaint(), font, SkMatrix::I()));
  ...
  SkFont(sk_ref_sp(glyphTf))
      .textToGlyphs(s, strlen(s), SkTextEncoding::kUTF8, runBuffer.glyphs, runSize);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(
    SkRemoteGlyphCache_DrawTextAsMaskWithPathFallback, reporter, ctxInfo) {
  auto direct = ctxInfo.directContext();
  sk_sp<DiscardableManager> discardableManager = sk_make_sp<DiscardableManager>();
  SkStrikeServer server(discardableManager.get());
  SkStrikeClient client(discardableManager, false);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SkRemoteGlyphCache_CacheMissReporting, reporter, ctxInfo) {
  ...
  REPORTER_ASSERT(reporter, discardableManager->cacheMissCount(SkStrikeClient::kFontMetrics) == 1);
  REPORTER_ASSERT(
      reporter, discardableManager->cacheMissCount(SkStrikeClient::kGlyphMetrics) == 10);
...
sk_sp<SkTextBlob> MakeEmojiBlob(
    sk_sp<SkTypeface> serverTf, SkScalar textSize, sk_sp<SkTypeface> clientTf = nullptr) {
  SkFont font;
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SkRemoteGlyphCache_TypefaceWithNoPaths, reporter, ctxInfo) {
  ...
  auto props = FindSurfaceProps(direct);
  std::unique_ptr<SkCanvas> cache_diff_canvas =
      server.makeAnalysisCanvas(500, 500, props, nullptr, direct->supportsDistanceFieldText());
...
class SkRemoteGlyphCacheTest {
 public:
  static sk_sp<SkTextBlob> MakeNormalBlob(
      SkPaint* paint, sk_sp<SkTypeface> serverTf, bool asPaths, SkScalar textSize,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(
    SkRemoteGlyphCache_TypefaceWithPaths_MaskThenPath, reporter, ctxInfo) {
  auto direct = ctxInfo.directContext();
  sk_sp<DiscardableManager> discardableManager = sk_make_sp<DiscardableManager>();
  SkStrikeServer server(discardableManager.get());
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(
    SkRemoteGlyphCache_TypefaceWithPaths_PathThenMask, reporter, ctxInfo) {
  auto direct = ctxInfo.directContext();
  sk_sp<DiscardableManager> discardableManager = sk_make_sp<DiscardableManager>();
  SkStrikeServer server(discardableManager.get());
  SkStrikeClient client(discardableManager, true);
