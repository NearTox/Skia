...
SkBitmap RasterBlob(
    sk_sp<SkTextBlob> blob, int width, int height, const SkPaint& paint, GrContext* context,
...
DEF_TEST(SkRemoteGlyphCache_SearchOfDesperation, reporter) {
  ...
  // Build a fallback cache.
  {
    ...
    SkScalerContext::MakeRecAndEffects(
        font, paint, SkSurfacePropsCopyOrDefault(nullptr), flags, SkMatrix::I(), &rec, &effects);
  ...
  REPORTER_ASSERT(reporter, testCache == nullptr);
  testCache =
      strikeCache.createStrikeExclusive(*desc, clientTf->createScalerContext(effects, desc));
  ...
  {
    SkPoint pt{
        SkFixedToScalar(lostGlyphID.getSubXFixed()), SkFixedToScalar(lostGlyphID.getSubYFixed())};
    SkPackedGlyphID packedID{lostGlyphID.glyphID(), pt, rounding.ignorePositionFieldMask};
  ...
  {
    SkPoint pt{SkFixedToScalar(SK_FixedQuarter), SkFixedToScalar(SK_FixedQuarter)};
    SkPackedGlyphID packedID{lostGlyphID.glyphID(), pt, rounding.ignorePositionFieldMask};
    const SkGlyph* lostGlyph = metricsAndImages.glyph(packedID);
...
DEF_TEST(SkRemoteGlyphCache_ReWriteGlyph, reporter) {
...

  {
    ...
    font.setTypeface(clientTf);
    SkScalerContext::MakeRecAndEffects(
        font, paint, SkSurfaceProps(0, kUnknown_SkPixelGeometry), flags, SkMatrix::I(), &rec,
        &effects);
    auto desc = SkScalerContext::AutoDescriptorGivenRecAndEffects(rec, effects, &ad);
    ...
    if (glyph) {
      REPORTER_ASSERT(reporter, memcmp(glyph->image(), glyphImage, glyph->imageSize()) == 0);
    }
  }
