...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SkRemoteGlyphCache_TypefaceWithNoPaths, reporter, ctxInfo) {
  ...
  for (SkScalar textSize : {70, 180, 270, 340}) {
    ...
    SkBitmap expected = RasterBlob(serverBlob, width, height, paint, ctxInfo.grContext());
    SkBitmap actual = RasterBlob(clientBlob, width, height, paint, ctxInfo.grContext());
...
DEF_TEST(SkRemoteGlyphCache_ReWriteGlyph, reporter) {
...

  {
    ...
    REPORTER_ASSERT(reporter, memcmp(glyph->fImage, glyphImage, glyph->computeImageSize()) == 0);
  }
