...
static void gpu_tests(
    GrDirectContext* dContext, skiatest::Reporter* reporter, const TestCase& test) {
  const SkImageInfo nativeII = SkImageInfo::Make(kSize, kSize, test.fColorType, test.fAlphaType);
  const SkImageInfo f32Unpremul =
      SkImageInfo::Make(kSize, kSize, kRGBA_F32_SkColorType, kUnpremul_SkAlphaType);
  ...
  for (bool fullInit : {false, true}) {
    ...
    auto img = SkImage::MakeFromTexture(
        dContext, backendTex, kTopLeft_GrSurfaceOrigin, test.fColorType, test.fAlphaType, nullptr);
