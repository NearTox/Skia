...
static const TestCase gTests[] = {
    ...
    {kR16G16B16A16_unorm_SkColorType, kPremul_SkAlphaType, kRGBA_SkColorChannelFlags, false, false},
};
...
static void raster_tests(skiatest::Reporter* reporter, const TestCase& test) {
  ...
  const SkImageInfo f32Unpremul =
      SkImageInfo::Make(kSize, kSize, kRGBA_F32_SkColorType, kUnpremul_SkAlphaType);
...
static void compare_pixmaps(
    skiatest::Reporter* reporter, const SkPixmap& expected, const SkPixmap& actual, SkColorType ct,
    const char* label) {
  ...
  auto error = std::function<ComparePixmapsErrorReporter>(
      [reporter, ct, label](int x, int y, const float diffs[4]) {
        SkASSERT(x >= 0 && y >= 0);
        ERRORF(
            reporter, "%s %s - mismatch at %d, %d (%f, %f, %f %f)", ToolUtils::colortype_name(ct),
...
static void gpu_tests(GrContext* context, skiatest::Reporter* reporter, const TestCase& test) {
  ...
  for (bool fullInit : {false, true}) {
    ...
    if (fullInit) {
      backendTex =
          context->createBackendTexture(&nativeExpected, 1, GrRenderable::kNo, GrProtected::kNo);
    } else {
    ...
    auto img = SkImage::MakeFromTexture(
        context, backendTex, kTopLeft_GrSurfaceOrigin, test.fColorType, test.fAlphaType, nullptr);
...

    {
      ...
      {
        const SkImageInfo rgba8888Premul =
        ...
        compare_pixmaps(
            reporter, f32Expected, f32Actual, test.fColorType, "SkSurface::drawn to RGBA8888");
      }
    }
