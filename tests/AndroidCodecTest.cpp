...
static bool invalid(const SkISize& size) { return size.width() < 1 || size.height() < 1; }
      ...
      if (codec->getEncodedFormat() == SkEncodedImageFormat::kWEBP) {
        ...
        if (size.width() >= dims.width() || size.height() >= dims.height()) {
          ERRORF(
              ...
              file, computedSampleSize, dims.width(), dims.height(), size.width(), size.height());
        }
        REPORTER_ASSERT(
            r, size.width() >= requested.width() && size.height() >= requested.height());
    ...
    for (int sampleSize : {1, 2, 3, 4, 8, 16, 32}) {
      ...
      if (sampledDims != size) {
        ERRORF(
            ...
            file, sampleSize, computedSampleSize, sampledDims.width(), sampledDims.height(),
...
DEF_TEST(AndroidCodec_orientation, r) {
  ...
  for (const char* filePathFormatStr :
       {"images/orientation/%c_420.jpg", "images/orientation/%c.webp"})
    for (char i = '1'; i <= '8'; ++i) {
      ...
      androidCodec = SkAndroidCodec::MakeFromCodec(
          SkCodec::MakeFromData(std::move(data)),
      ...
      REPORTER_ASSERT(r, gen->getPixels(info, fromGenerator.getPixels(), fromGenerator.rowBytes()));

      SkBitmap fromAndroidCodec;
      fromAndroidCodec.allocPixels(info);
      auto result =
          androidCodec->getPixels(info, fromAndroidCodec.getPixels(), fromAndroidCodec.rowBytes());
      ...
      for (int i = 0; i < info.width(); ++i)
        for (int j = 0; j < info.height(); ++j) {
          ...
          if (c1 != c2) {
            ERRORF(
                r,
                "Bitmaps for %s do not match starting at position %i, %i\n"
                "\tfromGenerator: %x\tfromAndroidCodec: %x",
                path.c_str(), i, j, c1, c2);
            return;
          }
        }
    }
...
DEF_TEST(AndroidCodec_sampledOrientation, r) {
  ...
  auto androidCodec = SkAndroidCodec::MakeFromCodec(
      SkCodec::MakeFromData(std::move(data)), SkAndroidCodec::ExifOrientationBehavior::kRespect);
...
DEF_TEST(AndroidCodec_animatedOrientation, r) {
  ...
  for (auto rec : gRec) {
    ...
    for (auto behavior :
      ...
      auto androidCodec = SkAndroidCodec::MakeFromCodec(SkCodec::MakeFromData(data), behavior);
      ...
      switch (behavior) {
        case SkAndroidCodec::ExifOrientationBehavior::kRespect:
          if (rec.origin != kDefault_SkEncodedOrigin) {
            REPORTER_ASSERT(
                r, result == SkCodec::kInvalidParameters,
