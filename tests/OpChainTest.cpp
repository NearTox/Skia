...
class TestOp : public GrOp {
 public:
  ...
  static std::unique_ptr<TestOp> Make(
      GrContext* context, int value, const Range& range, int result[],
  ...
  for (int p = 0; p < kNumPermutations; ++p) {
    ...
    for (int g = 1; g < kNumOps; ++g) {
      for (int c = 0; c < kNumCombinabilitiesPerGrouping; ++c) {
        ...
        GrOpsTask opsTask(
            drawingMgr, context->priv().arenas(), GrSurfaceProxyView(proxy, kOrigin, writeSwizzle),
        ...
        for (int i = 0; i < kNumOps; ++i) {
          ...
          opsTask.addOp(
              drawingMgr, std::move(op), GrTextureResolveManager(context->priv().drawingManager()),
              *context->priv().caps());
        }
