...
class TestOp : public GrOp {
 public:
  ...
  static std::unique_ptr<TestOp> Make(
 ...
 private:
  ...
  CombineResult onCombineIfPossible(
      GrOp* t, GrRecordingContext::Arenas* arenas, const GrCaps&) override {
  ...
  for (int p = 0; p < kNumPermutations; ++p) {
    ...
    for (int g = 1; g < kNumOps; ++g) {
      for (int c = 0; c < kNumCombinabilitiesPerGrouping; ++c) {
        ...
        GrOpsTask opsTask(
            drawingMgr, dContext->priv().arenas(), GrSurfaceProxyView(proxy, kOrigin, writeSwizzle),
