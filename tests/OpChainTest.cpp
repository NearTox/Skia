...
class TestOp : public GrOp {
 public:
  ...
  static GrOp::Owner Make(
      GrRecordingContext* context, int value, const Range& range, int result[],
 ...
 private:
  ...
  CombineResult onCombineIfPossible(GrOp* t, SkArenaAlloc* arenas, const GrCaps&) override {
    ...
    if (result == GrOp::CombineResult::kMerged) {
      std::move(
          that->fValueRanges.begin(), that->fValueRanges.end(), std::back_inserter(fValueRanges));
    }
    return result;
  }
...
DEF_GPUTEST(OpChainTest, reporter, /*ctxInfo*/) {
  ...
  auto proxy = dContext->priv().proxyProvider()->createProxy(
      format, kDims, GrRenderable::kYes, 1, GrMipmapped::kNo, SkBackingFit::kExact, SkBudgeted::kNo,
      GrProtected::kNo, /*label=*/"OpChainTest", GrInternalSurfaceFlags::kNone);
  SkASSERT(proxy);
    ...
    for (int g = 1; g < kNumOps; ++g) {
      for (int c = 0; c < kNumCombinabilitiesPerGrouping; ++c) {
        ...
        GrOpFlushState flushState(
            dContext->priv().getGpu(), dContext->priv().resourceProvider(), &tracker);
        skgpu::v1::OpsTask opsTask(
            drawingMgr, GrSurfaceProxyView(proxy, kOrigin, writeSwizzle),
            dContext->priv().auditTrail(), arenas);
        ...
        for (int i = 0; i < kNumOps; ++i) {
          ...
          opsTask.addOp(
              drawingMgr, std::move(op), GrTextureResolveManager(dContext->priv().drawingManager()),
              *caps);
        }
