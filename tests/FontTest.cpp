...
static SkFont serialize_deserialize(const SkFont& font, skiatest::Reporter* reporter) {
  ...
  SkASSERT((!font.getTypeface() && count == 0) || (font.getTypeface() && count == 1));
  if (count) {
    SkTypeface* typeface;
...
DEF_TEST(Font_flatten, reporter) {
  ...
  const SkFont::Edging edges[] = {SkFont::Edging::kAlias, SkFont::Edging::kSubpixelAntiAlias};
  const SkFontHinting hints[] = {SkFontHinting::kNone, SkFontHinting::kFull};
  const unsigned int flags[] = {
      kForceAutoHinting, kEmbeddedBitmaps, kSubpixel, kLinearMetrics,
      kEmbolden,         kBaselineSnap,    kAllBits,
  };
  const sk_sp<SkTypeface> typefaces[] = {nullptr, ToolUtils::sample_user_typeface()};
