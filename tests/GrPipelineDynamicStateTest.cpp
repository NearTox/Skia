...
static const GrColor kMeshColors[kNumMeshes]{
    ...
    GrColorPackRGBA(0, 0, 255, 255), GrColorPackRGBA(0, 0, 0, 255)};
...
class PipelineDynamicStateTestProcessor : public GrGeometryProcessor {
 public:
  static GrGeometryProcessor* Make(SkArenaAlloc* arena) {
    return arena->make([&](void* ptr) { return new (ptr) PipelineDynamicStateTestProcessor(); });
  }
...
class GrPipelineDynamicStateTestOp : public GrDrawOp {
 public:
  ...
  static GrOp::Owner Make(
      GrRecordingContext* context, GrScissorTest scissorTest, sk_sp<const GrBuffer> vbuff) {
 ...
 private:
  ...
  GrPipelineDynamicStateTestOp(GrScissorTest scissorTest, sk_sp<const GrBuffer> vbuff)
    ...
    this->setBounds(SkRect::MakeIWH(kScreenSize, kScreenSize), HasAABloat::kNo, IsHairline::kNo);
  }
  ...
  void onExecute(GrOpFlushState* flushState, const SkRect& chainBounds) override {
    GrPipeline pipeline(
        fScissorTest, SkBlendMode::kSrc, flushState->drawOpArgs().writeView().swizzle());
    ...
    GrProgramInfo programInfo(
        flushState->caps(), flushState->writeView(), flushState->usesMSAASurface(), &pipeline,
        &GrUserStencilSettings::kUnused, geomProc, GrPrimitiveType::kTriangleStrip,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrPipelineDynamicStateTest, reporter, ctxInfo) {
  ...
  auto sdc = skgpu::v1::SurfaceDrawContext::Make(
      dContext, GrColorType::kRGBA_8888, nullptr, SkBackingFit::kExact, {kScreenSize, kScreenSize},
      SkSurfaceProps());
  if (!sdc) {
  ...
  Vertex vdata[kNumMeshes * 4] = {{0, 0, kMeshColors[0]}, {0, d, kMeshColors[0]},
                                  {d, 0, kMeshColors[0]}, {d, d, kMeshColors[0]},

                                  {0, 0, kMeshColors[1]}, {0, d, kMeshColors[1]},
                                  {d, 0, kMeshColors[1]}, {d, d, kMeshColors[1]},

                                  {0, 0, kMeshColors[2]}, {0, d, kMeshColors[2]},
                                  {d, 0, kMeshColors[2]}, {d, d, kMeshColors[2]},

                                  {0, 0, kMeshColors[3]}, {0, d, kMeshColors[3]},
                                  {d, 0, kMeshColors[3]}, {d, d, kMeshColors[3]}};

  sk_sp<const GrBuffer> vbuff(
      rp->createBuffer(vdata, sizeof(vdata), GrGpuBufferType::kVertex, kDynamic_GrAccessPattern));
  if (!vbuff) {
  ...
  for (GrScissorTest scissorTest : {GrScissorTest::kEnabled, GrScissorTest::kDisabled}) {
    ...
    auto ii =
        SkImageInfo::Make(kScreenSize, kScreenSize, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
    ...
    for (int y = 0; y < kScreenSize; ++y) {
      for (int x = 0; x < kScreenSize; ++x) {
        ...
        if (expected != actual) {
          ERRORF(
              reporter, "[scissor=%s] pixel (%i,%i): got 0x%x expected 0x%x",
              GrScissorTest::kEnabled == scissorTest ? "enabled" : "disabled", x, y, actual,
              expected);
          return;
        }
      }
