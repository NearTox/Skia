...
static void test_read(
    skiatest::Reporter* reporter, SkStream* bufferedStream, const void* expectations,
    size_t bytesToRead) {
...
static void test_rewind(
    skiatest::Reporter* reporter, SkStream* bufferedStream, bool shouldSucceed) {
...
static void test_hasLength(
    skiatest::Reporter* reporter, const SkStream& bufferedStream,
...
static void test_incremental_buffering(skiatest::Reporter* reporter, size_t bufferSize) {
  ...
  auto bufferedStream =
      SkFrontBufferedStream::Make(std::unique_ptr<SkStream>(memStream), bufferSize);
...
class AndroidLikeMemoryStream : public SkMemoryStream {
 public:
  AndroidLikeMemoryStream(void* data, size_t size, bool ownMemory)
      : INHERITED(data, size, ownMemory), fIsAtEnd(false) {}
...
class LengthOptionalStream : public SkStream {
 public:
  LengthOptionalStream(bool hasLength, bool hasPosition)
      : fHasLength(hasLength), fHasPosition(hasPosition) {}

  bool hasLength() const override { return fHasLength; }

  bool hasPosition() const override { return fHasPosition; }
...
static void test_length_combos(skiatest::Reporter* reporter, size_t bufferSize) {
  for (int hasLen = 0; hasLen <= 1; hasLen++) {
    for (int hasPos = 0; hasPos <= 1; hasPos++) {
      ...
      auto buffered = SkFrontBufferedStream::Make(std::unique_ptr<SkStream>(stream), bufferSize);
...
static void test_initial_offset(skiatest::Reporter* reporter, size_t bufferSize) {
  ...
  while (currentPosition < bufferedLength) {
    ...
    test_read(
        reporter, bufferedStream.get(), gAbcs + arbitraryOffset + currentPosition, amountToRead);
...
class FailingStream : public SkStream {
 public:
  ...
  bool isAtEnd() const override { return fAtEnd; }
