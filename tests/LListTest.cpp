...
static void check_list(
    const SkTInternalLList<ListElement>& list, skiatest::Reporter* reporter, bool empty,
    int numElements, bool in0, bool in1, bool in2, bool in3, ListElement elements[4]) {
  ...
  for (int j = 0; j < 100; ++j) {
    if (list1.isEmpty() || random.nextBiasedBool(3 * SK_Scalar1 / 4)) {
      ...
      switch (insertionMethod) {
        case 0: list1.addToHead(ListElement(id)); break;
        case 1: list1.addToTail(ListElement(id)); break;
