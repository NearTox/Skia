...
bool check_pixels(
    skiatest::Reporter* reporter, GrDirectContext* dContext, const GrBackendTexture& tex,
    const SkImageInfo& info, SkColor expectedColor) {
  ...
  sk_sp<SkSurface> surface = SkSurface::MakeFromBackendTexture(
      dContext, tex, kTopLeft_GrSurfaceOrigin,
      /*sampleCnt=*/4, kRGBA_8888_SkColorType, nullptr, nullptr);
  SkBitmap actual;
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SurfaceResolveTest, reporter, ctxInfo) {
  ...
  auto managedTex =
      ManagedBackendTexture::MakeFromInfo(dContext, info, GrMipmapped::kNo, GrRenderable::kYes);
  if (!managedTex) {
    return;
  }
  ...
  // for both backends.
  bool autoResolves = caps->msaaResolvesAutomatically() || caps->preferDiscardableMSAAAttachment();
