...
DEF_TEST(SkRuntimeEffectInvalid, r) {
  auto test = [r](const char* hdr, const char* body, const char* expected) {
    SkString src = SkStringPrintf("%s half4 main(float2 p) { %s return half4(0); }", hdr, body);
  ...
  test("", "shader child;", "must be global");
  test(
      "in shader child; half4 helper(shader fp) { return sample(fp); }",
      "half4 color = helper(child);", "parameter");
  test(
      "in shader child; shader get_child() { return child; }", "half4 color = sample(get_child());",
      "return");
  test("in shader child;", "half4 color = sample(shader(child));", "construct");
  test(
      "in shader child1; in shader child2;", "half4 color = sample(p.x > 10 ? child1 : child2);",
      "expression");
...
class TestEffect {
 public:
  ...
  void build(const char* header, const char* body) {
    SkString src = SkStringPrintf("%s half4 main(float2 p) { %s }", header, body);
    auto [effect, errorText] = SkRuntimeEffect::Make(src);
    if (!effect) {
      REPORT_FAILURE(
  ...
  void test(GrColor TL, GrColor TR, GrColor BL, GrColor BR, PreTestFn preTestCallback = nullptr) {
    ...
    if (0 != memcmp(actual, expected, sizeof(actual))) {
      REPORT_FAILURE(
          fReporter, "Runtime effect didn't match expectations",
          SkStringPrintf(
              "\n"
              "Expected: [ %08x %08x %08x %08x ]\n"
              "Got     : [ %08x %08x %08x %08x ]\n"
              "SkSL:\n%s\n",
              TL, TR, BL, BR, actual[0], actual[1], actual[2], actual[3],
              fBuilder->effect()->source().c_str()));
    }
  }
...
static void test_RuntimeEffect_Shaders(skiatest::Reporter* r, GrRecordingContext* rContext) {
  ...
  effect.build("uniform float4 gColor;", "return half4(gColor);");
  ...
  effect.build("in shader child;", "return sample(child);");
  ...
  effect.build("in shader child;", "return sample(child, p.yx);");
  effect.child("child") = rgbwShader;
  effect.test(0xFF0000FF, 0xFFFF0000, 0xFF00FF00, 0xFFFFFFFF);

  // Sampling with a matrix (again, reflecting about the diagonal)
  effect.build("in shader child;", "return sample(child, float3x3(0, 1, 0, 1, 0, 0, 0, 0, 1));");
  effect.child("child") = rgbwShader;
  effect.test(0xFF0000FF, 0xFFFF0000, 0xFF00FF00, 0xFFFFFFFF);
