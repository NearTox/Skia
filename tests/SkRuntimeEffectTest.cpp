...
void test_invalid_effect(skiatest::Reporter* r, const char* src, const char* expected) {
  ...
  REPORTER_ASSERT(
      r, errorText.contains(expected),
      "Expected error message to contain \"%s\". Actual message: \"%s\"", expected,
      errorText.c_str());
};
...
DEF_TEST(SkRuntimeEffectInvalid_UndefinedFunction, r) {
  test_invalid_effect(
      r, "half4 missing(); half4 main(float2 p) { return missing(); }",
...
DEF_TEST(SkRuntimeEffectInvalid_SkCapsDisallowed, r) {
  ...
  test_invalid_effect(
      r, "half4 main(float2 p) { return sk_Caps.floatIs32Bits ? half4(1) : half4(0); }",
...
DEF_TEST(SkRuntimeEffectCanEnableVersion300, r) {
  ...
  test_invalid_effect(
      r, "#version 100\nfloat f[2] = float[2](0, 1);" EMPTY_MAIN, "construction of array type");
...
DEF_TEST(SkRuntimeEffectForBlender, r) {
  ...
  // sk_FragCoord should not be available
  test_invalid("half4 main(half4 s, half4 d) { return sk_FragCoord.xy01; }", "unknown identifier");
...
DEF_TEST(SkRuntimeEffectForShader, r) {
  ...
  test_invalid(
      "half4 main(float2 p) { return sk_FragCoord.xy01; }", "unknown identifier 'sk_FragCoord'");
...
static void verify_2x2_surface_results(
    skiatest::Reporter* r, const SkRuntimeEffect* effect, SkSurface* surface,
  ...
  if (actual != expected) {
    REPORT_FAILURE(
        r, "Runtime effect didn't match expectations",
        SkStringPrintf(
            "\n"
...
class TestEffect {
 public:
  ...
  void build(const char* src) {
    ...
    if (!effect) {
      REPORT_FAILURE(
          fReporter, "effect", SkStringPrintf("Effect didn't compile: %s", errorText.c_str()));
      return;
    }
  ...
  SkRuntimeShaderBuilder::BuilderChild child(const char* name) { return fBuilder->child(name); }
...
class TestBlend {
 public:
  ...
  SkRuntimeBlendBuilder::BuilderChild child(const char* name) { return fBuilder->child(name); }
...
static void test_RuntimeEffect_Shaders(skiatest::Reporter* r, GrRecordingContext* rContext) {
  ...
  sk_sp<SkSurface> surface = rContext ? SkSurface::MakeRenderTarget(rContext, SkBudgeted::kNo, info)
  ...
  effect.test({0xFF000000, 0xFF00007F, 0xFF007F00, 0xFF007F7F}, [](SkCanvas* canvas, SkPaint*) {
    canvas->rotate(45.0f);
  });
  ...
  effect.test(0xFF00FFFF, [](SkCanvas*, SkPaint* paint) {
    paint->setColor4f({1.0f, 1.0f, 0.0f, 1.0f});
  });
...
DEF_TEST(SkRuntimeEffectSimple, r) { test_RuntimeEffect_Shaders(r, nullptr); }
...
DEF_TEST(SkRuntimeEffectTraceShader, r) {
  for (int imageSize : {2, 80}) {
    SkImageInfo info =
        SkImageInfo::Make(imageSize, imageSize, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
    ...
    auto expectation = SkSL::String::printf(
...
exit half4 main(float2 p)
        ...
        center, center);
    REPORTER_ASSERT(
        r, dump == expectation, "Trace output does not match expectation for %dx%d:\n%.*s\n",
...
exit half4 main(float2 p)
  ...
  REPORTER_ASSERT(
      r, dump == kExpectation, "Trace output does not match expectation:\n%.*s\n", (int)dump.size(),
      dump.data());
}
...
static void test_RuntimeEffect_Blenders(skiatest::Reporter* r, GrRecordingContext* rContext) {
  ...
  effect.build(
      ...
      "half4 main(half4 s, half4 d) { return child.eval(s, d); }");
  effect.child("child") = nullptr;
  effect.test({0xFF000080, 0xFF008000, 0xFF800000, 0xFF808080}, [](SkCanvas*, SkPaint* paint) {
    paint->setColor4f({0.0f, 0.0f, 0.0f, 0.497f});
  });
  ...
  effect.test({0xFF4523FF, 0xFF45FF01, 0xFFFF2301, 0xFFFFFFFF}, [](SkCanvas*, SkPaint* paint) {
    paint->setColor(0xFF012345);
  });
...
DEF_TEST(SkRuntimeEffect_Blender_CPU, r) { test_RuntimeEffect_Blenders(r, /*rContext=*/nullptr); }
...
DEF_TEST(SkRuntimeColorFilterSingleColor, r) {
  ...
  SkColor4f c = cf->filterColor4f({0.25, 0.5, 0.75, 1.0}, sk_srgb_singleton(), sk_srgb_singleton());
...
static void test_RuntimeEffectStructNameReuse(skiatest::Reporter* r, GrRecordingContext* rContext) {
  ...
  auto [childEffect, err] = SkRuntimeEffect::MakeForShader(
      SkString("uniform shader paint;"
               "struct S { half4 rgba; };"
  ...
  sk_sp<SkShader> child = childEffect->makeShader(
      /*uniforms=*/nullptr, &nullChild,
      /*childCount=*/1);
...
DEF_TEST(SkRuntimeStructNameReuse, r) { test_RuntimeEffectStructNameReuse(r, nullptr); }
...
DEF_TEST(SkRuntimeColorFilterFlags, r) {
  {  // Here's a non-trivial filter that doesn't change alpha.
    auto [effect, err] = SkRuntimeEffect::MakeForColorFilter(
        SkString{"half4 main(half4 color) { return color + half4(1,1,1,0); }"});
  ...
  {  // Here's one that definitely changes alpha.
    auto [effect, err] = SkRuntimeEffect::MakeForColorFilter(
        SkString{"half4 main(half4 color) { return color + half4(0,0,0,4); }"});
    REPORTER_ASSERT(r, effect && err.isEmpty());
    sk_sp<SkColorFilter> filter = effect->makeColorFilter(SkData::MakeEmpty());
    REPORTER_ASSERT(r, filter && !filter->isAlphaUnchanged());
  }
...
DEF_TEST(SkRuntimeShaderSampleCoords, r) {
  ...
  auto test = [&](const char* src, bool expectExplicit, bool expectReferencesSampleCoords) {
    ...
    auto fp = GrSkSLFP::Make(
        effect, "test_fp", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "child",
        std::move(child));
  ...
  test(
      ...
      "half4 main(float2 xy) { adjust(xy); return child.eval(xy); }",
      true, true);
  ...
  test(
      ...
      "half4 main(float2 xy) { return helper(xy); }",
      true, true);
}
...
DEF_TEST(SkRuntimeShaderIsOpaque, r) {
  ...
  auto test = [&](const char* body, bool expectOpaque) {
    auto [effect, err] = SkRuntimeEffect::MakeForShader(SkStringPrintf(
        R"(
...
DEF_GPUTEST_FOR_ALL_CONTEXTS(GrSkSLFP_Specialized, r, ctxInfo) {
  ...
  auto make_color_fp = [&](SkPMColor4f color, bool specialize) {
    ...
    result.fp = GrSkSLFP::Make(
        std::move(effect), "color_fp", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "color",
        GrSkSLFP::SpecializeIf(specialize, color));
  ...
  FpAndKey uRed = make_color_fp({1, 0, 0, 1}, false), uGreen = make_color_fp({0, 1, 0, 1}, false),
           sRed = make_color_fp({1, 0, 0, 1}, true), sGreen = make_color_fp({0, 1, 0, 1}, true);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrSkSLFP_UniformArray, r, ctxInfo) {
  ...
  for (const auto& colorArray : {kRed, kGreen, kBlue, kGray}) {
    ...
    testCtx->fillWithFP(GrSkSLFP::Make(
        std::move(effect), "test_fp",
        /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "color", SkMakeSpan(colorArray)));
    ...
    if (actual !=
        GrColorPackRGBA(
            255 * colorArray[0], 255 * colorArray[1], 255 * colorArray[2], 255 * colorArray[3])) {
      REPORT_FAILURE(
          r, "Uniform array didn't match expectations",
          SkStringPrintf(
              "\n"
              "Expected: [ %g %g %g %g ]\n"
              "Got     : [ %08x ]\n",
              colorArray[0], colorArray[1], colorArray[2], colorArray[3], actual));
      break;
    }
  }
}
