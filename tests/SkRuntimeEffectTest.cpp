...
class TestEffect {
 public:
  TestEffect(skiatest::Reporter* r, const char* hdr, const char* body) {
    ...
    auto [effect, errorText] = SkRuntimeEffect::Make(src);
    if (!effect) {
      REPORT_FAILURE(r, "effect", SkStringPrintf("Effect didn't compile: %s", errorText.c_str()));
      return;
    }
  ...
  struct InputVar {
    template <typename T>
    InputVar& operator=(const T& val) {
      SkASSERT(sizeof(T) == fVar.sizeInBytes());
      memcpy(SkTAddOffset<void>(fOwner->fInputs->writable_data(), fVar.fOffset), &val, sizeof(T));
      return *this;
    }
    TestEffect* fOwner;
  ...
  InputVar operator[](const char* name) {
    auto input = std::find_if(
        fEffect->inputs().begin(), fEffect->inputs().end(),
  ...
  void test(
      skiatest::Reporter* r, sk_sp<SkSurface> surface, uint32_t TL, uint32_t TR, uint32_t BL,
      uint32_t BR) {
    if (!fEffect) {
      return;
    }
