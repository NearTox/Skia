...
DEF_TEST(SkRuntimeEffectInvalid_FPOnly, r) {
  ...
  test_invalid_effect(
      r,
      "in bool Flag; "
      "layout(when=Flag) uniform float Input;" EMPTY_MAIN,
      "when");
...
DEF_TEST(SkRuntimeEffectInvalid_MarkerRequiresFloat4x4, r) {
  ...
  test_invalid_effect(
      r, "layout(marker=local_to_world) uniform float3x3 localToWorld;" EMPTY_MAIN, "float4x4");
}

DEF_TEST(SkRuntimeEffectInvalid_UndefinedFunction, r) {
  test_invalid_effect(
      r, "half4 missing(); half4 main() { return missing(); }", "undefined function");
}
...
DEF_TEST(SkRuntimeEffectInvalid_ShaderLimitations, r) {
  ...
  test_invalid_effect(r, "half4 main() { shader child; return sample(child); }", "must be global");
  test_invalid_effect(
      r,
      "uniform shader child; half4 helper(shader fp) { return sample(fp); }"
      "half4 main() { return helper(child); }",
      "parameter");
  test_invalid_effect(
      r,
      "uniform shader child; shader get_child() { return child; }"
...
DEF_TEST(SkRuntimeEffectInvalid_SkCapsDisallowed, r) {
  ...
  test_invalid_effect(
      r, "half4 main() { return sk_Caps.integerSupport ? half4(1) : half4(0); }",
