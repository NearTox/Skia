...
DEF_TEST(SkRuntimeEffectInvalid, r) {
  auto test = [r](const char* hdr, const char* body, const char* expected) {
    SkString src = SkStringPrintf("%s void main(float2 p, inout half4 color) { %s }", hdr, body);
  ...
  test(
      "in shader child; half4 helper(shader fp) { return sample(fp); }", "color = helper(child);",
      "parameter");
  test(
      "in shader child; shader get_child() { return child; }", "color = sample(get_child());",
      "return");
  test("in shader child;", "color = sample(shader(child));", "construct");
  test(
      "in shader child1; in shader child2;", "color = sample(p.x > 10 ? child1 : child2);",
      "expression");
...
class TestEffect {
 public:
  ...
  void build(const char* header, const char* body) {
    SkString src = SkStringPrintf("%s void main(float2 p, inout half4 color) { %s }", header, body);
    auto [effect, errorText] = SkRuntimeEffect::Make(src);
    if (!effect) {
      REPORT_FAILURE(
...
static void test_RuntimeEffect_Shaders(skiatest::Reporter* r, GrRecordingContext* rContext) {
  ...
  effect.build("uniform float4 gColor;", "color = half4(gColor);");
  ...
  effect.build("in shader child;", "color = sample(child);");
  ...
  effect.build("in shader child;", "color = sample(child, p.yx);");
  effect.child("child") = rgbwShader;
  effect.test(0xFF0000FF, 0xFFFF0000, 0xFF00FF00, 0xFFFFFFFF);

  // Sampling with a matrix (again, reflecting about the diagonal)
  effect.build("in shader child;", "color = sample(child, float3x3(0, 1, 0, 1, 0, 0, 0, 0, 1));");
  effect.child("child") = rgbwShader;
  effect.test(0xFF0000FF, 0xFFFF0000, 0xFF00FF00, 0xFFFFFFFF);
