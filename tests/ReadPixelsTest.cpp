...
static void init_bitmap(
    SkBitmap* bitmap, const SkIRect& rect, BitmapInit init, SkColorType ct, SkAlphaType at) {
  SkImageInfo info = SkImageInfo::Make(rect.size(), ct, at);
  size_t rowBytes = 0;
  switch (init) {
    case kTight_BitmapInit: break;
  ...
  for (size_t rect = 0; rect < SK_ARRAY_COUNT(gReadPixelsTestRects); ++rect) {
    ...
    for (BitmapInit bmi = kFirstBitmapInit; bmi <= lastBitmapInit; bmi = nextBMI(bmi)) {
      for (size_t c = 0; c < SK_ARRAY_COUNT(gReadPixelsConfigs); ++c) {
        SkBitmap bmp;
        init_bitmap(
            &bmp, srcRect, bmi, gReadPixelsConfigs[c].fColorType, gReadPixelsConfigs[c].fAlphaType);
...
// plus a tail pixel.
static const uint32_t rgba[kNumPixels] = {0xFF0000FF, 0xFF00FF00, 0xFFFF0000, 0xFFFFFFFF,
                                          0xFF000000};
static const uint32_t bgra[kNumPixels] = {0xFFFF0000, 0xFF00FF00, 0xFF0000FF, 0xFFFFFFFF,
                                          0xFF000000};
static const uint16_t rgb565[kNumPixels] = {SK_R16_MASK_IN_PLACE, SK_G16_MASK_IN_PLACE,
                                            SK_B16_MASK_IN_PLACE, 0xFFFF, 0x0};
...
static const uint64_t f16[kNumPixels] = {kAlpha | kRed, kAlpha | kGreen, kAlpha | kBlue,
                                         kAlpha | kBlue | kGreen | kRed, kAlpha};
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(AsyncReadPixels, reporter, ctxInfo) {
  ...
  auto legacy_callback = [](SkSurface::ReleaseContext context, const void* data, size_t rowBytes) {
  ...
  for (auto origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
    ...
    for (int sct = 0; sct <= kLastEnum_SkColorType; ++sct) {
      ...
      auto surf = SkSurface::MakeRenderTarget(
          ctxInfo.grContext(), SkBudgeted::kNo, info, 1, origin, nullptr);
      if (!surf) {
        continue;
      }
      ...
      float d =
          std::sqrt((float)surf->width() * surf->width() + (float)surf->height() * surf->height());
        ...
        for (const sk_sp<SkColorSpace>& readCS :
          ...
          for (const auto& rect :
               {SkIRect::MakeWH(kW, kH), SkIRect::MakeLTRB(1, 2, kW - 3, kH - 4),
                SkIRect::MakeXYWH(1, 1, 0, 0), SkIRect::MakeWH(kW + 1, kH / 2)}) {
            for (bool legacy : {false, true}) {
              ...
              if (legacy) {
                ...
                surf->asyncRescaleAndReadPixels(
                    info, rect, kGamma, kQuality, legacy_callback, &context);
                ...
                surf->asyncRescaleAndReadPixels(
                    info, rect, kGamma, kQuality, async_callback, &asyncContext);
                ...
                if (asyncContext.fResult) {
                  ...
                  if (count == 1) {
                    ...
                    ERRORF(reporter, "Unexpected AsyncResult::count(): %d", count);
                    continue;
                  }
                }
              }
              ...
              if (succeeded) {
                REPORTER_ASSERT(reporter, readCT != kUnknown_SkColorType && !rect.isEmpty());
              } else {
                ...
                if (readCT != kUnknown_SkColorType && readCT != kGray_8_SkColorType &&
                  ...
                  ERRORF(
                      ...
                      rect.fLeft, rect.fTop, rect.fRight, rect.fBottom, origin, didCSConversion);
                }
                continue;
              }
              SkPixmap ref;
              ...
              int rgbBits =
                  std::min({min_rgb_channel_bits(readCT), min_rgb_channel_bits(surfCT), 8});
