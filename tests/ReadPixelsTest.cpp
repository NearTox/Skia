...
static const SkRect DEV_RECT_S = SkRect::MakeWH(DEV_W * SK_Scalar1, DEV_H* SK_Scalar1);
...
static SkPMColor convert_to_pmcolor(
    SkColorType ct, SkAlphaType at, const uint32_t* addr, bool* doUnpremul) {
  *doUnpremul = (kUnpremul_SkAlphaType == at);
  ...
  switch (ct) {
    ...
    case kRGBA_8888_SkColorType:
      ...
      a = static_cast<U8CPU>(c[3]);
      break;
    default: SkDEBUGFAIL("Unexpected colortype"); return 0;
  }
  ...
  if (bmp.isNull()) {
    ...
    for (int y = 0; y < DEV_H; ++y) {
      for (int x = 0; x < DEV_W; ++x) {
        SkPMColor* pixel =
            reinterpret_cast<SkPMColor*>(pixels + y * bmp.rowBytes() + x * bmp.bytesPerPixel());
      ...
      if (kAlpha_8_SkColorType == bitmap->colorType()) {
        ...
        *alpha = SkGetPackedA32(initColor);
        ...
        SkPMColor* pixel = reinterpret_cast<SkPMColor*>(
            pixels + y * bitmap->rowBytes() + x * bitmap->bytesPerPixel());
        *pixel = initColor;
      }
    }
  }
...
static bool check_read(
    skiatest::Reporter* reporter, const SkBitmap& bitmap, int x, int y, bool checkSurfacePixels,
    bool checkBitmapPixels, SkImageInfo surfaceInfo) {
        ...
        if (clippedSrcRect.contains(devx, devy)) {
          if (checkSurfacePixels) {
            ...
            if (surfaceAlpha != *alpha) {
              ERRORF(
                  reporter, "Expected readback alpha (%d, %d) value 0x%02x, got 0x%02x. ", bx, by,
                  surfaceAlpha, *alpha);
              return false;
            }
          }
...
static void init_bitmap(
    SkBitmap* bitmap, const SkIRect& rect, TightRowBytes tightRB, SkColorType ct, SkAlphaType at) {
...
bool read_should_succeed(
    const SkIRect& srcRect, const SkImageInfo& dstInfo, const SkImageInfo& srcInfo) {
  return SkIRect::Intersects(srcRect, DEV_RECT) && SkImageInfoValidConversion(dstInfo, srcInfo);
}

static void test_readpixels(
    skiatest::Reporter* reporter, const sk_sp<SkSurface>& surface, const SkImageInfo& surfaceInfo) {
  ...
  for (size_t rect = 0; rect < SK_ARRAY_COUNT(gReadPixelsTestRects); ++rect) {
    ...
    for (auto tightRB : {TightRowBytes::kYes, TightRowBytes::kNo}) {
      for (size_t c = 0; c < SK_ARRAY_COUNT(gReadPixelsConfigs); ++c) {
        SkBitmap bmp;
        init_bitmap(
            &bmp, srcRect, tightRB, gReadPixelsConfigs[c].fColorType,
        ...
        REPORTER_ASSERT(
            ...
            "Read succeed=%d unexpectedly, src ct/at: %d/%d, dst ct/at: %d/%d", success,
            surfaceInfo.colorType(), surfaceInfo.alphaType(), bmp.info().colorType(),
            bmp.info().alphaType());
        ...
        if (success || startsWithPixels) {
          check_read(
              reporter, bmp, srcRect.fLeft, srcRect.fTop, success, startsWithPixels, surfaceInfo);
        } else {
...
static void test_readpixels_texture(
    ...
    std::unique_ptr<GrSurfaceContext> sContext, const SkImageInfo& surfaceInfo) {
  for (size_t rect = 0; rect < SK_ARRAY_COUNT(gReadPixelsTestRects); ++rect) {
    const SkIRect& srcRect = gReadPixelsTestRects[rect];
    for (auto tightRB : {TightRowBytes::kYes, TightRowBytes::kNo}) {
      for (size_t c = 0; c < SK_ARRAY_COUNT(gReadPixelsConfigs); ++c) {
        ...
        if (startsWithPixels) {
          fill_dst_bmp_with_init_data(&bmp);
          bool success = sContext->readPixels(
              dContext, bmp.info(), bmp.getPixels(), bmp.rowBytes(), {srcRect.fLeft, srcRect.fTop});
          ...
          REPORTER_ASSERT(
              ...
              bmp.info().alphaType());
          if (success) {
            check_read(reporter, bmp, srcRect.fLeft, srcRect.fTop, success, true, surfaceInfo);
          }
        }
      }
    }
  ...
  for (auto origin : {kBottomLeft_GrSurfaceOrigin, kTopLeft_GrSurfaceOrigin}) {
    for (auto renderable : {GrRenderable::kNo, GrRenderable::kYes}) {
      ...
      auto sContext = GrSurfaceContext::Make(
          dContext, std::move(view), grColorType, kPremul_SkAlphaType, nullptr);
...
static const void* five_reference_pixels(SkColorType colorType) {
  switch (colorType) {
    case kUnknown_SkColorType: return nullptr;
    case kAlpha_8_SkColorType: return alpha8;
    case kRGB_565_SkColorType: return rgb565;
    case kARGB_4444_SkColorType: return rgba4444;
    case kRGBA_8888_SkColorType: return rgba;
    case kBGRA_8888_SkColorType: return bgra;
...
static void test_conversion(
    skiatest::Reporter* r, const SkImageInfo& dstInfo, const SkImageInfo& srcInfo) {
  ...
  if (success) {
    if (kGray_8_SkColorType == srcInfo.colorType() && kGray_8_SkColorType != dstInfo.colorType()) {
      // TODO: test (r,g,b) == (gray,gray,gray)?
      return;
    }

    if (kGray_8_SkColorType == dstInfo.colorType() && kGray_8_SkColorType != srcInfo.colorType()) {
    ...
    REPORTER_ASSERT(
        r, 0 == memcmp(
                    dstPixels, five_reference_pixels(dstInfo.colorType()),
...
DEF_TEST(ReadPixels_ValidConversion, reporter) {
  const SkColorType kColorTypes[] = {
      ...
      kRGBA_8888_SkColorType, kBGRA_8888_SkColorType, kGray_8_SkColorType,  kRGBA_F16_SkColorType,
  };
        ...
        for (SkColorType srcCT : kColorTypes) {
          for (SkAlphaType srcAT : kAlphaTypes) {
            for (const sk_sp<SkColorSpace>& srcCS : kColorSpaces) {
              test_conversion(
                  reporter, SkImageInfo::Make(kNumPixels, 1, dstCT, dstAT, dstCS),
  ...
  auto runTest = [&](const T& src, const SkPixmap& srcPixels, const SkImageInfo& readInfo,
    ...
    if (!guardOk) {
      ERRORF(
          ...
          rect.fLeft, rect.fTop, rect.fRight, rect.fBottom, ToolUtils::colortype_name(srcCT),
          ToolUtils::colortype_name(readCT), csConversion);
    }
  };
  ...
  auto make_ref_f32_data = [](SkAlphaType srcAT, SkColorType srcCT) -> SkAutoPixmapStorage {
    ...
    if (srcAT != kOpaque_SkAlphaType &&
      ...
      paint.setShader(SkGradientShader::MakeRadial(
          {kW / 2.f, kH / 2.f}, (kW + kH) / 10.f, kColors3, kPos3, 5, SkTileMode::kMirror));
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SurfaceAsyncReadPixels, reporter, ctxInfo) {
  ...
  auto reader = std::function<GpuReadSrcFn<Surface>>(
      [](const Surface& surface, const SkIVector& offset, const SkPixmap& pixels) {
        ...
        surface->asyncRescaleAndReadPixels(
            pixels.info(), rect, SkImage::RescaleGamma::kSrc, kNone_SkFilterQuality, async_callback,
            &context);
        direct->submit();
  ...
  for (GrSurfaceOrigin origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
    auto factory = std::function<GpuSrcFactory<Surface>>([context = ctxInfo.directContext(),
      ...
      auto surf =
          SkSurface::MakeRenderTarget(context, SkBudgeted::kYes, src.info(), 0, origin, nullptr);
      if (surf) {
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ImageAsyncReadPixels, reporter, ctxInfo) {
  ...
  auto reader = std::function<GpuReadSrcFn<Image>>(
      [context](const Image& image, const SkIVector& offset, const SkPixmap& pixels) {
      ...
      for (bool yuv : {false, true}) {
        ...
        if (yuv) {
          surf->asyncRescaleAndReadPixelsYUV420(
              ...
              SkImage::RescaleGamma::kSrc, kNone_SkFilterQuality, &async_callback, &cbContext);
        } else {
          surf->asyncRescaleAndReadPixels(
              ii, ii.bounds(), SkImage::RescaleGamma::kSrc, kNone_SkFilterQuality, &async_callback,
              &cbContext);
        }
        direct->submit();
