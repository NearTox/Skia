...
static void test_readpixels_texture(
    skiatest::Reporter* reporter, std::unique_ptr<GrSurfaceContext> sContext,
      ...
      for (size_t c = 0; c < SK_ARRAY_COUNT(gReadPixelsConfigs); ++c) {
        ...
        if (startsWithPixels) {
          ...
          bool success = sContext->readPixels(
              bmp.info(), bmp.getPixels(), bmp.rowBytes(), {srcRect.fLeft, srcRect.fTop});
  ...
  for (auto origin : {kBottomLeft_GrSurfaceOrigin, kTopLeft_GrSurfaceOrigin}) {
    for (auto renderable : {GrRenderable::kNo, GrRenderable::kYes}) {
      ...
      auto sContext = context->priv().makeWrappedSurfaceContext(
          std::move(proxy), SkColorTypeToGrColorType(bmp.colorType()), kPremul_SkAlphaType);
...
template <typename T>
using GpuReadSrcFn = bool(const T&, const SkIVector& offset, const SkPixmap&);
  ...
  auto runTest = [&](const T& src, const SkPixmap& srcPixels, const SkImageInfo& readInfo,
    ...
    if (success && srcReadRect.intersect(surfBounds, rect)) {
      ...
      if (readAT != kOpaque_SkAlphaType && srcAT != kOpaque_SkAlphaType) {
        const int alphaBits = std::min(alpha_channel_bits(readCT), alpha_channel_bits(srcCT));
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(AsyncReadPixels, reporter, ctxInfo) {
  ...
  auto reader = std::function<GpuReadSrcFn<Surface>>(
      [](const Surface& surface, const SkIVector& offset, const SkPixmap& pixels) {
        ...
        surface->asyncRescaleAndReadPixels(
            pixels.info(), rect, SkSurface::RescaleGamma::kSrc, kNone_SkFilterQuality,
  ...
  for (GrSurfaceOrigin origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
    auto factory = std::function<GpuSrcFactory<Surface>>([context = ctxInfo.grContext(),
      ...
      for (bool yuv : {false, true}) {
        ...
        if (yuv) {
          surf->asyncRescaleAndReadPixelsYUV420(
              ...
              SkSurface::RescaleGamma::kSrc, kNone_SkFilterQuality, &async_callback, &cbContext);
        } else {
          surf->asyncRescaleAndReadPixels(
              ii, ii.bounds(), SkSurface::RescaleGamma::kSrc, kNone_SkFilterQuality,
