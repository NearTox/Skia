...
bool check_success_expectation(ReadSuccessExpectation expectation, bool actualSuccess) {
  switch (expectation) {
    case ReadSuccessExpectation::kMaybe: return true;
    case ReadSuccessExpectation::kNo: return !actualSuccess;
    case ReadSuccessExpectation::kYes: return actualSuccess;
  }
  return false;
}

ReadSuccessExpectation read_should_succeed(
    const SkIRect& srcRect, const SkImageInfo& dstInfo, const SkImageInfo& srcInfo, bool isGPU) {
...
static void test_readpixels_texture(
    skiatest::Reporter* reporter, sk_sp<GrSurfaceContext> sContext,
      ...
      for (size_t c = 0; c < SK_ARRAY_COUNT(gReadPixelsConfigs); ++c) {
        ...
        if (startsWithPixels) {
          ...
          bool success = sContext->readPixels(
              bmp.info(), bmp.getPixels(), bmp.rowBytes(), srcRect.fLeft, srcRect.fTop, flags);
...
static void compare_pixmaps(
    const SkPixmap& a, const SkPixmap& b, const float tol[4],
  ...
  SkConvertPixels(
      afloat.info(), afloat.writable_addr(), afloat.rowBytes(), a.info(), a.addr(), a.rowBytes());
  SkConvertPixels(
      bfloat.info(), bfloat.writable_addr(), bfloat.rowBytes(), b.info(), b.addr(), b.rowBytes());
      ...
      for (const auto& rect : {SkIRect::MakeWH(kW, kH),                  // full size
        ...
        for (int rct = 0; rct <= kLastEnum_SkColorType; ++rct) {
          ...
          for (const sk_sp<SkColorSpace>& readCS :
            ...
            info =
                SkImageInfo::Make(rect.width(), rect.height(), readCT, kPremul_SkAlphaType, readCS);
            result.alloc(info);
            ...
            surf->asyncRescaleAndReadPixels(
                info, rect, SkSurface::RescaleGamma::kSrc, kNone_SkFilterQuality, callback,
                &context);
            ...
            auto error =
                std::function<ComparePixmapsErrorReporter>([&](int x, int y, const float diffs[4]) {
                  ERRORF(
                      ...
                      surfCT, readCT, rect.fLeft, rect.fTop, rect.fRight, rect.fBottom, origin,
