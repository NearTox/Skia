...
DEF_TEST(SkSLMetalHelloWorld, r) {
  test(
      ...
      "};\n"
      "fragment Outputs fragmentMain(Inputs _in [[stage_in]], bool _frontFacing [[front_facing]], "
...
DEF_TEST(SkSLMetalMatrices, r) {
  test(
      ...
      *SkSL::ShaderCapsFactory::Default(),
      ...
      "4.0))[0][0] + float2x2_from_float4(float4(0.0))[0][0]) + float2x2_from_float4(float4(1.0, "
      "2.0, 3.0, 4.0))[0][0]) + float2x2_from_float(1.0)[0][0]) + m5[0][0]) + float2x2(float2(1.0, "
      "2.0), float2(3.0, 4.0))[0][0]) + float2x2(float2(5.0, 6.0), float2(7.0, 8.0))[0][0]) + "
