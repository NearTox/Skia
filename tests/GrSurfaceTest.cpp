  ...
  auto createTexture = [](int width, int height, GrColorType colorType,
    ...
    if (compressed) {
      ...
      switch (config) {
        ...
        default: SK_ABORT("Unexpected config");
      }
      ...
      return rp->createCompressedTexture(
          width, height, format, SkImage::kETC1_CompressionType, SkBudgeted::kNo, data.get());
    } else {
      GrSurfaceDesc desc;
      desc.fWidth = width;
      desc.fHeight = height;
    ...
    for (GrSurfaceOrigin origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
      ...
      // support check is working
      {
        ...
        sk_sp<GrSurface> tex = createTexture(
            kW, kH, combo.fColorType, combo.fFormat, GrRenderable::kNo, resourceProvider);
        REPORTER_ASSERT(
        ...
        bool expectedMipMapability =
            isTexturable && caps->mipMapSupport() && !caps->isFormatCompressed(combo.fFormat);
      ...
      // Check if 'isFormatAsColorTypeRenderable' agrees with 'createTexture' (w/o MSAA)
      {
        ...
        sk_sp<GrSurface> tex = resourceProvider->createTexture(
            desc, combo.fFormat, GrRenderable::kYes, 1, SkBudgeted::kNo, GrProtected::kNo,
            GrResourceProvider::Flags::kNone);
        REPORTER_ASSERT(
      ...
      // Check if 'isFormatAsColorTypeRenderable' agrees with 'createTexture' w/ MSAA
      {
        ...
        sk_sp<GrSurface> tex = resourceProvider->createTexture(
            desc, combo.fFormat, GrRenderable::kYes, 2, SkBudgeted::kNo, GrProtected::kNo,
            GrResourceProvider::Flags::kNone);
        REPORTER_ASSERT(
            reporter, SkToBool(tex) == isRenderable, "ct:%s format:%s, tex:%d, isRenderable:%d",
            GrColorTypeToStr(combo.fColorType), combo.fFormat.toStr().c_str(), SkToBool(tex),
...
DEF_GPUTEST(InitialTextureClear, reporter, baseOptions) {
  ...
  for (int ct = 0; ct < sk_gpu_test::GrContextFactory::kContextTypeCnt; ++ct) {
    ...
    for (auto combo : combos) {
      ...
      {
        GrPixelConfig config = caps->getConfigFromBackendFormat(combo.fFormat, combo.fColorType);
      ...
      uint32_t expectedClearColor = (components & kAlpha_SkColorTypeComponentFlag) ? 0 : 0xFF000000;
              ...
              if (texCtx->readPixels(
                ...
                for (int i = 0; i < kSize * kSize; ++i) {
                  if (expectedClearColor != readback.addr32()[i]) {
                    ERRORF(
                        reporter, "Failed on ct %s format %s 0x%x != 0x%x",
          ...
          // Try creating the texture as a deferred proxy.
          {
            ...
            if (renderable == GrRenderable::kYes) {
              surfCtx = context->priv().makeDeferredRenderTargetContext(
                  fit, desc.fWidth, desc.fHeight, combo.fColorType, nullptr, 1, GrMipMapped::kNo,
                  kTopLeft_GrSurfaceOrigin, nullptr);
            } else {
              surfCtx = context->priv().makeDeferredTextureContext(
                  fit, desc.fWidth, desc.fHeight, combo.fColorType, kUnknown_SkAlphaType, nullptr,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ReadOnlyTexture, reporter, context_info) {
  ...
  for (auto ioType : {kRead_GrIOType, kRW_GrIOType}) {
    ...
    auto copySrc = proxyProvider->createTextureProxy(
        SkImage::MakeFromRaster(write, nullptr, nullptr), 1, SkBudgeted::kYes,
        SkBackingFit::kExact);
