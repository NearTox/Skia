...
DEF_GPUTEST_FOR_ALL_CONTEXTS(GrSurfaceRenderability, reporter, ctxInfo) {
  ...
  auto createTexture = [](int width, int height, GrPixelConfig config, GrRenderable renderable,
    ...
    if (GrPixelConfigIsCompressed(config)) {
      ...
      switch (config) {
        case kRGB_ETC1_GrPixelConfig: type = SkImage::kETC1_CompressionType; break;
        default: SK_ABORT("Unexpected config"); return nullptr;
      }
      ...
      return rp->createCompressedTexture(
          width, height, SkImage::kETC1_CompressionType, SkBudgeted::kNo, data.get());
    } else {
      GrSurfaceDesc desc;
      desc.fWidth = width;
      desc.fHeight = height;
  ...
  for (int c = 0; c <= kLast_GrPixelConfig; ++c) {
    ...
    if (config == kAlpha_8_as_Alpha_GrPixelConfig || config == kAlpha_8_as_Red_GrPixelConfig ||
        config == kGray_8_as_Lum_GrPixelConfig || config == kGray_8_as_Red_GrPixelConfig ||
        config == kAlpha_half_as_Red_GrPixelConfig || config == kAlpha_half_as_Lum_GrPixelConfig) {
      continue;
    }
    ...
    for (GrSurfaceOrigin origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
      ...
      sk_sp<GrSurface> tex = createTexture(kW, kH, config, GrRenderable::kNo, resourceProvider);
      REPORTER_ASSERT(
          reporter, SkToBool(tex) == ict, "config:%d, tex:%d, isConfigTexturable:%d", config,
          SkToBool(tex), ict);
      ...
      REPORTER_ASSERT(
          reporter,
          SkToBool(proxy.get()) == (caps->isConfigTexturable(desc.fConfig) &&
      ...
      REPORTER_ASSERT(
          reporter, SkToBool(tex) == isRenderable, "config:%d, tex:%d, isRenderable:%d", config,
          SkToBool(tex), isRenderable);

      tex = resourceProvider->createTexture(
          desc, GrRenderable::kYes, 2, SkBudgeted::kNo, GrProtected::kNo,
          GrResourceProvider::Flags::kNoPendingIO);
        ...
        for (GrSurfaceOrigin origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
          for (auto fit : {SkBackingFit::kApprox, SkBackingFit::kExact}) {
            auto proxy = proxyProvider->testingOnly_createInstantiatedProxy(
                desc, renderable, 1, origin, fit, SkBudgeted::kYes, GrProtected::kNo);
            if (!proxy) {
              continue;
            }
            auto texCtx = context->priv().makeWrappedSurfaceContext(
                std::move(proxy), GrPixelConfigToColorType(desc.fConfig), kPremul_SkAlphaType);
            SkImageInfo info =
            ...
            if (texCtx->readPixels(info, data.get(), 0, {0, 0})) {
              ...
              for (int i = 0; i < kSize * kSize; ++i) {
                if (cmp != data.get()[i]) {
                  ERRORF(
                      reporter, "Failed on conf %d (pix %d) 0x%x != 0x%x", desc.fConfig, i, cmp,
                      data.get()[i]);
                  break;
                }
              }
            ...
            for (int i = 0; i < 2; ++i) {
              ...
              if (renderable == GrRenderable::kYes) {
                surfCtx = context->priv().makeDeferredRenderTargetContext(
                    fit, desc.fWidth, desc.fHeight, colorType, nullptr, 1, GrMipMapped::kNo, origin,
                    nullptr);
              } else {
                surfCtx = context->priv().makeDeferredTextureContext(
                    fit, desc.fWidth, desc.fHeight, colorType, kUnknown_SkAlphaType, nullptr,
                    GrMipMapped::kNo, origin);
              }
              if (!surfCtx) {
                continue;
              }
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ReadOnlyTexture, reporter, context_info) {
  ...
  for (auto ioType : {kRead_GrIOType, kRW_GrIOType}) {
    auto backendTex =
        context->priv().createBackendTexture(&srcPixmap, 1, GrRenderable::kYes, GrProtected::kNo);
    ...
    auto surfContext = context->priv().makeWrappedSurfaceContext(
        proxy, GrColorType::kRGBA_8888, kPremul_SkAlphaType);
    ...
    auto copySrc = proxyProvider->createTextureProxy(
        SkImage::MakeFromRaster(write, nullptr, nullptr), GrRenderable::kNo, 1, SkBudgeted::kYes,
        SkBackingFit::kExact);
    ...
    auto gpuCopyResult = context->priv().getGpu()->copySurface(
        proxy->peekTexture(), copySrc->peekTexture(), SkIRect::MakeWH(kSize, kSize), {0, 0});
  ...
  for (const auto& m : makers) {
    for (const auto& keyAdder : keyAdders) {
      for (int type = 0; type < sk_gpu_test::GrContextFactory::kContextTypeCnt; ++type) {
        ...
        rtc->drawTexture(
            GrNoClip(), proxy, GrSamplerState::Filter::kNearest, SkBlendMode::kSrcOver,
            SkPMColor4f(), SkRect::MakeWH(w, h), SkRect::MakeWH(w, h), GrAA::kNo,
            GrQuadAAFlags::kNone, SkCanvas::kFast_SrcRectConstraint, SkMatrix::I(), nullptr);
        ...
        rtc->drawTexture(
            GrNoClip(), std::move(proxy), GrSamplerState::Filter::kNearest, SkBlendMode::kSrcOver,
            SkPMColor4f(), SkRect::MakeWH(w, h), SkRect::MakeWH(w, h), GrAA::kNo,
            GrQuadAAFlags::kNone, SkCanvas::kFast_SrcRectConstraint, SkMatrix::I(), nullptr);
        REPORTER_ASSERT(reporter, idleIDs.find(2) == idleIDs.end());
        context->flush();
