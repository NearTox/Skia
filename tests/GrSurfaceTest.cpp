      ...
      for (auto renderable : {GrRenderable::kNo, GrRenderable::kYes}) {
        ...
        for (auto fit : {SkBackingFit::kApprox, SkBackingFit::kExact}) {
          ...
          // Try creating the texture as a deferred proxy.
          {
            GrImageInfo info(
                combo.fColorType,
                GrColorTypeHasAlpha(combo.fColorType) ? kPremul_SkAlphaType : kOpaque_SkAlphaType,
                nullptr, {desc.fHeight, desc.fHeight});

            auto sc = dContext->priv().makeSC(
                info, combo.fFormat, fit, kTopLeft_GrSurfaceOrigin, renderable);
            if (!sc) {
              continue;
            }
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ReadOnlyTexture, reporter, context_info) {
  ...
  for (auto ioType : {kRead_GrIOType, kRW_GrIOType}) {
    ...
    auto gpuWriteResult = dContext->priv().getGpu()->writePixels(
        proxy->peekTexture(), SkIRect::MakeWH(kSize, kSize), GrColorType::kRGBA_8888,
