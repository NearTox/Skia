...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(EGLImageTest, reporter, ctxInfo) {
  ...
  GrSwizzle swizzle = context0->priv().caps()->getReadSwizzle(texProxy->backendFormat(), colorType);
  ...
  TestCopyFromSurface(
      reporter, context0, surfaceContext->asSurfaceProxy(), surfaceContext->origin(), colorType,
