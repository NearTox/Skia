...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(EGLImageTest, reporter, ctxInfo) {
  ...
  sk_sp<GrTextureContext> surfaceContext =
      context0->priv().makeBackendTextureContext(backendTex, kBottomLeft_GrSurfaceOrigin, nullptr);
  ...
  test_copy_from_surface(
      reporter, context0, surfaceContext->asSurfaceProxy(), pixels.get(), true,
      "EGLImageTest-copy");
