...
static void cleanup(
    GLTestContext* glctx0, GrGLuint texID0, GLTestContext* glctx1, sk_sp<GrContext> grctx1,
    GrBackendTexture* backendTex1, GrEGLImage image1) {
  if (glctx1) {
    glctx1->makeCurrent();
    if (grctx1) {
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(EGLImageTest, reporter, ctxInfo) {
  ...
  backendTexture1 = context1->createBackendTexture(
  ...
  GR_GL_CALL(
      glCtx1->gl(),
      TexSubImage2D(
          texInfo.fTarget, 0, 0, 0, kSize, kSize, GR_GL_RGBA, GR_GL_UNSIGNED_BYTE, pixels.get()));
  GR_GL_CALL(glCtx1->gl(), Finish());
  ...
  sk_sp<GrSurfaceProxy> texProxy = context0->priv().proxyProvider()->wrapBackendTexture(
      backendTex, colorType, origin, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, kRW_GrIOType);
  if (!texProxy) {
  ...
  auto surfaceContext =
      GrSurfaceContext::Make(context0, std::move(texProxy), colorType, alphaType, nullptr);
  ...
  REPORTER_ASSERT(
      reporter, proxy->peekTexture()->texturePriv().textureType() == GrTextureType::kExternal);
  ...
  TestCopyFromSurface(
      reporter, context0, surfaceContext->asSurfaceProxy(), colorType, pixels.get(),
      "EGLImageTest-copy");
