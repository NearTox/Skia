...
static void cleanup(
    GLTestContext* glctx0, GrGLuint texID0, GLTestContext* glctx1, sk_sp<GrDirectContext> dContext,
    GrEGLImage image1) {
  if (glctx1) {
    glctx1->makeCurrent();
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(EGLImageTest, reporter, ctxInfo) {
  ...
  auto mbet = sk_gpu_test::ManagedBackendTexture::MakeWithoutData(
      context1.get(), kSize, kSize, kRGBA_8888_SkColorType, GrMipmapped::kNo, GrRenderable::kNo,
      GrProtected::kNo);
  ...
  GrSwizzle swizzle =
      context0->priv().caps()->getReadSwizzle(texProxy->backendFormat(), colorInfo.colorType());
  ...
  {
    auto temp = GrSurfaceFillContext::MakeFromBackendTexture(
        context0, colorInfo, backendTex, 1, origin,
  ...
  TestCopyFromSurface(
      reporter, context0, surfaceContext->asSurfaceProxy(), surfaceContext->origin(),
      colorInfo.colorType(), pixels.get(), "EGLImageTest-copy");
