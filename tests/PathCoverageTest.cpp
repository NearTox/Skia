...
static const int gSawtooth[] = {0, 0,  10,  10, 20,  20, 30,  10, 40,
                                0, 50, -10, 60, -20, 70, -10, 80, 0};

static const int gOvalish[] = {0, 0, 5, 15, 20, 20, 35, 15, 40, 0};

static const int gSharpSawtooth[] = {0, 0, 1, 10, 2, 0, 3, -10, 4, 0};

// Curve crosses back over itself around 0,10
static const int gRibbon[] = {-4, 0, 4, 20, 0, 25, -4, 20, 4, 0};
...
static bool one_d_pe(const int* array, const unsigned int count, skiatest::Reporter* reporter) {
  ...
  for (unsigned i = 4; i < count; i += 2) {
    path[0] = path[1];
    path[1] = path[2];
    path[2] = SkPoint::Make(SkIntToScalar(array[i]), SkIntToScalar(array[i + 1]));
    ...
    if (!isAccurate) {
      ERRORF(
          reporter,
          "Curve from %.2f %.2f through %.2f %.2f to "
          "%.2f %.2f computes %d, estimates %d\n",
          path[0].fX, path[0].fY, path[1].fX, path[1].fY, path[2].fX, path[2].fY, computedCount,
          estimatedCount);
      numErrors++;
    }
  }
...
DEF_TEST(PathCoverage, reporter) { TestQuadPointCount(reporter); }
