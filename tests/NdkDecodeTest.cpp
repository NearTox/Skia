...
DEF_TEST(NdkDecode, r) {
  static const struct {
    ...
    SkISize fSize;
      ...
      {"images/CMYK.jpg", {642, 516}},          {"images/arrow.png", {187, 312}},
      {"images/baby_tux.webp", {386, 395}},     {"images/color_wheel.gif", {128, 128}},
      {"images/rle.bmp", {320, 240}},           {"images/color_wheel.ico", {128, 128}},
      {"images/google_chrome.ico", {256, 256}}, {"images/mandrill.wbmp", {512, 512}},
  };
  for (auto& rec : recs) {
...
DEF_TEST(NdkDecode_ColorSpace, r) {
  for (const char* path : {
    ...
    for (auto gamut :
         {SkNamedGamut::kSRGB, SkNamedGamut::kAdobeRGB, SkNamedGamut::kDisplayP3,
    ...
    for (auto gamut : {SkNamedGamut::kAdobeRGB, SkNamedGamut::kDisplayP3, SkNamedGamut::kXYZ}) {
      unsupportedCs.push_back(SkColorSpace::MakeRGB(SkNamedTransferFn::kRec2020, gamut));
    }

    for (auto gamut :
         {SkNamedGamut::kAdobeRGB, SkNamedGamut::kDisplayP3, SkNamedGamut::kRec2020,
          SkNamedGamut::kXYZ}) {
      unsupportedCs.push_back(SkColorSpace::MakeRGB(SkNamedTransferFn::kLinear, gamut));
    }

    for (auto gamut : {SkNamedGamut::kAdobeRGB, SkNamedGamut::kRec2020, SkNamedGamut::kXYZ}) {
  ...
  static const struct {
    ...
    bool fIsOpaque;
      ...
      {"images/example_1.png",
       SkColorSpace::MakeRGB(SkNamedTransferFn::k2Dot2, SkNamedGamut::kAdobeRGB), false},
  };
  for (auto& rec : recs) {
    auto gen = make_generator(rec.fPath, r);
    if (!gen) continue;
...
DEF_TEST(NdkDecode_webpArbitraryDownscale, r) {
  for (const char* path : {
    ...
    const int height = actualDimensions.height();
    for (SkISize dims :
         {SkISize{width / 2, height / 2}, SkISize{width / 4, height / 4},
          SkISize{width / 7, height / 7}, SkISize{width - 1, height - 1}, SkISize{1, 1},
          SkISize{5, 20}}) {
      auto info = gen->getInfo().makeDimensions(dims);
      SkBitmap bm;
      bm.allocPixels(info);
...
DEF_TEST(NdkDecode_Gray8, r) {
  static const struct {
    ...
    bool fGrayscale;
      ...
      {"images/CMYK.jpg", false},          {"images/arrow.png", false},
      {"images/baby_tux.webp", false},     {"images/color_wheel.gif", false},
      {"images/rle.bmp", false},           {"images/color_wheel.ico", false},
      {"images/google_chrome.ico", false}, {"images/mandrill.wbmp", true},
      {"images/grayscale.jpg", true},      {"images/grayscale.png", true},
  };
  for (auto& rec : recs) {
    auto gen = make_generator(rec.fPath, r);
    if (!gen) continue;
    ...
    if (success != rec.fGrayscale) {
      ERRORF(
          r, "Expected decoding %s to Gray8 to %s. Actual: %s\n", rec.fPath,
