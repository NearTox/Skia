...
DEF_TEST(AnimatedImage_rotation, r) {
  ...
  for (int i = 1; i <= 8; i++) {
    for (const SkString& name :
         {SkStringPrintf("images/orientation/%d.webp", i),
...
static bool compare_bitmaps(
    skiatest::Reporter* r, const char* file, int expectedFrame, const SkBitmap& expectedBm,
...
DEF_TEST(AnimatedImage, r) {
  ...
  for (const char* file : {
    ...
    auto testDraw = [r, &frames, &imageInfo, file](
                        const sk_sp<SkAnimatedImage>& animatedImage, int expectedFrame) {
      SkBitmap test;
      test.allocPixels(imageInfo);
      test.eraseColor(0);
      SkCanvas c(test);
    ...
    for (int j = 0; j < (int)frameInfos.size(); ++j) {
      ...
      for (int i = 0; i <= j; ++i) {
        if (i == j) {
          animatedImage->reset();
          if (!testDraw(animatedImage, 0)) {
            ERRORF(r, "reset failed for image %s from frame %i", file, i);
            failed = true;
            break;
          }
        } else if (i != 0) {
          ...
          if (!testDraw(animatedImage, i)) {
            ERRORF(r, "failed to match frame %i in %s on iteration %i", i, file, j);
            failed = true;
            break;
          }
        }
    ...
    for (int loopCount : {0, 1, 2, 5}) {
      animatedImage =
          SkAnimatedImage::Make(SkAndroidCodec::MakeFromCodec(SkCodec::MakeFromData(data)));
      ...
      for (int loops = 0; loops <= loopCount; loops++) {
        ...
        for (size_t i = 1; i <= frameInfos.size(); ++i) {
          ...
          if (frameTime == SkAnimatedImage::kFinished) {
            if (loops != loopCount) {
              ERRORF(
                  r, "%s animation stopped early: loops: %i\tloopCount: %i", file, loops,
                  loopCount);
              failed = true;
            }
            if (i != frameInfos.size() - 1) {
              ERRORF(
                  r, "%s animation stopped early: i: %zu\tsize: %zu", file, i, frameInfos.size());
              failed = true;
            }
            break;
          }
      ...
      if (!animatedImage->isFinished()) {
        ERRORF(
            r, "%s animation should have finished with specified loop count (%i)", file, loopCount);
      }
    }
  }
}
