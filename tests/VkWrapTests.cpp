...
void wrap_tex_test(skiatest::Reporter* reporter, GrDirectContext* dContext) {
  ...
  CreateBackendTexture(
      dContext, &origBackendTex, kW, kH, kColorType, SkColors::kTransparent, GrMipmapped::kNo,
  ...
  sk_sp<GrTexture> tex = gpu->wrapBackendTexture(
      origBackendTex, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, kRead_GrIOType);
...
void wrap_trt_test(skiatest::Reporter* reporter, GrDirectContext* dContext) {
  ...
  {
    int maxSamples =
        dContext->priv().caps()->maxRenderTargetSampleCount(origBackendTex.getBackendFormat());
    bool shouldSucceed = maxSamples > 1;
    tex = gpu->wrapRenderableBackendTexture(
        origBackendTex, 2, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo);
