...
void wrap_tex_test(skiatest::Reporter* reporter, GrContext* context) {
  ...
  GrBackendTexture origBackendTex = context->createBackendTexture(
      kW, kH, kColorType, SkColors::kTransparent, GrMipMapped::kNo, GrRenderable::kNo,
      GrProtected::kNo);
  GrVkImageInfo imageInfo;
  SkAssertResult(origBackendTex.getVkImageInfo(&imageInfo));

  sk_sp<GrTexture> tex = gpu->wrapBackendTexture(
      origBackendTex, kGrColorType, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, kRead_GrIOType);
  ...
  // image is null
  {
    ...
    tex = gpu->wrapBackendTexture(
        backendTex, kGrColorType, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, kRead_GrIOType);
    REPORTER_ASSERT(reporter, !tex);
    tex = gpu->wrapBackendTexture(
        backendTex, kGrColorType, kAdopt_GrWrapOwnership, GrWrapCacheable::kNo, kRead_GrIOType);
    REPORTER_ASSERT(reporter, !tex);
  }
...
void wrap_rt_test(skiatest::Reporter* reporter, GrContext* context) {
  ...
  GrBackendTexture origBackendTex = context->createBackendTexture(
      kW, kH, kColorType, SkColors::kTransparent, GrMipMapped::kNo, GrRenderable::kYes,
      GrProtected::kNo);
...
void wrap_trt_test(skiatest::Reporter* reporter, GrContext* context) {
  ...
  // image is null
  {
    ...
    backendCopy.fImage = VK_NULL_HANDLE;
    GrBackendTexture backendTex = GrBackendTexture(kW, kH, backendCopy);
    tex = gpu->wrapRenderableBackendTexture(
        backendTex, 1, kGrColorType, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo);
    REPORTER_ASSERT(reporter, !tex);
    tex = gpu->wrapRenderableBackendTexture(
        backendTex, 1, kGrColorType, kAdopt_GrWrapOwnership, GrWrapCacheable::kNo);
    REPORTER_ASSERT(reporter, !tex);
  }
