...
static void test_partial(
    skiatest::Reporter* r, const char* name, const sk_sp<SkData>& file, size_t minBytes,
    size_t increment) {
  SkBitmap truth;
  ...
  while (true) {
    const SkCodec::Result startResult =
        partialCodec->startIncrementalDecode(info, incremental.getPixels(), incremental.rowBytes());
...
DEF_TEST(Codec_frameCountUpdatesInIncrementalDecode, r) {
  ...
  REPORTER_ASSERT(
      r, SkCodec::kSuccess ==
             partialCodec->startIncrementalDecode(info, bitmap.getPixels(), bitmap.rowBytes()));
  REPORTER_ASSERT(r, SkCodec::kIncompleteInput == partialCodec->incrementalDecode());
...
DEF_TEST(Codec_requiredFrame, r) {
  ...
  while (true) {
    ...
    for (; frameToCompare < partialInfo.size(); frameToCompare++) {
      REPORTER_ASSERT(
          r,
          partialInfo[frameToCompare].fRequiredFrame == frameInfo[frameToCompare].fRequiredFrame);
    }
...
DEF_TEST(Codec_partialAnim, r) {
  ...
  for (size_t i = 0; true; i++) {
    ...
    const SkCodec::Result result =
        fullCodec->getPixels(info, frame.getPixels(), frame.rowBytes(), &opts);
  ...
  std::unique_ptr<SkCodec> partialCodec(
      SkCodec::MakeFromStream(std::unique_ptr<SkStream>(haltingStream)));
  if (!partialCodec) {
    ERRORF(
        r, "Failed to create a partial codec from %s with %i bytes out of %i", path,
        frameByteCounts[0], file->size());
    return;
  }
  ...
  for (size_t i = 0; i < frames.size(); i++) {
    ...
    SkCodec::Result result =
        partialCodec->startIncrementalDecode(info, frame.getPixels(), frame.rowBytes(), &opts);
    if (result != SkCodec::kSuccess) {
      ERRORF(r, "Failed to start incremental decode for %s on frame %i", path, i);
      return;
    }
    ...
    for (size_t len = 14; len <= file->size(); len += 5) {
      ...
      if (codec) {
        if (result != SkCodec::kSuccess) {
          ERRORF(
              r,
              "Created an SkCodec for %s with %lu bytes, but "
              "reported an error %i",
              name, len, result);
        }
        break;
      ...
      if (SkCodec::kIncompleteInput != result) {
        ERRORF(r, "Reported error %i for %s with %lu bytes", result, name, len);
        break;
      }
    }
  }
