...
DEF_TEST(Codec_partialAnim, r) {
  ...
  if (!partialCodec) {
    ERRORF(
        r, "Failed to create a partial codec from %s with %zu bytes out of %zu", path,
  ...
  for (size_t i = 0; i < frames.size(); i++) {
    ...
    if (result != SkCodec::kSuccess) {
      ERRORF(r, "Failed to start incremental decode for %s on frame %zu", path, i);
      return;
    }
  ...
  for (const char* name : {
    ...
    for (size_t len = 14; len <= file->size(); len += 5) {
      ...
      if (SkCodec::kIncompleteInput != result) {
        ERRORF(r, "Reported error %i for %s with %zu bytes", (int)result, name, len);
        break;
      }
    }
  }
