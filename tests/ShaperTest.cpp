...
struct RunHandler final : public SkShaper::RunHandler {
  ...
  SkShaper::RunHandler::Buffer runBuffer(const SkShaper::RunHandler::RunInfo& info) override {
    ...
    return SkShaper::RunHandler::Buffer{
        fGlyphs.get(), fPositions.get(), nullptr, fClusters.get(), {0, 0}};
  }
  void commitRunBuffer(const RunInfo& info) override {
    ...
    for (unsigned i = 0; i < fGlyphCount; ++i) {
      REPORTER_ASSERT(
          fReporter, fClusters[i] >= fRange.begin(), "%s %u %u", fResource, i, fGlyphCount);
      REPORTER_ASSERT(
          fReporter, fClusters[i] < fRange.end(), "%s %u %u", fResource, i, fGlyphCount);
    }
  }
...
void shaper_test(skiatest::Reporter* reporter, const char* name, SkData* data) {
  ...
  shaper->shape(
      (const char*)data->data(), data->size(), fontIterator, bidiIterator, scriptIterator,
      languageIterator, kWidth, &rh);
}
...
#  define SHAPER_TEST(X) \
    DEF_TEST(Shaper_cluster_##X, r) { cluster_test(r, "text/" #X ".txt"); }
SHAPER_TEST(arabic)
