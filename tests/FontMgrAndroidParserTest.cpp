...
static void ValidateLoadedFonts(
    SkTDArray<FontFamily*> fontFamilies, const char* firstExpectedFile,
  ...
  REPORTER_ASSERT(
      reporter, !strcmp(fontFamilies[0]->fFonts[0].fFileName.c_str(), firstExpectedFile));
  ...
  for (int i = 0; i < fontFamilies.count(); ++i) {
    ...
    for (int j = 0; j < family.fFonts.count(); ++j) {
      ...
      REPORTER_ASSERT(
          reporter,
          !file.fFileName.isEmpty() && file.fFileName[0] >= 'A' && file.fFileName[0] <= 'Z');
    }
  }
}
...
static void DumpFiles(const FontFamily& fontFamily) {
  for (int j = 0; j < fontFamily.fFonts.count(); ++j) {
    ...
    for (const auto& coordinate : ffi.fVariationDesignPosition) {
      SkDebugf(
          " @'%c%c%c%c'=%f", (coordinate.axis >> 24) & 0xFF, (coordinate.axis >> 16) & 0xFF,
          (coordinate.axis >> 8) & 0xFF, (coordinate.axis) & 0xFF, coordinate.value);
    }
    SkDebugf("\n");
  }
}
...
static double test_parse_fixed_r(
    skiatest::Reporter* reporter, double low, double high, double inc) {
...
DEF_TEST(FontMgrAndroidParser, reporter) {
  ...
  SkFontMgr_Android_Parser::GetCustomFontFamilies(
      preV17FontFamilies, SkString("/custom/font/path/"),
  ...
  SkFontMgr_Android_Parser::GetCustomFontFamilies(
      v17FontFamilies, SkString("/custom/font/path/"),
  ...
  SkFontMgr_Android_Parser::GetCustomFontFamilies(
      v22FontFamilies, SkString("/custom/font/path/"),
      GetResourcePath("android_fonts/v22/fonts.xml").c_str(), nullptr);
...
DEF_TEST(FontMgrAndroidSystemVariableTypeface, reporter) {
  ...
  for (int i = 0; i < 10; ++i) {
    SkScalar styleValue = SkDoubleToScalar(0.5 + i * ((2.0 - 0.5) / 10));
    ...
    SkFont fontStream(
        fontMgr->makeFromStream(
            distortableStream->duplicate(), SkFontArguments().setVariationDesignPosition(position)),
        kTextSize);
...
DEF_TEST(FontMgrAndroidSystemFallbackFor, reporter) {
  ...
  sk_sp<SkTypeface> typeface(
      fontMgr->matchFamilyStyleCharacter("sans-serif", SkFontStyle(), nullptr, 0, '!'));
