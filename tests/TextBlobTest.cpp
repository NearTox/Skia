...
class TextBlobTester {
 public:
  ...
  static void TestBuilder(skiatest::Reporter* reporter) {
    ...
    RunBuilderTest(
        reporter, builder, set5, SK_ARRAY_COUNT(set5), mergedSet5, SK_ARRAY_COUNT(mergedSet5));
    ...
    RunBuilderTest(
        reporter, builder, set6, SK_ARRAY_COUNT(set6), mergedSet6, SK_ARRAY_COUNT(mergedSet6));
    ...
    RunBuilderTest(
        reporter, builder, set7, SK_ARRAY_COUNT(set7), mergedSet7, SK_ARRAY_COUNT(mergedSet7));
  }
  ...
  static void TestPaintProps(skiatest::Reporter* reporter) {
    ...
    REPORTER_ASSERT(reporter, defaultFont.isEmbeddedBitmaps() != font.isEmbeddedBitmaps());
    ...
    AddRun(font, 1, SkTextBlobRunIterator::kHorizontal_Positioning, SkPoint::Make(0, 0), builder);
 ...
 private:
  ...
  static void RunBuilderTest(
      skiatest::Reporter* reporter, SkTextBlobBuilder& builder, const RunDef in[], unsigned inCount,
  ...
  static void AddRun(
      const SkFont& font, int count, SkTextBlobRunIterator::GlyphPositioning pos,
      const SkPoint& offset, SkTextBlobBuilder& builder, const SkRect* bounds = nullptr) {
    switch (pos) {
      case SkTextBlobRunIterator::kDefault_Positioning: {
        const SkTextBlobBuilder::RunBuffer& rb =
            builder.allocRun(font, count, offset.x(), offset.y(), bounds);
      ...
      case SkTextBlobRunIterator::kHorizontal_Positioning: {
        const SkTextBlobBuilder::RunBuffer& rb =
            builder.allocRunPosH(font, count, offset.y(), bounds);
        for (int i = 0; i < count; ++i) {
          rb.glyphs[i] = i;
      ...
      default: SK_ABORT("unhandled positioning value");
    }
  }
};
...
DEF_TEST(TextBlob_paint, reporter) { TextBlobTester::TestPaintProps(reporter); }
...
DEF_TEST(TextBlob_extended, reporter) {
  ...
  auto run = SkTextBlobBuilderPriv::AllocRunText(
      &textBlobBuilder, font, glyphCount, 0, 0, SkToInt(strlen(text2)), SkString(), nullptr);
...
static void add_run(
    SkTextBlobBuilder* builder, const char text[], SkScalar x, SkScalar y, sk_sp<SkTypeface> tf) {
  SkFont font;
...
static sk_sp<SkImage> render(const SkTextBlob* blob) {
  auto surf = SkSurface::MakeRasterN32Premul(
      SkScalarRoundToInt(blob->bounds().width()), SkScalarRoundToInt(blob->bounds().height()));
  if (!surf) {
