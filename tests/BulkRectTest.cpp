...
static sk_sp<GrSurfaceProxy> create_proxy(GrRecordingContext* rContext) {
  ...
  const GrBackendFormat format =
      rContext->priv().caps()->getDefaultBackendFormat(GrColorType::kRGBA_8888, GrRenderable::kYes);
...
typedef void (*BulkRectTest)(
    skiatest::Reporter*, GrDirectContext*, PerQuadAAFunc, GrAAType overallAA, SkBlendMode,
    bool addOneByOne, bool allUniqueProxies, int requestedTotNumQuads, int expectedNumOps);

//-------------------------------------------------------------------------------------------------
static void fillrectop_creation_test(
  ...
  skgpu::v1::FillRectOp::AddFillRectOps(
      sdc.get(), nullptr, dContext, std::move(paint), overallAA, SkMatrix::I(), quads,
...
//-------------------------------------------------------------------------------------------------
static void textureop_creation_test(
    skiatest::Reporter* reporter, GrDirectContext* dContext, PerQuadAAFunc perQuadAA,
    GrAAType overallAA, SkBlendMode blendMode, bool addOneByOne, bool allUniqueProxies,
    int requestedTotNumQuads, int expectedNumOps) {
  std::unique_ptr<skgpu::v1::SurfaceDrawContext> sdc = new_SDC(dContext);
  ...
  if (!allUniqueProxies) {
    ...
    proxyViewA = GrSurfaceProxyView(std::move(proxyA), kTopLeft_GrSurfaceOrigin, GrSwizzle::RGBA());
  ...
  for (int i = 0; i < requestedTotNumQuads; ++i) {
    if (!allUniqueProxies) {
      ...
      sk_sp<GrSurfaceProxy> proxyA = create_proxy(dContext);
      set[i].fProxyView =
          GrSurfaceProxyView(std::move(proxyA), kTopLeft_GrSurfaceOrigin, GrSwizzle::RGBA());
    }
  ...
  if (addOneByOne) {
    for (int i = 0; i < requestedTotNumQuads; ++i) {
      ...
      GrOp::Owner op = skgpu::v1::TextureOp::Make(
          ...
          skgpu::v1::TextureOp::Saturate::kYes, blendMode, overallAA, &quad, nullptr);
      sdc->addDrawOp(nullptr, std::move(op));
    }
  } else {
    skgpu::v1::TextureOp::AddTextureSetOps(
        sdc.get(), nullptr, dContext, set, requestedTotNumQuads,
        requestedTotNumQuads,  // We alternate so proxyCnt == cnt
        GrSamplerState::Filter::kNearest, GrSamplerState::MipmapMode::kNone,
        skgpu::v1::TextureOp::Saturate::kYes, blendMode, overallAA,
...
static void run_test(GrDirectContext* dContext, skiatest::Reporter* reporter, BulkRectTest test) {
  ...
  {
    auto noAA = [](int i) -> GrQuadAAFlags { return GrQuadAAFlags::kNone; };

    static const int kNumExpectedOps = 2;

    test(
        reporter, dContext, noAA, GrAAType::kNone, SkBlendMode::kSrcOver, false, false,
  ...
  // the per-quad AA is still none, all the quads should be downgraded to non-AA.
  {
    ...
    test(
        reporter, dContext, noAA, GrAAType::kCoverage, SkBlendMode::kSrcOver, false, false,
        2 * GrResourceProvider::MaxNumNonAAQuads(), kNumExpectedOps);
  }
  ...
  // We should end up with several aa-sized clumps
  {
    ...
    int numExpectedOps =
        2 * GrResourceProvider::MaxNumNonAAQuads() / GrResourceProvider::MaxNumAAQuads();

    test(
        reporter, dContext, alternateAA, GrAAType::kCoverage, SkBlendMode::kSrcOver, false, false,
  ...
  // its size. We expect one clump of non-AA quads followed by one clump of AA quads.
  {
    auto runOfNonAA = [](int i) -> GrQuadAAFlags {
      return (i < GrResourceProvider::MaxNumAAQuads()) ? GrQuadAAFlags::kNone : GrQuadAAFlags::kAll;
    };

    static const int kNumExpectedOps = 2;

    test(
        reporter, dContext, runOfNonAA, GrAAType::kCoverage, SkBlendMode::kSrcOver, false, false,
  ...
  {
    auto fixedAA = [](int i) -> GrQuadAAFlags { return GrQuadAAFlags::kAll; };

    static const int kNumExpectedOps = 2;

    test(
        reporter, dContext, fixedAA, GrAAType::kCoverage, SkBlendMode::kSrcATop, false, false,
        2 * GrResourceProvider::MaxNumAAQuads(), kNumExpectedOps);
  }
  ...
  // get chained together so the quad count can exceed the AA maximum.
  {
    ...
    test(
        reporter, dContext, onlyOneAA, GrAAType::kCoverage, SkBlendMode::kSrcOver, true, true, 1024,
        kNumExpectedOps);
  }
  ...
  // method had no way to break up the set of texture ops at the AA quad limit.
  {
    ...
    test(
        reporter, dContext, onlyOneAA, GrAAType::kCoverage, SkBlendMode::kSrcOver, false, true,
