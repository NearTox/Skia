...
static std::unique_ptr<GrRenderTargetContext> new_RTC(GrContext* context) {
  return context->priv().makeDeferredRenderTargetContext(
      SkBackingFit::kExact, 128, 128, GrColorType::kRGBA_8888, nullptr);
}
...
typedef void (*BulkRectTest)(
    skiatest::Reporter* reporter, GrContext* context, PerQuadAAFunc perQuadAA, GrAAType overallAA,
    SkBlendMode blendMode, int requestedTotNumQuads, int expectedNumOps);

//-------------------------------------------------------------------------------------------------
static void bulk_fill_rect_create_test(
    skiatest::Reporter* reporter, GrContext* context, PerQuadAAFunc perQuadAA, GrAAType overallAA,
    SkBlendMode blendMode, int requestedTotNumQuads, int expectedNumOps) {
...
//-------------------------------------------------------------------------------------------------
static void bulk_texture_rect_create_test(
    skiatest::Reporter* reporter, GrContext* context, PerQuadAAFunc perQuadAA, GrAAType overallAA,
    SkBlendMode blendMode, int requestedTotNumQuads, int expectedNumOps) {
  std::unique_ptr<GrRenderTargetContext> rtc = new_RTC(context);
  ...
  GrTextureOp::AddTextureSetOps(
      rtc.get(), GrNoClip(), context, set, requestedTotNumQuads, GrSamplerState::Filter::kNearest,
...
static void run_test(GrContext* context, skiatest::Reporter* reporter, BulkRectTest test) {
  // This is the simple case where there is no AA at all. We expect 2 non-AA clumps of quads.
  {
    ...
    test(
        reporter, context, noAA, GrAAType::kNone, SkBlendMode::kSrcOver,
  ...
  // the per-quad AA is still none, all the quads should be downgraded to non-AA.
  {
    ...
    test(
        reporter, context, noAA, GrAAType::kCoverage, SkBlendMode::kSrcOver,
        2 * GrResourceProvider::MaxNumNonAAQuads(), kNumExpectedOps);
  }
  ...
  // We should end up with several aa-sized clumps
  {
    ...
    test(
        reporter, context, alternateAA, GrAAType::kCoverage, SkBlendMode::kSrcOver,
  ...
  // its size. We expect one clump of non-AA quads followed by one clump of AA quads.
  {
    ...
    test(
        reporter, context, runOfNonAA, GrAAType::kCoverage, SkBlendMode::kSrcOver,
  ...
  // responsible for revising its expectations.
  {
    ...
    test(
        reporter, context, fixedAA, GrAAType::kCoverage, SkBlendMode::kSrcATop,
        2 * GrResourceProvider::MaxNumAAQuads(), kNumExpectedOps);
  }
}
