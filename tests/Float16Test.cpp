...
static bool is_denorm(uint16_t h) { return (h & 0x7fff) < 0x0400; }

static bool is_finite(uint16_t h) { return (h & 0x7c00) != 0x7c00; }
...
DEF_TEST(SkHalfToFloat_finite_ftz, r) {
  for (uint32_t h = 0; h <= 0xffff; h++) {
    ...
    float expected = SkHalfToFloat(h), alternate = is_denorm(h) ? 0.0f : expected;
...
DEF_TEST(SkFloatToHalf_finite_ftz, r) {
  ...
  REPORTER_ASSERT(
      r, actual == expected || actual == expected - 1 || actual == alternate ||
             actual == alternate - 1);
}
}
