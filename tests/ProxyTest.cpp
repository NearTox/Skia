            ...
            for (auto numSamples : {1, 4, 16, 128}) {
              ...
              // Renderable
              {
                ...
                if (SkBackingFit::kApprox == fit) {
                  ...
                  tex = resourceProvider->createTexture(
                      desc, GrRenderable::kYes, numSamples, budgeted, GrProtected::kNo,
                      GrResourceProvider::Flags::kNoPendingIO);
                }

                sk_sp<GrTextureProxy> proxy = proxyProvider->createProxy(
                    format, desc, GrRenderable::kYes, numSamples, origin, fit, budgeted,
                    GrProtected::kNo);
                ...
                if (proxy) {
                  ...
                  int supportedSamples = caps.getRenderTargetSampleCount(numSamples, config);
                  check_rendertarget(
              ...
              // Not renderable
              {
                ...
                if (SkBackingFit::kApprox == fit) {
                  ...
                  tex = resourceProvider->createTexture(
                      desc, GrRenderable::kNo, numSamples, budgeted, GrProtected::kNo,
                      GrResourceProvider::Flags::kNoPendingIO);
                }

                sk_sp<GrTextureProxy> proxy(proxyProvider->createProxy(
                    format, desc, GrRenderable::kNo, numSamples, origin, fit, budgeted,
                    GrProtected::kNo));
                REPORTER_ASSERT(reporter, SkToBool(tex) == SkToBool(proxy));
                if (proxy) {
                  // This forces the proxy to compute and cache its
      ...
      for (int width : {0, 100}) {
        for (int height : {0, 100}) {
          ...
          sk_sp<GrTextureProxy> proxy = provider->createProxy(
              format, desc, renderable, 1, kBottomLeft_GrSurfaceOrigin, fit, SkBudgeted::kNo,
              GrProtected::kNo);
