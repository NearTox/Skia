...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(DeferredProxyTest, reporter, ctxInfo) {
  ...
  for (auto origin : {kBottomLeft_GrSurfaceOrigin, kTopLeft_GrSurfaceOrigin}) {
    for (auto widthHeight : {100, 128, 1048576}) {
      for (auto config : {kAlpha_8_GrPixelConfig, kRGB_565_GrPixelConfig, kRGBA_8888_GrPixelConfig,
                          kRGBA_1010102_GrPixelConfig, kRGB_ETC1_GrPixelConfig}) {
            ...
            for (auto numSamples : {1, 4, 16, 128}) {
...

              {
                ...
                sk_sp<GrTextureProxy> proxy =
                    proxyProvider->createProxy(format, desc, origin, fit, budgeted);
    ...
    for (auto colorType :
      ...
      if (ctxInfo.grContext()->colorTypeSupportedAsSurface(colorType)) {
        ...
        sk_sp<GrSurfaceProxy> sProxy(
            proxyProvider->wrapBackendRenderTarget(backendRT, origin, nullptr, nullptr));
        check_surface(
            reporter, sProxy.get(), origin, kWidthHeight, kWidthHeight, backendRT.pixelConfig(),
            SkBudgeted::kNo);
      ...
      for (auto numSamples : {1, 4}) {
        ...
        // Tests wrapBackendRenderTarget with a GrBackendTexture
        {
          GrBackendTexture backendTex = context->createBackendTexture(
              kWidthHeight, kWidthHeight, colorType, GrMipMapped::kNo, GrRenderable::kYes);
          ...
          check_surface(
              reporter, sProxy.get(), origin, kWidthHeight, kWidthHeight, backendTex.pixelConfig(),
              SkBudgeted::kNo);
          check_rendertarget(
              reporter, caps, resourceProvider, sProxy->asRenderTargetProxy(), supportedNumSamples,
              SkBackingFit::kExact, caps.maxWindowRectangles());
        ...
        // Tests wrapBackendTexture that is only textureable
        {
          ...
          GrBackendTexture backendTex = context->createBackendTexture(
              kWidthHeight, kWidthHeight, colorType, GrMipMapped::kNo, GrRenderable::kNo);

          sk_sp<GrSurfaceProxy> sProxy = proxyProvider->wrapBackendTexture(
              backendTex, origin, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, kRead_GrIOType);
          if (!sProxy) {
            context->deleteBackendTexture(backendTex);
            continue;
          }
