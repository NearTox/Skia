...
static void check_surface(
    ...
    int height, GrPixelConfig config, SkBudgeted budgeted) {
            ...
            for (auto numSamples : {1, 4, 16, 128}) {
              ...
              // Renderable
              {
                ...
                if (SkBackingFit::kApprox == fit) {
                  ...
                  tex = resourceProvider->createTexture(
                      desc, format, GrRenderable::kYes, numSamples, budgeted, GrProtected::kNo,
                      GrResourceProvider::Flags::kNoPendingIO);
                }

                sk_sp<GrTextureProxy> proxy = proxyProvider->createProxy(
                    format, desc, GrRenderable::kYes, numSamples, origin, GrMipMapped::kNo, fit,
                ...
                if (proxy) {
                  ...
                  check_surface(
                      reporter, proxy.get(), origin, widthHeight, widthHeight, config, budgeted);
              ...
              // Not renderable
              {
                ...
                if (SkBackingFit::kApprox == fit) {
                  ...
                  tex = resourceProvider->createTexture(
                      desc, format, GrRenderable::kNo, numSamples, budgeted, GrProtected::kNo,
                      GrResourceProvider::Flags::kNoPendingIO);
                }

                sk_sp<GrTextureProxy> proxy(proxyProvider->createProxy(
                    format, desc, GrRenderable::kNo, numSamples, origin, GrMipMapped::kNo, fit,
                    budgeted, GrProtected::kNo));
                REPORTER_ASSERT(reporter, SkToBool(tex) == SkToBool(proxy));
                if (proxy) {
                  // This forces the proxy to compute and cache its
  ...
  for (auto origin : {kBottomLeft_GrSurfaceOrigin, kTopLeft_GrSurfaceOrigin}) {
    for (auto colorType :
      ...
      if (ctxInfo.grContext()->colorTypeSupportedAsSurface(colorType)) {
        ...
        check_surface(
            reporter, sProxy.get(), origin, kWidthHeight, kWidthHeight, config, SkBudgeted::kNo);
      ...
      for (int width : {0, 100}) {
        for (int height : {0, 100}) {
          ...
          sk_sp<GrTextureProxy> proxy = provider->createProxy(
              format, desc, renderable, 1, kBottomLeft_GrSurfaceOrigin, GrMipMapped::kNo, fit,
