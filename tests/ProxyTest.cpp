...
static void check_surface(
    skiatest::Reporter* reporter, GrSurfaceProxy* proxy, GrSurfaceOrigin origin, int width,
    int height, SkBudgeted budgeted) {
          ...
          for (auto budgeted : {SkBudgeted::kYes, SkBudgeted::kNo}) {
            for (auto numSamples : {1, 4, 16, 128}) {
              ...
              // Renderable
              {
                ...
                if (SkBackingFit::kApprox == fit) {
                  tex = resourceProvider->createApproxTexture(
                      desc, format, GrRenderable::kYes, numSamples, GrProtected::kNo);
                } else {
                ...
                sk_sp<GrTextureProxy> proxy = proxyProvider->createProxy(
                    format, desc, swizzle, GrRenderable::kYes, numSamples, origin, GrMipMapped::kNo,
                ...
                if (proxy) {
                  ...
                  check_surface(reporter, proxy.get(), origin, widthHeight, widthHeight, budgeted);
              ...
              // Not renderable
              {
                ...
                if (SkBackingFit::kApprox == fit) {
                  tex = resourceProvider->createApproxTexture(
                      desc, format, GrRenderable::kNo, numSamples, GrProtected::kNo);
                } else {
                  tex = resourceProvider->createTexture(
                ...
                sk_sp<GrTextureProxy> proxy(proxyProvider->createProxy(
                    format, desc, swizzle, GrRenderable::kNo, numSamples, origin, GrMipMapped::kNo,
                    fit, budgeted, GrProtected::kNo));
                REPORTER_ASSERT(reporter, SkToBool(tex) == SkToBool(proxy));
                if (proxy) {
                  // This forces the proxy to compute and cache its
    ...
    for (auto colorType :
      ...
      if (ctxInfo.grContext()->colorTypeSupportedAsSurface(colorType)) {
        ...
        sk_sp<GrSurfaceProxy> sProxy(proxyProvider->wrapBackendRenderTarget(
            backendRT, grColorType, origin, nullptr, nullptr));
        check_surface(reporter, sProxy.get(), origin, kWidthHeight, kWidthHeight, SkBudgeted::kNo);
      ...
      for (auto numSamples : {1, 4}) {
        ...
        if (GrBackendApi::kOpenGL == ctxInfo.backend()) {
          ...
          sk_sp<GrSurfaceProxy> sProxy(proxyProvider->wrapBackendRenderTarget(
              backendRT, grColorType, origin, nullptr, nullptr));
          check_surface(
              reporter, sProxy.get(), origin, kWidthHeight, kWidthHeight, SkBudgeted::kNo);
          check_rendertarget(
        ...
        // Tests wrapBackendTexture that is only renderable
        {
          ...
          sk_sp<GrSurfaceProxy> sProxy = proxyProvider->wrapRenderableBackendTexture(
              backendTex, origin, supportedNumSamples, grColorType, kBorrow_GrWrapOwnership,
        ...
        // Tests wrapBackendTexture that is only textureable
        {
          ...
          sk_sp<GrSurfaceProxy> sProxy = proxyProvider->wrapBackendTexture(
              backendTex, grColorType, origin, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo,
              kRead_GrIOType);
          if (!sProxy) {
            context->deleteBackendTexture(backendTex);
            continue;
      ...
      for (int width : {0, 100}) {
        for (int height : {0, 100}) {
          ...
          sk_sp<GrTextureProxy> proxy = provider->createProxy(
              format, desc, swizzle, renderable, 1, kBottomLeft_GrSurfaceOrigin, GrMipMapped::kNo,
