...
static void assert9(
    skiatest::Reporter* reporter, const SkMatrix& m, SkScalar a, SkScalar b, SkScalar c, SkScalar d,
    SkScalar e, SkScalar f, SkScalar g, SkScalar h, SkScalar i) {
  SkScalar buffer[9];
  m.get9(buffer);
...
static void test_matrix_recttorect(skiatest::Reporter* reporter) {
  ...
  matrix.setRectToRect(src, dst, SkMatrix::kFill_ScaleToFit);
  REPORTER_ASSERT(
      reporter, (SkMatrix::kTranslate_Mask | SkMatrix::kScale_Mask) == matrix.getType());
  REPORTER_ASSERT(reporter, matrix.rectStaysRect());
...
static void test_matrix_min_max_scale(skiatest::Reporter* reporter) {
  ...
  REPORTER_ASSERT(
      reporter, SkScalarNearlyEqual(SK_Scalar1, rotate.getMinScale(), SK_ScalarNearlyZero));
  REPORTER_ASSERT(
      reporter, SkScalarNearlyEqual(SK_Scalar1, rotate.getMaxScale(), SK_ScalarNearlyZero));
  ...
  SkMatrix big;
  big.setAll(
      2.39394089e+36f, 8.85347779e+36f, 9.26526204e+36f, 3.9159619e+36f, 1.44823453e+37f,
      1.51559342e+37f, 0.f, 0.f, 1.f);
  success = big.getMinMaxScales(scales);
  REPORTER_ASSERT(reporter, !success);

  // skbug.com/4718
  SkMatrix givingNegativeNearlyZeros;
  givingNegativeNearlyZeros.setAll(
      0.00436534f, 0.114138f, 0.37141f, 0.00358857f, 0.0936228f, -0.0174198f, 0.f, 0.f, 1.f);
  ...
  REPORTER_ASSERT(
      reporter, !success && -5 * SK_Scalar1 == scales[0] && -5 * SK_Scalar1 == scales[1]);

  SkMatrix baseMats[] = {scale, rot90Scale, rotate, translate, perspX, perspY};
...
static void test_matrix_preserve_shape(skiatest::Reporter* reporter) {
  ...
  mat.reset();
  mat.setScale(SkIntToScalar(15), SkIntToScalar(15), SkIntToScalar(2), SkIntToScalar(2));
  REPORTER_ASSERT(reporter, mat.isSimilarity());
  REPORTER_ASSERT(reporter, mat.preservesRightAngles());

  // scale with different size at a pivot point
  mat.reset();
  mat.setScale(SkIntToScalar(15), SkIntToScalar(20), SkIntToScalar(2), SkIntToScalar(2));
  REPORTER_ASSERT(reporter, !mat.isSimilarity());
  REPORTER_ASSERT(reporter, mat.preservesRightAngles());
  ...
  mat.reset();
  mat.setSkew(SkIntToScalar(15), SkIntToScalar(15), SkIntToScalar(2), SkIntToScalar(2));
  REPORTER_ASSERT(reporter, !mat.isSimilarity());
  REPORTER_ASSERT(reporter, !mat.preservesRightAngles());

  // skew with different size at a pivot point
  mat.reset();
  mat.setSkew(SkIntToScalar(15), SkIntToScalar(20), SkIntToScalar(2), SkIntToScalar(2));
  REPORTER_ASSERT(reporter, !mat.isSimilarity());
  REPORTER_ASSERT(reporter, !mat.preservesRightAngles());
  ...
  mat.setAll(0, SK_Scalar1, 0, -SK_Scalar1, 0, 0, 0, 0, SkMatrix::I()[8]);
  REPORTER_ASSERT(reporter, mat.isSimilarity());
  REPORTER_ASSERT(reporter, mat.preservesRightAngles());
...
static bool scalar_nearly_equal_relative(
    SkScalar a, SkScalar b, SkScalar tolerance = SK_ScalarNearlyZero) {
  // from Bruce Dawson
  // absolute check
...
static bool check_matrix_recomposition(
    const SkMatrix& mat, const SkPoint& rotation1, const SkPoint& scale, const SkPoint& rotation2) {
  ...
  bool result =
      scalar_nearly_equal_relative(mat[SkMatrix::kMScaleX], scaleX * c1 * c2 - scaleY * s1 * s2) &&
      scalar_nearly_equal_relative(mat[SkMatrix::kMSkewX], -scaleX * s1 * c2 - scaleY * c1 * s2) &&
      scalar_nearly_equal_relative(mat[SkMatrix::kMSkewY], scaleX * c1 * s2 + scaleY * s1 * c2) &&
      scalar_nearly_equal_relative(mat[SkMatrix::kMScaleY], -scaleX * s1 * s2 + scaleY * c1 * c2);
  return result;
}
...
static bool naive_homogeneous_mapping(const SkMatrix& m, const SkPoint3& src, const SkPoint3& dst) {
  SkPoint3 res;
  SkScalar ms[9] = {m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7], m[8]};
...
static bool check_decompScale(const SkMatrix& original) {
  ...
  const SkPoint testPts[kNumPoints] = {
      {0.0f, 0.0f}, {1.0f, 1.0f}, {1.0f, 0.5f}, {-1.0f, -0.5f}, {-1.0f, 2.0f}};
...
DEF_TEST(Matrix, reporter) {
  ...
  mat.setAll(0.0f, 1.0f, 2.0f, 0.0f, 1.0f, -3.40277175e+38f, 1.00003040f, 1.0f, 0.0f);
  REPORTER_ASSERT(reporter, !mat.invert(nullptr));
  REPORTER_ASSERT(reporter, !mat.invert(&inverse));
  ...
  // rectStaysRect test
  {
    static const struct {
      ...
      bool mStaysRect;
    } gRectStaysRectSamples[] = {{0, 0, 0, 0, false},
    ...
    for (size_t i = 0; i < SK_ARRAY_COUNT(gRectStaysRectSamples); i++) {
      ...
      REPORTER_ASSERT(reporter, m.rectStaysRect() == gRectStaysRectSamples[i].mStaysRect);
    }
  }
...
DEF_TEST(Matrix_maprects, r) {
  ...
  for (int i = 0; i < 10000; ++i) {
    SkRect src = SkRect::MakeLTRB(
        rand.nextSScalar1() * scale, rand.nextSScalar1() * scale, rand.nextSScalar1() * scale,
...
DEF_TEST(Matrix_Ctor, r) { REPORTER_ASSERT(r, SkMatrix{} == SkMatrix::I()); }
