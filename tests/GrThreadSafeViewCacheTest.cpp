...
class TestHelper {
 public:
  ...
  void accessCachedView(
      SkCanvas* canvas, int wh, bool failLookup = false, bool failFillingIn = false) {
    GrRecordingContext* rContext = canvas->recordingContext();

    auto view = AccessCachedView(
        rContext, this->threadSafeViewCache(), wh, failLookup, failFillingIn, &fStats);
    SkASSERT(view);
  ...
  size_t gpuSize(int wh) const {
    ...
    return GrSurface::ComputeSize(
        *fDContext->priv().caps(), format, {wh, wh}, 1, GrMipMapped::kNo, false);
  }
 ...
 private:
  ...
  static GrSurfaceProxyView AccessCachedView(
      GrRecordingContext*, GrThreadSafeUniquelyKeyedProxyViewCache*, int wh, bool failLookup,
      bool failFillingIn, Stats*);
  static GrSurfaceProxyView CreateViewOnCpu(GrRecordingContext*, int wh, Stats*);
  static std::tuple<GrSurfaceProxyView, sk_sp<Trampoline>> CreateLazyView(
      GrDirectContext*, int wh, Stats*);
  static bool FillInViewOnGpu(
      GrDirectContext*, int wh, Stats*, const GrSurfaceProxyView& lazyView, sk_sp<Trampoline>);
...
std::tuple<GrSurfaceProxyView, sk_sp<TestHelper::Trampoline>> TestHelper::CreateLazyView(
  ...
  sk_sp<GrRenderTargetProxy> proxy = proxyProvider->createLazyRenderTargetProxy(
      ...
      format, {wh, wh},
      /* renderTargetSampleCnt */ 1, GrInternalSurfaceFlags::kNone, &texInfo,
      GrMipmapStatus::kNotAllocated, SkBackingFit::kExact, SkBudgeted::kYes, GrProtected::kNo,
      /* wrapsVkSecondaryCB */ false, GrSurfaceProxy::UseAllocator::kYes);
...
GrSurfaceProxyView TestHelper::AccessCachedView(
    GrRecordingContext* rContext, GrThreadSafeUniquelyKeyedProxyViewCache* threadSafeViewCache,
    int wh, bool failLookup, bool failFillingIn, Stats* stats) {
  GrUniqueKey key;
  create_key(&key, wh);
