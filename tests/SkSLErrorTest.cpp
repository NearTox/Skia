...
static void check_expected_errors(
    skiatest::Reporter* r, const char* testFile, const std::vector<std::string>& expectedErrors,
  ...
  for (const std::string& expectedError : expectedErrors) {
    ...
    if (pos == std::string::npos) {
      ERRORF(
          r, "%s: Expected an error that wasn't reported:\n%s\n",
  ...
  if (reportOriginalErrors) {
    ERRORF(
        r, "%s: The following errors were reported:\n%s\n", SkOSPath::Basename(testFile).c_str(),
        originalErrors.c_str());
  }
}
...
static void test_expect_fail(skiatest::Reporter* r, const char* testFile, SkSL::ProgramKind kind) {
  ...
  std::string shaderString{reinterpret_cast<const char*>(shaderData->bytes()), shaderData->size()};
  ...
  std::unique_ptr<SkSL::Program> program =
      compiler.convertProgram(kind, std::move(shaderString), settings);

  // If the code actually generated a working program, we've already failed.
  if (program) {
    ERRORF(
        r, "%s: Expected failure, but compiled successfully", SkOSPath::Basename(testFile).c_str());
    return;
  }
...
static void iterate_dir(
    const char* directory, const char* extension, const std::function<void(const char*)>& run) {
