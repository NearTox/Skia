...
DEF_TEST(CanvasState_test_complex_layers, reporter) {
  ...
  SkRect rect = SkRect::MakeXYWH(
      SkIntToScalar(SPACER), SkIntToScalar(SPACER), SkIntToScalar(WIDTH - (2 * SPACER)),
      SkIntToScalar((HEIGHT - (2 * SPACER)) / 7));

  const SkColorType colorTypes[] = {kRGB_565_SkColorType, kN32_SkColorType};
  ...
  bool (*drawFn)(SkCanvasState * state, float l, float t, float r, float b, int32_t s);

  OpenLibResult openLibResult(reporter);
  if (openLibResult.handle() != nullptr) {
    *(void**)(&drawFn) = dlsym(openLibResult.handle(), "complex_layers_draw_from_canvas_state");
  } else {
  ...
  for (size_t i = 0; i < SK_ARRAY_COUNT(colorTypes); ++i) {
    SkBitmap bitmaps[2];
    for (int j = 0; j < 2; ++j) {
      bitmaps[j].allocPixels(SkImageInfo::Make(WIDTH, HEIGHT, colorTypes[i], kPremul_SkAlphaType));
      ...
      for (size_t k = 0; k < SK_ARRAY_COUNT(layerAlpha); ++k) {
        ...
        if (j) {
          ...
          bool success = complex_layers_draw_from_canvas_state(
              state, rect.fLeft, rect.fTop, rect.fRight, rect.fBottom, SPACER);
          ...
          complex_layers_draw(&canvas, rect.fLeft, rect.fTop, rect.fRight, rect.fBottom, SPACER);
        }
    ...
    REPORTER_ASSERT(
        reporter,
        !memcmp(bitmaps[0].getPixels(), bitmaps[1].getPixels(), bitmaps[0].computeByteSize()));
  }
}
#endif
...
DEF_TEST(CanvasState_test_complex_clips, reporter) {
  ...
  bool (*drawFn)(
      SkCanvasState * state, int32_t l, int32_t t, int32_t r, int32_t b, int32_t clipOp,
      int32_t regionRects, int32_t * rectCoords);

  OpenLibResult openLibResult(reporter);
  if (openLibResult.handle() != nullptr) {
    *(void**)(&drawFn) = dlsym(openLibResult.handle(), "complex_clips_draw_from_canvas_state");
  } else {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    for (size_t j = 0; j < SK_ARRAY_COUNT(flags); ++j) {
      ...
      if (i) {
        ...
        bool success = drawFn(
            state, clipRect.fLeft, clipRect.fTop, clipRect.fRight, clipRect.fBottom, clipOps[j],
        ...
        complex_clips_draw(
            &canvas, clipRect.fLeft, clipRect.fTop, clipRect.fRight, clipRect.fBottom, clipOps[j],
            localRegion);
      }
...
DEF_TEST(CanvasState_test_saveLayer_clip, reporter) {
  ...
  canvas.saveLayer(SkCanvas::SaveLayerRec(
      &bounds, nullptr, (SkCanvas::SaveLayerFlags)SkCanvasPriv::kDontClipToLayer_SaveLayerFlag));
