...
class SurfaceParameters {
 public:
  ...
  bool modify(int i) {
    ...
    switch (i) {
      case 0: set(fWidth, 63); break;
      case 1: set(fHeight, 63); break;
      case 2: set(fOrigin, kBottomLeft_GrSurfaceOrigin); break;
      case 3: set(fColorType, kRGBA_F16_SkColorType); break;
      case 4:
        // This just needs to be a colorSpace different from that returned by MakeSRGB().
        // In this case we just change the gamut.
        set(fColorSpace, SkColorSpace::MakeRGB(SkNamedTransferFn::kSRGB, SkNamedGamut::kAdobeRGB));
        break;
      case 5: set(fSampleCount, 4); break;
      case 6: set(fSurfaceProps, SkSurfaceProps(0x0, kRGB_H_SkPixelGeometry)); break;
      case 7:
        set(fSurfaceProps,
            SkSurfaceProps(
                SkSurfaceProps::kUseDeviceIndependentFonts_Flag, kUnknown_SkPixelGeometry));
        break;
      case 8: set(fShouldCreateMipMaps, false); break;
      case 9:
  ...
  SkSurfaceCharacterization createCharacterization(GrDirectContext* dContext) const {
    ...
    GrBackendFormat backendFormat = dContext->defaultBackendFormat(fColorType, GrRenderable::kYes);
    ...
    SkSurfaceCharacterization c = dContext->threadSafeProxy()->createCharacterization(
        ...
        fVkRTSupportsInputAttachment, fForVulkanSecondaryCommandBuffer);
    return c;
  }
  ...
  sk_sp<SkSurface> make(GrDirectContext* dContext) const {
    ...
    if (fUsesGLFBO0) {
      ...
      sk_sp<SkSurface> result = SkSurface::MakeFromBackendRenderTarget(
    ...
    if (fIsTextureable) {
      surface = sk_gpu_test::MakeBackendTextureSurface(
          dContext, {fWidth, fHeight}, fOrigin, fSampleCount, fColorType, fColorSpace,
          GrMipmapped(fShouldCreateMipMaps), fIsProtected, &fSurfaceProps);
    } else {
      ...
      surface = sk_gpu_test::MakeBackendRenderTargetSurface(
          dContext, {fWidth, fHeight}, fOrigin, fSampleCount, fColorType, fColorSpace, fIsProtected,
          &fSurfaceProps);
    }
...
void DDLSurfaceCharacterizationTestImpl(GrDirectContext* dContext, skiatest::Reporter* reporter) {
  ...
  SkImageInfo imageInfo = SkImageInfo::Make(64, 64, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
  SkBitmap bitmap;
  ...
  for (int i = 0; i < SurfaceParameters::kNumParams; ++i) {
    ...
    REPORTER_ASSERT(
        reporter, !s->draw(ddl), "DDLSurfaceCharacterizationTest failed on parameter: %d\n", i);
    dContext->flush();
  }
  ...
  // Exercise the createBackendFormat method
  {
    ...
    auto newBackendFormat =
        dContext->defaultBackendFormat(kRGB_565_SkColorType, GrRenderable::kYes);

    if (newBackendFormat.isValid()) {
      SkSurfaceCharacterization char1 =
          char0.createBackendFormat(kRGB_565_SkColorType, newBackendFormat);
      REPORTER_ASSERT(reporter, char1.isValid());
      REPORTER_ASSERT(reporter, char1.backendFormat() == newBackendFormat);

      SkSurfaceCharacterization invalid;
      REPORTER_ASSERT(reporter, !invalid.isValid());
      auto stillInvalid = invalid.createBackendFormat(kRGB_565_SkColorType, newBackendFormat);
      REPORTER_ASSERT(reporter, !stillInvalid.isValid());
    }
  }
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(CharacterizationFBO0nessTest, reporter, ctxInfo) {
  ...
  for (bool isFBO0 : {true, false}) {
    for (int numSamples : {availableSamples, 1}) {
      characterizations[index] = proxy->createCharacterization(
          resourceCacheLimit, ii, format, numSamples, kTopLeft_GrSurfaceOrigin, surfaceProps,
          kNotMipMapped, isFBO0, kNotTextureable);
      ...
      surfaces[index] = SkSurface::MakeFromBackendRenderTarget(
          context, backendRT, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, nullptr,
          &surfaceProps);
      ++index;
    }
  }
  ...
  for (int c = 0; c < 4; ++c) {
    for (int s = 0; s < 4; ++s) {
      REPORTER_ASSERT(
          reporter,
          kExpectedCompatibility[c][s] == surfaces[s]->isCompatible(characterizations[c]));
    }
  }
}
#endif
...
static void test_make_render_target(
    skiatest::Reporter* reporter, GrDirectContext* dContext, const SurfaceParameters& params) {
  {
...
struct TextureReleaseChecker {
  ...
  int fReleaseCount;
  static void Release(void* self) { static_cast<TextureReleaseChecker*>(self)->fReleaseCount++; }
};
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(DDLWrapBackendTest, reporter, ctxInfo) {
  ...
  auto mbet = sk_gpu_test::ManagedBackendTexture::MakeWithoutData(
      dContext, kSize, kSize, kRGBA_8888_SkColorType, GrMipmapped::kNo, GrRenderable::kNo,
      GrProtected::kNo);
  if (!mbet) {
    return;
  }
  ...
  sk_sp<SkImage> image = SkImage::MakeFromTexture(
      rContext, mbet->texture(), kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType,
      kPremul_SkAlphaType, nullptr, sk_gpu_test::ManagedBackendTexture::ReleaseProc,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(DDLCreateCharacterizationFailures, reporter, ctxInfo) {
  ...
  auto check_create_fails = [proxy, reporter, maxResourceBytes](
                                ...
                                SkColorType ct, bool willUseGLFBO0, bool isTextureable,
                                GrProtected prot, bool vkRTSupportsInputAttachment,
    ...
    SkSurfaceCharacterization c = proxy->createCharacterization(
        ...
        willUseGLFBO0, isTextureable, prot, vkRTSupportsInputAttachment,
        forVulkanSecondaryCommandBuffer);
    REPORTER_ASSERT(reporter, !c.isValid());
  };

  GrBackendFormat goodBackendFormat =
      dContext->defaultBackendFormat(kRGBA_8888_SkColorType, GrRenderable::kYes);
  ...
  check_create_fails(
      goodBackendFormat, goodWidth, badHeights[0], kGoodCT, kGoodUseFBO0, kIsTextureable,
      GrProtected::kNo, kGoodVkInputAttachment, kGoodForVkSCB);
  check_create_fails(
      goodBackendFormat, goodWidth, badHeights[1], kGoodCT, kGoodUseFBO0, kIsTextureable,
      GrProtected::kNo, kGoodVkInputAttachment, kGoodForVkSCB);
  check_create_fails(
      goodBackendFormat, badWidths[0], goodHeight, kGoodCT, kGoodUseFBO0, kIsTextureable,
      GrProtected::kNo, kGoodVkInputAttachment, kGoodForVkSCB);
  check_create_fails(
      goodBackendFormat, badWidths[1], goodHeight, kGoodCT, kGoodUseFBO0, kIsTextureable,
      GrProtected::kNo, kGoodVkInputAttachment, kGoodForVkSCB);
  check_create_fails(
      badBackendFormat, goodWidth, goodHeight, kGoodCT, kGoodUseFBO0, kIsTextureable,
      GrProtected::kNo, kGoodVkInputAttachment, kGoodForVkSCB);
  check_create_fails(
      goodBackendFormat, goodWidth, goodHeight, kBadCT, kGoodUseFBO0, kIsTextureable,
      GrProtected::kNo, kGoodVkInputAttachment, kGoodForVkSCB);
  ...
  check_create_fails(
      goodBackendFormat, goodWidth, goodHeight, kGoodCT, kBadUseFBO0, kIsTextureable,
      GrProtected::kNo, kGoodVkInputAttachment, kGoodForVkSCB);
  if (dContext->backend() == GrBackendApi::kVulkan) {
    ...
    check_create_fails(
        goodBackendFormat, goodWidth, goodHeight, kGoodCT, kGoodUseFBO0, kIsTextureable,
        GrProtected::kYes, kGoodVkInputAttachment, kGoodForVkSCB);
    ...
    check_create_fails(
        goodBackendFormat, goodWidth, goodHeight, kGoodCT, kGoodUseFBO0, kIsTextureable,
        GrProtected::kNo, kGoodVkInputAttachment, kBadForVkSCB);
    // The following fails because forVulkanSecondaryCommandBuffer is true and
    // vkRTSupportsInputAttachment is true. This is not a legal combination.
    check_create_fails(
        goodBackendFormat, goodWidth, goodHeight, kGoodCT, kGoodUseFBO0, kIsNotTextureable,
        GrProtected::kNo, kBadVkInputAttachment, kBadForVkSCB);
    // The following fails because forVulkanSecondaryCommandBuffer is true and
    // willUseGLFBO0 is true. This is not a legal combination.
    check_create_fails(
        goodBackendFormat, goodWidth, goodHeight, kGoodCT, kBadUseFBO0, kIsNotTextureable,
        GrProtected::kNo, kGoodVkInputAttachment, kBadForVkSCB);
  } else {
    // The following set vulkan only flags on non vulkan backends.
    check_create_fails(
        goodBackendFormat, goodWidth, goodHeight, kGoodCT, kGoodUseFBO0, kIsTextureable,
        GrProtected::kNo, kBadVkInputAttachment, kGoodForVkSCB);
    check_create_fails(
        goodBackendFormat, goodWidth, goodHeight, kGoodCT, kGoodUseFBO0, kIsNotTextureable,
        GrProtected::kNo, kGoodVkInputAttachment, kBadForVkSCB);
  }
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(DDLSkSurfaceFlush, reporter, ctxInfo) {
...

  {
    ...
    sk_sp<SkImage> promiseImage = SkImage::MakePromiseTexture(
        canvas->recordingContext()->threadSafeProxy(), format, SkISize::Make(32, 32),
        GrMipmapped::kNo, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, kPremul_SkAlphaType,
        nullptr, tracking_fulfill_proc, tracking_release_proc, &fulfillInfo);
  ...
  for (GrGLenum target : {GR_GL_TEXTURE_EXTERNAL, GR_GL_TEXTURE_RECTANGLE, GR_GL_TEXTURE_2D}) {
    for (auto mipMapped : {GrMipmapped::kNo, GrMipmapped::kYes}) {
      ...
      sk_sp<SkImage> image = SkImage::MakePromiseTexture(
          recorder.getCanvas()->recordingContext()->threadSafeProxy(), format,
          SkISize::Make(32, 32), mipMapped, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType,
          kPremul_SkAlphaType,
          /*color space*/ nullptr, noop_fulfill_proc,
