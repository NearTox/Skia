  ...
  sk_sp<SkSurface> make(GrContext* context, GrBackendTexture* backend) const {
    ...
    if (fUsesGLFBO0) {
      ...
      sk_sp<SkSurface> result = SkSurface::MakeFromBackendRenderTarget(
          context, backendRT, fOrigin, fColorType, fColorSpace, &fSurfaceProps);
      SkASSERT(result->isCompatible(c));
      return result;
    }
#endif

    *backend = context->createBackendTexture(
        fWidth, fHeight, fColorType, SkColors::kTransparent, mipmapped, GrRenderable::kYes,
        fIsProtected);
    if (!backend->isValid()) {
      return nullptr;
    }
    ...
    if (!fIsTextureable) {
      ...
      surface = SkSurface::MakeFromBackendTextureAsRenderTarget(
          context, *backend, fOrigin, fSampleCount, fColorType, fColorSpace, &fSurfaceProps);
    } else {
...
void DDLSurfaceCharacterizationTestImpl(GrContext* context, skiatest::Reporter* reporter) {
  ...
  for (int i = 0; i < SurfaceParameters::kNumParams; ++i) {
    ...
    REPORTER_ASSERT(
        reporter, !s->draw(ddl.get()), "DDLSurfaceCharacterizationTest failed on parameter: %d\n",
        i);
  ...
  // Exercise the createBackendFormat method
  {
    ...
    auto originalBackendFormat =
        context->defaultBackendFormat(kRGBA_8888_SkColorType, GrRenderable::kYes);
    REPORTER_ASSERT(reporter, originalBackendFormat.isValid());
    REPORTER_ASSERT(reporter, char0.backendFormat() == originalBackendFormat);

    auto newBackendFormat = context->defaultBackendFormat(kRGB_565_SkColorType, GrRenderable::kYes);
...
static void test_make_render_target(
    skiatest::Reporter* reporter, GrContext* context, const SurfaceParameters& params) {
  {
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(DDLWrapBackendTest, reporter, ctxInfo) {
  ...
  GrBackendTexture backendTex = context->createBackendTexture(
      kSize, kSize, kRGBA_8888_SkColorType, SkColors::kTransparent, GrMipMapped::kNo,
  ...
  sk_sp<SkImage> image = SkImage::MakeFromAdoptedTexture(
      deferredContext, backendTex, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType,
  ...
  image = SkImage::MakeFromTexture(
      deferredContext, backendTex, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType,
      kPremul_SkAlphaType, nullptr, TextureReleaseChecker::Release, &releaseChecker);
  REPORTER_ASSERT(reporter, !image);
