 ...
 public:
  ...
  void modify(int i) {
    switch (i) {
      ...
      case 9:
        ...
        break;
      case 10: fIsTextureable = false; break;
    }
  }
  ...
  SkSurfaceCharacterization createCharacterization(GrContext* context) const {
    ...
    SkSurfaceCharacterization c = context->threadSafeProxy()->createCharacterization(
        ...
        fShouldCreateMipMaps, fUsesGLFBO0, fIsTextureable);
    return c;
  }
  ...
  sk_sp<SkSurface> make(GrContext* context, GrBackendTexture* backend) const {
    ...
    *backend =
        context->createBackendTexture(fWidth, fHeight, fColorType, mipmapped, GrRenderable::kYes);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(DDLSurfaceCharacterizationTest, reporter, ctxInfo) {
  ...
  for (int i = 0; i < SurfaceParameters::kNumParams; ++i) {
    ...
    if (SurfaceParameters::kMipMipCount == i && !context->priv().caps()->mipMapSupport()) {
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(DDLSkSurfaceFlush, reporter, ctxInfo) {
  ...
  if (!create_backend_texture(
          context, &backendTexture, ii, GrMipMapped::kNo, SK_ColorCYAN, GrRenderable::kNo)) {
