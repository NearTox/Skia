...
DEF_TEST(GrBlockAllocatorPreallocSize, r) {
  ...
  GrBlockAllocator* placement =
      new (mem) GrBlockAllocator(GrowthPolicy::kLinear, 1024, 1024 - sizeof(GrBlockAllocator));
  REPORTER_ASSERT(r, placement->preallocSize() == 1024);
  REPORTER_ASSERT(
      r, placement->preallocUsableSpace() < 1024 &&
...
DEF_TEST(GrBlockAllocatorResize, r) {
  ...
  REPORTER_ASSERT(
      r, reinterpret_cast<uintptr_t>(pNext.fBlock->ptr(pNext.fAlignedOffset)) -
                 reinterpret_cast<uintptr_t>(pNext.fBlock->ptr(p.fAlignedOffset)) ==
             32);
...
DEF_TEST(GrBlockAllocatorScratchBlockReserve, r) {
  ...
  REPORTER_ASSERT(
      r, (size_t)pool->testingOnly_scratchBlockSize() >= reserve &&
  ...
  REPORTER_ASSERT(
      r, pool->testingOnly_scratchBlockSize() > 0 && pool->testingOnly_scratchBlockSize() < 256);
