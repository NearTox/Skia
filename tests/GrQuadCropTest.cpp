...
#define ASSERT_NEARLY_EQUAL(expected, actual) \
  ASSERTF(SkScalarNearlyEqual(expected, actual), "expected: %f, actual: %f", expected, actual)
...
static void run_crop_axis_aligned_test(
    skiatest::Reporter* r, const SkRect& clipRect, GrAA clipAA, const SkMatrix& viewMatrix,
    const SkMatrix* localMatrix) {
  // Should use run_crop_fully_covers_test for non-rect matrices
  SkASSERT(viewMatrix.rectStaysRect());

  DrawQuad quad = {
      GrQuad::MakeFromRect(kDrawRect, viewMatrix),
...
static void run_crop_fully_covered_test(
    skiatest::Reporter* r, GrAA clipAA, const SkMatrix& viewMatrix, const SkMatrix* localMatrix) {
  ...
  DrawQuad quad = {
      GrQuad::MakeFromRect(drawRect, viewMatrix),
  ...
  if (localMatrix) {
    ...
    ASSERTF(quad.fEdgeFlags == originalQuad.fEdgeFlags, "Expected edge flags not to be modified");
    ...
    GrQuadAAFlags expectedFlags = clipAA == GrAA::kYes ? GrQuadAAFlags::kAll : GrQuadAAFlags::kNone;
    ASSERTF(expectedFlags == quad.fEdgeFlags, "Expected edge flags do not match clip AA setting");
...
static void test_axis_aligned_all_clips(
    skiatest::Reporter* r, const SkMatrix& viewMatrix, const SkMatrix* localMatrix) {
  ...
  static const SkRect kInsideClipRect =
      SkRect::MakeLTRB(-kInsideEdge, -kInsideEdge, kInsideEdge, kInsideEdge);
  static const SkRect kContainsClipRect =
      SkRect::MakeLTRB(-kOutsideEdge, -kOutsideEdge, kOutsideEdge, kOutsideEdge);
  static const SkRect kXYAxesClipRect =
      SkRect::MakeLTRB(-kIntersectEdge, -kIntersectEdge, kIntersectEdge, kIntersectEdge);
  static const SkRect kXAxisClipRect =
      SkRect::MakeLTRB(-kIntersectEdge, -kOutsideEdge, kIntersectEdge, kOutsideEdge);
  static const SkRect kYAxisClipRect =
      SkRect::MakeLTRB(-kOutsideEdge, -kIntersectEdge, kOutsideEdge, kIntersectEdge);
...
static void test_axis_aligned(skiatest::Reporter* r, const SkMatrix& viewMatrix) {
  ...
  SkMatrix normalized =
      SkMatrix::MakeRectToRect(kDrawRect, SkRect::MakeWH(1.f, 1.f), SkMatrix::kFill_ScaleToFit);
