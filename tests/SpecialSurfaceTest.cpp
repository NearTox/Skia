...
class TestingSpecialSurfaceAccess {
 public:
  static const SkIRect& Subset(const SkSpecialSurface* surf) { return surf->subset(); }
};
...
static void test_surface(
    const sk_sp<SkSpecialSurface>& surf, skiatest::Reporter* reporter, int offset) {
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SpecialSurface_Gpu1, reporter, ctxInfo) {
  ...
  for (auto colorType : {kRGBA_8888_SkColorType, kRGBA_1010102_SkColorType}) {
    ...
    SkImageInfo ii =
        SkImageInfo::Make({kSmallerSize, kSmallerSize}, colorType, kPremul_SkAlphaType);

    auto surf(SkSpecialSurface::MakeRenderTarget(
        dContext, ii, SkSurfaceProps(), kTopLeft_GrSurfaceOrigin));
    test_surface(surf, reporter, 0);
  }
}
