...
void check_result(
    skiatest::Reporter* reporter, sk_sp<SkVertices> verts, ExpectVerts expectVerts,
    bool expectSuccess) {
  if (expectSuccess != SkToBool(verts)) {
    ERRORF(
        reporter, "Expected shadow tessellation to %s but it did not.",
...
void tessellate_shadow(
    skiatest::Reporter* reporter, const SkPath& path, const SkMatrix& ctm,
  ...
  verts = SkShadowTessellator::MakeSpot(path, ctm, heightParams, {0, 0, 128}, 128.f, false, false);
  check_result(reporter, verts, expectVerts, expectSuccess);
...
DEF_TEST(ShadowUtils, reporter) {
  ...
  // super high path
  tessellate_shadow(
      reporter, path, canvas.getTotalMatrix(), {0, 0, 4.0e+37f}, kDo_ExpectVerts, true);
  ...
  path.cubicTo(
      -2.33326106e+21f, 7.36298265e-41f, 3.72237738e-22f, 5.99502692e-36f, 1.13631943e+22f,
      2.0890786e+33f);
  ...
  //    tessellate_shadow(reporter, path, canvas.getTotalMatrix(), {0, 0, 9}, kDo_ExpectVerts,
  //    true);
...
void check_xformed_bounds(skiatest::Reporter* reporter, const SkPath& path, const SkMatrix& ctm) {
  SkDrawShadowRec rec = {
      SkPoint3::Make(0, 0, 4), SkPoint3::Make(100, 0, 600), 800.f, 0x08000000, 0x40000000, 0};
  // point light
  SkRect bounds;
  ...
  verts = SkShadowTessellator::MakeSpot(
      path, ctm, rec.fZPlaneParams, devLightPos, rec.fLightRadius, false, false);
  if (verts) {
    REPORTER_ASSERT(reporter, bounds.contains(verts->bounds()));
  }
  ...
  verts = SkShadowTessellator::MakeSpot(
      path, ctm, rec.fZPlaneParams, devLightPos, rec.fLightRadius, false, true);
  if (verts) {
    REPORTER_ASSERT(reporter, bounds.contains(verts->bounds()));
  }
}
