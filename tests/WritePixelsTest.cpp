...
static void test_write_pixels(skiatest::Reporter* reporter, GrContext* context, int sampleCnt) {
  ...
  for (auto& origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
    sk_sp<SkSurface> surface(
        SkSurface::MakeRenderTarget(context, SkBudgeted::kNo, ii, sampleCnt, origin, nullptr));
    if (surface) {
...
static void test_write_pixels_non_texture(
  ...
  for (auto& origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
    GrBackendTexture backendTex = context->createBackendTexture(
        DEV_W, DEV_H, kRGBA_8888_SkColorType, SkColors::kTransparent, GrMipMapped::kNo,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(WritePixelsPendingIO, reporter, ctxInfo) {
...

  {
    ...
    sk_sp<GrTextureProxy> temp = proxyProvider->createProxy(
        format, kDims, swizzle, GrRenderable::kNo, 1, GrMipMapped::kNo, SkBackingFit::kApprox,
