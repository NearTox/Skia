...
bool write_should_succeed(const SkImageInfo& dstInfo, const SkImageInfo& srcInfo, bool isGPU) {
  ...
  if (srcInfo.alphaType() == kPremul_SkAlphaType && dstInfo.alphaType() == kUnpremul_SkAlphaType) {
    return false;
  }
...
static bool check_write(
    skiatest::Reporter* reporter, SkSurface* surf, SkAlphaType surfaceAlphaType,
    ...
    for (int cx = 0; cx < DEV_W; ++cx) {
      ...
      if (writeRect.contains(cx, cy)) {
        ...
        uint32_t bmpColor8888 =
            get_bitmap_color(bx, by, bitmap.width(), bmInfo.colorType(), bmInfo.alphaType());
        bool mul = (kUnpremul_SkAlphaType == bmInfo.alphaType());
        SkPMColor bmpPMColor =
            convert_to_PMColor(bmInfo.colorType(), bmInfo.alphaType(), bmpColor8888);
        if (bmInfo.alphaType() == kOpaque_SkAlphaType || surfaceAlphaType == kOpaque_SkAlphaType) {
...
static void test_write_pixels(
    skiatest::Reporter* reporter, SkSurface* surface, const SkImageInfo& surfaceInfo) {
  ...
  for (size_t r = 0; r < SK_ARRAY_COUNT(testRects); ++r) {
    ...
    for (int tightBmp = 0; tightBmp < 2; ++tightBmp) {
      for (size_t c = 0; c < SK_ARRAY_COUNT(gSrcConfigs); ++c) {
        ...
        SkBitmap bmp;
        REPORTER_ASSERT(
            reporter, setup_bitmap(&bmp, ct, at, rect.width(), rect.height(), SkToBool(tightBmp)));
        ...
        REPORTER_ASSERT(
            reporter,
            check_write(reporter, surface, surfaceInfo.alphaType(), bmp, rect.fLeft, rect.fTop));

        // we should change the genID iff pixels were actually written.
        SkIRect canvasRect = SkIRect::MakeSize(canvas->getBaseLayerSize());
        SkIRect writeRect = SkIRect::MakeXYWH(rect.fLeft, rect.fTop, bmp.width(), bmp.height());
...
DEF_TEST(WritePixels, reporter) {
  ...
  for (auto& tightRowBytes : {true, false}) {
    ...
    auto surface(
        SkSurface::MakeRasterDirectReleaseProc(info, pixels, rowBytes, free_pixels, nullptr));
    test_write_pixels(reporter, surface.get(), info);
  }
}

static void test_write_pixels(
    skiatest::Reporter* reporter, GrRecordingContext* rContext, int sampleCnt) {
  const SkImageInfo ii = SkImageInfo::MakeN32Premul(DEV_W, DEV_H);
  for (auto& origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
    sk_sp<SkSurface> surface(
        SkSurface::MakeRenderTarget(rContext, SkBudgeted::kNo, ii, sampleCnt, origin, nullptr));
    if (surface) {
...
static void test_write_pixels_non_texture(
    skiatest::Reporter* reporter, GrDirectContext* dContext, int sampleCnt) {
  ...
  for (auto& origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
    ...
    auto surface = sk_gpu_test::MakeBackendRenderTargetSurface(
        dContext, {DEV_W, DEV_H}, origin, sampleCnt, colorType);
    if (surface) {
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(WritePixelsPendingIO, reporter, ctxInfo) {
  ...
  const SkImageInfo fullII =
      SkImageInfo::Make(kFullSize, kFullSize, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
  const SkImageInfo halfII =
      SkImageInfo::Make(kHalfSize, kFullSize, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
