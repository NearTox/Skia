...
static void test_rectanizer_inserts(
    skiatest::Reporter*, GrRectanizerSkyline* rectanizer, const SkTDArray<SkISize>& rects) {
  int i;
...
DEF_GPUTEST(GpuRectanizer, reporter, factory) {
  ...
  for (int i = 0; i < 50; i++) {
    rects.push_back(SkISize::Make(rand.nextRangeU(1, kWidth / 2), rand.nextRangeU(1, kHeight / 2)));
  }
