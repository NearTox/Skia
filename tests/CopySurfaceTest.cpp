            ...
            for (const SkIPoint& dstPoint : kDstPoints) {
              for (const SkImageInfo& ii : kImageInfos) {
                auto srcView = sk_gpu_test::MakeTextureProxyViewFromData(
                    dContext, sRenderable, sOrigin, ii, srcPixels.get(), kRowBytes);
                auto dstView = sk_gpu_test::MakeTextureProxyViewFromData(
                    dContext, dRenderable, dOrigin, ii, dstPixels.get(), kRowBytes);
                ...
                if (ii.colorType() == kRGBA_8888_SkColorType) {
                  ...
                  if (!dContext->defaultBackendFormat(kBGRA_8888_SkColorType, GrRenderable::kNo)
                           .isValid()) {
                    continue;
                  }
                  if (!srcView || !dstView) {
                ...
                auto dstContext = GrSurfaceContext::Make(
                    dContext, std::move(dstView), grColorType, ii.alphaType(), nullptr);
                ...
                if (!dstContext->readPixels(dContext, ii, read.get(), kRowBytes, {0, 0})) {
