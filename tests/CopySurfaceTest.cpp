...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(CopySurface, reporter, ctxInfo) {
  ...
  static const SkIPoint kDstPoints[]{
      {0, 0}, {1, 1}, {kW / 2, kH / 4}, {kW - 1, kH - 1}, {kW, kH}, {kW + 1, kH + 2}, {-1, -1},
  };
            ...
            for (auto dstPoint : kDstPoints) {
              for (auto ii : kImageInfos) {
                auto src = sk_gpu_test::MakeTextureProxyFromData(
                    context, sRenderable, sOrigin, ii, srcPixels.get(), kRowBytes);
                auto dst = sk_gpu_test::MakeTextureProxyFromData(
                    context, dRenderable, dOrigin, ii, dstPixels.get(), kRowBytes);
                ...
                if (ii.colorType() == kRGBA_8888_SkColorType) {
                  ...
                  if (!context->defaultBackendFormat(kBGRA_8888_SkColorType, GrRenderable::kNo)
                           .isValid()) {
                    continue;
                  }
                  if (!src || !dst) {
                ...
                GrSwizzle dstSwizzle =
                    context->priv().caps()->getReadSwizzle(dst->backendFormat(), grColorType);
                GrSurfaceProxyView dstView(std::move(dst), dOrigin, dstSwizzle);
                auto dstContext = GrSurfaceContext::Make(
                    context, std::move(dstView), grColorType, ii.alphaType(), nullptr);

                bool result = false;
                if (sOrigin == dOrigin) {
                  result = dstContext->testCopy(src.get(), sOrigin, srcRect, dstPoint);
                } else if (dRenderable == GrRenderable::kYes) {
                  SkASSERT(dstContext->asRenderTargetContext());
                  GrSwizzle srcSwizzle =
                      context->priv().caps()->getReadSwizzle(src->backendFormat(), grColorType);
                ...
                SkIPoint dstOffset = {dstPoint.fX - srcRect.fLeft, dstPoint.fY - srcRect.fTop};
                SkIRect copiedDstRect =
                ...
                if (expectedResult != result) {
                  ERRORF(
                      reporter,
                      "Expected return value %d from copySurface, "
                      "got %d.",
