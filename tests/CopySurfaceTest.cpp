            ...
            for (auto dstPoint : kDstPoints) {
              for (auto ii : kImageInfos) {
                auto src = sk_gpu_test::MakeTextureProxyFromData(
                    context, sRenderable, kW, kH, ii.colorType(), ii.alphaType(), sOrigin,
                    srcPixels.get(), kRowBytes);
                auto dst = sk_gpu_test::MakeTextureProxyFromData(
                    context, dRenderable, kW, kH, ii.colorType(), ii.alphaType(), dOrigin,
                ...
                auto dstContext = context->priv().makeWrappedSurfaceContext(
                    std::move(dst), SkColorTypeToGrColorType(ii.colorType()), ii.alphaType());
