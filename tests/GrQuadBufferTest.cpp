...
static void assert_quad_eq(skiatest::Reporter* r, const GrQuad& expected, const GrQuad& actual) {
  ASSERTF(
      expected.quadType() == actual.quadType(), "Expected type %d, got %d",
...
static void assert_metadata_eq(
    skiatest::Reporter* r, const TestData& expected, const TestData& actual) {
  ASSERTF(
      expected.fItem1 == actual.fItem1 && expected.fItem2 == actual.fItem2,
      "Expected { %d, %f } for metadata, got: { %d %f }", expected.fItem1, expected.fItem2,
      actual.fItem1, actual.fItem2);
}
...
static std::vector<GrQuad> generate_quads(float seed, int cnt, const GrQuad::Type types[]) {
  ...
  for (int i = 0; i < cnt; ++i) {
    GrQuad quad;
    switch (types[i]) {
      case GrQuad::Type::kAxisAligned: quad = GrQuad(rect); break;
      case GrQuad::Type::kRectilinear: quad = GrQuad::MakeFromRect(rect, rotate); break;
      case GrQuad::Type::kGeneral: quad = GrQuad::MakeFromRect(rect, skew); break;
      default:
...
TEST(Append) {
  ...
  for (int i = 0; i < kQuadCount; ++i) {
    buffer.append(
        expectedDeviceQuads[i],                          // device quad
        {2 * i, 3.f * i},                                // metadata
