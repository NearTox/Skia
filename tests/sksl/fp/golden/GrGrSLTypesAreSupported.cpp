 ...
 public:
  ...
  void emitCode(EmitArgs& args) override {
    ...
    fragBuilder->emitFunction(
        kInt_GrSLType, test_i_name.c_str(), {test_i_args, 1},
        R"SkSL(for (; ; ) {
    return a;
}
)SkSL");
    fragBuilder->emitFunction(
        kInt2_GrSLType, test_i2_name.c_str(), {test_i2_args, 1},
        R"SkSL(for (; ; ) {
    return a;
}
)SkSL");
    fragBuilder->emitFunction(
        kInt3_GrSLType, test_i3_name.c_str(), {test_i3_args, 1},
        R"SkSL(for (; ; ) {
    return a;
}
)SkSL");
    fragBuilder->emitFunction(
        kInt4_GrSLType, test_i4_name.c_str(), {test_i4_args, 1},
        R"SkSL(for (; ; ) {
    return a;
}
)SkSL");
    fragBuilder->emitFunction(
        kHalf3x4_GrSLType, test_h3x4_name.c_str(), {test_h3x4_args, 1},
        R"SkSL(for (; ; ) {
    return a;
}
)SkSL");
    fragBuilder->emitFunction(
        kFloat2x4_GrSLType, test_f2x4_name.c_str(), {test_f2x4_args, 1},
        R"SkSL(for (; ; ) {
    return a;
}
)SkSL");
...
void GrGrSLTypesAreSupported::onGetGLSLProcessorKey(
...
#if GR_TEST_UTILS
SkString GrGrSLTypesAreSupported::onDumpInfo() const { return SkString(); }
#endif
