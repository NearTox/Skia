...
GrGLSLFragmentProcessor* GrInUniform::onCreateGLSLInstance() const { return new GrGLSLInUniform(); }
...
bool GrInUniform::usesExplicitReturn() const { return false; }
GrInUniform::GrInUniform(const GrInUniform& src)
    : INHERITED(kGrInUniform_ClassID, src.optimizationFlags()), color(src.color) {
...
SkString GrInUniform::onDumpInfo() const {
  return SkStringPrintf(
      "(color=half4(%f, %f, %f, %f))", color.left(), color.top(), color.right(), color.bottom());
}
#endif
