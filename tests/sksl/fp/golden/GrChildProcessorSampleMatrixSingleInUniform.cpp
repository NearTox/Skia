...
class GrGLSLChildProcessorSampleMatrixSingleInUniform : public GrGLSLFragmentProcessor {
 public:
  ...
  void emitCode(EmitArgs& args) override {
    ...
    const GrChildProcessorSampleMatrixSingleInUniform& _outer =
        args.fFp.cast<GrChildProcessorSampleMatrixSingleInUniform>();
    (void)_outer;
 ...
 private:
  void onSetData(const GrGLSLProgramDataManager& pdman, const GrFragmentProcessor& _proc) override {
    const GrChildProcessorSampleMatrixSingleInUniform& _outer =
        _proc.cast<GrChildProcessorSampleMatrixSingleInUniform>();
    {
      static_assert(1 == 1);
...
void GrChildProcessorSampleMatrixSingleInUniform::onGetGLSLProcessorKey(
    const GrShaderCaps& caps, GrProcessorKeyBuilder* b) const {}
bool GrChildProcessorSampleMatrixSingleInUniform::onIsEqual(
    const GrFragmentProcessor& other) const {
  const GrChildProcessorSampleMatrixSingleInUniform& that =
      other.cast<GrChildProcessorSampleMatrixSingleInUniform>();
  (void)that;
  if (matrix != that.matrix) return false;
  return true;
}
bool GrChildProcessorSampleMatrixSingleInUniform::usesExplicitReturn() const { return false; }
GrChildProcessorSampleMatrixSingleInUniform::GrChildProcessorSampleMatrixSingleInUniform(
...
SkString GrChildProcessorSampleMatrixSingleInUniform::onDumpInfo() const {
  return SkStringPrintf(
      "(matrix=float3x3(%f, %f, %f, %f, %f, %f, %f, %f, %f))", matrix.rc(0, 0), matrix.rc(1, 0),
      matrix.rc(2, 0), matrix.rc(0, 1), matrix.rc(1, 1), matrix.rc(2, 1), matrix.rc(0, 2),
      matrix.rc(1, 2), matrix.rc(2, 2));
}
#endif
