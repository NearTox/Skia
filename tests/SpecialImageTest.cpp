...
static SkBitmap create_bm() {
  ...
  temp.drawRect(
      SkRect::MakeXYWH(
          SkIntToScalar(kPad), SkIntToScalar(kPad), SkIntToScalar(kSmallerSize),
          SkIntToScalar(kSmallerSize)),
      p);
...
static void test_image(
    ...
    bool isGPUBacked, int offset, int size) {
  ...
  REPORTER_ASSERT(
      reporter, SK_ColorRED == bm.getColor(kSmallerSize + kPad - 1, kSmallerSize + kPad - 1));
  ...
  }
  {
    ...
    GrBackendTexture backendTex =
        tightSurf->getBackendTexture(SkSurface::kDiscardWrite_BackendHandleAccess);
...
DEF_TEST(SpecialImage_Raster, reporter) {
  ...
  sk_sp<SkSpecialImage> fullSImage(
      SkSpecialImage::MakeFromRaster(SkIRect::MakeWH(kFullSize, kFullSize), bm));
...
static void test_specialimage_image(skiatest::Reporter* reporter) {
  ...
  sk_sp<SkSpecialImage> fullSImage(
      SkSpecialImage::MakeFromImage(nullptr, SkIRect::MakeWH(kFullSize, kFullSize), fullImage));
...
static void test_texture_backed(
    skiatest::Reporter* reporter, const sk_sp<SkSpecialImage>& orig,
  ...
  REPORTER_ASSERT(
      reporter, gpuBacked->subset().width() == orig->subset().width() &&
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SpecialImage_MakeTexture, reporter, ctxInfo) {
  ...
  {
    // raster
    sk_sp<SkSpecialImage> rasterImage(
        SkSpecialImage::MakeFromRaster(SkIRect::MakeWH(kFullSize, kFullSize), bm));
...

  {
    ...
    sk_sp<GrTextureProxy> proxy = proxyProvider->createTextureProxy(
        rasterImage, kNone_GrSurfaceFlags, 1, SkBudgeted::kNo, SkBackingFit::kExact);
    if (!proxy) {
      return;
    }
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SpecialImage_Gpu, reporter, ctxInfo) {
  ...
  sk_sp<SkSpecialImage> fullSImg(SkSpecialImage::MakeDeferredFromGpu(
      context, SkIRect::MakeWH(kFullSize, kFullSize), kNeedNewImageUniqueID_SpecialImage, proxy,
      nullptr));
...

  {
    sk_sp<SkSpecialImage> subSImg1(SkSpecialImage::MakeDeferredFromGpu(
        context, subset, kNeedNewImageUniqueID_SpecialImage, std::move(proxy), nullptr));
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SpecialImage_ReadbackAndCachingSubsets_Gpu, reporter, ctxInfo) {
  ...
  REPORTER_ASSERT(
      reporter, redBM.getColor(0, 0) == SK_ColorRED, "0x%08x != 0x%08x", redBM.getColor(0, 0),
      SK_ColorRED);
  REPORTER_ASSERT(
      reporter, blueBM.getColor(0, 0) == SK_ColorBLUE, "0x%08x != 0x%08x", blueBM.getColor(0, 0),
      SK_ColorBLUE);
}
