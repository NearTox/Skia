...
DEF_TEST(SpecialImage_Raster, reporter) {
  ...
  sk_sp<SkSpecialImage> fullSImage(
      SkSpecialImage::MakeFromRaster(SkIRect::MakeWH(kFullSize, kFullSize), bm, SkSurfaceProps()));

  const SkIRect& subset = SkIRect::MakeXYWH(kPad, kPad, kSmallerSize, kSmallerSize);

  {
    sk_sp<SkSpecialImage> subSImg1(SkSpecialImage::MakeFromRaster(subset, bm, SkSurfaceProps()));
...
static void test_specialimage_image(skiatest::Reporter* reporter) {
  ...
  sk_sp<SkSpecialImage> fullSImage(SkSpecialImage::MakeFromImage(
      nullptr, SkIRect::MakeWH(kFullSize, kFullSize), fullImage, SkSurfaceProps()));

  const SkIRect& subset = SkIRect::MakeXYWH(kPad, kPad, kSmallerSize, kSmallerSize);

  {
    sk_sp<SkSpecialImage> subSImg1(
        SkSpecialImage::MakeFromImage(nullptr, subset, fullImage, SkSurfaceProps()));
    test_image(subSImg1, reporter, nullptr, false);
  }
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SpecialImage_Gpu, reporter, ctxInfo) {
  ...
  sk_sp<SkSpecialImage> fullSImg = SkSpecialImage::MakeDeferredFromGpu(
      context, SkIRect::MakeWH(kFullSize, kFullSize), kNeedNewImageUniqueID_SpecialImage, view, ct,
...

  {
    sk_sp<SkSpecialImage> subSImg1 = SkSpecialImage::MakeDeferredFromGpu(
        context, subset, kNeedNewImageUniqueID_SpecialImage, std::move(view), ct, nullptr,
        SkSurfaceProps());
