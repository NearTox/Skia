...
DEF_TEST(TestNormal, reporter) { REPORTER_ASSERT(reporter, reporter); }

// This is an example of a conditional test with a true condition.
DEF_CONDITIONAL_TEST(TestTrueCondition, reporter, 1 == 1) { REPORTER_ASSERT(reporter, reporter); }
...
DEF_GPUTEST(TestGpuFactory, reporter, factory) { REPORTER_ASSERT(reporter, reporter); }
...
DEF_CONDITIONAL_GPUTEST_FOR_ALL_CONTEXTS(
    TestGpuAllContextsWithTrueCondition, reporter, ctxInfo, true) {
  REPORTER_ASSERT(reporter, reporter);
  REPORTER_ASSERT(reporter, ctxInfo.directContext());
}

DEF_CONDITIONAL_GPUTEST_FOR_ALL_CONTEXTS(
    TestGpuAllContextsWithFalseCondition, reporter, ctxInfo, false) {
...
DEF_CONDITIONAL_GPUTEST_FOR_RENDERING_CONTEXTS(
    TestGpuRenderingContextsWithTrueCondition, reporter, ctxInfo, true) {
  REPORTER_ASSERT(reporter, reporter);
  REPORTER_ASSERT(reporter, ctxInfo.directContext());
}

DEF_CONDITIONAL_GPUTEST_FOR_RENDERING_CONTEXTS(
    TestGpuRenderingContextsWithFalseCondition, reporter, ctxInfo, false) {
