...
static void run_test(
    GrContext* context, const char* testName, skiatest::Reporter*,
...
#define VALIDATE(buff)                     \
  ...
  run_test(
      context, "draw", reporter, rtc, gold,
  ...
  run_test(
      context, "drawIndexed", reporter, rtc, gold,
      [&](DrawMeshHelper* helper) {
      ...
      [&](DrawMeshHelper* helper) {
        ...
        while (i < kBoxCount) {
          ...
          auto pass = helper->bindPipeline(GrPrimitiveType::kTriangles, false, true);
          pass->bindBuffers(helper->fIndexBuffer.get(), nullptr, helper->fVertBuffer.get());
          pass->drawIndexed(
  ...
  run_test(
      context, "drawIndexPattern", reporter, rtc, gold,
      [&](DrawMeshHelper* helper) {
        helper->fIndexBuffer = helper->getIndexBuffer();
        VALIDATE(helper->fIndexBuffer);
        helper->fVertBuffer = helper->makeVertexBuffer(vertexData);
  ...
  for (bool indexed : {false, true}) {
    ...
    run_test(
        context, indexed ? "drawIndexedInstanced" : "drawInstanced", reporter, rtc, gold,
        [&](DrawMeshHelper* helper) {
          ...
          for (int y = 0; y < kBoxCountY; ++y) {
            ...
            switch (y % 3) {
              case 0:
                ...
                // Fallthru.
              case 1: vertexBuffer = helper->fVertBuffer.get(); break;
              case 2:
            ...
            if (indexed) {
              const GrBuffer* indexBuffer =
                  (y % 2) ? helper->fIndexBuffer2.get() : helper->fIndexBuffer.get();
              VALIDATE(indexBuffer);
              int baseIndex = (y % 2);
              pass->bindBuffers(indexBuffer, helper->fInstBuffer.get(), vertexBuffer);
  ...
  for (bool indexed : {false, true}) {
    ...
    run_test(
        context, (indexed) ? "drawIndexedIndirect" : "drawIndirect", reporter, rtc, gold,
        [&](DrawMeshHelper* helper) {
          SkTArray<uint16_t> baseIndexData;
          baseIndexData.push_back(kBoxCountX / 2 * 6);  // for testing base index.
        ...
        [&](DrawMeshHelper* helper) {
          ...
          if (indexed) {
            ...
            pass->bindBuffers(
                helper->fIndexBuffer2.get(), helper->fInstBuffer.get(), helper->fVertBuffer.get());
            for (int i = 0; i < 3; ++i) {
            ...
            pass->bindBuffers(nullptr, helper->fInstBuffer.get(), helper->fVertBuffer.get());
            for (int i = 0; i < 2; ++i) {
...
class GrMeshTestOp : public GrDrawOp {
 public:
  ...
  static std::unique_ptr<GrDrawOp> Make(
      GrContext* context, std::function<void(DrawMeshHelper*)> prepareFn,
...
static void run_test(
    GrContext* context, const char* testName, skiatest::Reporter* reporter,
  ...
  rtc->clear(
      nullptr, SkPMColor4f::FromBytes_RGBA(0xbaaaaaad),
