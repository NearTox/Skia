...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrMeshTest, reporter, ctxInfo) {
  ...
  sk_sp<GrRenderTargetContext> rtc(context->priv().makeDeferredRenderTargetContext(
      format, SkBackingFit::kExact, kImageWidth, kImageHeight, kRGBA_8888_GrPixelConfig, nullptr));
  if (!rtc) {
...
#define VALIDATE(buff)                     \
  ...
  run_test(context, "setNonIndexedNonInstanced", reporter, rtc, gold, [&](DrawMeshHelper* helper) {
  ...
  run_test(context, "setIndexed", reporter, rtc, gold, [&](DrawMeshHelper* helper) {
    ...
    while (i < kBoxCount) {
      ...
      GrMesh mesh(GrPrimitiveType::kTriangles);
      mesh.setIndexed(
          ibuff, repetitionCount * 6, baseRepetition * 6, baseRepetition * 4,
  ...
  for (bool indexed : {false, true}) {
    ...
    run_test(
        context, indexed ? "setIndexedInstanced" : "setInstanced", reporter, rtc, gold,
          ...
          for (int y = 0; y < kBoxCountY; ++y) {
            ...
            if (indexed) {
              ...
              mesh.setIndexedInstanced(
                  idxbuff, 6, instbuff, kBoxCountX, y * kBoxCountX, GrPrimitiveRestart::kNo);
            } else {
            ...
            switch (y % 3) {
              case 0:
                ...
                // Fallthru.
              case 1: mesh.setVertexData(vbuff); break;
              case 2: mesh.setVertexData(vbuff2, 2); break;
            }
            helper->drawMesh(mesh);
          }
        });
...
class GrMeshTestOp : public GrDrawOp {
 public:
  ...
  static std::unique_ptr<GrDrawOp> Make(
      GrContext* context, std::function<void(DrawMeshHelper*)> testFn) {
 ...
 private:
  ...
  GrMeshTestOp(std::function<void(DrawMeshHelper*)> testFn)
      : INHERITED(ClassID()), fTestFn(testFn) {
    this->setBounds(SkRect::MakeIWH(kImageWidth, kImageHeight), HasAABloat::kNo, IsZeroArea::kNo);
  }
...
void DrawMeshHelper::drawMesh(const GrMesh& mesh) {
  ...
  fState->rtCommandBuffer()->draw(
      mtp, pipeline, nullptr, nullptr, &mesh, 1, SkRect::MakeIWH(kImageWidth, kImageHeight));
}
