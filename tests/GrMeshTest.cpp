...
class DrawMeshHelper {
 public:
  ...
  template <typename T>
  sk_sp<const GrBuffer> makeVertexBuffer(const SkTArray<T>& data) {
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrMeshTest, reporter, ctxInfo) {
  ...
  auto sdc = skgpu::v1::SurfaceDrawContext::Make(
      dContext, GrColorType::kRGBA_8888, nullptr, SkBackingFit::kExact, {kImageWidth, kImageHeight},
      SkSurfaceProps());
  if (!sdc) {
  ...
  for (int y = 0; y < kBoxCountY; ++y) {
    for (int x = 0; x < kBoxCountX; ++x) {
      ...
      for (int i = 0; i < 4; ++i) {
        boxVertices[i] = {box.fX + (i / 2) * kBoxSize, box.fY + (i % 2) * kBoxSize, box.fColor};
      }
...
#define VALIDATE(buff)                     \
  ...
  run_test(
      dContext, "draw", reporter, sdc, gold,
  ...
  run_test(
      dContext, "drawIndexed", reporter, sdc, gold,
      [&](DrawMeshHelper* helper) {
      ...
      [&](DrawMeshHelper* helper) {
        ...
        while (i < kBoxCount) {
          ...
          pass->drawIndexed(
              repetitionCount * 6, baseRepetition * 6, baseRepetition * 4,
              (baseRepetition + repetitionCount) * 4 - 1, (i - baseRepetition) * 4);
  ...
  run_test(
      dContext, "drawIndexPattern", reporter, sdc, gold,
      [&](DrawMeshHelper* helper) {
        helper->fIndexBuffer = helper->getIndexBuffer();
        VALIDATE(helper->fIndexBuffer);
        helper->fVertBuffer = helper->makeVertexBuffer(vertexData);
      ...
      [&](DrawMeshHelper* helper) {
        ...
        for (int y = 0; y < kBoxCountY; ++y) {
          auto pass = helper->bindPipeline(GrPrimitiveType::kTriangles, false, true);
          pass->bindBuffers(helper->fIndexBuffer, nullptr, helper->fVertBuffer);
          pass->drawIndexPattern(6, kBoxCountX, kIndexPatternRepeatCount, 4, y * kBoxCountX * 4);
        }
      });
  ...
  for (bool indexed : {false, true}) {
    ...
    run_test(
        dContext, indexed ? "drawIndexedInstanced" : "drawInstanced", reporter, sdc, gold,
        [&](DrawMeshHelper* helper) {
          ...
          helper->fIndexBuffer2 =
              helper->makeIndexBuffer(baseIndexData.begin(), baseIndexData.count());
          ...
          for (int y = 0; y < kBoxCountY; ++y) {
            ...
            switch (y % 3) {
              case 0:
                ...
                [[fallthrough]];
              case 1: vertexBuffer = helper->fVertBuffer; break;
              case 2:
            ...
            auto pass = helper->bindPipeline(primitiveType, true, SkToBool(vertexBuffer));
            if (indexed) {
              sk_sp<const GrBuffer> indexBuffer =
                  (y % 2) ? helper->fIndexBuffer2 : helper->fIndexBuffer;
              VALIDATE(indexBuffer);
              int baseIndex = (y % 2);
              pass->bindBuffers(
                  std::move(indexBuffer), helper->fInstBuffer, std::move(vertexBuffer));
              pass->drawIndexedInstanced(6, baseIndex, kBoxCountX, y * kBoxCountX, baseVertex);
            } else {
              pass->bindBuffers(nullptr, helper->fInstBuffer, std::move(vertexBuffer));
  ...
  for (bool indexed : {false, true}) {
    ...
    run_test(
        dContext, (indexed) ? "drawIndexedIndirect" : "drawIndirect", reporter, sdc, gold,
        [&](DrawMeshHelper* helper) {
          SkTArray<uint16_t> baseIndexData;
          baseIndexData.push_back(kBoxCountX / 2 * 6);  // for testing base index.
          ...
          helper->fInstBuffer = helper->makeVertexBuffer(boxes);
          VALIDATE(helper->fInstBuffer);
          helper->fVertBuffer = helper->makeVertexBuffer(
              std::vector<float>{-1, -1, 0, 0, 0, 1, 1, 0, 1, 1, -1, -1, 0, 0, 1, 0, 0, 1, 1, 1});
          VALIDATE(helper->fVertBuffer);
          ...
          if (indexed) {
            ...
            indexedIndirectWriter = helper->target()->makeDrawIndexedIndirectSpace(
                kBoxCountY, &helper->fDrawIndirectBuffer, &helper->fDrawIndirectBufferOffset);
          } else {
            // Make helper->fDrawIndirectBufferOffset nonzero.
            sk_sp<const GrBuffer> ignoredBuff;
            size_t ignoredOffset;
            ...
            helper->target()->makeDrawIndexedIndirectSpace(7, &ignoredBuff, &ignoredOffset);
          ...
          for (int y = 0; y < kBoxCountY; ++y) {
            ...
            if (indexed) {
              int baseIndex = 1 + y * 6;
              indexedIndirectWriter.writeIndexed(
                  6, baseIndex, kBoxCountX, y * kBoxCountX, baseVertex);
            } else {
        ...
        [&](DrawMeshHelper* helper) {
          ...
          if (indexed) {
            ...
            pass->bindBuffers(helper->fIndexBuffer2, helper->fInstBuffer, helper->fVertBuffer);
            for (int i = 0; i < 3; ++i) {
              int start = kBoxCountY * i / 3;
              int end = kBoxCountY * (i + 1) / 3;
              size_t offset =
                  helper->fDrawIndirectBufferOffset + start * sizeof(GrDrawIndexedIndirectCommand);
              pass->drawIndexedIndirect(helper->fDrawIndirectBuffer.get(), offset, end - start);
            }
          } else {
            ...
            for (int i = 0; i < 2; ++i) {
              ...
              size_t offset =
                  helper->fDrawIndirectBufferOffset + start * sizeof(GrDrawIndirectCommand);
              pass->drawIndirect(helper->fDrawIndirectBuffer.get(), offset, end - start);
            }
          }
        });
  }
...
class MeshTestOp : public GrDrawOp {
 public:
  ...
  static GrOp::Owner Make(
      GrRecordingContext* rContext, std::function<void(DrawMeshHelper*)> prepareFn,
 ...
 private:
  ...
  MeshTestOp(
    ...
    this->setBounds(SkRect::MakeIWH(kImageWidth, kImageHeight), HasAABloat::kNo, IsHairline::kNo);
  }
...
class MeshTestProcessor : public GrGeometryProcessor {
 public:
  static GrGeometryProcessor* Make(SkArenaAlloc* arena, bool instanced, bool hasVertexBuffer) {
    return arena->make(
        [&](void* ptr) { return new (ptr) MeshTestProcessor(instanced, hasVertexBuffer); });
  }
...
std::unique_ptr<GrGeometryProcessor::ProgramImpl> MeshTestProcessor::makeProgramImpl(
  ...
  class Impl : public ProgramImpl {
   public:
    void setData(
        const GrGLSLProgramDataManager&, const GrShaderCaps&, const GrGeometryProcessor&) final {}
...
GrOpsRenderPass* DrawMeshHelper::bindPipeline(
    GrPrimitiveType primitiveType, bool isInstanced, bool hasVertexBuffer) {
  ...
  processorSet.finalize(
      ...
      nullptr, fState->caps(), GrClampType::kAuto, &overrideColor);
  ...
  GrGeometryProcessor* mtp =
      MeshTestProcessor::Make(fState->allocator(), isInstanced, hasVertexBuffer);
...
static void run_test(
    ...
    const std::unique_ptr<skgpu::v1::SurfaceDrawContext>& sdc, const SkBitmap& gold,
    std::function<void(DrawMeshHelper*)> prepareFn,
    std::function<void(DrawMeshHelper*)> executeFn) {
