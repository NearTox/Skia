...
DEF_GPUTEST_FOR_METAL_CONTEXT(MtlCopySurfaceTest, reporter, ctxInfo) {
  ...
  sk_sp<GrSurfaceProxy> srcProxy =
      proxyProvider->wrapBackendRenderTarget(backendRT, GrColorType::kBGRA_8888);

  GrSurfaceProxyView dstView = GrSurfaceProxy::Copy(
      context, srcProxy.get(), kTopLeft_GrSurfaceOrigin, GrColorType::kBGRA_8888, GrMipMapped::kNo,
  ...
  sk_sp<GrTexture> dst = gpu->createTexture(
      {kWidth, kHeight}, backendFormat, GrRenderable::kNo, 1, GrMipMapped::kNo, SkBudgeted::kNo,
      GrProtected::kNo);

  bool result = gpu->copySurface(
      dst.get(), src, SkIRect::MakeXYWH(0, 0, kWidth, kHeight), SkIPoint::Make(0, 0));
