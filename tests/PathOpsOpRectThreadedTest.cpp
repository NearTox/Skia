        ...
        for (int d = c + 1; d < 7; ++d) {
          for (int e = SkPath::kWinding_FillType; e <= SkPath::kEvenOdd_FillType; ++e) {
            for (int f = SkPath::kWinding_FillType; f <= SkPath::kEvenOdd_FillType; ++f) {
              ...
              pathA.addRect(
                  SkIntToScalar(state.fA), SkIntToScalar(state.fA), SkIntToScalar(state.fB),
                  SkIntToScalar(state.fB), SkPath::kCW_Direction);
              pathA.addRect(
                  SkIntToScalar(state.fC), SkIntToScalar(state.fC), SkIntToScalar(state.fD),
                  SkIntToScalar(state.fD), SkPath::kCW_Direction);
              pathA.close();
              pathB.setFillType((SkPath::FillType)f);
              pathB.addRect(
                  SkIntToScalar(a), SkIntToScalar(a), SkIntToScalar(b), SkIntToScalar(b),
                  SkPath::kCW_Direction);
              pathB.addRect(
                  SkIntToScalar(c), SkIntToScalar(c), SkIntToScalar(d), SkIntToScalar(d),
                  SkPath::kCW_Direction);
              pathB.close();
                ...
                if (state.fReporter->verbose()) {
                  ...
                  pathStr.appendf(
                      ...
                      e == SkPath::kWinding_FillType
                          ? "Winding"
                          : e == SkPath::kEvenOdd_FillType ? "EvenOdd" : "?UNDEFINED");
                  pathStr.appendf(
                  ...
                  pathStr.appendf(
                      "    pathB.setFillType(SkPath::k%s_FillType);\n",
                      f == SkPath::kWinding_FillType
                          ? "Winding"
                          : f == SkPath::kEvenOdd_FillType ? "EvenOdd" : "?UNDEFINED");
                  pathStr.appendf(
      ...
      for (int c = 0; c < 6; ++c) {
        for (int d = c + 1; d < 7; ++d) {
          *testRunner.fRunnables.append() =
              new PathOpsThreadedRunnable(&testPathOpsRectsMain, a, b, c, d, &testRunner);
        }
      }
        ...
        for (int d = 0; d < 6; d += step) {
          for (int e = SkPath::kWinding_FillType; e <= SkPath::kInverseEvenOdd_FillType; ++e) {
            for (int f = SkPath::kWinding_FillType; f <= SkPath::kInverseEvenOdd_FillType; ++f) {
              ...
              if (a) {
                pathA.addRect(
                    SkIntToScalar(state.fA), SkIntToScalar(state.fA), SkIntToScalar(state.fB) + c,
                    SkIntToScalar(state.fB), SkPath::kCW_Direction);
              }
              pathA.close();
              pathB.setFillType((SkPath::FillType)f);
              if (b) {
                pathB.addRect(
                    SkIntToScalar(state.fC), SkIntToScalar(state.fC), SkIntToScalar(state.fD) + d,
                    SkIntToScalar(state.fD), SkPath::kCW_Direction);
              }
              pathB.close();
              const char* fillTypeStr[] = {"Winding", "EvenOdd", "InverseWinding",
                                           "InverseEvenOdd"};
              for (int op = 0; op <= kXOR_SkPathOp; ++op) {
                if (state.fReporter->verbose()) {
                  pathStr.printf(
    ...
    for (int b = a + 1; b < 7; b += step) {
      for (int c = 0; c < 6; c += step) {
        for (int d = c + 1; d < 7; d += step) {
          *testRunner.fRunnables.append() =
              new PathOpsThreadedRunnable(&testPathOpsFastMain, a, b, c, d, &testRunner);
        }
      }
    }
  }
