...
static float wangs_formula_conic_reference_impl(
    float precision, const SkPoint P[3], const float w) {
...
DEF_TEST(WangsFormula_worst_case_cubic, r) {
  {
    ...
    REPORTER_ASSERT(
        r, GrWangsFormula::worst_case_cubic(kPrecision, 100, 100) ==
               wangs_formula_cubic_reference_impl(kPrecision, worstP));
    REPORTER_ASSERT(
        r, GrWangsFormula::worst_case_cubic_log2(kPrecision, 100, 100) ==
  ...
  auto check_worst_case_cubic = [&](const SkPoint* pts) {
    ...
    int worst_log2 = GrWangsFormula::worst_case_cubic_log2(kPrecision, bbox.width(), bbox.height());
...
DEF_TEST(WangsFormula_quad_within_tol, r) {
  ...
  for_random_beziers(
      ...
      [&r](const SkPoint pts[]) {
        const int nsegs =
            static_cast<int>(std::ceil(wangs_formula_quadratic_reference_impl(kPrecision, pts)));
...
DEF_TEST(WangsFormula_conic_within_tol, r) {
  ...
  const auto eval_conic = [](const SkPoint pts[3], float w, float t) -> Sk2d {
    const auto eval = [](Sk2d A, Sk2d B, Sk2d C, float t) -> Sk2d { return (A * t + B) * t + C; };
  ...
  const auto dot = [](const Sk2d& a, const Sk2d& b) -> double { return a[0] * b[0] + a[1] * b[1]; };
