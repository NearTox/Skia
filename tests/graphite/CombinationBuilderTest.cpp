...
class CombinationBuilderTestAccess {
 public:
  static int NumCombinations(SkCombinationBuilder* builder) { return builder->numCombinations(); }
#ifdef SK_DEBUG
  static int Epoch(const SkCombinationBuilder& builder) { return builder.epoch(); }
  static int Epoch(const SkCombinationOption& option) { return option.epoch(); }
#endif
};
...
void big_test(Context* context, skiatest::Reporter* reporter) {
  ...
  [[maybe_unused]] auto sweepGrad_0 =
      builder.addOption(SkShaderType::kSweepGradient, kMinNumStops, kMaxNumStops);
  ...
  // first child slot of blendShader_0
  {
    ...
    [[maybe_unused]] auto linearGrad_0 =
        blendShader_0.addChildOption(0, SkShaderType::kLinearGradient, kMinNumStops, kMaxNumStops);

    // second option (solid_0)
    [[maybe_unused]] auto solid_0 = blendShader_0.addChildOption(0, SkShaderType::kSolidColor);
  }
  ...
  // second child slot of blendShader_0
  {
    // first option (linearGrad_1)
    {
      [[maybe_unused]] auto linearGrad_1 = blendShader_0.addChildOption(
          1, SkShaderType::kLinearGradient, kMinNumStops, kMaxNumStops);
    }
    ...
    // second option (blendShader_1)
    {
      ...
      // nested: first child slot of blendShader_1
      {
        ...
        [[maybe_unused]] auto radialGrad_0 = blendShader_1.addChildOption(
            0, SkShaderType::kRadialGradient, kMinNumStops, kMaxNumStops);

        // second option (solid_1)
        [[maybe_unused]] auto solid_1 = blendShader_1.addChildOption(0, SkShaderType::kSolidColor);
      }

      // nested: second child slot of blendShader_1
      {
        SkTileModePair tilingOptions[] = {
            {SkTileMode::kRepeat, SkTileMode::kRepeat}, {SkTileMode::kClamp, SkTileMode::kClamp}};

        // only option (imageShader_0)
        [[maybe_unused]] auto imageShader_0 =
            blendShader_1.addChildOption(1, SkShaderType::kImage, SkMakeSpan(tilingOptions));
      }
    }
  }
