...
DEF_GRAPHITE_TEST_FOR_CONTEXTS(BackendTextureTest, reporter, context) {
  ...
  TextureInfo info = caps->getDefaultSampledTextureInfo(
      ...
      /*levelCount=*/1, Protected::kNo, Renderable::kNo);
...
DEF_GRAPHITE_TEST_FOR_CONTEXTS(SurfaceBackendTextureTest, reporter, context) {
  ...
  sk_sp<SkSurface> surface = MakeGraphiteFromBackendTexture(
      recorder.get(), texture, kRGBA_8888_SkColorType,
      /*colorSpace=*/nullptr,
      /*props=*/nullptr);
  ...
  // color type.
  surface = MakeGraphiteFromBackendTexture(
      recorder.get(), texture, kAlpha_8_SkColorType,
      /*colorSpace=*/nullptr,
      /*props=*/nullptr);
  REPORTER_ASSERT(reporter, !surface);
