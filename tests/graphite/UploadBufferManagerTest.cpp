...
DEF_GRAPHITE_TEST_FOR_CONTEXTS(UploadBufferManagerTest, reporter, context) {
  ...
  REPORTER_ASSERT(
      reporter, memcmp(lgBufferMap, expectedLgBufferMap, sizeof(expectedLgBufferMap)) == 0);
  ...
  REPORTER_ASSERT(
      reporter, memcmp(smBufferMap, expectedSmBufferMap, sizeof(expectedSmBufferMap)) == 0);
  ...
  REPORTER_ASSERT(
      reporter, memcmp(alBufferMap, expectedAlBufferMap0, sizeof(expectedAlBufferMap0)) == 0);

  alBufferMap = SkTAddOffset<const void>(alBufferMap, 8);
  REPORTER_ASSERT(
      reporter, memcmp(alBufferMap, expectedAlBufferMap1, sizeof(expectedAlBufferMap1)) == 0);
}
