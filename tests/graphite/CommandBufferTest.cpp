...
class UniformRectDraw final : public RenderStep {
 ...
 private:
  UniformRectDraw()
      : RenderStep(
            "UniformRectDraw", "test-only", Flags::kPerformsShading,
            /*uniforms=*/{{"scale", SkSLType::kFloat2}, {"translate", SkSLType::kFloat2}},
            PrimitiveType::kTriangleStrip, {{}, {}, 0, true, CompareOp::kAlways, false, false},
            /*vertexAttrs=*/{},
            /*instanceAttrs=*/{}) {}
};
...
class TriangleRectDraw final : public RenderStep {
 ...
 private:
  TriangleRectDraw()
      : RenderStep(
            "TriangleRectDraw", "test-only", Flags::kPerformsShading,
            /*uniforms=*/{{"scale", SkSLType::kFloat2}, {"translate", SkSLType::kFloat2}},
            PrimitiveType::kTriangles, kTestDepthStencilSettings,
            /*vertexAttrs=*/{{"position", VertexAttribType::kFloat2, SkSLType::kFloat2}},
            /*instanceAttrs=*/{}) {}
};
...
class InstanceRectDraw final : public RenderStep {
 ...
 private:
  InstanceRectDraw()
      : RenderStep(
            "InstanceRectDraw", "test-only", Flags::kPerformsShading,
            /*uniforms=*/{}, PrimitiveType::kTriangles, kTestDepthStencilSettings,
            /*vertexAttrs=*/{},
            /*instanceAttrs=*/
...
DEF_GRAPHITE_TEST_FOR_CONTEXTS(CommandBufferTest, reporter, context) {
  ...
  SkUniquePaintParamsID uniqueID;
  {
    ...
    uniqueID = CreateKey(keyContext, &builder, SkShaderType::kSolidColor, SkBlendMode::kSrc);
  }
  ...
  TextureInfo depthStencilInfo = gpu->caps()->getDefaultDepthStencilTextureInfo(
      DepthStencilFlags::kDepthStencil, 1, Protected::kNo);
  ...
  commandBuffer->beginRenderPass(
      renderPassDesc, target->refTexture(), nullptr, depthStencilTexture);
  ...
  auto draw = [&](const RenderStep* step, std::vector<RectAndColor> draws) {
    ...
    drawWriter.newPipelineState(
        step->primitiveType(), step->vertexStride(), step->instanceStride());
    ...
    for (auto d : draws) {
      ...
      if (gatherer.hasUniforms()) {
        ...
        commandBuffer->bindUniformBuffer(
            UniformSlot::kRenderStep, sk_ref_sp(bindInfo.fBuffer), bindInfo.fOffset);
      }
      gatherer.reset();
      ...
      commandBuffer->bindUniformBuffer(
          UniformSlot::kPaint, sk_ref_sp(bindInfo.fBuffer), bindInfo.fOffset);
  ...
  draw(
      TriangleRectDraw::Singleton(),
      {{fullRect.makeInset(kTextureWidth / 4.f, kTextureHeight / 4.f), SkColors::kMagenta}});

  // Draw green and cyan rects using instance buffer
  draw(
      InstanceRectDraw::Singleton(),
      {{{kTextureWidth / 3.f, kTextureHeight / 3.f, kTextureWidth / 2.f, kTextureHeight / 2.f},
        SkColors::kGreen},
       {{kTextureWidth / 2.f, kTextureHeight / 2.f, 5.f * kTextureWidth / 8.f,
         5.f * kTextureHeight / 8.f},
        SkColors::kCyan}});
