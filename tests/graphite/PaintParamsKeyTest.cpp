...
sk_sp<SkImage> make_image(Recorder* recorder) {
  SkImageInfo info =
      SkImageInfo::Make(32, 32, SkColorType::kRGBA_8888_SkColorType, kPremul_SkAlphaType);
...
std::tuple<SkPaint, int> create_paint(
    Recorder* recorder, SkShaderType shaderType, SkTileMode tm, SkBlendMode bm) {
  ...
  int numTextures = 0;
  switch (shaderType) {
    case SkShaderType::kSolidColor: break;
    ...
    case SkShaderType::kSweepGradient:
      s = SkGradientShader::MakeSweep(0, 0, colors, offsets, 2, tm, 0, 359, 0, nullptr);
      break;
    case SkShaderType::kConicalGradient:
      s = SkGradientShader::MakeTwoPointConical(
          {100, 100}, 100, {-100, -100}, 100, colors, offsets, 2, tm);
      break;
    ...
    case SkShaderType::kImage: {
      ++numTextures;
      s = SkImageShader::Make(make_image(recorder), tm, tm, SkSamplingOptions(), nullptr);
      break;
    }
...
DEF_GRAPHITE_TEST_FOR_CONTEXTS(PaintParamsKeyTest, reporter, context) {
  ...
  for (auto s :
       {SkShaderType::kSolidColor, SkShaderType::kLinearGradient, SkShaderType::kRadialGradient,
        SkShaderType::kSweepGradient, SkShaderType::kConicalGradient, SkShaderType::kLocalMatrix,
        SkShaderType::kImage, SkShaderType::kBlendShader}) {
    for (auto tm :
      ...
      for (auto bm : {SkBlendMode::kSrc, SkBlendMode::kSrcOver}) {
        ...
        auto [uniqueID1, uIndex, tIndex] =
            ExtractPaintData(recorder.get(), &gatherer, &builder, {}, PaintParams(p));
