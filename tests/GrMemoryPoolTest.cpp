...
class A {
 public:
  A() {}
  virtual void setValues(int v) { fChar = static_cast<char>(v & 0xFF); }
  virtual bool checkValues(int v) { return fChar == static_cast<char>(v & 0xFF); }
  virtual ~A() {}
  ...
  static void ResetAllocator() { gPool.reset(nullptr); }
...
class AutoPoolReleaser {
 public:
  ...
  void add(void* ptr) { fAllocated.push_back(ptr); }
