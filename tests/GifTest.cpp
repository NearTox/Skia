...
static unsigned char gGIFData[] = {
    0x47, 0x49, 0x46, 0x38, 0x37, 0x61, 0x03, 0x00, 0x03, 0x00, 0xe3, 0x08, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0x00, 0x00, 0xff, 0x00, 0xff, 0x80, 0x80, 0x80,
    0x00, 0xff, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0x2c, 0x00, 0x00, 0x00, 0x00, 0x03, 0x00, 0x03, 0x00,
    0x00, 0x04, 0x07, 0x50, 0x1c, 0x43, 0x40, 0x41, 0x23, 0x44, 0x00, 0x3b};
...
static unsigned char gInterlacedGIF[] = {
    0x47, 0x49, 0x46, 0x38, 0x37, 0x61, 0x09, 0x00, 0x09, 0x00, 0xe3, 0x08, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0xff, 0xff, 0x00, 0x00, 0xff, 0x00, 0xff, 0x80, 0x80, 0x80, 0x00, 0xff,
    0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0x2c, 0x00, 0x00, 0x00, 0x00, 0x09, 0x00, 0x09, 0x00, 0x40, 0x04, 0x1b, 0x50, 0x1c,
    0x23, 0xe9, 0x44, 0x23, 0x60, 0x9d, 0x09, 0x28, 0x1e, 0xf8, 0x6d, 0x64, 0x56, 0x9d, 0x53,
    0xa8, 0x7e, 0xa8, 0x65, 0x94, 0x5c, 0xb0, 0x8a, 0x45, 0x04, 0x00, 0x3b};
...
static void test_gif_data_dims(
    skiatest::Reporter* r, void* data, size_t size, int width, int height) {
  SkBitmap bm;
  bool imageDecodeSuccess = decode_memory(data, size, &bm);
  REPORTER_ASSERT(r, imageDecodeSuccess);
...
DEF_TEST(Gif, reporter) {
  ...
  test_interlaced_gif_data(reporter, static_cast<void*>(gInterlacedGIF), sizeof(gInterlacedGIF));
  ...
  // Likewise, incremental decoding should succeed here.
  {
    ...
    if (codec) {
      ...
      REPORTER_ASSERT(
          reporter,
          SkCodec::kSuccess == codec->startIncrementalDecode(info, bm.getPixels(), bm.rowBytes()));
  ...
  test_interlaced_gif_data(
      reporter, static_cast<void*>(gInterlacedGIF),
...
DEF_TEST(Codec_GifInterlacedTruncated, r) {
  ...
  if ((sizeof(gInterlacedGIF) != 102) || (gInterlacedGIF[72] != 0x1b) ||
      (gInterlacedGIF[100] != 0x00) || (gInterlacedGIF[101] != 0x3b)) {
...
DEF_TEST(Gif_Sampled, r) {
  ...
  const SkCodec::Result result =
      codec->getAndroidPixels(codec->getInfo(), bm.getPixels(), bm.rowBytes(), &options);
...
DEF_TEST(Codec_gif_out_of_palette, r) {
  ...
  REPORTER_ASSERT(
      r, result == SkCodec::kSuccess, "Failed to decode %s with error %s", path,
  ...
  for (auto& pixel : pixels) {
    ...
    REPORTER_ASSERT(
        r, actual == pixel.expected, "pixel (%i,%i) mismatch! expected: %x actual: %x", pixel.x,
        pixel.y, pixel.expected, actual);
  }
}
  ...
  for (bool use565 : {false, true}) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      if (use565 && i > 0) {
        ...
        REPORTER_ASSERT(r, result != SkCodec::kSuccess, "Unexpected success to decode frame %i", i);
        continue;
      }
#endif
