...
static SkBitmap read_back(
    GrDirectContext* dContext, skgpu::v1::SurfaceDrawContext* sdc, int width, int height) {
...
static void run_test(GrDirectContext* dContext, skiatest::Reporter* reporter) {
  SkPath invPath = make_path(
      SkRect::MakeXYWH(0, 0, kBigSize, kBigSize), kBigSize / 2 - 1,
      SkPathFillType::kInverseWinding);
  SkPath path =
      make_path(SkRect::MakeXYWH(0, 0, kBigSize, kBigSize), kPad, SkPathFillType::kWinding);
...

  {
    ...
    sdc->drawPath(nullptr, std::move(paint), GrAA::kNo, SkMatrix::I(), path, style);
...
DEF_GPUTEST_FOR_CONTEXTS(
    DefaultPathRendererTest, sk_gpu_test::GrContextFactory::IsRenderingContext, reporter, ctxInfo,
    only_allow_default) {
