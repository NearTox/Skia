...
static void assert_eq(skiatest::Reporter* reporter, const SkString& skString, const char* str) {
  assert_eql(reporter, skString, str, strlen(str));
}

template <typename T>
static void assert_emit_eq(skiatest::Reporter* reporter, T& object, const char* string) {
...
static void TestPDFArray(skiatest::Reporter* reporter) {
  ...
  assert_emit_eq(reporter, *array, "[42 .5 0 true /ThisName /AnotherName (This String)]");
...
static void check_pdf_scalar_serialization(skiatest::Reporter* reporter, float inputFloat) {
...
static SkGlyphRun make_run(
    size_t len, const SkGlyphID* glyphs, SkPoint* pos, const SkFont& font, const uint32_t* clusters,
    size_t utf8TextByteLength, const char* utf8Text) {
  return SkGlyphRun(
      font, SkSpan<const SkPoint>{pos, len}, SkSpan<const SkGlyphID>{glyphs, len},
      SkSpan<const char>{utf8Text, utf8TextByteLength}, SkSpan<const uint32_t>{clusters, len});
}
...
DEF_TEST(SkPDF_Clusterator, reporter) {
  SkFont font;
  {
    ...
    SkClusterator::Cluster expectations[] = {
        {&text[3], 1, 0, 1}, {&text[2], 1, 1, 2},  {&text[1], 1, 3, 1},
        {&text[0], 1, 4, 1}, {&text[4], 1, 5, 2},  {&text[7], 1, 7, 1},
        {&text[6], 1, 8, 2}, {&text[5], 1, 10, 1}, {nullptr, 0, 0, 0},
    };
  ...
  }
  {
    ...
    SkClusterator clusterator(run);
    SkClusterator::Cluster expectations[] = {
        {&text[0], 1, 0, 1}, {&text[1], 3, 1, 1}, {&text[4], 1, 2, 1},
        {&text[5], 1, 3, 1}, {&text[6], 1, 4, 1}, {nullptr, 0, 0, 0},
    };
    for (const auto& expectation : expectations) {
      REPORTER_ASSERT(reporter, clusterator.next() == expectation);
    }
