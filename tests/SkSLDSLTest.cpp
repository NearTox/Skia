...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLFlags, r, ctxInfo) {
...

  {
    ...
    EXPECT_EQUAL(
        All(GreaterThan(Float4(1), Float4(0))), "all(greaterThan(float4(1.0), float4(0.0)))");
  }
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLUInt, r, ctxInfo) {
  ...
  EXPECT_EQUAL(UInt3(UInt2(0, 1), -2), "uint3(0, 1, -2)");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLUShort, r, ctxInfo) {
  ...
  EXPECT_EQUAL(UShort3(UShort2(0, 1), -2), "ushort3(0, 1, -2)");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLFunction, r, ctxInfo) {
  ...
  DSLFunction(kVoid_Type, "main", coords).define(sk_FragColor() = Half4(coords, 0, 1));
  REPORTER_ASSERT(r, DSLWriter::ProgramElements().size() == 1);
  EXPECT_EQUAL(
      *DSLWriter::ProgramElements()[0],
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLInterfaceBlock, r, ctxInfo) {
  ...
  REPORTER_ASSERT(r, DSLWriter::ProgramElements().size() == 1);
  EXPECT_EQUAL(
      *DSLWriter::ProgramElements().back(), "uniform InterfaceBlock1 { float a; int b; };");
  ...
  REPORTER_ASSERT(r, DSLWriter::ProgramElements().size() == 2);
  EXPECT_EQUAL(
      *DSLWriter::ProgramElements().back(),
  ...
  EXPECT_EQUAL(
      *DSLWriter::ProgramElements().back(), "uniform InterfaceBlock3 { float z; } arrayVar[4];");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLSwitch, r, ctxInfo) {
  ...
  Statement x = Switch(
      b, Case(0, a = 0, Break()), Case(1, std::move(caseStatements)),
      Case(2, a = 2 /*Fallthrough*/), Default(Discard()));
  EXPECT_EQUAL(x, R"(
        switch (b) {
            case 0: (a = 0.0); break;
  ...
  Statement y = StaticSwitch(
      b, Case(0, a = 0, Break()), Case(1, a = 1, Continue()), Case(2, a = 2 /*Fallthrough*/),
      Default(Discard()));
  EXPECT_EQUAL(y, R"(
        @switch (b) {
            case 0: (a = 0.0); break;
  ...
  EXPECT_EQUAL(Switch(b, Default(), Case(0), Case(1)), "switch (b) { default: case 0: case 1: }");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLVarSwap, r, ctxInfo) {
  ...
  EXPECT_EQUAL(Statement(Block(Declare(a), a = 123)), "{ int a; (a = 123); }");
}
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLLayout, r, ctxInfo) {
  ...
  EXPECT_EQUAL(
      Declare(v1),
      "layout (location = 1, offset = 4, binding = 3, index = 5, set = 2, "
...

  {
    ...
    DSLGlobalVar v(DSLModifiers(DSLLayout().srgbUnpremul(), kUniform_Modifier), kHalf4_Type, "v");
    Declare(v);
  }
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLStruct, r, ctxInfo) {
  ...
  DSLFunction(simpleStruct, "returnStruct")
      .define(
          Declare(result), result.field("x") = 123, result.field("b") = result.field("x") > 0,
          result.field("a")[0] = result.field("x"), Return(result));
  REPORTER_ASSERT(r, DSLWriter::ProgramElements().size() == 2);
  EXPECT_EQUAL(
      *DSLWriter::ProgramElements()[0], "struct SimpleStruct { float x; bool b; float[3] a; };");
  EXPECT_EQUAL(
      *DSLWriter::ProgramElements()[1],
  ...
  REPORTER_ASSERT(r, DSLWriter::ProgramElements().size() == 3);
  EXPECT_EQUAL(
      *DSLWriter::ProgramElements()[2], "struct NestedStruct { int x; SimpleStruct simple; };");
}
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLRTAdjust, r, ctxInfo) {
  {
    AutoDSLContext context(
        ctxInfo.directContext()->priv().getGpu(), no_mark_vars_declared(),
        SkSL::ProgramKind::kVertex);
    DSLGlobalVar rtAdjust(kUniform_Modifier, kFloat4_Type, "sk_RTAdjust");
    Declare(rtAdjust);
    DSLFunction(kVoid_Type, "main").define(sk_Position() = Half4(0));
    REPORTER_ASSERT(r, DSLWriter::ProgramElements().size() == 2);
    EXPECT_EQUAL(
        *DSLWriter::ProgramElements()[1],
        "void main() {"
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLInlining, r, ctxInfo) {
  ...
  DSLFunction sqr(kFloat_Type, "sqr", x);
  sqr.define(Return(x * x));
  DSLFunction(kVoid_Type, "main").define(sk_FragColor() = (sqr(2), Half4(sqr(3))));
