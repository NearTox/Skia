...
SkSL::ProgramSettings default_settings() { return SkSL::ProgramSettings{}; }
...
class AutoDSLContext {
 public:
  AutoDSLContext(
      GrGpu* gpu, SkSL::ProgramSettings settings = default_settings(),
...
class ExpectError : public SkSL::ErrorReporter {
 public:
  ExpectError(skiatest::Reporter* reporter, const char* msg)
      : fMsg(msg), fReporter(reporter), fOldReporter(&GetErrorReporter()) {
  ...
  void handleError(std::string_view msg, SkSL::Position pos) override {
    ...
    REPORTER_ASSERT(
        fReporter, !fMsg || msg == fMsg, "Error mismatch: expected:\n%s\nbut received:\n%.*s", fMsg,
        (int)msg.length(), msg.data());
    fMsg = nullptr;
  }
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLFlags, r, ctxInfo) {
...

  {
    ...
    AutoDSLContext context(
        ctxInfo.directContext()->priv().getGpu(), settings, SkSL::ProgramKind::kFragment);
    Var x(kHalf_Type, "x");
    Var y(kFloat_Type, "y");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLFloat, r, ctxInfo) {
  ...
  REPORTER_ASSERT(
      r, atof(e1.release()->description().c_str()) == std::numeric_limits<float>::max());

  Expression e2 = Float(std::numeric_limits<float>::min());
  REPORTER_ASSERT(
      r, atof(e2.release()->description().c_str()) == std::numeric_limits<float>::min());

  EXPECT_EQUAL(Float2(0), "float2(0.0)");
  EXPECT_EQUAL(Float2(-0.5, 1), "float2(-0.5, 1.0)");
  EXPECT_EQUAL(Float3(0.75), "float3(0.75)");
  EXPECT_EQUAL(Float3(Float2(0, 1), -2), "float3(0.0, 1.0, -2.0)");
  EXPECT_EQUAL(Float3(0, 1, 2), "float3(0.0, 1.0, 2.0)");
  EXPECT_EQUAL(Float4(0), "float4(0.0)");
  EXPECT_EQUAL(Float4(Float2(0, 1), Float2(2, 3)), "float4(0.0, 1.0, 2.0, 3.0)");
  EXPECT_EQUAL(Float4(0, 1, Float2(2, 3)), "float4(0.0, 1.0, 2.0, 3.0)");
  EXPECT_EQUAL(Float4(0, 1, 2, 3), "float4(0.0, 1.0, 2.0, 3.0)");
...

  {
    ExpectError error(
        r,
        "'float4' is not a valid parameter to 'float2' constructor; use '.xy' "
        "instead");
...

  {
    ExpectError error(
        r,
        "invalid arguments to 'float4' constructor (expected 4 scalars, but "
        "found 3)");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLHalf, r, ctxInfo) {
  ...
  EXPECT_EQUAL(Half2(0), "half2(0.0)");
  EXPECT_EQUAL(Half2(-0.5, 1), "half2(-0.5, 1.0)");
  EXPECT_EQUAL(Half3(0.75), "half3(0.75)");
  EXPECT_EQUAL(Half3(Half2(0, 1), -2), "half3(0.0, 1.0, -2.0)");
  EXPECT_EQUAL(Half3(0, 1, 2), "half3(0.0, 1.0, 2.0)");
  EXPECT_EQUAL(Half4(0), "half4(0.0)");
  EXPECT_EQUAL(Half4(Half2(0, 1), Half2(2, 3)), "half4(0.0, 1.0, 2.0, 3.0)");
  EXPECT_EQUAL(Half4(0, 1, Half2(2, 3)), "half4(0.0, 1.0, 2.0, 3.0)");
  EXPECT_EQUAL(Half4(0, 1, 2, 3), "half4(0.0, 1.0, 2.0, 3.0)");
...

  {
    ExpectError error(
        r,
        "'half4' is not a valid parameter to 'half2' constructor; use '.xy' "
        "instead");
...

  {
    ExpectError error(
        r,
        "invalid arguments to 'half4' constructor (expected 4 scalars, but "
        "found 3)");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLInt, r, ctxInfo) {
  ...
  EXPECT_EQUAL(Int(std::numeric_limits<int32_t>::max()), "2147483647");
  EXPECT_EQUAL(Int2(std::numeric_limits<int32_t>::min()), "int2(-2147483648)");
  EXPECT_EQUAL(Int2(0, 1), "int2(0, 1)");
  EXPECT_EQUAL(Int3(0), "int3(0)");
  EXPECT_EQUAL(Int3(Int2(0, 1), -2), "int3(0, 1, -2)");
  EXPECT_EQUAL(Int3(0, 1, 2), "int3(0, 1, 2)");
  EXPECT_EQUAL(Int4(0), "int4(0)");
  EXPECT_EQUAL(Int4(Int2(0, 1), Int2(2, 3)), "int4(0, 1, 2, 3)");
  EXPECT_EQUAL(Int4(0, 1, Int2(2, 3)), "int4(0, 1, 2, 3)");
  EXPECT_EQUAL(Int4(0, 1, 2, 3), "int4(0, 1, 2, 3)");

  {
    ExpectError error(
        r,
        "'int4' is not a valid parameter to 'int2' constructor; use '.xy' "
        "instead");
    Int2(Int4(1)).release();
  }

  {
    ExpectError error(
        r,
        "invalid arguments to 'int4' constructor (expected 4 scalars, but "
        "found 3)");
    Int4(Int3(1)).release();
  }
}
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLUInt, r, ctxInfo) {
  ...
  EXPECT_EQUAL(UInt(std::numeric_limits<uint32_t>::max()), "4294967295");
  EXPECT_EQUAL(UInt2(std::numeric_limits<uint32_t>::min()), "uint2(0)");
  EXPECT_EQUAL(UInt2(0, 1), "uint2(0, 1)");
  EXPECT_EQUAL(UInt3(0), "uint3(0)");
  EXPECT_EQUAL(UInt3(0, 1, 2), "uint3(0, 1, 2)");
  EXPECT_EQUAL(UInt4(0), "uint4(0)");
  EXPECT_EQUAL(UInt4(UInt2(0, 1), UInt2(2, 3)), "uint4(0, 1, 2, 3)");
  EXPECT_EQUAL(UInt4(0, 1, UInt2(2, 3)), "uint4(0, 1, 2, 3)");
  EXPECT_EQUAL(UInt4(0, 1, 2, 3), "uint4(0, 1, 2, 3)");
...

  {
    ExpectError error(
        r,
        "'uint4' is not a valid parameter to 'uint2' constructor; use '.xy' "
        "instead");
...

  {
    ExpectError error(
        r,
        "invalid arguments to 'uint4' constructor (expected 4 scalars, but "
        "found 3)");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLShort, r, ctxInfo) {
  ...
  EXPECT_EQUAL(Short(std::numeric_limits<int16_t>::max()), "32767");
  EXPECT_EQUAL(Short2(std::numeric_limits<int16_t>::min()), "short2(-32768)");
  EXPECT_EQUAL(Short2(0, 1), "short2(0, 1)");
  EXPECT_EQUAL(Short3(0), "short3(0)");
  EXPECT_EQUAL(Short3(Short2(0, 1), -2), "short3(0, 1, -2)");
  EXPECT_EQUAL(Short3(0, 1, 2), "short3(0, 1, 2)");
  EXPECT_EQUAL(Short4(0), "short4(0)");
  EXPECT_EQUAL(Short4(Short2(0, 1), Short2(2, 3)), "short4(0, 1, 2, 3)");
  EXPECT_EQUAL(Short4(0, 1, Short2(2, 3)), "short4(0, 1, 2, 3)");
  EXPECT_EQUAL(Short4(0, 1, 2, 3), "short4(0, 1, 2, 3)");

  {
    ExpectError error(
        r,
        "'short4' is not a valid parameter to 'short2' constructor; use '.xy' "
        "instead");
...

  {
    ExpectError error(
        r,
        "invalid arguments to 'short4' constructor (expected 4 scalars, but "
        "found 3)");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLUShort, r, ctxInfo) {
  ...
  EXPECT_EQUAL(UShort(std::numeric_limits<uint16_t>::max()), "65535");
  EXPECT_EQUAL(UShort2(std::numeric_limits<uint16_t>::min()), "ushort2(0)");
  EXPECT_EQUAL(UShort2(0, 1), "ushort2(0, 1)");
  EXPECT_EQUAL(UShort3(0), "ushort3(0)");
  EXPECT_EQUAL(UShort3(0, 1, 2), "ushort3(0, 1, 2)");
  EXPECT_EQUAL(UShort4(0), "ushort4(0)");
  EXPECT_EQUAL(UShort4(UShort2(0, 1), UShort2(2, 3)), "ushort4(0, 1, 2, 3)");
  EXPECT_EQUAL(UShort4(0, 1, UShort2(2, 3)), "ushort4(0, 1, 2, 3)");
  EXPECT_EQUAL(UShort4(0, 1, 2, 3), "ushort4(0, 1, 2, 3)");
...

  {
    ExpectError error(
        r,
        "'ushort4' is not a valid parameter to 'ushort2' constructor; use "
        "'.xy' instead");
...

  {
    ExpectError error(
        r,
        "invalid arguments to 'ushort4' constructor (expected 4 scalars, but "
        "found 3)");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLBool, r, ctxInfo) {
  ...
  EXPECT_EQUAL(Bool2(false), "bool2(false)");
  EXPECT_EQUAL(Bool2(false, true), "bool2(false, true)");
  EXPECT_EQUAL(Bool3(false), "bool3(false)");
  EXPECT_EQUAL(Bool3(Bool2(false, true), false), "bool3(false, true, false)");
  EXPECT_EQUAL(Bool3(false, true, false), "bool3(false, true, false)");
  EXPECT_EQUAL(Bool4(false), "bool4(false)");
  EXPECT_EQUAL(Bool4(Bool2(false, true), Bool2(false, true)), "bool4(false, true, false, true)");
  EXPECT_EQUAL(Bool4(false, true, Bool2(false, true)), "bool4(false, true, false, true)");
  EXPECT_EQUAL(Bool4(false, true, false, true), "bool4(false, true, false, true)");

  {
    ExpectError error(
        r,
        "'bool4' is not a valid parameter to 'bool2' constructor; use '.xy' "
        "instead");
...

  {
    ExpectError error(
        r,
        "invalid arguments to 'bool4' constructor (expected 4 scalars, but "
        "found 3)");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLMatrices, r, ctxInfo) {
...

  {
    ExpectError error(
        r,
        "invalid arguments to 'float3x3' constructor (expected 9 scalars, but "
        "found 2)");
...

  {
    ExpectError error(
        r,
        "invalid arguments to 'half2x2' constructor (expected 4 scalars, but "
        "found 5)");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLPlus, r, ctxInfo) {
  ...
  EXPECT_EQUAL(a + 1, "(a + 1.0)");
  EXPECT_EQUAL(0.5 + a + -99, "((0.5 + a) + -99.0)");
  EXPECT_EQUAL(a += b + 1, "(a += (b + 1.0))");
  EXPECT_EQUAL(+a, "a");
  EXPECT_EQUAL(+(a + b), "(a + b)");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLMinus, r, ctxInfo) {
  ...
  EXPECT_EQUAL(2 - a - b, "((2 - a) - b)");
  EXPECT_EQUAL(a -= b + 1, "(a -= (b + 1))");
  EXPECT_EQUAL(-a, "-a");
  EXPECT_EQUAL(-(a - b), "-(a - b)");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLCall, r, ctxInfo) {
  AutoDSLContext context(ctxInfo.directContext()->priv().getGpu());
  {
    DSLExpression sqrt(SkSL::ThreadContext::Compiler().convertIdentifier(SkSL::Position(), "sqrt"));
  ...
  {
    DSLExpression pow(SkSL::ThreadContext::Compiler().convertIdentifier(SkSL::Position(), "pow"));
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLContinue, r, ctxInfo) {
  ...
  Var i(kInt_Type, "i", 0);
  DSLFunction(kVoid_Type, "success")
      .define(For(Declare(i), i < 10, ++i, Block(If(i < 5, Continue()))));
  REPORTER_ASSERT(r, SkSL::ThreadContext::ProgramElements().size() == 1);
  EXPECT_EQUAL(
      *SkSL::ThreadContext::ProgramElements()[0],
...

  {
    ...
    DSLFunction(kVoid_Type, "fail").define(Continue());
  }
}
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLFor, r, ctxInfo) {
  AutoDSLContext context(ctxInfo.directContext()->priv().getGpu());
  EXPECT_EQUAL(For(Statement(), Expression(), Expression(), Block()), "for (;;) {}");

  Var i(kInt_Type, "i", 0);
  EXPECT_EQUAL(For(Declare(i), i < 10, ++i, i += 5), "for (int i = 0; (i < 10); ++i) (i += 5);");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLFunction, r, ctxInfo) {
  ...
  DSLFunction(kVoid_Type, "main", coords).define(sk_FragColor().assign(Half4(coords, 0, 1)));
  REPORTER_ASSERT(r, SkSL::ThreadContext::ProgramElements().size() == 1);
  EXPECT_EQUAL(
      *SkSL::ThreadContext::ProgramElements()[0],
...

  {
    ...
    sqr.define(Return(x * x));
    EXPECT_EQUAL(sqr(sk_FragCoord().x()), "sqr(sk_FragCoord.x)");
    REPORTER_ASSERT(r, SkSL::ThreadContext::ProgramElements().size() == 1);
    EXPECT_EQUAL(
        *SkSL::ThreadContext::ProgramElements()[0], "float sqr(float x) { return (x * x); }");
  }

  {
    ...
    dot.define(Return(x * x + y * y));
    EXPECT_EQUAL(
        dot(Float2(1.0f, 2.0f), Float2(3.0f, 4.0f)), "dot(float2(1.0, 2.0), float2(3.0, 4.0))");
    REPORTER_ASSERT(r, SkSL::ThreadContext::ProgramElements().size() == 1);
    EXPECT_EQUAL(
        *SkSL::ThreadContext::ProgramElements()[0],
...

  {
    ...
    DSLWriter::Reset();
    DSLFunction(kFloat_Type, "broken").define(Return(true));
  }

  {
    ExpectError error(r, "expected function to return 'float'");
    DSLWriter::Reset();
    DSLFunction(kFloat_Type, "broken").define(Return());
  }

  {
    ...
    DSLFunction(kFloat_Type, "broken").define(Declare(x), If(x == 1, Return(x)));
  }

  {
    ExpectError error(r, "may not return a value from a void function");
    DSLWriter::Reset();
    DSLFunction(kVoid_Type, "broken").define(Return(0));
  }
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLInterfaceBlock, r, ctxInfo) {
  AutoDSLContext context(ctxInfo.directContext()->priv().getGpu());
  DSLGlobalVar intf = InterfaceBlock(
      kUniform_Modifier, "InterfaceBlock1", {Field(kFloat_Type, "a"), Field(kInt_Type, "b")});
  REPORTER_ASSERT(r, SkSL::ThreadContext::ProgramElements().size() == 1);
  EXPECT_EQUAL(
      *SkSL::ThreadContext::ProgramElements().back(),
      "uniform InterfaceBlock1 { float a; int b; };");
  EXPECT_EQUAL(intf.field("a"), "InterfaceBlock1.a");

  DSLGlobalVar intf2 = InterfaceBlock(
      kUniform_Modifier, "InterfaceBlock2", {Field(kFloat2_Type, "x"), Field(kHalf2x2_Type, "y")},
      "blockVar");
  REPORTER_ASSERT(r, SkSL::ThreadContext::ProgramElements().size() == 2);
  EXPECT_EQUAL(
      *SkSL::ThreadContext::ProgramElements().back(),
  ...
  DSLGlobalVar intf3 = InterfaceBlock(
      kUniform_Modifier, "InterfaceBlock3", {Field(kFloat_Type, "z")}, "arrayVar", 4);
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLSwitch, r, ctxInfo) {
  ...
  Statement x = Switch(
      b, Case(0, a.assign(0), Break()), Case(1, std::move(caseStatements)),
      Case(2, a.assign(2) /*Fallthrough*/), Default(Discard()));
  EXPECT_EQUAL(x, R"(
        switch (b) {
            case 0: (a = 0.0); break;
  ...
  Statement y = StaticSwitch(
      b, Case(0, a.assign(0), Break()), Case(1, a.assign(1), Continue()),
      Case(2, a.assign(2) /*Fallthrough*/), Default(Discard()));
  EXPECT_EQUAL(y, R"(
        @switch (b) {
            case 0: (a = 0.0); break;
  ...
  EXPECT_EQUAL(Switch(b), "switch (b) {}");

  EXPECT_EQUAL(
      Switch(b, Default(), Case(0), Case(1)), "switch (b) { default: ; case 0: ; case 1: ; }");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLSwizzle, r, ctxInfo) {
  ...
  EXPECT_EQUAL(a.x(), "a.x");
  EXPECT_EQUAL(a.y(), "a.y");
  EXPECT_EQUAL(a.z(), "a.z");
  EXPECT_EQUAL(a.w(), "a.w");
  EXPECT_EQUAL(a.r(), "a.x");
  EXPECT_EQUAL(a.g(), "a.y");
  EXPECT_EQUAL(a.b(), "a.z");
  EXPECT_EQUAL(a.a(), "a.w");
  EXPECT_EQUAL(Swizzle(a, R), "a.x");
  EXPECT_EQUAL(Swizzle(a, ZERO, G), "float2(0.0, a.y)");
  EXPECT_EQUAL(Swizzle(a, B, G, G), "a.zyy");
  EXPECT_EQUAL(Swizzle(a, R, G, B, ONE), "float4(a.xyz, 1.0)");
  EXPECT_EQUAL(Swizzle(a, B, G, R, ONE).r(), "a.z");
}
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLVarSwap, r, ctxInfo) {
  ...
  EXPECT_EQUAL(Statement(Block(Declare(a), a.assign(123))), "{ int a; (a = 123); }");
}
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLModifiers, r, ctxInfo) {
  ...
  REPORTER_ASSERT(
      r, v6.modifiers().flags() == (SkSL::Modifiers::kIn_Flag | SkSL::Modifiers::kOut_Flag));

  Var v7(kInOut_Modifier, kInt_Type, "v7");
  REPORTER_ASSERT(
      r, v7.modifiers().flags() == (SkSL::Modifiers::kIn_Flag | SkSL::Modifiers::kOut_Flag));
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLLayout, r, ctxInfo) {
  ...
  Var v1(
      DSLModifiers(
          DSLLayout().location(1).offset(4).index(5).builtin(6).inputAttachmentIndex(7),
          kConst_Modifier),
      kInt_Type, "v1", 0);
  EXPECT_EQUAL(
      Declare(v1),
      "layout (location = 1, offset = 4, index = 5, "
...

  {
    ExpectError error(
        r,
        "layout qualifier 'blend_support_all_equations' appears more than "
        "once");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLSampleShader, r, ctxInfo) {
  AutoDSLContext context(
      ctxInfo.directContext()->priv().getGpu(), default_settings(),
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLStruct, r, ctxInfo) {
  ...
  DSLType simpleStruct = Struct(
      "SimpleStruct", Field(kFloat_Type, "x"), Field(kBool_Type, "b"),
      Field(Array(kFloat_Type, 3), "a"));
  DSLVar result(simpleStruct, "result");
  DSLFunction(simpleStruct, "returnStruct")
      .define(
          Declare(result), result.field("x").assign(123),
          result.field("b").assign(result.field("x") > 0),
          result.field("a")[0].assign(result.field("x")), Return(result));
  REPORTER_ASSERT(r, SkSL::ThreadContext::ProgramElements().size() == 2);
  EXPECT_EQUAL(
      *SkSL::ThreadContext::ProgramElements()[0],
      "struct SimpleStruct { float x; bool b; float[3] a; };");
  EXPECT_EQUAL(
      *SkSL::ThreadContext::ProgramElements()[1],
      "SimpleStruct returnStruct() { SimpleStruct result; (result.x = 123.0);"
      "(result.b = (result.x > 0.0)); (result.a[0] = result.x); return result; }");

  Struct("NestedStruct", Field(kInt_Type, "x"), Field(simpleStruct, "simple"));
  REPORTER_ASSERT(r, SkSL::ThreadContext::ProgramElements().size() == 3);
  EXPECT_EQUAL(
      *SkSL::ThreadContext::ProgramElements()[2],
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLRTAdjust, r, ctxInfo) {
  {
    AutoDSLContext context(
        ctxInfo.directContext()->priv().getGpu(), default_settings(), SkSL::ProgramKind::kVertex);
    DSLGlobalVar rtAdjust(kUniform_Modifier, kFloat4_Type, "sk_RTAdjust");
    Declare(rtAdjust);
    DSLFunction(kVoid_Type, "main").define(sk_Position().assign(Half4(0)));
    REPORTER_ASSERT(r, SkSL::ThreadContext::ProgramElements().size() == 2);
    EXPECT_EQUAL(
        *SkSL::ThreadContext::ProgramElements()[1],
        "void main() {"
...

  {
    ...
    DSLGlobalVar intf = InterfaceBlock(
        kUniform_Modifier, "uniforms",
        {Field(kInt_Type, "unused"), Field(kFloat4_Type, "sk_RTAdjust")});
    REPORTER_ASSERT(r, SkSL::ThreadContext::RTAdjustState().fInterfaceBlock);
...

  {
    ...
    InterfaceBlock(
        kUniform_Modifier, "uniforms1",
        {Field(kInt_Type, "unused1"), Field(kFloat4_Type, "sk_RTAdjust")});
    InterfaceBlock(
        kUniform_Modifier, "uniforms2",
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLInlining, r, ctxInfo) {
  ...
  DSLFunction sqr(kFloat_Type, "sqr", x);
  sqr.define(Return(x * x));
  DSLFunction(kVoid_Type, "main").define(sk_FragColor().assign((sqr(2), Half4(sqr(3)))));
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLPrototypes, r, ctxInfo) {
...

  {
    ...
    REPORTER_ASSERT(r, SkSL::ThreadContext::ProgramElements().size() == 3);
    EXPECT_EQUAL(
        *SkSL::ThreadContext::ProgramElements()[2], "float sqr(float x) { return (x * x); }");
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLModifiersDeclaration, r, ctxInfo) {
  ...
  REPORTER_ASSERT(r, SkSL::ThreadContext::ProgramElements().size() == 1);
  EXPECT_EQUAL(
      *SkSL::ThreadContext::ProgramElements()[0], "layout(blend_support_all_equations) out;");
}
