...
static void test_loop_stream(
    skiatest::Reporter* reporter, SkStream* stream, const void* src, size_t len, int repeat) {
...
static void TestPackedUInt(skiatest::Reporter* reporter) {
  ...
  const size_t sizes[] = {0,          1,          2,          0xFC,      0xFD,       0xFE,
                          0xFF,       0x100,      0x101,      32767,     32768,      32769,
                          0xFFFD,     0xFFFE,     0xFFFF,     0x10000,   0x10001,    0xFFFFFD,
                          0xFFFFFE,   0xFFFFFF,   0x1000000,  0x1000001, 0x7FFFFFFE, 0x7FFFFFFF,
 ...
 */
static size_t compare_peek_to_read(
    skiatest::Reporter* reporter, SkStream* stream, size_t bytesToPeek) {
...
#  ifdef SK_ENABLE_ANDROID_UTILS
static void test_peeking_front_buffered_stream(
    skiatest::Reporter* r, const SkStream& original, size_t bufferSize) {
  std::unique_ptr<SkStream> dupe(original.duplicate());
  REPORTER_ASSERT(r, dupe != nullptr);
  auto bufferedStream = android::skia::FrontBufferedStream::Make(std::move(dupe), bufferSize);
...
static void stream_peek_test(
    skiatest::Reporter* rep, SkStreamAsset* asset, const SkData* expected) {
  ...
  for (size_t i = 0; i < asset->getLength(); ++i) {
    uint32_t maxSize = SkToU32(std::min(sizeof(buffer), asset->getLength() - i));
...
class DumbStream : public SkStream {
 public:
  DumbStream(const uint8_t* data, size_t n) : fData(data), fCount(n), fIdx(0) {}
  ...
  bool isAtEnd() const override { return fCount == fIdx; }
...
static void stream_copy_test(
    skiatest::Reporter* reporter, const void* srcData, size_t N, SkStream* stream) {
