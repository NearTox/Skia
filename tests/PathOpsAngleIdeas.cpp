...
static double testArc(
    skiatest::Reporter* reporter, const SkDQuad& quad, const SkDQuad& arcRef, int octant) {
  SkDQuad arc = arcRef;
...
static void orderQuads(
    skiatest::Reporter* reporter, const SkDQuad& quad, double radius,
    SkTArray<double, false>* tArray) {
  double r = radius;
  double s = r * SK_ScalarTanPIOver8;
  double m = r * SK_ScalarRoot2Over2;
  // construct circle from quads
  const QuadPts circle[8] = {{{{r, 0}, {r, -s}, {m, -m}}},    {{{m, -m}, {s, -r}, {0, -r}}},
                             {{{0, -r}, {-s, -r}, {-m, -m}}}, {{{-m, -m}, {-r, -s}, {-r, 0}}},
                             {{{-r, 0}, {-r, s}, {-m, m}}},   {{{-m, m}, {-s, r}, {0, r}}},
                             {{{0, r}, {s, r}, {m, m}}},      {{{m, m}, {r, s}, {r, 0}}}};
...
static bool checkParallel(
    skiatest::Reporter* reporter, const SkDQuad& quad1, const SkDQuad& quad2) {
...
static bool orderTRange(
    skiatest::Reporter* reporter, const SkDQuad& quad1, const SkDQuad& quad2, double r,
    TRange* result) {
      ...
      if (stepUp) {
        ...
        if (gPathOpsAngleIdeasVerbose) {
          SkDebugf(
              ...
              lowerRange->tMin, upperRange->tMin, r, tRange.tMin);
        }
...
static void bruteForce(
    skiatest::Reporter* reporter, const SkDQuad& quad1, const SkDQuad& quad2, bool ccw) {
...
static void testQuadAngles(
    skiatest::Reporter* reporter, const SkDQuad& quad1, const SkDQuad& quad2, int testNo,
  ...
  int realOverlap =
      PathOpsAngleTester::ConvexHullOverlaps(*seg1->debugLastAngle(), *seg2->debugLastAngle());
  ...
  bool oldSchoolOverlap = radianBetween(a1s, a2s, a1e) || radianBetween(a1s, a2e, a1e) ||
                          radianBetween(a2s, a1s, a2e) || radianBetween(a2s, a1e, a2e);
  ...
  if (oldSchoolOverlap != (overlap < 0)) {
    overlap =
        quadHullsOverlap(reporter, quad1, quad2);  // set a breakpoint and debug if assert fires
  ...
  if (useIntersect) {
    ...
    if (gPathOpsAngleIdeasVerbose) {
      SkDebugf(
          ...
          sIndex == 0, vDir[sIndex] < 0, midXray < 0);
    }
    SkASSERT(
        SkScalarSignAsInt(SkDoubleToScalar(midXray)) ==
...
DEF_TEST(PathOpsAngleBruteTOne, reporter) {
  //    gPathOpsAngleIdeasVerbose = true;
  const QuadPts qPts[] = {
      {{{-770.8492431640625, 948.2369384765625},
      ...
      {{{563.8267822265625, -107.4566650390625},
      ...
      {{{598.857421875, 846.345458984375},
...
static double mDistance(
    skiatest::Reporter* reporter, bool agrees, const SkDQuad& q1, const SkDQuad& q2) {
  if (1 && agrees) {
    return SK_ScalarMax;
  }
  ...
  if (gPathOpsAngleIdeasVerbose) {
    SkDebugf(
        ...
        agrees ? 'T' : 'F', endCtrlRatio(q1), endCtrlRatio(q2), m, dist1, dist2,
        dir > 0 ? '+' : '-', mV1[0].crossCheck(v2), mV1[1].crossCheck(v2), mV2[0].crossCheck(v1),
        mV2[1].crossCheck(v1));
  }
  if (1) {
...
static void midPointAgrees(
    skiatest::Reporter* reporter, const SkDQuad& q1, const SkDQuad& q2, bool ccw) {
