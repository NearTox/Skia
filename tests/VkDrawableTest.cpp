...
class TestDrawable : public SkDrawable {
 public:
  TestDrawable(const GrVkInterface* interface, GrContext* context, int32_t width, int32_t height)
      : INHERITED(), fInterface(interface), fContext(context), fWidth(width), fHeight(height) {}
  ...
  static void ImportDraw(
    ...
    sk_sp<SkSurface> surf = SkSurface::MakeRenderTarget(td->fContext, SkBudgeted::kYes, bufferInfo);
  ...
  std::unique_ptr<GpuDrawHandler> onSnapGpuDrawHandler(
    ...
    if (fContext) {
      draw.reset(
          new DrawHandlerImport(this, ImportDraw, ImportSubmitted, matrix, clipBounds, bufferInfo));
    } else {
...
void draw_drawable_test(skiatest::Reporter* reporter, GrContext* context, GrContext* childContext) {
  ...
  sk_sp<SkSurface> surface(SkSurface::MakeRenderTarget(
      context, SkBudgeted::kNo, ii, 0, kTopLeft_GrSurfaceOrigin, nullptr));
...
DEF_GPUTEST(VkDrawableImportTest, reporter, options) {
  for (int typeInt = 0; typeInt < sk_gpu_test::GrContextFactory::kContextTypeCnt; ++typeInt) {
    ...
    if (ctxInfo.grContext()) {
      sk_gpu_test::ContextInfo child = factory.getSharedContextInfo(ctxInfo.grContext(), 0);
      if (!child.grContext()) {
        continue;
      }
