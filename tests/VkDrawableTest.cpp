...
class TestDrawable : public SkDrawable {
 public:
  TestDrawable(
      const GrVkInterface* interface, GrDirectContext* dContext, int32_t width, int32_t height)
      : INHERITED(), fInterface(interface), fDContext(dContext), fWidth(width), fHeight(height) {}
  ...
  class DrawHandlerBasic : public GpuDrawHandler {
   public:
    DrawHandlerBasic(const GrVkInterface* interface, int32_t width, int32_t height)
        : INHERITED(), fInterface(interface), fWidth(width), fHeight(height) {}
    ...
    void draw(const GrBackendDrawableInfo& info) override {
      ...
      GR_VK_CALL(
          fInterface,
          CmdClearAttachments(vkInfo.fSecondaryCommandBuffer, 1, &attachment, 1, &clearRect));
  ...
  typedef void (*DrawProc)(
      TestDrawable*, const SkMatrix&, const SkIRect&, const SkImageInfo&, const GrVkDrawableInfo&);
  ...
  class DrawHandlerImport : public GpuDrawHandler {
   public:
    DrawHandlerImport(
        TestDrawable* td, DrawProc drawProc, SubmitProc submitProc, const SkMatrix& matrix,
        const SkIRect& clipBounds, const SkImageInfo& bufferInfo)
        : INHERITED(),
          fTestDrawable(td),
          fDrawProc(drawProc),
    ...
    ~DrawHandlerImport() override { fSubmitProc(fTestDrawable); }
  ...
  static void ImportDraw(
      TestDrawable* td, const SkMatrix& matrix, const SkIRect& clipBounds,
      const SkImageInfo& bufferInfo, const GrVkDrawableInfo& info) {
    td->fDrawContext = GrVkSecondaryCBDrawContext::Make(td->fDContext, bufferInfo, info, nullptr);
    if (!td->fDrawContext) {
      return;
    }
    ...
    sk_sp<SkSurface> surf =
        SkSurface::MakeRenderTarget(td->fDContext, SkBudgeted::kYes, bufferInfo);
  ...
  std::unique_ptr<GpuDrawHandler> onSnapGpuDrawHandler(
      GrBackendApi backendApi, const SkMatrix& matrix, const SkIRect& clipBounds,
    ...
    if (fDContext) {
      draw = std::make_unique<DrawHandlerImport>(
          this, ImportDraw, ImportSubmitted, matrix, clipBounds, bufferInfo);
    } else {
  ...
  SkRect onGetBounds() override { return SkRect::MakeLTRB(fWidth / 2, 0, fWidth, fHeight); }

  void onDraw(SkCanvas*) override { SkASSERT(false); }
...
void draw_drawable_test(
    skiatest::Reporter* reporter, GrDirectContext* dContext, GrDirectContext* childDContext) {
  ...
  sk_sp<SkSurface> surface(SkSurface::MakeRenderTarget(
      dContext, SkBudgeted::kNo, ii, 0, kTopLeft_GrSurfaceOrigin, nullptr));
  ...
  for (int cy = 0; cy < DEV_H && !failureFound; ++cy) {
    for (int cx = 0; cx < DEV_W && !failureFound; ++cx) {
      ...
      if (expectedPixel != canvasPixel) {
        failureFound = true;
        ERRORF(
            reporter, "Wrong color at %d, %d. Got 0x%08x when we expected 0x%08x", cx, cy,
            canvasPixel, expectedPixel);
      }
    }
  }
}
...
DEF_GPUTEST(VkDrawableImportTest, reporter, options) {
  for (int typeInt = 0; typeInt < sk_gpu_test::GrContextFactory::kContextTypeCnt; ++typeInt) {
    ...
    if (ctxInfo.directContext()) {
      sk_gpu_test::ContextInfo child = factory.getSharedContextInfo(ctxInfo.directContext(), 0);
