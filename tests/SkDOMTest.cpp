...
static const SkDOM::Node* check_node(
    skiatest::Reporter* r, const SkDOM& dom, const SkDOM::Node* node, const char* expectedName,
...
DEF_TEST(SkDOM_test, r) {
  ...
  {
    const auto* elem1 = check_node(r, dom, dom.getFirstChild(root), "elem1", SkDOM::kElement_Type);
    const auto* elem2 =
        check_node(r, dom, dom.getNextSibling(elem1), "elem2", SkDOM::kElement_Type);
    const auto* elem3 =
        check_node(r, dom, dom.getNextSibling(elem2), "elem3", SkDOM::kElement_Type);
    {
      const auto* subelem1 =
          check_node(r, dom, dom.getFirstChild(elem3), "subelem1", SkDOM::kElement_Type);
      { check_node(r, dom, dom.getFirstChild(subelem1), "Some text.", SkDOM::kText_Type); }
      const auto* subelem2 =
          check_node(r, dom, dom.getNextSibling(subelem1), "subelem2", SkDOM::kElement_Type);
      const auto* subelem3 =
          check_node(r, dom, dom.getNextSibling(subelem2), "subelem3", SkDOM::kElement_Type);
