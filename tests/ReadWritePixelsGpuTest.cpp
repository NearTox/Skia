...
// the pixmap.
template <typename T>
using GpuSrcFactory = T(SkPixmap&);
...
template <typename T>
using GpuDstFactory = T(const SkImageInfo& ii);
...
static SkAutoPixmapStorage make_ref_data(const SkImageInfo& info, bool forceOpaque) {
  ...
  if (info.alphaType() != kOpaque_SkAlphaType && !forceOpaque) {
    ...
    paint.setShader(SkGradientShader::MakeRadial(
        {info.width() / 2.f, info.height() / 2.f}, (info.width() + info.height()) / 10.f, kColors3,
        kPos3, 5, SkTileMode::kMirror));
...
template <typename T>
static void gpu_read_pixels_test_driver(
    skiatest::Reporter* reporter, const GpuReadPixelTestRules& rules,
    const std::function<GpuSrcFactory<T>>& srcFactory, const std::function<GpuReadSrcFn<T>>& read,
    SkString label) {
  if (!label.isEmpty()) {
  ...
  auto runTest = [&](const T& src, const SkPixmap& srcPixels, const SkImageInfo& readInfo,
    ...
    std::fill_n(
        static_cast<char*>(dstPixels.writable_addr()), dstPixels.computeByteSize(), kInitialByte);
    ...
    if (!SkIRect::Intersects(rect, surfBounds)) {
      ...
      if (SkColorTypeToGrColorType(readCT) != GrColorType::kUnknown) {
        ERRORF(
            ...
            label.c_str(), ToolUtils::colortype_name(srcCT), ToolUtils::alphatype_name(srcAT),
            ToolUtils::colortype_name(readCT), ToolUtils::alphatype_name(readAT), rect.fLeft,
            rect.fTop, rect.fRight, rect.fBottom, csConversion);
      }
      return result;
    }
    ...
    if (result == Result::kSuccess && srcReadRect.intersect(surfBounds, rect)) {
      ...
      if (readAT != kOpaque_SkAlphaType && srcAT != kOpaque_SkAlphaType) {
        ...
        // color format.
        const int alphaBits = std::min({alpha_channel_bits(readCT), alpha_channel_bits(srcCT), 8});
        alphaTol = 2.f / (1 << alphaBits);
      }

      const float tols[4] = {tol, tol, tol, alphaTol};
      auto error =
          std::function<ComparePixmapsErrorReporter>([&](int x, int y, const float diffs[4]) {
            SkASSERT(x >= 0 && y >= 0);
            ERRORF(
                reporter,
                ...
                label.c_str(), ToolUtils::colortype_name(srcCT), ToolUtils::alphatype_name(srcAT),
                ToolUtils::colortype_name(readCT), ToolUtils::alphatype_name(readAT), rect.fLeft,
                rect.fTop, rect.fRight, rect.fBottom, csConversion, x, y, diffs[0], diffs[1],
                diffs[2], diffs[3]);
          });
      ...
      if (readAT == kUnknown_SkAlphaType) {
        ...
        SkPixmap unpremulRef(
            refInfo.makeAlphaType(kUnpremul_SkAlphaType), ref.addr(), ref.rowBytes());
        SkPixmap unpremulSRc(
            srcPixels.info().makeAlphaType(kUnpremul_SkAlphaType), srcPixels.addr(),
            srcPixels.rowBytes());
    ...
    if (!guardOk) {
      ERRORF(
          ...
          ToolUtils::colortype_name(srcCT), ToolUtils::colortype_name(readCT), csConversion);
    }
    return result;
  };
  ...
  for (int sat = 0; sat < kLastEnum_SkAlphaType; ++sat) {
    ...
    for (int sct = 0; sct <= kLastEnum_SkColorType; ++sct) {
      ...
      if (SkColorTypeIsAlphaOnly(srcCT) && srcCTTestedThoroughly[srcCT] &&
          (kUnpremul_SkAlphaType == srcAT || kOpaque_SkAlphaType == srcAT ||
        ...
        for (const sk_sp<SkColorSpace>& readCS :
          ...
          for (int at = 0; at <= kLastEnum_SkAlphaType; ++at) {
            ...
            if (SkColorTypeIsAlphaOnly(readCT) && readCTTestedThoroughly[readCT] &&
                (kUnpremul_SkAlphaType == readAT || kOpaque_SkAlphaType == readAT ||
            ...
            for (const auto& rect : rects) {
              const auto readInfo =
                  SkImageInfo::Make(rect.width(), rect.height(), readCT, readAT, readCS);
    ...
    for (GrSurfaceOrigin origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
      auto factory =
          std::function<GpuSrcFactory<Surface>>([direct, origin, renderable](const SkPixmap& src) {
            auto sc =
                CreateSurfaceContext(direct, src.info(), SkBackingFit::kExact, origin, renderable);
            if (sc) {
...
DEF_GPUTEST_FOR_ALL_CONTEXTS(WritePixels_InvalidRowBytes_Gpu, reporter, ctxInfo) {
  ...
  for (int ct = 0; ct < kLastEnum_SkColorType + 1; ++ct) {
    ...
    REPORTER_ASSERT(
        reporter, !surf->getCanvas()->writePixels(srcII, storage.get(), badRowBytes, 0, 0));
  }
}
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SurfaceAsyncReadPixels, reporter, ctxInfo) {
  ...
  auto reader = std::function<GpuReadSrcFn<Surface>>(
      [](const Surface& surface, const SkIPoint& offset, const SkPixmap& pixels) {
        ...
        surface->asyncRescaleAndReadPixels(
            pixels.info(), rect, SkImage::RescaleGamma::kSrc, SkImage::RescaleMode::kNearest,
        ...
        SkRectMemcpy(
            pixels.writable_addr(), pixels.rowBytes(), context.fResult->data(0),
  ...
  for (GrSurfaceOrigin origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
    auto factory = std::function<GpuSrcFactory<Surface>>([context = ctxInfo.directContext(),
                                                          origin](const SkPixmap& src) {
      auto surf =
          SkSurface::MakeRenderTarget(context, SkBudgeted::kYes, src.info(), 1, origin, nullptr);
      if (surf) {
    ...
    auto backendRTFactory = std::function<GpuSrcFactory<Surface>>(
        [context = ctxInfo.directContext(), origin](const SkPixmap& src) {
          ...
          auto surf = sk_gpu_test::MakeBackendRenderTargetSurface(context, src.info(), origin, 1);
          if (surf) {
            surf->writePixels(src, 0, 0);
          }
          return surf;
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ImageAsyncReadPixels, reporter, ctxInfo) {
  ...
  auto reader = std::function<GpuReadSrcFn<Image>>(
      [context](const Image& image, const SkIPoint& offset, const SkPixmap& pixels) {
        ...
        SkRectMemcpy(
            pixels.writable_addr(), pixels.rowBytes(), asyncContext.fResult->data(0),
            asyncContext.fResult->rowBytes(0), pixels.info().minRowBytes(), pixels.height());
        return Result::kSuccess;
      });
  ...
  for (auto origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
    for (auto renderable : {GrRenderable::kNo, GrRenderable::kYes}) {
      auto factory = std::function<GpuSrcFactory<Image>>([&](const SkPixmap& src) {
        return sk_gpu_test::MakeBackendTextureImage(
            ctxInfo.directContext(), src, renderable, origin);
      });
      auto label = SkStringPrintf("Renderable: %d, Origin: %d", (int)renderable, origin);
      gpu_read_pixels_test_driver(reporter, rules, factory, reader, label);
    }
...
DEF_GPUTEST(AsyncReadPixelsContextShutdown, reporter, options) {
  const auto ii = SkImageInfo::Make(
      10, 10, kRGBA_8888_SkColorType, kPremul_SkAlphaType, SkColorSpace::MakeSRGB());
      ...
      for (bool yuv : {false, true}) {
        ...
        if (yuv) {
          surf->asyncRescaleAndReadPixelsYUV420(
              kIdentity_SkYUVColorSpace, SkColorSpace::MakeSRGB(), ii.bounds(), ii.dimensions(),
              SkImage::RescaleGamma::kSrc, SkImage::RescaleMode::kNearest, &async_callback,
              &cbContext);
        } else {
          surf->asyncRescaleAndReadPixels(
              ii, ii.bounds(), SkImage::RescaleGamma::kSrc, SkImage::RescaleMode::kNearest,
        ...
        if (!cbContext.fResult) {
          ERRORF(
              reporter, "Callback failed on %s. is YUV: %d",
        ...
        switch (sequence) {
          ...
          case ShutdownSequence::kFreeResult_Abandon_DestroyContext: break;
          case ShutdownSequence::kDestroyContext_FreeResult: factory.destroyContexts(); break;
        ...
        switch (sequence) {
          ...
          case ShutdownSequence::kAbandon_FreeResult_DestroyContext:
          case ShutdownSequence::kReleaseAndAbandon_DestroyContext_FreeResult:
          case ShutdownSequence::kAbandon_DestroyContext_FreeResult: break;
        }
      }
    }
  }
...
template <typename T>
static void gpu_write_pixels_test_driver(
    skiatest::Reporter* reporter, const std::function<GpuDstFactory<T>>& dstFactory,
  ...
  auto runTest = [&](const T& dst, const SkImageInfo& dstInfo, const SkPixmap& srcPixels,
                     SkIPoint offset) {
    const bool csConversion =
    ...
    if (!SkIRect::Intersects(rect, surfBounds)) {
      ...
      if (SkColorTypeToGrColorType(writeCT) != GrColorType::kUnknown) {
        ERRORF(
            ...
            ToolUtils::colortype_name(dstCT), ToolUtils::alphatype_name(dstAT), rect.fLeft,
            rect.fTop, rect.fRight, rect.fBottom, csConversion);
      }
      return result;
    }
    ...
    if (!secondReadPM.addr()) {
      ...
      ERRORF(
          reporter, "could not read from dst (CT: %s, AT: %s)\n", ToolUtils::colortype_name(dstCT),
    ...
    if (writeAT != kOpaque_SkAlphaType && dstAT != kOpaque_SkAlphaType) {
      // Alpha can also get squashed down to 8 bits going through an intermediate
      // color format.
      const int alphaBits = std::min({alpha_channel_bits(writeCT), alpha_channel_bits(dstCT), 8});
      alphaTol = 2.f / (1 << alphaBits);
    }
    ...
    auto error = std::function<ComparePixmapsErrorReporter>([&](int x, int y,
      ...
      ERRORF(
          ...
          "Error at %d, %d. Diff in floats: (%f, %f, %f, %f)",
          ToolUtils::colortype_name(writeCT), ToolUtils::alphatype_name(writeAT),
          ToolUtils::colortype_name(dstCT), ToolUtils::alphatype_name(dstAT), rect.fLeft, rect.fTop,
          rect.fRight, rect.fBottom, csConversion, x, y, diffs[0], diffs[1], diffs[2], diffs[3]);
    });
    ...
    SkAssertResult(make_pixmap_have_valid_alpha_type(srcPixels).readPixels(
        make_pixmap_have_valid_alpha_type(ref), std::max(0, -offset.fX), std::max(0, -offset.fY)));
      ...
      for (int wct = 0; wct <= kLastEnum_SkColorType; ++wct) {
        ...
        for (const sk_sp<SkColorSpace>& writeCS :
          ...
          for (int wat = 0; wat <= kLastEnum_SkAlphaType; ++wat) {
            ...
            if (SkColorTypeIsAlwaysOpaque(writeCT) && writeCTTestedThoroughly[writeCT] &&
            ...
            if (SkColorTypeIsAlphaOnly(writeCT) && writeCTTestedThoroughly[writeCT] &&
                (kUnpremul_SkAlphaType == writeAT || kOpaque_SkAlphaType == writeAT ||
                 kUnknown_SkAlphaType == writeAT)) {
              continue;
            }
            const auto& rects = dstCTTestedThoroughly[dct] && writeCTTestedThoroughly[wct]
                                    ? shortRectArray
                                    : longRectArray;
            for (const auto& rect : rects) {
              auto writeInfo = SkImageInfo::Make(rect.size(), writeCT, writeAT, writeCS);
              // CPU and GPU handle 1010102 differently. CPU clamps RGB to A, GPU
              // doesn't.
              bool forceOpaque =
                  writeCT == kRGBA_1010102_SkColorType || writeCT == kBGRA_1010102_SkColorType;
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SurfaceContextWritePixels, reporter, ctxInfo) {
  ...
  auto reader = std::function<GpuReadDstFn<Surface>>([direct](const Surface& s) {
    ...
    auto skInfo =
        SkImageInfo::Make(grInfo.dimensions(), ct, grInfo.alphaType(), grInfo.refColorSpace());
    result.alloc(skInfo);
    ...
    for (GrSurfaceOrigin origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
      auto factory = std::function<GpuDstFactory<Surface>>(
          [direct, origin, renderable](const SkImageInfo& info) {
            return CreateSurfaceContext(direct, info, SkBackingFit::kExact, origin, renderable);
          });
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SurfaceContextWritePixelsMipped, reporter, ctxInfo) {
  ...
  static constexpr int kW = 25, kH = 37;
  SkAutoPixmapStorage refP = make_ref_data(
      SkImageInfo::Make({kW, kH}, kRGBA_F32_SkColorType, kPremul_SkAlphaType, nullptr), false);
  SkAutoPixmapStorage refO = make_ref_data(
      SkImageInfo::Make({kW, kH}, kRGBA_F32_SkColorType, kOpaque_SkAlphaType, nullptr), true);
      ...
      for (auto renderable : {GrRenderable::kNo, GrRenderable::kYes}) {
        for (GrSurfaceOrigin origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
          auto sc = CreateSurfaceContext(
              direct, info, SkBackingFit::kExact, origin, renderable,
              /*sample count*/ 1, GrMipmapped::kYes);
          if (!sc) {
            continue;
          }
          ...
          if (unowned) {
            ...
            levels[1] = GrCPixmap(levels[1].info(), keepAlive.addr(), keepAlive.rowBytes());
          }
          ...
          auto dstSC = CreateSurfaceContext(
              direct, info, SkBackingFit::kExact, kBottomLeft_GrSurfaceOrigin, GrRenderable::kYes);
          SkASSERT(dstSC);
          GrSamplerState sampler(SkFilterMode::kNearest, SkMipmapMode::kNearest);
          for (int i = 1; i <= 1; ++i) {
            auto te = GrTextureEffect::Make(
                sc->readSurfaceView(), info.alphaType(), SkMatrix::I(), sampler,
                *direct->priv().caps());
            dstSC->asFillContext()->fillRectToRectWithFP(
                SkIRect::MakeSize(sc->dimensions()), SkIRect::MakeSize(levels[i].dimensions()),
                std::move(te));
            GrImageInfo readInfo = dstSC->imageInfo().makeDimensions(levels[i].dimensions());
            ...
            if (ct == GrColorType::kRGBA_8888_SRGB) {
              a = GrCPixmap(
                  a.info().makeColorType(GrColorType::kRGBA_8888), a.addr(), a.rowBytes());
              b = GrCPixmap(
                  b.info().makeColorType(GrColorType::kRGBA_8888), b.addr(), b.rowBytes());
            }
            ...
            auto error =
                std::function<ComparePixmapsErrorReporter>([&](int x, int y, const float diffs[4]) {
                  ...
                  ERRORF(
                      ...
                      "(%f, %f, %f, %f)",
                      GrColorTypeToStr(info.colorType()), i, unowned, x, y, diffs[0], diffs[1],
                      diffs[2], diffs[3]);
                });
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(GLReadPixelsUnbindPBO, reporter, ctxInfo) {
  ...
  AsyncContext asyncContext;
  image->asyncRescaleAndReadPixels(
      info, SkIRect::MakeSize(info.dimensions()), SkImage::RescaleGamma::kSrc,
      SkImage::RescaleMode::kNearest, async_callback, &asyncContext);
  ...
  auto error = std::function<ComparePixmapsErrorReporter>([&](int x, int y, const float diffs[4]) {
    SkASSERT(x >= 0 && y >= 0);
    ERRORF(
        reporter,
        "Expect sync and async read to be the same. "
        "Error at %d, %d. Diff in floats: (%f, %f, %f, %f)",
