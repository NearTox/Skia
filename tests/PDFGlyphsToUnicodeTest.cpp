...
static bool stream_equals(
    const SkDynamicMemoryWStream& stream, size_t offset, const char* buffer, size_t len) {
...
DEF_TEST(SkPDF_ToUnicode, reporter) {
  ...
  SkPDFAppendCmapSections(
      &glyphToUnicode[0], &subset, &buffer, true, 0, std::min<SkGlyphID>(0xFFFF, lastGlyphID));
...
endbfrange\n";
  ...
  REPORTER_ASSERT(reporter, stream_equals(buffer, 0, expectedResult, buffer.bytesWritten()));

  // Remove characters and ranges.
  buffer.reset();

  SkPDFAppendCmapSections(
      &glyphToUnicode[0], &subset, &buffer, true, 8, std::min<SkGlyphID>(0x00FF, lastGlyphID));
...
endbfrange\n";
  ...
  REPORTER_ASSERT(reporter, stream_equals(buffer, 0, expectedResultChop1, buffer.bytesWritten()));

  // Remove characters from range to downdrade it to one char.
  buffer.reset();

  SkPDFAppendCmapSections(
      &glyphToUnicode[0], &subset, &buffer, true, 0x00D, std::min<SkGlyphID>(0x00FE, lastGlyphID));
...
endbfchar\n";
  ...
  REPORTER_ASSERT(reporter, stream_equals(buffer, 0, expectedResultChop2, buffer.bytesWritten()));

  buffer.reset();

  SkPDFAppendCmapSections(
      &glyphToUnicode[0], nullptr, &buffer, false, 0xFC, std::min<SkGlyphID>(0x110, lastGlyphID));
...
endbfrange\n";
  ...
  REPORTER_ASSERT(
      reporter, stream_equals(buffer, 0, expectedResultSingleBytes, buffer.bytesWritten()));
  ...
  SkPDFAppendCmapSections(
      &glyphToUnicode[0], &subset2, &buffer2, true, 0, std::min<SkGlyphID>(0xFFFF, lastGlyphID));
...
endbfrange\n";
  ...
  REPORTER_ASSERT(reporter, stream_equals(buffer2, 0, expectedResult2, buffer2.bytesWritten()));
}
