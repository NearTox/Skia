...
DEF_TEST(HighContrastFilter_FilterImage, reporter) {
  ...
  SkRect r =
      SkRect::MakeLTRB(SkIntToScalar(2), SkIntToScalar(2), SkIntToScalar(8), SkIntToScalar(8));
  ...
  for (int y = r.top(); y < r.bottom(); ++y) {
    for (int x = r.left(); x < r.right(); ++x) {
      ...
      REPORTER_ASSERT(reporter, filterColor == paint.getColorFilter()->filterColor(paintColor));
    }
  }
}
