...
bool VkYcbcrSamplerTestHelper::init(skiatest::Reporter* reporter) {
  ...
  if (!sk_gpu_test::CreateVkBackendContext(
          getProc, &fBackendContext, &fExtensions, &fFeatures, &fDebugCallback, nullptr,
          sk_gpu_test::CanPresentFn(), false)) {
    return false;
  }
...
sk_sp<SkImage> VkYcbcrSamplerTestHelper::createI420Image(skiatest::Reporter* reporter) {
  ...
  fVkGetPhysicalDeviceFormatProperties(
      fBackendContext.fPhysicalDevice, VK_FORMAT_G8_B8R8_2PLANE_420_UNORM, &formatProperties);
  ...
  GrVkYcbcrConversionInfo ycbcrInfo(
      vkImageInfo.format,
      /*externalFormat=*/0, VK_SAMPLER_YCBCR_MODEL_CONVERSION_YCBCR_709,
      VK_SAMPLER_YCBCR_RANGE_ITU_NARROW, VK_CHROMA_LOCATION_COSITED_EVEN,
      VK_CHROMA_LOCATION_COSITED_EVEN, VK_FILTER_LINEAR, false,
      formatProperties.linearTilingFeatures);
  GrVkAlloc alloc(fImageMemory, 0 /* offset */, requirements.size, 0 /* flags */);
  GrVkImageInfo imageInfo(
      fImage, alloc, VK_IMAGE_TILING_LINEAR, VK_IMAGE_LAYOUT_UNDEFINED, vkImageInfo.format,
      1 /* levelCount */, VK_QUEUE_FAMILY_IGNORED, GrProtected::kNo, ycbcrInfo);

  texture = GrBackendTexture(kImageWidth, kImageHeight, imageInfo);
  sk_sp<SkImage> image = SkImage::MakeFromTexture(
      fGrContext.get(), texture, kTopLeft_GrSurfaceOrigin, kRGB_888x_SkColorType,
...
DEF_GPUTEST(VkYCbcrSampler_DrawImageWithYcbcrSampler, reporter, options) {
  ...
  if (!surface->readPixels(
          SkImageInfo::Make(kImageWidth, kImageHeight, kRGBA_8888_SkColorType, kOpaque_SkAlphaType),
  ...
  for (size_t y = 0; y < kImageHeight; y += 2) {
    for (size_t x = 0; x < kImageWidth; x += 2) {
      ...
      int expectedG = round_and_clamp(
          (yChannel - 0.13397432f / 0.7152f * uChannel - 0.33480248f / 0.7152f * vChannel) * 255.0);
