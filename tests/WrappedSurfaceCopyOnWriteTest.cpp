...
DEF_GPUTEST_FOR_ALL_CONTEXTS(WrappedSurfaceCopyOnWrite, reporter, ctxInfo) {
  ...
  auto makeDirectBackendSurface = [&]() {
    ...
    return sk_gpu_test::MakeBackendTextureSurface(
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SkipCopyTaskTest, reporter, ctxInfo) {
  ...
  GrImageInfo info(
      ...
      /*color space*/ nullptr, 10, 10);

  auto dstSC = CreateSurfaceContext(
      dContext, info, SkBackingFit::kExact, kBottomLeft_GrSurfaceOrigin, GrRenderable::kYes);
  dstSC->asFillContext()->clear(SkPMColor4f{1, 0, 0, 1});

  auto srcSC = CreateSurfaceContext(
      dContext, info, SkBackingFit::kExact, kBottomLeft_GrSurfaceOrigin, GrRenderable::kYes);
  srcSC->asFillContext()->clear(SkPMColor4f{0, 0, 1, 1});

  sk_sp<GrRenderTask> task = dContext->priv().drawingManager()->newCopyRenderTask(
      srcSC->asSurfaceProxyRef(), SkIRect::MakeWH(10, 10), dstSC->asSurfaceProxyRef(), {0, 0},
  ...
  float kTol[4] = {};
  std::function<ComparePixmapsErrorReporter> errorReporter([&](int x, int y, const float diffs[4]) {
    ERRORF(
        reporter, "Expected {1, 0, 0, 1}. diff {%f, %f, %f, %f}", diffs[0], diffs[1], diffs[2],
        diffs[3]);
  });
