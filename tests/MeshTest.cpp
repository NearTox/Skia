...
static SkString make_description(
    SkSpan<const Attribute> attributes, size_t stride, SkSpan<const Varying> varyings,
...
static bool check_for_failure(
    skiatest::Reporter* r, SkSpan<const Attribute> attributes, size_t stride,
    SkSpan<const Varying> varyings, const SkString& vs, const SkString& fs) {
  ...
  ERRORF(
      r, "Expected to fail but succeeded:\n%s",
      make_description(attributes, stride, varyings, vs, fs).c_str());
  return false;
}

static bool check_for_success(
    skiatest::Reporter* r, SkSpan<const Attribute> attributes, size_t stride,
    SkSpan<const Varying> varyings, const SkString& vs, const SkString& fs,
  ...
  ERRORF(
      r, "Expected to succeed but failed:\n%sError:\n%s",
      make_description(attributes, stride, varyings, vs, fs).c_str(), error.c_str());
  return false;
}
...
static void test_good(skiatest::Reporter* r) {
  for (const auto& validFS : kValidFSes) {
    if (!check_for_success(
            r, SkMakeSpan(kValidAttrs, SK_ARRAY_COUNT(kValidAttrs)), kValidStride,
            SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), kValidVS, validFS)) {
      return;
    }
  }
}
...
static void test_bad_sig(skiatest::Reporter* r) {
  ...
  for (const char* vsSig : kInvalidVSSigs) {
    ...
    for (const auto& validFS : kValidFSes) {
      if (!check_for_failure(
              r, SkMakeSpan(kValidAttrs, SK_ARRAY_COUNT(kValidAttrs)), kValidStride,
              SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), invalidVS, validFS)) {
        return;
      }
    }
  }
  ...
  for (const char* noColorFSSig : kInvalidNoColorFSSigs) {
    ...
    if (!check_for_failure(
            r, SkMakeSpan(kValidAttrs, SK_ARRAY_COUNT(kValidAttrs)), kValidStride,
            SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), kValidVS, invalidFS)) {
      return;
    }
  }
...
static void test_float4_color(skiatest::Reporter* r) {
  ...
  check_for_success(
      r, SkMakeSpan(kValidAttrs, SK_ARRAY_COUNT(kValidAttrs)), kValidStride,
      SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), kValidVS, kFloat4FS);
}

// We don't allow child effects in custom meshes currently.
static void test_bad_globals(skiatest::Reporter* r) {
  static constexpr const char* kBadGlobals[]{"uniform shader myshader;"};
  for (const auto& global : kBadGlobals) {
    SkString badVS = kValidVS;
    badVS.prepend(global);
    if (!check_for_failure(
            r, SkMakeSpan(kValidAttrs, SK_ARRAY_COUNT(kValidAttrs)), kValidStride,
            SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), badVS, kValidFSes[0])) {
      return;
    }
  }
  for (const auto& global : kBadGlobals) {
    ...
    if (!check_for_failure(
            r, SkMakeSpan(kValidAttrs, SK_ARRAY_COUNT(kValidAttrs)), kValidStride,
            SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), kValidVS, badFS)) {
      return;
    }
  }
}
...
static void test_good_uniforms(skiatest::Reporter* r) {
  ...
  struct {
    ...
    const std::vector<SkMeshSpecification::Uniform> expectations;
      ...
      // A single VS uniform.
      {{"uniform float x;"}, {}, {make_uni(Type::kFloat, "x", 0, kVS)}},

      // A single FS uniform.
      {{}, {"uniform float2 v;"}, {make_uni(Type::kFloat2, "v", 0, kFS)}},
      ...
      {{
       ...
       {make_uni(Type::kFloat, "x", 0, kVS | kFS, 2), make_uni(Type::kInt, "y", 8, kVS, 0)}},
      ...
      {{
       ...
       {make_uni(Type::kFloat3x3, "m", 0, kVS | kFS), make_uni(Type::kInt2, "i2", 36, kFS)}},
      ...
      {{
       ...
       {make_uni(Type::kFloat4x4, "m", 0, kVS | kFS, 4), make_uni(Type::kInt3, "i3", 256, kFS, 1)}},
  ...
  for (const auto& c : kTestCases) {
    ...
    if (uniforms.size() != c.expectations.size()) {
      ERRORF(
          r, "Expected %zu uniforms but actually %zu:\n%s", c.expectations.size(), uniforms.size(),
          desc.c_str());
      return;
    }
    for (const auto& [actual, expected] : SkMakeZip(uniforms, c.expectations)) {
      if (actual.name != expected.name) {
        ERRORF(
            r, "Actual uniform name (%s) does not match expected name (%s)", actual.name.c_str(),
            expected.name.c_str());
        return;
      }
      ...
      if (actual.type != expected.type) {
        ERRORF(
            r, "Uniform %s: Actual type (%d) does not match expected type (%d)", name,
            static_cast<int>(actual.type), static_cast<int>(expected.type));
        return;
      }
      if (actual.count != expected.count) {
        ERRORF(
            r, "Uniform %s: Actual count (%d) does not match expected count (%d)", name,
            actual.count, expected.count);
        return;
      }
      if (actual.flags != expected.flags) {
        ERRORF(
            r, "Uniform %s: Actual flags (0x%04x) do not match expected flags (0x%04x)", name,
            actual.flags, expected.flags);
        return;
      }
      if (actual.offset != expected.offset) {
        ERRORF(
            r, "Uniform %s: Actual offset (%zu) does not match expected offset (%zu)", name,
...
static void test_no_main(skiatest::Reporter* r) {
  ...
  if (!check_for_failure(
          r, SkMakeSpan(kValidAttrs, SK_ARRAY_COUNT(kValidAttrs)), kValidStride,
          SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), SkString{}, kValidFSes[0])) {
    return;
  }
  ...
  if (!check_for_failure(
          r, SkMakeSpan(kValidAttrs, SK_ARRAY_COUNT(kValidAttrs)), kValidStride,
          SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), kHelper, kValidFSes[0])) {
    return;
  }
  ...
  if (!check_for_failure(
          r, SkMakeSpan(kValidAttrs, SK_ARRAY_COUNT(kValidAttrs)), kValidStride,
          SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), kValidVS, SkString{})) {
    return;
  }
  ...
  if (!check_for_failure(
          r, SkMakeSpan(kValidAttrs, SK_ARRAY_COUNT(kValidAttrs)), kValidStride,
          SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), kValidVS, kHelper)) {
    return;
  }
}
...
static void test_zero_attrs(skiatest::Reporter* r) {
  ...
  check_for_failure(
      r, SkSpan<Attribute>(), kValidStride,
      SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), kValidVS, kValidFSes[0]);
}

static void test_zero_varyings(skiatest::Reporter* r) {
  // Varyings are not required.
  check_for_success(
      r, SkMakeSpan(kValidAttrs, SK_ARRAY_COUNT(kValidAttrs)), kValidStride, SkSpan<Varying>(),
      kValidVS, kValidFSes[0]);
}

static void test_bad_strides(skiatest::Reporter* r) {
  // Zero stride
  if (!check_for_failure(
          r, SkMakeSpan(kValidAttrs, SK_ARRAY_COUNT(kValidAttrs)), 0,
          SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), kValidVS, kValidFSes[0])) {
    return;
  }
  ...
  // Unaligned
  if (!check_for_failure(
          r, SkMakeSpan(kValidAttrs, SK_ARRAY_COUNT(kValidAttrs)), kValidStride + 1,
          SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), kValidVS, kValidFSes[0])) {
    return;
  }
  ...
  // Too large
  if (!check_for_failure(
          r, SkMakeSpan(kValidAttrs, SK_ARRAY_COUNT(kValidAttrs)), 1 << 20,
          SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), kValidVS, kValidFSes[0])) {
    return;
  }
}
...
static void test_bad_offsets(skiatest::Reporter* r) {
  {  // offset isn't aligned
    ...
    if (!check_for_failure(
            r, SkMakeSpan(kAttributes, SK_ARRAY_COUNT(kAttributes)), 32,
            SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), kValidVS, kValidFSes[0])) {
      return;
    }
  }
  {  // straddles stride boundary
    ...
    if (!check_for_failure(
            r, SkMakeSpan(kAttributes, SK_ARRAY_COUNT(kAttributes)), 20,
            SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), kValidVS, kValidFSes[0])) {
      return;
    }
  }
  {  // straddles stride boundary with attempt to overflow
    ...
    if (!check_for_failure(
            r, SkMakeSpan(kAttributes, SK_ARRAY_COUNT(kAttributes)), 4,
            SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), kValidVS, kValidFSes[0])) {
      return;
    }
  }
...
static void test_too_many_attrs(skiatest::Reporter* r) {
  ...
  check_for_failure(
      r, SkMakeSpan(attrs), 4 * 4, SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)),
      kValidVS, kValidFSes[0]);
}
...
static void test_too_many_varyings(skiatest::Reporter* r) {
  ...
  check_for_failure(
      r, SkMakeSpan(kValidAttrs, SK_ARRAY_COUNT(kValidAttrs)), kValidStride, SkMakeSpan(varyings),
      kValidVS, kValidFSes[0]);
}
...
static void test_duplicate_attribute_names(skiatest::Reporter* r) {
  static const Attribute kAttributes[]{
      ...
      {Attribute::Type::kFloat2, 16, SkString{"var"}}};
  check_for_failure(
      r, SkMakeSpan(kAttributes, SK_ARRAY_COUNT(kAttributes)), 24,
      SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), kValidVS, kValidFSes[0]);
}

static void test_duplicate_varying_names(skiatest::Reporter* r) {
  static const Varying kVaryings[]{
      {Varying::Type::kFloat4, SkString{"var"}}, {Varying::Type::kFloat3, SkString{"var"}}};
  check_for_failure(
      r, SkMakeSpan(kValidAttrs, SK_ARRAY_COUNT(kValidAttrs)), kValidStride,
      SkMakeSpan(kVaryings, SK_ARRAY_COUNT(kVaryings)), kValidVS, kValidFSes[0]);
}
...
static void test_sneaky_attribute_name(skiatest::Reporter* r) {
  ...
  check_for_failure(
      r, SkMakeSpan(kAttributes, SK_ARRAY_COUNT(kAttributes)), 16,
      SkMakeSpan(kValidVaryings, SK_ARRAY_COUNT(kValidVaryings)), kValidVS, kValidFSes[0]);
}
