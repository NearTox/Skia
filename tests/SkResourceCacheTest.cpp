...
static void check_data(
    skiatest::Reporter* reporter, const SkCachedData* data, int refcnt, CachedState cacheState,
...
static void testBitmapCache_discarded_bitmap(
    skiatest::Reporter* reporter, SkResourceCache* cache,
...
static void test_discarded_image(
    skiatest::Reporter* reporter, const SkMatrix& transform, sk_sp<SkImage> (*buildImage)()) {
...
struct TestRec : SkResourceCache::Rec {
  ...
  void postAddInstall(void*) override { *fFlags |= kDidInstall; }
  const char* getCategory() const override { return "test-category"; }
};

static void test_duplicate_add(
    SkResourceCache* cache, skiatest::Reporter* reporter, bool purgable) {
  int sharedID = 1;
  int data = 0;
