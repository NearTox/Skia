...
static void check_data(
    skiatest::Reporter* reporter, SkCachedData* data, int refcnt, CachedState cacheState,
...
DEF_TEST(YUVPlanesCache, reporter) {
  ...
  SkYUVAInfo yuvaInfo(
      {5, 5}, SkYUVAInfo::PlaneConfig::kY_U_V, SkYUVAInfo::Subsampling::k420,
      kRec601_Limited_SkYUVColorSpace);
  SkYUVAPixmapInfo yuvaPixmapInfo(
      yuvaInfo, SkYUVAPixmapInfo::DataType::kUnorm8,
      /*rowBytes[]*/ nullptr);
  ...
  for (int i = 0; i < yuvaPixmaps.numPlanes(); ++i) {
    ...
    REPORTER_ASSERT(
        reporter, yuvaPixmaps.plane(i).rowBytes() == yuvaPixmapsRead.plane(i).rowBytes());
  }
