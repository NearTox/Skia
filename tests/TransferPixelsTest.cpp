...
void fill_transfer_data(
    int left, int top, int width, int height, int bufferWidth, GrColorType dstType, char* dst) {
  ...
  for (int j = top; j < top + height; ++j) {
    for (int i = left; i < left + width; ++i) {
      ...
      uint32_t srcPixel = GrColorPackRGBA(red - (red >> 8), green - (green >> 8), 0xff, 0xff);
...
bool read_pixels_from_texture(GrTexture* texture, GrColorType dstColorType, char* dst) {
  ...
  GrCaps::SupportedRead supportedRead =
      caps->supportedReadPixelsColorType(srcCT, texture->backendFormat(), dstColorType);
  if (supportedRead.fColorType != dstColorType || supportedRead.fSwizzle != GrSwizzle("rgba")) {
    ...
    if (!gpu->readPixels(
            texture, 0, 0, w, h, supportedRead.fColorType, tmpPixels.get(), tmpRowBytes)) {
      return false;
    }
    ...
    return GrConvertPixels(
        dstInfo, dst, rowBytes, tmpInfo, tmpPixels.get(), tmpRowBytes, false,
...
void basic_transfer_to_test(
  ...
  fill_transfer_data(
      0, 0, kTextureWidth, kTextureHeight, srcBufferWidth, allowedSrc, srcData.get());
  ...
  result = gpu->transferPixelsTo(
      tex.get(), 0, 0, kTextureWidth, kTextureHeight, allowedSrc, buffer.get(), 0, srcRowBytes);
  ...
  compare_pixels(
      srcInfo, srcData.get(), srcRowBytes, dstInfo, dstBuffer.get(), dstRowBytes, kTol, error);
  ...
  result = gpu->transferPixelsTo(
      tex.get(), kLeft, kTop, kWidth, kHeight, allowedSrc, buffer.get(), offset, srcRowBytes);
  REPORTER_ASSERT(reporter, result);
...
void basic_transfer_from_test(
  ...
  fill_transfer_data(
      0, 0, kTextureWidth, kTextureHeight, kTextureWidth, colorType, textureData.get());
  GrMipLevel data;
  data.fPixels = textureData.get();
  data.fRowBytes = textureDataRowBytes;
  sk_sp<GrTexture> tex = resourceProvider->createTexture(
      desc, renderable, 1, SkBudgeted::kNo, GrProtected::kNo, &data, 1);
  if (!tex) {
    return;
  }
  ...
  GrPixelInfo readInfo(
      allowedRead.fColorType, kPremul_SkAlphaType, nullptr, kTextureWidth, kTextureHeight);
  ...
  bool result = gpu->transferPixelsFrom(
      tex.get(), 0, 0, kTextureWidth, kTextureHeight, allowedRead.fColorType, buffer.get(), 0);
  if (!result) {
  ...
  if (allowedRead.fSwizzle != GrSwizzle("rgba")) {
    GrConvertPixels(
        transferInfo, transferData.get(), fullBufferRowBytes, transferInfo, transferData.get(),
        fullBufferRowBytes, false, allowedRead.fSwizzle);
  }
  ...
  GrPixelInfo textureDataInfo(
      colorType, kPremul_SkAlphaType, nullptr, kTextureWidth, kTextureHeight);
  compare_pixels(
      textureDataInfo, textureData.get(), textureDataRowBytes, transferInfo, transferData.get(),
      fullBufferRowBytes, kTol, error);

  ///////////////////////
  // Now test a partial read at an offset into the buffer.
  result = gpu->transferPixelsFrom(
      tex.get(), kPartialLeft, kPartialTop, kPartialWidth, kPartialHeight, allowedRead.fColorType,
  ...
  compare_pixels(
      ...
      partialBufferRowBytes, kTol, error);
#if GR_GPU_STATS
