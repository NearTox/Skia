...
bool read_pixels_from_texture(
  ...
  if (supportedRead.fColorType != colorType) {
    ...
    if (!gpu->readPixels(
            texture, 0, 0, w, h, colorType, supportedRead.fColorType, tmpPixels.get(),
            tmpRowBytes)) {
      return false;
    }
...
void basic_transfer_to_test(
  ...
  fill_transfer_data(
      0, 0, kTexDims.fWidth, kTexDims.fHeight, srcBufferWidth, allowedSrc.fColorType,
      srcData.get());
  ...
  result = gpu->transferPixelsTo(
      tex.get(), 0, 0, kTexDims.fWidth, kTexDims.fHeight, colorType, allowedSrc.fColorType,
  ...
  fill_transfer_data(
      left, top, width, height, srcBufferWidth, allowedSrc.fColorType, srcData.get());
  data = buffer->map();
  memcpy(data, srcData.get(), size);
  buffer->unmap();

  result = gpu->transferPixelsTo(
      tex.get(), left, top, width, height, colorType, allowedSrc.fColorType, buffer.get(), offset,
      srcRowBytes);
  if (!result) {
    gpu->transferPixelsTo(
        tex.get(), left, top, width, height, colorType, allowedSrc.fColorType, buffer.get(), offset,
        srcRowBytes);
    ERRORF(
...
void basic_transfer_from_test(
  ...
  fill_transfer_data(
      0, 0, kTexDims.fWidth, kTexDims.fHeight, kTexDims.fHeight, colorType, textureData.get());
  GrMipLevel data;
  ...
  bool result = gpu->transferPixelsFrom(
      tex.get(), 0, 0, kTexDims.fWidth, kTexDims.fHeight, colorType, allowedRead.fColorType,
      buffer.get(), 0);
  if (!result) {
  ...
  result = gpu->transferPixelsFrom(
      tex.get(), kPartialLeft, kPartialTop, kPartialWidth, kPartialHeight, colorType,
      allowedRead.fColorType, buffer.get(), partialReadOffset);
  if (!result) {
    ERRORF(reporter, "transferPixelsFrom failed.");
    return;
  }
