...
void fill_transfer_data(
    int left, int top, int width, int height, int rowBytes, GrColorType dstType, char* dst) {
  ...
  for (int j = top; j < top + height; ++j) {
    for (int i = left; i < left + width; ++i) {
      ...
      GrConvertPixels(
          GrPixmap(dstInfo, dstLocation(i, j), dstBpp), GrPixmap(srcInfo, &srcPixel, 4));
    }
  }
}
...
bool read_pixels_from_texture(
    GrTexture* texture, GrColorType colorType, char* dst, float tolerances[4]) {
  ...
  if (supportedRead.fColorType != colorType) {
    ...
    if (!gpu->readPixels(
            texture, SkIRect::MakeWH(w, h), colorType, supportedRead.fColorType, tmpPixels.get(),
            tmpRowBytes)) {
      return false;
    }
    ...
    return GrConvertPixels(
        GrPixmap(dstInfo, dst, rowBytes), GrPixmap(tmpInfo, tmpPixels.get(), tmpRowBytes));
  }
  return gpu->readPixels(
      texture, SkIRect::MakeWH(w, h), colorType, supportedRead.fColorType, dst, rowBytes);
}
...
void basic_transfer_to_test(
  ...
  fill_transfer_data(
      0, 0, kTexDims.fWidth, kTexDims.fHeight, srcRowBytes, allowedSrc.fColorType, srcData.get());

  // create and fill transfer buffer
  size_t size = srcRowBytes * kBufferHeight;
  sk_sp<GrGpuBuffer> buffer(resourceProvider->createBuffer(
      size, GrGpuBufferType::kXferCpuToGpu, kDynamic_GrAccessPattern));
  if (!buffer) {
    return;
  }
  ...
  bool result;
  result = gpu->transferPixelsTo(
      tex.get(), SkIRect::MakeSize(kTexDims), colorType, allowedSrc.fColorType, buffer, 0,
      srcRowBytes);
  ...
  if (!result) {
    ERRORF(
        reporter, "Could not read pixels from texture, color type: %d",
  ...
  ComparePixels(
      GrCPixmap(srcInfo, srcData.get(), srcRowBytes),
      GrCPixmap(dstInfo, dstBuffer.get(), dstRowBytes), compareTolerances, error);
  ...
  fill_transfer_data(left, top, width, height, srcRowBytes, allowedSrc.fColorType, srcData.get());
  data = buffer->map();
  memcpy(data, srcData.get(), size);
  buffer->unmap();

  result = gpu->transferPixelsTo(
      tex.get(), SkIRect::MakeXYWH(left, top, width, height), colorType, allowedSrc.fColorType,
      buffer, offset, srcRowBytes);
  if (!result) {
    ERRORF(
        reporter, "Could not transfer pixels to texture, color type: %d",
        static_cast<int>(colorType));
    return;
  }
...
void basic_transfer_from_test(
  ...
  fill_transfer_data(
      0, 0, kTexDims.fWidth, kTexDims.fHeight, textureDataRowBytes, colorType, textureData.get());
  GrMipLevel data;
  ...
  size_t partialReadOffset =
      kStartingOffset + (offsetAlignment - kStartingOffset % offsetAlignment) % offsetAlignment;
  ...
  //////////////////////////
  // transfer full data
  bool result = gpu->transferPixelsFrom(
      tex.get(), SkIRect::MakeSize(kTexDims), colorType, allowedRead.fColorType, buffer, 0);
  if (!result) {
  ...
  ComparePixels(
      GrCPixmap(textureDataInfo, textureData.get(), textureDataRowBytes),
      GrCPixmap(transferInfo, transferData.get(), fullBufferRowBytes), tol, error);

  ///////////////////////
  // Now test a partial read at an offset into the buffer.
  result = gpu->transferPixelsFrom(
      tex.get(), SkIRect::MakeXYWH(kPartialLeft, kPartialTop, kPartialWidth, kPartialHeight),
      colorType, allowedRead.fColorType, buffer, partialReadOffset);
  if (!result) {
    ERRORF(reporter, "transferPixelsFrom failed.");
    return;
  }
  ...
  ComparePixels(
      GrCPixmap(textureDataInfo, textureDataStart, textureDataRowBytes),
      GrCPixmap(transferInfo, transferData.get(), partialBufferRowBytes), tol, error);
#if GR_GPU_STATS
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(TransferPixelsToTextureTest, reporter, ctxInfo) {
  ...
  for (auto renderable : {GrRenderable::kNo, GrRenderable::kYes}) {
    for (auto colorType : {
             GrColorType::kAlpha_8,        GrColorType::kBGR_565,
             GrColorType::kABGR_4444,      GrColorType::kRGBA_8888,
             GrColorType::kRGBA_8888_SRGB, GrColorType::kRGB_888x,
             GrColorType::kRG_88,          GrColorType::kBGRA_8888,
             GrColorType::kRGBA_1010102,   GrColorType::kBGRA_1010102,
             GrColorType::kGray_8,         GrColorType::kAlpha_F16,
             GrColorType::kRGBA_F16,       GrColorType::kRGBA_F16_Clamped,
             GrColorType::kRGBA_F32,       GrColorType::kAlpha_16,
             GrColorType::kRG_1616,        GrColorType::kRGBA_16161616,
             GrColorType::kRG_F16,
         }) {
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(TransferPixelsFromTextureTest, reporter, ctxInfo) {
  ...
  for (auto renderable : {GrRenderable::kNo, GrRenderable::kYes}) {
    for (auto colorType : {
             GrColorType::kAlpha_8,      GrColorType::kAlpha_16,     GrColorType::kBGR_565,
             GrColorType::kABGR_4444,    GrColorType::kRGBA_8888,    GrColorType::kRGBA_8888_SRGB,
             GrColorType::kRGB_888x,     GrColorType::kRG_88,        GrColorType::kBGRA_8888,
             GrColorType::kRGBA_1010102, GrColorType::kBGRA_1010102, GrColorType::kGray_8,
             GrColorType::kAlpha_F16,    GrColorType::kRGBA_F16,     GrColorType::kRGBA_F16_Clamped,
             GrColorType::kRGBA_F32,     GrColorType::kRG_1616,      GrColorType::kRGBA_16161616,
             GrColorType::kRG_F16,
         }) {
