...
DEF_TEST(SkColorSpaceXformSteps, r) {
  auto srgb = SkColorSpace::MakeSRGB(),
       ...
       srgb1 = srgb->makeLinearGamma(), adobe1 = adobe->makeLinearGamma();

  auto premul = kPremul_SkAlphaType, opaque = kOpaque_SkAlphaType, unpremul = kUnpremul_SkAlphaType;
  ...
  for (auto t : tests) {
    ...
    uint32_t bits = (uint32_t)t.unpremul << 0 | (uint32_t)t.linearize << 1 |
                    (uint32_t)t.gamut_transform << 2 | (uint32_t)t.encode << 3 |
                    (uint32_t)t.premul << 4;
    tested |= (1 << bits);
  }
  ...
  for (uint32_t t = 0; t < 32; t++) {
    ...
    ERRORF(
        r, "{ xxx, yyy, at, %s,%s,%s,%s,%s }, not covered", (t & 1) ? " true" : "false",
        (t & 2) ? " true" : "false", (t & 4) ? " true" : "false", (t & 8) ? " true" : "false",
        (t & 16) ? " true" : "false");
  }
}
