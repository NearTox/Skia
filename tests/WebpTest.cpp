...
DEF_TEST(WebpCodecBlend, r) {
  ...
  for (int i = 0; i < codec->getFrameCount(); i++) {
    ...
    if (result != SkCodec::kSuccess) {
      ERRORF(
          r, "Failed to decode %s frame %i (premul) - error %s", path, i,
    ...
    auto alphaType =
        frameInfo.fAlphaType == kOpaque_SkAlphaType ? kOpaque_SkAlphaType : kPremul_SkAlphaType;
    result = codec->getPixels(
        premulInfo.makeAlphaType(alphaType), changeBm.getPixels(), changeBm.rowBytes(), &options);
    if (result != SkCodec::kSuccess) {
      ERRORF(
          r, "Failed to decode %s frame %i (change) - error %s", path, i,
          SkCodec::ResultToString(result));
      return;
    }

    REPORTER_ASSERT(
        r, ToolUtils::equal_pixels(premulBm, changeBm),
        "%s frame %i does not match"
        " with mismatched SkAlphaType",
        path, i);
  }
}
