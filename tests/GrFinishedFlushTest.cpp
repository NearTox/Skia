...
static void busy_wait_for_callback(
    int* count, int expectedValue, GrContext* ctx, skiatest::Reporter* reporter) {
  ...
  if (*count != expectedValue) {
    ERRORF(
        reporter, "Expected count failed to reach %d within 1 second of busy waiting.",
        expectedValue);
  }
}
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(FlushFinishedProcTest, reporter, ctxInfo) {
  ...
  bool expectAsyncCallback =
      ...
      ctx->backend() == GrBackendApi::kDawn || ctx->backend() == GrBackendApi::kDirect3D;
