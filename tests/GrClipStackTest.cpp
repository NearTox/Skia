...
enum class SavePolicy { kNever, kAtStart, kAtEnd, kBetweenEveryOp };
...
class TestCase {
 ...
 private:
  ...
  TestCase(
      SkString name, const SkIRect& deviceBounds, ClipStack::ClipState expectedState,
      std::vector<ClipStack::Element> actual, std::vector<ClipStack::Element> expected)
      : fName(name),
...
class ElementsBuilder {
 public:
  ...
  ElementsBuilder& localToDevice(const SkMatrix& m) {
    fLocalToDevice = m;
    return *this;
  }
  ElementsBuilder& aa() {
  ...
  ElementsBuilder& nonAA() {
    fAA = GrAA::kNo;
    return *this;
  }
  ElementsBuilder& intersect() {
    fOp = SkClipOp::kIntersect;
    return *this;
  }
  ElementsBuilder& difference() {
    fOp = SkClipOp::kDifference;
    return *this;
  }

  // Add rect, rrect, or paths to the list of elements, possibly overriding the last set
  // matrix, aa, and op state.
  ElementsBuilder& rect(const SkRect& rect) { return this->rect(rect, fLocalToDevice, fAA, fOp); }
  ...
  ElementsBuilder& path(const SkPath& path) { return this->path(path, fLocalToDevice, fAA, fOp); }
  ...
  TestCaseBuilder& finishElements() { return *fBuilder; }

 private:
  friend class TestCaseBuilder;

  ElementsBuilder(TestCaseBuilder* builder, std::vector<ClipStack::Element>* elements)
      : fBuilder(builder), fElements(elements) {}
...
class TestCaseBuilder {
 public:
  ...
  TestCase finishTest() {
    TestCase test(
        fName, fDeviceBounds, fExpectedState, std::move(fActualElements),
        std::move(fExpectedElements));
 ...
 private:
  ...
  explicit TestCaseBuilder(const char* name, const SkIRect& deviceBounds)
      : fName(name), fDeviceBounds(deviceBounds), fExpectedState(ClipStack::ClipState::kWideOpen) {}
...
SkString TestCase::getTestName(const std::vector<int>& order, SavePolicy policy) const {
  ...
  SkString policyName;
  switch (policy) {
    case SavePolicy::kNever: policyName = "never"; break;
    case SavePolicy::kAtStart: policyName = "start"; break;
    case SavePolicy::kAtEnd: policyName = "end"; break;
    case SavePolicy::kBetweenEveryOp: policyName = "between"; break;
  }
...
std::pair<SkIRect, bool> TestCase::getOptimalBounds() const {
  ...
  for (const ClipStack::Element& e : fExpectedElements) {
    ...
    if (intersect) {
      ...
      elementBounds = GrClip::GetPixelIBounds(
          e.fLocalToDevice.mapRect(e.fShape.bounds()), e.fAA, GrClip::BoundsType::kExterior);
    } else {
      op = SkRegion::kDifference_Op;
      expectOptimal = false;
      if (e.fShape.isRect() && e.fLocalToDevice.isIdentity()) {
        elementBounds =
            GrClip::GetPixelIBounds(e.fShape.rect(), e.fAA, GrClip::BoundsType::kInterior);
      } else if (e.fShape.isRRect() && e.fLocalToDevice.isIdentity()) {
        elementBounds = GrClip::GetPixelIBounds(
            SkRRectPriv::InnerBounds(e.fShape.rrect()), e.fAA, GrClip::BoundsType::kInterior);
      } else {
...
static bool compare_elements(
    const skgpu::v1::ClipStack::Element& a, const skgpu::v1::ClipStack::Element& b) {
  if (a.fAA != b.fAA || a.fOp != b.fOp || a.fLocalToDevice != b.fLocalToDevice ||
      a.fShape.type() != b.fShape.type()) {
    return false;
  }
  switch (a.fShape.type()) {
    case GrShape::Type::kRect: return a.fShape.rect() == b.fShape.rect();
    case GrShape::Type::kRRect: return a.fShape.rrect() == b.fShape.rrect();
    ...
    default: SkDEBUGFAIL("Shape type not handled by test case yet."); return false;
  }
}

void TestCase::run(
    const std::vector<int>& order, SavePolicy policy, skiatest::Reporter* reporter) const {
  ...
  for (int i : order) {
    ...
    switch (e.fShape.type()) {
      ...
      case GrShape::Type::kPath:
        ...
        break;
      default: SkDEBUGFAIL("Shape type not handled by test case yet.");
    }
  }
  ...
  REPORTER_ASSERT(
      reporter, cs.clipState() == fExpectedState, "%s, clip state expected %d, actual %d",
  ...
  if (expectOptimal) {
    REPORTER_ASSERT(
        reporter, actualBounds == optimalBounds,
        "%s, bounds expected [%d %d %d %d], actual [%d %d %d %d]", name.c_str(),
        optimalBounds.fLeft, optimalBounds.fTop, optimalBounds.fRight, optimalBounds.fBottom,
        actualBounds.fLeft, actualBounds.fTop, actualBounds.fRight, actualBounds.fBottom);
  } else {
    REPORTER_ASSERT(
        reporter, actualBounds.contains(optimalBounds),
        "%s, bounds are not conservative, optimal [%d %d %d %d], actual [%d %d %d %d]",
        name.c_str(), optimalBounds.fLeft, optimalBounds.fTop, optimalBounds.fRight,
        optimalBounds.fBottom, actualBounds.fLeft, actualBounds.fTop, actualBounds.fRight,
        actualBounds.fBottom);
  }
  ...
  for (const ClipStack::Element& a : cs) {
    ...
    REPORTER_ASSERT(
        reporter, found, "%s, unexpected clip element in stack: shape %d, aa %d, op %d",
        name.c_str(), (int)a.fShape.type(), (int)a.fAA, (int)a.fOp);
    matchedElements += found ? 1 : 0;
  }
  REPORTER_ASSERT(
      reporter, matchedElements == fExpectedElements.size(),
      "%s, did not match all expected elements: expected %d but matched only %d", name.c_str(),
      fExpectedElements.size(), matchedElements);
  ...
  if (policy == SavePolicy::kAtEnd) {
    ...
    REPORTER_ASSERT(
        reporter, cs.clipState() == ClipStack::ClipState::kWideOpen,
        "%s, restore should make stack become wide-open, not %d", (int)cs.clipState());
  }
}
...
static void run_test_case(skiatest::Reporter* r, const TestCase& test) {
  ...
  auto runTest = [&]() {
    static const SavePolicy kPolicies[] = {
        SavePolicy::kNever, SavePolicy::kAtStart, SavePolicy::kAtEnd, SavePolicy::kBetweenEveryOp};
...
class NoOp : public GrDrawOp {
 ...
 private:
  ...
  void onPrePrepare(
      GrRecordingContext*, const GrSurfaceProxyView&, GrAppliedClip*, const GrDstProxyView&,
      GrXferBarrierFlags, GrLoadOp) override {}
...
DEF_TEST(ClipStack_RectRectAACombine, r) {
  ...
  SkRect fracRect2 = {
      fracRect1.fLeft + 0.75f * fracRect1.width(), fracRect1.fTop + 0.75f * fracRect1.height(),
  ...
  run_test_case(
      r, TestCase::Build("aa", kDeviceBounds)
             .actual()
             .aa()
             .intersect()
             .rect(fracRect1)
  ...
  run_test_case(
      r, TestCase::Build("nonaa", kDeviceBounds)
             .actual()
             .nonAA()
             .intersect()
             .rect(fracRect1)
  ...
  run_test_case(
      r, TestCase::Build("aligned-aa+nonaa", kDeviceBounds)
             .actual()
             .intersect()
             .aa()
             .rect(pixelAligned)
  ...
  run_test_case(
      r, TestCase::Build("aa+aligned-nonaa", kDeviceBounds)
             .actual()
             .intersect()
             .aa()
             .rect(fracRect1)
  ...
  run_test_case(
      r, TestCase::Build("aa+nonaa", kDeviceBounds)
             .actual()
             .intersect()
             .aa()
             .rect(fracRect1)
...
DEF_TEST(ClipStack_DifferenceNoCombine, r) {
  ...
  run_test_case(
      r, TestCase::Build("no-combine", kDeviceBounds)
             .actual()
             .aa()
             .intersect()
             .rect(r1)
...
DEF_TEST(ClipStack_RectRectNonAxisAligned, r) {
  ...
  run_test_case(
      r, TestCase::Build("local-aa", kDeviceBounds)
             .actual()
             .intersect()
             .localToDevice(lm)
             .aa()
...
DEF_TEST(ClipStack_RRectRRectAACombine, r) {
  ...
  run_test_case(
      r, TestCase::Build("local-aa", kDeviceBounds)
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("local-nonaa", kDeviceBounds)
             .actual()
             .nonAA()
             .intersect()
             .localToDevice(lm)
...
DEF_TEST(ClipStack_RectRRectCombine, r) {
  ...
  run_test_case(
      r, TestCase::Build("still-rrect", kDeviceBounds)
             .actual()
             .intersect()
             .aa()
             .rrect(rrect)
  ...
  run_test_case(
      r, TestCase::Build("to-rect", kDeviceBounds)
             .actual()
             .intersect()
             .aa()
             .rrect(rrect)
...
DEF_TEST(ClipStack_RectDeviceClip, r) {
  ...
  SkRect crossesDeviceEdge = {20.f, kDeviceBounds.fTop - 13.2f, kDeviceBounds.fRight + 15.5f, 30.f};
  SkRect insideDevice = {20.f, kDeviceBounds.fTop, kDeviceBounds.fRight, 30.f};

  run_test_case(
      r, TestCase::Build("device-aa-rect", kDeviceBounds)
             .actual()
             .intersect()
             .aa()
             .rect(crossesDeviceEdge)
             .finishElements()
             .expect()
             .intersect()
             .aa()
  ...
  run_test_case(
      r, TestCase::Build("device-nonaa-rect", kDeviceBounds)
             .actual()
             .intersect()
             .nonAA()
             .rect(crossesDeviceEdge)
...
DEF_TEST(ClipStack_ShapeDeviceBoundsClip, r) {
  ...
  // RRect
  run_test_case(
      r, TestCase::Build("device-rrect", kDeviceBounds)
             .actual()
             .intersect()
             .aa()
  ...
  // Path
  run_test_case(
      r, TestCase::Build("device-path", kDeviceBounds)
             .actual()
             .intersect()
             .aa()
...
DEF_TEST(ClipStack_PathSimplify, r) {
  ...
  SkPath empty;
  run_test_case(
      r, TestCase::Build("empty", kDeviceBounds)
             .actual()
             .path(empty)
             .finishElements()
  ...
  run_test_case(
      r, TestCase::Build("point", kDeviceBounds)
             .actual()
             .path(point)
             .finishElements()
             .state(ClipState::kEmpty)
  ...
  run_test_case(
      r, TestCase::Build("line", kDeviceBounds)
             .actual()
             .path(line)
             .finishElements()
             .state(ClipState::kEmpty)
  ...
  run_test_case(
      r, TestCase::Build("rect", kDeviceBounds)
             .actual()
             .path(rectPath)
             .finishElements()
             .expect()
  ...
  run_test_case(
      r, TestCase::Build("oval", kDeviceBounds)
             .actual()
             .path(ovalPath)
             .finishElements()
             .expect()
  ...
  run_test_case(
      r, TestCase::Build("rrect", kDeviceBounds)
             .actual()
             .path(rrectPath)
             .finishElements()
             .expect()
...
DEF_TEST(ClipStack_RepeatElement, r) {
  ...
  run_test_case(
      r, TestCase::Build("same-rects", kDeviceBounds)
             .actual()
             .rect(rect)
             .rect(rect)
             .rect(rect)
  ...
  run_test_case(
      r, TestCase::Build("same-local-rects", kDeviceBounds)
             .actual()
             .localToDevice(lm)
             .rect(rect)
             .rect(rect)
  ...
  run_test_case(
      r, TestCase::Build("same-rrects", kDeviceBounds)
             .actual()
             .rrect(rrect)
             .rrect(rrect)
             .rrect(rrect)
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("same-local-rrects", kDeviceBounds)
             .actual()
             .localToDevice(lm)
             .rrect(rrect)
             .rrect(rrect)
  ...
  // Same convex path, by ==
  run_test_case(
      r, TestCase::Build("same-convex", kDeviceBounds)
             .actual()
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("same-local-convex", kDeviceBounds)
             .actual()
             .localToDevice(lm)
             .path(make_octagon(rect))
             .path(make_octagon(rect))
  ...
  run_test_case(
      r, TestCase::Build("same-path", kDeviceBounds)
             .actual()
             .path(path)
             .path(path)
             .path(path)
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("same-local-path", kDeviceBounds)
             .actual()
             .localToDevice(lm)
             .path(path)
             .path(path)
...
DEF_TEST(ClipStack_InverseFilledPath, r) {
  ...
  run_test_case(
      r, TestCase::Build("inverse-rect-intersect", kDeviceBounds)
             .actual()
             .aa()
             .intersect()
             .path(inverseRectPath)
             .finishElements()
             .expect()
             .aa()
             .difference()
  ...
  run_test_case(
      r, TestCase::Build("inverse-rect-difference", kDeviceBounds)
             .actual()
             .aa()
             .difference()
             .path(inverseRectPath)
  ...
  run_test_case(
      r, TestCase::Build("inverse-path-intersect", kDeviceBounds)
             .actual()
             .aa()
             .intersect()
             .path(inverseComplexPath)
             .finishElements()
             .expect()
             .aa()
             .difference()
  ...
  run_test_case(
      r, TestCase::Build("inverse-path-difference", kDeviceBounds)
             .actual()
             .aa()
             .difference()
             .path(inverseComplexPath)
...
DEF_TEST(ClipStack_Offscreen, r) {
  ...
  SkRect offscreenRect = {
      kDeviceBounds.fRight + 10.f, kDeviceBounds.fTop + 20.f, kDeviceBounds.fRight + 40.f,
      kDeviceBounds.fTop + 60.f};
  ...
  // Intersect -> empty
  run_test_case(
      r, TestCase::Build("intersect-combo", kDeviceBounds)
             .actual()
             .aa()
             .intersect()
             .rect(offscreenRect)
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("intersect-rect", kDeviceBounds)
             .actual()
             .aa()
             .intersect()
             .rect(offscreenRect)
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("intersect-rrect", kDeviceBounds)
             .actual()
             .aa()
             .intersect()
             .rrect(offscreenRRect)
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("intersect-path", kDeviceBounds)
             .actual()
             .aa()
             .intersect()
             .path(offscreenPath)
  ...
  run_test_case(
      r, TestCase::Build("difference-combo", kDeviceBounds)
             .actual()
             .aa()
             .difference()
             .rect(offscreenRect)
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("difference-rect", kDeviceBounds)
             .actual()
             .aa()
             .difference()
             .rect(offscreenRect)
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("difference-rrect", kDeviceBounds)
             .actual()
             .aa()
             .difference()
             .rrect(offscreenRRect)
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("difference-path", kDeviceBounds)
             .actual()
             .aa()
             .difference()
             .path(offscreenPath)
...
DEF_TEST(ClipStack_EmptyShape, r) {
  ...
  // Intersect -> empty
  run_test_case(
      r, TestCase::Build("empty-intersect", kDeviceBounds)
             .actual()
             .intersect()
  ...
  // Difference -> no-op
  run_test_case(
      r, TestCase::Build("empty-difference", kDeviceBounds)
             .actual()
             .difference()
             .rect(SkRect::MakeEmpty())
             .finishElements()
  ...
  run_test_case(
      r, TestCase::Build("noop-difference", kDeviceBounds)
             .actual()
             .difference()
             .rrect(rrect)
             .rect(SkRect::MakeEmpty())
...
DEF_TEST(ClipStack_DifferenceBounds, r) {
  ...
  run_test_case(
      r, TestCase::Build("difference-cut", kDeviceBounds)
             .actual()
             .nonAA()
             .difference()
             .rect(rightSide)
...
DEF_TEST(ClipStack_NoDifferenceInterference, r) {
  ...
  run_test_case(
      r, TestCase::Build("cross-diff-combine", kDeviceBounds)
             .actual()
             .rect(intR1, GrAA::kYes, SkClipOp::kIntersect)
             .rect(diff, GrAA::kYes, SkClipOp::kDifference)
             .rect(intR2, GrAA::kYes, SkClipOp::kIntersect)
             .finishElements()
             .expect()
             .rect(intCombo, GrAA::kYes, SkClipOp::kIntersect)
             .rect(diff, GrAA::kYes, SkClipOp::kDifference)
             .finishElements()
             .state(ClipState::kComplex)
             .finishTest());
...
DEF_TEST(ClipStack_MultiplePaths, r) {
  ...
  run_test_case(r, b.expectActual().state(ClipState::kComplex).finishTest());
...
DEF_TEST(ClipStack_DeviceRect, r) {
  ...
  run_test_case(
      r, TestCase::Build("device-rect", kDeviceBounds)
             .actual()
             .intersect()
             .aa()
             .rect(rect)
  ...
  run_test_case(
      r, TestCase::Build("unaligned-rect", kDeviceBounds)
             .actual()
             .localToDevice(lm)
             .intersect()
             .aa()
  ...
  run_test_case(
      r, TestCase::Build("diff-rect", kDeviceBounds)
             .actual()
             .difference()
             .aa()
             .rect(rect)
...
DEF_TEST(ClipStack_DeviceRRect, r) {
  ...
  SkMatrix lm = SkMatrix::RotateDeg(15.f);
  run_test_case(
      r, TestCase::Build("unaligned-rrect", kDeviceBounds)
             .actual()
             .localToDevice(lm)
             .intersect()
             .aa()
  ...
  // Not intersect -> kComplex
  run_test_case(
      r, TestCase::Build("diff-rrect", kDeviceBounds)
             .actual()
             .difference()
             .aa()
             .rrect(rrect)
...
DEF_TEST(ClipStack_ScaleTranslate, r) {
  ...
  run_test_case(
      r, TestCase::Build("st+rect", kDeviceBounds)
             .actual()
  ...
  run_test_case(
      r, TestCase::Build("st+rrect", kDeviceBounds)
             .actual()
...
DEF_TEST(ClipStack_ConvexPathContains, r) {
  ...
  run_test_case(
      r, TestCase::Build("convex+rect-intersect", kDeviceBounds)
             .actual()
             .aa()
             .intersect()
             .rect(rect)
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("convex+rrect-intersect", kDeviceBounds)
             .actual()
             .aa()
             .intersect()
             .rrect(rrect)
  ...
  run_test_case(
      r, TestCase::Build("convex+rect-difference", kDeviceBounds)
             .actual()
             .aa()
             .difference()
             .rect(rect)
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("convex+rrect-difference", kDeviceBounds)
             .actual()
             .aa()
             .difference()
             .rrect(rrect)
  ...
  run_test_case(
      r, TestCase::Build("convex-diff+rect-int", kDeviceBounds)
             .actual()
             .aa()
             .intersect()
             .rect(rect)
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("convex-diff+rrect-int", kDeviceBounds)
             .actual()
             .aa()
             .intersect()
             .rrect(rrect)
  ...
  run_test_case(
      r, TestCase::Build("convex-int+rect-diff", kDeviceBounds)
             .actual()
             .aa()
             .intersect()
             .path(bigPath)
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("convex-int+rrect-diff", kDeviceBounds)
             .actual()
             .aa()
             .intersect()
             .path(bigPath)
...
DEF_TEST(ClipStack_NonAxisAlignedContains, r) {
  ...
  run_test_case(
      r, TestCase::Build("rect-rect-ii", kDeviceBounds)
             .actual()
             .rect(bigR, lm1, GrAA::kYes, SkClipOp::kIntersect)
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("rrect-rrect-ii", kDeviceBounds)
             .actual()
             .rrect(bigRR, lm1, GrAA::kYes, SkClipOp::kIntersect)
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("rect-rrect-ii", kDeviceBounds)
             .actual()
             .rect(bigR, lm1, GrAA::kYes, SkClipOp::kIntersect)
             .rrect(smRR, lm2, GrAA::kYes, SkClipOp::kIntersect)
             .finishElements()
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("rrect-rect-ii", kDeviceBounds)
             .actual()
             .rrect(bigRR, lm1, GrAA::kYes, SkClipOp::kIntersect)
             .rect(smR, lm2, GrAA::kYes, SkClipOp::kIntersect)
             .finishElements()
  ...
  run_test_case(
      r, TestCase::Build("rect-rect-dd", kDeviceBounds)
             .actual()
             .rect(bigR, lm1, GrAA::kYes, SkClipOp::kDifference)
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("rrect-rrect-dd", kDeviceBounds)
             .actual()
             .rrect(bigRR, lm1, GrAA::kYes, SkClipOp::kDifference)
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("rect-rrect-dd", kDeviceBounds)
             .actual()
             .rect(bigR, lm1, GrAA::kYes, SkClipOp::kDifference)
             .rrect(smRR, lm2, GrAA::kYes, SkClipOp::kDifference)
             .finishElements()
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("rrect-rect-dd", kDeviceBounds)
             .actual()
             .rrect(bigRR, lm1, GrAA::kYes, SkClipOp::kDifference)
             .rect(smR, lm2, GrAA::kYes, SkClipOp::kDifference)
             .finishElements()
  ...
  run_test_case(
      r, TestCase::Build("rectD-rectI", kDeviceBounds)
             .actual()
             .rect(bigR, lm1, GrAA::kYes, SkClipOp::kDifference)
             .rect(smR, lm2, GrAA::kYes, SkClipOp::kIntersect)
             .finishElements()
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("rrectD-rrectI", kDeviceBounds)
             .actual()
             .rrect(bigRR, lm1, GrAA::kYes, SkClipOp::kDifference)
             .rrect(smRR, lm2, GrAA::kYes, SkClipOp::kIntersect)
             .finishElements()
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("rectD-rrectI", kDeviceBounds)
             .actual()
             .rect(bigR, lm1, GrAA::kYes, SkClipOp::kDifference)
             .rrect(smRR, lm2, GrAA::kYes, SkClipOp::kIntersect)
             .finishElements()
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("rrectD-rectI", kDeviceBounds)
             .actual()
             .rrect(bigRR, lm1, GrAA::kYes, SkClipOp::kDifference)
             .rect(smR, lm2, GrAA::kYes, SkClipOp::kIntersect)
             .finishElements()
  ...
  run_test_case(
      r, TestCase::Build("rectI+rectD", kDeviceBounds)
             .actual()
             .rect(bigR, lm1, GrAA::kYes, SkClipOp::kIntersect)
             .rect(smR, lm2, GrAA::kYes, SkClipOp::kDifference)
             .finishElements()
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("rrectI+rrectD", kDeviceBounds)
             .actual()
             .rrect(bigRR, lm1, GrAA::kYes, SkClipOp::kIntersect)
             .rrect(smRR, lm2, GrAA::kYes, SkClipOp::kDifference)
             .finishElements()
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("rrectI+rectD", kDeviceBounds)
             .actual()
             .rrect(bigRR, lm1, GrAA::kYes, SkClipOp::kIntersect)
             .rect(smR, lm2, GrAA::kYes, SkClipOp::kDifference)
             .finishElements()
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("rectI+rrectD", kDeviceBounds)
             .actual()
             .rect(bigR, lm1, GrAA::kYes, SkClipOp::kIntersect)
             .rrect(smRR, lm2, GrAA::kYes, SkClipOp::kDifference)
             .finishElements()
...
DEF_TEST(ClipStack_MixedAAContains, r) {
  ...
  run_test_case(
      r, TestCase::Build("mixed-outeraa-combine", kDeviceBounds)
             .actual()
             .rect(r1, lm1, GrAA::kYes, SkClipOp::kIntersect)
  ...
  // Vice versa
  run_test_case(
      r, TestCase::Build("mixed-inneraa-combine", kDeviceBounds)
             .actual()
             .rect(r1, lm1, GrAA::kNo, SkClipOp::kIntersect)
  ...
  run_test_case(
      r, TestCase::Build("mixed-outeraa-nocombine", kDeviceBounds)
             .actual()
             .rect(r1, lm1, GrAA::kYes, SkClipOp::kIntersect)
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("mixed-inneraa-nocombine", kDeviceBounds)
             .actual()
             .rect(r1, lm1, GrAA::kNo, SkClipOp::kIntersect)
...
DEF_TEST(ClipStack_ShapeContainsDevice, r) {
  ...
  // Intersect -> no-op
  run_test_case(
      r, TestCase::Build("rect-intersect", kDeviceBounds)
             .actual()
             .intersect()
             .rect(rect)
             .finishElements()
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("rrect-intersect", kDeviceBounds)
             .actual()
             .intersect()
             .rrect(rrect)
             .finishElements()
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("convex-intersect", kDeviceBounds)
             .actual()
             .intersect()
             .path(convex)
             .finishElements()
  ...
  // Difference -> empty
  run_test_case(
      r, TestCase::Build("rect-difference", kDeviceBounds)
             .actual()
             .difference()
             .rect(rect)
             .finishElements()
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("rrect-difference", kDeviceBounds)
             .actual()
             .difference()
             .rrect(rrect)
             .finishElements()
             ...
             .finishTest());
  run_test_case(
      r, TestCase::Build("convex-difference", kDeviceBounds)
             .actual()
             .difference()
             .path(convex)
             .finishElements()
...
DEF_TEST(ClipStack_DisjointShapes, r) {
  ...
  // I+I
  run_test_case(
      r, TestCase::Build("iii", kDeviceBounds)
             .actual()
             .aa()
             .intersect()
             .rect(rt)
  ...
  // D+D
  run_test_case(
      r, TestCase::Build("ddd", kDeviceBounds)
             .actual()
             .nonAA()
             .difference()
             .rect(rt)
  ...
  // I+D from rect
  run_test_case(
      r, TestCase::Build("idd", kDeviceBounds)
             .actual()
             .aa()
             .intersect()
             .rect(rt)
  ...
  // I+D from rrect
  run_test_case(
      r, TestCase::Build("did", kDeviceBounds)
             .actual()
             .aa()
             .intersect()
             .rrect(rr)
  ...
  // I+D from path
  run_test_case(
      r, TestCase::Build("ddi", kDeviceBounds)
             .actual()
             .aa()
             .intersect()
             .path(p)
  ...
  for (ShapeType type : {kRect, kRRect, kConvex}) {
    for (int opBits = 6; opBits < 16; ++opBits) {
      ...
      for (int i = 0; i < 4; ++i) {
        ...
        switch (type) {
          ...
          case kRRect: {
            ...
            if (op == SkClipOp::kIntersect) {
              expectedRRectIntersection =
                  SkRRectPriv::ConservativeIntersect(expectedRRectIntersection, rrect);
      ...
      if (type == kConvex) {
        ...
        if (type == kRect) {
          SkASSERT(
              expectedRectIntersection != SkRect::Make(kDeviceBounds) &&
          ...
          SkASSERT(
              expectedRRectIntersection != SkRRect::MakeRect(SkRect::Make(kDeviceBounds)) &&
...
DEF_TEST(ClipStack_ReplaceClip, r) {
  ...
  REPORTER_ASSERT(
      r, cs.clipState() == ClipStack::ClipState::kDeviceRect, "Clip did not become a device rect");
  ...
  REPORTER_ASSERT(
      r,
      replaceElement.fShape.rect() == SkRect::Make(replace) && replaceElement.fAA == GrAA::kNo &&
  ...
  cs.restore();
  REPORTER_ASSERT(
      r, cs.clipState() == ClipStack::ClipState::kDeviceRRect,
...
DEF_TEST(ClipStack_DiffRects, r) {
  ...
  std::unique_ptr<SurfaceDrawContext> sdc = SurfaceDrawContext::Make(
      ...
      kDeviceBounds.size(), SkSurfaceProps());
  ...
  GrClip::Effect effect =
      cs.apply(context.get(), sdc.get(), NoOp::Get(), GrAAType::kCoverage, &out, &drawBounds);
...
DEF_TEST(ClipStack_ForceAA, r) {
  ...
  REPORTER_ASSERT(r, nonAARectElement.fAA == GrAA::kNo, "Axis-aligned non-AA rect ignores forceAA");
  REPORTER_ASSERT(
      r, nonAARectElement.fShape.rect() == nonAARect, "Mixed AA rects should not combine");
  ...
  REPORTER_ASSERT(r, aaRectElement.fShape.rect() == aaRect, "Mixed AA rects should not combine");
...
DEF_TEST(ClipStack_PreApply, r) {
  ...
  REPORTER_ASSERT(
      r, result.fEffect == GrClip::Effect::kClippedOut, "Offscreen draw is kClippedOut");

  // Intersecting screen with wide-open clip is kUnclipped
  result = cs.preApply({-10.f, -10.f, 10.f, 10.f}, GrAA::kYes);
  REPORTER_ASSERT(
      r, result.fEffect == GrClip::Effect::kUnclipped,
  ...
  REPORTER_ASSERT(
      r, result.fEffect == GrClip::Effect::kClippedOut,
  ...
  REPORTER_ASSERT(
      r, result.fEffect == GrClip::Effect::kUnclipped, "Draw contained within clip is kUnclipped");
  ...
  REPORTER_ASSERT(
      r, result.fEffect == GrClip::Effect::kClipped && !result.fIsRRect,
  ...
  REPORTER_ASSERT(
      r,
      result.fEffect == GrClip::Effect::kClipped && result.fAA == GrAA::kYes && result.fIsRRect &&
...
DEF_TEST(ClipStack_Shader, r) {
  ...
  REPORTER_ASSERT(
      r, effect == GrClip::Effect::kClipped, "apply() should return kClipped for a clip shader");
  REPORTER_ASSERT(
      r, out.hasCoverageFragmentProcessor(),
  ...
  effect = cs.apply(context.get(), sdc.get(), NoOp::Get(), GrAAType::kCoverage, &out2, &drawBounds);
  REPORTER_ASSERT(
      r, effect == GrClip::Effect::kClippedOut,
      "apply() should still discard offscreen draws with a clip shader");

  cs.restore();
  REPORTER_ASSERT(
      r, cs.clipState() == ClipStack::ClipState::kWideOpen,
...
DEF_TEST(ClipStack_SimpleApply, r) {
  ...
  // Draw contained in clip is kUnclipped
  {
    ...
    cs.save();
    cs.clipPath(
        SkMatrix::I(), make_octagon(drawBounds.makeOutset(5.f, 5.f), 5.f, 5.f), GrAA::kYes,
        SkClipOp::kIntersect);
  ...
  // Draw bounds are cropped to device space before checking contains
  {
    ...
    GrAppliedClip out(kDeviceBounds.size());
    GrClip::Effect effect =
        cs.apply(context.get(), sdc.get(), NoOp::Get(), GrAAType::kCoverage, &out, &drawRect);
    REPORTER_ASSERT(
        r, SkRect::Make(kDeviceBounds).contains(drawRect),
        "Draw rect should be clipped to device rect");
    REPORTER_ASSERT(
        r, effect == GrClip::Effect::kUnclipped,
  ...
  // Non-AA device rect intersect is just a scissor
  {
    ...
    REPORTER_ASSERT(
        r, !out.hardClip().windowRectsState().enabled(), "Clip should not need window rects");
    REPORTER_ASSERT(
        r, out.scissorState().enabled() && out.scissorState().rect() == expectedScissor,
  ...
  {
    cs.save();
    cs.clipRect(SkMatrix::I(), {10.2f, 8.342f, 63.f, 23.3f}, GrAA::kYes, SkClipOp::kDifference);
  ...
  // Axis-aligned round rect can be an analytic FP
  {
    ...
    cs.save();
    cs.clipRRect(
        SkMatrix::I(), SkRRect::MakeRectXY(rect, 3.f, 3.f), GrAA::kYes, SkClipOp::kIntersect);
...
DEF_GPUTEST_FOR_CONTEXTS(
    ClipStack_SWMask, sk_gpu_test::GrContextFactory::IsRenderingContext, r, ctxInfo,
  ...
  auto verifyKeys = [&](const std::vector<GrUniqueKey>& expectedKeys,
    ...
    REPORTER_ASSERT(
        r, (int)expectedKeys.size() == numProxies, "Unexpected proxy count, got %d, not %d",
