...
static void TestBitmapSerialization(
  ...
  sk_sp<SkImageFilter> deserializedFilter(TestFlattenableSerialization<SkImageFilter_Base>(
...
static void serialize_and_compare_typeface(
    sk_sp<SkTypeface> typeface, const char* text, const SkSerialProcs* serial_procs,
    const SkDeserialProcs* deserial_procs, skiatest::Reporter* reporter) {
    ...
    skiatest::Reporter* reporter) {
  {
    ...
    if (!typeface) {
      ...
      serialize_and_compare_typeface(
          std::move(typeface), "A!", serial_procs, deserial_procs, reporter);
    }
  }
...
static void TestTypefaceSerialization(skiatest::Reporter* reporter, sk_sp<SkTypeface> typeface) {
  ...
  REPORTER_ASSERT(
      reporter, typeface->countGlyphs() == cloneTypeface->countGlyphs(),
      "Typeface: \"%s\" CloneTypeface: \"%s\"", name.c_str(), cloneName.c_str());
  REPORTER_ASSERT(
      reporter, typeface->fontStyle() == cloneTypeface->fontStyle(),
      "Typeface: \"%s\" CloneTypeface: \"%s\"", name.c_str(), cloneName.c_str());
  ...
  REPORTER_ASSERT(
      reporter, fontMetrics == cloneMetrics, "Typeface: \"%s\" CloneTypeface: \"%s\"", name.c_str(),
      cloneName.c_str());
}
