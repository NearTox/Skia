...
static void test_premul_alpha_roundtrip(skiatest::Reporter* reporter, SkSurface* surf) {
  for (size_t upmaIdx = 0; upmaIdx < SK_ARRAY_COUNT(gUnpremul); ++upmaIdx) {
    ...
    const SkImageInfo info =
        SkImageInfo::Make(256, 256, gUnpremul[upmaIdx].fColorType, kUnpremul_SkAlphaType);
    SkBitmap readBmp1;
...
DEF_TEST(PremulAlphaRoundTripGrConvertPixels, reporter) {
  ...
  for (int y = 0; y < 256 && success; ++y) {
    ...
    for (int x = 0; x < 256 && success; ++x) {
      uint32_t c1 = pixels1[y * 256 + x], c2 = pixels2[y * 256 + x];
...
DEF_TEST(PremulAlphaRoundTripSkConvertPixels, reporter) {
  ...
  auto convert = [](const SkBitmap& dst, const SkBitmap& src) {
    SkAssertResult(SkConvertPixels(
        dst.info(), dst.getAddr(0, 0), dst.rowBytes(), src.info(), src.getAddr(0, 0),
        src.rowBytes()));
  };

  SkBitmap surf;
  surf.allocPixels(pmInfo);
  convert(surf, src);

  SkBitmap read1;
  read1.allocPixels(upmInfo);
  convert(read1, surf);

  SkBitmap surf2;
  surf2.allocPixels(pmInfo);
  convert(surf2, read1);

  SkBitmap read2;
  read2.allocPixels(upmInfo);
  convert(read2, surf2);
