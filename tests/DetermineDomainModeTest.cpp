...
class GrTextureProducer_TestAccess {
 public:
  ...
  static DomainMode DetermineDomainMode(
      const SkRect& constraintRect, GrTextureProducer::FilterConstraint filterConstraint,
    ...
    return GrTextureProducer::DetermineDomainMode(
        constraintRect, filterConstraint, coordsLimitedToConstraintRect, proxy,
        filterModeOrNullForBicubic, domainRect);
  }
};
...
class RectInfo {
 public:
  ...
  void set(
      const SkRect& rect, EdgeType left, EdgeType top, EdgeType right, EdgeType bot,
      const char* name) {
    fRect = rect;
    fTypes[kLeft] = left;
    fTypes[kTop] = top;
  ...
  void print(const char* label) const {
    SkDebugf(
        "%s: %s (%.1f, %.1f, %.1f, %.1f), L: %s T: %s R: %s B: %s\n", label, fName, fRect.fLeft,
        fRect.fTop, fRect.fRight, fRect.fBottom, ToStr(fTypes[kLeft]), ToStr(fTypes[kTop]),
...
static sk_sp<GrTextureProxy> create_proxy(
    GrContext* ctx, bool isPowerOfTwo, bool isExact, RectInfo* rect) {
  ...
  rect->set(
      SkRect::MakeWH(size, size), RectInfo::kHard, RectInfo::kHard,
  ...
  return proxyProvider->createProxy(
      format, {size, size}, swizzle, GrRenderable::kNo, 1, GrMipMapped::kNo, fit, SkBudgeted::kYes,
      GrProtected::kNo);
}

static RectInfo::EdgeType compute_inset_edgetype(
    RectInfo::EdgeType previous, bool isInsetHard, bool coordsAreLimitedToRect, float insetAmount,
...
static const SkRect* generic_inset(
    const RectInfo& enclosing, RectInfo* result, bool isInsetHard, bool areCoordsLimitedToRect,
    float insetAmount, float halfFilterWidth, uint32_t flags, const char* name) {
  ...
  if (flags & kInsetLeft_Flag) {
    ...
    left = compute_inset_edgetype(
        left, isInsetHard, areCoordsLimitedToRect, insetAmount, halfFilterWidth);
  } else {
    left = compute_inset_edgetype(left, isInsetHard, areCoordsLimitedToRect, 0.0f, halfFilterWidth);
  }

  RectInfo::EdgeType top = enclosing.edgeType(RectInfo::kTop);
  if (flags & kInsetTop_Flag) {
    newR.fTop += insetAmount;
    top = compute_inset_edgetype(
        top, isInsetHard, areCoordsLimitedToRect, insetAmount, halfFilterWidth);
  } else {
    top = compute_inset_edgetype(top, isInsetHard, areCoordsLimitedToRect, 0.0f, halfFilterWidth);
  }

  RectInfo::EdgeType right = enclosing.edgeType(RectInfo::kRight);
  if (flags & kInsetRight_Flag) {
    newR.fRight -= insetAmount;
    right = compute_inset_edgetype(
        right, isInsetHard, areCoordsLimitedToRect, insetAmount, halfFilterWidth);
  } else {
    right =
        compute_inset_edgetype(right, isInsetHard, areCoordsLimitedToRect, 0.0f, halfFilterWidth);
  }

  RectInfo::EdgeType bot = enclosing.edgeType(RectInfo::kBot);
  if (flags & kInsetBot_Flag) {
    newR.fBottom -= insetAmount;
    bot = compute_inset_edgetype(
        bot, isInsetHard, areCoordsLimitedToRect, insetAmount, halfFilterWidth);
  } else {
    bot = compute_inset_edgetype(bot, isInsetHard, areCoordsLimitedToRect, 0.0f, halfFilterWidth);
  }
...
static const SkRect* right_only(
    const RectInfo& enclosing, RectInfo* result, bool isInsetHard, bool areCoordsLimitedToRect,
    float insetAmount, float halfFilterWidth) {
...
static const SkRect* full_inset(
    const RectInfo& enclosing, RectInfo* result, bool isInsetHard, bool areCoordsLimitedToRect,
    float insetAmount, float halfFilterWidth) {
...
static const SkRect* no_inset(
    const RectInfo& enclosing, RectInfo* result, bool isInsetHard, bool areCoordsLimitedToRect,
    float insetAmount, float halfFilterWidth) {
  static const char* name = "none";
  return generic_inset(
      enclosing, result, isInsetHard, areCoordsLimitedToRect, insetAmount, halfFilterWidth, 0,
      name);
}
...
static void proxy_test(skiatest::Reporter* reporter, GrContext* context) {
  ...
  static const GrSamplerState::Filter* gModePtrs[] = {&gModes[0], &gModes[1], nullptr, &gModes[2]};
  ...
  for (auto isPowerOfTwoSized : {true, false}) {
    for (auto isExact : {true, false}) {
      ...
      sk_sp<GrTextureProxy> proxy = create_proxy(context, isPowerOfTwoSized, isExact, &outermost);
      ...
      for (auto isConstraintRectHard : {true, false}) {
        for (auto areCoordsLimitedToConstraintRect : {true, false}) {
          for (int filterMode = 0; filterMode < 4; ++filterMode) {
            for (auto constraintRectMaker :
                 {left_only, top_only, right_only, bot_only, full_inset, no_inset}) {
              for (auto insetAmt : {0.25f, 0.75f, 1.25f, 1.75f, 5.0f}) {
                ...
                const SkRect* constraintRect = (*constraintRectMaker)(
                    outermost, &constraintRectStorage, isConstraintRectHard,
                    areCoordsLimitedToConstraintRect, insetAmt, gHalfFilterWidth[filterMode]);
                ...
                actualMode = GrTextureProducer_TestAccess::DetermineDomainMode(
                    *constraintRect,
                    isConstraintRectHard ? GrTextureProducer::kYes_FilterConstraint
                                         : GrTextureProducer::kNo_FilterConstraint,
                    areCoordsLimitedToConstraintRect, proxy.get(), gModePtrs[filterMode],
                    &actualDomainRect);
