...
DEF_TEST(WindowRectangles, reporter) {
  ...
  SkASSERT(wr.count() == GrWindowRectangles::kMaxWindows - 1);
  {
    ...
    REPORTER_ASSERT(
        reporter, !memcmp(A.data(), windowData, GrWindowRectangles::kMaxWindows * sizeof(SkIRect)));
    REPORTER_ASSERT(
        reporter,
        !memcmp(B.data(), windowData, (GrWindowRectangles::kMaxWindows - 1) * sizeof(SkIRect)));
    REPORTER_ASSERT(
        reporter, B.data()[GrWindowRectangles::kMaxWindows - 1] == SkRectPriv::MakeILarge());
  }
  {
    GrWindowRectangles A(wr), B(wr);
    REPORTER_ASSERT(reporter, B == A);
    ...
    REPORTER_ASSERT(reporter, B.data() != A.data());
    REPORTER_ASSERT(
        reporter, !memcmp(B.data(), A.data(), GrWindowRectangles::kMaxWindows * sizeof(SkIRect)));
    REPORTER_ASSERT(
        reporter, !memcmp(A.data(), windowData, GrWindowRectangles::kMaxWindows * sizeof(SkIRect)));
  }
}
