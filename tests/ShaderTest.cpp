...
static void check_isaimage(
    skiatest::Reporter* reporter, SkShader* shader, int expectedW, int expectedH,
    SkTileMode expectedX, SkTileMode expectedY, const SkMatrix& expectedM) {
...
DEF_TEST(ComposeShaderSingle, reporter) {
  ...
  SkPaint p;
  p.setShader(SkShaders::Blend(
      SkBlendMode::kClear, SkShaders::Empty(),
