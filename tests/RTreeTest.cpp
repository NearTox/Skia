...
static void run_queries(
    skiatest::Reporter* reporter, SkRandom& rand, SkRect rects[], const SkRTree& tree) {
...
DEF_TEST(RTree, reporter) {
  ...
  while (tmp > 0) {
    tmp -= static_cast<int>(
        pow(static_cast<double>(SkRTree::kMaxChildren), static_cast<double>(expectedDepthMin + 1)));
    ++expectedDepthMin;
  }
  ...
  while (tmp > 0) {
    tmp -= static_cast<int>(
        pow(static_cast<double>(SkRTree::kMinChildren), static_cast<double>(expectedDepthMax + 1)));
    ++expectedDepthMax;
  }
