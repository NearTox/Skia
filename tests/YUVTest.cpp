...
static void codec_yuv(
    skiatest::Reporter* reporter, const char path[], const SkYUVAInfo* expectedInfo) {
...
DEF_TEST(Jpeg_YUV_Codec, r) {
  auto setExpectations = [](SkISize dims, SkYUVAInfo::Subsampling subsampling) {
    return SkYUVAInfo(
        dims, SkYUVAInfo::PlaneConfig::kY_U_V, subsampling, kJPEG_Full_SkYUVColorSpace,
        kTopLeft_SkEncodedOrigin, SkYUVAInfo::Siting::kCentered, SkYUVAInfo::Siting::kCentered);
  };
...
DEF_TEST(YUVMath, reporter) {
  ...
  for (auto cs : spaces) {
    SkColorMatrix r2ym = SkColorMatrix::RGBtoYUV(cs), y2rm = SkColorMatrix::YUVtoRGB(cs);
