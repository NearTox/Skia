...
template <int N>
static void check_allocator_helper(
    GrTBlockList<C, N>* allocator, int cnt, int popCnt, skiatest::Reporter* reporter) {
...
template <int N>
static void check_iterator_helper(
    GrTBlockList<C, N>* allocator, const std::vector<C*>& expected, skiatest::Reporter* reporter) {
...
static void check_allocator(
  ...
  for (int i = 0; i < cnt; ++i) {
    switch ((ItemInitializer)(i % kInitCount)) {
      case kCopyCtor: allocator->push_back(C(i)); break;
      case kMoveCtor: allocator->push_back(std::move(C(i))); break;
      case kCopyAssign: allocator->push_back() = C(i); break;
      case kMoveAssign: allocator->push_back() = std::move(C(i)); break;
      case kEmplace: allocator->emplace_back(i); break;
    }
...
static void run_reserve_test(skiatest::Reporter* reporter) {
  ...
  REPORTER_ASSERT(
      reporter,
      (size_t)list.allocator()->testingOnly_scratchBlockSize() >= extraReservedCount * sizeof(C));
  ...
  REPORTER_ASSERT(
      reporter,
      (size_t)list.allocator()->testingOnly_scratchBlockSize() >= kItemsPerBlock * sizeof(C));
