...
static sk_sp<GrTextureProxy> deferred_tex(
    skiatest::Reporter* reporter, GrRecordingContext* rContext, GrProxyProvider* proxyProvider,
    SkBackingFit fit) {
  ...
  sk_sp<GrTextureProxy> proxy = proxyProvider->createProxy(
      format, kSize, GrRenderable::kNo, 1, GrMipmapped::kNo, fit, SkBudgeted::kYes,
      GrProtected::kNo);
...
static sk_sp<GrTextureProxy> deferred_texRT(
  ...
  sk_sp<GrTextureProxy> proxy = proxyProvider->createProxy(
      format, kSize, GrRenderable::kYes, 1, GrMipmapped::kNo, fit, SkBudgeted::kYes,
      GrProtected::kNo);
  // Only budgeted & wrapped external proxies get to carry uniqueKeys
  REPORTER_ASSERT(reporter, !proxy->getUniqueKey().isValid());
  return proxy;
...
static sk_sp<GrTextureProxy> wrapped(
    skiatest::Reporter* reporter, GrRecordingContext*, GrProxyProvider* proxyProvider,
    SkBackingFit fit) {
...
static sk_sp<GrTextureProxy> create_wrapped_backend(
    GrDirectContext* dContext, SkBackingFit fit, sk_sp<GrTexture>* backingSurface) {
  ...
  *backingSurface = resourceProvider->createTexture(
      kSize, format, GrRenderable::kNo, 1, GrMipmapped::kNo, SkBudgeted::kNo, GrProtected::kNo);
  if (!(*backingSurface)) {
    return nullptr;
  }
...
static void invalidation_test(GrDirectContext* dContext, skiatest::Reporter* reporter) {
  ...
  if (dContext->backend() == GrBackendApi::kDawn || dContext->backend() == GrBackendApi::kVulkan ||
...
static void invalidation_and_instantiation_test(
  ...
  sk_sp<GrTextureProxy> proxy =
      deferred_tex(reporter, dContext, proxyProvider, SkBackingFit::kExact);
  SkAssertResult(proxyProvider->assignUniqueKeyToProxy(key, proxy.get()));
