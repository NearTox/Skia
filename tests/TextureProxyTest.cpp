...
int GrProxyProvider::numUniqueKeyProxies_TestOnly() const { return fUniquelyKeyedProxies.count(); }
...
static sk_sp<GrTextureProxy> deferred_tex(
    skiatest::Reporter* reporter, GrContext* ctx, GrProxyProvider* proxyProvider,
    SkBackingFit fit) {
  ...
  sk_sp<GrTextureProxy> proxy = proxyProvider->createProxy(
      format, desc, swizzle, GrRenderable::kNo, 1, kBottomLeft_GrSurfaceOrigin, GrMipMapped::kNo,
...
static sk_sp<GrTextureProxy> deferred_texRT(
    skiatest::Reporter* reporter, GrContext* ctx, GrProxyProvider* proxyProvider,
    SkBackingFit fit) {
  const GrCaps* caps = ctx->priv().caps();
...
static sk_sp<GrTextureProxy> wrapped(
    skiatest::Reporter* reporter, GrContext* ctx, GrProxyProvider* proxyProvider,
    SkBackingFit fit) {
  sk_sp<GrTextureProxy> proxy = proxyProvider->testingOnly_createInstantiatedProxy(
      kSize, kColorType, GrRenderable::kNo, 1, kBottomLeft_GrSurfaceOrigin, fit, SkBudgeted::kYes,
      GrProtected::kNo);
  // Only budgeted & wrapped external proxies get to carry uniqueKeys
  REPORTER_ASSERT(reporter, !proxy->getUniqueKey().isValid());
  return proxy;
...
static sk_sp<GrTextureProxy> wrapped_with_key(
    skiatest::Reporter* reporter, GrContext* ctx, GrProxyProvider* proxyProvider,
    SkBackingFit fit) {
...
static sk_sp<GrTextureProxy> create_wrapped_backend(
    GrContext* context, SkBackingFit fit, sk_sp<GrTexture>* backingSurface) {
  ...
  *backingSurface = resourceProvider->createTexture(
  ...
  return proxyProvider->wrapBackendTexture(
      backendTex, GrColorType::kRGBA_8888, kBottomLeft_GrSurfaceOrigin, kBorrow_GrWrapOwnership,
...
static void basic_test(
    GrContext* context, skiatest::Reporter* reporter, sk_sp<GrTextureProxy> proxy) {
  static int id = 1;
  ...
  REPORTER_ASSERT(
      reporter,
      proxyProvider->findOrCreateProxyByUniqueKey(key, kColorType, kBottomLeft_GrSurfaceOrigin));
  REPORTER_ASSERT(reporter, 1 == proxyProvider->numUniqueKeyProxies_TestOnly());
  ...
  // deleted.
  bool expectResourceToOutliveProxy =
      proxy->peekSurface()->resourcePriv().budgetedType() == GrBudgetedType::kUnbudgetedCacheable;
  ...
  bool expectDeletingProxyToDeleteResource =
      proxy->peekSurface()->resourcePriv().budgetedType() == GrBudgetedType::kUnbudgetedUncacheable;
  ...
  proxy = proxyProvider->findOrCreateProxyByUniqueKey(key, kColorType, kBottomLeft_GrSurfaceOrigin);
...
static void invalidation_and_instantiation_test(GrContext* context, skiatest::Reporter* reporter) {
  ...
  sk_sp<GrTextureProxy> proxy =
      deferred_tex(reporter, context, proxyProvider, SkBackingFit::kExact);
  SkAssertResult(proxyProvider->assignUniqueKeyToProxy(key, proxy.get()));
