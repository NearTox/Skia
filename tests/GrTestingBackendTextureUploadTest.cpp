...
static void testing_only_texture_test(
    skiatest::Reporter* reporter, GrDirectContext* dContext, SkColorType ct,
  ...
  if (doDataUpload) {
    ...
    backendTex = dContext->createBackendTexture(&expectedPixels, 1, renderable, GrProtected::kNo);
  } else {
    backendTex = dContext->createBackendTexture(
        kWidth, kHeight, ct, SkColors::kTransparent, mipMapped, renderable, GrProtected::kNo);
  ...
  GrSwizzle swizzle = dContext->priv().caps()->getReadSwizzle(wrappedProxy->backendFormat(), grCT);
  GrSurfaceProxyView view(std::move(wrappedProxy), kTopLeft_GrSurfaceOrigin, swizzle);
  auto surfaceContext =
      GrSurfaceContext::Make(dContext, std::move(view), grCT, kPremul_SkAlphaType, nullptr);
  REPORTER_ASSERT(reporter, surfaceContext);

  bool result = surfaceContext->readPixels(
      dContext, {grCT, kPremul_SkAlphaType, nullptr, kWidth, kHeight}, actualPixels.writable_addr(),
      actualPixels.rowBytes(), {0, 0});
  ...
  for (auto colorType : {kRGBA_8888_SkColorType, kBGRA_8888_SkColorType}) {
    for (auto renderable : {GrRenderable::kYes, GrRenderable::kNo}) {
      for (bool doDataUpload : {true, false}) {
        testing_only_texture_test(
            reporter, ctxInfo.directContext(), colorType, renderable, doDataUpload,
            GrMipmapped::kNo);
