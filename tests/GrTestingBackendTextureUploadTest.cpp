...
void testing_only_texture_test(
  ...
  if (GrRenderable::kYes == renderable) {
    ...
    wrappedProxy = context->priv().proxyProvider()->wrapBackendTexture(
        backendTex, grCT, kAdopt_GrWrapOwnership, GrWrapCacheable::kNo, GrIOType::kRW_GrIOType);
  }
  ...
  bool result = surfaceContext->readPixels(
      {grCT, kPremul_SkAlphaType, nullptr, kWidth, kHeight}, actualPixels.writable_addr(),
      actualPixels.rowBytes(), {0, 0}, context);
