...
static void rand_fill(uint8_t buffer[], int count) {
  for (int i = 0; i < count; i++) buffer[i] = (uint8_t)((gRand.nextU() >> 8) & 0x3);
}
...
static void test_pack8(skiatest::Reporter* reporter) {
  ...
  for (size_t i = 4; i < SK_ARRAY_COUNT(gTests); i++) {
    ...
    size_t dstSize = SkPackBits::Pack8(gTests[i].fSrc, gTests[i].fCount, dst, maxSize - 1);
    REPORTER_ASSERT(reporter, dstSize == 0);
    dstSize = SkPackBits::Pack8(gTests[i].fSrc, gTests[i].fCount, dst, sizeof(dst));
    ...
    bool match = gTests[i].fCount == srcCount &&
                 memcmp(gTests[i].fSrc, src, gTests[i].fCount * sizeof(uint8_t)) == 0;
...
DEF_TEST(PackBits, reporter) { test_pack8(reporter); }
