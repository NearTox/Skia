...
static GrProcessorSet::Analysis do_analysis(
    const GrXPFactory* xpf, const GrProcessorAnalysisColor& colorInput,
    GrProcessorAnalysisCoverage coverageInput, const GrCaps& caps) {
  GrPaint paint;
...
class GrPorterDuffTest {
 public:
  struct XPInfo {
    XPInfo(
        skiatest::Reporter* reporter, SkBlendMode xfermode, const GrCaps& caps,
      ...
      sk_sp<const GrXferProcessor> xp(GrXPFactory::MakeXferProcessor(
          xpf, inputColor, inputCoverage, false, caps, GrClampType::kAuto));
      TEST_ASSERT(
...
static void test_lcd_coverage_fallback_case(skiatest::Reporter* reporter, const GrCaps& caps) {
  ...
  TEST_ASSERT(
      !(GrXPFactory::GetAnalysisProperties(xpf, color, coverage, caps, autoClamp) &
...
DEF_GPUTEST(PorterDuffNoDualSourceBlending, reporter, options) {
  ...
  CreateBackendTexture(
      ctx, &backendTex, 100, 100, kRGBA_8888_SkColorType, SkColors::kTransparent, GrMipmapped::kNo,
      GrRenderable::kNo);
  ...
  GrXferProcessor::DstProxyView fakeDstProxyView;
  {
    ...
    GrSwizzle swizzle = caps.getReadSwizzle(backendTex.getBackendFormat(), GrColorType::kRGBA_8888);
