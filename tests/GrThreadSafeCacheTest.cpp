...
static std::unique_ptr<skgpu::v1::SurfaceDrawContext> new_SDC(
...
static bool default_is_newer_better(SkData* incumbent, SkData* challenger) { return false; }
...
class TestHelper {
 public:
  ...
  TestHelper(
      ...
      : fDContext(dContext), fIsNewerBetter(isNewerBetter) {
  ...
  typedef void (TestHelper::*addAccessFP)(
      SkCanvas*, int wh, int id, bool failLookUp, bool failFillingIn);
  typedef bool (TestHelper::*checkFP)(
      SkCanvas*, int wh, int expectedHits, int expectedMisses, int expectedNumRefs, int expectedID);

  // Add a draw on 'canvas' that will introduce a ref on the 'wh' view
  void addViewAccess(
      SkCanvas* canvas, int wh, int id = kNoID, bool failLookup = false,
      bool failFillingIn = false) {
    auto rContext = canvas->recordingContext();

    auto view = AccessCachedView(
  ...
  bool checkView(
      SkCanvas* canvas, int wh, int expectedHits, int expectedMisses, int expectedNumRefs,
      int expectedID) {
    if (fStats.fCacheHits != expectedHits || fStats.fCacheMisses != expectedMisses) {
      SkDebugf(
          "Hits E: %d A: %d --- Misses E: %d A: %d\n", expectedHits, fStats.fCacheHits,
          expectedMisses, fStats.fCacheMisses);
      return false;
    }
  ...
  bool checkImage(skiatest::Reporter* reporter, sk_sp<SkSurface> s) {
    ...
    auto error =
        std::function<ComparePixmapsErrorReporter>([reporter](int x, int y, const float diffs[4]) {
          SkASSERT(x >= 0 && y >= 0);
          ERRORF(
              reporter, "mismatch at %d, %d (%f, %f, %f %f)", x, y, diffs[0], diffs[1], diffs[2],
              diffs[3]);
        });
  ...
  bool checkImage(skiatest::Reporter* reporter) { return this->checkImage(reporter, fDst); }

  bool checkImage(skiatest::Reporter* reporter, sk_sp<SkDeferredDisplayList> ddl) {
    sk_sp<SkSurface> tmp =
        SkSurface::MakeRenderTarget(fDContext, SkBudgeted::kNo, default_ii(kImageWH));
    if (!tmp) {
      return false;
    }
 ...
 private:
  static GrSurfaceProxyView AccessCachedView(
      GrRecordingContext*, GrThreadSafeCache*, int wh, bool failLookup, bool failFillingIn, int id,
      Stats*);
  static GrSurfaceProxyView CreateViewOnCpu(GrRecordingContext*, int wh, Stats*);
  static bool FillInViewOnGpu(
      GrDirectContext*, int wh, Stats*, const GrSurfaceProxyView& lazyView,
...
class GrThreadSafeVertexTestOp : public GrDrawOp {
 public:
  ...
  static GrOp::Owner Make(
      GrRecordingContext* rContext, TestHelper::Stats* stats, int wh, int id, bool failLookup,
      bool failFillingIn, GrThreadSafeCache::IsNewerBetter isNewerBetter) {
 ...
 private:
  ...
  GrProgramInfo* createProgramInfo(
    ...
    auto gp = MakeForDeviceSpace(
        arena, color, Coverage::kSolid_Type, LocalCoords::kUnused_Type, SkMatrix::I());
  ...
  GrProgramInfo* createProgramInfo(GrOpFlushState* flushState) const {
    return this->createProgramInfo(
        ...
        flushState->usesMSAASurface(), flushState->detachAppliedClip(), flushState->dstProxyView(),
  ...
  void findOrCreateVertices(GrRecordingContext* rContext, bool failLookup, bool failFillingIn) {
    if (!fVertexData) {
      ...
      auto [tmpV, tmpD] = threadSafeViewCache->addVertsWithData(key, fVertexData, fIsNewerBetter);
      if (tmpV != fVertexData) {
  ...
  void onPrePrepare(
    ...
    fProgramInfo = this->createProgramInfo(
        rContext->priv().caps(), arena, writeView, usesMSAASurface, std::move(appliedClip),
...
GrSurfaceProxyView TestHelper::CreateViewOnCpu(GrRecordingContext* rContext, int wh, Stats* stats) {
  ...
  sk_sp<GrTextureProxy> proxy = proxyProvider->createProxyFromBitmap(
      create_bitmap(wh), GrMipmapped::kNo, SkBackingFit::kExact, SkBudgeted::kYes);
  if (!proxy) {
    return {};
  }
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeCache4_75View, reporter, ctxInfo) {
  test_4_75(ctxInfo.directContext(), reporter, &TestHelper::addViewAccess, &TestHelper::checkView);
}

DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeCache4_75Verts, reporter, ctxInfo) {
  test_4_75(ctxInfo.directContext(), reporter, &TestHelper::addVertAccess, &TestHelper::checkVert);
}
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeCache15View, reporter, ctxInfo) {
  test_15(
      ctxInfo.directContext(), reporter, &TestHelper::addViewAccess, &TestHelper::checkView,
      create_view_key);
}

DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrThreadSafeCache15Verts, reporter, ctxInfo) {
  test_15(
      ctxInfo.directContext(), reporter, &TestHelper::addVertAccess, &TestHelper::checkVert,
      create_vert_key);
}
