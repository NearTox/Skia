...
void test_wrapping(
  ...
  if (GrRenderable::kYes == renderable && dContext->colorTypeSupportedAsSurface(skColorType)) {
    sk_sp<SkSurface> surf = SkSurface::MakeFromBackendTexture(
        dContext, mbet->texture(), kTopLeft_GrSurfaceOrigin, 0, skColorType, nullptr, nullptr);
    if (!surf) {
      ERRORF(
          reporter, "Couldn't make SkSurface from backendTexture for %s\n",
...

  {
    sk_sp<SkImage> img = SkImage::MakeFromTexture(
        dContext, mbet->texture(), kTopLeft_GrSurfaceOrigin, skColorType, kUnpremul_SkAlphaType,
        nullptr);
    if (!img) {
      ERRORF(
          reporter, "Couldn't make SkImage from backendTexture for %s\n",
          ToolUtils::colortype_name(skColorType));
    } else {
...
static void check_solid_pixmap(
    skiatest::Reporter* reporter, const SkColor4f& expected, const SkPixmap& actual, GrColorType ct,
  ...
  auto error = std::function<ComparePixmapsErrorReporter>(
      [reporter, ct, label1, label2](int x, int y, const float diffs[4]) {
        SkASSERT(x >= 0 && y >= 0);
        ERRORF(
            reporter, "%s %s %s - mismatch at %d, %d (%f, %f, %f %f)", GrColorTypeToStr(ct), label1,
            label2, x, y, diffs[0], diffs[1], diffs[2], diffs[3]);
      });
...
static void check_mipmaps(
    GrDirectContext*, const GrBackendTexture&, GrColorType, const SkColor4f expectedColors[6],
    skiatest::Reporter*, const char* label);

static void check_base_readbacks(
    GrDirectContext* dContext, const GrBackendTexture& backendTex, GrColorType colorType,
    GrRenderable renderableTexture, const SkColor4f& color, skiatest::Reporter* reporter,
    const char* label) {
  ...
  for (GrRenderable renderableCtx : {GrRenderable::kNo, GrRenderable::kYes}) {
    ...
    if (!surfaceContext->readPixels(dContext, actual, {0, 0})) {
      ...
      auto name = SkStringPrintf(
          "%s::readPixels",
          (renderableCtx == GrRenderable::kYes ? "GrSurfaceFillContext" : "GrSurfaceContext"));
...
void test_color_init(
    GrDirectContext* dContext, skiatest::Reporter* reporter,
    std::function<
        sk_sp<ManagedBackendTexture>(GrDirectContext*, const SkColor4f&, GrMipmapped, GrRenderable)>
        create,
  ...
  auto checkBackendTexture = [&](const SkColor4f& testColor) {
    if (mipmapped == GrMipmapped::kYes) {
      ...
      check_mipmaps(dContext, mbet->texture(), colorType, expectedColors, reporter, "colorinit");
    }

    // The last step in this test will dirty the mipmaps so do it last
    check_base_readbacks(
        dContext, mbet->texture(), colorType, renderable, testColor, reporter, "colorinit");
  };
  ...
  if (skColorType != kUnknown_SkColorType) {
    dContext->updateBackendTexture(
        mbet->texture(), skColorType, newColor, ManagedBackendTexture::ReleaseProc,
        mbet->releaseContext());
...
static void check_mipmaps(
  ...
  if (GrBackendApi::kOpenGL == dContext->backend()) {
    ...
    if (colorType == GrColorType::kRGBA_F32 && glGPU->ctxInfo().standard() == kGLES_GrGLStandard) {
      return;
    }
  }
#endif
  ...
  auto proxy = dContext->priv().proxyProvider()->wrapBackendTexture(
      backendTex, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, kRW_GrIOType);
  if (!proxy) {
    ERRORF(reporter, "Could not make proxy from backend texture");
    return;
  }
  auto swizzle = dContext->priv().caps()->getReadSwizzle(backendTex.getBackendFormat(), colorType);
  GrSurfaceProxyView readView(proxy, kTopLeft_GrSurfaceOrigin, swizzle);
  ...
  for (int i = 0, rectSize = 32; i < numMipLevels; ++i, rectSize /= 2) {
    ...
    static constexpr GrSamplerState kNearestNearest(
        GrSamplerState::Filter::kNearest, GrSamplerState::MipmapMode::kNearest);
    auto fp = GrTextureEffect::Make(
        readView, kUnpremul_SkAlphaType, texMatrix, kNearestNearest, *dstFillContext->caps());
    ...
    if (GrColorTypeIsAlphaOnly(colorType)) {
      ...
      fp = GrBlendFragmentProcessor::Make(std::move(fp), std::move(black), SkBlendMode::kModulate);
    }
...
static int make_pixmaps(
  ...
  for (int level = 0; level < numMipLevels; ++level) {
    SkImageInfo ii = SkImageInfo::Make(levelSize, levelSize, skColorType, kUnpremul_SkAlphaType);
...
static void test_pixmap_init(
    ...
    std::function<sk_sp<ManagedBackendTexture>(
        GrDirectContext*, const SkPixmap srcData[], int numLevels, GrSurfaceOrigin, GrRenderable)>
        create,
    SkColorType skColorType, GrSurfaceOrigin origin, GrMipmapped mipMapped,
  ...
  auto checkBackendTexture = [&](SkColor4f colors[6]) {
    ...
    if (mipMapped == GrMipmapped::kYes) {
      SkColor4f expectedColors[6] = {
          get_expected_color(colors[0], grColorType), get_expected_color(colors[1], grColorType),
          get_expected_color(colors[2], grColorType), get_expected_color(colors[3], grColorType),
          get_expected_color(colors[4], grColorType), get_expected_color(colors[5], grColorType),
      };

      check_mipmaps(dContext, mbet->texture(), grColorType, expectedColors, reporter, "pixmap");
    }

    // The last step in this test will dirty the mipmaps so do it last
    check_base_readbacks(
        dContext, mbet->texture(), grColorType, renderable, colors[0], reporter, "pixmap");
  };
  ...
  dContext->updateBackendTexture(
      mbet->texture(), pixmaps, numMipLevels, origin, ManagedBackendTexture::ReleaseProc,
      mbet->releaseContext());
  ...
  for (auto combo : combinations) {
    ...
    for (auto mipmapped : {GrMipmapped::kNo, GrMipmapped::kYes}) {
      ...
      for (auto renderable : {GrRenderable::kNo, GrRenderable::kYes}) {
...

        {
          auto uninitCreateMtd = [colorType](
            ...
            auto mbet = ManagedBackendTexture::MakeWithoutData(
                dContext, 32, 32, colorType, mipmapped, renderable, GrProtected::kNo);
        ...
        {
          auto createWithColorMtd = [colorType](
            ...
            auto mbet = ManagedBackendTexture::MakeWithData(
                dContext, 32, 32, colorType, color, mipmapped, renderable, GrProtected::kNo);
          ...
          test_color_init(
              context, reporter, createWithColorMtd, SkColorTypeToGrColorType(colorType),
              combo.fColor, mipmapped, renderable);
        }

        for (auto origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
          auto createWithSrcDataMtd = [](GrDirectContext* dContext, const SkPixmap srcData[],
                                         int numLevels, GrSurfaceOrigin origin,
                                         GrRenderable renderable) {
            SkASSERT(srcData && numLevels);
            auto mbet = ManagedBackendTexture::MakeWithData(
                dContext, srcData, numLevels, origin, renderable, GrProtected::kNo);
            check_vk_layout(mbet->texture(), VkLayout::kReadOnlyOptimal);
#ifdef SK_DEBUG
            {
          ...
          test_pixmap_init(
              context, reporter, createWithSrcDataMtd, colorType, origin, mipmapped, renderable);
        }
      }
    }
  }
