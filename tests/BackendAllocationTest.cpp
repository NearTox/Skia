...
static void compare_pixmaps(
  ...
  for (int y = 0; y < expected.height(); ++y) {
    for (int x = 0; x < expected.width(); ++x) {
      ...
      ERRORF(
          reporter, "Mismatched pixels at %d %d ct: %d expected: 0x%x actual: 0x%x diff: %d\n", x,
          y, colorType, expectedCol, actualCol, maxDiff);
      return;
    }
  }
}
      ...
      for (auto renderable : {GrRenderable::kNo, GrRenderable::kYes}) {
...

        {
          ...
          auto createWithColorMtd = [colorType](
            ...
            return context->priv().createBackendTexture(
                32, 32, colorType, color, mipMapped, renderable);
          };
...
DEF_GPUTEST_FOR_ALL_GL_CONTEXTS(GLBackendAllocationTest, reporter, ctxInfo) {
  ...
  struct {
    ...
    SkColor4f fColor;
      ...
      {kRGB_888x_SkColorType, GR_GL_RGBA8, kRGBA_8888_GrPixelConfig, {1, 1, 0, 0.5f}},
      {kRGB_888x_SkColorType, GR_GL_RGB8, kRGB_888_GrPixelConfig, {0, 1, 1, 0.5f}},
      ...
      {kBGRA_8888_SkColorType, GR_GL_SRGB8_ALPHA8, kSBGRA_8888_GrPixelConfig, SkColors::kCyan},
      ...
      {kGray_8_SkColorType, GR_GL_LUMINANCE8, kGray_8_as_Lum_GrPixelConfig, SkColors::kLtGray},
      {kGray_8_SkColorType, GR_GL_R8, kGray_8_as_Red_GrPixelConfig, SkColors::kDkGray},
      ...
      {kUnknown_SkColorType, GR_GL_RG32F, kRG_float_GrPixelConfig, {0.7f, 0.7f, 0, 0}},
    ...
    for (auto mipMapped : {GrMipMapped::kNo, GrMipMapped::kYes}) {
      ...
      for (auto renderable : {GrRenderable::kNo, GrRenderable::kYes}) {
...

        {
          ...
          if (GrRenderable::kYes != renderable || kRGB_888x_SkColorType == combo.fColorType) {
            continue;
          }
          ...
          auto createWithColorMtd = [format](
                                        ...
                                        GrMipMapped mipMapped, GrRenderable renderable) {
            return context->priv().createBackendTexture(
                32, 32, format, color, mipMapped, renderable);
          };
