...
void test_wrapping(
  ...
  if (!backendTex.isValid()) {
    ERRORF(
        reporter, "Couldn't create backendTexture for colorType %d renderable %s\n", colorType,
...

  {
    sk_sp<SkImage> img = SkImage::MakeFromTexture(
        context, backendTex, kTopLeft_GrSurfaceOrigin, colorType, kPremul_SkAlphaType, nullptr);
    if (!img) {
...
static void compare_pixmaps(
    const SkPixmap& expected, const SkPixmap& actual, SkColorType colorType,
  ...
  for (int y = 0; y < expected.height(); ++y) {
    for (int x = 0; x < expected.width(); ++x) {
      ...
      ERRORF(
          reporter, "Mismatched pixels at %d %d ct: %d expected: 0x%x actual: 0x%x\n", x, y,
          colorType, expectedCol, actualCol);
      return;
    }
  }
}
...
void test_color_init(
    ...
    SkColorType colorType, const SkColor4f& color, GrMipMapped mipMapped, GrRenderable renderable) {
  ...
  SkAlphaType at = SkColorTypeIsAlwaysOpaque(colorType) ? kOpaque_SkAlphaType : kPremul_SkAlphaType;
  ...
  {
    sk_sp<SkImage> img = SkImage::MakeFromTexture(
        context, backendTex, kTopLeft_GrSurfaceOrigin, colorType, at, nullptr);
    if (img) {
      ...
      // mipMap levels.
      {
        ...
        sk_sp<SkSurface> surf = SkSurface::MakeRenderTarget(
            context, SkBudgeted::kNo, newII, 1, kTopLeft_GrSurfaceOrigin, nullptr);
        if (!surf) {
          context->deleteBackendTexture(backendTex);
          return;
        }
        ...
        for (int i = 0, rectSize = 32; i < numMipLevels; ++i, rectSize /= 2) {
          ...
          if (!colors_eq(actualColor, rasterColor.toSkColor(), 1)) {
            ERRORF(
                reporter, "Pixel mismatch colorType %d: level: %d e: 0x%x a: 0x%x\n", colorType, i,
                rasterColor.toSkColor(), actualColor);
          }
        }
      }
    }
    ...
    for (auto mipMapped : {GrMipMapped::kNo, GrMipMapped::kYes}) {
      ...
      for (auto renderable : {GrRenderable::kNo, GrRenderable::kYes}) {
...

        {
          ...
          test_wrapping(context, reporter, uninitCreateMtd, colorType, mipMapped, renderable);
        }

        {
          ...
          test_color_init(
              context, reporter, createWithColorMtd, colorType, combo.fColor, mipMapped,
              renderable);
        }
      }
    }
...
DEF_GPUTEST_FOR_ALL_GL_CONTEXTS(GLBackendAllocationTest, reporter, ctxInfo) {
  ...
  struct {
    ...
    SkColor4f fColor;
      ...
      {kRGBA_8888_SkColorType, GR_GL_RGBA8, kRGBA_8888_GrPixelConfig, SkColors::kRed},
      {kRGBA_8888_SkColorType, GR_GL_SRGB8_ALPHA8, kSRGBA_8888_GrPixelConfig, SkColors::kRed},

      {kRGB_888x_SkColorType, GR_GL_RGBA8, kRGBA_8888_GrPixelConfig, SkColors::kYellow},
      {kRGB_888x_SkColorType, GR_GL_RGB8, kRGB_888_GrPixelConfig, SkColors::kCyan},

      {kBGRA_8888_SkColorType, GR_GL_RGBA8, kRGBA_8888_GrPixelConfig, SkColors::kBlue},
      {kBGRA_8888_SkColorType, GR_GL_BGRA8, kBGRA_8888_GrPixelConfig, SkColors::kBlue},
      ...
      {kRGBA_1010102_SkColorType,
       ...
       {0.5f, 0, 0, 1.0f}},
      {kRGB_565_SkColorType, GR_GL_RGB565, kRGB_565_GrPixelConfig, SkColors::kRed},
      {kARGB_4444_SkColorType, GR_GL_RGBA4, kRGBA_4444_GrPixelConfig, SkColors::kGreen},

      {kAlpha_8_SkColorType, GR_GL_ALPHA8, kAlpha_8_as_Alpha_GrPixelConfig, kTransCol},
      {kAlpha_8_SkColorType, GR_GL_R8, kAlpha_8_as_Red_GrPixelConfig, kTransCol},

      {kGray_8_SkColorType, GR_GL_LUMINANCE8, kGray_8_as_Lum_GrPixelConfig, kGrayCol},
      {kGray_8_SkColorType, GR_GL_R8, kGray_8_as_Red_GrPixelConfig, kGrayCol},

      {kRGBA_F32_SkColorType, GR_GL_RGBA32F, kRGBA_float_GrPixelConfig, SkColors::kRed},

      {kRGBA_F16Norm_SkColorType, GR_GL_RGBA16F, kRGBA_half_Clamped_GrPixelConfig,
       SkColors::kLtGray},
      {kRGBA_F16_SkColorType, GR_GL_RGBA16F, kRGBA_half_GrPixelConfig, SkColors::kYellow},

      // These backend formats don't have SkColorType equivalents
      {kUnknown_SkColorType, GR_GL_RG8, kRG_88_GrPixelConfig, {0.5f, 0.5f, 0, 0}},
      {kUnknown_SkColorType, GR_GL_R16F, kAlpha_half_as_Red_GrPixelConfig, {1.0f, 0, 0, 0.5f}},
      {kUnknown_SkColorType, GR_GL_LUMINANCE16F, kAlpha_half_as_Lum_GrPixelConfig, kGrayCol},
      {kUnknown_SkColorType, GR_GL_COMPRESSED_RGB8_ETC2, kRGB_ETC1_GrPixelConfig, SkColors::kRed},
      {kUnknown_SkColorType, GR_GL_COMPRESSED_ETC1_RGB8, kRGB_ETC1_GrPixelConfig, SkColors::kRed},
      {kUnknown_SkColorType, GR_GL_R16, kR_16_GrPixelConfig, SkColors::kRed},
      {kUnknown_SkColorType, GR_GL_RG16, kRG_1616_GrPixelConfig, SkColors::kYellow},

      // Experimental (for Y416 and mutant P016/P010)
      {kUnknown_SkColorType, GR_GL_RGBA16, kRGBA_16161616_GrPixelConfig, SkColors::kLtGray},
      {kUnknown_SkColorType, GR_GL_RG16F, kRG_half_GrPixelConfig, SkColors::kYellow},
  };
