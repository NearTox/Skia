...
static void addTs(
    const SkDCubic& cubic, double precision, double start, double end, SkTArray<double, true>* ts) {
  double tDiv = calc_t_div(cubic, precision, 0);
    ...
    switch (verb) {
      ...
      case SkPathVerb::kCubic:
        ...
        for (int index = 0; index < quads.count(); ++index) {
          SkPoint qPts[2] = {quads[index][1].asSkPoint(), quads[index][2].asSkPoint()};
    ...
    switch (verb) {
      ...
      case SkPathVerb::kCubic: {
        ...
        for (int index = 0; index <= inflections; ++index) {
          ...
          simplePath->cubicTo(
              cPts[0].fX, cPts[0].fY, cPts[1].fX, cPts[1].fY, cPts[2].fX, cPts[2].fY);
          lo = hi;
        }
        break;
      }
