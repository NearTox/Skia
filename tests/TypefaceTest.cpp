...
static void TypefaceStyle_test(
    skiatest::Reporter* reporter, uint16_t weight, uint16_t width, SkData* data) {
  ...
  REPORTER_ASSERT(
      reporter,
      newStyle.weight() == weight || (weight <= 10 && newStyle.weight() == 100 * weight) ||
  ...
  REPORTER_ASSERT(reporter, newStyle.width() == width || newStyle.width() == 5);
}
...
DEF_TEST(TypefaceAxes, reporter) {
  ...
  auto test = [&](SkTypeface* typeface, const Variation& expected, int alsoAcceptedAxisTagCount) {
    ...
    REPORTER_ASSERT(
        reporter,
        actualCount == expected.coordinateCount || actualCount == alsoAcceptedAxisTagCount);
    ...
    for (int actualIdx = 0; actualIdx < actualCount; ++actualIdx) {
      ...
      REPORTER_ASSERT(
          reporter, actualFound, "Actual axis '%c%c%c%c' with value '%f' not expected",
          (actual[actualIdx].axis >> 24) & 0xFF, (actual[actualIdx].axis >> 16) & 0xFF,
          (actual[actualIdx].axis >> 8) & 0xFF, (actual[actualIdx].axis) & 0xFF,
...
DEF_TEST(TypefaceAxesParameters, reporter) {
  ...
  auto test = [&](SkTypeface* typeface, const Axis* expected, int expectedCount,
    ...
    REPORTER_ASSERT(
        reporter, actualCount == expectedCount || actualCount == alsoAcceptedAxisTagCount);
    ...
    for (int actualIdx = 0; actualIdx < actualCount; ++actualIdx) {
      ...
      REPORTER_ASSERT(
          reporter, actualFound,
          "Actual axis '%c%c%c%c' with min %f max %f default %f hidden %s not expected",
          (actual[actualIdx].tag >> 24) & 0xFF, (actual[actualIdx].tag >> 16) & 0xFF,
          (actual[actualIdx].tag >> 8) & 0xFF, (actual[actualIdx].tag) & 0xFF,
          actual[actualIdx].min, actual[actualIdx].def, actual[actualIdx].max,
...
static void check_serialize_behaviors(
    sk_sp<SkTypeface> tf, bool isLocalData, skiatest::Reporter* reporter) {
  if (!tf) {
    return;
  }
...
DEF_TEST(Typeface_serialize, reporter) {
  ...
  check_serialize_behaviors(
      SkTypeface::MakeFromStream(GetResourceAsStream("fonts/Distortable.ttf")), true, reporter);
}
...
DEF_TEST(Typeface_glyph_to_char, reporter) {
  ...
  if (std::any_of(
          glyphs.get(), glyphs.get() + codepointCount, [](SkGlyphID g) { return g == 0; })) {
    ERRORF(
        reporter, "Unexpected typeface \"%s\". Expected full support for emoji_sample_text.",
        familyName.c_str());
    return;
  }
  ...
  for (size_t i = 0; i < codepointCount; ++i) {
    ...
    if (gSkFontMgr_DefaultFactory == &SkFontMgr_New_GDI && 0xFFFF < originalCodepoints[i] &&
        newCodepoints[i] == 0) {
      continue;
    }
#endif
