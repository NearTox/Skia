...
DEF_TEST(SkVx, r) {
  ...
  REPORTER_ASSERT(
      r, all(skvx::shuffle<2, 1, 2, 1, 2, 1, 2, 1>(float4{1, 2, 3, 4}) ==
  ...
  for (int x = 0; x < 256; x++)
    for (int y = 0; y < 256; y++) {
      ...
      {
        uint8_t got = skvx::div255(skvx::Vec<8, uint16_t>(x) * skvx::Vec<8, uint16_t>(y))[0];
        REPORTER_ASSERT(r, got == want);
      }

      {
        uint8_t got = skvx::approx_scale(skvx::Vec<8, uint8_t>(x), skvx::Vec<8, uint8_t>(y))[0];
...

  {
    ...
    skvx::Vec<8, uint16_t> hs = {0x0000, 0x3800, 0x3c00, 0x4000, 0xc400, 0xb800, 0xbc00, 0xc000};
