...
DEF_TEST(SkVx, r) {
  ...
  REPORTER_ASSERT(
      r, all(if_then_else(float4{1, 2, 3, 2} <= float4{2, 2, 2, 2}, float4(42), float4(47)) ==
             float4{42, 42, 47, 42}));

  REPORTER_ASSERT(
      r, all(floor(float4{-1.5f, 1.5f, 1.0f, -1.0f}) == float4{-2.0f, 1.0f, 1.0f, -1.0f}));
  REPORTER_ASSERT(
      r, all(ceil(float4{-1.5f, 1.5f, 1.0f, -1.0f}) == float4{-1.0f, 2.0f, 1.0f, -1.0f}));
  REPORTER_ASSERT(
      r, all(trunc(float4{-1.5f, 1.5f, 1.0f, -1.0f}) == float4{-1.0f, 1.0f, 1.0f, -1.0f}));
  REPORTER_ASSERT(
      r, all(round(float4{-1.5f, 1.5f, 1.0f, -1.0f}) == float4{-2.0f, 2.0f, 1.0f, -1.0f}));
  ...
  REPORTER_ASSERT(
      r, buf[0] == 2 && buf[1] == 3 && buf[2] == 4 && buf[3] == 5 && buf[4] == 5 && buf[5] == 6);
  ...
  REPORTER_ASSERT(
      r,
      all(shuffle<2, 1, 2, 1, 2, 1, 2, 1>(float4{1, 2, 3, 4}) == float8{3, 2, 3, 2, 3, 2, 3, 2}));
  ...
  for (int x = 0; x < 256; x++)
    for (int y = 0; y < 256; y++) {
...

      {
        ...
        REPORTER_ASSERT(r, got == want - 1 || got == want || got == want + 1);
...

  {
    ...
    float8 fs = {+0.0f, +0.5f, +1.0f, +2.0f, -4.0f, -0.5f, -1.0f, -2.0f};
    Vec<8, uint16_t> hs = {0x0000, 0x3800, 0x3c00, 0x4000, 0xc400, 0xb800, 0xbc00, 0xc000};
...
DEF_TEST(SkVx_xyzw, r) {
  ...
  REPORTER_ASSERT(
      r, all(naive_if_then_else(
                 int2(0, ~0), shuffle<3, 2>(float4(0, 1, 2, 3)), float4(4, 5, 6, 7).xy()) ==
             float2(4, 2)));
  REPORTER_ASSERT(
      r,
      all(if_then_else(int2(0, ~0), shuffle<3, 2>(float4(0, 1, 2, 3)), float4(4, 5, 6, 7).xy()) ==
          float2(4, 2)));
  REPORTER_ASSERT(
      r,
      all(naive_if_then_else(int2(0, ~0).xyxy(), float4(0, 1, 2, 3).zwxy(), float4(4, 5, 6, 7)) ==
          float4(4, 3, 6, 1)));
  REPORTER_ASSERT(
      r, all(if_then_else(int2(0, ~0).xyxy(), float4(0, 1, 2, 3).zwxy(), float4(4, 5, 6, 7)) ==
             float4(4, 3, 6, 1)));

  REPORTER_ASSERT(
      r, all(pin(float4(0, 1, 2, 3).yxwz(), float2(1).xyxy(), float2(2).xyxy()) ==
             float4(1, 1, 2, 2)));
}
...
DEF_TEST(SkVx_ScaledDividerU32, r) {
  ...
  auto test = [&](uint32_t denom) {
    ...
    uint32_t maxCheck = static_cast<uint32_t>(std::floor((double)(kMax - d.half()) / denom + 0.5));
