...
DEF_TEST(Codec_565, r) {
  ...
  const auto result = codec->getPixels(info, bm.getPixels(), bm.rowBytes(), &options);
...
SkString to_string(SkCodecAnimation::Blend blend) {
  switch (blend) {
    case SkCodecAnimation::Blend::kSrcOver: return SkString("kSrcOver");
    case SkCodecAnimation::Blend::kSrc: return SkString("kSrc");
    default: return SkString();
  }
}
...
void reporter_assert_equals(
    skiatest::Reporter* r, const char* name, int i, const char* prop, T expected, T actual) {
  REPORTER_ASSERT(
      r, expected == actual,
      "%s's frame %i has wrong %s! expected:"
      " %s\tactual: %s",
      name, i, prop, to_string(expected).c_str(), to_string(actual).c_str());
}
}  // namespace
...
DEF_TEST(Codec_frames, r) {
  ...
  constexpr SkCodecAnimation::DisposalMethod kKeep = SkCodecAnimation::DisposalMethod::kKeep;
  ...
  static const struct {
  ...
  } gRecs[] = {
    {
        ...
        {kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver},
        {{0, 0, 100, 100},
         {0, 0, 75, 75},
         {0, 0, 50, 50},
         {0, 0, 60, 60},
    ...
    },
    {
        ...
        {kUnpremul, kUnpremul, kUnpremul, kUnpremul, kUnpremul, kUnpremul, kUnpremul, kUnpremul,
         kUnpremul, kUnpremul, kUnpremul, kUnpremul},
        {100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100},
        0,
        {kKeep, kRestorePrev, kRestorePrev, kRestorePrev, kRestorePrev, kRestoreBG, kKeep,
         kRestoreBG, kRestoreBG, kKeep, kKeep, kRestoreBG, kKeep},
        {true, false, true, false, true, true, true, true, true, true, true, true, true},
        {kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver,
    ...
    },
    {
        ...
        {kSrc, kSrcOver, kSrcOver, kSrcOver, kSrc, kSrcOver, kSrcOver},
        {{0, 0, 100, 100},
         {0, 0, 75, 75},
         {0, 0, 50, 50},
         {0, 0, 60, 60},
  ...
  for (const auto& rec : gRecs) {
    ...
    if (repetitionCount != rec.fRepetitionCount) {
      ERRORF(
          r, "%s repetition count does not match! expected: %i\tactual: %i", rec.fName,
          rec.fRepetitionCount, repetitionCount);
    }

    const int expected = rec.fFrameCount;
    if (rec.fRequiredFrames.size() + 1 != static_cast<size_t>(expected)) {
      ERRORF(
          r, "'%s' has wrong number entries in fRequiredFrames; expected: %i\tactual: %zu",
    ...
    if (expected > 1) {
      if (rec.fDurations.size() != static_cast<size_t>(expected)) {
        ERRORF(
            r, "'%s' has wrong number entries in fDurations; expected: %i\tactual: %zu", rec.fName,
            expected, rec.fDurations.size());
        continue;
      }

      if (rec.fAlphas.size() + 1 != static_cast<size_t>(expected)) {
        ERRORF(
            r, "'%s' has wrong number entries in fAlphas; expected: %i\tactual: %zu", rec.fName,
            expected - 1, rec.fAlphas.size());
        continue;
      }
    ...
    for (auto mode : {TestMode::kVector, TestMode::kIndividual}) {
      ...
      switch (mode) {
        case TestMode::kVector:
          ...
          break;
        case TestMode::kIndividual: frameCount = codec->getFrameCount(); break;
      }
      ...
      for (int i = 0; i < frameCount; i++) {
        SkCodec::FrameInfo frameInfo;
        switch (mode) {
          case TestMode::kVector: frameInfo = frameInfos[i]; break;
        ...
        if (rec.fDurations[i] != frameInfo.fDuration) {
          ERRORF(
              r, "%s frame %i's durations do not match! expected: %i\tactual: %i", rec.fName, i,
              rec.fDurations[i], frameInfo.fDuration);
        }
        ...
        if (expectedAlpha != alpha) {
          ERRORF(
              r, "%s's frame %i has wrong alpha type! expected: %s\tactual: %s", rec.fName, i,
              to_string(expectedAlpha), to_string(alpha));
        }
        ...
        if (0 == i) {
          ...
          ERRORF(
              r, "%s's frame %i has wrong dependency! expected: %i\tactual: %i", rec.fName, i,
              rec.fRequiredFrames[i - 1], frameInfo.fRequiredFrame);
        }

        REPORTER_ASSERT(r, frameInfo.fDisposalMethod == rec.fDisposalMethods[i]);

        reporter_assert_equals<bool>(
            r, rec.fName, i, "alpha within bounds", rec.fAlphaWithinBounds[i],
            frameInfo.fHasAlphaWithinBounds);

        reporter_assert_equals(r, rec.fName, i, "blend mode", rec.fBlends[i], frameInfo.fBlend);

        reporter_assert_equals(
            r, rec.fName, i, "frame rect", rec.fFrameRects[i], frameInfo.fFrameRect);
      }
      ...
      auto decode = [&](SkBitmap* bm, int index, int cachedIndex) {
        ...
        if (cachedIndex != SkCodec::kNoFrame) {
          ...
          const bool success = ToolUtils::copy_to(bm, kN32_SkColorType, cachedFrames[cachedIndex]);
          REPORTER_ASSERT(r, success);
        }
        SkCodec::Options opts;
        opts.fFrameIndex = index;
        opts.fPriorFrame = cachedIndex;
        const auto result = codec->getPixels(decodeInfo, bm->getPixels(), bm->rowBytes(), &opts);
        if (cachedIndex != SkCodec::kNoFrame && restore_previous(frameInfos[cachedIndex])) {
        ...
        if (result != SkCodec::kSuccess) {
          ERRORF(
              ...
              "error %i",
              index, rec.fName, cachedIndex, result);
        }
...
static void test_animated_AndroidCodec(skiatest::Reporter* r, const char* file) {
  ...
  for (int sampleSize : {8, 32, 100}) {
    ...
    for (int i = 0; i < codec->codec()->getFrameCount(); ++i) {
      ...
      if (5 == i) {
        REPORTER_ASSERT(
            r, frameInfo.fDisposalMethod == SkCodecAnimation::DisposalMethod::kRestoreBGColor);
      }
      options.fFrameIndex = i;
      options.fPriorFrame = i - 1;
      info = info.makeAlphaType(frameInfo.fAlphaType);

      auto result = codec->codec()->getPixels(info, bm.getPixels(), bm.rowBytes(), &options);
      REPORTER_ASSERT(r, result == SkCodec::kSuccess);
      ...
      options.fPriorFrame = SkCodec::kNoFrame;
      result = codec->codec()->getPixels(info, bm2.getPixels(), bm2.rowBytes(), &options);
      REPORTER_ASSERT(r, result == SkCodec::kSuccess);
...
DEF_TEST(AndroidCodec_animated, r) { test_animated_AndroidCodec(r, "images/required.webp"); }

DEF_TEST(AndroidCodec_animated_gif, r) { test_animated_AndroidCodec(r, "images/required.gif"); }

DEF_TEST(EncodedOriginToMatrixTest, r) {
  // SkAnimCodecPlayer relies on the fact that these matrices are invertible.
  for (auto origin :
       {kTopLeft_SkEncodedOrigin, kTopRight_SkEncodedOrigin, kBottomRight_SkEncodedOrigin,
        kBottomLeft_SkEncodedOrigin, kLeftTop_SkEncodedOrigin, kRightTop_SkEncodedOrigin,
        kRightBottom_SkEncodedOrigin, kLeftBottom_SkEncodedOrigin}) {
...
DEF_TEST(AnimCodecPlayer, r) {
  static constexpr struct {
    ...
    SkISize fSize;
      ...
      {"images/alphabetAnim.gif", 1300, {100, 100}}, {"images/randPixels.gif", 0, {8, 8}},
      {"images/randPixels.jpg", 0, {8, 8}},          {"images/randPixels.png", 0, {8, 8}},
      {"images/stoplight.webp", 2500, {11, 29}},     {"images/stoplight_h.webp", 2500, {29, 11}},
      {"images/orientation/1.webp", 0, {100, 80}},   {"images/orientation/2.webp", 0, {100, 80}},
      {"images/orientation/3.webp", 0, {100, 80}},   {"images/orientation/4.webp", 0, {100, 80}},
      {"images/orientation/5.webp", 0, {100, 80}},   {"images/orientation/6.webp", 0, {100, 80}},
      {"images/orientation/7.webp", 0, {100, 80}},   {"images/orientation/8.webp", 0, {100, 80}},
  };
  ...
  for (const auto& test : gTests) {
    ...
    REPORTER_ASSERT(
        r, f0->bounds().size() == test.fSize, "Mismatched size for initial frame of %s",
        test.fFile);
    ...
    REPORTER_ASSERT(
        r, f1->bounds().size() == test.fSize, "Mismatched size for frame at 500 ms of %s",
        test.fFile);
  }
}
