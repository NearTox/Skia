...
DEF_TEST(SkShaperTest_cluster, reporter) {
  ...
  for (auto& oneCase : cases) {
    ...
    auto checker = [&](size_t glyphStartIndex, size_t glyphEndIndex, uint32_t utf8StartIndex,
    ...
    SkShaperJSONWriter::BreakupClusters(0, oneCase.utf8Len, SkMakeSpan(oneCase.clusters), checker);
...
DEF_TEST(SkShaperTest_VisualizeCluster, reporter) {
  ...
  for (auto& oneCase : cases) {
    size_t answerCount = 0;
    auto checker = [&](int codePointCount, SkSpan<const char> utf1to1,
    ...
    SkShaperJSONWriter::VisualizeClusters(
        oneCase.utf8.c_str(), 0, oneCase.utf8.size(), SkMakeSpan(oneCase.glyphIDs),
        SkMakeSpan(oneCase.clusters), checker);
  }
}
