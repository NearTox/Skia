...
static void test_wacky_bitmapshader(skiatest::Reporter* reporter, int width, int height) {
  ...
  SkCanvas c(dev);
  matrix.setAll(
      -119.34097f, -43.436558f, 93489.945f, 43.436558f, -119.34097f, 123.98426f, 0, 0, SK_Scalar1);
  c.concat(matrix);
  ...
  matrix.setAll(
      0.0078740157f, 0, SkIntToScalar(249), 0, 0.0078740157f, SkIntToScalar(239), 0, 0, SK_Scalar1);
  SkPaint paint;
  paint.setShader(
      bm.makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions(), matrix));
...
static void test_giantrepeat_crbug118018(skiatest::Reporter* reporter) {
  ...
  for (size_t i = 0; i < SK_ARRAY_COUNT(gTests); ++i) {
    test_wacky_bitmapshader(reporter, gTests[i].fWidth, gTests[i].fHeight);
  }
}
...
DEF_TEST(DrawBitmapRect, reporter) {
  ...
  canvas.drawImageRect(
      src.asImage(), srcR, dstR, SkSamplingOptions(), nullptr, SkCanvas::kStrict_SrcRectConstraint);
