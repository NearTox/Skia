...
static void assert_equal(
    skiatest::Reporter* reporter, GrDirectContext* dContextA, SkImage* a, const SkIRect* subsetA,
    SkImage* b) {
...
static sk_sp<SkImage> create_picture_image() {
  ...
  return SkImage::MakeFromPicture(
      recorder.finishRecordingAsPicture(), SkISize::Make(10, 10), nullptr, nullptr,
...
static sk_sp<SkImage> create_rasterproc_image(RasterDataHolder* dataHolder) {
  ...
  return SkImage::MakeFromRaster(
      SkPixmap(info, dataHolder->fData->data(), info.minRowBytes()), RasterDataHolder::Release,
      dataHolder);
}
...
static sk_sp<SkImage> create_gpu_image(
    GrRecordingContext* rContext, bool withMips = false, SkBudgeted budgeted = SkBudgeted::kYes) {
  const SkImageInfo info = SkImageInfo::MakeN32(20, 20, kOpaque_SkAlphaType);
...
DEF_TEST(ImageEncode, reporter) { test_encode(reporter, nullptr, create_image().get()); }
...
static void make_bitmap_mutable(SkBitmap* bm) { bm->allocN32Pixels(10, 10); }
  ...
  for (auto mipmapped : {GrMipmapped::kNo, GrMipmapped::kYes}) {
    for (const auto& factory : imageFactories) {
      ...
      if ((origProxy = sk_gpu_test::GetTextureImageProxy(image.get(), dContext))) {
        REPORTER_ASSERT(
            reporter, (origProxy->mipmapped() == GrMipmapped::kYes) == image->hasMipmaps());
      ...
      for (auto budgeted : {SkBudgeted::kNo, SkBudgeted::kYes}) {
        ...
        GrTextureProxy* copyProxy = sk_gpu_test::GetTextureImageProxy(texImage.get(), dContext);
        SkASSERT(copyProxy);
        // Did we ask for MIPs on a context that supports them?
        bool validRequestForMips =
        ...
        REPORTER_ASSERT(reporter, shouldBeMipped == (copyProxy->mipmapped() == GrMipmapped::kYes));
        ...
        if (image->isTextureBacked() && (!validRequestForMips || origIsMippedTexture)) {
          ...
          GrTextureProxy* texProxy = sk_gpu_test::GetTextureImageProxy(texImage.get(), dContext);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(UnpremulTextureImage, reporter, ctxInfo) {
  ...
  for (int y = 0; y < 256; ++y) {
    for (int x = 0; x < 256; ++x) {
      *bmp.getAddr32(x, y) = SkColorSetARGB((U8CPU)y, 255 - (U8CPU)y, (U8CPU)x, 255 - (U8CPU)x);
    }
  }
  auto dContext = ctxInfo.directContext();
  ...
  unpremul.allocPixels(
      SkImageInfo::Make(256, 256, kRGBA_8888_SkColorType, kUnpremul_SkAlphaType, nullptr));
  if (!texImage->readPixels(
          dContext, unpremul.info(), unpremul.getPixels(), unpremul.rowBytes(), 0, 0)) {
  ...
  for (int y = 0; y < 256; ++y) {
    for (int x = 0; x < 256; ++x) {
      if (*bmp.getAddr32(x, y) != *unpremul.getAddr32(x, y)) {
        ERRORF(
            reporter, "unpremul(0x%08x)->unpremul(0x%08x) at %d, %d.", *bmp.getAddr32(x, y),
            *unpremul.getAddr32(x, y), x, y);
        return;
      }
    }
  }
  ...
  if (!texImage->readPixels(dContext, premul.info(), premul.getPixels(), premul.rowBytes(), 0, 0)) {
    ERRORF(reporter, "Unpremul readback failed.");
    return;
  }
  for (int y = 0; y < 256; ++y) {
    for (int x = 0; x < 256; ++x) {
      ...
      if (origA != readA || SkTAbs(readB - origB) > tol || SkTAbs(readG - origG) > tol ||
        ...
        ERRORF(
            reporter, "unpremul(0x%08x)->premul(0x%08x) expected(0x%08x) at %d, %d.",
...
DEF_GPUTEST(AbandonedContextImage, reporter, options) {
  ...
  for (int ct = 0; ct < Factory::kContextTypeCnt; ++ct) {
    ...
    auto gsurf = SkSurface::MakeRenderTarget(
        ...
        SkImageInfo::Make(100, 100, kRGBA_8888_SkColorType, kPremul_SkAlphaType), 1, nullptr);
    if (!gsurf) {
      continue;
    }
...
DEF_TEST(ImageEmpty, reporter) {
  ...
  REPORTER_ASSERT(
      reporter, nullptr == SkImage::MakeFromGenerator(std::make_unique<EmptyGenerator>()));
}
...
static void image_test_read_pixels(
    GrDirectContext* dContext, skiatest::Reporter* reporter, SkImage* image) {
  if (!image) {
  ...
  REPORTER_ASSERT(
      reporter, !image->readPixels(dContext, info, pixels, rowBytes, image->width(), 0));
  REPORTER_ASSERT(
      reporter, !image->readPixels(dContext, info, pixels, rowBytes, 0, image->height()));
  ...
  REPORTER_ASSERT(
      reporter,
      image->readPixels(dContext, info, pixels, rowBytes, image->width() - w, image->height() - h));
  REPORTER_ASSERT(reporter, has_pixels(pixels, w * h, expected));
  ...
  REPORTER_ASSERT(
      reporter,
      image->readPixels(dContext, info, pixels, rowBytes, image->width() - 1, image->height() - 1));
...
static void check_legacy_bitmap(
    skiatest::Reporter* reporter, GrDirectContext* dContext, const SkImage* image,
  ...
  REPORTER_ASSERT(
      reporter, image->readPixels(dContext, info, &imageColor, sizeof(SkPMColor), 0, 0));
  REPORTER_ASSERT(reporter, imageColor == *bitmap.getAddr32(0, 0));
}

static void test_legacy_bitmap(
    skiatest::Reporter* reporter, GrDirectContext* dContext, const SkImage* image) {
  if (!image) {
    ERRORF(reporter, "Failed to create image.");
    return;
...
static void test_peek(skiatest::Reporter* reporter, SkImage* image, bool expectPeekSuccess) {
  ...
  if (success) {
    ...
    REPORTER_ASSERT(
        reporter,
        kPremul_SkAlphaType == info.alphaType() || kOpaque_SkAlphaType == info.alphaType());
...
static void test_cross_context_image(
    skiatest::Reporter* reporter, const GrContextOptions& options, const char* testName,
  ...
  for (int i = 0; i < GrContextFactory::kContextTypeCnt; ++i) {
    ...
    // Case #6: Verify that only one context can be using the image at a time
    {
      ...
      testContext->makeCurrent();
      std::tie(viewSecondRef, std::ignore) = as_IB(refImg)->asView(dContext, GrMipmapped::kNo);
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(makeBackendTexture, reporter, ctxInfo) {
  ...
  for (const TestCase& testCase : testCases) {
    ...
    bool result =
        SkImage::MakeBackendTextureFromSkImage(context, std::move(image), &newBackend, &proc);
    if (result != testCase.fExpectation) {
      static const char* const kFS[] = {"fail", "succeed"};
      ERRORF(reporter, "This image was expected to %s but did not.", kFS[testCase.fExpectation]);
    }
    ...
    if (testCase.fCanTakeDirectly != tookDirectly) {
      ...
      ERRORF(
          reporter, "This backend texture was %s to be taken directly.",
...
static sk_sp<SkImage> create_picture_image(sk_sp<SkColorSpace> space) {
  ...
  return SkImage::MakeFromPicture(
      recorder.finishRecordingAsPicture(), SkISize::Make(10, 10), nullptr, nullptr,
      SkImage::BitDepth::kU8, std::move(space));
};
...
DEF_TEST(Image_ColorSpace, r) {
  ...
  sk_sp<SkSurface> surface =
      SkSurface::MakeRaster(SkImageInfo::MakeN32Premul(SkISize::Make(10, 10)));
...
static void check_scaled_pixels(skiatest::Reporter* reporter, SkPixmap* pmap, uint32_t expected) {
  ...
  for (auto y = 0; y < pmap->height(); ++y) {
    for (auto x = 0; x < pmap->width(); ++x) {
      ...
      if (pixel != expected) {
        ERRORF(
            reporter, "Expected scaled pixels to be the same. At %d,%d 0x%08x != 0x%08x", x, y,
            pixel, expected);
        return;
      }
    }
...
static void test_scale_pixels(
    skiatest::Reporter* reporter, const SkImage* image, uint32_t expected) {
...
static sk_sp<SkImage> any_image_will_do() { return GetResourceAsImage("images/mandrill_32.png"); }
...
constexpr SkM44 gCentripetalCatmulRom(
    0.0f / 2, -1.0f / 2, 2.0f / 2, -1.0f / 2, 2.0f / 2, 0.0f / 2, -5.0f / 2, 3.0f / 2, 0.0f / 2,
    1.0f / 2, 4.0f / 2, -3.0f / 2, 0.0f / 2, 0.0f / 2, -1.0f / 2, 1.0f / 2);

constexpr SkM44 gMitchellNetravali(
    1.0f / 18, -9.0f / 18, 15.0f / 18, -7.0f / 18, 16.0f / 18, 0.0f / 18, -36.0f / 18, 21.0f / 18,
    1.0f / 18, 9.0f / 18, 27.0f / 18, -21.0f / 18, 0.0f / 18, 0.0f / 18, -6.0f / 18, 7.0f / 18);
