...
static void assert_equal(
    skiatest::Reporter* reporter, SkImage* a, const SkIRect* subsetA, SkImage* b) {
...
static sk_sp<SkImage> create_gpu_image(GrContext* context, bool withMips = false) {
  ...
  auto surface(SkSurface::MakeRenderTarget(
      context, SkBudgeted::kNo, info, 0, kBottomLeft_GrSurfaceOrigin, nullptr, withMips));
  draw_image_test_pattern(surface->getCanvas());
  return surface->makeImageSnapshot();
}
...
DEF_TEST(ImageEncode, reporter) { test_encode(reporter, create_image().get()); }
  ...
  for (auto& dstColorSpace : dstColorSpaces) {
    for (auto mipMapped : {GrMipMapped::kNo, GrMipMapped::kYes}) {
      for (auto factory : imageFactories) {
        ...
        sk_sp<SkImage> texImage(image->makeTextureImage(context, dstColorSpace.get(), mipMapped));
        if (!texImage) {
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrContext_colorTypeSupportedAsImage, reporter, ctxInfo) {
  ...
  for (int ct = 0; ct < kLastEnum_SkColorType; ++ct) {
    ...
    GrBackendTexture backendTex =
        context->createBackendTexture(kSize, kSize, colorType, GrMipMapped::kNo, GrRenderable::kNo);

    auto img = SkImage::MakeFromTexture(
        context, backendTex, kTopLeft_GrSurfaceOrigin, colorType, kOpaque_SkAlphaType, nullptr);
    REPORTER_ASSERT(
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(UnpremulTextureImage, reporter, ctxInfo) {
  ...
  if (false) {
    ...
    unpremul.allocPixels(
        SkImageInfo::Make(256, 256, kRGBA_8888_SkColorType, kUnpremul_SkAlphaType, nullptr));
    if (!texImage->readPixels(unpremul.info(), unpremul.getPixels(), unpremul.rowBytes(), 0, 0)) {
  ...
  for (int y = 0; y < 256; ++y) {
    for (int x = 0; x < 256; ++x) {
      ...
      if (origA != readA || SkTAbs(readB - origB) > tol || SkTAbs(readG - origG) > tol ||
        ...
        ERRORF(
            reporter, "unpremul(0x%08x)->premul(0x%08x) at %d, %d.", *bmp.getAddr32(x, y),
            *premul.getAddr32(x, y), x, y);
        return;
      }
    }
  }
...
DEF_TEST(ImageEmpty, reporter) {
  ...
  REPORTER_ASSERT(
      reporter, nullptr == SkImage::MakeFromGenerator(skstd::make_unique<EmptyGenerator>()));
}
...
static void image_test_read_pixels(skiatest::Reporter* reporter, SkImage* image) {
  ...
  sk_memset32(pixels, notExpected, w * h);
  REPORTER_ASSERT(
      reporter, image->readPixels(info, pixels, rowBytes, image->width() - w, image->height() - h));
  REPORTER_ASSERT(reporter, has_pixels(pixels, w * h, expected));
  ...
  sk_memset32(pixels, notExpected, w * h);
  REPORTER_ASSERT(
      reporter, image->readPixels(info, pixels, rowBytes, image->width() - 1, image->height() - 1));
...
static void check_legacy_bitmap(
    skiatest::Reporter* reporter, const SkImage* image, const SkBitmap& bitmap) {
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(SkImage_NewFromTextureRelease, reporter, ctxInfo) {
  ...
  if (!create_backend_texture(
          ctx, &backendTex, ii, GrMipMapped::kNo, SK_ColorRED, GrRenderable::kNo)) {
  ...
  sk_sp<SkImage> refImg(SkImage::MakeFromTexture(
      ctx, backendTex, texOrigin, kRGBA_8888_SkColorType, kPremul_SkAlphaType, nullptr,
...
DEF_GPUTEST(SkImage_MakeCrossContextFromEncodedRelease, reporter, options) {
  ...
  test_cross_context_image(
      reporter, options, "SkImage_MakeCrossContextFromEncodedRelease", [&data](GrContext* ctx) {
...
DEF_GPUTEST(SkImage_MakeCrossContextFromPixmapRelease, reporter, options) {
  ...
  test_cross_context_image(
      reporter, options, "SkImage_MakeCrossContextFromPixmapRelease", [&pixmap](GrContext* ctx) {
...
DEF_GPUTEST(SkImage_CrossContextGrayAlphaConfigs, reporter, options) {
  for (SkColorType ct : {kGray_8_SkColorType, kAlpha_8_SkColorType}) {
    ...
    for (int i = 0; i < GrContextFactory::kContextTypeCnt; ++i) {
      ...
      sk_sp<GrTextureProxy> proxy =
          as_IB(image)->asTextureProxyRef(ctx, GrSamplerState::ClampNearest(), nullptr);
      REPORTER_ASSERT(reporter, proxy);
...
DEF_GPUTEST_FOR_ALL_CONTEXTS(ImageFlush, reporter, ctxInfo) {
  ...
  REPORTER_ASSERT(
      reporter, !static_cast<SkImage_GpuYUVA*>(as_IB(i2.get()))->testingOnly_IsFlattened());
