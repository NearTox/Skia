    ...
    switch (verb) {
      ...
      case SkPath::kQuad_Verb:
        SkDebugf(
            ...
            pts[1].fY, pts[2].fX, pts[2].fY);
        lastPt = pts[2];
        lastPtSet = true;
        break;
      case SkPath::kConic_Verb:
        SkDebugf(
            "{{%1.9g,%1.9g}, {%1.9g,%1.9g}, {%1.9g,%1.9g}},  //weight=%1.9g\n", pts[0].fX,
            pts[0].fY, pts[1].fX, pts[1].fY, pts[2].fX, pts[2].fY, iter.conicWeight());
        lastPt = pts[2];
        lastPtSet = true;
        break;
...
void showOp(const SkPathOp op) {
  switch (op) {
    case kDifference_SkPathOp: SkDebugf("op difference\n"); break;
    case kIntersect_SkPathOp: SkDebugf("op intersect\n"); break;
    case kUnion_SkPathOp: SkDebugf("op union\n"); break;
    case kXOR_SkPathOp: SkDebugf("op xor\n"); break;
    case kReverseDifference_SkPathOp: SkDebugf("op reverse difference\n"); break;
    default: SkASSERT(0);
  }
}
...
static int pathsDrawTheSame(
    SkBitmap& bits, const SkPath& scaledOne, const SkPath& scaledTwo, int& error2x2) {
  if (bits.width() == 0) {
...
static int pathsDrawTheSame(
    const SkPath& one, const SkPath& two, SkBitmap& bits, SkPath& scaledOne, SkPath& scaledTwo,
    int& error2x2) {
  SkMatrix scale;
...
int comparePaths(
    skiatest::Reporter* reporter, const char* filename, const SkPath& one, const SkPath& two,
    SkBitmap& bitmap) {
  int errors2x2;
...
static void showPathOpPath(
    const char* testName, const SkPath& one, const SkPath& two, const SkPath& a, const SkPath& b,
...
static int comparePaths(
    skiatest::Reporter* reporter, const char* testName, const SkPath& one, const SkPath& scaledOne,
    const SkPath& two, const SkPath& scaledTwo, SkBitmap& bitmap, const SkPath& a, const SkPath& b,
    const SkPathOp shapeOp, const SkMatrix& scale, ExpectMatch expectMatch) {
...
static void appendTest(
    const char* pathStr, const char* pathPrefix, const char* nameSuffix, const char* testFunction,
...
bool testSimplify(
    SkPath& path, bool useXor, SkPath& out, PathOpsThreadState& state, const char* pathStr) {
...
static void json_path_out(
    const SkPath& path, const char* pathName, const char* fillTypeName, bool lastField) {
  ...
  if (PathOpsDebug::gOutputSVG) {
    ...
    do {
      ...
      for (int i = pointIndex[verbIndex]; i < pointCount[verbIndex]; ++i) {
        fprintf(
            PathOpsDebug::gOut, ", \"0x%08x\", \"0x%08x\"", SkFloat2Bits(points[i].fX),
  ...
  fprintf(
      PathOpsDebug::gOut, "  \"fillType%s\": \"k%s_FillType\"%s", fillTypeName,
...
static bool check_for_duplicate_names(const char* testName) {
  if (PathOpsDebug::gCheckForDuplicateNames) {
    if (gUniqueNames.end() !=
        std::find(gUniqueNames.begin(), gUniqueNames.end(), std::string(testName))) {
...
static bool inner_simplify(
    skiatest::Reporter* reporter, const SkPath& path, const char* filename,
...
bool testSimplify(skiatest::Reporter* reporter, const SkPath& path, const char* filename) {
  return inner_simplify(
      reporter, path, filename, ExpectSuccess::kYes, SkipAssert::kNo, ExpectMatch::kYes);
}

bool testSimplifyFuzz(skiatest::Reporter* reporter, const SkPath& path, const char* filename) {
  return inner_simplify(
      reporter, path, filename, ExpectSuccess::kFlaky, SkipAssert::kYes, ExpectMatch::kFlaky);
}

bool testSimplifyCheck(
    skiatest::Reporter* reporter, const SkPath& path, const char* filename, bool checkFail) {
  return inner_simplify(
      reporter, path, filename, checkFail ? ExpectSuccess::kYes : ExpectSuccess::kNo,
      SkipAssert::kNo, ExpectMatch::kNo);
}

bool testSimplifyFail(skiatest::Reporter* reporter, const SkPath& path, const char* filename) {
  return inner_simplify(
      reporter, path, filename, ExpectSuccess::kNo, SkipAssert::kYes, ExpectMatch::kNo);
}
...
static bool innerPathOp(
    skiatest::Reporter* reporter, const SkPath& a, const SkPath& b, const SkPathOp shapeOp,
    const char* testName, ExpectSuccess expectSuccess, SkipAssert skipAssert,
...
bool testPathOp(
    ...
    const char* testName) {
  return innerPathOp(
      reporter, a, b, shapeOp, testName, ExpectSuccess::kYes, SkipAssert::kNo, ExpectMatch::kYes);
}

bool testPathOpCheck(
    skiatest::Reporter* reporter, const SkPath& a, const SkPath& b, const SkPathOp shapeOp,
    const char* testName, bool checkFail) {
  return innerPathOp(
      reporter, a, b, shapeOp, testName, checkFail ? ExpectSuccess::kYes : ExpectSuccess::kNo,
      SkipAssert::kNo, ExpectMatch::kNo);
}
...
bool testPathOpFuzz(
    ...
    const char* testName) {
  return innerPathOp(
      reporter, a, b, shapeOp, testName, ExpectSuccess::kFlaky, SkipAssert::kYes,
      ExpectMatch::kFlaky);
}
...
void RunTestSet(
    skiatest::Reporter* reporter, TestDesc tests[], size_t count,
