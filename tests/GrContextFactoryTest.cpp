...
DEF_GPUTEST(GrContextFactory_NVPRContextOptionHasPathRenderingSupport, reporter, options) {
  ...
  for (int i = 0; i < GrContextFactory::kContextTypeCnt; ++i) {
    ...
    GrContext* context =
        testFactory.get(ctxType, GrContextFactory::ContextOverrides::kRequireNVPRSupport);
    if (!context) {
      continue;
    }
    REPORTER_ASSERT(reporter, context->priv().caps()->shaderCaps()->pathRenderingSupport());
  }
}
...
DEF_GPUTEST(GrContextFactory_NoPathRenderingIfNVPRDisabled, reporter, options) {
  ...
  for (int i = 0; i <= GrContextFactory::kLastContextType; ++i) {
    ...
    GrContext* context = testFactory.get(ctxType, GrContextFactory::ContextOverrides::kDisableNVPR);
    if (context) {
      REPORTER_ASSERT(reporter, !context->priv().caps()->shaderCaps()->pathRenderingSupport());
    }
  }
