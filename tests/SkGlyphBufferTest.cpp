...
DEF_TEST(SkPackedGlyphIDCtor, reporter) {
  ...
  auto freqRound = [](uint32_t x) -> uint32_t { return ((x + 2) >> 2) & PG::kSubPixelPosMask; };
...
DEF_TEST(SkDrawableGlyphBufferBasic, reporter) {
...

  {
    ...
    for (auto [i, packedID, pos] : SkMakeEnumerate(accepted.input())) {
      ...
      REPORTER_ASSERT(
          reporter, pos.x() == SkScalarFloorToInt(
                                   positions[i].x() * 0.5 + 50 + SkPackedGlyphID::kSubpixelRound));
      REPORTER_ASSERT(reporter, pos.y() == SkScalarFloorToInt(positions[i].y() * 0.5 + 50 + 0.5));
    }
  }
