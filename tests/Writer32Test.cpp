...
static void check_contents(
    skiatest::Reporter* reporter, const SkWriter32& writer, const void* expected, size_t size) {
...
static void testOverwriteT(skiatest::Reporter* reporter, SkWriter32* writer) {
  ...
  REPORTER_ASSERT(
      reporter, writer->readTAt<SkScalar>(padding + sizeof(uint32_t) + sizeof(SkRect)) == scalar1);
  ...
  REPORTER_ASSERT(
      reporter, writer->readTAt<SkScalar>(padding + sizeof(uint32_t) + sizeof(SkRect)) == scalar2);
}
...
DEF_TEST(Writer32_data, reporter) {
  ...
  auto d0(reader.readByteArrayAsData()), d1(reader.readByteArrayAsData()),
