...
void check_save_state(
    skiatest::Reporter* reporter, SkPicture* picture, unsigned int numSaves,
    unsigned int numSaveLayers, unsigned int numRestores) {
...
static void test_clip_bound_opt(skiatest::Reporter* reporter) {
  ...
  SkRect rect1 =
      SkRect::MakeXYWH(SkIntToScalar(4), SkIntToScalar(4), SkIntToScalar(2), SkIntToScalar(2));
  SkRect rect2 =
      SkRect::MakeXYWH(SkIntToScalar(7), SkIntToScalar(7), SkIntToScalar(1), SkIntToScalar(1));
  SkRect rect3 =
      SkRect::MakeXYWH(SkIntToScalar(6), SkIntToScalar(6), SkIntToScalar(1), SkIntToScalar(1));
...
class ClipCountingCanvas : public SkCanvas {
 public:
  ClipCountingCanvas(int width, int height) : INHERITED(width, height), fClipCount(0) {}
...
struct CountingBBH : public SkBBoxHierarchy_Base {
  ...
  void search(const SkRect& query, SkTDArray<int>* results) const override { this->searchCalls++; }
...
class SpoonFedBBHFactory : public SkBBHFactory {
 public:
  ...
  sk_sp<SkBBoxHierarchy> operator()() const override { return fBBH; }
...
DEF_TEST(Placeholder, r) {
  ...
  sk_sp<SkPicture> p1 = SkPicture::MakePlaceholder(cull), p2 = SkPicture::MakePlaceholder(cull);
...
DEF_TEST(Picture_fillsBBH, r) {
  ...
  for (int n = 0; n <= 2; n++) {
    ...
    REPORTER_ASSERT(
        r, results.count() == n, "results.count() == %d, want %d\n", results.count(), n);
  }
}
