...
DEF_TEST(RecordOpts_SaveSaveLayerRestoreRestore, r) {
  ...
  switch (record.count()) {
    ...
    case 2:
      ...
      break;
    case 0: break;
    default: REPORTER_ASSERT(r, false);
  }
}

#ifndef SK_BUILD_FOR_ANDROID_FRAMEWORK
static void assert_savelayer_restore(
    skiatest::Reporter* r, SkRecord* record, int i, bool shouldBeNoOped) {
...
static void assert_savelayer_draw_restore(
    skiatest::Reporter* r, SkRecord* record, int i, bool shouldBeNoOped) {
  SkRecordNoopSaveLayerDrawRestores(record);
  if (shouldBeNoOped) {
    assert_type<SkRecords::NoOp>(r, *record, i);
...
DEF_TEST(RecordOpts_NoopSaveLayerDrawRestore, r) {
  ...
  // saveLayer w/ clip mask should also NOT go away
  {
    ...
    recorder.saveLayer({nullptr, nullptr, nullptr, surface->makeImageSnapshot().get(), nullptr, 0});
    recorder.drawRect(draw, opaqueDrawPaint);
    recorder.restore();
...
static void assert_merge_svg_opacity_and_filter_layers(
    skiatest::Reporter* r, SkRecord* record, int i, bool shouldBeNoOped) {
...
DEF_TEST(RecordOpts_MergeSvgOpacityAndFilterLayers, r) {
  ...
  colorFilterPaint.setColorFilter(SkColorFilters::Blend(SK_ColorLTGRAY, SkBlendMode::kSrcIn));
          ...
          for (size_t j = 0; j < SK_ARRAY_COUNT(firstPaints); ++j) {
            for (size_t k = 0; k < SK_ARRAY_COUNT(secondBounds); ++k) {
              for (size_t m = 0; m < SK_ARRAY_COUNT(secondPaints); ++m) {
                ...
                recorder.saveLayer({firstBounds[i], firstPaints[j], outerF, nullptr, nullptr, 0});
                recorder.save();
                recorder.clipRect(clip);
                recorder.saveLayer({secondBounds[k], secondPaints[m], innerF, nullptr, nullptr, 0});
                recorder.restore();
                recorder.restore();
                recorder.restore();
                assert_merge_svg_opacity_and_filter_layers(r, &record, index, outerNoOped);
#ifndef SK_BUILD_FOR_ANDROID_FRAMEWORK
...
DEF_TEST(savelayer_srcmode_opaque, r) { do_savelayer_srcmode(r, SK_ColorRED); }

DEF_TEST(savelayer_srcmode_alpha, r) { do_savelayer_srcmode(r, 0x80FF0000); }
