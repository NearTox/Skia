...
static void check_pixels(
    skiatest::Reporter* reporter, const SkBitmap& bitmap, GrSurfaceOrigin origin) {
  ...
  for (int cy = 0; cy < 8 && !failureFound; ++cy) {
    ...
    if (cy == 3) {
      ...
      if (canvasPixel != expectedPixel) {
        failureFound = true;
        ERRORF(
            reporter, "Wrong color at %d, %d. Got 0x%08x when we expected 0x%08x. Origin is: %s",
...
static void run_test(
    skiatest::Reporter* reporter, GrDirectContext* context, GrSurfaceOrigin origin) {
  auto beTexture = context->createBackendTexture(
      8, 8, kRGBA_8888_SkColorType, GrMipmapped::kNo, GrRenderable::kYes, GrProtected::kNo);
