...
static sk_sp<SkSL::SkVMDebugTrace> make_trace(skiatest::Reporter* r, std::string src) {
  ...
  std::unique_ptr<SkSL::Program> program =
      compiler.convertProgram(SkSL::ProgramKind::kGeneric, src, settings);
...
static std::string make_stack_string(
    const SkSL::SkVMDebugTrace& trace, const SkSL::SkVMDebugTracePlayer& player) {
...
DEF_TEST(SkSLTracePlayerHelloWorld, r) {
  sk_sp<SkSL::SkVMDebugTrace> trace = make_trace(
      r,
      R"(                // Line 1
int main() {       // Line 2
    return 2 + 2;  // Line 3
...
int main() {                          // Line 6
  ...
  REPORTER_ASSERT(
      r, player.getLineNumbersReached() ==
             LineNumberMap({{3, 1}, {4, 1}, {7, 1}, {8, 1}, {9, 1}, {10, 1}, {11, 1}, {12, 1}}));
  player.step();
  ...
  REPORTER_ASSERT(r, make_stack_string(*trace, player) == "int main()");
  REPORTER_ASSERT(
      r, make_local_vars_string(*trace, player) == "##[func].result = 6, b = true, a = 123");
  player.step();
  ...
  REPORTER_ASSERT(r, make_stack_string(*trace, player) == "int main()");
  REPORTER_ASSERT(
      r, make_local_vars_string(*trace, player) ==
...
int main() {                // Line 2
  ...
  player.reset(trace);
  REPORTER_ASSERT(
      r, player.getLineNumbersReached() == LineNumberMap(
                                               {{3, 1},
                                                {5, 1},
                                                {7, 1},
                                                {9, 1},
