...
bool GrShape::testingOnly_isPath() const { return Type::kPath == fType; }
...
static void check_equivalence(
    skiatest::Reporter* r, const GrShape& a, const GrShape& b, const Key& keyA, const Key& keyB) {
...
static void check_original_path_ids(
    skiatest::Reporter* r, const GrShape& base, const GrShape& pe, const GrShape& peStroke,
    const GrShape& full) {
...
class RectGeo : public Geo {
 public:
  ...
  GrShape makeShape(const SkPaint& paint) const override { return GrShape(fRect, paint); }
...
class RRectGeo : public Geo {
 public:
  ...
  GrShape makeShape(const SkPaint& paint) const override { return GrShape(fRRect, paint); }
...
class PathGeo : public Geo {
 public:
  ...
  GrShape makeShape(const SkPaint& paint) const override { return GrShape(fPath, paint); }
...
DEF_TEST(GrShape_empty_shape, reporter) {
  ...
  strokeEmptyRRectCase.compare(
      reporter, strokeEmptyCase, TestCase::kAllDifferent_ComparisonExpecation);
  ...
  strokeInvertedEmptyRRectCase.compare(
      reporter, strokeInvertedEmptyCase, TestCase::kAllDifferent_ComparisonExpecation);
...
void test_rrect(skiatest::Reporter* r, const SkRRect& rrect) {
      ...
      for (unsigned start = 0; start < 8; ++start) {
        for (Style style : {kFill, kStroke, kHairline, kStrokeAndFill}) {
          for (bool dash : {false, true}) {
            ...
            shapes[index(inverted, dir, start, style, dash)] = GrShape(
  ...
  const GrShape& exampleFillCase = shapes[index(false, kExamplesDir, kExamplesStart, kFill, false)];
  Key exampleFillCaseKey;
  make_key(&exampleFillCaseKey, exampleFillCase);

  const GrShape& exampleStrokeAndFillCase =
  ...
  const GrShape& exampleInvFillCase =
  ...
  const GrShape& exampleStrokeCase =
  ...
  const GrShape& exampleInvStrokeCase =
  ...
  const GrShape& exampleHairlineCase =
  ...
  const GrShape& exampleInvHairlineCase =
    ...
    for (SkPathDirection dir : {SkPathDirection::kCW, SkPathDirection::kCCW}) {
      for (unsigned start = 0; start < 8; ++start) {
        for (bool dash : {false, true}) {
          ...
          const GrShape& hairlineCase = shapes[index(inverted, dir, start, kHairline, dash)];
