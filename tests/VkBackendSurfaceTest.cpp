...
DEF_GPUTEST_FOR_VULKAN_CONTEXT(VkDRMModifierTest, reporter, ctxInfo) {
  ...
  sk_sp<SkImage> drmImage = SkImage::MakeFromTexture(
      dContext, drmBETex, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, kPremul_SkAlphaType,
      nullptr);
  REPORTER_ASSERT(reporter, drmImage);

  REPORTER_ASSERT(
      reporter, GrBackendTexture::TestingOnly_Equals(drmImage->getBackendTexture(false), drmBETex));
...
DEF_GPUTEST_FOR_VULKAN_CONTEXT(VkImageLayoutTest, reporter, ctxInfo) {
  ...
  sk_sp<SkImage> wrappedImage = SkImage::MakeFromTexture(
      dContext, backendTex1, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, kPremul_SkAlphaType,
