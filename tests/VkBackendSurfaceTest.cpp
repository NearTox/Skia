...
DEF_GPUTEST_FOR_VULKAN_CONTEXT(VkImageLayoutTest, reporter, ctxInfo) {
  ...
  CreateBackendTexture(
      context, &backendTex, 1, 1, kRGBA_8888_SkColorType, SkColors::kTransparent, GrMipMapped::kNo,
...
DEF_GPUTEST_FOR_VULKAN_CONTEXT(VkReleaseExternalQueueTest, reporter, ctxInfo) {
  ...
  for (bool useExternal : {false, true}) {
    GrBackendTexture backendTex = context->createBackendTexture(
        1, 1, kRGBA_8888_SkColorType, SkColors::kTransparent, GrMipMapped::kNo, GrRenderable::kNo,
        GrProtected::kNo);
    sk_sp<SkImage> image;
    int count = 0;
    if (useExternal) {
      ...
      image = SkImage::MakeFromTexture(
          context, vkExtTex, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, kPremul_SkAlphaType,
          nullptr, testing_release_proc, (void*)&count);

    } else {
      image = SkImage::MakeFromTexture(
          context, backendTex, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType,
          kPremul_SkAlphaType, nullptr, testing_release_proc, (void*)&count);
    }
...
DEF_GPUTEST_FOR_VULKAN_CONTEXT(VkPrepareForExternalIOQueueTransitionTest, reporter, ctxInfo) {
  ...
  for (bool useSurface : {false, true}) {
    for (bool preparePresent : {false, true}) {
      ...
      GrBackendTexture backendTex = context->createBackendTexture(
          4, 4, kRGBA_8888_SkColorType, SkColors::kTransparent, GrMipMapped::kNo,
          useSurface ? GrRenderable::kYes : GrRenderable::kNo, GrProtected::kNo);
      ...
      if (useSurface) {
        surface = SkSurface::MakeFromBackendTexture(
            context, vkExtTex, kTopLeft_GrSurfaceOrigin, 0, kRGBA_8888_SkColorType, nullptr,
            nullptr);
        ...
        image = SkImage::MakeFromTexture(
            context, vkExtTex, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType,
      ...
      if (useSurface) {
        newBackendTexture = surface->getBackendTexture(SkSurface::kFlushRead_TextureHandleAccess);
      } else {
      ...
      if (preparePresent) {
        REPORTER_ASSERT(reporter, newVkInfo.fCurrentQueueFamily == vkGpu->queueIndex());
        REPORTER_ASSERT(reporter, newVkInfo.fImageLayout == VK_IMAGE_LAYOUT_PRESENT_SRC_KHR);
      } else {
