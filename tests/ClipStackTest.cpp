...
static void test_bounds(
  ...
  static const SkClipOp gOps[] = {SkClipOp::kIntersect, SkClipOp::kDifference};
  ...
  for (int invBits = 0; invBits < numBitTests; ++invBits) {
    for (size_t op = 0; op < SK_ARRAY_COUNT(gOps); ++op) {
      ...
      if (SkClipStack::Element::DeviceSpaceType::kRect == primType) {
        REPORTER_ASSERT(reporter, isIntersectionOfRects == (gOps[op] == SkClipOp::kIntersect));
      } else {
...
static void set_region_to_stack(const SkClipStack& stack, const SkIRect& bounds, SkRegion* region) {
  ...
  while (const SkClipStack::Element* element = iter.next()) {
    ...
    region->op(
        elemRegion,
        element->isReplaceOp() ? SkRegion::kReplace_Op : (SkRegion::Op)element->getOp());
  }
}
