...
DEF_TEST(Time_GetDateTime, r) {
  ...
  if (dateTime.fYear < kMinimumSaneYear) {
    ERRORF(
        r, "SkTime::GetDateTime: %u (CurrentYear) < %u (MinimumSaneYear)",
        static_cast<unsigned>(dateTime.fYear), static_cast<unsigned>(kMinimumSaneYear));
  }
  if (dateTime.fYear > kMaximumSaneYear) {
    ERRORF(
        r, "SkTime::GetDateTime: %u (CurrentYear) > %u (MaximumSaneYear)",
        static_cast<unsigned>(dateTime.fYear), static_cast<unsigned>(kMaximumSaneYear));
  }
