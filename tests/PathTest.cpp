  ...
  for (auto [verb, pts, w] : SkPathPriv::Iterate(path)) {
    switch (verb) {
      ...
      case SkPathVerb::kLine:
        ...
        srcPts++;
        break;
      case SkPathVerb::kQuad: REPORTER_ASSERT(reporter, false, "unexpected quad verb"); break;
      case SkPathVerb::kConic: REPORTER_ASSERT(reporter, false, "unexpected conic verb"); break;
      case SkPathVerb::kCubic: REPORTER_ASSERT(reporter, false, "unexpected cubic verb"); break;
...
static void check_direction(
    skiatest::Reporter* reporter, const SkPath& path, SkPathFirstDirection expected) {
...
static void test_direction(skiatest::Reporter* reporter) {
  ...
  SkPath path;
  REPORTER_ASSERT(
      reporter, SkPathPriv::ComputeFirstDirection(path) == SkPathFirstDirection::kUnknown);
...
static void check_convexity(
    skiatest::Reporter* reporter, const SkPath& path, bool expectedConvexity) {
  // We make a copy so that we don't cache the result on the passed in path.
  SkPath copy(path);  // NOLINT(performance-unnecessary-copy-initialization)
...
static void test_convexity(skiatest::Reporter* reporter) {
  ...
  for (size_t i = 0; i < SK_ARRAY_COUNT(gRec); ++i) {
    ...
    if (kDontCheckDir != gRec[i].fExpectedDirection) {
      ...
      REPORTER_ASSERT(
          reporter, (gRec[i].fExpectedDirection == SkPathFirstDirection::kUnknown) ^ foundDir);
  ...
  for (int index = 0; index < (int)(13 * nonFinitePtsCount * axisAlignedPtsCount); ++index) {
    ...
    REPORTER_ASSERT(reporter, SkPathPriv::GetConvexityOrUnknown(path) == SkPathConvexity::kUnknown);
  }
...
static void check_simple_rect(
    skiatest::Reporter* reporter, const SkPath& path, bool isClosed, const SkRect& rect,
      ...
      for (auto [v, verbPts, w] : SkPathPriv::Iterate(path)) {
        switch (v) {
          case SkPathVerb::kMove:
            ...
            break;
          case SkPathVerb::kLine: path2.lineTo(verbPts[1]); break;
          default: break;
        }
      }
...
static void write_and_read_back(skiatest::Reporter* reporter, const SkPath& p) {
  ...
  REPORTER_ASSERT(
      reporter,
      SkPathPriv::GetConvexityOrUnknown(readBack) == SkPathPriv::GetConvexityOrUnknown(p));
...
#define CONIC_PERSPECTIVE_BUG_FIXED 0
...

  {
    ...
    p.transform(matrix, &p1);
    REPORTER_ASSERT(
        reporter, SkPathPriv::ComputeFirstDirection(p1) == SkPathFirstDirection::kUnknown);
  }
    ...
    for (int j = 0; j < numVerbs; ++j) {
      ...
      switch (nextVerb) {
        ...
        case SkPathVerb::kConic:
          ...
          expectedPts[numPoints] = randomPts[(rand.nextU() >> 16) % 25];
          expectedPts[numPoints + 1] = randomPts[(rand.nextU() >> 16) % 25];
          path.conicTo(expectedPts[numPoints], expectedPts[numPoints + 1], rand.nextUScalar1() * 4);
          numPoints += 2;
          lastWasClose = false;
          break;
        case SkPathVerb::kCubic:
          ...
          path.cubicTo(
              expectedPts[numPoints], expectedPts[numPoints + 1], expectedPts[numPoints + 2]);
          numPoints += 3;
          lastWasClose = false;
          break;
        case SkPathVerb::kClose:
    ...
    for (auto [verb, pts, w] : SkPathPriv::Iterate(path)) {
      ...
      switch (verb) {
        ...
        case SkPathVerb::kCubic:
          ...
          numIterPts += 3;
          break;
        case SkPathVerb::kClose: lastPt = lastMoveTo; break;
        default: SkDEBUGFAIL("unexpected verb");
      }
    }
    REPORTER_ASSERT(reporter, numIterPts == numPoints);
...
static void test_circle_skew(
    skiatest::Reporter* reporter, const SkPath& path, SkPathFirstDirection dir) {
  SkPath tmp;
...
static void test_rrect_is_convex(skiatest::Reporter* reporter, SkPath* path, SkPathDirection dir) {
  REPORTER_ASSERT(reporter, path->isConvex());
  REPORTER_ASSERT(
      reporter, SkPathPriv::ComputeFirstDirection(*path) == SkPathPriv::AsFirstDirection(dir));
...
static void check_move(
    skiatest::Reporter* reporter, SkPathPriv::RangeIter* iter, SkScalar x0, SkScalar y0) {
  auto [v, pts, w] = *(*iter)++;
...
static void check_line(
    skiatest::Reporter* reporter, SkPathPriv::RangeIter* iter, SkScalar x1, SkScalar y1) {
  auto [v, pts, w] = *(*iter)++;
...
static void check_done_and_reset(
    skiatest::Reporter* reporter, SkPath* p, SkPathPriv::RangeIter* iter) {
...
static void assert_points(
    skiatest::Reporter* reporter, const SkPath& path, const std::initializer_list<SkPoint>& list) {
  const SkPoint* expected = list.begin();
  SkPath::RawIter iter(path);
  for (size_t i = 0;;) {
    SkPoint pts[4];
    switch (iter.next(pts)) {
      case SkPath::kDone_Verb: REPORTER_ASSERT(reporter, i == list.size()); return;
      case SkPath::kMove_Verb:
...
static void test_addPath_and_injected_moveTo(skiatest::Reporter* reporter) {
   ...
   */
  auto test_before_after_lineto =
      [reporter](SkPath& path, SkPoint expectedLastPt, SkPoint expectedMoveTo) {
...
DEF_TEST(path_convexity_scale_way_down, r) {
  SkPath path = SkPathBuilder().moveTo(0, 0).lineTo(1, 0).lineTo(1, 1).lineTo(0, 1).detach();
...
DEF_TEST(path_moveto_addrect, r) {
  ...
  SkRect rects[] = {{207.0f, 237.0f, 300.0f, 237.0f}, {207.0f, 237.0f, 300.0f, 267.0f}};
