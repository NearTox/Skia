...
const SkScalar widths[] = {
    -FLT_MAX, -1,     -0.1f,  -FLT_EPSILON, 0,      FLT_EPSILON, 0.0000001f, 0.000001f, 0.00001f,
    0.0001f,  0.001f, 0.01f,  0.1f,         0.2f,   0.3f,        0.4f,       0.5f,      1,
    1.1f,     2,      10,     10e2f,        10e3f,  10e4f,       10e5f,      10e6f,     10e7f,
...
DEF_TEST(QuadStrokerUnbounded, reporter) {
  ...
  for (int i = 0; i < 1000000; ++i) {
    ...
    if (best < gMaxRecursion[2]) {
      if (reporter->verbose()) {
        SkDebugf("\n%s quad=%d width=%1.9g\n", __FUNCTION__, gMaxRecursion[2], p.getStrokeWidth());
        path.dumpHex();
        SkDebugf("fill:\n");
        fill.dumpHex();
      }
...
DEF_TEST(CubicStrokerUnbounded, reporter) {
  ...
  for (int i = 0; i < 1000000; ++i) {
    ...
    path.moveTo(unbounded(r), unbounded(r));
    path.cubicTo(
        unbounded(r), unbounded(r), unbounded(r), unbounded(r), unbounded(r), unbounded(r));
    p.setStrokeWidth(unboundedPos(r));
    p.getFillPath(path, &fill);
#if defined(SK_DEBUG) && QUAD_STROKE_APPROX_EXTENDED_DEBUGGING
    if (bestTan < gMaxRecursion[0] || bestCubic < gMaxRecursion[1]) {
      if (reporter->verbose()) {
        SkDebugf(
            "\n%s tan=%d cubic=%d width=%1.9g\n", __FUNCTION__, gMaxRecursion[0], gMaxRecursion[1],
            p.getStrokeWidth());
        path.dumpHex();
        SkDebugf("fill:\n");
        fill.dumpHex();
...
DEF_TEST(CubicStrokerRange, reporter) {
  ...
  for (int i = 0; i < 1000000; ++i) {
    ...
    path.cubicTo(
        r.nextRangeF(0, 500), r.nextRangeF(0, 500), r.nextRangeF(0, 500), r.nextRangeF(0, 500),
        r.nextRangeF(0, 500), r.nextRangeF(0, 500));
...
DEF_TEST(QuadStrokerOneOff, reporter) {
  ...
  path.quadTo(
      SkBits2Float(0x4285d839), SkBits2Float(0x43ed6645), SkBits2Float(0x43c941c8),
      SkBits2Float(0x42b3ace3));
  p.getFillPath(path, &fill);
  if (reporter->verbose()) {
    SkDebugf("\n%s path\n", __FUNCTION__);
    path.dump();
...
DEF_TEST(CubicStrokerOneOff, reporter) {
  ...
  path.cubicTo(
      SkBits2Float(0x4331cb76), SkBits2Float(0x43ea3340), SkBits2Float(0x4388f498),
      SkBits2Float(0x42f7f08d), SkBits2Float(0x43f1cd32), SkBits2Float(0x42802ec1));
  p.getFillPath(path, &fill);
  if (reporter->verbose()) {
    SkDebugf("\n%s path\n", __FUNCTION__);
    path.dump();
