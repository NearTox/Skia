...
static std::unique_ptr<skgpu::v1::SurfaceDrawContext> random_surface_draw_context(
  ...
  return skgpu::v1::SurfaceDrawContext::Make(
      ...
      {kRenderTargetWidth, kRenderTargetHeight}, SkSurfaceProps(), sampleCnt, GrMipmapped::kNo,
...
bool GrDrawingManager::ProgramUnitTest(GrDirectContext* direct, int maxStages, int maxLevels) {
  ...
  GrMipmapped mipmapped = GrMipmapped(caps->mipmapSupport());
  {
    ...
    auto proxy = proxyProvider->createProxy(
        format, kDims, GrRenderable::kYes, 1, mipmapped, SkBackingFit::kExact, SkBudgeted::kNo,
  ...
  }
  {
    ...
    auto proxy = proxyProvider->createProxy(
        format, kDims, GrRenderable::kNo, 1, mipmapped, SkBackingFit::kExact, SkBudgeted::kNo,
        GrProtected::kNo, /*label=*/{}, GrInternalSurfaceFlags::kNone);
...
static void test_programs(skiatest::Reporter* reporter, const sk_gpu_test::ContextInfo& ctxInfo) {
  ...
  REPORTER_ASSERT(
      reporter, GrDrawingManager::ProgramUnitTest(ctxInfo.directContext(), maxStages, maxLevels));
}
