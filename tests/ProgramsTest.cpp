...
static std::unique_ptr<GrRenderTargetContext> random_render_target_context(
  ...
  return GrRenderTargetContext::Make(
      ...
      {kRenderTargetWidth, kRenderTargetHeight}, sampleCnt, GrMipMapped::kNo, GrProtected::kNo,
      origin);
}
...
static std::unique_ptr<GrFragmentProcessor> create_random_proc_tree(
    GrProcessorTestData* d, int minLevels, int maxLevels) {
  ...
  if (d->fRandom->nextF() < 0.5f) {
    fp = GrXfermodeFragmentProcessor::MakeFromTwoProcessors(
        std::move(minLevelsChild), std::move(otherChild), mode);
    SkASSERT(fp);
  } else {
    fp = GrXfermodeFragmentProcessor::MakeFromTwoProcessors(
        std::move(otherChild), std::move(minLevelsChild), mode);
    SkASSERT(fp);
  }
  return fp;
}

static void set_random_color_coverage_stages(
    GrPaint* paint, GrProcessorTestData* d, int maxStages, int maxTreeLevels) {
...
bool GrDrawingManager::ProgramUnitTest(GrContext* context, int maxStages, int maxLevels) {
  ...
  GrMipMapped mipMapped = GrMipMapped(context->priv().caps()->mipMapSupport());
  {
    ...
    proxies[0] = {
        proxyProvider->createProxy(
            format, dummyDesc, swizzle, GrRenderable::kYes, 1, kBottomLeft_GrSurfaceOrigin,
            mipMapped, SkBackingFit::kExact, SkBudgeted::kNo, GrProtected::kNo,
  ...
  }
  {
    ...
    proxies[1] = {
        proxyProvider->createProxy(
            format, dummyDesc, swizzle, GrRenderable::kNo, 1, kTopLeft_GrSurfaceOrigin, mipMapped,
...
static void test_programs(skiatest::Reporter* reporter, const sk_gpu_test::ContextInfo& ctxInfo) {
  ...
  REPORTER_ASSERT(
      reporter, GrDrawingManager::ProgramUnitTest(ctxInfo.grContext(), maxStages, maxLevels));
}
...
DEF_GPUTEST(Programs, reporter, options) {
  ...
  skiatest::RunWithGPUTestContexts(
      test_programs, &skiatest::IsRenderingGLOrMetalContextType, reporter, opts);
}
