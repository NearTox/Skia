...
sk_sp<SkSurface> VulkanTestHelper::createSkSurface(skiatest::Reporter* reporter) {
  ...
  sk_sp<SkSurface> surface = SkSurface::MakeFromBackendTexture(
      this->grContext(), backendTex, kTopLeft_GrSurfaceOrigin, 1, kRGBA_8888_SkColorType, nullptr,
      &surfaceProps);
...
DEF_GPUTEST(VkProtectedContext_CreateProtectedSkSurface, reporter, options) {
  ...
  const int kW = 8;
  const int kH = 8;
  GrBackendTexture backendTex = protectedTestHelper->grContext()->createBackendTexture(
