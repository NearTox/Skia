...
static sk_sp<SkSurface> create_protected_sksurface(
    GrDirectContext* dContext, skiatest::Reporter* reporter, bool textureable = true) {
  const int kW = 8;
  const int kH = 8;
  ...
  if (textureable) {
    surface = sk_gpu_test::MakeBackendTextureSurface(
        dContext, {kW, kH}, kTopLeft_GrSurfaceOrigin, 1, kRGBA_8888_SkColorType,
        /* color space */ nullptr, GrMipmapped::kNo, GrProtected::kYes, &surfaceProps);
  } else {
    surface = sk_gpu_test::MakeBackendRenderTargetSurface(
        dContext, {kW, kH}, kTopLeft_GrSurfaceOrigin, 1, kRGBA_8888_SkColorType,
        /* color space */ nullptr, GrProtected::kYes, &surfaceProps);
  }
  if (!surface) {
...
DEF_GPUTEST(VkProtectedContext_CreateNonprotectedTextureInProtectedContext, reporter, options) {
  ...
  const int kW = 8;
  const int kH = 8;
  GrBackendTexture backendTex = protectedTestHelper->directContext()->createBackendTexture(
      kW, kH, kRGBA_8888_SkColorType, GrMipmapped::kNo, GrRenderable::kNo, GrProtected::kNo);
  REPORTER_ASSERT(reporter, !backendTex.isValid());
}
...
DEF_GPUTEST(VkProtectedContext_CreateProtectedTextureInNonprotectedContext, reporter, options) {
  ...
  const int kH = 8;
  GrBackendTexture backendTex = protectedTestHelper->directContext()->createBackendTexture(
      kW, kH, kRGBA_8888_SkColorType, GrMipmapped::kNo, GrRenderable::kNo, GrProtected::kYes);
  REPORTER_ASSERT(reporter, !backendTex.isValid());
}
...
DEF_GPUTEST(VkProtectedContext_AsyncReadFromProtectedSurface, reporter, options) {
  ...
  surface->asyncRescaleAndReadPixelsYUV420(
      kIdentity_SkYUVColorSpace, SkColorSpace::MakeSRGB(), image_info.bounds(),
      image_info.dimensions(), SkSurface::RescaleGamma::kSrc, SkSurface::RescaleMode::kNearest,
...
DEF_GPUTEST(VkProtectedContext_DrawRectangleWithFilter, reporter, options) {
  ...
  paint.setMaskFilter(SkMaskFilter::MakeBlur(SkBlurStyle::kOuter_SkBlurStyle, 1.1f));
  canvas->drawRect(SkRect::MakeWH(4, 4), paint);
}
