...
class BaseTestHelper {
 public:
  ...
  virtual sk_sp<SkImage> importHardwareBufferForRead(
      skiatest::Reporter* reporter, AHardwareBuffer* buffer) = 0;
  virtual sk_sp<SkSurface> importHardwareBufferForWrite(
      skiatest::Reporter* reporter, AHardwareBuffer* buffer) = 0;

  virtual void doClientSync() = 0;
  virtual bool flushSurfaceAndSignalSemaphore(skiatest::Reporter* reporter, sk_sp<SkSurface>) = 0;
  virtual bool importAndWaitOnSemaphore(
      skiatest::Reporter* reporter, int fdHandle, sk_sp<SkSurface>) = 0;
...
class EGLTestHelper : public BaseTestHelper {
 public:
  ...
  void cleanup() override { this->releaseImage(); }

  bool init(skiatest::Reporter* reporter) override;

  sk_sp<SkImage> importHardwareBufferForRead(
      skiatest::Reporter* reporter, AHardwareBuffer* buffer) override;
  sk_sp<SkSurface> importHardwareBufferForWrite(
      skiatest::Reporter* reporter, AHardwareBuffer* buffer) override;

  void doClientSync() override;
  bool flushSurfaceAndSignalSemaphore(skiatest::Reporter* reporter, sk_sp<SkSurface>) override;
  bool importAndWaitOnSemaphore(
      skiatest::Reporter* reporter, int fdHandle, sk_sp<SkSurface>) override;
 ...
 private:
  ...
  typedef EGLImageKHR (*EGLCreateImageKHRProc)(
...
bool EGLTestHelper::importHardwareBuffer(skiatest::Reporter* reporter, AHardwareBuffer* buffer) {
  ...
  fImage = fEGLCreateImageKHR(
      eglDisplay, EGL_NO_CONTEXT, EGL_NATIVE_BUFFER_ANDROID, eglClientBuffer, eglAttribs);
  if (EGL_NO_IMAGE_KHR == fImage) {
...
sk_sp<SkImage> EGLTestHelper::importHardwareBufferForRead(
    skiatest::Reporter* reporter, AHardwareBuffer* buffer) {
  ...
  sk_sp<SkImage> image = SkImage::MakeFromTexture(
      fGrContext, backendTex, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, kPremul_SkAlphaType,
      nullptr);
...
sk_sp<SkSurface> EGLTestHelper::importHardwareBufferForWrite(
    skiatest::Reporter* reporter, AHardwareBuffer* buffer) {
  if (!this->importHardwareBuffer(reporter, buffer)) {
    return nullptr;
  }
  ...
  sk_sp<SkSurface> surface = SkSurface::MakeFromBackendTexture(
      fGrContext, backendTex, kTopLeft_GrSurfaceOrigin, 0, kRGBA_8888_SkColorType, nullptr,
      nullptr);
...
bool EGLTestHelper::flushSurfaceAndSignalSemaphore(
...
bool EGLTestHelper::importAndWaitOnSemaphore(
    skiatest::Reporter* reporter, int fdHandle, sk_sp<SkSurface> surface) {
  EGLDisplay eglDisplay = eglGetCurrentDisplay();
  EGLint attr[] = {EGL_SYNC_NATIVE_FENCE_FD_ANDROID, fdHandle, EGL_NONE};
  EGLSyncKHR eglsync = fEGLCreateSyncKHR(eglDisplay, EGL_SYNC_NATIVE_FENCE_ANDROID, attr);
  if (EGL_NO_SYNC_KHR == eglsync) {
    ERRORF(reporter, "Failed to create EGLSync when importing EGL_SYNC_NATIVE_FENCE_FD_ANDROID\n");
    return false;
  }
...
#  define ACQUIRE_INST_VK_PROC(name)                                            \
    fVk##name = reinterpret_cast<PFN_vk##name>(                                 \
        getProc("vk" #name, fBackendContext.fInstance, VK_NULL_HANDLE));        \
    if (fVk##name == nullptr) {                                                 \
...
#  endif
 ...
 private:
  ...
  bool importHardwareBuffer(
      skiatest::Reporter* reporter, AHardwareBuffer* buffer, bool forWrite,
...
bool VulkanTestHelper::init(skiatest::Reporter* reporter) {
  ...
  if (!sk_gpu_test::CreateVkBackendContext(
          getProc, &fBackendContext, fExtensions, fFeatures, &fDebugCallback)) {
    return false;
  }
...
bool VulkanTestHelper::checkOptimalHardwareBuffer(skiatest::Reporter* reporter) {
  ...
  err = fVkGetPhysicalDeviceImageFormatProperties2(
      fBackendContext.fPhysicalDevice, &imageFormatInfo, &imgFormProps);
  if (VK_SUCCESS != err) {
  ...
  REPORTER_ASSERT(
      reporter,
      SkToBool(AHARDWAREBUFFER_USAGE_GPU_SAMPLED_IMAGE & hwbUsage.androidHardwareBufferUsage));

  return true;
}

bool VulkanTestHelper::importHardwareBuffer(
    skiatest::Reporter* reporter, AHardwareBuffer* buffer, bool forWrite,
    GrVkImageInfo* outImageInfo) {
  VkResult err;
  ...
  REPORTER_ASSERT(
      reporter, SkToBool(VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT & hwbFormatProps.formatFeatures) &&
  ...
  if (forWrite) {
    REPORTER_ASSERT(
        reporter, SkToBool(VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BIT & hwbFormatProps.formatFeatures));
  }
...
sk_sp<SkImage> VulkanTestHelper::importHardwareBufferForRead(
    skiatest::Reporter* reporter, AHardwareBuffer* buffer) {
  ...
  sk_sp<SkImage> wrappedImage = SkImage::MakeFromTexture(
      fGrContext.get(), backendTex, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType,
...
bool VulkanTestHelper::setupSemaphoreForSignaling(
    skiatest::Reporter* reporter, GrBackendSemaphore* beSemaphore) {
  ...
  fVkGetPhysicalDeviceExternalSemaphoreProperties(
      fBackendContext.fPhysicalDevice, &exSemInfo, &exSemProps);

  if (!SkToBool(
          exSemProps.exportFromImportedHandleTypes &
          VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_SYNC_FD_BIT)) {
    ERRORF(reporter, "HANDLE_TYPE_SYNC_FD not listed as exportFromImportedHandleTypes");
    return false;
  }
  if (!SkToBool(exSemProps.compatibleHandleTypes & VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_SYNC_FD_BIT)) {
    ERRORF(reporter, "HANDLE_TYPE_SYNC_FD not listed as compatibleHandleTypes");
    return false;
  }
  if (!SkToBool(
          exSemProps.externalSemaphoreFeatures & VK_EXTERNAL_SEMAPHORE_FEATURE_EXPORTABLE_BIT) ||
      !SkToBool(
          exSemProps.externalSemaphoreFeatures & VK_EXTERNAL_SEMAPHORE_FEATURE_IMPORTABLE_BIT)) {
...
bool VulkanTestHelper::exportSemaphore(
    skiatest::Reporter* reporter, const GrBackendSemaphore& beSemaphore) {
...
bool VulkanTestHelper::importAndWaitOnSemaphore(
    skiatest::Reporter* reporter, int fdHandle, sk_sp<SkSurface> surface) {
  VkSemaphoreCreateInfo semaphoreInfo;
  semaphoreInfo.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO;
...
sk_sp<SkSurface> VulkanTestHelper::importHardwareBufferForWrite(
    skiatest::Reporter* reporter, AHardwareBuffer* buffer) {
  GrVkImageInfo imageInfo;
  ...
  sk_sp<SkSurface> surface = SkSurface::MakeFromBackendTexture(
      fGrContext.get(), backendTex, kTopLeft_GrSurfaceOrigin, 0, kRGBA_8888_SkColorType, nullptr,
      nullptr);
...
static bool check_read(
    skiatest::Reporter* reporter, const SkBitmap& srcBitmap, const SkBitmap& dstBitmap) {
  bool result = true;
  for (int y = 0; y < DEV_H && result; ++y) {
    for (int x = 0; x < DEV_W && result; ++x) {
      ...
      if (srcPixel != dstPixel) {
        ERRORF(
            reporter, "Expected readback pixel (%d, %d) value 0x%08x, got 0x%08x.", x, y, srcPixel,
            dstPixel);
        result = false;
      } /*else {
...
static void cleanup_resources(
    BaseTestHelper* srcHelper, BaseTestHelper* dstHelper, AHardwareBuffer* buffer) {
  if (srcHelper) {
    srcHelper->cleanup();
  }
  if (dstHelper) {
...
void run_test(
    skiatest::Reporter* reporter, const GrContextOptions& options, SrcType srcType, DstType dstType,
    bool shareSyncs) {
  ...
  if (SrcType::kCPU == srcType) {
    ...
    for (int y = 0; y < DEV_H; ++y) {
      for (int x = 0; x < DEV_W; ++x) {
        ...
        if (srcPixel != dstPixel) {
          ERRORF(
              reporter, "CPU HWB Expected readpix (%d, %d) value 0x%08x, got 0x%08x.", x, y,
              srcPixel, dstPixel);
        }
      }
    }
  ...
  sk_sp<SkSurface> dstSurf = SkSurface::MakeRenderTarget(
      grContext, SkBudgeted::kNo, imageInfo, 0, kTopLeft_GrSurfaceOrigin, nullptr, false);
  if (!dstSurf.get()) {
