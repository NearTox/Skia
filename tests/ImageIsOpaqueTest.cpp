...
static void check_isopaque(
    skiatest::Reporter* reporter, const sk_sp<SkSurface>& surface, bool expectedOpaque) {
...
DEF_TEST(Image_isAlphaOnly, reporter) {
  ...
  SkPixmap pmap = {SkImageInfo::MakeN32Premul(1, 1), &pmColors, sizeof(pmColors)};
  for (auto& image : {
           ...
           SkImage::MakeFromPicture(
               make_picture(), {10, 10}, nullptr, nullptr, SkImage::BitDepth::kU8,
  ...
  REPORTER_ASSERT(
      reporter,
      SkImage::MakeRasterCopy({SkImageInfo::MakeA8(1, 1), (uint8_t*)&pmColors, 1})->isAlphaOnly() ==
          true);
}
