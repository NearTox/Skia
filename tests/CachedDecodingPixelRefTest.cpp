...
class TestImageGenerator : public SkImageGenerator {
 public:
  ...
  TestImageGenerator(
      TestType type, skiatest::Reporter* reporter, SkColorType colorType = kN32_SkColorType)
 ...
 protected:
  static SkImageInfo GetMyInfo(SkColorType colorType) {
    return SkImageInfo::Make(
        TestImageGenerator::Width(), TestImageGenerator::Height(), colorType, kOpaque_SkAlphaType);
  }

  bool onGetPixels(
      const SkImageInfo& info, void* pixels, size_t rowBytes, const Options& options) override {
    ...
    switch (info.colorType()) {
      case kN32_SkColorType:
        for (int y = 0; y < info.height(); ++y) {
          sk_memset32((uint32_t*)bytePtr, TestImageGenerator::PMColor(), info.width());
          bytePtr += rowBytes;
        }
        break;
      case kRGB_565_SkColorType:
        for (int y = 0; y < info.height(); ++y) {
          sk_memset16(
              (uint16_t*)bytePtr, SkPixel32ToPixel16(TestImageGenerator::PMColor()), info.width());
          bytePtr += rowBytes;
        }
        break;
...
DEF_TEST(Image_NewFromGenerator, r) {
  ...
  const SkColorType testColorTypes[] = {kN32_SkColorType, kRGB_565_SkColorType};
  for (size_t i = 0; i < SK_ARRAY_COUNT(testTypes); ++i) {
    ...
    for (const SkColorType testColorType : testColorTypes) {
      ...
      if (TestImageGenerator::kSucceedGetPixels_TestType == test) {
        REPORTER_ASSERT(r, TestImageGenerator::Color() == bitmap.getColor(0, 0));
      } else {
