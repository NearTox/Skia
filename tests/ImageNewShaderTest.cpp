...
static void paint_source(SkSurface* sourceSurface) {
  ...
  SkRect rect = SkRect::MakeXYWH(
      SkIntToScalar(1), SkIntToScalar(0), SkIntToScalar(1), SkIntToScalar(sourceSurface->height()));

  sourceCanvas->drawRect(rect, paintColor);
}

static void run_shader_test(
...
DEF_TEST(ImageRawShader, reporter) {
  ...
  REPORTER_ASSERT(reporter, !image->makeRawShader(SkSamplingOptions{SkCubicResampler::Mitchell()}));
}
