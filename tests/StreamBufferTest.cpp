...
static void test_get_data_at_position(
    skiatest::Reporter* r, SkStreamBuffer* buffer, size_t position, size_t length) {
...
static void test_buffer_from_beginning(
    skiatest::Reporter* r, std::unique_ptr<SkStream> stream, size_t length) {
  if (!stream) {
    return;
  }
...
DEF_TEST(StreamBuffer, r) {
  ...
  struct {
    ...
    bool skipIfNoTmpDir;
      ...
      {[&path]() {
         return path.isEmpty() ? nullptr : skstd::make_unique<SkFILEStream>(path.c_str());
       },
       true},
  };
