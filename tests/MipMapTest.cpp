...
DEF_TEST(MipMap, reporter) {
  ...
  for (int i = 0; i < 500; ++i) {
    ...
    REPORTER_ASSERT(
        reporter, !mm->extractLevel(SkSize::Make(SK_Scalar1 * 2, SK_Scalar1 * 2), nullptr));
...
static void test_mipmap_generation(
    int width, int height, int expectedMipLevelCount, skiatest::Reporter* reporter) {
  SkBitmap bm;
  bm.allocN32Pixels(width, height);
  bm.eraseColor(SK_ColorWHITE);
  sk_sp<SkMipmap> mm(SkMipmap::Build(bm, nullptr));
...
DEF_TEST(MipMap_ComputeLevelCount, reporter) {
  const LevelCountScenario tests[] = {// Test mipmaps with negative sizes
                                      {-100, 100, 0},
                                      {100, -100, 0},
                                      {-100, -100, 0},
...
DEF_TEST(MipMap_ComputeLevelSize, reporter) {
  ...
  for (auto& currentTest : tests) {
    SkISize levelSize = SkMipmap::ComputeLevelSize(
        currentTest.fBaseWidth, currentTest.fBaseHeight, currentTest.fLevel);
...
static void fill_in_mips(SkMipmapBuilder* builder, sk_sp<SkImage> img) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    surf->getCanvas()->drawImageRect(
        img, SkRect::MakeIWH(pm.width(), pm.height()), SkSamplingOptions());
  }
}
