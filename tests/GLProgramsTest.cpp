...
bool GrDrawingManager::ProgramUnitTest(GrContext* context, int maxStages, int maxLevels) {
  ...
  for (int t = 0; t < NUM_TESTS; t++) {
    ...
    GrPaint paint;
    GrProcessorTestData ptd(&random, context, renderTargetContext.get(), proxies, proxyColorTypes);
  ...
  for (int i = 0; i < fpFactoryCnt; ++i) {
    ...
    for (int j = 0; j < 10; ++j) {
      GrProcessorTestData ptd(
          &random, context, renderTargetContext.get(), proxies, proxyColorTypes);
