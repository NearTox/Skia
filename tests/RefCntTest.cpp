...
class Effect {
 public:
  Effect() : fRefCnt(1) { gNewCounter += 1; }
  virtual ~Effect() {}
...
static sk_sp<Effect> Create() { return sk_make_sp<Effect>(); }
...
class Paint {
 public:
  ...
  void set(sk_sp<Effect> value) { fEffect = std::move(value); }
};

struct EffectImpl : public Effect {
  ~EffectImpl() override {}

  static sk_sp<EffectImpl> Create() { return sk_sp<EffectImpl>(new EffectImpl); }
  int fValue;
};
