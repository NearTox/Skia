...
sk_sp<SkImage> create_image(GrContext* context, const GrBackendTexture& backendTex) {
  ...
  return SkImage::MakeFromCompressedTexture(
      context, backendTex, kTopLeft_GrSurfaceOrigin, at, nullptr);
}
...
static void check_compressed_mipmaps(
    GrContext* context, sk_sp<SkImage> img, SkImage::CompressionType compressionType,
...
static void test_compressed_color_init(
    ...
    std::function<GrBackendTexture(GrContext*, const SkColor4f&, GrMipMapped)> create,
  ...
  check_compressed_mipmaps(
      context, img, compression, expectedColors, mipMapped, reporter, "colorinit");
  check_readback(context, std::move(img), compression, color, reporter, "solid readback");
...
static void test_compressed_data_init(
    GrContext* context, skiatest::Reporter* reporter,
    std::function<GrBackendTexture(GrContext*, const char* data, size_t dataSize, GrMipMapped)>
        create,
  ...
  check_compressed_mipmaps(
      context, img, compression, expectedColors, mipMapped, reporter, "pixmap");
  check_readback(
      context, std::move(img), compression, expectedColors[0], reporter, "data readback");
    ...
    for (auto mipMapped : {GrMipMapped::kNo, GrMipMapped::kYes}) {
      ...
      // color initialized
      {
        auto createWithColorMtd =
            [format](GrContext* context, const SkColor4f& color, GrMipMapped mipMapped) {
              return context->createCompressedBackendTexture(
                  32, 32, format, color, mipMapped, GrProtected::kNo);
            };

        test_compressed_color_init(
            context, reporter, createWithColorMtd, combo.fColor, combo.fCompression, mipMapped);
      }

      // data initialized
      {
        auto createWithDataMtd =
            [format](GrContext* context, const char* data, size_t dataSize, GrMipMapped mipMapped) {
              return context->createCompressedBackendTexture(
