...
static void text_blob_cache_inner(
    skiatest::Reporter* reporter, GrDirectContext* dContext, int maxTotalText, int maxGlyphID,
    int maxFamilies, bool normal, bool stressTest) {
  // setup surface
  uint32_t flags = 0;
        ...
        for (int subpixel = 0; subpixel < 2; subpixel++) {
          for (int lcd = 0; lcd < 2; lcd++) {
            ...
            const SkTextBlobBuilder::RunBuffer& run =
                builder.allocRun(font, maxTotalText, 0, 0, nullptr);
