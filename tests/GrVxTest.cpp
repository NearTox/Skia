...
DEF_TEST(grvx_approx_angle_between_vectors, r) {
  ...
  REPORTER_ASSERT(
      r, SkScalarNearlyZero(
             grvx::approx_angle_between_vectors<1>(std::numeric_limits<float>::infinity(), 1, 2, 3)
                 .val));

  // Test NaNs.
  REPORTER_ASSERT(
      r, SkScalarNearlyZero(
             grvx::approx_angle_between_vectors<1>(std::numeric_limits<float>::quiet_NaN(), 1, 2, 3)
                 .val));
  ...
  REPORTER_ASSERT(
      r, SkScalarNearlyZero(
             grvx::approx_angle_between_vectors<1>(epsilon, epsilon, epsilon, epsilon).val));
