...
static bool check_approx_acos(skiatest::Reporter* r, float x, float approx_acos_x) {
  ...
  if (!(fabsf(error) <= GRVX_APPROX_ACOS_MAX_ERROR)) {
    ERRORF(
        r,
        "Larger-than-expected error from grvx::approx_acos\n"
        "  x=              %f\n"
        "  approx_acos_x=  %f  (%f degrees\n"
        "  acosf_x=        %f  (%f degrees\n"
        "  error=          %f  (%f degrees)\n"
        "  tolerance=      %f  (%f degrees)\n\n",
        x, approx_acos_x, SkRadiansToDegrees(approx_acos_x), acosf_x, SkRadiansToDegrees(acosf_x),
...
static bool check_approx_angle_between_vectors(
    skiatest::Reporter* r, SkVector a, SkVector b, float approxTheta) {
  ...
  if (!(fabsf(error) <= GRVX_APPROX_ACOS_MAX_ERROR + SK_ScalarNearlyZero)) {
    ...
    ERRORF(
        r,
        "Larger-than-expected error from grvx::approx_angle_between_vectors\n"
        "  a=                {%f, %f}\n"
        "  b=                {%f, %f}\n"
        "  expA=             {%u, %u}\n"
        "  expB=             {%u, %u}\n"
        ...
        SkRadiansToDegrees(approxTheta), expectedTheta, SkRadiansToDegrees(expectedTheta), error,
        SkRadiansToDegrees(error), GRVX_APPROX_ACOS_MAX_ERROR,
        SkRadiansToDegrees(GRVX_APPROX_ACOS_MAX_ERROR));
    return false;
  }
  return true;
...
static bool check_approx_angle_between_vectors(skiatest::Reporter* r, SkVector a, SkVector b) {
  float approxTheta =
      grvx::approx_angle_between_vectors(bit_pun<float2>(a), bit_pun<float2>(b)).val;
...
DEF_TEST(grvx_approx_angle_between_vectors, r) {
  ...
  // Test infinities.
  REPORTER_ASSERT(
      r, SkScalarNearlyZero(grvx::approx_angle_between_vectors<2>(
                                {std::numeric_limits<float>::infinity(), 1}, {2, 3})
                                .val));
  ...
  REPORTER_ASSERT(
      r, SkScalarNearlyZero(
             grvx::approx_angle_between_vectors<2>({epsilon, epsilon}, {epsilon, epsilon}).val));
