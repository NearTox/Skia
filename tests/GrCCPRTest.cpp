 ...
 public:
  ...
  void drawPath(const SkPath& path, const SkMatrix& matrix = SkMatrix::I()) const {
    ...
    fCCPR->testingOnly_drawPathDirectly(
        {fDContext.get(), std::move(paint), &GrUserStencilSettings::kUnused, fRTC.get(), nullptr,
 ...
 protected:
  ...
  CCPRCacheTest() {
    ...
    for (size_t i = 0; i < SK_ARRAY_COUNT(fPaths); ++i) {
      int numPts = rand.nextRangeU(
          GrStyledShape::kMaxKeyFromDataVerbCnt + 1, GrStyledShape::kMaxKeyFromDataVerbCnt * 2);
      int step;
      do {
