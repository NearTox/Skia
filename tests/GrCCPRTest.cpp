 ...
 private:
  bool apply(
    ...
    out->addCoverageFP(fCCPR->makeClipProcessor(
        rtc->priv().testingOnly_getOpListID(), fPath, SkIRect::MakeWH(rtc->width(), rtc->height()),
        rtc->width(), rtc->height(), *context->priv().caps()));
    return true;
  }
  ...
  CCPRPathDrawer(sk_sp<GrContext> ctx, skiatest::Reporter* reporter, bool doStroke)
      : fCtx(ctx),
        ...
        fRTC(fCtx->priv().makeDeferredRenderTargetContext(
            ...
            SkBackingFit::kExact, kCanvasSize, kCanvasSize, kRGBA_8888_GrPixelConfig, nullptr)),
        fDoStroke(doStroke) {
    if (!fCCPR) {
...
class CCPRCacheTest : public CCPRTest {
 protected:
  ...
  class RecordLastMockAtlasIDs : public GrOnFlushCallbackObject {
   public:
    ...
    void preFlush(
        GrOnFlushResourceProvider*, const uint32_t* opListIDs, int numOpListIDs,
  ...
  CCPRCacheTest() {
    ...
    for (size_t i = 0; i < SK_ARRAY_COUNT(fPaths); ++i) {
      int numPts =
          rand.nextRangeU(GrShape::kMaxKeyFromDataVerbCnt + 1, GrShape::kMaxKeyFromDataVerbCnt * 2);
      int step;
      do {
 ...
 private:
  ...
  virtual void onRun(
      skiatest::Reporter* reporter, CCPRPathDrawer& ccpr, const RecordLastMockAtlasIDs&) = 0;
...
class CCPR_cache_mostlyVisible : public CCPRCacheTest {
  void onRun(
    ...
    matrices[1].postTranslate(
        SkScalarFloorToInt(kCanvasSize * -.25f), SkScalarFloorToInt(kCanvasSize * -.25f));
    for (int i = 0; i < 10; ++i) {
      this->drawPathsAndFlush(ccpr, matrices, 2);
      REPORTER_ASSERT(reporter, 0 == atlasIDRecorder.lastCopyAtlasID());
      REPORTER_ASSERT(reporter, 0 == atlasIDRecorder.lastRenderedAtlasID());
    ...
    for (int i = 0; i < 5; ++i) {
      ...
      for (GrCCPathCacheEntry* entry : lru) {
        ...
        if (0 == i) {
          ...
          if (1 == i) {
            REPORTER_ASSERT(
                reporter, CoverageType::kFP16_CoverageCount == cachedAtlas->coverageType());
          } else {
            REPORTER_ASSERT(
                reporter, CoverageType::kA8_LiteralCoverage == cachedAtlas->coverageType());
          }
...
class CCPR_cache_multiFlush : public CCPRCacheTest {
  void onRun(
    ...
    static constexpr int kBigPrimes[] = {9323,  11059, 22993, 38749, 45127,
                                         53147, 64853, 77969, 83269, 99989};
      ...
      switch (testIdx) {
        case 0:
          if (0 == repetitionIdx) {
            ...
            REPORTER_ASSERT(reporter, 0 != atlasIDRecorder.lastCopyAtlasID());
            REPORTER_ASSERT(reporter, atlasIDRecorder.lastRenderedAtlasID() == firstAtlasID);
          } else {
            REPORTER_ASSERT(reporter, 0 == atlasIDRecorder.lastCopyAtlasID());
