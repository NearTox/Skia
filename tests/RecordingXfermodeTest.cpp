...
class Drawer {
 public:
  ...
  void draw(SkCanvas* canvas, const SkRect& clipRect, SkBlendMode mode) const {
    ...
    SkRect canvasRect(
        SkRect::MakeWH(SkIntToScalar(fImageInfo.width()), SkIntToScalar(fImageInfo.height())));
    ...
    canvas->drawImageRect(
        fCircleImage, SkRect::MakeXYWH(20, 20, 60, 60), SkSamplingOptions(), &blackPaint);
    canvas->restore();
    canvas->restore();
  }
...
class RecordingStrategy {
 public:
  ...
  virtual const SkBitmap& recordAndReplay(
      const Drawer& drawer, const SkRect& intoClip, SkBlendMode) = 0;
};

class BitmapBackedCanvasStrategy : public RecordingStrategy {
  // This version just draws into a bitmap-backed canvas.
 public:
  BitmapBackedCanvasStrategy(const SkImageInfo& imageInfo) { fBitmap.allocPixels(imageInfo); }

  const SkBitmap& recordAndReplay(
      const Drawer& drawer, const SkRect& intoClip, SkBlendMode mode) override {
...
class PictureStrategy : public RecordingStrategy {
 ...
 public:
  ...
  const SkBitmap& recordAndReplay(
    ...
    SkCanvas* canvas =
        recorder.beginRecording(SkIntToScalar(fWidth), SkIntToScalar(fHeight), &factory);
...
#if FINEGRAIN
    REPORTER_ASSERT(reporter, 0 == memcmp(goldenBM.getPixels(), pictureBM.getPixels(), pixelsSize));
#else
    if (0 != memcmp(goldenBM.getPixels(), pictureBM.getPixels(), pixelsSize)) {
      numErrors++;
      errors.appendf(
          "For SkXfermode %d %s:    SkPictureRecorder bitmap is wrong\n", iMode,
