...
void TestReadPixels(
    skiatest::Reporter*, GrDirectContext*, skgpu::SurfaceContext*, uint32_t expectedPixelValues[],
    const char* testName);

// See if trying to write RGBA 8888 pixels to 'dstContext' matches matches the
// expectation ('expectedToWork')
void TestWritePixels(
    skiatest::Reporter*, GrDirectContext*, skgpu::SurfaceContext*, bool expectedToWork,
    const char* testName);
...
void TestCopyFromSurface(
    skiatest::Reporter*, GrDirectContext*, sk_sp<GrSurfaceProxy> proxy, GrSurfaceOrigin origin,
...
std::unique_ptr<skgpu::SurfaceContext> CreateSurfaceContext(
    ...
    int sampleCount = 1, GrMipmapped = GrMipmapped::kNo, GrProtected = GrProtected::kNo,
