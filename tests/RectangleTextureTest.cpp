...
static void test_basic_draw_as_src(
  ...
  sk_sp<GrRenderTargetContext> rtContext(context->priv().makeDeferredRenderTargetContext(
      format, SkBackingFit::kExact, rectProxy->width(), rectProxy->height(), rectProxy->config(),
      nullptr));
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(RectangleTexture, reporter, ctxInfo) {
  ...
  for (auto origin : {kBottomLeft_GrSurfaceOrigin, kTopLeft_GrSurfaceOrigin}) {
    ...
    test_copy_from_surface(
        reporter, context, rectProxy.get(), refPixels, false, "RectangleTexture-copy-from");

    sk_sp<GrSurfaceContext> rectContext =
        context->priv().makeWrappedSurfaceContext(std::move(rectProxy));
