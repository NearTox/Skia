...
static void test_basic_draw_as_src(
  ...
  for (auto filter : {GrSamplerState::Filter::kNearest, GrSamplerState::Filter::kLinear}) {
    for (auto mm : {GrSamplerState::MipmapMode::kNone, GrSamplerState::MipmapMode::kLinear}) {
      ...
      TestReadPixels(
          reporter, dContext, rtContext.get(), expectedPixelValues, "RectangleTexture-basic-draw");
    }
  }
}
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(RectangleTexture, reporter, ctxInfo) {
  ...
  for (auto origin : {kBottomLeft_GrSurfaceOrigin, kTopLeft_GrSurfaceOrigin}) {
    ...
    GrSwizzle swizzle = dContext->priv().caps()->getReadSwizzle(
        rectangleTex.getBackendFormat(), GrColorType::kRGBA_8888);
    GrSurfaceProxyView view(rectProxy, origin, swizzle);

    test_basic_draw_as_src(
        reporter, dContext, view, GrColorType::kRGBA_8888, kPremul_SkAlphaType, refPixels);

    // Test copy to both a texture and RT
    TestCopyFromSurface(
        reporter, dContext, rectProxy.get(), origin, GrColorType::kRGBA_8888, refPixels,
        "RectangleTexture-copy-from");

    auto rectContext = GrSurfaceContext::Make(
