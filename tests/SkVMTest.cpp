...
DEF_TEST(SkVM, r) {
  ...
  for (int s = 0; s < 3; s++)
    for (int d = 0; d < 3; d++) {
      auto srcFmt = (Fmt)s, dstFmt = (Fmt)d;
...

  {
    ...
    skvm::Builder b;
    skvm::Arg sp = b.varying<int>(), dp = b.varying<int>();

    skvm::I32 byte = b.splat(0xff);

    skvm::I32 src = b.load32(sp), sr = b.extract(src, 0, byte), sg = b.extract(src, 8, byte),
              sb = b.extract(src, 16, byte), sa = b.extract(src, 24, byte);

    skvm::I32 dst = b.load32(dp), dr = b.extract(dst, 0, byte), dg = b.extract(dst, 8, byte),
              db = b.extract(dst, 16, byte), da = b.extract(dst, 24, byte);

    skvm::I32 R = b.add(sr, dr), G = b.add(sg, dg), B = b.add(sb, db), A = b.add(sa, da);

    skvm::I32 rg = b.pack(R, G, 8), ba = b.pack(B, A, 8), rgba = b.pack(rg, ba, 16);
  ...
  auto test_8888 = [&](skvm::Program&& program) {
    ...
    test_jit_and_interpreter(std::move(program), [&](const skvm::Program& program) {
      ...
      for (auto got : dst) {
        auto want = expected;
        for (int i = 0; i < 4; i++) {
          uint8_t d = got & 0xff, w = want & 0xff;
          if (abs(d - w) >= 2) {
  ...
  test_jit_and_interpreter(
      SrcoverBuilder_F32{Fmt::RGBA_8888, Fmt::G8}.done(), [&](const skvm::Program& program) {
        uint32_t src[9];
        ...
        uint8_t want =
            SkComputeLuminance(SkGetPackedR32(over), SkGetPackedG32(over), SkGetPackedB32(over));
        program.eval((int)SK_ARRAY_COUNT(src), src, dst);
  ...
  test_jit_and_interpreter(
      SrcoverBuilder_F32{Fmt::A8, Fmt::A8}.done(), [&](const skvm::Program& program) {
        ...
        for (int i = 0; i < 256; i++) {
          uint8_t want =
              SkGetPackedA32(SkPMSrcOver(SkPackARGB32(src[i], 0, 0, 0), SkPackARGB32(i, 0, 0, 0)));
...
DEF_TEST(SkVM_Pointless, r) {
  ...
  skvm::Builder b;
  { b.add(b.splat(5.0f), b.splat(4.0f)); }
...
DEF_TEST(SkVM_memcpy, r) {
  skvm::Builder b;
  {
    auto src = b.varying<int>(), dst = b.varying<int>();
    b.store32(dst, b.load32(src));
  }

  test_jit_and_interpreter(b.done(), [&](const skvm::Program& p) {
    int src[] = {1, 2, 3, 4, 5, 6, 7, 8, 9}, dst[] = {0, 0, 0, 0, 0, 0, 0, 0, 0};
...
DEF_TEST(SkVM_LoopCounts, r) {
  ...
  skvm::Builder b;
  skvm::Arg arg = b.varying<int>();
  b.store32(arg, b.add(b.splat(1), b.load32(arg)));
...
DEF_TEST(SkVM_gather32, r) {
  skvm::Builder b;
  {
    skvm::Arg uniforms = b.uniform(), buf = b.varying<int>();
  ...
  test_jit_and_interpreter(b.done(), [&](const skvm::Program& program) {
    ...
    int i = 0;
    REPORTER_ASSERT(r, buf[i] == 12);
    i++;
    REPORTER_ASSERT(r, buf[i] == 34);
    i++;
    REPORTER_ASSERT(r, buf[i] == 56);
    i++;
    REPORTER_ASSERT(r, buf[i] == 78);
    i++;
    REPORTER_ASSERT(r, buf[i] == 90);
    i++;
    REPORTER_ASSERT(r, buf[i] == 98);
    i++;
    REPORTER_ASSERT(r, buf[i] == 76);
    i++;
    REPORTER_ASSERT(r, buf[i] == 54);
    i++;
...
DEF_TEST(SkVM_gathers, r) {
  skvm::Builder b;
  {
    skvm::Arg uniforms = b.uniform(), buf32 = b.varying<int>(), buf16 = b.varying<uint16_t>(),
  ...
  test_jit_and_interpreter(b.done(), [&](const skvm::Program& program) {
    ...
    int i = 0;
    REPORTER_ASSERT(r, buf32[i] == 12 && buf16[i] == 12 && buf8[i] == 12);
    i++;
    REPORTER_ASSERT(r, buf32[i] == 34 && buf16[i] == 0 && buf8[i] == 0);
    i++;
    REPORTER_ASSERT(r, buf32[i] == 56 && buf16[i] == 34 && buf8[i] == 0);
    i++;
    REPORTER_ASSERT(r, buf32[i] == 78 && buf16[i] == 0 && buf8[i] == 0);
    i++;
    REPORTER_ASSERT(r, buf32[i] == 90 && buf16[i] == 56 && buf8[i] == 34);
    i++;
    REPORTER_ASSERT(r, buf32[i] == 98 && buf16[i] == 0 && buf8[i] == 0);
    i++;
    REPORTER_ASSERT(r, buf32[i] == 76 && buf16[i] == 78 && buf8[i] == 0);
    i++;
    REPORTER_ASSERT(r, buf32[i] == 54 && buf16[i] == 0 && buf8[i] == 0);
    i++;

    REPORTER_ASSERT(r, buf32[i] == 12 && buf16[i] == 90 && buf8[i] == 56);
    i++;
    REPORTER_ASSERT(r, buf32[i] == 34 && buf16[i] == 0 && buf8[i] == 0);
    i++;
    REPORTER_ASSERT(r, buf32[i] == 56 && buf16[i] == 98 && buf8[i] == 0);
    i++;
    REPORTER_ASSERT(r, buf32[i] == 78 && buf16[i] == 0 && buf8[i] == 0);
    i++;
    REPORTER_ASSERT(r, buf32[i] == 90 && buf16[i] == 76 && buf8[i] == 78);
    i++;
    REPORTER_ASSERT(r, buf32[i] == 98 && buf16[i] == 0 && buf8[i] == 0);
    i++;
    REPORTER_ASSERT(r, buf32[i] == 76 && buf16[i] == 54 && buf8[i] == 0);
    i++;
    REPORTER_ASSERT(r, buf32[i] == 54 && buf16[i] == 0 && buf8[i] == 0);
    i++;

    REPORTER_ASSERT(r, buf32[i] == 12 && buf16[i] == 12 && buf8[i] == 90);
    i++;
    REPORTER_ASSERT(r, buf32[i] == 34 && buf16[i] == 0 && buf8[i] == 0);
    i++;
    REPORTER_ASSERT(r, buf32[i] == 56 && buf16[i] == 34 && buf8[i] == 0);
...
DEF_TEST(SkVM_select_is_NaN, r) {
  skvm::Builder b;
  {
    skvm::Arg src = b.varying<float>(), dst = b.varying<float>();

    skvm::F32 x = b.loadF(src);
    x = select(is_NaN(x), b.splat(0.0f), x);
    b.storeF(dst, x);
  }
...
DEF_TEST(SkVM_fms, r) {
  ...
  skvm::Builder b;
  {
    ...
    skvm::F32 x = b.to_F32(b.load32(arg)), v = b.sub(b.mul(x, b.splat(2.0f)), b.splat(1.0f));
    b.store32(arg, b.trunc(v));
  }
...
DEF_TEST(SkVM_fnma, r) {
  ...
  skvm::Builder b;
  {
    ...
    skvm::F32 x = b.to_F32(b.load32(arg)), v = b.sub(b.splat(1.0f), b.mul(x, b.splat(2.0f)));
    b.store32(arg, b.trunc(v));
  }
...
DEF_TEST(SkVM_NewOps, r) {
  ...
  skvm::Builder b;
  {
    skvm::Arg buf = b.varying<int16_t>(), uniforms = b.uniform();
  ...
  test_jit_and_interpreter(b.done(), [&](const skvm::Program& program) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      if (i < 2) {
        x = 0;
      }  // Notice i == 1 hits x == 0 exactly...
      if (i > 5) {
        x = 15;
      }  // ...and i == 6 hits x == 15 exactly
...
DEF_TEST(SkVM_assert, r) {
  skvm::Builder b;
  b.assert_true(b.lt(b.load32(b.varying<int>()), b.splat(42)));
...
static void test_asm(skiatest::Reporter* r, Fn&& fn, std::initializer_list<uint8_t> expected) {
  ...
  auto got = (const uint8_t*)buf, want = expected.begin();
  for (int i = 0; i < (int)std::min(a.size(), expected.size()); i++) {
    REPORTER_ASSERT(r, got[i] == want[i], "byte %d was %02x, want %02x", i, got[i], want[i]);
  }
}
...
DEF_TEST(SkVM_Assembler, r) {
  ...
  test_asm(
      ...
      {
          0x48, 0x83, 0b11'000'000, 0x08, 0x48, 0x83, 0b11'101'000, 0x20,

          0x48, 0x83, 0b11'000'111, 0x0c, 0x48, 0x83, 0b11'101'111, 0x08,

          0x49, 0x83, 0b11'000'000, 0x07, 0x49, 0x83, 0b11'101'000, 0x04,

          0x48, 0x81, 0b11'000'110, 0x80, 0x00, 0x00, 0x00,         0x49, 0x81, 0b11'101'000,
          0x40, 0x42, 0x0f,         0x00,

          0x48, 0x83, 0x06,         0x07, 0x48, 0x83, 0x46,         0x0c, 0x07, 0x48,
          0x83, 0x44, 0x24,         0x0c, 0x07, 0x49, 0x83,         0x44, 0x24, 0x0c,
          0x07, 0x48, 0x83,         0x44, 0x84, 0x0c, 0x07,         0x49, 0x83, 0x44,
          0x84, 0x0c, 0x07,         0x4a, 0x83, 0x44, 0xa0,         0x0c, 0x07, 0x4b,
          0x83, 0x44, 0x43,         0x0c, 0x07, 0x49, 0x83,         0x44, 0x03, 0x0c,
          0x07, 0x4a, 0x83,         0x44, 0x18, 0x0c, 0x07,

          0x4a, 0x83, 0x6c,         0x18, 0x0c, 0x07,

          0x48, 0x01, 0xc8,         0x48, 0x01, 0x08, 0x48,         0x01, 0x48, 0x0c,
          0x48, 0x03, 0x48,         0x0c, 0x48, 0x2b, 0x48,         0x0c,
      });
  ...
  test_asm(
      ...
      {
          0xc5, 0xe5, 0xfd, 0xe2, 0xc5, 0xe5, 0xe3, 0xe2, 0xc5, 0xe5,
          0x75, 0xe2, 0xc5, 0xe5, 0x65, 0xe2,

          0xc5, 0xe5, 0xea, 0xe2, 0xc5, 0xe5, 0xee, 0xe2, 0xc4, 0xe2,
          0x65, 0x3a, 0xe2, 0xc4, 0xe2, 0x65, 0x3e, 0xe2,

          0xc4, 0xe2, 0x65, 0x0b, 0xe2, 0xc4, 0xe2, 0x7d, 0x1d, 0xe3,
          0xc5, 0xdd, 0x71, 0xf3, 0x0c, 0xc5, 0xdd, 0x71, 0xe3, 0x0c,
      });
  ...
  test_asm(
      ...
      {
          0xc5, 0xf4, 0xc2, 0x05, 0x1c, 0x00, 0x00, 0x00, 0x00, 0xc5, 0xf5, 0x76, 0xc2,
          0xc5, 0xf5, 0x66, 0xc2, 0xc5, 0xf4, 0xc2, 0xc2, 0x00, 0xc5, 0xf4, 0xc2, 0xc2,
          0x01, 0xc5, 0xf4, 0xc2, 0xc2, 0x02, 0xc5, 0xf4, 0xc2, 0xc2, 0x04,
      });
  ...
  test_asm(
      r, [&](A& a) { a.vpblendvb(A::ymm0, A::ymm1, A::ymm2, A::ymm3); },
      {
          0xc4,
          0xe3,
          0x75,
  ...
  test_asm(
      ...
      {
          0xc4, 0xe2, 0x6d, 0x16, 0x4f, 0x20, 0xc4, 0xe3, 0x6d, 0x06, 0x0d,
          0x06, 0x00, 0x00, 0x00, 0x20, 0xc4, 0xe3, 0xfd, 0x00, 0xca, 0x05,
      });
  ...
  test_asm(
      ...
      {
          0xc4, 0xe3, 0x7d, 0x08, 0xca, 0x00, 0xc4, 0xe3, 0x7d, 0x08, 0xca, 0x01,
          0xc4, 0xe3, 0x7d, 0x08, 0xca, 0x02, 0xc4, 0xe3, 0x7d, 0x08, 0xca, 0x03,
      });
  ...
  test_asm(
      ...
      },
      {
          ...
          0x48, 0x83, 0xfa, 0x01, 0x48, 0x83, 0xf8, 0x0c, 0x49, 0x81, 0xfe, 0x00, 0x94, 0x35, 0x77,
      });
  ...
  test_asm(
      ...
      {
          0xc5, 0xfc, 0x10, 0x2c, 0x24, 0xc5, 0xfc, 0x10, 0x6c, 0x24,
          0x40, 0xc5, 0xfc, 0x10, 0xac, 0x24, 0x80, 0x00, 0x00, 0x00,

          0xc5, 0xfc, 0x11, 0x2c, 0x24, 0xc5, 0xfc, 0x11, 0x6c, 0x24,
          0x40, 0xc5, 0xfc, 0x11, 0xac, 0x24, 0x80, 0x00, 0x00, 0x00,
      });
  ...
  test_asm(
      ...
      },
      {
          ...
          0x49, 0x0f, 0xb6, 0x00, 0x4c, 0x0f, 0xb6, 0x06, 0x4c, 0x0f, 0xb6, 0x46, 12,
          0x4c, 0x0f, 0xb6, 0x86, 0x90, 0x01, 0x00, 0x00,

          0x48, 0x0f, 0xb7, 0x06,  // movzwq (%rsi), %rax
          0x49, 0x0f, 0xb7, 0x00, 0x4c, 0x0f, 0xb7, 0x06, 0x4c, 0x0f, 0xb7, 0x46, 12,
          0x4c, 0x0f, 0xb7, 0x86, 0x90, 0x01, 0x00, 0x00,

          0xc5, 0xf9, 0x7e, 0x00, 0xc5, 0x79, 0x7e, 0x00, 0xc4, 0xc1, 0x79, 0x7e, 0x00,

          0xc5, 0xf9, 0x6e, 0x00, 0xc5, 0x79, 0x6e, 0x00, 0xc4, 0xc1, 0x79, 0x6e, 0x00,

          0xc5, 0xf9, 0x6e, 0x04, 0x88, 0xc4, 0x21, 0x79, 0x6e, 0x3c, 0x40, 0xc4, 0xc1,
          0x79, 0x6e, 0x04, 0x08,

          0xc5, 0xf9, 0x7e, 0xc0, 0xc5, 0x79, 0x7e, 0xc0, 0xc4, 0xc1, 0x79, 0x7e, 0xc0,

          0xc5, 0xf9, 0x6e, 0xc0, 0xc5, 0x79, 0x6e, 0xc0, 0xc4, 0xc1, 0x79, 0x6e, 0xc0,

          0x48, 0x88, 0x02, 0x4c, 0x88, 0x02, 0x49, 0x88, 0x00,

          0x48, 0x8a, 0x10, 0x49, 0x8a, 0x10, 0x4c, 0x8a, 0x00,

          0x48, 0xc6, 0xc2, 0x0c, 0x48, 0xc6, 0xc0, 0x04, 0x49, 0xc6, 0xc0, 0xff,

          0x48, 0xc6, 0x02, 0x0c, 0x48, 0xc6, 0x00, 0x04, 0x49, 0xc6, 0x00, 0xff,
      });
  ...
  test_asm(
      ...
      {
          0xc4, 0xe3, 0x39, 0x22, 0x0e, 1,    0xc4, 0x43, 0x71, 0x22, 0x00, 3,

          0xc5, 0xb9, 0xc4, 0x0e, 4,    0xc4, 0x41, 0x71, 0xc4, 0x00, 12,

          0xc4, 0xe3, 0x39, 0x20, 0x0e, 4,    0xc4, 0x43, 0x71, 0x20, 0x00, 12,

          0xc4, 0x63, 0x7d, 0x39, 0xc1, 1,    0xc4, 0xc3, 0x7d, 0x39, 0xc8, 0,

          0xc4, 0x63, 0x79, 0x16, 0x06, 3,    0xc4, 0xc3, 0x79, 0x16, 0x08, 2,

          0xc4, 0x63, 0x79, 0x15, 0x06, 7,    0xc4, 0xc3, 0x79, 0x15, 0x08, 15,

          0xc4, 0x63, 0x79, 0x14, 0x06, 7,    0xc4, 0xc3, 0x79, 0x14, 0x08, 15,
      });

  test_asm(
      r, [&](A& a) { a.vpandn(A::ymm3, A::ymm12, A::ymm2); },
      {
          0xc5,
          0x9d,
          0xdf,
  ...
  test_asm(
      ...
      },
      {
          ...
          0xc5, 0xfd, 0x6f, 0x1e, 0xc5, 0xfd, 0x6f, 0x1c, 0x24, 0xc4, 0xc1, 0x7d, 0x6f, 0x1b,

          0xc5, 0xfd, 0x6f, 0x5e, 0x04, 0xc5, 0xfd, 0x6f, 0x5c, 0x24, 0x04,

          0xc5, 0xfd, 0x6f, 0x5c, 0xc6, 0x04, 0xc4, 0xc1, 0x7d, 0x6f, 0x5c, 0x43, 0x04, 0xc4,
          0xa1, 0x7d, 0x6f, 0x5c, 0x9e, 0x04, 0xc4, 0xa1, 0x7d, 0x6f, 0x5c, 0x1e, 0x04, 0xc4,
          0xa1, 0x7d, 0x6f, 0x5c, 0x1e, 0x04,

          0xc4, 0xa1, 0x7d, 0x6f, 0x5c, 0x1e, 0x40, 0xc4, 0xa1, 0x7d, 0x6f, 0x9c, 0x1e, 0x80,
          0x00, 0x00, 0x00,

          0xc5, 0xfd, 0x6f, 0x1d, 0x10, 0x00, 0x00, 0x00,

          0xc5, 0xfe, 0x5b, 0xda, 0xc5, 0xfc, 0x5b, 0xda, 0xc5, 0xfd, 0x5b, 0xda, 0xc5, 0xfc,
          0x51, 0xda,
      });
  ...
  test_asm(
      ...
      {
          0xc4, 0xe3, 0x7d, 0x1d, 0xd3, 0x04, 0xc4, 0xe3, 0x7d, 0x1d, 0x6c, 0xc6, 0x20, 0x02,

          0xc4, 0x22, 0x7d, 0x13, 0x7c, 0x0f, 0x0c, 0xc4, 0xe2, 0x7d, 0x13, 0xd3,
      });
  ...
  test_asm(
      ...
      {
          0xc4, 0xe2, 0x6d, 0x92, 0x0c, 0x87, 0xc4, 0xe2, 0x75, 0x92, 0x04, 0x10, 0xc4, 0x62,
          0x75, 0x92, 0x14, 0x10, 0xc4, 0xa2, 0x75, 0x92, 0x04, 0x20, 0xc4, 0xc2, 0x75, 0x92,
          0x04, 0x11, 0xc4, 0xe2, 0x1d, 0x92, 0x04, 0x10, 0xc4, 0xe2, 0x1d, 0x92, 0x04, 0xd0,
      });
  ...
  test_asm(
      ...
      {
          0x48, 0x8b, 0x07, 0x48, 0x8b, 0x47, 0x01, 0x48, 0x8b, 0x87, 0x00, 0x02,
          0x00, 0x00, 0x4d, 0x8b, 0x7d, 0x2a, 0x49, 0x8b, 0x45, 0x2a, 0x4c, 0x8b,
          0x78, 0x2a, 0x48, 0xc7, 0xc0, 0x01, 0x00, 0x00, 0x00, 0x48, 0x89, 0xc8,
      });
  ...
  test_asm(
      ...
      {
          0x64, 0x1c, 0x21, 0x4e, 0x64, 0x1c, 0xa1, 0x4e, 0x64, 0x1c, 0x21, 0x6e, 0x64, 0x1c,
          0x61, 0x4e, 0x64, 0x1c, 0x61, 0x6e, 0x64, 0x58, 0x20, 0x6e,

          0x64, 0x84, 0xa1, 0x4e, 0x64, 0x84, 0xa1, 0x6e, 0x64, 0x9c, 0xa1, 0x4e,

          0x64, 0x8c, 0xa1, 0x6e, 0x64, 0x34, 0xa1, 0x4e,

          0x64, 0x84, 0x61, 0x6e, 0x64, 0x9c, 0x61, 0x4e,

          0x64, 0xd4, 0x21, 0x4e, 0x64, 0xd4, 0xa1, 0x4e, 0x64, 0xdc, 0x21, 0x6e, 0x64, 0xfc,
          0x21, 0x6e, 0x64, 0xf4, 0xa1, 0x4e, 0x64, 0xf4, 0x21, 0x4e, 0x64, 0xf8, 0xa0, 0x6e,

          0x64, 0xcc, 0x21, 0x4e, 0x64, 0xcc, 0xa1, 0x4e,

          0x64, 0xe4, 0x21, 0x4e, 0x64, 0xe4, 0xa1, 0x6e, 0x64, 0xe4, 0x21, 0x6e,
      });
  ...
  test_asm(
      ...
      {
          0x64, 0x54, 0x20, 0x4f, 0x64, 0x54, 0x21, 0x4f, 0x64, 0x54, 0x28, 0x4f,
          0x64, 0x54, 0x30, 0x4f, 0x64, 0x54, 0x3f, 0x4f,

          0x64, 0x04, 0x3f, 0x4f, 0x64, 0x04, 0x38, 0x4f, 0x64, 0x04, 0x21, 0x4f,

          0x64, 0x04, 0x3f, 0x6f, 0x64, 0x04, 0x38, 0x6f, 0x64, 0x04, 0x21, 0x6f,

          0x64, 0x04, 0x1f, 0x6f, 0x64, 0x04, 0x18, 0x6f, 0x64, 0x04, 0x11, 0x6f,
      });
  ...
  test_asm(
      ...
      {
          0x64, 0x54, 0x20, 0x6f, 0x64, 0x54, 0x21, 0x6f, 0x64, 0x54,
          0x28, 0x6f, 0x64, 0x54, 0x30, 0x6f, 0x64, 0x54, 0x3f, 0x6f,
      });
  ...
  test_asm(
      ...
      {
          0xff, 0x83, 0x00, 0xd1, 0xe0, 0x07, 0x80, 0x3d, 0xe1, 0x03, 0x80, 0x3d,
          0xe0, 0x1b, 0x00, 0xbd, 0xe0, 0xbf, 0x00, 0x3d, 0xe9, 0xab, 0x40, 0x3d,
          0xe7, 0x2b, 0x40, 0xbd, 0xe5, 0x03, 0xc2, 0x3d, 0xff, 0x83, 0x00, 0x91,
      });
  ...
  test_asm(
      ...
      {
          0x00, 0x00, 0x20, 0xd4, 0xe0, 0xff, 0x3f, 0xd4,

          0xc0, 0x03, 0x5f, 0xd6, 0xa0, 0x01, 0x5f, 0xd6,

          0x42, 0x10, 0x00, 0x91, 0x43, 0x80, 0x00, 0x91,

          0x42, 0x10, 0x00, 0xd1, 0x43, 0x80, 0x00, 0xd1,

          0x42, 0x10, 0x00, 0xf1, 0x43, 0x80, 0x00, 0xf1,

          0x5f, 0x10, 0x00, 0xf1, 0x5f, 0x10, 0x00, 0xf1,
  ...
  test_asm(
      ...
      {
          0x00, 0x28, 0x61, 0x0e, 0x00, 0x28, 0x21, 0x0e, 0x00, 0x00, 0x00, 0xbd,

          0x00, 0x00, 0x40, 0xbd, 0x00, 0xa4, 0x08, 0x2f, 0x00, 0xa4, 0x10, 0x2f,

          0x83, 0xa8, 0xb1, 0x6e, 0x83, 0x00, 0x26, 0x1e,
      });
...
DEF_TEST(SkVM_approx_math, r) {
  ...
  // sine, cosine, tangent
  {
    ...
    for (float rad = -5 * P; rad <= 5 * P; rad += 0.1f) {
      test(rad, sk_float_sin(rad), tol, [](skvm::F32 x) { return approx_sin(x); });
      test(rad, sk_float_cos(rad), tol, [](skvm::F32 x) { return approx_cos(x); });
    }
    ...
    for (float rad = -P / 2 + eps; rad <= P / 2 - eps; rad += 0.01f) {
      err += test(rad, sk_float_tan(rad), tol, [](skvm::F32 x) { return approx_tan(x); });
      // try again with some multiples of P, to check our periodicity
      test(rad, sk_float_tan(rad), tol, [=](skvm::F32 x) { return approx_tan(x + 3 * P); });
      test(rad, sk_float_tan(rad), tol, [=](skvm::F32 x) { return approx_tan(x - 3 * P); });
    }
    if (0) {
  ...
  // asin, acos, atan
  {
    ...
    for (float x = -1; x <= 1; x += 1.0f / 64) {
      err += test(x, asin(x), tol, [](skvm::F32 x) { return approx_asin(x); });
      test(x, acos(x), tol, [](skvm::F32 x) { return approx_acos(x); });
    }
    if (0) {
      SkDebugf("asin error %g\n", err);
    }

    err = 0;
    for (float x = -10; x <= 10; x += 1.0f / 16) {
      err += test(x, atan(x), tol, [](skvm::F32 x) { return approx_atan(x); });
    }
    if (0) {
      SkDebugf("atan error %g\n", err);
    }

    for (float y = -3; y <= 3; y += 1) {
      for (float x = -3; x <= 3; x += 1) {
        err += test2(
            y, x, atan2(y, x), tol, [](skvm::F32 y, skvm::F32 x) { return approx_atan2(y, x); });
      }
    }
    if (0) {
      SkDebugf("atan2 error %g\n", err);
    }
  }
}
...
DEF_TEST(SkVM_min_max, r) {
  ...
  // (Whether x and y are varying or uniform shouldn't make any difference.)
  {
    skvm::Builder b;
    {
      ...
      skvm::F32 x = b.loadF(src), y = b.uniformF(b.uniform(), 0);
...
DEF_TEST(SkVM_halfs, r) {
  const uint16_t hs[] = {0x0000, 0x3800, 0x3c00, 0x4000, 0xc400, 0xb800, 0xbc00, 0xc000};
  const float fs[] = {+0.0f, +0.5f, +1.0f, +2.0f, -4.0f, -0.5f, -1.0f, -2.0f};
  {
    skvm::Builder b;
    skvm::Arg src = b.varying<uint16_t>(), dst = b.varying<float>();
  ...
  {
    skvm::Builder b;
    skvm::Arg src = b.varying<float>(), dst = b.varying<uint16_t>();
...
DEF_TEST(SkVM_is_NaN_is_finite, r) {
  skvm::Builder b;
  {
    skvm::Arg src = b.varying<float>(), nan = b.varying<int>(), fin = b.varying<int>();
  ...
  test_jit_and_interpreter(b.done(), [&](const skvm::Program& program) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      REPORTER_ASSERT(r, fin[i] == ((i == 2 || i == 3 || i == 4 || i == 5) ? 0xffffffff : 0));
    }
  });
}
...
DEF_TEST(SkVM_args, r) {
  ...
  skvm::Builder b;
  {
    skvm::Arg dst = b.varying<float>(), A = b.varying<float>(), B = b.varying<float>(),
              C = b.varying<float>(), D = b.varying<float>(), E = b.varying<float>();
    storeF(dst, b.loadF(A) + b.loadF(B) + b.loadF(C) + b.loadF(D) + b.loadF(E));
  }
...
DEF_TEST(SkVM_Q14, r) {
  ...
  const uint16_t src[] = {
      0xc000, 0xc400, 0xc800, 0xcc00, 0xd000, 0xd400, 0xd800, 0xdc00, 0xe000, 0xe400, 0xe800,
      0xec00, 0xf000, 0xf400, 0xf800, 0xfc00, 0x0000, 0x0400, 0x0800, 0x0c00, 0x1000, 0x1400,
      0x1800, 0x1c00, 0x2000, 0x2400, 0x2800, 0x2c00, 0x3000, 0x3400, 0x3800, 0x3c00, 0x4000,
  };
  ...
  struct {
    ...
    uint16_t expected[33];
      ...
      {[](skvm::Q14 x) { return x * x; },  // square ±1/16 (0x0400) -> 1/256 (0x0040), etc.
       {0x4000, 0x3840, 0x3100, 0x2a40, 0x2400, 0x1e40, 0x1900, 0x1440, 0x1000, 0x0c40, 0x0900,
        0x0640, 0x0400, 0x0240, 0x0100, 0x0040, 0x0000, 0x0040, 0x0100, 0x0240, 0x0400, 0x0640,
        0x0900, 0x0c40, 0x1000, 0x1440, 0x1900, 0x1e40, 0x2400, 0x2a40, 0x3100, 0x3840, 0x4000}},
      ...
      {[](skvm::Q14 x) { return x >> 1; },  // divide by 2
       {0xe000, 0xe200, 0xe400, 0xe600, 0xe800, 0xea00, 0xec00, 0xee00, 0xf000, 0xf200, 0xf400,
        0xf600, 0xf800, 0xfa00, 0xfc00, 0xfe00, 0x0000, 0x0200, 0x0400, 0x0600, 0x0800, 0x0a00,
        0x0c00, 0x0e00, 0x1000, 0x1200, 0x1400, 0x1600, 0x1800, 0x1a00, 0x1c00, 0x1e00, 0x2000}},

      {[](skvm::Q14 x) { return shr(x, 1); },  // logical shift by 1
       {0x6000, 0x6200, 0x6400, 0x6600, 0x6800, 0x6a00, 0x6c00, 0x6e00, 0x7000, 0x7200, 0x7400,
        0x7600, 0x7800, 0x7a00, 0x7c00, 0x7e00, 0x0000, 0x0200, 0x0400, 0x0600, 0x0800, 0x0a00,
        0x0c00, 0x0e00, 0x1000, 0x1200, 0x1400, 0x1600, 0x1800, 0x1a00, 0x1c00, 0x1e00, 0x2000}},

      {[](skvm::Q14 x) { return x - (x >> 2); },  // 3/4 x, version A
       {0xd000, 0xd300, 0xd600, 0xd900, 0xdc00, 0xdf00, 0xe200, 0xe500, 0xe800, 0xeb00, 0xee00,
        0xf100, 0xf400, 0xf700, 0xfa00, 0xfd00, 0x0000, 0x0300, 0x0600, 0x0900, 0x0c00, 0x0f00,
        0x1200, 0x1500, 0x1800, 0x1b00, 0x1e00, 0x2100, 0x2400, 0x2700, 0x2a00, 0x2d00, 0x3000}},
      ...
      {[](skvm::Q14 x) { return min(x, +0.5f); },  // clamp down to 0x2000, version A
       {0xc000, 0xc400, 0xc800, 0xcc00, 0xd000, 0xd400, 0xd800, 0xdc00, 0xe000, 0xe400, 0xe800,
        0xec00, 0xf000, 0xf400, 0xf800, 0xfc00, 0x0000, 0x0400, 0x0800, 0x0c00, 0x1000, 0x1400,
        0x1800, 0x1c00, 0x2000, 0x2000, 0x2000, 0x2000, 0x2000, 0x2000, 0x2000, 0x2000, 0x2000}},
      ...
      {[](skvm::Q14 x) { return select(x == 1.0f, 0.5f, x); },
       {0xc000, 0xc400, 0xc800, 0xcc00, 0xd000, 0xd400, 0xd800, 0xdc00, 0xe000, 0xe400, 0xe800,
        0xec00, 0xf000, 0xf400, 0xf800, 0xfc00, 0x0000, 0x0400, 0x0800, 0x0c00, 0x1000, 0x1400,
        0x1800, 0x1c00, 0x2000, 0x2400, 0x2800, 0x2c00, 0x3000, 0x3400, 0x3800, 0x3c00, 0x2000}},

      {[](skvm::Q14 x) { return max(x, -0.5f); },  // clamp up to 0xe000
       {0xe000, 0xe000, 0xe000, 0xe000, 0xe000, 0xe000, 0xe000, 0xe000, 0xe000, 0xe400, 0xe800,
        0xec00, 0xf000, 0xf400, 0xf800, 0xfc00, 0x0000, 0x0400, 0x0800, 0x0c00, 0x1000, 0x1400,
        0x1800, 0x1c00, 0x2000, 0x2400, 0x2800, 0x2c00, 0x3000, 0x3400, 0x3800, 0x3c00, 0x4000}},
  };
  ...
  for (const auto& test : cases) {
    skvm::Builder b;
    {
      skvm::Arg dst = b.varying<uint16_t>(), src = b.varying<uint16_t>();
