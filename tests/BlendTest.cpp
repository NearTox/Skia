...
static Results test(Fn&& multiply) {
  ...
  for (int x = 0; x < 256; x++) {
    for (int y = 0; y < 256; y++) {
      int p = multiply(x, y), ideal = (x * y + 127) / 255;
      if (p != ideal) {
        r.diffs++;
        if (x == 0x00 || y == 0x00) {
          r.diffs_0x00++;
...
static sk_sp<SkSurface> create_gpu_surface_backend_texture_as_render_target(
    ...
    GrPixelConfig config, GrSurfaceOrigin origin, sk_sp<GrTexture>* backingSurface) {
  ...
  *backingSurface = resourceProvider->createTexture(
      backingDesc, GrRenderable::kYes, sampleCnt, SkBudgeted::kNo, GrProtected::kNo,
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(ES2BlendWithNoTexture, reporter, ctxInfo) {
  ...
  for (auto testCase : testCases) {
    ...
    REPORTER_ASSERT(
        reporter,
        bitmap.tryAllocPixels(SkImageInfo::Make(kWidth, kHeight, kColorType, kPremul_SkAlphaType)));
    REPORTER_ASSERT(
        reporter, surface->readPixels(bitmap.info(), bitmap.getPixels(), bitmap.rowBytes(), 0, 0));

    // Check the in/out pixels.
    REPORTER_ASSERT(
        reporter, bitmap.getColor(outPoint.x(), outPoint.y()) == SkColorSetRGB(0xFF, 0xFF, 0x80));
    REPORTER_ASSERT(
        reporter, bitmap.getColor(inPoint.x(), inPoint.y()) == SkColorSetRGB(0x80, 0xFF, 0x80));
