...
static sk_sp<SkSurface> create_gpu_surface_backend_texture_as_render_target(
  ...
  *backingSurface = resourceProvider->createTexture(
      dimensions, format, GrRenderable::kYes, sampleCnt, GrMipMapped::kNo, SkBudgeted::kNo,
      GrProtected::kNo);
  if (!(*backingSurface)) {
    return nullptr;
  }
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(ES2BlendWithNoTexture, reporter, ctxInfo) {
  ...
  for (auto testCase : testCases) {
    ...
    REPORTER_ASSERT(
        reporter,
        bitmap.tryAllocPixels(SkImageInfo::Make(kDimensions, kColorType, kPremul_SkAlphaType)));
    REPORTER_ASSERT(
