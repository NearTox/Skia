...
static void drawBG(SkCanvas* canvas) { canvas->drawColor(bgColor); }
 ...
 */
static bool compare(
    const SkBitmap& ref, const SkIRect& iref, const SkBitmap& test, const SkIRect& itest) {
  ...
  for (int style = 0; style <= kLastEnum_SkBlurStyle; ++style) {
    ...
    for (bool respectCTM : {false, true}) {
      ...
      for (size_t test = 0; test < SK_ARRAY_COUNT(tests); ++test) {
        ...
        refCanvas.translate(SkIntToScalar(-iref.fLeft), SkIntToScalar(-iref.fTop));
        drawBG(&refCanvas);
        ...
        for (int view = 0; view < tests[test].viewLen; ++view) {
          ...
          testCanvas.translate(SkIntToScalar(-itest.fLeft), SkIntToScalar(-itest.fTop));
          drawBG(&testCanvas);
          testCanvas.drawPath(path, paint);

          REPORTER_ASSERT(reporter, compare(refBitmap, iref, testBitmap, itest));
        }
      }
    }
  }
...
static void brute_force_1d(
    SkScalar stepMin, SkScalar stepMax, SkScalar gaussianSigma, int* result, int resultCount) {
...
static void cpu_blur_path(
    const SkPath& path, SkScalar gaussianSigma, int* result, int resultCount) {
  SkBitmap bitmap;
...
static void test_looper(
    skiatest::Reporter* reporter, sk_sp<SkDrawLooper> lp, SkScalar sigma, SkBlurStyle style,
    bool expectSuccess) {
  SkDrawLooper::BlurShadowRec rec;
  bool success = lp->asABlurShadow(&rec);
  REPORTER_ASSERT(reporter, success == expectSuccess);
...
static void test_layerDrawLooper(
    skiatest::Reporter* reporter, sk_sp<SkMaskFilter> mf, SkScalar sigma, SkBlurStyle style,
    bool expectSuccess) {
  SkLayerDrawLooper::LayerInfo info;
...
DEF_TEST(BlurAsABlur, reporter) {
  ...
  //
  {
    SkEmbossMaskFilter::Light light = {{1, 1, 1}, 0, 127, 127};
    for (size_t j = 0; j < SK_ARRAY_COUNT(sigmas); ++j) {
      const SkScalar sigma = sigmas[j];
...
DEF_TEST(BlurredRRectNinePatchComputation, reporter) {
  ...
  // not nine-patchable
  {
    ...
    ninePatchable = SkComputeBlurredRRectParams(
        rr, rr, SkRect::MakeEmpty(), kBlurRad, kBlurRad, &rrectToDraw, &size, rectXs, rectYs, texXs,
        texYs, &numX, &numY, &skipMask);
  ...
  // test-out occlusion
  {
    ...
    SkScalar testLocs[] = {-18.0f, -9.0f, 1.0f,   9.0f,   18.0f,  29.0f,  39.0f,
                           49.0f,  91.0f, 109.0f, 118.0f, 119.0f, 139.0f, 149.0f};
      ...
      for (int maxY = minY + 1; maxY < (int)SK_ARRAY_COUNT(testLocs); ++maxY) {
        for (int minX = 0; minX < (int)SK_ARRAY_COUNT(testLocs); ++minX) {
          for (int maxX = minX + 1; maxX < (int)SK_ARRAY_COUNT(testLocs); ++maxX) {
            SkRect occluder =
                SkRect::MakeLTRB(testLocs[minX], testLocs[minY], testLocs[maxX], testLocs[maxY]);
            if (occluder.isEmpty()) {
              continue;
            }

            ninePatchable = SkComputeBlurredRRectParams(
                rr, rr, occluder, kBlurRad, kBlurRad, &rrectToDraw, &size, rectXs, rectYs, texXs,
                texYs, &numX, &numY, &skipMask);
            ...
            for (int y = 0; y < numY - 1; ++y) {
              for (int x = 0; x < numX - 1; ++x) {
                SkRect cell = SkRect::MakeLTRB(rectXs[x], rectYs[y], rectXs[x + 1], rectYs[y + 1]);
                REPORTER_ASSERT(
                    reporter,
                    SkToBool(skipMask & checkBit) == (cell.isEmpty() || occluder.contains(cell)));

                REPORTER_ASSERT(reporter, texXs[x] >= 0 && texXs[x] <= size.fWidth);
                REPORTER_ASSERT(reporter, texYs[y] >= 0 && texXs[y] <= size.fHeight);
...
DEF_TEST(EmbossPerlinCrash, reporter) {
  ...
  static constexpr SkEmbossMaskFilter::Light light = {{1, 1, 1}, 0, 127, 127};
