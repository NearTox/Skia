...
static void test_Nf(skiatest::Reporter* r) {
  auto assert_nearly_eq = [&](float eps, const SkNx<N, float>& v, float a, float b, float c,
                              float d) {
    auto close = [=](float a, float b) { return fabsf(a - b) <= eps; };
    float vals[4];
    v.store(vals);
    bool ok = close(vals[0], a) && close(vals[1], b) && close(v[0], a) && close(v[1], b);
    REPORTER_ASSERT(r, ok);
    if (N == 4) {
      ok = close(vals[2], c) && close(vals[3], d) && close(v[2], c) && close(v[3], d);
      REPORTER_ASSERT(r, ok);
    }
  };
  ...
  float vals[] = {3, 4, 5, 6};
  SkNx<N, float> a = SkNx<N, float>::Load(vals), b(a), c = a;
  SkNx<N, float> d;
  d = a;
  ...
  auto assert_eq = [&](const SkNx<N, T>& v, T a, T b, T c, T d, T e, T f, T g, T h) {
    ...
    switch (N) {
      case 8:
        ...
        [[fallthrough]];
      case 4: REPORTER_ASSERT(r, vals[2] == c && vals[3] == d); [[fallthrough]];
      case 2: REPORTER_ASSERT(r, vals[0] == a && vals[1] == b);
    }
    switch (N) {
      case 8: REPORTER_ASSERT(r, v[4] == e && v[5] == f && v[6] == g && v[7] == h); [[fallthrough]];
      case 4: REPORTER_ASSERT(r, v[2] == c && v[3] == d); [[fallthrough]];
      case 2: REPORTER_ASSERT(r, v[0] == a && v[1] == b);
    }
  };

  T vals[] = {1, 2, 3, 4, 5, 6, 7, 8};
  SkNx<N, T> a = SkNx<N, T>::Load(vals), b(a), c = a;
  SkNx<N, T> d;
  d = a;
...
DEF_TEST(SkNi_min_lt, r) {
  ...
  for (int i = 0; i < (1 << 16); i++) {
    uint16_t a = rand.nextU() >> 16, b = rand.nextU() >> 16;
...
DEF_TEST(SkNx_4fLoad4Store4, r) {
  float src[] = {0.0f, 1.0f, 2.0f,  3.0f,  4.0f,  5.0f,  6.0f,  7.0f,
