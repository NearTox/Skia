...
DEF_TEST(SkScalerCacheMultiThread, Reporter) {
  sk_sp<SkTypeface> typeface = ToolUtils::create_portable_typeface("serif", SkFontStyle::Italic());
  ...
  SkStrikeSpec strikeSpec = SkStrikeSpec::MakeMask(
      font, defaultPaint, SkSurfaceProps(0, kUnknown_SkPixelGeometry), SkScalerContextFlags::kNone,
      SkMatrix::I());
  ...
  for (int tries = 0; tries < 100; tries++) {
    ...
    auto perThread = [&](int threadIndex) {
      ...
      for (int i = 0; i < 100; i++) {
        ...
        drawable.startBitmapDevice(
            rejects.source(), {0, 0}, SkMatrix::I(), scalerCache.roundingSpec());
