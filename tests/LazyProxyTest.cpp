...
class LazyProxyTest final : public GrOnFlushCallbackObject {
 public:
  LazyProxyTest(skiatest::Reporter* reporter)
      : fReporter(reporter), fHasOpTexture(false), fHasClipTexture(false) {}
  ...
  class Op final : public GrDrawOp {
   public:
    ...
    static GrOp::Owner Make(
        GrRecordingContext* context, GrProxyProvider* proxyProvider, LazyProxyTest* test,
        bool nullTexture) {
   ...
   private:
    ...
    Op(GrRecordingContext* ctx, GrProxyProvider* proxyProvider, LazyProxyTest* test,
       bool nullTexture)
        : GrDrawOp(ClassID()), fTest(test) {
      const GrBackendFormat format =
          ctx->priv().caps()->getDefaultBackendFormat(GrColorType::kBGR_565, GrRenderable::kNo);
      fProxy = GrProxyProvider::MakeFullyLazyProxy(
          [this, nullTexture](GrResourceProvider* rp, const GrSurfaceProxy::LazySurfaceDesc& desc)
    ...
    GrProcessorSet::Analysis finalize(
        const GrCaps&, const GrAppliedClip* clip, GrClampType) override {
  ...
  class ClipFP : public GrFragmentProcessor {
   public:
    ClipFP(
        GrRecordingContext* ctx, GrProxyProvider* proxyProvider, LazyProxyTest* test,
      ...
      fLazyProxy = GrProxyProvider::MakeFullyLazyProxy(
          ...
          GrSurfaceProxy::UseAllocator::kYes);
      auto atlasEffect =
          GrTextureEffect::Make({fLazyProxy, kOrigin, readSwizzle}, kPremul_SkAlphaType);
  ...
  class Clip : public GrClip {
   public:
    Clip(LazyProxyTest* test, GrTextureProxy* atlas) : fTest(test), fAtlas(atlas) {}

   private:
    SkIRect getConservativeBounds() const final { return SkIRect::MakeSize(fAtlas->dimensions()); }
    Effect apply(
        GrRecordingContext* rContext, skgpu::v1::SurfaceDrawContext*, GrDrawOp*, GrAAType,
...
DEF_GPUTEST(LazyProxyTest, reporter, /* options */) {
  ...
  for (bool nullTexture : {false, true}) {
    ...
    auto mockAtlas = skgpu::v1::SurfaceDrawContext::Make(
    ...
    sdc->addDrawOp(&clip, LazyProxyTest::Op::Make(ctx.get(), proxyProvider, &test, nullTexture));
      ...
      class TestCallback {
       public:
        TestCallback(int* value, bool releaseCallback, sk_sp<GrTexture> tex)
            : fValue(value), fReleaseCallback(releaseCallback), fTexture(std::move(tex)) {}
        ...
        LazyInstantiationResult operator()(
            GrResourceProvider*, const GrSurfaceProxy::LazySurfaceDesc&) const {
          *fValue = 1;
...
class LazyFailedInstantiationTestOp : public GrDrawOp {
 public:
  ...
  static GrOp::Owner Make(
    ...
    return GrOp::Make<LazyFailedInstantiationTestOp>(
        rContext, rContext->priv().caps(), proxyProvider, testExecuteValue,
...
DEF_GPUTEST(LazyProxyFailedInstantiationTest, reporter, /* options */) {
  ...
  for (bool failInstantiation : {false, true}) {
    ...
    sdc->addDrawOp(LazyFailedInstantiationTestOp::Make(
