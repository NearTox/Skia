...
static void fill_and_stroke(
    SkCanvas* canvas, const SkPath& p1, const SkPath& p2, sk_sp<SkPathEffect> effect) {
  SkPaint paint;
  paint.setAntiAlias(true);
  ...
  for (auto& test_func : {&test_drawPathEmpty, &test_drawSameRectOvals}) {
    for (auto& sampleCount : {1, 4, 16}) {
      ...
      auto surface(SkSurface::MakeRenderTarget(
          ctxInfo.directContext(), SkBudgeted::kNo, info, sampleCount, nullptr));
      if (!surface) {
        continue;
      }
...
DEF_GPUTEST_FOR_ALL_CONTEXTS(GrDrawCollapsedPath, reporter, ctxInfo) {
  ...
  SkMatrix m;
  m.setAll(
      0.966006875f, -0.125156224f, 72.0899811f, -0.00885376986f, -0.112347461f, 64.7121124f,
...
DEF_GPUTEST(GrPathKeys, reporter, /* options */) {
  ...
  GrStyle styles[] = {GrStyle::SimpleFill(), GrStyle::SimpleHairline(), GrStyle(strokePaint)};
