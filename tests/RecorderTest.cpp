...
DEF_TEST(Recorder_drawImage_takeReference, reporter) {
...

  {
    ...
    recorder.drawImageRect(
        image.get(), SkRect::MakeWH(100, 100), SkRect::MakeWH(100, 100), SkSamplingOptions(),
...
DEF_TEST(Recorder_boundsOverflow, reporter) {
  ...
  REPORTER_ASSERT(reporter, recorder.imageInfo().width() > 0 && recorder.imageInfo().height() > 0);
}
