...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrTextureMipMapInvalidationTest, reporter, ctxInfo) {
  ...
  auto isMipped = [reporter](SkSurface* surf) {
    ...
    GrTextureProxy* proxy =
        sk_gpu_test::GetTextureImageProxy(image.get(), surf->recordingContext());
  ...
  for (auto allocateMips : {false, true}) {
    auto surf1 = SkSurface::MakeRenderTarget(
        context, SkBudgeted::kYes, info, 0, kBottomLeft_GrSurfaceOrigin, nullptr, allocateMips);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ReimportImageTextureWithMipLevels, reporter, ctxInfo) {
  ...
  img = SkImage::MakeFromTexture(
      dContext, btex, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, kPremul_SkAlphaType,
      nullptr);
  ...
  surf = SkSurface::MakeRenderTarget(
      dContext, SkBudgeted::kYes, singlePixelInfo, 1, kTopLeft_GrSurfaceOrigin, nullptr);

  surf->getCanvas()->drawImageRect(
      img, SkRect::MakeWH(1, 1), SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kLinear));
  uint32_t pixel;
