...
static void drawAndTest(
    skiatest::Reporter* reporter, const SkPath& path, const SkPaint& paint, bool shouldDraw) {
  SkBitmap bm;
  ...
  if (!success) {
    ...
    ERRORF(
        reporter,
        "%s style[%d] cap[%d] join[%d] antialias[%d]"
        " filltype[%d] ptcount[%d]",
        str, paint.getStyle(), paint.getStrokeCap(), paint.getStrokeJoin(), paint.isAntiAlias(),
...
static void test_emptydrawing(skiatest::Reporter* reporter) {
  ...
  for (int doClose = 0; doClose < 2; ++doClose) {
    for (size_t i = 0; i < SK_ARRAY_COUNT(gMakeProc); ++i) {
      ...
      bool allowCaps = make_L == gMakeProc[i] || make_Q == gMakeProc[i] || make_C == gMakeProc[i] ||
                       make_MZM == gMakeProc[i];
...
DEF_TEST(EmptyPath, reporter) { test_emptydrawing(reporter); }
