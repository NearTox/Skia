...
DEF_TEST(TypefaceMacVariation, reporter) {
  ...
  for (auto& originalCoordinate : originalPosition) {
    SkMaybeDebugf(
        "(%s: %f) ", tagToString(originalCoordinate.axis).c_str(), originalCoordinate.value);
  }
  SkMaybeDebugf("\n\n");
      ...
      for (float testCoordinate : {100, 300, 400, 500, 700, 900}) {
        ...
        for (auto& originalCoordinate : originalPosition) {
          ...
          if (originalCoordinate.axis == kOpszTag && omitOpsz) {
            SkMaybeDebugf("#%s: %f# ", tagToString(originalCoordinate.axis).c_str(), requestValue);
          } else {
        ...
        for (auto& expectedCoordinate : expectedPosition) {
          SkMaybeDebugf(
              "(%s: %f) ", tagToString(expectedCoordinate.axis).c_str(), expectedCoordinate.value);
        }
        SkMaybeDebugf("\n");
        ...
        for (auto& cloneCoordinate : clonePosition) {
          SkMaybeDebugf(
              "(%s: %f) ", tagToString(cloneCoordinate.axis).c_str(), cloneCoordinate.value);
        }
        SkMaybeDebugf("\n");
