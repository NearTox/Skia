...
void TestReadPixels(
    skiatest::Reporter* reporter, GrDirectContext* dContext, GrSurfaceContext* srcContext,
  ...
  SkImageInfo ii = SkImageInfo::Make(
      srcContext->width(), srcContext->height(), kRGBA_8888_SkColorType, kPremul_SkAlphaType);
  ...
  for (int i = 0; i < pixelCnt; ++i) {
    if (pixels.get()[i] != expectedPixelValues[i]) {
      ERRORF(
          reporter, "%s: Error, pixel value %d should be 0x%08x, got 0x%08x.", testName, i,
...
void TestWritePixels(
    skiatest::Reporter* reporter, GrDirectContext* dContext, GrSurfaceContext* dstContext,
  ...
  SkImageInfo ii = SkImageInfo::Make(
      dstContext->width(), dstContext->height(), kRGBA_8888_SkColorType, kPremul_SkAlphaType);
...
void TestCopyFromSurface(
    skiatest::Reporter* reporter, GrDirectContext* dContext, GrSurfaceProxy* proxy,
    GrSurfaceOrigin origin, GrColorType colorType, uint32_t expectedPixelValues[],
    const char* testName) {
  auto copy = GrSurfaceProxy::Copy(
      dContext, proxy, origin, GrMipmapped::kNo, SkBackingFit::kExact, SkBudgeted::kYes);
  ...
  auto dstContext =
      GrSurfaceContext::Make(dContext, std::move(view), colorType, kPremul_SkAlphaType, nullptr);
  SkASSERT(dstContext);
...
bool CreateBackendTexture(
    GrDirectContext* dContext, GrBackendTexture* backendTex, int width, int height,
    SkColorType colorType, const SkColor4f& color, GrMipmapped mipMapped, GrRenderable renderable,
    GrProtected isProtected) {
  SkImageInfo info = SkImageInfo::Make(width, height, colorType, kPremul_SkAlphaType);
  return CreateBackendTexture(
      dContext, backendTex, info, color, mipMapped, renderable, isProtected);
}

bool CreateBackendTexture(
    GrDirectContext* dContext, GrBackendTexture* backendTex, const SkImageInfo& ii,
...
bool DoesFullBufferContainCorrectColor(
    const GrColor* srcBuffer, const GrColor* dstBuffer, int width, int height) {
...
bool compare_pixels(
    int width, int height, const char* floatA, std::function<AccessPixelFn>& atA,
    const char* floatB, std::function<AccessPixelFn>& atB, const float tolRGBA[4],
...
bool ComparePixels(
    const GrImageInfo& infoA, const char* a, size_t rowBytesA, const GrImageInfo& infoB,
    const char* b, size_t rowBytesB, const float tolRGBA[4],
    std::function<ComparePixmapsErrorReporter>& error) {
  ...
  if ((infoA.alphaType() == kPremul_SkAlphaType || infoB.alphaType() == kPremul_SkAlphaType) &&
      (infoA.alphaType() == kUnpremul_SkAlphaType || infoB.alphaType() == kUnpremul_SkAlphaType)) {
  ...
  GrImageInfo floatInfo(
      GrColorType::kRGBA_F32, floatAlphaType, std::move(floatCS), infoA.width(), infoA.height());
  ...
  return compare_pixels(
      infoA.width(), infoA.height(), floatA.get(), at, floatB.get(), at, tolRGBA, error);
}

bool ComparePixels(
    const SkPixmap& a, const SkPixmap& b, const float tolRGBA[4],
    std::function<ComparePixmapsErrorReporter>& error) {
  return ComparePixels(
      a.info(), static_cast<const char*>(a.addr()), a.rowBytes(), b.info(),
      static_cast<const char*>(b.addr()), b.rowBytes(), tolRGBA, error);
}

bool CheckSolidPixels(
    const SkColor4f& col, const SkPixmap& pixmap, const float tolRGBA[4],
    std::function<ComparePixmapsErrorReporter>& error) {
  size_t floatBpp = GrColorTypeBytesPerPixel(GrColorType::kRGBA_F32);
  ...
  // First convert 'col' to be compatible with 'pixmap'
  {
    ...
    SkAssertResult(GrConvertPixels(dstInfo, floatA.get(), floatBpp, srcInfo, col.vec(), floatBpp));
  }
  ...
  {
    GrImageInfo dstInfo(
        GrColorType::kRGBA_F32, pixmap.alphaType(), pixmap.refColorSpace(), pixmap.width(),
        pixmap.height());

    SkAssertResult(GrConvertPixels(
        dstInfo, floatB.get(), floatRowBytes, pixmap.info(), pixmap.addr(), pixmap.rowBytes()));
  }

  auto atA = std::function<AccessPixelFn>([](const char* floatBuffer, int /* x */, int /* y */) {
  ...
  return compare_pixels(
      pixmap.width(), pixmap.height(), floatA.get(), atA, floatB.get(), atB, tolRGBA, error);
}
...
void CheckSingleThreadedProxyRefs(
  ...
  REPORTER_ASSERT(
      reporter, proxy->refCntGreaterThan(expectedProxyRefs - 1) &&
...
static SkGlyphID hash_to_glyph(uint32_t value) { return SkToU16(((value >> 16) ^ value) & 0xFFFF); }
