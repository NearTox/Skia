...
void test_copy_from_surface(
    skiatest::Reporter* reporter, GrContext* context, GrSurfaceProxy* proxy,
    uint32_t expectedPixelValues[], bool onlyTestRTConfig, const char* testName) {
  ...
  for (auto flags : {kNone_GrSurfaceFlags, kRenderTarget_GrSurfaceFlag}) {
    ...
    auto origin =
        (kNone_GrSurfaceFlags == flags) ? kTopLeft_GrSurfaceOrigin : kBottomLeft_GrSurfaceOrigin;
...
void test_copy_to_surface(
    skiatest::Reporter* reporter, GrContext* context, GrSurfaceContext* dstContext,
    const char* testName) {
  int pixelCnt = dstContext->width() * dstContext->height();
  SkAutoTMalloc<uint32_t> pixels(pixelCnt);
  for (int y = 0; y < dstContext->width(); ++y) {
  ...
  for (auto renderable : {GrRenderable::kNo, GrRenderable::kYes}) {
    for (auto origin : {kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin}) {
      auto src = sk_gpu_test::MakeTextureProxyFromData(
          context, renderable, dstContext->width(), dstContext->height(), kRGBA_8888_SkColorType,
          origin, pixels.get(), 0);
...
bool create_backend_texture(
    GrContext* context, GrBackendTexture* backendTex, const SkImageInfo& ii, GrMipMapped mipMapped,
    SkColor color, GrRenderable renderable) {
  ...
  *backendTex = gpu->createTestingOnlyBackendTexture(
      ii.width(), ii.height(), ii.colorType(), mipMapped, renderable, bm.getPixels(),
      bm.rowBytes());
...
bool does_full_buffer_contain_correct_color(
    GrColor* srcBuffer, GrColor* dstBuffer, int width, int height) {
