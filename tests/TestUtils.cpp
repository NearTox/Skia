...
void TestReadPixels(
  ...
  SkImageInfo ii =
      SkImageInfo::Make(srcContext->dimensions(), kRGBA_8888_SkColorType, kPremul_SkAlphaType);
  ...
  for (int i = 0; i < pixelCnt; ++i) {
    if (pm.addr32()[i] != expectedPixelValues[i]) {
      ERRORF(
          ...
          expectedPixelValues[i], pm.addr32()[i]);
      break;
    }
  }
}
...
void TestWritePixels(
  ...
  SkImageInfo ii =
      SkImageInfo::Make(dstContext->dimensions(), kRGBA_8888_SkColorType, kPremul_SkAlphaType);
  SkAutoPixmapStorage pm;
  pm.alloc(ii);
