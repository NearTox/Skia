...
static struct quadCubic {
  ...
  QuadPts quad;
    ...
    {{{{945.08099365234375, 747.1619873046875},
       {982.5679931640625, 747.1619873046875},
       {1013.6290283203125, 719.656005859375},
       {1019.1910400390625, 683.72601318359375}}},
    ...
    {{{{55, 207}, {52.238574981689453, 207}, {50, 204.76142883300781}, {50, 202}}},
     {{{55, 207},
       {52.929431915283203, 206.99949645996094},
       {51.464466094970703, 205.53553771972656}}}},

    {{{{49, 47}, {49, 74.614250183105469}, {26.614250183105469, 97}, {-1, 97}}},
     {{{-8.659739592076221e-015, 96.991401672363281},
       {20.065492630004883, 96.645187377929688},
...
static void cubicQuadIntersection(skiatest::Reporter* reporter, int index) {
  ...
  for (int pt = 0; pt < roots; ++pt) {
    ...
    if (!xy1.approximatelyEqual(xy2)) {
      SkDebugf(
          "%s [%d,%d] x!= t1=%g (%g,%g) t2=%g (%g,%g)\n", __FUNCTION__, iIndex, pt, tt1, xy1.fX,
          xy1.fY, tt2, xy2.fX, xy2.fY);
    }
...
DEF_TEST(PathOpsCubicQuadIntersectionOneOff, reporter) { cubicQuadIntersection(reporter, 0); }
