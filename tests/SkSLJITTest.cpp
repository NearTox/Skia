...
void test(skiatest::Reporter* r, const char* src, type x, type y, type result) {
  ...
  std::unique_ptr<SkSL::Program> program =
      compiler.convertProgram(SkSL::Program::kPipelineStage_Kind, SkSL::String(src), settings);
...
DEF_TEST(SkSLJITMul, r) {
  ...
  test<float>(r, "float test(float x, float y) { return (float2(x) * float2(y)).x; }", 36, 6, 216);
}
...
DEF_TEST(SkSLJITDiv, r) {
  ...
  test<int>(r, "int test(int x, int y) { return (int2(x) / int2(y)).x; }", 0, -100, 0);
  test<float>(r, "float test(float x, float y) { return (float2(x) / float2(y)).x; }", 36, 6, 6);
}
...
DEF_TEST(SkSLJITIf, r) {
  ...
  test<int>(
      r,
      "int test(int x, int y) { if (x > y) if (x > 0) return x; else return -x; "
      "else if (y > 0) return y; else return -y; }",
      -8, -17, 8);
}
...
DEF_TEST(SkSLJITTernary, r) {
  ...
  test<int>(
      r,
      "int test(int x, int y) { return x > y ? (x > 0 ? x : -x) :"
      "(y > 0 ? y : -y); }",
      -8, -17, 8);
}
