...
struct PromiseTextureChecker {
  ...
  // image.
  explicit PromiseTextureChecker(
      const GrBackendTexture& tex, skiatest::Reporter* reporter, bool shared)
  ...
  static void Done(void* self) { static_cast<PromiseTextureChecker*>(self)->fDoneCount++; }
};
...
static void check_fulfill_and_release_cnts(
    skiatest::Reporter* reporter, const PromiseTextureChecker& promiseChecker,
    int expectedFulfillCnt, ReleaseBalanceExpectation releaseBalanceExpecation,
  ...
  switch (doneBalanceExpecation) {
    case DoneBalanceExpectation::kBalanced: REPORTER_ASSERT(reporter, !doneDiff); break;
    ...
    case DoneBalanceExpectation::kUnknown:
      ...
      break;
    case DoneBalanceExpectation::kUnbalancedByOne: REPORTER_ASSERT(reporter, doneDiff == 1); break;
...
static void check_unfulfilled(
    const PromiseTextureChecker& promiseChecker, skiatest::Reporter* reporter) {
  check_fulfill_and_release_cnts(
      reporter, promiseChecker, 0, ReleaseBalanceExpectation::kBalanced,
      DoneBalanceExpectation::kBalanced);
}

static void check_only_fulfilled(
    skiatest::Reporter* reporter, const PromiseTextureChecker& promiseChecker,
    int expectedFulfillCnt = 1) {
  check_fulfill_and_release_cnts(
      reporter, promiseChecker, expectedFulfillCnt, ReleaseBalanceExpectation::kAllUnbalanced,
      DoneBalanceExpectation::kAllUnbalanced);
}

static void check_all_flushed_but_not_synced(
  ...
  check_fulfill_and_release_cnts(
      reporter, promiseChecker, expectedFulfillCnt, ReleaseBalanceExpectation::kBalanced,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(PromiseImageTextureReuseDifferentConfig, reporter, ctxInfo) {
  ...
  GrBackendFormat gray8Format = ctx->defaultBackendFormat(kGray_8_SkColorType, GrRenderable::kNo);
  GrBackendFormat alpha8Format = ctx->defaultBackendFormat(kAlpha_8_SkColorType, GrRenderable::kNo);
  ...
  GrBackendTexture grayBackendTex = ctx->createBackendTexture(
      kWidth, kHeight, gray8Format, SkColors::kTransparent, GrMipMapped::kNo, GrRenderable::kNo,
      GrProtected::kNo);
  ...
  sk_sp<SkImage> alphaImg(SkImage_Gpu::MakePromiseTexture(
      ctx, alpha8Format, kWidth, kHeight, GrMipMapped::kNo, kTopLeft_GrSurfaceOrigin,
      kAlpha_8_SkColorType, kPremul_SkAlphaType, nullptr, PromiseTextureChecker::Fulfill,
  ...
  sk_sp<SkImage> grayImg(SkImage_Gpu::MakePromiseTexture(
      ctx, gray8Format, kWidth, kHeight, GrMipMapped::kNo, kBottomLeft_GrSurfaceOrigin,
      kGray_8_SkColorType, kOpaque_SkAlphaType, nullptr, PromiseTextureChecker::Fulfill,
      PromiseTextureChecker::Release, PromiseTextureChecker::Done, &promiseChecker,
      SkDeferredDisplayListRecorder::PromiseImageApiVersion::kNew));
  ...
  check_fulfill_and_release_cnts(
      reporter, promiseChecker, 2, ReleaseBalanceExpectation::kUnbalancedByOne,
...
DEF_GPUTEST(PromiseImageTextureShutdown, reporter, ctxInfo) {
  ...
  for (int type = 0; type < sk_gpu_test::GrContextFactory::kContextTypeCnt; ++type) {
    ...
    for (auto contextDeath : contextKillers) {
      ...
      GrBackendTexture backendTex = ctx->createBackendTexture(
          kWidth, kHeight, kAlpha_8_SkColorType, SkColors::kTransparent, GrMipMapped::kNo,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(PromiseImageTextureFullCache, reporter, ctxInfo) {
  ...
  sk_sp<SkImage> image(SkImage_Gpu::MakePromiseTexture(
      ctx, backendTex.getBackendFormat(), kWidth, kHeight, GrMipMapped::kNo,
      kTopLeft_GrSurfaceOrigin, kAlpha_8_SkColorType, kPremul_SkAlphaType, nullptr,
      PromiseTextureChecker::Fulfill, PromiseTextureChecker::Release, PromiseTextureChecker::Done,
      &promiseChecker, SkDeferredDisplayListRecorder::PromiseImageApiVersion::kNew));
  REPORTER_ASSERT(reporter, image);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(PromiseImageNullFulfill, reporter, ctxInfo) {
  ...
  sk_sp<SkImage> refImg(SkImage_Gpu::MakePromiseTexture(
      ctx, backendFormat, kWidth, kHeight, GrMipMapped::kNo, texOrigin, kRGBA_8888_SkColorType,
      kPremul_SkAlphaType, nullptr, fulfill, release, done, &counts,
      SkDeferredDisplayListRecorder::PromiseImageApiVersion::kNew));
