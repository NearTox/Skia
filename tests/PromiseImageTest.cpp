...
static void check_fulfill_and_release_cnts(
  ...
  switch (releaseBalanceExpecation) {
    ...
    case ReleaseBalanceExpectation::kUnknown:
      REPORTER_ASSERT(reporter, releaseDiff >= 0 && releaseDiff <= promiseChecker.fFulfillCount);
      break;
...
static void check_unfulfilled(
  ...
  check_fulfill_and_release_cnts(reporter, promiseChecker, 0, ReleaseBalanceExpectation::kBalanced);
}
...
static void check_all_flushed_but_not_synced(
  ...
  check_fulfill_and_release_cnts(
      reporter, promiseChecker, expectedFulfillCnt, releaseBalanceExpectation);
}
  ...
  for (int type = 0; type < sk_gpu_test::GrContextFactory::kContextTypeCnt; ++type) {
    ...
    for (const DeathFn& contextDeath : contextKillers) {
      ...
      auto mbet = sk_gpu_test::ManagedBackendTexture::MakeWithoutData(
          ctx, kWidth, kHeight, kAlpha_8_SkColorType, GrMipmapped::kNo, GrRenderable::kNo);
      if (!mbet) {
