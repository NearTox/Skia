  ...
  for (int type = 0; type < sk_gpu_test::GrContextFactory::kContextTypeCnt; ++type) {
    ...
    for (const DeathFn& contextDeath : contextKillers) {
      ...
      CreateBackendTexture(
          ctx, &backendTex, kWidth, kHeight, kAlpha_8_SkColorType, SkColors::kTransparent,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(PromiseImageNullFulfill, reporter, ctxInfo) {
  ...
  GrBackendTexture backendTex;
  CreateBackendTexture(
      dContext, &backendTex, kWidth, kHeight, kRGBA_8888_SkColorType, SkColors::kTransparent,
  ...
  GrSurfaceOrigin texOrigin = kTopLeft_GrSurfaceOrigin;
  sk_sp<SkImage> refImg(SkImage_Gpu::MakePromiseTexture(
      dContext, backendFormat, kWidth, kHeight, GrMipmapped::kNo, texOrigin, kRGBA_8888_SkColorType,
