...
static void draw_paint_with_aa(skgpu::v1::SurfaceDrawContext* sdc,
    ...
    sdc->drawRect(
        nullptr, std::move(paint), GrAA::kYes, SkMatrix::I(), SkRect::MakeIWH(kWidth, kHeight),
        nullptr);
}
...
DEF_GPUTEST_FOR_CONTEXTS(DMSAA_preserve_contents,
    ...
    auto sdc = skgpu::v1::SurfaceDrawContext::Make(
        dContext, GrColorType::kRGBA_8888, nullptr, SkBackingFit::kApprox, {kWidth, kHeight},
        kDMSAAProps);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(DMSAA_dual_source_blend_disable, reporter, ctxInfo) {
    ...
    auto sourceTexture = context->createBackendTexture(
        texDims.width(), texDims.height(), kRGBA_8888_SkColorType, SkColors::kBlue,
    ...
    auto texture1 = context->createBackendTexture(
        surfaceDims.width(), surfaceDims.height(), kRGBA_8888_SkColorType, SkColors::kRed,
        GrMipmapped::kNo, GrRenderable::kYes, GrProtected::kNo);

    auto texture2 = context->createBackendTexture(
        surfaceDims.width(), surfaceDims.height(), kRGBA_8888_SkColorType, SkColors::kYellow,
        GrMipmapped::kNo, GrRenderable::kYes, GrProtected::kNo);
