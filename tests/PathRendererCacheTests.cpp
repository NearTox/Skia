...
static void draw_path(
    ...
    skgpu::v1::PathRenderer* pr, GrAAType aaType, const GrStyle& style, float scaleX = 1.f) {
  GrPaint paint;
...
static void test_path(
    ...
    std::function<skgpu::v1::PathRenderer*(GrRecordingContext*)> makePathRenderer, int expected,
...
DEF_GPUTEST(TriangulatingPathRendererCacheTest, reporter, /* options */) {
  auto createPR = [](GrRecordingContext*) { return new skgpu::v1::TriangulatingPathRenderer(); };
  ...
  GrStyle style(paint);
  test_path(
      reporter, create_concave_path, createPR, kExpectedResources, false, GrAAType::kNone, style);
}
...
DEF_GPUTEST(SoftwarePathRendererCacheTest, reporter, /* options */) {
  ...
  test_path(reporter, create_concave_path, createPR, kExpectedResources, true, GrAAType::kCoverage);
  ...
  GrStyle style(paint);
  test_path(
      reporter, create_concave_path, createPR, kExpectedResources, true, GrAAType::kCoverage,
      style);
}
