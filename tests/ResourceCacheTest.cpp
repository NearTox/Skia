...
static sk_sp<GrRenderTarget> create_RT_with_SB(
  ...
  sk_sp<GrTexture> tex(provider->createTexture(
      desc, GrRenderable::kYes, sampleCount, budgeted, GrProtected::kNo,
...
DEF_GPUTEST_FOR_CONTEXTS(
    ResourceCacheStencilBuffers, &is_rendering_and_not_angle_es3, reporter, ctxInfo, nullptr) {
  GrContext* context = ctxInfo.grContext();
  ...
  int smallSampleCount = context->priv().caps()->getRenderTargetSampleCount(2, grColorType, format);
  if (smallSampleCount > 1) {
    // An RT with a different sample count should not share.
    sk_sp<GrRenderTarget> smallMSAART0 =
        create_RT_with_SB(resourceProvider, 4, smallSampleCount, SkBudgeted::kNo);
    ...
    int bigSampleCount = context->priv().caps()->getRenderTargetSampleCount(5, grColorType, format);
    if (bigSampleCount > 0 && bigSampleCount != smallSampleCount) {
      sk_sp<GrRenderTarget> smallMSAART2 =
          create_RT_with_SB(resourceProvider, 4, bigSampleCount, SkBudgeted::kNo);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ResourceCacheWrappedResources, reporter, ctxInfo) {
  ...
  backendTextures[0] = context->createBackendTexture(
      kW, kH, kRGBA_8888_SkColorType, SkColors::kTransparent, GrMipMapped::kNo, GrRenderable::kNo,
      GrProtected::kNo);
  backendTextures[1] = context->createBackendTexture(
      kW, kH, kRGBA_8888_SkColorType, SkColors::kTransparent, GrMipMapped::kNo, GrRenderable::kNo,
      GrProtected::kNo);
...
class TestResource : public GrGpuResource {
 ...
 public:
  ...
  static TestResource* CreateScratch(
      GrGpu* gpu, SkBudgeted budgeted, SimulatedProperty property, size_t size = kDefaultSize) {
    return new TestResource(gpu, budgeted, property, kScratchConstructor, size);
  }
  static TestResource* CreateWrapped(
      GrGpu* gpu, GrWrapCacheable cacheable, size_t size = kDefaultSize) {
    return new TestResource(gpu, cacheable, size);
  }

  ~TestResource() override { --fNumAlive; }

  static int NumAlive() { return fNumAlive; }

  void setUnrefWhenDestroyed(sk_sp<TestResource> resource) { fToDelete = std::move(resource); }
 ...
 private:
  ...
  TestResource(
      GrGpu* gpu, SkBudgeted budgeted, SimulatedProperty property, ScratchConstructor,
      size_t size = kDefaultSize)
      : INHERITED(gpu), fToDelete(nullptr), fSize(size), fProperty(property), fIsScratch(true) {
    ++fNumAlive;
    this->registerWithCache(budgeted);
  }
...
static void test_no_key(skiatest::Reporter* reporter) {
  ...
  REPORTER_ASSERT(
      reporter, a->gpuMemorySize() + b->gpuMemorySize() + c->gpuMemorySize() + d->gpuMemorySize() ==
  ...
  REPORTER_ASSERT(
      reporter,
      b->gpuMemorySize() + c->gpuMemorySize() + d->gpuMemorySize() == cache->getResourceBytes());
...
static void test_purge_unlocked(skiatest::Reporter* reporter) {
  ...
  TestResource* c =
      TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kA_SimulatedProperty, 13);

  GrUniqueKey uniqueKey2;
  make_unique_key<0>(&uniqueKey2, 1);

  TestResource* d =
      TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kA_SimulatedProperty, 14);
...
static void test_budgeting(skiatest::Reporter* reporter) {
  ...
  TestResource* scratch =
      TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kB_SimulatedProperty, 10);
  ...
  REPORTER_ASSERT(
      reporter, scratch->gpuMemorySize() + unique->gpuMemorySize() +
                        wrappedCacheable->gpuMemorySize() + wrappedUncacheable->gpuMemorySize() +
                        unbudgeted->gpuMemorySize() ==
                    cache->getResourceBytes());
  REPORTER_ASSERT(reporter, 2 == cache->getBudgetedResourceCount());
  REPORTER_ASSERT(
      reporter,
      scratch->gpuMemorySize() + unique->gpuMemorySize() == cache->getBudgetedResourceBytes());
  ...
  REPORTER_ASSERT(
      reporter, scratch->gpuMemorySize() + unique->gpuMemorySize() +
                        wrappedCacheable->gpuMemorySize() + unbudgeted->gpuMemorySize() ==
                    cache->getResourceBytes());
  REPORTER_ASSERT(reporter, 0 == cache->getPurgeableBytes());
  ...
  REPORTER_ASSERT(
      reporter, scratch->gpuMemorySize() + wrappedCacheable->gpuMemorySize() +
                        wrappedUncacheable->gpuMemorySize() + unbudgeted->gpuMemorySize() ==
                    cache->getResourceBytes());
  ...
  REPORTER_ASSERT(reporter, 3 == cache->getResourceCount());
  REPORTER_ASSERT(
      reporter, unbudgeted->gpuMemorySize() + wrappedCacheable->gpuMemorySize() +
...
/*static*/ void test_unbudgeted_to_scratch(skiatest::Reporter* reporter) {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    REPORTER_ASSERT(
        reporter,
        GrBudgetedType::kUnbudgetedUncacheable == resource->resourcePriv().budgetedType());
    REPORTER_ASSERT(
        reporter,
        nullptr == cache->findAndRefScratchResource(
                       key, TestResource::kDefaultSize, GrResourceCache::ScratchFlags::kNone));
    REPORTER_ASSERT(reporter, 1 == cache->getResourceCount());
    ...
    resource = static_cast<TestResource*>(cache->findAndRefScratchResource(
        key, TestResource::kDefaultSize, GrResourceCache::ScratchFlags::kNone));
    REPORTER_ASSERT(reporter, resource);
    REPORTER_ASSERT(reporter, resource->resourcePriv().getScratchKey() == key);
    REPORTER_ASSERT(reporter, resource->cacheAccess().isScratch());
    REPORTER_ASSERT(reporter, GrBudgetedType::kBudgeted == resource->resourcePriv().budgetedType());
    ...
    if (0 == i) {
      ...
      REPORTER_ASSERT(reporter, !resource->cacheAccess().isScratch());
      REPORTER_ASSERT(
          reporter, GrBudgetedType::kBudgeted == resource->resourcePriv().budgetedType());
...
static void test_duplicate_scratch_key(skiatest::Reporter* reporter) {
  ...
  TestResource* a =
      TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kB_SimulatedProperty, 11);
  TestResource* b =
      TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kB_SimulatedProperty, 12);
  ...
  REPORTER_ASSERT(
      reporter, nullptr == cache->findAndRefScratchResource(
                               scratchKey1, TestResource::kDefaultSize,
                               GrResourceCache::ScratchFlags::kNone));
...
static void test_remove_scratch_key(skiatest::Reporter* reporter) {
  ...
  REPORTER_ASSERT(
      ...
      cache->findAndRefScratchResource(
          scratchKey, TestResource::kDefaultSize, GrResourceCache::ScratchFlags::kNone) == nullptr);
  ...
  GrGpuResource* find;
  find = cache->findAndRefScratchResource(
      scratchKey, TestResource::kDefaultSize, GrResourceCache::ScratchFlags::kNone);
...
static void test_scratch_key_consistency(skiatest::Reporter* reporter) {
  ...
  TestResource::ComputeScratchKey(TestResource::kB_SimulatedProperty, &scratchKey);
  GrGpuResource* find = cache->findAndRefScratchResource(
      scratchKey, TestResource::kDefaultSize, GrResourceCache::ScratchFlags::kNone);
  ...
  find = cache->findAndRefScratchResource(
      scratchKey2, TestResource::kDefaultSize, GrResourceCache::ScratchFlags::kNone);
  REPORTER_ASSERT(reporter, find != nullptr);
  REPORTER_ASSERT(reporter, find == a || find == b);

  GrGpuResource* find2 = cache->findAndRefScratchResource(
      scratchKey2, TestResource::kDefaultSize, GrResourceCache::ScratchFlags::kNone);
...
static void test_purge_invalidated(skiatest::Reporter* reporter) {
  ...
  TestResource* c =
      TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kA_SimulatedProperty);
  ...
  GrScratchKey scratchKey;
  TestResource::ComputeScratchKey(TestResource::kA_SimulatedProperty, &scratchKey);
  GrGpuResource* scratch = cache->findAndRefScratchResource(
      scratchKey, TestResource::kDefaultSize, GrResourceCache::ScratchFlags::kNone);
  ...
  // Get rid of c.
  cache->purgeAllUnlocked();
  scratch = cache->findAndRefScratchResource(
      scratchKey, TestResource::kDefaultSize, GrResourceCache::ScratchFlags::kNone);
  REPORTER_ASSERT(reporter, 0 == TestResource::NumAlive());
  REPORTER_ASSERT(reporter, 0 == cache->getResourceCount());
  REPORTER_ASSERT(reporter, 0 == cache->getResourceBytes());
...
static void test_partial_purge(skiatest::Reporter* reporter) {
  ...
  for (int testCase = 0; testCase < kEndTests_TestCase; testCase++) {
    ...
    TestResource* scratch1 =
        TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kA_SimulatedProperty, 13);
    TestResource* scratch2 =
        TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kB_SimulatedProperty, 14);
...
////////////////////////////////////////////////////////////////////////////////
static sk_sp<GrTexture> make_normal_texture(
    GrResourceProvider* provider, GrRenderable renderable, int width, int height, int sampleCnt) {
  GrSurfaceDesc desc;
  desc.fWidth = width;
  desc.fHeight = height;
  desc.fConfig = kRGBA_8888_GrPixelConfig;
  ...
  return provider->createTexture(
      desc, renderable, sampleCnt, SkBudgeted::kYes, GrProtected::kNo,
      GrResourceProvider::Flags::kNoPendingIO);
}

static sk_sp<GrTextureProxy> make_mipmap_proxy(
    GrProxyProvider* proxyProvider, const GrCaps* caps, GrRenderable renderable, int width,
    int height, int sampleCnt) {
  GrSurfaceDesc desc;
  desc.fWidth = width;
  desc.fHeight = height;
  desc.fConfig = kRGBA_8888_GrPixelConfig;
  ...
  return proxyProvider->createMipMapProxy(
      format, desc, renderable, sampleCnt, origin, SkBudgeted::kYes, GrProtected::kNo);
}
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GPUMemorySize, reporter, ctxInfo) {
  ...
  // Normal versions
  {
    ...
    size_t sampleCount =
        (size_t)context->priv().caps()->getRenderTargetSampleCount(4, kRGBA_8888_GrPixelConfig);
    if (sampleCount >= 4) {
      tex = make_normal_texture(resourceProvider, GrRenderable::kYes, kSize, kSize, sampleCount);
  ...
  if (caps->mipMapSupport()) {
    ...
    size_t sampleCount =
        (size_t)context->priv().caps()->getRenderTargetSampleCount(4, kRGBA_8888_GrPixelConfig);
    if (sampleCount >= 4) {
      proxy = make_mipmap_proxy(proxyProvider, caps, GrRenderable::kYes, kSize, kSize, sampleCount);
      size = proxy->gpuMemorySize();
      REPORTER_ASSERT(
          reporter,
          kSize * kSize * 4 + (kSize * kSize * 4) / 3 == size ||  // msaa4 failed
              kSize * kSize * 4 * sampleCount + (kSize * kSize * 4) / 3 ==
                  size ||  // auto-resolving
              kSize * kSize * 4 * (sampleCount + 1) + (kSize * kSize * 4) / 3 ==
                  size);  // explicit resolve buffer
    }
