...
static sk_sp<GrRenderTarget> create_RT_with_SB(
  ...
  sk_sp<GrTexture> tex(provider->createTexture(
      desc, format, GrRenderable::kYes, sampleCount, budgeted, GrProtected::kNo,
...
DEF_GPUTEST_FOR_CONTEXTS(
  ...
  int smallSampleCount = context->priv().caps()->getRenderTargetSampleCount(2, format);
...
static sk_sp<GrTexture> make_normal_texture(
  ...
  return provider->createTexture(
      desc, format, renderable, sampleCnt, SkBudgeted::kYes, GrProtected::kNo,
      GrResourceProvider::Flags::kNoPendingIO);
}

static sk_sp<GrTextureProxy> make_mipmap_proxy(
    GrContext* context, GrRenderable renderable, int width, int height, int sampleCnt) {
