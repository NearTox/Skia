...
static bool check_rect(
    GrDirectContext* dContext, SurfaceDrawContext* sdc, const SkIRect& rect, uint32_t expectedValue,
...
std::unique_ptr<SurfaceDrawContext> newSDC(GrRecordingContext* rContext, int w, int h) {
  return SurfaceDrawContext::Make(
      rContext, GrColorType::kRGBA_8888, nullptr, SkBackingFit::kExact, {w, h}, SkSurfaceProps());
}
