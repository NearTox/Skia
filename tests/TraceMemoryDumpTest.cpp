...
void ValidateMemoryDumps(
    skiatest::Reporter* reporter, GrContext* context, size_t size, bool isOwned) {
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(SkTraceMemoryDump_ownedGLTexture, reporter, ctxInfo) {
  ...
  auto texture =
      sk_make_sp<GrGLTexture>(gpu, SkBudgeted::kNo, desc, idDesc, GrMipMapsStatus::kNotAllocated);
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(SkTraceMemoryDump_unownedGLTexture, reporter, ctxInfo) {
  ...
  auto texture = GrGLTexture::MakeWrapped(
      gpu, desc, GrMipMapsStatus::kNotAllocated, idDesc, std::move(params), GrWrapCacheable::kNo,
      kRead_GrIOType);
