...
class TestSkTraceMemoryDump : public SkTraceMemoryDump {
 public:
  ...
  void dumpNumericValue(
      const char* dumpName, const char* valueName, const char* units, uint64_t value) override {
  ...
  void setMemoryBacking(
      const char* dumpName, const char* backingType, const char* backingObjectId) override {}
...
void ValidateMemoryDumps(
    skiatest::Reporter* reporter, GrDirectContext* dContext, size_t numDumpedObjects, size_t size,
    bool isOwned) {
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(SkTraceMemoryDump_ownedGLTexture, reporter, ctxInfo) {
  ...
  auto texture = sk_make_sp<GrGLTexture>(gpu, SkBudgeted::kNo, desc, GrMipmapStatus::kNotAllocated);
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(SkTraceMemoryDump_unownedGLTexture, reporter, ctxInfo) {
  ...
  auto texture = GrGLTexture::MakeWrapped(
      gpu, GrMipmapStatus::kNotAllocated, desc, std::move(params), GrWrapCacheable::kNo,
      kRead_GrIOType);
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(
    SkTraceMemoryDump_ownedGLTextureRenderTarget, reporter, ctxInfo) {
  auto dContext = ctxInfo.directContext();
  GrGLGpu* gpu = static_cast<GrGLGpu*>(dContext->priv().getGpu());
