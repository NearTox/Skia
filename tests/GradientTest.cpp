...
static void test_big_grad(skiatest::Reporter* reporter) {
  ...
  const SkScalar affine[] = {1.06608627e-06f, 4.26434525e-07f, 6.2855f,
                             2.6611f,         273.4393f,       244.0046f};
  SkMatrix matrix;
...
struct GradRec {
  ...
  void gradCheck(
      skiatest::Reporter* reporter, const sk_sp<SkShader>& shader, SkShader::GradientInfo* info,
    ...
    REPORTER_ASSERT(reporter, !memcmp(info->fColors, fColors, fColorCount * sizeof(SkColor)));
    REPORTER_ASSERT(reporter, !memcmp(info->fColorOffsets, fPos, fColorCount * sizeof(SkScalar)));
...
static void linear_gradproc(
    skiatest::Reporter* reporter, const GradRec& buildRec, const GradRec& checkRec) {
  sk_sp<SkShader> s(SkGradientShader::MakeLinear(
      buildRec.fPoint, buildRec.fColors, buildRec.fPos, buildRec.fColorCount, buildRec.fTileMode));
...
static void radial_gradproc(
    skiatest::Reporter* reporter, const GradRec& buildRec, const GradRec& checkRec) {
  sk_sp<SkShader> s(SkGradientShader::MakeRadial(
      buildRec.fPoint[0], buildRec.fRadius[0], buildRec.fColors, buildRec.fPos,
...
static void sweep_gradproc(
    skiatest::Reporter* reporter, const GradRec& buildRec, const GradRec& checkRec) {
  sk_sp<SkShader> s(SkGradientShader::MakeSweep(
      buildRec.fPoint[0].fX, buildRec.fPoint[0].fY, buildRec.fColors, buildRec.fPos,
...
static void conical_gradproc(
  ...
  sk_sp<SkShader> s(SkGradientShader::MakeTwoPointConical(
      ...
      buildRec.fColors, buildRec.fPos, buildRec.fColorCount, buildRec.fTileMode));
...
static void TestConstantGradient(skiatest::Reporter*) {
  const SkPoint pts[] = {{0, 0}, {SkIntToScalar(10), 0}};
...
static void TestGradientShaders(skiatest::Reporter* reporter) {
  ...
  static const SkPoint gPts[] = {{0, 0}, {SkIntToScalar(10), SkIntToScalar(20)}};
  ...
  static const GradProc gProcs[] = {
      none_gradproc,   color_gradproc, linear_gradproc,
      radial_gradproc, sweep_gradproc, conical_gradproc,
  };
...
static void test_two_point_conical_zero_radius(skiatest::Reporter* reporter) {
  ...
  p.setShader(SkGradientShader::MakeTwoPointConical(
      SkPoint::Make(2.5f, 2.5f), 0, SkPoint::Make(3.0f, 3.0f), 10, colors, nullptr,
...
static void test_unsorted_degenerate(skiatest::Reporter* r) {
  ...
  const SkColor4f colors[] = {
      {0.f, 0.f, 0.f, 0.f}, {0.00784314f, 0.f, 0.f, 0.0627451f}, {0.f, 0.00392157f, 0.f, 0.f}};
  ...
  sk_sp<SkShader> gradient =
      SkGradientShader::MakeLinear(points, colors, nullptr, positions, 3, SkTileMode::kMirror);
  ...
  GrColorInfo dstColorInfo(GrColorType::kRGBA_8888, kPremul_SkAlphaType, SkColorSpace::MakeSRGB());
...
static void test_linear_fuzzer(skiatest::Reporter*) {
  ...
  static const SkScalar gMatrix1[9] = {-2.75294113f,    6.40969056e-10f, 6.40969056e-10f,
                                       6.40969056e-10f, 6.40969056e-10f, -3.32810161e+24f,
                                       6.40969056e-10f, 6.40969056e-10f, 0};
  static const SkScalar gMatrix2[9] = {7.93481258e+17f, 6.40969056e-10f, 6.40969056e-10f,
                                       6.40969056e-10f, 6.40969056e-10f, 6.40969056e-10f,
                                       6.40969056e-10f, 6.40969056e-10f, 0.688235283f};
  static const SkScalar gMatrix3[9] = {1.89180674e+11f, 6.40969056e-10f, 6.40969056e-10f,
                                       6.40969056e-10f, 6.40969056e-10f, 6.40969056e-10f,
  ...
  for (const SkColorSpace* colorSpace : colorSpaces) {
    sk_sp<SkSurface> surface = SkSurface::MakeRaster(
        SkImageInfo::Make(100, 100, kN32_SkColorType, kPremul_SkAlphaType, sk_ref_sp(colorSpace)));
    ...
    for (const auto& config : gConfigs) {
      ...
      paint.setShader(SkGradientShader::MakeLinear(
          config.fPts, config.fColors, config.fPos, config.fCount, config.fTileMode, config.fFlags,
...
static void test_sweep_fuzzer(skiatest::Reporter*) {
  ...
  static const SkScalar gMatrix0[9] = {1.12116716e-13f, 0,
                                       8.50489682e+16f, 4.1917041e-41f,
                                       3.51369881e-23f, -2.54344271e-26f,
                                       9.61111907e+17f, -3.35263808e-29f,
                                       -1.35659403e+14f};
  static const struct {
    SkPoint fCenter;
    const SkColor* fColors;
    ...
    const SkScalar* fGlobalMatrix;
      ...
      {{0, 0}, gColors0, gPos0, SK_ARRAY_COUNT(gColors0), gMatrix0},
  };
  ...
  for (const auto& config : gConfigs) {
    paint.setShader(SkGradientShader::MakeSweep(
        config.fCenter.x(), config.fCenter.y(), config.fColors, config.fPos, config.fCount));
