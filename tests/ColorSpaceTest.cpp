...
static void test_space(
    skiatest::Reporter* r, SkColorSpace* space, const float red[], const float green[],
    const float blue[], bool expectSRGB = false) {
...
static void test_path(
    skiatest::Reporter* r, const char* path, const float red[], const float green[],
    const float blue[], bool expectSRGB = false) {
...
DEF_TEST(ColorSpaceParseICCProfiles, r) {
  ...
  test_path(r, "images/webp-color-profile-crash.webp", red, green, blue);
  test_path(r, "images/webp-color-profile-lossless.webp", red, green, blue);
  test_path(r, "images/webp-color-profile-lossy.webp", red, green, blue);
  test_path(r, "images/webp-color-profile-lossy-alpha.webp", red, green, blue);
}
...
static inline void check_primaries(
    skiatest::Reporter* r, const SkColorSpacePrimaries& primaries,
...
DEF_TEST(ColorSpace_Primaries, r) {
  ...
  bool result = skcms_PrimariesToXYZD50(
      0.64f, 0.33f, 0.30f, 0.60f, 0.15f, 0.06f, 0.3127f, 0.3290f, &srgbToXYZ);
  REPORTER_ASSERT(r, result);
...
DEF_TEST(ColorSpace_equiv, r) {
  ...
  sk_sp<SkColorSpace> x = SkColorSpace::MakeRGB(tf, gamut), y = SkColorSpace::MakeRGB(tf, gamut);
