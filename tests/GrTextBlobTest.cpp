...
DEF_TEST(GrBagOfBytesBasic, r) {
  const int k4K = 1 << 12;
  {
    ...
    GrBagOfBytes::PlatformMinimumSizeWithOverhead(std::numeric_limits<int>::max() - k4K - 1, 16);
...
class GrSTSubRunAllocator : private GrBagOfBytes::Storage<inlineSize>, public GrSubRunAllocator {
 public:
  explicit GrSTSubRunAllocator(
      int firstHeapAllocation = GrBagOfBytes::PlatformMinimumSizeWithOverhead(inlineSize, 1))
...
DEF_TEST(GrSubRunAllocator, r) {
  ...
  // Exercise the singly-link list of unique_ptrs use case
  {
    ...
    struct Node {
      Node(std::unique_ptr<Node, GrSubRunAllocator::Destroyer> next) : fNext{std::move(next)} {
        created++;
      }
      ~Node() { destroyed++; }
