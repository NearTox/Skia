...
SkBitmap rasterize_blob(
    SkTextBlob* blob, const SkPaint& paint, GrRecordingContext* rContext, const SkMatrix& matrix) {
  const SkImageInfo info = SkImageInfo::Make(500, 500, kN32_SkColorType, kPremul_SkAlphaType);
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrTextBlobMoveAround, reporter, ctxInfo) {
  ...
  auto rasterizeBlob = [&](SkTextBlob* blob, SkPoint origin, const SkMatrix& matrix) {
    SkPaint paint;
    const SkImageInfo info = SkImageInfo::Make(350, 80, kN32_SkColorType, kPremul_SkAlphaType);
...
DEF_TEST(BagOfBytesBasic, r) {
  const int k4K = 1 << 12;
  {
    ...
    BagOfBytes::PlatformMinimumSizeWithOverhead(std::numeric_limits<int>::max() - k4K - 1, 16);
...
class GrSTSubRunAllocator : private BagOfBytes::Storage<inlineSize>, public SubRunAllocator {
 public:
  explicit GrSTSubRunAllocator(
      int firstHeapAllocation = BagOfBytes::PlatformMinimumSizeWithOverhead(inlineSize, 1))
...
DEF_TEST(SubRunAllocator, r) {
  ...
  // Exercise the singly-link list of unique_ptrs use case
  {
    ...
    struct Node {
      Node(std::unique_ptr<Node, SubRunAllocator::Destroyer> next) : fNext{std::move(next)} {
        created++;
      }
      ~Node() { destroyed++; }
...
DEF_TEST(KeyEqualityOnPerspective, r) {
  ...
  auto key1 = std::get<1>(TextBlob::Key::Make(glyphRunList, paint, matrix1, strikeDevice));
  auto key2 = std::get<1>(TextBlob::Key::Make(glyphRunList, paint, matrix1, strikeDevice));
  auto key3 = std::get<1>(TextBlob::Key::Make(glyphRunList, paint, matrix2, strikeDevice));
