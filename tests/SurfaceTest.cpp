...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrContext_colorTypeSupportedAsSurface, reporter, ctxInfo) {
  ...
  for (int ct = 0; ct < kLastEnum_SkColorType; ++ct) {
    ...
    if (surf) {
      ...
      int allowedCnt =
          context->priv().caps()->getSampleCount(storedCnt, rtc->asSurfaceProxy()->config());
      REPORTER_ASSERT(
...
static sk_sp<SkSurface> create_gpu_surface_backend_texture(
  ...
  if (!create_backend_texture(ctx, outTexture, ii, GrMipMapped::kNo, color, GrRenderable::kYes)) {
    return nullptr;
  }
...
static void test_surface_clear(
    skiatest::Reporter* reporter, sk_sp<SkSurface> surface,
    std::function<sk_sp<GrSurfaceContext>(SkSurface*)> grSurfaceGetter, uint32_t expectedValue) {
  if (!surface) {
  ...
  std::function<sk_sp<GrSurfaceContext>(SkSurface*)> grSurfaceContextGetters[] = {
      ...
      [context](SkSurface* s) {
        ...
        return context->priv().makeWrappedSurfaceContext(
            std::move(proxy), gpuImage->refColorSpace());
      }};
...
static void test_surface_draw_partially(
  ...
  surface->getCanvas()->drawRect(SkRect::MakeWH(SkIntToScalar(kW), SkIntToScalar(kH) / 2), paint);
  ...
  for (bool useTexture : {false, true}) {
    ...
    if (useTexture) {
      ...
      if (!create_backend_texture(
              ctx, &backendTex, ii, GrMipMapped::kNo, SK_ColorRED, GrRenderable::kYes)) {
        continue;
      }
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ReplaceSurfaceBackendTexture, reporter, ctxInfo) {
  ...
  for (int sampleCnt : {1, 2}) {
    ...
    if (!create_backend_texture(
            context, &backendTexture1, ii, GrMipMapped::kNo, SK_ColorTRANSPARENT,
            GrRenderable::kYes)) {
      continue;
    }
    SkScopeExit delete1(
    ...
    if (!create_backend_texture(
            context, &backendTexture2, ii, GrMipMapped::kNo, SK_ColorTRANSPARENT,
            GrRenderable::kYes)) {
    ...
    if (!create_backend_texture(
            context, &backendTexture3, ii2, GrMipMapped::kNo, SK_ColorTRANSPARENT,
            GrRenderable::kYes)) {
