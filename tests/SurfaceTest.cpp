...
static sk_sp<SkSurface> create_surface(
    SkAlphaType at = kPremul_SkAlphaType, SkImageInfo* requestedInfo = nullptr) {
...
static sk_sp<SkSurface> create_direct_surface(
    SkAlphaType at = kPremul_SkAlphaType, SkImageInfo* requestedInfo = nullptr) {
  const SkImageInfo info = SkImageInfo::MakeN32(10, 10, at);
  if (requestedInfo) {
    *requestedInfo = info;
  ...
  return SkSurface::MakeRasterDirectReleaseProc(
      info, storage, rowBytes, release_direct_surface_storage, storage);
}
static sk_sp<SkSurface> create_gpu_surface(
    GrRecordingContext* rContext, SkAlphaType at = kPremul_SkAlphaType,
...
static sk_sp<SkSurface> create_gpu_scratch_surface(
    GrRecordingContext* rContext, SkAlphaType at = kPremul_SkAlphaType,
    SkImageInfo* requestedInfo = nullptr) {
  const SkImageInfo info = SkImageInfo::MakeN32(10, 10, at);
  if (requestedInfo) {
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SurfaceEmpty_Gpu, reporter, ctxInfo) {
  ...
  REPORTER_ASSERT(
      reporter,
      nullptr == SkSurface::MakeRenderTarget(ctxInfo.directContext(), SkBudgeted::kNo, info));
}
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrContext_colorTypeSupportedAsSurface, reporter, ctxInfo) {
  ...
  for (int ct = 0; ct < kLastEnum_SkColorType; ++ct) {
...

    {
      ...
      surf = SkSurface::MakeFromBackendTextureAsRenderTarget(
          context, backendTex, kTopLeft_GrSurfaceOrigin, 1, colorType, nullptr, nullptr);
      REPORTER_ASSERT(
          reporter, can == SkToBool(surf), "ct: %d, can: %d, surf: %d", colorType, can,
          SkToBool(surf));
    ...
    if (context->colorTypeSupportedAsSurface(colorType)) {
      ...
      auto surf = SkSurface::MakeRenderTarget(context, SkBudgeted::kYes, info, kSampleCnt, nullptr);
      REPORTER_ASSERT(
          reporter, can == SkToBool(surf), "ct: %d, can: %d, surf: %d", colorType, can,
          SkToBool(surf));
      ...
      REPORTER_ASSERT(
          reporter, can == SkToBool(surf), "colorTypeSupportedAsSurface:%d, surf:%d, ct:%d", can,
          SkToBool(surf), colorType);
      ...
      if (surf) {
        ...
        REPORTER_ASSERT(
            reporter, storedCnt == allowedCnt, "Should store an allowed sample count (%d vs %d)",
            allowedCnt, storedCnt);
      }
...

    {
      ...
      GrBackendRenderTarget backendRenderTarget =
          gpu->createTestingOnlyBackendRenderTarget(16, 16, SkColorTypeToGrColorType(colorType));
      bool can = context->colorTypeSupportedAsSurface(colorType);
      auto surf = SkSurface::MakeFromBackendRenderTarget(
          context, backendRenderTarget, kTopLeft_GrSurfaceOrigin, colorType, nullptr, nullptr);
      REPORTER_ASSERT(
          reporter, can == SkToBool(surf), "ct: %d, can: %d, surf: %d", colorType, can,
          SkToBool(surf));
      surf.reset();
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(GrContext_maxSurfaceSamplesForColorType, reporter, ctxInfo) {
  ...
  for (int ct = 0; ct < kLastEnum_SkColorType; ++ct) {
    ...
    SkScopeExit freeTex([&backendTex, context] { context->deleteBackendTexture(backendTex); });
    ...
    auto surf = SkSurface::MakeFromBackendTexture(
        context, backendTex, kTopLeft_GrSurfaceOrigin, max, colorType, nullptr, nullptr);
    REPORTER_ASSERT(reporter, surf);
    if (!surf) {
      continue;
    }
    int sampleCnt =
        ((SkSurface_Gpu*)(surf.get()))->getDevice()->accessRenderTargetContext()->numSamples();
    REPORTER_ASSERT(reporter, sampleCnt == max, "Exected: %d, actual: %d", max, sampleCnt);
  }
}

static void test_canvas_peek(
    skiatest::Reporter* reporter, sk_sp<SkSurface>& surface, const SkImageInfo& requestInfo,
...
static void test_snapshot_alphatype(
    skiatest::Reporter* reporter, const sk_sp<SkSurface>& surface, SkAlphaType expectedAlphaType) {
...
static void test_backend_rendertarget_access_copy_on_write(
  ...
  // If the access mode triggers CoW, then the backend objects should reflect it.
  REPORTER_ASSERT(
      reporter, GrBackendRenderTarget::TestingOnly_Equals(rt1, rt2) == (snap1 == snap2));
}
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SurfaceBackendHandleAccessIDs_Gpu, reporter, ctxInfo) {
  for (auto& surface_func : {&create_gpu_surface, &create_gpu_scratch_surface}) {
    {
      auto surface(surface_func(ctxInfo.directContext(), kPremul_SkAlphaType, nullptr));
      test_backend_unique_id<GrBackendTexture, &SkSurface::getBackendTexture>(
...
static void test_copy_on_write(skiatest::Reporter* reporter, SkSurface* surface) {
  ...
  const SkRect testRect =
      SkRect::MakeXYWH(SkIntToScalar(0), SkIntToScalar(0), SkIntToScalar(4), SkIntToScalar(5));
  SkPath testPath;
  testPath.addRect(
      SkRect::MakeXYWH(SkIntToScalar(0), SkIntToScalar(0), SkIntToScalar(2), SkIntToScalar(1)));
...
#define EXPECT_COPY_ON_WRITE(command)                          \
  ...
  EXPECT_COPY_ON_WRITE(
      drawPoints(SkCanvas::kPoints_PointMode, testPointCount, testPoints, testPaint))
...
DEF_TEST(SurfaceCopyOnWrite, reporter) { test_copy_on_write(reporter, create_surface().get()); }
...
static void test_crbug263329(
    skiatest::Reporter* reporter, SkSurface* surface1, SkSurface* surface2) {
...
static SkBudgeted is_budgeted(const sk_sp<SkSurface>& surf) {
  ...
  GrRenderTargetProxy* proxy =
      gsurf->getDevice()->accessRenderTargetContext()->asRenderTargetProxy();
...
static SkBudgeted is_budgeted(const sk_sp<SkImage> image) { return is_budgeted(image.get()); }
...
static void test_no_canvas1(
    skiatest::Reporter* reporter, SkSurface* surface, SkSurface::ContentChangeMode mode) {
  // Test passes by not asserting
  surface->notifyContentWillChange(mode);
}
static void test_no_canvas2(
    skiatest::Reporter* reporter, SkSurface* surface, SkSurface::ContentChangeMode mode) {
...
static sk_sp<SkSurface> create_gpu_surface_backend_texture(
    GrDirectContext* dContext, int sampleCnt, const SkColor4f& color,
  ...
  if (!CreateBackendTexture(
          dContext, outTexture, ii, color, GrMipmapped::kNo, GrRenderable::kYes)) {
    return nullptr;
  }

  sk_sp<SkSurface> surface = SkSurface::MakeFromBackendTexture(
      dContext, *outTexture, kTopLeft_GrSurfaceOrigin, sampleCnt, kRGBA_8888_SkColorType, nullptr,
      nullptr);
  if (!surface) {
...
static void test_surface_context_clear(
    skiatest::Reporter* reporter, GrDirectContext* dContext, GrSurfaceContext* surfaceContext,
  ...
  surfaceContext->readPixels(
      dContext, readback.info(), readback.writable_addr(), readback.rowBytes(), {0, 0});
  for (int y = 0; y < h; ++y) {
    for (int x = 0; x < w; ++x) {
      ...
      if (pixel != expectedValue) {
        ...
        ERRORF(
            reporter, "%s but read 0x%08x (instead of 0x%08x) at %x,%d", msg.c_str(), pixel,
            expectedValue, x, y);
        return;
      }
    }
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(SurfaceClear_Gpu, reporter, ctxInfo) {
  ...
  auto makeImageSurfaceContext = [dContext](SkSurface* surface) {
    ...
    return GrSurfaceContext::Make(
        dContext, *gpuImage->view(dContext), SkColorTypeToGrColorType(i->colorType()),
        kPremul_SkAlphaType, gpuImage->refColorSpace());
  };
  ...
  for (auto& surface_func : {&create_gpu_surface, &create_gpu_scratch_surface}) {
    ...
    for (auto& surfaceFunc : {&create_gpu_surface_backend_texture,
      ...
      auto imageSurfaceCtx = makeImageSurfaceContext(surface.get());
      test_surface_context_clear(reporter, dContext, imageSurfaceCtx.get(), kOrigColor.toSkColor());
...
static void test_surface_draw_partially(
  ...
  REPORTER_ASSERT(
      reporter,
      surface->readPixels(readback.info(), readback.writable_addr(), readback.rowBytes(), 0, 0));
  bool stop = false;

  SkPMColor origColorPM = SkPackARGB_as_RGBA(
      SkColorGetA(origColor), SkColorGetR(origColor), SkColorGetG(origColor),
      SkColorGetB(origColor));
  SkPMColor rectColorPM = SkPackARGB_as_RGBA(
      SkColorGetA(kRectColor), SkColorGetR(kRectColor), SkColorGetG(kRectColor),
...
struct ReleaseChecker {
  ...
  int fReleaseCount;
  static void Release(void* self) { static_cast<ReleaseChecker*>(self)->fReleaseCount++; }
};
  ...
  for (bool useTexture : {false, true}) {
    ...
    if (useTexture) {
      SkImageInfo ii = SkImageInfo::Make(
          kWidth, kHeight, SkColorType::kRGBA_8888_SkColorType, kPremul_SkAlphaType);
      if (!CreateBackendTexture(
              ctx, &backendTex, ii, SkColors::kRed, GrMipmapped::kNo, GrRenderable::kYes)) {
        continue;
      }

      surface = SkSurface::MakeFromBackendTexture(
          ctx, backendTex, texOrigin, 1, kRGBA_8888_SkColorType, nullptr, nullptr,
          ReleaseChecker::Release, &releaseChecker);
    } else {
      backendRT =
          gpu->createTestingOnlyBackendRenderTarget(kWidth, kHeight, GrColorType::kRGBA_8888);
      if (!backendRT.isValid()) {
        continue;
      }
      surface = SkSurface::MakeFromBackendRenderTarget(
          ctx, backendRT, texOrigin, kRGBA_8888_SkColorType, nullptr, nullptr,
          ReleaseChecker::Release, &releaseChecker);
    }
    if (!surface) {
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ReplaceSurfaceBackendTexture, reporter, ctxInfo) {
  ...
  for (int sampleCnt : {1, 2}) {
    ...
    if (!CreateBackendTexture(
            context, &backendTexture1, ii, SkColors::kTransparent, GrMipmapped::kNo,
            GrRenderable::kYes)) {
      continue;
    }
    SkScopeExit delete1(
    ...
    if (!CreateBackendTexture(
            context, &backendTexture2, ii, SkColors::kTransparent, GrMipmapped::kNo,
            GrRenderable::kYes)) {
    ...
    if (!CreateBackendTexture(
            context, &backendTexture3, ii2, SkColors::kTransparent, GrMipmapped::kNo,
            GrRenderable::kYes)) {
    ...
    auto surf = SkSurface::MakeFromBackendTexture(
        context, backendTexture1, kTopLeft_GrSurfaceOrigin, sampleCnt, kRGBA_8888_SkColorType,
        ii.refColorSpace(), nullptr);
    if (!surf) {
      continue;
    }
    ...
    for (int y = 0; y < ii.height() && !bad; ++y) {
      for (int x = 0; x < ii.width() && !bad; ++x) {
        ...
        if (found != expected) {
          bad = true;
          ERRORF(
              reporter, "Expected color 0x%08x, found color 0x%08x at %d, %d.", expected, found, x,
              y);
        }
      }
    }
    ...
    surf = SkSurface::MakeFromBackendTexture(
        context, backendTexture1, kBottomLeft_GrSurfaceOrigin, sampleCnt, kRGBA_8888_SkColorType,
        ii.refColorSpace(), nullptr);
    if (!surf) {
    ...
    REPORTER_ASSERT(
        reporter, !surf->replaceBackendTexture(backendTexture1, kTopLeft_GrSurfaceOrigin));
    ...
    REPORTER_ASSERT(
        reporter, !surf->replaceBackendTexture(backendTexture3, kTopLeft_GrSurfaceOrigin));
