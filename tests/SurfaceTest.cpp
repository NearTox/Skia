  ...
  for (int ct = 0; ct < kLastEnum_SkColorType; ++ct) {
...

    {
      ...
      surf = sk_gpu_test::MakeBackendTextureSurface(
          context, {kSize, kSize}, kTopLeft_GrSurfaceOrigin,
          /*sample cnt*/ 1, colorType);
      REPORTER_ASSERT(
          reporter, can == SkToBool(surf), "ct: %d, can: %d, surf: %d", colorType, can,
          SkToBool(surf));
    }
    ...
    if (context->colorTypeSupportedAsSurface(colorType)) {
      ...
      if (surf) {
        ...
        int allowedCnt = context->priv().caps()->getRenderTargetSampleCount(storedCnt, format);
        REPORTER_ASSERT(
    ...
    for (int sampleCnt : {1, 2}) {
      auto surf = sk_gpu_test::MakeBackendRenderTargetSurface(
          context, {16, 16}, kTopLeft_GrSurfaceOrigin, sampleCnt, colorType);
      bool can = context->colorTypeSupportedAsSurface(colorType) &&
      ...
      REPORTER_ASSERT(
          reporter, can == SkToBool(surf), "ct: %d, sc: %d, can: %d, surf: %d", colorType,
          sampleCnt, can, SkToBool(surf));
      if (surf) {
        auto rtc = ((SkSurface_Gpu*)(surf.get()))->getDevice()->accessRenderTargetContext();
        auto backendFormat = rtc->asSurfaceProxy()->backendFormat();
        int storedCnt = rtc->numSamples();
        int allowedCnt =
            context->priv().caps()->getRenderTargetSampleCount(storedCnt, backendFormat);
        REPORTER_ASSERT(
            reporter, storedCnt == allowedCnt, "Should store an allowed sample count (%d vs %d)",
            allowedCnt, storedCnt);
      }
...
static sk_sp<SkSurface> create_gpu_surface_backend_texture(
  ...
  auto surf = sk_gpu_test::MakeBackendTextureSurface(
      dContext, {kWidth, kHeight}, kTopLeft_GrSurfaceOrigin, sampleCnt, kRGBA_8888_SkColorType);
  if (!surf) {
    return nullptr;
  }
...
static sk_sp<SkSurface> create_gpu_surface_backend_render_target(
    GrDirectContext* dContext, int sampleCnt, const SkColor4f& color) {
  const int kWidth = 10;
  const int kHeight = 10;
  ...
  for (bool useTexture : {false, true}) {
    ...
    if (useTexture) {
      ...
      mbet = sk_gpu_test::ManagedBackendTexture::MakeFromInfo(
          ctx, ii, GrMipMapped::kNo, GrRenderable::kYes);
      if (!mbet) {
        continue;
      }
      ...
      surface = SkSurface::MakeFromBackendTexture(
          ...
          /*sample count*/ 1, kRGBA_8888_SkColorType,
          /*color space*/ nullptr,
          /*surface props*/ nullptr, sk_gpu_test::ManagedBackendTexture::ReleaseProc,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ReplaceSurfaceBackendTexture, reporter, ctxInfo) {
  ...
  for (int sampleCnt : {1, 2}) {
    ...
    auto surf = SkSurface::MakeFromBackendTexture(
        context, mbet1->texture(), kTopLeft_GrSurfaceOrigin, sampleCnt, kRGBA_8888_SkColorType,
    ...
    surf = SkSurface::MakeFromBackendTexture(
        context, mbet1->texture(), kBottomLeft_GrSurfaceOrigin, sampleCnt, kRGBA_8888_SkColorType,
        ii.refColorSpace(), nullptr);
    if (!surf) {
    ...
    REPORTER_ASSERT(
        reporter, !surf->replaceBackendTexture(mbet1->texture(), kTopLeft_GrSurfaceOrigin));
    ...
    REPORTER_ASSERT(
        reporter, !surf->replaceBackendTexture(mbet3->texture(), kTopLeft_GrSurfaceOrigin));
