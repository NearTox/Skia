...
static sk_sp<SkSurface> create_gpu_surface(
    GrContext* context, SkAlphaType at = kPremul_SkAlphaType,
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SurfaceEmpty_Gpu, reporter, ctxInfo) {
  const SkImageInfo info = SkImageInfo::Make(0, 0, kN32_SkColorType, kPremul_SkAlphaType);
  REPORTER_ASSERT(
      reporter, nullptr == SkSurface::MakeRenderTarget(ctxInfo.grContext(), SkBudgeted::kNo, info));
}
...
static void test_copy_on_write(skiatest::Reporter* reporter, SkSurface* surface) {
  ...
  const SkPoint testPoints[3] = {{SkIntToScalar(0), SkIntToScalar(0)},
...
DEF_TEST(SurfaceNoCanvas, reporter) {
  SkSurface::ContentChangeMode modes[] = {SkSurface::kDiscard_ContentChangeMode,
...
static sk_sp<SkSurface> create_gpu_surface_backend_texture(
  ...
  if (!create_backend_texture(ctx, outTexture, ii, color, GrMipMapped::kNo, GrRenderable::kYes)) {
    return nullptr;
  }

  sk_sp<SkSurface> surface = SkSurface::MakeFromBackendTexture(
      ctx, *outTexture, kTopLeft_GrSurfaceOrigin, sampleCnt, kRGBA_8888_SkColorType, nullptr,
      nullptr);
  if (!surface) {
...
static void test_surface_context_clear(
    skiatest::Reporter* reporter, GrSurfaceContext* surfaceContext, uint32_t expectedValue) {
  ...
  surfaceContext->readPixels(
      readback.info(), readback.writable_addr(), readback.rowBytes(), {0, 0});
  for (int y = 0; y < h; ++y) {
    for (int x = 0; x < w; ++x) {
