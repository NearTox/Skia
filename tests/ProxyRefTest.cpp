...
static sk_sp<GrTextureProxy> make_deferred(GrRecordingContext* rContext) {
  ...
  return proxyProvider->createProxy(
      format, {kWidthHeight, kWidthHeight}, GrRenderable::kYes, 1, GrMipmapped::kNo,
      SkBackingFit::kApprox, SkBudgeted::kYes, GrProtected::kNo, /*label=*/"ProxyRefTest");
}
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(ProxyRefTest, reporter, ctxInfo) {
  ...
  for (auto make : {make_deferred, make_wrapped}) {
    // An extra ref
    {
      ...
      if (proxy) {
        sk_sp<GrTextureProxy> extraRef(
            proxy);  // NOLINT(performance-unnecessary-copy-initialization)
    ...
    // Continue using (reffing) proxy after instantiation
    {
      sk_sp<GrTextureProxy> proxy((*make)(dContext));
      if (proxy) {
        sk_sp<GrTextureProxy> firstExtraRef(
            proxy);  // NOLINT(performance-unnecessary-copy-initialization)
        ...
        sk_sp<GrTextureProxy> secondExtraRef(
            proxy);  // NOLINT(performance-unnecessary-copy-initialization)
        CheckSingleThreadedProxyRefs(reporter, proxy.get(), 3, 1);
      }
      CheckSingleThreadedProxyRefs(reporter, proxy.get(), 1, 1);
