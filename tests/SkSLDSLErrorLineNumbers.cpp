...
class ExpectErrorLineNumber : public SkSL::ErrorReporter {
 public:
  ExpectErrorLineNumber(skiatest::Reporter* reporter, const char* msg, int line)
      : fMsg(msg), fLine(line), fReporter(reporter), fOldReporter(&GetErrorReporter()) {
  ...
  void handleError(skstd::string_view msg, SkSL::PositionInfo pos) override {
    ...
    REPORTER_ASSERT(
        fReporter, pos.line() == fLine, "Line number mismatch: expected %d, but received %d\n",
        fLine, pos.line());
...
DEF_GPUTEST_FOR_MOCK_CONTEXT(DSLErrorLineNumbers, r, ctxInfo) {
  ...
  {
    ExpectErrorLineNumber error(
        r, "type mismatch: '+' cannot operate on 'float', 'bool'", __LINE__ + 1);
...

  {
    ...
    ExpectErrorLineNumber error(
        r, "type mismatch: '=' cannot operate on 'bool', 'float'", __LINE__ + 1);
    (a = 5.0f).release();
  }
